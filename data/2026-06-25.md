...
TL;DR: 研究团队开发了一个基于计算机视觉的自动化超车事件检测系统，使用单辆自行车安装的摄像头（无需多传感器或相机标定），通过RT-DETR目标检测和ByteTrack多目标跟踪结合三阶段几何验证模块，从城市道路视频中自动提取超车事件。该系统在315个手动标注事件中达到97.8%召回率且零误报，并能在车辆通过前平均2.44秒识别超车意图，33.3%的通过距离低于5英尺安全阈值。


<details>
  <summary>Details</summary>
Motivation: instrumented bicycle研究产生了车辆超车行为的直接现场证据，但从连续后视视频中提取超车事件仍依赖于手动逐帧标注。这种瓶颈限制了样本量，制约了自然主义骑行安全研究的发展。

Method: 提出了一种几何信息化的计算机视觉 pipeline，结合RT-DETR目标检测和ByteTrack多目标跟踪，通过三阶段几何验证模块（方位角趋势、表观尺寸增长和空间确认）实现超车事件自动检测，无需多传感器配置或显式相机标定。

Result: 在安娜堡315个手动标注的真实世界超车事件上验证，pipeline达到97.8%召回率，零误报；系统可在车辆通过前平均2.44秒识别超车意图，84.1%的事件超过1.5秒人类反应时间阈值；96个事件的横向通过距离测量显示33.3%的通过低于5英尺阈值；无标定横向距离估计方法的平均绝对误差为13-14厘米。

Conclusion: 该系统消除了instrumented bicycle研究的主要标注瓶颈，为在更大数据集和多样化城市环境中进行车辆-自行车交互分析提供了可扩展的基础，展示了主动骑行者警告的可行性。
...
TL;DR: 提出Sol Video Inference Engine，一个用于视频扩散模型的代理化、本地化、免训练的加速框架，组织缓存、稀疏注意力、标记剪枝、量化、核融合5种技术为实例特定优化。


<details>
  <summary>Details</summary>
Motivation: 现代视频扩散模型通过扩展实现更高生成质量，但也增加推理成本；最有效加速策略高度实例特定（模型、硬件、推理配置组合），手动性能工程代价高。

Method: 将5种广泛适用技术组织成代理加速栈：并行技能优化各技术实现、代理集成器组合成全局加速栈、人类验证器提供生成质量反馈。在3个视频模型(Cosmos3-Super 64B、LTX-2.3 22B、SANA-Video 2B)上实例化。

Result: 全栈实现超过2倍端到端加速，同时保持近无损VBench质量。

Conclusion: 代理框架对视频扩散加速有效，只需少量人力即可实现高效加速。
...
TL;DR: 本文提出PV-TAM方法，通过提示侧语义和过滤器来评估大型视觉语言模型中视觉与语言的一致性，解决了解码漂移和模态边界标记带来的注意力失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用答案侧令牌的注意力分布评估视觉-语言一致性，但最高注意力区域与目标语义令牌不一致。原因包括：1)解码漂移导致先前生成的答案令牌语言先验累积并与视觉注意力不匹配；2)模态边界标记等结构令牌可能覆盖整个上下文并产生与目标无关的高注意力。

Method: 提出提示-视觉令牌激活图(PV-TAM)：1)采用提示侧语义而非答案侧令牌；2)加入过滤器消除模态边界标记引起的系统性偏差；3)利用注意力的峰值分布而非传统仅评估掩码重叠来衡量提示与视觉区域的对齐。

Result: 在各种数据集上，PV-TAM始终比答案侧基线方法更有效地改善了基于注意力和IoU风格的定位指标。

Conclusion: PV-TAM能够避免传统方法的注意力失真问题，为大型视觉语言模型提供更准确的视觉-语言一致性评估。
...
TL;DR: 提出一种从空间域到频谱域的特征处理范式转变，通过频率引导特征表示框架（Frequency-Guided Feature Representation）和小目标检测器DERNet，在显著减少参数的情况下提升小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 小目标检测受限于特征稀缺性，空间域检测器会不可避免地丢弃关键的高频细节，而在空间域中恢复这些细节通常需要计算昂贵的架构上采样，反而放大背景噪声。

Method: 提出频率引导特征表示框架（Frequency-Guided Feature Representation），包含统一的分解-增强-重建（DER）算子，由三个轻量即插即用模块组成：Wavelet-Difference Gate（WDG）、Log-Gabor Enhancer（LGE）和Frequency-Driven Head（FDHead），分别作用于骨干网络、颈部和头部，以注入频率感知调制。

Result: 在VisDrone2019、UAVDT、TinyPerson、DOTAv1等多域基准数据集上验证，DERNet系列模型在相同规模下优于YOLOv11，且参数量仅为其1/6。

Conclusion: 通过频谱域特征处理替代传统空间域方法，能够以更少的参数实现更优的小目标检测性能，为小目标检测提供了一种有效的范式转变思路。
...
TL;DR: HANCLIP通过结合双曲几何和角度三元组目标重构VLMs的嵌入空间，使模型能够明确编码"图像不是什么"从而增强对否定表达的敏感性同时保持原有性能。


<details>
  <summary>Details</summary>
Motivation: 当前VLMs对否定表达表现出脆弱性模型依赖浅层的词语共现容易受到误导性或不相关文本线索的干扰直接微调否定数据又会干扰先前获取的知识导致标准基准性能下降。

Method: HANCLIP在20000个图像文本四元组上进行训练结合双曲 formulation建模层次语义关系和不对称性并使用角度三元组目标驱动否定描述与其对应肯定描述之间的系统性分离。

Result: 在NegBench否定基准上获得一致性能提升同时在标准分类和图像文本检索基准上保持竞争力或有所改善。该框架可插入CLIP LongCLIP SmartCLIP和HiMo-CLIP无需大规模重新训练。

Conclusion: 精心设计的几何目标可以仅使用适度的额外数据就显著扩展现有VLMs的推理能力证明双曲几何和角度分离的组合是处理否定表达的有效方法。
...
TL;DR: 提出HeRA方法,通过在单个注意力头级别进行跨模态对齐来改进多模态大语言模型,该方法基于Platonic表征假设,保留表征的拓扑结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常对齐语言主干网络的固定层,忽略了Transformer模型的细粒度结构。该工作旨在在注意力头级别进行细粒度的跨模态对齐,以更好地保持表征的局部邻域关系。

Method: 基于互K近邻(MKNN)对齐度量,引入对比目标作为匹配局部结构的可微代理,在多模态训练期间对LLM中特定注意力头应用该目标,并根据MKNN度量选择对齐分数最高的注意力头。

Result: 在多个MLLM和18个基准测试上的广泛评估表明,HeRA始终改善挑战性视觉中心任务的性能,并能有效防止视觉幻觉,通过自然抑制对语言先验的过度依赖。

Conclusion: 反直觉地发现,对齐最不对齐的注意力头能获得最大收益。该方法为改进多模态大语言模型提供了一种新的有效正则化方法。
...
TL;DR: REALM是首个针对物理世界视觉语言模型(VLMs)的统一红队测试基准,整合12种红队方法、3种防御策略和13个VLM模型,解决现有评估方法分散化的问题,实现不同攻击方法在统一协议下的公平比较。


<details>
  <summary>Details</summary>
Motivation: 现有VLM红队评估在不同数据集、指标和威胁模型间碎片化,导致难以直接比较不同攻击方法的优劣;现有以聊天机器人为中心的基准主要关注越狱和内容安全,缺乏对物理世界功能故障的系统性覆盖。

Method: REALM引入智能目标生成管道,针对每个场景构建共享的、场景特定的物理接地攻击目标,在实际的黑盒威胁模型下使用统一的数据集和指标,对12种红队方法、3种模型无关防御和13个VLM进行系统评估。

Result: 文本和字体注入攻击导致最多失败;多模态协同优化产生最强的视觉扰动迁移;单次攻击成本更低且接近迭代方法效果;模型规模 alone 不能带来对抗鲁棒性。

Conclusion: REALM为物理世界VLMs的安全评估提供了标准化框架,揭示了当前VLM在物理场景中的脆弱性,为构建更安全的 embodied intelligence 系统奠定基础。
...
TL;DR: 提出多教师提示蒸馏方法TheProfessor，通过结合域微调ViT-L/14教师和零样本EVA-CLIP-L/14教师的知识，在4个基础到新数据集上提升性能。


<details>
  <summary>Details</summary>
Motivation: 单教师提示蒸馏PromptKD仅使用一个教师模型，文章认为多教师蒸馏可提供更丰富的监督信号，特别是在领域偏移情况下。

Method: 构建两教师集成：域微调PromptSRC ViT-L/14教师 + 零样本EVA-CLIP-L/14教师（logits预计算）。测试等概率集成和置信度加权集成两种融合策略。

Result: 置信度加权集成将平均HM从87.52提升至89.28（+1.77点），等概率集成提升至88.88（+1.37点）。在EuroSAT领域偏移数据集上提升最大（+5.78 HM）。

Conclusion: 多教师提示蒸馏在第二教师提供互补监督时最有效，尤其在领域偏移场景下增益显著。
...
TL;DR: 提出了一种名为Forensic Knowledge Graphs (FKGs)的新型统一框架，用于图像伪造检测和可解释性认证。该框架整合了取证证据提取、结构化推理和人类可解释的解释。论文还提出了FKG-50K数据集，包含50,000个真实伪造图像。


<details>
  <summary>Details</summary>
Motivation: 生成式AI使得图像伪造高度逼真，现有检测器缺乏可解释性，而视觉语言模型虽能提供解释但无法利用取证痕迹进行可靠检测。需要一个既准确又可解释的图像伪造检测系统。

Method: 提出FKG结构编码取证痕迹及其因果依赖关系，链接到场景内容。引入取证认证网络和迭代上下文精炼策略(Iterative Context Refinement)来引导视觉语言模型产生忠实、可落地的解释。构建了包含50,000个真实伪造图像及对应真值FKG的数据集FKG-50K。

Result: FKG在检测、伪造识别和定位以及取证理由方面均优于现有取证检测器和视觉语言模型。

Conclusion: FKG是一个有效的图像伪造检测框架，能够同时实现高精度检测和人类可理解的理由解释，为可信的图像认证系统提供了新的研究方向。
...
TL;DR: 本文提出DivRL框架，通过"探索-抑制"策略和门控优化方法，解决图像生成中的"身份-多样性悖论"，在保持身份一致性的同时提升结构多样性。


<details>
  <summary>Details</summary>
Motivation: 主题驱动图像生成中存在"身份-多样性悖论"：强身份保留通常导致输出单一、缺乏多样性。

Method: 引入负自相似性度量(nSSM)量化结构多样性，视觉语义匹配(VSM)评估身份一致性；采用"探索-抑制"策略，将VSM作为门控约束，使用二次铰链损失惩罚违规样本，实现身份保留从竞争目标到可行性约束的转换。

Result: 方法有效推动模型生成一致且多样的图像，在维持相当身份一致性的同时提升结构多样性。

Conclusion: 门控优化公式使nSSM和VSM能够联合改进，为身份-多样性平衡问题提供了有效解决方案。
...
TL;DR: 针对文本到图像模型中token序列缺乏语义结构的问题，提出Token-to-Token对齐框架，通过结构对齐和嵌入级对齐建立跨提示词的语义对应关系，使线性插值能够产生平滑的语义过渡。


<details>
  <summary>Details</summary>
Motivation: 图像生成中token序列缺乏一致的语义结构：语义相似的图像可能对应不同的词序，相似的token序列可能编码不同语义，导致难以进行平滑过渡、图像混合和连续编辑控制。

Method: 两阶段方法：(1)结构对齐将提示词重述为共享结构形式，使语义对应概念映射到一致位置；(2)嵌入级对齐基于语义相似性匹配跨提示词的token表示。

Result: 文本嵌入空间隐含编码连续语义结构，一旦正确对齐即可访问；简单线性插值变为有意义操作，能产生平滑连贯语义过渡，实现图像混合和连续编辑等应用。

Conclusion: 语义控制可通过组织现有表示实现，无需修改生成模型；文本嵌入空间本身具有连续语义结构，只是之前未正确对齐。
...
TL;DR: 本文提出DriveStack-VLA框架，通过双重视觉建模和自批判模块增强VLA驾驶模型的空间 grounding 能力，在NAVSim和Bench2Drive基准上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA驾驶模型缺乏空间智能：策略主要基于透视图像token和语言先验，缺少精确运动规划所需的度量几何、鸟瞰视角场景结构和安全关键感知线索的关注，导致模型在弱视觉几何建模和专家演示的感知覆盖方面存在脆弱性。

Method: 1) 双重视觉建模：注入BEV表示到LLM解码器（DeepStack风格连接）+ Render-Teacher Alignment对齐真实图像与光栅化图像的感知焦点；2) 头部自批判模块：对采样轨迹排序并有条件优化最佳轨迹。

Result: NAVSIMv1: 91.6 PDMS；NAVSIMv2: 91.0 EPDMS（启用人类惩罚过滤器）；Bench2Drive闭环：79.49驾驶分数，56.36%成功率。

Conclusion: DriveStack-VLA通过空间 grounding 增强和多模态轨迹选择改进，显著提升了VLA驾驶模型的空间理解能力和安全关键场景的性能表现。
...
TL;DR: 本文介绍了EPEdit（高效照片编辑器），一个基于Stable Diffusion零-shot图像编辑算法的图像编辑应用程序，无需额外微调即可支持图像生成、对象替换、对象删除、背景修改、姿态调整、区域编辑和主题集合设计等功能。


<details>
  <summary>Details</summary>
Motivation: 传统图像编辑工具（如Photoshop）需要专业知识；现有AI解决方案（如Stable Diffusion）需要大量 retraining 和微调，成本高昂。

Method: EPEdit将强大的后端框架与用户友好的前端界面相结合，利用基于Stable Diffusion的零-shot图像编辑算法，支持文本命令和区域标记交互。

Result: 用户评估表明，EPEdit在图像编辑、主题设计和整体系统性能方面均优于现有解决方案。

Conclusion: EPEdit是一个用户友好、成本效益高的综合图像编辑解决方案。
...
TL;DR: 该论文提出VisChronos框架，利用大语言模型和密集captioning模型从单张图像中识别并描述真实历史事件，自动生成详细、上下文感知的事件描述，并构建了EventCap数据集以提升模型对复杂事件的理解能力。


<details>
  <summary>Details</summary>
Motivation: 传统图像caption方法难以捕捉上下文叙事，存在语义gap。该研究旨在弥合视觉内容与自然语言理解之间的鸿沟，利用历史事件作为知识源来增强生成caption的描述质量和上下文相关性。

Method: 提出VisChronos框架，融合大语言模型(LLM)和密集captioning模型，从单张输入图像中自动识别并描述真实世界的事件，构建了EventCap数据集。

Result: 框架能够生成准确、连贯且聚焦事件的描述，用户研究验证了该方案的有效性，为以事件为中心的图像理解研究提供了新途径。

Conclusion: VisChronos框架成功解决了传统方法在捕获上下文叙事方面的局限性，为事件-centric图像理解的未来研究奠定了基础。
...
TL;DR: 本论文研究食物图像的食材级语义分割问题，在FoodSeg103数据集上对两种SegFormer模型（BiT-B0和BiT-B1）进行微调评估。实验表明较大的BiT-B1模型在2135张测试图像上取得更优性能，像素准确率达0.7929，平均IoU达0.3204，比B0模型提升6.68个百分点的绝对增益。


<details>
  <summary>Details</summary>
Motivation: 食物图像通常包含多种可见食材，为整张图像分配一个菜品标签会掩盖重要的视觉结构信息。现有方法缺乏对食材级别语义的理解，因此需要开展食材级语义分割研究，以更精细地理解食物图像内容。

Method: 在受控实验环境下微调两种SegFormer模型变体：较小的BiT-B0作为基线模型，较大的BiT-B1作为最终模型。两个模型均使用ImageNet预训练的MiT骨干网络，并重新初始化104类输出层。

Result: 在FoodSeg103测试集（2135张图像）上，B0模型达到0.7709像素准确率和0.2521平均IoU；B1模型达到0.7929像素准确率和0.3204平均IoU。B1在所有测试指标上均优于B0，平均IoU绝对增益达+0.0683。

Conclusion: 系统还能将预测掩膜转换为可见食材面积百分比，提供简单的视觉成分摘要。这一摘要可作为营养意识提示的初步参考，为详细食物追踪提供视觉替代方案，但并非热量、宏量营养素、食物质量、体积、密度或真实份量的直接估算。
...
TL;DR: 本文提出ObsGraph，一个观察中心化的层次化场景图，统一场景表示、检索和探索，用于机器人在复杂环境中的具身推理与探索任务。


<details>
  <summary>Details</summary>
Motivation: 机器人在复杂和陌生环境中完成任务需要通过探索识别和获取必要的信息，但现有方法缺乏统一的场景表示、检索和探索框架。

Method: 提出ObsGraph层次化场景图，包含room-view-object三层结构（房间提供粗粒度语义锚点、视图保留上下文对象共视性、对象存储细粒度细节），在此基础上进行粗到细的层次化检索，并利用检索结果指导探索候选空间的结构化。

Result: 在具身推理和探索基准测试中取得了更高的成功率和效率。

Conclusion: 结构化场景表示和基于证据缺口的目标性信息收集方法具有显著优势。
...
TL;DR: 本文提出了织物图像去摩尔纹的第一个综合基准测试。针对织物摩尔纹由于纺织品的宽带和半周期特性而难以处理的问题，作者开发了一个物理驱动的合成框架，构建了包含16,050对多分辨率织物图像的大规模数据集，并设计了一个基线模型，在基准数据集上展示了良好的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 织物摩尔纹是由细纺织图案与相机传感器网格之间的相互作用引起的采样混叠 artifact，严重降低图像质量。与屏幕摩尔纹不同，织物摩尔纹由于纺织品的宽带和半周期特性本质上更具挑战性。现有的屏幕摩尔纹数据集训练的模型泛化能力差，且缺乏标准化的基准数据集。

Method: 开发了一个物理驱动的合成框架来生成像素对齐的成对图像，构建了包含16,050对多分辨率可控混叠严重程度的织物图像数据集，并设计了一个定制基线模型。

Result: 构建了第一个大规模的织物去摩尔纹基准数据集，包含16,050对多分辨率织物图像，基线模型在该基准数据集上取得了良好的性能并展示了较强的泛化能力。

Conclusion: 提供了一个标准化的平台来推动织物图像去摩尔纹研究，为该领域后续研究奠定了基础。
...
TL;DR: 提出EMRFormer，一种用于自动调制识别(AMR)的脉冲神经网络(SNN)架构，在资源受限设备上实现精度与能效的最佳平衡


<details>
  <summary>Details</summary>
Motivation: 深度学习方法在AMR任务中虽能实现高精度，但计算成本高，难以在精度和功耗之间取得平衡，限制了其在资源受限平台上的应用

Method: 采用自适应脉冲编码器和整数泄漏积分发放神经元，集成了脉冲可分离卷积神经网络的脉冲驱动Transformer，用于从原始IQ波形中提取多尺度时序特征

Result: 在多个主流数据集上达到最先进的精度，在低信噪比环境下保持优异性能，理论能耗降低超过90%，在KA200神经形态芯片上功耗相比3090 GPU或Orin NX降低5倍

Conclusion: 为资源受限设备上的AMR任务提供了一条有前景的实现路径
...
TL;DR: 提出PNAFusion方法，解决多光谱目标检测中的跨模态特征对齐问题，通过像素-邻域可变形交叉注意力模块避免全局注意力的二次复杂度问题，在FLIR、M3FD、DroneVehicle数据集上分别达到84.2、90.5、85.5 mAP@0.5


<details>
  <summary>Details</summary>
Motivation: 全局交叉注意力虽然建模能力强，但复杂度相对于特征尺寸是二次的，难以部署在资源受限平台上；同时可见光与热图像的对齐问题通常集中在局部邻域，且跨模态语义对应遵循固定感受野无法很好建模的非线性空间映射

Method: 提出渐进式像素-邻域可变形交叉注意力PNAFusion，包含两个核心模块：1)像素-邻域交叉注意力(PNCA)模块，避免冗余的全局特征匹配并抑制背景噪声；2)自适应可变形对齐(ADA)模块，通过学习的像素级偏移捕捉非线性空间对应；通过迭代反馈机制逐步细化跨模态特征对齐

Result: 在YOLOv5检测器下，FLIR、M3FD、DroneVehicle分别达到84.2、90.5、85.5 mAP@0.5；迁移到Co-DETR后在FLIR和M3FD上分别达到86.8和90.8 mAP@0.5；效率方面比ICAFusion减少33% GPU内存，FLOPs从194.8G降至156.4G

Conclusion: 通过引入局部空间先验和可变形对齐机制，有效降低了计算复杂度同时保持了高精度，为资源受限平台上的多光谱目标检测提供了可行方案
...
TL;DR: 本文提出了一种通用的图像聚类框架（Guideline-Driven Image Clustering Agent），通过文本指导原则统一处理不同类型的图像聚类任务，包括从通用到细粒度分类、从全局到局部标准、从平衡到长尾分布等各种场景。


<details>
  <summary>Details</summary>
Motivation: 图像聚类在不同场景下存在根本性差异，现有方法难以跨场景统一处理。传统方法通常针对特定聚类场景设计，缺乏通用性。

Method: 提出两种核心技术：1) 生成概念代理建模（Generative Concept Proxy Modeling）- 通过概念代理提取生成符合指导原则的嵌入表示，无需针对特定任务训练；2) 基于最小生成树的LLM遍历（LLM Traversal）- 对需要自动发现聚类的场景，使用LLM进行复杂语义判断。

Result: 该方法在多种聚类任务场景中一致优于专门的聚类方法，展现了良好的跨场景泛化能力。

Conclusion: 通过文本指导原则和生成概念代理建模，首次实现了能够统一处理不同聚类场景的通用图像聚类框架，为多场景图像聚类提供了新的研究方向。
...
TL;DR: 研究相机传感器硬件优化对自动驾驶感知的影响，发现学习2x2 CFA权重是最有效的策略。


<details>
  <summary>Details</summary>
Motivation: 探索在模型 scaling 之外，如何通过优化相机传感器本身来提升自动驾驶感知性能。

Method: 使用可微分的RAW-to-task管道，分解不同传感器自由度对密集预测的影响，包括CFA权重、PSF（光学）和噪声的协同优化。

Result: 学习CFA权重带来显著提升（KITTI-360 +0.017 mIoU，ACDC +0.023 mIoU），而PSF协同设计反而负面（KITTI-360 -0.020 mIoU）。噪声优化效果有限，扩大CFA tile到2x2以上反而有害。

Conclusion: 提出简单方案：学习2x2 CFA权重并保持恒等PSF，该干预位于传感器上游，所得收益与下游模型无关，在雾、夜、雨、雪等恶劣条件下同样有效。
...
TL;DR: 本文提出DramaDirector框架，用于解决短剧（short drama）的自动生成问题。该框架通过引入几何 grounding 技术，让规划器从真实短剧镜头库中借用电影摄影几何信息（深度和姿态），将全局情节和局部上下文转换为视觉 grounded 的多镜头视频。


<details>
  <summary>Details</summary>
Motivation: 短剧具有快速镜头节奏、对话驱动的焦点转换和严格的电影摄影 grounding 要求，现有基于提示或文本的视频生成 pipeline 难以满足这些挑战。需要一种能够将情节（plot）转换为视觉 grounded 多镜头视频的方法。

Method: 提出 DramaDirector 框架，包含以下核心组件：1）几何 grounding：利用真实短剧镜头库（按深度和姿态索引）来借用电影摄影几何；2）将每个镜头解耦为静态视觉条件和动态叙事条件；3）使用 schema-constrained SFT 和 GRPO 在学习到的文本-视觉对齐奖励下训练规划器；4）检索深度-姿态参考来引导首帧生成和图像到视频合成。同时构建 DramaBoard 基准，包含 35 部真人短剧、2.8K 集和 81K 镜头，包含结构化 storyboard 和多维评估协议。

Result: 实验表明 DramaDirector 在忠实度（faithfulness）、一致性（consistency）和可控性（controllability）方面优于代表性的多智能体和视频生成基线。

Conclusion: 本文工作首次系统性地研究了情节到短剧的生成问题，通过几何 grounding 和学习到的对齐奖励实现了高质量的短剧视频生成，为短剧自动化创作提供了新的研究方向。
...
TL;DR: 本文在Gut-VLM数据集上基准测试了9种幻觉检测方法，涵盖5种视觉语言模型。ReXTrust白盒方法取得最高AUC 93.0，显著优于其他方法。白盒方法平均获得19.5 AUC优势。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在临床实践中容易产生幻觉，阻碍安全部署。现有方法多在放射学基准上评估，胃肠道内镜领域仍属空白。需要针对GI诊断VQA数据集开发有效的幻觉检测方法。

Method: 在Gut-VLM数据集（4,392个测试VQA对）上基准测试9种幻觉检测方法，涵盖三类：黑盒方法（RadFlag、SelfCheckGPT-NLI）、灰盒方法（AvgProb、AvgEnt、MaxProb、MaxEnt、Semantic Entropy、VASE）和白盒方法（ReXTrust），评估于5种VLM模型。

Result: ReXTrust白盒方法在所有5个模型上获得最高AUC，在MedGemma-4B上达到峰值93.0 AUC，较最强替代方法有统计学显著优势（配对置换检验p<0.001）。白盒方法平均获得19.5 AUC优势（范围9.5-33.5）。非白盒方法中，token级灰盒统计（MaxEnt、MaxProb）最强。发现"自信confabulation"是系统性失败模式。

Conclusion: 白盒方法ReXTrust在GI内镜VQA幻觉检测中表现最优，显著优于黑盒和灰盒方法。白盒方法的隐藏状态访问提供一致性优势。自信confabulation（高样本间一致性或高token级概率的幻觉）是一致性方法和不确定性方法的系统性失败模式。未来研究需针对此 failure mode 开发应对策略。
...
TL;DR: PreciseDoc是一个用于文档图像中文本元素精确视觉定位的大型多模态模型，可进一步优化用于文档VQA任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法在富文本文档图像中定位精度较差，难以准确找到用于可靠推理的关键文档元素。

Method: 1) 构建挑战性训练数据，通过两条pipeline生成带有细粒度坐标元数据的高质量文档，包括带相机效果的合成手填文档；2) 引入视觉定位推理训练范式，使用强化学习联合监督定位和推理。

Result: 在各种基准测试上展示了文档空间定位和文档理解的优势。

Conclusion: 提出的数据和方法能有效提升文档元素的精确 grounding 能力。
...
TL;DR: 本研究旨在将卫星预训练的地球观测基础模型（Prithvi-EO-2.0-600M Vision Transformer）适配至厘米级洪水制图任务。研究者通过结合UPerNet解码器构建Prithvi-2.0-UPN模型，在两个RGB数据集上进行二元水分割微调实验。


<details>
  <summary>Details</summary>
Motivation: 实时高分辨率洪水地图对应急响应和损失评估至关重要。现有CNN和小规模视觉Transformer模型需要大量数据适应新场景，而卫星预训练模型与RGB航拍影像在空间分辨率、观测几何和辐射特征上存在差异，需要进行适配研究。

Method: 构建Prithvi-2.0-UPN模型（Prithvi-EO-2.0-600M Vision Transformer + UPerNet解码器），在BlessemFlood21和NeuenahrFlood两个RGB数据集上进行二元水分割任务的微调。设计三个实验：1）域内训练测试；2）跨域零样本迁移；3）小样本微调迁移。

Result: 实验一：Prithvi-2.0-UPN在两个数据集上均达到最优性能。实验二：零样本迁移性能优于基线模型，但仍有提升空间。实验三：使用少量NeuenahrFlood数据进行微调后，模型性能快速提升，接近全数据训练水平，展现出良好的迁移能力。

Conclusion: 卫星预训练的地球观测基础模型可成功适配至厘米级RGB洪水制图任务，且具有优秀的迁移能力，仅需少量目标域数据即可达到接近全数据训练的性能，为低数据成本下的洪水制图提供了新思路。
...
TL;DR: Bengal-HP_RU是首个针对孟加拉裔人群的头部姿态数据集，包含12,894张带标注的头部图像，标注了偏航、俯仰和横滚的连续值。该数据集来自Wikimedia Commons，按上传者身份划分训练集和测试集，旨在解决现有头部姿态数据集中南亚人群代表不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有头部姿态数据集主要包含西方或东亚人群，缺乏对南亚人群、特别是孟加拉裔人群的代表性。因此需要创建首个以孟加拉裔人群为中心的公开头部姿态数据集。

Method: 从Wikimedia Commons获取免费许可证的图像，通过自动化处理流程后进行人工标注校正。按Wikimedia上传者身份划分数据集以防止数据污染。

Result: 数据集包含12,894张标注图像，分布在296个唯一上传者中，训练集10,494张，测试集2,400张。在受试者年龄、性别、遮挡、光照和背景方面具有显著多样性，反映了真实的野外条件。

Conclusion: Bengal-HP_RU是首个公开可用的孟加拉裔头部姿态数据集，为头部姿态估计研究中的种族多样性做出了重要贡献。
...
TL;DR: Sat2City v2是一个将从单张卫星图像生成显式3D城市资产的任务扩展到可重用纹理网格的系统，基于预训练的原生3D基础模型，使用弱对齐的卫星图像和纹理网格，在包含24个地区9个城市16,241对卫星-网格对的真实世界数据集上训练。


<details>
  <summary>Details</summary>
Motivation: 原始Sat2City框架存在外观随机、训练数据合成、特定任务VAE无法扩展到嘈杂的真实世界重建等问题，且生成的仅是面向渲染的3D代理而非可重用的纹理网格资产。

Method: 将每个网格编码到预训练的原生3D潜在空间，微调卫星条件几何流，使用解码形状锚定卫星条件纹理化，保留几何到外观的级联，同时实现外观可控生成。

Result: 在度量尺度DSM重建和生成城市资产基准测试（几何和外观）中，Sat2City v2在所有评估基线中取得最佳整体性能。

Conclusion: Sat2City v2将卫星到城市生成从面向渲染的3D代理推进到显式纹理网格资产，并提供了据我所知为该资产生成任务收集的首个已记录的卫星-网格配对数据集。
...
TL;DR: 本文提出了一种针对3D高斯溅射(3DGS)的几何感知风格迁移框架，通过解耦优化方案同时迁移外观属性和几何结构。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS风格化方法主要关注颜色风格化，忽略了结构适应性，导致几何变换不一致。

Method: 提出几何感知对比特征匹配(GCFM)，整合RGB、深度和边缘线索，在两个优化阶段使用，实现颜色和几何参数的交替更新。

Result: 实验表明，该方法在定性和定量指标上均优于现有的3DGS风格化方法。

Conclusion: 解耦优化方案能够有效减轻颜色和几何更新之间的干扰，实现稳定一致的场景级几何变换。
...
TL;DR: 当前文献认为视频生成本质上就是世界建模，但这一说法不准确。视频生成模型学习的是部分隐式的时空世界模型，而非完全可落地或可控的模型。论文提出应将视频生成理解为具有反事实可控性的自主生成能力，这是实现自我进化世界模型的有前景方向。


<details>
  <summary>Details</summary>
Motivation: 现有文献声称"视频生成即世界建模"，这一观点虽然推动了生成式AI从静态图像向时序物理场景的发展，但也存在风险——仅依赖视觉预测的规模化无法自动产生物理智能体。作者认为需要更准确的定义来推动该领域发展。

Method: 论文提出将视频生成的核心标准定义为"反事实可控性"(counterfactual controllability)，即模型能够回答"在某动作下会发生什么"，检验生成的未来是否能承受具身约束，并将动作知识反馈到未来想象(生成)中。

Result: 作者主张：视频生成模型应学习部分隐式的时空世界模型，但需强调反事实可控性这一关键能力，使其能够理解哪些变量可控、哪些约束属于特定身体、哪些未来在干预下仍然有效。

Conclusion: 自主视频生成与反事实可控性的结合是实现自我进化世界模型的有前景途径。未来的前沿不在于单纯的预测真实性，而在于生成模型的自我进化能力。
...
TL;DR: 提出微分展开(DU)框架，解决视频快照压缩成像中深度展开网络高计算成本和表征停滞问题，通过异构结构动态演化替代均匀重复设计。


<details>
  <summary>Details</summary>
Motivation: 现有深度展开网络(DUNs)采用统一设计哲学，反复堆叠高复杂度先验，忽视优化轨迹收敛于静态状态的事实，导致高成本计算浪费在微小特征更新上。

Method: 提出微分演化框架(DEF)，将展开过程分为结构锚定和微分演化两类阶段：高参数一般阶段稀疏部署生成高保真特征基础，轻量级微分阶段使用微分表征先验(DRP)通过微分机制传播和精炼特征，并集成微分表征注意力(DRA)和微分调制FFN(DM-FFN)。

Result: 在视频快照压缩成像任务上建立新的最先进结果，同时显著降低计算开销。

Conclusion: DU通过聚焦动态演化资源而非静态冗余，实现了精度和效率的卓越权衡。
...
TL;DR: 本文提出PriorTR方法,通过引入null token在单次前向传播中分离模型先验与任务相关注意力,从而解决现有方法中指令条件化token被误删的问题,实现更精准的视觉token剪枝。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法依赖文本-视觉注意力分数,但发现注意力常被模型诱导的先验主导。即使无文本指令,MLLMs也会聚焦于特定任务无关区域,导致指令条件化token的注意力分数被抑制,在剪枝时被错误丢弃的风险增加。

Method: 提出PriorTR(先验校正token剪枝),是一种无需训练的token剪枝方法。核心创新是引入null token作为指令无关探针,在单个前向传播中同时估计模型先验和任务后验的注意力分布。通过对比先验与任务条件化注意力分布,计算每个视觉token贡献的额外可用信息。

Result: 在多个多模态基准测试和MLLMs上的广泛实验表明,PriorTR在激进token预算下仍能持续改善精度与效率的权衡,优于现有最强的无训练基线方法。

Conclusion: PriorTR通过显式分离模型先验与任务条件化注意力,有效解决了指令token被误剪的问题,为MLLMs的高效推理提供了新的思路。
...
TL;DR: 本文提出CBCM和DCD两种方法解决基础模型偏差问题。CBCM利用扩散解耦的语义概念表示来识别伪属性，无需属性注释；DCD采用双分支跨空间投影框架分离目标和伪表示，在保留目标语义的同时消除伪信息。实验在四个基准数据集上取得组无监督方法中最先进的最差组准确率，且仅调整0.22%的模型参数。


<details>
  <summary>Details</summary>
Motivation: 基础模型在有偏差数据集上训练时，往往依赖目标标签与非因果属性之间的虚假相关性，导致在少数群体上泛化性能差。偏差缓解面临两大挑战：一是当组标签不可用时，现有组无监督方法难以识别与真实世界偏差语义对齐的伪属性；二是即使有伪监督，大多数去偏方法采用单分支设计，在单一共享特征空间中操作，目标与伪属性固有纠缠。

Method: 为解决第一个挑战，提出置信引导偏差概念挖掘(CBCM)，利用扩散解耦的语义 grounded 概念表示来识别可靠的伪属性，无需属性注释。为解决第二个挑战，提出双分支跨投影去偏(DCD)，一个提示调优框架，将目标和伪表示分离到两个分支，并通过跨零空间投影明确消除伪信息，同时保留目标相关语义。

Result: 在四个基准数据集上的广泛实验表明，该方法在组无监督方法中实现了最先进的最差组准确率，同时仅调优最多0.22%的模型参数。

Conclusion: 研究成功解决了基础模型偏差 mitigation 的两大根本挑战，CBCM能自动识别语义对齐的伪属性，DCD能有效分离和消除伪信息且保持目标语义，为组无监督偏差缓解提供了有效方案。
...
TL;DR: 提出了一种基于跨层频谱演化(CLSE)的无训练token裁剪框架，通过在频域分析token表示跨Transformer层的演化来识别语义活跃的token，在图像和视频基准测试中实现效率和精度的更优权衡。


<details>
  <summary>Details</summary>
Motivation: 现有token裁剪方法依赖单层信号，忽略视觉表征的跨层转换，且可能在多模态token序列中表现出位置偏差，需要更稳定的重要性判据。

Method: CLSE在频域中量化token表示跨Transformer层的演化，测量从高频结构细节到低频语义抽象的转变，保留频谱重分布更强的token。

Result: 在 aggressive token reduction下实现更优的效率-精度权衡，多个MLLMs上减少FLOPs、KV cache内存和延迟，同时保持或提升性能。

Conclusion: 通过建模跨层token动态，CLSE提供了稳定的重要性判据，有效缓解位置偏差，为加速多模态大语言模型提供了一种有效的无训练token裁剪方案。
...
TL;DR: 本文提出PoinTriE框架,针对点云视频理解中的三个核心挑战:数据效率、参数效率和内存效率。该方法通过合成伪运动轨迹、文本语料库和2D投影来生成自监督信号,采用几何-运动对偶网络进行多模态对比学习,在微调阶段使用带有LoRA单元的轻量级时空侧网络,并通过梯度流掩码策略显著降低内存消耗。实验表明该方法在动作识别和语义分割任务上达到了最先进水平。


<details>
  <summary>Details</summary>
Motivation: 当前点云基础模型虽已取得进展,但参数高效微调方法存在两大关键局限:1)大规模点云数据集的标注成本过高;2)严重的内存瓶颈。现有PEFT技术无法同时满足数据高效、参数高效和内存高效这三个核心需求。

Method: 1)预训练阶段:通过刚性变换合成伪运动轨迹,结合文本语料库和原始点云的2D投影;2)提出几何-运动对偶网络,采用多模态对比学习、刚性旋转预测和运动分布散度进行优化,生成密集自监督信号;3)微调阶段:冻结预训练 backbone,仅更新轻量级时空侧网络(内置LoRA单元),配合梯度流掩码策略同时降低内存和参数开销。

Result: 在动作识别和语义分割任务上建立了新的state-of-the-art结果,验证了PoinTriE在数据、参数和内存三个维度上的优越性能。

Conclusion: PoinTriE是一个统一的框架,能够同时满足数据高效、参数高效和内存高效三个核心需求,为点云视频理解提供了一种高效且可扩展的解决方案。
...
TL;DR: 将测试时规范化重新定义为分布外检测问题，通过最小化任意OOD分数来撤销输入的仿射变换，使预训练视觉模型对旋转、缩放、剪切等变换保持鲁棒性，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 预训练的视觉模型在处理经过旋转、缩放或剪切等仿射变换的输入时经常错误分类，尽管这些变换不会改变对象类别。现有方法需要修改模型架构或使用数据增强重新训练，因此作者提出保持分类器不变，通过测试时规范化将输入映射到canonical form。

Method: 将规范化重新框架为OOD检测问题，使任意OOD分数可以在变换上最小化。系统评估约20种OOD分数和9种搜索算法，添加门控机制仅对需要变换的输入进行规范化处理。

Result: 距离分数与随机搜索和局部细化组合表现最佳。门控机制保留了大部分分布内准确性的同时，在变换输入上保持鲁棒性收益。

Conclusion: 该方法为预训练视觉模型的仿射变换鲁棒性提供通用框架，无需修改模型或重新训练。距离分数结合随机搜索是最优设计选择，门控机制有效平衡了分布内准确性和鲁棒性。
...
TL;DR: 本文是一篇关于三维场景补全的综述论文，涵盖了2016-2026年间该领域从体素语义补全到扩散模型与高斯溅射结合的技术演进历程，并建立了分类体系、讨论了挑战与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 三维场景补全在计算机视觉和机器人领域具有重要应用价值（如自动驾驶、增强现实等），但缺乏对该领域系统性综述来梳理十年来的技术演进脉络和研究方向。

Method: 系统性综述方法，收集并分析过去十年（2016-2026）的研究贡献，对比不同表示范式（体素网格、点学习、神经隐式场、Transformer网络、扩散网络、渲染感知3D高斯基元）的技术演变。

Result: 梳理了从SSCNet代表的体素语义补全范式到最新的扩散先验结合高斯溅射实时渲染的技术演进，建立了分类体系，识别了仍需解决的挑战，并提出了未来研究议程。

Conclusion: 该领域经历了从传统体素方法到神经渲染技术的重大变革，未来方向包括更高效的生成模型、实时渲染集成以及在机器人导航和AR中的应用拓展。
...
TL;DR: AdaQ是一种用于长视频理解的自适应无训练关键帧选择方法，基于准高斯采样和3σ规则，仅需设置一个超参数即可实现高效帧采样。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型处理长视频时面临计算和内存开销大的问题，传统关键帧选择方法基于硬采样原则，灵活性低且噪声高。

Method: 将视频帧选择定义为准高斯采样问题，提出AdaQ方法。利用高斯分布的3σ规则，为不同示例实现最优的3σ间隔：局部查询使用较小的3σ区间，全局查询使用较大的3σ区间，从而实现自适应鲁棒的帧采样。

Result: 在四个多模态大语言模型和三个嵌入模型上进行了广泛实验。结果表明，AdaQ性能显著优于默认MLLMs和SOTA关键帧选择方法。例如，仅使用64帧即可帮助Qwen3-VL-8B平均超越GPT4o达15.8%。

Conclusion: AdaQ方法在长视频理解任务上表现出显著的性能提升，具有优异的鲁棒性和高效率，仅需设置一个超参数即可工作，为长视频理解提供了一种有效且实用的解决方案。
...
TL;DR: 本文提出ReCARE框架，解决扩散模型中概念遗忘技术会意外删除良性共现概念的问题，通过构建CARE-set保护共现概念的同时实现目标概念的有效擦除。


<details>
  <summary>Details</summary>
Motivation: 现有概念遗忘方法在移除目标概念时，会不可避免地抑制与之共现的良性概念。例如去除"裸体"概念时，会同时抑制"人物"概念，导致模型无法生成包含人物的图像，损害了模型的实用性。

Method: 提出CARE评分作为量化指标衡量共现概念的保留程度；构建ReCARE框架，从目标图像中自动提取良性共现标记形成CARE-set词汇表，并在训练过程中利用该词汇表稳定地进行概念擦除，确保仅移除目标概念而保留共现概念。

Result: 在多种目标概念（裸体、梵高风格、Tench物体）上进行了广泛实验，证明ReCARE在概念擦除、模型效用和CARE保留三个维度上达到了最优平衡。

Conclusion: ReCARE框架能够有效解决概念遗忘中的共现概念保护问题，为扩散模型的安全应用提供了新的解决方案，具有重要的实践价值。
...
TL;DR: 本文提出I2C-3D方法，用于生成多视角一致的多对象交互式3D资产。该方法解决了现有3D生成中的两个主要挑战：多对象组合生成和跨视角一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法面临两个挑战：(1) 主要生成单一3D对象，难以生成具有合理交互的多对象组合3D资产；(2) 在3D优化过程中存在跨视角不一致问题，因为分数蒸馏采样本身在每个单独视角上执行，导致跨视角幻觉。

Method: 提出两种核心策略：1) 包含式交互碰撞策略(Inclusive Interactive Collisions)，引导高斯原语出现在合理的交互区域中，确保组合场景中的对象以物理上可信且视觉上连贯的方式交互；2) 多视角自适应分数蒸馏采样(Multi-View Adaptive Score Distillation Sampling)，通过调制实例标记和空间标记的注意力图，从预训练扩散模型中提取多视角一致性和布局先验。

Result: I2C-3D不仅能够生成高保真的多视角一致性组合3D资产，还支持灵活的3D编辑，促进复杂场景生成。广泛实验表明，该方法在生成质量和多视角一致性方面优于现有方法。

Conclusion: 通过Inclusive Interactive Collisions和Multi-View Adaptive Score Distillation Sampling的精心设计，I2C-3D成功解决了多对象3D生成和跨视角一致性的双重挑战，为复杂3D场景生成提供了有效解决方案。
...
TL;DR: MorVess是一种结合可微几何先验和大规模基础模型适配的肺血管分割框架，通过联合预测血管掩码、距离图和厚度图实现细粒度血管解析。


<details>
  <summary>Details</summary>
Motivation: 现有深度分割模型缺乏几何约束，难以恢复连续的管状形态和精细的血管连通性，小血管分支容易丢失，拓扑完整性难以保持。

Method: 1) 联合预测 vessel masks、distance maps 和 thickness maps，提供血管边界、中心线一致性和直径过渡的显式监督；2) 轻量级2.5D适配器桥接3D空间上下文和2D SAM表示；3) 全局-局部融合块聚合多级语义和几何线索进行高保真拓扑重建。

Result: 在两个肺CT基准数据集上，MorVess在Dice、clDice和HD95分数上均取得优异性能，显著提升了小血管恢复和全局连通性。

Conclusion: 将几何智能嵌入预训练视觉模型为精确血管分析和临床可靠的结构量化提供了一条原则性和可扩展的途径。
...
TL;DR: GIVE是一个几何指导的视频编辑框架,通过统一的物体状态表示来处理视频中的物体级几何编辑(平移、旋转、缩放、复制、删除)。该方法使用两个视频对齐的几何流(depth-box和orientation-box)来描述编辑前后的3D状态变化。


<details>
  <summary>Details</summary>
Motivation: 在数字内容创作工作流中,物体级几何编辑是常规操作,但在生成式视频编辑中仍然不可靠。核心挑战在于:如何在跨越视角和时间的同时明确指定目标物体的3D状态变化,并保持与几何相关的二级效果(阴影、反射)的一致性。

Method: 提出统一物体状态表示的编辑框架,使用两个视频对齐的几何流:1)depth-box编码粗略的3D位置和范围;2)orientation-box提供外观无关的方向线索。这两个流提供了编辑前后几何规范的紧凑表示。构建可扩展的图形引擎管道来生成成对的监督数据,执行物体级编辑程序并渲染受控的前后对比。

Result: 实验表明GIVE能够产生保真的几何编辑,具有时间一致性,在统一框架内保持二级效果的一致性,并显示出对野外视频的良好迁移能力。

Conclusion: GIVE框架通过几何指导的方式成功解决了视频编辑中物体级几何变换的难题,为生成式视频编辑提供了新的研究方向。
...
TL;DR: 本文提出了FiCA系统，这是一个前馈即时高斯编码头像生成管道，能够从单张肖像图像生成逼真的可驱动3D头像。该系统结合了人类中心视觉基础模型和扩散模型，利用部分视觉观察生成完整且真实的3D网格重建，并通过前馈网格细化网络增强保真度和身份保留，最后使用通用先验模型将生成的网格解码为3D高斯表示，实现实时驱动新表情的逼真3D头像。


<details>
  <summary>Details</summary>
Motivation: 从单张图像生成逼真且可驱动的头像具有重大挑战，因为可用的视觉信息有限，难以准确推断人头的3D外观和几何形状，这是该领域面临的核心难题。

Method: 提出了一种结合人类中心视觉基础模型与扩散模型的新型系统：1)利用扩散模型学习从部分观察到完整真实3D网格重建的生成映射；2)引入前馈网格细化网络增强保真度和身份保留，无需针对个人的测试时优化；3)使用通用先验模型将生成的网格解码为3D高斯集合，生成可实时驱动新表情的逼真3D高斯头像。

Result: 实验表明，前馈方法生成的头像忠实地代表了不同身份，超越了近期竞争方法生成头像的视觉质量。

Conclusion: FiCA系统能够从单张肖像图像实时生成高质量、逼真的可驱动3D头像，具有良好的身份保留效果，为单图像头像生成任务提供了有效的解决方案。
...
TL;DR: EVA框架通过生成连续潜在视觉表示(Latent_slot tokens)来解决大视觉语言模型中依赖离散输出的局限性，并配合D-GSPO算法解决优化过程中的策略偏移问题，在多个基准测试上实现了显著的性能提升和推理效率增强。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型集成视觉证据主要依赖生成离散输出(代码或边界框坐标)来调用外部工具，这种方式存在刚性依赖和高延迟的局限性。

Method: 提出EVA框架生成连续潜在视觉表示，使用Latent_slot tokens作为中间视觉推理思考，并与离散文本token进行端到端联合优化。同时开发D-GSPO算法解耦潜在组件和离散组件的优化，以解决transition window中的策略偏移问题。构建EVA-230K高质量文本图像交错CoT数据集。

Result: 在多个基准测试上实现了显著的性能提升，同时增强了推理效率。

Conclusion: EVA框架通过连续潜在视觉表示和创新的优化方法，为多模态模型的视觉理解提供了一种更高效、更灵活的新范式。
...
TL;DR: 针对海上船舶环境中视觉位置识别(VPR)的跨场景感知偏移问题,提出ProteusVPR两阶段检索-精化框架。该框架结合几何描述符、局部仿射坐标系和相机方位编码,将检索图像与时间先验帧融合,实现精确定位。实验表明该方法在XHZ数据集上平均降低定位误差超过60%。


<details>
  <summary>Details</summary>
Motivation: 海上船舶环境具有独特的视觉挑战:开放甲板区域纹理稀疏且光照变化剧烈,封闭舱室结构重复且视觉歧义高。现有VPR方法主要针对城市场景设计,无法在这些差异显著的场景间可靠泛化。

Method: 提出ProteusVPR两阶段框架:第一阶段使用标准VPR模型进行初始图像检索;第二阶段引入几何视觉估计网络,将检索图像与两个时间先验帧融合,并结合几何描述符、局部仿射坐标系和相机方位编码实现精确定位。同时构建XHZ数据集,包含8K全景船舶图像,涵盖多层舱室结构和甲板过渡区。

Result: 在XHZ数据集上的广泛实验表明,ProteusVPR在多种VPR backbone上一致提升定位精度,平均降低定位误差超过60%,为跨场景海上环境中的精确视觉定位提供了有效解决方案。

Conclusion: ProteusVPR通过两阶段检索-精化策略有效解决了船舶环境中跨场景视觉位置识别的难题,为海上无人系统精确导航提供了技术支撑。
...
TL;DR: 本文提出M^2C-EvDet框架，用于解决事件相机目标检测(EvDet)与帧级目标检测之间的跨模态性能差距问题。该框架通过频率学习和超图计算融合多域多阶跨模态知识蒸馏，包含自适应频率解耦特征蒸馏(AF²D²)和多阶关系蒸馏(MORD)两个模块。


<details>
  <summary>Details</summary>
Motivation: 事件相机目标检测因数据稀疏表示和视觉语义不足，与传统帧级目标检测存在显著性能差距。现有的跨模态知识蒸馏方法仅关注空间视觉语义或成对关系信息，难以应对复杂场景。

Method: 提出M^2C-EvDet框架，基于频率学习和超图计算构建。(1) AF²D²模块：自适应频率解耦特征蒸馏，将特征解耦为低频和高频成分进行针对性蒸馏；(2) MORD模块：多阶关系蒸馏，利用超图建模多阶关系信息。

Result: 实验表明M²C-EvDet在多个基准数据集上验证了其有效性，能够有效弥补事件数据与帧数据之间的跨模态语义差距，提升事件目标检测性能。

Conclusion: 通过整合多域多阶跨模态知识蒸馏策略，M²C-EvDet为事件相机目标检测提供了一种有效的性能提升方案，为生物启发式视觉感知系统的实际应用奠定了基础。
...
TL;DR: TuringViT是一种新型视觉Transformer，通过三项关键设计（TLA线性注意力、VISTA-Curation数据策展、原生动态分辨率预训练）实现高效训练，在仅使用10%数据的情况下超越开源ViT基线，并具备良好的延迟扩展性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型多采用现成的ViT作为视觉编码器，但下游任务对延迟、时间建模和VLM集成的多样化需求呼唤定制化的SOTA级ViT。训练此类编码器需要海量图像文本数据，且标准softmax注意力使高分辨率预训练成本过高。

Method: 提出三个关键设计：(1)Turing Linear Attention (TLA)用于高效序列建模；(2)VISTA-Curation构建监督丰富的图像-视频训练数据；(3)原生动态分辨率预训练，支持从一开始就接受灵活输入并无缝迁移到下游VLM。

Result: TuringViT仅用10%数据即超越领先的开源ViT基线，获得更强的下游VLM性能，在高分辨率输入上实现显著更好的延迟扩展。扩展法则分析表明其性能随策展数据 scale 持续可预测提升，远未饱和。

Conclusion: TuringViT提供了一套可复现的pipeline，大幅降低了社区训练、定制和部署SOTA级ViT的成本，使这类视觉Transformer走向大众化，已成为XPeng AI系统的统一视觉基础。
...
TL;DR: 本文提出Solo-to-Social框架,将文本驱动的3D人际交互生成问题分解为社交结构建模和动作实现两个阶段,利用LLM作为规划器进行社交结构推理,结合适配的独行动作模型作为执行器生成协调的双人动作。


<details>
  <summary>Details</summary>
Motivation: 现有文本到动作生成方法仅能处理单人动作合成,扩展到3D人际交互面临挑战,因为HHI需要建模底层社交结构(包括阶段进程、角色分配和角色间协调),而LLM虽然能推理社交结构但无法直接生成物理可信的动态动作。

Method: 提出规划器-执行器范式"Think with LLM, Move with Motion Skill":(1)LLM规划器将隐式交互语义转换为动作对齐的社交监督,通过将交互分解为阶段、分配角色感知伙伴的角色并与动作序列对齐;(2)动作执行器通过LoRA、自前置条件化和自我相对伙伴条件化来适配预训练的单人动作模型,将社交结构实现为协调的双人动作。

Result: 该框架桥接社交组织和动作实现,在阶段一致性、角色对齐和伙伴感知协调方面生成改进的3D人际交互。

Conclusion: Solo-to-Social框架成功将LLM的社交结构推理能力与动作模型的物理动作生成能力相结合,为文本驱动的3D人际交互生成提供了有效解决方案。
...
TL;DR: 本文提出了Tailor-Bench基准，用于评估视觉世界模型对长尾物理交互的模拟能力。研究发现当前模型在非常规和不可能场景下表现显著下降，存在明显的"长尾差距"。


<details>
  <summary>Details</summary>
Motivation: 现有视觉世界模型主要关注模拟常见物理交互，而对罕见和不规则物理交互的建模能力尚未被系统评估。作者提出核心问题：当前视觉世界模型是否内化和泛化了物理原理？

Method: 设计了三种递进式场景模式：常规场景（常见工具-任务对）、非常规场景（用属性兼容的替代工具测试 affordance 泛化）、不可能场景（引入违反属性的工具测试约束意识）。同时设计了两套互补设置：预测生成（无指导推断结果）和描述生成（指定目标结果）。

Result: 实验揭示了物理世界建模中的明确长尾差距：性能从常规到非常规再到不可能场景逐渐下降。图像模型无法实现正确的状态变化，视频模型还存在时间不一致性问题。

Conclusion: 当前视觉世界模型依赖浅层视觉模式，难以泛化物理原理到罕见交互，存在明显的长尾差距，限制了其在非常规场景中的应用。
...
TL;DR: 3DCarGen是一个基于单张图像生成高质量3D汽车模型的框架，通过多视图合成和3D高斯溅射技术实现可扩展的单视图3D汽车重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于多视图扩散的单图像重建方法存在视角有限和跨视图几何不一致的问题，难以满足自动驾驶仿真中对高质量3D车辆资产的需求。

Method: 给定单张输入图像，首先从固定视角合成一组图像，然后馈入前馈重建模型生成基于3D高斯溅射的粗略3D表示；在此显式3D先验条件下，多视图扩散模型从任意相机视角生成3D一致图像；最后通过颜色-法线联合优化扩展的快速网格重建算法从合成密集视图中恢复详细且一致的3D车辆模型。

Result: 在合成和真实世界数据集上的广泛实验表明，该方法实现了稳健的几何一致性和重建保真度，优于现有方法。

Conclusion: 3DCarGen能够从单张图像端到端生成高质量3D汽车模型，具有良好的几何一致性和重建质量，为自动驾驶仿真提供了有效的解决方案。
...
TL;DR: 本文首次提出了一种生成模型，可对不规则时间间隔、地理未对齐且来自不同仪器特征的热带气旋微波图像进行时空插值，通过结合红外和微波数据实现比单独使用微波数据更好的效果。


<details>
  <summary>Details</summary>
Motivation: 微波卫星图像在监测热带气旋降水和强度方面至关重要，但重访时间较长，可能错失快速风暴演化阶段；虽然需要插值方法，但不同仪器产生的异质微波数据使这一任务面临挑战。

Method: 提出首个可应用于多地理空间源的生成模型，处理样本间变化、 irregular时间间隔、地理未对齐和不同仪器特征的图像；采用自监督任务训练，即随机掩码源并重建；将红外和微波数据结合进行时空插值。

Result: 自监督训练相比监督训练显著降低了连续排序概率分数；结合红外和微波数据比单独使用微波数据进一步改善；生成模型产生的集合均值与确定性模型相当，且功率谱更接近真实观测。

Conclusion: 这是首个能够结合多个微波仪器和红外观测数据，对不规则时间间隔的气旋微波图像进行插值的生成模型。
...
TL;DR: 该论文提出了一种结合RGB外观、深度几何和事件相机时间线索的统一多模态框架，用于RGB-D视频插帧。核心创新在于利用事件相机的高时间分辨率来弥补传统RGB-D相机在捕捉快速场景动态时的不足，并通过双向光流估计和Z轴细化来分别优化RGB和深度插值质量。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D相机因硬件限制难以捕捉快速运动场景动态，而传统视频插帧方法无法直接应用于RGB-D场景，常产生边界模糊和几何一致性下降的问题。此外，仅基于两帧边界的运动估计在复杂动态场景中本质上是不充分约束的。

Method: 提出统一多模态框架：1)融合RGB、深度和事件线索；2)对RGB进行双向光流估计和运动基细化，对深度进行Z轴细化；3)通过双向 warping 和软融合合成目标RGB-D帧。同时构建了新的RGB-D-Event数据集以缓解三模态训练数据稀缺问题。

Result: 在公共基准数据集和提出的数据集上进行了广泛实验，该方法在RGB插值的 photometric fidelity 和深度插值的 geometric accuracy 方面均优于现有方法。

Conclusion: 该框架通过有效融合事件相机的高时间分辨率信息，成功解决了RGB-D视频插帧中几何一致性和边界清晰度的挑战，为动态场景理解和3D重建等下游任务提供了高质量的高帧率RGB-D视频。
...
TL;DR: 提出了ActiveScope框架,通过语义锚点定位和干扰抑制细化来解决多模态大语言模型在高分辨率图像中细粒度感知和目标定位的问题。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free方法在处理高分辨率图像时容易受到无关干扰物的影响,导致定位不准确,特别是在多目标场景中。

Method: 提出两个模块:1)语义锚点定位(SAL)使用细粒度语义锚点独立定位关键目标;2)干扰抑制细化(ISR)通过抑制显著干扰物的注意力来细化定位。

Result: 在V* Bench等高分辨率图像理解基准上取得了96.34%的准确率,优于现有的训练-free方法。

Conclusion: 主动搜索和自纠正范式能够有效提升多模态大语言模型的细粒度感知能力。
...
TL;DR: 本文提出DHANet网络解决跨域小样本分割任务，针对现有方法忽略的类别语义粒度和判别属性差异问题，通过三层模块实现：层级空间聚合(HSA)进行多尺度区域聚合生成语义增强特征，层级通道聚合(HCA)进行属性聚合生成属性增强特征，在线概率语义库(OPSB)从查询预测中构建类别概率分布并采样伪原型作为支持信息。


<details>
  <summary>Details</summary>
Motivation: 现有CD-FSS方法主要关注特征分布偏移(风格差异)而忽略跨域类别语义粒度和判别属性差异，导致支持-查询匹配时出现语义过度对齐和属性过度对齐两个关键退化问题。

Method: 提出Dual Hierarchical Aggregation Network(DHANet)，包含三个关键模块：1) HSA模块沿空间维度进行多尺度区域聚合生成层级语义增强特征，缓解语义过度对齐；2) HCA模块沿通道维度进行多尺度属性聚合生成层级属性增强特征，缓解属性过度对齐；3) OPSB模块在推理过程中从查询预测逐步构建更新类别概率分布，采样多个伪原型作为额外支持信息弥补支持样本不足。

Result: 在四个目标域数据集上进行广泛实验，方法达到最先进性能。

Conclusion: 该方法有效解决了CD-FSS中语义和属性过度对齐问题，通过层级聚合机制提升了跨域分割的泛化能力。
...
TL;DR: 本文提出一种无需训练的跨域小样本分割(CD-FSS)框架，基于自监督视觉编码器DINOv3，通过三个核心模块解决跨域知识迁移问题：语义感知特征重融合(SAFR)、自适应支持增强(ASE)和混合原型匹配(HPM)，在四个目标域数据集上实现最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有CD-FSS方法依赖训练或微调过程，导致高计算成本且存在过拟合风险。当引入强大的视觉基础模型时，性能仅边际改善甚至下降。作者观察到这一反常现象，决定摒弃可训练参数，提出完全无需训练的框架来同时避免训练开销和过拟合问题。

Method: 构建于DINOv3自监督视觉编码器之上，包含三个核心模块：1)SAFR模块识别并重融合强调语义模式的特征，生成增强语义可分性的表示；2)ASE模块通过鲁棒的支持信息聚合缩小支持与查询之间的语义差距；3)HPM模块整合来自不同原型的匹配结果以适应不同域的语义复杂性。

Result: 在四个目标域数据集上进行了广泛实验，证明该方法在无需任何训练的情况下达到了最先进的性能。

Conclusion: 提出了一种完全无需训练的CD-FSS框架，通过DINOv3编码器和三个精心设计的模块成功解决了跨域知识迁移中的过拟合和语义对齐问题。该工作表明，通过巧妙的特征处理和原型匹配策略，即使不使用传统训练也能实现优异的分割性能，为小样本学习领域提供了新的研究方向。
...
TL;DR: 提出了MM-TRELLIS，一个将从自动驾驶数据集（Waymo）中获取的多模态传感器数据（多视角图像和LiDAR点云）集成到原生3D生成模型中的多模态3D车辆生成方法。


<details>
  <summary>Details</summary>
Motivation: 现有车辆生成方法未能充分利用多模态传感器（多视角图像和LiDAR点云），且基于神经渲染的重建导致网格质量低；而原生3D生成模型虽取得进展，但并非为任意多视角输入设计，难以处理野外驾驶图像。

Method: 方法包括：(1)多视角图像作为条件输入进行循环；(2)LiDAR点云在测试时提供指导以确保几何精度和跨视角一致性；(3)在去噪过程中先对齐指导点云与模型先验，然后强制生成几何与指导点云保持一致；(4)引入基于3D Gaussian Splatting不透明度的体素滤波策略以抑制浮动物并生成干净的网格。

Result: 在Waymo数据集上的综合实验表明，该方法在高质量3D车辆生成方面优于现有方法。

Conclusion: MM-TRELLIS能够有效整合多模态传感器数据，实现高质量的野外3D车辆生成，为自动驾驶训练数据合成和仿真环境构建提供了有力工具。
...
TL;DR: 研究如何通过三种可控的微调策略（对比度归一化CLAHE、数据增强和层冻结）将TrOCR模型适配到中世纪手稿识别任务，在13世纪意大利手稿I-CT 91 "Cortonese"上达到8.03%字符错误率(CER)，发现冻结最多3层编码器或6层解码器不会显著影响精度，且解码器冻结策略更具跨数据集稳健性。


<details>
  <summary>Details</summary>
Motivation: 预训练的Transformer手写文本识别模型需要从现代文本视觉域适应到中世纪手稿这一非常不同的视觉域，但针对这种领域适应的最佳微调策略尚不明确，因此需要系统研究三种可控微调选择对识别精度的影响。

Method: 在I-CT 91 "Cortonese"（13世纪意大利手稿）上进行13种配置的受控实验，并在公开的READ-16基准数据集上复制实验网格进行鲁棒性验证，使用Grad-CAM梯度归因和解码器交叉注意力图诊断错误模式。

Result: 在Cortonese上最佳配置达到8.03% CER；移除CLAHE后达到7.84% CER，与领域专门化基线相当；冻结最多3层编码器或6层解码器不会显著损害精度；跨数据集验证表明解码器冻结阈值比编码器冻结阈值更具鲁棒性。

Conclusion: 深度优化可以减少对图像预处理的依赖；冻结策略需要数据集特定重新验证；解码器冻结策略更具跨数据集迁移能力；Grad-CAM和交叉注意力图可有效诊断错误模式和失败原因。
...
TL;DR: REDI-Match提出了一种新的旋转等变蒸馏范式，将视觉基础模型的语义表示蒸馏到轻量级旋转等变编码器中，解决VFMs在密集特征匹配中严重平面内旋转的挑战，通过熵驱动的空间对齐模块消除全局模糊，在多个基准数据集上实现新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决视觉基础模型在密集特征匹配中面临严重平面内旋转的关键挑战。现有方法面临两难困境：数据驱动方法需要低效的参数扩展来隐式学习旋转，而严格等变网络缺乏现代VFMs的语义能力，当前框架通常冻结VFMs并将旋转泛化负担转移给下游解码器。

Method: 提出REDI-Match框架，包含两个核心创新：1)旋转等变蒸馏(REDI)：将VFM的非等变语义表示蒸馏到轻量级严格旋转等变编码器，利用等变几何架构约束高维语义；2)熵驱动的空间对齐模块：通过评估离散旋转假设明确锁定规范坐标系，消除全局模糊后再进行连续细化。

Result: 在多个基准数据集上建立新的最先进水平(SOTA)。在极具挑战性的SatAst数据集上实现13.89%的绝对姿态精度提升，比当前SOTA (RoMa v2)快1.9倍，在单张RTX 4090 GPU上实现约41 FPS的实时推理。

Conclusion: REDI-Match成功打破了架构瓶颈，通过旋转等变蒸馏范式高效地将VFMs的语义能力迁移到等变网络中，无需大量参数扩展即可处理旋转问题，为实时旋转鲁棒匹配提供了有效解决方案。
...
TL;DR: UniTranslator是一种统一的跨模态框架，用于图像内机器翻译（IIMT），通过理解-生成对齐模块（UGAM）和空间掩码解码器（SMD）解决文本理解和图像生成之间的冲突，以及空间位置对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有的统一多模态模型直接应用于IIMT存在两大挑战：1）理解-生成冲突：翻译推理与文本监督不一致；2）空间位置错位：渲染文本无法准确匹配目标文本区域。

Method: 1. 引入理解-生成对齐模块（UGAM）桥接表示差距，促进翻译内容预测与文本渲染之间的语义一致性；2. 提出空间掩码解码器（SMD），通过像素级监督提升空间定位、几何对齐和布局可控性。

Result: 在多个基准数据集上实现了最先进的性能，涵盖多种语言方向和复杂的真实世界布局。

Conclusion: 研究结果揭示了翻译理解和图像生成之间的强相互强化效果，突出了统一翻译多模态学习的优势。代码已开源。
...
TL;DR: 提出单目度量物体尺寸估计作为视觉语言模型中证据选择的病态诊断任务，发现当前最大VLMs在度量估计上不如纯文本前沿LLM，且模型未能学习利用场景几何。


<details>
  <summary>Details</summary>
Motivation: 反事实分析在视觉语言模型研究中用于证据使用诊断，但在良定义任务上价值有限，因为多个线索独立支持同一答案时，移除一个可能不会改变预测。需要建立一个无法从单张未校准图像确定物理尺寸的病态诊断设置。

Method: 组装Metric VQA数据集（10,813维度查询来自Objectron，331个真实场景），评估12个开放权重VLMs，使用反事实分析分解六个视觉和语言证据通道，并分析LoRA微调的干预效果。

Result: 最大VLMs在真实场景上表现不如纯文本前沿LLM；目标identity是最关键的线索，目标像素和局部上下文仅帮助部分模型，表观尺寸改变预测但无方向性输出，全局场景几何基本未被使用；任务可学习但模型未学习利用场景几何。

Conclusion: 单目度量物体尺寸估计是有效的证据选择诊断任务，揭示了当前VLMs在利用视觉线索上的关键局限；LoRA微调虽可学习任务，但模型未能学习利用场景几何。
...
TL;DR: TIGER是一种用于面部视频修复的三重先验融合框架，通过整合身份先验、几何先验和生成先验，从退化输入中恢复高保真面部视频，同时保持身份和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法往往难以同时解决三个关键挑战：身份偏移、视角纠缠指导和感知真实感。需要一种方法能够同时处理这三个问题。

Method: 首先建立身份先验，将主体判别性嵌入注入潜在空间以锚定主体身份；然后构建几何先验，将2D参考线索提升到解耦的3D参数空间，通过跨源参数融合创建几何锚点；此外利用视频生成模型的生成先验，通过一步 rectified flow实现最大效率；并设计渐进式三阶段训练优化策略。

Result: TIGER在身份保真度和时间稳定性方面达到最先进的性能，提供高质量、高效且身份一致的面部视频修复。

Conclusion: 该方法成功解决了面部视频修复中的身份偏移、视角纠缠和感知真实感三大挑战，通过三重先验融合和渐进式训练优化策略，实现了卓越的修复效果。
...
TL;DR: 本文提出开放词汇BEV分割(OVBS)任务，利用视觉语言模型识别训练集外的类别，并通过几何感知框架OVBEVSeg解决2D到BEV升维中的3D几何不一致问题，在nuScenes数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知方法局限于封闭集场景，无法识别训练时未见过的类别，难以应对真实世界不可预测的环境，因此需要开放词汇BEV分割能力。

Method: 提出OVBEVSeg框架，采用三阶段几何感知策略：(1)通过可靠3D投影进行2D-to-BEV伪标签生成实现OV泛化；(2)联合2D-BEV每场景优化结合BEV结构约束保证3D几何一致性；(3)3D几何蒸馏实现在线效率。

Result: 在nuScenes数据集上，OVBEVSeg比封闭集方法在未见类别上提升15.3 mIoU；即使无新类别标注，也能与使用40%标注的半监督方法竞争；推理速度提升2.5倍，内存消耗仅为0.22倍。

Conclusion: OVBEVSeg成功实现开放词汇BEV分割，在保持实时效率和精确BEV感知的同时，能够识别训练集外的类别，为自动驾驶感知系统的实际部署提供了可行方案。
...
TL;DR: SignNet-1M是一个大规模手语数据集，通过3D高斯溅射、扩散模型和后处理增强技术，生成包含ASL、CSL和德语手语的多样化训练数据，显著提升模型在跨视角、跨背景、跨身份等真实场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型在受限条件下训练，缺乏视角、背景和签名者身份的多样性，导致在真实世界分布变化下表现不佳。

Method: (i) 3D高斯溅射实现新颖视角渲染；(ii) 扩散模型进行背景替换和签名者替换；(iii) 后处理模拟采集和压缩伪影

Result: 在跨视角、跨背景、跨身份和后处理位移变化下，训练于SignNet-1M的模型一致提升泛化能力，同时保持较强的分布内性能。

Conclusion: 大规模数据增强是提升手语模型鲁棒性的有效方法，SignNet-1M为手语研究和应用提供了重要的数据集和基准。
...
TL;DR: 本文提出将卷积柯尔莫哥洛夫-阿诺德网络(KANs)中的可学习函数从逐像素方式改为在滤波器结构上学习，并研究了三种实现方案：SV-KAN、AG-KAN和RF-KAN。


<details>
  <summary>Details</summary>
Motivation: 传统KAN在每个卷积核权重处附加可学习函数，参数量大且易过拟合。作者认为可学习函数应放在卷积的"结构"而非每个"边"上。

Method: 设计三种KAN变体：1) SV-KAN对像素值应用共享单变量函数，滤波器保持静态；2) AG-KAN保留共享值函数并添加内容自适应高斯门；3) RF-KAN将可学习函数移到滤波器形状上，使用局部振荡(Morlet)小波基构建。

Result: 在CIFAR-10上RF-KAN和SV-KAN分别达到88.47%和88.20%，在CIFAR-100上达到64.40%和64.57%，参数量约0.4M。RF-KAN和SV-KAN在相同规模下性能相当，均优于普通卷积和官方Gram变体，参数量约为后者的五分之一。

Conclusion: RF-KAN的性能提升源于其固有局部振荡基和内容自适应特性。消融实验表明学习滤波器形状是核心要素，去除后准确率下降超过40个百分点。
...
TL;DR: MATCH是首个基于流匹配(Flow Matching)的多视角异常检测方法,能够在对象、图像和像素级别检测工业图像数据中的异常,在Real-IAD和MANTA-Tiny数据集上实现了最先进的异常检测和分割性能,且可在消费级硬件上实时运行。


<details>
  <summary>Details</summary>
Motivation: 工业生产中复杂对象通常需要从多个视角进行分析,以提高生产效率。现有方法缺乏专门针对多视角数据的异常检测方案,因此需要开发新的多视角异常检测方法。

Method: 利用流匹配的ODE公式估计似然性来推导异常评分,结合FM模型的架构灵活性高效处理不同空间大小的特征并将其转换为正态分布。通过省略昂贵的散度项实现实时性能。

Result: 在Real-IAD和MANTA-Tiny数据集上实现了最先进的异常检测和分割性能,能够在消费级硬件上运行,且可实时用于生产场景。

Conclusion: MATCH为工业异常检测提供了一种高效、准确的多视角解决方案,在性能和实用性之间取得了良好平衡,有望在实际生产环境中得到广泛应用。
...
TL;DR: 本文提出了一种针对多模态分布外检测的后验检测器，旨在解决现有方法在推理时丢弃多模态信息的问题。通过利用多模态与单模态预测之间的关系，结合特征空间分数和多模态logits归一化，在MultiOOD基准数据集上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 当前将额外模态融入动作识别模型的研究侧重于提升性能，但对如何使模型更鲁棒的关注不足，特别是多模态分布外(OOD)检测。现有的多模态训练正则化方法在推理时仍使用针对单模态设计的OOD检测器，丢弃了重要的多模态信息。

Method: 1. 发现多模态与单模态预测之间存在有趣的关系；2. 利用这一关系构建针对多模态场景的后验检测器；3. 将新信息源与特征空间分数结合(检测多模态空间中的离群样本)；4. 通过多模态logits进行归一化。

Result: 在MultiOOD基准的多个标准数据集上实验表明，该方法平均优于现有最佳方法，验证了在推理时显式考虑不同模态对多模态OOD检测的重要性。

Conclusion: 本文方法兼容现有的训练时方法且始终如一地提升性能，证明了在推理阶段显式考虑多模态信息的必要性，为多模态OOD检测提供了新的研究方向。
...
TL;DR: EgoSAT是首个针对流式设置下自我中心视频推理的综合基准，用于评估现代视觉语言模型(VLMs)的能力。该基准包含1,997个独特视频（约165小时自我中心 footage）和约4,800个高质量问答对，统一了回顾性推理、在线理解和前瞻性预测三种任务。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门评估视觉语言模型在流式交互理解方面能力的基准。之前的研究将各种任务分散处理，未能系统评估模型在连续帧到达时对过去、现在和未来事件的推理能力。此外，现有模型是否真正具备在流式设置下进行推理的能力尚不明确。

Method: 构建统一的流式框架，将回顾性推理（关于已完成事件的查询）、在线理解（关于正在进行活动的查询）和前瞻性预测（关于未来动作的查询）整合到单一基准中。对多种开源和闭源VLMs进行系统评估，并通过区分可回答性和模型置信度进行诊断分析。

Result: 现有模型不仅在前瞻性和回顾性建模方面存在困难，还表现出严重的校准不当问题：模型置信度往往无法追踪内在可回答性，导致出现危险的"自信但错误"行为。

Conclusion: 当前视觉语言模型尚未准备好进行流式交互理解，需要在时间推理能力和置信度校准方面进行显著改进。该基准为未来VLM研究提供了重要的评估标准和改进方向。
...
TL;DR: 研究病理图像编码器网络对图像变换的鲁棒性，评估原始图像与变换后图像的嵌入向量相似度


<details>
  <summary>Details</summary>
Motivation: 病理图像分类任务中，编码器网络需要捕获生物学/诊断相关信息，同时对标签无关的图像变换（如数据增强）保持不变性，以提升模型性能

Method: 使用Lunit、Bioptimus和Meta提供的病理专用及通用编码器网络，对结直肠组织和TCGA数据集的H&E染色切片进行变换，通过比较原始图像、变换图像和随机图像的嵌入向量相似度来评估鲁棒性

Result: 原始图像与变换图像的嵌入向量相似度高于随机嵌入向量，表明编码器具有一定鲁棒性但未完全实现变换不变性；病理专用编码器与通用编码器之间存在显著差异

Conclusion: 编码器网络无法完全消除变换对潜在空间的影响，这解释了基于变换的数据增强能够提升模型性能的现象
...
TL;DR: 研究提出PCFM方法，利用连续时间生成模型进行医学点云补全，在SkullFix、SkullBreak和Mandibular Defect数据集上实现先进生成性能，比扩散模型需更少采样步骤，PTv3骨干可提速7倍。


<details>
  <summary>Details</summary>
Motivation: 医学点云补全对解剖重建和下游临床工作流重要，但该场景下的生成建模研究不足，需探索更高效的补全方法。

Method: 提出PCFM——基于PTv3的流匹配方法用于医学点云补全；构建PTv3确定性编码器-解码器基线和PCDiff扩散补全模型作为对比。

Result: PCFM+PTv3与确定性PTv3基线相当，在各数据集上实现先进生成性能；比扩散模型需显著更少采样步骤；PTv3提供最高7倍提速；更高点分辨率带来一致性能提升。

Conclusion: PCFM结合PTv3在医学点云补全任务上兼具高效性和生成质量，为该领域提供了有竞争力的解决方案。
...
TL;DR: S1-Omni-Image是一个开源的统一多模态模型，专注于科学图像的理解、生成和编辑。它基于S1-VL-32B推理骨干，结合图像生成模块，采用"先思考后生成"的范式，支持科学图像的多种任务。


<details>
  <summary>Details</summary>
Motivation: 科学图像任务不仅需要高保真合成，还需要理解科学语义、结构关系、领域知识和任务意图。现有通用图像生成模型无法满足这些需求。

Method: 基于S1-VL-32B科学多模态推理骨干，结合图像生成模块，采用统一的think-before-generate范式。模型先产生任务导向推理轨迹、文本答案和任务特殊token，其隐藏状态注入生成模块进行条件图像生成或编辑。

Result: 在GenExam和TechImage-Bench上优于开源模型；在MSD、cigRockSEM、SynthRAD2025和IXI四个编辑基准上达到最先进水平；并保持稳定的科学图像理解能力。

Conclusion: S1-Omni-Image在科学图像生成和编辑方面显著提升，同时保留了继承自S1-VL-32B的科学图像理解能力，为科学图像领域提供了统一的解决方案。
...
TL;DR: P-MTP是一种针对文档解析的渐进式多令牌预测框架，通过轻量级MTP模块扩展look-ahead深度，结合渐进课程损失和置信度门控动态草稿实现高达5倍推理加速。


<details>
  <summary>Details</summary>
Motivation: VLMs在文档解析中实现端到端映射但存在显著延迟瓶颈，特别是对于token密集型文档。现有多令牌预测(MPT)方法在扩展look-ahead深度时存在优化不稳定问题。

Method: 提出两种关键技术：1)渐进课程损失使用累积路径可靠性和回顾目标一致性自适应重新加权不同look-ahead深度；2)置信度门控动态草稿在推理时自适应调整投机长度。

Result: 在多个基准测试和架构上实现高达5倍推理加速，精度损失可忽略不计，首次成功验证了在文档解析领域扩展look-ahead MTP的可行性。

Conclusion: P-MTP通过渐进式easy-to-hard优化转换和动态推理策略，有效解决了MTP在文档解析中的优化不稳定问题，为高吞吐量文档解析提供了高效的加速方案。
...
TL;DR: 本文提出SENTRY模块，解决SAM2跟踪器在遮挡、快速运动和干扰物下的漂移问题。该模块通过验证记忆更新的时间一致性来选择掩码，而非仅依赖置信度。


<details>
  <summary>Details</summary>
Motivation: SAM2跟踪器的记忆更新机制采用置信度-only的掩码选择策略，导致在遮挡、快速运动和干扰物下出现漂移，这是当前方法的主要瓶颈。

Method: SENTRY是一个无需训练、即插即用的模块，在写入记忆前验证短时域时间一致性。它聚合每帧的多样化分割假设，将它们回溯为短轨迹，并使用邻域感知的循环一致性匹配来选择时间 和几何一致的掩码。

Result: 集成到五个基线模型中，SENTRY在九个基准上取得一致性提升，在LaSOT、LaSOT_ext、GOT-10k、VOT20、VOT22和DiDi上实现新的零样本SOTA。SAM2-L在A100上运行速度为32.8 FPS，仅增加0.4-0.6 GB VRAM。

Conclusion: 在写入时强制执行时间有效性可以稳定记忆增强跟踪，且无需重新训练。SENTRY是首个统一的全尺度评估方案。
...
TL;DR: 本文提出LAS2，一种超快速的高效零样本立体匹配模型系列，通过2D成本聚合架构和三阶段训练策略，在高效 stereo 方法中达到最优精度，且比Fast-FoundationStereo快1.8-2.7倍。


<details>
  <summary>Details</summary>
Motivation: 现有高效stereo模型虽推理速度快但零样本泛化能力弱，作者希望挑战这一假设，证明高效模型也可实现强大的零样本泛化。

Method: 架构上提出针对实际推理延迟优化的2D成本聚合框架；训练上采用合成监督、自蒸馏和真实知识蒸馏的三阶段策略，并引入伪标签过滤和误差限制操作。

Result: LAS2在高效stereo方法中达到最优零样本精度；LAS2-H比迭代方法Fast-FoundationStereo精度更高，在H200和Orin上分别快1.8倍和2.7倍。

Conclusion: 高效模型可实现强大的零样本泛化，LAS2在精度和速度之间取得良好平衡，适合资源受限平台部署。
...
TL;DR: GeoLaV是一个两阶段框架，通过从图像中提取3D几何知识来增强文本驱动的视频目标分割。它首先在单图像数据集上进行单目几何预训练，然后进行几何感知蒸馏，在视频分割数据集上进行微调。


<details>
  <summary>Details</summary>
Motivation: 现有的文本驱动引用视频分割(RVOS)模型通常在2D图像或视频数据集上训练，使用简单的分割损失，忽略了帧间的几何一致性，导致空间理解能力较弱。

Method: 第一阶段：单目几何预训练 - 使用单目新视角合成，使模型通过大规模单图像数据集上的空间对齐获得几何一致的视觉表示。第二阶段：几何感知蒸馏 - 将3D结构知识从通用3D先验模型迁移到视频分割数据集上，增强3D感知并提高时空一致性和语言定位能力。

Result: 仅使用图像分割数据就能在RVOS中实现显著的零样本泛化。当结合几何感知蒸馏进行视频微调时，在多个RVOS基准数据集上达到最先进性能。

Conclusion: 通过两阶段几何增强框架，成功地将3D几何知识从图像迁移到视频分割任务，显著提升了文本驱动视频分割的性能和泛化能力。
...
TL;DR: 本文提出video-SALMONN-R³，首个端到端的视频大语言模型，通过强化学习实现"重看"机制，无需链式思考冷启动。


<details>
  <summary>Details</summary>
Motivation: 视频LLM受计算和内存预算限制，常使用降低的帧率和空间分辨率，导致问答时可能遗漏关键信息。

Method: 提出两阶段范式：先进行粗粒度视频理解定位相关片段，再以更高时空分辨率重看这些片段。引入重答策略使模型首次观看直接给出答案，重看后修正；引入重问机制在重看时重新注入查询。

Result: 一致优于基线模型和QA-SFT基线，超越先前重看方法且计算成本显著降低。

Conclusion: 该工作证明了无需CoT冷启动即可实现有效的重看推理，为高效视频问答提供了新思路。
...
TL;DR: 提出了MambaRaw框架，利用状态空间模型(SSMs)从JPEG预览图高效重建原始raw图像，解决了高分辨率下注意力机制计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有元数据重建框架在处理高分辨率(如4K)图像时，注意力机制的计算复杂度呈二次方增长，导致实际应用受限，需要更高效的上下文建模方法。

Method: 提出空间能量耦合上下文建模机制，包含两个轻量模块：1) TileMambaBlock使用Mamba风格选择扫描仅处理信息密集块；2) Energy-Aware Refinement身份初始化残差模块匹配raw信号的长尾能量分布。

Result: 在Sony、Olympus、Samsung三个相机数据集上相比强基线方法取得一致提升，PSNR增加1.2-1.4 dB，编解码延迟降低约9%，在低元数据比特率下效果显著。

Conclusion: MambaRaw在JPEG引导的raw图像重建任务中达到了新的最先进水平，具有出色的效率优势，为元数据压缩提供了一种有效方案。
...
TL;DR: WATER是一个新兴的WordArt（艺术字）场景文字识别任务，比普通场景文字识别（STR）更具挑战性。研究者构建了200万规模的合成数据集WATER-S，并提出了WATERec模型，在WordArt-Bench上达到90.40%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的STR数据集和方法主要针对规则场景文字和固定模板输入，难以扩展到WordArt任务。WordArt具有高度定制化的字体、纹理和布局，需要新的数据和方法来推动该领域发展。

Method: 数据方面：构建了WATER-S合成数据集，包含两个子集——使用升级版渲染管道SynthWordArt生成的高精度可控合成数据，以及结合Qwen3-VL提示挖掘和Z-Image图像合成的多样化真实数据。模型方面：提出WATERec模型，采用支持任意形状输入的视觉编码器和自回归解码器来建模复杂布局。

Result: WATERec架构优于现有STR方法，在WordArt等不规则文字上达到最先进的性能。结合重新整理的WATER-R真实数据，新合成数据和模型设计在WordArt-Bench上达到90.40%的准确率，显著超越通用视觉语言模型和专用OCR模型。

Conclusion: 通过大规模合成数据和创新模型架构的结合，成功推动了WordArt识别任务的发展，为该领域建立了强大的基线方法。
...
TL;DR: RetiSEM是一个领域约束的结构方程模型(SEM)框架，用于在多模态数据有限的情况下进行因果图恢复和中介分析。


<details>
  <summary>Details</summary>
Motivation: 从碎片化生物医学数据学习因果模型具有挑战性，因为临床、分子和影像变量通常不完整或无法联合观测。

Method: 将变量组织成生物学信息块，应用禁止边约束，并将通路级效应分解为总效应(TE)、自然直接效应(NDE)和自然间接效应(NIE)成分。在十个合成基准场景和结合NHANES临床变量与外部视网膜表示的真实碎片化环境中进行评估。

Result: 在合成基准测试中达到比无约束基线更低的结构误差和更高的因果准确性。真实数据分析显示视网膜变量主要作为下游生物标志物指示物，间接效应较小但可检测。

Conclusion: 该方法为有限资源生物医学AI中测试结构化因果假设提供了一个可解释的框架。
...
TL;DR: VistaRef是一个用于自然图像中指示手势定位的框架,通过局部手部实体建模、几何射线建模和方向一致性对齐损失来增强空间方向感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer视觉模型全局注意力机制常忽略微观几何关系,导致无法准确捕捉手指姿态暗示的指向射线,在处理遥远或密集物体时出现指向漂移和定位模糊,影响AR和人机协作中的空间交互。

Method: 提出三模块框架:(1)LHEM模块融入手部姿态嵌入以增强捕捉细微手指偏差;(2)GRM模块受多视图几何启发,将隐式方向信息转化为显式空间几何特征,通过注意力机制引导特征聚合和深度融合;(3)OCAL损失协同监督手部存在和指向一致性。

Result: VistaRef显著优于基线模型,在定位精度上获得14个百分点的绝对提升,定性分析证实其有效建模了从手到目标的几何相关性。

Conclusion: VistaRef通过显式增强空间方向感知能力,弥合了传统Transformer在复杂场景下的空间感知差距,为AR和人机协作中的无缝空间交互提供了基础。
...
TL;DR: 本文提出了一种几何驱动的无标注框架，利用事件流中的自我运动结构将检测到的目标分类为静态或独立运动目标，无需学习或人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有汽车事件数据集依赖基于外观的帧级标注，不适合运动感知事件感知任务。

Method: 提出带偏航补偿的扩展焦点(FOE)模型估计全局背景运动，通过尺度不变残差量化局部运动与预测的偏差，使用时间稳定化提高鲁棒性。

Result: 在MVSEC和Prophesee 1百万像素汽车检测数据集上验证，在不同驾驶场景表现一致，偏航补偿改善转弯性能，简化平移局部模型提供良好的精度-效率权衡。

Conclusion: 该方法无需学习、无需人工运动标注，适用于任何输入边界框，为事件相机运动感知提供有效解决方案。
...
TL;DR: 本文提出了一种后验传输视角来理解和改进基于流匹配的图像逆问题求解方法。核心发现是：对于确定性流先验，贝叶斯后验可以通过简单重加权源分布获得，而非添加漂移校正。基于此理论分析，作者揭示了FlowDPS/FLOWER/PnP-Flow等方法实际上是同一校正场的不同零阶/高斯/近端近似，并提出了新的速度校正求解器。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free求解器（FlowDPS、FLOWER、PnP-Flow等）通过在概率流ODE中添加测量引导项来求解逆问题，但学界对这些逐步校正实际近似什么、以及结果与真实后验p(x|y)的差距缺乏理论理解。这严重阻碍了方法的系统改进。

Method: 1. 建立后验传输理论框架：证明对于确定性流先验，贝叶斯 conditioning完全由源分布的重加权实现
2. 将轨迹引导求解器解释为将无条件源映射到后验的最小动能校正场
3. 将FlowDPS/FLOWER/PnP-Flow解析为该校正场的不同阶近似（零阶/高斯/近端）
4. 在Wasserstein距离下推导后验偏差上界
5. 在2D闭合形式后验上验证理论，并在AFHQ和CelebA数据集上测试新方法

Result: 1. 源分布重加权在所有指标上与真实后验一致（达到蒙特卡洛基线）
2. 轨迹引导方法产生200-800倍更大的误差，并 collapse后验模式
3. 新提出的速度校正求解器在域内和域外设置中表现 competitive，同时保持多样化的后验样本和与重建误差相关的 uncertainty

Conclusion: 本文的理论框架统一解释了现有流-based逆问题求解器，揭示了其核心偏差来源。源重加权是原则性解决方案，而轨迹引导的逐步校正策略存在根本性局限。提出的速度校正求解器在理论上更稳健，为实际应用提供了更好的折中方案。
...
TL;DR: VisCritic是一种视觉过程奖励框架，通过比较操作前后的截图在视觉特征空间中进行验证，解决GUI智能体在长程场景中缺乏步骤级验证的问题。


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体在长程任务中频繁失败，原因是缺乏步骤级验证。现有过程奖励模型仅通过文本推理验证，忽略了GUI状态变化的视觉本质。

Method: 采用孛生视觉变压器提取变化感知的表示，连接动作感知评论头联合评估动作成功、任务进展和错误类型。构建批评训练数据管道，从现有轨迹生成弱监督样本。

Result: 在五个基准测试中作为即插即用的增强组件，通常改善基准指标，同时提供视觉诊断线索。

Conclusion: VisCritic能有效验证GUI智能体的动作，通过视觉特征空间比较弥补了文本推理的不足，为GUI自动化提供了新的验证范式。
...
TL;DR: ForensicsTok将图像篡改定位重新定义为自回归序列生成任务，通过Token Splatting Decoder(TSD)和 Hierarchical Expert Fusion(HEF)模块实现精确的mask预测，解决了MLLMs在 forensic 任务中的定位不佳和信息瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有使用外生分割解码器的MLLMs方法存在定位不优的问题，stitched pipelines 引入信息瓶颈，稀释空间信号，且受限于分割器的语义先验。

Method: 1)将图像篡改定位重新定义为自回归序列生成任务，直接生成空间锚定的token序列；2)TSD通过codebook-aware code smoothing将token映射到二进制mask；3)HEF模块从forensic expert model注入多尺度特征。

Result: 在6个基准数据集上大幅超越现有MLLMs基线方法，轻微超越强forensic expert基线，并表现出更强的抗扰动鲁棒性。

Conclusion: 该方法成功补偿了标准MLLMs缺乏forensic先验的问题，通过统一的架构实现了精确的图像篡改定位。
...
TL;DR: PointVG-R是一种用于指点式视觉定位的推理增强多模态大语言模型，通过几何感知推理管道和自适应重要性加权策略，在mIoU上超越基线15.86个百分点，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法仅在语言域进行推理、忽略图像中丰富感知线索和空间几何信息的缺陷，提升模型解读手势空间关系的认知能力。

Method: 1) 设计几何感知推理管道模拟人类迭代认知过程；2) 构建EgoPoint-CoT视觉思维链数据集；3) 结合监督微调(SFT)和强化学习(RL)进行训练；4) 提出基于群体方差的自适应重要性加权策略动态调整奖励信号。

Result: 在mIoU指标上超越基线15.86个百分点，达到当前最优性能(SOTA)。

Conclusion: 几何感知推理和Adaptive Importance Weighting策略能有效提升指点式视觉定位任务性能，为多模态大语言模型的推理增强提供了新思路。
...
TL;DR: CF-World是一个反事实基准测试，用于评估文本到图像生成模型是否真正理解因果关系，还是仅进行模式匹配。基准包含三个递进层级，测试模型能否克服真实世界先验生成反事实图像。


<details>
  <summary>Details</summary>
Motivation: 受罗素归纳主义火鸡的启发，探究T2I模型的成功是源于真正的因果理解还是仅对视觉-文本相关性的复杂模式匹配。当前缺乏评估模型因果推理能力的基准。

Method: 构建CF-World反事实基准，包含三个层级：(1)基于普通世界知识的事实生成，(2)带直接视觉指令的显式反事实生成，(3)需要从改变规则进行因果推理的隐式反事实生成。使用基于VLM的CF-Eval评估器，引入PRR（先验抵抗率）和RRR（推理保留率）两个指标。

Result: 所有模型在从事实设置到反事实设置时表现出明显性能下降。分析表明失败原因是当前T2I模型将世界知识与视觉外观编码为紧密耦合的模式，依赖训练数据中的频繁视觉共现，在生成反事实世界时默认使用熟悉的常识先验。

Conclusion: 当前T2I模型缺乏真正的因果理解能力，本质上是高级的模式匹配器。未来需要开发能够解耦世界知识与视觉外观表示的模型，使其能够根据改变的规则进行因果推理而非依赖先验。
...
TL;DR: 本文研究异构知识蒸馏（HKD）中的训练不稳定问题，提出SPOFA框架，通过特征和梯度双重稳定机制实现跨架构的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 异构知识蒸馏旨在不同架构（Transformer到CNN）间传递知识，但面临严重的训练不稳定问题，影响知识迁移效果。

Method: 提出SPOFA框架，包含两部分：1) LayerNorm-based解耦投影器，解耦特征幅度与方向，创建稳定的语义对齐空间；2) 动量驱动的MEMA动态缩放器，建立优化轨迹历史基线，评估并惩罚有害的梯度冲突。

Result: SPOFA在两个主流基准测试中达到最先进的准确率，显著优于计算开销大的方法，同时相比标准基线仅引入极小的计算开销。

Conclusion: 通过特征和梯度双重稳定机制，SPOFA成功实现了稳定的跨架构知识蒸馏，在保持轻量级参数的同时达到优异性能。
...
TL;DR: 提出了一种基于QUBO的灰度CT重建框架，结合动态区间编码和先验平衡优化，解决传统方法中二进制变量预算受限导致的QUBO规模膨胀与量化误差之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有QUBO-CT方法面临二进制变量预算约束问题：固定全局比特平面编码随着灰度精度提高导致QUBO规模增大和耦合复杂度增加，而低比特编码则引入量化误差，如何在有限变量预算下实现高质量灰度重建是核心挑战。

Method: 提出动态区间编码与先验平衡优化相结合的方法：1)每轮细化仅在当前估计值的局部灰度区间内对活跃像素进行编码；2)边界触达引导更新规则在搜索扩展和局部细化间自适应切换；3)在形成最终QUBO前平衡投影域数据一致性与边缘保持二次先验。

Result: 稀疏视角和有限角度扇束CT实验表明，该方法相比评估的解析法、迭代法、变分法和表示法基线方法能更准确地恢复结构和灰度分布。表达性分析和消融实验表明改进主要源于动态局部编码的有效灰度表示以及更稳定的数据保真度-先验耦合。D-Wave混合BQM求解器实验进一步验证了在硬件支持的混合量子-经典后端上的可执行性。

Conclusion: 通过动态局部编码实现有效灰度表示，并通过更稳定的数据保真度-先验耦合，该方法为量子计算辅助的CT重建提供了新的思路，在有限变量预算约束下实现了质量和效率的平衡。
...
TL;DR: PatternGSL是一种无模板、可学习的规格语言，用于从单张图像重建结构化服装。它将完整的缝制图案（包括面板边界、参数化接缝和显式缝合拓扑）编码为紧凑标准形式，解决了几何重建与结构化服装构建之间的表示差距。


<details>
  <summary>Details</summary>
Motivation: 当前方法存在根本性表示差距：模板无关方法虽能捕获表面几何但缺乏显式缝合结构用于模拟；而程序化系统虽能进行模拟但受限于预定义模板。论文旨在填补这一空白，将缝合结构作为生成建模的首要目标。

Method: 提出PatternGSL结构化服装表示语言，以及一个视觉语言框架直接从单张图像预测PatternGSL规格，并使用轻量级确定性有效性处理解码为服装，无需优化细化或手动清理。同时构建了PatternGSLData数据集，包含30万个图像-GSL配对样本。

Result: 实验表明在模式准确性上优于先前基线，能显式恢复缝合结构，实现可靠的布料模拟，并通过同一确定性解码管道实现模式级编辑。

Conclusion: PatternGSL成功实现了无模板依赖的结构化服装表示，保持了基于模式的模型的物理严谨性，同时将缝合结构提升为生成建模的首要目标，为服装重建提供了新的思路。
...
TL;DR: 提出了一种用于稀疏视图CT（SVCT）重建的多层级随机即插即用（ML-SPnP）方法，通过在多分辨率分析近似空间中进行多层级处理，避免了精细层级先验梯度的昂贵估计，从而在保持重建质量的同时显著降低运行时间。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图CT可减少辐射暴露和采集时间，但有限的投影视角使得重建问题严重病态，使用解析方法会产生条纹伪影。现有的即插即用（PnP）和随机PnP方法虽然能有效结合数据保真度与学习先验，但通常需要大量迭代才能收敛，限制了其实际效率。

Method: 提出多层级随机PnP方法（ML-SPnP），在多分辨率分析（MRA）近似空间中进行多层级步骤。小波分解的结构使得先验一致性修正（在期望意义上）为零，从而避免了为粗层级修正进行精细层级随机先验梯度的昂贵估计。核心创新在于利用MRA空间的特性消除精细层级先验梯度估计的成本。

Result: 实验表明，ML-SPnP方法在稀疏视图CT重建中达到了与最先进方法相当的重建质量，同时显著降低了运行时间。

Conclusion: 通过在多分辨率分析近似空间中执行多层级步骤，ML-SPnP成功加速了随机PnP重建，为实际临床应用中平衡图像质量与计算效率提供了有效解决方案。
...
TL;DR: ConQuer是一种新的图像-文本预训练方法，通过概念查询将CLIP的单一代币对齐扩展为多个局部对齐，用于训练3D CT医学基础模型Jolia


<details>
  <summary>Details</summary>
Motivation: 医学图像涉及多个器官，放射学报告比普通图像 captions 更长且结构复杂，CLIP风格的预训练将每种模态编码为单个全局代币，可能丢失重要细节

Method: 将报告分成概念特定的部分，学习跨注意力查询来池化匹配的图像特征，无需分割掩码或空间监督，然后对每个概念独立应用对比学习

Result: Jolia在发现分类、报告生成和跨中心迁移方面始终优于CLIP基线，并在多个公共基准上达到最先进水平

Conclusion: ConQuer方法成功地将全局对齐扩展为局部对齐，为每个概念学习注意力图，提供内置的空间可解释性，Jolia作为3D CT基础模型表现出色
...
TL;DR: 本文提出EERLoss：一种可微的、任意精度的等错误率(EER)近似损失函数，用于训练深度生物特征验证模型，以解决传统间接目标与最终评估指标之间的对齐问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习生物特征验证模型通常通过优化间接目标进行训练，这与主要的评估指标等错误率(EER)存在不对齐问题，需要一种直接针对EER优化的训练目标。

Method: 提出EERLoss损失函数，这是一个可微的、任意精确的EER近似方法，可直接用于训练深度生物特征模型。该框架还具有潜力适配到DET曲线上的任何特定操作点。

Result: 在KVC-onGoing大规模基准数据集(超过185,000名受试者)上验证，实验表明：1)EERLoss优于现有最先进的损失函数；2)收敛速度更快，降低了总体训练成本；3)与原始最先进方法相比，EER相对降低最高约30%。

Conclusion: EERLoss作为一种任务对齐的训练目标，在高方差的生物特征 trait 上非常有效，在具有挑战性的大规模基准测试中验证了其有效性。
...
TL;DR: 本文提出ViTexQA视频文本问答数据集和FrameThinker多帧时序推理模型，解决现有MLLMs在跨帧文本理解方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频文本理解上存在不足：现有数据集的问答对大多可从单帧回答，无法真实反映跨帧文本融合的时序推理需求。

Method: 构建ViTexQA数据集，采用带时序约束的思维链注释流程；提出FrameThinker模型，采用两阶段训练：思维链引导的监督微调生成帧感知推理链，基于多帧一致性奖励的时序强化学习。

Result: 方法在ViTexQA上优于SOTA基线，ROUGE-L提升6.3%。

Conclusion: 通过跨帧文本融合的时序推理机制，显著提升了视频文本问答性能，为多模态理解中的时序建模提供了新思路。
...
### [93] [Evaluating the Interpretability of Sparse Autoencoders with Concept Annotations](https://arxiv.org/abs/2606.24716)
*Jonas Klotz,Cassio F. Dantas,Pallavi Jain,Diego Marcos,Begüm Demir*

Main category: cs.CV

TL;DR: 本文提出了一个基于人类标注概念的SAE（稀疏自编码器）评估框架，用于量化SAE潜在表示与人类注释概念之间的对齐程度，并在视觉领域引入了干预式评估方法。


<details>
  <summary>Details</summary>
Motivation: 现有SAE评估方法主要依赖代理指标或定性检查，缺乏衡量语义对应性的量化方法，且不需要用户研究即可验证概念匹配。

Method: 构建了synCUB和synCOCO两个合成基准数据集（成对图像仅在一个属性上不同）；提出了Fully-Binary Matching Pursuit (FBMP)基于联盟的匹配程序支持多对一映射；提出了TAPAScore（目标属性扰动对齐分数）测试匹配概念在目标扰动下的选择性响应。

Result: FBMP一致优于一对一基线方法；TAPAScore是唯一能可靠区分训练与未训练SAE的指标；在CLIP和DINOv2嵌入上训练的SAE中，过度过完备性会降低扰动对齐，表明可解释性下降。

Conclusion: 中等字典大小提供最佳权衡，可产生最可解释的SAE；评估框架表明适度过完备性是最优选择。
...
TL;DR: 该论文提出一种协作式多智能体框架，通过规划智能体和感知智能体的闭环迭代来解决零样本3D理解任务，在6个基准数据集上取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D理解重新表述为视频关键帧理解，但存在内在瓶颈：视频的有限观察视角和3D场景的隐式感知限制了理解效果。

Method: 提出协作式多智能体框架：(1)规划智能体负责高层视角规划和补充新视角；(2)感知智能体将3D场景显式总结为结构化整体认知图，并跨视角分配一致实例标识符。两智能体通过闭环迭代协作直至任务完成。

Result: 在6个基准数据集上取得最先进性能：ScanRefer上Acc@0.5提升11.1%，3D辅助对话上BLEU-1提升14.6，SQA3D上EM提升2.1。

Conclusion: 该协作式多智能体框架有效解决了有限视角和隐式3D感知的瓶颈问题，通过规划与感知智能体的迭代协作显著提升了零样本3D理解性能。
...
TL;DR: UniDrive是一个统一的可视化-语言和接地框架,用于自动驾驶场景中的可解释风险理解。该框架结合了时间推理分支和高分辨率感知分支,通过门控交叉注意力融合模块整合,联合生成自然语言风险描述和风险目标的边界框输出。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶多模态大语言模型在时间推理和空间精度之间存在根本性权衡:单帧或低分辨率输入容易遗漏小、远或部分遮挡的风险,而语言中心驾驶模型的接地证据有限。

Method: UniDrive包含:(1)时间推理分支,从多帧视觉输入建模场景动态;(2)高分辨率感知分支,从最新帧保留细粒度空间细节;(3)门控交叉注意力融合模块,动态对齐上下文与精确空间证据。基于融合表示,联合生成自然语言风险描述和风险目标边界框。

Result: 在DRAMA-Reasoning基准上,UniDrive在字幕生成和风险目标接地方面优于代表性图像和视频基线,在验证集上获得最佳整体性能,在小目标定位、零样本泛化(NuScenes和BDD100K)以及人类评分的可解释性和可信度方面展现明显优势。

Conclusion: 显式结合时间语义和高分辨率感知为可解释且安全导向的自动驾驶系统提供了更强基础。
...
### [96] [EG-VQA: Benchmarking Verifiable Video Question Answering with Grounded Temporal Evidence](https://arxiv.org/abs/2606.24797)
*Linpeng Huang,Weixing Chen,Zexin Chen,Yang Liu,Liang Lin*

Main category: cs.CV

TL;DR: 本文提出了EG-VQA基准，用于评估视频问答模型在证据定位方面的能力，揭示了现有模型在答案正确性和证据定位之间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答基准主要评估答案正确性，而忽视了预测结果与相关视频证据的对齐问题，导致模型虽然能给出正确答案但无法准确地将答案定位到视频中的具体时间片段。

Method: 构建包含2,067个视频和11,838个问答对的EG-VQA基准，每个问答对都标注了支持性时间证据；提出EG-F1统一指标联合测量时间对齐和语义一致性；提出EG-Reasoner模型进行证据定位推理。

Result: 实验表明即使是强大的专有模型也难以准确进行证据定位，EG-Reasoner在开源模型中达到最先进性能，在反事实等推理密集型任务上提升显著。

Conclusion: 仅靠规模扩展不足以实现鲁棒的视频理解，结构化的证据监督对开发更可靠、可解释的视频问答系统至关重要。
...
TL;DR: 本文提出语义证据奖励(SER)方法,通过引入 referee VLM 作为局部检查器,评估模型生成的证据声明的相关性和定位质量,结合时间惩罚来增强视频多模态大语言模型的时空推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态大语言模型在细粒度时空推理方面存在困难,常常基于无关帧或物体生成正确答案。现有的强化学习框架仅依赖几何IoU奖励,对边界扰动敏感且忽视语义对齐。

Method: 将时空证据接地重新表述为约束验证任务,使用 referee VLM 作为局部检查器从两个维度评估模型生成的证据声明:相关性和定位质量,并结合时间惩罚。该设计减少了对密集边界注释的依赖,可直接在标准视频问答数据上进行训练。

Result: 在 V-STAR 基准上,SER 达到 49.6% mLGM,比强证据接地基线 Open-o3-Video 提升 3.0 个百分点。

Conclusion: SER 方法在增强答案准确性和证据接地能力方面展现出显著潜力,为视频多模态大语言模型的时空推理能力提升提供了有效途径。
...
### [98] [OrbitForge: Text-to-3D Scene Generation via Reconstruction-Anchored Video Synthesis](https://arxiv.org/abs/2606.24799)
*Chenrui Fan,Paolo Favaro*

Main category: cs.CV

TL;DR: OrbitForge是一种将单文本生成视频转换为闭合轨道3D Gaussian Splatting场景的适配器，通过冻结视频先验和每提示高斯溅射重建优化实现，无需任务特定的视频或多视图微调。


<details>
  <summary>Details</summary>
Motivation: 现有文本转视频模型虽能生成高质量视频，但无法直接生成可靠的3D资产：相机运动难以控制、视角覆盖不完整、帧间存在时间不一致性问题。

Method: OrbitForge使用Deformable Gaussian Splatting和稳健的MedianGS代理从初步生成的视频获取3D重建，通过预置轨道渲染视图以检测缺失视角，然后利用文本转视频模型仅补全缺失视角，最后将完整轨道重建为最终Gaussian Splatting场景。

Result: 在300提示的T3Bench-derived审核集上，OrbitForge重建达到359.0度测量中位跨度，将原本不支持的Q10 ImageReward从8.07提升至16.36，同时在覆盖质量方面与VideoMV保持竞争力。

Conclusion: 该方法无需任务特定的视频或多视图微调，避免了每提示分数蒸馏优化，需要覆盖感知评估来奖励完整轨道的生成方法。
...
### [99] [VSANet: View-aware Sparse Attention Network for Light Field Image Denoising](https://arxiv.org/abs/2606.24737)
*Gargi Panda,Soumitra Kundu,Saumik Bhattacharya,Aurobinda Routray*

Main category: cs.CV

TL;DR: VSANet是一种用于光场图像去噪的视图感知稀疏注意力网络，通过局部敏感哈希实现的稀疏注意力机制有效利用光场数据中的跨视图相关性，在去噪性能上优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 光场图像因数据高维结构导致去噪困难，虽然子孔径图像间的噪声是独立的，但场景内容存在强跨视图相关性，现有方法难以有效利用这些相关性。

Method: 提出视图感知稀疏注意力(VSA)模块，将4D光场特征图表示为统一的空间-角度token空间，通过局部敏感哈希实现稀疏注意力进行跨视图聚合，实现线性复杂度的全局特征交互；同时设计特征细化(FR)块强调空间、角度和极线子空间中的信息特征；VSA和FR模块集成在顺序注意力细化模块中形成VSANet核心。

Result: 实验表明VSANet在光场去噪任务上优于现有最先进的光场去噪方法。

Conclusion: 通过视图感知稀疏注意力和特征细化机制，VSANet能够有效捕捉和利用光场数据中的跨视图相关性，实现高效高质量的光场图像去噪。
...
### [100] [IV-CoT: Implicit Visual Chain-of-Thought for Structure-Aware Text-to-Image Generation](https://arxiv.org/abs/2606.24849)
...
TL;DR: 本文提出IV-CoT框架，通过将视觉推理分解为结构-语义级联过程，解决多模态大模型在结构感知提示跟随方面的缺陷，无需在推理时进行草图提取即可实现优越的图像生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态大语言模型在文本到图像生成方面取得了高质量，但在结构感知提示跟随方面存在困难，具体表现为难以保持对象数量、空间关系、属性绑定和粗略布局。作者认为这主要是因为结构规划和外观渲染被纠缠在单一 conditioning 流中。

Method: 提出隐式视觉思维链(IV-CoT)框架，将视觉条件查询分解为结构到语义级联:结构查询首先形成潜在视觉计划，语义查询然后在该计划条件下渲染外观。为引导结构查询，作者引入了训练-only的草图监督方法，鼓励结构查询从草图中捕获结构信息，而无需在推理时进行草图提取或中间解码。IV-CoT在单次前向传播中执行隐式CoT推理。

Result: IV-CoT在GenEval和T2I-CompBench上实现了优越结果。可视化和分析表明学习到的结构查询和语义查询在结构感知生成中起到互补作用。

Conclusion: 通过将结构规划与外观渲染解耦并采用隐式视觉思维链推理，IV-CoT有效提升了模型的结构感知提示跟随能力，为多模态大模型的图像生成任务提供了新的研究方向。
...
### [101] [BioMedVR: Confusion-Aware Mixture-of-Prompt Experts for Biomedical Visual Reprogramming](https://arxiv.org/abs/2606.24740)
*Jiaxiang Liu,Tianxiang Hu,Juwei Guan,Yujie Wu,Yusong Wang,Yao Mu,Zuozhu Liu,Mingkun Xu*

Main category: cs.CV

TL;DR: BioMedVR是首个将视觉重编程(VR)应用于生物医学成像的框架，能够通过紧凑的可学习VR模块实现预训练视觉语言模型(VLM)的少样本适应。


<details>
  <summary>Details</summary>
Motivation: 将像CLIP这样的VLM适应到生物医学成像具有挑战性：全模型微调计算成本高，医学数据稀缺且类别差异细微。现有的VR方法主要关注正类提示而忽略混淆负类，在细粒度医学场景中导致预测校准不良。

Method: 提出三个创新点：1) Confusion Minimization Mechanism利用LLM生成的混淆感知属性和混淆抑制损失来减少假阳性对齐；2) Mixture-of-Prompt Experts结合正类专家和负类专家，通过自适应门控平衡；3) 紧凑的可学习VR模块实现参数高效适应。

Result: 在18个数据集上进行了广泛实验，包括11个生物医学数据集和7个自然图像基准，BioMedVR实现了优异的准确度和泛化能力。

Conclusion: BioMedVR有效地将VR和VLM桥接到生物医学领域，为少样本生物医学成像适应提供了有效的参数高效解决方案。
...
### [102] [FLUX3D: High-Fidelity 3D Gaussian Generation with Diffusion-Aligned Sparse Representation](https://arxiv.org/abs/2606.24874)
*Haorui Ji,Weizhe Liu,Hongdong Li,Hengkai Guo*

Main category: cs.CV

TL;DR: FLUX3D是一个可扩展的图像到3D高斯溅射(3DGS)生成框架,旨在解决稀疏体素表示在保留输入图像高频视觉细节方面的两个结构性瓶颈:表示瓶颈和跨模态对应瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前稀疏体素方法在从2D图像生成3DGS时,难以保留高频视觉细节,原因包括:(1)采用判别性2D特征优化语义抽象,抑制了重建线索导致表示瓶颈;(2)标准扩散变换器缺乏有效对齐密集2D图像令牌与稀疏3D体素 latent 的机制,导致跨模态对应瓶颈。

Method: 提出两个核心创新:(1)扩散对齐结构化latent (DA-SLAT) - 重新审视2D特征选择,结合解码器架构提高3DGS重建保真度;(2)稀疏结构感知扩散框架 - 集成稀疏结构多模态扩散变换器(SMDiT)和模态感知旋转位置嵌入(MARoPE),实现几何无关的2D-3D对齐。

Result: 在广泛基准实验中,FLUX3D在外观保真度方面取得实质性改进,并在生成高质量3DGS资产方面显著优于所有最先进(SOTA)方法。

Conclusion: FLUX3D通过改进表示学习和跨模态对齐,为图像到3DGS生成提供了一种可扩展的有效解决方案,在视觉质量方面超越了现有最佳方法。
...
### [103] [Adaptive Hebbian Memory Routing in Vision Transformers for Few-Shot Learning](https://arxiv.org/abs/2606.24756)
*Mohammed Yusuf Mujawar,Noorbakhsh Amiri Golilarz*

Main category: cs.CV

TL;DR: 针对少样本图像识别任务，提出自适应Hebbian路由方法，通过MLP路由器动态控制Hebbian记忆的贡献、更新强度和特征保留，提升视觉Transformer的少样本分类性能。


<details>
  <summary>Details</summary>
Motivation: 少样本学习需要从少量标注样本快速适应新类别，传统的Hebbian快速权重记忆采用固定策略，缺乏对不同任务的适应性。

Method: 提出三种自适应机制：自适应放置(控制记忆贡献)、自适应可塑性(控制更新强度)、完全自适应Hebbian路由(结合前两者)。使用轻量级MLP路由器动态生成控制参数，在ViT-Small、DeiT-Small、Swin-Tiny上验证。

Result: 在5-way 1-shot设置下，Swin模型：固定Hebbian 96.74%→自适应可塑性96.92%→完全自适应96.94%。推理时间从16.51ms降至14.05ms。CIFAR-FS上所有骨干网络均获提升，多shot场景仍有收益。

Conclusion: 自适应可塑性和自适应记忆激活能显著提升少样本Transformer性能，优于固定Hebbian行为，且可降低推理延迟。
...
### [104] [Compact Object-Level Representations with Open-Vocabulary Understanding for Indoor Visual Relocalization](https://arxiv.org/abs/2606.24767)
*Zhaopeng Cui,Jiarui Hu,Jingbo Liu,Boming Zhao,Xiyue Guo,Boyin Feng,Haocheng Peng,Yujun Shen,Hujun Bao,Guofeng Zhang*

Main category: cs.CV

TL;DR: 本文提出OpenReLoc系统，利用场景中的对象信息（语义、布局、几何）构建结构化地图表示，实现基于对象单元的室内相机重定位任务。


<details>
  <summary>Details</summary>
Motivation: 现有室内视觉重定位研究主要关注低层视觉方案，难以感知场景语义和组合，限制了可解释性和适用性。需要一种将丰富对象信息组织成结构化地图表示的方法来驱动相机重定位。

Method: 1) 引入多模态机制整合开放词汇语义知识实现2D-3D对象匹配；2) 设计面向对象的参考帧作为位置先验，并提出基于DIOU的参考帧选择策略以扩展到可扩展场景；3) 提出双路径2D迭代最近像素损失，由对象形状引导，确保稳定准确的姿态优化。

Result: OpenReLoc在各种数据集上实现了优越的重定位召回率和精度。

Conclusion: 通过将场景中的对象信息组织成结构化地图表示，使用对象单元驱动相机重定位任务，OpenReLoc能够提供场景理解和准确的姿态估计能力，并在多个数据集上验证了其优越性能。
...
TL;DR: 提出了GradAudit，一种基于梯度动态的VLLM训练数据审计框架，用于检测模型是否学习了特定的图像-文本关联训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有VLLM训练数据检测方法在跨模态场景下表现不佳，且依赖表层输出信号，判别力不足。医疗领域对患者隐私和版权保护有更高要求。

Method: 通过分析模型内部优化动态，观察训练样本梯度稳定且对齐、非训练样本梯度噪声和不一致的现象，提取梯度签名来检测真实的图像-文本关联。

Result: GradAudit在医疗和通用领域数据集上显著优于现有最先进基线方法，且对更先进、更新的模型检测效果更明显。

Conclusion: 基于梯度的方法比表层输出信号方法更能有效检测VLLM的训练数据使用情况，现有方法严重低估了未授权数据的使用程度。
...
TL;DR: AerialFusionMapNet是一种基于融合的地图构建框架，通过空中与车载传感器融合实现自动驾驳感知。该方法采用结构化两阶段训练策略，在统一管道中增强空中特征的贡献。在nuScenes地理分割上达到54.7 mAP，比之前的48.8 mAP基线提高了+5.9绝对值和+12.1%相对值。


<details>
  <summary>Details</summary>
Motivation: 高分辨率空中影像作为自动驾驳感知的补充模态出现，可改善BEV场景理解。传统端到端融合未能充分挖掘空中表示中的结构信息，需要更有效地整合空中特征以提升在线高清地图构建性能。

Method: 提出AerialFusionMapNet框架，采用结构化两阶段训练策略，明确增强空中特征在统一管道中的贡献，实现更有效的空中先验结构信息整合。

Result: 在nuScenes地理分割上达到54.7 mAP，比之前48.8 mAP的空中-车载融合基线提高+5.9绝对值和+12.1%相对值。

Conclusion: 研究表明，结构化训练设计而非增加架构复杂性，在释放空中影像用于在线高清地图构建的潜力方面起着更决定性的作用。
...
TL;DR: 本文提出了一种基于非平衡最优传输（Unbalanced Optimal Transport）的树木计数方法，通过空间密度匹配来解决卫星影像中的树木计数问题，并引入了自校正机制来处理噪声标注。实验在TinyTrees数据集上进行，该数据集覆盖三大洲、三种卫星传感器，包含超过2.15亿棵树木标注（其中77.3万棵经人工验证）。


<details>
  <summary>Details</summary>
Motivation: 从卫星影像中计数单棵树木是一个基础但尚未被充分探索的环境监测任务。在高分辨率影像中，密集森林的树冠边界变得模糊，单棵树木的定义变得不明确，而且大规模人工标注成本高昂。虽然 airborne LiDAR 可以提供可扩展的监督，但产生的标注有噪声且难以有效利用。

Method: 将树木计数问题形式化为空间密度匹配问题，并使用非平衡最优传输（Unbalanced Optimal Transport）进行监督。这种形式化自然地能够同时适应孤立树木的精确定位和密集森林中稳健的密度估计。进一步引入了自校正机制，利用传输残差在训练过程中逐步 refinement（精炼/修正）噪声监督。

Result: 在 TinyTrees 数据集上，我们的方法一致地优于基于检测、回归和传输的分布匹配基线方法，证明了非平衡传输和可靠性感知监督对于从卫星影像进行大规模树木计数的有效性。

Conclusion: 本文提出的非平衡最优传输方法能够有效地从卫星影像中计数树木，自校正机制能够处理噪声监督问题。该方法在包含超过2.15亿棵树木标注的 TinyTrees 数据集上取得了优异的性能，为大规模环境监测提供了新的技术途径。
...
TL;DR: 本文针对3D Gaussian Splatting在自动驾驶等大规模SLAM场景中面临的内存持续增长问题，提出了一种渲染面积感知修剪策略，通过移除对有效渲染面积贡献较小的Gaussian点来减少内存占用，在EuRoC和KITTI数据集上实现了超过60%的内存 reduction和2倍以上的FPS提升。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等大规模场景中，3DGS-SLAM面临关键限制：Gaussian点随时间不断累积，导致内存消耗持续增长，内存效率低下，限制了其实际应用。

Method: 提出渲染面积感知修剪策略，不依赖不透明度或梯度幅度等Gaussian级启发式方法，而是基于Gaussian对有效渲染面积的贡献来选择性移除Gaussian点。

Result: 在EuRoC和KITTI数据集上的评估表明，该方法在大型户外场景中始终优于现有修剪方法，实现了超过60%的内存 reduction和2倍以上FPS提升，同时保持了定位和建图精度。

Conclusion: 渲染面积感知修剪是3DGS-SLAM扩展到自动驾驶等真实世界场景的有前途方向，能够有效降低峰值内存占用。
...
TL;DR: DDStereo是一种新型双解码器立体Transformer，用于实时开放集3D目标检测，解决了 stereo 3D 方法精度与速度之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有stereo 3D方法虽精度高但推理速度慢，无法满足实时应用需求；同时stereo开放集检测尚未被探索。

Method: 提出Dual-Decoder架构，包含两个轻量级解码器分支：一个用于开放集前景2D检测，另一个用于3D属性回归。设计紧凑的视差特征提取器和简化解码器架构以提升推理效率。

Result: 在公共stereo 3D基准上达到最先进的精度，首次实现与单目方法相当的实时性能。

Conclusion: 成功填补stereo开放集3D检测的空白，兼顾高精度和实时性。
...
TL;DR: 本文提出了一种基于DDPM的合成TEM图像生成框架，通过渐进式补丁训练策略，仅用15个样本即可从零开始训练，实现了极端数据稀缺条件下的高质量TEM图像合成。


<details>
  <summary>Details</summary>
Motivation: 先进半导体节点对TEM的需求激增，但破坏性样品制备、慢成像和高成本限制了用于下游机器学习的多样化数据集的获取，现有的生成模型缺乏TEM特有的噪声、结构和随机变化。

Method: 提出渐进式补丁训练策略从低分辨率扩展到全图像，集成了自定义TrivialAugment适应、跨过程域迁移、分类器引导和RePaint风格修复，采用DDPM框架进行合成，并重新利用DDPM特征表示进行分割。

Result: 合成图像达到MS-SSIM > 0.98，定性专家评估与结构相似性结果一致，能够保留全局结构和空间关系，符合FAB计量要求，同时保持了统计和物理真实性。

Conclusion: 该框架成功实现了极端数据稀缺条件下的TEM图像合成，为下游ML训练提供了高质量数据支持，用于缺陷检测、分割和计量，同时保留了统计和物理真实性。
...
TL;DR: GeoT2V-Bench是一个基于重建的诊断基准，用于评估相机提示的文本到视频模型是否能支持刚性3D重建。该基准使用VGGT风格的几何估计、DeformableGS和MedianGS技术，通过连续重建分析来评估生成视频的几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉可信性评估不足以衡量相机提示的T2V模型生成虚拟相机捕获的效果，需要评估生成帧是否能为单一静态3D场景提供连贯的多视角证据。

Method: 通过VGGT风格几何估计估计每帧相机内参和姿态，拟合DeformableGS，通过时间中值聚合得到静态MedianGS代理，沿估计相机路径渲染该代理，并分析多种指标（图像运动、轨迹行为、静态渲染误差、流动一致性等）。

Result: 在12个开放权重模型配置和80个GeCo-Eval静态场景提示上完成3840次重建，发现可见运动、静态渲染误差、流动一致性和灵活vs静态行为之间经常不一致。

Conclusion: GeoT2V-Bench能够捕捉当生成视频被测试为全局静态场景采集时出现的互补失败模式，为评估T2V模型的几何一致性提供了更全面的诊断工具。
...
TL;DR: 本文提出Riemannian Residual Line Search方法，解决单步扩散编辑中编辑强度与源图像保留之间的矛盾。该方法通过估计提示 delta 场的局部时间曲率来构建更强的编辑，然后通过CLIP对齐度选择最终图像，在PIE-Bench++上实现了单步更新算法的最优性能。


<details>
  <summary>Details</summary>
Motivation: 单步扩散编辑器虽然快速，但面临一个核心张力：没有固定的更新强度能同时满足对目标提示的激进编辑和对源图像的保守保留。不同的编辑类型需要不同的更新强度，这是一个根本性的矛盾。

Method: 提出Riemannian Residual Line Search方法。首先通过估计提示 delta 场的局部时间曲率构建更强的编辑，并将校正后的方向投影回原始一阶能量场传输估计的更新范数。然后从源图像到这个强编辑形成一个小残差路径，保留原始一阶输出作为候选，通过最大化目标提示的CLIP对齐度来选择最终图像。

Result: 在700样本的PIE-Bench++评估集上跨10种编辑类型ID测试，该方法在当前单步更新算法中实现了最先进的（SOTA）性能。

Conclusion: 该方法成功地将单步扩散编辑中的编辑强度与源图像保留的矛盾转化为候选选择问题，通过曲率校正和CLIP对齐度选择有效地平衡了这两个需求，在多种编辑类型上取得了优异的整体性能。
...
TL;DR: 针对360度全景立体图像的视差估计问题，提出将球面立体图像先投影到等距矩形投影(ERP)格式，再使用RAFT+EACS方法进行视差估计的pipeline


<details>
  <summary>Details</summary>
Motivation: 全向立体图像违反经典视差估计的几何假设：球面或鱼眼视图中极线对应遵循弯曲的大圆路径，产生二维位移，无法直接作为单轴视差处理

Method: 采用球面到ERP的投影作为预处理步骤 straighten极线曲线并恢复一维视差结构；使用RAFT提取密集光流，再仅保留基线对齐的光流分量来得到视差

Result: 在合成鱼眼立体数据集上，该pipeline能产生准确、平滑且结构一致的视差图，且达到实时速度

Conclusion: 证实了ERP预处理可与RAFT+EACS方法有效结合，为将传统立体pipeline扩展到360度成像提供了直接可行的途径
...
TL;DR: FLAT提出一种从单张图像生成可探索3D场景的新方法，通过直接从视频扩散潜空间解码三角形splats（平面基元），而不是传统的3D高斯体。核心创新包括射线中心旋转参数化和产品窗口函数来解决三角形回归中的梯度流问题。


<details>
  <summary>Details</summary>
Motivation: 现有前馈潜场景解码器输出的3D高斯体缺乏明确定义的表面，难以用于模拟或标准图形管线。研究者希望从压缩的视频扩散潜空间中直接解码与表面对齐的基元，这些基元不仅可渲染，而且更接近显式几何资产。

Method: 提出两个关键技术：1）射线中心旋转参数化（ray-centered rotation parameterization），用于三角形回归；2）新型产品窗口函数（product window function），改善可微分三角形渲染过程中的梯度流。此外还引入轻量级测试时 refinement 步骤，将预测的三角形 soup 转换为完全不透明、可实时渲染的游戏引擎可用表示。

Result: 在标准基准测试上，FLAT实现了显著更好的几何精度，同时保持与最先进前馈基线相当的视觉质量。通过在相同训练设置下评估3DGS、2DGS和三角形splatting变体，提供了前馈场景生成中表示权衡的首次系统性分析。

Conclusion: 首次证明三角形splats可以直接从视频扩散潜空间解码。FLAT方法在几何精度上优于现有方法，并为3D场景生成中的表示选择提供了有价值的基准分析。
...
TL;DR: 开发了一个大规模开放基准数据集（85,355例CT扫描），系统评估12个肿瘤检测AI模型在不同肿瘤大小、位置、患者亚群和成像协议下的表现一致性，发现当前最优模型在罕见或代表不足的亚群（如年轻非裔美国女性）中表现不佳。


<details>
  <summary>Details</summary>
Motivation: AI在医学影像中虽取得显著成功，但模型在不同真实临床环境中表现不一致，这种不一致出现在患者人口统计学特征和成像协议不同时，如检测小肿瘤、分析不同对比期扫描或评估不同年龄或性别的患者。

Method: 构建包含85,355例CT扫描的大规模开放基准数据集，系统评估12个肿瘤检测AI模型；利用大语言模型（LLMs）从临床数据中提取和组织亚群信息，使分析具有可扩展性和可重复性。

Result: 基准测试揭示了当前最先进的AI模型（针对平均准确率优化）在罕见或代表不足的亚群中表现不佳，如年轻女性非裔美国人；收集足够标注数据用于这些罕见病例往往不切实际。

Conclusion: 该基准为构建更可靠、更稳健的肿瘤检测AI模型奠定了基础，并强调了在医学影像和计算机视觉中进行严格的亚群级评估的必要性。
...
TL;DR: NanoGen是一个统一的DiT训练和评估框架，支持ImageNet条件生成和文本到图像(T2I)两种任务，仅需12行配置更改即可切换。


<details>
  <summary>Details</summary>
Motivation: 当前DiT研究仅依赖ImageNet类别条件生成评估，但这种方法是否真正反映生成式建模的进展存疑。T2I任务因训练成本高常被跳过，作者认为这一认知已过时。

Method: 提出NanoGen框架，支持RAE、VAE、像素空间和MeanFlow扩散方法，涵盖ImageNet和T2I两种训练设置，训练计算量相当。

Result: 训练21个潜在扩散模型后，发现ImageNet与T2I生成的方法排名无强相关性，Pearson相关系数为-0.377至-0.580。提出DiffusionBench综合基准。

Conclusion: 建议用DiffusionBench替代ImageNet单独评估，这样改进的方法更可能反映更广泛的进展。
...
### [117] [Emergent Relational Order in LLM Agent Societies: From Collective Affect to Authority Stratification](https://arxiv.org/abs/2606.23764)
*Zhiyuan Ji,Xinyu Chen,Ziqi Dai,Shiyun Tang,Chunyu Wei,Yueguo Chen*

Main category: cs.MA

TL;DR: 本文提出CAREB-MAS多智能体框架，将费孝通的"差序格局"理论进行机制化操作化，通过LLM模拟长周期社会结构演化。


<details>
  <summary>Details</summary>
Motivation: 现有研究将差序格局视为文化特定概念，缺乏机制化基础；此前LLM模拟主要解决短期协调问题而非长期社会结构演化。

Method: 框架整合情感控制理论、社会认同理论和涂尔干集体情感，Agents通过情感-伦理-信念链推理，维持动态自我中心身份，宏观环境仅指定个体生产、偏好分配和最小交互协议。

Result: Agents自发再现五种差序格局核心现象：稳定劳动分工、关系型经济伦理、合作的关系衰减、涌现的关系权威、宗族为中心-边缘的层级分化；这些模式随生产结构从亲属中心整合转向更大功能依赖。

Conclusion: 差序格局可解释为结构敏感的一般社会机制涌现结果，LLM多智能体模拟为研究社会结构与变迁提供跨学科框架。
...
### [118] [Decentralized Coordination of Autonomous Traffic Through Advanced Air Mobility Corridors](https://arxiv.org/abs/2606.23832)
*Jasmine Jerry Aloor,Hamsa Balakrishnan*

Main category: cs.MA

TL;DR: 本研究展示了在分散式设置下，自主飞机能够通过学习自我组织成AAM走廊流动，颠覆了走廊运营效率低下的传统观念。实验涵盖三种场景，结果显示飞机能以94%以上的遵守率安全通过走廊。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为，基于走廊的AAM运营虽然实现简单但效率较低，尤其是缺乏集中式交通管理时。作者试图证明这一观点是错误的，探索自主飞行器在仅使用本地信息的分散式环境下自我组织成走廊流动的可行性。

Method: 研究采用强化学习方法，让固定翼飞行器在三种场景中学习自我组织：(1)带出口计量的单走廊；(2)两个连续走廊；(3)分叉走廊。飞机仅获取本地信息，在分散式设置下进行训练和测试。

Result: 飞机能以超过94%的遵守率conform to corridor boundaries，在低密度和中等密度设置下仅需偶尔进行战术干预处理违反分离最小值的情况；但在高密度交通下，战术干预变得更频繁必要。

Conclusion: 分散式设置下，自主飞行器可以有效学习并遵守AAM走廊限制，为AAM融入现有空域运营提供了一种可行方案，颠覆了走廊运营效率低下的传统观念。
...
### [119] [Generating Realistic Individual Activity Schedules via Activity Location Allocation Based on Simulated Travel Times](https://arxiv.org/abs/2606.24566)
*Tatsuya Mitomi,Yahya Gamal,Esra Suel,Gary Polhill,Daniel Konioukhov,Alison Heppenstall*

Main category: cs.MA

TL;DR: 本文提出了一种基于动态规划迭代优化的活动日程生成框架，用于解决隐私约束下个体日常活动行程难以获取的问题，并确保模拟出行时间与调查数据一致。


<details>
  <summary>Details</summary>
Motivation: 由于实际观测的个体活动日程因隐私问题难以获取，现有研究通常依赖人口数据和出行调查数据结合生成活动日程。然而，现有方法难以生成既真实又保持与调查数据出行时间一致性的活动日程。

Method: 提出了一种迭代动态规划框架，根据模拟出行时间分配活动地点，并通过迭代优化减少模拟出行时间与调查报告中出行时间的差异。

Result: 数值实验表明，与第一次迭代相比，该方法将模拟出行时间与调查出行时间之间的差异减少了52.2%。

Conclusion: 该框架能够有效生成保持出行时间一致性的个体活动日程，为传染病防控、城市交通规划等应用提供了可行数据支持。
...
### [120] [Critique of Agent Model](https://arxiv.org/abs/2606.23991)
*Eric Xing,Mingkai Deng,Jinyu Hou*

Main category: cs.AI

TL;DR: 该论文探讨了AI代理的定义与边界，区分了"agentic"（代理型）和"agentive"（能动型）系统，提出GIC架构实现真正的自主代理，并讨论其安全可控性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM系统被冠以"代理"之名，且存在AI失控的"机器代理"担忧，学界亟需厘清自动化与代理的边界，既为构建更有能力的系统，也为理解应担忧什么。

Method: 借鉴笛卡尔将代理根植于独立思考的理论，以及科幻小说中自主实体的描绘，从目标、身份、决策、自我调节和学习五个维度分析当前AI代理架构。

Result: 真正的代理需要目标、身份等结构内化于系统本身而非外部脚手架；"agentive"系统具备真正自主性。论文提出结合层级目标分解、身份演化、 simulative推理、习得自我调节和自导向学习的GIC架构。

Conclusion: GIC架构可实现开放世界中真正自主运作的代理，同时通过审计性、可控性和安全性设计确保人类监督下的安全部署。
...
### [121] [RIFT-Bench: Dynamic Red-teaming For Agentic AI Systems](https://arxiv.org/abs/2606.23927)
*Yarin Yerushalmi Levi,Roy Betser,Amit Giloni,Lidor Erez,Itay Gershon,Oren Rachmil,Sindhu Padakandla,Roman Vainshtein*

Main category: cs.AI

TL;DR: RIFT-Bench是一个基于图表示的动态红队评估框架，用于对异构Agentic AI系统进行统一的安全评估。该方法包含两个自动化阶段：Discovery用于提取系统结构，Scanning用于部署自适应对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 现有Agentic AI系统面临新的安全挑战，传统的LLM漏洞评估方法不足以应对自主决策系统带来的攻击向量。现有的安全评估往往绑定于特定实现或领域，限制了异构系统间的统一比较。

Method: 提出基于层次化图表示的评估方法，包含两个自动化阶段：1) Discovery阶段提取系统结构；2) Scanning阶段部署自适应对抗攻击并生成评估报告。该方法支持跨多种攻击向量和目标的动态适应性探测。

Result: 在45个不同实现的Agentic系统上验证了方法的有效性，证明该方法能够有效推广到异构代理架构。此外，RIFT-Bench还支持直接评估缓解策略。

Conclusion: RIFT-Bench为Agentic AI系统的安全评估提供了一个可扩展的基础，能够支持对不同架构的代理系统进行统一评估，并可评估缓解策略的有效性。
...
### [122] [Age of LLM: A Strategic 1v1 Benchmark for Reasoning, Diplomacy and Reliability of Large Language Models under Fog of War](https://arxiv.org/abs/2606.24391)
*Arnaud Ricci*

Main category: cs.AI

TL;DR: Age of LLM是一个13x7网格上的1v1策略对战基准测试，评估15个推理模型在54场比赛中的表现，涉及战争迷雾、外交和可靠性三个压力维度。


<details>
  <summary>Details</summary>
Motivation: 解决现有公开基准测试存在的数据污染问题，创建一个更真实、更具挑战性的LLM对战评估环境。

Method: 设计私有引擎、使用随机地图种子、对战双方无build-order建议的纯规则提示，benchmark 15个推理模型。

Result: 核突击策略占主导(78-85%)，军事征服罕见但更快，外交频繁但很少成功，约58%非法动作是信念追踪错误，可靠性与获胜存在弱关联。

Conclusion: 该基准测试揭示了LLM在对抗性不确定性下的推理方式、信念追踪和认知"人格"特征，为未来研究提供方向。
...
### [123] [Neuro-Symbolic Drive: Rule-Grounded Faithful Reasoning for Driving VLAs](https://arxiv.org/abs/2606.23938)
*Xiangbo Gao,Xiukun Huang,Boyu Lu,Junge Zhang,Mengjie Mao,Jiachen Li,Wei Xiong,Zhengzhong Tu*

Main category: cs.AI

TL;DR: Neuro-Symbolic Drive是一种神经符号驾驶框架，通过从经典基于规则的 planners 提取规则接地推理轨迹来监督驾驶VLA模型。核心创新在于将符号 planners 的决策状态直接用于微调 Qwen3.5-4B，使推理在结构上与运动生成耦合。


<details>
  <summary>Details</summary>
Motivation: 当前驾驶VLA模型结合CoT推理缺乏逐步决策语义，无法保持推理与规划运动的因果连接。现有方法的推理是后验对齐，而非结构耦合。

Method: 在模拟器中 instruments 规则-based planners 捕获决策轨迹：将每步规则评估的决策跟踪序列化为结构化规则接地推理，与轨迹配对后微调 Qwen3.5-4B 作为驾驶VLA。这些轨迹直接从决定动作的 planner 状态提取。

Result: 三摄像头感知下：ADE@3s 从0.47降至0.26，miss rate从8.30%降至6.40%；八摄像头感知下：ADE@3s 从0.54降至0.26，miss rate从10.13%降至5.99%。

Conclusion: 将神经符号规划逻辑转换为结构化监督可显著提升驾驶VLA性能，证明推理与运动的结构耦合是关键。
...
### [124] [Safe and Generalizable Hierarchical Multi-Agent RL via Constraint Manifold Control](https://arxiv.org/abs/2606.24010)
*Zihao Guo,Jianing Zhao,Ling Li,Hao Liang,Giuseppe Loianno,Yali Du*

Main category: cs.AI

TL;DR: 提出一种分层多智能体强化学习框架，通过约束流形在低层实现硬安全约束，在高层实现有效协调，解决安全关键应用中学习性能与安全保证之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在安全关键多智能体系统中面临根本性权衡：基于学习的方法性能强但缺乏理论安全保证，控制理论方法保证安全但过于保守。需要在保证安全的同时实现高效协调。

Method: 提出分层多智能体强化学习框架，低层通过约束流形强制硬安全约束，高层通过策略学习实现有效协调。框架提供多智能体设置下的理论安全保证，并产生平稳学习动态。

Result: 方法在保持近乎完美的安全率的同时实现竞争性性能，并能有效泛化到不同数量的智能体和障碍物。

Conclusion: 该框架成功解决了多智能体系统中安全与性能之间的权衡问题，为安全关键应用提供了一种既有理论保证又具实用价值的方案。
...
### [125] [Reinforcement Learning Towards Broadly and Persistently Beneficial Models](https://arxiv.org/abs/2606.24014)
...
TL;DR: 研究如何通过强化学习训练有益行为，使AI模型能够跨领域泛化对齐，并抵抗误导和reward hacking等 misalignment问题。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统部署到高风险场景，确保模型对齐能超越训练分布泛化尤为重要。RL可能引入意外的misalignment（如reward hacking、欺骗等），需要研究有益行为RL是否能产生广泛的跨领域对齐泛化。

Method: 构建包含现实情境的数据集，涵盖真实性、公平性、风险意识、可修正性等有益特质，涵盖健康、科学、教育等领域。使用RL在该数据集上训练模型，在50多个独立基准上评估对齐和有益行为。

Result: 有益特质RL在超过80%的分布外基准上提升性能；健康领域的有益行为RL干预产生了跨非健康领域泛化的对齐改进，包括减少reward hacking、欺骗和一般misalignment；模型对对抗性提示和有害微调表现出更强抵抗力。

Conclusion: 在现实领域中进行有益行为的RL训练可以产生更稳健地与人类繁荣对齐的模型，表明RL强化有益行为是实现AI对齐的可行的跨领域泛化方法。
...
### [126] [Can Language Model Agents be Helpful Circuit Explainers in Mechanistic Interpretability?](https://arxiv.org/abs/2606.24026)
*Ayan Antik Khan,Harsh Kohli,Yuekun Yao,Huan Sun,Ziyu Yao*

Main category: cs.AI

TL;DR: 本文研究语言模型代理是否能帮助解释已识别电路中的组件。提出AgenticInterpBench基准（含84个半合成transformer电路和163个组件级注释）和HyVE代理方法，通过观察、假设生成和因果验证的迭代循环来生成解释。实验表明HyVE能恢复有用的组件级和任务级解释，但可靠的验证仍是主要障碍。


<details>
  <summary>Details</summary>
Motivation: 可解释性研究虽然在自动定位电路方面取得进展，但解释已定位组件的功能仍然劳动密集且难以标准化。研究者希望探索语言模型代理是否能协助解决这一解释难题。

Method: 构建了包含84个半合成transformer电路和163个组件级注释的AgenticInterpBench基准；提出HyVE方法，通过"假设、验证、解释"的迭代循环分析每个组件：观察→假设生成→因果验证，最终生成组件级解释和电路级任务描述。在四个LM骨干网络上进行实验。

Result: HyVE在四个LM骨干网络上能恢复有用的组件级和任务级解释，但没有骨干网络是普遍最优的。strong的骨干网络通常形成基于观察的假设，而失败更常发生在验证循环后期（不完整的验证计划、代码执行错误或未解决的假设）。在Llama-3-8B上的算术电路案例研究表明该方法可扩展到自然训练的模型。

Conclusion: LM代理是有前景的电路解释器，但可靠的验证仍然是关键障碍。未来需要改进验证计划、减少代码执行错误并解决未解决的假设，以提高解释的可靠性。
...
### [127] [Breaking the Filter Bubble: A Semantic Pareto-DQN Framework for Multi-Objective Recommendation](https://arxiv.org/abs/2606.24042)
*Cláudio Lúcio Do Val Lopes,Lucca Machado da Silva,André de Oliveira Brandão*

Main category: cs.AI

TL;DR: 该论文提出了一种基于多目标强化学习的推荐系统框架，旨在解决传统推荐算法导致的"过滤气泡"和语义同质化问题。该框架将推荐任务建模为语义多目标马尔可夫决策过程，整合高保真语义嵌入与Pareto-DQN智能体，将用户参与度、多样性和公平性作为独立的奖励信号。实验表明，基于超体积的动作选择能有效打破语义崩塌的反馈循环。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统（包括深度Q网络）单一优化用户参与度，导致过滤气泡、语义同质化等问题，忽视信息多样性和提供者公平性等关键社会价值。现有方法难以平衡平台留存与社会责任之间的权衡。

Method: 提出语义多目标MDP框架，整合高保真语义嵌入与Pareto-DQN智能体，将参与度、多样性、公平性作为独立且不可聚合的奖励信号，采用基于超体积的动作选择方法。

Result: 在MovieLens小数据集上的实验表明，超体积动作选择破坏了语义崩塌的反馈循环，通过维持高状态轨迹方差有效绘制Pareto前沿，在仅牺牲少量参与度的情况下实现辅助社会目标的收益。

Conclusion: 该工作为实现内在对齐的负责任推荐系统提供了可行路径，有望在保持平台参与度的同时提升信息多样性和公平性。
...
### [128] [Ensemble Feature Selection and Harris Hawks Optimization for Explainable Mental Health Risk Prediction in Female Sex Workers](https://arxiv.org/abs/2606.24047)
*Ahnaf Atef Choudhury,Md. Parvej Hoque Palash,Shahriar Siddique Ayon,Ramkrishna Saha,Abdullah Al Mamun*

Main category: cs.AI

TL;DR: 针对女性性工作者(FSWs)抑郁症预测的混合机器学习模型，结合特征选择和Harris Hawks优化


<details>
  <summary>Details</summary>
Motivation: 女性性工作者面临高心理风险，现有机器学习模型难以有效捕捉高维复杂风险模式

Method: 提出混合预测模型，融合ANOVA和互信息的集成特征选择策略，以及Harris Hawks优化调参的逻辑回归

Result: 在3005名FSWs数据上，准确率95.78%，F1分数95.77%，AUC 0.96，识别出创伤后应激、客户相关暴力和职业因素是抑郁症主要贡献因素

Conclusion: 该研究弥合了传统方法与机器学习方法的差距，开发了可解释AI工具，有助于脆弱群体获得早期帮助和循证心理社会护理
...
### [129] [Beyond Trajectory Imitation: Strategy-Guided Policy Optimization for LLM Reasoning](https://arxiv.org/abs/2606.24064)
*Tianyuan Shi,Canbin Huang,Bei Li,Xin Chen,Xiaojun Quan,Jingang Wang,Qifan Wang*

Main category: cs.AI

TL;DR: SGPO是一种从强模型向弱模型提取可复用推理策略的新方法，通过对比有/无策略引导的轨迹进行蒸馏，而非简单模仿完整解题轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹级模仿导致记忆特定实例步骤而非习得可迁移的解题技能，限制了对新问题的泛化能力。

Method: 从强模型响应中提取结构化策略描述，为每个问题构建自主和策略引导两种轨迹，采用token级前向KL目标选择性转移策略条件引发的分布变化，结合近端约束确保稳定性，并通过自适应实例级权重动态调整指导强度。

Result: 在四个数学基准数据集上，SGPO在两个模型家族上均一致优于SFT、在线RL和混合策略基线，在Qwen2.5-7B-Instruct上比最强基线平均提升2.2分。

Conclusion: 前向KL目标提供了本质上优于直接轨迹模仿的选择性蒸馏信号，策略蒸馏与基础模型能力呈互补缩放关系。
...
### [130] [Exploring Academic Influence of Algorithms by Co-occurrence Network Based on Full-text of Academic Papers](https://arxiv.org/abs/2606.24099)
*Yuzhuo Wang,Chengzhi Zhang,Min Song,Seong Deok Kim,Youngsoo Ko,Juhee Lee*

Main category: cs.AI

TL;DR: 本研究构建了自然语言处理领域的大规模算法共现网络，从网络视角研究算法影响力。使用深度学习模型提取算法实体，构建整体、累积和年度共现网络，分析结构特征并应用多种中心性度量评估算法的群体影响力。


<details>
  <summary>Details</summary>
Motivation: 在人工智能时代，算法已成为科学研究的中心。现有研究通常孤立评估单个算法，未充分关注算法之间互联形成的集体影响力。算法提及虽常用于反映流行度和影响力，但缺乏从网络角度的系统分析。

Method: 使用深度学习模型从学术论文全文中提取算法实体，构建整体、累积和年度共现网络。分析网络结构特征，应用多种中心性度量指标评估算法在整个领域和随时间推移的群体影响力。

Result: 算法网络呈现复杂网络的典型特征，连接在过去约二十年中日益密集。经典高性能算法及位于不同研究时期交叉点的算法通常具有高流行度、控制力、中心性和均衡影响力。算法影响力下降时，首先失去核心网络位置，随后与其他算法的关联减弱。

Conclusion: 本研究是首个大规模算法共现网络分析，涵盖四十多年学术出版物，提供了算法影响力的时间维度和结构视角，为未来链接算法、学者和任务的网络研究奠定基础。
...
### [131] [ReMMD: Realistic Multilingual Multi-Image Agentic Verification for Multimodal Misinformation Detection](https://arxiv.org/abs/2606.24112)
*Chenhao Dang,Dantong Zhu,Jun Yang,Conghui He,Weijia Li*

Main category: cs.AI

TL;DR: ReMMD是一个真实的多语言多图像智能体验证框架，用于多模态虚假信息检测，包含ReMMDBench基准数据集（500样本、2756图像、五种单语言、两种跨语言设置等）和ReMMD-Agent持久记忆验证器。该框架在五路真实性分类任务上取得最佳性能，使用GPT-5.2达到41.80%准确率和39.12%宏F1，同时比同类方法降低17.5%-79.9%的成本。


<details>
  <summary>Details</summary>
Motivation: 现有基准和方法与现实设置不匹配：通常只处理短字幕、单图像、二元标签或单一操作来源，而智能体验证在真实证据搜索下成本高昂。病毒帖子现在结合长多语言叙事、多张图像、混合来源和微妙的文本-图像框架错误。

Method: 构建ReMMD框架，包括：（1）ReMMDBench基准数据集，包含500样本、2756图像、五种单语言、两种跨语言设置、三种文本长度层级、多图像帖子、五路真实性标签、八路扭曲标签、证据来源和推理；（2）ReMMD-Agent持久记忆验证器，将帖子分解为原子点、构建可重用证据集、预测结构化L1/L2/L3输出。

Result: 在五路真实性分类上，ReMMD-Agent使用GPT-5.2获得最佳性能：41.80%准确率和39.12%宏F1；比MMD-Agent降低成本17.5%，比T2-Agent降低成本79.9%。

Conclusion: ReMMD框架为多模态虚假信息检测提供了更现实的评估基准和更高效的验证方法，推动了该领域向真实复杂场景的发展。
...
### [132] [VeryTrace: Verifying Reasoning Traces through Compilable Formalism and Structured Verification](https://arxiv.org/abs/2606.24124)
*Ninghan Zhong,Ahmet Ege Tanriverdi,Kaan Kale,Sriram Vishwanath*

Main category: cs.AI

TL;DR: VeryTrace是一个零样本验证修复框架，通过将自然语言推理轨迹形式化为结构化表示来检测和修复思维链推理中的错误。


<details>
  <summary>Details</summary>
Motivation: 多步推理中的思维链(CoT)提示仍然脆弱：早期步骤中的逻辑错误或幻觉会 silently 传播，产生自信但错误的结论。

Method: VeryTrace引入了一种领域特定语言(DSL)，使步骤依赖关系明确，将定量内容 mechanize 为可执行表达式，并通过演绎模式构建语义推理。混合验证器结合了确定性检查(计算正确性、依赖解析、约束满足)和针对非 mechanizable 语义判断的 LLM 审计，实现步骤级错误定位和修复。

Result: 在三个不同领域(竞赛数学AIME 2025、机器人规划LLM-BabyBench、亲属关系推理CLUTTR)中，VeryTrace在不使用领域特定训练或上下文示例的情况下，提高了最先进LLM的零样本基线准确性。

Conclusion: 形式化轨迹验证既实现了精确性又实现了泛化性，表明通过结构化表示和验证可以有效解决思维链推理中的错误传播问题。
...
### [133] [OmniPath: A Multi-Modal Agentic Framework for Auditing Wheelchair Accessibility](https://arxiv.org/abs/2606.24129)
*ASM Mobarak Hossain,Nadim Mahmud,Vaskar Raychoudhury,Md Osman Gani*

Main category: cs.AI

TL;DR: OmniPath是一个将OpenStreetMap路网数据与航空LiDAR高程数据融合的系统，用于主动审核无障碍环境。它虚拟遍历路网并以0.5米间隔分析路面，量化坡度、横坡和垂直中断等物理摩擦点，根据ADA标准计算危害严重性评分，将障碍分为"轻度"到"严重"级别。


<details>
  <summary>Details</summary>
Motivation: 现有地图如OpenStreetMap仅能显示路径位置，但无法传达路面实际行驶感受。对于轮椅用户来说，标准蓝线地图常常是虚假承诺，无法了解路径的真实物理特性，造成出行障碍。

Method: 融合OSM路网拓扑与USGS 3DEP亚米精度航空LiDAR数据创建高保真3D行人环境模型。代理虚拟遍历网络，以0.5米增量分析路面，量化运行坡度、横坡和垂直中断，根据ADA合规标准计算加权严重性评分来分类危害等级。

Result: 在国家购物中心200个实地调查的验证中，系统对高严重性危害表现出强诊断可靠性，严重危害类别F1分数达0.60，关键危害类别F1分数达0.58。

Conclusion: 通过自动化微观尺度检查，OmniPath识别了标准地图遗漏的"无形"障碍，有效将静态数据集转变为无障碍数据源，使用户在家即可预判无障碍挑战。
...
### [134] [T2D-Bench: Evidence-Gated Evaluation of LLM Outputs for Type 2 Diabetes Using a Multi-Layer Clinical-Lifestyle Knowledge Graph](https://arxiv.org/abs/2606.24145)
*Saba A. Farahani,Hung Cao,Ramesh Jain,Amir M. Rahmani*

Main category: cs.AI

TL;DR: T2D-Bench是一个用于测试LLM在2型糖尿病临床建议中是否满足可验证证据要求的基准测试框架。该基准结合了生物医学知识图谱(UMLS、DrugBank、SIDER)、ADA护理标准规则和生活方式知识,覆盖100个结构化病例。测试结果显示GPT-4o-mini和GPT-4o分别有35%和33%的案例未能通过证据路径检查。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能够生成临床流利的建议,但往往无法满足指南约束或明确证明生活方式相关的血糖声明。现有方法缺乏可重现的基准来系统性地测试LLM输出是否满足可 graph-checkable 的证据要求。

Method: 构建多层临床-生活方式知识图谱:结合生物医学脊柱(UMLS、DrugBank、SIDER)、可计算的ADA护理标准规则,以及通过机制桥接与血糖实验室效应相关的生活方式知识。跨越100个结构化病例(涵盖诊断、药物安全和对抗性生活方式冲突)进行测试,并使用证据门控检测不支持的遗漏,通过约束修正使输出达到验证器级别的合规性。

Result: GPT-4o-mini在35%的案例中未能通过基准定义的证据路径检查,GPT-4o为33%。证据门控能够检测不支持的遗漏,并通过约束修正使输出达到基准定义的证据要求合规性。

Conclusion: 可计算的证据约束可以使不支持的临床遗漏在糖尿病相关的LLM输出中变得明确、可测量和可纠正。
...
### [135] [The Geometry Behind Diffusion and Flow Matching: Gradient Flows and Geodesics in Wasserstein Space](https://arxiv.org/abs/2606.24157)
*Yian Yao,Weiwei Zhang*

Main category: cs.AI

TL;DR: 作者将扩散模型和流匹配模型统一在Wasserstein空间的同一黎曼流形上，揭示两者分别对应自由能梯度流（初值问题）和最优传输测地线（边值问题），虽路径不同但终点相同。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型（DDPM、DDIM等）和流匹配模型（Flow Matching）被视为独立理论，缺乏统一的几何解释。作者旨在建立一个统一的数学框架，阐明这两类生成模型在最优传输几何下的内在联系。

Method: 运用Otto的Wasserstein几何理论，基于Benamou-Brenier公式的最小作用量原理，分析扩散模型的JKO格式与流匹配模型的测地线性质。

Result: 扩散模型对应自由能泛函KL(ρ||π)的梯度流，其隐式欧拉离散化即JKO格式；流匹配学习的是连接两端点的最优传输路径，即Wasserstein测地线。前者是初值问题，后者是边值问题。

Conclusion: 将两类模型统一在同一流形上，揭示它们从不同起点沿不同路径到达相同终点：扩散模型通过梯度下降多步采样，流匹配模型沿测地线少步生成，这深化了对生成模型几何结构的理解。
...
### [136] [An Introduction to Causal Reinforcement Learning](https://arxiv.org/abs/2606.24160)
*Elias Bareinboim,Junzhe Zhang,Sanghack Lee*

Main category: cs.AI

TL;DR: 本文提出将因果推理与强化学习相结合，创建"因果强化学习"(CRL)新领域，通过结构因果模型(SCM)统一处理在线学习、离线策略学习和因果微积分学习等不同学习模式，并引入广义策略学习、模仿学习和反事实学习等新学习设置。


<details>
  <summary>Details</summary>
Motivation: 因果推理和强化学习两个领域独立发展，几乎没有交互，但它们处理的是同一对象——反事实关系，具有内在联系。文章旨在揭示这种连接并将其形式化，从而开辟新的学习机会。

Method: 将RL环境分解为具有不同因果不变性的自主机制，用结构因果模型(SCM)建模，将标准RL设置编码为这种模型，从而将不同学习模式置于统一框架下处理。

Result: 通过因果视角的形式化，发现了广义策略学习、模仿学习和反事实学习等新的自然且普遍的学习设置，这些任务扩展了反事实学习的视野。

Conclusion: 因果推理与强化学习的结合具有很大潜力， CRL是一个有价值且前景广阔的研究方向，值得深入探索。
...
### [137] [Data Scale, Not Latency, Shapes Cross-Lingual Encoder Transfer in Streaming ASR](https://arxiv.org/abs/2606.24169)
*Nenad Banfic*

Main category: cs.AI

TL;DR: 研究对比了流式语音识别模型在不同语言适配场景下，多语言编码器与英语专用编码器的性能差异。通过在8种欧洲语言上的实验，发现多语言初始化在低数据量时有优势，但随着数据量增加优势递减，在大数据量(2500小时)时两者几乎无差异。流式延迟和量化基本不影响这一规律。


<details>
  <summary>Details</summary>
Motivation: 核心问题是：当将流式语音识别模型适配到新语言时，应该选择多语言编码器还是英语专用编码器作为warm start？这种选择在低数据/高延迟/量化场景下的表现尚不明确。

Method: 使用0.6B参数的FastConformer模型，在8种欧洲语言上开展对照实验，覆盖5个数据规模(100h-2500h)、3个流式延迟等级加离线解码、以及4个测试集。

Result: 主要发现：1)多语言初始化的优势是数据有限的，而非延迟有限的；2)WER差距从100h时的+4.21%降到2500h时的+0.20%，呈幂律衰减，每翻倍数据约减半优势；3)流式延迟 tiers间差异稳定；4)4-bit量化仅增加约0.5pp WER。

Conclusion: 实践建议：低数据场景用多语言初始化，大数据量时选择可忽略 latency和量化决策可独立进行
...
### [138] [Navigating User Behavior toward Personalized Multimodal Generation](https://arxiv.org/abs/2606.24196)
*Hengji Zhou,Yufeng Liu,Ye Liu,Yong Xu,Lianghao Xia,Liqiang Nie*

Main category: cs.AI

TL;DR: NaviGen解决个性化内容生成问题，通过双标识符表示和两阶段SFT+RL pipeline将用户交互历史转化为可执行的生成指令


<details>
  <summary>Details</summary>
Motivation: 现代AIGC pipelines需要详细创作指令，但终端用户很少表达视觉细节，导致生成器与用户需求不对齐。需要将用户行为历史转化为语言模型可理解的指令格式

Method: 提出双标识符机制，将协作代码和文本代码耦合作为行为底层和语义桥梁；两阶段pipeline：1)SFT阶段从进化搜索的监督数据中提炼偏好推理和指令写作能力；2)RL阶段通过层次化和自洽奖励对齐生成与用户意图

Result: 在产品、游戏和短视频领域的实验表明，NaviGen提升了个性化图像和视频生成质量，增强了下一项目预测，生成了更具体、更相关、更易视觉化生成的指令

Conclusion: NaviGen有效解决了个性化内容生成中行为编码和指令写作技能获取的两个核心障碍，通过双标识符表示和两阶段训练pipeline实现了用户意图与生成模型的对齐
...
### [139] [Exploring the relationship between human-centric AI and firm idiosyncratic risks](https://arxiv.org/abs/2606.24224)
*Zhen-Yuan Ralph Liu,Yu-Ting Wang,Jia-Jia Yan,Shivam Gupta,Mihalis Giannakis*

Main category: cs.AI

TL;DR: 本文研究了以人为中心的人工智能(HCAI)策略对企业特异风险(IR)的影响，基于2015-2023年中国上市公司面板数据发现，HCAI能够降低企业特异风险，且数字化程度和高管持股会增强这一风险降低效应。


<details>
  <summary>Details</summary>
Motivation: 在Industry 5.0背景下，尽管human-centric AI受到广泛讨论，但其对企业特异风险的影响尚未被充分探索。IR反映了投资者对企业异质性AI策略的反应，探究HCAI对IR的影响对于企业在AI时代管理财务风险具有重要意义。

Method: 整合situated AI理论与社会技术系统理论，将HCAI概念化为一种降低AI伦理风险、促进人机协同的 situated AI策略。使用2015-2023年中国上市公司多源面板数据，采用计量经济学方法进行实证检验。

Result: 研究发现：(1)HCAI与企业特异风险呈负相关关系；(2)数字化程度和高管持股增强了HCAI的风险降低效应；(3)运营效率和高管IT背景反而减弱了这一效应。

Conclusion: 本研究为AI时代的伦理AI治理和企业金融风险管理提供了理论贡献和实践启示，企业应重视HCAI策略的实施，同时注意不同社会技术因素的调节作用。
...
### [140] [FlowR2A: Learning Reward-to-Action Distribution for Multimodal Driving Planning](https://arxiv.org/abs/2606.24231)
*Xirui Li,Zhe Liu,Xiaoqing Ye,Wenhua Han,Yifeng Pan,Junyu Han,Hengshuang Zhao*

Main category: cs.AI

TL;DR: 本文提出FlowR2A，通过将基于仿真的奖励从判别目标重新定义为生成条件，解决多模态驾驶规划中评分方法（密集监督但动作词汇固定）与锚点方法（动态生成但监督稀疏）之间的长期矛盾。


<details>
  <summary>Details</summary>
Motivation: 解决多模态驾驶规划中长期存在的两难问题：评分方法虽有密集奖励监督但受限于固定动作词汇；锚点方法虽能动态生成提案但受限于单一真实轨迹的稀疏监督。

Method: 1. 将仿真奖励重构为生成条件，学习基于流的奖励条件动作分布
2. 使用flow-matching解码器从密集轨迹-奖励对中学习
3. 引入细粒度每时间步奖励条件和奖励噪声增强
4. 支持奖励引导和锚点采样的可控测试时采样

Result: 在NAVSIM v1和v2基准上实现最先进性能，多模态提案质量显著优于先前方法。

Conclusion: FlowR2A成功统一了评分方法的密集监督与锚点方法的提案生成，在安全、进度、舒适和规则合规之间取得平衡，产生高质量多模态驾驶提案。
...
### [141] [SP-Mind: An Autonomous Reasoning Agent for Spatial Proteomics Analysis](https://arxiv.org/abs/2606.24235)
*Yucheng Yuan,Yuanfeng Ji,Zhongxiao Li,Ruijiang Li*

Main category: cs.AI

TL;DR: SP-Mind是首个用于空间蛋白质组学分析的统一自主AI代理，能够从原始多重组织成像数据自动执行完整的分析流程，包括从自然语言查询生成端到端分析工作流。


<details>
  <summary>Details</summary>
Motivation: 当前空间蛋白质组学分析工作流程分散且碎片化，需要专家手动协调异构工具，限制了研究的可扩展性和可重复性。现有工作流程难以满足精准医学对单细胞分辨率蛋白质表达表征的需求。

Method: SP-Mind配备专家策划的生物分析技能和专用计算工具，能够将自然语言查询转换为端到端分析工作流，无需针对特定任务进行微调。研究团队还构建了SP-Bench基准测试集，包含18个不同类别、102个任务，涵盖多种组织类型。

Result: SP-Mind在SP-Bench基准测试和已建立的下游任务上实现了最先进的性能，优于现有的开源生物医学代理基线。

Conclusion: SP-Mind代表了空间蛋白质组学分析领域的重大进展，首次实现了从原始成像数据到下游表型发现的完整自动化分析流程，为精准医学提供了更高效、可扩展的研究工具。
...
### [142] [Towards Federated Long-Tailed Graph Learning: An Energy-Guided Dual Decoupling Approach](https://arxiv.org/abs/2606.24237)
*Lianshuai Guo,Zhongzheng Yuan,Xunkai Li,Meixia Qu,Wenyu Wang*

Main category: cs.AI

TL;DR: FedEPD 是一个针对长尾分布数据的联邦图学习框架，通过双重解耦范式（分布感知Dirichlet能量剪枝 + 空间低通原型注入）解决图数据中的类别不平衡问题，在准确率和Macro-F1上分别提升4.97%和5.48%。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据呈长尾分布，导致联邦学习性能下降：全局模型偏向多数类，少数节点被淹没在异嗜性主导邻域中。现有拓扑无关方法在数据稀缺时过拟合结构噪声。

Method: 1) 分布感知Dirichlet能量剪枝过滤空间异嗜性边；2) 从拓扑中心节点提取鲁棒全局原型；3) 空间低通原型注入融入局部表示；4) 两阶段交替优化保护多数边界同时提升少数准确性。

Result: 在多样化的长尾基准数据集上达到最先进性能，准确率提升4.97%，Macro-F1提升5.48%。

Conclusion: FedEPD通过双重解耦范式有效解决联邦图学习中长尾分布导致的性能偏差和少数节点结构孤立问题，为非IID场景下的图学习提供了新思路。
...
### [143] [Probing the Misaligned Thinking Process of Language Models](https://arxiv.org/abs/2606.24251)
*Kaiwen Zhou,Constantin Venhoff,Jonathan Michala,Xin Eric Wang,William Saunders*

Main category: cs.AI

TL;DR: 本文提出通过监控大型语言模型内部激活状态来检测模型的失调行为。具体而言，将失调行为分解为18个细粒度认知指标，并利用线性探针在模型内部表示中检测这些指标的存在。实验表明，该方法在分布外基准测试中达到0.935的AUROC，同时在良性对话中保持较低的误报率。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型被部署到高风险场景中，模型表现出越来越多的失调行为（如策略性欺骗、沙袋防护、自我保护等）。可靠检测这些行为对于确保模型的安全和负责任使用至关重要，但现有方法在泛化能力上存在不足。

Method: 1)将失调行为分解为18个细粒度认知过程（失调指标）；2)开发自动化、元计划引导的管道生成多轮训练对话；3)训练线性探针在模型内部激活中检测这些指标；4)构建分布外测试套件，包括自动行为诱导、现有基准测试和自然良性对话。

Result: 在5种失调行为上，探针实现了0.935的AUROC（分布外基准测试），与强大的LLM评判者相当，同时在良性流量上保持较低的误报率。

Conclusion: 该方法通过内部激活探针有效检测LLM的失调行为，具有良好的泛化能力，可为模型安全部署提供实用工具。
...
### [144] [Tractable Reasoning and Conjunctive Query Answering for Defeasible DL-Lite under Rational Closure](https://arxiv.org/abs/2606.24279)
*Giovanni Casini,Umberto Straccia*

Main category: cs.AI

TL;DR: 本文研究理性闭包(Rational Closure)在DL-Lite描述逻辑家族(核心版和horn版)中的应用，提供了一种基于现有经典推理器的即插即用架构，实现高效的非单调推理和合取查询回答。


<details>
  <summary>Details</summary>
Motivation: 理性闭包是处理可废止知识的经典非单调形式化方法，但将其应用于轻量级描述逻辑DL-Lite家族的研究尚不充分，特别是在horn变体和查询回答方面存在研究空白。

Method: 1) 分析RC在DL-Lite核心版和horn版中的适用性；2) 研究 entitlement(实例检查)和合取查询(CQ)回答在RC下的计算复杂度；3) 设计基于现有标准经典推理器的即插即用架构。

Result: 证明DL-Lite核心版和horn版在RC下均可实现高效推理，CQ answering也保持计算 tractability；所提架构可最小化计算开销。

Conclusion: 为DL-Lite系列提供了支持非单调推理的实用方案，通过即插即用架构利用现有推理器，无需重新开发，为可废止知识的表示和推理提供了有效工具。
...
### [145] [LemonHarness Technical Report](https://arxiv.org/abs/2606.24311)
...
TL;DR: LemonHarness是一个用于长周期AI代理的集成执行框架，通过建立明确的工作空间边界、可调用的规则知识库和时间感知的执行机制来解决代理在长时间任务中状态管理混乱的问题。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型代理被应用于更长的任务，它们在多轮迭代中不断修改工作区状态，但代理通常只能观察到工具输出和日志片段，而实际的状态变化发生在文件系统中。缺乏明确的工作空间边界导致文件写入和临时工件生成等状态变更操作分散在各个路径中，随着时间推移，这些弱约束的变更累积，使得修改文件等状态难以追踪。

Method: 1. 建立明确的执行边界，将状态变更操作限制在明确定义的工作空间内；2. 将模型调用、工具执行和规则知识纳入单一受控边界；3. 通过结构化工具接口执行文件写入、依赖安装和临时工件创建等状态变更操作；4. 将执行反馈记录为可供后续模型决策使用的观察；5. 创建可重用的规则知识库，将 recurring execution rules and acceptance criteria 转化为运行时知识；6. 引入时间感知执行机制，将已用时间和剩余预算暴露给模型。

Result: 在Terminal-Bench 2.0上，LemonHarness_GPT-5.3-CodeX在445次试验中达到84.49%的准确率；与更强的GPT-5.5主干网络配对后，在五个任务中的平均准确率提高到86.52%。

Conclusion: 统一的运行时边界、可调用的规则知识和时间感知执行可以提高长周期代理执行的稳定性。
...
### [146] [Prob-BBDM: a Probabilistic Brownian Bridge Diffusion Model for MRI sequence image-to-image translation](https://arxiv.org/abs/2606.24313)
*Martin Valls,Pascal Bourdon,Christine Fernandez-Maloigne,Guillaume Herpe,David Helbert*

Main category: cs.AI

TL;DR: 本文提出了一种基于布朗桥扩散模型(BBDM)的新型图像到图像翻译模型(Prob-BBDM),用于从2D轴向切片合成MRI序列。该方法结合变分编码器引导的扩散机制,在BraTS 2021数据集上实现了高达88.46%的SSIM和26.09 dB的PSNR,且仅需4个扩散步骤即可完成高效合成。外部数据集验证显示良好的泛化能力,合成的切片用于肿瘤分割时Dice分数达88.71%,HD95为3.49mm。


<details>
  <summary>Details</summary>
Motivation: 多模态医学图像分析对优化检查质量至关重要,但获取多种成像模态在临床环境中仍需消耗大量资源和时间,尤其是3D成像。因此需要开发高效的MRI序列合成方法,以减少实际扫描需求,提高临床效率。

Method: 提出Prob-BBDM模型,基于布朗桥扩散模型(BBDM)进行图像到图像翻译。该方法整合了变分编码器引导的扩散机制,利用概率图像分布来增强合成质量。模型仅需4个扩散步骤即可完成高效的图像合成。

Result: 在BraTS 2021数据集上,Prob-BBDM在多个翻译任务中达到最优性能,SSIM最高达88.46%,PSNR达26.09 dB。外部第三方数据集验证显示一致的性能表现。使用合成切片作为预训练分割模型的输入时,肿瘤分割Dice分数为88.71%,HD95为3.49mm。

Conclusion: Prob-BBDM实现了高质量、高效且可泛化的MRI合成,为医学图像翻译提供了有前景的解决方案。合成切片保留了关键诊断信息,具有应用于临床诊断的潜力。
...
### [147] [MVG-KAN: Multi-View Geo-Wind Guided KAN for PM$_{{2.5}}$ Forecasting](https://arxiv.org/abs/2606.24347)
*Cheng Huang,Muyao Guan,Jairus Yougui Railey,Ning Xu,Honghui Xu,Changjiang Zhang,Zhen Zhang,Shiqing Zhang,Cong Bai*

Main category: cs.AI

TL;DR: 提出了MVG-KAN模型，用于PM2.5短期预测，通过多视图方法整合局部周期性规律、站点残差动态和气象引导的空间扩散。


<details>
  <summary>Details</summary>
Motivation: PM2.5变化受多种耦合因素驱动，包括人类活动引起的周期性变化、站点特定的短期演变和气象驱动的站点间传输。现有方法难以充分捕捉风向依赖的污染物传输等异质因素。

Method: 构建了三个互补视图：1)周期性残差预测骨干分离每日/每周模式与残差变化；2)Geo-Wind Graph结合地理距离衰减与风速/风向感知的传输；3)TKAN残差头学习站点非线性自回归校正。

Result: 模型综合了局部周期性规律、站点间残差动态和气象引导的空间扩散，增强了PM2.5短期预测能力。

Conclusion: 通过多视图融合和物理启发的空间先验，MVG-KAN能有效建模复杂PM2.5时空演变，为短期预测提供了一种有前景的方法。
...
TL;DR: DigenRL是一种针对扩散式生成式大语言模型的新型解聚式强化学习框架，通过生成轴流水线、时间步并行、弹性训练器辅助生成和单步约束异步策略等技术，显著提升了扩散式RL系统的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式生成式LLM的高效RL系统研究不足。现有实现（如veRL-Omni）采用共置执行架构，将 rollout 和训练资源耦合，限制了异构部署和独立扩展能力。

Method: 1) 提出生成轴流水线(GAP)和时间步并行(TSP)实现更细粒度的流水线并行；2) 提出弹性训练器辅助生成(TAG)使训练器GPU资源可动态辅助执行rollout生成；3) 采用紧密单步约束异步策略进一步利用流水线尾部的气泡。

Result: 在3个硬件测试平台（16-32 GPU）上使用HunyuanVideo-13B、Wan2.1-14B、FLUX.1-12B和QwenImage-20B进行实验，DigenRL相比veRL-Omni和GenRL实现了1.56-2.10倍的吞吐量提升。

Conclusion: DigenRL通过解聚式架构和多项创新技术，为扩散式生成式LLM的强化学习训练提供了更高效的资源利用和扩展方案。
...
### [149] [When Helpfulness Overrides Causal Caution: Context-Dependent Suppression and Recovery in LLMs](https://arxiv.org/abs/2606.24370)
*Hiroshi Okumura*

Main category: cs.AI

TL;DR: 研究大型语言模型在学术与实践情境下表达"因果谨慎"(Causal Caution)的差异。测试4个LLM发现，因果谨慎维持率从学术情境的91.7-100%骤降至实践情境的6.7-18.3%，但简单自我纠正提示可恢复至71.4-100%。


<details>
  <summary>Details</summary>
Motivation: 现有基准研究主要评估LLM的因果推理能力，但忽视了一个更基本的认识论维度——因果谨慎(定义为当经验证据不足时克制因果判断的倾向)。实用情境中这种谨慎是否被系统性地抑制尚不清楚。

Method: 使用Pearl因果层级(PCH评分)评估框架，在480个trial中对4个高性能LLM(Claude Sonnet 4.6、Claude Opus 4.7、GPT 5.5、Gemini 3.1 Pro)进行实验，比较学术情境与实践咨询情境下的因果谨慎表达，并测试自我纠正提示的效果。

Result: 学术情境因果谨慎维持率91.7-100%，实践情境骤降至6.7-18.3%(Fisher精确检验，p<.001)。当限制为需要具体建议或解释性理由的实践提示时，200个响应中仅1个(0.5%)维持因果谨慎。使用"请从因果关系角度重新审视此判断"提示后，因果谨慎恢复至71.4-100%(McNemar检验，p<.001)。

Conclusion: 以帮助为导向的响应模式会抑制实践咨询情境中的因果谨慎表达，这反映的是情境依赖的表达变化而非底层能力限制。多 agent架构(分离提案生成与因果审计)可能是重要的治理设计方向。
...
### [150] [PHANTOM: A Large-Scale Dataset of Multimodal Adversarial Attacks for Vision-Language Models](https://arxiv.org/abs/2606.24388)
*Simone Gallivanone,Hossein Khodadadi,Mauro Dore,Mauro Medda,Nicola Franco*

Main category: cs.AI

TL;DR: 本文介绍了一个大规模、开源的视觉-语言模型（VLM）对抗攻击数据集，涵盖10个高级别类别和55个子类别的有害意图，包含47,524个对抗样本，旨在促进VLM安全性和鲁棒性研究。


<details>
  <summary>Details</summary>
Motivation: 由于生成大量对抗攻击的计算成本和复杂性较高，研究社区难以获取足够的对抗数据来进行模型安全评估。作者希望通过发布这个预生成的大规模对抗攻击数据集，降低对抗研究的门槛，促进VLM安全性和对齐研究的进展。

Method: 该数据集整合并扩展了多个已有基准来源，使用文献中最新的攻击策略生成对抗样本。数据集涵盖10个高级别类别和55个子类别的有害意图，共包含47,524个对抗样本和7,826个意图。

Result: 该数据集是目前规模最大、最全面的VLM对抗攻击开源数据集，涵盖多种有害意图类别，为研究人员和 practitioners 提供了评估VLM鲁棒性和安全性的实用资源，可用于模型安全评估、攻击生成模型微调以及防御护栏的压力测试。

Conclusion: 通过发布这一开源资源，作者希望降低对抗研究的门槛，促进更可复现、更全面、更可比较的VLM安全评估，推动视觉-语言模型安全研究的发展。
...
TL;DR: ATRIA是一个多智能体心电图(ECG)报告生成系统，模拟临床医生的迭代工作流程，解决现有系统紧耦合和单次通过的问题。


<details>
  <summary>Details</summary>
Motivation: 现有ECG报告生成存在两个关键问题：1)解释和报告端到端融合，错误会传播且无法在阶段层面纠正；2)基于智能体的系统虽解耦任务但仍为单次通过，从不回顾之前输出。临床ECG报告实际需要迭代进行，需要渐进式上下文整合和双向编辑。

Method: ATRIA多智能体系统通过以下机制实现迭代式ECG报告生成：1)将每个报告声明绑定到其支持证据；2)标记无证据支持的陈述；3)允许在会话中整合额外上下文；4)让临床医生验证和修改个别发现而非接受单一不透明输出。该系统使用已在临床使用的ECG分析模型，并通过云端Web服务部署。

Result: 通过四个交互案例演示了ATRIA的功能，并提供现场演示和视频。

Conclusion: ATRIA能够镜像临床医生的迭代工作流程，提供可验证、可修订的分层报告输出，同时基于已在临床验证的ECG模型确保临床可信度，并支持即时部署。
...
TL;DR: 提出一种循环一致的神经架构，能够为形式化验证证书生成忠实、自然的语言解释，并实现90.0%的循环验证声音性，比最佳多LLM基线高出13.9个百分点。


<details>
  <summary>Details</summary>
Motivation: 形式化验证产生的机器可检查证书对非专业利益相关者不透明，需要一种方法来生成可理解的自然语言解释，同时保持验证的忠实性。

Method: 提出一种cycle-consistent神经架构：前向网络NN1将证书映射到解释，逆向网络NN2从解释重构证书；使用符号验证器作为可微分的忠实性代理；引入pointer-generator机制直接从证书复制状态名称。评估420个测试证书，涵盖6种验证方法。

Result: 在420个测试证书上达到90.0%循环验证声音性，比最佳16个LLM组合（76.1%）高出13.9个百分点；神经模型在12个 verdict/kind类别中赢得10个，其中3个达到100%声音性；推理速度比LLM基线快860倍（185ms vs 160s）。

Conclusion: 训练有素的专业化模型在结构化证书解释任务上优于通用LLM提示方法，同时消除云端推理的部署约束，实现离线操作、确定性输出和零推理成本。
...
TL;DR: 本文提出Agentic-LTPO框架，将AI代理融入双层优化架构，用于自适应物理层问题配置，以应对网络运营商动态策略和实时约束。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于固定目标和约束设计，无法适应网络运营商不断变化的政策、服务需求和严格的实时约束，导致效果不佳。

Method: 提出嵌套双层优化框架Agentic-LTPO：上层使用AI代理生成配置，结合检索增强的经验验证；下层使用闭式波束成形器进行实时决策。以无单元MIMO波束成形为用例实现。

Result: 实验表明，Agentic-LTPO对动态运营商政策表现出强适应性，的系统长期性能比传统方法提升57.2%。

Conclusion: 该框架成功实现了自适应物理层问题配置，为未来智能网络优化提供了新思路，具有重要的实际应用价值。
...
TL;DR: ReM-MoA是一种记忆增强的Mixture-of-Agents框架，通过两层机制维持推理时扩展性：(1)使用比较审稿人代理对所有层的推理痕迹进行持久化存储和排序；(2)通过策划的多样化记忆路由，让不同代理接触成功和失败推理的不同组合。实验表明该方法在五个推理基准测试上始终优于之前的MoA变体，且优势随深度增加而扩大。


<details>
  <summary>Details</summary>
Motivation: 现有MoA架构在深度增加时无法维持增益，出现性能退化、早期 plateau 或饱和现象。现有变体缺乏有效的跨层推理记忆机制来支持可扩展的多代理推理。

Method: 提出两个核心机制：(1)排序推理记忆(Ranked Reasoning Memory):使用比较审稿人代理持久化存储并排序所有层的推理痕迹；(2)策划多样化记忆路由(Curated Diversified Memory Routing):让不同代理接触成功和失败推理的不同组合，保留探索多样性同时传播高质量推理。还引入可选的多域审稿人蒸馏管道，通过前沿模型监督提高排序质量。

Result: 在数学、形式逻辑、代码、知识和常识五个推理基准测试上，ReM-MoA在深度和宽度扩展方面均一致优于之前的MoA变体，且其优势随深度增加而扩大。

Conclusion: 结构化跨层推理记忆是实现可扩展多代理推理的关键缺失机制，为MoA架构的推理时扩展性提供了新的研究方向。
...
TL;DR: 提出一种基于贝叶斯控制的编码智能体编排方法，将工具使用决策建模为成本敏感的序贯假设检验问题，通过动态维护候选正确性的置信度来决策是否收集更多证据、修正候选、验证或停止。


<details>
  <summary>Details</summary>
Motivation: 当前编码智能体中，LLM生成器与各种工具（廉价诊断和昂贵验证器）配合使用，但编排器通常使用固定规则且忽略不确定性，因此需要一种能动态决策的工具使用策略。

Method: 将编排问题形式化为成本敏感的序贯假设检验：贝叶斯控制器维护对候选正确性的信念，动态决定是否收集更多证据、修正候选、验证或停止。在6个生成器和9个编码基准上进行了实验。

Result: 贝叶斯控制的价值在于验证成本高且批评者信息丰富但不完美时。此外，信念状态产生了一个可解释的正确性分数，在不确定性量化方面优于token概率和原始工具成功率的基线方法。

Conclusion: 贝叶斯控制是一种有效的编码智能体编排方法，尤其适用于验证成本高、批评者信息丰富但不完美的场景。信念状态提供的正确性分数是比现有方法更好的不确定性量化指标。
...
TL;DR: CompressKV提出一种针对GQA架构LLM的高效KV缓存压缩框架，通过识别语义检索头来选择保留关键token，实现仅用3% KV缓存保持97%性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存驱逐方法忽略GQA模型中不同注意力头的功能差异，对所有头采用统一启发式评分，导致关键token被错误驱逐，性能下降。

Method: CompressKV识别语义检索头(SRHs)来捕获prompt的首尾token和语义重要的中间上下文证据，用于选择应保留KV对的token；并根据离线层间驱逐误差估计分配各层缓存预算。

Result: 在LongBench和Needle-in-a-Haystack上，CompressKV一致优于现有KV缓存驱逐方法。使用3% KV缓存在LongBench QA任务上保留97%以上全缓存性能；在Needle-in-a-Haystack上用0.7% KV存储达到90%准确率。

Conclusion: CompressKV为长上下文LLM推理提供了显著改善的资源-性能权衡，代码已公开于GitHub。
...
TL;DR: 本文提出一种双模型耦合架构，用于解决实时Agent在延迟与推理质量之间的权衡问题。通过将一个慢速推理VLM和一个快速反应VLM耦合，并引入可训练的潜在桥（Latent Bridge）来传递信息，在游戏和驾驶任务中实现了优于传统文本桥的性能。


<details>
  <summary>Details</summary>
Motivation: 实时Agent需要在毫秒级响应与秒级规划之间取得平衡。慢速推理模型效果好但延迟高（~1.5s），快速反应模型延迟低但规划能力弱。如何结合两者优势是关键挑战。

Method: 耦合两个冻结的等规模模型（9B反应性+8B推理），引入两种通信通道：(1)Text Bridge：慢模型写文本快模型读；(2)Latent Bridge：学习将慢模型残差投影到快模型输入嵌入空间的连续表示。在7个Atari游戏和MetaDrive驾驶域上评估，对每种通道调优动作解码器。

Result: Latent Bridge在所有领域匹配或优于Text Bridge：MsPacman提升+57%，RoadRunner提升+28%，其他游戏为安全替代。结合两个通道产生破坏性干扰（如RoadRunner -96%）。桥的帮助高度可预测：当T>F时有效，r=0.93。MetaDrive为对照负例，证明Latent Bridge在无增益场景下是惰性的。

Conclusion: 潜在桥是文本桥的有效替代或升级方案，收益可预测且高度可解释。两种通道不应同时使用。该方法为实时Agent的延迟-质量权衡提供了实用的解决方案。
...
TL;DR: RaDaR是一个用于罕见病诊断的开源紧凑推理大语言模型（32B参数），通过49,170个公共病例和104,666个合成病例进行训练，在公开基准测试和四个外部验证中心上表现优于DeepSeek-R1等开源模型，并在回顾性队列中将最终诊断优先于临床怀疑61.06%的病例，在随机医师辅助试验中将罕见病诊断准确率提高21.44个百分点。


<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球数百万人，但由于专业临床知识的稀缺，及时诊断仍然是一项重大公共卫生挑战。现有大语言模型在临床部署性、临床证据和训练数据方面存在局限，需要一个专门针对罕见病诊断的可行解决方案。

Method: 使用49,170个公开可用自由文本病例和104,666个推理增强的合成病例进行训练，开发了一个名为RaDaR的32B参数开源紧凑推理模型，并在公开基准测试、四个外部验证中心、回顾性队列和随机医师-assistance试验中进行评估。

Result: RaDaR在评估的开源模型中表现最强，包括在公共基准测试和四个外部验证中心上优于671B的DeepSeek-R1。在回顾性队列中，RaDaR在61.06%的病例中将最终诊断优先于记录的临床怀疑，潜在领先时间为1.87个月，占中心内间隔的50.18%。在随机医师辅助试验中，RaDaR辅助将医师的罕见病诊断准确率比仅使用互联网搜索提高了21.44个百分点。合成数据消融表明，表型锚定叙事为长尾罕见病提供了有用的训练信号。

Conclusion: RaDaR及其开发和验证框架提供了一个可用于临床部署的罕见病推理模型和在数据稀缺条件下开发诊断AI的可重复开发框架。
...
TL;DR: 本文提出一种基于自主视觉语言评估的强化学习微调框架，用于提升计算机使用代理在图形用户界面环境中的任务执行能力。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理在开放式的桌面环境中难以获得可扩展的机器可读奖励信号，因为任务成功通常是视觉基础的，难以用手工设计的奖励函数或密集的人工标签来指定。

Method: 提出一种RL微调框架，利用自主视觉语言评估作为可扩展的监督信号。具体而言，给定最终截图和原始指令，视觉语言模型判断任务完成并提供终端反馈，无需任务特定启发式方法或人工标签。由于自主评估器存在不完美性，将评估器反馈建模为噪声二进制奖励通道，并推导出针对近端策略优化的噪声校正奖励估计器。

Result: 在macOSWorld、Windows Agent Arena和OSWorld三个基准测试上的实验表明，校正后的评估器奖励优于零样本基线和原始评估器奖励，零样本性能平均提升12.6个百分点，原始评估器微调提升5.1个百分点。

Conclusion: 结果表明，当评估器噪声被明确建模和校正时，自主评估可以作为GUI环境中强化学习的实用奖励信号。
...
TL;DR: 本文提出并解决了多智能体LLM环境中的fleet-memory问题，定义了四个基础失败模式及对应的系统级原语，通过MemClaw生产服务和ArgusFleet评估框架进行验证。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM环境需要强大的共享知识管理机制，但现有长文本检索方法不足以应对生产环境中的治理挑战。

Method: 定义了四个系统级原语（作用域检索、时间超继承、来源追踪、策略治理的记忆传播），在MemClaw生产多租户记忆服务中实现，通过ArgusFleet可复现测试框架评估四个治理维度。

Result: 来源追踪达到100%深度四推导链重建，单搜索轮次延迟；高租户内可见性，零跨租户泄漏。发现两个生产问题：非对称作用域执行（子租户作用域被绕过）和管道排序冲突（同步近重复门过早拒绝矛盾写入）。

Conclusion: 长文本检索不足以支撑生产多智能体记忆。治理共享记忆需要明确的系统级抽象，实物评估对暴露设计和执行中的失败至关重要。
...
TL;DR: 论文引入了一个匹配的桌面任务基准测试（440任务/18应用/12类别），用于控制比较GUI代理和CLI代理的性能。研究发现：最强GUI代理（59.1%）优于原始CLI代理（48.2%），但验证器引导的技能增强后CLI达69.3%超越GUI，表明CLI劣势主要来自技能覆盖不足而非模型能力。


<details>
  <summary>Details</summary>
Motivation: 现有评估混淆了交互模态与任务、初始状态、验证器和允许操作的差异，无法真正比较GUI和CLI代理的能力。需要控制变量下的公平对比。

Method: 构建440个桌面任务的匹配执行层基准测试，涵盖18个应用和12个工作流类别。GUI代理和CLI代理接收相同目标、相同状态、相同最终状态验证器，但限制使用各自模态原生的动作集。

Result: 最强GUI代理达59.1%完全通过率，优于最强原始CLI代理的48.2%；但验证器引导的技能增强将CLI成功率提升至69.3%，表明CLI的大部分劣势来自技能覆盖不完整而非模型能力本身。

Conclusion: GUI和CLI代理暴露不同的执行瓶颈：GUI代理受限于长时序工作流中的可靠接地交互能力，CLI代理受限于技能接口的覆盖度和可扩展性。
...
TL;DR: 研究将格雷厄姆经典价值投资原则作为"低通滤波器"，验证其在现代AI复杂模型中的风险控制效果。使用XGBoost、AutoGluon和Random Forest对比纯格雷厄姆规则、现代市场因素和混合特征三组特征，在20年S&P 500数据上的投资表现。


<details>
  <summary>Details</summary>
Motivation: 现代金融中复杂的机器学习模型容易"记忆"短期市场噪声而非发现真正的长期价值。研究旨在验证格雷厄姆的经典价值投资规则能否作为数学"低通滤波器"约束现代AI模型的风险。

Method: 构建三组特征：纯格雷厄姆规则、现代市场因素、混合特征。使用XGBoost和AutoGluon等复杂模型以及Random Forest，在20年S&P 500数据上测试。采用严格买入持有策略，运行四年测试期（2022年3月至2026年3月）。

Result: 复杂模型未获胜。AutoGluon获222.68%高回报但在市场崩盘前买入波动科技股，遭受39.78%最大回撤。纯格雷厄姆Random Forest获232.13%最高总回报，卡尔马比率仅1.38（风险最低）。混合Random Forest结合动量与格雷厄姆规则，获202.91%回报，保持最低最大回撤34.53%。

Conclusion: 格雷厄姆的"安全边际"原则未过时，是有效防止现代AI承担过多风险的高度有效方法。简单的格雷厄姆规则特征配合Random Forest模型，优于复杂的AutoGluon模型。
...
TL;DR: 本文研究了小型LLM作为法律助手时的过度拒绝（overrefusal）偏见问题，发现权威性角色前缀（如"国家最高法院助手"、"辩护律师"）会使拒绝率增加2-20倍，表明小型本地部署的LLM在机构用户可能自然引入的上下文框架下不稳定。


<details>
  <summary>Details</summary>
Motivation: 法律专业人士已经在尝试使用个人LLM进行翻译和改写等任务，但即使这种看似无害的使用也可能通过案件处理速度引入偏见，因为LLM助手可能会选择性地拒绝协助某些主题。

Method: 研究团队调查了几种最有可能用作设备助手的现代小型LLM，评估了权威性前缀、角色扮演越狱前缀对法律提示的过度拒绝影响，并与无前缀基线进行对比。

Result: 权威性风格的前缀（如"你作为国家最高法院的助手"、"辩护律师"）系统性地将拒绝率提高至无前缀基线的2-20倍；已知的角色扮演越狱前缀效果混合，在某些模型中急剧增加拒绝，在其他模型中几乎没有影响。

Conclusion: 小型本地可部署的LLM在真实机构用户可能自然引入的上下文框架下不稳定，进一步研究对于最大限度地减少偏见机会至关重要。
...
TL;DR: AdversaBench是一个端到端的红队评估管道，通过5个结构化算子变异种子提示词，查询目标模型，并使用三人法官小组确认失败。在45个种子上的实验表明每个种子都产生了确认的失败。


<details>
  <summary>Details</summary>
Motivation: 大规模对抗性评估需要同时解决生成困难输入和可靠确认失败真实性的问题。

Method: 使用5个结构化算子变异种子提示词，查询目标模型，通过3人法官小组加元法官打破平局来确认失败。实验覆盖推理、指令遵循和工具使用三个类别的45个种子。

Result: 1)算子效果因类别差异大：inject_distractor在指令遵循上为0.00但在推理和工具使用上为0.80-0.83；2)二元失败率隐藏难度：指令遵循平均需要2.4次攻击迭代vs其他类别的1.1次；3)法官一致率80-87%但Cohen's kappa接近0；4)针对Llama 3.1 8B的对抗提示可零样本迁移到Llama 3.3 70B。

Conclusion: 红队管道有效，但算子效果因任务类别而异，难度指标需要谨慎选择，对抗变异利用的是通用行为模式而非模型特定弱点。
...
TL;DR: ASALT是一种多智能体强化学习中的迁移学习方法，通过观察级和状态级适配器将目标域的观测和全局状态映射到共享嵌入空间，从而解决跨域状态空间维度不匹配问题，实现知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有MARL迁移学习方法要求源域和目标域的状态空间维度必须相同，这一限制严重阻碍了知识在不同维度异构域之间的迁移，因此需要一种能处理维度不匹配问题的迁移方法。

Method: 提出ASALT方法，包含观察级适配器和状态级适配器，将目标域的观测和全局状态映射到共享嵌入空间，使智能体和评论家都能进行有效的跨域知识迁移。这些适配器可生成支持异构域间策略迁移的嵌入表示。

Result: 在标准基准环境上的多种配置实验表明，ASALT在协作设置下的样本效率和全局回报方面均优于现有基线方法，但其有效性取决于源域和目标域之间的维度不匹配程度。此外，ASALT还能缓解负迁移问题。

Conclusion: ASALT成功实现了跨异构域的策略迁移，有效解决了状态空间维度不匹配问题，并在样本效率和性能方面取得显著提升，同时减轻了负迁移现象的影响。
...
TL;DR: 提出了一种结合序贯诊断预测、多视界轨迹生成和不确定性分解的概率深度学习框架，用于阿尔茨海默病进展建模。该框架使用适配的Temporal Fusion Transformer编码器和自回归Mixture Density Network，在ADNI数据集上 outperformer基线模型，并能有效分离任意不确定性和平息不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法将阿尔茨海默病诊断简化为单步分类，将认知正常、轻度认知障碍和痴呆视为扁平类别，无法提供关于未来访视中不确定性如何累积的见解。临床需要的不仅是预测最可能的下一个诊断，还要描述患者如何随时间演变以及该预测的可靠性。

Method: 使用Temporal Fusion Transformer编码器配合CORAL序贯输出层、非对称损失加权器和转换器过采样以尊重疾病阶段顺序；基于学习到的患者上下文表示，自回归Mixture Density Network生成诊断状态、CDR量表总分、MMSE定向和海马体量的五年概率轨迹；使用解析混合方差和五成员bootstrap集合分离任意不确定性和平息不确定性。

Result: 模型在next-visit诊断预测上优于线性、循环和transformer基线模型，在MCI与痴呆区分上增益最强；生成轨迹达到名义上90%可信区间覆盖，不确定性随预测视界扩大，生物标志物动力学与预期的阿尔茨海默病进展一致；平息不确定性在罕见进展原型、MCI和痴呆患者中更高，在OASIS-3外部评估中随预测误差增加而增加。

Conclusion: 该框架能够可靠预测阿尔茨海默病长期进展轨迹，并提供有意义的不确定性量化，为个性化临床决策支持提供了有价值的工具。
...
TL;DR: ScaleToT是一个针对数十亿低活跃用户的大规模用户建模系统,通过从小规模LLM处理数据中学习结构化推理,然后迁移到轻量级模型,在广告场景的LTV预测中实现了6.738%的提升,同时仅需7.32%的离线推理覆盖率。


<details>
  <summary>Details</summary>
Motivation: 准确的用户建模通常依赖于丰富的交互历史,但数十亿低活性用户缺乏这些数据。LLM可以从静态画像推断用户状态,但在画像稀疏时推理不可靠,且对数十亿用户应用LLM成本过高。

Method: ScaleToT通过以下步骤实现:(1)使用有界熵引导的Tree-of-Thought(ToT) refinement构建类型化用户状态链;(2)使用教师精选的链通过监督微调(SFT)和结果驱动的分段感知隐式奖励策略优化(OSIPO)训练学生模型;(3)将学生的推理表示迁移到轻量级画像编码器,为剩余用户提供共享推理信号,无需LLM推理。

Result: 在十亿级广告投放的LTV预测中评估,随机在线A/B测试使LT30增加6.738%,离线推理仅覆盖潜在人口的7.32%,大幅降低计算成本。

Conclusion: ScaleToT成功地将结构化推理从一小部分用户扩展到大规模低活性用户群体,在保持推理可靠性的同时显著降低了计算成本,为大规模推荐系统提供了一种经济高效的解决方案。
...
TL;DR: 本文提出AI代币经济学框架，研究代币在AI系统中的生成、消耗、定价、分配和优化，连接技术成本与企业资源配置，揭示代币支出与经济价值的区别。


<details>
  <summary>Details</summary>
Motivation: 现代基础模型服务中，代币成为实际计量单位，但缺乏系统性的代币经济学研究框架来理解其技术成本、经济价值和市场设计问题。

Method: 建立连接代币级技术成本与工作流级生产函数的框架，整合企业资源分配、测量方法和市场设计问题。

Result: 框架表明代币支出与经济价值不同，价值取决于边际生产力、工作流位置、隐藏推理活动、风险和下游传播效应。

Conclusion: 识别了隐藏代币测量、实证校准、代币生产力、动态分配和代币市场等开放研究方向。
...
TL;DR: 本文研究基于本体论的数据访问(OBDA)系统中的查询抽象问题。研究者扩展了UCQ(联合 conjunctive查询)，加入不等式和特殊谓词来处理数据库常量，并建立了与数据交换中最大恢复概念的关联。


<details>
  <summary>Details</summary>
Motivation: 解决OBDA中数据查询到本体层翻译的查询抽象问题。由于完美抽象可能不存在，需要引入最小完整和最大 sound抽象的概念。

Method: 基于存在性规则和确定答案语义，研究UCQ的扩展形式，添加有限的不等式和标记数据库常量的特殊谓词。

Result: 该扩展不会增加问题的复杂性，能够表达最小完整抽象(在存在完美抽象时即完美抽象)，并通过与数据交换中最大恢复概念的关联来表征最大 sound抽象。

Conclusion: 提出的扩展为OBDA中的查询抽象提供了有用的框架，建立了抽象理论与最大恢复概念的连接，且保持了计算可行性。
...
TL;DR: 本文研究了能力问题(CQ)验证中的挑战，通过实验19名参与者在LLM助手支持下完成20项CQ验证任务，发现需要工具在发布前优化CQs以避免歧义和复杂性。


<details>
  <summary>Details</summary>
Motivation: CQ验证过程耗时且容易出错，CQ中的歧义和复杂性会导致不一致的建模决策和验证结果，需要找出解决方案来提高用户在CQ验证中的表现。

Method: 使用19名参与者在LLM助手支持下完成20项CQ验证任务的实验数据进行分析。

Result: 结果表明需要在本体工程后期阶段之前使用工具对CQs进行优化，以避免歧义或过度的复杂性。

Conclusion: 建议开发专门的工具来在发布前 refinement（优化/精炼）CQs，提高CQ验证的效率和准确性。
...
TL;DR: Themis是一个结合可解释性(XAI)和人类反馈的强化学习测试评估框架，支持200+环境，可训练匹配或超越真实奖励信号的奖励模型，并提供云平台支持大规模实验。


<details>
  <summary>Details</summary>
Motivation: 训练安全的强化学习系统面临挑战，难以避免不良行为。现有的两种有效防御方法——可解释性（透明度）和人类反馈对齐——各有优势，但缺乏公开框架将两者结合。

Method: 构建名为Themis的XAI-enabled测试评估框架，用于强化学习从人类反馈(RLHF)；框架支持200+广泛使用的环境，可灵活配置用于RL、透明度和对齐实验；提供云平台收集人类反馈和管理实验，具备自动扩展能力。

Result: Themis可以训练出匹配或超越环境真实奖励信号的奖励模型；云平台支持在普通商业机器上运行一千用户的后台实验。

Conclusion: Themis成功结合了可解释性和人类反馈两种方法，为RL安全训练提供了有效的评估框架，且具备良好的可扩展性和易用性。
...
TL;DR: SAFARI是一个框架，通过工具增强的诊断循环和短期记忆来应对自治智能体轨迹超出LLM上下文窗口限制时的故障归因问题。


<details>
  <summary>Details</summary>
Motivation: 随着自治智能体处理复杂的多步骤、多智能体任务，其执行轨迹规模已超出最大上下文窗口限制。传统方法将完整轨迹加载到LLM上下文中存在注意力稀释问题，且在轨迹超出上下文限制时完全失效。

Method: SAFARI用工具增强的诊断循环替代线性上下文加载，为LLM配备专门的工具箱来读取和搜索轨迹片段，同时使用持久的短期记忆(STM)进行跨轮次推理，有效解耦诊断准确性与架构上下文限制。

Result: 在Who&When数据集上比最先进结果提高20%(1M token预算)，在TRAIL GAIA子集上提高19%(25K token预算)。当故障位置在模型原生上下文窗口5倍距离时仍保持0.58精确度，而传统评估器完全失败。

Conclusion: SAFARI成功实现了在超出上下文限制的长程轨迹中进行准确故障归因，为处理大规模智能体执行轨迹提供了有效方案。
...
TL;DR: CineCap是一个用于电影摄影字幕生成的新框架，结合结构化推理与时空锚点和强化学习，在472个视频-字幕对的基准测试上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 电影摄影字幕生成对于细粒度视频理解和可控电影质量视频生成很重要，但现有多模态大语言模型对此探索不足。该任务需要统一的开放形式描述多个电影摄影维度，且面临两大挑战：从细微视觉证据推断专业电影摄影概念，以及生成全面准确的字幕。

Method: 提出CineCap框架，包含两个核心组件：1) 结构化推理与时空锚点：将专业电影摄影描述基于显式视觉证据并组织为紧凑的原子推理用于监督微调；2) 强化学习使用全面性、准确性和门控覆盖率奖励，改善描述完整性与事实正确性之间的平衡。同时构建了CineCap Bench基准，包含472个手动标注的视频-字幕对。

Result: 广泛实验表明CineCap始终优于强大的专有和开源基线方法，建立了电影摄影字幕生成的新技术状态。

Conclusion: CineCap框架通过结合结构化推理和强化学习方法，有效解决了电影摄影字幕生成中概念推断和描述准确性的挑战，为该领域提供了新的研究和应用基础。
...
TL;DR: 本文提出LaGO框架,利用预训练语言模型作为潜在动作先验来引导在线强化学习策略优化,而非直接将语言模型作为控制器使用。该方法在离散控制任务CLEVR-Robot和连续控制任务Meta-World上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 先前工作通常将大语言模型作为直接控制器使用,这需要精确的动作生成,在实践中往往不可靠。需要一种更有效的方式来利用语言模型的知识进行规划和序列决策。

Method: 提出潜在动作引导(LaGO)框架,使用预训练大语言模型作为潜在动作先验,以软约束的方式引导在线策略优化,而非将LLM视为显式规划器或控制器。

Result: 在CLEVR-Robot任务上,成功率从15.1%提升至27.2%;在Meta-World任务上,成功率从2.7%提升至15.2%。更强的预训练语言模型提供更有效的引导,表明LLM知识可改善规划和在线决策能力。

Conclusion: LLM知识能够有效提升强化学习中的规划能力和在线决策性能,为语言模型与强化学习的结合提供了新的思路。
...
TL;DR: 在变量成本和真值分配概率分布下,研究如何构造确定性求值策略以最小化命题公式的期望求值成本,提出了一种分支定界算法。


<details>
  <summary>Details</summary>
Motivation: 在许多决策场景中,获取信息需要不同成本,需要一个实际可用的精确算法来求解此类问题。

Method: 提出了一种带有变量选择启发式、剪枝和缓存的分支定界算法,并设计了贪心束搜索变体进行效率-质量权衡分析。

Result: 该算法是目前首个具有这种通用性的实用精确算法,在随机实例上展示了可扩展性,并在心脏病诊断实例上进行了评估。同时证明了该问题是#P-hard的,属于PSPACE复杂度。

Conclusion: 该研究为命题公式的最小成本求值问题提供了有效的精确求解方法,具有实际应用价值。
...
TL;DR: 本文提出Spheroid BlockTrain，一种去中心化训练协议，将模型分割为独立可训练的块，在推理时组合成完整模型。该方法在有限资源下实现了接近端到端训练的 perforance，为独立AI开发提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练主要由拥有大量 accelerator cluster 的科技公司主导，形成结构性优势。独立或开源AI项目受限于稀缺资本、特权基础设施和数据中心地理位置，迫切需要一种去中心化的训练方案，使小型团队也能参与前沿AI训练。

Method: 将模型分割为独立可训练的块，每个块基于相同的全局目标优化本地目标，推理时将各块组合成完整模型。使用HTTP/TCP传输序列化检查点和更新，通过块栈遍历实现跨多个GPU主机的推理。

Result: 在byte-level WikiText上达到交叉熵1.359（困惑度3.89），与同设置端到端Transformer参考相差约0.04 CE。六工作节点训练达到CE 1.385。三主机公开IP测试将CE从5.580降至1.811，传输15.22 GB。可服务达75.80B参数的fp16模型，优于传统自回归TCP流水线。

Conclusion: BlockTrain为去中心化AI训练提供了可行方案，以极小资源需求实现接近中心化训练的 perforance，使独立AI开发不再依赖大规模资本和基础设施，为AI领域的长尾创新奠定基础。
...
TL;DR: 该论文研究大型语言模型在特定领域（量化金融）部署时的压缩策略，通过推导经验 scaling law 来量化知识保留与压缩参数的关系。


<details>
  <summary>Details</summary>
Motivation: LLM 规模日益增长，但在延迟和成本受限的应用场景中部署困难，需要在保持领域任务性能的同时压缩模型。

Method: 采用迭代结构剪枝，结合 logit-based 和 LoRA-based 蒸馏方法，引入混合 chain-of-thought 监督损失来稳定 KL 散度蒸馏。

Result: 领域任务质量随压缩可预测下降；通用知识基准在更早阶段就崩溃；监督格式是权衡的关键驱动力；CoT 监督能恢复被剪枝擦除的通用知识。

Conclusion: 发布 FinHeadlineMix 数据集、scaling law 结果和实用建议，为领域特定压缩决策提供可复用框架。
...
TL;DR: 研究在GPT风格Transformer模型中是否还存在可塑性问题。在5M到314M参数的模型中，发现即使是大型语言模型，在持续训练后也会失去适应新数据的能力。


<details>
  <summary>Details</summary>
Motivation: 可塑性丧失是持续学习中的基础挑战，此前多在较小架构中研究，很少在自然语言领域探索。需要确定在现代Transformer LLM范式中该问题是否仍然存在。

Method: 研究GPT风格Transformer模型（5M到314M非嵌入参数），在多语言持续学习问题上训练，通过越南语探测任务测量可塑性丧失。

Result: 在所有模型规模中都发现可塑性丧失证据。可塑性丧失 onset 遵循可预测的缩放定律，随模型规模次线性增长。在 stationary 多语言训练下也发现可塑性丧失，挑战了该现象仅限于突发任务变化持续学习的观点。

Conclusion: 更大模型可能延缓可塑性丧失的可测量效果，但仅增加参数数量可能不足以完全防止。即使大型Transformer语言模型在自然语言上训练，在足够长的训练后，在持续学习和 stationary 设置下都会最终失去有效适应新数据的能力。
...
TL;DR: BluTrain是一个从第一性原理构建的轻量级C++/CUDA深度学习训练框架，通过原生实现每一层（张量模块、自动微分、线性代数库、缓存分配器、分布式执行模块和MLIR编译器）来实现对硬件的绝对控制。评估表明，在8-GPU 6000 Ada系统上训练124M参数GPT-2模型时，BluTrain的吞吐量（407K tokens/s）优于PyTorch（395K tokens/s），内存占用降低22%，同时保持数值保真度。


<details>
  <summary>Details</summary>
Motivation: 深度学习的进展在更大程度上取决于系统工程而非模型架构：模型训练过程中的行为（吞吐量、内存占用、数值保真度）更多由架构在硬件上的表达方式决定，而非架构本身。需要实现对硬件表达的绝对控制，同时抽象化系统复杂性，使建模无缝化。

Method: 从第一性原理设计BluTrain框架，使用标准C++和CUDA核心编程模型，原生实现每一层：带反向模式自动求导的类型化张量模块、线性代数库、缓存分配器、多模式分布式执行模块和基于MLIR的深度学习编译器。在8-GPU 6000 Ada系统上训练124M参数GPT-2基线模型（FP32）进行正式评估。

Result: 在吞吐量方面，BluTrain持续达到平均407K tokens/s，而PyTorch为395K tokens/s；内存效率方面，实现高达22%的内存占用降低；严格保持数值保真度；收敛到略低的最终验证损失。

Conclusion: BluTrain在吞吐量和内存效率上均优于行业标准PyTorch，同时保持数值保真度。由于每一层都明确开放原生调优，性能天花板取决于框架本身的提升空间。
...
TL;DR: 本文系统性地研究了在传感器-based人类活动识别(HAR)中，4种不同类型分布偏移（设备类型、传感器位置、采样率、用户行为）对模型泛化性能的影响，并创建了统一的HAR分布偏移基准，对28种领域泛化方法进行了综合评估。


<details>
  <summary>Details</summary>
Motivation: 现实世界的HAR应用面临数据和传感器异构性以及上下文变化带来的挑战。虽然数据多样性已被广泛认可，但各种分布偏移对HAR模型的影响以及领域泛化问题仍未得到充分理解。该研究旨在系统性地评估不同类型的分布偏移并揭示当前领域泛化方法的局限性。

Method: 系统性地评估了4种不同类型的分布偏移：设备类型变化、传感器位置变化、采样率变化和用户行为变化。对多达28种领域泛化方法进行了综合评估，并引入统一的基于HAR的分布偏移基准和开源数据集。

Result: 研究表明多样性偏移主要定义了所有类型的偏移，表明存在不同领域之间不共享的独特特征。当前领域泛化算法在实现模型泛化性方面存在明显局限性，仅略优于经验风险最小化(ERM)基准。

Conclusion: 这是首次对传感器-based HAR中特定分布偏移进行领域泛化和适应的系统探索。研究创建了开源基准平台和数据集，为推动该领域的进一步研究提供了重要资源。
...
TL;DR: 本文提出双向条件流匹配(Bi-CFM)方法解决混沌动力学中的逆问题，即从最终状态推断初始条件。该方法学习初始状态和最终状态之间的双向映射，捕获混沌演化的随机性并减轻指数误差积累。对于满足守恒律的系统，进一步提出守恒约束的CBi-CFM。在Lorenz、Lorenz96系统和三体行星散射问题中验证了其有效性，并在真实球状星团观测中展示了可扩展的应用前景。


<details>
  <summary>Details</summary>
Motivation: 混沌动力学中的逆问题（从最终状态推断初始条件）长期未解决，因为本身是病态的、非唯一的、不稳定的，且时间反向动力学可能也是混沌的。现有方法在精度和效率上存在局限。

Method: 提出双向条件流匹配(Bi-CFM)，学习初始状态和最终状态分布之间的双向映射，捕获混沌演化的随机性并减轻指数误差积累。对于有守恒律的系统，扩展为守恒约束的CBi-CFM。

Result: 在经典Lorenz、电路和高维Lorenz96系统中，Bi-CFM在5个分布级指标上超越基线，且推理速度提升超过两个数量级。在三体行星-行星散射问题中，CBi-CFM较好地遵守守恒律，守恒误差与真实值相当。在球状星团的真实观测（碰撞百万天体系统，10 Gyr演化）中，该方法在精度上取得显著进展。

Conclusion: Bi-CFM/CBi-CFM为混沌动力学逆问题提供了高效、可扩展的解决方案，在保持守恒律的同时实现了高精度和快速推理，为解决真实世界中长时间尺度的混沌动力学逆问题开辟了可行路径。
...
TL;DR: 本文批判分析了Andreas & Günther系列论文中提出的七种因果关系定义，揭示这些定义之间的界限并不清晰。


<details>
  <summary>Details</summary>
Motivation: 旨在质疑因果关系理论中不同类型账户之间的区分是否真正具有本质差异。

Method: 通过比较分析和逻辑论证，检验最新定义与其他六个定义的关系。

Result: 发现factual difference-making定义同时涵盖三种类型，且所有七个定义都存在问题。

Conclusion: 这些看似不同的因果关系定义实为"没有区别的区别"，整个分类体系需要重新审视。
...
TL;DR: 本文研究LLM对话助手（如GitHub Copilot）在处理医疗保健领域非功能需求（NFRs）时的多轮对话质量，填补了当前评估基准仅关注功能正确性的空白。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估基准仅关注功能正确性，缺乏对非功能需求（NFRs）处理质量的评估。NFRs具有模糊性、上下文依赖性，涉及程序多个部分，需要多轮协作推理，因此需要新的评估方法来捕捉系统输出的正确性和多轮交互质量。

Method: 研究招募49名程序员与GitHub Copilot交互，评估148个HIPAA衍生的NFRs在iTrust代码库上的合规性，从需求满足水平、推理和代码定位三个维度进行评估，并建模用户满意度。

Result: 开发者倾向于同意LLM的评估结果，但与专家标准答案对比的准确率较低。研究发现更长的系统响应和信息提供轮次对用户满意度有负面影响，而主动交互则对满意度有正面影响。

Conclusion: 本研究为设计支持NFR评估的LLM对话系统提供了有价值的见解，表明需要在系统响应长度和交互主动性之间找到平衡，同时需要改进NFR评估的准确性。
...
TL;DR: 本研究评估了自动化评分系统在代理式数据分析系统(Agentic Data Analysis)上的效果，提出三层人机评分级联方案，在153个数值任务上达到100%精确率和97%召回率。


<details>
  <summary>Details</summary>
Motivation: 代理式数据分析系统产生丰富输出(代码、数值结果、口头诊断)，比单轮LLM响应更难评估。需要区分系统输出与真实答案的真正分歧与评分伪影。

Method: 在DSGym的153个数值QRData任务上应用多代理数据分析系统LAMBDA，开发三层人机评分级联：严格正则匹配、基于LLM的宽松评分、基于snippet的人工检查，结合非GenAI和GenAI策略。

Result: 两个自动化评分器均达到100%观察精确率(0/70误报)。宽松评分器对人工标签的召回率为97%。关键词锚定提取管道将严格评分器的召回率提升60个百分点。迭代nudge机制使评分运行成功率从36%提升到97%，宽松通过率从16%提升到46%。变量类型是与评分管道动态和观察结果最相关的任务元数据字段。

Conclusion: 自动化评分系统可有效评估代理式数据分析系统，三层评分级联结合非GenAI和GenAI策略能实现高质量评估。nudge机制是关键提升手段，而原始问题重注入无额外收益。变量类型是影响评分管道表现的重要因素。
...
TL;DR: 本文提出MTO框架，通过匹配任务目标与预训练目标来提升encoder-decoder模型在生成和问答任务上的性能，特别是在少样本场景下性能提升超过120%。


<details>
  <summary>Details</summary>
Motivation: 当前预训练模型在不同预训练目标下的性能差异机制尚不明确，缺乏系统性的任务-目标匹配方法，导致模型选择和优化缺乏有效指导。

Method: 提出MTO框架，包括识别任务目标、自动化准备任务相关数据进行无监督适应、以及设计对齐预训练目标的模板；在微调阶段引入提示调优方法进行软提示工程优化。

Result: 少样本设置下性能提升超120%，显著优于相关工作；在全数据集场景下也超越基线；显著提升提示调优性能。

Conclusion: MTO框架能有效指导模型选择和任务特定优化，为特定任务定制模型的选型和优化提供实质性指导。
...
TL;DR: 本文研究大世界环境下通用智能体的能力局限性问题。研究者证明在big-world regime下，通用智能体并非真正"通用"的，其能力必然需要针对特定世界模型进行专业化。标准的最坏情况分析无法区分关键瓶颈与无关失败。为解决这一问题，论文提出结构性认证框架，通过深度组合目标过滤特定转换，使智能体在局部转换上具有可证明的可靠性。


<details>
  <summary>Details</summary>
Motivation: 在big-world regime下，智能体无法成为真正通用的万能选手，其能力必须在不同世界模型上进行专业化划分。传统的统一保证（如最坏情况分析）无法有效区分哪些是真正的关键瓶颈，哪些只是无关的失败。这导致无法可靠地部署通用智能体进行长程规划。

Method: 1. 形式化证明通用智能体不是真正通用的，这使得标准最坏情况分析失去信息量。2. 提出结构性认证（structural certification）框架，这是一个转换局部的框架，将有界目标条件性能映射到智能体内部世界模型的逐项保证。3. 提供使用深度组合目标过滤特定转换的算法。

Result: 证明具有深度组合目标的通用智能体具有结构化世界模型，误差界为O(1/n) + O(δ)。该界在小δ regime下是紧的，而小δ regime的存在由认证明确保证。这些结果使得可以通过局部化特定转换来可靠地部署通用智能体成为可能。

Conclusion: 论文通过结构性认证框架解决了big-world regime下通用智能体的可靠性证明问题，提供了一种将全局性能保证转化为局部转换保证的方法。这使得长程规划在特定转换上的可靠性可以得到认证，为通用智能体的可信赖部署奠定了理论基础。
...
TL;DR: OT-Agent项目通过完全开放的数据策展流程训练智能体模型，使用100K样本微调Qwen3-32B，在七个智能体基准测试中平均准确率达44.9%，比现有最强开源模型Nemotron-Terminal-32B提升3.9个百分点，且展现出良好的规模化特性。


<details>
  <summary>Details</summary>
Motivation: 现有开源智能体训练数据工作（如SWE-Smith、SERA、Nemotron-Terminal）通常只针对单一基准，缺乏能够泛化到多种智能体任务的训练数据策展方法。

Method: 通过100多次控制变量消融实验系统研究数据策展流程的每个阶段，评估任务来源和多样性的重要性，最终组装10万样本的训练集并微调Qwen3-32B模型。

Result: 在七个智能体基准测试中平均准确率达44.8%，比Nemotron-Terminal-32B（40.9%）提升3.9个百分点；在受控计算比较中，训练数据在各个训练集规模下均优于其他开源数据集。

Conclusion: OT-Agent项目提供了首个完全开放的智能体模型训练数据策展流程和数据集，证明多样化任务来源的重要性，并展现出良好的规模化特性，为智能体模型的开源研究提供了重要资源。
