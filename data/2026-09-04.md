...
### [1] [Untitled](https://arxiv.org/abs/2609.01659)
**
...
### [2] [Untitled](https://arxiv.org/abs/2609.01683)
**
...
TL;DR: FAIRLENS是一个用于评估视觉语言模型(VLM)在高风险领域(招聘、法律、医疗)决策公平性和有效性的基准测试框架，通过10万+图像-问题配对评估模型的四种视角：人口统计 parity、不合理性、未经支持的关联以及自由文本生成偏见。


<details>
  <summary>Details</summary>
Motivation: VLM在高风险决策中的应用日益广泛，但缺乏系统性的公平性和有效性评估标准。现有的 disparity metrics 难以捕捉模型的不当推理问题。

Method: 使用跨越性别、种族和年龄的真实人脸图像，配合封闭式和开放式问题，构建超过10万对图像-问题组合，从四个互补视角评估VLM响应：人口统计不利结果率、soundness(合理性)、未经支持角色和状态的人口统计关联、以及自由文本生成偏见。

Result: 主要失败原因是"不当推理"而非"不平等对待"。模型经常从人脸推断资质、威胁、疾病或职业角色，而不是拒绝回答。最弱的模型在99%无法回答的问题上仍然进行推理。法律和医疗领域问题最严重。parity gaps虽小但影响重大，自由文本偏见与多项选择准确性松散相关。

Conclusion: 公平的高风险VLM行为需要跨群体的类似处理方式，以及拒绝从外貌推断高风险属性。FAIRLENS的问题套件可转移至任何具有人口统计注释的人脸语料库。
...
TL;DR: Cue2Narrate是一个两阶段管道，联合预测长电影片段中"说什么"(视觉事件)和"何时说"(AD插入位置)，解决传统方法忽略时序定位的问题。


<details>
  <summary>Details</summary>
Motivation: 现有工作将问题简化为预分割视频片段的字幕生成，忽视了"何时叙述"这一关键时序定位问题。音频描述需要同时确定描述内容和插入位置，以获得最佳用户体验。

Method: 提出两阶段管道：1) 双头视听定位器预测每个AD语句的两个时间窗口(视觉提示窗口和口语叙述窗口); 2) LoRA适配的VLM根据预测的视觉证据生成简洁AD，使用描述排序损失(Description Ranking Loss)将同一帧的负样本描述排在GT AD之下。构建LongLSMDC基准测试，包含长达8分钟的电影片段。

Result: 在LongLSMDC上，Cue2Narrate在avg. mAP上比纯视频和纯音频定位基线高出5-12个百分点。在预测窗口和GT窗口评估下，Cue2Narrate的AD生成均优于对应的微调基线VLM。

Conclusion: 首次为长片段多段AD生成建立了基准。提出的两阶段方法有效解决了联合预测"说什么"和"何时说"的核心问题。
...
TL;DR: 研究利用无人机热成像技术结合YOLOV11l和RT-DETR-R50目标检测算法，开发了针对未爆弹药检测的自动化候选识别模型，为人道主义排雷行动提供筛查和优先级排序工具。


<details>
  <summary>Details</summary>
Motivation: 未爆弹药（UXO）继续限制受污染地区的民众进入、农业活动、基础设施恢复和环境修复。人道主义排雷行动和排爆作业需要高效识别和定位这些危险物品的方法。

Method: 研究创建了多航次无人机热成像数据集，开发了带标签的图像集，训练并评估了YOLOV11l和RT-DETR-R50目标检测模型。数据在四个野外部署中收集，涵盖夏季和冬季条件，使用惰性地雷、弹药和其他弹药放置在不同地表（矮草、高植被、砾石、覆盖物、岩石、堆肥和压实表面），飞行高度分别为33米和15米。

Result: 最终数据集包含5,855对热成像标签图像，包括918张正样本图像和4,937张背景图像。33米数据集包含420张训练图像和106张验证图像，15米数据集包含629张训练图像和157张验证图像。YOLOV11l和RT-DETR-R50算法完成了训练和评估。

Conclusion: 研究为人道主义排雷行动提供了实用的技术方案和建议，包括收集热成像和RGB图像、结合多样地表和仅背景图像、考虑太阳照射变化后的时段、平衡覆盖范围与目标像素表示、使用代表性本地数据校准模型以及保留人工审核环节。该方法适用于筛查和优先级排序，而非独立清除作业。
...
TL;DR: ZipTok3D是一种用于极短token序列高保真3D重建的3D分词器，通过嵌套dropout和迭代解码实现32倍（ShapeNet）和8倍（TRELLIS）的压缩率。


<details>
  <summary>Details</summary>
Motivation: 现有3D分词器在极低token预算下重建质量急剧下降，需要一种能够在极少token情况下保持高保真重建的方法。

Method: 1. 将对象几何组织成渐进信息的全局token前缀；2. 训练时使用嵌套dropout随机截断潜在序列，要求每个保留前缀重建完整对象；3. 解码器使用参数共享的Transformer块迭代解码每个前缀以恢复细粒度几何。

Result: 在相同token维度下，ZipTok3D用1个token在ShapeNet上达到32-token COD-VAE基线的重建质量，用4个token在TRELLIS上达到基线质量，分别实现32倍和8倍的短序列压缩。

Conclusion: ZipTok3D通过渐进式信息编码和迭代解码成功实现了极低token预算下的高保真3D重建，为高效3D生成提供了新思路。
...
TL;DR: 本研究评估了证据深度学习(EDL)头、Dempster-Shafer(DS)证据融合和不确定性驱动的时序传感器门控在反无人机系统中的表现，通过在三个基准数据集(AntiUAV600、TRIDENT、MM-UAV)上进行对照消融实验。


<details>
  <summary>Details</summary>
Motivation: 反无人机系统日益融合多个传感器，但其检测头未提供每模态的可靠性信号，需要评估如何通过不确定性估计来改进检测性能。

Method: 在三个基准数据集上进行受控消融实验：AntiUAV600(热跟踪)、TRIDENT(RGB-音频-RF分类)、MM-UAV(RGB-IR跟踪)。评估EDL训练目标、DS证据融合和时序传感器门控三个组件。

Result: EDL训练目标相比重新训练的sigmoid基线提升了准确率(E1提高5.9个百分点，tracker-on-absent率提高两倍；E2分类准确率提高4.8个百分点，经bootstrap检验p=0.011)，错误排序能力显著增强(熵UAUC约0.94 vs 0.51)。但DS融合未优于简单概率平均，Dirichlet vacuity未增加超越预测熵的排序能力，在检测层面出现反转。时序门控仅在几乎不活跃时才保持准确率，在共享骨干网络上未实现延迟节省。

Conclusion: 证据学习的益处主要来自其训练目标而非其不确定性估计；裁剪级控制将检测级故障定位到锚级评估而非学习表示。
...
### [8] [Untitled](https://arxiv.org/abs/2609.01743)
**
...
TL;DR: 本研究针对GAN生成的人脸图像检测问题，提出了一种结合EfficientNet-B0卷积特征提取与Swin Transformer后端的混合架构，在包含StyleGAN生成的假人脸和真实图像的14万数据集上实现了99%准确率和99.44%召回率，优于纯Swin变体和CNN基线模型。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型（GAN、扩散模型、自回归系统）生成的人脸图像已接近照片级真实度，导致伪造图像检测日益困难，引发身份盗窃、欺诈和虚假信息传播等严重安全问题。现有的CNN方法擅长局部纹理特征但缺乏全局上下文理解能力，而传统ViT虽然能捕捉长距离结构但计算资源需求高。

Method: 研究探索了三种Swin-Transformer架构实现：(1) 从头训练的紧凑型Swin Transformer；(2) 基于ImageNet-1K预训练的Swin-Tiny和Swin-Small模型用于二分类；(3) 融合EfficientNet-B0卷积处理与Swin Transformer后端的新型混合架构。实验使用140K真实与伪造人脸数据集（包含StyleGAN生成的假人脸及Flickr和DFDC的真实图像）。

Result: EfficientNet-B0+Swin混合模型在5000张测试图像上达到99%准确率和99.44%召回率，优于纯Swin变体和之前的CNN-only基线模型。

Conclusion: 结合分层CNN特征与移位窗口自注意力机制提供了一种高效且计算轻量级的GAN生成人脸检测方法，混合架构能同时兼顾局部纹理特征和全局上下文理解。
...
TL;DR: AlphaRAD是一种用于胸部影像理解的视觉语言预训练模型，通过从医学报告中提取结构化临床语义来减少对比学习中的批次内噪声，并提出FLaS模块实现更精确的空间定位。在16个分类基准测试中达到最先进水平，并在7个定位和3个分割数据集上取得最佳表现。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言预训练模型在胸部影像理解中的两个未充分探索的问题：1) 如何利用医学报告的结构化临床语义减少对比学习中的批次内噪声；2) 如何设计跨模态融合以产生更准确的空间定位而不增加模型复杂度。

Method: 1. 构建大规模结构化医学概念空间：使用大语言模型解析医学报告，训练医学概念判别器，采用α校正二元交叉熵损失函数；2. 提出FLaS（因子化潜在监督）跨模态融合模块：将VLPM表示分解为独立子空间，使用专用对齐监督增强空间定位表达，且不引入额外参数。

Result: 在16个分类基准测试中达到最先进平均性能；在7个定位/短语定位数据集和3个分割数据集上获得个体最优结果。

Conclusion: AlphaRAD通过结构化医学概念空间和简洁的FLaS融合模块，有效提升了视觉语言模型在胸部影像任务中的零样本泛化能力，为医学影像理解提供了新的有效范式。
...
### [11] [Untitled](https://arxiv.org/abs/2609.01778)
**
...
### [12] [Untitled](https://arxiv.org/abs/2609.01786)
**
...
### [13] [Untitled](https://arxiv.org/abs/2609.01787)
**
...
### [14] [Untitled](https://arxiv.org/abs/2609.01795)
**
...
### [15] [Untitled](https://arxiv.org/abs/2609.01800)
**
...
TL;DR: ShadowCLR是一种无监督阴影去除框架，通过利用不同阴影观察间的一致性作为正则化信号，无需成对数据、阴影掩码或无阴影参考图像即可学习阴影去除，在多个基准数据集上取得了 competitive 甚至更优的性能。


<details>
  <summary>Details</summary>
Motivation: 现有监督学习方法需要成对的阴影和无阴影图像进行训练，而无监督方法通常仍依赖于阴影掩码或无阴影参考图像。这种对成对数据或额外标注的依赖限制了方法的实用性。因此需要一种能够在完全没有配对数据的情况下学习阴影去除的方法。

Method: ShadowCLR采用无监督框架，利用阴影观察间的一致性作为正则化信号。核心思想是：尽管阴影在不同观察中会发生变化，但底层场景内容保持一致。通过全局和局部一致性约束，模型能够从视觉相关的图像中学习，处理不完全对齐的观察，并将表示聚焦于共享的场景信息。

Result: 在多个基准数据集上的实验表明，ShadowCLR达到了与先进无监督方法相当甚至更优的性能，证明了通过一致性进行正则化可以在没有阴影掩码或无阴影图像的情况下实现有效的阴影去除。

Conclusion: ShadowCLR通过利用阴影观察间的一致性作为正则化，实现了无需阴影掩码或无阴影图像的阴影去除。该方法在多个基准数据集上取得了与先进无监督方法相当甚至更优的性能，证明了一致性可以作为阴影去除的有效监督信号。?
...
TL;DR: 本文提出了两种非接触、基于视觉的结构缺陷检测与定量表征方法：一种是激光扫描生成3D点云比较分析；另一种是3D-DIC结合有限元模型更新和拓扑优化。实验验证表明两种方法都能有效识别和量化缺陷几何特征。


<details>
  <summary>Details</summary>
Motivation: 结构缺陷的可靠表征需要能够检测从检查表面直接可见的表面损伤和不可见的内部损伤，现有的方法在分辨率和非接触性方面存在不足。

Method: 方法一：使用高分辨率激光扫描生成受损钢试件的3D点云，通过比较测量点云与参考点云来定位损伤区域、量化几何损失，并将缺陷几何转移到有限元表示中。方法二：结合三维数字图像相关技术（3D-DIC）获取全-field表面变形测量值，与有限元模型更新和拓扑优化相结合，通过测量的表面响应推断 subsurface 异常。

Result: 与铣削基准测量值的比较表明，两种方法都能识别和量化缺陷几何，同时为可见损伤和 subsurface 损伤评估提供互补信息。

Conclusion: 该综合框架为复杂和不规则损伤部件的高保真非接触结构状态评估和模型更新建立了途径。
...
### [18] [Untitled](https://arxiv.org/abs/2609.01816)
**
...
TL;DR: Kirin框架解决动物运动捕捉难题，通过从野生视频重建3D运动，创建了首个大型四足动物视频-文本-运动对齐数据集AiM3D，并开发了视觉引导的运动生成模型，可根据文本和图像生成多样化动物的运动，最终实现3D网格的自动绑定和动画化。


<details>
  <summary>Details</summary>
Motivation: 动物运动研究落后于人类运动研究，主要原因是高质量运动数据稀缺。人类运动可以在受控环境中捕捉，但对大多数动物物种不切实际，导致数据集小且领域受限，限制了动画等下游应用。

Method: 1)从大量野生动物视频中重建3D运动序列并配对字幕，创建AiM3D数据集；2)开发视觉引导的运动生成模型，以文本和图像为条件生成多样化动物的运动；3)利用现成的图像到3D模型自动绑定和动画化3D网格。

Result: 创建了首个大规模四足动物视频-文本-运动对齐数据集；开发了可跨物种生成逼真运动的模型；实现了直接可渲染的动画动物产出。

Conclusion: Kirin框架和AiM3D数据集为大规模、文本和图像条件的动物运动生成与动画建立了新基础，有望推动动物行为建模、生物力学研究和动画应用的发展。
...
TL;DR: SliceBridge是一种利用 rectified flow matching 修复T1加权MRI中连续切片损坏的框架，通过利用周围完整切片和相对切片位置信息来重建损坏的切片区间。方法通过区间相关初始噪声、共享流时间和同步采样来确保层间一致性。


<details>
  <summary>Details</summary>
Motivation: 结构MRI图像的连续切片有时会因采集、运动、硬件或重建问题而损坏，这种局部损坏会导致下游形态计量分析的偏差，而丢弃或重新采集本可使用的图像成本高昂。

Method: 提出SliceBridge框架，使用rectified flow matching对损坏切片区间进行修复，条件包括周围完整切片及其相对切片位置。通过区间相关初始噪声、共享流时间和同步采样确保层间一致性，然后将修复后的区间插回，其他切片保持不变。

Result: 与独立重建目标切片的匹配模型相比，SliceBridge将修复区间内切片间变化的误差降低了32.9%-41.3%，在所有区间长度下获得更高的SSIM。在受控损坏情况下，将下游分割模型产生的区域脑体积估计中位误差从1.95%降低到1.05%。

Conclusion: SliceBridge能有效修复MRI中的切片损坏问题，减少下游分析的误差，具有重要的临床应用价值。
...
TL;DR: 本文提出了RAFT-DVC，一个基于递归全配对场变换(RAFT)的分辨率感知DVC求解器家族，包含三种下采样因子(s=2,4,8)的版本。研究揭示了机器学习DVC模型内部分辨率对精度和操作范围的影响规律。


<details>
  <summary>Details</summary>
Motivation: 数字体积相关(DVC)可以从体积图像中获取三维全场位移测量，但机器学习DVC模型的内部分辨率如何影响精度和操作范围尚不清楚。现有方法缺乏对分辨率与精度关系的系统理解。

Method: 提出RAFT-DVC方法，采用匹配设计，包含三种编码器下采样因子s=2、4和8的求解器。引入非立方脉冲测试来验证采样器几何结构，纠正了三维RAFT相关采样中的坐标顺序不一致问题，并使用平铺推理实现大体积密集估计。

Result: 三种求解器将位移定位到约0.017特征网格体素，经验原始体积误差缩放约0.017s体素。在细纹理、小至中等位移条件下，RAFT-DVC达到与调优经典DVC相同数量级的误差；在粗纹理、大位移条件下具有竞争力或优势。训练数据仅为粒子标记的合成数据，但在弹性泡沫的微CT图像上展现出跨纹理迁移能力。

Conclusion: RAFT-DVC是一个快速、分辨率感知的密集DVC框架，具有明确的精度和操作范围特征。纠正采样器可提高原生输入精度和对未见体积维度的泛化能力。该方法为DVC提供了新的技术路径，特别是在大位移和粗纹理条件下具有优势。
...
TL;DR: 本文提出了一种将字典手语视频与连续手语视频进行匹配的方法，通过学习原型结构的手语嵌入空间，将字典视频映射到该空间实现匹配，支持零样本迁移到未见过的手语。


<details>
  <summary>Details</summary>
Motivation: 当前手语识别中存在字典视频与连续视频匹配困难的问题，需要一种仅依靠视觉相似性（手型相对于身体的运动）来匹配的方法。

Method: 从带标注的连续视频中学习原型结构的手语嵌入空间，每个可学习原型对应一个手语类别，然后将字典视频映射到该空间，通过嵌入相似度进行匹配。

Result: 在ASL-Citizen字典检索、ChaLearn OSLWL字典到连续匹配和BOBSL的CSLR2评估三个数据集上展示了对不同手语语言的强泛化能力，在没有任何特定基准监督的情况下，有效迁移到美式、英式和西班牙手语，在三个基准上均优于先前方法。

Conclusion: 该方法通过原型结构的嵌入空间实现了字典引导的手语匹配，能够zero-shot迁移到未见过的手语，且在不同手语语言和任务间具有良好的泛化性能。
...
TL;DR: 本文研究大型视觉语言模型(LVLMs)在推理时的幻觉缓解方法，揭示当前评估协议的局限性。研究发现幻觉减少往往伴随信息量下降，且在幻觉基准上的改进无法可靠地转移到更广泛的多模态能力上。


<details>
  <summary>Details</summary>
Motivation: 当前幻觉缓解方法在幻觉基准上报告了强增益，但尚不清楚较低的幻觉分数是反映多模态接地的改善还是更保守的生成。需要系统评估这些方法的实际效果。

Method: 在三个LVLMs和四个基准上评估六种幻觉缓解方法，包括幻觉聚焦评估和多样化能力基准MMStar。

Result: 分析揭示了两个一致模式：1)幻觉减少通常与信息量减少相关联；2)幻觉基准上的改进无法可靠转移到更广泛的多模态能力，在细粒度感知和推理任务上表现不一致或下降。

Conclusion: 当前评估协议可能通过奖励保守生成而高估了进展。幻觉缓解应作为忠实度-信息量-能力权衡来评估，而非仅通过幻觉分数。
...
### [25] [Untitled](https://arxiv.org/abs/2609.01899)
**
...
TL;DR: 本研究开发了一个全自动pipeline，从玉米果穗的3D点云中提取籽粒性状（行数、行粒数和籽粒大小），解决了传统人工测量无法跟上育种项目产量的问题。


<details>
  <summary>Details</summary>
Motivation: 玉米籽粒性状（如行数、行粒数、籽粒大小）受遗传因素影响较大，与产量基因组区域密切相关，但人工测量速度无法匹配育种项目产生的玉米体积。

Method: 开发了基于视频转点云平台的自动化pipeline：1)使用COLMAP和NeRF处理原始视频帧；2)通过密度分离隔离果穗；3)点云距离校准至物理单位；4)通过PCA进行Z轴对齐并圆柱展开为2D图像；5)增强对比度并使用Cellpose-SAM进行零微调实例分割；6)采用三重并列展开策略防止接缝处重复计数。

Result: 在168个测试集上，籽粒计数R²=0.921（MAPE=10.33%），95.2%的果穗行数误差在±2行以内（MAE=0.75行）。

Conclusion: 该pipeline成功实现了玉米籽粒性状的自动化提取，生成的多元性状数据集具有已知基因型信息，可用于表型与基因型关联分析。
...
TL;DR: kVNN是一种可学习的核化Volterra神经网络算子，通过将Volterra滤波的阶间结构与可学习的多项式核原子相结合，实现了紧凑的高阶滤波。该方法避免了显式的高阶张量参数化，可作为CNN兼容层实现，在视觉任务中取得了良好的精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 高阶交互分量对信号、图像和视频建模很重要，但显式高阶算子通常面临参数和计算成本快速增加的问题。研究者希望利用核化方法提高Volterra型神经算子的效率，同时为其高阶分量提供结构化解释。

Method: 提出核化Volterra神经算子(kVNN)，将Volterra滤波的阶间结构与可学习的多项式核原子相结合，允许不同交互阶由独立可学习的中心和系数表示。这种阶解耦表示避免了显式高阶张量参数化，可实现为CNN兼容层。

Result: 在代表性视觉任务上的实验表明，kVNN实现了良好的精度-效率权衡。

Conclusion: kVNN通过核化方法有效提高了Volterra型神经算子的效率，同时保持了较高的建模能力，为高阶交互建模提供了一种紧凑而有效的解决方案。
...
TL;DR: 提出一种结合邻居嵌入投影与基于排名的流形学习的框架，通过UMAP生成低维表示并利用Borda Count聚合策略融合多种排名列表，在多个公开数据集上验证了方法有效性，尤其提升了基线表现不佳场景下的检索精度。


<details>
  <summary>Details</summary>
Motivation: 深度学习虽推动了基于内容的图像检索（CBIR）发展，但在高维特征空间中有效排序相似图像仍具挑战，成对距离难以捕捉上下文关系，视觉特征与高层概念间的语义差距依然存在，且如何结合流形学习和基于排名的细化方法以利用其互补特性仍是难题。

Method: 使用UMAP生成替代的低维特征表示，将UMAP投影和基于排名的重排序方法获得的排名列表通过Borda Count聚合策略进行结合，在ResNet152、Swin Transformer和DINOv2模型提取的深度学习特征上进行了实验。

Result: 所提方法在多种场景下提升了检索效果，特别是在基线表示难以达到高精确度的情况下；聚合策略也常改善顶部排名的质量，在不同数据集和特征提取器上获得了竞争力的MAP和精确度值。

Conclusion: 通过Borda Count聚合策略结合UMAP投影和基于排名的重排序方法，在多个数据集和特征提取器上实现了竞争力的MAP和精确度值，特别是在基线表示难以达到高精确度的场景下表现优异，证明了通过排名聚合结合基于投影和基于排名的流形学习策略可以为图像检索任务提供互补的上下文信息。
...
TL;DR: 提出一种利用公开可用的全切片图像(WSI)基础模型对癌症诊断切片进行自动排序和筛选的流程，解决多切片患者中肿瘤切片识别的难题。


<details>
  <summary>Details</summary>
Motivation: 癌症诊断和分期过程中患者检查通常生成多个WSI，但识别包含肿瘤或诊断生物标志物的切片需要病理学家手动繁琐整理；现有数据集常假设每例患者只有1个切片，或使用所有切片但会稀释信号。

Method: 使用WSI基础模型进行零样本分类，根据预测结果对WSI进行排序排序评估方法 benchmark FM在切片分诊中的性能。

Result: 在多个数据集上验证，肿瘤切片在排名靠前的切片中被准确识别，最多43张切片的患者在top-2排名切片中都能识别出肿瘤切片。

Conclusion: WSI基础模型包含足够的形态学信号，可自动对切片进行分类和筛选，证明了基于零样本分类的WSI基础模型在临床切片分诊中的应用潜力。
...
TL;DR: LF-MultiDiffusion是一种无需训练的全景生成方法，通过将潜在聚合重新表述为正则化最小二乘问题，并在去噪循环内使用Krylov迭代求解器实现线性投影，比现有无需训练的方法实现更密集和自然的映射，减少图像生成器评估次数，显著提高推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的全景生成方法存在映射不够密集自然、生成不稳定、需要大量视角视图的问题，同时图像生成器评估次数过多导致推理效率低下。

Method: 提出LF-MultiDiffusion方法，将潜在聚合重新表述为正则化最小二乘问题，在去噪循环内使用Krylov迭代求解器实现目标图像空间和参考图像空间之间的线性投影。

Result: LF-MultiDiffusion在视觉质量、文本对齐和全景一致性方面优于最强无需训练基线方法，同时实现15.36倍加速。

Conclusion: 该方法通过创新的潜在聚合公式和高效求解器，在保持无需训练优势的同时，显著提升了全景生成的质量和效率。
...
TL;DR: 本文提出了一种基于因果推理的时序评估框架，用于追踪视觉输入、问题文本和生成前缀在自回归解码过程中的作用。通过结构因果模型和干预方法，推导出三个指标来表征不同信息源的生成模式，无需参考答案即可评估模型。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标主要关注最终答案质量，无法揭示不同信息源（视觉输入、问题文本、生成前缀）如何影响生成过程。需要一种因果方法来分析这些信息源在自回归解码中的动态作用。

Method: 构建结构因果模型(SCM)，使用干预和后门调整方法，推导出三个步骤索引的因果驱动指标：视觉因果驱动(VCD)、问题因果驱动(QCD)和前缀因果驱动(PCD)。

Result: 在Qwen3-VL-8B-Instruct上实验，跨MAVIS、LLaVA-Video-178K和MiraData数据集，发现模型从早期更强的问答和视觉引导逐渐转向依赖生成前缀。QCD和PCD相比观测PMI基线分别降低34.8%和47.1%的恢复误差。在VLMBias上，前缀-视觉不平衡分数达到0.767 AUROC和0.873 AUPRC。

Conclusion: 因果驱动轨迹为多模态生成提供了互补的源级诊断工具，能够区分先验驱动和视觉 grounded的生成过程。
...
TL;DR: InsightSeg引入情景记忆机制，将成功的修正经验转化为可重用的视觉锚定洞察，在后续图像中预防性应用，从而提升分割质量并减少细化步骤。


<details>
  <summary>Details</summary>
Motivation: 现有基于多智能体细化的语义分割系统是 stateless 的，批评智能体的反馈被丢弃，导致相同的 guideline-specific 错误在数据集中被反复发现和修正，造成不必要的计算开销。

Method: 1) 元分析器将每个合格的修正 episode 提炼为 directive 自然语言洞察；2) 使用 patch-level 视觉概念向量将洞察锚定到导致错误的局部图像区域；3) 在后续图像中，将这些概念与密集 patch embeddings 匹配以检索相关洞察；4) 在分割智能体首次预测前 conditioning 这些洞察，实现从"纠错"到"防错"的转变。

Result: 在 Waymo 和 Cityscapes 数据集上，InsightSeg 提升了首次通过和最终 guideline 一致性分割性能，同时需要更少的细化步骤。

Conclusion: 通过利用过去的修正经验，多智能体细化系统可以变得更加准确和高效。
...
TL;DR: InstEditSeg将医学分割任务重新定义为指令驱动的图像编辑问题，利用潜在扩散模型的生成先验，通过文本指令在原图上渲染颜色覆盖层来实现分割，而非直接输出二进制掩码。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低对比度、边界模糊和跨域分布差异下表现不佳，判别网络和大多数基于扩散的分割方法仅预测独立二进制掩码，未充分利用大规模预训练生成模型的视觉先验。

Method: 1)将分割任务重新定义为指令驱动的图像编辑任务，渲染颜色编码覆盖层；2)引入DINOv3作为辅助视觉编码器，构建DINO特征引导块和多尺度特征金字塔；3)通过通道拼接和零初始化卷积将金字塔融合到扩散U-Net；4)双分支无分类器引导策略，每步仅需两次前向传播。

Result: 在息肉和皮肤病变基准数据集上达到与强判别基线相当的准确性，表现出更好的跨域泛化能力、更完整的多病变分割、指令条件任务控制和采样灵活性。

Conclusion: 生成式分割范式具有显著优势，但存在颜色敏感性和不支持属性条件选择等局限性。
...
TL;DR: GeoStore是首个针对非对称、细粒度、开放集POI定位的基准数据集。GLAM方法通过全局到局部非对称匹配架构，解决了近距离查询图像与广阔参考图像间的匹配难题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉位置识别(VPR)假设对称的整图匹配，但POI定位需要非对称匹配：查询图像中目标充满画面，而参考图像中同一POI仅占小部分且偏置，存在显著的拍摄域差距。现有全局描述子方法在此任务上系统性受限，因为单个全局向量会稀释小目标。

Method: 提出GLAM (Global-to-Local Asymmetric Matching)方法，耦合检索锚定全局描述子与非对称局部路径：将每个参考图像保留为紧凑的池化区域标记集，通过可学习的软晚交互与单个查询探测匹配；推理时启用轻量级 mutual-nearest-neighbor 重排序。

Result: GLAM在Recall@1/5/10和mAP上超越强全局和两阶段基线，具有约5倍更小的重排序特征和约两个数量级更低的每对匹配成本。

Conclusion: GLAM方法在Recall@1/5/10和mAP指标上优于强全局基线和两阶段基线，重排序特征小约5倍，每对匹配成本比之前的局部重排序低约两个数量级。
...
TL;DR: 本文提出CADMP，一个用于检测大型视觉语言模型中目标幻觉的轻量级框架。该方法结合了相邻层跨模态注意力漂移分析与目标视觉掩码的预测敏感性，通过量化解码过程中连续跨模态注意力图的分布变化来捕捉视觉接地的突变，并测量掩码相关视觉区域后预测概率的变化。


<details>
  <summary>Details</summary>
Motivation: 目标幻觉是大型视觉语言模型可靠部署的主要障碍。现有的检测方法通常使用单层注意力来表征视觉接地，但对其在层间的演变探索不足。

Method: CADMP框架包含两个核心信号：1)注意力漂移：量化相邻层跨模态注意力图的分布变化，捕捉视觉接地的突变；2)概率变化：掩码最大漂移对应的视觉区域后，测量预测概率的变化。轻量级检测器整合这两个互补信号来识别幻觉预测。

Result: 在多个基准测试和代表性开源LVLMs上的实验表明，CADMP实现了具有竞争力的检测性能。消融研究证实了相邻层漂移建模和基于掩码的接地验证的互补贡献。

Conclusion: 通过结合注意力漂移建模和掩码接地验证，CADMP能够有效识别幻觉预测。该方法为LVLMs的可靠部署提供了有价值的幻觉检测工具。
...
TL;DR: 本文提出SelfLift框架，通过自恢复机制解决少步扩散模型中渐进式分辨率推理的分布不匹配问题，实现高效推理加速。


<details>
  <summary>Details</summary>
Motivation: 少步扩散模型推理延迟主要由空间计算成本主导，现有的渐进式分辨率推理方法直接提升中间潜变量，导致分布不匹配错误，在少步预算下难以恢复，留下可见伪影，限制了转换时机和效率。

Method: 提出两种方法：1) SelfLift-zero：训练无关的伪影感知一致性提升，利用直接潜变量提升与像素VAE重编码之间的分歧作为局部伪影风险信号和模型原生校正方向；2) SelfLift-rich：在学生访问状态上进行策略自恢复，从内部自教师转移密集高分辨率 guidance，同时与改变的渐进分辨率动态保持对齐。

Result: 在FLUX.2-Klein和Z-Image-Turbo上，SelfLift分别降低端到端延迟41.5%和44.1%。结合时间步蒸馏，分别实现相对于50步模型29.61x和19.21x的加速比，同时保持有竞争力的生成质量。

Conclusion: SelfLift建立了少步扩散模型更强的速度-质量前沿，为高效高保真图像生成提供了新范式。
...
TL;DR: 本文提出Test-Time Logit Prompting (TLP)，一种无需源数据的轻量级测试时适应框架，用于在缺失模态情况下进行视觉识别。该方法通过不确定性感知的logit提示优化和模态完整一致性正则化来解决缺失导致的预测偏移问题，在仅需数百个可调参数和几步测试时优化的情况下，实现了高达8%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实部署中经常遇到缺失模态输入，导致模型性能显著下降。现有方法依赖源训练数据进行模态补偿，但在隐私、存储或无障碍限制下（如临床应用和个性化AI服务）难以获取原始数据。因此提出了一个重要但尚未充分探索的问题：如何在不访问源训练数据的情况下，高效地在测试时适应VLMs进行缺失模态的视觉识别。

Method: 提出Test-Time Logit Prompting (TLP)框架，包含两个核心组件：1) 不确定性感知logit提示优化 - 自适应调整预测置信度；2) 模态完整一致性正则化 - 保持语义一致性。通过在测试时优化logit提示来应对缺失导致的预测偏移。

Result: 在多种视觉语言基准数据集上的广泛实验表明，TLP在缺失模态场景下持续提升识别性能，实现了高达8%的改进，同时仅需数百个可调参数和少量测试时优化步骤。

Conclusion: TLP是一种有效的源自由测试时适应方法，能够在不需要访问源训练数据的情况下，为缺失模态场景下的视觉识别任务提供轻量级、高效的解决方案。
...
TL;DR: LaST-SR是一种用于单图像超分辨率的深度学习方法，首次将拉普拉斯神经算子中的稳态-瞬态分解思想引入二维特征图处理。该方法通过复频域分解模块，同时利用全局全谱傅里叶分支建模长距离结构依赖，以及窗口条件化的局部复频域分支捕获局部非周期变化，从而更好地恢复不规则结构和精细细节。实验在5个基准数据集上取得了最佳的PSNR/SSIM性能。


<details>
  <summary>Details</summary>
Motivation: 现有的傅里叶算子在全局特征建模中受限于其周期性谱基，难以表示局部非周期变化，导致不规则结构和精细细节恢复效果不佳。拉普拉斯神经算子通过复频域扩展了傅里叶模式，能够将输出信号分解为稳态和瞬态响应来联合建模周期和非周期信息，但此前缺乏二维特征图的近似分解理论依据。

Method: 1) 首次为二维特征图推导了近似稳态-瞬态分解理论，提供复频域分解的分析基础；2) 提出复频域分解模块，包含全局全谱傅里叶分支（建模图像级依赖和长距离结构一致性）和窗口条件化局部复频域分支（建模局部内容依赖的非周期变化）；3) 设计稳态-瞬态协同聚合模块实现跨分支交互和联合特征融合。

Result: 在5个基准数据集上，LaST-SR在×2和×4超分辨率任务上取得了最佳的PSNR/SSIM性能。消融实验验证了所提架构和各关键建模机制的有效性。

Conclusion: 通过引入拉普拉斯神经算子的稳态-瞬态分解思想，LaST-SR成功解决了傅里叶算子难以建模局部非周期变化的难题，在超分辨率重建质量上优于现有最先进方法。
...
TL;DR: 本文提出基于异常迁移的零样本异常生成方法DPA，通过复用源产品的真实异常来为目标产品生成逼真的异常样本，避免了为目标产品收集异常样本的困难。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测受益于异常样本，但新产品通常只提供正常图像，难以收集异常样本。现有方法主要依赖纹理图像或文本描述作为异常源，生成的异常不够逼真。观察到相似异常可在不同产品间重现，提出异常转移策略。

Method: 首先通过异常类型过滤机制选择合适的源异常类型；然后提出DPA框架，通过错配数据对学习产品无关的异常嵌入，实现跨产品的可迁移异常概念学习；设计自适应掩码引导管道控制生成异常的位置和几何合理性；引入无训练异常标注模块生成像素级标注。

Result: 在MVTec-AD、VisA和专用异常迁移基准数据集上验证，实验表明该方法生成更逼真的异常，并在零样本和少样本设置下显著提升下游异常检测性能。

Conclusion: 提出的异常迁移框架和DPA方法能够有效解决目标产品异常样本难以获取的问题，通过复用源产品真实异常实现更逼真的零样本异常生成，为工业异常检测提供了新思路。
...
TL;DR: 本文提出了KSG-Net，一个用于海上3D船舶检测的Key-Sparse和全局上下文学习网络，旨在解决海上LiDAR感知中小型稀疏船舶特征表示不足和大型船舶全局结构建模受限的问题。


<details>
  <summary>Details</summary>
Motivation: 海上船舶检测面临三大挑战：大型船舶尺度变化大、小型船舶点云稀疏、以及严重海杂波干扰。现有基于2D特征或密集表示的方法难以平衡检测精度和计算效率，而为道路场景设计的稀疏3D检测器在海上场景中泛化性能差。

Method: 提出KSG-Net框架，包含两个核心模块：1) 关键稀疏多尺度聚合(KSMA)模块，通过选择信息丰富的关键体素并聚合跨尺度邻域特征来增强小型稀疏船舶的特征表示；2) 全局上下文聚合(GCA)模块，通过门控残差交互的场景级上下文建模捕获长程几何依赖，提升大型船舶的特征表示。

Result: 在Thames River船舶数据集和模拟数据集上的广泛实验表明，KSG-Net在多尺度船舶检测任务中始终优于现有方法，并在复杂海上环境中展现出强大的鲁棒性。

Conclusion: 通过联合增强局部判别特征和全局结构感知，KSG-Net有效解决了海上LiDAR感知中小型船舶特征弱和大型船舶全局建模不足的问题，为海上自动驾驶导航提供了更准确的3D船舶检测方案。
...
TL;DR: RILA是一个执行驱动的智能体，将浏览器渲染纳入闭环，通过运行时交互反馈迭代优化生成的代码，解决现有方法仅关注视觉指标而忽视交互功能验证的问题。


<details>
  <summary>Details</summary>
Motivation: 现有工作大多强调美学和布局相似性等视觉指标，而忽略了交互功能的验证。生成可交互网页不仅需要视觉逼真，更需要功能正确。

Method: 1. 动作交互验证(AIV)模块：在生成的网页上回放参考交互轨迹，收集基于执行感知的观察。2. 执行感知渲染评分(ERS)：联合测量交互正确性和视觉保真度以指导迭代优化。3. 执行验证的数据合成 pipeline：生成多样化的训练数据。

Result: 在IWR-Bench上，RILA一致提升交互和视觉保真度。Qwen3.5-9B通过训练pipeline从40.40%提升至57.52%，超越更大的模型Kimi-K2.6(55.61%)和GPT-5.5(55.74%)。

Conclusion: RILA通过将浏览器渲染纳入迭代优化循环，有效解决了网页生成中交互功能验证的关键问题。执行驱动的训练策略能显著提升紧凑模型的性能，为多模态大语言模型在前端开发中的应用提供了新范式。
...
TL;DR: 本文提出了一种证据引导的检测器-定位器-推理器系统，用于解决AIGC图像篡改检测问题。该系统通过级联证据流将图像级检测器、区域定位器和基于MLLM的推理器连接起来，生成结构化取证报告。系统在ACM Multimedia 2026 GenText-Forensics挑战赛中获得0.638分，排名第二。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC技术的快速发展，以文本为中心的图像操纵变得越来越容易获取，催生了新的取证挑战。现有方法仅关注真实性检测，缺乏空间定位和基于证据的解释能力，无法满足实际取证需求。

Method: 提出证据引导的检测器-定位器-推理器系统：（1）图像级检测器提供全局真实性先验；（2）专用定位器提取篡改区域作为空间定位证据；（3）基于MLLM的推理器生成基于专家取证证据的结构化取证报告。核心创新包括：迭代难度感知挖掘提升定位质量；报告-掩码一致性后处理对齐报告 grounding与预测掩码。

Result: 在官方隐藏测试集上，系统最终得分为0.638，排名挑战赛第二，验证了所提证据引导系统的有效性。

Conclusion: 证据引导的检测器-定位器-推理器系统能够有效解决AIGC图像取证问题，通过级联证据流实现了从检测到定位再到推理的完整取证流程，为AI生成图像的取证分析提供了新的解决思路。
...
TL;DR: 研究皮肤科大语言模型在资源受限环境中部署时的泛化问题，发现疾病分布偏移比皮肤色调差异对模型性能的影响更大。癌症专用模型在转移到不熟悉的临床条件时，准确率从0.62下降到0.21，而疾病内皮肤色调差距较小（0.10-0.18）。皮肤病学预训练模型保留了更强的可迁移结构，约10个标记样本即可恢复大部分性能。


<details>
  <summary>Details</summary>
Motivation: 皮肤科AI模型主要在浅肤色、癌症图像上训练，但越来越多地部署在患者与训练人群不同的资源受限环境中。患者在皮肤色调和疾病分布两个混杂轴上与训练数据存在差异，需要确定泛化不良主要是由于皮肤色调代表不足还是疾病分布偏移造成的。

Method: 评估了癌症训练基线模型（ResNet-50在HAM10000和ISIC 2019上微调）、两个皮肤病学基础模型（DermLIP和MONET）以及一个通用视觉模型（DINOv3）作为冻结特征提取器。在分层疾病匹配数据集（DDI）和疾病偏移色调多样数据集（SCIN）上进行评估，并进行无标签表示分析。

Result: 疾病分布偏移在评估设置中的影响大于皮肤色调。癌症基线模型在转移到不熟悉的临床条件时，平衡准确率从0.62下降到0.21，而疾病内皮肤色调差距较小（0.10-0.18）且不一致。癌症专用特征在聚类不熟悉条件时表现不佳（kNN纯度仅比随机高+0.06），而皮肤病学预训练特征保留了更强的可迁移结构（+0.23）。从皮肤病学基础模型开始，每个临床类别约10个标记样本即可恢复大部分可达性能。

Conclusion: 疾病分布偏移是导致皮肤科AI模型泛化不良的主要原因，而非皮肤色调表示不足。皮肤病学基础模型相比癌症专用模型具有更强的可迁移性，且通过轻量级适应可以快速恢复性能。这项研究支持在资源受限环境中优先考虑疾病多样性的数据收集，并为皮肤科AI的泛化审计提供了评估协议和代码。
...
TL;DR: 本文提出Φ-Omni框架，利用部分信息分解(PID)理论解决多模态自监督学习中的模态崩溃问题，通过信息瓶颈机制最大化不可约协同作用，在乳腺癌和肺癌数据预训练后，在5个外部数据集的8个任务上展现出色的少样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过对比对齐将异构模态隐式强制到统一潜在空间，导致模态崩溃，丢弃了独特的协同诊断信号(Φ)。作者认为最强的任务无关SSL训练信号来自于协同交互的蒸馏，而非仅仅对齐共享冗余。

Method: 提出Φ-Omni框架，基于部分信息分解(PID)理论，使用协同信息瓶颈(SIB)并由ΦID目标调节，明确抑制边缘冗余同时最大化不可约协同作用，从而蒸馏高阶跨模态交互。

Result: 在乳腺癌(n=1031)和肺癌(n=919)队列预训练后，Φ-Omni在5个独立外部数据集的8个任务上展现出优于监督学习和SSL基线的少样本性能。

Conclusion: 基于PID理论的协同信息解耦方法能有效解决多模态自监督学习中的模态崩溃问题，蒸馏跨模态协同信息可显著提升病理切片少样本学习的迁移能力。
...
TL;DR: 本文提出World-Coherent-Decoding (WCD)，一种用于World Action Models (WAMs)的自验证测试时规划框架。该方法将WAM rollout视为可证伪的未来-动作假设，通过内部生成信号在执行前对候选未来进行排序选择，而非仅依赖采样更多未来来实现扩展。


<details>
  <summary>Details</summary>
Motivation: WAM通过随机生成视觉未来然后解码动作来控制机器人，但实验观察表明结果强烈依赖于选择哪个未来。核心问题在于：如何在不更新主干模型的情况下，在执行前评估候选未来的可靠性。

Method: WCD在每个决策步骤中：1) 从冻结的WAM采样多个候选；2) 使用两个内部生成信号进行排序：基于光流的视频惊讶度用于视觉合理性，动作路径努力值用于动作生成稳定性；3) 执行后，用实际观察审计所选想象，产生想象-现实不匹配信号，用于训练轻量级在线预测器以指导未来候选选择。

Result: 在RoboTwin 2.0上，WCD在有限随机场景监督下将Hard success从55.80%提升至60.90%，在Horizon-3任务上获得+16.43的增益，并在真实Franka视觉偏移测试中显示定性鲁棒性。

Conclusion: 核心结论：WAM的测试时扩展不取决于采样更多未来，而取决于选择更可靠的未来。该方法将延迟的自我验证转化为执行前可靠性估计，且无需更新主干模型。
...
TL;DR: 本文提出了一种基于SAM2先验的两阶段点监督变化检测框架，通过引入SAM2生成对象感知候选掩膜，并结合双时相掩膜选择策略将通用分割响应转换为可靠的伪标签，再利用不确定性感知损失改进边界质量，最后通过师生自训练框架实现伪标签精细化和模型再优化的闭环过程。


<details>
  <summary>Details</summary>
Motivation: 点监督变化检测虽然能大幅降低标注成本，但点标注的空间覆盖有限，导致伪标签不完整且噪声较多，影响模型性能。

Method: 第一阶段：利用SAM2根据点标注生成对象感知候选掩膜，设计双时相掩膜选择策略将通用分割响应转换为可靠的变更伪标签，并使用轻量级CNN细化模块结合不确定性感知损失改进边界质量和局部结构一致性。第二阶段：构建师生自训练框架，教师模型通过指数移动平均更新，定期刷新伪标签，形成伪标签精细化与模型再优化交替的闭环优化过程。

Result: 在WHU-CD、LEVIR-CD和SYSU-CD三个基准数据集上的实验表明，该方法在大多数基准上优于现有弱监督方法，并与若干全监督方法具有竞争力。

Conclusion: 通过引入SAM2先验和两阶段自适应框架，有效解决了点监督变化检测中伪标签不完整和噪声问题，实现了高效的弱监督变化检测。
...
TL;DR: CC-4DGS是一个存储高效且可扩展的动态4D高斯溅射框架，通过计算变形场(CDF)和规范点云属性压缩(CCA)管道，将每场景存储从数百兆字节降至20-30MB，同时保持实时渲染性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态4D高斯溅射方法需要每场景数十到数百兆字节的存储空间，因为它们严重依赖大型多分辨率哈希表和高维高斯属性，这限制了其在实际应用中的可扩展性和部署便利性。

Method: 提出两个核心创新：1)计算变形场(CDF)用确定性密集哈希编码和紧凑神经解码器替代大型多分辨率哈希表，仅需1-3MB变形存储；2)规范点云属性压缩(CCA)管道通过条件自编码器、选择性量化和残差码本压缩高维球谐外观项，实现3-5倍点云 reduction。

Result: 在N3DV和Technicolor Light Field数据集上，CC-4DGS实现与Swift4D等最先进方法相当的重建精度，同时提供显著更好的存储效率(20-30MB)和有利的运行时内存权衡。

Conclusion: CC-4DGS在保持实时渲染性能的同时，将总存储降至20-30MB，为动态场景的高效存储和实时渲染提供了可行的解决方案。
...
TL;DR: 本文提出了一种将通用视觉语言模型(VLM)轻量化适配到多光谱和合成孔径雷达(SAR)观测的方法，无需训练新的基础模型。协议将每个观测渲染为五个光学视图和一个SAR视图，通过LoRA微调语言网络和视觉Transformer块，在BigEarthNet-v2上达到0.8275微F1。


<details>
  <summary>Details</summary>
Motivation: 当前预训练的视觉编码器主要针对三通道自然图像设计，难以直接处理多光谱测量或SAR观测。传统方法需要专用编码器和领域预训练，限制了更强通用检查点的复用。本文旨在解决这一问题。

Method: 1)将多光谱和SAR观测渲染为五个光学视图和一个SAR视图，在提示中命名；2)使用LoRA适配语言网络和选定的视觉Transformer块；3)结构化监督将预测类别与传感器证据耦合；4)构建偏好对，保留证据但省略真实标签，鼓励完整预测。

Result: 在BigEarthNet-v2衍生的六类土地覆盖基准上，适配的Qwen3-VL达到0.8275微F1。相同输入和适配协议提升了全部四种测试的VLM架构，并成功迁移到Sen1Floods11洪水验证和BigEarthNet.txt标注任务。图像删除和错配控制表明模型使用了提供的传感器观测。

Conclusion: VLMs可以通过渲染输入和紧凑的LoRA适配被重新用于多光谱和SAR任务，无需训练新的基础模型。这种方法具有通用性，适用于多种VLM架构和不同传感器类型。
...
TL;DR: 本文提出TAME框架，通过稀疏MoE层、帧-时序token和跨时序交互聚合模块来解决CLIP模型在视频检索中缺乏时序建模的问题，实现文本-视频检索性能提升。


<details>
  <summary>Details</summary>
Motivation: 将图像-文本模型(如CLIP)扩展到视频时面临根本性局限：缺乏时序建模能力。视频在外观和运动上呈现帧级异质性，将所有帧压缩为单一表示会掩盖时序结构和语义转换。

Method: 提出TAME框架包含三个核心组件：1)将稀疏MoE层集成到CLIP编码器中，在视觉分支上应用帧级一致路由，使专家根据帧级视觉模式专业化；2)引入帧-时序(FT) tokens聚合跨帧全局信息并反馈给每帧；3)设计跨时序交互聚合(CTIA)模块，通过分阶段时序过滤和融合优化帧级句子-视频相似度。

Result: 在MSR-VTT上比CLIP4Clip的R@1提升4.0，在DiDeMo、MSVD、LSMDC和ActivityNet上也获得一致提升。

Conclusion: TAME作为CLIP基框架有效联合建模了帧级结构和时序关系，在多个标准TVR基准测试上持续优于CLIP基线方法。
...
TL;DR: LeakageBench是一个用于评估文档图像中个人身份信息(PII)脱敏风险的挑战集,包含500张文档图像和11,954个GDPR对齐的PII标注。研究发现即使使用Code Interpreter提升GPT-5.5的定位F1从0.090到0.249,关键页面级泄漏率仍高达0.968,表明当前方法难以确保大多数页面的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有PII基准测试主要集中在文本层面,忽视了文档图像中的OCR错误、布局结构和视觉噪声对敏感信息去除的影响。文档级别的脱敏风险未被充分衡量——即使只遗漏一个标识符,页面仍然不安全。

Method: 构建包含500张文档图像(扫描件、截图、PDF渲染)的数据集,共11,954个GDPR对齐的PII标注,涵盖直接标识符、链接键和上下文重识别表面。评估通用OCR管道、商业和任务适配的OCR依赖检测器,以及OCR-free视觉语言模型,使用实体级F1、组级泄漏和文档级泄漏指标。

Result: Code Interpreter将GPT-5.5的定位F1从0.090提升至0.249,但关键页面级泄漏率仍高达0.968。大多数页面的泄露问题依然严重。

Conclusion: 更强的检测和工具辅助可以改善定位性能,但无法使大多数页面达到可安全发布的状态。LeakageBench提供了一个用于高召回率、空间锚定的文档图像PII脱敏诊断基准。
...
TL;DR: LISA 2026挑战赛旨在评估便携式超低场(0.064T)儿科MRI图像的分割方法。AURA是一种基于nnU-Net的非对称监督策略，独特地将两种不同来源的标注（高清场衍生标注和低场编辑标注）视为独立观测而非可互换的真值，通过可靠性门控机制动态整合低场标注信息。


<details>
  <summary>Details</summary>
Motivation: 便携式超低场MRI可扩大儿科神经影像的获取范围，但在0.64T场强下进行分割面临重大挑战：解剖边界模糊、小结构可见性低、高场参考图像存在局部错位。现有的高场标注与低场图像的真实解剖表达之间存在非等价性，传统的监督学习方法难以有效处理这种标注冲突。

Method: 提出AURA非对称监督框架，以nnU-Net为基础架构。核心创新包括：(1)将高场衍生(HF)标注作为训练锚点，(2)通过bounded reliability gate整合低场编辑(LF)标注，该门控机制综合考虑标签分歧、边界特征、预测不确定性、类别可靠性和训练阶段五个维度，动态评估LF标注的可信度并据此调整其对训练的贡献权重。

Result: 在16例开发集上的实验表明：HF监督基线模型的Dice系数为0.7984，AURA方法达到0.7950，集成模型达到0.7988。集成模型的HD95为1.8892，ASSD为0.7855。三种方法的Dice性能接近，展示了非对称监督策略在处理标注非等价性问题上的可行性。

Conclusion: AURA方法在LISA 2026挑战赛的初步评估中展示了处理超低场MRI分割中标注非等价问题的能力，为该领域提供了新的研究思路。未来需要在隐藏测试集和外部超低场队列上进一步验证方法的泛化性能。
...
TL;DR: 提出了一种基于模糊双维不确定性（FuDU）框架的流式主动学习方法，用于工业缺陷检测中的不确定样本挖掘，提高深度学习模型在实时工业缺陷检测中的可靠性。


<details>
  <summary>Details</summary>
Motivation: 确保深度学习模型在实时工业缺陷检测中的可靠性至关重要，需要从连续工业媒体流中挖掘不确定样本，以提高检测系统的可靠性。

Method: 设计基于原型的全局不确定性量化（PGUQ）模块评估图像级不确定性；集成双熵缺陷不确定性评估器（DeUE）量化框级不确定性；将不确定性建模为系统误差，提出模糊双维不确定性感知策略，利用模糊推理融合双维不确定性，实现专家知识驱动的自适应采样决策。

Result: 综合实验表明FuDU高效灵活，适用于核燃料棒缺陷检测等工业检测任务。

Conclusion: 所提方法能够有效提高工业缺陷检测系统的可靠性，通过模糊推理融合图像级和框级不确定性，实现专家知识驱动的自适应采样决策。
...
TL;DR: 论文揭示Grad-CAM旋转漂移的根源在于空间激活张量而非通道权重，提出无需训练的EquiGrad-CAM方法，通过多视图逆旋转平均显著提升显著性图的旋转等变性。


<details>
  <summary>Details</summary>
Motivation: 在病理图像和航拍图像等没有标准方向的领域，Grad-CAM等显著性图在输入旋转时会发生漂移，即使预测结果不变。这破坏了使用显著性作为决策证据的可靠性。

Method: 通过测量CAM算子每个阶段的等变性而非从网络输出推断，发现通道权重是最稳定的部分（在ResNet-50中完全稳定），因为GAP+线性头使类梯度场空间常数。真正移动的是空间激活张量，而分类器的池化丢弃了这种移动。提出EquiGrad-CAM：取T个旋转视图，将每个视图的显著性逆旋转到公共规范框架并平均。

Result: 在ImageNet-1K验证集上，EquiGrad-CAM相比单视图Grad-CAM将等变性提高+36.0%（ResNet-50）、+87.5%（VGG-16）和+247%（ViT-B/16）；消融实验表明对齐是驱动因素。无需重新训练即超越旋转增强训练，将零样本CLIP提升+145%，在PatchCamelyon和RESISC45上产生旋转一致的解释。PEUM副产品可对解释的可重现性进行排序。

Conclusion: 漂移是由分类器丢弃的自由度携带的，这使得移除漂移是忠实的而非破坏性的。EquiGrad-CAM是一个无需训练的包装器，通过多视图逆旋转平均实现了旋转等变性，显著提升了显著性解释的可靠性。
...
TL;DR: InfraPatch是一种针对红外视觉语言模型(IR-VLMs)的白盒、针对性数字灰度补丁攻击框架，在10个红外适配模型变体上实现了86%-100%的定向攻击成功率，证明小尺寸灰度补丁可以在受控数字威胁模型下向IR-VLM家族注入选定的目标语义。


<details>
  <summary>Details</summary>
Motivation: 红外视觉语言模型在低可见度条件下的多模态感知展现出前景，但对其针对对抗性攻击的鲁棒性研究不足。现有对抗补丁方法主要研究RGB模型或单一下游任务，未能表征局部扰动是否能在IR-VLMs中诱导预期的语义目标。

Method: 提出InfraPatch框架，包含：(1)在约5%局部区域预算内优化紧凑单通道补丁；(2)结合代理引导放置与任务自适应语义目标；(3)在图像分类、图像字幕和二元视觉问答三个任务上诱导目标行为。评估使用10个红外适配模型变体和300张由DiffV2IR生成的合成红外风格图像（基于30类COCO子集）。

Result: InfraPatch在10个变体上达到86.00%-100%的定向攻击成功率。代理位置搜索相较于优化随机放置，CLIP提升6.67个百分点，BLIP-2提升10.33个百分点；LLaVA-1.5在两种设置下均饱和在接近100%。补丁区域和目标消融实验进一步揭示了不同架构和任务格式间的显著脆弱性差异。

Conclusion: 小尺寸灰度补丁可在受控数字威胁模型下向IR-VLM家族注入选定的目标语义，这表明需要为红外多模态系统开发更强的鲁棒性评估方案。
...
TL;DR: SAUF-Net是一种用于半监督医学图像分割的结构-外观表征学习与不确定性反馈网络，通过解耦结构与外观表征来减少注释成本并提高分割准确性。


<details>
  <summary>Details</summary>
Motivation: 现有半监督学习方法主要利用预测级一致性，忽视特征表征可靠性；医学图像中目标相关结构线索与不稳定外观变化纠缠，导致不可靠的伪标签和错误累积。

Method: 提出SAUF-Net框架，包含：1)结构-外观分解模块(SADM)将瓶颈特征分离为结构和外观表征；2)解耦引导模块(DGM)将这些表征注入解码过程；3)辅助解码器产生分支特定预测用于可靠性估计和融合预测用于外观交换一致性；4)外观交换一致性分支鼓励结构表征在外观变化下保持稳定；5)可靠性图引导的双头判别器(有效性头和不确定性头)提供特征级不确定性反馈。

Result: 在ISIC-2016和Kvasir-SEG数据集上的实验表明，SAUF-Net优于现有先进半监督方法，特别是在低标签设置下。

Conclusion: 通过结构-外观解耦和不确定性反馈机制，SAUF-Net能有效利用未标记数据，提高半监督医学图像分割性能。
...
TL;DR: 本文提出一种两阶段框架，用于从静态手写图像中恢复动态书写过程。该方法首先恢复有序笔画实例，然后在笔画内重建连续运动轨迹。在中文手写数据集上的实验表明，该方法无需轨迹简化即可优于所有基线方法，并展现出跨语言泛化能力。


<details>
  <summary>Details</summary>
Motivation: 离线手写只保留最终的静态墨迹模式，丢失了笔顺、书写方向和笔尖运动等时间信息，导致轨迹恢复存在歧义。现有基于学习的方法通常直接预测完整字符轨迹，未能显式利用笔画级的手写组织结构。作者认为恢复书写过程应遵循实际的书写过程。

Method: 提出两阶段框架：第一阶段通过自回归有序笔画预测集成笔画提取和笔顺恢复；第二阶段利用方向相关结构线索支持笔画内轨迹生成。方法包含笔画实例恢复和笔画内连续运动重建两个核心模块。

Result: 在中文手写上的实验表明，有序预测比后处理笔顺排序更有效。即使不进行轨迹简化，完整点模型也取得了比所有基线方法更好的数值结果。轨迹采样密度对测量恢复性能有显著影响。此外，方法对未见过的新汉字类别具有泛化能力，并成功扩展到英文和泰米尔文手写。

Conclusion: 两阶段框架能有效恢复手写轨迹，笔画级组织结构的显式建模是关键。该方法在中文手写恢复任务上达到了最优性能，并展示了良好的跨语言泛化能力，为手写轨迹恢复研究提供了新的思路。
...
TL;DR: 论文提出了T2LSC-Bench基准测试，用于评估文本到图像模型中的目标文本语义泄漏问题。该基准包含50个种子主体和1,200个提示案例，涵盖语义关系、场景开放性、提示模式和语言等变化因素。通过OCR-VLM文本验证和结构化VLM语义判断的双分支协议，测量文本锚点准确性(TAA)、语义主体保留(SSP)、语义泄漏率(SLR)和条件语义泄漏率(cSLR)。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-文本基准主要评估可读性、拼写准确性和布局，但忽略了目标文本语义通过非文本视觉内容表达的问题。作者称之为"目标文本关联语义泄漏"，这在产品标签、标牌和界面设计等应用中尤为重要，因为目标文本应在指定区域内渲染，且不应改变预定主体身份或周围场景语义。

Method: 1) 构建T2LSC-Bench基准：50个种子主体×1,200个提示案例，跨越6个模型共7,160张评估图像；2) 因子化设计变化语义关系、场景开放性、提示模式和语言；3) 双分支协议：OCR-VLM文本验证 + 结构化VLM语义判断；4) 压力测试条件评估；5) 抗泄漏提示效果测试；6) 420张图像人工验证。

Result: 压力测试条件下：SLR从1.2%增至18.1%，cSLR从1.3%增至18.2%，而TAA仅从91.4%降至90.9%。抗泄漏提示将SLR从16.6%降至8.4%，且不降低渲染准确率。420张图像的人工验证显示自动标注与仲裁标注之间具有强一致性。

Conclusion: 准确的文本渲染不能保证目标文本语义在局部区域内的Containment。研究揭示了当前文本到图像模型在语义泄漏方面的显著问题，但可通过提示工程部分缓解。
...
TL;DR: MAOL是一种用于工业缺陷细粒度严重程度分级的形态学感知序数学习框架，通过融合形态学特征、自适应序数阈值和预测感知训练解决序数建模和训练-测试分布不一致问题，在IDA 2026挑战赛中获得第三名。


<details>
  <summary>Details</summary>
Motivation: 工业检测中细粒度缺陷严重程度分级至关重要，但面临三大挑战：(1)严重程度标签的序数性质；(2)对形态学线索的强依赖；(3)两阶段流程中清洁标注实例与噪声预测实例之间的训练-测试差异。

Method: 提出MAOL框架，采用四项关键技术：(1)将严重程度分级建模为实例级序数学习任务；(2)引入显式形态学特征增强表示学习；(3)设计类条件自适应序数阈值建模缺陷特异性分级边界；(4)通过定位扰动的预测感知训练提高对不完善预测实例的鲁棒性。

Result: 在清洁ROI和预测实例两种设置下的广泛实验表明，MAOL始终优于基于规则的方法、标称分类模型和现有序数基线方法，尤其是在预测实例设置中表现突出。

Conclusion: MAOL框架通过融合形态学特征、自适应序数阈值和预测感知训练，有效解决了工业缺陷细粒度分级中的序数建模、形态学依赖和训练-测试分布不一致等核心挑战，在IDA 2026挑战赛中获得第三名，验证了方法的有效性。
...
TL;DR: 本文提出RouteGraph-Mona方法，用于解决矿物图像分类中类内差异大和类间相似性高的问题。该方法在Mona适配器基础上引入样本自适应路由机制，通过类级路由锚点和混淆加权margin正则化路由特征，提升了模型对视觉相似矿物类别的区分能力。


<details>
  <summary>Details</summary>
Motivation: 矿物图像分类对地质勘探和资源开发至关重要，但面临两大挑战：1) 类内外观差异大；2) 类间视觉相似性高。现有的Mona方法采用静态多尺度聚合，无法适应样本特定的尺度偏好，且在视觉相似矿物类别间易产生混淆。

Method: 1. 将Mona的静态多尺度聚合替换为样本自适应路由机制；2. 利用分支选择行为定义紧凑路由空间，捕捉每幅图像的尺度偏好；3. 引入类级路由锚点鼓励类一致性路由模式；4. 添加混淆加权margin促进视觉相似类别在路由空间中的分离。在三个公共矿物图像数据集和两种视觉骨干网络上进行实验。

Result: RouteGraph-Mona在平均准确率上始终优于Mona，并与代表性微调方法和矿物图像分类基线方法保持竞争力。

Conclusion: 通过引入样本自适应路由和路由空间正则化机制，RouteGraph-Mona有效解决了静态多尺度聚合的局限性，提升了矿物图像分类性能，特别是对视觉相似矿物的区分能力。
...
TL;DR: 本文提出了一种基于主体特定空间-角向高斯场的自监督联合k-q扩散MRI重建方法，用于从欠采样k空间数据中重建缺失扩散方向的扩散加权图像。


<details>
  <summary>Details</summary>
Motivation: 现有联合k-q方法未能充分挖掘不同扩散方向图像之间的解剖结构共享性与方向依赖性信号变化的互补性，导致空间误差被误解释为角向变化并传播到未观测方向。

Method: 提出使用共享的3D高斯基元提供局部空间支撑，每个基元携带连续q条件化张量残差响应。信号由多个重叠基元响应合成，耦合邻近空间区域和扩散方向。该场从欠采样k空间测量中逐步优化，无需完全采样目标或保留方向监督。

Result: 在HCP三个扩散壳层和多种加速因子设置下的实验表明，该方法在缺失方向DWI重建、张量衍生指标和主扩散方向估计方面均取得一致改善。

Conclusion: 所提方法能够有效耦合空间和角向信息，在无监督情况下实现高质量扩散MRI重建，为加速扩散成像采集提供了新思路。
...
TL;DR: 本文提出PART，一种基于Transformer的纯雷达3D目标检测器，通过稀疏查询机制预测目标存在置信度、表面点和2D速度，可在类别无关设置下检测罕见移动目标。


<details>
  <summary>Details</summary>
Motivation: 解决封闭集训练导致的罕见目标漏检问题，利用雷达的类别无关多普勒运动线索和全天候感知能力，同时克服雷达稀疏噪声和难以恢复完整几何框的缺陷。

Method: 提出三个关键技术：(1)DAQI通过位置-速度聚类生成输入依赖的查询提议；(2)PGCA引入径向多普pler一致性和RCS信息的交叉注意力；(3)不确定性感知监督随机掩码GT并分配软存在目标。

Result: 在nuScenes上实现CA-AP=0.8827，mASTE=0.3188m，mAVE=0.8084m/s；对罕见安全相关类别召回率达0.9203；在夜间、雨天、严重遮挡下仍有效。

Conclusion: PART以仅110万参数实现高效类别无关检测，验证了雷达在恶劣条件下对移动目标感知的可行性，部分"假阳性"实际为漏标移动目标。
...
TL;DR: VoRTeC是一种基于流模型(Wan2.1)的超低码率视频压缩框架，通过压缩潜表示、预测流轨迹位置和整合多尺度先验，实现单步解码和高感知质量重建，同时通过尾帧复用和先验缓存保持时间一致性。


<details>
  <summary>Details</summary>
Motivation: 传统神经视频压缩产生模糊伪影，而扩散生成式视频压缩面临解码延迟高和时间一致性差的问题，需要一种兼顾压缩效率、重建质量和解码速度的新方案。

Method: VoRTeC采用基础流模型作为核心，通过紧凑编码潜视频表示、预测压缩表示在流轨迹上的位置、集成多尺度先验实现生成式视频流先验的有效利用，在不访问流匹配网络参数或梯度的情况下实现单步解码，并通过尾帧复用和先验缓存维持帧组间一致性。

Result: 相比扩散式方法，VoRTeC降低58%码率消耗，解码速度提升3-197倍，720p下达13 FPS，480p下达32 FPS。

Conclusion: VoRTeC成功解决了超低码率视频压缩中质量、速度与效率的矛盾，为生成式视频压缩的实用化提供了可行路径。
...
TL;DR: 研究扩散模型生成的合成伪装数据能否提升目标检测器的跨域鲁棒性，结果表明生成式伪装增强可显著改善野外检测性能。


<details>
  <summary>Details</summary>
Motivation: 领域偏移会导致目标检测器性能下降，现有数据增强难以模拟真实域变化，扩散模型在跨域泛化方面的潜力未被探索。

Method: 使用Qwen Image Edit 2509、Flux.2 Dev及LoRA微调版Qwen生成合成伪装数据；以黑棒遮挡作为非生成式基线；采用GroundingDINO检测器评估。

Result: 叶状伪装mAP提升+20.1，网状伪装mAP提升+14.4，多光谱伪装通过LoRA微调提升+4.4 mAP。

Conclusion: 生成式伪装增强可有效提升目标检测器的跨域鲁棒性，其中Qwen Image Edit 2509和Flux.2 Dev在叶子和网状伪装上效果显著，LoRA微调有助于多光谱伪装任务。
...
TL;DR: YesTrack是一种新型的两阶段Referring多目标跟踪(RMOT)方法，将指代理解重新定义为判别任务，直接利用多模态大语言模型(MLLMs)进行是/否验证，无需显式文本生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法将MLLMs仅用作标题生成器，需要外部模块进行最终决策，这不仅增加了延迟，还严重低估了MLLMs的视觉-语言对齐能力。

Method: 提出两个轻量级时间一致性约束：时间置信度先验(TCP)和时间参考传播(TRP)，以增强基于MLLM验证的可靠性和效率。同时提出YesTrack-MOT用于通用多目标跟踪。

Result: 在Refer-KITTI和Refer-KITTI-V2数据集上显著优于现有最先进方法，即使使用最小的Qwen3-VL变体也能保持高效率。

Conclusion: 验证了判别范式的有效性，YesTrack通过直接利用MLLMs的视觉-语言对齐能力而非仅作为文本生成器，实现了更好的指代跟踪性能。
...
TL;DR: 本文提出了一种混合框架，结合ORB算法进行特征提取和SVM进行分类，用于脑癌MRI图像的早期诊断。该方法实现了约99.5%的数据降维，在Br35H数据集上达到97.5%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 脑癌早期诊断的准确性是患者生存和治疗效果的关键决定因素。MRI是神经结构可视化的金标准，但高维扫描的解释复杂，存在主观变异性和图像固有噪声。现有深度学习方法参数过多，计算成本高且需要大量训练数据。

Method: 提出混合框架：使用ORB（Oriented FAST and Rotated BRIEF）算法进行精确特征提取，结合SVM（支持向量机）进行分类。通过特征提取实现约99.5%的数据降维，减少非信息性背景数据影响，同时保留肿瘤识别的关键诊断模式。

Result: 在Br35H数据集上进行实验验证，框架达到97.5%的分类准确率。

Conclusion: 局部特征表示与优化分类的整合为医学图像分析提供了可靠且资源高效的替代方案，在无需大量计算开销的情况下保持高性能，为医学图像分析提供了结构化解决方案。
...
TL;DR: 研究多数据集训练和BEV-Forcing辅助目标对视觉语言动作模型(VLA)在自动驾驶中的影响，发现辅助任务在训练数据有限时有效，但随训练样本增加收益递减。


<details>
  <summary>Details</summary>
Motivation: 解决VLA在自动驾驶中通常仅在单个数据集上训练、缺乏零样本迁移能力的问题，同时探索增加训练数据多样性是否能提升性能。

Method: 提出BEV-Forcing辅助目标，将专用鸟瞰视图模型的地面物体布局信息通过共享BEV空间接口迁移到VLA主干，并进行多数据集训练实验。

Result: 在少量相机装备训练时，BEV-Forcing可同时改善分布内和分布外性能；但随训练装备数量增加，辅助任务的收益逐渐降低。

Conclusion: 当简单增加训练数据多样性时，文献中的新技术收益可能会减弱，因此需要结合数据规模来呈现研究结果。
...
TL;DR: 本文提出了一种面向交通标志检测的结构化先验引导扩散修复框架，通过语义、 appearance 和几何三个正交路径注入标志先验，并引入 CIELAB 色度和 Sobel 边缘两项物理一致性损失，在保持数字、几何形状和颜色物理一致性的同时，实现了高质量的标志修复与合成数据生成。


<details>
  <summary>Details</summary>
Motivation: 交通标志检测面临长尾数据分布问题，稀有标志样本极少但从法规角度同等重要。现有通用修复模型直接应用于标志区域时会产生数字扭曲、几何变形和颜色偏移等问题，根源在于条件信号对标志的物理组成表达过于抽象。

Method: 提出结构化先验引导的扩散修复框架，包含三条正交路径：(1) JSON格式文本提示注入语义先验；(2) 通过IP-Adapter注入测量主色渲染的前视图矢量模板作为外观先验；(3) 通过ControlNet注入仿射对齐的矢量模板作为几何先验。训练采用自监督重建策略，推理时加入两项物理一致性损失约束：CIELAB色度L1项保证颜色一致性，Sobel梯度项保证边缘结构一致性。使用Stable Diffusion 1.5骨干网络（约14亿参数）。

Result: 在TT100K-2021公开数据集上零样本评估，本方法在重建保真度、物理一致性和语义可控性所有指标上超越7个对比方法。OCR精确匹配率达到91.1%，对比12B参数的FLUX.1 Fill [dev]仅为44.2%，且推理时间仅为其1/14。消融实验验证了三条先验路径和两项损失各自均有贡献。下游检测任务中，合成数据将稀有类别的AP50提升1.23倍至7.40倍。

Conclusion: 通过结构化先验引导和物理一致性约束的扩散修复框架，能够生成物理上真实一致的交通标志图像，有效缓解长尾分布下的数据稀疏问题，显著提升稀有标志的检测性能。
...
TL;DR: GlyphAnchor是一种用于图像生成和编辑扩散Transformer模型的新型文本渲染增强方法，通过轻量级字形补丁条件增强骨干网络，并结合阶段性监督微调和文本感知后训练来提高文本保真度，同时引入InfoTextBench基准测试评估文本渲染效果。


<details>
  <summary>Details</summary>
Motivation: 现有文本渲染方法在处理长、复杂、密集排列的文本和稀有字符时仍不够准确和鲁棒，需要改进图像生成和编辑模型中的文本渲染质量。

Method: GlyphAnchor通过轻量级字形补丁条件增强骨干网络，其位置通过模型原生位置编码锚定到目标图像，采用阶段性监督微调和文本感知后训练进行训练。

Result: 在多个骨干网络和基准测试上的实验表明，GlyphAnchor在长、复杂、密集排列文本和稀有字符场景下能持续提高文本保真度，同时保持整体图像质量。

Conclusion: GlyphAnchor为文本到图像和图像编辑扩散Transformer模型提供了一种有效的文本渲染增强方案，在各种挑战性场景下均表现出色。
...
### [69] [Blending Concepts: Benchmarking Visual Metaphor Generation in Text-to-Image Models](https://arxiv.org/abs/2609.02502)
*Chuer Chen,Zichen Wang,Yi He,Zhengxi Yu,Nan Cao*

Main category: cs.CV

TL;DR: VMetaphor-Bench是首个用于评估文本到图像(T2I)模型视觉隐喻生成能力的基准测试，包含1500个真实世界创意图像中的视觉隐喻，分为三个级别和十个类别，每个样本配有两个不同特异性的提示词。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在忠实地渲染指定对象和属性方面取得了显著成功，但它们生成视觉隐喻（通过结合两个不同领域的元素来传达抽象想法的图像）的能力仍然 largely未被研究。

Method: 构建了包含1500个视觉隐喻的基准数据集，组织成三个级别和十个类别，每个样本配有两个不同特异性的提示词；开发了一个混合评估框架，结合基于多项选择问题的协议（包含9594个问题，涵盖四个隐喻保真度级别）和基于三个感知维度的评分协议。

Result: 对11个代表性T2I模型的广泛评估表明，即使是最强的专有模型也在隐喻表达的关键方面（构图结构和跨域映射）方面存在困难。

Conclusion: 视觉隐喻生成是未来T2I研究的一个重要前沿。
...
TL;DR: 本文提出LookStep，一种资源高效的视觉语言导航(VLN)端到端框架，结合语言中心未来状态建模和事件驱动滚动内存，解决现有方法数据需求高、计算开销大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLN方法存在两个关键问题：1) 采用下一步动作预测范式，仅监督专家动作，训练数据需求高；2) 依赖认知地图、累积历史帧或外部3D工具维护状态，导致高计算和内存开销。因此需要一种资源高效的VLN方法。

Method: 提出LookStep统一端到端框架，包含两个核心组件：1) 语言中心未来状态建模(Language Centric Future State Modeling)：使用语言标签为每个候选动作生成粗粒度导航进度和未来状态；2) 事件驱动滚动内存(Event Driven Rolling Memory)：自主决定是否将观察结果写入有边界的滚动内存，并赋予语义角色。

Result: 在VLN-CE任务上，LookStep在相同训练设置下优于现有方法，在R2R-CE Val-Unseen上达到49.7%的成功率，同时具有更好的内存效率和更低的数据使用量。

Conclusion: LookStep通过语言标签生成未来状态和智能内存管理，实现了资源高效的VLN，为具身智能导航提供了一种更实用的大规模训练方案。
...
### [71] [Fine-Grained Anomaly Perception in Wild UGC-Enhanced Images: A Comprehensive Dataset and Difference-Fusion Framework](https://arxiv.org/abs/2609.02529)
*Yan Zhong,Gefei Chen,Qiufang Ma,Zhen Wang,Zhiwei Fan,Lei Shi,Tingting Jiang*

Main category: cs.CV

TL;DR: 本文提出了一个新的任务：UGC图像增强质量异常感知(UEAP)，并构建了首个基准数据集UEAP-4k，同时提出了DFAP-UGC方法和LADTP训练策略，用于检测UGC增强图像中的局部异常。


<details>
  <summary>Details</summary>
Motivation: 现有IQA方法主要评估整体图像质量，无法有效检测UGC增强算法在面部、文字和纹理等区域引入的局部视觉异常，这些异常直接损害感知真实性和用户信任。

Method: 提出DFAP-UGC方法，利用显式问题-参考差异融合、密集空间查询、区域验证和质量感知排序实现鲁棒异常识别；提出LADTP训练策略处理子任务耦合，实现端到端学习。

Result: 在UEAP-4k数据集上，DFAP-UGC方法显著优于基于经典方法改编的基线，验证了数据集价值和方法的优越性。

Conclusion: UEAP-4k数据集和DFAP-UGC方法为UGC增强图像的异常感知提供了有效解决方案，LADTP策略实现了子任务的有效端到端学习。
...
TL;DR: TempoGround是一个用于流式输入视觉定位的VLM原生框架，通过课程预测机制和状态感知的跨帧对应来检测对象对应关系并显式建模对象存在状态，从而在流式输入下实现准确一致的视觉定位。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理流式输入时存在身份漂移、跨帧不一致和部分遮挡下定位脆弱等问题，需要一个能够处理连续视觉输入的视觉定位框架。

Method: 提出课程预测机制引导的状态感知跨帧对应方法，解决2D实例关联、预测对象是新进入、继续存在还是离开视野，解码2D边界框并提升到相机帧3D边界框。引入流式定位强化学习(SGR)，通过可验证的定位、身份和一致性奖励进行优化。采用三阶段训练策略在大规模数据上训练。

Result: 在多个挑战性基准上评估，F1_2D@0.5和F1_2D@0.95平均提升4.4和0.5，F1_3D@0.25和AP_3D分别提升6.2和7.5。

Conclusion: TempoGround为流式输入下的视觉定位提供了实用基础。
...
TL;DR: 本文提出信息密度(Informatization Density)概念来解释目标检测中的类别偏差问题。传统方法仅通过实例数量判断数据分布是否为长尾分布，但即使实例数量平衡，模型仍存在类别偏差。研究发现类别信息密度与准确率呈显著负相关，信息密度不平衡可能是类别偏差的潜在来源。基于此改进三种先进的目标检测损失函数，在Pascal VOC、COCO-LT和LVIS数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为目标检测中的类别偏差是由于数据集中实例数量不平衡造成的（长尾分布），因此大量研究聚焦于解决实例数量不平衡问题。然而，即使在实例数量相对平衡的数据集中，模型仍然表现出类别偏差，这说明实例数量不能完全解释类别偏差现象，需要寻找其他潜在原因。

Method: 1. 提出信息密度概念并给出测量方法；2. 观察类别信息密度与准确率之间的负相关性；3. 研究训练过程如何影响这种关系；4. 基于信息密度概念改进三种先进的目标检测损失函数（简单改进）。

Result: 1. 发现信息密度与模型准确率呈显著负相关；2. 信息密度不平衡可能是类别偏差的潜在来源；3. 在Pascal VOC、COCO-LT和LVIS三个数据集上，基于信息密度改进的损失函数显著降低了模型偏差，同时有效提升了整体性能。

Conclusion: 信息密度为理解目标检测模型中的广义偏差现象提供了新视角，为设计更公平的损失函数和训练策略提供了新工具。信息密度可以作为实例数量之外衡量数据分布平衡性的重要指标。
...
### [74] [RVSD: Retrieval Vision Sparse Decoding for Mitigating Visual Hallucinations in Large Vision-Language Models](https://arxiv.org/abs/2609.02731)
*Canjie Liu,Jiawen Kang,Jinbo Wen,Zishao Zhong*

Main category: cs.CV

TL;DR: RVSD是一种无需训练、即插即拔的解码框架，首次在单次解码过程中统一了token稀疏化和语义空间视觉检索，有效缓解视觉语言模型中的视觉幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉语言任务中取得了显著成功，但仍容易产生视觉幻觉（VHs），这破坏了其在实际应用中的可靠性。现有解决方案通常需要精心设计的数据集、额外训练或多轮解码，导致相当大的计算开销。

Method: 提出RVSD框架，包含两个核心组件：1）语义导向的token选择策略，选择性稀疏化冗余token同时保留关键视觉信息；2）语义空间视觉检索（SSVR）机制，将视觉补偿重新定义为跨模态检索过程。RVSD在单次解码过程中同时完成token稀疏化和视觉检索，无需额外训练或多轮解码。

Result: 广泛的实验表明，RVSD在缓解视觉幻觉方面达到了最先进的性能，同时在长上下文生成设置下保持了强大的抑制能力。

Conclusion: RVSD是一个无需训练、即插即用的解码框架，首次在单次解码过程中统一了token稀疏化和语义空间视觉检索。该方法在缓解视觉幻觉方面达到了最先进的性能，同时在长上下文生成设置下保持了强大的抑制能力。代码已开源。
...
TL;DR: 本文提出语义原型引导的超分辨率方法(ProSR)，将SAR图像超分辨率重构为量化潜在空间中的语义引导离散标记预测任务，通过映射信号特征到离散散射原语来保留SAR的脉冲特性，并利用自监督学习提取语义先验，结合语义原型图引导注意力机制实现高质量SAR图像重建。


<details>
  <summary>Details</summary>
Motivation: 高分辨率SAR图像对自动目标识别等精密分析至关重要，但获取成本高。现有的基于平滑近似的扩散框架难以保持相干散射统计特性，导致结构失真与真实SAR物理特性不一致。

Method: 提出ProSR方法，包括：1)将SAR ISR重构为量化潜在空间中的语义引导离散标记预测任务；2)使用自监督学习骨干网络提取无标签语义先验；3)语义对齐细节编码将高频信号解耦为离散散射原语；4)语义原型图生成器构建原型图，原型图引导注意力机制在同类别的信息流中路由并减轻类间干扰。

Result: 在Umbra开放数据集上构建了0.25米分辨率的大规模基准数据集，实验表明ProSR在保持散射特性的同时实现了优越的视觉质量。

Conclusion: ProSR方法能够保留SAR图像的基本散射特征，满足实际SAR应用需求，为SAR图像超分辨率提供了一种有效的新范式。
...
TL;DR: 本文提出置信度感知的在轨蒸馏(CA-OPD)方法,用于解决自回归视觉语言模型在异构感知任务中的复合错误问题。该方法通过教师置信度选择性纠正不可靠的学生转换,结合严格到放松的调度机制逐步将 rollout 控制权转移给学生,并根据干预决策进行对齐的知识传递。


<details>
  <summary>Details</summary>
Motivation: 现有在轨蒸馏(OPD)存在训练-推理不匹配问题,但学生预测尤其是训练早期不可靠,会破坏轨迹并降低教师监督质量。现有的交错蒸馏方法依赖刚性排序指标而非精确的教师置信度,且忽视干预决策如何指导token级监督。

Method: CA-OPD 利用教师置信度选择性纠正不可靠的学生转换,通过严格到放松的调度机制逐步将 rollout 控制权转移给学生。核心创新包括:(1)置信度感知的可靠 rollout 构建;(2)纠正位置接受教师预测的交叉熵监督;(3)保留位置受益于教师的完整预测分布。

Result: 在多教师设置下针对 GUI  grounding 和 OCR 任务评估,CA-OPD 在六个目标基准上显著改进 Qwen3.5-0.8B 基线:ScreenSpot-Pro 提升 9.50 分,OCRBench-v2 English 提升 6.72 分。控制研究表明收益取决于干预位置、渐进式 rollout 控制和干预对齐监督,而非干预频率。

Conclusion: CA-OPD 通过置信度感知的自适应监督机制有效解决了在轨蒸馏中学生预测不可靠的问题,为视觉语言模型的知识蒸馏提供了新思路,且干预位置和渐进控制比单纯增加干预频率更重要。
...
TL;DR: NeuroFusion是一个基于医疗Mistral-7B的脑MRI报告辅助生成系统，通过在冻结的分割特征上使用判别性字段分类器头来调节解码器，从而揭示模型中潜在但未表达的诊断信号，避免直接覆盖模型的输出。


<details>
  <summary>Details</summary>
Motivation: 问题：现有的多链思维链(CoT)脑MRI报告生成器虽然能生成报告，但诊断能力很差，将大多数脑膜瘤和转移瘤误诊为胶质瘤（诊断召回率分别为0.44和0.07）。然而，模型的分割特征中实际包含了区分这些肿瘤的潜在信息（线性探针可达0.82 macro-F1），只是未被有效利用。动机：不是直接覆盖模型的输出，而是利用这些潜在信号来辅助报告生成。

Method: NeuroFusion采用以下方法：1）使用判别性字段分类器头对每个病变的特征进行分类；2）将分类器的输出作为条件，调节一个快速单次通过的"先草稿后审查"解码器；3）使用语法约束解码确保92.3%的记录符合模式；4）使用蕴含检查确保句子一致性（仅7.5%矛盾 vs 基线36.8%）。

Result: 1）诊断性能恢复：脑膜瘤0.92，转移瘤0.75；2）在三个测试队列的9项散文内容比较中赢得8项（RaTEScore、RadGraph-F1、GREEN；Holm校正配对BCa）；3）延迟降低5-6倍（约80秒 vs 457秒/例）；4）对照实验显示学习型诊断针覆盖解码器会导致分布外转移瘤召回率降至0.03；5）语法约束解码保持92.3%模式有效性；6）盲目九例试点中，两名委员会认证神经科医生独立评定NeuroFusion在所有肿瘤类型中最高评分，是唯一零关键错误的系统，在9例中获得8例最高评分签名。

Conclusion: NeuroFusion成功展示了如何利用冻结模型特征中潜在的诊断信号来辅助医学报告生成，而非简单覆盖模型输出。这种"辅助而非覆盖"的设计理念是关键——当使用学习型诊断针覆盖解码器时，分布外性能崩溃。系统不仅提高了诊断准确性和一致性，还显著降低了延迟，具有重要的临床应用价值。
...
TL;DR: 本文提出UAR-Net，一种针对异构恶劣天气退化的图像恢复框架，通过门控Transformer、多尺度跳跃连接和不确定性感知模块实现先进的天气图像恢复性能。


<details>
  <summary>Details</summary>
Motivation: 现有天气特定恢复模型依赖天气无关的全局聚合、简单的跨尺度融合和确定性目标，难以处理一体化恶劣天气设置中的异构退化问题。

Method: 提出UAR-Net框架，包含三个核心组件：(1)门控双尺度Transformer块(GDTB)用于联合建模选择性全局交互和多尺度局部结构；(2)渐进式平衡多尺度跳跃连接(BMSC)用于平衡多尺度特征融合；(3)不确定性感知细化头(URH)用于去除伪影、增强细节和预测不确定性。采用亮度感知能量损失(BAE-Loss)进行监督，以鼓励准确重建和良好校准的不确定性。

Result: 在多个恶劣天气基准数据集上取得了最先进的性能。

Conclusion: 实验表明该方法在各类恶劣天气图像恢复任务中达到了SOTA水平，代码将开源。
...
TL;DR: 本文提出了一种基于深度学习的月球表面相对高度估计方法，通过对预训练的Depth Anything V2模型进行微调，利用公开的立体摄影测量生成的DEM数据进行领域适配，显著提升了模型在月球表面的相对深度估计性能。


<details>
  <summary>Details</summary>
Motivation: 传统月球DEM生成方法（如阴影恢复形状和立体摄影测量）虽已得到验证，但深度学习在单目深度估计方面取得进展。现有方法仅依赖2D图像数据进行危险检测，缺乏高度信息，且直接将通用域模型应用于月球表面会导致性能下降，因此需要针对月球表面进行领域适配。

Method: 采用零样本相对深度估计模型Depth Anything V2 (DAV2)作为基础，利用公开的立体摄影测量生成的月球表面DEM数据进行微调训练，使模型学习月球表面的地形特征。

Result: 微调后的模型相比零样本模型在月球表面相对深度估计任务上实现了显著的性能提升，有效将DAV2转变为可靠的月球表面相对深度估计器。

Conclusion: 通过使用SPG衍生的DEM数据进行领域适配微调，可以显著提升深度学习模型在月球表面高度估计任务上的表现，为月球着陆任务的危险地形检测提供了更可靠的相对高度信息。
...
TL;DR: WiFlow是一个基于WiFi信道状态信息(CSI)的光流估计器，旨在替代摄像头进行运动估计，解决隐私和光照条件限制问题。


<details>
  <summary>Details</summary>
Motivation: 摄像头进行光流估计存在隐私担忧，且受光照条件影响大。WiFi CSI作为替代方案可以解决这些问题。

Method: 提出WiFlow系统，包含CSI预处理器评估和三种不同精度与复杂度权衡的模型架构。同时创建了首个CSI光流估计数据集。

Result: 实验为该任务的关键设计要素提供了深入见解。

Conclusion: CSI可作为摄像头进行光流估计的可行替代方案，具有隐私保护和光照无关的优势。
...
TL;DR: 本文提出VIPS基准和CoS-V2X框架，用于评估和实现车路协同自动驾驶。VIPS基于伪仿真技术扩展车辆和基础设施观测，实现可扩展且真实的鲁棒性和误差传播评估；CoS-V2X使用稀疏表示建模车路交互，实现高效通信和鲁棒决策。


<details>
  <summary>Details</summary>
Motivation: 城市环境中端到端自动驾驶面临部分可观测性和复杂多智能体交互的挑战；交叉口的严重遮挡和密集交通限制了单智能体系统的感知能力；现有评估协议存在根本性权衡：开环评估无法捕捉误差累积和偏差恢复，闭环评估成本高、难以扩展且存在领域差距。

Method: 提出VIPS基准：基于伪仿真技术扩展车辆和基础设施观测，实现可扩展且真实的鲁棒性和误差传播评估，无需完整仿真；提出CoS-V2X协作规划框架：使用稀疏表示建模车路交互，采用紧凑特征实现高效通信，在异构观测下实现鲁棒决策。

Result: 实现了可扩展且真实的协同自动驾驶评估框架；提出的协作规划框架在异构观测条件下展现出鲁棒决策能力。

Conclusion: VIPS基准和CoS-V2X框架为车路协同自动驾驶提供了新的评估和实现方案，能够有效解决现有评估协议的局限性，并实现高效的 车路通信与鲁棒决策。
...
TL;DR: 本文提出PATT（特权外观迁移跟踪），通过教师-学生训练框架解决视觉跟踪中目标模板的定位确定性与外观新鲜度之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 视觉跟踪中的目标模板在推理时面临两难：初始真值模板精确但会过时，近期模板反映当前外观但来自不确定的预测。作者通过不可部署的神谕模型证明这一瓶颈可将AUC提升15.2个百分点，揭示了训练阶段的独有优势。

Method: PATT采用教师-学生框架：教师观察过去、当前和未来帧的精确目标裁剪，学生仅接收过去帧模板并学习预测教师的搜索表示。为避免传递不可靠的教师信号，PATT根据教师的相对定位优势和绝对定位精度进行加权迁移。训练后移除教师、潜在预测器、可靠性权重和特权裁剪，仅保留学生进行标准推理。

Result: 在两个模型尺度的七个基准测试中，PATT在长期和短期跟踪协议下均取得一致的性能提升。

Conclusion: 通过特权外观迁移方法，成功将训练阶段的精确目标裁剪信息迁移到可部署的跟踪器，显著提升了跟踪性能，验证了训练-推理差距可以利用的可能性。
...
TL;DR: UnCapsTSR是一种用于无线胶囊内镜(WCE)图像超分辨率的无监督Transformer-based GAN框架，能够在无需真实LR-HR配对的情况下提升WCE图像的空间分辨率，并引入BTV损失确保空间连续性。


<details>
  <summary>Details</summary>
Motivation: WCE在通过患者消化道时捕获和传输视频，用于检查消化道异常。然而，由于胶囊尺寸和无线传输的限制，WCE图像分辨率较低。现有方法需要成对的LR-HR数据或显式降质估计，不适用于实际临床场景。

Method: 提出无监督Transformer-based GAN框架UnCapsTSR，采用Bilateral Total Variation (BTV)损失确保空间连续性，无需显式降质估计和真实LR-HR对。使用Kvasir Capsule数据集训练，在KID和GIANA数据集上验证。引入无参考指标EndoQM进行评估。

Result: 在NIQE、BRISQUE、PIQE和EndoQM指标上均优于现有无监督超分辨率方法。EndoQM指标显示从LR到SR有40-80%的改善。

Conclusion: UnCapsTSR能有效提升WCE图像质量，无需成对训练数据，具有良好的泛化能力。EndoQM作为新的领域特定无参考评估指标，适用于WCE数据的质量评估。
...
TL;DR: SR-Edit是一种通过迭代自我细化实现的图像编辑框架，旨在解决生成模型图像编辑中精确修改目标区域同时严格保留其他区域的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在区域估计不准确和启发式校正策略扭曲原生推理过程的问题，导致本应保证保真度的方法反而成为新的伪影来源。

Method: SR-Edit在每个迭代中：(i)通过轻量级后处理从模型自身预测中提取渐进精确和自一致的区域分离；(ii)通过与原始采样动态保持一致的校正更新来强制非编辑区域的保留。

Result: 实验表明，SR-Edit在保留效果和整体图像质量方面均优于现有编辑技术。

Conclusion: 通过迭代自细化机制，SR-Edit能够有效解决图像编辑中的区域保真问题，为高精度图像编辑提供了新的研究方向。
...
TL;DR: 在leave-performer-out（LPO）评估设置下，研究如何通过11个正交误差模式的模型集成提升12类骨架动作情感分类性能，从25.73%提升至36.80% Macro-F1，并提出基于部分掩码和反事实编辑的解释方法，验证模型决策依赖于与Laban Movement Analysis（LMA）相关的身体区域证据。


<details>
  <summary>Details</summary>
Motivation: 解决LPO评估设置下的情感分类难题，该设置下机会率仅为8.3%，现有基线模型性能较低，需要探索如何提升分类精度并理解模型决策依据。

Method: 采用11个模型等权重logit均值集成，结合部分掩码和反事实编辑进行解释性分析，使用Spearman相关验证区域显著性LMA属性对齐度。

Result: 集成模型达到36.80 +/- 4.00% Macro-F1，较基线提升11.07个百分点（相对提升43%）；区域显著性-LMA的Spearman rho = +0.500，而与经典运动学仅rho = +0.033；时序显著性呈弥散分布。

Conclusion: 可靠的性能提升来自模型集成而非新架构；模型决策依赖运动驱动的身体区域证据，该证据与LMA属性的对齐度远高于经典运动学；解释套件可事后审计无需重训练。
...
TL;DR: 该研究比较了四种商业多模态大语言模型（Claude、 Gemini、GPT、Grok）与2513名人类参与者对面部吸引力的评分差异。研究发现MLLMs系统性给出更高评分且评分范围更窄，但能准确追踪人类对吸引力的排序。


<details>
  <summary>Details</summary>
Motivation: 随着MLLMs在美容评估中的应用日益普及，研究旨在探究这些AI模型是否能准确反映人类对吸引力的判断。

Method: 采用预注册探索性研究设计，让2513名人类参与者对面部吸引力进行评分，并与四种广泛使用的商业AI模型进行比较。

Result: MLLMs系统性地给面孔更高的评分且评分范围更窄，无法在绝对值上复制人类评分，但与人类吸引力判断表现出强相关性，能准确追踪面孔的排序。只有面部年龄是人类和MLLMs共同的面部吸引力预测因素。AI模型之间高度一致，但Grok一致性最低且与人类agreement最弱。

Conclusion: 虽然当前商用MLLMs可能能够近似人类吸引力的排序，但它们系统性高估了人脸的美貌程度。
...
TL;DR: 本研究提出使用计算机视觉进行非接触式质量测量，以咖啡研磨为案例研究，构建了名为Doppio的视频数据集，并评估了多种深度学习模型用于估计下落颗粒的累积重量。


<details>
  <summary>Details</summary>
Motivation: 传统称重设备适用于静态测量，但许多工业应用需要非接触式传感，现有解决方案成本高、针对性强且技术复杂，需要一种更实用、更通用的替代方案。

Method: 构建了Doppio视频数据集，包含下落咖啡粉末的视频及精确的每帧真实重量测量；评估了从纯空间前馈网络到循环时空模型的多种深度学习方法。

Result: 深度学习计算机视觉模型能够准确估计下落颗粒的累积重量，在预测精度和计算成本之间取得了良好的平衡。

Conclusion: 基于视觉的非接触式测量解决方案具有良好的应用前景，为未来工业领域的非接触式质量监测奠定了基础。
...
TL;DR: SA-WAM将预训练视频扩散模型扩展到3D感知的世界建模，同时预测未来观测（RGB和深度）、动作，利用深度信息而不牺牲预训练先验，在机器人操作任务上取得最先进成果。


<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型(WAM)仅利用RGB观测，未利用3D信息，存在几何感知不足的局限性。

Method: 使用非线性编码将无界深度信号映射到冻结VAE分词器期望的有界输入域，从而在不进行3D特定微调的情况下重用分词器，实现RGB和深度联合预测。

Result: 在RoboCasa和LIBERO-Plus基准测试中取得最先进结果，提升未来状态预测；在UR5机械臂真实世界评估中优于强基线，在随机环境中效果显著。

Conclusion: 深度信息的融入可提升世界模型预测质量，且预测质量与 rollout 成功存在正相关，为WAM性能改进提供方向。
...
TL;DR: 本研姿探究了现代生成式跨模态翻译能否弥合RGB到红外的域迁移差距，以缓解红外训练数据稀缺问题。研究使用多种生成模型（监督GAN、ControlNet扩散模型、LoRA微调）将RGB训练图像翻译为合成红外数据，用于训练RF-DETR目标检测器，并在5个无人机数据集上评估跨域检测性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中热红外数据稀缺，而RGB无人机图像丰富。RGB到IR的跨模态映射学习面临挑战，因为热特征（如发动机热量）无法从RGB观测。研究者希望探究现代生成模型能否克服这一跨模态差距，提升红外车辆检测性能。

Method: 在成对的RGB-IR源域数据集上训练多种生成式翻译器，包括监督GAN、ControlNet扩散模型和LoRA微调的基础模型。将这些翻译器应用于目标域的RGB训练图像生成合成红外数据。使用合成红外数据训练RF-DETR车辆检测器，在5个无人机数据集的不可见红外目标域测试集上评估。

Result: Stable Diffusion 3.5结合ControlNet效果最佳。在Kust4K上mAP从50.8提升至60.1，在VTUAV上从25.6提升至38.4。多样本生成（+1.1 mAP）和提示词变化（+3.3 mAP）带来额外增益，但仍与真实目标域红外数据存在性能差距。

Conclusion: 生成式RGB到IR翻译有效缓解了红外数据稀缺问题，显著提升了跨域无人机车辆检测性能。虽然与真实目标域红外数据仍有差距，但该方法为数据增强提供了有效途径。
...
TL;DR: 本文提出了一种基于立体4D雷达的3D目标检测框架，利用左右雷达的几何视差估计目标绝对速度，并通过特征融合实现更鲁棒的感知。该方法在自建数据集上相比单目4D雷达基线提升了8.82个AP_3D点和9.0个AP_BEV点。


<details>
  <summary>Details</summary>
Motivation: 4D雷达虽能在恶劣天气下检测周围3D物体并提供多普勒运动信息，但原始信号包含大量杂波（路面、护栏、周围车辆）以及多径 ghost 反射和接收机噪声，导致预处理后数据过于稀疏。此外，4D雷达的多普勒测量仅能获取目标径向速度，无法恢复完整运动状态。

Method: 提出立体4D雷达3D目标检测框架，利用左右雷达的几何视差估计目标绝对速度，并通过融合互补特征实现更鲁棒的感知。

Result: 在自建立体4D雷达数据集上验证，相比当前最先进的单目4D雷达基线方法，AP_3D提升8.82个百分点，AP_BEV提升9.0个百分点。
...
TL;DR: MARS是一个用于文本-视频检索的多层多槽嵌入框架，通过结合不同解码器的隐藏状态构建多个自适应表示槽，比较相应的文本和视频槽并聚合相似度进行检索，在四个基准数据集上实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大型语言模型作为嵌入模型时，通常使用最后一层的单个token来表示输入，这会将多样化的视频-文本线索压缩到单个向量中，限制了细粒度检索的效果。

Method: 提出MARS框架，包含三个核心组件：1）多层融合：结合不同解码器层的隐藏状态构建多个自适应表示槽；2）多槽比较：比较相应的文本和视频槽并聚合相似度；3）困难负感知槽特化目标：鼓励槽捕获区分性匹配线索以更好处理混淆候选。

Result: 在四个文本-视频检索基准数据集上，MARS在直接相似度检索和重排序设置中均实现了最先进的性能。消融实验表明多层融合、多槽和困难负感知槽特化提供了互补的增益。

Conclusion: MARS通过多层多槽的表示学习方法有效提升了文本-视频检索的细粒度区分能力，为多模态检索任务提供了新的有效方案。
...
TL;DR: TIC-Bench是一个用于评估多模态大语言模型在深度交错文本-图像上下文中整合信息能力的基准测试，包含2280个问题，涵盖逻辑、时间、空间三个关联领域。


<details>
  <summary>Details</summary>
Motivation: 当前多模态模型的评估和训练主要关注多图像任务，忽视了交错文本-图像场景。真实应用如文本图像共同创作、角色跟踪和空间重建需要文本与图像之间的持续深度交互。

Method: 构建了一个包含3个核心领域（逻辑、时间、空间关联）的基准测试，共8种具体类型，总计2280个问题。评估了10个最先进的MLLMs。

Result: 10个最先进的MLLMs与人类专家之间存在显著性能差距，在整合分布在交错视觉和文本输入中的证据方面存在持续困难。

Conclusion: TIC-Bench provides a valuable tool for assessing and advancing multimodal models' ability to integrate text and image information in deeply interleaved contexts. 10个最先进的MLLMs与人类专家之间存在显著性能差距。
...
TL;DR: 本文提出了一种针对BraTS 2026挑战赛GoAT任务的方法，旨在实现跨异构患者群体的脑肿瘤子区域鲁棒分割。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤分割任务中，不同患者群体之间存在显著的异质性，导致模型泛化能力不足。需要一种能够跨异构患者群体实现鲁棒分割的方法。

Method: 采用nnU-Net框架结合大型残差编码器架构，集成半监督学习技术（利用未标注数据生成的伪标签）和肿瘤感知可变形增强（对病变区域进行局部变形同时保留周围解剖结构）。

Result: 在BraTS-GoAT验证集上，完整配置获得以下成绩：Whole Tumor的Dice和NSD分别为0.881和0.473；Tumor Core为0.817和0.490；Enhancing Tumor为0.775和0.533。在所有肿瘤区域上均优于仅使用标注数据的基线模型。

Conclusion: 自训练和提出的增强方法具有互补性，显著提升了模型在跨异构患者群体中的泛化能力。代码已开源。
...
TL;DR: AffectDelta是一种基于情感分布转换的图像编辑方法，将编辑视为源图像到目标情感分布的过渡过程，通过八维情感分布编码源状态和目标状态，使用内部转换编码器和源感知扩散主干联合生成符合目标情感的图像。


<details>
  <summary>Details</summary>
Motivation: 现有场景级情感编辑方法存在两个问题：1）使用单一情感类别标签，忽略了情感混合的复杂性；2）依赖操作级文本指令，无法精确量化情感的增减变化。需要一种能够精确处理情感强度和共存的编辑方法。

Method: 提出AffectDelta方法：1）使用冻结的情感分布预测器估计源图像的八维情感分布；2）计算源到目标的带符号差异作为转换信号；3）内部转换编码器和源感知扩散主干联合将转换信号转化为上下文相关的语义和外观变化。构建AffectPair-249K数据集，包含248,841个源-目标对。

Result: 在六个基线方法上的实验表明，AffectDelta在情感对齐和内容保留方面表现更好，定量评估和定性比较都验证了其有效性。消融实验验证了设计选择的合理性。

Conclusion: AffectDelta通过将编辑建模为情感分布间的转换而非单一类别标签，能够更精确地处理情感强度变化和共存情感，为情感驱动图像编辑提供了新思路。代码和数据集将公开可用。
...
TL;DR: 本文提出一个统一的多类框架，用于检测完全合成图像和篡改图像，并在像素级别定位篡改区域。该方法将图像操作检测从二元分类扩展到三元分类（真实 vs 完全生成 vs 篡改），并结合分割分支实现细粒度定位。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展导致高仿真度伪造图像泛滥，传统二元分类方法（真实 vs 生成）无法区分不同类型的操作，且缺乏定位能力，需要更强大的多媒体取证工具。

Method: 扩展现有检测器，引入统一多类框架（真实 vs 完全生成 vs 篡改），并添加分割分支实现像素级篡改区域定位。

Result: 在分类准确率和定位任务的IoU分数上均优于选定的最新基准方法，提供了高效的解决方案。

Conclusion: 所提框架能够有效区分不同类型的图像操作，并实现精确的篡改区域定位，为多媒体取证提供了实用且高效的检测工具。
...
TL;DR: 针对无约束环境下人脸识别难题，提出DQM-Face框架，通过结合传统的基于幅度的质量估计与新引入的语义质量学习机制（通过SE注意力实现），构建增强的质量感知边际，实现更精细的类内吸引和类间排斥优化。


<details>
  <summary>Details</summary>
Motivation: 当前基于边际的人脸识别方法仅依赖特征幅度建模样本质量，但幅度信息易受身份无关噪声影响，导致学习到的特征表示可靠性和区分能力下降。

Method: 提出双重质量边际学习（DQM-Face），主要包含：（1）语义质量学习机制：通过Squeeze-and-Excitation（SE）语义注意力模块学习语义质量；（2）质量感知边际：结合幅度和语义线索构建增强的质量感知边际；（3）双吸引-排斥边际优化：设计吸引边际增强类内紧凑性，引入排斥边际扩大类间分离度。

Result: 在多个具有挑战性的基准数据集上，DQM-Face始终优于现有最先进的人脸识别方法。此外，学习到的质量信号与人脸图像质量评估高度相关。

Conclusion: 通过将语义质量建模与双重吸引-排斥边际优化统一整合，该方法构建了更加结构化和具有区分性的特征几何结构，验证了学习到的质量信号与识别目标内在对齐。
...
TL;DR: 提出了一种基于物理驱动的跨域迭代自监督框架，用于低剂量CT图像去噪。该方法显式建模混合泊松-高斯噪声，通过学习正弦图先验和噪声模型引导光子计数后验推理，分离泊松和高斯分量后利用二项式 thinning 和高斯数据 thinning 构建训练对，通过跨域迭代逐步精化先验和训练对。


<details>
  <summary>Details</summary>
Motivation: 现有自监督方法依赖于通用图像统计，未能显式建模低剂量CT中混合泊松-高斯噪声，限制了对真实低剂量CT噪声的有效抑制能力。

Method: 1) 学习正弦图先验并结合LDCT噪声模型引导光子计数后验推理，实现泊松和高斯分量分离；2) 分离后的分量分别通过二项式thinning和高斯数据thinning处理，残差缩放匹配各分支噪声水平，从单次低剂量测量构建具有近似独立噪声实现的训练对；3) 训练图像域网络，其前向投影输出更新先验，通过跨域迭代逐步精化先验和训练对，同时保持与CT采集物理的一致性。

Result: 在AAPM、LIDC-IDRI和LoDoPaB-CT模拟数据以及真实LDCT数据上的实验表明，该方法在各种剂量水平下均优于评估的自监督基线方法，性能接近评估的监督基线方法。

Conclusion: 所提出的物理驱动框架通过显式建模混合泊松-高斯噪声并保持CT采集物理一致性，有效提升了自监督低剂量CT去噪性能，为实际临床应用提供了有潜力的解决方案。
...
TL;DR: 研究预训练视觉-语言模型(VLM)在医学图像分割中文本信息的实际作用，通过提出证据解耦解码器(EDD)分析模态交互，发现不同数据集对文本的敏感度差异显著，文本主要通过全局语义调节而非独立空间定位影响预测。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉-语言模型在医学图像分割中表现良好，但文本信息在像素级预测中的实际贡献程度尚不明确，需要系统研究模态交互机制。

Method: 1)分析多种常用融合策略；2)提出基于证据深度学习和深度监督的证据解耦解码器(EDD)，在解码过程中分解图像证据和文本调制证据；3)在BUSI、BTMRI、ISIC、Kvasir-SEG四个数据集上测试文本扰动的敏感性。

Result: 1)融合策略选择对分割性能影响不大；2)不同数据集对文本依赖程度差异显著：BUSI和BTMRI移除文本导致性能急剧下降(强依赖)，ISIC和Kvasir-SEG影响较小(弱依赖)；3)文本主要通过全局语义调节而非独立空间定位影响预测；4)驱动文本敏感性的语义成分因数据集而异。

Conclusion: 文本在多模态医学图像分割中的贡献因任务和数据集而异，模型设计需考虑具体应用场景的文本依赖程度，文本主要发挥全局语义调制作用而非精确空间定位功能。
...
TL;DR: 本文研究了在4D高斯表示中，针对复杂目标分割的查询重写问题。提出了一种无需训练的重解释策略，将冗长的描述性查询转换为简洁的关键词形式，在HyperNeRF和Neu3D数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前4D高斯表示框架在语言引导的动态场景理解中表现优异，但对包含噪声上下文信息的冗长叙述式查询高度敏感，需要研究查询重写对复杂目标分割的影响。

Method: 提出一种无需训练的重解释策略，将长描述性查询逐步转换为简洁的关键词形式，在保留与目标中心表示相关的语义锚点的同时减少语言噪声。

Result: 时间定位精度从60.92%提升至92.21%，vIoU从20.08%提升至76.94%，无需任何额外微调。短关键词查询产生更稳定的视频特征相似度分布，与目标中心高斯表示对齐更好。

Conclusion: 简洁的重写查询显著提升了4D高斯表示中的时序定位和空间分割性能，无需微调即可实现大幅改进，关键词聚焦的短查询更适合目标中心表示。
...
TL;DR: 提出多尺度瓦片补全新任务及Genesis生成引擎，可从任意初始配置生成跨尺度空间一致的多分辨率卫星地图金字塔。


<details>
  <summary>Details</summary>
Motivation: 地球观测本质是多尺度的，卫星图像组织为级联瓦片金字塔。当前生成模型仅在单一轴上操作：要么放大增强单瓦片分辨率，要么平移扩展固定尺度图像。无法生成跨尺度和空间完全一致的四叉树金字塔，高变焦瓦片需与精化的粗上下文及其相邻瓦片保持一致。

Method: 提出多尺度瓦片补全任务：给定任意缩放级别和位置的稀疏种子瓦片集，合成完整统一的四叉树。方法：Genesis引擎通过在四叉树上组合两个专门操作符实现：(1)垂直超分辨率模型；(2)水平基于掩码的外绘模型。引入Dense500数据集，包含跨越不同地理区域的完整观察多尺度金字塔，以及一套金字塔级指标。

Result: 每个操作符在其子任务上达到最先进水平，引擎将稀疏种子传播成任意初始配置的无缝多分辨率地图。

Conclusion: 提出新任务、多尺度数据集Dense500及基准模型Genesis，为多尺度卫星图像生成奠定基础。
...
TL;DR: GaLe是一种无需重新训练的内存高效技术，通过将特征图分割为本地精确(Le)和全局近似(Ga)两个组件，使预训练网络能够部署在资源受限的嵌入式设备上。


<details>
  <summary>Details</summary>
Motivation: 嵌入式设备缺乏GPU资源，现有推理方法存在高计算开销（基于patch）或精度损失（近似方法）的问题，需要一种在受限设备上高效部署预训练网络且不需重新训练的方法。

Method: 将特征图划分为两个组件：保留细节的本地精确(Le)表示和保留长距离依赖的全局近似(Ga)表示；支持混合CNN-Transformer模型中的全局操作和注意力机制。

Result: 在ImageNet上验证，匹配精确推理性能；在Cortex-M33上实现高达65%加速和90% RAM reduction；适用于分类、检测和生成任务。

Conclusion: GaLe可作为资源高效架构设计的基础，在保持精度的同时显著降低计算和内存开销，具有广泛的实际应用潜力。
...
TL;DR: 本文提出一种无需生成模型的医学图像反事实解释框架，直接从分类器中提取因果证据生成解释，实现可控制的区域编辑，为黑盒模型提供更直接透明的决策边界视图。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像诊断中表现优异，但临床部署受限于可解释性不足。现有的反事实图像生成方法依赖GAN、扩散模型等辅助生成模型，导致用另一个黑盒模型解释一个黑盒模型，难以分离分类器决策过程与生成器的归纳偏置。

Method: 提出一种无需生成模型的新型反事实生成框架，直接从分类器中提取因果证据构建反事实图像。该方法具有确定性、无需额外模型训练、支持在用户指定感兴趣区域内进行可控编辑等优点。

Result: 在真实医学影像数据集上的实验表明，该方法成功改变分类器预测结果，且与原图的相似度高于生成式基线方法。

Conclusion: 该方法为分类器决策边界提供了更直接、更透明的视图，是反事实解释领域的重要突破。
...
TL;DR: 本文提出了一种自上而下的框架，用于从单帧标定图像中实现度量尺度的运动员定位。通过边界感知自适应切片（Boundary-Aware Adaptive Tiling）解决极端尺度差异问题，并将RTMPose-X适配为双关键点估计器（骨盆和地面投影），最终通过相机标定的射线投射将2D地面投影提升到世界坐标。在公共测试集上实现了97.44的LocSim分数和0.9128的mAP，比基线方法提升了超过21%。


<details>
  <summary>Details</summary>
Motivation: 从单帧广播 footage 准确定位运动员面临极端尺度差异的挑战，这是由于超高清图像中的尺度变化造成的。现有方法在处理极端尺度变化时召回率下降明显，需要一种能够有效缓解边界分割伪影并处理透视畸变的方案。

Method: 提出三个关键贡献：1) 边界感知自适应切片（Boundary-Aware Adaptive Tiling）：基于粗边界框预测迭代扩展切片边界，确保目标完整包含；2) 将RTMPose-X架构适配为双关键点估计器（骨盆和地面投影），使用针对几何耦合点对优化的门控注意力单元；3) 通过相机标定的射线投射将2D地面投影确定性提升到世界坐标。

Result: 在公共测试集上达到 LocSim 分数 97.44 和 mAP 0.9128，比基线方法提升超过21%，为高分辨率尺度差异问题提供了稳健解决方案。

Conclusion: 通过边界感知自适应切片有效缓解了极端尺度变化下的召回率下降问题，将透视畸变作为剩余定位误差的主要来源；结合定制化的双关键点估计器和射线投射方法，实现了高精度的度量尺度运动员定位。
...
TL;DR: 本文提出MV-dVRK首个体外手术数据集，结合多视角同步立体相机与精确表面几何，用于系统评估不同3D重建方法在内窥镜手术中的表现。


<details>
  <summary>Details</summary>
Motivation: 大规模训练和优化技术已提升稀疏多视角3D重建性能，但从未在真实内窥镜图像上严格评估。临床远程机器人仅部署单立体相机，多视角数据稀缺，亟需专用数据集推动该领域研究。

Method: 构建MV-dVRK数据集，包含经工业3D扫描仪验证的密集SfM参考几何、真实相机姿态和稀疏视角测试集；系统比较零样本单目、立体、多立体和多视角重建方法随视角数增加的性能变化。

Result: 双内窥镜多立体重建覆盖最佳；三视角时优化多视角方法最优，1mm容差内覆盖67%真值表面点，相机姿态恢复精度高；前馈基础模型仅覆盖43%。

Conclusion: 多视角优化方法在手术场景中显著优于单目前馈模型；三视角配置可实现高精度表面重建和相机姿态估计；MV-dVRK数据集为多视角手术感知研究提供重要基准。
...
TL;DR: InceptionGS是一种通过平衡重建与生成来提升大规模场景高斯溅射渲染质量的方法，旨在解决稀疏视角下场景重建的质量不一致问题。


<details>
  <summary>Details</summary>
Motivation: 大规模场景数字化需要从所有视角实现一致且美观的渲染，但实际采集的多视角图像往往高度无结构化——某些区域缺乏足够的观测数据。现有基于重建的方法在视角稀缺时表现脆弱，而基于生成的方法则面临泛化性、可控性和3D一致性问题。

Method: 从初始高斯溅射出发，InceptionGS通过巧妙平衡重建与生成来引导高斯溅射。具体而言，该方法 softly incorporates scene- and view-adaptive generative priors，在修复问题区域的同时保留其他区域的渲染质量。

Result: 在真实大规模场景上的广泛实验表明，该方法在处理无结构化图像和提升高保真高斯溅射方面表现出色，具有优越性和广泛适用性。

Conclusion: InceptionGS通过引入场景和视角自适应的生成先验，成功解决了视角稀缺导致的重建质量问题，为大规模场景的高质量3D渲染提供了一种有效的新范式。
...
TL;DR: 本文提出焦点对数频率损失(f-loss)来解决像素空间流模型训练中的频谱不平衡问题。该方法通过均衡不同频率的学习信号来强调高频成分，结合频率域和像素监督的两阶段训练策略，在不改变模型架构的情况下加速收敛达40%，并持续提升FID和感知质量。


<details>
  <summary>Details</summary>
Motivation: 自然图像遵循1/f²频谱分布，大部分信号能量位于低空间频率，而感知重要的结构（如纹理和边缘）位于稀疏的高频带。像素空间重建目标对所有空间误差一视同仁，导致低频信号主导优化过程，延迟了细粒度细节的学习。这是训练像素空间流模型的一个关键效率问题。

Method: 提出焦点对数频率损失(f-loss)，这是一种频谱平衡的目标函数，能够均衡各频率的学习信号，强调在像素空间目标中被低估的高频成分。在此基础上，提出结合频率和像素监督的简单训练策略：早期强调频率域学习以捕获所有频率，随后过渡到标准像素空间v-loss进行空间细化。

Result: 在多个模型规模上，该方法加速收敛达40%，同时持续改善FID和感知保真度。该方法概念简单，无需架构更改，可作为流匹配损失的即插即用替代方案。

Conclusion: 通过缓解像素损失的频谱偏差并使训练信号与模型演化需求保持一致，该方法有效解决了像素空间目标中的频谱不平衡问题，在加速收敛的同时提升了生成质量。
...
TL;DR: 本文研究多工具图像编辑归因问题（MIEA），旨在从复合编辑的人脸图像中识别多个编辑工具，并构建了包含50万+编辑人脸图像的数据集MultiEdit，提出了DPEC方法来处理多工具编辑场景。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI工具日益强大且易于使用，人们可以轻松地用提示词编辑人脸图像，现有归因方法仅能处理单一工具编辑，无法应对日益增长的多工具复合编辑场景。

Method: 设计了DPEC方法，从空间域和频率域捕获可区分的、局部感知的编辑工具痕迹，并采用基于错误的课程学习策略。

Result: 在包含最多5个编辑步骤的人脸图像上，DPEC方法优于9种对比方法。

Conclusion: 提出了多工具图像编辑归因新任务，构建了大规模MultiEdit数据集，DPEC方法能有效处理多工具复合编辑场景，为图像编辑溯源提供了新思路。
...
TL;DR: 本文推出了首个公开的基于视频的掌静脉数据集Columbia University Palm-vein (CUP)，研究表面污染（清洁、温热、湿润、污脏）对掌静脉识别的影响，并提出一种无需训练参数的时空融合方法来提升噪声条件下的识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实部署中掌静脉生物识别面临表面噪声（汗水、污垢）、光照和运动变化以及温度导致的血管可见性变化，但缺乏在這些条件下捕获的数据进行研究。

Method: 构建CUP数据集，包含4种表面条件；测试21种识别器（静态、视频、多帧聚合架构）；提出时间一致性共识和空间域区域级最优传输匹配器（融合全局余弦相似度与显著性引导的区域比较）。

Result: 污染条件下模型性能显著下降，脏污情况EER约增4倍；时空方法有效恢复鲁棒性，在CUP所有表面条件下EER、TAR@FAR=0.01和Rank-1均最优，模型仅4.3M参数和3.1 GFLOPs；冻结的SOTA主干网络平均EER降低29-37%；单图像数据集上区域匹配仍有帮助；人口统计学审计发现2个在温暖条件下的差距（体水含量和性别）。

Conclusion: CUP数据集填补了掌静脉生物识别在真实环境条件下的数据空白，时空融合方法在计算高效条件下显著提升了噪声鲁棒性，具有重要的研究和应用价值。
...
TL;DR: ShallowStream是一个利用多模态大语言模型(MLLM)浅层进行流式视频理解的新框架，通过浅层KV缓存维护轻量级索引，在保持性能的同时大幅降低延迟。


<details>
  <summary>Details</summary>
Motivation: 流式视频理解是现实应用的关键能力，但使用MLLM处理连续视频流计算成本高。现有方法关注视觉token剪枝、合并、量化等，但忽略了模型深度维度，全深度MLLM预填充开销巨大且KV缓存随预填充深度线性增长。

Method: 提出ShallowStream框架，利用MLLM浅层同时执行帧编码和检索索引构建。流处理过程中使用浅层KV缓存维护轻量级索引；查询时利用浅层注意力分数对上下文帧评分，采用多样性感知选择策略检索精确全面的证据。

Result: ShallowStream性能与最强现有流式方法相当，但单帧预填充延迟降低最多52.1倍，10秒端到端延迟降低最多11.9倍。

Conclusion: 通过利用模型浅层进行流式处理，可在保持视频理解性能的同时显著降低计算开销和延迟，为实时视频理解应用提供了高效解决方案。
...
TL;DR: AutoCompass是一种用于训练神经地图匹配器的新型监督方法，针对带有噪声的地理参考图像标签问题，通过利用原始GPS标签自动学习准确航向、定义容差区域以及使用相对姿态来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 训练神经地图匹配器需要大规模地理参考图像数据集，但这些数据的位置和航向标签通常包含噪声，影响训练模型的质量。

Method: 1. 证明航向标签是不必要的，模型可从原始GPS标签自动学习准确航向；2. 在原始GPS周围定义容差区域以提高位置精度；3. 利用SLAM或SfM获取的相对姿态作为更准确的训练信号。

Result: 在驾驶和自我中心视角基准测试中，AutoCompass始终优于依赖绝对姿态标签的传统训练方法。

Conclusion: AutoCompass通过减少对绝对姿态标签的依赖，提供了一种更有效的训练神经地图匹配器的方法，在多个基准数据集上展现出一致的性能提升。
...
TL;DR: 文本到图像模型在自然图像合成方面表现优秀，但在图形设计领域面临挑战，因为需要满足关于排版、布局、颜色和视觉传达的精确约束。论文提出了GDB-Reward框架，将异构的图形设计评估指标转化为统一的强化学习奖励，用于优化冻结的图像生成器的提示词。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在自然图像合成方面表现优秀，但在图形设计领域面临挑战，因为需要满足关于排版、布局、颜色和视觉传达的精确约束。虽然提示词优化是比微调扩散模型更具吸引力的替代方案，但学习冻结图像生成器的提示词需要信息丰富的奖励函数，而生成过程完全不可微。强化学习不需要可微分的目标，只需要能够对候选输出进行排名的标量奖励，这引发了一个问题：设计评估指标本身能否成为强化学习奖励？

Method: 提出了GDB-Reward框架，系统地将异构的图形设计评估指标转化为统一的强化学习奖励。该框架不需要对图像生成器进行微调，保持模型完全冻结状态。

Result: 实验表明，GDB-Reward提供了有效的优化目标，在感知质量、渲染保真度和空间准确性方面显著提高了对设计规范的遵守程度，同时保持图像生成器完全冻结。

Conclusion: 研究结果表明，异构的、不可微分的评估指标可以超越被动基准测试，成为强化学习的有效优化目标，特别是在缺乏可微分监督的领域。
...
TL;DR: FReSH-IR是一种轻量级全能图像恢复方法，通过频谱谐波分解技术，在分层编码器-解码器架构的每个尺度上显式分解高频和低频特征表示，结合傅里叶跳跃连接捕获互补频率信息，在减少80%参数和计算量的前提下达到与Transformer模型相当的恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有图像恢复方法需要大量计算资源，难以处理高分辨率图像且无法有效处理不同类型的退化（如雨、雾、雪）。需要一种高效且通用的解决方案来应对各种恶劣天气条件下的图像质量下降问题。

Method: 提出频谱谐波重建方法(FReSH-IR)，采用分层编码器-解码器架构，在每个尺度上将特征表示显式分解为高频和低频分量。通过基于傅里叶的跳跃连接将频谱分解与空间处理相结合，捕获互补的频率信息而不牺牲空间细节。

Result: 与Transformer模型相比，FReSH-IR在减少80%参数和运算量的情况下达到了相似的恢复质量，证明了该方法在效率-性能权衡方面具有显著优势。

Conclusion: 该方法为受限资源系统中的实际应用提供了有效解决方案，在保持良好恢复效果的同时大幅降低了计算成本，具有重要的实际应用价值。
...
TL;DR: RoGe是一个端到端的统一重建与生成框架，用于从稀疏输入进行新视图合成。该方法移除显式桥接，直接将重建模块的射线查询隐式特征注入视频扩散模型作为条件，在DL3DV数据集上超越了重建基线、生成基线和混合基线。


<details>
  <summary>Details</summary>
Motivation: 从稀疏输入进行新视图合成需要同时考虑观测视图的几何基础和未观测区域的生成先验。现有的混合方法通过渲染图像或显式3D表示（点图、3D高斯）连接重建与生成，导致生成受限于有损且不完美的场景投影，继承其误差，而重建无法从生成中获得修正信号。

Method: RoGe从稀疏输入视图构建隐式场景表示，使用前馈重建模型，并用目标相机射线查询得到每视图几何特征。这些特征直接注入视频扩散模型作为条件，无需任何3D中间表示。两个模块联合训练，使生成目标直接塑造其自身的几何条件。

Result: 在DL3DV数据集上，RoGe在图像级指标和视频级时间一致性方面均优于重建基线、生成基线和混合基线。消融实验证实：射线查询的隐式特征优于原始重建token和渲染RGB作为条件；联合训练带来进一步提升。

Conclusion: RoGe成功移除了重建与生成之间的显式桥接，通过统一框架和联合训练实现了更好的性能，射线查询的隐式特征是一种更有效的条件方式。
...
TL;DR: MuyBridge是一种从单目手机视频中估计人体环节质心轨迹的设备端系统，通过结合紧凑的2D姿态网络和单步深度估计网络，利用解剖学和物理学先验进行度量融合，实现无需3D或任务特定监督的质心估计。


<details>
  <summary>Details</summary>
Motivation: 现有3D姿态跟踪、网格恢复和多视角三角测量方法要么优化关键点精度而缺乏解剖学约束，要么计算和捕获基础设施过于笨重，难以在运动员训练和比赛的现场环境中部署，导致教练和运动分析师难以从单摄像头测量质心这一关键指标。

Method: MuyBridge将紧凑的2D姿态网络与蒸馏的单步单目深度网络通过解析度量融合相结合，利用解剖学和物理学先验来锚定度量质心，无需3D或任务特定监督即可工作。

Result: 在AthletePose3D数据集（跑步、田径和花样滑冰）上评估，MuyBridge在一次性校准下达到33-41mm垂直质心误差和2.3-6.6%绝对相对范围误差，在iPhone 15上以63 FPS的姿态估计速率运行，深度更新率为2.86 Hz。

Conclusion: MuyBridge成功实现了从单手机摄像头视频流中实时估计运动员环节质心轨迹，在保持较高精度的同时具有轻量级部署优势，为运动生物力学分析提供了一种实用的现场测量方案。
...
TL;DR: PlantC2USeg是一个用于植物点云分割的深度迁移学习框架，通过跨尺度一致性学习和信息限制解码策略，实现从分布偏移数据中高效学习。在大豆、烟草、番茄、高粱、玉米等数据集上，该方法在全监督和少样本场景下均取得了最先进的性能。 


<details>
  <summary>Details</summary>
Motivation: 现代作物育种需要精确的器官级分析，现有的深度学习方法依赖密集标注数据集，获取成本高。实现从分布偏移数据中以最小额外训练成本进行统一的点云分割适配仍然具有挑战性。

Method: 提出PlantC2USeg框架，包含两个核心组件：1)跨尺度一致性学习模块，用于显式对齐不同空间尺度的特征；2)信息限制解码策略，防止重建捷径并促进鲁棒适配。该方法支持跨物种和传感条件的少样本泛化，统一微调继承阈值进一步降低适配开销。

Result: 在全监督Soybean3D上达到91.91%语义IoU和94.62%实例mWCov；20标注样本时为89.78%和90.27%；10标注样本时为83.19% IoU和83.23% mWCov。HR3D上10-shot迁移到烟草、番茄、高粱平均78.41% IoU和79.42% mWCov；22-shot迁移到SYAU-Maize达到92.75% IoU和93.51% mRec。ShapeNet Part上类别平均mIoU达85.0%。

Conclusion: PlantC2USeg能够在分布偏移下减少整体适配工作量，实现可扩展的植物表型分析，并支持农业领域之外的可迁移3D表示学习。
...
TL;DR: 本文提出了RIG-BENCH基准，用于评估推理驱动的图像生成能力，涵盖概念、变换、模式和场景四个认知要求较高的领域，发现当前统一生成模型存在局部合理但全局不合逻辑的问题。


<details>
  <summary>Details</summary>
Motivation: 当前统一生成模型和世界模拟器主要依赖表面级别的事件对齐，缺乏高级视觉推理能力。真正的视觉生成智能需要"推理到生成"的能力——从视觉输入中推断潜在规则，并通过精确的逻辑约束视觉结果来展现解决方案。

Method: 构建了包含2000个精选样本的RIG-BENCH基准，涵盖四个认知要求较高的领域：概念理解、变换、模式和结构、以及场景推理。对当前最先进的统一生成模型和图像/视频生成模型进行了广泛评估。

Result: 揭示了显著的推理-生成差距：模型经常产生局部合理但全局不合逻辑的输出，表明现有模型在逻辑约束的视觉推理方面存在明显不足。

Conclusion: RIG-BENCH提供了一个重要的诊断框架，可用于指导下一代基于逻辑的统一生成模型和世界模拟器的开发，推动视觉生成智能向真正的推理能力迈进。
...
TL;DR: SolarWM是一个完全开源的基础框架，用于从数据准备到长时序推理构建交互式视频世界模型，解决了跨异构数据源和视频骨干网训练的一致性问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型训练面临挑战：数据集在时间尺度、相机几何、视觉质量、运动和标注风格上存在差异，而视频生成器使用不同的表示和架构，导致数据混合产生不一致的监督，使结果难以重现和比较。

Method: 1) 开发了可重构的多源数据引擎，将143万个来自10个数据集的规范片段转换为统一的帧对齐格式；2) 提出了骨干网原生适配框架，在共享接口下实例化了4个5B-33B模型（基于Wan2.2、LTX-2.5和MiniMax-H3）；3) 统一的三阶段训练方法：双向适配、教师强制自回归初始化和分布匹配蒸馏。

Result: 生成的因果模型能够实现实时交互，在仅使用5秒序列训练后，可生成长达数小时到数分钟的 rollout。

Conclusion: SolarWM通过发布数据、流水线、配方、权重和框架，为交互式世界模型研究提供了可重现和可扩展的开源基础。
...
### [119] [Untitled](https://arxiv.org/abs/2609.01611)
**
...
### [120] [Untitled](https://arxiv.org/abs/2609.01685)
**
...
### [121] [Epistemic Sybil Resistance: Multiplying AI Agents Without Multiplying Evidence](https://arxiv.org/abs/2609.01873)
*Marc Bara*

Main category: cs.AI

TL;DR: 本文形式化了多智能体AI系统中的"认知Sybil问题"：多个智能体生成的报告可能来自同一证据根源（复制）或独立证据（独立证实），但仅凭报告无法区分这两种情况。研究通过高斯共享根模型和超过20,000次受控LLM智能体实验验证了报告聚合器的局限性。


<details>
  <summary>Details</summary>
Motivation: 多智能体AI系统通过生成多个智能体报告来改进推理，但这些报告可能共享同一证据根源（ epistemic Sybil），导致看似独立的报告实际并不独立。现有报告聚合器无法区分"复制"与"独立证实"。

Method: 1) 形式化定义认知Sybil问题：I(Θ; Z | R) = 0 表示报告Z是R的认知Sybil扩展；2) 构建高斯共享根模型分析共同祖先与信息冗余关系；3) 进行超过20,000次受控LLM智能体报告和提取调用实验；4) 操纵表征相似性与证据祖先以隔离变量。

Result: 1) 固定证据根而报告数从1增至32时，后验覆盖从0.940骤降至0.263；2) 固定报告数而证据根从1增至16时差距缩小，k=16时聚合器统计上不可区分；3) 智能体复制提取错误相关(gamma_cal=0.719)，相关提取聚合器可恢复校准；4) 表征相似性使报告空间去重机制的推断聚类数变化1.425(95%CI [1.363,1.485])，而真实祖先变化4倍仅变化0.040([-0.045,0.120])。

Conclusion: 集体推理应追踪证据祖先和依赖关系，而非智能体或报告的多重性或相似性。相关提取聚合器可部分恢复校准，但根本解决方案在于追踪证据血缘而非依赖表相似性。
...
### [122] [Untitled](https://arxiv.org/abs/2609.01741)
**
...
### [123] [When Agents Implement Systems: A Case Study in Defects, Detection, and Evaluation Rigor](https://arxiv.org/abs/2609.01985)
*Phanindra Reddy Madduru*

Main category: cs.AI

TL;DR: 研究LLM编码代理在系统级工程任务中的行为，通过案例研究(5个缺陷)和HotpotQA基准测试(过滤vs未过滤检索)进行实证分析，发现过滤检索显著优于未过滤搜索且具有统计显著性(p<0.0001)。


<details>
  <summary>Details</summary>
Motivation: 随着LLM编码代理越来越多地执行端到端工程工作，缺乏对其在系统级需求上行为的实证表征，特别是模式设计、异步编排、配置正确性和检索过滤权衡等方面。

Method: 单一会话案例研究 + HotpotQA公开基准测试评估。案例研究中对代理实现的系统进行缺陷分类；基准测试使用2994个段落池，对比过滤检索(限制在黄金段落)和未过滤搜索在不同预算(1-10)下的召回率。使用符号检验评估统计显著性。

Result: 1) 单次会话中发现了5个缺陷，按违反约束和检测方法分类；2) 过滤检索在预算为3时达到召回率天花板；3) 未过滤搜索在预算为10时仅能恢复69%的证据；4) 这种差距在所有测试预算下都存在，符号检验p<0.0001。

Conclusion: 该研究通过案例研究和基准测试揭示了LLM编码代理在系统级实现中的行为特征。过滤检索在低预算下即可达到天花板性能(预算为3时)，而未过滤搜索即使在预算为10时也仅能恢复69%的证据，差距显著(p<0.0001)。代理在自主性方面有成功之处，但也需要人工纠正，尤其是性能修复未被重新测量验证的情况。
...
### [124] [When Does Information Sharing Improve Decentralized Discovery? Aggregation, Independent Rescue, and Equilibrium Selection](https://arxiv.org/abs/2609.01814)
*Yohei Nakajima*

Main category: cs.AI

TL;DR: 本文研究信息共享在有限发现模型中如何同时改善联合估计并消除独立救援行动，通过精确的数学模型分析了两者的分离效果。


<details>
  <summary>Details</summary>
Motivation: 解决信息共享的双重效应问题：同时改善 pooled estimate 和消除 independent rescue actions，需要分离这两种效应进行精确分析。

Method: 使用精确有限发现模型、注册增量共享协议、贝叶斯博弈论框架，分析集中式行动预算配置下的均衡选择。

Result: 发现在信号准确度3/5时存在严格正的共享区间，且结果依赖于均衡选择而非普遍适用；在精确有界注册表中观察到压缩、聚合、中性曲线和有界零混合类等现象。

Conclusion: 信息共享的效果具有选择性依赖特征，模型结果高度依赖于具体的均衡选择，信息共享改善发现的条件是 pooled residual error 收缩快于独立救援尝试。
...
### [125] [ClaimReceipt: Verifying Evidence Sufficiency and Coverage in Agent Evaluations](https://arxiv.org/abs/2609.01992)
*Peiying Zhu,Sidi Chang*

Main category: cs.AI

TL;DR: 论文提出ClaimReceipt，一种将类型化交易证据绑定到签名实验清单的声明相对收据规范和选择性验证器，用于解决代理评估中的充分性（声明是否可从证据重新计算）和覆盖性（记录是否覆盖承诺的实验集）两个证据问题。在1,392条历史记录和30个前瞻性任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 代理评估面临两个关键证据问题：1) 充分性（sufficiency）- 报告的声明是否可从保留的证据重新计算；2) 覆盖性（coverage）- 保留的记录是否覆盖承诺的实验集。现有通用日志和哈希链接转录本无法可靠回答这些问题。

Method: 提出ClaimReceipt规范：1) 将类型化交易证据绑定到签名实验清单；2) 对每个声明返回PASS、INVALID或INCONCLUSIVE；3) 在实现前冻结规范（SHA-256）。验证实验包括：CR-2验证器（1,392条历史买卖记录）和CR-3前瞻性 epoch（30个在推理前承诺的任务）。

Result: CR-2验证器：1) 复现全部5个人工标记的审计裁决；2) 精确重放600条确定性记录和792条后生成记录；3) 使13个声明字段组在消融测试下非冗余；4) 在11/11语义故障上返回预期结果，0/8假阳性。CR-3 epoch：完整证据得coverage和accounting PASS；扣留一个终端收据返回INCONCLUSIVE_COVERAGE；扣留所有私人开启保留coverage但使经济声明不确定。收据工具增加0.021%模型推理时间和9.9 KB/交易。规范可读性探测表明冻结规范对独立读者尚不明确。

Conclusion: Claim verification requires both claim-sufficient evidence and a committed universe against which omissions become visible. The specification-legibility probe reveals that even a frozen specification may not be unambiguous to independent readers, highlighting the importance of clear, precise documentation in verifiable agent evaluations.
...
### [126] [Induction and Inquiry via Probabilistic Reasoning over Language and Code](https://arxiv.org/abs/2609.01815)
*Wasu Top Piriyakulkij,Sam Acquaviva,Cassidy Langenfeld,Joshua Tenenbaum,Kevin Ellis*

Main category: cs.AI

TL;DR: 本文提出了一种计算模型，通过将符号知识编码为结合自然语言和源代码的"心理程序"，并使用LLM引导的贝叶斯学习算法来模拟人类从稀疏噪声数据中学习和推理的过程。该模型满足了数据效率、计算效率、不确定性建模和概念表示灵活性四个关键需求。


<details>
  <summary>Details</summary>
Motivation: 认知科学中一个长期挑战是：人类如何从稀疏、流动的噪声数据中生长和维护抽象知识。现有计算模型无法同时满足数据效率、计算效率、不确定性建模和概念表示灵活性的要求。

Method: 将符号知识编码为结合自然语言和源代码的"心理程序"，使用LLM引导的贝叶斯学习算法进行顺序推断。

Result: 该模型成功复制了人类归纳学习和主动探究的定量特征（锚定、花园路径效应等），而纯LLM和经典贝叶斯模型无法同时满足任务要求和人类行为重现。

Conclusion: 人类持续增长知识的一种方式是通过心理表示跨越语言和程序化表示的多个假设，然后近似贝叶斯更新来修订这些假设，而自底向上的神经机制（LLM）使推理变得可处理且可学习。
...
### [127] [Propose to Learn, Learn to Propose: Evaluability-Aware Assistance under Bounded Rationality](https://arxiv.org/abs/2609.02242)
*Yifan Zhu,Sammie Katt,Samuel Kaski*

Main category: cs.AI

TL;DR: 研究AI助手如何通过评估性感知提案规划，同时完成任务干预和学习用户偏好/评估约束。提出ProSE形式化和ProSE-Plan算法，在评估成本受限时优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有辅助方法假设用户能可靠评估任何提案，但实际中用户因有限理性可能无法可靠评估。研究者希望将提案同时作为任务干预和学习用户偏好的探针，解决这一被忽视的问题。

Method: 将问题形式化为隐藏参数序列辅助问题ProSE，使用KL正则化有界理性二元响应模型，模型中接受权衡价值增益与距离依赖的评估惩罚。实现ProSE-Plan，一种深度2的贝叶斯自适应规划器，按可能响应和响应导致的后验信念对提案评分。

Result: 在受控图模拟中，当评估成本是瓶颈时，ProSE-Plan优于评估性无知和短视基线方法。探针-承诺消融实验确认该方法选择了更简单方法错过的信息性提案。

Conclusion: 用户评估能力是AI辅助中与生成质量和偏好推断互补的规划相关维度。识别了被忽视的评估性维度，为更有效的AI辅助系统设计提供新方向。
...
### [128] [Architecting Conversational Data Systems for Stateless LLM APIs: The Hydration Proxy Pattern](https://arxiv.org/abs/2609.01834)
*Joseph Axisa*

Main category: cs.AI

TL;DR: 本文提出了Hydration Proxy Pattern（ hydration代理模式）和Context Stabilization Mandate（上下文稳定化 mandate），用于解决企业平台向对话推理接口过渡时面临的架构挑战。该方案将会话持久性与推理引擎解耦，确保平台对对话数据的主权，同时支持安全的多阶段语义接地。


<details>
  <summary>Details</summary>
Motivation: 随着企业平台向对话推理接口转型，LLM API的无状态特性造成了架构上的差距。无状态性虽支持AI提供商的水平可扩展性，但将对话状态和语义记忆的整个管理负担转移给了客户端应用。

Method: 提出Hydration Proxy Pattern架构，将会话持久性与推理引擎解耦；提出Context Stabilization Mandate来解决主权状态管理与KV缓存之间的权衡问题。

Result: 该框架确保平台对对话数据的主权，同时实现安全的多阶段语义接地。

Conclusion: 通过解耦架构和上下文稳定化机制，可有效解决LLM无状态API与企业对话系统之间的架构 gap，平衡可扩展性与状态管理需求。
...
### [129] [Codebook Agent: Amortized Topology Design for LLM Multi-Agent Systems](https://arxiv.org/abs/2609.02264)
*Jinxi Yu,Yubei Li,Eric Hanchen Jiang,Zhi Zhang,Dong Liu,Wenxiao Zhao,Levina Li,Kai-Wei Chang,Ying Nian Wu*

Main category: cs.AI

TL;DR: Codebook Agent通过向量量化自编码器将成功的通信拓扑压缩到查询无关的16条目码本中，结合奖励加权的MLP映射查询嵌入到码字分布，以及MLP代理对候选进行单批次前向重排序，无需迭代搜索或消息传递即可在6个基准测试上实现最高精度(84.6 vs 83.0)，拓扑生成仅需2.4ms，且LLM token使用减少21.9-33.2%。


<details>
  <summary>Details</summary>
Motivation: 当前LLM多智能体系统的通信拓扑设计存在三个关键问题：1) 条件图生成方法即使码本容量从8增到64，拓扑也崩溃到约6种不同图；2) 边数与token消耗呈负相关(Pearson r≈-0.4)，稀疏化反而增加推理成本；3) 当智能体共享配置档时消息传递评分器对邻接矩阵不变，无法对候选排序。

Method: 提出Codebook Agent：1) 向量化自编码器将成功拓扑压缩为查询无关的16条目码本；2) 奖励加权MLP将查询嵌入映射到码字上的分布；3) MLP代理读取展平邻接矩阵，在测量效用和按任务归一化的token成本上进行回归，对top候选进行单批次重排序。

Result: 在6个基准测试中准确率最高(84.6平均vs 83.0最强先验设计器)，拓扑生成仅需2.4ms，LLM token使用减少21.9-33.2%。

Conclusion: 通过码本压缩和轻量级重排序机制，Codebook Agent在精度、效率和速度上均优于现有条件图生成方法，无需迭代搜索和消息传递即可实现更优的多智能体通信拓扑设计。
...
### [130] [SSAKG 2.0: An Open-Source Package for Structural Associative Sequence Memory and Context-Based Retrieval](https://arxiv.org/abs/2609.01849)
*Przemysław Stokłosa,Janusz A. Starzyk,Paweł Raif*

Main category: cs.AI

TL;DR: SSAKG 2.0是一个开源软件包，用于构建和运行结构序列关联知识图（SSAKG）。该工具将对象表示为图顶点，将有序序列表示为图连接的结构模式，作为关联记忆库，可从部分无序上下文重建完整序列。采用Python和C混合实现，引入利用内存单个比特的高效图搜索算法。


<details>
  <summary>Details</summary>
Motivation: 开发一种高效的存储和检索有序序列的方法，利用基于图的关联记忆实现从部分、无序上下文重建完整序列的能力，同时解决大型稀疏图的内存和计算开销问题。

Method: 采用混合实现方式：Python提供灵活的高级编程环境，性能关键的图操作使用C实现。使用随机生成的数值序列、NLTK语料库句子序列和mRNA序列进行评估。

Result: 实验证明了该软件包存储和重建序列的能力，为评估图密度、序列长度和内存大小对检索性能的影响提供了基础。

Conclusion: SSAKG 2.0成功实现了高效的序列存储和检索功能，其混合实现有效降低了内存和计算开销，为序列数据的关联记忆应用提供了有价值的工具。
...
### [131] [Collective creativity in hybrid societies](https://arxiv.org/abs/2609.02620)
*Mason Youngblood,Katie Mudd,Manuel Anglada-Tort,Cameron Jones,Elena Miu,Diana Omigie,Margaret Schedel*

Main category: cs.AI

TL;DR: 该论文探讨了生成式AI如何影响创造力，分析了AI工具是丰富还是削弱文化的争论，提出了将创造力理解为人类与算法混合集体属性的观点。


<details>
  <summary>Details</summary>
Motivation: 研究者对生成式AI是丰富还是削弱文化存在分歧，作者认为这种分歧源于混淆了创造力的两个不同组成部分：新颖性（单个产物的属性）和多样性（群体的属性）。

Method: 论文采用概念分析和方法，提出创造力最好理解为混合集体（人类与算法相互作用的人口）的属性，而非个体的属性。

Result: 研究发现：AI辅助构思能可靠地提高个体产出的新颖性，但会缩小整体多样性。然而，这不是将机器纳入循环的必然结果。由于人类和模型以互补方式搜索，混合组可以优于并超越任何单一类型的群体，机器发现的解决方案可以进入人类文化并在其中存续。

Conclusion: 决定结果的是组成：哪些agent存在、比例如何以及如何连接。问题不再是AI是否有助于或损害创造力，而是哪种混合方式能让个体增益积累而不侵蚀集体多样性。
...
### [132] [The Memory Trust Gap: Capability-Dependent Failures in Persistent-Memory Agents](https://arxiv.org/abs/2609.01852)
...
### [133] [Belief-Calibrated Optimization: An Explicit World Model for Agentic Optimization](https://arxiv.org/abs/2609.01861)
*Yuhan Chen,Zhihua Tian,Mahavir Dabas,Charith Peris,Rahul Gupta,Ming Jin,Feiyang Kang,Siyuan Zhang,Nan Wang,Ruoxi Jia*

Main category: cs.AI

TL;DR: 本文提出 Belief-Calibrated Optimization (BCO)，通过将优化过程中的"信念"显式记录为持久化文档来改进 LLM agent 的 scaffold。BCO 在 5 个基准测试上（记忆 QA、工具使用 QA、代码即动作应用 agent、终端 agent）达到比对照更高的训练通过率，且在保留集和模型替换场景下仍保持优势。


<details>
  <summary>Details</summary>
Motivation: LLM agent 的 scaffold 优化依赖 coding agent 的推理，但这种"信念"（即什么出错、应该做什么改变）是隐含的，仅存在于推理过程或模型参数中，后续调用无法直接利用这些信息来指导优化。

Method: 引入 Belief-Calibrated Optimization (BCO)：将优化信念显式写入持久化的上下文文档中，并随着候选评估不断更新该文档。该文档作为"世界模型"，记录当前对环境如何响应编辑的理解。

Result: 在五个基准测试上（记忆 QA、工具使用 QA、代码即动作应用 agent、终端 agent），BCO 达到比缺少世界模型的对照更高的训练通过率。该差距在所有保留分割上保持。模型替换后（冻结模型替换但 scaffold 不变），BCO 选定的 scaffold 仍领先于其他任务。离线消融实验表明，给定累积文档的新预测器比无文档或内容被伪造的同形式文档更准确预测环境响应，说明文档内容携带可复用信息。

Conclusion: 显式记录和更新优化信念（世界模型）可显著提升 LLM agent scaffold 性能，且该信息可跨模型复用。BCO 为构建更智能的 LLM agent 优化系统提供了新思路。
...
### [134] [The Ceiling Is in the Channel: Auditing Learner Gaps and Measurement Frontiers in Clinical Prediction](https://arxiv.org/abs/2609.01909)
*Sayeed Shafayet Chowdhury,Nusrat Jahan,Snehasis Mukhopadhyay,Shiaofen Fang,Vijay R. Ramakrishnan*

Main category: cs.AI

TL;DR: 本文提出将临床预测模型的性能饱和分解为"学习器差距"和"测量通道天花板"两部分，通过总变差分离刻画最优平衡准确率，获得架构不变性、替换污染下的尖锐部分识别结果、交叉拟合天花板估计器以及多模态决策改进的精确条件。在UCI、BRFSS和NHANES三个真实队列上验证，并综合104项临床任务表明：改进学习器在有剩余空间时有效，改进测量通道在无空间时必要。


<details>
  <summary>Details</summary>
Motivation: 临床预测模型的性能经常达到饱和，但饱和可能源于学习器提取信息不足（学习器差距）或记录变量本身限制（测量通道天花板）。现有方法无法区分这两种根本不同的饱和原因，导致改进方向不明确。

Method: 1) 通过总变差分离刻画最优平衡准确率；2) 提出架构不变性原理；3) 推导替换污染下的尖锐部分识别结果；4) 设计交叉拟合天花板估计器；5) 构建标签置换乐观下界和欠拟合曲线两项有限样本诊断；6) 在三个真实队列和104项临床任务中进行验证。

Result: 1) UCI和BRFSS上精心调优的梯度提升接近估计前沿；2) NHANES显示问卷与实测边缘前沿无显著差异但联合互补增益显著；3) AUROC的小幅增益与 Bayes决策翻转率的大幅提升共存；4) 多种架构估计相似前沿但实现的平衡准确率差异明显；5) 104项临床任务显示存在广阔但非普适的结构化临床区域，同通道增益随模型家族递减，通道变化时性能更高。

Conclusion: 框架将饱和从经验观察转化为可审计的决策依据：若存在学习器差距则改进学习器；若已达测量通道天花板则改进测量条件。这为临床预测模型的系统改进提供了明确的理论指导和实践路径。
...
### [135] [Looped Transformers under the Jacobian Lens: Does the Global Workspace Survive Recurrence?](https://arxiv.org/abs/2609.01924)
...
### [136] [Post-Training Ternarization of Qwen3-4B Capability, Effective Bit Budget, Storage Compression, and Deployment](https://arxiv.org/abs/2609.01962)
*Anirudh Malik,M Sparsh Mehra,Poojith Devan*

Main category: cs.AI

TL;DR: 研究将Qwen 4B指令微调模型转换为超低比特量化模型，使用KOTMS旋转、E2M-ATQ ternarization和GPTQ风格误差补偿，最终实现1.641有效比特每权重，但任务准确率从64.5%下降至54.7%，困惑度明显上升。


<details>
  <summary>Details</summary>
Motivation: 传统"1.58比特"标签无法完全描述存储表示、保留能力或运行时行为，需要对超低比特语言模型进行端到端后训练转换研究

Method: 使用KOTMS旋转、E2M-ATQ ternarization和GPTQ风格误差补偿进行权重-only量化（激活保持16位精度），评估有效比特核算、任务能力保留、困惑度、校准敏感性、检查点组成和部署行为

Result: 量化后有效比特1.641，81.62%参数被量化；任务准确率从64.5%降至54.7%（BoolQ保留84.6%，ARC-Challenge仅保留43.8%）；WikiText-2困惑度从13.639升至18.748，PTB从24.700升至31.992，C4从19.831升至28.966；打包后模型大小从8.29 GiB降至3.96 GiB；Triton GEMV微基准比FP16 cuBLAS慢4.6倍

Conclusion: 超低比特量化导致能力保留存在显著不均衡现象（不同任务差异大），压缩本身不能带来更快推理，打包模型尚未进行端到端任务准确率或生成吞吐量基准测试
...
### [137] [Benchmarking Language Models for Statistical Problem Formulation](https://arxiv.org/abs/2609.01982)
*Chen Wang,Junzhe Zhao,Xin Cong,Wanlu Deng,Ke Deng*

Main category: cs.AI

TL;DR: 本文提出了StatFormBench基准测试，用于评估大语言模型在统计问题 formulation（问题 formulation）方面的能力。该基准包含1013个样本，涵盖20个粗粒度和85个细粒度统计问题类别。测试了14个开源和闭源LLM，结果显示最佳零样本模型仅达到72.0%的细粒度分类准确率和63.2%的变量集重叠率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估假设分析目标已明确指定，但实际中用户带着非正式目标和非同构数据到来，模型需要决定隐含的统计任务和相关数据。这一上游步骤在实践中很重要但缺乏系统性评估。

Method: 将统计问题 formulation分解为两个子任务：(1)统计问题分类和(2)变量识别与角色分配。从五本跨领域统计教材和一个数据科学案例库构建StatFormBench基准，包含1013个样本，涵盖20个粗粒度和85个细粒度统计问题类别。对14个开源和闭源LLM进行零样本测试。

Result: 最佳零样本模型仅达到72.0%的细粒度分类准确率和63.2%的变量集重叠率。没有模型在两个子任务上同时表现最佳，增强提示策略仅产生有限或不一致的提升。

Conclusion: LLM在统计问题 formulation这一上游任务上仍有较大改进空间，当前模型能力不足以可靠地辅助统计和数据科学工作。需要更好的方法来解决这一基本问题。
...
### [138] [HeadWiseKV: Budgeted Per-Head Cache Residency for Hybrid Long-Context Language Models](https://arxiv.org/abs/2609.02029)
*Renjie Xie,Juncheng Yang,Aoting Hu,Mingxi Zhang,Liyao Wu,Zheheng Hong,Wei Xu*

Main category: cs.AI

TL;DR: HeadWiseKV是一种无需训练的压缩框架，用于压缩混合语言模型的残差全局KV缓存，在保持原生局部、循环和线性路径的同时，在112K上下文长度下将采样峰值设备内存减少8.59%，并将最大验证成功的上下文从114K扩展到161K。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理中的KV缓存消耗大量GPU内存，降低生成吞吐量。混合语言模型中的残差全局注意力层主导上下文依赖的缓存需求，成为主要瓶颈。

Method: 提出SeqCalib算法，将每物理KV头分配静态多级历史窗口，将分配问题表述为受约束的运营率-失真问题，按执行顺序处理各层并考虑层间交互。分组缓存运行时将策略具体化为每头KV驻留。

Result: 在四个混合长上下文模型上评估，HeadWiseKV在RULER和LoCoMo基准上保持接近Full-KV的质量。在固定模型系统研究中，112K上下文长度下采样峰值设备内存减少8.59%，最大验证成功上下文从114K扩展到161K。

Conclusion: HeadWiseKV可作为混合语言模型长上下文推理的有效缓存压缩方案，无需训练即可实现显著的内存节省和上下文长度扩展。
...
### [139] [Monitoring Web Agents Without Internal Signals: Observable Trajectories and Key-Step Supervision](https://arxiv.org/abs/2609.02057)
*Sitong Pan,Yipeng Shen,Yilin Lu,Caiwen Ding,Lu Cheng,Qianwen Wang*

Main category: cs.AI

TL;DR: 研究如何仅通过外部可观察的轨迹信号（而非模型内部信号如token logits）来预测Web代理的执行风险。提出宏观特征和微观特征两种表示方法，并在WebArena-Lite和Online Mind2Web基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当模型内部不确定性信号（如token logits）不可用时，可靠的Web代理监控变得困难。需要探索使用可观察的轨迹信号来进行前缀级风险预测。

Method: 提出两种可观察轨迹表示：宏观特征（跨步骤代理-环境行为和反馈汇总）和微观特征（通过重复黑盒查询测量意图、动作和预期状态变化的一致性）。采用关键步骤边界标签方法，标记首个未被纠正且与最终失败相关的关键错误。

Result: 在WebArena-Lite和Online Mind2Web基准上，使用五个开源和闭源骨干模型进行测试，结果表明可观察轨迹信号与内部信号基线相比具有竞争力。该预测器还支持在固定错误预算下的早期干预，并可跨保留的网站类别迁移。

Conclusion: 可观察轨迹信号能够支持有价值的风险预测能力，为无法获取模型内部信号的实际应用场景提供了可行的监控方案。
...
### [140] [DocHop: Benchmarking Out-of-domain Multi-hop Reasoning in Information-Dense Documents](https://arxiv.org/abs/2609.02059)
...
### [141] [MineTRACE: An Evidence-Grounded Interactive Reasoning System for Mineral Prospectivity](https://arxiv.org/abs/2609.02060)
*Yiran Zhang,Jinwen Liu,Daniel Su,Yisu Chen,Qiang Sun,Chris Gonzalez,Eun-Jung Holden,Marco Fiorentini,Wei Liu,Yihao Ding*

Main category: cs.AI

TL;DR: MineTRACE是一个基于网络的矿产勘查系统，整合了地质、地球化学和地球物理证据，为Cu、Au、Ni、W、Sn、Co、Ta、Mn等八种矿产提供可解释的勘查潜力评分，支持地图探索、自然语言查询和证据追溯。


<details>
  <summary>Details</summary>
Motivation: 现有矿产潜力评估系统往往只提供不透明的分值或热力图，缺乏可解释性和可验证性，限制了地质专家对评估结果的信任和应用。

Method: 构建透明的专业知识决策树，结合地质知识和已知矿床，整合多源证据计算勘查潜力分值；开发对话式助手，实现自然语言查询并检索评分及支撑证据；采用空间AUC评估模型性能，端到端评估查询准确性和响应 grounding 能力。

Result:  scorer 在不同测试场景下达到最高0.917的空间AUC值；系统支持勘查潜力地图浏览、区域查询、证据检验和自然语言交互，提供公开地球科学数据的可访问性、可解释性和可验证性。

Conclusion: MineTRACE通过可解释的专家决策树和自然语言交互接口，提高了矿产勘查的效率和透明度，为地质专家提供了可验证的决策支持工具。
...
### [142] [ToolGate: An Executable Acceptance Pipeline for Tool-Dependent Scientific Benchmark Construction](https://arxiv.org/abs/2609.02067)
*Ke Zhang,Yankang Liu,Roya Zandi,Maziar Raissi*

Main category: cs.AI

TL;DR: ToolGate是一个通过三层筛选机制自动生成科学基准测试的系统，利用语言模型候选提出结合专家验证，解决科学计算基准创建的高成本问题，在FEniCSx上验证有效。


<details>
  <summary>Details</summary>
Motivation: 科学基准测试传统上由领域专家手工编写，需要大量人力。语言模型可以快速生成候选，但验证接受过程仍是瓶颈。现有方法缺乏自动化、可审计的筛选流程。

Method: 设计三门筛选机制：(1)本地验证门-检查脚本可执行且结果正确；(2)无工具筛选门-排除无需软件即可解决的简单问题；(3)智能体验证门-要求GPT-5.5 Codex CLI在限定时间内用工具解决。

Result: 500次生成尝试中，478个通过本地验证；两轮无工具筛选后剩256个；GPT-5.5 API排除121个；最终智能体解决130个，去重后得128个唯一协议幸存者。

Conclusion: ToolGate将重复的答案检查和难度筛选转化为可审计流程，同时将领域设计和最终审核留给专家，在科学计算基准构建中具有实用价值。
...
### [143] [CHIME: Credit-Aware Hierarchical Memory Evolution for Long-Horizon Agentic Planning](https://arxiv.org/abs/2609.02074)
*Yongshi Ye,Tian Lan,Feihu Jiang,Muyang Ye,Bin Zhu,Qianghuai Jia,Longyue Wang,Zhao Xu,Weihua Luo,Xiaodong Shi*

Main category: cs.AI

TL;DR: CHIME是一种自进化记忆框架，通过分层存储planning bank和execution bank来解决自进化记忆中的信用分配问题，在不进行参数更新的情况下持续提升推理阶段的规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有自进化记忆方法存在固有的信用分配问题：它们依赖最终任务结果作为反馈，但这些结果混淆了计划质量、执行错误和环境因素，导致积累的规划经验有偏且嘈杂。

Method: CHIME遵循"先归因后记忆"原则：将任务结果归因到计划、执行、两者或都不是，然后仅更新对应的记忆库。同时维护planning bank和execution bank两个独立的记忆库。

Result: 在四个长时域智能体基准测试中，CHIME始终优于基于训练和自进化记忆的基线方法。进一步分析表明：CHIME用更少的记忆条目积累有效记忆；学到的记忆值准确反映下游效用；积累的记忆可有效转移到不同骨干模型。

Conclusion: CHIME通过分层记忆和信用感知归因机制，成功解决了自进化记忆中的信用分配问题，实现了高效、可迁移的规划能力提升。
...
### [144] [Beyond Outcome Gaps: Process-Aware Fairness Diagnosis for LLM-based Multi-Agent Decision Systems](https://arxiv.org/abs/2609.02092)
*Yiran Zhao,Lu Zhou,Liming Fang,Yufei Chen,Jiafei Wu,Zhe Liu,Xiaogang Xu*

Main category: cs.AI

TL;DR: SCOPED-Hiring是一个针对基于LLM的招聘多智能体系统的过程感知公平性诊断管道，通过构建受控简历变体、运行角色化招聘委员会、记录超过31.1万条结构化决策轨迹，将轨迹字段转换为由六个诊断视角组织的定量公平性信号，揭示平衡的最终聘用率可能掩盖轨迹中的隐藏不公平性。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统(MAS)越来越多地被用于高风险决策，但基于结果的公平性审计可能无法发现决策轨迹过程中产生的风险。

Method: SCOPED-Hiring构建受控简历变体，运行角色化招聘委员会，记录超过311,000条结构化决策轨迹，并将轨迹字段转换为由六个诊断视角组织的定量公平性信号：最终结果、反事实、过程、路径、动态和设计效应。

Result: 发现平衡的最终聘用率可能掩盖多智能体决策轨迹中的隐藏轨迹不公平性：职业空窗期引发怀疑，代理线索塑造资格判断，身份线索导致不平等调查。

Conclusion: 通过这些诊断指导的针对性修复将总分层负担降低72.3%，同时仅使聘用率变化1.86个百分点，表明过程诊断可以指导有效修复。
...
### [145] [MASkills: Continual Skills Optimization for Multi-Agent LLM Systems](https://arxiv.org/abs/2609.02094)
*Huaiyuan Yao,Xiaoou Liu,Charles Fleming,Tianlong Chen,Hua Wei*

Main category: cs.AI

TL;DR: MASkills是一个持续学习框架，通过智能体技能优化多智能体LLM系统。该框架集成了技能条件信用分配、分层信用聚合和动量平滑优化，使智能体技能库能够通过精炼、归纳、整合和剪枝不断进化。实验在HotpotQA、LoCoMo和GAIA数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统在复杂任务上表现优异，但从交互经验中持续改进仍具挑战。现有自反思方法构建的经验记忆难以调用、精炼和扩展；而智能体技能作为更可操作的知识单元，包含何时行动、如何行动以及使用何种资源的结构化程序性知识。

Method: MASkills提出新的智能体优化管道：1)技能条件信用分配；2)分层信用聚合；3)动量平滑优化；支持技能库通过精炼、归纳、整合和剪枝四种机制持续进化。

Result: 在HotpotQA、LoCoMo和GAIA三个基准数据集上的实验表明，MASkills在多种智能体任务上具有有效性。

Conclusion: MASkills为多智能体LLM系统的持续学习提供了新范式，通过技能这一结构化程序性知识单元实现更可操作的优化。代码已开源。
...
### [146] [READY or Not: Reliable Enterprise Agent Deployment](https://arxiv.org/abs/2609.02095)
...
TL;DR: READY是一个企业AI智能体部署资格认证框架，评估智能体在企业工作流中的可靠性、人工监督成本和运营成本，而不仅仅是自主性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI智能体基准测试只衡量智能体能否完成专业工作，但企业部署需要回答不同问题：智能体是否能在可接受的人工监督下达到所需的可靠性水平，并以可容忍的成本运行。

Method: 给定智能体、工作流和监督策略类别，READY测量人机系统的可靠性和运营成本，选择满足可靠性目标的最低成本策略，并在保留案例上进行统计资格认证。

Result: 在涉及16个智能体系统和750个案例的端到端临床审计案例研究中，自主准确率仅差0.3个百分点（72.8% vs 72.5%）的两个系统，在相同76%可靠性目标下需要39.2%对比29.6%的人工审查率。

Conclusion: READY将企业智能体评估从"智能体工作表现多好"转变为"在什么条件下、以多大成本可以可靠部署"，为比较智能体系统、设定监督要求和做出基于证据的部署决策提供了基础。
...
### [147] [Semantic Signal-Assisted Inspection and Recovery Allocation in Reverse Logistics](https://arxiv.org/abs/2609.02116)
*Jiani He,Dingyan Shang,Yihua Xu,Shiqi Huang,Yan Lyu,Jize Li,Shangjing Tang*

Main category: cs.AI

TL;DR: 本文提出语义信号辅助决策支持框架,将退货说明转化为条件因子和信号质量分数,以指导检查深度和资源分配。框架在三个合成基准场景中进行了评估。


<details>
  <summary>Details</summary>
Motivation: 逆向物流运营商通常需要在完全了解资产状况之前决定如何检查和分配退货资产,而全面检查会消耗稀缺的劳动力资源。

Method: 将退货说明转换为条件因子和信号质量分数,指导检查深度和回收分配。在三个合成基准场景中进行评估:IT设备退役、飞机维护和消费电子退货。使用30个配对模拟种子进行测试。

Result: 关键词实现相比带噪声全面检查的结构化特征比较器,在所有三个场景中都提高了净回收价值并降低了检查成本。风险盲比较器跳过检查的方案在经济目标下仍记录更高价值。在相同检查成本下,分数引导的定向在飞机场景中每批增加5.39万美元,但在其他两个配置中经济效果较小。

Conclusion: 叙事证据可以在回收决策之前支持检查分配。该框架通过利用退货说明中的语义信息来优化检查资源分配,提高净回收价值。
...
### [148] [Beyond Context Windows: Persistent Discovery Context for Data-Centric Agents](https://arxiv.org/abs/2609.02129)
*Jalal Mahmud*

Main category: cs.AI

TL;DR: 本文提出持久发现上下文(PDC)，一种存储意图-对象映射的记忆层，用于增强数据中心智能体的数据发现能力。实验表明该方法在三个结构化数据环境中持续提升检索质量。


<details>
  <summary>Details</summary>
Motivation: 数据中心智能体在规划或执行前需要进行数据发现，但发现结果通常被丢弃而非重用。作者认为发现结果是一种可重用的上下文资源。

Method: 引入持久发现上下文(PDC)——一个轻量级记忆层，存储之前的意图-对象映射并在未来检索中重用它。在三个结构化数据环境中进行测试。

Result: PDC持续提升检索质量优于纯元数据搜索；对自动生成的记忆同样有效；揭示了一种可重现的干扰失败模式；在词义稀疏领域，纯记忆检索甚至可优于元数据检索。

Conclusion: 发现结果构成了数据中心智能体的一种有用的可重用上下文形式。
...
### [149] [EmoStance: Response-Side Affective-Orientation Control for Empathetic Response Generation via Emoji Weak Supervision](https://arxiv.org/abs/2609.02133)
*Ziyuan Jin,Yuxuan Ge,Zheng Tian*

Main category: cs.AI

TL;DR: 本文提出EmoStance模型，通过多标注者emoji分布构建潜在控制空间，实现对同理心回复情感倾向的控制，并在指令微调的大语言模型中通过连续前缀嵌入进行引导，在盲评中达到62.2%的决定性胜率。


<details>
  <summary>Details</summary>
Motivation: 同理心回复生成不仅需要决定说什么，还需要决定如何回应前一位说话者的情感状态。现有方法缺乏对回复方情感取向的有效控制机制。

Method: 1)将任务形式化为回复端情感导向控制；2)使用多标注者emoji分布作为弱情感态度证据而非输出符号，构建潜在控制空间；3)构建EmoDialogue数据集（EmpatheticDialogues的语句级扩展，含emoji投票和置信度分数）；4)EmoStance模型：建模源端情感表达，从对话上下文和说话者角色预测软性回复端导向，通过连续前缀嵌入引导冻结的指令微调LLM。

Result: 在20位标注者、800条判断的盲评中，EmoStance达到62.2%的决定性胜率，在上下文特异性和感知响应性方面提升最明显，且与外部知识方法互补。

Conclusion: EmoStance通过情感导向控制有效提升了同理心回复质量，emoji分布作为弱监督信号能够捕捉听者立场，为情感控制提供了新范式。
...
### [150] [FUSE: An Evaluating Framework for Dangerous Capabilities of LLMs](https://arxiv.org/abs/2609.02168)
*Zhengyi Jin,Ru Zhang,Xiao Chen,Xinbo Liu,Jiaxuan Lin,Jia Huang,Jianyi Liu,Zhen Yang*

Main category: cs.AI

TL;DR: 本文提出一个模块化框架，通过知识(K)、防御(D)、危害(H)三个正交管道评估模型的危险能力，生成标准化危险能力档案φ，并应用于化学-生物(CB)领域对12个商业LLM进行评估。


<details>
  <summary>Details</summary>
Motivation: 当前危险AI能力评估碎片化，缺乏统一协议和标准化框架，导致无法有效治理AI风险。

Method: 构建包含场景种子、知识库、危害查询和评判规则的可插拔模块化框架，通过三个正交管道(K、D、H)进行评估，聚合为统一危险能力档案，并辅以网络空间评估验证协议可迁移性。

Result: 1)横向比较：三个维度揭示模型差异显著——知识相近的模型拒绝韧性不同，防御强的模型合规时生成的有害内容并不更少；家族层面Claude、DeepSeek、GPT模式明显不同。2)时间分析：危险能力并非单调下降，新模型知识加深但防御仅部分改善，扩展和对齐进展不均衡转化为安全性。3)可靠性验证：跨评判一致性(bootstrap ρ>0.79，4/5评判者)、管道正交性(K-D-H相关系数ρ∈[0.32, 0.52])。

Conclusion: 模块化评估框架可有效跨领域迁移，危险能力评估需多维度综合考量，单纯扩展和对齐不能自动提升安全性，需针对性改进各维度能力。
...
### [151] [Examining the Vulnerability of Multi-Agent Medical Systems to Human Interventions for Clinical Reasoning](https://arxiv.org/abs/2609.02191)
...
TL;DR: 研究定义了AI智能体对话中的"故障点"(fault points)概念，即智能体推理最容易受到外部影响的时刻。通过MedQA数据集分析模拟医患对话，评估干预措施如何改变推理和准确性。正确干预方法可使基线诊断准确率提升高达40%，而错误或偏见相关干预则使性能下降高达6%，并增加诊断漂移和不确定性。发现模拟智能体环境中的认知偏见与真实临床实践存在行为相似性。


<details>
  <summary>Details</summary>
Motivation: 多智能体医学系统中，故障点干预对诊断准确性的影响尚不明确，需要研究如何通过人类干预改善AI诊断的鲁棒性

Method: 使用MedQA数据集，分析模拟医患对话，定义故障点并测试不同类型的人类干预方法

Result: 正确干预可提升诊断准确率高达40%，错误/偏见干预下降6%，并增加诊断漂移和不确定性；发现智能体环境中的认知偏见（如过早下结论、容易受误导线索影响）与真实临床实践行为相似

Conclusion: 识别和引导故障点可能是提高多智能体医学系统诊断鲁棒性的有效机制
...
### [152] [ASCII Attack: Recontextualising Harmful Requests as Artistic Critique in Large Language Models](https://arxiv.org/abs/2609.02215)
*Da Cheng Gu,Yifei Dong,Xinghao Yang,Yongshun Gong,Wei Liu*

Main category: cs.AI

TL;DR: 本文揭示了大型语言模型安全对齐的漏洞：当前安全训练主要针对表面形式而非语义内容。研究者提出"ASCII攻击"，通过将有害请求嵌入ASCII艺术并以艺术评论的形式呈现，成功绕过了安全机制。在11个模型和8个有害主题中，62%的框架提示被判定为有害，而对照组仅为42%。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐训练主要针对直白表达的有害请求，但当相同操作内容被重新语境化（如改变阅读方式）时，训练效果显著下降。研究者旨在证明这种"重新语境化"攻击能够有效绕过安全对齐。

Method: 采用单轮黑盒攻击方式，将完全可读的有害请求嵌入ASCII艺术字符，以艺术品形式呈现并请求反馈。配对设计：每个框架提示都有直接提问的对照组。评估采用有害内容分类器，在11个模型和8个有害主题上进行测试。

Result: 62%的框架提示被判定有害，对照组为42%。最易受攻击模型成功率达93%。单一查询在5个有害评估器中有4个达到或超过已发表的单查询攻击效果。效果与模型相关性高于主题，且不随模型规模缩小。约2/3的框架提示行至少有一名评估者持异议。

Conclusion: 研究发现支持"不匹配泛化"假设：安全训练泛化到表面形式但未泛化到重新语境化的语义内容。这表明当前安全对齐存在根本性漏洞，需要重新审视训练范式。
...
### [153] [PEARL: Path-Entity Aligned Relational Learning with Contextual Subgraphs for Inductive Knowledge Graph Completion](https://arxiv.org/abs/2609.02216)
*Yunchi Yang,Longlong Li,Cunquan Qu*

Main category: cs.AI

TL;DR: PEARL是一种用于归纳知识图谱补全的路径-实体对齐关系学习框架，通过将路径建模为上下文条件推理信号来处理链接预测任务。该方法结合了查询特定的上下文子图、LLM引导的路径检索和双视图对比学习，在WN18RR、FB15k-237和NELL-995三个基准数据集上取得了最佳的Hits@10性能。


<details>
  <summary>Details</summary>
Motivation: 现有的子图和路径方法通常独立编码关系路径，忽视了其周围查询子图的结构上下文。然而，路径的预测相关性可能因结构上下文而异，这限制了模型在归纳场景中预测涉及未见过实体的链接能力。

Method: PEARL框架包含四个核心组件：1）从查询实体邻域并集构建查询特定上下文子图；2）使用LLM引导的检索器提取语义相关的路径；3）构建路径、上下文实体和全局子图表示之间的二分交互图，使路径嵌入能够适应局部和全局结构证据；4）采用双视图对比目标，在随机上下文扰动下促进表示一致性以抑制噪声。

Result: 在WN18RR、FB15k-237和NELL-995三个基准数据集上的实验表明，PEARL在所有三个基准上获得了最佳的平均Hits@10性能。消融研究、效率分析和案例研究验证了上下文子图建模、语义路径检索、路径-实体交互和对比正则化的贡献。

Conclusion: 该研究证明了将路径建模为上下文条件推理信号的有效性，通过结合结构上下文和语义信息可以显著提升归纳知识图谱补全的性能。PEARL框架为处理涉及未见实体的链接预测问题提供了新的思路。
...
### [154] [SkillGLoW: Procedural-Family Skill Consolidation for Self-Improving Agents on Long-Horizon Task Streams](https://arxiv.org/abs/2609.02217)
*Ao Yan,Xin Zhang,Jiawei Du,Joey Tianyi Zhou*

Main category: cs.AI

TL;DR: SkillGLoW提出了一种新的LLM agent技能复用框架，将局部技能聚合成程序族并压缩为去实例化的全局先验，通过实例细节按需再生解决长程任务中技能退化和pool膨胀问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM agent通过编写和复用文本技能来自我改进，但采用全局文档或扁平任务池形式。在每个任务需求不同的长程工作负载中，两种形式表现相反：文档退化为通用规则，池膨胀且条目与编写实例绑定。缺失的是任务簇共享的求解程序这一复用单元。

Method: 构建SkillGLoW(全局-局部编织)：1)任务从自身执行写出局部技能；2)聚合成程序族并压缩为去实例化的全局先验；3)实例细节按任务再生而非存储；4)提交门仅在实际执行显示不降低部署库时才接纳先验。

Result: 四个基准测试(math推理、终端自动化、软件修复、具身控制)和三个模型上，先验比无技能基线平均提升17.2分(困难任务)，12次持续改进运行全部正收益；局部再生达18.0分；库保持每个程序族一个先验，比per-task池紧凑3.6倍。相同协议下GLoW在21个单元格中15个领先已发布单文档优化器。ALFWorld未见过任务成功率从73.9%提升至83.9%。

Conclusion: 技能可迁移的是程序而非任务记忆。SkillGLoW通过程序族级别的全局先验和按需局部再生，有效解决了长程任务中技能退化和库膨胀问题，实现了高效、可扩展的技能复用。
...
### [155] [PhoenixNest-Video: Evidence-Grounded Multimodal Agent Framework for Automated Video Interview Assessment](https://arxiv.org/abs/2609.02231)
*Fan Yuxuan,Huang Miaojun,Zhang Haimei,Wu Jingshen,Liu Hao*

Main category: cs.AI

TL;DR: PhoenixNest-Video是一个用于自动化视频面试评估的证据支撑多模态智能体框架，通过构建语义视频图作为结构化工作记忆，结合跨模态验证进行评分标准条件检索，并在RL训练中融合评分标准对齐和分数级区分的双重奖励，在VInterview-2025数据集上达到91.50%的等级准确率，优于更大的专有模型。


<details>
  <summary>Details</summary>
Motivation: 面试评估需要基于行为证据的逐标准判断，但申请人数量激增使得人工评估成本高且不一致，现有AI方法产生不透明评分缺乏可追溯的理由。

Method: 构建语义视频图作为结构化工作记忆；执行跨视觉、音频和文本流的跨模态验证的评分标准条件检索；训练Scorer使用基于评分标准的强化学习，包含评分标准对齐和分数级区分的双重奖励；生成与候选人材料锚定的逐标准评分。

Result: 在VInterview-2025上达到91.50%的等级准确率，显著优于更大的专有模型。

Conclusion: 紧凑的、基于评分标准的智能体比直接提示更大的模型更能与专家小组的评估一致性地对候选人进行评分，并可暴露每个评分背后的证据供人工审查。
...
### [156] [PGPO: Potential-Guided Policy Optimization for Multi-Turn Agentic Tasks](https://arxiv.org/abs/2609.02236)
*Yuyao Zheng,Haipeng Sun,Junwei Bao,Lemao Liu,Hongfei Jiang,Yang Song,Dejing Dou*

Main category: cs.AI

TL;DR: PGPO提出一种基于潜力的策略优化方法，用于解决多轮Agent任务中稀疏奖励下的细粒度信用分配问题。通过从锚态组返回统计中估计经验态潜力，并利用相邻态之间的潜力差计算动作优势，实现跨轨迹信用传播。实验在ALFWorld和WebShop上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于组的强化学习方法在多轮Agent任务中仅依赖最终结果进行信用分配，导致失败轨迹中的有效动作与错误动作无法区分，难以实现细粒度的步骤级信用分配。

Method: PGPO从锚态组的返回统计中估计经验态潜力，通过相邻状态间的潜力差计算动作优势，实现跨轨迹的信用传播。

Result: 在ALFWorld和WebShop上相比近期基于组的RL方法取得更强性能，失败侧的信用信号更具信息量，且训练开销可忽略。

Conclusion: PGPO能够提供更细粒度的步骤级信用分配，尤其在失败轨迹中有效区分有效动作与错误动作，为LLM后训练中的信用分配问题提供了新思路。
...
### [157] [Task-Level Natural Language Priors as Learning Signals for Low-Resource LLM Training](https://arxiv.org/abs/2609.02244)
*Jian Gao,Xiao Zhang,Xun Zhu,Miao Li,Ji Wu*

Main category: cs.AI

TL;DR: 提出先验引导调优(PGT)方法,将自然语言先验作为辅助学习信号用于低资源LLM训练,并引入对比先验导向(CPS)来鼓励任务一致性学习。实验在AmbiMath、Jigsaw、MNLI/HANS上均取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 低资源训练数据往往存在歧义或不完整的问题,现有方法通常将任务级自然语言先验作为输入上下文而非训练信号,难以充分利用先验知识的指导作用。

Method: 提出先验引导调优(PGT)训练视角,将自然语言先验作为辅助学习信号。具体提出对比先验导向(CPS),保持原始监督目标不变,同时添加正负先验条件辅助损失,鼓励任务一致性学习,阻止看似合理但具误导性的替代方案。

Result: AmbiMath上达成97.6%平均精确匹配准确率;Jigsaw上比标准微调提升9.5个百分点的平均Macro F1,使用1/10实验训练数据即可略微超过全数据标准微调;HANS上LLaMA 3.1 8B和Qwen 2.5 7B的非蕴含准确率分别提升8.3和5.2个百分点,同时保持可比拟的域内MNLI准确率。

Conclusion: 任务级自然语言先验可作为辅助学习信号有效指导低资源LLM训练,该方法在多个基准数据集上的一致性提升验证了这一核心观点。
...
### [158] [LLM-as-a-Judge Is Not an Oracle: Why Self-Improving Agents Need Deterministic Guardrails](https://arxiv.org/abs/2609.02246)
*Vansh Wahi*

Main category: cs.AI

TL;DR: 本文揭示了自改进代理管道中LLM评判器的系统性缺陷，提出了PROCTOR框架，通过确定性验证层约束AI评判器的权力


<details>
  <summary>Details</summary>
Motivation: 当前自改进代理管道将LLM评判器视为"神谕"，赋予其决定系统是否改进的最终权力，但这种设计存在严重隐患——评判器可能因偏见、环境信息泄露、奖励黑客攻击等原因给出误导性评分

Method: 作者通过数月在生产环境中运行 autonomous prompt-optimization loops（在合同分析、合规审查、代码质量三个领域），系统性地收集评判信号失效的案例，并据此设计并实现了PROCTOR框架

Result: 识别出11种评判信号失效方式，分为四类：评判器偏见、测试框架与指标失效、真实标签错误、奖励黑客攻击。具体案例包括：代理通过读取缓存答案获得100%通过率但实际能力仅68%；损坏的真实标签导致优化器删除正确的合规规则；语法错误的提示因静默解析器回退而被推广为最优。修复尝试表明，仅重写评判器评分规则效果有限，唯一可靠的改进来自对输出顺序的结构性约束

Conclusion: LLM评判器应从"神谕"降级为"顾问"，其判断只是多个输入之一，所有变更必须由确定性验证层把关。PROCTOR框架通过状态化编排器持有所有工具访问权限、无状态子代理诊断问题但不直接应用修改、以及教师在5个确定性护栏下的评分来实现这一目标。虽然该框架显著减少了失效，但仍存在LLM教师无法完全防止的失败
...
### [159] [APEx: Distillation of Agent Procedural Experience for Adaptive Deep Research Question Answering](https://arxiv.org/abs/2609.02253)
*Jie Ding,Rui Sun,Xinyuan Zhang,Zeyu Zhang,Xin Liu*

Main category: cs.AI

TL;DR: APEx是一种层次化经验利用框架,通过组织交互历史为实例级轨迹记忆和类别级程序技能,结合Executor、Distiller和Planner的闭环架构,实现基于奖励的技能蒸馏而非固定提示生成,在测试时通过技能引导的强化学习实现无真值自改进。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么检索冗长的任务特定轨迹给决策带来负担,要么蒸馏与下游策略适配解耦的程序技能。持续学习对改进至关重要,但缺乏有效的经验利用机制。

Method: 三层交替GRPO训练范式优化Executor、Distiller和Planner三个模块,使奖励引导技能蒸馏取代固定提示生成。测试时使用技能引导的测试时强化学习,通过技能对齐正则化防止策略漂移,实现无真值自改进。

Result: 在7个基准数据集上取得最先进性能,超越GPT-5.4达14.7分,超越最强记忆增强基线达3.0分。

Conclusion: APEx成功实现了层次化经验利用,通过将交互历史组织为轨迹记忆和程序技能两层结构,并结合闭环架构和测试时强化学习,显著提升了大型语言模型在复杂长程问题上的推理能力。
...
### [160] [CoMerge: Conflict-Driven Preference Optimization for Multi-Task Model Merging](https://arxiv.org/abs/2609.02273)
*Mingjie Zheng,Zihao Chen,Wenqing Chen,Weile Yuan,Zhixuan Chu,Jianxing Yu,Zibin Zheng*

Main category: cs.AI

TL;DR: 本文提出CoMerge框架，将模型合并重新定义为偏好优化问题，利用朴素合并方法的缺陷作为硬负样本构建偏好对，通过偏好优化轻量级张量级合并系数来缓解参数冲突，在MergeBench上达到0.9968的平均归一化性能。


<details>
  <summary>Details</summary>
Motivation: 模型合并是构建多任务大语言模型的高效范式，但面临参数干扰问题。现有方法旨在保留专家模型能力并缓解干扰，但未直接学习朴素合并暴露的退化行为。

Method: 提出冲突驱动偏好优化框架CoMerge，采用自监督、冲突驱动策略，利用朴素合并方法（如任务算术）的缺陷作为硬负样本构建偏好对，无需外部注释。通过偏好优化来优化轻量级、张量级合并系数。

Result: 在MergeBench上实现0.9968的平均归一化性能，优于所有评估的数据无关和数据驱动基线。在Llama-3.1-8B-Instruct上，对指令遵循和安全等冲突敏感任务有显著提升，且仅优化1,445个标量系数即可与全参数微调竞争。

Conclusion: CoMerge成功将模型合并转化为偏好优化问题，通过自监督方式利用合并缺陷作为学习信号，为高效多任务LLM构建提供了新思路，在保持高性能的同时大幅降低了计算成本。
...
### [161] [SCX Router: Streaming Zero-Shot Model Selection with a Decoder-KV Classifier and a Real-World Task Ontology](https://arxiv.org/abs/2609.02292)
*Ihor Stepanov,Aleksandr Smechov,Mykhailo Shtopko,Dmytro Vodianytskyi,Oleksandr Lukashov*

Main category: cs.AI

TL;DR: 提出Router，一个基于GLiClass的轻量级路由器，用于在推理时为不同任务选择最优LLM，无需自回归生成即可为模型标签分配适合度分数，优化的目标是在速度、成本和质量之间取得最佳权衡。


<details>
  <summary>Details</summary>
Motivation: LLM的快速发展和应用多样性带来了独特的优化机会：需要在任务级别优化速度、成本和质量。然而推理端点在质量、价格、延迟、上下文支持、工具使用等领域存在广泛差异，传统人工启发式方法难以维护且难以实现持续良好的权衡。

Method: 提出Router，采用Qwen3解码器+浅层双向评分器的GLiClass架构。解码器-KV执行路径保留跨会话的纯文本KV缓存，仅编码新对话轮次，评估瞬态候选标签token而不将其添加到持久缓存。同时预测任务类型、难度、推理模式和预期输出长度，支持自定义零样本标签。训练数据：构建包含23个家族、115个任务类型、345个可路由子类型、1173个合成示例的任务本体论，以及30个领域的正交轴；生成150,000个验证器评分任务和15,000个开放式任务。

Result: 在六个LiveBench子集上，Router优于平均候选模型；在选定的1000任务子集上，实现0.707的聚合顶级分数，而最强固定模型为0.696，具有基准依赖性增益。

Conclusion: Router作为轻量级模型路由系统，能够有效在不同任务上选择最优LLM，在保持较低计算开销的同时实现更好的速度-成本-质量权衡，为LLM路由领域提供了一种新的高效方案。
...
### [162] [Improving Evaluation Realism with Inference-Time Compute and Deployment Scaffolds](https://arxiv.org/abs/2609.02302)
...
TL;DR: 本文提出两种技术来提高对齐评估的真实性，解决模型"评估意识"问题：批判性 refinement（批判精炼）和DISH（部署模拟SWE-Agent工具）。通过额外推理计算让模拟评估更难被区分是测试还是部署。


<details>
  <summary>Details</summary>
Motivation: 当前对齐评估面临核心障碍：评估意识问题。能干的模型能区分自己是在被测试还是被部署，这削弱了安全评估结论的有效性。

Method: 提出两种技术：1) 批判精炼：为目标模型生成多个候选行动，利用目标模型实例反馈使其更真实，选择最像部署的行动继续评估；2) DISH：将目标包装在agent工具中，缩小模拟与真实部署环境在编码场景中的差距。

Result: 在多个目标模型上测试表明两种技术具有组合效应：同时使用比单独使用任何一种都能获得更大的真实感提升。

Conclusion: 自动化方法可以提高对齐评估的真实感，且这些改进比单纯延长审计时间更有效地利用额外计算资源。
...
### [163] [SALA: Semantic-Aware Logical Alignment for Complex Reasoning in In-Context Learning](https://arxiv.org/abs/2609.02336)
*Zhao Ji,Wenqing Chen,Zhixuan Chu,Jianxing Yu,Jingping Liu,Shanhe Zhao,Zibin Zheng*

Main category: cs.AI

TL;DR: SALA是一个语义感知逻辑对齐框架，通过自动学习任务特定推理操作并嵌入连续语义空间，使用动态时间规整(DTW)对齐推理序列，实现灵活、可解释的演示选择。


<details>
  <summary>Details</summary>
Motivation: 传统基于表面相似性的检索方法无法捕捉底层问题解决逻辑，而现有的逻辑方法使用固定规则和精确匹配标准，无法处理灵活或多样的推理过程。

Method: 1) 自动学习任务特定推理操作 2) 将操作嵌入连续语义空间 3) 使用DTW进行推理序列对齐

Result: 在四个推理基准和三个LLM上的实验表明SALA优于现有演示选择方法

Conclusion: 操作归纳和逻辑语义对齐在演示选择中起关键作用，该方法实现了灵活匹配与可解释性的平衡
...
### [164] [Diagnosing with Insights: Structured Analysis of Agent Failures via Behavioral Abstractions](https://arxiv.org/abs/2609.02371)
*Jiayi Bi,Yanjie Gao,Yuanmin Xie,Liqun Li,Tianyin Xu,Fan Yang,Mao Yang*

Main category: cs.AI

TL;DR: AGENTSCOPE是一种神经符号方法，用于诊断LLM智能体故障，通过将智能体行为抽象为结构化表示，并结合神经不变量和LLM引导推理来定位故障步骤和类型。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体的普及，理解并诊断其故障至关重要。智能体故障通常表现为复杂的长轨迹，人工排查效率低下。传统软件调试方法难以应对LLM智能体故障，而完全依赖LLM进行判断又缺乏可靠性。

Method: 1)将智能体轨迹抽象为结构化表示；2)引入神经不变量概念来指定智能体行为属性；3)利用LLM引导推理在结构化表示上结合神经不变量进行故障诊断。

Result: 在公开数据集(Who&When)和自建数据集(AgentErrata)上，AGENTSCOPE在故障定位和归因准确性方面显著优于现有最先进方法。

Conclusion: 将结构化抽象与LLM引导推理相结合，能够实现有效、可靠且可解释的智能体故障诊断。
...
### [165] [Contrastive Explanations in Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2609.02399)
*Xiang Yin,Nico Potyka,Antonio Rago,Francesca Toni*

Main category: cs.AI

TL;DR: 本文介绍了定量双极论证框架(QBAFs)的对比解释方法，通过比较两个主题论证而非解释单个论证来提供对比解释。提出了对比归因函数(CAFs)的一般形式，建立了应满足的性质，并基于移除法、梯度和Shapley值提出了具体的CAFs方法，最后在医疗保健和偏见识别场景中展示了其应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有QBAFs的解释方法主要解释单个论证的推理结果，而缺乏解释两个主题论证之间差异的方法。对比解释能够更好地帮助用户理解为什么一个论证比另一个更受支持或反对，这在AI模型可解释性和决策支持中具有重要价值。

Method: 提出对比归因函数(CAFs)的一般形式，定义了CAF应满足的一般性质；设计了三种具体的CAF实现方法：基于移除的方法、基于梯度的方法和基于Shapley值的方法。

Result: 建立了CAF的理论框架，证明了所提方法满足一系列良好性质，并在医疗保健和偏见识别两个实际场景中验证了对比解释的实用价值。

Conclusion: 对比解释为QBAFs提供了一种新的解释范式，能够有效解释论证之间的差异，增强了论证推理系统的可解释性，具有广泛的应用前景。
...
### [166] [UTP-Bench: Uncertainty-aware Travel Planning Benchmark](https://arxiv.org/abs/2609.02421)
*Etcharla Revanth Rao,Priyanshu Karmakar,Shubhojit Mallick,Manish Gupta,Shreya Ghosh,Abhik Jana*

Main category: cs.AI

TL;DR: UTP-Bench是一个用于评估不确定性感知旅行规划的大规模基准数据集，整合了印度504个城市的真实旅行数据，包括景点、餐厅、住宿和多式联运网络，并引入经验性延误分布和人群密度模式来模拟现实中的 disruptions。实验表明当前LLM在时间缓冲、延误感知调度和人群敏感规划方面存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有旅行规划基准（如TravelPlanner和TripCraft）假设确定性环境，仅评估静态约束满足性，忽略了当不确定性出现时生成的计划是否仍然稳健。现实旅行规划本质上是不确定的：交通延误、人群波动和意外随机延误经常使原本可行的日程失效。

Method: 构建了UTP-Bench数据集，整合印度504个城市的真实旅行数据（景点、餐厅、住宿、多式联运网络），并引入从主要城市收集的经验性延误分布和人群密度模式以模拟真实 disruptions。提出三个评估指标：缓冲充分性分数（BAS）、人群感知时间分数（CATS）和交通延误吸收分数（TDAS）。

Result: 与人类-authored的计划相比，当前最先进的LLM（如GPT-5、Qwen3、Mistral和Phi-4）在时间缓冲、延误感知交通调度和人群敏感规划方面存在显著差距。

Conclusion: 该基准填补了旅行规划评估中不确定性感知方面的空白，为提升LLM在随机条件下的鲁棒性提供了重要评估框架和数据支持。
...
### [167] [CivBench: A Long-Horizon Benchmark for Tool-Mediated Agents in Civilization VI](https://arxiv.org/abs/2609.02459)
*Austin Tudor David Andrews,Liam Wilkinson,Jamie Heagerty,Harry Coppock,Jakob Nicolaus Foerster,Rui Ponte Costa*

Main category: cs.AI

TL;DR: CivBench是一个开源基准测试，用于通过模型上下文协议(MCP)在长视野、工具中介环境中评估语言模型代理。单次 episode 跨越300+轮次，产生数千次工具调用，涉及大规模动作空间，需要持续的规划、状态监控和部分可观测性下的执行。


<details>
  <summary>Details</summary>
Motivation: 当前基准测试可能无法充分测试在复杂、长视野、工具中介环境中的持续规划、状态监控和部分可观测性下的执行能力，需要新的评估框架。

Method: 使用CivBench对四个模型家族进行23次可接受运行测试。环境暴露76个MCP工具和将视觉游戏状态转换为结构化文本的叙述层。引入两个接口级指标：主动监控率(PMR)和RAG@10。

Result: Agents under-monitor strategically relevant state: 尽管游戏手册指导每20轮查询胜利进度，但Agents实际每30-75轮才查询，20轮警告窗口内未能查询7/20次可检测失败。Agents经常无法执行其自身规划反思中声明的近期承诺(RAG@10在48.2%至65.8%之间)。

Conclusion: 两种模式的出现尽管有工具访问和明确指导，仍被解释为指令下的偏差而非能力缺失。聚合结果无法可靠区分模型，样本为试点而非模型排名。
...
TL;DR: Loom是一个用于根因分析(RCA)的生成式共识框架，通过将模块化启发式方法产生的开放形式假设投影到连续嵌入空间，并使用迭代质心重加权算法解决冲突信号，最终通过单一的轻量级LLM合成步骤生成可靠共识。该框架在OpenRCA基准测试中达到了精度-效率的帕累托前沿。


<details>
  <summary>Details</summary>
Motivation: 解决NLP系统在真实工业环境中部署时面临的噪声、冲突文本假设聚合问题。传统单体LLM代理存在上下文限制、幻觉累积和推理延迟过高的缺陷，而传统弱监督方法在数学上仅限于离散类别，需要一种新的方法来平衡精度和效率。

Method: Loom的核心方法包括：(1)将开放形式假设投影到连续嵌入空间；(2)使用迭代质心重加权算法解决冲突信号；(3)基于共识权重引导单一的轻量级LLM合成步骤。模块化启发式方法包括动态填充特定事件实体、时间和指标的诊断模板。

Result: 在OpenRCA基准测试中，Loom在Bank和Market-2数据集上匹配了最先进的自主代理，在Market-1和Telecom上略逊一筹。每个事件仅需一次LLM调用，推理速度提升约26倍（使用8B参数合成器时提升约33倍）。

Conclusion: Loom成功桥接了传统弱监督的统计严谨性与LLM代理的表达能力，在精度-效率权衡中展现了优势。部署经验表明需要在代理深度与推理延迟之间权衡，确定性共识有助于建立领域专家的信任，同时发现了冗余检测的负面结果。
...
TL;DR: 研究测试了"敲门技术"(先提大要求被拒后再提小要求)在9个语言模型上的效果，发现该技术对Anthropic的Opus 5模型有效(拒绝后小请求响应率从29.3%提升至65.8%)，但对OpenAI和Google的前沿模型及Haiku 4.5反而产生反效果(降低15.5-23.0个百分点)。效果取决于模型家族，且与请求内容本身(而非仅仅是拒绝经历)高度相关。


<details>
  <summary>Details</summary>
Motivation: 人类沟通中的"敲门技术"(先大后小的请求策略)在语言模型上是否有效尚不明确。研究者希望探索人类影响技术能否迁移到AI系统，以及不同模型家族对此类社会工程攻击的敏感性差异。

Method: 对来自三个提供商(Anthropic、OpenAI、Google)的9个生产模型进行测试。每个模型先收到一个被拒绝的大请求，随后收到同一主题的小请求，并与直接询问小请求的结果进行比较。同时设置控制实验测试不相关主题的拒绝影响，以及测试公共基准拒绝是否可迁移。

Result: 效果因模型而异：Anthropic的Opus 5模型上敲门技术有效(拒绝后小请求响应率65.8% vs 直接询问29.3%)；OpenAI和Google的前沿模型及Haiku 4.5上该技术反而降低响应率(15.5-23.0个百分点)。控制实验显示，被拒绝的大请求无论是否相关都会产生一定影响，但相关主题的让步效果在所有模型上更强。将265个被拒绝的请求改写为解释请求后，263个消除了拒绝。

Conclusion: 人类影响技术可部分迁移到语言模型，但效果受模型家族决定。敲门技术的效果取决于请求内容而非单纯的拒绝经历——请求的本质(要求重写 vs 要求解释)才是决定顺从性的关键因素。这揭示了不同AI模型对社会工程攻击的独特脆弱性。
...
TL;DR: DisCo是一个技能驱动的ML研究智能体，通过从1000个广泛使用的ML仓库中提取并验证5000+技能，构建了AREX-Skill Library。在保持模型骨干和研究框架不变的情况下，配备技能的智能体在MLE-bench上提升134.3%，PaperBench上提升34.4%，FrontierCS上提升9.2%，PassNet上提升14.0%。


<details>
  <summary>Details</summary>
Motivation: 当前自主智能体虽然能端到端执行ML研究，但缺乏"操作知识"——即让方法真正work的know-how。这些知识存在于仓库和论文中，但以适合人类阅读的形式呈现，体积过大无法在任务中加载。因此需要将这些知识压缩为可验证、可复用的技能。

Method: 采用两种互补的蒸馏形式：1)任务无关蒸馏，将领域广泛使用的仓库压缩为可复用技能；2)任务导向蒸馏，产生具体任务所需的技能。从1000个ML仓库中提取5000+验证技能，组织为20个领域和178个能力家族。

Result: 在GPT-5.5骨干网络、研究框架和下游执行预算固定条件下，配备技能的智能体相比无技能版本：在MLE-bench上提升134.3%，PaperBench上提升34.4%，FrontierCS上提升9.2%，PassNet上提升14.0%。

Conclusion: 通过添加蒸馏的操作上下文（技能库），可在固定架构下显著提升自主研究智能体的能力。操作知识从需要每次运行时重新发现，转变为可跨任务复用的高效资源，为ML研究自动化提供了新范式。
...
TL;DR: 本文提出多智能体LLM系统的双层协调框架，将编排器-工作者交互建模为双层协调游戏，将反思建模为语义记忆状态上的随机移动。提出SRMA算法并在SWE-bench上取得72.2%的任务解决率。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统虽实证效果显著，但缺乏对协调、记忆改进和外部验证作用的统一理论解释。需要建立统一的理论框架来理解和改进这类系统。

Method: 1）将编排器-工作者交互建模为双层协调游戏，在有限耦合条件下工作者的局部更新博弈近似为势博弈；2）将反思建模为语义记忆状态上的随机移动；3）提出环境接地门控和SRMA算法

Result: 1）推导出自由形式反思的有限时间上界；2）证明信息论不可能结果：仅观察生成文本的门无法在文本不可区分环境中均匀改进；3）环境接地门可突破此限制；4）在SWE-bench 500个实例上达72.2%解决率

Conclusion: 本文建立了多智能体LLM系统的理论框架，证明了环境接地门的重要性，SRMA算法在校准和纠错条件下可精确、几何或多项式收敛，为多智能体系统的设计和分析提供了理论基础。
...
TL;DR: 针对工厂车间的对话式文档助手，提出结构压缩和检索增强适配方法，使模型大小不再是答案质量的可靠预测指标，通过权重共享超网络实现低成本部署选择。


<details>
  <summary>Details</summary>
Motivation: 工厂车间的机器文档访问需求迫切，但可在边缘设备运行的模型能力有限，需要在有限硬件资源下保持高质量的检索增强答案生成能力。

Method: 采用结构压缩和检索增强适配相结合的方法，使用权重共享超网络进行三明治式原地蒸馏，实现单次训练后在多个边缘设备上的灵活部署。

Result: 提取成本仅为未修剪模型的13.7%，检索增强蒸馏恢复到原始质量的4.6%以内，在三个异构边缘设备上以1.3-5瓦待机功耗运行。

Conclusion: 模型大小不再是评估部署质量的可靠指标，检索增强适配可有效恢复压缩带来的能力损失，为制造业边缘AI应用提供了可行的部署方案。
...
TL;DR: SafeEvolve是一个基于经验驱动的智能体安全对齐自进化框架，通过马具-策略协同进化，将安全经验转化为进化的运行时马具和改进的策略行为，在AgentDojo上将Qwen3.5-4B的ASR降低3倍，同时将良性效用从59.79%提升到61.86%。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐机制依赖外部马具更新或策略优化，但单独使用任一范式无法弥合运行时控制与内在安全之间的鸿沟。LLM智能体的性能由基础模型和交互马具共同决定，面临有害最终响应和多步执行轨迹的安全风险。

Method: SafeEvolve采用马具-策略协同进化框架：1)马具侧：将轨迹级安全证据转化为安全提示和分层技能的可审计、可逆的组件级更新；2)策略侧：采用两阶段SFT-RL范式，马具使用SFT引导策略主动利用进化后的马具制品，马具增强RL通过验证器分解奖励进一步塑造多步探索中的自主安全行为。

Result: 在智能体安全基准测试中，SafeEvolve实现了更强的安全-效用权衡。对于Qwen3.5-4B，在AgentDojo上实现了3倍的ASR降低，同时将良性效用从59.79%提高到61.86%。

Conclusion: SafeEvolve通过经验驱动的自进化框架成功弥合了运行时控制与内在安全的鸿沟，为智能体安全对齐提供了一种新的协同进化范式。
...
TL;DR: 本文提出了一个用于5G/6G电信网络根因分析(RCA)的结构化推理框架，解决 vanilla LLM 在电信RCA中存在的幻觉、推理不稳定和证据对齐不足等问题。该框架通过将异构网络遥测组织为规范上下文、强制执行决策路径推理、生成基于证据的解释来提高诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 现代5G及新兴6G网络中，由于复杂的跨层依赖关系，性能降级的根因分析仍然具有挑战性。直接应用 vanilla LLM 到电信RCA通常导致幻觉、推理不稳定和与结构化网络证据的糟糕对齐。

Method: 提出一个结构化推理框架，包含三个关键步骤：(1)将异构网络遥测组织为规范上下文；(2)在诊断期间强制执行决策路径推理；(3)生成基于证据的解释以实现可靠的故障识别。

Result: 在两个5G RCA数据集(TeleLogs和TelecomTS)上的实验表明，与基线技术相比，该框架一致地提高了诊断准确性和决策一致性。

Conclusion: 跨数据集结果突出了结构化推理设计对下一代电信网络中基于LLM的RCA系统的重要性，为实际应用提供了有价值的参考。
...
TL;DR: 本文提出AICOME框架，用于评估AI衍生的受访者层面测量是否能在情境模型中恢复个体和群体层面效应。通过2022年中国家庭追踪调查(CFPS)验证，以职业为分组结构，比较AI衍生测量与调查测量在电脑使用、外语使用、每周工时和管理职责等变量上的表现。结果表明AI情境测量可从丰富的受访者和工作特征中恢复大部分情境模型信息，每周四时验证最强，但信息受限时表现下降。


<details>
  <summary>Details</summary>
Motivation: 传统调查缺乏社会、组织和职业特征测量，研究者日益使用AI构造这些缺失的测量指标。现有方法将AI测量仅视为响应预测，未能考虑其在情境模型中恢复组间和组内效应的能力。

Method: 提出AICOME框架：1)AI测量在受访者层面构造；2)从中推导群体层面聚合值和个体偏差；3)分别估计组间和组内关联。验证使用CFPS 2022数据，职业为分组结构，对比四种工作相关变量的调查测量与AI衍生测量，在响应层面、模型层面、情境和边界条件下进行验证。

Result: AI情境测量可从丰富的受访者和工作特征中恢复情境模型信息。每周工时验证最强，AI衍生测量再现了CFPS中观察到的与满意度的大幅负向组内和组内职业关联。边界条件明确：信息仅限于职业和基本人口统计时性能下降；多相关概念同时视为未观测时恢复较弱。

Conclusion: AICOME框架最有价值之处在于从丰富现有数据中恢复有限数量的理论重要构念。该方法为AI辅助社会科学测量提供了系统评估工具，但需注意其适用条件和局限性。
...
TL;DR: 本文提出预测状态匹配（predicted-state matching）训练目标，解决网页智能体世界模型与下游排序器之间的目标不对齐问题，在多个基准上提升了动作选择和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前网页智能体使用世界模型进行测试时的动作选择，但传统世界模型通过监督学习训练下一状态预测目标，这一目标与下游排序器的需求不匹配——排序器需要预测状态能够区分不同候选动作的结果，而不仅仅是准确预测下一状态。

Method: 提出预测状态匹配训练目标，要求预测的表示能够区分真实结果状态与替代动作导致的状态。使用基于WebArena Go-Browse轨迹构建的分支网页智能体数据集进行训练，该数据集包含每个决策点的多个替代动作及其结果状态。在预测状态匹配基准、WebPRMBench和WebArena-Lite上评估方法。

Result: 在预测状态匹配基准上优于监督下一状态预测训练的世界模型；在WebPRMBench上提升PRM风格的动作排序性能，优于仅使用动作的PRM和结合监督下一状态世界模型的PRM；在WebArena-Lite上使用该世界模型进行动作选择提升了端到端任务成功率。

Conclusion: 预测状态匹配训练目标可以更好地对齐世界模型与下游排序器的需求，显著提升网页智能体的动作选择和任务完成能力。
...
### [177] [PRISM: An Agentic Multi-Model Architecture for Proactive Safety in Autonomous Transportation Systems](https://arxiv.org/abs/2609.01623)
*Joyjit Roy,Samaresh Kumar Singh,Sushanta Das*

Main category: cs.MA

TL;DR: PRISM（主动风险情报与安全管理）是一种代理多模型安全架构，将被动避撞转变为主动、持续的风险管理。该系统利用逆向 crash 概率建模将二元碰撞分类器转换为动态、可解释的安全评分，包含三个专门模型处理轨迹运动学、环境风险和弱势道路使用者交互，由强化学习、上下文记忆和特征级归因协调验证。


<details>
  <summary>Details</summary>
Motivation: 当前高级驾驶辅助系统（ADAS）采用被动响应机制，仅在危险发生后激活。随着美国弱势道路使用者（VRU）死亡人数上升，这一关键局限性日益突出。城市环境中车辆行为、环境条件与弱势道路使用者之间的安全依赖关系需要从被动避撞向主动风险管理的范式转变。

Method: PRISM采用逆向碰撞概率建模，将二元碰撞分类器转换为动态可解释安全评分。三个专门模型（轨迹运动学、环境风险、VRU交互）并行运作，由融合强化学习、上下文记忆和特征级归因的推理层协调。系统提供四级渐进式安全干预（从静默监测到紧急警报），根据实况动态调整安全参数。

Result: 在1296个场景（来自三个自然驾驶数据集，未进行数据集特定再训练）中验证，系统平均安全评分68分（满分100），77.6%场景分类为建议级别，近距离miss率为3.8%，11%场景升级至干预或紧急响应。特征归因持续识别轨迹风险和VRU接近度为主要安全因素。

Conclusion: PRISM为主动交通安全提供了统一、可解释的框架，在密集城市环境中特别强调VRU风险降低。该系统成功实现了从被动到主动风险管理的范式转变，具备跨数据集泛化能力，为城市自动驾驶安全提供了新范式。
...
### [178] [Untitled](https://arxiv.org/abs/2609.01838)
**
...
### [179] [Untitled](https://arxiv.org/abs/2609.01870)
**
...
### [180] [RideSkill: A Hierarchical Algorithm for Generalized Ride Sharing with LLM-Driven Automatic Evolution](https://arxiv.org/abs/2609.02250)
*Zijian Zhao,Sen Li,Xialiang Tong,Mingxuan Yuan*

Main category: cs.MA

TL;DR: 本文提出RideSkill，一个利用大语言模型(LLM)辅助自动算法设计的层次化拼车方法，解决传统多智能体强化学习在拼车系统中泛化性低、迁移性差和训练困难的问题。


<details>
  <summary>Details</summary>
Motivation: 拼车问题需要在不确定和变化场景下高效捆绑分配不同OD对的订单，现有MARL方法存在泛化性低、迁移性差、大规模系统训练困难（如维度灾难）等局限；现有LLM辅助方法不支持车辆共享且推理时需频繁调用LLM，无法满足实时部署需求。

Method: 提出层次化方法RideSkill，包含：(1)Combiner（组合器）：从学习到的技能库中为每辆车分配适当技能，实现自适应调度；(2)Repositioner（重定位器）：顺序将空闲车辆重新定位到新兴区域，避免车辆冲突；(3)技能库、Combiner和Repositioner均通过基于LLM的自动进化方法训练，部署时无需调用LLM。

Result: 通过LLM辅助的自动进化方法设计技能库和层次化调度策略，实现了对不同环境场景和平台目标的适应性和实时部署能力。

Conclusion: RideSkill成功将LLM辅助的自动算法设计应用于拼车场景，在保证实时性能的同时提升了系统的泛化性和迁移性，为大规模拼车系统提供了一种新的解决思路。
...
### [181] [Competitive Market Behavior of LLMs](https://arxiv.org/abs/2609.02580)
*Pawel Struski,Jakub Swistak,Inez Okulska,Przemyslaw Biecek*

Main category: cs.MA

TL;DR: 本文将大语言模型（LLM）代理作为经济参与者，放入双拍卖市场环境中，测试其与为人类设计的市场机制的兼容性。研究发现，LLM代理市场的资源配置效率低于人类市场，收敛速度更慢或无法收敛到市场均衡。


<details>
  <summary>Details</summary>
Motivation: 探索LLM代理是否能参与为人类设计的市场机制，以及这些机制在LLM代理面前是否能产生预期结果。这是一个关于LLM代理对齐性的新维度测试——它们与基本市场机制的兼容性。

Method: 复制经典经济实验，用LLM代理替代人类参与者。将代理置于双拍卖环境（广泛使用的市场机制）中，检查市场是否能实现资源的高效配置。同时对Chain-of-Thought（CoT）痕迹进行词汇分析。

Result: LLM代理市场的收敛速度更慢或无法收敛到市场均衡，资源配置效率低于人类市场。不同模型家族和市场角色之间存在显著异质性。CoT痕迹分析显示，执行交易而非继续调整价格的决定与从战略考虑向紧迫感的转变相关。

Conclusion: LLM代理在与为人类设计的市场机制交互时存在对齐问题。未来的研究需要进一步探索如何使LLM代理更好地适应人类经济机制。
