...
TL;DR: 提出了一种基于深度学习的框架,用于从地面图像中自动检测、分段和估计木质电线杆的倾斜角度,并对附着的电气警示标志进行分类。该系统使用Google街景中提取的4,570张标注图像进行训练。


<details>
  <summary>Details</summary>
Motivation: 电线杆是支撑电力分配系统和关键公共服务的关键基础设施,其定期检查对于确保电网的稳定性和安全至关重要。传统人工检查效率低下,需要自动化检测方案。

Method: 提出基于Detection Transformer(DETR)的改进模型,结合分割头实现实例分割,并利用分割结果估计电线杆倾斜角度。模型在自定义数据集上训练,该数据集包含来自Google Street View的4,570张标注图像,具有挑战性的真实场景。

Result: 模型在电线杆检测上达到90.43%的平均精度,在标志检测上达到88.26%。倾斜角度估计中,1,433个测试样本中准确估计1,367个,平均绝对误差为1.01度。

Conclusion: 该方法优于RetinaNet、Faster R-CNN、YOLOv3-Tiny等标准目标检测器,并提供了公开的基准数据集,为电力基础设施自动化检测提供了有效解决方案。
...
TL;DR: 本文提出LoRetta，一个用于全球尺度多时相光学图像匹配的基础模型，将密集匹配重新表述为定位-配准问题，并引入LEVIR-GM大规模多时相匹配基准数据集。


<details>
  <summary>Details</summary>
Motivation: 全球尺度遥感图像匹配面临重大挑战：图像对可能在获取时间、季节、视角、空间分辨率和地表覆盖状态上存在差异，导致大的几何偏移、部分重叠和本质上不可匹配的区域，使得直接密集对应预测不可靠且低效。

Method: 将密集匹配重新表述为定位-配准问题：首先定位可匹配的重叠区域和仿射几何，然后在配准框架内细化密集残差。提出LoRetta模型，耦合可匹配性感知仿射定位与引导密集配准。同时构建LEVIR-GM基准数据集，包含103K对齐对和827K增强对，覆盖六大洲、五年时间、0.5-1024米分辨率。

Result: 在LEVIR-GM上，LoRetta达到83.3%的AUC，比最强基线RoMa v2高1.6个百分点，在1像素和2像素处的正确关键点百分比(PCK)分别提升6.5和8.2个百分点，同时推理延迟降低47.8%。

Conclusion: LoRetta作为可重复使用的几何对齐器，在宇航员到卫星和无人机到卫星的地理定位实验中展现出良好的迁移能力，为全球尺度多时相图像匹配提供了有效解决方案。
...
TL;DR: GEB-Bench是一个跨模态基准测试，用于评估AI模型识别抽象结构 motifs（如自指、奇怪环、莫比乌斯扭曲）的能力。每个motif以四种"声音"呈现：自然场景、民间故事、数学定理、程序骨架。研究发现模型存在从识别到跨声音映射的鸿沟：模型在单一声音内识别结构的能力远强于跨声音迁移能力。


<details>
  <summary>Details</summary>
Motivation: 受《哥德尔、艾舍尔、巴赫》启发，测试AI模型能否理解抽象结构并跨不同模态迁移。核心问题是：模型能否看到河流三角洲和闪电并发现它们共享某种结构？

Method: 构建包含自指、奇怪环等抽象结构motif的跨模态类别，每个motif有四种表达形式（自然场景、民间故事、数学定理、程序骨架），表面参数作为干扰变量。评估12个开源和闭源模型。

Result: 发现识别与跨声音映射之间存在显著差距：模型在单一声音内识别结构的能力远强于跨声音迁移。所有模型都受此影响，只有前沿模型才能缩小这一差距。错误更倾向于与设计的几何结构相关，而非感知几何；表面复杂度对所有模型都有影响。

Conclusion: 抽象失败是有规律的。跨声音映射是核心挑战，前沿模型虽表现更好但仍存在此问题。容量只能提供缓冲而非免疫力。
...
TL;DR: 本文提出动态潜在推理(DyLaR)方法,用于视频问答任务。该方法首先将问题定位到感知潜在变量中(包含相关视觉证据的连续隐藏状态),然后自适应决定是否添加推理潜在变量进行推理。与传统依赖长文本推理链的方法不同,DyLaR仅生成不到20个token即可回答问题,在9个视频基准测试上提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答方法通常依赖冗长的文本链式推理,但许多问题在定位到相关对象、动作或帧后即可回答,无需过度推理。这种过度推理导致不必要的计算开销和过长的响应长度。

Method: DyLaR采用三阶段训练:(1)将感知潜在变量锚定在经验证的视觉证据上;(2)将经验证的推理理由蒸馏到推理潜在变量中;(3)使用强化学习进一步优化推理时机的自适应路由策略。该方法适用于四种多模态语言模型骨干网络。

Result: 在9个视频基准测试上,DyLaR相比同骨干网络基线提升了平均准确率,同时将响应长度控制在20个token以内。以Qwen3-VL-4B为例,DyLaR将准确率从54.0%提升至58.2%,同时将响应长度从1220.7个token减少至18.5个token。消融实验验证了感知潜在变量锚定、推理理由监督和自适应路由各自的贡献。

Conclusion: DyLaR通过在潜在空间中进行自适应推理,实现了高效且准确的视频问答。该方法证明了视觉证据定位与潜在推理相结合的有效性,为多模态推理提供了一种更高效、更简洁的替代方案。
...
TL;DR: mmMind是一个将毫米波雷达与大型语言模型结合的系统，通过3D姿态作为训练监督学习雷达表示，实现人类行为 captioning 和时空问答，在真实世界数据集上优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理需要感知物理环境中的人类行为，毫米波雷达提供隐私友好且无接触的感知方式，但雷达观测难以与语言对齐，现有方法存在依赖合成数据或缺乏对身体结构运动显式监督的问题。

Method: 提出mmMind框架，包含三个阶段：1)时空雷达编码器使用同步3D姿态进行预训练，学习身体配置和运动动态；2)推理时移除姿态头，仅使用雷达输入；3)将学到的雷达表示与LLM对齐，实现行为描述和时空问答。同步3D姿态仅作为训练监督，不用于推理。

Result: 在mmMind-Bench数据集上（17.9小时录音，23名参与者，7个室内环境），在captioning、问答和 unseen-action generalization 任务上均优于现有雷达-语言基线方法。消融实验证实了姿态引导预训练的重要性。

Conclusion: 通过姿态引导的预训练方法，mmMind成功将毫米波雷达感知能力与语言模型结合，在多个任务上展现优异性能，为隐私友好的物理环境人类行为理解提供了新思路。
...
TL;DR: Radar4D-VLM是一个纯雷达感知的时序视觉语言模型，仅使用4D雷达点云数据进行自动驾驶场景理解，无需相机或LiDAR输入。该模型通过提取基于几何的目标提议并将雷达证据组织成对象、场景和运动 tokens的层次结构，再映射到冻结的语言 backbone 中进行推理。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶的视觉语言模型主要依赖相机和LiDAR，而4D雷达作为一种感知模态虽然对恶劣能见度具有鲁棒性且能直接测量径向速度，但尚未被充分探索。该研究旨在填补这一空白，建立纯雷达感知的 multimodal scene and motion reasoning 基础。

Method: Radar4D-VLM采用以下方法：(1)从连续10帧4D雷达点云中提取基于几何的目标提议；(2)将雷达证据组织成对象、场景和运动 tokens的紧凑层次结构；(3)使用参数高效的 projector 将这些 tokens 映射到冻结的语言 backbone；(4)通过可审计的预测头联合建模目标数量、空间分布、运动状态、碰撞风险、语义类别和径向速度。

Result: 在 sequence-isolated K-Radar 开发验证集上，Top-64 提议召回率达到 98.13%（4m阈值），比固定格网和均匀随机对照分别高 6.40 和 22.83 个百分点。对24个匹配运行的评估涵盖 Qwen、Phi、Mistral、Llama 和 Gemma 五个语言模型家族，结果显示 radar-token 接口在各模型家族间保持兼容，但语言监督对齐未显示稳定的直接头增益。

Conclusion: Radar4D-VLM 建立了纯雷达感知的多模态场景和运动推理的可复现基础，证明了4D雷达作为自动驾驶独立感知模态的可行性，同时将接口兼容性与语言监督的益处分离开来。该工作为雷达-only 的 vision-language model 奠定了方法论基础。
...
TL;DR: RUTA是一种原则性的视觉 token 压缩方法，通过学习为每个图像-查询对分配保留的 token 数量和哪些 token 应该保留，在 LLaVA-NeXT-7B 和 Qwen3-VL-8B 上仅使用 2.0% 和 4.2% 的视觉 token 就保留了 88.2% 和 94.4% 的任务性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率图像和长视频会产生很长的视觉 token 序列，导致大语言模型端的计算和内存成本高昂。现有视觉 token 压缩器通常以固定比率操作，而近期方法虽然能自适应 token 数量，但使用特定于方法的 learned thresholds 或 importance predictors，缺乏原则性框架。

Method: RUTA 采用查询条件候选 token 构建，为每个候选预测保留概率。训练时，这些概率参数化独立的 Bernoulli 门，其总和提供每对的可微分训练时间 token 数量估计。保留的 token 作为锚点，根据语义亲和力和空间邻近性聚合非保留 token 的信息。优化目标为惩罚的 rate-utility 目标，平衡下游任务损失与预期 token 使用量。

Result: 在五个基准测试上平均，与完整 token 基线相比，RUTA 在 LLaVA-NeXT-7B 上仅使用 2.0% 的视觉 token 保留 88.2% 任务性能，在 Qwen3-VL-8B 上使用 4.2% 保留 94.4% 性能。

Conclusion: RUTA 提供了一种原则性的端到端可学习框架，能够自适应地为不同输入分配不同数量的视觉 token，在大幅降低计算成本的同时保持较高的任务性能。
...
TL;DR: TRNet是一种用于山区和丘陵地区高分辨率水稻制图的双编码器深度学习网络，结合0.5米GaoJing-1 RGB影像、5米TanDEM-X DEM和坡度数据。通过 topographic energy-spectral rectification（地形能量-频谱校正）模块抑制陡坡杂波、增强兼容的低坡水稻特征。在Area A和Area B测试集上分别达到85.10%和80.68%的水稻IoU，比双编码器U-Net提升9.15和18.83个百分点。


<details>
  <summary>Details</summary>
Motivation: 在山区和丘陵地区进行水稻制图面临独特挑战：地形改变光学外观，增加与视觉上相似的植被的混淆。现有方法难以有效处理陡坡地区的复杂地形影响，导致误报率高。

Method: 提出TRNet架构，包含：(1) 分离的视觉和地形编码器保留模态特定特征；(2) 早期编码器阶段的topographic energy-spectral rectification（地形能量-频谱校正）模块，应用地形条件低频调制和非对称高频 regulation抑制陡坡杂波、有条件增强兼容的低坡水稻线索；(3) Topography-guided paddy structure decoder结合语义、水稻-背景边界和内部线索，使用粗地形作为上下文。

Result: 在Area A（内部测试集）和Area B（留置测试集，更陡峭地形和更低水稻覆盖率）上，TRNet分别达到85.10%和80.68%的水稻IoU，比原始Dual-Encoder U-Net分别提升9.15和18.83个百分点。消融实验和坡度分层结果将收益归因于频率校正、结构学习和减少陡坡误报。

Conclusion: 研究结果支持将粗地形作为高分辨率水稻制图的上下文先验。TESR模块的频率校正和结构学习机制有效解决了山区地形带来的挑战，显著降低了陡坡地区的误报率。
...
TL;DR: TriCLE是一个面向边缘计算的三模态视觉语言系统,仅从单张RGB飞机图像生成热成像和伪LiDAR深度图,结合任务指令通过紧凑的Qwen3-VL骨干网络实现飞机分类学分组。该系统在有限内存、计算资源和间歇连接条件下运行,解决了边缘平台上飞机识别困难的挑战。


<details>
  <summary>Details</summary>
Motivation: 边缘平台用于 aerial observation必须在有限内存、有限计算和间歇连接条件下解释飞机图像。标准RGB识别模型和通用视觉语言模型在此设置下难以处理,尤其是当校准的热成像和LiDAR数据不可用时。这促使研究者开发专门针对边缘约束优化的飞机分类系统。

Method: 1)从单张RGB图像生成结构保留的FLIR风格热视图和伪LiDAR深度投影
2)将对齐的多视图与任务指令融合到紧凑的Qwen3-VL骨干网络
3)模型与基于推进方式、机体家族、尺寸、设计时代和配置的专家飞机分类学对齐
4)评估多种训练策略:监督微调、旋转保留SFT、以及三种策略对齐方法(GRPO、GSPO、DAPO)
5)进行4-bit量化和注意力内存优化以适配边缘部署

Result: 序列级GSPO获得最佳验证性能:88.33%验证准确率和0.91加权F1。在保留的飞机测试集上,GSPO达到78.00%准确率和0.793加权F1,同时保持94.00%可解析输出格式化。经过4-bit量化和注意力优化后,4B模型可部署在8GB目标上,每 triplet处理时间为1.48秒。

Conclusion: TriCLE作为可解释、边缘可行的飞机分组实用原型获得了有力验证,但仍需在真实对齐的热成像和LiDAR传感器流上进一步验证。该研究展示了通过合成多模态数据和策略对齐训练,可以在边缘约束下实现可靠的飞机识别任务。
...
TL;DR: 本文提出PADFormer，一种基于Vision Transformer的图像空间方法，用于姿态无关的异常检测。该方法通过交叉视图掩码重建技术，在仅使用正常数据训练的条件下，从查询图像重建无异常版本，同时保留姿态信息。通过动态patch选择和空间对齐机制，能够在显著姿态变化下从稀疏参考视图中有效学习。推理时使用不同掩码模式进行多次前向传播，生成异常无重建的集成，实现全面的查询图像覆盖。


<details>
  <summary>Details</summary>
Motivation: 现有的姿态无关异常检测(PAD)方法依赖复杂的3D重建技术，存在计算成本高且需要大量多视角数据的局限性。该研究旨在提出一种无需3D重建的轻量级图像空间方法，能够处理任意视角下的异常检测问题。

Method: 提出PADFormer方法，核心创新包括：1)利用Vision Transformer直接重建查询图像的异常无关版本；2)采用交叉视图掩码重建策略，仅在正常数据上训练；3)引入动态patch选择和空间对齐机制，支持稀疏参考视图下的有效学习；4)推理时使用多种掩码模式生成集成重建结果。

Result: PADFormer在PAD基准测试上达到最先进水平，同时在经典少样本异常检测(FSAD)任务上保持相当性能，展现出优异的效率和泛化能力，且无需3D重建。

Conclusion: 该方法成功实现了无需3D重建的姿态无关异常检测，通过图像空间方法结合Transformer和集成重建策略，有效解决了显著姿态变化下的异常检测难题，在性能和效率上均优于现有方法。
...
TL;DR: OmniVR是首个联合音视频生成式修复模型，基于22B参数的音视频生成主干网络，将修复问题建模为统一多模态DiT中的条件生成任务。模型在200个真实历史片段上实现了全面的音视频质量提升。


<details>
  <summary>Details</summary>
Motivation: 历史电影同时存在视觉和音频退化（模糊、噪声、闪烁、嘶声、削波、掉线），但现有方法仅独立修复各模态，导致质量差距和跨模态不一致。需一个联合修复框架。

Method: 1）联合音视频退化pipeline，从互联网数据模拟真实老电影特征；2）架构保留的T2AV到AV2AV转换，配合prompt annealing保持生成先验；3）首帧I2V锚定配合loss重加权和波形监督，实现长视频外推和音频保真度。

Result: OmniVR在所有六个视觉指标上超越所有先前方法，获得最佳音频质量，并产生自然的着色——首个联合解决这三个方面的方法。

Conclusion: 首次实现音视频联合修复的统一框架OmniVR，在视觉质量、音频质量、时间一致性、音视频同步四个维度均取得最优表现，同时提出首个音视频修复基准OmniVRBench。
...
TL;DR: SIGNPOST-Bench是一个用于评估多模态大语言模型(MLLMs)在文本-视觉冲突解决能力的受控反事实基准测试。该基准通过创建图像的反事实五元组(Original、Blank、Similar、Random、Adversarial)来测试模型如何在冲突的视觉和文本证据之间进行仲裁。评估了来自7个提供商的20个MLLMs，发现对抗性变体将定位误差中位数从282公里提升到1347公里(增加4.8倍)。


<details>
  <summary>Details</summary>
Motivation: 现有基准很少揭示多模态大语言模型如何在冲突的视觉和文本证据之间进行仲裁，缺乏评估模型文本-视觉冲突解决能力的受控测试环境。

Method: 构建SIGNPOST-Bench基准，包含5,111个反事实组和25,555个图像变体(来自4个数据集)。创建原始图像的反事实五元组变体:Original、Blank、Similar、Random和Adversarial。设计合成的、局部化的场景文本干预，保持非文本内容不变，从而能够对定位性能和地理目标方向变化进行配对测量。

Result: 与原始图像相比，Adversarial变体将定位误差中位数从282公里提高到1,347公里，增加4.8倍。在可地理编码的对抗样本中，6.5-20.1%的预测位于注入目标50公里以内，每个评估的模型都表现出从Blank到Adversarial的正向平均配对目标距离减少。兼容、不相关和冲突的文本替换对模型预测产生不同影响，而干净输入的定位性能不能完全预测对冲突文本的鲁棒性。

Conclusion: 视觉地理定位可作为场景文本仲裁的连续诊断指标，并提供了评估MLLMs如何解决冲突多模态证据的受控框架。表明当前模型在处理冲突的多模态输入时存在显著问题，需要改进文本-视觉证据的仲裁机制。
...
TL;DR: 本文提出BIND框架，利用生物特征密码学思想，将人类用户的生物特征数据与AI代理身份和授权范围绑定，实现对AI代理的实时人类认证和授权委托证明。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理系统普及，需要对AI代理执行的关键任务进行授权和问责。生物特征识别是最可靠的身份验证方法之一，可用于实现对AI代理的授权认证。

Method: 1)提出BIND框架，将用户生物特征与代理ID和任务约束绑定；2)基于人脸特征实现实际系统，使用深度神经网络提取人脸特征；3)提出特征适配模块，将实值特征嵌入转换为固定长度的二进制表示，用于基于turbo纠错码的模糊承诺构造。

Result: 实现96%的真实匹配率（TMR）和零误匹配率（FMR），支持1024位代理令牌。

Conclusion: 所提出的基于人脸的密码系统具有实际可行性，能够实现AI代理的实时人类认证和授权委托的可否认证明。
...
TL;DR: 本文提出了CLIP-CC-Bench，一个用于长视频描述评估的基准测试框架。该基准包含5小时的电影内容片段（每个90秒），每个片段配有一个专家撰写的段落式参考描述。使用5个最先进的LLM嵌入模型进行评估，采用粗粒度和细粒度语义匹配两种方法，对17个视频语言模型进行了评估。


<details>
  <summary>Details</summary>
Motivation: 现有的视频-语言模型评估主要集中在短片段和单句指标上，缺乏对长形式、段落级描述生成能力的评估。作者认为需要一个新的评估框架来填补这一空白。

Method: 1. 构建包含5小时电影内容（90秒片段）的评估数据集，每个片段配有专家撰写的段落式描述
2. 使用5个最先进的LLM嵌入模型组成集成，以提高可靠性并减轻单一模型偏差
3. 采用两种互补的评估方法：粗粒度语义匹配和细粒度语义匹配
4. 通过Borda聚合排名和引导排名稳定性来量化协议的内部可靠性

Result: 对17个最先进的视频-语言模型进行了评估，报告了Borda聚合排名和平均分数，并验证了评估协议的内部可靠性（通过判断者间一致性和引导排名稳定性）。

Conclusion: CLIP-CC-Bench提供了一个实用的长视频描述评估框架，填补了现有短片段和仅QA基准的空白。该基准、标准化评估脚本、模型输出和聚合工具已开源以支持可重复性。
...
TL;DR: 本文介绍了Seam Carving（缝线雕刻）算法的C++实现，这是一种内容感知的图像缩放技术，通过重复移除或插入8连通的单调像素路径（seams）来改变图像尺寸，同时保留显著内容而丢弃低能量区域。


<details>
  <summary>Details</summary>
Motivation: 传统的图像缩放方法（如均匀缩放或裁剪）会扭曲或丢失重要内容，而Seam Carving算法能够绕过显著内容，仅在低能量区域进行操作，从而在保持图像整体结构的同时实现尺寸调整。

Method: 算法基于能量最小化原理，每次迭代计算图像的能量图，识别能量最低的单调路径（seam）进行移除或插入。实现支持前向能量准则（forward-energy criterion），可实现图像缩小、放大、多通道放大、用户自定义权重蒙版用于对象保护/移除，以及能量图导出和seam可视化。

Result: 该实现完整复现了Avidan和Shamir (2007)及Rubinstein、Shamir和Avidan (2008)提出的算法，支持多种功能扩展，包括大倍率放大和对象保护/移除功能。

Conclusion: 本文详细阐述了算法细节、参数设置、计算复杂度，并与原始描述进行了设计权衡讨论，展示了算法在自然图像上的实际效果。
...
TL;DR: 针对多模态学习中特征表示无效导致的冗余、分散和泛化问题，提出了一种基于列排列问题(CPP)原理的图增强描述符排序(GEDS)算法，并将其嵌入到顺序感知的高效Transformer框架中。实验表明该方法能有效减少特征分散，提升预测性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态学习（图像与表格数据）常因特征表示无效而受到损害，导致冗余、分散和泛化问题

Method: 提出图增强描述符排序(GEDS)算法，基于列排列问题(CPP)原理，通过相似图计算优化特征描述符，确定有效特征排序；并将其嵌入顺序感知的高效Transformer框架，使用顺序感知记忆令牌通过专用损失函数遵循特征排序

Result: 有效减少特征分散，提升预测性能和鲁棒性

Conclusion: 结构化特征排序对多模态学习具有重要意义
...
TL;DR: Poly-OPD是一个将多个异构教师模型的优势整合到单一紧凑流匹配学生模型中的框架。通过像素桥接进行on-policy蒸馏，使用冻结的DINOv2空间进行跨潜空间监督，并采用梯度兼容性诊断组织适配器。在将FLUX.1-dev和Z-Image蒸馏到2.5B SD3.5-Medium学生模型后，GenEval从67.3提升至73.3，DrawBench HPSv3从9.34提升至11.35。


<details>
  <summary>Details</summary>
Motivation: 当前领先的文本到图像模型各有优势：有的在美学偏好上表现更好，有的在遵循组合指令上更准确。但由于不同模型的自动编码器和噪声调度存在差异，难以将这些优势跨模型转移。现有方法难以有效整合异构教师模型的互补优势。

Method: 1. 像素桥接进行on-policy蒸馏：学生生成的图像由选定教师的编码器重新编码，并根据教师噪声调度在匹配噪声水平下进行细化
2. 冻结DINOv2空间监督：将目标与学生在冻结DINOv2空间中匹配，实现跨不兼容潜空间的监督
3. 梯度兼容性诊断组织适配器：注意力LoRA模块跨教师共享，前馈适配器保持教师特定
4. 差距感知课程：更多训练资源分配给学生仍落后的组合类别，随差距缩小转向更大差距的类别

Result: 将FLUX.1-dev和Z-Image蒸馏到2.5B SD3.5-Medium学生模型后：
- GenEval得分从67.3提升至73.3，超越两个更大的教师模型
- DrawBench HPSv3从9.34提升至11.35
成功在可切换模型中整合了双方优势

Conclusion: Poly-OPD成功实现了异构教师模型优势的有效整合，通过创新的像素桥接和跨潜空间监督方法，解决了不同模型间自动编码器和噪声调度不兼容的问题。实验证明该方法能够生成同时具备美学优势和组合指令遵循能力的紧凑模型，为多教师蒸馏提供了新思路。
...
TL;DR: 本文提出了一种优化CNN特征相关性的方法，用于增强量子神经网络(QNN)的图像分类性能。与之前使用正交分解预处理的方法不同，该研究故意引入相关性特征，以更好地利用QNN利用量子纠缠表示相关态的优势。蒙特卡洛模拟表明，特征平均相关性为0.5时达到最佳分类精度。在三个数据集上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 1. 之前的方法使用正交分解作为预处理，这可能不适合QNN 2. QNN具有利用量子纠缠表示相关态的内在能力，这是经典神经网络无法实现的 3. 假设将特征相关性与QNN的纠缠结构对齐可以提高二分类性能

Method: 1. 对QNN输出进行数学推导 2. 蒙特卡洛模拟确定最优相关性 3. 引入相关性正则化项驱动CNN输出的特征相关矩阵对角线条目趋向目标常数 4. 在三个任务上评估：CIFAR-10(汽车vs卡车)、Fashion-MNIST(衬衫vs外套)、雷达微多普勒签名(机器狗vs非机器狗)

Result: 1. 特征平均相关性为0.5时达到最优分类精度 2. 在所有数据集上，引入中间相关性比低相关性、高相关性或无正则化都能提高精度 3. 分类精度方差降低 4. 无需修改量子电路即可增强分类精度和稳定性

Conclusion: 通过将特征统计与QNN的纠缠结构对齐，在不修改量子电路的情况下增强分类准确性和稳定性。这项研究突出了QNN在更多量子比特可用时超越经典分类器性能的潜力。
...
TL;DR: 本文提出ReGround框架，解决视觉语言模型在多步推理中失去视觉 grounding 的问题。该框架通过两阶段方法教会VLMs自我诊断 grounding 失败并选择性重新查看视觉证据，无需架构修改或外部工具。通过能力引导，更强模型在数据构建时提供诊断支持，而策略模型在推理时学习自主诊断。实验在8个基准测试和2个VLM backbone上展示了一致性提升。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在多步推理中，随着推理链变长，后续推理步骤越来越依赖语言先验而非图像证据，导致视觉 grounding 退化。这种退化表现为注意力熵的变化模式：第1轮图像token注意力下降，图像重新注入后再次上升。

Method: 提出ReGround两阶段框架：(1)图像重新注入：向推理过程重新引入图像信息；(2)针对性自我诊断：模型学会诊断自身的 grounding 失败。关键发现是单独重新查看图像可能损害性能，需要配合准确的自我诊断才能带来收益。采用能力 bootstrapping：同模型族中更强的变体仅在数据构建时提供诊断支撑，策略模型在推理时学习自主诊断。

Result: 在8个基准测试和2个VLM backbone上展示一致性提升，尤其在视觉密集型多步推理任务上。诊断质量是关键因素：没有诊断的重新查看甚至会降低性能，而准确诊断可带来数个百分点的收益提升。

Conclusion: ReGround框架成功解决了VLM在多步推理中视觉 grounding 退化的问题，通过自我诊断和选择性视觉重看机制，无需外部工具或架构修改即可实现显著性能提升，为提升VLM推理可靠性提供了新思路。
...
TL;DR: 本文提出SITN方法，通过选择性修复和定制噪声来解决跨域少样本目标检测中的域差距问题，利用扩散模型合成训练数据，在6个CDFSOD数据集和4个CDFSS数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: CDFSOD任务存在显著的域差距和数据稀缺问题，传统扩散模型无法直接产生良好效果，需要分析域差距的具体组成部分并针对性解决。

Method: 将域差距分解为视觉差距和语义差距分别处理：1)视觉差距通过添加减弱的噪声解决；2)语义差距利用背景修复来桥接，因为背景语义差距小于前景语义差距。提出包含生成模块（添加定制噪声）和选择模块（动态选择修复区域）的SITN方法。

Result: 在6个CDFSOD数据集和4个CDFSS数据集上验证，实现新的最先进性能。

Conclusion: 通过分析域差距的不同组成部分并采用针对性的合成策略，可以有效利用扩散模型为数据稀缺的领域合成有用数据。
...
TL;DR: 本文提出CofactVLA框架，通过因果干预解决视觉-语言-动作(VLA)模型中的"视觉覆盖"问题。该问题表现为模型过度依赖视觉特征而忽视语言指令。核心方法是构建双路径去混淆图(DDG)，在单次前向传播中生成语言掩码的反事实分支，结合动作级正交投影引导(OPG)和特征级反事实协方差降低(CCR)两种机制消除视觉混淆因素。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人操作任务中取得进展，但存在严重的模态不平衡问题：密集的视觉流与稀疏的语言指令之间的不平衡导致模型产生因果混淆。策略模型不是将语言作为主要因果驱动因素，而是完全绕过原始指令，过度拟合于突出的物体或熟悉布局等伪视觉混淆因素。

Method: 提出双路径去混淆图(DDG)形式化动作生成过程，通过在单次前向传播中动态构建语言掩码的反事实分支实现因果干预。具体包含两个协同机制：(1)动作级正交投影引导(OPG)在连续流匹配中将事实速度场几何投影远离反事实视觉偏差，提取纯语义意图；(2)特征级反事实协方差降低(CCR)通过惩罚协方差差异的正特征空间来数学上去混淆潜在表示，显式抑制主导性视觉捷径同时保留因果语言意图。

Result: 在多样化模拟基准测试中建立新的最先进水平。真实机器人实验证明方法在弥合泛化差距方面的因果效果，在分布外场景下实现52.3%的绝对成功率提升。

Conclusion: CofactVLA通过因果干预方法成功解决了VLA模型中的视觉覆盖问题，显著提升了机器人操作任务在分布外场景下的泛化能力，为构建更可靠的视觉-语言-动作控制系统提供了有效方案。
...
TL;DR: 本文提出Faster-WAM，一种高效的future-conditioning World Action Model，在保持future representations的同时避免昂贵的video-action交互，通过稀疏future-conditioning框架、SparseMoT和Interval KV-Fusion等技术，在性能和效率之间取得更好的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有WAMs面临两难境地：Joint-WAMs保留future-aware representations但计算成本高昂，而高效替代方案在推理时移除future modeling可能失去时间推理的鲁棒性优势。本文发现推理时的future conditioning对分布迁移下的泛化至关重要。

Method: 提出稀疏future-conditioning框架：1) SparseMoT用选择性video-action交互替代逐层融合；2) Interval KV-Fusion聚合多深度future representations而不增加注意力复杂度；3) 计算future representations一次并在action去噪过程中选择性复用。

Result: 在LIBERO-Plus基准上，Faster-WAM成功率达73.57%，比Fast-WAM的49.14%显著提升；运行速度比Joint-WAM快2.21倍；在LIBERO和RoboTwin 2.0上达到SOTA；在真实世界操作中展现强鲁棒性。

Conclusion: Faster-WAM实现了比现有WAMs显著更好的性能-效率权衡，证明了推理时future conditioning对鲁棒性的关键作用。
...
TL;DR: muSync-GS是一个物理同步的驾驶视频合成框架，能够在 adverse weather 和 road-elevation 场景下生成逼真的驾驶数据，同时保持车辆动力学与场景编辑的同步。


<details>
  <summary>Details</summary>
Motivation: 高质量驾驶数据对自动驾驶系统和生成式世界模型至关重要，但涉及恶劣天气、低摩擦制动和不平道路等危险场景数据难以大规模采集。现有视频生成和3D Gaussian编辑方法仅能修改天气或道路几何，无法耦合轮胎-路面交互与车辆动力学。

Method: 提出muSync-GS框架，包含：(1) 降水驱动的路面状况，同时控制路面外观和轮胎摩擦系数；(2) 共享道路高程剖面，同步驱动可见道路几何编辑和车轴激励；(3) 校准车辆模型预测速度、滑移率、法向载荷和俯仰角，用于构建自车相机轨迹和物理标注。

Result: 在12个保留的CarSim测试场景中，平均RMSE为：速度0.0273 m/s、俯仰角0.0590°、滑移率0.0101、单轮法向载荷26.61 N。重建场景实验表明该方法能准确复现车辆响应，同时保持与可控场景编辑和自车相机运动的同步。

Conclusion: muSync-GS能够准确复现保留场景下的车辆响应，并将物理一致的车辆动力学与可控的场景编辑和自车运动同步，为自动驾驶训练数据生成提供了有效解决方案。
...
TL;DR: AmodalDINO是一种无需可见掩码的端到端模态分割模型，可从单张RGB图像预测完整叶片、主脉和细脉，用于古植物化石的完整形态重建。该模型在合成数据上训练后可直接迁移到真实化石标本，并可在浏览器端离线运行。


<details>
  <summary>Details</summary>
Motivation: 化石叶片很少完整保存——沉积岩会隐藏、破碎和侵蚀叶片组织，而古植物学依赖于叶片的完整形状和轮廓。现有方法通常需要可见掩码作为输入，缺乏端到端的完整叶片重建能力。

Method: 采用多头密集预测架构，从单张RGB图像联合预测四个掩码：可见叶片、模态完整叶片、主脉和细脉。两项关键改进：(1)完全微调DINOv3 ViT-L/16而非冻结；(2)在叶片头部旁添加辅助叶脉头部。

Result: 验证集达到95.0% Dice / 90.5% IoU，可迁移到真实化石标本。在KINS数据集达85.05全mIoU / 66.65遮挡mIoU，COCOA-cls达80.90 / 38.15。4-bit量化后可在浏览器离线运行，IoU达0.91。

Conclusion: AmodalDINO成功实现了端到端的叶片模态分割，无需上游实例分割器。通过完全微调和叶脉辅助头部的改进，模型学会了叶片结构先验，可应用于化石形态重建和基准数据集，并在边缘设备上具有实用价值。
...
TL;DR: ADOPD 2026是一个面向推理的文档理解基准测试，将页面分解扩展为空间锚定的文档理解。该基准通过整合人类清理的标题、语义标签和链式思考（CoT）痕迹来丰富页面锚点，支持区域级语义标记、统一视觉-语言锚定和密集计数任务，揭示了当前最先进模型在文档语义理解中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准测试主要关注定位页面元素，但现实世界的文档智能需要模型对区域语义、空间关系和视觉结构进行联合推理。标准布局基准测试往往隐藏了长尾语义失败问题，需要更全面的推理能力评估。

Method: 将文本块、视觉实体、语义标签、边界框和多边形掩码统一视为共享的视觉锚点词汇表。实现了三个核心能力：1）区域级语义标记；2）统一视觉-语言锚定；3）基于DocCount基准的密集计数任务评估。

Result: 1）揭示了标准布局基准测试常隐藏的长尾语义失败问题；2）当前最先进的模型在DocCount基准的密集计数任务上仍表现困难；3）提出了Thinking-with-Anchors管道以提升文档语义理解能力。

Conclusion: ADOPD 2026通过将页面分解与可验证的视觉锚点推理相连接，提供了超越定位的锚定文档智能任务框架，为文档理解领域从 localization 走向 anchor-grounded intelligence 提供了新的评估范式。
...
TL;DR: 本文提出预测状态检索(PSR)任务：模型观察短视频前缀和关于物体未来状态的时间问题，然后从其他视频或图像中检索展示该状态的实例。构建了来自4个数据集的基准测试，并提出轻量级检索器LFTR。研究发现预测而非感知是核心挑战。


<details>
  <summary>Details</summary>
Motivation: 现有任务存在局限：动作 anticipation 预测标签，时序检索定位事件，视频生成合成像素。PSR 结合了 anticipation 与跨实例检索，需要处理多个时间视野。

Method: 提出 LFTR 方法：使用冻结编码器的轻量级检索器，预测问题与时间范围条件下的未来潜在表示，并在互补的语义和视觉空间中进行匹配。采用跨空间融合和硬负样本训练。

Result: oracle 分解揭示明确瓶颈：一旦具体指定，真正未来状态高度可检索，但所有预测器（包括访问前缀帧的大型多模态语言模型）都远低于 oracle。LFTR 以更低推理成本缩小了这一差距。

Conclusion: 预测（forecasting）而非感知（perception）是核心可学习挑战。跨空间融合和硬负样本训练是性能提升的关键因素，潜在 rollout 贡献有限。发布基准测试、代码和评估脚本。
...
TL;DR: 本文提出了一种统一的无监督增强框架，利用CLIP引导的提示学习来改善背光和低光图像的曝光问题，无需依赖成对的地面真值数据。


<details>
  <summary>Details</summary>
Motivation: 背光和低光图像经常遭受严重的曝光不平衡或全局曝光不足，这对视觉感知和下游计算机视觉任务都构成重大挑战。

Method: 1. 基于CLIP引导的提示学习，使用学习到的正向和负向文本提示进行语义监督<br>2. 对称残差U-Net骨干网络，集成Atrous Spatial Pyramid Pooling模块<br>3. CLIP-based语义相似度损失函数<br>4. 迭代提示优化机制

Result: 在BAID、Backlit300、LOL和VE-LOL-L等配对和未配对数据集上的广泛实验表明，该框架在保真度、感知质量和泛化能力方面始终优于最先进的监督和无监督方法。

Conclusion: 该框架为不同光照条件下的真实世界照明增强提供了稳健、可扩展的解决方案，同时强调了背光增强领域需要更强基准测试协议的必要性。
...
TL;DR: OmniRouting是首个用于评估大语言模型在PCB布线推理方面能力的大规模基准测试，包含1,681个工业级PCB设计，涵盖四个任务：几何路由推理、设计规则感知路由推理、电气功能推理和工具增强的智能体路由。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在约束感知导航、迷宫推理和图推理方面取得了显著进展，但其 reasoning about complex routing problems under strict geometric, topological, and electrical constraints 的能力仍未被探索，而布线是电子设计自动化中最具挑战性和关键性的阶段之一。

Method: 构建包含板几何形状、可布线元件布局、焊盘位置、网络列表、叠层信息和布线约束的工业级PCB设计基准；设计四个评估任务分别测试几何路由、设计规则遵守、电气功能保留和工具辅助能力。

Result: 当前大语言模型在PCB路由方面存在显著局限性，包括路径规划能力薄弱、设计规则约束遵守性差、电气功能保留不一致等问题。

Conclusion: OmniRouting基准测试揭示了现有LLM在PCB路由领域的重大不足，为该领域未来研究提供了重要的评估基础和方向。
...
TL;DR: ToolArtist是一个完全代理化的图像生成模型，通过对统一多模态模型(UMM)进行后训练，动态协调推理、外部工具使用和原生图像生成能力。该模型在开放式世界图像生成任务中表现优于固定流程或部分代理控制的方案。


<details>
  <summary>Details</summary>
Motivation: Text-to-image模型在需要复杂语义理解、多步推理和整合外部世界知识的开放世界任务中仍然受限。现有工作虽引入代理能力，但要么采用固定工作流，要么仅对开放世界图像生成过程的部分环节进行代理控制，导致推理、工具调用和图像生成无法由单一策略协调。

Method: 1)监督微调(SFT)阶段：配备搜索工具和图像生成工具的教师代理收集轨迹，转换为UMM兼容格式，隐藏图像生成工具但保留生成图像；2)强化学习(RL)阶段：开发适用于UMM的代理化RL基础设施，提出Reason-Act-Draw GRPO (RAD-GRPO)，使用互补的意图奖励和质量奖励联合优化模型。

Result: 将整个开放世界图像生成过程置于代理策略下 consistently outperforms 固定流程或仅部分代理控制的方案。实验表明该方法在复杂语义理解和多步推理任务上具有优势。

Conclusion: ToolArtist通过统一策略协调推理、工具调用和图像生成，为开放世界图像生成提供了一种有效的完全代理化范式，显著提升了模型的复杂任务处理能力。
...
TL;DR: 提出Season框架，针对异构模型间的对抗迁移攻击难题。该方法将梯度更新分解为低频（结构）和高频（纹理）分支，通过低显著性引导重新分配高频能量，并通过正交投影消除特征干扰。作为即插即用的训练-free包装器，Season在8个CNN、ViT和MLP目标模型上平均提升6.6个百分点转移成功率，最高可达16个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前基于迁移的对抗攻击在异构架构间转移效果较差，原因在于CNN偏好局部纹理而ViT依赖全局形状。这种架构差异导致针对白盒surrogate模型生成的对抗样本难以有效攻击黑盒目标模型。

Method: Season采用频谱感知正交梯度细化框架：1）将每次更新分解为低频分支（捕获结构线索）和高频分支（捕获纹理）；2）低显著性引导 scheme将高频能量重新分配到背景区域，保留ViT依赖的前景结构；3）正交投影强制纹理更新位于结构方向的正交补集中，缓解特征干扰。

Result: 在8个CNN、ViT和MLP目标模型上，Season在统一协议下相比强基线平均提升转移成功率6.6个百分点，最高提升16.0个百分点。

Conclusion: Season通过分离结构与纹理更新成分，有效增强了异构架构间的对抗迁移攻击能力。该方法作为训练-free即插即用包装器，可增强8种梯度稳定化和输入增强攻击，具有良好的通用性和实用价值。
...
TL;DR: 本研究分析扩散Transformer在降分辨率训练时梯度信号的变化，识别出两个影响项：噪声依赖项和σ独立底项。通过实验发现1024->768路线在0.65<σ<0.95噪声窗口下，降尺度梯度与原生梯度非常接近。在此窗口内训练LoRA适配器可在保持接近原生性能的同时减少14.6%训练时间。


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer图像生成效果出色，但训练成本随分辨率超线性增长。现有工作仅从频谱角度证明高噪声下降尺度可保留信号，但未验证降尺度步骤是否保留原生训练梯度信号。

Method: 将降尺度时信号变化分解为两个项：1)由降噪比决定的噪声依赖项，高噪声下衰减；2)由目标网格绝对token数决定的σ独立底项，存在于计算图本身，任何噪声水平都无法消除。通过(route, σ)映射进行实证验证，并在验证的路线和噪声窗口内训练LoRA适配器。

Result: 实验证实了理论分析。在1024->768路线上发现0.65<σ<0.95的窗口，在此窗口内降尺度梯度保持在原生梯度的很小范围内。使用该方法训练LoRA适配器在固定步数预算下减少14.6%训练时间，同时在权重空间保持接近原生性能。

Conclusion: 通过(route, σ)映射验证的特定路线和噪声窗口内使用降尺度步骤进行训练，可显著提高训练效率，在质量损失最小的情况下大幅减少训练时间。这为扩散Transformer的高效训练提供了新途径。
...
TL;DR: Q-CueGraph是一种用于多模态大语言模型的视觉定位策略，它通过问题条件下的坐标级观察选择机制，使模型能够有针对性地检查图像局部区域，而非简单依赖全局图像输入。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型虽然具备高分辨率像素和裁剪/缩放工具，但缺乏一种可靠的、基于任务的策略来决定应该检查图像的哪个位置。

Method: Q-CueGraph将问题和图像表示映射到预算化的坐标级观察。针对文本丰富的图像使用可重用的OCR/布局图；自然图像搜索则在相同的选择、组合和预算界面下实例化查询条件的视觉节点。可选的utility refinement通过训练答案正确性来学习哪些候选裁剪可以被冻结的reader使用，无需区域框监督。

Result: 使用冻结的Qwen2.5-VL-7B reader，Q-CueGraph在V*Bench上达到0.833准确率（相比全图像推理的0.696），仅使用19%的图像面积预算；在InfographicVQA上以约一半的图像面积达到92%的全图像ANLS。

Conclusion: 显式观察策略最有价值的场景是：证据可定位、问题能辨别其位置、且分辨率限制了全图像阅读。
...
TL;DR: TwinIR是一种针对自动驾驶在线高清地图构建的新型物理攻击方法，通过近红外照明技术和最小攻击点优化，在保持隐蔽性的同时有效降低地图构建精度(mAP下降约8-9个百分点)，并显著增加规划模块的不可达目标率和不安全轨迹率。


<details>
  <summary>Details</summary>
Motivation: 现有对在线地图构建的物理攻击受限于跨边界补偿效应：当目标边界被干扰后，其他可见边界可能保留足够的几何线索供模型恢复原始道路几何。因此需要找到一种方法能够抑制这种补偿效应。

Method: TwinIR采用机制引导的物理攻击方法，联合优化攻击有效性和点稀疏性，寻求最小攻击点数以抑制周围边界的补偿几何线索。为降低多点攻击的可感知性，模型化了相机对近红外照明的响应，将优化攻击点映射到可行的物理位置，产生最小的可见光谱变化。

Result: 在nuScenes数据集上，TwinIR在RSA攻击下使mAP降低8.18-8.96个百分点，ETA攻击下降低2.84-5.62个百分点；不可达目标率增加25-28个百分点，不安全规划轨迹率增加19-20个百分点。真实世界测试中成功诱导道路直线化和早期转弯变形，同时在全彩色视图中保持隐蔽。

Conclusion: TwinIR成功实现了对在线地图构建的有效物理攻击，在nuScenes数据集上显著降低地图精度(mAP下降8.18-8.96个百分点)，同时增加不可达目标率(25-28个百分点)和不安全轨迹率(19-20个百分点)。真实世界测试验证了攻击的有效性和隐蔽性。
...
TL;DR: 针对MoE-VLMs的专家合并问题，提出RoleMerge方法，通过构建相位归一化的路由角色画像(RRP)来保留相位条件下的专家角色，相比全局路由聚合方法，在6任务宏观平均性能上提升最高9.6%。


<details>
  <summary>Details</summary>
Motivation: MoE-VLMs部署需存储完整专家池，现有训练-free专家合并方法使用全局路由统计，但图像上下文token远多于其他类型，导致全局聚合过度强调图像处理，掩盖相位条件下的专家角色差异，影响模型性能。

Method: 提出RoleMerge方法：1)构建每个专家的路由角色画像(RRP)，从相位归一化路由统计中提取相对相位偏好；2)基于专家-相位信息损失，合并具有兼容画像的专家及其路由条目；3)保留答案解码专家的区分。

Result: 在三个模型和多个基准测试上，RoleMerge在相同专家保留比率下比替代方法保留更多完整模型性能，六任务宏观平均性能相对提升最高9.6%。

Conclusion: 相位条件下的专家角色比全局路由聚合更适合作为MoE-VLMs专家合并的依据，验证了相位感知方法的有效性。
...
TL;DR: REZE是一种无需训练的零样本视频时序定位方法，通过将视频分割为短片段，获取片段级置信度分数，并使用确定性算法转换为输出结果。


<details>
  <summary>Details</summary>
Motivation: 现有零样本策略依赖大型视觉语言模型直接生成起止时间戳，结果高度依赖模型设计和训练，不同VLM间定位精度差异大。

Method: 将视频分割为短片段，让模型对每个片段给出查询相关的置信度分数，然后使用确定性算法将分数曲线转换为任务所需的输出。支持单区间和多区间moment检索以及高光检测等多种任务。

Result: 在QVHighlights上，零样本moment检索mAP从38.23提升至40.32；高光检测达44.18 mAP和73.41 HIT@1。在7个来自3个模型家族的后端上测试，在Charades-STA和QVHighlights上均优于直接时间戳生成。较早一代模型通过REZE可接近新一代模型的原生性能。

Conclusion: REZE在训练-free方法中建立新的最先进水平，其HIT@1甚至超越QVHighlights测试集上所有全监督方法。该方法将时间聚合置于模型外部，使其能适应不同任务输出。
...
TL;DR: 本文研究视觉语言模型中视觉token剪枝与token功能角色之间的关系。通过分析EmbedLens识别的token角色，发现现有剪枝方法存在特定的角色偏见，但这些偏见与下游性能并不直接相关。研究表明保留非活跃token有时可以维持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型推理过程中的计算瓶颈问题。现有的token剪枝方法虽然能移除冗余token，但不清楚剪枝决策与token功能角色之间的关系。

Method: 1)通过EmbedLens识别token角色，分析代表性剪枝方法的token角色偏见；2)改进token角色分配程序；3)评估角色保护剪枝变体。

Result: 代表性剪枝方法表现出不同的token角色偏见，但这些偏见与下游性能不直接相关。保留非活跃token有时可维持或提升性能。

Conclusion: 具有弱直接语义对齐的token可能仍会影响剪枝下的模型行为。这表明在视觉token剪枝中，token的角色比传统的"冗余"概念更为复杂。
...
TL;DR: DIVE是一个无需训练的视觉token剪枝框架，通过动态迭代方式构建视觉证据保留集，解决了现有一次性评分方法忽视已保留证据的问题，在视觉token减少88.9%的情况下仍能保留98.2%的模型性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中视觉token数量远多于文本，导致推理效率低下。现有一 次性评分方法不足是因为token的prompt相关性取决于已保留的证据，需要动态更新评估。

Method: 提出动态迭代视觉证据构建框架DIVE，重复选择残余条件分数最高的token，更新视觉和prompt残余以折扣已解释的证据，重新评估剩余token，保留互补且与prompt相关的证据集。

Result: 在八个图像理解基准测试中，DIVE在不同token预算下均能保持性能。视觉token减少88.9%时，仍保留未压缩模型平均性能的98.2%。

Conclusion: DIVE通过动态迭代方式有效解决了视觉token剪枝问题，无需训练即可实现高效推理，性能损失极小。
...
TL;DR: 这是一篇关于隐私保护动作识别(PPAR)的系统性综述论文，涵盖2018-2026年间的32篇同行评审论文，将方法分为五类：对抗学习(52%)、基于骨骼(20%)、密码学(12%)、差分隐私(8%)和混合方法(8%)。论文分析了评估方法的不足，提出 PPAR 统一评估协议和路线图。


<details>
  <summary>Details</summary>
Motivation: 现有PPAR调查存在局限性：仅涵盖单一机制家族、早于对抗学习和混合方法、未充分探讨评估标准。导致文献碎片化、威胁模型不兼容、指标不一致、缺乏共享评估标准。

Method: 采用PRISMA指南的系统性综述，从885篇筛选论文中纳入32篇同行评审论文。提出二维隐私空间分类法、形式化威胁模型、比较权衡分析和PPAR统一评估协议。

Result: 方法评估：骨骼方法达85%准确率但丢失外观信息；对抗方法在cMAP 0.9至0.3-0.5区间保持约80%效用；差分隐私通常低于70%。评估缺陷：仅10%采用形式化隐私定义，65%依赖临时指标，40%报告定义不一致的cMAP。困难条件测试不足：<15%检查跨数据集泛化，<10%测试自适应攻击，实时边缘部署几乎未触及。

Conclusion:  PPAR可从原型推进到部署阶段，经验可扩展到人脸识别和医学影像。建议通过基准标准化推动领域发展。
...
TL;DR: 本文提出了上下文变量高估（CVE）问题，即视觉语言模型倾向于过度重视主导性视觉-文本线索，而忽视稀疏但关键的决策上下文变量。作者构建了一个包含真实广告创意和多国点击率数据的多模态数据集，并提出了GeoReward奖励模型，通过三种机制（市场感知检索增强、上下文引导的视觉调制、选择性敏感度损失）来预测不同地理市场的广告偏好。实验表明该框架能有效缓解CVE问题并优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在许多多模态任务中表现出色，但存在一个微妙但重要的失败模式：在预测不同地理市场的广告图像偏好时，模型常常忽略真实的区域差异，输出趋于一致。这是因为高_volume信号（如产品属性和密集图像 patches）淹没了编码市场特定上下文的关键稀疏标记。

Method: 1）构建真实广告创意及其多国点击率表现的多模态数据集；2）提出GeoReward奖励模型，包含三个专用机制：市场感知检索增强、上下文引导的视觉调制、选择性敏感度损失；3）利用GeoReward指导VLM的RL微调，生成市场感知的广告创意。

Result: 框架有效缓解了CVE问题，在广告偏好预测任务中优于现有基线方法。

Conclusion: 本研究不仅诊断了VLM对主导感知特征的系统性偏见，还为稀疏上下文变量决定决策的应用场景提供了针对性解决方案。
...
TL;DR: CARVE是一种无需训练的视觉token压缩框架，专门用于3D医学Volume的多模态大语言模型(MLLM)，通过预算约束的2.5D分配策略，在减少约80% token的情况下保持98.1%的全token性能。


<details>
  <summary>Details</summary>
Motivation: 当前slice-based MLLM将3D体数据表示为大量2D切片序列，产生冗余的视觉token（相邻切片间存在重复内容），导致计算成本高但准确率饱和。现有的token压缩方法针对2D图像或视频设计，不适用于3D医学影像的深度轴冗余问题。

Method: 将深度轴划分为一致窗口，根据归一化跨切片证据进行非均匀token分配；在共享预算下，在代表性切片上构建空间锚点，从完整volume中检索局部变化证据，然后将剩余符合条件的token合并到每个窗口内的附近锚点中。

Result: 在Hulu-Med-7B上移除约80%视觉token后，CARVE在所有AMOS-MM报告生成指标上超越所有压缩基线，比最强基线高出6.2点的全token质量保留率；在三个VQA基准上保持98.1%的全token性能。

Conclusion: CARVE通过创新的2.5D token分配策略，有效解决了3D医学影像的视觉token冗余问题，在大幅压缩token的同时保持了高性能，为资源受限场景下的3D医学MLLM提供了高效解决方案。
...
TL;DR: DualTSR是一个统一的场景文本图像超分辨率框架，将STISR建模为耦合的连续-离散生成任务。条件流匹配恢复连续图像latent，吸收态离散扩散重建文本token，两者共享多模态transformer骨干网络，在生成过程中图像和文本状态可交互，无需外部OCR先验。


<details>
  <summary>Details</summary>
Motivation: 现有STISR系统依赖外部生成的先验或分离的图像和文本模型，导致错误传播和昂贵的多阶段推理过程。现有方法在推理时需要外部OCR提供先验，增加了系统复杂度和错误累积。

Method: 提出DualTSR统一框架：1)条件流匹配(Conditional Flow Matching)恢复连续图像latent；2)吸收态离散扩散(Absorbing-state Discrete Diffusion)重建文本token；3)两者共享多模态transformer骨干网络；4)图像和文本状态在整个生成过程中相互交互；5)推理时无需外部OCR先验。

Result: 在CTR-TSR数据集上，DualTSR在X2和X4倍率下均达到最佳FID、LPIPS、ACC和NED；在RealCE子集上获得最佳FID、ACC和NED，LPIPS具有竞争力。与DiffTSR X4相比，ACC提升12.78个百分点，参数从1.23B降至203M，延迟从13.3s降至132ms。

Conclusion: DualTSR作为STISR的准确且高效的方法，通过统一框架有效解决了现有方法中错误传播和效率低下的问题，显著提升了性能同时大幅降低了模型参数量和推理延迟。
...
TL;DR: FocusMem是一种用于GUI代理的潜在记忆框架，通过将多模态轨迹压缩为连续标记来同时保留可重用的经验（情景记忆）和任务进度（工作记忆）。该方法包含角色感知内容basis、状态条件读出和轻量级信任门三个组件，可在GUI策略冻结的情况下训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个实际问题：1）压缩过程中重要细节可能丢失；2）同一记忆块需要服务不同决策阶段；3）不相关的检索轨迹可能误导代理。现有方法将每个轨迹映射到一个固定记忆块，主要通过下一个动作监督进行训练。

Method: 提出FocusMem框架，包含三个核心组件：1）角色感知内容basis鼓励情景记忆保留可重用经验，工作记忆保留任务进度；2）状态条件读出生成决策特定的同一存储证据视图；3）轻量级信任门可抑制与当前步骤无关的记忆块。所有组件在GUI策略冻结时训练。

Result: 在五个GUI代理基准测试中，FocusMem始终优于完全匹配的动作-only固定记忆基线和先前潜在记忆适配方法。进一步分析表明：语义和功能监督保留互补信息；状态条件读出在周围轨迹上下文增长时更鲁棒；信任门减少注入无关情景证据造成的伤害。

Conclusion: 有效潜在记忆不仅取决于压缩过去交互，还取决于保留什么、暴露什么以及允许什么。FocusMem通过分离情景记忆和工作记忆的职责，结合状态条件读出和信任门机制，实现了更有效的潜在记忆管理。
...
TL;DR: EgoAfford是一个用于研究多步骤桌面任务中感知与规划联合学习的新基准测试。该工作提出将物体功能区域分割与任务步骤推理相结合，要求模型根据自我视角观察和高级任务描述，生成剩余计划并分割下一个动作的功能区域（直接对象、工具和目标）。同时提出EgoLens，一个30亿参数的多模态大语言模型，包含角色特定掩码解码器。


<details>
  <summary>Details</summary>
Motivation: 当前部分级 affordance 定位已能识别与基本动作相关的功能区域，但扩展到复杂任务需要将参与物体的语义角色与任务状态对齐的视觉观察和多步骤规划连接起来。现有的基准测试缺乏将感知（功能区域分割）与规划（步骤推理）联合研究的数据和框架。

Method: 构建了EgoAfford基准测试，包含约15.5k张人类验证的图像，来自2000个生成的多步骤场景，以及EgoAfford-Real（102张真实捕获图像，涵盖26个任务）。提出EgoLens，一个30亿参数的多模态大语言模型，配备角色特定掩码解码器，用于联合执行下一步推理和动作角色条件化的部分 grounding。

Result: 对近期referring-segmentation MLLMs、commercial VLM-SAM2管道和EgoLens进行评估，突出了下一步推理和动作角色条件化部分grounding的互补挑战。EgoLens在生成和手动捕获的观察上都建立了强有力的参考性能。

Conclusion: EgoAfford和EgoLens为多步骤桌面任务中感知与规划的联合研究提供了基础。该工作填补了将功能区域分割与任务步骤推理相结合的空白，为复杂任务中的 affordance grounding 开辟了新途径。
...
TL;DR: VoxStruct3D是一种用于高保真3D MRI体积合成的 voxel 空间流匹配框架，通过 Volumetric Voxel Generator (VVG) 直接建模全分辨率 MRI 体积，并结合 Structure-First, Image-Follows (SFIF) 策略引入解剖先验，在病理和健康 T1 加权脑 MRI 数据集上实现了最佳的总体性能。


<details>
  <summary>Details</summary>
Motivation: 高保真3D MRI合成需要全局一致的解剖结构和细粒度的体素级细节。虽然潜在扩散使体积生成变得可行，但其图像自编码器引入了重建瓶颈，限制了最终体积中可恢复的细粒度细节。

Method: 1) VVG结合因子化3D块嵌入、重叠上采样、时间调制残差 refinement 和跳跃融合，使相邻token联合重建共享体素区域并抑制块边界伪影；2) SFIF策略使用冻结的预训练3D医学编码器和StructVAE提取保留主要解剖结构的紧凑结构token，结构领先调度使其轨迹领先于图像轨迹，Patch-Aligned RoPE对齐不均匀token网格，非对称注意力强制结构到图像的单向引导。

Result: 在病理和健康T1加权脑MRI数据集上的实验表明，VoxStruct3D在特征分布对齐、样本多样性和感知质量方面实现了最强的总体性能，产生了解剖结构连贯且视觉逼真的体积。

Conclusion: VoxStruct3D通过直接在voxel空间建模结合结构先验策略，成功解决了高分辨率医学体积合成中细节丢失和重建瓶颈的问题，为3D医学影像生成提供了新的有效框架。
...
TL;DR: 本文提出ColorFD，一种基于纯色贴片的黑盒物理攻击方法，用于攻击遥感目标检测器。该方法通过差分进化算法联合优化贴片位置和颜色参数，针对飞机检测任务进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注数字域或白盒设置，而黑盒物理攻击研究不足。物理攻击受到物理可行性限制和高维搜索空间优化效率低下的挑战。

Method: 提出ColorFD方法，包含：1)差分进化(DE)联合优化贴片位置和颜色参数；2)目标-wise适应度选择机制评估攻击状态；3)关键区域定位通过有限差分颜色探测识别敏感区域；4)常见特征提取提供类别级空间先验。测试于YOLOv3u、YOLOv5u和Faster R-CNN。

Result: ColorFD在所有评估的检测器上优于测试的黑盒贴片方法，并与强白盒基线方法具有竞争力。物理世界实验表明优化后的纯色贴片可从数字域迁移到真实成像条件。

Conclusion: ColorFD是一种有效的黑盒物理攻击方法，能够成功从数字域迁移到真实成像条件，为遥感目标检测器的对抗鲁棒性研究提供了新思路。
...
TL;DR: OutLangSplat是一种将开放词汇语言特征嵌入3D Gaussian Splatting的方法，专门针对无人机户外场景的3D场景理解。通过设计2D-3D双分支表示和训练无关的高斯特征聚合策略，解决了户外大规模场景中的语义激活错误和目标缺失问题，并发布了首个无人机户外开放词汇3D场景理解数据集。


<details>
  <summary>Details</summary>
Motivation: 现有3D语言高斯 splatting 方法仅适用于室内或小规模场景，在无人机户外场景中因严重遮挡和远距离视角导致语义激活错误和目标响应缺失。

Method: 1. 2D-3D双分支表示：基于区域的 alignment 和 fusion，提高空间一致性，减少不完整目标响应和背景误激活；2. 训练无关的贡献和一致性感知高斯特征聚合：利用像素贡献可靠性和跨视图语义一致性抑制噪声视角的不可靠响应；3. 构建了标注完善的无人机户外场景数据集。

Result: 在开放词汇语义分割和实例定位任务上均优于现有最先进方法。

Conclusion: OutLangSplat成功将语言高斯表示适配到无人机户外场景，为大规模户外3D场景的开放词汇理解提供了有效解决方案，首个公开数据集将推动该领域进一步发展。
...
TL;DR: 本文提出Talk2Sensors，首个基于相机、LiDAR和4D雷达的多传感器3D视觉定位数据集，以及TSFormer统一Transformer框架，实现语言引导的自动驾驶3D视觉定位。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位研究主要集中在室内场景或单目图像，缺乏对户外多传感器融合的实际感知能力补充。户外真实场景需要整合视觉纹理、3D几何和物体运动等多模态物理属性来实现灵活稳健的查询自适应定位。

Method: 提出TSFormer框架，采用粗到细的属性感知融合策略：1) 语言路由属性采样器通过查询级语言线索调节传感器采样权重进行粗粒度文本条件特征检索；2) 稀疏保留模态仲裁器进行细粒度模态仲裁和文本引导 refinement，确定精确的空间位置。

Result: TSFormer在Talk2Sensors上比最强基线提升8.05 mAP，在Mono3DRefer单目基准上达到53.05% Acc@0.5。

Conclusion: 该工作首次构建了多传感器3D视觉定位数据集，提出动态路由外观、几何和运动线索的统一框架，为自动驾驶场景的语言引导3D感知提供了新范式。
...
TL;DR: PhysMind是一个无需训练的智能体框架，通过为每个视频构建可重用的、问题无关的可执行世界模型来实现可靠的物理推理。该方法通过对象分割、网格重建和6D姿态跟踪恢复时空一致的动态场景，然后拟合分析连续时间动态和潜在物理参数。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLM）在解释物理动态和可靠地推理未来及反事实结果方面存在困难，需要一种能够理解物体运动、交互和对干预响应的方法。

Method: PhysMind包含三个关键步骤：1）对象分割、网格重建和6D姿态跟踪以恢复时空一致的动态场景；2）拟合分析连续时间动态和潜在物理参数，无需展开时间步进模拟器；3）给定问题时，检查、继续或编辑世界，并从生成的轨迹和交互中获取答案。

Result: 相比使用相同VLM的直接思维链（CoT）推理，PhysMind在CLEVRER上准确率提升38.23分，在Physion++上提升8.08分。在反事实问题上，它超越最强的VLM基线GPT-5.5达19.25分。

Conclusion: PhysMind通过构建可执行的世界模型显著提升了物理推理能力，特别是在反事实推理方面表现出色，证明无需训练的智能体框架在物理理解任务中的有效性。
...
TL;DR: 本文提出了自适应容量锚点框架用于4D高斯溅射(4DGS)，通过自适应锚点基数和自适应锚点特征掩码技术，根据局部时空需求动态分配表示容量，在复杂运动区域集中原语，抑制冗余。实验表明该方法可在MPEG、Panoptic Sports和N3DV数据集上实现显著存储压缩，在复杂运动序列上达到1.5倍于现有先进锚点方法的压缩率，同时保持相当的质量。


<details>
  <summary>Details</summary>
Motivation: 4D高斯溅射技术虽能实现高保真实时时空渲染，但存在运动表达性与存储效率之间的根本权衡。现有的锚点设计通过锚点级参数共享实现紧凑性，但其刚性均匀参数化强制固定的神经高斯数量和特征预算，导致通过过度增加锚点密度而非轻量级、针对性增加神经高斯数量或特征容量来提高保真度，造成内存浪费。

Method: 提出两种核心方法：1)自适应锚点基数(Adaptive Anchor Cardinality)：根据局部几何或运动复杂度动态调整每个锚点的神经高斯数量，在高复杂度区域集中原语，抑制冗余；2)自适应锚点特征掩码(Adaptive Anchor Feature Masking)：调制锚点级特征通道，为复杂区域分配丰富特征，为简单区域分配轻量表示。

Result: 在MPEG、Panoptic Sports和N3DV数据集上的实验表明，该方法可在不降低视觉质量的前提下实现显著存储压缩。在具有复杂运动的挑战性MPEG序列上，相较于现有最先进的锚点方法，实现了高达1.5倍的压缩率，同时保持相当的质量。

Conclusion: 自适应容量锚点框架成功克服了固定参数预算的刚性，能够根据局部时空需求动态分配表示容量，在保持高视觉质量的同时实现高效的内存使用，为4DGS中的运动表达性与存储效率权衡提供了有效解决方案。
...
TL;DR: MetaVideoAgent是一个自动进化视频智能体的框架，针对长视频理解任务，通过分析信息密度和证据需求来指导初始设计，将失败案例压缩为可独立验证的最小任务，构建证据锚定的Gold Paths，审计Student轨迹，聚合失败案例并归因于负责模块，实现模块化更新。在VA-EvoBench上经过4次迭代后，准确率从38.44%提升至51.47%。


<details>
  <summary>Details</summary>
Motivation: 现实世界视频分布差异大，模态特定信息密度、内容结构和证据模式不同，固定视频智能体设计会导致冗余处理或失败。从文本到视频的自动化智能体扩展具有挑战性，因为完整长视频执行使候选验证昂贵，失败会跨耦合证据处理阶段传播，复杂预处理、感知工具和定位策略使代码级更新难以可靠实现。

Method: 框架首先从稀疏采样帧和关联查询中分析信息密度和证据需求以指导初始设计；然后将定位失败压缩为独立可执行的最小验证任务；构建证据锚定的Gold Paths；审计Student轨迹；跨样本聚合失败案例并归因于负责模块；模块化智能体表示约束每次更新到主要负责模块及其必要依赖。

Result: 在VA-EvoBench（8个视频分布）上，4次迭代后，所有初始智能体均获得提升，宏平均准确率从38.44%提升至51.47%，每个分布平均进化成本3.54M tokens。比最强固定设计视频智能体高出6.39个百分点，同时使用最少的tokens和视频帧。

Conclusion: MetaVideoAgent能够有效自动进化视频智能体以适应不同视频分布，为长视频理解任务提供了一种新的自动化智能体设计方法，在准确率和效率上均取得显著提升。
...
TL;DR: EgoCross是一个跨域自我中心视频问答基准，用于评估多模态大语言模型在非日常场景中的泛化能力。首届挑战赛包含四个目标领域：外科手术、工业装配、极限运动和动物视角。


<details>
  <summary>Details</summary>
Motivation: 评估多模态大语言模型能否超越常见日常生活场景进行泛化，推动跨域自我中心视频理解的发展。

Method: 构建包含第一人称视频、问题及四个候选答案的测试集；设置两个官方赛道：Source-Limited Track限制使用官方基线模型和小支持集，Open-Source Track允许更广泛选择但禁止手动构建目标域训练数据。

Result: 挑战赛共收到超过1500份提交，来自130多名参与者，Open-Source Track有19支团队，Source-Limited Track有38支团队。

Conclusion: 该报告为跨域自我中心视频理解提供了技术参考，所有资源包括挑战数据、基线实现和获胜团队代码均已公开。
...
TL;DR: 本文提出COSMO方法，解决无源域自适应(SFDA)中视觉语言模型引导下的样本级可靠性分配问题，通过共识驱动移位调制平衡源模型和VLM的证据保留。


<details>
  <summary>Details</summary>
Motivation: 现有跨模型引导方法未明确考虑源模型和VLM在不同目标样本上的相对可靠性差异，可能在冲突时覆盖有效的源派生证据（源派生证据遗忘问题）。

Method: COSMO将VLM引导的SFDA定义为样本级可靠性分配问题，采用共适应机制替代专家对专家指导。具体包括：1）形成样本特异性初始共识，偏好更集中的预测；2）在适应过程中重新聚合两个分支的演化证据；3）基于共识不确定性和训练进度调节共识相对于初始锚点的移动距离。

Result: 在四个基准数据集上使用匹配的VLM骨干网络，COSMO达到了最先进的性能。分析表明该方法更好地平衡了有效源派生证据的保留与互补VLM证据的吸收。

Conclusion: COSMO通过锚定共享共识机制，保持共享监督的同时保持适应性，有效解决了源派生证据遗忘问题，为无源域自适应提供了新的样本级可靠性分配范式。
...
TL;DR: 本文提出统一框架，整合空间、关系和时间线索进行手术三要素识别，解决现有方法无法同时建模帧内标签依赖和帧间时间语义的问题。


<details>
  <summary>Details</summary>
Motivation: 现有手术三要素识别方法难以联合建模帧内标签依赖和帧间时间语义，需要统一框架整合空间、关系和时间线索以保持跨时间的关系一致性。

Method: 提出多尺度编码器提取类特定空间先验；Label Correlation Modeling模块使用MS-CAMRE捕捉静态共现模式和动态上下文依赖；BTRFA模块协调时间和关系表示实现连贯时间推理；引入TCER新评估指标。

Result: 在CholecT45和ProstaTD数据集上，AP_IVT分别提升5.1%和7.8%；TCER相对降低36%和25%。

Conclusion: 框架在时间-关系协同推理方面有效，验证了统一建模空间、关系和时间线索的优越性。
...
TL;DR: 本文提出HiSC，一个无需训练的3D视觉语言模型(3D VLM)令牌压缩框架，通过层次化空间聚类方法解决3D场景中的令牌冗余问题。


<details>
  <summary>Details</summary>
Motivation: 3D VLMs在多视角场景中进行空间推理时存在严重的令牌冗余问题，原因包括重复观测和大面积无信息区域，导致计算成本高昂。现有2D VLM的视觉令牌压缩方法无法捕捉3D场景的结构化特性，导致空间覆盖不完整和细粒度细节丢失。

Method: 提出两种策略：1) 空间图合并(SGraM)：将跨视图冗余建模为空间连接，在LLM推理前合并物理一致的区域，消除高度相似的冗余令牌；2) 空间聚类剪枝(SCluP)：在LLM推理中执行层次化压缩(跨聚类 and 聚类内)，保留对象实例完整性同时保留重要区域的细粒度细节。

Result: 在多种3D推理基准测试上验证了HiSC的有效性，特别是在高视觉令牌剪枝比例下表现优异。实现了超过90%的令牌 reduction同时保持最小性能下降。

Conclusion: HiSC作为无需训练的框架，能够有效解决3D VLM中的令牌冗余问题，通过层次化空间聚类方法在保证推理性能的同时大幅降低计算成本，为3D多模态推理的效率优化提供了新思路。
...
TL;DR: DAC-Pose提出了一种基于双智能体的姿态引导人体生成框架，通过结合先验语义推理(PSR)智能体和差异感知视觉编码(DAVE)智能体来解决大幅视角变化下的人体图像生成问题。该方法将传统的被动像素级映射转变为协作式语义推理，实现了高保真细节合成。


<details>
  <summary>Details</summary>
Motivation: 当前姿态引导人体生成方法在面对大幅视角变化时会产生严重的视觉伪影，根本原因在于缺乏对不可见区域的逻辑推理能力和复杂空间变形建模能力。现有方法本质上是被动的像素级映射，无法进行复杂的语义推理。

Method: 提出DAC-Pose框架，包含两个互补组件：1)先验语义推理(PSR)智能体作为认知引擎，利用协作推理推断不可见区域的细粒度属性；2)差异感知视觉编码(DAVE)智能体作为专业视觉感知智能体，量化和编码视角引起的空间错位，持续向生成过程反馈鲁棒的空间约束。两者形成语义推理与视觉感知之间的自主反馈循环。

Result: 在DeepFashion和Market-1501基准数据集上进行了广泛实验，验证了智能体驱动范式的优越性。DAC-Pose在大幅视角变化下能够出色地保持纹理对齐和身份一致性。

Conclusion: 所提出的智能体驱动范式为姿态引导人体生成提供了新的思路，通过双智能体协作实现了高保真的人体图像合成，特别是在处理大幅视角变化场景时表现出色。
...
TL;DR: DisMix是一种用于医学图像序数分类的顺序感知混合框架，通过双码本VQ-VAE将序数特征和非序数特征解耦，分别进行混合以保持疾病严重程度评级的序数结构。


<details>
  <summary>Details</summary>
Motivation: 标准mixup方法在序数分类任务（如医学疾病分级）中不适用，因为它会混淆序数特征（疾病严重程度）和非序数特征（外观变化），破坏了临床严重程度分级的序数结构。

Method: 提出DisMix框架，使用双码本VQ-VAE将特征解耦为序数码本和非序数码本；序数码通过插值产生有意义的中间等级，非序数码通过变化增加外观多样性而不破坏序数信号。

Result: 在4个医学影像数据集上，DisMix在6种图像mixup基线方法和6种序数分类器的组合中表现最佳，且在数据稀缺和临床分级变异性条件下仍保持有效性。

Conclusion: DisMix成功解决了医学图像序数分类中的mixup难题，通过特征解耦和针对性混合，既保留了序数结构又引入了外观多样性，为医学疾病分级提供了更有效的增强方法。
...
TL;DR: 本文提出TDSM-MM方法，通过分类-生成范式解决零样本骨骼动作识别问题。利用文本条件去噪Transformer结合RGB条件token，让RGB和文本联合条件单个类评分函数，避免了传统多模态方法中融合权重选择的问题。


<details>
  <summary>Details</summary>
Motivation: 零样本骨骼动作识别存在歧义：未见的动作可能共享相似的骨骼关节动态，但在物体或场景上下文上不同。RGB提供这些缺失线索，但现有方法通常维护独立的骨骼和RGB评分分支并融合输出，固定融合权重无法捕捉类别依赖的模态可靠性，而自适应规则缺乏目标侧反馈。

Method: 提出分类-生成范式：每个类别的评分取决于文本条件去噪器预测添加到骨骼特征的噪声的准确程度。构建TDSM-MM：文本条件去噪Transformer配合非扩散的RGB条件token，作为骨骼数据重建期间的稳定视觉锚点。

Result: 在NTU-60/120的4个分割中3个获得最佳归纳精度；在NTU-120 96/24上超越转导最先进水平（71.3% vs 69.1%），无需测试时适应。

Conclusion: 扩散基方法可成为零样本学习的很有前景的方向。
...
### [59] [CSGen: A Multi-Domain Curvilinear Structure Generation Model via Hierarchical Multimodal Diffusion](https://arxiv.org/abs/2608.04655)
...
### [60] [YOLO-PVC: 2D-to-3D Consolidation of Slice-wise Detections for Volumetric Liver Tumor Localization in MRI](https://arxiv.org/abs/2608.04642)
*Talha Waqas,Mounir Lahlouh,Kawther Taibouni,Mahnoor Waqas,Salar Ahmed,Sébastien Mulé,Yasmina Leroul-Chenoune*

Main category: cs.CV

TL;DR: 提出YOLO-PVC框架，用于将2D切片目标检测器在3D体数据上的检测结果进行2D到3D的整合。该方法通过深度连续性约束、鲁棒百分位统计聚合Box坐标，以及轻量级MLP校准模块来改进轴向范围估计。实验在3D肝脏MRI上验证，IoU3D从0.665提升至0.710。


<details>
  <summary>Details</summary>
Motivation: 切片式2D目标检测器虽计算高效且可扩展，但沿深度轴的预测常常碎片化且不稳定，需要有效的3D整合方法。

Method: 提出轻量级模型无关的YOLO-PVC框架，包含：1) 深度连续性约束；2) 鲁棒百分位统计聚合边界框坐标；3) 轻量级MLP校准模块精化轴向范围。

Result: 在3D肝脏MRI三种肿瘤类别上，启发式PVC达到IoU3D=0.665，校准版本提升至0.710，BEV IoU≈0.78。

Conclusion: 结构化几何整合为临床MRI中肝肿瘤体绘制准确定位提供了有效实用的解决方案。
...
TL;DR: 提出了一种针对广义指代表达理解(GREC)任务的新型强化学习方法RC-GRPO，旨在增强多模态大语言模型(MLLMs)在拒绝不存在物体时的能力，同时保持对正样本的定位准确性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)在指代表达理解任务中表现优秀，但难以拒绝不存在的目标物体（负样本），因为训练时缺乏负样本，导致产生幻觉 bounding box。现有的后训练方法如SFT和RL虽然能增强拒绝能力，但会降低正样本的定位准确性。

Method: 提出RC-GRPO，一种校准的强化学习策略，通过在rollout中对有效负样本强制输出"None"来计算优势估计，并对正样本施加惩罚以防止过度拒绝，实现准确性和可靠性之间的平衡。第二阶段推理强化进一步巩固因果理解和可解释性。

Result: 在三个GREC基准测试上的实验表明，RC-GRPO在保持强大拒绝能力的同时，获得了 superior 的定位准确性。

Conclusion: RC-GRPO能够在增强拒绝能力的同时保持定位性能，解决了现有方法中准确性和可靠性之间的 trade-off 问题。
...
### [62] [FUSEP: A Multi-Center Benchmark for Diverse Tasks in Early Pregnancy Fetal Ultrasound Screening](https://arxiv.org/abs/2608.04766)
...
TL;DR: 本文提出了FUSEP数据集，这是首个用于早孕胎儿超声筛查的公开基准数据集，包含来自3家医院的4,017张超声图像，涵盖CRL和NT两个标准切面，共标注了14个关键解剖结构，提供了45,820个边界框级专家标注。


<details>
  <summary>Details</summary>
Motivation: 全球每年有大量先天缺陷婴儿出生，尤其在医疗资源欠发达地区。胎儿超声筛查是早孕解剖检测的主要方式，但缺乏早孕胎儿超声数据集，阻碍了自动化辅助诊断的发展。

Method: 构建了包含两个国际指南推荐标准视图（CRL和NT视图）的数据集，收集了来自不同超声医师、设备、扫描角度和医院的图像，并由医学专家进行14个关键解剖结构的边界框标注。同时报告了半监督学习、全监督学习、无监督域适应和源自由无监督域适应四种方法在超声图像多目标检测上的性能。

Result: 该数据集是首个公开可用的早孕胎儿超声筛查数据集和基准，可用于标准平面识别、超声图像质量控制、早孕胎儿自动化辅助诊断、医学多目标检测、目标检测域适应等多个任务。

Conclusion: FUSEP数据集和基准有望为医学界在早孕胎儿超声筛查的智能化发展做出贡献，推动标准平面识别、质量控制、自动化辅助诊断及域适应等多项任务的研发。
...
### [63] [Overcoming Statistical Bias in Action-Controllable World Models](https://arxiv.org/abs/2608.04653)
*Yuhong Shi,Zhenhao Chu,Jie Wei,Jun Hao,Jianyi Liu,Jingwen Fu*

Main category: cs.CV

TL;DR: 本文提出CoCo框架,通过counterfactual consistency约束增强动作可控性,解决world model中动作信息被统计捷径取代的问题,在多个基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前action-conditioned world models存在统计捷径问题:模型可通过视觉惯性或运动模式预测未来帧,而无需真正依赖action信息,导致不同action产生相似future或零action仍有motion。现有方法仅注入action特征不足以解决此问题。

Method: 提出CoCo框架包含两个互补约束:1)Multi-step counterfactual consistency:约束reference、inverse-action、zero-action rollouts;2)Action-spatial counterfactual consistency:强制mirror scenes和transformed actions下预测一致。同时引入ARC和DE指标评估action controllability,以及Mini-SSMB进行same-state multi-action counterfactual评估。

Result: 在Mini-SSMB上达到ARC_inv=0.412, ARC_ref=0.483,DE相对baseline降低17.07%;在VP2 visual planning上达73.1%平均成功率(最佳SOTA);在BAIR和RoboNet上验证了视频预测质量和跨模型设置的可迁移性。

Conclusion: 通过counterfactual consistency约束可有效减少模型对统计捷径的依赖,增强action-dependent dynamics,在视觉规划和视频预测任务上取得显著提升,且具有良好的泛化能力。
...
TL;DR: 本文介绍了遥感图像操纵检测与定位的首个基准数据集，旨在应对生成式AI带来的卫星图像深度伪造问题。数据集包含60幅图像（30幅操纵图像和30幅真实图像），支持像素级定位评估和元数据分析。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI技术发展，高质量卫星图像伪造问题日益严峻，但现有数据集缺乏细粒度操纵标注和真值掩码，无法有效评估检测算法的定位性能。

Method: 构建包含复制粘贴拼接和扩散模型修复两种操纵类型的基准数据集，每幅图像配备像素级真值掩码和采集元数据。

Result: 发布首个包含60幅图像的遥感图像操纵检测基准数据集，支持图像级分类和像素级定位评估。

Conclusion: 该数据集填补了遥感图像取证领域缺乏高质量细粒度操纵标注的空白，为地理空间深度伪造检测研究提供了重要基础。
...
### [65] [VQ-VAD: Vector-quantized Motion Representation Learning for Human-centric Video Anomaly Detection](https://arxiv.org/abs/2608.05069)
*Narges Rashvand,Ghazal Alinezhad Noghre,Shanle Yao,Gabriel Maldonado,Hamed Tabkhi*

Main category: cs.CV

TL;DR: 提出VQ-VAD框架，通过向量量化GAN学习关键点序列的离散运动表示，用于视频异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于姿态的视频异常检测方法在连续潜在空间中建模，难以学习紧凑的运动模式；同时原始视频存在视觉噪声和隐私问题。

Method: 将VQ-GAN从图像生成领域改编为关键点序列操作，构建正常行为的运动码本，仅使用正常运动序列训练，通过重建误差检测异常。

Result: 在HR-SHT数据集上达到81.83%的域内准确率；从CMU Panoptic跨域迁移无需再训练可达76.69%；具有竞争力的跨数据集鲁棒性。

Conclusion: VQ-VAD通过离散运动表示学习有效检测异常，具有良好的域内性能和跨域泛化能力。
...
TL;DR: MobileWAM是一种基于视频生成骨干网络的移动操作策略，通过混合transformer架构将互联网规模的运动先验迁移到全身控制任务中。


<details>
  <summary>Details</summary>
Motivation: 现有世界行动模型局限于桌面操作，移动操作需要同时处理场景尺度的 locomotion（运动）和全身操作，但当前仍被动力学无关的视觉编码器和手工协调所主导。

Method: 提出三层混合专家架构：1) 层间联合注意力融合预训练视频扩散transformer与轻量行动专家；2) 每个前馈层包含共享、locomotion和操作三个专家的混合，通过行动令牌中的运动意图进行软路由；3) Chain-of-Foresight (CoF) 中间表示预测未来潜在块序列，每步基于前驱条件。

Result: 在ManiSkill-HAB基准上超越SOTA移动操作策略，并在真实ARX Lift2移动机械臂上成功微调，展现强泛化能力。

Conclusion: 视频生成 backbone 可有效迁移互联网运动先验到移动操作任务，混合专家和前瞻预测链为异构动力学融合提供了有效方案。
...
### [67] [Robust and Efficient Motion Reasoning for Privacy-Aware Classroom Incident Recognition](https://arxiv.org/abs/2608.05115)
*Paritosh Parmar,Landy Lan,Hong Yang,Chen Yi,Chiat Pin Tay*

Main category: cs.CV

TL;DR: 本研究探讨了隐私保护和计算高效的教室事件识别技术，提出了一种结合生成CCTV风格视频与真实教室姿态数据的混合基准，并设计了轻量级运动推理框架。该方法通过构建分层运动学表示，并将多阶运动学知识从大型教师模型蒸馏到小型学生模型，实现高效的个人推理。


<details>
  <summary>Details</summary>
Motivation: 教室安全是重要但研究不足的领域，现有的CCTV风格教室事件识别缺乏基准测试，且现有方法无法满足隐私保护、计算效率和域外泛化的实际部署需求。

Method: 1）构建结合生成CCTV风格视频与真实教室姿态数据的混合基准；2）提出分层运动学表示方法，将人类动作转化为层级运动学特征；3）将多阶运动学推理从大型教师模型蒸馏到小型学生模型，实现高效推理。

Result: 模型以不到十分之一的计算成本显著优于更大的基线模型，展现更强的域外运动推理能力和零样本合成到真实泛化能力。

Conclusion: 轻量级运动推理框架可有效实现隐私保护且高效的教室事件识别，为隐私-aware的教室安全研究提供了基准数据集、代码库和工具。
...
TL;DR: 本文提出了一种差分姿态估计方法，用于从帧间图像位移和已知3D控制点直接恢复平台运动，避免了传统方法的校准误差敏感性问题，在微动估计中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 精确的六自由度运动估计对机器人操作、自动驾驶和结构位移监测至关重要。传统3D-2D方法分别估计每帧的绝对相机姿态，通过相机-平台外参恢复平台运动，对外参校准误差敏感，尤其是在微动估计场景中。

Method: 通过差分透视投影方程、使用深度不变近似、在SE(3)上建模运动，直接从帧间图像位移恢复平台运动。支持单目和多相机系统。理论贡献包括：证明平移外参误差完全抵消，旋转误差产生有界扰动；推导通用可观性条件、克拉美-罗下界和无偏一致估计子；确定近似有效的条件限制。

Result: 在综合和真实世界实验中建立六自由度平台微动估计的新技术状态。在精度、校准鲁棒性和计算效率方面优于代表性PnP和广义PnP方法。五点、0.5像素图像噪声条件下：单目求解器旋转RMSE为10.09角秒，平移RMSE为3.70mm，运行时间0.34ms；双目求解器旋转RMSE为10.58角秒，平移RMSE为3.91mm，运行时间0.27ms。

Conclusion: 差分姿态估计方法通过避免独立绝对姿态估计，有效解决了外参校准误差问题，在微动估计任务中实现了更高的精度、更好的校准鲁棒性和更低的计算成本，为精密测量和监测应用提供了有效的解决方案。
...
### [69] [OPD-V: Visual On-Policy Self-Distillation with Modality Balance](https://arxiv.org/abs/2608.05131)
*Aniri,Jinhe Bi,Peng Liao,Zengjie Jin,Volker Tresp,Fei Shen,Yunpu Ma,Tat-Seng Chua*

Main category: cs.CV

TL;DR: 本文提出OPD-V方法，通过构建正负教师模型来利用模态平衡信息作为特权知识，解决多模态大语言模型中模态不平衡问题，提升视觉推理能力并降低训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有自蒸馏方法忽略了多模态大语言模型中的模态不平衡问题，当文本信息主导生成时，模型无法充分整合多模态输入，导致精心设计的特权信息未被充分利用，限制了OPSD效果。

Method: 构建Zoom-In Image正教师和Mask Image负教师展示不同程度的模态不平衡；提出正模态平衡logit边界定义模态平衡信任区域，选择用于自蒸馏的on-policy tokens；提出视觉OPSD范式OPD-V。

Result: 在6个基准测试、4个MLLM骨干网络和5种后训练方法上实验表明，OPD-V持续提升推理性能并降低训练成本。

Conclusion: 模态平衡本身可作为特权信息，OPD-V通过利用这一信息有效改善多模态推理，为视觉自蒸馏提供了新范式。
...
TL;DR: SurgNarrator是一种专为手术视频理解设计的生成式检索框架，通过构建手术中心词汇表、改编预训练视觉语言模型和学习具有时间感知的对比目标，实现快速而有效的手术视频理解。该方法在12个基准测试中取得一致的性能提升，输出延迟比生成基线降低超过两个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解方法存在权衡问题：自回归视频-语言模型支持全面推理但不适合时间敏感的临床应用，而对比模型延迟低但难以理解复杂手术场景。生成式检索虽在通用领域被探索，但转移到手术领域面临挑战，因为近似的视觉外观可能表示语义不同的事件，且术语高度专业化。

Method: 1)从手术字幕构建手术中心词汇表，定义临床有意义的检索空间；2)改编预训练Qwen3-VL-Embedding-8B，使用时间感知对比目标学习区分性临床表示；3)推理时采用分层、过程感知的检索策略，将搜索空间缩小到相关过程类型。

Result: 在零-shot设置下，在12个基准测试中实现一致的性能提升，输出阶段延迟比生成基线降低超过两个数量级。

Conclusion: SurgNarrator成功实现了手术视频的快速准确理解，通过手术中心词汇表和分层检索策略有效解决了手术领域特殊性和延迟要求的挑战，为术中决策支持提供了实用解决方案。
...
TL;DR: UniWorld-View是一个统一框架，用于从单目输入实现可控的大基线新视图合成。该方法将显式3D引导与生成式扩散模型相结合，通过遮挡感知的点云渲染策略解决可见性歧义，为扩散合成提供准确先验。实验在WorldScore基准和零样本NVS基准上验证了其在可控性、几何一致性和视觉保真度方面的有效性。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上大量捕获的单目视频和图像为沉浸式内容创作提供了宝贵资源，但从稀疏观察中生成新视图可以极大提升用户体验。然而，当输入覆盖极其有限时，产生具有精确相机控制的照片级几何一致视图仍然具有挑战性。重建方法（如NeRF和3D Gaussian Splatting）在稀疏输入下严重恶化，无法显式处理遮挡。生成方法虽能缓解数据需求，但仍因不准确或隐式的几何引导而在大幅视角合成中困难重重。

Method: 提出UniWorld-View统一框架，用于从单目输入的可控大基线新视图合成。该方法将显式3D引导与生成式扩散模型集成，实现精确相机控制和几何一致视图生成。几何引导通过遮挡感知的点云渲染策略获得，解决可见性歧义并为扩散合成提供准确先验。将此渲染策略与强大的视频扩散骨干网络耦合，即使在极端相机运动和大基线变化下也能实现高保真新视图生成，并可为下游动态3DGS重建提供多视角视频。

Result: 在WorldScore基准和零样本NVS基准上的实验表明，UniWorld-View在可控性、几何一致性和视觉保真度方面有效。即使在极端相机运动和大基线变化下也能实现高保真新视图生成，并可提供多视角视频用于下游动态3DGS重建。

Conclusion: UniWorld-View成功实现了从单目输入的可控大基线新视图合成。通过将显式3D引导与生成式扩散模型相结合，该方法克服了现有方法的局限性，能够处理极端相机运动和宽基线变化，同时保持几何一致性。该方法代表了从稀疏单目观察生成高质量、可控新视图的重要进展。
...
TL;DR: 德国铁路数字化计划(DSD)下的DB InfraGO AG和understandAI GmbH开发了一个包含700多万标注的多传感器数据集,专门用于铁路环境感知AI系统的训练和验证。该数据集涵盖铁路专用和通用感知目标,可用于从部分自动化(GoA2)到完全自动化(GoA4)的各级别自动驾驶铁路系统。


<details>
  <summary>Details</summary>
Motivation: 随着铁路自动化程度提高(GoA2-GoA4),需要AI系统实时检测、分类和响应潜在危险。开发这类AI感知系统需要大量准确标注的数据进行训练和验证,但当时缺乏专门的铁路环境感知数据集。

Method: 在Digitale Schiene Deutschland(DSD)计划框架下,采用多传感器方式采集数据,涵盖铁路专用目标物和通用感知目标物,在不同运营场景下进行采集,最终生成超过700万个高质量标注。

Result: 成功开发了一个全面的多传感器数据集,包含超过700万个高质量标注,可应用于铁路环境感知AI系统的训练和验证。该数据集现已可通过DB InfraGO AG申请获取。

Conclusion: 该数据集作为铁路领域AI驱动环境监测的宝贵资源,将推动铁路自动驾驶技术的发展,为不同自动化等级(GoA2-GoA4)的铁路系统提供重要数据支撑。
...
TL;DR: YOLOv14是一个统一的目标检测框架，通过四项协同创新解决了非理想输入条件下（如鱼眼畸变、游戏渲染角色、航拍视角和360°全景）目标检测性能急剧下降的问题。


<details>
  <summary>Details</summary>
Motivation: 当前实时目标检测器在受控条件下精度很高，但在非理想输入下性能急剧下降，包括鱼眼畸变、游戏渲染角色、航拍视角和360°全景图等场景。

Method: 提出四项协同创新：(1) 可变形区域注意力(D-AAttn)用学习的2D变形场替代刚性注意力网格，实现几何畸变下的自适应采样；(2) Game2Real域适应通过自适应实例归一化(AdaIN)和对抗域混淆对齐渲染游戏与真实图像的特征分布；(3) 多视角条件化将学习的视角嵌入注入骨干网，并使用跨视角对比损失拉近同一类不同视角的特征；(4) 自适应增强策略自动分类输入场景类型并路由到最优增强，同时动态尺度路由器学习每输入特征金字塔权重。

Result: 在T4 GPU上达到49.1 mAP（延迟2.91ms），在鱼眼(+4.1 mAP)、全景(+6.6 mAP)、无人机(+6.4 mAP)和游戏角色(+26.1 mAP)基准上取得显著提升。

Conclusion: YOLOv14通过四项创新有效解决了非理想条件下的目标检测难题，实现了高精度与低延迟的平衡。
...
TL;DR: MVFGA是一种新型多视角一致的上半身虚拟形象生成管道，通过分别建模面部和手部并与参数化上半身网格模型融合，实现了精细面部表情和手部姿势的高质量捕获，并使用3D高斯溅射实现高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么只关注头部而忽略手部手势，要么重建全身但无法保留精细的面部保真度和手部姿势准确性，无法捕捉对面部表情和手部手势的细微动态，这是自然人类交流的关键。

Method: 提出MVFGA管道：1)分别建模面部和手部；2)与参数化上半身网格模型融合；3)将3D高斯溅射到网格上实现高质量新视角渲染。同时引入MVFGA-MoCap多视角上半身运动捕捉数据集。

Result: MVFGA生成具有高保真面部表情和手部动作的视觉真实虚拟形象，在上半身虚拟形象动画任务上优于基线方法。

Conclusion: 该方法成功解决了面部表情和手部姿势捕获的难题，通过分离建模和多视角一致的方式实现了高质量的上半身虚拟形象重建和渲染。
...
TL;DR: 提出了一种基于深度引导双分支Vision Transformer的dToF传感器密集度量深度补全框架，能够在无真实标签的情况下实现跨传感器、跨稀疏度和噪声条件的零样本泛化。


<details>
  <summary>Details</summary>
Motivation: dToF传感器虽然精度高、抗干扰能力强，但制造成本高、光电二极管阵列尺寸小，导致深度图极端稀疏、低分辨率且噪声严重，无法满足VR/XR、机器人和3D感知任务对密集度量深度信息的需求。现有单目和深度补全方法难以处理dToF独特的采样模式和硬件伪影，在严重稀疏或噪声条件下性能显著下降。

Method: 1) 深度引导双分支Vision Transformer编码器：分别处理RGB图像和稀疏dToF测量；2) 掩码联合注意力模块：允许深度token可靠地引导图像特征，避免被覆盖；3) 轻量级解码器：高效重建密集度量深度，无需扩散或重细化后处理；4) dToF仿真 pipeline：模拟flash、sub-VGA flash和rotating传感器的特性，包括硬件退化、不规则稀疏度和真实噪声分布。

Result: 在完全使用合成数据训练的情况下，该模型在6个数据集和3个真实dToF设备上实现了强大的零样本泛化，在准确性和计算效率方面均优于现有最先进方法。

Conclusion: 该工作为来自稀疏直接ToF传感器的密集度量深度补全提供了一种稳健实用的解决方案，建立了可跨不同传感器类型、稀疏度和噪声条件运行的通用框架。
...
TL;DR: 本文提出了一个可扩展的评估框架，用于评估文本到图像模型对明喻的理解能力。研究发现前沿模型经常误解隐喻载体，将其与被描述的对象混淆，揭示了 figurative language 与视觉 grounding 之间的差距。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型虽然能从明喻提示生成视觉上吸引人的输出，但即使是最先进的模型也经常误解隐喻载体，将其与对象混淆。这些系统性失败揭示了 figurative language 与 object-level visual grounding 之间的差距。

Method: 提出包含三个部分的评估框架：(1) 控制的明喻数据集，其中隐喻载体来自预定义的可检测对象类别并与多样化模板结合；(2) 基于 YOLO (You Only Look Once) 检测的自动 grounding 指标；(3) 使用 Diffusion Lens 进行文本编码器层分析，以追踪生成过程中隐喻载体的出现。对 architecturally diverse 的 t2i 模型进行实验。

Result: 跨不同架构的 t2i 模型 reveals consistent literalization failure patterns（一致的字面化失败模式）。

Conclusion: 研究揭示了 t2i 模型在明喻理解方面的系统性问题，并讨论了潜在的缓解策略以改善 t2i 模型中的明喻 grounding。
...
TL;DR: 本文提出了一种针对稀疏视角锥束CT的3D高斯体素重建方法，通过联合优化几何参数来解决实际CT系统中因视角分布不均匀和有限视角导致的条纹和条带伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有的splat-based CT在模拟数据集上表现优异，但在实际CT系统中部署时出现明显的条纹和条带伪影，这些伪影主要源于姿态估计误差而非视角稀疏本身。

Method: 通过详细分析splatting公式中的姿态敏感性，导出一种稳定的基于梯度的框架，在重建过程中联合细化几何参数，实现几何参数的联合优化。

Result: 该方法揭示了姿态扰动如何通过可微分投影算子传播，解释了splat-based CT为何对几何对准误差特别敏感，并在实际稀疏视角条件下显著提高重建质量。

Conclusion: 该方法保持轻量级特性，易于集成到现有管道中，同时在真实稀疏视角条件下大幅提升重建保真度。
...
TL;DR: 本文提出了一种无需训练的模块化框架，用于多模态大语言模型的空间推理验证和修正。通过构建空间证据图（SEG）关联推理中的原子空间证据与视觉实体，并使用空间证据可靠性评估（SERA）识别被可靠视觉证据反驳的最早空间证据单元，引导模型修正后续推理和最终答案。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的空间推理可能产生与输入图像不一致的中间判断，导致错误在推理链中传播影响最终答案。现有方法主要通过训练或额外空间信息改进空间推理，未考虑推理过程本身是否忠实于模型输入。

Method: 提出模块化无需训练的框架：1）构建空间证据图（SEG），将链式思考推理中提取的原子空间证据与视觉实体、空间关系、源步骤和视觉证据关联；2）空间证据可靠性评估（SERA）基于对象存在、定位和几何测量评估视觉证据可靠性；3）识别被可靠视觉证据反驳的最早空间证据单元，引导原始MLLM修正后续推理和最终答案。

Result: 在15个模型-数据集设置中，该方法平均准确率达到68.94%，相比基线方法平均提升8.55个百分点。

Conclusion: 该框架通过验证和修正推理链中的不忠实空间证据，有效提升了多模态大语言模型的空间推理准确率，且具有模块化和无需训练的优势。
...
TL;DR: 本文提出了一种基于splat的金属伪影 reduction框架，将物理驱动的多色前向模型融入锥形束CT的高斯表示中，通过紧凑的材料参数化实现能量依赖衰减的联合优化，无需金属掩模即可有效抑制金属伪影并保留高频结构。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心问题：1) 传统重建算法基于单色假设，无法处理多色X射线的非线性衰减；2) 现有神经渲染方法存在平滑偏差、细结构丢失；3) 大规模锥形束CT计算成本过高。

Method: 提出splat-based框架：1) 采用连续高斯表示编码体积；2) 每个高斯核参数化能量依赖的衰减系数；3) 联合优化几何和材料参数，无需金属掩模；4) 融入物理驱动的多色前向模型。

Result: 在模拟和真实锥形束CT扫描上验证：收敛速度显著提升，金属伪影抑制效果优于现有重建方法和神经场方法。

Conclusion: 紧凑的衰减公式捕捉了生物组织和金属植入物的跨能量变化，使模型能够解释金属导致的非线性同时保留高频结构，为锥形束CT金属伪影校正提供了一种有效新范式。
...
TL;DR: 本文提出一种智能体框架，将个性化烹饪需求系统性地分解为结构化、可验证的控制程序，而非直接将语言映射为动作。该框架通过多个AI智能体协作，将用户意图转换为规范食谱、工作流程序和可执行的Python代码，在真实厨房环境中实现可靠的任务完成、透明的执行逻辑和有效的异常处理。


<details>
  <summary>Details</summary>
Motivation: 传统烹饪机器人依赖预定义程序和规则控制，执行稳定但个性化不足；近期大模型方法虽支持自然语言交互，但决策过程不透明且在真实厨房中执行不可靠。

Method: 构建三阶段紧密耦合的系统：1) 离线食谱到代码生成（多智能体协作）；2) 带有多模态感知的在线闭环执行与监督干预；3) 长期个性化的用户偏好模型后运行适应更新。

Result: 在物理烹饪平台上的真实实验表明，该框架在多样化个性化场景中实现了可靠的任务完成、透明执行逻辑和有效异常处理。

Conclusion: 验证了该框架在真实环境中实现可信自动化烹饪的实用性。
...
TL;DR: 本文首次系统比较了联邦学习中视觉-语言模型(VLM)适应策略在遥感图像分类任务中的表现，研究了全参数微调、编码器微调、提示学习和LoRA四种策略在非IID数据下的泛化能力、通信开销和计算复杂度的权衡。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临数据异质性（非IID数据）问题，会影响模型收敛和泛化能力。视觉-语言模型因其可迁移表示和分布偏移鲁棒性成为潜在解决方案，但大参数规模带来通信和计算挑战，需要选择合适的VLM适应策略来平衡效率与性能。

Method: 在四个遥感数据集（BigEarthNet-S2、EuroSAT、RESISC45、ImageNet）上系统比较了四种VLM适应策略：1）全参数微调 2）编码器特定微调 3）提示学习 4）低秩适应（LoRA）调优，从泛化能力、通信开销和本地计算复杂度三个维度进行评估。

Result: 实验揭示了任务专业化、跨域泛化与效率之间存在明显的权衡关系，不同策略各有优劣。

Conclusion: 基于研究发现，为不同操作约束下遥感图像分类的联邦学习场景提供了VLM适应策略选择指南。
...
TL;DR: 研究利用分割伪标签预训练3D ResNet编码器，仅用20%的人工标注数据即可达到接近全监督性能，显著降低腰椎MRI病理评估的标注成本。


<details>
  <summary>Details</summary>
Motivation: 腰椎MRI大规模专家标注数据获取成本高，而分割伪标签可由自动化工具零成本生成。研究旨在探索分割预训练是否能替代部分人工分级标注。

Method: 使用3D ResNet编码器预训练分割椎间盘、椎管结构，微调不同比例（10%-100%）的分级任务头进行对比实验。

Result: 分割预训练Dice达0.94，在所有标注比例下均提升任务平均ROC-AUC。仅用20%标注即可接近全监督性能，对低患病率或空间定位性病变提升最大。

Conclusion: 分割预训练可有效减少腰椎MRI病理评估所需的人工标注工作量，具有重要的临床应用价值。
...
TL;DR: 本文提出了StaticSegFormer方法，一种用于注意力层的静态结构化剪枝技术，解决动态剪枝方法在GPU上帧率低的问题。该方法在Cityscapes数据集上实现了34%的相对帧率提升，同时保持mIoU不下降。


<details>
  <summary>Details</summary>
Motivation: 现有动态结构化剪枝方法虽然在mIoU和FLOPs上表现相近，但在GPU平台上帧率意外地低于简单的静态方法。作者旨在解决这一效率问题，同时保持分割性能。

Method: 提出一种针对注意力层的静态结构化剪枝方法StaticSegFormer，通过消除推理过程中的参数组来提高DNN效率。

Result: 在ADE20K和Cityscapes基准测试中，StaticSegFormer在Cityscapes数据集上实现了34%的相对帧率提升，FLOPs降低，且没有任何mIoU性能下降。该方法对小型编码器和大图像效果最佳。

Conclusion: 静态结构化剪枝在语义分割任务中能够同时实现高效率和高性能，特别是在计算资源受限的场景下具有重要应用价值。
...
TL;DR: 提出Interval Denoiser框架，实现无潜在变量的少步生成。通过流匹配ODE推导出中间轨迹状态的精确解析映射，使预测在任意时间间隔内位于低维流形上，避免经验代数替换，正确分离纯时间导数。结合残差裁剪和时间采样课程策略，在ImageNet 256x256上从头训练，一步FID为4.55，两步FID为3.98。


<details>
  <summary>Details</summary>
Motivation: 解决现代扩散和流模型中多步采样的计算开销以及外部自编码器的重建瓶颈问题，推动向少步骤、无潜在变量的生成方式发展，使网络能够直接在像素上操作。

Method: 从流匹配ODE直接推导，建立中间轨迹状态的精确解析映射；预测在任意时间间隔内位于低维流形上；避免经验代数替换，正确分离纯时间导数以防止有偏梯度评估；引入残差裁剪和时间采样课程策略，实现有效的长间隔训练。

Result: 在ImageNet 256x256上从头训练，无需感知损失，一步生成（1-NFE）FID为4.55，两步生成（2-NFE）FID为3.98。

Conclusion: 提供了无潜在变量生成的严格理论框架，能够实现有效的长间隔训练并显著提升少步生成性能，为高效像素级生成提供了新思路。
...
TL;DR: ReSem-Face是一种用于人脸修复的级联扩散框架，通过引入基于身份条件的多参考语义先验，在严重遮挡和冲突文本指导下仍能保持身份保真度。该方法从多个参考图中提取代表性身份特征来重建缺失语义区域，并通过多流调节架构引导扩散过程。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在人脸修复方面已取得令人印象深刻的视觉效果，但在严重遮挡和冲突文本指导下保持身份保真度仍然是一个重大挑战。

Method: 提出了一种级联扩散框架ReSem-Face，引入显式的身份条件语义先验进行多参考人脸修复。具体包括：从多个参考图中提取代表性身份特征来重建缺失语义区域，通过多流调节架构引导扩散过程，在像素缺失时提供强语义约束，同时兼容提示驱动编辑。

Result: 在CelebAHQ-IDI-5和VGGFace2数据集上的实验表明，ReSem-Face在严重语义掩码下能够实现更可靠的身份保持修复，并相比代表性基线方法提高了文本控制编辑质量。

Conclusion: ReSem-Face方法能够有效稳定身份重建，同时保持与提示驱动编辑的兼容性，为人脸修复中的身份保真度问题提供了有效解决方案。
...
TL;DR: 提出一种新的图像裁剪方法GAFIC，通过注意力引导的特征融合和全局对齐的裁剪评估器来解决现有方法忽略全局关系的问题，在GAIC和CPC数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有图像裁剪方法主要关注显著区域，对图像组件间的全局关系敏感性有限，限制了其效果。

Method: 提出GAFIC方法，包含两个核心组件：1) AGFF（注意力引导特征融合）聚合局部区域重要性构建全局表示；2) GACE（全局对齐裁剪评估器）对齐候选裁剪特征与全局表示，使评估对边界变化敏感。并结合三个多尺度排序损失。

Result: 在GAIC和CPC数据集上优于现有图像裁剪方法，尤其在准确性和稳定性方面。

Conclusion: GAFIC不修改像素，而是从原图中选择美学偏好裁剪，适用于需要像素完整性和高效批处理的场景。
...
TL;DR: 本文提出RegisterBridgeMM，一种基于register tokens的RGB-IR目标检测融合框架，通过三阶段register生命周期（Aggregate→Bridge→Project）实现跨模态通信，在LLVIP、M3FD、DroneVehicle、FLIR-Aligned四个基准上取得最高mAP50-95。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-IR融合方法在表达性patch级交互与轻量级适应机制之间存在权衡，且在光照变化、天气变化和复杂场景下性能受限。预训练的register tokens包含模态共享和模态特定信息，可作为跨模态通信的紧凑载体。

Method: 提出三阶段register生命周期框架：1) Aggregate保留预训练的模态特定register摘要；2) Bridge执行双向register-to-patch读取with共识残差调节；3) Project将register摘要翻译为空间自适应校准。冻结双流backbone，仅使用register通路实现跨模态融合。

Result: 在LLVIP、M3FD、DroneVehicle、FLIR-Aligned四个基准上取得最高mAP50-95，且保持预训练的patch表示完整性。

Conclusion: Register tokens可作为有效的跨模态通信载体，register介导的融合框架在不引入密集patch交互的情况下，实现了优于现有方法的检测性能。
...
TL;DR: EventKitchen是一个大规模立体事件相机基准数据集，记录了10名参与者在13个不同厨房中的烹饪活动，包含5.5小时的立体事件录制、同步的RGB、深度和IMU数据，以及10,762个动作片段和13,482个边界框标注。


<details>
  <summary>Details</summary>
Motivation: 事件相机虽具有高时间分辨率、高动态范围和低功耗优势，但现有研究和数据集主要集中在汽车和无人机应用，缺乏以人为中心的日常场景数据集；现有的事件基人类活动数据集多为脚本化动作，无法捕捉自然人类行为。

Method: 该数据集以自我为中心的方式收集，参与者佩戴带有多个传感器的头盔，在13个不同厨房中自然进行烹饪活动（无脚本动作）。同时提供动作分割和目标检测标注，并训练了动作识别、目标检测和立体深度估计的基线模型。

Result: EventKitchen包含5.5小时的立体事件录制数据（同步RGB、深度、IMU），10,762个动作片段标注和13,482个边界框标注，建立了动作识别、目标检测和立体深度估计的基线模型。

Conclusion: EventKitchen通过捕捉自然、真实的烹饪活动，为神经形态视觉提供了一个超越自动驾驶的具有挑战性的基准测试平台，推动了以人为中心的事件视觉研究。
...
TL;DR: SlotNarrative 通过基于槽位的接口将视频组织为持久对象叙事，使用144个视觉 token 即可实现与帧数无关的视频理解，在准确率和 token 效率之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: Video-LLMs在开放域视频理解方面取得了进展，但其视觉接口仍然 token 密集，且对于跨时间链接重复出现的对象证据提供有限的结构化表示。

Method: 提出 SlotNarrative，一个基于槽位的接口，将视频组织为持久的对象叙事，使用紧凑的对象状态 token。方法包括：1)先将视觉特征分组为对象-like的槽位；2)然后通过轻量级、无参数的内存将重复观测与片段级对象条目关联，整合多个互补匹配线索；3)每个保留的条目序列化为两种 token 类型：身份 token（总结持久外观）和状态 token（编码片段级外观、几何、可见性和轨迹信息）。

Result: 该设计仅使用144个分配的视觉 token 位置，与采样帧数无关。在多个数据集上相比之前的紧凑 Video-LLM 接口实现了准确率和视觉 token 数量之间的良好权衡。

Conclusion: 持久对象叙事为 Video-LLMs 提供了一种紧凑、结构化且时间有序的视觉接口。
...
TL;DR: 本研究揭示了视觉语言模型(VLM)在医学诊断中的可靠性问题。研究发现，即使临床证据保持不变，仅通过改变影像切片顺序或标签排列位置，模型预测结果就会发生显著变化，预测翻转率高达48.9%-67.8%，且存在严重的诊断过度承诺问题。


<details>
  <summary>Details</summary>
Motivation: 标准准确率指标往往掩盖了VLM在敏感领域(如医学诊断)中的可靠性问题。作者认为需要系统评估VLM在临床场景下的稳健性，而不仅仅是关注整体准确率。

Method: 使用经组织病理学验证的脑MRI数据集，对四个VLM家族进行系统评估。通过两种证据保持扰动测试：(1)重新排序解剖切片顺序；(2)交换目标标签位置。设计负对照组测试，移除专家标注的病变切片后观察模型诊断行为。

Result: 研究揭示三个关键发现：1)顺序稳定性存在显著脆弱性，简单序列逆转导致最高48.9%的预测翻转；2)文本选择偏见，标签重新排列触发最高67.8%的不一致诊断；3)诊断过度承诺，移除病变切片后模型仍在76.1%的案例中生成确定性诊断。

Conclusion: 高准确率可能高估临床可靠性，掩盖了对序列呈现和文本框架的敏感性。稳定性和一致性指标对于VLM在安全关键临床应用中的部署至关重要。
...
TL;DR: STEP-OPD是一种用于图像生成的知识蒸馏框架，通过扩展学生的学习目标超越教师模型，并引入对内部表示演化的显式约束，解决现有 OPD 方法仅监督输出速度而忽视学生内部表示演化的问题。


<details>
  <summary>Details</summary>
Motivation: 现有OPD方法主要优化学生匹配教师的输出速度，将教师作为优化目标的上限，但仅靠输出级监督无法约束学生内部块的表示演化，削弱了跨层渐进式能力迁移。

Method: 1. 使用每个任务特定教师与共享基础模型之间的速度差作为进一步学习方向，并将缩放后的差异添加到教师速度中，使学生能够超越教师学习；2. 对齐学生和教师之间表示变化的方向和幅度，使学生学习跨网络块逐步转换表示的方式。

Result: 在组合对齐、文本渲染和人类偏好方面均优于标准OPD方法。GenEval分数从0.927提升到0.961，OCR和所有偏好指标均有提升。统一的Student在所有三个能力组上超越了相应的单任务教师。

Conclusion: 输出外推实现了超越教师的学习，表示变化对齐为学生的内部转换提供了补充指导。
...
TL;DR: TD-V2A是一种利用时序差分(Temporal Differences)增强视频到音频生成的方法，通过层级持续学习策略和退火时序差分引导技术，在最小化架构修改的情况下有效提升V2A生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有条件扩散模型V2A方法通常需要额外的音频视觉监督、声学结构预测或大模型推理，需要额外网络或强归纳偏置。作者希望借鉴视觉表示学习进展，找到一种minimal architectural modification的方法来增强视觉条件。

Method: 1)在帧级和特征级研究时序差分表示，确定最有效表示层；2)开发层级持续学习策略，在扩散训练过程中逐步学习和利用TD信息；3)提出退火时序差分引导方法，在采样过程中利用TD信息。

Result: 在基准数据集上的广泛实验表明，通过框架有效利用TD信息显著提升了端到端V2A生成质量，甚至优于专门的V2A表示（如对比音频视觉预训练）。

Conclusion: 时序差分是区分V2A和I2A的关键表示，TD-V2A通过最小架构修改实现了优异的性能，为V2A任务提供了一种有效且简洁的解决方案。
...
TL;DR: 本研究提出了一种基于扩散模型的框架SpineSegDiff，用于对腰椎MRI扫描进行鲁棒且准确的语义分割，无论扫描是T1加权还是T2加权。该框架在SPIDER数据集上与 vertebrae、椎间盘(IVD)和椎管分割的高级模型进行了比较。


<details>
  <summary>Details</summary>
Motivation: 本研究旨在提高腰椎间盘突出症(LBP)患者腰椎MRI扫描的语义分割准确性和鲁棒性，特别是在处理不同加权扫描(T1/T2)以及改善退行性椎间盘识别方面存在挑战。

Method: 开发了基于扩散模型的SpineSegDiff框架，并与包括最先进的非扩散模型nnUnet在内的高级分割模型进行了对比实验，使用SPIDER数据集对椎骨、IVD和椎管进行分割。

Result: SpineSegDiff实现了与nnUnet相当的分割性能，特别是在改善退行性IVD的识别方面表现出色。此外，模型生成的不确定性图为临床审查提供了有价值的见解，增强了分割结果的鲁棒性和可靠性。

Conclusion: 扩散模型具有通过更精确的分析病理脊柱MRI来增强LBP诊断和管理的潜力。
...
TL;DR: 本文提出一种在线主动学习机制，使轻量级深度估计网络能够在部署后持续适应新环境。通过预测-评估-纠正的闭环学习范式选择信息丰富的样本，并利用弹性权重 consolidation 实现选择性可塑性，在MobileNetV3-Small backbone上实现约75%的计算成本降低同时保持竞争性精度。


<details>
  <summary>Details</summary>
Motivation: 边缘设备的感知模型在部署后静态冻结，在领域偏移下性能严重下降；轻量级网络难以高效学习通用几何表示，导致性能饱和。生物视觉系统能够持续整合时序线索并在稳定性和可塑性间保持平衡，而人工模型缺乏这种适应能力。

Method: 引入在线主动学习(OAL)机制，建立预测-评估-纠正的闭环学习范式，从流式视觉输入中主动选择高置信度、信息丰富的信号；采用弹性权重 consolidation(EWC)实现选择性可塑性，保留编码全局相关结构知识的参数，同时允许局部对齐新环境。

Result: 基于MobileNetV3-Small backbone的系统实现约75%的计算成本降低，同时保持具有竞争力的深度估计精度。实验表明适应能力不取决于模型大小，而取决于如何在动态环境中有效调节参数可塑性。

Conclusion: 轻量级网络可通过在线主动学习和弹性权重 consolidation 实现持续适应能力；参数可塑性的有效调控是动态环境下适应能力的关键，而非单纯的模型规模。
...
TL;DR: 本文提出语义原型校准(SPC)方法，利用视觉语言模型Perception Encoder(PE)的语言对齐表征提升AI生成图像(AIGI)检测性能。PE具有比DINOv3更强的局部 provenance 组织能力，但传统线性探测无法充分利用。SPC通过从法医语义信息构建类别原型并进行监督数据校准来解决这一问题。PE-SPC在跨生成器、后处理和野外基准测试中均超越DINOv3基线，达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 1. 现有研究发现DINOv3作为视觉基础模型的线性探测在AIGI检测中表现优异，成为主流基线；2. 然而作者发现视觉语言模型Perception Encoder(PE)具有更大潜力，其语言对齐表征保留了高级provenance语义；3. PE在冻结特征空间中表现出比DINOv3更强的局部provenance组织能力；4. 但语义无关的线性探测(PE-Linear)无法利用这一结构，比DINOv3-Linear低4.1%。

Method: 1. 提出语义原型校准(Semantic Prototype Calibration, SPC)方法；2. 从法医语义信息构建类别原型；3. 使用监督数据对原型进行校准；4. 将SPC应用于PE，得到PE-SPC检测器。

Result: PE-SPC实现了更强的泛化能力；在跨生成器(cross-generator)、后处理(post-processing)和野外(in-the-wild)基准测试中均超越之前的DINOv3基线；取得新的最先进(state-of-the-art)结果。

Conclusion: 1. PE的语言对齐表征在AIGI检测中具有比DINOv2更大的潜力；2. 简单的线性探测无法充分挖掘PE的结构化信息；3. SPC通过语义原型校准有效利用了PE的provenance组织能力；4. 该方法为基于视觉基础模型的AIGI检测提供了新的研究方向。
...
TL;DR: 本文提出不确定性引导的UMRE网络(UG-UMRE)，解决现有统一多模态关系提取中忽略固有偶然性不确定性和模态分布异质性带来的噪声传播和对齐困难问题。


<details>
  <summary>Details</summary>
Motivation: 现有UMRE研究存在两个关键问题：1) 忽略固有偶然性不确定性导致噪声传播；2) 深层模态分布异质性阻碍跨模态对齐。

Method: 设计两个核心模块：1) 不确定性驱动单模态增强(UDUA)模块，基于变分信息瓶颈将特征建模为高斯分布，通过不确定性感知自监督对比学习过滤噪声；2) 联合偶然性不确定性对齐(JAUA)模块，利用概率分布一致性构建共享潜在空间，通过同步跨模态统计特性消除分布差距。

Result: 在UMRE、MORE和MNRE三个基准数据集上取得最先进性能，验证了UDUA和JAUA模块的即插即用性和有效性。

Conclusion: UG-UMRE通过不确定性建模和分布对齐机制有效解决了多模态关系提取中的噪声和对齐问题，为细粒度交互奠定了坚实基础。
...
TL;DR: 本文提出WDR框架，通过在多阶段B-Rep生成的中间线框阶段进行检测和修复，无需重新训练即可提高CAD模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 多阶段边界表示（B-Rep）生成中，中间线框存在的几何和拓扑风险（如自交、边塌陷、断开顶点）会传播到最终的B-Rep中导致无效，而重新训练大型生成模型的计算成本过高。

Method: 提出WDR框架，包含两个核心模块：1) GTAD（几何-拓扑异常检测器），结合并行VLM粗筛与几何/拓扑检测器预测下游无效风险；2) EGGTR（能量引导几何-拓扑修复模块），通过检测器触发的引导再生进行修复。可通过能量引导重采样扩展测试时计算，无需训练即可引导扩散模型。

Result: 在广泛实验中，WDR在保持CAD模型多样性和分布质量的同时，显著提高了内核检查的有效性。

Conclusion: WDR可集成到自回归和扩散流程中无需重新训练，为改善CAD模型生成质量提供了一种计算高效的训练-free方案。
...
TL;DR: 本文提出了交互式多镜头视频创作(IMVC)任务，并推出ContextMaster统一模型。该模型通过角色感知上下文表示、固定预算稀疏路由和ConstraintSink机制，在保持历史上下文的同时实现高效推理。提出两阶段特权上下文蒸馏框架解决稀疏上下文访问和少步推理的挑战，在三项任务上超越专用基线，达到16 FPS。


<details>
  <summary>Details</summary>
Motivation: 现有视频模型将生成、参考条件控制和编辑作为固定输入的独立操作，但实际创作需要跨多镜头保持共享历史。现有方法无法在一个模型中灵活处理这些操作并维护一致的历史上下文。

Method: 1) ContextMaster：角色感知上下文表示，可重用的清洁上下文状态，固定预算稀疏上下文路由，ConstraintSink保持任务约束可见；2) 两阶段特权上下文蒸馏：一致性蒸馏从密集教师转移完整上下文行为，分布匹配微调部署 rollout。

Result: 在三项基元任务上提升任务完成度和镜头间一致性；用户研究验证灵活组合工作流能力；单GPU达到16 FPS。

Conclusion: ContextMaster成功实现了统一的多镜头视频创作模型，通过创新的上下文管理机制和蒸馏训练策略，在保持高效推理的同时实现了跨镜头的一致性，为交互式视频创作提供了新的研究方向。
...
TL;DR: 本文提出利用视觉基础模型进行动物姿态估计和跟踪的方法，在有限标注数据下实现有效的跨物种泛化。提出两种模型：无监督方法利用基础模型特征进行无需训练的对应对匹配；有监督方法通过关键点提示编码器将结构先验注入特征匹配。


<details>
  <summary>Details</summary>
Motivation: 动物姿态估计和跟踪对野生动物监测和保护研究重要，但受限于专家标注时间。人类姿态估计因大规模标注数据集快速发展，而动物姿态估计因物种间形态行为差异大、标注数据有限而面临挑战。现有方法在泛化性和性能之间难以平衡。

Method: 提出两种关键点跟踪模型：1）无监督方法：利用多样化的基础模型特征进行无需训练的对应对匹配，实现强跨物种鲁棒性；2）有监督方法：采用关键点提示编码器，将参考帧的结构先验显式注入特征匹配，提高跟踪精度。

Result: 在APTv2和TigDog两个具有挑战性的动物视频基准上进行了广泛评估，框架实现了 strong performance，在准确性和泛化性之间保持有效平衡。

Conclusion: 该框架为动物行为分析和保护应用提供了实用的解决方案，能够在有限标注数据下有效跟踪用户选择的视频关键点。
...
TL;DR: 本文通过系统性实验探索了多模态预训练的设计空间，揭示了模态交互的四个关键机制：知识流动模式、协同与竞争关系、早期统一的重要性，以及高效预训练配方，并在13.5B MoE模型上验证了核心发现。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉对推进基础模型至关重要，但统一多模态预训练的设计空间和模态交互的基本机制尚未得到充分探索，需要实证澄清。

Method: 在合成和大规模真实数据集上进行受控实验，系统性地探索多模态预训练机制，并在2T tokens上训练13.5B MoE模型进行验证。

Result: 四个关键发现：(i)知识流动：揭示了语言、视觉理解和视觉生成之间的知识转移模式及不对称性；(ii)协同vs竞争：数据复杂度决定模态是否协同，共享注意力+模态特定前馈层等架构选择促进协同；(iii)早期统一：早期统一训练优于后对齐或顺序训练，发现视觉懒惰现象；(iv)配方：仅用5%计算预算实现强生成性能。

Conclusion: 为理解和扩展多模态预训练提供了原则性基础，验证了核心发现在大规模训练中的有效性。
...
TL;DR: 本文提出了OmniEdit-Bench，一个针对指令驱动视频编辑（IVE）的综合评估基准。现有的评估基准存在两个主要局限：继承自图像编辑的任务覆盖不足，以及无法衡量指令保真度的评估指标。OmniEdit-Bench从空间、时间、音频和参考等多个视频特定维度分解编辑任务，并区分显式和隐式指令，结合推理场景进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑评估基准存在两大问题：1）任务覆盖有限，仅继承图像编辑维度，忽视视频特有的时空等多模态特性；2）评估指标不足，无法有效衡量指令保真度，导致错误的编辑因保留原视频强视觉先验而获得高分。这阻碍了IVE模型的正确评估与发展。

Method: 提出OmniEdit-Bench基准测试：1）将编辑任务分解为空间、时间、音频和参考四大视频特定维度；2）区分显式与隐式指令，引入推理场景；3）构建四维评估框架（准确性、保留度、真实性和一致性），结合人类判断和视觉语言模型；4）引入准确性感知惩罚机制，将其他分数基于准确性条件化，防止视觉似是而非的错误编辑获得过高评价。

Result: 对主流开源和商业IVE模型的广泛实验表明，当前模型表现远未达到满意水平。OmniEdit-Bench为指令驱动视频编辑提供了全面可靠的测试平台，并为未来研究指明方向。

Conclusion: OmniEdit-Bench是一个全面、结构化的IVE评估基准，有效解决了现有基准的任务覆盖不足和评估指标缺陷问题。准确性感知惩罚机制确保了评估的可靠性。实验揭示了当前IVE模型的能力边界，为该领域未来研究提供了重要参考和明确方向。
...
TL;DR: 提出了一种基于场景射线预测的相机标定框架，用于三维重建。该方法使用重建误差和交叉误差等新型度量指标，结合自举程序评估不同标定对象和标定流程。设计了一种二十面体标定靶和基于环形特征的检测器。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖2D平面标定，缺乏灵活性且难以充分利用新型相机模型。三维重建中重投影误差可能误导三维精度评估，需更可靠的标定方法。

Method: 提出基于场景射线预测的标定框架，使用重建误差和交叉误差作为新型度量指标。结合自举程序统计评估不同标定对象和标定流程（内参和外参）。设计二十面体标定靶配合环形特征检测器。

Result: 广义畸变模型更准确捕捉物理相机效应，标定精度提升。二十面体靶在合成数据上交叉误差降低约40%，自举试验中标定更稳定。实数据性能受制于制造公差。

Conclusion: 射线度量指标比重投影误差更能全面评估三维精度。广义畸变模型更准确捕捉物理相机效应。二十面体靶需严格制造公差才能在实际数据中发挥作用。
...
TL;DR: HelloWorld是一个支持用户与视频世界角色进行社交互动的视频世界模型,用户可通过单次按钮触发角色回应镜头(如转身、挥手、点头或问候)。


<details>
  <summary>Details</summary>
Motivation: 尽管视频世界模型近期取得显著进展,但用户与这些世界中角色之间的社交互动仍然无法实现,作者希望填补这一空白。

Method: 1)提出自蒸馏pipeline,使用模型自身合成的数据进行微调,合成片段包含社交互动和相机运动,学习相机姿态条件化而不降低互动质量。2)引入训练-free模块,在推理时确定互动发生时机,调制DiT的cross-attention masks,使互动相关文本提示仅关注按下窗口内的帧。3)构建包含400样本的HelloWorldBench基准,包含三个社交互动指标和三个传统指标。

Result: HelloWorld在互动质量上超越各种基线,同时保持最先进的画面美学和相机姿态跟随能力。

Conclusion: 该研究成功实现了视频世界模型中的社交互动功能,通过自蒸馏和训练-free调制方法,在保持视频质量的同时实现了自然的角色互动响应。
...
TL;DR: 本文提出了一种弱监督的词袋可视化(BoVW)管道,用于在整张切片图像(WSI)上进行肺腺癌生长模式的空间映射。该方法利用冻结的基础模型嵌入学习视觉词汇表,通过最近原型分类在滑动窗口区域上进行模式映射,产生可解释的空间模式图。


<details>
  <summary>Details</summary>
Motivation: 现有方法在单个瓦片级别操作,产生通用的形态学聚类而非临床定义的模式图,无法解决区域级别的架构上下文问题。

Method: 1)从少量标注的感兴趣区域(ROI)中提取冻结基础模型嵌入;2)学习视觉词汇表;3)将同标签ROI的BoVW直方图构建为模式原型;4)使用Jensen-Shannon散度对滑动窗口区域进行最近原型分类;5)将预测结果投影到WSI瓦片网格上产生空间模式图。

Result: 在87例CPTAC-LUAD患者上,肿瘤/健康分类最佳配置达到0.974的平衡准确率(H-Optimus-1),接近监督SVM的0.987。二元组织学分级分类中,BoVW管道在所有编码器上均优于监督基线。

Conclusion: ROI级别的模式分解保留了被全局均值池化衰减的分级相关异质性,BoVW管道在保持可解释性的同时能够有效进行空间模式映射。
...
TL;DR: SALT是一种自监督预训练方法，通过空间自适应标签引导的温度调节，针对预训练期间可用的弱边界框标注，在病变区域锐化教师网络的softmax温度并增加掩码补丁损失的权重，从而增强模型对小型病变的特征学习能力。冻结编码器后仅训练轻量级检测头，在4个CT队列的3D病变检测中表现优异。


<details>
  <summary>Details</summary>
Motivation: 自监督预训练的目标函数在空间上是均匀的：教师温度和每补丁损失权重在图像各处相同，因此几个补丁大小的病变对训练信号的贡献不超过周围正常组织。先前工作将视图偏向标注区域，但这只是改变了模型看到的内容，没有对目标函数施加额外压力。

Method: 提出SALT方法（Spatially Adaptive Label-guided Temperature，空间自适应标签引导温度）。利用预训练期间可用的弱边界框标注，在编码器的补丁网格上定义一个紧凑区域，在该区域内锐化教师的softmax温度并上调掩码补丁损失权重。目标函数、掩码策略和居中统计量保持不变；在下游使用时，编码器是没有任何标签和条件作用的普通特征提取器。

Result: 通过冻结编码器并仅训练轻量级多深度CenterNet-style检测头，在4个CT队列上进行3D病变检测评估。报告了补丁级可分离性、按队列和病变大小分层的3D检测性能、边界框质量，以及无检测器的探测实验（单个冻结补丁嵌入无需配准、掩码或微调即可重新识别随访扫描中的病变）。

Conclusion: 由于条件作用通过空间指示器而非标签语义表达，该方法可适配任何弱空间标注；并在病变检测任务上得到验证。消融实验表明，与使用相同架构、预训练数据、调度计划和标签引导裁剪但无目标条件的骨干网络相比，该机制具有明显优势。
...
TL;DR: HexMIL（层次化可解释多实例学习）是一种无需掩码的医学深度伪造检测器，仅使用二进制体积级监督即可同时解决生成架构泛化差和缺乏可解释性两大局限。该方法将CT体积分解为两层 patch 和 slice 层次结构，通过门控注意力模块聚合，生成全分辨率3D注意力图来定位伪造区域。


<details>
  <summary>Details</summary>
Motivation: 现有医学深度伪造检测器存在两个关键局限：1）对 unseen 生成架构的泛化能力差；2）缺乏可解释性。这严重威胁临床工作流程。

Method: HexMIL 将每个CT体积分解为两层层次的 patch 和 slice，通过独立的门控注意力模块聚合，其权重直接组合成全分辨率3D注意力图，无需像素级标注即可定位伪造子区域。与Grad-CAM等事后方法不同，HexMIL的注意力权重构成驱动分类决策的前向计算，提供前向且结构忠实的空间归因。

Result: 在M3DSynth和CT-GAN数据集上，采用严格的跨生成器泛化协议（训练时使用单一生成架构，测试时使用未见过的架构），HexMIL在域外分类任务中优于所有基线方法+9.1 AUC和+9.4 F1，并在定位任务中取得最佳平均IoU和Pointing Game分数。

Conclusion: HexMIL成功同时解决了医学深度伪造检测中的泛化性和可解释性两大局限，无需像素级注释即可实现准确的假体检测和定位。
...
TL;DR: 研究视觉变换器(ViTs)如何在训练过程中形成类似生物视觉系统的方向选择性特征，并提出神经科学启发的度量方法来量化这一现象。


<details>
  <summary>Details</summary>
Motivation: ViTs缺乏局部归纳偏置，以全局方式处理信息，与生物视觉系统通过局部区域构建低级特征的方式形成对比。研究旨在探索ViTs是否也能产生类似生物视觉系统的方向选择性特征。

Method: 引入三个神经科学启发的度量：表征相似性分数(RSS)、方向招募分数(ORS)和方向调谐带宽，通过深度分析来量化方向选择性在表征几何中的编码方式。

Result: (1)训练范式是方向选择性的最强决定因素，相同目标的模型在相对相同深度达到峰值；(2)训练早期就有许多单元具有方向选择性，中早期层随时间招募更多选择性单元，更深层失去选择性并拓宽调谐转向语义编码；(3)这些指标为确定下游任务中最佳解冻层数提供了机制启发。

Conclusion: 该框架提供了一种在ViT训练过程中追踪生物接地特征的方法，为理解ViTs如何跨任务泛化提供了系统理解，并可指导迁移学习中的微调策略。
...
TL;DR: 本文提出了一种新的网格演化模型(MT-GNN)，通过预测表面内在几何特性（第一基本形式/度量张量）来预测亚皮层结构的形状演变，避免了传统方法的高维嵌入或直接顶点变形问题。


<details>
  <summary>Details</summary>
Motivation: 现有纵向网格预测方法要么通过高维嵌入外推形状轨迹，要么直接回归顶点变形。支持预后和临床试验优化的亚皮层结构形状演变预测具有重要临床价值。

Method: 提出一种基于图网络的方法，预测未来每顶点第一基本形式（度量张量），可处理任意因果多访史和预测视野，受时间间隔傅里叶编码条件约束。预测的度量通过可微分的尽可能刚性(As-Rigid-As-Possible)求解器解码为表面，模型端到端训练于刚性对齐顶点误差。

Result: 在ADNI数据集14个亚皮层结构上，MT-GNN在所有视野上表现最佳（平均顶点误差-2.29% vs 时间均值，p=6.1×10^-5，在14/14个结构上优于基线），优于DCM（-0.19%）和TransforMesh（-0.45%，p=1.2×10^-4），且优势随视野增大而扩大。

Conclusion: 通过度量张量预测和可微分ARAP解码的端到端训练框架能有效预测亚皮层结构形状演变，性能优于现有方法，为临床预后和试验优化提供了新工具。
...
TL;DR: SmartMage是一个统一的多模态大语言模型，通过动态编排异构模态来实现语义感知的3D场景理解，解决了现有方法忽略查询相关模态需求的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs依赖固定模态组合，忽略查询依赖的模态需求，导致无关模态引入语义噪声、有信息模态未被充分利用，造成计算浪费和推理稀释。

Method: 提出两个核心模块：(1) SMAR语义引导模态自适应路由模块，利用语义先验、文本-模态对齐和模态质量选择任务相关模态；(2) MAGE模态感知门控专家模块，利用模态先验引导专家激活，促进多模态推理中的自适应专业化。

Result: 在5个3D场景理解基准上取得SOTA性能，在RGB-only视频理解基准上获得竞争力结果。诊断基准ScanFacet显示任务按细粒度语义类别划分，分析了各语义类型偏好的模态组合。

Conclusion: SmartMage通过动态模态选择和自适应专家激活，有效提升了3D场景理解的准确性和效率，模态-语义模式分析进一步证实了其有效性。
...
TL;DR: AV-MSF是一种新颖的物体级声学表示方法，通过多视角图像和少量撞击声音记录即可重建。该方法结合3D高斯散射和密集3D视觉特征提供几何感知先验，并使用紧凑的物理意义模态参数表示撞击声场，实现稳健的少样本重建。在两个真实世界数据集上取得了最先进的效果，并支持接触定位和物体声音编辑等下游应用。


<details>
  <summary>Details</summary>
Motivation: 现代3D重建在建模物体几何和外观方面表现出色，但忽略了物理交互中揭示的丰富声学线索。物体撞击声音传达了材料、刚度和结构特性，这些信息可以补充视觉信息。然而，现有的撞击声建模方法要么依赖昂贵的基于物理的模拟，要么需要大型数据集才能进行数据驱动的泛化。

Method: AV-MSF建立在3D高斯散射基础上，集成了密集的3D视觉特征以提供强大的几何感知先验，并使用紧凑的物理意义模态参数表示撞击声场，实现稳健的少样本重建。

Result: 在两个真实世界数据集上的实验表明，AV-MSF实现了最先进的撞击声音渲染，效果优于基于物理和数据驱动的基线方法。

Conclusion: 该方法成功实现了少样本物体级声学表示重建，不仅在撞击声音渲染方面取得突破，还支持接触定位和物体声音编辑等下游应用，为声学-视觉重建领域开辟了新途径。
...
TL;DR: 本文提出上下文组合图像检索(CoCo-IR)任务，允许用户通过多轮交互逐步细化搜索结果。基于大语言模型(LMM)构建上下文感知推理器，生成可转换图像嵌入(TIE)。在CIRCO单轮基准上达到39.4 mAP@5，在4轮对话中达到44.1 R@1，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于指令的图像检索系统功能强大但局限于单轮交互，无法捕捉复杂现实视觉搜索的迭代性质。用户在实际搜索中通常需要多轮交互来逐步细化需求，但现有系统缺乏这种能力。

Method: 1) 提出基于大语言模型(LMM)的新模型作为CoCo-IR的上下文感知推理器；2) 模型解析整个交互历史生成跨轮次演化的可转换图像嵌入(TIE)；3) 开发完全自主可扩展的数据引擎，利用LMM生成高质量上下文检索数据；4) 使用模型引导验证挖掘困难负样本进行训练。

Result: 在CIRCO单轮基准达到39.4 mAP@5；在新的CoCo-IR基准上，4轮对话中达到44.1 R@1，大幅领先现有方法的28.2 R@1。

Conclusion: 成功建立新的最先进性能，验证了CoCo-IR任务的有效性和方法的优势。
...
TL;DR: 本文提出了一个基于冗余调整的人工年龄评分(AAS)的AI系统长期持续性框架，将静态评估指标扩展为循环级功能，生成重复操作下的年龄序列。研究证明AI系统可以无限循环同时保持结构年龄有界，从而为长期人工持续性提供了形式化基础。


<details>
  <summary>Details</summary>
Motivation: 核心问题：AI系统能否在无限期运行而不产生无界结构老化？传统观点认为持续运行必然导致累积退化，本文旨在挑战这一假设，探索AI系统长期运行的理论可能性。

Method: 构建了冗余感知的对数惩罚函数来定义循环级结构年龄，通过加权和冗余调整的方式确保年龄定义良好且一致有界。在此基础上建立了渐近 regime 层次结构，包括负担持续、零负担持续、振荡持续和累积终结负担等模式。

Result: 主要结论：无限循环延续不需要无界结构老化。AI系统可经历无限多次循环而保持结构年龄有界；在更强正则条件下边际老化消失，最强 regime 下循环级负担收敛至零。

Conclusion: 该框架为长期人工持续性分析提供了形式化基础，将长期运行问题从不可避免的累积退化问题转化为有界结构负担问题，具有重要的理论意义。
...
TL;DR: 本文提出了一种结构化验证方法，用于在代理自身状态和自报告不可信的情况下验证长视野代理。该方法的核心是一个确定性执行器拥有所有信念，语言模型只能提交类型化提议，声明只有在预测与观察匹配时才被接受。此外，运行在违反特定阈值时会自动失效，并使用渲染不可见的影子引用来定义漂移度量。实验表明，消融承诺机制会使目标放弃率从0.00变为1.00，而绑定错误保持为0.00。


<details>
  <summary>Details</summary>
Motivation: 解决长视野代理验证的核心难题：当代理自身的状态和自报告完全不可信时，如何建立一个可靠的验证机制。这是一个根本性的挑战，因为传统的事后验证方法无法应对这种信任缺失的情况。

Method: 构建一个结构化的验证仪器：1) 确定性执行器拥有所有信念；2) 语言模型只能提交类型化提议；3) 声明只有在行动前预先注册的预测与观察匹配时才被接受。同时设计了两个关键属性：运行自失效机制（当写入错误、渲染大小或盐化金丝雀回声阈值被突破时失效）和渲染不可见的影子引用（编译完整系统在每个消融单元中原本会提交的方案）。

Result: 消融承诺机制将目标放弃率从0.00翻转为1.00，同时绑定错误保持在0.00（三种种子每单元，最多394个参考节拍每运行）。绑定通道在修复被消融时不会作为每节拍漂移重新出现，因为绑定是代码拥有的，失败类在结构上被吸收。任务效能在52个门控运行中为零（ARC-AGI-3上零级完成）。

Conclusion: 本文的贡献在于为代理开发提供了验证方法论，并使其可测量的漂移分解成为可能。该方法通过结构化设计而非事后验证来解决信任问题，实现了自我验证的科学仪器。八个架构运行中有四个被失效，每个都定位到一个真实缺陷。
...
### [114] [CoPlan: A Trustworthy Co-Intelligence Interface for Care Planning through Role-Based Contestable Argument Graphs](https://arxiv.org/abs/2608.05107)
*Hung Truong Thanh Nguyen,Hélène Fournier,Piper Jackson,Makoto Itoh,Shannon Freeman,Rene Richard,Hung Cao*

Main category: cs.AI

TL;DR: CoPlan是一个用于人类-AI协作护理计划的可争议性界面，采用多智能体工作流程生成护理干预候选方案及支持/挑战性论点，允许人类护理规划者在最终计划生成前接受、拒绝、修改或添加论点。


<details>
  <summary>Details</summary>
Motivation: 当前许多AI系统将推荐作为固定输出呈现，限制了利益相关者检查、挑战和修订计划的能力，当这些计划与临床判断、患者价值观或现实可行性冲突时尤为明显。需要一种能够支持协同智能和可争议性的AI护理计划系统。

Method: CoPlan使用多智能体工作流程，专业AI智能体生成候选干预措施及支持或挑战性论点，而人类护理规划者可以在最终计划生成前接受、拒绝、修改或添加论点。在居家养老护理计划场景中演示了该系统。

Result: 该系统支持自适应护理团队招募、基于角色的论点审查、最终护理计划生成，以及通过调度智能体进行实际后续跟进。

Conclusion: CoPlan贡献了一个可争议的护理计划界面和值得信赖的人类-AI护理计划设计框架，保留了人类自主权和临床问责制。
...
### [115] [Monte Carlo Tree Search for Table-to-Multimodal Report Generation](https://arxiv.org/abs/2608.04071)
...
### [116] [FinProBench: Evaluating Financial AI Agents with Role-Grounded Rubrics Derived from Professional Deliverables](https://arxiv.org/abs/2608.04077)
*Ben Wang,Kang Zhou,Lifan Guo,Feng Chen,Chi Zhang*

Main category: cs.AI

TL;DR: 本文提出FinProBench基准和RGRC方法，通过从业者实际交付物提取评价标准，解决金融AI代理评估中缺乏专业实践标准的问题。RGRC在角色专业化任务中显著优于纯提示方法（99.1% vs 78.0%），且 rubric复用可将构建工作量减少6.7倍。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅从任务提示或模型输出推导评价标准，忽视了从业者交付物中可见的隐性专业标准。金融领域AI代理评估需要与真实专业工作对齐的标准。

Method: 提出角色导向 rubric 构建流程(RGRC)，包含四个阶段：交付物收集、能力提取、rubric合成与验证。将57个职业按交付物类型分为30个先验丰富的传统角色和27个先验稀疏的角色专业化角色。使用异质LLM评判器和角色级rubric进行评估。

Result: 对传统角色，提示工程方法与RGRC性能接近（89.2% vs 90.7%）；对角色专业化角色，RGRC显著优于提示方法（99.1% vs 78.0%）。人类交付物平均得分最高（73.7 vs 70.3, 70.2, 69.6），但各系统95%置信区间重叠。Rubric复用使每任务构建工作量减少6.7倍。

Conclusion: 当惯例在模型先验中有良好表示时，提示工程可近似rubric；但对于先验之外的专业标准，需要基于专业实践的rubric。FinProBench为金融任务评估提供了1,723个交付物、161种交付物类型的资源，RGRC方法可推广至其他专业领域。
...
### [117] [FinPerMA: A Theory-Informed, Event-Grounded Personalized-Memory Benchmark for LLM Agents](https://arxiv.org/abs/2608.04095)
*Ben Wang,Kang Zhou,Lifan Guo,Feng Chen,Chi Zhang*

Main category: cs.AI

TL;DR: FinPerMA是一个事件驱动的个性化记忆基准测试，评估LLM智能体在金融领域长期用户建模能力，测试其是否能整合重大事件到持久用户模型中


<details>
  <summary>Details</summary>
Motivation: 现有个性化记忆基准主要测试事实保留或依赖弱约束的模型生成轨迹，对事件驱动的偏好适应探索不足

Method: 生成管道结合确定性理论影响的规则、受控LLM叙述和自动化质量筛选；Post-Shock检查点隔离智能体是否将重大事件整合到持久用户模型中

Result: 2994个问题来自276个角色，七个前沿LLM和七种记忆配置远未饱和：没有任何全上下文配置超过约0.47的整体准确率或约39%的多项选择题准确率

Conclusion: 基于摘要的记忆往往保留事实细节但丢失个性化所需的偏好信号；简单检索因此可以优于专用记忆系统，差距在冲击后扩大
...
### [118] [BrainBench: Benchmarking Large Language Models for Comprehensive EEG Understanding](https://arxiv.org/abs/2608.04156)
*Yangxuan Zhou,Sha Zhao,Yuning Chen,Chen Wu,Jiquan Wang,Shijian Li,Gang Pan*

Main category: cs.AI

TL;DR: 论文提出了EEGBench，一个用于评估大型语言模型(LLM)综合脑电图(EEG)理解能力的统一基准测试平台。该基准包含四个子集(基础分析、睡眠评估、神经认知评估和生理整合)，涵盖17个数据集和多个任务，旨在评估LLM在给定指令和EEG数据后生成科学合理报告的能力。


<details>
  <summary>Details</summary>
Motivation: 现有EEG分析评估主要针对孤立解码任务，缺乏对LLM综合EEG理解能力的量化评估。该研究旨在建立统一的基准测试，以系统性地评估LLM在EEG领域的理解和分析能力。

Method: 构建包含17个数据集、多个任务和大量真实数据实例的基准测试平台，采用6种验证方式(数值、分类、集合、序列、语义和 artifact validation)，在两个范式(CodeAct自主代码执行和BrainAgent结构化代理分析)下对多个代表性LLM进行评估，总计超过100K次执行。

Result: 不同模型、子集、难度级别和执行范式之间的结果存在显著差异，表明EEG能力取决于模型及其操作化方式。测试的LLM在EEG理解方面表现不一，存在较大提升空间。

Conclusion: EEGBench为推进基于LLM的EEG理解提供了可复现的测试平台。不同LLM的表现差异显著，表明该领域需要进一步研究和改进。代码和基准将陆续发布，评估结果将持续更新。
...
### [119] [Adversarially Robust Abductive Fusion of Pre-trained Transformer-based Perception Models](https://arxiv.org/abs/2608.04190)
*Mario Leiva,Yue Ma,Qinru Qiu,Gerardo Simari,Paulo Shakarian*

Main category: cs.AI

TL;DR: 本文提出一种无领域知识的元认知层学习方法，通过利用向量空间几何（标签向量池LVP）来检测预训练感知模型的误差，并将多个不完美ViT检测器的融合建模为一致性溯因问题，通过整数规划或多项式时间启发式算法求解。


<details>
  <summary>Details</summary>
Motivation: 在未知环境中部署预训练感知模型会因分布偏移导致精度下降，现有的组合器（如多数投票）存在召回率与精确率的权衡问题，且对协同失效脆弱。之前的元认知方法依赖手工领域知识（目标大小先验、分割掩膜），无法迁移到真正的新场景。

Method: 1. 提出标签向量池(LVP)：从每个模型的训练嵌入构建，检测几何相对于训练确定的原型<br>2. 从检测几何中学习误差检测规则，实现无领域知识的元认知层<br>3. 将多ViT检测器融合表述为一致性溯因问题<br>4. 使用精确整数规划(IP)和多项式时间启发式算法在测试时求解

Result: 1. 在测试集上F1与领域知识规则达到0.002以内的 parity<br>2. 在干净数据上与最强多数投票变体F1差距在0.005以内<br>3. 协同标签翻转攻击下（90%翻转率）：平均F1为0.42 vs MV-Plurality的0.35（相对提升22%）<br>4. 翻转率超过0.4时在所有测试集上均获得最高F1

Conclusion: 所提出的无领域知识元认知层通过几何方法学习误差检测规则，可与领域知识互补。在分布偏移和对抗性攻击下显著优于多数投票基线，为鲁棒的多模型感知融合提供了新范式。
...
### [120] [MatrAIx: Simulating the World with 8.3 Billion Persona Agents](https://arxiv.org/abs/2608.04205)
...
TL;DR: MatrAIx是一个大规模人口模拟用户评估基础设施，用于使用异质用户测试AI系统和数字产品。它包含三个核心组件：Persona 8B（83亿人物角色记录，1290个分类维度）、MatrAIx Playground（四个评估环境：调查、AI聊天机器人、Web和App）以及1010个应用任务（跨越25+个领域）。该系统通过模拟多样化的用户行为和偏好，为AI系统评估提供了可扩展的解决方案。


<details>
  <summary>Details</summary>
Motivation: 人类对AI系统和数字产品的评估成本高、速度慢且难以扩展。离线评估虽然更可扩展，但往往抽象化了人类多样性和交互行为。因此需要一种可扩展的解决方案，能够保留人类多样性并模拟真实交互行为。

Method: 1) 构建Persona 8B：83亿人物角色记录，包含1290个分类维度，记录来自保留相关属性的依赖图采样或人类撰写的人物画像；2) 发布约100万个经过质量过滤的角色集（599,847个基于人类的记录+40万个合成记录）；3) 建立MatrAIx Playground四个环境；4) 在8个代表性任务中进行18,189次评估试验；5) 使用三种LLM驱动角色代理（Claude Opus 4.8、GPT 5.5、Claude Haiku 4.5）；6) 进行两项验证研究：400次对照研究和人类/LLM评判评估。

Result: 1) 反馈捕捉了决策和偏好如何随人物背景变化，包括价格增加后的犹豫、AI助手失败后是否继续的意愿以及延迟容忍度；2) 角色遵守性验证：在400次对照研究中，10个行为属性在四种环境中的声明行为在366次试验中表达或正确抑制（91.5%）；3) 人类和LLM评判验证了基于人类的角色提取质量。

Conclusion: MatrAIx提供了一个端到端基础设施，使用多样化模拟人类用户评估AI系统和数字产品，能够捕捉不同背景用户的决策和偏好差异，为AI系统评估提供了一种可扩展且保留人类多样性的方法。
...
### [121] [Interoceptive Attention as Dynamic Homeostatic Prioritization in a Foraging Agent](https://arxiv.org/abs/2608.04232)
...
TL;DR: 研究生物系统如何在有限感知带宽下在不同需求间分配感知精度的问题。通过主动推理框架建模觅食代理，根据身体需求动态重新分配固定的内感受精度预算。在四通道网格世界(AffectWorld)中，这种选择性分配使生存率提高一倍多(0.414 vs 0.199)，且收益同时来自感知和规划过程。


<details>
  <summary>Details</summary>
Motivation: 生物系统必须在有限感知带宽下调节竞争性需求——强化某一估计会削弱其他估计的能力。固定预算系统需要决定将感知精度分配到哪里。这一问题源于感知资源的本质性限制——生物体无法同时精确感知所有信息，必须做出权衡。

Method: 使用主动推理(Active Inference)框架建模觅食代理。代理在每一步读取自身身体状态信念，识别最需要的通道，并将固定的内感受精度预算重新分配给它，使相同的精度塑形似然同时用于信念更新和规划。在AffectWorld四通道网格世界中进行测试，使用11种布局、每种32个随机种子进行配对聚类bootstrap检验。

Result: 选择性精度分配在匹配预算下使学习阶段生存率提高一倍以上(0.414 vs 0.199，p≤10^-4)。收益约一半来自规划过程(剥夺规划者的塑形似然损失约一半收益)。精度分配需与需求对齐——将精度分配给最不需要的通道反而比均匀分配更差。被注意通道对自身动力学的学习速度提高约两倍。

Conclusion: 生物体通过动态路由感知精度来优化多需求管理是可行的。这种精度路由机制同时作用于感知和规划，且与身体需求对齐。注意力通道表现出更快的学习速度，这是该机制的行为痕迹。该研究为理解生物系统如何管理有限感知资源提供了计算框架。
...
### [122] [The RAIL Principles for Neurosymbolic AI: Reasoning, Assurances, Interfacing and Learning](https://arxiv.org/abs/2608.04285)
...
TL;DR: 神经符号AI系统结合机器学习和符号推理，能够弥补纯神经网络数据驱动方法的不足，可应用于高风险领域和低数据场景。论文提出RAIL框架（推理、保障、接口、学习）作为分析AI系统的统一视角，将多种AI技术纳入神经符号范畴，包括物理感知机器学习、神经引导搜索、因果学习和工具增强的大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统设计需要重新审视，神经符号方法不仅是AI中的一个细分领域，而是支撑可靠、高效、可信AI系统的关键技术基础。

Method: 提出RAIL四原则框架（Reasoning推理、Assurances保障、Interfacing接口、Learning学习），从这四个维度分析各种AI系统。

Result: 证明许多领先AI系统（包括非传统意义的神经符号系统）都可通过RAIL框架统一分析，涵盖物理感知机器学习、Alpha-*系列、因果学习、工具增强LLM等。

Conclusion: RAIL框架能帮助工程师在生产级AI系统的设计和部署中做出更明智、更有原则的决策，指导将神经符号方法整合到下一代AI技术中。
...
### [123] [SafeCommit: Certifying When Memory-Grounded Agents May Safely Act](https://arxiv.org/abs/2608.04289)
*Mayur Akewar,Ravi Ranjan*

Main category: cs.AI

TL;DR: 针对长视野智能体过早承诺的安全问题，提出SafeCommit框架，在智能体推理和外部执行之间构建风险控制层，通过构建校准的潜在世界集合来确保行动安全。


<details>
  <summary>Details</summary>
Motivation: 解决智能体在内存接地状态不确定（过时、冲突、不完整或损坏）时过早行动的核心失败模式。

Method: 构建校准的潜在世界集（包含记忆、观察、工具输出、来源和政策约束），使用共形动作证书确保行动在所有保留世界中安全，否则选择低副作用探测或保守回退。

Result: 在标准校准世界覆盖下，不安全认证承诺的概率不超过目标水平α；世界提议不完美时，界分离校准误差和表示误差。

Conclusion: 提供了一种具体方法，不仅决定智能体应该做什么，还决定何时有足够证据安全地执行行动。
...
### [124] [NeuMoSync: End-to-End Neuromodulatory Control for Plasticity and Adaptability in Continual Learning](https://arxiv.org/abs/2608.04358)
...
### [125] [Improving Auto-Design of Neural PDE Solvers with a Domain-Specific Language](https://arxiv.org/abs/2608.04384)
*Shengxin Kong,Liwen Xu,Jingwen Fu*

Main category: cs.AI

TL;DR: ADSL-PDE是一种神经偏微分方程(PDE)求解器自动设计框架,通过引入结构化搜索状态表示,在求解器概念和可执行代码之间建立中间层,避免直接生成Python程序带来的大量实现失败。


<details>
  <summary>Details</summary>
Motivation: 在无限制Python程序空间中,有效的求解器是极其稀疏的子集——大多数候选程序在语法上不正确、语义上不兼容或数值上不稳定。直接代码生成使LLM将大部分搜索能力用于导航实现失败,而非推理求解器质量。

Method: ADSL-PDE引入了结构化搜索状态,表示决定神经PDE求解器的功能决策(架构、物理约束、目标、采样和优化),同时抽象掉低级实现细节。确定性编译器将每个有效搜索状态映射到可执行求解器。在此基础上,进化代理迭代提出、评估和优化求解器搜索状态。

Result: 在多个PDE基准测试中,ADSL-PDE提高了搜索效率和优化稳定性,在前十次进化迭代中实现了超过52%的改进。

Conclusion: 结果表明:有效的LLM驱动自动设计不仅需要更强的推理能力,更需要一种将探索集中在有效和关键决策上的搜索表示。这一原则可能推广到更广泛的自动设计领域。
...
### [126] [Architectural Implications of Agentic AI Workflows](https://arxiv.org/abs/2608.04458)
*Jirong Yang,Peizhe Liu,Chaojie Zhang,Jovan Stojkovic*

Main category: cs.AI

TL;DR: 本文首次对数据中心中Agentic AI的架构影响进行了系统性研究，通过在Microsoft Azure的生产研究和开源框架的对照研究，揭示了Agentic执行的高度碎片化和异构性特征，并提出了针对Agentic AI服务器的架构设计建议及其原型系统Agora。


<details>
  <summary>Details</summary>
Motivation: Agentic AI正在数据中心中兴起，但其架构影响尚未被充分探索。现有的统一服务器架构无法有效应对Agentic工作负载的碎片化、异构性和突发性特征，导致CPU和GPU资源利用率低下。

Method: 1) 建立Agentic工作流的分类学体系；2) 在Microsoft Azure进行生产研究；3) 对开源框架进行对照研究；4) 设计并实现原型系统Agora进行验证。

Result: 1) Agentic执行是碎片化和异构的，请求扩展为包含LLM推理、工具调用和编排决策的工作流；2) CPU处于关键路径上；3) 执行结构导致负载随时间呈现低水平但突发 spikes的特征；4) 模型组合影响GPU使用的均衡性；5) 任务和工具的多样性进一步扩大了这一范围；6) Agora在保持Agent尾部延迟的同时提高了资源利用率和服务器吞吐量。

Conclusion: 传统统一服务器架构与Agentic AI工作负载存在根本性的架构不匹配。碎片化执行导致资源浪费，同构CPU配置效率低下，多路复用Agents会损害微架构局部性。需要动态Harvest空闲CPU核心、GPU内存超订阅、核心池化、亲和性感知调度等新型架构方案来提升Agentic AI服务器的性能和资源效率。
...
### [127] [CARGO-VL: Counterfactual Arbitration with Risk-Constrained Group Optimization for Vision-Language Models](https://arxiv.org/abs/2608.04509)
*De Jiang,Zhengyang Zhang,Kehong Yuan,Shaohua Ma*

Main category: cs.AI

TL;DR: CARGO-VL是一个组相对框架,通过优化匹配变体(A/V/T/N四种证据状态)来处理视觉-语言系统中的证据冲突问题,并引入XMC冲突训练资源和CMCBench/Modality-Bias评估基准。


<details>
  <summary>Details</summary>
Motivation: 视觉语言系统中图像和检索文本可能不一致或都无法支持答案,现有后训练目标独立评分实例,无法在反事实证据变化下强制一致行为,需要一种能识别可信来源并在两者都不足时弃权的可靠模型。

Method: 提出CARGO-VL框架,将对齐、图像正确、文本正确、两者都错(A/V/T/N)四种证据状态作为捆绑优化;目标函数结合条件wise正确性与答案不变性、来源等变性、答案到弃权切换的过渡奖励;使用原始-对偶控制器平衡不安全答案与过度 deferral。

Result: 在CMC-Bench和Modality-Bias上评估,多种子实验表明CARGO-VL在冲突处理、不支持答案避免和模态平衡方面优于点wise基线。消融实验验证了关系过渡信号和自适应风险控制的互补效益。

Conclusion: 反事实一致性可作为可靠多模态证据仲裁的实用目标,为视觉-语言系统的可信度提升提供了新思路。
...
### [128] [Leak-Resistant Unlearning: A New Benchmark for Evaluating Multi-Hop Reasoning Consistency and Recovery Robustness](https://arxiv.org/abs/2608.04519)
*Haoting Qian,Qingjie Zhang,Zhicong Huang,Cheng Hong,Han Qiu*

Main category: cs.AI

TL;DR: 本文提出了一个新的LLM知识遗忘基准测试框架，旨在评估语言模型中敏感知识的移除效果，特别关注多跳推理路径和恢复攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘基准测试主要包含单跳问题和少量多跳问题，存在两个关键挑战：(1)知识并非孤立存在，多样化的多跳推理路径可能导致知识泄漏；(2)遗忘方法可能脆弱，遗忘后的知识可通过轻量级后遗忘适应等恢复攻击部分恢复，使得静态评估不足。

Method: 构建了一个名为\unlearning的新型基准测试，在3个模型、6种遗忘方法和2个精心设计的数据集上进行了实验评估。

Result: 实验结果表明现有方法在多跳推理路径和恢复攻击面前表现脆弱，存在明显的安全隐患。

Conclusion: 研究进一步探讨了遗忘质量、鲁棒性和模型效用之间的权衡关系，为LLM遗忘技术的安全部署提供了重要参考。
...
### [129] [What Is a Skill Worth? Structure-Aware Shapley Valuation of Agent Skills](https://arxiv.org/abs/2608.04562)
*Tao Li,Junfeng Liu,Qinghua Zhao,Yifan Li,Lei Wang,Bo Shao,Xuejun Liu,Linjun Shou*

Main category: cs.AI

TL;DR: SkillSV是一个结构感知的Shapley风格框架，用于对AI智能体技能中的内部单元（如规则、示例、脚本、启发式）进行价值评估，解决传统数据评估无法处理的单元依赖、文档层次和上下文消耗问题。


<details>
  <summary>Details</summary>
Motivation: 随着自动化反馈循环优化的智能体技能日益复杂，产生大量结构化工件，但其内部价值不明确。现有数据或提示片段评估方法无法处理技能单元的结构化特性：单元间可能存在依赖关系、属于文档层次结构、可触发智能体行为，并消耗有限的提示上下文。

Method: SkillSV将技能编译为单元、依赖关系和层次结构，确保只评估有效的反事实技能；使用配对删除和长度中性填充来分离内容价值与上下文成本；使用 rollout 预算估计器处理有噪声的智能体评估。

Result: 在四个智能体基准测试中，SkillSV能够恢复单元交互、保留技能提升效果，并指导安全剪枝和压缩。

Conclusion: SkillSV框架具有忠诚度、可操作性和可解释性，为理解和优化AI智能体技能提供了有效工具。
...
### [130] [Joint UAV Flight and Opportunistic Routing under Reinforcement Learning for Delay-Tolerant Networks](https://arxiv.org/abs/2608.04590)
*Xiao Wang,Shun-Ren Yang*

Main category: cs.AI

TL;DR: 本文研究延迟容忍网络(DTN)中结合无人机(UAV)飞行控制的机会路由联合优化问题。提出JUROR框架，基于近端策略优化(PPO)和集中训练分散执行(CTDE)架构，实现去中心化的机会路由与无人机航向控制协同，以扩大未来接触机会并提高消息传递率。


<details>
  <summary>Details</summary>
Motivation: DTN中稀疏连接、间歇性接触、有限缓冲区和消息TTL限制导致消息传递率低和网络拥塞问题。现有研究主要单独优化路由或移动性控制，缺乏对两者的联合优化设计。

Method: 将问题建模为因子化部分可观测马尔可夫决策过程(POMDP)，包含顺序运动-路由耦合和每步团队奖励；采用PPO框架，分布式执行器基于本地观察行动，训练时评论家使用全局统计，可选多 horizon热点预测器提供辅助监督。

Result: 在四种流量模式下，JUROR相比PRoPHET和MaxProp算法取得显著性能提升，同时保持接触受限的去中心化执行特性。

Conclusion: 通过联合优化无人机飞行与机会路由，在CTDE框架下可有效解决DTN中稀疏连接和拥塞问题，验证了分散式执行结合集中训练的可行性。
...
### [131] [Agreement Before Diversity: Verification-First Complementarity for Heterogeneous Language-Model Coordination](https://arxiv.org/abs/2608.04618)
*Ruitong Li,Binjie Guo,Aisheng Mo,Guowei Su,Jie Li,Ru Zhang*

Main category: cs.AI

TL;DR: 本文提出Agreement-Before-Diversity (ABD)方法，通过解耦候选扩展与替换授权，在不依赖独立性和校准置信度假设的情况下，为异构语言模型集成提供了一种原则性的决策规则。ABD在LiveCodeBench-v6和GPQA-Diamond上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 异构语言模型集成扩展了候选响应空间，但缺乏一个原则性的标准来决定新生成的答案何时应该取代已支持的答案。需要一个明确的、可审计的替换授权机制。

Method: 提出Agreement-Before-Diversity (ABD)方法：冻结的、无标签的决策规则。如果两个额外的可信样本在固定等价关系下 corroborate（证实）锚定答案，则保留锚定答案；否则被异构综合取代。证明两个精确恒等式：(1)相对于无条件综合的准确率差距由协议覆盖率和锚定答案在保护子集上的优势共同决定；(2)相对于永不综合的差距反映授权恢复与授权破坏之间的对比。不假设独立性或校准置信度。

Result: 在LiveCodeBench-v6上达到59.43%（Single9为52.57%，HAC为52.00%，n=175）；在GPQA-Diamond上达到75.00%（两个对照均为72.78%，n=180）。这些恒等式将每个 aggregate 差异本地化到可枚举的保护层：LiveCodeBench上3个保护案例中无不一致项；GPQA-Diamond上132个案例中13对8；冻结锚定扰动下71个案例中12对0。

Conclusion: 多样性提供潜力，验证结构提供授权。ABD方法通过解耦候选扩展与替换授权，实现了原则性的决策，在基准测试中展现了显著的性能提升。
...
### [132] [A/B Agent: A Self-Evolving Agent for Strategy Iteration in Industrial A/B Testing](https://arxiv.org/abs/2608.04625)
*Zhuohang Jiang,Yuxin Chen,Yongsen Pan,Zheng Hu,Wenqi Fan,Qing Li,Hongyang Wang,Jun Wang,Wenwu Ou*

Main category: cs.AI

TL;DR: 本文提出了A/B Agent，一个用于工业推荐策略优化的闭环A/B智能体框架。该框架通过层次化经验树组织历史策略，利用多路径Tree-RAG检索可迁移证据生成策略，并通过对在线A/B反馈的分析实现策略的自主调优和自进化。


<details>
  <summary>Details</summary>
Motivation: 工业推荐策略迭代严重依赖大规模A/B测试，传统调优需要专家反复设计策略、配置实验、分析结果和调整参数，过程 labor-intensive 且耗时。同时，历史实验的知识往往是碎片化的，难以通过人工专家努力实现系统化复用。现有的RAG智能体虽然部分缓解了这一负担，但通常以扁平方式组织经验，忽视了业务场景、推荐阶段、优化目标和实验上下文之间的层次关系，导致检索不匹配和跨场景迁移受限，且无法通过顺序A/B反馈持续优化策略和参数。

Method: 提出A/B Agent框架，包含三个紧密耦合的核心组件：1) 历史策略知识组织：将历史策略组织为层次化经验树；2) 自主目标感知策略生成：通过多路径Tree-RAG检索可迁移证据生成可执行策略；3) 实验引导策略自进化：持续分析在线A/B反馈以指导自主调优并更新经验树实现自进化。

Result: 在真实短视频电商推荐系统中进行了广泛的离线和在线评估。实验结果表明：A/B Agent实现了GMV提升4.829%，同时在所有护栏指标上保持正向收益。

Conclusion: A/B Agent通过层次化经验树和多路径Tree-RAG有效解决了传统推荐策略迭代中知识碎片化和人工依赖的问题，实现了从经验复用、策略生成到自进化的闭环优化，为工业推荐系统的策略迭代提供了新的研究方向。
...
### [133] [AI Literacy for Legal Translation: Developing Digital Resilience](https://arxiv.org/abs/2608.04641)
*Łucja Biel*

Main category: cs.AI

TL;DR: 本文探讨了生成式AI对法律翻译的影响，提出了一个包含四个维度（基础性、程序性、批判性、战略性）的AI素养框架，旨在培养法律翻译人员的数字韧性。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在带来机遇的同时，也引入了语言、技术、法律、伦理和认知方面的风险，需要重新审视法律翻译专业能力的构成。

Method: 本文采用理论分析框架，提出AI素养框架，并通过设计课堂活动来验证其教学可行性。

Result: AI不改变法律翻译的根本目标，但需要通过AI素养来扩展专业能力，AI素养被视为法律翻译能力中促进数字韧性的横向组成部分。

Conclusion: 提出的AI素养框架可帮助未来译者以批判性、负责任的态度整合AI，符合专业标准，为法律翻译教育提供了具体的教学路径。
...
### [134] [Calibrating Artificial Guilt: Neurally Grounded Reward Shaping for Prosocial Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2608.04663)
*Aaditya Mehta,Arya Shah*

Main category: cs.AI

TL;DR: 研究探索是否可以从人类神经和行为数据中校准"内疚信号"并转移到人工智能agent。使用SoDec责任fMRI数据集（40名参与者），通过回归分析恢复内疚权重，并将其嵌入双人社会彩票环境中训练PPO算法。结果显示校准后的agent最接近人类的社会安全选择率。


<details>
  <summary>Details</summary>
Motivation: 合作多智能体强化学习通常在个体奖励中添加社会项，但这些项的规模通常是手动选择的。研究者提出问题：是否可以从人类神经和行为数据中校准内疚信号并转移到人工agent。

Method: 使用公开的SoDec责任fMRI数据集（40名参与者），对即时幸福感变化与结果类型计数进行受试者固定效应回归，从"合作伙伴-负面"减去"社会-负面"对比中恢复内疚权重（ŵ=1.118，Cohen's d=0.214）。将该权重嵌入双人社会彩票环境，训练四种塑造方案下的独立近端策略优化（PPO）actor-critic：神经校准、统一常数、零（自私）和单位系数oracle。

Result: 在每种条件1000次评估中，校准后的agent最接近人类的社会安全选择率（0.459 vs 人类0.484，KL=0.0012），而其他三种条件的KL偏差大一到三个数量级。

Conclusion: 人类神经行为先验可以作为亲社会奖励塑造的定量约束，为强化学习中的社会奖励设计提供了基于人类神经数据的校准方法。
...
### [135] [Traceable LLM-Generated Hazard Scenarios for Operational Safety Analysis of Aviation Systems Using ASRS Reports](https://arxiv.org/abs/2608.04697)
*Cristian Mascia,Roberto Pietrantuono,Daniel Rodriguez,Stefano Russo*

Main category: cs.AI

TL;DR: 本文提出了一种AI辅助方法，从NASA航空安全报告系统(ASRS)生成候选危险场景。该方法针对特定不良结果，生成包含分类因素的结构化假设和描述操作事件序列的叙事场景，并附带基于历史共现证据的可信度评分和与ASRS报告的溯源链接。


<details>
  <summary>Details</summary>
Motivation: 航空系统操作的危险分析需要考虑天气、ATC行动、空域约束、飞机操作和人为因素的交互作用，这与飞机系统级别的功能危险评估不同。现有方法在生成结构化假设和叙事场景方面存在不足。

Method: 1) 基于ASRS数据生成候选危险场景；2) 产生结构化假设(分类因素)和叙事场景；3) 使用历史共现证据计算可信度评分；4) 建立与保留ASRS报告的溯源联系；5) 提出混合变体，通过进化溯因生成结构化假设来条件化叙事生成；6) 评估多种大语言模型、零样本与少样本提示策略及微调效果。

Result: 通过比较不同LLM、提示策略和微调，测量其对生成结构和叙事有效性及真实性的影响，验证了方法在正确性和减少变异性方面的改进。

Conclusion: 所提出的AI辅助方法能够有效生成航空危险场景假设，结合进化溯因的混合方法可提高叙事生成的正确性并降低变异性，为航空安全分析提供了新的技术手段。
...
### [136] [Diagnosing Tool-Selection Reasoning in LLM Agents with Canary Tools](https://arxiv.org/abs/2608.04719)
*Atul Anand,Sourav Chattaraj*

Main category: cs.AI

TL;DR: 本研究引入"Canary tools"（金丝雀工具）来诊断AI智能体在工具选择时的弱点，通过六类探针工具（语义诱饵、参数陷阱、能力幻象、先决条件盲点、时间诱饵、粒度陷阱）分析模型推理过程。评估8个模型在8640次运行中的表现，发现能力越强的模型 susceptibility 越低，但能力等级本身不能预测安全性，且不同探针类型对不同能力层级的模型效果不同。


<details>
  <summary>Details</summary>
Motivation: 当前Agent评估只能告诉模型选错了工具，但无法揭示为什么错。研究动机是创建一种诊断工具来探测模型在工具选择时的具体弱点，将单一的"错误工具"结果转化为多维度的推理分析。

Method: 在MCP工具集中植入6类金丝雀探针工具，评估8个模型（6个托管模型+2个8B开源模型）在120个任务上、3种金丝雀密度条件、3个随机种子下的表现（共8640次运行），另有2880次subtlety ablation。使用独立评判员评估任务成功率（第二独立评判员验证，Cohen's kappa=0.75）。

Result: 三个主要发现：1) susceptibility随能力提升急剧下降，CSR范围跨度36x；2) 能力等级本身不能预测安全性，最易受攻击的托管模型是中等层级；3)  taxonomy具有能力分层特性：能力幻象最可靠地捕获前沿模型，其他类型对强模型基本无效但能捕获小型开源模型。软化" giveaway phrase"后前沿CSR基本不变，说明探针测量的是推理而非短语识别。Susceptibility还预测任务失败（Spearman rho=-0.34），最稳健的模型不会因金丝雀压力而显著降级。

Conclusion: 金丝雀工具框架能有效诊断模型的工具选择推理弱点，不同探针类型具有能力分层特性，可用于评估和提升模型安全性。
...
### [137] [When Prompts Become Pixels: Prompt-Region Grounding for Multimodal Reasoning](https://arxiv.org/abs/2608.04726)
*Yongxin Wang,Ruizhe Zhou,Yueling Tang,Yingying Zhu,Xuemin Zhao,Xiaojun Chang,Xiaodan Liang*

Main category: cs.AI

TL;DR: 本文提出Visualized Task Semantics (VTS)，将问题移入图像而保持题目和答案不变。跨6个MLLMs和4个基准测试，24个模型-任务配对中准确率全部下降，平均下降17.8分。模型常能正确转录视觉问题却无法使用，揭示了超越OCR的语义通道差距。提出prompt-region grounding方法，将问题区域与类型语义对齐并从掩码视图中恢复干净表示。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型日益对截图和文档进行推理，任务本身可能以像素形式呈现。但基准测试通常将问题放在文本中，不清楚模型是否在不同通道中同等使用指令。

Method: VTS控制干预：将问题移入图像，同时保持源题目和答案固定。提出prompt-region grounding，其核心设计是将问题区域与类型语义对齐，并从掩码视图中恢复其干净表示。

Result: 在所有24个模型-任务配对中准确率均下降，平均下降17.8分。该方法在匹配训练成本下，将四基准VTS准确率从58.0提升至66.3，同时保持原始界面准确率，且推理时无需OCR或区域元数据。

Conclusion: 阅读任务承载文本与将其作为推理指令接地是两种不同能力。模型能正确转录视觉问题却无法使用，揭示了超越OCR的语义通道差距。
...
### [138] [Chain-of-Thought Monitoring Can Be Unreliable in Implicit-Influence Settings](https://arxiv.org/abs/2608.04735)
*Agatha Duzan,Asa Cooper Stickland*

Main category: cs.AI

TL;DR: 该研究提出了首个直接比较显性影响和隐性影响下思维链（CoT）可监控性的基准测试。研究发现显性影响下监控检测率达60-94%，但隐性影响下检测率下降41-46个百分点，系统提示词添加甚至可将检测率降至5%。


<details>
  <summary>Details</summary>
Motivation: 当前CoT监控评估主要研究显性影响设置（提示词直接要求模型隐藏信息），但缺乏对隐性影响设置的研究。隐性影响指提示词无隐藏指令，但任务或上下文特征仍能影响模型行为。

Method: 构建基准测试比较两种影响机制，通过四种任务格式（二元选择、数值评分、多选问答、开放式编码）和七个前沿推理模型测试模型行为在 nudge 影响下的变化。nudge 以随意评论（隐性）或直接指令（显性）方式呈现。

Result: 显性影响下CoT监控器可检测60-94%的行为变化，即使模型被指示隐藏仍会在思维链中泄露。隐性影响下检测率下降41-46个百分点。开发者部署的系统提示词（减少无关偏差）进一步降低隐性检测至5%，但行为影响本身保持不变。

Conclusion: 显性影响设置下的监控性评估可能高估实际监控能力，善意部署选择会进一步降低监控性。研究提供了基准测试代码以供复现。
...
### [139] [EviGraph: Evidence-Guided Autonomous Research Agents](https://arxiv.org/abs/2608.04738)
*Zhenjiang Ren,Ruiji Li,Xujing Zhang,Ziliang Pang,Shuo Ren,Jiajun Zhang*

Main category: cs.AI

TL;DR: EviGraph是一个自主研究框架，通过将研究过程表示为包含Problem、Gap、Hypothesis、Experiment、Finding和Claim节点的类型化证据图来解决自主研究代理输出中缺乏支持的主张和不一致的问题。该框架将图作为代理的操作状态而非事后记录，检查证据链中的缺失依赖、语义不对齐和结果-主张不一致，定位最早薄弱节点并重新生成其受影响的下游子图。实验表明EviGraph在研究性能上优于基线，主张支持率提高40.19%，实验数据一致性达87.73%。


<details>
  <summary>Details</summary>
Motivation: 自主研究代理可以生成假设、执行实验和起草手稿，但其输出往往包含不支持的主张以及研究问题、实验、结果和结论之间的不一致。这种问题部分源于架构问题：现有系统将研究组织为顺序管道，但没有明确维护或验证各阶段之间演变的声称-证据结构。

Method: 提出EviGraph框架，将研究过程表示为包含Problem、Gap、Hypothesis、Experiment、Finding和Claim节点的类型化证据图。框架将图作为代理的操作状态而非事后记录进行检查：1)检查证据链的缺失依赖、语义不对齐和结果-主张不一致；2)定位最早薄弱节点；3)重新生成受影响的下游子图。图形检查点防止不成功修复破坏先前验证的证据。手稿仅在每个保留主张都有经过验证的证据链支持时才生成。

Result: 在ARC-Bench-ML和NanoResearch-20上的实验表明：EviGraph在整体研究性能上优于比较的端到端研究代理基线；主张支持率比最强基线提高40.19%；实验数据一致性达到87.73%。

Conclusion: 结果表明，显式证据状态维护对于可靠的自主研究具有重要价值。该框架通过图结构有效解决了自主研究代理输出中的一致性和支持性问题。
...
### [140] [Fewer Tokens, Smaller Cache: Reward-Coordinated Efficient Reasoning](https://arxiv.org/abs/2608.04771)
*Qiyuan Zhu,Dezhi Li,Pengyu Cheng,Tianle Chen,Jiacheng Wang,Ruijie Shen,Hao Gu,Sida Lin,Zirui Liu,Jiacheng Liu,Sirui Han*

Main category: cs.AI

TL;DR: ReCo是一种奖励协调的压缩框架，通过轻量级过程奖励估计器驱动三个组件：奖励自适应KV缓存压缩、反思token奖励惩罚和基于置信度的早停，在三个推理模型和六个基准测试上减少生成的token 37%-65%，端到端延迟降低2.08x-2.35x，同时大幅保持准确率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过长链式思考完成复杂任务，但冗长的中间步骤导致严重过度思考，增加推理成本。现有推理导向方法在整个轨迹上应用统一策略，仅根据从缓存中删除的内容判断压缩效果。两个观察点表明新方向：一是推理状态对上下文丢失的容忍度沿轨迹变化，过程奖励可追踪：删除高奖励步骤的token比随机删除相同预算更能保持准确率；二是压缩在生成侧并非免费，因为更小的缓存导致模型生成更多token，部分抵消了节省效果。

Method: 提出ReCo（奖励协调压缩），一个逐步框架，使用轻量级过程奖励估计器对每个完成的步骤进行评分，并驱动三个组件：（1）奖励自适应KV缓存压缩，在高奖励步骤更大幅度缩小保留缓存，在低奖励步骤减少压缩；（2）反思token的奖励带惩罚，抑制冗余生成；（3）基于置信度的早停，当推理可靠时触发。

Result: 在三个推理模型和六个基准测试中，ReCo相比完整CoT减少生成的token 37%-65%，端到端延迟降低2.08x-2.35x，同时大幅保持准确率。

Conclusion: ReCo通过协调压缩和生成两侧，在单一过程奖励下有效降低推理成本，同时保持准确率，为大型推理模型的高效部署提供了有效方案。
...
### [141] [NSF-HRPT: Neural Semantic Field meets Hierarchical Risk Perception Tree for Safety-Critical Scenario Assessment](https://arxiv.org/abs/2608.04776)
*Yu Zhao,Jiangyu Pan,Tao Hu,Ming Yin,Fan Yang,Jiangfan Liu,Xiubo Liang*

Main category: cs.AI

TL;DR: 本文提出NSF-HRPT框架，结合神经语义场(NSF)学习场景语义、轨迹预测和概率TTC分布，用于从单目相机输入进行定量风险评估。框架通过层次化风险感知树(HRPT)实现高效并行计算和空间推理，并引入Sim2Real增强策略提升真实世界适用性。


<details>
  <summary>Details</summary>
Motivation: 现有研究在碰撞预测方面取得进展，但由于多智能体交互的复杂动态和真实世界环境的不确定性，从单目视觉输入准确量化风险水平仍具有挑战性。

Method: 1) 神经语义场(NSF)：从仿真数据学习场景语义、轨迹预测和概率TTC分布；2) 层次化风险感知树(HRPT)：利用预训练NSF作为先验进行高效并行计算和多智能体风险空间推理；3) Sim2Real增强策略：无需重训练即可提升真实世界适用性。

Result: 在合成基准测试上达到最先进性能，在真实世界数据集上TTC估计准确性和风险定位精度方面达到接近最先进水平的竞争性结果。

Conclusion: 该框架为单目相机输入的实时风险感知提供了有效解决方案，融合了学习感知与结构化推理的优势。
...
### [142] [Privileged, but Biased: How PI-Conditioned Teachers Break Self-Distillation](https://arxiv.org/abs/2608.04794)
*Sarthak Harne,Chinmay Karkar,Yash Pandya,Ahmed Awadallah,Akshay Nambi*

Main category: cs.AI

TL;DR: 自蒸馏（SD）作为强化学习的替代方法，在简单任务上有效但在困难任务上失败。研究发现SD无法作为单一目标教会模型推理，因为特权信息（PI）偏差导致模型优化目标与任务成功脱钩。


<details>
  <summary>Details</summary>
Motivation: 探索自蒸馏作为单一目标是否能够教会模型推理能力，验证其在困难任务上的有效性

Method: 在简单任务中复现SDPO的增益，然后将相同设置应用于困难任务（问答、数学、编码、多轮工具使用），使用PI偏差评分量化偏差程度

Result: 在困难任务上，验证准确率没有提升甚至下降；per-token loss持续下降但与任务成功脱钩；学生的探索性token产生最高分歧，惩罚了推理所需的犹豫

Conclusion: 作为单一目标，自蒸馏优化的是与任务成功脱钩的信号，PI偏差导致学生的目标几乎盲于rollout是否正确，无法培养推理能力
...
### [143] [ContextWeave: A Real-World Workflow Benchmark](https://arxiv.org/abs/2608.04830)
...
TL;DR: ContextWeave是一个纵向基准测试，用于评估语言智能体在现实办公工作流中通过 recalled experience 提升下游任务表现的能力。该基准将14名参与者的隐私保护多月工作流重建为1,005个可执行任务，包含568个核心评估任务。


<details>
  <summary>Details</summary>
Motivation: 现有评估往往将记忆简化为检索或问答任务，但记忆对于智能体从孤立任务过渡到长期、有状态的工作流程至关重要。

Method: 构建了包含指令、容器化环境、轨迹和任务特定评分标准的基准；测量工作区质量和与参与者特定偏好的对齐度；补充了相关性、连续性、可解决性和对误导性回忆的鲁棒性诊断。

Result: 在固定模型下，最强配置将工作区评分从68.08提升至78.20，将偏好评分从41.50提升至70.60。在固定记忆组件下，回忆改善了所有五个测试基础模型的结果，但增益差异显著。

Conclusion: 可操作、经验丰富的记忆比紧凑摘要更能支持工作流延续和减少冗余探索，但同时更容易受到误导性回忆的影响。记忆系统应优化检索相关性以及执行过程中的可靠使用。
...
### [144] [When Shared Rollouts Fail in Defensive Driving Evaluation: A NAVSIM Score Basis Audit](https://arxiv.org/abs/2608.04896)
*Ziang Wei,Minjun Yu,Zheyuan Lai,Mingjie Pang,Wei Li*

Main category: cs.AI

TL;DR: 本文审计了NAVSIM v2.2防御性驾驶评分系统的数值稳定性问题，发现参考条件 forgiveness 机制会传播共享参考失败导致合规分数虚高，通过更换solver可解决此问题。


<details>
  <summary>Details</summary>
Motivation: 防御性驾驶评分只有在区分观察周围行为者与不观察行为者的策略时才有意义，但NAVSIM的原始场景单阶段评分存在数值不稳定风险。

Method: 使用blind probes (route-blind Ignore-All和route-aware actor-blind)、固定诊断集、依赖敏感数值行为隔离、450-token控制池对照实验等审计方法。

Result: 在受影响的文档化栈条件下，两种blind probes在12,146-token navtest上超越人类回放和PDM-Closed。共享velocity refit中的数值不稳定是直接触发因素，替换solver可消除rollout分歧并恢复blind-last排序。

Conclusion: 贡献了一套审计协议：要求评分依据和栈披露、blind probes、覆盖报告和rollout稳定性测试，以防数值不稳定被参考条件forgiveness传播为合规分数。
...
### [145] [WorldCycle: Self-Verifiable Reinforcement Learning for Long-Horizon Video World Models](https://arxiv.org/abs/2608.04964)
...
TL;DR: WorldCycle是一个自验证的强化学习框架，通过构建可逆动作循环来解决交互式视频世界模型中的复合误差问题。该方法利用可逆动作周期作为无标注监督信号，强迫模型学习一致的状态算子而非记忆的时间模式。


<details>
  <summary>Details</summary>
Motivation: 交互式视频世界模型对于长期规划和探索至关重要，但存在复合误差问题。现有的强化学习后训练方法需要地面真实未来状态来衡量长期漂移，但对于任意动作序列并不存在这种标注，形成验证瓶颈。

Method: 核心思路是利用可逆动作周期进行无标注监督：一个序列与其逆序列组合必须能analytically返回初始状态。方法包括：1) 构建闭环动作周期及其重复执行；2) 空间闭合奖励：强制执行镜像正向和反向段之间的对称性；3) 时间一致性奖励：对齐重复周期执行之间的状态。这些奖励强迫模型学习动作作为一致的状态算子而非记忆的时间模式。

Result: WorldCycle将状态返回漂移减少高达44%，将复合动作准确率提升近4倍优于基线模型。

Conclusion: 该方法为物理基础的世界模型提供了重要基础，能够自然扩展到分布外复合动作循环。发布了CycleBench诊断基准用于测试复杂动作结构下的状态返回能力。
...
### [146] [Short-term load forecasting under EU-AI Act Requirements in Safety-Critical Environments: Results from a 41-day live challenge on the aggregated German transmission-grid load](https://arxiv.org/abs/2608.05018)
*Thomas Bartz-Beielstein*

Main category: cs.AI

TL;DR: 本文介绍了一个符合欧盟AI法案的短期负荷预测(STLF)管道spotforecast2-safe，在41天实时挑战中对德国聚合输电网负荷进行预测，击败了ENTSO-E官方基线，且透明、低成本的本地模型(maCl2l)与超过1亿参数的大型预训练基础模型(如chronos-2)具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 短期负荷预测对电力行业至关重要，支撑着欧洲和德国法律指定的关键基础设施。STLF不再仅仅是准确性问题，更是软件工程和合规性问题。EU-AI法案对关键环境中的AI系统提出了严格要求，需要确定性、可重复性和可审计性。

Method: 该管道基于开源Python库spotforecast2-safe，从ENTSO-E数据预测目标日的24小时负荷值。包含异常检测、间隙感知数据准备、日历和天气协变量、递归多步预测算法和超参数调优。使用EU-AI Act合规的spotforecast2-safe管道进行预测。

Result: 符合EU-AI法案的spotforecast2-safe管道击败了ENTSO-E基线。上下文模型显示出竞争力的性能。透明、低成本和可审计的本地模型(maCl2l)与超过1亿参数的大型能源密集型预训练基础模型(如chronos-2)具有竞争力。

Conclusion: 在关键基础设施中部署AI系统时，确定性、可重复性和可审计性是工程要求而非可选附加项。符合EU-AI法案要求的STLF管道能够实现准确预测，同时满足合规性要求。轻量级本地模型可与大型预训练模型竞争，为实际应用提供了可行的解决方案。
...
### [147] [From Score Matrices to Football-Aware Match-State Simulation: An Auditable LLM Harness for Exact-Score Reranking](https://arxiv.org/abs/2608.05030)
*Shaopeng Liang*

Main category: cs.AI

TL;DR: 本文提出一种将动态泊松模型与大语言模型(LLM)相结合的足球比分预测混合架构，通过四轮迭代(V1-V4)逐步改进。在2025-26赛季英超前150场比赛的测试中，V4达到了14.7%的Top-1准确率和30.7%的Top-3准确率，相比基线V1(10.0%/26.7%)有所提升。该研究贡献在于提供了一个可审计的混合架构，展示了足球感知模拟在比分选择中的优势与局限。


<details>
  <summary>Details</summary>
Motivation: 传统动态泊松模型虽能估计球队实力和预期进球，但无法理解战术对位、动机或进球后的行为变化；大语言模型虽能推理这些概念，但缺乏校准的概率输出能力。因此需要结合两者优势，通过可审计的信息框架实现更准确的足球比分预测。

Method: 研究采用四轮迭代方法：V1为动态Dixon-Coles基线模型；V2将LLM情境评分映射回预期进球参数；V3用冻结候选比分集进行逐球模拟替代标量校正；V4增加共享首次突破判断、进球后级联判断、时间感知停止和确定性尾部候选。在2025-26赛季英超前150场比赛进行时间顺序回放测试。

Result: V1实现10.0% Top-1和26.7% Top-3准确率；V3达到12.0%和30.0%；V4达到14.7%和30.7%。V4将候选覆盖率从77.3%提升至84.7%，但尾部候选未产生额外Top-3命中。V1原生1X2分布达53.3%.argmax准确率、0.9878对数损失、0.5870布里尔分数、0.2095排名概率分数。

Conclusion: 研究展示了可审计的混合预测架构的清晰设计演进。实验表明足球-aware模拟在比分选择上有所改进，但效果有限。结果具有探索性：开发切片非独立基准，且封闭LLM无法排除结果记忆。未来需进一步探索LLM与统计模型的最优结合方式。
...
### [148] [Item Response Theory for AI Safety](https://arxiv.org/abs/2608.05086)
*Joshua Fonseca Rivera,Neil Shah,David Demitri Africa,Konstantinos Voudouris*

Main category: cs.AI

TL;DR: 研究将心理测量学中的项目反应理论(IRT)应用于LLM安全评估，对8个安全基准在192个语言模型上进行分析，发现三个可解释的因素（拒绝严格性、真实性和情境危害）可以解释基准间的大部分方差，并实现了评估成本降低97-99%。


<details>
  <summary>Details</summary>
Motivation: 安全基准分数难以信任和解释，因为基准之间存在重复和高度相关，且模型可能在接受评估时故意降低表现（sandbagging）。需要一种更可靠、可解释的评估方法。

Method: 对8个安全基准和192个语言模型拟合IRT模型，这是迄今为止最大规模的LLM安全评估心理测量分析。

Result: 1）三个可解释因素（拒绝严格性、真实性和情境危害）解释了模型间的大部分方差；2）心理测量精选项目比随机子集误差更低，约10个自适应选择项目即可充分评估单个基准；3）IRT可检测出简单的sandbagging和API背后的模型变化。

Conclusion: IRT是现成的工具，可用于解读、简化和审计安全基准，建议前沿实验室和评估机构采用。
...
### [149] [Hierarchical Graph Memory for LLM Agents with Path-level Localization and Rewrite](https://arxiv.org/abs/2608.05095)
*Xiawei Yue,Boran Wang,Xiaoqing Zhang,Shuxin Zheng,Ziwei Zhang*

Main category: cs.AI

TL;DR: 提出HiGram框架，解决长期推理智能体中图记忆的效率和问题。该框架通过层次化图记忆、基于MicroGraph的路径级定位和协调重写方法，在长程对话问答和冲突感知记忆评估任务上显著提升了答案质量和token效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法将所有记忆存储在扁平图中，累积的历史记忆会引入无关上下文，增加检索成本；同时独立更新记忆单元需要重复重写相关联的变化。

Method: 提出三层创新方法：1）层次化图记忆，将记忆组织成粗粒度到细粒度的上层节点和MemoryUnits结构；2）基于MicroGraph的路径级定位，利用查询和更新条件的MicroGraph识别支持子图和证据路径；3）协调重写方法，联合修订单元内记忆和单元间依赖关系。

Result: 在长程对话问答基准和冲突感知记忆评估上，相比基线方法在答案质量、token效率、答案准确性和查询有效证据选择方面均取得显著提升。

Conclusion: HiGram通过层次化结构和路径级定位与重写有效解决了长期推理中的记忆效率和有效性问题，具有良好的应用前景。
...
### [150] [ABSeeker: Training Long-Horizon Search Agents via Answer-Backtracked Credit Assignment](https://arxiv.org/abs/2608.05102)
*Yijun Lu,Rui Ye,Jiajun Wang,Yuwen Du,Tian Jin,Songhua Liu,Siheng Chen*

Main category: cs.AI

TL;DR: 本文提出答案回溯信用分配(ABC)框架，用于训练长程搜索智能体。该框架将稀疏的轨迹级结果转换为密集的步骤级监督，能够奖励成功轨迹中的有用动作，同时抑制失败轨迹中的错误或冗余动作。基于Qwen3.5-4B训练的ABSeeker在BrowseComp上达到55.3%(含上下文管理)的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练长程搜索智能体时，对轨迹中的所有步骤一视同仁，无法区分有用动作、错误动作和冗余动作。这种均匀对待导致训练效果不佳，因为即使最终答案错误，中间的某些步骤也可能是有价值的。

Method: ABC框架包含两个核心组件：1)答案回溯线索恢复(Answer-Backtracked Clue Recovery)：从答案反向追溯恢复解决问题所需的中介线索；2)线索锚定步骤评分(Clue-Anchored Step Scoring)：根据线索评估每个搜索步骤，将稀疏的二值结果监督转换为密集的步骤级奖励。在此基础上提出ABC-SFT(重加权损失)和ABC-GRPO(作为GRPO的奖励)。

Result: ABSeeker在BrowseComp上达到37.3%，BrowseComp-ZH上达到39.1%。使用上下文管理后，分别提升至55.3%和52.9%。显著优于同规模(4B)智能体，并可媲美约30B参数的更大模型。

Conclusion: 答案回溯的步骤级信用分配对训练长程搜索智能体非常有效。该方法能够充分利用失败轨迹中的有价值信息，为智能体训练提供更精细的监督信号。
...
TL;DR: 本文介绍了OctoLong，一个利用AST解析器、语言服务器后端和包管理器构建的代码上下文工程管道，可生成数百万token的依赖丰富代码上下文。基于此数据训练了OctoLong-Instruct系列长上下文开源LLM（600M-14B），仅用12%的OctoLong数据替换传统上下文扩展语料即可显著提升长程检索、长期状态跟踪、仓库级代码理解和下游代理任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文语料库以书籍、学术论文和代码仓库为主，资源有限且缺乏长距离依赖关系，需要构建依赖丰富的代码上下文数据来提升长上下文LM的性能。

Method: 1. 构建OctoLong管道：使用AST解析器、语言服务器后端和包管理器进行代码引用的递归检索，生成数百万token的代码上下文
2. 训练OctoLong-Instruct模型族（600M-14B参数）：在约50B token混合数据上进行上下文扩展中间训练（其中约6.2B token为OctoLong数据），随后进行约10B token的指令微调

Result: 仅用12%的OctoLong数据替换传统上下文扩展语料，即可显著提升：长程检索、长期状态跟踪、仓库级代码理解、下游代理任务，以及短上下文编码场景中的API使用能力。在18个最先进的开源长上下文LM对比中表现优异。

Conclusion: OctoLong数据即使在较小比例下也能有效提升长上下文LLM训练效果，为长上下文模型训练提供了一种高质量的代码数据构建方法。
...
TL;DR: Argus是一个持久的自我进化运行时系统，通过Manager、Planner、Engineer和Reviewer四个角色执行有边界的任务，实现长期推理。该系统在GPT-5.5基准测试中表现优异，SWE-Bench Pro达到78%（vs Direct Copilot的59%），并在验证引导的自我进化后实现了21%的token减少和15%的工作流时间优化。


<details>
  <summary>Details</summary>
Motivation: 长期推理需要一个能够持久运行的智能体运行时，当证据支持当前方法时保持坚持，当测量揭示失败、隐藏约束或错误指定的目标时能够转向。现有系统缺乏这种持久性和自适应能力。

Method: Argus采用多角色架构（Manager、Planner、Engineer、Reviewer）执行有边界的任务，将稳定用户意图与操作目标、约束和验证标准分离。它包含记忆、技能、程序、验证器、路由决策等功能，只有经过角色所有者的审查和任务本机验证后才能 admit 记忆。模型权重保持固定，自我进化通过持久的运行时状态和控制策略实现。

Result: SWE-Bench Pro达到78%（vs Direct Copilot的59%），使用1.41倍总token；验证后成熟阶段减少21%的solve-input token和15%的工作流时间；记录34次验证器恢复和22次严格审查循环救援；AARRI-Bench达76.8%；数学数据合成28.0分差距；优化RWKV6内核合并上游；数学活动保留伪造路线和证明支持的边界更新；6个论文管道完成254个任务，16次阶段回滚。

Conclusion: 固定权重的自我进化框架可以修订、恢复和累积已验证的方法，同时为未来的监督学习和强化学习生成结构化轨迹。这表明即使模型权重不变，通过运行时状态和控制策略的优化也能实现显著的智能体能力提升。
...
### [153] [Efficient Algorithms for the Bottleneck Path Problem in Geometric Graphs](https://arxiv.org/abs/2608.04203)
*Matthew J. Katz,Rachel Saban,Micha Sharir*

Main category: cs.CG

TL;DR: 本文研究几何设置下的瓶颈路径问题，包括平面中方向天线图（天线角度有常数下界）和可见性图（顶点位于1.5维地形上或以上），使用欧几里得距离作为边权重。提出了决策问题的近线性算法，确定保留权重不超过某阈值的边所得到的子图是否包含从s到t的路径。然后利用决策程序得到瓶颈路径问题的随机预期时间O*(n^{{8/7}})算法，同时也能在相同性能界内解决最多k条边的有界跳版本。


<details>
  <summary>Details</summary>
Motivation: 瓶颈路径问题在无线通信、机器人导航等应用中自然出现，需要在几何图（如方向天线图和地形可见性图）中找到最坏情况下边权重最小的路径。现有算法效率不足，需要更高效的解决方案。

Method: 1. 为两类几何图设计近线性决策算法；2. 利用随机化技术和参数化方法；3. 结合决策程序和路径搜索构建完整算法；4. 处理有界跳约束的变体。

Result: 1. 决策问题近线性时间算法；2. 瓶颈路径问题O*(n^{{8/7}})随机预期时间算法；3. 有界跳版本在相同界内可解；4. 两类几何设置均适用。

Conclusion: 首次在这两类重要几何图中建立次二次时间算法，为瓶颈路径问题的实际应用提供理论基础，并展示了随机化方法在几何图算法中的有效性。
...
### [154] [Discrete homology computations by reduction to zero differentials](https://arxiv.org/abs/2608.04262)
*Sterling Ebel,Chris Kapulkin,Nathan Kershaw*

Main category: cs.CG

TL;DR: 开发了一种新算法，通过约化到零微分和主动枚举来计算图的(持续)离散同调群。该算法能够计算Greene球面的第四同调群以及几个先前未知的群，并在高噪声非度量设置下比Vietoris-Rips复形的单纯同调计算更快。


<details>
  <summary>Details</summary>
Motivation: 需要为噪声数据集（特别是在非度量设置下）开发更快的同调群计算方法。传统的单纯同调计算在处理高噪声数据时效率较低。

Method: 通过约化到零微分(reduction to zero differentials)和主动枚举(active enumeration)来计算图的持久离散同调群。

Result: 成功计算了Greene球面的第四同调群和几个先前未知的同调群；证明了在_high-noise非度量设置下，持久离散同调比Vietoris-Rips复形的单纯同调计算速度更快。

Conclusion: 持久离散同调是噪声数据集的更好选择，因为它在处理高噪声非度量数据时具有更高的计算效率。
...
### [155] [Fast Thick-Thin Decomposition for Sparse Spanners on Hyperbolic Surfaces](https://arxiv.org/abs/2608.04585)
*Sándor Kisfaludi-Bak,Geert van Wordragen*

Main category: cs.CG

TL;DR: 研究在双曲平面或闭合双曲表面上构造非交叉(1+ε)-spanner的问题。通过引入Steiner点，在双曲平面上得到O(n/ε²)边的非交叉Steiner spanner；在 genus g的双曲表面上得到O(n/ε^(3/2)+g/ε²)边的非交叉spanner，或O(n/√ε+g/ε)边的可交叉spanner。首次实现了对genus的线性依赖，并得到双曲表面TSP的EPTAS。


<details>
  <summary>Details</summary>
Motivation: 将非交叉欧几里得spanner的概念自然推广到双曲几何环境，解决在双曲平面上构造稀疏非交叉spanner以及在双曲表面上实现对genus的线性依赖而非指数依赖的核心问题。

Method: 1. 使用厚-薄分解(thick-thin decomposition)处理双曲表面；2. 为凸双曲多边形引入颈分解(neck decomposition)；3. 算法计算：genus-g表面的厚-薄分解需O(g⁴log g)时间，n顶点多边形的颈分解需O(n)时间

Result: 双曲平面：获得O(n/ε²)边的非交叉Steiner (1+ε)-spanner；双曲表面：非交叉边O(n/ε^(3/2)+g/ε²)，可交叉边O(n/√ε+g/ε)， sparsity项n/ε^(3/2)和n/√ε与当前最佳欧几里得结果相当；作为推论获得双曲表面TSP的EPTAS

Conclusion: 首次在双曲表面上实现了对genus的线性依赖稀疏spanner，突破了之前指数依赖的局限，颈分解和厚-薄分解技术为双曲几何中的图论问题提供了有效算法工具。
...
### [156] [Strategic Evaluation of Planning Strategies for LLM Agents in Cyber-Physical Systems](https://arxiv.org/abs/2608.04265)
*J. de Curtò,I. de Zarzà*

Main category: cs.MA

TL;DR: 本文引入了一个物理基础的基准测试，用于评估LLM规划代理在智能电网需求响应系统中的表现。研究发现架构选择显著影响结果，提前进行截止日期可行性检查可将遗憾值从90.7降至29.0。


<details>
  <summary>Details</summary>
Motivation: 当前LLM规划代理的评估主要关注任务是否成功或计划是否被遵循，但在战略网络物理系统中，更重要的问题是规划架构在自主参与者响应和物理约束结果后是否仍然合适。

Method: 构建了一个基于智能电网需求响应的控制轨迹基准测试，包含40个异构产消者及独立模拟的径向馈线。实现预定义顺序、分层和搜索执行器，LLM受限于类型化策略声明和短操作员消息，调度构建、产消者动态和功率流保持为显式代码。使用成对强制模式反事实、共同随机响应绘制和事件级截止日期可行性进行评估。

Result: 强制搜索在所有五个基线种子中是预言机；客观替换使协议保持1.0但将电压 shortfall 增加2.68倍；144场景、576回合的银行中，三个架构产生可行预言机；压力测试岭保持部分显示平均遗憾90.7，在截止日期可行性预测前应用已知截止日期可行性可将遗憾降至29.0；全可行消融无法击败固定搜索。

Conclusion: 架构本质上改变结果；执行保真度需要更多模式协议；提前进行截止日期可行性检查可显著减少遗憾并优于固定顺序；在可行范围内选择质量仍是剩余挑战。
...
### [157] [Responsibility in Multi-Agent Sequential Decision-Making: Comparing Human Judgments to Formal Models of Causal Attribution](https://arxiv.org/abs/2608.04318)
*Nripsuta Ani Saxena,Stelios Triantafyllou,Goran Radanović*

Main category: cs.MA

TL;DR: 研究AI系统在多智能体sequential决策场景中，如何将形式化的责任归属定义(基于实际因果关系框架)与人类责任判断对齐。发现没有单一方法能持续与人类判断一致。 


<details>
  <summary>Details</summary>
Motivation: 随着AI在高压决策场景中的广泛应用，识别结果原因(尤其是失败)及确定责任归属成为关键问题。需要理解形式化责任归属与人类直觉的一致性。

Method: 进行大规模问卷调查，使用修改版Goofspiel纸牌游戏获取人类对多智能体sequential决策场景中的责任判断。评估多种责任归属方法与人类判断的对齐程度。

Result: 没有单一责任归属方法持续与人类判断一致。影响人类责任判断的关键因素包括：智能体特定偏见(intentionality、controllability)及智能体决策时可获取的信息量。

Conclusion: 形式化责任归属方法与人类责任判断存在差距。未来设计AI责任归属机制时需考虑信息可用性及智能体特定偏见等因素。
...
### [158] [Continuous Improvement and Parallel Autonomous Exploration: An LLM-Agent Framework for Searching Large Solution Spaces](https://arxiv.org/abs/2608.04341)
*Dulmini Hettiarachchi,Andre Rusli,Julio Christian Young,Sho Akiyama*

Main category: cs.MA

TL;DR: 提出了一个让LLM智能体能够自主搜索大型解决方案空间的框架，包含两个机制：1) 基于留出数据的排行榜作为奖励信号，推动智能体通过重复提交来优化方案；2) 支持多个智能体并行运行，完全自主运行。该框架在电商产品目录匹配任务上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 解决LLM智能体在大型解决方案空间中自主探索的问题，突破单一种子范式的局限性，实现更广泛的解决方案空间探索。

Method: 1) 排行榜奖励机制：基于留出数据评分，驱动智能体迭代优化；2) 并行自主运行：多个智能体独立分析、调研、实现、自评、提交和修订，管理员智能体仅处理后勤事务；3) 在产品-目录匹配任务（电商检索核心任务）上验证。

Result: 单智能体最佳 qualified coverage 达47.8-57.4%，五智能体达62.8-69.4%，基线为33.3%。单智能体在种子范式内优化，并行智能体发现定性不同的解决方案。

Conclusion: 框架本身是该工作的贡献：提供持续改进的奖励循环和完全自主并行探索的基础设施。实验证明并行运行能显著扩展解决方案空间的探索区域，而非仅在单一范式内精炼。
...
### [159] [Emergence of Reputation-Based Cooperation in LLM Agents](https://arxiv.org/abs/2608.04507)
*Kazuya Horibe,Kenji Itao,Wataru Toyokawa*

Main category: cs.MA

TL;DR: 研究大型语言模型(LLM)代理之间的合作是否能在进化上抵制白嫖者(搭便车者)的入侵。


<details>
  <summary>Details</summary>
Motivation: 探索LLM代理之间的合作是否具有进化稳定性，以及白嫖者是否能够入侵合作策略。

Method: 使用间接互惠捐赠游戏，LLM代理观察行为痕迹并连续捐赠。策略以自然语言提示表示，通过跨代文化传播进化。测试了四个LLM后端。

Result: 对白嫖者入侵的鲁棒性差异超过一个数量级。最强预测因素是对手 endowment 敏感性，即代理区分合作与不合作对手的程度，这是经典的图像评分机制。相比之下，对"主导八条L1规范"的遵守不能预测鲁棒性。鲁棒性取决于排斥排斥者：虽然合作者奖励和排斥者惩罚在不同模型中有所不同，但只有排斥者的排斥严格性预测了对白嫖者入侵的抵抗力。

Conclusion: LLM代理仅限于图像评分式的歧视，未能发展出更具鲁棒性的"主导八条"规范，揭示了文化进化LLM合作的基本弱点，并激励了自下而上的规范构建方法。
...
### [160] [HELENA:Hierarchical Sparse Coordination over a Union of Complementary Topologies for MAS](https://arxiv.org/abs/2608.04634)
*Zhifang Mao,Linyao Zheng,Xuhang Shi,Xiuquan Hou*

Main category: cs.MA

TL;DR: HELENA是一个多智能体框架，通过结合蒙特卡洛树搜索和行列式点过程选择互补拓扑构建联合图，并使用层次化稀疏协调只在每步激活稀疏子图，同时通过局部自 refinement阶段识别和重写有差异的决策单元。实验在8个基准上取得最先进结果，平均提升3.47%，在MMLU-Pro上提升达10.34%。


<details>
  <summary>Details</summary>
Motivation: 当前LLM多智能体系统通常只优化单一拓扑，限制了推理轨迹的广度和综合分析能力。简单地合并多个拓扑会产生冗余噪声传播，降低解的质量。

Method: 1) 使用蒙特卡洛树搜索和行列式点过程选择互补拓扑构建联合MAS图；2) 层次化稀疏协调模块在每步仅激活稀疏子图，智能体交换压缩的潜在简报抑制噪声；3) 局部自refinement阶段识别有差异证据的决策单元，仅当对比证据同时确认可靠方案失败和挑战方改进时进行重写。

Result: 在8个基准上达到最先进结果，比最强基线平均提升3.47%，在MMLU-Pro上提升10.34%，在更难的基准上改进更大。

Conclusion: HELENA成功平衡了多样化推理路径与稀疏任务依赖执行，在复杂问题综合分析上显著优于现有方法，且额外计算成本合理。
