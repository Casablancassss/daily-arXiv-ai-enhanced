...
TL;DR: 本文提出SKETCH作为步态识别的新视觉模态，通过边缘检测器从RGB图像中提取高频结构线索（如肢体关节和自遮挡轮廓），无需标签监督。结合标签引导的解析和无标签草图，提出SKETCHGAIT分层解耦多模态框架，在SUSTech1K和CCPG数据集上分别达到92.9%和93.1%的Rank-1准确率。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法主要基于轮廓和解析，但轮廓稀疏且缺乏内部结构细节，解析方法依赖上游人工解析器导致性能不稳定。作者从结构角度重新审视步态表示，发现了一个未被充分探索的范式：密集的部件级结构但没有明确的语义标签。

Method: 提出SKETCHGAIT，一个分层解耦的多模态框架，包含两个独立流用于模态特定学习，以及一个轻量级的早期融合分支来捕捉结构互补性。该框架结合了标签引导的解析和无标签的草图两种互补的表示。

Result: 在SUSTech1K数据集上实现92.9%的Rank-1准确率，在CCPG数据集上实现93.1%的平均Rank-1准确率，验证了草图模态和SKETCHGAIT框架的有效性。

Conclusion: SKETCHGAIT在SUSTech1K上达到92.9%的Rank-1准确率，在CCPG上达到93.1%的平均Rank-1准确率，验证了所提出的模态和框架的有效性。
...
TL;DR: DreamCAD是一个多模态CAD生成框架，通过参数化面片表示和可微分tessellation方法，从点级监督生成可编辑的BRep，无需CAD特定注释即可进行大规模训练，并引入百万级CAD标注数据集CADCap-1M，在多项基准上取得最优效果。


<details>
  <summary>Details</summary>
Motivation: 现有生成方法受限于带有明确设计历史或边界表示(BRep)标签的小型标注数据集，而数百万个未标注的3D网格未被充分利用，阻碍了可扩展CAD生成的发展。

Method: 提出DreamCAD多模态生成框架，将每个BRep表示为参数化面片集合（如Bézier曲面），使用可微分 tessellation方法生成网格，实现从点级监督直接生成可编辑BReps，无需CAD特定注释。同时构建了包含100万+描述的CADCap-1M数据集。

Result: 在ABC和Objaverse基准数据集上跨文本、图像和点模态实现最优性能，提升了几何保真度，用户偏好度超过75%。

Conclusion: DreamCAD框架使得无需CAD特定注释的大规模CAD生成成为可行，CADCap-1M数据集的引入推动了文本到CAD研究的发展。该方法在几何保真度和用户偏好方面均取得了显著提升。
...
TL;DR: 本文提出ABRA方法，将生物批次效应缓解问题重新定义为领域泛化问题，通过对抗性批次表示增强和分布对齐策略，有效提升深度学习模型在未见生物批次数据上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 高内容筛选产生的细胞图像存在技术变异导致的生物批次效应，引起协变量偏移并降低深度学习模型对未知数据的泛化能力。现有批次校正方法依赖额外先验知识或难以泛化到未见生物批次。

Method: 提出ABRA方法，将批次统计波动建模为结构化不确定性，通过min-max优化框架在表示空间中主动合成最坏情况的生物批次扰动，并利用严格的角几何边距保持细粒度类别可区分性，同时引入协同分布对齐目标防止表示崩溃。

Result: 在大规模RxRx1和RxRx1-WILDS基准测试上的广泛评估表明，ABRA在siRNA扰动分类任务上实现了新的最优性能。

Conclusion: ABRA通过对抗性批次表示增强方法，将生物批次效应缓解问题转化为领域泛化问题，在大规模RxRx1和RxRx1-WILDS基准测试中建立了siRNA扰动分类的新技术标准，显著提升了模型对未见生物批次的泛化能力。
...
TL;DR: 本文提出PFS后融合稳定器模块，通过稳定特征统计、抑制降质区域和残差校正增强BEV融合检测器在域迁移和传感器故障下的鲁棒性，仅需3.3M参数即可提升相机dropout和低光照场景下的检测性能


<details>
  <summary>Details</summary>
Motivation: 现有BEV融合检测器在域迁移和传感器故障时性能显著下降，而现有鲁棒性方法需修改融合架构或重新训练专用模型，难以集成到已部署系统中

Method: 提出后融合稳定器(PFS)，通过三个核心机制实现：1)稳定域迁移下的特征统计 2)抑制传感器降质影响的空间区域 3)通过残差校正自适应恢复削弱的特征线索，采用近恒等变换设计

Result: 在nuScenes数据集上实现SOTA性能，相机 dropout鲁棒性提升+1.2% mAP，低光照性能提升+4.4% mAP，保持轻量级3.3M参数

Conclusion: PFS作为一个轻量级后处理模块，可无缝集成到现有BEV融合检测器中，在不修改原始架构的情况下显著提升域迁移和传感器故障场景下的鲁棒性，且仅增加3.3M参数
...
TL;DR: CBM-Suite是一个系统性解决概念瓶颈模型(CBMs)四大基本局限的方法论框架：概念相关性评估指标、线性化问题、精度差距以及视觉骨干和VLM的对比分析。


<details>
  <summary>Details</summary>
Motivation: CBMs虽然将预测建立在人类可理解的概念上，但存在四个 fundamental limitations: 1) 缺乏预评估概念相关性的指标 2) "线性化问题"导致近期CBMs完全绕过概念瓶颈 3) 与不透明模型相比存在精度差距 4) 缺乏对不同视觉骨干和VLM影响的系统研究

Method: 提出四个创新方案：1) 基于熵的度量指标量化概念集对数据集的内在适用性 2) 在概念激活和分类器之间插入非线性层解决线性化问题 3) 利用线性教师探针引导的蒸馏损失缩小精度差距 4) 对不同视觉编码器、视觉语言模型和概念集的相互作用进行综合分析

Result: 广泛评估表明CBM-Suite生成更精确的模型，并为改进基于概念的可解释性提供洞察

Conclusion: CBM-Suite是一个全面的方法论框架，能够系统性地克服CBMs的现有局限，为概念基可解释性提供更准确的模型和有用的设计指导
...
TL;DR: 本文提出interpolated FID (iFID)，通过在VAE潜在空间中检索最近邻并进行插值解码来解决重建FID与扩散模型生成FID不相关的问题，是首个与扩散模型gFID强相关的指标。


<details>
  <summary>Details</summary>
Motivation: 重建FID (rFID) 与潜在扩散模型的生成FID (gFID) 相关性很差，需要找到一种能够预测扩散模型生成质量的指标。

Method: 对数据集中每个元素，检索其在潜在空间中的最近邻(NN)，对两个 latent 表示进行插值，然后解码插值后的 latent 并计算解码样本与原始数据集之间的 FID。

Result: iFID 是首个与扩散模型 gFID 展现强相关的指标，Pearson 线性和 Spearman 秩相关系数均达到约 0.85。

Conclusion: rFID 与扩散模型 refinement 阶段样本质量相关，而 iFID 与 navigation 阶段质量相关；该发现与扩散模型泛化和幻觉研究结果相联系，解释了为什么重建指标与 gFID 负相关。
...
TL;DR: 提出隐式错误计数(IEC)方法填补无参考答案场景下后训练方法的空白，通过列举错误而非检查正确性来生成奖励信号，在虚拟试穿任务上超越基于评分标准的方法。


<details>
  <summary>Details</summary>
Motivation: 现有 RLVR 和 RaR 方法依赖于理想参考答案来生成评分标准，但许多现实世界任务没有单一理想答案，导致当前后训练方法存在参考自由设置的空白。虚拟试穿(VTO)任务同时受到过度约束和过度宽松的限制，非常适合作为案例研究。

Method: 提出隐式错误计数(IEC)方法，通过列举响应在任务相关轴上的错误，应用严重性加权分数，并将其转换为校准的每方面奖励。设计了两种关键设计：隐式分数发射和组校准，以提高错误计数的可靠性。提出级联错误计数(CEC)作为评估指标。

Result: 在 MDressBench 上，IEC 优于 RaR（CEC：5.31 vs 5.60 平坦参考；5.20 vs 5.53 非平坦）。在 VITON-HD 和 DressCode 上，IEC 在 8 个感知指标中的 6 个上匹配或超越六个基线。CEC 指标与人类偏好相关性良好（60% top-1 vs 30% 其他）。

Conclusion: 当没有理想答案时，计数错误比构建评分标准提供更强的训练信号。IEC方法在虚拟试穿任务上超越了基于评分标准的方法，表明错误计数方法在缺乏参考答案的场景中具有显著优势。
...
TL;DR: 本文提出SemVID，一种针对视频时间定位任务的训练-free视觉token剪枝框架，通过保持边界敏感证据(证据保留原则)和跨帧连接性(连接强度原则)，在仅使用12.5%视觉tokens的情况下保留95.4%mIoU，实现5.8倍加速。


<details>
  <summary>Details</summary>
Motivation: 视频时间定位(VTG)任务需要处理长未修剪视频，导致视频语言模型管道成本高昂。虽然近期训练-free视觉token剪枝在视频问答中取得成功，但直接应用于VTG会导致性能大幅下降，因为VTG关键依赖于边界敏感证据和跨帧推理链。

Method: SemVID采用训练-free剪枝框架，通过两个核心步骤实现高效token选择：首先根据查询相关性和帧间变化率分配每帧token预算以避免过度剪裁；然后选择三类互补语义角色tokens：1)对象tokens用于多样化的查询关键证据；2)运动tokens捕捉有意义的变化并作为跨帧中继；3)少量场景连续性上下文tokens。

Result: 在VTG基准测试中，SemVID实现了显著的性能提升：仅使用12.5%的视觉tokens即可保留95.4%的mIoU，达到最高5.8倍的预填充加速，在相同预算下始终优于先前方法。

Conclusion: SemVID通过保持边界敏感证据和跨帧连接性，在VTG任务中实现了显著的计算效率提升，同时保持了高定位精度。该工作为训练-free视觉 token 剪枝在时间定位任务中的应用提供了重要指导。
...
TL;DR: 本文提出了一种名为OWL的感知相关函数，用于运动中的3D感知。该函数直接从两个基本视觉运动线索中获取值，可在无需了解环境、物体或相机运动先验知识的情况下，从原始视觉运动图像序列中获取OWL值，实现基于视觉运动线索的缩放3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 解决运动中3D感知的复杂挑战，提供一种统一、分析性的时间方法，增强和简化缩放3D映射和相机航向等关键感知能力。现有方法通常需要先验知识或明确测量相对瞬时方向范围和3D方向平移。

Method: OWL函数从两个视觉运动线索派生值：1)相对于注视点的点局部视觉looming，2)刚体相对于注视点的感知旋转。该方法使用极简的基于像素的并行计算，无需显式测量或预先了解各参数值。

Result: OWL实现了3D对象随时间的几何恒常性，能够仅从视觉运动线索进行缩放3D场景重建。通过直接测量原始视觉运动图像序列获取OWL值，提供了一种替代性的实时表示方法。

Conclusion: OWL为机器人技术和自主导航中的3D点相对运动提供了一种替代实时表示方法，弥合了理论概念与实际应用之间的差距，可能为下一代自主系统提供构建模块，对自然感知和行为心理学理解也有潜在影响。
...
TL;DR: MultiHaystack是首个在大型跨模态条件下评估检索和推理的基准，包含超过46,000个多模态检索候选和747个问题。研究发现当前模型在给定证据时表现良好，但检索能力不足，多模态检索是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试限制了检索范围到小型、单模态候选集，大大简化了搜索空间并高估了端到端可靠性。现有设置未评估从大型异构多模态语料库中检索相关证据这一关键现实世界需求。

Method: 构建MultiHaystack基准测试，包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的验证证据项，需要跨模态的证据定位和细粒度推理。

Result: 当提供相应证据时，模型表现竞争性强，但从完整语料库中检索时性能急剧下降。即使是最强的检索器E5-V也仅达到40.8%的Recall@1，而最先进的MLLMs如GPT-5在推理准确率上出现显著下降：从提供相应证据时的80.86%下降到Top-5检索下的51.4%。

Conclusion: 多模态检索是MLLMs的主要瓶颈。MultiHaystack作为一个有价值的测试平台，可以突出小规模评估所掩盖的底层局限性，并促进多模态系统中以检索为中心的进步。
...
TL;DR: 本文提出Audiovisual Geolocation框架，利用音频+视频进行全球地理定位。引入AVG基准数据集（2万个视频片段，1000个地点），通过三阶段方法：感知阶段分离音频原子、推理阶段用GRPO微调MLLM、预测阶段用黎曼流匹配。实验表明音频信息提供了关键的独立信号，显著优于单模态基线。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在精确全球地理定位方面存在困难，原因包括：1）视觉景观本身具有歧义性；2）音频线索的潜力未被充分挖掘。因此需要一种结合视听信息的新方法来突破地理定位的精度瓶颈。

Method: 提出三阶段框架：1）感知阶段：使用混合自回归稀疏自编码器将噪声音频分解为语义锚定的"声学原子"；2）多模态推理阶段：通过GRPO微调MLLM，将声学原子与视觉特征合成；3）精度预测阶段：在S²流形上使用黎曼流匹配进行定位。同时构建AVG基准数据集，包含2万个视频片段覆盖1000个不同地点。

Result: 框架显著优于单模态基线方法。实验证明，对声景的可解释感知提供了关键的、正交的信号，当与多模态推理结合时，能够实现高精度的全球定位。

Conclusion: 音频信息是地理定位的重要补充线索，多模态推理结合视觉和音频特征能够有效突破单模态方法的精度限制，为精确全球地理定位提供了新思路。
...
TL;DR: Any2Full是一个单阶段、域无关、模式无关的深度补全框架，将补全问题重新定义为预训练单目深度估计模型的尺度提示适应问题，通过Scale-Aware Prompt Encoder处理不同深度稀疏度和不规则空间分布，实现优于现有方法的鲁棒性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有RGBD融合深度补全方法学习与训练RGB分布和特定深度模式相关的先验，限制了域泛化和对各种深度模式的鲁棒性；现有两阶段整合策略依赖显式相对度量对齐，带来额外计算成本和结构失真。

Method: 提出Scale-Aware Prompt Encoder，将稀疏输入的尺度线索提取为统一尺度提示，引导MDE模型实现全局尺度一致的预测，同时保留其几何先验。采用单阶段域无关、模式无关的框架。

Result: 在平均AbsREL上比OMNI-DC提高32.2%，与PriorDA使用相同MDE主干时实现1.4倍加速。

Conclusion: Any2Full通过将深度补全重新定义为预训练单目深度估计模型的尺度提示适应问题，实现了卓越的鲁棒性和效率。它比OMNI-DC在平均AbsREL上提高32.2%，与PriorDA使用相同MDE主干时实现1.4倍加速，建立了通用深度补全的新范式。
...
TL;DR: 本文提出了一个自动化 pipeline，将 ImageNet 训练集转换为多标签数据集，无需人工标注。利用自监督 Vision Transformers 进行无监督目标发现，选择与原始标签对齐的区域训练轻量级分类器，生成一致的多标签标注。实验表明这种方法显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: ImageNet 基准采用单标签假设，但实际图像通常包含多个对象，导致标签噪声并限制学习信号的丰富性。多标签标注更能准确反映现实世界的视觉场景，使模型能够学习更丰富、更鲁棒的表示。虽然已有工作改进了验证集，但训练集缺乏可扩展、高质量的多标签标注。

Method: 提出自动化 pipeline：(1) 使用自监督 Vision Transformers 进行无监督目标发现；(2) 选择与原始标签对齐的区域训练轻量级分类器；(3) 将分类器应用于所有区域以生成一致的多标签标注。

Result: 标签与人类判断高度一致。在定量基准测试中持续提升性能：ReaL 上最高 +2.0% top-1 准确率，ImageNet-V2 上 +1.5%；迁移到下游任务表现更强：COCO 上 +4.2 mAP，VOC 上 +2.3 mAP。

Conclusion: 准确的多标签标注对提升分类性能和表示学习至关重要。这种自动化方法无需人工标注即可生成高质量多标签数据，为计算机视觉模型提供了更丰富的监督信号。
...
TL;DR: 提出了一种基于CLIP的多模态框架,用于从手术视频自动生成手术时间线和叙述文。该方法通过将手术视频帧与文本手势描述对齐来实现自动时间线创建。


<details>
  <summary>Details</summary>
Motivation: 当前手术时间线创建方法存在极端问题:术后(OP)报告过于模糊,而手动标注手术视频又极其耗时,需要一种介于两者之间的自动化解决方案。

Method: 采用基于CLIP的多模态框架:使用CLIP视觉编码器从手术视频帧中提取特征表示,使用文本编码器将手势句子嵌入共享嵌入空间。然后微调模型以改进视频手势与文本标记之间的对齐。训练后,模型预测视频帧的手势和阶段,从而构建结构化手术时间线。

Result: 该模型能够预测视频帧的手势和阶段,实现结构化手术时间线的构建。该方法利用预训练的多模态表示来桥接视觉手势和文本叙述。

Conclusion: 该方法有效减少了外科医生手动审查和标注视频的需求,为手术视频分析提供了一种高效的自动化解决方案。
...
TL;DR: Icarus是一种全天候天空模型，能够学习全动态范围(FDR)物理捕获的户外图像，可实现太阳和云层形成的用户可控定位和大气形成纹理化，在图像照明中提供前所未有的准确性、逼真度、光照方向性和色调。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在高分辨率下难以准确模拟14EV+的类不平衡太阳区域，导致视觉效果差、场景光照扭曲、阴影和色调不准确。物理捕获和体积渲染方法成本高、耗时长，需要更灵活高效的替代方案。

Method: 提出Icarus模型，能够学习FDR物理捕获户外图像的曝光范围，支持太阳和云层形成的有条件生成和直观用户定位，扩展当前最先进技术以支持大气形成用户可控纹理化。

Result: Icarus可与FDR物理捕获户外图像或参数化天空模型互换，在IBL中展示前所未有的准确性、逼真度、光照方向性（阴影）和色调。

Conclusion: Icarus成功解决了深度学习天空模型在处理高动态范围太阳区域时的局限性，为户外场景建模提供了更准确、更灵活的解决方案。
...
TL;DR: LayerBind是一种无需训练的即插即用方法，通过将区域生建模为不同层并在生成过程中绑定，实现文本到图像生成中的区域和遮挡顺序控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：(i) 基于训练的方法继承数据偏差并常导致图像质量下降；(ii) 当前技术难以处理遮挡顺序，限制实际应用。观察到空间布局和遮挡在非常早期的去噪阶段就已建立，因此重新排列早期潜变量结构即可修改最终输出。

Method: 提出两阶段方案：(1) 层-wise实例初始化：利用多模态联合注意力中的上下文共享机制，创建每实例分支，在指定早期步骤根据层顺序融合形成具有预定义布局的统一潜变量；(2) 层-wise语义护理：通过层-wise注意力增强强化区域细节并维持遮挡顺序，顺序分层注意力路径与标准全局路径并行运行，更新在层透明度调度器下合成。

Result: 实现了精确的区域和遮挡可控性，支持可编辑工作流，能够灵活修改实例或重新排列可见顺序。在Diffusion Transformers上有效运行，定性和定量结果均验证了其有效性。

Conclusion: LayerBind是一个无需训练、即插即用的区域和遮挡控制器，可应用于Diffusion Transformers。除了生成任务外，还支持可编辑的工作流，允许灵活修改如更换实例或重新排列可见顺序。定性和定量结果均证明了LayerBind的有效性，展示了其在创意应用中的强大潜力。
...
TL;DR: 提出BM25-V方法，将传统文本检索算法BM25应用于稀疏自编码器提取的视觉词特征，实现高效、可解释的两阶段图像检索，在7个基准上Recall@200≥0.993。


<details>
  <summary>Details</summary>
Motivation: 解决密集图像检索的三个局限：1)可解释性和归因性有限；2)大规模计算成本高；3)文档频率分布不平衡问题未被充分利用。

Method: 使用Sparse Auto-Encoder(SAE)从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分进行检索。利用视觉词文档频率的高度不平衡性（遵循Zipfian分布），通过IDF权重抑制常见低信息量词汇，强调稀有区分性词汇。

Result: 在7个基准测试上达到Recall@200≥0.993，使两阶段管道仅需对每个查询的K=200个候选进行重排序，平均可恢复接近密集检索精度的0.2%以内。SAE在ImageNet-1K上训练一次后可零样本迁移到7个细粒度基准而无需微调。

Conclusion: BM25-V是一种有效的两阶段图像检索方法，通过将BM25算法应用于稀疏自编码器提取的稀疏视觉词激活，在7个基准测试上实现Recall@200≥0.993。该方法仅需对K=200个候选进行重排序即可恢复接近密集检索的精度（平均差距0.2%），同时提供了可归因的检索决策和可解释性。
...
### [18] [Spectral Probing of Feature Upsamplers in 2D-to-3D Scene Reconstruction](https://arxiv.org/abs/2603.05787)
*Ling Xiao,Yuliang Xiu,Yue Chen,Guoming Wang,Toshihiko Yamasaki*

Main category: cs.CV

TL;DR: 本文提出了一个频谱诊断框架，用于分析2D到3D重建管道中特征上采样的质量。框架包含六个互补指标，评估幅度重分配、结构频谱对齐和方向稳定性。研究发现结构频谱一致性(SSC/CSC)是预测新视角合成质量的最强指标，而高频频谱斜率漂移(HFSS)往往与重建性能负相关。


<details>
  <summary>Details</summary>
Motivation: 2D到3D管道通常使用视觉基础模型(VFM)提取特征，然后进行空间上采样以进行3D重建。虽然可学习上采样方法主要增强空间细节(如更锐利的几何或更丰富的纹理)，但它们对3D感知的影响尚未被充分探索。

Method: 提出一个包含六个互补指标的频谱诊断框架，表征幅度重分配、结构频谱对齐和方向稳定性。在CLIP和DINO主干的经典插值和可学习上采样方法上进行实验。

Result: 三个关键发现：1) SSC/CSC是新视角合成质量的最强预测因子，而HFSS常与重建性能负相关；2) 几何和纹理对不同频谱特性有不同响应：角能量一致性(ADC)与几何相关指标相关性更强；3) 虽然可学习上采样器产生更清晰的空间特征，但很少在重建质量上超过经典插值。

Conclusion: 重建质量与保留频谱结构更密切相关，而非增强空间细节。这突出了频谱一致性作为2D到3D管道中设计上采样策略的重要原则。
...
TL;DR: EventGeM是一个用于事件相机视觉地点识别(VPR)的先进全局到局部特征融合管道，结合ViT-S/16、MaxViT和深度估计模型实现高精度实时定位。


<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率特性，在机器人和计算机视觉任务中日益流行，尤其适用于需要精确快速定位或对能耗有严格要求的场景。现有事件基VPR方法在精度和实时性方面仍有提升空间。

Method: 提出三阶段融合 pipeline：1) 使用预训练ViT-S/16获取全局特征patch进行初始匹配预测；2) 采用预训练MaxViT检测局部特征关键点，通过RANSAC进行2D单应性重排序；3) 利用预训练视觉基础模型进行深度估计，比较参考图与查询图的结构相似度进一步细化重排序。

Result: 在多个基准数据集和不同光照条件下实现最先进的定位精度，优于现有最佳事件基VPR方法，同时可在多种计算架构上实时运行，并成功部署于机器人平台进行事件流的在线定位。

Conclusion: EventGeM通过融合全局上下文信息、局部几何验证和深度结构相似性，成功实现了高精度、高效率的事件基视觉地点识别，为事件相机在机器人导航和定位领域的实际应用提供了有效方案。
...
TL;DR: 提出 LIPAR 框架，通过潜在帧间剪枝和注意力恢复机制，在无需额外训练的情况下将视频生成速度提升 1.45 倍，实现 12.2 FPS 的实时推理。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型存在高计算延迟问题，使得实时应用成本过高难以落地。

Method: 提出 Latent Inter-frame Pruning with Attention Recovery (LIPAR) 框架，通过检测并跳过重复计算潜在补丁来减少计算量；同时引入注意力恢复机制，用于近似被修剪标记的注意力值，从而消除简单修剪带来的视觉伪影。

Result: 将视频编辑吞吐量提升1.45倍，在 NVIDIA A6000 上平均达到 12.2 FPS，而基线为 8.4 FPS，且不影响生成质量。

Conclusion: 该方法有效地弥合了传统压缩算法与现代生成管道之间的差距，可无缝集成到模型中而无需额外训练，在不牺牲生成质量的前提下显著提升了视频编辑的吞吐量。
...
### [21] [CORE-Seg: Reasoning-Driven Segmentation for Complex Lesions via Reinforcement Learning](https://arxiv.org/abs/2603.05911)
*Yuxin Xie,Yuming Chen,Yishan Yang,Yi Zhou,Tao Zhou,Zhen Zhao,Jiacheng Liu,Huazhu Fu*

Main category: cs.CV

TL;DR: 提出首个复杂病变推理分割基准ComLesion-14K及CORE-Seg框架，通过语义引导提示适配器和渐进式训练策略实现推理与分割的端到端整合。


<details>
  <summary>Details</summary>
Motivation: 现有通用MLLMs缺乏复杂病变的专业视觉推理能力，而传统分割模型缺乏逻辑可解释性，需建立专门的推理驱动分割基准和方法。

Method: 提出CORE-Seg框架，包含语义引导提示适配器(Semantic-Guided Prompt Adapter)，采用从SFT到GRPO的渐进式训练策略，并配备自适应双粒度奖励机制以缓解奖励稀疏性问题。

Result: 实现最优Dice分数37.06%，较最佳基线提升14.89%，失败率降至18.42%。

Conclusion: CORE-Seg通过将推理能力与分割任务相结合，在复杂病变分割任务上取得了显著进展，Dice分数提升14.89%，失败率降至18.42%，验证了推理驱动方法在医学图像分割领域的有效性。
...
TL;DR: 本文提出了BlackMirror框架，用于在黑盒设置下检测文本到图像模型的后门攻击。该框架包含两个组件：MirrorMatch用于对齐视觉模式与对应指令以检测语义偏差；MirrorVerify用于评估这些偏差在不同提示下的稳定性。BlackMirror是一个通用的、无需训练的框架，可作为即插即用模块部署在MaaS应用中。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖于分析图像级相似性，假设后门触发生成在样本间表现出一致性。然而，它们难以泛化到新兴的后门攻击，因为这些攻击的生成图像可以呈现视觉多样性。

Method: BlackMirror基于一个观察：在各种后门攻击中，只有生成图像内的部分语义模式被稳定操纵，而其余内容保持多样或良性。框架包含两个组件：1) MirrorMatch：对齐视觉模式与相应指令以检测语义偏差；2) MirrorVerify：评估这些偏差在不同提示下的稳定性，以区分真正的后门行为与良性响应。

Result: 全面实验表明，BlackMirror在各种攻击中实现准确检测。

Conclusion: BlackMirror是一个通用的、无需训练的框架，可以作为即插即用模块部署在Model-as-a-Service(MaaS)应用中。
...
### [23] [Cog2Gen3D: Sculpturing 3D Semantic-Geometric Cognition for 3D Generation](https://arxiv.org/abs/2603.05845)
*Haonan Wang,Hanyu Zhou,Haoyue Liu,Tao Gu,Luxin Yan*

Main category: cs.CV

TL;DR: Cog2Gen3D是一个3D认知引导的扩散框架，用于3D高斯生成。该模型通过认知特征嵌入、3D潜在认知图和认知引导的潜在扩散三个关键设计，将语义信息和绝对几何相结合，实现可控的3D生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法只能建模相对关系，容易出现绝对几何尺度不一致的问题。作者认为语义信息和绝对几何赋能3D认知，能够实现物理世界的可控3D生成。

Method: 提出三个关键设计：1)认知特征嵌入：将不同模态编码为语义和几何表示，并提取逻辑表示；2)3D潜在认知图：将不同表示构建为双流语义-几何图，通过基于公共的交叉注意力融合获得3D认知图；3)认知引导的潜在扩散：利用融合的3D认知图作为条件引导潜在扩散过程进行3D高斯生成。

Result: 在Marble World Labs构建的验证集上，Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架确保了3D生成的物理可信性和结构合理性，为3D生成领域提供了新的研究方向。
...
### [24] [RAC: Rectified Flow Auto Coder](https://arxiv.org/abs/2603.05925)
*Sen Fang,Yalin Feng,Yanxin Zhang,Dimitris N. Metaxas*

Main category: cs.CV

TL;DR: 提出了一种基于Rectified Flow的Rectified Flow Auto Coder (RAC)，通过多步解码和双向推理替代传统VAE，在减少41%参数和70%计算成本的同时，在重建和生成质量上超越SOTA VAE。


<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，且通常需要独立编码器和解码器，导致参数量大。作者希望用Rectified Flow来改进这一问题。

Method: 1. 采用多步解码，将解码器应用于流时间步；2. 内在支持双向推理，通过时间反转将解码器作为编码器；3. 解码路径直且可修正，实现逐步细化。

Result: 在重建和生成质量上超越SOTA VAE，同时参数量减少约41%，计算成本降低约70%。

Conclusion: RAC通过流模型和双向推理机制，有效缩小了重建-生成差距，以更少的参数和计算代价获得了更好的生成效果。
...
TL;DR: VS3R是一个结合前馈3D重建与生成式视频扩散的视频稳定化框架，通过联合估计相机参数、深度和掩码、混合稳定渲染模块以及双流视频扩散模型，解决了传统方法在裁剪和极端运动下的局限性，实现了高保真完整帧稳定化


<details>
  <summary>Details</summary>
Motivation: 视频稳定化面临几何鲁棒性与完整帧一致性之间的根本权衡：2D方法遭受剧烈裁剪，3D技术常因脆弱的优化流程在极端运动下失败

Method: 提出VS3R框架，将前馈式3D重建与生成式视频扩散相结合。流水线联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；最后使用双流视频扩散模型通过结构指导和语义锚点协同修复遮挡区域和矫正伪影

Result: 在多种相机模型上实现高保真完整帧稳定化，在鲁棒性和视觉质量方面显著优于现有最先进方法

Conclusion: VS3R成功实现了高保真、完整帧的视频稳定化，在各种相机模型上均表现出色，在鲁棒性和视觉质量方面显著优于现有最先进方法
...
TL;DR: 本文构建了TumorCoT大规模数据集（150万CoT标注的VQA指令+3D CT扫描），并提出TumorChain多模态交错推理框架，实现从finding到impression到pathology的跨模态推理，提高肿瘤分析的可追溯性并降低幻觉风险。


<details>
  <summary>Details</summary>
Motivation: 准确的肿瘤分析对临床放射学和精准肿瘤学至关重要，早期检测、可靠的病变特征描述和病理级风险评估指导诊断和治疗计划。Chain-of-Thought（CoT）推理在临床肿瘤分析中特别重要，因为它能够实现从成像发现到临床印象和病理结论的逐步解释，提高可追溯性并减少诊断错误。

Method: 1. 构建TumorCoT数据集：150万个CoT标注的VQA指令，配合3D CT扫描，包含从finding到impression到pathology的步骤对齐理由和跨模态对齐；2. 提出TumorChain多模态交错推理框架：紧密结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代交错因果推理进行视觉证据定位、结论聚合和多轮自refinement后的病理预测。

Result: 在病灶检测、印象生成和病理分类方面一致优于强基线，并在DeepTumorVQA基准上展现出强大的泛化能力。

Conclusion: TumorChain框架通过跨模态对齐和迭代交错因果推理，能够对视觉证据进行定位、聚合结论并在多轮自 refinement后做出病理预测，提高可追溯性并降低幻觉风险。实验表明，该方法在病灶检测、印象生成和病理分类方面均优于基线模型，并在DeepTumorVQA基准上展现出强大的泛化能力，突出了多模态推理在临床肿瘤分析中的潜力。
...
### [27] [Energy-Driven Adaptive Visual Token Pruning for Efficient Vision-Language Models](https://arxiv.org/abs/2603.05950)
*Jialuo He,Huangxun Chen*

Main category: cs.CV

TL;DR: E-AdaPrune 提出基于奇异值谱能量保留的自适应视觉 token 剪枝框架，根据图像信息密度动态分配 token 预算，无需额外参数。在三个 VLM 模型和九个基准上，平均提升 0.6%，MMVet 提升 5.1%，延迟仅 8ms。


<details>
  <summary>Details</summary>
Motivation: 现有视觉 token 压缩方法大多采用固定预算策略，忽视了不同图像间信息密度的显著差异。图像的信息含量各异——某些场景包含丰富细节需要更多 token 表征，而另一些场景可能存在大量冗余信息可以进一步压缩。因此，需要一种能够根据图像内容自适应调整 token 预算的方法，以实现更高效的视觉语言模型推理。

Method: E-AdaPrune 采用能量驱动的自适应剪枝框架，通过分析视觉特征的奇异值谱来确定 token 预算。具体而言，该方法保留一定比例的谱能量（singular value spectrum），并据此为不同信息密度的场景分配合适的 token 数量——信息丰富的场景分配更多 token，冗余场景则进行激进压缩。推理时使用随机奇异值分解（SVD）计算，额外延迟控制在 8ms 以内，且不引入任何可学习参数。

Result: 在 LLaVA-1.5-7B、LLaVA-1.5-13B 和 LLaVA-NeXT-8B 三个 VLM 骨干模型上，E-AdaPrune 在九个基准数据集上进行了评估。在匹配平均 token 预算的条件下，该方法实现了平均最高 0.6% 的性能提升；在 MMVet 推理任务上获得了显著的 +5.1% 相对提升；额外推理延迟仅为 8ms/图。

Conclusion: E-AdaPrune是一种无需额外可学习参数的高效 token 压缩方法，通过保留奇异值谱的能量比例来实现自适应的 token 预算分配。在三个主流 VLM 骨干模型和九个基准数据集上，该方法在保持平均 token 预算一致的前提下，实现了平均 0.6% 的性能提升，MMVet 推理任务相对提升 5.1%，额外延迟仅 8ms，证明了基于谱能量的自适应剪枝策略的有效性。
...
TL;DR: PatchCue是一种新型的基于补丁的视觉提示范式，通过将图像分割成补丁并在补丁级别表示提示来增强视觉语言模型的推理能力，采用两阶段训练方法（冷启动监督微调和强化学习），在多种VLMs和基准测试上表现优于像素级边界框和点提示。


<details>
  <summary>Details</summary>
Motivation: 现有的推理范式（如思维链CoT）仅依赖文本信息，未充分利用视觉线索。虽然先前工作引入了像素级视觉线索，但这些表示需要精确的空间定位，增加了学习复杂度。

Method: 提出两阶段训练方法：第一阶段是冷启动监督微调，使模型输出补丁级线索；第二阶段是强化学习，使用过程监督的线索奖励来指导中间视觉推理步骤。

Result: 在多种VLMs和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue一致性地提升了模型整体性能。补丁级线索优于像素级边界框和点提示。

Conclusion: 补丁级线索提供了一种更有效且更符合认知的视觉推理范式，能够更好地与人类感知习惯对齐，并利用现代VLMs的补丁-token化输入。
...
### [29] [Skeleton-to-Image Encoding: Enabling Skeleton Representation Learning via Vision-Pretrained Models](https://arxiv.org/abs/2603.05963)
*Siyuan Yang,Jun Liu,Hao Cheng,Chong Wang,Shijian Lu,Hedvig Kjellstrom,Weisi Lin,Alex C. Kot*

Main category: cs.CV

TL;DR: 本文提出Skeleton-to-Image Encoding (S2I)方法，将3D人体骨架序列转换为图像-like表示，使预训练视觉模型能够应用于骨架数据的自监督表示学习。


<details>
  <summary>Details</summary>
Motivation: 大规模预训练视觉模型在跨模态和多模态任务中表现出色，但直接应用于3D人体骨架数据存在格式差异挑战。此外，骨架数据集规模有限，且需要将骨架数据融入多模态动作识别而不引入额外分支。

Method: 提出S2I表示方法，通过基于身体部位语义对关节点进行分区和排列，并调整至标准化图像尺寸，将骨架序列转换为图像-like数据。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上验证了方法的有效性和泛化性，包括在挑战性的跨格式评估设置下。

Conclusion: S2I方法首次实现了强大的视觉预训练模型用于自监督骨架表示学习，同时提供统一的图像-like格式，自然容纳来自不同数据源的异构骨架数据。
...
TL;DR: ProCap是一个将变化建模从静态图像比较转变为动态过程建模的新框架，通过两阶段设计学习变化过程的潜在动态表示，并使用可学习的过程查询在编码器-解码器架构中生成变化描述。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅操作静态图像对，忽略了变化过程的丰富时间动态，而时间动态是理解发生了什么变化以及如何发生的关键。

Method: 提出两阶段框架：第一阶段训练过程编码器从关键帧学习变化过程，通过自动生成中间帧使隐含的程序动态显式化并采样以减少冗余，通过标题条件的掩码重建任务捕获关键帧的潜在动态；第二阶段将训练好的编码器集成到编码器-解码器模型中用于标题生成，引入可学习的过程查询来提示编码器推断潜在过程表示，然后由解码器将其转换为文本。整个模型通过标题生成损失进行端到端训练。

Result: 在三个数据集上的实验证明了ProCap的有效性。

Conclusion: ProCap在三个数据集上证明了其有效性，通过将静态图像比较转变为动态过程建模，有效提升了变化描述的质量和一致性。
...
TL;DR: 本文系统评估了合成新颖视角在视频地点识别(VPR)中的应用。使用5个公共VPR数据库和7种图像相似度方法进行实验，发现少量合成视角添加可提升VPR识别效果，但大量添加时，视图数量和图像类型比视角变化幅度更为重要。


<details>
  <summary>Details</summary>
Motivation: 合成新颖视角有潜力积极影响机器人导航。在基于图像的导航中，从地面机器人拍摄场景生成的合成鸟瞰图可用于引导空中机器人；在视频地点识别(VPR)中，来自空中视角的地面位置合成视图可使无人机识别地面机器人见过的地点，反之亦然。

Method: 使用五个公共VPR图像数据库和七种典型的图像相似度方法进行系统性评估。实验比较了不同大小的合成视图添加对VPR识别统计的影响。

Result: 研究表明，较小的合成视图添加能够改善VPR识别统计结果。当合成添加量较大时，视角变化幅度的重要性降低，添加的视图数量和数据集图像类型成为更关键的因素。

Conclusion: 对于较大的合成视图添加，视角变化幅度不如添加的视图数量和数据集图像类型重要。这意味着在VPR任务中，增加视图多样性可能比追求视角一致性更为关键。
...
TL;DR: 该报告探讨了手术器械制造缺陷问题，利用深度学习模型（YOLOv8、ResNet-152、EfficientNet-b4）自动检测裂纹、锈蚀等关键缺陷，通过分析4414张高分辨率图像的数据集，提升巴基斯坦手术器械制造的质量标准。


<details>
  <summary>Details</summary>
Motivation: 手术器械制造中的微小缺陷可能导致致命后果，传统人工检测效率低且易出错，需要自动化光学检测（AOI）方法来提升质量控制水平，保障患者安全。

Method: 使用新开发的4414张高分辨率图像数据集，集成YOLOv8、ResNet-152和EfficientNet-b4三种深度学习架构进行缺陷自动检测，并与Sialkot手术器械集群的Daddy D Pro和Dr. Frigz International公司合作获取行业洞察。

Result: 识别了巴基斯坦制造的各类手术器械及其相关缺陷类型（裂纹、锈蚀、结构不规则等），建立了自动化缺陷检测系统，为制造商、医疗专业人员和监管机构提供质量保证资源。

Conclusion: 通过深度学习与自动光学检测技术的结合，可有效提升手术器械制造的质量标准，减少缺陷导致的患者风险和经济损失，推动医疗制造行业的可靠性发展。
...
TL;DR: CylinderSplat是一种用于全景3D高斯溅射的前馈框架，通过圆柱三平面表示和双分支架构解决全景图像的新视角合成问题，能够处理从单视角到多视角的任意数量输入视图。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两大局限：1）多视图代价体积依赖密集视图，难以处理稀疏视图下的遮挡问题；2）标准笛卡尔三平面表示无法有效捕捉360°场景的内在几何结构，导致畸变和混叠现象。

Method: 提出圆柱三平面表示方法，更符合全景数据和曼哈顿世界假设的真实世界结构。采用双分支架构：像素分支重建观测良好的区域，体积分支利用圆柱三平面完成遮挡或稀疏观测区域。框架可灵活处理可变数量的输入视图。

Result: 在单视图和多视图全景新视角合成任务中均达到最先进水平，重构质量和几何精度均优于先前方法。

Conclusion: 该工作成功将前馈3DGS扩展到全景领域，圆柱三平面表示有效解决了360°场景的几何表示难题，双分支架构有效处理了遮挡和稀疏观测问题。
...
TL;DR: 本文提出MM-ISTS，一个利用视觉-文本大语言模型增强的多模态框架，用于解决不规则采样时间序列(ISTS)预测问题


<details>
  <summary>Details</summary>
Motivation: 现有ISTS预测方法仅利用历史观测预测未来，缺乏对上下文语义和细粒度时间模式的学习能力

Method: 提出两阶段编码机制：(1)跨模态视觉-文本编码模块，自动生成图像和文本，捕捉时间模式和上下文；(2)ISTS编码提取多视图嵌入融合和时间变量编码的特征；同时提出自适应查询特征提取器和模态感知门控的多模态对齐模块

Result: 在真实数据上进行了广泛实验，验证了所提方案的有效性

Conclusion: MM-ISTS框架通过桥接时间、视觉和文本模态，有效提升了不规则采样时间序列的预测性能
...
TL;DR: PixARMesh是一种从单张RGB图像自回归重建完整3D室内场景网格的方法，与依赖隐式符号距离场和后处理布局优化的现有方法不同，它在统一模型内联合预测对象布局和几何结构，在单次前向传递中生成连贯的、艺术级品质的网格。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在以下问题：1）依赖隐式符号距离场(SDF)表示，难以直接生成可用的网格；2）需要后处理布局优化，流程繁琐且效率低下；3）无法在单次前向传递中同时准确预测对象布局和几何形状。

Method: 1）基于点云编码器，通过交叉注意力机制融合像素对齐的图像特征和全局场景上下文；2）采用自回归生成方式，从包含上下文、姿态和网格的统一token流中逐步生成场景；3）在统一模型内联合预测对象布局和几何结构。

Result: 在合成数据集和真实世界数据集上的实验表明，PixARMesh实现了最先进的重建质量，同时生成轻量级、高品质的网格，可直接用于下游应用。

Conclusion: 该方法为从单张图像进行3D场景重建提供了一种高效、统一的解决方案，生成的网格兼具高质量几何细节和紧凑性，具有重要的应用价值。
...
TL;DR: RepKAN是一种新型遥感图像分类架构，通过融合CNN和KAN的优势，采用双路径设计实现可解释的物理推理，在EuroSAT和NWPU-RESISC45数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像分类对地球观测至关重要，但标准CNN和Transformer通常作为不可解释的黑盒运作，缺乏物理可解释性。

Method: 提出了一种名为RepKAN的新型架构，采用双路径设计——空间线性(Spatial Linear)和光谱非线(Spectral Non-linear)，融合了CNN的结构效率和KAN的非线性表示能力，实现了对类特定光谱指纹和物理交互流形的自主发现。

Result: 在EuroSAT和NWPU-RESISC45数据集上的实验表明，RepKAN在性能上优于最先进模型，同时提供了明确的物理可解释推理能力。

Conclusion: RepKAN具有作为未来可解释视觉基础模型骨干的显著潜力，能够在保持高性能的同时提供明确的物理可解释性推理。
...
TL;DR: InnoAds-Composer是一种单阶段电商产品海报生成框架，通过三重条件控制token（主体、字形、风格）实现高效控制，解决传统多阶段pipeline存在的保真度差、文字不准、风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 电商产品海报生成需要同时呈现产品主体、文字和设计风格，现有扩散模型虽具备细粒度控制能力，但通常依赖多阶段流水线，且三重条件同时控制研究不足。多阶段方法存在三个关键问题：主体保真度差、文字生成不准确、风格不一致。

Method: 1)提出单阶段InnoAds-Composer框架，实现三重条件控制token；2)通过层和时间步的重要性分析，将每个条件路由到最响应位置，缩短活跃token序列；3)设计Text Feature Enhancement Module (TFEM)整合字形图像和字形裁剪特征提升中文文字渲染准确率；4)构建高质量电商产品海报数据集和基准。

Result: 实验表明InnoAds-Composer显著优于现有产品海报方法，且不明显增加推理延迟。

Conclusion: InnoAds-Composer通过单阶段设计有效解决了多阶段pipeline的三个核心问题，为电商产品海报生成提供了高效且高质量的解决方案，同时构建的首个三元条件联合数据集为后续研究提供了重要基准。
...
TL;DR: 研究VLM在自动驾驶场景中失败的原因，通过线性探针分析激活发现两种失败模式：感知失败（视觉信息未线性编码）和认知失败（信息存在但未与语言对齐），并揭示目标距离对线性可分性的影响。


<details>
  <summary>Details</summary>
Motivation: VLM在自动驾驶应用中越来越常见，但常在高度相关的简单视觉问题上失败，且失败原因尚不明确，需要理解其失败机制。

Method: 通过创建仅在特定视觉概念上不同的反事实图像集，然后使用线性探针在四个SOTA VLM的激活上训练来区分这些图像，从而评估视觉概念的线性编码程度。

Result: 某些概念（如物体/ agent的存在）被显式和线性编码，而其他空间视觉概念（如方向）仅被视觉编码器保留的空间结构隐式编码；即使概念线性编码，模型有时仍会失败；目标距离增加会快速降低线性可分性。

Conclusion: 提高对VLMs在自动驾驶相关简单视觉任务中失败案例的理解，发现两种失败模式：感知失败（视觉信息未线性编码）和认知失败（视觉信息存在但未与语言语义正确对齐），目标距离增加会快速降低线性可分性。
...
TL;DR: TempoSyncDiff是一种参考条件潜扩散框架，通过师生蒸馏实现少步推理，用于高效音频驱动的说话人头生成，兼顾身份稳定性、时间一致性和唇部运动控制。


<details>
  <summary>Details</summary>
Motivation: 当前说话人头生成(THG)面临三大挑战：高推理延迟、时间不稳定（闪烁和身份漂移）、困难语音条件下的视听对齐不佳，限制了扩散模型在实际应用中的可行性。

Method: 采用师生蒸馏框架，用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器；引入身份锚定和时间正则化来缓解身份漂移和帧间闪烁；使用音素基音频调节提供粗略唇部运动控制；在LRS3数据集上评估去噪阶段组件级指标和CPU/边缘计算延迟。

Result: 在LRS3数据集上的实验表明，蒸馏扩散模型能在大幅降低延迟的同时保留教师的重建性能，证明了少步推理的可行性及边缘部署的潜力。

Conclusion: 蒸馏扩散模型能够在显著降低延迟的情况下保留较强教师的重建行为，是实现约束计算环境下实际扩散式说话人头生成的第一步。该研究为在边缘设备上部署高效的音频驱动说话人头生成系统提供了可行性依据。
...
TL;DR: 针对无人机图像中小目标检测中尺度变化、结构细节退化和计算资源受限的问题，提出CollabOD轻量级协作检测框架。该框架通过结构细节保留、跨路径特征对齐和定位感知轻量级设计三大策略，在高效推理的同时增强特征表示稳定性，提升定位鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 无人机图像中小目标检测面临三大挑战：1)尺度变化(scale variation)；2)结构细节退化(structural detail degradation)；3)计算资源有限(limited computational resources)。高海拔场景下，精细特征在层级降采样和跨尺度融合过程中进一步削弱，导致定位不稳定和鲁棒性下降。

Method: 提出CollabOD框架，整合三大策略：1)结构细节保留(Structural Detail Preservation)；2)跨路径特征对齐(Cross-Path Feature Alignment)；3)定位感知轻量级设计(Localization-Aware Lightweight Design)。采用统一细节感知检测头(Detail-Aware Detection Head)提升回归鲁棒性，从图像处理、通道结构和轻量级设计三个角度优化传统无人机感知模型架构。

Result: 该框架增强了表示稳定性(representation stability)，保持了高效推理(efficient inference)，并提升了回归鲁棒性(regression robustness)。

Conclusion: CollabOD框架通过结构细节保留、跨路径特征对齐和定位感知轻量级设计，在保持高效推理的同时增强了表示稳定性，提高了回归鲁棒性，且不引入额外的部署开销。
...
TL;DR: 本文提出情感连续对话人脸生成(EC-TFG)新任务及TIE-TFG模型，通过Temporal-Intensive Emotion Fluctuation Modeling实现动态情感变化管理，生成具有平滑情感过渡和高品质视觉效果的对话视频。


<details>
  <summary>Details</summary>
Motivation: 现有Talking Face Generation (TFG)研究虽然已掌握创建自然面部运动的能力，但通常在合成视频中表达固定目标情感，缺乏像人类一样传达信息时展现连续变化和自然表情的能力。

Method: 提出TIE-TFG模型，采用Temporal-Intensive Emotion Fluctuation Modeling技术，创新性地管理动态情感变化，提供与输入文本对应的情感变化序列，驱动合成视频中的连续面部表情变化。

Result: 广泛评估表明，该方法在多种情感状态下都能产生平滑的情感过渡，并保持高质量的视觉和运动真实性。

Conclusion: 提出的TIE-TFG模型通过Temporal-Intensive Emotion Fluctuation Modeling技术，能够有效管理动态情感变化，生成具有平滑情感过渡、高质量视觉和运动真实感的对话视频。
...
TL;DR: 提出艺术视差合成（Artistic Disparity Synthesis）新范式，从物理准确的视差估计转向艺术上一致的视差合成。Art3D框架通过双路径架构解耦全局深度参数与局部艺术效果，学习专业3D电影数据，可实现电影级3D效果


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽能达到几何精度，但在艺术表现上存在不足，无法复制专业3D电影的沉浸式和情感共鸣体验。原因是几何重建范式将故意的艺术意图（如用于突出效果的战略零平面偏移和局部深度雕刻）误认为是数据噪声或歧义

Method: 提出Art3D框架，采用双路径架构将全局深度参数（宏意图）与局部艺术效果（视觉笔触）解耦，通过间接监督从专业3D电影数据中学习

Result: 实验表明，该方法在复制关键的空间出屏效果和与电影3D内容的全局深度风格对齐方面展现出潜力

Conclusion: 为艺术家驱动的转换工具奠定了基础，提供了一种有前景的新方法来实现电影级3D效果
...
### [43] [Place-it-R1: Unlocking Environment-aware Reasoning Potential of MLLM for Video Object Insertion](https://arxiv.org/abs/2603.06140)
*Bohai Gu,Taiyi Wu,Dazhao Du,Jian Liu,Shuai Yang,Xiaotong Zhao,Alan Zhao,Song Guo*

Main category: cs.CV

TL;DR: Place-it-R1是一个利用多模态大语言模型(MLLM)实现视频物体插入的端到端框架，通过"先思考后放置"的范式，结合CoT推理和空间DPO优化，在保证视觉真实性的同时实现物理一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术虽能实现高视觉保真度，但专注于视觉优化而忽视物理因果关系，导致插入物体与环境物理上不一致。该工作旨在解决视频物体插入中的物理合理性问题。

Method: 提出三大创新：1) MLLM进行物理场景理解和交互推理，生成环境感知的CoT tokens并推断有效插入区域；2) MLLM引导的空间直接偏好优化(DPO)，将扩散输出反馈给MLLM评分形成闭环迭代优化；3) 提供两种可选模式：可修改场景增强物理合理性的灵活模式，以及保持场景完整性的标准模式。

Result: 实验表明Place-it-R1实现了比现有SOTA方案和商业模型更具物理一致性的视频物体插入效果。

Conclusion: 通过利用MLLM的认知推理能力结合扩散模型的生成执行能力，成功实现了物理合理性与视觉保真度的平衡，为视频编辑领域提供了新的解决方案。
...
TL;DR: Pano3DComposer是一个用于全景图像的高效前馈3D场景生成框架，通过Object-World Transformation Predictor解耦对象生成与布局估计，并采用伪几何监督训练和C2F对齐机制，在约20秒内生成高保真360度3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有组合图像到3D场景生成方法存在以下局限：(1)耗时的迭代布局优化；(2)不灵活的联合对象-布局生成；(3)依赖有限视野透视图像，无法生成完整的360度环境。

Method: 1) 设计了Object-World Transformation Predictor模块，将3D对象从局部坐标转换到世界坐标，实现对象生成与布局估计的解耦；2) 适配VGGT为Alignment-VGGT，使用目标对象裁剪、多视图对象渲染和相机参数预测变换；3) 采用伪几何监督训练解决生成对象与真实对象之间的形状差异；4) 引入Coarse-to-Fine(C2F)对齐机制迭代细化几何一致性。

Result: 在合成和真实数据集上实现了图像/文本到3D任务的优越几何精度，在RTX 4090 GPU上约20秒生成高保真3D场景。

Conclusion: Pano3DComposer是一个高效的图像到3D场景生成前馈框架，能够在约20秒内生成高保真3D场景，在合成和真实数据集上实现了优越的几何精度。
...
### [45] [VLM-RobustBench: A Comprehensive Benchmark for Robustness of Vision-Language Models](https://arxiv.org/abs/2603.06148)
*Rohit Saxena,Alessandro Suglia,Pasquale Minervini*

Main category: cs.CV

TL;DR: 提出了VLM-RobustBench基准测试，涵盖49种图像退化类型，用于评估视觉语言模型在真实世界图像失真下的表现，揭示了当前VLM语义强但空间脆弱的特性。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现优异，但人们对它们在真实世界图像失真下的表现了解有限，需要系统性地评估模型在各种图像退化情况下的鲁棒性。

Method: 构建了包含49种增强类型的基准测试，涵盖噪声、模糊、天气、数字和几何扰动，在低/中/高三个严重等级和二元变换下进行评估，共133种退化设置。评估了四个家族的VLM模型（Qwen、InternVL、Molmo、Gemma）在两个互补基准（MMBench和MMMU-Pro）上的表现。

Result: 研究发现视觉严重程度是较弱的难度预测指标：低严重程度的空间扰动往往比视觉上更严重的 photometric 退化对性能影响更大。特别是低严重程度的glass_blur使MMBench准确率平均下降约8个百分点，而最大的性能下降来自重采样和几何失真（如upsample、elastic_transform），降幅高达34个百分点。

Conclusion: 当前VLM语义理解能力强但空间处理脆弱，需要新的鲁棒性评估协议和训练方法来强调重采样和几何不变性。
...
### [46] [Reflective Flow Sampling Enhancement](https://arxiv.org/abs/2603.06165)
*Zikai Zhou,Muyao Wang,Shitong Shao,Lichen Bai,Haoyi Xiong,Bo Han,Zeke Xie*

Main category: cs.CV

TL;DR: 本文提出RF-Sampling，一种为流模型设计的无训练推理增强框架，通过文本表示线性组合和流反转技术改善FLUX等模型的生成质量和提示对齐能力，并首次实现流模型上的测试时扩展。


<details>
  <summary>Details</summary>
Motivation: 现有推理时增强策略主要适用于传统扩散模型，在流模型上表现不佳，需要为流模型（尤其是CFG蒸馏变体如FLUX）设计专门的推理增强方法。

Method: 提出Reflective Flow Sampling (RF-Sampling)框架，通过文本表示的线性组合并与流反转集成，让模型探索更符合输入提示的噪声空间。该方法基于严格推导，证明RF-Sampling隐式执行文本-图像对齐分数的梯度上升。

Result: 在多个基准测试上的广泛实验表明，RF-Sampling持续改善了生成质量和提示对齐性能，且是首个能在FLUX上展现测试时扩展能力的推理增强方法。

Conclusion: RF-Sampling是首个能够在FLUX上展现一定测试时扩展能力的推理增强方法，为流模型的推理优化提供了新思路。
...
### [47] [Contrastive-to-Self-Supervised: A Two-Stage Framework for Script Similarity Learning](https://arxiv.org/abs/2603.06180)
*Claire Roman,Philippe Meyer*

Main category: cs.CV

TL;DR: 本文提出一种两阶段框架解决文字历史关系不确定性问题：先在人造字母表上训练监督对比学习教师模型，再通过师生蒸馏让学生模型学习历史文字的无监督表示，实现跨脚本相似性发现。


<details>
  <summary>Details</summary>
Motivation: 字形和书写系统相似性度量的核心挑战在于：虽然人造字母表中的单个字素可以被可靠标记，但不同书写系统之间的历史关系仍不确定且存在争议，传统的监督学习方法难以直接应用。



Method: 采用两阶段框架：第一阶段使用对比损失在标记的人造字母表上训练编码器，建立具有强判别特征的教师模型；第二阶段通过师生蒸馏扩展到历史文献脚本，学生模型在教师知识指导下学习无监督表示，可自由发现潜在跨脚本相似性。



Result: 实验表明，该方法在多种书写系统上实现了有效的少量样本字形识别和有意义的脚本聚类，无需依赖真实演化关系标签。



Conclusion: 本框架通过两阶段师生蒸馏方法，有效结合了监督对比学习与无监督发现，在无需真实演化关系标签的情况下，实现了少量样本的字形识别和有意义脚本聚类，为文字学历史关系研究提供了新的计算工具。
...
### [48] [Cut to the Chase: Training-free Multimodal Summarization via Chain-of-Events](https://arxiv.org/abs/2603.06213)
*Xiaoxing You,Qiang Huang,Lingyu Li,Xiaojun Chang,Jun Yu*

Main category: cs.CV

TL;DR: CoE是一种无需训练的多模态摘要框架，通过层次化事件图（HEG）和事件链（Chain-of-Events）引导进行结构化推理，解决现有方法中领域依赖、跨模态对齐薄弱和时间建模不足的问题，在8个数据集上显著优于现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个主要挑战：(1)依赖领域特定监督；(2)隐式融合导致跨模态对齐较弱；(3)平面时间建模缺乏事件转换。这些问题限制了多模态摘要系统的泛化能力和可解释性。

Method: 提出CoE，一个无需训练的MMS框架，通过事件链（Chain-of-Events）引导和层次化事件图（HEG）进行结构化推理。HEG将文本语义编码为明确的事件层次结构，支持跨模态对齐和时间推理。在该结构引导下，CoE定位关键视觉线索、建模事件演化和因果转换，并通过轻量级风格适应进行领域对齐。

Result: 在8个不同数据集上的广泛实验表明，CoE始终优于最先进的视频CoT基线方法，取得了显著的性能提升。

Conclusion: CoE框架在8个不同数据集上展现出优异的性能，实现了+3.04 ROUGE、+9.51 CIDEr和+1.88 BERTScore的平均提升，证明了其强大的鲁棒性、可解释性和跨域泛化能力。
...
TL;DR: RAID是一个大规模驾驶场景数据集，包含4,691个注释视频片段，用于研究驾驶员风险感知和上下文风险评估，涵盖多种交通场景并标注了驾驶员意图操作、道路拓扑、风险情况、驾驶员反应和行人注意力等标签。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞移动是智能车辆系统的关键目标，这需要理解驾驶员风险感知——一个复杂的认知过程，受驾驶员对外部刺激的自主反应和周围道路用户对自车的注意力影响。

Method: 提出了一种弱监督风险目标识别框架，通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。同时分析了行人在估计风险中的作用。

Result: 在RAID和HDDS数据集上，该方法分别实现了20.6%和23.1%的性能提升，超越先前最先进的方法。

Conclusion: RAID数据集和提出的弱监督风险识别框架能够有效支持驾驶员风险感知研究，为智能车辆系统的安全性提升提供了有价值的资源。
...
### [50] [TaPD: Temporal-adaptive Progressive Distillation for Observation-Adaptive Trajectory Forecasting in Autonomous Driving](https://arxiv.org/abs/2603.06231)
*Mingyu Fan,Yi Liu,Hao Zhou,Deheng Qian,Mohammad Haziq Khan,Matthias Raetsch*

Main category: cs.CV

TL;DR: TaPD是一个统一的可插拔框架，用于可变历史长度下的观测自适应轨迹预测，通过渐进知识蒸馏和时间回填技术解决短观测条件下的轨迹预测难题


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器假设固定长度的历史轨迹，在真实场景中当观测可变或极短时（如遮挡或传感范围有限）性能显著下降

Method: 提出TaPD框架，包含两个协同模块：1) 基于渐进知识蒸馏(PKD)的观测自适应预测器(OAF)，通过分层特征回归将长视距"教师"知识转移到短视距"学生"；2) 时间回填模块(TBM)用于显式重建缺失历史片段。引入余弦退火蒸馏权重方案平衡预测监督和特征对齐，采用解耦的预训练-重建-微调协议保留真实运动先验

Result: TaPD在所有观测长度上持续优于强基线，在极短输入下提升尤其显著，并能以即插即用方式改进其他预测器

Conclusion: TaPD在Argoverse 1和Argoverse 2数据集上显著优于现有方法，特别是在极短输入条件下表现优异，且可以作为即插即用的模块改进其他预测器（如HiVT）
...
### [51] [Beyond Static Frames: Temporal Aggregate-and-Restore Vision Transformer for Human Pose Estimation](https://arxiv.org/abs/2603.05929)
*Hongwei Fang,Jiahang Cai,Xun Wang,Wenwu Yang*

Main category: cs.CV

TL;DR: 提出TAR-ViTPose，一种用于视频2D人体姿态估计的时间聚合-恢复Vision Transformer，通过联合中心时间聚合和全局恢复注意力增强静态ViT表示，在PoseTrack2017上比ViTPose提高+2.3 mAP。


<details>
  <summary>Details</summary>
Motivation: 现有ViT姿态估计器为静态图像设计，每帧独立处理，忽略视频序列中的时间一致性，导致在运动模糊、遮挡或失焦等挑战性场景中预测不稳定。

Method: 提出TAR-ViTPose，包含两个关键组件：1) 联合中心时间聚合(JTA)：为每个关节分配可学习查询令牌，选择性地关注相邻帧中对应区域；2) 全局恢复注意力(GRA)：将聚合的时间特征恢复到当前帧的令牌序列中，丰富姿态表示同时保留全局上下文。

Result: 在PoseTrack2017基准上比单帧基线ViTPose提高+2.3 mAP，优于现有最先进的视频方法，且运行帧率更高。

Conclusion: TAR-ViTPose在PoseTrack2017基准上比单帧基线ViTPose提高了+2.3 mAP，优于现有最先进的视频方法，同时具有更高的运行帧率。
...
### [52] [GazeMoE: Perception of Gaze Target with Mixture-of-Experts](https://arxiv.org/abs/2603.06256)
*Zhuangzhuang Dai,Zhongxi Lu,Vincent G. Zakka,Luis J. Manso,Jose M Alcaraz Calero,Chen Li*

Main category: cs.CV

TL;DR: GazeMoE是一种基于Mixture-of-Experts的新型端到端框架，通过MoE模块从冻结的视觉基础模型中自适应提取目光目标线索，并结合类别平衡损失和数据增强策略，在目光估计任务中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 从可见图像中估计人类目光目标是机器人理解人类注意力的关键任务，但开发可泛化的神经架构和训练范式仍具挑战性。预训练视觉基础模型的发展为定位目光目标提供了新途径，但整合多模态线索（眼睛、头部姿态、手势、上下文特征）需要自适应和高效的解码机制。

Method: 提出GazeMoE框架，使用Mixture-of-Experts模块从冻结的视觉基础模型中选择性提取与 gaze target 相关的线索，并引入类别平衡辅助损失解决类别不平衡问题，采用区域裁剪和光度变换等数据增强策略提升鲁棒性。

Result: 在基准数据集上的广泛实验表明，GazeMoE优于现有方法，在具有挑战性的目光估计任务上实现了最先进的性能。

Conclusion: GazeMoE通过MoE模块自适应地利用冻结基础模型中的目光目标相关线索，结合类别平衡损失和数据增强策略，在基准数据集上实现了最先进的性能，代码和预训练模型已开源。
...
TL;DR: 本文提出一种前馈三角形原语生成框架，可从多视角图像直接预测连续三角形表面，在单次前向传播中生成仿真就绪模型，无需逐场景优化或后处理。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3D Gaussian Splatting方法虽然渲染质量出色，但依赖耗时的逐场景优化，限制了实时部署。现有前馈高斯溅射方法缺乏显式流形几何结构，无法直接用于模拟。

Method: 提出了一种用于三角形原语生成的前馈框架，直接从校准的多视角图像预测连续三角形表面。引入像素对齐的三角形生成模块，并结合相对3D点云监督以增强几何学习稳定性和一致性。

Result: 实验表明，该方法在保持与标准图形和机器人模拟器无缝兼容性的同时实现了高效重建。

Conclusion: 该方法在单次前向传播中实现高效重建，同时保持与标准图形和机器人模拟器的无缝兼容性。
...
TL;DR: OD-RASE是一个增强自动驾驶系统安全性的框架，通过本体论和大语言模型检测导致交通事故的道路结构，生成基础设施改进提案并创建标注数据集，最终实现对事故道路结构的准确预测和改善后道路环境的图像生成。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统虽感知性能高，但在处理罕见情况或复杂道路结构时仍存在局限。这些专为人类驾驶员设计的道路基础设施通常只在事故发生后才进行安全改进，这种被动方式无法满足自动驾驶系统所需的主动风险缓解要求。

Method: 1) 基于道路交通系统领域知识构建本体论；2) 使用大规模视觉语言模型(LVLM)生成基础设施改进提案；3) 通过本体论驱动的数据过滤提高提案可靠性；4) 在事故前道路图像上自动标注改进提案，构建新数据集；5) 结合LVLM和扩散模型生成改进提案及改善后的道路环境图像。

Result: 实验表明，本体论驱动的数据过滤能够实现对事故道路结构及其相应改进方案的高精度预测，成功构建了包含改进标注的新数据集，并生成了改善后的道路环境图像。

Conclusion: 该研究提出了OD-RASE框架，通过本体论驱动的数据过滤实现对事故道路结构的准确预测和改进方案的生成，为自动驾驶系统的安全性和道路基础设施的改善提供了重要贡献，是推动自动驾驶系统广泛采用的重要一步。
...
### [55] [DEX-AR: A Dynamic Explainability Method for Autoregressive Vision-Language Models](https://arxiv.org/abs/2603.06302)
*Walid Bousselham,Angie Boggust,Hendrik Strobelt,Hilde Kuehne*

Main category: cs.CV

TL;DR: DEX-AR是一种针对自回归视觉语言模型的新型可解释性方法，通过计算token生成过程中的层-wise梯度，生成per-token和sequence-level的2D热图，突出显示对模型文本响应至关重要的图像区域。


<details>
  <summary>Details</summary>
Motivation: 传统的可解释性方法专为分类任务设计，难以应对现代自回归VLMs的复杂token-by-token生成过程以及视觉与文本模态之间的复杂交互作用。

Method: DEX-AR通过在token-by-token生成过程中计算关于注意力图的层-wise梯度，生成per-token和sequence-level的2D热图。提出两个关键创新：1)动态头过滤机制，识别专注于视觉信息的注意力头；2)序列级过滤方法，在聚合per-token解释的同时区分视觉接地token和纯语言token。

Result: 在ImageNet、VQAv2和PascalVOC数据集上的评估表明，基于扰动的指标（使用新型归一化困惑度度量）和基于分割的指标均获得一致性提升。

Conclusion: DEX-AR成功为自回归VLMs提供了解释能力，通过区分不同类型的token并识别关键注意力头，有效提升了模型决策的透明度。
...
### [56] [K-MaT: Knowledge-Anchored Manifold Transport for Cross-Modal Prompt Learning in Medical Imaging](https://arxiv.org/abs/2603.06340)
*Jiajun Zeng,Shadi Albarqouni*

Main category: cs.CV

TL;DR: K-MaT（知识锚定流形传输）是一个提示学习框架，通过融合Gromov-Wasserstein最优传输将高端成像（如CT）的决策结构迁移到低端模态（如X光片），无需低端训练图像。在四个跨模态基准测试中达到最先进水平，准确率调和均值为44.1%，宏F1为36.2%。


<details>
  <summary>Details</summary>
Motivation: 大型生物医学视觉-语言模型（VLM）在高端成像（如CT）上训练后，往往无法迁移到前端低端模态（如X光片），会退化为模态特定的捷径。需要在不需要低端训练图像的情况下将决策结构迁移到低端模态。

Method: K-MaT将提示进行分解，将提示锚定到临床文本描述，并使用融合Gromov-Wasserstein最优传输将低端提示流形与视觉 grounded 的高端空间对齐。

Result: 在四个跨模态基准测试中取得最先进结果，平均准确率调和均值从BiomedCoOp的42.0%提升至44.1%，宏观F1提升至36.2%。在具有挑战性的乳腺成像任务中，缓解了标准方法（如CoOp准确率降至27.0%）的灾难性遗忘问题，在不同模态间保持了稳健的性能。

Conclusion: 对齐提示流形通过最优传输为零样本跨模态医学VLM部署提供了一条非常有效的路径。K-MaT能够缓解标准方法（如CoOp）的灾难性遗忘问题，在不同模态间保持稳健的性能。
...
### [57] [Dynamic Chunking Diffusion Transformer](https://arxiv.org/abs/2603.06351)
*Akash Haridas,Utkarsh Saxena,Parsa Ashrafi Fashi,Mehdi Rezagholizadeh,Vikram Appia,Emad Barsoum*

Main category: cs.CV

TL;DR: DC-DiT通过学习的编码器-路由器-解码器框架，自适应地将图像压缩为可变长度的token序列，在细节丰富区域分配更多token，在背景区域分配更少token，并根据去噪阶段动态调整压缩程度，从而在ImageNet上提升生成质量且降低计算成本。


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers将图像处理为固定长度的token序列，采用统一的计算方式处理低信息和高信息区域，忽略了图像包含不同细节程度的区域，且去噪过程从早期时间步的粗略结构到晚期时间步的精细细节不断发展。

Method: 引入动态分块扩散Transformer（DC-DiT），在DiT主干上添加学习的编码器-路由器-解码器框架，通过端到端学习的分块机制将2D输入自适应压缩为更短的token序列。该机制学习将均匀背景区域压缩为更少token，将细节丰富区域压缩为更多token，且无需监督即可出现有意义的视觉分割。此外，还学习跨扩散时间步调整压缩程度，在噪声阶段使用更少token，在细节出现时使用更多token。

Result: 在class-conditional ImageNet 256×256上，DC-DiT在4×和16×压缩率下相对于参数匹配和FLOP匹配的DiT基线 consistently improves FID和Inception Score。

Conclusion: DC-DiT是一种有前景的技术，可应用于像素空间、视频和3D生成。它具有实用性：可以从预训练的DiT检查点升级，只需很少的后训练计算（最多减少8倍训练步骤），并可与其他动态计算方法结合，进一步降低生成FLOPs。
...
TL;DR: 本文提出了一种基于2D离散傅里叶幅度谱的自适应径向投影(ARP)方法来估计文档图像的倾斜角度，并构建了高质量的DISE-2021数据集。实验表明该方法鲁棒可靠，性能优于所有对比方法。


<details>
  <summary>Details</summary>
Motivation: 倾斜估算是文档处理系统中的关键任务，直接影响后续处理步骤的性能。随着数字化时代的到来，对扫描文档图像的倾斜估算研究日益重要。

Method: 提出了一种新颖的倾斜估算方法，通过在2D离散傅里叶幅度谱上应用自适应径向投影来提取文档图像的主倾斜角度。同时构建了DISE-2021数据集用于评估不同估算器的性能，并对傅里叶方法进行了多方面的改进分析。

Result: 所提出的方法在实验中表现出鲁棒性和可靠性，性能优于所有对比方法。源代码已开源发布。

Conclusion: 基于自适应径向投影的傅里叶方法是文档图像倾斜估算的有效方案，所构建的DISE-2021数据集为该领域的研究提供了重要基准。
...
### [59] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
*Yonghuang Wu,Zhenyang Liang,Wenwen Zeng,Xuan Xie,Jinhua Yu*

Main category: cs.CV

TL;DR: 针对文本引导医学图像分割中提示词敏感性问题，提出将提示词敏感性问题重新表述为组内一致性问题的解决方案，通过质量引导的组正则化和对数级别一致性约束提升 nuclei 分割的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 基础模型（如SAM3）的预测对提示词 formulation 高度敏感，即使语义等价的描述也会产生不一致的掩码，限制了其在临床和病理工作流中的可靠性。

Method: 提出了一种提示组感知训练框架，包含两部分：(i) 质量引导的组正则化，利用分割损失作为隐式排序信号；(ii) 对数级别的一致性约束，采用停止梯度策略对齐同一提示组内的预测。

Result: 在多数据集 nuclei 基准上的一致性增益显著，性能方差大幅降低；在6个零样本跨数据集任务中，Dice 分数平均提升2.16分。

Conclusion: 该方法无需架构修改，推断过程保持不变，显著提升了视觉语言分割在计算病理学中的鲁棒性和泛化能力，为临床和病理工作流提供了更可靠的文本引导分割工具。
...
TL;DR: LucidNFT是一个用于流匹配真实世界图像超分辨率的多奖励强化学习框架，通过引入退化鲁棒的语义评估器LucidConsistency、解耦优势归一化策略和大规模真实退化图像数据集LucidLR，解决现有方法中的语义幻觉、优势崩溃和退化覆盖不足问题。


<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率(Real-ISR)虽然可以合成逼真的细节，但随机采样导致关键失败模式：输出看起来清晰但与低分辨率输入不一致（语义和结构幻觉），而这种基于LR的保真度在没有HR真值的情况下难以评估。基于偏好的强化学习是自然选择，但Real-ISR的有效对齐面临三大障碍：(1)缺乏退化鲁棒的LR参考保真度信号；(2) rollout组优化瓶颈，简单多奖励标量化后归一化会压缩目标间对比，导致优势崩溃；(3)真实退化覆盖不足限制rollout多样性和偏好信号质量。

Method: 提出LucidNFT，一个用于流匹配Real-ISR的多奖励RL框架。包括：(1) LucidConsistency - 退化鲁棒的语义评估器，使基于LR的保真度可测量和可优化；(2) 解耦优势归一化策略，在融合前保留每个LR条件rollout组内的目标间对比，防止优势崩溃；(3) LucidLR - 大规模真实退化图像集合，支持鲁棒RL微调。

Result: 实验表明LucidNFT持续提升强大的基于流的Real-ISR基线，在多样化真实场景中实现更好的感知-保真度权衡和稳定的优化动态。

Conclusion: LucidNFT通过创新的多奖励RL框架有效解决了Real-ISR中的保真度-感知权衡问题，为流匹配模型在真实世界图像超分辨率任务中的应用提供了新的训练范式。
...
### [61] [Physical Simulator In-the-Loop Video Generation](https://arxiv.org/abs/2603.06408)
*Lin Geng Foo,Mark He Huang,Alexandros Lattas,Stylianos Moschoglou,Thabo Beeler,Christian Theobalt*

Main category: cs.CV

TL;DR: PSIVG是一个将物理模拟器集成到视频扩散过程中的新框架，通过重建4D场景和物体网格，在物理模拟器中生成一致轨迹来引导视频生成，并采用TTCO技术保持纹理一致性，从而生成符合物理规律的高质量视频。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成虽然实现了显著的视觉真实感，但仍难以遵循基本物理定律（如重力、惯性、碰撞），生成的物体在帧间运动不一致，表现出不合理的动力学行为或违反物理约束，限制了AI生成视频的真实感和可靠性。

Method: PSIVG框架：首先由预训练扩散模型生成模板视频，然后重建4D场景和前景物体网格，在物理模拟器中初始化并生成物理一致的轨迹，最后利用这些轨迹引导视频生成器产生时空中物理一致的运动。TTCO技术：根据模拟器的像素对应关系调整文本和特征嵌入，以提升纹理一致性。

Result: 实验表明，PSIVG能在保持视觉质量和多样性的同时，生成更符合真实世界物理规律的视频。

Conclusion: PSIVG通过将物理模拟器集成到视频扩散流程中，有效解决了AI生成视频中缺乏物理一致性的问题。TTCO技术进一步提升了物体移动过程中的纹理一致性。实验表明，该方法能在保持视觉质量和多样性的同时，生成更符合真实世界物理规律的视频。
...
### [62] [CLoPA: Continual Low Parameter Adaptation of Interactive Segmentation for Medical Image Annotation](https://arxiv.org/abs/2603.06426)
*Parhom Esmaeili,Chayanin Tangwiriyasakul,Eli Gibson,Sebastien Ourselin,M. Jorge Cardoso*

Main category: cs.CV

TL;DR: CLoPA 是一种持续适应策略，通过调优 nnInteractive 少量参数实现医学影像分割的在线学习，在8个不同任务中将性能快速提升至专家水平，无需新增参数或改变推理流程。


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型（如 nnInteractive）在多样化医学影像任务中无法始终达到专家级性能，而标注活动产生大量任务特定标注数据，因此模型的在线适应成为自然补充。

Method: 提出CLoPA（持续适应策略），通过轻量级 episode 调度触发，对 nnInteractive 模型的一小部分参数进行调优。方法完全在现有标注工作流程内运行，无需改变推理流程或增加新参数。

Result: 在8个医学分割十项全能（Medical Segmentation Decathlon）任务中，CLoPA 快速将性能提升至专家水平，即使在 nnInteractive 之前失败的任务中也取得成功。多数收益在单次训练 episode 后实现。不同参数组的调优效果取决于任务特征和数据量。对于复杂几何目标（如肝血管），实例归一化和低级特征调优效果达到饱和。

Conclusion: 对于复杂几何目标（如肝血管），实例归一化和低级特征调优达到饱和，表明在最具挑战性的场景中需要更深入的表示对齐。CLoPA能够在无需新增参数的情况下快速提升模型性能至专家水平，且大多数收益在单次训练 episode 后即可实现。
...
TL;DR: 本文提出VINE框架，通过spatial-view graph建模结构一致性，从support-query差异学习判别prior重新加权特征，并利用masked cross-attention整合多源特征生成class-consistent prototypes用于SAM decoder，在few-shot segmentation任务中特别是viewpoint变化和复杂结构场景下取得优异性能。


<details>
  <summary>Details</summary>
Motivation: Few-shot segmentation(FSS)能够通过少量监督泛化到新类别，但在large appearance或viewpoint variations下面临结构错位和跨视图不一致的挑战。

Method: 提出VINE (View-Informed NEtwork)统一框架：1) 在backbone features上构建spatial-view graph，其中spatial graph捕捉局部几何拓扑，view graph连接不同视角特征传播视图不变的结构语义；2) 从support-query特征差异导出判别prior，通过强调显著区域重新加权SAM特征，重新校准backbone activations以改善结构聚焦；3) 通过masked cross-attention逐步整合foreground-enhanced SAM features和structurally enriched ResNet features，生成class-consistent prototypes作为SAM decoder的自适应提示。

Result: 在多个FSS基准数据集上验证了VINE的有效性和鲁棒性，特别是在viewpoint shifts和复杂结构等挑战性场景下表现优异。

Conclusion: VINE通过联合建模结构一致性和前景判别来细化class-specific prototypes，有效解决了FSS中结构错位和跨视图不一致的问题。
...
TL;DR: OVGGT是一种无需训练的框架，通过自选择缓存和动态锚点保护技术，使几何基础模型能够在固定VRAM预算下处理任意长度的视频流，实现SOTA的3D几何精度。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型虽然重建质量高，但全注意力机制带来二次方计算成本，仅限于短序列；现有的因果注意力变体如StreamVGGT虽支持单次推理，但KV缓存持续增长，在数百帧内耗尽GPU内存，无法实现长时域部署。

Method: 提出OVGGT框架，结合两种技术：1）自选择缓存(Self-Selective Caching)：利用FFN残差幅度压缩KV缓存，与FlashAttention完全兼容；2）动态锚点保护(Dynamic Anchor Protection)：保护坐标关键token不被驱逐，抑制长轨迹几何漂移。

Result: 在室内、室外和超长序列基准测试中，OVGGT能在固定VRAM包络内处理任意长度视频，同时达到SOTA的3D几何精度。

Conclusion: OVGGT通过训练-free的方式实现了内存和计算量的固定预算限制，为长时域流式3D几何重建提供了可行方案。
...
### [65] [Artificial Intelligence for Detecting Fetal Orofacial Clefts and Advancing Medical Education](https://arxiv.org/abs/2603.06522)
...
TL;DR: 本研究开发了一个基于45,139张超声图像训练的人工智能系统，可诊断胎儿口面裂，灵敏度超93%、特异度超95%，性能匹配高级放射科医生，并能将初级放射科医生灵敏度提升6%以上，同时有助于培训放射科医生在罕见病方面的专业知识。


<details>
  <summary>Details</summary>
Motivation: 口面裂是最常见的先天性颅面异常之一，但由于经验丰富的专业人员稀缺且该病相对罕见，产前准确检测仍然具有挑战性。早期可靠诊断对于及时临床干预和降低相关发病率至关重要。

Method: 使用来自22家医院、9,215名胎儿的45,139张超声图像训练人工智能系统，并将其性能与高级和初级放射科医生进行比较，同时进行了一项涉及24名放射科医生和实习生的试点研究，评估该模型在专业知识发展中的作用。

Result: 该AI系统诊断胎儿口面裂的灵敏度和特异度分别超过93%和95%，达到高级放射科医生的水平，显著优于初级放射科医生。作为医疗助手，该系统将初级放射科医生的灵敏度提高了6%以上。试点研究表明该模型还能改善罕见病的专业知识发展。

Conclusion: 该AI系统为缺乏经验丰富的放射科医生的地区提供了可扩展的解决方案，既能提高诊断准确性，又能加速罕见病临床专业知识的发展，具有双重用途的应用前景。
...
### [66] [SUREON: A Benchmark and Vision-Language-Model for Surgical Reasoning](https://arxiv.org/abs/2603.06570)
*Alejandra Perez,Anita Rau,Lee White,Busisiwe Mlambo,Chinedu Nwoye,Muhammad Abdullah Jamal,Omid Mohareri*

Main category: cs.CV

TL;DR: SUREON是一个大规模手术视频问答数据集，通过多智能体管道从手术学术视频中提取监督信号，包含134.7K个片段、170种手术类型和206.8K个问答对，并提出两个视觉语言模型SureonVLM和SureonR1来评估手术推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前手术AI无法回答关于手术场景的复杂问题（如意图理解、风险评估、结果预测），主要原因是难以大规模标注包含手术推理的训练数据。然而手术视频讲座中已包含专家的意图解释和预期说明，这些叙述可以编码手术推理知识。

Method: 1) 定义12个问题类别覆盖安全评估、决策理由和预测；2) 使用多智能体管道从手术学术视频中提取和结构化监督信号；3) 开发SureonVLM（监督微调）和SureonR1（基于GRPO的推理模型）两个模型进行评估。

Result: 在206.8K个QA对和354个专家验证基准示例上，SureonVLM-R1达到84%以上的准确率，显著优于更大的通用领域模型，并在标准手术感知任务上表现更优。定性分析显示SureonR1表现出显式推理行为，如从视觉上下文推断手术意图。

Conclusion: 通过从手术视频讲座中 harvesting 监督信号可以有效构建手术推理能力，为手术AI从感知向理解的发展提供了新范式。
...
TL;DR: BEVLM通过结合BEV表示与LLM，解决了自动驾驶中多视角视觉处理的冗余和空间一致性问题，显著提升了推理准确率和驾驶安全性


<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理多视角多帧图像的token，导致计算冗余和空间一致性有限，阻碍了准确的3D空间推理和跨视角几何一致性的维持

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLM连接，利用BEV特征作为统一输入

Result: 在跨视角驾驶场景推理中准确率提升46%，在安全关键场景中闭环端到端驾驶性能提升29%

Conclusion: BEVLM通过将空间一致且语义提炼的BEV表示与LLM连接，有效提升了跨视角驾驶场景的推理能力，并在安全关键场景中显著改善了闭环端到端驾驶性能
...
TL;DR: 本文提出CR-QAT（课程关系量化感知训练），用于解决开放词汇目标检测模型在极端低位量化下的性能下降问题。该方法结合分阶段优化和关系知识蒸馏，在4-bit量化下仍能保持良好的视觉-语言对齐和区域关系结构。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测（OVOD）通过视觉-语言对齐实现新类别检测，但大规模模型阻碍了在资源受限设备上的部署。虽然量化提供了实用的压缩方案，但研究发现 naive 极端低位（如4-bit）量化会严重损害细粒度视觉-语言对齐，并扭曲区域间关系结构。

Method: 提出课程关系量化感知训练（CR-QAT）框架，包含两个核心组件：1）课程QAT（CQAT）通过分区分阶段进行渐进式量化，缓解错误积累，实现稳定优化；2）文本中心关系知识蒸馏（TRKD）构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试上的实验表明，CR-QAT在极端低位量化设置下始终优于现有QAT基线，分别实现了高达38.9%和40.9%的相对AP提升。

Conclusion: CR-QAT有效解决了极端低位量化带来的性能退化问题，通过课程量化训练和关系知识蒸馏的结合，在保持模型压缩优势的同时，显著保留了开放词汇目标检测的视觉-语言对齐能力和区域关系结构。
...
### [69] [Breaking Smooth-Motion Assumptions: A UAV Benchmark for Multi-Object Tracking in Complex and Adverse Conditions](https://arxiv.org/abs/2603.05970)
*Jingtao Ye,Kexin Zhang,Xunchi Ma,Yuehan Li,Guangming Zhu,Peiyi Shen,Linhua Jiang,Xiangdong Zhang,Liang Zhang*

Main category: cs.CV

TL;DR: DynUAV是一个新的动态无人机视角多目标跟踪基准，旨在解决现有基准缺乏复杂相机动态和真实无人机机动场景的问题。该基准包含42个视频序列、170万+边界框注释，涵盖车辆、行人及工业机械类别，并引入由自我运动引起的剧烈尺度变化、视角变化和运动模糊等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角的MOT基准缺乏真实无人机快速移动和敏捷机动带来的复杂观测挑战，多为可预测的相机动态和线性运动模式，无法有效测试跟踪器在真实动态场景下的性能。

Method: 构建了DynUAV基准数据集，包含42个视频序列、超过170万标注框，涵盖车辆、行人及挖掘机、推土机、起重机等工业类别。对多个SOTA跟踪器进行 comprehensive 评估。

Result: 现有SOTA跟踪器在DynUAV上表现不佳，特别是在处理检测与关联交织的动态条件挑战时暴露明显局限性，证明了该基准的难度和实用性。

Conclusion: DynUAV将成为推动真实无人机视角MOT研究的重要测试平台，有助于促进该领域在复杂动态场景下的算法进步。
...
TL;DR: 本文针对草图着色任务中的分布偏移问题，提出了双分支框架结合Gram正则化损失、动漫Tagger Network和SDXL调制的方法，实现了高质量、高分辨率、可控制的着色效果，达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有研究将分布偏移视为主要困难，但仅关注缓解由此产生的伪影，未能从根本上解决问题。草图着色在动画和数字插画创作中具有关键作用，需要更高质量的着色效果。

Method: 提出双分支框架显式建模训练和推理过程的数据分布：语义对齐分支处理训练数据，语义错位分支处理测试数据；采用Gram正则化损失强制跨域分布一致性和稳定性；引入动漫专用Tagger Network提取参考图像的细粒度属性并调制SDXL条件编码器；添加纹理迁移插件模块增强质感。

Result: 实验表明，该方法有效克服了分布偏移挑战，在质量和可控性指标上建立了最优性能。消融实验验证了各组件的贡献。

Conclusion: 本文提出的双分支框架通过Gram正则化损失有效解决了草图着色中的分布偏移问题，结合Tagger Network和SDXL调制实现了高质量、高分辨率和可控制的着色效果，在质量和可控性指标上均达到最优性能。
...
TL;DR: RePer-360是一种用于单目全景深度估计的失真感知自调制框架，通过几何对齐引导模块和自调节AdaLN-Zero机制将预训练视角先验迁移到全景域，以1%训练数据实现超越标准微调方法的性能，RMSE提升约20%。


<details>
  <summary>Details</summary>
Motivation: 当前基于透视图像的深度基础模型在泛化到360°图像时表现不佳，因为透视与全景域之间存在显著几何差异，且完全微调这些模型通常需要大量全景数据。

Method: 提出RePer-360框架，包含三个核心组件：1)几何对齐引导模块从ERP和CP两个互补投影中提取调制信号；2)自调节AdaLN-Zero机制生成像素级缩放因子以减少特征分布差距；3)立方体映射域一致性损失提升训练稳定性和跨投影对齐。

Result: 在仅使用1%训练数据的条件下，RePer-360超越了标准微调方法；在相同域内训练设置下，RMSE提升约20%。

Conclusion: RePer-360通过轻量级几何对齐引导模块和自调节AdaLN-Zero机制，成功将预训练视角先验迁移到360°全景深度估计任务，在仅使用1%训练数据的情况下超越了标准微调方法，RMSE提升约20%。
...
### [72] [EffectMaker: Unifying Reasoning and Generation for Customized Visual Effect Creation](https://arxiv.org/abs/2603.06014)
*Shiyuan Yang,Ruihuang Li,Jiale Tao,Shuai Shao,Qinglin Lu,Jing Liao*

Main category: cs.CV

TL;DR: EffectMaker是一个统一的推理-生成框架，通过多模态大语言模型解释高级效果语义并推理其如何适应目标主体，同时利用扩散变换器从参考视频中捕获细粒度视觉线索，实现无需针对每个效果进行微调的精确、可控和效果一致的合成。


<details>
  <summary>Details</summary>
Motivation: 视觉效果(VFX)对提升视频内容的表达力和创造力至关重要，但制作高质量效果通常需要专业知识和昂贵的制作流程。现有的AIGC系统在VFX生成方面面临重大挑战，主要原因是效果特定数据稀缺以及模拟超自然或风格化效果的固有难度。此外，这些方法通常需要针对每个效果进行微调，严重限制了其对新颖VFX的可扩展性和泛化能力。

Method: EffectMaker采用语义-视觉双路径引导机制：1)多模态大语言模型解释高级效果语义并推理其如何适应目标主体；2)扩散变换器利用上下文学习从参考视频中捕获细粒度视觉 cues。这两个组件形成语义-视觉双路径引导机制，实现准确、可控和效果一致的综合，且无需针对每个效果进行微调。

Result: 在EffectData数据集（包含3k VFX类别的130k视频，是最大的高质量合成数据集）上进行实验表明，EffectMaker在视觉质量和效果一致性方面优于最先进的基线方法，为定制VFX生成提供了一种可扩展且灵活的范式。

Conclusion: EffectMaker是一个可扩展且灵活的定制VFX生成范式，无需针对每个效果进行微调即可实现准确的、可控的且效果一致的视频合成。
...
TL;DR: MOSIV是一个新的多对象系统识别框架，通过可微分模拟器直接从视频中优化连续逐对象材质参数，并引入几何目标作为监督信号，在新提出的接触丰富多对象交互基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单对象场景或固定材质原型集的离散材质分类，无法处理多对象系统的材质识别问题。因此需要提出新的方法来处理视频中多个对象的连续材质参数估计。

Method: MOSIV框架通过可微分模拟器直接优化连续逐对象材质参数，并结合从视频中提取的几何目标进行指导。该方法利用几何约束作为监督信号，实现材质参数的精确估计。

Result: 在新建的多对象交互基准测试上，MOSIV显著提升了接地精度和长时域模拟保真度，性能优于-adapted基线方法。分析表明对象级细粒度监督和几何对齐目标对复杂多对象场景的稳定优化至关重要。

Conclusion: MOSIV作为该新任务的强基线方法，证明了基于可微分模拟器直接优化连续材质参数的有效性。对象级细粒度监督和几何对齐目标对于复杂多对象场景中的稳定优化至关重要。未来工作可扩展到真实场景和更复杂的物理交互。
...
TL;DR: StruVis是一种新的推理框架，通过使用基于文本的结构化视觉表示作为中间推理状态，让MLLM在纯文本推理过程中感知视觉结构，从而增强推理-based文本到图像生成性能，且可与多种T2I生成器无缝集成。


<details>
  <summary>Details</summary>
Motivation: 现有的推理框架存在局限性：(1)纯文本推理虽然计算高效，但缺乏视觉上下文，经常遗漏关键的空间和视觉元素；(2)文本-图像交错推理虽然通过T2I生成器提供视觉参考增强了视觉 grounding，但计算成本高昂，且MLLM的推理能力受限于生成器的表示能力。

Method: 提出StruVis框架，采用"结构化视觉思考"方式。不依赖中间图像生成，而是使用基于文本的结构化视觉表示作为中间推理状态，使MLLM能够在纯文本推理过程中有效"感知"视觉结构。通过结构化视觉引导的推理来释放MLLM在T2I生成方面的推理潜力。

Result: 在推理-based T2I基准测试上取得显著性能提升：在T2I-ReasonBench上提升4.61%，在WISE上提升4%。

Conclusion: StruVis是一个生成器无关的推理框架，可以无缝集成到各种T2I生成器中，在推理-based T2I生成任务中提升性能。在T2I-ReasonBench上提升4.61%，在WISE上提升4%。
...
TL;DR: 提出了一种名为Occlusion-Aware SORT (OA-SORT)的无训练、即插即用的多目标跟踪框架，用于解决2D MOT中因部分遮挡导致的定位成本混淆问题。该框架包含遮挡感知模块(OAM)、遮挡感知偏移(OAO)和偏置感知动量(BAM)三个核心组件。


<details>
  <summary>Details</summary>
Motivation: 2D多目标跟踪(MOT)面临由部分遮挡引起的定位成本混淆挑战，现有方法在遮挡情况下跟踪性能下降明显。

Method: 1) OAM分析物体遮挡状态，引入高斯图(GM)降低背景干扰；2) OAO和BAM利用OAM描述的遮挡状态来缓解成本混淆并抑制估计不稳定性；3) 在DanceTrack、SportsMOT和MOT17数据集上进行了全面评估。

Result: 在DanceTrack测试集上达到63.1%的HOTA和64.2%的IDF1；将遮挡感知框架集成到其他四个跟踪器后，平均提升HOTA 2.08%和IDF1 3.05%。

Conclusion: OA-SORT框架有效解决了遮挡情况下的跟踪问题，具有良好的通用性和可复用性，可作为即插即用模块提升现有跟踪器的性能。
...
TL;DR: 针对高维超列计算复杂度高的问题，提出分层子采样方法降低计算成本，并研究集成学习在稀疏超列上的表现。实验表明该方法在低样本场景（N≤20）下优于多尺度UNet基线，Dice分数提升24.53%。


<details>
  <summary>Details</summary>
Motivation: 高维超列受生物视觉启发，可用于像素级图像分割，但实际应用很少，主要原因是处理concatenated dense hypercolumns的计算复杂度随训练集大小N线性增长。

Method: 应用分层子采样（stratified subsampling）到VGG16-based超列以降低计算复杂度；研究稀疏超列上的集成学习性能（堆叠和投票集成）；比较逻辑回归、集成方法与多尺度UNet基线的效果。

Result: 在脑肿瘤数据集上：1）N≤20时简单逻辑回归最有效；2）堆叠和投票集成在较大数据集上竞争力强；3）10%子采样率下N=20时最佳Dice=0.66；4）比标准多尺度UNet基线提升24.53%（p=3.07e-11）。

Conclusion: 通过分层子采样和集成学习策略，有效降低了超列的计算复杂度。在极端低样本情况下（N≤20），简单逻辑回归分类器表现最佳；对于较大数据集，堆叠和投票集成方法具有竞争力。10%分层子采样率下N=20时达到最佳平均Dice分数0.66，比多尺度UNet基线提高24.53%，差异具有统计学意义（p=3.07e-11）。
...
### [77] [FontUse: A Data-Centric Approach to Style- and Use-Case-Conditioned In-Image Typography](https://arxiv.org/abs/2603.06038)
*Xia Xin,Yuki Endo,Yoshihiro Kanamori*

Main category: cs.CV

TL;DR: 提出数据驱动方法解决文本到图像模型排版控制难题，通过结构化注释管道创建70K规模的FontUse数据集，使模型能一致解释字体风格和使用场景条件，无需架构修改即可提升排版渲染效果


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型虽然能生成高质量图像，但控制排版仍具挑战性：请求的字体外观常被忽略或仅弱遵循

Method: 构建包含约70K图像的大规模排版数据集FontUse，使用分割模型和多模态大语言模型自动生成注释；注释包含用户友好提示、文本区域位置和OCR识别字符串；基于Long-CLIP设计评估指标衡量生成排版与请求属性的对齐程度

Result: 在多样化提示和布局的实验中，经过微调的模型生成的文本渲染与提示的一致性优于竞争基线

Conclusion: 本文提出的FontUse数据集和注释管道能有效提升文本到图像模型对排版样式的控制能力，通过数据驱动方法无需修改模型架构即可实现更好的字体风格和使用场景条件遵循。
...
TL;DR: 针对统一多模态模型(UMMs)视觉理解能力强但生成能力弱的问题，提出GvU token级内在对齐奖励机制和自监督强化学习框架，利用模型自身理解能力指导生成，无需外部监督即可提升生成质量并缩小能力差距。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型(UMMs)在视觉理解和生成方面取得进展，但存在能力差距：理解能力较强而生成能力较弱，这种差异源于理解和生成过程的内在解耦，UMM虽能准确解读细粒度视觉细节，却难以从复杂文本提示生成语义连贯的图像。

Method: 提出token级内在文本-图像对齐奖励机制GvU，使UMM同时充当教师和学生角色，利用理解分支评估自身输出并据此指导生成；基于此设计自监督强化学习框架，通过基于理解的内在奖励信号迭代提升生成质量。

Result: 显著提升了UMM的生成质量，进而增强了细粒度视觉理解，有效缩小了UMM视觉理解与生成之间的能力差距。

Conclusion: 提出了一种基于自我监督强化学习框架的解决方案GvU，通过利用UMMs自身的理解能力作为内在奖励信号来指导生成过程，无需外部监督即可迭代提升生成质量，有效缩小了UMMs在视觉理解与生成之间的能力差距。
...
### [79] [GenHOI: Towards Object-Consistent Hand-Object Interaction with Temporally Balanced and Spatially Selective Object Injection](https://arxiv.org/abs/2603.06048)
...
TL;DR: GenHOI是一个针对手-物体交互视频合成的轻量级增强模块，通过Head-Sliding RoPE解决时间衰减问题，通过双层空间注意力门实现空间选择性注意力，在真实场景中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有HOI重演方法在领域内训练和评估，但在复杂、真实的场景中泛化失败；而全视频编辑模型虽具有更广泛的鲁棒性，但仍难以处理HOI特定问题（如物体外观不一致）。需要一个轻量级增强方案来解决物体身份跨帧一致性和交互真实感的挑战。

Method: 提出GenHOI方法，包含两个核心组件：(1) Head-Sliding RoPE：为参考token分配头部特定的时间偏移量，使它们的影响在帧间均匀分布，缓解3D RoPE的时间衰减问题；(2) 双层空间注意力门：在HOI区域集中物体条件注意力并自适应调整强度，在保持背景真实感的同时增强交互保真度。

Result: 在未见过的真实场景上进行了大量定性和定量评估，GenHOI显著优于最先进的HOI重演和全视频编辑方法。

Conclusion: GenHOI作为一个轻量级增强模块，在保持预训练模型泛化能力的同时，有效解决了HOI视频中的物体身份保真度和交互真实感问题。通过Head-Sliding RoPE和双层空间注意力门两大创新设计，在未见过的真实场景中显著优于现有的HOI重演和全视频编辑方法。
...
### [80] [Devil is in Narrow Policy: Unleashing Exploration in Driving VLA Models](https://arxiv.org/abs/2603.06049)
...
TL;DR: 本文提出Curious-VLA框架解决VLA模型在自动驾驶中的探索-利用困境，通过IL阶段的可行轨迹扩展（FTE）和逐步归一化轨迹表示，以及RL阶段的自适应多样性感知采样（ADAS）和跨域驾驶奖励（SDR），在Navsim基准上取得最先进成果。
...
<details>
  <summary>Details</summary>
Motivation: 识别出自动驾驶VLA模型中一个基本的"窄策略"限制：驾驶模仿学习（IL）倾向于崩溃探索行为，并限制后续强化学习（RL）阶段的潜力，后者往往由于反馈多样性不足而过早饱和。



Method: 提出两阶段框架：IL阶段采用可行轨迹扩展（FTE）策略生成多条物理有效轨迹，并使用逐步归一化轨迹表示来处理多样化数据；RL阶段提出自适应多样性感知采样（ADAS）来优先处理高多样性样本，并引入跨域驾驶奖励（SDR）配合焦距式加权来放大奖励的价值跨度。



Result: 在Navsim基准上实现了最佳性能：PDMS 90.3，EPDMS 85.4，Best-of-N PDMS达94.8。



Conclusion: Curious-VLA在Navsim基准上取得了最先进的性能（PDMS 90.3，EPDMS 85.4），并且Best-of-N PDMS达到94.8，证明了其解锁VLA模型探索潜力的有效性。
...
TL;DR: 本文提出DeepSight，首个专用深度多模态大型语言模型，用于增强三维场景理解。通过利用深度图像的单通道特性、构建深度数据集、改进ViT编码器并开发深度问答基准，DeepSight在深度感知方面取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 多模态大型语言模型虽在图像描述和视觉问答等任务表现优异，但往往难以准确解读视觉数据中固有的深度信息。

Method: 1) 利用深度图像的单通道特性（灰度值直接反映深度线索）来改进空间推理；2) 构建深度图像-文本对数据集和深度指令数据集，使用GLPN模型从视觉图像生成深度图，GPT-4生成深度指令（并通过LLaVA验证）；3) 修改CLIP中的ViT编码器以整合局部对象信息，更好地捕捉深度连续变化；4) 开发基于现有深度图像数据集的深度问答基准测试。

Result: DeepSight显著增强了深度感知能力，提升了下游任务性能。

Conclusion: 这是多模态三维理解领域的重大进展，为深度感知和下游任务性能带来了显著提升。
...
TL;DR: 本文提出FedARKS框架，通过RK和KS机制解决现有联邦学习在行人重识别中全局特征提取和均匀聚合的局限性，提升模型在未见域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有FedDG-ReID方法存在两个主要问题：(1) 仅使用全局特征难以捕捉细微的域不变局部细节（如配饰或纹理）；(2) 统一参数平均忽略客户差异，稀释高质量客户的贡献。

Method: 提出FedARKS框架，包含两个核心机制：RK（Robust Knowledge）和KS（Knowledge Selection）。RK机制用于提取鲁棒的域不变局部特征，KS机制用于根据客户质量进行差异化的知识选择和聚合。

Result: 通过引入局部特征提取和差异化聚合机制，FedARKS能够更好地捕捉域不变特征，并根据客户质量进行智能聚合。

Conclusion: 提出的FedARKS框架通过RK和KS两个机制有效解决了现有FedDG-ReID方法的局限性，提升了模型在未见域的泛化能力，同时保护了客户数据隐私。
...
TL;DR: 论文研究VLM的系统性感知弱点，通过空间颜色混合框架评估9个VLM，发现准确率随扭曲增加急剧下降，Scaling无法缓解，人类显著优于VLM，但简单预处理可恢复部分性能。


<details>
  <summary>Details</summary>
Motivation: 尽管VLM在基准测试中取得强劲结果，但存在系统性感知弱点：对像素值的结构性大变化会导致自信但无意义的预测，即使人类可以轻松识别底层场景

Method: 使用空间颜色混合框架，包含8种变体（在RGB和Ostwald颜色系统中），在4个数据集上评估9个VLM（涵盖3个模型家族），并进行了61人参与的人类研究

Result: 在所有模型和数据集上，准确率随扭曲程度增加而急剧下降；语言模型Scaling不能可靠地缓解失败；人类在相同扭曲下显著优于VLM；简单预处理可恢复部分性能

Conclusion: 简单的类人预处理步骤可以恢复部分性能，这为提高VLM鲁棒性提供了实用策略。Scaling模型并不能可靠地缓解这种失败。
...
TL;DR: 提出虚拟治疗(VT)框架，将NSCLC进展问题表述为剂量感知多模态条件图像到图像翻译任务，在222名III期NSCLC患者（895次CT扫描）数据集上评估，扩散模型表现优于GAN模型


<details>
  <summary>Details</summary>
Motivation: 预测放疗期间肿瘤演变是临床关键挑战，特别是当纵向变化由解剖结构和治疗共同驱动时，需要一种能够预测治疗诱导的解剖变化的工具

Method: 将NSCLC进展问题表述为剂量感知多模态条件图像到图像翻译问题，使用CT扫描、临床变量和辐射剂量增量作为条件输入，基准测试GAN和扩散模型在2D和2.5D配置下的表现

Result: 扩散模型比GAN模型更一致地受益于多模态剂量感知条件，产生更稳定和解剖学上合理的肿瘤进化轨迹

Conclusion: VT框架有望成为NSCLC silico治疗监测和自适应放疗研究的工具，扩散模型比GAN模型表现更稳定，能产生更解剖学上合理的肿瘤进化轨迹
...
TL;DR: FreeOcc是一个利用预训练基础模型（分割模型和重建模型）实现无需训练的语义和全景占用预测的pipeline，通过深度和置信度感知过滤、时间融合和体素化等步骤，在Occ3D-nuScenes上实现了与弱监督方法相当的性能，并能为下游模型提供高质量伪标签。


<details>
  <summary>Details</summary>
Motivation: 当前纯相机的语义和全景占用预测方法通常依赖高成本的密集3D监督或需要在目标域数据上训练模型，这限制了它们在未知环境中的部署。因此需要一种无需训练的方案来克服这些限制。

Method: FreeOcc是一个无需训练的pipeline，利用预训练基础模型从多视角图像中恢复语义和几何信息。具体包括：1）使用可提示的基础分割模型（promptable foundation segmentation model）和prompt-to-taxonomy规则提取每视角全景先验；2）使用重建基础模型重构度量3D点；3）深度和置信度感知的过滤将可靠标签提升到3D；4）随时间融合并用确定性细化栈进行体素化；5）对于全景占用，通过拟合和合并鲁棒的当前视角3D框候选来恢复实例，实现无需学习3D模型的实例感知占用预测。

Result: 在Occ3D-nuScenes数据集上，FreeOcc作为train-free方法实现了16.9 mIoU和16.5 RayIoU，与当前最先进的弱监督方法相当。当作为伪标签生成pipeline用于训练下游模型时，实现了21.1 RayIoU，超越了之前的SOTA弱监督基线。此外，FreeOcc为train-free和弱监督全景占用预测分别设立了新基线：3.1 RayPQ和3.9 RayPQ。

Conclusion: FreeOcc实现了无需训练的语义和全景占用预测，在Occ3D-nuScenes上达到16.9 mIoU和16.5 RayIoU；作为伪标签生成pipeline时达到21.1 RayIoU，超越之前的弱监督基线；为train-free和weakly supervised panoptic occupancy prediction建立了新基线（3.1 RayPQ和3.9 RayPQ）。结果强调了基础模型驱动的感知是实现无需训练的3D场景理解的实际路径。
...
TL;DR: 本文提出一种基于训练无关伪标签生成的半监督学习框架，利用视觉语言模型(VLM)和简单外观描述实现乳腺超声图像分割，在仅2.5%标注数据下达到全监督性能。


<details>
  <summary>Details</summary>
Motivation: 1. 半监督学习在乳腺超声图像分割中存在不稳定伪标签问题，导致监督不准确和性能下降
2. 现有视觉语言模型在医学图像上的有效性受限于难以迁移的领域特定提示
3. 极端有限标注情况下的性能稳定性需求

Method: 1. 利用简单外观描述（如深色椭圆）实现跨域结构迁移，使VLM生成结构一致的伪标签
2. 使用伪标签预热静态教师模型，获取乳腺病变全局结构先验
3. 结合指数移动平均教师，引入不确定性熵加权融合
4. 引入自适应不确定性引导的逆向对比学习以提升边界判别能力

Result: 在4个乳腺超声数据集上，仅用2.5%标注数据即可达到与全监督模型相当的性能，显著优于现有半监督方法

Conclusion: 该范式具有良好的可扩展性，只需全局外观描述即可应用于其他成像模态或疾病，实现有限标注下的可扩展半监督医学图像分割
...
TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据实现语言驱动的场景理解，将RGB-D全景图像转换为多模态输入并对齐视觉-语言特征，支持自然语言查询生成语义掩码。


<details>
  <summary>Details</summary>
Motivation: 语义分割任务面临标注数据稀缺和固定标签模型适应性有限的挑战，难以有效处理3D点云和全景图像等不同视觉模态。

Method: 将RGB-D全景图像转换为切向透视图像和3D点云，利用这些模态提取并对齐基础视觉-语言特征，实现自然语言查询在两种输入模态上生成语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D在开放词汇和封闭词汇的2D和3D语义分割任务中均取得了显著的先进性能提升。

Conclusion: JOPP-3D能够生成跨全景图像和3D域的连贯且语义有意义的分割结果，在开放词汇和封闭词汇的2D和3D语义分割任务中均显著优于现有最先进方法。
...
TL;DR: 本文提出了一种利用强化学习(PPO)和多尺度反馈增强3D扩散模型的方法，用于医学MRI图像生成。通过结合2D切片级和3D体积级评估的奖励系统，同时优化局部纹理和全局结构。在BraTS 2019和OASIS-1数据集上验证，显著改善了FID分数，并提升了合成数据在下游分类任务中的实用性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽已成为3D医学图像生成的强大工具，但标准训练目标与临床相关性之间存在差距，需要增强模型以生成更具临床价值的医学图像。

Method: 首先在MRI体积数据上预训练3D扩散模型以建立强大的生成先验，然后使用近端策略优化(PPO)进行微调。奖励系统创新性地结合了2D切片级评估和3D体积分析，使模型能够同时优化局部纹理细节和全局结构一致性。

Result: 在BraTS 2019和OASIS-1数据集上验证，结果表明RL反馈有效提升了生成质量。定量指标显示FID显著改善，更重要的是，合成数据在下游肿瘤和疾病分类任务中相比未优化基线表现出更强的实用性。

Conclusion: 提出的方法在BraTS 2019和OASIS-1数据集上验证，RL反馈有效引导生成过程向更高质量分布转变。定量分析显示FID分数显著改善，且合成数据在下游肿瘤和疾病分类任务中表现出更强的实用性。
...
TL;DR: 本文研究了在无需额外训练的情况下，如何利用预训练的文本到图像扩散模型进行语义分割的问题。针对现有方法无法有效利用更强大生成模型的问题，作者提出了两种技术：自动聚合和每像素重缩放，以更好地利用生成能力进行无训练分割。


<details>
  <summary>Details</summary>
Motivation: 尽管更强的生成模型应该带来更好的分割效果，但现有无训练扩散分割器往往无法相应扩展。作者发现两个关键问题：(i) 跨多个头和层计算的交叉注意力图与统一全局表示之间存在差异；(ii) 即使有全局图，由于不同文本token的分数不平衡，也不能直接转化为准确的语义相关性。

Method: 提出两种技术：(1) 自动聚合(Auto Aggregation)：解决跨多头多层的注意力图与统一全局表示之间的差异；(2) 每像素重缩进(Pixel-wise Rescaling)：解决不同文本token分数不平衡问题，使全局图能直接转化为准确的语义相关性。

Result: 在标准语义分割基准上进行了评估，并将其集成到生成技术中，展示了改进的性能和广泛的应用性。

Conclusion: 通过提出的自动聚合和每像素重缩放技术，成功实现了无训练分割方法对更强生成模型能力的更好利用，提升了语义分割性能。
...
TL;DR: 本文提出ALANet方法解决语言描述不准确情况下的复杂反射去除问题，通过过滤和优化双重策略以及语言线索解耦特征层，在新提出的CRLAV数据集上取得了优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 由于反射图像中的干扰往往模糊和扭曲，机器生成的图像语言描述通常不准确，这会损害语言引导的反射去除方法的性能。现有方法难以处理复杂反射问题。

Method: ALANet采用双重策略：1) 过滤策略降低语言的负面影响同时保留其益处；2) 优化策略增强语言与视觉特征的对齐。此外，利用语言线索从特征图中解耦特定层内容，提升处理复杂反射的能力。

Result: 实验结果表明，ALANet在图像反射去除任务中超越了最先进的方法。

Conclusion: ALANet通过结合过滤和优化策略，以及利用语言线索解耦特征图中的特定层内容，有效提升了在语言描述不准确情况下的复杂反射去除能力，在CRLAV数据集上超越了现有最先进的方法。
...
TL;DR: 提出了一种基于点监督的骨架动作分割框架，只需对每个动作段标注单帧即可训练模型。该方法利用多模态骨架数据（关节、骨骼、运动信息），通过预训练统一模型提取特征，并结合原型相似度、能量函数和约束K-Medoids聚类生成伪标签，实现高效的动作分割。


<details>
  <summary>Details</summary>
Motivation: 完全监督方法需要逐帧标注，成本高昂且对动作边界模糊敏感。为减少标注工作量并提高对边界模糊的鲁棒性，提出点监督框架，仅需对每个动作段标注一帧即可。

Method: 1）利用多模态骨架数据（关节、骨骼、运动信息）并通过预训练统一模型编码提取丰富特征表示；2）提出原型相似度方法生成可靠伪标签，并与能量函数和约束K-Medoids聚类两种现有方法结合；3）采用多模态伪标签集成策略增强伪标签可靠性并指导模型训练。

Result: 在PKU-MMD（X-Sub和X-View）、MCFS-22和MCFS-130数据集上建立新基准并实现有竞争力的性能，部分指标超越全监督方法，同时显著减少标注工作量。

Conclusion: 点监督框架能够以极少的标注成本（每段仅需一帧标注）达到与全监督方法相当甚至更好的性能，为骨架动作分割提供了一种高效低成本的解决方案，具有重要应用价值。
...
TL;DR: 本文提出VG3S框架，将视觉基础模型(VFM)的强几何 grounding 能力注入到基于高斯溅射的3D语义占用预测中，通过层次化几何特征适配器提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 纯视觉三维语义占用预测缺乏准确的几何线索，而高斯溅射方法的性能高度依赖精确的几何信息。现有视觉中心方法几何线索不足，限制了占用预测质量。

Method: 提出VG3S框架，包含一个即插即用的层次化几何特征适配器。该适配器通过特征聚合、任务特定对齐和多尺度重构，将冻结的VFM通用tokens有效转化为富含几何先验的任务特征。

Result: 在nuScenes占用基准上，VG3S相比基线模型实现IoU提升12.6%、mIoU提升7.5%。实验表明VG3S可无缝泛化至不同VFM，持续提升占用预测精度。

Conclusion: 验证了将强大的预训练几何 grounding VFM先验集成到占用预测任务中的巨大价值，为自动驾驶场景理解提供了更可靠的几何感知能力。
...
TL;DR: 针对3DGS几何对齐问题，提出基于协方差矩阵特征值计算邻域Eigenentropy的交替优化densification策略，在DTU和TUM2TWIN数据集上实现几何-渲染-效率的权衡优化


<details>
  <summary>Details</summary>
Motivation: 标准3DGS的高斯中心与底层几何对齐差，表面重建方法牺牲光度精度，传统densification仅依赖view-space梯度幅值缺乏几何感知

Method: 提出Eigenentropy概念：从协方差矩阵特征值计算邻域结构有序度；交替优化框架：标准梯度 densification + Eigenentropy-aware densification（低熵区域增强分裂，高熵区域剪枝）

Result: 几何精度提升33%，渲染质量提升7%，高斯数量减少50%，训练时间减少23%

Conclusion: EntON通过特征值分解的Eigenentropy方法实现了3DGS的几何-渲染-效率平衡，但交替优化的计算开销和特征值实时求解的计算复杂度需要关注
...
TL;DR: 本文提出WAFL，将时序伪造定位从时序回归/连续定位转变为词级二分类新范式，通过取证特征重对齐模块和以伪造物为中心的非对称损失函数解决特征粒度对齐和类别不平衡问题，实现高效准确的时序伪造定位。


<details>
  <summary>Details</summary>
Motivation: 现有时序伪造定位方法存在特征粒度对齐问题和计算成本高的问题。当前方法依赖时序边界回归或连续帧级异常检测，但这些方法存在特征粒度不对齐和计算昂贵的问题。

Method: 提出词锚定时序伪造定位(WAFL)新范式，包括：1)将时序伪造定位重新定义为词级二分类任务；2)引入取证特征重对齐(FFR)模块，将预训练语义空间映射到判别取证流形；3)设计以伪造物为中心的非对称(ACA)损失函数，动态抑制大量真实样本梯度，同时非对称地优先处理细微取证伪造痕迹。

Result: WAFL在数据集内和跨数据集设置下的定位性能显著优于最先进的方法，同时需要更少的可学习参数且计算效率高。

Conclusion: WAFL通过将时序伪造定位任务从时序回归和连续定位转变为离散的词级二分类，实现了最先进的定位性能。该方法在跨数据集设置下显著优于现有方法，同时需要更少的可学习参数和更高的计算效率。
...
### [95] [Low-latency Event-based Object Detection with Spatially-Sparse Linear Attention](https://arxiv.org/abs/2603.06228)
...
TL;DR: 提出HCF-RES框架，通过层次化视觉语义分解和渐进式多层次融合解决3D场景中基于自然语言的对象定位问题，在ScanRefer和Multi3DRefer数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖稀疏点云，缺乏丰富的视觉语义信息来处理细粒度描述。3D-GRES任务需要根据自然语言在3D场景中定位对象，即使描述匹配多个或零个目标也能处理。

Method: 提出HCF-RES多模态框架，包含两个关键创新：1) 层次化视觉语义分解(Hierarchical Visual Semantic Decomposition)：利用SAM实例掩码引导CLIP在像素级和实例级两个粒度进行编码，在2D到3D投影过程中保留物体边界；2) 渐进式多层次融合(Progressive Multi-level Fusion)：通过模态内协作、2D语义与3D几何特征之间的跨模态自适应加权以及语言引导的细化来整合表示。

Result: 在ScanRefer和Multi3DRefer两个数据集上均实现了最先进的性能表现。

Conclusion: HCF-RES通过层次化视觉语义分解和渐进式多层次融合，有效解决了3D场景中基于自然语言的对象定位问题。该方法在ScanRefer和Multi3DRefer两个基准数据集上均达到了最先进的性能，验证了多模态融合策略的有效性。
...
### [97] [NOVA: Next-step Open-Vocabulary Autoregression for 3D Multi-Object Tracking in Autonomous Driving](https://arxiv.org/abs/2603.06254)
*Kai Luo,Xu Wang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: NOVA是一种新型的开放词汇3D多目标跟踪范式，将传统基于距离匹配的跟踪任务转变为生成式时空语义建模任务，利用大语言模型的自回归能力实现下一时刻序列补全，从而突破封闭集假设和语义盲 heuristic 的限制。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪 pipeline 受到封闭集假设和"语义盲"heuristic 的限制，无法处理开放世界感知中的未知目标泛化问题。传统方法依赖距离匹配，缺乏深层语言先验和语义推理能力。

Method: 提出Next-step Open-Vocabulary Autoregression (NOVA)方法，将3D轨迹重新表述为结构化时空语义序列，同时编码物理运动连续性和深层语言先验。利用大语言模型的自回归能力，将跟踪任务转化为下一时刻序列补全过程，利用语言空间的层次结构解决细粒度语义歧义并通过常识推理维护长序列身份一致性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上验证。在nuScenes上，NOVA对Novel类别实现22.41%的AMOTA，相对基线提升20.21%的绝对增益。模型参数量仅为0.5B。

Conclusion: NOVA通过将3D跟踪从传统碎片化距离匹配转向生成式时空语义建模，成功实现了开放词汇3D跟踪，在novel类别上取得显著性能提升，证明了自回归大语言模型在多目标跟踪任务中的潜力。
...
TL;DR: 本文提出StrSR，一种用于DiT模型的一步对抗蒸馏框架，通过非对称判别蒸馏和频率分布匹配解决轨迹不匹配和周期伪影问题，在Real-ISR任务上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: Diffusion transformer (DiT)架构在真实场景图像超分辨率(Real-ISR)中表现出色，但其迭代采样计算成本高，需要一步蒸馏。现有一步蒸馏方法存在轨迹不匹配问题，并产生严重的网格状周期伪影。

Method: 提出StrSR框架，采用非对称判别蒸馏架构弥合轨迹差距，设计频率分布匹配策略抑制高频光谱泄漏引起的周期伪影，实现单步对抗蒸馏。

Result: StrSR在Real-ISR任务上实现了最先进的性能，无论是在定量指标还是视觉感知方面。

Conclusion: StrSR通过非对称判别蒸馏架构和频率分布匹配策略，有效解决了DiT模型在单步超分辨率中的轨迹不匹配和周期伪影问题，在定量指标和视觉感知方面均达到了Real-ISR的先进水平。
...
### [99] [Can we Trust Unreliable Voxels? Exploring 3D Semantic Occupancy Prediction under Label Noise](https://arxiv.org/abs/2603.06279)
*Wenxin Li,Kunyu Peng,Di Wen,Junwei Zheng,Jiale Wei,Mengfei Duan,Yuheng Zhang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: 本文提出了OccNL基准和DPR-Occ框架，解决3D语义占用预测中标签噪声问题。传统2D标签噪声学习方法在稀疏3D体素空间中失效。DPR-Occ通过双源部分标签推理，结合时序模型记忆和表示级结构亲和力，在90%噪声下仍可提升2.57% mIoU和13.91% IoU。


<details>
  <summary>Details</summary>
Motivation: 现实世界的体素注释存在结构伪影和动态拖尾效应，导致标签不可靠。这是一个关键但未被探索的问题：自主系统能否安全地依赖这种不可靠的占用监督？现有2D标签噪声学习方法在稀疏3D体素空间中会灾难性失效，暴露了现有范式的关键漏洞。

Method: 提出DPR-Occ框架，通过双源部分标签推理构建可靠监督。1) 时序模型记忆：利用时间信息存储和检索可靠的特征表示；2) 表示级结构亲和力：计算体素间的特征相似度；3) 动态扩展和剪枝：扩展候选标签集以保留真实语义，同时剪枝以抑制噪声传播。

Result: 在SemanticKITTI数据集上，DPR-Occ在极端损坏下防止了几何和语义崩溃。在90%标签噪声下，比现有的3D占用预测标签噪声学习基线方法提升高达2.57% mIoU和13.91% IoU。

Conclusion: 本文填补了标签噪声学习与3D感知之间的鸿沟。OccNL是首个针对3D占用预测中非对称和动态拖尾噪声的基准。DPR-Occ通过双源部分标签推理有效应对极端噪声，为动态环境中的安全关键机器人感知提供了可靠基础。
...
### [100] [Attribute Distribution Modeling and Semantic-Visual Alignment for Generative Zero-shot Learning](https://arxiv.org/abs/2603.06281)
*Haojie Pu,Zhuoming Li,Yongbiao Gao,Yuheng Jia*

Main category: cs.CV

TL;DR: 本文提出ADiVA方法，通过属性分布建模和语义-视觉对齐解决生成式零样本学习中的类实例差距和语义-视觉域差距问题，在基准数据集上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习存在两个内在挑战：1) 类级属性因类内差异大而无法捕捉实例级视觉外观，导致类实例差距；2) 语义与视觉特征分布不匹配（类间相关性），导致语义-视觉域差距。

Method: 提出ADiVA方法，包含两个模块：1) 属性分布建模(ADM)模块：学习可迁移的属性分布，为未见类采样实例级属性；2) 视觉引导对齐(VGA)模块：优化语义表示以更好反映视觉结构。

Result: 在三个基准数据集上显著优于最先进方法，在AWA2和SUN数据集上分别获得4.7%和6.1%的性能提升。

Conclusion: ADiVA作为一种生成式零样本学习方法，通过属性分布建模和语义-视觉对齐两个模块，有效解决了类实例差距和语义-视觉域差距两大挑战。实验表明，该方法在AWA2和SUN数据集上分别提升了4.7%和6.1%，且可作为插件增强现有生成式ZSL方法。
...
### [101] [FlowMotion: Training-Free Flow Guidance for Video Motion Transfer](https://arxiv.org/abs/2603.06289)
*Zhen Wang,Youcan Xu,Jun Xiao,Long Chen*

Main category: cs.CV

TL;DR: FlowMotion是一个免训练框架，通过直接利用基于流的T2V模型的预测输出来实现高效灵活的运动转移


<details>
  <summary>Details</summary>
Motivation: 现有免训练方法依赖于预训练T2V模型的中间输出，导致计算开销大且灵活性有限

Method: 提出流引导（flow guidance），从潜在预测中提取运动表示，以对齐源视频和生成视频之间的运动模式；引入速度正则化策略来稳定优化并确保平滑的运动演变

Result: 在时间和资源效率上表现卓越，性能与最先进方法相当

Conclusion: FlowMotion通过直接在模型预测上操作，实现了卓越的时间和资源效率，以及与最先进方法相当的竞争性能
...
TL;DR: 本文提出了一种基于垂直分数生成扩散模型的3D牙科种植体修复方法，用于减少CBCT图像中的种植体伪影。该方法通过结合两个在不同平面上训练的2D分数生成扩散模型来建模投影序列的3D分布，在投影域中进行修复。实验结果表明该方法能有效生成高质量的3D CBCT图像。


<details>
  <summary>Details</summary>
Motivation: CBCT技术虽然能提供高分辨率图像并减少辐射暴露，但容易受到高密度物体（如牙科种植体）产生的伪影影响，这些伪影会损害图像质量和诊断准确性。在投影序列中进行种植体修复是减少伪影的关键步骤。

Method: 提出了一种基于垂直分数生成扩散模型的3D牙科种植体修复方法。该方法在两个不同平面上分别训练2D分数生成扩散模型，并在采样方案中结合这两个模型来建模投影序列的3D分布，从而在投影域中进行种植体修复。

Result: 实验结果表明，该方法在生成高质量、减少伪影的3D CBCT图像方面有效。

Conclusion: 该方法在生成高质量、减少伪影的3D CBCT图像方面表现出色，是改善临床成像的有前景的解决方案。
...
### [103] [Latent Transfer Attack: Adversarial Examples via Generative Latent Spaces](https://arxiv.org/abs/2603.06311)
*Eitan Shaar,Ariel Shaulov,Yalcin Tur,Gal Chechik,Ravid Shwartz-Ziv*

Main category: cs.CV

TL;DR: LTA（潜在迁移攻击）是一种迁移攻击方法，通过在预训练Stable Diffusion VAE的潜在空间中进行扰动优化，而非传统的像素空间优化，以产生更鲁棒、更易迁移的对抗样本。


<details>
  <summary>Details</summary>
Motivation: 当前像素空间优化方法产生的对抗扰动是高频、纹理-like的噪声，对常见预处理（如调整大小和裁剪）很脆弱，且在不同架构之间迁移效果差。

Method: 将图像编码到Stable Diffusion VAE的潜在空间中，优化潜在表示以最大化替代分类器损失，同时在解码后软性强制执行像素空间ℓ∞预算。纳入EOT（期望过变换）通过随机调整大小、插值和裁剪来提高对分辨率不匹配和标准输入管道的鲁棒性，并应用周期性潜在高斯平滑来抑制出现的伪影并稳定优化。

Result: LTA在CNN和视觉Transformer目标上实现了强大的迁移攻击成功，同时产生空间连贯的、主要是低频的扰动，与像素空间基线在质上不同，在迁移-质量权衡中占据独特的点。

Conclusion: 预训练生成式潜在空间是进行对抗优化的有效且结构化的领域，将鲁棒性评估与现代生成先验相结合。LTA在CNN和视觉Transformer目标上实现了强迁移攻击成功，同时产生空间连贯的、主要是低频的扰动，在迁移-质量权衡中占据独特位置。
...
### [104] [WMoE-CLIP: Wavelet-Enhanced Mixture-of-Experts Prompt Learning for Zero-Shot Anomaly Detection](https://arxiv.org/abs/2603.06313)
*Peng Chen,Chao Huang*

Main category: cs.CV

TL;DR: 提出一种基于小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器整合语义表示、小波分解提取多频率特征、语义感知MoE模块提升异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定文本提示难以捕捉复杂语义，且仅关注空间域特征，无法有效检测细微异常。

Method: 采用变分自编码器建模全局语义表示并集成到提示中；利用小波分解提取多频率图像特征，通过跨模态交互动态细化文本嵌入；引入语义感知专家混合模块聚合上下文信息。

Result: 在14个工业和医学数据集上进行了广泛实验，验证了所提方法的有效性。

Conclusion: 提出的Wavelet-MoE方法在14个工业和医学数据集上展现出优异的异常检测性能，验证了多频率特征融合和专家混合模块的有效性。
...
### [105] [P-SLCR: Unsupervised Point Cloud Semantic Segmentation via Prototypes Structure Learning and Consistent Reasoning](https://arxiv.org/abs/2603.06321)
*Lixin Zhan,Jie Jiang,Tianjian Zhou,Yukun Du,Yan Zheng,Xuehu Duan*

Main category: cs.CV

TL;DR: 本文提出P-SLCR方法，一种基于原型库驱动的无监督点云语义分割策略，通过结构学习和一致推理实现无需人工标注的点云分割，在S3DIS数据集Area-5上达到47.1%的mIoU，超越有监督的PointNet方法2.5%。


<details>
  <summary>Details</summary>
Motivation: 当前点云语义分割方法严重依赖人工标注，而针对原始点云的无监督语义分割研究仍处于早期阶段。缺乏标注信息和预训练是无监督点云学习面临的主要挑战，因此开发有效的无监督策略至关重要。

Method: 提出两种关键技术：1) 一致结构学习(Consistent Structure Learning)：通过选择高质量特征，在一致点与一致原型库之间建立结构特征学习；2) 语义关系一致推理(Semantic Relation Consistent Reasoning)：分别针对一致和模糊原型库构建原型互关系矩阵，通过对该矩阵施加约束来保持语义一致性。

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估。在S3DIS数据集Area-5上达到47.1%的mIoU，比经典全监督方法PointNet高出2.5%，实现了无监督方法中的最佳性能。

Conclusion: 该研究成功实现了无需人工标注的点云语义分割，通过原型库驱动的结构学习和一致推理机制，有效解决了无监督学习中的特征学习和语义一致性问题，为点云无监督分割提供了新的研究方向。
...
### [106] [WorldCache: Accelerating World Models for Free via Heterogeneous Token Caching](https://arxiv.org/abs/2603.06331)
...
TL;DR: WorldCache是一个针对扩散世界模型的缓存框架，通过曲率引导的异构 token 预测和混沌优先自适应跳过机制，实现了高达3.7倍的速度提升，同时保持98%的 rollout 质量。


<details>
  <summary>Details</summary>
Motivation: 扩散世界模型在统一世界模拟方面展现出强大潜力，但迭代去噪过程对于交互式使用和长视野 rollout 仍然成本过高。现有针对单模态扩散设计的策略无法直接应用于世界模型，因为存在两个核心障碍：1) 来自多模态耦合和空间变化的 token 异构性；2) 非均匀时间动态，其中少量困难 token 驱动误差增长，使得均匀跳过策略要么不稳定，要么过于保守。

Method: 提出 WorldCache 缓存框架，包含两个核心技术：1) 曲率引导的异构 token 预测：使用基于物理的曲率分数估计 token 可预测性，并对具有突变方向的混沌 token 应用 Hermite 引导的阻尼预测器；2) 混沌优先自适应跳过：累积曲率归一化的无量纲漂移信号，仅当瓶颈 token 开始漂移时重新计算。

Result: 在扩散世界模型上的实验表明，WorldCache 实现了高达 3.7 倍的端到端加速，同时保持 98% 的 rollout 质量。

Conclusion: WorldCache 展示了在资源受限场景下扩散世界模型的实用性和巨大优势，为高效的世界模型推理提供了有效解决方案。
...
### [107] [LATO: 3D Mesh Flow Matching with Structured TOpology Preserving LAtents](https://arxiv.org/abs/2603.06357)
...
TL;DR: LATO是一种新颖的拓扑保持潜在表示方法，实现基于流匹配的可扩展3D网格合成。该方法将网格表示为锚定在表面上的顶点位移场(VDF)，结合稀疏体素变分自编码器(VAE)将显式信号压缩为结构化的拓扑感知体素潜在空间。


<details>
  <summary>Details</summary>
Motivation: 现有的等值面/三角形扩散模型和自回归生成方法在生成具有复杂几何形状和良好拓扑结构的网格时存在挑战，且推理效率较低。

Method: LATO采用以下技术方案：1)将网格表示为锚定在表面上的顶点位移场(VDF)；2)使用稀疏体素VAE将显式信号压缩为拓扑感知的体素潜在空间；3)VAE解码器逐步细分和修剪潜在体素以实例化精确的顶点位置；4)专用连接头查询体素潜在空间直接预测顶点对之间的边连接，从而无需等值面提取或启发式网格化即可恢复网格拓扑；5)生成建模采用两阶段流匹配过程，先合成结构体素，再细化体素级拓扑特征。

Result: LATO能够生成具有复杂几何形状和良好拓扑结构的网格，同时具有极高的推理效率。

Conclusion: 与现有的等值面/三角形扩散模型和自回归生成方法相比，LATO在网格几何复杂性、拓扑质量和推理效率方面均取得了更好的效果。
...
### [108] [Rewis3d: Reconstruction Improves Weakly-Supervised Semantic Segmentation](https://arxiv.org/abs/2603.06374)
*Jonas Ernst,Wolfgang Boettcher,Lukas Hoyer,Jan Eric Lenssen,Bernt Schiele*

Main category: cs.CV

TL;DR: Rewis3d是一个利用3D重建技术改进弱监督语义分割的框架，通过双学生-教师架构在2D图像和3D点云之间实施语义一致性，利用前馈3D重建生成可靠的几何监督信号，在稀疏监督下实现最先进性能，比现有方法提高2-7%。


<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注仍是训练分割模型的高成本瓶颈，稀疏标注虽然更高效但存在性能差距，需要寻找方法来缩小这一性能差距。

Method: 引入双学生-教师架构，利用前馈3D重建技术从2D视频中恢复3D几何结构，将稀疏标注传播到整个场景，在2D图像和重建的3D点云之间实施语义一致性监督。

Result: Rewis3d在稀疏监督下实现了最先进的性能，比现有方法高出2-7%，且不需要额外标签或推理开销。

Conclusion: 3D几何结构可以作为强监督信号来改进弱监督语义分割，通过将2D图像与3D点云的语义一致性相结合，可以有效缩小稀疏标注与密集标注之间的性能差距。
...
### [109] [CHMv2: Improvements in Global Canopy Height Mapping using DINOv3](https://arxiv.org/abs/2603.06382)
...
TL;DR: 该研究提出了CHMv2，一个基于DINOv3深度学习模型从高分辨率光学卫星影像生成的全球米级冠层高度地图。通过大规模多样化训练数据、自动化数据处理和定制化损失函数，该产品在精度、偏差控制和精细结构保留方面均优于现有产品，并经多种独立数据源验证了在全球主要森林生物群落中的可靠性能。


<details>
  <summary>Details</summary>
Motivation: 目前高保真机载激光扫描(ALS)测量数据在全球范围内分布不均，而准确的冠层高度信息对于森林碳储量量化、森林恢复与退化监测以及栖息地结构评估至关重要。因此需要开发一种能够覆盖全球、高精度、高分辨率的冠层高度制图方法。

Method: 该研究采用基于DINOv3构建的深度估计模型，利用高分辨率光学卫星影像预测冠层高度。关键技术包括：1)大规模地理多样性训练数据的扩展；2)自动化数据整理与配准；3)针对冠层高度分布定制的损失函数和数据采样策略。验证方法包括独立ALS测试集评估以及与GEDI和ICESat-2激光雷达卫星观测数据的对比。

Result: CHMv2相比现有产品显著提高了精度，减少了高大森林中的偏差，并更好地保留了冠层边缘和间隙等精细尺度结构。在主要森林生物群落中经独立ALS测试集验证表现一致，同时与GEDI和ICESat-2数十亿次观测对比也证实了性能的稳定性。

Conclusion: CHMv2是一个高分辨率(米级)的全球冠层高度地图，通过利用DINOv3深度学习模型和大规模、多样化的训练数据，实现了相比现有产品更高的精度、更少的偏差，并能更好地保留冠层边缘和间隙等精细结构。该产品经独立ALS测试集以及GEDI和ICESat-2观测数据验证，在主要森林生物群落中表现出一致的性能，有望为全球森林碳储量估算、森林恢复与退化监测以及栖息地结构评估提供更可靠的遥感数据支持。
...
### [110] [REACT++: Efficient Cross-Attention for Real-Time Scene Graph Generation](https://arxiv.org/abs/2603.06386)
*Maëlic Neau,Zoe Falomir*

Main category: cs.CV

TL;DR: REACT++是一个用于实时场景图生成的新模型，通过原型空间中的高效特征提取和交叉注意力机制，在不牺牲目标检测性能的情况下，实现了最高推理速度和显著提升的关系预测准确度（比REACT快20%，准确度提升10%）。


<details>
  <summary>Details</summary>
Motivation: Scene Graph Generation (SGG)将图像中物体间的视觉关系编码为图结构，是具身智能体推理等下游任务的基础组件。现有方法往往只关注三个目标中的一个：提高关系预测准确度、提高目标检测准确度或降低延迟，缺乏同时平衡这三个目标的方法。

Method: 基于REACT架构构建REACT++，利用高效特征提取和原型空间中的主体-客体交叉注意力机制，在保持低延迟的同时增强表示能力。

Result: REACT++在现有SGG模型中实现最高推理速度，提升了关系预测准确度且未牺牲目标检测性能。相比前代REACT，推理速度提升20%，平均关系预测准确度提升10%。

Conclusion: REACT++是一个新的实时SGG最优模型，在延迟和表示能力之间实现了良好平衡。
...
### [111] [Solving Jigsaw Puzzles in the Wild: Human-Guided Reconstruction of Cultural Heritage Fragments](https://arxiv.org/abs/2603.06389)
*Omidreza Safaei,Sinem Aslan,Sebastiano Vascon,Luca Palmieri,Marina Khoroshiltseva,Marcello Pelillo*

Main category: cs.CV

TL;DR: 提出一种用于大规模真实世界文化遗产重建的人机循环（HIL）拼图求解框架，通过结合自动松弛标记求解器与交互式人工指导，解决考古文物碎片修复中的复杂性和规模问题。


<details>
  <summary>Details</summary>
Motivation: 传统拼图求解器难以应对真实考古文物修复中的侵蚀、缺失区域、不规则形状和大规模歧义等问题，特别是在碎片数量达数千的RePAIR基准测试中。

Method: 引入两种互补的交互策略：迭代锚定（Iterative Anchoring）和连续交互优化（Continuous Interactive Refinement），将自动松弛标记求解器与交互式人工指导相结合。

Result: 在RePAIR多个组上的实验表明，混合方法在准确性和效率上显著优于纯自动和纯人工基线。

Conclusion: 提出的人机协作框架在准确性和效率上均优于纯自动和纯人工基线，为大规模文物修复提供了实用解决方案。
...
### [112] [Locating and Editing Figure-Ground Organization in Vision Transformers](https://arxiv.org/abs/2603.06407)
*Stefan Arnold,René Gröbner*

Main category: cs.CV

TL;DR: 本文研究了BEiT视觉Transformer如何通过内部组件实现Gestalt凸性先验。研究使用飞镖形状创造感知冲突，发现BEiT可靠地偏好凸性补全，并通过分析注意力头识别出L0H9作为引入凸性偏见的早期功能单元。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer需要在局部几何证据和全局组织先验之间做出选择，导致特征-背景组织的感知歧义。作者试图定位经典Gestalt凸性先验在BEiT内部组件中的实现位置。

Method: 使用合成飞镖形状创造受控的感知冲突，创造同时支持凹性或凸性补全的区域。通过logit归因将内部激活投影到模型的离散视觉码本空间，分析注意力头的直接效应来识别功能单元。

Result: BEiT在竞争中可靠地偏好凸性补全。特征-背景组织在早期和中间层是模糊的，在后期层突然resolve。通过分解注意力头的直接效应，发现L0H9作为早期种子引入微弱的凸性偏见。缩小这个单一注意力头可以将感知冲突的分布质量转移到连续决策边界上，允许凹性证据指导补全。

Conclusion: 凸性偏好由transformer子结构中的可识别功能单元 governing。单个注意力头(L0H9)的调节足以改变模型的感知决策边界，表明视觉Transformer中的特征-背景组织可以通过干预特定神经单元来操控。
...
### [113] [What if? Emulative Simulation with World Models for Situated Reasoning](https://arxiv.org/abs/2603.06445)
...
TL;DR: WanderDream是首个大规模情境推理心理探索数据集，包含15.8K全景视频（1088个真实场景）和158K问答对，用于评估无需主动探索的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现实场景中主动探索往往不可行（机器人物理限制、视障用户安全顾虑），现有方法无法在仅依赖有限观察的情况下进行心理模拟并回答空间假设问题。

Method: 提出WanderDream数据集，包含两个子集：(1) WanderDream-Gen：15.8K全景视频，涵盖1088个真实场景的想象轨迹；(2) WanderDream-QA：158K问答对，覆盖轨迹的起点、路径和终点。采用世界模型和多模态大语言模型进行实验评估。

Result: WanderDream是首个大规模心理探索模拟数据集；世界模型在轨迹生成上表现良好；想象机制显著提升推理能力；数据具有良好的真实场景迁移性。

Conclusion: 实验表明：(1) 心理探索对情境推理至关重要；(2) 世界模型在WanderDream-Gen上表现出色；(3) 想象显著提升WanderDream-QA的推理能力；(4) WanderDream数据具有优秀的真实场景迁移能力。代码和数据将开源发布。
...
### [114] [CaTok: Taming Mean Flows for One-Dimensional Causal Image Tokenization](https://arxiv.org/abs/2603.06449)
*Yitong Chen,Zuxuan Wu,Xipeng Qiu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: CaTok是一个1D因果图像分词器，通过MeanFlow解码器和时间间隔token选择实现因果表示学习，支持快速单步生成和高保真多步采样，在ImageNet上以更少训练轮次达到SOTA重建质量


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型依赖因果token化，但视觉领域的扩展面临挑战：现有视觉tokenizer将2D patches展平为非因果序列或采用启发式排序；diffusion autoencoders也存在因果性缺失或nested dropout导致的不平衡问题

Method: 提出CaTok：1D因果图像分词器，使用MeanFlow解码器，通过时间间隔选择token并绑定MeanFlow目标；提出REPA-A正则化方法，将encoder特征与视觉基础模型(VFM)对齐

Result: CaTok在ImageNet重建任务上达到SOTA：FID 0.75，PSNR 22.53，SSIM 0.674，使用更少训练轮次；AR模型性能与领先方法相当

Conclusion: CaTok在ImageNet重建任务上达到最先进水平，FID为0.75，PSNR为22.53，SSIM为0.674，训练轮次更少；AR模型性能与领先方法相当
...
### [115] [Pinterest Canvas: Large-Scale Image Generation at Pinterest](https://arxiv.org/abs/2603.06453)
*Yu Wang,Eric Tzeng,Raymond Shiau,Jie Yang,Dmitry Kislyuk,Charles Rosenberg*

Main category: cs.CV

TL;DR: Pinterest Canvas是一个大规模图像生成系统，通过在多样化数据集上训练基础扩散模型，然后针对特定用例快速微调，专门化模型在背景增强和宽高比外绘任务上分别获得18.0%和12.5%的参与度提升，并可推广到多图像场景合成和图像到视频生成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有的图像生成模型虽然灵活但难以通过提示或简单推理调整进行精确控制，无法满足严格的产品要求。

Method: Pinterest Canvas是一个大规模图像生成系统，首先在多样化的多模态数据集上训练基础扩散模型以获得广泛的图像编辑能力，然后针对特定用例数据集快速微调基础模型以产生专门化模型。关键组件包括数据集策展、训练和推理优化。

Result: 在线A/B实验显示，背景增强和宽高比外绘的增强图像分别获得18.0%和12.5%的参与度提升；与人类评估者的比较表明，模型在这些任务上优于第三方模型。

Conclusion: 该方法可推广到多种下游任务，包括多图像场景合成和图像到视频生成。通过快速微调基础模型来适应特定用例的方法具有广泛的应用前景。
...
### [116] [Training Flow Matching: The Role of Weighting and Parameterization](https://arxiv.org/abs/2603.06454)
*Anne Gagneux,Ségolène Martin,Rémi Gribonval,Mathurin Massias*

Main category: cs.CV

TL;DR: 该研究系统性地分析了去噪生成模型（特别是flow matching）的训练目标选择问题，重点关注损失加权策略和输出参数化方式，并探索了这些选择与数据流形内在维度、模型架构和数据集规模的相互作用关系。


<details>
  <summary>Details</summary>
Motivation: 当前去噪生成模型的训练目标存在多种选择（噪声参数化、干净图像参数化、速度参数化），但尚不清楚这些选择如何与数据流形内在维度、模型架构和数据集大小相互作用，需要系统性地 disentangle 这些影响因素。

Method: 采用系统性数值实验方法，在受控几何的合成数据集和真实图像数据上进行比较研究，使用PSNR（峰值信噪比）评估去噪精度，FID（Fréchet inception distance）评估生成质量。

Result: 实验揭示了不同训练目标在各因素交互下的表现差异，为实际应用中的设计选择提供了量化依据和实践洞察。

Conclusion: 研究提供了关于如何根据数据流形维度、模型架构和数据集规模选择合适训练目标的实用指导原则，帮助从业者在实际应用中做出更明智的设计决策。
...
### [117] [GreenRFM: Toward a resource-efficient radiology foundation model](https://arxiv.org/abs/2603.06467)
*Yingtai Li,Shuai Ming,Mingyue Zhao,Haoran Lai,Rongsheng Wang,Rui Zhou,Rundong Wang,Yujia Li,Wei Wei,Shaohua Kevin Zhou*

Main category: cs.CV

TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督策略（More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning）在仅使用单块24GB GPU训练24小时或6GB VRAM训练4小时的条件下，达到了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 当前放射学基础模型（RFMs）的发展受制于暴力扩展方法。现有方法直接借鉴自然图像的处理方式，优先考虑规模而非精度，导致模型在临床实践中脆弱且昂贵。

Method: 提出GreenRFM框架，采用MUST监督策略，最大化利用监督信号的质量而非单纯堆砌数据量。提供两种配置：(i)高性能模型使用单块24GB GPU训练24小时；(ii)轻量级模型仅需6GB VRAM训练4小时。实验使用来自四个机构的超过20万张图像，涵盖CT和MRI两种模态。

Result: 在胸部和腹部CT数据集上取得优异性能，超越各种基线模型。内部肌肉骨骼MRI图像结果证明该监督策略在不同模态间具有可迁移性。计算需求降低数个数量级，同时超越复杂的重参数模型。

Conclusion: 挑战了"规模即一切"的观念，使临床医生甚至在笔记本电脑上也能民主化开发先进放射学基础模型，促进公平发展。
...
### [118] [Match4Annotate: Propagating Sparse Video Annotations via Implicit Neural Feature Matching](https://arxiv.org/abs/2603.06471)
*Zhuorui Zhang,Roger Pallarès-López,Praneeth Namburi,Brian W. Anthony*

Main category: cs.CV

TL;DR: 提出Match4Annotate框架，利用SIREN隐式神经表示和DINOv3特征实现视频标注的跨视频和同视频传播，在临床超声数据集上达到最优跨视频传播性能。


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，获取逐帧视频标注是部署计算机视觉的主要瓶颈，因为专家标注既慢又昂贵。现有的标签传播方法存在根本性局限：视频追踪器和分割模型只能在单个序列内传播标签，需要每视频初始化且无法跨视频泛化；经典对应管道在低纹理场景中表现困难；而密集特征匹配和一次性分割方法缺乏时空平滑性，且无法统一支持点和掩码标注。

Method: 提出Match4Annotate框架，用于点 和掩码标注的跨视频和同视频传播。方法是在测试时将基于SIREN的隐式神经表示拟合到DINOv3特征，产生连续的高分辨率时空特征场，并学习帧对之间的平滑隐式变形场以指导对应匹配。

Result: 在三个具有挑战性的临床超声数据集上评估，Match4Annotate实现了最先进的跨视频传播性能，优于特征匹配和一次性分割基线方法，同时在同视频传播方面与专业追踪器保持竞争力。

Conclusion: 轻量级、测试时优化的特征匹配管道有潜力为可扩展标注工作流程提供高效且可访问的解决方案。
...
### [119] [Self-Supervised Flow Matching for Scalable Multi-Modal Synthesis](https://arxiv.org/abs/2603.06507)
*Hila Chefer,Patrick Esser,Dominik Lorenz,Dustin Podell,Vikash Raja,Vinh Tong,Antonio Torralba,Robin Rombach*

Main category: cs.CV

TL;DR: Self-Flow是一种自监督流匹配方法，通过Dual-Timestep Scheduling机制在生成模型内部集成表示学习，无需外部监督即可同时实现强大的语义表示学习和高质量多模态生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型获取语义表示，需单独训练且目标不对齐，Scaling行为不可控。根本原因在于模型训练目标（去噪任务）缺乏学习语义表示的激励机制。

Method: 提出Self-Flow自监督流匹配范式，核心机制是Dual-Timestep Scheduling（双时间步调度）：对不同token应用异构噪声水平，制造信息不对称，强制模型从损坏输入中推断缺失信息，从而在学习生成能力的同时学习强语义表示。

Result: 在图像、视频和音频生成任务上取得优异性能，支持多模态训练，遵循预期的Scaling定律。

Conclusion: Self-Flow通过Dual-Timestep Scheduling机制，在不依赖外部监督的情况下实现强大的语义表示学习和生成能力的结合。该方法可泛化到多模态，并在图像、视频和音频生成任务上达到最优性能，同时遵循预期的Scaling定律。
...
### [120] [AV-Unified: A Unified Framework for Audio-visual Scene Understanding](https://arxiv.org/abs/2603.06530)
*Guangyao Li,Xin Wang,Wenwu Zhu*

Main category: cs.CV

TL;DR: AV-Unified是一个统一框架，用于在动态真实场景中进行跨多个音频-视觉场景理解任务的联合学习，包括事件定位、解析、分割和问答等任务。该框架通过将所有任务的输入输出转换为离散标记序列，建立共享表示，使单一架构能够在异构数据集上联合训练。


<details>
  <summary>Details</summary>
Motivation: 当前音频-视觉任务大多被单独探索，难以全面理解复杂的音频-视觉场景并探索任务间关系。人类在感知世界时会自然整合多个音频-视觉任务，因此需要一种统一框架来实现跨任务的联合学习。

Method: 1. 将所有任务的输入输出统一转换为离散标记序列，建立共享表示；2. 设计多尺度时间感知模块捕捉不同时间粒度的关键线索；3. 设计跨模态引导的空间感知模块，克服视觉域中缺乏听觉监督的问题；4. 使用任务特定文本提示增强模型适应性和任务感知能力。

Result: 在基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS和AVS）上的广泛实验证明了AV-Unified在时间、空间和时空任务上的有效性。

Conclusion: AV-Unified能够使用单一架构在异构数据集上联合训练，有效整合多个音频-视觉理解任务，为多模态场景理解提供了新的统一范式。
...
### [121] [Spatial Calibration of Diffuse LiDARs](https://arxiv.org/abs/2603.06531)
*Nikhil Behari,Ramesh Raskar*

Main category: cs.CV

TL;DR: 针对漫反射dToF LiDAR与RGB相机校准问题，提出一种空间校准程序，通过扫描回复反射patch估计每个LiDAR像素的footprint和空间灵敏度，建立跨模态对齐与融合的对应关系。


<details>
  <summary>Details</summary>
Motivation: 漫反射直接飞行时间（dToF）LiDAR通过聚合宽视场内的光子返回来报告每像素深度直方图，违反了标准LiDAR-RGB校准所基于的单射线假设，导致传统校准方法失效。

Method: 采用扫描回复反射patch结合背景减除的方法，恢复每个LiDAR像素的响应图，从而估计每个漫反射LiDAR像素的footprint（有效支撑区域）和RGB图像平面中的相对空间灵敏度。

Result: 在ams OSRAM TMF8828设备上验证了该方法的有效性，成功获得了每个LiDAR像素的footprint和相对空间灵敏度，建立了明确的LiDAR-to-RGB对应关系。

Conclusion: 所提出的空间校准方法能够有效建立漫反射dToF LiDAR与RGB相机之间的精确对应关系，为跨模态对齐与融合提供了可行方案。
...
TL;DR: 本研究聚焦于自动驾驶数据集中的冗余问题，通过在nuScenes和AV2数据集上建模多源相机和多模态数据的冗余，发现选择性移除冗余标签可提升YOLOv8目标检测性能，验证了冗余作为可操作数据质量因素的可行性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶研究主要关注算法设计，而忽视了数据质量分析。实际中由于环境条件和传感器限制，数据质量在不同来源和模态间存在差异。冗余作为一个基本但未被充分探索的数据质量问题，值得深入研究。

Method: 研究使用nuScenes和Argoverse 2数据集，对多源相机数据和多模态图像-LiDAR数据中的冗余进行建模和测量，并评估移除冗余标签对YOLOv8目标检测任务的影响。实验采用选择性移除策略，针对具有共享视野的相机进行冗余标签去除。

Result: 在nuScenes数据集上，选择性移除冗余多源图像对象标签后，三个代表性重叠区域的mAP50分别从0.66提升至0.70、0.64提升至0.67、0.53提升至0.55，其他重叠相机对的检测性能在更强剪枝下仍保持基线水平。在AV2数据集中，移除4.1%-8.6%的标签后mAP50维持在0.64基线附近。多模态分析也揭示了图像与LiDAR数据之间存在大量冗余。

Conclusion: 研究强调了冗余作为自动驾驶感知中可测量和可操作的数据质量因素的价值，并呼吁采用以数据为中心的视角来评估和改进自动驾驶数据集。研究表明，通过选择性移除冗余标签可以提升目标检测性能，nuScenes中三个代表性重叠区域的mAP50分别从0.66提升至0.70、0.64提升至0.67、0.53提升至0.55，AV2中移除4.1%-8.6%的标签后mAP50保持在0.64附近。代码和数据已开源。
...
TL;DR: EgoReasoner是一个针对以自我为中心的4D视频推理任务的两阶段框架，通过任务自适应思维模板和任务感知奖励函数，在仅16K样本训练下使3B参数模型在HD-EPIC基准上达到37.5%平均准确率，超越Qwen2.5-VL-7B超过10个百分点。


<details>
  <summary>Details</summary>
Motivation: 当前以自我为中心的视频理解由于环境动态4D特性而固有复杂，需要连续重新评估空间关系。现有方法存在不足：通用思维链方法缺乏任务适当的推理原语，统一强化学习会主动破坏空间任务性能。

Method: 提出两阶段框架EgoReasoner：第一阶段使用任务自适应思维模板引导结构化思维链痕迹合成，通过监督微调使模型跨任务类型自适应推理；第二阶段使用任务感知奖励函数验证实体接地、时间对齐和任务自适应逻辑一致性，通过GRPO强化微调选择性强化每条推理路径。

Result: 3B参数模型仅使用16K样本训练，在具有挑战性的HD-EPIC基准上实现37.5%平均准确率，比Qwen2.5-VL-7B（25.7%）高出超过10个百分点。

Conclusion: 该工作表明，针对任务认知结构对齐推理支架和奖励信号的方法，能够有效处理需要空间锚定、时间跟踪和持续时间推理的以自我为中心4D推理任务。
...
TL;DR: Penguin-VL挑战了VLM必须依赖对比预训练视觉编码器的传统做法，转而使用从文本-only LLM初始化的视觉编码器，在轻量级模型（2B和8B）上实现了与大型VLM相当的性能，并在文档理解、视觉知识等任务上超越后者。


<details>
  <summary>Details</summary>
Motivation: 当前VLM开发严重依赖模型缩放，阻碍了在智能手机和机器人等计算受限的移动和边缘设备上的部署。同时，现有的最先进VLM依赖通过大规模对比预训练初始化的视觉编码器，但对比学习优化的判别性目标与细粒度视觉理解之间存在目标不匹配，导致粗粒度和类别级不变性，压制了密集描述和复杂推理所需的细粒度视觉线索。

Method: 提出Penguin-VL，其视觉编码器从文本-only LLM初始化，而非使用传统的对比预训练（CLIP/SigLIP）。该方法旨在解决对比学习带来的"目标不匹配"问题——对比学习优化判别性，但抑制了密集描述和复杂VLM推理所需的细粒度视觉线索。

Result: Penguin-VL在数学推理方面与领先VLM（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务上超越它们。这些成果通过轻量级架构实现，表明改进的视觉表示而非模型缩放是性能提升的主要驱动因素。消融实验表明，Penguin-Encoder始终优于对比预训练编码器，能够保留对密集感知和复杂推理至关重要的细粒度空间和时间线索。

Conclusion: Penguin-VL通过使用文本-only LLM初始化的视觉编码器，为计算受限设备上的VLM部署提供了更优解决方案。该方法证明了改进的视觉表示而非模型缩放是提升性能的主要驱动因素，为资源受限环境中的高效多模态理解开辟了新途径。
...
TL;DR: Omni-Diffusion是首个基于mask-based离散 diffusion模型构建的any-to-any多模态语言模型，统一处理文本、语音和图像的理解与生成任务，在多模态基准测试中表现出色，证明了离散扩散模型作为多模态基础模型主干的可行性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型(MLLMs)主要采用传统的自回归架构作为主干，架构设计方面存在显著的改进空间；同时，离散扩散模型在视觉理解和图像生成等领域已展现出作为多模态系统主干的巨大潜力。

Method: 采用统一的mask-based离散扩散模型，直接捕捉离散多模态token的联合分布，支持双模态任务以及涉及多个模态的更复杂场景。

Result: 在多种基准测试中，本方法优于或持平于现有的处理两个或更多模态的多模态系统。

Conclusion: Omni-Diffusion是首个完全基于mask-based离散扩散模型的any-to-any多模态语言模型，在文本、语音和图像的统一理解与生成方面展现出显著潜力。该方法在多种基准测试中优于或持平于现有的双模态或多模态系统，证明了扩散模型在推动下一代多模态基础模型方面的巨大潜力。
...
TL;DR: 本文研究了多模态大型语言模型(MLLM)分类性能评估中的协议和标注质量问题。研究发现现有比较MLLM与监督模型的研究得出矛盾结论的根本原因在于评估协议存在缺陷(结果膨胀或低估)。作者识别并修复了三个关键问题：超出类别列表的输出被丢弃、弱干扰项导致结果膨胀、以及开放世界设置中因输出映射不当导致的性能低估。此外还量化了批大小、图像排序和文本编码器选择等常被忽视的设计选择的影响。在ReGT数据集(625个ImageNet-1k类别的多标签重标注)上的评估表明，MLLM从修正后的标签中获益最多(高达+10.8%)，显著缩小了与监督模型之间的感知差距。


<details>
  <summary>Details</summary>
Motivation: 当前研究在比较MLLM与监督模型和视觉-语言模型时得出了相互矛盾的结论，这些冲突源于评估协议存在缺陷，会导致性能被夸大或低估。

Method: 1)识别并修复常见评估协议中的关键问题：超出提供类别列表的模型输出被丢弃、弱多项选择干扰项导致结果膨胀、开放世界设置因输出映射不当而表现不佳；2)量化常被忽视的设计选择影响(批大小、图像排序、文本编码器选择)；3)在ReGT(625个ImageNet-1k类别的多标签重标注数据集)上进行评估；4)设计控制性案例研究测试MLLM辅助人工标注的潜力。

Result: MLLM从修正后的标签中获益最多，性能提升高达+10.8%，显著缩小了与监督模型的感知差距；大部分报告的MLLM性能不足实际上是噪声标注和评估协议缺陷的人为产物，而非真正的模型缺陷；不太依赖监督训练信号的模型对标注质量最敏感；案例研究中，标注员在约50%的困难案例中确认或整合了MLLM预测。

Conclusion: MLLM在分类任务上的大部分性能不足是噪声标注和评估协议缺陷的人为产物，而非真正的模型缺陷；MLLM具有辅助人工标注的潜力，可用于大规模数据集整理，在控制性案例研究中约50%的困难案例获得标注员确认或整合。
...
TL;DR: 本研究对EleutherAI/gpt-j-6B进行白盒分析，使用对比提示生成激活空间中的嵌入向量，探索LLM内部如何表征信任概念，发现其信任表征与人类信任理论模型（尤其是Castelfranchi社会认知模型）高度一致。


<details>
  <summary>Details</summary>
Motivation: 信任在促进有效合作、减少不确定性和指导决策方面起着关键作用，但在LLMs如何内部概念化和推理信任方面缺乏深入理解，需要白盒分析来揭示其内部表征机制。

Method: 采用对比提示（contrastive prompting）方法在LLM的激活空间内生成二元信任和相关人际关系属性的嵌入向量。首先从五个成熟的人类信任模型中识别信任相关概念，然后通过计算60个一般情感概念的成对余弦相似度来确定显著概念对齐的阈值，最后测量LLM内部信任表征与派生信任相关概念之间的余弦相似度。

Result: EleutherAI/gpt-j-6B的内部信任表征与Castelfranchi社会认知模型对齐度最高，其次是Marsh模型。这些发现表明LLM在其激活空间中编码了支持有意义比较分析的社会认知构念。

Conclusion: LLMs能够在其激活空间中编码社会认知构念，这为有意义的社会认知比较分析提供了可能性，并为人类-AI协作系统的设计提供了理论支持。EleutherAI/gpt-j-6B的内部信任表征与Castelfranchi社会认知模型最为接近，其次是Marsh模型。
...
### [128] [Recognizing Subgraphs of Regular Tilings](https://arxiv.org/abs/2603.06367)
*Eliel Ingervo,Sándor Kisfaludi-Bak*

Main category: cs.CG

TL;DR: The paper presents algorithms for recognizing subgraphs of {p,q}-tiling graphs across sphere, Euclidean, and hyperbolic geometries. The main result is a quasi-polynomial time algorithm for hyperbolic tilings, showing this case is computationally easier than Euclidean tilings (which are NP-hard for square grid) despite initial expectations.


<details>
  <summary>Details</summary>
Motivation: This paper addresses the problem of recognizing (induced) subgraphs of {p,q}-tiling graphs, which are planar graphs where all faces are cycles of length p and all vertices have degree q. The complexity of this problem varies depending on whether the tiling is on the sphere (finite), Euclidean plane, or hyperbolic plane. The authors aim to understand this computational problem across different geometric contexts.

Method: The authors develop algorithms with different complexities: (1) For spherical tilings (1/p+1/q>1/2), constant time recognition due to finiteness; (2) For Euclidean tilings (1/p+1/q=1/2), a divide-and-conquer algorithm achieving subexponential time, with NP-hardness for T_{4,4} already established; (3) For hyperbolic tilings (1/p+1/q<1/2), quasi-polynomial time n^{O(log n)} for fixed q. The key technical approach uses convex hulls within tiling graphs and dynamic programming with sphere cut decompositions.

Result: The main contribution is a quasi-polynomial time algorithm for subgraph isomorphism in hyperbolic tilings T_{p,q} for any fixed q. The paper shows hyperbolic case has significantly lower complexity than Euclidean case, and is unlikely to be NP-hard. For Euclidean tilings, subexponential time is achieved, with almost matching lower bound under ETH for T_{4,4}. Spherical tilings can be recognized in constant time.

Conclusion: The complexity of subgraph recognition in {p,q}-tiling graphs follows a clear hierarchy: constant time for spherical tilings, subexponential for Euclidean tilings, and quasi-polynomial for hyperbolic tilings. The key insight is that convex hulls in hyperbolic tilings have desirable properties enabling efficient dynamic programming via sphere cut decompositions, despite known treewidth bounds not being sufficient alone.
...
### [129] [Conversational Demand Response: Bidirectional Aggregator-Prosumer Coordination through Agentic AI](https://arxiv.org/abs/2603.06217)
*Reda El Makroum,Sebastian Zwickl-Bernhard,Lukas Kranzl,Hans Auer*

Main category: cs.AI

TL;DR: 本文提出会话式需求响应(CDR)，通过agentic AI实现聚合器与产消者之间的双向自然语言协调，开发两层多智能体架构，概念验证显示交互可在12秒内完成，兼具自动化DR的可扩展性与用户透明度。
...
Method: 开发了两层多智能体架构：聚合器智能体发送灵活 性请求，产消者家庭能源管理系统(HEMS)通过调用基于优化的工具评估可交付性和成本效益。实现了产消者发起的上游通信。

Result: 概念验证评估显示交互在12秒内完成，证明agentic AI可以弥合聚合器-产消者协调差距。

Conclusion: CDR通过agentic AI实现了聚合器与产消者之间的双向自然语言协调，在保持自动化DR可扩展性的同时，保留了透明度、可解释性和用户参与度。开源发布有助于后续研究和开发。
...
### [130] [RoboLayout: Differentiable 3D Scene Generation for Embodied Agents](https://arxiv.org/abs/2603.05522)
*Ali Shamsaddinlou*

Main category: cs.AI

TL;DR: RoboLayout 通过引入 agent 感知的可达性约束和局部细化机制，扩展了 LayoutVLM 框架，实现了可被具身 agents 导航和操作的室内场景布局生成，支持多种不同类型的 agents。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型(VLMs)在空间推理和3D场景布局生成方面展现出潜力，但生成既语义一致又适合具身 agents 交互的布局仍然具有挑战性，特别是在物理受限的室内环境中。现有的 LayoutVLM 框架缺乏 agent 感知的推理能力，无法确保生成的布局可被具身 agents 导航和操作。

Method: 提出 RoboLayout，作为 LayoutVLM 的扩展，包含两个关键技术贡献：1) 将显式可达性约束集成到可微分的布局优化过程中；2) 提出局部细化阶段，选择性地重新优化有问题的物体放置位置，同时保持场景其余部分不变。

Result: RoboLayout 能够生成可导航且可由具身 agents 操作的布局，支持多种不同物理能力的 agents（如服务机器人、仓库机器人、不同年龄段的人类或动物），并在各种场景配置中验证了其有效性。

Conclusion: RoboLayout 在保留 LayoutVLM 强大语义对齐和物理合理性的同时，增强了面向 agent 的室内场景生成适用性，提高了收敛效率而无需增加全局优化迭代次数。
...
### [131] [Talk Freely, Execute Strictly: Schema-Gated Agentic AI for Flexible and Reproducible Scientific Workflows](https://arxiv.org/abs/2603.06394)
...
TL;DR: 本文研究了大语言模型(LLM)在将研究人员自然语言目标转换为可执行计算时，如何满足科学工作流对确定性、溯源和治理的需求。通过对18位专家的访谈和20个系统的评估，提出"模式门控编排"(schema-gated orchestration)作为解决确定性执行与对话灵活性之间权衡的原则，并识别出三个操作原则。


<details>
  <summary>Details</summary>
Motivation: LLM能够将研究人员的自然语言目标转换为可执行计算，但科学工作流要求确定性、溯源和治理，而这些在LLM自主决定执行内容时难以保证。现有系统无法同时满足确定性约束执行和对话灵活性的需求。

Method: 1)对10个行业研发利益相关者的18位专家进行半结构化访谈；2)从验证范围维度评估20个跨5个架构组的系统；3)采用多模型协议——跨3个LLM家族的15个独立评分会话

Result: 1)提出模式门控编排作为解决原则：模式在组合工作流层面成为强制执行边界；2)经验性帕累托前沿——无系统同时实现高灵活性和高确定性；3)生成式与工作流中心极端之间出现收敛区；4)多模型评分协议显示substantial-to-near-perfect的模型间一致性(Krippendorff α=0.80 for ED, α=0.98 for CF)

Conclusion: 模式门控架构将对话权威与执行权威分离，能够解耦确定性-灵活性权衡。三个操作原则包括：执行前澄清、约束计划-动作编排、工具-工作流级门控。
...
### [132] [Reasoning Models Struggle to Control their Chains of Thought](https://arxiv.org/abs/2603.05706)
*Chen Yueh-Han,Robert McCarthy,Bruce W. Lee,He He,Ian Kivlichan,Bowen Baker,Micah Carroll,Tomek Korbak*

Main category: cs.AI

TL;DR: 本文研究大型语言模型对思维链（CoT）的可控性，发现模型的CoT可控性远低于输出可控性（如Claude Sonnet 4.5仅为2.7%），更大模型可控性更高，但RL训练、测试时计算和问题难度增加会降低可控性，目前CoT可控性不太可能构成对CoT监控的威胁。


<details>
  <summary>Details</summary>
Motivation: 思维链（CoT）监控是检测现代推理模型不当行为和理解其动机的有前景的工具，但如果模型能够控制其在CoT中表达的内容，可能会削弱CoT的可监控性。

Method: 引入CoT-Control评估套件，包含要求模型在遵守CoT指令的同时解决问题的任务（例如：在不使用"chromosome"一词的情况下推理遗传学问题）。通过比较CoT可控性与输出可控性，评估模型规模、RL训练、测试时计算和问题难度对CoT可控性的影响，以及激励和对抗性提示优化的效果。

Result: 推理模型的CoT可控性显著低于输出可控性，例如Claude Sonnet 4.5的CoT可控性仅为2.7%，但输出可控性达61.9%。更大的模型具有更高的CoT可控性；更多的RL训练、测试时计算和问题难度会降低CoT可控性。即使在模型被给予激励（而非直接请求）规避CoT监控的情况下，CoT可控性失败仍然存在，但当模型被告知正在被监控时表现出略高的可控性。对抗性提示优化也无法显著提高可控性。

Conclusion: 目前CoT可控性不太可能成为CoT可监控性的失败模式，但低可控性的机制尚未被深入理解。鉴于其对维护CoT可监控性的重要性，建议前沿实验室在未来模型中追踪CoT可控性。
...
### [133] [The World Won't Stay Still: Programmable Evolution for Agent Benchmarks](https://arxiv.org/abs/2603.05910)
...
TL;DR: 本文提出ProEvolve框架，通过图变换实现智能体环境的可控演化，将数据、工具和模式表示为类型化关系图，从而将环境变化表达为图变换操作，用于生成多样化的环境和任务沙箱。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试假设静态环境，忽略现实世界的演化性质和智能体对环境变化的鲁棒性。需要一种可扩展、可控的方式来演化智能体环境，以更好评估智能体对真实世界动态的适应性。

Method: 提出ProEvolve框架，核心是类型化关系图统一表示环境（数据、工具、模式）。在此形式化下，添加、删除或修改能力被表达为跨工具、模式和数据传播更新的图变换。在此基础上，通过图变换自动生成环境，并通过子图采样和编程实例化任务沙箱。

Result: 将单一环境演化为200个环境和3000个任务沙箱，并在此基础上对代表性智能体进行了基准测试。

Conclusion: ProEvolve能够以可编程、可控的方式实现智能体环境的演化，为评估智能体在动态环境中的适应性提供了有效的基准测试框架。
...
### [134] [An Interactive Multi-Agent System for Evaluation of New Product Concepts](https://arxiv.org/abs/2603.05980)
*Bin Xuan,Ruo Ai,Hakyeon Lee*

Main category: cs.AI

TL;DR: 本研究提出了一种基于大语言模型的多智能体系统（MAS）用于产品概念评估，通过8个虚拟智能体（涵盖研发、营销等专业领域）利用RAG和实时搜索工具收集客观证据进行结构化评估。案例研究显示该系统评估结果与资深专家一致。


<details>
  <summary>Details</summary>
Motivation: 传统专家主导的产品概念评估方法存在主观偏见、高时间成本等问题，企业需要更客观、高效的评估手段来支持战略资源配置和项目成功。

Method: 构建了基于LLM的八智能体MAS系统，智能体涵盖研发、营销等专业领域，采用RAG和实时搜索工具收集客观证据，通过结构化 deliberation进行概念验证，并使用专业产品评审数据进行微调。

Result: 专业显示器概念评估的案例研究表明，系统评估排名与资深行业专家排名一致，验证了系统的可用性。

Conclusion: 提出的多智能体评估方法可有效支持产品开发决策，为企业产品概念评估提供了一种可行的新工具。
...
### [135] [Agentic LLM Planning via Step-Wise PDDL Simulation: An Empirical Characterisation](https://arxiv.org/abs/2603.06064)
*Kai Göbel,Pierrick Lorang,Patrik Zips,Tobias Glück*

Main category: cs.AI

TL;DR: 本文提出PyPDDLEngine，一个开源PDDL模拟引擎，通过MCP接口将规划操作暴露为LLM工具调用。LLM作为交互式搜索策略，每次选择一个动作，观察结果状态，可重置重试。在102个IPC Blocksworld实例上评估四种方法，Fast Downward达85.3%成功率，直接LLM规划63.7%，智能体LLM规划66.7%，但 token 成本高5.7倍。


<details>
  <summary>Details</summary>
Motivation: 任务规划是自主机器人系统的核心能力，如何让大型语言模型(LLM)作为可行规划器与经典符号方法互补仍待探索。

Method: 提出PyPDDLEngine开源PDDL引擎，通过MCP接口将规划操作作为LLM工具调用。LLM作为交互式搜索策略，每次选择一个动作，观察结果状态，可重置重试，而非预先提交完整动作序列。

Result: Fast Downward lama-first达85.3%成功率；直接LLM规划(Claude Haiku 4.5)达63.7%；智能体LLM规划(通过PyPDDLEngine)达66.7%，比直接方法高3个百分点，但token成本高5.7倍。在大多数难度级别上，LLM方法产生比seq-sat-lama-2011更短的规划。

Conclusion: 智能体规划的收益取决于环境反馈性质。编码智能体受益于编译器错误等外部验证信号，而PDDL步骤反馈是自我评估的，智能体需自行评估进度而无外部验证。当前LLM规划成功率仍低于经典方法，但智能体方式略优于直接规划。
...
### [136] [Aggregative Semantics for Quantitative Bipolar Argumentation Frameworks](https://arxiv.org/abs/2603.06067)
*Yann Munro,Isabelle Bloch,Marie-Jeanne Lesot*

Main category: cs.AI

TL;DR: 本文提出了一种名为聚合语义(aggregative semantics)的新型渐进语义家族，用于定量双极论证框架(QBAF)。该方法将攻击者和支持者的聚合分开处理，通过三阶段计算来得出论证的可接受性。


<details>
  <summary>Details</summary>
Motivation: 在双极论证框架中，攻击者和支持者扮演不对称的角色，而现有模态语义未能充分体现这种不对称性。需要一种能够区分处理攻击者和支持者的新方法。

Method: 提出三阶段计算方法：首先分别计算攻击者和支持者的全局权重，然后与论证的内在权重进行聚合。通过讨论三个聚合函数应满足的性质及其与经典渐进语义原则的关系来支撑该方法，并使用500个聚合语义进行测试比较。

Result: 该方法保持了双极性，产生了更易理解的渐进语义，并展示了聚合语义可能出现的各种行为特征。

Conclusion: 将计算分解为三个独立且可解释的步骤可以实现更多参数化设计，比现有文献中的方法更能体现双极性，为论证推理提供了更灵活且可解释的框架。
...
### [137] [Offline Materials Optimization with CliqueFlowmer](https://arxiv.org/abs/2603.06082)
*Jakub Grudzien Kuba,Benjamin Kurt Miller,Sergey Levine,Pieter Abbeel*

Main category: cs.AI

TL;DR: 本文提出CliqueFlowmer，一种结合离线模型优化与transformer/flow生成的材料发现新方法，在优化目标材料属性方面显著优于现有生成基线。


<details>
  <summary>Details</summary>
Motivation: 深度学习驱动的计算材料发现(CMD)领域虽然取得进展，但现有生成建模方法由于最大似然训练的局限性，难以大胆探索材料空间的有吸引力的区域。

Method: 提出CliqueFlowmer模型，融合团块(clique)基于的离线模型优化(MBO)与transformer和flow生成技术，将目标材料属性的直接优化嵌入到生成过程中。

Result: CliqueFlowmer生成的材料性能显著优于生成基线方法提供的产品。

Conclusion: CliqueFlowmer通过将基于团块的离线模型优化与transformer和flow生成相结合，在材料优化任务中显著优于生成基线方法，为计算材料发现提供了一种新的有效工具。
...
### [138] [Artificial Intelligence for Climate Adaptation: Reinforcement Learning for Climate Change-Resilient Transport](https://arxiv.org/abs/2603.06278)
...
TL;DR: 本文提出了一种基于强化学习的新型决策支持框架，用于城市交通系统在气候变化下的长期洪涝适应规划。该框架整合了降雨预测、洪水建模、交通模拟和影响量化，通过RL学习自适应策略来平衡投资成本与避免的损失。哥本哈根案例研究表明该方法优于传统优化方法。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧降雨和内涝，对城市交通系统造成 disruptions。传统规划方法难以处理长期、顺序性的基础设施投资决策、深层气候不确定性以及洪水、基础设施和交通之间的复杂交互作用。

Method: 提出了一种集成评估模型(IAM)框架，结合降雨预测、洪水建模、交通模拟，量化基础设施和交通的直接和间接影响。采用强化学习(RL)方法学习自适应策略，平衡投资维护成本与避免的损失。

Result: 在哥本哈根内城2024-2100年的案例研究中，该框架通过发现协调的时空适应路径，学习影响减少与适应投资之间的权衡，优于传统优化方法，产生更具弹性的策略。

Conclusion: 强化学习作为灵活决策支持工具在气候不确定性下进行自适应基础设施规划具有潜力。
...
### [139] [SAHOO: Safeguarded Alignment for High-Order Optimization Objectives in Recursive Self-Improvement](https://arxiv.org/abs/2603.06333)
*Subramanyam Sahoo,Aman Chadha,Vinija Jain,Divya Chaudhary*

Main category: cs.AI

TL;DR: SAHOO是一个通过三道防护措施监控和控制递归自我改进中对齐漂移的实用框架，在代码生成、数学推理和真实性任务上实现了显著的质量提升，同时保持低违规率。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进正在从理论走向实践，现代系统可以批判、修订和评估自己的输出，但迭代自我修改存在细微的对齐漂移风险，需要有效的监控和控制机制。

Method: SAHOO框架包含三个防护措施：(i)目标漂移指数(GDI)，一种结合语义、词汇、结构和分布测量的多信号检测器；(ii)约束保存检查，执行句法正确性和非幻觉等安全关键不变式；(iii)回归风险量化，标记可能撤销先前收益的改进周期。

Result: 在189个代码生成、数学推理和真实性任务中，SAHOO在代码任务中提升18.3%，推理任务中提升16.8%，同时在两个领域保持约束，在真实性领域维持低违规率。

Conclusion: SAHOO使递归自我改进中的对齐保护变得可测量、可部署并在规模上得到系统验证，揭示了能力-对齐前沿的动态：早期改进周期高效但后期对齐成本上升，存在领域特定张力如流畅性与事实性之间的权衡。
...
### [140] [Boosting deep Reinforcement Learning using pretraining with Logical Options](https://arxiv.org/abs/2603.06565)
*Zihan Ye,Phil Chau,Raban Emunds,Jannis Blüml,Cedric Derstroff,Quentin Delfosse,Oleg Arenz,Kristian Kersting*

Main category: cs.AI

TL;DR: 提出H^2RL框架，通过逻辑选项预训练将符号结构注入神经强化学习，解决智能体过度开发早期奖励信号的问题，在长期决策任务中优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 深度强化学习智能体经常出现对齐问题，过度开发早期奖励信号。纯符号化方法虽然可以解决这些挑战，但难以扩展并难以应用于连续环境。

Method: 提出了一种名为混合层次强化学习（H^2RL）的两阶段框架，将符号结构注入到基于神经的强化学习智能体中，使用逻辑选项预训练策略来引导学习策略远离短期奖励循环，趋向目标导向行为。

Result: 该方法持续改善了长期决策能力，产生的智能体优于强大的神经、符号和神经符号基线方法。

Conclusion: H^2RL成功结合了符号化和神经方法的优点，比现有的神经、符号和神经符号方法表现更好。
