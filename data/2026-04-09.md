...
TL;DR: 将陨石坑分析重新定义为实例级图像检索任务，提出CraterBench-R基准数据集（约25,000个陨石坑身份），验证自监督Vision Transformers在域内预训练后的优越性，并提出实例令牌聚合的可扩展无训练方法以平衡精度与存储效率。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习 pipelines 将陨石坑仅视为检测问题，但科学工作流程（如目录去重、跨观测匹配、形态模拟发现）本质上需要检索能力，现有方法未能满足这一需求。

Method: 1) 构建 CraterBench-R 基准数据集，包含多尺度图库视图和手动验证查询；2) 评估多种架构性能；3) 提出实例令牌聚合方法：选择 K 个种子令牌，通过余弦相似度分配剩余令牌聚类，并聚合为单一代表令牌；4) 设计两阶段管道：单向量预筛选 + 实例令牌重排序。

Result: 1) 自监督 ViT（尤其域内预训练）显著优于参数量更多的通用模型；2) 多令牌晚期交互匹配优于标准单向量池化；3) K=16 时聚合提升 mAP 17.9 点，K=64 时达到与全部 196 令牌相当的精度；4) 两阶段管道在仅搜索小候选集情况下恢复 89-94% 的完整精度。

Conclusion: 实例级检索是陨石坑分析的关键范式，域内预训练 ViT + 多令牌晚期交互 + 实例令牌聚合 + 两阶段管道为大规模行星表面分析提供了实用高效解决方案。
...
TL;DR: 提出一种无需参考重建的全自动CBCT迭代重建参数优化框架，采用改进的crow search算法和混沌初始化，在3台设备、4个数据集、3种重建算法上验证，效果优于手动设置和标准CSA。


<details>
  <summary>Details</summary>
Motivation: 迭代重建技术可通过减少投影数来降低辐射剂量，但需要手动调整多个超参数，工作耗时且增加操作人员负担。现有方法缺乏有效的自动参数优化机制。

Method: 提出一种基于改进crow search算法(CSA)的全自动化参数优化框架。该方法包含：1）基于集合的局部搜索机制；2）感知搜索空间的全局搜索策略；3）局部与全局搜索的目标驱动平衡；4）采用混沌对角线性均匀初始化方案来加速算法收敛。可应用于多种CBCT迭代重建算法。

Result: 所提方法在三个不同迭代重建算法（参数最多、最具挑战性的场景）上表现优于手动设置和标准CSA，平均适应度提升4.19%，CHILL@UK和RPI_AXIS指标分别提升4.89%和3.82%，定性结果清晰显示其保持细节的优势。

Conclusion: 提出的CSA优化框架在3台成像设备和4个真实数据集上进行了评估，相比手动设置和标准CSA，在平均适应度上提升4.19%，在CHILL@UK和RPI_AXIS两个无参考质量指标上分别提升4.89%和3.82%。该框架对不同迭代重建算法均表现有效且鲁棒，能够在无需参考重建的情况下自动优化参数。
...
TL;DR: DISSECT基准测试揭示了视觉-语言模型在化学和生物学领域中的感知-集成差距：模型能正确识别分子图像但推理失败，单配置基准测试将感知与集成混为一谈导致问题被隐藏。


<details>
  <summary>Details</summary>
Motivation: 现有VLM基准测试将视觉感知和下游推理混为一个准确率数字，无法揭示视觉信息提取成功但推理失败的问题。化学和生物学领域需要真正测试视觉推理能力。

Method: 引入DISSECT基准测试，包含12,000个问题（化学7,000+生物学5,000），每题在五种输入模式下评估：视觉+文本、纯文本、纯视觉、人工Oracle和新型Model Oracle（模型先描述图像再推理）。

Result: 评估18个VLM发现：(1)化学的语言先验利用率显著低于生物学；(2)开源模型用自身描述推理比直接看图得分更高，暴露集成瓶颈；(3)闭源模型无此差距，表明感知-集成是开源与闭源多模态能力的分界线。

Conclusion: Model Oracle协议可模型和基准无关地诊断集成失败；化学的分子视觉内容是更难的视觉推理测试；开源模型存在系统性集成瓶颈，需改进视觉信息与推理的集成能力。
...
TL;DR: 本综述系统回顾了视频生成技术的发展历程，从早期GAN到扩散模型，再到新兴的自回归(AR)模型和多模态技术，涵盖OpenAI Sora、Google Veo3、字节Seedance、开源Wan和HunyuanVideo等代表性系统，旨在为构建"世界模型"提供技术演进视角和应用前景分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成综述存在局限性：1) 多聚焦于GAN或扩散模型等单一技术路线；2) 缺乏对自回归模型和多模态融合的系统性讨论；3) 未能覆盖最新AIGC突破性进展。因此需要一篇全面整合历史演进与当代创新的综述。

Method: 采用系统性文献回顾方法，追溯视频生成技术从GAN到扩散模型的技术演进轨迹，深入分析AR模型和多模态融合的技术原理与优势，并通过对比分析揭示各技术路线的适用场景与局限性。

Result: 梳理了视频生成技术的完整发展脉络，识别出GAN→扩散模型→AR模型的技术演进阶段；总结了多模态信息融合在提升上下文理解能力方面的关键作用；展望了虚拟/增强现实、个性化教育、自动驾驶模拟、数字娱乐及世界模型构建等应用前景。

Conclusion: 本综述填补了现有研究在技术全面性和时效性上的空白，为未来视频生成研究提供了从历史演进到前沿趋势的系统性参考框架，对推动AIGC在多领域应用具有重要指导意义。
...
TL;DR: 提出一种基于前后对比图像的食物营养分析视觉语言框架，利用自然语言提示定位食品项目并估计重量，通过两阶段训练策略预测食物消耗量，在三个公开数据集上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前图像营养评估方法存在局限：仅依赖餐前单一图像、只能提供粗糙的餐级估计、无法确定实际消耗量，且通常需要深度传感、多视角或显式分割等限制性输入。

Method: 提出使用配对的餐前餐后图像进行食物项目级营养分析。不同于依赖刚性分割掩码，该方法利用自然语言提示从单张RGB图像中定位特定食物项目并直接估计重量。采用两阶段训练策略预测配对图像间的重量差异来估算食物消耗量。

Result: 在三个公开数据集上评估，一致性地超越了现有方法，为前后对比饮食图像分析建立了强有力的基线。

Conclusion: 该研究成功解决了先前方法的局限，仅需简单RGB图像和自然语言提示即可实现食物项目级别的营养分析，为精准营养评估提供了更实用有效的解决方案。
...
### [6] [MTA-Agent: An Open Recipe for Multimodal Deep Search Agents](https://arxiv.org/abs/2604.06376)
*Xiangyu Peng,Can Qin,An Yan,Xinyi Yang,Zeyuan Chen,Ran Xu,Chien-Sheng Wu*

Main category: cs.CV

TL;DR: 本文提出MTA-Agent，一个多模态深度搜索智能体，可自动选择工具及其参数从视觉和文本来源检索和验证证据，生成结构化的多跳问答轨迹。构建了包含21K高质量多跳示例的MTA-Vision-DeepSearch数据集，在六个挑战性基准上达到54.63%的平均准确率，超越GPT-5、Gemini-2.5-Pro和Gemini-3-Pro。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)在视觉理解方面表现强大，但在复杂多步推理（需要深度搜索并将视觉证据与外部知识结合）方面仍存在局限。

Method: 1）从多样化VQA种子数据集出发，构建多跳工具增强智能体MTA-Agent；2）自动选择工具及其参数从视觉和文本来源检索和验证证据，生成结构化多跳问答轨迹；3）通过多阶段验证流程确保事实一致性和答案唯一性；4）通过回放缓存交互进行训练，显著降低训练成本。

Result: 32B开源多模态搜索智能体在六个挑战性基准上达到54.63%的平均准确率，超越GPT-5（51.86%）、Gemini-2.5-Pro（50.98%）和Gemini-3-Pro（54.46%）。训练后平均推理步骤从2.27增加到4.28，搜索策略更加系统和持久。

Conclusion: MTA-Agent是一个完全开放的多模态深度搜索方案，发布了完整数据集、训练轨迹和实现细节，为开放多模态搜索智能体的可复现性和未来研究奠定基础。
...
TL;DR: PhysHead一种基于3D Gaussian混合表示的可动画头部avatar系统，能够实现真实的头发动态效果（如风吹运动），解决现有方法中头发刚性运动的问题。


<details>
  <summary>Details</summary>
Motivation: 现有头部avatar方法假设刚性头发运动，无法将头发与头部 disentangle，简单的外壳表示无法捕捉头发的自然体积行为。

Method: 提出3D Gaussian-based分层表示：结合3D参数化网格（头部）与strand-based头发（物理引擎模拟），使用附着在网格和头发段上的Gaussian primitives进行外观建模。提出VLM模型生成训练序列中被遮挡区域的外观。

Result: 实现具有动态头发行为（风吹运动等）的真实感头部avatar，定量和定性实验表明该方法优于现有基线方法。

Conclusion: 该方法能够合成物理上可信的头发运动，同时支持表情和相机控制，为数字人avatar的逼真度提升提供了有效解决方案。
...
TL;DR: 本文提出了一种基于提升理论的单目深度估计方法LiftFormer，通过构建深度定向几何表示(DGR)子空间和边缘感知表示(ER)子空间，将深度值预测问题转化为几何表示学习，解决了从颜色到深度映射的不适定问题，并在多个数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计(MDE)是从单目图像/视频中估计深度图以表示场景的3D结构，这是一个高度不适定问题。现有方法在处理从颜色值到几何深度值的学习过程中存在挑战，且边缘区域通常呈现深度图的剧烈变化，容易被错误预测。

Method: 提出了基于提升理论拓扑结构的LiftFormer，构建了深度定向几何表示(DGR)子空间和边缘感知表示(ER)子空间。DGR子空间利用深度箱的线性相关向量提供冗余且鲁棒的表示，将图像空间特征转换到DGR子空间中，使其直接对应深度值。ER子空间则对深度特征进行转换以增强边缘周围的局部特征。

Result: LiftFormer在广泛使用的数据集上实现了最先进的性能。

Conclusion: 实验结果表明，LiftFormer在多个广泛使用的数据集上实现了最先进的性能，消融实验验证了所提出的两个提升模块的有效性。
...
TL;DR: VAMAE是一种针对OCTA图像的自监督预训练框架，通过解剖学引导的掩码策略和多目标重建来学习血管表征。该方法在OCTA-500数据集上，特别是在有限标注场景下，显著优于标准掩码自编码器基线方法。


<details>
  <summary>Details</summary>
Motivation: OCTA成像虽然能无创显示视网膜微血管，但学习鲁棒表征面临挑战：血管结构稀疏、拓扑约束强。现有自监督学习方法（如掩码自编码器）主要针对密集自然图像设计，采用均匀掩码和像素级重建，难以有效捕获血管几何特性。

Method: 提出VAMAE框架，包含两个核心创新：1）解剖学引导的掩码策略，利用血管ness和骨架线索强调血管丰富区域，迫使模型关注血管连通性和分支模式；2）多目标重建预训练目标，同时重建外观、结构、拓扑等多维互补信息。

Result: 在OCTA-500基准数据集上针对多种血管分割任务进行评估。结果表明，血管感知掩码和多目标重建相比标准掩码自编码基线方法带来一致性能提升，尤其在有限标注设置下提升更为显著。

Conclusion: 验证了几何感知的自监督学习方法在OCTA分析中的潜力，为医学图像自监督学习提供了新的思路，即结合领域知识（解剖结构先验）可有效提升表征学习效果。
...
TL;DR: 提出Holistic Optimal Label Selection (HopS)方法，通过局部密度过滤器和全局最优传输目标，从近邻候选集中选择最可能的标签，提升部分标签环境下prompt学习的性能。


<details>
  <summary>Details</summary>
Motivation: 当前prompt学习在适配大型预训练视觉-语言模型时，仅在部分标签可用的情况下性能受限，主要原因是标签歧义和监督信息不足。

Method: 设计了两个互补策略：1）局部密度过滤器：从近邻候选集中选择最高频标签，结合softmax分数识别最可信标签；2）全局最优传输选择目标：将均匀采样分布映射到批次级候选标签分布，通过最小化期望传输成本确定最可能的标签分配。

Result: 在8个基准数据集上的广泛实验表明，HopS在部分监督设置下持续提升性能，且优于所有基线方法。

Conclusion: 研究验证了整体标签选择策略的价值，为弱监督环境下的prompt学习提供了实用解决方案。
...
TL;DR: 本文提出WeatherRemover模型，一种基于UNet结构和多尺度金字塔视觉Transformer的去天气图像恢复方法，通过通道注意力机制和门控机制实现高效的多天气场景图像增强。


<details>
  <summary>Details</summary>
Motivation: 现有图像去天气方法主要针对特定天气条件，且忽略性能考量，导致模型参数过大、推理时间长、内存开销高，难以满足实际应用需求。

Method: 提出WeatherRemover模型，采用UNet-like结构结合门控机制和多尺度金字塔视觉Transformer；引入CNN驱动的通道注意力优化特征提取；使用线性空间降维降低注意力计算成本；在前馈和下采样阶段部署门控机制，选择性处理信息冗余。

Result: 该轻量级模型在恢复质量、参数效率、计算开销和内存使用之间取得良好平衡，可处理多种天气场景。

Conclusion: WeatherRemover模型能够有效应对实际应用需求，为多天气条件下的图像恢复提供了高效且实用的解决方案。
...
TL;DR: 提出一种特征提取框架，通过可变分潜在瓶颈和动态二元掩码抑制跨模型迁移，同时保持目标分类器准确性。在CIFAR-100上实现未授权模型准确率<2%，抑制比>45倍。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习推理在共享和云环境中的部署增加，输入再利用问题日益突出。现有隐私防御主要关注限制数据访问，但对释放的表示能支持的下游用途缺乏控制。需要一种方法在保持指定分类器准确性的同时，抑制跨模型转移。

Method: 该框架采用可变分潜在瓶颈训练，使用任务驱动的交叉熵目标和KL正则化，不使用像素级重建损失将输入编码到紧凑潜在空间。动态二元掩码基于每维KL散度和关于冻结目标模型的梯度显著性计算，抑制对目标任务无信息的潜在维度。编码器在白盒环境下训练，推理时只需对冻结目标模型进行前向传播。

Result: 在CIFAR-100上，处理后的表示在保持指定分类器强实用性的同时，将所有未授权分类器的准确率降至2%以下，抑制比超过45倍。在CIFAR-10、Tiny ImageNet和Pascal VOC上的初步实验显示该方法可扩展到不同任务设置。

Conclusion: 该框架通过可变分潜在瓶颈和动态二元掩码技术，有效抑制了跨模型迁移，同时保持目标分类器的准确性。在CIFAR-100上，未授权模型的准确率降至2%以下，抑制比超过45倍。该方法为深度学习模型在共享和云环境中的隐私保护提供了新思路，但仍需进一步评估对自适应攻击的鲁棒性。
...
TL;DR: 提出了一种可控生成视频压缩(CGVC)范式，通过关键帧的结构先验和每帧的密集控制先验来在保持感知真实性的同时恢复信号保真度，解决感知压缩中感知与保真之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 感知视频压缩使用生成模型提高感知真实性但牺牲了信号保真度，违背了视频压缩忠实再现视觉信号的目标。需要解决感知与保真之间的困境。

Method: 1) 利用关键帧提供结构先验用于非关键帧生成；2) 编码每帧的密集控制先验以更好保留细节和语义；3) 通过可控视频生成模型进行具有时间一致性和内容一致性的非关键帧重建；4) 开发颜色距离引导的关键帧选择算法来准确恢复色彩信息。

Result: CGVC在信号保真度和感知质量方面均优于之前的感知视频压缩方法。

Conclusion: 提出的CGVC范式成功实现了感知与保真度的平衡，通过关键帧结构先验和每帧控制先验的结合，以及颜色距离引导的关键帧选择，在视频压缩任务中实现了同时提升感知质量和信号保真度的目标。
...
TL;DR: 针对文本到图像扩散模型，提出了一种名为实例特定双边水印（ISTS）的新水印方案，通过动态控制水印注入时间和模式来抵抗去除攻击和伪造攻击。


<details>
  <summary>Details</summary>
Motivation: 生成内容引发版权保护、图像来源和署名归因的严重担忧，现有水印技术容易受到去除攻击和伪造攻击的威胁。

Method: 提出ISTS水印方案，包含：1）基于用户提示语义动态控制水印注入时间和模式；2）新的双边检测方法增强水印检测鲁棒性。

Result: 实验表明该水印方案在抵抗去除攻击和伪造攻击方面具有优越性。

Conclusion: ISTS水印范式能有效解决扩散模型水印面临的对抗性攻击问题，为AI生成图像的版权保护提供了新的技术途径。
...
TL;DR: VDPP是一种视频深度后处理框架，通过在低分辨率空间中进行纯几何精炼而非计算密集型的场景重建，实现了超过43.5 FPS的实时推理速度，同时保持与端到端系统相当的时间一致性。其RGB无关架构确保可立即集成任何新出现的单图像深度估计器，解决了现有方法在速度、准确性和可扩展性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前端到端视频深度模型作为紧耦合系统，当更优的单图像深度估计器发布时会遭受显著的适应延迟。现有的后处理方法如NVDS虽然提供模块化即插即用的替代方案，但仍受限于速度、准确性和RGB依赖性，难以匹配端到端系统的效率和实用性。

Method: VDPP通过将范式从计算密集型的场景重建转变为目标导向的几何细化，纯几何精炼在低分辨率空间中进行，使用密集残差学习驱动几何表示而非完整重建。

Result: VDPP实现了超过43.5 FPS的推理速度（在NVIDIA Jetson Orin Nano上），同时匹配端到端系统的时间一致性，在速度、准确性和内存效率之间提供了最优平衡。

Conclusion: VDPP在NVIDIA Jetson Orin Nano上实现了43.5 FPS以上的推理速度，在速度、准确性和内存效率之间取得了最佳平衡，是实时边缘部署最实用的解决方案。其RGB无关架构确保了真正的可扩展性，可以立即集成任何新出现的图像深度模型。
...
TL;DR: 本文提出检索增强语义推理(RASR)框架，用于多模态假新闻视频检测，通过跨实例语义解析器、领域引导多模态推理和多视角特征解耦融合模块，有效利用历史关联证据并结合领域专家知识，提升跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要局限：(1)缺乏跨实例全局语义关联，难以有效利用历史关联证据验证当前视频；(2)跨领域语义差异阻碍了通用知识迁移，缺乏领域特定专家知识的指导。

Method: 提出RASR框架包含三个核心模块：(1)跨实例语义解析与检索器(CSPR)将视频解构为高级语义原语并从动态记忆库中检索相关关联证据；(2)领域引导多模态推理(DGMP)模块结合领域先验知识，驱动专家多模态大语言模型生成领域感知的深度分析报告；(3)多视角特征解耦与融合(MVDFF)模块通过自适应门控机制整合多维特征，实现鲁棒的真实性判断。

Result: 在FakeSV和FakeTT数据集上的广泛实验表明，RASR显著优于现有最佳基线方法，实现了 superior cross-domain generalization，并提升了最高0.93%的整体检测准确率。

Conclusion: RASR框架通过结合检索增强和领域知识引导的方式，有效解决了假新闻视频检测中的跨实例语义关联和跨域泛化问题，为多模态虚假信息检测提供了新的研究思路。
...
TL;DR: 针对半密集图像匹配中MNN过度排除和局部一致性问题，提出尺度感知匹配模块和级联流精炼方案，显著提升匹配鲁棒性和精度。


<details>
  <summary>Details</summary>
Motivation: 两个长期问题困扰半密集图像匹配方法：1) 粗阶段：互最近邻(MNN)匹配层的过度排除问题，难以处理图像间的尺度差异；2) 细阶段：忽视最终匹配的局部一致性，降低鲁棒性。

Method: 提出尺度感知匹配模块，利用分数矩阵中的提示信息指示尺度比例；在细阶段将任务重构为级联流精炼问题，引入梯度损失来鼓励流场的局部一致性。

Result: 实验表明，新颖的匹配流程在下游任务上实现了鲁棒且准确的匹配性能。

Conclusion: 所提出的修改方案有效解决了尺度差异和局部一致性问题，显著提升了半密集图像匹配的鲁棒性和精度。
...
TL;DR: 本文提出了一种无需训练的视觉思维链框架来解决多模态大语言模型在3D空间推理方面的局限性。该方法通过MLLM引导从单图像重建高保真3D网格，并利用外部知识库迭代计算最优相机参数合成新视图，模拟人类视角获取。实验表明该方法在3DSRBench和Rel3D等基准测试中超越了GPT-5.2和Gemini-2.5-Flash等先进模型。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)在复杂3D空间推理方面存在困难，主要因为它们依赖2D视觉先验。现有的解决方案要么需要计算昂贵的3D数据集后训练，要么缺乏明确的几何理解和视角灵活性。

Method: 提出了一个无需训练的框架，引入基于显式3D重建的视觉思维链机制。具体包括：(1)使用MLLM引导的关键词提取和多粒度掩码生成，从单图像重建高保真3D网格；(2)利用外部知识库迭代计算最优相机外参并合成新视图，模拟人类视角获取。

Result: 在3DSRBench和Rel3D等主要基准测试中，该框架显著优于专用空间模型和通用MLLMs（包括GPT-5.2和Gemini-2.5-Flash）。

Conclusion: 该研究通过将2D视觉先验转换为显式3D重建，无需额外训练即可显著增强模型的空间理解能力，为多模态模型的3D推理提供了新的有效途径。
...
TL;DR: URMF是一种不确定性感知的鲁棒多模态融合框架，通过显式建模模态可靠性来解决多模态 sarcasm 检测中模态不可靠问题，使用交叉/自注意力融合和Gaussian后验不确定性建模动态调节模态贡献，并在公开基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设所有模态同等可靠，但实际社交媒体中文本可能模糊、视觉内容可能弱相关甚至无关，导致确定性融合引入噪声证据，削弱鲁棒推理能力。

Method: 提出统一框架URMF，包含：1）多头交叉注意力将视觉信息注入文本表示；2）多头自注意力在融合语义空间增强不一致性推理；3）将每种模态参数化为可学习的Gaussian后验进行统一不确定性建模；4）利用估计的不确定性动态调节模态贡献；5）设计联合训练目标：任务监督、模态先验正则、跨模态分布对齐、不确定性驱动的自采样对比学习。

Result: 在公开MSD基准上一致优于强单模态、多模态和MLLM基线，验证了不确定性感知融合对提升准确性和鲁棒性的有效性。

Conclusion: URMF通过不确定性感知融合机制有效解决了多模态 sarcasm 检测中模态不可靠的问题，实验表明该方法在公开基准上显著优于单模态、多模态和基于MLLM的强基线模型，同时提升了准确性和鲁棒性。
...
TL;DR: LiveStre4m是一种用于从无姿态多视角视频进行实时新视角合成的 feed-forward 模型，能够在仅使用两个同步输入流的情况下实现时间一致的新视角视频流式传输。


<details>
  <summary>Details</summary>
Motivation: 现有的动态场景表示方法通常需要地面真实相机参数（ground-truth camera parameters），并且涉及耗时的优化过程（约2.67秒），这使得它们不适合直播场景中的应用。

Method: 提出三个关键模块：1) 多视角视觉Transformer（Multi-view Vision Transformer）用于关键帧3D场景重建；2) Diffusion-Transformer插值模块确保时间一致性和稳定流式传输；3) 相机姿态预测器模块（Camera Pose Predictor）直接从RGB图像估计姿态和内参，消除对已知相机校准信息的依赖。

Result: 在1024×768分辨率下达到平均0.07秒/帧的重建速度，比基于优化的动态场景表示方法在运行时间上快几个数量级。

Conclusion: LiveStre4m使得实时NVS流式传输在实际应用中变得可行，代表了可部署的实时新视角合成系统的重要进步。
...
TL;DR: 本文提出Interactive DeLVM方法，将静态的视觉上下文学习模型转换为可交互的用户驱动系统，通过将交互信号（涂鸦、点击、边界框）编码到示例输入-输出对中，实现用户对模型预测的动态引导。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉上下文学习模型虽然可以适应新任务，但缺乏用户交互机制，无法接收用户的指导信号如涂鸦、点击或边界框来调整预测，这在实际应用中受到限制，因为用户通常需要主动引导模型预测。

Method: 将交互信号直接编码到示例输入-输出对中，保持视觉上下文学习的理念不变，使用户能够通过未见过的交互提示模型，无需微调即可动态引导模型预测。

Result: 实验表明，最先进的视觉上下文学习模型无法有效利用交互信号，而本文方法在可控的用户引导场景中表现出色：交互分割IoU提升+7.95%，定向超分辨率PSNR提升+2.46，交互对象去除LPIPS降低-3.14%。

Conclusion: 本文工作弥合了僵化的静态任务适应与灵活的用于以用户为中心的视觉上下文学习之间的差距，使视觉上下文学习模型具备用户交互能力。
...
TL;DR: VENUSS是一个用于系统分析VLM在连续驾驶场景中性能的框架，通过对25个以上模型在2600多个场景中的评估，发现顶级模型准确率仅57%，低于人类基准的65%，揭示了VLM在车辆动态理解和时序关系推理方面的显著能力差距。


<details>
  <summary>Details</summary>
Motivation: 当前VLMs在自动驾驶任务中的应用日益增多，但其在连续驾驶场景中的性能表现缺乏系统性的表征，特别是输入配置如何影响其能力尚不明确。

Method: VENUSS框架从驾驶视频中提取时间序列，并在自定义类别上进行结构化评估。研究对比了25个以上现有VLM模型，涵盖2600多个场景，分析了输入图像配置（分辨率、帧数、时间间隔、空间布局和呈现模式）对模型性能的影响。

Result: 即使是表现最佳的模型也仅达到57%的准确率，无法匹配人类在类似约束条件下的表现（65%）。分析表明VLMs在静态目标检测方面表现良好，但在理解车辆动态和时序关系方面存在明显不足。

Conclusion: VENUSS是首个系统性地分析VLMs在连续驾驶场景中表现的框架，揭示了当前VLMs与人类驾驶理解能力之间存在显著差距。该研究为未来VLM在自动驾驶领域的研究提供了重要的基线和方向指引。
...
TL;DR: FlowExtract是一个从ISO 5807标准化流程图中提取有向图的管道系统，分离元素检测与连接性重建，使用YOLOv8和EasyOCR进行节点检测和文本提取，并提出分析箭头方向和反向追踪连接线的新颖边缘检测方法。


<details>
  <summary>Details</summary>
Motivation: 制造业设施的维护程序通常以静态PDF或扫描图像中的流程图形式记录，其中包含对资产生命周期管理至关重要的程序性知识，但这些知识无法被现代操作支持系统访问。 vision-language models难以从此类图表中重建连接拓扑。

Method: 该系统将元素检测与连接性重建分离，使用YOLOv8和EasyOCR进行标准域对齐的节点检测和文本提取，并结合一种新颖的边缘检测方法，该方法分析箭头方向并反向追踪连接线到源节点。

Result: 在工业故障排除指南上评估，FlowExtract实现了非常高的节点检测性能，并在边缘提取方面显著优于vision-language model基线，为组织提供了通往可查询程序性知识表示的实际路径。

Conclusion: FlowExtract提供了一种实用的方法来从静态流程图中提取结构化知识，能够实现程序性知识的查询和利用，性能优于现有的vision-language模型方法。
...
### [26] [Walk the Talk: Bridging the Reasoning-Action Gap for Thinking with Images via Multimodal Agentic Policy Optimization](https://arxiv.org/abs/2604.06777)
...
TL;DR: MAPO通过要求模型生成视觉工具获取内容的显式文本描述，并结合语义对齐与任务奖励进行优势估计，解决多模态大语言模型中文本推理与视觉动作执行不一致导致的训练崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法仅依赖结果奖励，忽略了文本推理与视觉动作执行之间的差异。模型可能产生看似合理但实际执行错误或不相关的视觉动作，这种推理-动作不一致会在多轮推理中累积噪声，严重损害模型的多模态推理能力甚至导致训练崩溃。

Method: 提出多模态智能体策略优化(MAPO)：1)强制模型生成视觉工具获取内容的显式文本描述；2)设计新型优势估计，将描述与实际观测的语义对齐程度与任务奖励耦合。

Result: 理论分析证明MAPO能降低梯度方差。实验表明该方法在多个视觉推理基准上取得最优性能。

Conclusion: 通过显式建模文本推理与视觉动作的语义对齐，MAPO有效解决了多模态LLM训练中的关键问题，为提升多模态推理能力提供了新思路。
...
TL;DR: EventFace是一个基于事件相机的人脸识别框架，通过整合空间结构和时间动力学进行身份建模。构建了EFace事件人脸数据集，并利用LoRA从预训练RGB模型迁移结构先验，同时引入运动提示编码器和时空调制器增强身份相关事件模式的表示。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有光照鲁棒性和隐私友好性优势，但事件流缺乏传统RGB人脸识别依赖的稳定光度外观。因此事件-based人脸识别应建模由刚性面部运动和个体几何结构驱动的时空身份表示，且当时缺乏专用数据集。

Method: 1)构建EFace小规模事件人脸数据集；2)使用LoRA将预训练RGB人脸模型的结构面部先验迁移到事件域；3)引入运动提示编码器(MPE)显式编码时间特征；4)设计时空调制器(STM)融合时间与空间特征。

Result: Rank-1识别率达94.19%，等错误率(EER)为5.35%，在光照退化条件下比竞争方法具有更强鲁棒性，学习到的表示具有更低的模板可重建性。

Conclusion: EventFace在事件人脸识别任务中达到最佳性能，有效解决了事件数据有限情况下的学习问题，通过结构先验迁移和时空建模提升了身份识别准确率和光照鲁棒性，同时增强了隐私保护能力。
...
TL;DR: 针对联邦学习中跨领域数据导致的领域偏移问题，提出FedDAP通过构建领域特定的全局原型并实施领域感知的特征对齐来提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 在现实世界的联邦学习场景中，客户端持有来自不同领域的数据，导致严重的领域偏移问题，现有的原型方法存在两个关键局限：(1) 构建单一全局原型而未保留领域信息；(2) 特征-原型对齐是领域无关的，强制客户端与全局原型对齐。

Method: 提出FedDAP框架，通过相似度加权融合机制构建领域特定的全局原型，使用双对齐策略：使本地特征与同领域原型对齐，同时鼓励与不同领域原型分离。

Result: 在DomainNet、Office-10和PACS三个数据集上验证了框架有效性，能够有效应对领域偏移挑战。

Conclusion: FedDAP通过领域特定的原型学习增强了本地领域的特异性学习，使全局模型能够泛化到不同领域。
...
### [29] [Physical Adversarial Attacks on AI Surveillance Systems:Detection, Tracking, and Visible--Infrared Evasion](https://arxiv.org/abs/2604.06865)
*Miguel A. DelaCruz,Patricia Mae Santos,Rafael T. Navarro*

Main category: cs.CV

TL;DR: 本文是一篇关于物理对抗攻击的综述论文，从监控系统的角度重新审视物理对抗攻击，探讨了时间持久性、感知模态、载体现实性和系统级目标等核心问题。


<details>
  <summary>Details</summary>
Motivation: 当前物理对抗攻击研究逐渐转向实际部署的监控系统场景，但现有文献缺乏从这一角度的系统性分析。传统孤立基准测试无法可靠评估监控系统的鲁棒性，需要从时间、传感器和物理部署约束等多维度进行评估。

Method: 作者提出了一个四部分类法，组织了物理对抗攻击领域的研究工作，涵盖多目标跟踪、可见光-红外双模态规避和可控服装等主题，并总结了当前评估实践和未解决的差距。

Result: 研究表明，监控鲁棒性不能仅从孤立的首帧基准测试来可靠判断；它必须作为随时间展开、跨传感器并在现实物理部署约束下的系统问题来检验。

Conclusion: 物理对抗攻击在监控场景下需要考虑时序持久性、感知模态、载体现实性和系统级目标；未来研究应关注距离鲁棒性、相机流程变化、身份级指标和激活感知测试等未解决问题。
...
TL;DR: 本文提出全局视频引导多模态翻译框架，通过预训练语义编码器、向量数据库字幕检索和区域感知跨模态注意力机制，有效解决长视频翻译中全局上下文捕捉不足的问题，实验表明该方法显著优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限：大多数现有视频引导多模态翻译方法依赖于本地对齐的视频片段与字幕的一对一配对，难以捕捉长视频中多个片段间的全局叙事上下文。

Method: 提出全局视频引导多模态翻译框架，包含三个关键技术：1）预训练语义编码器与基于向量数据库的字幕检索，构建与目标字幕语义紧密相关的视频片段上下文集；2）注意力机制聚焦高度相关的视觉内容同时保留剩余视频特征以维持更广泛的上下文信息；3）区域感知跨模态注意力机制增强翻译过程中的语义对齐。

Result: 在大型纪录片翻译数据集上的实验表明，该方法显著优于基线模型，在长视频场景中表现出色。

Conclusion: 该研究提出的全局视频引导多模态翻译框架有效解决了长视频中全局叙事上下文捕捉不足的问题，通过预训练语义编码器、向量数据库字幕检索和区域感知跨模态注意力机制，显著提升了翻译质量，特别是在长视频场景中表现优异。
...
### [31] [Multi-modal user interface control detection using cross-attention](https://arxiv.org/abs/2604.06934)
*Milad Moradi,Ke Yan,David Colwell,Matthias Samwald,Rhona Asgari*

Main category: cs.CV

TL;DR: 本文提出了一种多模态YOLOv5扩展方法，通过GPT生成的文本描述与视觉特征融合，提升UI控件检测性能，在16,000+截图数据集上实现显著改进。


<details>
  <summary>Details</summary>
Motivation: UI控件检测在自动化测试、无障碍访问和软件分析中至关重要，但现有仅基于像素的方法面临视觉歧义、设计变化和缺乏上下文信息的挑战。

Method: 提出一种多模态YOLOv5扩展，通过跨注意力模块将GPT生成的UI图像文本描述整合到检测流程中，实现视觉特征与文本嵌入语义信息的对齐。实验中比较了三种融合策略：逐元素相加、加权求和和卷积融合。

Result: 在包含23类控件、超过16,000张标注UI截图的大型数据集上评估，卷积融合策略表现最佳，显著提升了语义复杂或视觉模糊类别的检测性能。

Conclusion: 结合视觉和文本模态可显著增强UI元素检测能力，尤其在仅凭视觉信息不足的边缘情况下效果明显，为软件测试、无障碍支持和UI分析的智能工具开发开辟了新途径。
...
TL;DR: 提出了一种基于多模态大语言模型(MLLMs)的无训练声源定位(GAR)框架，通过生成-分析- refinement三个阶段实现音频-视觉一致性验证，在单源和多源基准数据集上取得了竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有SSL方法依赖于对比学习特征匹配，缺乏显式的推理和验证机制，在复杂声学场景中效果受限。作者受人类元认知过程启发，试图利用MLLMs内在的推理能力来解决这一问题。

Method: 提出了一个训练-free的GAR pipeline，包含三个阶段：1)Generation生成初始边界框和音频分类；2)Analysis通过开放集角色标注和锚点投票量化音频-视觉一致性；3)Refinement应用自适应门控防止不必要的调整。

Result: 在单源和多源基准数据集上进行了广泛实验，证明了该方法的竞争力性能。

Conclusion: 该工作创新性地将MLLMs的推理能力应用于声源定位任务，无需训练即可实现有效的音频-视觉一致性验证，为复杂声学场景中的声源定位提供了新的思路。
...
### [33] [CAAP: Capture-Aware Adversarial Patch Attacks on Palmprint Recognition Models](https://arxiv.org/abs/2604.06987)
*Renyang Liu,Jiale Li,Jie Zhang,Cong Wu,Xiaojun Jia,Shuxin Li,Wei Zhou,Kwok-Yan Lam,See-kiong Ng*

Main category: cs.CV

TL;DR: CAAP是一个面向掌纹识别的捕获感知对抗贴片攻击框架，通过十字形拓扑结构和ASIT、RaS、MS-DIFE三个模块实现可迁移的物理对抗攻击，实验证实在多个数据集和模型上具有高攻击成功率，且对抗训练无法完全防御。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要局限于数字设置，未充分考虑掌纹识别的纹理主导特性以及物理采集过程中引入的失真，因此深度掌纹识别系统对物理可实现攻击的鲁棒性仍缺乏充分理解。

Method: 提出CAAP框架，包含三个核心模块：ASIT（输入条件化贴片渲染）、RaS（随机捕获感知模拟）和MS-DIFE（特征级身份破坏引导）。采用十字形贴片拓扑结构以在固定像素预算下扩大空间覆盖范围并更有效地破坏长程纹理连续性。

Result: CAAP在Tongji、IITD和AISEC数据集上针对通用CNN主干网和特定掌纹识别模型进行了评估，表现出强大的非定向和定向攻击性能，具有良好的跨模型和跨数据集可迁移性。对抗训练虽可部分降低攻击成功率，但仍存在显著残余漏洞。

Conclusion: 深度掌纹识别系统对物理实现的对抗性攻击存在显著且难以完全消除的脆弱性，即使采用对抗训练也只能部分降低攻击成功率。这表明在实际应用中需要更有效的防御手段来保护掌纹识别系统的安全性。
...
TL;DR: 提出了RePL框架，通过masked reconstruction来识别和纠正半监督LiDAR语义分割中 noisy pseudo-labels 的错误，解决错误传播和确认偏差问题。


<details>
  <summary>Details</summary>
Motivation: 半监督学习用于LiDAR语义分割时，依赖于伪标签进行训练，但伪标签中的噪声会导致错误传播和确认偏差问题，严重影响模型性能。

Method: 提出RePL框架，包含两个核心组件：1) 通过masked reconstruction增强伪标签质量，识别和纠正伪标签中的潜在错误；2) 专门设计的训练策略。同时提供理论分析证明伪标签精炼有益的条件。

Result: 在nuScenes-lidarseg和SemanticKITTI数据集上进行了广泛评估，RePL显著提升了伪标签质量，取得了LiDAR语义分割的state-of-the-art性能。

Conclusion: RePL通过masked reconstruction有效解决了半监督学习中伪标签噪声带来的错误传播和确认偏差问题，理论分析和实验结果均验证了该方法的有效性和实用性。
...
TL;DR: 首个生成式 photomosaic（照片马赛克）创建方法，使用扩散模型条件生成瓦片图像，通过低频条件扩散机制对齐全局结构，同时保留提示驱动的细节。


<details>
  <summary>Details</summary>
Motivation: 传统 photomosaic 方法依赖于大量瓦片图像和基于颜色的匹配，限制了多样性和结构一致性。

Method: 提出生成式 photomosaic 框架，使用基于扩散的生成来合成瓦片图像，条件于参考图像。采用低频条件扩散机制对齐全局结构，同时保留提示驱动的细节。利用少样本个性化扩散模型。

Result: 实现了语义表达力和结构一致性兼备的 photomosaic 组合，有效克服了基于匹配方法的根本局限。

Conclusion: 该方法无需大量图像集合即可生成用户特定或风格一致的瓦片，为 photomosaic 创建开辟了新的生成式路径。
...
TL;DR: VGGT-SLAM++是一个利用视觉几何 grounding Transformer (VGGT)几何输出的完整视觉SLAM系统，通过融合VGGT前馈Transformer与Sim(3)解算的视觉里程计、基于数字高程图(DEM)的图构建模块以及空间校正后端，实现高精度的bounded-memory大规模建图。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的SLAM管道(如VGGT-SLAM)主要依赖稀疏闭环检测或全局Sim(3)流形约束，存在短期位姿漂移问题，缺乏高频率的局部束束调整(LBA)能力。

Method: 该系统包含三个核心模块：1)视觉里程计前端，融合VGGT前馈Transformer与Sim(3)解算方案；2)基于数字高程图(DEM)的图构建模块，将每个VGGT子图构建为稠密平面-规范DEM，分割成Patch并计算DINOv2嵌入以集成到共可见性图；3)空间校正后端，通过视觉地点识别(VPR)模块在共可见性窗口内检索空间邻域，触发频繁的局部优化。

Result: 在标准SLAM基准数据集上实现最先进的精度，显著降低了短期漂移，加速了图优化收敛，并通过紧凑的DEM瓦贴和子线性检索维持全局一致性。

Conclusion: VGGT-SLAM++成功实现了高精度的大规模视觉SLAM建图，通过DEM稠密地图分块和DINOv2嵌入特征实现子线性检索，在标准SLAM基准数据集上达到最先进的精度，显著降低了短期漂移，加速了图优化收敛，并保持了全局一致性。
...
TL;DR: SurFITR是一个用于监控风格图像伪造检测和定位的大规模数据集，包含超过137k个通过多模态LLM管道生成的伪造图像，可显著提升检测器在监控场景中的性能。


<details>
  <summary>Details</summary>
Motivation: 近年来开源图像生成模型的进步引发了视觉证据伪造的担忧。现有伪造检测模型在目标中心图像上训练，难以泛化到监控场景，因为监控图像中的篡改通常是局部且微妙的，场景具有多变的视角、小型或遮挡的物体以及较低的视觉质量。

Method: 提出SurFITR数据集，通过多模态LLM驱动的管道生成监控风格图像，实现语义感知的细粒度编辑。包含超过137k个伪造图像，具有不同分辨率和编辑类型，使用多种图像编辑模型生成。

Result: 现有检测器在SurFITR上性能显著下降，而在SurFITR上训练可显著提升领域内和跨领域性能。

Conclusion: SurFITR为监控风格图像伪造检测和定位提供了大规模基准数据集，可有效提升检测模型的泛化能力。
...
TL;DR: IRIDE项目通过机载处理能力为地球观测服务提供低延迟解决方案，以燃烧区域制图为例展示其优势。


<details>
  <summary>Details</summary>
Motivation: 当前运营的地球观测服务（CEMS、EFFIS、CLMS）依赖地面处理流水线，受限于下行链路延迟、带宽限制和自主观测优先级能力不足。

Method: IRIDE项目设计为"星座的星座"，集成异构传感技术于统一服务架构。HEO支持在轨数据产品生成，实现处理链中更早的信息提取。以IRIDE燃烧区域制图服务为案例研究。

Result: 实现了更高空间细节（亚3米地面采样距离）、更小可探测事件（最小制图单元3公顷）和改进的系统响应能力。

Conclusion: IRIDE HEO能力作为Copernicus服务的补充层，提供图像驱动预分类以支持下游应急和土地管理工作流。凸显了机载智能在新兴低延迟EO服务架构中的运营价值。
...
TL;DR: RefineAnything是一个多模态扩散精化模型，用于区域特定的图像细节恢复。通过Focus-and-Refine策略将分辨率预算重新分配到目标区域，并使用Boundary Consistency Loss减少拼接痕迹，在Refine-30K数据集和RefineEval基准上实现了高效的局部精化和近乎完美的背景保持。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型在局部细节恢复上存在缺陷（如文字、标志、细结构扭曲），且现有编辑模型容易忽视局部缺陷或意外改变背景。当目标区域只占输入分辨率的小部分时问题更严重。需要一个专门针对区域特定图像精化的解决方案。

Method: 1. 提出Focus-and-Refine策略：将输入图像裁剪并调整大小到目标区域，提高局部重建质量，然后将结果混合粘贴回原图
2. 设计Boundary Consistency Loss：减少拼接伪影，提高自然度
3. 构建Refine-30K数据集（20K参考基+10K无参考）和RefineEval基准

Result: 在RefineEval基准上相比竞争基线有显著提升，实现了近乎完美的背景保持和高精度的局部精化效果。

Conclusion: RefineAnything为高 precision局部精化提供了实用解决方案，通过区域聚焦策略有效解决了固定VAE分辨率下的小目标区域精化问题。
...
### [40] [Energy-based Tissue Manifolds for Longitudinal Multiparametric MRI Analysis](https://arxiv.org/abs/2604.07180)
*Kartikay Tehlan,Lukas Förner,Nico Schmutzenhofer,Michael Frühwald,Matthias Wagner,Nassir Navab,Thomas Wendler*

Main category: cs.CV

TL;DR: 提出一种基于患者特定能量建模的几何框架，用于纵向多参数MRI分析，无需分割标签或监督分类即可追踪组织变化。


<details>
  <summary>Details</summary>
Motivation: 现有纵向MRI分析方法依赖解剖分割或监督分类，缺乏患者特异性的几何参考系统来追踪组织状态演变。

Method: 将每个体素表示为多序列强度向量（T1, T1c, T2, FLAIR, ADC），使用去噪分数匹配训练隐式神经表示学习能量函数E_θ(u)；将基线能量流形作为固定几何参考，通过分析后续扫描向量分布相对于基线能量的变化进行纵向评估。

Result: 复发儿童病例在影像学明确复发前显示能量进行性增加和向肿瘤相关区域的定向位移；稳定疾病病例体素分布保持在低能量basin内无系统性漂移。

Conclusion: 患者特异性能量流形可作为纵向多参数MRI分析的几何参考系统，为神经肿瘤学中组织风险追踪提供基础。
...
### [41] [TeaLeafVision: An Explainable and Robust Deep Learning Framework for Tea Leaf Disease Classification](https://arxiv.org/abs/2604.07182)
*Rafi Ahamed,Sidratul Moon Nafsin,Md Abir Rahman,Tasnia Tarannum Roza,Munaia Jannat Easha,Abu Raihan*

Main category: cs.CV

TL;DR: 本研究利用深度学习技术开发茶叶病害自动检测系统，DenseNet201模型达到99%准确率，并结合可解释性方法实现了实际应用。


<details>
  <summary>Details</summary>
Motivation: 茶叶作为全球第二大消费饮品，其病害的精确识别和检测对农业生产具有重要意义，需要开发高效的自动化检测方法来保障茶叶产量和品质。

Method: 研究评估了多个CNN模型（ DenseNet201、MobileNetV2、InceptionV3），并采用Grad CAM、遮挡敏感性分析和对抗训练技术增强模型的可靠性和可解释性。

Result: DenseNet201在teaLeafBD数据集（包含7个类别：6个病害类+1个健康类）上取得了99%的最高测试准确率，成功开发了可在实际农业中应用的原型系统。

Conclusion: 该研究成功开发了基于深度学习的茶叶病害检测系统，DenseNet201模型达到99%的测试准确率，并通过Grad CAM、遮挡敏感性分析和对抗训练技术提高了模型的可解释性和鲁棒性，最终实现了可在实际农业中应用的原型系统。
...
### [42] [Region-Graph Optimal Transport Routing for Mixture-of-Experts Whole-Slide Image Classification](https://arxiv.org/abs/2604.07298)
*Xin Tian,Jiuliu Lu,Ephraim Tsalik,Bart Wanders,Colleen Knoth,Julian Knight*

Main category: cs.CV

TL;DR: ROAM是一种空间感知的多专家混合MIL聚合器，通过容量约束的熵最优传输和图正则化实现平衡专家利用，用于全切片图像分类


<details>
  <summary>Details</summary>
Motivation: 当前MIL聚合器将所有实例通过共享路径，限制了对病理异质性的处理能力；MoE方法虽可分区实例到专业专家，但无约束的softmax路由导致专家利用不平衡

Method: 提出ROAM方法：1）将密集patch包压缩为空间bin区域token；2）区域到专家分配使用带容量边际的Sinkhorn算法；3）图正则化Sinkhorn迭代在空间区域图上扩散路由分配

Result: 在四个WSI基准测试中性能与强MIL和MoE基线相当，NSCLC泛化测试（TCGA-CPTAC）达到外部AUC 0.845±0.019

Conclusion: ROAM通过容量约束的熵最优传输和图正则化机制，实现了平衡的专家利用率和空间一致性，在四个WSI基准测试中达到竞争力表现，NSCLC外部验证AUC达0.845±0.019
...
### [43] [MoRight: Motion Control Done Right](https://arxiv.org/abs/2604.07348)
*Shaowei Liu,Xuanchi Ren,Tianchang Shen,Huan Ling,Saurabh Gupta,Shenlong Wang,Sanja Fidler,Jun Gao*

Main category: cs.CV

TL;DR: MoRight是一个统一的运动控制视频生成框架，通过解耦运动建模和运动因果性学习，解决了现有方法中相机与物体运动纠缠以及缺乏因果关系建模的问题，支持前向和逆向推理。


<details>
  <summary>Details</summary>
Motivation: 生成动作控制视频需要两个关键能力：解耦的运动控制和运动因果性。现存方法将相机和物体运动混为单一追踪信号，且将运动视为运动学位移而非建模因果关系。

Method: 提出MoRight框架：(1)使用时序跨视角注意力将物体运动从规范静态视图转移到目标相机视角，实现解耦控制；(2)将运动分解为主动（用户驱动）和被动（后果）两部分，训练模型学习运动因果性。

Result: 在三个基准数据集上实现最优性能，具备高质量生成、运动可控性和交互感知能力。

Conclusion: 该工作实现了分离控制物体运动和相机视角的能力，同时支持前向推理（预测后果）和逆向推理（恢复驱动动作），在生成质量、运动可控性和交互感知方面达到先进水平。
...
### [44] [POS-ISP: Pipeline Optimization at the Sequence Level for Task-aware ISP](https://arxiv.org/abs/2604.06938)
*Jiyun Won,Heemin Yang,Woohyeok Kim,Jungseul Ok,Sunghyun Cho*

Main category: cs.CV

TL;DR: POS-ISP是一种序列级强化学习框架，将模块化ISP优化问题形式化为全局序列预测问题，通过单次前向传播预测整个模块序列及其参数，使用终端任务奖励优化pipeline，无需中间监督和冗余执行。


<details>
  <summary>Details</summary>
Motivation: 现有ISP优化方法存在局限性：神经架构搜索(NAS)存在训练-推理不匹配问题；逐步强化学习(RL)导致训练不稳定且计算开销高，因为需要分阶段决策。亟需一种稳定高效的ISP模块序列与参数联合优化方法。

Method: 提出POS-ISP序列级RL框架，将模块化ISP优化形式化为全局序列预测问题。该方法在单次前向传播中预测整个模块序列及其参数，使用终端任务奖励优化pipeline，消除对中间监督和冗余执行的需求。

Result: 在多个下游任务的实验中，POS-ISP在提升任务性能的同时降低了计算成本，验证了序列级优化作为任务感知ISP的稳定高效范式的有效性。

Conclusion: 序列级优化是一种稳定且高效的ISP优化范式，能够同时改善任务性能并降低计算开销，为模块化ISP的任务感知优化提供了新思路。
...
### [45] [Grounded Forcing: Bridging Time-Independent Semantics and Proximal Dynamics in Autoregressive Video Synthesis](https://arxiv.org/abs/2604.06939)
*Jintao Chen,Chengyu Bai,Junjun hu,Xinda Xue,Mu Xu*

Main category: cs.CV

TL;DR: 针对自回归视频合成的三个核心挑战（语义遗忘、视觉漂移、可控性丧失），提出Grounded Forcing框架，通过 Dual Memory KV Cache、Dual-Reference RoPE Injection 和 Asymmetric Proximity Recache 三个机制协同解决这些问题，实验验证其能显著提升长视频生成的语义一致性和视觉稳定性。


<details>
  <summary>Details</summary>
Motivation: 自回归视频合成在生成长时序视频方面有潜力，但面临三个相互关联的核心挑战：语义遗忘（上下文限制导致）、视觉漂移（位置外推）、可控性丧失（交互指令切换时）。现有方法往往单独处理这些问题，限制了长时序一致性。

Method: 提出Grounded Forcing框架，包含三个 interlocking机制：1) Dual Memory KV Cache - 解耦局部时序动态与全局语义锚点；2) Dual-Reference RoPE Injection - 将位置嵌入限制在训练流形内；3) Asymmetric Proximity Recache - 近距离加权缓存更新实现平滑语义继承。

Result: 大量实验表明Grounded Forcing显著增强长时序一致性和视觉稳定性。

Conclusion: 为交互式长视频合成提供了稳健基础。
...
### [46] [NTIRE 2026 Challenge on Bitstream-Corrupted Video Restoration: Methods and Results](https://arxiv.org/abs/2604.06945)
...
TL;DR: NTIRE 2026挑战赛聚焦于比特流损坏视频恢复任务，建立了通用的基准测试数据集和评估协议，汇集了多种参赛方法并总结了技术趋势，为该新兴领域的研究提供了重要参考。


<details>
  <summary>Details</summary>
Motivation: 随着比特流损坏导致的视频解码经常产生严重的时空伪影和内容失真，亟需一个通用的基准来评估和推动比特流损坏视频恢复技术的发展。

Method: 挑战赛提供了通用的数据集、评估协议和基准测试，基于最近的比特流损坏视频恢复进展，构建了逼真的损坏设置来评估恢复方法。

Result: 挑战赛吸引了多个参与方法，汇总了最终结果和技术趋势，证明了该任务具有相当大的难度。

Conclusion: 该挑战赛突出了这一新兴任务的难度，并为未来在实际比特流损坏情况下进行稳健视频恢复的研究提供了有用的见解。挑战赛建立了通用的基准测试，为该领域的研究提供了重要的参考和推动力。
...
### [47] [Making MLLMs Blind: Adversarial Smuggling Attacks in MLLM Content Moderation](https://arxiv.org/abs/2604.06950)
*Zhiheng Li,Zongyang Ma,Yuntong Pan,Ziqi Zhang,Xiaolei Lv,Bo Li,Jun Gao,Jianing Zhang,Chunfeng Yuan,Bing Li,Weiming Hu*

Main category: cs.CV

TL;DR: 论文提出了针对多模态大语言模型的新型对抗性攻击——Adversarial Smuggling Attacks，利用人-AI能力差异将有害内容编码为人类可读但AI不可读的视觉格式，从而绕过自动内容审核。构建了包含1700个攻击实例的SmuggleBench基准测试，发现GPT-5和Qwen3-VL等最先进模型的攻击成功率均超过90%。


<details>
  <summary>Details</summary>
Motivation: 随着MLLMs被广泛部署为自动化内容审核工具，需要识别新的安全威胁。现有对抗性攻击（扰动用于误分类、越狱用于有害输出生成）无法完全涵盖利用人-AI能力差距的新攻击形式，即将有害内容编码成人类可读的视觉格式但AI无法读取，从而绕过检测机制。

Method: 论文构建了SmuggleBench基准测试，包含1700个对抗性smuggling攻击实例。提出两类攻击路径：1) Perceptual Blindness感知盲区，破坏文本识别；2) Reasoning Blockade推理阻断，抑制语义理解。通过评估多款先进模型在此基准上的表现，从感知和推理角度分析漏洞根因，并探索测试时间扩展(CoT)和对抗训练(SFT)两种防御策略。

Result: 在SmuggleBench上的评估显示，包括GPT-5(专有)和Qwen3-VL(开源)在内的最先进的闭源和开源模型都极易受到此攻击，攻击成功率(ASR)均超过90%。识别出三个主要漏洞根因：视觉编码器能力有限、OCR robustness差距、以及领域特定对抗性示例稀缺。

Conclusion: 研究揭示了针对MLLMs内容审核的新型严重威胁——Adversarial Smuggling Attacks，该攻击利用人-AI能力差距可有效绕过先进模型的检测。论文从感知和推理两个维度分析漏洞原因，并初步探索了可能的防御方法，为MLLMs安全部署提供了重要参考。
...
### [48] [Compression as an Adversarial Amplifier Through Decision Space Reduction](https://arxiv.org/abs/2604.06954)
*Lewis Evans,Harkrishan Jandu,Zihan Ye,Yang Lu,Shreyank N Gowda*

Main category: cs.CV

TL;DR: 研究了压缩对对抗鲁棒性的影响，发现压缩可作为对抗放大器。在压缩表示中直接应用攻击比在像素空间更有效，原因是压缩导致决策空间缩小和分类边际收缩。


<details>
  <summary>Details</summary>
Motivation: 研究压缩对对抗鲁棒性的影响是一个未被充分探索的领域。图像压缩在现代视觉管道中普遍存在，但在压缩表示中直接应用攻击的影响尚不清楚。

Method: 在标准基准和架构上进行了广泛实验，比较压缩感知攻击与像素空间攻击在相同标称扰动预算下的效果，分析压缩对决策边界和分类边际的影响。

Result: 压缩感知攻击在相同扰动预算下比像素空间攻击效果显著更强。压缩诱导的不可逆、信息丢失的变换会收缩分类边际并增加对扰动的敏感性。

Conclusion: 压缩可作为深度图像分类器的对抗放大器，在压缩表示中应用攻击比像素空间攻击更有效。压缩引起的决策空间缩小是导致这种脆弱性的根本原因。需要在压缩-循环部署设置中重新评估对抗鲁棒性。
...
### [49] [Auditing Demographic Bias in Facial Landmark Detection for Fair Human-Robot Interaction](https://arxiv.org/abs/2604.06961)
*Pablo Parte,Roberto Valle,José M. Buenaposada,Luis Baumela*

Main category: cs.CV

TL;DR: 本文系统性地审计了面部地标检测中的 demographic bias（年龄、性别和种族偏见），引入控制统计方法来分离 demographic 因素与混淆视觉因素。结果表明，混淆视觉因素（尤其是头部姿势和图像分辨率）显著大于 demographic 属性影响；在控制混淆因素后，性别和种族差异消失，但发现显著的年龄相关效应——老年人面临更高偏见。这表明即使在低级别视觉组件中也会出现公平性问题，并可能通过人机交互 pipeline 传播，对弱势群体造成不成比例的影响。


<details>
  <summary>Details</summary>
Motivation: 虽然 demographic biases 已在高级人脸分析任务中得到广泛研究，但低级别面部地标检测中的 biases 仍未被探索。该研究旨在系统性地审计面部地标检测中的年龄、性别和种族偏见，以确定公平性问题是否存在于低级别视觉组件中。

Method: 引入受控统计方法，将 demographic 效应与混淆视觉因素（头部姿势、图像分辨率等）分离，评估标准代表性模型在 demographic 组间的性能差异。

Result: 混淆视觉因素（头部姿势和图像分辨率）的影响远大于 demographic 属性；控制混淆因素后，性别和种族差异消失；但发现显著的年龄效应——老年人面临更高的检测偏差。

Conclusion: 公平性问题可能出现在低级别视觉组件中，并可沿 HRI pipeline 传播，对弱势群体造成不成比例的影响。审计和纠正这些 bias 是实现可信和公平机器人感知系统的必要步骤。
...
### [50] [MAR-GRPO: Stabilized GRPO for AR-diffusion Hybrid Image Generation](https://arxiv.org/abs/2604.06966)
*Xiaoxiao Ma,Jiachen Lei,Tianfei Ren,Jie Huang,Siming Fu,Aiming Hao,Jiahong Wu,Xiangxiang Chu,Feng Zhao*

Main category: cs.CV

TL;DR: 本文针对掩码自回归模型(MAR)中扩散头导致的梯度噪声问题，提出stabilized RL框架，通过多轨迹期望、不确定性估计和一致性感知token选择三项技术提升训练稳定性与生成质量。


<details>
  <summary>Details</summary>
Motivation: 将强化学习扩展到混合AR-diffusion框架面临挑战，原因在于推理过程交错且对数概率估计存在噪声。MAR模型中扩散头引入的噪声梯度导致训练不稳定和早期性能饱和。

Method: 提出多轨迹期望(MTE)通过平均多条扩散轨迹估计优化方向以降低梯度噪声；估计token级别不确定性并只对top-k%不确定token应用多轨迹优化以避免过度平滑；引入一致性感知token选择策略过滤与最终生成内容对齐度低的AR token。

Result: 在多个基准测试上，本文方法一致提升了视觉质量、训练稳定性和空间结构理解，性能优于baseline GRPO和预训练RL模型。

Conclusion: 本文提出的stabilized RL框架能有效解决MAR模型在RL训练中的梯度噪声问题，通过MTE、不确定性估计和一致性感知token选择三个核心技术，显著提升了训练稳定性和生成质量。
...
### [51] [Canopy Tree Height Estimation Using Quantile Regression: Modeling and Evaluating Uncertainty in Remote Sensing](https://arxiv.org/abs/2604.06988)
*Karsten Schrödter,Jan Pauls,Fabian Gieseke*

Main category: cs.CV

TL;DR: 将分位数回归引入现有卫星树高估计模型，通过轻微修改预测头实现不确定性量化。模型能提供统计校准的不确定性估计，且不确定性程度与地形复杂性和植被异质性相关。


<details>
  <summary>Details</summary>
Motivation: 准确的树高估计对生态监测和生物量评估至关重要。当前大多数树高估计方法依赖于点预测，限制了在风险敏感场景中的应用。现有模型无法提供不确定性估计，难以评估预测的可信度。

Method: 将分位数回归应用于现有的基于卫星数据的树高估计模型，通过对预测头进行轻微修改，使模型能够提供统计校准的不确定性估计。

Result: 研究表明，经过微小修改后，现有模型即可提供统计校准的不确定性估计。不确定性估计与遥感中的已知挑战（地形复杂性、植被异质性）相关联，模型在更具挑战性的条件下置信度较低。

Conclusion: 该方法为树高估计引入了不确定性量化能力，增强了模型在风险敏感场景中的适用性。不确定性估计可反映数据的内在难度，为模型可靠性评估提供了有价值的参考。
...
TL;DR: IQ-LUT是一种针对资源受限设备的图像超分辨率方法，通过将插值和量化集成到ECNN、引入残差学习以及基于知识蒸馏的非均匀量化，在保证优质重建效果的同时将存储需求降低50倍。


<details>
  <summary>Details</summary>
Motivation: LUT方法在加速图像超分辨率推理方面具有潜力，但追求更高质量需要更大感受野和位深，这会导致LUT索引空间呈指数级增长，形成存储瓶颈，限制了在资源受限设备上的部署。

Method: 提出三项核心技术：(1)将插值和量化集成到单输入多输出ECNN中，大幅减少索引空间和LUT尺寸；(2)引入残差学习，降低对LUT位深的依赖，提高训练稳定性并优先重建细粒度细节；(3)基于知识蒸馏的非均匀量化优化量化级别，在减少存储的同时补偿量化损失。

Result: 与ECNN相比，存储成本降低高达50倍，同时实现了更优的超分辨率质量。

Conclusion: IQ-LUT方法成功实现了存储成本大幅降低(与ECNN相比高达50倍)的同时保证了优异的超分辨率质量，有效解决了资源受限设备上部署的存储瓶颈问题。
...
TL;DR: 本文提出了一个在Unity中实现的虚拟扫描框架，用于生成逼真的合成3D扫描数据集，以支持场景重建和物体补全的深度学习训练。


<details>
  <summary>Details</summary>
Motivation: 获取带有完整真实几何信息的成对部分扫描数据集成本高且耗时，特别是对于遮挡区域需要精确的真实坐标，这限制了学习-based方法在3D场景重建和物体补全领域的发展。

Method: 提出了一个虚拟扫描框架，通过可配置参数（扫描分辨率、测量范围、距离依赖噪声）模拟真实扫描器行为；使用基于射线的扫描从虚拟视角进行，实现传感器可见性和遮挡效应的逼真建模；在扫描位置捕获全景图像为点云上色；集成了程序化室内场景生成管道，自动生成多样化的房间布局和家具排列。

Result: 生成了V-Scan数据集，包含合成室内扫描、对象级别的部分点云、基于体素的遮挡网格和完整真实几何信息，可为场景重建和物体补全的学习方法提供监督训练和评估。

Conclusion: 该虚拟扫描框架和V-Scan数据集为3D场景重建和物体补全学习方法的训练与评估提供了有价值的数据支持，有望推动该领域的发展。
...
TL;DR: ModuSeg是一个无需训练的弱监督语义分割框架，通过解耦对象发现和语义分配来实现像素级预测。该方法结合通用掩码提议器和语义基础模型，通过非参数特征检索实现分割，并提出语义边界净化和软掩码特征聚合策略来缓解边界模糊和量化误差。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督语义分割方法通常将语义识别和对象定位纠缠在一起，导致模型仅关注稀疏的判别区域。尽管基础模型展现出巨大潜力，但许多方法仍遵循紧密耦合的优化范式，难以有效缓解伪标签噪声，且通常依赖耗时多阶段重训练或不稳定的端到端联合优化。

Method: 提出ModuSeg框架，包含三个核心组件：1）通用掩码提议器提取具有可靠边界几何提议；2）语义基础模型构建离线特征库，将分割转化为非参数特征检索；3）语义边界净化和软掩码特征聚合策略缓解边界模糊和量化误差，提取高质量类别原型。

Result: 在标准基准数据集上的广泛实验表明，所提出的解耦架构能更好地保留精细边界，无需参数微调即可实现具有竞争力的性能。

Conclusion: 通过解耦对象发现和语义 assignment，ModuSeg成功实现了无需训练的弱监督语义分割，在保持精细边界的同时获得了优异性能，为该领域提供了一种新的有效范式。
...
TL;DR: 本文提出DARE框架，通过分布修正的无分类器引导(DR-CFG)和空间表示对齐(SRA)两大技术，解决条件扩散模型在文本到视频生成中忽视语义重要token的问题，在多个数据集上实现了生成质量和语义对齐的显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前条件扩散模型在推理过程中往往忽视语义重要的token，导致在无分类器引导下产生偏向性或不完整的生成结果。问题源于训练数据中token频率的长尾分布导致的分布偏差，以及交叉注意力中的空间错位问题——语义重要的token被信息量较少的token所掩盖。

Method: 提出Distribution-Aware Rectification and Spatial Ensemble (DARE)框架，包含两个核心组件：1) Distribution-Rectified Classifier-Free Guidance (DR-CFG)，通过动态抑制低语义密度的主导token来正则化训练过程；2) Spatial Representation Alignment (SRA)，根据token重要性自适应重新加权交叉注意力图并强制表示一致性。

Result: 实验表明DARE在多个基准数据集上持续提升生成保真度和语义对齐性能，取得了显著的性能增益。

Conclusion: DARE在多个基准数据集上显著提升了生成质量和语义对齐性能，超越了现有方法。该框架通过结合分布纠正和空间对齐两大策略，有效解决了条件扩散模型在文本到视频生成中忽视语义重要token的问题。
...
TL;DR: AnchorSplat是一种新的前馈3DGS框架，通过引入anchor-aligned高斯表示和3D几何先验引导，实现了与图像分辨率和视图数量无关的几何感知可渲染3D高斯，在ScanNet++ v2 NVS基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈高斯重建模型采用像素对齐公式，将每个2D像素映射到3D高斯，使高斯表示与输入图像紧密耦合。这种方法限制了效率和解耦性。

Method: AnchorSplat引入了anchor-aligned高斯表示，由3D几何先验（如稀疏点云、体素或RGB-D点云）指导，实现了与图像分辨率和视图数量无关的几何感知可渲染3D高斯。此外，还使用了Gaussian Refiner仅通过少量前向传递来调整中间高斯。

Result: 在ScanNet++ v2 NVS基准测试中实现了SOTA性能，显著减少了高斯原语数量，同时提高了视图一致性。

Conclusion: AnchorSplat在ScanNet++ v2 NVS基准测试中实现了最先进的性能，具有更好的视图一致性，同时使用更少的高斯原语。该方法通过anchor-aligned设计显著减少了所需的高斯数量，提高了计算效率并增强了重建保真度。
...
TL;DR: LIANet是一个基于坐标的神经表示方法，将多时态星载地球观测数据建模为连续时空神经场，仅给定空间和时间坐标即可重建卫星图像，，预训练后可适应语义分割等下游任务，且无需访问原始卫星数据。


<details>
  <summary>Details</summary>
Motivation: 现有的地理空间基础模型(GFMs)存在数据访问和预处理开销高的问题，缺乏用户友好的替代方案，需要一种简化端用户工作流程的方法。

Method: 将地球观测数据表示为连续时空神经场，通过坐标输入重建卫星图像，预训练后仅基于标签进行微调以适应下游任务。

Result: 在目标区域上预训练后，微调用于下游任务时性能与从零训练或使用现有GFMs相当。

Conclusion: LIANet成功作为GFMs的用户友好替代方案，消除了数据访问和预处理的开销，仅基于标签即可完成微调。
...
TL;DR: 针对异常检测中正常样本定义模糊的问题，提出了新的场景和评估指标，并设计了RePaste方法通过迭代重粘贴高异常分数区域来增强学习，在MVTec AD上取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中正常样本的定义常常是模糊的。例如，在某些情况下样本有轻微划痕或污渍但仍可接受，而在设备升级后需要更高精度时，正常样本中可能包含需要被分类为异常的轻微划痕、微小灰尘或异物。这种情况在工业场景中经常出现但尚未被讨论。

Method: 提出RePaste方法，通过将前一步中高异常分数的区域重新粘贴到下一步的输入中来增强学习。

Result: 在MVTec AD基准测试上，RePaste在提出的评估指标上达到了最先进性能，同时保持了高AUROC和PRO分数。

Conclusion: RePaste在MVTec AD数据集上达到了最先进性能，在提出的评估指标上表现优异，同时保持了较高的AUROC和PRO分数。
...
TL;DR: 针对语义分割中标签成本高的问题，提出改进的半监督学习方法，通过利用有标签图像的类标签和数据质量正则化来提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 语义分割中像素级标签创建成本高，传统半监督学习方法ClassMix存在伪标签不准确和无标签与有标签图像间数据质量差距的问题。

Method: 提出两种方法：一是将有标签图像的类标签及对应图像区域粘贴到无标签图像及其伪标签图像上；二是训练模型使对无标签图像的预测更接近有标签图像的预测。

Result: 在Chase和COVID-19数据集上验证，平均mIoU提升2.07%。

Conclusion: 该研究提出的方法在Chase和COVID-19数据集上相比传统半监督学习方法平均提升了2.07%的mIoU，有效解决了ClassMix中伪标签不准确和数据质量差距的问题。
...
TL;DR: 提出了一种用于跨医院放射影像数据共享的实用保留去标识化流程(UPDP)，在移除隐私敏感信息的同时保留诊断相关的病理线索，使模型能在去标识化数据上训练并保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 大规模放射影像数据对开发医学AI系统至关重要，但跨医院共享受限于隐私问题。现有去标识化研究主要关注移除身份信息以实现合规数据发布，但去标识化数据是否能保留足够的实用性用于大规模视觉语言模型训练和跨医院迁移仍未被充分探索。

Method: 构建隐私敏感术语黑名单和病理相关术语白名单；使用生成式过滤机制合成原始影像的隐私过滤且病理保留的对应版本；结合ID过滤的报告，形成可跨医院安全共享的去标识化数据。

Result: 在公开胸部X光基准测试上验证，该方法有效移除隐私敏感信息同时保留诊断相关病理线索。在去标识化数据上训练的模型保持与原始数据相当的诊断准确性，但身份相关准确性显著下降，确认隐私保护有效。跨医院场景下，去标识化数据与本地数据结合可获得更好性能。

Conclusion: UPDP方法成功实现了跨医院放射影像数据共享中隐私保护与实用保留的平衡，为大规模医学AI模型训练提供了可行的数据共享方案。
...
### [61] [USCNet: Transformer-Based Multimodal Fusion with Segmentation Guidance for Urolithiasis Classification](https://arxiv.org/abs/2604.07141)
...
TL;DR: USCNet is a novel deep learning method that enables precise preoperative classification of kidney stones by integrating CT images with clinical data from EHR, using a Transformer-based multimodal fusion framework with attention mechanisms and a dynamic loss function, achieving superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Kidney stone disease is a prevalent urological condition, and understanding stone composition is crucial for personalized treatment and recurrence prevention. Current methods rely on postoperative specimen analysis, which prevents rapid preoperative classification. This limitation motivates the development of a new approach for precise preoperative classification.

Method: The study introduces USCNet (Urinary Stone Segmentation and Classification Network), which integrates CT images with Electronic Health Records (EHR) data using a Transformer-based multimodal fusion framework. The model incorporates CT-EHR attention and segmentation-guided attention modules for accurate classification. Additionally, a dynamic loss function is proposed to balance segmentation and classification objectives.

Result: Experiments on an in-house kidney stone dataset demonstrate that USCNet achieves outstanding performance across all evaluation metrics, with classification efficacy significantly surpassing existing mainstream methods.

Conclusion: This study presents a promising solution for precise preoperative classification of kidney stones, offering substantial clinical benefits. The source code is publicly available.
...
### [62] [DINO-QPM: Adapting Visual Foundation Models for Globally Interpretable Image Classification](https://arxiv.org/abs/2604.07166)
*Robert Zimmermann,Thomas Norrenbrock,Bodo Rosenhahn*

Main category: cs.CV

TL;DR: DINO-QPM是一个轻量级可解释性适配器，将DINOv2的高维特征转换为人类可理解的对比性类别独立表示，实现了全球可解释的图像分类。


<details>
  <summary>Details</summary>
Motivation: DINOv2等视觉基础模型虽然性能先进，但其复杂的高维表示给可解释性带来挑战。现有方法难以同时实现高分类精度和可解释性。

Method: 1)使用平均池化连接patch嵌入而非CLS token，实现空间定位 2)应用稀疏损失最小化空间散射和背景噪声 3)在冻结的DINObackbone上运行QPM适配器

Result: 在分类准确性和解释质量上均优于冻结视觉基础模型的其他方法，超越DINOv2线性探测的精度。

Conclusion: 成功将QPM的可解释性以适配器形式集成到视觉基础模型中，在保持冻结backbone的同时实现了高精度和人类可理解的可解释特征。
...
### [63] [Multiple Domain Generalization Using Category Information Independent of Domain Differences](https://arxiv.org/abs/2604.07175)
*Reiji Saito,Kazuhiro Hotta*

Main category: cs.CV

TL;DR: 本文提出一种基于领域泛化的医学图像分割方法，通过分离领域无关特征与使用SQ-VAE量子向量吸收领域差距，在血管和细胞核分割任务上提升了跨领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 模型在特定数据集上训练后，当应用于不同环境的数据集时，准确性会显著下降。这是因为领域差异导致的，如成像设备和染色方法的不同。该问题在医学图像分割任务中尤为突出。

Method: 提出两种方法：1）分离与领域差异无关的类别信息与源领域特定信息；2）利用随机量化变分自编码器（SQ-VAE）中的量子向量来吸收领域差距。

Result: 在血管分割和细胞核分割数据集上评估了该方法，结果表明准确率相比传统方法有所提升。

Conclusion: 该方法通过提取领域无关信息并使用量子向量处理领域差距，实现了更好的跨领域分割性能。
...
TL;DR: INSPATIO-WORLD是一个实时框架，能够从单个参考视频中恢复和生成高保真、可动态交互的场景。该方法采用时空自回归（STAR）架构，通过隐式时空缓存和显式空间约束模块实现一致且可控的场景演化，并引入联合分布匹配蒸馏（JDMD）克服合成数据导致的保真度退化问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成范式缺乏空间持久性和视觉真实性，难以支持复杂环境中的无缝导航。构建具有空间一致性、实时交互性的世界模型仍是计算机视觉中的 fundamental challenge。

Method: 提出STAR架构，包含两个紧耦合组件：1）隐式时空缓存（Implicit Spatiotemporal Cache）：聚合参考和历史观测到潜在世界表示，确保长时域导航的全局一致性；2）显式空间约束模块（Explicit Spatial Constraint Module）：强制执行几何结构，将用户交互转化为精确且物理合理的相机轨迹。此外，引入联合分布匹配蒸馏（JDMD），利用真实世界数据分布作为正则化引导，克服合成数据导致的保真度退化。

Result: 在空间一致性和交互精度方面显著优于现有SOTA模型；在WorldScore-Dynamic基准上排名实时交互方法第一；建立了从单目视频重建4D环境进行导航的实用 pipeline。

Conclusion: INSPATIO-WORLD成功实现了从单参考视频生成高保真动态交互场景的实时框架，通过STAR架构和JDMD技术有效解决了空间一致性和视觉保真度的核心挑战，为4D环境导航提供了实用方案。
...
TL;DR: VersaVogue是一个统一的多条件可控时尚合成框架，通过特征路由注意力和自动化偏好优化技术，同时支持服装生成和虚拟试穿任务，解决了属性纠缠问题，并在基准数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将服装生成和虚拟试穿作为独立问题处理，限制了实际工作流程的灵活性；同时，多源异构条件下的时尚图像合成仍然困难，传统方法依赖简单特征拼接或静态分层注入，导致属性纠缠和语义干扰。

Method: 提出两个核心技术：1) 特征路由注意力(TA)模块，利用混合专家机制动态路由条件特征到兼容的专家和生成层，实现纹理、形状、颜色等视觉属性的解纠缠注入；2) 自动化多视角偏好优化(MPO)管道，无需人工标注或任务特定奖励模型，通过内容保真度、文本对齐和感知质量三个评估器构建偏好数据，并使用DPO进行优化。

Result: 在 garment generation 和 virtual dressing 两个基准数据集上的广泛实验表明，VersaVogue在视觉保真度、语义一致性和细粒度可控性方面始终优于现有方法。

Conclusion: VersaVogue通过统一的框架和创新的技术，有效解决了时尚图像合成中的条件控制和解纠缠问题，在 garment generation 和 virtual dressing 两个任务上都取得了最先进的性能。
...
TL;DR: PhyEdit是一个利用显式几何模拟进行3D感知图像编辑的框架，通过结合3D先验和联合2D-3D监督来提升物体操作的物理准确性，并提出RealManip-10K数据集和ManipEval基准用于评估。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型在精确空间操作方面存在局限，无法正确缩放和定位对象，主要原因在于缺乏明确的3D几何和透视投影机制，导致物理准确性不足。

Method: 开发了PhyEdit图像编辑框架，利用显式几何模拟作为上下文3D感知视觉指导，结合即插即用的3D先验和联合2D-3D监督。

Result: 在RealManip-10K数据集上进行了广泛实验，PhyEdit在3D几何准确性和操作一致性方面均优于现有方法，包括强大的闭源模型。

Conclusion: PhyEdit通过结合显式几何模拟和2D-3D联合监督，有效提升了图像编辑中的物理准确性和操作一致性，在3D几何精度和操作一致性方面优于现有方法，包括强大的闭源模型。
...
TL;DR: Geo-EVS是一个用于自动驾驶中轨迹外新颖视图合成的几何条件框架，通过Geometry-Aware Reprojection和Artifact-Guided Latent Diffusion两个组件，在稀疏监督下提高了合成质量和下游3D检测性能。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶中，异构传感器生成的标准化虚拟视图可以减少相机支架依赖。但现有方法在记录轨迹外会退化，因为外推姿态提供弱几何支持且没有密集的目标视图监督。关键是在训练时明确让模型暴露于轨迹外条件的缺陷。

Method: 提出Geo-EVS，一个基于稀疏监督的几何条件框架，包含两个组件：1) Geometry-Aware Reprojection (GAR)：使用微调的VGGT重建彩色点云并重投影到观察和虚拟目标姿态，生成几何条件图，统一了训练和推理的重投影路径；2) Artifact-Guided Latent Diffusion (AGLD)：在训练时注入重投影导出的 artifact masks，使模型学习在缺失支撑下恢复结构。

Result: 在Waymo数据集上，Geo-EVS提高了稀疏视图合成质量和几何精度，特别是在高角度和低覆盖设置下。它还改善了下游3D检测。

Conclusion: Geo-EVS通过几何条件重投影和artifact引导的潜在扩散，有效解决了轨迹外视图合成中几何支持弱和监督缺失的问题，在稀疏视图合成和3D检测任务上取得了显著改进。
...
TL;DR: GenLCA是一种基于扩散的生成模型，能够从文本和图像输入生成和编辑逼真的全身化身，支持高保真面部和全身动画。核心创新在于利用可见性感知训练策略解决部分观察问题，使大规模视频数据可用于训练3D扩散模型。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在解决从文本和图像输入生成和编辑逼真的全身化身的难题。现有方法缺乏高保真的面部和全身动画能力。核心挑战在于：训练全身3D扩散模型需要3D数据，但实际只能获取部分可观测的2D数据（视频），且大多数真实世界视频仅提供身体部位的局部观察，导致3D token中出现过度模糊或透明伪影。

Method: 提出一种新颖的范式，允许从部分可观测的2D数据训练全身3D扩散模型。具体方法包括：1）利用预训练的前向化身重建模型作为可动画的3D tokenizer，将非结构化视频帧编码为结构化3D token；2）提出可见性感知扩散训练策略，用可学习token替换无效区域，仅在有效区域计算损失；3）在token数据集上训练基于流的扩散模型，保持预训练化身重建模型提供的逼真性和可动画性。

Result: 实现高保真生成和编辑结果，在生成质量上大幅超越现有解决方案。支持高保真的面部和全身动画，具有卓越的逼真性和泛化能力。

Conclusion: 该方法有效地利用大规模真实世界视频数据原生训练3D扩散模型，为化身生成和编辑领域树立了新的技术标杆。
...
TL;DR: Mem3R是一种流式3D重建模型，采用混合内存设计，将相机跟踪与几何映射解耦，以改善长序列的时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的循环模型在处理长视觉流时存在漂移累积和时间遗忘问题，原因是压缩潜在记忆的容量有限。

Method: Mem3R采用混合内存设计：相机跟踪使用隐式快速权重记忆（轻量级MLP，通过测试时训练更新）；几何映射维护显式基于token的固定大小状态。

Result: 相比CUT3R，Mem3R显著提升长序列性能，模型参数量从793M降至644M。与TTT3R集成后，在500-1000帧序列上绝对轨迹误差降低39%。同时支持视频深度估计和3D重建等下游任务，保持恒定GPU内存和相当推理吞吐量。

Conclusion: Mem3R通过混合内存设计有效解决了流式3D重建中的长序列一致性问题，在性能和效率之间取得良好平衡。
...
TL;DR: 提出AlignPrune方法，通过损失轨迹动态对齐分数在噪声标签环境下实现鲁棒的数据剪枝，作为即插即用模块可提升现有动态剪枝框架性能最多6.3%。


<details>
  <summary>Details</summary>
Motivation: 现有动态数据剪枝方法在噪声标签环境下效果不佳，因为这些方法通常依赖单样本损失作为排序标准，容易错误保留高损失的噪声样本，导致性能显著下降。

Method: 提出动态对齐分数（DAS），基于损失轨迹来识别噪声样本，通过AlignPrune模块动态对齐样本的损失变化趋势，从而更准确地区分噪声标签和干净样本。

Result: 在五种广泛使用的基准数据集上，针对多种噪声类型和剪枝比例进行实验，AlignPrune相比最先进的基线方法准确率提升最高达6.3%。

Conclusion: AlignPrune提供了一种可泛化的噪声数据剪枝解决方案，作为即插即用模块可无缝集成到现有动态剪枝框架中，在五种基准数据集上验证了有效性，为现实场景中的学习提供了新思路。
...
TL;DR: 提出了SUMI方法，通过模拟降解-增强学习，将低质量的常规CT（EICT）增强为高质量的光子计数CT（PCCT）质量，解决PCCT临床可用性有限的问题。


<details>
  <summary>Details</summary>
Motivation: 光子计数CT（PCCT）相比传统能量积分CT（EICT）具有更高的空间分辨率和更低的噪声，但PCCT的临床可用性有限，限制了大规模研究和临床部署。需要一种方法将高质量PCCT的优势迁移到常规EICT上。

Method: 提出SUMI方法，核心思路是显式建模真实采集退化过程，将PCCT转化为临床可信的低质量对应物，然后学习逆转这一过程。具体包括：(1)在1046个PCCT上训练潜在扩散模型，使用在1046个PCCT和145家医院的405379个EICT上预训练的自动编码器提取通用CT潜在特征；(2)构建超过17316个公开EICT数据集，增强至PCCT质量，并经放射科医生验证标注气道树、动脉、静脉、肺叶等结构。

Result: 外部数据上，SUMI在SSIM上提升15%，PSNR提升20%，优于现有最先进的图像转换方法；读者研究中，放射科医生评定的临床效用显著改善；下游顶级病变检测性能提升，灵敏度提高15%，F1分数提高10%。

Conclusion: 使用有限的高质量扫描作为参考，可以系统地将新兴成像技术优势提炼到常规EICT中，为推动PCCT技术在临床广泛可用提供可行路径。
...
TL;DR: 论文提出Gaussian Wrapping方法，通过为每个高斯元素引入可学习的定向法线和适应性衰减公式，从3D Gaussian Splatting中提取精确的水密网格，解决了传统方法中表面提取困难的问题。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) 虽然在快速新视图合成方面表现出色，但其基于不透明度的 formulation 使得表面提取从根本上困难。不同于基于符号距离场或占据的隐式方法，3DGS 缺乏全局几何场，现有方法只能借助 TSDF 融合等启发式手段。

Method: 1) 引入可学习的定向法线于每个高斯元素；2) 定义适应性衰减公式，推导出任意位置法线和占据场的闭式表达式；3) 引入新颖的一致性损失和专门的密集化策略，使高斯包裹整个表面并封闭几何孔洞；4) 修改可微 rasterizer 输出深度为等值面；5) 提出 Primal Adaptive Meshing 实现任意分辨率的兴趣区域网格化。

Result: 在 DTU 和 Tanks and Temples 数据集上达到最先进水平，能够生成完整的水密网格，同时恢复细薄结构（如自行车辐条），且网格尺寸仅为并发工作的一小部分。

Conclusion: Gaussian Wrapping 方法为 3DGS 的表面提取提供了原则性的解决方案，通过引入可学习法线和适应性衰减公式，成功从高斯 splatting 中提取精确 watertight 网格，为该领域树立了新的技术标杆。
...
TL;DR: 提出一个多类别、跨文化的基准数据集，用于评估视觉-语言模型从图像中推断结构化文化遗产元数据的能力，并使用LLM-as-Judge框架进行评估。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在图像 captioning 方面取得进展，但从视觉输入推断结构化文化遗产元数据（如创作者、起源、时期）的能力尚未得到充分探索。

Method: 构建多类别跨文化基准数据集；采用LLM-as-Judge框架测量与参考注释的语义对齐；报告精确匹配、部分匹配和属性级别的准确率，并按文化区域进行细分。

Result: 模型只能捕获零散的信号，在不同文化和元数据类型之间存在显著的性能差异，产生不一致且缺乏充分依据的预测。

Conclusion: 当前视觉-语言模型在超越视觉感知推断结构化文化遗产元数据方面存在明显局限。
...
TL;DR: TC-AE是一种基于ViT的深度压缩自编码器架构，通过将token-to-latent压缩分解为两阶段并引入自监督训练来解决潜在表示崩溃问题，在深度压缩下实现了更好的重建和生成效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法在高位压缩比下通常通过增加潜在表示的通道数来维持重建质量，但这往往导致潜在表示崩溃，劣化生成性能。TC-AE旨在从token空间角度解决这一问题，而非依赖日益复杂的架构或多阶段训练方案。

Method: 提出两大创新：(1)将token-to-latent压缩分解为两个阶段，减少结构信息损失，实现有效的token数量缩放；(2)通过联合自监督训练增强图像token的语义结构，生成更友好的潜在表示。

Result: TC-AE在深度压缩条件下实现了显著的重建和生成性能提升。

Conclusion: 该研究提出TC-AE架构，旨在推动ViT tokenizer在视觉生成领域的发展。通过解决token-to-latent压缩中的结构信息损失和潜在表示崩溃问题，实现了深度压缩下的重建和生成性能的显著提升。
...
### [75] [High-Precision Estimation of the State-Space Complexity of Shogi via the Monte Carlo Method](https://arxiv.org/abs/2604.06189)
*Sotaro Ishii,Tetsuro Tanaka*

Main category: cs.AI

TL;DR: 本文通过蒙特卡罗采样结合新颖的逆向可达性测试方法，估计日本将棋的合法局面对数为6.55×10^68，并将该方法应用于迷你将棋，估计其复杂度约为2.38×10^18。


<details>
  <summary>Details</summary>
Motivation: 解决将棋状态空间复杂度的估计难题，之前的估计存在五个数量级的巨大差距(10^64到10^69)，难以区分合法可达局面与大量有效棋盘配置。

Method: 结合蒙特卡罗采样与新型可达性测试，使用反向搜索至"王将仅"局面集合而非单一目标逆向搜索，显著降低确定不可达性的搜索工作量，基于50亿局面样本进行统计估计。

Result: 估计将棋合法局面数为6.55×10^68（三位有效数字），具有3σ置信度；迷你将棋复杂度约为2.38×10^18。

Conclusion: 该方法大幅提升了之前已知界限的精度，成功解决了将棋状态空间复杂度的精确估计问题，为棋类复杂性研究提供了新的统计方法论。
...
### [76] [Blind Refusal: Language Models Refuse to Help Users Evade Unjust, Absurd, and Illegitimate Rules](https://arxiv.org/abs/2604.06233)
*Cameron Pattison,Lorenzo Manuali,Seth Lazar*

Main category: cs.AI

TL;DR: 研究发现语言模型在面对帮助打破规则请求时存在"盲目拒绝"现象——即使能识别规则本身的不合理性，仍会系统性地拒绝帮助。测试显示75.4%的失效规则请求被拒绝，模型的法律推理能力与拒绝行为相脱节。


<details>
  <summary>Details</summary>
Motivation: 安全训练使语言模型系统性地拒绝帮助打破规则的请求，但这种拒绝忽视了规则本身是否值得遵守——存在不合法、不合理或应允许例外的规则。模型在面对此类"失效规则"时仍盲目拒绝，反映出道德推理的缺陷。

Method: 构建了涵盖5种规则失效类型和19种权威类型的合成数据集，通过自动化质量检验和人工审核验证；使用GPT-5.4作为评判器，对18种模型配置进行盲评，分类响应类型（提供帮助/硬拒绝/回避）和规则失效识别情况。

Result: 75.4%（N=14,650）的失效规则请求被拒绝；57.5%的情况下模型能识别失效条件但仍拒绝帮助；拒绝行为与安全或双重用途担忧无关。

Conclusion: 语言模型存在"盲目拒绝"现象：即使能够识别规则的不合理或可例外情况，仍在75.4%的情况下拒绝帮助打破规则。这表明模型的拒绝行为与其规范推理能力相脱节，模型难以灵活判断规则本身的合法性并据此调整行为。
...
### [77] [Toward Reducing Unproductive Container Moves: Predicting Service Requirements and Dwell Times](https://arxiv.org/abs/2604.06251)
*Elena Villalobos,Adolfo De Unánue T.,Fernanda Sobrino,David Aké,Stephany Cisneros,Jorge Lecona,Alejandra Matadamaz*

Main category: cs.AI

TL;DR: 该研究在集装箱码头实施数据科学方法，通过机器学习模型预测集装箱的清关前处理服务需求和停留时间，以减少非生产性集装箱移动，优化堆场运营的资源配置和战略规划。


<details>
  <summary>Details</summary>
Motivation: 在集装箱码头运营中，存在大量的非生产性集装箱移动操作，需要通过预测服务需求和集装箱停留时间来减少这些无效操作。

Method: 开发并评估了机器学习模型，利用历史运营数据预测哪些集装箱在货物放行前需要清关前处理服务，并估算其在码头的停留时间。数据准备过程中，对货物描述进行了分类系统实施，并对收货人记录进行了去重处理以提高数据一致性和特征质量。采用了多个时间验证周期进行模型评估。

Result: 所提出的模型在多个时间验证周期内，在精确率和召回率方面持续优于现有的基于规则的启发式方法和随机基线。

Conclusion: 研究证明了预测分析在集装箱码头物流中提升运营效率和支撑数据驱动决策的实用价值，所提出的模型在精确率和召回率上持续优于基于规则的启发式方法和随机基线。
...
### [78] [Weakly Supervised Distillation of Hallucination Signals into Transformer Representations](https://arxiv.org/abs/2604.06277)
*Shoaib Sadiq Salehmohamed,Jinal Prashant Thakkar,Hansika Aredla,Shaik Mohammed Omar,Shalmali Ayachit*

Main category: cs.AI

TL;DR: 提出将外部幻觉检测监督蒸馏到LLM内部表示的方法，通过弱监督框架结合三种接地信号标注数据，训练探针分类器从隐藏状态检测幻觉，验证了幻觉检测信号可内化到模型表示中的假设。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)中的幻觉问题严重影响其可靠性。现有检测方法依赖于推理时的外部验证(需要标准答案、检索系统或辅助判断模型)，作者探索是否可以将这种外部监督蒸馏到模型自身的表示中，实现仅从内部激活就能检测幻觉。

Method: 提出了一个弱监督框架，结合三种互补的接地信号：子字符串匹配、句子嵌入相似度、LLM作为判断器来标记生成的响应为接地或幻觉。基于此框架构建了来自SQuAD v2的15000样本数据集(10500训练/开发样本和5000测试样本)。训练了五种探针分类器：ProbeMLP(M0)、LayerWiseMLP(M1)、CrossLayerTransformer(M2)、HierarchicalTransformer(M3)和CrossLayerAttentionTransformerV2(M4)。

Result: Transformer探针获得最强区分能力，M2在5折平均AUC/F1上表现最佳，M3在单折验证和保留测试集上表现最佳。探针延迟范围为0.15-5.62ms(批处理)和1.55-6.66ms(单样本)，端到端生成加探针吞吐量约为0.231查询/秒，表明实际开销可忽略不计。

Conclusion: 假设得到支持——幻觉检测信号可以蒸馏到transformer表示中，实现推理时仅从内部激活检测幻觉，无需外部验证。这一方法为构建更可靠的LLM提供了新思路。
...
### [79] [SymptomWise: A Deterministic Reasoning Layer for Reliable and Efficient AI Systems](https://arxiv.org/abs/2604.06375)
*Isaac Henry,Avery Byrne,Christopher Giza,Ron Henry,Shahram Yazdani*

Main category: cs.AI

TL;DR: SymptomWise框架将语言理解与诊断推理分离，结合专家策划的医学知识、确定性法典驱动推理和受约束的大型语言模型使用，以提升AI症状分析系统的可靠性、可解释性和可追溯性。


<details>
  <summary>Details</summary>
Motivation: AI驱动的症状分析系统在可靠性、可解释性和幻觉方面面临持续挑战。端到端生成方法缺乏可追溯性，可能在安全关键场景中产生不支持或不一致的诊断输出。

Method: 该系统将自由文本输入映射到经验证的症状表示，然后由在有限假设空间上运行的确定性推理模块进行评估，生成排序的鉴别诊断。语言模型仅用于症状提取和可选解释，不用于诊断推理。

Result: 在42个专家编写的具有挑战性的儿科神经学病例的初步评估中，与临床医生共识有意义的重叠，88%的病例正确诊断出现在前五位鉴别诊断中。

Conclusion: 该架构提高了可追溯性，减少了不支持的结论，支持系统组件的模块化评估，并可推广到其他溯因推理领域，或可作为基础模型的确定性结构和路由层。
...
### [80] [ProofSketcher: Hybrid LLM + Lightweight Proof Checker for Reliable Math/Logic Reasoning](https://arxiv.org/abs/2604.06401)
*Kranthi Kommuru,Kunal Khanvilkar,Gaurav Parekh*

Main category: cs.AI

TL;DR: 本文针对大语言模型在数学证明中易犯隐蔽错误、而交互式定理证明器形式化要求过高的困境,提出混合管道方案:让LLM生成紧凑DSL中的类型化证明草稿,再由轻量级可信内核扩展为明确证明义务。


<details>
  <summary>Details</summary>
Motivation: 纯LLM证明易含隐蔽错误难以检测;纯形式化定理证明器(如Lean/Coq)虽可靠但要求完全形式化,代价高昂且需要大量低-level细节。

Method: 设计混合管道:LLM在紧凑领域特定语言(DSL)中生成类型化证明草稿，轻量级可信内核将草稿扩展为明确的证明义务。

Result: 实现LLM生成与可信内核验证的结合,兼具LLM的灵活性和形式化证明的可靠性。

Conclusion: 本文提出了一种混合管道方法，平衡了LLM的灵活性和形式化定理证明器的可靠性，为AI辅助定理证明开辟了新途径。
...
### [81] [BDI-Kit Demo: A Toolkit for Programmable and Conversational Data Harmonization](https://arxiv.org/abs/2604.06405)
*Roque Lopez,Yurong Liu,Christos Koutras,Juliana Freire*

Main category: cs.AI

TL;DR: BDI-Kit是一个用于数据协调的可扩展工具包，提供模式匹配和值匹配功能，具有Python API和AI辅助聊天两种接口。


<details>
  <summary>Details</summary>
Motivation: 数据协调是整合分析的主要瓶颈，因为模式、值表示和领域特定约定存在异质性。

Method: 通过Python API或AI辅助聊天界面两种方式，让用户能够迭代探索、验证和优化模式与值匹配，结合自动匹配、AI辅助推理和用户驱动的优化。

Result: 展示了两个场景：(i)使用Python API编程组合原語、检查中间输出并复用转换；(ii)通过自然语言与AI助手对话来访问功能并根据建议迭代优化。

Conclusion: BDI-Kit为开发者提供了编程构建协调流水线的手段，同时为领域专家提供了通过自然语言对话进行数据协调的途径。
...
### [82] [Rethinking Generalization in Reasoning SFT: A Conditional Analysis on Optimization, Data, and Model Capability](https://arxiv.org/abs/2604.06628)
*Qihan Ren,Peng Wang,Ruikun Cai,Shuai Shao,Dadi Guo,Yuejin Xie,Yafu Li,Quanshi Zhang,Xia Hu,Jing Shao,Dongrui Liu*

Main category: cs.AI

TL;DR: 本文挑战了"监督微调(SFT)只能记忆，强化学习(RL)才能泛化"的主流观点。研究发现，长链式思考(CoT)的推理SFT确实具备跨领域泛化能力，但这种能力是有条件的，取决于优化动态、训练数据和基础模型能力三者共同作用。


<details>
  <summary>Details</summary>
Motivation: 现有观点普遍认为SFT仅导致记忆而RL才能泛化，本文针对这一叙事提出质疑，试图验证推理SFT在跨领域泛化方面的真实表现。

Method: 通过长CoT监督的推理SFT实验，系统性地考察优化动态（训练时长）、数据质量与结构、以及基础模型能力三个维度对跨领域泛化的影响。

Result: 1)优化动态：存在"先降后升"的dip-and-recovery模式，短期训练低估了泛化能力；2)数据质量：低质量解决方案广泛损害泛化，经验证的优质长CoT轨迹可带来一致收益；3)模型能力：强模型能内化可迁移的程序化模式（如回溯），弱模型仅模仿表面冗辞；4)泛化不对称性：推理能力提升但安全性下降。

Conclusion: 推理SFT的泛化能力是有条件的，取决于优化程度、数据质量与模型强度，且需付出安全性下降的代价。问题不在于SFT能否泛化，而在于"在何种条件下、以何种代价"实现泛化。
...
### [83] [Reasoning Fails Where Step Flow Breaks](https://arxiv.org/abs/2604.06695)
*Xiaoyu Xu,Yulan Pan,Xiaosong Yuan,Zhihong Shen,Minghao Su,Yuanhao Su,Xiaofeng Zhang*

Main category: cs.AI

TL;DR: 本文提出了Step-Saliency工具来分析长链思考推理模型的信息流问题，并基于发现的问题设计了StepFlow干预方法，在不需重新训练的情况下提升模型在数学、科学和编码任务上的准确率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型虽然能生成很长的思考链，但在多步数学、科学和编码任务上表现良好，其行为仍不稳定且难以解释。现有分析工具无法有效处理这种长而结构化的推理痕迹。

Method: 提出了Step-Saliency工具，将注意力-梯度分数沿问题-思考-摘要轨迹聚合成步骤到步骤的映射。通过该工具发现了两个信息流失败模式：浅层锁定(Shallow Lock-in)和深层衰减(Deep Decay)。基于这些发现，提出了StepFlow干预方法，包括通过Odds-Equal Bridge调整浅层显著性模式，以及通过Step Momentum Injection在深层添加步级残差。

Result: StepFlow在多个大型推理模型的数学、科学和编码任务上提升了准确率，且无需重新训练。

Conclusion: 修复信息流可以恢复模型缺失的部分推理性能，表明对推理过程的干预是有效的。
...
### [84] [AgentGate: A Lightweight Structured Routing Engine for the Internet of Agents](https://arxiv.org/abs/2604.06696)
*Yujun Cheng,Enfang Cui,Hao Qin,Zhiyuan Liang,Qi Xu*

Main category: cs.AI

TL;DR: AgentGate是一个轻量级结构化路由引擎，将路由问题分解为动作决策和结构 grounding 两个阶段，采用候选感知监督和硬负例进行微调，使3B-7B紧凑模型能在资源约束下实现高效智能体调度。


<details>
  <summary>Details</summary>
Motivation: AI智能体系统快速发展形成"Internet of Agents"，但高效请求分发在延迟、隐私和成本约束下仍是开放系统问题。虽然近期工作改进了智能体命名、发现和交互，但请求分发效率仍有待提升。

Method: 将路由问题分解为两个阶段：1）动作决策阶段：确定查询应触发单智能体调用、多智能体规划、直接响应还是安全升级；2）结构 grounding 阶段：将选定的动作实例化为可执行输出。提出候选感知监督和硬负例的路由导向微调方案。

Result: 在包含多个3B-7B开源模型的路由基准测试上实验表明，紧凑模型能够在约束设置下提供具有竞争力的路由性能。模型差异主要体现在动作预测、候选选择和结构 grounding 质量三个方面。

Conclusion: 结构化路由是高效和隐私感知智能体系统的可行设计点，特别是在资源受限的部署条件下做出路由决策时。紧凑模型（3B-7B）能够在约束设置下提供具有竞争力的路由性能，模型差异主要体现在动作预测、候选选择和结构 grounding 质量上。
...
### [85] [ATANT: An Evaluation Framework for AI Continuity](https://arxiv.org/abs/2604.06710)
*Samuel Sameer Tanguturi*

Main category: cs.AI

TL;DR: ATANT是一个用于评估AI系统连续性的开源框架，定义了7个必需属性和10个检查点评估方法，可在无LLM参与的情况下测量系统持久、更新、消歧和重建上下文的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI行业虽已产生多种记忆组件（RAG管道、向量数据库、长上下文窗口、profile层），但缺乏正式定义和衡量这些组件是否真正产生连续性的框架。

Method: 定义连续性为7个必需属性的系统属性；引入10检查点评估方法（评估循环中无LLM）；使用包含250个故事、1835个验证问题、涵盖6个生命领域的叙事测试语料库；对参考实现进行5次测试迭代。

Result: 从58%（遗留架构）提升到100%（250个故事隔离模式）；100%（50个故事累积模式）；96%（250个故事累积规模）。

Conclusion: ATANT是系统无关的、模型无关的 sequenced methodology，可用于构建和验证连续性系统。累积结果（250个故事累积规模达到96%）是主要衡量指标。
...
### [86] [Steering the Verifiability of Multimodal AI Hallucinations](https://arxiv.org/abs/2604.06714)
*Jianhong Pang,Ruoxi Cheng,Ziyi Ye,Xingjun Ma,Zuxuan Wu,Xuanjing Huang,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: 本文研究了多模态大语言模型（MLLMs）产生的幻觉问题，首次提出了根据人类用户验证难度将幻觉分为"明显幻觉"和"模糊幻觉"两类，并提出了一种基于激活空间干预的方法来控制模型的验证性。


<details>
  <summary>Details</summary>
Motivation: 当前研究忽视了多模态AI幻觉在验证难度上的显著差异。现有方法无法精细控制AI应用在不同安全和可用性需求下的验证性。构建一个能够区分明显幻觉和模糊幻觉的数据集，并实现细粒度控制具有重要理论和实践价值。

Method: 1) 构建包含4,470个人类回复的幻觉数据集，基于人类用户的验证难度将幻觉分类为明显和模糊两种类型；2) 提出激活空间干预方法，分别学习明显和模糊幻觉的独立探针；3) 通过干预探针实现对模型验证性的精细控制。

Result: 实验表明：(1) 明显幻觉和模糊幻觉激活不同的干预探针，支持细粒度控制；(2) 针对性干预在调节相应验证性方面表现优异；(3) 简单混合这些干预即可实现不同场景下灵活多变的验证性控制。

Conclusion: 本文首次系统性地研究了MLLM幻觉的可验证性问题，提出了创新性的激活空间干预框架。实验结果证明了方法的有效性，为构建不同安全性和可用性需求的AI应用提供了新思路。该工作填补了AI幻觉验证性控制领域的空白。
...
### [87] [TurboAgent: An LLM-Driven Autonomous Multi-Agent Framework for Turbomachinery Aerodynamic Design](https://arxiv.org/abs/2604.06747)
*Juan Du,Yueteng Wu,Pan Zhao,Yuze Liu,Min Zhang,Xiaobin Xu,Xinglong Zhang*

Main category: cs.AI

TL;DR: TurboAgent：基于 LLM 的多智能体自主框架，实现 turbomachinery 气动设计的端到端自动化


<details>
  <summary>Details</summary>
Motivation: 现有 turbomachinery 气动设计智能方法通常聚焦于单独阶段或依赖松耦合流水线，难以实现完全自主的端到端设计

Method: 提出 TurboAgent，一个 LLM 驱动的自主多智能体框架，包含任务规划与协调中心，以及生成式设计、快速性能预测、多目标优化和物理验证等专用智能体

Result: R² 对于质量流量、总压比和等熵效率均超过 0.91，归一化 RMSE 低于 8%；优化后等熵效率提升 1.61%，总压比提升 3.02%；完整工作流约 30 分钟完成

Conclusion: TurboAgent 实现了从自然语言需求到最终设计生成的自主闭环设计，提供了高效且可扩展的 turbomachinery 气动设计范式
...
### [88] [What's Missing in Screen-to-Action? Towards a UI-in-the-Loop Paradigm for Multimodal GUI Reasoning](https://arxiv.org/abs/2604.06995)
*Songze Li,Xiaoke Guo,Tianqi Liu,Biao Yi,Zhaoyan Gong,Zhiqiang Liu,Huajun Chen,Wen Zhang*

Main category: cs.AI

TL;DR: 本文提出UI-in-the-Loop(UILoop)GUI推理范式，将推理过程视为循环的屏幕-UI元素-动作过程，使MLLMs能显式学习UI元素的定位、语义和功能，并构建了26K样本的UI Comprehension-Bench基准数据集，在UI理解和GUI推理任务上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有GUI推理任务仍具挑战性，当前方法依赖直接基于屏幕的决策，缺乏可解释性且忽略了对UI元素的全面理解，最终导致任务失败。

Method: 提出UI-in-the-Loop(UILoop)范式，将GUI推理任务视为循环的"屏幕-UI元素-动作"过程，使多模态大语言模型(MLLMs)能够显式学习关键UI元素的定位、语义功能和实际使用。

Result: UILoop在UI理解任务上达到最先进水平，同时在GUI推理任务中产生优越结果。

Conclusion: UILoop方法通过显式学习UI元素的定位、语义功能和实际使用，实现了精确的元素发现和可解释的推理，在GUI推理任务中取得了优越的结果。
...
### [89] [EmoMAS: Emotion-Aware Multi-Agent System for High-Stakes Edge-Deployable Negotiation with Bayesian Orchestration](https://arxiv.org/abs/2604.07003)
*Yunbo Long,Yunhan Liu,Liming Xu*

Main category: cs.AI

TL;DR: EmoMAS 是一个贝叶斯多智能体框架，通过协调博弈论、强化学习和心理一致性三个专业智能体，将情感决策战略性化，使小型和大型语言模型都能在隐私敏感的边缘设备上实现高性能且符合伦理的谈判


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在自动化谈判中广泛应用，但其高计算成本和隐私风险限制了移动助手或救援机器人等隐私敏感的边缘设备部署；小型语言模型（SLM）虽具可行性，却在高度 stakes 谈判中的复杂情感动态处理上存在困难

Method: 提出 EmoMAS 框架：一个贝叶斯多智能体系统，将情感决策从反应式转变为战略性。该框架利用贝叶斯编排器协调三个专业智能体（博弈论模型、强化学习模型和心理一致性模型），融合实时洞察以优化情感状态转换，并基于谈判反馈持续更新智能体可靠性。该混合智能体架构支持在线策略学习，无需预训练。

Result: 在四个高 stakes、可边缘部署的谈判基准测试（债务、医疗保健、应急响应、教育领域）中，通过广泛的智能体间模拟，配备 EmoMAS 的 SLM 和 LLM 均一致超越所有基线模型，并在平衡伦理行为的同时提升谈判表现

Conclusion: 战略性情感智能是谈判成功的关键驱动因素。通过将情感表达视为贝叶斯多智能体优化框架中的战略性变量，EmoMAS 为适合高 stakes 边缘部署的有效、私密且自适应的谈判 AI 确立了新范式
...
### [90] [CAFP: A Post-Processing Framework for Group Fairness via Counterfactual Model Averaging](https://arxiv.org/abs/2604.07009)
*Irina Arévalo,Marcos Oliva*

Main category: cs.AI

TL;DR: 提出了一种名为CAFP的模型无关后处理方法，通过对原始输入和反事实输入的预测进行平均来减轻敏感属性的不公平影响，无需重新训练或修改原始分类器，理论上可实现人口统计均等并减少均等几率差距。


<details>
  <summary>Details</summary>
Motivation: 确保机器学习预测的公平性是一个关键挑战，特别是在信用评分、医疗保健和刑事司法等敏感领域。现有的公平性干预方法通常依赖于数据预处理或训练期间的算法约束，这些方法需要完全控制模型架构和访问受保护属性信息，而在现实世界的系统中这些条件往往不可行。

Method: CAFP通过生成每个输入的反事实版本（翻转敏感属性），然后对事实和反事实实例的预测进行平均来运作。这是一种模型无关的后处理方法，不需要重新训练或修改原始分类器。

Result: 理论分析表明，CAFP能够消除对受保护属性的直接依赖，降低预测与敏感属性之间的互信息，并可证明地限制相对于原始模型的扭曲。在温和假设下，CAFP实现完美的人口统计均等，并将均等几率差距减少至少50%的平均反事实偏差。

Conclusion: CAFP作为一个模型无关的后处理方法，能够消除对受保护属性的直接依赖，降低预测与敏感属性之间的互信息，并在温和假设下实现完美的人口统计均等和至少减少50%的均等几率差距。

这种方法的优势在于它不需要修改原始分类器，也不需要访问受保护属性信息，使得它特别适合部署在无法控制模型架构或敏感信息的实际系统中。
...
### [91] [A-MBER: Affective Memory Benchmark for Emotion Recognition](https://arxiv.org/abs/2604.07017)
*Deliang Wen,Ke Sun,Yu Wang*

Main category: cs.AI

TL;DR: 论文提出A-MBER基准，用于评估AI助手利用长期交互记忆理解用户当前情感状态的能力。


<details>
  <summary>Details</summary>
Motivation: 现有情感数据集主要评估即时或局部情感，缺乏长期记忆测试；现有记忆基准关注事实回忆而非情感解释，无法验证模型是否能够利用交互历史理解用户当前情感状态。

Method: 通过多阶段管道构建A-MBER基准，包括长期规划、对话生成、注释、问题构建和打包。支持判断、检索、解释任务及鲁棒性测试。实验比较局部上下文、长上下文、检索记忆、结构化记忆和黄金证据等条件。

Result: A-MBER在长程隐含情感、高依赖记忆水平、基于轨迹推理和对抗性设置等压力子集上显示出强区分能力。记忆支持情感解释的关键在于选择性、grounded和上下文敏感地使用过去交互，而非仅提供更多历史。

Conclusion: 该研究填补了情感识别在长期记忆方面的评估空白，证明了记忆对情感理解的价值在于其能支持有选择性的、基于证据的上下文敏感推理。
...
### [92] [Planning Task Shielding: Detecting and Repairing Flaws in Planning Tasks through Turning them Unsolvable](https://arxiv.org/abs/2604.07042)
*Alberto Pozanco,Marianela Morales,Pietro Totis,Daniel Borrajo*

Main category: cs.AI

TL;DR: 本文引入规划任务屏蔽问题，解决如何检测并修复规划任务中的缺陷，通过最小修改动作使规划任务无解来避免到达错误状态。


<details>
  <summary>Details</summary>
Motivation: 传统规划研究关注生成达成目标的计划，但当目标规格编码了不应成立的条件时，需要修改规划任务以防止到达错误状态，而非简单生成避免错误状态的计划。

Method: 提出allmin算法，通过最小修改原始动作集合，使原本可解的规划任务变得无解，从而实现屏蔽。

Result: 实验表明allmin能有效屏蔽规划任务，随着任务规模增大仍保持有效性。

Conclusion: allmin是解决规划任务屏蔽问题的最优算法，能够通过最小修改动作使规划任务无解，实现对错误状态的防护。
...
### [93] [EVGeoQA: Benchmarking LLMs on Dynamic, Multi-Objective Geo-Spatial Exploration](https://arxiv.org/abs/2604.07070)
*Jianfei Wu,Zhichun Wang,Zhensheng Wang,Zhiyu He*

Main category: cs.AI

TL;DR: 本文引入EVGeoQA基准测试，用于评估大型语言模型在动态地理空间环境中的目标驱动探索能力，特别关注电动汽车充电场景中的位置绑定和双目标设计。


<details>
  <summary>Details</summary>
Motivation: 现有地理空间问答基准主要关注静态检索，无法捕捉现实世界规划中的动态用户位置和复合约束的复杂性。LLMs在动态地理空间环境中的目的驱动探索潜力尚未得到充分研究。

Method: 提出GeoRover评估框架，基于工具增强的智能体架构，系统评估LLMs在复杂地理空间设置中的动态多目标探索能力。实验在EVGeoQA基准上进行。

Result: 实验表明LLMs成功利用工具解决子任务，但在长距离空间探索方面存在困难。同时发现LLMs具有总结历史探索轨迹以提高探索效率的 emergent capability。

Conclusion: EVGeoQA作为具有挑战性的地理空间智能测试平台，为未来地理空间智能研究提供了重要的基准和方向。
...
### [94] [Reason in Chains, Learn in Trees: Self-Rectification and Grafting for Multi-turn Agent Policy Optimization](https://arxiv.org/abs/2604.07165)
*Yu Li,Sizhe Tang,Tian Lan*

Main category: cs.AI

TL;DR: T-STAR是一种针对大语言模型智能体的树结构自我教学修正框架，通过认知树整合轨迹并识别关键步骤，解决了强化学习中稀疏奖励和步骤级信用分配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如GRPO）将采样轨迹视为独立链，对链中所有步骤均匀分配信用，忽略了关键步骤对推理结果的不成比例影响，且未利用轨迹间的潜在关联结构。

Method: T-STAR框架包含三个核心组件：1) 认知树机制将相似步骤合并为统一树结构；2) 内省估值机制通过树结构反向传播轨迹级奖励获得低方差的相对优势；3) 对比式上下文思维嫁接在关键分歧点对比成功与失败分支；4) 布拉德斯特里型外科策略优化针对关键步骤集中优化策略梯度。

Result: 在嵌入式、交互式、推理和规划基准测试中，T-STAR相比强基线方法实现了一致的性能提升，且提升在使用较长推理链的任务上最为显著。

Conclusion: T-STAR通过认知树结构成功解决了传统方法中轨迹奖励均匀分配的问题，实现了关键步骤的精准优化，在需要长链推理的任务上提升最为显著。
...
### [95] [The Josehedron: A space-filling plesiohedron based on the Fischer-Koch S Triply Periodic Minimal Surface](https://arxiv.org/abs/2604.07160)
*Mathias Bernhard*

Main category: cs.CG

TL;DR: 报道了基于Fischer-Koch S TPMS极值点的新型空间填充多面体Josehedron,具有12个面、12个顶点和22条边,可填满三维空间,并提出从任意周期函数或TPMS寻找SFPH的通用方法,额外发现7个新SFPH


<details>
  <summary>Details</summary>
Motivation: 发现一种基于Fischer-Koch S TPMS极值点的新型空间填充多面体，探索其几何性质并扩展到其他TPMS

Method: 从Fischer-Koch S三重周期最小曲面(TPMS)的极值点推导新型空间填充多面体(Josehedron)，并提出基于周期函数/TPMS寻找SFPH的一般方法

Result: Josehedron是一个具有12个面(4个等腰三角形和8个镜像对称四边形)、12个顶点和22条边的plexiohedron;每个立方体单位胞包含12个实例,有6种不同方向;生成点集与五边形Cairo tiling在投影到任何坐标平面时存在显著联系;具有整数顶点坐标、交织迷宫和手性对称性

Conclusion: 提出了一种基于任何周期函数、TPMS或其它函数寻找新型SFPH的通用方法，并展示了7个额外的新型SFPH
...
### [96] [Intertemporal Demand Allocation for Inventory Control in Online Marketplaces](https://arxiv.org/abs/2604.07312)
*Rene Caldentey,Tong Xie*

Main category: cs.MA

TL;DR: 研究平台如何通过跨时间的需求分配策略来影响卖家的库存决策，无需直接控制库存。平台通过改变每个卖家销售流的可预测性来影响其安全库存需求。


<details>
  <summary>Details</summary>
Motivation: 在线市场平台不仅匹配买卖双方，还提供履约服务使卖家库存成为平台收入来源。平台希望通过需求分配影响卖家库存选择，但不直接控制库存。

Method: 构建模型：平台观察总需求、在时间维度上分配订单给不同卖家；卖家在FBM和FBP两种履约方式间选择，根据状态依赖的基点库存策略补货。核心机制是信息机制：通过改变销售流的可预测性来改变安全库存需求。在同一类非歧视性分配策略中研究（给予相同需求份额和预测风险）。

Result: 统一分配（均匀分割）最小化预测不确定性；任何高于统一基准的不确定性可通过简单低内存分配规则实现；提高不确定性需要防止卖家从自身销售历史推断总需求的路由规则。将平台问题简化为选择预测不确定性水平，权衡平台履约的采用率与采用者的库存水平。

Conclusion: 需求分配是数字市场中强大的运营和信息设计杠杆。平台可通过控制需求分配的不确定性程度来平衡履约服务的采用与库存成本。
