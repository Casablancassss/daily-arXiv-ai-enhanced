...
TL;DR: 提出一种全自动AI系统，通过Agent种群协作和基于YouTube喜剧视频训练的LLM批评者，自动生成类似SNL的短喜剧视频。


<details>
  <summary>Details</summary>
Motivation: 自动化生成类似《周六夜现场》风格的短喜剧视频是一个具有挑战性的任务，需要同时处理创意生成、角色一致性和幽默评估等多个方面。

Method: 采用基于真实制作工作室角色的Agent种群，通过迭代竞争、评估和改进来优化想法和输出的质量和多样性。使用LLM批评者，通过分析YouTube上的喜剧视频语料库来自动评估幽默感，与真实观众偏好保持一致。

Result: 实验表明，该框架生成的短剧质量接近专业制作的水平，同时在视频生成方面展示了最先进的性能。

Conclusion: 该框架生成的视频质量接近专业制作的短剧，同时在视频生成方面展示了最先进的性能。
...
TL;DR: 本文提出利用非平衡Wasserstein重心学习高光谱图像的低维表示，结合谱聚类实现无监督标签学习，提高了聚类的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像标注工作量大，传统统计方法难以处理；基于字典学习的Wasserstein空间聚类方法需要平衡光谱轮廓，导致类别模糊且对异常值和噪声的鲁棒性较差。

Method: 提出利用非平衡Wasserstein重心学习高光谱数据的低维表示，然后在该表示上部署谱聚类进行无监督学习。

Result: 所提方法在保持类别区分度的同时，提高了對异常值和噪声的鲁棒性，实现了有效的无监督聚类。

Conclusion: 使用非平衡Wasserstein重心学习低维表示结合谱聚类，为高光谱图像的无监督标签学习提供了一种更鲁棒的有效方法。
...
TL;DR: 本文提出Delta-K框架，通过在共享的交叉注意力Key空间中操作，利用Vision-language model提取缺失概念的差异键ΔK，在扩散过程早期注入以建立语义锚点，解决Diffusion Models在复杂多实例场景中的概念遗漏问题。该方法具有即插即用、无需训练、通用性强的特点。


<details>
  <summary>Details</summary>
Motivation: 现有Diffusion Models在合成复杂多实例场景时经常出现概念遗漏问题。已有的免训练方法试图通过重新缩放注意力图来解决，但这只是加剧了无结构噪声，未能建立连贯的语义表示。

Method: 提出Delta-K框架，通过Vision-language model提取缺失概念的差异键ΔK，在扩散过程的早期语义规划阶段注入该信号。采用动态优化的调度机制来平衡新概念与现有概念的关系，将噪声转化为稳定的结构锚点。

Result: Delta-K在现代DiT模型和经典U-Net架构上都能持续改善组合对齐性能，且无需空间掩码、额外训练或架构修改。

Conclusion: Delta-K是一个通用的、无需训练的即插即用推理框架，能够在不修改模型架构、不使用空间掩码的情况下，有效解决Diffusion Models在复杂多实例场景中的概念遗漏问题。该方法在DiT和U-Net架构上都展现出了良好的通用性和有效性。
...
TL;DR: OilSAM2是一个针对无序SAR图像的油溢分割记忆增强框架，采用层次化特征感知多尺度记忆库和结构语义一致性记忆更新策略，在两个公共SAR油溢数据集上实现了最先进的分割性能。


<details>
  <summary>Details</summary>
Motivation: SAR图像油溢分割面临严重的外观变异、尺度异质性和真实监测场景中时间连续性缺失的挑战。现有基于SAM的方法只能处理单张图像，而SAM2等记忆增强变体假设时间一致性，当应用于无序SAR图像集合时容易产生语义漂移。

Method: 提出OilSAM2框架，包含两个核心组件：1）层次化特征感知多尺度记忆库，显式建模纹理、结构 和语义级别表征，实现鲁棒的跨图像信息复用；2）结构语义一致性记忆更新策略，基于语义差异和结构变化选择性刷新记忆，有效缓解记忆漂移问题。

Result: 在两个公共SAR油溢数据集上的实验表明，OilSAM2实现了最先进的分割性能，在嘈杂的SAR监测场景下提供稳定准确的分割结果。

Conclusion: OilSAM2为无序SAR油溢监测提供了一种有效的记忆增强分割方案，源代码已开源发布。
...
TL;DR: I2X框架通过从训练过程中提取原型来构建结构化解释，解决深度学习模型黑盒问题。该方法使用后验XAI方法（如GradCAM）量化训练检查点的进度，提供模型决策的结构化视图。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然预测性能优异，但其黑盒性质限制了透明度和可信度。现有XAI方法主要提供显著性图或概念（即非结构化可解释性），且常依赖辅助模型（如GPT、CLIP）描述模型行为，牺牲了对原始模型的忠诚度。

Method: 提出I2X（Interpretability to Explainability）框架，通过使用后验XAI方法（如GradCAM）从训练过程中提取原型，量化选定检查点的进度，提供训练期间类内和类间决策的结构化视图。

Result: 在MNIST和CIFAR10上的实验表明，I2X能有效揭示各种图像分类模型的基于原型的推理过程。此外，I2X可识别不确定原型，通过针对性扰动样本进行微调，最终提高不同模型架构和数据集的预测准确率。

Conclusion: I2X不仅忠实地解释了模型行为，还提供了引导优化朝向期望目标的实用方法，证明了结构化解释在提升模型可解释性和性能方面的有效性。
...
TL;DR: 本文研究类别增量学习中任务大小不平衡的问题，提出One-A框架，通过非对称子空间对齐、信息自适应加权和方向门控机制，在保持恒定推理成本的同时实现对动态任务大小的自适应。


<details>
  <summary>Details</summary>
Motivation: 现有的类别增量学习方法和设置假设任务流是平衡的，即每个任务包含相同数量的类别。但在实际应用中，每个任务的类别数量往往差异很大。作者将这个问题称为"步骤不平衡"（step imbalance），大任务包含更多类别主导学习，而小任务引入不稳定更新。现有CIL方法假设任务平衡，因此统一对待所有任务，产生不平衡更新从而降低整体学习性能。

Method: 提出One-A，一个统一且具有不平衡感知能力的框架，将任务增量更新合并到单个适配器中，保持恒定的推理成本。具体包括：1）非对称子空间对齐：保留大任务学习的优势子空间，同时约束其中的低信息更新；2）信息自适应加权：平衡基础适配器和新适配器的贡献；3）方向门控机制：沿每个奇异方向选择性地融合更新，在头部方向保持稳定性，在尾部方向保持可塑性。

Result: 在多个基准测试和步骤不平衡流上，One-A实现了具有竞争力的准确率，同时具有显著较低的推理开销，表明单个非对称融合的适配器可以同时适应动态任务大小并保持高效部署。

Conclusion: One-A框架成功解决了类别增量学习中的任务大小不平衡问题，通过非对称子空间对齐、信息自适应加权和方向门控三种技术，在保持恒定推理成本的情况下实现了对不同规模任务的良好适应，为实际应用中的类别增量学习提供了有效的解决方案。
...
TL;DR: 本文提出DeepIntuit框架，将开放实例视频分类从模仿范式转变为直觉推理范式。通过冷启动监督对齐、GRPO强化学习和直觉校准三阶段，充分利用视觉语言模型的推理能力，实现更优的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频分类模型在同质数据分布下表现良好，但现实世界的开放实例挑战中类内差异巨大。视觉语言模型(VLMs)虽有更好泛化能力，但未充分利用其推理能力。该研究旨在弥合这一差距，将开放实例视频分类从模仿提升到直觉推理。

Method: 提出DeepIntuit框架，包含三个阶段：1)冷启动监督对齐初始化推理能力；2)使用Group Relative Policy Optimization (GRPO)进行强化学习，提升推理连贯性；3)直觉校准阶段，在VLM生成的推理轨迹上训练分类器，实现稳定知识迁移。

Result: 实验表明，DeepIntuit在开放实例视频分类任务上显著受益于从简单特征模仿向内在推理的转变，超越了传统基于模仿的方法。

Conclusion: DeepIntuit通过将开放实例视频分类从模仿提升到直觉推理，显著优于传统基于特征模仿的方法。框架通过冷启动监督对齐、GRPO强化学习和直觉校准三个阶段，有效解决了分布不匹配问题，实现了稳定的知识迁移。
...
TL;DR: 本文提出Fuel Gauge方法，通过提取隐藏的"燃料"信号来提前预测大型多模态模型的思维链(CoT)长度，以解决CoT过程冗长不可预测导致的内存碎片化和思考不足/过度问题。实验表明该方法在多个基准上有效，预测误差降低一半以上，内存分配频率降低13.37倍。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型依赖的思维链(CoT)过程冗长且不可预测，导致计算资源利用效率低下（内存碎片化）和准确率不理想（思考不足或过度）。

Method: 提出Fuel Gauge方法，提取隐藏的"燃料"信号并提前预测CoT长度。具体包括：1）通过经验观察发现CoT过程遵循简单形式，行为与生成样本无关；2）基于隐藏参数"燃料"来估计CoT长度；3）应用于下游任务：预测性KV缓存分配和CoT长度调制。

Result: 在GPQA-Diamond基准上，Fuel Gauge的CoT长度预测误差不到基线的一半；内存分配频率降低13.37倍。该方法在文本、图像-文本和视频-文本问答基准上均表现出色。

Conclusion: Fuel Gauge能够准确预测CoT长度，在GPQA-Diamond基准上实现了比基线低一半以上的预测误差，并将内存分配频率降低13.37倍。该方法在文本、图像-文本和视频-文本问答基准上展现出了有效性、可泛化性和实用价值。
...
TL;DR: CGVD is a training-free, model-agnostic inference framework that stabilizes VLA policies in cluttered environments by using Fourier-based inpainting to suppress semantic distractors while preserving spatial geometry


<details>
  <summary>Details</summary>
Motivation: VLA models suffer from "Precision-Reasoning Gap" in cluttered environments due to background-induced feature dilution, where high-frequency semantic noise corrupts geometric grounding needed for precise manipulation

Method: Concept-Gated Visual Distillation (CGVD): 1) Parses instructions into safe and distractor sets; 2) Two-layer target refinement combining cross-validation and spatial disambiguation to penalize false positives; 3) Fourier-based inpainting to suppress semantic distractors while preserving spatial geometry and visual proprioception

Result: Achieves 77.5% success rate vs baseline's 43.0% in environments with dense semantic distractors

Conclusion: CGVD establishes inference-time visual distillation as a critical prerequisite for robust robotic manipulation in cluttered environments
...
TL;DR: StyleGallery是一种无需训练、语义感知的图像风格迁移框架，通过语义区域分割、聚类区域匹配和风格迁移优化三个阶段，解决了现有方法在语义鸿沟、依赖额外约束和缺乏自适应全局-局部对齐方面的问题，支持任意参考图像输入和有效个性化定制。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的图像风格迁移方法存在三个主要局限：1)语义鸿沟：风格参考可能缺少适当的内容语义，导致不可控的风格化；2)依赖额外约束（如语义 masks），限制了适用性；3) rigid特征关联缺乏自适应全局-局部对齐，无法平衡细粒度风格化和全局内容保留。这些限制导致风格迁移在个性化、准确性和适应性方面受到根本性制约。

Method: 提出StyleGallery框架，包含三个核心阶段：1)语义区域分割（对潜在扩散特征进行自适应聚类以无需额外输入划分区域）；2)聚类区域匹配（对提取特征进行块过滤以实现精确对齐）；3)风格迁移优化（基于能量函数的扩散采样结合区域风格损失优化风格化）。

Result: StyleGallery在内容结构保留、区域风格化、可解释性和个性化定制方面优于现有最先进方法，特别是在利用多个风格参考时表现出色。

Conclusion: StyleGallery在内容结构保留、区域风格化、可解释性和个性化定制方面优于现有最先进方法，特别是在利用多个风格参考时表现突出。该框架无需训练，支持任意参考图像输入，实现了有效的个性化定制。
...
TL;DR: 本文提出统一框架解决多模态大语言模型幻觉问题，通过视觉令牌的双重作用（增强视觉表征+创建负样本校正偏差），在仅1.06倍推理开销下显著减少物体幻觉


<details>
  <summary>Details</summary>
Motivation: 现有训练-free方法采用分离策略（增强视觉信号或抑制文本惯性），但存在关键权衡：单纯增强视觉无法对抗强语言先验，抑制语言会引入图像无关噪声，简单组合也无效，因此需要统一框架

Method: 设计了两个基于视觉令牌的模块：SVC模块通过整合增强图像的令牌来强化视觉表示；CRC模块通过剪枝令牌创建潜在空间负样本来纠正模型内部偏差。两个模块协同工作实现视觉-语言平衡

Result: 在LLaVA-1.5多个基准上POPE准确率平均提升2%绝对值，推理延迟开销仅1.06倍

Conclusion: 提出了一种统一的视觉-语言平衡框架，通过协同视觉校准(SVC)和因果表示校准(CRC)两个模块，有效减少物体幻觉，在LLaVA-1.5上POPE准确率平均提升2%，推理延迟仅增加1.06倍
...
TL;DR: 提出几何自编码器(GAE)框架，通过VFM语义监督、潜在空间归一化和动态噪声采样机制，解决潜在扩散模型中语义可区分性、重建质量和潜在紧致性的统一问题，在ImageNet-1K 256×256上无需分类器自由引导达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型中的潜在设计大多是启发式的，难以统一语义可区分性、重建保真度和潜在紧致性三个关键目标。

Method: 1.分析各种对齐范式，从VFM构建优化的低维语义监督目标；2.用潜在归一化替换标准VAE的限制性KL散度；3.引入动态噪声采样机制确保高强度噪声下的鲁棒重建。

Result: 在ImageNet-1K 256×256基准上，GAE在仅80个epoch达到gFID 1.82，800个epoch达到gFID 1.31，显著超越现有SOTA方法。

Conclusion: GAE在压缩、语义深度和鲁棒重建稳定性之间建立了更优的平衡，为潜在扩散建模提供了有前景的范式。
...
### [14] [UniPINN: A Unified PINN Framework for Multi-task Learning of Diverse Navier-Stokes Equations](https://arxiv.org/abs/2603.10466)
*Dengdi Sun,Jie Chen,Xiao Wang,Jin Tang*

Main category: cs.CV

TL;DR: UniPINN是一种统一多流PINN框架，通过共享-专用架构、跨流注意力和动态权重分配三大组件，解决多流学习中物理约束捕获、负迁移和训练不稳定三大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有PINN方法主要针对单流场景设计，扩展到多流场景时面临三个关键挑战：难以同时捕捉共享物理原理和流特定特征、易受任务间负迁移影响导致预测精度下降、以及异构流态间损失量级差异导致的训练不稳定。

Method: 提出UniPINN框架，包含三个核心组件：(1) 共享-专用架构解耦通用物理定律与流特定特征；(2) 跨流注意力机制选择性强化相关模式并抑制任务无关干扰；(3) 动态权重分配策略自适应平衡损失贡献以稳定多目标优化。

Result: 在三个典型流场上的广泛实验表明，UniPINN有效统一多流学习，实现卓越预测精度和异构流态间的均衡性能，同时成功缓解负迁移问题。

Conclusion: UniPINN通过共享-专用架构、跨流注意力机制和动态权重分配策略，有效统一多流学习，在异构流态下实现卓越预测精度，成功缓解负迁移问题。
...
TL;DR: 本论文提出了一种让多模态大语言模型具备感知不足自我意识的新框架，使其能在2D线索不足时自主调用几何特征进行推理，无需盲目注入几何信息。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型(MLLMs)在空间理解方面存在局限，而现有方法往往盲目地向每个输入注入几何信号，忽视实际需求并造成计算开销。

Method: 1. 引入独立的几何输入通道并进行对齐训练，使模型能有效利用几何特征；2. 构建空间感知监督微调数据集，激活模型的内部线索，使其能自主判断几何信息的必要性。

Result: 在多个空间推理基准测试中验证了该方法的有效性，实现了显著的空间推理提升，同时不损害2D视觉推理能力。

Conclusion: 该框架为实现更强大、高效和自我感知的多模态智能提供了路径，在不损害2D视觉推理能力的情况下实现了显著的空间推理提升。
...
### [16] [Prompting with the human-touch: evaluating model-sensitivity of foundation models for musculoskeletal CT segmentation](https://arxiv.org/abs/2603.10541)
*Caroline Magg,Maaike A. ter Wee,Johannes G. G. Dobbe,Geert J. Streekstra,Leendert Blankevoort,Clara I. Sánchez,Hoel Kervadec*

Main category: cs.CV

TL;DR: 本研究对11种可提示基础模型在骨骼和植入物分割任务中进行了系统评测，比较了2D和3D提示策略，揭示了模型性能差异、人体提示与理想提示的差距，以及模型对提示变化的敏感性。


<details>
  <summary>Details</summary>
Motivation: 随着自然图像分割中可提示基础模型的出现，医学图像分割也经历了革命性变化。然而，不同模型在数据集、指标和比较基准上的评估结果差异较大，导致难以直接比较模型性能，也给临床任务选择最合适的模型带来困难。

Method: 研究使用了非迭代2D和3D提示策略，在专注于骨骼和植入物分割的私有和公共数据集上测试了11种可提示基础模型（覆盖手腕、肩部、髋部和下肢四个解剖区域）。通过专用观察研究收集人体提示，识别Pareto最优模型并进行进一步分析。

Result: 主要发现：1)不同模型和提示策略的分割性能差异显著；2D中SAM和SAM2.1为Pareto最优，3D中nnInteractive和Med-SAM2为Pareto最优；3)定位准确性和评分者一致性因解剖结构而异，简单结构（腕骨）一致性较高，复杂结构（骨盆、胫骨、植入物）一致性较低；4)使用人体提示时性能下降，表明基于参考标签提取的"理想"提示可能高估了实际人体驱动场景中的性能；5)所有模型对提示变化敏感，仅两个模型表现出评分者内鲁棒性，但无法推广到评分者间设置。

Conclusion: 在人体驱动场景中选择最優基础模型仍然具有挑战性，即使高性能模型对人体输入提示的变化也敏感。需要进一步研究提高模型的鲁棒性和对人体提示的适应性。
...
TL;DR: 本文针对多人姿态估计中现有度量指标过度依赖置信度排名、忽视低置信度假正例的问题，提出了OCpose度量方法。该方法基于最优传输理论，平等评估所有检测到的姿态，同时利用置信度分数提高匹配可靠性，提供更公平的评估视角。


<details>
  <summary>Details</summary>
Motivation: 现有度量指标过于依赖姿态检测置信度排名，忽视低置信度假正例，导致即使检测到大量低置信度假正例也能获得高分，评估不够公平。

Method: 提出基于最优传输的OCpose度量方法，将检测到的姿态与姿态标注进行匹配，平等对待所有检测到的姿态（无论置信度高低），同时利用置信度分数提高匹配分数的可靠性。

Result: OCpose提供了一种不同于其他基于置信度排名度量的评估视角，能够更公平地权衡真正例和假正例。

Conclusion: OCpose通过最优传输框架提供了一种新的评估视角，能够公平地权衡真正例和假正例，不受置信度排名影响，为姿态估计提供了更可靠的评估方法。
...
TL;DR: 提出一种基于视觉语言模型的零样本 CFRP  Subsurface 缺陷检测框架，通过 AIRT-VLM 适配器解决热成像与自然图像领域差距，实现 10dB SNR 增益和 70% IoU


<details>
  <summary>Details</summary>
Motivation: 传统AI方法需要大量昂贵且耗时的CFRP检测数据集进行训练，且存在自然图像与热成像数据之间的领域差距问题

Method: 提出AIRT-VLM适配器增强缺陷可见性并对齐热成像域与VLM学习表征，采用三种代表性VLM（GroundingDINO、Qwen-VL-Chat、CogVLM）进行零样本缺陷理解和定位

Result: AIRT-VLM适配器相比传统热成像降维方法实现超过10dB的信噪比增益，零样本缺陷检测的交并比(IoU)达到70%

Conclusion: 该框架仅需预训练多模态VLM编码器和轻量级适配器即可实现零样本缺陷检测，无需大规模训练数据集，显著降低了实际工业应用的成本和时间。
...
### [19] [Motion Forcing: A Decoupled Framework for Robust Video Generation in Motion Dynamics](https://arxiv.org/abs/2603.10408)
*Tianshuo Xu,Zhifei Chen,Leyi Wu,Hao Lu,Ying-cong Chen*

Main category: cs.CV

TL;DR: 本文提出Motion Forcing框架，通过层级化的"Point-Shape-Appearance"范式解耦物理推理与视觉合成，解决视频生成中高质量、物理一致性和可控性三难平衡问题，并在自动驾驶基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视频生成的终极目标是在高视觉质量、严格物理一致性和精确可控性之间实现平衡，但现有模型在简单孤立场景中虽能保持平衡，在复杂场景（如涉及碰撞或密集交通）时这种平衡会被打破。

Method: 提出Motion Forcing框架，包含三个核心创新：1）层级化的"Point-Shape-Appearance"范式，将生成分解为可验证的阶段——用稀疏几何锚点(Point)建模复杂动力学，扩展为显式解决3D几何的动态深度图(Shape)，最后渲染高保真纹理(Appearance)；2）掩码点恢复(Masked Point Recovery)策略，通过随机掩码输入锚点并强制重建完整动态深度，使模型超越被动模式匹配，学习潜在物理定律（如惯性）来推断缺失轨迹。

Result: 在自动驾驶基准测试上显著优于最先进基线，在复杂场景中保持三难平衡稳定性；在物理和机器人任务上的评估进一步确认了框架的泛化能力。

Conclusion: 通过显式解耦物理推理与视觉合成，Motion Forcing成功解决了复杂场景下视频生成的三难平衡问题，具有良好的泛化能力。
...
### [20] [Are Video Reasoning Models Ready to Go Outside?](https://arxiv.org/abs/2603.10652)
*Yangfan He,Changgyu Boo,Jaehong Yoon*

Main category: cs.CV

TL;DR: ROVA是一个通过建模时空干扰下的鲁棒性感知一致性奖励来提升视觉语言模型鲁棒性的训练框架，结合难度感知的在线训练策略和新的PVRBench基准，在真实世界干扰下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在真实部署中常遇到天气、遮挡、相机运动等干扰，在这些条件下理解和推理能力显著下降，揭示了干净受控评估设置与真实世界鲁棒性之间的差距。

Method: 提出难度感知的在线训练策略，通过自反射评估持续重新估计样本难度，并引入鲁棒性感知的一致性奖励进行自适应训练。

Result: ROVA有效缓解了性能下降，相比基线模型（QWen2.5/3-VL、InternVL2.5、Embodied-R）相对准确率提升至少24%，推理能力提升超过9%，且增益可迁移到干净标准基准。

Conclusion: ROVA能够有效提升视觉语言模型在真实世界干扰下的鲁棒性，在PVRBench、UrbanVideo和VisBench等基准上相比基线模型提升至少24%的准确率和超过9%的推理能力，且这些提升可以迁移到干净的标准基准测试中。
...
### [21] [TractoRC: A Unified Probabilistic Learning Framework for Joint Tractography Registration and Clustering](https://arxiv.org/abs/2603.10418)
*Yijie Li,Xi Zhu,Junyi Wang,Ye Wu,Lauren J. O'Donnell,Fan Zhang*

Main category: cs.CV

TL;DR: TractoRC是一个统一的概率框架，联合执行白质纤维追踪的配准和聚类任务，通过共享潜在嵌入空间实现两个任务的协同优化。


<details>
  <summary>Details</summary>
Motivation: 白质纤维追踪分析中的两个关键任务——tractogram配准和streamline聚类通常独立执行，但它们共享捕捉几何相似结构的目标，可以相互补充信息。

Method: 提出TractoRC框架，在共享的潜在嵌入空间中联合建模两个任务：1) 配准学习概率关键点分布以对齐不同受试者的tractogram；2) 聚类学习流线结构原型以形成连贯的流线簇。引入变换等变自监督策略学习几何感知和变换不变的嵌入。

Result: 实验表明，联合优化配准和聚类显著提升了两个任务的性能，优于当前最先进的方法。

Conclusion: 该研究实现了tractogram配准和streamline聚类的联合优化，为白质纤维追踪分析提供了更有效的一致性表征方法。
...
### [22] [UAV traffic scene understanding: A cross-spectral guided approach and a unified benchmark](https://arxiv.org/abs/2603.10722)
*Yu Zhang,Zhicheng Zhao,Ze Luo,Chenglong Li,Jin Tang*

Main category: cs.CV

TL;DR: 提出CTCNet用于无人机交通场景理解，通过PGKE模块利用交通法规记忆注入领域知识，通过QASC模块融合光学和热成像数据构建了首个大规模光学-热红外基准数据集Traffic-VQA


<details>
  <summary>Details</summary>
Motivation: 现有方法在真实监控中面临重大挑战：严重依赖光学图像导致在夜间和雾等不良照明条件下性能严重下降；当前视觉问答(VQA)模型局限于基本感知任务，缺乏评估复杂交通行为所需的领域特定监管知识

Method: 提出原型引导知识嵌入(PGKE)模块和质量感知光谱补偿(QASC)模块，前者利用外部交通法规记忆(TRM)中的高级语义原型来锚定领域知识，后者利用光学和热成像的互补特性进行双向上下文交换以补偿退化特征

Result: CTCNet在认知和感知场景中均显著优于最先进方法

Conclusion: CTCNet在认知和感知场景中均显著优于现有最先进方法，Traffic-VQA数据集包含8,180对对齐图像和130万问答对，涵盖31种不同类型
...
### [23] [World2Act: Latent Action Post-Training via Skill-Compositional World Models](https://arxiv.org/abs/2603.10422)
*An Dinh Vuong,Tuan Van Vo,Abdullah Sohail,Haoran Ding,Liang Ma,Xiaodan Liang,Anqing Duan,Ivan Laptev,Ian Reid*

Main category: cs.CV

TL;DR: World2Act 是一个后训练框架，通过对比匹配将 VLA 动作与 WM 视频动态潜在变量对齐，并使用 LLM 技能分解解决任意长度视频生成问题，在机器人任务上取得最先进结果，真实世界性能提升 6.7%。


<details>
  <summary>Details</summary>
Motivation: World Models (WMs) 用于后训练 Vision-Language-Action (VLA) 策略以提高鲁棒性和泛化性，但现有方法依赖像素空间监督，导致策略对像素级伪影和 WM rollout 产生的幻觉敏感。此外，WM 主要在固定长度片段上训练，而机器人执行时间差异大，难以生成任意长度的视频。

Method: 提出 World2Act 框架，通过对比匹配目标将 VLA 动作与 WM 视频动态潜在变量对齐，减少对像素的依赖。同时提出基于 LLM 的技能分解管道，将高级指令分割为低级提示，生成 RoboCasa-Skill 和 LIBERO-Skill 数据集，支持技能组合的 WM 保持时间一致性。

Result: 将 World2Act 应用于 GR00T-N1.6 和 Cosmos Policy 等 VLA，在 RoboCasa 和 LIBERO 上达到最先进水平，在真实世界中提升 6.7%。

Conclusion: 该方法通过减少对像素的依赖和解决任意长度视频生成问题，显著增强了具身智能体的泛化能力。
...
### [24] [Taking Shortcuts for Categorical VQA Using Super Neurons](https://arxiv.org/abs/2603.10781)
*Pierre Musacchio,Jaeyi Jeong,Dahun Kim,Jaesik Park*

Main category: cs.CV

TL;DR: 本文提出Super Neurons (SNs)，通过探测VLM的标量激活而非注意力向量，在第一层第一个生成token处实现极早退出，在保持分类性能的同时达到5.10x加速。


<details>
  <summary>Details</summary>
Motivation: Sparse Attention Vectors (SAVs)虽然表现优异，但其搜索空间受限。本文旨在扩大搜索空间，从注意力向量转向标量激活，以在第一层第一个生成token立即找到更判别性的神经元。

Method: 该方法直接探测VLM的标量激活值，而非注意力向量，通过在第一层第一个生成token处寻找判别性神经元来实现极早退出。

Result: SNs在各类视觉 grounded 下游任务上 robustly 提高分类性能，同时实现最高5.10x的加速比。

Conclusion: Super Neurons (SNs)是一种有效的训练-free替代方案，可在Vision Language Models中实现高达5.10x的加速比，同时保持稳健的分类性能。
...
TL;DR: 本文提出LCAMV方法，通过对投影仪和相机的横向色差进行逐像素补偿，并利用泊松-高斯噪声模型和最小方差估计自适应融合多通道相位数据，实现了对非均匀着色物体的高精度3D重建，无需额外硬件即可快速采集。


<details>
  <summary>Details</summary>
Motivation: 有色物体的结构光3D重建受到光学元件中横向色差（LCA）以及RGB通道间噪声特性不均匀的阻碍。现有方法需要额外硬件或多次曝光，缺乏快速采集能力。

Method: LCAMV采用单投影仪-相机对，无需额外硬件或采集约束。该方法首先对投影仪和相机中的LCA进行逐像素建模和补偿，然后利用泊松-高斯噪声模型和最小方差估计对多通道相位数据进行自适应融合。

Result: 实验表明，LCAMV在平面和非平面有色表面上的表现优于灰度转换和传统通道加权方法，深度误差降低高达43.6%。

Conclusion: LCAMV establishes itself as an effective solution for high-precision 3D reconstruction of nonuniformly colored objects, enabling fast acquisition without additional hardware constraints.
...
### [26] [On the Reliability of Cue Conflict and Beyond](https://arxiv.org/abs/2603.10834)
*Pum Jun Kim,Seung-Ah Lee,Seongho Park,Dongyoon Han,Jaejun Yoo*

Main category: cs.CV

TL;DR: 论文指出了现有形状-纹理偏好评判基准的不稳定性问题，并提出REFINED-BIAS框架，通过构建平衡可识别的cue对和使用排名度量实现更可靠和公平的偏好评判。


<details>
  <summary>Details</summary>
Motivation: 当前基于风格化的cue-conflict基准存在不稳定性：stylization可能无法可靠地实例化感知有效且可分离的cue；基于比率的偏好评判可能掩盖绝对cue敏感性；仅评估预选类别可能因忽略完整决策空间而扭曲模型预测。这些因素会将偏好与cue有效性、cue平衡和可识别性伪影混淆。

Method: REFINED-BIAS使用形状和纹理的显式定义构建平衡的、人类和模型可识别的cue对，并通过基于排名的度量在完整标签空间上测量cue特异性敏感性。

Result: REFINED-BIAS在多样化的训练方式和架构中实现了更公平的跨模型比较、更忠实的形状和纹理偏好评判，以及更清晰的实证结论。

Conclusion: REFINED-BIAS能够实现更公平的跨模型比较、更忠实的形状和纹理偏好评判，并解决先前cue-conflict评估无法可靠区分的不一致问题。
...
TL;DR: 首个面向具身场景的可操作地理定位推理基准WanderBench及GeoAoT框架，将静态地理定位转化为交互式探索，通过将推理与可执行动作耦合，在19个模型上实现优异细粒度定位和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管先进的大型多模态模型（LMMs）已展现出丰富的世界知识和复杂推理能力，但其地理定位任务表现尚未被探索。现有地理定位任务多为静态识别，缺乏交互式探索能力。

Method: 1) 构建WanderBench: 包含超过32K全景图，跨越六大洲，组织为可导航图结构，支持旋转和移动等物理动作，将地理定位从静态识别转化为交互式探索；2) 提出GeoAoT框架: 将推理与具身动作耦合，不仅生成文本推理链，还产生可执行计划（如接近地标、调整视角）以主动减少不确定性；3) 建立联合评估协议: 同时衡量地理定位准确性和难度感知的地理定位提问能力。

Result: 在19个大语言模型上的实验表明，GeoAoT实现了卓越的细粒度定位和动态环境中更强的泛化能力。

Conclusion: WanderBench和GeoAoT为具身视觉理解中的可操作推理驱动地理定位定义了新范式。实验表明GeoAoT在19个大语言模型上实现了卓越的细粒度定位和更强的动态环境泛化能力。
...
### [28] [GroundCount: Grounding Vision-Language Models with Object Detection for Mitigating Counting Hallucinations](https://arxiv.org/abs/2603.10978)
*Boyuan Chen,Minghao Shao,Siddharth Garg,Ramesh Karri,Muhammad Shafique*

Main category: cs.CV

TL;DR: 本文提出GroundCount框架，通过将CNN目标检测模型（如YOLO）的空间定位能力融入视觉语言模型来解决计数幻觉问题。最佳情况下在Ovis2.5-2B上达到81.3%准确率（提升6.6pp），同时推理时间减少22%。研究表明位置编码是关键组件，置信度分数对大多数架构有害，而结构化提示的显式符号 grounding 优于隐式特征融合。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）在计数任务中存在持续幻觉，准确性显著低于其他视觉推理任务。即使是最先进的推理型VLMs也不例外。CNN目标检测模型（如YOLO）在空间定位和实例计数方面表现优秀且计算开销小。

Method: 提出GroundCount框架，通过目标检测模型（ODM）为VLMs提供显式空间grounding。方法包括：（1）基于提示的增强策略；（2）位置编码（对强模型有益但对弱模型有害）；（3）去除置信度分数（避免噪声）；（4）评估特征级融合架构与结构化提示的对比。

Result: 最佳情况（Ovis2.2-2B）达到81.3%准确率，提升6.6pp，推理时间减少22%。五个模型中四个获得6.2-7.5pp提升，一个因架构不兼容导致性能下降。位置编码对强模型有益但对弱模型有害；置信度分数对大多数架构引入噪声。

Conclusion: 计数失败源于空间-语义整合的基本限制而非架构特定缺陷。结构化提示的显式符号grounding优于隐式特征融合。架构兼容性在增强策略中至关重要。
...
### [29] [Does AI See like Art Historians? Interpreting How Vision Language Models Recognize Artistic Style](https://arxiv.org/abs/2603.11024)
...
TL;DR: 研究视觉语言模型(VLMs)预测艺术风格的能力及其与人类艺术史专家判断的一致性。通过潜在空间分解方法发现，VLMs提取的概念中73%具有语义意义，90%与艺术风格预测相关，表明模型能学习到与专家相通的视觉特征概念。


<details>
  <summary>Details</summary>
Motivation: 当前VLMs在艺术领域表现出色，但缺乏对其理解艺术风格机制的深入理解，特别是与艺术史学家专业标准的一致性程度未知。

Method: 采用潜在空间分解方法识别驱动艺术风格预测的概念，并结合定量评估、因果分析和艺术史学家的专业判断进行综合评估。

Result: 1) 73%的提取概念被艺术史学家判定为具有连贯性和语义意义的视觉特征；2) 90%用于预测风格的概念被判定为相关；3) 不相关概念成功预测的原因可归因于模型对概念的更形式化理解（如明暗对比）；4) 跨学科方法有效揭示了VLMs的艺术风格认知机制。

Conclusion: 该研究揭示了视觉语言模型(VLMs)在艺术风格预测方面与人类专家判断的高度一致性。73%的概念被认定为语义有意义，90%的预测概念被判定为相关。对于模型使用表面不相关概念但成功预测的情况，可以通过形式化理解（如明暗对比）来解释。这表明VLMs能够学习到与艺术史学家相通的视觉特征概念，尽管可能通过不同的学习路径。
...
TL;DR: CIPHER是一种无训练方法，通过构建反事实数据集OHC-25K提取视觉诱导幻觉的低秩子空间，在推理时将中间隐藏状态投影出该子空间来抑制幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)在多模态任务中表现强大，但经常产生幻觉——与视觉输入不对应的错误输出。现有无训练方法主要关注文本诱导的幻觉，而本文方法专门针对视觉模态产生的幻觉。

Method: CIPHER采用两阶段方法。离线阶段：构建OHC-25K数据集（包含25000个反事实样本），使用扩散编辑图像故意与原始真实caption矛盾，将编辑后的图像与未改变的真实caption配对并通过LVLM提取幻觉相关表示，对比真实(图像,caption)对的表示揭示跨低秩子空间的系统性偏移。推理阶段：通过将中间隐藏状态投影出该幻觉子空间来抑制幻觉。

Result: 在多个基准测试上的实验表明，CIPHER显著降低幻觉率同时保持任务性能，证明了反事实视觉扰动在改善LVLM忠实度方面的有效性。

Conclusion: CIPHER通过在推理阶段将中间隐藏状态投影出幻觉子空间来抑制视觉诱导的幻觉，实验表明该方法在多个基准测试中显著降低幻觉率同时保持任务性能，证明了反事实视觉扰动在提高LVLM忠实度方面的有效性。
...
### [31] [V2M-Zero: Zero-Pair Time-Aligned Video-to-Music Generation](https://arxiv.org/abs/2603.11042)
*Yan-Bo Lin,Jonah Casebeer,Long Mai,Aniruddha Mahapatra,Gedas Bertasius,Nicholas J. Bryan*

Main category: cs.CV

TL;DR: V2M-Zero是一种零样本视频到音乐生成方法，通过独立捕获音乐和视频的时间结构（事件曲线）来实现时间对齐，无需跨模态配对训练数据，在多个数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有文本到音乐模型缺乏细粒度的时间控制，难以生成与视频事件时间对齐的音乐。关键洞察是：时间同步需要匹配"何时"和"多少变化"，而不是"什么变化"。音乐和视觉事件在语义上不同，但具有共享的时间结构，可以独立捕获。

Method: 通过预训练的音乐和视频编码器，使用模态内相似度计算事件曲线来捕获这种时间结构。通过独立测量每个模态内的时间变化，这些曲线提供跨模态的可比表示。训练策略：微调文本到音乐模型的音乐事件曲线，然后在推理时替换为视频事件曲线，无需跨模态训练或配对数据。

Result: 在OES-Pub、MovieGenBench-Music和AIST++数据集上，相比配对数据基线方法：音频质量提升5-21%，语义对齐提升13-15%，时间同步提升21-52%，舞蹈视频节拍对齐提升28%。大规模众包主观听力测试也得到类似结果。

Conclusion: 通过模态内特征实现时间对齐，而非跨模态监督，对视频到音乐生成是有效的。这种无需配对数据的零样本方法在多个指标上取得了显著改进。
...
### [33] [LiTo: Surface Light Field Tokenization](https://arxiv.org/abs/2603.11047)
*Jen-Hao Rick Chang,Xiaoming Zhao,Dorian Chan,Oncel Tuzel*

Main category: cs.CV

TL;DR: 提出一种联合建模对象几何和视点依赖外观的3D潜在表示方法，通过将RGB-D图像作为表面光场采样编码为潜在向量，并结合潜在流匹配模型实现从单张图像生成具有逼真视点依赖效果的3D对象。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多专注于重建3D几何或预测视点无关的漫反射外观，难以捕捉真实的视点依赖效果（如镜面高光和菲涅尔反射）。

Method: 该方法利用RGB-D图像作为表面光场的采样，将随机采样的表面光场编码为紧凑的潜在向量集，使模型能够在统一的3D潜在空间中表示几何和外观。此外，还在该表示上训练了潜在流匹配模型，以学习基于单张输入图像生成3D对象的分布。

Result: 该方法能够复现复杂光照下的视点依赖效果，如镜面高光和菲涅尔反射。通过学习基于单张输入图像的潜在分布，可以生成与输入图像中光照和材质一致的3D对象。

Conclusion: 实验表明，该方法在视觉质量和输入保真度方面均优于现有方法，能够生成与输入图像中光照和材质一致的3D对象。
...
TL;DR: IMTBench是一个新的图像机器翻译基准数据集，包含2500个样本，涵盖4个场景和9种语言，支持翻译质量、背景保留、图像质量和跨模态对齐等多维度评估，旨在推动端到端图像文本翻译任务的发展。


<details>
  <summary>Details</summary>
Motivation: 现有IIMT基准多为合成数据，无法反映真实世界复杂性；当前评估协议仅关注单一模态指标，忽视了渲染文本与模型输出之间的跨模态忠实度问题。

Method: 构建了包含2500个样本的IMTBench基准，涵盖4个实际场景和9种语言，支持多维度评估，包括翻译质量、背景保留、图像整体质量和跨模态对齐分数（衡量模型输出的翻译文本与渲染在图像中的文本之间的一致性）。同时对商业级联系统、闭源和开源统一多模态模型进行了基准测试。

Result: 实验观察到模型在不同场景和语言间存在较大性能差距，特别是在自然场景和资源有限语言上表现不佳，显示出端到端图像文本翻译任务的巨大提升空间。

Conclusion: IMTBench作为标准化基准数据集，为端到端图像文本翻译任务建立了统一评估框架，揭示了当前模型在自然场景和资源有限语言上的显著性能差距，为该领域未来研究提供了重要方向。
...
TL;DR: 提出一种用于UHD图像去模糊的自回归流方法，通过粗到细的渐进式流程、Flow Matching条件向量场建模和ill-conditioning抑制方案，在4K及以上分辨率上实现细节恢复与效率的平衡。


<details>
  <summary>Details</summary>
Motivation: UHD图像去模糊面临重大挑战，需在细粒度细节恢复和实际推理效率之间取得平衡。现有的判别式和生成式方法虽取得了显著成果，但在计算成本和生成细粒度细节能力之间仍存在权衡。此外，多步生成在UHD分辨率下可能存在数值不稳定问题。

Method: 提出一种基于自回归流的新型UHD图像去模糊方法，采用粗到细的渐进式处理流程。具体包括：1)将UHD修复分解为渐进式粗到细过程，每一尺度通过上采样前一尺度结果并添加当前尺度残差来形成清晰估计；2)引入Flow Matching将残差生成建模为条件向量场，使用高效的Euler/Heun求解器进行少步ODE采样；3)提出 ill-conditioning 抑制方案，通过对特征诱导的注意力矩阵施加条件数正则化，提高收敛性和跨尺度一致性。

Result: 在4K（3840×2160）或更高分辨率的模糊图像上展示了优异的性能，有效提升了细节丰富度的同时保持了可负担的推理成本。

Conclusion: 所提出的自回归流方法在4K及以上分辨率的模糊图像去模糊任务中展现出优异性能，成功平衡了细粒度细节恢复与推理效率之间的权衡。
...
TL;DR: 提出了一种 Visually-Guided Text Disentanglement (VG-MedGen) 框架，用于解决医学图像合成中的模态差距和语义纠缠问题。该方法通过跨模态潜在对齐机制和混合特征融合模块，实现对 Diffusion Transformer 的细粒度结构控制。


<details>
  <summary>Details</summary>
Motivation: 医学图像合成面临两大挑战：1) 复杂视觉细节与抽象临床文本之间存在显著的模态差距；2) 粗粒度文本嵌入导致解剖结构与成像风格的语义纠缠，削弱了生成过程中的可控性。

Method: 提出跨模态潜在对齐机制，利用视觉先验将非结构化文本显式解耦为独立语义表示；设计混合特征融合模块(HFFM)，通过分离通道将特征注入 Diffusion Transformer (DiT)，实现细粒度结构控制。

Result: 在三个数据集上的实验表明，该方法在生成质量方面优于现有方法，并显著提升了下游分类任务的性能。

Conclusion: VG-MedGen 框架通过视觉引导的文本解耦策略，有效解决了医学图像合成中的模态差距和语义纠缠问题，为文本到医学图像的生成提供了更精确的控制能力。
...
### [38] [P-GSVC: Layered Progressive 2D Gaussian Splatting for Scalable Image and Video](https://arxiv.org/abs/2603.10551)
*Longan Wang,Yuang Shi,Wei Tsang Ooi*

Main category: cs.CV

TL;DR: P-GSVC是首个分层渐进式2D Gaussian Splatting框架，通过将Gaussian splats组织为基础层和增强层，实现从粗到精的图像和视频重建。提出联合训练策略对齐各层优化轨迹，实验表明相比顺序训练方法可提升PSNR达1.9-2.6 dB。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting已成为图像和视频重建的竞争性显式表示，但现有的方法缺乏一个统一的、分层的、可扩展的解决方案。现有方法难以实现质量与分辨率的可扩展性，且层间训练缺乏协调性。

Method: 提出P-GSVC，这是首个分层渐进式2D Gaussian Splatting框架。该框架将2D Gaussian splats组织为基础层和连续增强层，实现从粗到精的重建。同时提出联合训练策略，同时更新各层的高斯，对齐优化轨迹以确保层间兼容性和稳定的渐进重建。

Result: 联合训练策略相比顺序分层训练方法，视频PSNR提升高达1.9 dB，图像PSNR提升高达2.6 dB。

Conclusion: P-GSVC提供了一个统一的、可扩展的Gaussian表示解决方案，支持质量和分辨率的可扩展性，通过联合训练策略有效优化分层表示，显著提升重建质量。
...
### [39] [PET-F2I: A Comprehensive Benchmark and Parameter-Efficient Fine-Tuning of LLMs for PET/CT Report Impression Generation](https://arxiv.org/abs/2603.10560)
...
TL;DR: 本文提出了一个沉浸式计算机图形学(CG)质量评估框架。针对现有CG数据集缺乏系统性质量描述和评估方法无法提供文本解释的问题，作者构建了包含3500张CG图像的质量描述数据集，并提出了一种基于检索增强的两流检索框架来提升视觉语言模型(VLM)的CG质量评估能力。


<details>
  <summary>Details</summary>
Motivation: 现有CG质量评估面临两大挑战：1) 现有CG数据集缺乏系统性的渲染质量描述；2) 现有的CG质量评估方法无法提供合理的基于文本的解释。这限制了CG渲染质量的全面评估和VLM在CG理解领域的发展。

Method: 1. 从用户视角识别出6个关键的CG质量感知维度；2. 构建包含3500张CG图像及对应质量描述的数据集，描述涵盖CG风格、内容和所选维度的感知质量；3. 使用数据集子集构建问答基准来评估现有VLM；4. 提出检索增强生成方法，采用两流检索框架，利用视觉相似图像的描述来增强VLM对CG图像的理解。

Result: 研究发现当前VLM在判断细粒度CG质量方面不够准确，但视觉相似图像的描述能显著提升VLM对CG图像的理解能力。实验表明该方法显著提升了多个代表性VLM在CG质量评估上的性能。

Conclusion: 该研究成功构建了CG质量评估数据集和基准，发现了VLM的不足并提出了有效的增强方案。两流检索框架通过利用相似图像描述有效解决了VLM在细粒度CG质量评估上的局限，为沉浸式CG质量评估提供了新的研究方向。
...
TL;DR: 本文提出LIDA框架，将AI生成图像归因问题重新定义为实例检索问题，通过低比特指纹生成模块和无监督预训练+少样本适配的策略，实现模型无关的图像归因，在零样本和少样本场景下达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法无法应对日益逼真的AI生成图像；现有方法依赖模型，需要访问生成模型，缺乏对新型未见过生成器的通用性和可扩展性。

Method: 提出LIDA框架，包含两个关键模块：1) Low-Bit Fingerprint Generation（低比特指纹生成）模块，用于生成输入；2) Unsupervised Pre-Training（无监督预训练）+ Few-Shot Attribution Adaptation（少样本归因适配）的训练策略。方法的核心创新在于将归因问题从传统的图像分类问题重新定义为实例检索问题。

Result: LIDA在零样本和少样本设置下，对Deepfake检测和图像归因均达到了最先进的性能。

Conclusion: LIDA通过将AI生成图像归因问题重新定义为实例检索问题，提出了一种新的范式。该方法具有模型无关性，无需访问生成模型即可工作，在零样本和少样本设置下均达到了最先进的性能。
...
TL;DR: Marigold-SSD是一种单步、后期融合的深度补全框架，利用强大的扩散先验同时消除了扩散方法中常见的昂贵测试时优化成本。通过将计算负担从推理转移到微调，使其实现在真实世界延迟约束下的高效且鲁棒的3D感知。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散基于深度补全方法虽然性能优异，但通常需要昂贵的测试时优化，导致推理速度慢，难以满足真实世界的延迟约束需求。

Method: Marigold-SDD采用单步推理和后期融合策略，通过将计算负担从推理阶段转移到微调阶段，在保持扩散先验优势的同时实现高效推理，训练成本仅需4.5 GPU天。

Result: 在两个室内和两个室外基准数据集上实现了强大的跨域泛化能力和零样本性能，显著缩小了扩散模型与判别模型之间的效率差距。

Conclusion: Marigold-SSD成功地在保持扩散先验强大性能的同时实现了高效推理，为实时3D感知应用提供了一种实用的解决方案。
...
### [43] [HyPER-GAN: Hybrid Patch-Based Image-to-Image Translation for Real-Time Photorealism Enhancement](https://arxiv.org/abs/2603.10604)
*Stefanos Pasios,Nikos Nikolaidis*

Main category: cs.CV

TL;DR: HyPER-GAN是一种轻量级的图像到图像转换方法，采用U-Net风格生成器，通过混合训练策略结合真实世界图像块，在保持语义一致性的同时提高视觉真实感，实现实时推理。


<details>
  <summary>Details</summary>
Motivation: 生成模型虽广泛用于增强合成数据的真实感，但常引入视觉伪影，降低计算机视觉算法精度，且需要高计算资源，限制了在实时训练或评估场景中的应用。

Method: 提出HyPER-GAN，一种基于U-Net风格生成器的轻量级图像到图像翻译方法，采用混合训练策略，结合真实世界数据的匹配图像块来提高视觉真实感和语义一致性。

Result: HyPER-GAN在推理延迟、视觉真实感和语义鲁棒性方面优于最先进的配对图像到图像转换方法。混合训练策略确实提高了视觉质量和语义一致性。

Conclusion: 混合训练策略能有效改善视觉质量和语义一致性。HyPER-GAN在实时推理任务中具有显著优势。
...
### [44] [Less is More: Decoder-Free Masked Modeling for Efficient Skeleton Representation Learning](https://arxiv.org/abs/2603.10648)
*Jeonghyeok Do,Yun Chen,Geunhyuk Youk,Munchurl Kim*

Main category: cs.CV

TL;DR: SLiM是一个统一的骨架动作表示学习框架，通过将掩码自编码器(MAE)与对比学习(CL)结合在共享编码器中，消除了重建解码器的需求。核心创新包括语义管掩码和骨骼感知增强，以解决高骨骼-时间相关性导致的平凡重建问题。该方法在保持最先进性能的同时，将推理计算成本降低了7.89倍。


<details>
  <summary>Details</summary>
Motivation: 当前骨架动作表示学习从对比学习(CL)发展到掩码自编码器(MAE)，但各自存在固有局限：CL往往忽略细粒度局部细节，而MAE的解码器计算负担沉重。更重要的是，MAE存在严重的计算不对称问题——预训练时得益于高效掩码，但下游任务却需要处理完整序列。这些瓶颈亟需解决。

Method: 提出SLiM框架，通过共享编码器协调掩码建模与对比学习。主要创新包括：1)摒弃重建解码器，迫使编码器直接捕获判别性特征；2)引入语义管掩码(Semantic Tube Masking)防止高骨骼-时间相关性导致的平凡重建；3)设计骨骼感知增强确保跨时间粒度的解剖学一致性。

Result: 实验表明SLiM在所有下游协议上持续达到最优性能。值得注意的是，该方法在保持卓越精度的同时展现出极高的效率，与现有MAE方法相比，推理计算成本降低了7.89倍。

Conclusion: SLiM是首个无需解码器的掩码建模表示学习框架，成功实现了效率与性能的兼得，为骨架动作表示学习提供了新的范式。
...
TL;DR: 本文系统分析了地理空间基础模型（GeoFM）在地球观测（EO）工作流中的嵌入设计策略，发现压缩500倍以上的嵌入仍能保持良好的任务性能。_transformerbackbonewithmeanpooling_provides_strong_default_embeddings_intermediate_resnet_layers_can_outperform_final_layers_self-supervised_objectives_exhibit_task-specific_strengths_and_combining_embeddings_from_different_objectives_often_improves_robustness">系统研究了在EO任务中backbone架构、预训练策略、表示深度、空间聚合和表示组合对性能的影响。


<details>
  <summary>Details</summary>
Motivation: 地球观测任务产生大量多光谱图像，GeoFMs被广泛使用。但作为特征提取器时，嵌入的获取、聚合和组合方式会影响下游性能和可扩展性，需要理解这些权衡以构建可扩展的嵌入工作流。

Method: 利用NeuCo-Bench对GeoFM嵌入设计进行系统分析，研究backbone架构、预训练策略、表示深度、空间聚合和表示组合五个维度。

Result: 1）嵌入可压缩至原始数据的1/500以上仍保持可用性；2）带mean pooling的Transformer backbone提供强大的默认嵌入；3）中间ResNet层可优于最终层；4）自监督目标表现出任务特异性优势；5）组合不同目标的嵌入可提高鲁棒性。

Conclusion: 为可扩展的嵌入基EO工作流提供了设计指导：优先选择Transformer+mean pooling方案，可考虑利用中间层特征，自监督目标需根据任务选择，多目标组合可增强鲁棒性。
...
TL;DR: A²-Edit是一个统一的任意类别对象替换修复框架，通过Mixture of Transformer模块和Mask Annealing训练策略解决大规模多类别图像编辑问题，在UniEdit-500K数据集上训练后，在VITON-HD和AnyInsertion基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据集存在严重的同质化问题和有限的类别覆盖率，导致模型难以处理多样化的对象类别编辑任务。

Method: 提出三个核心技术创新：(1) Mixture of Transformer模块，通过动态专家选择对不同对象类别进行差异化建模，并通过专家协作增强跨类别语义迁移和泛化能力；(2) Mask Annealing Training Strategy (MATS)，在训练过程中逐步放松mask精度要求，提高模型对不同编辑任务的鲁棒性；(3) 构建大规模多类别数据集UniEdit-500K，包含8个主要类别、209个细粒度子类别和500,104个图像对。

Result: A²-Edit在VITON-HD和AnyInsertion基准测试中在所有指标上 consistently 优于现有方法。

Conclusion: A²-Edit在任意对象编辑任务上优于现有方法，在VITON-HD和AnyInsertion等基准测试中取得最佳性能，为任意对象编辑提供了新的高效解决方案。
...
TL;DR: UniCom提出通过压缩连续表示统一多模态理解和生成，验证了通道维度压缩的有效性以及transfusion架构的优势，在统一模型中达到SOTA生成性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前统一多模态模型的两难困境：离散视觉分词器不可避免地丢失细粒度语义信息，导致视觉理解任务性能不佳；而直接建模连续语义表示（如CLIP、SigLIP）在高维生成建模中面临收敛慢和训练不稳定的问题。

Method: 1) 设计注意力机制的语义压缩器，将密集特征蒸馏为紧凑统一表示；2) 采用 transfusion 架构替代查询式设计；3) 通过通道维度压缩而非空间下采样实现高效重建和生成。

Result: 实验表明，UniCom在统一模型中实现了最先进的生成性能，具有出色的图像编辑可控性，且无需依赖VAE即可保持图像一致性。

Conclusion: UniCom通过压缩连续表示统一了多模态理解和生成，在统一模型中实现了最先进的生成性能。该方法保留了丰富的语义先验，在图像编辑中表现出卓越的可控性，且无需VAE即可保持图像一致性。核心贡献在于验证了通道维度压缩比空间下采样更有效，以及 transfusion 架构在收敛性和一致性上优于查询式设计。
...
TL;DR: WalkGPT是一种像素接地的LVLM，用于无障碍行人导航任务，通过MSQP和CTP组件统一语言推理和分割，无需用户提示即可生成分割掩码和深度估计，在41k图像的PAVE基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型缺乏显式接地，导致目标幻觉和不可靠的深度推理，无法满足无障碍行人导航对语义和空间推理的需求。

Method: 提出Multi-Scale Query Projector (MSQP)聚合空间层次中的图像令牌，以及Calibrated Text Projector (CTP)在Region Alignment Loss指导下将语言嵌入映射到分割感知表示，实现细粒度接地和深度推理。

Result: WalkGPT在PAVE基准（41k行人视角图像）上实现了强大的接地推理和分割性能。

Conclusion: WalkGPT通过统一的架构实现了语言推理和分割的结合，无需用户提供的提示或锚点即可生成分割掩码和相对深度估计，在PAVE基准上展现出强大的接地推理和分割性能，为无障碍行人导航提供了新的解决方案。
...
TL;DR: 本研究发布了eLasmobranc数据集，这是一个包含7种东西班牙地中海软骨鱼类的公开图像集，旨在支持物种级分类和AI生物多样性监测系统开发，解决现有数据集在细粒度形态分类方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 软骨鱼类种群正在全球范围内大幅下降，多个物种被列为受威胁物种。现有视觉数据集主要为检测导向、水下获取或仅限于粗粒度分类，限制了其在细粒度形态分类中的应用。可靠的监测和物种级识别对于支持ISRA等保护和平面规划至关重要。

Method: 通过专门的数据采集（包括实地考察、与当地鱼类市场和项目合作）以及开源公共来源获取图像，采用标准化协议在水面外环境中拍摄，以确保诊断形态特征的清晰可视化，并结合专家验证的物种注释、结构化时空元数据和物种级补充信息。

Result: 创建了eLasmobranc数据集，包含7种生态相关软骨鱼类物种的图像，来自西班牙地中海东部海岸（已识别两个ISRAs区域），数据集专门用于支持监督物种级分类、人口研究和用于生物多样性监测的人工智能系统开发。

Conclusion: 该数据集通过整合形态学清晰性、分类学可靠性和公共可访问性，填补了细粒度软骨鱼类鉴定的关键空白，促进了保护导向计算机视觉的可重复研究，为ISRAs等保护规划提供了数据支持。
...
TL;DR: 提出JiT，一种无需训练的空间域加速框架，通过稀疏锚点标记和确定性微流实现扩散变换器最高7倍加速


<details>
  <summary>Details</summary>
Motivation: 现有加速方法主要关注时间域，忽略了生成过程中固有的空间冗余——全局结构在精细细节形成之前就已出现，而所有空间区域的统一计算处理导致了关键的低效问题

Method: 提出JiT框架，包含：1）空间近似生成常微分方程（ODE），基于动态选择的稀疏锚点标记子集驱动潜在状态演化；2）确定性微流（deterministic micro-flow），一种简单有效的有限时间ODE，用于在潜在状态维度扩展时保持结构一致性和统计正确性

Result: 在FLUX.1-dev模型上实现最高7倍加速，且几乎无性能损失，显著优于现有加速方法

Conclusion: JiT建立了一种新的优越的推理速度与生成质量之间的权衡，为扩散变换器的实际部署提供了可行性
...
### [51] [CodePercept: Code-Grounded Visual STEM Perception for MLLMs](https://arxiv.org/abs/2603.10757)
...
TL;DR: 研究MLLMs在STEM视觉推理中的失败原因，发现感知能力是关键限制因素，提出用代码作为感知媒介增强多模态模型能力的新范式。


<details>
  <summary>Details</summary>
Motivation: 当多模态大语言模型(MLLMs)在STEM(科学、技术、工程、数学)视觉推理中失败时，一个根本问题是：失败原因是感知缺陷还是推理限制？

Method: 通过系统性扩展分析，独立扩展感知和推理组件，发现扩展感知始终优于扩展推理。提出将代码作为强大的感知媒介，建立ICC-1M数据集(包含1M图像-字幕-代码三元组)，采用两种方法：1)代码基础字幕生成；2)STEM图像到代码翻译。引入STEM2Code-Eval基准测试进行验证。

Result: 揭示感知是当前STEM视觉推理的真正限制因素；通过代码作为感知范式可有效增强MLLMs的感知能力。

Conclusion: 代码作为感知媒介能够消除现有知识蒸馏方法中的幻觉，通过可执行代码生成实现图像重建，提供确定性可验证的评估。
...
### [52] [Guiding Diffusion Models with Semantically Degraded Conditions](https://arxiv.org/abs/2603.10780)
*Shilong Han,Yuming Zhang,Hongxia Wang*

Main category: cs.CV

TL;DR: 本文提出CDG方法，通过将空提示符替换为策略性降级的条件，引导模型从"好vs空"转向"好vs接近好"的精细判别，从而捕捉细微语义差异，提升文本到图像模型的组合精度。
...
<details>
  <summary>Details</summary>
Motivation: Classifier-Free Guidance(CFG)依赖语义空白的空提示符(∅)产生引导信号，容易出现几何纠缠，是限制复杂组合任务精度的关键因素。



Method: 提出条件降级引导(CDG)方法，将transformer文本编码器中的token分为两类：内容token（编码对象语义）和上下文聚合token（捕获全局上下文）。通过仅对内容token进行选择性降级来构建退化条件c_deg，无需外部模型或训练。



Result: 在Stable Diffusion 3、FLUX和Qwen-Image等多种架构上验证，CDG显著提高了组合准确性和文本-图像对齐能力，且具有轻量级、即插即用的特点，计算开销可忽略不计。



Conclusion: CDG通过将"好vs无"的粗略对比转变为"好vs接近好"的精细判别，使模型能够捕捉细微的语义差异。实验表明CDG在多个架构上显著提升了组合准确性和文本-图像对齐，且具有轻量级、即插即用的特点，计算开销可忽略不计。
...
### [53] [Phase-Interface Instance Segmentation as a Visual Sensor for Laboratory Process Monitoring](https://arxiv.org/abs/2603.10782)
*Mingyue Li,Xin Yang,Shilin Yan,Jinye Ran,Morui Zhu,Zirui Peng,Huanqing Peng,Wei Peng,Guanghua Zhang,Shuo Li,Hao Zhang*

Main category: cs.CV

TL;DR: 本文提出CTG 2.0数据集和LGA-RCM-YOLO模型，用于透明玻璃器皿中相界面实例分割，在复杂实验室场景下实现高精度近实时分割，并成功应用于实验室自动化视觉监测。


<details>
  <summary>Details</summary>
Motivation: 透明玻璃器皿中的化学实验视觉监测面临挑战，传统分割方法受限于微弱的相边界和光学伪影的影响。现有方法难以有效区分玻璃器皿与内部相界面。

Method: 提出LGA-RCM-YOLO模型，基于YOLO11m-seg架构，集成了局部-全局注意力模块(LGA)用于鲁棒的语义表征，以及矩形自校准模块(RCM)用于细长薄弱边界的精细化处理。同时添加辅助颜色属性头用于液体实例的颜色分类。

Result: 在CTG 2.0数据集上，模型达到84.4% AP@0.5和58.43% AP@0.5-0.95，相较YOLO11m基线分别提升6.42和8.75个AP点。推理速度为13.67 FPS（RTX 3060）。颜色属性头实现98.71%精确率和98.32%召回率。成功应用于分液漏斗相分离和结晶过程的连续过程监测。

Conclusion: LGA-RCM-YOLO通过结合局部-全局注意力机制和矩形自校准模块，有效解决了透明玻璃器皿中相界面分割的难题。该模型在CTG 2.0数据集上显著优于基线方法，并实现了近实时的推理速度。辅助颜色属性头能够高精度区分有色和无色液体。实验证明了相界面实例分割在分液漏斗相分离和结晶过程中的实用价值，为实验室自动化提供了有效的视觉传感器方案。
...
### [54] [The Quadratic Geometry of Flow Matching: Semantic Granularity Alignment for Text-to-Image Synthesis](https://arxiv.org/abs/2603.10785)
*Zhinan Xiong,Shunqi Yuan*

Main category: cs.CV

TL;DR: 本文从几何视角分析生成式微调的优化动态，在Flow Matching框架下将MSE目标表述为NTK驱动的二次型，发现隐含的数据交互矩阵。为解决交叉项干扰和同质性假设的局限性，提出语义粒度对齐(SGA)方法，通过向量残差场干预缓解梯度冲突，在Text-to-Image任务上验证了方法有效性。


<details>
  <summary>Details</summary>
Motivation: 标准训练虽优化交叉项干扰但缺乏显式控制；数据同质性假设可能限制模型有效容量

Method: 在Flow Matching框架下，将MSE目标函数表述为由动态演化神经切向核(NTK)控制的二次型，提出语义粒度对齐(SGA)方法，通过干预向量残差场来缓解梯度冲突

Result: 在DiT和U-Net架构上验证，SGA提升了效率-质量权衡，加快收敛速度并改善结构完整性

Conclusion: SGA通过在向量残差场中进行针对性干预来缓解梯度冲突，在DiT和U-Net架构上验证了方法的有效性，显著提升了收敛速度和质量-效率权衡
...
### [55] [PolGS++: Physically-Guided Polarimetric Gaussian Splatting for Fast Reflective Surface Reconstruction](https://arxiv.org/abs/2603.10801)
*Yufei Han,Chu Zhou,Youwei Lyu,Qi Chen,Si Li,Boxin Shi,Yunpeng Jia,Heng Guo,Zhanyu Ma*

Main category: cs.CV

TL;DR: PolGS++是一种物理引导的极化高斯泼溅框架，用于快速重建反射表面。该方法将极化BRDF模型集成到3DGS中，显式解耦漫反射和镜面反射分量，并通过深度引导的可见性掩码获取机制实现基于极化角(AoP)的切线空间一致性约束。


<details>
  <summary>Details</summary>
Motivation: 准确重建反射表面仍是计算机视觉中的基础挑战，在实时虚拟现实和数字内容创建中有广泛应用。3DGS在反射表面上的性能仍落后于隐式神经方法，特别是在恢复精细几何和表面法线方面。

Method: 提出PolGS++框架，具体包括：1) 将极化BRDF(pBRDF)模型集成到3DGS中，显式解耦漫反射和镜面反射分量；2) 引入深度引导的可见性掩码获取机制，实现基于AoP的切线空间一致性约束，无需 costly ray-tracing intersections。

Result: 该方法只需约10分钟训练时间，在合成和真实世界数据集上的广泛实验验证了其有效性，在反射表面重建质量上优于现有方法。

Conclusion: PolGS++通过物理引导的设计提高了重建质量和效率，为快速反射表面重建提供了一种有效方案。
...
### [56] [Backdoor Directions in Vision Transformers](https://arxiv.org/abs/2603.10806)
...
### [57] [HanMoVLM: Large Vision-Language Models for Professional Artistic Painting Evaluation](https://arxiv.org/abs/2603.10814)
*Hongji Yang,Yucheng Zhou,Wencheng Han,Songlian Li,Xiaotong Zhao,Jianbing Shen*

Main category: cs.CV

TL;DR: 本文提出将大型视觉语言模型转化为中国艺术领域专业评估专家的方法。通过构建包含真实拍卖级作品的HanMoVLM数据集，提出HanMoVLM模型和专家验证的思维链，并设计奖励函数优化推理过程。实验表明该模型可作为生成模型的高质量验证器，显著提升中国绘画生成质量。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(VLMs)虽然在通用视觉能力方面表现出色，但在艺术评估方面存在不足，无法像人类专家一样对特定艺术领域（如中国艺术领域）的艺术品提供专业评估。中国艺术领域更加抽象，需要大量的艺术训练才能进行评估。

Method: 1. 构建HanMo-Bench数据集，包含真实的拍卖级杰作和AI生成的作品，基于真实市场价格
2. 提出HanMoVLM模型
3. 构建专家验证的思维链(CoT)，引导模型进行专家级推理：从内容识别、兴趣区域(ROI)定位到专业评估
4. 设计奖励函数来优化HanMoVLM的推理过程

Result: HanMoVLM可以作为测试时扩展(Test-time Scaling)的关键骨干，作为高质量验证器使生成模型能够从多个候选中选择最具艺术价值的输出。实验和人类研究证实了该方法的有效性。

Conclusion: HanMoVLM有效地缩小了机器与专业艺术专家之间的差距，在中文绘画生成质量方面取得了显著提升。
...
### [58] [A dataset of medication images with instance segmentation masks for preventing adverse drug events](https://arxiv.org/abs/2603.10825)
*W. I. Chu,S. Hirani,G. Tarroni,L. Li*

Main category: cs.CV

TL;DR: MEDISEG是一个新的药丸实例分割数据集，包含8262张图像，涵盖32种不同药丸类型，从单个药丸图像到混乱的药盒场景，支持强监督和有限监督下的药丸识别模型训练


<details>
  <summary>Details</summary>
Motivation: 药物错误和不良药物事件对患者安全构成重大风险，现有药丸图像数据集很少捕捉真实世界的复杂性（如重叠的药丸、可变光照和遮挡），且缺乏综合数据集阻碍了AI药丸识别模型的发展

Method: 在MEDISEG数据集上训练YOLOv8和YOLOv9模型进行实例分割，并在少样本检测协议下评估数据集性能

Result: 在3-Pills子集上达到99.5%的mAP@0.5，在32-Pills子集上达到80.1%的mAP@0.5；少样本检测表明，基于MEDISEG的预训练显著改善了遮挡多药丸场景中未知药丸类别的识别

Conclusion: MEDISEG数据集能够支持强监督训练，并促进有限监督下的可迁移表示，是开发AI驱动药物安全系统的宝贵资源
...
TL;DR: 研究从部署角度出发，探讨少样本药片识别在真实复杂场景下的泛化能力，采用两阶段目标检测框架，通过基线训练+少样本微调的方式适应新药片类别。


<details>
  <summary>Details</summary>
Motivation: 真实世界的药片识别系统面临复杂视觉条件（杂乱场景、药片重叠、反光、不同采集环境），现有方法在域偏移下的泛化能力不足，需研究少样本学习在药物安全部署中的实用性。

Method: 采用两阶段目标检测框架：先进行基线训练，再进行少样本微调。使用1、5、10个标注样本per class对模型进行适配，并在包含多目标、杂乱场景的独立部署数据集上评估。

Result: 分类性能即使仅用一个标注样本也能快速达到饱和；但在重叠和遮挡条件下，定位和召回能力显著下降。训练数据视觉真实性越高，少样本场景下的鲁棒性越强。

Conclusion: 语义药片识别可快速适应少样本监督，但定位能力仍是部署瓶颈。训练数据的真实性和少样本微调对提升部署适应性至关重要。
...
TL;DR: 针对多模态大语言模型在长上下文场景下视觉衰减的问题，提出DIPE方法，通过解耦模态内/模态间位置编码，使视觉信号在不同上下文长度下保持感知一致性。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型(MLLMs)在长上下文场景下存在视觉衰减问题——随着文本序列长度增加，对视觉token的关注度下降，导致文本生成脱离视觉约束。研究者将此归因于Multimodal RoPE的归纳偏差：它对视觉和文本token之间距离较远的情况进行惩罚。

Method: 提出了一种名为Inter-modal Distance Invariant Position Encoding (DIPE)的方法。该方法解耦位置编码：保留自然相对位置编码以维持模态内交互的局部结构，同时为模态间交互强制设定锚定感知近邻。

Result: 实验结果表明，DIPE与Multimodal RoPE结合后，模型在长上下文场景中保持稳定的视觉grounding，显著缓解视觉衰减问题，同时在标准短上下文基准上保持性能。

Conclusion: 本文提出的DIPE方法通过解耦位置编码，有效解决了多模态大语言模型在长上下文场景下的视觉衰减问题。该方法在保持短上下文基准性能的同时，显著提升了长上下文场景下的视觉 grounding 能力，且无需额外的训练成本，具有良好的通用性。
...
TL;DR: 本文提出S2D（Sparse to Dense lifting）框架，通过桥接点云和3D高斯溅射（3DGS）两种表示，实现仅需极少量输入的高质量3DGS重建。


<details>
  <summary>Details</summary>
Motivation: 当前点云和3DGS各自存在局限性：点云渲染不真实，3DGS在稀疏输入下质量显著下降。现有方法需要大量输入才能实现高质量重建，限制了3DGS的应用场景。

Method: 提出两阶段S2D提升方法：1）高效的单步扩散模型，将稀疏点云提升以修复高质量图像伪影；2）结合随机样本丢弃和加权梯度的重建策略，实现从稀疏输入视图到密集新视图的鲁棒模型拟合。

Result: S2D在新视图引导生成方面达到最佳一致性，在不同输入稀疏度下实现了一流的稀疏视图重建质量。该方法能用最少的捕获重建稳定场景。

Conclusion: S2D成功桥接了点云与3DGS表示，在最小化输入的情况下实现高质量重建，为3DGS应用确立了新的稀疏视图重建标准。]
...
TL;DR: 研究评估了两种口腔癌检测RPA实现，OC-RPAv2通过Singleton设计模式和批处理将每张图像预测时间从0.29秒降至0.06秒，实现60-100倍效率提升。
...
Method: 评估两种RPA实现（OC-RPAv1和OC-RPAv2），其中OC-RPAv2采用Singleton设计模式和批处理技术。

Result: OC-RPAv1平均每张图像处理时间为0.29秒，OC-RPAv2将时间缩短至0.06秒，相比标准RPA方法提升60-100倍效率。

Conclusion: 设计模式（Singleton）和批处理可以显著提升口腔癌检测的可扩展性并降低成本，OC-RPAv2实现了60-100倍的效率提升。
...
TL;DR: 提出一种基于多模态潜在空间的终身模仿学习框架，通过增量特征调整机制保持任务间区分度，在LIBERO基准测试中取得最先进成果。


<details>
  <summary>Details</summary>
Motivation: 解决现实内存和数据约束下跨顺序任务的持续策略优化问题，突破传统经验回放的局限。

Method: 提出一种在多模态潜在空间中运行的终身模仿学习框架，使用视觉、语言和机器人状态的紧凑表示进行存储和重用。引入增量特征调整机制，通过角度边际约束正则化任务嵌入的演变，保持任务间区分度。

Result: 在LIBERO基准测试中实现AUC提升10-17分，遗忘减少高达65%，优于先前领先方法。

Conclusion: 在LIBERO基准测试中取得最先进水平，AUC提升10-17分，遗忘减少高达65%。消融实验验证了各组件的有效性。代码已开源。
...
TL;DR: CBCTRepD是一个用于口腔颌面CBCT报告生成的双语AI系统，通过大规模数据集（7,408个研究，55种疾病实体）训练，并在多层次评估框架下显示出优异性能，可帮助不同经验水平的放射科医生提高报告质量、减少遗漏，具有作为实际助手的潜力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在医学报告生成方面发展迅速，但在口腔颌面CBCT报告中的应用仍然有限，主要原因是高质量配对CBCT-报告数据的稀缺以及体积CBCT解释的内在复杂性。

Method: 1. 建立包含约7,408个研究的大规模高质量CBCT-报告配对数据集，涵盖55种口腔疾病实体；2. 开发CBCTRepD双语口腔颌面CBCT报告生成系统；3. 建立临床基础的多层次评估框架，评估AI生成的草稿和放射科医生编辑的合作报告，使用自动指标以及放射科医生和临床医生为中心的评估。

Result: CBCTRepD实现了卓越的报告生成性能，生成的草稿在写作质量和标准化方面可与中级放射科医生相媲美。在放射科医生-AI协作中，CBCTRepD在各个经验水平上提供一致且有临床意义的益处：帮助新手放射科医生提高至中级水平，使中级放射科医生接近高级水平，甚至帮助高级放射科医生减少遗漏相关错误（包括临床上重要的漏诊病变）。

Conclusion: CBCTRepD作为一个实用的助手，在真实世界的CBCT报告工作中显示出强大且可靠的潜力，可改善报告结构、减少遗漏、促进对各解剖区域共存病变的关注，适用于多层级医疗环境。
...
TL;DR: 本文提出SSL-V3方法，将视频质量评估(VQA)与视频分类相结合，通过Combined-SSL机制解决VQA标注数据不足的问题，在医疗视频数据集上实现了94.87%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 视频质量对视频分类有显著影响，研究团队在区分轻度认知障碍时发现清晰视频分类效果好但模糊视频效果差，因此意识到可引入视频质量评估(VQA)来改善视频分类性能。

Method: 提出了一种结合自监督学习与视频视觉变换器的SSL-V3方法，采用Combined-SSL机制将视频质量评分直接调整视频特征图，将质量评分作为连接点链接VQA与分类任务，利用分类任务的监督信号来调整VQA参数，解决VQA标注数据不足的问题。

Result: 在I-CONECT数据集（面部视频医疗数据集）的访谈视频上达到了94.87%的准确率，验证了SSL-V3方法的有效性。

Conclusion: SSL-V3通过将视频质量评分作为连接点，将VQA任务与分类任务联合优化，实现了视频分类性能的提升。在I-CONECT数据集上达到了94.87%的准确率，验证了方法的有效性。
...
TL;DR: 提出Med-DualLoRA联邦参数高效微调框架，通过解耦全局和本地LoRA模块实现隐私保护的医学Foundation models跨中心适应，仅需Adapt少量Transformer块即可在多中心3D CMR疾病检测任务上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 单站点数据微调可能导致性能次优和模型偏见增加，而集中式微调因隐私限制通常不可行。传统联邦方法在异构非IID多中心数据下表现不佳，且 adapting大型模型时通信开销巨大。

Method: 提出Med-DualLoRA框架，通过加性分解将全局共享和本地低秩适应(Low-Rank Adaptations, LoRA)解耦。全局LoRA在本地训练后跨站点共享聚合，而本地LoRA保持隐私，仅Adapt两个Transformer块以提高效率。

Result: 在ACDC和M&Ms数据集上进行3D CMR疾病检测评估，将每个供应商视为联邦客户端，Med-DualLoRA获得统计学显著提升的性能(平衡准确率0.768，特异性0.612)，同时保持通信效率。

Conclusion: Med-DualLoRA通过解耦全局共享和本地低秩适应，在保护隐私的同时实现高效的联邦模型微调，仅需Adapt两个Transformer块即可保持性能，显著降低通信成本，为在现实临床约束下的医学Foundation models本地联邦适应提供了可扩展的解决方案。
...
TL;DR: VCR是一种用于弱光图像增强的新型框架，通过通道自适应调整(CAA)和颜色分布对齐(CDA)模块解决现有方法中的 luminance-chrominance 分离和颜色分布对齐问题，在多个基准数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱光图像增强方法存在以下问题：1) 基于sRGB的方法存在luminance和color纠缠；2) HSV色彩空间引入红黑噪声伪影；3) HVI色彩空间存在luminance-chrominance通道级不一致；4) 颜色分布对齐问题导致增强结果不自然。

Method: 提出VCR框架包含两个主要组件：1) CAA模块采用variance-guided特征过滤，增强模型对高强度和颜色分布区域的关注；2) CDA模块在颜色特征空间中进行分布对齐。这些设计增强了弱光条件下的感知质量。

Result: 在多个基准数据集上的实验表明，该方法相比现有方法达到最先进的性能。

Conclusion: 通过variance-driven通道重校准和颜色分布对齐策略，VCR有效解决了弱光图像增强中的luminance-chrominance分离和颜色分布问题，显著提升了增强结果的视觉质量。
...
TL;DR: 该论文提出了颜色保真度数据集(CFD)和颜色保真度指标(CFM),用于客观评估真实风格文本到图像生成中的颜色保真度,并设计了无需训练的颜色保真度细化方法(CFR)来增强生成图像的颜色真实性。


<details>
  <summary>Details</summary>
Motivation: 现有评估范式存在偏见:人类评分和偏好训练指标往往偏好视觉鲜艳的图像,饱和度和对比度过高,导致生成图像过于鲜艳而不真实,难以生成看起来像真实世界摄影的图像。

Method: 1)构建了包含超过130万张真实和合成图像的CFD数据集,具有有序的颜色真实度级别;2)使用多模态编码器学习感知颜色保真度,训练CFM指标;3)提出无需训练的CFR方法,通过自适应调节生成中的空间-时间指导尺度来增强颜色真实性。

Result: CFD支持CFM进行评估,CFM学习到的注意力进一步指导CFR细化T2I保真度,形成了一个用于评估和改进真实风格T2I生成颜色保真度的渐进框架。

Conclusion: 该研究提供了一个系统的颜色保真度评估和改进框架,有望推动文本到图像生成向更真实的视觉表现发展。
...
### [69] [DynVLA: Learning World Dynamics for Action Reasoning in Autonomous Driving](https://arxiv.org/abs/2603.11041)
...
TL;DR: DynVLA提出一种用于驾驶的视觉语言动作(VLA)模型，引入"动力学思维链(Dynamics CoT)"新范式，在动作生成前预测紧凑的世界动态。


<details>
  <summary>Details</summary>
Motivation: 现有Textual CoT缺乏细粒度时空理解，Visual CoT因密集图像预测带来大量冗余，而驾驶场景中环境动态丰富，需要更紧凑、可解释、高效的世界演化表示方法。

Method: 1)引入动力学Tokenizer将未来演化压缩为少量动力学token；2)解耦自我中心和环境中心动力学建模；3)通过SFT和RFT训练模型生成动力学token后再生成动作。

Result: 在NAVSIM、Bench2Drive和大规模内部数据集上持续优于Textual CoT和Visual CoT方法。

Conclusion: Dynamics CoT能以紧凑、可解释、高效的形式捕捉世界演化，验证了动力学思维链的有效性和实用价值。
...
TL;DR: Agentar-Fin-OCR是一个针对金融文档的解析系统，能够将超长金融PDF转换为语义一致、高精度的结构化输出，支持审计级溯源。


<details>
  <summary>Details</summary>
Motivation: 金融文档具有复杂的布局、跨页结构不连续性以及单元级引用能力等特殊挑战，现有通用OCR和文档解析系统难以有效处理，需要专门针对金融领域的解决方案。

Method: (1) 跨页内容整合算法(Cross-page Contents Consolidation)恢复页面连续性，文档级标题层级重构模块(DHR)构建全局一致的目录树；(2) 难度自适应课程学习策略训练表格解析，CellBBoxRegressor模块利用结构锚点标记从解码器隐藏状态定位表格单元。

Result: 模型在OmniDocBench表格解析指标上表现优异；引入FinDocBench基准测试，包含6个金融文档类别和专家验证标注，评估指标包括目录编辑距离相似度(TocEDS)、跨页串联TEDS和表格单元IoU(C-IoU)；对多种先进模型进行了评估。

Conclusion: Agentar-Fin-OCR和FinDocBench为可靠的下游金融文档应用提供了实用基础。
...
### [71] [Large chirotopes with computable numbers of triangulations](https://arxiv.org/abs/2603.10251)
*Mathilde Bouvel,Valentin Féray,Xavier Goaoc,Florent Koechlin*

Main category: cs.CG

TL;DR: 本文研究chirotopes的分解方法及其在平面点集三角剖分计数问题中的应用，推广了链状chirotopes的凸和凹和运算，并通过函数方程和核方法得到了双圆三角剖分数量的精确渐近估计。


<details>
  <summary>Details</summary>
Motivation: Chirotopes是平面点集的一种常见组合抽象，作者希望研究chirotopes的分解方法，并将其应用于计数给定平面点集所支撑的三角剖分数量的问题。

Method: 推广了Rutschmann和Wettstein为特定chirotopes族（称为链）定义的凸和凹和运算，使用函数方程和核方法进行分析。

Result: 得到了双圆（double circle）的三角剖分数量的精确渐近估计。

Conclusion: 通过推广分解方法并结合核方法，能够有效估计特定平面点集的三角剖分数量，为chirotopes理论在几何计数问题中的应用提供了新的分析工具。
...
### [72] [HEAL: Hindsight Entropy-Assisted Learning for Reasoning Distillation](https://arxiv.org/abs/2603.10359)
...
### [73] [Beyond Scalars: Evaluating and Understanding LLM Reasoning via Geometric Progress and Stability](https://arxiv.org/abs/2603.10384)
*Xinyan Jiang,Ninghao Liu,Di Wang,Lijie Hu*

Main category: cs.AI

TL;DR: TRACED框架通过几何运动学方法评估LLM推理质量，将推理轨迹分解为"进展"(位移)和"稳定性"(曲率)，发现正确推理表现为高进展、稳定轨迹，而幻觉则表现为低进展、不稳定模式(停滞位移伴高频曲率波动)。该框架在多个基准测试中表现竞争力且鲁棒性优越。


<details>
  <summary>Details</summary>
Motivation: 现有通过标量概率评估LLM可靠性的方法往往无法捕捉推理的结构动态特性，需要引入理论依据的几何运动学方法来评估推理质量。

Method: 将推理轨迹分解为两个核心维度：进展(Progress/位移)和稳定性(Stability/曲率)，利用这些拓扑特征构建概率框架，识别"犹豫循环"(高曲率)和"确定性积累"(位移)的模式。

Result: 在多样化基准测试中达到竞争力性能，且具有 superior robustness(卓越鲁棒性)。

Conclusion: TRACED成功架起了几何与认知的桥梁，通过物理视角(高曲率对应犹豫循环、位移对应确定性积累)解码机器思维的内部动态，为理解LLM推理过程提供了新范式。
...
### [74] [Verbalizing LLM's Higher-order Uncertainty via Imprecise Probabilities](https://arxiv.org/abs/2603.10396)
*Anita Yang,Krikamol Muandet,Michele Caprio,Siu Lun Chau,Masaki Adachi*

Main category: cs.AI

TL;DR: 提出基于不精确概率的提示不确定性引发技术，解决传统概率框架在LLM不确定性量化中的不足，区分一阶不确定性（对响应的 uncertainty）和二阶不确定性（对概率模型本身的 uncertainty），提高LLM不确定性报告的可信度。


<details>
  <summary>Details</summary>
Motivation: Despite growing demand for eliciting uncertainty from LLMs, LLM behavior is not always adequately captured by elicitation techniques developed under classical probabilistic uncertainty framework, leading to systematic failure modes in ambiguous question-answering, in-context learning, and self-reflection settings.

Method: Propose novel prompt-based uncertainty elicitation techniques grounded in imprecise probabilities, introducing general-purpose prompting and post-processing procedures to directly elicit and quantify both first-order and second-order uncertainty.

Result: Demonstrates effectiveness across diverse settings, enabling more faithful uncertainty reporting from LLMs.

Conclusion: The approach enables more faithful uncertainty reporting from LLMs, improving credibility and supporting downstream decision-making.
...
### [75] [Resource-constrained Amazons chess decision framework integrating large language models and graph attention](https://arxiv.org/abs/2603.10512)
*Tianhao Qian,Zhuoxuan Li,Jinde Cao,Xinli Shi,Hanjie Liu,Leszek Rutkowski*

Main category: cs.AI

TL;DR: 本文提出一种轻量级混合框架用于Amazons游戏，结合图注意力自动编码器、多步蒙特卡洛树搜索、随机图遗传算法和GPT-4o-mini，从噪声监督中学习，在资源受限环境下实现超越教师模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法依赖大量数据集和计算资源，在资源受限环境中面临重大挑战。论文旨在探索弱到强泛化范式，从有噪声和不完美的监督中学习，而非依赖专家演示。

Method: 提出了一种轻量级混合框架，整合以下技术：1）图注意力自动编码器（Graph Attention Autoencoder）告知多步蒙特卡洛树搜索；2）随机图遗传算法（Stochastic Graph Genetic Algorithm）优化评估信号；3）GPT-4o-mini生成合成训练数据。图注意力机制作为结构过滤器对LLM输出进行去噪。

Result: 在10×10 Amazons棋盘上，该混合方法相比基线实现了15%–56%的决策精度提升；超越教师模型GPT-4o-mini，在N=30节点时达到45.0%胜率，N=50节点时达到66.5%胜率。

Conclusion: 该框架成功实现了从通用大模型（GPT-4o-mini）到专用高性能游戏AI的进化，在10×10 Amazons棋盘上实现了45.0%（N=30节点）和66.5%（N=50节点）的胜率，验证了在严格计算约束下从基础模型发展专业游戏AI的可行性。
...
### [76] [IH-Challenge: A Training Dataset to Improve Instruction Hierarchy on Frontier LLMs](https://arxiv.org/abs/2603.10521)
...
TL;DR: 本文提出IH-Challenge强化学习训练数据集，通过在线对抗样本生成提升GPT-5-Mini的指令层级鲁棒性，使IH评估表现从84.1%提升至94.1%，不安全行为降低至0.7%，并发布数据集支持未来研究。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)在处理系统指令、开发者指令、用户指令和工具指令之间的冲突时缺乏明确的优先级策略，导致容易受到jailbreak、系统提示词提取和agentic prompt injection攻击。IH-Challenge数据集旨在解决训练中IH鲁棒性难以提升的问题，因为IH失败与指令遵循失败容易混淆，冲突可能很微妙，且模型可能学习到过度拒绝等捷径。

Method: 构建IH-Challenge强化学习训练数据集，结合在线对抗样本生成对GPT-5-Mini进行微调。该数据集针对16个分布内、分布外和人工红队基准测试进行评估。

Result: IH鲁棒性平均提升10.0%（从84.1%到94.1%），不安全行为从6.7%降至0.7%，同时在一般安全评估中提升了有用性，并在内部静态agentic prompt injection评估中达到饱和，能力退化极小。

Conclusion: IH-Challenge数据集能够有效提升LLM的指令层级鲁棒性，显著降低安全风险，同时保持模型能力，为未来鲁棒指令层级研究提供了重要资源。
...
### [77] [Does LLM Alignment Really Need Diversity? An Empirical Study of Adapting RLVR Methods for Moral Reasoning](https://arxiv.org/abs/2603.10588)
*Zhaowei Zhang,Xiaohan Liu,Xuekai Zhu,Junchao Huang,Ceyao Zhang,Zhiyuan Feng,Yaodong Yang,Xiaoyuan Yi,Xing Xie*

Main category: cs.AI

TL;DR: 首次全面比较了强化学习（RLVR）中奖励最大化与分布匹配两种范式在LLM对齐任务上的表现，发现与预期相反，奖励最大化方法在道德推理任务中同样有效，因为道德推理的高奖励分布比数学推理更为集中。


<details>
  <summary>Details</summary>
Motivation: LLM对齐任务（如道德推理）是否存在本质上不同的训练需求？由于对齐任务对多种有效_response的容忍度，自然假设需要多样性寻求的分布匹配算法，而非奖励最大化的策略方法。

Method: 在MoReBench上比较两种范式；构建基于规则的奖励管道，训练Qwen3-1.7B作为评判模型；通过语义可视化将高奖励_response映射到语义空间。

Result: 与假设相反，分布匹配方法在对齐任务上并未展现显著优势；道德推理的高奖励分布比数学推理更集中（后者多种解题策略均可获得高奖励）。

Conclusion: 对齐任务本质上不需要多样性保留算法，标准奖励最大化的RLVR方法可有效迁移到道德推理而无需显式多样性机制。
...
### [78] [Trajectory-Informed Memory Generation for Self-Improving Agent Systems](https://arxiv.org/abs/2603.10600)
*Gaodan Fang,Vatche Isahagian,K. R. Jayaram,Ritesh Kumar,Vinod Muthusamy,Punleuk Oum,Gegi Thomas*

Main category: cs.AI

TL;DR: 本文提出一个从智能体执行轨迹中提取可操作学习经验并通过上下文记忆检索改进未来性能的框架，包含轨迹智能提取器、决策归因分析器、上下文学习生成器和自适应记忆检索系统四个组件，在AppWorld基准上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: LLM智能体面临的核心挑战是：从执行经验中学习以改进未来性能。现有智能体常重复低效模式、无法从类似错误中恢复、错失应用过去成功策略的机会。

Method: 提出一个包含四个组件的框架：轨迹智能提取器（执行语义分析）、决策归因分析器（识别导致失败/恢复/低效的决策）、上下文学习生成器（生成三类指导）和自适应记忆检索系统（基于多维相似性注入相关学习）。

Result: 在AppWorld基准测试上，保留任务场景目标完成率最高提升14.3个百分点，复杂任务场景目标提升28.5个百分点（相对提升149%）。

Conclusion: 该框架在AppWorld基准测试上展现出显著改进，尤其是在复杂任务上表现突出。最高可在保留任务上实现14.3个百分点的场景目标完成率提升，在复杂任务上实现28.5个百分点的提升（相对提升149%）。
...
### [79] [Emulating Clinician Cognition via Self-Evolving Deep Clinical Research](https://arxiv.org/abs/2603.10677)
...
TL;DR: 本文提出DxEvolve，一个自进化的临床诊断智能体，通过交互式工作流程自主获取检查并积累临床经验作为可治理的学习资产，在MIMIC-CDM基准上显著提升诊断准确率，达到与临床医生相当的水平。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统与临床诊断现实脱节，将诊断视为单次回顾性预测，缺乏可审计的改进机制。临床诊断是一个复杂的认知过程，根植于动态线索获取和持续的专业知识积累。

Method: 开发了DxEvolve，一个自进化的诊断智能体，采用交互式深度临床研究工作流程，能够自主要求检查，并将临床经验作为诊断认知原语外部化。

Result: 在MIMIC-CDM基准上，DxEvolve相比基线模型平均提高诊断准确率11.2%，在读者研究子集上达到90.4%，与临床医生参考标准(88.8%)相当。在外部验证队列中，准确率分别提高10.2%(覆盖类别)和17.1%(未覆盖类别)。

Conclusion: DxEvolve通过将经验转化为可治理的学习资产，支持临床人工智能的持续进化，为临床AI的负责任发展提供了可行路径。
...
### [80] [Nurture-First Agent Development: Building Domain-Expert AI Agents Through Conversational Knowledge Crystallization](https://arxiv.org/abs/2603.10808)
*Linghao Zhang*

Main category: cs.AI

TL;DR: 本文提出Nurture-First Development（NFD）范式，将AI智能体构建从预先设计的工程阶段转变为通过与领域专家持续对话逐步培养的过程，通过知识结晶周期机制将对话中的碎片化知识整合为结构化知识资产，并设计了三层认知架构和双工作区模式支撑这一范式。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）智能体框架的出现将构建领域专家AI智能体的主要挑战从原始能力转变为领域专业知识的有效编码。现有两种主导范式——代码优先开发（将专业知识嵌入确定性流程）和提示优先开发（在静态系统提示中捕获专业知识）都将智能体构建视为部署前的离散工程阶段。这种顺序假设与领域知识的本质存在根本性不匹配，因为领域知识在很大程度上是 tacit（默会）的、深度个性化的且持续演化的。

Method: 提出Nurture-First Development（NFD）范式，通过三部分形式化：(1) 三层认知架构，按波动性和个性化程度组织智能体知识；(2) 知识结晶周期，包含结晶操作的形式化定义和效率指标；(3) 操作框架，包括双工作区模式和螺旋开发模型。智能体以最小脚手架初始化，通过与领域从业者的结构化对话交互逐步成长。

Result: 通过构建美国股票分析金融研究智能体的详细案例研究进行了说明。

Conclusion: 讨论了NFD的条件、局限性以及人机协同进化的更广泛影响。
...
TL;DR: PharmGraph-Auditor是一个基于混合药学知识库(HPKB)和知识图谱范式的处方审核系统，通过迭代模式细化算法构建知识库，并引入基于知识库的验证链(CoV)将不可靠的LLM生成器转变为透明推理引擎，以实现安全、可验证的处方审核。


<details>
  <summary>Details</summary>
Motivation: 药物错误对患者安全构成重大威胁，药剂师验证(PV)是最后一道防线但负担繁重。直接将大型语言模型(LLM)应用于这一零容忍领域存在事实不可靠、缺乏可追溯性和复杂推理能力弱的问题。

Method: 1.提出可信混合药学知识库(HPKB)，采用虚拟知识图谱(VKG)范式，统一关系组件和图组件；2.提出迭代模式细化(ISR)算法，实现图和关系模式的共同演化；3.提出基于知识库的验证链(CoV)，将LLM从不可靠生成器转变为透明推理引擎。

Result: 实验结果表明系统具有强大的知识提取能力，可帮助药剂师实现更安全、更快速的处方验证。

Conclusion: PharmGraph-Auditor通过结合知识图谱和LLM的优势，在保证处方审核安全性的同时提高了效率，为药学领域的人工智能应用提供了新的解决方案。
...
TL;DR: Daily-Omni is a multiple-choice Audio-Visual QA benchmark for evaluating cross-modal temporal reasoning in multimodal large language models, featuring 684 real-world videos and 1,197 questions across 6 task families


<details>
  <summary>Details</summary>
Motivation: Recent MLLMs achieve promising performance on visual and audio benchmarks independently, but the ability to process cross-modal information synchronously remains largely unexplored

Method: Daily-Omni benchmark with 684 videos and 1,197 questions across 6 task families; semi-automatic annotation pipeline with cross-modal consistency refinement, temporal alignment elicitation, and text-only leakage filtering; evaluation of 24 foundation models under 37 modality settings

Result: Many end-to-end MLLMs still struggle on alignment-critical questions

Conclusion: robust cross-modal temporal alignment remains an important open challenge
...
### [83] [LLMGreenRec: LLM-Based Multi-Agent Recommender System for Sustainable E-Commerce](https://arxiv.org/abs/2603.11025)
*Hao N. Nguyen,Hieu M. Nguyen,Son Van Nguyen,Nguyen Thi Hanh*

Main category: cs.MA

TL;DR: LLMGreenRec是一个利用大语言模型的多智能体框架，通过协作分析和迭代提示优化来推断用户的绿色消费意图，优先推荐环保产品，同时减少数字碳足迹。


<details>
  <summary>Details</summary>
Motivation: 传统基于会话的推荐系统优化短期转化率，难以捕捉用户对环保产品的细微意图，导致绿色意愿与实际购买行为之间存在差距。同时，推荐系统本身的数字碳足迹也需关注。

Method: LLMGreenRec采用多智能体框架，通过协作分析用户交互行为和迭代提示优化，专门化智能体能够推断用户的绿色导向意图，并优先推荐环保产品。该方法通过意图驱动的方式减少了不必要的交互和能源消耗。

Result: 在基准数据集上的广泛实验验证了LLMGreenRec在推荐可持续产品方面的有效性，证明了其在促进负责任数字经济方面的鲁棒解决方案。

Conclusion: LLMGreenRec通过多智能体协作框架有效弥合了用户绿色意图与实际购买行为之间的差距，同时降低了数字碳足迹，为负责任的数字经济发展提供了可行的技术方案。
