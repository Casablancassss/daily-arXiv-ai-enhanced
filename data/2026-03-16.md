...
TL;DR: 针对医学影像跨域泛化难题，提出域保角Bounds(DCB)理论框架评估域间因果差异，并设计GenEval多模态视觉语言模型方法，在糖尿病视网膜病变分级和癫痫发作起始区检测任务上显著提升单源域泛化性能。


<details>
  <summary>Details</summary>
Motivation: 在关键医学影像任务（如基于眼底图像的糖尿病视网膜病变分级和静息态fMRI癫痫发作起始区检测）中，跨域泛化仍然具有挑战性。当域间存在未知因果因素差异时，难以实现跨域泛化，且缺乏客观评估这种差异的方法论，通常无法获取数据提供者的元数据或协议级信息。

Method: 提出域保角Bounds(DCB)评估域间因果因素差异，并构建GenEval多模态视觉语言模型方法，融合MedGemma-4B等基础模型与人类知识，通过低秩适应(LoRA)增强单源域泛化能力。

Result: 在8个DR数据集和2个SOZ数据集上，GenEval实现了优异的单源域泛化性能：DR平均准确率69.2%，SOZ平均准确率81%，分别比最强基线高出9.4%和1.8%。

Conclusion: GenEval通过结合基础模型与人类知识 via LoRA，成功桥接因果差距，在糖尿病视网膜病变分级和癫痫发作起始区检测两个任务上实现了优异的单源域泛化性能，为医学影像领域的跨域泛化提供了新的方法论框架。
...
TL;DR: SPARROW是一个像素级视频MLLM，通过目标特定跟踪特征和双提示设计解决视频中空间精度与时间一致性的难题，在三个开源模型和六个基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有视频MLLM依赖静态分割令牌进行逐帧接地，缺乏时间上下文，导致物体移动或重现时出现空间漂移、身份切换和不稳定初始化等问题，亟需在空间精度和时间一致性之间取得平衡。

Method: 提出两大核心组件：(1)目标特定跟踪特征(TSF)在训练时注入时间对齐的引用线索；(2)双提示设计解码box和segmentation令牌以融合几何先验与语义接地。还构建了包含30,646个视频和45,231个QA对的引用视频数据集，并采用基于SAM2的类无关提议器实现端到端处理。

Result: 在六个基准上实现一致提升：RVOS上J&F提升+8.9，视觉接地mIoU提升+5，GCG上CLAIR提升+5.4，显著改善了引用稳定性、空间精度和时间连贯性。

Conclusion: SPARROW通过TSF和双提示设计有效解决了视频MLLM的空间精度和时间一致性难题，在三个开源模型和六个基准上取得显著提升，证明了像素级视频理解的稳定性和准确性可被大幅改善。
...
TL;DR: 本文提出了ABRA方法，用于在无训练图像的目标域中进行开词汇目标检测的域适应。ABRA将类别的检测知识从有标签源域迁移到无标签目标域，通过在预检测器权重空间中建立几何传输问题来实现。


<details>
  <summary>Details</summary>
Motivation: 现有开词汇目标检测模型（如Grounding DINO）在域偏移下性能显著下降，而许多实际应用场景（如夜间或雾天）缺乏大规模标注数据集，无法直接进行微调。因此需要一种能够在目标域无训练数据的情况下实现域适应的方法。

Method: ABRA将域适应问题表述为预训练检测器权重空间中的几何传输问题，对齐源域和目标域的专家，从而传输类别特定的检测知识。

Result: 在多种挑战性域偏移场景下的广泛实验表明，ABRA成功地在多种恶劣条件下实现了类别级专业知识的迁移。

Conclusion: ABRA能够在没有目标域训练图像的情况下，有效地将类别检测知识从源域迁移到目标域，为开词汇目标检测的域适应提供了一种新思路。
...
TL;DR: 本文研究异构视觉基础模型(VFM)之间的模型拼接可行性，发现使用目标模型倒数第二层的简单特征匹配损失可实现可靠的模型拼接，并提出VFM拼接树(VST)为多模态LLM提供可控制的精度-延迟权衡。


<details>
  <summary>Details</summary>
Motivation: 先前研究发现相同数据集上训练的模型可以相互拼接，但不同目标、数据和模态混合的异构VFMs（如CLIP、DINOv2、SigLIP 2）是否仍可拼接仍是未知数。

Method: 系统性地探索了拼接点、拼接层族、训练损失和下游任务，采用在目标模型倒数第二层使用简单特征匹配损失的方法。

Result: 发现传统特征匹配或端到端优化方法在浅层拼接点表现不佳，但提出的特征匹配损失方法使异构VFMs在视觉任务中实现可靠拼接。深层拼接点可在小推理开销下超越任一组成模型。VST可共享早期层并保留后期层，提供可控的精度-延迟权衡。

Conclusion: 模型拼接从诊断探针升级为整合互补VFM优势的实际方案，可识别模型表示的对齐或分歧位置。
...
TL;DR: AbSteering是一个异常中心的VideoLM框架，通过Chain-of-Thought推理和直接偏好优化，引导VideoLMs从HRCT生成精确诊断报告，优于现有最先进领域特定CT基础模型。


<details>
  <summary>Details</summary>
Motivation: 从HRCT生成精确诊断报告对临床工作流程至关重要，但因3D体积的高病理多样性和空间稀疏性而面临严峻挑战。VideoLMs在通用领域表现出色，但在特定领域高容量医学影像解释中的适应性尚未被充分探索。

Method: 提出AbSteering异常中心框架，包含：(i)异常中心Chain-of-Thought方案，强制进行异常推理；(ii)直接偏好优化(DPO)目标，利用临床易混淆异常作为硬阴性样本增强细粒度区分能力。

Result: 通用VideoLMs在AbSteering范式指导下展现出向高容量医学影像的强大迁移能力。AbSteering优于使用大规模CT预训练的最先进领域特定CT基础模型，实现了更高的检测灵敏度同时减少幻觉。

Conclusion: AbSteering框架成功地将通用VideoLMs应用于医学影像领域，优于使用大规模CT预训练的最先进领域特定CT基础模型，在提高检测灵敏度的同时有效减少幻觉问题。
...
TL;DR: GDO是一个目标驱动的数据优化框架，通过为每个候选样本计算六个描述符来构建优化的训练子集，在仅需约26-35k样本的情况下达到512k样本基线的性能，准确率提升0.84-3.08个百分点。


<details>
  <summary>Details</summary>
Motivation: 多模态指令调整通常计算效率低下，因为训练预算分散在大型混合图像-视频池中，且其效用高度不均衡。

Method: GDO框架为每个候选样本计算六个样本描述符，并为不同目标构建优化的1×训练子集。在8块H20 GPU上使用固定的一轮Qwen3-VL-8B-Instruct训练和评估方案。

Result: 与固定的512k样本Uni-10x基线相比，GDO在MVBench上仅需35.4k样本达到参考精度，准确率提升+1.38个百分点；VideoMME上需26.6k样本，提升+1.67个百分点；MLVU上需27.3k样本，提升+3.08个百分点；LVBench上需34.7k样本，提升+0.84个百分点。MVBench和MLVU提升最大，LVBench提升较小，这与其超长视频设置与短视频/图像主导的训练池之间的不匹配一致。在MinLoss、Diverse、Temp和Temp+中，更强的时间强调带来更好的长视频理解表现。

Conclusion: GDO提供了一个目标驱动的数据优化框架，能够在固定训练协议下使用更少的训练样本实现更快的收敛速度。代码已开源。
...
### [8] [RAW-Domain Degradation Models for Realistic Smartphone Super-Resolution](https://arxiv.org/abs/2603.12493)
*Ali Mosleh,Faraz Ali,Fengjia Zhang,Stavros Tsogkas,Junyong Lee,Alex Levinshtein,Michael S. Brown*

Main category: cs.CV

TL;DR: 本文研究智能手机数字变焦中的超分辨率问题，提出通过校准和设备特定降质建模来合成训练数据的方法，无需获取真实传感器配对图像即可训练出性能优异的RAW转RGB超分辨率模型。


<details>
  <summary>Details</summary>
Motivation: 智能手机上的数字变焦依赖于基于学习的超分辨率模型，但获取特定传感器的训练数据具有挑战性，因为缺乏真实的高分辨率-低分辨率配对图像。虽然可以通过"去处理"流程合成训练数据，但现有方法存在领域差距问题。

Method: 本文提出通过校准进行设备特定降质建模，将公开可用的渲染图像"去处理"到不同智能手机的RAW域中，从而生成训练数据对。具体包括：1) 不使用通用先验而是针对特定设备进行模糊和噪声建模；2) 利用校准获取设备特定的降质参数；3) 训练单一的RAW到RGB超分辨率模型。

Result: 实验表明，精确的降质建模能带来显著的性能提升，我们的超分辨率模型优于在大量任意选择降质方法上训练的基线模型。

Conclusion: 本文证明了在超分辨率训练中设备特定降质建模的重要性，为解决智能手机数字变焦的训练数据获取问题提供了有效途径。
...
### [9] [Naïve PAINE: Lightweight Text-to-Image Generation Improvement with Prompt Evaluation](https://arxiv.org/abs/2603.12506)
*Joong Ho Kim,Nicholas Thai,Souhardya Saha Dip,Dong Lao,Keith G. Mills*

Main category: cs.CV

TL;DR: 提出Naïve PAINE方法，通过预测初始噪声和给定提示词的图像质量，选择高质量噪声来改进扩散模型的生成效果。


<details>
  <summary>Details</summary>
Motivation: 扩散模型(DM)依赖随机高斯噪声，导致相同输入产生不同结果，用户需要多次生成才能获得满意结果，类似赌博。生成内容质量高度依赖于提示词和DM的生成能力。

Method: 利用T2I偏好基准，直接从初始噪声和提示词预测图像的数值质量，选择少量高质量噪声并将其传递给DM进行生成。该方法可提供关于DM对特定提示词生成质量的反馈，且足够轻量可无缝集成到现有DM流程中。

Result: 实验结果表明，Naïve PAINE在多个提示词语料库基准测试上优于现有方法。

Conclusion: 通过预测噪声质量并选择性使用高质量噪声，有效提高了扩散模型的生成质量，为T2I生成提供了一种轻量级且有效的改进方案。
...
### [10] [MemRoPE: Training-Free Infinite Video Generation via Evolving Memory Tokens](https://arxiv.org/abs/2603.12513)
*Youngrae Kim,Qixin Hu,C. -C. Jay Kuo,Peter A. Beerel*

Main category: cs.CV

TL;DR: MemRoPE是一种无需训练的长视频生成框架，通过记忆token和在线RoPE索引技术解决自回归扩散模型中的上下文遗忘问题，实现稳定的长程视频生成。


<details>
  <summary>Details</summary>
Motivation: 自回归扩散模型在实时帧流生成中，现有的滑动窗口缓存会丢弃历史上下文，导致保真度下降、身份漂移和运动停滞；现有方法保留固定早期token作为注意力锚点，但静态锚点无法反映不断增长的视频内容。

Method: 提出MemRoPE框架，包含两个协同设计的组件：1) Memory Tokens通过指数移动平均将所有历史键压缩为长期和短期流，维持全局身份和近期动态；2) Online RoPE Indexing缓存未旋转的键，在注意力计算时动态应用位置嵌入，避免位置相位冲突。

Result: 在分钟到小时级别的生成任务中，MemRoPE在时间一致性、视觉保真度和主体一致性方面优于现有方法。

Conclusion: MemRoPE无需训练即可实现分钟到小时级别的长视频生成，在时间一致性、视觉保真度和主体一致性方面均优于现有方法。
...
### [11] [Addressing Data Scarcity in 3D Trauma Detection through Self-Supervised and Semi-Supervised Learning with Vertex Relative Position Encoding](https://arxiv.org/abs/2603.12514)
*Shivam Chaudhary,Sheethal Bhat,Andreas Maier*

Main category: cs.CV

TL;DR: 本文提出了一种标签高效的方法，结合自监督预训练和半监督学习，用于腹部CT扫描中创伤性损伤的3D医学图像分析。


<details>
  <summary>Details</summary>
Motivation: 在急诊放射学中，腹部CT扫描的创伤性损伤准确检测和定位仍面临关键挑战，主要原因是带注释的医学数据严重稀缺。

Method: 1. 使用基于块的掩码图像建模（MIM）在大规模无注释CT数据上预训练3D U-Net编码器，学习鲁棒解剖表示；2. 将预训练编码器用于两个下游临床任务：使用VDETR和顶点相对位置编码进行3D损伤检测，以及多标签损伤分类；3. 采用一致性正则化的半监督学习策略。

Result: 检测任务：使用144个标注样本和2000个无标注样本，达到56.57%验证mAP@0.50和45.30%测试mAP@0.50，比纯监督训练提升115%；分类任务：使用2244个标注样本，达到94.07%测试准确率，涵盖7个损伤类别。

Conclusion: 自监督预训练结合半监督学习有效解决了医学影像中的标签稀缺问题，能够在有限注释下实现稳健的3D目标检测。
...
### [12] [Do You See What I Am Pointing At? Gesture-Based Egocentric Video Question Answering](https://arxiv.org/abs/2603.12533)
*Yura Choi,Roy Miles,Rolandos Alexandros Potamias,Ismail Elezi,Jiankang Deng,Stefanos Zafeiriou*

Main category: cs.CV

TL;DR: 针对自我中心AI助手的指向手势问答问题，提出EgoPointVQA数据集和HINT模型，通过3D手部关键点编码提供空间时间上下文，HINT-14B准确率68.1%，超越InternVL3-14B达6.6%。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型(MLLMs)在理解用户指向手势并回答相关问题方面存在困难，原因在于缺乏手势丰富的数据集，以及从自我中心视频中推断细粒度指向意图的能力有限。

Method: 提出Hand Intent Tokens (HINT)方法，使用现成的重建模型从3D手部关键点生成token，并将其与模型输入交错排列，为解释指向意图提供明确的空间和时间上下文。

Result: HINT-14B在6项任务上平均达到68.1%的准确率，超越当前最先进模型InternVL3-14B达6.6%。

Conclusion: 该研究提出了EgoPointVQA数据集和HINT模型，为基于手势指向的自我中心视觉问答提供了新的解决方案。HINT-14B在6项任务上平均达到68.1%的准确率，比InternVL3-14B高出6.6%，验证了3D手部关键点编码对理解指向意图的有效性。未来将开源代码、模型和数据集以推动该领域研究。
...
TL;DR: SERA是一种用于指代图像分割的时空语义专家路由架构，通过在视觉语言框架中引入表达式感知的专家细化来改进语义接地。该方法包含两个阶段：SERA-Adapter在选定骨干块中插入表达式条件适配器，通过专家引导细化和跨模态注意力提高空间一致性和边界精度；SERA-Fusion通过将标记特征重塑为空间网格并应用保几何专家变换来增强中间视觉表示。轻量级路由机制自适应加权专家贡献，同时兼容预训练表示。为使路由在冻结编码器下稳定，SERA使用参数高效调优策略，仅更新归一化和偏置项，影响不到1%的骨干参数。在标准指代图像分割基准测试中，SERA始终优于强基线，尤其在需要精确空间定位和边界描绘的表达式上表现更佳。


<details>
  <summary>Details</summary>
Motivation: 指代图像分割旨在为自然语言表达描述的图像区域生成像素级掩码。尽管预训练的视觉语言模型改进了语义接地，但现有方法仍依赖于均匀细化策略，无法完全匹配指代表达式的多样化推理需求。由于这种不匹配，预测通常包含碎片化区域、不准确边界甚至错误对象，特别是当为计算效率而冻结预训练骨干时。

Method: 提出SERA（时空语义专家路由架构），包含两个互补阶段的表达式感知专家细化：1）SERA-Adapter：在选定骨干块中插入表达式条件适配器，通过专家引导细化和跨模态注意力提高空间一致性和边界精度；2）SERA-Fusion：通过将标记特征重塑为空间网格并应用保几何专家变换来增强中间视觉表示的多模态交互。轻量级路由机制自适应权重专家贡献。为使路由在冻结编码器下稳定，仅更新归一化和偏置项（<1%骨干参数）。

Result: 在标准指代图像分割基准测试中，SERA一致优于强基线，在需要精确空间定位和精确边界描绘的表达式上具有特别明显的提升。

Conclusion: 通过引入表达式感知的专家路由机制和参数高效调优策略，SERA有效解决了传统均匀细化策略与多样化推理需求不匹配的问题，在保持计算效率的同时显著提升了指代图像分割的准确性和边界精度。
...
TL;DR: 诊断研究表明，VLMs在空间推理方面存在困难，原因是CLIP风格编码器和1D位置编码，但这些因素无法完全解释该问题。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）发展迅速，但在基本空间推理方面仍存在困难。论文认为这种失败不仅与数据问题相关，还与当前VLM管道中的主导设计选择密切相关：CLIP风格图像编码器的依赖，以及将图像展平为1D标记序列并使用1D位置编码的处理方式。

Method: 在LLaVA框架内进行受控诊断研究，评估前沿模型和LLaVA变体在空间基准测试上的表现，对比CLIP编码器与采用更密集或生成式目标的替代方案，以及添加2D位置编码的变体。

Result: 模型间存在一致的空间性能差距，编码器目标和位置结构会影响空间行为，但无法完全解决问题。

Conclusion: 当前VLM设计选择（CLIP编码器、1D位置编码）影响空间 grounding，但无法完全解释空间推理缺陷。
...
### [15] [Mastering Negation: Boosting Grounding Models via Grouped Opposition-Based Learning](https://arxiv.org/abs/2603.12606)
*Zesheng Yang,Xi Jiang,Bingzhang Hu,Weili Guan,Runmin Cong,Guo-Jun Qi,Feng Zheng*

Main category: cs.CV

TL;DR: 本文提出D-Negation数据集和分组对立学习框架，旨在解决视觉-语言模型难以处理否定语义的问题，通过在小样本条件下学习否定感知的表示，在正负语义评估中分别提升4.4和5.7 mAP。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言检测和定位模型主要关注正向语义提示，难以准确理解和定位包含否定语义的复杂表达，根本原因是缺乏高质量的训练数据来显式捕捉判别性负样本和否定感知的语言描述。

Method: 1) 构建D-Negation数据集，提供带有正向和负向语义描述的标注对象；2) 提出分组对立学习框架，将对立语义描述组织成结构化组，设计两个互补的损失函数来鼓励模型进行否定推理和语义限定符推理；3) 在少于10%的模型参数上进行微调。

Result: 在与最先进语言定位模型集成后，正向语义评估提升4.4 mAP，负向语义评估提升5.7 mAP。

Conclusion: 显式建模否定语义可显著增强视觉-语言定位模型的鲁棒性和定位精度，为处理自然语言中的否定表达提供了有效解决方案。
...
TL;DR: 本文提出CLGT框架，通过因果特征提取器、几何拓扑融合模块和数据自适应池化模块，有效解决跨视图地理定位中的视角差异和干扰因素问题，在多个基准数据集上达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 跨视图地理定位(CVGL)旨在通过将街景图像与对应的航拍图像进行匹配来估计地理位置，这对自动驾驶和复杂现实场景中的地图绘制至关重要。然而，由于显著的视角差异和干扰因素的存在，该任务仍面临巨大挑战。

Method: 提出CLGT框架，包含三个核心组件：1) 因果特征提取器(CFE)：通过因果干预机制减轻干扰因素影响，使模型聚焦于稳定且任务相关的语义信息；2) 几何拓扑融合(GT Fusion)模块：将鸟瞰视图(BEV)道路拓扑信息注入街景特征，以缓解极端视角变化导致的跨视图不一致；3) 数据自适应池化(DA Pooling)模块：增强语义丰富区域的特征表示。

Result: 在CVUSA、CVACT及其鲁棒性增强变体(CVUSA-C-ALL和CVACT-C-ALL)数据集上的广泛实验表明，CLGT实现了最先进的性能，特别是在具有挑战性的真实世界图像腐蚀情况下表现出色。

Conclusion: CLGT框架在跨视图地理定位任务中展现了卓越的性能，特别是在处理真实世界中的挑战性场景时表现出色。通过结合因果学习、几何拓扑融合和数据自适应池化技术，该方法有效解决了视角差异和干扰因素带来的问题，为自动驾驶和地图绘制提供了更可靠的解决方案。
...
### [17] [LR-SGS: Robust LiDAR-Reflectance-Guided Salient Gaussian Splatting for Self-Driving Scene Reconstruction](https://arxiv.org/abs/2603.12647)
*Ziyu Chen,Fan Zhu,Hui Zhu,Deyi Kong,Xinkai Kuang,Yujia Zhang,Chunmao Jiang*

Main category: cs.CV

TL;DR: 本文提出LR-SGS方法，通过引入结构感知的显著高斯表示，将LiDAR反射率作为光照不变材质通道与RGB联合对齐，在Waymo数据集上实现卓越重建性能，复杂光照场景PSNR超越OmniRe 1.18dB。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法要么仅依赖相机，要么仅将LiDAR用于高斯初始化或深度监督，未充分挖掘点云中的反射率信息及LiDAR与RGB的互补性，导致在高自运动和复杂光照等挑战性自动驾驶场景中出现性能下降。

Method: 1)从LiDAR提取几何和反射率特征点进行高斯初始化；2)通过显著变换和改进的密度控制进行高斯细化，捕获边缘和平面结构；3)将LiDAR强度校准为反射率并作为光照不变材质通道附加到每个高斯；4)与RGB联合对齐以强化边界一致性。

Result: 在Waymo Open数据集上实现了更优的重建性能，使用更少的高斯和更短的训练时间。特别是在复杂光照场景下，PSNR比OmniRe高1.18 dB。

Conclusion: LR-SGS通过引入结构感知的显著高斯表示，结合LiDAR反射率作为光照不变的材料通道，与RGB联合对齐增强边界一致性，在Waymo数据集上实现了卓越的重建性能。该方法使用更少的高斯和更短的训练时间，特别是在复杂光照场景下比OmniRe高1.18 dB PSNR，证明了LiDAR-相机融合在自动驾驶场景重建中的巨大潜力。
...
### [18] [AccelAes: Accelerating Diffusion Transformers for Training-Free Aesthetic-Enhanced Image Generation](https://arxiv.org/abs/2603.12575)
*Xuanhua Yin,Chuanzhi Xu,Haoxian Zhou,Boyu Wei,Weidong Cai*

Main category: cs.CV

TL;DR: 提出AccelAes框架，通过美学感知的时空约简无需训练地加速Diffusion Transformers，在Lumina-Next上实现2.11倍加速同时提升11.9%的ImageReward。


<details>
  <summary>Details</summary>
Motivation: DiTs因高可扩展性和高分辨率对齐能力成为文本到图像生成的主流骨干，但其二次方复杂度的自注意力导致高推理延迟，限制部署应用。

Method: 提出AccelAes框架，包含三个核心组件：1) AesMask：基于提示语义和交叉注意力信号生成一次性美学聚焦掩码；2) SkipSparse：当局部计算可行时，将计算和引导重新分配给掩码区域；3) 轻量级步级预测缓存：定期替换完整Transformer评估以减少时间冗余。

Result: 在代表性DiT家族上实验显示一致加速和美学质量提升。在Lumina-Next上实现2.11倍加速，ImageReward较密集基线提升11.9%。

Conclusion: AccelAes是一个无需训练的框架，通过美学感知的时空约简加速DiTs，同时提升感知美学质量。该方法在Lumina-Next上实现了2.11倍加速，ImageReward提升11.9%，证明了美学感知优化在提升生成效率与质量方面的双重价值。
...
### [19] [Marker-Based 3D Reconstruction of Aggregates with a Comparative Analysis of 2D and 3D Morphologies](https://arxiv.org/abs/2603.12667)
*Haohang Huang,Jiayi Luo,Issam Qamhia,Erol Tutumluer,John M. Hart,Andrew J. Stolba*

Main category: cs.CV

TL;DR: 本文提出了一种基于标记的摄影测量方法，用于骨料颗粒的3D重建，该方法成本低、操作简便，可用于建筑和交通基础设施中骨料的3D形态分析。


<details>
  <summary>Details</summary>
Motivation: 骨料是建筑和交通基础设施中的关键功能组件，其尺寸和形状信息对质量保证/质量控制(QA/QC)过程至关重要。然而，传统的3D表征方法（如3D激光扫描仪或X射线计算机断层扫描）设备昂贵，难以在采石场或施工现场广泛应用。

Method: 提出了一种基于标记的摄影测量方法，该方法包含标记设计以实现背景抑制、点云拼接和比例参考，从而获得高质量的骨料模型。对选定的骨料样本进行了重建精度验证，并对2D和3D形态特性进行了比较分析。

Result: 重建结果经过地面真值验证，表明该方法可行。2D和3D形态特性的比较分析显示存在显著差异。

Conclusion: 基于所提出的方法，可以轻松且低成本地获取骨料的3D形状信息，从而实现便捷的骨料检查、数据收集和3D形态分析。
...
### [20] [HSEmotion Team at ABAW-10 Competition: Facial Expression Recognition, Valence-Arousal Estimation, Action Unit Detection and Fine-Grained Violence Classification](https://arxiv.org/abs/2603.12693)
*Andrey V. Savchenko,Kseniia Tsypliakova*

Main category: cs.CV

TL;DR: 本文提出了一种基于预训练EfficientNet模型提取面部嵌入的快速方法，用于ABAW竞赛中的帧级面部表情识别、情感价效-唤醒度估计和行为单元检测任务，同时探讨了预训练架构在细粒度暴力检测中的应用。该方法通过置信度阈值和滑动窗口平滑策略提升预测稳定性，并在竞赛中取得了显著优于基线的验证结果。


<details>
  <summary>Details</summary>
Motivation: ABAW竞赛中的帧级面部情感理解任务需要快速准确的方法来处理面部表情识别、情感价效-唤醒度估计和行为单元检测，且现有方法在噪声处理和细粒度暴力检测方面存在挑战。

Method: 提出基于预训练EfficientNet情感识别模型提取面部嵌入的快速方法；对于高置信度预测直接使用，否则将嵌入输入到在AffWild2数据集上训练的多层感知机；使用固定大小的滑动窗口对帧级预测进行平滑；对暴力检测任务使用预训练架构提取帧嵌入并进行聚合用于视频分类。

Result: 该方法在ABAW挑战赛的四个任务上显著改善了验证指标，优于现有基线方法。

Conclusion: 在ABAW挑战赛的四个任务上，该方法显著改善了验证指标，证明了基于预训练模型嵌入提取的方法在情感识别任务上的有效性。
...
TL;DR: MRGeo是首个针对图像损坏环境下鲁棒交叉视图地理定位的系统性方法，通过空间通道增强块和区域级几何对齐模块的层次化防御策略，显著提升了模型在模糊、天气等损坏场景下的定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有交叉视图地理定位方法在标准数据集上表现接近完美，但对真实世界图像损坏环境（模糊、天气等）的鲁棒性研究不足，导致实际部署中严重性能下降或失败。

Method: 提出MRGeo方法，包含两个核心模块：1）空间通道增强块（SCEB），其中空间自适应表示模块（并行建模全局和局部特征，动态门控机制仲裁融合）和通道校准模块（多粒度通道依赖建模进行补偿调整）；2）区域级几何对齐模块（RGAM），对最终描述子施加几何结构确保粗粒度一致性。

Result: 在CVUSA-C-ALL、CVACT_val-C-ALL和CVACT_test-C-ALL三个鲁棒性基准上平均R@1提升2.92%，同时在跨区域评估中建立最优性能。

Conclusion: MRGeo通过层次化防御策略（空间通道增强块+区域级几何对齐模块）有效提升了交叉视图地理定位在图像损坏环境下的鲁棒性，在三个鲁棒性基准数据集上平均R@1提升2.92%，同时在跨区域评估中展现出色泛化能力。
...
### [22] [IGASA: Integrated Geometry-Aware and Skip-Attention Modules for Enhanced Point Cloud Registration](https://arxiv.org/abs/2603.12719)
*Dongxu Zhang,Jihua Zhu,Shiqi Li,Wenbiao Yan,Haoran Xu,Peilin Fan,Huimin Lu*

Main category: cs.CV

TL;DR: 本文提出IGASA点云配准框架，采用层级金字塔架构结合跨层注意力机制和几何感知细化模块，在多个基准数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在面对真实世界挑战时（如严重噪声、显著遮挡和大规模变换）往往失败，导致配准精度受损和在复杂环境中鲁棒性不足。

Method: 提出IGASA框架，基于层级金字塔架构(HPA)进行鲁棒多尺度特征提取和融合，集成了层级跨层注意力(HCLA)模块和迭代几何感知细化(IGAR)模块。HCLA利用跳跃注意力机制对齐多分辨率特征并增强局部几何一致性，IGAR模块利用粗匹配阶段建立的可靠对应关系进行精细匹配。

Result: 在3D(Lo)Match、KITTI和nuScenes四个广泛使用的基准数据集上进行评估，实验结果一致表明IGASA显著超越现有最先进方法，并在配准精度方面取得显著提升。

Conclusion: IGASA为点云配准技术提供了坚实的理论基础和卓越的性能表现，在自动驾驶、机器人和环境建模等实际3D视觉应用中具有重要的实用价值和推广意义。
...
### [23] [Neural Gate: Mitigating Privacy Risks in LVLMs via Neuron-Level Gradient Gating](https://arxiv.org/abs/2603.12598)
*Xiangkui Cao,Jie Zhang,Meina Kan,Shiguang Shan,Xilin Chen*

Main category: cs.CV

TL;DR: 论文提出Neural Gate方法，通过神经元级模型编辑来缓解LVLMs的隐私风险，学习特征向量定位隐私相关神经元，精确更新模型参数，在提升隐私保护的同时保持模型效用，并在未见过的隐私查询上展现泛化能力。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)被部署在金融、医疗等关键领域，但面临严重的安全和隐私风险。现有的隐私保护方法在泛化性和非破坏性方面存在局限，难以鲁棒地处理未见的隐私相关查询，且可能降低模型在标准任务上的性能。

Method: Neural Gate通过学习一个特征向量来识别与隐私相关概念相关的神经元，从而实现对模型参数的精确更新。该方法首先在模型的表征中对隐私相关主题进行神经元定位，然后利用定位结果指导模型参数的精准编辑。

Result: 在MiniGPT和LLaVA模型上的全面实验表明，Neural Gate显著提升了模型的隐私保护能力，将拒绝隐私相关问题的概率提高到更高水平，并将这种保护行为扩展到编辑过程中未见过的新敏感查询，同时有效保留了模型的原始效用。

Conclusion: Neural Gate通过神经元级模型编辑成功提升了LVLMs的隐私保护能力，在 unseen privacy-related queries 上展现出良好的泛化能力，同时保持了模型在标准任务上的原有性能，为大型多模态模型的隐私安全提供了新的解决方案。
...
### [24] [CognitionCapturerPro: Towards High-Fidelity Visual Decoding from EEG/MEG via Multi-modal Information and Asymmetric Alignment](https://arxiv.org/abs/2603.12722)
...
TL;DR: 本文提出受人类视觉系统启发的预测-感知(PAP)框架，通过将预测模块的输出作为感知模块的查询输入，实现预测与感知的深度融合。实验表明该框架在nuScenes数据集上使UniAD的目标跟踪精度提升10%，推理速度提升15%。


<details>
  <summary>Details</summary>
Motivation: 人类在观察快速移动的物体（如鸟类或昆虫）时，需要预测其下一位置并聚焦视线才能清晰感知。受此启发，提出将预测-感知架构集成到3D目标感知任务中，以提升模型的感知精度。

Method: PAP框架包含两个核心模块：预测模块和感知模块。预测模块基于当前帧感知结果预测自车和周围交通参与者的未来位置；这些预测位置作为查询传递给下一帧的感知模块；感知结果迭代反馈回预测模块。实验采用UniAD模型在nuScenes数据集上评估。

Result: 在nuScenes数据集上的评估表明，PAP结构使UniAD的目标跟踪精度提升10%，推理速度提升15%。

Conclusion: PAP框架通过将预测模块的输出作为感知模块的查询输入，实现了预测与感知的深度融合。这种双向迭代反馈机制模拟了人类视觉系统的预测-感知协同工作方式，在保持高感知精度的同时显著降低了计算资源消耗。实验结果表明，该框架在目标跟踪精度上提升10%，推理速度提升15%，证明了该生物启发式设计的有效性。
...
### [26] [MoKus: Leveraging Cross-Modal Knowledge Transfer for Knowledge-Aware Concept Customization](https://arxiv.org/abs/2603.12743)
*Chenyang Zhu,Hongxiang Li,Xiu Li,Long Chen*

Main category: cs.CV

TL;DR: 本文提出了知识感知概念定制（Knowledge-aware Concept Customization）新任务，旨在将多样化的文本知识绑定到目标视觉概念，并提出MoKus框架通过两阶段（视觉概念学习和文本知识更新）实现高保真定制生成，首个基准KnowCusBench验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 现有概念定制方法绑定稀有token存在性能不稳定问题，且稀有token无法传达目标概念的内在知识，因此需要一种将文本知识绑定到视觉概念的新方法。

Method: MoKus框架包含两个阶段：1）视觉概念学习阶段学习锚点表示以存储目标概念的视觉信息；2）文本知识更新阶段将知识查询的答案更新到锚点表示，实现跨模态知识转移，使文本模态的修改自然传递到视觉模态。

Result: MoKus在KnowCusBench基准上显著优于现有最先进方法，并可扩展到虚拟概念创建和概念擦除等应用，同时在世界知识基准上取得提升。

Conclusion: 知识感知概念定制是一个具有前景的新任务，跨模态知识转移机制为高保真定制生成提供了有效路径，MoKus框架为该领域奠定了基础。
...
TL;DR: 本文提出了A2Z数据集，包含1000万个多模态注释和100万个ABC CAD模型，用于解锁边界表示（BRep）学习。该数据集整合了高分辨率网格、带注释的3D手绘草图、BRep几何拓扑信息以及文本描述，并额外添加了25000个电子外壳CAD模型。作者基于15万CAD模型训练了基础模型，用于从3D扫描中检测BRep共边和角点顶点。


<details>
  <summary>Details</summary>
Motivation: 当前几何深度学习技术缺乏对参数化CAD特征的多模态理解，这些特征存储在边界表示（BRep）中。从3D扫描、草图或简单文本提示逆向工程和快速原型设计CAD模型在工业产品设计中至关重要，但现有方法无法充分处理BRep的复杂几何和拓扑信息。

Method: 构建了迄今为止最大的多模态CAD数据集A2Z，包含：1)高分辨率网格与显著3D扫描特征；2)配备BRep共边、角点和表面几何拓扑信息的3D手绘草图；3)描述机械产品的文本标题和标签。数据集存储需求近5TB。此外还合并了25000个电子外壳CAD模型。在15万CAD模型子集上训练基础模型，检测BRep共边和角点顶点。使用GPT-5、Gemini和广泛人工反馈评估数据集质量。

Result: 建立了包含1000万注释的100万ABC CAD模型数据集A2Z，以及25000个电子外壳CAD模型。在15万CAD模型上训练的基础模型能够从3D扫描中检测BRep共边和角点顶点，这是CAD逆向工程中的关键下游任务。数据集、指标和检查点将公开可用。

Conclusion: A2Z是迄今为止最大、最全面的多模态CAD数据集，为BRep学习提供了前所未有的数据支持。该数据集的发布将推动CAD逆向工程、检索和相关研究领域的发展。
...
### [28] [FC-Track: Overlap-Aware Post-Association Correction for Online Multi-Object Tracking](https://arxiv.org/abs/2603.12758)
*Cheng Ju,Zejing Zhao,Akio Namiki*

Main category: cs.CV

TL;DR: FC-Track是一种轻量级的在线多目标跟踪后关联校正框架，通过IoA过滤和外观相似度比较来解决目标重叠导致的身份切换问题，在MOT17和MOT20数据集上取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 在线多目标跟踪方法在复杂动态环境中容易受到频繁遮挡和目标重叠导致的身份切换影响，错误的关联会随时间传播并降低跟踪可靠性。现有方法需要全局优化或重识别，不适合实时机器人应用。

Method: 提出基于Intersection over Area(IoA)的过滤策略，在高重叠条件下抑制不可靠的外观更新；通过对重叠轨迹对进行外观相似度比较来局部校正检测-轨迹匹配错误，从而防止短期错误传播。

Result: MOT17测试集：81.73 MOTA、82.81 IDF1、66.95 HOTA，速度5.7 FPS；MOT20测试集：77.52 MOTA、80.90 IDF1、65.67 HOTA，速度0.6 FPS；长期身份切换仅29.55%，显著低于现有在线跟踪器；在MOT20上达到最优性能。

Conclusion: 该框架无需全局优化或重识别即可有效减少长期身份切换，适合实时机器人应用，在目标重叠场景下表现优异。
...
### [29] [Cheers: Decoupling Patch Details from Semantic Representations Enables Unified Multimodal Comprehension and Generation](https://arxiv.org/abs/2603.12793)
...
TL;DR: Cheers是一个统一多模态模型，通过将patch级细节与语义表示解耦，在稳定语义理解的同时通过门控细节残差提高图像生成质量。该模型在视觉理解和生成任务上表现优异，且训练效率高（仅需20%训练成本），4x token压缩率使其更加高效。


<details>
  <summary>Details</summary>
Motivation: 统一视觉理解和生成是多模态建模的前沿课题，但两项任务需要不匹配的解码机制和视觉表示，在共享特征空间中进行联合优化并非易事。

Method: 提出三个关键组件：(1)统一视觉分词器，将图像潜在状态编码压缩为语义token；(2)基于LLM的Transformer，统一文本自回归生成和图像扩散生成；(3)级联流匹配头，先解码视觉语义，再注入门控细节残差以细化高频内容。

Result: Cheers在流行基准测试中匹配或超越先进UMM；在视觉理解和生成方面表现优异；实现4x token压缩，实现更高效的高分辨率图像编码和生成；在GenEval和MMBench上超越Tar-1.5B，仅需20%训练成本。

Conclusion: Cheers实现了有效且高效的统一多模态建模，具有4x token压缩率。在GenEval和MMBench上超越了Tar-1.5B，同时仅需20%的训练成本，展示了在视觉理解和生成方面的卓越性能。
...
TL;DR: RoboStereo是一种对称双塔4D世界模型，通过双向跨模态增强解决几何幻觉问题，并首次提出统一的世界模型策略优化框架(TTPA、IEPL、OEPL)，在细粒度操作任务上实现超过97%的性能提升


<details>
  <summary>Details</summary>
Motivation: Scalable Embodied AI面临现实世界交互的高成本和安全风险限制，而现有的Embodied World Models (EWMs)存在几何幻觉问题，缺乏统一的优化框架来改进策略

Method: 提出RoboStereo对称双塔4D世界模型，采用双向跨模态增强确保时空几何一致性，缓解物理幻觉；在此基础上构建统一框架包含：(1)测试时策略增强(TTPA)用于执行前验证，(2)模仿进化策略学习(IEPL)利用视觉感知奖励从专家演示学习，(3)开放探索策略学习(OEPL)实现自主技能发现和自纠正

Result: RoboStereo达到最先进的生成质量，统一框架在细粒度操作任务上实现超过97%的平均相对提升

Conclusion: 该研究首次提出了世界模型基于策略优化的统一框架，通过双向跨模态增强有效解决几何和物理幻觉问题，在细粒度操作任务上取得显著性能提升
...
### [31] [Hierarchical Dual-Change Collaborative Learning for UAV Scene Change Captioning](https://arxiv.org/abs/2603.12832)
*Fuhai Chen,Pengpeng Huang,Junwen Wu,Hehong Zhang,Shiping Wang,Xiaoguang Ma,Xuri Ge*

Main category: cs.CV

TL;DR: 提出无人机场景变化captioning新任务(UAV-SCC)，解决移动视角下航拍图像对的语义变化描述问题，设计了HDC-CL方法包含DALT和HCM-OCC组件，并构建了UAV-SCC基准数据集。


<details>
  <summary>Details</summary>
Motivation: 传统变化captioning主要描述固定视角下图像对的差异，而UAV场景变化captioning需要处理移动相机产生的时空场景变化带来的视角偏移问题，核心挑战在于理解因视角变化导致的部分重叠场景中的变化。

Method: 提出层次化双变化协作学习(HDC-CL)方法，包含动态自适应布局Transformer(DALT)和层次化跨模态方向一致性校准(HCM-OCC)两个核心组件。DALT自适应建模图像对的多样空间布局，在统一编码层中学习重叠和非重叠区域的关联特征；HCM-OCC增强模型对视角变化方向的敏感性。

Result: 在新建的UAV-SCC数据集上进行了广泛实验，验证了所提方法的有效性并达到了最先进的性能。

Conclusion: 提出的HDC-CL方法在UAV-SCC任务上实现了最先进的性能，新构建的UAV-SCC数据集为该领域研究提供了重要基准。
...
### [32] [Team LEYA in 10th ABAW Competition: Multimodal Ambivalence/Hesitancy Recognition Approach](https://arxiv.org/abs/2603.12848)
*Elena Ryumina,Alexandr Axyonov,Dmitry Sysoev,Timur Abdulkadirov,Kirill Almetov,Yulia Morozova,Dmitry Ryumin*

Main category: cs.CV

TL;DR: 本文提出了一种用于无约束视频中犹豫/矛盾情绪识别得多模态方法，整合场景、面部、音频和文本四种模态，在BAH数据集上取得了71.43%的最终测试性能。


<details>
  <summary>Details</summary>
Motivation: 无约束视频中的犹豫/矛盾情绪识别是一个具有挑战性的问题，因为这种行为状态具有微妙、多模态和上下文依赖的特点。

Method: 提出了一种多模态融合方法，整合四种互补模态：场景（VideoMAE）、面部（情感帧级嵌入+统计池化）、音频（EmotionWav2Vec2.0+Mamba时序编码器）和文本（微调transformer）。使用多模态融合模型（包括原型增强变体）进一步结合单模态嵌入。

Result: 最佳单模态配置达到70.02%的平均MF1，最佳多模态融合模型达到83.25%，五个原型增强融合模型集成的最终测试性能为71.43%。

Conclusion: 结果强调了互补多模态线索和鲁棒融合策略对于犹豫/矛盾情绪识别的重要性。
...
### [33] [From Sparse to Dense: Multi-View GRPO for Flow Models via Augmented Condition Space](https://arxiv.org/abs/2603.12648)
...
TL;DR: 针对文本到图像流模型偏好对齐中单视图评估方案的局限性，提出MV-GRPO方法，通过条件增强器生成多样化字幕实现多视图优势重估计，显著提升了对齐性能


<details>
  <summary>Details</summary>
Motivation: 观察到标准GRPO范式在评估单条件下一组生成样本时存在样本间关系探索不足的问题，限制了对齐效能和性能上限

Method: 提出MV-GRPO方法，利用条件增强器生成语义相近但多样化的字幕，通过多视图优势重估计捕获多样化语义属性，为优化提供更丰富的信号，同时通过基于原始样本在新字幕条件下的概率分布实现无需重新生成样本的训练

Result: 广泛的实验表明MV-GRPO在文本到图像模型的偏好对齐任务中实现了优于最先进方法的对齐性能

Conclusion: MV-GRPO通过增强条件空间创建密集多视图奖励映射，有效解决了单视图评估方案的局限性，在文本到图像模型的偏好对齐任务中实现了最先进的性能
...
### [34] [Finite Difference Flow Optimization for RL Post-Training of Text-to-Image Models](https://arxiv.org/abs/2603.12893)
*David McAllister,Miika Aittala,Tero Karras,Janne Hellsten,Angjoo Kanazawa,Timo Aila,Samuli Laine*

Main category: cs.CV

TL;DR: 提出了一种用于扩散模型后训练的在线强化学习新方法，通过将整个采样过程作为单一动作并对配对轨迹进行对比，降低了模型更新方差，实现了更快的收敛和更好的图像质量与提示对齐。


<details>
  <summary>Details</summary>
Motivation: 现有方法将每个采样步骤视为单独的政策动作，导致模型更新方差较高，需要一种能够降低方差并加速收敛的强化学习方法。

Method: 提出了一种在线强化学习变体，通过采样配对轨迹并将流速拉向更有利的图像方向来降低模型更新方差。与现有方法不同，该方法将整个采样过程视为单一动作。实验采用高质量视觉语言模型和现成的质量指标作为奖励信号。

Result: 该方法收敛速度更快，输出质量更高，提示对齐效果优于以往方法。

Conclusion: 所提出的方法在收敛速度、输出质量和提示对齐方面均优于现有方法。
...
TL;DR: VGGT-World是一种几何世界模型，通过预测冻结的VGGT特征演化而非生成视频来实现场景预测，使用z预测参数化和两阶段流强制课程解决高维特征空间中的流匹配崩溃和曝光偏差问题，在深度预测任务上以更少参数和更快速度超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前预测场景演化的世界模型主要关注光度细节，导致几何不一致性问题，作者认为应绕过视频生成，直接预测几何基础模型（GFM）特征的时序演化。

Method: 1. 将冻结的VGGT潜在标记作为世界状态；2. 训练轻量级时间流变换器自回归预测未来轨迹；3. 采用清洁目标（z预测）参数化解决速度预测流匹配崩溃问题；4. 设计两阶段潜在流强制课程逐步让模型基于自身部分去噪 rollout 进行条件化。

Result: 在KITTI、Cityscapes和TartanAir数据集上，VGGT-World在深度预测方面显著超越最强基线，且推理速度快3.6-5倍，仅需0.43B可训练参数。

Conclusion: VGGT-World以仅0.43B可训练参数实现了显著优于最强基线的深度预测性能，运行速度快3.6-5倍，验证了冻结的几何基础模型特征是3D世界建模的有效且高效预测状态。
...
TL;DR: VFMRecon首次将视觉基础模型的迁移先验与场景级神经重建的尺度一致性要求结合，通过尺度对齐和轻量适配器实现跨域鲁棒的三维重建，在室外Tanks and Temples数据集上F1分数70.1%显著领先。


<details>
  <summary>Details</summary>
Motivation: 场景级神经体素重建在严重域迁移下仍具挑战，虽视觉基础模型(VFM)提供可迁移的跨域先验，但其尺度模糊预测与体素融合所需的尺度一致性不兼容。

Method: 1) 引入轻量级尺度对齐阶段恢复多视角尺度一致性；2) 通过轻量级任务特定适配器将预训练VFM特征融入神经体素重建管道，在保留跨域鲁棒性的同时训练重建任务。

Result: 在ScanNet测试集及域外TUM RGB-D和Tanks and Temples数据集上均达SOTA；Tanks and Temples上F1为70.1%，远超VGGT的51.8%。

Conclusion: VFMRecon通过轻量级适配器将VFM先验与尺度一致性结合，在ScanNet测试集及域外TUM RGB-D和Tanks and Temples数据集上实现SOTA性能，在Tanks and Temples上F1分数达70.1%，显著领先VGGT的51.8%。
...
### [37] [Stake the Points: Structure-Faithful Instance Unlearning](https://arxiv.org/abs/2603.12915)
*Kiseong Hong,JungKyoo Shin,Eunwoo Kim*

Main category: cs.CV

TL;DR: 本文针对机器遗忘中忽略语义关系维护的问题，提出利用语义锚点（stakes）保持知识结构的框架，通过结构感知对齐和正则化技术，在图像分类、检索和人脸识别任务上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 机器学习中的隐私风险推动了对机器遗忘（MU）的研究。现有方法在删除指定数据影响时，往往忽视保留数据之间的语义关系维护，导致模型遭受渐进性结构崩溃，破坏了删除-保留平衡。

Method: 提出structure-faithful框架，利用stakes（语义锚点）作为参考点来维持知识结构。具体包括：1）从语言驱动的属性描述中实例化锚点，使用语义编码器（如CLIP）编码；2）通过structure-aware alignment（结构感知对齐）使遗忘前后的知识结构围绕锚点对齐；3）通过structure-aware regularization（结构感知正则化） регулируя更新结构关键参数。

Result: 在图像分类、检索和人脸识别三个任务上的实验表明，该方法分别实现了32.9%、22.5%和19.3%的平均性能提升，有效平衡了删除-保留权衡并增强了泛化能力。

Conclusion: 本文提出的structure-faithful框架通过引入stakes（语义锚点）有效解决了机器遗忘中的结构崩溃问题。实验表明该方法在图像分类、检索和人脸识别任务上分别获得32.9%、22.5%和19.3%的性能提升，成功平衡了删除-保留权衡并增强了模型的泛化能力。
...
TL;DR: AVION是一个用于遥感图像视觉-语言模型适应的知识蒸馏框架，通过教师模块构建语义丰富的文本原型，学生模块集成可学习提示对齐跨模态嵌入，在六个遥感数据集上提升了少样本分类和跨模态检索性能。


<details>
  <summary>Details</summary>
Motivation: 遥感图像的视觉-语言模型适应面临两大挑战：文本表示的语义覆盖不足和视觉特征的适应性不足。这些问题在航拍场景中尤为突出，因为场景视觉外观多样且物体细粒度区分复杂。

Method: 提出AVION框架，包含：(1)教师模块利用大语言模型收集描述并通过遥感图像特征验证，构建语义丰富的文本原型；(2)学生模块将轻量级可学习提示注入视觉和语言编码器，在教师指导下对齐嵌入及其跨模态关系；(3)推理时学生模块独立运行。

Result: 在六个光学遥感基准数据集上的实验表明：AVION提升了少样本分类和基类准确率，且未损害对新类别的泛化能力；同时增强了跨模态检索的平均召回率，且仅需极少的可训练参数。

Conclusion: AVION通过知识蒸馏有效解决了遥感图像领域中视觉-语言模型适应的两大挑战，在保持对新类别泛化能力的同时，显著提升了少样本分类和跨模态检索性能，且仅需极少的可训练参数。
...
### [39] [SAW: Toward a Surgical Action World Model via Controllable and Scalable Video Generation](https://arxiv.org/abs/2603.13024)
...
TL;DR: SAW是一个手术动作世界模型，通过视频扩散技术生成逼真的外科手术 action 视频，仅需四个轻量级条件信号：语言提示、参考手术场景、组织 affordance 掩码和2D工具尖端轨迹。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法需要昂贵的标注或复杂的结构化中间条件信号，限制了可扩展性；其他方法在复杂腹腔镜场景中时间一致性有限，缺乏足够的真实感。手术世界模型可以解决外科AI和数据稀缺等基础挑战。

Method: 设计了一种条件视频扩散方法，将视频到视频扩散重新表述为轨迹条件的手术动作合成。骨干扩散模型在12,044个腹腔镜手术视频片段的数据集上进行微调，使用轻量级时空条件信号，并利用深度一致性损失来强制几何可行性，无需在推理时需要深度信息。

Result: SAW在保留测试数据上实现了最先进的时间一致性(CD-FVD: 199.19 vs. 546.82)和强大的视觉质量。在下游应用中：a)外科AI方面，使用SAW生成的视频增强稀有动作后，动作识别性能显著提升(剪辑F1: 20.93%→43.14%；切割: 0.00%→8.33%)；b)外科模拟方面，可从模拟器导出的轨迹点渲染工具-组织交互视频。

Conclusion: SAW通过轻量级条件信号实现了高质量手术视频生成，为外科AI数据增强和视觉保真模拟引擎提供了有效解决方案。
...
TL;DR: V3Fusion是一种基于视觉语言模型(VLM)集成的多模态推理框架，通过CKA焦散度度量评估模型在视觉嵌入上的分歧，并利用遗传算法剪枝无效模型，实现高效融合。在MMMU和MMMU-Pro基准上分别提升8.09%和4.87%准确率，同时在A-OKVQA和OCR-VQA生成任务上超越Intern-VL2-8b和Qwen2.5-VL-7b。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注基于语言的VLM集成，而本文旨在利用视觉和语言双重模态进行模型选择。通过引入焦散度(focial error diversity)来捕捉VLM之间的互补推理能力，解决多模型推理中的模型选择和路由问题。

Method: 1)提出CKA焦散度度量(CKA-focal)衡量模型在视觉嵌入上的分歧；2)构建候选VLM池的集成表面；3)应用遗传算法有效剪枝不增值的组件VLM；4)为每个任务识别最佳组合并融合输出，实现双焦散度融合预测。

Result: 在四个VLM基准(A-OKVQA、MMMU、MMMU-Pro、OCR-VQA)上验证：MMMU提升8.09%，MMMU-Pro提升4.87%；生成任务上超越Intern-VL2-8b和Qwen2.5-VL-7b。即使无多数共识或多数模型错误时仍保持高性能，并能动态捕捉认知不确定性、减少幻觉。

Conclusion: V3Fusion能够产生高性能的双焦散度融合预测，有效处理无多数共识场景和多数模型错误情况，通过动态捕捉认知不确定性减轻幻觉问题，为视觉语言推理提供了一种有效的异构模型集成方案。
...
### [41] [SortScrews: A Dataset and Baseline for Real-time Screw Classification](https://arxiv.org/abs/2603.13027)
*Tianhao Fu,Bingxuan Yang,Juncheng Guo,Shrena Sribalan,Yucheng Chen*

Main category: cs.CV

TL;DR: SortScrews是一个用于逐个视觉分类螺丝的数据集，包含560张512×512分辨率的RGB图像，涵盖6种螺丝类型和1个背景类，使用标准化采集设置拍摄。基线实验表明，尽管数据集规模有限，预训练模型通过迁移学习仍能取得良好的分类效果。


<details>
  <summary>Details</summary>
Motivation: 公开可用的螺丝分类数据集稀缺，特别是在自动化分拣系统中常见的受控单目标场景，因此需要建立专门的螺丝分类数据集。

Method: 使用ImageNet预训练的EfficientNet-B0和ResNet-18进行迁移学习，并对分类失败进行了详细分析。

Result: 轻量级模型在有限数据集上实现了较强的分类准确率，证明了受控采集条件下的有效性。

Conclusion: SortScrews数据集在有限规模下仍能达到较高的分类准确率，表明受控的采集条件能够支持有效的学习，即使在相对较小的数据集上也能取得良好效果。
...
### [42] [Team RAS in 10th ABAW Competition: Multimodal Valence and Arousal Estimation Approach](https://arxiv.org/abs/2603.13056)
*Elena Ryumina,Maxim Markitantov,Alexandr Axyonov,Dmitry Ryumin,Mikhail Dolgushin,Denis Dresvyanskiy,Alexey Karpov*

Main category: cs.CV

TL;DR: 提出一种ITW条件下多模态 valence-arousal估计方法，融合面部、行为和音频三个互补模态，采用Directed Cross-Modal MoE和Reliability-Aware融合策略，在Aff-Wild2数据集上CCC达到0.658


<details>
  <summary>Details</summary>
Motivation: ITW条件下连续情绪效价-唤醒度估计面临外观、头部姿态、光照、遮挡及个体表达差异等巨大挑战

Method: 1) 面部模态：GRADA帧级嵌入+Transformer时序回归；2) 行为模态：Qwen3-VL-4B-Instruct提取视频片段信息+Mamba建模跨片段时序动态；3) 音频模态：WavLM-Large+注意力统计池化+跨模态滤波；4) 融合策略： Directed Cross-Modal MoE融合+Reliability-Aware视听融合

Result: 在Aff-Wild2开发集上达到CCC=0.658

Conclusion: 在Aff-Wild2开发集上，多模态融合策略实现了0.658的CCC值，验证了所提方法的有效性
...
TL;DR: 提出STRAP-ViT，利用Jensen-Shannon散度检测对抗补丁影响的异常token，并通过随机复合变换减轻攻击，无需训练即可作为即插即用模块提升ViT对对抗补丁的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 对抗补丁可以劫持Vision Transformers的自注意力机制，通过高对比度区域吸引注意力并腐蚀class token导致错误分类。作者发现包含对抗噪声的token与正常token具有不同的统计特性，这为防御提供了理论基础。

Method: 提出两阶段防御机制：(1)检测阶段使用Jensen-Shannon散度识别行为异常的token；(2)缓解阶段对这些token应用随机复合变换使对抗噪声失效。转换的最小token数量设为能覆盖至少50%补丁的区域。该方法作为非可训练即插即用模块集成到ViT架构中，仅用于推理，计算开销极小且无需额外训练。

Result: 在ViT-base-16和DinoV2架构、ImageNet和CalTech-101数据集上，针对Adversarial Patch、LAVAN、GDPA和RP2等多种对抗攻击进行测试，鲁棒准确率保持在干净基线的2-3%范围内，优于现有最先进方法。

Conclusion: STRAP-ViT通过利用token统计特性差异实现对抗补丁防御，无需训练即可部署，为ViT架构提供了一种高效实用的鲁棒性提升方案，在多种架构和数据集上展现优异性能。
...
### [44] [Geometry-Guided Camera Motion Understanding in VideoLLMs](https://arxiv.org/abs/2603.13119)
*Haoan Feng,Sri Harsha Musunuri,Guan-Ming Su*

Main category: cs.CV

TL;DR: 针对VideoLLM相机运动识别能力不足的问题，构建了数据集和基准进行诊断，发现视觉编码器对相机运动线索表示薄弱，提出利用3D基础模型提取几何线索并通过结构化提示注入的轻量级解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前视频视觉语言模型(VideoLLM)很少显式表示相机运动信号，在细粒度运动基元识别上经常失败。相机运动是塑造视觉感知和电影风格的基本几何信号，但现有模型缺乏对这一重要信号的建模能力。

Method: 构建了三大模块：1)基准测试：创建CameraMotionDataset大规模合成数据集和CameraMotionVQA问答基准；2)诊断：对Qwen2.5-VL视觉编码器进行探针实验，发现相机运动线索表示薄弱；3)注入：提出轻量级模型无关管道，从3D基础模型提取几何线索，用时序分类器预测约束运动基元，通过结构化提示注入下游推理。

Result: 实验表明：1)多种现成VideoLLM在相机运动识别上存在显著错误；2)相机运动线索在视觉编码器中表示较弱，尤其在深层ViT块中；3)提出的方法改善了运动识别和相机感知响应。

Conclusion: 几何驱动的线索提取和结构化提示是实现相机感知VideoLLM和VLA系统的实用步骤，无需昂贵训练或微调。
...
### [45] [Visual-ERM: Reward Modeling for Visual Equivalence](https://arxiv.org/abs/2603.13224)
*Ziyu Liu,Shengyuan Ding,Xinyu Fang,Xuanlang Dai,Penghui Yang,Jianze Liang,Jiaqi Wang,Kai Chen,Dahua Lin,Yuhang Zang*

Main category: cs.CV

TL;DR: 针对视觉到代码强化学习中的奖励信号不对齐问题，提出Visual-ERM多模态生成式奖励模型，能够在渲染视觉空间中提供细粒度反馈，显著提升图表、表格和SVG解析任务效果，并在VC-RewardBench基准上以小参数模型超越大模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉到代码任务的强化学习方法面临奖励信号不对齐的问题：现有奖励要么依赖文本规则，要么依赖粗粒度的视觉嵌入相似度，两者都无法捕捉细粒度的视觉差异，且容易受到奖励黑客攻击。

Method: 提出Visual-ERM（视觉等价奖励模型），一个多模态生成式奖励模型，直接在渲染视觉空间中评估视觉到代码质量，提供细粒度、可解释的任务无关反馈，并将其集成到强化学习流程中。

Result: 1. Qwen3-VL-8B-Instruct在图表到代码任务上提升+8.4分；2. 表格和SVG解析任务平均提升+2.7和+4.1分；3. 通过反思和修订进一步增强了测试时扩展；4. 在VC-RewardBench基准上，8B参数的Visual-ERM性能超越Qwen3-VL-235B-Instruct，接近领先的闭源模型。

Conclusion: Visual-ERM作为一种多模态生成式奖励模型，通过在渲染视觉空间中提供细粒度、可解释且任务无关的反馈，有效解决了视觉到代码强化学习中的奖励信号对齐问题。实验表明，该方法在图表、表格和SVG解析任务上均取得显著提升，且在VC-RewardBench基准测试中以8B参数超越了235B参数的模型，验证了细粒度视觉奖励监督的必要性和充分性。
...
TL;DR: VCBench是一个流式计数基准，将计数作为诊断视频模型世界状态维护能力的最小化探针，包含406个视频、10071个事件标注和1000个问答对，形成8个细粒度子类别，揭示了当前模型在状态维护方面的显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解基准虽在多个维度取得进展，但对模型如何维护世界状态的观察仍然不足。需要一个能够诊断模型世界状态维护能力的最小化探针。

Method: 将计数能力分解为对象计数（追踪当前可见对象 vs 追踪累计唯一标识）和事件计数（检测瞬时动作 vs 追踪完整活动周期），形成8个细粒度子类别。通过流式多点查询观察状态维护轨迹，设计了三个互补指标：数值精度、轨迹一致性和时间感知能力。

Result: 评估主流视频语言模型显示，当前模型在时空状态维护方面仍存在显著缺陷，尤其在周期性事件计数等任务上表现困难。

Conclusion: VCBench为视频理解系统中的状态维护能力提供了诊断框架，揭示了当前主流视频语言模型在时空状态维护方面的显著缺陷，特别是在周期性事件计数任务上表现不佳。该基准有助于衡量和改进视频理解系统的状态维护能力。
...
TL;DR: 本文提出HFP-SAM框架，用于高性能海洋动物分割。针对SAM在细粒度细节和频率信息感知方面的不足，设计了频率引导适配器(FGA)、频率感知点选择(FPS)和全视角Mamba(FVM)三个模块，在4个公开数据集上验证了优越性能。


<details>
  <summary>Details</summary>
Motivation: 海洋动物分割任务面临复杂海洋环境和长距离建模挑战。现有深度学习方法在此方面表现不足，而新兴的SAM虽在通用分割表现出色，但缺乏细粒度细节和频率信息感知能力。

Method: 提出层次化频率提示SAM框架，包含三大创新：1)频率引导适配器(FGA)通过频域先验掩码将海洋场景信息注入冻结的SAM主干；2)频率感知点选(FPS)通过频域分析生成高亮区域，与SAM粗预测结合生成点提示；3)全视角Mamba(FVM)以线性复杂度提取空间和通道上下文信息。

Result: 在4个公开数据集上进行了大量实验，验证了方法的有效性。

Conclusion: HFP-SAM在海洋动物分割任务上实现了优越性能，代码已开源。
...
TL;DR: 本文研究无监督跨域图像检索问题，针对现有方法中伪标签语义指导不准确、域对齐忽视信息纠缠导致语义退化的问题，提出TPSNet方法。该方法通过CLIP生成的文本先验和域不变相位特征构成的相位先验双重协同，在保持语义完整性的同时实现跨域对齐，实验表明其性能大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 无监督跨域图像检索（UCDIR）旨在不依赖标注数据的情况下检索跨域的同类图像。现存方法通常使用聚类算法产生的伪标签作为监督信号，但这些离散的伪标签难以提供准确、全面的语义指导。此外，对齐过程往往忽视域特定信息和语义信息之间的纠缠，导致学习到的表示出现语义退化，最终损害检索性能。

Method: 提出Text-Phase Synergy Network with Dual Priors (TPSNet)。具体包括：1）利用CLIP为每个域生成类特定提示（域提示），作为文本先验提供更精确的语义监督；2）引入相位先验（域不变相位特征），将其集成到原始图像表示中，在保持语义完整性的同时弥合域分布差距；3）利用双重先验的协同作用进行跨域对齐。

Result: 在UCDIR基准数据集上显著优于现有最先进的方法。

Conclusion: 通过引入文本先验（CLIP生成的域提示）和相位先验（域不变相位特征）的协同机制，TPSNet有效解决了传统方法中语义退化的问题，为无监督跨域图像检索提供了一种新的有效框架。
...
TL;DR: UNIStainNet利用病理基础模型UNI的密集空间tokens作为条件，指导SPADE-UNet进行H&E到IHC的虚拟染色转换，通过错位感知损失和染色嵌入实现单一模型处理多种染色标记，在MIST和BCI数据集上达到最先进的分布指标。


<details>
  <summary>Details</summary>
Motivation: 从H&E图像虚拟生成免疫组化(IHC)染色可以在组织有限时减少重复切片，加速诊断并提供初步的分子信息。现有方法通过对比学习、原型匹配或域对齐提高真实感，但生成器本身未获得病理基础模型的直接指导。

Method: 提出UNIStainNet，一个SPADE-UNet，以冻结的病理基础模型(UNI)的密集空间 tokens为条件，提供组织级语义指导用于染色转换。采用错位感知损失套件保持染色量化准确性，并使用学习的染色嵌入使单个模型能够同时服务于多个IHC标记物。

Result: 在MIST数据集上，UNIStainNet在所有四种染色(HER2、Ki67、ER、PR)上实现最先进的分布指标，且来自单一统一模型(先前方法通常训练单独的per-stain模型)。在BCI上也实现最佳分布指标。组织类型分层的失败分析表明剩余错误是系统性的，集中于非肿瘤组织。

Conclusion: 成功利用基础模型为染色转换提供组织级语义指导，以统一模型实现多个染色的最先进性能，剩余错误主要存在于非肿瘤组织中。
...
TL;DR: 针对文档布局分析(DLA)现有评估指标的局限性，提出结构语义单元(SSU)标注方法和COTe评分体系，以更准确地衡量页面解析质量。


<details>
  <summary>Details</summary>
Motivation: 当前DLA模型评估使用的IoU、F1、mAP等指标原本设计用于2D投影的3D空间图像，而非原生2D印刷媒体，导致模型性能评估可能产生误导性或不准确的解读。

Method: 提出结构语义单元(SSU)关系标注方法，将重点从物理结构转向语义结构；提出覆盖、重叠、侵入和过剩(COTe)评分，这是一个可分解的页面解析质量度量标准。

Result: COTe评分比传统指标更具信息量，能揭示模型的独特失败模式（如语义边界突破、重复解析同一区域）；相对F1将解释-性能差距减少76%；即使没有显式SSU标注也能保持良好的细粒度鲁棒性。

Conclusion: COTe评分比传统指标更具信息量，能揭示模型的不同失败模式，即使在没有显式SSU标注的情况下也能保持鲁棒性，有效缩小了解释-性能差距。
...
### [51] [Thinking in Dynamics: How Multimodal Large Language Models Perceive, Track, and Reason Dynamics in Physical 4D World](https://arxiv.org/abs/2603.12746)
...
TL;DR: Dyn-Bench是一个用于评估多模态大语言模型时空推理能力的大规模基准,包含1k视频、7k VQA对和3k动态目标定位对。研究发现现有模型难以同时具备时空推理和动态目标定位能力,而结构化整合方法可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在静态视觉理解方面表现出色,但在"动态思考"能力上存在不足,即在 evolving scenes 中感知、跟踪和推理时空动态的能力尚未得到系统评估

Method: 构建Dyn-Bench基准（包含1k视频、7k VQA对、3k动态目标定位对）; 对通用型、空间型和区域级MLLMs进行多阶段评估; 测试链式思考和caption-based提示等传统策略; 验证Mask-Guided Fusion和ST-TCM结构化整合方法

Result: 现有模型无法同时保持时空推理和动态目标定位的强性能,常产生运动和交互的不一致解释; 传统提示策略改进有限; 结构化整合方法可显著增强MLLMs的动态感知和时空推理

Conclusion: 现有模型难以同时在时空推理和动态目标定位方面保持 strong performance; 传统提示策略改进有限; Mask-Guided Fusion和ST-TCM等结构化整合方法可显著提升MLLMs的动态感知和时空推理能力
...
### [52] [SLICE: Semantic Latent Injection via Compartmentalized Embedding for Image Watermarking](https://arxiv.org/abs/2603.12749)
*Zheng Gao,Yifan Yang,Xiaoyu Li,Xiaoyan Feng,Haoran Fan,Yang Song,Jiaojiao Jiang*

Main category: cs.CV

TL;DR: 针对扩散模型图像来源追踪的语义水印方法SLICE，通过将图像语义解耦为四个因子（主体、环境、动作、细节）并锚定到初始噪声的不同区域，实现细粒度的语义绑定和篡改定位。


<details>
  <summary>Details</summary>
Motivation: 现有语义感知水印方法依赖单一全局语义绑定，容易被局部但全局连贯的语义编辑攻击破解；需要更细粒度的语义绑定来实现可定位的篡改检测。

Method: 提出SLICE框架，将图像语义解耦为四个因子（subject, environment, action, detail），将每个语义因子精确锚定到初始高斯噪声的不同区域，实现细粒度的语义绑定。

Result: SLICE显著优于现有基线方法，有效抵抗语义引导的再生攻击，大幅降低攻击成功率，同时保持图像质量和语义保真度。

Conclusion: SLICE提供了一种无需训练的实用图像来源解决方案，具有细粒度诊断能力和对现实对抗性操作的鲁棒性。
...
### [53] [Show, Don't Tell: Detecting Novel Objects by Watching Human Videos](https://arxiv.org/abs/2603.12751)
...
TL;DR: 本文提出"Show, Don't Tell"自监督系统，让机器人通过观察人类演示自动创建数据集并训练定制目标检测器，从而快速识别新物体，无需语言描述或提示工程。


<details>
  <summary>Details</summary>
Motivation: 现有闭集目标检测器难以识别演示中的新物体（分布外），开集检测器虽可识别但需要昂贵且繁琐的人工提示工程来区分新物体实例。

Method: 开发集成了"Show, Don't Tell"范式的机器人系统，在人类任务演示中自动创建数据集并训练定制检测器，通过向检测器"展示"目标物体而非用语言"告知"来绕过语言依赖。

Result: 实验表明该pipeline在manipulated objects的检测和识别方面显著优于最先进方法，提升了机器人任务完成率。

Conclusion: 该范式无需语言描述和提示工程即可快速训练定制检测器，为机器人实时识别演示中新物体提供了有效解决方案。
...
TL;DR: SAP是一个用于4K高分辨率全景实例分割的基础模型，通过将全景分割转化为固定轨迹的透视视频分割任务，在保留4K分辨率的同时恢复了视角转换的平滑性，并在零样本条件下相比SAM2提升17.2 mIoU。


<details>
  <summary>Details</summary>
Motivation: 当前在具身系统和AR系统中广泛采用的提示性实例分割，其在透视图像上训练的基础模型在360°全景图上性能通常会下降。

Method: 将全景分割重新定义为固定轨迹透视视频分割，将全景图分解为沿连续球面遍历采样的重叠透视 patches。该方法在保留4K原生分辨率的同时，恢复平滑视角转换所需的稳定性。

Result: 在真实世界4K全景图基准上，SAP相比不同规模的 vanilla SAM2 实现了 +17.2 的零样本 mIoU 提升。

Conclusion: SAP通过将全景分割重新定义为固定轨迹透视视频分割，成功解决了在4K高分辨率全景图像上进行实例分割的难题。该方法保留了原生4K分辨率，同时恢复了跨视图传播所需的平滑视角转换。通过大规模合成数据训练，SAP在真实世界360°图像上展现出良好的泛化能力。
...
### [55] [HIFICL: High-Fidelity In-Context Learning for Multimodal Tasks](https://arxiv.org/abs/2603.12760)
*Xiaoyu Li,Yuhang Liu,Zheng Luo,Xuanshuo Kang,Fangqi Lou,Xiaohua Wu,Zihan Xiong*

Main category: cs.CV

TL;DR: 针对大视觉语言模型的情境学习(ICL)性能敏感和计算昂贵问题，提出HiFICL方法，通过虚拟键值对、低秩分解和端到端训练来更准确地建模ICL机制，在多模态基准测试中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前ICL方法对演示配置敏感且计算昂贵，现有近似方法通过学习"偏移向量"简化处理，但无法准确建模ICL机制的真实影响。

Method: 提出高保真上下文学习(HiFICL)方法，包含三个核心组件：1)虚拟键值对作为可学习上下文；2)低秩分解实现稳定正则化训练；3)简单端到端训练目标。从上下文感知参数高效微调角度建模ICL机制。

Result: 在多个多模态基准测试中一致优于现有近似方法

Conclusion: HiFICL通过虚拟键值对、低秩分解和端到端训练目标，在多个多模态基准测试中一致优于现有近似方法，验证了高保真ICL机制的有效性。
...
TL;DR: TerraFlow是一种新型多模态多时间学习框架，通过时间训练目标实现序列感知学习，在GEO-Bench-2基准测试上超越现有最先进基础模型，并在自然灾害风险图预测任务上展现独特能力。


<details>
  <summary>Details</summary>
Motivation: 当前方法在处理真实世界地球观测数据中常见的可变长度输入时存在局限性，缺乏对空间、时间和模态的序列感知学习能力。

Method: TerraFlow建立在时间训练目标之上，能够在空间、时间和模态上进行序列感知学习，同时对真实世界地球观测数据中常见的可变长度输入保持鲁棒性。

Result: TerraFlow在GEO-Bench-2基准测试的所有时间任务上表现优于最先进的基础模型，并在自然灾害风险图预测方面取得初步进展，而其他最先进的基础模型经常在此任务上失败。

Conclusion: TerraFlow在F1分数上优于最先进的基础模型高达50%，在Brier分数上优于24%，标志着地球观测多模态多时间学习的重要进展。
...
TL;DR: 提出Ego→Exo模仿错误检测新任务，处理跨视角、异步、长度不匹配的视频。提出SAVA-X框架，通过视图条件自适应采样、场景自适应视图嵌入和双向交叉注意力融合解决领域迁移和时间对齐问题，在EgoMe基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设单视角设定，无法处理使用第三人称（外视角）演示来评估第一人称（内视角）模仿的实际场景。跨视角领域迁移、时间不对齐和大量冗余是关键挑战。

Method: 提出SAVA-X框架，包含三个核心组件：(i)视图条件自适应采样，(ii)场景自适应视图嵌入，(iii)双向交叉注意力融合

Result: SAVA-X在EgoMe基准上在AUPRC和mean tIoU上均优于所有基线方法

Conclusion: SAVA-X在EgoMe基准上显著优于所有基线方法，验证了各组件的互补优势。代码已开源。
...
TL;DR: Catalyst4D是一个将高质量3D编辑迁移到动态4D高斯场景的框架，同时保持空间和时间一致性，解决了现有方法在动态场景编辑中产生的运动伪影、时间闪烁和风格传播不一致问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景编辑使用NeRF和3DGS技术已能实现高质量的静态场景编辑，但动态场景编辑仍具挑战性。直接将2D扩散模型扩展到4D的方法往往产生运动伪影、时间闪烁和风格传播不一致等问题。

Method: 核心方法包括两部分：1) 基于锚点的运动引导(AMG)：从原始和编辑后的高斯中构建一组结构稳定、空间代表性的锚点，通过最优传输建立对应关系，实现一致的变形传播，避免跨区域干扰和运动漂移；2) 颜色不确定性引导的外观细化(CUAR)：通过估计每个高斯的颜色不确定性，选择性细化容易出现遮挡伪影的区域，保持时间外观一致性。

Result: 实验表明Catalyst4D实现了时间稳定、高保真的动态场景编辑，在视觉质量和运动一致性方面优于现有方法。

Conclusion: Catalyst4D成功地将高质量3D编辑迁移到动态4D高斯场景，通过锚点对应和颜色不确定性估计技术有效解决了动态编辑中的时间一致性问题，达到了领先的编辑效果。
...
### [59] [PVI: Plug-in Visual Injection for Vision-Language-Action Models](https://arxiv.org/abs/2603.12772)
...
TL;DR: 提出PVI模块，通过零初始化残差路径将时序视觉特征注入动作专家，解决VLA架构中细粒度几何线索衰减和时序信息缺失问题，实现轻量级单阶段微调。


<details>
  <summary>Details</summary>
Motivation: 现有VLA架构中，预训练VLM通常针对语义抽象优化且基于静态视觉观察，容易削弱细粒度几何线索，缺乏动作专家所需的时序证据。现有方法或聚焦静态空间表征，或需大幅修改架构，时序信息利用不足。

Method: 提出Plug-in Visual Injection (PVI)，一种轻量级、编码器无关的模块，通过零初始化残差路径将辅助视觉表征注入预训练动作专家，仅需单阶段微调即可保留预训练行为。

Result: 相比基线策略和多种竞争性注入策略，PVI持续获得性能提升。控制实验表明时序视频特征(V-JEPA2)优于强静态图像特征(DINOv2)，在需要状态跟踪和协调的多阶段任务中提升最大。真实机器人长程双臂布料折叠实验验证了实用性。

Conclusion: PVI通过零初始化残差路径注入视觉特征，在保持预训练行为的同时实现单阶段微调。实验表明时序视频特征(V-JEPA2)显著优于静态图像特征(DINOv2)，在多阶段任务中提升最大。真实机器人长程双臂布料折叠任务验证了PVI的实用性。
...
TL;DR: 该研究构建了包含11,000+视频片段的最大的基础外科动作数据集，开发了跨专科通用识别基础模型，验证了在技能评估和行动规划等下游应用中的有效性，为外科超级智能奠定基础。


<details>
  <summary>Details</summary>
Motivation: 人工智能、成像技术和大型语言模型有潜力改变外科实践、培训和自动化。理解和建模基础外科动作(BSA)是推动该领域发展的关键，但此前缺乏大规模的BSA数据集和通用基础模型。

Method: 构建了包含10种基础外科动作、覆盖6个外科专科、超过11,000个视频片段的BSA数据集；基于该数据集开发了通用的基础动作识别基础模型；在不同手术类型和身体部位的数据集上验证跨专科性能；通过前列腺癌根治术的技能评估、胆囊切除术和肾切除术的行动规划展示了下游应用；由多国外科医生评估语言模型输出的可解释文本。

Result: 该方法是迄今为止最大的BSA数据集；基础模型展现出强大的跨专科性能；在前列腺癌根治术的技能评估和胆囊切除术、肾切除术的行动规划中展现出良好的应用效果；多国外科医生评估认为语言模型输出具有临床相关性。

Conclusion: 基础外科动作可以在不同场景下被稳健识别，准确理解基础外科动作的模型可以有效促进复杂应用的发展，加速外科超级智能的实现。
...
TL;DR: 本文提出ME-RSRG多实体推理遥感接地基准数据集和EAR框架，将遥感接地任务重新定义为多实体推理任务，采用监督微调和实体感知奖励驱动的GRPO优化方法，实验验证了框架的有效性。


<details>
  <summary>Details</summary>
Motivation: 推理语言模型和可验证奖励的强化学习最新进展显著提升了多步推理能力，这促使将推理范式扩展到遥感视觉接地任务。现有遥感接地方法仍局限于感知级匹配和单实体公式，限制了显式推理和实体间建模的能力。

Method: 提出Entity-Aware Reasoning(EAR)框架，基于视觉语言基础模型构建，采用监督微调进行冷启动初始化，并通过实体感知的奖励驱动Group Relative Policy Optimization(GRPO)进行进一步优化，生成结构化推理轨迹和主体-客体接地输出。

Result: 在ME-RSRG数据集上的广泛实验证明了多实体推理任务的挑战性，并验证了所提出的EAR框架的有效性。

Conclusion: 该研究成功构建了ME-RSRG多实体推理基准数据集，并提出了EAR框架来应对遥感视觉接地任务中的多实体推理挑战。通过监督微调和基于实体感知的奖励驱动GRPO优化，EAR框架能够生成结构化推理轨迹和主客体检索结果。实验验证了多实体推理任务的挑战性和EAR框架的有效性。
...
TL;DR: 针对3D Gaussian Splatting资源目标攻击问题，提出Spectral Defense方法，通过3D频率滤波器剪枝异常高频Gaussians，并利用2D光谱正则化区分自然频率与噪声模式，有效抑制过度生长、降低内存消耗、提升渲染速度。


<details>
  <summary>Details</summary>
Motivation: 当前3D Gaussian Splatting面临资源目标攻击（resource-targeting attack）的威胁，攻击者通过污染训练图像诱导Gaussian过度增长导致资源耗尽。现有的空间域防御策略（如平滑、阈值化、剪枝）仅关注可见结构，忽视 stealthy perturbations 对训练数据底层光谱行为的扭曲，导致噪声模式被误解释为细节结构，引起不稳定的Gaussian过度生长。

Method: 提出Spectral Defense方法，包含两个核心组件：1) 3D频率滤波器：选择性剪枝表现出异常高频的Gaussians；2) 2D光谱正则化：区分自然各向同性频率同时惩罚各向异性角能量，以约束噪声模式。

Result: 实验表明，该防御方法在遭受攻击时：抑制过度生长达5.92×，内存减少达3.66×，速度提升达4.34×。

Conclusion: Spectral Defense通过3D频率滤波器和2D光谱正则化技术，有效构建了鲁棒、准确、安全的3DGS。该方法在资源消耗、场景保真度和安全性之间取得了良好的平衡，为3DGS抵御投毒攻击提供了新的防御范式。
...
TL;DR: 提出OARS框架，一种基于COMPASS MLLM奖励模型的在线感知对齐方法，用于真实世界图像超分辨率，解决感知-保真度权衡问题。


<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率模型与人类视觉偏好对齐具有挑战性，原因包括：1) 感知-保真度权衡；2) 多样且未知的退化；3) 现有离线偏好优化和静态度量聚合方法不可解释性强，且在强条件下容易出现伪多样性。

Method: 提出OARS过程感知在线对齐框架，基于COMPASS：1) MLLM奖励模型，联合建模保真度保持和感知增益，具有输入质量自适应权衡；2) 构建COMPASS-20K数据集，包含合成和真实退化；3) 引入三阶段感知标注流程，生成校准的细粒度训练标签；4) 使用渐进式在线对齐，从冷启动流匹配到全参考最终无参考RL，通过浅层LoRA优化进行策略探索。

Result: 广泛实验和用户研究表明，OARS在保持保真度的同时实现一致的感知提升，在Real-ISR基准上达到最先进性能。

Conclusion: OARS框架通过COMPASS MLLM奖励模型和渐进式在线对齐策略，成功解决了超分辨率中的感知-保真度权衡问题，为真实世界图像超分辨率提供了有效的感知对齐方法。
...
TL;DR: coDrawAgents是一个多智能体协作的文本到图像生成框架，通过Interpreter、Planner、Checker和Painter四个专业智能体的分工合作，有效解决了复杂场景中多对象组合和属性保留的难题，在基准测试中展现了优异的性能。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成技术快速发展，但现有模型在复杂场景中仍难以准确组合多个对象并保持其属性一致性。这是当前文本到图像生成领域面临的主要挑战。

Method: 提出了coDrawAgents，一个交互式多智能体对话框架，包含四个专业智能体：Interpreter（解释器）、Planner（规划器）、Checker（检查器）和Painter（绘制器）。Interpreter自适应决定直接文本到图像路径或布局感知多智能体过程；Planner采用分而治之策略，逐步为同一语义优先级对象提出布局；Checker通过验证空间一致性和属性对齐来纠正错误；Painter逐步合成图像。

Result: 在GenEval和DPG-Bench基准测试上的广泛实验表明，coDrawAgents在文本图像对齐、空间准确性和属性绑定方面相较于现有方法有显著改进。

Conclusion: coDrawAgents通过四个专业智能体的协作，有效解决了文本到图像生成中的三个关键挑战：降低布局复杂度、将规划基于视觉上下文以及实现显式错误修正。该框架在复杂场景中对象组合和属性保留方面表现出显著改进。
...
TL;DR: ERBA是一种新型酶动力学参数预测框架，通过两阶段条件建模(底物识别+构象适应)整合酶序列、底物化学和活性位点结构信息，在多个动力学指标上超越传统浅层融合方法。


<details>
  <summary>Details</summary>
Motivation: 传统学习方法将酶动力学预测简化为酶与底物之间的静态兼容性预测，使用浅层操作融合表示并回归单一数值。这种方法忽略了催化的阶段性特征——包括底物识别和构象适应。现有方法未能充分捕捉酶序列、底物化学和活性位点构象适应之间的复杂交互。

Method: 提出酶-反应桥接适配器(ERBA)，通过微调将跨模态信息注入蛋白质语言模型(PLM)，同时保留其生物化学先验。ERBA采用两阶段条件建模：1) 分子识别交叉注意力(MRCA)将底物信息注入酶表示以捕捉特异性；2) 几何感知专家混合(G-MoE)整合活性位点结构并将样本路由到口袋专业化专家。酶-底物分布对齐(ESDA)在再生核希尔伯特空间中强制执行PLM流形内的分布一致性。

Result: 在三个动力学指标和多个PLM backbone上进行了实验，ERBA相比仅序列和浅层融合基线方法获得一致的性能提升，展现更强的分布外泛化能力。

Conclusion: ERBA提供了一种生物学上合理的可扩展动力学预测方法，为添加辅因子、突变和时间分辨结构线索奠定了基础。
...
TL;DR: 本文提出了一种基于视觉的分层分类框架，用于自动化磨轮磨损状态监测。与整体分类方法不同，该方法将问题分解为三个层次：状态检测、磨损类型识别和严重程度评估。使用EfficientNetV2迁移学习，在自定义数据集上达到93.8%-99.3%的分类准确率。


<details>
  <summary>Details</summary>
Motivation: 磨料磨轮因其灵活性常用于复杂自由曲面的精加工，但这种灵活性导致复杂的磨损模式（如凹/凸flap轮廓或flap撕裂），影响磨削结果。需要实现磨轮磨损状态的自动化监测。

Method: 构建了定制的真实磨轮图像数据集，采用EfficientNetV2架构的迁移学习方法。框架分为三个层次：（1）状态检测（新/磨损），（2）磨损类型识别（矩形、凹形、凸形）及flap tear检测，（3）严重程度评估（部分/完全变形）。使用Grad-CAM验证模型学习的特征。

Result: 分类准确率高且鲁棒性好，范围从93.8%（flap tears）到99.3%（concave severity）。Grad-CAM验证模型学习到了物理相关特征，并可用于检查错误分类。

Conclusion: 所提出的分层方法为自适应过程控制和自动化磨削中的磨损考虑奠定了基础。通过Grad-CAM验证了模型学习到了物理相关特征，实现了93.8%（ flap tears）到99.3%（concave severity）的高分类准确率。
...
TL;DR: CompoSIA是一个组合驾驶视频模拟器，通过解耦场景结构、物体身份和自我动作这三个交通因素，实现对对抗性驾驶场景的细粒度控制。该方法提出噪声级身份注入和分层双分支动作控制机制，在多项指标上优于现有最先进基线方法


<details>
  <summary>Details</summary>
Motivation: 自动驾驶面临"长尾"安全关键边缘场景的挑战，这些场景由不常见的交通元素组合产生。现有可控生成模型提供不完整或纠缠的引导，无法独立操控场景结构、物体身份和自我动作

Method: 1) 噪声级身份注入：实现姿态无关的身份生成，仅需单张参考图像即可替换场景元素；2) 分层双分支动作控制机制：提升动作可控性

Result: 身份编辑FVD提升17%，旋转和translation误差分别降低30%和47%。下游压力测试显示规划器失败显著增加：3秒内碰撞率平均增加173%

Conclusion: CompoSIA在多个指标上显著优于现有方法，能够生成更具威胁性的对抗性场景，导致下游规划器的碰撞率大幅增加（3秒内增加173%），证明了可控生成对自动驾驶安全测试的重要价值
...
TL;DR: TRACE是一种基于扩散模型的字符编码框架，利用字符结构稳定性实现抗噪声的数据嵌入，适用于文档安全领域。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖边缘特征或预定义码书，易受噪声干扰。本文利用字符结构固有的稳定性和跨字符统一表示特性，提供更强的抗噪声干扰能力。

Method: 框架包含三个关键组件：1) 自适应扩散初始化 - 通过移动概率估计器(MPE)、目标点估计(TPE)和掩码绘制模型(MDM)自动识别控制点、目标点和编辑区域；2) 引导扩散编码 - 精确移动选定点；3) 掩码区域替换 - 使用专门损失函数最小化扩散后的特征改变。

Result: 实验表明TRACE在PSNR上提升超过5dB，跨媒体传输后提取准确率提高5%，支持多语言和多字体泛化。

Conclusion: TRACE在文档安全应用中具有优越性能，尤其适合需要高抗噪声能力的实际场景。
...
TL;DR: 提出了视频-based癫痫发作预测的新任务，使用3-10秒的发作前期视频片段预测未来5秒内是否会发生癫痫发作。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖脑电图等神经信号，需要专业设备且难以实现长期部署；视频数据提供了一种非侵入性且易于获取的替代方案，但现有研究主要集中于发作后检测，发作前预测领域仍属空白。

Method: 提出了一个跨物种迁移学习框架，利用大规模啮齿类动物视频数据进行辅助预训练，使模型能够捕获跨物种的癫痫相关行为动态。

Result: 在严格的纯视频设置下，预测准确率超过70%，优于现有基线方法。

Conclusion: 研究证明了跨物种学习在构建非侵入性、可扩展的癫痫早期预警系统方面的潜力，为癫痫看护提供了新的解决方案。
...
TL;DR: SG-NLF是一种无姿态LiDAR NeRF框架，通过光谱先验混合表示、置信度图全局对齐和对抗学习解决LiDAR数据稀疏性和姿态依赖问题，实现高质量场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR NeRF方法高度依赖精确相机姿态，且LiDAR数据的稀疏性和无纹理特性导致几何空洞和表面不连续问题，亟需一种无需精确姿态即可实现高质量重建的框架。

Method: 提出SG-NLF框架，包含三个核心组件：1)基于光谱先验的混合表示重建平滑几何；2)基于特征兼容性的置信度图实现全局姿态对齐；3)对抗学习策略增强跨帧一致性。

Result: 在具有挑战性的低频场景中表现优异，重建质量提升35.8%以上，姿态精度提升68.8%以上，超越了先前最先进方法。

Conclusion: SG-NLF在LiDAR视图合成方面提供了新视角，通过结合光谱先验、图结构全局对齐和对抗学习，有效解决了无姿态LiDAR NeRF的挑战。该方法在重建质量和姿态精度上分别提升超过35.8%和68.8%，证明了光谱信息与几何一致性结合的有效性。
...
TL;DR: 本文提出VIRD方法，通过极坐标变换和上下文增强的位置注意力弥合地面与卫星视图的视角差距，实现跨视图相机姿态估计，在KITTI和VIGOR数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: GNSS-based定位常因遮挡和多径效应而退化，现有的跨视图姿态估计方法由于地面与卫星视图之间视角差异大且空间对应有限，难以准确预测相机6-DoF姿态。

Method: VIRD采用双轴变换构建视角不变表示：首先对卫星视图进行极坐标变换以建立水平对应；然后对地面视图和极坐标变换后的卫星特征使用上下文增强的位置注意力来解决垂直错位问题；引入视角重建损失来增强视角不变性，鼓励派生表示重建原始视图和跨视图图像。

Result: 在KITTI和VIGOR数据集上的实验表明，VIRD在无需方向先验的情况下优于最先进的方法：KITTI上中位位置误差减少50.7%，方向误差减少76.5%；VIGOR上分别减少18.0%和46.8%。

Conclusion: VIRD通过双轴变换和视角重建损失有效弥合了地面与卫星视图之间的视角差距，在没有方向先验的情况下，在KITTI数据集上实现了50.7%的位置误差减少和76.5%的方向误差减少，在VIGOR数据集上分别减少18.0%和46.8%，验证了其在自动驾驶和机器人定位中的有效性和优越性。
...
TL;DR: 本文研究如何充分利用视觉语言模型(VLMs)辅助图像伪造检测和定位(IFDL)任务，发现VLMs先验因偏向语义合理性而非真实性而难以直接应用，提出利用位置掩码作为额外先验的IFDL-VLM方法，在9个基准数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能生成内容(AIGC)的快速发展，图像操纵变得日益便捷，对图像伪造检测和定位(IFDL)提出了重大挑战。研究发现视觉语言模型(VLMs)的先验由于固有地偏向语义合理性而非真实性，几乎无法惠及检测和定位性能，甚至产生负面影响。

Method: 提出了一种新的IFDL-VLM流水线，利用位置掩码编码的伪造概念作为视觉语言模型的额外先验，帮助VLMs进行训练优化，从而增强检测和定位结果的可解释性。

Result: 在9个流行基准数据集上进行了广泛实验，评估了模型在域内和跨数据集泛化设置下的性能。实验结果表明，该方法在检测、定位和可解释性方面均一致取得了新的最优性能。

Conclusion: 提出的IFDL-VLM方法通过利用位置掩码作为视觉语言模型的额外先验，有效解决了传统方法中VLMs先验对IFDL任务贡献有限甚至产生负面影响的问题，在9个基准数据集上实现了检测、定位和可解释性的新最优性能。
...
TL;DR: SGMatch是一个基于学习的非刚性形状匹配框架，通过语义引导的局部交叉注意力模块将视觉基础模型的语义特征整合到几何描述符中，并引入基于条件流匹配的监督目标来增强空间平滑性，在非等距变形和拓扑噪声情况下实现一致的改进。


<details>
  <summary>Details</summary>
Motivation: 建立非刚性3D形状之间的精确点对点对应关系仍然是一个关键挑战，特别是在非等距变形和拓扑噪声情况下。现有的功能映射管道存在模糊性，空间投影也存在不一致问题。

Method: 设计了语义引导的局部交叉注意力模块(Semantic-Guided Local Cross-Attention)，将视觉基础模型的语义特征与几何描述符集成，同时保留局部结构连续性；引入基于条件流匹配的正则化目标，监督时变速度场以鼓励恢复对应关系的空间平滑性。

Result: 在多个基准数据集上进行了实验，SGMatch在近等距设置下实现了竞争性能，并在非等距变形和拓扑噪声下获得一致的改进。

Conclusion: SGMatch成功地将语义信息与几何特征相结合，通过流匹配正则化提升了匹配质量，在处理复杂变形和噪声情况时表现出更强的鲁棒性。
...
TL;DR: 提出CleanSight，一种针对大型视觉语言模型后门攻击的免训练、仅在测试时工作的防御方法。该方法通过检测跨模态融合层中的异常注意力重分配（"注意力窃取"现象），并选择性剪枝高注意力视觉tokens来净化中毒输入。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法依赖用干净数据重新训练后门参数（如adapters或LoRA模块），计算成本高且会损害模型性能。需要新的机制理解来开发更高效的防御手段。

Method: 1. 发现关键机制：触发器通过异常跨模态注意力重分配起作用（注意力窃取），而非低层视觉模式
2. 提出CleanSight：基于选定跨模态融合层中的相对视觉-文本注意力比率检测中毒输入，通过选择性剪枝可疑高注意力视觉 tokens 来净化输入

Result: CleanSight在多种数据集和后门攻击类型上显著优于现有的基于像素的净化防御方法，同时保留模型在干净和中毒样本上的实用性

Conclusion: 通过理解和利用注意力窃取机制，可以设计出免训练的即插即用防御方法，有效检测和净化后门攻击，同时保持模型效用
...
TL;DR: 本文提出了一种无需训练、无需标注数据的闭式去偏方法，在跨模态空间中实现帕累托最优公平性且有界utility损失，可同时对视觉和文本模态进行去偏，并在零样本图像分类、文本到图像检索和文本到图像生成等下游任务中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLM)会从训练数据中继承社会偏见，并将这些偏见传播到下游应用中，而现有去偏方法缺乏保留模型效用的理论保证。

Method: 提出一种闭式去偏解决方案，在跨模态空间中实现帕累托最优公平性，具有有界utility损失。该方法无需训练、不需要标注数据，可联合对视觉和文本模态进行去偏。

Result: 在多种公平性指标和数据集上，该方法在群体公平性和交叉公平性方面均优于现有方法，同时保留了任务性能，成功应用于零样本图像分类、文本-图像检索和文本-图像生成等下游任务。

Conclusion: 该方法能有效对VLM进行去偏，且具有理论保证，在多个下游任务和模态上实现了优越的公平性-效用权衡。
...
TL;DR: 提出 Multimodal OCR (MOCR)，将文本和图形统一解析为文本表示，将图表、表格、图标等视觉元素作为一等解析目标，实现文档的结构化重建，支持端到端训练，并将图形转换为可重用的代码级监督。


<details>
  <summary>Details</summary>
Motivation: 传统OCR系统专注于文本识别并将图形区域留作裁剪的像素，需要一种将图表、图表、表格和图标等视觉元素作为一等解析目标的方法，以在解析文档时保留元素间的语义关系，并将先前丢弃的图形转换为可重用的代码级监督。

Method: 构建了来自PDF、渲染网页和原生SVG资产的综合数据引擎，通过分阶段预训练和监督微调训练了紧凑的30亿参数模型。将文本和图形重建为结构化输出，支持对异构文档元素的端到端训练，使模型能够利用文本和视觉组件之间的语义关系。

Result: 在文档解析基准测试中，在OCR Arena Elo排行榜上仅次于Gemini 3 Pro，超越现有开源文档解析系统，在olmOCR Bench上创下83.9的新纪录。在结构化图形解析方面，在图像到SVG基准测试中比Gemini 3 Pro实现更高重建质量，在图表、UI布局、科学图形和化学图谱上表现出色。

Conclusion: 该研究展示了一条可扩展的构建大规模图像到代码语料库用于多模态预训练的路径。代码和模型已在GitHub上公开可用。
...
### [77] [ESPIRE: A Diagnostic Benchmark for Embodied Spatial Reasoning of Vision-Language Models](https://arxiv.org/abs/2603.13033)
*Yanpeng Zhao,Wentao Ding,Hongtao Li,Baoxiong Jia,Zilong Zheng*

Main category: cs.CV

TL;DR: ESPIRE是一个用于具身空间推理的诊断基准，通过模拟物理世界将VLMs具象化评估，创新地将机器人任务分解为定位和执行两个生成性问题，突破了传统判别性评估（VQA）的局限，提供细粒度的空间推理分析。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽在空间认知方面取得进展，但评估范式和覆盖范围有限，阻碍了模型的快速迭代开发。现有评估主要依赖判别性方法（如视觉问答），需要干扰项且忽略执行环节，与真实世界部署存在差距。

Method: 提出ESPIRE基准测试，包含一个模拟世界将VLMs物理具象化。创新性地将任务分解为定位和执行两个阶段，并将其框架为生成性问题（而非传统的判别性VQA评估）。在指令级和环境级进行系统设计，确保空间推理场景的广泛覆盖。

Result: 论文对多种前沿VLMs进行了系统诊断，提供了对其空间推理行为的深入分析，展示了ESPIRE基准在评估模型空间推理能力方面的有效性。

Conclusion: ESPIRE enables fine-grained analysis beyond passive spatial reasoning toward reasoning to act, providing broad coverage of spatial reasoning scenarios for diagnosing VLMs' capabilities in embodied domains.
...
TL;DR: 本文提出VTON-IQA，一个无需参考图像的虚拟试穿质量评估框架，用于评估虚拟试穿图像的质量。


<details>
  <summary>Details</summary>
Motivation: 现有VTON系统在真实场景中缺乏真实参考图像，导致基于参考的评估不可行；而FID/KID等分布级指标无法反映个体生成图像的感知质量。

Method: 构建了大规模人类标注基准VTON-QBench（包含14个VTON模型生成的62,688张试穿图像和13,838名标注者的431,800个质量标注）；提出交错交叉注意力模块，在Transformer块的自注意力和MLP之间插入交叉注意力层。

Result: VTON-IQA实现了可靠的人类对齐图像级质量预测，并对14个代表性VTON模型进行了综合基准评估。

Conclusion: 该方法无需真实参考图像即可进行符合人类感知的图像级质量评估，为虚拟试穿系统的实际应用提供了有效的评估工具。
...
TL;DR: 针对文本到图像扩散模型的版权和隐私风险，提出两种互补方法：RAPTA通过区域感知提示增强减少过拟合，ADMCD通过多模态特征融合检测复制，均取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 最先进的文本到图像扩散模型可能记忆并复现训练图像，造成版权和隐私风险；现有推理时的提示扰动方法虽能降低复制，但往往损害图像-提示对齐和整体质量。

Method: RAPTA使用目标检测器定位显著区域，生成语义锚定的提示变体并在训练中随机采样以增加多样性；ADMCD融合局部patch、全局语义和纹理线索，用轻量级Transformer生成融合表示，采用简单阈值决策规则检测复制，无需大规模标注数据集训练。

Result: 实验表明RAPTA在保持高合成质量的同时减少过拟合，ADMCD可靠检测复制且优于单模态指标。

Conclusion: RAPTA通过区域感知的提示增强有效减少过拟合，同时保持高合成质量；ADMCD通过多模态特征融合可靠检测复制，优于单模态指标。两种方法互补，共同解决扩散模型的版权和隐私风险。
...
TL;DR: 本研究开发了一个从观测学习的框架，利用风洞实验PIV数据，从稀疏传感器重建屋顶风场。对比了Kriging插值与UNet、ViTAE、CWGAN三种深度学习模型，并研究了训练策略、传感器密度和优化方法对重建效果的影响。


<details>
  <summary>Details</summary>
Motivation: 屋顶风场对无人机、城市空中交通系统、风控制系统和安全屋顶利用至关重要。然而，屋顶流动表现出强非线性、分离和跨方向变异性，使得从稀疏传感器重建流场变得困难。

Method: 开发了从观测学习的框架，使用风洞实验的粒子图像测速(PIV)数据。对比了Kriging插值与三种深度学习模型(UNet、ViTAE、CWGAN)。评估了两种训练策略(单风向训练SDT和混合风向训练MDT)，传感器密度从5到30，并在±1网格的传感器位置扰动下测试鲁棒性，使用QR分解的POD进行传感器布局优化。

Result: 深度学习方法可有效从稀疏传感器重建屋顶风场。与Kriging相比，深度学习模型SSIM提升最高32.7%，FAC2提升24.2%，NMSE提升27.8%。混合风向训练相比单风向训练，SSIM提升173.7%，FAC2提升16.7%，MG提升98.3%。QR优化在传感器扰动下鲁棒性提升27.8%，但存在指标权衡。

Conclusion: 传感器配置、优化和训练策略应联合考虑以实现可靠部署。QR优化提升了鲁棒性，但存在指标依赖的权衡。基于实验数据的训练为不同场景下的方法选择和传感器布局提供了实践指导。
...
TL;DR: 本文提出InterEdit3D数据集和TMME基准，用于研究文本引导的多人3D运动编辑任务。提出InterEdit模型，采用同步无分类器条件扩散架构，通过语义感知计划令牌对齐和交互感知频率令牌对齐技术，有效捕捉高层交互线索和周期性运动动态，在多人运动编辑任务中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导3D运动编辑在单人场景中取得成功，但扩展到多人场景面临挑战，原因包括：1)配对数据有限；2)人物间交互的复杂性。现有方法缺乏专门针对多人运动编辑的研究和基准。

Method: 提出InterEdit，一个同步无分类器条件扩散模型，包含两个核心模块：1)语义感知计划令牌对齐(Semantic-Aware Plan Token Alignment)，使用可学习令牌捕捉高层交互线索；2)交互感知频率令牌对齐(Interaction-Aware Frequency Token Alignment)，利用DCT和能量池化建模周期性运动动态。

Result: 实验表明，InterEdit在文本到运动一致性和编辑保真度方面都有显著提升，在TMME基准上实现了最先进的性能。

Conclusion: InterEdit在TMME任务上实现了最先进的性能，提高了文本到运动的一致性和编辑保真度。该方法通过语义感知计划令牌对齐和交互感知频率令牌对齐两个创新模块，有效捕捉了高层交互线索和周期性运动动态。
...
TL;DR: V-Bridge是一个将大规模视频生成模型应用于少样本图像恢复的框架，通过将图像恢复重新解释为渐进生成过程，仅用1000个训练样本（<2%现有方法）即可实现多任务图像恢复，性能匹敌专门设计的架构。


<details>
  <summary>Details</summary>
Motivation: 大规模视频生成模型在多样化的视觉数据上训练，内化了丰富的视觉世界结构、语义和动态先验。然而，这些模型作为通用视觉学习器的潜力尚未被充分挖掘。传统图像恢复方法需要大量任务特定数据，且为每个任务设计专门架构。

Method: 将图像恢复重新解释为渐进生成过程，利用视频模型模拟从退化输入到高保真输出的逐步细化过程。通过极少量数据（1000个多任务训练样本）即可激活视频生成模型中隐含的强可迁移恢复先验。

Result: 仅使用1000个多任务训练样本（少于现有恢复方法的2%），预训练视频模型即可实现竞争性的图像恢复性能，能够用单一模型完成多个任务，性能可与专门为此目的设计的专门架构相媲美。

Conclusion: V-Bridge框架挑战了生成建模与低层视觉之间的传统边界，展示了视频生成模型可以作为视觉任务的基础模型，为视觉任务中的基础模型设计开辟了新的范式。仅需1000个多任务训练样本（少于现有方法的2%），预训练视频模型即可实现与专门设计的架构相竞争的图像恢复性能。
...
TL;DR: 该研究针对具身智能体的抽象时空推理能力评估提出了VAEX-BENCH基准，通过构建合成自我中心视频数据集和结构化评估框架，揭示了现有MLLMs在抽象推理任务上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要强调提取性推理，而多模态大语言模型能否执行抽象性时空推理——即整合时间观察、组合分散线索、推断隐含空间和上下文结构——仍不清楚。

Method: 构建了结构化评估分类法，设计了可控的、场景驱动的合成自我中心视频数据集，包含物体、房间和楼层平面三个层级，并设置五对抽象与提取推理任务进行对比实验。

Result: 实验暴露了当前最先进的MLLMs在抽象推理任务上的局限性，并提供了细粒度的瓶颈分析。

Conclusion: 该研究提出了VAEX-BENCH基准，用于评估多模态大语言模型在视频抽象时空推理方面的能力，并揭示了当前模型在此类任务上的局限性。
...
TL;DR: NOIR是一个将医学影像任务重新定义为连续函数空间算子学习的框架，通过隐式神经表示和神经算子实现分辨率无关的转换，在多个任务和数据集上展现出竞争力的性能和强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 本文旨在挑战当前医学影像领域基于离散网格的深度学习范式，提出将核心医学影像任务重新定义为连续函数空间之间的算子学习问题。现有方法受限于固定像素或体素网格，而作者认为应该探索分辨率无关的函数到函数转换方法。

Method: NOIR框架将离散医学信号嵌入到共享的隐式神经表示(Implicit Neural Representations)中，学习神经算子(Neural Operator)来映射潜在调制，实现分辨率无关的函数到函数转换。在多个2D和3D下游任务上进行评估，包括分割、形状补全、图像到图像翻译和图像合成。

Result: 在Shenzhen、OASIS-4、SkullBreak、fastMRI等多个公开数据集以及内部临床数据集上，NOIR在原生分辨率下达到具有竞争力的性能，表现出对未见离散化的强鲁棒性，并实证验证了神经算子的关键理论性质。

Conclusion: NOIR成功地将医学影像任务重新定义为连续函数空间中的算子学习问题，为医学影像分析提供了一种新的分辨率无关的深度学习范式，具有良好的泛化能力和理论保证。
...
TL;DR: FDeID-Toolbox是一个综合性人脸去识别工具箱，通过模块化架构实现标准化数据加载、统一方法实现、灵活推理和系统性评估，促进FDeID方法的公平可比研究。


<details>
  <summary>Details</summary>
Motivation: 人脸去识别（FDeID）领域存在碎片化的实现、不一致的评估协议以及研究结果之间无法比较的问题。这些挑战源于任务本身的复杂性：FDeID涉及多个下游应用（如年龄估计、性别识别、表情分析），需要从三个维度（隐私保护、效用保留和视觉质量）进行评估，导致现有代码库难以使用和扩展。

Method: 提出FDeID-Toolbox，一个用于可重现FDeID研究的综合工具箱，采用模块化架构，包含四个核心组件：(1)主流基准数据集的标准化数据加载器，(2)从经典方法到最先进生成模型的统一方法实现，(3)灵活的推理管道，(4)涵盖隐私、效用和质量指标的系统性评估协议。

Result: 通过实验证明，FDeID-Toolbox能够在一致条件下对不同的FDeID方法进行公平和可重现的比较。

Conclusion: FDeID-Toolbox解决了FDeID研究中可重现性和可比性问题，为隐私保护计算机视觉研究提供了标准化工具。
...
TL;DR: 针对HRC流式感知中的延迟问题，提出利用前一帧输出进行实时感知模块调度的轻量级框架，在降低27.52%延迟的同时保持高精度


<details>
  <summary>Details</summary>
Motivation: 在现代人-机器人协作(HRC)应用中，尽管逐帧执行多感知模块可提升离线感知质量，但在流式感知场景中会累积延迟导致系统性能大幅下降；现有感知管道面临信息冗余和计算资源分配不当的挑战

Method: 提出了一种轻量级感知调度框架，利用前一帧的输出根据场景上下文实时估计和调度必要的感知模块

Result: 实验表明该框架将计算延迟降低最多27.72%，MMPose激活召回率提升72.73%，关键帧准确率达到98%

Conclusion: 所提出的感知调度框架能够有效提升实时感知效率，同时不显著牺牲精度，为HRC中的多模态流式感知系统提供了一种可扩展且系统的解决方案
...
TL;DR: 提出ActionGenome4D数据集和世界场景图生成(WSGG)任务，通过三种方法（PWG、MWAE、4DST）实现对场景中观察和未观察对象的推理，推进视频场景理解向世界中心化、时序持久化方向发展。


<details>
  <summary>Details</summary>
Motivation: 现有时空场景图方法从根本上仍然是帧中心的：仅推理当前可见对象、对象遮挡时丢弃实体且仅在2D空间操作。需要解决对象持久性问题和未观察对象的推理问题。

Method: 提出三种互补方法探索未观察对象推理的不同归纳偏置：1) PWG通过零阶特征缓冲区实现对象持久性；2) MWAE将未观察对象推理重新定义为掩码完成与跨视图关联检索；3) 4DST用可微分的每对象时间注意力替代静态缓冲区，并丰富3D运动和相机姿态特征。此外还设计评估了基于Graph RAG的强开源视觉语言模型在WSGG任务上的表现。

Result: 构建了ActionGenome4D数据集（将Action Genome视频升级为4D场景，包含世界坐标系的边界框和密集关系注释），并为未定位关系预测建立了基线。

Conclusion: WSGG任务推进视频场景理解向世界中心化、时间持久化和可解释的场景推理方向发展，为未定位关系预测建立了基线。
...
TL;DR: STEVO-Bench是一个评估视频世界模型能否将状态演化与观察解耦的基准测试


<details>
  <summary>Details</summary>
Motivation: 探索视频世界模型生成的"世界"是否能在不被观察时自主演化，即模型能否将状态演化与观察过程解耦

Method: 通过遮挡物插入、关闭光源、相机"视线转移"轨迹等观察控制手段，对自然演化过程进行评估，自动检测和 disentangle 视频世界模型的失败模式

Result: 暴露了当前视频世界模型在将状态演化与观察解耦方面的局限性，揭示了现有模型的数据和架构偏差

Conclusion: 为理解和改进视频世界模型提供了新的视角，强调模型需要具备独立于观察的状态演化建模能力
...
### [89] [DIALECTIC: A Multi-Agent System for Startup Evaluation](https://arxiv.org/abs/2603.12274)
*Jae Yoon Bae,Simon Malberg,Joyce Galang,Andre Retterath,Georg Groh*

Main category: cs.MA

TL;DR: 提出了DIALECTIC，一个基于LLM的多智能体创业评估系统，通过事实收集、层级问题树构建、论证合成和模拟辩论迭代来评估投资机会，产生数值决策分数帮助VC高效筛选项目。


<details>
  <summary>Details</summary>
Motivation: 风险投资家面临大量投资机会但只能投资少数，成功者更少。早期筛选受限于投资者带宽，需要在评估详尽程度和评估机会数量之间进行艰难权衡。

Method: DIALECTIC系统包含三个主要阶段：1）收集创业公司的 factual knowledge 并组织成层级问题树；2）将事实合成为支持和反对投资的自然语言论证；3）通过模拟辩论迭代批评和精炼论证，最终产生数值决策分数。

Result: 通过使用五个VC基金的真实投资机会进行回测，DIALECTIC在预测创业公司成功方面达到了与人类VC相当的精确度。

Conclusion: DIALECTIC能够有效帮助VC在早期筛选阶段平衡评估详尽程度和机会数量，通过模拟辩论产生最有力的投资论证，并生成可量化的决策分数。
...
### [90] [Collaborative Multi-Agent Optimization for Personalized Memory System](https://arxiv.org/abs/2603.12631)
*Wenyu Mao,Haoyang Liu,Zhao Liu,Haosong Tan,Yaorui Shi,Jiancan Wu,An Zhang,Xiang Wang*

Main category: cs.MA

TL;DR: 针对个性化LLM记忆系统的多智能体协作优化问题，提出CoMAM框架，通过将智能体执行建模为顺序MDP并利用群体级排名一致性量化贡献，实现局部与全局奖励的整合


<details>
  <summary>Details</summary>
Motivation: 现有方法专注于通过提示工程或微调独立优化各个智能体，但忽略了跨智能体协作，导致独立优化无法保证全局系统性能

Method: 提出CoMAM框架，将智能体执行正则化为顺序马尔可夫决策过程以嵌入智能体间依赖关系，通过群体级排名一致性量化每个智能体的贡献，将其作为自适应权重分配全局信用并整合局部-全局奖励

Result: 实验表明CoMAM优于领先的记忆系统，验证了所提出的协作强化学习联合优化的有效性

Conclusion: CoMAM通过协作强化学习框架成功实现了多智能体记忆系统的联合优化，实验结果验证了该方法在提升系统整体性能方面的有效性
...
TL;DR: ReBalance是一个无需训练的框架，通过置信度驱动的动态控制机制平衡大型推理模型的思考程度，在减少冗余输出的同时提升准确率，适用于多种推理任务


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在过思考（在简单问题上消耗冗余计算步骤）和欠思考（未能充分探索推理路径）问题，导致效率低下和潜在的不准确，在资源受限环境中实际部署受限，现有缓解过思考的方法可能无意中导致欠思考

Method: 提出ReBalance框架，利用置信度作为推理动态的连续指标，通过小规模数据集聚合隐藏状态构建推理模式原型并计算 steering vector，设计动态控制函数根据实时置信度调节该向量的强度和方向，在过思考时剪枝冗余，在欠思考时促进探索

Result: 在4个模型（0.5B到32B）和9个基准测试（数学推理、常识问答、编码任务）上证明ReBalance有效减少输出冗余同时提升准确性

Conclusion: ReBalance是一个无需训练的通用、可插拔框架，通过基于置信度的动态控制机制，有效平衡LRMs的推理过程，显著降低输出冗余同时提升准确性，在9个基准测试上验证了有效性
...
### [93] [AI Planning Framework for LLM-Based Web Agents](https://arxiv.org/abs/2603.12710)
*Orit Shahnovsky,Rotem Dror*

Main category: cs.AI

TL;DR: 本文提出将Web任务形式化为序列决策问题，通过将现代Agent架构映射到传统规划范式来诊断LLM Agent失败原因，并引入5个新的评估指标和794条人工标注的WebArena轨迹数据集。


<details>
  <summary>Details</summary>
Motivation: LLM Agent虽能解析复杂用户请求，但作为黑盒运作，难以诊断失败原因或理解其规划过程。

Method: 将Web任务形式化为序列决策过程；建立 taxonomy 将Step-by-Step、Tree Search、Full-Plan-in-Advance等Agent架构分别映射到BFS、Best-First Tree Search、DFS等经典规划范式；提出5个评估轨迹质量的新指标；构建794条WebArena人工标注轨迹数据集；对比基线Step-by-Step Agent与新型Full-Plan-in-Advance实现。

Result: Step-by-Step Agent与人类金标准轨迹对齐度更高（38%成功率），但Full-Plan-in-Advance Agent在技术指标上更优（元素准确率89%）。

Conclusion: 不同Agent架构适合不同应用场景，所提评估指标对根据具体应用约束选择合适的Agent架构至关重要。
...
### [94] [On Using Machine Learning to Early Detect Catastrophic Failures in Marine Diesel Engines](https://arxiv.org/abs/2603.12733)
*Francesco Maione,Paolo Lino,Giuseppe Giannino,Guido Maione*

Main category: cs.AI

TL;DR: 提出一种基于偏差导数和随机森林的海洋发动机灾难性故障早期检测方法，通过分析传感器读数与预期值的偏差导数，在故障达到临界阈值前预警


<details>
  <summary>Details</summary>
Motivation: 海洋发动机灾难性故障具有突发性、不可预测性，可能导致严重的人员伤亡和财产损失。现有研究主要关注组件的渐进退化，对突发异常现象关注不足，因此需要一种能够早期检测灾难性故障的方法

Method: 基于实际故障发动机数据，提出两种创新方法：1)使用偏差导数而非传统偏差来捕捉异常动态；2)采用随机森林作为机器学习预测算法，并通过深度学习数据增强方法解决训练数据获取问题

Result: 实验结果表明，该方法能够有效预测灾难性故障的发生，在真实数据上验证了方法的鲁棒性和实用性，可在测量值达到临界阈值和触发传统报警之前检测到异常

Conclusion: 基于偏差导数和随机森林的方法能够实现灾难性故障的早期检测，为操作员提供足够的预警时间以关闭发动机或安全改变航线，从而避免潜在损害
...
### [95] [ToolTree: Efficient LLM Agent Tool Planning via Dual-Feedback Monte Carlo Tree Search and Bidirectional Pruning](https://arxiv.org/abs/2603.12740)
*Shuo Yang,Soyeon Caren Han,Yihao Ding,Shuhe Wang,Eduard Hoy*

Main category: cs.AI

TL;DR: ToolTree是一种基于蒙特卡洛树搜索的LLM智能体工具规划范式，通过双向剪枝机制和双阶段LLM评估来优化多步工具使用序列，解决了现有方法缺乏前瞻性和忽视工具间依赖性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体工具规划方法通常依赖贪婪的、反应式的工具选择策略，缺乏前瞻性且无法处理工具间的依赖关系，导致在复杂多步任务中表现不佳。

Method: 提出ToolTree框架，灵感来自蒙特卡洛树搜索，采用双阶段LLM评估机制和双向剪枝策略，在工具执行前后对不太有希望的分支进行剪枝，使智能体能够在扩展的工具使用序列中做出明智的自适应决策。

Result: 在4个基准数据集上针对开放集和封闭集工具规划任务进行实证评估，ToolTree在保持最高效率的同时持续提升性能，平均性能提升约10%，优于现有最先进规划范式。

Conclusion: ToolTree通过引入前瞻性规划和考虑工具依赖关系，显著提升了LLM智能体在复杂多步工具使用任务中的表现，为工具规划领域提供了一种更高效、更智能的解决方案。
...
### [96] [AI Model Modulation with Logits Redistribution](https://arxiv.org/abs/2603.12755)
*Zihan Wang,Zhongkui Ma,Xinguo Feng,Zhiyang Mei,Ethan Ma,Derui Wang,Minhui Xue,Guangdong Bai*

Main category: cs.AI

TL;DR: 本文提出AIM（Additive Intervention Modulation）模型调制范式，允许单一模型通过logits重分配策略实现多种行为，无需重新训练或访问训练数据。支持效用调制和焦点调制两种模式，分别控制输出质量和输入特征聚焦。已在图像分类、语义分割和文本生成任务上验证有效性。


<details>
  <summary>Details</summary>
Motivation: 大规模模型需要适应不同用户需求，但维护多个专业版本效率低下。需要一种无需重新训练的动态调制方法，使单一模型能够满足不同终端需求。

Method: 提出logits重分配策略，基于logits排序的统计特性（联合概率分布）建立形式化理论基础，实现训练数据无关且无需重新训练的模型调制。包含两种调制模式：效用调制（动态控制输出质量）和焦点调制（精确控制输入特征聚焦）。

Result: 在图像分类、语义分割和文本生成任务上验证了方法的有效性，涵盖ResNet、SegFormer和Llama等主流架构。

Conclusion: AIM为AI模型调制提供了实用且通用的解决方案，能够在不修改模型参数的情况下动态调整模型行为，具有广泛的应用前景。
...
### [97] [Context is all you need: Towards autonomous model-based process design using agentic AI in flowsheet simulations](https://arxiv.org/abs/2603.12813)
*Pascal Schäfer,Lukas J. Krinke,Martin Wlotzka,Norbert Asprion*

Main category: cs.AI

TL;DR: 本文提出一种将Agentic AI集成到化学过程流程建模的框架，利用LLM生成Chemasim代码，并通过多智能体系统分解过程开发任务，展示了在反应/分离、变压精馏和非均相共沸精馏等案例中的有效性。


<details>
  <summary>Details</summary>
Motivation: 将Agentic AI系统（集成LLM与推理和工具使用能力）应用于化学过程流程建模的探索不足，希望填补这一空白

Method: 开发了一个多智能体系统，使用GitHub Copilot和Claude Opus 4.6 LLM，一个智能体负责使用工程知识解决抽象问题，另一个智能体将解决方案实现为Chemasim代码

Result: 成功演示了三个典型流程建模案例：(i)反应/分离过程，(ii)变压精馏，(iii)含共沸剂选择的非均相共沸精馏

Conclusion: 所提框架在工业流程模拟环境中提供有效帮助，但目前仍存在局限性，需要进一步研究以增强其能力
...
### [98] [ODRL Policy Comparison Through Normalisation](https://arxiv.org/abs/2603.12926)
*Jaime Osvaldo Salas,Paolo Pareti,George Konstantinidis*

Main category: cs.AI

TL;DR: 本文提出了一种ODRL政策的参数化规范化方法，将包含权限和禁止的政策转换为仅包含权限的政策，并将复杂逻辑约束简化为简单约束，以解决ODRL语言的复杂性和政策比较困难的问题。


<details>
  <summary>Details</summary>
Motivation: ODRL语言虽已成为数字权限策略的标准，但其复杂性阻碍了其应用，导致许多相关研究聚焦于互不兼容的ODRL子集。此外，语义等价的政策可以多种方式表达，增加了比较和处理的难度。

Method: 提出一种参数化规范化方法，将政策规范化为最小组成部分，包括：将权限和禁止重构为仅含权限的形式，将复杂逻辑约束简化为简单约束。提供了计算ODRL政策范式的算法，并证明了这些算法保持政策的语义。

Result: 算法复杂度在属性数量上呈指数级，在属性唯一值数量上呈线性级。结果表明该方法使复杂政策可在更基础的ODRL子集中表示，并将政策比较问题简化为检查两条规则是否相同。

Conclusion: 通过规范化方法降低了ODRL的复杂性，使政策比较和处理变得更加简单，为ODRL的更广泛应用奠定了基础。
...
### [99] [Efficient and Interpretable Multi-Agent LLM Routing via Ant Colony Optimization](https://arxiv.org/abs/2603.12933)
...
TL;DR: AMRO-S是一个高效且可解释的多智能体系统路由框架，通过小语言模型意图推断、任务特定信息素专家和质量门控异步更新三大机制，在复杂推理和工具使用场景中优化质量-成本权衡。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的多智能体系统在实际部署中面临高推理成本、高延迟和有限透明度的问题；现有路由策略依赖昂贵的LLM选择器或静态策略，在动态负载和混合意图下缺乏可控性，导致性能不稳定和资源利用低效。

Method: 提出三大核心机制：(1)利用监督微调的小语言模型进行意图推断，提供低开销的语义接口；(2)将路由内存分解为任务特定的信息素专家，减少跨任务干扰并优化混合工作负载下的路径选择；(3)采用质量门控的异步更新机制，将推理与学习解耦，在不增加延迟的情况下优化路由。

Result: 在五个公共基准测试和高并发压力测试中，AMRO-S相比强基线路由方法持续改善了质量-成本权衡，并通过结构化信息素模式提供了可追踪的路由证据。

Conclusion: AMRO-S通过语义条件路径选择方法，为多智能体系统提供了一种高效、可解释且可控制的路由解决方案，在保持低延迟的同时提升了路由性能和资源利用效率。
...
### [100] [Beyond Final Answers: CRYSTAL Benchmark for Transparent Multimodal Reasoning Evaluation](https://arxiv.org/abs/2603.13099)
...
### [101] [Steve-Evolving: Open-World Embodied Self-Evolution via Fine-Grained Diagnosis and Dual-Track Knowledge Distillation](https://arxiv.org/abs/2603.13131)
*Zhengwei Xie,Zhisheng Chen,Ziyan Weng,Tingyu Wu,Chenglong Li,Vireo Zhang,Kun Wang*

Main category: cs.AI

TL;DR: 提出Steve-Evolving框架，通过体验锚定、体验蒸馏和知识驱动的闭环控制三阶段，实现嵌入式智能体在长程任务中的自我进化，无需模型参数更新即可实现持续演化。


<details>
  <summary>Details</summary>
Motivation: 开放世界嵌入式智能体解决长程任务的主要瓶颈不在于单步规划质量，而在于如何组织和发展交互经验。现有方法缺乏将执行诊断与知识提炼紧密结合的闭环机制。

Method: 提出三阶段框架：1)体验锚定将子目标尝试固化为结构化经验元组，组织在三维体验空间中；2)体验蒸馏将成功轨迹泛化为可复用技能，将失败蒸馏为可执行护栏；3)知识驱动闭环控制将技能和护栏注入LLM规划器，实现诊断触发的在线局部重规划。

Result: 在Minecraft MCU长程任务套件上相比静态检索基线取得一致改进。

Conclusion: 该非参数自我进化框架通过细粒度执行诊断与双轨知识蒸馏的紧密耦合，能有效组织和发展交互经验，为开放世界嵌入式智能体的持续学习提供了新范式。
...
### [102] [When Right Meets Wrong: Bilateral Context Conditioning with Reward-Confidence Correction for GRPO](https://arxiv.org/abs/2603.13134)
*Yu Li,Tian Lan,Zhengling Qi*

Main category: cs.AI

TL;DR: 本文提出BICC和RCC两种机制来改进GRPO，通过引入对比学习和动态基线校正来增强推理模型训练，无需额外采样或辅助模型，在数学推理任务上取得一致改进。


<details>
  <summary>Details</summary>
Motivation: GRPO在训练推理模型时虽基于群体均值计算优势，但将每个输出视为独立样本，忽略了同一组内正确与错误解决方案之间的天然对比信号，浪费了可利用的丰富对比数据。

Method: 1) 提出GRPO的对比式重构，证明GRPO隐式最大化正确与错误样本策略比率的边际；2) 引入BICC机制，使模型在优化过程中交叉参考成功和失败的推理轨迹；3) 引入RCC机制，利用一阶方差最小化估计的奖励-置信度协方差动态调整优势基线以稳定训练。

Result: 在数学推理基准测试上的实验表明，BICC在多种模型和算法上均取得一致的性能提升。

Conclusion: BICC和RCC机制可以适配到所有GRPO变体中，无需额外采样或辅助模型，在数学推理基准测试上的一致性改进证明了其有效性和通用性。
...
### [103] [Developing and evaluating a chatbot to support maternal health care](https://arxiv.org/abs/2603.13168)
...
TL;DR: 本文介绍了一个在印度开发的产前健康电话聊天机器人，通过结合分阶段分类、混合检索和LLM生成来应对低资源环境中的挑战，并提出了针对高风险医疗部署的多方法评估工作流程。


<details>
  <summary>Details</summary>
Motivation: 在低资源环境中，用户健康素养低、获取医疗服务有限，但电话聊天机器人可以提供可信的产前健康信息。然而面临技术挑战：用户查询简短、描述不充分、跨语言混合、答案需要区域特定背景、部分或缺失的症状上下文使安全路由决策困难。

Method: 系统结合三种核心方法：(1)阶段感知分类，将高风险查询路由到专家模板；(2)针对产前/新生儿指南的混合检索；(3)基于LLM的证据条件生成。评估方面构建了：(i)150条标注的分类基准，达成86.7%紧急召回率；(ii)100条合成多证据检索基准，带有分块级证据标签；(iii)781条真实查询的LLM评判比较，使用临床医生设计的标准；(iv)专家验证。

Result: 分类器实现86.7%紧急召回率，明确报告了漏报紧急情况与过度升级的权衡。系统在多语言、多证据场景下表现良好，LLM评判与专家评估具有一致性。

Conclusion: 在多语言、噪声环境中构建可信医疗助手需要"纵深防御"设计理念，配合多方法评估，而非依赖任何单一模型或评估方法。跨学科合作（学术研究人员、健康科技公司、公共卫生非营利组织和医院）对成功至关重要。
