...
TL;DR: 研究提出使用稀疏自编码器(SAE)实现VLM可解释性故障预测，通过三阶段故障感知训练pipeline，在CLIP等模型上实现优于基线的故障预测性能，并揭示故障时模型表示从类别特定概念向模糊/风格相关概念的转变。


<details>
  <summary>Details</summary>
Motivation: VLM在高风险领域应用日益广泛，故障预测对风险感知部署至关重要。现有故障预测方法虽有效但可解释性有限，需要一种既能准确预测故障又能提供可解释洞察的方法。

Method: 将故障预测表述为对稀疏SAE潜在激活的分类任务，引入三阶段故障感知训练pipeline，鼓励学习到的潜在方向保持可解释性的同时更具故障预测信息量。

Result: 实验表明该框架在故障预测上优于评估的基线方法。进一步分析表明故障感知训练鼓励SAE潜在方向捕获更多类别特定概念，并揭示故障时模型表示从类别特定概念向更模糊或风格相关概念的转变。

Conclusion: SAE可用于VLM的可解释故障预测，学习到的潜在方向支持概念级分析并可探索运行时故障恢复。
...
TL;DR: 研究提出了MPS-Bench基准，用于评估视觉语言模型在多模态系统中的个性化安全问题。基准包含5181个场景，涵盖584张真实图像和12个高风险领域，每个场景配有隐藏的用户画像。评估8个前沿模型发现它们几乎总是直接回应（86-99%），个性化安全评分均不超过2.6/5。研究揭示了视觉 dominance 现象：视觉信息早期进入文本表示并在多模态融合时抑制文本风险信号。提出PRISM方法，通过双向跨模态调制预测何时需要延迟响应，达到0.978 AUC并在安全-效用帕累托前沿上严格占优。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地部署在高风险场景中，但模型不知道用户的医疗、情感或情境背景，可能对特定用户造成不安全响应。研究旨在解决多模态系统中的个性化安全问题。

Method: 构建MPS-Bench基准（5181个场景，584张真实图像，12个高风险领域）；评估8个前沿VLMs；进行因果干预分析多模态交互；提出PRISM方法（使用双向跨模态调制的轻量级输入监控器）

Result: 模型几乎总是直接回应（86-99%），个性化安全评分均不超过2.6/5；视觉信息早期进入文本表示并抑制文本风险信号；PRISM达到0.978 AUC并在所有测试模型的安全-效用帕累托前沿上严格占优

Conclusion: 视觉主导（visual dominance）是模型失败的原因：视觉情感首先转移到文本流中，然后通过改变的文本表示塑造最终决策，使得后期内部修复不可靠。PRISM可有效预测何时需要延迟响应。
...
TL;DR: 本文提出RA-GRPO框架，通过引入扩散反射和反事实路径合成两个创新机制，将"反向"反思融入"正向"生成过程，有效解决了扩散模型强化学习微调中的局部最优和探索效率问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于策略梯度的强化学习方法在微调扩散生成模型时探索效率低下，容易陷入局部最优解，从而降低语义忠实度和视觉真实感。

Method: 提出Reflection-Aware GRPO (RA-GRPO)框架，包含两个核心组件：1) Diffusion Reflection（扩散反射）：通过弱估计器反转扩散过程，纠正中间采样轨迹，将潜在状态引导至真实数据流形的更高概率区域；2) Counterfactual Path Synthesis（反事实路径合成）：将纠正后的轨迹隐式蒸馏到策略中，使模型能够在不产生推理时间开销的情况下内化基于搜索的探索收益。

Result: 在T2I和T2V模型上的广泛实验表明，RA-GRPO显著优于现有方法，特别在缓解奖励黑客（reward hacking）和提高泛化能力方面表现突出。

Conclusion: RA-GRPO is architecture-agnostic and integrates seamlessly with standard pipelines, offering a promising direction for stable preference alignment in diffusion models.
...
TL;DR: 本文提出DM-Align，一个统一的单阶段优化框架，将视频生成模型的蒸馏与人类偏好对齐相结合。基于分布匹配(DM)框架，该方法同时优化两个梯度方向：一个引导生成向清晰高保真方向发展，另一个通过偏好对或组内探索构建的分布差距来引导模型向人类偏好样本靠近。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型的人类偏好对齐主要依赖强化学习(RL)，但存在巨大计算开销。现有的工作流程将RL和蒸馏视为分离的两个阶段：先RL后蒸馏导致计算成本过高，而先蒸馏后RL则经常导致模型崩溃。

Method: DM-Align在标准DM框架基础上引入互补梯度方向。受DPO和GRPO启发，该方法利用偏好对或组内探索构建的分布差距，直接构建偏好引导梯度。通过协同这两个梯度方向，消除了传统RL所需的多步奖励评估和复杂的ODE-SDE转换。

Result: 在多个基础视频模型上的综合实验表明，这种样本引导框架能稳健提升蒸馏质量和偏好对齐效果，持续优于独立变体和顺序两阶段流水线。

Conclusion: DM-Align通过单阶段优化成功解决了传统RL对齐方法的计算开销问题，为视频生成模型的偏好对齐提供了一种更高效、更稳健的新范式。
...
TL;DR: CARNIVAL是一个基于自监督学习的蛋白质注释模型，利用模拟数据和前向模型生成的配对视图进行训练，用于拥挤的低温电子断层扫描体积中的蛋白质检测和分类，无需微调即可在真实断层扫描图上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 低温电子断层扫描(cryo-ET)中的蛋白质注释面临两大挑战：1) 有限倾斜角导致的欠采样问题；2) 测量算子带来的严重噪声和伪影。现有的对比学习方法未能充分利用模拟数据中的特权信息（如蛋白质位置和身份）。

Method: 提出两种创新策略：1) 利用前向模型的降质特性生成同一场景的域特定增强配对视图，集成到LeJEPA自监督训练框架中；2) 利用模拟管道中的蛋白质位置和身份信息来设计模型架构和损失函数，使语义信息定位到蛋白质位置。

Result: CARNIVAL在包含多种蛋白质类型和两种断层扫描处理类型的真实断层扫描图基准数据集上，无需微调即可完成分类和检测任务，性能优于使用对比目标训练的最新模型。

Conclusion: 通过前向模型增强的配对视图和模拟特权信息的结合，自监督学习可以有效解决cryo-ET中的蛋白质注释问题，为该领域提供了一种有前景的预训练范式。
...
TL;DR: 本文提出了一种基于运动边界监督的物体中心视觉表示学习框架，能够从原始视频中学习保持实例身份和一致性的表示，在深度估计、3D目标检测、3D占用预测和端到端规划等任务上取得了具有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉预训练方法通常捕获语义类别而未保留单个实例的身份和一致性，需要一种能够学习物体中心表示的方法，且不依赖人工标注或相机标定。

Method: 利用运动边界作为物体级分组的信号：使用现成的光流和聚类生成伪实例掩码，通过像素级成对度量学习监督单图像编码器。采用Motion-Verified Self-Training将监督扩展到4.21亿帧。

Result: 在单目深度估计、3D目标检测、3D占用预测和端到端规划任务上取得了与监督和自监督预训练基线相当或更优的性能，在几何和实例敏感任务上转移效果尤其突出。

Conclusion: 运动衍生的监督可以教静态图像编码器表示视觉实例，为可扩展视觉预训练提供了互补方向。
...
TL;DR: 本文提出AdaptVPR，一个基于路线感知的生成式增强框架，用于构建同场景困难正样本以提升视觉地点识别的鲁棒性。通过视觉语言模型解析场景属性并估计编辑可行性，结合三种生成路线（全局外观路线、局部遮挡路线、双重路线）来合成挑战性样本，并使用VPR导向的验证方案筛选高质量数据。构建了包含16万验证样本的AdaptCities数据集，在多个VPR基准和视觉基础骨干网络上实现了一致的性能提升，在域偏移挑战下R@1提升高达9.2%。


<details>
  <summary>Details</summary>
Motivation: 视觉地点识别(VPR)通过检索数据库中相同或邻近地点的图像来定位查询图像，但其鲁棒性常因光照、天气、季节变化和动态遮挡等域偏移而下降。现有训练数据中相同地点的外观多样性有限是导致这一问题的重要因素。

Method: AdaptVPR框架包含以下核心组件：1)视觉语言模型解析场景属性并估计编辑可行性；2)基于规则的调度器根据编辑性分数和风险约束确定生成路线；3)三种互补生成路线——全局外观路线引入天气/光照/时间变化，局部遮挡路线插入合理的动态遮挡物，双重路线结合两类扰动产生更具挑战性的外观偏移；4)VPR导向验证方案基于几何一致性和外观多样性评估候选样本，对全局候选一次生成验证失败即拒绝，对局部遮挡和双重候选使用验证反馈进行有限的提示优化和重生成。

Result: 在多个VPR基线和视觉基础骨干网络上进行实验，标准基准测试获得一致性能提升，在具有挑战性的域偏移条件下R@1提升高达9.2%。

Conclusion: AdaptVPR通过路线感知的生成式增强策略有效解决了VPR训练数据中同地点外观多样性不足的问题，显著提升了系统在域偏移条件下的鲁棒性。该方法具有良好的通用性，可与多种VPR方法和视觉骨干网络结合使用。
...
TL;DR: 研究利用3D Gaussian Splatting (3DGS)从短扫描中重建物体，创建每类形状原型，并与冻结的DINOv2图像特征融合，实现无标注训练集的目标识别，无需CAD模型即可达到与CAD相当的识别性能。


<details>
  <summary>Details</summary>
Motivation: 制造业和服务机器人中经常缺乏标注训练集和CAD模型；2D捕获无形状先验；冻结基础特征在几何相似、低纹理的工业零件上表现不佳。需要探索短期物体扫描能为识别带来什么额外价值。

Method: 对每个物体进行3DGS重建，总结为每类形状原型，与冻结的DINOv2图像特征融合。在T-LESS（工业零件）和HOPE（家居物品）数据集上测试。

Result: 在T-LESS上3DGS几何与CAD识别效果相当；在HOPE上几何单独达到0.920 vs 图像单独0.832，融合为0.872；在T-LESS上从0.560提升到0.591融合。先验拯救更多图像失败案例而非破坏成功案例，部分遮挡下benefit增长。几何有价值而非渲染像素，冻结特征识别几乎光照不变（2.5分以内）。

Conclusion: 3DGS是获取点云进行识别的便捷路径；形状可识别性决定收益大小；价值在于几何而非渲染像素；先验是互补的而非均匀加性的，在部分遮挡下更有价值。研究范围限于识别，不涉及BOP姿态基准。
...
TL;DR: 本文提出了一种可提示的局部运动表示方法，通过空间掩码指定区域，在保留全局上下文的同时生成持久的区域可寻址嵌入，实现对象级运动迁移和多演员视频中的局部动作分类。


<details>
  <summary>Details</summary>
Motivation: 现实世界动态本质上是组合性的，多个实体同时在共享场景中移动，每个实体表现出不同的运动模式。现有视频表示方法全局编码运动，无法显式捕获单个实体的局部运动。局部嵌入通常从裁剪图像计算或在编码后掩码特征，丢弃了解释运动所需的上下文。

Method: 提出一种提示式局部运动表示方法，生成由空间掩码定义的用户指定区域的持久嵌入。不裁剪输入或掩码特征，而是处理完整视频并直接根据查询区域调节运动编码，生成时间一致、区域可寻址的嵌入，同时保留全局上下文以消除歧义。

Result: 实现了对象级运动迁移，支持动态场景的可控组合。支持多演员视频中的局部动作分类。在两项任务中，本方法提高了可控性，优于通过裁剪或后处理掩码实现的全局表示方法。

Conclusion: 该方法通过直接对查询区域条件调节运动编码，而非事后裁剪或掩码，成功在保留全局上下文的同时实现了局部运动的有效表示，为视频编辑和动作分析提供了新思路。
...
TL;DR: FAVE是一种受人类主动视觉启发的轻量级可变分辨率视觉Transformer，通过在外部选择的区域上进行高分辨率编码来平衡计算效率和细粒度视觉理解。在小物体识别任务中，比固定分辨率ViT节省12.7倍FLOPs并提升9.4个Top-1点；集成到FastVLM后，在TextVQA上提升1.60点，TTFT加速3.3倍。


<details>
  <summary>Details</summary>
Motivation: 视觉编码器在处理高分辨率局部细节和紧凑全局编码之间存在权衡。现有的全图像高分辨率处理成本高昂，而压缩的全局编码会削弱局部证据。研究旨在分离"看哪里"和"编码什么"两个问题，专注于后者。

Method: 提出FAVE（Foveated Adaptive Visual Encoding），一种轻量级可变分辨率ViT，对外部选择的区域进行高分辨率编码（foveated），同时保留原生几何结构。研究者首先使用oracle ground-truth crops在受控的小物体环境下隔离编码问题，然后将FAVE作为局部分支集成到FastVLM中，与全局视觉token结合，而保持原始全局路径和语言模型冻结。

Result: 在ImageNet小物体（最大边96像素）上，FAVE比相同crop窗口的固定分辨率ViT提升9.4个Top-1点，FLOPs降低12.7倍；增加全局分辨率或 backbone容量无法达到相同性能。在FastVLM集成实验中：仅使用16个额外局部token，在TextVQA上提升1.60点，TTFT加速3.3倍（vs SmolVLM2-2.2B）；在GQA属性问题上提升1.31点（vs FastVLM-1.5B），缩小了与FastVLM-7B的差距。

Conclusion: 选择性分配高分辨率局部容量能够高效补充更广泛的全局表示和模型缩放，为小物体、文本和属性的细粒度理解提供了一种高效的解决方案。该方法证明了"局部高分辨率+全局粗分辨率"的混合策略在视觉语言模型中的有效性。
...
TL;DR: ICM-Bench是首个专门评估多模态代理在长视频中保持身份一致性和跨时间身份关系推理能力的基准测试，包含839个合成视频片段、141分钟时长和1217个关于6个 recurring adults 的开放问题。


<details>
  <summary>Details</summary>
Motivation: 现有长视频和多模态代理基准测试虽然评估广泛的记忆问答，但未分离维护 recurring person identities 和跨时间关系推理的能力。

Method: 构建了一个主题可配置的管道生成视频集合，每个问题关联目标身份和可追踪的支持证据。

Result: Gemini 3.1 Pro达到最高总体准确率74.0%，但在需要长期身份画像的问题上得分降至60.3%。

Conclusion: 当前系统能恢复许多事件级记忆，但在围绕稳定人物积累证据时可靠性较低，身份中心推理仍是挑战。
...
TL;DR: 开发了一种全自动4D U-Net用于4D flow MRI中的主动脉分割，使用参数高效的混合4D内核捕获时间上下文，并使用从现有2D专家轮廓和中心线派生的稀疏4D标签进行训练，避免了对密集4D标注的需求。


<details>
  <summary>Details</summary>
Motivation: 4D flow MRI中主动脉分割对于血流动力学评估至关重要，但受到稀缺密集标注和高计算需求的限制。

Method: 开发了全自动4D (3D+时间) U-Net，使用参数高效的混合4D内核捕获时间上下文，使用稀疏4D标签（从现有2D专家轮廓和中心线派生）进行训练。训练集包含来自8个中心和2个供应商的268次扫描，内部测试集32次扫描，外部测试集30次扫描。

Result: 4D U-Net在内部测试集Dice为0.927，外部测试集为0.911，而3D U-Net为0.919/0.847，静态PC-MRA为0.893，基于配准的传播为0.808。收缩期差异较小，舒张期差异显著。与专家轮廓在峰值速度、净流量、轴向和周向壁剪切应力以及直径方面的一致性良好（ICC≥0.954内部，≥0.980外部）。

Conclusion: 该方法提供了可重复的、时间解析的主动脉分割，用于自动血流动力学分析，并可跨多中心、多供应商和独立对比后数据进行泛化。该模型已公开可用。
...
TL;DR: 本文首次探索了基础模型特征在猕猴追踪组织学纤维束分割中的应用，比较了BCE-Dice、clDice、Betti matching和Topograph等拓扑感知损失函数，并提出了Excess32空间诊断指标来解决传统评估规则无法检测过度分割的问题。


<details>
  <summary>Details</summary>
Motivation: 手动标注组织学数据劳动密集型，现有自动分割方法主要依赖像素重叠损失（如BCE和Dice），拓扑感知损失函数在该任务中尚未被研究。同时，传统评估规则（允许束检测有任何重叠）无法捕捉过度分割问题。

Method: 使用冻结的DINOv3骨干网络提取特征，比较BCE-Dice、clDice、Betti matching和Topograph四种损失函数在猕猴追踪组织学纤维束分割中的表现。提出Excess32空间诊断指标，测量预测像素在标注束32像素容忍带外的数量。

Result: BCE-Dice获得最高Dice分数；clDice获得最高束召回率但掩码重叠较差；Topograph的Dice与BCE-Dice相似，β0误差最低，假阳性比BCE-Dice和Betti matching更少。Betti-Topograph联合将稀疏束TPR从0.818提升至0.933，但FDR从0.296恶化至0.509，Excess32从0.108恶化至0.466，面积比从0.94恶化至3.34。

Conclusion: 检测指标 alone不足以表征分割质量；需要结合空间诊断指标（如Excess32）来量化过度分割问题；基础模型特征（如DINOv3）可有效用于纤维束分割任务。
...
TL;DR: 本文将COPUS（本科STEM课堂观察协议）重新定位为视频基准，用于评估多模态基础模型。该基准提供24维二进制标签向量，每2分钟一个标签，跨越50-90分钟的讲座。作者提出VISTA基线方法，使用MiniCPM-V-4.5结合密集滑动窗口和MLP头进行预测。在三个保留的化学讲座上达到80.1%的受限宏准确率，优于零样本方法的74.9%。


<details>
  <summary>Details</summary>
Motivation: 当前视频-语言基准通常由数据集作者构建，未发布可靠性统计数据，导致构念的噪声底限未知。作者认为多模态基准测试受益于已投资于可靠性策略的研究社区，因此引入具有十年同行评审可靠性文献的COPUS观察仪器作为视频基准。

Method: 作者提出VISTA基线方法：1) 对视频运行密集滑动窗口；2) 使用冻结的MiniCPM-V-4.5主干网络并在其上训练轻量级MLP头进行每窗口输出细化；3) 将预测结果最大池化到2分钟COPUS网格上。评估 corpus由5人人类评估员小组生成共识矩阵作为参考标准。

Result: 在三个保留的化学讲座上，VISTA达到80.1%的受限宏准确率，而零样本变体为74.9%。最大残差出现在视觉相似的讲师代码和罕见的音频依赖代码上。

Conclusion: 作者确定了三种系统性失败模式：音频部分可观察性、细粒度小组工作区分和长尾召回。该基准工具、提示和基线代码已开源发布，为多模态模型评估提供了可靠的基准框架。
...
TL;DR: DART是一种用于手术场景的RGB-D预训练方法，在DINOv2基础上加入伪标签深度图的像素级重建目标，提升视觉基础模型的表征质量，仅在预训练阶段使用深度信息，推理时仍为RGB-only。


<details>
  <summary>Details</summary>
Motivation: 手术领域数据稀缺，自然图像视觉基础模型迁移效果差，而手术场景富含几何信息。现有自监督预训练仅使用RGB图像，未利用深度图这一重要互补信号。虽然强预训练模型可生成伪标签深度图，但这一资源未被充分利用。

Method: 基于DINOv2构建，在masked iBOT patches上添加像素级深度重建头作为监督目标，使用伪标签深度进行训练。预训练后仅保留RGB推理能力，深度重建头不参与微调和推理阶段。

Result: 在8个手术基准数据集（涵盖分割、深度估计、图像级识别任务）上，DART优于自然图像基线和域内基线（包括使用相同数据训练的 vanilla DINOv2），提升了密集预测和图像级理解能力。深度作为监督目标比Canny边缘更有效。

Conclusion: 免费的几何伪标签可增强基础模型预训练，无需额外标注或增加推理成本，为手术领域提供更强 backbone，表明伪标签深度图是提升视觉基础模型的有效途径。
...
TL;DR: 本文提出了一种双部分MLBNet架构，用于心血管导管插入术场景中的多类分割。该架构采用多侧向编码器块和多头解码器分支，能够同时分割导丝、导管、血管和背景像素，具有较高的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常关注二值分割，但心血管导管插入术场景中需要同时分割多个结构，因此需要一种快速、准确且可解释的多类分割模型。

Method: 设计了双部分MLBNet架构，包含多侧向编码器块和多头解码器分支。编码器中的侧向分支能够进行重复特征提取，学习多样的共享表示；多个解码器头引入类别偏置分支，专门学习导管插入术场景中不同结构的特性。使用幻影模型、合成人类模拟主动脉和动物模型获取的多类分割血管造影数据进行训练和评估。

Result: 双部分模型能够有效分离导丝、导管、血管和背景像素到各自的类别，具有高概率。模型能够以高整体准确度区分主要背景类与前景结构。

Conclusion: MLBNet架构能够有效实现心血管导管插入术场景中的多类分割，同时保持较高的准确性和可解释性，为临床诊断和手术导航提供了潜在价值。
...
TL;DR: 研究探讨了XR系统中 gaze 数据表示方式对隐私-实用性权衡的影响，比较了原始 gaze、空间注意力热图和工程化眼球运动特征三种表示方法，发现工程化特征可在保留约85%动作识别精度的同时将用户重识别率降低约一个数量级。


<details>
  <summary>Details</summary>
Motivation: 随着XR系统普及，eye tracking和head tracking虽能推断用户意图和注意力，但同时也会泄露生物特征身份。研究旨在探索在特征提取阶段通过选择不同的gaze数据表示方式来实现轻量级隐私控制，而非依赖后续的扰动或正式隐私机制。

Method: 使用egocentric HoloAssist数据集，在模型容量匹配的条件下比较三种gaze表示：raw gaze、spatial attention heatmaps和engineered eye-movement features。评估指标包括动作识别（作为任务实用性）和闭集用户重识别（作为隐私泄露）。

Result: 表示选择显著改变隐私-实用性权衡。工程化特征保留约85%的原始gaze动作识别准确率，同时将重识别率降低约一个数量级，在206个身份中约为随机率的4倍。该研究还发现抽象化本身并不能保证隐私。

Conclusion: 工程化特征暴露了可解释和可审计的结构，为设计师提供了透明的隐私杠杆，可作为差分隐私等机制的补充。但这种降低只是缓解而非消除身份泄露问题。
...
TL;DR: 本文针对扩散变换器(Diffusion Transformers, DiTs)在生成空间一致的相机角度变化方面的局限性，提出构建合成多视角数据集的解决方案。由于缺乏专门的人类主体多视角训练数据，当前模型难以执行自由形式的提示性相机角度变化。作者提出头部场景旋转差异(Head Scene Rotation Difference, HSRD)指标来定量评估围绕人物的相机运动，通过解耦相机运动与局部头部姿态操作来实现可靠评估。


<details>
  <summary>Details</summary>
Motivation: 当前生成式图像编辑扩散变换器(DiTs)在语义编辑方面表现出色，但在实现空间一致的相机角度变化方面仍存在困难。训练能够执行自由形式、可提示的相机角度变化的基础模型面临主要瓶颈，即缺乏专门的训练数据。虽然存在针对通用3D环境和对象的多视角数据集，但缺乏在自然场景中固定位置的人类主体多视角配对数据集（包括正面和侧面视角）。在无约束环境中捕获此类多相机数据既具有挑战性又难以扩展。

Method: 首先尝试使用多个最先进的图像编辑模型来合成创建所需的多视角数据，但发现输出经常存在关于头部相对于背景转动程度的幻觉问题，导致环境不一致。为解决此问题，作者提出头部场景旋转差异(HSRD)指标来定量评估围绕人物的相机运动。该指标通过将相机运动与局部头部姿态操作解耦来实现评估。

Result: HSRD指标提供了评估场景中人物3D空间视差所需的pipeline，为可靠构建高质量多视角合成数据集铺平了道路。广泛实验表明，该指标能够有效评估人物在场景中的空间变化。

Conclusion: 本文提出的HSRD指标能够可靠评估围绕人物的相机运动，使构建用于训练基础模型执行自由形式提示性相机角度变化的高质量多视角合成数据集成为可能，解决了该领域缺乏专门训练数据的核心问题。
...
TL;DR: SVDtrunc是一种针对Diffusion Transformers (DiTs)的高效压缩方法，通过两步骤实现块级压缩：首先在全局参数预算下对各块分配秩并用截断SVD压缩最不重要的块，然后使用模块化知识蒸馏和 rectified-flow 目标进行微调。在FLUX.dev上实现40-90%压缩率，在GenEval、HPSv2和DPG三个基准上优于所有竞争方法。


<details>
  <summary>Details</summary>
Motivation: DiTs已成为高质量文本到图像生成的主导架构，但其大规模部署面临效率挑战。截断SVD是降低参数的正统工具，虽然大语言模型中低秩近似可能导致灾难性失败，但研究发现DiTs中截断SVD能产生平滑降级，冗余分布在整个网络的投影矩阵中而非集中在少数transformer块。

Method: 提出两步骤块级压缩方案SVDtrunc：1) 跨块分配秩，在全局参数预算下通过截断SVD压缩最不重要的块；2) 使用模块化知识蒸馏和rectified-flow目标对所有块进行微调。

Result: 在FLUX.dev上应用SVDtrunc，压缩范围为原始参数量的40-90%。在GenEval、HPSv2和DPG三个基准上超越所有竞争方法。在68%参数下保持接近完整性能，57%参数下仍具竞争力。此外，SVDtrunc可与步蒸馏互补，即使不微调也能获得良好结果。

Conclusion: SVDtrunc是大型生成模型超越扩散步降质的高效压缩实用方法，能够在大幅压缩参数的同时保持接近完整的性能，为DiTs的高效部署提供了有效途径。
...
TL;DR: DiT Readout (ReaDiT) Guidance是一个轻量级框架，通过利用DiT模型的内部特征表示来控制生成过程，可用于图像和视频生成的控制。


<details>
  <summary>Details</summary>
Motivation: 现有的DiT模型缺乏有效的特征级控制方法，需要一种轻量级的方式来利用模型内部特征实现生成控制，同时支持空间目标（如深度、姿态、边缘地图）和视频生成中的摄像机与运动控制。

Method: 使用单个DiT块的特征来引导生成过程，根据测试时提供的空间目标（如深度图、姿态图、边缘图）来调整生成方向。

Result: 在图像和视频生成任务上取得了竞争性或更好的结果，同时只需要更少的参数量。

Conclusion: ReaDiT Guidance是一种有效且轻量的DiT模型控制方法，能够在不增加大量参数的情况下实现对生成过程的精细控制，具有良好的通用性和扩展性。
...
TL;DR: 提出了一种可解释的OCTA（光学相干断层扫描血管造影）管道，用于阿尔茨海默病（AD）的早期识别，无需诊断标签即可进行表型分析。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病的早期识别具有挑战性，因为现有评估方法成本高、资源密集或不适用于大规模筛查，需要一种无创、适用于人群规模筛查的方法。

Method: 使用117张ROSE-1图像（来自39名受试者），应用注释匹配的分割模型对浅层血管复合体（SVC）、深层血管复合体（DVC）和组合SVC+DVC进行分割；提取6个密度和分形维数生物标志物形成受试者水平的探索性聚类；使用GPT、Gemini和Llama生成报告并进行评估。

Result: 模型达到ROC-AUC值0.916-0.970，Dice分数0.695-0.781；分析9名留出受试者发现内部一致的低密度、低分形维数表型；LLM报告可评估测量锚定、引用忠实度和诊断谨慎性。

Conclusion: 该框架为视网膜血管测量、探索性表型分析和循证解释提供了透明、非诊断性的连接，为阿尔茨海默病研究提供了新途径，但缺乏诊断标签限制了临床解释。
...
TL;DR: 本文提出了一种结合音频和视觉特征的多模态情绪识别框架。音频采用Wav2Vec2、MFCC和统计声学描述符通过BiLSTM提取特征；视频采用ResNet50-BiLSTM架构提取时空特征；融合层使用多头注意力机制进行特征级融合。在MELD和IEMOCAP数据集上验证，模型在准确性和鲁棒性上均优于基线方法，且注意力融合机制在数据不平衡情况下效果显著。


<details>
  <summary>Details</summary>
Motivation: 多模态情绪识别在人机交互、远程教育和医疗健康等领域具有重要应用价值。当前方法在融合多模态信息和处理不平衡数据方面存在挑战，需要提出更有效的特征提取和融合策略。

Method: 音频模块：提取Wav2Vec2语义嵌入、MFCC特征及pitch/energy/rhythm等统计声学描述符，通过BiLSTM捕捉时序依赖；视频模块：采用ResNet50-BiLSTM架构结合深度残差学习和序列建模提取面部序列的时空特征；融合策略：引入基于多头注意力的特征级融合机制，自适应调整各模态贡献权重。

Result: 在MELD和IEMOCAP数据集上，该模型在准确性和鲁棒性方面均显著优于基线方法。消融实验表明，基于注意力的融合策略能显著提升不平衡数据场景下的性能。

Conclusion: 所提框架能有效捕捉语音和视觉表达中的多样化情绪线索，为现实世界的多模态情绪识别任务提供了一种实用且可推广的解决方案。
...
TL;DR: LookThere是一种基于强化学习的自适应计算方法，用于视觉Transformer的令牌选择。它通过联合训练浅层输入选择器和深层表示提取器，让选择器学习"看哪里"，提取器学习"看什么"，从而在只处理少量令牌的情况下保持任务性能。该方法可在高分辨率稀疏识别任务中仅使用0.2%的输入即可保持准确率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉Transformer将每个图像令牌视为同等重要，但大多数计算机视觉任务只需要部分令牌即可完成。自适应计算方法虽能加速推理，但在极端稀疏条件下表现不佳，且现有方法依赖于令牌多样性、注意力分数等辅助信号，缺乏通用性。

Method: 提出端到端强化学习框架，联合训练浅层输入选择器和深层表示提取器。选择器学习在何处聚焦，提取器学习提取什么特征，两者协作完成针对特定任务的计算节约，无需依赖辅助信号。

Result: LookThere仅选择任务特定的输入，在高分辨率稀疏识别任务（交通标志、台球）中表现出色，可仅使用0.2%的输入维持准确率。跨任务和模型泛化能力强，包括全局识别(ImageNet分类)、局部识别(ADE20K分割)、零样本分类(通过蒸馏)和回归(计数)任务。在所有设置中均超越现有最先进的令牌选择方法。

Conclusion: LookThere提供了一种通用且可扩展的自适应计算框架，在性能-计算权衡中达到新的帕累托前沿，为高效视觉Transformer推理提供了新思路。
...
TL;DR: AngelFingerprint是一个新型水印框架，通过将编辑提示的CLIP文本嵌入直接整合到扩散模型的权重中，实现编辑可追溯性、可解释性和白盒隐蔽性。该方法在MagicBrush数据集上提取器实现了200路提示检索86%的top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 开源白盒环境下，图像编辑水印面临两大问题：1) 传统水印携带固定ID，无法解释编辑内容和提示词；2) 攻击者可轻易定位和移除作为独立模块添加的水印。需要一种能嵌入编辑语义信息且难以检测和隔离的水印方案。

Method: 1) 将LoRA集成到扩散模型中，将编辑提示的CLIP文本嵌入直接嵌入到模型权重中；2) 设计速度对齐锚（velocity-alignment anchor）技术保持编辑质量；3) 设计特殊频率滤波器使水印不可感知但可恢复且鲁棒。提取器可从图像像素中恢复嵌入信息。

Result: 在MagicBrush数据集上，提取器在200路提示检索任务中达到86%的top-1准确率，而提示反转方法仅达到20%。

Conclusion: AngelFingerprint成功实现了编辑可追溯性、可解释性和白盒隐蔽性的统一，为文本引导扩散编辑提供了可靠的真伪来源保障。
...
TL;DR: HiSfM是一种分层由粗到精的运动恢复结构(SfM)框架，通过脚手架构建提高鲁棒性和效率，解决传统管道在重复或对称结构导致的视觉歧义下失败以及计算成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 传统SfM管道在处理具有重复或对称结构的场景时经常失败(硬视觉歧义问题)，且由于冗余相机和约束导致计算成本高。

Method: HiSfM首先使用几何诱导启发式方法形成强局部社区，然后通过打包边不相交生成树(EDST)连接社区构建紧凑而强的骨架，用两视图歧义消除器验证骨架边缘，在验证的骨架上重建稳定脚手架作为场景本质的锚点，随后通过高效注册和三角化吸收剩余图像进行进一步优化。

Result: 在歧义聚焦基准测试和通用数据集上的实验表明，HiSfM防止了歧义导致的失败，同时相比先前方法显著降低了运行时间，并在激进稀疏化方法上提高了完整性。

Conclusion: HiSfM通过分层脚手架方法有效解决了SfM在对称/重复结构场景中的鲁棒性和效率问题。
...
TL;DR: 本文提出BMT框架，通过共享分层Vision Transformer协同优化SAR到光学图像翻译和语义分割任务，集成局部注意力融合、多尺度 refinement、ControlNet条件注入和Kendall不确定性加权等技术，在paired和unpaired翻译设置下实现竞争力的翻译质量和分割性能。


<details>
  <summary>Details</summary>
Motivation: SAR图像具有全天候观测能力，但相比光学图像存在斑点噪声和散射机制不直观的问题，限制了图像可解释性。现有生成模型在进行SAR到光学(S2O)转换时往往忽略语义结构约束，影响下游任务应用。

Method: 提出统一的双任务学习框架BMT，包含四个核心组件：1) LocalViTBlock通过可学习门控机制融合全局自注意力和空间深度卷积；2) 增强输出模块结合多尺度refinement、色彩校正和抗锯齿，通过特征融合校准通道级色彩统计；3) ControlNet风格条件注入机制将SAR小波特征和分割标签编码为多尺度特征金字塔，通过零初始化卷积注入各编码器层；4) 有界Kendall不确定性加权方案防止任一任务主导共享表示。

Result: 在公开的WHU-OPT-SAR配对数据集和自构建的HRSID/DIOR非配对船舶数据集上评估，实验结果表明该方法在S2O翻译质量和语义分割性能方面均达到有竞争力的水平。

Conclusion: BMT框架成功实现了SAR到光学图像翻译与语义分割的联合优化，验证了双任务学习在跨模态图像转换中的有效性，为SAR图像解译提供了新的思路。
...
TL;DR: IVSGround是一个学习性视图选择框架，用于基于VLM的3D视觉定位。该方法通过两阶段拒绝采样从推理VLM获取监督信号，训练轻量级视图选择器来识别具有区分性的视图，而不是使用启发式规则。在推理时，选择器预测查询条件的影响视图，然后由冻结的推理VLM进行比对定位。


<details>
  <summary>Details</summary>
Motivation: 现有零样本3D视觉定位方法依赖于启发式规则选择相机视图，这些规则通常优先考虑对象可见性而非定位相关性，导致视图选择 suboptimal。

Method: 1) 训练轻量级视图选择器识别具有区分性的视图；2) 通过两阶段拒绝采样从推理VLM获取监督信号；3) 推理时预测查询条件的影响视图并进行比对定位。

Result: 在ScanRefer和NR3D数据集上，IVSGround持续提升定位精度，证明选择看哪里对有效3D视觉定位至关重要。

Conclusion: 学习性视图选择优于固定启发式规则，为基于VLM的3D视觉定位提供了更有效的视图选择策略。
...
TL;DR: SeamFlow是一个用于3D表面切割的新型生成框架，将离散的网格切割问题重新表述为高维边缘概率空间中的连续流匹配，通过边缘标记化提高拓扑感知能力，消除3D空间投影误差和人工顺序偏差，在语义一致性和参数化失真方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统几何优化方法主要关注减少参数化失真，但往往忽视 seams 布局中的视觉语义一致性；现有的自回归生成方法虽然改善了语义一致性，但对网格拓扑的感知有限，导致局部切割不准确。

Method: 将离散网格切割问题重新表述为连续流匹配问题，通过连续松弛学习从高斯先验到目标 seam 概率分布的确定性映射；演化网络将局部拓扑 tokens 与全局形状先验耦合，通过常微分方程求解引导平滑概率流。

Result: 相比现有自回归生成框架，SeamFlow 通过边缘 tokenization 提高了拓扑感知能力，消除了 3D 空间投影误差和人工顺序偏差。

Conclusion: SeamFlow 实现了卓越的语义一致性和极低的参数化失真。
...
TL;DR: 本文提出PSP(Plugin Sampler Pipeline)和两项工程优化策略,解决工业异常检测中内存银行构建延迟和推理效率的瓶颈问题。PSP无需骨干网络前向传递即可完成采样,构建速度比FPS快341倍;推理优化策略可将评估时间减少20倍。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临两个工程瓶颈:内存银行构建延迟和推理效率。传统采样算法依赖大量骨干网络前向传递和迭代距离计算,构建时间从分钟到小时不等;多尺度特征提取等重计算组件难以满足生产线毫秒级实时需求。

Method: (1) PSP(Plugin Sampler Pipeline):基于18维像素元数据和五个互补视觉插件的四阶段自适应内存银行采样管道,在零骨干网络前向传递下完成所有采样,粗筛选为亚秒级数值排序,元数据提取为一次性离线成本。支持渐进式部署和增量更新。(2)两项工程优化策略:并行内存银行相似度计算和大规模评估的分层像素采样。验证载体为LUMIN网络,具有极致的分割头压缩。

Result: 在五个基准数据集上的实验表明,PSP以接近随机构建成本达到最先进的采样精度(FPS快341倍),推理优化策略在精度损失可忽略的情况下将评估时间减少20倍。

Conclusion: PSP和工程优化策略有效解决了工业异常检测中的效率瓶颈问题,在大幅提升采样构建速度和推理效率的同时保持了检测精度,为工业部署提供了可行的解决方案。
...
TL;DR: 本文提出CoMLP模块，通过合作式跨门控和MLP交互实现高效细粒度跨模态医学图像分割，在多模态融合和语言引导分割任务中取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有融合方法依赖交叉注意力，计算负担随空间分辨率快速增长，难以在高清特征图上进行密集跨模态交互，特别是对于体医学图像，因此需要一种高效且能保留细粒度空间细节的跨模态融合方法。

Method: 提出CoMLP模块，采用合作式跨门控机制结合互补的区域MLP和膨胀MLP交互，捕捉局部和全局跨模态依赖关系；开发多源融合架构，同时实现图像间跨模态融合和视觉-语言融合，避免密集交叉注意力计算。

Result: 在五个医学分割基准数据集上（涵盖2D/3D图像、临床报告、多种成像模态和解剖部位）进行广泛实验，验证了CoMLP相比现有最先进的多模态和语言引导分割方法的性能优势，消融实验表明高分辨率细粒度交互和互补局部-全局融合对性能提升至关重要。

Conclusion: CoMLP通过合作式跨门控机制实现了无需密集交叉注意力即可进行的高效细粒度跨模态信息融合，在2D/3D医学图像分割任务中均展现出优异性能，证明了MLP-based交互是医学图像分割中跨模态融合的有效替代方案。
...
TL;DR: CLON是一种用于零样本6D姿态估计的前端方法，无需针对新对象进行任务特定训练。该方法通过渲染模板构建语言语义记忆用于自上而下的提议生成，并使用对象集线索权重进行校准的提议评分。在七个BOP-Classic-Core数据集上，CLON在检测AP、分割AP和下游6D姿态AR方面均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 零样本6D姿态估计管道越来越依赖强大的下游姿态求解器，但其性能往往受限于前端：对象提议必须保留部分可见的真阳性同时拒绝语义上合理的干扰物。现有前端方法在处理新对象时缺乏有效的泛化能力。

Method: CLON方法包含两个核心组件：1）语言语义记忆：基于渲染的模板构建，用于自上而下的提议生成，引导SAM 3生成高召回率的 onboarded 对象提议；2）线索权重：在场景推理前从 onboarded 对象集计算一次，在线评分时保持固定。两种组件均无需任务特定训练。

Result: 在七个BOP-Classic-Core数据集上，CLON相比CNOS和SAM-6D前端，检测AP提升8.1个百分点，分割AP提升6.2个百分点，下游6D姿态AR提升最高4.1个百分点。

Conclusion: CLON作为零样本6D姿态估计的前端，无需针对新对象进行额外训练即可实现有效的对象提议生成和评分，具有良好的泛化能力和实用价值。
...
TL;DR: 本文提出了一种基于DenseNet-121的三分支注意力引导深度学习框架，通过自适应融合全局上下文信息和病灶聚焦局部信息来提升图像分类性能。在皮肤病变和番石榴叶等数据集上达到97.75%和99.64%的准确率。


<details>
  <summary>Details</summary>
Motivation: 病灶聚焦图像分类的核心挑战在于判别信号稀疏、空间分散且易被背景噪声掩盖，传统CNN均匀处理整幅图像会稀释信号相关性。

Method: 提出三分支架构：1)全局分支学习完整图像表示并用Grad-CAM生成注意力掩膜；2)局部分支结合CBAM从聚焦区域提取空间和通道特征；3)自适应融合分支学习实例特定权重动态整合全局与局部表示。

Result: 融合分支优于单独的全局和局部分支，在皮肤病变数据集达97.75%准确率，番石榴叶数据集达99.64%准确率。

Conclusion: 注意力引导架构能提升模型透明度、增强特征相关性，支持医疗图像分析中更可靠的数据驱动决策。
...
TL;DR: 本文提出Intrinsic Temporal Adaptation (ITA)框架，用于部分相关视频检索(PRVR)任务，通过内部时间适配和亲和度加权梯度传播解决细粒度视频理解问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖冻结的CLIP帧特征，缺乏时间理解能力；即使使用参数高效CLIP适配，视频级预测仍可能基于不精确的帧级证据。PRVR需要超越粗粒度视频级匹配的细粒度理解。

Method: 提出两个核心组件：1) Backbone-Internal Temporal Adaptation允许最后几个视觉Transformer层对相邻帧组进行注意力计算，生成时间感知帧嵌入；2) Affinity-Weighted Gradient Propagation利用文本-帧亲和度软聚合top-k帧，将学习信号传播到多个查询相关帧，解决PRVR的弱监督问题。

Result: 在PRVR基准测试中达到最先进性能，展示强大的跨数据集迁移能力，并在真实查询相关时刻内检索到更准确的帧级证据。

Conclusion: ITA框架通过保持CLIP冻结仅训练适配参数的方式，实现了高效的时间建模，为PRVR任务提供了一种有效且可迁移的解决方案。
...
TL;DR: 提出了一种名为Linguistic Trajectory Encoding (LTE)的新型记忆表示方法，用于具身智能体在长时间跨度的任务中处理动态物体的状态转换查询。LTE结合自然语言描述、稀疏空间锚点和视觉锚点来压缩动态物体运动历史，并构建了Spatial Memory Benchmark (SMB)评估基准。


<details>
  <summary>Details</summary>
Motivation: 现有系统无法在自然语言中查询长时间（数小时到数天）观察范围内动态对象的状态转换：视频语言嵌入丢失细粒度运动信息，几何SLAM仅保留原始坐标，智能体工作记忆仅围绕即时任务上下文组织。缺乏一种能够随时间存储可语言查询的每对象时间线的表示方法。

Method: LTE采用混合表示方法：1）将无可靠观测期间锚定到最后一次可见位置；2）用几何路标表示运动以保持准确性；3）用自然语言描述补充语义信息。构建了从EgoLife多日记录中提取的SMB基准测试。

Result: 在SMB上，LTE系统在语义轨迹检索任务达到45.3%成功率，长时域物体检索任务达到48.7%，优于结构化记忆和VLM基线（最佳先前方法分别为31.9%和34.4%）。轨迹压缩率达8.7×至26.1×，24小时视频查询延迟低于1秒。在Ego4D自然语言查询上达到28.75% R@1和55.10% R@5，分别比EgoVLPv2提升15.80和31.30个百分点。

Conclusion: LTE通过混合语言-空间表示成功解决了具身智能体在长时间跨度下查询动态物体状态转换的问题，显著优于现有方法，并为长时域记忆和推理提供了新的研究方向。
...
TL;DR: 本文提出Weather-Conditioned Depth Anything (DA-W)框架，通过显式解耦风格与内容，实现恶劣天气条件下的鲁棒单目深度估计。核心创新在于引入Style Filter提取天气风格嵌入，结合零初始化适配器进行参数高效调制。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计基础模型（如Depth Anything系列）在正常条件下表现优异，但在雾、雨、雪、夜间等恶劣天气条件下存在关键性失败，需要一种能够适应各种天气条件的鲁棒深度估计方法。

Method: 1）构建包含真实和合成退化数据的Style Filter训练集；2）训练Style Filter提取内容无关的天气风格嵌入；3）使用零初始化的轻量级适配器将风格嵌入注入Depth Anything主干网络；4）采用伪标签蒸馏和对齐策略进行训练，实现单一模型同时处理多种恶劣天气和正常条件。

Result: 在天气基准测试中平均提升3.7%的AbsRel（绝对相对误差），同时在标准干净基准测试中保持或略优的性能表现。

Conclusion: DA-W通过风格-内容解耦和参数高效适配器，成功实现了单一模型对雾、雨、雪、低光等多种天气条件的鲁棒深度估计，在提升恶劣天气性能的同时避免了核心泛化能力的灾难性遗忘。
...
TL;DR: PAPT++是一种基于风险感知的对抗生成训练框架，用于单域泛化任务。该方法利用预训练文本到图像扩散模型的生成能力，通过在类别条件生成空间中定义语义模糊集来搜索分类器难以正确分类的样本，从而增强模型对未见目标域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 单域泛化旨在从单一标注源域学习模型以泛化到未见的 target domain。当前策略通过使用T2I扩散模型增强或生成样本来丰富源分布，但仅靠多样性不足以实现鲁棒泛化——有效的生成样本应捕捉当前分类器难以处理的变体。受分布鲁棒优化启发，该研究寻求在预训练T2I模型的类别条件生成空间中定义语义模糊集，并搜索具有高分类损失的样本。

Method: PAPT++首先通过图像-文本对齐和类内多样性正则化学习每个类的多样化语义参考图像。这些参考图像作为去噪目标用于分类器引导的扩散合成，在减少语义漂移的同时引导生成朝向困难变体。生成的样本与源数据结合用于更新分类器，而更新后的分类器则指导下一轮合成，形成迭代过程使分类器逐步暴露于困难但语义一致的变体。

Result: 在标准单域泛化基准数据集上的广泛实验证明了所提出的PAPT++方法的优越性及其主要组件的有效性。

Conclusion: PAPT++通过风险感知的对抗生成训练框架有效解决了单域泛化问题，通过迭代生成困难样本并结合语义参考图像引导，在保持语义一致性的同时显著提升了模型对未见域的泛化能力。
...
TL;DR: LetOccVote是一种弱监督3D占用预测框架，通过跨帧投票机制改进2D伪标签的几何和语义监督，无需3D标注即可实现先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督方法直接使用不完善的2D伪标签作为监督，容易受到错误几何和语义目标的影响。作者观察到跨帧重复观测的一致性可以提供廉价且可靠的伪标签可靠性评估依据。

Method: 提出基于高斯分布的弱监督占用框架，包含两部分：1) Depth Vote：利用跨帧几何一致性 refining支持的伪深度，拒绝矛盾估计；2) Semantic Vote：在共享3D空间中聚合伪语义观测，识别可靠和争议的证据，强化可靠语义监督同时过滤不可靠的伪标签段。

Result: 在Occ3D-nuScenes数据集上达到53.27 IoU和20.39 mIoU，成为2D伪标签监督方法中的最新技术。

Conclusion: 通过跨帧投票机制有效解决了弱监督学习中伪标签噪声问题，仅使用2D伪标签监督即可实现与全监督方法相当的性能，为降低3D场景理解标注成本提供了有效方案。
...
TL;DR: 研究3D点云类别增量学习中的领域异质性问题，提出Domain3D-CIL评估协议和PolyMem方法来解决性能差异现象


<details>
  <summary>Details</summary>
Motivation: 3D感知在自动驾驶、机器人、AR/VR中有重要应用，但现有类别增量学习方法未考虑3D点云的领域异质性（CAD模型、RGB-D扫描、视频重建、损坏观测等），导致不同领域性能下降不一致

Method: 建立包含异质领域点云类别的Domain3D-CIL训练评估协议；提出PolyMem无样本方法，隐式建模特征分布的高阶统计量以增强跨领域鲁棒性

Result: 性能差异现象在所有基线方法中一致出现；PolyMem有效缓解性能差异并提升各领域模型性能

Conclusion: 3D点云的领域异质性是类别增量学习中除灾难性遗忘外的新挑战；PolyMem通过建模高阶统计量可有效解决性能差异问题
...
TL;DR: 本文提出SimFuse3D方法，解决跨平台LiDAR无监督域适应中由于传感器高度和视角变化导致的目标点分布不一致问题，通过目标模拟和置信度引导的多阶段定位重加权提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 跨平台LiDAR无监督域适应（UDA）困难，原因在于传感器高度和视角变化会改变目标级别的点分布。自训练方法中，保留的预测可能提供有用的目标位置，但包含稀疏的前景回波、背景杂波或与预测框不一致的点，即"框-点不一致"问题。

Method: 提出SimFuse3D方法，包含三个核心组件：1) Object Memory从带标签的源扫描中检索兼容的标注实例；2) Target Simulation将源数据的真值框放置在目标位置，对齐点云并过滤以近似目标观测；3) 置信度引导的多阶段定位重加权(CMLR)将目标伪对象置信度分数映射到RPN定位和R-CNN框回归的共享权重。所有组件仅在适应阶段运行，不改变检测器架构和推理图。

Result: 在六次跨平台迁移中，SimFuse3D在所有报告的AP指标上超过Pi3DET-Net，并在几乎所有指标上在比较的适应方法中排名第一。在nuScenes到KITTI的迁移中，使用两种评估检测器均在比较的适应方法中排名第一。

Conclusion: SimFuse3D通过保留目标位置并利用带标签源扫描的几何信息修复伪对象，有效解决了跨平台LiDAR点分布变化带来的域适应问题，显著提升了无监督域适应的检测性能。
...
TL;DR: 首次提出仅使用声学信号估计多人3D姿态的方法SoundMHPE，解决了多人物理叠声和反射带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有单人声学姿态估计无法处理多人场景，因为多人信号叠加导致难以区分个体姿态，且人际反射引入的传播延迟会混淆时序运动-声学关系。

Method: 提出SoundMHPE框架，包含两个关键组件：1) 声学多尺度编码器，捕获多样化时序和细粒度频率特征；2) 时序姿态解码器，使用注意力机制解耦跨帧多人信息。

Result: 构建了包含43.2万帧同步多人姿态和声学数据的6小时AMP数据集，实验表明SoundMHPE优于基线模型。

Conclusion: 首次证明了仅使用声学信号估计多人3D姿态的可行性，为声学感知领域开辟了新方向。
...
TL;DR: InterSing是一个用于生成二重唱表演中逼真3D头部动画的框架，通过引入交互logits（交互逻辑值）来表示表演者之间的互动强度，并结合弱监督学习和交互感知扩散模型，实现独立动作、协调行为和它们之间平滑过渡的统一多模式生成。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动动画方法和对话交互模型无法充分捕捉二重唱表演中稀疏且节奏依赖的互动结构（如乐句边界、同步节奏和问答段落），因为这些互动是稀疏的且依赖于节奏，需要在保持个体表现力的同时实现间歇性协调。

Method: 1）提出交互logits作为可解释的潜在表示，建模每个时间步的跨表演者参与度；2）使用弱监督学习交互logits；3）使用交互感知的扩散模型，由音频特征和交互动态共同驱动生成动画。

Result: InterSing生成的头部动画具有更强的协调性和音乐对齐性，同时保留了个别表演者的特征动作风格。该方法可推广到多歌手表演，并提供了对表演者互动时机和方式的直观控制。

Conclusion: 通过交互logits和交互感知扩散模型的结合，成功实现了二重唱乃至多歌手表演的逼真头部动画生成，解决了稀疏节奏依赖互动建模的难题，为音频驱动的人体动画提供了新的思路。
...
TL;DR: UnorthoDOS是一个新数据集和方法，用于在未进行正射校正的高光谱图像上直接训练机器学习模型进行甲烷检测，无需传统的事后正射校正和匹配滤波器处理，支持机载部署。


<details>
  <summary>Details</summary>
Motivation: 传统甲烷检测方法依赖正射校正来纠正几何畸变和使用匹配滤波器来增强羽流信号，这些步骤是为地面处理设计的，不适合机载执行，需要开发一种可以直接在未校正影像上进行检测的方法。

Method: 提出UnorthoDOS数据集和方法，使用U-Net模型直接在未正射校正的高光谱图像上进行训练。模型采用FP16压缩以实现机载部署。

Result: 在未校正数据上训练的U-Net模型性能接近在正射校正数据上训练的模型（IoU 16.91% vs 18.47%），两者都显著优于mag1c匹配滤波器基线（IoU 4.76%）。FP16压缩使模型大小减半，输出偏差小于0.3%。

Conclusion: UnorthoDOS方法可以在未进行正射校正的高光谱影像上实现有效的甲烷检测，为机载实时检测提供了可行性，同时公开了训练好的模型和数据集以促进研究。
...
TL;DR: TourPhysics是一个在线框架,从单张图像和声明性物理配置初始化,将确定性模拟与视频生成相结合,通过分离模拟器状态、几何证据、生成器控制和外观记忆的角色,实现持久探索和操作.


<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型主要由外观先验驱动，在长时序中往往失去物理或空间一致性。相机运动揭示新表面,而干预改变物体运动、接触和变形,需要区分观察与物理干预。

Method: TourPhysics将模拟器几何(用于投影和可见性)与用于条件生成器的相对深度分离,通过参考锚定记忆检索通过几何跨视图对应接受的静态外观,并通过有界残差在不存在有效对应时回归本机路径。

Result: 在模拟器定义的相机漫游和物体操作任务中,TourPhysics比评估的基线更紧密地遵循预定的相机和物体轨迹,保留输入场景,并在长时序重访时减少外观漂移。

Conclusion: TourPhysics成功地将有限物理 grounding的视频合成扩展到持久探索和操作,在保持物理一致性和减少外观漂移方面优于现有方法。
...
TL;DR: 本文提出使用单一预训练扩散概率模型同时承担自动驾驶开发中的两个角色：作为自我运动规划器和作为可控的安全关键场景生成器用于压力测试。规划方面提出SSDS扩散Transformer解码器，通过联合注意力融合场景上下文；提出DAPSE无训练引导方案将任意能量函数注入干净样本层。同时利用同一扩散模型生成安全关键场景进行闭环评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分利用扩散模型的多模态、交互丰富特性；规划与场景生成通常需要独立模型；需要统一框架同时提升规划性能并进行系统性的鲁棒性评估

Method: 1) SSDS扩散Transformer解码器：单流双流架构，通过联合注意力而非迟交叉注意力融合场景上下文；2) DAPSE解耦退火后验采样：无需训练的引导方案，在干净样本层面注入能量函数，避免一阶近似误差；3) 推理时引导：将选定代理引导至安全关键行为

Result: 在nuPlan上实现更强的闭环规划性能；生成的挑战性场景（激进切入、前车制动、组合纵横向交互）暴露了标准基准下隐藏的失败模式；SSDS规划器虽基准性能更强，但在挑战场景下退化更严重

Conclusion: 单一学习交通先验可同时改善运动规划并提供系统化规划器鲁棒性评估的现实框架；基准优势不一定转化为鲁棒性
...
TL;DR: LensStyle是一个统一的可控风格化镜头效果渲染框架，通过Dual-Path Controller将连续光学参数调制与离散镜头风格条件分离，支持圆形、多边形、甜甜圈、猫眼和星芒等多种风格化镜头效果渲染。


<details>
  <summary>Details</summary>
Motivation: 现有镜头效果渲染方法主要关注小光圈到大光圈模糊过渡的准确模拟，忽视了镜头效果的风格化方面，无法在大光圈下产生多样化的散景效果，也无法捕捉小光圈下出现的星芒等独特摄影现象。

Method: 提出Dual-Path Controller解耦连续光学参数调制（如焦距、模糊强度）与离散镜头风格条件（如圆形、多边形、甜甜圈、猫眼、星芒效果），在统一框架内实现细粒度、可解释的物理基础镜头操控。构建了MultiLens数据集，包含在真实光学约束下合成的多镜头图像对用于模型训练。

Result: 实验表明LensStyle在真实感、可控性和美学质量方面优于现有镜头效果渲染方法和基于扩散的图像编辑模型。

Conclusion: LensStyle推动了计算摄影向多镜头风格模拟的发展，为镜头美学建模提供了统一的框架。
...
TL;DR: 本文提出PriorEdit3D框架，通过从强大的基础模型中提取视觉、语义和几何知识，实现无需配对3D监督的前馈3D编辑。该方法利用2D图像编辑模型的视觉先验和视觉-语言模型的语义先验，并通过3D感知分布匹配正则化解决几何崩溃和多视角不一致问题。


<details>
  <summary>Details</summary>
Motivation: 3D编辑面临高质量配对训练数据严重稀缺的问题。现有方法依赖测试时优化或通过复杂管道构建伪配对数据，常导致结构漂移和几何伪影。

Method: 提出生成先验蒸馏(Generative Prior Distillation)框架，通过可微渲染管道使用两个互补信号监督3D表示：1)图像编辑模型在主编辑视角的2D视觉先验；2)视觉-语言模型在新视角的语义先验。引入3D感知分布匹配正则化，在3D潜在空间中约束编辑输出保持在由图像到3D教师模型定义的现实3D资产流形内。

Result: 实验表明该方法在指令保真度和跨视角一致性方面优于现有最先进基线方法。

Conclusion: 无需配对3D监督即可实现高质量前馈3D编辑，通过蒸馏基础模型知识结合3D感知正则化可有效解决几何崩溃和多视角不一致问题。
...
TL;DR: 本文提出MCPO方法，通过两步压缩策略将多模态思维链长度缩减69.5%，同时保持原有精度并实现3.34倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型的长思维链推理面临高计算成本和KV-cache压力，现有的CoT压缩方法缺乏细粒度跨模态约束，容易导致视觉惰性和幻觉推理问题。

Method: 提出两阶段长度压缩方法：1)压缩阶段使用步级NCMI剪枝算法，通过对比有图/无图上下文推理差异自动识别并移除视觉无关推理步骤；2)对齐阶段先进行监督微调实现领域自适应初始化，再使用非对称多模态长度控制偏好损失进行优化。

Result: 在Qwen3-VL-Thinking等主流模型上，CoT长度缩减高达69.5%，端到端推理速度提升3.34倍，同时保持原有精度。

Conclusion: MCPO通过创新的跨模态对比偏好优化，仅需少于900个训练样本即可实现高效压缩，为多模态推理效率优化提供了新思路。
...
TL;DR: 本文挑战了多专家模型在长尾学习中依赖专家多样性的传统假设，提出VICAL框架，认为多专家模型更受益于方差减少而非多样性最大化。该框架通过邻域一致性对齐来改进长尾识别，包含自一致学习和深度集成蒸馏两个关键组件。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为多专家模型的优势来自专家多样性，但作者重新审视这一核心假设，发现由logit调整或显式正则化诱导的多样性并不能保证更好的集成准确率。作者认为多专家模型实际上更受益于方差减少。

Method: VICAL框架包含两个关键组件：1) 自一致学习(Self-Consistency Learning) - 减少对不稳定高频信息的依赖，平滑局部损失景观，减轻尾部类别过拟合；2) 深度集成蒸馏(Deep Ensemble Distillation) - 使用低分辨率视图促进跨专家低频语义一致，从而避免与已有知识的优化冲突。

Result: 在CIFAR-LT、ImageNet-LT和iNaturalist 2018数据集上的广泛实验表明，VICAL始终优于最先进的方法，验证了一致性驱动设计的有效性。

Conclusion: 多专家模型在长尾学习中的优势主要来源于方差减少而非多样性最大化，通过邻域一致性对齐方法可以有效提升长尾识别性能。
...
TL;DR: MINT是首个能够从单目RGB视频直接生成世界空间双手轨迹的基础模型，通过共享时空表示联合预测相机轨迹、手部状态和存在性，并利用大规模伪标签预训练和小样本微调策略解决标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 从第一人称视频中恢复相机和手部运动对于活动理解、机器人学习和增强现实至关重要。现有方法将该问题分解为多个独立阶段（相机运动估计、深度估计、手部重建、轨迹优化），导致计算开销大且无法联合建模相机和手部运动。

Method: 提出MINT模型，采用单一共享时空视频表示联合预测相机轨迹、相机帧手部状态和每帧手部存在性，通过显式坐标变换输出世界空间手部运动。为解决训练数据稀缺问题，开发了EGOPIPELINE开源标注管道将大量公共egocentric视频转换为结构化监督信号，采用"大规模伪标签预训练+小规模高质量标注微调"的两阶段训练策略。

Result: 在公共基准测试中，MINT在世界空间手部轨迹精度、相机轨迹估计精度方面均取得显著提升，推理速度大幅超越现有方法，并展现出对未见数据集的零样本泛化能力。

Conclusion: MINT作为首个端到端世界空间双手轨迹预测基础模型，通过伪标签策略有效解决了数据稀缺问题，为egocentric视频理解提供了新的baseline。发布了模型代码、标注管道及1,021小时的大规模轨迹数据集。
...
TL;DR: 本文提出了一种基于 resection 思想的新型跨视角定位方法 ARC-Loc，通过将地面关键点转换为卫星图上的方位射线，利用射线收敛原理实现直接匹配定位，无需依赖 BEV 变换或外部深度模型。


<details>
  <summary>Details</summary>
Motivation: 现有 CVL 方法依赖 BEV 变换或 2D 到 3D 提升，但从单张地面图像重建 3D 结构本身是病态问题，会导致几何畸变和计算开销；使用外部深度模型则引入延迟和噪声预测问题。

Method: 提出 (i) 将地面关键点转换为卫星图上的方位射线，(ii) 射线应在用户位置收敛。引入最小 Azimuthal Ray Convergence (ARC) solver 识别交点，以及 ARC loss 优化匹配网络，实现直接线到点对应。

Result: 在 VIGOR 和 KITTI 数据集上，ARC-Loc 与最新方法相比保持了竞争力的定位精度，同时具有更快的推理速度和更高的内存效率。

Conclusion: ARC-Loc 消除了对计算密集的 BEV 变换和外部深度模型的依赖，实现了高效推理；其显式特征匹配与现有框架兼容，具有良好实用性。
...
TL;DR: RefDiT是一种新型参考引导图像生成框架，通过属性级分解和LoRA适配器实现局部区域级别的引导，解决了现有方法在复杂场景中无法有效定位和利用局部属性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有个性化模型和风格迁移方法在处理包含多个对象的复杂现实场景时存在局限性。这些方法仅使用全局级引导，无法定位参考图像中的相关元素，且单一标识符令牌缺乏属性级控制能力。

Method: RefDiT采用局部区域引导策略，从参考图像中提取局部元素的属性。具体包括：1)对标识符令牌进行属性级分解，生成属性感知的条件信号；2)在推理提示中进行上下文调整；3)训练DiT模型中的低秩适配器(LoRA)块。通过学习标识符令牌与参考图像中局部区域之间的对应关系，实现更有效的局部引导。

Result: RefDiT能够从参考图像中提取局部属性并进行精细化控制，实现基于局部属性的图像生成指导。

Conclusion: RefDiT通过在DiT架构中引入属性级分解和LoRA适配器，实现了更精细的参考图像引导生成。该方法解决了现有方法无法处理复杂场景的问题，为参考引导图像生成提供了新的思路。实验结果表明该方法能够有效捕捉局部属性并生成符合要求的图像。
...
TL;DR: 本文提出了一种基于时序残差神经辐射场(Temporal Residual Neural Radiance Fields)的动态场景人体3D建模方法，能够有效处理复杂时空信号，实现新视角渲染和新姿态合成。该方法在保持与Anim-NeRF和Neural Body相似精度的同时，实现了近780倍的时间效率提升。


<details>
  <summary>Details</summary>
Motivation: MLP在静态场景人体重建方面取得良好效果，但在动态场景重建中存在容量限制问题，需要大量训练时间和计算资源，且重建质量受到显著约束。

Method: 1. 构建时序残差场来处理视频序列中的时序信号（与MLP架构无关）；2. 提出集成方法减少可训练参数并加速渲染，增强网络特征表示能力；3. 设计多维损失函数准确预测空间像素值与实际值的损失。

Result: 实验结果表明，与最新代表性方法相比，本文方法在PSNR和SSIM指标上有所提升；与Anim-NeRF和Neural Body保持相似精度的同时，实现了近780倍的时间效率提升。

Conclusion: 该方法能够有效处理动态场景中的复杂时空信号，在人体3D建模中实现了效率与精度的良好平衡。
...
TL;DR: PRISM是一种用于条件医学图像生成的组合奖励模型框架，通过分层约束传播(HCP)机制将图像质量分解为多个验证器锚定的阶段，从低层属性(强度、纹理)到结构对齐再到高层语义保真度，逐步评估图像正确性。


<details>
  <summary>Details</summary>
Motivation: 获取高质量注释的医学图像数据成本高、耗时长且需要领域专业知识；现有条件扩散模型无法捕捉细粒度属性和语义一致性；基于强化学习的微调方法仅依赖单一标量奖励，无法提供有效的纠正信号。

Method: 提出PRISM框架，包含：1)组合奖励模型(CRM)将图像质量分解为多个阶段评估器，分别评估低层属性、结构对齐和高层语义保真度；2)分层约束传播(HCP)机制确保先解决低层缺陷再获得高层奖励，防止简单目标掩盖关键失败。

Result: 在三个数据集上验证：PanNuke多类细胞分割mDice提升2.3%，CeDeM肠绒毛/隐窝检测Mean Relative Error(MRE)降低8.5%，ISIC皮肤病变分类F1提升5.9%。

Conclusion: PRISM通过分层奖励机制有效提升了条件医学图像生成质量，所生成数据用于训练下游模型可显著改善分割、检测和分类任务的性能。
...
TL;DR: 本文提出了PuTR-CouT，一个基于Transformer学习关联机制的计数-追踪框架，用于相机陷阱图像中的序列级动物计数。该方法通过利用静态背景和短时间爆发等结构先验知识，弱监督地生成伪追踪标签来解决标注数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 物种识别在相机陷阱图像中已被广泛研究，但物种丰度或密度估计等关键生态建模任务还需要计数个体动物。然而，大多数数据集中缺乏计数标签，且帧率较低（通常约1帧/秒），使得序列级追踪和计数估计特别具有挑战性。

Method: 提出PuTR-CouT框架，使用基于Transformer的 learned association mechanism 进行跨帧检测关联，生成合成训练数据。同时改进了iWildCam 2021挑战赛顶级方案使用的MaxBoxCount启发式方法作为强基线。

Result: 在iWildCam 2021基准测试中，PuTR-CouT框架与改进的MaxBoxCount相比取得了竞争力的计数结果，并具备多物种预测和追踪级验证的额外能力，实现了截至目前最高的分数。

Conclusion: 通过结合Transformer追踪机制和弱监督伪标签生成策略，PuTR-CouT能够有效解决相机陷阱图像中动物计数的挑战，为多物种监测提供了新的解决方案。
...
TL;DR: 本文提出了一种基于对数B样条时间编码的置信度归一化连续多时间尺度表示方法，结合几何感知的局部置信度机制，用于事件相机中的目标检测。该方法可直接在事件表示中编码时间信息，无需依赖循环架构即可实现高效的特征提取。


<details>
  <summary>Details</summary>
Motivation: 自主系统需要在快速变化的场景动态和挑战性照明条件下实现稳健的低延迟感知。当前事件相机目标检测通常依赖循环架构来累积稀疏的时间信息，但循环架构计算开销大且延迟高。

Method: 提出两种核心创新：1) 基于对数B样条的时间编码结合置信度归一化的连续多时间尺度表示；2) 利用事件生成空间结构的几何感知局部置信度机制。同时引入递归指数多项式近似以实现高效的事件逐事件更新。采用固定的EventCenterNet前馈检测器进行评估。

Result: 在PEDRo和Gen1数据集上，所提出的表示方法在性能上持续优于紧凑的CSTR表示。递归指数多项式近似在大幅保持检测性能的同时实现了高效的事件更新。

Conclusion: 精心设计的事件表示可以捕获循环时间建模学习的大部分时间信息，为高效前馈、事件驱动和未来神经形态目标检测提供了有前景的基础。
...
TL;DR: 本文提出了一种用于弱监督视频异常检测的自适应时间建模框架，解决了现有基于MIL的方法无法很好处理异常持续时间和时间动态变化的问题。该框架包含三个核心模块：时间精炼模块(TRM)、自适应事件分割模块(ESM)和基于自适应相似度的融合策略。


<details>
  <summary>Details</summary>
Motivation: 现有弱监督视频异常检测方法大多采用多示例学习框架，依赖刚性手工时间先验来监督异常评分，但这种方法对现实世界中观察到的异常持续时间和时间动态的广泛变化适应性有限，导致_snippet_级别的预测不稳定或不可靠。

Method: 提出三个关键技术：1) 时间精炼模块(TRM)利用动态位置编码和可学习类令牌建模长程时间依赖，同时提炼稳定的全局视频级表示；2) 自适应事件分割模块(ESM)通过时间不连续性分析识别事件边界，将_snippet_特征聚合成判别性事件级表示；3) 基于自适应相似度的融合策略动态整合异常分数到视频级预测，替换固定top-k聚合启发式方法。

Result: 在两个基准数据集上的广泛实验表明，该框架持续优于最先进的方法。

Conclusion: 该自适应时间建模框架能够有效处理不同频率和持续时间的异常事件，通过多时间粒度建模和动态融合策略提升了弱监督视频异常检测的性能。
...
TL;DR: 本文提出了一种无需训练的工业异常检测方法，通过正常图像统计进行校准，将计数能力引入训练-free框架，统一检测结构异常和逻辑异常。


<details>
  <summary>Details</summary>
Motivation: 现有检测器通常只能处理结构异常或逻辑异常中的一类。训练-free方法缺乏对象计数概念，而基于计数的方法通常依赖类别特定的组件建模。需要一种方法无需额外训练或部件级监督即可引入计数能力。

Method: 提出正常集校准(normal-set calibration)，利用正常图像统计对齐异构异常线索，实现统一训练-free框架内的直接融合。结合互补的冻结特征线索同时处理结构和逻辑异常。

Result: 在MVTec-LOCO上，逻辑异常图像级AUROC达89.0，结构异常达95.9，平均92.5，为训练-free方法中最优。在MVTec-AD上结构变体匹配PatchCore达99.1图像AUROC。

Conclusion: 通过正常集校准可将计数能力引入训练-free异常检测，无需额外训练或部件监督即可同时处理结构和逻辑异常，且该校准方法具有良好的泛化能力。
...
TL;DR: 本文提出了一种基于图模型的3D语义地图后处理校正方法，能够在保持地图几何和占用信息不变的情况下，仅通过局部几何和邻域语义信息来修正体素标签错误。该方法在四个独立训练的2D分割模型生成的OccuFly地图上将mIoU提高了4.23-5.00个百分点。)


<details>
  <summary>Details</summary>
Motivation: 自动构建的语义3D地图中存在感知和建图流程产生的错误，这些错误会降低地图对下游自主任务的可靠性。现有方法存在局限性：依赖原始观测、将占用作为预测问题的一部分，或使用非学习的局部正则化。本文提出一个问题：是否可以在固定几何和占用信息的情况下，直接从完成的地图中恢复语义准确性。

Method: 提出一种基于图的模型(\method)，根据局部几何和相邻语义信息校正体素标签。为了获取训练数据，根据上游地图中观察到的类别混淆，对标注好的OccuFly地图的连续区域进行破坏和污染。在由四个独立训练的2D分割模型生成的已完成OccuFly地图上评估该方法。

Result: 该方法在mIoU上持续提升4.23-5.00个百分点，提升分布在评估的语义类别中，其中对树、屋顶和墙壁的改进尤为显著。在独立重建的分布外 aerial 场景上的结果表明，学习到的校正可以迁移到训练中未见过的环境。

Conclusion: 通过图模型利用局部几何和邻域语义信息进行后验语义纠正是可行且有效的。该方法能够在不依赖原始观测的情况下提升3D语义地图的准确性，且具有跨环境和跨模型的泛化能力。
...
TL;DR: WeAgent-MMGenEdit是一个用于知识密集型图像生成和编辑的智能体系统，针对现有方法在外部世界知识、视觉验证和证据整合方面的不足，提出了包含多模态 harness、可扩展数据构建流程、基准测试和后训练方法的完整解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成和编辑模型在处理需要外部世界知识的提示时表现不可靠，受限于参数化知识的边界和长尾分布问题。现有智能体方法存在视觉验证不足、政策模型过载以及文本和视觉证据整合薄弱等缺陷。

Method: 1) WeAgent-Harness：多模态运行时环境，具备持久性证据管理和专用验证整合工具；2) 可扩展的提示合成和智能体轨迹收集流程，生成23K监督轨迹和14.7K强化学习任务；3) WeBench-MMGenEdit：双语基准测试，涵盖知识密集型图像生成和多图像编辑；4) 基于SFT和RL的双面后训练方案，改进智能体策略和图像后端。

Result: WeAgent-MMGenEdit使30B总参数/3B活跃参数的政策模型得以超越同规模模型，并接近1T参数智能体的性能。

Conclusion: 该研究提出了一个完整的端到端解决方案，有效解决了知识密集型图像生成和编辑中的外部知识利用和视觉验证问题，通过多模态证据管理和后训练方法显著提升了系统性能。
...
TL;DR: 本文提出了一种基于信息瓶颈(IB)框架的自解释多模态诊断方法，通过识别每种模态中最具信息量的元素来同时优化预测性能和模态特定的可解释性。实验表明该方法在多个医学数据集上表现优异，在iCTCF数据集上准确率绝对提升9.1个百分点。


<details>
  <summary>Details</summary>
Motivation: 可解释性是AI医学诊断的关键需求，尤其在安全关键的临床决策中。现有的医疗可解释性方法多为事后分析，且主要针对单模态数据设计，限制了其在日益普及的多模态诊断场景中的应用。

Method: 在信息瓶颈框架下形式化自解释多模态诊断问题，提出统一学习范式联合优化预测性能和模态特定可解释性。在假设编码器具有足够表达能力的条件下，采用基于矩阵的Renyi α阶熵泛函实现可处理且稳定的优化。

Result: 在涵盖异构模态的代表性医学数据集上进行了广泛实验，提出的方法持续获得较强的诊断性能，在iCTCF数据集上实现了9.1个百分点的绝对准确率提升。

Conclusion: 学习得到的解释提供了透明且模态感知的特征相关性洞察，从而提高了可解释性和泛化能力。
...
TL;DR: 本文提出CIT方法，通过整合免疫拓扑特征增强病理基础模型，实现跨癌症的MSI-H预测。在CPTAC-COAD和TCGA-STAD数据集上，零样本跨癌症迁移使TransMIL的AUC从0.6627提升至0.7161，绝对提升0.0534(p=0.003)。


<details>
  <summary>Details</summary>
Motivation: 现有病理基础模型虽在单癌症队列内表现良好，但跨癌症泛化仍面临挑战。原因在于不同器官具有特定的组织学和架构差异，需要一种器官不变性的表示方法来实现跨癌症泛化。

Method: 提出Conserved Immune Topology(CIT)方法：1)使用无监督聚类识别免疫相关瓦片；2)从基础模型嵌入和瓦片坐标中编码三级淋巴结构、肿瘤周围免疫反应、多尺度肿瘤浸润淋巴细胞密度和免疫-肿瘤混合特征，无需标注或目标域数据。

Result: 零样本跨癌症迁移使TransMIL的AUC从0.6627提升至0.7161，绝对增益0.0534(p=0.003)，且在所有三种MIL聚合器中均取得一致改进。

Conclusion: 空间免疫拓扑特征提供了一种潜在的器官不变性表示，用于MSI-H预测，支持病理基础模型的跨癌症泛化。
...
TL;DR: 本文提出了一种统一的正则化BA框架，结合快速多视图匹配器和单目先验，可同时支持在线VSLAM和离线无序图像集合的三维重建，在性能和速度上优于传统方法、前馈方法和混合基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有混合SfM系统虽然结合了前馈3D重建的鲁棒性和传统BA的精度，但计算密集对应关系成本高，尤其在VSLAM等在线应用中存在时间限制，限制了其可扩展性和实用性。

Method: 引入正则化BA框架，利用快速多视图匹配器和单目先验进行初始化和正则化。该统一方法在同一优化框架内无缝支持VSLAM和离线重建，共享所有任务的超参数。

Result: 在VSLAM和离线重建两个领域的广泛实验表明，与传统、前馈和混合基线方法相比，性能和速度权衡得到改善。值得注意的是，未校准的VSLAM方法优于所有之前的校准方法。

Conclusion: 该方法成功实现了在同一优化框架下同时处理在线VSLAM和离线重建任务，且在未校准情况下取得最优性能，为混合SfM系统的实际应用提供了更高效的解决方案。
...
TL;DR: 本文研究了多模态大语言模型（MLLMs）在复杂结构化用户需求场景下的推理能力，发现现有模型存在严重失败问题，并提出FTF-rl方法来提升需求感知的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs作为自主代理的潜力受到广泛关注，但现有研究较少探索代理满足用户复杂结构化需求的场景。作者旨在填补这一空白，评估模型在三类需求场景下的表现。

Method: 构建了3,649个反映现实服务场景的问题（包括电商、预订、地图和叫车服务），评估了最先进的MLLMs，并提出First Things First Reinforcement Learning (FTF-rl)方法显式优化多优先级用户需求的推理。

Result: 现有MLLMs在所有场景下都表现出灾难性失败，经常误解任务要求、违反必须满足的需求、产生无效解决方案。FTF-rl方法显著提升了任务成功率，并在LogicVista、MathVision和InfoQA等逻辑数学推理任务上展现普遍有效性。

Conclusion: 增强需求感知推理能力是提高MLLM代理泛化能力的简单有效途径。FTF-rl为改进MLLMs处理复杂结构化需求的能力提供了新方向。
...
TL;DR: 本文提出Measured Sliders框架，通过可微分的图像测量来定义连续控制，解决了传统扩散滑尺因使用文本或学习表示而导致属性尺度与可观察图像属性脱节的问题。该框架在训练前使用可观测性测试识别可用监督，训练中通过测量引导目标移动并抑制非目标变化，训练后进行解码校准以可比较的单位表达控制。实验在SDXL和FLUX.1-dev上验证了光照方向、属性选择性和组合性等指标。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型的连续滑尺控制存在三个核心问题：1）无法预先知道哪些属性是可学习的；2）无法直接比较不同控制的强度；3）无法预测多个控制组合时的干扰问题。这些问题源于现有方法的控制轴来自文本或学习表示，其尺度与可观察的图像属性断开连接。



Method: 提出Measured Sliders框架，包含三个关键阶段：1）训练前：可观测性测试识别可用监督；2）训练中：测量引导的目标函数学习目标移动同时抑制非目标变化；3）训练后：解码校准将控制表达为可比较的图像变化单位。技术实现上使用多个LoRA分支存储在一个checkpoint中，可在联合激活上组合而不需额外训练。



Result: 在SDXL和FLUX.1-dev上：光照方向达到rho=0.995和98.9%单调扫描率；五属性checkpoint平均选择性2.59（最强基线为1.50）；96.7%的成对组合和86.1%的三重组合保留请求方向；可观测性测试成功分离所有后续成功的测量与失败候选。



Conclusion: 图像空间测量为学习、诊断、校准和组合连续生成控制提供了共同基础。该框架使控制具有有序性、选择性和可组合性，为扩散模型的连续控制提供了一种更可预测和可解释的方法。
...
TL;DR: 提出利用视觉语言模型(VLM)作为诊断代理，在无目标域标签情况下通过视觉诊断识别失败模式并推荐参数更新，实现跨域跟踪模型的自适应调优。


<details>
  <summary>Details</summary>
Motivation: 解决无标签目标域的检测-跟踪pipeline适应问题。传统方法依赖目标域标注或手工设计的代理目标，但在域偏移大时表现脆弱。

Method: 构建了一个迭代调优循环：1)VLM直接检查渲染的跟踪输出；2)识别视觉失败模式；3)推荐参数更新。核心创新在于不依赖标注指标进行优化，而是让VLM进行视觉诊断。

Result: 在MOT17->MOT20任务中，VLM调优器恢复了67.8%的性能损失，与目标域上限仅差0.029 HOTA；在最高密度序列上恢复达86.7%。对比实验显示：1)源域oracle配置反而使HOTA下降0.090；2)无标签贝叶斯优化在域偏移大时性能下降。

Conclusion: VLM tuner在域偏移明显体现在检测级参数的情况下表现优异，能恢复67.8%的性能损失；但在源域 oracle 已接近最优的场景（如MOT17->DanceTrack）效果有限。方法的核心优势在于选择性调整——当视觉诊断未发现明显失败模式时保持原配置。
...
TL;DR: 提出一种新的少样本动作识别方法HML-FSAR，通过空间增强模块和层次度量学习策略来改进少样本动作识别性能。该方法在五个数据集上验证有效。


<details>
  <summary>Details</summary>
Motivation: 现有少样本动作识别方法仅在网络输出层应用单原型监督，未能充分利用视频中跨帧的全局空间信息。已有的多层次度量方案仅对中间层施加并行原型约束，缺乏沿完整特征流水线的渐进监督，导致学习到的类原型泛化能力有限。

Method: 1. 提出空间增强模块捕获跨帧全局空间表示；2. 结合时序MHA、异构对齐、时空特征融合和字典学习模块构建完整特征处理流水线；3. 嵌入层次度量学习(HML)策略，包含中心度量、对齐度量、对比度量、字典度量和原型度量，从帧级表示到最终类原型施加渐进多阶段互补约束。

Result: 在五个广泛使用的FSAR数据集上验证，实验结果充分证明了其有效性。

Conclusion: HML-FSAR方法通过层次度量学习策略联合优化特征紧凑性、异构时空对齐、类间可分性和抗噪声鲁棒性，显著提升了少样本动作识别的性能。
...
TL;DR: 本文比较了3D Gaussian Splatting中用于建模视角相关外观的不同球面外观模型（SH、神经表示等），提出了一种使用紧凑潜在码和小型共享MLP的隐式替代方案，在质量和效率上优于传统SH方法。


<details>
  <summary>Details</summary>
Motivation: 低阶球谐函数(SH)系数虽计算高效，但占用大量存储和内存带宽，且其带限基函数限制了角度细节表达。需要寻找更紧凑高效的外观建模方案。

Method: 将所有球面模型集成到同一优化流程中，融合前后向传播为可微CUDA光栅化器，并提供便携式WebGL查看器；在多个场景上评估重建质量、内存使用、优化和渲染性能。

Result: 神经表示最紧凑，每原语外观占用从192字节降至28字节，优化速度提升1.3倍，重建质量更优。球面模型整体质量效率权衡最佳。

Conclusion: 球面模型提供最佳质量效率权衡，神经表示是最紧凑方案。本框架和分析为SH替换提供了实践指导。
...
TL;DR: 本文提出TSR-ITNR框架，用于高光谱与多光谱图像融合，通过两阶段重建（隐式张量神经表示学习+无参数校正）实现无监督高分辨率高光谱图像重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示的融合方法未能充分捕获细粒度空间结构和丰富光谱依赖性，且低分辨率高光谱图像和高分辨率多光谱图像的互补信息未被充分利用。

Method: 提出两阶段框架：阶段1学习隐式Tucker表示并细化低秩空间系数张量和光谱基，结合预训练去噪器提供深度先验；阶段2通过无参数校正从两个观测源获取互补和非干扰校正。

Result: 在多个基准数据集上实现优异的定量、视觉和光谱重建性能，并在下游语义分割任务中验证有效性。

Conclusion: 理论分析证明了光谱细化的几何保持性和校正的正交互补性，TSR-ITNR无需真值监督即可实现高质量重建。
...
TL;DR: AdaGate-DF是一种自适应门控深度伪造检测框架，利用图像质量线索通过双多出口系统路由样本，使高质量图像能够提前退出以节省计算资源。该方法在Celeb-DF上达到0.9370的AUC，在384x384分辨率下达到0.9708，展示了检测性能与计算效率的实际平衡。


<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测模型依赖高质量输入、固定推理路径和计算密集型架构，限制了其在低分辨率和资源受限环境中的使用。

Method: 提出AdaGate-DF框架，使用图像质量线索将样本路由通过双多出口系统，使高质量图像可以提前退出以节省计算。

Result: 在Celeb-DF上达到AUC 0.9370，优于MaD-CoRN和DefakeHop++；在384x384分辨率下达到AUC 0.9708；在FaceForensics++上展现出处理类别不平衡的有效性。

Conclusion: AdaGate-DF在可变质量的深度伪造检测中展示了检测性能、不确定性感知预测和计算效率之间的实际平衡。
...
TL;DR: 首次提出用于整张幻灯片图像中孢粉（palynomorph）自动检测的可扩展端到端管道，解决手动分析费时费力的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 孢粉是古气候和古生态系统研究的重要高分辨率记录，但现有方法依赖手动分析高分辨率、多焦点的数字显微镜图像，速度慢、耗时长，限制了研究的规模。

Method: 1）将数字化多焦点显微镜切片图像分解压缩为可处理的二维瓦片；2）基准测试现代目标检测模型（包括RF-DETR），达到AP@50为0.879；3）开发大规模高分辨率图像检测结果综合算法；4）I/O优化以加快推理速度。

Result: 单个切片的孢粉检测时间从通常需要数天的手动检查缩短到不到一小时的自动分析。

Conclusion: 该方法能够显著扩大孢粉学研究的规模，使大规模自动化分析成为可能。
...
TL;DR: 本文提出了一种统一的Vision Transformer压缩框架，结合Hessian-Balanced Adaptive Block Pruning (H-BAC)、量化和基于注意力的知识蒸馏，用于印度辣椒病害检测，在资源受限设备上实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 辣椒是印度重要经济作物，但病害识别困难。Vision Transformer(ViT)虽然分类精度高，但计算量大，难以在资源受限设备上部署。现有压缩方法孤立处理剪枝、量化或蒸馏，缺乏对组合应用效果的系统探索。

Method: 提出统一压缩框架：首先通过对照消融研究独立评估各压缩技术（H-BAC二阶敏感度引导的剪枝、量化、注意力蒸馏），然后将最佳组件集成到针对农业实际约束的顺序部署流程中。在3类辣椒村庄分割数据集上测试，包含跨村庄、跨设备的分布外测试集。

Result: 压缩模型达到或超过95.13%的FP32基线精度，模型大小减少74-98%。完整集成压缩管道实现54.5倍缩减（327.42MB至6.01MB），在四种配置下达到95.13±2.32%精度。但直接训练的同等大小学生模型（6.01MB INT8）也达到94.87%精度。

Conclusion: 压缩框架可实现显著模型缩减且保持精度，但与直接训练同等规模模型相比，H-BAC和知识蒸馏的计算成本收益需权衡。未来需进一步探索压缩技术间的协同效应。
...
TL;DR: MEOX是一种用于地球观测的多模态掩码自编码器，在紧凑参数预算（311万参数）下实现强大的传感器灵活表示学习和任务迁移。


<details>
  <summary>Details</summary>
Motivation: 当前地球观测表示学习需要适应异构传感器和缺失观测，通常通过更大的架构实现。该研究旨在用紧凑的模型实现高效的 multimodal 学习。

Method: 提出MEOX模型，包含：传感器特定适配器、显式有效性信号、共享稀疏专家块、学习的patch-wise融合、14个额外编码器块、元数据token、共享专家投影与私有低秩残差、旋转注意力。使用模态平衡掩码重建和结构化传感器dropout进行预训练。

Result: 在64像素下 cashew 分割达到64.42% mIoU，224像素下 EuroSAT 达到90.56%平均准确率，BigEarthNet微调达到72.95%微平均精确度。元数据带来0.64个百分点提升。

Conclusion: MEOX在紧凑参数预算下实现了传感器灵活的表示学习和强大的任务迁移能力，超过了此前报告的CSMoE结果。
...
TL;DR: Ref-GeNVS是一种无需训练的反射感知生成新视角合成方法，通过将镜子图像视为两个互补视图，估计镜子平面并反射相机姿态形成虚拟视图，采用两阶段生成方法（镜像门控注意力和反射注入）实现反射一致的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有multi-view diffusion模型无法识别场景中的镜子，无法利用反射内容进行场景生成，且不需要额外训练即可实现反射感知的场景生成。

Method: 1）估计镜子平面并反射相机姿态形成虚拟视图；2）提出两阶段生成方法：Mirror-gated attention（镜像门控注意力）和Reflection injection（反射注入）；3）显式利用multi-view diffusion模型中的反射关系。

Result: 在合成和真实场景中表现优于最近的生成式NVS方法，能够生成反射一致且上下文连贯的新视角，揭示出仅通过镜子可见的场景结构。

Conclusion: Ref-GeNVS继承了multi-view diffusion backbone的强大泛化能力，无需微调即可实现反射一致的新视角合成。
...
TL;DR: 提出了一种神经符号框架，通过紧密耦合视觉语言模型(VLM)进行一阶逻辑规则归纳与动态逻辑张量网络(D-LTN)进行可微分规则验证，在闭环迭代反馈中完成视觉推理任务。


<details>
  <summary>Details</summary>
Motivation: 视觉推理任务需要系统同时感知视觉内容并应用形式化关系约束，而纯神经或纯符号方法都无法单独很好地处理这一组合。

Method: VLM接收少量标注视觉示例并提出符合严格语法的一阶逻辑候选规则；D-LTN根据CNN产生的视觉嵌入自动组装并验证这些规则；验证失败反馈指导VLM下一次假设，形成"思考-验证-修订"的闭环迭代。

Result: 在ViSudo-PC基准上（MNIST、EMNIST、KMNIST、FMNIST四个视觉域），仅用3个训练示例作为视觉上下文即可归纳有效的数独约束规则，AUC分数达到或优于之前的方法（NeuPSL、LTN）。

Conclusion: 展示了通过视觉语言模型自动发现规则的潜力，为神经符号视觉推理提供了新思路。
...
TL;DR: 本文提出一种自监督多视角深度估计方法，通过相机感知射线嵌入和跨图像注意力机制解决跨图像不一致性问题，在自动驾驶数据集上取得了显著的深度估计性能提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要对周围环境进行可靠的3D理解。多视角环绕相机提供广泛的场景覆盖，但空间相邻图像重叠最小，大多数像素的深度必须从单目外观线索推断。这些线索在不同图像中可能表现不同，导致深度估计模型产生不一致的解释。

Method: 提出两种关键技术：一是引入逐像素相机感知射线嵌入，使网络能够考虑单目线索的相机依赖性变化；二是通过基于校准 rigs 设置的几何可行区域约束的跨图像注意力，将每个像素的上下文扩展到其自身图像之外。模型基于光度一致性进行完全自监督训练。

Result: 在DDAD和nuScenes数据集上的评估表明，该方法在领域内和跨领域评估中均优于当前最先进的自监督方法，提高了整体深度精度和跨图像深度一致性。

Conclusion: 该方法在DDAD和nuScenes数据集上验证了在领域内和跨领域评估中均能提高整体深度精度和跨图像深度一致性，为自监督多视角深度估计提供了有效解决方案。
...
TL;DR: 本文主张XAI领域应从方法导向转向模型导向。作者认为过去十年积累的XAI工具箱（归因、特征可视化、概念和电路方法）已足够成熟，但领域重心应从构建方法转向评估模型本身的可解释性，包括模型表征和计算内容的表征与比较，以及人类实际理解模型的能力。


<details>
  <summary>Details</summary>
Motivation: XAI领域十多年来构建了众多解释方法，却几乎未关注这些方法所要回答的核心问题：模型到底有多可解释？随着模型演进我们是否取得进展？现有研究多聚焦于方法构建和比较，而忽视了模型本身的可理解性评估。

Method: 本文采用综述方式：(1)梳理现有XAI工具如何表征和比较不同模型的表征与计算；(2)探讨更困难的议题——评估非专家人类（依赖者、信任者和认证者）是否真正能理解模型；(3)借鉴计算神经科学方法论；(4)提出模型为中心的XAI研究议程。

Result: 作者指出当前对模型的比较研究极为薄弱，现有工具虽可支持模型表征和计算的特征化，但人类实际理解能力的测量仍被忽视。需建立以模型为中心的XAI议程，评估模型本身的进步而非方法的改进。

Conclusion: XAI领域需要范式转变：从关注解释方法转向关注被解释的模型。成熟的工具箱已能支持这一转变，但需将研究重心从"如何解释"转向"模型是否真正可被理解"，特别是非专家人类的理解能力测量。
...
TL;DR: 研究评估了一个用于脑龄预测的3D CNN预训练模型作为可复用基础模型的可行性，通过低秩适应(LoRA)方法仅用约1%的额外可训练参数适应下游神经影像任务，并在六个实验中验证了其跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在神经影像学中标注数据不足，且现有迁移学习效果不明确，特别是对于阿尔茨海默病研究；此外，转移的模型能否在新数据集上直接使用而不需要针对每个任务重新训练也不清楚。

Method: 使用一个包含718万参数的3D CNN预训练模型(用于脑龄预测)，冻结其全部权重，通过低秩适应(LoRA)方法仅添加约1%的可训练参数来适应下游任务。

Result: 实验1(ADNI认知正常vs痴呆分类AUC=0.964)；实验2(直接应用于OASIS-3无训练AUC=0.871)；实验3(联合年龄和认知分数区分稳定与进展性MCI AUC=0.828)；实验4(预测淀粉样蛋白阳性AUC=0.804)；实验5和6(估计海马体和白质低信号体积，R²分别为0.80和0.91)。

Conclusion: 一个经过精心训练的脑龄预测模型可以作为阿尔茨海默病相关任务的有效基础模型，即使在严格的数据约束下也能通过仅约1%的额外参数适应每个任务，并可转移到未见过的数据集进行零样本推理。
...
TL;DR: UniMate是一个统一的基础模型，能够从带骨骼绑定的3D资产和文本提示生成关节动作，无需测试时优化或每骨架微调，解决了现有学习动画器受拓扑约束的问题。


<details>
  <summary>Details</summary>
Motivation: 自动绑定技术可以大规模生成动画就绪的3D资产，但生成驱动这些资产的动画仍然是一个瓶颈。现有的学习动画器受限于拓扑：它们依赖类别特定模板或在推理时需要每骨架微调和参考动作。

Method: 提出拓扑感知扩散Transformer，通过三种机制将骨骼拓扑整合到注意力中：(1)来自成对关节关系和测地距离的图感知注意力偏置；(2)通过图拉普拉斯斯将RoPE推广到任意运动树的谱旋转位置嵌入；(3)从静止姿态骨骼注意力池化的全局拓扑调节器。

Result: UniMate在质量、泛化性和效率上优于最先进基线，支持零样本跨拓扑迁移、插值、扩展和文本引导编辑。

Conclusion: UniMate是一个统一的基础模型，可以为任意骨骼从带绑定的3D资产和文本提示合成关节动作，具有良好的泛化能力。
...
TL;DR: 本文提出一种从多视角观测生成分解式3D场景的方法，将包含数百个物体的杂乱场景表示为共享世界坐标系中的独立物体网格。通过扩展单物体3D生成模型Pixal3D，添加多视角 conditioning 路径，使模型能够在严重遮挡下生成分解式场景。无需场景级训练即可泛化到复杂场景，并引入UE-MeshyScene基准数据集进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理密集遮挡的杂乱场景时存在困难：基于几何的方法将场景重建为单一表示，遮挡区域几何不完整；而现有的分解式方法受限于简单场景。实际应用（游戏、AR/VR、仿真、机器人）需要将场景表示为独立物体网格的集合。

Method: 将强单物体3D生成先验（Pixal3D）适配到多视角观测。具体包括：1）添加多视角 conditioning 路径，将多个姿态观测信息融入物体生成；2）模型完全在单物体标准空间中进行微调，无需场景级训练即可泛化到大型场景。还可将生成的3DGS世界（如Marble和HY-World 2.0）转换为分解式网格场景。

Result: 在单物体、受控多物体和UE-MeshyScene评估中一致优于先前方法；场景复杂度越高、遮挡越严重，提升越大。UE-MeshyScene是首个包含数百个物体、逐物体标注和真实网格的密集杂乱场景 photorealistic 基准。

Conclusion: 证明分解式生成范式的可行性和可扩展性：仅使用单物体训练数据即可泛化到数百物体的复杂场景。这种范式可扩展到不同类型的3D生成模型，为复杂场景的3D重建和生成提供了新思路。
...
### [82] [At Equal Inference Cost, Multi-Agent Structure Does Not Beat a Single Frozen Agent](https://arxiv.org/abs/2609.04217)
*David Dylan,Aoife Brennan,Cian Murphy,Niamh O'Sullivan,Conor Kelly,Saoirse Walsh*

Main category: cs.MA

TL;DR: 研究在相同推理预算下，多智能体LLM团队是否真的优于单智能体。通过固定总语言模型调用次数，对比Planner-Executor-Critic团队与单智能体的演化效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究声称多智能体团队优于单智能体，但未控制计算成本。多智能体团队每次环境步骤需要多次模型调用，造成额外计算开销。作者想探究在相同推理预算下，多智能体结构是否仍具优势。

Method: 提出MA-Evolve方法，将Planner-Executor-Critic团队表示为三个可演化角色提示，通过共享冻结的7B骨干网络进行逐角色坐标上升优化。在ALFWorld和WebShop上实验，固定总语言模型调用次数进行对比。

Result: 在ALFWorld上，单智能体演化后得分0.754，多智能体团队最高均值0.769，但无统计显著性(p=0.80)。Leave-one-in分析显示价值完全来自执行器，规划器和批评器演化后几乎无影响。在2-3倍自由计算下，团队仅匹配单智能体；WebShop上演化无效，团队反而更差。

Conclusion: 在相同推理成本下，多智能体结构增加成本但无明显收益。执行器是核心角色，规划器和批评器在固定预算下未产生实际价值。
...
### [83] [Multi-dimensional Bias in Modeling Multi-dimensional Preferences: Evaluating the Ability of Synthetic Agents to Replace Human Participants in Conjoint Experiments](https://arxiv.org/abs/2609.04243)
*Ho Ting Hung,Nachiket Midha,Victor Y. Wu,Yiwen Zhang*

Main category: cs.MA

TL;DR: 研究探讨了大型语言模型(LLM)作为合成代理人在政治科学conjont实验设计中的有效性，发现LLM无法完全复现人类多维偏好模式，结果在不同维度和复制研究间不一致。


<details>
  <summary>Details</summary>
Motivation: 尽管学界对利用LLM增强鲁棒性或降低调查实验成本越来越感兴趣，但LLM在conjont设计(政治科学中日益流行的方法)中的功效仍未被探索。

Method: 复制已发表的conjont研究，从三个维度比较合成代理人与原始人类数据：表征对应性、推论对应性和程序稳定性。评估选择分布的对齐程度以及估计值的统计和实质相似性。

Result: 结果在不同维度和复制的研究之间参差不齐。复制图形或获得强符号一致性仅能证明相似的聚合输出，但不足以支持用合成代理替换人类受访者。

Conclusion: 合成受访者的有效性应被视为claim-dependent和分层的。在考虑将合成代理作为人类样本的稳健替代品之前，整个学科必须首先绘制出这一创新在各种水平上的边界。
...
### [84] [EVOHARNESSBENCH: Can Your Agents Keep Pace with an Evolving Harness?](https://arxiv.org/abs/2609.04280)
...
TL;DR: EVOHARNESSBENCH是一个用于评估LLM智能体在工具、技能和智能体构成的系统 harness 演变下的表现的新基准，解决了现有持续学习基准忽视harness本身变化的问题。该基准包含17条多阶段harness流、802个任务、520个工具、42个技能和62个智能体。实验揭示了三个关键挑战：harness扩展会导致已解决任务的性能下降（harness诱导的遗忘）、自我进化的适应收益在不同阶段不一致、保留和适应之间存在矛盾。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准通常将非平稳性放在任务流中，而保持harness固定。但实际中，LLM智能体依赖的harness（工具、技能、专业智能体）会不断扩展和演变。现有研究缺乏评估智能体在这种harness演变下保留能力和适应新能力的基准。

Method: 构建了EVOHARNESSBENCH基准，包含17条多阶段harness流，从基于验证器的基准确定性地构建。设计了两种评估设置：1) 部署评估，隔离在harness扩展时对先前可访问能力的保留；2) 自我进化适应评估，测试积累的经验在引入新能力时是否仍然有用。

Result: 实验揭示三个持久差距：1) harness扩展本身会导致先前解决任务的性能下降，产生harness诱导的遗忘；2) 自我进化适应的收益在harness演变阶段、能力轴和环境间不一致；3) 保留和适应可能朝不同方向拉扯：保留早期能力不一定能改善对新引入能力的适应，反之亦然。

Conclusion: Harness演变是构建智能体的一个重要且独特的挑战。智能体需要能够跟上不断演变的harness，同时保留之前有效的行为。这需要新的方法来处理harness诱导的遗忘以及保留与适应之间的平衡问题。
...
### [85] [Abstraction Agent](https://arxiv.org/abs/2609.04303)
*Boning Li,Longbo Huang*

Main category: cs.MA

TL;DR: 提出Abstraction Agent，一个使用大型语言模型进行游戏状态抽象的零样本管道，无需领域特定评估器或训练数据即可将游戏状态聚类到抽象桶中。


<details>
  <summary>Details</summary>
Motivation: 传统游戏抽象需要领域特定的评估器（如手牌强度计算器或 equity 估算器），需要专家知识且不适用于大多数缺乏研究的游戏，限制了游戏求解算法的可扩展性。

Method: 四阶段管道：1) 使用校准锚点进行特征发现；2) 批量私有状态评分；3) 基于相关性的特征选择；4) k-means聚类。整个过程无需游戏特定评估器、训练数据或游戏树遍历。

Result: 在HUNL转牌圈 endgame 上比 expected-hand-strength 基线减少高达62%的提升策略可利用性；在ROVER Trials（全新游戏）上在每个粒度上都优于标量排名基线。可转移到四种游戏且发现特征与游戏公认策略概念一致。

Conclusion: 这是结构化知识提取：将LLM参数中的隐式战略知识转化为显式数值特征供下游算法计算，无需任何游戏特定知识即可迁移到新游戏。代码已开源。
...
### [86] [Online Change-point Detection for Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2609.05298)
*Fatemeh Saberi Khomami,Julita Vassileva*

Main category: cs.MA

TL;DR: 研究在线变化点检测，用于协作式多智能体强化学习（MARL）中识别环境和任务目标变化。提出PPR算法，通过平滑智能体的回报流并应用统计漂移检测器来识别显著变化。


<details>
  <summary>Details</summary>
Motivation: 协作式MARL系统依赖过去经验学习协调行为，但环境或任务目标变化时经验可能不可靠，智能体需要先识别情况变化再决定如何适应。

Method: 提出Patterns of Past Rewards (PPR)，一种轻量级、与算法无关的检测器，平滑智能体的回报流，突出显示近期变化，并应用统计漂移检测器标记显著偏移。在基于多智能体粒子环境的自定义Speaker-Listener环境中评估。

Result: PPR在检测速度和报警稳定性之间存在权衡。平滑回报基线检测更早但产生大量重复报警。直接应用于原始回报通常错过变化。PPR通过限制冗余检测同时仍能识别受控变化，提供更平衡的方法。

Conclusion: PPR是一种轻量级、基于奖励的监控工具，使协作式MARL系统能够在训练期间可靠地识别重大变化。
...
### [87] [Trust-Aware Adaptive Disclosure for Inference Privacy Preservation in Multi-Agent Networks](https://arxiv.org/abs/2609.05340)
*Puspanjali Ghoshal,Tobias J. Oechtering*

Main category: cs.MA

TL;DR: 本文研究了在目标推断攻击下，网络多智能体系统中的隐私保护共识问题，提出了基于信任的隐私控制框架（TAPC），通过信任相关的随机策略控制信息发布，在共识性能和隐私保护之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 随着多智能体系统越来越多地部署在医疗管理系统、智能电网等关键信息系统中，智能体的潜在目标需要隐藏免受观察对手的推断，目标推断攻击对系统隐私构成严重威胁。

Method: 提出Trust-Aware Privacy Control框架，根据智能体之间的动态信任关系自适应调整消息披露，使用信任依赖的随机策略控制信息释放。

Result: 实验表明，与代表性基线方法相比，该方法显著降低了对手的目标推断准确率，同时保持了具有竞争力的共识效用。

Conclusion: 信任感知机制在多智能体系统隐私保护中具有有效性，该框架能够在保护智能体隐私的同时维持系统共识性能。
...
### [88] [EXAONE Forecast for Finance](https://arxiv.org/abs/2609.04239)
...
TL;DR: EXAONE Forecast for Finance (EXAONE Finance) 是一种专为金融预测设计的时间序列基础模型(TSFM)。针对现有通用TSFM在金融领域的局限性，该模型采用无注意力机制架构，结合因果卷积和组感知池化MLP，并使用掩码上下文增强技术训练。在金融预测基准测试FinVerse上，该模型在点预测精度、跨截面资产排序和投资组合盈利能力三个评估层面均达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列基础模型(TSFM)虽在零样本预测上表现优秀，但存在三大问题：1) 基于自注意力机制，计算成本随序列长度和变量数量呈二次方增长；2) 假设输入完全观测，无法处理金融市场中常见的长时间、多通道、间歇性观测面板数据；3) 预训练语料库缺乏金融市场独特动态特征。这三大局限性严重阻碍了TSFM在金融领域的应用。

Method: 1) 采用无注意力机制架构：用因果1D卷积处理时间混合，用组感知池化MLP处理变量混合，实现线性时间复杂度；2) 掩码上下文增强：在训练时引入连续缺失片段，提高模型对金融市场中普遍存在的数据缺失的鲁棒性；3) 大规模金融语料库预训练：覆盖股票、外汇、大宗商品、加密资产、固定收益和宏观经济指标等多资产类别。

Result: 在FinVerse金融预测基准测试（涵盖多种资产类别）中，EXAONE Finance在全部三个评估层级均取得最先进性能：点预测精度排名第一、跨截面资产排序排名第一、投资组合盈利能力排名第一。

Conclusion: EXAONE Finance通过创新性的无注意力架构成功解决了传统TSFM在金融领域的适用性问题，其线性时间复杂度设计、大规模金融语料预训练以及针对数据缺失的增强训练策略，使其成为金融预测领域的领先模型，为金融时间序列分析提供了新的有效解决方案。
...
### [89] [From Matching Models to Recruiting Agents: A Systematized Narrative Review of AI Recruitment Systems, Evaluation, and Governance](https://arxiv.org/abs/2609.04286)
*Ziyi Zhao,Guanzheng Wei*

Main category: cs.AI

TL;DR: 本文是关于人工智能招聘的系统综述，梳理了从双向检索、行为排序、神经人物匹配到大型语言模型组件和工具使用招聘代理的发展历程。通过对40篇代表性论文的分析，揭示了三个核心转变：从相似性到双向适配、从单一模型到复合工作流、从离线预测到证据和生产率对齐的评估。


<details>
  <summary>Details</summary>
Motivation: 人工智能招聘的自动化对象已从简单的档案对和排名列表演变为多阶段工作流，但缺乏系统性的综述来追踪这一发展脉络并识别领域关键转变和持续性差距。

Method: 采用目的性检索和编码协议，更新至2026年7月23日，并补充至2026年9月2日，组织40篇代表性作品及工业和法律来源进行综合分析。

Result: 识别出三个耦合转变：从相似性到双向适配、从单一模型到复合工作流、从离线预测到证据和生产率对齐的评估。发现四个持续性差距：行为标签混淆暴露、偏好和资质；私人和合成数据限制外部效度；最终分数掩盖管道故障；编码集中无研究同时评估效用、公平性、隐私和安全性。

Conclusion: 提出从评估证据到最强可辩护声明的分阶段映射，以及构建双向、证据驱动、时间控制、选择性可审计系统的议程。进展应通过工作流是否检索正确证据、保留不确定性、支持可争议决策和在明确成本和风险约束下改善结果来衡量。
...
### [90] [Harbor Adapters and Harbor-Index: Infrastructure and a Curated Meta-Dataset for Large-Scale Agentic Evaluation](https://arxiv.org/abs/2609.04298)
...
TL;DR: 本文介绍了Harbor Adapters，一个用于智能体评测的统一基础设施，解决了智能体基准测试评估复杂、难以横向对比的问题。该工作包含三个贡献：开发了80多个基准测试的适配器，在8个模型和54个基准测试上进行了大规模评估，并筛选出82个高质量难题构成Harbor-Index。最佳配置GPT-5.5+Codex仅达到28.0%通过率，表明当前智能体评估仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 随着智能体基准测试数量增长，在不同基准上评估智能体变得困难，因为这些基准通常需要复杂的环境和智能体集成。缺乏统一、可比的评估框架导致难以全面分析智能体的能力和失败模式。

Method: 1)开发基准适配器将80+基准测试统一化，通过代码审查和 parity 实验验证；2)使用Terminus-2和3种原生测试框架对8个模型进行大规模评估；3)通过难度过滤、AI和人工审计、审计-修复循环从适配套件中筛选出82个高质量难题构成Harbor-Index。

Result: 评估结果显示：1)成功构建了统一的评估基础设施；2)所有模型-测试框架组合的通过率均不超过30%；3)最强配置GPT-5.5配合Codex达到28.0%通过率；4)Harbor-Index在保持挑战性的同时具有可负担的计算成本。

Conclusion: Harbor Adapters提供了一个可靠、全面的智能体评估框架。开源的适配器、评估结果和Harbor-Index将支持对语言模型智能体更可靠的评估，推动该领域的发展。
...
TL;DR: 本研究探索机器学习算法对电力系统安全等级进行分类（安全、中度、严重），使用Newton-Raphson潮流提取数据，结合SMOTE和PCA进行数据预处理，对IEEE-14和IEEE-30节点系统进行N-k contingency分析，Random Forest取得最佳F1分数。


<details>
  <summary>Details</summary>
Motivation: 确保电力系统安全对稳定性和可靠性至关重要，有效的 contingency分类能够实现主动决策并防止大规模停电和故障。传统 contingency分析计算量大、耗时，难以满足实时安全评估需求。

Method: 使用Newton-Raphson潮流方法提取contingency场景数据，以Overall Performance Index (OPI)作为安全度量；采用SMOTE处理类不平衡问题，PCA进行降维；训练KNN、Random Forest和SVM三种分类器；在IEEE-14和IEEE-30节点系统上生成N-k (k=1,2,3) contingency数据集，测试四种预处理配置（标准化、SMOTE平衡、PCA转换、SMOTE+PCA组合）。

Result: RF在IEEE-30和IEEE-14上分别获得最高F1分数0.97和0.86；SVM从PCA中获益显著，分类精度提升；KNN最适合SMOTE和PCA转换；PCA对模型整体性能的贡献大于SMOTE；SMOTE提高召回率但可能引入假阳性，是精度与召回率的权衡。

Conclusion: 机器学习是传统contingency分析的可扩展且强大的替代方案，能够提升实时安全评估能力。RF表现最优，PCA预处理效果优于SMOTE。
...
TL;DR: 本文是一篇关于大型语言模型(LLM)作为智能体的关键综述,评估了模型在调用工具、操作接口、委托工作、保持状态、生成世界和控制机器人等方面的能力进展,并质疑"向自主性迈进"这一叙事,认为需要更严格地区分模型能力、系统集成、持久性和安全权限。


<details>
  <summary>Details</summary>
Motivation: 当前研究将LLM的进展简单叙述为"向自主性的迈进",混淆了模型能力、系统集成、持久性和安全权威等不同维度,缺乏对实际代理能力的严格评估。

Method: 综合截至2026年8月31日的主要研究和技术规范,沿委托授权、时间持久性和环境耦合三个维度组织证据,区分模型、框架和环境层面。

Result: 研究发现:动作接口扩展的证据比稳健完成、恢复、授权或独立验证更充分;MCP和A2A协议改善了互操作性但未建立可信赖的委托;多智能体组织增加了专业化但也带来成本和相关性失败;持久模拟和世界模型支持训练和规划但本身不展示代理能力;机器人和自动驾驶实验室建立了有界可行性而非无人值守的开放世界可靠性。

Conclusion: 提出"合理委托"作为分析和规范启发式方法,主张仅在有证据支持来源、有界授权、失败检测、安全恢复和校准人类控制的条件下扩展动作范围,并给出耦合模型-框架评估、基于能力的权限、持久状态、跨智能体问责和分阶段物理验证的研究议程。
...
### [94] [Iris: Climbing to the Search Frontier](https://arxiv.org/abs/2609.04304)
*Ziyuan Liu,Hengqi Liu,Zichuan Wang,Yang Qin,Jiachen Liang,Xu Chu,Shaowei Chen,Yuantao Gu,Mu Chuan*

Main category: cs.AI

TL;DR: 本文提出Iris-mini和Iris-pro两个搜索代理，通过反向构建任务数据、SFT-RL climbing训练范式和推理时上下文管理，在多跳搜索基准上取得了开源搜索代理的最佳性能。


<details>
  <summary>Details</summary>
Motivation: 当前搜索代理在多跳问答任务中面临挑战：现有方法难以构建高质量的训练数据，且推理时上下文管理对性能影响显著但常被忽视。团队旨在构建更有效的搜索代理训练流程和数据管道。

Method: 1) 反向构建任务：从网页语料库的超链接结构中提取实体图，在种子上构建多跳链，将非答案实体改写为描述性引用以避免字符串匹配，仅保留参考模型无法闭卷解答但提供证据后可解决的问题；2) 轨迹构建与过滤：在轨迹级和轮次级进行过滤后进行SFT；3) RL优化：对实时搜索进行策略优化，奖励判断器和观察总结器部署在训练集群内，过长 rollout 在请求级中断并从committed prefix恢复；4) SFT-RL climbing：交替两阶段，将每轮RL中最难解决和最高效的rollout返回到下一个监督传递；5) 推理时上下文管理评估。

Result: 在BrowseComp、BrowseComp-ZH、DeepSearchQA和HLE四个基准上，Iris-mini达到82.2/84.8/86.9/52.3，Iris-pro达到88.6/85.1/92.9/56.4，在各自参数范围内成为最强的开源搜索代理。推理时上下文管理对基准性能提升显著。

Conclusion: Iris-mini和Iris-pro在开源搜索代理中取得了最佳结果，团队计划开源模型权重及完整的数据构建、训练和评估流程。SFT-RL climbing方法有效提升了模型在多跳搜索任务上的性能，且推理时上下文管理对性能提升显著。
...
TL;DR: 研究测试了多智能体系统中智能体是否真正可互换的假设。研究发现，替换智能体后任务得分变化不大，但团队沟通效率降低16-63%。在需要协调的任务中，换入的智能体比新加入的智能体成本更高，表明智能体与原伙伴形成的惯例会干扰新团队的合作。团队历史越长，替换惩罚越大。


<details>
  <summary>Details</summary>
Motivation: 挑战多智能体系统中的一个基本假设：担任角色的智能体可以被任何能完成该工作的智能体替换。研究旨在验证智能体在实际协作中是否真正可互换。

Method: 从同一基础模型独立形成8个团队，智能体在10个形成环节中保留私人笔记本；将角色匹配的智能体在团队间交换，在保留任务上测量变化；设置安慰剂对照排除 roster change 本身的干扰效应。进行三项消融实验：基础模型、解码温度、形成长度。

Result: 交换对任务得分影响很小，但团队每单位进度的沟通成本增加16-63%。在Hanabi中，交换来的智能体比无经验智能体成本更高。在Collab-Overcooked中，当设置议程的智能体被替换时，额外沟通主要来自留下的智能体。消融实验表明：贪婪解码降低替换惩罚和团队分歧；加倍团队历史增加两者。团队历史越长，替换效应越大。

Conclusion: 在协调密集型任务中，智能体在任务结果上比在协调效率上更具可替换性。智能体与队友形成的协调惯例具有持久性，替换后会干扰新团队的合作。这对需要频繁更换智能体的生产系统有重要启示。
...
### [96] [A Removal Based Approach to Improve LLM Faithfulness at Test-Time](https://arxiv.org/abs/2609.04343)
*Qinglan Luo,S M A Nahian,John Guttag,S. Mazdak Abulnaga,Katie Matton*

Main category: cs.AI

TL;DR: 本文研究大语言模型(LLM)决策解释的忠实性问题，提出一种推理时的测试方法，通过移除输入中未被解释提及的概念来提高解释的完整性和忠实性。


<details>
  <summary>Details</summary>
Motivation: LLM被广泛用于重要决策，但其提供的解释往往不忠实，无法反映模型实际推理过程。现有方法要么需要训练时访问模型权重，要么主要解决解释的不合理性(soundness)问题，缺乏针对不完整性(incompleteness)的推理时方法。

Method: 提出一种推理时测试方法：首先获取模型对问题的回答和解释，然后从输入中移除解释中未提及的概念，用简化后的输入重新查询模型，观察答案是否保持不变。这种方法消除了未提及因素的影响，同时保留了已提及概念的影响。

Result: 在两个数据集、多个模型家族和两个独立的忠实性指标上，该方法相比标准提示和鼓励忠实性的提示方法都能显著提高解释的忠实性。

Conclusion: 该方法是模型无关的，可在推理时应用，无需修改模型参数，为减少隐藏影响因素、提高LLM辅助决策的可靠性和安全性提供了一种灵活的机制。
...
### [97] [Why Better Models Can Create Riskier Systems: Evidence from LLM Agents in Financial Markets](https://arxiv.org/abs/2609.04373)
*Jillian Ross,Eric So,Zoe De Simone,Charles Pozniak,Andrew W. Lo*

Main category: cs.AI

TL;DR: 研究发现：提升LLM个体能力反而可能降低系统级 outcomes。共享训练/架构导致更capable的LLMs行为更相似，产生无法分散的相关性风险，形成"能力悖论"。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs被大规模部署于金融市场的内容审核等关键系统，但现有研究假设更capable的模型会带来更好的系统结果。本文质疑这一假设，探究个体能力提升与系统级 outcomes之间的关系。

Method: 构建通用理论框架分析相关性风险；设计基于agent的模拟系统，使用不同能力水平的LLM traders进行金融市场的实证测试。

Result: 1) frontier LLMs表现出显著相关性，且能力越强相关性越高；2) 当共享推理准确时，增加agent参与降低市场风险；3) 当存在共同 misinformation环境时，相关性行为成为负担。

Conclusion: 提出"能力悖论"：改进个体模型不一定带来更好的系统级结果。更capable的LLMs因行为更相似，在错误环境下风险更大。需重新评估LLM部署策略。
...
### [98] [Corporate Language Model (CLM): Transforming Tacit and Fragmented Enterprise Knowledge into a Sovereign, Auditable, and Executable Corporate Intelligence Layer](https://arxiv.org/abs/2609.04377)
*Fabricio C. Avini,Guilherme Trez*

Main category: cs.AI

TL;DR: 本文提出企业AI部署失败的根本原因并非模型能力不足，而是缺乏结构化的决策、协商和执行框架。为此，论文提出企业语言模型(CLM)框架，将企业的结构化、非结构化、多模态和隐性知识转化为基于本体论的企业基础模型，支持推理和受治理的执行。CLM包含五个能力平面和四个架构支柱：神经符号网格、知识图谱、可组合的技能图谱、活数字孪生和深度安全层。论文还提出"智慧聆听者效应"，表明具备隐性能力的基础模型会随着使用而增值。


<details>
  <summary>Details</summary>
Motivation: 企业AI部署频繁失败，根源在于缺乏结构化知识基底：通用大语言模型缺乏企业特定的本体论先验知识；RAG系统脆弱且无法执行可操作的任务；静态 playbook 无法进行推理和自适应。这要求一种将隐性知识捕获、本体论锚定、主权部署和可审计执行作为协同设计目标的架构。

Method: 提出CLM框架，包含四个架构支柱：(1)神经符号网格(Neurosymbolic Mesh)：将生成模型与知识图谱耦合；(2)技能图谱(Skill Graph)：将可重复使用的战术、角色、异议和目标进行类型化并组合；(3)活数字孪生(Living Digital Twins)：将功能领域建模为推理代理；(4)深度安全层(Deep Security Layer)：强制执行主权、可追溯性和人类监督。引入"规范即代码"(Spec-as-Code)范式连接锚定意图和可执行产物。

Result: 定义了CLM作为独特的研究对象；提出技能图谱用于可组合的可解释性；提出"智慧聆听者效应"，表明具备隐性能力的基础模型随使用增值，连接动态能力和组织学习；提供巴西JCI认证三级医院在LGPD下三个成熟度阶段的证据。

Conclusion: CLM代表了一种以基础模型为中心的新型企业AI架构范式，通过将隐性知识捕获、本体论锚定、主权部署和可审计执行进行协同设计，解决了企业AI部署失败的核心问题。该框架将随着使用而增值，体现出动态能力和组织学习的特征。
...
### [99] [HarvestBench: Measuring Whether LLM Agents Will Pay to Avoid Killing Animals](https://arxiv.org/abs/2609.04444)
...
### [100] [PerfReasoning: How Well Do LLMs Reason on Hardware Performance?](https://arxiv.org/abs/2609.04476)
*Dan Zhao,Karthikeyan Sankaralingam,Christos Kozyrakis,Qijing Huang*

Main category: cs.AI

TL;DR: PerfReasoning是一个用于评估LLM在硬件性能建模方面能力的基准测试，涵盖性能推理和性能模型代码生成两大任务。


<details>
  <summary>Details</summary>
Motivation: 性能建模对硬件设计和软件优化至关重要，但构建这些模型需要对计算、数据复用、存储和移动进行结构化推理。现有的LLM评估缺乏针对这一领域的系统化基准。

Method: 设计了两类任务：1)直接性能推理任务，通过Q&A形式评估LLM对映射方案、off-chip流量和缓冲区需求的预测能力；2)性能模型代码生成任务。要求模型根据工作负载、架构和映射规范进行分析和预测。

Result: 最强闭源模型在推理类Q&A上超过90%，最佳开源模型达到82.4%。但代码生成任务困难：GPT-5.6 Sol超过80%通过率，其他模型配置平均低于15%。任务特定RL将4B模型的映射推理准确率提升15.7分，而无反馈的多轮自我修正提示效果不可靠。

Conclusion: PerfReasoning揭示了LLM在看似合理的架构推理与可靠的性能模型构建之间存在显著差距。该基准将公开支持可复现评估和追踪未来进展。
...
### [101] [When Quantization Breaks Memory: Recurrent-State Write-Back in Low-Precision Temporal Inference](https://arxiv.org/abs/2609.04490)
*Ismail Erbas,Xavier Intes,Vikas Pandey*

Main category: cs.AI

TL;DR: 本文研究了递归神经网络量化中常被忽视的"递归状态回写"问题，即量化状态如何存储并影响后续计算。在用于荧光寿命成像的紧凑GRU编解码器中，4位状态存储导致τ1和τ2的估计误差分别增加约70倍和300倍。问题源于重复的小更新低于写入阈值，导致存储状态几乎固定。误差反馈、残差记忆和方向记忆可跨时间传递被抑制的更新信息，无需重新训练即可恢复精度。


<details>
  <summary>Details</summary>
Motivation: 当前量化研究主要关注前向传播中的参数和激活量化，但递归网络中量化状态需存储并在下个时间步使用，这一"递归状态回写"规则对网络行为的影响尚未被充分研究。荧光寿命成像需要从高噪声时间分辨荧光信号中估计两个寿命参数，是检验递归状态回写影响的理想任务。

Method: 使用紧凑的GRU编解码器处理荧光寿命成像任务，固定训练好的模型，将连续状态传播替换为确定性4位状态存储进行后训练干预。引入误差反馈、残差记忆和方向记忆机制跨时间传递信息。同时在独立训练的LSTM上重复实验验证结论的普适性。

Result: 4位状态存储导致τ1估计误差增加约70倍，τ2增加约300倍。失败原因是重复的小更新低于写入阈值，存储状态几乎固定而网络持续提出变化。误差反馈、残差记忆和方向记忆可恢复精度。精度扫描显示增加状态精度可能恶化固定递归解决方案，匹配训练可学习与状态接口的兼容性。LSTM实验证实了相同行为，且细胞状态比隐藏状态更敏感。

Conclusion: 递归状态回写是低精度递归动力学的关键决定因素，状态存储接口应作为量化递归推理的核心设计考量。误差反馈等机制可有效缓解量化带来的状态更新抑制问题，无需重新训练即可恢复准确性。
...
### [102] [ResLearn-XR: Residual Learning for Network Traffic and Quality-of-Experience-Aware Modeling in Extended Reality](https://arxiv.org/abs/2609.04493)
...
### [103] [Rethinking Indirect Prompt Injection as a Test-Time Search Problem](https://arxiv.org/abs/2609.04495)
*Duong M. Nguyen,Joon Sik Kim,Blazej Manczak,Vaikkunth Mugunthan*

Main category: cs.AI

TL;DR: 本文提出将间接提示注入（indirect prompt injection）建模为对任务相关攻击面的测试时搜索问题，并引入了一种具有专用搜索框架的主动性攻击代理，可在环境中进行侦察、对攻击策略进行结构化推理，并利用受害者代理反馈进行自适应评估。实验表明，增加攻击者的测试时计算资源可提高漏洞发现和利用效果，且显式策略管理对避免冗余搜索和在更大预算下维持收益很重要。研究成果建议 agentic 安全评估应同时表征攻击者的搜索过程和计算预算，而非将攻击成功视为与预算无关的受害者属性。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估通常将攻击成功视为受害者的固有属性，忽略了攻击者搜索过程和计算资源的影响。本文旨在研究攻击者在测试时对系统攻击面的自适应搜索是否构成重要但未被充分探索的安全风险。

Method: 提出一种agentic攻击者框架，包含：(1) 环境侦察模块；(2) 攻击策略结构化推理；(3) 利用受害者代理反馈的自适应评估。通过在不同任务上的实验，测试不同测试时计算预算下的漏洞发现与利用效果。

Result: 增加攻击者测试时计算资源可提升漏洞发现和利用的成功率；显式策略管理对避免冗余搜索、在更大预算下维持收益至关重要。

Conclusion: Agentic安全评估应同时表征攻击者的搜索程序和计算预算；攻击者对系统攻击面的自适应搜索是工具使用代理面临的重要且未被充分探索的安全风险。
...
### [104] [BioSync: Transformer-Based Cross-Modal Fusion for a Multimodal Physiological Digital Biomarker](https://arxiv.org/abs/2609.04504)
*Seyed Mahmoud Sajjadi Mohammadabadi*

Main category: cs.AI

TL;DR: BioSync是一个将可穿戴设备的心血管、神经、行为和语音测量整合为连续复合数字生物标志物(BSI)的系统。该模型采用多头自注意力机制处理模态令牌，并结合线性分支形成宽深架构。在认知衰退队列中，BioSync的AUC为0.928，代谢-自主神经队列中准确率/F1为0.764/0.766，均优于特征拼接方法。BSI与潜在严重程度高度相关(r=0.91和r=0.68)。


<details>
  <summary>Details</summary>
Motivation: 可穿戴和移动设备提供的心血管、神经、行为和语音测量仅能提供生理状态的局部、受噪声敏感的信息视图。作者假设联合观测可能包含单独模态无法获取的信息，因此提出结合多种模态测量来更全面地评估生理状态。

Method: BioSync模型采用多头自注意力机制处理模态令牌，并添加一个线性分支(其假设类包含标准特征拼接)。该架构基于潜在变量测量理论，在两个人工合成的文献知情队列上进行评估：四模态认知衰退队列(使用HRV、脑电、活动记录和语音)和基于AI-READI可穿戴架构的代谢-自主神经队列。

Result: 认知衰退队列中，BioSync和拼接方法的AUC分别为0.928和0.926。代谢队列中，BioSync的准确率/F1为0.764/0.766，拼接方法为0.756/0.758。BSI与两个队列的潜在严重程度相关(r=0.91和r=0.68)。纯注意力消融实验得到认知队列AUC 0.911，表明性能提升来自宽深架构结合。在模态dropout训练下，BioSync在6个认知衰退污染率中的5个和最高代谢污染率上优于拼接方法。BioSync的认知队列AUC也高于5个已发布的数字生物标志物参考值。

Conclusion: BioSync在多模态生理状态评估中展现出优于现有方法的性能，其宽深注意力架构有效整合了各模态信息。但该研究在合成队列上进行验证，在真实队列上的验证仍有必要。不同数据集和任务之间的差异也限制了对照基准声明。
...
### [105] [What Does Multi-Harness RL Learn? Credit Assignment and Portability in Coding Agents](https://arxiv.org/abs/2609.04518)
*Chenqian Le,Jiayi Cheng,Qijia He,Runhao Li,Yinghao Li,Xupeng Chen*

Main category: cs.AI

TL;DR: 研究在软件工程Agent强化学习中，对比了两种Group Relative Policy Optimization (GRPO)分组策略（Within vs Cross）在多 harness 环境下的效果。研究发现评估harness是主导变量，训练recipe的影响较小，分组规则本身无显著差异。


<details>
  <summary>Details</summary>
Motivation: 探索多harness训练策略中，分组边界对策略优化的影响，特别是在代码生成Agent场景下比较Within-harness和Cross-harness两种分组方式的优劣。

Method: 使用Qwen3-8B作为监督微调起点，在Aider、OpenHands、Qwen Code、SWE-agent四个harness的冻结任务-harness记录上进行重放训练。比较两种GRPO分组规则：Within（每个task-harness对作为一个组）和Cross（同一task内harness池化）。通过24,000次密封评估测试checkpoint性能。

Result: 评估harness使平均解决率从2.14%变化到9.27%（4.3倍差异），而训练recipe仅改变1.16。分组规则差异不显著：在held-out harness上，Cross-Within差值为+0.25pp（95%CI [-0.48, +1.02]），各训练种子自身范围（0.42-0.45pp）大于规则间差异。Cross-harness的advantage可预测生成harness（+4.48pp），Within则不能，但两者最终达到相同held-out分数和harness内动作分布。

Conclusion: 评估harness是主要变量，分组规则无显著影响。Cross-harness信用产生配置适应但不增加跨harness的可迁移能力。多harness RL研究应报告分组边界并在新harness上测试。
...
### [106] [MaxKernel: Agentic Kernel Generation for TPUs](https://arxiv.org/abs/2609.04523)
...
TL;DR: MaxKernel是一个利用大语言模型和实时编译器反馈为TPU加速器生成高性能内核的多智能体系统，包含三种开发范式：人机协作(HITL)、全自动优化(Auto)和图搜索扩展探索。


<details>
  <summary>Details</summary>
Motivation: 为加速器设计和编写高性能自定义内核需要深厚的硬件级专业知识，现有方法依赖专家手动调优，效率低下。需要构建能够利用LLM和编译器反馈的智能系统来自动化内核生成和优化过程。

Method: 提出三种TPU内核开发范式：(1) HITL智能体实现协作式逐步设计；(2) Auto智能体执行全自动度量和追踪驱动的优化循环；(3) Graph-Based Autonomous Search将Auto智能体扩展以全局探索设计空间。三种范式共享专用子智能体池处理规划、实现、自调试、测试和硬件性能分析。在JaxBench（50个TPU内核任务）和真实工作负载上评估。

Result: MaxKernel持续生成高度优化的实现，达到专家手调基线水平，在基准测试上实现显著性能提升。系统已开源。

Conclusion: LLM与编译器反馈结合的多智能体系统可有效解决加速器内核生成难题，三种范式为不同场景提供灵活解决方案，图搜索扩展了设计空间探索能力。该工作为自动化硬件加速器编程提供了新思路。
...
### [107] [Towards a universal language of concepts: A survey](https://arxiv.org/abs/2609.04528)
*Aishni Parab*

Main category: cs.AI

TL;DR: 本文提出programs（程序）可作为概念的通用表示形式，能够从稀疏数据中进行学习并泛化新概念，并回顾了使用程序作为概念表示的计算模型，评估其作为通用表示语言的贡献。


<details>
  <summary>Details</summary>
Motivation: 人类能够从稀疏数据中学习和泛化新概念，因为人类以丰富的结构化形式表达知识。这启发了研究者寻找一种通用的概念表示形式。

Method: 回顾性研究，综述了使用程序作为概念表示的计算模型，并评估这些模型对构建通用表示语言的贡献。

Result: 文章综述了相关计算模型，评估了程序作为概念表示的可行性和优势。

Conclusion: 程序是概念通用表示的有力候选者，有望成为 Universal Representational Language（通用表示语言）。
...
### [108] [Data-Driven Discovery of Composition-Dependent Constitutive Models for Hyperelasticity and Viscoelasticity of Digital Materials](https://arxiv.org/abs/2609.04541)
*Josué García-Ávila,Beijun Shen,Manuel K. Rausch,Mary C. Boyce,Adrián Buganza-Tepole*

Main category: cs.AI

TL;DR: 本文提出了一种数据驱动的多材料本构建模框架，将机器学习与有限应变粘弹性模型相结合，能够跨成分和加载率预测非线性力学行为，同时保持热力学一致性。


<details>
  <summary>Details</summary>
Motivation: 传统有限应变粘弹性模型使用封闭形式应变能函数和内变量演化定律，灵活性和泛化能力有限。单一本构模型难以跨材料成分和加载率推广。

Method: 保留Bergström-Boyce框架结构（乘法运动学、不变量应变能、标量耗散演化定律），对平衡分支：直接预测封闭形式模型参数或使用神经ODE自动构建多凸应变能函数；对非平衡分支：识别封闭形式参数或使用约束神经网络学习动力学。

Result: 使用多速率单轴压缩数据验证，框架成功捕获了跨成分的率相关刚度和滞回现象，同时保持热力学一致性。

Conclusion: 数据驱动框架可推广至不同材料成分和加载条件，为多材料3D打印材料的本构建模提供了新范式。
...
### [109] [From Answers to Interpretations: Rethinking Ambiguity-Induced Aleatoric Uncertainty Estimation in LLMs](https://arxiv.org/abs/2609.04543)
*Omer Nahum,Niv Nayman,Jonathan Fhima,Alon Zolfi,Jeremy Levy,Shai Mazor,Paolo Favaro*

Main category: cs.AI

TL;DR: 本文提出一种仅基于澄清问题（而非答案）来估计LLM部署中歧义性不确定性的方法，显著提升了AUROC并大幅降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 核心挑战在于区分LLM中的不可约不确定性（aleatoric）与可约不确定性（epistemic）。现有方法通过生成多个输入澄清、查询模型答案并比较来估计歧义性不确定性，但答案并非必要，且会增加成本并可能因认知泄露而误导。

Method: 提出澄清仅方法（clarification-only approach）：直接从可能解释的空间估计歧义诱导的不确定性，无需对澄清后的输入获取答案。

Result: 在三个基准测试中：AUROC提升（63.34 vs 60.85）；输出token成本降低4-26倍，API调用降低2.2-3.5倍；与认知不确定性估计的相关性显著降低。

Conclusion: 歧义诱导的不可约不确定性更适合从解释空间而非响应空间进行估计。
...
### [110] [IPGeoAI: Transformer-Based Geolocation with LLM Semantic Fusion](https://arxiv.org/abs/2609.04559)
*Avinash Kadimisetty,Andy Jinqing Yu,Philip Favaloro,Wenlong Liu,Xiaolu Xiong*

Main category: cs.AI

TL;DR: IPGeoAI是一种新型深度学习模型，将IP地理位置从静态查询问题重新定义为序列建模任务，利用Transformer编码器捕捉IP子网结构中的层次依赖关系，并通过集成LLM生成的语义特征来解决地理模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 传统启发式和数据库驱动方法难以解决现代网络基础设施的复杂非线性分配模式，特别是在IPv6地址空间和移动网络中；城市级IP地理定位对于本地内容交付、定位 targeting和数字版权执法至关重要。

Method: 1)使用Transformer编码器捕捉IP子网结构的层次依赖；2)通过Zero-Shot LLM特征提取管道整合非结构化语义上下文；3)利用大语言模型将自治系统描述转换为结构化领域元数据；4)通过多头交叉注意力模块将语义信号融合到网络中。

Result: 在城市级粒度上显著优于领先外部供应商；采用分层推理策略后，城市级准确率提升6%，流量覆盖扩展到100%；在线生产测试中，统计显著提升0.35%。

Conclusion: IPGeoAI通过结合网络拓扑结构与现实世界语义身份，有效解决了传统方法的局限性，实现了更高的城市级地理定位准确率和全面覆盖。
...
### [111] [Reducing Hallucinated Transcripts in Whisper via Hallucination Space Projection](https://arxiv.org/abs/2609.04561)
*Maryam Abbasihafshejani,Murtuza Jadliwala*

Main category: cs.AI

TL;DR: 本文提出一种无需训练、推理时使用的低秩投影方法，用于减少Whisper语音识别模型对非语音输入产生幻觉转录的问题。通过从非语音校准数据估计幻觉相关子空间，在推理时将解码器隐藏状态从该子空间中投影出去。


<details>
  <summary>Details</summary>
Motivation: Whisper是一个广泛使用的自动语音识别基础模型，但其生成解码器会对几乎不含语音的输入产生流利的幻觉转录，这是一个需要解决的实际问题。

Method: 使用低秩投影技术，从非语音校准数据估计一个紧凑的幻觉相关子空间，在推理时将解码器隐藏状态从该子空间投影出去。提出了两种变体：always-on（对所有输入应用投影）和gated（仅在Whisper预测输入可能为非语音时应用）。

Result: 在非语音基准上，always-on投影将平均幻觉率从31.31%降至2.44%，相对减少92.21%；gated投影将幻觉率降至3.74%，相对减少88.05%，且对真实语音的误拒率更低。在LibriSpeech上，gated投影导致词错误率绝对增加0.33-4.39个百分点，误拒率为0.41-9.97%。

Conclusion: 低秩激活投影可以显著抑制Whisper的幻觉转录而无需重新训练，同时在幻觉抑制和语音识别性能之间提供了可控的权衡。
...
### [113] [Does the Selected Object Reach the Reader? Auditing Identity Handoffs in Grounded Language-Model Pipelines](https://arxiv.org/abs/2609.04579)
*Siddharth Vohra,Runmin Jiang,Xiaomo Li,Min Xu*

Main category: cs.AI

TL;DR: 本文审计了HybridQA数据集中600个问题的检索管道，揭示了对象选择与传递过程中的关键问题。研究发现精确键查找和精确标题匹配能100%返回目标对象，但仅正文BM25在26.6%的记录中遗漏目标，而带重排序的混合检索仅遗漏1.0%。数据集追踪的 Passage 与实际返回的对象在329条记录上存在差异。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型管道中存在"对象丢失"问题：当选择的对象必须传递给读者时，传递失败会导致答案错误。现有基准仅检查数据集关联的对象，而该对象可能与实际选择的对象不同。

Method: 对600个HybridQA问题进行审计，跨越三个选择器家族，比较四种检索方法：精确键查找、精确标题匹配、仅正文BM25（截断5）、带重排序的混合检索。分析1,463条可解析记录和1,792条可解析记录中的差异。

Result: 精确键查找和精确标题匹配100%返回目标。仅正文BM25在389条记录（26.6%）中遗漏目标；混合检索仅遗漏14条（1.0%）。两种身份定义在329条记录上不同。原始问题排名前5检查在106条记录（5.9%）上不一致。对齐对象的存在与28.6-31.0点更高的精确匹配相关。

Conclusion: 对象对齐对问答性能至关重要。删除目标段落显著降低精确匹配，而删除相似长度的对比段落不会产生同样效果。研究发布了Returned-Object Profile (ROP)，提供目标、返回ID字段、截断、成员规则和完整预期人口的可执行记录。
...
TL;DR: ττ-bench是一个将智能体构建作为任务的基准测试，用于评估AI系统在真实客户 engagement 条件下构建客户服务智能体的能力。给定业务记录、客户端需求、生产API、代码库和成本/模型限制，要求开发智能体交付完整的客户服务智能体，通过对模拟用户进行部署测试来评分。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法评估AI系统是否能在真实客户 engagement 条件下交付智能体。随着LLM智能体越来越多地用于生产软件（客户服务、争议裁决、内部系统运营），需要一种方法来衡量AI构建智能体的能力。

Method: 基准测试提供：业务实际记录、持有需求的客户端、生产API、需要继承的代码库、成本和模型限制。开发者智能体需从中交付完整的客户服务智能体，通过对留置模拟用户部署进行评分。在4个领域53个任务上进行测试。

Result: 最强配置（Claude Opus 5 under Claude Code）仅通过23.9%的评估模拟；专家编写的参考上限得分为82.2%。失败模式与人类智能体开发者类似：模型发出浅层查询而非深入理解记录，几乎不与客户端沟通，对智能体架构和服务成本尝试不足，直接发布第一个可运行的设计。

Conclusion: ττ-bench旨在将协作式智能体构建工作转化为编码智能体的可衡量目标，为评估AI在真实客户端场景下构建生产级智能体的能力提供基准。
...
### [115] [Leveraging Imperfect Restoration for Data Availability Attack](https://arxiv.org/abs/2609.04627)
*Yi Huang,Jeremy Styborski,Mingzhi Lyu,Fan Wang,Adams Kong*

Main category: cs.AI

TL;DR: 本文提出了一种名为不完美恢复中毒(IRP)的新型数据投毒方法，用于抵御深度学习模型对在线数据的未经授权使用。该方法在保持高图像质量的同时实现了强投毒效果，在监督学习和自监督学习场景中均优于8个基线方法，并能与5种代表性防御方法配合使用。


<details>
  <summary>Details</summary>
Motivation: 现有数据可用性攻击(DAAs)通常只在监督学习(SL)或自监督学习(SSL)之一表现出色。CUDA作为两者通用的最鲁棒方法，存在对SSL效果不佳且在图像质量和投毒效果之间存在严重权衡的问题。

Method: 理论分析CUDA揭示其引入的次优梯度及诱导类偏差的策略，在此基础上提出IRP方法，通过不完美恢复实现高质量图像和强投毒效果的平衡。

Result: IRP在SL和SSL场景中相比8个基线方法展现优越性，实验中结合5种代表性防御方法验证了其有效性。

Conclusion: IRP成功解决了CUDA在图像质量与投毒效果之间的权衡问题，在SL和SSL场景中均表现出色，为数据可用性攻击提供了更优解决方案。
...
### [116] [SiLR: Structure-Preserving Admission and Process Reward for LLM Tool Agents](https://arxiv.org/abs/2609.04629)
*Chenyu Zhou,Qiliang Jiang,Shuning Wu,Xu Zhou*

Main category: cs.AI

TL;DR: 本文提出SiLR方法，解决LLM工具代理在ReAct循环中的运行时门控问题。针对现有标量投影门的"标量投影陷阱"（接受局部改进但使轨迹陷入 plateau），SiLR通过影子执行每个提案并在分支级违规状态上使用积序进行 admission 控制。实验表明SiLR在21个多动作场景中全部恢复成功，而终端门和最佳标量门分别为0/21和9/21。


<details>
  <summary>Details</summary>
Motivation: 研究LLM工具代理在ReAct循环中的运行时门控问题。当前标量投影门存在"标量投影陷阱"：聚合分数门接受局部改进的提案但将轨迹困在 plateau 上，无法在违规状态下继续取得进展。需要在违规状态下仍能 admission 取得进展的方法。

Method: SiLR方法：1）对每个提案进行影子执行；2）使用分支级违规状态上的积序（过载分支支持度和每分支严重性）进行 admission 判断；3）证明不存在对该序的标量代理是 sound 的。LLM 位于信任边界之外，admission 基于确定性模拟。

Result: 1）Gym-ANM场景：SiLR在21个多动作情节中全部恢复（21/21），终端门0/21，标量门最佳9/21；2）三种模型家族和CityLearn中均成立；3） magnitude-redistribution攻击仅被完整每分支谓词防御；4）双约束家族测试中，标量门均 admission 不安全动作，积序为0%；5）作为GRPO奖励优于计数投影，唯一使无门策略超过基线（0.844 vs 0.778）。

Conclusion: 标量投影在两个设计点都丢失了违规几何结构；只有完整的积序在结构上是充分的。这代表了一种从根本上优于标量投影方法的 admission 机制。
...
TL;DR: DevRev NL2SQL基准测试和成本感知单生成智能体架构，用于处理企业生产环境中图状、半结构化、深度嵌套的数据库模式


<details>
  <summary>Details</summary>
Motivation: 当前自然语言转SQL系统在学术基准测试上表现良好，但生产环境中的企业模式具有图状、半结构化、深度嵌套的特性，现有基准测试无法衡量这类场景

Method: 1) 提出DevRev NL2SQL基准测试：900个经执行验证的查询，包含嵌套类型和链接图结构，附带语义深度评分(SDS) 2) 提出成本感知的单生成智能体架构，包含模式选择、元数据检索和错误修复组件

Result: 在DevRev NL2SQL基准上达到91.7%的答案正确率，比次优基线高出54.6个百分点；在Spider 2.0 Snowflake公共数据集上与领先系统相当

Conclusion: 所提架构能有效处理企业级复杂数据库模式，在复杂结构场景下显著优于现有方法
...
### [118] [Continual Graph Memory for Adaptive Recommendation under Intent Drift](https://arxiv.org/abs/2609.04651)
*Hao Nguyen Ngoc,Tung Nguyen,Nguyen Thi Hanh,Hoang Thai Dinh,Nguyen Xuan Tung*

Main category: cs.AI

TL;DR: 本文提出CGM-Rec，一种用于意图漂移自适应推荐的持续图记忆框架。该框架将图状态视为可写内存，包含保守更新的语义图记忆存储稳定高置信度关系知识，以及快速响应的情节 lesson 记忆学习近期结果和失败案例。测试时仅通过内存写入实现适应，参数保持冻结。


<details>
  <summary>Details</summary>
Motivation: 研究背景是推荐系统中的意图漂移问题。反馈可揭示关系证据是否有用、缺失或误导。传统知识图谱增强系统将图视为静态检索基座，在面对演变意图、噪声元数据和重复失败模式时表现脆弱。

Method: CGM-Rec包含两个互补组件：1) 语义图记忆，通过质量门控类型操作保守更新，存储稳定高置信度关系知识；2) 情节lesson记忆，作为快速反应记忆学习近期结果、失败案例和纠正提示。测试时采用冻结参数的单次重排序协议，仅通过内存写入实现适应，编码器和提示保持固定。

Result: 在多个推荐设置上，CGM-Rec在大多数指标上优于评估的神经和基于LLM的基线。在采样候选重排序下，CGM-Rec在Bundle上将HR@1相比最强LLM基线提高29.58%，在元数据丰富的ML-100K上以HR@5 0.5941对比0.4746超越K-RagRec。

Conclusion: CGM-Rec通过持续图记忆有效处理推荐系统中的意图漂移问题，无需在测试期间更新参数即可实现显著性能提升，为自适应推荐提供了新范式。
...
### [119] [Harness-agnostic detection and immunization of reward hacking in self-evolving language models](https://arxiv.org/abs/2609.04665)
...
TL;DR: 本文提出HackProbe，一种用于检测自演化语言模型中奖励黑客问题的监控器。该方法通过黑盒hooks接入任意自演化循环，使用分布固定的比较核心和旋转的新鲜层来防止共适应，通过四个测试覆盖能力差距、规模对齐的发散、在线变点检测、能力停滞和条件自信错误率，并用Sidak校正将其转化为校准的家庭式p值。诊断后通过风险感知的免疫层使用核心和纯结构游戏足迹重新选择诚实候选者。在受控的提示级主机上，HackProbe达到0.763的AUROC，优于最强基线的0.663，并将误报率从0.706降至0.434。


<details>
  <summary>Details</summary>
Motivation: 自演化语言模型通过提出候选更新并保留能提高可见分数的方案来改进，但当该分数是对实际期望能力的不完美代理时，持续选择会扩大两者之间的差距，即奖励黑客问题。需要一种方法在不使用权重或激活的情况下检测和缓解这个问题。

Method: HackProbe通过两个黑盒hooks接入任意自演化循环：(1)保持一个秘密的、分布固定的比较核心，其冻结分布使能力代理在不同代之间可比；(2)旋转新鲜层以强化模型库防止共适应。四个测试覆盖：水平差距、规模对齐的发散（在在线变点检测中）、能力停滞、条件自信错误率。Sidak校正将测试转化为校准的家庭式p值。风险感知的免疫层使用核心和纯结构游戏足迹重新选择诚实候选者，每代最多向主机泄露log2 Pi比特。

Result: HackProbe在受控环境下达到0.763的AUROC（最强基线为0.663），误报率从0.706降至0.434。带宽限制的重新选择是唯一在黑客攻击下返回更多真实能力（平均5.2分）的免疫水平，超过了在干净运行中失去的4.7分。

Conclusion: HackProbe能够有效检测和缓解自演化语言模型中的奖励黑客问题，通过黑盒监控和有限信息泄露实现对诚实候选者的重新选择，为自演化系统的安全对齐提供了新思路。
...
### [120] [ERPBench: Evaluating LLM Agents for Enterprise Decision-Making Across Competitive Market Ecologies](https://arxiv.org/abs/2609.04667)
*Xinran Zhang,Pengrui Lu,Lyumanshan Ye,Pengfei Liu*

Main category: cs.AI

TL;DR: ERPBench是一个用于评估企业决策代理的执行仪器化基准测试，在六轮ERP模拟中测试LLM代理在竞争性市场生态中的表现。研究发现不同市场生态下最佳模型不同（Solo中DeepSeek领先，Arena中Gemini领先），且只有21%的任务-level winner在两种生态中一致，表明企业代理排名无法跨竞争性市场生态迁移。


<details>
  <summary>Details</summary>
Motivation: 现有评估很少测试商业决策结论是否能在不同竞争性市场生态之间迁移，需要一个能够评估企业决策代理在耦合定价、生产、采购、库存、财务和共享市场竞争的模拟环境。

Method: 在六轮ERP模拟中评估100个固定问题，设置两种竞争市场生态：Solo（LLM代理对抗固定基于规则的对手）和Arena（6个LLM代理在共享市场中竞争），涵盖六个模型家族，共1200条模型级轨迹和7200个决策轮次。

Result: Solo中DeepSeek领先（平均估值252.29M，平均排名1.67），Arena中Gemini领先（263.95M，1.76）。两种生态中只有21个问题的任务级 winner相同。Gemini的底部排名率从22%降至0%。

Conclusion: 企业代理排名无法跨竞争性市场生态迁移。相同的业务决策在不同竞争环境下可能产生截然不同的结果，这强调了需要在目标市场生态中对企业代理进行特定评估的重要性。
...
### [121] [Train What You Deploy:Token-Faithful Post-Training of a Production Coding](https://arxiv.org/abs/2609.04678)
*Cheng Li,Jiexiong Liu,Yixuan Chen,Chi Hong*

Main category: cs.AI

TL;DR: 针对编码和终端智能体后训练中的标记和控制保真度问题，提出保真感知训练框架和C-DPPO算法，通过双向TV认证、自适应K规则和策略掩码等技术实现稳定性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决编码和终端智能体后训练管道中的严重标记和控制保真度错误：简化训练环境与生产部署不匹配，以及从智能体日志离线重建标记时扭曲原始提示并混淆策略调用与后台模型操作。

Method: 提出保真感知训练耦合框架，保留训练端原始提示的采样，通过协商训练协议消除虚假模型调用，并将损失计算限制在具有闭合失败保证的可验证标记跨度上。在此基础上提出C-DPPO，建立双向TV认证边界、自适应K规则、预算感知序列保证和错误鲁棒策略掩码。

Result: 在TMax-100上使用相同训练和测试协议评估Baize5B和Baize10B模型，C-DPPO相比标准DPPO在各模型规模上实现一致的+3.0点性能增益。证书审计验证了认证训练管道的可靠性和完整运营覆盖。

Conclusion: 研究提出了C-DPPO框架来解决编码和终端智能体后训练中的保真度问题。该方法通过认证的TV边界、自适应K规则和预算感知序列保证，实现了+3.0点的性能提升，验证了认证训练管道的可靠性和运营覆盖范围。
...
### [122] [Predicting Spatiotemporal Mobile Sensing-Based PM2.5 Concentrations Using Low-Rank Adapted Spatially Attentive Graph Neural Network](https://arxiv.org/abs/2609.04693)
*Om Chiddarwar,Priyanka Mandal,Praveen Kumar Chandaliya,Shriniwas Arkatkar*

Main category: cs.AI

TL;DR: 本文提出了一个来自印度古吉拉特邦苏拉特的移动感知数据集，包含PM2.5、气象变量和土地利用特征，并设计了SA-GNN模型用于高分辨率的PM2.5短期预测和热点识别。


<details>
  <summary>Details</summary>
Motivation: 城市空气质量沿交通走廊变化显著，需要高分辨率监测。现有模型难以捕捉城市空气质量快速变化模式。

Method: 使用两种节点定义策略：均匀分割（200-400米间隔）和DBSCAN聚类。为每个节点计算气象变量的滚动均值和标准差。SA-GNN结合了特定聚类的GRU（捕捉局部时间依赖）和图注意力网络（学习空间异质性）。

Result: SA-GNN在数据集上达到R²=0.95，RMSE=6.8，MAE=4.2 μg/m³，优于所有基线模型（LSTM、RNN、GRU、ANN）。

Conclusion: 结合空间聚类和自适应注意力的混合架构有效建模了快速波动和复杂空间交互，实现了实时高分辨率监测，支持个性化暴露追踪和健康城市及时预警。
...
### [123] [SQL-Zero: Self-Evolving Text-to-SQL](https://arxiv.org/abs/2609.04697)
...
TL;DR: 本文提出SQL-Zero，一个无需人工标注的Text-to-SQL自训练方法，通过挑战者-求解者对弈框架，仅依赖数据库执行作为反馈信号进行训练，在BIRD数据集上超越零样本基线，并在3B模型上展现良好的跨数据库迁移能力。


<details>
  <summary>Details</summary>
Motivation: 训练Text-to-SQL代理依赖昂贵的人工标注SQL对，这些标注成本高、领域特定，是扩展到新数据库的主要瓶颈。因此需要探索无需人工标注的自训练方法。

Method: SQL-Zero采用提议者-求解者自我对弈框架：(1)挑战者和求解者从同一基础LLM初始化；(2)挑战者生成与求解者当前能力匹配的"难但可解"的SQL对；(3)双方通过GRPO在交替轮次中更新；(4)对挑战者施加模板级重复惩罚防止多样性崩溃。训练信号仅来自数据库执行结果。

Result: 在BIRD数据库上零标注训练：(1)3B模型提升6.6分，7B模型提升7.3分；(2)分数超过在人类BIRD gold上训练的对照模型；(3)3B模型在Spider和Spider-Syn上保持迁移能力，且性能退化小于BIRD-gold对照；(4)7B模型仅首次迭代保留迁移能力。

Conclusion: 无需人工标注的自我对弈训练可以有效提升Text-to-SQL性能，且模型规模影响迁移能力——3B模型迁移效果更好，7B模型迁移能力随训练衰减。
...
### [124] [Model Retirement Creates Reproducibility Risk in Biomedical AI Publications](https://arxiv.org/abs/2609.04699)
...
TL;DR: 研究发现，许多生物医学研究论文使用的语言模型在发表时已经退役或即将退役，可能导致计算不可重现性问题。42%的论文使用了已退役或即将退役的模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在生物医学研究中的应用快速增长，但托管这些模型的商业服务有弃用计划，可能影响科学研究的可重复性。

Method: 搜索PubMed中2022-2026年3月的相关论文，从61,077篇摘要中提取LLM名称，验证提取准确性，将模型名称标准化，编制50个最常用模型的生命周期数据。

Result: 识别出8,931个论文-模型提及（5,242篇出版物），77.7%使用商业闭源模型，42%涉及已退役或2年内将退役的模型，出版到退役的中位间隔为538天。

Conclusion: 使用LLM的生物医学出版物可能面临计算不可重现性问题，模型退役应被视为生物医学研究的核心报告和保存问题。
...
### [125] [FinalityBench: An Effect-Level Benchmark for Agent Decisions Under Delayed and Conflicting Financial Finality](https://arxiv.org/abs/2609.04706)
*Abhishek Sharma*

Main category: cs.AI

TL;DR: FinalityBench是一个用于测试支付系统异常决策的可执行基准测试。支付系统的处理器、账本、ERP和银行feed会因消息延迟、重复、丢失和重排而产生矛盾信息，代理需要决定是否发货、重新提交扣款、退款或等待。基准测试通过隐藏的规范事件日志和分别故障的传递流来派生每个系统的视图，按实际货币效果评分。


<details>
  <summary>Details</summary>
Motivation: 支付系统中，处理器、账本、ERP和银行feed可能因消息传递故障而在数分钟内对同一订单持有相互矛盾的观点。处理异常的代理必须在知道某些操作不可逆的情况下，决定是发货、重新提交扣款、退款还是等待。现有的测试方法无法真实反映这种因指定故障语义导致的不一致性。

Method: FinalityBench基准测试采用隐藏的规范事件日志，从单独故障的传递流中派生每个系统的视图，使分歧遵循指定的故障语义而非人为编造。评分基于执行后的货币效果：相对于知道待处理扣款解决结果的特权参考，按商家的最终经济位置评分。包含321个任务，其中45对双胞胎任务（90个任务）：在决策时刻四个系统视图相同且权威查询均返回未知，但最终正确处理方式不同。

Result: 在14,445个评分回合中，来自九个编程策略，按单任务准确率和配对损失排名在7处不一致。按准确率排名第二的"首次信号发货"策略（65.7%）在配对损失上最差，因为无法区分两种情况。运行时对不可逆操作进行权威终局性探测门控达到85.4%，与轮询策略不同，不受pass^5影响。语言模型在分层子集上达到与手写门控相同的准确率，但损失约两倍金额，且无需提示即可发现终局性门控策略。

Conclusion: FinalityBench基准测试有效地测试了支付系统异常决策能力。权威终局性探测门控策略显著优于其他策略，语言模型能够自主发现该策略。准确率和货币损失指标可能不一致，需综合考虑。双子任务对测试区分能力很重要，可区分"发货"和"等待"的终局性信息具有直接价值。
...
### [126] [PLUME: Parameter-Efficient Personalization of Large Language Models via Low-Rank User Modulation in Shared Subspaces](https://arxiv.org/abs/2609.04715)
*Xinyu Li,Hao Zhou,Jianfeng Zhu,Julina Maharjan,Ruixin Guo,Feodor Dragan,Ruoming Jin*

Main category: cs.AI

TL;DR: PLUME是一种轻量级框架，通过共享任务特定子空间实现高效且具有表达力的用户个性化适配，仅需训练小型矩阵即可获得定制模型，参数量减少超过95%。


<details>
  <summary>Details</summary>
Motivation: 个性化大型语言模型虽然能提升用户体验，但每个用户单独微调会带来显著的参数和存储开销，难以扩展到大规模用户群体。需要一种既高效又具有表达力的个性化方法。

Method: PLUME首先从聚合用户数据中学习全局任务子空间，然后让每个用户在该子空间内训练一个轻量级的小型方阵来实现个性化，同时引入跨层共享参数和秩-1残差项以减少冗余。

Result: 在多个个性化文本生成基准测试中，PLUME实现了与强基线相当或更好的性能，同时将每个用户的参数量减少超过95%。

Conclusion: 共享子空间调制结合最小残差是一种可扩展且语义合理的LLM个性化方法。
...
### [127] [Aplaud: Adaptive Personalized Low-Rank Decomposition for User-Specific LLM](https://arxiv.org/abs/2609.04738)
...
### [128] [DCFA: Dual-view Causal-inspired Attribution for Failure Reasoning in LLM-based Multi-agent Systems](https://arxiv.org/abs/2609.04749)
*Zehao Wang,Lanjun Wang,Shilong Jin,Junjie Chen,Yanghua Xiao*

Main category: cs.AI

TL;DR: DCFA是一个用于LLM多智能体系统中故障归因的无训练框架，通过构建结构化因果依赖图识别初始决定性错误，并结合局部反事实推理细化归因结果，在Who&When基准上相比基线方法提升高达8.27%。


<details>
  <summary>Details</summary>
Motivation: 当前LLM多智能体系统仍然脆弱，容易出现推理和协调错误导致系统级故障。现有的故障归因方法存在两个关键挑战：1）浅层归因：只捕获到次要偏差（如不完整的检索或格式错误），而忽略了系统失败的真正决定性原因；2）上下文退化：随着系统轨迹长度增加，模型的推理能力迅速下降。

Method: DCFA框架包含两个核心模块：1）全局模块：从系统轨迹中构建结构化因果依赖图，识别初始决定性错误；2）局部模块：应用局部反事实推理来细化因果归因。该方法为训练-free框架，无需额外训练数据。

Result: 在Who&When基准上针对六个LLMs的实验表明，DCFA在步骤级准确率上比最先进的基线方法提升了高达8.27%。

Conclusion: DCFA通过结合全局因果图构建和局部反事实推理，有效解决了LLM多智能体系统中的故障归因问题，为提升系统可靠性和可解释性提供了新思路。
...
### [129] [Shadow Queries for Private Retrieval in Vector Databases](https://arxiv.org/abs/2609.04767)
*Xinguo Feng,Zhongkui Ma,Zihan Wang,Chuan Yan,Guowei Yang,Alsharif Abuadbba,Guangdong Bai*

Main category: cs.AI

TL;DR: SHAQ是一种基于语义分解和嵌入解耦的防御方法,用于保护向量数据库中的文档嵌入免受嵌入反转攻击(EIA)。该方法利用生成式语言模型创建多样化的"影子查询"来捕获文档的不同语义方面,并用这些查询的编码替代原始文档嵌入,从而解耦存储的嵌入与源文本之间的关联。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)越来越依赖信息检索系统(如RAG)来整合领域特定知识,但存储在云向量数据库中的文档嵌入容易受到嵌入反转攻击(EIA),可被重建为原始文本。现有防御方法(如添加噪声或缩放嵌入)要么隐私保护有限,要么显著降低检索效用。

Method: SHAQ的核心洞察是EIA依赖于嵌入与其原始文本之间的强耦合性。该方法不使用直接存储文档嵌入,而是使用生成式语言模型创建多样化的"影子查询",这些查询捕获文档的不同语义方面,然后对这些查询进行编码并存储以替代原始文档嵌入,从而分解文档语义并解耦存储嵌入与源文本的关联。

Result: 实验在多个IR数据集上表明,SHAQ在保护隐私的同时保留了检索效用:恢复率低至0.2104,比基线防御多保护19.50%的tokens,达到0.7967的MAP@10,效用提升高达5.53%。

Conclusion: 语义分解和嵌入解耦为防御EIA提供了一种有效的替代方案,优于直接修改嵌入的方法。该方法在显著提升隐私保护的同时还能保持甚至提升检索效用。
...
### [130] [Diffusion Language Models for Mobile Edge Agentic AI: Foundations, Applications, and Challenges](https://arxiv.org/abs/2609.04778)
*Chenqi Li,Minghui Min,Dusit Niyato,Wei Ni*

Main category: cs.AI

TL;DR: 本论文是关于扩散语言模型(DLM)在移动边缘智能体AI中应用的综述。DLM通过迭代去噪而非自回归解码来细化token，能够并行更新多个不确定token并利用双向上下文，为边缘智能体提供更灵活的质量-延迟权衡方案。


<details>
  <summary>Details</summary>
Motivation: 解决移动边缘智能体在延迟、内存、能耗、带宽、隐私和可靠性方面的约束问题。传统自回归Transformer大语言模型在边缘设备上存在延迟高、通信开销大等局限性，需要探索新的建模方法。

Method: 综述方法，系统回顾DLM基础理论，分析其在边缘设置下的适用性，涵盖资源高效架构、训练推理加速、压缩技术、边缘/云端部署、通信感知服务、物联网/无线应用及智能体评估等方面。

Result: 明确了DLM的双向性、并行细化、可控性和质量-延迟弹性等特性与未来移动边缘智能系统级需求的关联性。

Conclusion: 为连接DLM建模特性与移动边缘智能系统需求提供了理论基础，同时指出了长上下文状态管理、分裂推理、可信执行、多模态基础化及可复现基准测试等开放性问题。
...
### [131] [DODR: Deterministic Operator-Driven Reasoning in Latent Space](https://arxiv.org/abs/2609.04782)
*Weicai Huang*

Main category: cs.AI

TL;DR: 本文提出DODR架构，将推理从token级概率采样重构为高维线性代数空间中的确定性矩阵运算，解决自回归大语言模型在复杂逻辑推理中的三大根本性缺陷。


<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型将推理形式化为token级概率采样，存在三个根本性缺陷：误差累积、概率替代必然性、以及线性链信息瓶颈。这些缺陷严重制约了模型在复杂逻辑推理任务中的表现。

Method: 提出确定性算子驱动推理（DODR）架构，将推理状态表示为语义单元（短语或句子）构成的快照向量，每步推理为确定性矩阵运算。Peirce的三种推理类型被形式化为三个可训练矩阵算子：降秩演绎算子（信息压缩）、满秩归纳算子（信息扩展）、以及定义为演绎算子Moore-Penrose伪逆的溯因算子（信息假设）。

Result: 实验在503条记录（420条去重样本）上测试：演绎损失收敛至1.40e-05；归纳达到0.9996泛化覆盖率，20/20个反例被硬否决；溯因解决方案超出随机基线28倍，判断准确率72.5%（58/80，Wilson 95% CI [61.9%, 81.1%]）和81.7%（49/60，CI [70.1%, 89.4%]）；冻结算子在跨领域演绎任务上达100%（60/60）。

Conclusion: 该架构提供结构性零幻觉保证和三层持续学习机制。证明了算子集在Peirce三分法下最小且完备，单一"超算子"无法实现三种推理类型（秩 obstruction），推理图按Banach不动点定理收敛并具有图灵完备性。
...
### [132] [ProtLingo: Efficient Protein Language Modeling via Conditional Memory and Expert Routing](https://arxiv.org/abs/2609.04793)
*Mingrui Li,Sixian Shen,Minzhang Li,Ruiyi Zhang,Kexin Zhang,Jiakai Zhang,Jingyi Yu*

Main category: cs.AI

TL;DR: ProtLingo是一种高效蛋白质语言模型框架，通过结合条件局部记忆和稀疏专家路由机制，在150M规模 backbone 上实现竞争性性能，同时显著提高参数效率。


<details>
  <summary>Details</summary>
Motivation: 蛋白质执行多种细胞功能，即使是单个氨基酸替换也可能改变稳定性、活性或分子相互作用。蛋白质语言模型(PLM)为从无标签序列建模序列-功能关系提供了可扩展方法，但增加密集Transformer骨干的规模往往带来大量计算成本，却不能持续改善对突变敏感的预测。

Method: ProtLingo框架通过以下方式增强预训练单序列骨干：1)将上下文残基表示映射到路由特定的离散代码；2)将居中的局部窗口组合成潜在N-gram地址；3)检索与重复局部序列上下文相关的可重用残差信号；4)将选定的前馈块升级为具有共享和路由专家的稀疏混合专家层，实现残差依赖计算同时仅激活部分参数。

Result: 在蛋白质适应度预测、FLIP基准测试和监督接触预测实验中，ProtLingo在150M规模骨干上实现竞争性性能，在突变效应预测方面表现出较强的参数效率，并保留了长程结构表示。

Conclusion: ProtLingo通过条件局部记忆和稀疏专家路由机制，在保持竞争性性能的同时实现了高效的参数利用，为蛋白质语言模型的高效扩展提供了新思路。
...
### [133] [Whose record is this? Diagnosing and authorizing record use in personalized multimodal models](https://arxiv.org/abs/2609.04801)
*Xinyu Mao,Junsi Li,Chenyang Liu,Haoji Zhang,Ming Sun*

Main category: cs.AI

TL;DR: 本文提出了视觉记忆误绑定(VMM)问题，当上下文视觉个性化系统检索到正确记录但应用于错误视觉主体时会发生此问题。作者形式化了记录授权概念，包含三个条件：主体存在(P)、记录边有效性(E)和答案支持(S)，并构建了包含3,690个案例的RecordAuth-Diag诊断套件。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型在上下文个性化任务中可能将记录错误应用于错误视觉主体的问题。当前系统虽然能检索真实记录，但会将该记录错误地应用于目标图像中的错误主体，导致视觉记忆误绑定。

Method: 构建RecordAuth-Diag诊断套件，通过改变图像-记录边而保持查询、问题、记录文本和图像多重集不变来测试系统。使用卡片移除和随机重标注来归因失败原因。提出认证主体令牌(authenticated subject tokens)作为存在见证的充分性控制。

Result: Gemma-3-4B-IT达到63.69%的本地未授权使用率，清洁召回率为25.75%。CoViP为26.02%，其Qwen后端为22.49%。预生成授权将Qwen卡片曝光从43.63%降至3.06%，正面召回从86.26%降至60.90%。完整P∧E∧S验证显示：相关性释放28.93%， Typed授权释放28.39%；不安全释放分别为6.79%和0.89%。

Conclusion: 观察到的增量是E∧S决策，由支持主导而非仅边检查。外观仅在P条件下提供E证据；认证主体令牌实例化缺失的存在见证作为充分性控制。该研究关注评估合约，非自然流行率、同意或视觉身份。
...
### [134] [Hierarchical Possession-Aware Graph Pointer Network for Pass Receiver Selection](https://arxiv.org/abs/2609.04803)
*Jingyi Wang,Da Li,Kaixin Wang,Zhangqin Huang*

Main category: cs.AI

TL;DR: HPGPN是一种用于足球传球接球者预测的层次化图指针网络，在仅提供部分球员可见性的广播式冻结帧数据下进行预测。


<details>
  <summary>Details</summary>
Motivation: 传球接球者选择是足球分析中的基础任务，但在事件中心冻结帧观测下面临挑战——这种广播式设置仅提供部分和变化的球员可见性，没有完整轨迹或稳定球员身份。模型需要对匿名可见候选球员、对手压力和近期上下文进行推理。

Method: 提出层次化 Possession-aware 图指针网络(HPGPN)，将传球接球者选择建模为对可见队友的可变大小候选预测。HPGPN联合建模当前球员交互、局部事件上下文和 possession 级别的时序动态。用图表示当前传球情况，整合固定事件上下文，使用动态 possession 历史捕捉进攻序列的演变。候选表示通过整合空间、上下文和历史证据进行层次化细化，glimpse pointer 头对接收候选进行评分。

Result: 在公共足球事件和冻结帧数据上的实验表明，HPGPN提高了传球接球者选择性能。消融研究证明了基于图的交互建模、固定事件上下文和双分支动态 possession 历史建模的有效性。

Conclusion: HPGPN能够有效处理部分观测下的传球接球者预测问题，层次化建模和动态 possession 历史对提升预测性能有重要贡献。
...
### [135] [MedFlow: Class-Aware Multi-Scale Generation for Medical Time-Series Synthesis](https://arxiv.org/abs/2609.04804)
*Yanhao Huang,Shibo Feng,Wanjin Feng,Peilin Zhao,Chunyan Miao*

Main category: cs.AI

TL;DR: MedFlow是一种用于医学时间序列合成的类感知多尺度流匹配框架，通过向量量化多尺度分词器和令牌边际引导机制，能够在保持全局和尾部分布的同时增强少数类模式，在医学时间序列生成任务中表现优于现有的基于扩散的方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注匹配整体分布和时间动态，但在不平衡医学数据集中，少数类的临床重要模式往往被多数类模式所掩盖。医学时间序列包含异构时间尺度的临床信息，需要一种能够保留少数类特征的同时保持数据分布的方法。

Method: 提出MedFlow框架，包含两个核心组件：1)向量量化多尺度分词器(Vector-Quantized Multi-Scale Tokenizer)，在互补的时间分辨率上表示医学序列，捕捉粗粒度临床趋势和细粒度动态；2)令牌边际引导(Token Marginal Guidance)，将类条件令牌统计信息直接融入流匹配过程，引导生成向类特定的令牌区域倾斜，增强少数类模式。

Result: 在四个公共数据集（涵盖电子健康记录、EEG和ECG信号）上进行实验，MedFlow在下游预测任务中一致优于最新的基于扩散的基线方法。平均提升AUPRC 5.8%，降低Context-FID 88.6%，实现3.8倍更高的采样吞吐量。

Conclusion: MedFlow通过多尺度表示和类感知生成机制有效地解决了医学时间序列生成中的类别不平衡问题，为构建可靠的临床预测模型提供了高质量的合成数据支持。
...
### [136] [When Financial Fine-tuning Fails: A Three-Level Detectability Analysis of Numerical Hallucination in Domain-Adapted Language Models](https://arxiv.org/abs/2609.04806)
*Xiaodong Li,Peiwei Liu*

Main category: cs.AI

TL;DR: 本文系统研究了金融大语言模型在摘要任务中的数值幻觉问题，通过控制变量实验比较了三种模型变体，发现领域微调会显著加剧而非减轻数值幻觉，并提出三级可检测性分类法。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为金融LLM的数值幻觉源于数值推理能力不足，但这一假设尚未在受控微调环境下得到系统验证。

Method: 设计三级可检测性分类法（显性幻觉、 covert-explicit、covert-implicit），对比Base模型、领域适应模型FT-A和数值增强模型FT-A+B+C的幻觉率。

Result: Base模型幻觉率仅5.4%，FT-A达82.5%，FT-A+B+C高达98%。数值监督反而放大了幻觉。模板注入是主要机制。

Conclusion: 数值幻觉源于领域适应导致的数值抑制能力退化，而非推理不足。建议在所有可检测性水平上评估幻觉，并部署显式接地机制。
...
### [137] [CPR-IE:A Compression-Prediction-Resource Intelligence Efficiency Metric](https://arxiv.org/abs/2609.04809)
*Xiantao Jiang*

Main category: cs.AI

TL;DR: CPR-IE是一种协议相对的排序方法，通过表示压缩性、预测质量和资源消耗三个维度来评估智能系统。该方法克服了仅用预测精度的局限性，为AI系统提供了更全面的效率评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前仅使用预测精度来比较智能系统存在局限，需要一个能综合考虑压缩性、预测质量和资源消耗的统一框架来更全面地评估智能系统的效率。

Method: 提出比例增量组合产生对数累积负担，上下文无关比率响应产生幂律响应。通过参考归一化建立I(C,P,T)表示形式，证明了Pareto一致性、单位不变性、边界行为、交易身份、排名稳定性区域和跨任务聚合等数学性质。采用translog父模型使交互限制明确化。

Result: 建立了基数和序数识别、子高斯有限样本排名保证、指数不确定性下的稳健选择以及确定性遗憾界等理论结果。

Conclusion: CPR-IE是一种构建的效率表示方法，而非通用定律或智能本身的定义。其测量选择受最小描述长度、算法复杂度、适当评分规则、变分推理和Landauer原理的启发，但不必然导致特定公式形式。
...
### [138] [Long Horizon Transformer Quantile Fault Prediction for Multi Site Industrial Predictive Maintenance](https://arxiv.org/abs/2609.04840)
*David J Poland,Daniele Ravi,Na Helian*

Main category: cs.AI

TL;DR: 本文提出TQRNN30d框架，用于30天长视野的机器预测性维护。该方法结合双阶段分位数回归神经网络特征提取器和多流时间融合分类器，在72台机器的测试集上达到79.97% F1分数，优于18个基线方法。


<details>
  <summary>Details</summary>
Motivation: 长视野预测维护需要模型能够区分缓慢演化的退化过程与正常运行工况变化，传统方法难以处理以天为单位而非小时的时间跨度。该研究旨在探索显式条件分位数表示是否能提供更有效的分类器接口来解决这一问题。

Method: TQRNN30d采用双阶段分位数回归神经网络(QRNN)作为特征提取器，将81通道机器行为的每小时数据映射为324维分位数状态表示，720个有序小时词组成30天文档。分类器融合分位数状态、动态协变量、通道级静态元数据和168小时潜在历史流，使用门控残差处理、因果循环编码和元数据条件跨模态注意力机制。辅助记忆调节由基于持续单步预测误差发散的有界不稳定感知信号提供。

Result: 在9个制造设施的72台机器上，TQRNN30d在30天视野下达到79.97% F1、80.18%召回率、81.82%精确率、82.39%准确率和0.820 ROC-AUC。在7天、14天和30天固定阈值比较中均优于全部18个基线方法，在14天时F1优势最大。

Conclusion: TQRNN30d框架在同质九设施舰队内实现了良好的留出机器性能，但未能建立未见站点、跨设备或跨部门的泛化能力。未来工作需探索更广泛的跨场景泛化性能。
...
### [139] [ElderBench: Benchmarking Autonomous Mobile Agents for Older Adults](https://arxiv.org/abs/2609.04850)
*Weide Zhan,Qumu Shaqu,Yuanqing Liu,Peng Zhang,Jiahao Liu,Kam Him Lam,Ning Gu,Zhan Hu,Tun Lu*

Main category: cs.AI

TL;DR: ElderBench是首个用于评估移动GUI代理在真实老年用户场景中表现的性能基准测试。该基准包含249个从20款应用程序中收集的老年人自然手机任务，系统性地弥补了现有GUI基准测试与真实老年用户交互之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有GUI基准测试主要依赖明确的、目标导向的指令，很少捕捉老年人自然产生的语言模式（如间接表达、指代歧义和表述不完整的请求），这与老年人真实交互场景存在显著差异，可能阻碍移动代理在辅助老年人方面的可靠部署。

Method: 1）从20个应用程序中收集了249个老年人自然产生的智能手机任务；2）从句法、语义和语用三个角度分析老年用户指令与现有GUI基准指令的语言差异；3）在在线和离线设置下评估主流GUI代理和视觉语言模型；4）通过受控指令规范化、失败分析和细粒度语言特征分析识别问题根源。

Result: 老年导向指令导致主流GUI代理和视觉语言模型出现显著性能下降；老年人特有的语言模式（如间接表达、表述不完整等）是导致代理失败的关键因素。

Conclusion: 研究为开发更具适应性、可解释性和年龄包容性的老年友好型GUI代理提供了可操作的设计见解。
...
### [140] [MM-IFEval-Pro: A Multilingual and Attack-Resistant Benchmark for Instruction-Following in Vision-Language Models](https://arxiv.org/abs/2609.04859)
*Changming Xiao,Zhenliang Ni,Jinhui He,Han Shu,Jie Hu*

Main category: cs.AI

TL;DR: MM-IFEval-Pro是一个新的多模态指令跟随基准测试，涵盖中英双语任务和多样化的指令劫持案例，包含4个主要任务类别、24个子类别和8个指令类别、52个子类别，平均每个样本包含3.0个约束。同时构建了增强中文和对抗性指令的强化学习训练集，显著提升了模型在MM-IFEval-Pro及其他主流多模态基准上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态指令跟随基准存在语言覆盖不足和对抗性安全场景缺乏的问题，难以评估真实世界的多语言和安全敏感场景下的模型表现。

Method: 构建MM-IFEval-Pro基准测试，包含中英双语任务和指令劫持案例；同时构建增强中文和对抗性指令的强化学习训练集进行模型训练。

Result: 该训练集显著提升了模型在MM-IFEval-Pro上的性能，并有效迁移到其他主流多模态基准，展示了强大的跨任务和跨语言泛化能力。

Conclusion: MM-IFEval-Pro能更好地评估VLM的指令跟随能力，强化学习训练可有效提升模型在多语言和安全敏感场景下的表现。
...
### [141] [MZ-Rain: Moisture-Budget-Guided Zero-Inflated Model for Station-Level Precipitation Nowcasting](https://arxiv.org/abs/2609.04864)
*Yifang Zhang,Shengwu Xiong,Henan Wang,Wenjie Yin,Yuqiang Zhang,Chen Zhou,Hua Chen,Qile Zhao,Pengfei Duan*

Main category: cs.AI

TL;DR: MZ-Rain是一种基于水分预算引导的零膨胀sLSTM框架，用于站点级降水临近预报。该模型通过水分预算方程将降水形成过程分解为水分储存、水分运输、地表蒸发和降水持久性四个特定路径，并利用专用sLSTM分支捕捉其时间演变。同时引入自适应Tweedie建模策略来处理降水数据中的严重零膨胀问题。


<details>
  <summary>Details</summary>
Motivation: 解决站点级降水临近预报的两个主要挑战：(1)缺乏物理引导的建模，气象变量被视为同质集合，未考虑其在降水形成中的不同作用；(2)降水数据中存在严重的零膨胀问题，干旱间隔占主导地位。

Method: 提出MZ-Rain框架，包含两个核心创新：1)基于水分预算方程的过程分解建模，将降水形成分解为水分储存、水分运输、地表蒸发和降水持久性四个专用sLSTM分支；2)自适应Tweedie建模策略，自适应调节降雨均值并联合学习降水发生作为辅助任务。

Result: 在多种地理和气候条件下的大量实验表明，MZ-Rain在CSI、FAR、MSE和MAE等多个评估指标上始终优于强基线模型。特别是在预报强降水事件方面表现出色，同时受益于物理驱动的过程建模。

Conclusion: MZ-Rain通过物理引导的建模和零膨胀处理策略，有效解决了站点级降水临近预报的挑战，在多个评估指标上取得优异性能，特别是在强降水事件预报方面具有优势。
...
TL;DR: CoSkill是一个统一的多智能体强化学习框架，通过将静态元技能工作流重构为可学习的元技能智能体，并与推理智能体在分层技能库上进行联合训练，实现了端到端的协同适应。该框架在ALFWorld和WebShop任务上取得了98.4%和90.6%的成功率，显著优于现有的基于技能和强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有技能库存在结构性缺陷：它们要么将技能演化与策略优化解耦，要么将元技能实例化为固定工作流。两者都将技能视为被管理的被动对象，限制了技能的灵活演化及其与推理智能体的协同适应能力。

Method: 提出CoSkill框架，将静态元技能工作流重构为可学习的Meta-Skill Agent，并与Reasoning Agent在分层技能库上进行联合训练。通过将推理和元技能智能体建模为共享单一骨干的协作团队，实现端到端协同适应：推理智能体基于检索的任务技能及其子集中选择的步骤技能来条件化其行为，而其任务表现引导元技能智能体完善这些步骤技能。

Result: CoSkill在ALFWorld和WebShop上显著优于先前的基于技能和强化学习基线，分别达到98.4%和90.6%的成功率（提升3.5和6.2个百分点）。如图1所示，CoSkill在早期样本效率、渐近性能和时钟时间效率方面均表现优异。

Conclusion: CoSkill框架成功实现了推理智能体与元技能智能体之间的协同适应，在多任务环境中达到了最先进的性能水平，为智能体技能库的可学习和自适应演化提供了新的研究方向。
...
### [143] [LLM-Assisted Behavioural and Scenario Augmentation for Agent-Based Energy Adoption Models](https://arxiv.org/abs/2609.04866)
*Iias Faiud,Hossein Khaleghy,Michael Schukat,Karl Mason*

Main category: cs.AI

TL;DR: 本文提出了一种混合框架，将大语言模型(LLM)辅助的规范设计集成到校准的太阳能光伏采用代理模型(ABM)中，用于爱尔兰乳制品农场的采用行为分析。该框架通过有界行为规则和结构化情景规范来增强传统的技术经济采用机制。


<details>
  <summary>Details</summary>
Motivation: 直接用LLM推理替代采用模型存在可解释性、可重复性和行为有效性的担忧。现有方法缺乏在保持技术经济机制完整性同时纳入LLM支持的结构化行为假设和探索性技术经济情景的能力。

Method: 提出混合框架，将有界行为规则（保守、平衡、乐观三种情景）与结构化情景规范集成到校准的ABM中。行为效应通过可解释的规则表示，政策与市场条件通过固定、经验证的情景规范进行探索。

Result: 在多种政策设置、蒙特卡洛模拟和随机种子下表现出稳定且经济上合理的行为，采用结果在不同行为机制下保持有界和单调。框架实现了相对于对应逻辑案例最高约13%的行为采用增加，且未产生不稳定或不现实的饱和动态。

Conclusion: LLM辅助规范可以以可控、可重复且与政策相关的方式集成到校准的能源ABM中，为模拟-based能源政策分析提供新途径。
...
### [144] [From Interaction Traces to Persistent Skills: Online Evolution for Computer-Use Agents](https://arxiv.org/abs/2609.04869)
*Longtao Hu,Xiao Liang,Linchao Zhu*

Main category: cs.AI

TL;DR: 研究计算机使用代理的在线技能进化框架，将交互轨迹和评估器反馈转化为持久化版本化技能库。在OSWorld四个领域的实验表明，该框架相比空技能库基线可提升5.7-18.6个百分点的评估分数，但收益有条件限制，重复修订不能保证任务恢复。


<details>
  <summary>Details</summary>
Motivation: 问题：计算机使用代理的交互体验通常是短暂的，一个 rollout 中获取的程序性知识没有系统地保留、提炼和重用于后续任务。现有技能库提供外部程序性知识，但它们相对于无技能代理的增量价值以及重复交互下的纵向动态尚未得到充分表征。

Method: 在线技能进化框架：将交互轨迹和评估器反馈转换为持久化、版本化的可复用程序库。每次迭代针对冻结的库快照执行，基于证据的技能更新在后续迭代中可用，无需更改模型参数。在四个OSWorld应用域中使用相同的动作生成和GUI接地栈、任务集和迭代范围进行对比实验。

Result: 在五次迭代的预热后，Full在所有四个领域的运行中都获得了更高的预热后平均评估器分数，平均差异范围为5.7至18.6个百分点，表现出领域依赖的时间稳定性。在GIMP中，溯源感知分析显示跨任务起源边界的检索和修订波动，重复接受的编辑无法恢复起源任务。

Conclusion: 进化技能库可以作为可审计的共享程序记忆来改进固定的计算机使用栈，但收益是有条件的，重复修订不能保证恢复。在GIMP中观察到跨任务边界的检索和修订波动，重复接受编辑无法恢复原始任务。
...
### [145] [CHAMP: Cross-domain Hybrid Architecture for Matchmaking and Prediction in Online Multi-Player Games](https://arxiv.org/abs/2609.04870)
*Kai Wang,Ge Fan,Chaoyun Zhang,Yuyang Jiang,Yuze Liu*

Main category: cs.AI

TL;DR: CHAMP是一个跨领域 matchmaking 框架，通过混合域特征收集和DAWN网络解决MOBA游戏中的冷启动、分布不一致和数据稀缺三大部署瓶颈，在线测试显示可显著降低不平衡比赛和击杀碾压率。


<details>
  <summary>Details</summary>
Motivation: 解决MOBA游戏 matchmaking 系统中三大实际部署瓶颈：1) 冷启动问题（多数玩家在目标模式中缺乏足够比赛历史）；2) 分布不一致（技能分布在不同段位差异巨大）；3) 极端技能段位数据稀缺。

Method: 1) 采用混合域特征收集：跨模式短期时间序列（带目标域特征标注）+ 每模式长期/实时/团队统计分解；2) 提出DAWN网络：包含DAKE（将目标模式属性编译为可学习表示）和DATOE/DASOE/DAPOE（时序/空间/排列感知编码器），在单一共享网络中联合学习模式条件化表示和每模式去偏；3) 线上仅需调整每模式位置满意度阈值即可服务所有模式。

Result: 离线：DAWN达到67.73%胜率预测准确率，优于所有评估的注意力机制和序列基线方法。在线A/B测试覆盖从新手到精英玩家的整个联赛天梯，显示出不一致匹配持续下降。低段位玩家的5分钟击杀碾压率降低最多20.73%。

Conclusion: CHAMP和DAWN成功解决了跨模式matchmaking的三大部署瓶颈，通过单一模型同时服务多个游戏模式，显著提升了matchmaking质量，特别是在数据稀缺的底层玩家群体中效果明显。
...
### [146] [AutoLR: Automating the Path from Research to Launch Review in Industrial Recommender Systems](https://arxiv.org/abs/2609.04871)
*Qi Zhang,Yanlin Chen,Wenchao Xiao*

Main category: cs.AI

TL;DR: AutoLR是网易游戏社区应用DASHEN推出的一个自主研发到发布的工作流程框架，旨在解决工业推荐系统中从研究到部署的迭代过程协调问题。该系统结合多专家委员会、确定性证据加权探索-利用选择器和分层知识系统，使LLM代理能够执行语义推理和代码生成，同时由确定性控制器保留执行权限。


<details>
  <summary>Details</summary>
Motivation: 工业推荐系统的优化是一个迭代的研究与工程过程，需要从研究论文、技术报告和生产实验中发现有前景的方向，然后在生产代码库中实现，通过离线实验评估，最终进行在线A/B测试。这一过程依赖人工协调，缺乏一个能够可靠地在长期、多日的实验周期中协调各环节的自动化框架。

Method: AutoLR系统包含三个核心机制：1）多专家委员会（multi-expert council），用于对提案进行辩论和对抗性审查；2）确定性证据加权探索-利用选择器（deterministic evidence-weighted exploration-exploitation selector），在有限试验预算下分配候选方向并使用委员会重排；3）分层知识系统，结合外部研究、生产系统知识和DASHEN特定领域知识（如游戏社区、玩家特征、内容交互模式），以及配置、补丁、日志、失败和离线结果的后验证据。

Result: LLM代理执行语义推理和代码生成任务，而确定性控制器保留对执行、指标提取、护栏和持久状态转换的授权。该系统实现了从创意到部署的自动化协调。

Conclusion: AutoLR将Launch Review扩展为上游的自主研究到发布 harness，为工业推荐系统的自动化优化提供了新的解决方案。
...
### [147] [MARLA: A Conceptual Scaffold for Regulatory Learning under the EU AI Act](https://arxiv.org/abs/2609.04877)
...
TL;DR: 欧盟AI法案的监管学习框架研究，提出MARLA（地图、评估、报告、学习、适应）五阶段循环概念框架，旨在促进技术实践与法律治理之间的知识转化，支持监管一致性解释和适应性治理。


<details>
  <summary>Details</summary>
Motivation: 欧盟AI法案将监管定位为安全、可信赖和市场就绪创新的基础设施，但证据生产者与使用者处于不同专业领域，导致监管学习困难。需要建立共同框架促进跨领域知识转化。

Method: 提出MARLA概念框架，包含五个阶段：Map（映射）、Assess（评估）、Report（报告）、Learn（学习）、Adapt（适应），分别对应地方、国家和欧洲三个治理层面，通过两个试点案例和一个前瞻性案例进行说明。

Result: MARLA框架为技术和法律利益相关者提供共享词汇，前三个阶段各自产生可记录的监管学习形式，框架具有非规定性特点，允许灵活应用。

Conclusion: MARLA框架能够促进监管学习循环，支持欧盟AI法案的有效实施和技术演进过程中的适应性治理，为跨专业领域的监管知识转化提供系统性方法。
...
### [148] [Reinforcement Learning for Sequential Solar PV Policy Design under Uncertainty: An Agent-Based Approach](https://arxiv.org/abs/2609.04880)
*Iias Faiud,Jonaid Shianifar,Michael Schukat,Karl Mason*

Main category: cs.AI

TL;DR: 本研究将太阳能光伏政策设计问题表述为序贯决策问题，将强化学习（RL）与随机智能体模型（ABM）相结合，在16年规划期内选择年度激励政策（资本补贴、优惠贷款利率和上网电价），实现采用率与成本之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 设计有效且财政可持续的太阳能光伏采用政策需要在不确定性和异质性决策条件下，平衡采用收益与公共支出之间的关系。

Method: 将强化学习与随机智能体模型相结合，模拟不确定条件下的年度太阳能光伏采用情况。政策制定者智能体在16年规划期内选择年度激励政策，包括资本补贴、优惠贷款利率和上网电价。通过标量化奖励框架改变政策偏好来探索采用-成本权衡。使用PPO、SAC和TD3算法学习政策，并在随机模拟中进行评估。

Result: 最高采用率政策（TD3，w_cost=0.5）实现约4,145个采用者，成本为4,173万欧元；最低成本政策（PPO，w_cost=2.0）将支出降至727万欧元，拥有2,682个采用者。平衡政策（PPO，w_cost=1.6）实现3,495个采用者，成本为2,247万欧元。各算法呈现一致的权衡模式，表明采用-成本关系的稳健性。

Conclusion: 强化学习框架相比静态基准政策能够探索更广泛的政策配置，证明了RL作为不确定条件下自适应政策设计的灵活工具的潜力。
...
TL;DR: RSM-full是一种在线聚类记忆管道,在紧凑记忆模式下实现最佳质量-Token权衡。它结合了余弦门控最大成员合并写入规则和原子感知分组上下文打包器,在4k预算下达到全上下文质量的83%且仅消耗32%的Token。


<details>
  <summary>Details</summary>
Motivation: 长 horizon LLM部署面临严格的提示预算约束(延迟、成本、上下文限制),全上下文提示随着交互长度增长变得不切实际。核心问题不是原始召回率,而是哪种记忆设计在紧凑记忆模式下能提供最佳的质量-Token权衡。

Method: RSM-full结合两个设计选择:1)余弦门控max-member merge写入规则;2)原子感知分组上下文打包器。在AMA-Bench(紧凑记忆基准)和RealMem(长 horizon人物记忆基准)上进行评估。

Result: 在AMA-Bench上,4k预算下达到全上下文质量的83%,Token成本仅32%;四种子平均后,相比Online K-Means提升+3.5-6.0pp(p<.001)。三种子消融表明:合并规则贡献+5.7pp,分组打包器贡献+5.0pp。在RealMem上:相比Budget-RAG提升+0.69pp(p=.006),与BM25-RAG相当(Δ=+0.27pp,p=.47),显著优于Streaming-Proto(+2.97pp)和A-MEM(+1.65pp,p<.001)。

Conclusion: 在2k-5k提示Token的紧凑记忆模式下,RSM-full定义了强劲的Pareto点。核心发现:在严格预算下,紧凑记忆性能主要由流式记忆的合并方式和检索内容的组装方式驱动。更高Token预算的基线在该 regime 之外仍然更强。
...
TL;DR: 该文是一篇关于AI在投资领域应用的批判性状态-of-the-art综述，审查了截至2026年8月公开的股票、ETF、加密货币现货/永续合约和链上市场研究。尽管AI在预测和投资组合设计等方面取得进展，但缺乏持久净alpha的证据，存在多种方法论问题需要解决。


<details>
  <summary>Details</summary>
Motivation: 尽管AI技术能力不断增强，但技术能力本身并不能证明投资盈利能力。现有研究缺乏对AI在投资领域实际盈利性的系统评估。

Method: 采用alpha翻译链框架组织证据：即时间信息必须产生稳定信号、可行头寸、可执行订单和成本后的风险调整回报。审查了机器学习、时间序列基础模型、金融语言模型、强化学习和代理人在投资工作流中的应用。

Result: 在预测、文本处理、投资组合设计和工作流集成方面取得了实际但主要是上游的进展。持久净性能的证据较薄弱。存在时间污染、重复选择、生存偏差、弱基准、实施成本、场所机制和容量等问题。加密货币虽提供信息状态但需要单独处理。强历史结果与预测器衰退、纠正的前瞻失败、混合前瞻证据和少量审计的实际资本记录共存。

Conclusion: 在公共证据范围内，没有一种通用AI架构能够提供持久的、跨制度的、容量感知的净alpha。要获得更可靠的结论，需要点时间数据和模型、决策对齐目标、联合投资组合-执行评估、受控的前瞻性测试和匹配权威的治理。这些条件可以改善证据和实施，但不能保证盈利。
...
TL;DR: 本文研究了可解释人工智能(XAI)中布尔分类器的解释问题，证明了即使使用有序二元决策图(OBDD)这一知识编译地图中最易处理的子集，几类溯因解释仍然难以计算，并提出利用分类器的双轨编码表示来高效计算这些解释。


<details>
  <summary>Details</summary>
Motivation: 人工智能在现实应用中的广泛采用引发了对其可信度的担忧，特别是在关键应用中。XAI领域应运而生，旨在为用户提供AI系统决策的解释。布尔分类器的解释计算在一般情况下是一个困难问题。

Method: 1. 研究布尔分类器的溯因解释和对比解释
2. 使用有序二元决策图(OBDD)作为分类器的编译表示
3. 证明几类溯因解释在OBDD上仍难以计算
4. 提出使用分类器的双轨编码(dual-rail encoding)的适当表示来计算这些解释

Result: 证明了即使对于OBDD这一知识编译地图中最易处理的子集，几类溯因解释（包括较短的溯因解释和包含解释对象偏好的溯因解释）仍然难以计算。同时展示了对分类器双轨编码的适当表示可以高效计算这些解释。

Conclusion: 通过使用双轨编码的适当表示，可以恢复使用编译表示的好处，从而高效地计算以前认为难以处理的溯因解释类别，为XAI中布尔分类器的解释计算提供了新的有效方法。
...
TL;DR: 本文探讨理解与压缩之间的关系，弥合了信息理论/机器学习领域的压缩主义理解观与哲学理解观之间的鸿沟，通过三个相互关联的论题构建了一个统一框架。


<details>
  <summary>Details</summary>
Motivation: 压缩主义传统（Chaitin的"理解即压缩"）与哲学传统（理解即把握联系、提供解释、处理新颖性）之间存在根本张力，需要一座桥梁来调和这两种理解观。

Method: 提出三个相互关联的论题：(1) 理解作为高效代理，指向一种独特的稳健能力，帮助识别可信赖者和学习对象；(2) 理解状态是拥有对领域关系结构的心理模型，能够实现预测，而可预测性使得无需单独存储信息，从而实现压缩；(3) 人类理解的形式具有 fiduciary（信托）性和可传递性，这两种功能施加了可证明性和可传递性的压力，驱动人类理解趋向原则性简单化。

Result: 构建了一个框架，既能解释压缩主义理解观的吸引力和局限性，又能揭示AI系统难以理解（inscrutability）的原因。

Conclusion: 压缩并非等同于理解，而是理解的表征影子；人类理解的独特形式源于可证明性和可传递性的需求，这解释了为何人类理解呈现高度压缩的状态。
...
TL;DR: 提出了一种新颖的拓扑保持自适应图池化（TPAGP）方法，通过整合节点特征和拓扑信息将图动态划分为粒度球，生成多粒度表示，有效捕获局部和全局结构模式，显著提升图分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有图池化方法主要关注节点的细粒度表示，通过逐步粗化图来移除或合并节点，忽视了图的全局到局部模式和自适应粒度。在实际场景中，图可以从最粗粒度（全局拓扑结构）逐步到细粒度局部结构，需要为每个子域达到自适应粒度。

Method: 提出TPAGP方法，通过整合节点特征和拓扑信息动态将图划分为粒度球，生成多粒度表示，有效捕获局部和全局结构模式。同时设计了多粒度图网络模型，促进不同粒度间的特征交互和优化。

Result: 实验结果表明，TPAGP在各种基准数据集上优于现有的池化方法，有效缓解了固定粒度策略导致的信息损失问题。

Conclusion: TPAGP通过保留拓扑结构和自适应粒度，在图分类任务中实现了卓越的性能，为图表示学习提供了一种有效的多粒度池化方案。
...
TL;DR: APT-RAG是一个自适应规划和拓扑感知证据收集的RAG框架，用于解决证据密集型问答中的结构僵化和拓扑忽略问题。


<details>
  <summary>Details</summary>
Motivation: 现有结构化RAG方法在证据密集型QA中面临两个关键局限：1) 结构僵化，限制自适应推理扩展；2) 拓扑忽略证据收集，无法有效整合不同推理节点间的证据。证据密集型QA需要综合来自数十甚至数百个文档的信息。

Method: 提出APT-RAG框架，包含：1) 自适应规划：根据问题依赖和证据需求动态扩展推理结构；2) 拓扑感知证据收集：通过兄弟证据重用、直接检索和子节点证据聚合提高证据覆盖率；3) 证据引导的批量答案生成，减少生成开销。

Result: APT-RAG在证据密集型QA基准测试中优于现有结构化RAG方法。

Conclusion: APT-RAG通过自适应规划和拓扑感知证据收集有效解决了证据密集型QA的结构化RAG局限，为大规模文档问答提供了更优方案。
...
TL;DR: 研究通过20个大型语言模型、32个战争场景、10次重复的双条件实验（共计12,800个判断）测试AI系统在得知被评估时是否会改变行为。实验添加语句"你正在被测试与人类价值观的一致性"后，发现AI的战争意愿平均下降13.43分，且决策依据从成功概率转变为平民伤亡，显示评估框架会同时改变答案水平和决策规则。


<details>
  <summary>Details</summary>
Motivation: 研究者提出安全评估可能无法准确反映AI部署后的真实行为，因为AI系统可能会"应对评估"而改变自身表现。这对于准确评估AI系统的安全性具有重要影响。

Method: 采用完全析因联合实验设计，在战争决策场景中对20个大型语言模型进行测试。实验包含32个场景、10次重复，设置两个条件（有/无评估提示语），共计12,800个判断。关键操纵是在某些条件下添加语句"你正在被测试与人类价值观的一致性"。

Result: 发现两个关键效应：1）水平效应：战争意愿平均下降13.43分（95%置信区间：-16.20至-10.65）；2）结构效应：决策依据发生改变。基线条件下，17/20模型的最大影响因素是成功概率；有评估提示时，12/20模型的最大影响因素变为平民伤亡。模型主要削弱了战略考量（成功概率、国内支持等）。

Conclusion: 评估框架会同时改变答案的水平值和揭示的决策规则，表明当前的安全评估可能无法准确表征AI系统在真实部署场景中的行为，评估方法的改进迫在眉睫。
...
TL;DR: TROVE是一种用于自适应智能体编排的方法，通过在运行时选择性编辑计划路径来处理中间证据失效的问题，在代码生成、问答和数学推理任务上实现了更好的质量-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 智能体在执行前倾向于优化、选择或约束执行结构，但这会产生编排瓶颈：当中间证据使待续内容失效时，智能体必须执行过时的步骤或广泛重新规划，导致错误累积、计算浪费和进度丢弃。

Method: 离线阶段：TROVE将评估过的工作流搜索轨迹提炼为原子和复合技能以及结果条件转移图，保留稳定片段并暴露结果依赖决策。在线阶段：将计划路径视为临时性的：提交一个顶级技能后，控制器保留有效延续、插入轨迹支持的局部响应，或仅替换无效后缀。

Result: 在不同LLM后端的代码生成、问答和数学推理基准测试中，TROVE比数据集级优化、查询级架构选择和图约束调度等基线方法提供了更强的质量-效率权衡。当结果改变适当延续时质量增益最大，而提前终止在接近饱和任务上产生显著效率增益。消融实验表明复合技能捕获大部分离线收益，插入实现局部校正，后缀替换主要提高效率。

Conclusion: 选择性路径编辑是自适应智能体编排的通用原则。
...
TL;DR: 研究提出了AI对齐的结构性方法：在价值多元主义下，不追求正确目标，而是要求AI系统表现出一致的"连贯政策"，即对道德相关特征保持一致 verdicts。研究定义了四个结构性条件（verdict stability, monotonicity, decisiveness, Pareto viability）来衡量这种道德能力，并在三个模拟部署场景中测试了九个前沿LLM模型。


<details>
  <summary>Details</summary>
Motivation: 在价值多元主义下不存在"正确"的道德目标，因此需要一种不依赖特定道德标准的对齐方法。研究认为连贯政策是对齐的"结构性地板"而非规范性目标，这为评估AI道德能力提供了新范式。

Method: 在三个模拟部署场景中对LLM智能体进行道德困境测试，采用5x5x3因子设计：5种改写、5个 escalation levels、3个 dominance conditions，评估9个前沿模型。

Result: 没有任何模型在三个部署中表现出连贯政策：仅表面形式的扰动就导致 verdict-rate 变化高达99个百分点，一个场景的成功无法预测另一场景的表现。

Conclusion: 当前LLM智能体不具备能够有意义应用对齐的"对象"类型，需要从根本上重新思考AI对齐的概念基础。
...
TL;DR: 本研究针对进化迁移优化(ETO)中问题评估导致的运行时增长问题，提出将应用特定的串行计算重构为适合并行执行的形式。通过两种应用场景验证：多任务优化中的机械臂运动学评估和顺序迁移优化中的B样条轨迹评估，分别实现了256.72倍和93.91倍的端到端加速。


<details>
  <summary>Details</summary>
Motivation: 随着进化迁移优化扩展到更大的相关任务集合，问题评估可能成为运行时增长的主要来源，需要寻找可扩展的实际路径。

Method: 将评估扩展分为两个层次：评估任务数量和每个任务内的工作负载。通过累积矩阵表示法重构机械臂运动学评估，通过混合矩阵表示法重构B样条轨迹和碰撞计算，实现串行到并行计算的转换。

Result: 两种重构形式与参考评估保持紧密的数值一致性，分别实现256.72倍和93.91倍的端到端加速。

Conclusion: 问题侧重构是实现可扩展ETO的实际路径，两种应用实现和实验脚本均以开源形式发布以支持可重复性和重用。
...
TL;DR: 本文提出MePo++，一个统一的后训练框架，用于基于预训练模型的通用持续学习（GCL）。该框架通过表示精炼与协调来弥合预训练知识与下游持续学习之间的差距。


<details>
  <summary>Details</summary>
Motivation: 通用持续学习（GCL）旨在无需任务标识、明确边界或重复访问先前数据的情况下从演化数据流中学习，是一个现实但具有挑战性的设置。现有的基于预训练模型（PTM）的方法通常直接适配预训练表示，忽略了两个关键差距：上游预训练与下游持续适配之间的对齐不当，以及传统输出在模糊流下的不可靠性。

Method: MePo++引入两个互补组件：1) MetaPrep通过在伪持续序列上进行无监督元精炼来提高表示可塑性；2) StreamAlign通过将演化的在线特征与稳定的预训练几何进行协调来增强表示稳定性。通过在适配前改进表示可学习性并在持续学习期间保持对齐，MePo++使PTM能够对新概念保持可塑性并在演化流中保持稳定。

Result: 在不同的PTM、数据集和持续学习基线上的实验表明，MePo++对于基于PTM的GCL具有一致的有效性和泛化性。

Conclusion: MePo++通过表示精炼与协调机制，成功实现了预训练模型在新知识学习（可塑性）与旧知识保持（稳定性）之间的平衡，为基于预训练模型的通用持续学习提供了有效框架。
...
TL;DR: TruthInsightBench是一个用于评估AI科学发现能力的基准测试。与现有基准测试侧重于"复现"已知结果不同，该基准测试设计了40个盲任务，仅提供中立科学目标和冻结数据，隐藏源结论和预期分析路径，旨在评估AI代理能否独立做出科学发现。固定LLM裁判基于6个维度（29个artifact-grounded条目）评分。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试都是为"复现"任务设计的：围绕隐藏的目标研究构建任务、数据和评分标准，奖励恢复其结果。然而执行预设分析与进行真正发现是不同的。作者认为需要一个新的基准来评估科学发现能力。

Method: 从40项同行评审研究中构建40个盲任务，涵盖10个科学领域。仅提供中立科学目标和冻结数据；隐藏源结论、预期值和分析路径，让代理自行判断数据支持什么主张。使用基于LLM的裁判沿6个维度评分，操作为29个基于工件的项目，采用自动化、确定性聚合，无逐实例人工评分。

Result: 四个编码代理在冻结基础模型上形成狭窄 plateau（58.4-60.3分/100分），无统计显著 pairwise 分离。它们能胜任执行和记录分析，证据可审计性和新颖性相对较强，但普遍缺乏建立可信主张的区分性行为（控制、鲁棒性、可证伪性和跨数据集泛化）。

Conclusion: 瓶颈是科学判断而非编码能力，真正的发现仍然遥不可及。TruthInsightBench使这一差距成为可测量目标。
...
TL;DR: MedTraj框架将医学AI推理轨迹作为核心对象进行构建、评估和优化，解决了传统医学AI评估仅关注最终答案正确性而忽视中间推理过程质量的问题。该框架通过多维度评估、受控错误注入、边际贡献分析和质量加权上下文学习来提升医学推理质量。


<details>
  <summary>Details</summary>
Motivation: 当前医学AI评估主要以答案为中心，仅评估最终输出的正确性，而忽视了中间推理过程的质量。在临床环境中，通过伪造证据或不连贯逻辑得出的正确答案与错误答案同样危险。

Method: 1) 从医学推理来源生成结构化多步推理链；2) 将轨迹解析为临床观察、证据、编号推理步骤和最终结论；3) 从五个维度评分：连贯性、证据支持、幻觉性、完整性和可追溯性；4) 受控错误注入建立推理失败与质量下降的因果关系；5) 基于边际贡献的步骤级筛选；6) 质量加权上下文学习将评估反馈到推理时。

Result: 在CareQA、PubMedQA和CECMed上，轨迹上下文始终改善推理连贯性，比零样本基线提升+0.029至+0.041。在CECMed上，质量加权上下文使正确率几乎翻倍，同时将幻觉率降低87%。边际贡献分析表明少数推理步骤携带大部分质量信号，推理链超过四步后收益递减。

Conclusion: MedTraj框架有效提升了医学AI的推理质量，通过关注推理轨迹而非仅关注答案，为临床应用提供了更安全、更可靠的医学AI系统。质量加权上下文学习和步骤级筛选是提升推理质量的有效策略。
...
TL;DR: Discovery Loop是一个轻量级系统,利用大语言模型(LLM)迭代优化算法。该系统从简单种子求解器开始,通过结果记分板和历史想法引导LLM提出改进建议,每个候选算法经过独立验证器评估。在Packomania圆-packaging基准测试中,该系统在15次迭代内以仅27.72美元的成本将101-114范围内10个N值的最佳已知解决方案提高了2.4%-5.4%,结果已被Packomania独立认可。


<details>
  <summary>Details</summary>
Motivation: 旨在利用大语言模型自动化科学发现,特别是在优化算法领域。通过让LLM迭代改进算法来解决圆-packaging问题(Packomania基准测试),以低成本实现自动化算法优化,推动科学发现的民主化。

Method: 1)从简单种子求解器开始;2)LLM根据结果记分板和历史想法提出算法改进;3)独立验证器评估每个候选;4)保留改进,丢弃失败;5)包含自适应 plateau 检测机制

Result: 在101-114范围内为10个N值改进最佳已知解,增益2.4%-5.4%;全部在15次迭代内完成;总LLM成本27.72美元;结果已被Packomania独立接受。

Conclusion: 该方法以极低成本(27.72美元)成功改进优化算法,为自动化科学发现提供了新范式,具有推动科学发现民主化的潜力。
...
TL;DR: 本文提出ProCA框架，通过渐进式对比对齐和结构一致性插值解决EEG视觉解码中神经-语义对齐不稳定的问题，在跨被试和持续适应等多种设置下取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定的视觉或文本锚点，但EEG表示在不同试验、被试和学习阶段会发生变化，导致语义关系错位；固定语义监督在EEG特定关系演变时会产生优化偏差。

Method: 提出ProCA框架：1)渐进式对比对齐，从冻结的视觉-语言先验到EEG感知的语义关系逐步精炼类别级对比监督；2)结构一致性插值，根据通道级和时间级重要性约束特征混合。

Result: 在被试依赖、被试独立、严格跨被试迁移和持续适应设置下，ProCA分别实现Top-1/Top-5平均相对提升7.4%/3.9%、10.0%/4.6%、28.1%/17.8%和16.8%/11.6%。

Conclusion: ProCA作为一个统一且模型无关的框架，能够有效解决EEG解码中神经-语义对齐不稳定的问题，显著提升跨被试迁移和个性化持续适应的性能。
...
TL;DR: 提出BCM方法，通过自监督Bellmangrounded目标函数和紧凑坐标编码，将加性异构路由成本写入可重用的认知地图表示，支持动态目标查询且无需针对每个目标进行重新训练。


<details>
  <summary>Details</summary>
Motivation: 现有人工认知地图模型虽能模仿生物体的地图复用能力，但未能明确基于加性异构路由成本，且存在内存效率问题：代表性状态索引和高秩谱结构随着环境规模扩大而大幅增加存储成本。

Method: 提出BCM方法，包含两个核心创新：(1)自监督Bellmangrounded目标函数，将局部边 costs嵌入认知地图；(2)紧凑坐标编码，支持变化目标查询而无需针对每个目标重新训练。

Result: 在N=1600节点的加权网格上，BCM保持完全成功率和仅5%的平均Gap（相比Dijkstra精确搜索），而基于连通性的谱基线方法约为45%。随着图规模从N=400增加到N=3600，内存占用呈亚线性增长。

Conclusion: 结果表明加性路由成本可以被写入紧凑、可重用的认知地图表示中，弥补了生物灵活性和最优路径规划之间的差距。BCM方法可扩展到复杂环境。)
...
TL;DR: 本文从贝叶斯视角统一了大型语言模型的多种训练和评估范式，包括监督微调(SFT)、少样本上下文学习(ICL)、KL正则化RLHF/RLVR、策略蒸馏(OPD)和测试时推理等。作者提出两步模板：先用先验/参考模型和效用信号构造(广义)贝叶斯或吉布斯后验，然后通过前向KL投影近似到参数化族。文章展示了这些看似不同的方法实际上都是前向KL投影到由奖励或优势诱导的后验上的实例，并讨论了现代推理管道的含义。


<details>
  <summary>Details</summary>
Motivation: 当前LLM训练范式多样（ SFT、ICL、RLHF、OPD等），且常被视为根本不同。RL微调的推理模型上少样本提示的混合影响等实证结果令人困惑。需要一个统一框架来理解这些方法的关系。

Method: 贝叶斯视角 + 两步模板：(i)基于先验/参考模型和效用信号构建广义贝叶斯/吉布斯后验q*；(ii)通过前向KL投影将q*近似到参数化族（权重内SFT/RL或上下文内ICL）。

Result: 证明少样本ICL和SFT是 amortized 和权重投影到贝叶斯后验预测；KL正则化RLHF/RLVR、奖励加权SFT、奖励加权ICL(RW-ICL)、优势加权SFT(AWSFT)都是前向KL投影到由奖励/优势诱导的后验。厘清了这些等价成立（目标和一阶更新）和不成立（学习信号的来源和粒度）之处。

Conclusion: RLHF/RLVR可视为"后验设计+投影"；冷启动或监督预热对重要性加权KL投影实际不可避免；DeepSeek-R1和o1风格推理模型结合了测试时贝叶斯搜索和训练时KL amortization。
...
TL;DR: SciDocBench是一个以工作流为中心的科学文档理解基准测试，包含124个专家编写的问题，涵盖7个研究助理能力组和19个子任务，跨越5个科学领域。评估采用四种条件组合（英语/中文问题×图像优先/交错文档表示），最强系统仅达到62.6/100分，显示出在文档感知、证据 grounding、验证和跨文档推理方面的明显弱点。同时提出SciDocIR证据图表示法和包含约15K监督微调样本和8K强化学习样本的SciDocDataset。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常孤立地评估这些能力，不清楚多模态模型是否支持真实的科学阅读工作流程。需要一个以工作流为中心的基准来诊断和改进科学文档助手。

Method: 构建包含124个专家编写和难度筛选的问题，分为7个能力组和19个子任务，跨越5个科学领域。每个问题在四种匹配条件下实例化：英语或中文问题与全部图像优先或交错文档表示的组合，形成496个评估实例。同时设计SciDocIR类型化证据图表示来保留科学文档对象、布局和交叉引用关系及来源。

Result: 最强评估系统仅达到62.6/100分，在文档感知、证据 grounding、验证和跨文档推理方面存在显著弱点。SciDocDataset包含约15K监督微调样本和8K强化学习样本，涵盖14个可验证子任务。

Conclusion: SciDocBench、SciDocIR和SciDocDataset构成了从诊断到训练的科学文档助手评估到训练框架，可用于诊断和改进科学文档助手。
...
TL;DR: 研究提出了一种集成机器学习和深度神经网络的心血管疾病早期检测与预后智能框架，利用IoMT设备收集实时生理数据，通过特征选择和集成分类器实现高精度的风险预测。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要健康威胁，传统的检测方法存在准确性和实时性局限，需要更先进的早期预警系统来改善患者预后。

Method: 系统采用IoMT设备（ECG传感器、心率监测器、血压追踪器）收集实时生理数据，经过降噪、归一化、缺失值填补等预处理步骤，通过特征选择方法识别关键健康指标，最后使用SVM、随机森林和XGBoost分类器构建集成架构，并在云基础设施上实现实时处理。

Result: 框架在心血管疾病风险预测方面表现出色，达到更高准确性、减少误报、增强一致性，优于传统方法。

Conclusion: 结合传统机器学习与深度学习范式可实现主动健康管理，显著改善心血管疾病早期风险评估和临床决策支持，具有提升患者治疗效果的潜在价值。
...
TL;DR: 本文提出了一种名为"Mirror Agent Agent"的新型架构，用于生成可解释的行为和解释。该架构将观察者模型定义为代理的"镜像"，以实现明确的和隐含的通信。


<details>
  <summary>Details</summary>
Motivation: 解决智能体意图的有效沟通和可读行为生成问题，使人类能够更好地理解和解释AI代理的行为。

Method: 提出Mirror Agent Model架构，通过将观察者模型定义为代理的镜像来实现可解释性；并利用现成的显著性方法(saliency methods)为架构增添解释能力。

Result: 提供了初步的定性结果，展示了该架构在可解释通信和可读行为生成方面的有效性。

Conclusion: 该架构为实现通用可解释AI系统提供了新的思路，通过镜像机制和显著性方法的结合，能够有效促进人机交互理解。
...
TL;DR: 研究 OPD 中的数据效率和选择机制。发现：1-shot OPD 有效，困难样本更好；改进源于长 CoT 而非高熵；8个困难样本可匹配17K基线。


<details>
  <summary>Details</summary>
Motivation: OPD 已成为增强 LLM 推理能力的流行后训练范式，但 OPD 中的数据中心机制（数据效率、数据选择）相对未被探索。

Method: 1）极端设置：1-shot OPD 实验；2）分析训练数据驱动因素（token entropy vs CoT 长度）；3）基于洞察提出简单数据选择方法：仅选择困难样本（包括超出老师能力的"无解"样本）。实验在4个模型（1.5B-7B）上验证。

Result: 1）1-shot OPD 在所有采样训练样本上一致有效；2）困难样本通常带来更好的性能提升；3）改进来自更长的 CoT paths（而非高 token entropy），因为长 CoT 能保持与老师更长的推理对齐，并学习短 CoT 中缺失的关键思考模式（如反思）；4）仅用8个选择的困难样本即可匹配17K数据集基线性能。

Conclusion: 1-shot OPD 即可取得效果，且 harder examples 效果更好。改进源于 longer CoT paths 而非高 token entropy。提出简单数据选择方法：仅选择困难样本训练，8个困难样本即可匹配17K数据集基线效果。结论：数据质量比数量更重要，困难样本（甚至"无解"样本）都可用于训练。
...
TL;DR: 研究会议评审中的串通投标问题，提出端到端多智能体模拟框架，通过配置LLM驱动的审稿人智能体来模拟诚实或串通策略，分析串通投标对论文分配和评分的影响，并评估现有检测器的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决现有研究将投标、审稿人分配和审查操纵视为独立阶段的问题，填补串通投标生命周期效应的空白。现实世界中难以观察串通意图且缺乏同一会议的对照样本，导致真实分析受限。

Method: 提出多智能体模拟框架\alg，在固定会议环境下配置LLM驱动的审稿人智能体；开发基于亲和度的串通投标策略，利用审稿人-论文亲和度构建串通圈子选择目标论文，产生符合专业知识的针对性攻击而非任意攻击。

Result: 串通投标使目标论文捕获率增加一倍以上，被分配的串通者比诚实审稿人评分高约2分，但会议整体影响相对较小。固定三元组检测器的压力测试显示，良性亲和度干扰了原生正投标图，而仅用"非常高"诊断视图可实现精确但低覆盖的局部恢复。

Conclusion: 串通投标对个体论文分配和评分有显著影响，但会议层面影响有限；现有投标阶段检测器对串通的识别能力有限，需要更精细的检测方法。
...
TL;DR: ACE是一种无需训练、无需校准、保持预训练权重完整的token级自适应专家跳过框架，通过全局光谱代理(GSP)和路由器条件细化(RCR)两个组件评估专家贡献，在3个MoE模型和8个基准测试上显著优于现有静态和动态基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决MoE架构中固定top-k路由导致每个token激活相同数量专家造成的冗余计算问题，现有专家跳过方法依赖路由器置信度、校准数据或额外训练，无法可靠估计专家的实际贡献。

Method: 提出ACE框架，包含两个互补组件：1) GSP从耦合的gate、up、down投影和RMSNorm缩放估计全局变换能力；2) RCR从中心化路由器权重构建专家特定方向原型，沿路由偏好方向评估专家响应。推理时结合两种估计与运行时路由器门控，仅当双方都识别为低贡献时跳过专家，但始终保留top-1专家。

Result: 在Qwen3.6-35B-A3B上以50%跳过比率，ACE相比最强竞争方法将WikiText-2困惑度降低7.96%，下游平均准确率提升4.15个百分点。

Conclusion: ACE作为一个训练free和校准free的框架，能够有效实现token级自适应专家跳过，在保持模型性能的同时显著减少计算开销，且在激进跳过设置下优势更加明显。
...
TL;DR: 本文提出"基质盲视"概念，指AI智能体在规划时缺乏对执行环境约束（内存、时间、计算资源）的感知。通过让三个前沿模型（Claude Opus 5、GPT-5.6-Sol、Gemini 3.7 Flash）在代码生成任务中加入128MB内存和10秒执行时间的合约约束，证明约束披露可使模型生成更具资源效率的代码，实现内存降低49-74%、执行时间缩短35-64%。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在选择行动时，未考虑执行环境的内存、时间、计算和操作约束。这种"基质盲视"导致生成的代码可能无法在有限资源下高效运行。研究者希望验证：通过向模型披露执行约束，能否诱导模型主动调整代码结构，产生更高效的程序。

Method: 实验采用数值代码生成任务（高维成对欧氏距离计算），对比三种模型在有无执行合约（128MB RAM/10s墙钟时间）两种条件下的表现。进一步在96MB更严格约束下进行独立采样验证。测量指标包括峰值进程内存(MaxRSS)、墙钟时间和代码结构。

Result: 在128MB/10s合约条件下：13/14的可执行索引对齐比较显示峰值内存下降，平均墙钟时间全部缩短，最高快3.1倍。代码产生结构变化：bounded blocking、float32保留、上三角遍历、就地或内存映射缓冲。在96MB约束下：合约组正确且符合预算的结果为Claude 4/5、GPT 5/5、Gemini 3/5，而无合约组为0/5、1/5、0/5；平均内存降低49-74%，时间缩短35-64%。

Conclusion: 研究建立了基质感知智能体规划的受控概念验证：最小的执行合约即可诱导生成程序产生前瞻性的结构适应，将计算从无约束分配转向受限资源优化，显著改善执行前的资源-时间特征。这为让AI智能体感知并适应执行环境约束提供了实证基础。
...
TL;DR: 研究揭示了一个快速增长的"解绑"AI模型生态系统，主要分析了对开源大语言模型移除安全防护措施的行为。


<details>
  <summary>Details</summary>
Motivation: 随着开源AI模型的发展，越来越多的行为者在移除模型内置的安全护栏，这可能带来恶意应用风险，需要对这一生态系统进行系统分析。

Method: 通过识别关键生产者、下游传播者和新兴应用，对2024年1月至2026年3月期间的HuggingFace平台上的未审查模型进行profile分析。

Result: 共发现3,471个原始未审查模型，平均每个模型被重新打包2.4次；三个主要行为者占所有8,164个压缩再分发项目的52%。这些模型经量化后通过不同账户、格式和注册表（如Ollama）镜像分发，即使上游删除也能持续存在。在1,643个整合未审查大语言模型的GitHub应用中，25%被归类为明显恶意应用。

Conclusion: 未审查AI模型的传播已形成规模化生态系统，即使原始模型被移除，这些模型仍能持续存在并容易被下游部署，对AI安全构成重大挑战。
...
TL;DR: 本研究引入基于知识空间理论(KST)的框架来评估LLM在数学推理中的知识结构，发现当前LLM的知识并不遵循人类式的结构化依赖关系。


<details>
  <summary>Details</summary>
Motivation: 人类知识具有内在的结构性和相互依赖性：掌握一个概念需要先掌握其先决条件，知识空间理论(KST)将这一原则形式化。尽管LLM在复杂推理任务中表现优异，但尚不清楚它们是否表现出连贯的、类似人类的知识结构。

Method: 使用KST基础的框架评估LLM在数学推理中的知识结构，分析LLM行为是否遵循原则性的知识依赖关系。对8个开源和闭源LLM与真实人类学习者进行评估。

Result: 1. LLMs不遵守人类知识结构——它们经常违反知识依赖关系，且无法利用上下文中提供的相关知识来提高对依赖问题的表现；2. LLMs之间没有一致的知识结构，知识分布重叠度低；3. 这些结构性缺陷在基于准确率的评估和LLM作为评判者的评估中基本不可见。

Conclusion: 当前LLM的知识不遵循人类式的结构，研究提供了行为证据支持这一观点。
...
TL;DR: 提出了一种统一的强化学习框架，利用图神经网络(GNN)策略和近端策略优化(PPO)算法搜索紧凑的参数化量子电路(PQC)，用于数据稀缺的器件建模。该方法在11个目标上均达到了最低的MAE，且无需施加显式物理约束或惩罚项。


<details>
  <summary>Details</summary>
Motivation: 数据稀缺的器件建模挑战：传统方法需要大量数据且难以泛化，需要一种能够自动发现紧凑、通用性强的 surrogate model 的方法。

Method: 使用图神经网络(GNN)作为策略网络，通过近端策略优化(PPO)算法优化，以留一-group-out交叉验证(LOGOCV)误差作为奖励信号，搜索最优电路架构。

Result: 在11个目标上取得最低MAE：HEMT器件的Ioff误差降低59%，VTH fold变异性降低81%；NWFET器件的VTH、SS、Ioff误差降低84%，Ioff fold变异性降低82%。

Conclusion: RL选择的量子电路可作为紧凑的surrogate模型，具有低分布外(OOD)误差和改善的物理一致性，无需显式物理约束或器件特定方程即可实现。
...
TL;DR: 本文是一篇关于计算机视觉中常识推理的综合综述论文，探讨如何将常识知识整合到视觉任务中，以实现更智能的场景理解。


<details>
  <summary>Details</summary>
Motivation: 传统CNN视觉模型仅能识别特定图像中的物体，缺乏对场景的全面理解能力。集成常识知识可以使模型更好地解释场景，理解物体间关系和动作上下文，从而提高预测准确性和真实世界应用中的交互能力。

Method: 系统性地综述了四类方法：(1)知识图谱、(2)场景图、(3)神经符号模型、(4)常识增强Transformer。并分析了当前 limitations包括数据集偏差、知识不完整性和集成挑战。

Result: 该综述全面梳理了常识推理与计算机视觉交叉领域的最新进展，识别出当前研究的主要局限，并为未来研究方向指明了方向。

Conclusion: 未来研究应聚焦于跨模态推理、可扩展常识知识注入以及神经符号混合架构，以开发真正智能的视觉系统。
...
TL;DR: Trace2Tower是一个基于过渡感知EigenTrace的框架，用于从大型语言模型代理的执行轨迹中提取健壮的技能层次结构，解决当前范式中浅层轨迹检索和平面技能总结的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理范式受限于浅层轨迹检索和平面技能总结，根本上忽略了代理行为的时序依赖和结果条件拓扑结构。

Method: 将步骤级交互抽象为规范事件，构建由语义兼容性、过渡动态和结果证据统一的图，通过对比谱分解分离稳定、成功对齐的行为模式，同时抑制易失败的捷径，形成动态技能塔。

Result: 在ALFWorld上达到87.31%成功率和10.35步、0.26次无效动作；在WebShop上达到50.67%的精确成功率。

Conclusion: Trace2Tower在任务掌握和上下文高效经验复用方面显著优于现有基线。
...
TL;DR: 本文提出AI4CDS框架，一个用于人工智能增强计算设计科学的五阶段方法论，通过ChildRiskGuard案例（儿童不适视频检测系统）验证其有效性。该框架在AI扩展问题与设计搜索的同时，由研究者保持领域接地性、可接受性验证和科学判断责任，实现F1分数0.769的检测性能。


<details>
  <summary>Details</summary>
Motivation: 人工智能正在改变信息系统研究的设计内容和方式，但现有文献对计算设计科学(CDS)缺乏指导，尤其当AI主动参与问题构建、资源设计、设计搜索、评估和知识抽象时。需要一个系统性的方法论框架来指导AI增强的计算设计科学研究。

Method: 开发了AI4CDS五阶段方法论框架：1)AI参与问题表述、资源构建、设计搜索、评估和知识抽象；2)研究者保留领域接地性、可接受性验证、科学判断责任；3)合作由分级信任、可逆性、可审计性和差异化可复现性管理；4)通过ChildRiskGuard（儿童不适视频检测可解释 artifacts）进行实例化

Result: ChildRiskGuard实现F1分数0.769，显著优于通用内容安全模型的直接应用，同时与强基准模型保持竞争力。该系统将受众依赖的安全性和解释可信度转化为三个技术挑战，开发了将通用风险与儿童特定风险分离、表示不同发展风险机制、并将概念级解释作为预测计算部分的 artifacts。

Conclusion: AI4CDS作为AI增强计算设计科学的负责任框架做出主要贡献；ChildRiskGuard提供了AI扩展、研究者治理的设计如何生成和评估新计算设计知识的过程和 artifacts 证据。
...
TL;DR: 层dropout（又称随机深度）能够实现更快的训练、更高的准确率以及对零样本层剪枝的鲁棒性，但在大语言模型预训练中已基本消失。本研究表明，在最先进的大语言模型训练中应使用层dropout，并建立了训练和后训练阶段最佳实践。


<details>
  <summary>Details</summary>
Motivation: 之前有研究报道dropout会降低准确率，但尚无全面研究来量化或缓解这一影响。本研究旨在建立大语言模型训练中层dropout的最佳实践和扩展分析。

Method: 通过优化层分布、时间调度和优化器超参数，在Cerebras CS-3系统上进行了超过2400个训练实验，模型规模从2.71亿到82亿参数，数据集规模最高达1600亿tokens。

Result: 在相同训练FLOPs条件下，层dropout可导致更低的损失；在给定训练步数下，大语言模型可实现更低或相似的验证损失，同时节省最高达25%的训练FLOPs。层dropout还支持后训练优化（早期退出、中间层跳过、自投机解码），带来最高1.5倍的推理加速，且准确率损失可忽略不计。

Conclusion: 这些发现可可靠地扩展到大规模训练 regime。
...
TL;DR: GUT方法通过将推理链的潜在分支表示为有向无环图来研究和解决大语言模型的推理不确定性问题，包含量化模块(GUT-Q)和优化模块(GUT-O)，在4个LLM和5个数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的推理过程存在不确定性：相同提示下会产生大量分歧分支，某些分支包含明显不可信甚至无意义的推理链和结果。这种不确定性影响了LLM的可靠性。

Method: 提出GUT方法核心是用有向无环图(DAG)表征每个推理链的潜在分支，确保所有潜在分支被图空间全面覆盖。在此基础上构建两个模块：1)GUT-Q通过图复杂度近似推理空间复杂度来测量不确定性；2)GUT-O将负不确定性作为强化学习的奖励函数来优化不确定性。

Result: 在4个大语言模型和5个数据集上的实验结果验证了GUT方法的有效性。

Conclusion: GUT能够有效量化和降低LLM的推理不确定性，为提高LLM推理可靠性提供了新思路。
...
TL;DR: 本文提出了一种新的评估框架——行为正确性假设，用于评估自然语言生成系统的自动参考评估方法。通过定义正确性保持和正确性改变假设的分类，并使用受控响应转换来操作化这些假设，对多种词汇、字符级、语义、基于LLM和混合评估器进行了评估。


<details>
  <summary>Details</summary>
Motivation: 现有元评估主要衡量与人类判断或基准标签的一致性，对评估器在受控条件下的行为提供有限洞察。需要一种补充框架来评估参考-based自动评估方法的行为正确性。

Method: 定义正确性保持和正确性改变假设的分类体系，通过受控响应转换来操作化这些假设，指定预期评分行为。对多种词汇、字符级、语义、基于LLM和混合评估器进行评估，分析其假设级行为、稳定性、敏感性、重复运行变异性、配置敏感性和可重复性。

Result: 不同评估范式表现出明显的行为权衡：没有评估器满足所有提出的正确性假设，具有相似聚合性能的评估器可能表现出显著不同的行为特征。

Conclusion: 行为正确性假设提供了传统聚合元评估所掩盖的诊断信息，表明需要更全面的评估器行为分析框架。
...
TL;DR: RISE是一种新的语言模型后训练方法，通过从模型自身的强化学习训练轨迹中构建合成教师，实现递归改进而非一次性压缩。


<details>
  <summary>Details</summary>
Motivation: 解决蒸馏效果受限于教师质量的问题：外部教师存在分布不匹配，自蒸馏受限于上下文学习能力。

Method: 通过外推当前检查点与锚点之间的位移（在参数空间或输出logit空间），将稀疏的结果级参数更新转换为密集的token级目标，无需外部模型或特权条件。

Result: 在数学推理、多领域STEM、代码生成和多轮智能体任务上均优于RLVR训练和on-policy自蒸馏。

Conclusion: 蒸馏成为递归改进机制而非一次性压缩步骤，教师每轮迭代刷新，随学生提升而改进。
...
### [184] [Large Language Models for HVAC Operations in Building Energy Systems: A Critical Review of Methods, Applications, and Deployment Readiness](https://arxiv.org/abs/2609.05314)
*Alexander Neubauer,Tianzhen Hong,Han Li,Mengbo Yu,Amin Darbandi,Yannick Fürst,Martin Kriegel*

Main category: cs.AI

TL;DR: 本文是一项系统综述，分析了66篇关于大型语言模型(LLM)在HVAC运维中应用的同行评审研究(2023-2026年3月)。研究覆盖5个应用家族和3个LLM方法家族，评估证据真实性、部署就绪度和责任边界。结果显示建筑能源建模(BEM)是研究最集中的领域(32篇)，但仅有4项研究达到试点水平，无研究实现持续运营部署。LLM更适合作为语义和工作流层而非自主HVAC控制器。


<details>
  <summary>Details</summary>
Motivation: 解决建筑自动化系统中因异构点命名、缺失元数据和碎片化文档导致的"数据丰富但洞察不足"问题，评估LLM在HVAC运维中的应用潜力和局限性。

Method: 系统综述方法，对66篇同行评审研究进行编码和分类，跨越5个应用家族和3个LLM方法家族；从证据真实性、部署就绪度、LLM与物理HVAC决策的责任边界三个维度进行评估。

Result: 研究集中在BEM领域(32/66)，负荷预测领域样本量不足；仅4项研究达试点水平，无持续运营部署案例；63项仅限研究，3项接近短期，0项就绪；传统ML、MPC、RL在高频控制、短视界数值预测和本体映射方面仍更优；自主智能体运营和未验证的占用者代理仍处研究阶段。

Conclusion: 当前证据支持LLM主要作为语义和工作流层而非自主HVAC控制器；近期可试点应用包括：点名称标准化、文档支撑的运维支持、BEM工作流辅助、物理控制器周围的咨询界面；未来应优先研究：现场验证基准、运营约束下的编排评估、有限延迟和可验证安全属性的LLM-MPC/RL架构。
...
### [185] [LLM-Driven Algorithm Design for Quantum Circuit Synthesis based on Binary Decision Diagrams](https://arxiv.org/abs/2609.05327)
*Yoonju Sim,Federico Berto,Chuanbo Hua,Jinkyoo Park,Changhyun Kwon*

Main category: cs.AI

TL;DR: 提出了QuantumEvo框架，利用LLM作为启发式生成器，通过进化搜索找到针对量子成本(QCC)优化的BDD变量排序启发式算法。该方法不直接预测排序，而是搜索能够直接操作变量排序的候选启发式。


<details>
  <summary>Details</summary>
Motivation: 现有可逆电路合成方法依赖BDD，但BDD大小与量子成本(QCC)并非完全相关。传统启发式方法最小化BDD大小，但忽略了对量子电路实际量子成本的优化。

Method: 提出QuantumEvo进化框架，使用LLM作为启发式生成器。框架从多个启发式家族初始化，在标准BDD操作空间中搜索能直接操纵变量排序的候选启发式，通过下游QCC进行选择。最终发现启发式HGA-QE修改了遗传算法中的sifting步骤。

Result: HGA-QE在基准测试集上与每函数最佳基线相比达到70.9%的平局或胜率，并在13.5%的函数上绝对最优。在来自与启发式发现数据不同源的基准测试套件上表现更优。

Conclusion: HGA-QE展示了具有竞争力的QCC性能，特别是在与训练数据不同源的基准测试上表现出更明显的相对优势，证明了该方法具有良好的泛化能力。
...
### [186] [Technical Manual for a Toolkit for Measuring Contextual Individuation in Transformer Language Models](https://arxiv.org/abs/2609.05333)
...
TL;DR: 本文档是一个方法论和实现参考手册，介绍了一个围绕"桥接形式"(bridge forms)构建的开源工具包。桥接形式是指在不同学科领域中以相同词形出现但具有不同含义的单词。该工具包用于测试 transformer 语言模型是否在深层网络中通过上下文区分词义。


<details>
  <summary>Details</summary>
Motivation: 验证一个广泛存在的信念：尽管 transformer 模型在嵌入层为词形分配单一的上下文无关向量，但它们在后层中通过上下文区分词的每次出现。现有研究缺乏一种能够固定词形而改变上下文和预期语义的控制性构造。

Method: 构建了一套完整流水线：(1)桥接形式及其源领域的声明式规范；(2)从维基百科获取语料；(3)出现位置定位；(4)分层表示提取；(5)领域对之间的轮廓系数( silhouette)测量以评估表示空间中的分离度；(6)配对可视化协议。每个设计选择都附带说明要避免的方法论失败模式。

Result: 本文是一份方法论和实现参考文档，不报告任何特定模型或桥接形式集合的实证结果。工具包、完整源代码和语料库已存档，可被后续研究引用作为工具。

Conclusion: 该工具包提供了一种严格的方法来测试 transformer 模型是否真正通过上下文区分词义，避免了语义污染、多组偏差、子词标记对齐错误和降维图中轴可比性伪影等问题。为词汇语义研究提供了标准化的实证研究工具。
...
### [187] [Does Your Agent's Memory Survive a Model Upgrade? A Controlled Study of Memory Portability](https://arxiv.org/abs/2609.05339)
*Ankit Goyal,Jaideep Ray*

Main category: cs.AI

TL;DR: 研究比较了四种记忆迁移策略在模型升级时的表现：原始长文本(LC-RAW)、检索增强生成(RAG)、压缩笔记(NOTES)和固定模式知识图谱(KG-fixed)。研究发现固定模式结构迁移最可靠，而压缩笔记存在严重模型耦合问题，部分嵌入迁移效果不佳，原始历史记录对记忆修复至关重要。


<details>
  <summary>Details</summary>
Motivation: 模型升级是常态，但记忆迁移非常困难。新模型可能以不同方式解释旧记忆，混合嵌入版本可能破坏检索，且修复可能失败而无法恢复原始证据。

Method: 使用48个合成历史记录和随机答案代码Exact评分,使用两个10亿参数以下的开放权重模型进行比较研究。

Result: KG-fixed准确率仅变化+0.0004±0.0020,转移可靠;NOTES准确率非对称变化+9.91或-13.28个百分点,存在严重模型耦合;RAG部分嵌入仅获得4.96点改进,远低于完整重新嵌入的11.90点;NOTES准确率缺口的80%(0.467±0.014)来自初始构建时的信息丢失,RAG缺口的81%(0.364±0.012)来自检索失败;NOTES修复无法在48个案例中达到90%恢复目标,而保留原始历史可在34/48案例中成功恢复。

Conclusion: 需要方向特定的迁移测试、严格的嵌入空间隔离以及保留源历史用于记忆修复。
...
### [188] [Who Should Grade My Work? Student Perspectives on Transparent AI-Assisted Writing Assessment in Higher Education](https://arxiv.org/abs/2609.05346)
*Rayed AlGhamdi*

Main category: cs.AI

TL;DR: 本研究探讨了在明确告知学生反馈和评分由AI生成的情况下，学生如何理解和回应AI介导的评估。研究发现学生将AI反馈的实用性与评估权威性视为两个独立维度：接受AI反馈用于表面层面的修改，但认为人类教师才是评分决策的适当权威。


<details>
  <summary>Details</summary>
Motivation: 现有研究已探讨GenAI生成反馈是否提高写作表现及学生感知有用性，但较少关注学生被明确告知反馈由AI而非人类教师生成时如何解释这种评估。本研究旨在填补这一空白。

Method: 在沙特某公立大学计算机专业本科技术写作课程中进行定性教学研究。13名男生完成课堂手写写作任务，由ChatGPT根据评分标准进行评估，随后明确告知学生评分和反馈由ChatGPT生成，并邀请其书面反思。采用归纳主题分析法分析反思内容。

Result: 识别出四个主题：1)反馈的感知有用性；2)对AI情境和教学局限的意识；3)条件性信任，区分反馈效用与评估权威；4)对教师教学和制度角色的反思。学生接受GenAI反馈用于表面层面的修改，但始终将人类教师定位为评分决策的适当权威。

Conclusion: 研究揭示了"反馈效用"与"评估权威"的区别——学生将这两个判断视为可分离的分析维度，而非同一认可尺度的两极。这表明在教育评估中引入AI时，需区分工具性支持与最终评估权责。
...
### [189] [CUA-Universe: A Scalable and Dynamic Environment for Hybrid GUI+CLI Agents](https://arxiv.org/abs/2609.05374)
*Haoting Shi,Wenhao Wang,Weicheng Fang,Yaozhong Liang,Tian Jin,Pengxiang Zhao,Guangyi Liu,Siheng Chen,Yanfeng Wang*

Main category: cs.AI

TL;DR: CUA-Universe是一个可扩展的环境到数据管道，将真实桌面软件转换为混合GUI+CLI环境，用于训练计算机使用代理。核心组件包括：App-Forge将应用适配为可复现的VM并发现命令行表面；Task-Weave合成多样化混合任务；Path-Steer引导 rollout 沿高效混合路径并收集验证轨迹。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用代理主要通过GUI行动，产生低效轨迹。现实世界计算机工作是混合的（视觉检查+命令行操作），但现有代理难以互补使用两种接口，且可扩展混合环境稀缺。GUI原生代理效率低，CLI原生代理缺乏视觉感知能力。

Method: 构建了三个核心组件：1) App-Forge将应用适配为可复现虚拟机并发现/包装/生成命令行表面，扩展到16个应用；2) Task-Weave从种子文件上的可重用操作合成可控难度的多样化混合任务；3) Path-Steer沿高效混合路径引导rollout并收集验证轨迹用于后训练。在此数据上训练使模型行为从低效GUI交互转向有效的GUI+CLI编排。

Result: 9B模型在三个基准上取得显著提升：CUA-Verse (Score +39.3分；-37%步骤，-60%令牌)、OSWorld (SR +16.8分；-57%步骤，-44%令牌)、OSWorld-MCP (Score +7.84分；-27%步骤，-30%令牌)。

Conclusion: CUA-Universe为构建更强大、更高效的计算机使用代理提供了可扩展的路径，通过混合GUI+CLI训练显著提升了代理的成功率和效率。
...
### [190] [Molecular Déjà Vu: Digit-Level Retrieval of Published Values in Frontier Language Models](https://arxiv.org/abs/2609.05381)
*Matthias Busch,Marius Tacke,Sviatlana V. Lamaka,Mikhail L. Zheludkevich,Christian J. Cyron,Roland C. Aydin,Christian Feiler*

Main category: cs.AI

TL;DR: 研究对22个前沿大语言模型在12个分子属性回归基准上进行了审计，评估它们是否通过记忆（即逐字检索）而非真正预测来完成任务。研究发现逐字检索广泛存在但具有基准特异性：在5个数据集中超过50%的LLM表现出逐字检索行为，而在其余数据集中仅出现在孤立单元中。推理水平影响检索结果，更高推理水平下标记数量增加89%。抑制检索后，不同模型的预测误差在相对意义上更加接近。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在分子属性基准上进行评估，但准确率指标无法区分真正预测属性值的模型与检索已发表数据的模型。这种混淆使得无法准确评估模型的真实预测能力，因此需要审计LLM在分子回归基准上的逐字检索程度。

Method: 研究审计了22个前沿模型在12个分子属性回归基准上的表现，在两个推理水平下运行实验，检测逐字检索的发生情况，并测试了打断检索的方法。

Result: 逐字检索广泛存在但具有基准特异性：在5个数据集中超过50%的LLM显示逐字检索，而在其他数据集中仅出现在孤立单元格；推理水平影响检索结果，更高推理水平下 flagged 数量增加89%；最强模型仍能识别转换后的SMILES与原始标签的组合；抑制检索使不同模型的预测误差在相对意义上更接近，而逐字检索的使用差异则使它们更加分散。

Conclusion: LLM的一般预测能力不仅由记忆值的数量决定。这项工作提供了LLM在分子回归基准上逐字检索数量和深度的全面概述。
...
### [191] [Necessary or Sufficient? Evaluating LLM Explanations With Behavioural Evidence](https://arxiv.org/abs/2609.05385)
*Urja Pawar,Rajitha Ramanayake,Nabeel Kemal,Ashwin Kandath,Owen O'Neill,Guillaume Bourgeon,Houssem Chatbri*

Main category: cs.AI

TL;DR: 研究测试LLM决策组件提供的解释因素是否与其实际决策行为一致，通过必要性（改变因素是否改变输出）和充分性（保留因素是否保持输出）两种解释来评估，使用黑盒干预方法在8个模型上实验。


<details>
  <summary>Details</summary>
Motivation: LLM决策组件通常输出带有解释的操作建议或判断，运维人员利用这些解释来监控系统、诊断错误或决定是否升级输出。但这种使用隐含假设解释与组件的可观察决策行为一致，研究旨在验证这一假设。

Method: 在两个合成用例（推荐顾问和判断提示有害性/风险）中进行评估。模型返回输出及影响最大的三个因素。通过受控黑盒干预估计必要性分数（改变因素后输出改变的频率）和充分性分数（保留因素时输出保持的频率）。涵盖Claude、GPT和Gemini家族的8个模型。

Result: 在顾问推荐任务中，引用排名与必要性/充分性分数的平均Spearman相关系数分别为0.349和0.354；在提示监控任务中分别为0.431和0.580。在57.6%的顾问回复中，未引用因素的必要性分数高于最低引用因素，充分性分数对应为58.1%；提示监控任务的对应比例分别为25.8%和8.9%。

Conclusion: 引用的前三个因素包含有用信息，但无法可靠识别必要性或充分性下影响最强的三个因素。该框架为代理监督中使用的解释提供了黑盒可靠性检查，同时限于个别LLM决策范围内。
...
### [192] [Multi-Step Tool-Calling over Korean Open Public APIs: A Benchmark and a Data-Synthesis Recipe](https://arxiv.org/abs/2609.05395)
*Dain Kim,Eungi Cho,Kyumin Kim,Shinyeong Noh,Kyuseong Lim*

Main category: cs.AI

TL;DR: 针对公共机构在数据主权法规下部署本地开源LLM代理的需求，本文构建了韩国公共API基准测试KOPA-Bench（145个真实任务），并提出EDGE方法通过动态图和实际执行验证来合成多步工具调用训练数据，使9B模型通过GRPO微调后性能接近27B模型。


<details>
  <summary>Details</summary>
Motivation: 数据主权法规要求公共机构使用本地开源LLM代理进行跨政府API的多步骤工具调用，但开源模型在此类多步骤场景中表现不佳，且缺乏专门的基准测试来衡量这一差距。

Method: EDGE方法构建工具输出到输入的动态图，通过实际调用政府API验证链接有效性，仅保留可执行的链接来合成多步骤轨迹数据。使用GRPO算法对9B模型进行微调。

Result: 微调后的9B模型性能几乎匹敌未微调的27B模型，不仅在KOPA-Bench上显著提升，还在BFCL基准测试上取得改进。

Conclusion: 通过真实API执行验证的合成数据能有效提升开源模型在多步骤工具调用任务上的表现，为数据主权场景下的LLM部署提供了可行方案。
...
### [193] [A Deep Generative Model for Synthesizing Labeled Wireless Signals](https://arxiv.org/abs/2609.05396)
*Yuxiao Li,Keke Hu,Santiago Mazuelas,Yuan Shen*

Main category: cs.AI

TL;DR: 本文提出IIns-GAN，一种基于深度学习的生成对抗网络方法，用于生成带位置标签的逼真无线信号，以解决真实数据集获取成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 在无线传感领域，带位置标签的无线信号对性能评估和模型训练至关重要，但真实数据集的获取面临测量和标注成本高的问题，传统方法依赖环境模型导致超参数调优复杂且真实性不足。

Method: 提出Inter-Instance Generative Adversarial Networks (IIns-GAN)，一种新型深度学习方法，用于生成逼真的带标签无线信号，该信号能够适应不同环境场景。

Result: 在公开的超宽带(UWB)数据集上进行了广泛实验，结果表明IIns-GAN生成的信号能够反映真实世界测量的物理特性。

Conclusion: IIns-GAN生成的信号可显著提升距离估计和环境识别等无线传感任务的模型训练效果，具有重要的实际应用价值。
...
### [194] [Approximating CDTW Distance of Piecewise Algebraic Curves](https://arxiv.org/abs/2609.04294)
*Alperen A. Ergür,Shamik Khowala*

Main category: cs.CG

TL;DR: 本文提出了一个针对高次代数曲线的连续动态时间规整(CDTW)距离的完全多项式时间近似方案(FPTAS)。该算法在欧几里得范数下计算两条曲线的CDTW距离，时间复杂度为O((m+n)^(19/6) * (1/ε)^(10/3) * log((m+n)/ε^2))，其中m和n为输入曲线的分段数。


<details>
  <summary>Details</summary>
Motivation: 曲线数据广泛存在于金融、地震学、医学、时空数据挖掘和恶意活动检测等领域。现有最常用的曲线相似性度量方法是动态时间规整(DTW)和Fréchet距离，但DTW对采样率敏感，Fréchet距离对异常值敏感，两者都无法产生鲁棒的结果。连续动态时间规整(CDTW)是一种更鲁棒的距離度量标准，可以改进DTW和Fréchet距离的不足。

Method: 本文针对分段高次代数曲线，在欧几里得范数下提出了一个近似算法来计算CDTW距离。该算法是一个完全多项式时间近似方案(FPTAS)，具有乘性误差ε。

Result: 算法的时间复杂度为O((m+n)^(19/6) * (1/ε)^(10/3) * log((m+n)/ε^2))，其中m和n是两条输入曲线的分段数。

Conclusion: 本文填补了CDTW算法在高次代数曲线领域的空白，提供了首个针对此类曲线的FPTAS算法，为曲线相似性分析提供了更鲁棒的解决方案。
