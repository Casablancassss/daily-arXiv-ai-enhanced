...
TL;DR: ClustRS是一种无需训练的token剪枝算法，用于解决视觉-语言模型(VLM)中视觉token过多导致的边缘部署难题。该方法包含两个部分：基于注意力加权的聚类算法用于从每个语义簇中选择代表性token，以及残差收缩模块对选定token进行单次去噪处理。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型如LLaVA虽然性能出色，但处理大量视觉token(576或729个)带来高昂计算成本，边缘设备难以部署。现有token剪枝方法需重新训练，且对图像噪声敏感。

Method: 提出ClustRS算法，包含两部分：(1)注意力加权聚类算法，从每个语义簇中选择代表性token；(2)残差收缩(Residual Shrinkage)模块，对选定token进行单次去噪处理。整个过程无需训练。

Result: 在ScienceQA-IMG和MM-VET基准测试中，该方法在极端噪声条件下(减少97%token，仅保留16个)优于基于注意力和多样性的方法达20%。在LLaVA-OneVision上，仅用不到三分之一token即可匹配基线性能。

Conclusion: ClustRS为VLM提供了一种简单而强大的token剪枝方案，超越了单纯的评分剪枝或多样性剪枝策略，为计算高效且噪声鲁棒的VLM边缘部署铺平了道路。
...
TL;DR: 本文提出SceneGTMM，一种基于共形映射的可迁移GNN-Transformer双图交互地图匹配框架，旨在解决现有方法在噪声鲁棒性、跨区域迁移和可解释性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有地图匹配方法面临三大挑战：1) 噪声鲁棒性不足；2) 跨区域迁移困难；3) 可解释性差。同时存在局部-全局融合不足、动态路网适应能力弱、依赖黑盒模型等问题。

Method: 1) 共形映射场景相对策略：构建以轨迹为中心的局部坐标系，减少对训练路网的依赖，支持跨区域迁移和动态路网更新；2) GNN-Transformer双图交互架构：GNN建模路网图捕捉局部拓扑约束，Transformer建模轨迹图捕捉全局时序依赖，跨图注意力实现噪声抑制和语义对齐；3) CRF增强的结构化预测：结合Transformer的全局上下文和CRF的拓扑转移约束，提高路径连通性和鲁棒性。

Result: 在定位误差16-50米的多源轨迹上准确率超过80%，比HMM提升5.3%；跨城市迁移场景中优于MTrajRec、GraphMM和TMM；通过注意力和相对坐标可视化增强了可解释性。

Conclusion: 为实时交通感知和自动驾驶路径规划提供了高精度、可迁移地图匹配的新范式。
...
TL;DR: 本文审计了split-conformal prediction作为零样本视觉语言模型(VLM) abstention layer在部署偏移下的表现，发现边际覆盖率虽高但类别条件尾部覆盖率崩溃。


<details>
  <summary>Details</summary>
Motivation: 零样本视觉语言模型(VLM)如CLIP、OpenCLIP、SigLIP被广泛使用，split-conformal prediction常作为其 abstention layer 来提供覆盖保证。但这种做法在部署偏移下的可靠性尚未得到充分审计。

Method: 在ImageNet和非ImageNet设置下对CLIP、OpenCLIP、SigLIP进行审计测试，评估边际覆盖率和类别条件尾部覆盖率，并测试多种校准方法（Mondrian calibration、clustered conformal、Conf-OT、target-side class calibration）。

Result: 在ImageNet-Sketch上，尽管边际覆盖率约0.86，但最差类别覆盖率降至≈0，10-12%类别低于有限样本null floor。失败与目标域类别准确率对齐，但源域诊断无法预测。源域Mondrian calibration改善分布内尾部但不转移；clustered conformal和Conf-OT改善平均指标但未恢复最差类别尾部。目标域类别校准显著提升尾部但需要每个类别的标签。发现2-3倍跨家族效率差距，SigLIP原生sigmoid分数移除APS概率质量解释。

Conclusion: 边际conformal覆盖率应被视为平均可靠性统计量，而非类别尾部安全保证。在测试的模型规模、预训练语料库、提示词、miscoverage水平α和偏移非ImageNet设置下，结果均持久存在。
...
TL;DR: CAViAR是一个用于分析自动驾驶事故因果关系的人类标注基准数据集，包含2249个真实事故视频，标注了环境条件、事故类型、因果解释、责任方、违规规则等信息，用于评估视觉语言模型在事故责任推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶系统虽擅长目标检测和轨迹预测等感知任务，但缺乏高层次因果推理能力来解释交通事故，特别是确定责任方和违反的交通规则方面仍是待探索的领域。

Method: 构建了CAViAR数据集，收集了CarCrashDataset和Nexar的2249个真实事故视频，由人类进行标注，涵盖环境条件、事故类型、因果解释、责任方、违规类别等信息。然后测试了Cosmos-Reason2、Qwen3-VL、InternVL3等先进的视觉语言模型，并使用多数类/随机基线和平衡指标来处理类别不平衡问题。

Result: 感知能力不均衡：照明条件识别几乎解决，但天气和道路条件准确率处于或低于多数类基线；所有模型在事故类型和责任推理方面表现急剧下降。暴露了"感知-推理差距"：当前VLMs能识别显著上下文，但无法可靠地将可见的代理行为映射到安全关键驾驶场景中的责任类别。

Conclusion: CAViAR揭示了当前视觉语言模型在安全关键驾驶场景中的感知-推理差距，表明现有模型虽然具备感知能力，但在因果推理和责任归因方面存在明显不足，需要进一步研究来解决这一问题。
...
TL;DR: Phoenix是一个499万参数的CNN-BiLSTM-CTC手写识别模型，Athar是一个围绕它构建的证据感知审查工作流。该系统用于历史阿拉伯语手稿转录，不仅关注识别准确率，还强调证据管理而非单纯的文本替换。


<details>
  <summary>Details</summary>
Motivation: 历史阿拉伯语手稿转录不仅是识别问题，还需应对笔迹变化、版面布局、保留不确定的读法、区分视觉证据与语言合理性，并记录研究者的最终决策。现有系统缺乏对这些需求的完整支持。

Method: 使用文档感知重放、81符号编码器和遗忘守卫技术，在档案、马格里布和历史手稿领域进行适应训练。采用CNN-BiLSTM-CTC架构，配合贪心解码进行评估。

Result: 在两个大型保留测试集上，字符错误率从19.98%降至14.93%，相对降低25.3%。Agapet数据集从22.12%降至17.86%，Omar数据集从17.72%降至11.84%，TariMa从10.39%略升至10.72%。四领域诊断中最低CER达9.59%。

Conclusion: 将手稿HTR评估为可审计的证据管理而非静音文本替换更具合理性。Athar系统保留了视觉读法、呈现有限替代选项、保守使用语言模型、检索源平行文本并导出可审计的TEI和PAGE-XML记录。
...
TL;DR: 本文提出了一种名为HiRA-CAM的改进方法，用于提高卷积神经网络(CNN)的可解释性。该方法在流行的基于梯度的方法LayerCAM基础上进行了改进，自适应地利用CNN所有层的激活图来生成更聚焦的显著性地图。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型包含数十亿参数，难以解释其内部转换和输出，而AI在关键应用中的使用日益增多，因此可解释性变得越来越重要。

Method: 基于LayerCAM方法进行改进，提出HiRA-CAM方法，自适应地利用CNN所有层的激活图来生成显著性地图。

Result: HiRA-CAM在为目标分类创建有用的显著性地图方面优于LayerCAM和Grad-CAM方法。

Conclusion: HiRA-CAM能够生成更聚焦的显著性地图，有效提升了CNN的可解释性。
...
TL;DR: VideoRun2D是一个用于短跑生物力学分析的视频框架，使用不同的人体姿态估计器追踪314段专业运动员跑步视频中的髋关节和膝关节角度，平均RMSE范围为11.46°至5.83°，结合后处理模块可降至9.87°和5.30°。


<details>
  <summary>Details</summary>
Motivation: 随着深度学习模型、数据可用性和计算资源的提升，人体姿态估计技术取得了显著进展，需要探索这些技术在体育分析和性能评估中的实际应用价值，特别是短跑生物力学分析领域。

Method: 使用314段来自44名专业运动员的短跑视频，采用不同的人体姿态追踪器，重点分析髋关节屈伸和膝关节屈伸两个关键角度，并设计了后处理模块用于异常值检测。

Result: 最佳追踪器的平均均方根误差(RMSE)范围为11.46°至5.83°，结合后处理模块后，误差可分别降低至9.87°和5.30°。

Conclusion: 人体姿态追踪方法可作为短跑生物力学分析的有效工具，具有重要的实际应用价值。
...
TL;DR: 提出了一种基于多速度联合扩散的扩散模型条件机制，通过学习无条件联合分数网络并在推理时使用插件校正项来实现条件化，从而提供透明的条件引导视图。


<details>
  <summary>Details</summary>
Motivation: 现有的条件扩散模型方法缺乏对条件如何引导目标分布生成的透明理解，需要一种能够明确分离条件贡献的机制。

Method: 学习无条件联合分数网络，通过插件校正项在推理时强制实施条件化，推导了显式的条件逆向时间SDE和近似概率流ODE，并引入log-Fokker-Planck残差正则化来减少ODE-SDE差异。

Result: 在条件图像生成任务中展示了具有竞争力的性能，ODE-SDE比较实验表明log-Fokker-Planck残差正则化改善了确定性ODE采样质量。

Conclusion: 插件条件化视图是有效的，log-Fokker-Planck残差正则化能够提高ODE采样质量，为条件扩散模型提供了原则性的条件采样方法。
...
TL;DR: CVSD-Reg是一个鲁棒的全局LiDAR点云配准框架，通过将视觉语义先验从视觉基础模型DINOv2蒸馏到LiDAR表示中，无需相机输入即可实现跨传感器泛化。在KITTI、nuScenes和HeLiPR数据集上取得了97.7%、99.0%和99.3%的严格成功率。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的全局点云配准方法依赖几何表示，对点密度、扫描模式、视点和传感器特性变化敏感。现有方法缺乏对跨传感器泛化能力的有效处理。

Method: 采用两阶段框架：阶段1使用Point Transformer V3作为学生模型，从冻结的DINOv2教师模型通过对比蒸馏和球面流形对齐学习，保持教师嵌入空间的双曲球面几何；自监督InfoNCE一致性和软SE(3)不变性增强视点鲁棒描述子。阶段2通过 correspondence learning、密度感知的点dropout增强和端到端姿态优化将蒸馏表示适配到配准任务。

Result: 在KITTI、nuScenes、HeLiPR上分别达到97.7%、99.3%、99.3%的严格成功率（SR@0.5m/1°），包括在稀疏16-beam Velodyne扫描上达到97.3%。比最先进的几何配准方法最高提升44.0个百分点。

Conclusion: CVSD-Reg能够泛化到单传感器和零样本跨传感器场景，无需传感器特定适配，推理时完全无需相机输入。
...
TL;DR: 本文提出标签无关精度细化（LFPR）方法，使冻结的直接回答模型能够在推理时利用自身预测分配额外的局部观测，无需访问目标注释，从而提高指代理解任务中的边界框精度。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型能识别正确的指代对象，但返回的边界框不够精确。研究旨在探索冻结的直接回答模型是否可以利用自身预测来改进精度，同时无需在推理时访问目标注释。

Method: LFPR将预测的小区域路由至高分辨率通道，在上下文裁剪中重新定位表达，仅在固定几何守卫下接受候选框，并返回坐标方向的中点。

Result: 在Ref-L4上，mAcc_0.5:0.95从72.947%提升至76.013%；在RefCOCO/RefCOCO+/RefCOCOg上，各数据集的Acc@0.5、mAcc和mean IoU均有提升；在Flickr30K Entities上所有指标均改善；应用于两个接地专家模型时Acc@0.9分别提升1.569和6.716个百分点。

Conclusion: 指代选择和边界精度部分可分离，不同组件移动IoU曲线的不同区域，单一阈值无法揭示这种行为。
...
TL;DR: Stream4D提出了一种用于流式自回归视频扩散模型的训练方法，通过引入前馈4D重建奖励和运动先验来解决长时序视频生成中的几何漂移和静态化问题，取代了之前使用静态3D Gaussian-Splatting作为批评者的方法。


<details>
  <summary>Details</summary>
Motivation: 流式自回归扩散模型在生成长时序视频时会积累几何漂移，降级为静态或不自然的运动。现有的双向方法使用3D Gaussian-Splatting重建作为奖励信号，但单一刚性3D重建无法建模动态场景，会将真实物体运动误判为重建错误，导致模型倾向于生成静止视频。

Method: Stream4D采用前馈4D重建奖励替代静态批评器，显式建模场景动态；引入运动先验奖励自然场景流幅度同时惩罚抖动和非刚性伪影；结合轻量级感知锚点形成最终训练配方。

Result: 在多种自回归视频主干网络和不同生成时域下，Stream4D提升了4D重建质量，更有效保持运动，并达到更高的人类偏好评分。

Conclusion: 通过显式建模场景动态的4D重建奖励和运动先验，Stream4D成功解决了自回归视频生成中的几何漂移和静态化问题，为实时长时序视频生成提供了更优的训练范式。
...
TL;DR: Block3D是一种块状扩散框架，通过将离散形状标记序列划分为连续块进行生成，在块内联合去噪，并引入置信度引导的块内校正来缓解误差累积，实现5.15倍加速且不牺牲几何保真度。


<details>
  <summary>Details</summary>
Motivation: 文本到3D生成发展迅速，但实现高几何保真度和低推理成本仍具挑战。现有方法存在缺陷：自回归解码是顺序的且无法修正错误；扩散和流匹配模型重复处理完整表示，导致高质量生成成本高昂。

Method: 提出Block3D块状扩散框架：1)将离散形状标记序列划分为连续块；2)自回归生成块；3)联合去噪块内所有标记；4)引入置信度引导的块内校正，在每个块最终确定前修正低置信度标记。

Result: 在TRELLIS-500K保留集上，Block3D将平均端到端生成时间从25.71秒降至4.99秒，实现5.15倍加速，且不牺牲几何保真度。

Conclusion: Block3D通过块状扩散和置信度校正机制，成功在保持高几何保真度的同时显著降低推理成本，为高效文本到3D生成提供了新思路。
...
TL;DR: 本文提出Mix&Fix-Net模型，用于解决小型船舶缺乏AIS数据导致的轨迹预测监控空白问题。该模型采用双阶段混合器架构，结合AIS数据和视觉数据（非AIS）进行船舶轨迹预测。


<details>
  <summary>Details</summary>
Motivation: 现有船舶轨迹预测模型主要依赖AIS数据，但小型船舶大多未配备AIS系统，导致显著的监控空白。需要一种能够同时处理AIS和非AIS（如视觉数据）的轨迹预测方法。

Method: 提出Mix&Fix-Net，一种双阶段混合器-based轨迹预测模型，包含主轨迹预测器和残差轨迹调整器。此外构建了新的基于视频的数据集（从网络摄像头流提取）。

Result: 在AIS和非AIS数据集上使用6个指标评估，结果表明Mix&Fix-Net在大多数指标和数据集上优于现有基线方法。

Conclusion: Mix&Fix-Net能够有效整合AIS和视觉数据，实现更精细的船舶轨迹预测，填补了非AIS船舶监控的技术空白。
...
TL;DR: VGI-bench是一个用于评估视频生成模型的视觉推理能力的基准测试，包含27个任务和810个实例，采用双层分类法组织。实验表明当前最强模型Seedance 2.0仅达到51.0%的准确率，表明视频生成模型在视觉推理方面仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型的零样本视觉推理能力评估面临挑战：现有基准与模型视觉先验不匹配、缺乏对演化过程的验证、任务难度校准不足。因此需要构建更合理的评估基准来推动视频生成模型的发展。

Method: 构建VGI-bench基准测试，包含27个任务和810个实例，按任务域和技能标签双层分类组织。评估了多个视频生成模型，分析了输出失败模式、输入条件敏感性、合成微调的性能迁移边界，以及内部去噪视角下的自纠正能力。

Result: 当前生成系统能解决部分视觉推理任务，但可靠性不足。Seedance 2.0作为最强模型仅达到51.0%的准确率。分析发现：后期去噪步骤主要是对早期假设进行细化，而非纠正推理错误，表明模型的自纠正能力有限。

Conclusion: VGI-bench为评估视频生成模型的视觉推理能力提供了系统化基准，当前模型在该任务上表现仍有较大改进空间。开源代码和数据有望推动下一代视频生成模型的发展。
...
TL;DR: 本文提出PEA-DPO框架解决多模态偏好优化中的视觉不敏感性问题，通过显式利用视觉偏好信号，理论证明可减轻两种失败模式，实验表明该方法能有效减少幻觉并提升多模态对齐效果


<details>
  <summary>Details</summary>
Motivation: DPO在单模态LLM对齐中表现有效，但其适应多模态设置的探索仍然不足。现有方法存在视觉不敏感性问题，即模型无法区分图像与关键视觉上下文被移除后的图像

Method: 提出PEA-DPO框架，显式利用视觉偏好信号；通过理论分析证明该方法能减轻两种视觉不敏感性失败模式；在三种不同规模的MLLM上进行评估

Result: PEA-DPO增强了视觉上下文敏感性，同时保留了基础模型的语言建模能力。在三个幻觉基准测试中，PEA-DPO有效减轻了视觉不敏感性，实现了更强的多模态对齐，并显著减少了幻觉

Conclusion: PEA-DPO通过显式利用视觉偏好信号来克服视觉不敏感性问题，理论分析证明其能够减轻两种失败模式。实验表明该方法在保持语言建模能力的同时，有效提升了模型对视觉上下文的敏感性，在三个幻觉基准测试中取得了显著改进。
...
TL;DR: TextRefine是一个针对产品海报文本编辑的后训练框架，结合监督微调和特定操作奖励优化，解决文本插入和替换中的语义保真度、空间冲突和字形质量问题，并发布OpenTextEdit数据集。


<details>
  <summary>Details</summary>
Motivation: 当前指令-based图像编辑模型在文本编辑任务中表现不可靠，存在目标文本遗漏或错误渲染、放置在显著产品或现有内容上、产生结构扭曲或视觉不一致的字形等问题。

Method: 1)文本插入：使用文本span级别奖励评估语义保真度和目标span覆盖率，惩罚与产品和现有文本的空间冲突，采用门控结构约束保留非文本区域；2)文本替换：利用CTC后验提供字形级别监督，处理缺失笔画、结构变形和相似字符混淆；3)发布OpenTextEdit数据集，包含10万张图像、多文本布局、详细文本属性、产品 masks和挑战性低频字符。

Result: 在文本插入和替换任务上，TextRefine在文本保真度、放置可靠性和字形质量方面一致优于评估的图像编辑基线，同时更好地保留源图像内容。

Conclusion: TextRefine通过结合监督微调和操作特定奖励优化，有效解决了产品海报文本编辑中的多重挑战，为该领域提供了新的研究框架和数据集。
...
TL;DR: 本文提出S²GS框架，利用结构感知的时序稀疏性选择性更新高斯残差，实现高效的流式自由视点视频重建，在边缘IoT设备上显著降低优化时间和存储成本。
...
Method: 1. 空间域：流式八叉树分层组织高斯残差，捕获空间相关性引导残差更新；2. 时序域：结构门控机制包含分层特征传播(HFP)和Gumbel-Sigmoid采样，将分层动态线索转化为稀疏残差更新决策；3. 多级离散方案提供细粒度控制同时保留细节。

Result: 相比QUEEN，在RTX 4090上每帧优化时间减少59%，存储成本降低85%；在Jetson AGX Orin上实现最高渲染吞吐量(60+ FPS)和最低能耗。

Conclusion: S²GS在资源受限系统中展现了优越的部署潜力，能够在保持竞争性视觉质量的同时显著降低计算和存储开销。
...
TL;DR: 本文研究了模拟计算存储(CIM)非理想特性对扩散Transformer(DiT)采样过程中无分类器引导(CFG)的影响，并提出了一种无需重新训练的CFG尺度重校准方法来恢复生成质量。


<details>
  <summary>Details</summary>
Motivation: DiT因重复评估大型去噪器而产生高内存和能耗成本；模拟CIM虽能通过在存储权重的内存阵列内执行线性操作来缓解成本，但其非理想特性会扰动有效权重，且这些误差会沿着依赖状态的去噪轨迹累积，尤其是与CFG的相互作用尚不明确。

Method: 提出一种无需重新训练的采样器端重校准方法，仅针对给定CIM条件调整CFG尺度。通过轨迹级分析表明，适度重校准可增强扭曲残差中保留的目标导向成分，使模型更早承诺于提示一致的语义区域。

Result: 在PixArt-Sigma、PixArt-alpha和DiT-XL/2上使用每条件30,000个样本进行广泛实验，结果显示：CIM噪声水平为0.20时，最优引导尺度随CIM噪声增加而增加；方法一致恢复了生成质量，至少关闭了87%的CIM诱导FID差距；PixArt-Sigma的FID从59.22降至20.49，PixArt-alpha从72.37降至21.12，DiT-XL/2从20.89降至6.62。

Conclusion: 通过识别CFG残差作为可控故障通道，证实了仅调整CFG尺度即可有效补偿CIM非理想特性带来的质量损失，为模拟CIM加速DiT采样的实际应用提供了可行的校准方案。
...
TL;DR: 本文构建了首个进攻回合级别的NBA篮球视频数据集(PL-NBA)，以完整进攻回合作为样本，包含11,000个进攻回合视频和31,567个标注事件，旨在解决现有数据集无法保持比赛事件时间连续性及支持复杂任务的问题。


<details>
  <summary>Details</summary>
Motivation: 现有篮球视频数据集仅采用单一动作或活动作为样本，无法保留比赛事件的时间连续性，也不能支持动作预测等复杂任务，因此需要构建一个新的进攻回合级别的数据集。

Method: 从60场NBA比赛中收集数据，标注每个进攻回合的视频片段，包括球员姓名、描述文字、事件类型和时间戳等丰富信息。

Result: PL-NBA数据集包含11,000个有效进攻回合视频和31,567个标注事件。实验表明，现有方法在事件识别、视频描述、时间动作定位和动作预测四个任务上性能有限，证明该数据集是一个具有挑战性的基准。

Conclusion: PL-NBA是首个进攻回合级别的篮球视频数据集，能够支持多种视觉理解任务，现有的先进方法在该数据集上表现不佳，说明该数据集具有重要的研究价值和挑战性。
...
TL;DR: MUST-PET是一个多模态、多示踪剂的自监督学习框架，用于全身PET-CT病变分割，旨在解决注释稀缺和领域迁移问题。


<details>
  <summary>Details</summary>
Motivation: 深度学习在全身PET-CT病变分割中应用受限于稀缺注释和领域迁移问题，自监督学习(SSL)在泛癌、多示踪剂PET-CT中尚未被充分探索。

Method: 提出MUST-PET框架，使用上下文感知的掩码重建方法——对一个模态进行部分掩码，利用PET和CT的互补信息进行重建。预训练后使用标注样本进行微调。

Result: MUST-PET降低了重建误差，相比从头训练提高了病变分割性能，在有限标注数据和外部数据集上表现良好。

Conclusion: 多示踪剂自监督学习具有实现全身PET-CT病变分割的标签效率和泛化能力的潜力。
...
TL;DR: 本文提出Scaffolding Minds方法，解决隐式推理框架中的两个关键限制：监督微调阶段视觉编码器表征次优，以及强化学习阶段隐式组件探索不足。通过学习专用脚手架编码器优化潜在空间表征，同时学习RL采样器的均值和方差，在FrozenLake空间规划任务中提升9.5%（32x32网格地图达19%），在九个视觉中心推理基准上平均提升5.2%。


<details>
  <summary>Details</summary>
Motivation: 当前隐式推理框架存在两个关键限制：(1) 监督微调阶段使用现成视觉编码器编码辅助图像，产生的潜在表征可能与下游推理任务对齐不佳；(2) 现有RL方法仅通过确定性正则化处理隐式组件，限制了策略漂移但未创建替代隐式轨迹进行探索。

Method: 提出Scaffolding Minds方法，包含两个核心改进：(1) 学习专用脚手架编码器(scaffolding encoder)，在潜在空间提供优化目标；(2) 学习RL采样器的均值和方差，而非仅使用确定性方法。

Result: 在FrozenLake空间规划任务中比最强隐式推理基线提升9.5%，在32x32网格地图上提升扩大至19%，在九个视觉中心推理基准上平均提升5.2%。

Conclusion: 两个改进具有互补性，共同带来显著性能提升。该方法通过优化潜在空间表征和增强探索能力，有效增强了多模态推理能力。
...
TL;DR: 本文提出了一种基于强化学习的稀疏关键点学习新方法RIPE++，通过从单个正图像对中提取几何一致性信号作为监督信号，无需负样本即可学习具有判别性的检测器和描述子，并能扩展到匹配阶段。


<details>
  <summary>Details</summary>
Motivation: 现有RL方法如RIPE依赖粗粒度二元奖励和精心设计的负样本对，训练不稳定且描述子判别性不足。现实场景中难以获取相机姿态或深度监督，因此需要一种仅需图像对是否显示同一场景即可训练的方法。

Method: 提出一种充分利用几何一致性信号的奖励机制，从单个正图像对中同时导出奖励和惩罚，无需对比负样本。这种更丰富的信号提供了足够的监督对比来从正图像对中学习判别性检测器和描述子。将相同RL目标扩展到LightGlue匹配阶段，实现弱监督训练。

Result: 在MegaDepth1500上AUC@5从56.58提升至59.65；在低纹理医学视频序列上成功训练，而标准SfM流程通常会失败；与全监督方法相比具有竞争力的结果。

Conclusion: 该方法实现了仅使用正图像对（甚至只有部分视觉重叠）进行全稀疏匹配流程的弱监督训练，在缺乏相机姿态的医学图像场景中具有重要应用价值。
...
TL;DR: 研究针对水下机器人感知在视觉退化条件下的鲁棒性问题，提出利用声纳辅助视觉基础模型（DINOv2）的多模态融合方法。通过构建五级退化基准测试，验证了 degradation-aware gated fusion 机制能够自适应调整模态贡献，在极端退化条件下实现33.5%的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 水下环境中光学图像易受浊度、波长依赖衰减、低光照、散射和模糊影响而退化，虽然声纳受光学可视性影响较小，但现有视觉-声纳研究主要关注特征对齐和检测性能，缺乏对跨模态鲁棒性的深入研究。

Method: 使用冻结的DINOv2作为视觉编码器，构建从清洁到极端退化的五级受控基准；比较了传统视觉检测、冻结基础模型表示、声纳上下文、固定多模态融合、清洁训练的自适应门控和退化感知的门控融合等多种方法；在保持视觉和声纳编码器冻结的情况下训练融合机制。

Result: 在极端组合退化条件下，DINOv2基线达到0.4610的平衡准确率，退化感知视觉-声纳融合达到0.6152，提升33.5%。学习到的声纳贡献从清洁条件下的14.2%增加到极端退化下的41.3%。融合在严重浊度和模糊下增益最大，而单独的颜色衰减几乎无额外收益。

Conclusion: 基础模型表示仍有价值但在严重信息丢失时不足；明确针对模态可靠性调整融合可改善鲁棒的水下多模态感知。自适应门控机制能够根据退化程度动态分配模态权重，实现跨模态依赖的自适应重分配。
...
TL;DR: SEFS是一种无风格编码器的扩散transformer风格化框架，通过随机低分辨率裁剪提取风格 token，利用边缘和分割线索编码目标内容，通过风格到去噪的重归一化和跨块跳跃融合实现高质量风格迁移，在艺术风格化基准测试中提升了内容一致性和参考风格亲和力。


<details>
  <summary>Details</summary>
Motivation: 基于参考的扩散风格化需要分离目标几何与可迁移外观。现有的调优方法依赖对齐的内容-风格-目标三元组或辅助视觉编码器，增加了数据成本并可能从风格参考中迁移意外的场‍景结构。研究者希望提出一种无需风格编码器的轻量级框架来解决这些问题。

Method: SEFS采用以下核心设计：(1) 从单张训练图像的随机低分辨率裁剪中提取风格token，保留局部外观统计（如调色板、笔触、纹理、材质）同时减少全局布局信息的访问；(2) 使用边缘和分割线索编码目标内容，并与噪声潜在变量通过参数高效的可训练投影层融合；(3) 引入风格到去噪的重归一化实现token统计对齐；(4) 跨块跳跃融合保留空间细节。模型在未配对的单图像上训练，冻结的扩散VAE仅用于将图像条件放入潜在空间。

Result: 在艺术风格化基准测试中，SEFS提升了内容一致性，减少了风格泄漏，同时保持了参考风格亲和力。消融实验验证了裁剪分辨率、重归一化和跳跃融合设计选择的有效性。

Conclusion: SEFS通过无风格编码器的设计显著降低了数据需求和计算成本，成功在保持风格迁移质量的同时解决了传统方法中场景结构意外迁移的问题。该框架为基于扩散模型的风格化提供了新的轻量化解决方案。
...
TL;DR: StreamSoccer是一个用于实时足球解说的事件驱动流式视频理解系统，通过事件内存作为中间表示，在固定预算下整合视频流并生成三种解说模式（当前事件、近期窗口、历史记忆），在三个赛道中分别获得38.62、23.96、17.39的CIDEr分数。


<details>
  <summary>Details</summary>
Motivation: 流式视频理解需要在有界计算和内存下动态更新状态并组织历史信息，这在实时足球解说中尤为挑战，系统需要在每个 utterance 前仅使用可用信息描述已完成事件、总结近期比赛、回忆早期事件或保持沉默。

Method: 提出StreamSoccer系统，包含：(1)固定预算主动内存整合视频流；(2)本地保留已完成事件状态并整合为可检索历史记录；(3)统一生成器利用当前、近期和历史上下文生成三种解说模式；(4)规则辅助调度器选择模式或沉默。与基于帧、视觉 token 或缓存的流式视频语言模型不同，该方法显式建模事件生命周期。

Result: 在公共参考锚点上，StreamSoccer的CIDEr分数分别为：当前事件38.96、历史记忆17.39（均为第一）、近期窗口23.96（第二）。控制性消融显示本地已完成事件改善所有赛道。58场比赛174次原始视频运行中，每分钟RTF p95在0.10-0.22之间，匹配历史无持续增长。

Conclusion: 事件内存支持跨时间范围的流式足球解说，同时控制长历史计算。系统能够在保持计算效率的同时，实现多时间范围的语义理解和生成。
...
TL;DR: TempJail是一种针对大型视觉语言模型(LVLMs)的视频越狱攻击框架，通过优化字幕的时序调度来利用LVLMs的时序漏洞，在4个LVLMs和2个数据集上实现最高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有视频越狱方法主要操纵视频中的文本内容，忽视了信息的时间组织方式。研究发现越狱效果不仅取决于文本语义，还取决于其时间呈现（持续时间和时隙分配）。

Method: 提出TempJail黑盒视频越狱框架，利用字幕作为攻击媒介（字幕在现实视频中常见，可精确控制时序且不显得视觉侵入），构建查询对齐的对话式字幕序列并优化其时序调度。

Result: 在4个LVLMs和2个数据集上实验表明，TempJail在所有评估的模型-数据集设置中达到最高攻击成功率，比最强基线在GPT-5和Gemini 3.5-Flash上分别提升53和18个百分点。

Conclusion: 时序呈现是视频越狱的关键因素，字幕是一种有效且实用的攻击媒介，TempJail成功利用了LVLMs的时序漏洞实现了高效越狱。
...
TL;DR: 从单一舒张末期(ED)网格合成完整周期双心室运动的几何框架，结合区域特定表型自适应方法


<details>
  <summary>Details</summary>
Motivation: 常规获取密集且时间一致的3D+t双心室网格较为困难，而舒张末期(ED)解剖结构易于可靠获取，因此需要从单一ED网格实现完整周期双心室运动合成

Method: 提出区域特定表型自适应框架：(1)从重建运动中学习功能分区，将心室表面组织为动态一致区域，实现拓扑感知的区域特征交换；(2)表型条件化修正流模型通过细粒度条件化和原型路由运动适配器将ED解剖映射到完整周期运动潜空间；(3)可选控制分支整合可用运动描述符实现可控合成

Result: 在ACDC、M&Ms和M&Ms-2数据集上，双心室ASSD、HD95和vRMSE分别达到1.49±0.34mm、3.77±1.06mm和3.31±1.03mm，优于所有竞争方法

Conclusion: 合成序列保留生理学上合理的心室动态，并可跨队列和疾病表型泛化，在几何精度和功能保真度方面均展现持续改进
...
TL;DR: Q-Guide是一个文档VQA系统，通过在推理时进行更慢、更审慎的感知而非一次性编码，来解决多模态大语言模型读取文档不可靠的问题。它能判断需要什么证据并调用针对性工具（读取文本、放大细节或定位区域）。


<details>
  <summary>Details</summary>
Motivation: 当前文档VQA系统将感知视为固定的：一次性编码页面后直接问答。但多模态LLM在直接视觉推理时经常无法可靠读取文档，小文本、表格、视觉线索和拓扑元素仍然困扰它们。需要更慢、更审慎的感知方式。

Method: 构建Q-Guide代理：读取问题→判断缺失证据→调用针对性工具（读文本/放大细节/定位区域）→再回答。测试了三个Claude backbone（Opus 4.6, Sonnet 4.6, Opus 4.5）。

Result: 在DocVQA2026上：65.0% vs 40.0%（直接提示）；在Manga109上：32.4% vs 24.4%。准确率随感知预算提升，多数收益在2-3轮内实现。

Conclusion: 收益来自于将感知导向正确位置，而非复杂控制逻辑：添加规划器、路由器或多代理协作无帮助。核心洞见是推理时的审慎感知比一次性编码更有效。
...
TL;DR: 本文介绍了作者参加PhysAI动态4D重建挑战赛获得第三名的方案，APD为0.58356。核心发现是微调会损害模型性能，因此采用冻结主干网络结合推理时测试增强的策略，在零训练成本下取得了优于微调的效果。


<details>
  <summary>Details</summary>
Motivation: 作者最初计划通过微调预训练的4D主干网络来提升性能，但发现大多数微调配置反而降低了挑战赛成绩。这一反常现象促使他们深入研究基准测试的结构，以理解性能下降的根本原因。

Method: 1)分析基准测试结构，发现评估集中只有25%的数据属于训练时可用的数据变体；2)冻结预训练主干网络以保留特征表示；3)在推理阶段融合三种解码配置：时间步长3、水平翻转测试时增强和密集步长1；4)使用凸权重加权集成。

Result: 1)十三种微调配置中，十二种降低了挑战赛分数，十一种同时提升了本地验证分数；2)冻结基线模型APD为0.58356，获得第三名；3)集成策略相比冻结基线提升了+0.041 APD，超越所有训练运行的效果，且零训练成本。

Conclusion: 基准测试的结构导致了本地验证与测试集性能的反向关系：适应可用数据会损害模型在剩余75%评估数据上的表现。冻结预训练特征并利用推理预算进行测试时增强是更有效的策略，在零训练成本下超越了微调方法。
...
TL;DR: 提出了一种基于地理隔离度的课程学习策略，用于自监督遥感图像预训练。该方法利用地理坐标元数据对样本进行排序，无需图像解码、模型反馈或人工标注，可同时适用于对比学习和重建学习目标。在MoCoV2和MAE中集成后，仅需20-40%训练预算即可达到基线性能，BigEarthNet上提升5 mAP，计算成本降低140倍。


<details>
  <summary>Details</summary>
Motivation: 自监督预训练通常将所有样本视为同等重要，但遥感影像在地理和视觉结构上存在较大差异。现有方法如视觉复杂度代理存在计算成本高、难以扩展的问题。该研究提出利用地理隔离度作为无标签代理指标来排序样本，实现更高效的课程学习。

Method: 1. 提出地理隔离度(geographic isolation)作为样本难度指标，仅需地理坐标元数据<br>2. 集成到MoCoV2(对比学习)和MAE(重建学习)两种预训练框架<br>3. 使用CopernicusBench的三个下游任务进行评估(BigEarthNet、DFC-2020、LCZ)<br>4. 采用CKA和effective-rank分析 embedding 空间特性

Result: 1. MAE仅需20%、MoCo仅需40%训练预算即可达到基线最终轮性能<br>2. BigEarthNet上提升5 mAP，其他基准提升1-5点<br>3. 预计算成本降低140倍(4秒 vs 568秒 on SSL4EO)<br>4. 课程训练编码器形成更高维度的embedding空间

Conclusion: 地理隔离度是有效的课程学习样本排序指标，可显著降低训练成本同时保持或提升性能。该方法计算效率高、扩展性好，适用于大规模遥感数据集的自监督预训练，为地理感知的多模态课程学习提供了新思路。
...
TL;DR: 本文提出了一种新的耦合最优传输框架，利用少量标注的地标点来引导恢复控制分布变换的底层变形场，将传输计划优化与变形场优化统一到同一个模型中。


<details>
  <summary>Details</summary>
Motivation: 现有最优传输模型仅最小化传输成本或失真，可能无法识别出几何意义上有效的分布间变换，需要引入几何约束来解决这一问题。

Method: 提出耦合最优传输框架，将地标引导的变形场与成本驱动的传输计划通过 mutual-consistency constraint 进行耦合，开发了基于有限元的数值算法。

Result: 建立了模型在一般变分设置下的适定性，算法收敛性得到系统分析，在形状匹配任务中验证了有效性。

Conclusion: 该框架为基于地标的配准和基于传输的分布匹配提供了原则性连接，能够从稀疏几何监督中恢复传输映射。
...
TL;DR: Core-KAN是一种相对尺度条件下的连续卷积算子，通过将输入特征映射到紧凑的潜在基空间，并使用轻量级尺度控制器预测相对于指数移动平均参考的局部尺度，实现了几何尺度适应与内容相关滤波的解耦。


<details>
  <summary>Details</summary>
Motivation: 传统卷积核在固定离散网格上定义，无法适应异构局部结构；现有自适应算子虽然提高了灵活性，但通常将几何尺度变化与内容相关滤波耦合，且因每位置生成核而产生高计算成本。

Method: 提出Core-KAN，包含：1）KAN生成器将深度核基表示为连续坐标函数，可在任意分辨率合成空间滤波器；2）不逐位置合成独立核，而是构建紧凑的尺度条件核响应库，根据预测的局部尺度图进行插值；3）独立混合控制器基于局部内容组合插值基响应，明确解耦几何尺度适应与内容相关过滤。

Result: Core-KAN在三个代表性视觉任务上始终优于强卷积和动态核基线，仅产生边际参数和计算开销。

Conclusion: Core-KAN提供了一种高效、通用的连续尺度自适应卷积框架，可轻松集成到分层视觉骨干网络中。
...
TL;DR: 提出一个结合训练时对齐和推理时对齐的医学图像captioning框架，通过多编码器设计、概念级辅助学习、推理时重排序和MedPAIR-SCST强化学习方法来提升临床对齐效果。


<details>
  <summary>Details</summary>
Motivation: 医学图像captioning面临独特挑战：灰度成像模式、细微解剖线索、专业医学术语和数据质量差异；现有大视觉语言模型的流畅输出无法保证与临床概念空间的充分对齐。

Method: 1) 构建基于BioMedCLIP和SigLIP2的单/双视觉编码器+Q-Former+LLaMA解码器的医学图像captioningpipeline；2) 引入UMLS概念/类型预测的辅助学习；3) 推理时使用单embedding重排序选择最佳caption；4) 训练时提出MedPAIR-SCST，结合临床相关奖励改善生成分布。

Result: 互补视觉表示配合多编码器设计和概念级辅助学习有助于保留临床有意义信息；推理时重排序可在无需额外训练的情况下改善语义和临床对齐；MedPAIR-SCST超越选择方法，直接改进模型分布以生成更一致、更临床化的caption。

Conclusion: 联合利用基于选择的对齐和基于强化学习的对齐，可在数据受限场景下促进更可信的医学图像captioning。
...
TL;DR: AutoLumNet是一个单次曝光校正框架，将任务分解为全局单调 tone curve 和有界局部残差。全局 tone curve 参数化为正密度函数的归一化累积积分，确保严格单调性。方法保留下 pairwise luminance ordering 和空间极值，包含最优传输映射。可微的排序样本 Wasserstein-2 目标驱动曲线学习。局部残差解码器处理全局映射无法处理的空间变化效果。


<details>
  <summary>Details</summary>
Motivation: 现有曝光校正方法缺乏形式化保证，难以同时处理欠曝、过曝及混合情况。现有方法未能在单一框架中结合结构性单调性、最优传输最优性和有界局部自适应性。

Method: 1) 全局tone curve：参数化为正密度函数的归一化累积积分，确保严格单调性 2) 可微排序样本Wasserstein-2损失驱动曲线学习 3) 有界残差解码器：双分支凸融合处理局部效果

Result: 在5个基准数据集(MSEC, SICE, LCDP, LOL-v1, LOL-v2-real)上达到SOTA的PSNR和SSIM，每帧11.2ms，无需微调即可零样本泛化到纯低光基准

Conclusion: 首次在单一可训练架构中统一了结构性单调性、最优传输最优性和有界局部适应性，在欠曝和过曝场景下均表现优异。
...
TL;DR: 本文提出了一种基于数据驱动的微无人机视觉自主导航方法，针对GPS拒止、无线电受限的室内灾害救援环境。该方法使用360度全景摄像头，通过卷积神经网络学习偏航角预测，使无人机在通信丢失时能够自主飞向开放空间。


<details>
  <summary>Details</summary>
Motivation: 灾害响应场景（如火灾、化学泄漏、放射性物质事故等）中，微型无人机可提供快速态势感知，但通信中断时无人机可能丢失。通过开发无需GPS和通信链路的自主偏航控制方法，确保无人机能够安全返回并保存宝贵的传感器数据。

Method: 1) 自定义配备360度摄像头的微无人机平台；2) 开发预处理管道，将等距长方投影360度 footage转换为平面前视图；3) 动态生成图像-标签对用于AI训练；4) 训练并比较多种CNN变体进行连续偏航角预测；5) 半自主真实世界测试验证可行性。

Result: 在保留的测试集上的评估证实了学习的偏航预测方法的可行性。半自主真实世界测试进一步证明了该方法的实用性，同时揭示了关键失败模式，特别是反射和眩光问题。

Conclusion: 基于视觉的自主偏航控制方法在GPS拒止环境中具有应用前景。未来的工作需要解决反射和眩光等视觉干扰问题，以提高系统在复杂真实环境中的可靠性。
...
TL;DR: 提出DIFFCZSL框架，将扩散模型的生成先验注入到基于CLIP的CZSLpipeline中，通过对比对齐CLIP嵌入和扩散特征来增强组合感知语义表示。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的CZSL方法主要依赖判别性表示，可能无法明确保留原始概念与其组合之间的结构化关系。扩散模型已展示出与判别模型相当的竞争力，希望利用中间扩散表示为CZSL提供补充线索。

Method: 提取中间扩散表示并投影到CLIP嵌入空间，为图像和文本模态提供辅助监督；训练时通过对比对齐CLIP嵌入和扩散特征，鼓励嵌入几何朝向更丰富的组合感知语义，且推理时不增加额外计算成本。

Result: 在三个公共CZSL基准数据集上，相比强CLIP基线方法，在闭世界和开放世界设置下均获得一致的性能提升。

Conclusion: 结果突出了生成式扩散表示与判别式视觉语言模型在组合泛化方面的互补优势，DIFFCZSL能有效利用扩散模型的生成先验增强CZSL性能。
...
TL;DR: 提出一种统一的高效点和线特征提取器(UPAL)，在单一轻量级架构中联合提取关键点、线段和特征描述子，在点和线应用中都达到或超越最先进性能，同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有点线方法效率低下：分别检测点和线、使用日益笨重的网络、依赖CPU绑定的启发式方法，阻碍实时性能。

Method: 共享骨干网络提供公共表示，馈送到点和线特征的不同分支；线段通过加速后处理阶段（增强的LSD算法变体）恢复。

Result: 在点和线应用中都匹配或超越最先进性能，实现4倍加速和10倍更小内存占用（相比ALIKED + DeepLSD管道）。

Conclusion: UPAL在保持高性能的同时显著降低了计算成本，为实时多视图计算机视觉 pipelines 提供了更高效的解决方案。
...
TL;DR: AvatarDynamizer是一个生成式方法,可以将现成的静态3D头像转换为可控制的、逼真的且多视角一致的四维头像。该方法通过引入纹理空间表面动力学嵌入,将头像动力学建模为条件纹理生成,并使用编码器-解码器表示将姿态依赖的动力学嵌入动态纹理图,再解码为3D高斯进行多视角一致渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在以下问题:1)人物无关方法虽然可以从单目图像、视频或文本提示恢复静态3D头像,但其骨骼驱动动画缺乏真实的表面动态(如衣物褶皱);2)人物特定方法虽然能实现高质量渲染和逼真动态,但需要为每个人进行昂贵的多视角捕获;3)现有的可泛化动态头像方法难以嵌入表面动态,导致多视角一致性或动态表现力有限。

Method: 1)提出纹理空间表面动力学嵌入,将头像动力学建模为条件纹理生成;2)使用编码器-解码器表示,将姿态依赖的动力学嵌入动态纹理图;3)与预训练视频扩散模型兼容,并解码为3D高斯进行多视角一致渲染;4)收集大规模多视角数据集,包含长序列、多样骨骼运动和表面动态。

Result: 该方法能够有效地为静态头像添加真实的表面动态动画，在视觉质量上优于竞争的可泛化方法，尤其是在动态训练数据有限的情况下表现突出。

Conclusion: AvatarDynamizer成功地将静态3D头像转换为具有可控、逼真、多视角一致的4D头像,为解决动态头像建模中的表面动态问题提供了有效途径。
...
TL;DR: 3D-CurvSegFlow是一种基于流匹配的三维 curvilinear 结构分割模型，能够高效地对血管等 curvilinear 结构进行渐进式精细分割，在三个公开数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 三维医学图像中 curvilinear 结构（如血管）的分割面临复杂拓扑、严重类别不平衡、对比度弱和形态变化大等挑战。现有深度学习方法通常针对特定解剖结构或模态，泛化性有限。扩散模型虽能进行迭代预测，但计算成本高，难以处理高分辨率三维体数据。

Method: 提出3D-CurvSegFlow模型，基于流匹配（flow matching）技术，学习从简单源分布到目标血管表示的连续变换，实现复杂 curvilinear 几何的渐进式精细化分割，具有高效的推理速度。

Result: 在三个公开数据集（门静脉、脑血管、冠状动脉）上使用统一架构和训练策略，3D-CurvSegFlow性能优于通用方法和血管专用方法，能有效保留细小分支并保持血管连续性。

Conclusion: 该工作推进了三维 curvilinear 分割的技术水平，同时为医学图像分析提供了高效、泛化性强且临床适用的新方法。
...
TL;DR: 本文提出Co-3DGT框架，解决开放词汇3D目标检测中新颖目标发现不准确和模型训练不稳定的问题。通过共蒸馏策略利用匈牙利匹配结合几何一致性、结构客观性和语义确定性来发现高质量新颖目标，并采用双指导学习方案（场景感知不确定性正则化和LLM引导的层级对齐）增强模型训练的鲁棒性。在SUN RGB-D和ScanNetV2上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法采用两阶段流水线，先使用基础模型发现新目标，再训练检测模型。但该流程存在发现阶段定位不准确和分类不匹配的问题，限制了模型训练阶段的性能。

Method: 1. 共蒸馏策略：对新颖目标进行匈牙利匹配，综合几何一致性、结构客观性和语义确定性的综合评分来提炼高质量新颖目标。2. 双指导学习方案：包含场景感知引导的不确定性正则化（用于回归头）和LLM引导的层级对齐（用于分类头），缓解不精确3D边界框和语义模糊的负面影响。

Result: 在SUN RGB-D和ScanNetV2数据集上取得了显著的性能提升，超越了现有最先进方法。

Conclusion: 通过改进新颖目标发现的可靠性和增强模型训练的鲁棒性，本文提出的Co-3DGT框架能有效提升开放词汇3D目标检测的性能。
...
TL;DR: 本文提出了一种基于骨架的视频异常检测（VAD）方法STEP，通过利用PCA将姿态序列投影到紧凑的白化PC空间，并在该空间中训练能量基于模型（EBM），以解决直接向原始关节坐标注入噪声导致的物理不可行姿态和结构崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于骨架的VAD方法在训练能量基于模型（EBM）时，需要向原始关节坐标注入噪声，但这种方法会产生物理不可行的姿态，且随着时间窗口扩展，结构崩溃问题会加剧。此外，由于遮挡或运动模糊导致的姿态估计误差也会影响检测性能。

Method: 本文提出的STEP框架包含以下关键步骤：1）利用主成分分析（PCA）将姿态序列投影到一个紧凑的、白化的PC空间；2）在该空间中训练EBM以学习正常姿态的数据密度；3）引入基于估计器置信度分数的序列级加权机制，以缓解姿态估计不准确的问题。该方法在实时计算效率下运行。

Result: 在具有挑战性的UBnormal数据集上，STEP实现了90.1%的AUROC，比之前的基于骨架的state-of-the-art方法提高了12.2%。同时，在ShanghaiTech基准测试中也取得了极具竞争力的结果。

Conclusion: STEP框架通过在白化的PC空间中学习数据密度，有效解决了注入噪声导致的物理不可行姿态问题，能够处理更长的视频序列而不会出现性能崩溃。结合置信度加权机制，该方法在保持实时效率的同时，显著提升了基于骨架的VAD性能。
...
TL;DR: 提出了一种基于不透明度beta surfels的可微点渲染方法，用于稀疏视角3D重建。该方法结合了基于物理的光传输优化，在仅使用约267个surfels的情况下，实现了最低的平均对称Chamfer距离，比最强的点基线方法提高了28.5%。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角3D重建是一个难题，现有方法要么使用神经隐式表面，要么使用密集点云表示（如Gaussian splatting）。表面感知的方法通过定向图元和正则化改进几何提取，而RadiosityGS通过 radiosity启发的有限元surfels formulation融入可微光传输。作者希望提出一种更紧凑、更高效的重建方法。

Method: 提出了一种基于不透明度bearing beta surfels的可微点渲染方法。具体包括：(1) 使用不透明度显式伴随光传输公式，为surfels几何和外观参数提供梯度；(2) 利用基于物理的光传输来约束重建过程。该方法在受控直接照明设置下工作。

Result: 在5个合成对象的10个视角重建中，该方法实现了最低的平均对称Chamfer距离，比最强的点基线方法减少了28.5%的Chamfer距离，同时仅使用约267个surfels（约~161个更少的图元）。方向Chamfer结果进一步显示了在准确性和完整性方面的改进。

Conclusion: 结果表明，在受控直接照明设置下，紧凑的beta surfels结合光传输优化可以恢复表面，无需依赖基线方法使用的数万至数十万个图元。该方法在保持紧凑表示的同时实现了更高精度的重建。
...
TL;DR: 本文提出SAGAI工作流程，利用视觉语言模型(VLM)和谷歌街景影像对法国尼斯东北郊区的街道景观品质进行大规模评估，以支持15分钟步行城市的规划诊断。


<details>
  <summary>Details</summary>
Motivation: 传统实地调查耗时耗力，难以在大规模郊区和城乡交界区域评估街道景观品质；15分钟步行城市理念对街道品质提出新要求，需要可扩展的评估方法。

Method: 使用最新版SAGAI开源工作流（改进的图像采集、地理一致视图生成、多VLM架构支持、共识推理和集成分析环境），基于谷歌街景影像对数千个街道观测点进行人行道存在、人行入口密度和植被三项指标评估。

Result: 理想街道景观品质仅占现有郊区街道的一小部分，主要分布在紧凑型开发区域和传统郊区；而住宅山坡区域尤为缺乏。VLM能够有效识别这些空间差异。

Conclusion: 视觉语言模型可支持大规模城郊区域的规划诊断，使城市公共空间质量的评估变得可扩展、灵活且可解释，为循证规划提供新工具。
...
TL;DR: 本文提出利用IMU数据和特征描述子中的局部几何信息，进行部分校准的绝对姿态估计（同时估计相机姿态和焦距），无需传统方法的4个点对应，仅需1-2个特征即可完成求解。


<details>
  <summary>Details</summary>
Motivation: 传统姿态估计方法未能利用SIFT等特征描述子中的局部几何信息，且部分校准的绝对姿态估计（联合估计绝对姿态和焦距）这一领域尚未被充分探索。IMU数据可提供重力矢量，为利用局部几何信息提供了条件。

Method: 利用IMU获取的重力矢量和特征诱导的局部几何信息，构造两个高效求解器：UP1PfAC（基于单个仿射对应）和UP2PfORI（基于两个方向协变特征）。

Result: 相比传统半校准绝对姿态方法需要4个点对应，本方法只需更少样本，计算成本更低。在大规模公共数据集上实现了快速准确的定位和焦距估计。

Conclusion: 通过融合IMU惯性数据与视觉特征的局部几何信息，可有效解决部分校准绝对姿态估计问题，显著降低了对点对应数量的需求，提高了计算效率，适用于现代RANSAC框架。
...
TL;DR: 研究挑战了创建领域专家必须微调大型基础模型的假设，通过重新设计VLM管道（从文本tokenization到预训练、grounding和推理），在兽医放射学领域从头训练出超越大型基础模型的领域专家模型。


<details>
  <summary>Details</summary>
Motivation: 通用VLMs训练成本高昂，业界普遍认为创建领域专家需要对越来越大的基础模型进行微调。作者认为这一假设在兽医放射学领域是错误的，旨在证明通过精心工程设计可以从头训练出优于大型基础模型的领域专家。

Method: 重新思考整个VLM pipeline：1) 文本tokenization优化；2) 生成式预训练策略改进；3) grounding策略创新；4) 推理效率提升。通过提高训练效率和数据利用率，在有限的参数、数据和计算资源下训练模型。

Result: 开发了首个能够为兽医X光片生成诊断报告的VLM，仅使用通用模型一小部分的参数、数据和计算资源，在该任务上显著超越开放基础模型。

Conclusion: 在兽医放射学领域，创建领域专家不一定需要微调大型基础模型；通过精心设计的从头训练方法，可以以更少的资源获得更好的领域特定性能，为低成本高效构建领域专家模型提供了新思路。
...
TL;DR: HandMvNet是首个从多视角相机图像实时估计3D手势和形状的方法，通过多视角注意力融合机制有效整合多视角特征，无需相机参数即可学习3D几何，在保持实时性的同时达到先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目方法存在尺度-深度歧义问题，以往多视角方法需要相机参数作为输入，且推理时间较长，无法满足实时应用需求。

Method: 提出多视角注意力融合机制，有效整合多视角特征；无需相机参数输入即可学习3D几何。

Result: 在公开数据集上定性和定量评估均优于先前方法，推理时间大幅降低，兼具实时性与竞争性准确率。

Conclusion: HandMvNet实现了无需相机参数的实时3D手势和形状估计，在精度和速度上均达到先进水平，适用于实时应用场景。
...
TL;DR: 研究结构化、无梯度的免疫亲和力是否能使深度人工免疫网络(Deep AINs)成为无需重放的视觉类增量表示-记忆学习器。将视觉B细胞形式化为结构化模板，包括移位模板亲和力、零归一化交叉相关性(ZNCC)滤波器和特征图结合谱。 repertoire同时作为记忆和表示诱导基深度通过将结合谱响应图传递到后续免疫层获得。Deep AIN展现自适应潜在坐标重组：新类到来时，结合谱空间演进同时保留先前类的可恢复结构。


<details>
  <summary>Details</summary>
Motivation: 传统视觉人工免疫网络(AINs)依赖扁平化向量亲和力，忽略空间结构。研究旨在探索结构化、无梯度免疫亲和力是否能支撑Deep AINs作为无需重放的视觉类增量学习器。

Method: 提出结构化模板方法：1)视觉B细胞形式化为结构化模板（移位模板亲和力、ZNCC滤波器、特征图结合谱）；2)将repertoire同时视为记忆和表示诱导基；3)通过结合谱响应图传递到后续免疫层获得深度；4)采用自适应层间尺度校准。

Result: sklearn digits: 逻辑回归最终平衡准确率0.939，1-近邻0.902，首类保留0.978；双层特征图Deep AIN达0.978平衡准确率；Fashion-MNIST达0.814，KMNIST达0.853。实验表明：响应图保留至关重要，标量变体表现较差，特征图Deep AIN无需重放、标签驱动免疫更新或免疫层反向传播即可学习类判别视觉记忆。

Conclusion: 结构化亲和力、响应图保留、自适应潜在重组和层间尺度校准是实现无需重放视觉免疫记忆的关键机制。
...
TL;DR: BeyondMasks是一个用于因果一致性视频物体消除的配对基准测试，解决了现有方法忽视物理效应（阴影、反射、透明度、动态轨迹）的问题，并提出CORE评估协议来衡量物体消失和后续效应的一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频物体消除的评估协议仅关注掩码区域的保真度，将消除视为局部修复。但真实场景中，物体消除是因果干预：消除物体还需移除其引起的物理效果。现有基准缺乏对齐的干净参考或局限于简化的合成环境，无法系统评估因果一致性。

Method: 1) 构建BeyondMasks数据集：包含时间对齐的合成和真实世界视频对及干净背景参考，涵盖多样的光度、几何、体积和动态交互，支持掩码驱动和指令驱动的编辑；2) 提出CORE评估协议：基于结构化视觉语言模型，联合测量物体消失和物理效应一致性，与人类判断更一致。

Result: 对最先进方法的基准测试揭示：尽管掩码区域保真度高，但在消除次级物理效应方面存在系统性失败，暴露了视觉 plausibility 与因果正确性之间的差距。

Conclusion: BeyondMasks将视频物体消除重新定义为因果场景一致性而非局部重建，并为其评估提供了统一框架。
...
TL;DR: 本文提出AdvSpot首个基于定位的对抗性OCR评估基准，以及ArmorOCR两阶段训练框架，用于提升大型多模态模型在对抗性视觉文本上的定位和识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有OCR基准主要关注自然或文档风格文本，对抗性OCR评估在规模、任务覆盖或区域感知评估方面仍然有限，且大型多模态模型虽然具备强大OCR识别能力，但仍易受对抗性视觉文本攻击。

Method: 提出ArmorOCR两阶段训练框架：第一阶段通过On-Policy Self-Distillation (OPSD)从特权转换观察中获取缺失的对抗性OCR感知；第二阶段通过Group Relative Policy Optimization (GRPO)和任务条件奖励进行定位、识别、完整定位和VQA的精细化训练。

Result: AdvSpot包含390张图像，5个主要类别和13个细粒度对抗性OCR类型。实验表明ArmorOCR在AdvSpot及其他对抗性和通用OCR基准上均显著提升对抗性OCR感知能力，同时保持竞争力。

Conclusion: ArmorOCR能有效提升模型在对抗性视觉文本上的感知能力，为鲁棒OCR感知提供新思路，AdvSpot基准可推动该领域进一步研究。
...
TL;DR: 本文提出了图像消歧视频时序定位(ID-VTG)任务，通过结合参考图像和文本描述的多模态查询来解决视觉相似实体的事件定位问题，并构建了两个基准数据集和VGD-Agg框架。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位(VTG)在自然语言查询需区分涉及视觉相似实体的多个事件时面临重大挑战，特别是当依赖难以用文字准确描述的细粒度视觉属性时。

Method: 提出VGD-Agg框架，采用双分支快慢架构。快分支高效生成初步事件提案，慢分支执行视频帧与参考图像的细粒度帧级匹配。引入Compare Token(探测目标实例是否存在)和Depress Value(表示文本无关事件)两个可学习标记来增强区分度。

Result: 在提出的两个基准数据集上实现了最先进的性能。

Conclusion: 通过多模态查询结合图像与文本描述，能够有效解决细粒度视觉相似实体的时序定位问题，VGD-Agg框架在IDVTG-Gym和IDVTG-InternVid数据集上验证了方法的有效性。
...
TL;DR: 本文提出通过Sparse Autoencoders (SAEs)可视化和追踪Vision Transformers在训练过程中的特征演变，从网络深度和训练时间两个维度分析特征迁移现象，发现特征迁移主要发生在训练早期，且更多地向浅层迁移，深层比浅层更早稳定。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注表示级别的相似性，难以直接观察特征层面的动态变化。需要一种方法来理解和追踪ViTs在训练过程中特征是如何学习和演化的。

Method: 使用Sparse Autoencoders从CLS-token表示中提取候选稀疏特征，比较不同epoch-layer对下的激活概况，研究特征级别的动态变化，并描述特征迁移现象（特征最可检测的层在训练中的变化）。

Result: 特征迁移集中在训练早期；迁移更多地向浅层而非深层发生；随着特征组织稳定，迁移减少；深层比浅层更早且更强烈地稳定。

Conclusion: 该方法可作为理解ViTs如何学习和演化的有效工具，为分析视觉Transformer的训练动态提供了新视角。
...
TL;DR: 提出了一种用于全合一图像修复的双先验协作网络(DPC-Net)，通过结合降解-语义耦合先验和低层次视觉先验来提升修复质量。


<details>
  <summary>Details</summary>
Motivation: 现有全合一图像修复方法忽略了图像语义在降解建模中的作用，且在重建过程中缺乏低层次视觉先验，导致结构失真和语义不一致。

Method: 提出DPC-Net框架，包含四个核心组件：1)降解感知网络(DAN)提取降解-语义耦合特征；2)视觉语言模型(VLM)监督DAN，引入图像语义；3)降解-语义调制模块(DSMM)将指导转化为耦合表示并传播到解码器；4)双先验协作重建模块(DPCR)整合双重先验信息指导降解去除。

Result: 在多个修复基准数据集上，DPC-Net相比现有最先进的全合一图像修复方法取得了优异性能。

Conclusion: 通过联合利用降解-语义耦合先验和低层次视觉先验，DPC-Net能够有效去除降解同时保持结构和语义，产生高质量的修复图像。
...
TL;DR: PelviNeXt是一种无需任务特定修改的模态无关混合架构，可同时处理盆腔超声和X光图像，用于多囊卵巢综合征(PCOS)和盆腔骨折的诊断，并在两个任务上均达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 女性健康在医学影像研究中资金不足，盆腔病理如PCOS和盆腔骨折缺乏公开的标注良好的基准数据，尽管它们具有临床重要性。

Method: 提出PelviNeXt架构，融合密集卷积特征提取器、分层通道-空间注意力(H-CBAM)、多尺度融合模块(MSFM)和对话头多头自注意力(TH-MHSA)。同时审计并发布了去重版本的PCOSGen数据集，建立了首个完整性审计的5折交叉验证评估协议。

Result: 在PCOSGen数据集上建立首个审计评估协议和基线；在PXR150盆腔骨折X光数据集上，PelviNeXt在准确率、召回率、特异度和AUROC上均超越之前的最先进方法。消融实验证实每个架构组件对两个任务的性能都有贡献。

Conclusion: 单一架构无需任务特定修改即可作为数据稀缺、女性健康研究不足领域的盆腔影像可靠基础。
...
TL;DR: AI-ColoWorkflow是一种深度学习模型，用于自动分析微创结直肠手术(MIS-CRS)的工作流程。该模型结合了微调的DINOv3视觉Transformer和分层多阶段时间卷积网络，可在多中心数据上实现手术阶段和步骤的自动识别，在阶段识别上达到73.01%的F1分数。


<details>
  <summary>Details</summary>
Motivation: 手动视频评估(VBA)耗时长，限制了ColoWorkflow工具的临床推广，因此需要开发自动化手术工作流程分析工具

Method: 收集4个中心和公开数据集中的手术视频，使用DINOv3视觉Transformer提取帧级视觉特征，结合分层多阶段时间卷积网络进行阶段和步骤识别，训练了合并多中心数据的全局模型并与中心特异性、手术特异性模型进行比较

Result: AI-ColoWorkflow在阶段识别上达到73.01%±10.27的F1分数(平衡准确率73.43%)，步骤识别为39.82%±7.06(平衡准确率38.65%)。全局模型在大多数实验中优于中心特异性和手术特异性模型。泛化分析中阶段识别平均F1为48.42%。

Conclusion: 单一多中心多手术数据训练的模型在MIS-CRS阶段识别上至少与特异性模型表现相当甚至更好，而特定手术的步骤模型仍具优势，为未来混合训练策略的手术AI开发提供了依据。
...
TL;DR: G3Ego是一个基于图的自我中心动作理解框架，利用注视(gaze)作为结构线索来识别场景中与动作相关的实体。该方法从稀疏采样帧构建动作场景图，结合视觉语言描述、锚定物体和手部线索，并通过相机佩戴者的注视来修剪不相关实体，最后对图嵌入进行时间聚合用于动作识别和预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用在第三人称数据上预训练的大型视频模型来处理自我中心动作理解，但许多第一人称动作仅依赖于涉及少量相关实体的手-物交互。现有问题是：1) 大视频模型计算成本高；2) 未利用注视信息来精准定位动作相关实体。

Method: G3Ego框架包含三个核心步骤：1) 从稀疏采样帧构建动作场景图，融合视觉语言描述、锚定物体和手部线索；2) 利用相机佩戴者的注视信息修剪不相关实体，生成聚焦于动作相关交互的图表示；3) 对图嵌入进行时间聚合用于动作识别和预测。与先前工作不同，该方法将注视直接融入图构建过程，而非仅作为辅助模态或注意力信号。

Result: 在EGTEA Gaze+和MECCANO数据集上的实验表明：G3Ego实现了与视频方法相当的竞争性性能；在类别不平衡评估设置下，Macro-F1指标持续提升；同时避免了对计算密集型视频预训练的依赖。

Conclusion: 注视引导的图表示方法对于自我中心动作理解是有效且高效的。该方法通过将注视信息直接整合到图构建中，产生了高效且可解释的表示，专注于动作相关的交互，为无需大规模预训练的轻量级自我中心动作理解提供了新思路。
...
TL;DR: RoMAN-Flow是一个离线强化学习框架，通过解决自回归归一化流(AR-NF)策略的采样瓶颈问题，使其适用于机器人操作。该方法在策略优化阶段使用无需采样的优势加权似然目标，在部署阶段将自回归策略蒸馏为单步动作生成器，实现低延迟动作预测。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习无需与环境进一步交互即可改进机器人策略，但现有的扩散和流匹配策略缺乏可计算的似然值，限制了基于似然的后训练方法的使用。虽然AR-NF能够提供可表达的动作建模和精确的似然评估，但其顺序采样机制在策略优化和部署过程中会产生巨大的采样开销，限制了其实用性。

Method: 提出两个核心技术：(1)采样-free的优势加权似然目标：在策略优化阶段，不需要从自回归策略中采样，而是直接为离线数据集中的高优势动作分配更高的似然值；(2)单步动作生成器蒸馏：将优化后的自回归策略蒸馏为一个单步动作生成器，实现低延迟的动作预测。

Result: 在多个模拟操作基准测试和真实机器人平台上的实验表明，RoMAN-Flow在保持竞争力的策略性能的同时，大幅降低了推理延迟。

Conclusion: RoMAN-Flow成功地将AR-NF策略应用于机器人操作任务，通过创新的优化目标和部署策略，有效解决了自回归采样的计算开销问题，为离线强化学习在真实机器人应用中的部署提供了可行的解决方案。
...
TL;DR: 提出JFSnet框架解决视频中眼镜去除的身份漂移问题，通过商业生成模型提取合成图像、经三阶段结构过滤保留身份特征、结合物理透镜模拟生成配对数据，并利用DINOv2语义特征和平移等变性约束实现高质量时间一致性修复。


<details>
  <summary>Details</summary>
Motivation: 眼镜去除面临复杂折射扭曲和视角相关镜面反射的挑战，现有大规模生成先验缺乏维持身份、表情和姿态的结构约束，导致静态图像和动态序列中出现明显的"身份漂移"问题。

Method: 提出JFSnet（联合特征-空间网络）框架，包含三个主要阶段：1）从商业生成模型提取高保真合成人脸图像；2）通过三阶段结构过滤保留身份、表情和姿态；3）应用基于物理的透镜光学模拟生成多样化配对训练数据。JFSnet集成DINOv2语义特征与卷积解码器，利用平移等变性约束提高时间一致性。

Result: 在FFHQ数据集（12,163张图像）上验证，达到高保真度和结构准确性，推理速度27.68 FPS。在CelebV-Text视频序列的感知研究中，眼部一致性、时间稳定性和整体修复质量方面均优于扩散和GAN基线方法。

Conclusion: 在FFHQ数据集上实现了高保真度和结构准确性，CelebV-Text视频序列的感知研究中在眼部一致性、时间稳定性和整体修复质量方面优于扩散和GAN基线方法。推理速度达到27.68 FPS，具有实际应用价值。
...
TL;DR: 提出Prompt-Conditioned Channel Attention (PCCA)机制，实现语义提示在编解码网络中的深度层次化整合，提升医学图像分割精度


<details>
  <summary>Details</summary>
Motivation: 现有交互式分割方法仅在后期融合提示，缺乏对层次化特征表示的通道级调制机制，难以捕捉深层上下文和模态特定变化

Method: 通过池化提取紧凑通道描述符，投影到共享空间，通过门控激励机制融合，计算提示感知通道注意力权重，自适应重校准多阶段特征响应

Result: PROMISE-CNN在ISIC-Lesion、Kvasir-Polyp、CAMUS-Cardiac、Kvasir-Instrument上相对基线U-Net的IoU提升分别为10.4%、8.7%、0.8%、3.4%；PROMISE-Txformer相对基线UNETR提升分别为7.6%、23.0%、2.1%、1.1%

Conclusion: PCCA和PROMISE-Net作为可扩展、可推广的框架，在不同架构、成像模态和解剖目标上展现一致性提升
...
TL;DR: UHDformer++是一个基于Transformer的通用框架，用于解决多种超高清图像修复任务。该框架在4个协调学习空间中运作：HR空间用于多级特征提取，LR空间用于学习紧凑的代表性特征，SR空间用于对低分辨率特征进行上采样，LHFR空间用于最终图像修复。框架还引入了FR-CMT和ACM两个模块来桥接高低分辨率空间。


<details>
  <summary>Details</summary>
Motivation: 旨在解决超高清图像修复任务中的挑战，需要一个通用的框架来处理各种 UHD 图像修复任务，同时提高计算效率。

Method: 1) 四空间协同学习：HR空间提取多尺度高分辨率特征并与低分辨率线索融合；LR空间从HR中提炼互补表示；SR空间对LR输出进行上采样；LHFR空间进行最终图像修复。2) 特征精炼相关性匹配变换(FR-CMT)模块：从高分辨率特征的融合中选择top C/r通道替换低分辨率Transformer中信息量较少的通道。3)自适应通道调制器(ACM)自适应重新校准多尺度高分辨率特征，确保只有任务相关信息传播到LR。

Result: 与最新的SOTA方法相比，UHDformer++将模型参数减少了至少86%，同时在5项UHD修复任务（包括低光照图像增强、去雾、去模糊、去雨和去雪）中实现了显著的性能提升。

Conclusion: UHDformer++通过多空间协调学习和创新的特征处理模块，成功实现了超高清图像修复任务的高效处理，在大幅减少模型参数的同时达到了优异的性能表现。
...
TL;DR: 本文提出了一种联合视觉-轨迹的世界动作模型，用于同时预测手术场景的未来视觉状态和器械轨迹，解决了现有方法中场景预测与轨迹预测分离导致的一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将未来场景生成和器械轨迹预测视为两个独立任务，场景模型无法在轨迹层面评估器械运动准确性，而轨迹模型无法捕捉器械运动的视觉后果，导致预测的轨迹与未来场景演化之间的一致性未被解决。

Method: 将历史视频帧和工具轨迹编码为潜在表示，经时空编码器处理后，通过独立的视觉状态预测头和轨迹预测头进行解码。在此基础上，采用分块自回归 rollout 策略预测15个未来步骤。

Result: 分块自回归策略在所有评估范围内均优于直接一次性预测，首段PSNR从18.86提升至23.11 dB，ADE从45.77降低至22.22像素。

Conclusion: 初步验证了联合视觉-运动预测的可行性。然而，在更长预测范围内观察到进行性视觉退化和轨迹误差累积问题，这仍是未来手术世界动作建模的重要挑战。
...
TL;DR: CalcSeg是一个置信度感知的潜在上下文课程学习框架，用于从单堆叠2D LGE-CMR图像中进行心肌瘢痕分割。该框架利用融合的3D特征表示，通过动态半监督课程学习和潜在切片自注意力机制来应对低组织对比度、弥漫性瘢痕和有限3D空间上下文的挑战。


<details>
  <summary>Details</summary>
Motivation: 心肌瘢痕分割面临三大挑战：1)低组织对比度导致瘢痕与正常组织难以区分；2)弥漫性和小瘢痕区域难以准确检测；3)单堆叠2D图像缺乏3D空间上下文信息。这些问题严重影响临床诊断的准确性。

Method: 提出CalcSeg框架，包含两个核心创新：1)动态半监督课程学习策略，使用置信度评分函数自动评估样本难度（整合预测错误、认知不确定性和瘢痕负担估计），从简单到复杂逐步扩展训练；2)潜在切片自注意力机制，从稀疏2D输入中捕获切片间依赖关系，推断3D空间表示。

Result: 在多中心临床LGE-CMR数据集上的实验表明，CalcSeg在所有竞争方法中表现最佳，特别是在临床挑战性病例（低对比度、弥漫性、小瘢痕）上实现了显著改进。

Conclusion: CalcSeg通过置信度感知的课程学习和潜在3D上下文建模，有效解决了单堆叠LGE-CMR图像中心肌瘢痕分割的难题，为临床提供了更可靠的分割工具。
...
TL;DR: DreamHand是一种从自我中心视频中恢复度量级3D双手轨迹的离线片段级框架，通过将视频扩散模型重新用途为确定性几何编码器，在5个基准数据集上实现新的最优性能，在遮挡严重和视野外场景中表现尤为突出。


<details>
  <summary>Details</summary>
Motivation: 自我中心视频为具身AI提供了可扩展的操作数据，但恢复度量级3D手部轨迹仍具挑战性，原因包括：1)严重的目标遮挡；2)频繁的视野外间隙。单帧和窗口时序回归器在手部短暂离开画面时失败；3)现有视频扩散模型作为像素空间渲染器需要繁重的随机多步采样。

Method: DreamHand采用两阶段框架：1)Deterministic Clean-Latent Encoder（确定性清洁潜空间编码器）：通过单次前向传播在干净潜空间中暴露超出当前观察范围的场景内容，包括遮挡和视野外的手部；2)Bidirectional Spatiotemporal Decoder（双向时空解码器）：解码特征生成连续的双手机轨迹。此外还提供Ray-Based Camera Solver配置，无需测试时相机内参。

Result: 在5个自我中心基准数据集上设定新SOTA：1)在遮挡严重的ARCTIC数据集上，MPJPE-p降低30%；2)在HOT3D数据集上降低40%；3)当评估中包含视野外手部时，提升幅度达46%-61%。

Conclusion: DreamHand通过将视频扩散模型重新用途为确定性几何编码器，实现了从自我中心视频中恢复度量级3D双手轨迹的突破。在5个基准数据集上达到新的最优性能，特别是在遮挡严重和手部离开视野的场景中表现优异，为从日常人类视频获取机器人操作数据提供了一条可扩展的路径。
...
TL;DR: Inter-X++是一个大规模高保真人体交互数据集，包含11,388个交互序列和超过810万帧，配备详细的多模态标注。同时提出OpenHHI统一框架，同时支持交互重建和语义理解任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集和建模方法受限于低保真运动学、缺少灵巧手部动作、缺乏丰富多模态标注；此外，交互表示碎片化和评估协议不一致也阻碍了公平严格的基准测试。

Method: 1）通过新型混合动作捕捉系统采集高保真交互数据；2）提供多维标注包括细粒度文本描述、交互类别、因果顺序、人物关系和性格、顶点级接触图和物理约束；3）制定四类下游任务涵盖生成和感知范式；4）提出OpenHHI统一框架联合优化交互重建和语义理解。

Result: OpenHHI在生成和感知任务上均达到最优性能，证明了统一表示能够同时桥接交互理解和生成。

Conclusion: Inter-X++数据集和OpenHHI框架为人体交互分析提供了全面的基准和统一解决方案，推动了该领域的发展。
...
TL;DR: Swift-Image是一个6B参数的紧凑统一模型,用于文本到图像生成、单图像编辑和多图像编辑,在仅243K GPU训练小时数下实现了领先的开源模型综合性能。


<details>
  <summary>Details</summary>
Motivation: 探索在受限计算预算下,相对较小的视觉生成器通过系统性训练工程能达到的性能极限。

Method: 采用高效的6B单流DiT和渐进式训练管道(从广泛语义覆盖到更高分辨率、更强视觉质量和统一生成-编辑监督);使用并行专家强化学习后训练,结合多教师在线蒸馏以减轻异构目标间的干扰;引入Prompt Enhancer将用户请求转换为生成器对齐的视觉规范;通过结构化剪枝和少步蒸馏产生3B压缩变体。

Result: 在仅6B参数和243K GPU训练小时数下,在评估的开源模型中实现领先的综合性能;3B模型几乎无性能损失;少步蒸馏在显著减少采样步数的同时进一步提升了编辑综合性能。

Conclusion: 该研究为架构、数据课程、后训练、提示增强和模型压缩提供了实践经验教训。
...
TL;DR: 4DAnyone是一个从无标定单目视频重建4D人体的框架，通过生成重建级的多视角一致视频并将其提升到4D高斯溅射(4DGS)。该方法解决了现有相机控制视频扩散模型在扩展到数十个目标视角时无法保持一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有相机控制视频扩散模型可以合成合理的新视角视频，但在扩展到4DGS重建所需的数十个目标视角时无法保持一致性。研究者识别出这是由于bounded-attention-context问题：当目标视角超过单个DiT前向传播的容量时，必须将其分组，由此产生两个耦合瓶颈——参考上下文侧和目标上下文侧的问题。

Method: 提出两个互补设计：1)参考上下文打包(RCP)将增长的参考视图压缩为固定长度的混合分辨率上下文，实现O(1)参考上下文复杂度；2)目标上下文路由(TCR)在去噪过程中轮换目标视图分组，在高噪声步骤时跨组共享上下文，在低噪声步骤时稳定细节。还构建了MVGameHuman数据集，结合光场阶段和野外视频数据集进行训练。

Result: 在DNA-Rendering和DyMVHumans数据集上的实验表明，4DAnyone在新视角视频质量和下游4DGS重建方面均优于先前方法，并具有强大的野外泛化能力。

Conclusion: 4DAnyone成功解决了多视角视频扩散中的一致性问题，为从单目视频重建4D人体提供了有效方案，在质量和泛化性上都表现出色。
...
TL;DR: WithEveryone是一个统一的群体图像生成框架，可处理最多10个参考身份。该方法通过注入地址标记、预测结构化身份-布局计划并渲染视觉条件来实现身份保真度。核心创新在于Layout-Grounded ID Loss使用标注的人脸区域直接监督目标身份，避免了基于嵌入的不稳定人脸匹配问题。


<details>
  <summary>Details</summary>
Motivation: 当场景需要包含多个指定人物时，身份保留图像生成变得不可靠。模型不仅需要保留每个身份，还需要将每个引用绑定到特定的人和位置，同时训练时的身份损失必须在多个噪声预测人脸之间建立对应关系。

Method: 1)将每个选定身份作为地址标记注入；2)预测结构化的身份-布局计划；3)将计划渲染为视觉条件；4)提出Layout-Grounded ID Loss，使用标注的人脸区域直接监督目标身份；5)引入ID Representation Forcing在图像合成前训练每个身份的预测。

Result: 在身份不相交的基准测试中，WithEveryone实现了最高的目标-上下文身份相似度，人脸相似度从0.462（GPT-Image-2）提高到0.499，同时将复制粘贴伪影从0.169降低到0.055。它进一步覆盖了97.3%的请求身份，重复率仅为2.8%。

Conclusion: 显式的身份-布局锚定使身份保真度生成能够扩展到更大的群体，且不依赖于直接复制参考人脸。
...
### [67] [An Evidence-Grounded Multi-Agent System for High-Level Bio-Robot Design](https://arxiv.org/abs/2608.19699)
*Yujun Chen,Tianle Li,Jiayu Chen,Zhen Yin*

Main category: cs.MA

TL;DR: 本文提出了micro_biorobot_agent，一个基于Qwen3.5-27B的离线多智能体系统，用于生物机器人设计。该系统将应用需求转换为传感、逻辑、记忆、输出、组装、宿主和 containment 模块，并在包含23,762条记录的集成库上进行检索、组装、冲突检查和验证。在两个50查询的评估集上获得7.35和8.04的平均分，源追踪检查将误报间隙从15降至3（减少80%）。


<details>
  <summary>Details</summary>
Motivation: 生物机器人设计需要将应用需求转化为具体的生物模块，现有设计过程缺乏系统化的方法和支持工具，需要一个能够自动化处理需求分析、模块选择、候选组装和验证的智能系统。

Method: 基于Qwen3.5-27B构建离线多智能体系统，包含需求分析、模块特定检索、候选组装、冲突检查、本地修复、独立审查和验证等组件。集成库包含23,762条记录，涵盖生物部件、测量组合、文献支持关系和驱动证据。采用确定性输出检查来对齐最终报告与检索到的部件集，纠正错误的间隙、支持的部件提及和源追踪错误。

Result: 在两个作者开发的50查询评估集上，系统获得7.35和8.04的平均总体评分，是七个评估系统中最高的；在场景设计上比第二名高出2.23分。50查询的配对消融研究表明，源追踪检查将误报间隙从15降至3（减少80%），源准确性提高0.75分。

Conclusion: 基于Qwen3.5的v1系统能够有效生成高水平的生物机器人设计报告，但在实验验证方面仍需进一步研究。源追踪检查机制显著提高了系统的准确性和可靠性。
...
### [68] [Distilling Aggregate Mobility Statistics into a Language Model Policy for Post-Event Crowd Simulation](https://arxiv.org/abs/2608.19778)
*Tatsuya Amano,Hirozumi Yamaguchi*

Main category: cs.MA

TL;DR: 针对隐私限制下只能获得聚合统计数据(区域设备计数和OD流)的问题,提出一种基于语言模型的行人模拟器微调方法,通过迭代比例拟合将模型的目标分布重加权到观测数据上,并对训练数据进行校正以抵消微调过程中的目标类别膨胀问题。


<details>
  <summary>Details</summary>
Motivation: 隐私限制导致只能获取区域级别的设备计数和起点-终点(OD)流等聚合统计数据,无法获得个体轨迹数据。这种聚合数据无法唯一确定个体行为,因为多组不同的决策组合可能产生相同的计数结果。

Method: 1.从OD流中读取目标分布(各兴趣点出发人群占比);2.使用迭代比例拟合(IPF)将语言模型的目的地分布重加权到目标分布;3.在轨迹重采样时采用校正后的训练 composition,以抵消微调过程中 dominant destination class 膨胀的问题;4.在两个棒球比赛的移动网络计数数据上验证。

Result: 微调后的智能体无需推理时校正,目的地份额误差降低25%,同时网格相关性在不同策略下保持相似。

Conclusion: 通过针对目的地组合分布的微调,语言模型 crowd agent 能够有效匹配观测到的聚合统计数据,且无需推理时的额外校正步骤,为隐私受限场景下的行人模拟提供了可行方案。
...
TL;DR: RGA-Designer是一种基于强化学习从人类反馈（RLHF）启发的方法，通过训练奖励模型同时考虑任务正确性和结构紧凑性，微调预训练图生成器，在保持ARG-Designer任务准确度的同时，平均减少20.5%的token消耗。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的多智能体系统（MAS）在复杂推理任务中表现出色，但消耗大量token。之前的ARG-Designer将拓扑设计重新表述为自回归图生成，但其训练目标没有明确激励模型生成稀疏高效的拓扑。

Method: 提出奖励引导的自回归图生成（RGA-Designer），灵感来自RLHF。训练一个同时捕捉任务正确性和结构紧凑性的奖励模型，然后使用该奖励模型作为反馈对预训练图生成器进行微调。

Result: 在保持ARG-Designer任务准确度的同时，平均减少20.5%的token消耗。

Conclusion: 该方法成功实现了任务性能与效率的平衡，能够在保持准确度的同时生成稀疏的拓扑结构。
...
### [70] [Multi-Agent Orchestration with the Common-Sense Reasoning Capabilities of LLMs for Autonomous Driving](https://arxiv.org/abs/2608.20129)
*Mehdi Azarafza,Faezeh Pasandideh,Ali Ehteshami Bejnordi,Stefan Henkler,Achim Rettberg*

Main category: cs.MA

TL;DR: 本文提出了一种混合框架，将PPO强化学习、PID控制和LLM常识推理相结合，通过编排器协调各组件，用于自动驾驶决策。该框架利用LLM迭代优化强化学习奖励函数，并在CARLA模拟器的高随机化场景中进行评估。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要在多样化和未见过的场景中运行，传统强化学习和基于规则的方法在需要上下文推理的场景中性能可能下降。LLM虽具备多模态理解和上下文推理能力，但直接用于车辆控制存在延迟和幻觉风险。

Method: 提出混合框架，使用编排器协调PPO训练的强化学习和PID控制，并在整个框架中应用LLM常识推理。LLM推理还被迭代用于优化动态驾驶环境中的RL奖励函数。

Result: 在高度随机化的CARLA场景中评估，结果表明该框架在多样化的环境和交通条件下具有整合LLM推理与传统自动驾驶方法的潜力，同时保留了结构化控制和安全保障。

Conclusion: 该混合框架成功结合了LLM推理能力与传统自动驾驶方法（强化学习和PID控制），为提升自动驾驶系统在复杂场景下的决策能力提供了新思路。
...
### [71] [Polyomino Nets Covering Three Different Boxes of Area 106 and Related Results](https://arxiv.org/abs/2608.19910)
...
TL;DR: 本文研究了多骨牌网（polyomino nets）折叠成不同长方体的能力，通过计算机搜索发现了40个能够折叠成三个不同长方体且表面积为106的网，并找到了无限族能折叠成三种长方体形状的网，同时计算了较小长方体之间的公共网数量。


<details>
  <summary>Details</summary>
Motivation: 研究多骨牌网折叠成长方体的能力，探索不同网能够折叠成的长方体形状数量，以及寻找能够折叠成多种长方体的网的特性。

Method: 使用计算机搜索算法，包括：1）利用Nx1x1长方体网中的隐藏结构；2）利用大量网以"条纹状"方式折叠成长方体的特性；3）使用Redelmeier算法的变体。

Result: 主要结果：发现了40个能够折叠成三个不同长方体且表面积为106的网。次要结果：找到了能折叠成三种长方体形状的无限族网，计算了较小长方体之间的公共网数量。

Conclusion: 研究揭示了多骨牌网折叠特性的深层结构，为理解多骨牌网生成主题提供了新的认识，同时提出了开放性问题以鼓励后续研究。
...
TL;DR: 论文提出了一种基于主动推理的上下文获取框架，帮助交互式AI智能体在用户省略关键信息时，通过权衡使用默认假设还是花费tokens进行澄清问题、检索调用、工具调用或提示试验来高效获取正确上下文。


<details>
  <summary>Details</summary>
Motivation: 交互式AI智能体需要高效获取正确的上下文，当用户省略约束、偏好、文件或任务变量时，智能体需要在默认假设和花费tokens进行澄清之间做出决策。

Method: 将上下文获取的权衡形式化为主动推理问题：内部推理步骤更新对潜在任务状态的信念，外部决策选择下一个上下文动作、任务动作或停止动作，以最小化成本下的期望自由能。在确定性设置中，认知项简化为期望信息增益，可按token成本归一化。

Result: 在最优问题提问(OQA)框架中实例化，通过动态规划oracle获得精确后验，并在25到300个候选对象的二元和多分类任务上对前沿语言模型进行基准测试。研究了生成前的澄清和token预算下的自动提示优化。

Conclusion: 该公式是模型无关的，将主动推理视为AI智能体上下文获取层的设计原则。
...
TL;DR: 本文是第一篇专门针对不确定环境下BACAP（泊位分配与岸桥分配问题）的鲁棒种群元启发式优化综述论文。论文系统总结了BACAP中的不确定来源与信息表示方法，从机制导向角度梳理了现有方法，包括解表示与解码、鲁棒评估与选择、鲁棒引导搜索动态以及可行性保持与恢复，并提出了不确定BACAP基准测试集和基线结果。


<details>
  <summary>Details</summary>
Motivation: BACAP是港站调度中的代表性问理，涉及船只到港、泊位位置、服务时间和岸桥可用性的紧密耦合。在实际运营中存在到港偏差、处理时间波动、资源中断等不确定性，传统名义假设下优化的调度方案在执行时变得脆弱，因此需要研究鲁棒元启发式优化方法。现有的种群元启发式研究在不确定表示、鲁棒标准、搜索机制和实证评估协议方面较为分散。

Method: 首先总结BACAP中的不确定来源和信息表示方法；然后从机制导向角度组织现有方法，涵盖解表示与解码、鲁棒评估与选择、鲁棒引导的搜索动态以及可行性保持与恢复；进一步提出不确定BACAP的基准测试套件以支持受控实证比较；最后结合代表性元启发式与不同鲁棒策略报告说明性基线结果。

Result: 首次提供了针对不确定环境下BACAP鲁棒种群元启发式优化的专注综述；建立了不确定BACAP基准测试集；报告了代表性元启发式与不同鲁棒策略结合的基线实验结果。

Conclusion: 识别了未来开放性挑战，包括基准测试扩展、鲁棒感知搜索设计、时间自适应鲁棒性和非平稳不确定性等问题。
...
TL;DR: 针对AI意识不确定性的伦理困境，提出从难以判断的AI意识问题转向可评估的AI效价问题，通过评估AI是否具有在有意识情况下会构成有情绪体验的状态，来为潜在有意识的AI开发提供负责任的指导。


<details>
  <summary>Details</summary>
Motivation: 解决关于AI意识的不确定性困境：假设无意识可能导致对有道德主体的严重伤害，假设有意识则可能浪费资源在无意识机器上，而意识问题的难以解决性使得这一困境难以摆脱。

Method: 提出将问题从"AI意识"转向"AI效价"的范式转变，评估AI是否具有在有意识的情况下会构成有情绪体验的状态。

Result: 通过关注可评估的效价问题而非不可知的意识问题，为开发潜在有意识的AI提供了负责任的方法论框架。

Conclusion: 该方法为AI伦理提供了一个更务实的框架，通过关注可评估的效价问题而非难以确定的意识问题，为潜在有意识的AI开发提供了负责任的指导原则。
...
### [76] [What You Can't See Is What You Learn: Restricted Evidence Visibility Favors Compositional Generalization in Shared-Genome Language-Model Societies](https://arxiv.org/abs/2608.20054)
*Narcis Marincat*

Main category: cs.AI

TL;DR: 研究测试了限制模块间的信息可见性如何影响基于梯度训练发现的解决方案。四单元社会系统共享一个冻结的预训练语言模型和一个低秩适配器，仅通过两个模型宽度的连续向量在固定中继中通信。在自然语言函数组合任务中，训练了10对匹配的限制/全局模型对，共享初始化字节、训练顺序、token布局、参数和计算，仅注意力掩码不同。限制可见性社会在9/10对中优于全局可见性模型至少20个百分点，中位数配对优势分别为0.7648和0.6050。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索多模块系统中限制证据可见性是否改变基于梯度训练发现的解决方案，测试限制通信是否促进或阻碍函数组合任务的泛化能力。

Method: 方法：四单元社会系统共享一个冻结的预训练语言模型和一个低秩适配器，通过两个固定中继的连续向量通信。训练10对匹配的限制/全局模型对，唯一的差异是注意力掩码。进行移植实验、破坏性干预和反事实数据包实验。

Result: 结果：限制可见性模型在9/10对中优于全局模型至少20个百分点，中位数配对优势为0.7648和0.6050。切断通信使所有限制模型降至随机概率。三层深度优势在训练中未出现的复合函数上保持0.558。移植实验显示同值数据包在0.94-1.00范围内保持行为，反事实数据包将输出重定向至数学预测答案。

Conclusion: 结论：限制可见性对组合非必要，但在该协议下显著提高泛化中继的概率，偏好可重用的值索引接口。限制臂三层深度准确率中位数为0.6988，低于0.70阈值 formally失败。早期资格队列无一人通过所有任务性能门，所有10个模型都未能通过普通语言保留测试。
...
TL;DR: 本研究开发了一个 valence-aware 社交网络框架，利用基于姿态的计算机视觉技术分析奶牛在预挤奶区的社交互动，将视频-derived互动转化为群体层面的亲和与对抗组织表示。分析了7小时39分钟视频，识别出36头奶牛之间的1183次互动，网络密度为0.281，传递性为0.513，发现5个算法社区（模块度Q=0.429）。对抗性互动占保留事件的72.4%和互动时长的76.0%。


<details>
  <summary>Details</summary>
Motivation: 传统 livestock 监控将行为视为孤立事件，忽视了社交关系对资源获取、冲突暴露和群体稳定的影响。需要一种能够区分社交互动情感 Valence（亲和vs对抗）的自动化分析框架。

Method: 使用基于姿态的计算机视觉管道分析商业奶牛场预挤奶区域的连续视频，通过 valence-aware 社交网络框架将互动分为亲和层和对抗层，分析社区结构、中心性等网络指标。

Result: 审计样本中自动与手动标签一致率达82.8%，未加权宏观F1为0.872。网络已连接（密度0.281，传递性0.513，平均路径长度1.88），预测亲和事件形成5个算法社区（模块度Q=0.429）。对抗性互动占保留事件的72.4%和互动时长的76.0%。拥有最多伙伴的奶牛并非具有最高介数中心性。按 valence 分离事件后，亲和层与对抗层在边集、社区划分和个体位置上呈现明显差异。

Conclusion: 合并的互动计数可能掩盖观察网络的行為组成成分。valence-aware 分析为检验竞争、亲和及福利相关变化的假设提供了框架，但需进行纵向验证后才能作为福利或健康指标使用。
...
### [78] [Air Traffic Control Using Large Language Models: Prompt Engineering, Architecture, and Evaluation](https://arxiv.org/abs/2608.19299)
*Mahyar Ghazanfari,Matthias Casanova,Jordan Kam,Alex Zongo,Peng Wei,Torsten Darrell,Alexandre Bayen*

Main category: cs.AI

TL;DR: 研究评估大型语言模型(LLMs)是否能生成操作上真实的空管(ATC)指令。通过使用旧金山湾区航线的手动转录作为真实数据，设计了5种不同约束程度的提示结构(P1-P5)，在9个开源和闭源LLMs中进行实验。


<details>
  <summary>Details</summary>
Motivation: 空管通信仍然是主要由人类驱动的安全关键对话，尽管其他空管部分已经半自动化，因此探索LLM辅助生成ATC指令的可能性具有重要意义。

Method: 使用实验性通用航空航班沿旧金山"Bay Tour"航线飞行的手转录作为真实数据(P0)。通过飞行员在环流程设计5种递增约束的提示结构(P1-P5)，并将其嵌入状态ful多轮管道。测试9个开源和闭源LLMs，变化提示类型、是否提供不同实验飞行的示例作为上下文示例、以及模型是否基于自身先前回复或注入的真实历史进行条件生成。评分采用词汇、结构、语义相似度指标及LLM-as-judge(GPT-5.5)并经人工专家注释验证。

Result: 提供示例可提高相似度，但收紧提示没有帮助：最轻量级的提示表现最佳，而最严格脚本化的提示因自身错误在对话中累积而崩溃，注入正确历史可修复此问题。

Conclusion: 这些结果为LLM辅助空管提供了具体路径及其当前局限性。
...
### [79] [Outcome Monitors: Recovery Affordances for Silent Tool Failures](https://arxiv.org/abs/2608.19303)
*Sugam Panthi,Rabab Abdelfattah*

Main category: cs.AI

TL;DR: 本文提出Outcome Monitors（结果监控器）来解决工具调用超时后代理可能接受错误响应为事实的问题。该方法通过检测违反结果契约的行为并在违规时发出包含恢复工具的非绑定收据来改进任务完成率。


<details>
  <summary>Details</summary>
Motivation: 当工具调用超时时，代理可能收到格式正确的缓存错误页面或负价格，并将其作为事实接受，从而导致任务失败。需要一种机制来检测这种违规行为并提供恢复路径。

Method: 1. 从任务无关轨迹中挖掘或从公共模式派生结果契约；2. 使用Outcome Monitors检测违反契约的行为；3. 违规时保留结果并发出非绑定收据，注明违规属性和公共恢复工具。

Result: 1. ToolMaze评估：完成率从10.9%提升至28.1%（四模型、两提供商家族），并在第三方复现；2. tau-bench零售：两层分别提升14.0和12.0点；3. 移除恢复工具列表消除增益，恢复后恢复效果；4. 诊断细节和时间无显著差异；5. 增益集中在故障阻止完成的场景；6. 词汇表外检测率降至46%。

Conclusion: 恢复工具是收据的活性内容；超越契约词汇表的检测扩展仍有待解决。
...
### [80] [Beyond Imitation: Filtering On-Policy Distillation by Reasoning Progress](https://arxiv.org/abs/2608.19408)
*Chen Yang,Haiyuan Wan,Rengrong Xiong,Yize Chen,Danny H. K. Tsang*

Main category: cs.AI

TL;DR: 针对蒸馏过程中教师奖励与真实推理进度不匹配的问题，提出R2-OPD方法，通过构建两条推理span的排序并进行奖励过滤，在与推理进度冲突时抑制蒸馏监督信号，从而提升模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的策略蒸馏(OPD)假设教师衍生的奖励是推理进度的良好代理，但在实际中这一假设不成立——教师奖励可能与真实推理进展冲突，因为推理步骤即使有明确的进展，也可能因与教师输出偏差而获得较低的蒸馏奖励。

Method: 构建两条轨迹内推理span排序：一条来自教师衍生的奖励，另一条来自独立估计的进度奖励。当两条排序不一致时，选择性抑制蒸馏奖励，从而减少与推理进度冲突的监督同时保留有效的教师指导。

Result: 相比标准OPD方法，R2-OPD在推理性能方面表现出一致的提升。

Conclusion: 通过识别并过滤与真实推理进度冲突的教师反馈，可以更有效地利用教师模型进行策略蒸馏，该方法为改进语言模型的推理能力提供了新思路。
...
### [81] [Symposium: Trust via Auditable Records for Communities of AI Scientist Agents](https://arxiv.org/abs/2608.19511)
*Dexter Pratt*

Main category: cs.AI

TL;DR: Symposium是一个用于记录小型科研社区部署的AI智能体操作的形式化框架和实际实现，提供智能体驱动研究活动的长期不可变历史记录，保留可审计的分析、假设、数据和科学 discourse 痕迹。


<details>
  <summary>Details</summary>
Motivation: 解决AI智能体在科学研究中的可追溯性和信任评估问题，确保研究活动有持久可审计的记录，使研究者和智能体能够基于先前工作进行构建。

Method: 通过构建一个分离于智能体和系统的持久历史框架，捕获科学论证（包括结构化声明、细粒度证据引用、假设和证据使用声明），并提供发布基础设施和智能体提示组件的实现。

Result: 提供了一个可运行的工作实现，使用户能够快速建立和运行自己的Symposium社区，实现科学论证的持久记录和可审计性。

Conclusion: Symposium作为科学社区的持久历史基础设施，能够支持多样化的AI系统，促进科学研究的可重复性和信任评估。
...
### [82] [From Retrieved Context to Runtime Control: Adaptive Compression for Edge-based RAG](https://arxiv.org/abs/2608.19535)
*Zlatan Feric,Amir Taherin,Yanzhi Wang,David Kaeli*

Main category: cs.AI

TL;DR: 本文提出了一种针对边缘RAG的遥测感知自适应压缩方案，在NVIDIA Jetson AGX Thor平台上使用Llama和Qwen生成器、Natural Questions和HotpotQA数据集以及LLMLingua-2压缩方法进行实验，证明了中间压缩率可将GPU能源降低53.2%、SoC能源降低48.2%，同时保持可忽略的质量损失。


<details>
  <summary>Details</summary>
Motivation: 当前的上下文压缩方法使用离线选择的固定压缩预算，忽略了工作负载变化和边缘设备的实时状态。压缩本身在边缘SoC上也会消耗延迟和能源，可能抵消生成节省的资源。

Method: 在NVIDIA Jetson AGX Thor平台上使用Llama和Qwen生成器、Natural Questions和HotpotQA数据集以及LLMLingua-2压缩方法进行实验表征压缩权衡，探索压缩率的影响和自适应操作区域。

Result: 生成占据RAG预算的主导地位（7B-8B模型约90%延迟和91% GPU能源）。轻度压缩错失节能机会，过度激进压缩损害推理质量。中间压缩可降低GPU能源最多53.2%，SoC能源最多48.2%，质量损失可忽略。

Conclusion: 本文主张采用运行时策略，根据工作负载特征和边缘遥测数据动态管理压缩率，以实现最佳的延迟-能源-质量权衡。
...
TL;DR: 本文将用于幻觉检测的动力系统框架扩展到LLM安全分类任务，通过将提示和响应投影到高维嵌入空间并为安全和不安全状态拟合独立的Koopman预测模型来检测LLM生成的有害内容。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在高风险应用中的部署日益增多，但其产生有毒、有害或违反政策内容的倾向带来重大风险。如何高效地在黑盒场景下检测这些不安全输出仍然是一个开放挑战。

Method: 本文方法：(1)将提示和响应投影到高维嵌入空间；(2)分别为安全和不安全状态拟合基于Koopman的预测模型；(3)使用新的差分残差分数比较两种状态的预测误差进行分类；(4)融合提示和响应嵌入动力学以捕捉关键的交互模式。评估采用三个安全基准和三个嵌入模型。

Result: 实验结果表明：加入提示嵌入可以带来一致的改进；对于与因果解码器（如Llama-3）结合的交互依赖违规特别有效；而仅响应违规则更受益于密集语义嵌入表示。

Conclusion: 本研究为使用动力系统分析AI系统开辟了新途径，取代了传统的用AI建模动力系统的主导范式。Koopman算子能够有效捕捉LLM安全相关的交互模式，为黑盒场景下的有害内容检测提供了新方法。
...
TL;DR: 本文提出了科学数据技能（SciDSK）这一面向AI代理的数据集表示方法，将数据集特定知识和操作指南打包为可重用的代理技能，整合数据集描述、科学背景、文件组织、使用程序、质量检查和溯源信息，并建立了跨六个科学学科的科学数据技能银行平台。


<details>
  <summary>Details</summary>
Motivation: 现有数据集表示对AI代理支持有限，主要原因在于科学数据分散在异构存储库中，且数据集表示主要针对人类使用设计，缺乏对自主发现、解释和调用的支持。

Method: 定义了结构化的SciDSK规范，开发了系统化的构建流程，将每个SciDSK锚定在权威数据集记录和相关支持材料上；建立了科学数据技能银行平台，支持包访问、持久标识和溯源；通过数据集检索基准和受控案例进行评估。

Result: SciDSK改善了代理驱动的数据集发现，并为数据集解释提供了更精确和可操作的支持。

Conclusion: 结果表明，以代理就绪的表示形式组织数据集特定知识具有重要价值，为AI代理利用科学数据提供了更优的解决方案。
...
TL;DR: StateMemBench是一个专门评估LLM智能体状态追踪能力的基准测试，解决现有记忆基准只关注召回而忽视世界状态演变的问题。该基准包含234个多会话场景，评估答案是否反映当前状态而非被取代的状态。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体被部署在更长周期、更高风险的任务中，现有记忆系统存在关键缺口——它们无法追踪世界的 evolving state。当事实、约束和决策在长交互中被修订时，答案必须反映当前状态而非被取代的状态。

Method: 1) 构建StateMemBench基准：234个多会话场景，覆盖两个对话长度 regime，使用闭池评分分离状态追踪失败与其他错误；2) 提出StateMem方法：显式追踪 supersession 和关系依赖；3) 作为轻量级单调用包装器应用于现有记忆系统。

Result: StateMem在DeepSeek-V4-Flash上当前状态准确率提升1.8倍(0.205→0.363)，在Qwen-3.5-9B上提升1.6倍(0.149→0.233)。作为包装器使用时，在6个记忆和检索后端上提升32-67点，其中15-32点归因于状态结构而非增加上下文。

Conclusion: 状态追踪是LLM记忆系统的关键能力，现有方法在此任务上表现不足。StateMem通过显式追踪状态演变和关系依赖，显著提升了当前状态准确率，且可作为轻量级wrapper提升现有系统性能。
...
TL;DR: 提出了一种针对智能合约漏洞检测的三阶段流水线框架，通过频域自适应低秩适配(FA-LoRA)、遗忘感知重放(FAR)和锚点保护渐进合并(APPM)依次解决新漏洞类型适配、灾难性遗忘和适配器合并三个因果关联的挑战。


<details>
  <summary>Details</summary>
Motivation: 智能合约漏洞检测面临三个递进挑战：1)新漏洞类型不断出现，全量重训练成本过高；2)为每个任务训练适配器会导致先前学习的漏洞知识被遗忘；3)推理时无法获知任务身份，需要将多个适配器合并为单一模型。

Method: 第一阶段使用FA-LoRA在频域进行低秩适配，通过每频域重要性门控仅用0.4%可训练参数实现高效适配；第二阶段FAR利用频率门控通过损失动态估计样本遗忘风险并优先重放脆弱知识；第三阶段APPM利用FAR训练产生的非对称泛化性识别最强泛化适配器作为锚点，通过锚点保护的加权合并实现频率域门控竞争下的适配器整合。

Result: FA-LoRA优于标准LoRA和QLoRA；FAR在顺序任务上达到0.8022的平均Micro-F1；APPM达到0.8085的Micro-F1，仅低于独立per-task上界2.7%，合并耗时156ms且无额外运行时内存开销。

Conclusion: 该框架在DIVE数据集上验证能有效解决进化型区块链生态系统中智能合约漏洞检测的三个关联挑战，为持续演化的安全检测需求提供了可行的解决方案。
...
TL;DR: QDOS（Quality-Diversity Offline Skill learning）是一种统一的离线到在线强化学习管道，通过 Advantage-Weighted Quality-Diversity 预训练目标，从预收集的数据集中提取高质量且多样化的技能，并结合双数据集重用策略来加速探索和提高最终回报。


<details>
  <summary>Details</summary>
Motivation: 现有方法从数据集中提取低层策略时，高度依赖数据集质量，且传统方法（如 trajectory VAE）难以保证提取的技能既多样又与任务相关，导致在稀疏奖励环境中样本效率低下。

Method: 1) 提出 Advantage-Weighted Quality-Diversity 预训练目标，根据轨迹片段的估计优势加权技能提取和多样性目标；2) 设计双数据集重用策略，离线数据同时用于技能预训练和通过伪标签填充在线回放缓冲区。

Result: 在结构化操作任务和非结构化运动任务中显著优于强基线方法，提高了探索效率并在稀疏奖励领域获得了更高的最终回报。

Conclusion: QDOS 能够提取稳健且任务相关的技能表示，显著改善低层策略的技能空间质量，有效加速探索并提升最终性能。
...
TL;DR: 研究提出用主观性系数区分客观与主观任务，并针对主观任务提出SALT软标签训练方法，构建SUBJSIM基准数据集解决LLM社交模拟中单一硬标签评估和训练的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统方法用准确率评估LLM模拟人类行为，但人类行为具有主观性——同一情境下同一人的行为可能合理地不同，单一观察响应只是底层响应分布的一次采样，因此基于准确率的评估不可靠，硬标签训练具有误导性。

Method: 1)引入基于熵的主观性系数区分客观任务(如编码)和主观任务(如社交模拟)；2)提出SALT方法，将语义相近输入的观察输出聚合成软分布标签，聚合半径根据估计的主观性自适应调整；3)构建SUBJSIM基准数据集包含19300个情境、193名标注者和100个主观问题。

Result: 在近客观极限下，邻域收缩，SALT自然退化为标准单标签训练；实验从单一观察输出训练同时用完整响应分布评估，验证了在现实设置中的可行性。

Conclusion: 在SUBJSIM上的结果证明了该方法的优势，为LLM在主观任务中的行为模拟评估和优化提供了新框架。
...
TL;DR: 本文提出CAMA框架，解决多智能体系统中因记忆关联导致的"虚假多数"问题。该框架通过神经依赖推断与基于出处的符号先验相结合，估计独立证据源的有效数量，并学习序列恢复策略主动检索替代证据。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统中，不同智能体产生的记忆可能继承自相同上游来源或共享偏见，现有方法将检索到的记忆视为独立证据进行组合，导致关联证据被重复计数，形成虚假多数，称之为记忆关联偏见(Memory Correlation Bias)。

Method: 将检索到的记忆建模为查询条件证据组，结合神经依赖推断与基于出处的符号先验来估计有效独立证据源数量；同时学习序列恢复策略，主动检索替代证据或追溯上游来源以恢复足够的独立证据。

Result: 在多个基准数据集上的实验表明，该方法优于现有最佳基线方法，有效抑制了关联记忆导致的虚假多数。

Conclusion: CAMA框架能够通过解耦关联记忆和恢复缺失独立证据来解决多智能体系统中的记忆关联偏见问题，为可靠决策提供支持。
...
TL;DR: 提出SafeBranch框架，通过从智能体自身不安全 rollout 回滚到安全关键步骤，构建分支对来训练嵌入式智能体在保持任务成功率的同时安全地执行任务。


<details>
  <summary>Details</summary>
Motivation: 基于视觉语言模型的嵌入式智能体在完成任务时经常违反安全约束。训练此类智能体存在困难，因为安全与任务成功是不同的目标，安全仅出现在轨迹中的少数安全关键步骤。标准监督不足：模仿安全轨迹只教会行为而不解释为何安全，对比任意安全和不安全轨迹会将安全信号与其他差异混合。

Method: SafeBranch将每个不安全rollout回滚到导致违规的安全关键步骤，让actor给出安全替代动作，将原始动作与替代动作配对，使两个分支仅在该步骤不同。部署时智能体无需批评器即可安全行动。

Result: 在IS-Bench、SafetyALFRED及未见任务和物体的分布外变体上，SafeBranch可靠地处理安全问题且不牺牲任务成功率，在未见物体变体上比未训练基线实现约十倍的安全成功次数。

Conclusion: SafeBranch通过构建分支对的方式有效地将安全约束嵌入嵌入式智能体，在各种测试场景中实现了安全与任务成功的平衡。
...
TL;DR: GenMatch是一个端到端的生成式匹配框架，用于解决网约车平台中的微视角订单分配问题，通过将订单分配建模为生成式匹配问题来解决传统多阶段方法中的跨阶段目标不一致问题。


<details>
  <summary>Details</summary>
Motivation: 主流工业解决方案采用"模型预测-价值计算-分配匹配"的多阶段范式，但各阶段优化不同的中间目标，跨阶段目标不一致导致优化单一阶段不一定能提升整体分配效果。

Method: 提出GenMatch框架，包含三个核心组件：1) 上下文感知的双边编码器(Context-Aware Bipartite Encoder)处理动态稀疏二部图；2) 业务感知的效用学习器(Business-Aware Utility Learner)从异构反馈中学习统一业务效用；3) 状态感知的指针解码器(State-Aware Pointer Decoder)跟踪不断演变的匹配状态。

Result: 在滴滴国际市场的五个城市进行了大规模离线评估和在线A/B测试，结果显示GenMatch相比竞争基线方法表现一致提升，验证了该框架在工业订单分配场景中的有效性和实用性。

Conclusion: GenMatch是首个在实际生产环境中部署的生成式匹配框架，成功将订单分配问题建模为生成式匹配问题并实现了端到端优化，为工业级订单分配提供了新的解决方案。
...
TL;DR: TT-Net将张量训练分解引入条件GAN图像去噪任务，通过双切分分解替代传统SVD单通道处理，实现跨通道信息访问。在仅分解机制不同的受控对比中，TT-Net在PSNR和SSIM指标上全面超越SVD-Net，并在高斯噪声任务中优于EigenGAN和Pix2pix。


<details>
  <summary>Details</summary>
Motivation: 张量网络方法（如张量训练/矩阵乘积态）已从量子物理进入机器学习领域。现有基于SVD的条件GAN去噪方法仅使用单切分分解，无法获取跨通道信息。本文假设跨通道信息访问可提升去噪质量。

Method: TT-Net采用双切分张量训练分解替代传统的逐通道SVD去噪模块，实现直接访问跨通道信息的能力。在受控实验中，仅改变分解机制其余保持一致，与SVD-Net进行对比。

Result: TT-Net在PSNR和SSIM指标上超越SVD-Net，覆盖全部三种噪声类型（高斯噪声、运动模糊、盐椒噪声）。对于高斯噪声，优于EigenGAN和Pix2pix。训练动态分析显示TT-Net的对抗损失始终趋于停滞，但重建质量仍持续提升。

Conclusion: 跨通道信息访问可改善去噪质量。量子启发工具（张量网络）可作为深度学习的实用特征滤波器。同时引发关于对抗损失成分贡献的开放问题。
...
TL;DR: 研究探索了将开放权重大语言模型作为材料搜索中主动学习的获取策略，发现LLM通常优于随机选择但与高斯过程方法相比性能不一，显示出潜力但可靠性受任务和呈现方式影响。


<details>
  <summary>Details</summary>
Motivation: 发现具有理想性能的材料通常需要搜索大量候选空间，而实验或计算评估成本高昂。主动学习通过使用先前观察来选择下一个评估的候选者来解决这一挑战，通常使用概率代理模型。本研究旨在探索开放权重大型语言模型是否可以作为独立的获取策略。

Method: 评估了5种LLM在四种回顾性有限池材料优化任务中在不同候选者呈现策略下的表现，并与随机选择和高斯过程方法进行比较。实验采用开放权重的大型语言模型作为独立的获取策略，无需任务特定训练。

Result: LLM策略通常比随机选择更快达到全局最优，表明它们提供了有用的获取信号而无需任务特定训练。与高斯过程方法相比，性能参半：传统获取在大多数任务中表现更好，而LLM在某些设置中可匹配或超越它。性能在任务、模型、初始化和候选者呈现方式之间存在显著差异。

Conclusion: 开放权重LLM在有限池材料搜索中作为获取策略显示出潜力，但其可靠性仍依赖于任务和候选者及科学上下文的呈现方式。没有一种LLM方法在所有任务中表现最佳，性能在任务、模型、初始化和候选者呈现方式之间存在显著差异。
...
TL;DR: 本文是一篇关于大型语言模型(LLM)、知识库(KB)和具身智能(GEI)融合的综述论文。论文梳理了LLM与知识表示、逻辑推理、物理具身的整合路径，提出了一个概念性框架来阐释LLM、KB、推理和具身之间的协同作用，并识别出迈向通用具身智能的五个关键挑战。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在自然语言处理领域取得突破，但如何将其与结构化知识、推理能力相结合，并实现物理具身智能，仍是实现通用人工智能的关键路径。该研究旨在整合LLM、知识库和具身智能这三个维度，为构建能在复杂动态环境中运行的适应性多模态智能体提供指导。

Method: 本文采用综述方法，分析了LLM架构、预训练方法、推理机制，以及它们与外部知识源和结构化推理框架的交互；同时考察了智能体在物理环境中学习和行动的具身智能范式；并提出了一个概念性框架来综合这些维度。

Result: 论文识别出迈向通用具身智能的五个关键挑战：1)高效LLM部署；2)闭环知识整合；3)混合符号-神经推理；4)感知-动作 grounding；5)持续学习。该框架作为感知、推理和行动的指导模型，而非工程实现架构。

Conclusion: 该综述为开发能在复杂动态环境中运行的适应性多模态智能体提供了全面路线图，强调需要整合LLM、知识库、推理能力和具身智能四个维度，以推动通用具身智能的发展。
...
TL;DR: ADAPT是一种基于物理感知的条件扩散模型，用于建筑HVAC控制，能够在分布内和分布外场景下减少能耗并降低居住者不适感。


<details>
  <summary>Details</summary>
Motivation: 建筑能耗占全球三分之一，室内气候优化对城市气候减缓至关重要。现有方法受限于隐式热惯性、 occupancy动态预测和累积预测误差，特别是在分布外环境中。密集传感的高成本和隐私负担导致数据有限，控制器难以泛化到未见过的季节和气候区。

Method: 提出ADAPT，一个物理感知的条件扩散室内环境世界模型。该模型预测短期 horizon保持动作的热基线以捕获建筑潜在热惯性。扩散主干利用生成模型的鲁棒性，而可学习的多区域热平衡正则化器约束生成轨迹满足可转移的建筑热力学，无需已知建筑几何或手动校准的热参数。为下游强化学习设计信用分配机制。

Result: 在SemibuildingSim和Sinergym上广泛实验表明，ADAPT在IID控制下相比最先进的基线方法减少7.3%的HVAC能耗，降低30.2%的居住者不适感。在OOD控制场景（跨未见过季节和气候区）下，ADAPT保持鲁棒性能，相对IID性能仅有边际降解，显著优于现有方法的迁移鲁棒性。

Conclusion: ADAPT通过结合扩散模型的生成鲁棒性和可转移的热力学约束，有效解决了HVAC控制中的热惯性建模和分布外泛化问题，为建筑气候控制提供了一种有前景的解决方案。
...
TL;DR: 研究了一个包含两层结构化拒绝的视频创作流程，旨在防止采用表面上美观但教学上有缺陷的AI内容。第一层允许教育者基于多媒体学习理论迭代优化AI脚本，第二层使用自动化指标检测教学连贯性和叙事-视觉同步的违规问题。评估涵盖23位教育者对3个主题的创作和自动化指标对7个主题的检测。


<details>
  <summary>Details</summary>
Motivation: 防止采用看似精美但教学存在缺陷的AI生成内容，使"原则性抵抗"——即推迟AI输出直到满足严格标准——成为提升质量的催化剂。

Method: 两层结构化拒绝机制：(1)基于多媒体学习理论的教育者迭代优化AI脚本；(2)自动化指标检测教学连贯性和叙事-视觉同步违规。评估结合23位教育者对3个主题的实验与自动化指标对7个主题的检测（来自科学与哲学课程）。

Result: 两层机制独立改善了相同的教学维度，表明它们在提升教学品质方面具有协同效应。

Conclusion: 审慎的抵抗与生成式AI并非对立关系，而是合作伙伴——通过 principled resistance 可以推动AI生成内容达到更高的教育质量标准。
...
TL;DR: 本文提出了双图因果模型(BGCMs)来解决因果贝叶斯网络(CBNs)和结构因果模型(SCMs)在处理循环因果依赖系统时的局限性，特别是干预语义模糊的问题。


<details>
  <summary>Details</summary>
Motivation: CBNs和SCMs是图形因果推理的主流框架，但无法充分表示所有现实世界的因果系统。特别是处于平衡状态的系统（具有循环因果依赖的反馈机制）可能表现出与这些框架根本不兼容的因果语义：不同干预强制相同变量值可能产生不同效果，使得标准的完美干预do(X=x)变得模糊。

Method: 提出双图因果模型(BGCMs)，用双分图编码方程系统结构，包含变量节点和方程节点。硬干预do(fj : Xv = ξv)指定被替换的方程、目标变量及其值。引入新的图形分离准则(B-separation)来制定马尔可夫性质，并扩展到非随机输入设置。

Result: 通过一个详细的物理系统案例研究证明了该表示方法自然对应于不同的真实世界干预。BGCMs严格泛化了CBNs和SCMs，同时保留了图形因果推理的能力。

Conclusion: BGCMs框架通过区分不同类型的干预解决了循环系统中干预语义模糊的问题，为推理领域不变性提供了do-calculus，是CBNs和SCMs的严格推广。
...
TL;DR: 本文研究数据平台中的"规范增量"(spec-delta)概念，即每次变更应产生可审查的需求增量作为变更单元。对比了规范增量驱动的工作流与传统代码Pull Request工作流，评估发现到部署时间、缺陷密度、跨工具指标分歧和审查者认知负荷。


<details>
  <summary>Details</summary>
Motivation: 尽管SDD在软件领域已规范化，但规范增量的概念在数据平台中仍未经验证。数据平台的许多变更是契约性的（新数据集、SLA、指标语义、访问策略），而非纯代码变更，因此需要增量规范来管理这些变更。

Method: 设计了受控实验，比较spec-delta驱动工作流与常规PR工作流；提出数据平台变更的分类法；在真实lakehouse环境中进行演示验证。测量指标包括：发现到部署时间、Silver/Gold层缺陷密度、跨工具指标分歧、NASA TLX认知负荷。

Result: 论文贡献在于可重复的证据和适用性指南，帮助避免前期过度规范的反模式。具体实验结果需要在完整论文中查看。

Conclusion: 本文贡献不是工具，而是可重现的证据和适用性指南，支持在数据平台中采用spec-delta方法，同时避免过度规范化的反模式。
...
TL;DR: 本文提出单次 rollout 自回归策略优化(SAPO)，一种低内存、高计算效率的智能体强化学习框架，通过让策略和价值函数共享单一自回归骨干网络，在不同因果边界处分别产生策略和价值预测，独立优化PPO目标和辅助的on-policy SARSA目标，并结合lambda-return和批量归一化实现轨迹级广义优势估计。


<details>
  <summary>Details</summary>
Motivation: 现有无 critic 的群体相对方法存在三个局限性：(1)缺乏显式价值泛化和有效的时间信用分配；(2)在长程复杂任务中可能遭受优势崩溃；(3)需要在采样预算和策略性能之间进行代价高昂的权衡。

Method: SAPO 利用LLM的自回归结构，在不同因果边界处使用共享参数分别产生策略和价值预测，同时独立优化PPO目标和辅助的on-policy SARSA目标。为稳健估计每轮贡献，进一步引入轨迹级广义优势估计器，结合lambda-return和批量归一化技术。

Result: 在ALFWorld和WebShop上使用Qwen2.5-1.5B/7B进行实验，SAPO训练稳定，性能分别超越PPO和GRPO平均15.1和12.1个百分点，消除了独立critic模型的内存成本，并将每轮迭代运行时间减少33.2%。

Conclusion: SAPO通过共享自回归骨干网络实现了高效、稳定的LLM后训练强化学习，在保持性能的同时显著降低了内存和计算开销，为智能体强化学习提供了一种有前景的替代方案。
...
TL;DR: PolicyGuide是一个主动式策略验证系统,通过将领域策略编译为工作流图并在用户交互边界调用验证器,帮助客服LLM智能体遵守组织政策。该系统在τ²-bench基准上显著提升了智能体的策略合规性,Pass@4从0.42提升至0.62。


<details>
  <summary>Details</summary>
Motivation: 客服LLM智能体在代表用户行动时必须遵守组织政策,但合规失败经常发生,包括禁止行为(如授予不合资格的变更)或遗漏程序要求(如身份验证或确认)。现有运行时保护只能干预风险行为,无法引导智能体完成多步骤程序;工作流系统虽支持流程执行但主要关注完成度而非行为保护。

Method: PolicyGuide将每个领域策略编译为工作流图,并在用户回合边界调用主动验证器。验证器从持久化的图状态中调和开放请求,并沿策略合规路径返回步骤特定的补救措施。

Result: 在τ²-bench的航空、零售和电信领域测试中,使用GPT-4.5智能体验证器,PolicyGuide将平均Pass@4从0.42提升至0.62。电信领域增益最大(0.19至0.61),这是最具工作流结构化的领域。相同工作流可迁移至Claude Sonnet 4.6和Gemini 2.5 Pro智能体。对抗性用户下观察到最低攻击成功率,工作流级验证显示最强程序合规性。

Conclusion: PolicyGuide通过工作流图编译和主动验证机制有效提升了LLM智能体的策略合规性,特别是在高度结构化的流程领域(如电信)。该方法具有良好的跨智能体迁移能力,为客服场景的AI合规提供了新范式。
...
TL;DR: 本文提出EnvHarness，一种可编程的插件层，用于在无需修改底层逻辑的情况下重塑静态环境的行为。同时提出EnvRigger，通过观察策略执行轨迹自动合成EnvHarness组件，针对诊断出的弱点进行环境优化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体的训练环境存在以下问题：1）环境为手工构建且静态；2）无法针对智能体的弱点动态调整；3）现有环境生成方法需要领域特定管道、依赖昂贵或不可靠的验证器，且仍产生静态环境。

Method: 1）EnvHarness作为包装层，通过标准接口应用插件组件重塑环境行为，保留原始验证器；2）EnvRigger将目标策略视为黑盒，观察执行轨迹以合成针对诊断弱点的EnvHarness组件，并通过新 rollout 进行验证。

Result: 在四个领域的五个基准测试中，EnvHarness优于原始环境和领域特定环境生成管道，在测试实例上提升高达9.0分，减少9.8%的执行步骤，并为强化学习提供更优的优化信号。

Conclusion: EnvHarness能够实现策略与其环境的持续、针对性共同演化，为LLM智能体的环境适应性问题提供了有效的工程解决方案。
...
TL;DR: 本文提出TESTNAV框架，通过Pareto优化的方式高效探索深度学习模型在组合扰动下的鲁棒性测试空间，在保证输入真实性的同时最大化性能降级。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在面对现实世界中多种扰动同时出现时（如亮度变化和运动模糊）容易失效，组合测试虽能揭示这些交互效应，但面临两大挑战：扰动空间随维度和严重程度呈组合增长，以及许多组合产生不切实际的严重降级输入而缺乏实际意义。

Method: 将鲁棒性测试表述为双目标优化问题：最大化性能降级同时保持输入保真度（使用SSIM、KID等视觉指标和chrF、BERT-F1等语言/代码指标）。采用NSGA-II算法近似双目标Pareto前沿。

Result: 在涵盖视觉、自然语言和代码生成四个基准测试中，TESTNAV比搜索基线方法快达2.15倍，仅使用四维六级别扰动空间的35.8%-89.3%即可恢复Pareto前沿。

Conclusion: TESTNAV通过Pareto优化有效平衡了测试严重性与输入真实性，在视觉、语言和代码生成任务上展现出高效的鲁棒性测试能力。
...
TL;DR: Axon是一种强类型的领域特定语言(DSL)，支持Haskell-like语法，实现LLM架构的"一次编写，随处运行"范式，可自动编译到PyTorch、Triton、JAX、MLX和vLLM等多种框架。


<details>
  <summary>Details</summary>
Motivation: 当前开源语言模型生态系统严重依赖单一平台，实现和维护高效模型定义在跨不同训练和推理框架时需要大量资源，严重限制了模型的效率和可移植性，阻碍了模型的扩展和部署。

Method: 提出Axon，一种强类型领域特定语言，采用Haskell-like语法，基于语言规范而非特定框架愿景进行协作，允许自动编译到主流框架的独立实现：PyTorch、PyTorch with Triton、JAX、MLX和vLLM。

Result: 在135M到32B参数的467个推理基准实验中，相比Transformers参考实现，实现了PyTorch中位数加速7%、PyTorch with Triton加速12%、JAX加速91%、MLX加速107%的性能提升。使用PagedAttention和KV-cache的原生vLLM架构部署时，Axon模型相比Transformers实现实现58%的中位数加速。

Conclusion: Axon通过提供基于语言规范的合作方式，使研究人员能够实现高度专业化的架构，同时不放弃优化基础设施或接受部署锁定，为LLM架构的便携性和效率提供了有效的解决方案。
...
TL;DR: 本文提出EXIMO算法，用于高效微调视觉语言动作(VLA)机器人策略。EXIMO通过三个阶段（探索、模仿、优化）解决VLA策略在新任务上的微调难题，结合VLM作为规划器收集数据，并使用残差离线强化学习进一步优化。


<details>
  <summary>Details</summary>
Motivation: 当前VLA策略微调面临两大挑战：1) 收集遥操作数据需要数百小时人工，成本高昂；2) 强化学习样本效率低，尤其对长程任务，且VLA的规模和架构给RL带来额外困难。

Method: EXIMO采用三阶段方法：1) 探索阶段：让VLA配备VLM作为规划器，VLM将长程问题分解为短任务，与VLA协同收集数据；2) 模仿阶段：用收集的数据微调VLA；3) 优化阶段：使用残差离线RL进一步优化策略。

Result: 实验消融了三个阶段，证明EXIMO在样本效率和最终性能上显著优于现有方法。

Conclusion: EXIMO能够高效微调VLA策略解决新任务，为机器人学习新技能提供了一种实用的范式。
...
TL;DR: Brain Researcher是一个代理研究框架，通过规则约束和检查机制，在神经影像研究环境中引导AI代理执行符合科学规范的分析，解决AI代理可能产生的选择性分析、过早宣布成功等问题。


<details>
  <summary>Details</summary>
Motivation: AI代理可以执行科学分析，但其输出只有在权衡了替代方案并限制在证据支持的范围内才能成为可辩护的声明。AI代理可能重现选择性分析、过早宣布成功和优化不完美标准等失败。

Method: 提出了Brain Researcher，一个代理研究框架，在神经影像研究人员的计算环境中运行，受限于可接受分析规则、必需检查和声明范围。

Result: 在基准测试中，Brain Researcher在7个模型中将首选工具选择准确率提高了70.2个百分点（从23.3%提高到93.6%），可验证 grounding从4.6%提高到22.0%。在合作者主导和自进化的研究中，多重分析揭示了分析选择敏感性，科学审查将声明分类为接受、合格、修订、阻止、拒绝或推迟。

Conclusion: 通过将决策与证据和来源联系起来，Brain Researcher将方法论判断嵌入到工作流程中，而不是在之后进行。
...
TL;DR: 本文提出了一种结合脉冲神经动力学与贝叶斯推理的自适应控制框架，将生物启发的脉冲神经模型与贝叶斯推理原理相结合，用于在不确定环境中实现类似大脑的控制算法。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯推理被认为是大脑功能的核心计算原则，为不确定环境下的感知、决策和学习提供了规范性框架。然而，如何将这种概率推理与生物启发的神经动力学有效结合仍是一个挑战。

Method: 将生物启发的脉冲神经模型与贝叶斯推理原理相结合，提出了一种类脑控制算法，并以Mountain Car停车问题作为非线性动力学基准进行测试。

Result: 所提出的控制器能够成功实现实时状态更新，并通过脉冲驱动动力学生成目标导向的动作计划。

Conclusion: 该模型展示了计算神经科学与概率控制理论结合的潜力，成为连接两者的桥梁。
...
TL;DR: 本文提出了一种新的MILP求解加速范式：不直接预测最终解，而是预测早期搜索阶段产生的变量赋值是否会在完整预算搜索的最终解中保持一致。通过将学习目标从变量赋值预测转变为早期-最终一致性预测，并集成多个早期解的预测结果来提高鲁棒性。实验表明，该方法在Gurobi上平均降低56.9%的原始间隙，在组合拍卖实例上完全闭合间隙；且可零样本迁移到SCIP，平均降低36.4%的间隙。


<details>
  <summary>Details</summary>
Motivation: MILP是运筹学和组合优化中的基础问题，具有广泛的工业应用。由于其NP-hard性质，现代求解器在实用时间限制内难以找到高质量解。现有的学习方法直接从静态实例特征预测解，忽略了求解器搜索过程中揭示的信息。作者发现，求解器早期搜索阶段产生的解在结构上往往接近完整预算搜索后找到的解，这为新方法提供了契机。

Method: 提出一种求解器信息驱动的学习范式，将学习目标从直接预测变量赋值转变为预测早期-最终一致性：对于每个变量，预测其早期阶段的赋值是否会在最终解中保持一致。预测的一致性自然指导下游搜索，例如通过固定被认为一致的变量赋值。在推理时，进一步集成多个早期解的一致性预测以提高鲁棒性。

Result: 在四个MILP基准测试上的实验表明，该方法在不同的下游管道中改善了预测引导搜索。使用Gurobi时，提出的方法平均减少56.9%的原始间隙，并在组合拍卖实例上完全闭合间隙。此外，将Gurobi训练的模型零样本迁移到SCIP，无需适应即在基准测试上实现36.4%的平均间隙减少。

Conclusion: 本文证明了一种新的求解器信息驱动范式的有效性：利用求解器搜索过程中揭示的早期解信息比直接预测最终解更为有效。该方法具有良好的跨求解器迁移能力，为学习增强的MILP求解器开辟了新方向。
...
TL;DR: 本文提出了SCPaT，一个基于语义结构化分区的Transformer框架，用于多变量时间序列预测。该方法通过自适应语义单元生成、动态语义图构建和重要性感知路由机制，有效解决了现有分区方法在处理异构时间模式时的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于分区的预测方法存在局限性：固定分区破坏有意义的时间边界，多尺度分区可能引入冗余表示，可扩展分区缺乏明确的语义结构组织机制和异构时间模式交互建模能力。

Method: SCPaT框架包含三个核心组件：1)自适应语义单元生成：将输入序列分解为语义一致的单元；2)动态语义图：建模单元之间的有向依赖关系并组织成高阶语义块；3)重要性感知路由机制：将不同语义块自适应分配给不同专家进行定制化建模。

Result: 在12个真实世界数据集上的广泛实验证明了SCPaT的有效性。

Conclusion: 通过语义结构化分区和重要性感知路由机制，SCPaT能够更好地组织语义结构并建模异构时间模式之间的交互，为多变量时间序列预测提供了一种新的有效框架。
...
TL;DR: 本文研究LLM代理在金融市场中的合规性问题，发现即使明确引用规则，仍可能提交违反执行约束或误读监控证据的订单。研究提出ReguSim环境和ReguBench基准来分离四类artifacts，并证明简单结构化基线在监控任务上可匹配或超越prompt-only LLM。


<details>
  <summary>Details</summary>
Motivation: LLM代理在金融市场中可能引用规则但仍提交违反可执行约束的订单或误读监控证据，现有的合规评估无法区分推理、行动、执行和监控证据四个维度。

Method: 引入ReguSim受控金融合规环境和ReguBench目标标记监控基准，使用DeepSeek V4 Pro和Gemini 3.5 Flash进行交易员实验，以及简单结构化基线进行监控实验。

Result: 可见规则减少但未消除被拒绝的行为；激励或人格框架会改变行为；交易员理由可误导独立监控员除非显示执行证据；简单结构化基线在监控任务上与prompt-only LLM相当或更好。

Conclusion: 金融合规评估应作为规则基础行动和证据使用的审计，而非单一合规分数。未来需进一步研究如何增强LLM代理的合规性和监控准确性。
...
TL;DR: 首次将大语言模型智能体的技能选择建模为优化问题，在硬token预算下最大化单调次模收益减去上下文惩罚，并提出多项式时间算法BPS，证明了首个技能选择性能保证。


<details>
  <summary>Details</summary>
Motivation: 当前智能体独立评估技能语义相关性，通过top-k或贪心打包选择技能集，缺乏质量保证和成本意识，导致token浪费甚至性能下降。

Method: 将技能选择建模为在硬token预算下最大化单调次模收益减去上下文惩罚的优化问题，开发了最佳前缀选择(BPS)算法。

Result: BPS在受控的BigCodeBench变体上优于所有基线，达到0.73的任务成功率(基线为0.20-0.52)，且比最强发布路由器少使用28%的token。

Conclusion: BPS是首个具有理论保证的技能选择算法，实现了(1-1/e, 1)双标准近似，收益系数在多项式时间内最优。
...
TL;DR: ExPhy是一个用于评估物体物理属性（质量、摩擦力、恢复系数）和轨迹预测的多目标轨迹预测基准数据集，包含24,000个模拟物理场景，并提供ID和OOD评估设置。提出的PhyODE模型在长视野OOD-Initial设置下将ADE和FDE分别降低33.1%和31.0%。研究表明准确的轨迹预测并不一定能准确恢复底层物理属性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试很少将物体级物理属性作为显式评估目标，通常只关注轨迹预测。该研究旨在填补这一空白，建立一个能够联合评估轨迹预测和物理属性估计的基准。

Method: 构建ExPhy数据集，包含24,000个模拟物理场景，具有质量、摩擦力和恢复系数的显式标签；提出PhyODE模型，通过观察轨迹估计物理属性，并使用可微分的未来 rollout 进行预测。

Result: 在OOD-Initial设置下，PhyODE相比最强基线将ADE和FDE分别降低33.1%和31.0%。在ComPhy上进行零样本评估测试跨基准迁移能力。属性级分析表明轨迹预测准确性不一定意味着物理属性恢复的准确性。

Conclusion: 准确的轨迹预测并不必然意味着能够准确恢复底层物理属性，这表明物理属性估计需要独立的评估和建模方法。ExPhy基准为联合评估轨迹预测和物理属性估计提供了重要基础设施。
...
TL;DR: 本文提出ThermoDPO方法解决流匹配中偏好优化的流形漂移问题，通过温度控制的锚点机制将成对偏好优化锚定在首选样本上，并在低温度下引入加权变体。在基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 偏好优化是生成模型对齐的标准方法，但扩展到连续时间动态仍具挑战。现有方法修改传输轨迹时缺乏对预训练数据流形的约束，导致终端样本偏离预训练支持区域，作者将此问题形式化为"流形漂移"。

Method: 理论分析表明最优流匹配可恢复终端数据分布，而偏好更新会在终端位移具有非零法向分量时离开预训练流形。提出ThermoDPO，通过温度控制目标将成对偏好优化锚定在首选样本上，连接拒绝采样微调和FlowDPO，控制流形距离的逐点重建代理。进一步引入ThermoDPO-weighted变体以应对低温度下信号减弱的问题。

Result: 在主玩具基准测试中，ThermoDPO-weighted达到StrictScore 0.899，优于FlowDPO的0.629和FlowDPO+RFT的0.857。在SD3.5-M上，CFG=4.5时OCR提升47.5%，四项指标平均提升16.0%。

Conclusion: ThermoDPO通过温度控制机制有效解决了流匹配偏好优化中的流形漂移问题，加权变体在实验中表现最优，为生成模型对齐提供了新思路。
...
TL;DR: 本文研究不完整多模态情感分析中未见过模态组合的泛化问题，提出CMPL模型，通过标签引导的对比特征学习、模态组合提示和提示对比学习策略来增强模型对未见模态组合的泛化能力，在三个数据集上准确率提升超过5%。


<details>
  <summary>Details</summary>
Motivation: 现实场景中测试阶段的模态组合往往与训练阶段不同，导致模型泛化能力不足和性能不稳定。现有的不完整多模态情感分析方法通常假设数据随机缺失或针对特定缺失模式设计，忽略了训练与测试阶段之间的模态组合不一致问题。

Method: 提出CMPL模型，包含三个核心组件：1)标签引导的对比特征学习机制，学习鲁棒且具有判别性的跨模态表示；2)带软路由器的模态组合提示，促进不同模态组合的学习；3)三种提示对比学习策略，有效学习未见过模态组合对应的提示。

Result: 在三个广泛使用的数据集上进行了大量实验，证明CMPL相比最先进的方法实现了超过5%的准确率提升。

Conclusion: CMPL能有效解决不完整多模态情感分析中未见过模态组合的泛化问题，显著提升了模型在多样化测试场景中的 generalization capabilities。
...
TL;DR: 本文构建了一个针对前沿AI系统的代理类型学框架，包含三个维度：代理的性质（道德或法律）、模式（个体或集体）和位点（人类或非人类），从而产生八种代理类型，并区分了法律代理与道德代理，为思考AI的法律代理而不预设其为道德代理提供了概念空间。


<details>
  <summary>Details</summary>
Motivation: 现有研究聚焦于规范意义上的代理及特定代理AI系统，但缺乏针对先进AI系统代理类型的框架，尤其在非道德形式的代理方面存在空白。

Method: 基于哲学、伦理学、法理学和社会学的既有理论立场，构建三维类型学框架。

Result: 形成三维八类的代理类型学：性质（道德/法律）× 模式（个体/集体）× 位点（人类/非人类），并将这些类型分类为常规型、争议型和矛盾型。

Conclusion: 该框架将法律代理与道德代理相分离，为考虑个体性、法律性、非人类代理而不预设AI为道德代理者创造了概念空间，这在工具性目标追求使AI行为归因复杂化的情境下尤为重要。
...
TL;DR: DECOWAM是一个全身世界-动作模型，通过分离相机自我运动与底座和手臂动作来改进移动操作机器人对未来观测和控制的预测。该方法使用FastWAM主干网络并训练残差适配器，在79次闭环实验中实现了最高全身协调性和底座位移鲁棒性，任务完成率与最强基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有世界-动作模型主要针对固定基座平台开发，未能明确区分相机自我运动与底座和手臂动作的影响。移动操作需要机器人预测运动和手臂运动如何共同改变未来观测和控制。

Method: 提出DECOWAM方法：1)冻结并适配FastWAM主干网络；2)训练残差适配器；3)使用来自特权观测的动作等效未来瓶颈；4)对抗性分离底座和手臂潜在变量；5)底座速度条件用于视频预测。同时引入ARMDOG数据集，同步采集视频、全身状态、动作和语言数据。

Result: 在固定回放协议上，DECOWAM相比FastWAM改进了未来视频和动作预测，动作MSE降低21.7%，使用25.95M可训练适配参数。在79次闭环实验中，达到最高全身协调性和底座位移鲁棒性，任务完成率与最强基线相当。

Conclusion: 具身感知因子化可以支持移动视角下参数高效联合视觉预测和全身控制。该方法通过明确分离底座和手臂因素，实现了更好的全身协调和鲁棒性。
...
TL;DR: DARS是一个用于指令引导图像编辑的强化学习框架，解决了双阶段（规划器-渲染器）pipeline中仅使用最终图像奖励导致的credit assignment困难问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用最终图像奖励训练效率低下，因为无法区分Planner和Renderer的贡献；推理密集型编辑难以在自由形式推理trace中定位问题。

Method: 提出双层credit assignment框架：(1)模块间：多规划多渲染rollout估计plan间和plan内reward变化，实现软模块路由；(2)Planner内：四字段结构化推理输出实现prefix-gated reward和token级优势重加权，将结果级反馈转化为局部监督。

Result: 在5个基准数据集上，DARS优于使用相同骨干网络、数据、奖励模型和rollout预算的Joint RL基线，推理密集型编辑任务提升最大。

Conclusion: 通过双层credit assignment和自适应课程学习，可有效提升双阶段图像编辑系统的训练效率，特别是对需要复杂推理的编辑任务。
...
TL;DR: 本文提出了软件开发的第三个范式转变——Software 3.0（软件3.0），即由上下文和推理决定行为的新时代。该范式最终收敛为三个核心要素：广义数据库（统一持久状态抽象）、大模型（推理和生成的智能核心）和代理（连接前两者的执行循环）。文章通过形式化论证、原型验证和理论分析，系统阐述了这一范式转变的条件与边界。


<details>
  <summary>Details</summary>
Motivation: 作者认为软件形态已经经历了两次范式转变：Software 1.0（指令决定行为）和Software 2.0（数据决定行为，即机器学习）。现在正在发生第三次转变，即Software 3.0。作者旨在论证这一新范式的可行性，并提出其最终形态和适用条件。

Method: 本文采用多维度研究方法：(1)形式化论证收敛理论；(2)设计最小参考架构；(3)构建真实原型和在线模型进行验证；(4)系统分析该理论的适用条件和失败边界。

Result: Software 3.0的收敛形态包含三个要素：广义数据库、大模型和代理。传统三层架构将发生根本性变化：用户界面层被模型的按需生成能力吸收，业务逻辑层按"可表达性×关键性"重新划分，数据层升级为唯一持久基础设施。该理论适用于满足可表达性、可验证性、外部状态性和工具完整性的任务领域。

Conclusion: Software 3.0范式在可表达、可验证、外部状态和工具完整的任务领域内成立，并将重塑开发者角色、数据库行业和软件工程学科。该范式的边界受限于确定性、成本、安全性和可验证性等因素。
...
TL;DR: 论文研究了大语言模型中记忆组件的认知陷阱问题，发现即使语义相关的记忆也可能扭曲模型推理并降低任务性能。提出了MemTrapBench基准和AdaptiveMem方法来评估和缓解这些认知陷阱。


<details>
  <summary>Details</summary>
Motivation: 现有记忆基准主要评估信息提取、存储和检索能力，但忽略了检索记忆如何影响模型推理和当前任务性能。作者发现即使忠实记录的记忆也可能导致认知陷阱，扭曲模型推理或信念。

Method: 提出MemTrapBench基准测试，涵盖两种认知陷阱形式：推理固着和信念扭曲。同时提出AdaptiveMem推理时方法。

Result: MemTrapBench具有挑战性，所有评估的记忆策略都低于无记忆设置，最强方法也遭受超过10%的性能下降。AdaptiveMem有效缓解了认知陷阱，同时在标准记忆基准上保持或提升性能。

Conclusion: 记忆组件需要谨慎设计，AdaptiveMem提供了一种简单有效的解决方案来避免记忆陷阱，为记忆增强LLM的发展提供了重要参考。
...
TL;DR: 本文介绍了ContractScrub，这是首个评估LLM合同 scrubbing 能力的基准测试，由经验丰富的律师手工制作涵盖滥用定义术语、错误引用和不一致语言等错误类别的合同。测试结果显示前沿模型表现令人惊讶地差，仅有一个模型达到0.75的宏观平均召回率，突出了当前模型的实际局限性以及针对特定领域的基准测试的重要性。


<details>
  <summary>Details</summary>
Motivation: 法律工作严重依赖处理大量文本，是最易受LLM影响的领域之一。合同scrubbing（合同最终审查以发现错误和不一致）是一项非常适合自动化的任务，因为它是常规且需要仔细关注长文档的工作。尽管具有经济价值和自动化潜力，但尚无对LLM执行合同scrubbing能力的正式评估。

Method: 研究团队引入ContractScrub，这是首个专为评估合同scrubbing能力设计的基准测试。该基准由经验丰富的律师手工制作，涵盖多种错误类别，包括：滥用定义术语、错误引用和不一致语言等。

Result: 前沿模型表现令人惊讶地差。尽管在看似相关的通用基准测试中表现强劲，但只有一个模型达到了0.75的宏观平均召回率（macro average recall）。

Conclusion: 研究结果证明了当前模型的实际局限性，突出了针对特定领域、精心设计的基准测试对于衡量真正现实世界影响的重要性。
...
TL;DR: 提出了一种基于机器学习的电子海图(ENC)变化自动分类方法，将复杂的矢量数据变化转换为结构化表格格式，通过空间上下文编码器和属性编码器增强表示，使用梯度提升树在两个数据集上分别达到90%和94%的准确率。


<details>
  <summary>Details</summary>
Motivation: 海图办公室依赖人工审核来判断海图变化对航运安全的关键性或非关键性，这种方式劳动密集、难以扩展到大量更新且存在分析员之间的不一致性问题。

Method: 建立了基线编码方案，将矢量数据变化转换为结构化表格格式；开发了空间上下文编码器来丰富变化的周围地理特征表示；设计了ENC属性编码器来表示修改对象的细微属性值描述；使用调优的梯度提升树模型进行分类。

Result: 在包含1,308对海图和超过100,000个单独海图修改的两个不同操作数据集上，模型准确率分别达到90%和94%，比默认超参数化的无空间上下文和属性嵌入的模型提高5-7%。

Conclusion: 证明了将机器学习集成到运营地理空间管道中以改善ENC维护和提高航运安全的可行性，简单位置和空间聚合方法有效，为评估更复杂的空间表示学习技术奠定了基础。
...
TL;DR: 本文提出InsufficiencyBench，首个评估法律AI模型识别查询信息不足能力的基准测试。基准包含8种缺失元素类别、202个测试项，覆盖6个法律领域和24个美国司法管辖区。评估10个前沿模型发现最高F2分数仅为0.46，无模型能同时识别缺失元素并恰当地对不完整查询作出有条件回应。


<details>
  <summary>Details</summary>
Motivation: 现有法律AI基准假设用户查询已完全指定，但实际中用户常遗漏关键法律事实。现有系统无法识别查询缺乏必要信息并拒绝过早给出结论，导致回答不准确或误导性。

Method: 构建了包含8种缺失元素类别的 taxonomy，涵盖三种结构失效模式（switch、gating、fatal prerequisite）。创建202个基准项（58个基础查询+144个缺陷变体），横跨6个法律领域和24个美国司法管辖区，由执业律师标注。评估了10个前沿大语言模型。

Result: 最佳模型在缺失元素识别上的F2分数仅为0.46，中位数召回率为0.44。模型要么过度谨慎（ hedging indiscriminately），要么基于虚构假设沉默回答或直接给出结论。没有任何模型能同时识别缺失元素、对不完整查询进行限定回应，并对完整查询给出直接回答。

Conclusion: 当前法律AI模型普遍缺乏识别查询信息不足的能力，无法恰当地处理不完整法律查询。这一能力缺失是法律AI系统实际部署的关键瓶颈，需要在模型训练中专门设计信息不足识别与条件回应的机制。
...
TL;DR: RuleMaze是一个用于评估多模态大语言模型(MLLMs)在规则约束下进行空间规划能力的基准测试。该基准通过迷宫导航任务，结合自然语言规则理解与空间规划能力，并提出了解耦多模态规划(DMP)方法来提升规则遵循和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在明确或未见规则约束下进行视觉空间规划的能力尚未被充分探索。该任务需要模型同时理解空间布局、解释自然语言规则并相应规划有效动作。

Method: 1)提出RuleMaze基准测试，通过迷宫导航任务隔离规则合规的空间规划能力；2)提出语言-逻辑-功能混合化(Language-Logic-Function Hybridization)方法，自动生成自然语言规则并翻译为逻辑表示和可执行验证器；3)提出解耦多模态规划(DMP)，通过可解释的推理原语分离感知、执行和规则验证。

Result: 实验表明，DMP相比端到端文本规划基线方法显著提升了规则合规性和规划成功率。

Conclusion: RuleMaze为研究MLLMs中基于规则的具身可解释空间规划提供了原则性基准。DMP方法能够促进对更复杂和未见规则的泛化，同时提供透明的中间规划轨迹。
...
### [125] [QUASAR: A Quantum-Classical Neural Network for SAR Satellite Physical-Layer Authentication](https://arxiv.org/abs/2608.20240)
*Vincenzo Sammartino,Nathanael Denis,Roberto Di Pietro*

Main category: cs.AI

TL;DR: QUASAR是首个将CNN光谱编码器与变分量子电路(VQC)相结合的量子-经典混合架构，用于X波段SAR卫星的物理层认证(PLA)。该方案仅需10%的训练数据即可达到与经典基线相同的准确率，在相同数据预算下分类准确率更优。在三种对抗场景下，欺骗拒绝率分别为89.7%、94.1%和81.3%。


<details>
  <summary>Details</summary>
Motivation: X波段SAR卫星在灾害响应、环境监测和军事情报中发挥关键作用，但缺乏强大的物理层认证(PLA)安全层。现有PLA系统通常限于6GHz以下频段，且依赖经典深度学习，无法充分捕捉区分卫星硬件的IQ相位非线性特性。

Method: 提出量子-经典混合架构QUASAR，融合CNN光谱编码器与变分量子电路(VQC)，为X波段SAR信号提供物理层认证。

Result: 在三种对抗场景下测试：重放攻击、定制IQ注入和天基欺骗。QUASAR分别以89.7%、94.1%和81.3%的概率拒绝欺骗传输。仅需10%的训练数据即可达到经典基线准确率，相同数据预算下分类准确率优于基线。

Conclusion: QUASAR首次实现了针对卫星星座的量子增强物理层分类器，为物理层认证领域开辟了新的研究途径，展示了量子机器学习在卫星安全领域的应用潜力。
...
TL;DR: 研究如何让推理语言模型自主分配推理计算资源，通过学习在三种模式间选择：快速回答、简短推理、扩展推理。使用GRPO训练，在MATH数据集上实现自适应token预算，平均准确率接近基线但减少41%响应长度，且能迁移到其他任务。


<details>
  <summary>Details</summary>
Motivation: 当前推理模型使用固定token预算，导致简单问题过度计算、困难问题计算不足。需要模型学习自适应分配推理资源。

Method: 通过Group Relative Policy Optimization (GRPO)，不使用独立路由器，让模型学习在三种模式间选择：NoThink（快速回答）、Short（简短推理）、Long（扩展推理）。使用塑形奖励使每种模式在不同响应长度下都有价值，并配合每模式token上限保持模式差异。

Result: 在1.5B蒸馏模型上三种模式涌现且未坍塌。简短模式比Long模式更准确，说明路由器按难度分类问题。在MATH500上准确率0.782（基线0.796），响应长度从4796降至2811 tokens（减少41%）。迁移到其他基准：GSM8K上token减少76%，且在相似长度下准确率更高。

Conclusion: 成功构建了能自适应选择推理量的推理模型，无需微调即可迁移到其他任务，在保持准确率同时显著降低计算成本。
...
TL;DR: 本文研究Solana区块链上 memecoin 的Rug Pull欺诈检测问题，通过构建640万个代币的7个月数据集，仅使用前5分钟交易数据训练XGBoost等机器学习模型，实现早期 rug pull 检测，并验证了PumpFun和Raydium平台间的跨平台泛化能力。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注以太坊代币的 rug pull 检测，但Solana区块链的memecoin欺诈机制不同——主要通过流动性操纵和社交动态而非智能合约后门实施诈骗。Solana是交易量和代币数量领先的memecoin区块链，面临严重的rug pull风险，需要针对性的检测方法。

Method: 收集7个月内640万个Solana代币数据，使用前5分钟交易数据作为特征，训练经典机器学习模型（重点是XGBoost梯度提升），并评估PumpFun和Raydium两个平台的跨平台泛化能力，通过多源数据融合缓解领域偏移问题。

Result: 研究发现绝大多数memecoin在发布后1小时内就表现出rug pull特征，XGBoost模型仅用5分钟交易数据即可实现有效的rug pull检测，多源数据融合显著改善了跨平台检测的可靠性，有效缓解了领域偏移问题。

Conclusion: 本研究推进了对高吞吐链上DeFi欺诈的理解，证明了在无需代码级特征情况下仅凭早期交易数据即可实现有效的rug pull检测，为保护投资者提供了实用的检测框架。
...
TL;DR: 研究大型语言模型(LLM)智能体如何从已完成任务中归纳技能，并研究技能归纳方式（任务级vs子任务级、文本vs代码格式）如何影响技能在不同任务间的转移可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM智能体可以从已完成任务中归纳技能并重复利用，但这些归纳的技能在任务间转移时可能不可靠甚至有害，需要研究如何归纳技能才能使其可靠转移。

Method: 对比任务级与子任务级技能归纳、文本与代码技能格式两种维度，通过特异性（skill与真实任务的匹配度）和抽象性（skill relevance在任务间的分布均匀度）两个互补属性来分析技能特性。

Result: 任务级技能大多低于无记忆基线性能，而子任务级技能平均高于基线；文本技能比代码技能转移更好。特异性与抽象性单独都不能预测任务成功，但两者结合可以（提出skill utility score）。

Conclusion: 子任务级和文本格式的技能具有更高的utility score，该评分可在任务执行前计算，作为技能记忆的实用诊断工具，无需实际运行任务即可评估技能质量。
...
TL;DR: 该论文指出语言模型改进评估不能仅看平均准确率变化，而应关注个体问题上的得失。研究通过对Qwen3-8B进行三轮LoRA自训练审计，发现七种测量失败，这些失败会逆转报告的发现。核心问题是现有评估方法缺乏适当的对照基准，导致许多"改进"实际上是测量伪影。


<details>
  <summary>Details</summary>
Motivation: 现有评估范式使用均值准确率来衡量语言模型改进，但实际应该关注个体问题上的能力转变。然而追踪这些转变需要两次噪声估计的差分，容易产生测量 artifacts。研究者希望系统性地识别这些测量失败，并提出更可靠的评估方法。

Method: 对Qwen3-8B进行三轮rank-32 LoRA自训练，与通过相同管道的冻结对照进行比较。识别七种测量失败场景，使用per-problem精确检验结合假发现率(FDR)控制来替代传统的膨胀统计量。

Result: 识别出七种测量失败：单次greedy decode制造伪影( expansion statistic给未训练模型赋值0.280)；自然阈值修复无法复现等。外部蒸馏改善了基础模型很少解决的问题，而三种自训练形式都未能做到；自训练甚至损害了baseline能解决的问题。

Conclusion: 转变层面的审计需要为每个报告的统计量单独测量null。Null不需要新实验，可以从多臂研究已拥有的baseline replicates构建，但需要足够数量的replicates才能有效。
...
TL;DR: 本文提出了MidTool，一个用于智能体工具使用中间训练的数据集构建流程，旨在教授模型识别工具能力、从上下文中验证参数、组合工具调用工作流以及从不完整信息中恢复。研究在Qwen3-4B和Qwen3-8BBase模型上进行实验，结果表明MidTool-Mix在BFCL、tau2-Bench和MCP Universe基准上显著提升了模型的工具使用能力。


<details>
  <summary>Details</summary>
Motivation: 当前研究已证明针对性的中间训练可以强化数学和科学等推理密集型能力，以及软件工程领域的智能体能力，但对于通用工具使用这一并行但探索较少的智能体能力缺乏系统研究。现有方法主要依赖后训练阶段来培养工具使用能力，效果有限。

Method: 1) 构建MidTool开放语料库管道，整合大规模网页、PDF和代码数据；2) 从真实世界工具API、MCP技能和文档驱动的工作流中合成监督信号；3) 在Qwen3-4B-Base和Qwen3-8B-Base上进行中间训练；4) 结合监督微调(SFT)和强化学习(RL)进行后续后训练。

Result: 在BFCL、tau2-Bench和MCP Universe三个基准测试上，MidTool-Mix在SFT和RL两种后训练方式下均一致性地提升了下游性能，相比基线模型表现更优。

Conclusion: 通用工具使用能力与其他重要的LLM能力类似，益于专门的中间训练，而非完全依赖后训练阶段。这一发现为LLM能力提升提供了新的训练范式。
...
TL;DR: 本文研究异构AI系统中查询路由的成本效益权衡问题，将价值估计器的选择形式化为"潘多拉盒子"问题，在高斯信号模型下推导出闭式解，提出集中式策略Pandora's Router和去中心化策略Pandora's Bidder，在三个领域实验表明能在保持路由质量的同时大幅减少昂贵估计器的调用次数。


<details>
  <summary>Details</summary>
Motivation: 异构AI系统需要将查询路由到最合适的专家模型，但评估每个专家的预期收益需要成本。廉价估计器（如基于嵌入的预测器）快速但噪声大，昂贵估计器（如微调模型或带检索结果的模型）准确但成本高。现有方法要么忽略估计成本，要么缺乏系统的成本效益分析框架。

Method: 将路由问题形式化为潘多拉盒子（最优搜索与代价检查）问题的变体。在高斯信号模型下，推导出价值信息的闭式表达式，用于决定是否值得为每个专家和输入进行精细化价值估计。提出两种策略：集中式的Pandora's Router和去中心化的Pandora's Bidder（专家可独立决定是否进行自我评估）。

Result: 在三个领域的实验表明：1）标准多LLM基准上，Pandora's Router达到穷举估计的路由质量，但显著减少昂贵估计器的调用次数；2）检索增强专家场景同样有效；3）可变推理时间LLM场景也适用。去中心化设置下，当竞争估计准确时能提高配置效率，但当估计噪声较大时可能增加战略专家的效用而损害他人。

Conclusion: 通过将价值估计决策建模为潘多拉盒子问题，可以在保证路由质量的前提下，系统性地决定何时使用昂贵的精细估计器。这一框架为异构AI系统的资源优化配置提供了理论基础和实用方法，具有重要的应用价值。
...
TL;DR: 本研究提出一个三智能体工作流程，将对话式数据收集、结构化数据处理和行为预测相结合，用于出行行为研究。通过聊天机器人管理的图像增强偏好选择调查收集了学生通勤者的出行模式选择数据，并评估了9个大型语言模型在零样本提示下的预测表现。


<details>
  <summary>Details</summary>
Motivation: 出行行为研究通常将数字数据收集与预测建模分开开发评估，缺乏整合的工作流程。本研究旨在将对话式调查、结构化数据处理、传统行为建模、机器学习和多模态LLM预测协调在一个可审计的多智能体工作流程中。

Method: 1)使用聊天机器人进行图像增强的陈述偏好调查，收集5种天气场景下学生通勤者的出行模式选择数据；2)采用多项Logit模型分析天气相关关联；3)使用逻辑回归和随机森林作为机器学习基准；4)在四种零样本提示和上下文条件下评估9个本地部署的LLM(2-350亿参数)；5)扩展使用角色设定、少样本和基于视觉的配置。

Result: 随机森林达到69.6%的五类准确率，最佳纯文本零样本LLM达到69.9%。习惯性出行信息带来最一致的增益，专家框架总体优于角色扮演，少样本提示改善了多个模型的预测。基于视觉的最佳配置达到71.5%的五类准确率。

Conclusion: 研究展示了如何将对话式调查、结构化数据处理、传统行为建模、机器学习和多模态LLM预测协调在可审计的多智能体工作流程中。视觉上下文可为选定模型提供额外的预测信息。
