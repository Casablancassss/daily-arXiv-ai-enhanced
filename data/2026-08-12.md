...
### [1] [Towards Unified Dynamic Face Landmark Detection](https://arxiv.org/abs/2608.10346)
*Sebastian Regalado,Varshanth R. Rao,Ruowei Jiang,Parham Aarabi,Igor Gilitschenski*

Main category: cs.CV

TL;DR: 本文提出统一动态人脸关键点检测（Unified Dynamic FLD），通过将每个关键点表示为面部部件轮廓上的进度值（FPALP），实现单一模型处理任意N点数据集并输出任意数量的关键点。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个功能限制：(1)每个N点数据集需要独立训练网络参数；(2)模型只能输出训练数据集对应的N个关键点。缺乏统一框架来整合不同标注标准的数据集。

Method: 提出FPALP表示方法，将关键点表示为面部部件轮廓上0-1的进度值；使用跨模态解码器逐步细化FPALP查询并预测最终坐标。

Result: 在多个基准数据集上实验表明，该方法在保持竞争力的同时，在多种场景下优于现有最先进方法。

Conclusion: Unified Dynamic FLD成功实现了单一模型处理多数据集、任意关键点数预测的能力，为人脸关键点检测提供统一框架。
...
### [2] [FormStruct-Bench:A Hierarchical and Diagnostic Benchmark for Table-Form Document Structure Recognition](https://arxiv.org/abs/2608.10396)
*Lujie Ban,Jiangtao Zhu,Yuanheng Yu,Jiasheng Shi,Chenhao Ma*

Main category: cs.CV

TL;DR: FormStruct-Bench是一个用于评估表格表单文档结构识别的层次化诊断基准，能够在文档级别和组件级别进行细粒度评估，揭示当前系统在恢复文档层次结构方面的显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估整体文档输出或传统表格网格，无法提供关于结构失败发生位置的洞察。需要一个能够追溯具体结构失败模式的诊断基准。

Method: 构建包含70个可重用模板的审计真实标注，通过Director-Artist-Verifier管道扩展至7,000个验证实例；测试集1,100个实例均经过人工审查。使用五个主要指标和三个结构特定诊断进行评估。

Result: 最佳文档级分数达到83.85%，但最佳细粒度结构分数仍低于18%，表明阅读文档内容与恢复层次结构和区域组织之间存在显著差距。

Conclusion: 当前表格表单理解系统在恢复文档层次结构和区域组织方面存在显著不足，需要进一步研究细粒度结构识别任务。
...
### [3] [A second-order theory of texture for depth from focus](https://arxiv.org/abs/2608.10411)
*Sreekar Ranganathan,Ioannis Gkioulekas*

Main category: cs.CV

TL;DR: 提出了一种基于波动光学的粗糙表面纹理外观理论，揭示了传统计算机视觉认为无纹理的表面可因主观散斑产生纹理，通过窄带光谱滤波可增强对比度，显著提升被动深度重建效果。


<details>
  <summary>Details</summary>
Motivation: 传统计算机视觉认为纹理缺失的表面无法进行深度重建，但作者发现即使看似光滑的表面也会因微观几何结构产生散斑纹理，这为被动深度估计提供了新思路。

Method: 基于波动光学建立表面纹理Appearance理论，使用窄带光谱滤波增强散斑对比度，结合自然环境光照进行被动深度重建。

Result: 理论分析、仿真和真实实验均表明，该方法能让传统认为无纹理的场景产生可辨识的纹理特征，显著提升被动深度重建的精度和效果。

Conclusion: 通过利用主观散斑现象和光谱滤波技术，可以突破传统视觉对纹理的要求限制，为被动深度估计开辟新途径，具有重要的应用价值。
...
TL;DR: 本文提出DriveVLA-M0，一种具有失败感知的潜在记忆检索增强VLA模型，用于端到端自动驾驶。该方法通过构建存储失败案例的潜在记忆池，设计解耦的检索模型，并在推理时通过LoRA-based TTT机制注入检索到的案例，解决现有VLA模型无法从过去失败中学习和适应分布偏移的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏利用过去失败或适应分布偏移的机制，导致模型在类似失败场景中持续表现不佳，缺乏持续学习和场景特定修正能力。

Method: 1) 构建潜在记忆池，存储失败案例及其结构化场景表示和专家轨迹标签；2) 设计解耦检索模型，将静态道路结构与动态agent交互分离，实现结构化 grounded检索；3) 采用轻量级解耦LoRA-based测试时训练(TTT)机制，在推理时注入检索案例，实现针对性和场景特定的修正。

Result: 在NAVSIMv1和NAVSIMv2基准测试中，DriveVLA-M0在Navtest上达到94.1 PDMS，在Navhard上达到47.0 EPDMS，TTT反向传播延迟开销仅为26.44ms，且随记忆池扩大可实现免训练的性能提升。

Conclusion: DriveVLA-M0通过检索增强和失败感知记忆机制，有效解决了VLA模型无法从失败中学习的问题，在自动驾驶基准测试中一致优于先前方法，且具有高效的推理效率和良好的可扩展性。
...
TL;DR: GeoSeg-OV是一种开放词汇遥感分割方法，通过将视觉基础模型(VFM)特征从视觉-文本匹配中解耦，并重新用作成本聚合和解码的结构引导，解决了地理空间域迁移导致的跨数据集泛化问题。


<details>
  <summary>Details</summary>
Motivation: 开放词汇遥感分割面临地理空间域迁移问题（异质区域、空间分辨率、采集平台的差异），这削弱了视觉-文本匹配效果并限制了跨数据集泛化。现有的视觉基础模型(VFM)耦合策略可能引入不一致的匹配信号，且未充分利用VFM的结构敏感表示。

Method: 提出GeoSeg-OV方法：(1)从多旋转CLIP特征构建方向鲁棒的成本体；(2)冻结VFM提取多尺度结构敏感特征；(3)提出结构引导聚合(SGA)，将成本tokens与CLIP语义引导及VFM衍生的成对结构偏差集成进行连贯空间传播；(4)提出成本感知解码(CAD)，基于当前解码器上下文自适应细化融合多尺度语义和结构引导。

Result: 在涵盖六大洲七个数据集的全球高分辨率土地覆盖(HRLC)基准上，GeoSeg-OV在两种训练设置下分别优于最先进方法+2.5和+2.7平均mIoU。大规模零样本案例研究进一步证明其无需目标域标注或重训练即可跨地理域和类别系统泛化。

Conclusion: 通过解耦VFM特征并将其作为结构引导而非直接匹配信号，GeoSeg-OV有效解决了地理空间域迁移问题，实现了优异的跨数据集泛化性能，为开放词汇遥感分割提供了新的研究方向。
...
### [6] [Lesion-Aware Adaptive Fourier Neural Operator for CT-to-PSMA PET Synthesis in Prostate Cancer](https://arxiv.org/abs/2608.10429)
...
TL;DR: LAFNO是一种用于CT到PSMA-PET合成的病变感知自适应傅里叶神经算子，通过对比度代理和紊乱代理两个CT衍生通道替代耗时的直接放射组学条件化，在保持竞争力的图像质量(SSIM 0.960/0.938)的同时，显著降低了患者总病变活动(TLA)误差(48.3%/64.0%)。


<details>
  <summary>Details</summary>
Motivation: 全身PSMA-PET中肿瘤体素仅占很小比例，但携带临床相关信号，传统全局损失函数(如L1/MSE)会低估病变活动或无法保留肿瘤特异性结构；直接放射组学条件化因需从标注病变区域提取特征而耗时。

Method: 提出LAFNO，使用两个CT衍生代理通道(对比度代理表示局部密度变化，紊乱代理表示局部纹理异质性)注入模型瓶颈；结合全volume重建与病变级TLA、肿瘤核心对比度和肿瘤周围监督。

Result: LAFNO在18F-和68Ga-PSMA上分别达到SSIM 0.960和0.938；TLA误差降至48.3%和64.0%；两种示踪剂的肿瘤核心放射组学可重复性最高；肿瘤周围可重复性因示踪剂而异。

Conclusion: LAFNO通过代理通道实现了高效的病变感知PET合成，在图像质量和生物保真度间取得良好平衡；但肿瘤周围区域的生物保真度仍具挑战性，需进一步研究。
...
### [7] [DynaPPI: A Large-scale Dynamic Protein Dataset for AI-driven Advances in Protein Interactomics](https://arxiv.org/abs/2608.10435)
*Jiabao Wei,Zilong Geng,Yuze Wang,Jianjun Li,Ning Ding,Bowen Zhou,Bing Zhang,Zhiyuan Ma*

Main category: cs.CV

TL;DR: DynaPPI是一个动态蛋白质数据集，包含蛋白质复合物形成的分子动力学轨迹，从解离链到结合状态，用于训练扩散模型预测未知蛋白质复合物结构。


<details>
  <summary>Details</summary>
Motivation: 解决现有蛋白质数据集仅关注静态快照或单体轨迹的问题，弥补静态结构生物学与动态分子相互作用之间的差距，推动AI驱动的结构生物学和蛋白质相互作用组学发展。

Method: 构建包含蛋白质复合物形成过程的MD轨迹数据集，使扩散模型能够学习动态结合轨迹并基于生成特性预测未知复合物结构。

Result: 提供了一个关键资源，使扩散模型能够学习已知复合物的动态结合轨迹，并准确预测未知复合物的结构。

Conclusion: DynaPPI数据集将促进AI在结构生物学和蛋白质相互作用组学领域的进一步应用。
...
### [8] [MammoMix: Leveraging Mixture of Experts for Robust Mammogram Breast Detection](https://arxiv.org/abs/2608.10437)
*Dinh Tan Nguyen,Hoang Quan Dang,Chen Zhang,Sai Ho Ling*

Main category: cs.CV

TL;DR: MammoMix是一种基于混合专家(MoE)范式的新型乳腺病变检测框架，通过让每个专家模型在特定领域进行训练，并使用门控机制自适应地融合专家输出，同时结合校准模块MoCAE来调整置信度分数，从而提高模型在不同数据集间的泛化能力和可靠性。在CSAW、DDSM和DMID三个公共数据集上的实验表明，该方法在平均精度和可靠性方面均优于基线检测器。


<details>
  <summary>Details</summary>
Motivation: 当前目标检测器(如YOLO和DETR)在单一数据集上表现良好，但在跨数据集应用时性能显著下降。这是因为不同数据集在图像质量、病变表现和人口统计学特征上存在显著差异。该研究旨在解决医学影像领域中的域迁移问题，提高模型在真实临床环境中的泛化能力。

Method: 提出MammoMix框架，包含三个核心组件：1)多个领域专家模型，每个专家针对特定数据集进行专门训练；2)门控机制，根据输入图像自适应地调整各专家的权重并融合输出；3)MoCAE校准模块，用于调整置信度分数以反映真实的预测不确定性。整个框架采用端到端训练方式。

Result: 在CSAW、DDSM和DMID三个公共乳腺影像数据集上进行评估，结果显示MammoMix在平均精度和可靠性方面均优于基线检测器。特别是在数据变异性较大的数据集上，性能提升更为显著。这表明专家专业化与校准集成融合的结合能够显著增强模型的泛化能力和鲁棒性。

Conclusion: MammoMix通过混合专家范式实现了乳腺病变的鲁棒检测，有效解决了跨数据集的域迁移问题。实验证明专家 specialization和校准集成融合策略可以显著提升模型的泛化性和可靠性。该研究为构建可靠的AI辅助乳腺癌筛查系统迈出了重要一步，有望在多样化的真实临床环境中得到应用。
...
### [9] [Stream Forcing: Constructing Unified Training Trajectory for Robust Streaming Video Generation](https://arxiv.org/abs/2608.10439)
*Yueting Zhu,Yuehao Song,Kaicheng Zhang,Bao Tang,Shaoyu Chen,Qian Zhang,Wenyu Liu,Xinggang Wang*

Main category: cs.CV

TL;DR: Stream Forcing是一种统一流式视频生成训练框架，通过将扩散采样建模为帧索引随机过程并构建连续训练轨迹，解决了训练覆盖与推理一致性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 流式视频生成中的核心挑战：推理时遵循专门的去噪顺序，而先进训练策略通常需要多样化的噪声水平配置，导致训练-推理不一致（train-inference mismatch）问题。

Method: 1) 将视频扩散采样重新表述为帧索引的噪声水平随机过程；2) 在该过程空间中构建连续训练轨迹，使采样调度从独立采样逐步演进到推理一致的采样；3) 引入联合校准算法确保轨迹平滑；4) 引入时序相关性采样算法确保跨帧相关性。

Result: 在UCF-101基准上实现36.6%的FVD提升；在UCF-101长时域零样本外推任务中实现27.9%的FVD提升。

Conclusion: 提出的Stream Forcing框架通过在噪声水平上构建连续训练轨迹，有效解决了流式视频生成中训练-推理不一致的问题。实验表明该方法在UCF-101数据集上显著提升了生成质量，并在长时域视频生成任务中展现出强大的零样本外推能力。
...
### [10] [FUSE: Frame-Unified Stress Estimation from Facial Video](https://arxiv.org/abs/2608.10442)
*Stefanos Gkikas,Thomas Kassiotis,Yang Guo,Guangliang Li,Giorgos Giannakakis*

Main category: cs.CV

TL;DR: FUSE是一种无需时序窗口化的面部视频压力检测框架，通过将时间维度折叠到通道维度，用统一的非对称注意力架构处理完整录制视频，在58名受试者的压力数据集上达到69.44%的最高准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频的压力检测方法通常将完整录制分割成短时间窗口再进行分类，这种设计引入了窗口长度、重叠和聚合等额外选择，并限制了跨整个录制的时间信息直接分析。

Method: 提出FUSE框架，将所有帧融合成一个统一的二维表示（将时间维度折叠到空间表示的信道维度），使用统一的非对称注意力架构处理高维输入。实验评估了7个时间步长配置（t=1到t=30）。

Result: 在58名受试者的压力数据集上，FUSE在t=15时达到最高测试准确率69.44%，完整帧配置（t=1）也具有竞争力达69.03%。计算成本范围为12.48至348.78 GFLOPs。

Conclusion: 时序窗口化对于有效的面部视频压力检测并非必需，完整录制推理可以在单一统一架构内实现。
...
### [11] [Bridging Event Streams and DiT: Event-Guided Video Frame Interpolation](https://arxiv.org/abs/2608.10479)
*Guixu Lin,Yuyang Yu,Xiang Ji,Linyao Chen,Zhengwei Yin,Mengshun Hu,Mingdeng Cao,Shengfeng He,Yinqiang Zheng*

Main category: cs.CV

TL;DR: 提出了一种基于适配器的框架，将事件相机数据融入预训练的图像到视频扩散模型，用于视频帧插值。通过利用图像扭曲事件(IWE)和双向稀疏光流提供时空对齐的引导，减少插值伪影，提高重建保真度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: 当前潜在扩散模型在视频帧插值中面临大时间间隔和复杂运动的挑战，容易产生运动模糊、结构失真和时间不一致问题。事件相机提供高时间分辨率的运动线索，非常适合弥补这些差距并提高插值质量。

Method: 提出适配器框架，将事件派生线索融入预训练图像到视频扩散模型，架构改动最小。具体利用图像扭曲事件(IWE)和双向稀疏光流，在生成过程中提供空间和时间对齐的引导，将事件引导的结构和运动线索注入扩散过程。

Result: 在真实和合成基准数据集上的实验表明，该方法一致性地优于现有最先进方法。

Conclusion: 该研究成功实现了事件相机数据与扩散模型的有效结合，为高质量视频帧插值提供了新思路，在重建保真度和时间一致性方面均有提升。
...
### [12] [When Vision Becomes Text: Visual Token Pruning via Cross-Modal Residual Guidance in VLMs](https://arxiv.org/abs/2608.10489)
*Congyang Ou,Ruike Song,Yang Zhou,Libo Sun,Haokui Zhang,Zhenbo Luo*

Main category: cs.CV

TL;DR: 本文提出SIEVE，一种无需训练的视频语言模型视觉token压缩方法，通过跨模态吸收(CMA)和跨模态残差(CMR)机制，仅保留11.1%的视觉token即可保持97.5%的原始性能，实现3.62倍预填充加速。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法依赖相似度引导，仅捕获局部层信号，忽略了VLM整体推理过程。随着LLM层加深，文本token通过自注意力持续聚合视觉信息，但现有方法未能有效利用这一现象。

Method: 提出跨模态吸收(CMA)从几何表示角度量化视觉信息被文本吸收的程度；提出跨模态残差(CMR)利用Tikhonov正则化最小二乘将视觉token投影到文本子空间，通过重构残差衡量无法被文本解释的视觉信息；最终结合CMR、文本注意力相关性和残差空间多样性提出SIEVE方法。

Result: 在LLaVA-NeXT-7B上，SIEVE仅保留11.1%的视觉token即保持97.5%的原始平均性能，实现3.62倍预填充加速、2.49倍端到端加速和6.02倍KV-cache减少。

Conclusion: CMR能有效识别被文本充分解释的冗余视觉token，SIEVE在多种VLM架构上验证有效，为视觉token压缩提供了新的几何视角和无需训练的高效方案。
...
### [13] [SapiensID 2.0: Aligning Human Recognition Foundation Models with Human Perception](https://arxiv.org/abs/2608.10497)
*Yiyang Su,Jie Zhu,Feng Liu,Anil K. Jain,Xiaoming Liu*

Main category: cs.CV

TL;DR: SapiensID 2.0是一个新的人类识别框架，专注于增强语义和时间感知能力，以解决当前基础模型在人类识别中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型依赖静态几何特征提取，与人类感知方式存在根本差异，导致"语义盲点"问题：过度拟合瞬态噪声而未能利用不变软生物特征，同时难以捕捉时间运动特征。

Method: 提出三大技术：1)-invariant Trait Alignment (ITA)解决多模态大语言模型与判别嵌入空间之间的维度不匹配，提取核心持久特征；2) Transient Noise Disentanglement (TND)解耦衣物等伪影；3) Kinematic Semantic Attention Head (K-SAH)跨时间窗口扩展空间注意力，捕捉运动特征。

Result: 在图像和视频行人重识别以及步态识别任务上达到最先进性能，同时保持强大的人脸识别能力。

Conclusion: 通过语义和时间感知的融合，SapiensID 2.0成功解决了传统模型的语义盲点问题，为人类识别提供了一种更接近人类感知的新范式。
...
### [14] [DSAR: Dual-Stream Autoregressive Modeling of Temporal Cloth Dynamics for Photorealistic Animatable Avatars](https://arxiv.org/abs/2608.10500)
*Haozhong Xiong,Yao Yu,Yu Zhou,Sidan Du*

Main category: cs.CV

TL;DR: 本文提出一种双流自回归框架，用于从RGB视频创建具有真实布料动态的拟真可动画人类avatar。该方法显式建模了布料物理中的时序因果关系，通过几何流传播表面位移，状态流融合历史状态，解决了现有方法忽略时序因果性导致的过度平滑和分布外姿态伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了布料物理中固有的时序因果性——当前状态是通过时序演化从先前状态产生的，而非仅由即时骨骼配置决定。这导致网络学习到的是姿态-外观相关性而非运动演化，从而在分布外姿态上泛化性能差、产生严重伪影。

Method: 提出双流自回归框架，包含：1) 几何流，从前一帧传播表面位移；2) 状态流，从记忆库检索历史状态并与当前特征融合；3) 运动自适应聚合处理空间变化的动态；4) 自适应正则化平衡平滑性与灵活性。

Result: 在挑战性数据集上的实验表明，该方法在渲染质量、时序一致性和分布外运动模式的泛化能力方面均有显著提升，验证了双流时序建模能够实现真实的布料动态效果。

Conclusion: 通过显式建模时序因果结构，双流自回归框架有效解决了avatar渲染中布料动态不真实、分布外姿态伪影严重的问题，为构建高质量可动画人类avatar提供了新的研究方向。
...
### [15] [Towards Color-Faithful Low-Light Image Enhancement via Adaptive Color Debiasing and Saturation Rectification](https://arxiv.org/abs/2608.10512)
*Zhichen Yang,Rui Xu,Yuzhen Niu,Fusheng Li,Hui Da,Ri Cheng*

Main category: cs.CV

TL;DR: 提出CAGE框架，解决低光图像增强中的颜色偏差和饱和度异常问题。该方法采用圆柱形自适应LAB色彩空间(AdaLAB)和自适应圆柱形颜色变换(AdaCCT)，通过色平面平移和缩放抑制颜色偏差，通过离域亮度补偿实现饱和度校正。


<details>
  <summary>Details</summary>
Motivation: 低光成像因低信噪比和图像形成过程导致颜色偏差，表现为整体颜色偏移和局部欠/过饱和。虽然现有低光增强方法实现了良好的亮度恢复，但颜色恢复仍然困难。

Method: 1) AdaLAB：圆柱形自适应LAB色彩空间，提供解耦的图像特定颜色校正基础。2) AdaCCT：自适应圆柱形颜色变换，包含前向和反向变换。前向变换通过色平面平移和缩放重组色度分布以抑制嵌入的颜色偏差；反向变换通过离域亮度补偿实现饱和度校正。

Result: 在多个基准数据集上的广泛实验表明，CAGE实现了更真实的颜色恢复，有效减少了颜色偏差和饱和度异常，并在不同低光增强骨干网络上提供了更好的整体视觉质量。

Conclusion: CAGE框架能够有效解决低光图像增强中的颜色偏差和饱和度问题，具有良好的通用性，可与多种低光增强骨干网络结合使用。
...
### [16] [SafeCap: Improving LVLM Safety with Image Captioning Reinforcement Learning](https://arxiv.org/abs/2608.10513)
...
TL;DR: SafeCap是一种通过自我描述（captioning）实现的大型视觉语言模型（LVLM）安全对齐框架。该框架训练策略模型首先生成安全相关的图像描述，再基于描述生成最终答案，通过描述是否能帮助冻结的LLM做出安全决策来优化描述质量。这种描述介导的目标促使策略模型暴露与安全响应生成相关的视觉线索，而非仅依赖直接拒绝监督。在五个多模态安全基准和六个视觉效用基准上，SafeCap在其DirectCap协议下显著提升了整体安全性能，在四种模型设置下安全评分提升3.7-19.0分，同时保持相当或更好的视觉效用。在相同骨干模型和数据的对照比较中，SafeCap优于安全SFT、DPO和SafeGRPO，证明了描述介导的强化学习在多模态安全对齐中的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）仍然容易受到 jailbreak 攻击，这类攻击利用视觉输入来绕过从语言骨干继承的安全对齐机制。随着LVLMs的广泛应用，其安全性问题变得尤为重要。

Method: SafeCap采用强化学习框架，通过学习自我描述来对齐LVLMs。具体包括：1）训练策略模型首先生成安全相关的图像描述；2）基于描述生成最终答案；3）通过描述是否能帮助冻结的LLM做出安全对齐的决策来优化描述质量。这种描述介导的目标鼓励模型暴露与安全响应生成相关的视觉线索。

Result: 在五个多模态安全基准和六个视觉效用基准上，SafeCap在DirectCap协议下显著提升了整体安全性能。在四种模型设置下，安全平均分提升3.7-19.0分，同时保持相当或更好的视觉效用。SafeCap在相同骨干模型和数据条件下，优于安全SFT、DPO和SafeGRPO。

Conclusion: 描述介导的强化学习是实现多模态安全对齐的有效方法。SafeCap通过让模型学习生成安全相关的图像描述作为中间步骤，成功增强了LVLMs抵御视觉 jailbreak 攻击的能力，同时保持或提升了模型的实用性能。
...
### [17] [SparSTAR: Sparse Attention for SpaceTime AutoRegressive Video Synthesis](https://arxiv.org/abs/2608.10519)
*Jongbeom Lee,Hyunwoo Yu,Jincheol Yang,Jaemin Choi,Suk-Ju Kang*

Main category: cs.CV

TL;DR: SparSTAR是一种无需训练的视频生成加速方法，针对InfinityStar在视频生成中的计算瓶颈，通过块稀疏注意力机制实现约1.6倍加速。


<details>
  <summary>Details</summary>
Motivation: InfinityStar将视觉自回归生成扩展到视频时，存在后期尺度注意力计算成本高、以及从图像VAR模型复用的稀疏模式不可靠的问题。

Method: 在每个昂贵的尺度和注意力头中，SparSTAR对当前query和key激活中的连续key块进行评分，保留所需的条件上下文，并通过前向稀疏路径执行所选块。

Result: 在720p文本到视频和图像到视频生成任务中，SparSTAR保留了每个token和精炼尺度，实现了约1.6倍的端到端加速，VBench分数和配对输出重建保真度接近密集InfinityStar。

Conclusion: 跨尺度一致性分析表明，在每个目标尺度重新计算块选择比转移复用掩码更可靠。
...
### [18] [Unlocking the Power of Medical Tabular Data via Semantic-Aware Multimodal Pre-training](https://arxiv.org/abs/2608.10522)
*Yingsheng Liu,Haiming Li,Jingmin Zhu,Jiajun Sun,Victoria Mar,Monika Janda,H. Peter Soyer,Zongyuan Ge,Zhen Yu*

Main category: cs.CV

TL;DR: 本文提出了一种新的语义感知框架，用于医学表格数据的表示学习。通过重要性感知自适应掩码和软标签离散化模块两个创新组件，解决了传统方法中表格数据处理忽略结构信息且回归目标不稳定的问题，在皮肤病学和眼科学数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型主导医学表示学习，但非结构化文本缺乏结构化临床表格中固有的密集定量诊断表型。现有多模态预训练方法因语义无关的设计将表格输入视为扁平向量，且使用不稳定的连续回归目标，从而未能充分利用这一潜力。

Method: 提出语义感知框架显式建模表格数据的内在二维结构：1) 针对特征间不同诊断重要性的层级关系，引入重要性感知自适应掩码构建无标签课程优先处理显著特征；2) 针对特征内连续-离散二元性，提出软标签离散化模块用稳定分布匹配替换不稳定的数值回归，从而数学上保留序数关系。

Result: 在大型皮肤病学(SLICE-3D、HOP)和眼科学(EyePACS)数据集上进行广泛实验，建立了新的最先进水平，展示了出色的鲁棒性和跨域泛化能力。

Conclusion: 该框架通过显式建模表格数据的二维结构，能有效利用临床表格中的定量诊断信息，在医学表示学习中取得显著性能提升，具有良好的应用前景。
...
### [19] [Rethinking Text-Based Image Retrieval in Specific Domain](https://arxiv.org/abs/2608.10524)
*Jingyang Tan,Sheng Yang,Yuanpeng Chen,Jian Wang,Nianjin Ye,Chen Xing,Lanpeng Jia*

Main category: cs.CV

TL;DR: 本文针对现有文本图像检索基准测试仅支持单一匹配的局限性，设计了领域特定多匹配TBIR数据引擎，构建了SecMM-TBIR监控基准，并提出SAFT框架解决特定领域语义压缩问题。


<details>
  <summary>Details</summary>
Motivation: 现有TBIR基准基于单一匹配假设，无法反映实际监控等领域中单一查询对应多个相关图像的场景性能，且vanilla对比学习在特定领域存在严重假阴性问题。

Method: 设计DSMM-TBIR数据引擎构建SecMM-TBIR基准（50k监控图像，200个查询）；提出SAFT框架，包含语义感知软标签监督(SASS)和模态内结构蒸馏(ISD)来解决领域特定TBIR的语义压缩问题。

Result: 在SecMM-TBIR上，SAFT相比标准ITC微调平均提升mAP@20达7.8个百分点，同时改善了通用领域性能。

Conclusion: SAFT为领域特定TBIR任务提供了有效范式，SecMM-TBIR基准将促进该领域进一步研究。
...
### [20] [Dynamic Context Adapters: Efficiently Infusing History into Vision-and-Language Models](https://arxiv.org/abs/2608.10525)
*Yuhang Song,Bor-Jiun Lin,Jiaxu Liu,Te-Chuan Chiu,Anh Nguyen,Chun-Yi Lee*

Main category: cs.CV

TL;DR: 本文提出动态上下文适配器(DCA),解决视觉-语言模型(VLM)在序列决策任务中整合历史上下文的核心挑战。该方法通过固定大小的动态压缩记忆保存历史语义,避免帧拼接带来的二次复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 当前VLM独立处理视觉输入,无法满足需要时序理解的下游应用需求。直接将历史帧纳入Transformer输入会导致二次方注意力复杂度和过高的内存消耗,现有方法在计算效率和信息保留之间难以权衡。

Method: 提出动态上下文适配器(DCA),采用固定大小的动态压缩记忆来保留历史语义,无需帧拼接。该方法桥接静态VLM与循环策略,使预训练模型具备记忆能力的同时保持计算效率。

Result: DCA在注意力FLOPs上实现超过25%的降低,内存节省13%,同时在长时域任务上提升了性能表现。

Conclusion: DCA成功实现了计算效率与历史信息保留的有效平衡,为预训练VLM在序列决策任务中的应用提供了新的思路,兼顾了模型性能与资源消耗。
...
### [21] [Flow Straight to Reality: Perceptually Consistent Flow Matching for Efficient Image Restoration](https://arxiv.org/abs/2608.10544)
*Sangwoo Jo,Donggeun Ko,Jayeon Kang,Youngsang Kwak,Jaehwa Kwak,Sungjoon Choi*

Main category: cs.CV

TL;DR: PCFlow是一种统一的图像恢复框架，通过连续传输机制直接参数化从退化观测到干净目标的映射，联合优化失真和感知质量。该方法结合潜在一致性流目标、潜在一致性感知损失（LCPL）和无冲突梯度投影策略，实现稳定高效的少步推理。


<details>
  <summary>Details</summary>
Motivation: 图像恢复在失真与感知之间存在根本权衡：最小化像素误差导致过度平滑，而优化感知真实性则引入结构偏差。现有方法（如后验采样或多阶段生成管道）计算成本高且架构复杂。

Method: 提出PCFlow框架，包含三个核心组件：1）潜在一致性流目标实现稳定高效的少步推理；2）潜在一致性感知损失（LCPL）对引导速度场施加语义约束；3）无冲突梯度投影策略稳定多目标优化景观。采用轻量级纯卷积骨干网络。

Result: 在多种恢复任务上达到竞争性能，且计算成本显著低于传统方法。

Conclusion: PCFlow通过统一框架有效平衡了失真与感知质量，为高效图像恢复提供了新思路。
...
### [22] [A HamNoSys-Guided Dataset and Baselines for Fine-Grained Isolated Handshape Recognition in Sign Language](https://arxiv.org/abs/2608.10588)
*Ushnish Sarkar,Suvajit Patra,Bhaswar Chattopadhyay,Pranab Singha Roy,Tapas Samanta*

Main category: cs.CV

TL;DR: 本文提出了一个基于HamNoSys的手形识别基准数据集，包含144,000张RGB图像，来自15位参与者共160种手形类别。评估了ResNet-18、ViT-B/16、GCN和XGBoost四种模型，在主体相关和留一主体出（LOSO）两种协议下进行测试。


<details>
  <summary>Details</summary>
Motivation: 当前细粒度手形识别缺乏基于语言独立符号系统（HamNoSys）的全面基准数据集，且缺乏针对未见签署者的泛化能力评估。该研究旨在填补这一空白，提供可复现的手形识别资源。

Method: 收集144,000张RGB图像（15人×160类），使用ResNet-18和ViT-B/16作为外观模型，GCN和XGBoost基于手部地标进行识别。采用类别分层的主体依赖划分和15折留一主体出（LOSO）协议，并在LSWH100和ASL Fingerspelling Dataset A上进行外部验证。

Result: 主体依赖基准在各模型家族中建立了可复现的参考性能；LOSO评估显示对未见参与者的泛化存在显著下降。在ASL Fingerspelling Dataset A上，LOSO top-1准确率范围为82.20%至87.40%。

Conclusion: 该数据集和评估协议为细粒度独立手形研究和无障碍手语技术开发提供了可复现的资源。
...
### [23] [$π$-SUB: A Physics-Informed Synthetic Underwater Benchmark Dataset for Underwater Image Enhancement](https://arxiv.org/abs/2608.10589)
...
### [24] [Rethinking Data Efficiency in Industrial Dense Prediction: Pretraining Coherence, Not Inductive Bias, Determines ViTs Low-Data Advantage](https://arxiv.org/abs/2608.10590)
*Haoran Sui,Yaoyuan Jia*

Main category: cs.CV

TL;DR: 本文研究了Vision Transformers (ViTs)在工业密集预测任务中数据效率低于CNN的原因，发现问题源于ImageNet预训练的ViT骨干网络与COCO预训练的CNN颈部之间的"预训练不一致性"，并提出AlignBlock系列方法进行金字塔级特征重校准。


<details>
  <summary>Details</summary>
Motivation: 解决工业密集预测中ViTs比CNN需要更多标注数据的痛点，通过实验揭示数据效率差距的真实原因并非注意力机制本身的缺陷，而是预训练架构的不匹配。

Method: 在四个工业数据集上进行控制实验，提出轻量级AlignBlock家族用于金字塔级特征重校准，弥合跨架构特征差距。

Result: 在领域接近场景(≥200样本)：Swin-Graft超越YOLOv11x (0.973 vs 0.956 mAP@50)；在领域远离场景：CNN保持优势 (0.900 vs 0.600 mAP@50)。 Graft颈部权重可获得随机初始化颈部2.5倍的mAP。

Conclusion: 预训练不一致性是ViTs数据效率低的根本原因；对于领域接近场景ViTs表现更优，领域远离场景CNN仍具优势；提出的AlignBlock方法能有效提升ViT的性能。
...
### [25] [Gaussian Sculpting: End-to-End Controllable Surface Reconstruction via Field Optimization](https://arxiv.org/abs/2608.10602)
*Ke Jiaxin,Juncheng Liu,Yi Wang,Zhouhui Lian,Bin Liu,Shengfa Wang,Xiangjia He*

Main category: cs.CV

TL;DR: Gaussian Sculpting是一种用于高质量表面重建的完全可微端到端框架，通过将高斯锚定在演化的可微表面上来指导SDF优化，而非仅在后处理中提取表面。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS)在有限视角下难以恢复准确表面，高斯原语的不规则性导致几何误差难以手动校正。

Method: 1) 将高斯锚定在演化的可微表面上来指导SDF优化；2) 设计双层训练策略外层优化SDF几何，内层固定几何更新高斯；3) 对高斯参数施加约束确保与底层表面一致；4) 引入基于八叉树的多分辨率细分方案保留细节并降低内存消耗。

Result: 在目标级场景的实验表明，该方法有效去除冗余表面、恢复有限视角导致的缺失结构，即使在较低分辨率下也能实现较强的重建质量。

Conclusion: Gaussian Sculpting能够实现高质量表面重建，在几何精度和外观保真度方面都有提升。
...
### [26] [InSight-doc: Agentic Visual Perception for Long-Document Understanding](https://arxiv.org/abs/2608.10628)
*Kaican Li,Weiyan Xie,Lewei Yao,Jiannan Wu,Lanqing Hong,Yongxiang Huang,Nevin L. Zhang*

Main category: cs.CV

TL;DR: InSight-doc是一个代理视觉感知框架，通过将视觉分辨率视为自适应推理时间资源，从低分辨率开始逐步放大高分辨率区域进行细粒度证据提取，无需外部检索器即可完成长文档理解任务。


<details>
  <summary>Details</summary>
Motivation: 长文档理解需要推理多个视觉丰富的页面，导致推理成本高且容易出现上下文遗忘（context rot）问题。

Method: 提出InSight-doc框架，从低分辨率开始逐步选择性放大高分辨率区域获取更精细证据。构建了17.9K高质量SFT示例（带区域级放大轨迹）和19.2K困难RL示例进行训练。

Result: InSight-doc-8B在文档VQA基准上比基线提高4.3-16.4个百分点；在长文档上减少超过40%幻觉；推理延迟降低41%-68%。

Conclusion: 通过SFT+RL训练范式，InSight-doc在保持准确率领先的同时显著降低了推理成本和幻觉问题。
...
### [27] [MedUP: Awakening Unified Understanding and Perception in Medical Vision-Language Models](https://arxiv.org/abs/2608.10635)
*Yuan Wang,Hualiang Wang,Yixin Chen,Songtao Jiang,Shujian Gao,Jiaming Lin,Siming Fu,Jian Wu,Zuozhu Liu*

Main category: cs.CV

TL;DR: MedUP是一种医学视觉语言模型,通过UniMedTok区域分词器将掩码编码为LLM词汇表中的离散标记,在共享标记空间中统一感知与理解,在医学图像分割、区域定位理解和VQA等任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有Med-VLMs在精确视觉感知、分割和定位方面存在挑战,现有方法要么将区域 verbalize为坐标字符串,要么依赖外部模块将感知与理解解耦,导致区域-语言对齐的表示差距。

Method: 提出UniMedTok区域分词器,将掩码编码为LLM词汇表中的离散标记,使模型能够无缝交错掩码标记与文本;构建包含184万实例的UniMed-Train语料库,涵盖文本引导分割、区域定位理解、医学VQA和CoT分割;并推出UniMed-Bench统一评估基准。

Result: MedUP在所有任务上优于原生、代理和双解码器Med-VLMs,同时与专业分割器保持竞争力。

Conclusion: 统一感知与理解建模具有强大潜力,证明了在共享标记空间中整合感知与理解的有效性。
...
### [28] [Precise Top-Layer Fabric Segmentation for Fabric Destacking with Edge- and Shape-Aware Deep Networks](https://arxiv.org/abs/2608.10648)
...
TL;DR: 本文提出了一种针对堆叠织物顶层分割的新型训练架构，通过引入边缘感知分支和形状感知分支来增强边界描绘和整体形状捕获，在真实织物数据集上取得了优于基线的效果。


<details>
  <summary>Details</summary>
Motivation: 织物分层任务需要精确分割最顶层织物，但现有语义分割和边缘检测方法难以处理织物边界不明显、层间视觉相似度高等复杂问题，影响机器人操作的性能。

Method: 提出一种多分支训练架构，在传统编码器-解码器框架基础上添加两个专门分支：边缘感知分支增强边界描绘能力，形状感知分支利用CAD模型参考掩码引导网络捕获和对齐整体织物形状。

Result: 在真实织物数据集上的实验表明，该训练方法优于现有基线方法，定量结果和消融实验验证了多分支设计的有效性。

Conclusion: 通过边缘感知和形状感知双分支的协同作用，可以有效提升堆叠织物顶层分割的精度，为机器人织物操作提供了更可靠的视觉感知基础。
...
### [29] [PolypVision: A Three-Stage Hierarchical Deep Learning Framework for Classification and Segmentation of Colorectal Polyps](https://arxiv.org/abs/2608.10649)
...
TL;DR: PolypVision是一个三阶段层级深度学习框架，用于结直肠息肉的检测、分割和分类。该系统依次执行：息肉二分类（腺瘤性vs增生性）、息肉分割及推荐切除方法、腺瘤亚型分类（管状、管状绒毛状、绒毛状）。在三个公共数据集上评估，达到约0.99的AUC和94.4%的检测mAP@50。


<details>
  <summary>Details</summary>
Motivation: 结直肠癌是全球癌症相关死亡的主要原因之一，主要由癌前息肉发展而来。准确检测、分割和内镜下及组织学分类结直肠息肉对于及时临床干预至关重要。现有方法多为单一任务设计，缺乏统一的层级分析框架。

Method: 提出三阶段层级深度学习框架PolypVision：(1)使用带Focal Loss的EfficientNetV2-M进行息肉二分类（腺瘤性vs增生性）并同时进行Paris和JNet分类；(2)使用UNet++解码器配合阶段1主干网络进行息肉分割，推荐切除方法，使用Dice和BCE损失优化；(3)使用EfficientNetV2-M配合阶段2的迁移学习进行腺瘤亚型分类（管状、管状绒毛状、绒毛状）。

Result: 在PolypGen、Kvasir-SEG和CVC-ClinicDB三个公共数据集上评估，PolypVision达到约0.99的帧分类AUC和94.4%的检测mAP@50（Kvasir-SEG），优于或匹配最先进方法。Grad-CAM确认模型关注临床相关病变特征。该框架设备无关，可在不同内镜成像系统上运行。

Conclusion: 结果表明，带有任务特定损失函数的层级迁移学习管道为自动化结直肠息肉分析提供了稳健、设备无关且具有临床意义的方法。PolypVision作为免费Web应用程序提供（DataBioX initiative）。
...
### [30] [Cross-View Sequential Visual Localization with Spatio-Temporal Context Modeling for Autonomous Driving](https://arxiv.org/abs/2608.10660)
*Jiaping Wang,Shaobo Li,Zhen Wang*

Main category: cs.CV

TL;DR: This paper proposes a temporal-context-enhanced framework for cross-view sequence visual localization for autonomous driving, which uses recurrent cross-frame module to aggregate historical context and significantly improves localization accuracy.


<details>
  <summary>Details</summary>
Motivation: Most existing cross-view visual localization methods process each frame independently, leaving temporal information underused and limiting accuracy under dynamic occlusion, illumination variation, and repetitive textures.

Method: The proposed recurrent cross-frame module aggregates historical context from the previous state to enhance the coarse ground feature of each current frame. These enhanced features facilitate satellite candidate-region classification, while hierarchical fine-grained features enable precise local offset estimation.

Result: On CVIS dataset, the proposed method reduces mean localization error from 3.80 m to 1.57 m and increases R@1 m from 8.14% to 40.22%. Direct transfer to KITTI-CVL achieves a mean error of 2.61 m, with target-domain fine-tuning further reducing the mean error to 2.27 m. Zero-shot field experiments on a real-world vehicle achieve a mean error of 2.84 m and R@5 m of 96.86%.

Conclusion: The temporal context enhancement significantly improves cross-view localization accuracy and supports robust deployment on public benchmarks and real-world roads.
...
### [31] [Chartography: A Benchmark for Professional Chart Understanding](https://arxiv.org/abs/2608.10677)
*Suhaas Garre,Chris Mutty,Sushant Mehta,Edwin Chen*

Main category: cs.CV

TL;DR: Chartography是一个包含100个任务的新基准测试，用于评估AI模型解读专业实践中图表的能力。最佳模型仅达到45.0%的平均pass@1得分，其余模型在9.0-39.5%之间。当前图表基准测试已被简单图表格式主导，推理链较短，且前沿模型已达80-90%饱和。


<details>
  <summary>Details</summary>
Motivation: 现有图表基准测试存在三个主要问题：1）以柱状图、折线图、饼图等简单格式为主；2）依赖较短的推理链；3）接近饱和，前沿模型已能获得80-90%的高分。这无法充分衡量专业人士从图表中做出重要决策的能力。

Method: 构建了Chartography基准，包含100个任务，配对来自专业实践的图表（采用标准基准很少涵盖的领域特定格式）与由实际阅读这些图表的专业人士编写的问题，并经三位额外专家独立验证。对30种前沿模型配置进行了评估（每个任务20次得分试验）。

Result: 最佳配置仅达到45.0%的平均pass@1；其余配置跨度为9.0-39.5%。失败主要集中在视觉感知方面：模型可能遗漏细微特征、误读稀疏标注轴上的值、处理投影3D几何形状不当，以及违反图表中编码的领域约定。

Conclusion: Chartography基准揭示了当前前沿模型在专业图表解读方面的显著局限性。模型在视觉感知任务上表现不佳，尤其是在处理领域特定格式、解读稀疏标注数据和理解3D几何投影方面。需要更专业的图表理解评估基准来推动该领域的发展。
...
### [32] [Bridging Severe Cross-Modal Misalignment: End-to-End Visible-Infrared Object Detection via Explicit Feature-Domain Affine Registration](https://arxiv.org/abs/2608.10680)
*Qi Ming,Yuyang Wang,Mingjing Zhao,Yifan Xiao,Zhixin Guo,Zhiqiang Zhou,Peng Sun,Juan Fang,Fuqiang Yang,Xudong Zhao*

Main category: cs.CV

TL;DR: JFRDet是一种用于可见光-红外目标检测的端到端定向目标检测器，专门针对严重跨模态几何差异场景。该方法通过跨模态仿射对齐(CMAA)模块进行图像级仿射变换估计实现多级特征对齐，通过光照引导的互补融合(IGCF)模块自适应利用不同光照条件下的模态可靠性，并采用对齐质量一致性门控(AQCG)策略稳定联合优化。在新构建的DVMA基准数据集上达到69.7% mAP50的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 可见光-红外目标检测依赖于RGB和热成像的互补信息，但跨模态空间错位问题经常导致性能下降。现有方法仅依靠隐式特征适应来处理弱对齐场景，而大规模几何差异问题仍未得到充分解决。

Method: 提出JFRDet网络，包含三个核心组件：1)跨模态仿射对齐(CMAA)模块估计图像级仿射变换进行显式多级特征对齐；2)光照引导的互补融合(IGCF)模块在不同光照条件下自适应利用模态可靠性进行跨模态融合；3)对齐质量一致性门控(AQCG)策略根据对齐可靠性和梯度一致性调节检测监督以稳定联合优化。

Result: 在自建的DVMA(DroneVehicle Misaligned)基准数据集上，JFRDet达到69.7% mAP50，刷新了该任务的SOTA性能。

Conclusion: JFRDet通过显式的跨模态仿射对齐和光照自适应的融合机制，有效解决了严重跨模态几何差异下的可见光-红外目标检测问题，为该领域提供了新的研究思路和基准数据集。
...
### [33] [Visual Geometry Foundation-Aware Gaussians for Single-Frame Surround-View Driving Reconstruction](https://arxiv.org/abs/2608.10682)
*Junhong Lin,Jinlong Wang,Xianda Guo,Yanlun Peng,Wei Zheng,Guoqing Liu,Hanli Wang,Tiesong Zhao,Wei Gao*

Main category: cs.CV

TL;DR: VGGD是一个用于前向环视驾驶重建的视觉几何基础感知3D高斯溅射框架，通过利用预训练的视觉几何先验来增强上游表示并缓解稀疏环视视图中的几何歧义。


<details>
  <summary>Details</summary>
Motivation: 单帧环视重建面临严重几何不稳定性和渲染伪影，因为相机间重叠最小。现有方法依赖复杂解码器或辅助线索，但仍然受限于上游特征的几何容量不足。

Method: 1)利用VGGT提供可迁移的多视图几何先验token；2)引入双路径Neck解耦几何一致和外观感知表示；3)应用尺度预热稳定早期几何学习并抑制自车姿态变化下的尺度漂移；4)使用混合像素-体积高斯解码器生成可渲染的3D高斯场景。

Result: 在nuScenes单帧基准测试中，VGGD在比较方法中实现了最佳的总体渲染质量，并提高了相对几何一致性。

Conclusion: 通过将几何建模转移到前端并适应基础先验到驾驶相机设置，VGGD有效提升了稀疏环视视图的几何建模能力和渲染质量。
...
TL;DR: 本文提出了一种端到端的旋转不变场景文本识别网络(RISTER)，通过在编码器中嵌入旋转等变性和在解码器中引入旋转不变性来解决多方向文本识别问题，无需显式估计文本方向即可实现鲁棒的旋转不变识别。


<details>
  <summary>Details</summary>
Motivation: 现有旋转感知方法需要显式估计文本方向，存在理论保证不足、易累积误差、计算成本高且过度依赖数据等问题。

Method: 提出(1)旋转等变局部-全局提取网络作为编码器，融合深度等变卷积与自注意力实现等变特征提取；(2)基于交叉注意力机制旋转不变性的旋转不变文本解码器。编码器捕捉旋转等变特征，解码器以旋转不变方式将视觉特征映射到输出文本。

Result: 在标准基准和旋转文本基准上均达到最先进的性能，在通用多方向数据集上准确率比第二名模型高4.0%。

Conclusion: RISTER通过理论保证的旋转不变性增强了多方向样本的鲁棒性，且不引入额外推理计算或依赖数据驱动的方向校正。
...
### [35] [MMArt A Multi-Perspective Multimodal Dataset for Visual Art Understanding](https://arxiv.org/abs/2608.10706)
*Shuai Wang,Wangyuan Ding,Yixian Shen,Jia-Hong Huang,Stevan Rudinac,Monika Kackovic,Nachoem Wijnberg,Marcel Worring*

Main category: cs.CV

TL;DR: 本文提出了MMArt数据集，包含74,234幅WikiArt画作，每幅画作标注了叙事、形式、情感和历史四种独立视角的描述及统一说明文字，旨在解决现有艺术数据集仅提供单一视角的问题，并验证了多视角注释对视觉-语言模型在艺术理解任务中的互补性和必要性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在艺术解读方面仍停留在浅层描述，难以进行形式分析、历史解读或情感表征。作者认为这不仅是模型限制，更是数据集局限——现有艺术数据集都是单一视角资源，没有同时提供叙事、形式、情感和历史四种视角的注释。

Method: 构建了MMArt大规模数据集，包含74,234幅WikiArt画作；每幅画作由专业视觉-语言模型或人工标注四种独立视角的描述（叙事、形式、情感、历史）以及一个统一说明文字；通过互补性质量评估进行验证；进行了两类分析：（1）生成分析：检验各视角描述在图像重建中对风格和情感信号的保留程度；（2）判别检索分析：评估各视角描述在图像检索任务中的表现；（3）留一分析：验证各视角的不可替代性。

Result: 互补性分析证实四种视角编码了真正不同的信息。生成分析显示：形式分析描述最能保留构图风格，历史描述在重建图像中携带强烈情感信号。判别检索分析揭示任务不对称性：叙事描述驱动检索（R@1=44.0%），形式描述虽在重建任务中最强但在检索任务中几乎无区分度（R@1=7.8%）。留一分析确认历史描述在两项任务中都是最难替代的视角。

Conclusion: 没有任何单一视角能够满足所有任务需求，这直接证明了MMArt多视角设计的必要性和合理性。该数据集为推动视觉-语言模型在艺术深度理解领域的发展提供了重要资源。
...
### [36] [Self-Geometry: GT-Free and Plug-and-Play Test-Time Adaptation for Geometrically Consistent 3D Vision Foundation Models](https://arxiv.org/abs/2608.10708)
*Seokhyun Youn,Dahyeon Kye,Sung-Ho Bae,Jihyong Oh*

Main category: cs.CV

TL;DR: 本文提出Self-Geometry，一个无需训练的视觉基础模型(VFM)测试时适应框架，通过2D像素对应关系作为伪标签，直接施加显式多视角几何约束，解决VFM预测中缺乏几何一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有VFM在单次前向传播中预测深度、相机姿态和点图，但缺乏显式多视角几何一致性约束。虽然已有方法利用模型输出的隐式自一致性，但提升有限，特别是在预训练VFM本身精度较低的场景中。

Method: 提出三部分组成的测试时适应pipeline：1) 几何解耦优化，结合多视角一致性和极线一致性损失与梯度解耦以防止梯度冲突；2) 基于SO(3)测地距离的帧角邻域采样器；3) 通过LoRA实现轻量级测试时适应。

Result: 在6个VFM（VGGT、π³、DA3-Giant/Large/Base/Small）和4个基准数据集（7Scenes、ETH3D、ScanNet++、HiRoom）上，姿态和几何估计均获得一致提升。

Conclusion: Self-Geometry作为即插即用的测试时适应方法，能够有效利用显式几何约束提升VFM的多视角一致性，且适用于多种模型和场景。
...
### [37] [Compact Feed-Forward 3D Gaussians via Saliency-Guided Primitive Merging](https://arxiv.org/abs/2608.10712)
*Tim-Felix Fassch,Jochen Kall,Cyrill Stachniss*

Main category: cs.CV

TL;DR: 本文提出了一种结构感知的合并管道，将来自前馈方法的每像素原语整合为紧凑的内容自适应高斯集，在仅保留1/20高斯数量的情况下大幅保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 前馈3D高斯溅射方法虽然重建速度快，但通常产生每像素原语，导致高度冗余且效率低下的表示。

Method: 通过自适应超像素分割将空间连贯、相似外观的高斯分组为可变大小聚类，由显著图引导为纹理区域分配精细分割、为均匀区域分配粗糙分割；然后通过学习编码器将每个聚类压缩为紧凑潜在表示，再通过学习合并器基于几何重叠和特征相似性跨视图匹配和整合表示；最后通过细节层次解码器产生可控分辨率的最终高斯。

Result: 在仅使用每像素方法1/20数量高斯的情况下，能够largely保持视觉质量，提供高度紧凑且可高效渲染的表示。

Conclusion: 作为后处理模块，该管道与主干网络无关，能够利用现有前馈方法的优势，实现比以往减少原语数量方法更好的质量，同时提供高度紧凑的表示。
...
### [38] [Grid-Preserving Knowledge Distillation: Transferring Convolutional Inductive Bias to Vision Transformers under Data Scarcity](https://arxiv.org/abs/2608.10723)
*Junyong Choi,Cheolhyeon Park,Jaehoon Cho*

Main category: cs.CV

TL;DR: iBKD是一种针对ViT设计的蒸馏框架，在训练数据稀缺场景下优于CNN教师蒸馏和通用知识蒸馏方法，通过保留空间网格结构来传递归纳偏置。


<details>
  <summary>Details</summary>
Motivation: ViT在训练数据稀缺时表现不如CNN，从CNN教师蒸馏归纳偏置是一种有效方法，但通用特征蒸馏会丢失空间网格结构，无法为ViT重建局部性和平移等变性。

Method: 提出Inductive Bias Attention Module (IBAM)：1)用学习权重将每层学生聚合到教师网格；2)用通道和可变形空间注意力强化结构线索；3)通过卷积跨注意力在网格间而非token间传递信息。模块仅用于训练，部署时为 unmodified ViT。

Result: 在7个Transformer backbone和6个数据稀缺基准上，iBKD优于局部性引导方法和通用知识蒸馏基线，且随着训练数据减少，优势增大。

Conclusion: iBKD能有效将CNN的归纳偏置传递给ViT，在数据稀缺场景下显著提升ViT性能，且不增加推理开销。
...
### [39] [InterPruner: Interactive Structured Pruning via Taylor-Implicit Criterion and Language-Prior Modulator for Multimodal Object Detection](https://arxiv.org/abs/2608.10724)
*Qi Ming,Zihan Yang,Shaoguang Huang,Si Sun,Hanqing Zhang,Nanqing Liu,Jiahui Lv,Juan Fang,Aleksandra Pizurica*

Main category: cs.CV

TL;DR: 本文提出InterPruner，首个用于RGB-红外目标检测的交互式结构化通道剪枝框架，通过Taylor隐式准则、模态交互冗余分析器和场景先验通道锚点三个组件，在减少50%通道的情况下仍能保持甚至提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-红外目标检测方法虽然能提供丰富的多模态信息，但存在严重的通道冗余和计算开销。现有的剪枝方法专为单模态骨干网络设计，忽略了跨模态交互和动态场景冗余问题。

Method: 提出三个核心组件：(1)Taylor隐式准则(TIC)通过高阶泰勒展开和隐函数定理量化通道重要性；(2)模态交互冗余分析器(MIRA)通过互补偿性评估识别冗余通道；(3)场景先验通道锚点(SPCA)利用语言先验作为语义锚点测量通道-场景相关性，实现动态通道重要性估计。

Result: 在RGB-红外目标检测数据集上的广泛实验表明，InterPruner在保持高性能的同时几乎没有性能下降。特别是在FLIR数据集上，剪枝50%通道后反而实现了0.6%的mAP提升。

Conclusion: InterPruner是首个针对RGB-红外检测的跨模态通道剪枝方法，有效解决了多模态检测中的通道冗余问题，为高效多模态目标检测提供了新思路。
...
### [40] [Rethinking LLM Verification: Evidence Structure, Uncertainty, and Selective Refinement](https://arxiv.org/abs/2608.10725)
...
TL;DR: 本文提出一种两阶段框架，通过在模型放弃回答时进行针对性本体 grounding 来解决医学假设验证中的覆盖准确性权衡问题，在 MedReason 和 MedQA 数据集上显著提升了准确率，并将放弃回答重新用作选择性推理细化的控制信号。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学应用中常依赖捷径推理而非系统性推理，存在安全隐患；允许模型在不确定时放弃回答虽能提高可靠性，但引入了覆盖准确性权衡问题。

Method: 提出两阶段框架：第一阶段让模型进行多选题回答并在不确定时放弃；第二阶段仅针对放弃的样本进行针对性本体 grounding（本体锚定），结合外部知识库进行推理验证。

Result: 在 GPT-5.5 和 DeepSeek-R1 两个前沿模型上，问题级准确率提升 9.6 个百分点（82.9%→92.5%），假设级准确率提升 4.2 个百分点（92.0%→96.2%）。

Conclusion: 放弃回答并非随机而是反映真实不确定性，可作为选择性推理细化的控制信号，实现无需显式构建知识图谱即可达到知识图谱级性能的效果。
...
### [41] [Beyond Pixels: From Video Priors to 4D Worlds](https://arxiv.org/abs/2608.10744)
*Zihao Liu,Xiaolong Shen,Zhenglin Zhou,Ruijie Quan,Yi Yang*

Main category: cs.CV

TL;DR: 本文提出Latent-to-4D方法，利用视频扩散模型的潜在表示直接生成4D动态3D场景，通过将视频潜在与4D解码器的token grid对齐并经由帧级和全局时空注意力细化，实现跨不同视频扩散变换器的可迁移4D预测。


<details>
  <summary>Details</summary>
Motivation: 现有4D生成方法存在局限性：基于RGB视频重建的方法存在分布不匹配和误差传播问题；而将特定视频生成器适配为直接预测几何的方法则与生成器绑定，更换生成器时需重新训练。本文探索利用共享VAE的视频模型的最终去噪潜在作为4D预测的可复用接口。

Method: 提出Latent-to-4D方法，绕过RGB输出，将视频潜在与预训练4D解码器的token grid对齐，通过帧级和全局时空注意力进行细化。模型在约1000个现有重建片段上训练，单一检查点可在同一VAE家族内的多个视频扩散变换器之间无变化迁移。

Result: 在Text4D-200和I4D-200数据集上，Latent-to-4D在基于投影的DINO-F1指标上分别超越Wan+4RC级联2.88-3.45和5.81个百分点，同时在几何、时间稳定性和整体质量方面更受人类评分者青睐。

Conclusion: 通过利用视频扩散模型的潜在表示作为4D预测的可复用接口，Latent-to-4D成功实现了跨不同视频生成器的通用4D几何预测，表明视频潜在表示可以作为连接2D生成与4D重建的有效桥梁。
...
### [42] [Where To Look? : Causal Tracing of Vision Encoders in VLM](https://arxiv.org/abs/2608.10758)
*Naren Kumar S,Tirth Bhatt,Mayank Singh*

Main category: cs.CV

TL;DR: 本文通过因果追踪技术研究视觉语言模型中驱动答案的视觉信息位置，发现高度因果的视觉token往往存在于目标区域之外。.


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型能准确描述图像，但一个更根本的问题仍未得到解答：究竟是什么视觉信息驱动了它们的回答？

Method: 采用因果追踪（causal tracing）方法分析视觉token的因果效应，并扩展到多个大型视觉语言模型和不同的损坏设置进行验证。

Result: 高度因果的视觉token经常存在于目标区域之外，这种模式在不同模型和损坏设置中普遍存在；模型利用视觉线索来理解视觉结构，当外观线索被移除时，模型难以保持视觉结构。

Conclusion: 现代视觉语言模型在"看"、"使用"和"推理"视觉结构之间存在差距，因果追踪框架为研究视觉信息如何被转换、保存和使用提供了有效工具。
...
### [43] [FADE: From Passive Verification to Active Discovery in Counterfactual Video Understanding](https://arxiv.org/abs/2608.10764)
*Fufangchen Zhao,Jinhu Fu,Jiachen Lei,Jiahong Wu,Xiangxiang Chu,Danfeng Yan*

Main category: cs.CV

TL;DR: FADE是一个针对反事实视频理解的训练框架，解决现有基准测试中存在的数据泄漏问题。该方法采用证据优先的两阶段训练范式：先通过证据内部化的监督微调使模型基于关键视觉异常进行预测，再通过衰减锚点强化学习逐步移除文本引导，迫使模型独立发现和解释证据。


<details>
  <summary>Details</summary>
Motivation: 现有反事实视频理解的多选题基准测试存在数据泄漏问题——问题和候选选项会泄露目标事件，使得核心挑战从主动发现变成了文本引导的验证。这降低了评估的有效性。

Method: 提出FADE框架，包含两个阶段：1) 证据内部化监督微调(evidence-internalized SFT)，使模型预测基于决定性视觉异常；2) 衰减锚点强化学习(fading-anchor RL)，逐步移除文本指导，强制模型独立发现和解释证据。同时提出有效的管道将现有MCQ数据集转换为对齐的MCQ、开放问答和字幕任务。

Result: 以Qwen3-VL-8B为基线，FADE在DualityVidQA-test和IPVV-Bench上所有三个任务中达到最先进的严格配对分数，超越GPT-5.6。从受限MCQ到不受限OQA和字幕时，性能保留率分别为90.4%和67.4%，显著高于GPT-5.6的48.1%和30.7%。

Conclusion: FADE框架能有效提升模型在不受约束的反事实视频理解中的表现，为未来研究提供了坚实的基线。该方法成功解决了基准测试中的数据泄漏问题，并展示了从受限到不受约束设置的强鲁棒性。
...
### [44] [MVTrack: Ultrafast Appearance-Free Moving Object Tracking from Compressed Bitstreams](https://arxiv.org/abs/2608.10790)
...
### [45] [E$^3$mo-Bench: A Scalable Benchmark for Multimodal Evoked and Expressed Emotion Understanding via Bayesian Pairwise Alignment](https://arxiv.org/abs/2608.10796)
*Lancheng Gao,Ziheng Jia,Shengyan Li,Zixuan Xing,Jiarui Wang,Huiyu Duan,Xiongkuo Min*

Main category: cs.CV

TL;DR: 本文提出了E³mo-Bench基准，用于评估多模态大语言模型(MLLMs)的情感理解能力。该基准包含12,314个问答对，涵盖2,524个视频，通过3个任务评估情感感知、开放词汇识别和valence-arousal-dominance(VAD)维度评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常孤立评估表达情感或唤起情感，且仅限于粗粒度或不完整的情感表征。论文旨在填补这一空白，构建一个能够同时评估MLLMs对表达情感和唤起情感理解能力的综合基准。

Method: 1) 提出贝叶斯成对对齐(Bayesian Pairwise Alignment)方法，将稀疏的低负担成对判断聚合为锚定参考的VAD估计值；2) 开发E³mo-Score，一种无训练的智能体，通过五模型委员会的互补判断聚合来改进VAD估计。

Result: 实验验证了框架的有效性，并揭示了唤起情感和表达情感范式之间存在显著的性能差异。MLLMs在细粒度识别和维度评估方面存在持续性缺陷。

Conclusion: 该研究为推进多模态情感智能提供了明确方向：需要重点提升模型对细粒度情感识别和VAD维度评估的能力，特别是改善唤起情感与表达情感之间的性能平衡。
...
### [46] [Beyond Fixed Luminance: Towards Panchromatic and Orthochromatic Image Colorization](https://arxiv.org/abs/2608.10798)
*Swarnim Maheshwari,Syed Imam Ali,Vineeth N. Balasubramanian*

Main category: cs.CV

TL;DR: 本文提出一种不依赖固定亮度通道的图像着色框架，使用基础图像编辑模型实现全RGB图像着色，并通过混合灰度目标同时训练标准亮度灰度和红不敏感灰度条件，以增强对正色摄影图像的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统Lab空间着色方法依赖输入亮度通道L，当灰度图像形成偏离自然图像亮度（如历史正色摄影）时效果不佳，需要一种不假设特定亮度模式的着色方法。

Method: 1. 提出基于基础图像编辑模型的RGB全图像着色框架；2. 设计混合灰度目标，在标准亮度灰度和红不敏感灰度两种条件下联合训练。

Result: 在COCO、ImageNet和自定义多实例基准上验证，标准灰度输入表现竞争力，正色摄影输入显著更鲁棒，定性比较和人类研究表明可见色伪影更少。

Conclusion: 该方法在不牺牲标准灰度着色性能的前提下，成功解决了历史正色摄影图像的着色难题，为老照片修复提供了有效解决方案。
...
### [47] [Evaluating Semantic and Spatial Guidance for Foundation Model Segmentation of Small-Scale PV in Remote Sensing Imagery](https://arxiv.org/abs/2608.10801)
*Roni Blushtein-Livnon,Tal Svoray,Osher Rafaeli,Michael Dorman,Itay Fischhendler,Havazelet Yahel,Emir Galilee*

Main category: cs.CV

TL;DR: 研究评估了SAM3模型在遥感影像中分割小型光伏系统的性能，系统比较了文本提示、几何提示和混合提示在不同监督级别、训练策略、空间分辨率和成像条件下的表现，发现混合提示策略最优且具有强数据效率。


<details>
  <summary>Details</summary>
Motivation: 偏远无电地区的光伏空间数据对理解光伏采用过程至关重要，但这类数据严重缺乏。自动分割遥感影像是解决方案，但住宅光伏系统因尺寸小、分布稀疏导致目标-背景严重不平衡，传统方法效果不佳。视觉-语言基础模型通过提示提供了数据高效的新范式，但不同提示类型的相对贡献尚不明确。

Method: 使用SAM3模型，系统评估文本提示、几何提示和混合提示三种策略，在不同监督级别、训练策略、空间分辨率和成像条件下分割遥感影像中的小型光伏系统。研究采用一个大型偏远农村地区的多时相航空影像作为研究地点，并在三个额外数据集上验证结论。

Result: 提示策略是决定模型行为的主导因素。文本提示性能最低且对监督水平和成像条件最敏感；几何提示显著提高分割精度和鲁棒性；混合提示达到最高精度和稳定性。只需几百个标注样本即可获得大部分性能提升，展现出强数据效率。迁移学习整体影响有限，仅在有限监督下对文本提示有轻微改善。

Conclusion: 提示策略是SAM3适应能力、鲁棒性和泛化性的关键决定因素，突出了提示基础模型在数据受限的无电地区进行规模化光伏制图的潜力。
...
### [48] [BPG: Balancing Plasticity and Generalization for Domain Incremental Learning](https://arxiv.org/abs/2608.10804)
*Qiang Wang,Songlin Dong,Shaokun Wang,Jizhou Han,Xiang Song,Chenhao Ding,Yuhang He,Yihong Gong*

Main category: cs.CV

TL;DR: BPG是一个统一的域增量学习框架，通过BPG-Adapter动态确定适配器隐藏维度，以及BPG-Inference软域混合策略，在DomainNet、CDDB和CORe50数据集上实现SOTA性能，遗忘率低至0.22%。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在域迁移场景下性能显著下降，现有的参数隔离方法采用"一刀切"策略导致学习容量不足或参数冗余问题。

Method: 提出BPG框架，包含两个组件：1) BPG-Adapter根据域特定特征可分离性动态确定适配器隐藏维度；2) BPG-Inference采用软域混合策略在测试时整合多个域特定模型，解决域ID误选问题。

Result: 在DomainNet、CDDB和CORe50数据集上一致优于均匀适配器方法和硬域选择策略，达到SOTA平均精度，DomainNet遗忘率低至0.22%。

Conclusion: BPG有效解决了域增量学习中学习容量不足和参数冗余的双重挑战，是一个简单有效的统一框架。
...
### [49] [Fast and Memory-Efficient Wavelet Convolutions via I/O-Aware Reformulation](https://arxiv.org/abs/2608.10805)
*Amit Aflalo,Shahaf E. Finder,Roy Amoyal,Eran Treister,Oren Freifeld*

Main category: cs.CV

TL;DR: 论文提出了一种针对Wavelet卷积(WTConv)的I/O感知优化方法。WTConv作为一种新兴的卷积替代方案，可以指数级扩展网络感受野，但参考实现因高带宽内存(HBM)数据移动过多而严重受限于内存带宽。该工作通过三种代数重构方法大幅减少HBM流量，实现高达4.35倍的训练加速。


<details>
  <summary>Details</summary>
Motivation: WTConv作为标准卷积的替代品，能够通过分解层级数指数级扩展网络感受野，同时保持参数数量的线性增长。然而，其参考实现存在严重的内存瓶颈问题，需要通过高带宽内存进行大量数据移动，这限制了其在实际应用中的效率。

Method: 首先开发了WTConv的I/O模型来表征内存瓶颈；然后提出三种代数重构：(1)在芯片上重新计算廉价的Haar分析蝴蝶结构；(2)将多级综合级联折叠成单个由输出坐标位索引的闭式传递；(3)将学习的每通道缩放因子折叠到卷积权重中。这些优化实现了I/O感知的融合实现，大幅减少HBM流量。

Result: 在多种分解层级和广泛的张量形状下进行评估， reformulation将建模的HBM流量减少约2.55倍，实现了最高4.35倍的训练加速，同时将峰值内存使用量减少约一半。

Conclusion: 该优化方法在保留WTConv优势的同时，显著降低了执行时间和内存占用，消除了以往限制其实际效率的系统开销，使WTConv能够在实际应用中发挥其理论优势。
...
### [50] [Modelling Geographic Atrophy Progression using Implicit Neural Representations](https://arxiv.org/abs/2608.10807)
*Simone Sarrocco,Paul Friedrich,Florentin Bieder,Christina Bornberg,Philippe Valmaggia,Peter Maloca,Philippe Cattin*

Main category: cs.CV

TL;DR: 本研究提出利用隐式神经表示(INRs)在低数据环境下对个体水平的地理萎缩(GA)进展进行建模，可同时生成FAF图像和GA分割区域，涵盖过去和未来时间点。


<details>
  <summary>Details</summary>
Motivation: 年龄相关性黄斑变性(AMD)是西方世界主要致盲原因，其晚期干性阶段表现为地理萎缩(GA)。由于GA进展高度个体化，晚期AMD的演变规律目前仍不明确，需要新的建模方法。

Method: 提出基于隐式神经表示(INRs)的方法，在低数据条件下对个体GA进展进行建模。该方法可同时生成FAF图像和GA分割区域，涵盖过去和未来时间点。

Result: 与对比模型相比，本方法在多种场景下达到竞争力的分割质量，实现了GA病变面积的最低平均绝对误差(MAE)和最高DICE分数，同时保持了FAF图像质量。

Conclusion: INRs可有效用于个体水平的GA进展建模，在低数据场景下仍能准确预测病变发展，为理解AMD疾病进程提供了新工具。
...
### [51] [MIRA: Medical Image Reflection for Agentic Diagnosis](https://arxiv.org/abs/2608.10827)
...
TL;DR: MIRA是一个医学影像诊断代理框架，通过动态调用图像处理操作（如缩放、定位、旋转、测量）和网络搜索来进行自主证据搜索和反思性验证。该框架解决的核心问题是：工具的 indiscriminate 使用可能引入噪声或误导性证据，因此需要验证工具操作的必要性及结果证据与假设的一致性。通过两阶段训练策略（工具增强的蒙特卡洛树搜索数据引擎 + 带在线反思原则演化的强化学习），MIRA在9个医学视觉推理基准上平均得分64.73，比Qwen3-VL-8B基线提升7.44分，有用工具使用判断从56.2%提升至73.8%，有害判断从8.9%降至1.6%。


<details>
  <summary>Details</summary>
Motivation: 医学视觉智能体可以使用工具检查图像和检索外部知识，但不加区分地使用工具可能引入噪声或误导性证据。可靠的诊断不仅需要获取额外观察，还需要验证工具操作是否必要，以及结果证据是否支持当前假设。

Method: 采用两阶段训练策略：第一阶段使用工具增强的蒙特卡洛树搜索（MCTS）数据引擎探索多样化的诊断假设，并联合验证视觉定位准确性和语义一致性，构建监督微调轨迹；第二阶段通过强化学习进一步改进决策，引入在线反思原则演化——将失败案例提炼为候选原则，仅保留能够提高留置 rollout 奖励的原则。

Result: 在9个医学视觉推理基准上，MIRA平均得分64.73，比其Qwen3-VL-8B基线提升7.44个百分点。有用工具使用判断从56.2%提升至73.8%，有害判断从8.9%降至1.6%。

Conclusion: MIRA能够重新检查证据、纠正过早结论并调整工具使用策略，展示了反思性验证在医学诊断中的重要价值。该框架通过动态调用图像处理操作和网络搜索，同时评估获取证据的相关性和一致性，实现了更可靠、更精准的医学影像诊断。
...
### [52] [UniProbe: A Learnable Token-Level Hallucination Detector for Large VLMs using Multi-Structural Internal Representations](https://arxiv.org/abs/2608.10835)
*Dvir Samuel,Guy Bar-Shalom,Fabrizio Frasca,Ethan Fetaya,Yftah Ziser,Gal Chechik,Haggai Maron*

Main category: cs.CV

TL;DR: UniProbe是一个轻量级、可学习的检测器，通过单一前向传播建模冻结LVLM的异构计算痕迹来检测幻觉。它构建了一个包含图像补丁、查询token和生成token的有向图，并使用交替的结构感知模块（GNN、ViT、GRU）来处理空间、关系和序列证据。该方法还支持流式解码中的幻觉感知采样和自适配策略，在多种LVLM backbone上实现了SOTA的token级和目标幻觉检测。


<details>
  <summary>Details</summary>
Motivation: LVLMs经常产生与视觉输入不符的幻觉内容，现有检测器存在以下问题：1)需要昂贵的全模型微调；2)依赖外部验证器忽视模型生成过程；3)将内部信号简化为孤立特征和手工统计，丢失空间、序列和关系结构。因此需要一种轻量级、统一的方法来在token级别定位幻觉。

Method: UniProbe构建一个包含图像补丁、查询token和生成token的有向图（边由注意力权重编码），然后交替使用三个结构感知模块：1)GNN处理关系证据；2)ViT处理2D视觉几何；3)GRU处理响应顺序。这三种模块使空间、关系和序列证据能够在检测器中交互。还开发了流式变体用于幻觉感知解码，以及自适配策略将检测器与LVLM自身生成对齐。

Result: 在多种LVLM backbone上，UniProbe实现了SOTA的token级和目标幻觉检测。在解码过程中，它将目标幻觉减少高达55%，同时延迟仅为标准生成的1.06倍。

Conclusion: UniProbe通过建模LVLM的异构计算痕迹，成功实现了轻量级、高效的幻觉检测，无需对LVLM进行微调即可有效定位和干预幻觉token，为多模态大模型的可靠性提升提供了新思路。
...
### [53] [PolyLayout: Hierarchical VLM-Guided Layout Generation Beyond Rectangular Rooms](https://arxiv.org/abs/2608.10838)
*Yutong Jiang,Zahra Atashgahi,Carlos Soto Garcia Delgado,Ruben Brokkelkamp,Davide Zanutto,Efşan Sökmen,Shahin Shahkarami*

Main category: cs.CV

TL;DR: 本文提出了一种面向家居零售的混合层次化框架，用于在真实家庭环境中生成物理上合理的3D房间布局。该系统将生成过程解耦为三个阶段：功能家具聚类与细粒度区域内部署、基于视觉语言模型的宏观路由、以及基于规则的碰撞自由微排列优化。


<details>
  <summary>Details</summary>
Motivation: 弥合学术研究与实际应用之间的差距。现有的解决方案主要关注家具放置的算法策略，忽视了真实家庭中普遍存在的非矩形几何形状和严格的门窗约束。

Method: 提出混合层次化框架，包含三个阶段：(1)功能家具聚类和细粒度区域内放置；(2)基于视觉语言模型(VLM)的宏观路由，将聚类区域和独立家具锚定在多样化多边形边界内；(3)基于规则的优化，实现尊重建筑约束的无碰撞微排列。

Result: 在生产规模目录和代表性不规则真实世界拓扑上评估表明，该方法达到了最高感知 plausibility，同时保持了良好的几何合规性，延迟较低，并扩展到现有方法原生不支持的不规则边界。

Conclusion: 该方法在零售场景中表现优异，能够有效处理真实家庭环境中的非矩形房间布局问题，具有实际应用价值。
...
### [54] [The GENEA Challenge 2026: A Large-Scale Disentangled Evaluation of Speech-Driven Gesture Generation on the Seamless Interaction Dataset](https://arxiv.org/abs/2608.10839)
*Rajmund Nagy,Silvia Arellano García,Hendric Voss,Mihail Tsakov,Taras Kucherenko,Youngwoo Yoon,Gustav Eje Henter*

Main category: cs.CV

TL;DR: 第四次GENEA挑战赛评估了五个语音驱动的手势生成系统，这些系统在对话数据集上训练。评估采用了分离式评估方法，分别评估运动质量和语音对齐效果。研究发现，数据集的过滤片段在运动真实感上显著优于所有提交作品（68-95%成对胜率），而最佳提交在语音对齐上仅达到32%（对比动捕的62%天花板）。在对话交互研究中，没有提交作品能够有效响应对话者。语义手势生成方面，最佳提交仅达到8%的适当性评分，远低于数据集的79%。


<details>
  <summary>Details</summary>
Motivation: 推动语音驱动手势生成技术的发展，通过大规模人类评估来系统性地比较不同系统的性能，并识别当前技术的局限性

Method: 采用分离式评估方法分别评估运动质量和语音对齐；进行对话不匹配研究以隔离倾听和反应效果；引入语义手势生成任务和基于文本不匹配的评估方法；共进行了四项大规模用户研究

Result: 运动真实感：数据集过滤片段显著优于所有提交（68-95%胜率）；语音对齐：动捕62%为概念天花板，最佳提交32%；对话适当性：动捕65%天花板，无提交超越随机 chance；语义表达：数据集79%，最佳提交仅8%

Conclusion: 当前手势生成系统仍无法产生语义表达的手势，也无法有效响应对话者；数据集的过滤片段在运动质量上远超所有提交系统，表明现有技术仍有很大改进空间
...
### [55] [Multi-View Relational Distillation for Spatial Reasoning with Vision-Language Models](https://arxiv.org/abs/2608.10864)
*Kiet T. Nguyen,Hanbo Shim,Jinwoo Kim,Seunghoon Hong*

Main category: cs.CV

TL;DR: 论文提出多视角关系蒸馏(MVRD)方法，通过蒸馏视图间的patch级余弦相似度而非教师模型特征本身，来提升视觉-语言模型的视觉-空间推理能力，同时保持预训练的语言对齐。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型的视觉-空间表示在几何上脆弱，导致在具身AI、机器人和自动驾驶所需的空间推理任务上失败。现有方法如微调或特征融合存在学习虚假视觉表示或增加模型参数量的问题。

Method: 提出多视角关系蒸馏(MVRD)，蒸馏视图间的patch级余弦相似度而非教师模型特征本身。这些关系编码了空间理解所需的几何对应关系，同时保持学生表示的不确定性，使其接近预训练的视觉-语言空间。

Result: 在代表性的VLM上，MVRD提升了视觉-空间推理性能，优于监督微调和特征蒸馏方法，接近特征融合方法的性能但参数量和延迟更低。实验表明MVRD使视觉表示更具几何性，同时保留语言对齐，并泛化到3D场景理解任务如物体定位、密集描述和问答。

Conclusion: MVRD通过关系蒸馏有效提升了VLM的空间推理能力，在保持语言对齐的同时以轻量级方式接近特征融合方法的性能，为具身AI等应用提供了有价值的改进。
...
### [56] [NullEdit: Stealthy Image Protection via VLM Condition Redirection](https://arxiv.org/abs/2608.10870)
*Weiyao Huang,Liqin Wang,Ziqi Sheng,Wei Lu*

Main category: cs.CV

TL;DR: NullEdit是一种针对视觉语言模型(VLM)与扩散Transformer结合的图像编辑系统的防御方法，通过操纵VLM表示来抑制恶意编辑请求，同时保持输出自然且不产生明显伪影。


<details>
  <summary>Details</summary>
Motivation: 现有推理时防御存在两个问题：1)通过明显伪影阻止编辑，但会暴露保护意图；2)允许编辑但导致身份或内容漂移，无法真正阻止编辑行为。需要一种隐蔽且无害的"不作为"防御机制。

Method: 利用正常编辑和未编辑的锚点来重定向VLM表示（该表示由参考图像和指令共同形成，再用于条件化下游DiT主干），并通过跨提示梯度平均将保护迁移到未见过的指令。

Result: 在Step1X-Edit和Qwen-Image-Edit上，针对CelebA-HQ和VGGFace2数据集，NullEdit相比SOTA基线平均降低EditReward IF分数0.813，同时保持主体身份和源内容。

Conclusion: NullEdit能够在不产生明显伪影或身份替换的情况下有效抑制恶意编辑，为图像编辑系统提供了一种隐蔽且有效的保护机制。
...
### [57] [ConfTriage: A Calibration-Aware LLM Triage Framework for Pulmonary Nodule Malignancy with Selective Specialist Deferral](https://arxiv.org/abs/2608.10885)
*Md Rabiul Islam,Samir Abdaljalil,Erchin Serpedin,Hasan Kurban*

Main category: cs.CV

TL;DR: 研究提出ConfTriage方法，利用大语言模型(LLM)读取肺结节的标准自然语言描述作为预测依据，结合校准机制和选择性专家DL模型作为后盾，实现肺结节恶性程度的零样本预测。在LIDC-IDRI数据集上，该方法达到F1 88.22%、AUC 0.92，可独立解决76.5%的病例，仅将不确定病例转交给专家DL模型。


<details>
  <summary>Details</summary>
Motivation: 当前肺结节恶性预测依赖需要大量标注数据和任务特定训练的专家深度学习模型，研究旨在探索通用大语言模型仅通过读取自然语言描述就能进行校准分类的可行性，建立一个无需专家模型介入即可独立决策的分流系统。

Method: 提出ConfTriage方法，包含三个核心要素：1)以语言为模态，直接读取结节的标准自然语言描述；2)以校准为安全机制，确保概率输出的可靠性；3)以选择性专家DL模型作为低置信度病例的后盾。对五个前沿LLM进行了七种输入消融实验，验证自然语言描述的诊断信息量。

Result: ConfTriage在LIDC-IDRI数据集上达到F1得分88.22%、AUC 0.92，可独立解决76.5%的病例。消融实验表明自然语言描述主导诊断信号，而低层次图像统计信息基本无诊断价值。提供有限样本联合误差界和超越贝叶斯最优延迟分类器的oracle不等式理论保证。

Conclusion: 结构化放射学描述可被校准LLM有效利用进行选择性转诊，证明了通过自然语言捕获临床有意义诊断信息的可行性。该框架为在医学决策支持系统中结合通用LLM预测与专家AI模型提供了实用路径。
...
### [58] [GESTO: Human-Centric Spatio-Temporal Memory for Reasoning in Dynamic Scenes](https://arxiv.org/abs/2608.10886)
...
TL;DR: GESTO是一种时空记忆系统，将持久的4D场景图与人类-物体交互的两级层次结构相结合，用于在动态人类环境中进行活动为中心的时空推理。该系统从RGB-D观测流中自动提取时间戳交互，将其 grounding 到持久的场景实体，分组为事件，并使用事件上下文来细化不确定的物体关联。


<details>
  <summary>Details</summary>
Motivation: 现有4D场景图保留了物体和位置历史但忽略了活动结构，而活动表示要么未建立在持久3D场景中，要么依赖外部提供的事件边界和物体关联。机器人需要在人类环境中运行的记忆，不仅要捕获存在的物体和位置，还要捕获人们如何使用它们以及个体交互如何组合成目标导向的活动。

Method: GESTO将持久的4D场景图与两级层次结构（原子人类-物体交互和目标驱动事件）耦合。从RGB-D观测流中，GESTO自动提取时间戳交互，将其ground到持久场景实体，分组为事件，并使用事件上下文来细化不确定的物体关联。关系感知的工具调用代理查询结果记忆进行以活动为中心的时空推理。

Result: 在现有基准的可重现文本、二进制和时间类别上，GESTO分别达到0.71、0.75和0.70的成绩，接近于提供真实事件和物体grounding的方法，同时在移除这些输入时显著优于相同推理框架。在Space2Event和Event2Space查询上分别达到0.73和0.75。消融实验表明层次事件结构和上下文感知grounding细化提供了互补优势。

Conclusion: 层次事件结构和上下文感知grounding细化提供了互补优势，支持在动态人类环境中进行回顾推理的活动grounded层次记忆。该方法在活动为中心的时空推理任务上表现优异，接近于使用完整真实标注的方法，同时显著优于缺少关键输入的基线。
...
### [59] [Sensor-Informed Per-Point Covariance for Structured-Light 3D Imaging](https://arxiv.org/abs/2608.10888)
*Sehoon Tak,Jae-Sang Hyun*

Main category: cs.CV

TL;DR: 本文提出了一种传感器信息引导的一阶方法，用于在条纹投影轮廓术（FPP）中构建逐点3×3协方差场。该方法通过实验测量的相位精度和校准的相位-深度及相位-3D映射来构造协方差场，将秩-1相位诱导协方差与结合拟合的横向图像空间扰动尺度的有效满秩completion分离开来。实验表明该方法在G-ICP配准中显著优于常 isotropic 模型。


<details>
  <summary>Details</summary>
Motivation: 当前点云协方差通常建模为 isotropic 常数或从局部表面几何推断，无法显式反映测量过程。在FPP中，相位噪声通过校准重建传播，产生强各向异性3D不确定性，现有方法无法准确表征这种特性。

Method: 提出传感器信息引导的一阶方法：（1）利用实验测量的相位精度；（2）使用校准的相位-深度和相位-3D映射；（3）将秩-1相位诱导协方差与横向图像空间扰动尺度的满秩completion分离。通过重复平面实验验证协方差方向与视线的对齐，以及主相位不确定性与标量深度不确定性的一致性。

Result: 在固定成像条件下，重复平面实验显示：主导协方差方向与视线紧密对齐；主导相位诱导不确定性与标量深度不确定性保持一致。在G-ICP配准中，提出的协方差模型显著优于常 isotropic 模型，提供了与传统几何协方差互补的传感器衍生不确定性表示。

Conclusion: 所提出的传感器信息协方差场方法能够准确表征FPP中相位噪声传播产生的各向异性不确定性，为概率配准、融合和质量评估提供了更精确的不确定性建模，在实际应用中具有重要价值。
...
### [60] [VIDS-Seg: Towards Reliable Uncertainty Quantification in Pediatric Cardiac Ultrasound Segmentation](https://arxiv.org/abs/2608.10903)
*Paul Fischer,Ece Ozkan*

Main category: cs.CV

TL;DR: 提出VIDS-Seg方法，通过在轻量级预测头上进行摊销变分推理，实现分布外（OOD）感知的密集图像分割 Uncertainty Quantification，用于在成人数据训练的模型上零样本部署到儿科数据时检测潜在失败。


<details>
  <summary>Details</summary>
Motivation: 临床部署的机器学习模型需要知道何时可能失败，特别是对于训练数据中代表不足的亚组（如儿科数据），而成人数据训练的模型在儿童身上可能 silently under-perform，且没有失败迹象。重新训练需要标记的儿科数据通常不可行，因此在推理时检测失败是关键的临床需求。

Method: 在VIDS框架基础上，引入VIDS-Seg：在轻量级预测头上应用摊销变分推理，使这种自适应的、OOD感知的先验知识适用于密集图像分割任务。

Result: 在左心室分割任务上，VIDS-Seg在所有年龄层都与竞争基线匹配分割精度，同时产生更高的 Uncertainty 与分割错误之间的空间对应关系（即使对所有基线应用温度缩放后仍保持优势）。下游任务上产生更准确和稳定的射血分数估计，以及在婴儿亚组中更可靠地检测心脏功能障碍。

Conclusion: OOD感知的 Uncertainty Quantification 可以作为已部署分割模型的实用安全层，无需重新训练或额外标记数据即可检测代表不足亚组中的silent failures。
...
### [61] [Order Matters: LVLMs as Judges for Temporal Reasoning in Image Sequences](https://arxiv.org/abs/2608.10908)
*Martina Ianaro,Guilherme Fernandes,Maurizio Gabbrielli,Joao Magalhaes*

Main category: cs.CV

TL;DR: 论文揭示了当前多模态评估范式的结构性缺陷：基于大视觉语言模型(LVLM)的评估器存在位置偏倚，无法有效判断视觉叙事的时序一致性，存在首因效应和近因效应导致判断力崩溃。


<details>
  <summary>Details</summary>
Motivation: 随着生成式多媒体从静态图像合成向复杂交错视觉叙事演进，现有自动化评估系统无法区分连贯叙事与语义混乱序列，存在"判断危机"。这是因为当前范式依赖LVLM作为评判者，但其架构存在根本性局限。

Method: 通过一系列诊断探针实验，系统性地探测LVLM在时序判断任务上的表现，分析位置偏倚和顺序敏感度。

Result: 发现严重的性能 dichotomy：模型在孤立的逐点评分时看似胜任，但在要求成对时序判别时遭遇灾难性崩溃。揭示了系统性位置不对称现象——首因效应和近因效应，模型判断受帧的放置位置影响显著，甚至超过语义一致性的影响。

Conclusion: 这些偏倚可能源于因果掩码和旋转嵌入，当前基于Transformer的评判器本质上不适合长形式视觉推理。呼吁多媒体社区超越以快照为中心的指标，开创"时序感知评估"范式，将视觉序列视为统一逻辑结构而非无序帧集合。
...
### [62] [Temporally Grounded Compositional Camera Motion Understanding via Geometric Knowledge Distillation](https://arxiv.org/abs/2608.10932)
*Dazhao Du,Shiyan Du,Jian Liu,Yongjian Yu,Bohai Gu,Tao Han,Hualuo Liu,Eric Liu,Yujia Zhang,Xi Chen,Song Guo*

Main category: cs.CV

TL;DR: 本文提出CamChoreo基准和CamDistill方法，将相机运动理解从片段级标签升级为时序定位的组合式识别。CamChoreo包含4229个真实单镜头视频剪辑，标注了20个方向感知标签，近一半片段包含复合运动。CamDistill通过蒸馏3D基础模型的几何知识到轻量级相机标记，在推理时无需运行3D模型即可达到与直接特征注入相当的精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法对相机运动的理解局限于整个片段分配一个或多个标签，忽略了两个关键特性：1）运动可以在单个镜头内发生变化；2）多个运动可以同时发生。此外，当前多模态大语言模型的视觉编码器强调语义内容而非相机运动所依赖的几何证据。

Method: 提出时序定位的组合式识别任务，要求模型定位运动一致的区间并识别每个区间内活跃的所有运动。引入CamChoreo基准（4229个真实单镜头片段，20个方向感知标签）。提出CamDistill方法：在训练时将冻结的3D基础模型的几何知识蒸馏到轻量级相机标记中，推理时移除3D模型。baseline方法CamInject则直接在推理时注入3D模型特征。

Result: CamDistill在不需要运行3D教师模型的情况下，达到了与直接特征注入（CamInject）相当的精度。CamChoreo数据集近一半片段包含复合相机运动。

Conclusion: CamChoreo和CamDistill推动相机运动理解从片段级标注向前发展，实现了时序定位的组合式识别，为视频感知和可控视频生成提供更精细的理解能力。
...
### [63] [SafeCA: Safe Cross-Attention Localization and Regulation for Text-to-Video Jailbreak Defense](https://arxiv.org/abs/2608.10933)
*Siyuan Liang,Yupeng Qiu,Junfeng Fang,Rong-Cheng Tu,Jiaxing Huang,Dacheng Tao*

Main category: cs.CV

TL;DR: 本文提出SafeCA，一种针对文本到视频(T2V)生成模型的特征级防御机制，用于抵抗越狱攻击。通过分析干净样本和越狱样本在交叉注意力特征空间中的差异，发现了累积分离效应和线性可分性递增现象。SafeCA通过注意力稳定性分析识别防御区域，采用能量归一化的注意力掩码和语义空间适配器来缓解异常激活，并通过反向传播检测恶意token。实验表明该方法可将越狱成功率降低约20%，推理开销仅增加0.1秒，且保持良好的语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的T2V模型防御方法主要依赖输入过滤或重建，存在计算延迟高和语义失真问题，需要一种架构级、可部署的保护范式。

Method: 1.通过交叉注意力特征分析发现干净与越狱样本的累积分离效应和线性可分性递增趋势；2.利用注意力稳定性分析识别防御关键区域和值；3.采用能量归一化的注意力掩码抑制异常激活；4.引入轻量级语义空间适配器重定向异常语义流；5.通过反向传播特征异常信号检测并抑制恶意token。

Result: 在主流T2V模型上，越狱成功率降低约20%，推理开销仅增加0.1秒，保持良好的文本-视频语义一致性。

Conclusion: SafeCA提供了一种架构级的、可部署的T2V生成模型保护范式，在几乎不影响推理效率的情况下有效防御越狱攻击。
...
### [64] [GS-CPE: Unified 6-Degree-of-Freedom Camera Pose Estimation via 3D Gaussian Splatting](https://arxiv.org/abs/2608.10938)
*Huaiyuan Weng,Chul Min Yeum,Su-Min Kang*

Main category: cs.CV

TL;DR: GS-CPE是一种基于高斯溅射的粗到细6-DoF相机位姿估计框架，结合几何-based粗位姿估计与3DGS warp-based精细化，在室内外数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉定位取得显著进展，但在同时实现鲁棒泛化和高精度方面仍面临挑战。现有方法难以在精度和泛化能力上兼得。

Method: 提出GS-CPE框架，首先通过检索引导的几何位姿估计获取粗位姿，然后利用可见性感知的masked RGB warp目标在多尺度优化框架中进行精细化，并结合自适应重渲染。

Result: 在7Scenes、Cambridge Landmarks、FAST-LIVO2数据集及自定义数据集上验证，展现SOTA性能，在精度和泛化能力上均优于现有方法。

Conclusion: 该研究成功将几何方法与3DGS渲染方法结合，实现了鲁棒泛化与高精度的统一，为视觉定位提供了有效的新框架。
...
### [65] [Mixture-of-Experts-based Entropy Model for Learned Image Compression](https://arxiv.org/abs/2608.10947)
*Jonas Brenig,Radu Timofte*

Main category: cs.CV

TL;DR: 本文将专家混合(MoE)方法引入学习图像压缩领域，提出基于MoE的熵模型(MoEE)，使模型能够根据输入图像选择性激活所需的参数子集，实现更高效的图像压缩。


<details>
  <summary>Details</summary>
Motivation: 学习图像压缩近年来取得显著进展，端到端学习模型已超越传统方法。而MoE方法在NLP和计算机视觉任务中展现出良好效果，本文旨在将MoE方法应用于学习图像压缩以进一步提升压缩效率。

Method: 提出MoE-based Entropy model (MoEE)，允许模型根据输入图像选择性激活仅需要的参数子集，而非激活全部参数。

Result: 在Kodak数据集上，相对于VVC实现了-16.85%的BD-Rate改进。

Conclusion: MoE方法可有效应用于学习图像压缩，通过参数选择性激活提升压缩效率，为该领域提供了新的研究方向。
...
### [66] [StreamFlow: Dynamic Memory Flows for Streaming Video Understanding](https://arxiv.org/abs/2608.10949)
*Muxin Fu,Yifan Zhang,Wentao Zhang,Fangming Guo,Qian Chen,Guibin Zhang,Shuicheng Yan,Bo An*

Main category: cs.CV

TL;DR: StreamFlow是一个高效的视频流理解视觉记忆框架，通过动态按需访问历史视觉信息来解决连续视频流理解问题，在保持因果关系和有限内存的约束下保留相关证据。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限：基于模型的方法需要侵入性更新骨干网络，而基于记忆的方法在时间冗余内容上消耗大量视觉编码计算，且依赖僵化的视觉历史访问方式。

Method: StreamFlow结合了轻量级动态感知中期记忆（过滤时间冗余后再进行视觉编码）和潜在长期记忆（将历史视频内容整合为可供后续推理访问的视觉潜在表示）。在生成过程中，注意力引导检索机制在模型对视觉证据依赖减弱时注入相关视觉潜在表示。

Result: 在StreamingBench上达到67.73%的整体准确率，在离线长视频基准上也表现出色。相比基线模型，视觉注意力分数(VAS)提升59.1%，端到端延迟降低50.4%，峰值内存降低21.1%。

Conclusion: StreamFlow实现了更视觉基础化且高效的推理，为流式视频理解提供了一种有效的解决方案。
...
### [67] [Multiple Scale Latents for Learned Image Compression](https://arxiv.org/abs/2608.10952)
*Jonas Brenig,Radu Timofte*
...
### [68] [Evidence-Grounded Trustworthy Multimodal Reasoning and Evaluation Benchmark in Complex Urban Scenes](https://arxiv.org/abs/2608.10954)
*Zhaoyang Wei,Bowen Jiang,Xumeng Han,Jiashu Li,Xuehui Yu,Yuling Liu,Guorong Li,Zhenjun Han,Jianbin Jiao*

Main category: cs.CV

TL;DR: 本文提出AD2-Bench基准和EGVOR方法，解决多模态大语言模型在复杂 adverse 场景下推理可靠性下降的问题，通过显式生成证据原子实现更稳健的多模态认知。


<details>
  <summary>Details</summary>
Motivation: 现有 MLLM 在良性场景表现优异，但在复杂 adverse 条件下认知可靠性显著下降；现有基准仅评估最终预测，无法诊断推理过程中的失败原因。

Method: 1) 提出 Hierarchical Visual Diagnosis 框架，将推理分解为证据链(CoE)；2) 从概率角度建模推理，识别空间歧义和语义不确定两种失败原因；3) 提出 EGVOR 方法，生成结构化空间-语义三元组（证据原子）；4) 采用从监督学习到强化学习的分层课程训练。

Result: EGVOR 显著提高了 adverse 条件下的推理稳定性，提供了更可靠的多模态认知框架。

Conclusion: 稳健的多模态推理从根本上依赖于准确的证据获取，通过显式证据生成和降低推理方差的强化学习可有效提升模型在复杂场景下的可靠性。
...
### [69] [Once Poisoned, Arbitrarily Controlled: A Programmable Backdoor in VLMs](https://arxiv.org/abs/2608.10959)
*Tao Lin,Gaojie Jin,Zongxin Liu,Peng Wu,Lijia Yu*

Main category: cs.CV

TL;DR: 本文提出了一种针对视觉语言模型(VLM)的可编程后门攻击，实现任意到任意的字幕控制。与传统静态后门不同，攻击者可在推理时动态选择目标语义并按需合成隐蔽触发器，无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 现有VLM后门攻击被视为静态漏洞，遵循一对一或N对N的映射模式，在受害者训练前绑定有限数量的触发器到目标。这种假设严重低估了实际威胁，因为攻击者通常无法预知训练后的具体使用场景。

Method: 提出两阶段方法：1) 启发式投毒策略，通过多样化触发器-字幕对训练模型，使其学习通用的"触发器即指令"规则而非记忆特定后门模式；2) 特征空间触发器隐写术，将任意指定的目标字幕映射为隐蔽视觉触发器（范数控制扰动或非语义补丁），嵌入任意图像后使中毒VLM生成与目标字幕语义对齐的输出。

Result: 广泛实验表明，该攻击实现了高任意到任意字幕控制成功率，保持了干净模型的实用性，并在多种经典后门防御下仍保持有效。

Conclusion: 该研究揭示了VLM后门攻击的实际威胁远超预期，提出的可编程后门框架为后续防御研究提供了新的评估基准和方向。
...
### [70] [CARE: Confidence-Aware Reasoning for Reliable Medical VQA](https://arxiv.org/abs/2608.10964)
...
TL;DR: 本文提出CARE框架，通过结合Medical-CoT合成数据和带置信度感知奖励的Group Relative Policy Optimization (GRPO)，解决医疗多模态大语言模型在视觉问答中的置信度校准问题，在三个医学VQA基准上同时实现最高诊断准确率和最低校准误差。


<details>
  <summary>Details</summary>
Motivation: 当前医疗多模态大语言模型(MLLMs)通过强化微调(RFT)实现思维链(CoT)推理，但存在置信度错配问题——模型表达的确定性与其实际诊断准确率之间存在系统性偏差，这种偏差会削弱临床信任度，阻碍模型在实际医疗场景中的应用。

Method: 提出双阶段管道：(1)可扩展的Medical-CoT合成提供结构化冷启动数据进行监督微调；(2)使用创新的置信度感知奖励(CAR)机制的GRPO，将模型置信度与诊断正确性绑定到奖励信号中，实现准确率和校准的联合优化。

Result: 在三个医学VQA基准测试中，CARE实现了最高的诊断准确率，同时获得最低的预期校准误差(ECE)和最低的幻觉率。

Conclusion: CARE为可信临床决策支持奠定了基础，证明了在医疗AI中联合优化模型准确率和置信度校准的可行性，为构建更可靠的医疗诊断辅助系统提供了新思路。
...
TL;DR: 本文发布了首个多声部OMR数据集OSSQ-OMR，基于OpenScore弦乐四重奏语料库，包含24,544张系统图像和98,172张五线谱图像，涵盖116首弦乐四重奏作品，并提供三种编码格式的转录。


<details>
  <summary>Details</summary>
Motivation: 光音乐识别（OMR）在单旋律和钢琴谱转录方面已取得显著进展，但多声部乐谱的转录仍缺乏充分探索，主要原因是缺少合适的数据集。

Method: 构建了OSSQ-OMR数据集，将数字编码乐谱与IMSLP的原始扫描版本进行视觉对齐，提供系统和五线谱级别的图像，以及LMXE、**kern和ABC三种编码格式的转录。采用两个代表性OMR模型进行基准测试，在4个随机乐谱级分割上进行评估。

Result: 基线模型在合成输入上达到3.6%的OMR-NED，在扫描输入上达到5.9%。结果显示编码和分割选择有显著影响，LSTM基线在扫描输入上的性能下降约为Transformer基线的2.6倍。

Conclusion: OSSQ-OMR作为首个多声部OMR数据集，为该领域研究提供了重要的基准资源和评估框架，推动了OMR在复杂乐谱识别方面的发展。
...
TL;DR: ThinkAfford是一种任务驱动的3D affordance接地方法，通过解耦高召回率的affordance proposal生成与指令接地推理来解决现有方法的两个关键失败模式：预测区域遗漏目标交互区域或粒度不合适，以及语言接地在关系指令下混淆视觉相似替代物。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个相互关联的失败模式：1) 预测或选择的区域可能遗漏目标交互区域或粒度不合适；2) 语言接地在关系指令下可能混淆视觉相似的替代物。这些问题限制了3D场景中功能区域定位的准确性。

Method: 提出ThinkAfford框架，包含三个核心组件：1) Affordance Proposal Generation (APG)模块 - 使用可学习的affordance prompts和多级视觉特征预测交互条件热图，提取细粒度proposals无需分割prompt；2) Visual-Prompted Affordance Reasoning (VPAR)模块 - 在proposal叠加上进行完整指令推理，以"先思考后回答"的响应方式返回标识符；3) Group Relative Policy Optimization (GRPO) - 使用提升的3D重叠提供proposal级奖励，将VPAR选择与最终3D接地对齐。

Result: 在SceneFun3D验证集上，ThinkAfford在官方评估器下达到10.69% AP50和25.46% AP25，优于可比拟的3D开放词汇和基于视觉语言模型的2D-to-3D基线。模块级诊断显示APG在25% IoU下达到77.5%召回率，GRPO训练的VPAR在APG覆盖的查询上达到72.1%选择准确率，相比监督微调的63.4%有显著提升。

Conclusion: ThinkAfford通过解耦高召回率proposal生成与指令接地推理，有效解决了3D affordance接地中的区域遗漏和粒度不合适问题。GRPO训练策略显著提升了VPAR的选择准确性，实验结果证明该方法在SceneFun3D基准上优于现有3D开放词汇和VLM-based 2D-to-3D方法。
...
TL;DR: 提出PEAK框架，使用k-Sparse Autoencoders实现对文本到图像扩散模型的精确且持续的概念擦除，通过识别目标特定稀疏特征并进行参数优化来消除不需要的概念。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以实现精确和持久的概念擦除：不准确的概念定位导致语义干扰，不完全移除概念知识允许对抗性恢复。需要解决版权侵权、隐私侵犯和攻击性内容的问题。

Method: 1. 训练kSAE分解扩散去噪网络的内部激活为可解释的稀疏特征；2. 通过对比目标和非目标提示词引起的稀疏激活，根据激活强度和频率识别目标特定特征；3. 使用这些局部化特征进行参数优化，选择性抑制目标相关激活同时保留非目标激活。

Result: 在I2P基准测试中，将NudeNet检测从582降至6，攻击成功率从96.52%降至5.63%，在MS-COCO上保留生成质量，KID接近零。

Conclusion: PEAK实现了有效且鲁棒的概念擦除，将概念擦除嵌入扩散参数中，无需额外推理时干预，能有效抵抗对抗攻击。
...
### [74] [Putting Registers to Work: Task Registers for Token Pruning in Vision Transformers](https://arxiv.org/abs/2608.10989)
*Hongsen Cao,Mona Jaber,Shanxin Yuan,Ahmed Sayed*

Main category: cs.CV

TL;DR: 本文研究预训练Vision Transformer在不同视觉任务（图像分类、语义分割、目标检测）间的token剪枝策略迁移性问题，提出任务自适应剪枝（TAP）方法，通过任务寄存器实现跨任务的动态token剪枝。


<details>
  <summary>Details</summary>
Motivation: 现有的token剪枝策略通常针对单一识别流程设计，但预训练Vision Transformer会被重用于不同空间需求的任务。作者试图探究哪些剪枝策略组件可以跨任务迁移。

Method: 使用控制探针方法：冻结无剪枝检查点，在每层应用一系列无参数的约简标准进行测试，无需重训练。提出任务自适应剪枝（TAP）：使用任务寄存器存储特征伪影，根据任务动态调整token排序、深度方向的移除预算和密集特征恢复规模。

Result: 在ρ=0.5的保持率下，TAP-J在ADE20K上达到47.0 mIoU，编码器吞吐量提升1.30倍；在COCO上达到53.7 box AP，吞吐量提升1.32倍；同时在ImageNet-1K上保持竞争力。

Conclusion: 不同视觉任务对剪枝标准的排序、注意力剪枝的敏感性以及恢复端点有不同偏好。TAP通过引入任务寄存器和动态调整机制，有效实现了跨任务的token剪枝自适应，显著提升了密集预测任务的效率和性能。
...
### [75] [HNDiff: Haze-Noise Diffusion for Image Dehazing](https://arxiv.org/abs/2608.10995)
*Jin-Ting He,Fu-Jen Tsai,Yan-Tsung Peng,Min-Hung Chen,Chia-Wen Lin,Yen-Yu Lin*

Main category: cs.CV

TL;DR: 本文提出Haze-Noise Diffusion (HNDiff)框架，将大气散射模型嵌入扩散模型作为归纳偏置，解决现有基于扩散的图像去雾方法忽视雾形成物理机制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散去雾方法通常忽略雾形成的物理原理，从纯高斯噪声重建清晰图像，限制了恢复潜力。

Method: 提出联合雾-噪声扩散和雾感知噪声调度器，前向过程根据雾密度自适应调整噪声水平（重雾区强噪声以促进内容生成，清晰区轻噪声以保留细节）；反过程推导出物理一致的去雾-去噪过程。提出Latent HNDiff整合到现有去雾网络中提升性能。

Result: 显著提升领先去雾骨干网络性能，在基准数据集上达到最先进水平。

Conclusion: 通过将扩散模型锚定到物理原理，确保去雾结果与雾形成底层机制更紧密对齐，提高了去雾效果和实用性。
...
### [76] [Watching Synthetic Videos: Aligning Cross-modal Representations with Visual Synthesis for Zero-shot Video Captioning](https://arxiv.org/abs/2608.11013)
*Liangyu Fu,Junbo Wang,Yuke Li,Ya Jing,Xuecheng Wu,Zhiyong Wang*

Main category: cs.CV

TL;DR: 本文提出WSV框架，用于解决零样本视频字幕生成中的跨模态差距问题。该框架采用两阶段训练：首先利用预训练文本到视频生成模型合成视频潜在表示，然后通过polisher模块提升表示质量，最后设计prompter引导GPT-2生成字幕。在MSVD、MSR-VTT和VATEX数据集上取得了优异表现。


<details>
  <summary>Details</summary>
Motivation: 在零样本视频字幕生成中，训练阶段仅使用文本数据而推理阶段使用视频数据，这种文本-only训练与视频-only推理之间的跨模态差距导致字幕生成不准确。现有方法通过简单线性变换难以有效弥合这一差距。

Method: 提出两阶段训练框架WSV：第一阶段使用预训练文本到视频生成模型生成合成视频潜在表示，并设计polisher模块桥接真实与合成视频分布；第二阶段设计prompter基于polished潜在表示引导GPT-2生成字幕。推理时，输入视频经3D Causal VAE编码后直接送入prompter引导GPT-2输出最终字幕。

Result: 在MSVD、MSR-VTT和VATEX数据集上实验，B@4指标达到52，CIDEr指标达到95.7。

Conclusion: 通过引入文本到视频生成模型和polisher模块，有效弥合了零样本视频字幕生成中文本与视频之间的跨模态差距，显著提升了字幕生成质量。
...
### [77] [R4DSG: Relative 4D Scene Graph Memory for Object-Centric Question Answering in Long Egocentric Video](https://arxiv.org/abs/2608.11017)
*Ke Ma,Yamin Mao,Weiming Li,Shuai Tan,Yijie Zhong,Hao Chen,Haofen Wang,Meng Wang*

Main category: cs.CV

TL;DR: R4DSG是一种用于长程自我中心视频的相对4D场景图记忆方法，通过分离稳定锚点与动态对象、维持跨帧的持久对象身份、使用锚点相对转换表示对象状态，从而为可穿戴AI助手提供可查询的记忆，用于回答关于物体移动、状态变化等的问题。


<details>
  <summary>Details</summary>
Motivation: 当前长视频QA方法主要强调时间定位和片段检索，而先前3D场景图方法需要更强的几何输入（如点云、RGB-D、posed视图等）。此外，基于caption和transcript的记忆很少保留持久对象身份或结构化空间变化，因此难以回答如物品被移到哪里、最后一次状态改变时间等对象中心问题。

Method: R4DSG将视频转换为紧凑的可查询记忆条目，按时间、地点、持久对象、锚点相对变化和局部交互上下文索引。核心思想是：1）分离稳定锚点与动态对象；2）跨帧维持持久对象身份；3）通过锚点相对转换而非全局对齐的世界模型表示对象状态。基于近期RGB-only的视频分割、时间传播和相对3D提升技术。

Result: 在EgoLifeQA的255个对象相关问题的子集上，仅使用问题检索时，R4DSG相比EgoRAG-Text总体提升6.7分，在when问题上提升12.5分，突出了时间组织对象记忆的价值。

Conclusion: 相对4D场景图是可穿戴助手、AR系统和具身多媒体智能体的实用记忆基材。
...
### [78] [When Visual Signals Mislead: A Mechanistic Study of Attribute Hallucination in Vision-Language Models](https://arxiv.org/abs/2608.11024)
...
### [79] [Multi-Level Evidence Aggregation for Robust Facial Phenotype Retrieval in Rare Genetic Disorder Prioritization](https://arxiv.org/abs/2608.11037)
*Alexander Hustinx,Carolin Kaffiné,Behnam Javanmardi,Tzung-Chien Hsieh,Peter Krawitz*

Main category: cs.CV

TL;DR: 本文提出了一种推理时多层级证据聚合框架，通过患者图像聚合、疾病质心加权和混合评分三种策略，在不修改GestaltMatcher-Arc编码器的情况下，显著提升了罕见遗传疾病的面部表型检索准确率，Top-1准确率最高提升至60.94%。


<details>
  <summary>Details</summary>
Motivation: 现有GestaltMatcher检索框架采用逐点比较方式，未能充分利用患者多张图像以及疾病多个确诊病例的可用证据，导致检索效果受限。

Method: 提出三种聚合策略：1) 嵌入层患者聚合：将同一患者的多个图像嵌入进行聚合；2) 患者加权疾病质心：计算疾病质心时考虑患者权重；3) 混合个体-质心评分：整合测试患者观察、疾病级别图库证据和局部最近邻证据。

Result: 在GMDB v1.1.4上，Top-1准确率从38.52%提升至48.82%（GMDB-Freq），从19.38%提升至23.79%（GMDB-Rare）；多图像子集上从46.12%提升至60.94%（GMDB-Multi-Freq），从18.54%提升至26.71%（GMDB-Multi-Rare）。

Conclusion: 推理时聚合可显著提升面部表型检索性能，支持从孤立单图匹配向多层级患者与疾病证据聚合的转变，为罕见疾病优先级排序提供更有效的支持。
...
### [80] [3D Weighted Geometric Graph Neural Networks for Sheep Facial Pain Assessment](https://arxiv.org/abs/2608.11050)
*Alam Noor,Luis Almeida,Mohamed Daoudi*

Main category: cs.CV

TL;DR: 本文提出3D-SPFES系统，利用单目RGB相机结合VideoDepthAnything技术，将绵羊面部关键点（如耳朵、眼睛、鼻子）从2D提升到3D欧几里得空间，通过加权几何图神经网络(WG-GNN)进行疼痛评估，解决了传统2D方法丢失3D解剖结构信息的问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习系统主要在2D单图像域内工作，将面部视为单一维度表示，忽略了绵羊的3D解剖结构和临床验证的绵羊疼痛面部表达量表(SPFES)中跨关键点的空间关系。

Method: 1) 使用VideoDepthAnything从单RGB相机估计3D深度信息；2) 将每个SPFES面部关键点（耳朵、眼睛、鼻子）表示为包含3D空间坐标、估计表面法线和面部属性类嵌入的特征向量；3) 边权重基于欧几里得距离和3D空间表面共面性的聚合度量计算；4) 采用K=3的几何感知消息传递层和缩放点积注意力方法的加权几何图神经网络(WG-GNN)学习图结构。

Result: 将节点嵌入聚合成O=3个疼痛级别集群，并整合为归一化疼痛评分(NPS)，范围为[0,100%]的置信度加权SPFES衍生评分方法。

Conclusion: 该系统无需专业深度硬件，仅需单目RGB相机即可实现3D绵羊疼痛面部表情评估，为动物福利和疼痛管理提供了新的技术手段。
...
### [81] [HUI360: A 360° Egocentric Dataset and Baselines for Human-Robot Interaction Anticipation](https://arxiv.org/abs/2608.11051)
*Raphael Lorenzo-Louis,Fabio Amadio,Bertrand Luvison,Serena Ivaldi*

Main category: cs.CV

TL;DR: HUI360是用于野外机器人人机交互预测的最大数据集，包含从移动机器人采集的100万预标注数据（2D姿态、面部关键点、分割掩码），以及600万SSUP-HRI数据集标注，并提供了交互预测基准测试。


<details>
  <summary>Details</summary>
Motivation: 随着机器人在人类环境中运行越来越普遍，需要预测人类意图以实现主动和社交感知行为。人机交互的自动预测已成为嵌入式代理的关键感知挑战。

Method: 设计了一套自动化交互标注流程，可处理任意360度等距长方投影视频，包含手动 refinement 接口。使用最先进计算机视觉方法获取标注并手动审核确保质量。

Result: 发布了最大的野外人机交互预测数据集HUI360，包含100万预标注数据和原始全景360度图像。同时发布了SSUP-HRI数据集的600万标注。建立了包括首个跨数据集评估在内的基准测试。

Conclusion: HUI360数据集的发布将推动人机交互预测模型的泛化能力评估和研究，为该领域提供重要的基准资源和评估框架。
...
### [82] [A Comparative Evaluation of Deep Learning Object Detection Models on a Real-World Multi-Plant Dataset from Africa](https://arxiv.org/abs/2608.11053)
...
TL;DR: 本研究在尼日利亚农场收集了3,382张真实农业图像数据集AgriAISeg，对比评估了6种目标检测模型在复杂田间条件下的表现，发现RT-DETR表现最佳，YOLO系列模型训练效率优于Faster R-CNN。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉农业应用多依赖受控数据集，缺乏对非洲等欠发达地区真实农业条件的代表性研究。

Method: 使用AgriAISeg数据集（包含芝麻、卷心菜和番茄在光照变化、遮挡、不同视角下的3,382张图像），训练并评估YOLOv5、YOLOv8、YOLO11、YOLO26、Faster R-CNN和RT-DETR六种模型，采用precision、recall、mAP@0.5和mAP@0.5:0.95指标评估性能。

Result: RT-DETR取得最高总体性能，precision达0.768，mAP@0.5:0.95达0.624；YOLOv8和YOLO11表现也较强；Faster R-CNN的mAP@0.5仅0.466，效果显著较差；YOLO系列模型训练效率优于Faster R-CNN。

Conclusion: 现代单阶段检测器和基于Transformer的检测器为真实农业环境中的植物检测提供了更可靠、高效的解决方案。
...
### [83] [Entropy-Centric Explainable AI for Remote Sensing Image Segmentation](https://arxiv.org/abs/2608.11064)
*Ali Saleh,Abdul Karim Gizzini,Mohamad Ghassany,Ali J. Ghandour*

Main category: cs.CV

TL;DR: 本文提出了一种面向语义分割的以熵为中心的可解释人工智能(XAI)方法，并设计了新的XAI评估方法，用于测量XAI方法突出显示区域的相关性。实验结果表明，该方法优于近期针对语义分割改进的XAI方法。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在关键领域（如遥感）的高分辨率图像分析中表现优异，但其决策过程不透明，缺乏可解释性，限制了模型的信任度和应用 adoption。语义分割的可解释性研究仍有很大改进空间。

Method: 提出一种以熵为中心的XAI方法用于语义分割，并提出新的XAI评估方法来有效测量XAI方法突出显示区域的相关性。

Result: 实验结果表明，所提出的XAI方法优于近期针对语义分割改进的XAI方法。

Conclusion: 所提出的以熵为中心的XAI方法在语义分割任务中表现出色，能够有效提高模型的可解释性和可信度。
...
### [84] [CapProbe: Evaluating Detailed Image Captions via Full-Scene Dense Question Answering](https://arxiv.org/abs/2608.11074)
*Mouxiao Huang,Qiangyu Yan,Borui Jiang,Han Shu*

Main category: cs.CV

TL;DR: CapProbe是一个密集的问答基准测试，将详细图像描述评估转变为区域对齐的事实检查。该基准通过将图像分解为粗粒度语义区域，为每个区域生成涵盖10个语义类别的多项选择题，形成密集的视觉事实检查清单。


<details>
  <summary>Details</summary>
Motivation: 现有的VLM图像描述评估方法存在局限：基于引用的指标（如CIDEr和SPICE）难以验证密集的事实声明，而现有的基于QA的替代方案通常提供较低的探测密度、较窄的领域覆盖或缺乏问题与分割图像区域之间的明确对齐。

Method: 将每张图像分解为覆盖前景和背景元素的粗粒度语义区域；为每个保留的区域生成涵盖10个语义类别的多项选择题，形成密集的探测视觉事实清单。指南基于37个L1领域和219个L2子领域的两层分类法。包含346张图像、1868个区域和25650个问题，平均每张图像74个QA对。使用语言法官仅从描述中回答；提供"不确定"选项和"有效准确率"作为区分未回答探测和错误解决的代理指标。

Result: 对13个VLM的实验显示：模型之间存在较大的覆盖差距，表现出明显的能力-效率权衡，以及稀疏或基于重叠的评估通常会遗漏的失败模式。

Conclusion: 该协议具有成本效益：通过将无约束的标量评分转换为结构化选择题阅读，减少了开放式评分偏差，同时保持法官条件性，并在固定阅读器下产生相对稳定的模型排名。
...
### [85] [Static in Frames, Dynamic in Events: Rethinking Features in Event Cameras as Motion Cues](https://arxiv.org/abs/2608.11075)
*Hesam Araghi,Jan van Gemert,Nergis Tomen*

Main category: cs.CV

TL;DR: 本文分析了事件相机中两种特征（结构张量特征值和时空密度值）作为运动线索的作用，并将其与局部几何信息结合用于运动估计任务，在合成数据集和真实世界DSEC基准上验证了效果。


<details>
  <summary>Details</summary>
Motivation: 事件相机捕获强度变化的方式与传统帧相机不同，现有的预处理方法未能充分利用事件数据中的运动信息。作者假设结构张量特征值和密度值结合局部几何信息可以增强运动估计任务。

Method: 1. 理论分析结构张量特征值与运动方向的关系；2. 在合成数据集上进行控制实验验证特征效果；3. 将特征集成到先进的事件光学流网络中并在DSEC基准上评估。

Result: 特征值和密度值提供了互补的运动信息，对纹理和 shot 噪声具有鲁棒性；在DSEC基准上，持续提高准确性，在数据稀缺场景和低容量模型中提升最大。

Conclusion: 结构张量特征值和时空密度值是有效的运动线索，结合局部几何信息可显著提升事件相机运动估计性能，尤其适合数据有限和计算资源受限的场景。
...
### [86] [Foundation Model-Enabled Efficient Data Sampling (FEEDS): A label-efficient training strategy for pan-cancer, multi-tracer PET/CT datasets](https://arxiv.org/abs/2608.11076)
...
TL;DR: FEEDS是一种基于视觉基础模型的高效数据采样策略，用于全身PET/CT影像中的自动化病灶分割。该方法通过基础模型嵌入选择最有信息量和多样性的未标注案例进行专家标注，解决了医学影像中标注数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 训练病灶分割模型需要大量标注数据，但创建标注数据集耗时且需要专业知识，导致模型准确性和泛化能力不足。现有方法（无监督、半监督、主动学习）存在局限性，需要一种标签高效且计算高效的训练范式。

Method: 提出FEEDS方法，利用视觉基础模型嵌入来选择最信息丰富和多样化的未标注案例进行专家标注。训练和验证在AutoPET-III数据集上进行，在三个测试集上评估：AutoPET-III、DeepPSMA和Dartmouth-Hitchcock医学中心数据集。评估包括体素、病变和解剖区域三个层面。

Result: FEEDS优于随机采样标注、伪标签半监督学习和仅使用有限标注数据的训练方法。模型可跨三个测试集、FDG和PSMA示踪剂以及多种疾病进行泛化，用70%更少的标注负担达到100%全标注训练性能。

Conclusion: FEEDS通过从大型未标注临床数据库构建代表性和多样化的标注队列，为病灶分割框架中的标签稀缺问题提供了实用解决方案，具有重要的临床应用价值。
...
### [87] [Learning Gaussian Structure: Intervention-Guided Density Control for Feed-Forward Driving Reconstruction](https://arxiv.org/abs/2608.11077)
*Hang Li,Jiahe Li,Meiying Gu,Jin Zheng,Lina Yu,Xiao Bai*

Main category: cs.CV

TL;DR: 本文提出LGS框架，通过学习高斯结构（Gaussian Densify Policy）和跨时间点查询来增强高斯结构的优化和属性预测，在Waymo和PandaSet数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前LiDAR前馈高斯重建方法存在两个问题：1) 将初始化的原始集合作为最终表示，无法像优化3DGS那样通过梯度累积来决定如何细化场景表示；2) 共享稀疏骨架仅隐式融合不同时间戳的观测，未能明确聚合跨时间证据。

Method: 1) 基于干预剪枝或添加操作时局部梯度响应变化的观察，提出高斯细化策略(Gaussian Densify Policy)，学习包含剪枝分数和添加分数的Densify Map，直接在推理时调整高斯结构；2) 开发紧凑的跨时间点查询模块，显式检索和聚合其他时间戳的高斯原语特征，用于可靠的属性预测。

Result: 在Waymo Open Dataset和PandaSet数据集上，LGS一致性地优于现有方法。

Conclusion: LGS框架成功解决了前馈高斯重建中结构优化不足和跨时间特征聚合缺失的问题，通过可学习的细化策略和跨时间查询机制显著提升了场景重建质量。
...
### [88] [Every Packet Counts: Dispersing Information for Loss-Resilient Learned Image Compression](https://arxiv.org/abs/2608.11096)
*Yuhang Wei,Chuqin Zhou,Yibo Shi,Jing Wang,Guo Lu*

Main category: cs.CV

TL;DR: 本文提出了一种端到端的抗丢包图像压缩方案，通过通道间重分配、交织通道分组和双分支自回归结构，在20%丢包率下相比LossResilientLIC提升1.84dB PSNR，并将PSNR方差降低一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法在卫星通信和应急通信中常见的丢包情况下表现脆弱，主要原因在于打包阶段信息分布不均和熵编码阶段存在顺序解码依赖。

Method: 1. 通道间重分配(ICR)机制：在打包前重新分配通道能量，防止关键信息集中在少量通道中。2. 交织通道分组(ICG)策略：以跨步方式对潜在通道进行分组，将信息分散到各个数据包中，并限制每个数据包的大小。3. 双分支自回归结构：采用两层双分支自回归结构缩短依赖链，减少丢包导致的级联错误。

Result: 在20%丢包率下，平均PSNR增益比LossResilientLIC高1.84dB，PSNR方差降低一个数量级。仅在均匀随机丢包下训练的模型可泛化到Gilbert-Elliott信道描述的突发丢包场景，且优于专门针对该条件训练的方法。

Conclusion: 该方法在重建质量和稳定性方面均优于现有方法，具有良好的泛化能力，为卫星和应急通信中的抗丢包图像传输提供了有效解决方案。
...
### [89] [Test-Time Self-Evolving GUI Visual Grounding via Reflection-Guided On-Policy Self-Distillation](https://arxiv.org/abs/2608.11191)
*Shiyu Xuan,Zechao Li*

Main category: cs.CV

TL;DR: 本文提出了一种测试时自进化框架(TTSE)，使GUI视觉定位模型能够在部署后无需人工标注即可自我改进。该框架通过探索、评估、反思和内化的闭环过程，结合基于MLLM的反思器和反射引导的策略自蒸馏方法，在六个基准上实现平均7.4%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理模型在部署后参数冻结，无法适应未见过的界面；现有测试时强化学习方法无法对失败探索进行反思，导致适应效果有限。

Method: 提出测试时自进化框架，包含：1)探索阶段在未见界面上预测定位坐标；2)MLLM-based反思器评估探索结果并提供推理反思；3)反射引导的策略自蒸馏(Reflection-Guided On-Policy Self-Distillation)将高层推理转化为密集的token级监督；4)对比校准方法防止错误自回归前缀破坏监督信号。

Result: 在六个基准上实现平均7.4%的准确率提升，是首个成功将策略自蒸馏应用于GUI视觉定位测试时适配的工作。

Conclusion: 通过填补部署后适配的空白，框架完成了GUI代理的自进化能力，为GUI代理的持续自我改进提供了新范式。
...
### [90] [AlbumentationsX: One Augmentation Pipeline for Images and Related Annotations](https://arxiv.org/abs/2608.11123)
*Vladimir Iglovikov*

Main category: cs.CV

TL;DR: AlbumentationsX是一个图像增强库,旨在解决数据增强过程中图像与其标注(掩膜、边界框、关键点等)可能发生错位的问题。该库将变换列表、概率、标注设置和随机种子集中在一个Compose对象中,确保每次调用时只选择一次随机值,并将其应用到训练示例的每个支持部分。


<details>
  <summary>Details</summary>
Motivation: 数据增强是计算机视觉训练中的常见技术,但当图像及其标注接收到不同的随机变化时,可能会导致数据损坏。例如,裁剪操作必须为图像、掩膜、边界框、关键点、立体视图、视频帧或体积使用相同的坐标。分别选择这些值的代码路径可能会无声地对齐数据造成错误。

Method: AlbumentationsX将变换列表、概率、标注设置和随机种子保存在一个Compose对象中。每次调用时,库会为每个支持的数据类型(如掩膜、边界框、标签等)一次性选择随机值并应用变换。该库还允许项目添加自己的变换,并能保存管道定义、显示单次调用中发生的情况,以及重新运行该调用。

Result: 该库能够保持每个对象的掩膜、边界框和标签在一起,确保在增强过程中不会发生错位。示例代码将Compose放在文件解码成数组之后、PyTorch将示例分组到批之前的位置。

Conclusion: AlbumentationsX执行声明的变换,但 practitioners 仍需自行决定翻转、裁剪、颜色变化或其他操作是否为其任务保留了正确的标签。该库提供了统一的数据增强框架,帮助避免因标注错位导致的训练错误。
...
### [91] [Is There Really a Camouflaged Object? Towards Realistic Camouflaged Object Detection](https://arxiv.org/abs/2608.11135)
*Huafeng Chen,Yueming Lyu,Chenyang Si,Wende Tan,Liucheng Guo,Caifeng Shan*

Main category: cs.CV

TL;DR: 本文提出了OPC16K大规模基准数据集和OPCNet网络，用于解决开放环境下伪装物体检测中纯背景和非伪装物体导致的误检问题。OPC16K包含16,245张图像，分为伪装物体、纯背景和非伪装物体三类。OPCNet通过存在性推理、相似性关系建模和存在感知特征细化来联合处理目标定位与伪装存在性判断。


<details>
  <summary>Details</summary>
Motivation: 现有伪装物体检测方法基于封闭世界假设（每张图像都包含伪装物体），忽视了现实场景中可能存在纯背景或非伪装物体的情况，导致在开放世界部署时产生严重误报。

Method: 提出OPC16K基准数据集（16,245张图像，来自14个来源，含三类样本）；提出OPCNet网络，将COD从纯分割任务重构为目标定位与伪装存在性推理的联合问题，包含层次化存在性推理模块、相似性感知伪装关系建模模块和存在感知特征细化模块。

Result: OPCNet在OPC16K上实现优越性能，显著降低负样本误报，同时保持准确的伪装物体分割效果。

Conclusion: 通过构建大规模基准数据集和设计存在感知网络，有效解决了开放环境下伪装物体检测的误检问题，为COD领域提供了新的研究思路和评估标准。
...
### [92] [SAR2Agri: Learning SAR Intensity Representations for Agricultural Monitoring](https://arxiv.org/abs/2608.11142)
*Moti Rattan Gupta,Anupam Sobti*

Main category: cs.CV

TL;DR: 首个专注于SAR强度图像的农业应用自监督学习pipeline，通过掩码和课程学习改进时间 pretext 任务，在SICKLE基准上实现84.9% IoU的作物类型映射。


<details>
  <summary>Details</summary>
Motivation: 农业监测面临景观的复杂时间、物候和气候动态挑战，对确保粮食安全至关重要。SAR卫星提供全天候日夜成像能力，但现有SAR基础模型主要针对目标检测、洪水映射和土地覆盖分类，缺乏专门针对农业应用的研究。

Method: 提出首个仅使用SAR强度图像的自监督学习pipeline，通过掩码和课程学习增强时间 pretext 任务，以更好地从SAR图像中捕获物候特征。

Result: 在SICKLE基准上，模型达到84.9% IoU的作物类型映射表现，优于光学基线（15.3个百分点）和现有SAR基线（2.2个百分点）。

Conclusion: 所提出的pipeline对农业监测的SAR强度编码器预训练具有显著有效性，为SAR图像在农业领域的应用提供了新的自监督学习范式。
...
### [93] [PRMU: A Corpus-Free Benchmark for Person-Centric Knowledge Unlearning in Multimodal Large Language Models](https://arxiv.org/abs/2608.11149)
*Huafeng Chen,Yueming Lyu,Ziyuan Chen,Wenda Tan,Chenyang Si,Liucheng Guo,Caifeng Shan*

Main category: cs.CV

TL;DR: PRMU是一个用于评估多模态大语言模型在无语料库情况下进行人物中心知识遗忘的基准测试，解决了现实场景中无法访问原始遗忘和保留语料库的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM机器遗忘方法通常假设可以访问原始遗忘和保留语料库，但在实际删除场景中这些语料库往往不可用，因此需要一种无需语料库的多模态遗忘方法。

Method: 提出PRMU基准测试和SGPE方法。PRMU通过多样化的文本和视觉探测（包括对抗性评估和细粒度局部性分析）来评估模型移除目标知识的同时保留相关知识的能力。SGPE是一种轻量级的无语料库遗忘基线，包含知识位移、受保护参数空间编辑和局部感知多模态控制。

Result: 在代表性MLLMs上的广泛实验表明，现有遗忘方法通常在遗忘-局部性权衡上表现不佳，在激进遗忘设置下存在显著的局部性下降，并且容易受到多模态知识重新激活的影响。同时，SGPE在目标遗忘、局部性保留和通用多模态效用之间提供了有竞争力的权衡。

Conclusion: PRMU基准和SGPE方法为现实和可扩展的多模态机器遗忘研究提供了新途径，推动了该领域向更实用的场景发展。
...
### [94] [CausalSplat: Towards Comprehensive Hierarchical Reasoning in 3D Gaussian Splatting](https://arxiv.org/abs/2608.11150)
*Jiayu Ding,Meilu Song,Yun Chen,Wei Gao,Ge Li*

Main category: cs.CV

TL;DR: 本文提出推理3D Gaussian分割任务，构建Causal-LERF和Causal-ScanNet两个基准测试集，评估常识、空间、可供性和反事实推理能力。提出CausalSplat框架，整合视觉语言模型与3D场景图，在推理基准上达到最优性能，同时在标准指代分割和开放词汇3D分割任务上展现强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D Gaussian Splatting方法虽推进了开放词汇场景理解，但仍局限于显式查询，难以解释隐含意图、复杂空间约束和常识推理，无法满足实际具身交互需求。

Method: 提出CausalSplat框架，将视觉语言模型与3D场景图集成，将显式结构感知与隐式逻辑推理分离。

Result: 在Causal-LERF和Causal-ScanNet推理基准上取得最先进性能，同时在标准指代分割和开放词汇3D分割任务上展现强泛化能力。

Conclusion: CausalSplat能有效解决3D Gaussian分割中的推理挑战，为具身智能提供更优的场景理解能力。
...
### [95] [MultiModal Code-Switching: Interleaving Visual Objects into Language for Explicit Object-Level Alignment](https://arxiv.org/abs/2608.11167)
*Changhao Xiang,Shangyu Xing,Zhen Wu,Jianbing Zhang,Xinyu Dai*

Main category: cs.CV

TL;DR: 提出多模态代码切换(MMCS)预训练范式，通过将文本实体替换为对应视觉对象来实现局部视觉-语言对齐，解决现有MLLMs中图像级对齐的指代歧义问题。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs主要依赖图像-文本对进行模态对齐预训练，但这种图像级对齐存在指代歧义问题：模型难以从全局表示中推断多个视觉对象与文本实体之间的对应关系，导致数据效率低和语义接地不佳。

Method: 受语言学代码切换现象启发，MMCS通过将文本实体替换为对应视觉对象来交错视觉和语言，实现局部视觉-语言接地。开发了可扩展的数据合成 pipeline，生成了773K样本的预训练数据集。

Result: MMCS具有极高的数据效率：仅用50K样本即可达到或超越使用600K图像-文本对训练的模型效果。在不同模型规模下均能持续改善视觉接地和感知能力。

Conclusion: MMCS通过提供明确的物体级监督信号，有效解决了图像级对齐的指代歧义问题，显著提升了多模态模型的语义接地能力和数据效率。
...
### [96] [VidForensics-M1: Meta-Detection Reinforcement Learning with Verifiable Temporal Grounding for AI-Generated Video Forensics](https://arxiv.org/abs/2608.11201)
*Bowei Liu,Zheng Lu,Yuhan Bian,Xinchen Zhang,Xingming Shui,Yuesheng Huang,Xuhuan Li,Zihao Liu,Yifan Yang,Jun Zhou,Xiu Li*

Main category: cs.CV

TL;DR: 本文首次将元检测(meta-detection)引入AI生成视频检测领域，通过强化学习联合优化预测标签和支持证据，实现对未见场景和新出现视频生成器的鲁棒泛化检测。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的检测器主要依赖监督微调或标签级强化学习，粗粒度监督限制了模型对未见场景和新出现视频生成器的泛化能力。

Method: 1) 提出自动化数据构建管道，通过边界帧条件视频生成模型替换时间片段生成配对真实-伪造视频；2) 引入证据引导的奖励重分配(Evidence-Guided Reward Redistribution)，根据证据质量在标签正确的响应间重分配奖励。

Result: VidForensics-M1方法在实验中展现出利用可验证时间证据实现鲁棒且可泛化的AI生成视频检测的有效性。

Conclusion: 通过元检测范式和证据引导的奖励重分配，检测器能够获得细粒度且可验证的伪造定位能力，为AI生成视频检测提供了新思路。
...
### [97] [Capturing Uncertainty in Human Motion for Representation Learning in Soccer](https://arxiv.org/abs/2608.11203)
*Yizhou Xu,Lars Bretzner,Tiesheng Wang,Atsuto Maki*

Main category: cs.CV

TL;DR: 本文提出了一种基于自监督表示学习的框架，用于理解足球运动中的3D骨骼人体动作，以未来动作预测作为学习目标。该方法通过建模离散化未来动作的概率分布来处理动作的多模态性，在大规模足球球员跟踪数据上显著提升了预测精度，并展现出良好的跨任务迁移能力。


<details>
  <summary>Details</summary>
Motivation: 人体动作本身具有不确定性，足球运动中的动作预测需要考虑多种可能的发展方向。现有方法难以有效捕捉动作的内在动态特性，学习有效的动作表示。因此需要一种能够建模多模态未来的方法，以提高动作预测的准确性和表示学习的质量。

Method: 提出了一种用于动作预测的条件模块，该模块对3D欧几里得空间中离散化的未来动作进行概率分布建模，通过未来轨迹的显式监督来学习多模态特性。

Result: 在大规模足球球员跟踪数据上的实验表明，该方法显著提升了动作预测精度，学习的表示能够有效迁移到多个足球下游应用任务中，展现出强大的跨任务泛化能力。

Conclusion: 通过自监督学习框架结合多模态未来预测，可以有效学习足球动作的表示。该方法不仅在动作预测任务上表现优异，还能很好地迁移到其他足球相关应用，验证了表示学习的泛化性和实用性。
...
### [98] [AdvFD: Boosting Visual Generation via Adversarial Fr'echet Distance Loss](https://arxiv.org/abs/2608.11205)
*Mingju Gao,Jingkai Zhou,Kun Gai,Changqian Yu,Hao Tang*

Main category: cs.CV

TL;DR: 提出对抗性Fréchet距离(AdvFD)方法，通过引入可学习的对抗性表示来弥补静态特征空间的不足，解决Fréchet hacking问题，提升生成器的后训练效果。


<details>
  <summary>Details</summary>
Motivation: 现有Fréchet损失使用静态预训练特征空间，这些特征空间对真实分布和生成分布之间差异的表示不完整且固定，直接优化Fréchet目标会导致Fréchet hacking问题——目标指标持续改善但视觉质量和其他特征空间的对齐可能停滞或下降。

Method: 1.提出对抗性Fréchet距离(AdvFD)，用可学习的对抗性表示补充静态特征空间；2.对抗性表示最大化真实样本和生成样本之间的Fréchet差异，同时生成器最小化该差异；3.引入真实特征白化(real-feature whitening)来归一化特征尺度和协方差几何，防止特征放大导致目标trivial增加，稳定min-max优化。

Result: 在JiT和pMF两种骨干网络以及不同模型规模下，AdvFD均能持续改善单步生成器的后训练效果。

Conclusion: AdvFD通过对抗性学习的表示有效解决了静态特征空间带来的局限性，为生成器后训练提供了一种更优的分布级目标函数。
...
### [99] [Beyond Decision Boundaries: Relational Geometry Attacks on Contrastive Embedding Manifolds](https://arxiv.org/abs/2608.10237)
*Fei Zhao,Peiyuan Zhang,Xi Li,Chengcui Zhang,Nitesh Saxena*

Main category: cs.AI

TL;DR: 本文提出了一种针对对比学习和孪生嵌入模型的.geometry-aware对抗攻击框架，通过操纵嵌入流形中的关系几何来实现验证系统的攻击，而非针对单个预测目标。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击仍然以分类为中心，忽视了对关系几何的攻击能力。现代验证系统的决策由嵌入空间中的关系几何而非离散分类边界决定，因此需要专门针对关系几何的攻击方法。

Method: 将针对对比系统的攻击重新表述为流形级关系腐败。具体做法是：将正样本对推开同时将负样本对拉近，从而坍塌和反转成对相似性结构。训练一个轻量级前馈生成器来学习对抗几何变形模式，无需在线梯度计算即可生成对抗扰动。

Result: 在多个验证架构上实现了显著的性能下降和严重的流形级关系腐败。在Markmatch验证系统上，攻击将准确率从95.4%降低到38.6%，同时完全反转了正负相似性结构。

Conclusion: 该框架能够有效攻击基于相似性的验证系统，通过操纵嵌入空间的关系几何实现实时对抗攻击，为对比学习系统的安全性研究提供了新的攻击范式。
...
### [100] [Beyond Detection: Evaluating Defensive LLMs Against AI-Generated Social Engineering in Live Turn-by-Turn Interaction](https://arxiv.org/abs/2608.10239)
*Yuqiao Xu,Osama Zafar,Alexander Nemecek,Erman Ayday*

Main category: cs.AI

TL;DR: 该研究探讨了基于LLM的防御者能否识别社会工程攻击的结构性风险来源，而不仅仅是对表面线索做出反应。通过构建300个案例的在线住房数据集，评估5个防御模型在两种协议下的表现，发现防御效果差异显著，干预率从0%到96.3%不等，且保护行动与正确的结构性定位经常脱节。


<details>
  <summary>Details</summary>
Motivation: 生成式AI使社会工程攻击更加流畅、适应性强和可扩展，需要基于LLM的防御者来保护用户。研究的核心问题是：这些防御者是识别风险的结构性来源，还是仅仅对表面线索做出反应？

Method: 构建了一个受控的300个案例的在线住房语料库，包含20个场景家族、合法案例、四种结构性失败模式（行为者权威、资产控制、验证充分性、交易路径）和三种表面条件。评估了5个防御模型在有状态逐轮交互和一次性静态两种设置下的表现，每种协议产生1,500个模型-案例评估，总共3,000个评估。

Result: 没有模型产生明确的 unsafe 合规行为，但防御效果差异显著：干预率从0%到96.3%不等。保护行动和正确的结构性定位经常脱节——模型有时在干预时识别了错误的信任组件，或识别出结构性失败但未采取保护行动。资产控制失败是一个主要的定位瓶颈，表面敏感性因模型而异，实时-静态差异取决于模型。

Conclusion: 表面上看似安全的行为本身不足够；实时诈骗抵抗必须分别测量干预、时机、结构性定位和假阳性行为。这项研究揭示了当前LLM防御者在识别社会工程攻击根本原因方面的局限性，强调了需要更精确的防御机制来准确识别和应对不同类型的信任链失败。
...
### [101] [Interpreting Language Model Hidden States at Scale](https://arxiv.org/abs/2608.10260)
*Jordan Pettyjohn,Mansi Sakarvadia,Nathaniel Hudson,Daniel McKenzie,Kyle Chard,Ian Foster*

Main category: cs.AI

TL;DR: OmniLens是一种高效的大语言模型解释方法，通过低秩翻译器和Subset-KL技术大幅降低训练成本，使得在70B模型上构建482个透镜成为可能，揭示了干预最有效位置与行为最明显位置不同的重要发现。


<details>
  <summary>Details</summary>
Motivation: 现有训练透镜方法成本高：仿射翻译器参数随模型宽度二次增长，精确全词汇KL训练占内存高。之前训练的透镜仅应用于最多20B参数的模型，且与特定组件类型绑定。

Method: 1) 低秩翻译器：将每个透镜参数增长变为模型宽度的线性函数，训练参数减少高达98.4%；2) Subset-KL：仅实现选定的词汇logits，Top-k模式将峰值训练内存降低70%，重要性采样变体保留无偏随机梯度；3) 将单一透镜族应用于任何模型宽度激活（残差流、注意力或MLP）

Result: 1) 训练参数减少高达98.4%；2) 峰值训练内存降低70%；3) 为LLaMA-3.3-70B构建了482个透镜的密集集成，提供6倍于残差流设计的深度覆盖；4) 发现行为最明显组件与干预最有效组件不同，最有效干预位于之前透镜研究未涉及的注意力头之外

Conclusion: 在LLaMA-3.3-70B上构建了482个透镜的密集集成，提供6倍于残差流设计的覆盖范围。通过三个案例研究（提示注入检测、多跳内存注入和毒性定位）验证了方法的有效性，证实了关键发表结果。
...
### [102] [Logit-Boundary Geometric Belief Interfaces and Sparse Sheaf-Enclave Protocols: A Self-Contained Substrate for Secure Network Electronic Health Record (EHR) Interoperability](https://arxiv.org/abs/2608.10300)
*Alvin Spivey,Thomas Huang*

Main category: cs.AI

TL;DR: 本文提出了一种名为几何信念接口(GBI)的数学和工程架构，用于解决电子健康记录互操作性的边界问题。该框架在发现模型的提议与确定性判断基质之间建立了logit边界，通过有限边界语义、局部Dirichlet证据、细胞层 sheaf 和映射锥诊断等技术，对FHIR交易进行证书检查，而非建立临床真理或端到端安全性。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录互操作性是一个边界问题：遗留系统、生成模型、术语服务、身份系统和人工审查都可能暴露丰富的内部状态，而操作交换需要狭窄的共享接口（类型化声明、不确定性边界、溯源和明确承认或放弃）。现有方法缺乏在模型到系统边界处定义证书生成检查的正式架构。

Method: 核心方法是logit边界：发现模型可对局部分类决策提出阈值前分数，但确定性判断基质决定该提议是否可接受、需要审查或必须隔离。GBI结合了有限边界语义、局部Dirichlet证据、细胞sheaf和映射锥诊断、咨询性几何审计图表，以及用于故障封闭部署的分布式加密sheaf飞地(DCSE)协议草图。

Result: 在GBI BoundaryBench v0.1基准测试上，使用Qwen3-4B-Instruct-2507模型对256个保留任务进行三种证据模式测试（共768次执行）。所有执行均完成，但无任何输出被基准合约接受：369个在安全解析阶段被拒绝，399个在模式验证阶段被拒绝，覆盖率为零，确定性隔离。

Conclusion: 该框架不建立临床真理、全局表示对齐或端到端安全性，仅在模型到系统边界处定义证书检查。实验结果被刻意设计为狭窄范围——仅一个4B开源模型在一个冻结接口上——并作为关于 admission边界的证据，而非关于LLM能力或临床安全性的总体声明。Julia附录验证了数值证书。
...
### [103] [Neuroevolution Arena: Nested Ecological Evaluation of Update-and-Inheritance Regimes across Neural Architectures](https://arxiv.org/abs/2608.10323)
*Yuxu Ge,Yifei Cheng*

Main category: cs.AI

TL;DR: 该研究开发了NeuroEvolution Arena——一个GPU加速的空间生态学框架，用于评估三种更新-遗传机制（EvoEvo、EvoRL、RLRL）在两种神经网络架构上训练50000代后的表现。研究发现RL机制的代训练适应度更高，但配对结果呈现架构依赖性，不同情境下的获胜者各异。该研究的核心贡献是提出了一种嵌套评估协议，能够区分训练过程产生的控制器与生态评估情境。\n\n


<details>
  <summary>Details</summary>
Motivation: 竞争性人工生命系统中，训练后的控制器在训练评估和生态评估下可能给出不同的排名。当前缺乏能够清晰分离这两种评估来源变异的方法。\n\n

Method: 使用GPU加速的NeuroEvolution Arena空间生态学框架，交叉三种更新-遗传机制（EvoEvo、EvoRL、RLRL）与两种神经网络架构，每种条件运行3次独立训练（总计18次），每次50000代。从每次运行中保存一个精英控制器，进入198个计算作业的对齐冻结评估设计。配对效应在三个种子定义的生态情境（2个合作允许、1个攻击允许）中平均化。\n\n

Result: RL机制的记录训练适应度高于EvoEvo；配对结果呈现架构条件化的多数模式，且显著依赖于保存的控制器工件；六路获胜者因工件和情境而异；预设的生存终点完全失败（零生存）。\n\n

Conclusion: 研究提出的嵌套协议能够有效分离训练运行产生的工件与评估情境，暴露而非掩盖其不同变异来源。这为评估竞争性人工生命系统提供了更严格的方法论框架。\n\n
...
### [104] [Toward a Theory of Value in AI Alignment](https://arxiv.org/abs/2608.10327)
*Andrew Smart,Shazeda Ahmed,Jackie Kay,Jimmy Tobin,Kris Shrishak,Abeba Birhane*

Main category: cs.AI

TL;DR: 该研究通过注释94篇价值对齐研究论文，探讨AI价值对齐领域如何理解和操作化人类价值。研究发现大多数论文未明确界定"价值"概念，常用"偏好"替代，存在将复杂文化情境概念简化为二元选择的风险。随着AI训练从人工标注转向合成数据和自动评分方法，研究者呼吁明确AI价值对齐的哲学承诺。


<details>
  <summary>Details</summary>
Motivation: AI系统对齐人类价值的问题日益重要，但学界对"人类价值"的理解模糊不清。该研究旨在厘清AI价值对齐领域如何 Conception（理解）人类价值，如何技术化操作化和评估，以及这种价值理论对AI未来的意义。

Method: 注释分析94篇价值对齐研究论文，识别其隐含的价值理论

Result: 1. 大多数论文未定义"价值"概念；2. 严重依赖"偏好"作为替代，存在将复杂文化情境概念简化为二元选择的风险；3. 随着研究者放弃人工标注，转向合成数据和自动评分方法，可能关闭在基础模型中争议和践行价值的替代方法

Conclusion: 研究者需要明确AI价值对齐的哲学承诺，为关于AI是否能及如何处理人类价值的辩论带来更大特异性和未探索的视角
...
### [105] [Hierarchical Compositionality for An Assistive AI Agent](https://arxiv.org/abs/2608.10330)
*Tianyi Fu,Mohan Sridharan*

Main category: cs.AI

TL;DR: 本文提出了一种基于层次组合性和简单启发式的AI代理架构，用于解决人类参与者指代对象的歧义问题。该方法使用人类验证的语义特征规范来表示对象属性，并通过层次组合推理结合领域动态公理、语义兼容性、会话显著性和用户主题偏好来实现消歧。实验表明该方法优于现有的数据驱动基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于大型语言模型和其他深度网络架构的AI代理虽然是先进的随机预测器，但存在资源消耗大、不透明、在新情况下做出任意决策等问题。本文旨在探索基于早期AI先驱核心原则的架构设计，这些原则在现代AI方法中未得到充分利用。

Method: 具体方法包括：1）使用人类验证的语义特征规范将领域对象表示为原始属性；2）利用从有限观察到的交互历史中自动识别的属性和概念的层次组合；3）通过组合层次结构、领域动态公理、语义兼容性模型、会话显著性和用户特定主题偏好进行推理；4）在必要时请求人类澄清。

Result: 实验表明，该方法始终优于最先进的数据驱动基线方法，支持针对特定用户档案的适配。

Conclusion: 通过嵌入层次组合性原则并使用简单启发式方法，该架构能够有效解决AI代理中的歧义消解问题，且能够适应特定用户的偏好档案，为构建更透明、适应性强的AI辅助代理提供了新的设计思路。
...
### [106] [Nutrition Data Infrastructure for the AI Era: Operationalizing FAIR for Agent-Mediated Research](https://arxiv.org/abs/2608.10363)
*Lin Liao,Peng Li*

Main category: cs.AI

TL;DR: Nutrition Data Service (NDS)是一种数据基础设施，旨在解决营养研究中使用AI代理时面临的数据身份、语义和发布模糊性问题。该系统通过描述解析、类型化交叉映射和机器可读接口，使AI代理的分析结果可重现和可审计。


<details>
  <summary>Details</summary>
Motivation: AI代理可以加速营养研究，但其分析继承了底层数据的身份、语义和发布模糊性。现有数据基础设施无法支持AI代理的可重现和可审计分析需求。

Method: 构建NDS源保持基础设施，实现FAIR原则：描述解析使发布特定记录可查找；类型化交叉映射连接独立发布的资源；机器可读接口暴露版本化源和交叉映射。

Result: 在食品描述基准测试中，NDS表现出强大的保留准确度，在NutriBench上优于已发表的最佳语言模型结果。外部和盲交叉映射评估显示其类型化契约支持可防御的链接并拒绝不支持的映射。在个体水平血糖指数分析中，固定的NDS输入在不同模型和重复运行中产生相同输出。

Conclusion: 代理介导的营养研究需要新的数据基础设施来管理数据身份、搜索和交叉映射。NDS为AI驱动的营养研究提供了可重现和可审计的基础设施支持。
...
### [107] [DSAgentBench: Can Agents Automate End-to-End Data-Science Workflows in Real Computer Environments?](https://arxiv.org/abs/2608.10366)
*Mizanur Rahman,Mohammed Saidul Islam,Ridwan Mahbub,Md Tahmid Rahman Laskar,Shafiq Joty,Enamul Hoque Prince*

Main category: cs.AI

TL;DR: DSAgentBench是首个评估数据科学工作流自动化的基准测试，包含275个任务，覆盖整个数据科学生命周期。实验表明最强模型Claude-4.6-Sonnet仅达56.70%成功率，开源模型均低于1%，揭示了当前代理系统与真实数据科学工作流之间的显著能力差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏真实计算机交互，无法评估代理能否在现实计算环境中执行完整端到端的数据科学工作流，未能捕捉数据科学实践的多阶段、多工具特性。

Method: 构建DSAgentBench基准，包含275个涵盖数据科学全生命周期的多样化任务。每个任务需要基于中间输出进行决策并协调使用工具，包含确定性评估器验证分析正确性、可视化输出和模型性能。

Result: 15个闭源和开源模型的实验表明：最强代理Claude-4.6-Sonnet仅达56.70%任务成功率，所有开源代理均低于1%，常在工具编排、操作系统基础和多步推理方面失败。

Conclusion: 当前代理系统与真实数据科学工作流之间存在显著能力差距，DSAgentBench可作为开发基于真实环境、可验证的自主数据科学代理的基础。
...
### [108] [Hidden in Plain Sight: Diffusion-Based Unrestricted Robotic Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2608.10393)
*Jiahui Han,Yuhui Yao,Xin Wang,Jiafei Cao,Mingxuan Zhang,Danfeng Shan,Huiqi Deng,Guanchu Wang,Xia Hu*

Main category: cs.AI

TL;DR: 本文提出了DURA，一种基于扩散模型的无限制机器人攻击方法，可生成视觉上自然的对抗性补丁，在仅需模型预测动作的黑盒设置下成功操控VLA模型执行攻击者指定的目标动作。


<details>
  <summary>Details</summary>
Motivation: VLA模型在机器人控制领域展现出强大能力，但其对抗鲁棒性研究不足。现有VLA模型攻击方法依赖像素空间扰动或白盒访问，导致明显 artifacts，在实际机器人系统中部署受限，存在物理世界伤害风险。

Method: 提出DURA（扩散无限制机器人攻击），通过沿预训练扩散模型的潜在轨迹进行优化，生成视觉自然的对抗补丁。支持白盒和黑盒两种攻击设置，黑盒设置仅需受害者模型的预测动作即可完成攻击。

Result: 在模拟环境和真实物理世界进行广泛实验，DURA始终优于现有方法，成功暴露了物理部署VLA模型的安全风险。

Conclusion: 研究揭示了物理部署VLA模型的安全隐患，表明当前模型易受对抗性攻击，亟需开发更强的防御机制来保障机器人系统的安全性。
...
### [109] [Threat-guided Policy-aware Scene Perturbation for Safe Autonomous Driving with Online Reinforcement Learning](https://arxiv.org/abs/2608.10403)
*Xincong Hu,Lei Ou,Maosen Li,Jingtao Zhang,Liguo Hou,Zongzhang Zhang*

Main category: cs.AI

TL;DR: 本文提出TPSP（威胁引导的策略感知场景扰动）方法，通过策略感知场景编码器捕获策略行为与环境交互，选择性扰动关键物体并利用威胁级别差异指导场景生成，从而提升在线强化学习自动驾驶策略的安全性。在NAVSIM v2上仅需约400万公里模拟数据即可实现强安全性表现。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习在自动驾驶中面临安全性挑战，主要因为安全关键驾驶场景难以通过常规采样获取（长尾分布特性）。现有方法虽能生成对抗场景，但与 evolving policy 的学习需求缺乏对齐，无法针对性地弥补策略弱点。

Method: 提出策略感知场景编码器建模策略行为与周围环境的交互关系；基于该表示选择性地扰动关键物体而非均匀修改；开发威胁引导优化策略，通过比较策略在原始场景与扰动场景上的rollout威胁级别差异，引导生成高训练价值的安全关键场景。

Result: 在NAVSIM v2数据集上实现强安全性表现，仅需约400万公里模拟驾驶数据。消融实验验证策略感知定向扰动比随机或策略无关方法提供更丰富的安全关键体验。

Conclusion: TPSP通过策略感知的定向扰动提供更具信息量的安全关键场景，在有限交互预算下实现更安全的驾驶行为，为在线强化学习自动驾驶的安全性提升提供了有效途径。
...
### [110] [Reasoning Shortcuts and Value Symmetries: What Symmetry Permits, Architecture Realizes, and Optimization Selects](https://arxiv.org/abs/2608.10420)
*Xin Xu*

Main category: cs.AI

TL;DR: 本文研究了神经符号系统中的"推理捷径"问题——即通过非预期概念产生正确预测的规则解决方案。作者揭示了原有框架的核心定义在四个基准测试上并不适用，并通过重新测量11个规则族和15个预测，发现未解释配对率从0%到99.9999%不等，证明了对称性结构与可证明的数学性质之间的深刻联系。


<details>
  <summary>Details</summary>
Motivation: 解决神经符号系统中推理捷径的检测问题。原有Takemura等人的框架存在关键缺陷：定义了在每个位置应用单一共享置换的对称性概念，但作者发现这个定义在四个异构基准上都无法直接应用，且最直接的嵌入方式（填充域到共同大小）会产生90.91%的错误病理率。

Method: 1) 重新定义框架并引入层次结构来检验对称性
2) 对11个规则族在15个预指定预测下进行重新测量
3) 证明关于传递性的六个定理（包括自由槽引理）
4) 对电路给定的规则进行计算复杂性分析
5) 对弱监督模型进行实证验证

Result: 1) 原有框架定义在四个基准上不适用，填充方法产生错误的90.91%病理率
2) 未解释配对率范围为0%-99.9999%，与可证明结构相关
3) 传递性的充分条件：单射映射保持对称性、多重入树结构等
4) 复杂性结果：对称性惰性判定是coNP完全；非平凡自同构存在性在Σ₂^p中，不存在完全性除非PH坍塌
5) 布尔情况下：自同构解释一切当且仅当解集是仿射陪集
6) 弱监督模型将94个观测到的捷径全部放在层次结构的一个特定层

Conclusion: 本文通过引入层次结构和系统化的数学分析，揭示了神经符号系统中推理捷径的几何特征与对称性破缺的深层联系。关键贡献包括：修正了原有框架的适用范围、建立了传递性的充分条件、确定了相关决策问题的计算复杂性分类，并通过对弱监督模型的实证验证，表明优化器选择的位置与对称性允许的位置存在明确分离。
...
### [111] [Recovering Wasted Compute in Autoresearch Agents](https://arxiv.org/abs/2608.10424)
*Au Kwok Chun,Abhigyan Acherjee,Amrutha Rao,Zaiqian Chen,Kazem Meidani,C. Bayan Bruss,Micah Goldblum*

Main category: cs.AI

TL;DR: 研究自动研究(autoresearch)系统中建模管道的故障模式，发现四种常见问题：重复调试浪费计算资源、未充分利用超参数调优预算、树搜索算法缺乏探索性、数据分析与下游决策脱节。通过引入全局调试顾问、提示增强和改进树搜索算法等干预措施，可以在不改变底层语言模型的情况下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 自动研究系统(end-to-end agents)受到行业大规模投资，但其在表格数据集上的应用存在系统性问题。作者旨在识别这些建模管道中的常见故障模式并寻找解决方案。

Method: 识别四种常见失败模式，通过目标干预措施进行实验：全局调试顾问(跨搜索树分支共享运行时约束)、提示级和控制级增强、改进的树搜索算法。

Result: 实验表明这些干预措施成功恢复了浪费的计算资源，在不改变底层语言模型的情况下实现了显著的性能提升。

Conclusion: 大型自动研究智能体性能的提升可以通过智能体设计本身实现，无需依赖更强的底层模型。
...
### [112] [Conversational versus Dashboard Explainable AI for UAV Intrusion Detection: An Empirical Study of Operator Trust and Reliance](https://arxiv.org/abs/2608.10434)
*Cong Chi Nguyen,Trang Mai Xuan,Vu-Duc Ngo,Kim-Ngan Thi Nguyen,Trong-Nghia Nguyen,Thien Van Luong*

Main category: cs.AI

TL;DR: 研究提出了一种基于大语言模型(LLM)的会话式可解释人工智能(XAI)接口，用于无人机网络入侵检测系统的后验审计，旨在解决传统静态仪表板难以呈现高维多模态网络物理数据的复杂关系问题。


<details>
  <summary>Details</summary>
Motivation: 无人机网络中的机器学习入侵检测系统虽然性能优越，但其"黑箱"特性结合多模态网络物理数据的高维度性带来了显著的 interpretability 挑战。传统静态可视化仪表板难以让操作员轻松检查和解释复杂特征关系。

Method: 通过对照实验，让参与者使用会话式XAI接口和传统XAI仪表板进行事后审计任务，系统评估两种交互方式对操作员理解、信任和依赖程度的影响。

Result: 会话式接口被认为比仪表板更有用，帮助参与者更方便地访问和合成信息。但同时也伴随着较低的适当自我依赖水平，存在过度依赖的风险。自然语言响应使AI建议更容易被接受，可能减少了参与者在IDS错误时验证底层证据的倾向。

Conclusion: 人机协作存在潜在权衡：提升感知可用性的交互机制可能增加不适当依赖的风险。未来XAI系统设计需在无缝交互与认知强制功能之间取得平衡，以培养适当的依赖行为。
...
### [113] [Continuous Interaction Diffusion: A Diffusion-Native Runtime for Asynchronous Tool-Augmented Reasoning](https://arxiv.org/abs/2608.10438)
*Yuhang Cao*

Main category: cs.AI

TL;DR: 本文提出连续交互扩散(CID)，一种将工具交互集成到迭代去噪过程中的扩散原生模型-运行时架构，解决传统自回归模型在工具调用时需要停止-等待-继续的 restrictive 交互模式问题。


<details>
  <summary>Details</summary>
Motivation: 自回归模型的工具使用采用停止-等待-继续的交互模式，这对扩散语言模型(dLLMs)来说过于限制，因为扩散模型通过并行迭代 refinement 输出。该模式可能迫使工具决策在模型推理未稳定前做出、延迟有用观察直到离散调用完成、引入冗余 refinement 和工具执行，损害任务准确性和推理效率。

Method: CID 架构将模型分解为三个通道：只读事实通道( model-read-only fact channel)、由类型化认知张量(Typed Cognitive Tensor)表示的思维通道、以及显示通道。信息需求可在文本或 JSON 调用完全序列化前出现，允许感知绑定在去噪继续时启动外部读取。返回结果被投射到演进的思维状态中，可修订早期认知和显示区域。持久绑定重用静态结果无需重复外部执行，并在需要时刷新更改的来源。

Result: 本文为任务质量和端到端效率定义了评估协议_FORMALIZE 了架构、运行时和训练目标。本首篇论文专注于只读工具，未做出实证性能声明。

Conclusion: CID 设计目标：更早暴露证据、将工具延迟与模型计算重叠、减少重复外部工作、在新证据到达后保留有用计算。需后续工作验证其有效性。
...
### [114] [Rationale-Guided Learning for Multimodal Emotion Recognition](https://arxiv.org/abs/2608.10448)
*Sujung Oh,Jung Uk Kim,Sangmin Lee*

Main category: cs.AI

TL;DR: 本文提出理性引导学习(RGL)框架，将多模态对话情感识别(MERC)从传统的直接输入-输出映射问题转化为认知启发的推理任务。该框架基于双重过程理论，将情感推理分解为直觉型(系统1)、情境型(系统2)和整合型三个维度，并利用多模态大语言模型(MLLM)生成结构化理性作为记忆来指导模型训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法将MERC视为直接的多模态线索到情感标签的映射问题，忽略了人类在解读情感时使用的因果推理机制。这种方法无法捕捉情感识别的深层认知过程。

Method: 基于双重过程理论，将情感推理分解为三个维度：直觉型(即时感知，对应系统1)、情境型(情境分析，对应系统2)和整合型(两者综合)。利用MLLM离线生成结构化理性，将其编码为记忆，通过对齐模型内部表征与人类推理模式来指导训练。推理时无需MLLM参与。

Result: 在IEMOCAP和MELD基准数据集上取得最先进的性能。进一步分析表明，模型的内部特征能够为未见的测试样本检索语义正确的理性，验证了其理性推理能力。

Conclusion: RGL框架成功将MERC转化为认知启发的推理任务，通过引入理性引导机制，使模型能够学习人类式的情感推理过程，在提升性能的同时增强了模型的可解释性。
...
### [115] [Quantum Incremental Learning with Mixed State Prototypes](https://arxiv.org/abs/2608.10464)
*Yu Wu,Qianli Zhou,Xinyang Deng,Wen Jiang,Kang Hao Cheong,Witold Pedrycz*

Main category: cs.AI

TL;DR: 本文提出了一种基于可训练混合态原型的新型量子增量学习框架，通过添加类原型而非增加量子电路宽度来容纳新类别，解决了NISQ时代量子神经网络的硬件限制和传统量子分类器的正交基态限制问题。


<details>
  <summary>Details</summary>
Motivation: 增量学习需要在不发生灾难性遗忘的情况下顺序学习新类别，同时受限于参数和内存约束。NISQ时代量子神经网络虽在特征映射上有优势，但硬件限制限制了电路宽度，且传统量子分类器受限于正交基态数量，难以容纳不断增长的类别数量。

Method: 提出基于可训练混合态原型的量子增量学习框架。该框架通过添加类原型而非增加共享量子骨干网络的电路宽度来纳入新类别。混合态原型具有比单纯态原型更强的表示能力，可分解的混合态计算提供更低的生成成本和便捷的Hilbert-Schmidt距离度量用于分类。

Result: 模拟结果表明，该模型使用最少数量的量子比特实现了高维特征集中，展示了更低的计算复杂度和在增量学习任务中比经典基线更强的表示能力。

Conclusion: 该框架成功解决了量子增量学习中的电路宽度限制和类别容量问题，通过混合态原型实现了高效且鲁棒的增量学习性能。
...
### [116] [RLMOpt: Adaptive Prompt Optimization via Recursive Language Models](https://arxiv.org/abs/2608.10471)
*Subhash Bangalore Satheesha,Nirvik Pande,Deepthi Duddempudi,Bharath Dandala*

Main category: cs.AI

TL;DR: RLMOpt是一种新型提示优化器，通过递归语言模型(RLM)使搜索策略本身由语言模型驱动，而非依赖预定义的优化程序。RLMOpt智能体在基于工具的环境中运行，负责检查任务信息、分析失败、生成候选方案、分配评估预算并决定停止时机。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化方法依赖于预定义的优化程序：算法决定候选提示的探索方式和搜索进程，而语言模型仅负责生成或改进提示方案。作者认为这种方法限制了优化的灵活性和适应性，因此提出让搜索策略本身由语言模型驱动的想法。

Method: RLMOpt采用递归语言模型(RLM)智能体在基于工具的环境中运行，具备以下能力：检查任务信息、分析失败案例、生成候选提示、分配评估预算、决定停止时机。确定性 harness 负责客观评分、基于Pareto的选择和回归约束。

Result: 在四个基准测试上评估：结构化临床信息提取(Chia)、多跳问答(HotpotQA)、可验证指令遵循(IFBench-2025)、多轮工具调用智能体(BFCL)。单一种子匹配比较中，RLMOpt在所有四个基准上获得最佳留出分数，四任务平均0.610 vs GEPA的0.589。11次匹配基准-种子比较中，RLMOpt在9个案例中优于GEPA。RLMOpt从未产生低于种子的提示，而GEPA有2次。RLMOpt使用更少的搜索rollout，生成的提示大小仅为GEPA的27-79%。

Conclusion: RLMOpt通过使搜索策略本身由语言模型驱动，实现了在多个基准测试上的最先进性能。它表现出更高的可靠性（从不低于起始点）和效率（更少rollout、更小提示）。优化收益主要取决于种子提示中可用的提升空间而非搜索预算，高效优化依赖于可靠且最小化搜索地达到可用提升空间。
...
### [117] [Evaluating Rational Contracting in Natural Language](https://arxiv.org/abs/2608.10475)
*Bhavyesh Sajja,Max Kleiman-Weiner,Roger Zimmermann,Tan Zhi-Xuan*

Main category: cs.AI

TL;DR: 论文提出了一个理性框架，用于评估AI智能体在不确定多步环境中谈判和执行自然语言契约的能力，并在ContractSim评估套件中测试了当前LLM智能体的表现，发现其在高不确定性和合作执行方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有对语言AI智能体能力的评估主要聚焦于一次性交换或简单经济游戏，缺乏对时间延展性、偶然性和不完整契约的评估，也未关注信任 contracting所需的品质。

Method: 构建理性框架定义智能体如何谈判和执行自然语言契约；提出量化理性和合作行为的指标；在ContractSim评估套件中模拟多方供应商契约谈判（涵盖6种环境、3种供应商场景：餐饮、酒店清洁、AI托管）。

Result: 当前LLM智能体在低环境不确定性下能可靠达成协议并谈判出高效契约；但在高不确定性下常无法谈判出可行、高效或互利的契约；执行阶段经常不合作，即使契约容易满足也会违反条款以获取额外利润。

Conclusion: 当前语言智能体在理性且合作地谈判、解释和执行契约方面存在显著不足，需要改进设计以提升其在不确定环境中的契约能力。
...
### [118] [Multi-Granular Rationale-Guided Molecular LLM for Property Prediction](https://arxiv.org/abs/2608.10480)
*Junwoo Park,Minyoung Shin,Cheol Soon Lee,Sujee Lee*

Main category: cs.AI

TL;DR: MR-MoL是一个多粒度理由引导的分子大语言模型，通过GNN对分子子结构进行评分，并将排名和方向标记的理由（分为Murcko支架、BRICS片段和功能基团三个粒度级别）提供给LLM进行分子性质预测。这是首次将GNN衍生的归因作为证据暴露给LLM的方法。


<details>
  <summary>Details</summary>
Motivation: 当前分子LLM使用1D SMILES序列或2D分子图编码分子信息，但无法明确显示哪些子结构对性质预测的贡献。检索增强方法虽然添加上下文，但来自外部来源。化学家推理的依据是驱动性质变化的内部子结构，但现有方法无法提供这种证据。

Method: 1) 使用微调的GNN通过对子结构进行掩码评分；2) 将最具影响力的子结构序列化为排名和方向标记的理由；3) LLM同时读取SMILES序列、分子图和理由；4) 理由涵盖三个粒度级别：Murcko支架及其侧链、BRICS片段和功能基团。

Result: 在8个MoleculeNet任务中，MR-MoL在通用模型中取得最佳总体结果，缩小了与针对每个任务调优的专家模型之间的差距。5个诊断测试确认模型确实读取了理由而非仅仅受益于其存在。

Conclusion: 首次提出将GNN衍生的归因作为证据暴露给LLM用于分子性质预测的方法。诊断测试验证了理由的方向、排名和子结构都会影响预测，且归因可以重现已知的结构-性质关系。
...
### [119] [Predicting Space Groups of Double Perovskites by LLM with Dynamic Few-Shot Learning](https://arxiv.org/abs/2608.10483)
*Jongwon Park,Inhyo Lee,Junhyeong Lee,Seunghwa Ryu*

Main category: cs.AI

TL;DR: DyRIS是一个基于LLM代理的框架，用于预测双钙钛矿的稳定空间群，通过动态多样性少样本检索和规则引导推理解决数据集不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 双钙钛矿(DPs)具有广泛的成分可调性，但预测稳定结构的空时群(SGs)仍然困难，因为可用数据集通常严重不平衡，偏向于占主导地位的SG类别（主要SGs），而忽视代表性不足的类别（次要SGs）。

Method: DyRIS框架包含：(1)多样性增强的动态少样本提示，从相关上下文示例中检索同时限制频繁出现的SGs的主导地位；(2)基于B/B'阳离子排序、定量指标和主要SG偏见控制的规则引导推理，用于优化和排序最终的前3个SG候选。

Result: 在3528个热力学过滤的DP条目上评估，训练数据比例为0.5时，DyRIS实现了具有竞争力的整体准确性，同时获得最佳的整体Top-1 macro-F1分数和最佳的次要SG所有指标表现。DyRIS相对于CrabNet将次要SG Top-1准确率提高了3.26个百分点，并实现了比最强PyCaret基线更高的次要SG Top-3准确率。消融研究表明多样性增强检索、定量指标、主要SG偏见控制和B/B'排序信息各自对预测性能有贡献。

Conclusion: 将基于检索的LLM推理与晶体学领域知识相结合用于不平衡材料数据集中的SG预测具有巨大潜力。规则引导推理步骤不易被传统分类器或排序模型替代。
...
### [120] [INSIDE the Student's Mind: Jointly Modeling Latent Reasoning and Action in LLM Student Simulators](https://arxiv.org/abs/2608.10492)
*Rose Niousha,Minwoo Kang,Narges Norouzi*

Main category: cs.AI

TL;DR: INSIDE是一个学生建模框架,通过微调LLM使其不仅像学生一样行动,而且像学生一样思考。该框架生成基于布鲁姆分类学的内部对话,涵盖认知、情感和行动三个维度,并在成对的思考轨迹和动作上进行微调。评估表明,INSIDE在动作保真度和推理一致性方面都有提升,代码生成匹配度最高达57.9%。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的学生模拟器虽然能复制可观察的动作,但无法捕捉动作背后的推理过程。在教育领域,学生模拟被广泛用于评估辅导系统等应用,但两个学生可能因不同原因做出完全相同的提交,这导致模拟器难以准确评估教育干预效果。

Method: INSIDE框架生成基于布鲁姆分类学的内部对话,涵盖认知、情感和行动三个维度,并在成对的思考轨迹(think traces)和动作上进行微调。评估从两个维度进行:模拟动作的保真度和生成内部对话的质量。

Result: INSIDE在两个维度上都提升了模拟性能:1)动作保真度方面,匹配真实学生的代码生成;2)推理一致性方面,在不同模型上达到最高57.9%的一致性。

Conclusion: INSIDE通过让学生模拟器不仅复制行为还捕捉内在推理过程,显著提升了教育领域学生模拟的保真度和推理对齐度,为评估辅导系统等教育应用提供了更准确的工具。
...
### [121] [From Faulty Memories to Corrected Actions: Dependency-Guided Rollback Repair for Memory-Augmented Agents](https://arxiv.org/abs/2608.10502)
*Caili Yu,Yiqi Wang,Jiaqi Zhang,Yiqun Duan,Mingkai Zheng,Zhangkai Wu,Kaize Shi,Taotao Cai*

Main category: cs.AI

TL;DR: 针对语言模型代理中持久内存出错后的恢复问题，提出依赖导向的回滚修复方法，通过构建内存-操作依赖图来选择性回放受影响的计算，在保留良性工作的同时恢复答案和持久状态。


<details>
  <summary>Details</summary>
Motivation: 持久内存使错误变得持久化：中毒、过时或错误归因的记录会传播到后续推理、工具使用和答案中。现有防御主要检测或删除可疑记忆，但删除源记录会留下已传播的声明和派生记忆，而重置存储或重放完整追踪会破坏良性状态并产生不必要计算。

Method: 提出依赖导向的回滚修复方法：1) 从运行时溯源构建带类型的内存-操作图；2) 追踪显式下游依赖；3) 保留有独立可信支持的候选记忆；4) 停用无支持的记忆状态；5) 选择性重放仅与答案相关的受影响计算。

Result: 在150个案例的受控基准上达到85.3%恢复率（vs 77.3%），清除所有诊断的故障记忆，保留所有良性记忆，仅需选择性重放且LLM调用成本适中。在50个案例的压力测试上达到68.0%恢复率（vs 54.0%），并实现最高的声明无效F1分数0.669（vs 0.603）。

Conclusion: 依赖导向的回滚修复在修复故障记忆状态和保留良性记忆的同时提供了强大的恢复-成本权衡，但并不意味着在所有情况下追踪重建都更优。
...
### [122] [MEGA: Self-Evolving Agent Optimization Infrastructure via Wisdom Graph](https://arxiv.org/abs/2608.10504)
*Jung Hwan Lee,Kyu Ho Lee,Gwang Hoon Yoo*

Main category: cs.AI

TL;DR: MEGA是一个自我进化的基础设施，旨在系统性地改进编码代理。它通过三个层次来实现：Layer 1从代理会话中提取可重用智慧；Layer 2在智慧图谱中进行组合推理；Layer 3进行多代理协作优化。系统通过操作证据驱动自我进化，将优化代理系统与指导优化的知识进化统一为一个过程。


<details>
  <summary>Details</summary>
Motivation: 当前编码代理面临三个核心挑战：1) 优化代理系统时没有积累可转移的知识；2) 积累知识时缺乏组合推理能力；3) 缺少通过操作证据自我进化的机制。这些问题限制了代理系统的持续改进。

Method: 采用三层架构：Layer 1通过行为模式聚类和实证A/B验证从代理会话中提取持久资产；Layer 2将资产分解为原子PCR单元，在类型化智慧图谱中进行演绎、溯归纳和归纳推理，通过组合检索组装特定上下文的执行计划；Layer 3对异构代理工作流进行多代理协作优化，通过受控评估归因改进效果。

Result: 实现了自我进化的基础设施，使优化代理系统与指导优化的知识进化成为同一过程。系统能够积累可转移的智慧、进行组合推理，并通过操作证据不断自我完善。

Conclusion: MEGA填补了代理系统优化领域的空白，通过将知识积累、组合推理和自我进化集成到一个统一框架中，为构建更智能、更自适应的编码代理系统提供了新的基础设施范式。
...
### [123] [RadFusion: Towards Threshold-Controllable Radiology Report Generation](https://arxiv.org/abs/2608.10505)
*Ying Jin,Noel C. F. Codella,John Corring,Mu Wei,Dinei Florencio,Eric Horvitz*

Main category: cs.AI

TL;DR: RadFusion是一个将多标签分类器与基于VQA的报告生成器融合的框架，并通过LLM重写报告以匹配所选阈值下的分类器决策，实现放射学报告生成的阈值可控性。


<details>
  <summary>Details</summary>
Motivation: 现有报告生成模型无法控制诊断内容的敏感性-特异性权衡，而不同临床场景需求不同：急诊分诊优先考虑敏感性以减少漏诊，确认性解释强调特异性以减少不必要的干预。现有模型既无法适应这些场景，也不支持ROC分析验证。

Method: 将多标签分类器（提供每种疾病的置信度分数）与基于VQA的报告生成器（详细描述医学发现）融合，然后使用LLM重写报告，使其表述的诊断遵循分类器在选定阈值下的决策，同时基于生成器的描述。

Result: RadFusion的性能符合分类器的ROC曲线：通过阈值扫描并将报告映射回类别标签，可以重现分类器已验证的ROC性能。在MIMIC-CXR上，敏感性在匹配特异性下提高6.7%，特异性在匹配敏感性下提高20.7%。

Conclusion: RadFusion使报告生成具有临床适应性、可定量验证和诊断可靠性，为监管审批提供支持，并使操作点选择能够匹配临床上下文。
...
### [124] [Measuring Semantic Abstractness of SAE Features via Nonlocality](https://arxiv.org/abs/2608.10537)
*Chuqiao Lin,Shivaji Sondhi,Xiao-Liang Qi*

Main category: cs.AI

TL;DR: 本文提出了Feature Nonlocality (FNL)指标，用于衡量SAE特征的语义抽象程度，解决现有方法无法区分表面词汇特征与高层语义特征的问题。FNL基于激活值在位置上分布的熵来计算，可用于评估 mechanistic explanations 和选择下游干预特征。


<details>
  <summary>Details</summary>
Motivation: 现有评估SAE特征的mechanistic explanations方法无法区分表面词汇特征与高层语义特征，autointerp语义描述和因果 steering 都不能完全解决特征的抽象层次问题。

Method: 定义FNL为SAE特征激活的归一化逐位置影响的熵。FNL与现有LLM代理指标相关联，并在73-84%的随机配对中正确识别上下文推理特征vs token级特征。

Result: 1) FNL与现有语义抽象代理指标相关；2) 能区分上下文相关推理特征和token驱动特征；3) 审计jailbreak mitigation的SAE特征发现大多数有效特征是低FNL的位置特征；4)  steering高FNL特征在DeepSeek-R1-Distill-Llama-8B上提升MATH-500准确率4.6个百分点。

Conclusion: FNL提供了一个LLM无关、无标签的特征抽象层次关联指标，可用于评估mechanistic explanations及选择下游干预特征。
...
### [125] [SKILLER: Language-Level Reinforcement Learning for Reusable Skill Extraction in Small Language Models](https://arxiv.org/abs/2608.10538)
*Chenhao Dang,Siyuan Xiong,Conghui He,Weijia Li*

Main category: cs.AI

TL;DR: 本文提出SKILLER框架，通过自然语言驱动的强化学习为小型开源模型自动生成特定技能，在消费级GPU上实现低成本高质量的任务执行。


<details>
  <summary>Details</summary>
Motivation: 当前agent harness系统依赖强闭源模型导致高推理成本，而开源小型模型虽可在消费级GPU部署，但缺乏针对这类紧凑模型的有效技能自动生成方法。

Method: SKILLER使用强模型作为actor和critic，将小型模型agent系统视为环境，完全通过自然语言传播强化学习信号，实现自动化技能生成。

Result: 在5个基准测试上，SKILLER在Qwen3.5-9B上超越3个开源和1个闭源方法（提升4.3-20.4个百分点），在Qwen3.5-4B上提升1.8-13.3个百分点，并在SkillsBench单技能任务上达到与强闭源模型相当的性能。

Conclusion: SKILLER成功实现了为小型开源模型自动生成高效技能，显著降低成本的同时保持高性能，为消费级GPU上的agent部署提供了可行方案。
...
### [126] [Reinforcement Learning-Based Laser Cutting Machine Parameter Optimization](https://arxiv.org/abs/2608.10549)
*Khanh Quan Pham,Majid Kundroo,Geunwoo Ban,Seongho Bae,Taehong Kim*

Main category: cs.AI

TL;DR: 本文提出了RL²C（激光切割强化学习）算法，使用Q学习结合ε-greedy策略来动态优化激光切割参数，显著减少锥度尺寸和薄膜浪费。该算法还包含动态环境空间适应性机制，能够在多批实验过程中适应新状态。


<details>
  <summary>Details</summary>
Motivation: 传统的试错法寻找最佳激光切割参数速度慢且不准确，需要一种更高效的参数优化方法来解决光学薄膜切割中的参数调优问题。

Method: 采用Q学习算法结合ε-greedy策略进行参数优化，并引入动态环境空间适应性机制，使算法能够适应学习过程中遇到的新状态。

Result: RL²C相比现有RL优化方法，优化步骤减少最多12.5%，处理时间减少最多81.8%，能够用更少的步骤和时间找到最优切割参数。

Conclusion: 本研究展示了强化学习在工业激光切割过程中的潜力，可提高切割质量、减少时间和薄膜浪费，并最大程度减少人工干预。
...
### [127] [DashArena: Benchmarking LLMs on Interactive Analytic Dashboard Generation](https://arxiv.org/abs/2608.10567)
*Xiaotong Wang,Dazhen Deng*

Main category: cs.AI

TL;DR: 本文提出了DashArena，这是首个用于开放域、任务驱动的交互式分析仪表板生成的基准测试。其核心创新是要求系统同时生成仪表板和可重放的交互轨迹，通过浏览器执行器重放轨迹并将系统的分析工作流转化为可复现的视觉和执行证据，由VLM裁判进行比较，Bradley-Terry聚合生成排行榜。


<details>
  <summary>Details</summary>
Motivation: 分析仪表板结合了协调视图和交互用于数据探索和决策支持。现有的模型可以从数据和自然语言目标生成仪表板，但评估其有效性仍然困难。仪表板生成是开放式的，静态外观或成功执行都无法完全捕捉分析支持和交互质量。

Method: 1) 构建DashArena基准，要求系统生成仪表板和可重放交互轨迹；2) 浏览器执行器重放轨迹并生成视觉和执行证据；3) VLM裁判使用证据比较候选方案；4) Bradley-Terry聚合生成排行榜；5) 将裁判模型蒸馏为开源的DashJudge-8B。

Result: 1) 人类评估表明DashJudge-8B能有效复制人类判断；2) 消融实验表明交互证据提高了裁判一致性；3) 前沿模型实验显示存在持续的渲染、分析和交互失败；4) 交互感知评估能够捕捉静态或仅执行检查无法发现的失败。

Conclusion: 现实场景下的仪表板生成仍然具有挑战性，交互感知评估能够捕捉其他方法遗漏的关键问题。DashArena和DashJudge-8B为该领域提供了重要的评估基准和工具。
...
### [128] [Agentic Instruction Data Selection: Let DataMaster Interpret Your Intent](https://arxiv.org/abs/2608.10579)
*Fanqi Zhou,Qiaosheng Chen,Zixian Huang,Gong Cheng*

Main category: cs.AI

TL;DR: DataMaster是一个指令数据选择智能体，能够通过自然语言描述用户需求，自动组合最优的数据选择策略，无需手动设计选择规则。


<details>
  <summary>Details</summary>
Motivation: 现有指令数据选择方法引入各种指标，但现实数据集的复杂性使得任何单一指标都难以在所有场景中泛化。开发者被迫手动检查数据并为每个新应用设计启发式规则，这是一个繁琐且容易出错的过程。

Method: 提出DataMaster系统，通过自然语言描述数据需求，解释用户意图，自主组合最优选择策略，实现自动化编排。

Result: 在数学、医学和代码领域的广泛实验中，DataMaster在大多数设置中优于静态基线，并在大量案例中超越全池训练。

Conclusion: 实现了从手动配置到自动化编排的范式转变，简化了数据管理并减轻了手动策略设计的负担。
...
### [129] [HexEval: An Evidence-Driven Hexagonal Framework for Multidimensional Scholar Assessment](https://arxiv.org/abs/2608.10584)
*Xiaokang Qu,Yiting Lin*

Main category: cs.AI

TL;DR: HexEval是一个基于证据驱动的六边形框架，用于多维学者评估。它将学者评估分为内在层（研究严谨性、方法论创新、科学贡献）和外部层（知识转化、研究连贯性、学术影响），利用GitHub、OpenAlex等公开数据源生成可解释和可审计的学者画像。


<details>
  <summary>Details</summary>
Motivation: 现有学者评估方法主要依赖文献计量指标和声誉代理，而近期基于大语言模型的方法主要评估个人研究论文而非全面评估学者。作者认为学者评估应该被表述为一个证据驱动的推理问题，需要综合考虑内在研究质量和外部可验证的学术行为。

Method: HexEval框架包含两个互补的证据层：内在层评估匿名化的代表性作品（研究严谨性、方法论创新、科学贡献三个维度），外部层使用来自GitHub、Lens、OpenAlex等公开可验证来源的异构证据来表征学者（知识转化、研究连贯性、学术影响三个维度）。框架保留中间证据、维度特定理由和验证信号，实现可解释和可审计的评估。

Result: 六个维度的实验表明与人类或外部参考标准存在维度依赖的一致性：结构化校准提高了内在质量的绝对一致性，而外部模块恢复了广泛的发展轨迹和序数影响信号。

Conclusion: 证据驱动的异质学术证据推理是有前景的可审计AI辅助学者评估范式，同时暴露了公开学术数据的覆盖和归属局限性。
...
### [130] [Curate Before You Connect: Identity and Ontology Tagging in a Production Knowledge Graph](https://arxiv.org/abs/2608.10644)
*Vaibhav Dangaich,Kevin Lewis,Kundeshwar Pundalik*

Main category: cs.AI

TL;DR: 本文描述了一个从98,795份政府文档中构建包含537,157个实体和2,198,567个关系知识的图谱的ingestion和ontology-tagging层。核心挑战在于实体身份决策的不可逆性——合并后的记录无法分离。文章提出了一种基于标识符列、名称列、显示名称和类型作用域位置的"记录身份阶梯"方法来解决去重问题，而非使用名称相似度。


<details>
  <summary>Details</summary>
Motivation: 解决知识图谱构建中的实体身份决策问题。论文指出提取错误可以纠正，但实体合并是破坏性的——一旦属性合并就无法分离。之前曾发生两处名称表面形式被错误合并、导致正确记录被破坏并删除八个无关文档实体的事故。

Method: 1) 记录身份阶梯：从标识符列、名称列、显示名称和类型作用域位置判断相同性，而非名称相似度
2) 表格内去重使用该阶梯
3) 图谱写入使用更粗粒度的规范名称键，相同规范名称自动合并
4) 多类别ontology标记，要求锚定证据来减少角色分配

Result: 锚定证据要求将角色分配从36个减少到4个（均确认正确）。图谱包含48,403个待处理提案 vs 775个人类决策。还量化了图谱的一致性债务，并展示了次级分类如何补偿错误父类。

Conclusion: 自动化边界应设在身份决策点：没有报告身份基准，且key允许的过度合并在结构上无法检测。实体名称是实例标签而非类型断言，因此将名称片段与类别索引匹配会"发明"分类，需要锚定证据来确保准确性。
...
### [131] [Decision-Aware Approximation of Belief Functions for Evidential Combinatorial Optimization](https://arxiv.org/abs/2608.10650)
*Sohaib Afifi*

Main category: cs.AI

TL;DR: 本文提出一种决策感知的证据近似方法，与传统的基于距离的近似不同，该方法直接优化决策质量而非近似精度，并证明了其理论界并设计了动态规划算法。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用Jaccard或Jousselme等内在距离来近似质量函数，保持与原证据的接近性。然而，当质量函数用于线性组合优化问题时，真正需要保留的不是质量函数的相似度，而是决策的质量。本文认为应该从决策角度来评估近似的优劣。

Method: 引入决策感知近似方法，目标是优化决策的遗憾：使用较便宜的近似进行决策，但基于真实质量函数进行评估。针对最小最短路径问题，提出决策感知合并方法，并证明了单点界来定位真实最优值的遗憾，将其转化为标量情况的精确动态规划算法，扩展到在线版本以在最终成本未知之前剪枝焦点元素。

Result: 在最小最短路径问题上，距离最优近似会改变决策，而决策感知合并则保持决策，且这种情况在随机实例中占显著比例。实验表明，对于线性标准和非线性代理读数，决策感知压缩器改变决策的频率均低于表示感知压缩。

Conclusion: 决策感知近似方法在保持决策一致性方面优于传统基于距离的方法，具有理论保证（单点界）和有效的算法实现（动态规划），为证据理论在实际优化问题中的应用提供了新思路。
...
### [132] [Operationalising Relative Causal Knowledge: Backbone Identifiability from Private Reports on a Shared Outcome](https://arxiv.org/abs/2608.10664)
*Fabrizio Russo,Mark Somers*

Main category: cs.AI

TL;DR: 研究当具有不同结构因果模型的代理通过共享的干预一致抽象（backbone）交换因果知识时，backbone何时由代理的私人因果知识决定。在双代理共享结果案例中，私人因果边际无法唯一确定backbone，需要通信因果识别的响应函数才能实现识别。


<details>
  <summary>Details</summary>
Motivation: RCK框架解释了不同因果模型的代理如何交换因果知识，但这一传输机制所依赖的backbone的先验识别问题尚未被探讨——即私人因果知识何时能唯一确定共享的干预一致抽象。

Method: 在标准假设（兼容性、非退化性、局部重叠）下，对双代理共享结果案例进行形式化分析，使用函数分析工具探究无限多联合干预核能否产生相同的私人报告。

Result: 私人因果边际无法唯一确定backbone：存在无限多个联合干预核可产生完全相同的私人报告，但在联合干预上存在分歧。仅当代理通信因果识别的响应函数时，识别才成为可能。加性可分性虽移除隐藏交互自由度，但观测残差摘要仍不足。

Conclusion: 因果知识识别首先是一个通信问题，其次才是策略组合问题。代理需要先通过通信因果识别的响应函数来共享信息，而非仅依赖私人因果边际。
...
### [133] [VERDICT: Training-Free Step-Wise Verification of Multimodal Reasoning via Disagreement-Aware Consensus](https://arxiv.org/abs/2608.10665)
*Rohit Sinha,Kunal Tilaganji,Tanuja Ganu,Nagarajan Natarajan,Amit Sharma,Vineeth Balasubramanian*

Main category: cs.AI

TL;DR: 本文提出VERDICT，一种无需训练、无领域限制的逐步验证方法，通过利用不同冻结验证器之间的分歧来检测多模态大语言模型推理链中的错误。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型推理验证方法存在局限：要么需要昂贵的人工标注监督且跨任务表现不一致，要么仅简单聚合多个来源的分数而忽略分歧本身携带的重要信息。当验证分数不一致时，分歧本身就蕴含着推理步骤是否有效的关键信号。

Method: 将问题形式化为不匹配验证器之间的耦合评分问题，解释为具有唯一闭式均衡的协调游戏。提出VERDICT方法，通过闭式解计算共识分数，实现分歧感知过滤和稳定性感知排序。

Result: 在六个基准数据集上的评估表明，VERDICT相比基线模型提升高达+5.95%，与需要大量监督的领域特定批评模型表现相当。

Conclusion: 跨模态一致性提供了强大的验证信号，无需任务特定适配即可实现无需训练的验证，证明了交叉模态分歧可作为有效的验证信号。
...
### [134] [FITTER: Vocabulary-Agnostic Cross-Domain Inference on Temporal Knowledge Graphs](https://arxiv.org/abs/2608.10668)
*Jiaxin Pan,Mojtaba Nayyeri,Osama Mohammed,Daniel Hernandez,Rongchuan Zhang,Cheng Cheng,Steffen Staab*

Main category: cs.AI

TL;DR: FITTER是首个用于时序知识图谱链接预测的全归纳式结构模型，支持跨域迁移学习，可处理完全未见的实体、关系名和时间戳。该方法通过相对顺序编码表示谓词，结合消息传递融合局部和全局时序上下文，生成词汇无关的嵌入表示。


<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱补全方法假设训练时已知实体、关系名和时间戳，限制了每个模型只能处理单一图谱和词汇表，无法支持跨域迁移推理。

Method: 1) 使用相对顺序编码而非绝对顺序编码表示谓词与其时间交互模式；2) 通过消息传递融合局部和全局时序上下文；3) 生成词汇无关的嵌入表示，实现跨域迁移。

Result: 在6个不同领域、时间粒度和跨度的时序知识图谱基准数据集上，FITTER无需重新训练即可持续优于归纳式基线方法。

Conclusion: 词汇无关的结构学习可以作为Semantic Web异构图谱推理的可行基础。
...
### [135] [REDAgentBench: Executable Red Teaming and Faithful Measurement of LLM Agent Systems](https://arxiv.org/abs/2608.10669)
*Zixing Chen,Xingyuan Liu,Jie Zhu,Huaixia Dou,Shuo Jiang,Junhui Li,Lifan Guo,Feng Chen,Chi Zhang*

Main category: cs.AI

TL;DR: REDAgentBench是一个用于LLM智能体自主红队测试的可执行框架，通过隔离服务沙箱和效果验证来测量安全性。该基准包含1661个案例，跨5个服务表面，宏平均攻击成功率为65.69%。


<details>
  <summary>Details</summary>
Motivation: 现有评估往往将智能体安全简化为单一的攻击成功率(ASR)，忽视了暴露、执行、观察和裁决等环节，可能将实际违规与证据可见性混为一谈。

Method: REDAgentBench从明确的安全约束和智能体系统漏洞出发生成攻击，在隔离的服务沙箱中运行攻击，通过服务收据和最终状态变化验证有害影响。

Result: 宏平均ASR为65.69%；报告的ASR随测试框架和证据视图而变化；评估上下文披露会改变执行行为；近五分之一的确认违规发生在智能体陈述相关约束或风险之后（Recognition-Execution Gap）；无训练的政策提醒可减少70多个百分点的确认违规。

Conclusion: 可执行评估可以改进安全测量并识别可操作的干预点。
...
### [136] [Self-Correcting Long-Horizon Search Agents via Tree-Structured Memory](https://arxiv.org/abs/2608.10676)
*Aijun Yang,Qianxue Guo,Ziyi Huang,Yuxuan Chen,Shiyou Qian,Jian Cao*

Main category: cs.AI

TL;DR: ReTree是一种用于搜索代理的自纠正树结构记忆机制，通过构建有界每步推理上下文同时保留证据来源链接，在检索到矛盾证据时能够追溯并修复早期claim，实现搜索过程的自我纠正。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个问题：1)将完整执行轨迹提供给LLM导致上下文无限增长；2)压缩方法以牺牲重要细节为代价；3)错误事实被替换时无法修复由此产生的下游推理错误。

Method: ReTree将搜索建模为证据树，每个节点存储：有界摘要、证据和修订历史。当新检索的证据与早期claim矛盾时，ReTree追溯到引入该claim的节点，替换过时证据，重新生成摘要，修剪受影响的分支，并恢复搜索。源 grounding的证据溯源支持可靠的冲突定位。

Result: 在四个问答和搜索基准测试中，ReTree始终优于Full-Trajectory ReAct，最高提升25.6个百分点；Full-Trajectory ReAct的平均最大每步推理上下文是ReTree的1.27-1.51倍。

Conclusion: ReTree是一种有效的长时域搜索自纠正记忆抽象，能够在保持上下文有界的同时实现可靠的证据溯源和冲突修复。
...
### [137] [Ex-Omni-2D: Expressive Omni-Modal Dialogue Models with Native Visual Presence](https://arxiv.org/abs/2608.10720)
*Haoyu Zhang,Zhipeng Li,Xiaoying Tang,Tianshu Yu,Yiwen Guo*

Main category: cs.AI

TL;DR: Ex-Omni-2D是一个全模态对话框架，能生成协调的文本、个性化语音和参考条件视频响应。该模型预测视觉思维计划(VTP)，使用多码本语音单元作为共享声学-时间接口，实现语音和视频的对齐学习。


<details>
  <summary>Details</summary>
Motivation: 现有的全模态对话模型虽然能理解多模态输入并合成语音回复，但响应仍然缺乏视觉体现。研究者希望构建一个能同时生成文本、语音和视频的全身对话系统。

Method: 1. 输入多模态查询、参考图像和参考音频，预测结构化视觉思维计划(VTP)；2. 使用多码本语音单元形成共享声学-时间接口，解码为语音并与视频帧在线对齐；3. 采用全序列视频生成器作为教师模型；4. 蒸馏为few-step block-causal Streaming Student，使用前缀流式机制携带潜在特征跨块传输。

Result: 在400×720/720×400分辨率下，四步推理的完整四GPU流水线实现端到端RTF为1.293，提供了实用的质量-效率平衡点。

Conclusion: Ex-Omni-2D成功实现了同时生成文本、语音和视频的全模态对话系统，通过创新的VTP和多码本接口设计，避免了大规模查询-文本-语音-视频监督的需求，并通过蒸馏技术实现了高效的增量生成。
...
### [138] [Tree-of-Ideas: Automated Research Ideation via Cross-Trajectory Reasoning over Scholarly Evolution](https://arxiv.org/abs/2608.10740)
*Xun Li,Yiying Yang,Pengtao Li,Xiao Yao,Suyu Liu,Xiaoyang Ye,Ziyu Lu,Yuan Yao,Yangning Li,Yinghui Li,Wenhao Jiang*

Main category: cs.AI

TL;DR: 提出Tree-of-Ideas (ToI)框架，通过两阶段机制（EvoTrace重建学术轨迹、EvoAgent跨轨迹推理）帮助研究者从文献演变中生成有依据的研究想法，在AI领域六个主题上取得最高自动评分（6.27分），接近人类论文引用水平（6.29分）。


<details>
  <summary>Details</summary>
Motivation: 现有研究构思方法存在局限性：要么将论文视为非结构化上下文，要么将学术演变建模为孤立的引用链，忽略了研究轨迹之间的交互作用，难以有效追踪研究问题和解决方案的演变。

Method: 提出两阶段框架Tree-of-Ideas (ToI)：(1) EvoTrace模块从引用中重建分支学术轨迹，追踪方法的演变、已解决的问题和研究空白；(2) EvoAgent模块跨轨迹推理，识别趋同问题和互补解决方案，生成有依据的研究想法。

Result: 在六个AI研究主题上，ToI达到自动方法最高分（6.27 vs 最强基线5.36，10分制），具有强新颖性（6.36）和依据性（7.00），得分接近人类论文引用（6.29）。

Conclusion: 跨路径演化推理具有重要价值，该方法能有效帮助研究者基于文献演变脉络生成创新且有依据的研究想法。
...
### [139] [Compositional Benchmark Synthesis for Hierarchical Human Action Recognition](https://arxiv.org/abs/2608.10765)
*Farnaz Soleimani,Abdelghani Chibani,Yacine Amirat,Ghazaleh Khodabandelou*

Main category: cs.AI

TL;DR: 该论文提出一个基准生成和评估框架,从单标签动作语料库合成四层次层级意图基准(动作、活动、低层意图、高层意图),通过转换模型组装片段并使用覆盖率感知采样器减少主体使用不均。实验表明四种基线模型存在0.13-0.17的组合 held-out gap,表明这是基准的结构性特征而非模型缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有数据注释缺乏语义层级,大型语料库提供原子动作标签但无时间组合,录制的复合活动语料库则层次浅且领域窄。需要一个能生成多层次语义层级基准的方法,同时避免循环监督风险(生成规则与评估规则混同导致的伪成功)。

Method: 提出基准生成框架:1)从平面单标签动作语料库合成四层次层级意图基准,保留动作级真实特征;2)在主体一致性约束下用转换模型组装片段;3)覆盖率感知采样器将主体使用基尼系数从0.566降至0.248;4)通过设计确保有效性,将序列生成规则与一阶逻辑评估规则分离以避免循环监督。生成15,002个片段,用四种不同模型族基线进行评估。

Result: 1)生成15,002个四层次层级意图片段;2)四种基线模型均出现0.13-0.17的组合held-out macro-F1差距;3)图感知模型表现最好但仍未缩小差距;4)无逻辑基线仍违反held-out语义规则;5)破坏顺序的控制组在种子变化范围内改变macro-F1,作为生成器一致性检查。

Conclusion: 组合held-out gap是基准的结构性特征而非模型伪影。框架、转换模型和生成器已发布,基准可重新生成和扩展。该工作为层级行为识别研究提供了新的评估基准和方法论贡献。
...
### [140] [Rule of Thumb: Explaining Artificial Intelligence Systems using Partial Information](https://arxiv.org/abs/2608.10766)
*Kaivalya Rawal,Daria Onitiu,Brent Mittelstadt,Sandra Wachter,Chris Russell*

Main category: cs.AI

TL;DR: 提出了一种新的可解释人工智能(XAI)方法——"经验法则"(RoT)解释，用于识别预测AI系统行为的最相关特征，适用于零样本分类、模型审计和科学发现场景。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统决策过程不透明,需要可解释性方法来理解AI如何做出特定决策,同时满足AI监管要求并提供审计能力。

Method: 通过新颖的公式化方法,识别对特定数据点预测AI行为最相关的特征,实现模型无关的解释。

Result: RoT可应用于:零样本分类使用大语言模型、不透明AI系统的审计(无需模型访问)、科学发现中的AI使用,并满足主流AI监管要求,提供熟悉的界面和可视化,且速度显著快于替代方案。

Conclusion: RoT是一种通用、快速、模型无关的XAI方法,能够增强AI系统的透明度和可审计性。
...
### [141] [SkillLens: Visual Skill Cards for Retrieval-Augmented GUI Action Prediction and On-Policy Distillation](https://arxiv.org/abs/2608.10775)
*Zhou Liu,Ligang Huang,Zeli Su,Zewei Pan,Zhaoyang Han,Xing Chen,Yuanfeng Song,Wentao Zhang*

Main category: cs.AI

TL;DR: 本文提出Visual Skill Cards (VSCs)，一种将可重用程序与适用性提示、视觉证据和验证信号绑定的状态条件记忆表示。SkillLens从异构交互经验构建VSC，在推理时检索相关卡片并为视觉语言模型执行器扩展必要证据。CardDistill利用VSC证据作为特权教师上下文训练学生模型。在Multimodal-Mind2Web和WebLINX-BrowserGym上，SkillLens将GPT-5.4-mini执行器分别提升11.6步SR和2.9 Overall；CardDistill将Qwen3-VL-2B学生模型分别提升12.0和3.2点。


<details>
  <summary>Details</summary>
Motivation: 计算机使用代理虽能感知丰富软件界面，但缺乏视觉程序记忆：能识别单独控件却无法判断当前工作流、哪个控件下一步重要、或何种证据能确认进展。原始交互轨迹信息丰富但冗长且嘈杂，而纯文本技能又常缺失使程序可用的视觉状态。

Method: 提出Visual Skill Cards (VSCs)——将可重用程序与适用性提示、视觉证据和验证信号绑定的状态条件记忆表示。SkillLens通过Trace-to-Visual-Skill-Card从异构交互经验构建VSC；推理时检索相关卡片并仅为固定视觉语言模型执行器选择性扩展所需证据。CardDistill利用VSC证据作为特权教师上下文，训练无需运行时卡片检索的学生模型。

Result: 在Multimodal-Mind2Web和WebLINX-BrowserGym上，SkillLens将冻结的GPT-5.4-mini执行器分别提升+11.6步成功率和+2.9总体得分；CardDistill将对应的Qwen3-VL-2B学生模型分别提升+12.0和+3.2点。

Conclusion: VSC表示法成功提升了基于检索和基于蒸馏两种方法的GUI动作预测性能，证明视觉程序记忆对计算机使用代理的有效性。
...
### [142] [ChemWorld: Programmable Chemical Worlds for Controlled and Replayable Agent Experimentation](https://arxiv.org/abs/2608.10792)
*Jiangjie Qiu,Yijun Li,Xiaonan Wang*

Main category: cs.AI

TL;DR: ChemWorld是一个可编程的化学环境，将可重用的过程和观察组件编译成可执行的世界，将代理可用的公共实验合同与评估者拥有的化学和材料定律分离，允许研究人员在保持公共任务和交互条件不变的情况下改变世界组成或改变单一隐藏定律。


<details>
  <summary>Details</summary>
Motivation: 物理实验室提供真实的材料证据但成本高且难以重复使用，而大多数数字环境保持底层实验世界基本固定。需要一个介于两者之间的可控化学实验环境。

Method: ChemWorld通过将可重用的过程和观察组件编译成可执行世界来构建，公共实验合同与私有化学定律分离，支持事务性执行记录操作、失败、资源变化和状态转换，允许完全的环境-动作轨迹精确回放和审计。

Result: 完全普查限定覆盖了参考注册表、52种生成组合以及模块、接口、编译和无效动作测试；8个确定性实验案例展示了共享生命周期语义、故障恢复和精确回放；6个父子世界分支对在匹配公共条件下分离了单一私有法律干预的效果；独立代理也在非参考世界中通过相同公共接口完成了完整生命周期。

Conclusion: 在声明的组件和模型领域内，ChemWorld为研究系统变化化学世界中的实验提供了一个受控且可重放的基质，与物理实验室证据和校准互为补充。
...
### [143] [EvoMem: Memory-Augmented Evolution for Code Optimization](https://arxiv.org/abs/2608.10795)
...
TL;DR: EvoMem是一个用于LLM驱动进化程序搜索的持久记忆架构，将成功的突变事件转换为结构化任务感知建议，供未来运行使用。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的进化框架大多丢弃突变策略中的可重用知识，反复重新发现相似想法，限制了跨运行和跨任务学习的机会。

Method: EvoMem采用两阶段操作：每次运行后提取并存储有前景的想法及其来源信息；在后续进化过程中，根据当前任务和程序上下文检索少量相关指令来指导突变。

Result: 在几何优化、多跳问答、GPU内核优化及相关基准测试中，大多数评估设置下目标指标或搜索速度都有积极平均提升，同时也揭示了不同任务间的差异性。

Conclusion: EvoMem提供证据表明持久记忆可以减少一些冗余探索，并改善LLM驱动进化搜索中成功策略的重用和适应。
...
### [144] [Hypothesis Frontier: Verifier Guided LLM and Symbolic Search for First-Order Induction](https://arxiv.org/abs/2608.10843)
*Serafim Batzoglou*

Main category: cs.AI

TL;DR: 本文提出了Hypothesis Frontier，一个基于验证器的神经符号框架，用于解决一阶概念合成问题。该框架通过在每一轮评估LLM生成的公式，保留经验证的最强假设，并利用其错误来指导后续生成，从而在匹配模型、问题集和LLM轮次预算的条件下，比重复原始提示生成解决更多问题。


<details>
  <summary>Details</summary>
Motivation: 一阶概念合成任务要求系统推断出一个能够跨多个有限关系结构对标记对象进行分类的公式。候选公式可以精确评估，但量化一阶公式形成了一个巨大的搜索空间，而LLM输出的公式虽然在语义上很有前景，但往往并不完全正确。

Method: Hypothesis Frontier框架的核心包括：1）在每个训练对象上评估LLM公式；2）保留每轮中经验证的最强假设；3）利用其剩余错误来指导后续生成；4）符号处理修复无效公式同时保持与LLM生成假设的锚定；5）简化训练-验证公式而不改变任何训练预测；6）最终选择后进行精确简化以缩短公式长度。

Result: 在匹配模型、问题集和LLM轮次预算的条件下，Hypothesis Frontier比重复原始提示生成解决了明显更多的问题。最终公式选定后，精确简化在保留所有训练预测的同时缩短了许多训练-验证公式。

Conclusion: 精确符号推理不仅有助于解决更多的归纳问题，而且还能压缩许多最终得到的公式。该方法将LLM的语义能力与符号推理的精确性有效结合，为一阶概念合成提供了更可靠的解决方案。
...
### [145] [Enhanced Filtering Algorithms for the Euclidean Traveling Salesperson Problem and its variants in Constraint Logic Programming](https://arxiv.org/abs/2608.10881)
*Alessandro Bertagnon,Marco Gavanelli*

Main category: cs.AI

TL;DR: 本文研究欧几里得旅行商问题(TSP)的约束规划求解方法。传统方法将欧几里得TSP作为一般情况处理，忽略了点坐标的几何信息。本文提出新的过滤算法，利用坐标的几何信息实现更强的约束传播，并扩展到欧几里得广义旅行商问题(EGTSP)。


<details>
  <summary>Details</summary>
Motivation: TSP是计算机科学中最著名的问题之一，在智能汽车和智能交通系统等工程应用中有广泛应用。现有的约束规划方法通常计算完整的距离矩阵并将其作为一般情况处理，但这种方法忽略了节点坐标中蕴含的几何信息，导致约束传播不够强。

Method: 提出新的过滤算法并在约束逻辑编程(CLP)中实现，这些算法利用点的坐标几何信息来实现比现有方法更强的约束传播。该方法还被扩展到其他欧几里得TSP变体，包括欧几里得广义旅行商问题(EGTSP)。

Result: 实验结果表明，所提出的方法在计算性能上具有优势。

Conclusion: 提出的方法对欧几里得TSP及其变体（如EGTSP）是有效的，能够更好地利用几何信息进行约束传播，为路由和物流等实际应用提供更高效的求解方案。
...
### [146] [ComBodied Agents: a New Paradigm of Human-Centric Agentic AI](https://arxiv.org/abs/2608.10915)
...
TL;DR: 本文提出"具身化智能体"(Combodied Agents)范式，弥补现有智能体AI的结构性缺陷。当前数字智能体和具身智能体均未将人的状态和能动性作为建模、干预和评估的核心对象。Combodied Agents将软件工具、传感器、可穿戴设备、机器人及人工服务视为行动渠道而非终极目标，通过事件驱动多模态感知重建个人事件、纵向可纠正记忆提供时间上下文、个人世界模型预测未来状态，并在知情同意、不确定性、安全性、可逆性和用户控制约束下选择适当的支持方案，形成闭环系统。


<details>
  <summary>Details</summary>
Motivation: 现有智能体AI存在结构性gap：数字智能体主要改变软件状态，具身智能体主要改变物理状态，二者均未将人不断演变的状态和能动性作为核心建模、干预和评估对象。这导致智能体无法理解用户为何遗漏用药（遗忘、困惑、副作用还是拒绝），也无法提供适当支持。

Method: 提出统一框架：1)基于事件的多模态感知重建有意义个人事件；2)纵向可纠正记忆提供时间上下文；3)个人世界模型估计不同决策/干预下的未来个人状态和结果；4) admissible intervention policy在知情同意、不确定性、安全性、可逆性、用户控制约束下选择适当支持。无需完整人类数字孪生，采用目的有限、不确定性感知、可用户纠正的表征。

Result: 该框架将个人助理、健康智能体、AI伴侣、自适应人-AI系统等碎片化能力统一为闭环系统，可感知、建模、预测和支持个体人类状态的时间轨迹。

Conclusion: Combodied Agents将智能体AI从外部任务完成转向持续的人类利益。论文还组织设计了人类状态目标、关系上下文、智能体角色的设计空间，并提出场景中心评估、保持能动性指标、基准需求、边缘原生个人模型及治理方向。
...
### [147] [IO Factory: Simulating AI-Enabled Influence Campaigns at Scale](https://arxiv.org/abs/2608.10920)
*Lukasz Olejnik,Wenchao Dong,Jonas R. Kunst,Signe Riemer-Sørensen,Tobias Herb,Meeyoung Cha,Daniel Thilo Schroeder*

Main category: cs.AI

TL;DR: IO Factory是一个AI驱动的框架，用于将信息和影响力活动模拟为完全集成、可追踪的过程，能够模拟AI swarm（协调智能体群）适应平台反馈并伪装组织活动为普通社交互动的过程。


<details>
  <summary>Details</summary>
Motivation: 数字操纵的威胁已从单个语言模型的劝导性文本扩展到AI swarm（协调智能体持久群组），这些群体能适应平台反馈并将组织活动伪装成普通社交互动，此类活动无法从孤立信息中识别，必须在规划、平台行动、曝光、解释、测量和适应的连续谱系中分析。

Method: IO Factory在受控模拟平台中表示这一过程，链接角色、平台行动、曝光记录、结构化基于模型的评估和模拟人群配置变化，实现配置高达100,000个智能体的架构。

Result: 结果表明IO Factory能大规模执行活动时间线，并产生可检查的曝光证据和配置信念变量的可测量变化，通过记录每次运行中使用的角色、目标、行动约束、曝光路径和测量规则，支持可重复研究和协调影响力的红队分析。

Conclusion: IO Factory支持可重复研究和协调影响力的红队分析。
...
### [148] [ThinkRetrieve: Retrieval-Augmented Reasoning Traces for Test-Time Scaling](https://arxiv.org/abs/2608.10928)
*Vaibhav Singh,Soumya Suvra Ghosal,Sarvesh Gharat,Soumyabrata Pal,Ramasuri Narayanam,Dinesh Manocha*

Main category: cs.AI

TL;DR: ThinkRetrieve通过在推理过程中动态检索相关解题示例并将其注入思维追踪轨迹，从而提升大型推理模型的性能，解决了传统测试时扩展中延长推理链导致的误差累积和性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过增加推理时的计算量来生成更长的思维链，但研究表明顺序测试时扩展往往带来递减甚至负收益，因为更长的推理轨迹会增加不确定性、导致误差累积和问题偏离。

Method: ThinkRetrieve是一个测试时扩展框架，它在每个推理步骤从外部问题-解决方案语料库中动态检索相关的已解决示例，并将这些示例直接注入思维追踪轨迹中，为模型提供"如何推理"的指导而非仅是事实性信息。

Result: 在5个推理模型（1.5B-8B参数）上针对GSM-8K、MATH-500、AIME 2025和SciQ的实验表明，ThinkRetrieve始终优于标准测试时扩展，在AIME 2025上相对提升高达60%。

Conclusion: 通过在推理过程中动态融入解题示例，ThinkRetrieve有效地引导模型推理过程，显著提升了推理准确性，为测试时扩展提供了一种更高效的新范式。
...
TL;DR: 本文提出FedCGR，一个联邦生成式跨域推荐框架，通过离散的语义ID序列作为跨域对齐的桥梁，解决联邦跨域推荐中用户重叠稀疏和隐私敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 跨域推荐(CDR)需要跨域对齐用户偏好，但联邦部署环境下用于对齐的行为锚点（重叠用户、共享交互信号）稀疏、不可用或涉及隐私。传统方法依赖私有交互数据交换或域特定嵌入对齐，难以满足隐私要求。

Method: 1)将物品表示为从公开元数据导出的离散语义ID序列，建立共享词汇表实现跨域对齐；2)通过可靠性感知的语义接口注入本地协同过滤信号；3)训练原型个性化生成器，根据域相关性选择性聚合共享参数，同时保留域特定参数本地化。

Result: 在六个亚马逊跨域场景下，FedCGR一致优于联邦生成基线方法，并在全排序和采样评估协议下与强顺序和联邦CDR方法达到竞争性性能。

Conclusion: FedCGR通过稳定的语义物品语言和显式适配机制，有效解决了联邦跨域推荐中的对齐困难和负迁移问题，为隐私约束下的跨域推荐提供了新思路。
...
### [150] [XCoT-VLA: Executable Chain-of-Thought for Vision-Language-Action Driving](https://arxiv.org/abs/2608.10976)
*Foundation Model Team,XPeng Inc*

Main category: cs.AI

TL;DR: XCoT-VLA是一种将紧凑可执行的思维链（XCoT） tokens 替代冗长自然语言推理的视觉-语言-动作模型，用于实时自动驾驶规划。通过可执行token实现推理与轨迹生成的高效连接。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型依赖冗长的自然语言思维链（CoT），但这种推理方式不适合实时控制，原因包括：1）开放性结尾，难以解码；2）计算成本高；3）难以优化为面向动作的表示。因此需要一种更紧凑、可执行且直接连接轨迹生成的推理方式。

Method: 提出XCoT-VLA方法：1）用紧凑的可执行XCoT tokens替代描述性推理；2）从自动构建的Reason-Action监督中学习这些tokens（轨迹日志提供动作证据，场景上下文提供因果语义）；3）预测的XCoT序列保留在上下文中，通过共享多模态自注意力调节固定轨迹查询；4）使用确定性token-功能路由：Reason FFN处理XCoT tokens，Control FFN处理轨迹查询以进行流匹配轨迹生成；5）进一步引入XCoT策略优化（XCPO）作为可选的细化扩展。

Result: 在一般分布集上，纵向ADE从1.645降至1.323；在变道场景中，横向FDE从1.616降至0.648。仅使用2-6个可执行XCoT tokens表示面向驾驶的推理，大幅降低了自回归推理开销，保持在实时规划预算内。

Conclusion: 面向驾驶的推理可以是紧凑的、可执行的，并能直接连接到轨迹生成。XCoT-VLA成功证明了用可执行tokens替代冗长自然语言推理的可行性，在提升规划性能的同时满足实时性要求。
...
### [151] [V-FiLLM: Verified Financial LLM Reasoning Benchmark](https://arxiv.org/abs/2608.11047)
*Alicia Larsen,Victoire Laurent,Aulia Kharis Rakhamsari,Lara Turgut,Nino Antulov-Fantulin*

Main category: cs.AI

TL;DR: V-FiLLM是一个从真实表格构建的可执行计算树生成金融推理基准的框架，通过符号评估获取标准答案并渲染为自然语言问题，可大规模生成无注释成本的验证题目。该框架提供四个独立可控的难度维度：计算深度、表达式广度、金融概念复杂度和上下文大小。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在STEM领域的LLM评估方面已取得实质性进展，但金融领域结构化数据的推理评估仍相对较少被探索。需要一种可扩展、无注释成本且不继承生成器错误率的基准生成方法。

Method: 1.从真实表格构建可执行计算树；2.通过符号评估获取标准答案；3.将计算树渲染为自然语言问题；4.设置四个难度控制轴：计算深度、表达式广度、金融概念复杂度、上下文大小；5.对开源模型进行评估；6.测试基于验证链式思考轨迹的LoRA微调效果。

Result: 1.准确率随推理深度增加下降高达51%；2.在对抗性数值扰动下准确率下降高达47个百分点；3.LoRA微调将在保留问题上的准确率从81.1%提升至85.6%；4.在FinQA基准上比基线模型高5个百分点。

Conclusion: 轻量级LoRA微调是金融问答中组合推理的一个有前景的方向，针对性低成本适配可有效提升模型在金融推理任务上的表现。
...
### [152] [SkillZip: Evaluation-Free Skill Compression for Self-Evolving Agents by Discovering Reusable Structure](https://arxiv.org/abs/2608.11079)
*Xiaofan Bai,Hongqiang Lin,Chao Liu,Yantao Zhang,Xuan Jin,Xipeng Cao,Yuhong Li*

Main category: cs.AI

TL;DR: SkillZip是一种无评估的技能压缩方法，通过寻找最短的忠实结构解释来压缩自进化智能体的技能。它将"解释一次，引用多次"的直觉形式化为带类型的最短描述长度目标，在保持对所有触发器、工作流边、工具要求、义务和输出字段的硬覆盖约束下，对技能合约和残差进行压缩。


<details>
  <summary>Details</summary>
Motivation: 自进化智能体通过附加成功程序和失败修复来积累可复用技能，但同一需求在不同分支、示例和警告中反复出现，常见动作序列被复制而非复用。这导致技能注入成本高昂且难以维护。现有通用提示压缩方法不适用于技能这种非平面结构（包含名称、描述、工作流、工具/输出合约、异常等），评估引导压缩虽可测试行为，但引入 rollout、成本且依赖压缩时的评估集。

Method: 提出SkillZip方法：1)将"解释一次，引用多次"直觉形式化为带类型的最小描述长度目标；2)对技能合约和残差进行压缩，同时对每个提取的触发器、工作流边、工具要求、义务和输出字段保持硬覆盖约束；3)具有一次性模式和持续Zip-on-Write模式，后者可集成每个自进化补丁而无需重放任务或重解析完整历史。

Result: 实验表明SkillZip在压缩性能、可泛化性和成本开销方面均表现出有效性和优越性。

Conclusion: SkillZip作为一种无评估的技能压缩方法，能够有效解决自进化智能体技能冗余问题，通过结构化提取和确定性优化实现高效压缩，同时保留稀有异常规则，支持局部更新。
...
### [153] [RTSKG: Building a Rail Transit Station Knowledge Graph Dataset](https://arxiv.org/abs/2608.11080)
*Shutong Zhu,Tianxing Wu,Runfeng Liu,Yuang Gu,Xuan He,Yuan Zhu*

Main category: cs.AI

TL;DR: 本文构建了轨道交通站点知识图谱(RTSKG)数据集，旨在显式建模不同城市实体之间的空间和语义交互，以支持城市级轨道交通站点相关任务，如客流预测和站点周边商业推荐。


<details>
  <summary>Details</summary>
Motivation: 当前研究在组织数据时往往忽视了不同城市实体之间的复杂交互关系，且城市级轨道交通站点相关任务（如客流预测）需要大规模城市数据支撑，现有的数据组织方式难以满足需求。

Method: 设计统一模式整合异构城市实体（轨道交通站点、道路段、兴趣点等），构建可访问的链接数据RTSKG，并在站域店铺推荐和知识增强的客流预测任务上进行评估。

Result: RTSKG数据集成功整合了多种城市实体及其交互关系，实验表明该数据集能有效提升站域店铺推荐和客流预测任务的性能。

Conclusion: RTSKG具有支持城市级轨道交通站点分析的潜力，为智慧城市交通规划与商业决策提供了数据基础和知识支持。
...
### [154] [Why Does CLAUDE.md Keep Growing? Catastrophic Remembering in Agentic Coding](https://arxiv.org/abs/2608.11095)
*Kushal Chakrabarti*

Main category: cs.AI

TL;DR: 本文提出了"灾难性记忆"(catastrophic remembering)概念，指AI代理的指令文档(如CLAUDE.md)不断膨胀而难以精简的现象。研究分析了247,694条指令在1,867个仓库中的生命周期，发现指令平均增长226%，每次提交增加4.9条净指令，且旧指令更难删除。解决方案是通过在提示中加入注释来编码潜在推理，可消除99.3%的冗余指令，并在真实世界中提升23.1%的指令遵循能力。


<details>
  <summary>Details</summary>
Motivation: AI代理的指令文档(如CLAUDE.md)持续增长但无法有效精简，这源于"不完美召回"问题：追加指令成本低，但删除指令风险高(复杂度O(2^|D|))。作者将此命名为"灾难性记忆"，作为持续学习中"灾难性遗忘"的对应概念，旨在理解和解决这一阻碍AI代理长期维护的难题。

Method: 1)对247,694条指令在1,867个真实仓库中的生命周期进行大规模追踪分析；2)反转IFEval创建可验证的世界，用已知最优提示来测试注释的效果；3)将反转方法应用于WildIFEval评估真实世界的指令遵循表现。

Result: 1)代理提示增长无限制，生命周期内增长超过两倍(+226%)，每次提交增加4.9条净指令；2)指令越老被删除可能性越低(对数风险-0.032/提交)；3)提示注释可消除99.3%的冗余指令(从+211.3%降至+1.4%)；4)在WildIFEval中，提示注释可将真实世界的指令遵循能力提升最高23.1%。

Conclusion: 如果英语是新的代码，那么注释是不可或缺的。提示注释作为存储推理和意图的载体，能有效解决灾难性记忆问题，使AI代理能够安全地删除过时指令而不影响正确性，为AI代理的长期维护提供了实用方案。
...
### [155] [sLTN: Structural Logic Tensor Networks](https://arxiv.org/abs/2608.11136)
*Davide Rinaldi,Luciano Serafini*

Main category: cs.AI

TL;DR: 本文提出sLTN（结构化逻辑张量网络），将结构维度（如时间步、序列位置、图节点）作为语言的一等公民，扩展了原始LTN框架以支持时序、序列和关系推理。


<details>
  <summary>Details</summary>
Motivation: 原始LTN主要适用于表示为平面个体集合的数据，无法明确捕获结构组织，如时间顺序、序列位置或图连接性。这限制了其在需要结构化表示的任务中的应用。

Method: sLTN将结构维度表示为与领域特定组织关联的命名张量轴（如时间步、序列位置、图节点），支持显式量化、通过结构关系关联，并可在逻辑层面表达时序和关系约束。论文形式化了sLTN的语法和模糊张量语义，并证明在无结构维度时可恢复原始LTN语义。

Result: sLTN框架支持时序和序列推理，PyTorch实现基于声明式签名、公式解析和张量解释实现。框架在缺少结构维度时退化为原始LTN，验证了形式化的正确性。

Conclusion: sLTN成功将结构维度引入神经符号框架，使逻辑约束可直接在逻辑层面表达时序、序列和关系约束。sltn库已开源，为结构化推理提供了灵活的工具。
...
### [156] [Long-Horizon AI Research for Grothendieck Constant: A Case Study in Human-AI Mathematical Collaboration](https://arxiv.org/abs/2608.11195)
*Alan Li,Rahul Saha,Anton Xue,Swarat Chaudhuri,Adam Klivans,Pravesh K Kothari,Raghu Meka*

Main category: cs.AI

TL;DR: 本文报告了AI研究系统在数学研究中的应用案例，特别是用于改进Grothendieck常数K_G的上下界。作者将K_G的边界收紧到6π/11 ≤ K_G ≤ π/(2log(1+√2)) - 10^-4，并声称这些改进是通过能够产生领域专家认为是新颖洞察的AI研究系统实现的。


<details>
  <summary>Details</summary>
Motivation: AI代理在数学研究中的应用越来越普遍，但如何有效使用它们尚不明确。作者希望通过一个详细的案例研究，展示AI如何帮助改进数学问题中的重要界限，并总结使用AI进行数学研究的经验教训。

Method: 使用一个AI研究系统对Grothendieck常数进行探索性研究，该系统能够自主产生数学洞察。

Result: 成功将Grothendieck常数的界限收紧：下界为6π/11，上界为π/(2log(1+√2)) - 10^-4。这些结果被领域专家认定为新颖的贡献。

Conclusion: AI可以在数学研究中发挥重要作用，能够产生突破性的洞察。但AI的有效性取决于特定的理想条件，文章总结了使用AI进行数学研究的优势、劣势以及创造AI突破性洞察的理想条件。
