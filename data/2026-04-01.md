...
TL;DR: OccSim是首个基于占用世界模型的3D仿真器，仅需单帧初始图像和未来自我动作即可稳定生成3000+连续帧，构建超过4公里的大规模3D占用地图，比SOTA提升超80倍；用于预训练占用预测模型可实现67%零样本性能，扩展5倍数据后达74%。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的自动驾驶仿真长期以来严重依赖预录制的驾驶日志或空间先验（如高清地图），这种根本性依赖严重限制了可扩展性，将开放域生成能力限制在现有采集数据集的有限规模内。

Method: OccSim包含两个核心模块：1) W-DiT基于的静态占用世界模型，通过在架构中显式引入已知刚体变换来处理超长时域静态环境生成；2) 布局生成器，基于合成的道路拓扑为动态前景填充反应性智能体。

Result: OccSim可稳定生成超过3000帧连续帧，能够构建超过4公里的连续3D占用地图，比之前的SOTA占用世界模型在稳定生成长度上提升超过80倍。用OccSim收集的数据可直接预训练4D语义占用预测模型，在未见数据上实现67%的零样本性能，比基于资产的模拟器提升11%；当OccSim数据集扩展至5倍时，零样本性能提升至约74%，相比资产模拟器的提升扩大至22.1%。

Conclusion: OccSim突破了自动驾驶仿真对预录制日志和HD地图的依赖，实现了从单帧初始图像和未来自我动作生成3000+连续帧的大规模3D仿真。在下游任务中，使用OccSim数据预训练的4D语义占用预测模型在未见数据上实现了67%的零样本性能，比基于资产的模拟器提升11%；当数据集扩展5倍时，零样本性能提升至74%，相比基线扩大至22.1%。
...
### [2] [Untitled](https://arxiv.org/abs/2603.28896)
**
...
TL;DR: Stepper是一个统一的文本驱动沉浸式3D场景合成框架，通过逐步全景场景扩展方法，结合多视角360°扩散模型和几何重建pipeline，解决了现有方法在视觉质量与场景可扩展性之间的权衡问题，在大规模数据集上训练后实现了最先进的场景生成质量。


<details>
  <summary>Details</summary>
Motivation: 沉浸式3D场景的文本合成技术快速发展，在AR/VR和世界建模中有广阔应用前景。然而现有方法在视觉保真度和可探索性之间存在权衡：自回归扩展存在上下文漂移问题，而全景视频生成受限于低分辨率。

Method: Stepper采用逐步全景场景扩展框架，包含两个核心组件：1) 新型多视角360°扩散模型，实现一致的高分辨率扩展；2) 几何重建pipeline，确保几何一致性。该模型在大规模多视角全景数据集上训练。

Result: Stepper在保真度和结构一致性方面超越了先前方法，达到了最先进水平。

Conclusion: Stepper为沉浸式场景生成设定了新的标准，在视觉保真度和结构一致性方面都达到了最优表现。
...
TL;DR: 本研究探索混合量子-经典机器学习在铝TIG焊接图像缺陷分类中的应用，比较两种量子方法与CNN模型的性能。


<details>
  <summary>Details</summary>
Motivation: 混合量子-经典机器学习为工业自动化质量控制提供了有前景的发展方向，需要研究其在实际工业缺陷检测中的可行性。

Method: 1) 使用CNN提取图像特征向量；2) 第一种量子方法：使用参数化量子特征映射（旋转门和纠缠门）编码特征，计算量子核矩阵，通过VQLS求解SVM优化问题；3) 第二种量子方法：使用角度编码结合变分量子电路和经典优化器；在二元和多元分类任务上测试并比较性能。

Result: CNN模型表现出稳健性能，混合量子-经典模型也具有竞争力的表现，量子核条件数影响分类性能。

Conclusion: 混合量子-经典方法在工业缺陷检测和质量保证方面具有实际应用的潜力，特别适合近期的实际应用场景。
...
TL;DR: 提出了一种前馈人类动作捕捉方法，可以从单目RGB流中渲染出演者的新视角。


<details>
  <summary>Details</summary>
Motivation: 该方法旨在解决单目视角下观测不足的问题，尤其是对于不可见区域。假设表演者连续运动，通过维护一个逐步更新的规范空间来积累外观信息，以应对直接观测缺失的挑战。

Method: 维护一个随每帧更新的规范空间作为上下文库，用于当前帧缺少直接观测的区域。将渲染过程表述为概率回归，以有效利用上下文同时尊重实时状态的变形。

Result: 在域内数据集(4D-Dress)和分布外数据集(MVHumanNet)上的实验表明，该方法能产生比确定性回归方法更清晰的重建效果。

Conclusion: 该概率回归方法解决了过去与当前观测之间的冲突，能够在没有任何先验观测的区域进行合理的合成，展现出良好的泛化能力。
...
TL;DR: MEDiC是一种结合像素空间和潜在特征空间的多目标掩码图像建模框架，通过三个互补目标实现：patch级token蒸馏、全局CLS对齐和像素重建。使用ViT-Base在300个epoch后达到73.9% kNN准确率和85.1%微调准确率。


<details>
  <summary>Details</summary>
Motivation: 现有掩码图像建模方法通常只在原始像素空间（重建掩码patch）或潜在特征空间（与预训练teacher对齐）中运行，缺乏同时利用两种空间的统一框架。

Method: 提出MEDiC框架，包含三个互补目标：1) 从冻结CLIP编码器进行patch级token蒸馏；2) 全局CLS对齐；3) 通过轻量级解码器进行像素重建。同时引入带有相对位置偏差的层次聚类进行进化掩码策略。

Result: 1) 三个目标提供互补信息，完整组合在ImageNet-1K上达到73.9% kNN准确率；2) 进化掩码虽产生更语义一致的掩码，但在teacher引导蒸馏设置下不如简单block掩码；3) 最优标量损失权重极其脆弱，小扰动可导致高达17个百分点的kNN准确率下降。

Conclusion: MEDiC框架成功结合了像素空间和特征空间的双重目标，在ViT-Base上实现了73.9% kNN和85.1%微调准确率。研究还发现teacher的语义感知能力会削弱进化掩码的优势，且损失权重设置高度敏感需要精细调整。
...
TL;DR: 本文提出了UltraG-Ray，一种基于可学习3D高斯场的超声场景表示方法，结合物理基础的B型超声合成模块，用于解决超声新视角合成中复杂组织纹理和视角依赖性声学效果的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前超声新视角合成方法在处理复杂组织纹理和视角依赖性声学效果方面存在困难，现有的可学习隐式场景表示与超声专用渲染模块的组合方法在仿真与真实数据之间仍存在显著差距。

Method: 提出UltraG-Ray方法：1) 基于可学习3D高斯场的场景表示；2) 将超声特定参数（衰减、反射）显式编码到高斯空间表示中；3) 在新型光线投射方案中进行图像合成，自然捕捉视角依赖的衰减效应。

Result: 与现有方法相比，在图像质量指标上获得一致提升，MS-SSIM指标提升高达15%，合成超声图像的真实性有明显改善。

Conclusion: UltraG-Ray通过物理信息驱动的B型超声图像合成方法，有效提升了新视角合成的真实感，为超声图像仿真训练和数据增强提供了更可靠的解决方案。
...
TL;DR: 本研究探索了隐式神经表示（INRs）中参数初始化策略的原理，通过对多种噪声进行预训练实验，发现具有经典1/|f^α|谱结构的噪声在信号拟合和逆成像任务上表现出色，与最佳数据驱动初始化方法相当。


<details>
  <summary>Details</summary>
Motivation: INRs的近似和收敛特性高度敏感于参数初始化策略，现有数据驱动初始化方法虽效果显著，但对其成功原因缺乏深入理解——即它们是否编码了经典统计信号先验或更复杂的特征。

Method: 在图像和视频数据上，对不同噪声类别（高斯、死叶谱、频谱等）进行预训练，测量INRs拟合未见信号和编码逆成像任务（去噪）先验的能力。

Result: 惊人发现：简单地对非结构化噪声（均匀、高斯）进行预训练即可显著提升信号拟合能力，但产生的深度图像先验去噪效果较差；而具有自然图像经典1/|f^α|谱结构的噪声在信号拟合和逆成像能力间取得最佳平衡。

Conclusion: 具有1/|f^α|谱结构的噪声可作为有效的INRs初始化方法，性能与最佳数据驱动初始化方法相当，为缺乏足够领域特定数据的应用提供了更高效的INRs训练方案。
...
TL;DR: 本文提出了一种基于视频扩散模型的方法，用于去除自我中心视角"步行tour"视频中的人类及其阴影，以支持环境建模应用。


<details>
  <summary>Details</summary>
Motivation: 自我中心步行tour视频是构建全球环境视觉模型的宝贵数据源，但视频中大量存在的行人和视角导致的密集人群使得这些视频难以直接用于环境建模。去除人类及其阴影对于后续的3D/4D城市建模至关重要。

Method: 构建了一个丰富的半合成视频数据集：每对数据包含纯背景视频和叠加了行走人物及模拟阴影的合成视频。背景和前景均来自全球真实的自我中心步行tour视频以保持视觉多样性。然后利用该数据集对Casper视频扩散模型进行微调，用于目标及效果修复。

Result: 微调后的模型在去除高人群密度和复杂背景的步行tour视频中的人类方面，无论定性还是定量评估均显著优于原始Casper模型。生成的视频片段可成功用于构建城市场所的3D/4D模型。

Conclusion: 通过半合成数据微调视频扩散模型可有效解决自我中心视频中的人类移除问题，为城市环境建模提供了高质量的训练数据，未来可进一步拓展到其他类型的视频修复任务。
...
TL;DR: DASES框架通过引入主动的"深渊 falsifier"机制来解决自主科学发现中的评估器被冻结问题，防止搜索过程仅通过学会通过考试而不学习任务机制。该框架共同演化可执行的科学工件和科学可接受的反例环境，并成功发现了一种可转移的FNG-CE损失，在ImageNet等标准基准测试中优于CE和CE+L2。


<details>
  <summary>Details</summary>
Motivation: 自主科学发现进入更危险 regime：当评估器冻结后，充分强大的搜索过程可以学会通过考试而不学习任务揭示的机制，即"让深渊凝视回来"——使评估主动通过适应性 falsification 拒绝候选，而不是被动通过静态验证认证。

Method: 引入DASES框架，包含三个协同演化的组件：1) Innovator（创新者）生成可执行的科学工件；2) Abyss Falsifier（深渊 falsifier）构建科学可接受的反例环境；3) Mechanistic Causal Extractor（机制因果提取器）在固定科学契约下提取因果机制。

Result: 在单一可编辑位点的受控损失发现问题中：1) DASES拒绝静态验证会接受的工件；2) 识别首个在可接受的falsification前沿中幸存的候选者；3) 发现FNG-CE损失，该损失可转移到合成发现环境之外，并在包括ImageNet在内的标准基准测试中一致优于CE和CE+L2。

Conclusion: DASES框架通过适应性falsification机制有效解决了静态验证的局限性，能够识别真正具有泛化能力的科学发现。FNG-CE的成功证明了该框架在发现可转移知识方面的有效性，为自主科学发现提供了一种更安全的评估范式。
...
TL;DR: LA-Sign是一种用于孤立手语识别（ISLR）的循环transformer框架，通过几何感知对齐在双曲空间中进行特征表示学习，在WLASL和MSASL上达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的手语识别方法主要依赖深度前馈架构，虽然增加了模型容量，但缺乏递归细化和结构化表示的机制。孤立手语识别需要对从细微手指动作到全局身体动作的多个空间尺度进行精细理解。

Method: 提出LA-Sign框架：1）使用循环transformer替代堆叠深层，通过递归重复访问潜在表示来逐步细化运动理解；2）提出几何感知对比目标，将骨骼和文本特征投影到自适应双曲空间，鼓励多尺度语义组织。研究了三种循环设计和多个几何流形。

Result: 在WLASL和MSASL基准测试中，LA-Sign实现了最先进的性能，encoder-decoder循环结合自适应Poincare对齐效果最佳。

Conclusion: 递归潜在细化和几何感知表示学习对于手语识别是有效的，LA-Sign使用更少的独特层达到了最优性能，证明了循环机制和双曲空间表示的优越性。
...
TL;DR: 现代多模态模型（如CLIP）在共享嵌入空间中显示 出图像和文本分布分离的"模态差距"现象。本文证明在某些条件下，最小化对比损失会使两模态被一个正交于嵌入的全局差距向量分隔，且模态差距与鲁棒性呈单调关系。实验表明，通过简单的后处理步骤将一个模态移向另一个模态的均值，可在不损失精度的前提下显著提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代多模态模型（如CLIP）追求将图像和文本对齐到同一嵌入空间，但几乎所有现有模型都表现出强烈的"模态差距"现象——图像和文本的分布在共享嵌入空间中被明显分离。尽管近期有多篇相关论文，但尚不清楚这种差距存在的原因，也不清楚在后处理中消除差距是否能在下游任务中获得更好的性能。

Method: 本文从理论角度分析，在某些条件下最小化对比损失会得到一种表示，其中两个模态被一个全局差距向量分隔，该向量正交于它们的嵌入。研究还表明模态差距与鲁棒性呈单调关系。实验采用简单的后处理方法：将一个模态向另一个模态的均值移动。

Result: 理论分析表明，最小化对比损失产生的表示中，两模态被正交于嵌入的全局差距向量分隔，且模态差距与鲁棒性呈单调关系。实验证明，对许多真实世界的视觉语言模型，可以通过简单的后处理步骤（将一个模态移向另一模态的均值）显著提高鲁棒性，且不损失任何干净准确率。

Conclusion: 模态差距是由对比学习目标函数的内在特性导致的，并非模型训练失败的表现。消除模态差距不会改善干净准确率，但能显著提升模型对嵌入扰动的鲁棒性。这一发现表明可以通过简单的后处理方法来提升多模态模型的鲁棒性，而无需重新训练模型。
...
TL;DR: WorldFlow3D是一种基于流匹配的新型无界3D世界生成方法，通过无潜在流方法生成准确的3D结构，并将其作为中间分布引导高质量纹理生成，支持场景布局和属性控制，在户外驾驶和室内场景上展现出优越的生成质量和更快的收敛速度。


<details>
  <summary>Details</summary>
Motivation: 无界3D世界生成正在成为计算机视觉、图形学和机器人领域的基础任务。现有的方法在生成无界场景时存在局限性，需要一种能够生成连续、准确且可控制的3D世界的方法。

Method: 提出WorldFlow3D，这是一种基于流匹配（flow matching）的新型无界3D世界生成方法。该方法将3D生成更一般地建模为通过3D数据分布流动的问题，不仅限于条件去噪。采用了无潜在（latent-free）的流方法生成因果性和准确的3D结构，并将其作为中间分布来引导更复杂结构和高质量纹理的生成。通过向量化场景布局条件实现几何结构控制，通过场景属性实现视觉纹理控制。

Result: 该方法在真实户外驾驶场景和合成室内场景上都取得了良好效果，验证了跨域泛化能力和真实数据分布的高质量生成。在所有测试的无界场景生成设置中都展现出优越的场景生成保真度，且收敛速度比现有方法更快。

Conclusion: WorldFlow3D在所有测试设置中都表现出优越的无界场景生成保真度，具有跨域泛化能力，能够在真实 outdoor driving scenes 和 synthetic indoor scenes 上生成高质量结果。该方法比现有方法收敛更快，能够通过场景布局条件控制几何结构，通过场景属性控制视觉纹理。
...
TL;DR: 本文提出了TrajectoryAtlas数据生成管道和TrajectoryMover视频生成器，能够在保持相对3D运动的同时移动对象的3D运动轨迹，解决了现有生成式视频编辑方法无法移动对象3D轨迹的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式视频编辑方法主要关注预设对象的3D或2D运动轨迹，或改变对象/场景的外观，但尚无法在保持相对3D运动的同时移动对象的3D运动轨迹。主要挑战在于获取该场景的配对视频数据。

Method: 提出TrajectoryAtlas大规模合成配对视频数据生成管道，并使用该数据微调TrajectoryMover视频生成器。不同于之前依赖从非配对视频构造数据的方法，该方法直接生成合成配对数据。

Result: 成功实现了对象轨迹的生成式移动，能够在保持相对3D运动的同时移动对象的3D运动轨迹。

Conclusion: 通过大规模合成配对数据训练的方式，有效解决了该任务缺乏配对训练数据的问题，为生成式视频编辑开辟了新的可能性。
...
TL;DR: 本文提出DIME框架，解决持续食品识别中的双重不平衡问题（类别样本不平衡和增量步骤类别数量不平衡），通过参数高效微调和谱合并策略实现有效的持续学习。


<details>
  <summary>Details</summary>
Motivation: 现有的食品识别持续学习研究假设每个增量步骤引入相同数量的新类别，但实际场景中新增类别数量变化很大，导致学习动态不均衡。同时，食品类别本身存在长尾分布，即某些类别频繁出现而很多类别很少出现。这两个因素导致持续食品识别面临双重不平衡问题：类别内样本不平衡和步骤间类别数量不平衡。

Method: 提出DIME（Dual-Imbalance-aware Adapter Merging）框架，包含三个关键组件：1）使用参数高效微调学习每个任务的轻量级适配器；2）提出类别计数引导的谱合并策略，逐步整合适配器；3）引入秩阈值调制机制，通过保留主导知识并允许自适应更新来稳定合并过程。最终模型保持单一合并适配器进行推理，实现高效部署。

Result: 在真实的长尾食品基准测试和步骤不平衡设置下进行实验，DIME相比最强的现有持续学习方法一致提升超过3%。代码已开源。

Conclusion: 本文首次系统性地研究了持续食品识别中的双重不平衡问题，并通过DIME框架证明了参数高效微调与谱合并策略相结合的有效性，为实际膳食日志场景中的食品识别系统提供了有效的持续学习解决方案。
...
TL;DR: 本文提出LatentPilot，一种新的VLN范式，通过利用训练时的未来观察来学习动作-conditioned视觉动力学，而推理时无需未来帧。通过飞轮式训练机制和可学习的视觉潜在token，使智能体能够"预见"动作对后续观察的影响。实验在R2R-CE、RxR-CE、R2R-PE数据集上达到新SOTA，并在真实机器人上验证了其对环境-动作动力学的优越理解。


<details>
  <summary>Details</summary>
Motivation: 现有VLN模型主要基于过去和当前的视觉观测进行推理，忽略了动作引起的未来视觉动态变化。这导致模型缺乏对动作与视觉世界变化之间因果关系的有效理解，限制了决策的稳健性。人类则能够通过动作-动态因果关系想象近期未来，从而改善环境理解和导航决策。

Method: 1) 飞轮式训练机制：迭代收集on-policy轨迹并重训模型以更好匹配智能体行为分布，当智能体偏离过度时触发专家接管。2) 学习视觉潜在token：无显式监督的潜在token，在连续潜在空间中全局attention，跨步骤传递，既作为当前输出也作为下一输入，使智能体能够"预见"并推理动作如何影响后续观测。

Result: 在R2R-CE、RxR-CE和R2R-PE基准测试上取得新SOTA结果。真实机器人测试表明LatentPilot在多样化环境中对场景环境-动作动力学具有 superior understanding。

Conclusion: LatentPilot通过引入未来观察作为训练数据源，学习动作-conditioned视觉动力学，无需推理时future frames即可实现因果推理，在多个基准测试达到SOTA并展现出真实场景应用潜力。
...
TL;DR: 本文研究跨模态知识蒸馏问题：利用CT影像作为训练监督，部署时仅需X射线影像进行疾病分类。通过JDCNet框架在配对胸部影像数据集上实验，发现简单的跨模态logit-KD控制在验证集上表现最佳（0.875准确率、0.714 macro-F1），而完整的模块增强JDCNet反而表现较差。在8个患者级Monte Carlo重采样实验中发现排名不稳定 late fusion达到最高平均准确率（0.885），同模态蒸馏达到最高macro-F1（0.554）和平衡准确率（0.660）。


<details>
  <summary>Details</summary>
Motivation: 当前大多数计算机辅助诊断模型仅在单一成像模态内训练和部署。实际部署中，CT和X射线提供互补的胸部疾病视图，但希望利用CT丰富的监督信息训练X射线分类器，同时在推理时无需CT数据。现有跨模态迁移研究缺乏明确的任务定义和可重复的实验协议。

Method: 采用跨模态师生蒸馏框架，以JDCNet作为可执行的基础架构。在公开的配对胸部影像数据集上进行患者级划分实验。实验包括：原始患者级配对分割的验证、四图像验证子集测试、八个患者级Monte Carlo重采样以检验排名稳定性、同模态蒸馏对照、注意力迁移和特征提示的更强机制控制、不平衡敏感分析。评估指标包括准确率、macro-F1和平衡准确率。

Result: 原始分割上：plain cross-modal logit-KD控制达到最高mean result（0.875准确率、0.714 macro-F1），完整模块增强JDCNet为0.750准确率、0.429 macro-F1。重采样协议下：late fusion平均准确率最高（0.885），同模态蒸馏macro-F1最高（0.554）、平衡准确率最高（0.660），plain cross-modal控制平衡准确率降至0.500。注意力迁移和特征提示均未恢复稳健的跨模态优势。重采样结果显示排名不稳定。

Conclusion: 本文贡献不在于提出经过验证的CT-to-X-ray架构，而在于建立了一个可重复的、有证据支持的试点协议，明确了任务定义、失败模式、排名不稳定性以及未来可信CT-to-X-ray迁移研究所需的最低要求。跨模态知识蒸馏在此设置下未能展现稳健优势，需谨慎对待此类迁移 claims。
...
TL;DR: 本文提出了一种基于MedSAM模型改进的多类脑组织分割方法，通过FSL工具进行预处理，将MRI转换为2D切片并扩展mask解码器至三类（背景、灰质、白质），在IXI数据集上Dice分数达到0.8751。


<details>
  <summary>Details</summary>
Motivation: 传统的脑组织分割方法如FSL FAST虽然能产生组织概率图，但需要针对特定任务进行调整，且在多样化的成像条件下面临挑战。MedSAM等基础模型通过大规模预训练提供了基于提示的分割方法，但原始设计不支持多类分割，因此需要改进以适应脑组织分割任务。

Method: 预处理流程包括：使用FSL BET进行颅骨剥离、FSL FAST生成组织概率图、将数据转换为2D轴向、矢状和冠状切片并标记多类标签（背景、灰质、白质）。模型改进方面：冻结预训练的图像编码器，微调提示编码器和解码器，将MedSAM的mask解码器扩展至三类。

Result: 在IXI数据集上的实验达到了最高0.8751的Dice分数，证明了基础模型如MedSAM可以通过最小的架构修改适应多类医学图像分割任务。

Conclusion: 基础模型如MedSAM可以通过最小的架构修改成功适应多类医学图像分割任务，这一方法可扩展至更多样化的医学成像场景，为未来的研究和应用提供了有价值的参考。
...
TL;DR: SplatHLoc是一种新型的层级式视觉重定位框架，利用特征高斯溅射作为场景表示，解决传统点云方法中图像观测稀疏和特征匹配弱的问题。


<details>
  <summary>Details</summary>
Motivation: 传统的点云层级式重定位方法受限于稀疏的数据库图像观测和较弱的特征匹配能力，这限制了重定位的准确性和鲁棒性。

Method: 1. 采用特征高斯溅射作为场景表示；2. 提出自适应视角检索方法，通过合成更接近查询视角的虚拟候选图像来改进初始姿态估计；3. 设计混合特征匹配策略，在粗阶段使用高斯渲染特征，在细阶段使用图像提取特征。

Result: SplatHLoc在室内和室外数据集上均表现出更强的鲁棒性，设立了新的最优水平。

Conclusion: 该方法通过结合特征高斯溅射表示和混合特征匹配策略，有效提升了视觉重定位的准确性和效率，为该领域提供了新的技术方案。
...
TL;DR: 本文提出SLVMEval基准，用于元评估文本到视频(T2V)评估系统在长视频（最长约3小时）上的表现，该基准通过成对比较框架和人群ourcing筛选，创建了10个不同方面的可控测试集。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对长视频（超过几分钟）的评估基准，而现有的T2V评估系统需要能够在与人类评估难度相当的设置下准确评估视频质量，这是一个 fundamental requirement。

Method: 基于密集视频字幕数据集，通过合成降质方法创建"高质量vs低质量"成对视频，涵盖10个不同方面；然后使用人群ourcing筛选，仅保留降质明显可感知的成对，建立了有效的最终测试床。

Result: 人类评估者在识别更好的长视频时准确率达84.7%-96.8%，而在10个方面中的9个方面，现有T2V评估系统的准确率低于人类评估，揭示了文本到长视频评估的弱点。

Conclusion: SLVMEval是一个有效的基准，可以揭示现有T2V评估系统在长视频评估方面的系统性弱点，为该领域的未来研究提供了重要的测试平台。
...
TL;DR: 提出一种基于正交视图的三维重建方法，使用Harris角点检测获取控制点，通过包络线交点计算三维点云，最后用计算几何重建表面并OpenGL渲染


<details>
  <summary>Details</summary>
Motivation: 使用正交视图重建三维对象

Method: 使用Harris角点检测器提取控制点，将控制点垂直投影到各自视图上构建包络线，通过这些相互垂直的包络线的交点获取三维点集，最后使用计算几何方法重建表面并用OpenGL渲染

Result: 从正交视图成功重建三维对象

Conclusion: 该方法实现了基于多视图的三维重建，能够从二维正交视图恢复三维几何形状
...
TL;DR: 提出一种自适应上下文压缩框架，通过重要性感知内存选择、一致性敏感过滤和动态预算分配来解决LLM长对话中的性能下降问题，在多个基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在长时间交互中常因上下文长度增加、内存饱和和计算开销而导致性能下降。

Method: 提出了一种自适应上下文压缩框架，整合了重要性感知的内存选择、一致性敏感的过滤和动态预算分配，以保留必要的对话信息同时控制上下文增长。

Result: 在LOCOMO、LOCCO和LongBench基准上的实验结果表明，该方法在对话稳定性和检索性能方面实现了一致的改进，同时与现有的基于内存和压缩的方法相比减少了token使用和推理延迟。

Conclusion: 自适应上下文压缩在持久LLM交互中在长期记忆保留和计算效率之间提供了有效的平衡。
...
TL;DR: 本文提出了一种多层次记忆框架（Multi-Layer Memory Framework），将对话历史分解为工作记忆、情景记忆和语义记忆三层，并结合自适应检索门控和保留正则化机制，以解决长程对话系统中的语义漂移和记忆不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 长程对话系统面临语义漂移和跨会话记忆不稳定的问题，在扩展会话中难以保持长期记忆和推理稳定性。

Method: 提出多层次记忆框架，将对话历史分解为工作层、情景层和语义层三层结构，配合自适应检索门控和保留正则化机制，控制跨会话漂移，同时保持有界上下文增长和计算效率。

Result: 在LOCOMO、LOCCO和LoCoMo数据集上取得显著改进：46.85%成功率、0.618总体F1分数（多跳F1为0.594）、56.90%六周期保留率，同时将错误记忆率降至5.1%，上下文使用量降至58.40%。

Conclusion: 该架构有效控制了跨会话漂移，同时保持有界上下文增长和计算效率，在受限上下文预算下增强了长期记忆保持和推理稳定性。
...
TL;DR: LightHarmony3D是一个用于在3DGS场景中实现照明一致性网格插入的框架，通过单次前向传递生成360° HDR环境图来预测场景照明，从而为插入的网格物体提供物理一致的着色和阴影，并保持多视图一致性。


<details>
  <summary>Details</summary>
Motivation: 在3DGS场景中插入外部网格物体实现交互式编辑和内容增强时，如何实现物理一致的照明和阴影是一个挑战，因为这需要准确的场景照明估计和多视图一致的渲染。

Method: 提出了一种生成模块，通过单次前向传递预测插入位置的完整360° HDR环境图，利用生成先验而非迭代优化来高效捕获主导场景照明；同时构建了第一个用于3DGS网格插入的专用基准测试。

Result: 在多个真实世界重建数据集上的广泛实验表明，LightHarmony3D实现了最先进的逼真度和多视图一致性。

Conclusion: LightHarmony3D能够为插入的网格物体提供物理一致的着色和阴影，同时保持多视图连贯性，为AR/VR、虚拟场景布置和数字内容创作等沉浸式应用提供了有效的解决方案。
...
### [25] [CCDNet: Learning to Detect Camouflage against Distractors in Infrared Small Target Detection](https://arxiv.org/abs/2603.29228)
*Zikai Liao,Zhaozheng Yin*

Main category: cs.CV

TL;DR: 本文提出了一种用于红外目标检测的Camouflage-aware Counter-Distraction Network (CCDNet)，通过设计加权多分支感知器(WMPs)backbone、聚合-细化融合颈部(ARFN)和对比辅助干扰物判别器(CaDD)来解决红外目标低对比度、背景复杂以及干扰物导致的误报问题。


<details>
  <summary>Details</summary>
Motivation: 红外目标检测任务在荒野救援和海上搜救等应用中至关重要，但面临目标对比度低、易与复杂背景融合、以及相似特征干扰物导致误报等挑战。

Method: 1. 设计了带加权多分支感知器(WMPs)的backbone，通过自条件多层级特征聚合来准确表示目标和背景；2. 提出聚合-细化融合颈部(ARFN)，从浅层/深层特征图中精炼结构/语义信息，双向重建目标和背景的关系，突出目标并抑制复杂背景；3. 提出对比辅助干扰物判别器(CaDD)，在局部和全局层面计算真实目标与背景的适应性相似性来更精确地区分干扰物。

Result: 在红外图像数据集上的广泛实验表明，CCDNet优于其他最先进的方法。

Conclusion: CCDNet通过camouflage-aware和counter-distraction机制有效提升了红外目标检测的准确率并降低了误报率，为复杂背景下的小目标检测提供了新的解决方案。
...
TL;DR: 本文提出M2H-MX，一个用于单目空间理解的实时多任务感知模型，可同时预测深度和语义，并可直接集成到单目SLAM系统中。


<details>
  <summary>Details</summary>
Motivation: 单目相机因其低成本和易部署而备受关注，但从单张图像实现可靠的实时空间理解仍然具有挑战性。虽然多任务密集预测模型改进了逐像素深度和语义估计，但如何将这些进展转化为稳定的单目映射系统仍非易事。

Method: 提出M2H-MX模型，包含：1) 保留多尺度特征表示；2) 引入register-gated全局上下文；3) 在轻量级解码器中实现受控的跨任务交互，使深度和语义预测在严格延迟约束下相互增强。模型输出通过紧凑的感知-映射接口直接集成到未修改的单目SLAM流程中。

Result: 在NYUDv2上，M2H-MX-L实现了最先进的性能，语义mIoU提升6.4%，深度RMSE降低9.4%。在ScanNet上部署于实时单目映射系统时，平均轨迹误差比强单目SLAM基线降低60.7%，同时生成更清洁的度量语义地图。

Conclusion: 结果表明，现代多任务密集预测可以可靠地部署用于机器人系统中的实时单目空间感知。
...
TL;DR: 提出扩散心理平均值(DMA)，通过在扩散模型语义空间中进行轨迹对齐来生成概念的平均表示，可处理多模态概念，是首个能产生一致、逼真抽象概念平均的方法


<details>
  <summary>Details</summary>
Motivation: 现有方法在应用扩散样本时产生模糊结果，因为这些数据为中心的技术在模型外部操作，忽略了生成过程

Method: 提出扩散心理平均值(DMA)方法：将平均操作在扩散模型的语义空间中进行，通过轨迹对齐来实现——优化多个噪声潜在变量，使其去噪轨迹逐渐收敛到共享的粗到细的语义，生成一个锐利的原型。对于多模态概念(如多种品种的狗)，通过在CLIP等语义丰富空间中进行聚类，并使用Textual Inversion或LoRA将CLIP聚类连接到扩散空间

Result: 能够在扩散模型的语义空间内生成锐利、逼真的原型平均，即使对抽象概念也能产生一致、现实的结果

Conclusion: 这是首个能够为抽象概念提供一致、现实平均结果的方法，可作为概念的具体视觉总结，也可作为理解模型偏见和概念表示的工具
...
TL;DR: 本文提出了一种无需训练的方法FlexMem，通过模拟人类视频观看行为（持续观看视频并回忆最相关的记忆片段）来帮助多模态大语言模型理解无限长度的视频。


<details>
  <summary>Details</summary>
Motivation: 长视频理解是多模态大语言模型（MLLMs）发展的关键挑战。现有方法需要一次性处理所有视频信息，存在输入长度上限限制，难以处理无限长度的视频。

Method: FlexMem将视觉KV缓存作为记忆源，通过双路径压缩设计实现有效的记忆传递和写入，并探索了不同的记忆读取策略以适应多样的视频理解任务，包括流行的流式任务。

Result: 在单个3090 GPU上，FlexMem相比现有高效视频理解方法取得明显改进，可处理超过1000帧视频，并帮助基础MLLMs在某些基准测试上达到与SOTA MLLMs（如GPT-4o和Gemini-1.5 Pro）相当或更好的性能。

Conclusion: FlexMem是一种有效的无需训练的长视频理解方法，能够帮助MLLMs实现无限长度视频的理解。
...
TL;DR: 提出Omni-NegCLIP，通过设计基于存在和基于缺席的两种否定对比目标，并微调CLIP文本编码器的前层transformer，显著提升了CLIP对否定表达的理解能力，同时保持了甚至提升了图像-文本检索性能。


<details>
  <summary>Details</summary>
Motivation: VLMs（如CLIP）在理解自然语言中的否定表达方面表现不佳，这是一个实际存在且需要解决的问题。

Method: 通过修改CLIP的InfoNCE对比损失，设计了两种对比目标：1）基于存在的否定对比目标，将图像嵌入拉近原始 caption 嵌入，同时推离基于存在的否定 caption 嵌入；2）基于缺席的否定对比目标，将图像嵌入与原始和基于缺席的否定 caption 嵌入对齐，同时保持两种文本嵌入之间的语义区分。此外，基于观察到CLIP文本编码器的前层transformer对否定文本具有更强的学习能力，在每个训练步骤中微调前层transformer层。

Result: 与预训练CLIP相比，Omni-NegCLIP在基于存在的否定任务上提升了高达52.65%，在基于缺席的否定任务上提升了12.50%，且在图像-文本检索任务上不仅没有牺牲一般能力，反而提升了高达19.62%。

Conclusion: Omni-NegCLIP在理解多种类型的否定任务上展现出更全面的能力，为提升VLM对否定表达的理解提供了一种有效的微调方案。
...
TL;DR: 针对深度学习模型中的偏见问题，提出了一种无需受保护属性信息的去偏算法NPAD，通过DACL和FRL两种损失函数利用非受保护属性的辅助信息实现偏见缓解，在面部属性预测任务上取得了显著的去偏效果.


<details>
  <summary>Details</summary>
Motivation: 现有大多数去偏算法都需要使用受保护属性（如性别、年龄等）信息来减轻模型偏见，但这严重限制了这些算法在实际场景中的应用，因为现实中往往无法获取或使用敏感属性信息.

Method: 提出两种损失函数：1) Debiasing via Attribute Cluster Loss (DACL) - 利用非受保护属性的聚类信息来优化模型；2) Filter Redundancy Loss (FRL) - 减少特征表示中的冗余信息，从而实现公平性目标.

Result: 在LFWA和CelebA数据集上进行面部属性预测实验，观察到在不同性别和年龄子群体上的偏见显著降低.

Conclusion: 所提出的NPAD算法能够在不依赖受保护属性信息的情况下，有效减轻深度学习模型中的偏见，在LFWA和CelebA数据集上的实验证明了该方法在性别和年龄子群体上的显著偏见 reduction，has broad practical application prospects in real-world scenarios.
...
TL;DR: 本文提出了ConInfer，一个用于开放词汇遥感分割的上下文感知推理框架，通过联合预测多个空间单元并建模单元间的语义依赖关系，解决了传统方法中孤立patch预测与遥感数据空间相关性不匹配的问题。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇遥感分割方法主要关注增强特征表示或减少模态差异以提高patch级预测精度，但这种独立预测方案与遥感数据的内在特性严重不对齐。实际应用中遥感场景规模大且存在强烈的空间和语义相关性，孤立的patch-wise预测不足以实现准确分割。

Method: 提出ConInfer框架，执行跨多个空间单元的联合预测，同时显式建模单元间的语义依赖关系。通过整合全局上下文线索，增强分割一致性、鲁棒性和复杂遥感环境的泛化能力。

Result: 在多个基准数据集上的广泛实验表明，所提方法一致超越最先进的基于VLM的逐像素基线方法SegEarth-OV，在开放词汇语义分割和目标提取任务上分别实现平均2.80%和6.13%的性能提升。

Conclusion: ConInfer通过上下文感知推理成功解决遥感分割中的空间相关性问题，为开放词汇遥感分割提供了一种新的联合推理范式，代码已开源。
...
TL;DR: MaskAdapt是一个用于物理人形控制的灵活运动适应框架，通过两阶段残差学习实现部分身体部位的目标导向适应，支持运动合成和文本驱动跟踪应用。


<details>
  <summary>Details</summary>
Motivation: 解决物理人形控制在面对部分观测时的运动适应问题，特别是需要灵活地对特定身体部位进行目标导向的修改，同时保持其他部位的原始行为。

Method: 采用两阶段残差学习框架：第一阶段使用随机身体部位掩码和正则化项训练掩码不变的基线策略，使动作分布在不同掩码条件下保持一致；第二阶段在冻结的基线控制器上训练残差策略，仅修改目标身体部位的行为。

Result: 实验表明MaskAdapt在掩码观测下具有强鲁棒性和适应性，能够在单个序列中通过不同掩码实现多部位适应，并在文本驱动的部分目标跟踪任务中优于先前的工作。

Conclusion: MaskAdapt框架通过两阶段残差学习范式实现了灵活的运动适应，在运动合成和文本驱动的部分目标跟踪两个应用中都表现出色的鲁棒性和适应性，优于先前的工作。该框架能够仅修改目标身体部位同时保留其他部位的行为，为物理人形控制中的运动适应提供了一种有效且通用的方法。
...
TL;DR: 本文提出了一种基于注视引导的CLIP模型，用于细粒度深度伪造归因与检测，通过挖掘注视差异和动态语言提示提升对未知生成器的泛化能力，在基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造归因/检测方法对新型生成方法（如扩散模型和流模型）的泛化能力较差，仅依赖视觉模态，忽略了两个任务的协同作用，且对新型生成器的评估不够细致。

Method: 提出了一种基于CLIP的注视引导模型，包含注视感知图像编码器（GIE）和语言细化编码器（LRE）。GIE利用真实与伪造图像之间的注视差异挖掘全局伪造特征；LRE通过自适应增强的词汇选择器生成动态语言嵌入。

Result: 在归因任务上平均提升6.56% ACC，检测任务上平均提升5.32% AUC。

Conclusion: 通过引入注视信息作为辅助特征，显著提升了模型对未知人脸伪造攻击的泛化能力，为深度伪造归因与检测提供了一种新的思路。
...
TL;DR: 提出MotionScale，一个用于从单目视频重建动态4D场景的高效高保真 Gaussian Splatting框架，通过可扩展运动场和渐进式优化策略解决大场景和长序列的4D重建问题


<details>
  <summary>Details</summary>
Motivation: 现有神经渲染方法在复杂环境中难以恢复准确的3D几何和时间一致的运动

Method: 提出MotionScale框架，采用基于聚类中心基变换的可扩展运动场参数化，包含两个解耦的传播阶段：背景扩展阶段和前景传播阶段（三阶段细化）

Result: 在具有挑战性的真实世界基准测试上展示了对最先进方法的显著超越

Conclusion: MotionScale显著优于现有最先进的重建方法，在重建质量和时间稳定性方面都有明显提升。
...
TL;DR: 将自洽性（self-consistency）技术从自然语言领域扩展到视觉领域，用于改进LLM生成的运动图形轨迹质量。通过对LLM采样的轨迹进行聚类，并利用几何变换（刚性、相似性、仿射）建模形状家族来判断轨迹一致性。


<details>
  <summary>Details</summary>
Motivation: 自洽性技术在自然语言推理任务中已证明有效，但如何将其适应到视觉领域仍未被探索。具体需要解决LLM生成的运动图形轨迹的验证问题。

Method: 1) 给定提示词，从LLM采样多样化的运动轨迹；2) 通过聚类识别一致轨迹组；3) 将形状家族建模为原型轨迹+几何变换组（刚性、相似性、仿射）；4) 如果轨迹可通过变换组相互转换，则视为一致；5) 使用候选变换组之间的层次关系自动恢复形状家族。

Result: 轨迹生成准确率提升4-6%；验证任务精确度比VLM基线提升11%。

Conclusion: 成功将自洽性技术适配到视觉领域，在运动图形轨迹生成和验证任务上取得显著性能提升。
...
TL;DR: FOSCU提出了一种结合3D潜在扩散模型ControlNet的医学图像合成框架 Duo-Diffusion，可生成高分辨率解剖真实的合成MRI volumes及其分割标签，有效缓解临床数据短缺和标注成本高的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临临床数据集获取受限、标注成本高和数据短缺等系统性障碍，严重阻碍了鲁棒分割算法的发展。

Method: 提出FOSCU框架，集成Duo-Diffusion（3D潜在扩散模型+ControlNet）和增强的3D U-Net训练 pipeline。Duo-Diffusion采用分割条件扩散确保生成数据的空间一致性和精确解剖细节。

Result: 在720例腹部MRI扫描上评估，使用真实+合成数据训练的模型比仅用真实数据训练的模型平均Dice分数提升0.67%，Fréchet Inception Distance (FID)降低36.4%，图像保真度显著提升。

Conclusion: FOSCU框架通过扩散模型合成逼真的医学图像和分割标签，能有效扩充训练数据，在 Dice分数和图像质量指标上均取得提升，为解决医学图像数据短缺问题提供了可行方案。
...
TL;DR: 提出一种统一框架(UATP)联合建模位置和语义不确定性，通过双头架构端到端估计预测方差，并在nuScenes数据集上验证了方法的有有效性。


<details>
  <summary>Details</summary>
Motivation: 真实地图存在两类不确定性：(1)由于传感器限制或环境遮挡导致的位置误差；(2)场景上下文误判导致的语义错误。这些不确定性直接影响轨迹预测的准确性。

Method: 采用双头架构独立估计语义和位置预测，以双通道方式推导预测方差作为不确定性指标，端到端地获取不确定性，并将这些不确定性与语义和位置预测融合以增强轨迹预测的鲁棒性。

Result: 该方法有效量化了地图在位置和语义维度上的不确定性，并在四种地图估计方法和两种轨迹预测基线模型上持续提升了性能指标，包括minADE、minFDE和Miss Rate。

Conclusion: 提出的统一框架(UATP)通过双头架构联合建模位置和语义不确定性，并在nuScenes数据集上验证了方法的有效性，在minADE、minFDE和MR指标上均提升了现有轨迹预测模型的性能。
...
TL;DR: 提出了TriDerm多模态框架，用于学习罕见皮肤病RDEB的伤口表征，通过视觉基础模型适配（伤口级注意力池和非对比学习）和文本模态的软序数嵌入，实现视觉与文本模态融合，在专家一致性上达到73.5%，比最佳单模态基础模型提升超过5.6个百分点。


<details>
  <summary>Details</summary>
Motivation: RDEB是一种罕见的遗传性皮肤病，临床医生需要通过图像和临床文本寻找相似病例。现有基础模型无法可靠捕获该异质性、长尾疾病的临床有意义特征，且缺乏结构化测量专家一致性的方法。

Method: 提出基于专家序贯比较（三元组判断）来评估嵌入空间；视觉侧使用伤口级注意力池和非对比表征学习适配视觉基础模型；文本侧提示大语言模型进行比较查询，通过软序数嵌入（SOE）恢复医学有意义表征；融合视觉和文本模态。

Result: 视觉和文本模态捕获了伤口表型的互补方面，融合双模态达到73.5%的专家一致性，比最佳单模态基础模型提升超过5.6个百分点。

Conclusion: TriDerm框架通过多模态融合有效提升了RDEB伤口表征的临床相关性，为罕见病图像检索提供了新思路，且公开了注释工具、模型代码和代表性数据集。
...
TL;DR: Extend3D是一种无需训练的从单张图像生成3D场景的pipeline，通过扩展对象中心3D生成模型的潜在空间来处理宽阔场景，采用重叠分块处理与空间耦合，结合点云先验初始化和"欠去噪"策略实现3D补全，并在去噪过程中进行潜在空间优化以确保几何和纹理质量。


<details>
  <summary>Details</summary>
Motivation: 1) 现有对象中心3D生成模型的潜在空间大小固定，难以表示宽阔场景；2) 分块3D生成需要严格的图像与潜在分块空间对齐；3) 对象中心模型对子场景生成存在次优问题。

Method: 1) 扩展对象中心3D生成模型的潜在空间到x和y方向；2) 将扩展潜在空间划分为重叠分块，对每个分块应用对象中心模型并在每步耦合；3) 使用单目深度估计器生成点云先验初始化场景；4) 通过SDEdit迭代细化遮挡区域；5) 提出"欠去噪"策略将3D结构不完整性视为噪声进行补全；6) 在去噪过程中优化扩展潜在空间，确保去噪轨迹与子场景动态一致；7) 引入3D感知优化目标改善几何和纹理质量。

Result: 在人类偏好和定量实验中都取得了优于先前方法的结果。

Conclusion: Extend3D是一个无需训练的3D场景生成 pipeline，通过扩展对象中心模型的潜在空间来克服固定大小潜在空间的局限性。该方法使用重叠分块处理和空间耦合，结合点云先验初始化和迭代遮挡区域细化，并通过"欠去噪"概念实现3D补全。3D感知优化目标确保了几何结构和纹理保真度的提升，在人类偏好和定量实验中都优于先前方法。
...
TL;DR: AA-Splat是一种用于稀疏视角3D重建和新视角合成的Feed-forward 3D Gaussian Splatting模型，通过引入不透明度平衡的带限设计，解决了现有方法在非分布采样率下渲染时产生的严重伪影问题，实现了任意分辨率下的稳健抗锯齿渲染。


<details>
  <summary>Details</summary>
Motivation: 现有的Feed-forward 3D Gaussian Splatting (FF-3DGS)方法建立在不正确的屏幕空间膨胀滤波器上，导致在渲染非分布采样率（即与训练时不同的分辨率）时会出现严重的渲染伪影，限制了其在实际应用中的鲁棒性。

Method: 提出了一种名为AA-Splat的FF-3DGS模型，采用不透明度平衡的带限(OBBL)设计，包含两个核心组件：1) 3D带限后滤波器，将多视图最大频率边界整合到前馈重建管道中，有效对生成的3D场景表示进行带限并消除退化的高斯函数；2) 不透明度平衡(OB)模块，将所有像素对齐的高斯原语无缝整合到渲染过程中，补偿扩展高斯原语之间增加的Overlap。

Result: AA-Splat在所有分辨率（4×到1/4×）的新视角合成任务中，相比现有最先进方法DepthSplat，平均实现了5.4~7.5dB的PSNR增益。

Conclusion: AA-Splat能够实现任意分辨率下的稳健抗锯齿渲染，显著提升了稀疏视角3D重建和新视角合成的质量。代码将公开可用。
...
TL;DR: 针对大型视觉-语言模型的幻觉问题，提出一种动态检测幻觉表示并进行消除编辑的框架(HIRE)，在 minimal额外计算成本下达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉-语言模型存在严重的幻觉问题，现有方法包括重训练方法和对比解码(CD)方法存在资源消耗大或双推理开销的局限性，阻碍了实际应用。

Method: 提出动态幻觉检测与消除框架，通过识别幻觉表示并进行针对性的幻觉消除编辑，在推理过程中以 minimal额外计算成本纠正模型输出。

Result: 在现有基准数据集上实现了最先进的性能，实验表明该方法具有高效、鲁棒的幻觉消除能力和强大的可控性。

Conclusion: 本文提出的HIRE框架通过动态检测和编辑幻觉表示，在 minimal额外计算开销下实现了先进的幻觉消除效果，具有高效、鲁棒和可控的特点，为大型视觉-语言模型的幻觉问题提供了实用的解决方案。
...
TL;DR: 提出AGFT框架，通过软对齐分布和分布一致性校准机制，在保持跨模态语义结构的同时提升预训练视觉语言模型的零样本对抗鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有分类引导的对抗性微调方法会破坏预训练的跨模态对齐，削弱视觉-文本对应关系，导致零样本性能下降

Method: AGFT框架利用原始模型的概率预测进行文本引导的对抗训练，通过软对齐分布将对抗性视觉特征与文本嵌入对齐，并引入分布一致性校准机制调整鲁棒模型输出以匹配预训练模型的预测

Result: 在多个零样本基准测试中，AGFT优于现有最先进方法，显著提升了零样本对抗鲁棒性

Conclusion: AGFT框架在保持跨模态语义结构的同时，有效提升了零样本对抗鲁棒性，显著优于现有方法
...
TL;DR: 提出了一种外部参数感知的跨注意力框架,用于相机-LiDAR融合的外参标定。该方法直接在原始域中对齐图像 patches 和 LiDAR 点 groups,避免了将 LiDAR 点投影到 2D 深度图导致的 3D 几何失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的相机-LiDAR融合方法通常将 LiDAR 点投影到深度图进行特征融合,这种方法会扭曲 3D 几何结构。当外参初始化与真实值偏差较大时,性能会显著下降。因此需要一种能够在较大外参偏差情况下建立可靠跨模态对应关系的方法。

Method: 提出了一个外部参数感知的跨注意力框架,该框架直接在原始域中对齐图像 patches 和 LiDAR 点 groups。具体来说:1) 设计了一种跨注意力机制,将外参参数假设显式注入到对应关系建模中;2) 不依赖投影的 2D 深度图,实现几何一致的跨模态交互。

Result: 在 KITTI 和 nuScenes 数据集上的广泛实验表明,该方法在准确性和鲁棒性方面均优于最先进的现有方法。在大外参扰动下,该方法在 KITTI 数据集中 88% 的案例和 nuScenes 数据集中 99% 的案例中实现了精确标定,显著超越了次优基线方法。

Conclusion: 该研究提出了一种外部参数感知的跨注意力机制,能够直接在原始域中进行跨模态对应关系建模,有效解决了大外参偏差下的相机-LiDAR 融合标定问题。该方法在准确性和鲁棒性方面均取得了显著提升,为自动驾驶中的多传感器融合提供了新的技术路径。代码已开源。
...
TL;DR: 本文研究了对封闭源多模态大语言模型(MLLMs)的不可感知视觉提示注入攻击，通过有界文本叠加和迭代优化将恶意提示嵌入输入图像，在粗细粒度级别对齐特征表示，实验表明该方法优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在实际应用中易受提示注入攻击，现有方法主要依赖文本提示或可被人类用户感知的视觉提示，研究不可感知的视觉提示注入具有重要安全意义。

Method: 通过有界文本叠加提供语义引导，将恶意提示自适应嵌入输入图像；迭代优化不可感知视觉扰动，在粗粒度和细粒度级别上使被攻击图像的特征表示与恶意视觉和文本目标的特征表示对齐；视觉目标实例化为文本渲染图像并在优化过程中逐步细化。

Result: 在两个多模态理解任务上对多个封闭源MLLMs进行广泛实验，证明了该方法相比现有方法的优越性能。

Conclusion: 成功实现了对封闭源MLLMs的不可感知视觉提示注入攻击，为MLLMs的安全性研究提供了新的攻击范式。
...
TL;DR: 本文探索将多模态模型（Paligemma、Llava、Qwen）整合到ID文档的演示攻击检测（PAD）中，通过结合视觉特征和文本元数据来增强反 spoofing 攻击的能力，但实验结果表明这些模型难以准确检测ID文档上的PAD。


<details>
  <summary>Details</summary>
Motivation: 传统PAD系统仅依赖视觉特征，往往无法检测到复杂的 spoofing 攻击，需要引入多模态信息来提升检测性能。

Method: 利用预训练的多模态模型（如Paligemma、Llava、Qwen），将深度视觉嵌入与文档元数据（文档类型、发行机构、日期等）进行融合。

Result: 实验结果表明，这些多模态模型在准确检测ID文档上的演示攻击方面存在困难。

Conclusion: 多模态模型在ID文档PAD任务中的直接应用效果不佳，需要进一步研究更有效的特征融合策略或专门针对PAD任务训练的模型。
...
TL;DR: 无需训练的tool-guided推理框架解决VLM在光学幻觉上的系统性预测偏差，通过通用图像操作工具和幻觉类型路由实现跨结构泛化，并发现三个值得进一步研究的现象：正检测偏差、空间推理与逻辑推理的分离、对图像压缩伪影的敏感性。


<details>
  <summary>Details</summary>
Motivation: VLM在面对经典光学幻觉时存在系统性偏差：无论图像是否被反事实修改，模型都压倒性地预测幻觉为"真实"，这一失败模式需要解决。

Method: 提出了一种无训练的tool-guided inference框架，给VLM提供通用图像操作工具（线条绘制、区域裁剪、并排对比、通道隔离）+ 幻觉类型路由系统提示。每个工具调用生成新的不可变图像资源并添加到持久注册表，模型可在推理链中引用和组合任何先前注释的视图。

Result: 实现强跨结构泛化：从验证集到包含结构不熟悉幻觉变体的测试集（如Mach Bands从垂直堆叠旋转为水平堆叠），性能保持一致。

Conclusion: 该框架通过工具引导推理无需训练即可解决VLM在光学幻觉上的失败，表现出强跨结构泛化能力，对不熟悉的幻觉变体也有效。三个关键发现值得深入研究：检测偏差、空间推理与逻辑推理的分离、以及对图像压缩伪影的敏感性。
...
TL;DR: SeGPruner是一种用于3D问答的语义感知和几何引导的视觉token剪枝框架，通过保留语义显著tokens和补充空间多样化tokens，在大幅压缩token的同时保持推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D问答系统中，多视角视觉token的冗余导致 token 集过大，严重阻碍了受限token预算下的推理效率。现有剪枝方法主要针对2D输入或依赖间接几何线索，无法显式保留语义关键对象并保持足够的空间覆盖。

Method: 提出了SeGPruner框架，包含两个核心模块：1) 基于注意力的重要性模块(Saliency-aware Token Selector)用于保留语义显著的tokens；2) 几何引导的token多样化模块(Geometry-aware Token Diversifier)通过 jointly考虑语义相关性和3D几何距离来补充空间多样化的tokens。

Result: 在ScanQA和OpenEQA数据集上，SeGPruner实现了视觉token预算减少91%，推理延迟降低86%，同时保持具有竞争力的3D推理性能。

Conclusion: SeGPruner通过结合语义感知的重要性和几何引导的多样性，在保持竞争力的3D推理性能的同时，实现了高达91%的视觉token压缩和86%的推理延迟降低，为多视角3D QA任务提供了一种高效且有效的视觉token剪枝方案。
...
TL;DR: 本教程介绍EarthEmbeddingExplorer，一个将静态地球观测研究资产（基础模型和嵌入数据集）转化为动态交互式工作流程的工具。


<details>
  <summary>Details</summary>
Motivation: 地球观测领域虽有大量高影响力基础模型和全球地球嵌入数据集，但将学术资产转化为免费可用的实际工具仍存在显著障碍。

Method: 构建云原生软件架构的交互式web应用，支持跨模态查询（自然语言、视觉、地理定位），提供从检索结果中派生科学洞察的功能。

Result: 发布可直接访问的web应用程序(https://modelscope.ai/studios/Major-TOM/EarthEmbeddingExplorer)，实现从先进模型和数据档案到实际应用的过渡。

Conclusion: 通过民主化预计算地球嵌入的访问，使研究人员能够无缝地将最先进模型和数据档案转化为现实世界的应用和分析。
...
TL;DR: 提出一种 context-driven HTR 框架，利用多模态 in-context learning 实现推理时 writer adaptation，仅需少量样本即可适应目标书写风格，无需参数更新，在 IAM 和 RIMES 上分别达到 3.92% 和 2.34% CER。


<details>
  <summary>Details</summary>
Motivation: 现有 SOTA 手写文本识别模型在标准基准测试上表现良好，但难以处理训练数据中代表性不足的特殊书写风格；现有 writer adaptation 方法需要离线微调或推理时参数更新，计算成本高且需谨慎调参。

Method: 提出受多模态 in-context learning 启发的 context-driven HTR 框架，推理时仅需目标 writer 的少量样本即可实现 adaptation，无需梯度计算和反向传播；设计了 8M 参数的紧凑 CNN-Transformer 模型；结合 context-driven 与标准 OCR 训练策略。

Result: 在 IAM 数据集上达到 3.92% 字符错误率，在 RIMES 数据集上达到 2.34% 字符错误率，超越了所有 writer-independent HTR 模型，且无需推理时参数更新。

Conclusion: 该研究提出了一种无需参数更新即可实现推理时 writer adaptation 的 HTR 框架，在 IAM 和 RIMES 数据集上分别达到 3.92% 和 2.34% 的字符错误率，超越了所有 writer-independent HTR 模型，为高效的手写识别个性化提供了新思路。
...
TL;DR: 提出FedDBP方法，通过客户端双分支特征投影器和服务器端个性化全局原型融合来解决异构联邦学习中的特征保真度和可辨别性平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦原型学习方法无法同时保证特征的保真度和可辨别性，且受限于单一全局原型，难以应对数据异构和模型异构的挑战。

Method: 客户端设计双分支特征投影器，同时采用L2对齐和对比学习确保特征的保真度和可辨别性；服务器端引入基于Fisher信息的个性化全局原型融合方法，识别局部原型的重要通道。

Result: 实验表明FedDBP优于十种现有先进方法。

Conclusion: FedDBP通过双分支特征投影和个性化原型融合有效解决了异构联邦学习中特征表示和全局原型适应的问题，性能表现优异。
...
### [51] [Square Superpixel Generation and Representation Learning via Granular Ball Computing](https://arxiv.org/abs/2603.29460)
*Shuyin Xia,Meng Yang,Dawei Dai,Fan Chen,Shilin Zhao,Junwei Han,Xinbo Gao,Guoyin Wang,Wen Lu*

Main category: cs.CV

TL;DR: 提出了一种基于多尺度方形块的超级像素生成方法，利用粒球计算的适应表示和覆盖特性，用方形块近似超像素以避免不规则形状带来的计算和实现困难，实现高效并行处理和可学习特征提取。


<details>
  <summary>Details</summary>
Motivation: 现有超像素算法产生不规则形状区域，与卷积等规则操作不对齐，通常作为离线预处理步骤，限制了并行实现和端到端优化。

Method: 使用多尺度方形块近似超像素，为每个块计算基于像素强度相似性的纯度分数，选择高质量块，生成方形超像素可作为GNN节点或ViT标记集成。

Result: 在下游任务中显示出一致的性能改进。
...
TL;DR: 针对Transformer视频模型处理长上下文视频时的二次复杂度问题，提出VecAttention向量级稀疏注意力框架，利用视频注意力图的垂直向量稀疏模式实现2.65倍加速，同时保持精度。


<details>
  <summary>Details</summary>
Motivation: 长上下文视频理解和生成对基于Transformer的视频模型带来巨大计算挑战，因为自注意力具有二次复杂度。现有的稀疏注意力方法使用粗粒度模式来提高效率，但通常会产生冗余计算和次优性能。

Method: 提出VecAttention，一种新颖的向量级稀疏注意力框架。观察到视频注意力图表现出强烈的垂直向量稀疏模式，并通过轻量级的重要向量选择模块动态选择信息丰富的垂直向量，以及优化的向量稀疏注意力内核来处理这些向量。

Result: 在视频理解（VideoMME、LongVideoBench、VCRBench）和生成（VBench）任务的综合评估表明，VecAttention相比全注意力实现2.65倍加速，相比最先进的稀疏注意力方法实现1.83倍加速，同时保持与全注意力相当的精度。

Conclusion: VecAttention通过利用视频注意力图的垂直向量稀疏模式，成功实现了优越的精度-效率权衡，为长上下文视频模型的高效处理提供了有效解决方案。
...
TL;DR: 本研究开发了一种AR系统，利用HoloLens 2深度传感和无标记表面配准技术，帮助外科医生术中精确定位头颈癌阳性切缘，将定位误差从14.2mm降低到3.2mm，证明了该技术的临床可行性。


<details>
  <summary>Details</summary>
Motivation: 头颈鳞状细胞癌阳性切缘常见，但术中再切除时切缘定位通常由病理科口头传达，导致定位不精确。

Method: 开发了一种一体化AR系统，使用HoloLens 2深度传感和完全自动化的无标记表面配准技术，将切除标本上的阳性切缘重新定位到切除床并原位可视化。

Result: 无标记配准达到与传统标记基线相当的精度（中位数1.8mm vs 1.7mm，最大值<4mm）；AR引导将切缘定位误差从口头指导的14.2mm降低到3.2mm，所有AR定位误差均在5mm以内。

Conclusion: 无标记AR切缘引导系统可实现更精确的术中再切除，误差可控制在5mm以内，具有临床应用可行性。
...
TL;DR: 本文提出一种两阶段夜间图像去雾框架，整合透射率校正与结构-纹理分层优化。第一阶段通过边界约束初始透射率图和区域自适应补偿进行透射率校正，在YUV空间用二次高斯滤波估计大气光，改进夜间成像模型生成初始去雾图像；第二阶段提出STAR-YUV分解模型，在YUV空间分离结构和纹理层，分别进行照度补偿、色彩恢复和细节增强，最后通过非线性Retinex融合和线性混合的两阶段融合策略输出最终结果。


<details>
  <summary>Details</summary>
Motivation: 现有夜间去雾方法仅解决部分问题（如抑制光晕或增强亮度），未能联合处理夜间雾霾图像的低可见性、颜色失真和对比度降低等全部退化因素。

Method: 第一阶段：提出基于边界约束的初始透射率校正方法，结合区域自适应补偿和归一化；在YUV色彩空间使用二次高斯滤波估计大气光 map，改进夜间成像模型生成初始去雾图像。第二阶段：提出STAR-YUV分解模型，在YUV空间分离结构和纹理层；结构层进行伽马校正和MSRCR色彩恢复，纹理层应用Laplacian-of-Gaussian滤波增强细节；采用非线性Retinex融合和线性混合的两阶段融合策略生成最终输出。

Result: 所提方法能够有效提升夜间雾霾图像的可见性、校正颜色偏差并增强对比度。

Conclusion: 实验结果表明，所提出的两阶段框架在处理夜间雾霾图像时能够有效提升可见性、校正颜色偏差并增强对比度，比现有方法更全面地解决了夜间图像退化的多种因素。
...
TL;DR: 本文提出一种在边缘设备上实现多任务生成式AI推理的统一框架，通过将LoRA权重作为运行时输入实现动态任务切换，并引入QUAD量化策略实现高效执行。


<details>
  <summary>Details</summary>
Motivation: 生成式AI功能日益集成到移动应用，但大型视觉模型在资源受限设备上部署面临高内存和计算挑战。现有移动部署为每个LoRA+基础模型编译单独二进制文件，导致冗余存储和运行时开销。

Method: 1) 将LoRA权重视为运行时输入而非嵌入编译模型图，实现动态任务切换；2) 提出QUAD（统一自适应蒸馏量化）量化感知训练策略，使多个LoRA适配器在共享量化配置文件下对齐；3) 实现与移动NPU兼容的轻量级运行时堆栈。

Result: 内存占用减少高达6倍，延迟改进高达4倍，同时保持多个GenAI任务的高视觉质量。

Conclusion: 成功实现在边缘设备上使用单一共享模型进行多任务GenAI推理，通过动态LoRA加载和统一量化策略显著降低资源消耗。
...
TL;DR: 本文提出了一种结合姿态估计的生成框架，用于生成Bharatanatyam古典舞蹈的关键姿态，通过关键点损失和姿态一致性约束确保解剖学准确性和风格完整性。实验表明，带姿态监督的条件扩散模型效果最佳，为传统舞蹈的数字保存和传播提供了新方法。


<details>
  <summary>Details</summary>
Motivation: Bharatanatyam作为印度古典舞蹈形式，强调编码化的adavu和精确的关键姿态。在数字时代，准确生成这些姿态对于保持解剖学和风格完整性、文档记录、分析以及向全球观众传播都至关重要，但也面临独特挑战。

Method: 提出了一种姿势感知的生成框架，集成姿态估计模块，使用基于关键点的损失和姿态一致性约束作为监督信号。评估了四种配置：标准条件GAN (cGAN)、带姿态监督的cGAN、条件扩散模型和带姿态监督的条件扩散模型。每个模型都基于关键姿态类别标签进行条件化，并优化保持几何结构。

Result: 在cGAN和条件扩散设置中，集成姿态引导都能使生成的姿态与真实关键点结构对齐，促进文化保真度。结果表明，引入姿态监督显著提升了生成的Bharatanatyam姿态的质量、真实感和 authenticity。

Conclusion: 所提出的姿势感知生成框架结合姿态估计模块、关键点损失和姿态一致性约束，能够有效提升生成Bharatanatyam舞蹈姿态的质量、真实感和文化保真度，为传统舞蹈形式的数字保存、教育和传播提供可扩展的方法。
...
TL;DR: 针对面部表情识别中分布偏移和对抗鲁棒性问题，提出基于条件生成扩散模型的EmoDC及其改进版本AMDiT，通过自适应margin差异训练提升模型判别能力，在多个数据集上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习FER方法依赖判别分类器，容易学习shortcuts，对分布变化敏感。EmoDC虽提升了对抗鲁棒性，但标准训练无法有效惩罚错误的类别描述，导致识别性能次优。固定margin也无法适应不同图像的难度差异。

Method: 提出自适应margin差异训练(AMDiT)，动态调整每个样本的margin。方法包括：1)采用条件生成扩散模型EmoDC；2)使用margin-based discrepancy training鼓励正确类别描述的准确预测，惩罚不匹配描述；3)引入自适应机制根据图像难度动态调整margin。

Result: AMDiT显著提升了EmoDC在RAF-DB基本子集、RAF-DB复合子集、SFEW-2.0和AffectNet上的准确率。EmoDC在噪声和模糊鲁棒性方面超越了最先进的判别分类器。

Conclusion: AMDiT在RAF-DB基本子集、RAF-DB复合子集、SFEW-2.0和AffectNet上显著提升了EmoDC的准确率，在100步评估中表现优异。EmoDC在噪声和模糊鲁棒性方面优于最先进的判别分类器。
...
TL;DR: 提出FlowID，一种结合单图像微调和注意力掩码的身份保持面部重建方法，用于移除暴力死亡痕迹同时保留身份信息，在新基准InjuredFaces上优于现有开源方法。


<details>
  <summary>Details</summary>
Motivation: 每天有许多人死于暴力事件（犯罪、战争、迁移或气候灾难），法医和法律机构需要记录死者肖像作为证据，但无法立即进行身份识别。传统图像编辑工具处理这些照片发布的工作流程冗长且效果不佳。

Method: 提出FlowID方法，结合单图像微调（使生成模型适应分布外的受伤面部）与基于注意力的掩码（将编辑局部化到受损区域，同时保留身份关键特征）。

Result: FlowID在InjuredFaces基准测试中优于最先进的开源方法，同时保持较低的内存需求，适合本地部署且不损害数据隐私。

Conclusion: 引入了InjuredFaces基准数据集，为极端面部损伤下的身份保持面部重建研究提供标准化评估工具。
...
TL;DR: 本文提出了Video-Oasis诊断套件，旨在重新审视视频理解领域的评估现状。研究发现54%的现有基准样本无需视觉输入或时间上下文即可解决，而剩余样本上最先进模型的性能仅略高于随机猜测。


<details>
  <summary>Details</summary>
Motivation: 视频理解具有内在复杂性，难以归因于视觉感知、语言推理还是知识先验。尽管已出现许多评估高级推理的基准，但构成视频理解的基本标准在很大程度上被忽视。

Method: 提出Video-Oasis可持续诊断套件，系统评估现有评估并提炼视频理解的时空挑战。研究 Algorithmic设计选择对稳健视频理解的贡献。

Result: 关键发现：(1) 54%的现有基准样本无需视觉输入或时间上下文即可解决；(2) 在剩余样本上，最先进模型的性能仅略高于随机猜测。

Conclusion: 为未来研究提供实用指导，希望作为基准构建和架构开发的严格评估标准指南。
...
TL;DR: 论文提出BigEarthNet.txt，这是一个包含46万张遥感图像和960万条文本注释的大规模多传感器数据集，用于推进遥感领域的视觉语言模型学习，包含三种类型的标注：地理锚定描述、视觉问答对和引用表达检测指令，实验表明微调该数据集可提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)在计算机视觉领域表现出色，但在遥感(RS)数据上的性能仍然有限，主要原因是缺乏大规模、多传感器的遥感图像-文本数据集，现有数据集主要以航空RGB图像为主，标注简短且缺乏地理关联，注释类型多样性不足

Method: 构建了BigEarthNet.txt数据集，包含464044张共注册的Sentinel-1合成孔径雷达和Sentinel-2多光谱图像，共960万条文本注释，包括：描述土地使用/土地覆盖(LULC)类别的地理锚定注释、空间关系和环境上下文；针对不同任务的视觉问答对；用于边界框预测的引用表达检测指令

Result: 通过比较统计分析证明BigEarthNet.txt在文本丰富度和注释类型多样性方面优于现有遥感图像-文本数据集；建立手动验证的基准分割来评估VLMs，结果显示这些模型在处理复杂LULC类别时存在局限性，但使用BigEarthNet.txt进行微调后在所有任务中均获得一致的性能提升

Conclusion: BigEarthNet.txt是首个大规模、多传感器遥感图像文本数据集，能够有效推动遥感领域指令驱动的图像-文本学习研究，通过微调可显著提升视觉语言模型在遥感任务上的性能
...
TL;DR: 针对边缘持续视频流提出流式检索架构，使用epsilon-net过滤器去除冗余帧，在低功耗边缘设备上实现高效跨模态检索。


<details>
  <summary>Details</summary>
Motivation: 边缘持续摄像产生的冗余帧会拥挤正确结果，降低跨模态检索的top-k搜索效果，需要在设备端进行有效过滤。

Method: 提出流式检索架构，包含三个核心组件：1) 设备端epsilon-net过滤器，仅保留语义新颖的帧构建去噪embedding索引；2) 跨模态适配器；3) 云端重排序器补偿紧凑编码器的弱对齐问题。在8个视觉语言模型(8M-632M)和2个自我中心数据集(AEA, EPIC-KITCHENS)上评估。

Result: 单次流式过滤器优于离线替代方案(k-means、最远点采样、均匀采样、随机采样)，在8M参数编码器、2.7mW功耗下达到45.6%的Hit@5命中率。

Conclusion: 该流式检索架构在边缘设备上实现了高效的跨模态检索，单次过滤即可超越离线聚类方法，在低功耗条件下达到45.6%的Hit@5命中率。
...
TL;DR: 研究标签稀缺视觉分类在去中心化异构数据下的自监督联邦学习(SSFL)，提出PreDi分区方案解耦标签空间异构性为患病率和类集大小差异，并提出PreP-WFL方法增强低患病率下的稀有类表示。实验表明SSFL优于本地训练，患病率是标签空间异构性的主导因素。


<details>
  <summary>Details</summary>
Motivation: 1)现有研究假设预训练和微调阶段数据异构模式相同；2)现有分区方案无法生成纯部分类不相交数据设置，难以控制真实世界标签空间异构性的模拟；3)硅藻分类作为标签稀缺视觉分类的真实世界代表实例。

Method: 1)提出PreDi分区方案，将标签空间异构性解耦为患病率(Prevalence)和类集大小差异(Disparity)两个正交维度；2)提出PreP-WFL(基于患病率的个性化加权联邦学习)方法，自适应增强低患病率场景下的稀有类表示。

Result: 1)SSFL在同构和异构设置下均一致优于本地训练；2)预训练阶段 unlabeled 数据量越大有助于改进表示预训练；3)标签空间异构下，患病率主导性能，差异影响较小；4)PreP-WFL有效缓解性能下降，且患病率越低增益越大。

Conclusion: SSFL在同构和异构设置下均优于本地训练；PreP-WFL有效缓解标签空间异构性带来的性能下降，且随着患病率降低，增益增加。这些发现为去中心化识别系统中标签空间异构性的表征提供了机制基础。
...
TL;DR: MacTok是一种掩码增强的1D连续图像标记器，通过图像掩码和表示对齐防止后验崩溃，同时学习紧凑且鲁棒的表示，仅需64或128个标记即可实现高效高保真图像标记化。


<details>
  <summary>Details</summary>
Motivation: 连续图像标记器能够实现高效的视觉生成，基于变分框架的标记器可通过KL正则化学习平滑、结构化的潜在表示。然而，当使用较少标记时，往往会导致后验崩溃，即编码器无法将信息特征编码到压缩的潜在空间中。

Method: MacTok采用两种掩码策略：随机掩码用于正则化潜在学习，DINO引导的语义掩码强调图像中的信息丰富区域，迫使模型从不完整的视觉证据中编码鲁棒语义。结合全局和局部表示对齐，MacTok在高度压缩的1D潜在空间中保留丰富的判别信息，仅需64或128个标记。

Result: 在ImageNet上，MacTok在256×256分辨率下达到1.44的竞争性gFID，在512×512分辨率下使用SiT-XL达到1.52的最先进水平，同时将标记使用量减少最多64倍。

Conclusion: 掩码与语义引导相结合可以有效防止后验崩溃，实现高效、高保真的图像标记化。
...
TL;DR: 本文提出了DynMask方法，通过引入运动频谱描述符(MSD)来增强掩码运动生成的复杂度感知能力。MSD是一种无需学习的确定性指标，用于测量局部运动动态复杂度。该方法在训练阶段引导内容聚焦掩码、在自注意力中提供频谱相似性先验，并在迭代解码时调节采样。实验表明该方法在动态复杂的运动上提升显著，并在HumanML3D和KIT-ML数据集上获得更好的FID分数。


<details>
  <summary>Details</summary>
Motivation: 当前掩码运动生成模型对所有运动帧采用统一的掩码、注意力和解码策略，这与运动的真实特性不匹配——运动的时间局部动态复杂度变化剧烈。研究发现当前模型在动态复杂的运动上表现明显下降，帧级生成误差与运动动态性高度相关，这促使作者设计一种考虑运动复杂度的生成方法。

Method: 1.提出运动频谱描述符(MSD)，通过运动速度的短时频谱计算局部动态复杂度，具有确定性、可解释性且无需学习；2.将MSD应用于三个方面：训练时的内容聚焦掩码、自注意力的频谱相似性先验、迭代解码时令牌级采样调制；3.在掩码运动生成器基础上构建DynMask方法。

Result: 在HumanML3D和KIT-ML两个数据集上，DynMask在动态复杂运动上改进最为明显，同时整体FID指标也有提升。这表明尊重局部运动复杂度是掩码运动生成的有效设计原则。

Conclusion: 掩码运动生成应考虑局部运动复杂度。通过引入参数无关的MSD指标引导掩码、注意力和解码过程，可显著提升模型在复杂动态运动上的生成质量，同时获得更强的整体性能。这一复杂度感知的设计思路对未来的运动生成研究具有指导意义。
...
TL;DR: CutClaw是一个自主多智能体框架，利用多模态大语言模型(MLLMs)将数小时的原始素材自动剪辑成具有同步音乐、说明和视觉吸引力的有意义的短视频。


<details>
  <summary>Details</summary>
Motivation: 手动视频编辑耗时且重复性强，一直是电影制作人和专业内容创作者的长期挑战。

Method: 采用分层多模态分解捕捉细粒度细节和全局结构；Playwriter Agent协调故事流程构建长期叙事，锚定视觉场景与音乐转换；Editor和Reviewer Agents基于美学和语义标准协作优化最终剪辑。

Result: 实验表明CutClaw在生成高质量、节奏对齐视频方面显著优于最先进的基线方法。

Conclusion: CutClaw成功实现了长视频到短片的自动化智能编辑，通过多智能体系统有效提升了视频编辑效率和质量。
...
TL;DR: 本文评估了三种分割架构在皮肤镜数据集上的表现，引入连续色素分析方法，发现低病灶-皮肤对比度是分割错误的关键驱动因素。


<details>
  <summary>Details</summary>
Motivation: 皮肤癌早期检测重要，AI分割系统常用但公平性研究不足。皮肤色调对分割影响未充分量化，且常用粗糙离散类别。

Method: 评估三种分割架构(UNet、DeepLabV3+ResNet50、DINOv2)在HAM10000和ISIC2017数据集上，使用Wasserstein距离分析像素ITA分布来量化病灶皮肤对比度。

Result: 全局皮肤色调指标(Fitzpatrick分组或平均ITA)与分割质量相关性弱；低病灶-皮肤对比度与较大分割错误一致相关。

Conclusion: 皮肤镜分割的公平性改进应优先处理低对比度病变；分布色素 measures 比离散皮肤色调类别提供更有信息量的审计信号。
...
TL;DR: 首次为手术技能评估（SSA）回归任务构建无监督域适应基准benchmark，提出CoRe-DA方法，通过相对分数监督和目标域自训练学习域不变表示，在干式实验室和临床目标数据集上分别达到0.46和0.41的Spearman相关系数。


<details>
  <summary>Details</summary>
Motivation: 手术技能评估（SSA）目前面临两大挑战：1）手动标注定量技能分数成本高、耗时长；2）现有回归模型对新手术任务和环境泛化能力差。同时大量未标注视频数据的可用性催生了无监督域适应（UDA）方法的需求。

Method: 提出CoRe-DA框架，包含两个核心机制：1）相对分数监督（relative-score supervision）学习域不变表示；2）目标域自训练（target-domain self-training）。在四个数据集（涵盖干式实验室、临床环境、开腹手术和机器人手术）上评估了八种代表性模型。

Result: 在两种UDA设置下，CoRe-DA优于现有最先进方法：在干式实验室目标数据集上Spearman相关系数达0.46，临床目标数据集上达0.46，且未使用任何目标域标注数据进行训练。

Conclusion: CoRe-DA实现了可扩展的SSA和可靠的跨域泛化能力，解决了现有方法在域间迁移性能不足的问题。代码和数据集将开源发布。
...
### [68] [TSHA: A Benchmark for Visual Language Models in Trustworthy Safety Hazard Assessment Scenarios](https://arxiv.org/abs/2603.29759)
*Qiucheng Yu,Ruijie Xu,Mingang Chen,Xuequan Lu,Jianfeng Dong,Chaochao Lu,Xin Tan*

Main category: cs.CV

TL;DR: TSHA是一个综合性的室内安全风险评估基准，旨在解决现有基准的三大局限：领域差距、任务简化、评估协议缺失。该基准包含81,809个训练样本和1,707个复杂测试样本，整合多源数据并引入视频和全景等多模态测试。实验证明在该基准上训练可显著提升VLM的安全评估性能(+18.3分)并增强跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型(VLM)在室内安全风险评估中的应用面临三大根本性局限：(1)过度依赖模拟软件生成的合成数据集，与真实场景存在显著领域差距；(2)安全任务设计过于简化，对风险类型和场景施加人为限制，限制了模型泛化；(3)缺乏严格评估协议来全面评估模型在复杂家庭安全场景中的能力。

Method: 构建包含81,809个训练样本和1,707个测试样本的综合基准数据集，涵盖四个数据源(现有室内数据集、互联网图像、AIGC图像、新采集图像)；对23个主流VLM进行广泛评估；引入视频和全景图像等多模态复杂场景测试。

Result: 实验表明当前VLM缺乏可靠的安全风险评估能力；在TSHA训练集上训练的模型在TSHA测试集上性能提升高达18.3分，且在其他基准数据集上展现出更强的泛化能力。

Conclusion: TSHA benchmark通过整合多源数据和严格的评估协议，有效填补了室内安全风险评估领域的空白。实验表明在该数据集上训练可显著提升模型性能(+18.3分)并具有跨数据集泛化能力，为视觉-语言模型在安全评估领域的实际应用提供了重要基准。
...
TL;DR: 本文提出了一种混合边缘动作检测系统，结合基于骨骼的运动分析与视觉语言模型，用于公共空间中的暴力行为检测。该系统针对边缘计算约束下的延迟、隐私和资源限制问题，旨在实现隐私保护、实时的视频分析。


<details>
  <summary>Details</summary>
Motivation: 公共空间（交通枢纽、市中心、活动场馆等）需要及时可靠地检测潜在暴力行为以支持公共安全。现有的自动视频分析虽取得显著进展，但实际部署仍受延迟、隐私和资源限制的约束，特别是在边缘计算条件下。

Method: 该系统采用混合边缘架构：基于骨骼的处理实现连续、隐私感知且计算开销低的监控；视觉语言模型提供上下文理解和零样本推理能力。系统在GPU边缘设备上实现，并针对延迟、资源使用和操作权衡进行了评估。

Result: 研究结果突出了以运动为中心和以语义为中心两种方法的互补优势与局限，表明混合架构能够选择性增强快速基于骨骼的检测与高级语义推理。

Conclusion: 该系统为公共安全应用中的隐私保护、实时视频分析提供了实践基础，验证了混合架构在边缘约束下的可行性。
...
TL;DR: 本文提出SkeletonContext框架，通过语言驱动的上下文prompt增强和关键部分解耦，解决零样本骨骼动作识别中缺乏上下文线索的难题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本骨骼动作识别方法通常对齐骨骼特征与文本嵌入，但缺乏动作涉及的上下文线索（如对象），导致骨骼与语义表示存在差距，难以区分视觉相似的动作。

Method: 提出SkeletonContext框架：(1)Cross-Modal Context Prompt Module：利用预训练语言模型在LLM指导下重建masked上下文prompts，将语言上下文转移到骨骼编码器实现实例级语义接地；(2)Key-Part Decoupling Module：解耦运动相关关节特征，确保无显式对象交互时的鲁棒动作理解。

Result: 在多个基准数据集上实现最先进的常规和广义零样本设置性能，有效推理上下文并区分细粒度、视觉相似的动作。

Conclusion: SkeletonContext通过融合语言上下文提示与运动特征解耦，成功弥合骨骼动作与语义描述之间的语义鸿沟，为零样本动作识别提供新思路。
...
### [71] [Training deep learning based dynamic MR image reconstruction using synthetic fractals](https://arxiv.org/abs/2603.29922)
...
TL;DR: 研究使用合成分形数据（Julia fractals）训练深度学习模型进行动态心脏MRI重建，发现其重建质量和临床测量指标与使用真实心脏数据训练的模型相当，且两者均优于传统方法。分形数据可作为临床数据集的开源替代方案。


<details>
  <summary>Details</summary>
Motivation: 心脏MRI训练数据集存在隐私、许可和可用性方面的限制，研究探索使用合成分形数据替代真实临床数据来训练深度学习重建模型。

Method: 使用四元数Julia分形生成2D+time训练图像，模拟多线圈MRI采集产生完全采样和径向欠采样的k空间数据。训练3D UNet深度Artifacts抑制模型（F-DL），与使用真实心脏MRI训练的模型（CMR-DL）比较，评估指标包括10例患者的前瞻性径向实时心脏MRI重建质量、心室容积和射血分数。

Result: F-DL与CMR-DL在定性排名上无显著差异（p=0.9），两者均优于CS和LR-DIP（p<0.001）。F-DL与CMR-DL的心室容积和功能测量结果相似，与参考 cine MRI比较无显著偏差且一致性良好。LR-DIP存在显著偏差（p=0.016）且一致性范围更宽。

Conclusion: 使用合成分形数据训练的深度学习模型可实现与真实心脏数据训练模型相当的实时心脏MRI重建质量和临床测量指标。分形训练数据提供了一种开源、可扩展的临床数据替代方案，可能有助于开发更具普适性的动态MRI深度学习重建模型。
...
TL;DR: 提出FED-Bench人脸表情图像编辑基准测试，包含747个测试样本和 FED-Score 评估协议（三维度：对齐性、保真度、相对表情增益），对18个模型基准测试发现细粒度指令跟随是主要瓶颈，并提供20k+训练集显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有编辑基准主要关注通用场景，缺乏高质量人脸图像和相应编辑指令；当前评估指标存在系统性偏差，往往偏向于懒编辑或过拟合编辑。

Method: 构建级联可扩展管道生成747个测试 triplets；提出FED-Score跨粒度评估协议，包含三个维度：对齐性（验证指令遵循）、保真度（测试图像质量和身份保留）、相对表情增益（量化表情变化幅度）；对18个图像编辑模型进行基准测试；利用基准引擎的可扩展性提供20k+野外人脸训练集。

Result: 当前方法难以同时实现高保真和精确表情操作，细粒度指令跟随被识别为主要瓶颈；使用20k+训练集微调基线模型可获得显著性能提升。

Conclusion: 提出FED-Bench基准测试，包含747个 triplets 和 FED-Score 评估协议。对18个图像编辑模型进行基准测试，发现当前方法难以同时实现高保真和精确表情操作，细粒度指令跟随是主要瓶颈。还提供20k+训练集，微调后性能显著提升。
...
### [73] [Trimodal Deep Learning for Glioma Survival Prediction: A Feasibility Study Integrating Histopathology, Gene Expression, and MRI](https://arxiv.org/abs/2603.29968)
*Iain Swift,JingHua Ye*

Main category: cs.CV

TL;DR: 研究将FLAIR MRI纳入双模态预后框架形成三模态模型，在TCGA-GBMLGG队列中评估多种融合策略。结果显示三模态早融合CS=0.854，但提升不显著且受限于小样本测试集（19例），需更大样本验证MRI的附加价值。


<details>
  <summary>Details</summary>
Motivation: 多模态深度学习已通过整合组织病理学和基因组数据提高脑肿瘤预后准确性，但容积MRI在统一生存框架中的贡献仍未被探索。

Method: 使用TCGA-GBMLGG队列（664例患者），将BraTS2021的FLAIR MRI作为第三模态纳入双模态框架。评估了3个单模态模型、9个双模态配置和3个三模态配置，采用早融合、晚融合和联合融合策略。

Result: 三模态早融合达到CS=0.854，比双模态基线提高ΔCS=+0.011，但无统计学显著性（p=0.250）。MRI单模态CS=0.755。测试集仅19例患者，bootstrap置信区间宽[0.400,1.000]。

Conclusion: 初步证据表明即使样本量有限，第三种影像模态也可增加预后价值；额外模态需要足够的多模态上下文才能有效贡献。
...
TL;DR: 研究探索合成数据在自我视角手-物体交互检测中的作用，在三个数据集上验证了合成数据配合少量真实数据的有效性，并发布了HOI-Synth数据集和生成工具


<details>
  <summary>Details</summary>
Motivation: 探索合成数据在提升自我视角图像中手-物体交互(HOI)检测的作用，特别是在真实标注数据稀缺或不可用的情况下

Method: 在VISOR、EgoHOS和ENIGMA-51三个数据集上进行广泛实验和对比分析，通过合成数据与仅10%真实标注数据结合训练

Result: 使用合成数据+10%真实数据训练，相比仅用真实数据训练的模型，Overall AP分别提升：VISOR +5.67%、EgoHOS +8.24%、ENIGMA-51 +11.69%

Conclusion: 合成数据与真实数据的对齐程度越高，效果越好。研究发布了新的数据生成Pipeline和HOI-Synth基准数据集，包含自动标注的手-物体接触状态、边界框和像素级分割掩码
...
TL;DR: 针对动态场景单目视频新视角合成问题，提出一种可泛化模型，包含递归循环和动态平面扫描两个组件，能够分离相机和场景运动，实现六自由度控制，在几何细节重建上 outperforms 现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决动态场景单目视频新视角合成的挑战：现有场景特定方法在高动态区域因多视角信息难以利用而失效；扩散方法存在几何不一致问题；两者都需要大量计算资源。

Method: 提出两个关键组件：(1)循环递归机制实现输入与目标视频之间的无界异步映射；(2)对动态输入的高效平面扫描技术，分离相机和场景运动，实现六自由度相机控制。采用可泛化模型框架处理动态输入。

Result: 在UCSD数据集和Kubric-4D-dyn数据集上 outperforms 四种基于高斯溅射的场景特定方法和两种扩散方法

Conclusion: outperforms both Gaussian Splatting-based scene-specific methods and diffusion-based approaches in reconstructing fine-grained geometric details across static and dynamic regions, while addressing computational efficiency issues
...
TL;DR: SHIFT是一种无需训练的攻击方法，通过随机扩散重采样偏转潜在空间中的生成轨迹，使重建图像与原始水印轨迹解耦，从而破解多种基于扩散的水印方法，攻击成功率达95%-100%。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的水印方法存在共同假设：验证成功依赖于扩散轨迹的忠实重建。这种对轨迹恢复的依赖构成了一个根本性且可利用的漏洞。

Method: 提出SHIFT（Stochastic Hidden-Trajectory Deflection）攻击方法，利用随机扩散重采样在潜在空间中偏转生成轨迹，使重建图像在统计上与原始水印嵌入轨迹解耦，同时保持强视觉质量和语义一致性。

Result: 在涵盖噪声空间、频域和优化范式的九种代表性水印方法上，SHIFT达到95%-100%的攻击成功率，同时几乎不损失语义质量。

Conclusion: SHIFT无需任何水印特定知识或模型再训练即可实现攻击，揭示了扩散水印范式的根本性安全漏洞，为评估现有水印系统的鲁棒性提供了新视角。
...
TL;DR: 提出IQPIR框架，利用从无参考图像质量评估(NR-IQA)模型提取的图像质量先验(IQP)来引导真实世界图像恢复过程，实现感知最优输出。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖地面真实(GT)监督，但GT仍包含感知保真度不一致的图像，导致模型收敛到训练数据的平均质量水平而非最高感知质量。

Method: 提出三个关键机制：(1)质量条件Transformer，以NR-IQA派生分数作为条件信号引导表示向最大感知质量偏移；(2)双分支码本结构，解耦通用特征与HQ特定特征；(3)基于离散表示的质量优化策略，缓解连续潜在空间中的过度优化问题。

Result: 在真实世界图像恢复任务上不仅超越最先进方法，还可作为现有方法的通用质量引导增强策略。

Conclusion: IQPIR有效解决了GT质量不一致问题，无需修改结构即可作为即插即用的增强策略兼容现有恢复架构。
...
TL;DR: 针对生成式AI合成图像检测问题，提出融合MSCN、CLIP和MLBP三种特征的多特征框架，实验证实在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI模型生成的合成图像日益逼真，传统区分方法面临挑战。现有检测器通常依赖单一特征空间，缺乏对不同生成模型的鲁棒性。

Method: 提出了一种多特征融合框架，结合三种互补特征空间：1) MSCN特征（低层次统计偏差）；2) CLIP嵌入（高级语义一致性）；3) MLBP特征（多尺度局部二值模式，中层次纹理异常）。

Result: 三种特征空间的融合在混合模型场景中表现优异且一致，与现有最先进方法相比，在所有评估的数据集上均获得更好的性能。

Conclusion: 该工作强调了混合表示对鲁棒GenAI图像检测的重要性，并提供了整合互补视觉线索的原则框架。融合三种特征表示的方法在所有评估的数据集上均优于现有方法。
...
TL;DR: SceneTeract是一个验证3D场景功能性的框架，通过结合语义推理与几何检查来评估场景是否满足具身智能体的约束条件，揭示了现有合成环境和VLM在物理可行性推理方面的严重不足。


<details>
  <summary>Details</summary>
Motivation: 具身AI依赖于支持多样化用户有意义活动的交互式3D环境，但评估其功能可供性（affordance）仍是核心挑战。现有方法缺乏对场景物理功能性的系统验证能力。

Method: 提出SceneTeract框架，包含接地验证引擎，耦合高层语义推理与低层几何检查。将复杂活动分解为原子动作序列，针对具身智能体配置文件验证每一步的可访问性要求（可达性、间隙、可导航性），使用显式物理和几何模拟。

Result: 对合成室内环境的评估揭示了频繁的功能性故障，阻止基本交互；对前沿视觉语言模型(VLM)的评估显示语义置信度与物理可行性之间存在系统性错配，即使是最强的当前模型也存在问题。SceneTeract可作为VLM后训练的奖励引擎，实现几何约束的可扩展蒸馏。

Conclusion: 该研究发布了SceneTeract验证套件和数据，旨在弥合感知与物理现实之间的鸿沟，为具身3D场景理解提供新的基准和工具。
...
TL;DR: CANVAS是一个用于完整小鼠大脑光片荧光显微镜的高分辨率基准数据集，包含六个细胞类型标记物、细胞注释和排行榜，旨在推动适合LSFM数据的深度学习模型开发。


<details>
  <summary>Details</summary>
Motivation: 完整组织处理和光片荧光显微镜(LSFM)的进展使得亚细胞分辨率的完整大脑3D显微镜数据成为可能，但缺乏针对这种PB级数据规模的可扩展处理和分析方法，且现有视觉任务模型难以泛化到此类数据。

Method: 构建包含六个神经元和免疫细胞类型标记物的完整小鼠大脑LSFM数据集，提供细胞注释和排行榜，并测试现有模型架构的泛化能力。

Result: CANVAS是首个在亚细胞水平捕获完整小鼠大脑组织的LSFM基准数据集，包含广泛的细胞注释；基线模型在跨表型和脑区异质性细胞形态上的泛化存在显著挑战。

Conclusion: CANVAS是首个且规模最大的完整小鼠大脑LSFM基准数据集，在亚细胞水平上捕获完整小鼠大脑组织，包含广泛的细胞注释，为开发适合LSFM数据的计算机视觉模型奠定了基础。
...
TL;DR: 提出了 ViTAS，一个针对放射学报告摘要的多阶段 pipeline，通过选择性关注病理相关视觉 patches 而非完整图像，在 MIMIC-CXR 基准上实现了 SOTA 性能。


<details>
  <summary>Details</summary>
Motivation: 挑战两个 prevailing assumptions：1) 更多的视觉输入总是更好；2) 当 findings 已包含丰富图像细节时，多模态模型价值有限。作者认为现有 multimodal models 在处理 visual noise 时表现不佳，且在 FINDINGS → IMPRESSION 转换中未能显著超越强 text-only 基线。

Method: 提出了 ViTAS (Visual-Text Attention Summarizer)，包含四个关键组件：1) 集成引导的 MedSAM2 肺部分割；2) 双向交叉注意力用于多视图融合；3) Shapley 引导的自适应 patch 聚类；4) 分层视觉 tokenization 送入 ViT 模型。

Result: ViTAS 达到 SOTA 结果：BLEU-4 29.25%，ROUGE-L 69.83%，定性分析中事实一致性改善，专家评分最高的人类评估得分。

Conclusion: 证明了选择性关注病理相关视觉 patches 的方法优于使用完整图像的方法，为多模态放射学摘要生成提供了新的研究方向，Less but more relevant visual input is superior。
...
TL;DR: 一种基于锚帧的角色视频生成方法，通过Superset Content Anchoring和RoPE as Weak Condition机制解决长时一致性难题，可生成长达10分钟的角色视频。


<details>
  <summary>Details</summary>
Motivation: 数字角色视频生成是现代媒体的核心任务，但生成长时长、保持多视角一致外观和表情身份仍具挑战。现有方法要么提供不足以保留身份的上下文，要么使用非角色中心的信息作为记忆，导致一致性欠佳。

Method: 提出通过一组紧凑的锚帧来表示角色视觉属性。为解决参考式视频生成面临的复制粘贴和多参考冲突问题，引入了两种机制：Superset Content Anchoring（提供剪辑内和额外训练线索防止重复）和RoPE as Weak Condition（编码位置偏移以区分多个锚点）。此外构建了可扩展的管道从大量视频中提取这些锚点。

Result: 实验表明该方法可生成长达10分钟以上的高质量角色视频，实现跨视角的表情身份和外观一致性，超越现有方法。

Conclusion: 通过锚帧表示和创新的冲突解决机制，成功实现了长时一致的角色视频生成，为数字角色动画提供了有效方案。
...
TL;DR: 系统研究图像分割中不确定性聚合策略，通过分析常见策略属性、提出融入空间结构的新策略和元聚合器，填补了该领域的系统性比较空白，为分割可靠性提供更优实践。


<details>
  <summary>Details</summary>
Motivation: 尽管不确定性聚合策略被常规使用，但其特性和对下游任务性能的影响尚未被全面研究。Global Average是默认选择但未考虑分割不确定性的空间和结构特征，现有的替代策略缺乏系统性比较，导致报告不一致和最佳实践不明确。

Method: (1) 形式化分析常见聚合策略的属性、局限性和缺陷；(2) 提出融入空间不确定性结构的新策略；(3) 在十个跨不同图像几何和结构的数据集上基准测试OOD和失败检测性能；(4) 提出集成多个聚合器的元聚合器。

Result: 利用空间结构的聚合器在OOD和失败检测两个下游任务中表现更强，但单个聚合器的性能高度依赖数据集特征，元聚合器能跨数据集稳健运行。

Conclusion: 研究填补了分割不确定聚合策略的系统性比较空白，提出利用空间结构的新策略和跨数据集表现稳健的元聚合器，对图像分割的可靠性提升具有重要价值。
...
TL;DR: EC-Bench是一个用于评估长视频中枚举、计数和时间证据 grounding 的基准测试。包含152个超过30分钟的视频和1,699个带明确证据跨度的查询。在22个多模态大语言模型中，最佳模型在枚举任务上仅达到29.98%准确率，计数任务上仅23.74%，而人类表现分别为78.57%和82.97%。


<details>
  <summary>Details</summary>
Motivation: 现有视频计数基准测试主要关注短片段，仅评估最终数值答案，缺乏对计数内容和模型是否在时间上持续识别相关实例的深入理解。现实世界的视频记录往往长达数十分钟或更长，包含稀疏且多样的事件，使得长程时间推理特别困难。

Method: 构建了EC-Bench基准测试，包含152个超过30分钟的长视频和1,699个配有明确证据跨度的查询语句。联合评估枚举、计数和时间证据定位三个任务。对22个多模态大语言模型(MLLMs)进行了全面评估。

Result: 最佳MLLM模型在枚举任务上仅达到29.98%准确率，在计数任务上仅23.74%。人类表现分别为78.57%和82.97%。分析显示枚举准确率、时间证据定位和计数性能之间存在强相关性。

Conclusion: 当前多模态大语言模型在长表单数量视频推理方面存在根本性局限。EC-Bench作为一个具有挑战性的基准测试，可推动该领域的进一步研究和改进。
...
TL;DR: DEUS是一个新型开放世界目标检测框架，通过ETF子空间未知分离(EUS)和能量已知区分(EKD)损失来解决未知物体检测和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测(OWOD)需要检测器增量学习已知类别同时识别未知类别。现有方法依赖已知类别预测来检测未知物体，难以有效学习未知物体表示；内存重放虽缓解旧类别遗忘，但牺牲新类别知识。

Method: 提出DEUS框架，包含两部分：(1) EUS利用ETF几何特性创建正交子空间，从已知和未知空间两个方向捕获未知物体模式，实现更清洁的分离；(2) EKD损失强制分离旧新分类器，最小化知识干扰。

Result: 在OWOD基准测试上验证，未知检测性能显著提升，同时保持有竞争力的已知类别性能。

Conclusion: DEUS通过几何和能量方法有效解决了OWOD中的未知检测和灾难性遗忘难题，性能表现优异。
...
TL;DR: 提出SurgTEMP框架解决手术视频问答的时序建模问题，通过层次化视觉记忆和SCP训练策略，在新构建的CholeVidQA-32K数据集（包含32K问答对和128小时手术视频）上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前手术VQA研究主要聚焦于静态帧分析，忽视了丰富的时序语义。手术视频问答面临低视觉对比度、高度知识驱动、跨越分散时序窗口的多样化分析需求，以及从基础感知到高级术中评估的层次化挑战。

Method: 提出SurgTEMP框架，包含两个核心组件：1) 查询引导的令牌选择模块，构建层次化视觉记忆库（空间记忆库和时序记忆库）；2) 手术能力进阶（Surgical Competency Progression, SCP）训练 scheme。该框架能够有效建模变长手术视频，同时保留手术相关线索和时序一致性。

Result: 在CholeVidQA-32K数据集上与当前最先进的开源多模态和视频LLM（微调和零样本）进行综合评估，SurgTEMP实现了显著的性能提升，推动了视频手术VQA技术的发展。

Conclusion: SurgTEMP通过层次化视觉记忆模块和SCP训练策略，有效解决了手术视频VQA中的时序建模难题，在CholeVidQA-32K数据集上实现了显著的性能提升，为手术视频问答领域树立了新的技术标杆。
...
TL;DR: SurgRec是一个可扩展、可复现的手术视频理解预训练方案，包含SurgRec-MAE和SurgRec-JEPA两个变体。该模型基于包含10,535个视频和214.5M帧的多源数据集进行训练，涵盖内镜、腹腔镜、白内障和机器人手术等领域，并在16个下游数据集和四个临床领域上实现了 superior 性能。


<details>
  <summary>Details</summary>
Motivation: 现有手术基础模型受限于数据规模小、手术类型有限、评估标准不一致，且缺乏可复现的训练流程。

Method: 构建包含10,535个视频和214.5M帧的大规模多源数据集，开发统一预训练流程（平衡采样），在16个下游数据集和四个临床领域上标准化可复现基准。

Result: SurgRec在下游数据集上持续优于SSL基线和视觉语言模型。VLM在细粒度时间识别上表现不可靠，存在性能差距和提示词敏感性。

Conclusion: SurgRec为社区提供了一个可复现、可扩展的基础，以构建更通用的手术视频模型。所有代码、模型和数据将公开发布。
...
TL;DR: 本研究提出了一个开源的HMD-AR手术导航框架，可实现患者和器械追踪、工具校准和图像配准功能，在HoloLens 2和Magic Leap 2设备上验证了针头插入和肋骨骨折定位应用，精度达到亚毫米级


<details>
  <summary>Details</summary>
Motivation: AR头戴式显示器可直接将3D术前影像数据叠加到患者身上，但要作为独立手术导航系统使用，需要集成患者和手术器械定位、术前影像与患者配准、实时可视化导航数据等技术，现有技术集成繁琐且需要专业知识，阻碍了该领域的科学进展

Method: 提出并评估一个可适应多种手术应用的集成HMD-AR手术导航框架；该框架使用跟踪attached到患者和手术器械上的2D图案作为参考标记；使用pivot和基于参考的校准技术校准手术工具；使用基于点匹配和手动定位进行图像到患者配准；在HoloLens 2和Magic Leap 2两个HMD设备上评估，测试场景为AR引导下针头插入和肋骨骨折定位的幻像实验

Result: 框架实现了平均探头校准精度1mm，配准精度3mm，两种手术用例的靶向精度低于5mm

Conclusion: 该框架是一个易于使用且可配置的工具，可扩展和适应于许多手术应用；框架已在GitHub上公开可用
...
TL;DR: GeoCodeBench是一个评估AI模型3D几何视觉编码能力的PhD级基准，最佳模型GPT-5仅达36.6%通过率，揭示了当前AI在复杂3D科学编码方面的重大差距。


<details>
  <summary>Details</summary>
Motivation: AI辅助编码已快速重塑软件实践和研究流程，但当前模型在复杂3D几何视觉的正确代码生成方面仍存在困难。若模型能可靠地编写此类代码，将显著改变该领域的研究方式，因此需要建立基准来衡量这一目标的进展。

Method: 研究团队构建了GeoCodeBench基准：1)从近期顶会论文官方仓库中使用工具提议候选函数；2)经过人工筛选确定核心3D几何组件；3)为每个目标生成多样化、边缘情况的单元测试，实现全自动可重复评分；4)评估了8个具有代表性的开源和闭源模型；5)将任务组织为两级层次结构（通用3D能力和研究能力）；6)进行上下文消融实验研究论文文本长度影响。

Result: 最佳模型GPT-5仅获得36.6%的通过率，显示当前能力与可信赖的3D科学编码之间存在较大差距。两级层次结构的得分呈正相关，但研究导向任务明显更难。上下文消融实验表明"更多论文文本并非总是更好"：在方法部分截断的输入在统计上优于完整论文输入。

Conclusion: GeoCodeBench揭示了当前AI模型在3D几何视觉编码领域的显著不足。最佳模型GPT-5仅达到36.6%的通过率，说明现有能力与可依赖的3D科学编码之间存在巨大差距。研究导向任务明显更难，且"更多论文文本并不总是更好"——在方法部分截断的输入反而优于完整论文输入，这突出了长上下文科学理解的未解难题。GeoCodeBench为推动从通用编码向可信赖的3D几何视觉编码提供了严格的测试平台。
...
TL;DR: 研究视频扩散模型在2D迷宫求解中的内部规划动态，发现模型在去噪早期就会承诺高层运动计划，之后仅改变视觉细节而不改变底层轨迹。路径长度而非障碍物密度是迷宫难度的主要预测因素，失败阈值约为12步。提出ChEaP方法通过仅在有前景的早期计划上投入计算并链接多个连续生成来处理复杂迷宫。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型展现出迷宫求解和谜题解决等新兴推理能力，但对其生成过程中的推理机制知之甚少。需要研究模型在去噪过程中的内部规划动态。

Method: 使用2D迷宫求解作为受控测试平台，通过分析去噪步骤中的轨迹变化和迷宫参数（路径长度、障碍物密度）来研究模型的内部规划机制。

Result: 发现两个关键现象：1) 早期计划承诺：模型在最初几次去噪步骤内就确定高层运动计划；2) 路径长度是迷宫难度的主要预测因素，存在约12步的失败阈值。ChEaP方法将长 horizon 迷宫准确率从7%提升到67%，困难任务整体提升2.5倍。

Conclusion: 当前视频模型具有比之前认识的更深层的推理能力，可以通过更好的推理时扩展更可靠地激发。ChEaP通过选择有前景的早期计划并链式组合多个生成来有效处理复杂任务。
...
TL;DR: 本文提出OmniRoam，一个可控的全景视频生成框架，通过结合全景表示的丰富场景覆盖和时空一致性，实现长时间场景漫游。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖透视视频模型，只能合成有限的场景观察，导致完整性和全局一致性问题。全景表示具有每帧丰富场景覆盖和内在长期时空一致性，可解决这些问题。

Method: 框架包含两个阶段：1) 预览阶段：轨迹控制的视频生成模型从输入图像或视频快速生成场景概览；2) 细化阶段：将视频时间扩展并空间上采样，生成长距离、高分辨率视频。训练采用两个全景视频数据集（合成和真实世界捕获）。

Result: 在视觉质量、可控性和长期场景一致性方面始终优于现有最先进方法。进一步展示了实时视频生成和3D重建的扩展应用。

Conclusion: OmniRoam成功实现了高保真世界漫游的全景视频生成，为场景建模和交互式探索提供了新思路。
...
TL;DR: 论文研究多智能体辩论系统中不同辩论协议的效果差异，比较了轮内WR、跨轮CR、新颖的RA-CR三种协议与无交互NI基准，发现RA-CR在共识形成上表现最优。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论系统中性能提升难以区分是协议因素还是模型因素导致的，因为辩论协议通常保持不变而只改变模型相关因素。

Method: 在受控的宏观经济案例研究中（20个不同事件、5个随机种子、匹配提示/解码），比较三种主要协议（WR、CR、RA-CR）与无交互NI基准。

Result: RA-CR比CR收敛更快；WR显示更高的同伴引用率；NI最大化论点多样性（主协议间不受影响）。结果揭示了交互（同伴引用率）与收敛（共识形成）之间的权衡。

Conclusion: 协议设计很重要。当优先考虑共识时，RA-CR优于其他协议。
...
TL;DR: 随着AI智能体发展,学界正从单一大型语言模型转向多智能体系统以克服自动研究中的认知瓶颈。本文通过系统性实证研究,比较了两种多智能体结构(子智能体架构与智能体团队架构)在自动机器学习优化中的效果,发现子智能体模式适合Broad浅层优化,而智能体团队架构适合复杂架构重构。


<details>
  <summary>Details</summary>
Motivation: AI智能体正从单一大语言模型向多智能体系统转变以克服认知瓶颈,但多智能体协作框架尚未被充分探索。

Method: 使用严格控制的基于执行测试床(配备Git worktree隔离和显式全局内存),在固定计算时间预算下,将单智能体基线与两种多智能体范式(后整合的子智能体架构和前执行的智能体团队架构)进行基准测试。

Result: 子智能体模式具有高韧性、高吞吐量,适合严格时间约束下的广泛浅层优化;智能体团队拓扑因多作者代码生成而操作 fragility更高,但在扩展计算预算下能实现复杂架构重构所需的深层理论对齐。

Conclusion: 为未来自动研究系统设计提供了可操作指南,倡导动态路由架构以实时适应任务复杂性。
...
### [94] [Towards Computational Social Dynamics of Semi-Autonomous AI Agents](https://arxiv.org/abs/2603.28928)
...
TL;DR: Mimosa是一个 evolving multi-agent 框架，通过自动合成任务特定的多智能体工作流并通过实验反馈进行迭代优化来解决当前自主科学研究系统的固定工作流限制问题。


<details>
  <summary>Details</summary>
Motivation: 当前自主科学研究(ASR)系统尽管利用了大语言模型和智能体架构，但仍受限于固定的工作流和工具集，无法适应不断演变的任务和环境。

Method: Mimosa利用模型上下文协议(MCP)进行动态工具发现，通过元编排器生成工作流拓扑结构，通过代码生成智能体调用可用工具和科学软件库执行子任务，并使用基于LLM的评判器对执行进行评分，其反馈驱动工作流优化。

Result: 在ScienceAgentBench上，Mimosa使用DeepSeek-V3.2达到43.1%的成功率，超越单智能体基线和静态多智能体配置。研究还表明模型对多智能体分解和迭代学习的响应具有异质性，工作流演化的好处取决于底层执行模型的能力。

Conclusion: Mimosa的模块化架构和工具无关设计使其易于扩展，完全日志化的执行追踪和归档工作流支持可审计性。结合领域专家指导，该框架有潜力自动化跨学科的广泛计算可访问的科学任务。作为完全开源的平台，Mimosa为社区驱动的ASR提供了开放基础。
...
### [96] [ChartDiff: A Large-Scale Benchmark for Comprehending Pairs of Charts](https://arxiv.org/abs/2603.28902)
*Rongtian Ye*

Main category: cs.AI

TL;DR: ChartDiff是一个用于跨图表比较摘要的大规模基准测试，旨在评估模型在多图表推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准测试几乎只关注单图表解释，缺乏跨多个图表的比较推理评估。图表是分析推理的核心，但目前没有专门针对多图表比较 reasoning 的基准。

Method: 构建了包含8,541对图表的数据集，涵盖不同数据源、图表类型和视觉风格，每对图表配有LLM生成并经人工验证的摘要，描述趋势、波动和异常差异。然后评估了通用型、专用型和基于管道的模型。

Result: 前沿通用型模型获得最高的GPT基于质量评分，而专用型和管道型方法获得更高的ROUGE分数但人工对齐评估较低，揭示了词汇重叠与实际摘要质量之间的明显不匹配。多系列图表在所有模型家族中仍然具有挑战性，而强大的端到端模型对不同绘图库的差异相对稳健。

Conclusion: 比较图表推理仍然是当前视觉语言模型面临的重大挑战，ChartDiff作为推进多图表理解研究的新基准。
...
### [97] [Working Paper: Towards a Category-theoretic Comparative Framework for Artificial General Intelligence](https://arxiv.org/abs/2603.28906)
*Pablo de los Riscos,Fernando J. Corbacho,Michael A. Arbib*

Main category: cs.AI

TL;DR: 本文提出使用范畴论(Category Theory)为AGI(通用人工智能)建立统一的代数形式化框架，以解决AGI缺乏正式定义的问题，并能够比较和分析不同的AGI候选架构（如RL、Active Inference、CRL、Schema based Learning等）。


<details>
  <summary>Details</summary>
Motivation: AGI已成为AI领域的"圣杯"，全球主要科技公司都在投入大量资源追求AGI，但目前既没有单一的正式定义，也缺乏系统的理论框架。现有的AGI基准测试框架仅基于经验方法，无法从架构层面进行形式化描述和比较。

Method: 采用范畴论方法，将AGI架构形式化为"范畴中的机器"(Machines in a Category)，提供一种现代的AGI架构视图。具体对RL(强化学习)、Causal RL(因果强化学习)和SBL(基于模式的学习)架构进行范畴论形式化分析。

Result: 该框架可对不同AGI架构进行统一的代数和范畴论形式化，揭示不同架构(如RL、Universal AI、Active Inference、CRL、Schema based Learning等)之间的共性和差异，并明确指出未来研究方向。同时，该框架支持定义架构的句法性质、信息性质和语义性质，并可评估智能体在具有明确特征的环境中的表现。

Conclusion: 范畴论与AGI将形成一种共生关系。该框架为AGI系统提供了统一的理论基础，整合了架构结构、信息组织、智能体实现、智能体与环境交互、行为发展时间线以及实证评估等多个维度，为AGI研究开辟了新的形式化道路。
...
### [98] [Enhancing Policy Learning with World-Action Model](https://arxiv.org/abs/2603.28955)
*Yuci Han,Alper Yilmaz*

Main category: cs.AI

TL;DR: World-Action Model (WAM) adds inverse dynamics objective to DreamerV2 to predict actions from latent state transitions, improving representation quality for control. On 8 CALVIN manipulation tasks, WAM achieves 92.8% success after PPO vs 79.8% baseline, with 8.7x fewer training steps.


<details>
  <summary>Details</summary>
Motivation: conventional world models are trained solely via image prediction, which may not capture action-relevant structure critical for downstream control tasks

Method: WAM incorporates an inverse dynamics objective into DreamerV2, predicting actions from latent state transitions to encourage representations that capture action-relevant structure

Result: BC success improved from 59.4% to 71.2%; after PPO fine-tuning, WAM achieves 92.8% vs 79.8% baseline, with two tasks reaching 100%, using 8.7x fewer training steps

Conclusion: WAM effectively enhances policy learning by integrating action prediction into world model training, significantly improving success rates and sample efficiency on manipulation tasks
...
### [99] [Drop the Hierarchy and Roles: How Self-Organizing LLM Agents Outperform Designed Structures](https://arxiv.org/abs/2603.28990)
*Victoria Dochkina*

Main category: cs.AI

TL;DR: 研究通过25,000任务实验探讨多智能体LLM系统的自主协调能力，发现无需预设角色，智能体可自发形成专业化分工、层级结构和任务拒绝行为。混合协调协议(Sequential)比集中式协调性能提升14%，在8-256智能体规模下保持稳定。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统依赖外部设计的角色分配和协调机制，文章试图探索：1)当前LLM智能体能否在没有预设角色的情况下自主协调；2)何种因素决定自主协调的有效性。

Method: 25,000任务计算实验，覆盖8个模型、4-256个智能体、8种协调协议（从外部强加层级到自发自组织）。

Result: 1)当前LLM智能体在最小结构支撑(固定顺序)下可自发发明专业化角色、主动拒绝超出能力任务、形成浅层层级；2)Sequential协议比集中式协调优14%(p<0.001)；3)协议间质量差异44%(Cohen's d=1.86, p<0.0001)；4)自主程度随模型能力提升而增强，弱模型仍需刚性结构；5)系统可扩展至256智能体且无质量下降；6)开源模型达到闭源模型95%质量，成本仅为1/24。

Conclusion: 智能体自主协调能力随基础模型能力提升而扩展。建议给予智能体使命和协议，而非预设角色。
...
### [100] [Emergence WebVoyager: Toward Consistent and Transparent Evaluation of (Web) Agents in The Wild](https://arxiv.org/abs/2603.29020)
*Deepak Akkil,Mowafak Allaham,Amal Raj,Tamer Abuelsaad,Ravi Kokku*

Main category: cs.AI

TL;DR: 本研究针对WebVoyager基准测试存在评估方法模糊的问题，提出了Emergence WebVoyager标准化评估框架，并将OpenAI Operator的评估结果从87%修正为68.6%，揭示了现有AI agent评估中任务定义模糊和操作变异性的缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前AI agent评估方法存在任务框架模糊、操作变异性高、缺乏标准化等问题，导致评估结果不可靠且难以复现，特别是在web agent评估领域这些问题尤为突出。

Method: 提出Emergence WebVoyager增强版基准测试，建立明确的任务实例化、失败处理、标注和报告规范，并采用多 annotator 一致性检验来验证评估框架的可靠性。

Result: 该框架实现95.9%的标注者间一致性；应用该框架评估OpenAI Operator显示其总体成功率为68.6%，显著低于OpenAI此前报告的87%。

Conclusion: 标准化评估方法对于准确评估AI agent性能至关重要，Emergence WebVoyager提供了更严格、可比的评估范式，当前AI agent的实际性能可能被高估。
...
### [101] [The Future of AI is Many, Not One](https://arxiv.org/abs/2603.29075)
*Daniel J. Singer,Luca Garzino Demo*

Main category: cs.AI

TL;DR: 论文认为当前对生成式AI的思考方式是个体化的，但从复杂系统、组织行为和科学哲学的研究表明，深刻的 intellectual breakthroughs 应来自多样化的AI智能体团队，而非单个超级智能体。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究和开发采用个体化思维模式——从用户交互、模型构建到商业策略都以此为基础，但这种模式可能阻碍突破性创新和科学发现。

Method: 综合引用复杂系统、组织行为和科学哲学领域的研究和形式化结果进行论证。

Result: 多样化AI团队能够：1) 扩大解决方案搜索范围；2) 延缓过早共识；3) 追求非常规方法。同时解决AI批评者对模型受限于过去数据、缺乏创新洞见的担忧。

Conclusion: transformative transformer-based AI的未来本质上是"多元"而非"单元"，建议放弃个体化AI思维，转向构建多样化的AI智能体团队。
...
### [102] [PAR$^2$-RAG: Planned Active Retrieval and Reasoning for Multi-Hop Question Answering](https://arxiv.org/abs/2603.29085)
*Xingyu Li,Rongguang Wang,Yuying Wang,Mengqing Guo,Chenyang Li,Tao Sheng,Sujith Ravi,Dan Roth*

Main category: cs.AI

TL;DR: PAR²-RAG是一种用于多跳问答的两阶段RAG框架，通过将"覆盖"与"承诺"分离来解决现有迭代检索系统的问题。第一阶段进行广度优先锚定构建高召回证据前沿，第二阶段进行深度优先 refinement并通过证据充分性控制进行迭代 refinement。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多跳问答任务中仍然脆弱，需要跨文档组合证据进行检索和推理。现有的迭代检索系统可能锁定低召回轨迹并放大下游错误，而仅规划方法产生静态查询集无法适应中间证据变化。

Method: 提出两阶段框架PAR²-RAG：1) 广度优先锚定阶段：构建高召回证据前沿；2) 深度优先 refinement阶段：在迭代循环中应用证据充分性控制进行 refinement。

Result: 在四个多跳问答基准上，PAR²-RAG始终优于现有先进基线。与IRCoT相比，准确率最高提升23.5%，检索NDCG提升最高10.5%。

Conclusion: 该工作成功地将覆盖与承诺解耦，通过两阶段策略有效避免了早期低召回轨迹锁定问题，在多跳问答任务上取得了显著的性能提升。
...
### [103] [GISTBench: Evaluating LLM User Understanding via Evidence-Based Interest Verification](https://arxiv.org/abs/2603.29112)
...
TL;DR: GISTBench是一个评估LLM从推荐系统交互历史中理解用户兴趣能力的基准测试，提出兴趣 groundedness 和兴趣特异性两个新指标家族，并在8个LLM上发现其在处理异构用户信号时的性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统基准仅关注物品预测准确性，缺乏对LLM从用户交互历史中提取和验证用户兴趣能力的评估。

Method: 构建基于真实用户交互的合成数据集，包含隐式和显式信号及丰富文本描述；提出两个指标家族：兴趣 groundedness（分解为精确率和召回率惩罚幻觉类别并奖励覆盖率）和兴趣特异性（评估验证后用户画像的独特性）；对8个7B到120B参数的开放权重LLM进行评估。

Result: 实验揭示了当前LLM的性能瓶颈，特别是其在异构交互类型中准确计数和归因用户兴趣信号的能力有限。

Conclusion: 当前LLM在从异构交互类型中准确计数和归因用户兴趣信号方面存在明显瓶颈，未来研究需提升LLM对用户行为信号的精确理解能力。
...
### [104] [REFINE: Real-world Exploration of Interactive Feedback and Student Behaviour](https://arxiv.org/abs/2603.29142)
*Fares Fawzi,Seyed Parsa Neshaei,Marta Knezevic,Tanya Nazaretsky,Tanja Käser*

Main category: cs.AI

TL;DR: 本文提出了REFINE，一个基于小型开源LLMs的多智能体反馈系统，将反馈从静态产物转变为交互式过程。系统包含反馈生成智能体、评判引导的再生循环和交互智能体。实验表明该系统能显著提高反馈质量，支持学生后续问题，交互效果与闭源模型相当。


<details>
  <summary>Details</summary>
Motivation: Formative feedback对有效学习至关重要，但在规模上提供及时、个性化的反馈仍然是一个持久挑战。虽然近期工作探索了使用大型语言模型（LLMs）来自动化反馈，但大多数现有系统仍将反馈视为静态的、单向的产物，对解释、澄清或后续跟进的支持有限。

Method: 本文介绍了REFINE，一个基于小型开源LLMs的本地可部署多智能体反馈系统，将反馈视为交互式过程。REFINE结合了一个基于教学法的反馈生成智能体、一个使用人类对齐评判者的LLM评判引导的再生循环，以及一个自我反思的工具调用交互智能体，支持学生带有上下文感知、可操作响应的后续问题。

Result: 自动评估显示评判引导的再生显著提高了反馈质量，交互智能体产生了与最先进的闭源模型相当的高效、高质量响应。对真实学生互动的分析进一步揭示了不同的参与模式，并表明系统生成的反馈系统性地引导了后续的学生探究。

Conclusion: 我们的发现证明了多智能体、工具增强的反馈系统在可扩展、交互式反馈方面的可行性和有效性。
...
### [105] [SimMOF: AI agent for Automated MOF Simulations](https://arxiv.org/abs/2603.29152)
*Jaewoong Lee,Taeun Bae,Jihan Kim*

Main category: cs.AI

TL;DR: SimMOF是一个基于大型语言模型的多智能体框架，可自动化端到端的MOF（金属有机框架）模拟工作流程，从自然语言查询开始。


<details>
  <summary>Details</summary>
Motivation: MOF模拟难以获取，因为可靠的分析需要专家决策，包括工作流构建、参数选择、工具互操作性和计算就绪结构的准备。

Method: SimMOF将用户请求翻译成依赖感知的计划，生成可运行的输入，编排多个智能体执行模拟，并用与用户查询相关的分析总结结果。

Result: 通过代表性案例研究表明，SimMOF能够实现适应性和认知自主的工作流程，反映了人类研究人员的迭代和决策驱动行为。
...
### [106] [AEC-Bench: A Multimodal Benchmark for Agentic Systems in Architecture, Engineering, and Construction](https://arxiv.org/abs/2603.29199)
*Harsh Mankodiya,Chase Gallik,Theodoros Galanos,Andriy Mulyar*

Main category: cs.AI

TL;DR: AEC-Bench是一个针对建筑、工程和施工(AEC)领域的Agent系统多模态 benchmark,涵盖绘图理解、跨表格推理和项目协调任务,用于评估Claude Code和Codex等基础模型的Agent能力,并开源了数据集和评估代码


<details>
  <summary>Details</summary>
Motivation: AEC领域缺乏针对Agent系统的真实世界任务评估基准,需要标准化benchmark来推动该领域Agent技术的发展

Method: 设计并构建了包含绘图理解、跨表格推理和项目级协调任务的 benchmark,测试了Claude Code和Codex等多个基础模型,并识别了可uniformly提升性能的工具和工具链设计技术

Result: 建立了包含多任务类型的AEC-Bench benchmark,测试了多个基础模型并验证了工具和工具链设计对性能的提升作用

Conclusion: AEC-Bench为AEC领域的Agent系统评估提供了标准化基准,开源了数据集、工具链和评估代码,有望推动该领域Agent技术的发展
...
### [107] [Xuanwu: Evolving General Multimodal Models into an Industrial-Grade Foundation for Content Ecosystems](https://arxiv.org/abs/2603.29211)
*Zhiqian Zhang,Xu Zhao,Xiaoqing Xu,Guangdong Liang,Weijia Wang,Xiaolei Lv,Bo Li,Jun Gao*

Main category: cs.AI

TL;DR: Xuanwu VL-2B是一个约20亿参数的多模态大模型，旨在将通用多模态模型发展为内容生态系统的工业级基础模型，特别专注于内容审核场景。模型采用InternViT-300M + MLP + Qwen3 1.7B架构，通过三阶段渐进式训练（预训练、中训练、后训练）实现业务专业化与通用能力的平衡。


<details>
  <summary>Details</summary>
Motivation: 当前主流多模态模型在现实内容审核和对抗性环境中仍存在以下问题：1) 泛化能力退化；2) 灾难性遗忘；3) 细粒度视觉感知有限；4) 长尾噪声建模不足。这些问题限制了模型在工业内容生态系统中的应用。

Method: 1) 模型架构：采用紧凑的InternViT-300M + MLP + Qwen3 1.7B结构，总参数量约2B，平衡细粒度视觉感知、语言语义对齐和部署成本；2) 数据策略：开发数据迭代和精选机制；3) 训练流程：采用渐进式三阶段训练管道（预训练→中训练→后训练）。

Result: 1) OpenCompass多模态基准：7项平均得分67.90（对比InternVL 3.5 2B的64.27）；2) 业务审核任务：7项独立业务审核任务平均召回率94.38%；3) 对抗性OCR场景：政策违规文本加权召回率82.82%，优于Gemini-2.5-Pro的76.72%。

Conclusion: 在有限参数量预算下，Xuanwu VL-2B实现了业务对齐、视觉感知、通用能力保留和部署成本之间的实际平衡，证明了通用多模态模型可以发展为工业级内容生态系统基础模型的可行性。
...
### [108] [Beyond pass@1: A Reliability Science Framework for Long-Horizon LLM Agents](https://arxiv.org/abs/2603.29231)
*Aaditya Khanal,Yangyang Tao,Junxiu Zhou*

Main category: cs.AI

TL;DR: 现有基准测试衡量的是能力（单次尝试成功与否），但生产部署需要可靠性（不同持续时间任务的持续成功）。本文引入可靠性科学框架，包含四个指标（RDC、VAF、GDS、MOP），在396个任务、23,392个episode上评估10个模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试（如pass@1）仅衡量能力，但生产环境需要可靠性。两者随任务持续时间增长而系统性分化，且短期任务的pass@1在结构上无法检测这种分化。

Method: 引入可靠性科学框架，包含四个指标：可靠性衰减曲线(RDC)、方差放大因子(VAF)、优雅降级分数(GDS)、熔毁 onset 点(MOP)。评估10个模型在23,392个episode上，涵盖396个任务、四个持续时间桶和三个领域。

Result: 主要发现：(1)可靠性衰减呈领域分层——SE的GDS从0.90降至0.44，文档处理几乎持平(0.74→0.71)；(2)VAF按能力层级分化——高VAF是能力标志而非不稳定信号；(3)能力和可靠性排名在长视野下大幅分化，出现多级逆序；(4)前沿模型熔毁率最高(达19%)，因其尝试雄心勃勃的多步策略；(5)记忆脚手架普遍损害所有10个模型的长期性能。

Conclusion: 可靠性应作为与能力同等重要的一级评估维度。能力与可靠性并非必然相关，需要独立度量。
...
### [109] [Grokking From Abstraction to Intelligence](https://arxiv.org/abs/2603.29262)
*Junjie Zhang,Zhen Shen,Gang Xiong,Xisong Dong*

Main category: cs.AI

TL;DR: 模运算中的grokking现象源于模型内部结构的自发简化，记忆化到泛化的转变对应冗余流形坍塌和信息压缩，为理解泛化机制提供了新框架。


<details>
  <summary>Details</summary>
Motivation: 现有研究将模运算中的grokking现象作为理解模型泛化的关键实验平台，但现有工作仅关注局部电路或优化参数调校，忽略了驱动grokking现象的全局结构演化问题。

Method: 该研究提出grokking源自内部模型结构的自发简化过程，受简约性原则支配。研究整合了因果分析、谱分析、算法复杂度测度以及奇异学习理论(Singular Learning Theory)来揭示这一现象。

Result: 研究发现从记忆化到泛化的转变对应着冗余流形的物理坍塌和深层信息压缩，这一过程揭示了模型过拟合与泛化的内在机制。

Conclusion: 该研究为理解模型过拟合和泛化提供了全新视角，强调全局结构简化而非局部优化调参是grokking现象的根本驱动力。
...
### [110] [PSPA-Bench: A Personalized Benchmark for Smartphone GUI Agent](https://arxiv.org/abs/2603.29318)
*Hongyi Nie,Xunyuan Liu,Yudong Bai,Yaqing Wang,Yang Liu,Quanming Yao,Zhen Wang*

Main category: cs.AI

TL;DR: PSPA-Bench是一个专注于评估智能手机GUI智能体个性化能力的基准测试，包含超过12,855条个性化指令，覆盖10个日常使用场景和22个移动应用，并引入结构化感知的过程评估方法。通过对11个SOTA GUI智能体的基准测试发现，当前方法在个性化设置下表现不佳，即使是最强的智能体成功也很有限。


<details>
  <summary>Details</summary>
Motivation: 现实世界的智能手机使用高度个性化，用户采用不同的工作流程和偏好，但现有的GUI智能体基准测试无法充分捕捉这种个性化维度，因为缺乏用户特定数据和细粒度评估指标。需要一个专门评估个性化能力的基准测试来解决这个差距。

Method: 构建了PSPA-Bench基准测试，包含12,855条个性化指令，涵盖10个代表性日常使用场景和22个移动应用；引入了结构化感知的过程评估方法(Structure-Aware Process Evaluation)，在细粒度层面测量智能体的个性化能力；对11个SOTA GUI智能体进行了基准测试。

Result: 当前方法在个性化设置下表现不佳，即使是最强的智能体成功也很有限。分析揭示了三个提升个性化GUI智能体的方向：(1)推理导向模型始终优于通用LLMs，(2)感知仍是简单但关键的能力，(3)反思和长期记忆机制是适应改进的关键。

Conclusion: PSPA-Bench为个性化GUI智能体的系统性研究和未来进展奠定了基础。研究表明需要关注推理能力、感知提升以及反思/记忆机制的改进。
...
### [111] [BenchScope: How Many Independent Signals Does Your Benchmark Provide?](https://arxiv.org/abs/2603.29357)
*Tommy Sha,Stella Zhao*

Main category: cs.AI

TL;DR: 本文提出"有效维度"(ED)作为评估基准测量广度的快速诊断指标，通过22个基准、8个领域、8400多次模型评估的实证分析，发现当前AI评估存在严重冗余：六分数的Open LLM Leaderboard实际仅约两个有效测量轴，BBH与MMLU-Pro几乎可互换(r=0.96)，不同基准的测量广度差异超过20倍。


<details>
  <summary>Details</summary>
Motivation: AI评估套件通常报告大量分数，但未检验这些分数是否携带独立信息。当前基准缺乏对测量冗余的系统性诊断工具，导致评估效率低下且可能高估模型能力多样性。

Method: 提出有效维度(ED)——中心化基准分数谱的参与比——作为群体条件下的测量广度上界诊断指标。对22个基准、8个领域、超8400次模型评估进行实例级ED计算，并进行匹配维度控制、零假设、可靠性和饱和度分析。

Result: ED揭示显著冗余：Open LLM Leaderboard六分数仅对应约1.7个有效维度；BBH与MMLU-Pro几乎可互换(r=0.96且在七个子群体中稳定)；不同基准的测量广度差异超20倍。ED相对排名在匹配维度控制下稳定。

Conclusion: ED可作为筛选统计量而非literal因子计数，用于标记冗余套件成分、监控条件压缩并指导基准维护。提供了22基准参考图谱和四步诊断工作流，基准维护者可用少量代码运行。
...
### [112] [Rigorous Explanations for Tree Ensembles](https://arxiv.org/abs/2603.29361)
*Yacine Izza,Alexey Ignatiev,Xuanxiang Huang,Peter J. Stuckey,Joao Marques-Silva*

Main category: cs.AI

TL;DR: 研究如何为随机森林和提升树这两种树集成方法计算严格定义且逻辑合理的预测解释，以增强模型的可解释性和可信度。


<details>
  <summary>Details</summary>
Motivation: 树集成方法虽然表示简洁，但预测过程对人类决策者不透明，需要通过识别解释来建立信任。

Method: 针对随机森林和提升树两种树集成方法，计算严格定义且逻辑合理的解释。

Result: 能够为随机森林和提升树的预测提供严格且逻辑合理的解释。

Conclusion: 该研究为树集成方法的解释提供了严格定义和逻辑合理的方法，增强了预测的可解释性和可信度。
...
### [113] [AI-Generated Prior Authorization Letters: Strong Clinical Content, Weak Administrative Scaffolding](https://arxiv.org/abs/2603.29366)
*Moiz Sadiq Awan,Maryam Raza*

Main category: cs.AI

TL;DR: 本研究评估了三种商业大型语言模型(GPT-4o、Claude Sonnet 4.5、 Gemini 2.5 Pro)在45个跨5个医学专科的医生验证合成场景中生成预先授权信件的能力。结果显示所有模型都能生成临床内容充分的信件，但存在行政要求方面的系统性问题。


<details>
  <summary>Details</summary>
Motivation: 预先授权是美国医疗保健中最繁重的行政流程之一，消耗大量资金和医生时间。尽管大型语言模型在临床文本任务中显示出前景，但它们生成准备提交的预先授权信件的能力此前仅受到有限关注，且仅限于单案例演示。

Method: 研究评估了三种商业可用的大型语言模型，使用45个医生验证的合成场景，涵盖风湿病学、精神病学、肿瘤学、心脏病学和骨科五个专科，并对生成的信件进行临床评分和行政要求二次分析。

Result: 所有三个模型都生成了具有强临床内容的信件：准确的诊断、结构良好的医学必要性论证和全面的步骤治疗文档。然而，二次分析显示存在一致的系统性差距，包括缺失账单代码、缺失授权期限请求和不充分的随访计划。

Conclusion: 研究重新定义了问题：临床部署的挑战不在于LLM是否能写出临床充分的信件，而在于围绕它们构建的系统是否能提供支付方工作流程所需的行政精确性。
...
### [114] [Structural Compactness as a Complementary Criterion for Explanation Quality](https://arxiv.org/abs/2603.29491)
*Mohammad Mahdi Mesgari,Jackie Ma,Wojciech Samek,Sebastian Lapuschkin,Leander Weber*

Main category: cs.AI

TL;DR: 提出了一种基于图结构的归因质量评估指标——最小生成树紧凑度(MST-C)，用于评估解释的可读性，通过捕获归因的高阶几何属性（散布和内聚性），将多个组件组合成单一分数来评估紧凑度。


<details>
  <summary>Details</summary>
Motivation: 归因质量的定量评估特别困难，因为归因的形状和内部组织的多样性无法被简单的统计数据捕获，现有方法缺乏对解释紧凑性的有效度量。

Method: 提出最小生成树紧凑度(MST-C)，一种基于图结构的度量方法，捕获归因的高阶几何属性（spread和cohesion），将这些组件组合成单一分数来评估紧凑度，偏好显著性点散布在小区域内且空间组织成少量内聚聚类的归因。

Result: MST-C能够可靠地区分不同的解释方法，揭示模型之间的基本结构差异，提供了一个稳健的、自成一体的解释紧凑性诊断方法，补充了现有的归因复杂度概念。

Conclusion: MST-C作为归因质量评估的新指标，能够有效评估解释的可读性，补充了现有归因复杂度概念，为归因方法的比较提供了新的工具。
...
### [115] [Learning to Generate Formally Verifiable Step-by-Step Logic Reasoning via Structured Formal Intermediaries](https://arxiv.org/abs/2603.29500)
*Luoxin Chen,Yichi Zhou,Huishuai Zhang*

Main category: cs.AI

TL;DR: PRoSFI通过形式化验证中间推理步骤来训练LLMs，提高推理可靠性，无需模型生成完整的形式化证明。


<details>
  <summary>Details</summary>
Motivation: 结果奖励忽视了有缺陷的中间推理步骤，导致即使最终答案正确，推理过程也不可靠。

Method: 模型输出与其自然语言推理一致的结构化中间步骤，每个步骤由形式化证明器验证，只有完全验证的推理链才能获得高奖励。

Result: 形式化验证引导模型生成可机器检查的逐步证明，产生更可信的最终答案。

Conclusion: PRoSFI提供了一种训练可信推理模型的简单有效方法。
...
### [116] [FlowPIE: Test-Time Scientific Idea Evolution with Flow-Guided Literature Exploration](https://arxiv.org/abs/2603.29557)
*Qiyao Wang,Hongbo Wang,Longze Chen,Zhihao Yang,Guhong Chen,Hamid Alinejad-Rokny,Hui Li,Yuan Lin,Min Yang*

Main category: cs.AI

TL;DR: FlowPIE是一个紧密耦合的检索-生成框架，将文献探索和idea生建模为协同进化过程，利用flow-guided MCTS引导自适应检索并通过进化算法产生多样化高质量的科学idea


<details>
  <summary>Details</summary>
Motivation: 现有科学idea生成(SIG)方法受限于静态检索-生成范式，导致生成的idea同质化严重且不够发散；同时过度依赖参数知识和静态文献容易形成信息茧房

Method: 1) 基于GFlowNets的flow-guided MCTS，利用LLM评估的GRM作为监督信号引导自适应检索；2) 将idea生建模为测试时进化过程，采用选择、交叉、变异和隔离岛屿范式；3) 使用GRM计算跨领域知识的适应度

Result: FlowPIE在idea的新颖性、可行性和多样性方面均优于强的LLM-based和agent-based框架，并能实现测试时奖励Scaling

Conclusion: FlowPIE通过将检索与生成紧密耦合，结合MCTS引导的文献探索与基于GRM的进化算法，实现了高质量、多样化的科学idea生成，有效缓解了信息茧房问题，显著优于现有LLM-based和agent-based框架。
...
### [117] [Beyond the Steeper Curve: AI-Mediated Metacognitive Decoupling and the Limits of the Dunning-Kruger Metaphor](https://arxiv.org/abs/2603.29681)
*Christopher Koch*

Main category: cs.AI

TL;DR: 本文挑战了生成式AI简单放大邓宁-克鲁格效应的观点，提出AI介导的元认知解耦模型：产出、底层理解、校准准确性和自我评估能力之间出现widening gap，这一四变量模型比简化的邓宁-克鲁格曲线更能解释过度自信、过度/不足依赖、拐杖效应和弱迁移现象。


<details>
  <summary>Details</summary>
Motivation: 挑战关于生成式AI简单放大邓宁-克鲁格效应的粗略 claim，认为这种说法 too coarse to capture the available evidence

Method: 综合了人机交互、学习研究和模型评估的证据

Result: LLM使用可改善可观察的输出和短期任务表现，但会降低元认知准确性并flatten the classic competence-confidence gradient across skill groups

Conclusion: 论文提出AI介导的元认知解耦模型 better explains overconfidence, over- and under-reliance, crutch effects, and weak transfer than the simpler Dunning-Kruger metaphor. Implications for tool design, assessment, and knowledge work.
...
### [118] [A First Step Towards Even More Sparse Encodings of Probability Distributions](https://arxiv.org/abs/2603.29691)
*Florian Andreas Marwitz,Tanya Braun,Ralf Möller*

Main category: cs.AI

TL;DR: 本文提出了一种从概率分布中提取一阶公式的方法，通过减少分布中的值数量来显著降低编码所需的数值，然后为每个值提取逻辑公式以进一步最小化。这种减少和最小化方法可以在增加编码稀疏性的同时泛化给定分布。评估表明，通过提取少量短公式可以极大地提高稀疏性，同时保留核心信息。


<details>
  <summary>Details</summary>
Motivation: 现实世界场景可以用提升的概率分布来捕获。然而，分布通常以表格或列表形式编码，需要指数级的数值数量来表示。

Method: 提出了一种从概率分布中提取一阶公式的方法，通过减少分布中的值数量来降低编码所需的数值，然后为每个值提取逻辑公式并进一步最小化。

Result: 评估表明，通过提取少量短公式可以极大地提高稀疏性，同时保留核心信息。

Conclusion: 该方法在增加编码稀疏性的同时能够泛化给定分布，为处理大规模概率分布提供了一种有效的压缩表示方式。
...
### [119] [Measuring the metacognition of AI](https://arxiv.org/abs/2603.29693)
*Richard Servajean,Philippe Servajean*

Main category: cs.AI

TL;DR: 本文提出使用meta-d'框架和信号检测理论(SDT)来评估AI系统的元认知能力，即其生成区分正确与错误响应的置信评级的能力，并在三个大型语言模型(GPT-5、DeepSeek-V3.2-Exp、Mistral-Medium-2508)上进行实验验证。


<details>
  <summary>Details</summary>
Motivation: AI系统日益集成到决策流程中，管理不确定性越来越依赖于这些系统的元认知能力，即评估自身决策可靠性并调节决策的能力。因此，迫切需要采用稳健的方法来测量AI的元认知能力。

Method: 采用meta-d'框架（或其无模型替代方案）作为评估AI元认知敏感性的金标准，并利用信号检测理论(SDT)测量AI基于不确定性和风险自发调节决策的能力。实验包括两个系列：第一组实验中LLMs执行主要判断后进行置信评级；第二组实验中LLMs仅执行主要判断，同时操纵与每个响应相关的风险。

Result: meta-d'框架允许沿三个轴进行比较：比较LLM与最优性、比较不同LLM在同一任务上的表现、以及比较同一LLM在不同任务上的表现。SDT允许评估LLMs在高风险情况下是否变得更加保守。

Conclusion: meta-d'框架和SDT是评估AI元认知能力的有效工具，可用于比较AI系统与人类认知功能的异同，为AI决策的可信性评估提供方法论贡献。
...
### [120] [Symphony for Medical Coding: A Next-Generation Agentic System for Scalable and Explainable Medical Coding](https://arxiv.org/abs/2603.29709)
*Joakim Edin,Andreas Motzfeldt,Simon Flachs,Lars Maaløe*

Main category: cs.AI

TL;DR: 一种名为Symphony的医学编码系统，模仿人类专家编码员工作方式，通过访问编码指南进行推理，可跨任何编码系统工作并提供预测依据，在多个数据集上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 医学编码是将临床文档转换为标准化代码，对账单、临床研究和质量报告至关重要。然而现有方法只能预测固定代码集，无法适应新代码或不同编码系统，且缺乏可解释性，难以在安全关键场景中建立信任。

Method: 提出Symphony系统，模仿人类专家编码员的方式工作：通过推理临床叙述并直接访问编码指南来进行编码。该设计允许系统跨任何编码系统运行，并提供span级别的证据链接，将每个预测代码与支持它的文本关联起来。

Result: 在两个公共基准和三个真实世界数据集上评估，覆盖美国和英国的门诊、住院、急诊和专科设置。Symphony在所有设置中均达到最先进水平。

Conclusion: Symphony成为一个灵活的、可直接部署的自动化临床编码基础方案，能够解决传统方法无法适应新编码系统和缺乏可解释性的问题。
...
### [121] [Reinforced Reasoning for End-to-End Retrosynthetic Planning](https://arxiv.org/abs/2603.29723)
*Chenyang Zuo,Siqi Fan,Yizhen Luo,Zaiqing Nie*

Main category: cs.AI

TL;DR: ReTriP是一个端到端的生成式框架，将逆合成规划重新表述为直接的思维链推理任务，通过路径一致的分子表示和渐进式训练课程（从推理蒸馏到带可验证奖励的强化学习）来实现。


<details>
  <summary>Details</summary>
Motivation: 逆合成规划是有机化学中的基础任务，但由于其组合复杂性而具有挑战性。传统方法通常依赖混合框架，结合单步预测和外部搜索启发式，这不可避免地破坏了局部分子转化与全局规划目标之间的逻辑连贯性。

Method: 建立路径一致的分子表示，采用渐进式训练课程，从推理蒸馏过渡到带可验证奖励的强化学习，有效地将逐步生成与实际路线效用对齐。

Result: 在RetroBench上的实证评估表明，ReTriP实现了最先进的性能，在长视野规划方面表现出比混合基线更强的鲁棒性。

Conclusion: ReTriP通过将逆合成重新定义为思维链推理任务，成功地将复杂的战略预见嵌入到模型的化学推理中，解决了传统方法中逻辑连贯性断裂的问题。
...
### [122] [Spontaneous Functional Differentiation in Large Language Models: A Brain-Like Intelligence Economy](https://arxiv.org/abs/2603.29735)
*Junjie Zhang,Zhen Shen,Gang Xiong,Xisong Dong*

Main category: cs.AI

TL;DR: 大型语言模型自发形成类似人脑的协同信息整合核心，中间层负责协同处理而两端层依赖冗余，这随任务难度呈现动态相变，协同组件的消除会严重损害模型性能。


<details>
  <summary>Details</summary>
Motivation: 人工系统中智力的进化提供了一个独特机会来识别通用计算原则，研究者试图通过大型语言模型揭示类似人脑的计算机制。

Method: 使用Integrated Information Decomposition（信息整合分解）跨多个架构进行分析，探究信息整合与不同层次的关系。

Result: 中间层表现出协同处理（synergistic processing），而早期和晚期层依赖冗余（redundancy）；这种组织是动态的，随任务难度增加呈现物理相变；关键的是，消除协同组件会导致灾难性性能损失。

Conclusion: 协同处理是抽象推理的物理实体，弥合了人工和生物智能之间的差距。这一发现为理解AI系统的内部工作机制提供了新的理论框架。
...
### [123] [CausalPulse: An Industrial-Grade Neurosymbolic Multi-Agent Copilot for Causal Diagnostics in Smart Manufacturing](https://arxiv.org/abs/2603.29755)
*Chathurangi Shyalika,Utkarshani Jaimini,Cory Henson,Amit Sheth*

Main category: cs.AI

TL;DR: CausalPulse是一个工业级多智能体副驾驶系统，用于智能制造中的因果诊断，通过神经符号架构统一异常检测、因果发现和推理，在博世制造厂部署后在两个数据集上达到98%+的成功率，具备50-60秒延迟和近线性可扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统分析流程将异常检测、因果推理和根因分析视为孤立阶段，限制了可扩展性和可解释性。现代制造环境需要实时、可信和可解释的根因洞察来维持生产率和质量。

Method: 提出了一种神经符号架构，建立在标准化智能体协议之上，统一了异常检测、因果发现和推理三个阶段。该系统采用多智能体协作模式，包含规划与工具使用、自我反思和协作等关键能力模块。

Result: 在Future Factories和Planar Sensor Element两个数据集上分别达到98.0%和98.73%的总体成功率；规划与工具使用达98.75%，自我反思达97.3%，协作达99.2%；端到端延迟50-60秒，具有近线性可扩展性(R²=0.97)；相比现有工业副驾驶具有模块化、可扩展性和部署成熟度的优势。

Conclusion: CausalPulse通过其模块化、可扩展的神经符号架构，成功实现了工业级制造环境中的因果诊断自动化。该系统在罗伯特·博世制造厂的部署表明其能够与现有监控系统无缝集成，支持生产规模的实时运行。其高成功率（98%+）、低延迟（50-60秒）和近线性可扩展性验证了下一代制造中可靠、可解释的自动化潜力。
...
### [124] [Tracking vs. Deciding: The Dual-Capability Bottleneck in Searchless Chess Transformers](https://arxiv.org/abs/2603.29761)
*Quanhao Li,Wei Jiang*

Main category: cs.AI

TL;DR: 本文提出了一种人类风格的象棋引擎训练方法，通过从棋步序列学习状态跟踪和决策质量两种能力，最终120M参数模型在Lichess bullet达到2570评分，人类棋步预测Top-1准确率达55.2%，超过Maia-2。


<details>
  <summary>Details</summary>
Motivation: 当前象棋引擎追求最大化棋力，但人类风格引擎应该模仿强人类的风格、错误和一致性。现有方法忽略了一个关键问题：从棋步序列训练要求模型同时学习两种能力——状态跟踪（从棋步历史重构棋盘）和决策质量（从重构的棋盘选择好棋），这两种能力对数据有矛盾的需求——低评级比赛提供跟踪所需的多样性，高评级比赛提供决策学习的质量信号。

Method: 1. 将模型从28M扩展到120M参数以改善状态跟踪；2. 引入Elo加权训练以改善决策同时保持多样性；3. 2x2因子消融实验验证各因素效果；4. 使用线性加权（过激加权会损害跟踪）；5. 提出覆盖率衰减公式t* = log(N/kcrit)/log b作为可靠性边界。

Result: 最终120M参数模型（无搜索）在Lichess bullet达到2570评分（253场比赛）。人类棋步预测Top-1准确率达55.2%，超过Maia-2 rapid和Maia-2 blitz。扩展改善跟踪，加权改善决策，组合效果超叠加。

Conclusion: 序列输入方法自然编码完整棋局历史，支持位置模型无法实现的历史依赖决策。模型扩展提升状态跟踪能力，Elo加权训练提升决策质量，二者结合产生超叠加效应。线性加权效果最佳，过度激进的加权虽降低验证损失但损害跟踪能力。
...
### [125] [Owl-AuraID 1.0: An Intelligent System for Autonomous Scientific Instrumentation and Scientific Data Analysis](https://arxiv.org/abs/2603.29828)
...
TL;DR: Owl-AuraID是一种具身智能体系统，通过GUI原生范式操作精密科学仪器，整合GUI操作与数据分析技能，实现端到端的自动化实验室工作流，支持FTIR、NMR、AFM、TGA等十类仪器，为自主实验室提供可扩展解决方案。


<details>
  <summary>Details</summary>
Motivation: 科学发现越来越依赖高通量表征，但自动化受到专有GUI和现有基于API系统的有限通用性的阻碍。

Method: 提出Owl-AuraID软件硬件协作的具身智能体系统，采用GUI原生范式通过与人类专家相同的界面操作仪器。其技能中心化框架将Type-1（GUI操作）和Type-2（数据分析）技能整合到端到端工作流中，将物理样品处理与科学解释相连接。

Result: 在十类精密仪器和多样化工作流上展示了广泛覆盖，包括多模态光谱分析、显微镜成像和晶体学分析，支持FTIR、NMR、AFM和TGA等表征手段。

Conclusion: Owl-AuraID为自主实验室提供了实用且可扩展的基础设施，通过可重用的操作和分析技能为实验室智能的演进开辟了道路。该系统展示了在多种精密仪器上的广泛适用性，包括FTIR、NMR、AFM和TGA等表征手段，为自动化科学研究奠定了技术基础。
...
### [126] [Spatiotemporal Robustness of Temporal Logic Tasks using Multi-Objective Reasoning](https://arxiv.org/abs/2603.29868)
*Oliver Schön,Lars Lindemann*

Main category: cs.AI

TL;DR: 研究时空鲁棒性(STR)，一种将空间和时间扰动联合建模的多维鲁棒性度量，用于时序逻辑规范的分析。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒语义仅考虑空间扰动，无法同时捕捉时序逻辑规范在空间和时间维度上的扰动需求，特别是在多智能体机器人、智慧城市和空中交通管制等交互系统中。

Method: 将STR形式化为多目标推理问题，通过空间和时间扰动上的偏序关系定义，并利用多目标优化工具计算帕累托最优解集。

Result: 提出了STR的形式化定义和计算方法，设计了计算上易于处理的鲁棒语义进行下界近似，并给出了相应的监控算法。

Conclusion: 这是首个通过多目标推理方法处理多维鲁棒性的工作，为自主系统在不确定性下的可靠性分析提供了新的理论框架。
...
### [127] [A Rational Account of Categorization Based on Information Theory](https://arxiv.org/abs/2603.29895)
*Christophe J. MacLellan,Karthik Singaravadivelan,Xin Lian,Zekun Wang,Pat Langley*

Main category: cs.AI

TL;DR: 提出基于信息论理性分析的新分类理论，通过与经典分类实验对比验证，表明该理论在解释人类分类行为方面优于或等同于现有主要分类模型。


<details>
  <summary>Details</summary>
Motivation: 需要提出新的分类理论来更好地解释人类分类认知机制，之前的理论在解释经典实验结果方面存在不足。

Method: 通过将新理论与经典分类实验（Hayes-Roth & Hayes-Roth 1977; Medin & Schaffer 1978; Smith & Minda 1998）的结果进行对比，评估其对人类分类行为的解释能力，并与四种现有模型进行比较。

Result: 该新理论能够解释经典分类实验中的人类分类行为，且解释效果至少与独立线索和上下文模型、理性模型、层次Dirichlet过程模型相当或更优。

Conclusion: 该信息论理性分析理论在解释人类分类行为方面表现出色，至少与现有主要模型（独立线索和上下文模型、理性模型、层次Dirichlet过程模型）相当或更优，为分类认知机制提供了新的理论框架。
...
### [128] [ATP-Bench: Towards Agentic Tool Planning for MLLM Interleaved Generation](https://arxiv.org/abs/2603.29902)
*Yinuo Liu,Zi Qian,Heng Zhou,Jiahao Zhang,Yajie Zhang,Zhihang Li,Mengyu Zhou,Erchao Zhao,Xiaoxi Jiang,Guanjun Jiang*

Main category: cs.AI

TL;DR: 本文提出ATP-Bench基准和MAM评估系统，用于评估多模态大语言模型(MLLMs)的智能体工具规划能力，以实现交错的文本-图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前的交错文本-图像生成范式将图像生成和检索增强视为互斥路径，无法统一事实性与创造力。现有模型在工具调用规划和协同方面存在显著不足。

Method: 1) 构建ATP-Bench基准：包含7,702个问答对(其中1,592个VQA对)，涵盖8个类别和25个视觉关键意图，采用人工验证的查询和真值；2) 提出Multi-Agent MLLM-as-a-Judge(MAM)系统：评估工具调用精度、识别工具使用遗漏机会、评估整体响应质量。

Result: 在10个最先进的MLLMs上进行广泛实验，结果显示模型在连贯的交错规划方面存在困难，工具使用行为表现出显著差异，存在很大的改进空间。

Conclusion: 本文工作为推进交错生成研究提供了可操作的指导方向，表明智能体工具规划是下一个重要里程碑。
...
### [129] [C-TRAIL: A Commonsense World Framework for Trajectory Planning in Autonomous Driving](https://arxiv.org/abs/2603.29908)
*Zhihong Cui,Haoran Tang,Tianyi Li,Yushuai Li,Peiyuan Guan,Amir Taherkordi,Tor Skeie*

Main category: cs.AI

TL;DR: C-TRAIL是一个结合常识世界和信任机制的自动驾驶轨迹规划框架，通过LLM-derived常识与蒙特卡洛树搜索的融合，在轨迹规划任务中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶的轨迹规划越来越依赖大型语言模型(LLM)进行常识推理，但LLM输出本身不可靠，在安全关键应用中存在风险。

Method: C-TRAIL采用闭环的Recall、Plan和Update循环：Recall模块查询LLM获取语义关系并通过双重信任机制量化可靠性；Plan模块通过Dirichlet信任策略将信任加权的常识注入蒙特卡洛树搜索(MCTS)；Update模块从环境反馈中自适应地细化信任分数和策略参数。

Result: 在Highway-env的四个模拟场景和两个真实世界数据集(highD、rounD)上的实验表明，C-TRAIL始终优于最先进的基线方法，ADE降低40.2%，FDE降低51.7%，SR提高16.9个百分点。

Conclusion: C-TRAIL通过将LLM的常识推理与信任机制相结合，为自动驾驶轨迹规划提供了一种新颖的解决方案。该框架在模拟和真实世界数据集上均取得了显著的性能提升，证明了其在提升轨迹规划安全性和可靠性方面的潜力。源代码已公开，为后续研究提供了重要资源。
...
### [130] [ScoringBench: A Benchmark for Evaluating Tabular Foundation Models with Proper Scoring Rules](https://arxiv.org/abs/2603.29928)
*Jonas Landsgesell,Pascal Knoll*

Main category: cs.AI

TL;DR: 本文介绍了ScoringBench，一个用于评估表格基础模型的基准测试框架。与传统使用RMSE和R²等点估计指标不同，该基准测试采用完整的概率评分规则（如CRPS、CRLS、Interval Score、Energy Score等）来评估模型性能，尤其关注分布尾部对于高风险决策场景的影响。


<details>
  <summary>Details</summary>
Motivation: 现有表格基础模型已能生成完整的预测分布，但当前回归基准测试几乎 exclusively使用点估计指标（RMSE、R²），这些聚合指标往往掩盖了模型在分布尾部区域的表现差异。这在高风险决策领域（如金融和临床研究）中是一个关键缺陷，因为这些领域通常面临非对称风险配置文件。

Method: 作者构建了ScoringBench开源基准测试，计算了一套综合的 proper scoring rules，包括CRPS、CRLS、Interval Score、Energy Score、加权CRPS和Brier Score，同时保留标准点估计指标。他们评估了使用不同评分规则目标微调的realTabPFNv2.5以及TabICL，与未微调的realTabPFNv2.5进行比较。

Result: 实验结果证实：1）模型排名取决于所选择的评分规则；2）没有任何单一的预训练目标在所有情况下都是最优的；3）对于对极端事件敏感的应用场景，评估指标的选择与数据本身一样是特定领域的 requirement。

Conclusion: ScoringBench基准测试表明，在评估表格基础模型时，选择合适的评分规则至关重要。未来的模型评估应该根据具体应用场景的风险特征选择相应的评分指标，而非仅依赖传统的点估计指标。该基准测试为概率预测的质量评估提供了更全面的视角。
...
### [131] [Physiological and Semantic Patterns in Medical Teams Using an Intelligent Tutoring System](https://arxiv.org/abs/2603.29950)
*Xiaoshan Huang,Conrad Borchers,Jiayi Zhang,Susanne P. Lajoie*

Main category: cs.AI

TL;DR: 本研究探究了医学团队在诊断虚拟患者时的社会共享学习调节(SSRL)与生理同步现象，发现对话中的语义转换与生理同步峰值相关，且高生理同步时语言使用更加多样化。成功的团队在共同发现时同步，而失败的团队在共同不确定时同步。


<details>
  <summary>Details</summary>
Motivation: 生理同步可以指示团队协作中的认知和情绪状态，但单独难以解释。需要将生理信号与对话数据融合，以理解问题解决过程中的关键时刻。

Method: 研究四个医学二元组使用智能辅导系统诊断虚拟患者病例，对对话进行语义编码和句子嵌入余弦相似度计算，同时测量生理信号同步，并进行定性分析三角验证。

Result: 激活先前知识的语义相似度显著低于简单的任务执行；高生理同步与低语义相似度相关，表明此时语言使用更具探索性和多样性；定性分析将同步峰值确定为"关键时刻"：成功团队在共同发现时同步，失败团队在共同不确定时同步。

Conclusion: 生理信号与对话的融合可以有效识别团队协作中的关键决策时刻，为人本AI提供新思路：生物信号可用于理解和优化问题解决过程。
...
### [132] [Structured Intent as a Protocol-Like Communication Layer: Cross-Model Robustness, Framework Comparison, and the Weak-Model Compensation Effect](https://arxiv.org/abs/2603.29953)
*Peng Gang*

Main category: cs.AI

TL;DR: 研究结构化意图框架（PPS）在跨模型、语言和提示框架下的目标对齐效果，发现其能显著降低跨语言方差并提升人机交互效率。


<details>
  <summary>Details</summary>
Motivation: 探究结构化意图表示在不同AI模型（Claude、GPT-4o、 Gemini）、语言和提示框架下保持用户目标的可靠性。

Method: 跨模型实验（3语言×6条件×3模型×3领域×20任务=3240个输出），独立评估器（DeepSeek-V3），与CO-STAR和RISEN对比，50人用户研究。

Result: 结构化提示将跨语言标准差从0.470降至约0.020；弱模型（Gemini）获益更大（D-A增益+1.006 vs Claude的+0.217）；5W3H、CO-STAR、RISEN达到相似高目标对齐分数；用户研究中交互轮次减少60%，满意度从3.16升至4.04。

Conclusion: 维度分解本身是关键因素；结构化意图表示作为人机交互的协议层具有实用价值和鲁棒性。
...
### [133] [Extending MONA in Camera Dropbox: Reproduction, Learned Approval, and Design Implications for Reward-Hacking Mitigation](https://arxiv.org/abs/2603.29993)
*Nathan Heath*

Main category: cs.AI

TL;DR: 本文是对MONA方法的复现与扩展研究。MONA通过限制智能体的规划视野同时提供远见approval信号来缓解多步奖励黑客问题。研究在MONA相机投放环境复制了原始结果（普通RL奖励黑客率91.5%，oracle MONA为0%），并引入了模块化的学习approval套件。实验表明，校准后的学习监督者虽达到零奖励黑客，但意图行为率（11.9%）远低于oracle MONA（99.9%），表明问题从概念验证转向构建足够远见且不重新打开奖励黑客通道的学习approval模型。


<details>
  <summary>Details</summary>
Motivation: 原始MONA论文提出的开放问题：approval的构建方式（特别是approval对已达成结果的依赖程度）如何影响MONA的安全保证。研究旨在通过复现和扩展来操作化这一"approval谱系猜想"。

Method: 将MONA发布代码重组为标准Python项目，使用脚本化PPO训练；复现原始论文中普通RL与oracle MONA的对比；引入模块化学习approval套件，涵盖oracle、noisy、misspecified、learned和calibrated等多种approval机制；在降低预算下进行Pilot实验，测试不同approval方法、视野、数据集大小和校准策略。

Result: 复现确认：普通RL奖励黑客率91.5%，oracle MONA为0%。最佳校准学习监督者实现零观察奖励黑客，但意图行为率显著低于oracle MONA（11.9% vs 99.9%），这与欠优化而非重新出现的黑客一致。

Conclusion: 研究将MONA论文的approval谱系猜想转化为可运行的实验对象，表明核心工程挑战从证明MONA概念转向构建保留足够远见的学习approval模型，同时不重新打开奖励黑客通道。
...
### [134] [Denoising data reduction algorithm for Topological Data Analysis](https://arxiv.org/abs/2603.29248)
*Seonmi Choi,Semin Oh,Jeong Rye Park,Seung Yeop Yang*

Main category: cs.CG

TL;DR: 本文提出RCLA算法，结合数据降维和去噪的单一程序，通过阈值参数k去除噪声并保留数据本质结构，在齐次泊松噪声模型下具有稳定性保证，实验表明优于现有方法，并在3D形状分类任务中得到验证。


<details>
  <summary>Details</summary>
Motivation: 持久同调是拓扑数据分析的核心工具，但在处理大规模和噪声数据时存在计算成本高和伪拓扑特征的问题。噪声不仅增加数据规模，还掩盖了数据的底层结构。

Method: 提出Refined Characteristic Lattice Algorithm (RCLA)，一种基于网格的方法，将数据降维与基于阈值的去噪集成在单个程序中。通过引入阈值参数k，在单次遍历中去除噪声同时保留数据的本质结构。

Result: RCLA在实验中始终优于现有方法，并在3D形状分类任务中得到验证。

Conclusion: 该方法通过稳定性定理提供了理论保证，在齐次泊松噪声模型下，以高概率约束输出与底层形状的持久图之间的瓶颈距离。此外，还引入了基于最近邻统计的自动参数选择方法。
...
### [135] [Computing Topological Transition Sets for Line-Line-Circle Trisectors in $R^3$](https://arxiv.org/abs/2603.29540)
*Eunku Park*

Main category: cs.CG

TL;DR: 本文提出了一种高效的精确验证框架，用于计算R^3中混合几何对象的Voronoi图。核心贡献是分析了两条 skew 线和一個圆的三重分割（trisector）的拓扑性质，通过切锥分析、判别式计算和方向 blow-up 坐标等方法，识别出 k=0,1 作为分叉值，并验证了这些确实是展示可约分裂的拓扑壁。这项工作为超越二次曲面范畴的混合对象Voronoi图构建提供了必要的精确谓词。


<details>
  <summary>Details</summary>
Motivation: 计算R^3中混合几何对象的Voronoi图具有挑战性，因为通过柱面代数分解(CAD)进行精确几何谓词的成本很高。现有方法（如 pencil-of-quadrics 分析）仅适用于三个直线的情况，无法处理圆和直线混合的情况。需要一个高效的精确验证框架来表征参数空间连通性。

Method: 1. 使用精确符号评估识别转换壁；2. Jacobian 计算认证没有仿射奇点；3. 投影闭包显示奇点行为在无穷远点 p∞ 处是孤立的；4. 在 p∞ 处进行切锥分析得到判别式 ΔQ = 4ks²(k-1)；5. 使用方向 blow-up 坐标验证拓扑局部恒定；6. 认证 k=0,1 是实际拓扑壁。

Result: 识别出 k=0,1 为分叉值，验证了这些是展示可约分裂的实际拓扑壁，确认在转换壁之间 trisector 的实拓扑保持局部恒定，为混合对象 Voronoi 图的精确构建提供了理论基础。

Conclusion: 成功突破了二次曲面分析的限制，为超越 quadric-only  regime 的混合对象 Voronoi 图构建提供了精确谓词的理论基础，对计算几何和符号计算领域具有重要贡献。
