...
TL;DR: 本文研究CLIP嵌入为何导致Stable Diffusion模型的记忆化问题，发现<pad>标记复制了<eot>嵌入，导致模型过度依赖<eot>从而引发记忆化，并提出两种简单有效的推理时缓解策略。


<details>
  <summary>Details</summary>
Motivation: 理解文本嵌入如何促进文本到图像扩散模型中的记忆化对于可解释性和安全性至关重要，但此前人们对此机制知之甚少。

Method: 将输入标记分类为<sot>、<pr>、<eot>和<pad>，分析各类嵌入对生成的影响，通过比较记忆化案例与非记忆化案例来确定关键嵌入。

Result: 发现<pr>嵌入在记忆化案例中贡献很小，而<pad>嵌入因复制了<eot>（CLIP训练中唯一明确优化的嵌入）而强烈影响记忆化。这种无意的复制放大<eot>的影响，导致记忆化。两种缓解策略（替换<pad>为! token并屏蔽<eot>，或部分屏蔽<pad>）均可抑制记忆化且不降低生成质量。

Conclusion: 通过识别<pad>标记对<eot>嵌入的复制是记忆化的根本原因，提出简单有效的推理时缓解策略，无需先验检测即可部署。
...
TL;DR: VANGUARD是一个统一的视频异常检测框架，通过三级课程训练整合异常分类、空间定位和思维链推理。在UCF-Crime上达到94% ROC-AUC和84% F1，并具备跨领域零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统视频异常检测方法仅做二元分类或离群点检测，缺乏可解释推理和精确空间定位。视觉语言模型虽然能理解场景，但难以可靠地进行空间定位，常产生幻觉或几何无效的边界框。

Method: 提出VANGUARD框架，统一异常分类、空间定位和思维链推理。采用三级课程训练：(1)冻结骨干特征进行分类器热身训练，(2)LoRA适应空间定位，(3)思维链生成。使用Qwen3-VL-4B作为教师模型生成结构化推理轨迹，GroundingDINO提供边界框监督。

Result: 在UCF-Crime数据集上，VANGUARD达到94% ROC-AUC和84% F1，同时生成可解释的思维链解释和异常物体空间定位。消融实验验证了三级训练优于单阶段优化，思维链推理作为隐式正则化使预测更平衡。零样本迁移到XD-Violence和ShanghaiTech证明了跨领域泛化能力。

Conclusion: VANGUARD在UCF-Crime上达到94% ROC-AUC和84% F1，显著优于现有方法。三级课程训练比单阶段优化效果更好，思维链推理作为隐式正则化使预测更平衡。零样本迁移到XD-Violence和ShanghaiTech证明了跨领域泛化能力。
...
TL;DR: 本研究聚焦于类器官（organoids）的三维细胞培养技术，提出了一种结合分段任意模型（SAM）与领域专用工具的复合方法，用于自动测量由多能干细胞衍生的 spheroids 的大小和形状。该方法在类器官图像数据上进行测试，并与人工分割结果进行比较。


<details>
  <summary>Details</summary>
Motivation: 类器官开发过程具有动态形态和细胞组织变化，需要先进成像和分析工具来准确监测类器官生长轨迹和研究疾病过程。现有工具无法在所有测试条件下达到足够精度，因此需要开发更准确的自动分割方法。

Method: 引入复合方法，将分段任意模型（SAM，一种通用基础模型）与现有领域专用工具相结合，并在类器官图像数据上评估该方法与多种现有工具的性能差异。

Result: 单一现有工具无法在所有测试条件下达到足够精度，但新引入的复合方法除少数最具挑战性的图像外，在所有图像上产生一致准确的结果。该方法在一个指标上达到观察者间变异的水平，在其他指标上非常接近观察者间变异。

Conclusion: 复合方法（SAM+领域专用工具）是类器官图像分割的有效工具，其精度接近人工标注者之间的一致性水平。
...
TL;DR: 本文提出一种弱监督医学图像分割方法，仅利用摘要统计信息（如标注区域面积）训练分割模型，并结合少量像素的弱监督信号来提高分割性能。提出一种结合图像重建质量、统计信息匹配和前景重叠的新型损失函数，在超声（乳腺癌）和CT（肾脏肿瘤）数据上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 医学专家通常手动分割图像以获取诊断统计信息，但只保留统计结果而丢弃标注数据。本文旨在训练分割模型来减轻这一负担，但仅使用保留的摘要统计信息（如区域面积），探索仅凭统计信息能否训练有效的分割模型。

Method: 提出一种新型损失函数，包含三个部分：1）图像重建质量项；2）摘要统计信息匹配项（使预测区域的统计值匹配保留的统计信息）；3）弱监督信号重叠项（利用少量像素标注作为弱监督）。在标准图像、超声（乳腺癌）和CT（肾脏肿瘤）数据集上进行了实验验证。

Result: 实验表明，仅使用统计信息的监督效果有限，但添加少量像素的弱监督信号后，分割性能显著提升。该方法在乳腺癌超声图像和肾脏肿瘤CT扫描数据上均表现出良好的实用性和潜力。

Conclusion: 仅依靠摘要统计信息训练分割模型存在困难，但结合少量像素的弱监督信号可有效解决这一问题。该方法为医学图像分割中的弱监督学习提供了新的思路，具有減少专家标注负担的临床应用前景。
...
TL;DR: 本研究针对医学图像分割中跨模态泛化能力不足的问题，系统地研究了针对跨模态迁移设计的数据增强技术，通过在三组脊柱分割模型上的实验，验证了该方法能在保持原域精度的同时，显著提升跨域性能（Dice增益155%），并将训练效率提高约10%，并以开源工具箱形式发布。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割模型性能受限于高标注数据的稀缺性和跨成像协议泛化能力不足的问题，特别是在MRI和CT中，单一采集序列训练的模型在应用于未见过的序列或对比度时鲁棒性显著下降。数据增强虽被广泛用于改善泛化性，但其对跨模态泛化的定量影响尚未被探索。

Method: 研究了一组针对跨模态迁移设计的数据增强技术。训练三个脊柱分割模型，每个模型使用单一模态/序列数据集，然后在七个域外数据集（涵盖CT和MRI）上进行评估反映了现实的单序列训练和多序列/对比度/模态部署场景。为降低计算成本，实现了GPU优化增强。

Result: 在未见域上获得 substantial 性能提升（平均Dice增益155%），同时保持域内精度（平均Dice下降0.008%），包括CT和MRI之间的有效迁移。GPU优化增强保持甚至提高了训练效率约10%。

Conclusion: 以开源工具箱形式发布，可无缝集成到nnUNet和MONAI等常用框架中。这些增强技术显著增强了对异构临床成像场景的鲁棒性，且不影响训练速度。
...
TL;DR: 本文提出了一个名为NucEval的统一评估框架，用于解决核实例分割评估中的四个关键问题：模糊区域处理、分数归一化、重叠实例和边界不确定性。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，核实例分割是一个基础任务，但评估流程受到的关注较少。作者识别出现有的评估方法存在四个关键问题：模糊区域处理、分数归一化、重叠实例和边界不确定性，这些问题会影响分割指标的准确性。

Method: 作者提出了四个针对性的修改方案，并将其整合到统一的NucEval框架中。使用NuInsSeg数据集以及两个外部数据集，对三种基于CNN和ViT的核实例分割模型进行评估，以展示这些修改对实例分割指标的影响。

Result: 通过在多个数据集和多种分割模型上的实验，验证了所提出的修改方案对实例分割指标的负面影响，证明了NucEval框架的必要性。

Conclusion: NucEval框架能够 robustly 评估核实例分割，为计算病理学领域提供了一个标准化的评估工具，有助于推动该领域的发展。
...
TL;DR: 本文针对野火蔓延预测中的不确定性量化问题，提出FCER框架来评估模型在关键火灾区域的不确定性表现，实验表明蒸馏学生模型与集成模型性能相当。


<details>
  <summary>Details</summary>
Motivation: 可靠的野火蔓延预测对风险感知应急规划至关重要，但大多数深度学习模型缺乏原则性的不确定性量化(UQ)；且对于边界敏感的野火蔓延案例，仅用全局metrics评估模型往往不够

Method: 引入Fire-Centered Evaluation Region (FCER)框架作为空间条件化协议，在关键火灾区域内表征不确定性；使用FCER比较ensemble模型与蒸馏的单程学生模型在WildfireSpreadTS数据集上的表现

Result: 学生模型表现出可比的校准性能和边界相关 regime 下互补的不确定性排序

Conclusion: FCER框架提供了一种更具操作相关性的方法来评估火灾边界区域的模型不确定性量化性能。
...
TL;DR: 本文提出CAFe-DINO模型，利用DINOv3和DINO.txt实现遥感图像的开放词汇语义分割(OVSS)，无需在遥感数据上进行微调。模型通过成本聚合和文本-图像相似度分数的无训练上采样，在关键遥感分割数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 遥感领域缺乏密集标注数据集，现有无需监督微调的模型性能不如监督方法。

Method: CAFe-DINO利用DINOv3强大特征表示，通过成本聚合(cost aggregation)和训练-free的特征上采样(text-image similarity scores upsampling)，在COCO-Stuff的遥感目标子集上微调，而非遥感图像。

Result: CAFe-DINO在关键遥感分割数据集上取得最先进性能，优于在遥感数据上微调的OVSS方法。

Conclusion: 首次将DINOv3与DINOtxt结合用于遥感开放词汇语义分割，在无遥感领域微调下实现SOTA表现。
...
TL;DR: 本文提出了Sentinel2Cap，一个新的人注释多模态卫星图像字幕数据集，包含Sentinel-1 SAR和Sentinel-2多光谱图像_patch，使用Qwen3-VL-8B-Instruct模型进行零样本字幕生成评估。


<details>
  <summary>Details</summary>
Motivation: 现有图像字幕数据集主要集中在自然图像和高分辨率光学遥感图像，而多模态卫星数据（尤其是SAR和中分辨率传感器）的字幕数据集仍然缺乏，这阻碍了跨模态场景理解的研究进展。

Method: 构建了Sentinel2Cap数据集，包含Sentinel-1 SAR和Sentinel-2多光谱图像patch（10m和20m空间分辨率），人工创建并验证字幕以确保语义准确性和语言质量。然后使用Qwen3-VL-8B-Instruct模型在三种图像模态（RGB、多光谱和SAR pseudo-RGB）上进行零样本字幕生成实验，并提供模态特定的上下文提示。

Result: RGB图像获得最高的字幕生成性能，而SAR图像对视觉语言模型来说更具挑战性。提供模态特定的上下文提示在所有指标上都持续提升了性能。

Conclusion: 多模态遥感图像字幕生成面临挑战，但人注释数据集具有推动跨模态场景理解研究的潜力。RGB模态表现最佳，SAR需要更多研究关注。
...
TL;DR: 本文提出了一种基于扩散模型的合成数据增强管道，用于解决医学长尾分类问题。通过结合修复扩散模型和分布外（OOD）后筛选机制，在ISIC2019皮肤病变分类数据集上实现了显著的性能提升，特别是在样本最少的尾类上提升了超过28%。


<details>
  <summary>Details</summary>
Motivation: 医学数据集中普遍存在长尾类别分布，深度学习模型在尾类上表现不佳，而罕见类往往对应严重疾病，需要高诊断准确率。现有方法如专用架构、重新加权损失函数和数据增强只能提供边际改进，且可扩展性有限。

Method: 提出了一种基于扩散模型的合成数据增强管道，包含新颖的修复扩散模型和OOD后筛选机制，以确保生成多样化、真实且具有临床意义的合成样本。

Result: 在ISIC2019皮肤病变分类数据集（最大且最不平衡的医学影像基准之一）上评估，方法实现了整体性能的实质性提升，在样本最少的类别上提升了超过28%。

Conclusion: 基于扩散模型的增强方法在缓解长尾不平衡和增强医学分类鲁棒性方面是有效的。
...
TL;DR: VEBENCH是首个全面评估大语言模型在真实视频编辑场景中编辑知识理解和操作推理能力的基准测试，包含3.9K高质量编辑视频（超257小时）和3080个人类验证的问答对。


<details>
  <summary>Details</summary>
Motivation: 现实世界的视频编辑不仅需要电影技术的专业知识，还需要多模态推理能力来选择、对齐和组合镜头。当前的大语言模型在多视频推理和操作编辑工作流程方面的能力尚未被探索，缺乏专门的评估基准。

Method: 通过三轮人机协作标注流程构建，包含两个互补的问答任务：1）视频编辑技术识别，评估模型使用多模态线索识别7种编辑技术的能力；2）视频编辑操作模拟，要求从多个候选镜头中选择相关片段并进行时间定位。实验覆盖了Gemini-2.5-Pro等专有模型和开源模型。

Result: 实验揭示了当前模型性能与人类水平编辑认知之间存在较大差距，表明需要紧急推进视频理解与创造性操作推理的结合研究。

Conclusion: VEBENCH作为视频编辑智能系统的基础设施，为推动复杂推理研究提供了重要平台，未来有望促进视频编辑领域的人工智能发展。
...
TL;DR: 本文提出FACTOR，一种无需参数更新的开放词汇目标检测测试时自适应框架，通过反事实推理机制识别并抑制非因果属性（如亮度、纹理）导致的虚假相关性，从而提升模型在分布偏移下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测在分布偏移下往往失败，因为模型容易被非因果视觉属性（如亮度、纹理）与目标类别之间的虚假相关性所误导。现有的测试时自适应方法要么依赖昂贵的在线优化，要么进行全局校准，忽略了这些失败的特殊属性本质。

Method: FACTOR通过反事实推理进行无训练测试时自适应：1)沿非因果属性扰动测试图像；2)比较原始视图与反事实视图的区域级预测；3)量化属性敏感性、语义相关性和预测变化；4)选择性抑制属性依赖性预测，无需参数更新。

Result: 在PASCAL-C、COCO-C和FoggyCityscapes数据集上的实验表明，FACTOR一致优于现有的测试时自适应方法，证明显式反事实推理能有效提升分布偏移下的鲁棒性。

Conclusion: 本文表明通过反事实推理机制，无需参数更新即可有效识别并抑制非因果属性对预测的影响，为开放词汇目标检测的分布偏移问题提供了一种轻量级且有效的解决方案。
...
TL;DR: TACO是一个紧耦合IMU+精细CVGL的定位管道，单次GNSS启动后仅依赖车载感知，在KITTI数据集上将 median ATE从97.0m降至16.3m（5.9倍提升），融合成本<0.1ms/帧，相机占空比5-10%。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度CVGL方法仅作为一次性定位器评估，从未作为实时管道的主要定位手段；而惯性导航虽提供高频率相对运动但会累积无界漂移。因此需要一种在GNSS信号受限环境下能够持续提供精确绝对定位的系统。

Method: 1) 闭环形式跨轨迹误差模型在IMU漂移超过匹配器捕获半径前触发CVGL；2) 前向偏置五点多crop搜索保持每次定位5次前向传递的固定推理成本；3) 航向残差门拒绝与 onboard 罗盘不符的定位；4) 各向异性体噪声模型按每次定位置信度缩放UKF更新；5) 带经验证回环闭合的因子图提供离线平滑轨迹。

Result: 在KITTI raw数据集上，TACO将中位数绝对轨迹误差(ATE)从97.0m（IMU-only）降至16.3m，实现5.9倍 reduction，融合成本<0.1ms/帧，相机占空比5-10%。

Conclusion: TACO成功实现了在单次GNSS启动后完全脱离GNSS的持续精确定位，通过紧耦合IMU和精细CVGL，显著降低了绝对轨迹误差，为GNSS受限环境下的定位提供了可行方案。
...
TL;DR: 本文提出Adaptive Hierarchical Prior Alignment (AHPA)框架，解决Diffusion Transformer训练中表征对齐的粒度适配问题。核心思想是利用VAE编码器的多层次特征作为动态监督信号，通过时间步条件的动态路由器自适应选择和权重不同层次的先验，从而在不同信噪比阶段提供匹配的训练监督。


<details>
  <summary>Details</summary>
Motivation: 现有表征对齐方法使用固定监督目标或固定对齐粒度，这是次优的。因为随信噪比变化，最优表征监督粒度也改变：高噪声时需要粗粒度语义/布局锚定，低噪声时需要细节精炼。静态单级监督器无法适应这种非平稳的对齐需求，导致表征不匹配。

Method: 提出AHPA框架，包含两个核心组件：1)多层次VAE特征提取：从冻结的VAE编码器提取多层特征，提供从局部几何/空间拓扑到粗粒度语义布局的互补先验；2)时间步条件动态路由器：根据当前时间步自适应选择和权重不同层次的先验，实现随去噪轨迹动态调整的对齐粒度。

Result: 大量实验表明AHPA在收敛速度和生成质量上优于基线方法，且无额外推理成本，同时避免了外部视觉编码器的监督。

Conclusion: AHPA通过层次化先验和动态路由机制，有效解决了扩散模型训练中表征对齐粒度不随时间步适配的问题，为表征对齐提供了新的有效范式。
...
TL;DR: 本文对4D Gaussian Splatting (4DGS) 动态场景重建方法进行了系统性分析，首次建立了可控基线FreeTimeGS_ours来复现FreeTimeGS的启发式方法，并在此基础上揭示了高斯时长驱动的emergent temporal partitioning和光度保真度与时空一致性之间的差异。基于这些发现，提出了FreeTimeGS++，采用门控边缘化和神经速度场实现更稳定的动态表示。


<details>
  <summary>Details</summary>
Motivation: 当前4DGS方法虽然取得了令人印象深刻的动态场景重建效果，但导致这些进展的具体驱动因素尚不清楚，缺乏对底层原理的系统性理解，使得后续研究和改进面临挑战。

Method: 1) 建立了FreeTimeGS_ours可控基线来形式化和复现FreeTimeGS的启发式方法；2) 沿基础轴对4DGS进行剖析，揭示了高斯时长驱动的emergent temporal partitioning机制；3) 发现了光度保真度与时空一致性之间的差异；4) 提出FreeTimeGS++，采用gated marginalization和neural velocity fields实现更稳健的动态表示。

Result: 所提出的FreeTimeGS++方法实现了更优的稳定性和鲁棒的动态表示，具有可复现的结果和降低的运行间方差。

Conclusion: 通过系统性分析揭示了4DGS的关键 secrets（高斯时长驱动的temporalpartitioning和光度保真度与时空一致性差异），基于此提出的FreeTimeGS++方法能够实现更稳定、可复现的动态场景重建，为未来4DGS研究提供了可靠基础。
...
TL;DR: 本文提出MedSR-Vision框架，统一评估医学图像超分辨率模型，为临床影像工作流中的模型选择提供标准化评估框架和实践指南。


<details>
  <summary>Details</summary>
Motivation: 医学图像超分辨率(MedSR)对于提高MRI、CT、X射线、超声和眼底成像等多种模态的诊断精度至关重要。尽管深度学习进展迅速，但在保持解剖准确性、感知质量和跨领域泛化方面仍面临挑战。

Method: 提出MedSR-Vision统一深度学习框架，在5种模态(脑MRI、胸部X光、肾超声、肾结石CT、脊柱MRI)和3种放大倍数(×2、×3、×4)下评估比较超分辨率模型。基准测试三个代表性模型：SRCNN、SwinIR和Real-ESRGAN，使用保真度、感知真实度和清晰度等多个定量指标。

Result: Real-ESRGAN在高放大倍数下获得最佳感知质量和边缘恢复；SwinIR在保留结构和诊断特征方面表现最优；SRCNN在较低放大倍数下提供高效稳定的性能。实验结果确立了对模态特定的见解和模型选择实践指南。

Conclusion: 该研究建立了医学图像超分辨率的领域特定见解，为临床影像工作流中的模型选择提供实践指南，为未来医学图像超分辨率研究和部署提供标准化评估框架。
...
TL;DR: 本文研究视频视觉语言模型(VLM)中的训练无关反重构方法，通过在验证状态存活时重用视觉状态、在需要新证据时才获取新帧，来减少冗余的视觉信息处理开销。实验表明该方法在Qwen2.5-VL-7B-Instruct-4bit上可将后续查询延迟降低14.90-35.92倍。


<details>
  <summary>Details</summary>
Motivation: 当前的VLM流程即使在视觉状态已经稳定的情况下，仍然重复处理密集的RGB帧或重新获取新的视觉前缀，造成计算资源的浪费。作者认为这种冗余处理是可以避免的。

Method: 提出两种训练无关的优化策略：1) 自适应同视频后续重用：当验证表明视觉状态存活时复用已有状态，需要新证据时才获取新帧；2) 新视频修剪：在生成答案前跳过视觉塔的时序工作。使用VideoMME基准和多种模型(Qwen2.5-VL-7B-Instruct-4bit、Gemma 4-E4B-4bit)进行测试。

Result: 后续查询重用策略在93查询的VideoMME breadth设置中使后续查询延迟降低14.90-35.92倍，同时保持配对选择和正确率。新视频修剪在Gemma模型上实现1.316x的首查询加速，无配对漂移或解析失败。但策略存在限制：50轮重复问题保持效果，但密集答案锚定的提示变化会将保守固定策略与快速激进策略区分开来。

Conclusion: 组件加速仅在与端到端加速成比例时才有效，建议VLM应原生支持暴露变化、运动、不确定性、对象状态、传感器时间和主动瓦片等媒体，使模型无需每帧都从密集RGB重新发现世界。
...
TL;DR: 首次评估多模态大语言模型在癫痫视频中自动识别病理运动的能力_zero-shot性能优于微调CNN和ViT模型


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在日常人类活动识别方面表现出色,但在临床神经障碍的病理运动分析方面尚未探索

Method: 在90个临床癫痫 recordings上评估顶级MLLMs对20个ILAE定义的癫痫症状特征的zero-shot表现,并使用面部裁剪、姿态估计、音频降噪等特征 targeted信号增强进行预处理优化

Result: MLLMs在18个特征中的13个上优于微调的CNN和ViT基线模型,在识别显著姿态和上下文特征方面表现特别强,但在细微高频运动方面表现不佳。特征增强改善了20个特征中10个的表现。94.3%的MLLM生成解释达到至少60%的忠实度评分

Conclusion: 通过针对性的预处理策略,通用MLLMs可以适应专业临床视频分析,为可解释、高效的诊断辅助提供了途径
...
TL;DR: 提出一个五阶段解剖引导的初始化流水线，将正畸临床工作流程转化为计算操作，为下游HRNet-W32检测器产生置信度加权空间注意力先验，在1502张X光片上实现1.04mm平均径向误差，超越之前最先进水平15.4%。


<details>
  <summary>Details</summary>
Motivation: 正畸医师在追踪头影测量X光片时有结构化工作流程（识别软组织轮廓、分分区、追踪轮廓、使用几何定义定位标志点），但现有自动化系统无法复制这种临床推理过程。

Method: 设计五阶段解剖引导初始化管道：1)识别软组织轮廓 2)分区颅骨解剖区域 3)追踪轮廓 4)使用几何定义定位标志点 5)生成置信度加权空间注意力先验，传入下游HRNet-W32检测器。

Result: 在1502张来自3个来源跨越7+成像设备的X光片上实现25个标志点1.04mm平均径向误差，比之前SOTA(1.23mm@19标志点)提升15.4%，12个标志点低于1mm。三路对照消融实验显示：去除解剖先验模型在验证集收敛到~1.03mm但在测试集发散到1.94mm；用随机位置高斯替换解剖先验产生更差泛化(2.24mm)。

Conclusion: 临床领域知识编码的空间先验提供了一种归纳偏差，这是架构和数据增强单独无法提供的。该方法成功将正畸医师的专业推理转化为可计算的自动化系统。
...
TL;DR: Mix3R是一种将前馈重建与生成式3D重建相结合的新框架，通过Mixture-of-Transformers架构实现稀疏体素、点地图和相机参数的联合生成，并利用重叠注意力偏置实现无监督的纹理放置。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏视角3D重建方法存在两个问题：前馈重建无法生成完整几何形状，而生成式方法缺乏输入对齐。现有方法难以同时实现良好的2D-3D对齐和完整几何生成。

Method: Mix3R采用两阶段生成：(1)稀疏体素生成阶段，使用Mixture-of-Transformers架构联合生成稀疏体素、点地图和相机参数，保持预训练先验的同时实现对齐；(2)纹理几何生成阶段，基于重叠注意力偏置将纹理无监督地放置到生成的几何形状上。

Result: Mix3R方法实现了比纯生成方法更好的输入对齐，同时产生比前馈方法更准确的相机姿态估计。

Conclusion: Mix3R通过将前馈重建和3D生成相结合，实现了互惠互利：前馈分支学习基于生成式3D先验进行预测，3D生成分支则受前馈分支的几何信息特征条件化，最终在输入对齐和相机估计方面都取得了性能提升。
...
TL;DR: 针对长尾分布下的类增量学习问题，提出梯度一致性正则化和自适应蒸馏损失权重调整两项技术，有效缓解灾难性遗忘，在多个基准数据集上实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 长尾类增量学习（LT-CIL）处理从类别不平衡分布数据集中顺序学习新类别的任务，面临灾难性遗忘、少数类欠学习和多数类过拟合的三重挑战。

Method: 提出两种技术：1) 梯度一致性正则化，利用梯度移动平均抑制训练波动，稳定学习过程；2) 通过归一化熵动态测量类别不平衡程度，自适应调整蒸馏损失权重，在保留旧知识和学习新知识之间建立最优平衡。

Result: 在CIFAR-100-LT、ImageNetSubset-LT和Food101-LT基准数据集上实现最高5.0%的准确率提升；在"In-ordered"设置（任务从多数类进展到少数类）中获得显著提升。

Conclusion: 该方法在不显著增加计算开销的情况下，有效缓解了长尾分布下的灾难性遗忘问题，特别是在"In-ordered"设置（从多数类到少数类）展现出显著优势，证明了框架的实用性。
...
TL;DR: 本文提出一种双基础模型的无监督域适应（UDA）框架，用于语义分割。利用SAM（Segment Anything Model）的超像素引导提示学习更广泛的目标域像素，结合DINOv3构建稳定的类别原型。在GTA-to-Cityscapes和SYNTHIA-to-Cityscapes数据集上分别提升1.3%和1.4%的mIoU。


<details>
  <summary>Details</summary>
Motivation: 现有UDA方法存在两大局限：(1)依赖高置信度伪标签，限制了对目标域的学习范围；(2)基于源域模型初始化的类别原型存在偏差且不稳定。因此需要新的方法来扩展学习范围并构建更稳健的原型。

Method: 提出双基础模型UDA框架：1) 使用SAM的超像素引导提示机制，使模型能够从更广泛的目标域像素中学习；2) 引入DINOv3构建稳定、域不变的类别原型，利用其强大的表示学习能力。

Result: 在GTA-to-Cityscapes和SYNTHIA-to-Cityscapes两大基准上分别实现+1.3%和+1.4%的mIoU提升，超越现有强UDA基线方法。

Conclusion: 通过结合SAM和DINOv3两种互补的基础模型，成功解决了UDA中伪标签限制和学习不稳定的问题，为语义分割的域适应提供了新的有效范式。
...
TL;DR: 本文提出SoDa²方法，解决跨场景高光谱图像开放集分类中的域偏移和计算成本问题，采用解耦对齐和单阶段双分支框架，在三个数据集上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 跨场景高光谱图像(HSI)分类是遥感中的基础研究课题，但面临两个关键挑战：(1)混合光谱-空间特征直接对齐导致的域偏移问题；(2)两阶段训练策略导致的高计算成本。

Method: 提出单阶段开放集域适应方法SoDa²，包含：贡献感知双模态特征提取模块解耦光谱序列信号和空间细节特征；解耦对齐模块独立减少源域和目标域之间的光谱和空间差异；成本有效的单阶段双分支框架学习MMD约束对齐特征和无约束内在特征；使用高斯混合模型对两种特征类型之间的平方余弦相似度分布进行建模。

Result: 在三个HSI数据集组上进行广泛实验，SoDa²优于最先进的方法，为开放集跨场景任务实现了卓越的分类精度和模型迁移性。

Conclusion: SoDa²通过单阶段框架和解耦对齐策略有效解决了域偏移和高计算成本问题，在跨场景HSI分类中表现出色。
...
TL;DR: 本文提出了一种训练无关的双系统（TFDS）框架，用于增强自监督说话头像伪造检测器的判别能力。该方法受人类认知双系统理论启发，将异常分数作为系统1的基础，通过轻量级阈值路由将样本分为置信集和不确定集，然后对不确定样本进行细粒度的证据引导推理来细化排序。


<details>
  <summary>Details</summary>
Motivation: 现有自监督检测器在跨生成器鲁棒性方面存在不足，特别是基于分数的检测器在困难样本上的判别能力有限，异常排序不可靠，需要进一步挖掘潜在判别能力。

Method: 提出训练无关双系统框架：系统1基于异常分数进行阈值路由，将样本划分为置信集和不确定集；系统2仅对不确定集进行细粒度证据引导推理，重新排序模糊样本。

Result: 在多个数据集和扰动设置下实现一致性改进，收益主要来自对不确定集内排序的修正。

Conclusion: 现有的自监督说话头像伪造检测器仍包含未被充分利用的判别线索，可以通过训练无关的双系统推理有效释放。
...
TL;DR: 针对视频时序定位(VTG)中跨模态语义差距问题,提出MASRA框架,利用多模态大语言模型(MLLM)在训练时生成事件级描述和片段级字幕两种文本先验,实现事件语义时序对齐(ESTA)和局部关系一致性对齐(LRCA),增强时序语义的区分度和一致性。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位面临跨模态语义差距问题,导致背景特征与查询错误对齐,而直接匹配查询与时刻导致时序语义区分度和一致性不足。

Method: MASRA框架包含:1)事件语义时序对齐(ESTA):将时序上下文与事件语义对齐,增强语义与时序事件的对应关系;2)局部关系一致性对齐(LRCA):从片段级字幕构建文本关系矩阵,与时序特征相似度矩阵对齐;3)语义引导增强和二阶关系注意力两个支持模块;4)解耦对齐交互(DAI)配合上下文感知码本自适应吸收查询无关语义。

Result: 实验表明MASRA优于现有方法,消融实验验证了其有效性。

Conclusion: 通过MLLM在训练时生成文本先验并实施双重对齐策略,可有效弥合跨模态语义差距,提升视频时序定位的准确性和鲁棒性,且推理时无需MLLM。
...
TL;DR: GRPO-TTA是一种将组相对策略优化应用于视觉语言模型测试时间适应的方法，通过构建输出组和设计针对TTA的奖励函数来实现无需ground-truth标签的视觉编码器调优。


<details>
  <summary>Details</summary>
Motivation: GRPO在大型语言模型和视觉语言模型的后训练中表现出色，但不清楚它是否能显著促进视觉语言模型的测试时间适应(TTA)。

Method: 1. 将类特定提示预测重新表述为群体策略优化问题；2. 通过从CLIP相似性分布中采样前K个类别候选者来构建输出组，实现无ground-truth标签的概率驱动优化；3. 设计针对测试时间适应的奖励函数，包括对齐奖励和分散奖励。

Result: 在各种基准测试中始终优于现有的测试时间适应方法，在自然分布转移下表现出显著更大的性能提升。

Conclusion: GRPO可以有效应用于视觉语言模型的测试时间适应，为TTA提供了一种新的优化范式。
...
TL;DR: 本文提出了一种新颖的框架，使用MK-ResRecon和IdentityRefineNet3D两个模型从稀疏采样的2D切片重建高保真3D MRI体积，仅需12.5%的轴向切片即可完成全分辨率3D重建。


<details>
  <summary>Details</summary>
Motivation: MRI采集耗时且会给患者带来不适，长时间扫描会增加运动伪影的风险，导致图像质量下降并需要重复扫描。

Method: MK-ResConc使用多核纹理感知损失预测缺失的中间2D切片，保留精细解剖细节；IdentityRefineNet3D将预测切片和原始稀疏切片作为单一3D体积进行细化，获得平滑解剖结构。模型在大型T1序列对比后脑MRI数据集上训练，并在大型异构脑MRI队列中进行评估。

Result: 该框架能够从高度稀疏输入实现准确、无幻觉、可推广且经过临床验证的3D MRI重建，为更快、更患者友好的MRI成像提供了临床可行的途径。

Conclusion: 提出的框架成功实现了从仅12.5%轴向切片的稀疏采样重建高保真3D MRI体积，具有准确、无幻觉、可推广的特性，已过临床验证。
...
TL;DR: 本文提出一种基于表面的3D点云异常检测方法，通过学习判别性符号距离函数来区分正常与异常点。该方法包含三个核心模块：噪声点生成(NPG)模块生成不同类型噪声以暴露异常点；多尺度细节特征(MLF)模块捕捉细粒度局部和粗粒度全局信息；隐式表面判别(ISD)模块利用多尺度特征学习隐式表面表示。在Anomaly-ShapeNet和Real3D-AD数据集上分别达到92.1%和85.9%的对象级AUROC，较最优方法提升2.1%和3.6%。


<details>
  <summary>Details</summary>
Motivation: 3D点云异常检测面临学习精确点级表示的挑战，主要原因是点云数据规模大且稀疏。现有基于组或基于点的方法难以有效处理此问题。

Method: 提出基于表面的方法，包含三个创新模块：1)NPG模块生成不同类型噪声，帮助学习判别性特征；2)MLF模块从点云中提取多尺度信息，同时包含细粒度局部和粗粒度全局特征；3)ISD模块利用多尺度特征学习隐式表面表示，通过训练符号距离函数区分正常与异常点。

Result: 在Anomaly-ShapeNet数据集上达到92.1% AUROC，在Real3D-AD数据集上达到85.9% AUROC，分别较当前最优方法提升2.1%和3.6%。

Conclusion: 所提方法通过表面表示和符号距离函数有效解决了3D点云异常检测的难题，在两个基准数据集上均取得最先进的性能表现。
...
TL;DR: MantIs是首个针对3D点云基础模型的Mamba原生参数高效微调框架，通过状态感知适配器在状态空间层面实现轻量级任务适应，同时使用双序列化一致性蒸馏减少序列化导致的不稳定性，仅用约5%的可训练参数即达到竞争性能。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法主要针对Transformer骨干设计，存在token-level适配与state-level序列动态的粒度不匹配问题，导致直接迁移到冻结的Mamba骨干时准确率显著下降且优化不稳定。

Method: 提出状态感知适配器(SAA)向选择性状态空间更新中注入轻量级任务条件控制信号，实现状态级适配；同时引入双序列化一致性蒸馏(DSCD)正则化不同点云序列化。

Result: 在多个基准数据集上仅用约5%可训练参数达到竞争性能。

Conclusion: 这是首个针对3D PFMs的Mamba原生PEFT框架，解决了粒度不匹配和序列化不稳定问题。
...
TL;DR: 开放世界目标检测论文，提出VL-SAM-v3框架，通过检索外部视觉记忆增强检测性能，特别在稀有类别上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界检测方法依赖粗粒度文本语义，对细粒度外观变化、稀有类别和杂乱场景提供视觉证据不足。

Method: 提出VL-SAM-v3统一框架：1)从非参数记忆库检索视觉原型；2)转换为两类视觉先验(稀疏先验用于空间锚定，密集先验用于局部上下文)；3)通过记忆引导提示细化与检测提示集成

Result: 在LVIS上零样本实验表明：开放词汇和开放端点推理下检测性能持续提升，稀有类别增益尤为显著；SAM3验证了检索-细化机制通用性

Conclusion: VL-SAM-v3通过视觉记忆和提示细化机制有效增强开放世界检测，为开放词汇和开放端点推理提供了统一解决方案
...
TL;DR: FSTM是一种统一的神经表面重建方法，通过两步过程学习几何和语义：首先是使用RGB输入和几何线索进行几何预热，然后进行语义场估计。实验表明，该方法在Replica上训练速度快2.3倍，在ScanNet++上对真实世界缺陷更具鲁棒性，并实现了更高的目标召回率。


<details>
  <summary>Details</summary>
Motivation: 现有神经表面重建方法虽然能成功集成语义标签，但继承了多SDF学习的慢训练时间和有限可扩展性问题，需要一种既能保持高效训练又能获得良好几何和语义重建效果的统一方法。

Method: 提出两阶段方法：1)使用RGB输入和几何线索进行几何预热优化；2)进行语义场估计。不依赖专门的模块或复杂的多SDF设计，通过先优化几何而不进行语义监督来实现改进。

Result: 在合成和真实室内数据集上的实验表明：1)在Replica数据集上训练速度快2.3倍；2)在ScanNet++上对真实世界缺陷更加鲁棒；3)通过恢复场景中更多物体的表面，实现了更高的召回率。

Conclusion: FSTM证明，简洁的 formulation（ formulations：公式化表示）足以实现强大的几何和语义重建效果，优于现有的多SDF方法。
...
TL;DR: 本文提出MHPR，一个全面的人本中心场景感知推理联合评估基准，包含多层次数据设计和自动化标注生成pipeline。基准涵盖个体、多人和人物交互维度，评估细粒度属性和高级语义。实验表明格式化SFT数据和RL数据能显著提升模型性能，使用MHPR训练的7B模型能达到与更大模型接近的效果。


<details>
  <summary>Details</summary>
Motivation: 当前LVLM基准测试主要关注单一任务设置，缺乏细粒度、以人为中心的评估。现实应用如电影分析和虚拟数字人类需要多维度的人类理解能力。

Method: 构建了多层次数据设计(C-RD、SFT-D、RL-D、T-D)，提出自动化caption/VQA生成pipeline(ACVG)实现类别属性分解、属性重写和多模型投票。在细粒度属性(外观、服装、姿态、部件)和高级语义(社会关系、动作语义、空间关系、意图功能性)两个层面对vision-language模型进行评估。

Result: 1) 格式化对齐的SFT数据显著提升指令跟随和稳定性；2) 基于bad-case分析的challenge-focused RL数据进一步增强困难实例上的感知推理；3) 使用MHPR训练的Qwen2.5-VL-7B取得与更大模型接近的性能。

Conclusion: MHPR benchmark and ACVG pipeline能够实现可扩展、高质量的标注生成，为人本中心感知与推理任务提供全面的评估框架。实验表明格式化对齐的SFT数据和基于bad-case分析的RL数据能够显著提升模型的指令跟随能力、感知推理性能。使用MHPR训练的Qwen2.5-VL-7B模型能够取得接近更大规模模型的性能表现。
...
TL;DR: BFORE是一种结合蝴蝶优化算法和萤火虫算法的混合元启发式优化框架，用于自动调优多阶段Retinex管道参数，实现低光照图像增强，在LOL数据集上PSNR达到17.22 dB，优于传统方法和深度学习基线RetinexNet。


<details>
  <summary>Details</summary>
Motivation: 现有的基于Retinex的方法依赖手动调参，无法在多样化的光照条件下推广。需要一种能够自动调参的混合元启发式优化框架来提升低光照图像增强的效果和泛化能力。

Method: 该方法将输入图像转换为HSV颜色空间，对亮度通道应用带权重分布的自适应伽马校正(AGCWD)，然后进行自适应去噪。蝴蝶优化算法(BOA)优化多尺度Retinex色彩恢复(MSRCR)参数，而萤火虫算法(FA)优化AGCWD和去噪参数。混合BOA-FA切换策略动态平衡全局探索和局部开发。

Result: BFORE在LOL基准数据集上达到最高PSNR (17.22 dB)，比直方图均衡化提高20.3%，比MSRCR提高17.5%。平均亮度(129.97)最接近理想中调值。在PSNR和SSIM上均优于深度学习基线RetinexNet，且无需训练数据。混合BOA-FA优化相比未优化管道提供12.3%的PSNR提升和14.8%的SSIM提升。

Conclusion: BFORE是一种有效的低光照图像增强方法，无需训练数据即可实现先进的性能。混合BOA-FA优化策略为传统Retinex管道提供了12.3%的PSNR提升和14.8%的SSIM提升，证明了元启发式优化在图像增强任务中的潜力。
...
TL;DR: DALPHIN是首个用于评估病理学AI助手的多中心开放基准测试，包含1236张图像、300个病例、130种诊断、来自6个国家的14个亚专科。评估了GPT-5、 Gemini 2.5 Pro和PathChat+三个模型，PathChat在6个任务中4个达到专家水平。


<details>
  <summary>Details</summary>
Motivation: 数字病理学中的基础模型和视觉问答技术正在兴起，但缺乏独立的基准测试来评估其在常规诊断中协助病理学家的潜力。

Method: 创建DALPHIN基准数据集，包含1236张图像和130种诊断；招募31名来自10个国家的病理学家建立人类性能基准；评估两个通用模型（GPT-5、 Gemini 2.5 Pro）和一个病理学专用模型（PathChat+）在顺序和独立答案生成任务上的表现。

Result: PathChat在6个任务中4个达到专家水平，Gemini 2.5 Pro在2个任务中达到，GPT-5在1个任务中达到，无统计显著差异。DALPHIN数据集和评估平台已公开释放。

Conclusion: 研究表明病理学专用AI助手（PathChat）性能优于通用模型，部分任务可达到专家病理学家水平，为AI辅助病理诊断提供了有价值的基准和方向。
...
TL;DR: PriorNet是一个先验引导的框架,用于从面部视频估计参与度,通过在预处理、模型适配和目标设计三个阶段注入任务相关先验来应对面部证据不完整、标注数据有限和 annotation 主观性等挑战。


<details>
  <summary>Details</summary>
Motivation: 从面部视频估计参与度具有挑战性,因为面部证据通常不完整,标记数据有限,且参与度注释是主观的。现有方法难以有效处理这些限制。

Method: PriorNet在三个阶段注入先验:1)预处理阶段将人脸检测失败转换为明确的零帧占位符,使缺失人脸事件在输入序列中得到表示;2)模型适配阶段通过先验引导的低秩适配模块(Prior-LoRA)调整冻结的自监督视频面部情感感知器(SVFAP)主干网；3)目标设计阶段使用Dirichlet证据不确定性加权目标进行硬标签监督训练。

Result: 在EngageNet、DAiSEE、DREAMS和PAFE数据集上使用各数据集的原生评估协议进行评估,PriorNet在每个数据集上都比最强的先验参考方法有提升。对EngageNet和DAiSEE的组件消融实验表明,性能提升来自于预处理、适配和目标层面先验的互补贡献。

Conclusion: 结果支持显式先验注入是face-video参与度估计的一个有用设计原则,适用于本研究中基准条件下的任务。
...
TL;DR: 开放词汇目标检测旨在识别开放类别集的目标，利用在大规模图像文本数据上预训练的视觉语言模型（VLM）。协同范式将目标检测器与VLM结合实现Novel目标的零样本识别，但全图像预训练的VLM往往难以捕捉局部目标细节，限制了其对区域级检测的有效性。提出解耦自适应训练（DAT），一种自监督微调方法，通过构建区域感知的伪标签数据集并以解耦方式微调VLM的视觉骨干，增强局部特征对齐同时通过权重插值保留全局语义知识。DAT是一个即插即用模块，无需推理开销，仅微调不到0.8M参数。在COCO和LVIS数据集上的实验表明，DAT一致地提升了Novel类别和已知类别的检测性能，建立了协同开放词汇检测的新SOTA。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测旨在识别开放类别集的目标，利用在大规模图像文本数据上预训练的视觉语言模型（VLM）。协同范式将目标检测器与VLM结合实现Novel目标的零样本识别，但全图像预训练的VLM往往难以捕捉局部目标细节，限制了其对区域级检测的有效性。

Method: 提出解耦自适应训练（DAT），一种自监督微调方法。首先使用预训练的闭集目标检测器构建区域感知的伪标签数据集，其中可能存在对应于Novel目标但未标注或错误标注的区域。然后以解耦方式微调VLM的视觉骨干，通过权重插值在增强局部特征对齐的同时保留全局语义知识。DAT是一个即插即用模块，无需推理开销，仅微调不到0.8M参数。

Result: 在COCO和LVIS数据集上的实验表明，DAT一致地提升了Novel类别和已知类别的检测性能。

Conclusion: DAT建立了协同开放词汇检测的新SOTA（最先进水平），有效提升了VLM在协同模型目标检测中的局部特征捕捉能力。
...
TL;DR: 本文提出Grounded Correspondence框架，用确定性二分匹配取代视频目标中心学习中的学习动态模块，实现零参数时间建模，在多个数据集上取得有竞争力性能。


<details>
  <summary>Details</summary>
Motivation: 传统视频目标中心学习方法依赖学习到的动态模块预测未来目标表示（slots）来保持时间一致性，但这些预测器实际上是对离散对应问题的昂贵近似。现代自监督视觉backbone已能编码实例判别特征，可靠区分不同物体。

Method: 提出Grounded Correspondence框架：(1)Slots从frozen backbone特征中的显著区域初始化，无需学习；(2)通过Hungarian匹配在帧间维护目标身份，实现确定性二分匹配；(3)整个时间建模过程需要零可学习参数。

Result: 在MOVi-D、MOVi-E和YouTube-VIS数据集上实现competitive performance，证明了去除学习动态模块的可行性。

Conclusion: 利用预训练backbone的实例判别特征可以 Eliminating the need for learned temporal prediction，实现高效的视频目标学习，为该领域提供了新思路。
...
TL;DR: AniMatrix是一个针对动漫而非物理真实性的视频生成模型,通过双通道条件机制和三步转换(重新定义正确性、覆盖物理先验、区分艺术与失败)来解决物理偏见模型无法处理动漫风格多样性问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型内置物理真实主义先验,而动漫故意违反物理规则(涂抹帧、冲击帧、Q版转换等),且其数千种艺术惯例没有单一的"动漫物理"可以让模型学习,因此物理偏见模型要么抹平定义该媒介的艺术性,要么在风格差异下崩溃。

Method: 1)双通道条件机制:Production Knowledge System将动漫编码为可控生产变量结构化分类学(Style/Motion/Camera/VFX),AniCaption从像素推断这些变量为导演指令;可训练标签编码器保留字段值结构,冻结T5编码器处理自由形式叙述;双路径注入(交叉注意力细粒度控制,AdaLN调制全局增强)确保分类指令不被稀释。2)样式-运动-变形课程将模型从近物理运动过渡到完整动漫表现力。3)变形感知偏好优化使用领域特定奖励模型区分有意艺术与病态崩溃。

Result: 在专业动画师评分的动漫特定人工评估中,AniMatrix在五个维度中排名第一四个,较Seedance-Pro 1.0最大增益:提示理解(+0.70,+22.4%)和艺术运动(+0.55,+16.9%)。

Conclusion: AniMatrix通过针对艺术正确性而非物理正确性的设计,成功解决了动漫视频生成的核心挑战,为动漫创作提供了有效的AI工具,并将公开释放模型权重和推理代码。
...
TL;DR: OneTrackerV2是一个统一的端到端多模态视觉目标跟踪框架，通过Meta Merger和Dual MoE技术创新，实现了任意模态的高效跟踪，在多个基准数据集上取得最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常为每种模态训练单独模型或依赖预训练模型适应新模态，限制了效率、可扩展性和可用性。需要一个统一的端到端多模态跟踪框架来解决这些问题。

Method: 提出统一多模态跟踪框架OneTrackerV2，包含三个核心创新：1) Meta Merger将多模态信息嵌入统一空间，实现灵活模态融合；2) Dual Mixture-of-Experts (DMoE)包含T-MoE建模时空关系和M-MoE嵌入多模态知识，解耦跨模态依赖并减少特征冲突；3) 采用共享架构、统一参数和单次端到端训练实现任意模态的跟踪。

Result: OneTrackerV2在5个RGB和RGB+X跟踪任务和12个基准数据集上达到最先进性能，保持高推理效率，模型压缩后仍保持较强性能，在模态缺失场景下表现出显著鲁棒性。

Conclusion: OneTrackerV2通过统一的端到端训练框架，在5个RGB和RGB+X跟踪任务及12个基准数据集上实现了最先进的性能，同时保持了高推理效率。该方法在模型压缩后仍能保持较强性能，并在模态缺失场景下表现出显著的鲁棒性。
...
TL;DR: 本文提出FluxFlow框架，利用保守像素空间流匹配实现地基到地基的天文超分辨率重建，通过引入观测不确定性和源区重要性权重进行训练，并采用无需训练的Wiener正则化测试时校正来抑制幻觉源。同时构建了包含19,500对真实配对图像的DESI-HST数据集作为基准。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用合成训练对无法捕捉真实大气统计特性，导致重建结果要么过度平滑，要么产生无物理对应源的幻觉伪影。地基地观测同时受限于像素采样分辨率和大气视宁度，后者引入的随机空间变化PSF无法通过简单上采样解决。

Method: 提出FluxFlow保守像素空间流匹配框架：(1)训练时融入观测不确定性信息和源区重要性权重；(2)测试时采用无需额外训练的Wiener正则化校正来抑制幻觉源同时保留恢复的细节。构建了DESI-HST数据集，包含19,500对真实共注册的地基-地基图像对，具有真实大气PSF变化。

Result: 实验表明FluxFlow在测光精度和科学精度上均一致优于现有基线方法。

Conclusion: FluxFlow通过保守的流匹配框架和Wiener正则化校正，有效解决了天文超分辨率中的过度平滑和幻觉源问题，DESI-HST数据集为该领域提供了首个大规模真实世界基准。
...
TL;DR: 本文提出ReMem基准，用于评估大型视觉语言模型(LVLMs)的记忆和遗忘行为，解决现有基准中模型未能有效记忆目标信息的根本问题。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs隐私保护基准忽视了一个关键问题：模型在第一阶段未能有效记忆目标信息，导致后续遗忘评估不可靠。研究识别出"记忆不足"和"多跳诅咒"是根本原因。

Method: 提出ReMem基准，包含三个核心设计：1)原则性数据缩放确保稳健基础学习；2)推理感知QA对；3)多样化视觉上下文。同时提出新的"暴露度"指标来量化信息从模型内部概率分布中的消除深度。

Result: 广泛实验表明ReMem能够提供严格且可信的框架，用于诊断LVLMs的学习和遗忘行为。

Conclusion: ReMem是一个可靠的多跳多图像记忆基准，为评估LVLMs隐私保护中的记忆和遗忘提供了严格可信的诊断框架。
...
TL;DR: 本文提出GeoTopoDiff，一个基于图扩散的框架，用于从稀疏CT切片重建3D多孔微结构。传统扩散模型需要完整CT扫描才能提供拓扑先验，导致吞吐量、拓扑保真度和视场之间的权衡。该方法创新性地将扩散先验学习从体素空间转移到混合图状态空间，同时建模连续孔隙几何和离散孔隙-喉道拓扑。实验表明该方法在PTFE和砂岩数据集上显著降低了形态误差和传输误差。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在重建大规模3D多孔微结构时面临挑战：同时建模连续孔隙形态和离散孔隙-喉道拓扑需要完整CT扫描提供拓扑先验，这导致工业应用中吞吐量、拓扑保真度和视场之间的固有权衡。

Method: 将扩散先验学习从基于体素的空间转移到混合图状态空间，同时包含连续孔隙几何和离散孔隙-喉道拓扑；引入基于稀疏CT切片的拓扑感知部分图先验来约束反向去噪过程。

Result: 在各向异性PTFE和Fontainebleau砂岩上的实验表明，GeoTopoDiff将形态相关误差减少19.8%，拓扑敏感传输误差平均减少36.5%。混合图状态空间促进了扩散去噪过程，在稀疏观测下减少后验不确定性。

Conclusion: 该研究证明了混合图状态空间能够有效处理3D多孔微结构重建中的稀疏观测问题，为工业CT扫描提供了一种高效、低误差的重建方案。
...
TL;DR: 针对叶片分割在精准农业中的研究空白，作者调研了四个公开叶片分割数据集，比较了一阶段、二阶段和Transformer检测器，发现YOLO26配置最适合实际应用。通过跨域泛化实验发现模型在不同植物物种和拍摄条件间存在显著性能下降，并创建了包含23个植物物种的新基准数据集。研究表明在多样数据集上训练的模型平均mAP50-95达83.9%，证明多样化叶片分割数据集对精准农业的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着全球粮食需求增长和气候压力增加，可持续精准农业实践变得日益重要。自动化、个性化植物处理需要细粒度视觉分析，但叶片级分割尽管对评估作物健康、生长动态、产量潜力和局部压力症状具有重要价值，却仍缺乏深入探索。现有研究受限的主要原因是缺乏专用数据集（尤其是物种覆盖方面）以及缺少对现代实例分割架构的系统评估。

Method: 1) 调研现有数据并识别四个适用的公开叶片分割数据集；2) 使用这些数据集比较一阶段、二阶段和Transformer检测器，筛选出最佳模型配置；3) 进行广泛的跨域泛化实验评估模型在不同植物物种和拍摄设置下的表现；4) 通过半自动标注方法创建新的基准数据集，包含23个植物物种的叶片级掩码。

Result: 1) YOLO26模型配置为实时精准农业任务提供最佳精度-速度权衡；2) 跨域泛化实验显示性能在不同植物物种和记录设置间显著下降，尤其是仅在实验室数据上训练的模型；3) 在四个现有数据集上训练的模型在其对应测试集上达到83.9%的平均mAP50-95，在新基准数据集上达到40.2%；4) 研究表明多样化的叶片分割数据集有助于提升模型泛化能力。

Conclusion: 在所有四个现有数据集上训练的模型展现出改进的泛化性能，证明了多样化叶片分割数据集对构建稳健精准农业系统的重要性。YOLO26等现代实例分割架构能够有效处理叶片分割任务，但跨域性能下降问题仍需通过构建更具多样性的数据集来解决。研究创建的23植物物种新基准数据集为未来研究提供了重要的数据支持。
...
TL;DR: 提出一种结合迁移学习和基于再生核希尔伯特空间的最大均值差异（RKHS-MMD）损失的无监督域适应框架，用于医学图像分类，显著提升跨域泛化性能


<details>
  <summary>Details</summary>
Motivation: 医学图像标注依赖领域专家知识，不同医学中心和成像设备导致的域偏移和模态差异限制了模型的泛化能力

Method: 结合迁移学习与RKHS-MMD损失进行源域和目标域对齐，联合优化分类损失和RKHS-MMD损失实现无监督域适应

Result: 在两个来自不同医学中心的胸部X光数据集上取得显著提升，优于标准MMD方法

Conclusion: RKHS-MMD方法能有效减小模态差距，提升医学图像分类性能，对AI驱动的先进医学诊断具有重要价值
...
TL;DR: 本文提出CoVQD引导的RAG框架，通过将思维链推理与视觉问题分解相结合，增强多模态大型语言模型在开放域视觉问答中的外部知识获取和推理能力。


<details>
  <summary>Details</summary>
Motivation: 视觉问答日益采用多模态大型语言模型，但在开放域设置中外部知识的准确检索和整合仍需改进。现有方法缺乏将结构化推理与知识获取有效融合的策略。

Method: 提出CoVQD逻辑提示策略，将思维链(CoT)推理与视觉问题分解(VQD)融合，引导检索过程获得更准确的相关知识。在此基础上提出CgRAG框架，使MLLMs能够访问更全面连贯的外部知识，同时受益于结构化的视觉-文本推理指导。

Result: 在E-VQA、InfoSeek和OKVQA基准测试上的广泛实验表明，该方法在复杂的跨域VQA场景中提高了泛化能力和可靠性。

Conclusion: 通过CoVQD引导的RAG框架成功增强了检索基VQA的多模态大型语言模型性能，验证了结构化推理与知识获取融合策略的有效性。
...
TL;DR: 提出了一种统一的多模态学习框架CPSC，利用共形预测实现自引导校准，解决低质量数据带来的模态不平衡和噪声损坏问题。


<details>
  <summary>Details</summary>
Motivation: 多模态学习中低质量数据问题（模态不平衡和噪声损坏）通常被分开研究，但它们有共同的根源——对单个模态和实例可靠性的预测不确定性。

Method: 提出CPSC框架，包含两个核心模块：(1)表示自校准——将单模态特征分解为分量，通过共形预测器识别并选择性融合最鲁棒的特征；(2)梯度自校准——基于实例级可靠性分数重新校准反向传播梯度流，引导优化朝向更可信的方向。同时设计了共形预测器的自更新策略。

Result: 在6个基准数据集上，在imbalanced和noisy设置下均优于现有最先进方法。

Conclusion: 所提CPSC框架能有效解决多模态学习中的低质量数据问题，通过自校准机制提升模型在不可靠数据环境下的性能。
...
TL;DR: IMPOSE是一个物理启发的无接触指纹多姿态生成框架，通过三个阶段（rolled指纹身份生成、跨模态转换、基于物理的多姿态模拟）合成保持身份一致性的无接触指纹样本，用于增强跨模态指纹匹配的识别性能。


<details>
  <summary>Details</summary>
Motivation: 无接触指纹识别存在由自由手指姿态引起的严重非线性几何畸变，导致无接触与传统接触式指纹之间存在显著的跨模态域差距。现有方法主要依赖显式几何校正或图像增强，在极端姿态变化下表现脆弱。

Method: 提出IMPOSE框架包含三个阶段：(1)使用离散码本表示的潜在扩散生成rolled指纹身份；(2)基于Sauvola局部自适应二值化作为身份锚引导的跨模态转换；(3)通过3D手指模型纹理映射和投影进行物理基础的多姿态模拟。生成样本在脊线拓扑层面保持严格身份一致性，并与标准指纹坐标空间对齐。

Result: 在UWA和PolyU CL2CB数据库上的广泛实验表明，使用IMPOSE合成数据微调固定长度密集描述符(FDD)实现了最先进的跨模态匹配，将EER分别降低至8.74%和2.26%。合成数据在DeepPrint和AFRNet等主流表示方法上也获得了一致性增益，混合策略结合合成和真实数据获得了最佳整体结果。

Conclusion: IMPOSE框架能够有效合成身份保持的无接触指纹多姿态样本，显著提升跨模态指纹识别的性能，为解决无接触指纹中的姿态畸变问题提供了新的研究方向。
...
TL;DR: 研究提出在散射介质中恢复场景颜色的充分条件，证明对于理想超光谱相机，图像中的交叉像素关系（恢复模式）可以将解限定为唯一候选，使问题从不适定转为适定。


<details>
  <summary>Details</summary>
Motivation: 在散射介质中恢复场景颜色是一个基本的光学成像逆问题，但本质上是不适定的，因为多个解可以解释相同的观测，且没有理解候选解空间就无法控制预测误差。

Method: 观察到不适定性源于（1）光谱信号投影到像素强度上，（2）介质参数未知，研究识别了图像中自然出现的交叉像素关系——恢复模式，并证明对于理想超光谱相机，这些恢复模式可以限制解为唯一候选。

Result: 提出了使颜色恢复成为适定问题的充分条件，仅靠传感器改进无法解决介质引起的失真，需要额外的约束条件（如恢复模式）。

Conclusion: 为基于第一原理的新一类视觉算法开辟了道路，能够在散射环境中进行定量分析图像。
...
TL;DR: 本文探讨了多视角熟练度估计任务，介绍了Ego-Exo4D数据集上的三项最新工作：SkillFormer（参数高效的选择性多视角融合架构）、PATS（保留局部密集运动片段的时序采样方法）和ProfVLM（将熟练度估计重新表述为条件语言生成）。这些方法实现了最先进精度，且参数量和训练轮次显著减少。


<details>
  <summary>Details</summary>
Motivation: 熟练度估计（评估一个人执行动作的好坏而非执行什么动作）是教练、康复和人才识别的核心任务。这一任务具有挑战性，因为熟练度编码在细微的时序、平衡、身体力学和执行差异中，通常分布在多个视角和短时事件中。

Method: 论文讨论了三种方法：1) SkillFormer：参数高效的选择性多视角融合判别架构；2) PATS：通过保留基本运动的局部密集片段改进时序采样；3) ProfVLM：将熟练度估计重新表述为条件语言生成，通过门控跨视角投影器和紧凑语言骨干生成熟练度标签和专家风格反馈。

Result: 这些方法在Ego-Exo4D上实现了最先进的准确率，模型参数减少高达20倍，训练轮次减少高达3倍，同时实现了从闭集分类向可解释反馈生成的转变。

Conclusion: 结果凸显了向高效多视角系统的转变，这些系统结合了选择性融合、熟练度感知采样和可操作生成反馈。
...
TL;DR: Stream-R1是一种基于可靠性-困惑度感知奖励的蒸馏框架，用于加速自回归流式视频扩散模型。它通过在 rollout 级和时空元素级自适应重新加权蒸馏目标来解决现有 DMD 方法忽视监督方差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有 DMD 蒸馏方法不加区分地让学生匹配教师输出，将每个 rollout、帧和像素视为同等可靠的监督。作者认为这限制了蒸馏质量，因为忽略了两个互补的方差轴：1) Inter-Reliability：不同 student rollouts 的监督可靠性可变；2) Intra-Perplexity：空间区域和时间帧对质量提升的贡献不均等。

Method: 提出 Stream-R1 框架，通过单一共享的奖励引导机制在两个层级自适应重新加权蒸馏目标：1) Inter-Reliability 级：用预训练视频奖励分数的指数重新调整每个 rollout 的损失，使可靠监督的 rollout 主导优化；2) Intra-Perplexity 级：将同一奖励模型反向传播提取每像素梯度显著性，纳入空间和时间权重以集中优化压力在可获得最大预期收益的区域和帧。自适应平衡机制防止任何单一质量轴主导。

Result: 在标准流式视频生成基准上，Stream-R1 在视觉质量、运动质量和文本对齐三个维度上均获得一致优于蒸馏基线的改进，且无需架构修改或额外推理成本。

Conclusion: Stream-R1 通过可靠性-困惑度感知的奖励蒸馏机制有效提升了蒸馏质量，在视频生成的所有核心质量指标上取得一致改进，是一种简单而有效的视频扩散模型加速方法。
...
TL;DR: DMGD是一种新型的免训练引导扩散数据集蒸馏框架，通过语义匹配和分布匹配双引导机制，在ImageNet系列数据集上超越需要微调的SOTA方法，准确率提升2.1%-5.4%


<details>
  <summary>Details</summary>
Motivation: 数据集蒸馏旨在将大规模数据集的信息蒸馏到更小的合成数据集以实现高效训练。近年来基于扩散的方法兴起，但通常需要额外的微调阶段，且有效的引导机制未被充分探索

Method: 提出Dual Matching Guided Diffusion (DMGD)框架，包含：1) Semantic Matching通过条件似然优化消除辅助分类器需求；2) 动态引导机制增强合成数据多样性同时保持语义对齐；3) 基于最优传输(OT)的分布匹配方法；4) 开发两种增强策略：Distribution Approximate Matching和Greedy Progressive Matching

Result: 在ImageNet-Woof、ImageNet-Nette和ImageNet-1K数据集上验证。训练-free方法相比需要额外微调的SOTA方法，平均准确率增益分别为2.1%、5.4%、2.4%

Conclusion: DMGD框架作为有效的训练-free方法，在多个ImageNet数据集上实现了显著性能提升
...
TL;DR: 本文对PolyProtect生物特征模板保护方法的不可逆性进行了深入分析。PolyProtect通过多元多项式将人脸嵌入向量转换为受保护模板。为增强不可逆性，作者提出了一种"密钥选择算法"来选择合适的多项式系数和指数。实验表明该算法能显著提高模板反转难度，并可平衡不同重叠参数下的不可逆性。此外，对嵌入向量进行归一化可改善识别准确率。


<details>
  <summary>Details</summary>
Motivation: PolyProtect是一种基于多元多项式的生物特征模板保护方法，用于保护人脸嵌入向量。原始工作关注模板的不可逆性，但作者希望进一步提高其安全性，使得受保护模板更难被反转攻击破解。

Method: 1. 使用基于余弦距离的数值求解器代替欧氏距离来分析模板反转问题；2. 提出密钥选择算法，选择能增强不可逆性的多项式系数和指数；3. 在应用PolyProtect前对嵌入向量进行归一化处理。

Result: 1. 余弦距离使模板反转更容易；2. 密钥选择算法显著增加了模板反转难度；3. 该算法可使不同重叠参数下的不可逆性趋于相等；4. 嵌入向量归一化可改善识别准确率。

Conclusion: 密钥选择算法能有效生成更难反转的PolyProtect模板，同时保留了对不可逆性与准确率之间权衡的良好控制能力。这为生物特征模板保护提供了更强的安全性保障。
...
### [53] [Label-Efficient School Detection from Aerial Imagery via Weakly Supervised Pretraining and Fine-Tuning](https://arxiv.org/abs/2605.03968)
*Zakarya Elmimouni,Fares Fourati,Mohamed-Slim Alouini*

Main category: cs.CV

TL;DR: 提出了一种弱监督学校检测框架，从航拍图像中自动检测学校基础设施，通过自动标注 pipeline和两阶段训练策略，在仅使用50张手动标注图像的低数据场景下实现有效检测，支持全球教育 initiatives。


<details>
  <summary>Details</summary>
Motivation: 准确的学校检测对于支持教育 initiatives（包括基础设施规划和扩大互联网连接未服务地区）至关重要，但许多地区面临记录过时、不完整或不可用的挑战。手动 mapping工作劳动密集且缺乏可扩展性。

Method: 提出了一个两阶段训练 pipeline：1）使用自动标注 pipeline生成训练数据，结合稀疏位置点和语义分割生成基础设施 masks，从中生成边界框，用自动标注的图像训练 detectors学习学校特征表示；2）使用少量手动标注的图像对预训练模型进行微调。该方法专门为低数据场景设计。

Result: 实验结果表明，在低数据场景下，使用仅50张手动标注图像就能获得良好的目标检测性能，显著减少了人工标注的需求。

Conclusion: 该框架能够以最少的人工标注（仅50张手动标注图像）实现大规模的学校基础设施检测，支持教育 iniciativas和全球Connectivity initiatives。
...
TL;DR: 本文提出一种基于自适应带宽核密度估计的混合模型方法，用于改进眼动追踪数据的注视点密度估计。该方法结合了Abramson自适应带宽、中心偏差、均匀组分和先进显着性模型，通过留一被试交叉验证优化参数，在多个基准数据集上显著提升了一致性估计，对最具影响的图像提升超过25%。


<details>
  <summary>Details</summary>
Motivation: 传统固定带宽各向同性高斯核密度估计方法数十年来未曾改变，但随着显着性研究转向样本级评估（如失败案例分析、逐图像模型比较），需要更可靠的逐图像密度估计。先前方法将经验注视密度视为固定 ground truth，影响基准测试结论、排行榜排名和科学声称的准确性。

Method: 提出一种原则性混合模型，结合Abramson自适应带宽核密度估计、中心偏差组分、均匀组分和先进显着性模型，以捕捉不同空间和语义类型的观察者间一致性。通过留一被试交叉验证对每个图像优化所有参数。

Result: 在多个基准数据集上实现显著更高的观察者间一致性估计，对数似然提升5-15%，AUC提升达2个百分点。对于最受影响的图像（失败案例分析中最相关的图像），提升超过25%。利用改进的估计识别了先进显着性模型的失败案例，证明模型仍有显著改进空间。

Conclusion: 经验注视密度不应被视为固定的 ground truth，而应被视为随方法改进而演进的估计量。显着性模型仍存在显著的改进空间。
...
TL;DR: 本文提出StateVLM模型，通过辅助回归损失(ARL)训练策略解决视觉-语言模型在目标检测和目标状态定位中的数值推理难题，并构建OSAR基准数据集进行评估。


<details>
  <summary>Details</summary>
Motivation: VLMs在机器人任务中表现优秀，但存在数值推理局限性，特别是在目标检测和目标状态定位方面。现有VLMs难以准确理解对象的位置和状态信息。

Method: 提出辅助回归损失(ARL)训练策略，利用box decoder输出计算回归损失，在微调时保持标准序列预测。开发StateVLM模型用于感知和学习细粒度对象表示，包括对象精确定位、状态和可抓取区域。

Result: 在RefCOCO、RefCOCO+和RefCOCOg基准上，ARL平均提升模型性能1.6%。在OSAR基准上，StateVLM+ARL比无ARL模型平均提升5.2%，并在 Affordance推理中提高输出一致性。

Conclusion: ARL训练策略能有效提升VLMs的数值推理能力，显著改善目标检测和目标状态定位性能，为机器人领域的视觉-语言理解提供新的研究方向。
...
TL;DR: 本文介绍了一个来自Groningen静态地质模型的高分辨率储层属性图像数据集，用于地质图像分析的基准测试。


<details>
  <summary>Details</summary>
Motivation: 储层表征工作流程越来越依赖图像和机器学习/深度学习方法，但缺乏公开可用的地质图像数据集进行可重复的基准测试。

Method: 从三维储层网格生成二维PNG图像（岩相、孔隙度、渗透率、水饱和度），提供可重现的处理工作流，包括数据增强、 mask生成、配对图像构建等。

Result: 提供了一个包含对齐二维图像的数据集和可重现的软件工作流，支持地质图像分析方法的基准测试和跨域关系研究。

Conclusion: 通过将固定图像数据集与可重现处理工作流分离，为地球科学、储层建模和机器学习应用提供了透明的复用基础。
...
TL;DR: 本文提出UnAC（理解、抽象和检查）多模态提示方法，旨在增强大型多模态模型在复杂视觉推理任务中的表现。通过自适应视觉提示策略、图像抽象提示和渐进式自我检查三部分，提升模型对多模态内容的理解和推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管近期的大型多模态模型在视觉感知方面表现强劲，但在需要多步视觉推理的复杂问题上仍不可靠。现有的模型在处理复杂多模态任务时存在推理能力不足的问题。

Method: 提出UnAC多模态提示方法，包含三个关键组件：1）自适应视觉提示策略，使模型聚焦于显著区域以更好地理解图像细节；2）图像抽象提示，有效提取图像关键信息；3）渐进式自我检查方案，通过验证每个分解的子问题及其答案来改进推理过程。

Result: 在三个公开基准数据集（MathVista、MM-Vet和MMMU）上进行广泛实验验证方法有效性。实验结果表明该方法能够显著提升模型在复杂多模态推理任务上的表现。

Conclusion: UnAC方法通过多模态提示策略有效增强了大型多模态模型在复杂视觉推理任务中的推理能力，为提升多模态理解和推理提供了一种有效的解决思路。
...
TL;DR: 本文提出一个从单张RGB图像重建3D人脸模型的 pipeline，包括人脸检测、关键点检测、3DMM模型参数回归和软渲染


<details>
  <summary>Details</summary>
Motivation: 随着CNN在图像处理领域的强大能力，研究者致力于从2D人脸图像或视频中重建详细的3D人脸形状。然而，CNN的训练需要大量标注数据。粗 morphable face model 可以合成标注数据，但难以生成包含皱纹等细节的逼真数据

Method: Pipeline 包含：1) 人脸检测 2) 关键点检测 3) 3DMM模型参数回归 4) 软渲染

Result: 提出从单张RGB图像重建人脸3D模型的pipeline

Conclusion: 该pipeline能够从单张RGB图像重建人脸3D模型，但未提供具体的性能指标或与现有方法的对比结果
...
TL;DR: HeadsUp是一种可扩展的前馈方法，用于从大规模多相机设置重建高质量3D Gaussian头部。该方法使用编码器-解码器架构，通过UV参数化表示将输入视图转换为锚定在中性头部模板上的3D高斯，能够在超过10,000个受试者的大规模数据集上实现最先进的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大规模多视图头部重建时存在局限性：传统的基于优化的方法需要大量计算资源且难以扩展到新身份；现有数据集规模较小（通常只有数百个受试者），限制了模型的泛化能力。需要一种可扩展的、高效的能够处理大规模数据和身份的方法。

Method: 提出HeadsUp方法，采用高效编码器-解码器架构：1）将多视角输入视图压缩成紧凑的潜在表示；2）将潜在表示解码为锚定在中性头部模板上的UV参数化3D高斯；3）UV表示解耦了3D高斯数量与输入图像数量和分辨率的关系，使模型能够使用许多高分辨率输入视图进行训练。

Result: 在超过10,000个受试者的内部数据集上训练和评估（比现有数据集大一个数量级），实现了 state-of-the-art 的重建质量；能够推广到新身份而无需测试时优化；揭示了质量-计算权衡的实际见解；展示了两个下游应用：生成新3D身份和使用表情blendshapes动画化3D头部。

Conclusion: HeadsUp成功实现了大规模多视图3D头部重建的前馈方法，通过UV参数化表示有效解耦了计算复杂度与输入视图数量的关系，为高质量3D头部重建提供了可扩展的解决方案，具有良好的泛化能力和实际应用价值。
...
TL;DR: 论文提出UniReasoner框架，通过利用LLM作为通用推理器，将文本到图像生成任务中的"理解-生成差距"问题进行形式化并解决。该方法让LLM首先生成粗略的视觉草稿，然后进行自我批判评估，最后引导扩散模型进行条件生成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成系统虽然能准确验证图像是否满足复杂提示，但在生成时却常常无法忠实对齐这些提示。这种"理解-生成差距"现象尚未被正式形式化和解决。论文旨在弥合这一差距。

Method: UniReasoner采用三步流程：(1)给定提示后，LLM首先生成由离散视觉 token组成的粗略视觉草稿；(2)LLM对草稿进行自我批判，评估其与提示的一致性，生成有根据的文本评价以指出需纠正之处；(3)扩散模型以提示、视觉草稿和评价三者共同为条件进行生成，确保生成过程受到明确的纠正信号引导。

Result: 实验表明，UniReasoner在保持相同扩散模型主干的情况下，提高了组合对齐性和语义忠实度，同时维持了图像质量。这证明了利用LLM推理来弥合理解-生成差距的可行性和有效性。

Conclusion: 论文通过UniReasoner框架展示了如何将LLM的理解能力转化为直接的生成指导，为解决文本到图像生成中的复杂提示对齐问题提供了实用的方法。
...
### [61] [UniCorrn: Unified Correspondence Transformer Across 2D and 3D](https://arxiv.org/abs/2605.04044)
*Prajnan Goswami,Tianye Ding,Feng Liu,Huaizu Jiang*

Main category: cs.CV

TL;DR: UniCorrn是首个统一二维图像到图像、图像到点云、点云到点云几何匹配任务的模型，采用共享权重设计，利用Transformer注意力捕捉跨模态特征相似性，通过双流解码器维持外观和位置特征流，在2D-3D和3D-3D任务上超越先最先进方法。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉对应任务结构相似，但现有方法使用任务特定设计，每个模态组合需要单独模型，缺乏统一性。

Method: 提出双流解码器维护分离的外观和位置特征流，使用堆叠层支持端到端学习，结合伪点云和真实3D对应注释进行联合训练。

Result: 在2D-2D匹配上具有竞争力，在2D-3D的7Scenes数据集上提升8%，在3D-3D的3DLoMatch数据集上提升10%的配准召回率。

Conclusion: UniCorrn是首个能够用共享权重统一三种几何匹配任务的模型，为跨模态对应学习提供了有效框架。
...
TL;DR: 本文是关于Audio-Visual Intelligence (AVI)的首篇综合综述论文，聚焦于大基础模型时代下的视听智能研究。论文建立了统一的AVI任务分类体系，涵盖理解、生成和交互三大类任务，并系统梳理了方法论基础（模态token化、跨模态融合、自回归和扩散生成、大规模预训练、指令对齐和偏好优化），同时整理了代表性数据集、基准测试和评估指标，为未来大规模AVI研究提供基础参考。


<details>
  <summary>Details</summary>
Motivation: 随着大基础模型的发展，音频和视觉的联合建模日益重要，但目前文献较为分散，缺乏统一的分类体系和系统的比较基础，需要一份综合性的综述来整合该领域的研究进展。

Method: 本文采用系统性文献综述方法，建立统一的AVI任务分类体系，涵盖理解（如语音识别、声音定位）、生成（如音频驱动视频合成、视频转音频）和交互（如对话、具身智能或智能体接口）三大类任务；系统梳理了方法论基础，包括模态token化、跨模态融合、自回归和扩散生成、大规模预训练、指令对齐和偏好优化；同时整理了代表性数据集、基准测试和评估指标。

Result: 论文建立了首个以大基础模型视角的AVI综合综述框架，统一了AVI任务的分类体系，系统梳理了方法论基础，整理了代表性数据集和基准测试，并识别了开放挑战（同步、空间推理、可控性和安全性）。

Conclusion: 本综述为视听智能领域提供了首个以大基础模型为中心的综合参考框架，统一了任务分类体系，系统化了方法论基础，整理了评估资源，并指出了该领域的开放挑战，为未来大规模AVI研究奠定了基础。
...
### [63] [Visibility Queries in Simple Polygons](https://arxiv.org/abs/2605.03334)
...
TL;DR: 研究简单多边形中可见性查询的数据结构构建问题。通过引入新的多边形分解方法，在不同空间限制下实现了更高效的查询时间，包含O(n^{{2+ε}})空间获取O(log n + k)查询时间，以及改进的边界情况处理等核心贡献。


<details>
  <summary>Details</summary>
Motivation: To obtain O(log n + k) query time, the previous best result requires O(n^3) space, which is space-intensive. This paper aims to reduce space complexity while maintaining efficient query time.

Method: We propose a new method for decomposing simple polygons, which may be of independent interest.

Result: For any query point q in a simple polygon P, we construct data structures achieving various trade-offs between space and query time: O(n^{{2+ε}}) space for O(log n + k) time, O(n^2) space for O(log n log log n + k) time, O(n log n) space for O(n^{{1/2+ε}} + k) time. For boundary query points: O(n log n) space for O(log^2 n + k) time or O(n^{{1+ε}}) space for O(log n + k) time.

Conclusion: We present a data structure achieving O(log n + k) query time with O(n^{{2+ε}}) space, improving O(n^3) space requirement. For O(n^2) space, we achieve O(log n log log n + k) query time, improving previous O(log^2 n + k). We also construct O(n log n) space data structure for O(n^{{1/2+ε}} + k) queries. Special case for boundary query points achieves O(log^2 n + k) with O(n log n) space or O(log n + k) with O(n^{{1+ε}}) space.
...
### [64] [Computing Planar Convex Hulls with a Promise](https://arxiv.org/abs/2605.03904)
...
TL;DR: 本文研究在弱输入假设下（即输入序列包含凸包作为子序列）计算平面点集凸包的问题。提出了确定性O(n√log n)算法和随机化O(n log^ε n)算法，并证明了该承诺的紧性下界。


<details>
  <summary>Details</summary>
Motivation: 经典结果显示在特殊输入假设（ lexicographic order、angular order、simple polygonal chain）下可达到线性时间或亚线性时间。本文探索在更弱的输入假设下——仅承诺输入序列包含凸包点作为子序列——是否能突破Ω(n log n)下界，从而回答Löffler和Raichel提出的开放问题。

Method: 1. 利用"仅 Hull 点已排序"的承诺设计算法框架；2. 确定性算法采用分治策略与√log n因子权衡；3. 随机化算法通过随机采样和概率分析优化至 O(n log^ε n)；4. 通过构造_adversarial_输入实例证明下界：允许一个 Hull 点乱序即导致 Ω(n log n) 时间复杂度。

Result: 1. 确定性算法：O(n√log n)时间复杂度；2. 随机化算法：期望 O(n log^ε n)（对任意常数 ε>0）；3. 下界证明：若承诺被轻微违反（仅一个 Hull 点乱序），任何算法需要 Ω(n log n)时间；4. 承诺验证复杂度：验证该承诺本身需要 Ω(n log n)比较。

Conclusion: 本文否定了Löffler和Raichel的猜想，证明即使在非常弱的"凸包作为子序列"承诺下，仍存在亚-O(n log n)算法；但该承诺必须严格成立——任何轻微违反都会导致下界回归。意义在于揭示了"Hull 点排序"这一极弱条件对于突破经典下界的关键作用。
...
TL;DR: 研究解决了Thiele规则在voter interval域上的计算复杂度开放问题，证明标准线性规划虽矩阵非完全单模但仍保最优整数解，并扩展到VCI和LC域，证明其包含关系，同时发现在树形推广下Thiele规则变为NP-hard。


<details>
  <summary>Details</summary>
Motivation: Thiele规则（特别是PAV）在一般情况下计算是NP-hard的，但在候选区间(CI)域上可通过线性规划多项式时间求解。研究者曾猜测 voter interval (VI) 域也可同样处理，但这一点长期未被证明，成为开放问题。

Method: 通过分析相关矩阵的非完全单模性，尽管标准线性规划的约束矩阵不是完全单模的，但仍存在最优整数解，并设计了快速算法求解。此外，通过图论连接建立VCI与LC域的关系。

Result: 解决了VI域的计算复杂度问题：标准LP仍存在最优整数解。证明了LC域严格包含VCI域。进一步发现树形推广下Thiele规则变为NP-hard。

Conclusion: 完全解决了VI域的计算复杂度问题，并将结果推广到VCI和LC域，同时揭示了树形推广下Thiele规则的计算困难性，为approval voting的理论和算法研究做出重要贡献。
...
TL;DR: 本文探讨了组织中AI采用失败的现象，研究表明员工在AI设计和使用决策中常被忽视，导致整合困难。通过对医疗、金融和管理领域专业人士的访谈，识别了关键障碍，并提出在个体、任务和组织层面进行适应性调整的策略。


<details>
  <summary>Details</summary>
Motivation: AI技术虽被广泛引入组织以推动创新和效率，但许多采用努力以失败告终，员工抵制并难以整合这些系统。这反映出更深层次的问题：负责与AI协作的员工在AI设计和使用的决策中往往被忽视。

Method: 通过对医疗、金融和管理领域每日与AI系统互动的专业人士进行访谈，收集定性数据，分析组织期望与员工体验之间的脱节，识别关键的采用障碍。

Result: 识别出四大关键障碍：可用性和互操作性差、期望不匹配、控制有限以及沟通不足。这些挑战突出了组织实施AI与实际 worker需求、任务和工作流程之间的差距。

Conclusion: 成功的AI采用需要将员工视为AI整合的核心，需要在个体、任务和组织层面制定适应性策略，以更好地使AI系统与现实实践相匹配。
...
TL;DR: 本文提出了一种新的基于大型语言模型（LLM）的符号回归进化搜索框架，通过程序化上下文增强方法，使模型能够主动与数据集进行代码交互并提取信息性信号，超越了传统的仅依赖均方误差等标量指标的评估方式。在LLM-SRBench等基准测试中，该方法展现出优于现有方法的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统的符号回归方法主要基于遗传算法等进化方法，存在可扩展性和表达能力受限的问题。近年来引入的LLM方法虽然显示出潜力，但现有方法通常仅依赖标量评估指标（如均方误差）作为搜索过程中的唯一反馈来源，忽略了数据集中嵌入的丰富信息。这一局限性影响了搜索效率和准确性。

Method: 提出了一种新的LLM进化搜索框架，包含程序化上下文增强（programmatic context augmentation）。该方法通过代码与数据集进行交互，使模型能够主动执行数据分析并提取信息性信号，而不仅仅依赖聚合的评估分数。

Result: 在高级基准测试（如LLM-SRBench）上评估了该框架，并展示了优于强基线的效率和准确性。

Conclusion: 该研究成功证明了程序化上下文增强方法在LLM-based符号回归中的有效性，通过允许模型主动分析数据集而不仅仅是评估拟合误差，可以显著提升搜索效率和最终表达式的准确性。这一方法为科学发现中的符号回归任务提供了新的思路。
...
TL;DR: 本文系统评估了多种提示策略在LLMs精确计算任务上的表现，发现思维程序（PoT）通过生成可执行代码能达到完美准确率，而传统推理方法改进有限，建议将LLMs与外部工具结合以提高可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在自然语言理解和推理方面表现出色，但其执行精确、确定性计算的能力尚不明确。作者试图系统评估多种提示策略在需要精确输出的任务上的表现。

Method: 作者评估了四种提示策略：思维链（CoT）、最少到最多分解（Least-to-Most）、思维程序（PoT）和自一致性（SC），在二进制计数最长子字符串检测和算术评估等任务上的表现，并引入了合成数据集进行受控评估。

Result: 标准提示方法在序列任务上仅达到中等准确率；CoT改进有限；Least-to-Most因错误累积表现不佳；PoT通过生成可执行代码达到完美准确率；自一致性通过多数投票提高鲁棒性但计算开销较大；使用CodeT5-small领域特定模型可在合成测试数据上达到完美准确率。

Conclusion: LLMs可能只是模拟推理模式而非可靠地执行精确符号计算。对于确定性任务，将LLMs与外部工具结合或使用专用模型能提供更可靠高效的解决方案。
...
TL;DR: 本文将旅行规划分解为五个原子子能力：约束提取、工具使用、计划生成、错误识别和错误修正，并提出解耦评估协议来隔离测量这些组件的性能。研究发现LLM在提取显式约束方面表现出色，但在推断隐式、开放世界需求方面存在困难，且在计划生成中存在结构性偏差，自修正能力无效（过度敏感和错误坚持）。


<details>
  <summary>Details</summary>
Motivation: 现有基准和评估主要对最终计划进行端到端评估，缺乏可解释性，难以分析失败的根本原因。需要一种更细粒度的方法来隔离分析LLM在旅行规划中各组件的能力。

Method: 将旅行规划分解为五个原子子能力（约束提取、工具使用、计划生成、错误识别、错误修正），利用oracle中间上下文实施解耦评估协议，隔离测量各组件性能，避免级联错误的干扰。

Result: LLM在提取显式约束方面表现擅长，但在推断隐式、开放世界需求方面存在困难；在计划生成中存在结构性偏差；自修正无效，表现为过度敏感和错误坚持。

Conclusion: 为改进LLM推理和规划能力提供了精确方向：需要提升隐式约束推断能力、减少计划生成的结构性偏差、增强有效的自我修正能力。
...
TL;DR: LaF-MCTS是一种利用大型语言模型自动设计大规模车辆路径问题（CVRP）求解器的新框架。该方法通过三层决策层次结构、语义剪枝和分支再生技术，在算法假设空间中进行高效搜索，自主构建出超越现有最先进求解器的高性能分解增强型求解器。


<details>
  <summary>Details</summary>
Motivation: 大规模CVRP（LSCVRP）问题节点数量众多，现有求解器难以有效处理。分解-征服方法虽能通过分解实例为子问题来扩展规模，但设计分解逻辑和配置子求解器需要高度专业知识和大量人工干预。现有LLM驱动的方法在处理LSCVRP时面临困难，主要因为在有限上下文窗口内难以生成复杂的搜索策略。

Method: 提出LLM辅助的灵活蒙特卡罗树搜索（LaF-MCTS）框架。该框架包含三层决策层次结构，实现分解策略和子求解器的增量设计。引入语义剪枝（semantic pruning）消除语义和结构上冗余的代码，以及分支再生（branch regrowth）重新生成代码并保持多样性。

Result: 在CVRPLib数据集上进行广泛实验。LaF-MCTS能够自主组合和优化分解增强型求解器，其性能超越了各种最先进的CVRP求解器。

Conclusion: 成功提出LaF-MCTS框架，为自动化算法设计提供了一种新的有效方法。通过LLM辅助的方式解决了大规模CVRP求解困难的问题，三层决策层次结构和相关技术的引入使得系统能够自动生成高质量的分解策略和子求解器配置。
...
TL;DR: 研究LLM Agent的记忆失败问题，发现控制电路在0.6B参数即可检测，而内容电路需4B才能检测到，提出特征空间分离方法可实现76.2%的无监督故障定位。


<details>
  <summary>Details</summary>
Motivation: LLM Agent在跨会话操作中可能 silently fail，即表面生成流畅回复但实际未能正确提取、保留或检索所需信息。传统pipeline分析无法定位内部计算具体哪个环节出现问题。

Method: 通过特征电路追踪(feature circuit tracing)方法，分析Qwen-3家族(0.6B-14B)和两种记忆框架(mem0和A-MEM)在write-manage-read循环中的内部计算过程。

Result: 发现三个关键结论：(1)控制电路(路由)在0.6B即可检测，内容电路需4B才能检测，小模型会看似能路由但实际在提取和接地阶段静默失败；(2)Write和Read共享一个late-layer hub作为上下文接地基质，仅在记忆框架中才被激活功能性接地方向；(3)内容电路在4B可检测但仅在8B才可靠可控，检测和干预有不同阈值。特征空间分离方法可实现76.2%的无监督故障定位精度。

Conclusion: Agent memory failures本质上是可诊断的，控制与内容电路在特征空间中存在可分离的表示，为静默失败的阶段级定位提供了实用诊断方案。
...
### [72] [ReasonAudio: A Benchmark for Evaluating Reasoning Beyond Matching in Text-Audio Retrieval](https://arxiv.org/abs/2605.03361)
*Honglei Zhang,Yuting Chen,Chenpeng Hu,Siyue Zhang,Yilei Shi*

Main category: cs.AI

TL;DR: ReasonAudio是首个针对文本-音频检索的推理密集型基准，包含五个推理任务和1,000个查询。评估十个最先进模型发现所有模型在Negation和Duration任务上表现最差，且当前训练范式无法保留检索设置中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有音频检索基准集中于语义匹配，未能捕捉真实世界查询所需的推理能力，包括否定理解、时序排列、重叠事件识别和时长判别。

Method: 构建了ReasonAudio基准，包含1,000个查询和10,000个复合音频片段，涵盖五个推理任务：Negation（否定理解）、Order（时序排列）、Overlap（重叠事件识别）、Duration（时长判别）和Mix（混合任务）。对十个最先进模型进行系统评估。

Result: 所有模型在推理密集型音频检索上均表现困难，在Negation和Duration任务上尤其差，而在Overlap和Order任务上相对较好。多模态大语言模型-based嵌入模型未能继承其骨干模型的推理能力。

Conclusion: 当前模型在推理密集型音频检索任务上表现不佳，尤其是Negation和Duration任务。基于多模态大语言模型的嵌入模型未能通过对比微调继承其骨干模型的推理能力，表明当前训练范式不足以保留检索设置中的推理能力。
...
### [73] [GeoDecider: A Coarse-to-Fine Agentic Workflow for Explainable Lithology Classification](https://arxiv.org/abs/2605.03383)
*Jiahao Wang,Mingyue Cheng,Yitong Zhou,Qingyang Mao,Xiaoyu Tao,Qi Liu,Enhong Chen*

Main category: cs.AI

TL;DR: GeoDecider是一种基于大型语言模型(LLM)的无训练岩性分类智能体工作流，采用粗到细的多阶段推理流程，包括基分类器引导的粗分类、工具增强的推理和地质精炼三个阶段，旨在实现类似专家的准确且可解释的岩性分类。


<details>
  <summary>Details</summary>
Motivation: 现有岩性分类方法将其视为单次分类任务，而实际地质专家会结合地质原理、外部知识和工具使用来进行准确分类。现有方法缺乏类似专家的多阶段推理能力。

Method: 提出GeoDecider框架，包含三个阶段：(1)基分类器引导的粗分类，使用预训练分类器提供粗糙参考，降低下游推理成本；(2)工具增强的推理，利用上下文分析和邻居检索等工具实现更精细的分类；(3)地质精炼，对最终结果进行后处理以确保地质一致性。

Result: 在四个基准数据集上的实验表明，GeoDecider优于代表性基线方法。该框架能够产生地质可解释的预测，同时在分类性能和推理效率之间实现更好的权衡。

Conclusion: GeoDecider通过模拟专家的分类思维过程，成功将岩性分类从单次任务转变为结构化多阶段推理任务，在提升分类准确性的同时保证了可解释性和推理效率。
...
TL;DR: RAC（鲁棒Agent补偿）是一个基于日志的恢复范式，通过架构扩展实现，为Agent框架提供安全网，支持可靠执行，避免意外副作用。


<details>
  <summary>Details</summary>
Motivation: 在复杂问题求解中，Agent可能会产生意外副作用，当前缺乏有效的安全网机制来恢复和保证可靠执行。

Method: 通过架构扩展点在现有Agent框架（如LangChain/LangGraph）中实现基于日志的恢复机制，无需修改用户代码。

Result: 在τ-bench和REALM-Bench上验证了可行性，相比最先进的LLM恢复方法，RAC在延迟和token经济方面提升1.5-8倍或更多。

Conclusion: RAC可作为大多数Agent框架的通用扩展，通过日志恢复机制有效提升Agent执行的可靠性和效率。
...
TL;DR: UFCOD是一个统一的少样本跨域分布外(OOD)检测框架，利用扩散模型的噪声预测作为分数函数（对数密度的梯度），提取路径能量和动力学能量两个特征，在单个数据集上训练后可部署到任意新领域，仅需约100个无标签样本即可实现高效OOD检测。


<details>
  <summary>Details</summary>
Motivation: 标准OOD检测器只能在单一域上检测，而实际应用中需要处理任意新任务对；同时现有方法需要大量训练样本（50k-163k），样本效率低下。研究者希望实现"训练一次，部署 anywhere"的通用OOD检测。

Method: 利用扩散模型噪声预测作为分数函数（log-density的梯度），提取两个能量特征：Path Energy（积分分数幅度）和Dynamics Energy（分数平滑度），形成离散Sobolev范数来捕获样本与扩散过程的交互。

Result: 在12个跨域基准上使用每任务100个ID样本达到93.7%平均AUROC，与使用50k-163k样本的方法相当，样本效率提升约500倍。

Conclusion: 成功实现了基于扩散轨迹信息几何分析的少样本跨域OOD检测，在无需重新训练的情况下可泛化到语义无关的新领域，具有极高的样本效率和应用价值。
...
### [76] [Replacing Parameters with Preferences: Federated Alignment of Heterogeneous Vision-Language Models](https://arxiv.org/abs/2605.03426)
*Shule Lu,Yujing Wang,Hainan Zhang,Xiaoshan Yang,Hongwei Zheng,Yongxin Tong,Changsheng Xu,Zhiming Zheng*

Main category: cs.AI

TL;DR: MoR是一个针对异构VLMs的联邦对齐框架，通过结合GRPO与Mixture-of-Rewards机制，实现隐私保护下的偏好对齐。客户端本地训练奖励模型捕获特定评估信号，服务器端通过GRPO与KL惩罚优化基础模型，无需共享原始数据或模型参数。


<details>
  <summary>Details</summary>
Motivation: VLMs在医疗、金融等隐私敏感领域有广阔应用前景，但严格的数据共享约束使得集中式训练不可行。联邦学习虽能实现去中心化训练，但在极端模型和数据异构性下面临挑战。传统的参数聚合方法需要直接交换参数或数据，不适用于此类场景。偏好-based协作提供更合适的接口，可消除参数或数据直接交换的需求。

Method: 提出MoR框架，结合GRPO与Mixture-of-Rewards机制。具体包括：(1)客户端本地训练奖励模型，从本地偏好标注中学习特定评估信号；(2)Mixture-of-Rewards机制通过学习路由自适应融合不同客户端的奖励模型；(3)服务器使用GRPO结合KL惩罚项优化基础VLM，无需共享模型架构或参数。

Result: 在多种公开视觉语言基准数据集上的实验表明，MoR在泛化能力和跨客户端适应性方面始终优于联邦对齐基线方法。

Conclusion: MoR为联邦设置下异构VLMs的隐私保护对齐提供了可扩展的解决方案，能够在满足数据隐私约束的同时，有效整合来自不同客户端的异构偏好信号，实现跨客户端的良好泛化性能。
...
### [77] [FinSTaR: Towards Financial Reasoning with Time Series Reasoning Models](https://arxiv.org/abs/2605.03460)
*Seunghan Lee,Jun Seo,Jaehoon Lee,Sungdong Yoo,Minjae Kim,Tae Yoon Lim,Dongwan Kang,Hwanil Choi,Soonyoung Lee,Wonbin Ahn*

Main category: cs.AI

TL;DR: 本文提出FinSTaR（金融时序思考推理模型），针对金融领域构建2x2能力分类体系，结合Compute-in-CoT和Scenario-Aware CoT两种思维链策略，在FinTSR-Bench基准上达到78.9%平均准确率，显著优于现有LLM和TSRM基线模型。


<details>
  <summary>Details</summary>
Motivation: 时序推理模型在通用领域表现良好，但在金融领域（具有独特特征） consistently fail。金融领域特别需要区分确定性评估与随机性预测，因为金融数据具有独特的噪声和模式。

Method: 提出2x2能力分类学：1）单实体vs多实体分析 × 2）当前状态评估vs未来行为预测。针对评估任务（确定性）采用Compute-in-CoT（程序化思维链）直接从原始价格推导答案；针对预测任务（随机性）采用Scenario-Aware CoT（场景感知思维链）先生成多样场景再判断，模拟金融分析师的不确定性推理方式。基于S&P股票构建包含10个金融推理任务的FinTSR-Bench基准。

Result: 在FinTSR-Bench上达到78.9%平均准确率，显著优于LLM和TSRM基线模型。四种能力类别通过联合训练互为补充、互相增强。Scenario-Aware CoT相比标准CoT持续提升预测准确率。

Conclusion: FinSTaR成功解决了金融时序推理难题，两种定制思维链策略有效区分了确定性评估与随机性预测任务。2x2能力分类体系为金融时序推理研究提供了系统化框架，验证了四种能力类别的互补性和互强化效果。
...
### [78] [Real-Time Evaluation of Autonomous Systems under Adversarial Attacks](https://arxiv.org/abs/2605.03491)
*Adithya Mohan,Xujun Xie,Venkatesh Thirugnana Sambandham,Torsten Schön*

Main category: cs.AI

TL;DR: 本文提出一个基于真实世界交叉口数据的离线轨迹学习和对抗鲁棒性评估框架，训练比较了MLP-BC、Transformer-BC和GAIL-IRL三种范式，发现状态结构设计和架构归纳偏置对抗稳定性有显著影响，PGD攻击可达8米FDE。


<details>
  <summary>Details</summary>
Motivation: 大多数自动驾驶策略的对抗性评估在仿真环境中进行，因为成本低且无物理风险。但纯虚拟测试无法捕捉真实世界中出现的数据结构不一致、监督约束和状态表示效应等问题。现有方法缺乏基于真实世界数据的离线轨迹学习和对抗鲁棒性评估框架。

Method: 在受控数据契约下，使用真实世界交叉口驾驶数据训练和比较三种轨迹学习范式：基于MLP的行为克隆(BC)、基于Transformer的对象标记化BC，以及在生成对抗模仿学习(GAIL)框架中提出的逆强化学习(IRL)。使用平均位移误差(ADE)和最终位移误差(FDE)进行评估。通过对训练好的策略施加梯度域对抗扰动来评估推理时的鲁棒性。

Result: -state-structure设计和架构归纳偏置显著影响对抗稳定性，导致尽管名义预测精度相近(ADE<0.8米)但鲁棒性差异明显。推理时的PGD攻击可导致高达约8米的最终位移误差。

Conclusion: 该框架为研究真实自动驾驶场景中的离线轨迹学习和对抗鲁棒性建立了一个可扩展的基准，揭示了架构选择对安全关键场景下策略稳定性的重要影响。
...
TL;DR: 研究大型语言模型在推理时如何被引导向特定伦理框架（功利主义或义务论），提出Convergent-Divergent Routing和Dual Logit Calibration方法，通过编辑transformer内部路径分支点和校准残差来实现道德偏好校准


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在不同设置下表现出异质的道德偏好，需要在推理时引导向期望的伦理框架，同时保持模型的一般能力

Method: 提出Convergent-Divergent Routing方法，追踪并编辑transformer块内的最小分支点，在伦理框架相关路径首次汇聚和发散的地点进行门控控制；adapt Common Spatial Patterns提取能够区分功利主义和义务论框架的方向对；引入Dual Logit Calibration进行封闭形式的最小ℓ2范数更新

Result: 该方法单独使用即可增加目标伦理框架推理；在真实道德困境上的实验表明方法可靠地实现偏好校准并很大程度上保持一般能力，优于最近的基线方法，同时提供可解释的机制

Conclusion: 成功实现对LLM道德偏好的细粒度控制，在保持模型能力的同时实现偏好校准
...
TL;DR: 研究使用生成模型（解码器-only transformer）在合成规划数据上生成高质量计划，通过自改进方法提升模型性能，在Blocksworld、Logistics、Labyrinth和Sokoban四个领域实现平均30%计划长度减少。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型研究仅关注找到任何有效计划，而高质量计划生成是计算难题；挑战在于如何在亚指数时间内生成高质量解决方案。

Method: 1) 证明在最优数据下，解码器-only transformer可为未见问题生成高质量计划；2) 提出自改进方法：每轮结合多模型调用和图搜索生成改进计划，用于模型微调；3) 在四领域进行实验验证。

Result: 平均计划长度减少30%，超过80%计划达到最优，推理时间搜索可进一步提升质量，模型延迟呈亚指数增长。

Conclusion: 自改进与生成模型结合为高质量计划生成提供了一种可扩展方法。
...
TL;DR: AdapShot是一种动态优化少样本数量并利用KV缓存重用实现高效推理的方法，通过基于输出熵的探测评估机制来确定最佳示例数量。


<details>
  <summary>Details</summary>
Motivation: 现有Many-Shot ICL方法依赖固定数量的示例，无法适应不同查询的难度差异；长上下文的计算和内存成本过高限制了方法的可行性。

Method: 设计基于探测的评估机制利用输出熵确定最优示例数量；引入语义感知的KV缓存重用策略绕过冗余预填充计算；提出解耦和重新编码方法解决位置编码不兼容问题。

Result: 与SOTA DBSA相比，AdapShot实现平均约10%的性能提升和4.64x的加速。

Conclusion: AdapShot能够根据查询难度动态调整示例数量，在保持推理效率的同时显著提升性能。
...
TL;DR: 提出OracleProto框架，用于评估大型语言模型的原生预测能力，通过重建已解决事件为有时间限制的预测样本来解决LLM预测评估的难题。


<details>
  <summary>Details</summary>
Motivation: LLM正从静态文本生成转向现实决策支持系统，预测能力急需评估，但现有基准存在缺陷：实时基准在事件解决前评估但会过期，回顾性基准虽可复现但无法区分真实预测与预训练已学知识。

Method: OracleProto通过模型截断点对齐的样本准入、工具级时间掩码、内容级泄漏检测、离散答案规范化和分层评分来重建已解决事件。在FutureX-Past衍生数据集上对六个当代LLM进行实例化测试。

Result: OracleProto能够区分预测质量、采样稳定性和成本效率，将残余泄漏降至1%，比纯工具时间过滤低一个数量级。

Conclusion: OracleProto将LLM预测从一次性评估转变为可审计、可复用、可训练的数据集级能力，提供统一的公平跨模型比较接口和下游SFT和RL的受控信号源。
...
TL;DR: 提出了一个任务无关的、基于Agent增强的LLM框架用于无人机蜂群控制，用户可以用自然语言表达任务目标，系统通过基于标准的Web of Things架构进行 grounding 的实时交互执行。


<details>
  <summary>Details</summary>
Motivation: LLM在无人机蜂群管理中应用面临异构接口、有限 grounding 和长时间闭环执行的挑战。

Method: 结合LLM Agent Core、MCP网关和基于W3C WoT标准的Web-of-Drones抽象层，将无人机、传感器和服务暴露为标准化的WoT Things，实现结构化工具交互、持续状态观察和安全执行。

Result: 基于ArduPilot仿真评估4个蜂群任务和6个SOTA LLMs，尽管LLM有强推理能力，无显式grounding和执行支持时仍难以可靠执行简单任务。任务特定规划工具和运行时guardrails显著提高鲁棒性，token消耗不反映执行质量。

Conclusion: 当前通用LLM在无人机蜂群控制中需要任务特定工具和运行时guardrails才能实现可靠执行，token消耗不是可靠性的有效指标。
...
TL;DR: 提出ScrapMem框架，解决边缘设备上LLM智能体长期个性化记忆的存储成本和 Multimodal 复杂性挑战，通过"剪贴簿页面"集成多模态数据。


<details>
  <summary>Details</summary>
Motivation: 在资源受限的边缘设备上，为LLM智能体提供长期个性化记忆面临高存储成本和多模态复杂性的双重挑战。

Method: ScrapMem框架包含两个核心创新：(1)光学遗忘(Optical Forgetting)机制，逐步降低旧记忆的分辨率以降低存储成本；(2)情景记忆图(EM-Graph)将关键事件组织为因果-时序结构以保持语义一致性。

Result: 在多模态ATM-Bench上实现：Joint@10得分51.0%达到新SOTA；通过光学遗忘存储节省高达93%；通过结构化聚合Recall@10提升至70.3%。

Conclusion: ScrapMem为多模态LLM智能体的设备端长期记忆提供了有效且存储高效的解决方案。
...
TL;DR: SOAR是一种统一的深度强化学习框架,用于解决仓储机器人移动履行系统(RMFS)中订单分配和机器人调度的实时联合优化问题。该方法通过将订单分配和机器人调度转换为统一流程,利用软订单分配作为观察结果,并采用事件驱动的马尔可夫决策过程来响应异步系统事件。实验表明该方法可将全局Makespan减少7.5%,平均订单完成时间减少15.4%,延迟低于100ms。


<details>
  <summary>Details</summary>
Motivation: RMFS的优化面临两大挑战:严格的实时性约束和多阶段决策的强耦合。现有的方法要么将问题分解为独立的子任务以保证响应速度但牺牲全局最优性,要么依赖计算昂贵的全局优化模型,不适用于动态工业环境。因此需要一种能够在保证实时性的同时实现全局优化的方法。

Method: 1.将订单分配和机器人调度问题统一处理,使用软订单分配作为观察
2.将问题建模为事件驱动的马尔可夫决策过程(EDMDP),使智能体能够响应异步系统事件进行同时调度
3.采用异构图Transformer编码仓库状态并整合分阶段领域知识
4.引入奖励塑造策略解决长视野任务中的稀疏反馈问题

Result: 在与Geekplus合作进行的大规模合成数据集和真实工业数据集实验中,SOAR将全局Makespan减少7.5%,平均订单完成时间减少15.4%,延迟低于100ms。sim-to-real部署验证了其在生产环境中的实用性和显著性能提升。

Conclusion: 本文提出的SOAR框架成功实现了RMFS中订单分配和机器人调度的实时联合优化,通过统一深度强化学习方法解决了传统方法中分解优化导致的全局最优性损失和计算昂贵难题。异构图Transformer和事件驱动MDP的创新设计使得该方法能够在严格的实时性约束下达到接近全局最优的性能,为工业仓储自动化提供了可行且高效的解决方案。
...
TL;DR: 本文提出"机械良知"概念,作为分布式智能系统中轨迹级规范调节的数学框架。该框架通过监督过滤器 minimal 修正智能体的行为,使其保持在规范允许范围内,同时考虑认知不确定性。建立的理论性质包括:可容许性等价、最优规制存在性和单调偏差递减。


<details>
  <summary>Details</summary>
Motivation: 分布式协同智能(DCI)系统中,局部正确的决策在不确定性下可能组合成全球不可接受的行为轨迹。现有方法仅在个体动作层面评估可接受性,无法处理多参与者和不确定性情境下的DCI部署风险。这是首个从轨迹层面而非动作层面对分布式智能系统进行规范调节的研究。

Method: 定义机械良知为监督过滤器,对基线策略的动作进行 minimal 修正以减少累积偏离规范允许区域的距离,同时考虑认知不确定性。引入 Conscience Score、Mechanical Guilt 和 Resonant Dependability 等构造提供可解释的词汇和可计算的治理信号。建立三个核心理论性质:可容许性等价、最优规制存在性和单调偏差递减。

Result: MC调节的智能体在轨迹层面保持规范可接受性,而传统控制器会偏离到允许范围外。框架自然扩展到抑制多智能体DCI设置中由交互引起的 emergent risk。MC能够在单智能体和分布式场景中提供轨迹级别的安全保障。

Conclusion: 机械良知为分布式智能系统提供了一种新的轨迹级规范调节方法。该框架在理论上建立了三个核心性质,在实践中展示了有效性,为DCI系统的安全治理提供了可解释和可计算的机制。未来的工作可能包括更复杂的场景验证和实际系统部署。
...
TL;DR: 提出TraceLift框架，通过将推理视为可消费的中间产物，利用冻结执行器的提升来奖励高质量且有用的推理轨迹，解决仅奖励最终答案正确性导致的推理不可靠问题。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习仅基于最终答案正确性奖励模型，这无法保证推理轨迹是可信、可靠或有用的。可能强化"因错误原因正确"的轨迹，或通过奖励捷径来夸大推理增益，导致推理中间状态存在缺陷。

Method: TraceLift是一个计划者-执行者训练框架。计划者生成带标签的推理，冻结执行器将推理转化为最终产物用于验证器反馈。执行器 grounding奖励将基于规则的推理奖励模型(RM)分数乘以在同一冻结执行器上的 measured uplift，从而奖励高质量且有用的轨迹。同时构建了TRACELIFT-GROUPS数据集，包含同一问题的高质量参考轨迹和多种保持任务相关性的缺陷轨迹。

Result: 在代码和数学基准上的广泛实验表明，执行器 grounding的推理奖励比纯执行训练更能改进两阶段计划者-执行者系统。

Conclusion: 推理监督不仅应评估推理轨迹看起来是否良好，还应评估它是否有助于消费该推理的模型。
...
TL;DR: EVOLM是一种无需外部监督的后训练方法，利用语言模型自身的评估能力生成判别性评分标准（rubrics），通过自举方式实现模型的自我改进。核心思想是将模型的评估能力结构化为显式的判别准则，用作训练信号。


<details>
  <summary>Details</summary>
Motivation: 当前后训练方法依赖外部监督（人工标注、外部API或奖励模型），存在局限性：人类无法监督超越自身能力，API创建依赖性，可验证奖励仅覆盖有标准答案的领域。作者提出利用模型自身的评估能力作为奖励来源，这一"自我改进"思路尚未被充分挖掘。

Method: EVOLM训练单一语言模型交替执行两个能力：(1)标准生成器：产生针对具体实例的评估标准，优化以最大化冻结小判别器区分偏好与非偏好响应的能力；(2)策略模型：使用基于标准的条件分数作为奖励。所有偏好信号由策略自身输出通过时间对比构建，无需人工标注或外部监督。

Result: Qwen3-8B模型生成的rubrics在RewardBench-2上超越GPT-4.1达25.7%。联合训练的策略在OLMo3-Adapt套件上达69.3%平均，超越使用GPT-4.1提示的rubrics 3.9%，超越8B奖励模型SkyWork-RM达16%。

Conclusion: 研究表明，将模型评估能力结构化为共演的判别性rubrics可实现无外部监督的自我改进，是LLM后训练的有价值新范式。
