...
TL;DR: DocArena是一个全自动的多模态文档搜索和问答训练数据构建管道，通过MLLM将原始文档集合转换为搜索智能体的训练环境，无需人工标注。构建了包含8,336份文档、79,000个QA对的DocArena-79K数据集，涵盖16个领域和49种语言。


<details>
  <summary>Details</summary>
Motivation: 现有搜索智能体训练方法主要依赖基于文本的环境，且难以构建可控、可扩展的多模态训练数据。现有环境无法满足多模态文档检索和问答的实际需求。

Method: 1) 通过MLLM的视觉感知对文档进行结构化和索引；2) 利用跨页面信息分布构建推理密集型QA对；3) 通过MLLM执行级联质量保证操作；4) 设计Doc-Search智能体基础设施，将视觉感知与策略模型解耦，使基于文本的LLM可作为多模态文档检索和问答的推理骨干。

Result: 在六个多模态文档场景和七个基于文本的QA基准测试中，基于DocArena数据训练的智能体在检索准确性和QA质量上均达到最佳性能。对智能体搜索行为的分析证实了构建训练环境的有效性和可控性。

Conclusion: DocArena能够自动化构建高质量、可控的多模态文档搜索训练环境，为多模态文档检索和问答智能体的训练提供了新的数据基础和评估框架。
...
TL;DR: 本研究提出了一种结合图像处理和深度学习的水果新鲜度检测混合方法，通过图像处理算法量化腐坏程度（0-100分），结合CNN进行二分类，并利用逻辑回归融合两种方法的结果，实现实时检测，准确率超过90%。


<details>
  <summary>Details</summary>
Motivation: 水果腐败是农业领域的重要问题，导致重大经济损失。该研究旨在开发一种实用、低计算资源需求的水果新鲜度检测方案，为农业部门提供可行的解决方案。

Method: 研究开发了图像处理算法量化腐坏程度（0-100分），训练CNN进行二分类（新鲜/腐败），使用逻辑回归综合两种方法的结果，并利用CNN的结果使图像处理算法能够进行二分类。

Result: 该方法在苹果和橙子数据集上验证准确率超过90%，实现实时性能，且不需要高计算资源。局限性在于需要水果在白色或透明背景上隔离。

Conclusion: 简单图像处理技术与机器学习的集成可为农业部门提供实用解决方案。未来可引入先进分割模型实现自动背景去除以提升适用性。
...
TL;DR: Long-Context Generation (LCG)是一个用于长上下文多图像文本到图像生成的框架，旨在解决现有图像生成模型在顺序输出时无法保持一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在单图像合成方面取得了令人印象深刻的质量，但往往无法在顺序输出（如漫画、故事板和视觉叙事）中保持一致性。

Method: LCG采用稀疏关系注意力(SRA)机制选择性地关注跨扩展视觉上下文的核特征，确保语义和布局信息的传播在计算上可控。为加强语义对齐，引入路由一致性约束(RCC)，利用身份感知掩码对齐跨生成分支的结构模式，有效缓解复杂多角色场景中的外观漂移。

Result: 实验表明，LCG在长上下文图像生成的提示对齐和角色一致性方面优于对比基线，包括多角色场景。

Conclusion: LCG框架通过SRA和RCC机制有效提升了长上下文多图像生成的一致性和可扩展性，为漫画、故事板等应用提供了新的解决方案。
...
TL;DR: 本研究提出了一个基于机载激光雷达和近红外RGB正射影像的城市树木地上生物量(AGB)冠层级估算框架，应用于加拿大安大略省810 km²景观区。通过双流交叉注意力网络进行语义分割，结合多尺度分水岭分割进行冠层描绘，并利用与物种特异性异速生长方程校准的功率律模型估算AGB。


<details>
  <summary>Details</summary>
Motivation: 城市树木生物量在空间上缺乏精细量化，传统方法依赖森林清单或粗分辨率产品，无法解析单个树冠或精细尺度异质性。需要一种无需人工标注、可在管理相关分辨率下量化非森林城市树木AGB的方法。

Method: 使用2018和2023年的叶季机载LiDAR(8-10脉冲/m²)和近红外RGB正射影像(0.16-0.20 m)。采用基于规则伪标签训练的双流交叉注意力网络进行建筑、针叶树和落叶树的语义分割；使用多尺度分水岭分割进行冠层描绘；利用与Lambert等(2005)物种特异性异速生长方程校准的冠幅-高度幂律模型估算AGB。

Result: 在独立验证集上，全球/平均精确率、召回率和Dice分数分别为0.86、0.83和0.84。18,713个匹配对的AGB预测R²=0.609(使用清单树冠几何)和R²=0.570(使用作业分割)。聚合至30 m后，2018年总AGB为1.73 Tg，2023年为1.81 Tg(811-850 Gg C)，局部密度可达~140 Mg/ha，5年净碳增益为39 Gg C。深度集成不确定性图识别出高认知不确定性区域。

Conclusion: 该框架使用标准省级数据，无需人工标注，可生成公开的双时相冠层级AGB数据库，分辨率与管理相关。为城市树木碳储量估算提供了可复制的方法论，但冠层描绘仍是主要不确定性来源。
...
TL;DR: 本文提出了一种创新的多任务深度学习模型，用于激光穿透焊接中熔透状态、熔深和焊缝形貌的预测与重建。模型结合卷积神经网络和状态空间模型，从焊缝池图像中提取时空特征。


<details>
  <summary>Details</summary>
Motivation: 激光穿透焊接中，准确评估熔透状态和焊缝形貌对保证焊接质量至关重要。传统方法难以实现实时、高精度的检测，因此需要一种新的智能检测方法。

Method: 提出基于CNN和状态空间模型的多任务深度学习框架，融合焊缝池图像的时空特征与焊接参数。采用CMOS相机采集焊接过程中的焊缝池图像，并构建了增强模型鲁棒性和泛化能力的数据集。

Result: 测试集验证结果：熔透状态预测准确率达99.35%，熔深预测误差为1.79毫米，焊缝横截面重建准确率为95.65%。

Conclusion: 该研究为激光穿透焊接系统的原位质量控制提供了新的方法论和技术路径，具有重要的工程应用价值。
...
TL;DR: 研究将fMRI认知任务onomy从单源迁移扩展到多源迁移，使用布尔整数规划(BIP)分析23个HCP任务状态在预算约束下的任务分配。发现运动状态在运动范式内迁移良好但对非运动目标支持有限，而工作记忆状态(0-back和2-back)在不同预算下获得优先分配。


<details>
  <summary>Details</summary>
Motivation: 现有重建-based任务onomy主要研究一对一迁移，但认知任务由共享和专门的神经过程组织，需要分析多源迁移和预算约束下的任务依赖关系。

Method: 使用掩码fMRI重建作为自监督目标，训练1127个任务特异性和迁移模型，采用布尔整数规划(BIP)分析预算约束下的任务分配，研究23个HCP任务状态。

Result: 单源迁移是方向性和范式结构的：运动状态在运动范式内迁移良好但对非运动目标支持有限。多源迁移取决于源集组成。预算约束下BIP反复分配直接监督到0-back和2-back工作记忆状态。

Conclusion: 揭示了跨范式限制的运动簇和工作记忆状态在高优先级的全局分配目标下的重要性。将重建-based fMRI任务onomy从一对一迁移扩展到多对一关系和预算约束依赖。
...
TL;DR: 本文提出GeMoE（基于门控熵的不确定性自适应路由）方法，将token路由视为信息编码任务，通过MDL框架动态确定每个token应该激活的专家数量，在保持99.5%性能的同时提升了36.5%的专家激活稀疏性。


<details>
  <summary>Details</summary>
Motivation: 传统MoE使用Top-k静态路由策略，无法根据输入变化自适应选择专家数量，导致资源利用不足。作者认为可以将token路由问题重新建模为最小描述长度（MDL）问题，从而在模型复杂度和性能之间取得更好的平衡。

Method: 1. 将token路由重新定义为信息编码任务
2. 建立MDL与门控熵之间的联系
3. 使用门控熵评估token的复杂度
4. 基于复杂度自适应确定每个token激活的专家数量
5. 在多种backbone和benchmark上验证

Result: 在多种backbone和基准测试集上，GeMoE方法实现了：
- 99.5%的平均性能保留（相比原始静态路由）
- 36.5%的平均专家激活稀疏性提升

Conclusion: GeMoE通过将动态路由问题形式化为MDL问题，能够自适应地为不同复杂度的token分配合适的专家资源，在显著提高稀疏性的同时几乎保持了原有的模型性能，为MoE架构的效率优化提供了新的思路。
...
TL;DR: 解决水下浑浊环境下计算机视觉模型性能评估的数据集和指标缺失问题，构建了TUB数据集并提出基于相位一致性的PCD指标。


<details>
  <summary>Details</summary>
Motivation: 水下浑浊环境会快速降低能见度，但现有研究依赖合成数据集，可能无法真实反映信息损失对视觉模型的影响。

Method: 构建包含1320张极端浑浊图像和16000多个高置信度分割掩膜的TUB数据集；提出基于相位 congruency maps的PCD指标，该指标对对比度不变，旨在捕捉结构信息损失。

Result: PCD与实例分割模型在真实和合成浑浊图像上的性能高度相关，而该领域的常用指标则呈现弱相关或无相关。

Conclusion: PCD指标能有效衡量水下浑浊对视觉模型性能的影响，为相关研究提供了实用的评估工具。
...
TL;DR: 本文研究视觉提示调优中图像令牌与提示令牌的融合方式，提出结合仿射变换和交叉注意力的混合融合方案，通过可微分架构搜索联合优化提示和融合策略，在34个数据集上取得优于基线的效果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示调优方法通常简单使用拼接或加法融合图像和提示令牌，缺乏对融合方案的深入研究和比较，因此需要探索更优的融合方案。

Method: 将任务建模为双层优化问题，利用可微分架构搜索联合学习提示和融合方案，搜索空间中除拼接和加法外，新增仿射变换和交叉注意力两种融合方案。

Result: 在VTAB-1k、FGVC和HTA的34个数据集上，方法一致优于提示调优基线，且在冻结骨干网络条件下，实现了更好的准确率-延迟-参数权衡。

Conclusion: 提示与图像令牌的融合方式对视觉提示调优有重要影响，混合融合方案能更有效利用ViT的层语义，为视觉提示调优研究提供新视角。
...
TL;DR: 本研究对视频、图像和音频深伪检测基准进行审计，发现使用简单线性探针在通用自监督表示上的性能即可接近复杂定制检测器，表明现有基准可能主要奖励的是通用模态理解而非真正的取证理解。


<details>
  <summary>Details</summary>
Motivation: 深伪生成器日益接近感知不可区分性，但基准上表现良好的检测器在实际应用中却频繁失败。这种反馈循环令人担忧：基准推动越来越复杂的工程检测器，但这些基准可能并未反映真实世界的深伪，因此复杂性可能在解决错误的问题。研究旨在探讨这些基准实际测量的是什么。

Method: 采用一种简单的诊断方法：对冻结的通用自监督表示使用线性探针。如果这种线性探针能近似定制检测器的性能，则表明基准主要奖励的是通用模态理解而非取证理解。同时使用Fréchet几何来解释生成器级别的难度。

Result: 在视频、图像和音频三种模态中，通用自监督表示上的线性探针性能与定制检测器高度接近。生成器级别的难度部分可由同一表示空间中的Fréchet几何解释。

Conclusion: 在将高分数视为取证理解的证据之前，值得先问基准在多大程度上已被通用表示所解决。基准可能未反映现实威胁模型，定制检测器可能并未真正学习取证理解。
...
TL;DR: VIGIL是一种针对多模态大语言模型的强化学习后训练框架，通过引入几何约束显式最大化视觉输入与生成响应之间的互信息，解决MLLMs因视觉懒惰导致的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: MLLMs继承了大语言模型的强大推理能力，但仍存在幻觉问题，表现为与视觉输入相矛盾。现有对齐方法主要基于文本的结果级奖励优化，导致优化偏差偏向语言捷径，使响应经常与视觉证据相矛盾。

Method: VIGIL引入几何约束，明确最大化视觉输入与生成响应之间的互信息。通过惩罚"盲目自信"实例——即在文本-视觉注意力被掩盖创建反事实盲状态时，模型仍然不当确定的情况。

Result: VIGIL在幻觉和推理基准测试中始终优于最新的对齐方法，且不损害纯文本能力。使用仅25%的偏好数据即达到最先进方法的完整数据性能，并展示了紧急的空间 grounding 能力，无需显式的边界框监督。

Conclusion: VIGIL通过因果视觉 grounding 而非数值奖励拟合来解决视觉懒惰问题，为多模态对齐提供了新思路。
...
TL;DR: 本文提出了一种自监督框架，用于从视频衍生的监督信号中直接学习隐式3D物理动力学。该方法将预测瓶颈从2D图像空间转移到"提升的"3D体素化潜在空间，通过体素特征平流学习动作条件下的转换算子，将物理视为时空状态平流问题来处理。


<details>
  <summary>Details</summary>
Motivation: 当前生成式视频模型虽然实现了高视觉保真度，但缺乏3D几何基础，常常导致物理不一致性和无法维持物体恒常性。现有混合模型依赖显式经典模拟器进行训练和/或推理，不够灵活。

Method: 提出将V-JEPA的语义特征提升到体素化网格中，以单目深度先验为锚点。通过体素特征平流学习动作条件下的转换算子，将物理视为时空状态平流问题。完全通过端到端视频衍生信号和动作条件进行监督，无需访问物理引擎内部状态、标签或代理模型。

Result: 在多个基准测试（CLEVERER、PhysInOne、PhysGaia）上展示了良好的长期结构稳定性和物理合理性，能够在单一统一管道中隐式模拟异质现象（如流体中的刚体运动）。

Conclusion: 该工作开辟了一条可扩展的路径，通过被动观察单目视频来内化物理世界的3D不变量，从而实现通用动态世界模型。
...
TL;DR: 本研究提出了一种高效的多模态深度学习模型，用于遥感图像中的甲烷烟羽检测与分割。该模型通过特征引导甲烷增强（FGME）机制将物理意义上的甲烷线索注入到基于Transformer的RGB表征中，在多语义尺度上实现融合。在MPDataset数据集上的实验表明，该方法在MIoU、MPrecision和Recall三个指标上分别提升了+0.92、+0.87和+1.01，且计算成本显著低于其他高性能架构。


<details>
  <summary>Details</summary>
Motivation: 甲烷是全球变暖的重要温室气体之一，高效检测甲烷烟羽对于理解和缓解全球变暖至关重要。准确识别和分割遥感图像中的甲烷烟羽对于大规模监测具有重要意义。然而，现有的甲烷检测方法在精度和效率之间难以取得良好平衡。

Method: 提出了一种多模态深度学习模型，集成了特征引导甲烷增强（FGME）机制。该机制将物理意义上的甲烷线索注入到基于Transformer的RGB表征中，在多个语义尺度上进行融合。通过多模态融合策略，实现甲烷烟羽的准确分割。

Result: 在MPDataset数据集上的评估表明，该方法优于现有最先进的方法：MIoU提升+0.92，MPrecision提升+0.87，Recall提升+1.01。更重要的是，该方法在计算成本上显著低于其他高性能架构，实现了良好的精度-效率权衡。

Conclusion: 该研究证明了高效的多模态融合策略对于实现准确且可扩展的甲烷烟羽分割具有巨大潜力。这种方法为大规模甲烷监测提供了有价值的解决方案，在实际遥感应用中具有重要的应用前景。
...
TL;DR: APRTrack是一种用于RGB-Event视觉目标跟踪的分层扰动检索框架，旨在增强对部分目标缺失和模态退化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现实场景中存在多种结构化信号退化问题，导致传统多模态融合方法失效。在恶劣环境中，任一模态可能严重失去可靠性，且目标常因遮挡、边缘截断和前景杂波而出现不完整。

Method: 提出两种对抗性扰动分支分别在模态和空间层面模拟信号损坏：全模态失败和局部目标区域缺失。设计分层路由机制消除特征坍塌。提出足迹引导的通道校准Hopfield检索(FCHR)模块进行历史信息补偿。

Result: 在FE108、COESOT、VisEvent和FELT数据集上进行了广泛实验，验证了所提策略的有效性。

Conclusion: APRTrack通过分层扰动和检索机制有效提升了RGB-Event跟踪在部分目标缺失和模态退化条件下的鲁棒性。
...
TL;DR: 本文提出Frame Forgetting Network (FFN)解决长时间视频的Test Time Training (TTT)计算问题，通过仅处理滑动窗口中的3帧和引入surprise度量实现自适应窗口调整。


<details>
  <summary>Details</summary>
Motivation: 现有TTT方法存在两大问题：1)滑动窗口计算量随窗口大小线性增长，对数小时长视频不可行；2)即使帧间相似仍执行TTT，导致计算浪费。

Method: FFN方法：1)仅处理退出帧、当前帧、后继帧三帧来保留时间上下文；2)定义surprise度量衡量新帧相对于历史帧的信息量；3)基于surprise实现自适应窗口调整算法。

Result: 在密集分割、视频分类任务上验证有效性，并泛化到深度估计和多小时长视频场景。

Conclusion: FFN能有效处理小时级长视频的自适应推理，surprise度量为自适应窗口选择提供了理论依据。
...
TL;DR: CRISP是一种新型结构诊断评估范式，用于评估视觉空间智能，通过一致性（隐式感知与显式推理的对齐）来区分语言先验和真正的空间推理。


<details>
  <summary>Details</summary>
Motivation: 当前VLM评估经常混淆语言先验与真正的空间推理，需要一种方法来解耦潜在推理能力与感知瓶颈。

Method: 使用度量3D场景图和oracle干预协议进行细粒度诊断，区分隐式感知和显式推理。

Result: 发现了一种系统的感知-推理脱节。专有模型具有强大的潜在推理引擎，但存在不准确的度量估计和关键的结构表示利用失败；开源模型受限于缺乏多跳组合推理能力。

Conclusion: CRISP将评估重点从仅通过语言先验"猜对"转向真正"感知、验证和推理"，为多模态对齐提供了严格的路线图。
...
TL;DR: PhyEditBench是一个用于评估图像编辑模型物理推理能力的新基准测试。针对现有基准缺乏物理推理评估的问题，该基准包含238个真实世界高分辨率实例和35个反物理合成实例，基于4类12子类的层次化 taxonomy构建。同时提出无训练基线PhyWorld，利用测试时扩展和潜在约简策略，在物理推理任务上超越现有SOTA方法。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑基准缺乏对物理推理的综合评估，而物理推理是处理真实世界场景的关键能力。当前的SOTA编辑方法在物理推理方面存在显著不足，亟需专门的基准来推动这一领域的研究。

Method: 1) 建立层次化taxonomy：4个主类、12个子类；2) 从视频中提取238个高质量高分辨率真实世界实例以捕捉真实物理动态；3) 创建35个合成Anti-Physics反例；4) 提出PhyWorld训练-free基线：结合测试时扩展(test-time scaling)和潜在约简(latent reduction)策略

Result: 对当前SOTA编辑方法的实证分析暴露了它们在物理推理方面的重大局限性。PhyWorld在物理推理任务上优于可比模型，证明了视频生成过程可有效作为图像编辑的推理机制。

Conclusion: PhyEditBench填补了物理推理评估的空白，为图像编辑模型的物理理解能力提供了标准化的测试平台。PhyWorld的成功表明无需额外训练即可通过视频生成机制增强物理推理能力，为未来研究提供了新思路。
...
TL;DR: ForeAgent是一个用于AI生成图像检测的主动式取证框架，采用感知-裁决架构聚合多视角线索（语义、空间、频域特征），并利用多模态大语言模型作为裁决模块进行逻辑推理判断。通过 hindsight驱动的自我改进策略（采样-反思-演进范式），实现持续自我进化。


<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展对现有深度伪造检测方法构成重大挑战。现有方法存在两大局限：对细粒度取证伪影敏感性不足，以及依赖静态合成监督导致灵活性差和成本高。

Method: 1) 采用感知-裁决架构，聚合语义、空间和频域特征的多视角线索；2) 使用MLLM作为裁决模块融合信号进行逻辑推理判断；3) 引入hindsight驱动的自我改进策略，按照采样-反思-演进范式，对训练样本进行推理 rollout，参考真实标签反思失败案例，生成高质量推理轨迹；4) 通过双专家质量门控模块严格筛选，自微调持续进化。

Result: 在Chameleon基准上达到82.18%准确率（比AIDE高16.41%）；在AIGCDetect-Benchmark上16个生成器平均准确率达93.3%；外部评估显示比GPT-5和GPT-5-mini产生更一致且因果可解释的推理。

Conclusion: ForeAgent通过主动式取证框架实现迭代自进化，在AI生成图像检测任务上达到最先进水平，证明了多模态大语言模型结合自我改进策略在深度伪造检测中的有效性。
...
TL;DR: 本文提出了一种基于粗到精策略的混合自监督方法，用于非刚性3D形状匹配。该方法采用双分支架构，包含基于拉普拉斯基和弹性基的两个对称功能 map 学习流，并通过细化模块确保粗映射和精化对应之间的一致性。


<details>
  <summary>Details</summary>
Motivation: 非刚性3D形状匹配是计算机视觉和图形学中的基础性任务，需要在非等距变形和拓扑噪声等挑战性场景下实现高精度匹配。现有的方法在保持计算效率和匹配精度之间难以平衡。

Method: 提出混合自监督方法，采用粗到精策略。架构包含双分支设计：两个对称的功能 map 学习流，分别基于拉普拉斯基和弹性基。细化模块确保粗映射与精化对应之间的一致性，并引入对比能量促进特征区分。

Result: 在多种挑战性场景下（包括非等距变形和拓扑噪声）达到最先进的性能，同时保持计算效率。对比能量能够有效促进特征区分，与现有方法集成可获得一致的改进。

Conclusion: 验证了粗到精混合自监督方法的有效性，为非刚性3D形状匹配提供了新的解决方案。
...
TL;DR: SpaceRipple是一个面向任务感知的轻量级框架,用于地球观测卫星网络中的语义信息传输与机载处理。该框架协调压缩、转发、恢复和语义推理的协作流程,实现语义导向传输而非像素级图像传输。


<details>
  <summary>Details</summary>
Motivation: 地球观测卫星网络生成海量高分辨率影像,但星间和下行资源有限。在时间敏感任务中,地面用户需要任务相关的语义信息而非完整的原始图像下传。

Method: 提出了SpaceRipple框架,包括:1)感知卫星执行自适应压缩和元数据生成以减少星间流量;2)边缘计算卫星恢复接收的表示并提取任务相关语义信息;3)引入压缩感知的MoE增强模块以提高退化视觉输入下的鲁棒性。协调压缩、转发、恢复和语义推理形成协作管道。

Result: 实验结果表明SpaceRipple实现了良好的重建质量、提升的语义检测性能和显著的带宽节省。

Conclusion: SpaceRipple在受限卫星网络资源下展现出高效可靠地球观测的潜力,为天基语义信息传递提供了新范式。
...
TL;DR: 本文提出了DiCoBench，一个针对多模态大语言模型(MLLMs)的高分辨率多图像细粒度感知基准测试，旨在填补现有基准测试无法评估模型自主感知隐式视觉线索能力的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要依赖显式文本线索或低分辨率输入，无法评估模型在高分辨率图像中自主感知隐式视觉线索的能力，导致当前最优MLLMs在微尺度细节捕获方面存在显著性能差距。

Method: 构建了包含765个样本的DiCoBench基准，分为两个递进 track：差异视觉线索和共性视觉线索，覆盖8个感知任务，采用接近2K的高分辨率图像，以多项选择题形式消除评估指标偏差。

Result: 对18个不同的MLLMs进行广泛评估，发现与人类准确率(98.3%)相比存在显著性能差距，最优模型在微尺度细节捕获方面表现困难。

Conclusion: DiCoBench将成为推动未来自主高分辨率多图像感知研究的重要挑战性测试平台。
...
TL;DR: 本文提出了LogicIR，这是首个专门用于图像恢复任务的逻辑门网络（Logic Gate Network, LGN）。该网络采用UNet架构，完全由逻辑门（如NAND、XOR）组成，并引入了可微分比特解码层和索引 shuffling 机制。在多个图像恢复基准数据集上的实验表明，LogicIR能在显著降低计算成本的同时实现较强的性能。


<details>
  <summary>Details</summary>
Motivation: 随着图像恢复模型的计算需求不断增长，学界对轻量级架构的需求日益增加。逻辑门网络（LGN）具有极高的计算效率，但其在图像恢复领域的潜力尚未得到充分探索。

Method: 本文设计了LogicIR，采用完全由逻辑门构成的UNet架构，并提出可微分比特解码层（differentiable bit decoding layer）和索引 shuffling 机制来增强信息传播。

Result: 在多个图像恢复基准数据集上的实验表明，LogicIR能够在显著降低计算成本的同时实现较强的性能表现。

Conclusion: LogicIR作为首个专门用于图像恢复任务的LGN，提供了一种可行且高效替代方案，有望推动轻量级图像恢复模型的发展。
...
TL;DR: TaskTok是一种任务驱动的图像恢复框架，通过选择性恢复任务相关tokens来提升下游高级视觉任务的性能，解决传统生成先验方法计算效率低和语义改变的问题。


<details>
  <summary>Details</summary>
Motivation: 传统图像恢复关注感知质量，而任务驱动图像恢复(TDIR)旨在最大化下游高级视觉任务的性能。现有的生成先验方法存在计算效率低和潜在语义改变的问题。

Method: 通过分析latent token空间，发现任务相关线索在token序列中分布不均匀，呈现索引级专业化。由此提出TaskTok框架，包含可学习的token switch和轻量级token refinement module，选择性地只恢复任务相关tokens。

Result: 在图像分类、语义分割和目标检测等任务上的广泛实验表明，TaskTok显著提升了任务性能，同时具有较高的计算效率。

Conclusion: 选择性恢复任务相关tokens的方法是有效的，能够在保证任务性能的同时提高计算效率，为TDIR领域提供了新的思路。
...
TL;DR: 本文提出位置重绑定缓存重用(PRCR)框架，用于多模态推理中的视觉基础化任务。该方法通过重新绑定视觉特征的坐标位置来实现无重放的视觉回顾，避免了传统方法中重复转发视觉token的高计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有 interleaved multimodal reasoning 方法通过 token replay（重复转发选定的视觉 token）实现视觉回顾，但计算开销巨大。虽然可直接重用历史视觉 KV 缓存，但发现关键问题：缓存的视觉 key 已绑定到原始位置上下文，这种 stale positional binding 在后续解码上下文下会导致注意力扭曲，严重时引发自回归解码崩溃。

Method: PRCR 将原始视觉 KV 缓存与其空间坐标一起存储，然后为选定条目重新分配与当前位置兼容的坐标，并在注入活动解码器缓存之前重新绑定其 key。该设计在保持文本位置连续性和视觉结构相对关系的同时重用历史视觉证据。

Result: 在多个多模态推理基准数据集上的实验表明，PRCR 达到或超过 replay 级别的性能，平均准确率提升 5%，视觉回顾计算量减少高达数万倍。

Conclusion: 有效的缓存重用需要在与当前解码状态兼容的位置下重建视觉证据，而不是直接复制绑定到位置的历史缓存条目。PRCR 为无重放的视觉回顾提供了一个缓存级框架。
...
TL;DR: 本文提出FAROS框架，通过光流引导的标签插值和零样本分割掩码传播，解决手术场景多任务学习中注释粒度不匹配的问题，从稀疏关键帧注释生成密集伪标签，实现时空任务的联合优化。


<details>
  <summary>Details</summary>
Motivation: 手术场景理解中的多任务学习受到注释粒度不匹配的 fundamental 阻碍：时间workflow任务（如相位识别、步骤识别和预测）受益于密集的帧级监督，而像素级空间任务（如器械分割和动作识别）仅在选定关键帧上稀疏注释。这种监督不平衡破坏了共享表示学习，限制了异构手术任务的联合优化。

Method: 提出Flow-guided Annotation for Robust Operating Scenes (FAROS)框架，结合零样本分割的掩码传播与光流估计，在遮挡、烟雾和运动模糊等挑战性手术条件下克服基于外观传播的局限性，从稀疏关键帧注释生成时序一致的密集伪标签。将增强的器械掩码和动作标签集成到统一的Transformer多任务框架中，联合学习手术相位识别、步骤识别、预测、器械分割和动作识别，实现密集时间监督与稀疏空间监督之间的平衡优化。

Result: 在DAVIS 2017基准上验证FAROS的标签插值质量，在稀疏真值协议下确认了跨手术域的鲁棒传播。GraSP、MISAW和AutoLaparo基准的广泛实验表明，FAROS显著改善了跨任务表示学习，提高了时空任务的整体手术场景理解性能。

Conclusion: FAROS通过光流引导的标签插值有效解决了手术场景多任务学习中的注释粒度不匹配问题，为异构手术任务的联合优化提供了新思路，显著提升了跨任务表示学习和整体手术场景理解性能。
...
TL;DR: FracEvent是一种新型事件相机模拟器，通过分数阶弛豫电压动力学模型来模拟像素级生命周期，从而生成更精确的事件流。现有模拟器简化了像素的时间结构，导致事件时序失真。FracEvent采用堆栈弛豫模式结合响应，生成电压状态，通过在连续电压轨迹上定位阈值交叉来发射ON/OFF事件，并保留内存模式以建立残差电压与后续事件时序的关联。


<details>
  <summary>Details</summary>
Motivation: 事件相机虽具有微秒级时间分辨率，但真实事件数据收集困难，需要专用传感器、同步和标注。现有模拟器大多基于对比度阈值事件生成，简化了像素生命周期的时间结构，导致事件时序失真并削弱下游任务迁移效果。因此需要更精确地模拟像素级时间动态。

Method: 给定对数强度轨迹，FracEvent驱动紧凑的弛豫模式堆栈，将响应合并为电压状态，通过在连续电压轨迹上定位阈值交叉来发射ON/OFF事件，更新参考值同时保留底层记忆模式，从而建立残差电压与后续事件时序的关联。

Result: FracEvent在多个数据集上改善了生成事件的时间结构，在图像重建和光流估计的下游迁移任务中取得了比现有模拟器更强的结果。

Conclusion: FracEvent通过分数阶弛豫电压动力学模型更准确地模拟了事件相机的像素级时间动态，显著提升了事件时序精度和下游任务迁移效果，证明了事件相机模拟的实用价值。
...
TL;DR: LayersReg是一种新型的3D/2D配准方法，通过逐层递进的回归范式解决外科导航中的实时配准问题。该方法赋予模型3D解剖感知能力，以迭代方式搜索正确姿态，在大偏移和多模态条件下达到高精度。


<details>
  <summary>Details</summary>
Motivation: 传统迭代优化算法在术中环境下效率低、失败率高。现有深度学习方法将配准问题重新表述为回归问题，实现了实时性能和精度的提升，但这些方法局限于特定姿态特征的记忆检索而非真正理解图像对齐任务本身，导致在复杂场景下泛化能力受限。

Method: 提出LayersReg，一种开创性的回归范式：1）赋予模型3D解剖感知能力；2）以逐层递进方式搜索正确姿态；3）在特征空间中搜索移动图像与固定图像之间的相关性，捕捉像素流趋势从而迭代收敛到正确空间姿态变换；4）设计节点级回归与渐进配准框架的耦合以增强模型对空间姿态变化的感知。

Result: 在大偏移和多模态条件下，X射线/CT配准达到0.68°、1.41mm，切片定位达到0.73°、1.55mm，优于现有最先进方法，同时满足术中精度和实时性要求。

Conclusion: LayersReg通过渐进式层搜索策略有效解决了3D/2D配准的精度和速度问题，为外科导航提供了一种有前景的实时高精度配准方案。
...
TL;DR: Disco-LoRA是一个统一的多概念视频定制框架，通过两阶段迭代解耦方法实现内容、风格和运动的可控生成，并提出Z-score统计正则化来协调不同LoRA权重分布。


<details>
  <summary>Details</summary>
Motivation: 现有T2V模型难以同时控制多个概念（内容、风格、运动），多概念视频定制是一个重要但尚未系统解决的研究问题。

Method: 提出迭代双LoRA解耦框架，将任务分解为内容-风格和内容-运动两个子任务分别处理；引入Z-score统计正则化对齐权重分布，保持层间趋势同时减少LoRA间干扰。

Result: Disco-LoRA在多概念视频定制任务上表现优异，能有效保留外观、风格和运动特征，实现可控的文本到视频生成。

Conclusion: 该工作为多概念视频定制提供了有效的统一框架和基准数据集，推动了T2V模型在多维度控制方面的发展。
...
TL;DR: 本文提出DeCoFlow，一种基于规范化流的持续异常检测方法，通过将子网络分解为冻结的通用基座和任务特定低秩适配器来解决灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 工业环境中新产品类别顺序到达，需要持续进行异常检测但无法访问历史数据。规范化流虽能提供精确密度估计，但在跨任务学习时会发生灾难性遗忘，参数更新会扭曲密度流形。

Method: 利用仿射耦合层保持变换有效性的内在特性，将子网络分解为冻结的通用基座和任务特定低秩适配器以隔离更新。引入任务特定对齐、辅助耦合层和尾部感知损失来补偿冻结基座的刚性。

Result: 在MVTec-AD上达到98.40%的图像级AUROC，在VisA上达到93.00%，同时实现参数级零遗忘(0.00% FM)，每任务仅需2.27M参数。

Conclusion: DeCoFlow成功在保持规范化流可逆性和Jacobian有效性的前提下，有效解决了持续学习中的灾难性遗忘问题，在工业异常检测任务上取得最先进性能。
...
TL;DR: PhysEditWorld是一个基于UE5引擎的多模态游戏数据集，专注于重力物理参数，通过重放范式记录同一场景在不同重力配置下的表现，包含12个UE5场景、100+小时游戏交互和6000万+渲染帧，用于可控世界建模研究。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型的交互行为受限，缺乏可控制的物理变量，无法应用于需要显式物理操控的创作环境；需要构建一个物理参数明确、可控可归因的数据集来解决这一问题。

Method: 基于UE5重放和渲染管道实现，在相同初始状态、角色控制器、动作序列和相机策略下记录多个重力配置的重放；提供RGB、深度、法线、音频、动作轨迹、相机轨迹、引擎状态、语义标注和显式重力标签等多模态信号。

Result: 构建了包含12个UE5场景、100+小时游戏交互和6000万+渲染帧的数据集；在生成视频模型和世界理解模型上验证了数据集的实用性，实现了更好的重力-faithful动力学建模和物理编辑下的一致性增强。

Conclusion: PhysEditWorld为可控世界建模研究提供了可扩展的基础，使物理动态从隐式相关变为显式可控变量，能够支持重力-faithful的动力学建模和物理编辑。
...
TL;DR: 提出了一个多任务框架，同时进行多标签分类、多类动脉瘤分割和多类血管分割，涵盖13个解剖位置和四种成像模态


<details>
  <summary>Details</summary>
Motivation: 现有自动方法仅限二元检测，缺乏细粒度解剖分类或多类分割，而动脉瘤破裂风险评估和治疗规划需要了解动脉瘤形态和解剖位置

Method: 两阶段方法：快速2D三轴ROI提取结合3D多任务nnU-Net主干网，双解码器设计缓解动脉瘤和血管类之间的极端体积不平衡，跨注意力池化和模态特定辅助头改善特征学习

Result: 在RSNA 2025颅内动脉瘤检测挑战赛中获得第2名

Conclusion: 该框架能够同时完成多标签分类、多类动脉瘤分割和多类血管分割，为动脉瘤破裂风险评估和治疗规划提供了更全面的工具
...
TL;DR: 本文提出M2C框架，将SAM3基础分割模型适配为医学图像的少样本自动标注器，无需外部模块或参数重训练。通过可学习概念嵌入和混合不确定性估计模块，实现自增强的标注循环。


<details>
  <summary>Details</summary>
Motivation: 将人类提示的基础分割模型转变为自动提示的标注器对于可扩展医学数据标注至关重要。现有方法依赖外部特征匹配器或辅助网络，带来架构开销且限制性能扩展。SAM3直接用于医学成像存在细粒度临床知识缺乏和人类描述歧义的问题。

Method: 提出Mask to Concept (M2C)框架：1)仅用少量标注图像初始化可学习概念嵌入；2)使用概念嵌入提示分割并通过最小化分割误差梯度更新嵌入；3)引入混合不确定性估计(HUE)模块计算预测熵并映射概念预测回边界框，测量概念-几何提示不一致性；4)标记高不确定性样本供人工修正，将修正掩膜反馈回M2C形成自增强标注循环。

Result: 在医学分割基准数据集上，该方法实现了SOTA少样本分割性能，具有出色的标注效率，为可扩展医学图像标注提供实用高效路径。

Conclusion: M2C无需外部模块、参数重训练或人工文本工程，即可在SAM3冻结架构内自动搜索可迁移视觉概念，完成医学少样本标注任务，显著降低专家标注工作量。
...
TL;DR: NeuMatEx是一种可微逆渲染方法,从图像中提取空间变化的神经材质。核心创新是训练大材质重建模型(LMRM)预测初始基色、神经材质潜在变量和不确定性引导,为后续逆路径追踪优化提供良好初始化和约束。


<details>
  <summary>Details</summary>
Motivation: 神经材质可以表示复杂的镜面反射和散射效果,但获取和创作此类材质具有挑战性。神经材质潜在空间的非线性结构使得使用朴素逆渲染进行优化不可行,需要新的方法来解决优化难题。

Method: 1)训练LMRM直接从图像预测初始基色、神经材质潜在变量和 aleatoric 不确定性引导;2)利用预测的不确定性将高置信度区域更紧密地锚定到LMRM预测,防止光照和复杂镜面效果被错误地烘焙到材质中;3)使用逆路径追踪进行优化。

Result: 在合成和真实资产上的实验表明,NeuMatEx 提取的复杂材质在视觉质量和材质分解方面均优于基于PBR的方法。

Conclusion: 该工作成功实现了从图像中提取神经材质,为神经材质的获取和创作提供了有效途径,有望推动图形学中神经材质表示的实用化发展。
...
TL;DR: 本文提出FUSE方法，通过频谱均衡策略生成全频段有效的不可学习样本，解决现有方法在低通滤波下失效的问题。


<details>
  <summary>Details</summary>
Motivation: 现有不可学习样本(UEs)存在关键缺陷：其有效扰动信号主要集中在高频成分，在低通滤波下会失效，无法提供可靠的数据保护。

Method: 提出全频段不可学习样本生成方法FUSE，包含两种策略：1) 随机频谱掩码(RSM)：训练时随机移除连续频段，迫使剩余频段维持不可学习性；2) 跨频段引导(CBG)：强制高低频分量之间的相互一致性，增强低频不可学习性并调节高频扰动以保持图像语义完整性。

Result: 在多个数据集、架构和频谱滤波下验证，FUSE实现了强大的全频段保护效果。

Conclusion: 可靠的不可学习样本应在整个频谱范围内保持有效，FUSE通过频谱均衡策略成功实现了这一目标。
...
TL;DR: 本文是首个全面分析开放词汇目标检测器（OV-ODs）在真实世界噪声下的鲁棒性的实证研究。通过逐层"剥开"模型分析特征崩塌现象，发现视觉骨干网络相似的模型表现出相近的鲁棒性，且主要受图像域影响而非标注策略。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测器（OV-ODs）因架构复杂其实在真实世界噪声下的鲁棒性表现如何目前理解有限，之前缺乏系统性的实证分析。

Method: 提出Robust Onion框架，使用受控的合成视觉退化逐层分析OV-ODs，揭示鲁棒性如何、在哪里退化，并系统性分析特征崩塌现象。同时提出轻量级plug-and-play方法NN & TK0。

Result: 相似视觉骨干的模型展现相似鲁棒性，源于相似层的特征崩塌；预训练策略、架构细节和标题监督贡献较小。鲁棒性主要由图像域决定而非标注，这解释了COCO和LVIS的相似鲁棒性影响，以及ODinW-13因大目标造成鲁棒性虚高的现象。在BDD100K、WiderFace、VisDRONE上验证了方法，效果提升显著且仅需端到端训练0.96%的参数量。

Conclusion: OV-ODs鲁棒性主要由视觉骨干和图像域决定，预训练和架构选择影响有限；轻量级NN & TK0方法可有效提升实际场景鲁棒性，为后续研究提供了改进方向和理论依据。
...
TL;DR: 本文提出3DLP基准测试，用于评估生成式图像编辑模型对真实世界光照的物理理解能力。通过构建包含1000对室内场景的新HDR数据集（光探针开/关状态），标注阴影和金属表面等区域，测试多个先进图像编辑模型。发现模型整体性能差异显著，在镜面高光区域表现稍好，但接收光较少的区域更易出错，VLM不适合像素级光传输分析。


<details>
  <summary>Details</summary>
Motivation: 生成式图像编辑模型虽已实现令人惊叹的视觉保真度，但尚不清楚这些系统是否具有对真实世界光照的内在理解。现有基准测试通常基于网络图像或合成数据，缺乏对物理正确光照传输的精细评估。

Method: 构建3DLP基准测试，创建新的高保真HDR数据集，包含1000对室内场景图像（光探针物理开关状态）。标注特定图像区域（投射阴影、金属表面等），使用两个新评分指标补偿AI生成的摄影效果（如白平衡调整），评估多个先进图像编辑模型，并测试VLM在光照传输分析中的适用性。

Result: 不同模型的整体性能差异显著，在镜面高光区域的差异稍小。最先进的图像编辑模型与真实世界物理一致性良好，但仍存在改进空间。所有模型在接收光较少的图像区域更容易出错。VLM不适合像素级光传输分析。

Conclusion: 3DLP基准测试和真实世界数据集的发布将推动该领域未来研究。模型需要改进对光照较少区域的渲染精度，VLM方法不适用于此任务。
...
TL;DR: 本文提出了一种新的流式视频编辑框架，通过三阶段蒸馏管道将强大的双向基础模型的编辑能力逐步转移到高效的单向流式编辑器，并引入AR导向的掩码缓存来重用跨帧的区域相关计算，实现因果化、逐帧编辑，在保持内容强保留的同时达到实时响应能力。


<details>
  <summary>Details</summary>
Motivation: 流式视频编辑面临两个核心问题：随时间保持稳定背景和非编辑区域，以及实现实时交互场景所需的低延迟。现有的流式视频生成方法主要用于合成，无法直接应用于编辑，因为编辑需要严格的保留要求和区域特定控制。

Method: 提出三阶段蒸馏管道，逐步将强大双向基础模型的编辑能力迁移到高效单向流式编辑器，实现稳定的长期编辑而不牺牲视觉质量；引入AR导向的掩码缓存，跨帧重用区域相关计算，大幅减少冗余处理并加速推理；建立专门的流式视频编辑基准。

Result: 该方法在流式基线中达到最先进的视觉质量，推理速度提升至12.66 FPS，适合交互式和增强现实应用。

Conclusion: 该框架实现了稳定的长期编辑，同时保持视觉保真度，使流式视频编辑适用于交互式和AR应用。
...
TL;DR: 本文提出了一种基于视觉-雷达融合的多模态点云生成方法，旨在解决毫米波雷达点云稀疏、噪声大、结构不完整的问题。


<details>
  <summary>Details</summary>
Motivation: 毫米波雷达点云通常稀疏、噪声大且结构不完整，直接影响目标检测与跟踪等下游感知任务的性能，需要一种方法来提升点云质量。

Method: 利用图像语义信息对雷达点云施加结构约束并实现空间对齐，同时引入稀疏补全策略增强点云密度并恢复缺失结构。

Result: 实验表明，该方法有效提升了点云质量，增强了感知模型在复杂环境下的检测精度和鲁棒性。

Conclusion: 为多传感器点云生成和智能感知系统提供了实用的解决方案。
...
TL;DR: 提出一种基于最优传输的3D Gaussian Splatting多视角风格化框架，通过列容量约束和跨视角匹配指导，实现保持跨视角一致性的3D风格化。


<details>
  <summary>Details</summary>
Motivation: 现有3D风格化方法对每个渲染视角独立应用2D特征匹配损失，导致风格分配不稳定、特征复用混乱以及跨视角一致性有限的问题。

Method: 1. 将局部风格匹配重新表述为半平衡最优传输问题
2. 引入可调强度的显式列容量约束
3. 引入跨视角匹配指导来约束场景内容与风格模式的对应关系
4. 添加几何正则化增强原始3DGS以表示更精细的纹理

Result: 显著提升多视角风格一致性，产生稳定且富有表现力的3D风格化，同时保留场景的核心语义结构。

Conclusion: 该框架通过最优传输原理为3DGS风格化提供了原则性的机制，能够在保持特征覆盖和风格多样性的同时实现稳定的跨视角对应。
...
TL;DR: ProtoKV是一种用于流式视频理解的恒定内存方法，通过保留近窗口KV缓存并将远历史聚合到语义空间原型库中，在查询延迟较长的情况下相比基线方法提升高达12.5个百分点的准确率。


<details>
  <summary>Details</summary>
Motivation: 流式视频理解面临异步查询和延迟查询的挑战，关键线索可能稍纵即逝，但后续更新会增加，导致这些线索在有限内存下被驱逐或稀释的风险。

Method: ProtoKV保留精确的近窗口KV缓存，将旧内容聚合到语义空间原型库中并附带残差统计。每个原型通过有界伪标记接口暴露，可与标准注意力机制无缝兼容。

Result: 在匹配的预算和相当的查询时间成本下，ProtoKV在长延迟场景的SVU基准测试中准确率提升高达12.5个百分点，且随着查询延迟增加，收益也随之增长。

Conclusion: ProtoKV通过原型库表示远历史而非保留标记实例，在恒定内存占用下有效解决了流式视频理解中的延迟查询问题，为视频理解系统提供了高效的记忆管理方案。
...
TL;DR: 提出了一种新的校准谐波叠加隐式神经表示(CHOIR)方法，用于多维数据恢复，解决了现有INR方法的优化不稳定性和频谱偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有使用周期激活函数的INR方法主要依赖函数复合，导致网络深度增加时优化不稳定，且缺乏适当的物理先验来有效缓解频谱偏差。

Method: 提出两个关键组件：1)协调谐波叠加(CHS)替代传统函数复合，确保优化稳定性；2)感知频谱校准(PSC)嵌入自然图像的幂律频谱先验，将固定频谱调整为物理合理的对数均匀分布。

Result: 在各种多维数据恢复问题上均取得了优于现有最先进方法的性能。

Conclusion: CHOIR方法有效解决了INR方法的优化不稳定性和频谱偏差问题，在多维数据恢复任务中展现出优越性能。
...
TL;DR: 提出了一种用于心脏电影MRI的4D可控生成框架,通过半监督变分自编码器和级联潜在扩散模型实现解剖结构和时间动态的分离,生成高质量合成4D序列用于数据增强,有效提升跨厂商分割模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 4D医学成像的AI模型开发受限于注释数据不足、设备间域偏移和隐私限制,需要一种可控的解剖学一致性数据增强方法来解决这些问题。

Method: 提出4D可控生成框架:1)半监督变分自编码器学习解剖体紧凑潜在表示并联合预测分割掩膜;2)级联潜在扩散模型将解剖结构与时间动态分离;3)静态LDM基于临床先验生成受试者特异性解剖;4)运动LDM估计残差潜在运动,确保4D序列时间一致性。

Result: 在心脏电影MRI上评估,静态解剖可控性Pearson r>0.8,时间一致性FVD=288.08。跨厂商泛化实验中,使用合成4D序列增强训练集显著提升分割性能:与仅用真实数据训练相比,nnU-Net平均Dice提升1.4%,Hausdorff距离减少3.0mm;左心室Dice提升2.8%,边界误差减少5.4mm。

Conclusion: 该框架为4D医学图像合成提供了可扩展且可控的解决方案,支持在有限注释和跨厂商可变性的情况下开发更鲁棒的模型。
...
TL;DR: 本文提出LearniBridge，一种基于可学习校准的特征缓存加速方法，用于加速Diffusion Transformers (DiTs)的图像和视频生成推理。该方法通过LoRA更新在多个时间步之间建立桥梁，仅需3-5个训练样本即可实现有效校准，在FLUX、HunyuanVideo和WAN2.1上分别实现最高5.87×、5.75×和4.10×加速。


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs)在图像和视频生成方面取得了显著进展，但存在 prohibitive computational costs（极高的计算成本）。现有的特征缓存加速方法依赖于历史特征来实现，导致在高加速比下出现严重的误差累积问题。

Method: 1. 研究特征校正的本质，发现最优校准更新具有跨不同prompt的共享低秩子空间结构
2. 提出LearniBridge，一种可学习的校准机制，通过轻量级LoRA更新在多个时间步之间建立桥梁
3. 该机制仅需3-5个训练样本即可实现有效校准

Result: 在FLUX、HunyuanVideo和WAN2.1上分别实现5.87×、5.75×和4.10×的加速效果。在WAN2.1上，相比之前的SOTA方法，在4.10×加速下VBench分数提升了1.28%。

Conclusion: LearniBridge通过揭示特征校正的低秩子空间结构，成功解决了高加速比下误差累积的难题，以极少的训练样本实现了高效的特征缓存校准，为扩散模型的推理加速提供了新的有效途径。
...
TL;DR: 本文提出了一种基于事件相机的主动视觉系统，用于实时跟踪和测量球类运动中的旋转（spin）。系统结合事件相机、高速万向节镜、可调焦距远摄镜头，采用2D事件检测与3D球定位的混合跟踪方法。离线方法s-CMax在静态球上达到2.1%幅值误差和4.0°轴向误差；在线方法在乒乓球比赛中达到8.8%幅值误差、6.4°轴向误差，延迟仅3ms，吞吐量750Hz。


<details>
  <summary>Details</summary>
Motivation: 在球类运动中，旋转对球的轨迹有重要影响，但传统相机由于球体积小、速度快、旋转迅速，难以准确估计球的旋转状态。

Method: 系统包括：1) 事件相机用于高时间分辨率和最小运动模糊；2) 高速pan/tilt galvanometer镜以保持球在视野内；3) 低延迟可调焦远摄镜头增加空间分辨率；4) 混合跟踪方法结合2D事件检测与3D球定位；5) 离线s-CMax方法进行对比度最大化；6) 在线方法使用不确定性感知CNN结合GPU加速的对比度最大化进行优化。

Result: 离线方法在静态球（乒乓球、棒球、网球、高尔夫）上达到最先进的精度，平均幅值误差2.1%，轴向误差4.0°。在线乒乓球应用中达到可靠跟踪和旋转估计，幅值误差8.8%，轴向误差6.4°，延迟3ms，吞吐量750Hz。

Conclusion: 所提出的事件基主动视觉系统能够实时准确地跟踪和测量球类旋转，在专业乒乓球比赛中表现出高精度和低延迟，具有广泛应用前景。
...
TL;DR: ReasonCLIP-58M是一个持续预训练框架，通过两阶段策略将大规模推理监督整合到CLIP风格模型中，同时保留描述性对齐，旨在提升视觉 grounded commonsense和组合推理能力。


<details>
  <summary>Details</summary>
Motivation: CLIP及其变体虽然在多模态系统中广泛使用，但其预训练仍以描述性图像-文本对齐为主。随着下游应用对视觉 grounded commonsense推理和组合推理的需求增加，不清楚CLIP风格编码器是否能在不改变架构的情况下支持此类推理。

Method: 提出两阶段策略：1）渐进式整合推理信号同时保留描述性对齐；2）类别结构化推理监督。构建两个互补数据集：ReasonLite-42M（开放形式视觉可验证推理 caption）和ReasonPro-16M（类别特定推理监督），以及评估基准RCLIP-Bench。

Result: 训练的一系列ReasonCLIP模型在视觉 grounded commonsense和组合推理方面取得改进，同时提升了零样本检索性能。作為多模态大语言模型（如LLaVA-NeXT）的即插即用视觉编码器，在不增加推理成本的情况下实现一致的性能提升。

Conclusion: 结构化推理监督增强了CLIP风格视觉表示的表达能力，表明无需架构变更即可提升推理能力。所有数据集、模型和训练代码均已开源。
...
TL;DR: 本文提出NaviCache，一种用于视频扩散模型(VDM)的即插即用测试时自校准方法，将特征演化重新概念化为惯性导航系统(INS)问题，以解决离线校准方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VDM加速方法面临两大困境：1) 离线校准方法存在校准数据依赖、校准时间长、易受分布偏移影响等问题；2) 离线无校准方法依赖瞬时零阶近似，受观测噪声影响且忽略扩散轨迹内在动量。

Method: 提出将特征演化建模为INS问题的框架，引入双状态估计架构自适应追踪特征变化比及其潜在漂移，通过专用初始对齐阶段初始化，并整合时间依赖噪声调度与不确定性感知测量更新机制。

Result: 在HunyuanVideo、Wan和Open-Sora系列模型上进行了广泛实验，NaviCache展现出更准确的计算跳过误差判断能力，达到了出色的综合性能。

Conclusion: NaviCache为VDM提供了一种理论上合理的误差有界计算跳过机制，有效解决了现有方法的各种局限性。
...
TL;DR: 针对恶劣天气条件下的多模态图像融合问题，提出一种掩码引导的MMIF方法，通过引入“伪真实值”和掩码引导的跨模态交叉注意力机制，实现特征恢复与交互的平衡。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气会导致多模态图像融合中显著的图像退化，破坏特征表示，需要同时进行特征恢复和跨模态互补。现有的方法在这种情况下难以实现有效的表示学习，限制了其实际性能。

Method: 1) 引入“伪真实值”简化训练，促进更快更有效的特征学习；2) 设计基于融合结果与源图像映射关系的掩码生成机制，量化各模态的相对贡献；3) 提出掩码引导的跨模态交叉注意力机制，选择性关注信息特征；4) 提出掩码引导学习策略和任务耦合的退化感知学习策略。

Result: 在合成和真实数据集上的广泛实验表明，该方法在视觉质量、定量指标和下游任务方面均优于现有最先进的方法。

Conclusion: 该方法通过掩码引导机制有效解决了恶劣天气条件下多模态图像融合的特征恢复与跨模态互补平衡问题，为复杂环境下的图像融合提供了新的思路。
...
TL;DR: 本文针对地面农业监测中大蒜幼苗在复杂户外光照条件下的检测问题，提出了一种基于对抗性增强策略学习的 illumination-robust 幼苗检测框架，并在新构建的大蒜幼苗数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 1) 现有研究多在稳定成像条件下(UAV或温室)评估，地面户外监测中严重且空间异质的光照下鲁棒检测研究不足；2) 许多光照鲁棒检测方法依赖额外增强或特征提取模块，增加推理时间开销；3) 现有方法未针对幼苗检测和下游缺苗定位任务进行优化。

Method: 1) 构建了新的地面平台大蒜幼苗数据集(真实户外田间条件，高度可变光照)；2) 提出基于对抗性增强策略学习的光照鲁棒幼苗检测框架；3) 联合优化随机增强策略代理和目标检测器，使检测器学习鲁棒表示；4) 引入结构化惩罚防止不现实扭曲同时鼓励训练中挑战性增强。

Result: AP₅₀达91.6%，比基线提升0.9个百分点，比之前最优方法提升0.2个百分点；下游缺苗定位：精确率75.0%，F1分数67.0%，比基线分别提升4.8和2.0个百分点。

Conclusion: 该框架在复杂户外光照条件下实现有效的大规模地面农业监测，且无需额外推理时间计算开销。
...
TL;DR: AnomNOVIC是一种无需提示词的开放词汇目标检测框架，结合 masked autoencoder (MAE) 异常检测与 NOVIC 开放词汇分类器，实现机器人无需预定义类别列表即可识别未知物体。


<details>
  <summary>Details</summary>
Motivation: 机器人需要在真实环境中识别前所未见的物体，现有的开放词汇检测器缺乏高效且无需提示词的解决方案，无法满足持续部署需求。

Method: 提出两阶段已知工作空间框架：1) MAE 生成通用物体无关的边界框，2) NOVIC 对显著区域进行实时无提示词分类。在 NICOL 人形机器人桌面环境中评估，并在包含48种独特物体的野外测试集上验证。

Result: 在桌面机器人环境中达到 47.1% AP / 57.5% AP50（无提示词）和 59.0% AP / 72.5% AP50（有候选类别）。在野外测试集上达到 82.6% 无提示词检测分类准确率。显著超越 YOLO-World-v2、OWLv2、YOLOE 等基线方法。

Conclusion: AnomNOVIC 成功实现无需预定义类别列表的开放词汇目标检测，在机器人场景和野外环境中均展现出优异性能，为开放世界机器人自主导航提供有效解决方案。
...
TL;DR: 本文提出了一种名为LFNet的通用显著目标检测方法，通过协调状态空间模型(SSM)和卷积神经网络(CNN)之间的频谱偏差来实现高性能检测。该方法利用液态神经网络的动态信息传播机制，创新性地融合了VMamba和ConvNeXt两种骨干网络的特征，并通过显著性引导上采样操作符提升特征传播效果。


<details>
  <summary>Details</summary>
Motivation: 当前显著目标检测方法忽视了不同神经网络范式（SSM和CNN）固有的频谱偏差问题。通过对数据集进行频谱分析，发现这两种网络的语义表示在频率偏好上具有天然的互补性，这为提升检测性能提供了新的研究方向。

Method: 1）进行数据集级频谱分析，揭示CNN和SSM的互补频率偏好；2）提出液态融合网络（LFNet），将VMamba特征视为演化状态，ConvNeXt特征视为外部刺激，采用动态门控机制实现内容感知的特征聚合；3）设计显著性引导上采样（SGU）算子，采用频谱-空间协同设计抑制上采样伪影并保留语义信息。

Result: 在五种不同任务（RGB、RGB-D、RGB-T、VSOD和VDT）上进行了广泛实验，LFNet实现了最先进的性能表现，在检测精度和模型效率之间提供了优越的权衡。

Conclusion: 通过桥接SSM和CNN之间的频谱偏差，LFNet成功实现了通用显著目标检测的高性能。该方法证明了频谱信息在融合异构神经网络表示中的重要价值，为未来多模态显著目标检测研究提供了新的思路。
...
TL;DR: 论文提出了一种实用的滚动快门(RS)相对位姿估计方法，通过引入仿射对应(ACs)和RS校正的仿射约束，仅需7个ACs即可在1.2ms内估计位姿和RS运动参数。


<details>
  <summary>Details</summary>
Motivation: 消费级设备普遍配备RS相机，但现有RS相对位姿估计方法需要至少20个点对应，导致基于RANSAC的鲁棒估计因迭代次数指数依赖而成本过高变得不实用。

Method: 推导了考虑点扰动与行相关本质矩阵耦合的RS校正仿射约束，利用RS参数物理规模小线性化约束，通过零空间投影消除12个RS未知量，最后用动作矩阵求解剩余20次系统。

Result: 在TUM RS基准上达到最佳位姿和RS参数精度，是唯一能提供准确平移速度估计的RS求解器；在全局快门EuRoC MAV数据集上达到与标准5点算法相当的精度。

Conclusion: 该方法首次使RS相对位姿估计变得实用，在RS和GS设置下均表现出色，为消费级设备视觉里程计算法发展奠定了基础。
...
TL;DR: 现有Flow-GRPO方法使用整体图像奖励，无法区分不同空间区域的贡献，导致细粒度编辑优化困难。


<details>
  <summary>Details</summary>
Motivation: 解决图像编辑中的空间均匀性假设问题——整体图像奖励无法区分不同空间区域对图像质量的贡献，阻碍了细粒度编辑优化。

Method: 提出SpatialFlow-GRPO框架：1)引入空间细粒度奖励反馈；2)将区域感知奖励转换为语义区域级优化信号；3)在策略更新中使区域优势与潜在位置对齐。训练区域感知奖励模型SFReward，构建SFReward-14K数据集，引入MultiEditBench评估基准。

Result: 在OmniGen2和FLUX.2-klein-4B上，SpatialFlow-GRPO在GEdit-Bench、ImgEdit-Bench和MultiEditBench上优于Flow-GRPO。

Conclusion: SpatialFlow-GRPO能够将局部反馈转换为空间对齐的更新信号，有效提高图像编辑质量。
...
TL;DR: 本论文提出了一种基于RIS（可重构智能表面）辅助的毫米波网络 proactive handover（PHO）优化方法，通过优化RIS元素数量来平衡信号处理复杂性与链路质量，并在 handover 时间约束下实现节能。


<details>
  <summary>Details</summary>
Motivation: 毫米波网络依赖视距传播，易受障碍物阻挡。尽管视觉辅助无线通信可实现主动切换，但当附近无基站时需依靠RIS恢复连接。然而大量RIS元素的配置时间限制了响应速度，因此需要在保证及时切换的同时优化RIS配置。

Method: 采用粒子群优化（PSO）算法离线求解最优RIS链路配置，优化目标是平衡信号处理复杂度和链路质量，通过调整分配给链路的RIS元素数量来满足 handover 时间约束并降低能耗。

Result: 减少12%的RIS元素可降低10%的能耗，同时保持信噪比不变；在受阻区域实现15-30 dB的链路质量提升，并维持准确的切换时间性能。

Conclusion: 该方法通过RIS元素数量优化实现了能耗与性能的平衡，为毫米波网络在视距受阻场景下提供了一种可行的主动切换解决方案，具有重要的实际应用价值。
...
TL;DR: OTF-CBM是一种新型的概念瓶颈模型，通过将概念对齐重新定义为动态跨模态传输过程来实现可解释的视觉-语言推理。该方法使用逆最优传输学习语义成本，并通过基于不平衡最优传输的流匹配来建模视觉patch与文本概念之间的语义转换。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言CBMs依赖于预对齐编码器或全局余弦相似性，这掩盖了细粒度概念定位并无法反映真实语义几何。现有方法将概念对齐视为静态投影，而非动态传输过程。

Method: 1) 使用逆最优传输学习数据驱动的语义成本来测量跨模态距离；2) 执行基于不平衡最优传输的流匹配来建模语义转换；3) 使用基于速度的概念激活来捕获可解释的几何关系，无需ODE积分。

Result: OTF-CBM实现了卓越的分类准确性和概念保真度，为可解释的跨模态推理提供了新的几何和动力学视角。

Conclusion: 该研究提出了一种创新的概念对齐范式，将静态投影转变为动态传输过程，为可解释的视觉-语言推理开辟了新的几何方法。
...
TL;DR: MICViT是一种用于脑MRI分析的3D视觉Transformer，通过四种注意力机制（模态特定局部/全局注意力和跨模态局部/全局注意力）来学习模态特定表示和跨模态交互，在三个数据集上实现脑年龄预测的最优性能。


<details>
  <summary>Details</summary>
Motivation: 脑MRI分析面临高维度3D数据、有限样本量以及多模态数据整合困难的核心挑战，需要一种能够有效融合T1、FLAIR、DWI、SWI等多模态MRI信息的方法。

Method: 提出MICViT框架，包含四种注意力机制：模态特定局部注意力和全局注意力用于模态内特征学习，跨模态局部注意力和全局注意力用于捕捉模态间交互。采用3D vision transformer架构处理3D脑MRI数据。

Result: MICViT在三个异质数据集（UK Biobank n=41404, SOOP n=1062, Cam-CAN n=613）上 consistently outperforms CNN和Transformer基线方法，并且从多模态输入中获益更多，添加更多模态时性能提升更大。
...
TL;DR: 本文提出了一种合成数据增强的框架，用于自动分割猕猴追踪组织学中的纤维束。该方法利用离体扩散MRI tractography作为生成先验，合成2D图像 patches进行训练，结合块面照片作为背景，并通过域随机化进行数据增强。训练2D U-Net后，在保留的大脑上展现出良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 扩散MRI tractography的精度受限于间接、低分辨率的轴突组织测量。 tracer注射研究在非人类灵长类动物中提供了验证dMRI tractography的黄金标准，但这需要在组织学切片中对手工纤维束进行耗时的人工标注。因此需要开发自动化分割方法来减少人工标注工作量。

Method: 提出合成数据增强框架：(1)利用离体dMRI tractography作为生成先验，合成2D图像patches用于训练，提供足够逼真的前景纹理；(2)将合成前景与块面照片背景合成；(3)通过域随机化进行数据多样化；(4)在混合真实和合成patches上训练2D U-Net。

Result: 实验表明：(1)与仅使用真实数据训练相比，泛化能力得到改善，能够更好地泛化到不同大脑和纤维束密度；(2)仅使用合成数据训练导致性能不佳，凸显了真实监督的必要性；(3)达到与最先进方法相当的性能，同时需要的标注数据减少了3倍。

Conclusion: 合成数据增强框架能够有效减少人工标注工作量，同时保持与最先进方法相当的性能。真实数据和合成数据的结合是必要的，单纯合成数据无法达到最佳性能。
...
TL;DR: 视频推理模型在面对运动模糊、眩光、遮挡等现实干扰时存在"盲目信任"问题，准确率下降15-30%。本文提出Robust-TO框架，通过引入每帧可信度评分来整合异构视觉感知工具，在推理过程中采用三层证据合成机制（高/中/低），并在奖励函数中联合优化正确性、证据可靠性和效率。


<details>
  <summary>Details</summary>
Motivation: 视频推理语言模型隐式假设每个输入帧同等可靠，但在面对运动模糊、眩光、遮挡等现实干扰时，准确率大幅下降15-30%，且模型无法意识到视觉证据已退化。这严重影响模型在真实场景中的应用。

Method: Robust-TO框架包含三个核心组件：1）统一证据接口：异构视觉感知工具接收子查询和可信帧集合，返回结构化证据（预测结果、时间定位、可信度评分）；2）可信度-相关性评分：选择可靠帧；3）三层证据合成：根据可信度评分引导证据加权；4）置信度-成本GRPO奖励：联合优化正确性、证据可靠性和效率。

Result: 在两个视频推理基准的八个任务上，Clean输入达56.4%平均准确率，比最强开源基线高10.6%，超越Gemini-2.5-Pro（46.2%）。在五种现实 corruption类型下，保持54.3%准确率，比最强开源基线高5.8%，且clean-to-corrupted准确率降幅最小。

Conclusion: Robust-TO通过引入每帧可信度评分机制，有效解决了视频推理模型的"盲目信任"问题，在干净和干扰环境下均取得显著性能提升，为构建更鲁棒的 embodied AI 系统提供了新思路。
...
TL;DR: 本文提出了Qwen-Image-Agent框架，解决文本到图像模型在处理现实世界中未明确说明、隐含或依赖最新知识的用户请求时面临的"上下文差距"问题。


<details>
  <summary>Details</summary>
Motivation: 文本到图像(T2I)模型在处理现实世界请求时存在"上下文差距"——用户上下文与生成所需的充分上下文之间的不匹配，因为用户请求往往是未明确说明的、隐含的或依赖最新知识的。

Method: 提出了统一的智能体框架Qwen-Image-Agent，以上下文为中心的方式整合计划、推理、搜索、记忆和反馈。该框架通过上下文感知规划(Context-Aware Planning)识别缺失的上下文并规划如何获取和使用它，以及上下文接地(Context Grounding)从推理、搜索、记忆和反馈中收集上下文。

Result: 在IA-Bench、Mindbench和WISE-Verified上的实验表明，Qwen-Image-Agent优于强基线模型并实现了最先进的性能。

Conclusion: Qwen-Image-Agent通过上下文感知规划和上下文接地有效弥合了上下文差距，为智能图像生成提供了新的研究方向。
...
TL;DR: PhysRAG是一个通过检索增强生成(RAG)技术提升视频生成模型物理感知能力的新 pipeline，旨在解决现有模型难以捕捉热力学、力学和光学等多样化物理现象的问题。


<details>
  <summary>Details</summary>
Motivation: 开发具有物理感知能力的视频生成模型面临重大挑战，原因在于难以捕捉热动力学、力学和光学等多样化的物理现象，同时缺乏高质量的训练数据。

Method: 1) 设计了两阶段数据过滤pipeline，基于WISA-80K数据集筛选出7K高质量视频；2) 构建物理视频数据库；3) 开发可学习查询机制将物理知识注入视频扩散模型；4) 在PhyGenBench和VBench等基准上评估模型性能。

Result: 该方法在视觉质量和物理规则遵循方面均达到最先进水平，超越了现有模型。通过消融实验验证了数据过滤pipeline、RAG机制和物理信息提取方法的有效性。

Conclusion: PhysRAG通过RAG技术成功提升了视频生成模型的物理感知能力，为物理感知视频生成研究提供了新的思路和数据资源。
...
TL;DR: PortraitGen 是一个基于 GRPO 的新型框架，通过引入真实图像和双奖励机制，显著提升了人像 photorealistic portrait generation，有效抑制 AI 伪影。


<details>
  <summary>Details</summary>
Motivation: 当前 GRPO 方法偏向 superficial aesthetics（如过度饱和色彩），未能解决 AI 伪影和生物不合理性问题，原因是缺乏真实图像引导和针对细粒度伪影的奖励。

Method: 提出 PortraitGen 框架：1) 通过图像反转将真实图像引入 GRPO 采样组，打破生成边界；2) 引入双奖励机制（OmniReward 和 AI-Portrait）；3) 推出 PortraitBench 基准测试。

Result: PortraitGen 显著优于现有基线，有效抑制 AI 伪影，达到前所未有的 photorealism。

Conclusion: 通过引入真实图像和双奖励机制，PortraitGen 为 photorealistic portrait generation 设立了新标准。
...
TL;DR: 针对扩散模型中MoE架构的路由分配问题，提出SharpMoE框架，利用干净潜在特征作为无噪声引导信号进行路由，并引入轨迹路由损失约束多步去噪轨迹的计算分配。


<details>
  <summary>Details</summary>
Motivation: 现有扩散MoE框架中，路由器依赖噪声污染的潜在特征，无法准确区分显著token并分配计算资源，因为随机噪声掩盖了关键的结构和纹理信息。

Method: SharpMoE使用干净潜在特征作为无噪声引导信号进行路由，绕过噪声扭曲的输入；同时引入轨迹路由损失约束整个多步去噪轨迹的计算分配。

Result: SharpMoE作为通用即插即用解决方案，进一步增强了预训练收敛的MoE模型，在视觉生成中达到最优性能。

Conclusion: 通过利用干净潜在特征提供清晰的显著性引导，SharpMoE使路由器即使在高噪声阶段也能有效识别显著token，为扩散MoE系统的路由改进提供了新思路。
...
TL;DR: 针对帕金森病面部表情质量评估的现有方法仅输出严重程度分数，缺乏可解释性问题，提出TraMP-LLaMA统一多模态框架，整合RGB外观和关键点轨迹线索，联合预测严重程度分数并生成结构化文本报告。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情质量评估方法只能产生严重程度分数，无法表达支持预测的面部运动证据，限制了模型输出的可解释性，难以在帕金森病评估中检查预测依据。

Method: 提出TraMP-LLaMA框架，整合RGB外观和关键点轨迹线索，采用解耦指令调优策略减少严重程度预测和语言生成之间的任务干扰。在PFED5数据集基础上扩展构建PFED5-plus数据集。

Result: 在PFED5-plus数据集上，TraMP-LLaMA在报告生成任务上优于竞争性视频语言基线方法，在联合多表情训练下达到最佳严重程度预测性能，Spearman等级相关系数比所有竞争方法提高至少4.39%。

Conclusion: TraMP-LLaMA框架能够同时实现可解释的文本报告生成和准确的严重程度预测，为帕金森病面部表情评估提供了有效工具。
...
TL;DR: 本文提出了OmniRef-Bench基准测试和DyRef训练框架，用于解决多参考图像生成(MRIG)任务。OmniRef-Bench覆盖了复杂的参考图像类型组合，而DyRef通过两阶段训练（监督微调+难度感知优势重加权+判别性奖励缩放）显著提升了开源模型在复杂MRIG场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 多参考图像生成是一个具有挑战性的任务，现有的基准测试无法充分评估复杂的MRIG场景，这阻碍了该领域的进一步发展。主流入源模型在复杂MRIG场景中表现不佳，且随着混合类型参考图像数量的增加，性能显著下降。

Method: 提出OmniRef-Bench基准测试和DyRef两阶段训练框架。第一阶段通过监督微调使模型具备处理复杂MRIG任务的基本能力。第二阶段引入难度感知优势重加权(DAR)和判别性奖励缩放(DRS)：DAR动态调整优化目标以处理大量混合类型参考图像；DRS扩大组内奖励差异以实现更有效的策略优化。

Result: 在OmniRef-Bench上的评估表明，主流入源模型在复杂MRIG场景中表现不佳，性能随混合类型参考图像数量增加而显著下降。DyRef显著提升了开源模型在OmniRef-Bench和单图像编辑基准测试上的性能。

Conclusion: DyRef框架能有效解决多参考图像生成任务中的挑战，具有良好的有效性和泛化能力。
...
TL;DR: 本文提出了Dota2-Vis，一个基于视频的电竞数据集,用于分析职业Dota2比赛中的可见性问题。该数据集包含2025年国际邀请赛的144场比赛(共288个全高清视频),并提供了2,477张手动标注的小地图图像。研究评估了YOLO系列目标检测器在小地图玩家图标检测上的性能,并分析了对手可见的玩家存在模式。


<details>
  <summary>Details</summary>
Motivation: 现有MOBA(多人在线竞技场)游戏分析主要依赖结构化数据,但这类数据无法直接反映比赛中各队伍实际能看到的内容,即视野信息对于理解比赛策略至关重要但传统数据难以捕捉。

Method: 1)构建数据集:收集TI2025的144场比赛,每场从双方视角录制,共288个全高清视频,并手动标注2,477张小地图图像;2)评估YOLO11l等多个目标检测器变体进行玩家图标检测;3)使用性能最优的模型估计对手可见的玩家存在随时间变化情况。

Result: YOLO11l(大型版本)在密集和视觉混乱的小地图场景中表现最佳,能可靠识别玩家图标。生成的可见性曲线揭示了玩家、英雄、角色和团队层面的模式,这些信息是传统结构化数据难以获取的,为MOBA分析提供了补充视角。

Conclusion: Dota2-Vis数据集和基线管道为MOBA游戏的视野分析提供了有效工具,YOLO11l模型可准确检测小地图玩家图标。该研究填补了视频基MOBA分析方法的空白,公开的数据集和代码将促进该领域进一步研究。
...
TL;DR: 本文提出GGR框架，通过梯度级控制解决开集半监督学习中的权衡问题——传统方法在过滤可疑样本与利用未标注数据之间存在矛盾，GGR将监督梯度作为锚点，将冲突的辅助梯度投影到允许区域，实现一阶非对抗更新。


<details>
  <summary>Details</summary>
Motivation: 现有OSSL方法面临两难：激进取样可能丢弃信息丰富的困难ID样本，而利用未标注数据时错误伪标签会引入与监督学习冲突的梯度。现有的样本过滤或软加权范式无法从根本上解决这个矛盾。

Method: 提出几何梯度 rectification（GGR）插件框架，使用监督梯度作为锚点，将冲突的辅助梯度投影到允许区域，使更新在一阶梯度上非对抗，同时保留可能仍有用的正交分量。还扩展了子空间感知的rectification以稳定锚点。

Result: 在CIFAR和ImageNet基准测试上，GGR提升了代表性OSSL基线模型在大多数设置下的性能，同时在闭集泛化和开集鲁棒性两方面都获得了增益。

Conclusion: 从样本选择转向梯度级控制是解决OSSL中挑战的有效范式，GGR框架具有通用性，可作为插件应用于多种OSSL方法。
...
TL;DR: Unison是一个综合基准测试，用于评估统一多模态模型的联合理解和生成能力，包含2169个高质量样本。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型的评估通常将理解和生成能力分开评估，忽视了两者之间的协同作用。

Method: 构建了Unison基准测试，提供三个关键优势：1)综合维度(内部一致性、理解引导生成、生成引导理解、相互增强)；2)诊断评估(统一和分离轨道)；3)人类一致性(Unison-Judge评估模型)。

Result: 通过对最先进模型的系统评估，揭示了当前统一多模态系统的关键限制。

Conclusion: 为统一多模态模型的未来研究指明了有前景的方向。
...
TL;DR: 本文提出EVIS，一种用于指代视频分割的方法。通过可学习的Event Query将视频分解为多个简单事件，实现逐事件层级化理解，并提出Object-Pixel-Hybrid Learning帮助多模态大语言模型在长视频中跟踪目标。


<details>
  <summary>Details</summary>
Motivation: 现有指代视频分割方法将视频视为包含多张图像的单一事件，忽略了视频通常包含多个独立事件的事实。这导致模型需要直接理解视频和文本中的所有复杂内容，容易产生混淆和幻觉问题。

Method: 1.使用可学习的Event Query将视频分解为多个简单事件；2.利用文本引导的Event Queries划分视频并提取事件感知的视觉-文本特征；3.提出Object-Pixel-Hybrid Learning，将细粒度像素特征与先验对象查询相结合，实现长视频目标跟踪。

Result: 在5个公开基准数据集上的广泛实验证明了EVIS在指代视频分割任务上的优异性能。

Conclusion: EVIS通过逐事件、易理解的方式处理复杂视频内容，有效避免了混淆和幻觉问题，在指代视频分割任务上取得了strong performance。
...
TL;DR: 本文提出了一种基于ResNet (RSFM) + FPN的无监督异常检测方法，用于遥感变化检测。该方法利用未经训练的FPN架构及其内在先验，通过检测连续轨道观测之间潜在空间中的微妙语义变化来识别各种异常，无需标注数据或专门训练。


<details>
  <summary>Details</summary>
Motivation: 遥感基础模型(RSFMs)作为监督模型的替代方案出现，能够使卫星在检测到异常时自主触发高分辨率采集或调整任务参数，从而最大化任务的有限功率和计算资源利用。无监督变化检测为灾害监测提供了一种无需昂贵标签的变革性路径。

Method: 提出了一种基于ResNet (RSFM) + FPN的新型无监督检测方法。该方法通过检测连续轨道观测之间潜在空间中的微妙语义变化来识别广泛的异常谱系。系统依赖于未经训练的FPN架构及其内在先验，实现了高效的全图像级生成和更高分辨率映射，与之前的方法（基于补丁的、需训练的）相比，只需最少量工作（无需训练）。

Result: 通过用RSFM替代定制模型，该方法无需专门的训练和广泛的开发工作即可实现相当的结果，同时确保了跨不同地形和传感器的高性能泛化能力。

Conclusion: 该方法提供了一种高效的训练自由方法来实现遥感异常检测，在保持高性能泛化的同时简化了工作流程，为灾害监测等应用提供了实用的解决方案。
...
TL;DR: PanoImager是一种无需SfM的稀疏全景图像3D重建框架，通过结合姿态/深度先验、扩散视图补全和深度引导的3DGS优化，在极端稀疏条件下实现稳定的3D重建。


<details>
  <summary>Details</summary>
Motivation: 在旋转主导、弱视差运动条件下，传统SfM/SLAM初始化常常病态且不可靠，导致稀疏全景图像的3D重建面临挑战。

Method: 将全景图像分解为局部透视视图，利用几何条件扩散模型合成辅助观察以丰富稀疏证据，并结合深度引导的3DGaussian Splatting优化来稳定跨视图一致性。

Result: 在多个基准数据集上展现出更好的极端稀疏条件下的稳定性。

Conclusion: 可作为SfM/SLAM初始化失败时的离线/后台地图细化组件，提升3D重建系统的鲁棒性。
...
TL;DR: 提出CT-3GDINO，一种基于伪文本条件的轻量级3D器官检测器，用于腹部CT影像中肝、脾、左肾、右肾和肠的定位任务。


<details>
  <summary>Details</summary>
Motivation: 可靠的器官定位可以为下游创伤分析提供空间先验信息，现需要针对固定器官定位的轻量级3D检测器。

Method: 结合Swin3D视觉骨干网、双向特征增强、伪文本引导的查询选择和跨模态解码器，使用冻结的伪文本类别token代替真实文本编码器，在193例RSNA/RATIC CT数据上训练评估。

Result: 最佳多尺度模型达到0.5830的整体top-1类-wise mAP (IoU 0.1-0.7)，优于固定和可训练骨干网的分类预训练变体(0.5570和0.4657)。在IoU 0.1时AP为0.9649，但在IoU 0.7时仅为0.1552。

Conclusion: CT-3GDINO作为伪文本条件3D器官定位的开源基线表现良好，但粗定位能力强，严格对齐定位仍有待提升。
...
TL;DR: 本文提出SubdivAR，一个基于Mesh Autoregressive Representation (MAR)的新型神经网格细分框架，通过将不同细分层次的网格组织成有序尺度序列，采用自回归下一尺度预测范式来改进网格细节合成。


<details>
  <summary>Details</summary>
Motivation: 传统规则基础网格细分方法依赖固定局部细化规则，常产生过度平滑表面；现有神经细分方法受限于局部建模，泛化能力有限。需要一种能保留细分拓扑同时恢复精细几何细节的新方法。

Method: 提出MAR将不同细分层次网格组织为有序尺度序列，引入Hybrid Topology-Aware Transformer结合全局语义注意力与拓扑约束的局部特征聚合，采用下一尺度坐标预测范式回归顶点偏移以保留拓扑并恢复细节。构建FII-40K数据集含近40000个多层次细分监督的高质量网格。

Result: SubdivAR优于现有最先进方法，Hausdorff距离降低18.8%，Chamfer距离降低14.2%，对复杂开放表面几何具有强鲁棒性。

Conclusion: 通过自回归下一尺度预测范式和混合拓扑感知Transformer，SubdivAR成功解决了传统方法和现有神经方法在细节保留与泛化方面的局限，为数字资产生成中的网格细分提供了更优解决方案。
...
TL;DR: 本文提出TMP树结构混合策略剪枝框架，用于压缩大型图像生成模型。将HunyuanImage 3.0从80B压缩到20B（75%压缩率），Z-Image turbo从6B压缩到4B（33%压缩率），同时保持可接受的生成质量，并实现单卡推理。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成模型参数规模快速增长，导致计算资源需求与GPU内存占用日益增大，使大型模型难以实际应用，需要有效的模型压缩方法。

Method: 提出TMP框架，这是首个树结构混合策略剪枝框架，可泛化到多种图像任务（T2I和TI2I）和架构（MoE和DiT）。通过工程技能优化使剪枝后模型能在单卡上运行。

Result: 将HunyuanImage 3.0从80B压缩到20B（75%压缩率），生成质量损失有限；Z-Image turbo从6B压缩到4B（33%压缩率），质量下降可忽略。剪枝后20B版本可在单张24GB 4090 GPU上运行推理。

Conclusion: TMP框架证明了树结构混合策略剪枝在大型图像生成模型压缩中的有效性，为降低模型部署门槛提供了实用方案。
...
TL;DR: FlameVQA是一个基于无人机的火灾监测多选视觉问答基准数据集，结合RGB和热红外图像，支持温度感知的火灾关键推理。


<details>
  <summary>Details</summary>
Motivation: 无人机 wildfire监测中，烟雾、尺度变化和遮挡常限制RGB-only的解释，需要跨模态推理能力来增强火灾情报获取的安全性和可靠性。

Method: 基于FLAME 3数据集构建FlameVQA，每幅图像包含34道多选问题，涵盖六个操作能力组（检测、定位、分布/覆盖估计、跨模态推理、飞行规划等）。采用MLLM辅助标注结合确定性热规则和交叉一致性检查，最后进行人工审计验证。

Result: 在提供明确跨模态线索时表现良好，但在浓烟下的存在检测和覆盖估计方面存在明显失败。当前MLLM需要领域特定适应才能更好支持灾难和火灾监测。

Conclusion: 数据集和基准代码已开源，为未来火灾监测MLLM研究提供基础baseline，表明当前模型在安全关键火灾推理任务中仍需改进。
...
TL;DR: 本文提出了一种用于自回归统一多模态模型的安全增强方法——迭代自改进码本。该方法利用模型自身的理解和判断能力来识别不安全图像，无需人工标注，即可消除有害映射并生成安全图像。


<details>
  <summary>Details</summary>
Motivation: 自回归统一多模态模型通过预测离散视觉标记来生成图像，但可能产生不安全内容。如何在无需外部反馈的情况下提升模型安全性是一个重要问题。

Method: 方法包含两步：(1) 使用统一模型识别不安全生成，构建有害-安全图像-文本对，构建有害空间并更新码本以消除有害输出；(2) 在无害空间中使用安全图像-文本对进行自适应微调，确保生成质量。两步重复直到无改进。

Result: 无需额外外部反馈，模型安全性通过迭代自改进不断提升，最终得到安全增强的模型码本。

Conclusion: 该方法利用模型自身能力实现安全性的迭代提升，为自回归多模态模型的安全部署提供了有效途径。
...
TL;DR: HarmVideoBench是一个用于评估大视觉语言模型(LVLMs)理解有害视频能力的多层次诊断基准,包含1,379个视频和4,137个多项选择题,涵盖三个层级维度:可观察证据、片段内部含义和超出片段推理。同时提出BCR方法,可在有害视频理解任务中显著提升模型性能(从61.7%提升至84.4%)。


<details>
  <summary>Details</summary>
Motivation: 现有有害视频基准存在两个主要局限:1)忽视有害视频的多层特征,仅将评估形式化为二元分类任务,无法捕捉隐含或深层上下文危害;2)完全缺乏解释性理由,当前框架仅测量模型是否正确标记视频,而不解释原因,使评估成为黑箱。

Method: 构建HarmVideoBench诊断基准,包含1,379个视频和4,137个多项选择题,涵盖三个层级维度:可观察证据(Observable Evidence)、片段内部含义(Clip-Internal Meaning)和超出片段推理(Beyond-Clip Reasoning)。同时提出BCR方法,预测推理边界并在需要时动态检索上下文。

Result: 评估了19个领先模型在HarmVideoBench上的表现,并展示BCR方法可将基线模型在有害视频理解方面的宏平均从61.7%显著提升至84.4%的先进水平。

Conclusion: HarmVideoBench能够有效评估模型对有害视频的深度理解能力,超越表面线索;BCR方法可显著提升模型在多维度有害视频理解方面的表现,为自动内容 moderation 提供更优解决方案。
...
TL;DR: 本文从score-based生成建模角度重新审视了双分支范式，提出LISA正则化方法，通过对齐侧网络中间特征与近似likelihood score来加速训练收敛、提高生成质量，并使特征更解耦。


<details>
  <summary>Details</summary>
Motivation: 双分支范式在可控视觉生成中广泛使用，但侧分支的作用机制和训练效率未被充分探索，缺乏理论解释。

Method: 1)从score-based视角分析双分支：主网络提供无条件score保持质量，侧网络隐式贡献likelihood score控制条件；2)提出LISA：hook侧网络指定层特征，用轻量解码器投影到score空间，构建近似likelihood score目标，计算正则化损失，联合优化侧网络和解码器。

Result: LISA在图像/视频多种任务、架构、扩散/流模型上验证：加速训练收敛、提高合成结果、使特征更解耦，额外训练成本低、推理零成本。

Conclusion: 从score-based角度理解双分支范式为优化提供了理论指导；LISA是有效的正则化方法，可提升条件可控生成的训练效率和生成质量，且具有实用价值。
...
TL;DR: SatSplatDiff是一种用于卫星3D重建的高质量方法，结合了高斯溅射与几何正则化及阴影引导的生成式细化。该方法旨在解决卫星图像视角受限导致的建筑立面监督不足，以及生成式细化独立处理各视图导致的幻觉和几何降质问题。


<details>
  <summary>Details</summary>
Motivation: 解决两个核心问题：1) 卫星图像有限的顶视角度导致建筑立面监督不足，产生表面空洞和视觉质量下降；2) 现有生成式细化方法独立优化每个视图，导致幻觉现象和光一致性破坏，造成几何降质。

Method: 1) 基于摄影测量DSM初始化和2DGS阴影投射；2) 引入单目深度监督和多尺度几何细化，建立几何准确且规则化的表面表示；3) 应用阴影引导的生成式细化，使用几何计算的阴影图引导高斯体与底层几何保持一致。

Result: 在IARPA2016和DFC2019数据集上实现最先进性能：几何MAE减少高达18%，视觉保真度(FID-CLIP)提升28-45%，支持最高5倍分辨率增强，幻觉最小且传感器一致外观，跨瓦片一致性强，适用于大规模重建。

Conclusion: SatSplatDiff通过结合几何正则化与阴影引导的生成式细化，成功在保持几何准确性的同时提升视觉质量，为大规模卫星3D重建提供了一种有效且可扩展的解决方案。
...
TL;DR: CORTEX是一个用于3D胸部CT的结构化推理基准测试，旨在解决多模态大语言模型在医学影像推理中的可解释性和可验证性问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的推理仅为自由形式文本，难以解释和验证。现有胸部CT问答数据集删除了将发现与结论联系起来的推理过程，且缺乏患者病史信息，导致无法训练和评估具有推理能力的3D胸部CT模型。

Method: CORTEX使用前沿大语言模型生成四阶段诊断追踪（任务理解、视觉观察、诊断推理、答案综合），并通过自动化评分与放射科专家审查相结合的阶段级评估协议进行过滤和验证。

Result: CORTEX基于CT-RATE数据集构建，包含76,177条经过验证的推理追踪，涵盖开放式VQA、封闭式VQA和报告生成任务。

Conclusion: CORTEX提供了训练和评估可信赖的3D胸部CT推理模型所需的结构化监督和阶段级评估协议，数据集和评估代码将公开发布。
...
TL;DR: 提出一种基于分层归一化(HN)的patch描述符精确检索方法，能够在只评估小部分数据库的情况下返回与穷举搜索完全相同的最近邻结果，且结果是确定性的。


<details>
  <summary>Details</summary>
Motivation: 解决近似最近邻(ANN)方法如HNSW和IVF-PQ的局限性：它们为了速度牺牲准确性，且每次运行可能返回不同结果。实际应用需要确定性的精确搜索。

Method: 将128维特征向量通过分层归一化分为K维主要分量((128-K)维次要分量，利用次要内积的alpha上界进行分支限界剪枝搜索。先扫描主要分量，对无法剪枝的条目才进行完整128维评估。

Result: K=8, alpha=1/32时，trevi和halfdome分别实现13.7x和12.7x加速，仅0.4%条目需完整评估。K=16时，7.2x加速，98.8%条目跳过完整评估，FPR@95仅从0.0062升至0.0064。

Conclusion: HN方法实现了确定性的精确最近邻检索，在大幅加速的同时保持与穷举搜索等价的结果，为ANN提供了一种可证明精确的替代方案。
...
TL;DR: 本文提出了一种无需外部网格或形状先验的3D生成强化学习方法，通过直接学习神经辐射场(NeRF)密度场(σ)作为奖励信号来微调预训练的3D感知GAN模型。


<details>
  <summary>Details</summary>
Motivation: 现有RLHF 3D生成管道通常需要将辐射场转换为网格并依赖表面监督数据，作者希望绕过这一限制，直接从辐射场密度中学习3D几何。

Method: 在无条件3D人脸GAN (EG3D)上，从少量偏好样本训练奖励模型，通过密度一致性约束保持2D外观相似性，同时重塑3D几何。

Result: 微调后的生成器在用户偏好比较中获得74.4%的首选率，但FID-50k从4.09上升到6.66，表明几何改进以一定的分布代价为代价。

Conclusion: 该方法证明仅使用单 annotator 偏好的奖励信号即可实现3D几何的显著改善，为无网格或形状先验的RLHF 3D生成提供了可行路径。
...
TL;DR: SmellNet-V是一个可扩展的视嗅数据集,通过利用气味身份在同一语义类别内对视觉变换保持不变这一洞察,将仅嗅觉样本与语义对齐的野生网络图像配对。基于该数据集,提出See & Sniff自监督框架,通过密集局部对齐学习联合视嗅表示,并自然产生气味显著性图用于气味源的空间定位,在气味分类任务中超越仅气味基线7%。


<details>
  <summary>Details</summary>
Motivation: 虽然现代多模态模型集成视觉与语言、音频或触觉,但由于缺乏成对的视嗅数据,嗅觉在很大程度上未被探索。

Method: 构建SmellNet-V数据集:利用气味身份在同一语义类别内对视觉变换保持不变的洞察,将仅嗅觉样本与语义对齐的野生网络图像合成配对,将单模态嗅觉数据集转换为跨模态基准。提出See & Sniff自监督框架:通过密集局部对齐学习联合视嗅表示,自然产生气味显著性图用于气味源的空间定位,并引入像素级气味定位任务和评估基准。

Result: 在气味分类任务中超越仅气味基线7%,并泛化到跨模态检索和气味定位。

Conclusion: 确立视嗅学习作为多模态感知的新方向。
...
TL;DR: ViQ是一个统一的视觉离散表示框架，通过两阶段量化学习（文本对齐预训练和特征离散化）来平衡语义和细节，支持任意分辨率的原生输入。


<details>
  <summary>Details</summary>
Motivation: 现有视觉离散表示方法难以同时保留低层细节和高层语义：重建导向表示缺乏语义信息，语义强的表示则丢失细节。需要一种既能处理原生分辨率、又能在语义和细节间取得平衡的统一离散表示。

Method: 提出两阶段量化学习：1)文本对齐预训练：从预训练语言模型增强视觉编码器的语义监督，处理原生分辨率输入；2)特征离散化：使用近端表示学习策略逐步压缩特征空间，配合位置感知头-wise量化机制处理任意分辨率。

Result: ViQ在多模态任务上达到与最先进的连续高维视觉特征编码器相当的性能，同时保持高精度的低层重建；训练效率提升20%-70%。

Conclusion: ViQ可作为任意视觉输入的通用离散表示，在语义保留、细节重建和训练效率之间实现良好平衡。
...
TL;DR: OctoSense是一个开源的多模态传感器平台，整合了RGB相机、事件相机、激光雷达、热像仪、IMU、RTK-GPS和本体感知数据，并发布了59小时的高质量同步数据集。提出了一种"后期融合"掩码自编码器架构，通过模态特定的分词器处理不同传感器的时空特征，在推理时缓存模态特定令牌以快速处理新数据。该方法在估计光流、深度、语义分割和自运动等任务上优于现有图像基础模型，且在夜间或传感器退化情况下表现出更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态感知领域面临传感器异构性挑战：不同传感器具有不同的表示方式、频率、延迟和噪声特性。现有的图像-only基础模型难以有效处理这种多样性，且在传感器退化或极端条件下性能下降明显。研究者希望构建一个能够统一处理多种异构传感器数据并进行自监督学习的开源平台。

Method: 提出一种"后期融合"掩码自编码器架构，包含两个核心设计：(1) 模态特定的分词器(tokenizers)，分别处理不同传感器的时空特征；(2) 推理时缓存机制，在获取新测量时直接调用缓存的模态令牌进行快速推理，避免重复计算。

Result: 该架构在NVIDIA 5090上推理时间为6.68ms，在Orin NX上为112ms。在光流、深度估计、语义分割和自运动(平移、旋转、转向角)估计等任务上，性能优于现有的图像-only基础模型。更重要的是，该方法在夜间或传感器数据退化情况下仍能保持鲁棒预测。

Conclusion: OctoSense提供了一个有价值的开源多模态传感器平台和数据集，证明后期融合掩码自编码器能有效处理异构传感器数据，为多模态自监督学习提供了新的思路，并在实际机器人任务中展现了良好的性能和鲁棒性。
...
TL;DR: 本文提出DexAC-WM，一种针对高自由度灵巧操作的动作条件世界模型，通过结构化动作建模和语义分支来解决现有方法中动作压缩和信息不平衡的问题。


<details>
  <summary>Details</summary>
Motivation: 高自由度 dexterous actions 具有内在异质性，跨越多个数量级，现有方法将整个动作序列压缩为单一表示，导致优化不平衡，阻碍细粒度效果建模并影响动作保真度。

Method: 提出DexAC：1) 通过动作标记化保持维度级语义；2) 通过局部细化和全局调制对齐动作信号与视觉动态；3) 引入语义分支提供丰富的对象-场景先验，支持高-DoF动作条件视频预测。

Result: 在EgoDex和EgoVerse数据集上显著提升FID、FVD和PCK指标，验证了结构化动作条件设计的可扩展性。

Conclusion: 扩展世界模型到高自由度控制需要结构化动作建模和语义接地。
...
TL;DR: 一种基于旋转位置嵌入（RoPE）的几何感知物体移动方法，可在单张图像中实现一致的空间重组，处理遮挡、生成新可视区域并保持阴影和反射的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在单图像场景中保持几何一致性，无法妥善处理遮挡、新区域的生成以及阴影反射等场景级依赖效应的保持。

Method: 将2D RoPE扩展为深度感知形式以编码3D空间结构，通过操作扩散变换器的位置表示来诱导受控运动；使用合成数据结合少量真实图像进行参数高效微调训练。

Result: 在保持物体身份的大位移情况下生成合理的新内容，一致更新阴影和光照等场景依赖效应，在标准物体移动基准测试中达到所有评估指标的先进水平。

Conclusion: 通过深度感知的旋转位置嵌入实现了几何一致的物体移动，最小真实监督下即可完成高质量的场景更新，为单图像物体操作提供了有效解决方案。
...
TL;DR: 本文提出SAM2Matting，一种从跟踪器到抠图的框架，旨在将视频目标跟踪器（VOS trackers）提升为高质量视频抠图模型，解决视频抠图中高级跟踪与低级精细抠图之间的差距问题。


<details>
  <summary>Details</summary>
Motivation: 视频抠图面临挑战：高级跟踪需要帧级理解，而低级抠图关注极其精细的细节，两者存在固有差距。现有方法依赖昂贵且范围狭窄的视频抠图数据集，限制了域外泛化能力并影响跟踪鲁棒性。

Method: SAM2Matting采用解耦范式：增强基础跟踪器（如SAM2、SAM3），引入区域提议桥（region-proposal bridge）和专用抠图头（dedicated matting heads），使跟踪器专注于时间一致性处理，而抠图组件负责解决精细细节。

Result: 尽管仅在图像上训练，SAM2Matting在视频抠图上建立了新的state-of-the-art性能，支持多样化提示类型，保持强时间一致性，并在人类中心场景和野外场景中展示强大的泛化能力。

Conclusion: 该方法成功证明了通过解耦设计，可以利用图像训练的模型实现高质量视频抠图，为视频抠图研究提供了新思路，具有重要的应用价值。
...
TL;DR: 本文提出RayPE，一种用于视频扩散Transformer的新型位置编码方法，将6D Plucker坐标注入到自注意力机制中，以增强模型对3D几何结构的建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散Transformer使用RoPE对(u,v,t)轴进行位置编码，仅描述相机采样网格，未捕捉场景的3D结构。而两条相机射线之间的几何关系可通过Plucker互易积表达，该形式与Transformer注意力的点积代数形式相同。

Method: 提出RayPE：将每token的6D Plucker坐标加性注入到自注意力的查询和键中，通过查询/键翻转排列使对称恒等配置恰好对应互易积；将射线方向与矩大小解耦，用对数大小的学习函数门控编码，并应用RMSNorm与QKNorm规范化的内容分支对齐；模块仅增加不到0.1%参数，零初始化以保持预训练权重。

Result: 注意力分数分解为内容项、几何项和两项交叉项，实验验证各部分均必要。在四数据集混合训练上提升了相机可控性、跨帧3D一致性和整体视频质量。

Conclusion: RayPE通过将Plucker几何引入注意力机制，有效增强了视频扩散模型的3D几何建模能力，且仅需极小额外参数即可实现。
...
TL;DR: 提出一种无需训练、无需额外奖励模型的自引导机制来解决flow模型在条件生成时的多样性崩溃问题，通过特征分散和流行正则化实现多样且保真的生成。


<details>
  <summary>Details</summary>
Motivation: 现有flow模型在条件生成时存在多样性崩溃问题，现有方法如潜在引导效果有限或依赖外部奖励模型导致推理开销大。

Method: 提出特征自引导机制：在批量生成时分散flow模型的内部特征，并引入流行正则化步骤将分散的特征投影回数据流行，确保多样性的同时保持与输入条件的对齐。

Result: 实验表明该方法在多种条件flow模型上显著提升多样性，同时保持保真度，包括多步和少步文本到图像、深度到图像和参考图像生成任务。

Conclusion: 该方法可作为即插即用模块集成到预训练flow模型中，仅增加边际推理成本，有效解决了多样性崩溃问题。
...
TL;DR: 本文提出Denoising Attention (DnA)去噪注意力机制，通过正负查询识别相关与无关特征，并投影到两个独立子空间以增强判别性，解决标准softmax在视觉任务中产生噪音注意力模式的问题。


<details>
  <summary>Details</summary>
Motivation: 标准softmax在多头注意力中会产生噪音注意力模式，会稀释相关特征并降低视觉感知任务的性能。

Method: 提出DnA方法：1)正查询识别属于正确类别的图像特征；2)负查询识别紧密相关但无关的图像特征；3)将这两种交互投影到两个具有更大主角度的独立子空间，促进子空间分离和提升判别性。

Result: 使用ViT-B backbone在ImageNet-1K上获得0.8%绝对增益；视频理解任务中视频transformers提升1.8%，视频LLMs提升0.5%。

Conclusion: DnA通过两个交互子空间的设计和去噪效果显著提升了视觉理解任务的性能，验证了设计选择的有效性。
...
TL;DR: 本文提出VISE框架，解决自进化多模态模型中的视觉欠条件化问题，通过几何和语义不变性奖励实现无监督训练。


<details>
  <summary>Details</summary>
Motivation: 现有自进化LMMs存在视觉欠条件化问题， decoder依赖语言先验而非图像进行生成，导致图像描述和视觉问答任务表现不佳。

Method: 提出两种基于不变性的奖励：几何不变性奖励强制空间一致性，语义不变性奖励惩罚无证据生成。VISE在单一模型上使用原始无标签图像进行无监督训练。

Result: 使用Qwen3-VL-2B作为基线模型，VISE在COCO上提升+16.85 CIDEr，TextCaps上提升+19.66 CIDEr，物体幻觉减少5.0 Chair-I点，并在4个模型家族和规模上实现泛化。

Conclusion: VISE通过直接规范化视觉条件化策略，有效解决了自进化LMMs中的视觉欠条件化问题，在18个基准测试上验证了方法的有效性。
...
TL;DR: 提出一种自进化训练框架，使统一的大语言多模态模型能够在仅使用无标注图像的情况下，自主提升视觉理解和图像生成两种能力，无需人工标注、偏好标签或外部奖励模型。


<details>
  <summary>Details</summary>
Motivation: 现有的统一大 multimodal models 仍依赖人工标注、偏好标签或外部奖励模型等 curation post-training supervision，论文提出能否仅用无标注图像实现自主提升。

Method: 引入三种内部角色：Proposer 生成视觉问题，Solver 解答并评估，Generator 合成图像。使用自衍生的一致性信号训练；引入 Solver Token Entropy (STE) 基于 token 级预测不确定性的连续难度信号；设计多尺度内部评估结合问答保真度评分与循环一致性 captioning。

Result: 在 BAGEL 上实现 MMMU +3.5% 绝对增益，GenEval 图像生成性能从 82% 提升至 85%。跨八个理解指标持续优于基线模型。

Conclusion: 该框架在不同架构（diffusion-based BLIP3o、rectified-flow BAGEL、autoregressive VARGPT-v1.1）上保持相同的角色分解、奖励逻辑和训练 schedule，只需每个 backbone 的原生 prompting 和生成接口即可应用。
...
TL;DR: DanceOPD是一个基于流匹配模型的在线策略生成场蒸馏框架，通过将每种能力（文本到图像生成、局部编辑、全局编辑）定义为共享流状态空间上的速度场，让学生模型从自身 rollout 状态查询的场中学习，从而实现多能力统一。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成需要单一模型具备多种能力（文本到图像生成、本地编辑、全局编辑），但这些能力往往相互冲突，例如编辑会降低T2I性能，全球和本地编辑相互干扰，如何有效组合这些能力成为核心挑战。

Method: 提出DanceOPD框架，包含：1）将每种能力源定义为共享流状态空间上的速度场；2）每个样本路由到一种能力场；3）查询一个低噪声学生诱导状态；4）使用简单的速度MSE目标训练。该公式还能吸收操作员定义的场如无分类器引导(CFG)。

Result: 在T2I、编辑、现实场吸收和CFG吸收等任务上的综合实验表明，该方法改善了多能力组合，在增强目标能力的同时保持锚点生成质量。

Conclusion: 这项工作为流匹配模型中的生成场蒸馏建立了一条实用路线，有望推动统一多能力的图像生成模型发展。
...
### [95] [A unified cell-merge algorithm for generating diverse Voronoi diagrams and new tessellations based on spatial chromatic model](https://arxiv.org/abs/2606.27235)
*Weining Zhu*

Main category: cs.CG

TL;DR: 提出了一种基于空间染色模型的新型元胞合并(CM)Voronoi图生成算法,可以快速生成多种不同类型的Voronoi图,无需为每种类型开发特定算法。


<details>
  <summary>Details</summary>
Motivation: Voronoi图有多种类型(普通VD、球面VD、线实体VD、加权VD、高阶VD等),传统方法需要为每种类型开发专门的生成算法,过程繁琐且效率低下。

Method: 基于空间染色模型,提出元胞合并(CM)算法。该算法通过从统一数据库中检索和合并元胞来快速生成各种类型的Voronoi图。

Result: CM算法可快速生成多样化的Voronoi图,特别适用于需要频繁计算和分析多种Voronoi图的场景(如位置-based空间分析),还能生成新型空间 tessellation如竞争强度图和 couples-cell图。

Conclusion: CM方法为Voronoi图的生成提供了一种通用、高效的解决方案,降低了为不同类型VD开发专门算法的需求,并能生成传统Voronoi图之外的新型空间 tessellation。
...
TL;DR: 该论文研究AI代理协议治理结构，比较ERC-8004（无权限链上）和Google A2A（企业主导）两种互操作性标准，分析治理模式如何影响主题优先权和社区结构。


<details>
  <summary>Details</summary>
Motivation: AI代理协议治理结构的互操作性标准缺乏实证研究，需要理解制度设计如何塑造技术治理。

Method: 构建LLM驱动的比较管道，整合自动注释、神经主题建模和多层网络分析，分析4,323条治理参与记录。

Result: 治理形式影响实质性重点；两种制度都呈现相当的参与不平等和社区分裂；无权限设置的话语对齐更密集，表明开放治理可能促进更大的主题趋同。

Conclusion: LLM辅助方法可推进技术治理的实证研究，对设计更公平的代理AI标准具有启示意义。
...
TL;DR: 研究定义了组合行为泄露（CBL）这一新失败模式：当编辑一个提示词模块时，会 silently 影响其他模块的行为，即使它们之间没有共享变量或可执行依赖。这种干扰源于架构非隔离性——Transformer自注意力机制在连接的模块之间没有提供形式边界。


<details>
  <summary>Details</summary>
Motivation: 提示词组合代理系统的实践者报告了一个反复出现的失败模式：编辑一个提示词模块会 silently 改变其他模块的行为，尽管它们之间没有共享变量或可执行依赖。这篇论文将此正式定义为组合行为泄露（CBL）。

Method: 通过一个可重用的三通道协议来探测CBL：在部署的求职评估代理（Claude Sonnet 4.6）上进行144次试验，沿体积、内容和形式三个维度对非焦点模块进行扰动。

Result: 只有内容通道产生了可检测的配对效应（Cohen's d = 0.63，bootstrap 95% CI排除零）；没有建议被翻转——这是一个标准QA无法检测到的亚阈值制度，但会在部署代理所做的数千个决策中累积。

Conclusion: CBL与已知的代理失败轴正交（对抗性注入、认知退化、多代理故障传播、隐私泄露）。作者贡献了操作定义、可重用协议、可证伪预测集和系统类特征，将跨模块干扰测量确立为提示词组合代理评估的要求。
...
TL;DR: 本论文提出将科学发现形式化为元优化问题，即同时优化研究目标和评价标准。通过"共识目标聚合"方法将LLM生成的目标函数进行相关性加权投票，形成自洽的演化评价标准。在3-SAT问题算法发现中实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 科学发现本质上是在庞大的理论和实验状态空间中进行优化，但现有方法仅关注搜索策略而忽视了评价标准本身的优化。作者认为同时修改评价标准同样重要。

Method: 提出"共识目标聚合"框架：1)利用LLM生成多个候选目标函数；2)通过相关性加权投票机制组合这些目标函数；3)形成稳定、自纠正的演化评价标准，使其随理解深入而动态调整。

Result: 在数字MemComputing机器上解决3-SAT问题，问题规模N的基准扩展从~N^2.51降低到~N^1.33，在最大测试实例上实现约67倍加速。

Conclusion: 该框架作为问题无关的一般方法，可显著促进科学发现自动化，为AI驱动的科学研究提供了新的范式。
...
TL;DR: 本文提出一种迭代数据生成管道，通过级联线性特征来检测和引导语言模型远离"谄媚"行为，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 当前激活转向方法需要大量对比样本来识别模型特征，但简单二元样本对难以有效分离特征与行为的关系，需要更精细的数据生成方法来提高可解释性和可控性。

Method: 提出迭代数据生成管道，生成展示线性缩放特征的级联样本，而非简单二元对比；利用这些样本检测谄媚特征的线性可分离子空间，实现激活转向。

Result: 级联样本发现的谄媚特征形成线性可分离子空间，选择的激活与期望行为更清晰；在检测、确定性评分和稳健转向方面与LLM-as-a-judge和系统提示基线相当或更优，同时计算需求更低且可解释性更强。

Conclusion: 级联线性特征方法能有效分离谄媚特征子空间，为检测和引导语言模型行为提供了更高效、可解释的方案。
...
TL;DR: 研究多智能体大语言模型循环中的语义早停机制，解决传统固定迭代上限的盲目性问题，通过监控连续草案嵌入的语义变化来判断是否应终止循环。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体LLM循环（如Writer-Critic模式）几乎都使用固定迭代上限作为终止条件，这是一种语法层面的" kill-switch"，无法判断答案是否仍在改进，导致简单输入浪费token、困难输入被截断。

Method: 1) 理论层面：证明确定性终止和良好定义性，将距离序列收敛作为经验验证的猜想而非Banach压缩定理；2) 评估协议：生成每个问题的完整轨迹，重放每个停止策略并缓存LLM-judge调用，实现严格配对的效率-质量比较；3) 实验：在HotpotQA的60题测试集上评估两种语义停止器（judge-free和quality-gated）。

Result: 无judge的语义停止器相比max_iterations减少38%操作token，同时保持等价质量（Delta-IS=-0.004, p=0.81）；完整的质量门控变体因每轮判断成本过高而适得其反；Oracle选择最佳轮次可达+0.015信息分数（p~4e-11），将问题重新定义为"选择最佳轮次"而非"何时停止"。

Conclusion: 语义早停机制可显著降低token消耗且保持质量，为多智能体LLM循环提供更优的终止策略。未来研究应聚焦于如何选择最佳轮次而非判断停止时机。
...
TL;DR: 当基准测试准确率饱和后，传统做法是退役该基准并替换为更难版本。本文提出这种做法过于单一，忽视了评估智能体性能的其它六个关键维度：构建效度问题、分布外泛化、效率、可靠性、模型与脚手架的相对重要性、以及人机协作的提升效果。作者以计算可复现性基准CORE-Bench Hard为案例，展示如何通过多维度评估获得有意义见解。


<details>
  <summary>Details</summary>
Motivation: 传统基准测试评估过度依赖准确率指标，当准确率饱和时基准被退役，导致大量有价值的评估信息丢失。作者认为应该从更多维度评估智能体性能，而不仅仅是准确率。

Method: 作者使用CORE-Bench Hard作为案例研究：（1）识别构建效度威胁并改进基准版本；（2）在准确率饱和后继续测量效率、可靠性、模型与脚手架性能；（3）开展小规模随机实验测量人机协作的提升效果。

Result: （1）发现了难以预见的构建效度问题，发布CORE-Bench v1.1和CORE-Bench OOD；（2）即使准确率饱和，基准仍可用于测量效率、可靠性、模型和脚手架性能；（3）人机协作带来约两倍的显著加速，但可能因五分之一的人类单独任务超时而低估了实际提升效果。

Conclusion: 本文提出了一种比单纯准确率评估更严格的替代方案，建议从多个维度全面评估智能体性能，以充分利用饱和基准的信息价值。
...
### [102] [Geometry-Aware MCTS for Extremal Problems in Combinatorial Geometry](https://arxiv.org/abs/2606.26399)
*Luoning Zhang,Xu Zhuang,Tianhao Wang,Nathan Kaplan*

Main category: cs.AI

TL;DR: 本文提出了一种几何感知的蒙特卡洛树搜索框架(GMCTS)来解决组合几何中的极值问题,如No-Three-in-Line和最小完备集问题。该方法通过增量更新可行动作空间来强制执行几何约束,并利用几何对称性进行规范剪枝和对称批处理转换。实验在6个问题中取得5个新的最佳结果。


<details>
  <summary>Details</summary>
Motivation: 传统精确求解器面临组合爆炸,强化学习和基于Transformer的模型存在稀疏奖励"有效性悬崖"和二次Token消耗限制。这些瓶颈严重阻碍了大规模n×n网格上组合几何问题的求解。

Method: 提出几何感知MCTS框架:1)通过增量更新可行动作空间强制执行几何约束,将共线点约束检查复杂度从O(n³)降至O(n²);2)利用几何对称性进行规范剪枝以降低分支因子;3)使用对称批处理转换加速发现优质配置。

Result: 在6个测试问题中5个取得新最佳结果。Max-N3IL问题:在82≤n≤119的网格上找到大小约1.8n的配置。最小完备集问题:找到大小约0.95n的配置,提供新的上界。

Conclusion: 几何感知MCTS是一个高度适应性的组合几何新配置发现框架,成功解决了传统方法面临的组合爆炸和稀疏奖励问题,为组合几何领域提供了新的计算工具。
...
### [103] [Refusal Lives Downstream of Persona in Chat Models](https://arxiv.org/abs/2606.26161)
*Viola Zhong,Qirui Li*

Main category: cs.AI

TL;DR: 该论文发现指令微调聊天模型中拒绝方向和角色方向存在相互作用，顺从的角色会"门控"拒绝行为。通过在Qwen2.5-7B-Instruct和Llama-3.1-8B-Instruct上的干预实验，证明消除角色方向可恢复拒绝能力，表明拒绝是一个依赖于角色的门控机制，而非独立的单一方向。


<details>
  <summary>Details</summary>
Motivation: 之前研究将拒绝和角色traits作为独立机制分别研究，未探索它们之间的相互作用。该论文旨在揭示这两个方向是否存在依赖关系。

Method: 提取顺从模型-角色方向和拒绝方向，在两个模型上进行双向干预实验：使用角色方向抑制拒绝，以及在晚层重新引入拒绝方向或消除角色方向来观察效果。

Result: 顺从角色方向 steering 显著抑制拒绝——Llama模型中拒绝率从97%降至2%。在晚层重新引入拒绝方向部分恢复拒绝能力，但仅在晚层有效。投影消除角色方向可恢复拒绝至基线水平，而随机方向投影无效。

Conclusion: 拒绝行为在晚层表达阶段被门控，位于计算下游。将拒绝视为单一孤立方向忽略了对角色的依赖性，需要将拒绝理解为受角色控制的门控机制。
...
### [104] [AlgoEvolve: LLM-driven Meta-evolution of Algorithmic Trading Programs](https://arxiv.org/abs/2606.26173)
*Dhruv Sharma,Gautam Shroff*

Main category: cs.AI

TL;DR: 本文提出AlgoEvolve框架，利用LLM作为语义变异算子，通过进化算法自动生成和优化Python代码形式的算法交易策略。


<details>
  <summary>Details</summary>
Motivation: 算法交易领域具有噪声大、非平稳性、高度不连续性的特点，传统方法难以应对，需要一种能够自适应演化的新范式。

Method: 提出双层进化框架：内层循环使用LLM生成交易策略代码并通过严格测试协议评估；外层循环进化指导提示词以发现更好的搜索启发式方法。

Result: 系统展现出自适应策略逻辑，包括交易规则的自适应转变；外层循环发现的启发式方法在平衡探索与开发、减少零交易失败方面 consistently优于人工设计的初始指令。

Conclusion: LLM驱动的语义进化为复杂环境中的持续程序合成提供了一种可行的新方法。
...
TL;DR: 研究开发了一种基于知识图谱的多智能体框架，整合Reddit帖子（466,525条）、WebMD评论（60,782条）和20年FDA不良事件报告，用于九种抗抑郁药。框架可识别药物和症状，F1分数分别达0.969和0.973，发现社区平台间的一致性高于与监管报告的一致性，且部分不良事件在社区中出现早于FDA报告数百天。


<details>
  <summary>Details</summary>
Motivation: 精神类药物信息分散在权威但抽象的监管不良事件记录与贴近体验但未验证的患者叙事中，整合这些来源而不混淆证据与轶事至关重要，因为信息缺乏上下文会加剧恐惧、安慰剂效应和非依从性。

Method: 开发了基于知识图谱的多智能体框架，整合466,525条Reddit帖子、60,782条WebMD评论及20年FDA不良事件报告，针对九种抗抑郁药。采用大语言模型实体识别管道，以医生标注为基准评估。使用Neo4j知识图谱，以ATC-N、ICD-10和MedDRA词汇表为基础，保留溯源性。

Result: 药物识别F1分数最高0.969，症状识别0.973。社区平台间一致性更高（Jaccard相似度达0.905），与监管报告差异较大。对于舍曲林，许多不良事件在社区中出现的时间比FDA报告早数百天。

Conclusion: 建立来源感知的整合是实现更可审计的精神类药物信息的途径，有用性和患者获益有待前瞻性验证。
...
TL;DR: 论文提出DD-Elo系统，将认知神经科学中的漂移扩散模型(DDM)应用于国际象棋评级，融合move级别数据来捕捉快速技能波动。


<details>
  <summary>Details</summary>
Motivation: 传统Elo系统仅依赖比赛结果，忽视游戏质量细微差别，且对技能变化响应滞后。

Method: 将技能表达建模为决策过程，整合move级别数据，建立DD-Elo数学框架并证明与Elo系统的有界偏差。

Result: DD-Elo比传统Elo更快适应技能变化，实验验证了系统的高响应性和理论对齐。

Conclusion: DD-Elo提供可解释、高响应、向后兼容的象棋评级解决方案。
...
TL;DR: 本文提出了一种计算治理模型，用于规范AI智能体执行临床处方和软件部署等高风险、不可逆行为。该模型的核心思想是：智能体保留规划和推理的自主权，但将高风险行为的执行权交给独立的权威来源进行验证。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体可能执行不可逆的高风险行为，人类社会通过要求在关键行动点提供独立验证证据来治理强大的自主行为者，而非监控其推理过程。这一制度模式启发了本文的计算治理模型设计。

Method: 形式化建模：智能体保留规划和推理的完全自主权，但对指定的高风险行为没有执行权。执行需要满足的前提条件由独立的权威来源逐一独立证明，与声明的意图加密绑定，由确定性策略评估，决策记录在防篡改日志中供独立重新验证。

Result: 提出了概念验证实现，并用软件部署和临床处方两个场景进行了说明。

Conclusion: 该模型为AI智能体的治理提供了新的技术方案，通过引入独立的验证机制和防篡改日志，既保证了智能体的自主性，又确保了高风险行为的可控性和可审计性。
...
TL;DR: COrigami是一个端到端的AI驱动管道，利用自然语言生成折纸 crease patterns。它将生成式AI应用于计算折纸领域，在数学严格的环境中生成满足几何约束和视觉美学的物理艺术作品。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在解决可验证解决方案问题上取得成功，但生成同时满足严格几何约束和主观视觉美学的物理艺术仍面临挑战。计算折纸提供了一个数学rigid环境，将艺术设计建立在平面可折叠性方程中。

Method: 管道包括5个阶段：1)从自然语言生成语义棒图；2)计算基础折叠；3)求解平面可折叠的折痕图案；4)塑造平面折叠的折痕图案；5)使用基于自主美学评估循环的强化学习细化模型。

Result: 系统作为高效的协作助手，生成人类艺术家可进一步扩展和塑造的结构起点。通过集成算法优化与自主美学批评，展示了AI系统如何满足多目标物理约束，实现数学基础的协作创作。

Conclusion: 该工作证明了AI可以与人类艺术家协作，在满足多目标物理约束的同时实现可靠的、数学基础的共同创作，为AI辅助物理艺术设计开辟了新途径。
...
TL;DR: 本文研究了AI编码代理中验证(verification)面临的根本挑战。随着基础模型生成能力增强,验证反而变得比生成更难,因为:1)用户意图本质上难以精确表达;2)训练过程中的优化会拉大代理与真实意图之间的差距(表现为奖励黑客或信号饱和)。作者从可扩展性、忠诚性、鲁棒性三个维度分析验证信号质量,研究四种奖励构建方案,并指出验证器必须与生成器协同演化。


<details>
  <summary>Details</summary>
Motivation: 传统观念认为"验证比生成容易",但在大模型时代这一观念被反转。当前编码代理面临的核心问题是:所有验证器都是用户意图的代理而非意图本身,且训练优化会扩大代理与意图的差距。这导致奖励黑客、信号饱和等问题,亟需系统性解决。

Method: 1)从可扩展性、忠诚性、鲁棒性三维度定义验证信号质量;2)研究四种奖励构建:通用编程任务测试验证器、前端任务规则验证器、真实世界代理任务中用户作为验证器、长周期任务自动化代理验证器;3)在不同任务类型和策略能力级别下进行深入实验分析。

Result: 实验表明:针对性验证设计可有效抑制奖励黑客、提高任务完成质量,在多个内部和公开基准上取得显著收益。核心发现是:随着策略能力增长,没有固定奖励函数能保持有效,验证必须与生成器协同演化。

Conclusion: 验证是AI代理系统的核心挑战,需要从三维度权衡验证信号质量。四种奖励构建各有优劣,需根据任务类型选择。关键结论:固定奖励函数在能力增长面前会失效,验证设计需要持续演进以匹配生成器的能力发展。
...
TL;DR: 该研究填补了能源领域AI代理评估的空白，构建了包含243个专业问题的基准测试集，评估工具增强的LLM代理在能源市场分析任务上的表现，涵盖市场数据检索、知识解读和定量建模三大类别。


<details>
  <summary>Details</summary>
Motivation: 能源行业需要实时数据检索、专业监管知识和多步定量推理能力，但现有基准测试仅局限于静态知识召回，存在关键研究空白。

Method: 构建包含243个专家策划问题的评估环境，分为市场数据检索分析、知识检索解读、高级定量建模决策三类；配备电力市场API、监管数据库、资产优化模型等域工具；采用多维度评估协议评分方法正确性、答案准确性、属性对齐和来源有效性。

Result: 评估了闭源和开源LLM在能源市场分析任务上的表现，揭示了模型能力与领域工具配置在专业领域中的交互作用。

Conclusion: 为能源领域的AI代理评估提供了可复现的基准测试框架和数据支持，推动了高风险专业领域中AI系统的应用研究。
...
TL;DR: 本文探讨了多模态大语言模型(MLLMs)的评估问题，指出现有评估基准的局限性，并识别了评估框架中的关键空白。


<details>
  <summary>Details</summary>
Motivation: 随着MLLMs能力快速发展，其评估方法却未能同步跟进。现有基准多局限于单一任务，难以评估模型是否真正整合了跨模态信息，亟需建立更全面的评估体系。

Method: 通过系统审查现有评估基准和分类法，识别当前评估体系中的关键差距，包括时空一致性、物理世界理解、多模态一致性和选择性注意等维度。

Result: 识别出四大评估空白：1)时空一致性；2)物理世界理解；3)多模态一致性；4)选择性注意力。这些缺失严重阻碍了对多模态智能真实进展的衡量。

Conclusion: 填补这些评估空白对于衡量多模态智能的真实进展、揭示模型能力边界至关重要，是该领域未来发展的关键方向。
...
TL;DR: OpenFinGym是一个统一的量化金融智能体开发环境，涵盖预测、市场生成、实时交易和欺诈检测，通过单一执行和验证接口支持多阶段金融工作流，并提供自动化任务构建管道、容器化运行时和回测交易引擎。


<details>
  <summary>Details</summary>
Motivation: 现有评估平台分散且孤立，过度关注单一任务，导致高估智能体能力、无法揭示泛化、真实市场交互和金融决策方面的弱点，而金融工作流本质上是多阶段的、相互依存的。

Method: 构建统一gym环境集成多任务，设计自动化pipeline将论文转为可执行任务包，开发容器化运行时防止数据泄露，实现低延迟数据流回测引擎，支持延期 resolution预测。

Result: 提供覆盖预测、市场生成、实时交易、欺诈检测的统一接口，支持SFT和RL后训练集成，实现可扩展智能体部署并防止训练测试泄露。

Conclusion: OpenFinGym为量化金融智能体开发提供标准化基准平台，有望推动该领域评估体系的统一和发展。
...
TL;DR: 本文对库兹韦尔"加速回报"理论进行了数学解读，并论证了加速回报本身无法解决科学发现的核心问题。作者区分了执行性/基础设施能力与定性推理能力的本质差异，指出真正的科学发现依赖于对现有框架结构性不足的质性判断。QES（定性科学引擎）被定位为弥补这一缺失能力的响应，其价值不取决于AGI何时实现，而在于科学发现过程本身作为值得保存和传承的人类智慧形式。


<details>
  <summary>Details</summary>
Motivation: 库兹韦尔理论预测技术进步将呈指数级自我强化，但作者认为这一理论忽视了科学发现的核心问题——即何时现有框架结构上不足以及需要何种概念性突破。ARC-AGI-3测试结果显示人类达到天花板而前沿AI系统准确率低于1%，凸显了当前AI与人类灵活推理之间的巨大差距。同时，Demis Hassabis强调人类必须保留对意义和人生聚焦的感知，这提醒我们AI的未来不仅是技术预测，更是关于何种人类理解形式值得保留和传承的问题。

Method: 首先对库兹韦尔的加速回报理论进行简单数学解读，然后区分两种能力：执行性/基础设施能力（最自然地适用于加速回报）和质性推理能力（对当前框架结构性不足的判断及下一步概念性移动的理解）。结合ARC-AGI-3基准测试结果说明当前AI系统的局限性，最后提出QES作为应对缺失能力的方案。

Result: 加速回报确实存在于计算、人工智能、脑科学和生物技术等领域的执行性能力中，但这种加速并不能自动解决科学发现的核心问题。真正的科学发现需要定性推理能力——即识别现有理论框架何时失效以及需要何种新概念突破的能力。ARC-AGI-3测试表明人类在基准测试中表现完美，而前沿AI系统准确率不足1%，证实了当前AI系统在质性推理方面的严重不足。QES被定位为专门解决这一缺失能力的技术方案。

Conclusion: 库兹韦尔理论有助于解释定量能力的加速，但QES解决了加速回报本身无法解决的科学发现核心问题。QES的价值不取决于AGI何时实现，而在于科学发现过程本身构成了一种值得保存、组织和accessibility的人类智慧形式。人类在科学探索中的质性判断和概念创新能力是当前AI系统无法替代的，这些过程应当作为人类文明的重要遗产得到保护和发展。
...
TL;DR: 研究引入"叙述性思维"(NoT)方法，通过五部分结构化提示(主角、利益相关者、双步后果、不确定性、承诺)修复道德推理中的两种失败模式：利益相关者崩溃(仅命名一个利益方)和不确定性抑制(不表达未知)。该方法无需训练或微调，在100个场景中显著降低两种失败率，并实现多轮辩论中的高共识度。


<details>
  <summary>Details</summary>
Motivation: 标准思维链在道德困境推理中存在两个关键失败模式：1)利益相关者崩溃，即推理过程仅命名最多一个有利害关系的当事人；2)不确定性抑制，即在承诺行动前缺乏明确的未知或对冲表达。这两个问题影响了AI在道德决策中的可靠性和可解释性。

Method: 引入叙述性思维(NoT)系统提示，将思维链结构化为五部分：主角、利益相关者、双步后果、不确定性、承诺。在100个DailyDilemmas场景上测试四种生成器，并进行了匹配预算的verbose-CoT对照、部分消融、文本梯度下降初始化、跨家族训练法官评估以及五轮多利益相关者辩论协议。

Result: NoT将利益相关者崩溃从最高31%降至1%以下，不确定性抑制从最高72%降至1-24%。保留Cliff's delta优势：利益相关者计数+0.79至+0.90，不确定性得分+0.65至+0.93(四种生成器中三种)。多利益相关者辩论将6%僵局转为校准集上95%完全共识和DailyDilemmas复制上100%联合收敛。

Conclusion: NoT提供可审计的推理框架，外化了支撑每个承诺的利益相关者、后果和不确定性，为依赖性智能体部署奠定了可靠基础。该方法无需额外训练或参数即可显著改善道德推理质量。
...
TL;DR: 本文提出了一种智能体聚合器框架，将优化模型与监督智能体相结合，用于协调电动公交车队的充电调度、V2G灵活性和电网互动。框架包含物理可行性约束的优化核心，以及用于扰动检测、电价适应和调度评估的智能体层。


<details>
  <summary>Details</summary>
Motivation: 电动公交车队运营需要协调服务可靠性、电池SOC、充电器可用性、电价、路线能耗不确定性和车网(V2G)机会等多重因素，现有决策环境复杂，需要引入新的协调范式。

Method: 提出耦合优化核心与智能体层的框架架构：优化核心强制执行路线、充电器、电池和V2G交换的物理可行性约束；智能体层解释变化的运营条件，触发实时再优化，并定义灵活价值在聚合器与公交运营商之间的分配方式。采用真实 depot 案例研究，评估利润导向和运营导向两种协调模式下的日前和实时运营。

Result: 智能体聚合可支持自适应的车队-电网协调，维持可行调度，选择性激活再优化，改善充电和V2G灵活性的利用。但存在关键权衡：同样的智能体能力在配置为利润导向定价时可从公交运营商处提取价值。

Conclusion: 智能体聚合器可用于管理电动公交车V2G运营，但在公共车队场景中的部署需要透明的协调模式、可审计的定价设定和明确的价值分享规则。
...
TL;DR: 本文提出了一种解决Oracle AI自指问题的新方法：报告一个自洽的信用集（credal set）而非单一概率值，并利用Knaster-Tarski不动点定理在完备格上构造最小不动点作为规范答案。


<details>
  <summary>Details</summary>
Motivation: 解决Oracle AI的自指困境：当AI的概率估计被学习和行动后，会改变被估计的概率，导致传统回答失效。现有counterfactual方法虽可行但会因前提被否定而立即失效。

Method: 使用Knaster-Tarski不动点定理在闭信用集的完备格上定义等距算子，求其最小不动点；变体方法求包含所有自洽点估计的最小不动点。纯格论发展，从二元事件B推广到任意随机变量X。

Result: 证明了存在性、自洽性和非空性；对非performative问题退化为经典点估计；对二元事件在自然hull-factoring假设下得到区间答案。

Conclusion: 提供了解决Oracle AI自指问题的规范框架，但区间特征是否在一般随机变量情形下保持有待开放问题。
...
TL;DR: 该论文评估了社会科学文本分类场景下（小样本、不频繁构念、嵌套文本）性能指标置信区间的估计方法，发现默认方法（如Wald区间和基本百分位Bootstrap）覆盖不足，而Agresti-Coull、Wilson、Clopper-Pearson和新型伪计数正则化Bootstrap方法更准确。


<details>
  <summary>Details</summary>
Motivation: 研究者使用文本分类模型测量自然语言构念时，通常报告精确率和召回率等指标，但这些指标是点估计，受抽样变异影响，不确定性测量（置信区间）却未被一致报告，且现有方法在小样本或高性能条件下并不适用。

Method: 通过模拟研究评估多种置信区间方法：Wald区间、Agresti-Coull、Wilson、Clopper-Pearson、基本百分位Bootstrap、层次Bootstrap、聚类Bootstrap及新型伪计数正则化Bootstrap，在小到中等样本量、稀有构念、文本嵌套于个体的典型社会科学文本分类条件下进行测试。

Result: 默认方法（Wald区间和基本百分位Bootstrap）准确性最低，覆盖率有时远低于标称95%水平；Agresti-Coull、Wilson、Clopper-Pearson和新型伪计数正则化Bootstrap（在F1计算中特别相关）提升了准确性；当文本嵌套于个体时，需调整有效N和适当自由度以产生准确的分析区间；层次Bootstrap在个体产生中等数量文本时比聚类Bootstrap更准确，但在个体只产生少量文本时过于保守。

Conclusion: 研究为该领域提供了选择适当区间估计方法的指导，以提高机器学习应用的透明度，并鼓励在设计阶段更加重视验证样本量的确定。
...
TL;DR: 该论文研究监督深度学习系统能否达到符号级三段论演绎推理的能力，通过理论分析和实验证明了其局限性。


<details>
  <summary>Details</summary>
Motivation: 探讨数据驱动的机器学习系统是否可以通过增加训练数据和训练时间来达到符号级演绎推理的水平，类似于球形神经网络在无训练数据情况下实现的符号级三段论推理。

Method: 理论分析：(1)证明训练数据无法区分所有24种有效的三段论推理类型；(2)分析端到端映射从前提到结论引入的模式识别与逻辑推理之间的矛盾训练目标。实验测试：Euler Net和ChatGPT（GPT-5-nano和GPT-5）在四种表面形式（单词、双词、简单符号、长随机符号）下的三段论语句可满足性。

Result: 理论证明存在两个方法论限制使监督深度学习无法达到符号级三段论推理；实验表明Euler Net无法实现严格的三段论推理；ChatGPT的表面形式影响推理性能，GPT-5可能达到100%准确率但仍提供错误解释。

Conclusion: 由于经验训练过程在达到100%准确率后停止，监督机器学习系统将无法达到符号逻辑推理的严谨性。
...
TL;DR: MKG-RAG-Bench是一个专门针对多模态知识图谱RAG系统中检索任务设计的跨领域基准测试，旨在评估多模态知识图谱检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多忽略了多模态知识图谱RAG中检索环节的挑战。实际应用中，多模态知识具有异构性，难以跨模态对齐，且针对非结构化语料库设计的检索器效果不佳，检索成为关键瓶颈。

Method: 构建了一个LLM驱动的策展管道，从两个多模态知识图谱（通用领域和医学领域）提取数据，过滤低效用知识，生成结构化接地查询并提供精确监督，系统性覆盖多种模态配置。

Result: 实验表明，有效的多模态检索仍然具有挑战性，但对端到端MKG-RAG性能至关重要，检索质量直接决定生成结果。

Conclusion: 通过将检索作为首要评估目标，MKG-RAG-Bench为诊断当前多模态知识图谱RAG系统的局限性提供了原则性基础，有助于推动该领域的研究进展。
...
TL;DR: 本文介绍了auto-psych系统，一个基于AI的自动化发现工具，用于计算认知科学中的理论生成。该系统通过嵌套的代理发现循环生成解释人类行为的理论，内环进行猜想、拟合和批评概率认知模型，外环设计实验、在线发布并分析数据。以经典的硬币抛掷序列随机性判断为测试场景，系统在合成数据上能快速恢复真实理论，并在三组独立的人类实验中找到比文献更好的理论。


<details>
  <summary>Details</summary>
Motivation: 数据收集是AI科学自动化的主要瓶颈。心理学和计算认知科学尤其适合AI实验，因为理论可以用代码表示，众包平台可实现程序化的人类数据收集。作者希望将自动化发现技术应用于计算认知科学中的理论生成。

Method: 作者开发了auto-psych系统，采用嵌套代理发现循环架构。内环进行猜想、拟合和批评概率认知模型；外环设计实验、在线发布众包调查并分析数据。以认知心理学中经典的硬币抛掷序列随机性判断为测试场景，进行三组独立的人类实验。

Result: 系统能通过系统化实验从合成数据中快速可靠地恢复真实理论，嵌套结构对模型性能至关重要。三组独立人类实验表明，系统发现的理论与数据拟合优于从科学文献中生成的理論。

Conclusion: 本研究证明了在计算认知科学中实现自动化数据收集和理论发现的可行性，为AI辅助科学研究提供了新的方法论框架。
...
TL;DR: 本文提出"临床AI技能"概念和"Clinical Harness"运行时治理架构，解决医学AI模型孤立缺乏持久性问题，以骨质疏松症为例展示知识驱动、数据驱动和物理增强技能的整合。


<details>
  <summary>Details</summary>
Motivation: 当前医学AI研究围绕孤立模型开展，缺乏跨时间持久和可追溯的临床能力，而实际临床护理需要能够在整个诊疗过程中持续发挥作用并被监管的能力体系。

Method: 提出Clinical Harness运行时治理架构，实现AI临床能力的注册、编排、监护和监控功能；以骨质疏松症为示例，整合知识驱动、数据驱动和物理增强三种AI技能支持全生命周期护理。

Result: 成功构建了可运行治理的AI临床能力框架，展示了 osteoporosis 示例中三种技能如何在运行时治理下协同工作，实现从诊断到随访的持续性AI辅助。

Conclusion: Clinical Harness为医学AI从孤立模型向持续性临床能力提供了可行架构路径，有望推动AI在临床实践中的规模化部署和监管合规。
...
TL;DR: 研究大型推理模型(LRM)与人类在思考过程中的差异：两者在跨项目难度感知上相似(registration)，但在单个项目内部对错误vs正确答案的投入上相反(allocation)。人类在答错时放弃(花更少时间)，LRM在不确定时延长推理链(花更多tokens)。


<details>
  <summary>Details</summary>
Motivation: 之前研究只关注跨项目相关性，忽略项目内部差异。作者想探究LRM和人类是否真的有相似的思维机制，还是只是表面相似。

Method: 在匹配的LRM-人类语料库上，对比5个thinking LRMs和人类在答题正确vs错误时的tokens/时间消耗，控制项目固定效应，跨数据集验证，并对比非thinking基线模型。

Result: 所有模型在跨项目难度上与人类一致(Cohen's d=1.47-3.13)；但在项目内部，人类答错时花更少时间，LRM答错时花更多tokens，效应相反。分离出两种停止策略：人类是engagement vs abandonment，LRM是uncertainty-driven length。

Conclusion: 表面相似性掩盖了根本性的控制差异。资源理性元推理框架下，两种策略共享难度信号但实现相反的控制。需要区分信号捕获与控制机制。
...
TL;DR: NeuraDock Agent是一个开源的EEG代理工具，专注于Alpha波动态分析和视觉认知负荷分析，提供从安装、预处理、质量控制、可视化、统计分析到实时API和LLM解释的完整端到端工作流程。


<details>
  <summary>Details</summary>
Motivation: 现有EEG工具包提供优秀的离线分析，但构建实时、质量可控的认知负荷Pipeline需要手动整合采集、定制QC、Alpha特征提取和Web API，缺乏从离线到在线的直接解决方案。

Method: 采用质量门控工作流程：下游Alpha和负荷指标仅在预处理和QC门控后计算，而非直接从原始EEG计算。包含18个 recordings的mini-dataset验证。

Result: 处理了18个 recordings，生成了10个受试者内比较，观察到10个对比中7个存在任务相关的后部Alpha抑制，获取了受试者内重复性的初步证据，并基准测试了本地在线API延迟。

Conclusion: 为研究人员、开发人员和应用团队提供了从EEG文件到实时视觉认知负荷原型的透明路径。
...
TL;DR: NeuraDock Agent是一个开源架构,将确定性本地EEG引擎与硬件感知语言层分离,通过受限制的上下文摘要确保LLM仅能做出EEG硬件和软件实际支持的声明,避免低通道EEG产生看似合理但实际不支持的解读。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型无法自动了解特定传感器的测量能力、当前软件实现的算法,或计算结果合理支持哪些结论。这些问题在低通道EEG中尤其重要,因为稀疏的空间覆盖和可变的信号质量容易产生看似合理但实际不支持的解读。

Method: 系统架构包含两部分:1)数值引擎解析原始记录、执行质量控制、执行已审查的光谱工作流并写入机器可读产物;2)LLM仅接收紧凑的允许列表摘要和版本化上下文包,描述七通道硬件、已审查工作流、结果字段、实现边界、科学限制和参考案例。原始EEG和密集每样本数组保留在本地。

Result: 三级评估结果:1)12个记录在十次数值重复中产生相同结构化结果,完整Rest/Task运行在三次重复中产生相同结果、报告和图形哈希;2)请求捕获和故障注入实验确认了数据边界和本地产物在HTTP、畸形输出和连接故障下的保存;3)边界感知基准测试在四种上下文消融和两个LLM下测试36个普通和对抗性问题,产生288个输出。

Conclusion: 硬件和实现感知的接地是校准EEG代理接受、限定或拒绝内容的实用机制,但不建立临床有效性或经验证的绝对认知负荷指标。
...
TL;DR: 本文构建了一个将AI系统解释为智能体的框架，融合了激进展观传统的哲学方法和机械论可解释性的工具。核心是解决给定系统计算事实后如何推断其信念、欲望和意义。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统部署增加，安全问题日益突出。需要建立对系统的信任，包括理解其目标或更务实地检测欺骗行为。这直接关系到AI安全。

Method: 本文提出表征主义和解释主义两种方法的标准，并将每种方法与当前可解释性方法能进行的测试联系起来。核心方法是强调信念、欲望和命题结构的联合约束，不能零散地做出归因。

Result: 关键发现是这些归因不能零敲碎打地完成。信念、欲望及其预设的命题结构是jointly constrained（联合约束）的，一种方法在测量其他维度时会产生扭曲。这种整体论对AI系统尤其紧迫，因为AI可能不共享解释者的概念。

Conclusion: 系统的态度约束其命题结构，该结构又约束哪些态度可以归因于此。机械论可解释性可帮助测量两者。需要采用整体论方法来可靠地解释AI系统的内部状态。
...
TL;DR: 本文提出PMDformer模型，解决长期时间序列预测中捕捉patches间形状相似性的挑战。核心创新包括：patch-mean decoupling (PMD) 分离趋势和残差形状信息；Trend Restoration Attention (TRA) 恢复趋势信息；Proximal Variable Attention (PVA) 关注近期相关时间片段。实验表明PMDformer在多个基准上优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: Transformer模型采用基于patch的策略捕捉长程依赖，但在建模不同scale的patches间形状相似性方面存在挑战。

Method: 提出patch-mean decoupling (PMD) 分离趋势和残差；设计Trend Restoration Attention (TRA) 恢复趋势同时计算注意力；设计Proximal Variable Attention (PVA) 关注近期相关变量关系。

Result: PMDformer在多个LTSF基准上优于现有最先进方法，具有更好的稳定性和准确性。

Conclusion: 通过PMD、TRA和PVA的协同作用，PMDformer能有效捕捉长期预测场景中的形状相似性，为时序预测提供了新的有效方法。
...
TL;DR: 本研究首次应用机器学习算法检测丙型肝炎患者的肝硬化，使用2038名埃及患者的数据集训练了四种模型，其中Extra Trees模型表现最佳，准确率达96.92%。


<details>
  <summary>Details</summary>
Motivation: 丙型肝炎会导致肝硬化，早期检测对避免并发症至关重要。尽管机器学习在其他疾病诊断中已显示有效性，但尚未有研究将其应用于丙型肝炎相关肝硬化的检测。

Method: 从加州大学欧文分校机器学习数据库获取包含28个特征的2038名埃及患者数据集，使用四种机器学习算法（随机森林、梯度提升机、XGBoost和Extra Trees）进行训练和诊断。

Result: Extra Trees模型表现最优，达到96.92%的准确率、94.00%的召回率、99.81%的精确率，以及96%的AUC值，且仅使用16个特征即实现该性能。

Conclusion: 机器学习算法可有效用于丙型肝炎患者肝硬化的诊断，Extra Trees模型在各项指标上均表现优异，具有较高的临床应用价值。
...
TL;DR: EvoOptiGraph是一个通过数据与模型共同演化来提升LLM优化建模能力的框架。


<details>
  <summary>Details</summary>
Motivation: 解决LLM优化建模的两个核心挑战：训练语料库缺乏结构多样性，以及数据生成管道与模型学习相脱节。

Method: 将混合整数线性规划(MILP)表示为属性二部图，应用保有效性演化算子生成结构多样性实例；通过监督微调(SFT)和带可验证奖励的强化学习(RLVR)两阶段训练，形成闭环持续更新训练分布。

Result: 在六个公共数据集上，EvoOptiGraph显著优于更大的通用模型、代理方法和专业基线，在准确性、可执行性和泛化性方面均表现出色。

Conclusion: 目标导向的数据-模型共同演化是提升LLM优化建模能力的有效策略。
...
TL;DR: 本文提出了一种多级验证和可追溯推理框架，用于提高AI生成的望远镜调度决策的可靠性。该框架通过数据引用验证、逻辑一致性检查和观测仪器约束验证来过滤和纠正无效决策，并将调度决策表示为相互关联的推理步骤序列。实验表明，该框架提高了AI调度的可执行性和可靠性，减少了 transient 机会的损失。


<details>
  <summary>Details</summary>
Motivation: AI在望远镜调度中的应用面临输出数据引用不一致、推理错误和不可执行决策等问题，限制了其在高可靠性观测任务中的应用。

Method: 提出了一个多级验证和可追溯推理框架，集成数据引用验证、逻辑一致性检查和观测仪器约束验证，引入了原子推理单元及其依赖关系来表示调度决策。

Result: 该框架提高了AI调度的可执行性和可靠性，减少了 transient 机会的损失，反馈纠正和结构化验证增强了修复和阻止错误决策的能力。

Conclusion: 该框架在保持灵活性的同时显著提高了可靠性和可执行性，为AI在高可靠性天文观测调度中的应用提供了一条可行且可验证的途径。
...
TL;DR: 该论文提出了一种基于大型语言模型(LLMs)的自动化邮件分发系统,用于根据邮件内容将电子邮件自动路由到工程院校各学期学生的相应WhatsApp群组,实现信息的高效传递。


<details>
  <summary>Details</summary>
Motivation: 手动处理大量电子邮件并将其转发到即时通讯平台不仅容易出错,而且耗时,导致生产力下降和额外压力。现有的人工分发方式效率低下,无法满足大规模组织的信息传递需求。

Method: 该系统采用基于代理(agents)查询大型语言模型的架构,通过精心设计的代理框架提示(包含邮件内容、指令和上下文),LLM分析邮件文本内容并决策应将邮件分发到哪个学生群组,无需依赖标注数据集。

Result: 系统能够自动分析邮件内容并将信息准确路由到相关的学生WhatsApp群组,实现信息的及时传递。

Conclusion: 该自动化邮件分发系统无需标注数据集即可工作,具有提高生产力和降低阅读邮件认知负担的优势,为组织提供了一种高效的信息传递解决方案。
...
TL;DR: 本文提出了一种结合大语言模型(LLM)、统计技术与人机协作的多语种客户反馈分析方法，旨在帮助税务等公共部门识别新兴服务质量和潜在不公平问题。该框架通过微调量化LLM与专家监督相结合，在保持计算效率的同时提高分析准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 公共部门尤其是税务机构的服务反馈分析面临挑战：反馈量不断增长，传统人工审核或静态指标方法难以规模化，且无法捕捉文本中的复杂模式。同时需要识别可能存在的不公平服务问题，但现有方法受限。

Method: 提出了一种整合LLM、统计技术与人机协作的新方法框架。具体包括：1)使用微调量化的大语言模型；2)引入专家监督机制；3)结合相似性分析和税务官员评估进行验证；4)采用人在环框架减少LLM幻觉输出。

Result: 实验表明，该方法与专家判断的对齐程度优于基线模型，有效降低了LLM虚构内容的产生，提高了生成洞见的可靠性和相关性。验证了LLM与人类专家结合支持公共部门规模化循证决策的可行性。

Conclusion: 该研究推动了负责任AI系统在公共部门的应用，通过更有效地分析多语种客户反馈，有助于提升服务质量、响应速度、公平性和公众信任，为公共部门提供了一种可扩展的循证决策支持方案。
...
TL;DR: 该论文提出了一种用于智能体安全的形式化策略自动生成管道，使用基于LLM的生成器-批评者循环将智能体提示、MCP工具描述和自然语言策略文档转换为Cedar策略语言的形式化验证策略，并在MedAgentBench基准上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 高风险领域的智能体安全需要形式化策略 enforcement，但现有方法要么依赖概率guardrails（无法提供形式化保证），要么依赖手工编写的符号enforcement（无法扩展）。需要一种可扩展且能覆盖实际策略规范广度的形式化策略 enforcement方法。

Method: 提出了一种自动形式化管道，包含：1）使用基于LLM的生成器-批评者循环；2）将智能体提示、MCP工具描述和自然语言策略文档转换为形式化策略；3）生成的策略使用Cedar策略语言编写；4）通过批评者组件进行形式化验证。

Result: 在MedAgentBench基准上，自动形式化策略比先前工作中手工编写的符号enforcement覆盖了更多的源自然语言规范。

Conclusion: 该方法为智能体安全提供了一种可扩展的形式化策略生成方案，能够自动将自然语言策略转换为可验证的形式化策略，并在基准测试中展现出优于手工编写方法的覆盖范围。
...
TL;DR: 本文提出SkillDisCo框架,用于从成功轨迹中提取可重用的参数化控制流子图(PFSM),并将其编译为可调用、可执行和可验证的程序化技能,解决Agent重复解决相似任务导致的推理成本高和执行轨迹长的问题。


<details>
  <summary>Details</summary>
Motivation: 当前Agent在解决相似任务时经常从头开始推理,导致不必要的推理成本和冗长的执行轨迹。现有工作探索了工作流重用和可执行技能诱导,但尚不清楚哪些任务场景支持程序化技能,以及如何在成功轨迹中表示共享的程序化结构。

Method: 在FSM定义场景中,将成功轨迹视为未知转换图中的路径,将程序化技能表示为可重用的参数化控制流子图(PFSM)。提出SkillDisCo框架:1)从成功轨迹中提取可重用的PFSM子图;2)将其编译为可调用、可执行和可验证的程序化技能。

Result: 在ALFWorld和WebArena上的实验表明,SkillDisCo在基准测试和模型规模上提高了成功率并减少了Agent回合数,证明了将共享经验表示为可重用执行结构的价值。

Conclusion: SkillDisCo框架能够有效从成功轨迹中提取和重用程序化技能,在多个基准测试上提升Agent性能,同时降低推理成本,为构建更高效的Agent系统提供了新思路。
...
TL;DR: 本文提出了NebulaExp，一个基于Qwen3-8B-base的透明后训练pipeline，包含3.84M SFT样本和200K RL候选池，采用三阶段SFT、GRPO强化学习和OPD方法。针对指令跟随和推理两个分支分别进行训练，最终在8B规模上实现了可复现的后训练配方。


<details>
  <summary>Details</summary>
Motivation: 后训练对齐决定了LLM的推理和人类偏好能力，但现有工作缺乏详细的数据构建、过滤规则和训练配方，阻碍了社区可复现性和轻量级模型优化。

Method: 构建了包含响应蒸馏、多维交叉验证、难度分级、任务分类和多样性采样的端到端数据处理栈。采用三阶段优化的监督微调、GRPO强化学习，以及单教师和多教师OPD方法。包含两个正交模型分支：通用指令模型和复杂推理专用模型。

Result: 指令分支：三阶段SFT将平均分数从55.01提升到60.99，GRPO进一步提升到61.85。推理分支：中等难度GRPO将推理分数从73.88提升到75.17。OPD使用4K样本在IFEval上比RL基线高3.26分，总分+4.43；MOPD使用10K样本比基线提升4.18分。

Conclusion: 提供了8B规模LLM的完全可复现后训练配方，全面分析了指令遵循、数学推理、代码生成和通用知识之间的能力权衡。
...
TL;DR: LeanGuard是一种无需链式推理（CoT）的轻量级内容审核Guardrail模型，仅用一个单次前向传播的395M参数编码器即可达到与大型推理Guard相当的审核效果，推理计算量减少约100倍。


<details>
  <summary>Details</summary>
Motivation: 当前Guardrail方法普遍采用链式推理（CoT）来做出判断，但这种设计使模型变得笨重且缓慢，不适合在设备端（如机器人）部署。作者质疑：安全Guardrail是否真的需要推理？

Method: 在相同语料库上训练一个轻量级双向编码器和一个推理Guard，然后仅移除推理部分，保持其他所有条件不变，进行受控的同基比较实验。

Result: LeanGuard在公开基准测试上达到平均F1 82.90±0.26，与基于更大解码器的推理Guard相当，但只需最多512个token的单次前向传播。推理计算量减少约100倍。

Conclusion: 链式推理并未提升内容审核准确性；当训练数据存在噪声时，标签-only编码器更加鲁棒，在严格误报率下保持更高的召回率。这表明当前Guardrail基准测试可能不够困难，未能奖励推理能力，CoT对内容审核的必要性仍未被证明。
...
TL;DR: 本文提出了一种名为Kalman Prototypical Network (KPN)的度量少样本学习框架，用于联合循环燃气轮机(CCGT)的故障诊断。该方法通过将类原型建模为动态系统中的潜在随机状态来减少情景方差并提高嵌入表示的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 联合循环燃气 turbines(CCGT)在现代发电中具有重要地位，但其复杂的热流体和机械相互作用使得故障检测变得困难，尤其是在标记故障数据稀缺的情况下。传统方法难以有效处理这类小样本故障诊断问题。

Method: 提出Kalman Prototypical Network (KPN)，将类原型的演化建模为动态系统中的潜在随机状态。采用基于Modelica的高保真动态模拟生成合成数据集，模拟海上CCGT系统的正常运行和瞬态条件下的渐进泄漏故障。在不同支持和查询配置下评估框架性能。

Result: KPN在准确性和稳定性方面均优于传统的少样本学习方法(如Matching Networks、Relation Networks和MAML)。该框架通过稳定类表示显著提高了训练收敛性和泛化能力。

Conclusion: KPN框架特别适用于标记数据有限的真实世界CCGT故障检测场景，为解决小样本故障诊断问题提供了一种有效的新方法。
...
TL;DR: LithoDreamer是首个用于计算光刻的物理信息世界模型(WM)框架，将光刻流程建模为决策驱动的多步演化系统，捕捉相邻状态间的特征变化来构建物理信息潜在空间，实现可解释的干预优化。


<details>
  <summary>Details</summary>
Motivation: 随着半导体技术节点缩小，计算光刻对确保良率和性能至关重要。但光刻是涉及掩模优化、光学成像、抵抗曝光和显影的连续物理过程，现有模型无法完整捕捉这一复杂流程。

Method: 提出LithoDreamer框架，将"布局-掩模-显影后图像(ADI)"管道表述为决策驱动的多步演化系统。捕捉相邻状态间的特征变化来建模阶段特定的物理信息潜在空间，控制过程干预探索并驱动后续状态转换。为实现无需连续监督的可解释干预优化，提出对比变分优化范式，将干预路径间的潜在差异与变分演化约束进行对比。

Result: 实验表明LithoDreamer在前向演化和逆向规划中达到最先进性能。光刻数据集已公开于GitHub。

Conclusion: LithoDreamer成功实现了计算光刻的物理信息世界模型建模，为光刻过程的正向演化和逆向规划提供了有效框架。
...
TL;DR: 提出了一种潜在大脑动力学模型，利用神经网络常微分方程和基于图的网格自动编码器，对双心室解剖结构和全周期 cine 运动进行连续潜轨迹编码，用于预测incident心力衰竭。


<details>
  <summary>Details</summary>
Motivation: 常规风险模型仅使用选定心动周期中的一些图像衍生指标，而 CMR 捕获了关于心室结构和运动的丰富时空信息，现有方法未能充分利用这些信息。

Method: 使用心率感知的神经 ODE 动力学和基于图的网格自动编码器重建解剖一致的 3D+t 心室运动；协变量条件先验定义预期的舒张末期潜状态；Cox 比例风险模型检验偏离该先验是否能预测心力衰竭。

Result: 72,386名UK Biobank参与者中367例incident心力衰竭事件；将潜在大脑评分添加到重新拟合的集合队列方程中，使分层C指数从0.704提高到0.785，而7个既定心脏标记物为0.764；该模型在重建保真度、生成真实性和下游预后性能之间取得最佳平衡。

Conclusion: 心室运动的连续全周期建模提供了超越传统CMR摘要的信息性心脏表型，但在临床风险预测使用前需要在更具代表性的患者队列中进行外部验证。
...
TL;DR: AHOIS是一个嵌入苏格拉底式追问的多智能体AI科学家系统，能够在复杂物理环境中进行自主科学发现，在多模态光纤平台上实现了高精度的图像分类任务。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统虽然能够操作仪器、优化参数和生成假设，但仍然停留于程序化执行固定工作流的阶段，缺乏真正的认识论自主性——即根据证据构建、挑战和修正物理解释的能力。

Method: AHOIS是一个多智能体AI科学家系统，将苏格拉底式产婆术嵌入闭环实验。物理批评智能体通过因果追问、约束检查、反例生成和伪证标准制定来审视假设。系统在多模态光纤光学平台上进行评估。

Result: 系统在没有先验编码方案、分类器或散斑模型的情况下，自主提出并验证了随机干涉编码假设，发现了任务自适应的稀疏测量策略，诊断出不同的失败模式（编码不稳定、荧光污染和探测器噪声），并实现了16x16测量，有效秩56.9，MNIST分类准确率76.97%，Fashion-MNIST分类准确率83.17%。消融实验表明苏格拉底式追问提高了物理一致性、假设完整性、不确定性校准和实验计划有效性。

Conclusion: 这些结果建立了从工作流自动化向复杂物理环境中基于证据的自我修正自主发现的发展路径。
...
TL;DR: 本文提出EGG框架，一个利用专家优化指导的LLM代理系统，用于自动生成高性能GPU内核。通过两阶段分层设计（算法结构设计+硬件特定调优）和多代理协作机制，实现自动化内核优化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的计算成本呈指数级增长，手动调优GPU内核依赖领域专家，耗时且效率低。现有LLM自动生成方法难以同时保证正确性和高性能，主要受限于缺乏领域特定的优化指导。

Method: 提出两阶段分层框架：1)算法结构设计阶段建立高质量计算结构；2)硬件特定调优阶段通过并行映射、tensor tiling和内存优化进行针对性调整。设计阶段感知的多代理协作机制管理阶段内和阶段间的上下文。

Result: 在KernelBench和实际工作负载上的实验表明，EGG相比PyTorch平均加速2.13倍，优于现有基于代理和强化学习的方法。

Conclusion: EGG框架通过将专家优化知识融入LLM决策过程，结合分层设计和多代理协作，成功实现了自动化GPU内核生成的突破，为LLM的高效部署提供了可行路径。
...
TL;DR: 本文提出ResilPhase框架，通过在常微分方程空间中稳定地进行宏轨迹外推，结合无导数重心拉格朗日外推器和有界相位映射，解决扩散模型加速推理中的质量退化问题。


<details>
  <summary>Details</summary>
Motivation: 当前缓存-预测方案通过基于导数多项式加速DiT，但在高加速比下产生严重质量退化。问题根源在于：与连续扩散轨迹不对齐的离散外推具有数值不稳定性，导致空间误差积累、导数噪声放大和高阶不稳定。

Method: 提出三大创新：(1)将加速推理重新表述为ODE空间中的稳定宏轨迹外推，与全局漂移(GD)对齐而非预测中间特征；(2)引入无导数重心拉格朗日外推器绕过导数不稳定性；(3)提出有界相位映射正则化外推域，抑制振荡误差增长。

Result: 在FLUX.1-dev和HyuanVideo上实现高加速比下的最优保真度。

Conclusion: ResilPhase通过消除特征不一致和内存开销，绕过导数不稳定性和近似误差，为扩散模型的高效推理提供了噪声弹性加速框架。
...
TL;DR: 研究语言代理的持久行为学习问题，提出"记忆深度"概念和EVAF机制，让代理在上下文卸载后仍能保持目标导向行为。


<details>
  <summary>Details</summary>
Motivation: 需要评估EVAF机制在GPT-2和TinyLlama上的目标持久性和卸载后恢复能力，使用loop-drift协议进行压力测试，并通过Memora事件流进行外部诊断。

Method: 引入loop-drift协议作为受控压力测试，评估EVAF惊喜和valence门控的LoRA整合机制，通过选择性整合的两个维度（选择和执行）进行机制控制实验。

Result: EVAF机制在目标持久性任务上优于检索系统，记忆深度与检索访问形成互补。选择性整合的因素可分解为选择和执行两个维度进行控制。

Conclusion: 选择性参数整合提供了区别于检索访问的"记忆深度"，是检索系统的补充而非替代。记忆深度研究仍面临 stale-memory 失效这一未解决问题。
...
TL;DR: 提出一种让大型语言模型在 token 生成过程中自动从知识库检索事实知识的新方法，使事实更新无需重新训练模型。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的事实知识更新困难且需要大量计算资源重新训练；模型输出难以追溯和解释；小模型的事实准确性通常低于大模型。

Method: 训练模型生成特殊 token 来触发对知识库的查询，实现事实知识的动态检索。

Result: 该方法在短文本和长文本生成中均提升了事实 grounding 能力，事实修订可通过知识库编辑而非参数更新实现。

Conclusion: 该方法实现了无需重新训练即可更新 LLM 事实知识的能力，增强了可追溯性和可解释性，并使小模型能达到与大模型相当的事实准确性。
...
TL;DR: 该研究评估40名健康成人（30-50岁）的HRV指标，建立时域、频域和非线性参数的临床参考标准。


<details>
  <summary>Details</summary>
Motivation: HRV是心脏疾病诊断的重要指标，但健康人群缺乏标准参考值，需建立可靠的正常基线数据。

Method: 采用计算方法分析时域、频域和非线性HRV指标，验证正态性、稳定性、相关性、可重复性和一致性五个问题。

Result: 时域和非线性指标呈正态分布且稳定；HF指标存在冗余；大部分指标可重复性好；推荐ApEn、IRRR、HRVi、SD2、MADRR/rMSSD作为最佳指标。

Conclusion: 所选指标可准确代表HRV各成分，增强HRV的临床和科研应用价值，为健康人群HRV评估提供参考标准。
...
TL;DR: 本文提出CASOP框架，用于在手动拣货仓库中构建和评估针对特定情境的优化管道。该框架通过模块化算法库、语义数据卡片、问题分类法、管道合成器和管道评估器，解决订单履约中 item分配、订单批处理和拣货路径决策的分解优化问题。实验在7个基准实例集上生成了超过100万个有效管道。


<details>
  <summary>Details</summary>
Motivation: 现有仓库系统因组织边界、责任分工或数据限制常需采用分解方法而非集成模型。现有研究主要评估孤立子问题或特定组合的算法，缺乏一种通用机制来确定适用算法配置、将其组合成有效解决方案管道并评估性能。

Method: 提出Context-Aware Synthesis of Optimization Pipelines (CASOP)框架，包含：(1)通用订单履约问题算法模块库；(2)描述仓库情境和算法需求的语义数据和算法卡片；(3)将订单履约问题结构化为相关子问题的分类法；(4)为给定仓库情境识别适用算法并组合所有有效优化管道的管道合成器；(5)评估所有生成管道的管道评估器。

Result: 在7个基准实例集（涵盖4个问题类）上演示框架，生成了1,063,044个有效管道。软件已开源发布。

Conclusion: CASOP框架支持研究人员和实践者为仓库运营设计、自动合成和选择有效的高性能算法管道，为仓库优化领域的算法选择和管道合成提供了系统化方法论。
...
TL;DR: 本研究提出了一种视角条件检索增强生成(RAG)框架，用于生命周期评估(LCA)的解释阶段，将量化改进机会转化为可操作的战略路径。


<details>
  <summary>Details</summary>
Motivation: LCA的解释阶段缺乏将环境热点转化为技术、社会和政策不确定性下可操作战略路径的结构化机制。

Method: 开发了包含学术、行业、公众话语和欧盟资助数据集的视角融合RAG架构，包含三步：(1)定义系统边界和脱碳目标的场景锚，(2)具有约束检索的视角特定微查询，(3)整合仅存储在分类账中的输出的中性合成步骤。以意大利苹果生产设施的氢能柴油减少用例演示，使用GPT-5 nano作为推理模型。

Result: 结构化检索和约束合成可降低幻觉风险，同时保留跨域多样性。该方法支持将影响结果更 disciplinedly转化为战略路径。

Conclusion: 本概念验证展示了AI辅助、证据支持的解释如何支持超越传统LCA的面向实施决策，为在LCA研究中，特别是大规模部署技术的研究中使用高级AI工具开辟了新途径。
...
TL;DR: 提出了AgentX，一个生产部署的多智能体系统，将推荐算法迭代从依赖人类工程师的手工过程转变为自动化的研究循环，实现自我改进


<details>
  <summary>Details</summary>
Motivation: 当前推荐算法迭代仍依赖人类工程师生成假设、修改代码、启动A/B实验并归因在线结果，创新只能随人数线性增长，无法随证据、计算和累积实验知识复合增长

Method: AgentX协调四个紧密耦合的阶段形成闭环：1)Brainstorm Agent综合历史实验、系统架构、数据分析和外部研究生成排名提案；2)Developing Agent将提案转化为生产就绪代码并进行多维可靠性验证；3)Evaluation Agent执行带护栏否决的A/B测试安全上线，将成功和失败转化为结构化知识；4)Harness Evolution层(SGPO)将执行轨迹提炼为语义梯度更新，持续提升智能体能力

Result: 系统作为一个自我进化的开发引擎，能够以任何手工工作流无法维持的规模和速度自主生成、实施、评估和学习推荐实验

Conclusion: 该系统不仅是自动化，而且实现了自我改进，从线性扩展转变为复合扩展
...
TL;DR: TAVR-VLM是一种用于TAVR术前规划的新型多模态大语言模型框架，通过引入风险条件因果接地注意力(R-CGA)机制，有效解决了多模态大语言模型在主动脉瓣置换领域面临的诊断幻觉问题。该框架在1482例患者的M³TAVR数据集上取得了先进性能，AUROC达0.896，CIDEr达0.936，幻觉率降至8.1%。


<details>
  <summary>Details</summary>
Motivation: TAVR(经导管主动脉瓣置换)术前规划需要细致的多模态推理，然而将多模态大语言模型(MLLMs)应用于这一高风险领域受到严重阻碍，主要问题在于诊断幻觉——生成的文本缺乏解剖学依据。

Method: 提出了一种名为风险条件因果接地注意力(R-CGA)的新框架，构建了模型内部的"风险→区域→词汇"结构接地路径。R-CGA将多模态输入压缩为因果风险瓶颈，将密集的视觉特征纯化为全局风险掩码。在自回归生成过程中，支持投影的因果一致性目标将 token级别的接地约束在风险定义的支持掩码内。

Result: 在M³TAVR数据集(1482例患者)上评估，TAVR-VLM建立了新的先进水平：AUROC达0.896，CIDEr提升至0.936，幻觉率大幅降低至8.1%，从而提高了循证外科AI的可解释性。

Conclusion: TAVR-VLM通过R-CGA机制成功创建了"风险→区域→词汇"的内部结构接地通路，有效减少了诊断幻觉，在TAVR规划的多模态大语言模型应用中取得了突破性进展，为循证外科AI提供了更高的可解释性。
...
TL;DR: 本文提出了一种用于生成合成临床笔记的模块化流水线及数据集，旨在支持临床AI工具开发，同时避免真实患者数据的隐私风险。该流水线结合了结构化患者生成、半结构化患者旅程模拟和基于大语言模型的非结构化临床笔记生成。发布了包含70名合成患者的数据集，每名患者有20-50份临床笔记。


<details>
  <summary>Details</summary>
Motivation: 医疗领域的临床文档因包含敏感信息而难以获取，限制了临床AI系统的开发与评估。真实患者数据的使用面临严格的隐私法规约束，因此需要高质量的合成数据来替代真实数据支持AI系统开发。

Method: 采用模块化流水线设计，包含三个主要阶段：1）结构化患者生成；2）半结构化患者旅程模拟；3）基于大语言模型的非结构化临床笔记生成。通过LLM验证和增强步骤提高生成笔记的真实性、现实性和多样性。数据集提供多个验证级别。

Result: 生成了包含70名合成患者的数据集，每名患者有20-50份临床笔记，涵盖完整的医院就诊旅程。数据集在多个验证级别下可用，用户可根据用例在现实性和可扩展性之间进行平衡。

Conclusion: 该合成临床笔记数据集可支持临床AI系统的开发、测试和评估，包括摘要工具、编码模型和决策支持系统等，且不依赖真实患者数据，有效解决了医疗数据隐私问题。
...
TL;DR: 本文提出模式保持属性检索任务，解决在保持种子集模式的同时提升目标属性的检索难题。通过MO-DiT+HPPO框架，结合度量有序训练和混合候选池偏好优化，在四个属性域上显著提升交集指标。


<details>
  <summary>Details</summary>
Motivation: 现有基于嵌入的检索方法无法解决模式保持与属性提升的矛盾：平均种子保持模式但属性低，全局属性检索会偏离原模式。需要在保持细粒度模式的同时提升目标属性的检索。

Method: 提出MO-DiT+HPPO分阶段框架：1)原始序列预训练；2)多域度量有序继续预训练；3)尾质心微调；4)HPPO混合候选池偏好优化。度量有序训练将稀疏在线检索标签转化为模式内轨迹，HPPO用在线交集度量标注混合候选池并应用参考锚定偏好优化。Pareto对过滤器保持同模式纯度高的胜对。

Result: 在四种属性域的项目和模式留出协议下，度量有序DiT相比预训练生成检索器提升交集指标，HPPO进一步提升，7/8域分割单元显著提升，1个最难分割单元平局。

Conclusion: 度量有序训练和HPPO偏好优化有效解决了模式保持属性检索问题，通过将稀疏在线标签转化为模式内有序轨迹学习度量改进方向，并通过对齐生成查询分布与真实在线目标实现显著收益。
...
TL;DR: ReTeX通过预测参数偏移量来恢复合并模型中各任务专家的性能，无需存储原始专家即可恢复95%以上的个体专家表现。


<details>
  <summary>Details</summary>
Motivation: 多任务模型静态合并存在参数干扰问题，而动态合并需要冗余专家组件的高成本存储和加载。

Method: 将参数干扰建模为仿射变换的加性偏移，预测偏移量以恢复专家性能；使用基于SVD子空间签名的无路由任务标识符识别任务身份。

Result: 在视觉和NLP领域均恢复了95%以上的个体专家性能，显著提升了对未见任务的泛化能力，并能自适应插值已见专家知识处理OOD任务。

Conclusion: ReTeX实现了高效的任务专家恢复，只需单一合并检查点即可，无需存储原始专家组件，为多任务学习提供了新的思路。
...
TL;DR: 本文研究大型语言模型作为长期决策智能体时的分布外泛化能力弱问题，识别任务不敏感性是失败根源，并提出任务扰动的负对数似然优化方法提升模型对任务指令的敏感度。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽能作为长期决策智能体，但其分布外泛化能力较弱，当面对相似但不同的任务时，模型倾向于沿用训练时学到的模式而非真正理解当前任务，这限制了模型在实际场景中的应用。

Method: 提出Task-Perturbed NLL Optimization（TPNLL），一种轻量级对比正则化器，通过显式鼓励智能体动作依赖于任务指令来缓解任务不敏感问题，并设计实验验证注意力机制的变化。

Result: 该方法显著提升了模型的任务敏感度和分布外泛化能力，同时使注意力更稳定地集中在任务token上，改变了训练时注意力从任务token漂移到局部观察的趋势。

Conclusion: 通过显式建模任务指令与动作之间的依赖关系，可以有效缓解LLM作为智能体时的捷径优化偏见问题，提升分布外泛化性能，为构建更可靠的语言智能体提供了新思路。
...
TL;DR: 研究Chain-of-thought (CoT)训练的实际效果，发现CoT训练主要改进的是模型直接根据提示预测行动的能力，而非通过生成推理改变行动的能力。


<details>
  <summary>Details</summary>
Motivation: CoT推理在语言模型代理中广泛使用，但先前工作表明口述的CoT不一定忠实，可能反映的是事后推理（即模型在推理前就知道答案）。因此需要研究CoT训练实际上在改进什么。

Method: 通过比较"prompt actions"（不使用CoT预测行动）与"CoT actions"（使用CoT预测行动）来研究这一问题，并在训练中选择性屏蔽部分样本的action-token监督。

Result: 1. 随着checkpoint增加，prompt-action质量显著提高；2. CoT actions相对于prompt actions的优势在环境交互中保持相似；3. 后期checkpoint更少因CoT而修改行动，显示出更强的prompt依赖性；4. 选择性屏蔽action-token监督的干预提高了域外泛化能力。

Conclusion: CoT训练并未显著扩大CoT推理的优势，而是主要改进了模型直接根据提示预测行动的能力；通过选择性屏蔽action-token监督可以提高模型的域外泛化性能。
...
TL;DR: 研究动态3D故事世界中的摄像机规划问题，提出Look-Before-Move框架，使摄像机能够主动决定观察什么、如何构图以及如何转移注意力，而非被动响应给定观察。


<details>
  <summary>Details</summary>
Motivation: 随着具身AI和世界模型在动态3D环境中运行，视觉感知需要从被动解释给定观察转向主动决定观察什么。这是通过动态3D故事世界中的摄像机规划问题来研究的。

Method: 提出Look-Before-Move框架，包含三个关键组件：(1)语义观察契约(Semantic Observation Contract)将导演意图转换为可执行的视觉约束；(2)蒙特卡洛视角搜索(Monte Carlo Viewpoint Search)寻找叙事一致且几何可行的视角；(3)语义轨迹接地(Semantic Trajectory Grounding)将选定的视角连接成连续、碰撞感知且时间连贯的摄像机运动。

Result: 基于StoryBlender构建了包含50个故事、457个场景和1585个shots的动态3D故事世界基准。实验表明该框架在主体感知、意图一致性和轨迹质量方面优于代表性基线方法。

Conclusion: Look-Before-Move框架证明了在生成摄像机运动之前组织视觉注意力的重要性，为动态3D环境中的智能摄像机规划提供了有效方案。
...
TL;DR: 论文提出Einstein World Models (EWMs)框架，通过在推理轨迹中嵌入视觉-时序 rollout，使LLM能够进行可视化思维实验，从而超越纯文本推理的局限。


<details>
  <summary>Details</summary>
Motivation: 智能是否需要超越直接经验进行推理？复杂思维能否仅通过语言捕捉？视觉化反事实事件能否作为语言的补充机制？LLM能否被训练利用这种视觉化机制来提升推理能力？

Method: 提出EWM蓝图：LLM调用world-module生成场景的短视觉rollout，返回的rollout作为可检查的假设而非最终答案，支持后续推理。EWM将工具调用能力（如网页搜索、代码执行）扩展到视觉思维实验领域。

Result: 建立了一个将视觉-时序rollout纳入推理轨迹的LLM推理系统框架，使LLM能够进行可视化思维实验来推理反事实场景。

Conclusion: Einstein World Models为LLM提供了一种新机制，通过可视化思维实验增强复杂推理能力，是对纯文本推理的重要扩展。]
...
TL;DR: 本文提出AURORA-AI框架,一种基于自适应效用驱动的资源编排方法,统一Hamilton-Jacobi-Bellman反馈控制、Lyapunov稳定性监控和公平性感知复合效用,在非稳态条件下持续重新分配计算资源,使全局效用(预测性能、人口统计公平性、成本、延迟、鲁棒性、可解释性联合优化)保持最大化。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统常部署在非稳态的计算、人口统计和操作环境中,静态资源分配策略会导致预测性能和公平性、可解释性等人文属性下降,需要在 disruptions 下实现弹性部署。

Method: AURORA-AI框架结合Hamilton-Jacobi-Bellman反馈控制、Lyapunov稳定性监控和公平性感知复合效用,形成闭环策略,在离散时间模拟中同时注入人口统计偏见冲击、渐进概念漂移和突发黑天鹅 disruptions,与Static、Round Robin、Greedy、LinUCB、PPO等五种控制器对比。

Result: AURORA-AI实现黑天鹅事件即时恢复(alpha量子提升29%、super量子提升25%),同时降低人口统计公平性差距,增加Lyapunov稳定运行步骤。

Conclusion: 基于稳定性理论的公平性自适应编排是实现弹性人文中心AI部署的实用且理论驱动的路径。
...
TL;DR: 本文提出JERP方法，用于解决LLM智能体在多步骤交互环境中如何有效利用累积交互经验的问题。该方法将经验规则池和策略从相同的交互轨迹中联合学习，在决策时检索相关规则并结合交互历史来条件化智能体。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将交互经验的利用分离为两种方式：作为自然语言规则用于提示，或通过轨迹更新模型参数。前者易于解释但可能与演化策略不同步；后者能更广泛改进策略，但在稀疏奖励设置下对局部错误的修正有限。

Method: JERP方法包含三个关键步骤：1) 在决策时检索任务相关规则并与交互历史结合来条件化智能体；2) 收集轨迹用于优化策略；3) 通过比较当前 rollout 与参考成功轨迹来修订规则池。这种耦合使规则池与演化策略保持对齐，同时允许稳定有效的行为逐渐被吸收到模型中。

Result: 在AlfWorld和WebShop上的实验表明，JERP在复杂交互任务的决策性能上获得了一致的提升。

Conclusion: JERP成功地将经验规则学习与策略学习相结合，解决了两种传统方法分离的局限性，为LLM智能体在多步骤交互环境中的有效学习提供了新的思路。
...
TL;DR: 本文提出PAVE协议和OpenRCA 2.0基准，用于评估LLM代理的根因分析能力。与现有数据集不同，该基准包含从原因到症状的因果传播路径标注，而非仅标注根因。


<details>
  <summary>Details</summary>
Motivation: 现有RCA数据集仅标注根因，缺少连接根因与观察症状的传播路径，导致任务简化为简单的模式匹配，无法全面评估LLM代理的长上下文理解、多步推理和工具使用等能力。

Method: 提出PAVE分步标注协议，利用故障注入的已知干预来重建因果传播路径，遵循从原因到效果的前向验证推理方式。构建OpenRCA 2.0基准，包含500个跨系统实例，首创分步因果标注。

Result: 在11个前沿LLM中测试显示：精确根因集恢复平均仅成功20.7%；宽松标准下，"无依据诊断"中76.0%能识别正确根因服务，但仅61.5%能在验证的因果传播路径中将其与症状关联。

Conclusion: 仅评估结果会掩盖LLM代理的真实失败模式；分步因果标注是实现可信RCA代理的关键缺失环节。
...
TL;DR: 本文提出TOPS方法，从第一性原理角度重新审视视觉token剪枝，通过信息论分析识别出任务相关性、信息覆盖和语义多样性三个基本原则，在移除77.8%视觉token的同时保持甚至提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因大量视觉token导致计算开销大，效率受限。现有视觉token剪枝方法存在缺陷：基于注意力的标准倾向于保留冗余token，基于多样性的标准忽略用户指令，缺乏对token剪枝内在目标的 principled 定义。

Method: 从第一性原理角度将视觉token剪枝形式化为构建Token Optimal Preservation Sets。通过自顶向下的信息论分析，识别出三个基本原则：任务相关性、信息覆盖和语义多样性。基于这些原则，提出TOPS——一种无需训练、模型无关的剪枝模块，可应用于各种MLLMs。

Result: 在7个MLLM backbone和14个benchmark上进行了广泛实验。TOPS在各种剪枝设置下优于先前方法。在LLaVA-NeXT上，TOPS移除了77.8%的视觉token，同时分别在7B和13B模型上保持100.0%和100.6%的性能。

Conclusion: 本文表明剪枝冗余视觉token有时可以缓解幻觉，并为未来轻量级MLLM设计提供启示。TOPS作为一个无需训练、模型无关的剪枝模块，在效率和性能之间实现了良好的平衡。
...
TL;DR: 流程驾驭（Process Harness）是一种将传统工作流升级为智能业务流程管理（Agentic BPM）的新机制，无需替换底层工作流引擎。该机制通过在确定性工作流引擎周围放置一个策略治理的智能层，在指定的控制点拦截并贡献推理、适应和监督。


<details>
  <summary>Details</summary>
Motivation: 解决如何在不替换遗留工作流引擎的情况下，将传统工作流升级为具有智能代理能力的现代业务流程管理系统，同时保留确定性引擎的结构化权威和合规性。

Method: 提出任务-决策-流（TDF）模型，定义数据模式和执行语义。TDF将LLM推理分解为三种策略治理的代理类型：TaskAgent（知识密集型任务执行）、DecisionAgent（按案例网关路由）、FlowAgent（通过原则性钩子机制管理运行时流适配）。每种代理在显式策略下推理，这些策略来自管理系统中所有LLM调用的过程FRAME聚合策略集。实现CUGA FLO设计和部署。

Result: 成功在贷款审批工作流上演示了三种代理类型和钩子驱动的监管覆盖，验证了流程驾驭机制能够协调强制性要求（通过确定性工作流执行实现结构化合规）和规范性要求（通过策略治理的智能代理自主性实现）。

Conclusion: 流程驾驭机制独特地调和了命令式要求和规范性要求，为遗留工作流向智能业务流程管理的演进提供了一种可行的升级路径，同时保持了结构化权威和合规性。
...
TL;DR: GAversary是一种混合遗传算法，用于对NLP模型生成对抗性攻击，将目标模型视为黑盒，仅使用logit值指导搜索，利用GloVe嵌入提出词替换以提高对抗样本的语义相似性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在各种领域取得了令人印象深刻的性能，但在NLP领域容易受到对抗性输入的攻击，这些攻击可能产生重大的现实世界后果。现有的对抗攻击方法通过针对特定脆弱词来提高精度，但通常需要访问模型内部结构。

Method: 提出GAversary，一种混合遗传算法(GA)来生成对抗性攻击。该算法将目标模型视为黑盒，仅需要模型的logit值输出作为搜索指导。与先前用于此问题的GA不同，GAversary使用GloVe嵌入来提出词替换(突变算子)，以提高对抗样本的语义相似性。

Result: GAversary能够显著降低目标模型在测试数据上的准确率。与BAE和A2T攻击相比，GAversary在最佳情况下将76.8%的准确率降低到5.8%，而BAE仅降至27.6%。代价是GAversary扰动的词数量略多于其他两种方法的两倍，语义相似性略低，运行时间增加约5%。

Conclusion: GAversary是一种有效的黑盒对抗攻击方法，在攻击效果上显著优于BAE和A2T，但存在扰动词数量较多和语义相似性略低的权衡。
...
TL;DR: BINEVAL是一种无需训练的任务无关框架，通过将评估标准分解为细粒度的二元原子问题来评估LLM输出，然后聚合这些判断生成可解释的多维度评分。


<details>
  <summary>Details</summary>
Motivation: 当前LLM输出评估面临三大瓶颈：人工评估昂贵且缓慢、词汇指标与开放生成的人类判断相关性差、整体LLM评判产生不透明的分数难以调试。

Method: 给定任务提示，元提示生成细粒度评估问题，LLM独立回答每个输出相关的问题，生成透明的问题级反馈和校准的总体分数。分解使评估更容易检查、诊断，并直接用于提示改进。

Result: 在SummEval、Topical-Chat和QAGS上，BINEVAL匹配或优于包括UniEval和G-Eval在内的强基线，在QAGS等事实一致性基准上表现尤其出色。BINEVAL更好地匹配人类分数分布，避免了先前LLM评判的天花板效应，能更好地区分边缘和有明显缺陷的输出。

Conclusion: BINEVAL提供了任务无关、无需训练且可解释的评估框架，结合了强大的实证性能与实用的诊断和优化价值。
...
TL;DR: EO-WM是一个用于多光谱地球观测预测的视频扩散变换器，将天气作为条件信号，通过物理信息化的条件框架（包括气候基线、天气异常和累积物理应力信号）来预测地球表面动态。


<details>
  <summary>Details</summary>
Motivation: 现有方法未能充分捕捉部分观测、天气驱动的世界建模问题：确定性模型将不确定性简化为单一预测，扩散方法将天气变量视为无差异的条件信号，现有的基准测试主要关注重建精度而非对改变的天气的响应正确性。

Method: 提出EO-WM模型，包含物理信息化的条件框架，通过气候基线、天气异常和累积物理应力信号表示气象强迫；通过不同的条件路径分离基线和异常，并累积异常强迫以捕捉持续的热度和干旱胁迫。

Result: EO-WM在预测NDVI下降幅度误差上相对降低5.63%，在方向命中率上相对提高7.80%，同时在标准像素级指标上保持竞争力。

Conclusion: EO-WM能够更好地响应变化的天气强迫，为极端夏季植被退化和季节性天气变化下的预测提供更准确的预测效果。
...
TL;DR: 研究将模拟-based inference (SBI) 神经后验估计作为MCMC的替代方法,用于贝叶斯校准COVID-19重症监护病房occupancy的SECIR流行病学模型。


<details>
  <summary>Details</summary>
Motivation: 传统的MCMC方法在处理高维非线性系统和实时分析时计算成本过高,需要更快速的贝叶斯校准方法。

Method: 使用SBI神经后验估计对德国2020年COVID-19 ICU occupancy数据进行贝叶斯校准,与MCMC在多个流行阶段进行比较,包括31天推理窗口和201天重建问题。

Result: SBI与MCMC在31天窗口中取得强一致的后验分布,准确重现观察到的ICU轨迹。201天设置中SBI保留了主要后验结构。计算时间方面:31天问题SBI约60-70秒 vs MCMC约1000秒;201天问题SBI约157秒 vs MCMC超过19000秒。

Conclusion: SBI提供了快速且计算高效的贝叶斯校准框架,可支持重复的近实时推理和快速爆发分析。
...
TL;DR: 研究大型语言模型在招聘筛选中易受提示注入攻击，分析其在不同候选人人质量和注入比例条件下的效果和公平性问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型日益被用于自动筛选和排名求职者，创建了候选人策略性操纵算法招聘系统的动机和incentives，需要研究这种新型操纵方式的危害。

Method: 通过受控实验的方法，向简历中嵌入微妙的自我推销文本（提示注入），在不增加新资质的情况下影响LLM评估。

Result: 提示注入在候选人质量同质且注入者较少时可靠地改善排名，但随着注入者增多效果迅速下降；在候选人质量异质时效果降低但仍可能让低质量候选人超越高质量候选人，引发公平性担扰。

Conclusion: LLM筛选在操纵罕见且候选人质量差异小时最脆弱；建议在部署LLM筛选系统时需要考虑防御提示注入攻击。
...
TL;DR: 研究多模型LLM系统（如路由、投票、级联、融合、混合代理）的增益上限，发现传统诊断指标rho无法识别真正的限制因素beta（所有模型同时错误的比率），并通过67个模型实验证明当前方法严重低估了all-wrong tail。


<details>
  <summary>Details</summary>
Motivation: 多模型LLM系统被广泛用于提升单一模型准确率，但该领域很少报告限制增益的关键指标。作者旨在揭示并量化这一被忽视的核心限制。

Method: 提出beta作为所有模型同时错误的比率指标，建立Clopper-Pearson bound提供有限样本证书；使用四因素校准的单因子模型和67个模型的高斯copula进行对比实验；在GPQA-Diamond数学题和代码执行题上测试，并通过自由响应vs多项选择形式检验答案格式的影响。

Result: 在开放数学题上观察到beta=0.052，而高斯copula下为0.023（约2.5倍低估，90% CI 1.7-3.4，k=17）；代码执行题上beta=0.079；改为自由响应形式后beta升至0.127；低rho异构集成优于高rho Self-MoA，但在无强查询级路由信号时，组合模型很少超越最佳单一模型。

Conclusion: 增益来自模型在不同问题上的失败差异，而非增加模型数量；需报告beta而非rho作为关键诊断指标；当前系统对all-wrong tail严重低估；路由信号对比组合更重要。
...
TL;DR: 提出一种基于语言的心理健康数字孪生框架，利用大语言模型模拟老年人的对话行为，通过风格线索和上下文元数据进行个性化建模，并使用多条件变分自编码器评估保真度和认知评分预测能力。


<details>
  <summary>Details</summary>
Motivation: 轻度认知障碍(MCI)的早期检测具有挑战性，语言和对话模式可作为无创生物标志物，数字孪生技术为个性化医疗和连续认知健康监测提供了新范式。

Method: 1)构建基于LLM的语言数字孪生框架，整合风格线索和上下文元数据；2)提出多头条件变分自编码器(cVAE)联合测量重建质量和预测认知评分(MoCA)；3)在I-CONECT数据集上验证。

Result: 数字孪生保留了个体特异性特征，重建误差和MoCA预测误差与真实数据相当，且优于GPT基线生成的响应。

Conclusion: 基于语言的数字孪生是一种可扩展、无创的个性化连续认知健康监测方法，具有临床应用潜力。
...
TL;DR: Kiko是一种新的基于协议的多智能体系统编程模型，通过决策器抽象来实现智能体的内部决策逻辑与公共决策的桥梁，并确保协议合规性。


<details>
  <summary>Details</summary>
Motivation: 当前的多智能体编程模型对决策的抽象能力不足，无法很好地将智能体的内部决策逻辑与其公共决策（发送消息）结合起来，开发者需要处理底层通信服务的细节。

Method: 提出Kiko编程模型，开发者编写决策器（decision makers）从有效决策集合中选择并做出相互兼容的消息发送决策。提供Kiko的操作语义并证明其协议合规性。

Result: Kiko能够完全抽象掉底层通信服务，支持实际的决策模式，使智能体开发者专注于业务逻辑，并能够实现任意协议 enactment。

Conclusion: Kiko为多智能体系统开发提供了一个良好的抽象层次，既保证了协议合规性，又提供了足够的灵活性来支持各种决策模式。
