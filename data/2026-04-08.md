...
### [1] [Rethinking IRSTD: Single-Point Supervision Guided Encoder-only Framework is Enough for Infrared Small Target Detection](https://arxiv.org/abs/2604.05363)
*Rixiang Ni,Boyang Li,Jun Chen,Yonghao Li,Feiyu Ren,Yuji Wang,Haoyang Yuan,Wujiao He,Wei An*

Main category: cs.CV

TL;DR: 本文将红外小目标检测(IRSTD)重新定义为质心回归任务，提出基于单点监督的红外概率响应编码方法(SPIRE)，通过点响应先验监督(PRPS)将单点标注转换为概率响应图，并设计高分辨率概率编码器(HRPE)实现端到端回归，在SIRST-UAVB和SIRST4数据集上实现了具有低误报率的竞争性检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用像素级监督的编码器-解码器分割范式，但忽略了两个关键问题：1)小目标仅占很少像素；2)目标边界因杂波背景而模糊。作者认为IRSTD的首要原则应是目标定位，而非分离所有目标区域伴随的不可区分背景噪声。

Method: 1.点响应先验监督(PRPS)：将单点标注转换为符合红外点目标响应特征的概率响应图；2.高分辨率概率编码器(HRPE)：保留高分辨率特征，增加有效监督密度，实现仅编码器的端到端回归，无需解码器重建。

Result: 在SIRST-UAVB和SIRST4等多个IRSTD基准数据集上，SPIRE实现了具有竞争力的目标级检测性能，具有一致的低误报率(Fa)，并显著降低了计算成本。

Conclusion: 通过将IRSTD重新定义为质心回归任务并采用单点监督，SPIRE有效解决了小目标检测中监督信息不足和优化不稳定的问题，在保持低误报率的同时大幅降低了计算开销。
...
### [2] [3DTurboQuant: Training-Free Near-Optimal Quantization for 3D Reconstruction Models](https://arxiv.org/abs/2604.05366)
*Jae Joong Lee*

Main category: cs.CV

TL;DR: 作者提出3DTurboQuant，无需训练和码本学习即可对3DGS和DUSt3R进行高效压缩，利用参数向量的维度特性实现数据无关的Lloyd-Max量化，在NeRF Synthetic上分别实现3.5倍和7.9倍的压缩比。


<details>
  <summary>Details</summary>
Motivation: 现有所有压缩3D Gaussian Splatting、NeRF或基于Transformer的3D重建器的方法都需要通过逐场景微调来学习数据依赖的码本，作者证明这种做法是不必要的。

Method: 利用参数向量的维度特性（45维球面谐波和1024维键值向量），通过单一随机旋转将输入转换为已知Beta分布的坐标，实现数据无关的Lloyd-Max量化。并推导出维度相关准则、范数分离边界、针对2D哈希网格特征的分组策略，以及可组合的剪枝-量化流水线。

Result: 在NeRF Synthetic数据集上，3DTurboQuant将3DGS压缩3.5倍，PSNR仅损失0.02dB；将DUSt3R的KV缓存压缩7.9倍，点图保真度达39.7dB。压缩过程无需训练、无需码本学习、无需校准数据，耗时仅数秒。

Conclusion: 3DTurboQuant能够在无需训练、无需码本学习、无需校准数据的情况下，实现高效且接近信息论下限的3D模型压缩。在NeRF Synthetic上，3DGS压缩3.5倍仅损失0.02dB PSNR，DUSt3R KV缓存压缩7.9倍达到39.7dB点图保真度，压缩时间仅需数秒。
...
### [3] [UAVReason: A Unified, Large-Scale Benchmark for Multimodal Aerial Scene Reasoning and Generation](https://arxiv.org/abs/2604.05377)
*Jintao Sun,Hu Zhang,Donglin Di,Gangyi Ding,Zhedong Zheng*

Main category: cs.CV

TL;DR: UAVReason是首个针对无人机俯视场景的大规模多模态基准，包含273K VQA问答对和22种推理类型，通过统一多任务学习基线模型验证了方法有效性，为无人机视觉语言研究提供数据支持。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型（VLMs）在地面视觉理解方面表现优异，但在无人机（UAV）高空俯视场景中性能显著下降。主要原因在于显著的领域偏移问题：目标小且密集、纹理重复、俯视方向模糊，这些因素严重破坏了语义定位，阻碍了空间推理和可控生成能力。

Method: 提出了UAVReason，这是首个针对俯视无人机场景的统一大规模多模态基准。该基准来自高保真无人机仿真平台，包含273K VQA问答对（23.6K单帧详细描述、68.2K两帧时序序列、188.8K跨模态生成样本），探测22种空间和时序推理类型，同时评估RGB、深度和分割模态的高保真生成。通过多任务学习建立了统一的基线模型。

Result: 实验验证了统一多任务学习方法的有效性，在VQA的EM/F1指标、分割的mIoU指标、生成的CLIP Score上均优于通用领域视觉语言模型，显著提升了无人机原生性能。

Conclusion: UAVReason基准填补了无人机多模态研究的空白，统一了VQA和跨模态生成任务，证明了多任务学习可以有效缓解领域偏移问题，为无人机视觉语言研究提供了重要的数据集和基线模型。
...
TL;DR: 本文提出目标锚定组合图像检索(OACIR)，一种新的细粒度检索任务，要求严格的实例级一致性。通过在组合查询中添加参考图像的边界框来视觉锚定目标，确保实例保留。为推进该任务研究，构建了OACIRR基准，包含160K四元组和四个包含困难负样本的候选库。提出AdaFocal框架，采用上下文感知注意力调制器，自适应强化指定实例区域的注意力。


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索(CIR)优先考虑语义匹配，难以可靠地在不同上下文中检索用户指定的实例。在实际应用中，强调具体的实例保真度往往比广泛语义更重要。

Method: 提出OACIR任务，通过边界框视觉锚定参考图像中的对象；构建OACIRR大规模多领域基准（160K四元组+4个含硬负样本的候选库）；提出AdaFocal框架，包含上下文感知注意力调制器，自适应强化锚定实例区域的注意力，动态平衡实例与更广泛组合上下文之间的关注度。

Result: AdaFocal显著优于现有组合检索模型，特别是在保持实例级保真度方面。

Conclusion: 为这一具有挑战性的任务建立了坚实的基线，开辟了更灵活的实例感知检索系统的新方向。
...
### [5] [LSGS-Loc: Towards Robust 3DGS-Based Visual Localization for Large-Scale UAV Scenarios](https://arxiv.org/abs/2604.05402)
*Xiang Zhang,Tengfei Wang,Fang Xu,Xin Wang,Zongqian Zhan*

Main category: cs.CV

TL;DR: 提出了LSGS-Loc，一种针对大规模3DGS场景的新型视觉定位 pipeline，通过尺度感知的 pose 初始化和拉普拉斯可靠性掩码机制来解决大规模无人机场景中的视觉定位问题。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机场景中的视觉定位面临几何复杂性和环境变化的挑战，现有的3DGS-based 定位方法在鲁棒的 pose 初始化和对大规模场景中渲染伪影（如模糊、浮动物）的敏感性方面存在局限。

Method: 1. 尺度感知 pose 初始化策略：结合场景无关的相对 pose 估计与显式3DGS尺度约束，实现无需场景特定训练的几何接地定位。2. _pose 优化_：开发了基于拉普拉斯的可靠性掩码机制，引导光度优化聚焦于高质量区域，以减轻重建伪影的影响。

Result: 在大型无人机基准数据集上实现了最先进的精度和鲁棒性，显著优于现有的3DGS方法。

Conclusion: LSGS-Loc能有效解决大规模3DGS场景中的视觉定位问题，通过尺度感知初始化和可靠性掩码机制提升了定位的准确性和鲁棒性。
...
### [6] [Weather-Conditioned Branch Routing for Robust LiDAR-Radar 3D Object Detection](https://arxiv.org/abs/2604.05405)
*Hongsheng Li,Lingfeng Zhang,Zexian Yang,Liang Li,Rong Yin,Xiaoshuai Hao,Wenbo Ding*

Main category: cs.CV

TL;DR: 提出了一种用于恶劣天气下3D目标检测的天气条件分支路由框架，通过动态调整LiDAR和4D雷达的融合权重来提高感知系统的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR-4D雷达融合方法主要依赖固定或弱自适应管道，无法根据环境条件变化动态调整模态偏好。

Method: 提出了一种天气条件分支路由框架，包含三个并行3D特征流（纯LiDAR分支、纯4D雷达分支、条件门控融合分支），由轻量级路由器根据条件token动态预测样本特定权重进行软聚合。引入天气监督学习策略，包含辅助分类和多样性正则化以防止分支崩溃。

Result: 在K-Radar基准测试中取得最先进性能，提供了清晰的模态偏好可解释性，展示自适应路由如何在不同恶劣天气下稳健地转移LiDAR和4D雷达之间的依赖。

Conclusion: 在K-Radar基准测试上实现了最先进的性能，并提供了可解释的模态偏好洞察，揭示了自适应路由如何在不同恶劣天气场景中动态调整LiDAR和4D雷达的依赖程度。
...
### [7] [CRISP: Rank-Guided Iterative Squeezing for Robust Medical Image Segmentation under Domain Shift](https://arxiv.org/abs/2604.05409)
*Yizhou Fang,Pujin Cheng,Yixiang Liu,Xiaoying Tang,Longxi Zhou*

Main category: cs.CV

TL;DR: 针对医学影像分布转移问题，提出基于秩稳定性的CRISP框架，无需目标域信息即可实现鲁棒分割，在多中心心脏MRI和CT肺血管分割上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 医学影像中的分布转移是临床翻译的主要瓶颈，现有领域自适应方法受限于预定义转移或伪监督，无法处理开放且不可预测的真实世界转移。

Method: 提出CRISP框架，基于"正区域秩稳定性"经验定律，通过潜在特征扰动模拟分布转移，构建高精度的HP和高召回的HR先验，在扰动下递归细化这些先验，设计迭代训练框架使HP和HR逐步"挤压"得到最终分割。

Result: 在多中心心脏MRI和CT肺血管分割任务上验证，CRISP显著优于最先进方法，在多中心转移、人口统计转移和模态转移场景下，HD95分别降低0.14（7.0%改善）、1.90（13.1%改善）和8.39（38.9%改善）。

Conclusion: CRISP通过基于秩的分割范式转变，在多中心心脏MRI和CT肺血管分割任务上显著优于现有方法，在多中心、人口统计和模态转移场景下分别实现了7.0%、13.1%和38.9%的HD95改善，证明了秩稳定性的有效性和普适性。
...
### [8] [Learning to Synergize Semantic and Geometric Priors for Limited-Data Wheat Disease Segmentation](https://arxiv.org/abs/2604.05415)
*Shijie Wang,Zijian Wang,Yadan Luo,Scott Chapman,Xin Yu,Zi Huang*

Main category: cs.CV

TL;DR: SGPer是一种小麦病害语义-几何先验协同分割框架，利用DINOv2提供类别感知的语义先验处理生长阶段的外观变化，并将其转换为密集的空间提示引导SAM实现病害边界的精确定位。


<details>
  <summary>Details</summary>
Motivation: 小麦病害分割面临严重的类内时间变化问题，不同生长阶段的外观差异显著，从头收集具有代表性的训练数据集既费时又不切实际。

Method: SGPer将病害分割视为疾病语义感知与病害边界定位的耦合任务。核心思路是：1）预训练DINOv2提供处理外观变化的鲁棒类别感知语义先验；2）将这些先验转换为粗空间提示引导SAM进行边界定位。具体设计了疾病敏感适配器插入DINOv2和SAM以对齐预训练表示与疾病特征；将DINOv2特征转换为密集的类别特定点提示；通过交叉参考SAM的迭代掩码置信度与DINOv2的类别语义一致性来过滤冗余提示；最终提取高信息量提示激活SAM的几何先验。

Result: 在小麦病害和器官分割基准数据集上始终达到最先进的性能，尤其是在数据受限场景下表现优异。

Conclusion: 通过语义-几何先验协同，SGPer实现了严格不变的时间外观变化下的精确且鲁棒的分害分割。
...
### [9] [VideoStir: Understanding Long Videos via Spatio-Temporally Structured and Intent-Aware RAG](https://arxiv.org/abs/2604.05418)
*Honghao Fu,Miao Xu,Yiwei Wang,Dailing Zhang,Liu Jun,Yujun Cai*

Main category: cs.CV

TL;DR: VideoStir是一种针对长视频的结构化、意图感知的检索增强生成(RAG)框架，通过将视频表示为时空图并进行多跳检索来解决传统方法破坏视频时空结构的问题，同时引入基于MLLM的意图相关性评分器来对齐查询的推理意图，并构建了IR-600K数据集来学习帧-查询意图对齐。


<details>
  <summary>Details</summary>
Motivation: 将多模态大型语言模型(MLLM)扩展到长视频受限于有限的上下文窗口。现有检索增强生成方法存在两个主要问题：(1)将视频扁平化为独立片段，破坏了其内在的时空结构；(2)依赖显式语义匹配，可能遗漏与查询意图隐式相关的线索。

Method: 提出VideoStir框架，包含三个核心组件：1)将视频结构化为 clip 级别的时空图；2)执行多跳检索以聚合跨越遥远但上下文相关事件的信息；3)引入基于MLLM的意图相关性评分器，根据帧与查询推理意图的对齐程度进行检索。为支持该能力，构建了IR-600K大规模数据集用于学习帧-查询意图对齐。

Result: 实验表明VideoStir在不使用辅助信息的情况下与最先进的基线方法具有竞争力，证明了长视频RAG从扁平化语义匹配向结构化、意图感知推理转变的可行性。

Conclusion: VideoStir成功地将长视频RAG从扁平化语义匹配转向结构化、意图感知的推理范式，为解决长视频理解中的上下文窗口限制问题提供了新思路。
...
### [10] [Few-Shot Semantic Segmentation Meets SAM3](https://arxiv.org/abs/2604.05433)
*Yi-Jen Tsai,Yen-Yu Lin,Chien-Yao Wang*

Main category: cs.CV

TL;DR: 本文提出了一种基于SAM3的少样本语义分割方法，通过简单的空间拼接策略将支持图像和查询图像组合，利用SAM3的Promptable Concept Segmentation能力实现无需微调的分割。该方法在PASCAL-5^i和COCO-20^i上取得了最先进性能，并发现负提示在少样本设置中会削弱目标表示导致预测崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有的少样本语义分割方法主要依赖大量的片段式训练（episodic training）来学习可迁移表示，这种方法计算成本高且对分布偏移敏感。作者重新审视少样本语义分割，从现代视觉基础模型的视角出发，探索SAM3作为训练-free解决方案的潜力。

Method: 该研究采用SAM3的Promptable Concept Segmentation (PCS)能力，设计了一种空间拼接策略。具体方法是将支持图像和查询图像放置在共享的画布中，使完全冻结的SAM3能够在无需任何微调或架构修改的情况下执行分割任务。

Result: 在PASCAL-5^i和COCO-20^i数据集上的实验表明，这种极简设计已经实现了最先进的性能，优于许多经过大量工程设计的方法。

Conclusion: 该研究首次探索了SAM3在少样本语义分割任务中的潜力，提出了一种无需微调的简单空间拼接策略。核心贡献在于揭示了负提示在少样本场景中的负面影响机制——虽然负提示原本用于抑制干扰物，但在此设置下反而会削弱目标特征表示，导致预测崩溃。这一发现为理解视觉基础模型如何处理冲突提示信号提供了重要见解，表明跨图像推理能力可以通过简单的空间公式化实现，而无需复杂的工程设计。
...
### [11] [Human Interaction-Aware 3D Reconstruction from a Single Image](https://arxiv.org/abs/2604.05436)
*Gwanghyun Kim,Junghun James Kim,Suh Yoon Jeon,Jason Park,Se Young Chun*

Main category: cs.CV

TL;DR: 本文提出HUG3D框架，从单张图像重建多个交互人物的3D模型。通过将输入变换到规范正交空间，然后利用HUG-MVD生成多视角法线和图像，最后用HUG-GR模块基于物理先验优化几何，实现高保真、物理可信的3D重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要聚焦于单人场景，在多人场景中简单组合单人重建会导致 artifacts（如不现实的遮挡、缺失几何、变形交互），需要显式建模群体级上下文和交互先验来解决遮挡和邻近问题。

Method: 提出两阶段方法：（1）将输入变换到规范正交空间以减轻透视几何畸变；（2）HUG-MVD组件通过联合建模个体和群体上下文生成完整多视角法线和图像以解决遮挡和邻近问题；（3）HUG-GR组件利用显式物理交互先验优化几何，实施物理可行性并准确建模人际接触。

Result: 广泛实验表明HUG3D显著优于现有单人和多人方法，能够从单张图像生成物理可信、高保真的交互人物3D重建。

Conclusion: 本文提出的HUG3D框架成功解决了多人场景下单图像3D重建的难题，通过群体-实例多视角扩散和几何重建模块，有效处理了遮挡和交互问题，为AR/VR和数字人应用提供了新方案。
...
### [12] [Not All Agents Matter: From Global Attention Dilution to Risk-Prioritized Game Planning](https://arxiv.org/abs/2604.05449)
*Kang Ding,Hongsong Wang,Jie Gui,Lei He*

Main category: cs.CV

TL;DR: 本文提出了GameAD框架，将端到端自动驾驶建模为风险感知博弈问题，通过风险优先的拓扑锚定、战略载荷适配器、极小化风险感知稀疏注意力和风险一致均衡稳定等技术，实现安全优先的自动驾驶决策。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型将所有智能体平等对待，难以从复杂背景中解耦真实的碰撞威胁；同时，传统方法未能充分考虑动态多智能体博弈中的风险因素。

Method: 提出风险优先博弈规划概念，集成风险感知拓扑锚定、战略载荷适配器、极小化风险感知稀疏注意力和风险一致均衡稳定四个模块；引入规划风险暴露度量指标。

Result: 在nuScenes和Bench2Drive数据集上显著优于现有方法，尤其是在轨迹安全性方面。

Conclusion: 通过将自动驾驶建模为风险感知的博弈问题，能够更好地实现安全优先的端到端自动驾驶决策。
...
### [13] [A Synthetic Eye Movement Dataset for Script Reading Detection: Real Trajectory Replay on a 3D Simulator](https://arxiv.org/abs/2604.05475)
*Kidus Zewde,Yuchen Zhou,Dennis Ng,Neo Tiangratanakul,Tommy Duong,Ankit Raj,Yuxin Zhang,Xingyu Shen,Simiao Ren*

Main category: cs.CV

TL;DR: 针对视觉-语言模型训练中行为数据稀缺的问题，提出利用3D眼动模拟器生成合成眼动视频的pipeline，生成了12小时的标注数据用于视频面试中的脚本阅读检测任务，并验证了合成数据的时间动态保真度。


<details>
  <summary>Details</summary>
Motivation: 解决视觉-语言模型训练中的数据不对称问题：文本和图像数据丰富，但视频行为数据(眼动、手势、社交信号)稀缺、标注成本高且涉及隐私。

Method: 提出一种基于3D眼动模拟器的合成数据生成pipeline：1)从真实视频中提取虹膜轨迹; 2)通过headless浏览器自动化在3D模拟器上重放轨迹; 3)生成自动标注的合成眼动视频。数据集包含144个会话(72阅读/72对话)，共12小时，25fps。

Result: 生成的轨迹保留了源数据的时间动态特性(各指标KS D < 0.14)。3D模拟器在阅读尺度运动上表现出bounded sensitivity，原因是缺少耦合的头部运动。

Conclusion: 该pipeline可生成preserve temporal dynamics的合成眼动数据(KS D < 0.14)，但在reading-scale movements上存在bounded sensitivity，这源于缺少耦合的头部运动。数据集和评估工具已发布以支持下游行为分类器开发。
...
### [14] [Unifying VLM-Guided Flow Matching and Spectral Anomaly Detection for Interpretable Veterinary Diagnosis](https://arxiv.org/abs/2604.05482)
*Pu Wang,Zhixuan Mao,Jialu Li,Zhuoran Zheng,Dianjie Lu,Youshan Zhang*

Main category: cs.CV

TL;DR: 针对犬气胸自动诊断的数据稀缺问题，该研究创建了首个公开的像素级标注数据集，并提出将诊断任务重新定义为信号定位和光谱检测的协同过程。定位阶段使用VLM指导的Flow Matching实现高精度分割，检测阶段则创新性地采用随机矩阵理论(RMT)分析分割后的特征，通过识别非随机病理信号的特征值来诊断疾病。


<details>
  <summary>Details</summary>
Motivation: 犬气胸自动诊断面临数据稀缺和需要可信赖模型两大挑战。

Method: ①定位：使用视觉语言模型(VLM)指导迭代流匹配(Flow Matching)过程，逐步细化分割掩码以实现更好的边界精度。②检测：使用分割掩膜隔离可疑病变的特征，然后应用随机矩阵理论(RMT)分析——将健康组织建模为可预测的随机噪声，通过检测表示非随机病理信号的特征值来识别气胸。

Result: 该协同方法结合生成式分割和第一原理统计分析，实现了高准确性和可解释性的诊断系统。

Conclusion: 该研究通过结合生成式分割(VLM+Flow Matching)和第一原理统计分析(RMT)，实现了高精度和可解释的犬气胸诊断系统。高保真定位对于净化信号至关重要，最大化了RMT检测器的灵敏度。开源代码已发布于GitHub。
...
### [15] [A Weak-Signal-Aware Framework for Subsurface Defect Detection: Mechanisms for Enhancing Low-SCR Hyperbolic Signatures](https://arxiv.org/abs/2604.05490)
*Wenbo Zhang,Zekun Long,Zican Liu,Yangchen Zeng,Keyi Hu*

Main category: cs.CV

TL;DR: WSA-Net是一种用于探地雷达(GPR) subsurface defect detection的轻量级检测框架,通过信号保留、杂波抑制、几何重建和上下文锚定四种机制增强微弱信号检测,在RTST数据集上以仅2.412M参数实现0.6958 mAP@0.5和164 FPS。


<details>
  <summary>Details</summary>
Motivation: GPR subsurface defect detection面临"弱信号"挑战:微弱的衍射双曲线低信杂比、高波场相似性和几何退化。现有的轻量级检测器优先考虑效率而忽视敏感性,无法保留低频结构或解耦异构杂波。

Method: 提出WSA-Net框架,集成四个机制:1)使用部分卷积进行信号保留;2)通过异构分组注意力进行杂波抑制;3)几何重建以锐化双曲线弧;4)上下文锚定解决语义歧义。

Result: 在RTST数据集上,WSA-Net实现了0.6958 mAP@0.5和164 FPS推理速度,仅使用2.412M参数。

Conclusion: 轻量级架构中的信号中心感知可以有效减少基础设施检测中的假阴性,在效率和敏感性之间取得良好平衡。
...
### [16] [Benchmarking Vision-Language Models under Contradictory Virtual Content Attacks in Augmented Reality](https://arxiv.org/abs/2604.05510)
*Yanming Xiu,Zhengayuan Jiang,Neil Zhenqiang Gong,Maria Gorlatova*

Main category: cs.CV

TL;DR: 提出了ContrAR基准，用于评估视觉语言模型在AR环境中对抗虚拟内容操纵和矛盾攻击的鲁棒性，包含312个真实AR视频和11个VLMs的基准测试。


<details>
  <summary>Details</summary>
Motivation: 随着AR技术日益融入日常生活，其安全性和可靠性成为关键挑战，其中矛盾虚拟内容攻击（将恶意或不一致的虚拟元素注入用户视图）会带来误导用户、造成语义混淆或传递有害信息的独特风险。

Method: 构建了包含312个真实AR视频的ContrAR基准测试，验证了11个VLMs（包括商业和开源模型）的性能。

Result: 实验结果表明，尽管当前VLMs对矛盾虚拟内容展现出合理的理解能力，但在检测和推理AR环境中的对抗性内容操作方面仍有改进空间。

Conclusion: ContrAR基准测试揭示了当前VLMs在检测和推理AR环境中对抗性内容操作方面仍有提升空间，同时平衡检测准确性和延迟仍具挑战性。
...
### [17] [Geometrical Cross-Attention and Nonvoid Voxelization for Efficient 3D Medical Image Segmentation](https://arxiv.org/abs/2604.05515)
*Chenxin Yuan,Shoupeng Chen,Haojiang Ye,Yiming Miao,Limei Peng,Pin-Han Ho*

Main category: cs.CV

TL;DR: 提出GCNV-Net——一种结合三向动态非空体素变换器与几何交叉注意力的3D医学图像分割框架，在多数据集取得最先进性能并大幅降低计算成本


<details>
  <summary>Details</summary>
Motivation: 现有3D医学扫描分割方法难以在多样解剖结构和成像模态下同时实现高精度与高计算效率的平衡

Method: 提出GCNV-Net框架，整合三向动态非空体素Transformer(3DNVT)、几何交叉注意力模块(GCA)和非空体素化技术。3DNVT沿横断面、矢状面和冠状面三个正交解剖平面动态划分相关体素；GCA在多尺度特征融合中融入几何位置信息；非空体素化仅处理信息区域

Result: 在BraTS2021、ACDC、MSD前列腺、MSD胰腺及AMOS2022等数据集上取得最优性能：Dice提升0.65%、IoU提升0.63%、NSD提升1%、HD95相对提升14.5%；同时实现FLOPs降低56.13%、推理延迟降低68.49%

Conclusion: GCNV-Net有效平衡了精度与效率，在不同器官、疾病状态和成像模态下均展现强鲁棒性，具有显著的临床应用潜力
...
### [18] [Cross-Resolution Diffusion Models via Network Pruning](https://arxiv.org/abs/2604.05524)
*Jiaxuan Ren,Junhan Zhu,Huan Wang*

Main category: cs.CV

TL;DR: 扩散模型在训练分辨率之外生成图像时质量下降，论文提出CR-Diff方法通过剪枝参数来提高跨分辨率视觉一致性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在固定分辨率训练后，在非训练分辨率生成时质量下降。原因在于权重与分辨率相关，在默认分辨率下有效的权重在空间尺度变化时可能变得不利，削弱语义对齐并导致UNet结构不稳定。

Method: CR-Diff方法包含两个阶段：(1) 块剪枝：选择性地消除不利权重；(2) 剪枝输出放大：进一步净化剪枝预测。

Result: 实验表明CR-Diff可在各种扩散骨干和未见过的分辨率上提高感知保真度和语义一致性，同时保持默认分辨率的性能。此外还支持按需提示特定细化。

Conclusion: 通过参数剪枝方法有效解决了扩散模型的跨分辨率一致性问题，方法简单有效且具有普适性。
...
### [19] [Prior-guided Fusion of Multimodal Features for Change Detection from Optical-SAR Images](https://arxiv.org/abs/2604.05527)
*Xuanguang Liu,Lei Ding,Yujie Li,Chenguang Dai,Zhenchao Zhang,Mengmeng Li,Ziyi Yang,Yifan Sun,Yongqi Sun,Hanyun Wang*

Main category: cs.CV

TL;DR: STSF-Net是一种用于光学和SAR图像多模态变化检测的深度学习框架，通过联合建模模态特定特征和时空共性特征来增强变化表示，并引入基于预训练基础模型的语义先验进行自适应特征融合。


<details>
  <summary>Details</summary>
Motivation: 现有文献中的多模态变化检测方法在跨模态交互和利用模态特定特征方面存在局限性，导致细粒度变化信息建模不足，阻碍了多模态数据中语义变化的精确检测。

Method: 提出STSF-Net框架，包含三个关键组件：1）模态特定特征提取，用于捕获真实语义变化信号；2）时空共性特征嵌入，用于抑制由成像机制差异引起的伪变化；3）基于预训练基础模型语义先验的光学与SAR特征自适应融合策略。

Result: 在Delta-SN6、BRIGHT和Wuhan-Het数据集上，STSF-Net的mIoU分别比最先进方法提高3.21%、1.08%和1.32%。同时发布了首个开源的多类别多模态变化检测基准数据集Delta-SN6，包含极高分全极化SAR和光学图像。

Conclusion: STSF-Net通过有效融合模态特定特征与时空共性特征，并结合语义引导的自适应融合策略，显著提升了多模态变化检测性能，为遥感图像变化监测提供了新的有效方案。
...
### [20] [Evaluation Before Generation: A Paradigm for Robust Multimodal Sentiment Analysis with Missing Modalities](https://arxiv.org/abs/2604.05558)
*Rongfei Chen,Tingting Zhang,Xiaoyu Shen,Wei Zhang*

Main category: cs.CV

TL;DR: 提出了ProMMA框架解决多模态情感分析中的缺失模态问题，通过缺失模态评估器、提示解耦、动态加权和多级连接四个模块，在三个基准数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态情感分析中的缺失模态问题是一个关键挑战，现有方法主要通过提示学习和预训练模型来提高鲁棒性，但存在两个主要局限：1) 缺乏对生成缺失模态必要性的严格评估；2) 多模态提示之间的结构依赖性和全局一致性探索不足。

Method: 提出了ProMMA框架，包含四个核心模块：1) 缺失模态评估器，在输入阶段使用预训练模型和伪标签动态评估缺失模态的重要性，避免低质量数据填充；2) 模态不变提示解耦模块，将共享提示分解为模态特定私有提示，捕捉内在局部相关性；3) 动态提示加权模块，从交叉注意力输出计算互信息权重，自适应抑制缺失模态干扰；4) 多级提示动态连接模块，通过残差连接整合共享提示与自注意力输出，利用全局提示先验强化关键引导特征。

Result: 在CMU MOSI、CMU MOSEI和CH SIMS三个公开基准数据集上进行了广泛实验，证明了该框架在多种缺失模态设置下达到了最先进的性能并保持了稳定的结果。

Conclusion: 该研究通过引入缺失模态评估机制和创新的提示学习方法，有效解决了多模态情感分析中的缺失模态问题，为实际应用场景中的鲁棒多模态理解提供了新的解决方案。
...
### [21] [Purify-then-Align: Towards Robust Human Sensing under Modality Missing with Knowledge Distillation from Noisy Multimodal Teacher](https://arxiv.org/abs/2604.05584)
*Pengcheng Weng,Yanyu Qian,Yangxin Xu,Fei Wang*

Main category: cs.CV

TL;DR: 提出PTA框架，通过"先净化后对齐"策略解决多模态人体感知中缺失模态的挑战。


<details>
  <summary>Details</summary>
Motivation: 多模态人体感知面临两个关键挑战：异构数据之间的表示差距，以及低质量模态造成的污染效应。这两个障碍存在因果关联，污染引入的腐败从根本上阻碍了表示差距的缩小。

Method: PTA框架结合元学习和知识迁移：1) 使用元学习驱动的权重机制动态降低噪声和低贡献模态的影响；2) 引入基于扩散的知识蒸馏范式，由净化后的共识形成信息丰富的干净教师，来精炼每个学生模态的特征。

Result: 在大型MM-Fi和XRF55数据集上，在显著的表示差距和污染效应下，PTA实现了最先进的性能，显著提高了各种缺失模态场景下单模态模型的鲁棒性。

Conclusion: "先净化后对齐"策略成功创建了具有跨模态知识的强大单模态编码器，有效解决了表示差距和污染效应的因果依赖问题。
...
### [22] [BPC-Net: Annotation-Free Skin Lesion Segmentation via Boundary Probability Calibration](https://arxiv.org/abs/2604.05594)
*Yujie Yao,Yuhaohang He,Junjie Huang,Zhou Liu,Jiangzhao Li,Yan Qiao,Wen Xiao,Yunsen Liang,Xiaofan Li*

Main category: cs.CV

TL;DR: BPC-Net是一种无需标注的皮肤病变分割框架，通过高斯概率平滑(GPS)进行局部概率空间校准，解决边界概率低估问题


<details>
  <summary>Details</summary>
Motivation: 现有无需标注方法性能受三方面制约：伪标签噪声监督、有限目标域数据下的不稳定迁移、边界概率低估。边界概率低估直接影响轮廓完整性，全局阈值调整无法有效纠正

Method: 提出BPC-Net边界概率校准框架，核心为高斯概率平滑(GPS)，在阈值化前进行局部概率空间校准恢复欠置信的病变边界。包含两个辅助设计：特征解耦解码器分别处理上下文抑制、细节恢复和边界精炼；交互分支适应策略仅更新伪标签交互分支，保留部署的纯图像分割路径

Result: 在ISIC-2017、ISIC-2018和PH2数据集上，无需标注协议下达到最新性能，宏平均Dice系数85.80%、Jaccard指数76.97%，在PH2上接近有监督参考性能

Conclusion: BPC-Net通过边界概率校准有效提升了无标注皮肤病变分割的性能，为低资源场景下的皮肤病学部署提供了有价值的解决方案
...
### [23] [ID-Selection: Importance-Diversity Based Visual Token Selection for Efficient LVLM Inference](https://arxiv.org/abs/2604.05601)
*Zhaohong Huang,Wenjing Liu,Yuxin Zhang,Fei Chao,Rongrong Ji*

Main category: cs.CV

TL;DR: ID-Selection是一种简单而有效的视觉token选择策略，用于加速大型视觉-语言模型推理。该方法将重要性估计与多样性感知迭代选择相结合，在保持重要信息的同时减少冗余。在5个LVLM backbone和16个基准测试上取得了优异的性能，特别是在极端剪枝比例下表现突出。


<details>
  <summary>Details</summary>
Motivation: 现有视觉token剪枝方法难以平衡token重要性和多样性：基于重要性保留的方法倾向于保留冗余token，而基于多样性的方法可能忽略重要信息。这种权衡在高剪枝比例下尤其成问题。ID-Selection旨在解决这一权衡问题。

Method: ID-Selection的核心思想是将重要性估计与多样性感知迭代选择耦合：首先为每个token分配重要性分数，然后逐个选择高分token，同时逐步抑制相似token的分数。通过这种方式，ID-Selection在统一的选择过程中保留信息性token同时减少冗余。

Result: 在LLaVA-1.5-7B上，ID-Selection剪枝了97.2%的视觉token，仅保留16个token，同时减少超过97%的推理FLOPs，并保持91.8%的原始性能，且无需额外训练。

Conclusion: ID-Selection在5个LVLM backbone和16个主要基准测试中一致实现卓越的性能和效率，特别在极端剪枝比例下表现优异。
...
### [24] [Evaluation of Randomization through Style Transfer for Enhanced Domain Generalization](https://arxiv.org/abs/2604.05616)
*Dustin Eisenhardt,Timothy Schaumlöffel,Alperen Kantarci,Gemma Roig*

Main category: cs.CV

TL;DR: 关于风格迁移用于域泛化的系统性实证研究，提出StyleMixDG方法解决Sim2Real差距


<details>
  <summary>Details</summary>
Motivation: 文献中存在三个关键设计轴的矛盾：风格池多样性、纹理复杂性、风格源选择，缺乏系统性验证

Method: 通过控制变量法系统性评估三个设计轴：(i)扩展风格池大小 (ii)纹理复杂性测试 (iii)比较不同风格源（艺术风格 vs 域对齐风格）

Result: (i)扩展风格池比重复使用少量风格增益更大 (ii)纹理复杂性在池足够大时无显著影响 (iii)多样化艺术风格优于域对齐替代方案

Conclusion: StyleMixDG是轻量级、模型无关的增强方案，在GTAV→{{BDD100k, Cityscapes, Mapillary Vistas}}基准上优于强基线，验证了经验设计原则的实用性
...
### [25] [Semantic-Topological Graph Reasoning for Language-Guided Pulmonary Screening](https://arxiv.org/abs/2604.05620)
*Chenyu Xue,Yiran Liu,Mian Zhou,Jionglong Su,Zhixiang Lu*

Main category: cs.CV

TL;DR: 提出一种结合大语言模型LLaMA-3-V与视觉基础模型MedSAM的语义拓扑图推理框架(STGR)，用于语言引导的肺结节筛查分割。该框架通过文本-视觉意图蒸馏模块提取诊断指导，将掩码选择建模为动态图推理问题，并采用选择性非对称微调策略(SAFT)仅更新不到1%的参数即可实现81.5%的Dice相似系数。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法面临两大挑战：1) 临床报告的自由文本指令存在语义歧义，多模态和基础模型难以理解；2) 低对比度扫描中解剖结构重叠复杂导致分割困难；3) 对大规模模型进行完全微调在有限医学数据上会导致严重过拟合问题。

Method: 提出STGR框架包含三个核心组件：1) Text-to-Vision Intent Distillation (TVID)模块：从临床文本中提取精确诊断指导，桥接语言模型与视觉模型；2) 动态图推理模块：将候选病变建模为图节点，边捕捉空间和语义亲和性，解决解剖歧义；3) Selective Asymmetric Fine-Tuning (SAFT)策略：仅更新不到1%的参数，降低过拟合风险。

Result: 在LIDC-IDRI和LNDb数据集上进行5折交叉验证，STGR在LIDC-IDRI上达到81.5%的Dice相似系数(DSC)，相比当前领先的LLM基工具LISA提升超过5%。SAFT策略作为强大正则化器，交叉折叠稳定性极高， DSC方差仅为0.6%。

Conclusion: STGR框架建立了新的state-of-the-art性能，在肺结节分割任务上显著超越现有方法。SAFT策略有效解决了有限医学数据上的过拟合难题，实现了出色的跨折叠稳定性(0.6%方差)，为稳健的上下文感知临床部署铺平了道路。
...
### [26] [FunRec: Reconstructing Functional 3D Scenes from Egocentric Interaction Videos](https://arxiv.org/abs/2604.05621)
...
TL;DR: FunRec是一种从第一人称RGB-D交互视频直接重建室内场景功能性3D数字孪生的方法，无需 CAD 先验或多状态捕捉，可自动发现铰接部件、估计运动参数、跟踪3D运动并重建仿真兼容网格。


<details>
  <summary>Details</summary>
Motivation: 现有铰接体重建方法依赖受控环境、多状态捕捉或CAD先验，无法直接从野外人类交互视频中恢复可交互的3D场景。

Method: FunRec直接从野外 egocentric RGB-D 交互序列操作，自动发现铰接部件、估计运动参数、跟踪3D运动，在规范空间中重建静态和动态几何，生成仿真兼容网格。

Result: 在真实和模拟 benchmark 上，相比现有方法提升高达 +50 mIoU 的部件分割精度，铰接和姿态误差降低 5-10 倍，重建精度显著提高。

Conclusion: FunRec 实现了从单目交互视频直接重建功能性3D场景的突破，可导出 URDF/USD 用于仿真，支持手势引导的affordance映射和机器人-场景交互，具有重要应用价值。
...
### [27] [DetailVerifyBench: A Benchmark for Dense Hallucination Localization in Long Image Captions](https://arxiv.org/abs/2604.05623)
*Xinran Wang,Yuxuan Zhang,Xiao Zhang,Haolong Yan,Muxi Diao,Songyu Xu,Zhonghao Yan,Hongbing Li,Kongming Liang,Zhanyu Ma*

Main category: cs.CV

TL;DR: DetailVerifyBench是一个严格的基准测试，用于评估长图像caption中幻觉的精确检测与定位，包含1000张高质量图像，涵盖5个不同领域，平均caption长度超过200词，提供细粒度的token-level注释。


<details>
  <summary>Details</summary>
Motivation: 准确检测和定位幻觉对于确保图像caption的高可靠性至关重要。在多模态大语言模型(MLLMs)时代，caption从简短句子演变为全面叙事，通常达数百词。这一转变极大增加了挑战：模型需要在广泛上下文中精确定位特定错误spans或words，而非仅标记响应级的不一致。现有基准缺乏细粒度和领域多样性。

Method: 引入DetailVerifyBench，一个包含1000张高质量图像的严格基准测试，涵盖5个不同领域。平均caption长度超过200词，采用密集的、token级别的多种幻觉类型标注。

Result: 这是迄今为止长图像caption领域最精确的幻觉定位基准测试。

Conclusion: DetailVerifyBench为评估细粒度幻觉定位能力提供了最严格的基准，可用于推动图像caption质量评估领域的发展。
...
### [28] [A Unified Foundation Model for All-in-One Multi-Modal Remote Sensing Image Restoration and Fusion with Language Prompting](https://arxiv.org/abs/2604.05629)
*Yongchuan Cui,Peng Liu*

Main category: cs.CV

TL;DR: LLaRS是首个统一的多模态多任务遥感低层视觉基础模型，通过Sinkhorn-Knopp最优传输对齐异构波段，采用卷积、通道混合和注意力三类混合专家层处理不同特征，并在百万级多任务数据集LLaRS1M上进行训练，在多项任务上超越七个竞争模型。


<details>
  <summary>Details</summary>
Motivation: 遥感图像受云层、雾霾、噪声、分辨率限制和传感器异构性影响，现有恢复和融合方法针对每种退化类型训练单独的模型，缺乏统一处理多任务多模态遥感图像的基础模型。

Method: 1) 使用Sinkhorn-Knopp最优传输将异构波段对齐到语义匹配的槽中；2) 通过三个互补的混合专家层路由特征：卷积专家处理空间模式、通道混合专家处理光谱保真度、带低秩适配器的注意力专家处理全局上下文；3) 通过步级动态权重调整稳定联合训练；4) 在LLaRS1M数据集上训练，该数据集包含100万规模的多任务样本，涵盖11种恢复和增强任务，整合了真实配对观测和受控合成退化，并配有自然语言提示。

Result: LLaRS在实验中一致性地超越了七个具有竞争力的模型，参数高效微调实验证明了在未见数据上具有强大的迁移能力和适应性。

Conclusion: LLaRS是首个用于多模态和多任务遥感低层视觉的统一基础模型，通过Sinkhorn-Knopp最优传输、混合专家层和动态权重调整等技术实现了有效的联合训练。参数高效微调实验表明该模型在未见数据上具有强大的迁移能力和适应性。
...
### [29] [SGANet: Semantic and Geometric Alignment for Multimodal Multi-view Anomaly Detection](https://arxiv.org/abs/2604.05632)
*Letian Bai,Chengyu Tao,Juan Du*

Main category: cs.CV

TL;DR: 提出了SGANet，一个用于多模态多视角异常检测的统一框架，通过语义和几何对齐来学习跨视角和模态的物理一致特征表示。


<details>
  <summary>Details</summary>
Motivation: 现有无监督方法常因视角变化和模态差异导致的特征不一致而受影响，难以有效识别复杂物体上的表面缺陷。

Method: 提出选择性跨视角特征细化模块(SCFRM)聚合相邻视图信息；语义-结构 patch对齐(SSPA)保持跨模态语义对齐和视角变换下的结构一致性；多视角几何对齐(MVGA)对齐几何对应patch。

Result: 在SiM3D和Eyecandies数据集上实现最先进的异常检测和定位性能。

Conclusion: SGANet通过联合建模特征交互、语义结构一致性和全局几何对应，有效提升了多模态多视角场景下的异常检测性能，在实际工业场景中具有有效性。
...
### [30] [Towards Athlete Fatigue Assessment from Association Football Videos](https://arxiv.org/abs/2604.05636)
...
TL;DR: 研究探索将单目广播视频作为足球运动员疲劳监测的低成本工具，基于比赛状态重建方法提取球员轨迹，通过运动学处理算法获取速度加速度估计，构建加速-速度(A-S)轮廓作为疲劳相关性能指标，并在SoccerNet-GSR基准数据集上验证了该方法的可行性。


<details>
  <summary>Details</summary>
Motivation: 当前足球运动中的疲劳监测主要依赖主观自评指标、实验室生物标志物检测或侵入性传感器（如心率监测器、GPS追踪），这些方法要么不够客观，要么成本高昂且具有侵入性，需要一种更低成本的替代方案。

Method: 1. 基于现有的比赛状态重建(GSR)方法从单目广播视频中提取球员在球场坐标系统中的轨迹；2. 提出新型运动学处理算法，从重建的轨迹中获得时间一致的速度和加速度估计；3. 构建加速-速度(A-S)轮廓作为疲劳相关性能指标；4. 在SoccerNet-GSR公开数据集上评估，分别测试30秒片段和45分钟半场的表现以检验短期可靠性和长期时间一致性。

Result: 单目GSR能够恢复与A-S轮廓分析兼容的运动模式，但同时揭示了对轨迹噪声、摄像机校准误差和时间不连续性的敏感性。结果支持将单目广播视频作为疲劳分析的低成本基础，并指出了未来研究的方法论挑战。

Conclusion: 单目广播视频可用于足球运动员疲劳监测，具有低成本优势；但需解决轨迹噪声、校准误差和时间不连续性带来的挑战，未来研究应着重改进这些问题以提高可靠性。
...
### [31] [PanopticQuery: Unified Query-Time Reasoning for 4D Scenes](https://arxiv.org/abs/2604.05638)
*Ruilin Tang,Yang Zhou,Zhong Ye,Wenxi Liu,Yan Huang,Shengfeng He*

Main category: cs.CV

TL;DR: PanopticQuery是一种在4D场景中通过自然语言进行统一查询推理的框架，基于4D Gaussian Splatting进行高保真动态重建，并引入多视角语义共识机制将2D语义预测聚合为结构化的4D语义 grounding。提出Panoptic-L4D新基准，在复杂语言查询任务上达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于神经表示的4D重建方法在上下文推理方面存在局限，难以处理复杂语义（如交互、时序动作、空间关系）。核心挑战在于将噪声大、视角依赖的预测转换为全局一致的4D解读。

Method: PanopticQuery框架：（1）基于4D Gaussian Splatting实现高保真动态场景重建；（2）引入多视角语义共识机制，通过聚合多视角和多时帧的2D语义预测来 grounding 自然语言查询；（3）过滤不一致输出、强制几何一致性，将2D语义提升为结构化的4D语义 grounding，并通过神经场优化实现。

Result: 在Panoptic-L4D基准上验证，PanopticQuery在复杂语言查询任务上设置新SOTA，有效处理属性、动作、空间关系和多对象交互等查询类型。

Conclusion: PanopticQuery通过创新的多视角语义共识机制成功解决了4D场景中自然语言查询的语义 grounding 问题，为动态场景的语义理解提供了新的研究方向。
...
### [32] [Analogical Reasoning as a Doctor: A Foundation Model for Gastrointestinal Endoscopy Diagnosis](https://arxiv.org/abs/2604.05649)
*Peixi Peng,Housheng Xie,Yanling Wei,Guangcong Ruan,Xiaoyang Zou,Qian Cao,Yongjian Nian,Guoyan Zheng*

Main category: cs.CV

TL;DR: RATNet是一个基于类比推理的胃肠内镜成像基础模型，通过周期性预训练策略从五个异构数据集中获取和转移知识，解决了现有模型泛化性、适应性、鲁棒性和可扩展性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 胃肠疾病带来日益增长的全球健康负担，内镜是早期诊断的主要工具，但常规图像解读存在漏诊和效率有限的问题。虽然AI辅助诊断显示前景，但现有模型因医学数据有限、领域偏移和异构注释而缺乏泛化性、适应性、鲁棒性和可扩展性。

Method: 开发了RATNet，包含编码器、相关性知识获取和转移(RAT)模块、投影器和多任务头，支持微调、线性探测和零样本迁移。采用循环预训练策略从五个胃肠内镜数据集中获取和转移异构专家知识。核心优势是类比推理机制，将图像后验知识与学习的先验知识库匹配，并转移相关知识指导诊断。

Result: RATNet在六个场景中优于现有基础模型(GastroNet和GastroVision)：常见胃肠疾病诊断、罕见疾病少样本学习、新医疗站点零样本迁移、长尾疾病分布下的鲁棒性、新疾病适应性和隐私保护的联邦学习部署。

Conclusion: RATNet通过类比推理机制提高泛化性和抗偏倚能力，是智能胃肠诊断的实用基础模型，尤其适合资源有限的场景。它开源且成本效益高，支持异构注释的自动整合，减少数据采集成本。
...
### [33] [Probing Intrinsic Medical Task Relationships: A Contrastive Learning Perspective](https://arxiv.org/abs/2604.05651)
*Jonas Muth,Zdravko Marinov,Simon Reiß*

Main category: cs.CV

TL;DR: 本文探索了医学视觉任务之间的内在关系，通过TaCo对比学习框架将30个任务和39个数据集的异构医学影像任务嵌入到共享表示空间中，揭示了任务的相似性、区分度和互联特性。


<details>
  <summary>Details</summary>
Motivation: 医学计算机视觉社区大多聚焦于推进特定任务的性能，而任务之间的底层关系（它们如何在表示层面关联、重叠或差异）仍未被充分探索。研究旨在揭示医学视觉任务之间的内在关系，探索30个任务（包括语义任务、图像生成任务和图像转换任务）在39个不同医学影像模态（CT、磁共振、电子显微镜、X射线、超声等）下的潜在结构。

Method: 提出Task-Contrastive Learning (TaCo)框架，这是一种对比学习框架，旨在将任务嵌入到共享的表示空间中。通过TaCo将来自不同模态的异构任务映射到联合空间中，分析它们的属性：识别哪些任务被明确表示，哪些任务混合在一起，以及任务的迭代变化如何在嵌入空间中反映。

Result: 通过TaCo框架成功将异构任务映射到联合表示空间中，揭示了任务间的相似性和差异性，识别出哪些任务被明确区分、哪些任务相互融合，以及任务迭代变化如何在嵌入空间中体现。为理解医学视觉任务的基本属性和互联性提供了 insights。

Conclusion: 研究为理解医学视觉任务的内在结构提供了基础，为理解任务相似性和嵌入空间中的互联特性提供了更深的理解。研究表明数据驱动的表示空间能够捕捉医学视觉任务之间的潜在结构和关系，不同任务在嵌入空间中有的清晰区分，有的相互融合。
...
### [34] [3D Smoke Scene Reconstruction Guided by Vision Priors from Multimodal Large Language Models](https://arxiv.org/abs/2604.05687)
*Xinye Zheng,Fei Wang,Yiqi Nie,Kun Li,Junjie Chen,Jiaqi Zhao,Yanyan Wei,Zhiliang Wu*

Main category: cs.CV

TL;DR: 本文提出了一种结合视觉先验与高效3D场景建模的框架，用于从烟雾退化的多视角图像中重建3D场景。该框架包含Nano-Banana-Pro图像增强模块和Smoke-GS中感知3D高斯溅射模块，能够在具有挑战性的烟雾环境中生成一致且视觉清晰的合成视角。


<details>
  <summary>Details</summary>
Motivation: 烟雾场景的3D重建面临重大挑战：烟雾引入强烈的散射效应、视角依赖的外观变化，以及跨视角一致性的严重退化。这些问题使得传统多视角三维重建方法难以直接应用于烟雾环境。

Method: 1) 使用Nano-Banana-Pro增强烟雾退化图像，提供更清晰的视觉观测；2) 提出Smoke-GS中感知3D高斯溅射框架，采用显式3D高斯建模场景；3) 引入轻量级视角依赖的介质分支，捕捉烟雾导致的方向依赖外观变化；4) 保持3D高斯溅射的渲染效率，同时提高对烟雾退化影响的鲁棒性。

Result: 实验结果表明，该方法在具有挑战性的烟雾环境中能够生成一致且视觉清晰的合成视角，有效解决了烟雾场景重建和面向恢复的新视角合成问题。

Conclusion: 该框架成功结合了图像增强与3D高斯溅射技术，通过中感知建模有效处理了烟雾散射和视角变化问题，为烟雾场景的高质量3D重建提供了有效解决方案。
...
### [35] [CRFT: Consistent-Recurrent Feature Flow Transformer for Cross-Modal Image Registration](https://arxiv.org/abs/2604.05689)
*Xuecong Liu,Mengzhu Ding,Zixuan Sun,Zhang Li,Xichao Teng*

Main category: cs.CV

TL;DR: CRFT是一种基于特征流学习的统一粗到细框架，用于鲁棒的跨模态图像注册。该方法在Transformer架构内学习模态无关的特征流表示，通过粗阶段建立全局对应关系，细阶段细化局部细节，并采用迭代差异引导注意力机制与空间几何变换 recurrently 细化流场。


<details>
  <summary>Details</summary>
Motivation: 跨模态图像配准在遥感、自动驾驶和医学成像等领域具有重要应用，但现有方法在处理大仿射和尺度变化时缺乏鲁棒性，需要一种能同时实现特征对齐和流估计的统一框架。

Method: 提出CRFT框架，包含：(1)Transformer架构学习模态无关特征流表示；(2)粗阶段通过多尺度特征相关性建立全局对应；(3)细阶段通过分层特征融合和自适应空间推理细化局部细节；(4)迭代差异引导注意力机制结合空间几何变换(SGT)递归细化流场，逐步捕捉细微空间不一致性并强制特征级一致性。

Result: 在多样跨模态数据集上的广泛实验表明，CRFT在准确性和鲁棒性方面始终优于最先进注册方法。该方法还为多模态空间对应提供了可泛化范式，适用于遥感、自动驾驶和医学成像。

Conclusion: CRFT提供了一种统一粗到细的跨模态图像配准框架，通过特征流学习和迭代细化机制有效处理大仿射和尺度变化，同时保持跨模态结构一致性。该方法具有良好的泛化能力，可应用于多个领域。
...
### [36] [Let Geometry GUIDE: Layer-wise Unrolling of Geometric Priors in Multimodal LLMs](https://arxiv.org/abs/2604.05695)
*Chongyu Wang,Ting Huang,Chunyu Sun,Xinyu Ning,Di Wang,Hao Tang*

Main category: cs.CV

TL;DR: 提出GUIDE框架，通过多层次几何采样和逐步对齐融合机制，将3D几何先验注入多模态大语言模型的早期层，提升模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有几何感知多模态大语言模型受限于单深层提取范式和平面融合方式，导致局部几何细节丢失和早期层语义不匹配问题。

Method: 1. 在几何编码器中进行多层次采样，捕获从局部边缘到全局拓扑的多粒度特征；2. 逐步将多层次几何先验与MLLM早期层对齐融合；3. 引入上下文感知门控机制，根据当前语义获取空间线索，抑制冗余几何噪声。

Result: GUIDE在多个复杂空间推理和感知任务上显著优于现有基线模型。

Conclusion: 建立了将3D几何先验集成到大模型的新范式，为提升多模态模型的空间感知能力提供了有效途径。
...
### [37] [GaussianGrow: Geometry-aware Gaussian Growing from 3D Point Clouds with Text Guidance](https://arxiv.org/abs/2604.05721)
*Weiqi Zhang,Junsheng Zhou,Haotian Geng,Kanle Shi,Shenkun Xu,Yi Fang,Yu-Shen Liu*

Main category: cs.CV

TL;DR: GaussianGrow是一种从3D点云生长3D高斯的新方法，通过多视图扩散模型和多视图约束来生成几何准确的高斯体，用于文本引导的高斯生成。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting在渲染效率和品质方面表现优异，但3D高斯的生成仍然缺乏适当的几何先验。现有方法通过预测点图作为几何参考来推断高斯图元，但不可靠的几何估计可能导致较差的生成效果。

Method: 设计了一种文本引导的高斯生长方案，利用多视图扩散模型从输入点云合成一致的外观进行监督。为减轻融合相邻视图产生的伪影，约束在重叠区域中非预设相机姿态生成的新视图。对于难以观察的区域，通过迭代检测最大未生长区域并使用预训练的2D扩散模型对渲染视图进行修复来完成。

Result: 在合成和真实扫描的点云上进行了广泛的评估，证明了GaussianGrow在文本引导高斯生成任务上的优越性能。

Conclusion: GaussianGrow通过从易获取的3D点云学习生长高斯，自然地保证了高斯生成的几何准确性，为3D Gaussian Splatting的生成提供了新的思路。
...
### [38] [Beyond Semantics: Disentangling Information Scope in Sparse Autoencoders for CLIP](https://arxiv.org/abs/2604.05724)
*Yusung Ro,Jaehyun Choi,Junmo Kim*

Main category: cs.CV

TL;DR: 本文提出信息scope（信息范围）作为解释CLIP视觉编码器SAE特征的新维度，通过上下文依赖性评分(CDS)区分局部范围和全局范围特征，发现不同信息范围的特征对CLIP预测和置信度有系统性不同影响。


<details>
  <summary>Details</summary>
Motivation: 现有SAE分析主要关注特征的语义含义，缺乏对特征空间聚合范围的系统理解。作者引入"信息范围"概念，区分patch级局部信号和图像级全局信号，以揭示特征聚合视觉证据的广度差异。

Method: 提出上下文依赖性评分(CDS)方法，通过空间扰动测试特征的位置稳定性：响应一致的为局部范围特征(positionally stable local scope)，随输入变化而转移的为全局范围特征(positionally variant global scope)。

Result: 实验表明：1)不同信息范围的SAE特征在空间扰动下表现差异显著；2)局部范围和全局范围特征对CLIP预测和置信度产生系统性不同的影响。

Conclusion: 信息范围是理解CLIP表征的关键新轴，为SAE特征提供了更深入的诊断视角，填补了特征语义分析之外的 interpretability 空白。
...
### [39] [FoleyDesigner: Immersive Stereo Foley Generation with Precise Spatio-Temporal Alignment for Film Clips](https://arxiv.org/abs/2604.05731)
*Mengtian Li,Kunyan Dai,Yi Ding,Ruobing Ni,Ying Zhang,Wenwu Wang,Zhifeng Xie*

Main category: cs.CV

TL;DR: 提出FoleyDesigner框架，通过多智能体架构和潜在扩散模型实现电影Foley音效的自动时空对齐生成，并构建首个专业立体声数据集FilmStereo。


<details>
  <summary>Details</summary>
Motivation: Foley艺术在增强电影沉浸式听觉体验中扮演着关键角色，但手动创建时空对齐的音频仍然劳动密集型。现有的方法缺乏对时空对齐的专业控制，且缺乏高质量的电影立体声音频数据集。

Method: 提出FoleyDesigner框架，采用多智能体架构进行精确的时空分析；使用潜在扩散模型结合从视频帧提取的时空线索进行 Foley生成；结合大语言模型(LLL)驱动的混合机制模拟电影后期制作流程；构建FilmStereo专业立体声数据集。

Result: 方法在时空对齐方面优于现有基线，与专业电影制作标准无缝兼容，支持5.1声道杜比Atmos系统，符合ITU-R BS.775标准。

Conclusion: FoleyDesigner是一个创新性框架，成功集成了视频分析、时空可控的Foley生成和专业音频混音能力，为电影制作提供了广泛的创作灵活性。
...
### [40] [On the Robustness of Diffusion-Based Image Compression to Bit-Flip Errors](https://arxiv.org/abs/2604.05743)
*Amit Vaisman,Gal Pomerants,Raz Lapid*

Main category: cs.CV

TL;DR: 本文研究了基于扩散的压缩方法（特别是Reverse Channel Coding(RCC)范式）对比特翻转的鲁棒性，发现其比传统和学习型编码器更具抵抗力，并提出了一种改进的Turbo-DDCM变体。


<details>
  <summary>Details</summary>
Motivation: 现代图像压缩方法通常优化率-失真-感知权衡，而对比特级腐败的鲁棒性很少被研究。作者想知道扩散-based compressors是否比传统codecs更鲁棒。

Method: 实验比较了基于RCC的扩散压缩器与传统和学习型codecs在比特翻转下的表现，并提出了一种更鲁棒的Turbo-DDCM变体。

Result: 基于RCC的扩散压缩器比传统和学习型codecs对比特翻转显著更鲁棒；改进的Turbo-DDCM变体在仅略微影响率-失真-感知权衡的情况下显著提高了鲁棒性。

Conclusion: RCC-based压缩可产生更具弹性的压缩表示，可能减少对高噪声环境下纠错码的依赖。
...
### [41] [SVC 2026: the Second Multimodal Deception Detection Challenge and the First Domain Generalized Remote Physiological Measurement Challenge](https://arxiv.org/abs/2604.05748)
...
TL;DR: 关于Subtle视觉挑战的研讨会比赛，旨在学习微妙视觉信号的稳健表示，包括跨域多模态欺骗检测和远程光电容积脉搏波谱(rPPG)估计两个任务，共有22个团队提交了最终结果。


<details>
  <summary>Details</summary>
Motivation: 微妙视觉信号虽然难以用肉眼感知，但包含揭示视觉数据中隐藏模式的重要信息。这些信号在生物特征安全多媒体取证、医学诊断、工业检测和情感计算等应用中发挥关键作用。现有研究通常专注于特定任务或模态，模型在处理真实环境中的微妙和弱信号时仍面临鲁棒性、表示能力和泛化方面的挑战。

Method: 组织Subtle视觉挑战赛(SVC)，包含两个任务：1)跨域多模态欺骗检测；2)远程光电容积脉搏波谱(rPPG)估计。提供MMDD2026平台和基线模型。

Result: 共有22个团队提交了最终结果到该研讨会比赛，相应的基线模型已发布在MMDD2026平台上。

Conclusion: 该挑战赛旨在鼓励开发更稳健和可泛化的微妙视觉理解模型，进一步推动计算机视觉和多模态学习的研究发展。
...
### [42] [Improving Controllable Generation: Faster Training and Better Performance via $x_0$-Supervision](https://arxiv.org/abs/2604.05761)
*Amadou S. Sangare,Adrien Maglo,Mohamed Chaouch,Bertrand Luvison*

Main category: cs.CV

TL;DR: 本文提出x_0监督方法，通过对干净目标图像的直接监督或等价地重新加权扩散损失，加速可控扩散模型的训练收敛。实验表明该方法可加速收敛达2倍，同时提升视觉质量和条件控制精度。


<details>
  <summary>Details</summary>
Motivation: 当前可控生成方法使用与初始模型相同的损失函数进行训练，导致在某些情况下收敛时间过长

Method: 通过对干净目标图像进行直接监督(x_0-supervision)或等价地重新加权扩散损失来实现更快的收敛

Result: 在多种控制设置下实验表明，该方法可加速收敛达2倍(mAUCC指标)，同时改善了视觉质量和条件控制精度

Conclusion: x_0监督方法通过重新审视扩散模型的去噪动态训练目标，有效解决了可控扩散模型训练收敛慢的问题，为可控生成提供了更高效的训练范式
...
### [43] [Beyond the Beep: Scalable Collision Anticipation and Real-Time Explainability with BADAS-2.0](https://arxiv.org/abs/2604.05767)
*Roni Goldshmidt,Hamish Scott,Lorenzo Niccolini,Hernan Matzner*

Main category: cs.CV

TL;DR: BADAS-2.0是一个先进的碰撞预警系统，通过构建长尾基准数据集、知识蒸馏和可解释性输出三个创新，在保持精度的同时实现了7-12倍的推理加速，特别在对罕见安全关键场景的预测上取得了最大改进。


<details>
  <summary>Details</summary>
Motivation: BADAS-2.0是第二代碰撞预警系统，基于BADAS-1.0构建。原有系统通过在大规模自我中心行车记录仪数据上微调V-JEPA2模型，在碰撞预测方面超越了学术基线和生产级ADAS系统。开发者希望进一步推进技术边界，特别是针对长尾分布中罕见且安全关键的场景。

Method: 该研究沿三个轴心推进技术：(i) 长尾基准构建：利用BADAS-1.0作为主动预言机对数百万未标记驾驶进行评分，筛选高风险候选样本进行标注，结合Nexar Atlas平台进行针对性数据收集，将数据集从4万扩展到17.85万个标注视频（约200万个片段）；(ii) 知识蒸馏至边缘端：在225万个未标记驾驶视频上进行领域特定的自监督预训练，蒸馏出紧凑模型BADAS-2.0-Flash（86M参数）和BADAS-2.0-Flash-Lite（22M参数）；(iii) 可解释性：生成实时对象级注意力热图定位预测依据，BADAS-Reason进一步利用视觉语言模型结合最后一帧和热图生成驾驶员动作和结构化文本推理。

Result: 数据集从4万扩展到17.85万个标注视频（约200万个片段），在所有子组均实现一致性能提升，尤其在最难的长尾案例上提升幅度最大。紧凑模型实现7-12倍加速且保持接近的精度，支持实时边缘部署。

Conclusion: BADAS-2.0在碰撞预警系统的准确性、效率和可解释性方面取得了显著进展，通过创新的长尾数据构建方法、模型蒸馏技术和可解释性输出，为自动驾驶安全系统提供了更实用的解决方案。
...
### [44] [PDMP: Rethinking Balanced Multimodal Learning via Performance-Dominant Modality Prioritization](https://arxiv.org/abs/2604.05773)
*Shicai Wei,Chunbo Luo,Qiang Zhu,Yang Luo*

Main category: cs.CV

TL;DR: 本文提出了一种新的多模态学习策略PDMP（性能主导模态优先），认为性能主导模态驱动的非平衡学习比平衡学习更有利于多模态模型性能提升。


<details>
  <summary>Details</summary>
Motivation: 多模态学习常出现优化不足问题，即多模态模型性能甚至不如单模态模型。现有问题归因于模态间的学习不平衡，并通过梯度调制解决，但本文认为平衡学习并非最优设置。

Method: 首先通过独立训练的单模态模型性能排名挖掘性能主导模态；然后引入非对称系数调制各模态梯度，使性能主导模态主导优化过程。该方法仅依赖单模态性能排名，与多模态模型结构和融合方法无关。

Result: 在多个数据集上的广泛实验验证了PDMP的优越性。

Conclusion: 性能主导模态的充分学习是解决多模态模型优化不足问题的关键，PDMP策略具有广泛的实际应用潜力。
...
### [45] [Sparsity-Aware Voxel Attention and Foreground Modulation for 3D Semantic Scene Completion](https://arxiv.org/abs/2604.05780)
*Yu Xue,Longjun Gao,Yuanqi Su,HaoAng Lu,Xiaoning Zhang*

Main category: cs.CV

TL;DR: VoxSAMNet通过显式建模体素稀疏性和语义不平衡来解决单目语义场景补全问题，提出DSFR模块和前景调制策略，在两个基准数据集上刷新mIoU纪录。


<details>
  <summary>Details</summary>
Motivation: 单目语义场景补全存在两大核心挑战：一是体素分布天然不平衡(超过93%为空体素，前景类别稀少)；二是现有方法对无信息体素进行冗余强调，且对长尾类别泛化性能差。

Method: (1) 虚像快捷特征细化(DSFR)模块：通过共享虚像节点绕过空体素，并利用变形注意力细化占用体素的特征；(2) 前景调制策略：结合前景Dropout(FD)和文本引导图像滤波(TGIF)减轻过拟合并增强类别相关特征。

Result: 在SemanticKITTI和SSCBench-KITTI-360公共基准上取得最佳性能，mIoU分别为18.2%和20.2%，超越现有单目和双目基线方法。

Conclusion: VoxSAMNet通过显式建模体素稀疏性和语义不平衡，解决了单目语义场景补全中的核心挑战。该框架创新性地引入虚像节点绕过空体素，并使用变形注意力增强占用体素的特征；同时通过前景 dropout和文本引导图像滤波缓解长尾类别过拟合问题。实验表明该方法在SemanticKITTI和SSCBench-KITTI-360上分别达到18.2%和20.2%的mIoU，超越现有单目和双目方法，为高效准确的3D场景补全提供了新思路。
...
### [46] [WikiSeeker: Rethinking the Role of Vision-Language Models in Knowledge-Based Visual Question Answering](https://arxiv.org/abs/2604.05818)
*Yingjian Zhu,Xinming Wang,Kun Ding,Ying Wang,Bin Fan,Shiming Xiang*

Main category: cs.CV

TL;DR: WikiSeeker是一种新型多模态RAG框架，通过将视觉语言模型重新定义为Refiner和Inspector两个专业角色，显著提升了知识基视觉问答的检索准确率和答案质量，在三个基准数据集上达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态检索增强生成(RAG)在知识基视觉问答(KB-VQA)领域存在两个主要问题：1) 主要依赖图像作为检索键，忽视了文本查询的重要性；2) 未能充分发挥视觉语言模型(VLM)的潜力，仅将其作为答案生成器。

Method: 提出WikiSeeker框架，包含：1) 多模态检索器；2) 将VLM重新定义为两个专业 agent——Refiner（根据图像重写文本查询以改进检索性能）和Inspector（通过解耦生成策略，选择性地将可靠检索上下文路由到LLM，或在检索不可靠时使用VLM内部知识）。

Result: 在EVQA、InfoSeek和M2KR数据集上实现了最先进的性能，检索准确率和答案质量均获得显著提升。

Conclusion: WikiSeeker通过重新定义VLM角色和提出多模态检索策略，有效解决了现有方法的局限性，为KB-VQA任务提供了新的解决方案。
...
### [47] [Reading Between the Pixels: An Inscriptive Jailbreak Attack on Text-to-Image Models](https://arxiv.org/abs/2604.05853)
*Zonghao Ying,Haowen Dai,Lianyu Hu,Zonglei Jing,Quanchen Zou,Yaodong Yang,Aishan Liu,Xianglong Liu*

Main category: cs.CV

TL;DR: 提出inscriptive jailbreak新型攻击，利用T2I模型的文本渲染能力生成包含恶意文本内容的图像，通过Etch框架分解为语义伪装、视觉空间锚定和字体编码三层进行攻击优化


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像(T2I)模型现在可以渲染可读的段落长度文本，这带来了一种根本性的新型滥用方式。传统的视觉 Jailbreak 技术设计用于粗粒度视觉操作，难以在维护字符级保真度的同时绕过多阶段安全过滤器

Method: 提出Etch黑盒攻击框架，将对抗性提示分解为三个功能正交层：语义伪装、视觉空间锚定和字体编码。通过零阶循环迭代优化这三个子问题，使用视觉语言模型评估生成的图像，定位失败并提供针对性修改建议

Result: 在7个模型和2个基准测试上实现平均攻击成功率65.57%（峰值91.00%），显著优于现有基线方法

Conclusion: 揭示了当前T2I安全对齐中的关键盲点，强调了需要 typography-aware defense 多模态机制来防御此类攻击
...
### [48] [Automatic dental superimposition of 3D intraorals and 2D photographs for human identification](https://arxiv.org/abs/2604.05877)
...
TL;DR: 提出了一种3D-2D牙科鉴定方法，将死亡后3D扫描与社交媒体上的生前照片进行形态学比较。开发了两种自动方法：配对地标法和牙齿分割法，在142个样本的20,164次交叉比较中分别获得1.6和1.5的平均排名值，性能优于现有自动牙科图表比对方法。


<details>
  <summary>Details</summary>
Motivation: 牙科比较是身份鉴定的主要方法（与指纹和DNA同等重要），但形态学比较步骤耗时且存在挑战。特别是在移民死亡边境地区或没有全民医疗的国家，缺乏生前医疗记录。社交媒体上可見牙齿的照片为牙科鉴定提供了新资源，但现有方法存在显著局限：缺乏透视畸变的正确建模，以及缺乏客观量化形态差异的方法。

Method: 提出3D（死亡后扫描）-2D（生前照片）方法。使用计算机视觉和优化技术，用3D模型重现生前照片进行形态学比较。开发了两种自动方法：1）使用配对地标；2）使用牙齿分割来估计相机参数。

Result: 在142个样本的20,164次交叉比较中，两种方法分别获得1.6和1.5的平均排名值。结果明显优于自动牙科图表比对方法，提供了自动、客观和可量化的形态对应评分，可通过叠加图像可视化轻松解释和分析。

Conclusion: 该方法成功解决了先前方法的局限，为牙科身份鉴定提供了非常 promising 的结果，实现了自动、客观和可量化的形态学比较，可通过可视化叠加图像轻松解释。
...
### [49] [Physics-Aware Video Instance Removal Benchmark](https://arxiv.org/abs/2604.05898)
*Zirui Li,Xinghao Chen,Lingyu Jiang,Dengzhe Hou,Fangzhou Lin,Kazunori Yamada,Xiangbo Gao,Zhengzhong Tu*

Main category: cs.CV

TL;DR: 提出了一个名为PVIR的视频实例移除基准数据集，包含95个高质量视频，分为简单和困难两个子集，专注于评估物理因果关系的恢复能力，实验表明现有方法在复杂物理交互场景下仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 当前视频实例移除方法的评估基准主要关注视觉 plausibility，忽略了物理因果关系（如残留阴影、光照交互等）的恢复，导致无法全面评估方法的真实效果。

Method: 构建了PVIR基准数据集，包含95个高质量视频，标注了实例精确的掩码和移除提示；将数据分为Simple和Hard子集，后者专门针对复杂物理交互；采用解耦的人工评估协议，从指令遵循、渲染质量和编辑排他性三个维度评估四种代表性方法。

Result: PISCO-Removal和UniVideo达到最佳性能；DiffuEraser频繁出现模糊伪影；CoCoCo在指令遵循方面存在明显困难；Hard子集上的性能持续下降凸显了恢复复杂物理副作用的挑战。

Conclusion: 该基准数据集揭示了当前视频实例移除方法在处理复杂物理交互方面的显著不足，PVIR为推动该领域的研究提供了重要的评估标准和挑战基准。
...
### [50] [AICA-Bench: Holistically Examining the Capabilities of VLMs in Affective Image Content Analysis](https://arxiv.org/abs/2604.05900)
*Dong She,Xianrong Yao,Liqun Chen,Jinghe Yu,Yang Gao,Zhanpeng Jin*

Main category: cs.CV

TL;DR: 提出AICA-Bench基准测试，用于评估视觉语言模型在情感图像内容分析上的能力，包含情感理解、情感推理和情感引导内容生成三个任务，并提出GAT Prompting框架来解决模型在强度校准和开放性描述方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型虽在感知方面表现出色，但缺乏整合感知、推理和生成的整体情感图像内容分析能力，现有的AICA研究仍不充分。

Method: 构建了包含三种核心任务的AICA-Bench基准测试（情感理解EU、情感推理ER、情感引导内容生成EGCG），评估了23个VLM。针对发现的两大局限（强度校准能力弱、开放性描述浅），提出GAT Prompting无训练框架，结合视觉脚手架与层级推理。

Result: GAT框架有效降低了情感强度误差，提升了描述的深度和丰富度，为情感多模态理解和生成提供了强有力的基线。

Conclusion: AICA-Bench和GAT Prompting为情感多模态理解与生成的后续研究奠定了坚实基础。
...
### [51] [Selective Aggregation of Attention Maps Improves Diffusion-Based Visual Interpretation](https://arxiv.org/abs/2604.05906)
...
### [52] [Appearance Decomposition Gaussian Splatting for Multi-Traversal Reconstruction](https://arxiv.org/abs/2604.05908)
*Yangyi Xiao,Siting Zhu,Baoquan Yang,Tianchen Deng,Yongbo Chen,Hesheng Wang*

Main category: cs.CV

TL;DR: ADM-GS是一种用于多遍历场景重建的新框架，通过将静态背景的外观分解为遍历不变的材料属性和遍历依赖的照明变化，从而缓解外观不一致问题。


<details>
  <summary>Details</summary>
Motivation: 多遍历场景重建对于高保真自动驾驶仿真和数字孪生构建至关重要。主要挑战是不同时间捕获的序列之间存在显著的 appearance 不一致性，尽管底层几何共享，但光照和环境条件不同。

Method: 提出神经光场采用频率分离的混合编码策略，结合表面法线和显式反射向量，分别捕获低频漫反射和高频镜面反射。

Result: 在Argoverse 2和Waymo Open数据集上的定量评估表明，ADM-GS在多遍历实验中相比现有潜在基线方法实现了+0.98 dB的PSNR提升，同时产生了更一致的外观。

Conclusion: 通过显式外观分解方法，ADM-GS有效解决了多遍历场景中的外观不一致问题，为自动驾驶仿真和数字孪生应用提供了更高质量的场景重建。
...
### [53] [Saliency-Guided Representation with Consistency Policy Learning for Visual Unsupervised Reinforcement Learning](https://arxiv.org/abs/2604.05931)
*Jingbo Sun,Qichao Zhang,Songjun Tu,Xing Fang,Yupeng Zheng,Haoran Li,Ke Chen,Dongbin Zhao*

Main category: cs.CV

TL;DR: 本文提出SRCP框架，通过显著性引导的表示学习和一致性策略学习来解决视觉无监督强化学习中成功表示方法的零样本泛化问题。


<details>
  <summary>Details</summary>
Motivation: 现有的成功表示(SR)方法在视觉环境中面临两个关键局限：1) SR目标导致次优表示，关注动力学无关区域，导致成功度量不准确；2) 这些有缺陷的表示阻碍SR策略建模多模态技能条件动作分布和确保技能可控性。

Method: 提出Saliency-Guided Representation with Consistency Policy Learning (SRCP)框架，包含两个核心组件：1) 引入显著性引导的动力学任务来捕获与动力学相关的表示；2) 集成快速采样的Consistency Policy与URL特定的无分类器引导和定制训练目标。

Result: SRCP在ExORL基准的16个任务上实现了最先进的零样本泛化性能，并且与各种SR方法兼容。

Conclusion: SRCP在视觉URL中实现了最先进的零样本泛化性能，并且与各种SR方法兼容，在ExORL基准的4个数据集的16个任务上进行了广泛验证。
...
### [54] [SonoSelect: Efficient Ultrasound Perception via Active Probe Exploration](https://arxiv.org/abs/2604.05933)
*Yixin Zhang,Yunzhong Hou,Longqi Li,Zhenyue Qin,Yang Liu,Yue Yao*

Main category: cs.CV

TL;DR: 提出了SonoSelect方法，用于超声主动视图探索，通过自适应引导探头移动来减少冗余扫描，仅使用2个视图实现多视图器官分类，并在肾脏囊肿检测任务中达到54.56%肾脏覆盖率和35.13%囊肿覆盖率。


<details>
  <summary>Details</summary>
Motivation: 超声检查通常需要多次扫描来减少诊断歧义、消除声学遮挡并提高解剖覆盖率，但并非所有探头视图都同等信息丰富，过多扫描会引入冗余并增加成本，因此需要一种自适应的探头移动指导方法。

Method: 将超声主动视图探索问题建模为顺序决策问题，每个新的2D超声视图被融合到观察解剖结构的3D空间记忆中，用于指导下一个探头位置，并提出结合器官覆盖率高、重建不确定性低和冗余扫描少的目标函数。

Result: 在超声模拟器上的实验表明，SonoSelect仅使用2个视图即可实现多视图器官分类准确率，对于更困难的肾脏囊肿检测任务，达到54.56%肾脏覆盖率和35.13%囊肿覆盖率，轨迹始终短且集中在目标囊肿上。

Conclusion: SonoSelect能够有效减少超声扫描的冗余，通过自适应选择信息丰富的探头视图来提高诊断效率，为超声检查的智能化扫查提供了一个有前景的方向。
...
### [55] [Leveraging Image Editing Foundation Models for Data-Efficient CT Metal Artifact Reduction](https://arxiv.org/abs/2604.05934)
...
TL;DR: 本文提出了一种基于视觉语言扩散基础模型的CT金属伪影去除方法，通过低秩自适应(LoRA)和多参考 conditioning 策略，仅需16-128个配对训练样本即可实现先进的伪影抑制性能。


<details>
  <summary>Details</summary>
Motivation: 高衰减植入物产生的金属伪影严重降低CT图像质量，遮挡关键解剖结构，且传统深度学习方法需要大量配对训练数据。

Method: 将伪影去除重新定义为上下文推理任务，采用参数高效的LoRA方法微调通用视觉语言扩散基础模型，提出多参考 conditioning 策略提供干净的解剖示例。

Result: 在AAPM CT-MAR基准上达到最先进的感知和放射学特征指标性能，仅需16-128个配对样本将数据需求降低两个数量级。验证了领域适应对缓解幻觉的重要性。

Conclusion: 适当adapted的基础模型为可解释、数据高效的医学图像重建提供了可扩展的替代方案。
...
### [56] [Mixture-of-Modality-Experts with Holistic Token Learning for Fine-Grained Multimodal Visual Analytics in Driver Action Recognition](https://arxiv.org/abs/2604.05947)
*Tianyi Liu,Yiming Li,Wenqian Wang,Jiaojiao Wang,Chen Cai,Yi Wang,Kim-Hui Yap*

Main category: cs.CV

TL;DR: 本文提出MoME框架和HTL策略，用于解决异构模态下自适应协作和细粒度动作线索捕获的问题，在驾驶员动作识别任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有 multimodal学习方法主要依赖固定融合模块或预定义跨模态交互，难以适应模态可靠性的变化和捕获细粒度动作线索。

Method: 提出Mixture-of-Modality-Experts (MoME)框架实现模态特定专家的自适应协作；提出Holistic Token Learning (HTL)策略，通过类别token和时空token改进专家内部细化和跨专家知识转移。

Result: 在驾驶员动作识别基准数据集上，MoME框架和HTL策略联合性能优于代表性的单模态和多模态基线方法。

Conclusion: HTL策略能够改善细微多模态理解并提供更好的可解释性，形成以知识为中心的多模态学习框架，提高专家专业化程度并减少多模态融合中的歧义。
...
### [57] [HumANDiff: Articulated Noise Diffusion for Motion-Consistent Human Video Generation](https://arxiv.org/abs/2604.05961)
*Tao Hu,Varun Jampani*

Main category: cs.CV

TL;DR: HumANDiff是一个增强人类运动控制的人体视频生成框架，通过关节噪声采样、联合外观-运动学习和几何运动一致性损失三个创新设计，无需修改扩散模型架构即可实现高质量可控人体视频生成。


<details>
  <summary>Details</summary>
Motivation: 尽管近期人类视频生成取得了巨大进展，但生成视频扩散模型仍难以真实捕捉人类动作的动态和物理特性。

Method: 提出三个关键设计：1）关节运动一致噪声采样：将潜在噪声的时空分布与统计人体模板密集表面流形上采样的3D关节噪声相关联，继承身体拓扑先验；2）联合外观-运动学习：联合预测像素外观和对应的物理运动；3）几何运动一致性学习：在关节噪声空间中定义几何运动一致性损失，强制跨帧物理运动一致性。

Result: 在渲染具有多样服装风格的动作一致、高保真人体方面实现了最先进的性能。

Conclusion: HumANDiff是一种与扩散模型无关的可扩展可控人体视频生成框架，只需对视频扩散模型进行微调，无需修改模型架构。推理时可在单一框架内实现图像到视频生成，无需额外的运动模块。在渲染多样化服装风格的一致性动作、高保真人体方面实现了最先进的性能。
...
### [58] [Is CLIP Cross-Eyed? Revealing and Mitigating Center Bias in the CLIP Family](https://arxiv.org/abs/2604.05971)
*Oscar Chew,Hsiao-Ying Huang,Kunal Jain,Tai-I Chen,Khoa D Doan,Kuan-Hao Huang*

Main category: cs.CV

TL;DR: 本文识别了CLIP模型中的中心偏置问题，即过度关注图像中心而忽略边缘物体。通过嵌入分解和注意力图分析，发现池化机制导致边缘物体信息丢失。无训练策略可有效缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究已发现CLIP缺乏细粒度视觉理解能力，但作者识别出CLIP家族中存在的一种独特失败模式——中心偏置，即模型过度关注图像中心区域而忽略边缘的重要物体。

Method: 采用可解释性方法（嵌入分解和注意力图分析）从表示和注意力两个角度分析问题根源，并通过无训练策略进行干预。

Result: 发现相关概念尤其是与偏离中心物体相关的概念在最终表示中消失，原因是视觉嵌入聚合过程中（尤其是池化机制）的信息丢失。训练-free策略可有效重定向模型注意力到边缘区域。

Conclusion: 中心偏置可通过无训练策略（如视觉提示和注意力重分配）有效缓解，使模型将注意力转向边缘区域。
...
### [59] [OmniCamera: A Unified Framework for Multi-task Video Generation with Arbitrary Camera Control](https://arxiv.org/abs/2604.06010)
*Yukun Wang,Ruihuang Li,Jiale Tao,Shiyuan Yang,Liyi Chen,Zhantao Yang,Handz,Yulan Guo,Shuai Shao,Qinglin Lu*

Main category: cs.CV

TL;DR: OmniCamera 是一个统一的视频生成框架，通过解耦相机运动和场景动态内容实现灵活的视频生成控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型往往将相机运动和动态内容混在一起，限制了独立控制能力，需要一种明确解耦和命令这两个维度的统一框架。

Method: 提出两个关键创新：1) 构建 OmniCAM 混合数据集，结合真实世界视频和合成数据；2) 提出双层次课程协同训练策略，从难到难引入控制模态（条件层次），从合成数据到真实数据适应（数据层次）。

Result: 在复杂相机运动控制和视觉质量方面均达到最先进性能。

Conclusion: OmniCamera 通过解耦相机运动和内容实现灵活的创意控制，在复杂相机运动控制方面达到最先进水平，同时保持卓越的视觉质量。
...
### [60] [Toward Aristotelian Medical Representations: Backpropagation-Free Layer-wise Analysis for Interpretable Generalized Metric Learning on MedMNIST](https://arxiv.org/abs/2604.06017)
*Michael Karnes,Alper Yilmaz*

Main category: cs.CV

TL;DR: 提出A-ROM框架，基于Platonic Representation Hypothesis，利用预训练Vision Transformers的通用度量空间，通过人类可读的概念字典和kNN分类器替代传统不透明决策层，实现可解释的医学影像少样本学习。


<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像领域取得成功，但其基于反向传播的"黑箱"模型阻碍了临床应用。现有模型的决策过程不透明，难以满足现代临床环境对模型可解释性的严格要求。

Method: 基于Platonic Representation Hypothesis理论，利用预训练Vision Transformers的通用度量空间构建A-ROM框架；采用人类可读的概念字典替代传统决策层，结合k-Nearest Neighbors分类器实现可解释的分类，无需进行梯度微调。

Result: 在MedMNIST v2数据集上的实验表明，A-ROM性能与标准基准相当，同时提供了简单可扩展的少样本解决方案，满足临床环境的透明度要求。

Conclusion: A-ROM通过可解释的概念字典和kNN分类器实现了透明可理解的医学影像分析，在保持竞争力的同时为临床应用提供了一种低计算负担、高可解释性的可行方案。
...
### [61] [Attention, May I Have Your Decision? Localizing Generative Choices in Diffusion Models](https://arxiv.org/abs/2604.06052)
*Katarzyna Zaleska,Łukasz Popek,Monika Wysoczańska,Kamil Deja*

Main category: cs.CV

TL;DR: 本文研究文本到图像扩散模型在处理模糊提示时的隐式决策机制，提出使用探针定位技术识别概念属性分离度最高的层，并基于此提出ICM（隐式选择修改）精确引导方法，通过针对自注意力层的定向干预实现优于现有方法的去偏效果。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然生成能力出色，但其内部运作机制不透明，特别是在处理未完全描述的提示时需要隐式决策细节。现有的定位技术关注显式条件作用，而隐式决策可能有所不同，因此需要新的方法来研究决策过程的计算局部化特性。

Method: 引入基于探针的定位技术来识别概念属性分离度最高的层；在发现自注意力层是最有效的干预点后，提出ICM（隐式选择修改）方法，该方法仅对少量自注意力层进行精确的定向干预。

Result: 自注意力层被识别为模糊概念解析的关键位置，干预这些特定的自注意力层能够取得优于现有最先进方法的去偏性能，同时最大程度减少精确定位较低方法常见的伪影。

Conclusion: 文本到图像扩散模型中处理模糊概念的决策过程集中在自注意力层，ICM方法通过针对这些层的精准干预实现了高效的去偏效果，为理解模型内部机制和引导生成提供新的思路。
...
### [62] [Scientific Graphics Program Synthesis via Dual Self-Consistency Reinforcement Learning](https://arxiv.org/abs/2604.06079)
*Juekai Lin,Yun Zhu,Honglin Lin,Sijing Li,Tianwei Lin,Zheng Liu,Xiaoyang Wang,Wenqiao Zhang,Lijun Wu*

Main category: cs.CV

TL;DR: 本文提出SciTikZ框架，用于将静态图像逆向工程为可编辑的TikZ代码。框架包含SciTikZ-230K大规模数据集和SciTikZ-Bench多维度基准，并创新性地引入Dual Self-Consistency强化学习优化范式。训练得到的SciTikZer-8B模型在TikZ代码生成任务上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 1) Graphics Program Synthesis对于可视数据的解释和编辑至关重要，可将静态图像逆向工程为可编辑TikZ代码；2) TikZ作为科学图表的事实标准，具有程序化灵活性但对空间精度要求严格，对多模态大语言模型构成重大挑战；3) 当前进展受限于两个主要差距：数据质量差距（现有图像-TikZ语料库缺乏严格可执行性和可靠视觉对齐）和评估差距（缺乏结构和视觉保真度的基准测试）。

Method: 提出闭环框架包含三个核心组件：1) SciTikZ-230K：来自Execution-Centric Data Engine的大规模高质量数据集，涵盖11个不同科学学科；2) SciTikZ-Bench：多层面基准测试，涵盖从基本几何构建到复杂层次 Schematic，评估视觉保真度和结构逻辑；3) 创新性提出Dual Self-Consistency强化学习优化范式，利用Round-Trip Verification惩罚退化代码并提升整体自一致性。

Result: 训练得到的SciTikZer-8B模型达到最先进性能，持续优于专有巨头如Gemini-2.5-Pro和大型模型如Qwen3-VL-235B-A22B-Instruct。

Conclusion: 该框架成功解决了TikZ图形程序 synthesis领域的数据质量和评估差距问题，通过高质量数据集、严格基准测试和创新性强化学习方法，显著提升了多模态大模型生成精确TikZ代码的能力。
...
### [63] [EDGE-Shield: Efficient Denoising-staGE Shield for Violative Content Filtering via Scalable Reference-Based Matching](https://arxiv.org/abs/2604.06063)
*Takara Taniguchi,Ryohei Shimizu,Minh-Duc Vo,Kota Izumi,Shiqi Yang,Teppei Suzuki*

Main category: cs.CV

TL;DR: 本文提出EDGE-Shield，一种在扩散模型去噪过程中进行可扩展内容过滤的框架，利用嵌入匹配提高效率，并通过x-pred转换将噪声潜在表示转换为后期清洁潜在估计，提升早期去噪阶段对侵权内容的分类准确率。实验表明在Z-Image-Turbo和Qwen-Image上分别实现约79%和50%的延迟降低。


<details>
  <summary>Details</summary>
Motivation: 当前Text-to-Image生成模型存在版权侵权和深度伪造风险，现有参考方法缺乏可扩展性且需等待图像生成完成，缺乏训练-free的实时保护方案。

Method: 1) 在去噪过程中实施内容过滤而非生成后过滤；2) 使用基于嵌入的匹配进行高效参考比较；3) 引入x-pred转换将模型中间噪声潜在转换为后期伪估计清洁潜在以增强早期分类准确性。

Result: 相比传统参考方法，EDGE-Shield在Z-Image-Turbo上降低约79%处理时间，在Qwen-Image上降低约50%，同时在不同模型架构上保持过滤准确率。

Conclusion: 该方法实现了可扩展且低延迟的侵权内容过滤，为Text-to-Image模型提供了有效的运行时保护机制。
...
### [64] [Lightweight Multimodal Adaptation of Vision Language Models for Species Recognition and Habitat Context Interpretation in Drone Thermal Imagery](https://arxiv.org/abs/2604.06124)
*Hao Chen,Fang Qiu,Fangchao Dong,Defei Yang,Eve Bohnett,Li An*

Main category: cs.CV

TL;DR: 研究提出轻量级多模态适配框架，将RGB预训练的视觉语言模型迁移到热红外无人机影像，实现物种识别和实例计数，并结合RGB影像生成栖息地背景信息。


<details>
  <summary>Details</summary>
Motivation: 弥合RGB预训练的视觉语言模型(VLM)与热红外图像之间的表征差距，利用无人机采集的真实数据集验证其实际应用价值。

Method: 构建无人机采集的热红外数据集，通过多模态投影器对齐微调VLM，实现从RGB视觉表征到热辐射输入的信息迁移。测试InternVL3-8B-Instruct、Qwen2.5-VL-7B-Instruct和Qwen3-VL-8B-Instruct三种模型在封闭集和开放集提示下的物种识别和实例计数性能。

Result: Qwen3-VL-8B-Instruct在开放集提示下表现最佳，鹿、犀牛和大象的F1分数分别为0.935、0.915和0.968，枚举准确率分别为0.779、0.982和1.000。热红外与RGB影像结合可生成栖息地背景信息(土地覆盖、景观特征、人为干扰)。

Conclusion: 轻量级投影器适配为将RGB预训练的VLM迁移到热红外无人机影像提供了有效途径，将应用从目标识别扩展到栖息地背景解读，为生态监测提供实用方案。
...
### [65] [PoM: A Linear-Time Replacement for Attention with the Polynomial Mixer](https://arxiv.org/abs/2604.06129)
...
TL;DR: PoM是一种新型线性复杂度的token混合机制，通过多项式函数实现token信息聚合，可替代自注意力，在多个领域达到与注意力模型相当的性能，同时显著降低长序列处理的计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决Transformer中自注意力机制的计算复杂度问题。当序列长度增加时，自注意力的计算复杂度呈二次方增长，导致处理长序列时计算成本过高。需要一种既能保持Transformer表达能力又具有线性复杂度的token混合机制。

Method: 提出Polynomial Mixer (PoM)：通过学习多项式函数将输入token聚合为紧凑表示，然后每个token从该表示中检索上下文信息。PoM具有线性复杂度，可作为自注意力的即插即用替代品。理论上证明了PoM满足上下文映射属性，确保配备PoM的Transformer仍是通用序列到序列近似器。

Result: 在五个不同领域（文本生成、手写文本识别、图像生成、3D建模、地球观测）进行实验，PoM达到了与注意力模型相当的性能，同时在处理长序列时大幅降低了计算成本。

Conclusion: PoM是一种有效的轻量级token混合机制，在保持Transformer强大表达能力的同时解决了计算效率问题，具有广泛的实际应用价值。
...
TL;DR: MMEmb-R1是一种自适应推理的多模态嵌入框架，通过将推理建模为潜在变量，结合配对感知推理选择和强化学习，在仅4B参数下于MMEB-V2基准达到71.2分，实现SOTA并大幅降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 直接将思维链推理整合到嵌入学习中存在两大根本挑战：一是实例级推理与成对对比监督之间的结构错位导致捷径行为；二是推理并非对所有嵌入任务都普遍有益，强制对所有输入进行推理会引入不必要的计算和延迟，甚至可能掩盖简单情况下的显著语义信号。

Method: 本文提出MMEmb-R1框架，采用三项关键技术：（1）将推理建模为潜在变量；（2）引入配对感知推理选择，利用反事实干预识别对查询-目标对齐有益的推理路径；（3）采用强化学习仅在必要时调用推理。

Result: 在MMEB-V2基准上的实验表明，模型仅用4B参数就达到了71.2分的成绩，刷新了最先进水平，同时显著降低了推理开销和推理延迟。

Conclusion: MMEmb-R1通过将推理建模为潜在变量，结合配对感知推理选择和强化学习，实现了自适应推理调用。该方法在MMEB-V2基准上以仅4B参数达到71.2分，刷新了最先进水平，同时显著降低了推理开销和延迟。
...
### [67] [DiffHDR: Re-Exposing LDR Videos with Video Diffusion Models](https://arxiv.org/abs/2604.06161)
...
TL;DR: DiffHDR是一个利用视频扩散模型在潜在空间中进行LDR到HDR转换的框架，通过在Log-Gamma色彩空间中操作，能够在过曝和欠曝区域合成逼真的HDR辐射率，支持文本提示或参考图像引导的可控转换。


<details>
  <summary>Details</summary>
Motivation: 大多数数字视频以8位低动态范围(LDR)格式存储，在饱和和量化过程中丢失了大量原始高动态范围(HDR)场景辐射率。高光和阴影细节的丢失阻碍了HDR显示的精确亮度映射，并限制了后期制作中的有意义重曝光。现有的LDR转HDR技术难以在过曝和欠曝区域恢复真实细节。

Method: 将LDR到HDR转换问题重新表述为视频扩散模型潜在空间中的生成辐射率修复任务；在Log-Gamma色彩空间中操作以利用时空生成先验；开发了从静态HDRI图合成HDR视频训练数据的管道以解决配对HDR视频数据稀缺问题。

Result: DiffHDR在辐射率保真度和时间稳定性方面显著优于最先进的方法，能够生成具有相当重曝光灵活性的逼真HDR视频。

Conclusion: DiffHDR通过扩散模型和生成修复方法成功解决了LDR到HDR转换中过曝和欠曝区域细节恢复的难题，为HDR视频处理提供了新的研究方向。
...
### [68] [SEM-ROVER: Semantic Voxel-Guided Diffusion for Large-Scale Driving Scene Generation](https://arxiv.org/abs/2604.06113)
...
TL;DR: 本文提出基于Σ-Voxfield网格的3D生成框架，通过语义条件扩散模型在局部体素邻域上操作，结合3D位置编码捕捉空间结构，采用渐进式空间扩展处理大场景，最终使用延迟渲染模块生成照片级图像，实现无需每场景优化的可扩展多视图一致性3D城市场景生成。


<details>
  <summary>Details</summary>
Motivation: 现有户外驾驶场景的可扩展生成方案存在局限：基于图像/视频生成模型蒸馏到3D空间的方法损害几何一致性且仅限训练视角；小规模3D场景或物体中心生成又无法扩展到大场景。需要一种保持多视角几何一致性且能扩展到大区域的方法。

Method: 提出Σ-Voxfield网格表示，每个占据体素存储固定数量的着色表面样本；训练语义条件扩散模型，在局部体素邻域上操作并使用3D位置编码捕捉空间结构；通过重叠区域渐进式空间扩展实现大场景缩放；使用延迟渲染模块将生成的Σ-Voxfield网格渲染为照片级真实感图像。

Result: 可生成多样化的规模化城市场景，能够以各种传感器配置和相机轨迹渲染为照片级真实感图像，与现有方法相比保持适中的计算成本，无需每场景优化。

Conclusion: 该方法实现了无需每场景优化的大规模多视图一致性3D场景生成，为自动驾驶模拟提供了可扩展的照片级渲染能力，在场景多样性和计算效率之间取得良好平衡。
...
### [69] [The Character Error Vector: Decomposable errors for page-level OCR evaluation](https://arxiv.org/abs/2604.06160)
*Jonathan Bourne,Mwiza Simbeye,Joseph Nockels*

Main category: cs.CV

TL;DR: 提出CEV作为OCR评估指标，解决页面解析错误时CER无法定义的问题，可分解为解析、OCR和交互误差三个部分，验证了其在文档理解评估中的有效性


<details>
  <summary>Details</summary>
Motivation: 传统CER指标假设文本已完美解析，但在页面解析错误时CER无法定义，限制了其在页面级OCR评估中的应用，尤其在不同标注方案的数据集上

Method: 提出字符错误向量(CEV)方法，将误差分解为解析误差、OCR误差和交互误差三部分；实现了两种CEV变体：SpACER（空间感知字符错误率）和基于Jensen-Shannon距离的字符分布方法

Result: CEV与CER、解析质量及页面级OCR质量均具有良好相关性；CEV能有效识别主要误差来源；档案报纸数据集上端到端模型不如传统流水线方法；简单阈值预测误差源F1达0.91

Conclusion: CEV是有价值的指标，可作为解析指标与CER等局部指标之间的桥梁；在档案报纸数据集上，先进端到端模型表现不如传统流水线方法；CEV使用字符级定位可实现最佳分类，简单阈值也可预测主要误差源（F1达0.91）；提供Python库支持文档理解研究
...
### [70] [HaloProbe: Bayesian Detection and Mitigation of Object Hallucinations in Vision-Language Models](https://arxiv.org/abs/2604.06165)
*Reihaneh Zohrabi,Hosein Hasani,Akshita Gupta,Mahdieh Soleymani Baghshah,Anna Rohrbach,Marcus Rohrbach*

Main category: cs.CV

TL;DR: 本研究提出了HaloProbe，一种基于贝叶斯框架的幻觉检测和缓解方法。该方法通过分解外部描述统计和内部解码信号来估计token级别的幻觉概率，解决了现有注意力分析方法因隐藏混淆因素导致的Simpson's悖论问题。实验表明，HaloProbe引导的解码在减少幻觉方面优于现有的基于干预的缓解方法，同时保持了模型效用。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图像描述中产生对象幻觉，现有的基于注意力权重的方法存在隐藏的混淆因素（token位置和对象重复），导致Simpson's paradox，使得基于注意力的分析变得不可靠。

Method: HaloProbe使用贝叶斯框架分解外部描述统计和内部解码信号来估计token级别的幻觉概率。具体采用平衡训练来隔离内部证据，并结合外部特征的学习先验来恢复真实后验概率。最后将HaloProbe作为外部评分信号用于非侵入性缓解。

Result: HaloProbe引导的解码在减少幻觉方面比最先进的基于干预的缓解方法更有效，同时能够保留模型效用。

Conclusion: HaloProbe引导的解码在减少幻觉方面比最先进的基于干预的缓解方法更有效，同时保留了模型的实用性。这种非侵入性的缓解方法避免了修改模型内部结构所带来的效用或流畅性下降问题。
...
### [71] [Action Images: End-to-End Policy Learning via Multiview Video Generation](https://arxiv.org/abs/2604.06168)
*Haoyu Zhen,Zixian Gao,Qiao Sun,Yilin Zhao,Yuncong Yang,Yilun Du,Tsun-Hsuan Wang,Yi-Ling Qiao,Chuang Gan*

Main category: cs.CV

TL;DR: 本文提出Action Images，一种统一的世界行动模型(WAM)，将政策学习表述为多视图视频生成。该方法将7自由度机器人动作翻译成可解释的"动作图像"——一种基于2D像素的多视图动作视频，使视频 backbone 本身能够作为零样本策略，无需单独的政策头或动作模块。


<details>
  <summary>Details</summary>
Motivation: 现有世界行动模型方法通常依赖单独的动作模块，或使用非像素级动作表示，难以充分利用预训练视频模型的知识，并限制了跨视角和跨环境的迁移能力。

Method: 将7-DoF机器人动作翻译成可解释的动作图像：多视图动作视频，基于2D像素并明确跟踪机械臂运动。这种像素级动作表示使视频 backbone 本身能够作为零样本策略。

Result: 在RLBench和真实世界评估中，该模型实现了最强的零样本成功率，并提高了视频-动作联合生成质量，超越了 prior 的视频空间世界模型。

Conclusion: 可解释的动作图像是政策学习的有前途的路径，同一统一模型支持视频-动作联合生成、条件视频生成和动作标注。
...
### [72] [From Retinal Evidence to Safe Decisions: RETINA-SAFE and ECRT for Hallucination Risk Triage in Medical LLMs](https://arxiv.org/abs/2604.05348)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: 提出了RETINA-SAFE基准和ECRT框架，用于检测医学大模型在糖尿病视网膜病变诊断中的幻觉问题，通过证据对齐、冲突和缺口三种任务评估模型的风险 triage 能力。


<details>
  <summary>Details</summary>
Motivation: 医学大模型在证据不足或冲突时容易产生幻觉，存在安全隐患。现有方法难以有效识别和分类这些风险。

Method: 提出两阶段白盒检测框架ECRT：第一阶段进行安全/非安全风险分类，第二阶段将不安全案例细分为矛盾驱动型和证据缺口型。利用CTX/NOCTX条件下的内部表征和logit变化，配合类别平衡训练实现鲁棒学习。

Result: ECRT在Stage-1风险分类上相比外部不确定性和自一致性基线提升0.15-0.19平衡准确率，比最强监督基线提升0.02-0.07，显著优于单阶段白盒消融实验。

Conclusion: 基于视网膜证据的白盒内部信号是实现可解释医学大模型风险分类的可行路径。
...
### [73] [ETR: Entropy Trend Reward for Efficient Chain-of-Thought Reasoning](https://arxiv.org/abs/2604.05355)
*Xuan Xiong,Huan Liu,Li Gu,Zhixiang Chi,Yue Qiu,Yuanhao Yu,Yang Wang*

Main category: cs.AI

TL;DR: 本文提出熵趋势奖励(ETR)方法,通过鼓励推理过程中不确定性逐步降低同时允许有限的局部探索,来解决思维链(CoT)推理过长的问题。该方法整合到GRPO中,在多个推理模型和基准测试上实现了优越的精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 当前CoT推理方法通常产生过长且低效的推理痕迹。现有方法使用长度惩罚或全局熵减来缩短CoT,隐含假设推理过程中低不确定性是 desirable的。作者发现推理效率实际上由不确定性的轨迹决定,具有主导向下熵趋势的CoT要短得多。

Method: 提出熵趋势奖励(Entropy Trend Reward, ETR)——一种轨迹感知的目标函数,鼓励渐进式不确定性降低,同时允许有限的局部探索。将ETR集成到Group Relative Policy Optimization (GRPO)中。

Result: ETR在多个推理模型和挑战性基准测试上 consistently 实现了优越的精度-效率权衡,将DeepSeek-R1-Distill-7B的准确率提高9.9%的同时,在四个基准测试上将CoT长度减少67%。

Conclusion: 该工作证明了推理效率受不确定性轨迹governed,而非全局不确定性水平。ETR作为一种新的训练目标,成功实现了高效推理,在保持甚至提升准确率的同时显著缩短了推理长度。
...
### [74] [LatentAudit: Real-Time White-Box Faithfulness Monitoring for Retrieval-Augmented Generation with Verifiable Deployment](https://arxiv.org/abs/2604.05358)
*Zhe Yu,Wenpeng Xing,Meng Han*

Main category: cs.AI

TL;DR: 本文提出LatentAudit，一种基于残差流几何的白盒审计方法，用于检测RAG系统中生成答案是否被检索到的证据支持。


<details>
  <summary>Details</summary>
Motivation: RAG系统虽然可以减轻幻觉，但无法完全消除幻觉，因此在推理时需要判断答案是否被检索到的证据真正支持，这是一个实时的、可验证的faithfulness监控问题。

Method: LatentAudit从开源生成器中提取中后期残差流激活，测量它们到证据表示的Mahalanobis距离，形成一个二次规则，无需辅助评判模型，可在生成时运行，并在小规模保留集上进行校准。

Result: 在PubMedQA上使用Llama-3-8B达到0.942 AUROC，额外开销仅0.77ms。跨三个QA基准和五个模型家族（Llama-2/3、Qwen-2.5/3、Mistral）保持稳定。在四向压力测试（矛盾、检索失败、部分支持噪声）下，PubMedQA达到0.9566-0.9815 AUROC，HotpotQA达到0.9142-0.9315 AUROC。在16位定点精度下保留99.8%的FP16 AUROC，支持Groth16公开验证。

Conclusion: 残差流几何可作为实时RAGfaithfulness监控的实用基础，支持可选的可验证部署。
...
### [75] [TFRBench: A Reasoning Benchmark for Evaluating Forecasting Systems](https://arxiv.org/abs/2604.05364)
...
TL;DR: TFRBench是首个用于评估预测系统推理能力的基准测试。与传统仅评估数值准确性的方法不同，它提供了评估预测系统 reasoning 能力的协议，包括跨通道依赖性、趋势和外部事件的分析。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测仅通过数值准确性来评估，将基础模型视为"黑箱"，缺乏对模型推理过程的解释性评估。

Method: 提出了一种系统的多智能体框架，利用迭代验证循环来合成数值基础的推理轨迹。

Result: 在10个数据集、5个领域的评估中，使用生成的推理轨迹提示LLM可将预测准确率从约40.2%显著提升至56.6%；而现成LLM在推理和数值预测方面均表现不佳。

Conclusion: TFRBench为时间序列预测建立了基于解释性推理评估的新标准。
...
### [76] [LLM-as-Judge for Semantic Judging of Powerline Segmentation in UAV Inspection](https://arxiv.org/abs/2604.05371)
*Akram Hossain,Rabab Abdelfattah,Xiaofeng Wang,Kareem Abdelfatah*

Main category: cs.AI

TL;DR: 研究使用大型语言模型(LLM)作为语义评判器，评估无人机搭载模型在电力线巡检中的分割结果可靠性。


<details>
  <summary>Details</summary>
Motivation: 无人机部署的轻量级分割模型在真实环境（特别是恶劣天气条件）下性能会不可预测地下降，带来安全隐患，需要一种可靠的方法来评估分割质量。

Method: 设计了两个评估协议：1）重复性评估：使用相同输入和固定提示重复查询LLM，测量其质量评分和置信度估计的稳定性；2）感知敏感性评估：引入受控的视觉退化（雾、雨、雪、阴影、太阳眩光），分析LLM输出对分割质量逐步下降的响应。

Result: LLM在相同条件下产生高度一致的分类型判断，且随着视觉可靠性下降表现出适当的置信度下降。此外，评判器仍能响应缺失或误识别的电力线等感知线索。

Conclusion: 在精心约束下，LLM可以作为安全关键的航空巡检任务中监控分割质量的可靠语义评判器。
...
### [77] [Towards Effective In-context Cross-domain Knowledge Transfer via Domain-invariant-neurons-based Retrieval](https://arxiv.org/abs/2604.05383)
*Jianzhi Yan,Zhiming Li,Le Liu,Zike Yuan,Shiwei Chen,Youcheng Pan,Buzhou Tang,Yang Xiang,Danny Dongning Sun*

Main category: cs.AI

TL;DR: 本文提出DIN-Retrieval方法，利用跨域示例和域不变神经元来提升LLM在专业领域（如数学推理、形式逻辑）的逻辑推理性能，解决专家稀缺领域演示示例获取困难的问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM推理提升依赖领域内专家演示，但在专业数学推理、形式逻辑、法律分析等专家稀缺的领域应用受限。需要探索跨域示例来提升LLM推理能力。

Method: 提出DIN-Retrieval方法：1)总结跨域通用的隐藏表示；2)使用DIN向量在推理阶段检索结构兼容的跨域演示样本用于上下文学习。

Result: 在数学和逻辑推理的多个迁移设置中，平均提升1.8超越最先进方法。

Conclusion: DIN-Retrieval通过域不变神经元实现跨域示例检索，在数学和逻辑推理任务上平均提升1.8，证明了跨域示例可有效提升LLM推理性能。
...
### [78] [Neural Assistive Impulses: Synthesizing Exaggerated Motions for Physics-based Characters](https://arxiv.org/abs/2604.05394)
*Zhiquan Wang,Bedrich Benes*

Main category: cs.AI

TL;DR: 本文提出Assistive Impulse Neural Control框架，将外部辅助从力空间转换到冲量空间，解决了物理-based角色动画中无法重现夸张、风格化动作的问题。


<details>
  <summary>Details</summary>
Motivation: 当前数据驱动的深度强化学习方法在合成复杂技能方面表现良好，但在重现夸张、风格化动作（如瞬间冲刺或空中轨迹改变）方面存在困难。这些动作违反标准物理定律，而将角色建模为欠驱动浮基系统，内部关节力矩和动量守恒严格约束运动，直接施加外部力会导致训练不稳定，产生稀疏的高 magnitude 力尖峰，阻碍策略收敛。

Method: 提出Assistive Impulse Neural Control框架，将外部辅助从力空间重新表述为冲量空间以确保数值稳定性。将辅助信号分解为：1) 来自逆动力学的高频解析分量；2) 由混合神经策略控制的低频残差校正学习部分。

Result: 该方法能够鲁棒跟踪高度敏捷、动态不可行的机动动作，这些动作在之前对于物理-based方法来说是无法实现的。

Conclusion: 通过在冲量空间而非力空间中处理外部辅助，本文成功解决了物理-based角色动画中风格化动作合成的核心问题，为动画中夸张动作的物理仿真开辟了新途径。
...
### [79] [Reason Analogically via Cross-domain Prior Knowledge: An Empirical Study of Cross-domain Knowledge Transfer for In-Context Learning](https://arxiv.org/abs/2604.05396)
*Le Liu,Zhiming Li,Jianzhi Yan,Zike Yuan,Shiwei Chen,Youcheng Pan,Buzhou Tang,Qingcai Chen,Yang Xiang,Danny Dongning Sun*

Main category: cs.AI

TL;DR: 本文研究跨域情境学习(ICL)中，是否可以利用源域的演示样本来辅助目标域的推理任务。研究通过综合实证实验验证了跨域知识转移的可行性，并发现存在"示例吸收阈值"，超过该阈值后正向转移更容易发生。


<details>
  <summary>Details</summary>
Motivation: 现有情境学习依赖于领域内的专家演示，当专家标注稀缺时其适用性受限。本文假设不同领域可能共享底层推理结构，使得源域演示能够在语义不匹配的情况下改善目标域推理。

Method: 对不同检索方法进行综合实证研究，验证跨域情境学习设置下的跨域知识转移可行性。

Result: 研究证实了跨域ICL中存在条件性正向转移。发现了明确的示例吸收阈值：超过该阈值后，正向转移更可能发生，且额外演示能带来更大收益。进一步分析表明这些收益源于跨域示例对推理结构的修复，而非语义线索。

Conclusion: 验证了利用跨域知识转移改善跨域ICL性能的可行性，为社区探索设计更有效的跨域检索方法提供了参考方向。
...
### [80] [Multi-Agent Pathfinding with Non-Unit Integer Edge Costs via Enhanced Conflict-Based Search and Graph Discretization](https://arxiv.org/abs/2604.05416)
*Hongkai Fan,Qinjing Xie,Bo Ouyang,Yaonan Wang,Zhi Yan,Jiawen He,Zheng Fang*

Main category: cs.AI

TL;DR: 本文提出MAPFZ——一种在非单位整数代价图上的多智能体路径规划变体，结合CBS-NIC（增强的基于冲突的搜索）算法和BOGD（贝叶斯优化图设计）离散化方法，在保持有限状态空间的同时提高了真实性和求解效率。


<details>
  <summary>Details</summary>
Motivation: 传统MAPF方法假设单位边代价和单时间步动作，限制了其在现实场景中的应用；MAPFR虽然处理非单位代价和连续时间动作，但其几何碰撞模型导致状态空间无界，影响求解器效率。

Method: 1) 提出MAPFZ变体：在非单位整数代价图上保持有限状态空间；2) 开发CBS-NIC：增强的基于冲突的搜索框架，包含基于时间间隔的冲突检测和改进的安全区间路径规划(SIPP)；3) 提出BOGD：用于非单位边代价的离散化方法，具有次线性遗憾 bound。

Result: 实验表明，该方法在多样化的基准测试场景中，在运行时间和成功率方面均优于现有最先进方法。

Conclusion: 通过MAPFZ变体和CBS-NIC算法，有效解决了非单位代价下的多智能体路径规划问题，BOGD离散化方法在效率和准确性之间取得了良好平衡。
...
### [81] [Automated Auditing of Hospital Discharge Summaries for Care Transitions](https://arxiv.org/abs/2604.05435)
*Akshat Dasula,Prasanna Desikan,Jaideep Srivastava*

Main category: cs.AI

TL;DR: 研究提出使用本地部署的LLM自动审核出院摘要，通过DISCHARGED框架将转诊护理要求转化为结构化检查清单，在MIMIC-IV数据库上验证了可扩展自动临床审核的可行性。


<details>
  <summary>Details</summary>
Motivation: 不完整或不一致的出院文档是护理碎片化和可避免的再入院的主要驱动因素。尽管出院摘要对患者安全至关重要，但审核工作主要依赖人工审查，难以规模化实施。

Method: 提出了一种使用本地部署的大型语言模型（LLM）进行大规模自动审核出院摘要的框架。该方法将核心的转诊护理要求（如随访指示、用药历史和变更、患者信息和临床过程等）操作化为基于DISCHARGED框架的结构化验证检查清单问题。使用MIMIC-IV数据库的成人住院患者摘要，利用隐私保护型LLM识别关键文档元素的存在、缺失或模糊性。

Result: 展示了可扩展的自动临床审核的可行性，为电子健康记录文档的系统性质量改进提供了基础。

Conclusion: 该研究证明了利用LLM进行自动化出院文档审核的可行性，为提高 EHR 文档质量和患者安全提供了新的技术方案。
...
### [82] [Adaptive Serverless Resource Management via Slot-Survival Prediction and Event-Driven Lifecycle Control](https://arxiv.org/abs/2604.05465)
*Zeyu Wang,Cuiqianhe Du,Renyue Zhang,Kejian Tong,Qi He,Qiyuan Tian*

Main category: cs.AI

TL;DR: 本文提出了一种自适应工程框架，通过事件驱动架构和概率建模优化serverless性能，采用双策略机制动态管理资源生命周期，在多云环境中实现了冷启动降低51.2%和成本效率提升近2倍的效果。


<details>
  <summary>Details</summary>
Motivation: Serverless计算虽然消除了基础设施管理开销，但带来了显著的冷启动延迟和资源利用率问题。传统静态资源分配在可变工作负载下效率低下，导致性能下降或成本过高。

Method: 提出了基于事件驱动架构和概率建模的自适应工程框架，采用双策略机制：1) 动态调整空闲持续时间；2) 基于槽位存活预测的智能请求等待策略。结合滑动窗口聚合和异步处理技术，实现资源生命周期的主动管理。

Result: 实验结果表明，该方法在多云环境下相比基线方法冷启动降低51.2%，成本效率提升近2倍。

Conclusion: 该框架通过预测性的资源生命周期管理，有效解决了serverless架构中的冷启动问题和资源利用率低下的问题，为多云环境下的serverless性能优化提供了新的工程思路。
...
### [83] [OntoTKGE: Ontology-Enhanced Temporal Knowledge Graph Extrapolation](https://arxiv.org/abs/2604.05468)
*Dongying Lin,Yinan Liu,Shengwei tang,Bin Wang,Xiaochun Yang*

Main category: cs.AI

TL;DR: 本文提出OntoTKGE框架，利用本体知识图谱中的层次化概念和概念-实体连接关系来指导时序知识图谱外推任务，通过有效整合本体知识和时序知识来增强实体嵌入，缓解稀疏交互实体的预测难题。


<details>
  <summary>Details</summary>
Motivation: 现有时序知识图谱外推模型面临的关键挑战是如何处理历史交互稀疏的实体。本体知识能够使这些实体从具有相同概念的实体继承行为模式，从而缓解稀疏性问题，但这一重要信息在先前的研究中被忽视了。

Method: 提出OntoTKGE框架，包含编码器-解码器结构，从本体视图知识图谱（建模抽象概念间的层次关系以及概念与实体的连接）中提取本体知识，通过有效整合本体知识与时序知识来指导时序知识图谱外推模型的学习过程，从而增强实体嵌入。该框架具有足够的灵活性，可以适配多种时序知识图谱外推模型。

Result: 在四个数据集上的广泛实验表明，OntoTKGE不仅能显著提升多种时序知识图谱外推模型的性能，还超越了众多当前最优的基线方法。

Conclusion: OntoTKGE框架通过引入本体知识有效解决了稀疏实体预测的难题，其设计具有良好的通用性和适应性，可为各类时序知识图谱外推模型提供性能提升，具有重要的应用价值。
...
### [84] [Can We Trust a Black-box LLM? LLM Untrustworthy Boundary Detection via Bias-Diffusion and Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.05483)
*Xiaotian Zhou,Di Tang,Xiaofeng Wang,Xiaozhong Liu*

Main category: cs.AI

TL;DR: 本文提出GMRL-BD算法，通过结合维基百科知识图谱和多智能体强化学习，在黑盒访问场景下识别LLM可能产生偏见的_topic边界，并通过有限查询实现高效检测，同时发布了包含Llama2、Vicuna等主流LLM的偏见主题数据集。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)在多种主题的问题回答中表现出色，但有时会产生偏见、意识形态化或错误的回答，这限制了其在实际应用中的可信度。由于缺乏对LLM可信度边界的清晰理解，急需一种方法来识别哪些主题的回答不可信任。

Method: 提出名为GMRL-BD的创新算法，基于维基百科构建通用知识图谱(KG)，在仅需黑盒访问LLM且满足特定查询约束的条件下，利用多智能体强化学习 Agents 高效识别LLM可能产生偏见的_topic边界（知识图谱中的特定节点）。

Result: 实验表明该算法具有较高效率，只需对LLM进行有限次数的查询即可检测出不可信的边界。此外，研究团队发布了包含Llama2、Vicuna、Qwen2等6种主流LLM的新数据集，并标注了各模型可能产生偏见的_topic标签。

Conclusion: GMRL-BD算法能够有效识别LLM的不可信_topic边界，在有限查询预算下实现高效检测，为评估LLM的可信度和安全应用提供了有价值的方法和数据支持。
...
### [85] [Thinking Diffusion: Penalize and Guide Visual-Grounded Reasoning in Diffusion Multimodal Language Models](https://arxiv.org/abs/2604.05497)
*Keuntae Kim,Mingyu Kang,Yong Suk Choi*

Main category: cs.AI

TL;DR: 扩散多模态大语言模型(dMLLMs)在结合思维链推理时存在两个关键问题：1) 模型在很早的时间步就生成最终答案，导致推理不充分；2) 初始时间步对视觉输入依赖不足。提出位置步骤惩罚(PSP)和视觉推理引导(VRG)两个方法来改善这些问题。


<details>
  <summary>Details</summary>
Motivation: dMLLMs有望保留LLM的推理能力同时实现并行生成加速，但与CoT结合时出现推理退化问题，需要解决模型过早生成答案和视觉信息利用不足的情况。

Method: 1. PSP (Position and Step Penalty): 惩罚早期时间步的后续位置token，延迟过早答案生成；2. VRG (Visual Reasoning Guidance): 借鉴无分类器引导方法，放大视觉 grounding 信号。

Result: 在多种dMLLMs上实验表明，方法可提升最高7.5%准确率，同时相比使用4倍扩散步骤的推理实现超过3倍加速。

Conclusion: PSP和VRG能有效解决dMLLMs中过早生成答案和视觉信息利用不足的问题，实现更好的推理性能与速度平衡。
...
### [86] [OmniDiagram: Advancing Unified Diagram Code Generation via Visual Interrogation Reward](https://arxiv.org/abs/2604.05514)
*Haoyue Yang,Xuanle Zhao,Xuexin Liu,Feibang Jiang,Yao Zhu*

Main category: cs.AI

TL;DR: 论文提出OmniDiagram，一个统一的图表代码生成框架，通过Viva视觉反馈策略和大规模数据集M3²Diagram实现图表代码生成的新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有可编程图表生成研究局限于狭窄的任务定义和语言支持，难以应用于多样化的图表类型。

Method: 提出统一框架OmniDiagram整合多种图表代码语言和任务定义；引入Viva视觉反馈策略，通过生成式方法主动生成视觉 inquiry 来检查图表视觉保真度并提供细粒度反馈，结合SFT和Viva-based RL进行训练。

Result: 构建了首个大规模图表代码生成数据集M3²Diagram，包含超过19.6万高质量实例；OmniDiagram在图表代码生成基准上建立新的SOTA。

Conclusion: 通过SFT和Viva-based RL的组合，OmniDiagram有效解决了图表代码生成中视觉保真度对齐的挑战，无需人工标注的ground truth代码即可实现自进化训练。
...
### [87] [UniCreative: Unifying Long-form Logic and Short-form Sparkle via Reference-Free Reinforcement Learning](https://arxiv.org/abs/2604.05517)
...
TL;DR: 提出了UniCreative框架，通过自适应约束感知奖励模型AC-GenRM和策略优化算法ACPO，在无需监督微调和真实标签的情况下对齐模型与人类偏好，并展现出自发区分长/短文本任务能力的元认知能力。


<details>
  <summary>Details</summary>
Motivation: 创意写作中存在长文本需要宏观规划与短文本需要自发表达的根本性张力；现有对齐范式依赖静态奖励信号和高成本标注数据，难以扩展。

Method: 提出AC-GenRM自适应约束感知奖励模型，动态合成查询特定标准提供细粒度偏好判断；在此基础上提出ACPO策略优化算法，无需监督微调和真实参考即可对齐模型。

Result: AC-GenRM与专家评估高度一致；ACPO在多样化写作任务中显著提升性能；模型展现出元认知能力——自发区分需要严谨规划的任务和适合直接生成的任务。

Conclusion: 直接对齐方法有效，模型能自主识别不同写作任务类型，展现元认知能力，为创意写作对齐提供了新思路。
...
### [88] [Inventory of the 12 007 Low-Dimensional Pseudo-Boolean Landscapes Invariant to Rank, Translation, and Rotation](https://arxiv.org/abs/2604.05530)
*Arnaud Liefooghe,Sébastien Verel*

Main category: cs.AI

TL;DR: 研究引入了排名景观不变性的更强概念，对伪布尔函数的一维、二维和三维情况进行了穷尽的景观类清单编制，发现12,007个不变景观类，非注入函数产生更多类，为景观难度理解和基准设计提供了资源。


<details>
  <summary>Details</summary>
Motivation: 现有随机优化算法仅依赖于解决方案的相对排序（排名不变性），但作者引入了更强的概念——排名景观不变性，它考虑问题的排名、邻域结构和对称性的综合等价性。这促使研究关注排名景观而非单个函数。

Method: 对伪布尔函数的一维、二维和三维情况进行了穷尽的不变景观类清单编制，包括非注入情况。通过分析排名的相对排序、邻域结构和对称性（平移和旋转）来定义等价性。

Result: 共发现12,007个不变景观类，相比仅考虑排名不变性显著减少。非注入函数比注入函数产生更多的不变景观类。此外，还发现了关于欺骗性、中性和爬山策略性能的复杂拓扑景观属性与算法行为的组合。

Conclusion: 该研究为景观难度和算法性能的理解提供了基础，可用于教学和基准设计，为构建具有受控难度的更大规模问题提供了框架。特别值得注意的是，非注入函数比注入函数产生更多的不变景观类，这表明在设计优化问题时应考虑函数的注入性。
...
### [89] [Experience Transfer for Multimodal LLM Agents in Minecraft Game](https://arxiv.org/abs/2604.05533)
*Chenghao Li,Jun Liu,Songbo Zhang,Huadong Jian,Hao Ni,Lik-Hang Lee,Sung-Ho Bae,Guoqing Wang,Yang Yang,Chaoning Zhang*

Main category: cs.AI

TL;DR: 提出Echo框架，通过将可复用知识分解为五维结构并利用上下文类比学习(ICAL)，使多模态LLM智能体能够从过去交互中提取可操作知识。在Minecraft实验中实现了1.3-1.7倍的加速，并观察到链式解锁现象。


<details>
  <summary>Details</summary>
Motivation: 多模态LLM智能体在复杂游戏环境中需要不断重用过去的经验来高效解决新任务。当前方法通常将记忆视为静态记录的被动存储库，而非主动转移可操作知识。

Method: 提出Echo框架，一个转移导向的记忆系统。将可复用知识分解为五个维度：结构、结构、过程、功能和交互。利用上下文类比学习(ICAL)通过情境示例检索相关经验并使其适应新任务。

Result: 在Minecraft环境中的从零学习设置下，Echo在对象解锁任务上实现了1.3x到1.7x的加速。此外，Echo展现出爆发式链式解锁现象，在获得可转移经验后能在短时间内快速解锁多个相似物品。

Conclusion: 经验转移是提高复杂交互环境中多模态LLM智能体效率和适应性的一个有前景的方向。
...
### [90] [SignalClaw: LLM-Guided Evolutionary Synthesis of Interpretable Traffic Signal Control Skills](https://arxiv.org/abs/2604.05535)
*Da Lei,Feng Xiao,Lu Li,Yuzhan Liu*

Main category: cs.AI

TL;DR: SIGNALCLAW是一种利用大语言模型(LLM)作为进化技能生成器,为自适应交通信号控制合成可解释控制技能的框架。该框架通过模拟器的指标反馈引导LLM改进技能,并引入事件驱动的组合进化机制。


<details>
  <summary>Details</summary>
Motivation: 当前交通信号控制面临两难选择:强化学习方法产生不透明的黑箱策略,而程序合成依赖于限制性的领域特定语言。实际部署需要既有效又可解释的策略,以便交通工程师理解和修改。

Method: 提出SIGNALCLAW框架,核心创新包括:(1)使用LLM作为进化技能生成器,每个技能包含理论依据、选择指导和可执行代码;(2)将仿真指标(队列百分位、延迟趋势、停滞情况)转化为自然语言反馈指导改进;(3)引入事件驱动组合进化:事件检测器识别紧急车辆、公交优先、事故和拥堵,优先调度器选择专门技能;(4)技能独立进化,优先链支持运行时组合无需重训练。

Result: 在SUMO场景评估中:(1)常规场景平均延迟7.8-9.2秒,最佳方法的3-10%范围内;(2)事件场景中紧急延迟11.2-18.5秒(MaxPressure为42.3-72.3秒,DQN为78.5-95.3秒);(3)公交人员延迟9.8-11.5秒(MaxPressure为38.7-45.2秒);(4)混合事件中调度器有效组合技能,保持稳定延迟。进化技能从简单线性规则发展为带多特征交互的条件策略,同时保持完全可解释性。

Conclusion: SIGNALCLAW成功实现了有效且可解释的交通信号控制,通过LLM驱动的进化策略合成和事件驱动的组合机制,在各种场景下达到或接近最佳性能,同时产生的技能完全可解释且可直接修改,为交通工程实践提供了实用工具。
...
### [91] [A canonical generalization of OBDD](https://arxiv.org/abs/2604.05537)
*Florent Capelli,YooJung Choi,Stefan Mengel,Martín Muñoz,Guy Van den Broeck*

Main category: cs.AI

TL;DR: 本文提出了树决策图(TDD)作为布尔函数的表示模型，它是OBDD的泛化，可以看作尊重vtree T的结构化d-DNNF的受限形式。TDD继承了OBDD的可计算性质（如模型计数、枚举、限定、apply操作），同时更加紧凑。特别地，树宽为k的CNF公式可以用FPT大小的TDD表示，而这对OBDD是不可能的。本文还研究了通过自底向上编译将CNF公式编译为确定性TDD的复杂度，并将其与Bova和Szeider引入的因子宽度概念联系起来。


<details>
  <summary>Details</summary>
Motivation: OBDD是布尔函数的重要表示模型，但在表示某些CNF公式时存在succinctness限制。本文旨在提出一个既保留OBDD tractability性质又比OBDD更紧凑的模型，以解决OBDD无法高效表示树宽较高的CNF公式的问题。

Method: 1. 将TDD定义为尊重vtree T的结构化d-DNNF的受限形式；2. 证明TDD具有与OBDD相同的可计算性质（model counting、enumeration、conditioning、apply）；3. 证明树宽为k的CNF公式可用FPT大小的TDD表示；4. 研究自底向上编译方法的复杂度并与因子宽度建立联系。

Result: TDD继承了OBDD的所有tractability性质，同时比OBDD更加succinct。关键结果是：树宽k的CNF公式可表示为FPT大小的TDD，这打破了OBDD的已知限制。自底向上编译的复杂度与因子宽度密切相关。

Conclusion: TDD是一个有价值的布尔函数表示模型，它成功地在保持OBDD可计算性质的同时实现了更好的succinctness，为处理高树宽CNF公式提供了新的思路。
...
### [92] [Label Effects: Shared Heuristic Reliance in Trust Assessment by Humans and LLM-as-a-Judge](https://arxiv.org/abs/2604.05593)
*Xin Sun,Di Wu,Sijing Qin,Isao Echizen,Abdallah El Ali,Saku Sugawara*

Main category: cs.AI

TL;DR: 研究挑战了LLM作为自动评估器的可靠性,揭示了LLM存在"来源标签偏见"——对人类撰写的标签信任度高于AI生成的标签。


<details>
  <summary>Details</summary>
Motivation: 随着LLM被广泛用作自动评估器(LLM-as-a-Judge),需要验证其评估结果的可靠性和公正性。之前研究较少关注LLM评估器是否会受到信息来源标签的偏见影响。

Method: 采用反事实设计,将相同内容分别标记为"人类撰写"和"AI生成",比较人类和LLM评估器的判断差异;同时使用眼动追踪和LLM内部状态分析(注意力分布、logits不确定性)进行验证。

Result: 1)人类和LLM评估器都对人类撰写的标签信任度更高;2)眼动数据显示人类依赖来源标签作为判断启发式线索;3)LLM在标签区域的注意力密度高于内容区域,且"人类"标签下更明显,与人类 gaze模式一致;4)AI标签下的决策不确定性高于人类标签。

Conclusion: 来源标签是人和LLM评估器共同的显著性启发式线索,这对标签敏感的LLM-as-a-Judge评估提出有效性担忧。模型与人类偏好对齐可能将人类的启发式依赖传播到模型中,需要去偏见评估和对齐。
...
### [93] [Beyond Behavior: Why AI Evaluation Needs a Cognitive Revolution](https://arxiv.org/abs/2604.05631)
*Amir Konigsberg*

Main category: cs.AI

TL;DR: 本文 arguing that Turing's behavioral test (the Turing Test) created an epistemological commitment in AI that limited the field by only considering behavioral outputs as evidence for intelligence, similar to how behaviorism limited psychology. The paper calls for a "post-behaviorist" revolution in AI that would allow researchers to ask questions about internal processes and mechanisms, not just outputs.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by a concern that AI research has been constrained for 70 years by Turing's behavioral epistemology - this commitment to only behavioral evidence prevents the field from distinguishing between systems that produce identical outputs through different computational processes. They see a parallel to psychology's behaviorist era which was overcome by the cognitive revolution.

Method: The paper uses philosophical analysis and historical comparison - tracing how Turing's test became embedded in AI's evaluative infrastructure, drawing structural parallels to the behaviorist-to-cognitivist transition in psychology, and articulating what questions a post-behaviorist AI epistemology would make askable.

Result: The result is a philosophical argument establishing that: (1) Turing's move was an epistemological commitment rather than just a pragmatic simplification; (2) this commitment has constrained AI research by rendering unaskable questions about process and mechanism; (3) AI needs an epistemological transition similar to psychology's cognitive revolution; (4) articulation of specific questions this new epistemology would enable the field to ask.

Conclusion: The conclusion proposes that AI requires a post-behaviorist epistemology that doesn't abandon behavioral evidence but recognizes it alone is insufficient for intelligence attribution. This would allow the field to distinguish between systems achieving identical outputs through fundamentally different computational processes - a distinction essential to intelligence but currently impossible under pure behavioral evaluation.
...
### [94] [PECKER: A Precisely Efficient Critical Knowledge Erasure Recipe For Machine Unlearning in Diffusion Models](https://arxiv.org/abs/2604.05634)
*Zhiyong Ma,Zhitao Deng,Huan Tang,Jialin Chen,Zhijun Zheng,Zhengping Li,Qingyuan Chuai*

Main category: cs.AI

TL;DR: 本文提出PECKER，一种高效的机器遗忘(MU)方法，通过引入显著性掩码在蒸馏框架中优先更新对遗忘目标数据贡献最大的参数，从而减少不必要的梯度计算，缩短训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法虽然有效，但训练时间长且计算开销大。分析表明根本原因在于梯度更新方向不当，导致训练效率降低和收敛不稳定。

Method: 在蒸馏框架中引入显著性掩码，优先更新对目标数据遗忘贡献最大的参数，减少不必要的梯度计算。

Result: 在CIFAR-10和STL-10数据集上实现了更短的类遗忘和概念遗忘训练时间，同时保持或优于现有方法的遗忘效果，与真实图像分布紧密对齐。

Conclusion: PECKER通过显著性掩码策略有效解决了机器遗忘的计算效率问题，在保持遗忘效果的同时显著缩短了训练时间。
...
### [95] [CuraLight: Debate-Guided Data Curation for LLM-Centered Traffic Signal Control](https://arxiv.org/abs/2604.05663)
*Qing Guo,Xinhang Li,Junyu Chen,Zheng Guo,Shengzhe Xu,Lin Zhang,Lei Li*

Main category: cs.AI

TL;DR: CuraLight是一个LLM中心的交通信号控制框架，利用RL agent辅助探索生成高质量交互数据，并通过多LLM ensemble deliberation系统提供偏好感知监督，实现可扩展可解释的交通信号控制。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习(RL)和大语言模型(LLM)的交通信号控制方法仍存在可解释性有限、交互数据不足、对异构交叉口泛化能力弱等问题。

Method: 提出CuraLight框架，包含：1）RL agent探索交通环境并生成高质量交互轨迹，转换为prompt-response对用于模仿微调；2）多LLM ensemble deliberation系统通过结构化辩论评估候选信号时序动作，提供偏好感知监督信号用于训练。

Result: 在SUMO上基于济南、杭州、亦庄的真实异构路网进行实验，CuraLight相比SOTA基线方法，平均旅行时间降低5.34%，平均排队长度降低5.14%，平均等待时间降低7.02%。

Conclusion: CuraLight通过结合RL辅助探索与基于 deliberation 的数据筛选，实现了大规可扩展且可解释的交通信号控制。实验表明该方法在真实异构城市场景中显著优于现有基线方法。
...
### [96] [QA-MoE: Towards a Continuous Reliability Spectrum with Quality-Aware Mixture of Experts for Robust Multimodal Sentiment Analysis](https://arxiv.org/abs/2604.05704)
*Yitong Zhu,Yuxuan Jiang,Guanxuan Jiang,Bojing Hou,Peng Yuan Zhou,Ge Lin Kan,Yuyang Wang*

Main category: cs.AI

TL;DR: 针对多模态情感分析中模态缺失和质量问题，提出连续可靠性光谱统一框架和QA-MoE方法，通过自监督aleatoric不确定性量化模态可靠性并引导专家路由，实现卓越性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中多模态输入常受动态噪声或模态缺失影响，现有方法将这些不完美情况视为离散情况或假设固定 corruption ratio，难以适应连续变化的可靠性条件。

Method: 提出QA-MoE框架，利用自监督学习的aleatoric不确定性来量化模态可靠性，该机制显式引导专家路由，使模型能够抑制来自不可靠信号的错误传播，同时保留任务相关信息。

Result: QA-MoE在多种退化场景下达到竞争性或最先进的性能，并展现出实用的"一键检查点"特性。

Conclusion: QA-MoE通过质量感知的专家路由机制有效解决了连续变化可靠性条件下的多模态情感分析问题，具有良好的实用价值。
...
### [97] [Can Large Language Models Reinvent Foundational Algorithms?](https://arxiv.org/abs/2604.05716)
*Jian Zhao,Haoren Luo,Yu Wang,Yuhan Cao,Pingyue Sheng,Tianxing He*

Main category: cs.AI

TL;DR: 论文提出Unlearn-and-Reinvent管道测试LLM的基础创新能力，通过移除特定算法知识后测试重新发明能力，发现最强模型可重新发明50-90%的算法，但复杂算法仍具挑战，生成验证器和测试时强化学习是关键。


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型是否具备基础创新能力，这是实现基础创新之前提。测试LLM能否在移除算法知识后重新发明计算机科学基础算法。

Method: 采用基于GRPO的on-policy unlearning方法移除LLM中特定基础算法知识，然后在受控环境中测试模型能否重新发明该算法；使用生成式验证器避免"思维崩溃"现象；引入测试时强化学习策略。

Result: Qwen3-4B-Thinking-2507在无提示下成功重新发明50%算法，提示级别1时达70%，提示级别2时达90%；少数高级提示可提高成功率，但复杂算法即使逐步提示也失败；测试时强化学习使Strassen算法在提示级别2下成功重新发明。

Conclusion: LLMs具有一定的创新思维能力，能够在移除特定算法知识后重新发明50-90%的算法，但复杂算法即使有逐步提示也难以成功；测试时强化学习可帮助解决复杂算法的重新发明问题；生成验证器在维持模型推理能力方面起关键作用。
...
### [98] [Emergent social transmission of model-based representations without inference](https://arxiv.org/abs/2604.05777)
*Silja Keßler,Miriam Bautista-Salinero,Claudio Tennie,Charley M. Wu*

Main category: cs.AI

TL;DR: 研究通过强化学习模拟证明，简单的社会学习线索（不涉及心智理论推断）可以间接传递更高层次的表征，使学习者表征向专家收敛。


<details>
  <summary>Details</summary>
Motivation: 人类如何在有限的认知能力下从他人那里获取丰富而灵活的环境知识？传统观点认为人类依赖计算成本较高的心智理论（如推断他人信念），而文化进化观点强调行为传递可以由简单的社会线索支持。

Method: 使用强化学习模拟，比较在重新配置环境中单独搜索奖励的 naïve agent 与观察专家学习的 learner；learner 不推断心智状态，而是启发式地选择动作或基于观察到的动作提升价值表征。

Result: 这些线索使 learner's 表征向 expert 收敛，基于模型的 learner's 受益最大，表现出更快的学习和更像 expert 的表征。

Conclusion: 文化传递可以源自简单的、非心智理论过程，利用非社会学习机制。
...
### [99] [Vision-Guided Iterative Refinement for Frontend Code Generation](https://arxiv.org/abs/2604.05839)
*Hannah Sansford,Derek H. C. Law,Wei Liu,Abhishek Tripathi,Niresh Agarwal,Gerrit J. J. van den Burg*

Main category: cs.AI

TL;DR: 本文提出了一种完全自动化的批评者-循环框架，使用视觉语言模型作为视觉批评者为生成的代码提供结构化反馈，指导前端网页开发中生成代码的迭代优化。在WebDev Arena数据集的真实用户请求上，该方法在三个优化周期内实现了高达17.8%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码生成中通常依赖多阶段的人工参与优化，这种方法虽然有效但成本高昂，特别是在前端网页开发等领域，因为解决方案质量依赖于渲染的视觉输出。

Method: 提出了一种完全自动化的批评者-循环框架，其中视觉语言模型充当视觉批评者，为渲染的网页提供结构化反馈，以指导生成代码的迭代优化。同时研究使用LoRA进行参数高效微调，探讨是否可以将批评者提供的改进内化到代码生成LLM中。

Result: 在三个优化周期内实现高达17.8%的性能提升；微调可在不显著增加token数量的情况下实现最佳批评者-循环解决方案25%的增益。

Conclusion: 自动化的、基于VLM的前端代码生成批评能够产生比单次LLM推理显著更高质量的解决方案，并突出了迭代优化对网页开发相关复杂视觉输出的重要性。
...
### [100] [Deep Researcher Agent: An Autonomous Framework for 24/7 Deep Learning Experimentation with Zero-Cost Monitoring](https://arxiv.org/abs/2604.05854)
*Xiangyue Zhang*

Main category: cs.AI

TL;DR: Deep Researcher Agent是一个开源框架，使LLM智能体能够全天候自主进行深度学习实验，涵盖假设形成、代码实现、训练执行、结果分析和迭代优化等完整实验生命周期。


<details>
  <summary>Details</summary>
Motivation: 现有AI研究助手仅关注论文写作或代码生成，缺乏覆盖完整实验周期的系统；长运行智能体存在上下文无限增长的问题，且多工具调用导致高token开销。

Method: 提出三个关键创新：(1)零成本监控-仅依赖进程级检查和日志读取，训练期间无LLM API调用；(2)双层固定大小内存-上限约5K字符，防止上下文无限增长；(3)最小工具集Leader-Worker架构-每个worker智能体仅配备3-5个工具。

Result: 在30+天持续部署中，自主完成500+实验循环，跨四个并发研究项目，在一个项目中通过200+自动化实验实现52%的基线指标提升，平均每24小时周期LLM成本仅$0.08。

Conclusion: 该框架实现了低成本、高效率的自主深度学习研究，为LLM智能体在科研实验自动化方面提供了可行方案。
...
### [101] [When Do We Need LLMs? A Diagnostic for Language-Driven Bandits](https://arxiv.org/abs/2604.05859)
*Uljad Berdica,Fernando Acero,Anton Ipsen,Parisa Zehtabi,Michael Cashmore,Manuela Veloso*

Main category: cs.AI

TL;DR: 研究针对含文本和数值信息的上下文多臂bandit问题，提出LLMP-UCB算法，但实验发现轻量级数值bandit在文本嵌入上表现更优且成本更低，嵌入维度可作为探索-利用平衡的杠杆，并提出几何诊断方法指导LLM与轻量级bandit的选择。


<details>
  <summary>Details</summary>
Motivation: 解决在非情节顺序决策问题中，使用LLM进行推理计算成本高且难以获取不确定性估计的问题。

Method: 引入LLMP-UCB算法，通过重复推理从LLM获取不确定性估计；使用轻量级数值bandit在文本嵌入（密集或Matryoshka）上操作；提出基于臂嵌入的几何诊断方法。

Result: 实验表明，轻量级数值bandit在文本嵌入上的准确率与LLM解决方案相当或更高，成本仅为其一小部分；嵌入维度是探索-利用平衡的实用杠杆，可实现成本-性能权衡。

Conclusion: LLMP-UCB算法在成本效益上不如轻量级数值bandit；嵌入维度可作为探索-利用平衡的实用杠杆；提出基于嵌入的几何诊断方法来决定使用LLM驱动推理还是轻量级数值bandit；为金融服务业中具有成本效益、不确定性感知的决策系统提供了原则性部署框架。
...
TL;DR: 一种针对多模态大语言模型的KV缓存混合压缩框架，通过将注意力头分类为静态/动态类型并采用差异化压缩策略，实现最高7.9倍内存降低和1.52倍加速，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型(MLLMs)处理文本、图像和视频的能力提升，KV缓存快速增长。每个视觉输入扩展为数千个token，导致缓存随上下文长度线性增长，占用GPU内存，严重制约推理效率。现有压缩方法仅关注分配策略，忽视了注意力头在压缩需求上的异质性。

Method: 提出HybridKV混合KV缓存压缩框架，包含三个阶段：1)基于文本中心注意力将头分类为静态或动态类型；2)自顶向下分配KV预算；3)静态头采用文本优先剪枝，动态头采用分块检索压缩。

Result: 在11个多模态基准测试上，使用Qwen2.5-VL-7B模型，HybridKV实现KV缓存内存降低最高7.9倍，解码加速1.52倍，性能几乎无损失或有所提升。

Conclusion: HybridKV通过识别注意力头的异质性并采用差异化压缩策略，有效解决了MLLMs中KV缓存膨胀问题，在显著降低内存和提升速度的同时保持模型性能，为多模态推理效率优化提供了新思路。
...
### [103] [Towards Trustworthy Report Generation: A Deep Research Agent with Progressive Confidence Estimation and Calibration](https://arxiv.org/abs/2604.05952)
*Yi Yuan,Xuhong Wang,Shanzhe Lei*

Main category: cs.AI

TL;DR: 针对深度研究智能体生成的报告缺乏可信度评估机制的问题，提出了一种结合渐进式置信度估计与校准的深度研究智能体系统，通过审议式搜索模型、多跳推理和证据锚定技术，为生成内容分配置信度分数，提升报告的可信赖性和透明度。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架基于主观维度，无法捕捉报告质量的关键维度——可信赖性；在开放域研究场景中缺乏真实答案，当前方法无法有效衡量生成内容的认知置信度，导致校准困难，用户容易受到误导或虚假信息的影响。

Method: 采用 Deliberative Search Model（审议式搜索模型），结合深度检索和多跳推理技术，将输出内容锚定在可验证的证据上，同时为每个论断分配置信度分数，并设计了精心策划的工作流程。

Result: 实验结果和案例研究表明，该方法显著提升了可解释性，并显著增强了用户信任。

Conclusion: 所提出的深度研究智能体通过渐进式置信度估计和校准机制，有效提升了研究报告中信息的可信度和透明度，为开放域研究场景中的信息可信度评估提供了新的解决思路。
...
### [104] [Beyond Compromise: Pareto-Lenient Consensus for Efficient Multi-Preference LLM Alignment](https://arxiv.org/abs/2604.05965)
*Renxuan Tan,Rongpeng Li,Zhifeng Zhao,Honggang Zhang*

Main category: cs.AI

TL;DR: 提出PLC框架，通过引入游戏理论谈判机制和动态宽松梯度修正，使多目标偏好对齐能够跳出局部次优，探索全局Pareto前沿。


<details>
  <summary>Details</summary>
Motivation: 当代多目标偏好对齐(MPA)方法依赖静态线性标量化或刚性梯度投影来权衡冲突目标，但这种严格避免冲突或同步下降的方式容易导致优化轨迹过早收敛到局部最优解，而非全局Pareto前沿。

Method: 提出Pareto-Lenient Consensus (PLC)框架，将对齐问题重新表述为动态谈判过程。核心创新是共识驱动的宽松梯度 rectification机制：动态容忍局部性能下降，只要存在足够的优势联盟剩余，赋予优化轨迹跳出局部次优均衡的能力，从而探索更优的Pareto前沿。

Result: 理论分析验证PLC能够促进僵局打破并渐进收敛到Pareto共识均衡。实验表明PLC在固定偏好对齐和全局Pareto前沿质量上均超越基线方法。

Conclusion: 本研究揭示了谈判驱动的对齐方法作为MPA的有前景方向的潜力，为打破保守妥协、获取更优全局Pareto最优提供新思路。
...
### [105] [Epistemic Blinding: An Inference-Time Protocol for Auditing Prior Contamination in LLM-Assisted Analysis](https://arxiv.org/abs/2604.06013)
*Michael Cuccarese*

Main category: cs.AI

TL;DR: 本文提出了"认知遮蔽"(epistemic blinding)方法，用于解决大型语言模型在生物数据分析中无法区分数据驱动推理与训练记忆的问题。该协议在推理时将实体标识符替换为匿名代码，通过对比遮蔽与未遮蔽的输出来衡量结果中有多少来自提供的数据而非模型的参数知识。


<details>
  <summary>Details</summary>
Motivation: 研究团队发现LLM在药物靶点优先级排序中会悄悄混合数据驱动的推理与训练记忆中的先验知识，且无法从单次输出中判断有多少来自页面上的数据、有多少来自模型的记忆。这导致了审计维度缺失的问题。

Method: 提出认知遮蔽协议：1)在推理时将实体标识符替换为匿名代码后再提示；2)将输出与未遮蔽的对照组进行比较；3)测量输出中有多少来自提供的数据、多少来自模型的参数知识。同时描述了完整的靶点识别系统，包括LLM引导的评分函数进化优化和遮蔽后的agentic推理。

Result: 在四个癌症类型的肿瘤药物靶点优先级排序中，遮蔽改变了16%的top-20预测，但验证靶点回收率相同。在S&P 500股票筛选中，品牌认知偏见改变了30-40%的top-20排名。该协议已作为开源工具和Claude Code技能发布。

Conclusion: 认知遮蔽协议恢复了关键的审计维度，使研究人员能够衡量agent在多大程度上遵循了设计的数据分析流程。该协议并不声称遮蔽分析会产生更好的结果，而是提供了原本缺失的透明度。
...
### [106] [How LLMs Follow Instructions: Skillful Coordination, Not a Universal Mechanism](https://arxiv.org/abs/2604.06015)
*Elisabetta Rocchetti,Alfio Ferrara*

Main category: cs.AI

TL;DR: 研究揭示指令跟随并非依赖单一通用机制，而是通过协调多种语言技能实现。诊断探针实验表明跨任务迁移有限，任务按复杂度分层，因果消融显示稀疏非对称依赖。


<details>
  <summary>Details</summary>
Motivation: 论文旨在探究指令微调后的语言模型如何实现指令跟随：是依赖通用的单一机制，还是通过组合部署多种语言技能？这一问题的理解对于改进指令微调和模型泛化能力至关重要。

Method: 在三个指令微调模型上针对九个不同任务进行诊断探针分析，包括：跨任务泛化能力测试、任务特异性与通用性对比、因果消融实验、层级分析以及时序分析。

Result: 1）通用探针表现持续低于任务特定专用探针，表明表征共享有限；2）跨任务迁移能力弱且按技能相似性聚类；3）因果消融揭示稀疏非对称依赖而非共享表征；4）任务按复杂度在模型各层中分层，结构约束较早出现，语义任务较晚出现；5）时序分析显示约束满足在生成过程中动态监控而非预先生成规划。

Conclusion: 指令跟随应被理解为多种语言技能的协调配合，而非单一抽象约束检查过程的部署。这一发现对理解指令微调机制和模型架构设计具有重要意义。
...
### [107] [Artificial Intelligence and the Structure of Mathematics](https://arxiv.org/abs/2604.06107)
*Maissam Barkeshli,Michael R. Douglas,Michael H. Freedman*

Main category: cs.AI

TL;DR: 探讨AI如何通过理解形式证明的整体结构（超越传统数理逻辑）来推动数学发现，并提出自动化数学发现的评估标准


<details>
  <summary>Details</summary>
Motivation: AI在数学领域的进展带来希望：帮助解决重大开放问题、自主发现新数学概念；现有方法需补充新视角来理解数学形式证明的整体结构

Method: 提出形式化数学结构框架（通用证明和结构超图）；建立自动化数学发现AI模型的标准

Result: 提出数学形式化结构的新视角（通用证明和结构超图）；建立AI自动数学发现的标准框架

Conclusion: AI agents可能帮助揭示数学的本质（发现还是发明），并帮助我们理解Platonic数学世界的疆域
