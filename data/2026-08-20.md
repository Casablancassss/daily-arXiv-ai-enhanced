...
TL;DR: 本文是一篇关于基础模型时代以人为中心智能的综合survey论文，提出了一个涵盖六个互联层次的完整人类上下文分类体系，并系统性地回顾了该领域的方法、数据集、基准和评估指标，同时讨论了未来的开放挑战和研究方向。


<details>
  <summary>Details</summary>
Motivation: 解决基础模型时代以人为中心智能研究分散在不同任务、模态和社区的问题，厘清其内在概念和方法论联系，为该领域提供统一框架和实践参考。

Method: 提出六层分类体系：将人类视为可观察对象（视觉外观和空间几何）、动态行为者（运动动力学和交互建模）、以及情境化智能体（世界模拟和具身智能）；系统综述代表性方法、数据集和评估指标。

Result: 建立了以人为中心智能的统一分类框架，涵盖六层次（视觉外观、空间几何、运动动力学、交互建模、世界模拟、具身智能），整合了相关数据集、基准和评估方法，识别出可扩展、可信、物理接地和可部署的关键挑战。

Conclusion: 为基础模型时代的以人为中心智能研究提供了系统性的框架和实践参考，确立了未来发展方向包括可扩展性、可信度、物理接地和可部署性。
...
TL;DR: 本研究提出分布无关风险控制方法，为冻结分割添加器官特异性召回保证。针对AMOS训练的nnU-Net进行每器官阈值校准，审计向RAOS的迁移，并使用病例级体素假阴性率估算本地重新认证成本。结果显示AMOS控制通过，但迁移后7/12器官超过α=0.10阈值；较小的校准集可能掩盖超限问题。RCPS提供人群均值风险的高概率控制，CRC提供较弱的期望控制。WSR下注界需要25个本地病例重新认证6个一级器官，而HB需要30-40个；CRC需要10-15个但有个体案例尾部更重的问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，如何在模型部署后确保分割质量满足临床要求，特别是在模型迁移到新数据集时如何保证各器官的召回率达到预定标准。现有方法无法提供针对每个器官的具体风险控制保证。

Method: 使用nnU-Net架构，结合AMOS数据集训练，通过每器官阈值校准实现分布无关风险控制。采用三种方法进行比较：RCPS（风险控制预测集）、CRC（一致性风险控制）和WSR（Waudby-Smith-Ramdas）下注界，以及HB（Hoeffding-Bentkus）方法。

Result: 1) AMOS控制通过但迁移后7/12器官超过α=0.10阈值 2) 较小校准集可能产生保守或空洞阈值从而掩盖问题 3) WSR方法用25个本地病例重新认证6个Tier-1器官 4) HB方法需要30-40个病例 5) CRC需要10-15个病例但尾部风险更重 6) 25个病例无法让任何Tier-2器官达到精确度标准

Conclusion: 分布无关风险控制可有效应用于医学分割的器官级质量保证。WSR方法在病例数量和尾部风险之间取得较好平衡。建议使用足够大的校准集以避免掩盖阈值超限问题，并为不同层级器官采用差异化认证策略。
...
TL;DR: LumiTokens提出一种基于潜在场景令牌的3D重光照新范式，通过Scene Token Editor在自注意力机制下联合处理场景令牌和光线令牌，实现无需显式3D表示或物理分解的重光照，并支持渐进式光照编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D重光照方法依赖显式材质分解、扩散模型或两者结合，每次新光照条件需全量重计算。潜在场景表示的进展为重光照开辟了新设计空间。

Method: 提出Scene Token Editor，通过自注意力联合处理场景令牌与光射线令牌(Plucker ray tokens)，输出更新后的令牌解码为多视角一致的重光照图像。所有光照类型统一参数化为Plucker射线令牌。

Result: 实现与现有方法相当或更优的重光照质量，支持渐进式、可组合的光照编辑。

Conclusion: LumiTokens开辟了基于潜在空间令牌变换的3D重光照新范式，无需物理模型或扩散模型即可实现高质量重光照。
...
TL;DR: 本文提出了一种跨不同GelSight Mini传感器单元泛化估计3D力图的方法，解决了触摸传感器制造过程中性能标准化困难的问题。该方法包含两个阶段：领域适应阶段（使用UniT模型将输入触觉图像重建为通用触觉图像）和3D力图估计阶段（使用U-Net网络）。


<details>
  <summary>Details</summary>
Motivation: 由于触摸传感器制造仍主要在研究实验室中手工完成，导致每个生产的传感器单元需要重复进行数据收集和模型训练，难以实现性能标准化。

Method: 采用两阶段方法：1)领域适应阶段：使用UniT模型将输入触觉图像重建为通用触觉图像；2)力估计阶段：使用U-Net网络估计3D力图。

Result: 图像重建阶段SSIM达到0.9338 ± 0.0358，力估计阶段MAE_F达到1.1294 ± 1.5934(N)。

Conclusion: 该方法在不同版本的GelSight Mini传感器上展现出良好的泛化能力，为触摸传感器的标准化和大规模应用提供了可行方案。
...
TL;DR: 本文提出了一种用于细粒度野生动物重识别的单阶段端到端检测与重识别模型，采用DINOv2和MegaDescriptor结合的方法，在潜在空间中进行身份搜索，在潜在查询中增强提示重识别特征。初步实验结果显示mAP为30.584%，低于SOTA两阶段方法的44.89%。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的野生动物重识别方法采用检测和重识别两阶段流水线，文章旨在提出一种更简洁的一阶段端到端模型来解决细粒度野生动物重识别问题。

Method: 1. 采用一阶段端到端架构，在潜在空间中进行身份搜索
2. 使用DINOv2获取稳健的空间几何特征
3. 使用MegaDescriptor进行野生动物重识别
4. 使用提示重识别特征增强潜在查询
5. 检测解码器查询场景潜在空间以建立目标身份的对象边界

Result: mAP达到30.584%，相比SOTA两阶段方法的44.89%仍有差距，但定性结果显示该方法能够有效bounding和识别动物身份。

Conclusion: 一阶段端到端方法在野生动物重识别任务上具有潜力，但目前性能仍落后于两阶段方法，需要进一步优化以提升准确率。
...
TL;DR: 本文提出XRF tile-group localization方法,通过利用采集几何约束将相关的XRFtiles作为一个整体进行光学图像定位,解决了XRF与光学显微镜图像难以对齐的问题。


<details>
  <summary>Details</summary>
Motivation: X射线荧光(XRF)显微镜与光学显微镜的对比度和分辨率不同,导致XRF视野(FOV)在光学图像中的定位困难。现有工作流程将每个XRF tile独立放置,忽略了已有采集元数据记录的tile相对位置信息。

Method: 提出XRF tile-group localization方法,估计整个tile组的光学框架单一放置位置,以采集几何为约束条件,使用组交并比(GroupIoU)进行量化评估。分别使用归一化互相关(NCC)和互信息(MI)作为相似性度量进行比较。

Result: 控制case研究中,独立定位GroupIoU为0.000(失败),组定位达到0.931。NCC与MI结果几乎相同。多尺度case研究中,利用粗XRF调查扫描连接细尺度tile组到光学图像，GroupIoU从0.694提高到0.856。

Conclusion: 使用采集几何作为显式约束来定位相关XRF tiles是有效的方法,能够显著提高定位准确性。
...
TL;DR: 本文提出了一种结合自由运行采集与分析合成软件管道的计算成像方法，用于解决单光子相机在高光子通量条件下的堆叠失真问题，并实现可靠的距离和反射率捕获。


<details>
  <summary>Details</summary>
Motivation: 单光子相机面临两个关键挑战：1) 在高光子通量条件下遭受称为"堆叠"的非线性失真；2) 产生大量原始光子数据，造成严重的数据瓶颈。现有压缩采集技术虽能缓解数据传输挑战，但无法保留足够的光子检测历史信息来进行后处理堆叠校正。

Method: 提出一种结合自由运行捕获与分析合成软件管道的新型计算成像方法，通过硬件仿真和全场景及单像素模拟进行验证。

Result: 该方法能够在广泛的光照条件下可靠地捕获场景距离和反射率。

Conclusion: 该工作将使严重带宽受限的高分辨率SPAD相机能够在现实世界的高通量场景中运行。
...
TL;DR: 本文评估了无训练视觉语言模型在XRF和光学显微镜图像配准中的定位能力，提出提议-验证工作流程解决了低对应情况下跨模态配准难题。


<details>
  <summary>Details</summary>
Motivation: 在相关性X射线荧光（XRF）和光学显微镜中，XRF图像通常只覆盖光学图像的一小部分视野，视场定位困难，特别是当跨模态的外观和结构不同时。

Method: 测试了无约束和元数据约束搜索，比较了VLM与几何控制、经典模板匹配和两种替代无训练方法（DINOv2和multiGradICON）。提出了提议-验证工作流程，使用重复的VLM预测作为候选图像，并使用图像相似度选择最终位置。

Result: 直接VLM提示产生了内容依赖的空间信号，但单独使用时不可靠。经典匹配在跨模态结构保留时最准确，但在低对应集合中失败。提议-验证工作流程在低对应情况下恢复了有用的定位效果。

Conclusion: VLM直接提示产生的空间信号具有内容依赖性，但单独使用时不可靠。当跨模态结构保留时，经典匹配最准确，但在低对应数据集上失败。提议-验证工作流程在低对应情况下恢复了有用的定位效果。
...
TL;DR: FedCoRe是一种联邦跨模态表征补全框架，通过配对样本学习表征/Logit空间校正来恢复缺失模态性能，可在验证支持下部署。


<details>
  <summary>Details</summary>
Motivation: 现有联邦多模态模型假设每个站点都拥有所有模态，但实际医院在EHR、心电图和胸部X光片的访问上存在差异。研究在MIMIC衍生的呼吸恶化任务上模拟FL客户端进行该设置。

Method: FedCoRe学习表征或logit空间的校正，而非生成合成ECG或CXR图像。当客户端观察到部署时可能缺失的模态时，会用带该模态和不带该模态两种方式评估同一示例以获得配对监督。只有拥有这种配对的客户端才能更新补全模块，验证可保留未更改的预测。在训练期间冻结多模态预测器。

Result: 隐藏ECG使AUROC降低约0.085；配对样本FedAvg恢复了0.0415 AUROC，即丢失性能的49.0%。对于CXR，效果感知补全在受控测试中恢复了52.8%的损失。

Conclusion: FedCoRe是一种验证门控的跨模态表征补全框架，在支持的设置中可以恢复缺失模态信号，但仅应在有配对样本和验证证据支持该模态时部署。验证选择的补全是一种任务特定的分类器logit校正，而非真正的ECG恢复。
...
TL;DR: 本文提出了一种可配置的隐私保护MRI处理工作流，通过扩展基于深度学习的脑提取方法，实现可自适应保留解剖结构、交互式选择保留程度以及集成质量控制功能。该工作流在Renku可重现研究环境中实现，使用IXI数据集进行评估，为隐私导向的神经影像研究提供了实用基础。


<details>
  <summary>Details</summary>
Motivation: 结构磁共振成像在神经影像研究和临床实践中广泛应用，但结构MRI图像可能保留面部和颅骨解剖信息，存在隐私泄露风险。现有基于深度学习的脑提取方法通常只产生单一固定输出，缺乏灵活性，无法满足不同应用对隐私保护与解剖结构保留之间平衡的不同需求。

Method: 1）使用SynthStrip进行自动化脑提取；2）通过形态学掩膜扩展生成基于壳层的可配置保留级别；3）开发交互式保留框架，使用户可比较不同保留配置并选择合适输出；4）集成质量控制框架，提供多平面可视化和脑掩膜叠加验证。采用Python实现，集成开源神经影像库，运行于Renku可重现研究环境。

Result: 实验结果表明，该工作流能够实现解剖学上合理的脑提取，并提供可配置的保留输出。系统性的视觉验证支持了这些发现。

Conclusion: 主要贡献在于提供了一个模块化、可重现的MRI预处理框架，增强了基于深度学习的脑提取功能，支持可配置的解剖结构保留、交互式用户引导处理和集成质量控制。该工作流为隐私导向的神经影像研究和协作医学图像分析提供了实用基础。
...
TL;DR: 提出Affordance Sheet文档框架，解决 affordance 预测方法评估困难的问题，促进可重复的基准测试和可靠的性能评估。


<details>
  <summary>Details</summary>
Motivation: 当前 affordance 预测方法难以评估和比较，原因包括：问题形式异构、数据集标注不一致、实验协议报告不完整、部署条件信息有限等，这些问题阻碍了公平的基准测试和性能比较。

Method: 提出 Affordance Sheet 文档规范，详细记录任务形式、输入模态、模型架构、训练信息、数据集和实验协议。

Result: 该框架支持可重复的基准测试，并可对真实场景中的 affordance 模型进行可靠评估，包括泛化到新条件和人类安全等方面。

Conclusion: Affordance Sheet 能够促进透明性，支持可重现的基准测试和可靠的性能评估，有助于推动 affordance 预测领域的发展。
...
TL;DR: 本文提出了一种名为TTA的视觉语言模型测试时自适应方法，通过Wasserstein最优传输理论将推理和适应目标统一起来，解决分布偏移问题。


<details>
  <summary>Details</summary>
Motivation: 现有VLM测试时自适应方法依赖于原始嵌入相似度预测的伪标签，在分布偏移下不可靠且会误导适应过程；且现有方法采用粗粒度的代理目标，未能显式建模样本级别的跨模态关系，导致推理与适应目标脱节。

Method: 1. 将零样本图像分类问题表述为跨模态对齐问题，通过Wasserstein OT公式化以获得样本级鲁棒伪标签；2. 采用软标签InfoNCE损失结合OT诱导的伪标签进行VLM适应，通过对比学习显式建模个体图像-文本对的细粒度关系；3. 理论证明InfoNCE损失可重构为Wasserstein OT公式，实现推理与适应目标的统一。

Result: 在广泛实验中，该方法优于最佳性能方法高达7%，同时具有最优的效率。

Conclusion: 通过Wasserstein OT理论统一VLM的推理和适应目标，可以有效解决分布偏移问题，显著提升零样本分类性能，且兼顾准确性和效率。
...
TL;DR: 本文提出Test-Time Self-Distillation (TTSD)框架，解决大视频语言模型在部分模态缺失情况下的多模态情绪识别问题。该方法通过冻结教师模型指导低秩学生模型进行自蒸馏，并引入Fisher-Annchored Restoration (FAR)机制监控Fisher信息稳定性以检测模型漂移，在MELD、DFEW、BAH数据集上0-50%模态缺失场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实场景中模态可能缺失或噪声，且测试时适应(TTA)方法如熵最小化或困惑度降低无法迁移到自回归LVLMs，检索增强生成在弱模态下性能下降，长期适应会导致模型漂移。

Method: 提出TTSD框架：1)冻结教师模型(完整模态训练)指导低秩学生模型进行自蒸馏，仅更新少量参数；2)FAR机制监控Fisher信息稳定性，检测收敛与漂移，在识别到分布偏移时将学生模型恢复至教师锚点。

Result: 在MELD、DFEW、BAH数据集上0%-50%模态缺失场景中，TTSD-FAR一致优于基于熵的适应、RAG和基于困惑度的生成方法。基线方法在长期适应中逐渐退化，而TTSD-FAR保持稳定。

Conclusion: 统一的自适应-恢复设计能够有效适应任意模态缺失模式，并在持续适应过程中保持有效性，为实际部署中处理模态不确定性提供了可行方案。
...
TL;DR: Depth Anything V4 (DAV4)是一个从单目视频进行动态4D场景重建的框架，核心贡献是将黎曼流匹配(RFM)应用于4D高斯溅射参数，在非欧几里得流形上定义概率路径。


<details>
  <summary>Details</summary>
Motivation: 动态4D场景重建需要在非欧几里得流形（尺度、旋转、不透明度）上处理参数，并确保所有中间状态都是有效的。

Method: 将RFM应用于4D高斯溅射参数，在非欧几里得流形上直接定义概率路径；通过对照实验分离RFM的贡献；使用确定性MLP基线进行对比。

Result: RFM实现F-score 0.762 vs 基线0.762，RFM贡献+0.044增益；预训练计算成本360 GPU小时；DAV4在动态重建和新视角合成上优于之前的Depth Anything模型和每场景4D-GS。

Conclusion: DAV4在动态重建和新视角合成任务上优于之前的Depth Anything模型和每场景4D-GS，且训练过程中不使用人工标注的深度标签。
...
TL;DR: 本文介绍了JSL-DC数据集，这是最大的日本手语(JSL)视频数据集，包含36.7K视频和19位手语者，旨在帮助听人父母学习手语与聋童沟通。


<details>
  <summary>Details</summary>
Motivation: 95%的聋童出生于听人家庭，父母通常不精通手语，而现有的日本手语数据集规模小且缺乏多样性，阻碍了能够泛化到新用户的模型开发。

Method: 采用以聋人为中心的设计方式：由 Deaf 和 Coda 语言学家选择270个JSL词汇，所有参与者为使用JSL的聋人，数据经过 Deaf 语言学家的两阶段审查，并提供语言学家对易混淆手语的描述。

Result: JSL-DC是最大的JSL数据集；基于语言学家描述的模型在易混淆手语子集上比最先进方法提升了9.8%。

Conclusion: 以聋人为中心的数据集设计和语言学描述可以为手语识别任务提供新的思路，易混淆手语的描述有助于开发更精确的识别模型。
...
TL;DR: CoMVS-GS是一个结合多视图立体几何(MVS)与3D高斯溅射的通用表面重建框架，通过从密集MVS点初始化高斯原语、引入PatchMatch-3DGS双向监督以及使用Delaunay图割网格化替代TSDF体素融合，解决了弱观测和遮挡区域中高斯原语不稳定和几何不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting在Novel View Synthesis方面表现出色，但在弱观测和遮挡区域进行精确网格重建仍然困难，高斯原语可能生长为不稳定或几何不一致的结构。

Method: 1. 从密集多视图立体点初始化高斯原语，使用预展平的尺度和法线对齐的方向，提供比稀疏SfM初始化更强的几何先验；2. 引入PatchMatch-3DGS双向监督，高斯渲染的深度和法线初始化PatchMatch refinement refined PatchMatch深度监督高斯优化；3. 使用Delaunay图割网格化pipeline替代TSDF体素融合，减少对体素分辨率的敏感性。

Result: 在DTU、GauU-Scene V2和MatrixCity数据集上的实验表明，CoMVS-GS在物体级重建上保持竞争力，并在户外场景中提升了几何精度和网格紧凑性，同时保持高质量渲染。

Conclusion: CoMVS-GS通过结合MVS和3D Gaussian Splatting，有效解决了弱约束区域的几何重建问题，在多种场景下实现了更准确、更紧凑的网格重建，同时保持了高渲染质量。
...
TL;DR: COSTA提出了一种用于航空点云开放集语义分割的测试时自适应方法，通过聚类级语义传播实现跨域标签空间的分割。


<details>
  <summary>Details</summary>
Motivation: 航空点云的语义分割在领域偏移下面临泛化危机，现有方法受限于封闭集标签假设，难以处理语义偏移。需要在推理阶段无需训练即可适应目标域，并支持超出源标签空间的目标特定类别分割。

Method: 提出聚类中心开放集语义传播范式：1) 通过测试时适应桥接域差距；2) 在适应后的特征空间中根据相似性分布将目标域点分组为少量语义聚类；3) 利用开放词汇视觉语言模型获取高置信度伪标签，通过聚类级投票传播到所有点。

Result: 以DALES为源域，COSTA在三个具有不同域和异构类别空间的航空点云基准上实现了按需分割，mIoU达到70.09%。

Conclusion: 所提出的聚类中心范式能够在显著域差距和混合语义偏移条件下实现航空点云的测试时自适应，为开放集点云分割提供了新思路。
...
TL;DR: SparsePR是一种无需训练的块稀疏注意力方法，结合响应耦合分区与探针拟合残差重建，在视频生成和世界模型中实现1.48x-2.61x加速，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有行式注意力集中方法未指定可执行的稀疏算子，查询共享块路由可能导致支持重叠差，且保留的注意力质量不能决定跳过交互的softmax后误差，分区几何影响 pooled support 和稀疏输出的残差可预测性。

Method: SparsePR结合响应耦合分区(Response-Coupled Partitioning)与探针拟合残差重建(Probe-Fitted Residual Reconstruction)。采样查询-键响应形成配对的K/V组，其质心诱导查询-响应坐标用于共享路由。少量精确查询行校准输出子空间中的仿射校正。

Result: 在四个异构视频生成和世界模型上，SparsePR一致降低注意力重建误差。消融显示探针拟合贡献大部分误差减少，响应耦合分区降低硬丢弃误差并在有限探针预算下改善重建。在22.0-26.0%实际执行对密度下保持生成质量，实现1.48x-2.61x端到端加速。

Conclusion: SparsePR通过分区几何优化和探针拟合残差重建，有效平衡了稀疏性与保真度，为视频transformer的高效推理提供了可行方案。
...
TL;DR: DyG²T是一个对象动力学建模框架，通过空间补全和时间判别关键点表示，以及在粒子图上进行多尺度交互建模，从有限的视觉观察中推断对象运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有动力学建模方法将重建的粒子表示压缩为稀疏关键点，并使用局部约束交互进行建模，丢弃了细粒度的局部细节，并在空间和时间尺度上模糊了区分性交互建模，导致轨迹漂移和外观预测不准确。

Method: 提出三个核心组件：1) 空间补全：通过聚合相邻原始粒子位置丰富每个关键点，并显式编码关键点之间的相对偏移以增强几何结构感知；2) 时间判别网络(TDN)：识别潜在空间中的跨帧主导变化并放大帧间差异，生成时间判别表示；3) 粒子图Transformer：利用全局注意力保留关键点之间的区分性长程依赖。

Result: 在合成和真实世界数据集上的实验表明，DyG²T实现了准确的动力学建模和推理，并表现出强大的跨对象和真实世界泛化能力。

Conclusion: DyG²T通过空间补全、时间判别和多尺度交互建模，有效解决了现有方法中轨迹漂移和预测不准确的问题，为具身交互场景中的准确运动轨迹预测提供了新的解决方案。
...
TL;DR: 本文提出了一种用于跨TVUS和MRI的卵巢分割的双分支框架，通过adapt MedSAM3并使用TVUS衍生的原型库来对齐解剖一致的特征表示，在子宫内膜异位症相关数据集上实现了超过5个百分点的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单模态分析或疾病分类，跨模态卵巢分割尚未被充分探索。MRI中卵巢分割困难因其目标小、与周围盆腔结构边界模糊。

Method: 提出双分支框架，通过adapt MedSAM3并结合TVUS-derived prototype bank来对齐两种模态间的解剖一致特征表示。消融实验验证了原型库和warm-up预训练的重要性。

Result: 与多种先进方法相比，所提双分支方法在定量和定性指标上均提升超过5个百分点。

Conclusion: 该方法成功实现了TVUS和MRI跨模态卵巢分割，原型库和warm-up预训练是性能提升的关键组件。
...
TL;DR: OptiModNet是一种轻量级混合架构，用于青光眼筛查中的视盘和视杯分割。该模型结合了多种注意力机制来增强局部和全局特征表示，并采用聚合金字塔损失在多个解码器深度进行监督。在REFUGE2数据集上，该方法达到了最先进性能，超过现有方法2.5%以上，同时保持高效率（仅3.73 GFLOPs和1.93M参数）。


<details>
  <summary>Details</summary>
Motivation: 青光眼的早期检测需要精确分割视盘和视杯，但在保持低计算需求的同时实现跨数据集的一致高性能仍是一项重大挑战。现有方法如UNet、Vision Transformers和扩散模型虽然分割性能强，但计算开销大。UNet擅长捕捉局部特征但缺乏全局上下文建模能力；ViT擅长长距离依赖建模但计算密集；混合架构虽性能提升但复杂度增加。

Method: 提出OptiModNet，一种轻量级混合架构，集成多种注意力机制在网络多个阶段以增强局部和全局特征表示；引入聚合金字塔损失（Aggregated Pyramid Loss）在多个解码器深度监督预测，以促进更好的梯度流和结构一致性。

Result: 在REFUGE2数据集上，视盘和视杯分割任务均达到最先进性能，超过现有方法2.5%以上，同时保持高效率：仅3.73 GFLOPs计算量和1.93M参数。

Conclusion: OptiModNet作为一种轻量级混合架构，在视盘和视杯分割任务中实现了最先进的性能，兼顾高精度和低计算成本，为资源受限的临床环境中的青光眼大规模筛查提供了可行方案。
...
TL;DR: 研究将AI生成图像检测作为迁移系统进行分析，考察分类器头训练在现代特征空间中增加价值的条件。通过在39个公共数据集（710万张图像）上的统一协议Percept-Lens，发现基于矩的判别器在匹配先验和编码器时具有竞争力或优于现有检测器头。


<details>
  <summary>Details</summary>
Motivation: 扩散生成器生成的合成图像已无处不在，但检测器在生成器、提示词/风格和源域同时发生变化时常常失效。需要理解分类器头训练何时能在现代特征空间中增加价值。

Method: 将AI生成图像检测建模为迁移系统，包含训练先验、冻结编码器特征空间和决策规则三个组件。构建先验条件化的高斯判别阶梯，在嵌套协方差假设下利用一阶和二阶特征统计量构建闭式分类头。

Result: 在Percept-Lens协议（39个公共数据集，710万张图像）上，最高阶梯在匹配先验和编码器时经常具有竞争力，有时甚至超越已发布的AI生成图像检测器头。研究还发现对训练先验的强敏感性、基于矩的分类头的数据效率，以及高斯偏移度量的表示依赖性。

Conclusion: 研究结果支持在（先验、编码器、头）层级进行报告，并为AI生成图像检测迁移提供更强的分析基线。强调需要更严格地评估检测器在不同条件下的迁移能力。
...
TL;DR: 本文提出StateTrace框架，解决现有VideoLLM在长视频中物体消失后无法正确推理其隐藏状态的难题，通过构建可重用的时空状态记忆实现隐状态推理，并在新构建的HSR-Bench基准上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在长视频时空推理方面存在明显缺陷：当目标物体变得不可见时，模型经常将"不可见(invisible)"误判为"未知(unknown)"，无法正确推断物体在消失期间的状态演变。这一问题被称为隐状态时空推理，即从上下文交互中推断物体在长时间不可见间隔期间的状态。

Method: 提出StateTrace框架，包含三个核心组件：(1)构建可重用的时空状态记忆模块，将物体轨迹、物体间关系和状态转换事件组织成结构化推理基材；(2)推理时检索与问题相关的状态演化轨迹；(3)将检索结果转换为紧凑的推理线索，使模型能够显式推理物体消失的原因、消失期间的状态演变以及查询时状态是否持续。

Result: 在HSR-Bench基准（包含1384个独特视频的1427个视频问答样本）上，StateTrace显著提升性能：VideoLLaMA3从39.6提升至64.2。同时在多个公共基准上也展现出一致的性能提升。

Conclusion: StateTrace通过引入显式的隐状态推理机制，有效解决了VideoLLM在长视频中物体不可见时的推理难题，为视频理解领域提供了新的研究方向和评估基准。
...
TL;DR: EpigraphNet是一个用于古楔形文字自动识别的分割引导Transformer管道，在132类基准上达到86.41% top-1准确率，显著优于CNN和现代骨干网络。


<details>
  <summary>Details</summary>
Motivation: 古楔形文字识别面临复合信号退化问题：粘土板的三维浮雕产生空间变化的照明和投射阴影，表面侵蚀引入与真实符号重叠的结构化噪声，以及141个符号类别间的严重类不平衡，这些都削弱了分类器的可靠性。

Method: 使用亮度自适应形态学预处理和零样本SAM2-Large分割从1,239张注释 tablet 图像生成干净的二进制符号掩码，然后使用逆频率类别加权的微调Vision Transformer (ViT-B/16)进行分类。

Result: EpigraphNet在132类基准上达到86.41% top-1准确率，比最强CNN基线(ResNet-101，69.20%)提高17.21个百分点，比四种现代骨干网络(DeiT-B/16、Swin-B、ConvNeXt-B、EfficientNet-B4)提高5.31-12.91%。在NVIDIA A100 GPU上运行约18毫秒/符号。符号频率与每类性能之间的Spearman相关性较低，表明频繁类和稀有类的识别更加平衡。

Conclusion: EpigraphNet成功解决了古楔形文字识别中的信号退化和类不平衡问题，在准确率和推理速度上均优于现有方法，为数字人文研究提供了有效工具。
...
TL;DR: 本文提出PATE-Forensics，采用"感知即工具"范式，将深度伪造检测中的检测、定位与解释生成解耦，通过DINOv3基础的工具实现多粒度检测和引导定位，为MLLM提供结构化上下文生成解释，在DDL-X Track 3上获0.89分。


<details>
  <summary>Details</summary>
Motivation: 现有可解释深度伪造检测方法通常依赖任务适配的多模态大语言模型(MLLM)来联合解决检测、定位和解释问题，这种紧密耦合的方式不够灵活。

Method: 提出 Perception-as-Tool 范式，将检测和定位与解释生成解耦。核心是DINOv3基础的工具，包含：1)多粒度检测模块，整合全局、patch级和segment级证据；2)引导定位模块，将这些证据空间化为伪造分数图以指导密集掩码预测。原始图像和感知输出形成结构化上下文，供通用MLLM通过提示约束生成解释。

Result: 在DDL-X Track 3上获得最佳官方分数0.89，比第二名团队高出0.19分。

Conclusion: 通过感知即工具范式成功实现了检测/定位与解释的解耦，实验证明该方法在可解释深度伪造检测任务上达到了最先进的性能。
...
TL;DR: MR-IQA-2是一个用于盲图像质量评估的演员-编辑-法官框架，通过解耦推理和评分的信用分配，并使用可验证的监督来提高推理的忠实性。该方法在IQA基准测试中实现了与人类竞争性的评分对齐。


<details>
  <summary>Details</summary>
Motivation: 当前大多数图像质量评估方法通过人类评分来监督推理，但评分准确性 alone 不能确保推理忠实于图像质量。正确的评分可能只是偶然发生，导致强化不忠实的推理。需要解耦推理和评分的信用分配，并提供可验证的监督来确保推理的忠实性。

Method: 提出MR-IQA-2框架，包含三个组件：1) 演员(actor)生成图像的质量推理；2) 编辑(editor)根据识别的质量因素修改图像；3) 冻结的法官(judge)比较原始和编辑后的图像，提供反思性监督。使用细粒度信用分配解耦推理和评分监督，法官反馈监督推理，人类评分监督预测评分，掩码token特定更新区分这些信号。

Result: 在IQA基准测试中实现了与人类竞争性的评分对齐。视觉反思能够实现更丰富和更忠实的视觉理解，超出单纯评分，可用于图像质量优化和相关下游任务。

Conclusion: 通过演员-编辑-法官框架和细粒度信用分配，MR-IQA-2成功解耦了推理和评分的监督，提高了推理的忠实性和可靠性。该方法为盲图像质量评估提供了一种新的思路，具有重要的应用价值。
...
TL;DR: SPARC是一种无需梯度或权重更新的少样本校准方法，通过对编码器和解码器之间的patch特征进行逐单元格子空间投影，去除部署时的干扰因素。仅需≤8张验证正常图像即可实现对7种异常检测器的校准，在存在分布偏移的基准数据集上，图像AUROC提升13.8个百分点，AU-PRO₀.₃提升3.5个百分点。


<details>
  <summary>Details</summary>
Motivation: 基于视觉的工业异常检测器在一种数据分布下校准，但部署时可能面临照明、夹具放置或传感器特性等差异，导致性能急剧下降。由于标注异常数据稀缺，需要仅使用少量验证正常图像进行适应。现有方法需要反向传播、检测器特定调优或特征方向选择，难以用少量校准样本实现。

Method: SPARC拦截编码器与检测器之间的patch特征，通过逐单元格子空间投影去除部署时干扰的闭式空间索引估计。使用代数饱和秩r=k-1（k≤8为验证正常图像数量）在编码器的原生patch网格上操作，无需梯度或权重更新，可兼容记忆库密度、基于原型和互信息等多种检测器。

Result: 在存在分布偏移的基准数据集上，SPARC使所有7种依赖校正patch特征的检测器的图像AUROC平均提升13.8个百分点，AU-PRO₀.₃提升3.5个百分点。在无工程偏移的基准上变化较小且混合。对照实验表明性能提升来自逐单元格子空间结构而非单纯校准图像。

Conclusion: SPARC是一种有效的少样本校准方法，通过patch级子空间投影解决工业异常检测中的分布偏移问题，在少至8张验证正常图像条件下即可显著提升检测性能，且无需梯度更新或检测器特定修改，具有广泛的适用性。
...
TL;DR: 本文推出了OmniHandwritingOCR，一个用于评估多模态大语言模型(MLLMs)和OCR系统在手写识别任务上的诊断基准。该基准涵盖手写文本识别和手写数学表达式识别，包含6个子任务和12个子集，共77.57K张标注图像。评估了13个开源和闭源系统，使用5个互补指标进行统一协议测试。


<details>
  <summary>Details</summary>
Motivation: 现有OCR基准主要关注印刷体文本或干净的单行输入，缺乏对现实手写OCR场景的覆盖，如多语言手写、书写错误和结构复杂的数学表达式。当前MLLMs作为OCR系统的能力尚未得到充分探索。

Method: 构建了包含六个子任务和十二个子集的OmniHandwritingOCR基准，总计77.57K张标注图像，来源于公共数据集和新收集的学生手写内容。核心组件是一个难度分层的多行公式语料库，用于测试模型在递增结构复杂性下的鲁棒性。采用统一协议对13个开源和闭源系统进行五项互补指标评估。

Result: 当前系统在手写转录方面仍远未达到忠实转录：复杂多行公式上性能急剧下降；模型排名因语言和公式设置而异；多个生成模型会产生看似合理但缺乏视觉支持的幻觉纠正。

Conclusion: OmniHandshakeOCR为诊断多模态模型在手写OCR场景中的语言、内容、结构化和视觉定位失败模式提供了具有挑战性的测试平台，推动了该领域的研究进展。
...
TL;DR: 针对紧凑型目标检测器表征能力有限的问题，提出了频率解耦交叉注意力知识蒸馏(FD-CanKD)框架，在三个互补层次上转移教师知识：头部级预测监督、关系级非局部上下文传递和频率级分量选择性对齐。学生在19.7M参数下达到48.87 mAP50:95。


<details>
  <summary>Details</summary>
Motivation: 紧凑型目标检测器适用于资源受限的视觉感知场景，但其有限的表征能力与大型模型存在精度差距。现有检测器知识蒸馏方法通常依赖于预测级监督或单一特征对齐目标（如响应、分布、相关性或频域匹配），无法充分转移教师模型的多层次知识。

Method: 提出FD-CanKD框架，包含三个互补的知识转移层次：1)头部级预测监督；2)关系级非局部上下文传递，通过交叉注意力机制聚合教师侧空间上下文；3)频率级分量选择性对齐，保留互补的结构和细节敏感线索。在YOLOv12师生设置中进行验证。

Result: 在COCO数据集上，固定50轮从头训练对比中，FD-CanKD与代表性检测器知识蒸馏基线保持竞争力。蒸馏后继续微调20轮，学生模型达到48.87 mAP50:95、65.84 mAP50和53.40 mAP75。最终部署的学生模型保持19.7M参数，蒸馏模块已全部移除。

Conclusion: FD-CanKD通过多层次知识转移有效缩小了紧凑检测器与大型模型的精度差距，验证了预测监督、关系上下文和频率对齐三者结合的互补性。该框架为资源受限场景下的高性能目标检测提供了有效方案。
...
TL;DR: ReX-Shot是一个统一的几何和相机grounded生成框架，用于单图像重摄影，能够从单张参考图像中联合控制视角、焦距和参数化摄影效果。该方法首次实现了在单一框架下同时控制这三个要素，并支持近实时的交互式重摄影。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常分别处理视角、焦距和摄影效果这些因素，在联合控制时表现不佳：新视角合成在焦距变化时可能产生几何扭曲，而超分辨率和指令引导编辑仍局限于2D，无法可靠地将细节恢复或外观控制扩展到新视角。这些限制源于不完美的单图像3D重建和连续焦距放大的采样极限。

Method: 1. 使用隐式转换的基础模型特征进行稳健的目标视角引导，以减少几何误差的投影偏差；2. 将焦距放大表述为几何引导的超分辨率问题，并利用生成式细节先验来恢复稀疏3D重采样中丢失的细节；3. 基于3D感知生成骨干网络，将摄影效果控制从2D滤波提升到3D感知的外观编辑，保持跨视角和焦距的内容一致性。

Result: ReX-Shot是首个能够从单图像联合控制视角、焦距和参数化摄影效果的统一框架。实验表明，ReX-Shot在所有三个控制维度上均优于代表性基线方法，并支持近实时的交互式重摄影。

Conclusion: ReX-Shot通过几何引导的生成式方法成功解决了单图像重摄影中的核心挑战，实现了视角、焦距和摄影效果的联合控制，具有重要的应用价值和创新意义。
...
TL;DR: 本文针对视频音频联合生成模型的强化学习训练中奖励信号设计不合理的问题，提出了VAPref-10K数据集、VA-Judger-Bench基准和VA-Judger链式思考全模态奖励模型，通过分阶段学习策略和维度化强化学习提升奖励模型与人类偏好的对齐程度。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过组合音频质量、视觉保真度和同步性等独立指标构建奖励信号，但这些指标无法捕捉文本提示、视频和音频之间的整体语义和时间一致性，导致奖励黑客现象——生成内容在指标上得分高但对人类观众来说不连贯或不符合预期。

Method: 首先构建大规模人类偏好数据集VAPref-10K（包含9K提示词和10.3K细粒度配对比较）；提出VA-Judger-Bench基准用于评估奖励模型与人类偏好的对齐程度；VA-Judger采用三阶段方法：1）从质量差异明显的样本学习结构和粗粒度偏好判别；2）通过拒绝采样验证人类注释来提炼近质量比较的可靠偏好解释；3）执行维度化强化学习，将人类反馈分解为单独质量维度以获得比单一二元偏好标签更密集的奖励信号。

Result: VA-Judger在域内和域外评估中均优于指标基线，更准确预测人类偏好；使用其与人类对齐的奖励信号进行视频音频生成模型的后训练，生成质量显著提升。

Conclusion: 通过构建高质量人类偏好数据集、设计针对性基准和提出链式思考奖励模型，有效解决了视频音频联合生成中奖励信号与人类偏好对齐的问题，为该领域的强化学习训练提供了更可靠的优化目标。
...
TL;DR: 本文提出CDGP（对比双高斯过程），一种弱监督框架，通过建模正常和异常诱导变量的预测分布来实现工业视觉缺陷检测和分割。该方法使用后验优势统计量标准化预测均值差，结合层次化正常重建残差实现精确定位，在MVTec AD数据集上取得领先的定位性能。


<details>
  <summary>Details</summary>
Motivation: 工业视觉检测需要同时判断产品是否有缺陷并定位缺陷位置，但像素级标注成本高昂。现有异常分割方法仅从无缺陷图像学习，通过评分偏差来检测异常。然而，真缺陷和异常但正常的区域都可能产生高偏差分数，导致误报。

Method: 提出对比双高斯过程（CDGP）框架：1）建模正常和异常诱导变量在密集token上的预测分布；2）使用后验优势统计量，通过联合预测不确定性标准化预测均值差，同时提供空间证据和图像级置信度；3）结合层次化正常重建残差进行精细定位。所有校准仅使用训练数据，无需人工像素标注或测试时拟合。

Result: 在MVTec AD、KSDD2和VisA三个数据集上，CDGP在MVTec AD的所有定位指标上排名第一，在KSDD2和VisA上排名第一或具有竞争力。消融实验验证了线性核高斯过程 formulations 的贡献和适用范围。

Conclusion: CDGP通过对比学习框架和高斯过程不确定性建模，实现了弱监督下的缺陷精确定位，为工业视觉检测提供了一种无需大规模像素标注的有效方案。
...
TL;DR: SPARC是一个用于胎儿心脏MRI的自动化3D+time cine图像重建 pipeline，结合了物理信息驱动的切片到体积重建(SVR)与深度学习模型，实现快速、高质量的胎儿心脏运动捕捉。


<details>
  <summary>Details</summary>
Motivation: 胎儿心脏MRI (fCMR)可提供超声心动图的补充诊断信息，尤其适用于复杂先天性心脏病(CHD)。但从2D+time切片重建3D+time cine体积面临胎儿运动不可预测、缺乏自动化处理工具等挑战。

Method: 提出SPARC pipeline：(1)使用多普勒超声(DUS)门控的切片堆栈进行物理信息驱动的切片到体积重建(SVR)；(2)利用深度学习模型进行胸腔分割和解剖重新定向；(3)集成聚合提高分割性能。

Result: SVR算法重建时间缩短10倍(4.8±1.0 vs 49.0±14.1分钟，p<0.0001)且质量提升；胸腔分割Dice 84.7±3.9%超过人工标注一致性81.4±7.7%(p<0.05)；解剖重定向成功率90.1%；121例临床队列端到端评估显示82.6%完全自动处理，平均运行时间7.1±1.3分钟。

Conclusion: SPARC pipeline实现了胎儿心脏MRI的自动化临床级处理，兼顾速度与质量，已作为Docker容器公开部署于研究机构。
...
TL;DR: PALATE是一个个性化肖像修图框架，通过将用户偏好分解为全局 backbone、类别级残差和轻量级用户适配器来捕捉个体审美差异。采用循环双层蒸馏方案将用户偏好迁移到类别级别，再整合到全局 backbone，实现仅需少量排名即可校准新用户。


<details>
  <summary>Details</summary>
Motivation: 现有肖像修图方法优化群体级审美标准，无法捕捉个体审美偏好；而为每个用户单独微调编辑模型会产生高昂的训练、存储和数据成本。需要一种高效且能捕捉个人审美偏好的方法。

Method: 提出PALATE框架，将用户奖励分解为三层：1) 全局backbone（所有用户共享）；2) 类别级残差（审美相似用户共享）；3) 轻量级用户适配器（用户特定）。采用循环双层蒸馏方案：将用户偏好先蒸馏到类别奖励，再将类别知识整合到全局backbone，并通过反崩溃正则化保持三层互补。

Result: 在PPR10K数据集上，PALATE达到72.83%的成对偏好预测准确率，超越所有基线方法。最强基线PickScore仅达58.06%。每个新用户仅需512字节参数和毫秒级推理。

Conclusion: PALATE成功实现了高效的个性化肖像修图选择，通过分层奖励分解和循环蒸馏，在保持模型共享的同时有效捕捉个体审美差异，仅需极少数据即可校准新用户。
...
TL;DR: 本文评估了基于深度学习的图像匹配方法在无人机视觉里程计（VO）中的应用。研究使用合成数据集和下视相机，发现RoMa匹配器效果最佳，但传统SIFT特征在某些场景下可超越部分先进方法。


<details>
  <summary>Details</summary>
Motivation: 无人机依赖GNSS进行导航，但在信号不可用或受干扰时会导致灾难性故障，因此需要探索视觉里程计作为替代导航方案。

Method: 在合成数据集上使用下视相机对多种最新的基于深度学习的图像匹配方法进行评估，测试其在无人机位置跟踪任务中的表现。

Result: RoMa匹配器生成最佳结果，但传统SIFT特征在某些情况下可以优于部分最先进的深度学习方法。

Conclusion: 深度学习图像匹配方法在视觉里程计中具有潜力，但传统方法如SIFT在特定场景下仍具竞争力，未来可进一步集成到完整VO系统中。
...
TL;DR: PCQA-R1是首个用于3D点云质量评估的强化学习大语言模型，通过群体相对策略优化(GRPO)实现质量理解和评分的协同建模，解决了跨数据集MOS尺度异构性带来的泛化难题。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的无参考点云质量评估方法主要依赖监督微调直接预测数值质量分数，缺乏跨异构MOS尺度和有限注释数据集的泛化能力。绝对MOS回归在不同分数尺度和失真分布的数据集间容易失效，而相对质量排序在此类迁移下更为稳定。

Method: 1.构建链式思维数据集PCQA-CoT，通过逆向推理策略作为冷启动训练数据，教导LMM生成推理过程；2.引入高斯邻近奖励机制，将分数预测锚定到源MOS范围，防止校准漂移；3.基于GRPO策略进行强化学习训练。

Result: 在五个基准数据集上实现跨数据集泛化最优性能，同时保持有竞争力的域内准确率。消融实验验证了排序、高斯奖励和冷启动轨迹的作用。

Conclusion: 首次将强化学习大语言模型应用于3D点云质量评估，通过相对排序和结构化奖励设计有效解决了跨数据集泛化问题，为PCQA领域提供了新范式。
...
TL;DR: 本文研究了安全对齐的视觉语言模型(VLM)中，安全约束指令导致的拒绝行为是否会抑制视觉 grounded 推理能力。研究发现，尽管模型表现出拒绝行为，但视觉证据在内部解码过程中仍然被保留，只是最终生成被重定向到拒绝响应。


<details>
  <summary>Details</summary>
Motivation: 作者观察到一个有趣的现象：在安全约束指令下，VLM 经常拒绝回答问题，即使在默认指令下可以正确回答相同输入。这引发了一个根本性问题：安全对齐是抑制了视觉 grounded 推理本身，还是视觉证据仍然可用但生成被重定向到拒绝？

Method: 1. 分析多个架构和 multimodal benchmarks 中安全诱导拒绝的内部解码动态
2. 干预实验：通过激活级别的干预抑制拒绝相关表征

Result: 1. 尽管表现出拒绝行为，视觉证据在解码过程中仍然一致地影响生成，表明感知 grounded 基本上被保留
2. 安全约束指令一致地改变晚期隐藏状态动态，使其朝向拒绝导向的解码（尽管不同架构的拒绝组织方式不同）
3. 抑制拒绝相关表征可以可靠地恢复 grounded 回答行为，无需重新训练或修改视觉输入

Conclusion: 揭示了对齐 VLM 中一个先前未被充分探索的失败模式：安全对齐可以覆盖 grounded 的视觉表达，即使感知证据在内部被保留。这表明安全对齐机制是通过干预生成阶段而非抑制感知阶段来发挥作用的。
...
TL;DR: SAM2Dual是一种无需训练的即插即用推理时增强方法，用于改善长视频对象分割的鲁棒性。通过双内存设计将短期记忆和长期记忆分离，并结合文本感知记忆模块，在不更新模型权重的情况下提升长视频稳定性。


<details>
  <summary>Details</summary>
Motivation: 长期视频对象分割面临扩展遮挡、重新出现和场景变化下的错误累积问题。SAM2的流式记忆在长时域中会放大漂移，因为最近的不可靠预测会主导记忆状态。

Method: 提出双内存设计：(1)短期记忆用于快速局部适应；(2)长期记忆通过间隔采样保留全局身份线索，采用门控融合策略结合两者。此外提出文本感知记忆(TAM)，从早期帧提取词级线索，使用文本嵌入根据语义兼容性重新加权记忆贡献。

Result: 在长期基准测试中，SAM2Dual持续提高长视频稳定性，MOSEv2上J&F从49.33提升至50.65，LVOSv2上也获得一致提升。

Conclusion: SAM2Dual是一种无需训练的即插即用推理时增强方法，无需更新模型权重即可改善长视频鲁棒性。
...
TL;DR: 本文提出了一种用于医学图像 captioning 的临床结构化奖励框架（Clinically Structured Surrogate Reward Framework），在 SFT 后阶段结合生物医学语义保真度、短期词汇保真度、分布图像邻域对齐和临床图一致性四种奖励，通过 GDPO 优化，在 ImageCLEFmedical Caption 的 Standard 和 Synthetical 赛道上，在三种视觉-语言骨干网上均取得了 Overall、Relevance 和 Factuality 的提升。


<details>
  <summary>Details</summary>
Motivation: 医学图像 captioning 需要将异构视觉证据转化为简洁的临床描述，但现有方法的奖励函数主要依赖全局文本相似度、图像-文本兼容性或无序概念重叠，无法捕捉视觉邻域和临床声明结构。序列级策略优化虽能直接优化完整 caption，但常用奖励忽略了医学图像银行分布和实体-断言-关系的一致性。

Method: 该框架结合四类奖励：1）生物医学语义保真度；2）短期词汇保真度；3）分布图像邻域对齐（匹配参考 caption 和生成 caption 诱导的医学图像银行分布）；4）临床图一致性（对实体、断言状态和类型化关系进行最大权重一对一匹配）。四个奖励在每个 rollout 组内独立归一化，结合固定相对权重，使用 GDPO 优化。

Result: 在 ImageCLEFmedical Caption 的 Standard 和 Synthetical 赛道以及三种视觉-语言骨干网上，该方法在所有 6 种骨干网-赛道组合中均优于匹配的 SFT 基线，Overall、Relevance 和 Factuality 的平均相对增益分别为 3.4%、2.1% 和 5.8%。消融实验表明结构化奖励提供互补信号，减少图像邻域差异，提高实体-断言-关系一致性。

Conclusion: 提出的临床结构化奖励框架通过结合语义保真度和结构化奖励（图像邻域对齐与临床图一致性），有效提升了医学图像 captioning 的事实性和相关性，为后 SFT 阶段的医学图像描述生成提供了一种有效的策略优化方法。
...
TL;DR: 本文提出Dynamic SpectraFormer，一种基于频域Transformer的水下图像增强方法。该方法通过超分辨率稀疏频谱注意力模块捕捉长距离依赖，并引入动态频谱权重生成层作为自适应频带选择器，同时处理低频（颜色、亮度失真）和高频（边缘、纹理失真）问题。在多个水下图像增强基准数据集上进行了广泛消融研究和比较评估，验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 水下图像因光的折射和吸收而遭受颜色失真、雾化和低可见度影响，这些挑战严重影响自主水下车辆(AUVs)或 marine robots的利用。低频失真表现为颜色和亮度失真，高频失真表现为边缘和纹理失真。传统方法主要集中于空间域，难以同时纠正这些混合失真。

Method: 提出Dynamic SpectraFormer，采用频域增强方法。核心包括：1) 超高分辨率稀疏频谱注意力模块，可在不丢失通用逼近能力的情况下捕捉长距离依赖；2) 动态频谱权重生成层，作为自适应频带选择器，增强关键频段并抑制不相关频段。

Result: 该方法显著改善了水下图像质量，同时解决高频和低频失真问题。在多个水下图像增强基准数据集上进行了广泛消融研究和比较评估。

Conclusion: Dynamic SpectraFormer在多个水下图像增强基准数据集上验证了有效性，消融研究和比较评估结果巩固了其优越性。源代码已公开于GitHub。
...
TL;DR: Teeth2Point是一种高效的基于点标记的Transformer框架，用于牙科CBCT语义分割，能够处理缺失或错位牙齿的情况，避免了全局上下文和细粒度结构细节之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习在牙科CBCT分割中表现良好，但准确标记缺失或错位牙齿仍是一个关键挑战。Transformer架构理论上可以利用全局解剖上下文解决这种模糊性，但由于CBCT体积的高分辨率和牙齿在体积内的广泛空间分布，密集基于块的体积处理面临固有的权衡问题：计算成本限制了自注意力中使用的块数量，导致无法同时捕获广泛的上下文和细粒度结构细节。

Method: Teeth2Point采用两阶段方法：首先使用卷积模型定位包含牙齿的体积感兴趣区域(ROI)，然后使用自适应采样将ROI转换为点标记。Transformer模型使用这些点标记进行预测，既能捕获全局上下文又能保持高分辨率。Transformer首先使用自监督学习(SSL)进行预训练，风格类似DINO但使用领域特定的增强策略，包括随机标记掩码以增强对复杂解剖变化的鲁棒性，然后进行监督微调。

Result: 与最强的两阶段基线相比，Teeth2Point在四个数据集的异常病例上平均提高了1.44 DSC分数；与第一阶段nnU-Net相比，提升了1.9个分数点。

Conclusion: Teeth2Point通过点标记方式有效解决了牙科CBCT分割中全局上下文和细粒度细节之间的权衡问题，自监督预训练策略增强了对复杂解剖变化的鲁棒性，显著提升了异常病例的分割性能。
...
TL;DR: 本文是一篇关于视觉语言模型(VLMs)用于自我中心视频理解的综述论文，涵盖了从传统识别架构到多模态基础模型和具身系统的发展历程。文献组织围绕任务、数据集、手-物体交互理解、时间推理、帧和片段选择、多模态表征学习、提示设计、语义对齐和模型适应等方面，特别关注图推理和物体中心推理机制。


<details>
  <summary>Details</summary>
Motivation: 自我中心视频从佩戴者视角捕捉活动，提供人类注意力、手-物体交互和目标导向行为的直接视图。这一视角对可穿戴智能、辅助系统、人机交互和具身AI日益重要，但同时带来挑战包括自我运动、遮挡、小型活动物体、视角依赖外观和长程时间依赖性。

Method: 综述以任务、数据集、手-物体交互理解、时间推理、帧/片段选择、多模态表征学习、提示设计、语义对齐、模型适应等维度组织文献，特别关注图方法和物体中心推理作为建模手、物体、动作和场景上下文在时间维度上关系的机制。

Result: 现有模型在识别可见物体方面比识别不断演变的交互、动作和用户意图方面更可靠，尤其是在长时间活动中表现不佳。

Conclusion: 未来关键优先研究方向包括：时间基础推理、交互感知监督、高效长视频处理、多模态融合、图增强表征、跨域泛化、隐私保护和可信评估，这些对部署具身智能至关重要。
...
TL;DR: 提出了DynCur-Geo，一种用于无人机主动地理定位的动态好奇心框架，根据剩余目标距离调整预测误差内在奖励，解决固定内在奖励权重导致的探索-收敛平衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有好奇心驱动方法在整搜索过程中分配固定的内在奖励权重，导致在 agent 接近目标后仍奖励新颖性，造成绕路问题，难以平衡探索与目标收敛。

Method: 提出动态好奇心框架DynCur-Geo：1) 距离感知门控机制鼓励早期探索并引导接近目标时的目标导向行为；2) 基于势能的奖励塑造提供密集的进度指导。

Result: 在多模态、跨场景、灾害影响和长距离等多种设置下，相比主动地理定位基线方法均获得一致的性能提升。

Conclusion: 动态好奇心方法能有效平衡探索与目标收敛，为无人机时间敏感应用（如搜救和应急检查）提供了有效的目标搜索方案。
...
TL;DR: FRAGMENT是一个文档生成框架，将文档表示为类型化关系图，并分解为p(structure, content) = p(structure) * p(content | structure)。该框架包含两阶段：Architect（生成图拓扑）和Builder（丰富属性）。可用于文档 forgery 检测和受控编辑。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在像素或token级别操作，难以有效捕捉文档中空间布局、文本内容和逻辑结构之间的依赖关系。需要一种能够显式建模文档结构和内容关系的框架。

Method: 采用两阶段图生成框架：第一阶段Architect是基于类别条件化的因果掩码Transformer，自回归生成图拓扑和类型化空间关系；第二阶段Builder是基于GATv2的图注意力网络，为图添加归一化边界框、文本和视觉风格属性。两阶段均定义显式似然模型，可作为异常分数用于伪造检测。通过prompt条件化扩展实现语义和实体感知的受控编辑。

Result: 在DocLayNet、FUNSD和SROIE数据集上进行了实验评估，与代表性自回归、仅布局和基于图的基线方法进行比较，提供了所提出分解图生成框架的特征和权衡的实证分析。

Conclusion: FRAGMENT框架通过显式建模文档结构与内容的关系，能够有效支持文档 forgery 检测和受控编辑任务，实验验证了该因子化图生成框架的有效性。
...
TL;DR: DocClaw是一个统一的智能文档处理代理系统，将OCR、文档问答和关键信息提取等不同任务统一为代理与文档之间的交互过程。该系统通过文档技能迭代识别信息、调用工具并整合观察结果，在共享的交互框架下处理多种文档任务。


<details>
  <summary>Details</summary>
Motivation: 尽管OCR、文档问答和关键信息提取等智能文档处理任务目标不同，但它们都需要感知文档内容、获取任务相关信息并逐步完善中间结果。然而这些任务通常被 formulation 为独立的预测问题，由特定任务的模型或处理管道分别处理，缺乏统一的处理框架。

Method: DocClaw将多样化的智能文档处理任务统一为代理与文档之间的共享交互过程。给定文档和特定任务查询后，DocClaw遵循适当的文档技能迭代识别所需信息、调用相关工具，并将观察结果整合到期望输出中。在此过程中，结构化的文档状态组织可重用的文档知识和任务特定的交互上下文，使代理能够累积、重访和逐步完善信息。

Result: 在多个智能文档处理基准数据集上进行了广泛实验，证明DocClaw能够在一个统一的代理框架内有效处理多样化任务，并与通用VLM和特定任务方法相比达到具有竞争力的性能。

Conclusion: DocClaw成功地将多种智能文档处理任务统一到一个共享的代理交互框架中，通过文档技能、工具空间和文档状态的共享设计，实现了对不同任务的有效处理，为智能文档处理提供了一种新的统一范式。
...
TL;DR: 本文提出Temporally Decomposable Image Representations (TDIR)，一种将历史照片分解为日期和内容组件的表示学习算法，通过正交子空间实现。无需标签监督即可实现时间信息提取和注入，在历史照片的组合图像检索任务中验证了理论性质。


<details>
  <summary>Details</summary>
Motivation: 神经嵌入空间是多维的、混沌的，难以解释。传统方法将嵌入空间约束到单一时间维度会牺牲下游任务性能。问题是：能否在学习保留时间结构表示的同时，保持图像和对象检索的有效性？

Method: 提出TDIR算法，通过正交子空间将历史照片分解为独立的日期和内容组件。证明了这种分解可实现的条件，分析了部分满足条件时的误差，展示了时间和类别子空间的正交性可从联合优化中自然涌现。

Result: TDIR支持嵌入空间上的传递操作：可将一张图像的时间信息提取并注入另一张图像的表示中，无需标签监督。在历史照片的组合图像检索任务中验证，在日期估计和对象检索方面保持竞争力的同时，提供了可解释的方式来浏览照片档案。

Conclusion: TDIR成功实现了时间结构保留与检索性能之间的平衡，证明了正交子空间分解的可行性，为历史照片档案的可解释导航提供了新方法。
...
TL;DR: 本文提出了一种针对复杂历史手稿文本的本地传统OCR流程，可以通过迭代微调来适应目标手稿的布局级别和外观级别，从而减少人工标注的工作量。同时创建了一个包含细粒度布局注释的梵文手稿数据集，并以标准PAGE-XML格式提供Unicode注释。


<details>
  <summary>Details</summary>
Motivation: 历史手稿具有复杂的异构布局和非标准外观，由于特定时期的书写风格、页面纹理、相机噪声等因素，使其难以进行OCR处理。传统OCR在这些复杂历史文献上效果不佳，而人工标注需要历史领域专业知识，成本高昂且耗时。

Method: 提出一种本地传统OCR管道，可在布局级别和外观级别上对目标手稿进行迭代微调。通过适应目标手稿分布，使后续页面的预测更加准确，从而迭代减少人工标注工作量。使用该流程对三个复杂的梵文历史手稿进行了数字化处理。

Result: 展示了所提出传统OCR流程迭代微调带来的定量增益，并在引入的数据集上对领先的多模态大语言模型进行了基准测试。

Conclusion: 所提出的传统OCR流程能够有效处理复杂历史手稿，通过迭代微调可以不断提高识别准确率，减少对专家标注的依赖，为历史文献数字化提供了可行的解决方案。
...
TL;DR: 首次系统评估不确定性量化方法在语义分割基础模型（SAM2）中的应用，benchmark了4种UQ方法在多个数据集上的表现


<details>
  <summary>Details</summary>
Motivation: 基础模型缺乏可解释性、容易过度自信，且对领域偏移敏感。UQ提供了一种原则性的方法来应对这些挑战，但其与分割基础模型的集成尚未被探索

Method: 在预训练SAM2编码器上微调轻量级DPT解码器，建立简单但具有竞争力的基线；benchmark四种代表性UQ方法：Monte Carlo Dropout、Deep Sub-Ensemble、Test-Time Augmentation和Evidential Deep Learning；在Cityscapes、NYUv2和两个挑战性的域外设置下进行评估

Result: 揭示了预测性能、可靠性和计算成本之间的明显权衡；结果突出了不确定性感知基础模型的前景和当前局限性

Conclusion: 需要未来工作共同优化准确性、鲁棒性和效率，以实现真实世界部署
...
TL;DR: 本文提出了CamWorldQA，这是首个用于相机控制世界视频生成的感知质量评估基准，并设计了CWQA网络（包含空间特征、时间运动特征和光流特征三个分支）来评估此类视频的质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视频质量评估方法主要针对自然视频设计，无法捕捉相机控制生成视频的独特感知特征（如视点一致性、运动连贯性和内容保留）。

Method: 构建包含720个视频的CamWorldQA数据集（来自6种生成方法、20个源视频、6种相机轨迹），并提出CWQA网络提取空间特征、时间运动特征和光流特征进行联合质量预测。

Result: CWQA在CamWorldQA数据集上优于现有的质量评估方法。

Conclusion: CamWorldQA填补了相机控制世界视频生成质量评估的空白，CWQA为该任务提供了有效的评估方案。
...
TL;DR: EgoHRV是一种从egocentric眼镜的视线摄像头中估计心率变异性(HRV)和心率(HR)的方法，解决了传统视觉系统无法捕捉生理指标的问题。该方法结合3D主干网络和低频-高频分解模块来提取血容量脉冲(BVP)信号，并通过跨域预训练对齐基于接触式和相机-derived信号的频域表示。


<details>
  <summary>Details</summary>
Motivation: 现有egocentric视觉系统只能从可见线索捕捉人类行为，忽略了自主状态（如压力、参与度、注意力）的生理指标。HRV是评估自主调节的非侵入性标记，但目前无法在egocentric平台上实现，因为眼球运动和视线视频中的噪声掩盖了连续心跳之间的精细时间差异。

Method: 1) 使用3D主干网络处理视线视频；2) 提出低频-高频分解模块从视线视频中提取BVP信号；3) 跨域预训练对齐接触式和相机-derived信号的频域表示；4) 不确定性感知设计提高下游行为建模。

Result: EgoHRV在HR和HRV估计方面达到最先进精度。将HRV估计和置信度集成到EgoExo4D的熟练度估计器中，准确率提高17.8%。

Conclusion: EgoHRV首次实现从egocentric视频中连续估计HRV，为压力和唤醒感知估计任务开辟了新途径，对技能评估和生理状态监测具有重要意义。
...
TL;DR: 研究CutMix数据增强对语义分割模型的可靠性影响，发现CutMix对精度影响较小，但能显著提升模型在校准和不确定性估计方面的可靠性，尤其在分布偏移场景下。


<details>
  <summary>Details</summary>
Motivation: 尽管CutMix被广泛使用，但其对密集预测任务可靠性的影响尚不明确。鉴于半监督分割方法中CutMix可能严重损害可靠性的发现，研究旨在系统分析CutMix对分割精度、校准和不确定性质量的影响。

Method: 在域内和域外场景下评估两种代表性架构：CNN-based DeepLabV3+和transformer-based SegFormer。

Result: CutMix对分割精度影响较小，但能持续提升可靠性，特别是在分布偏移条件下。这些改进表明CutMix主要增强的是模型校准和不确定性的可信度，而非原始分割预测本身。

Conclusion: CutMix主要提升模型的可信度而非原始性能。这一发现对安全关键应用（如自动驾驶）的部署具有重要意义，因为可靠的置信度估计与原始性能同等重要。
...
TL;DR: 研究通过LoRA适配MedSAM3，仅使用1、2、5、10个标注案例即可实现腹部器官的临床有用分割性能，10个案例即可达到与使用大量数据训练的专业系统相当的性能。


<details>
  <summary>Details</summary>
Motivation: 医学基础模型虽可通过大规模预训练减少新任务的标注负担，但零样本性能仍然有限。参数高效适配方法LoRA可实现高效的专业化，但关键问题是：需要多少专家标注案例才能达到临床有用的分割性能？

Method: 使用LoRA适配MedSAM3，对CT和MRI图像中的5个腹部器官（肝、肾、脾、胆囊、胰腺）进行分割，仅使用1、2、5和10个标注案例，在AMOS22数据集上进行评估，并在Whole Heart Segmentation数据集上进行外部验证。

Result: 仅用10个案例，模型性能即可与使用大量数据训练的专业系统竞争。胆囊分割Dice达到CT 0.68、MRI 0.59，而现有工具几乎完全失败（Dice 0.0004）。肝、肾、脾分割性能在MRSegmentator的5-10%范围内，但标注数据少100倍以上。外部验证显示该方法可扩展到心脏分割，左心室达到竞争性性能。训练仅需每个器官3-5小时，比nnU-Net快2-3倍。

Conclusion: 10个标注案例足以实现临床有用的分割效果，有效减少了图像标注和训练时间的瓶颈。
...
TL;DR: CL4D是首个直接在动态点云上运行的基础4D视觉编码器，通过对比学习目标将时空几何表示与自然语言描述对齐。4DVLM是首个直接在4D点云上运行的多模态大语言模型，不依赖2D图像、2D视频或静态3D点云。在新构建的DynAction4D数据集上训练后，CL4D在4D人体动作基准测试中取得最先进性能，比先前方法提升约16.75%，且4DVLM甚至优于提供RGB视频的Gemini和GPT-5。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编码器主要局限于静态2D图像、缺乏时间建模的3D点云，或缺乏准确几何深度推理的2D视频。因此当前方法无法联合捕获动态场景中的空间结构和运动演化。研究者认为需要一种能够在动态点云上直接操作的基础4D视觉编码器来解决这一问题。

Method: 提出CL4D——首个基础4D视觉编码器，直接在动态点云上运行，采用对比学习目标将时空几何表示与自然语言描述对齐。通过学习文本与4D场景动态之间的共享嵌入空间，实现零样本运动到文本和文本到运动的检索。在此基础上构建4DVLM——首个直接基于4D点云条件进行语言生成的多模态大语言模型。模型在 DynAction4D 数据集上进行训练。

Result: CL4D在多个4D人体动作基准测试中实现了最先进的性能，比先前方法提升约16.75%。4DVLM在4D点云表示的条件下，性能甚至优于使用相同场景RGB视频序列的前沿视频多模态大语言模型（如Gemini和GPT-5）。

Conclusion: 该研究首次提出了直接在动态点云上运行的4D视觉编码器和多模态大语言模型，验证了4D时空几何表示与语言对齐的可行性，为具身AI在动态物理环境中的4D理解和推理奠定了基础。
...
TL;DR: 本文提出了一种针对少样本SAR域适应目标检测的散射感知共享-特定特征分解框架。该方法将检测特征分解为共享路径和多个散射特定专家路径，分别学习可迁移的目标结构信息和补偿异质SAR响应，在四个双向异构SAR检测任务上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有域适应方法主要追求域不变特征对齐，抑制了有助于目标检测的传感器依赖散射特性。此外，在少样本场景下仅有少量带标注的目标域SAR图像可用，使得这一问题更加具有挑战性。

Method: 提出散射感知共享-特定特征分解框架：(1)将检测特征分解为共享路径和多个软门控散射特定专家路径；(2)共享路径学习可迁移的目标结构信息，用于非对称域对齐；(3)散射特定专家自适应补偿异质SAR响应；(4)引入路由域辅助损失鼓励特定专家捕获传感器依赖的路由偏好；(5)使用专家平衡损失防止路由崩溃。

Result: 在FARAD-X/FARAD-Ka和MiniSAR之间的四个双向异构SAR检测任务上进行了广泛实验，在不同少样本设置下验证了方法的有效性。

Conclusion: 所提方法在正向和反向适应方向上均取得了优越的检测性能。
...
TL;DR: 提出了一种用于皮肤疾病分类的模态不变公平表示框架(MIFR)，该框架结合临床照片和皮肤镜图像两种模态，并通过多目标损失函数实现公平性学习。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病是全球主要公共卫生负担，现有的机器学习诊断工具存在两个关键限制：1)仅依赖单一模态进行诊断；2)不同肤色之间存在系统性性能差异。现有的方法只能分别解决其中一个挑战。

Method: 提出模态不变公平表示框架(MIFR)，使用基于ViT的编码器将临床照片和皮肤镜图像投影到高维嵌入空间，通过模态特定投影头实现。训练采用五部分多目标损失：分类的加权交叉熵、混淆和肤色分类的公平性损失、类别对齐的监督对比损失、以及临床和皮肤镜模态对齐的模态不变损失。

Result: 在HIBA+Derm7pt配对数据集以及外部PAD-UFES-20和ISIC 2019数据集上的实验表明，模态不变表示学习提供了与相关基线模型相当的预测性能，并在内部数据集上实现了有竞争力的公平性。t-SNE可视化证实了相同疾病的临床和皮肤镜嵌入在几何上是对齐的。

Conclusion: 该框架成功结合了多模态学习和公平性学习，在保持竞争力的分类性能同时减少了不同肤色间的性能差异，为皮肤疾病辅助诊断提供了更公平、更有效的解决方案。
...
TL;DR: EfficientSync是一个实时基于变形的唇同步框架，通过保留真实纹理而非重新合成来实现高效、高质量的音频驱动嘴唇同步。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用重型GAN或扩散解码器重建整个下脸，导致高延迟并产生牙齿和唇纹等虚假口腔细节。问题根源在于缺乏保留真实纹理的机制，而非参考帧不足。

Method: 提出三个创新组件：1) 动态纹理混合器将多参考融合重新表述为通道选择，通过通道加权求和保留纹理完整性；2) 时空移位自适应遮罩将源帧分解为嘴唇生成条件和独立背景先验，抑制下脸泄漏；3) STAR Sampling零开销预处理检索最清晰、拓扑多样性最高的参考帧。

Result: 在HDTF和VFHQ数据集上实现最先进的视觉质量和身份保持，单GPU上达到166 FPS的实时性能。

Conclusion: EfficientSync通过基于变形的方法保留真实纹理而非重新合成，成功实现了高效、高质量的唇同步，为实时应用提供了可行方案。
...
TL;DR: EVADE是一种用于医学视觉语言模型的安全部署方法，通过区域定位、放大视图重新回答和跨视图一致性验证来提升模型的校准性和可靠性，在不进行额外训练的情况下同时改善校准和选择性风险。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型虽然准确率高但不可靠：过度自信、从测试时推理中获益甚少、缺乏可靠校准信任的能力。需要一种非训练方法来增强部署安全性。

Method: 提出EVADE方法：1)对不确定的区域进行诊断相关区域定位；2)在放大视图上重新回答；3)仅当整体图像和放大视图回答一致时才提交答案，否则弃权。通过跨不同图像视图验证一致性而非重新读取模型文本来解决验证幻觉问题。

Result: 在VQA-RAD、SLAKE和PathVQA数据集上使用Qwen2.5-VL-7B测试，EVADE是唯一同时改善校准和选择性风险且保持准确性的方法，将预期校准误差(ECE)降低高达45%。链式思维、自我一致性和自我验证都至少在一个指标上失败。

Conclusion: 7B VLM无法利用定位来修订答案，可靠性提升来自一致性门控和校准弃权。自我提议的区域在诊断结构定位上优于中心或随机裁剪，但一致性门控和校准弃权是可靠性提升的关键。
...
TL;DR: RVLoss是一种用于LiDAR场景流估计的自监督损失函数，通过运行投票机制将点态运动分组为少数主导流候选，并利用两阶段投票识别代表底层刚性运动的最优流，从而解决现有方法中物体实例内流不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自监督方法使用Chamfer损失，但最近邻搜索不强制运动刚性，导致物体实例内的流不一致，尤其对于大对象问题更严重。虽然已有方法引入正则化项，但流一致性仍然有限。

Method: 提出RVLoss，通过两阶段运行投票机制：(1)top-k投票将点态运动分组为少量主导流候选；(2)top-1投票识别最佳刚性运动流。基于此设计聚类刚性流和自由形式流作为自监督学习的伪标签，可无缝集成到现有前馈架构中。

Result: 在Argoverse2 2026挑战赛上达到自监督方法最优性能，比使用其他损失函数训练的基线模型提升20%。跨数据集评估显示在四个额外数据集上性能提升一致。

Conclusion: RVLoss通过引入运动刚性先验有效解决了点态流估计中的一致性问题，为自监督LiDAR场景流估计提供了新思路。
...
TL;DR: 本文探索了后训练强化学习(GRPO)来对齐开放词汇视觉定位模型的训练目标与评估指标，解决了传统监督微调中token级交叉熵与precision/recall指标不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 当前开放词汇视觉定位的自回归模型使用监督微调(SFT)训练，但优化的是代理目标(per-token交叉熵)，与感知指标(precision和recall)存在根本性的不对齐。

Method: 基于Falcon Perception模型，设计了针对感知任务的RL框架：1)针对集合结构化输出的奖励设计；2)多头采样控制；3)使用惩罚假阳性和假阴性的简单奖励函数；4)开发两种混合自标注管道分别针对困难指代表达和密集场景。

Result: 1) 在密集场景(最多500个物体/场景)达到最先进性能；2) 修复了mask重复问题；3) 几乎消除了对NMS和坐标去重的需求；4) 在PBench和SACO-Gold的指代表达分割任务上所有难度级别均有提升；5) 无需负样本训练即可保留物体存在性判断能力(MCC评估)。

Conclusion: 强化学习可以有效解决自回归感知模型的训练目标不对齐问题，在密集场景指代表达分割任务上取得显著提升，并简化了后处理流程。
...
TL;DR: 针对小规模图像分类中训练数据稀缺的问题，提出了学习状态感知的动态生成数据增强方法LSADA。该方法通过样本的当前损失和损失下降率构建学习状态，并映射到样本特定的增强强度。同时引入解耦的数据增强和扩散融合策略，对类相关区域进行强度控制的变换，为类无关区域生成多样化内容，在提高图像多样性的同时保持类别语义。


<details>
  <summary>Details</summary>
Motivation: 现有生成式数据增强方法存在三个主要局限：(1)采用任务无关的增强策略，忽视下游模型需求；(2)难以可靠地确定样本特定的增强强度；(3)无法同时平衡图像多样性和类别语义，尤其是对不同图像区域采用差异化增强策略。

Method: 提出LSADA方法，包含两个核心组件：(1)学习状态构建：为每个样本基于当前损失和损失下降率构建学习状态，并映射到样本特定的增强强度；(2)解耦数据增强与扩散融合策略：对类相关区域应用强度控制的变换，对类无关区域生成多样化内容，逐步融合以提升多样性同时保留类别语义。

Result: 在九个公共数据集上进行了实验，结果表明LSADA在六个自然图像数据集上平均超越现有最优动态GDA方法4.5%，在三个医学图像数据集上提升2.5%。

Conclusion: LSADA通过学习状态感知机制实现了样本自适应的动态生成数据增强，有效解决了现有方法无法平衡图像多样性与类别语义的问题，在自然图像和医学图像分类任务中均取得了显著性能提升。
...
TL;DR: Colorist是一种无需训练、完全可解释的数据增强策略，通过在RGB颜色空间中应用全局均值-标准差匹配来生成安全的域变化，提升医学图像分类器在硬件变化、颜色变化和患者特征变化下的鲁棒性，在多种医学影像数据集上比现有域泛化正则化方法提升高达9%的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 硬件设备变化、患者特征差异和颜色变化等因素导致在开发阶段训练的医学图像分类器在部署时性能下降。现有方法（如颜色抖动和深度生成风格迁移）存在多样性不足、特征幻想、破坏临床结构、计算资源浪费等问题。

Method: 重新利用经典统计颜色匹配方法，提出Colorist，在RGB颜色空间中直接应用全局均值-标准差匹配，无需训练过程，完全可解释，无需神经网络参与。

Result: 在域外 histopathology、peripheral blood、dermatology 和 retinal 数据集上，Colorist 比最先进的域泛化正则化方法提升高达 +9% 的平衡准确率，比未增强基线提升高达 +13%。在结构保真度和颜色对齐方面优于深度生成模型。

Conclusion: 统计颜色匹配是一种安全、可解释但被忽视的深度架构替代方案，能够保留解剖结构、最小化碳排放并无缝集成到标准数据加载器中，为临床鲁棒性提供了有效的解决方案。
...
TL;DR: 该技术报告介绍了一个开源的端到端管道"Institutional Books - Visual Elements"，用于从历史书籍集合中检测、分类、去重和标注视觉元素，并发布了包含从983,004册扫描书籍中提取的2260万个视觉元素的初始数据集。


<details>
  <summary>Details</summary>
Motivation: 历史书籍收藏包含丰富的视觉元素（插图、照片、版画、装饰艺术等），但这些元素在大规模数字化项目中通常未被充分探索。尽管OCR技术已标准化了文本内容提取，但这些视觉组件提供的细微差别和上下文内容仍无法被自动文本提取工作流程充分利用。

Method: 开发了一个端到端的开源管道，用于检测、分类、去重和标注历史书籍收藏中的视觉元素。

Result: 从哈佛图书馆Institutional Books数据集的983,004册扫描书籍中提取了2260万个视觉元素。

Conclusion: 这项工作为正在进行的社区努力做出了贡献，通过计算访问方式为数字化图书馆藏品启用新的用例，从人工智能模型训练到数字人文研究。
...
TL;DR: TRAIL是一种无需训练的AI图像编辑定位方法，利用冻结DINO编码器的patch级扰动响应进行编辑区域定位，在CocoGlide测试集上达到90.3% patch AUROC。


<details>
  <summary>Details</summary>
Motivation: 本地化图像编辑可以改变照片的含义但保留大部分真实内容，法医分析需要识别编辑发生的位置。

Method: TRAIL（Training-free Localization of AI-image Edits from patch-token Drift）应用全局Haar扰动，计算对应patch token之间的余弦漂移来生成定位图。

Result: 在80张CocoGlide测试图像上，TRAIL达到90.3% patch AUROC（对比监督方法Detective SAM的91.2%）；固定阈值Dice为61.9%对70.9%；迁移到泊松图像插值时达85.5% AUROC。16个DINO编码器中最佳块位于归一化深度0.80-0.94。全局上下文重要：AUROC从90.3%下降到局部扰动的85.7%和独立编码裁剪的73.5%。

Conclusion: 冻结DINO的patch token包含强烈的后层定位信号，其可见性取决于扰动类型和保留的上下文。
...
TL;DR: 提出SiConMo，一个轻量级语义分割框架，通过在瓶颈阶段整合局部和全局上下文来平衡准确性、效率和可扩展性。包含三个组件：Token Pyramid Extraction Module、Transformer-Branched Depthwise Convolution block和Feature Merging Module。在ADE20K、PASCAL Context、Cityscapes和COCO-Stuff数据集上实现最先进的准确率-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 语义分割需要在准确性、效率和可扩展性之间取得平衡。卷积网络善于建模局部模式但难以处理长距离依赖，而Vision Transformer虽能捕捉全局上下文但计算成本高。现有的工作主要集中在编码器设计，而瓶颈阶段（负责上下文聚合和信息流）相对被忽视。

Method: 提出SiConMo框架，包含三个组件：1) Token Pyramid Extraction Module用于分层多尺度表示；2) Transformer-Branched Depthwise Convolution block用于瓶颈感知上下文建模；3) Feature Merging Module在保留空间结构的同时增强语义一致性。核心设计原则是在极低计算预算下，瓶颈阶段是整合局部和全局上下文最有效的阶段。

Result: SiConMo在ADE20K、PASCAL Context、Cityscapes和COCO-Stuff数据集上实现了轻量级语义分割模型中最先进的准确率-效率权衡。

Conclusion: 简单性是一个强大的设计原则。在极低计算预算下，瓶颈阶段是整合局部和全局上下文的最佳位置，这为高效语义分割提供了新的设计思路。
...
TL;DR: 博物馆和档案数据集并不能真实反映历史艺术生产，而是体现了收集、保存、编目和数字化的偶然历史。这直接影响了对预训练图像表示的解释：它们看似编码了历史时间，但实际上编码的是物体作为数据可见的制度条件。作者将这种现象称为"时间纠缠"，并通过将艺术品年代测定构建为冻结图像嵌入上的不确定性感知回归任务来研究这一问题。


<details>
  <summary>Details</summary>
Motivation: 解决预训练视觉模型在艺术品年代测定任务中的可解释性问题，揭示模型所学到的时间知识是否真的反映了历史时间，还是仅反映了数据集偏差。

Method: 将艺术品年代测定建模为不确定性感知回归任务，在冻结的图像嵌入上进行处理，评估多种预训练视觉模型，并在Wikidata艺术品语料库上进行时间控制实验。

Result: 视觉-语言模型(VLM)优于纯视觉自监督基线模型，这些模型确实包含可用的时间信息。但定性分析表明，这种时间知识受到各种偏差的影响。

Conclusion: 预训练图像模型中的时间信息受到数据集偏差的塑造，模型学到的是制度条件而非真正的历史时间，需要更谨慎地解释视觉模型的时间推断能力。
...
TL;DR: X-LMC是首个用于自动评估脑膜侧支循环(Leptomeningeal collateral, LMC)评分的深度学习框架,从时间分辨的双平面DSA图像中自动预测ASITN/SIR分级。在134例M1段闭塞患者的多中心数据集中,该方法实现了QWK 0.398和二分类 macro-F1 0.711的性能,并与临床观察到的评分者间一致性(QWK: 0.314)相当。


<details>
  <summary>Details</summary>
Motivation: 目前临床LMC分级依赖人工视觉检查的ASITN/SIR量表,存在高度变异性。数字减影血管造影(DSA)是LMC评估的金标准,但手动评分效率低且主观性强。该研究旨在开发自动化工具以实现客观、可重复的侧支循环评估,为卒中治疗决策和预后分析提供标准化依据。

Method: 提出X-LMC框架:1)使用DINOv2骨干网络编码空间帧表示;2)通过token级交叉视图注意力模块融合正交投影;3)利用循环网络建模对比剂团注的时序动态。在134例M1段闭塞患者的多中心数据集上进行5折交叉验证评估。

Result: X-LMC在5折交叉验证中优于静态架构和时序基线方法,QWK达到0.398(基线0.322),二分类macro-F1达到0.711(基线0.663)。性能与临床观察到的评分者间一致性(QWK: 0.314)相当,表明该方法可捕捉侧支循环特异性对比剂动力学特征。

Conclusion: 首次实现了从DSA自动化LMC评分的深度学习方案,证明了多视图时序深度学习能够捕捉侧支循环特异性对比剂动力学。这些基准测试阐明了自动ASITN/SIR分级的临床模糊性和可达到的性能边界,为卒中队列中客观血流动力学表型分析建立了可重复的基础。
...
TL;DR: ForeSightGuide是一个前瞻性辅助导航框架，通过结合视觉语言模型的语义场景理解和预测性危险评估，为视障人士提供行动指引。该系统能够预测障碍物运动并过滤非威胁性物体，减少信息过载。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在动态场景中容易产生过多误报，导致认知过载。现有反应式系统无法满足视障人士安全独立出行的需求，需要一种能够预测危险的前瞻性辅助系统。

Method: 提出ForeSightGuide框架，利用视觉语言模型的推理能力预测障碍物运动，过滤非威胁性物体，提供简洁、可操作的指引。构建了复杂动态真实交通场景的新数据集用于基准测试。

Result: 在公共基准测试和自建数据集上均达到最先进性能。冗余警报降至0.299次/输出，漏报危险率为0.112，显著降低了信息过载。

Conclusion: ForeSightGuide能有效过滤非威胁性障碍物，提供简洁准确的引导，证明了其在安全行走辅助方面的有效性，为视障人士独立出行提供了有前景的技术方案。
...
TL;DR: 本文提出GrabVG框架，用于无人机航拍图像中的视觉定位任务。该方法通过模拟人类视觉搜索过程，将定位任务分解为预注意假设搜索和图注意力特征绑定两个阶段，有效解决了密集场景中的视觉冗余和拓扑歧义问题。


<details>
  <summary>Details</summary>
Motivation: 无人机航拍图像中存在大量小型、密集分布、视觉相似的物体，导致严重的视觉冗余；同时重复的局部配置引发强烈的拓扑歧义。现有方法主要关注视觉-语言特征对齐或密集上下文交互，难以区分细微的实例间差异并有效利用空间拓扑结构，导致密集场景中定位不准确。

Method: 提出GrabVG框架，包含两个顺序阶段：(1)预注意假设搜索：通过蒸馏引导的提议生成和文本感知的假设过滤，产生紧凑可靠的候选假设集，减少背景干扰和语义不匹配；(2)图注意力特征绑定：将假设组织为稀疏图，通过图注意力机制联合绑定语言引导的实例内视觉线索和实例间拓扑关系，实现高效空间推理和准确定位。

Result: 在AerialVG和AerialSense数据集上实验表明，GrabVG实现良好的精度-速度权衡，分别达到67.31%和80.34%的Acc@0.5准确率，较基线方法分别提升10.55和8.76个百分点。

Conclusion: GrabVG通过模拟人类视觉搜索机制，将复杂的视觉定位任务分解为假设搜索和特征绑定两个阶段，有效解决了无人机航拍图像中视觉冗余和拓扑歧义的挑战，在密集场景中实现了准确的视觉定位。
...
TL;DR: 本文提出Mise-en-Scène框架，通过两阶段方式实现从用户提供的元素自动合成图形设计。与传统方法不同，该方法让布局在预训练的图像编辑扩散Transformer中隐式生成，无需专门的布局规划器。


<details>
  <summary>Details</summary>
Motivation: 现有方法将布局预测与视觉合成分离，使用语言模型预测边界框坐标，再将元素粘贴到其中，导致组合生硬、比例失调。研究者希望探索布局能否在预训练的图像编辑扩散Transformer中隐式涌现。

Method: 第一阶段：使用小型knockout-selected LoRA适配的扩散Transformer生成完整设计，元素布局与渲染画布共同涌现。第二阶段：确定性匹配放置步骤将原始高分辨率资产移动到草稿位置，确保资产保真度并生成可编辑的分层设计。

Result: 在大型PrismLayersPlus基准测试中，Mise-en-Scène生成的设计在感知质量上最接近真实值，显著优于LLM布局规划器和专用布局Transformer。匹配放置阶段进一步弥合了与真实合成之间的保真度差距。

Conclusion: 对预训练Transformer的最小化适配已足够，无需为多元素生成引入专门的 conditioning 机制。该方法能够生成高质量、可编辑的分层设计，为图形设计自动化提供了新思路。
...
TL;DR: 本文是一篇关于自动驾驶中一阶段目标检测器的综合综述，涵盖YOLOv1、SSD、RetinaNet、EfficientDet、FCOS、CenterNet和YOLOv10等主流检测器，对比其架构设计、特征融合策略、损失函数、部署权衡和基准性能，并总结常用数据集、评估指标、开放挑战和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶依赖于快速可靠的目标检测系统来实时检测周围车辆、行人、骑行者、交通标志和其他道路目标，因此需要全面了解一阶段检测器的发展现状和技术特点。

Method: 采用综述方法，系统回顾主要一阶段检测器的演进历程，通过设计选择、特征融合策略、损失函数、部署权衡和基准性能等维度进行对比分析。

Result: 总结了一阶段检测器在速度、准确性、效率和鲁棒性之间的平衡方式，揭示了基准性能与实际可靠自动驾驶性能之间仍存在差距。

Conclusion: 一阶段检测器在自动驾驶感知系统中具有重要应用价值，但需要在实际部署中进一步提升性能和可靠性，以满足真实世界的自动驾驶需求。
...
TL;DR: 本文提出了一种稳定、无矩阵分解的全局协方差池化方法，通过多项式逼近矩阵对数来解决传统方法中因特征分解导致的数值不稳定问题，在细粒度识别任务上取得了更快的速度和更高的准确率。


<details>
  <summary>Details</summary>
Motivation: 全局协方差池化(GCP)通过捕获二阶特征统计量来提升深度网络的细粒度识别性能。由于协方差矩阵位于对称正定(SPD)流形上，需要进行归一化后才能使用欧几里得分类器。矩阵对数(MLN-COV)是正确的选择，因为它将SPD流形映射到切向空间；但由于其基于特征分解的梯度在数值上不稳定，实际中被矩阵平方根方法所取代。本文证明了这种不稳定性是源于特征分解的计算方式，而非对数本身。

Method: 使用有限多项式逼近对数，避免特征分解：1)均值特征值预归一化，使谱集中在1附近，远离对数奇点；2)标量后补偿，以闭式形式返回对数的奇异部分；3)推荐使用8次切比雪夫展开，通过三项矩阵递推求值，匹配反向递推用于反向传播。同时研究了Legendre、Laguerre、Taylor和Pade展开作为对照。

Result: 在三个细粒度基准数据集和ImageNet-1k上，无分解的对数方法比特征分解对数方法和其替代的平方根近似方法都更快且更准确。在相同基函数和次数条件下，对数目标函数优于平方根目标函数，证实了收益来自忠实的黎曼映射而非更好的多项式族。

Conclusion: 矩阵对数作为SPD流形到切向空间的忠实映射，其本身的数学性质优于平方根近似。数值不稳定性的根源在于特征分解的计算方式，而非对数映射本身。通过多项式逼近结合预归一化和后补偿策略，可以实现稳定且高效的矩阵对数计算，从而提升细粒度识别性能。
...
TL;DR: 提出一种基于对比分析（CA）的无分类器视觉反factual生成方法，通过解耦不同数据集之间的共同生成因素和显著因素来生成反事实图像，利用StyleGAN2的F空间改进细节保留。


<details>
  <summary>Details</summary>
Motivation: 现有VCE方法依赖分类器，容易受到分类器偏见和失败模式的影响（如对快捷特征的敏感性和校准误差），需要一种模型无关的方法来减少这些偏见。

Method: 基于对比分析的无分类器方法，解耦两个数据集（如健康和患者）之间的共同生成因素和显著因素，通过交换显著因素生成反事实图像。利用StyleGAN2的高质量合成和良好结构化的潜在空间，使用F空间而非W空间来改进细节保留，并引入针对VCE的适配框架和损失函数。

Result: 在三个医学影像数据集上评估，展示了比现有方法更优的反事实生成质量。

Conclusion: 成功提供了模型无关的VCEs，对分类器偏见不那么敏感，通过直接操作数据分布而非决策边界来实现反事实生成。
...
TL;DR: USR-Drive是一个统一的条件生成框架，给定带姿态的多视角驾驶视频，联合恢复密集动态几何和实例级物体布局。该方法将高斯原语和3D边界框表示为对齐的潜在标记流，通过统一的多模态扩散Transformer进行联合去噪。


<details>
  <summary>Details</summary>
Motivation: 现有方法将动态重建和实例级感知视为独立任务，导致动态重建缺乏约束，而3D检测缺乏几何基础。两者都旨在估计底层3D世界状态，但长期被割裂处理。

Method: 提出统一条件生成框架，将密集高斯原语和稀疏3D边界框表示为两个对齐的潜在标记流，使用统一的多模态扩散Transformer联合去噪。引入统一位置编码(UPE)将异构标记对齐到共享的度量时空坐标中。

Result: 在nuScenes和VKitti数据集上，动态重建和3D检测均达到最先进水平。几何为边界框预测提供密集度量证据，边界框提供实例级结构先验，帮助保持空间一致性并减少顺序3D几何表示的歧义。

Conclusion: 通过统一表示和生成框架，两种模态相互强化：几何提供密集度量证据支持框预测，框提供实例级结构先验帮助保持空间一致性。该方法成功实现了动态重建和3D检测的统一。
...
TL;DR: 本文提出了一种基于扩散模型的音乐驱动鼓手动画生成框架，通过双目标损失函数解耦骨骼完整性与鼓棒精度，实现厘米级鼓棒精度且保持自然身体动态，并提出两个新评估指标（impact-to-target distance和audio-motion correlation score）来衡量空间精度和时间对齐。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在从真实音频生成逼真鼓手动作方面的挑战：现有方法依赖动作匹配或MIDI输入，难以泛化到多样化的真实世界音频；同时缺乏能够区分精确鼓手动作与噪声动作的标准化评估指标。

Method: 1) 提出生成式扩散框架；2) 设计双目标损失函数解耦骨骼完整性与鼓棒精度；3) 构建自定义数据集并采用数据增强策略实现对非整理真实音频的泛化；4) 提出两个新评估指标：impact-to-target distance（空间精度）和audio-motion correlation score（时间对齐）。

Result: 定量分析和用户研究表明，系统生成的动作质量高，常常与真实表演难以区分。

Conclusion: 该框架成功实现了高精度鼓手动作生成，平衡了骨骼自然性与鼓棒精确性，为音乐驱动角色动画领域提供了新思路和评估标准。
...
TL;DR: 研究评估了PSMA和FDG PET/CT多模态影像融合的深度学习策略，用于前列腺癌全身肿瘤负荷分割。使用DEEP-PSMA数据集，比较了早期融合(OETD、OEOD)和中间融合(DECA-UNet)方法，发现Tracer特异性的单模态模型表现更强(PSMA Dice=0.93, FDG=0.81)，融合方法未能一致超越单追踪剂基线。


<details>
  <summary>Details</summary>
Motivation: PSMA和FDG PET/CT能捕获互补的生物学信息，但目前缺乏有效的深度学习架构来融合这些模态。研究旨在评估自动全身PET/CT病变分割的多模态影像融合策略，以估计总体肿瘤负荷。

Method: 使用公开的DEEP-PSMA Challenge数据集，训练Tracer特异性的3D nnU-Net基线模型，比较早期融合(单编码器单解码器OEOD、单编码器双解码器OETD)和中间融合(双编码器交叉注意力U-Net DECA-UNet)策略。

Result: Tracer特异性基线表现强劲(PSMA Dice=0.93; FDG=0.81)。融合结果混合：OEOD在较简单的非Tracer特异性任务上产生组合Dice=0.90，而Tracer特异性融合模型分别达到PSMA/FDG=0.69/0.64(OETD)和0.76/0.57(DECA-UNet)。

Conclusion: 虽然融合通常提供合理的PSMA分割，但FDG性能下降，且没有策略一致超越单追踪剂基线。在评估设置下，Tracer特异性模型仍是更强的基线；临床上多模态融合的获益可能需要能更好保留Tracer特异性表示的架构。
...
TL;DR: 提出了一种名为GS-VLA的轻量级即插即用框架，通过3D高斯新颖视角合成技术，在不重新训练策略的情况下，提高视觉-语言-动作(VLA)策略对视点变化的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前VLA策略隐含假设训练和部署时的相机配置相同，但即使相机安装位置的小幅移动也会导致成功率大幅下降（例如LIBERO基准从90%降至10%）。先前的方法如大规模微调或生成式数据增强计算成本高且存在灾难性遗忘风险。

Method: 将视点变化重新表述为局部新颖视角合成问题。在局部性假设下（相机扰动相对于工作空间保持在小边界区域内），视点归一化简化为与场景和策略无关的遮挡任务。实现上使用4M参数的3D-Gaussian规范化器前置到冻结的VLA策略中，无需修改策略权重。

Result: 在三个正交轴上提升性能：(1)策略架构 (2)未见任务套件 (3)扰动规模。GS-VLA能够恢复视点变化下损失的很大一部分性能，且无需策略重新训练。

Conclusion: 轻量级视觉模块可以在不进行策略重新训练的情况下，有效恢复因视点变化而损失的大部分性能，为VLA策略的视角鲁棒性提供了一种高效可行的解决方案。
...
TL;DR: 本文提出ReWEIGH，一种无需训练的解码干预方法，通过聚合视觉token的词汇等级来减少LVLMs的幻觉。在四个7B模型上减少高达21.3%的幻觉目标提及，同时保持或提升描述性和整体性能，平均延迟仅增加1.33%。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)经常产生幻觉，生成输入图像不支持的内容。现有的解码方法缺乏针对候选token的图像支持程度进行度量的机制，因此需要一种方法来评估视觉信息对每个生成token的支持强度。

Method: 提出ReWEIGH方法：1)利用模型的视觉token状态，通过输出头投影揭示每个位置偏好的词汇项；2)使用词汇等级作为可跨视觉位置池化的尺度不变基准；3)从无标签图像估计token-specific参考；4)在推理时缓存预填充期间的图像证据，仅对低于参考的候选应用有界惩罚。

Result: 在四个7B backbone上，ReWEIGH将幻觉目标提及减少高达21.3%，同时基本保持或提升描述性和一般性能。证据缓存后，平均每token增加延迟1.33%，且该方法在六个架构家族、达32B参数上均有效。

Conclusion: ReWEIGH是一种有效的训练-free解码干预方法，通过token-specific的视觉证据聚合来减少LVLMs幻觉，具有低延迟开销且可扩展到更大模型和多种架构。
...
TL;DR: 研究基础模型适应策略对医学成像子群公平性的影响，发现更丰富的表示可以提高准确率，但公平性影响因任务而异，无法从编码强度或整体性能推断。


<details>
  <summary>Details</summary>
Motivation: 基础模型越来越多地用于下游医学成像任务，但适应策略对子群公平性的影响尚不明确。

Method: 使用三种参数高效适应技术（CLS token线性头、MLP、注意力池化模块）应用于冻结的Rad-DINO胸部X光编码器，在MIMIC-CXR数据集上评估八种病理，按种族、性别、成像视角子群分析，同时探测各适配器对受保护属性的编码强度。

Result: 注意力池化获得最佳整体性能并最强地编码属性（尤其是种族），但更好的整体性能并不一致地减少子群差异；更强的属性编码并不对应更大的差异——早期网络层编码种族最弱却产生最大子群性能差距；不同注意力池化层组合与属性编码强度和子群公平性之间无一致关系。

Conclusion: 更丰富、更有表现力的表示可以提高准确率，但公平性影响是任务依赖和不可预测的，必须直接评估而非从编码强度或整体性能推断。
...
TL;DR: 本文提出SPK（Structured Prior Knowledge）框架，通过从预训练目标检测器中显式提取和利用隐含的先验知识来检测分布外（OoD）幻觉问题。SPK利用诊断监督信号提取目标检测器决策过程中的部分级语义概念，并结合几何和上下文先验形成紧凑的五维表示用于OoD检测。


<details>
  <summary>Details</summary>
Motivation: 目标检测器对其训练类别之外的物体经常产生过度自信的预测，导致所谓的OoD幻觉。现有的方法要么直接在学习的表示上构建评分函数，要么修改检测器本身来抑制幻觉产生，但这些表示中隐含的先验知识从未被显式解码用于OoD检测。

Method: SPK框架首先利用分布内数据和诱导幻觉的样本作为诊断监督信号，提取目标检测器决策过程中的部分级语义概念。然后将这些提取的语义先验与几何和上下文先验整合，形成紧凑的五维SPK表示用于OoD检测。

Result: 在多种目标检测器架构和多个OoD基准数据集上的广泛实验表明，SPK实现了最先进的OoD检测性能。

Conclusion: 预训练目标检测器编码的潜在知识比通常用于OoD检测的知识要丰富得多。更重要的是，这些知识可以被显式提取并组织成紧凑、结构化和可解释的知识空间，用于预测可靠性分析。这为通过显式挖掘和利用潜在先验来提高目标检测器可靠性提供了一条有前景的主动式路径。
...
TL;DR: DistScan是一种基于分布偏移的后门检测框架，用于检测目标检测模型中的后门攻击。该方法利用了后门注入会使模型预NMS预测类别分布偏离训练类别频率这一观察，无需模型权重访问、触发器知识或额外训练即可检测后门模型。


<details>
  <summary>Details</summary>
Motivation: 安全关键应用中部署的目标检测模型容易受到后门攻击。现有的检测方法依赖于触发器反演或特定架构假设，且无法有效泛化到场景级攻击（单个触发器同时影响场景中所有对象）。因此需要一种无需触发器知识、模型权重访问或额外训练的后门检测方法。

Method: DistScan通过在干净验证集上聚合中间类别预测来工作。核心观察是：后门注入会系统性地使模型的预NMS预测类别分布偏离训练类别频率，即使在无触发器的干净输入上也是如此。如果产生的分布与训练类别频率显著偏离，则标记模型为后门模型。

Result: 在MS-COCO和PASCAL VOC数据集上，针对两种架构和三种场景级攻击场景进行广泛实验表明，DistScan显著优于现有方法，平均检测准确率比最佳基线提高27.32个百分点。

Conclusion: DistScan是一种有效的目标检测模型后门检测方法，无需触发器知识、模型权重或额外训练即可工作。该方法特别擅长检测现有方法难以应对的场景级攻击，为安全关键应用中的模型审计提供了实用工具。
...
TL;DR: 本研究提出了一种针对公路路面检测的3D探地雷达(GPR)深度学习解决方案，包括低成本的数据标注pipeline和专用的3D CNN架构，用于路面缺陷（补丁和裂缝）的自动检测。


<details>
  <summary>Details</summary>
Motivation: GPR在路面检测中应用面临两大挑战：1）标注的真实世界数据集稀缺；2）缺乏针对3D GPR数据特点设计的深度学习模型。

Method: 1. 提出数据准备pipeline：整合正射RGB图像与3D GPR扫描，利用路面表面图像作为参考，将表面可见缺陷的标签转移到对应的GPR段落，实现大规模自动标注；2. 设计专用3D CNN架构：包含残差连接、混合卷积核尺寸，以及depthwise和channelwise注意力机制，提升特征表示和缺陷分类能力。

Result: 在二元分类任务（检测路面结构和裂缝缺陷）中，所提网络在多个评估指标上优于基线架构。消融实验验证了所设计组件的有效性。

Conclusion: 本研究贡献了一个可扩展的实用真实世界数据集生成方法，以及一种新型深度学习框架，为GPR路面检测的规模化应用提供了技术基础。
...
### [84] [DentAgent: Evidence-Centric Multi-Agent Coordination for Multimodal Dental Reasoning](https://arxiv.org/abs/2608.18878)
*Zijie Meng,Xiwei Dai,Yixuan Tang,Jin Hao,Yang Feng,Fudong Zhu,Xiaoqiang Liu,Shaosheng Cao,Zuozhu Liu*

Main category: cs.AI

TL;DR: DentAgent是一个以证据为中心的多智能体框架，通过协调五个专业智能体（覆盖不同模态）将异构牙科数据转化为结构化证据记录，实现可追踪的多模态牙科推理，在四个基准测试中表现领先，在多标签诊断任务上超越高级专科医生17.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 全球口腔疾病患者数以十亿计，现有牙科AI系统仍局限于单一模态或单一任务；虽有视觉-语言模型支持灵活的牙科问答，但其直接生成的响应缺乏可追溯的证据链。

Method: DentAgent框架包含：1）Orchestrator协调器负责调度；2）五个专业智能体分别处理不同模态的牙科数据（X光片、口腔照片、3D数据等）；3）Evidence Blackboard证据白板管理结构化证据记录，跟踪覆盖范围、空白和冲突；4）最终基于标准化证据表示生成响应。

Result: 在四个基准测试中取得领先性能；在多标签诊断任务上超越高级专科医生17.3个百分点。

Conclusion: DentAgent成功将孤立的牙科能力整合为统一的智能体工作流，实现了可追溯的多模态牙科推理，有望成为人群口腔健康评估与管理的技术基础。
...
### [85] [Position: Collusion Risks Among AI Reasoning Agents Justify Certification Requirements for Making Market Decisions](https://arxiv.org/abs/2608.18078)
*Matthew Riemer,Tommaso Tosato,Amin Memarian,Maximilian Puelma Touzel,Glen Berseth,Irina Rish,Guillaume Dumas*

Main category: cs.AI

TL;DR: 本文论证了具有思维链推理能力的AI代理容易表现出串通(collusion)行为,在部署到现实市场之前需要进行行为认证。实验表明DeepSeek-R1代理在伯川德寡头定价环境中存在默示串通倾向,且即使被要求不串通也无法改变。思维链可被引导至极端串通或高度竞争行为,且这种引导无法被LLM语义检测。因此需要基于可观察行为进行认证以防止串通,但开发全面认证机制仍是未来挑战。


<details>
  <summary>Details</summary>
Motivation: 作者认为将具备思维链推理能力的AI代理整合到社会经济中,可能会摧毁独立企业之间"竞争"与"串通"的法律证据区分,同时却不消除经济危害区分。这会导致在没有任何共谋证据或意图的情况下产生串通的经济后果,需要建立认证机制来预防。

Method: 使用DeepSeek-R1代理在伯川德寡头定价(Bertrand oligopoly pricing)领域进行实验,观察其行为模式;同时使用另一LLM分析思维链推理轨迹,检测是否存在语义可识别的引导。

Result: 实验揭示DeepSeek-R1代理存在默示串通倾向,且该倾向即使在人类明确提示不要串通时仍会持续。更关键的是,思维链可被引导至极端串通或高度竞争行为,但这种引导无法被另一LLM通过语义分析检测出来。

Conclusion: 部署推理代理进行市场决策会导致串通经济后果却无任何共谋证据,因此在代表性子情境中基于可观察行为进行认证是必要的。初步证据表明此类代理可被引导至高效竞争均衡,但在模型部署到真实市场前,仍需开发全面的行为认证机制以确保其稳定性和效率。
...
### [86] [Adaptive Memory and Reflection Multi-Agent System for Medical Question Answering](https://arxiv.org/abs/2608.19029)
*Pradeep Murugesan,Luoxiao Yang,Xueli Chen,Xinqi Fan*

Main category: cs.AI

TL;DR: 本文提出了一种自适应记忆与反思（AMR）智能体系统，用于医疗问答任务。这是一个多智能体框架，专门的智能体使用专用记忆和基于反思的反馈来检索相关先前案例并改进后续推理。


<details>
  <summary>Details</summary>
Motivation: 现有的医疗问答系统通常基于单智能体架构和静态检索，缺乏适应性、持久记忆和结构化决策能力，难以满足医疗领域复杂病例需要事实知识和细致推理的要求。

Method: 该框架包含：1）复杂度评估模块，根据问题复杂度选择单独、协作或升级工作流；2）专门的智能体使用记忆和反思机制；3）共识模块整合推理；4）伦理监督模块审查输出。在MedQA和MedMCQA数据集上进行评估。

Result: 在MedQA和MedMCQA数据集上表现优于多个基线模型。消融实验表明，结合智能体特定记忆、反思和外部检索能获得最佳性能。

Conclusion: 结构化记忆和反馈机制有助于开发更可信的医疗智能体系统。
...
### [87] [Position: Profiling Game Worlds by Transition Complexity](https://arxiv.org/abs/2608.18079)
*Lele Cao*

Main category: cs.AI

TL;DR: 论文提出Transition Complexity Profile (TCP)，用于量化游戏环境和游戏数据集的转移核复杂度，包括一阶分支、交互不确定性、时间/空间依赖跨度的标准化度量，旨在使GWM和RL研究中的环境复杂度成为可比较的标准化指标。


<details>
  <summary>Details</summary>
Motivation: 当前GWM和RL研究混淆不清，因为论文很少量化声明接口（像素/标记/潜变量及有限历史）下转移预测问题的难度，缺乏可比较的标准化度量。

Method: TCP包含三个核心指标：(i)内在一阶分支度，(ii)可观测时的交互诱导不确定性和对手影响力，(iii)通过标准化探测曲线表征时间/空间依赖跨度。报告时附带显式参考分布、协议随机性和版本化测量预算（采样/重采样及固定探测计算）。

Result: 概述了常见游戏家族和现代"神经游戏引擎"领域如何填充TCP景观，使跨基准的数值具有可比性。

Conclusion: 呼吁TCP成为标准基准元数据以及GWM和RL论文中的必需统计量，推动领域对环境复杂度的统一量化标准。
...
### [88] [Large Language Models in Mental Health: A Systematic Review of Applications, Innovations, and Ethical Challenges](https://arxiv.org/abs/2608.18080)
*Yisong Chen,Yifan Gao,Sijing Yu,Chuqing Zhao,Yang Lu*

Main category: cs.AI

TL;DR: 这是一篇关于大型语言模型（LLMs）在心理健康领域应用的综述论文，涵盖社交媒体分析、临床对话代理、治疗支持工具等技术应用，以及多模态融合和伦理监管挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs技术的快速发展，其在心理健康领域的应用潜力巨大，但缺乏系统性的综述来整合跨学科研究成果，为临床实践提供指导。

Method: 采用跨学科综述方法，整合多种数据来源（社交媒体、电子病历、多模态输入），分析LLMs在抑郁症早期检测、自杀风险评估、个性化治疗支持等方面的应用。

Result: 综述展示了LLM模型和注释策略的进展如何增强可解释性和临床相关性，提示工程如何支持领域适应，以及多模态融合技术如何改进心理健康诊断和监测。

Conclusion: 强调了确保LLMs在现实心理健康护理中安全、公平和负责任部署的伦理、社会技术和监管框架的必要性。
...
### [89] [Position: Behavioral Systems Require Behavioral Tests](https://arxiv.org/abs/2608.18081)
*Manuel Cherep,Nikhil Singh,Pattie Maes*

Main category: cs.AI

TL;DR: 本文主张AI智能体的评估应从单纯关注性能结果转向系统性观察、扰动和解释其行为过程，借鉴行为科学的方法论，提出开发严格行为测试的研究议程。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体评估方法主要关注性能结果，而非产生这些结果的行为过程。这种方法论的局限性在于无法理解智能体如何实现目标、适应环境，也无法预测其在新情境中的行为。

Method: 提出三种行为测试方法：1)从动作序列中恢复决策策略；2)构建可隔离行为差异的环境；3)探测多智能体系统中的涌现动力学。

Result: 该研究议程为开发AI行为科学提供了路线图，强调需要像研究其他行为系统一样研究AI智能体，通过系统观察、扰动和解释来理解其行为。

Conclusion: AI智能体评估需要范式转变，从结果导向转向过程导向，借鉴行为科学方法论，建立严格的AI行为测试体系，最终实现AI行为科学的建立。
...
### [90] [Position: Current Model Cards Are Insufficient for Downstream Governance of Open-Weight Foundation Models](https://arxiv.org/abs/2608.18086)
*Sungwon Chae,Keonwoo Kim,Hoki Kim,Jaeyeon Ju,Sangchul Park*

Main category: cs.AI

TL;DR: 随着开源基础模型(OWFMs)的增长，现有模型卡片难以有效传达其安全挑战。本文通过对Hugging Face上500张模型卡片进行分析，提出多层次治理框架需整合模型卡片、可接受使用政策(AUPs)和许可证三种互补组件。


<details>
  <summary>Details</summary>
Motivation: 开源基础模型的增长促使AI社区重新评估下游治理策略。现有的模型卡片框架未能充分告知下游开发者和用户OWFMs特有的安全挑战，存在监管空白。

Method: 分析Hugging Face上500张模型卡片，识别安全关键信息缺口，评估标准开源许可证对OWFMs的适用性。

Result: 发现现有监管方法存在安全缺口，包括模型血缘、对齐溯源和经验观察行为等方面信息缺失。标准开源许可证不适用于OWFMs，可能削弱AUPs的可执行性。

Conclusion: 需要将模型卡片、AUPs和许可证整合为综合安全工件，形成整合信息性、规范性和法律性维度的更全面治理框架。
...
### [91] [A Metamorphic Artificial Age Score Decision-Support Prototype for Flight-Log-Based Drone Propeller Health Monitoring](https://arxiv.org/abs/2608.18088)
*Seyma Yaman Kayadibi*

Main category: cs.AI

TL;DR: 本文提出了一种基于飞行日志的无人机螺旋桨健康监测决策支持原型——变形人工年龄分数(AAS)，通过计算6个健康相关指标评估螺旋桨故障影响，支持维护优先级排序和自主系统监管。


<details>
  <summary>Details</summary>
Motivation: 无人机螺旋桨故障的影响通常分布在多个飞行日志通道中，而非表现为单一诊断信号，现有多指标决策支持方法不足，需要开发能够综合评估多通道故障效应的健康监测框架。

Method: 使用2024 DronePropA公共数据集中的历史真实飞行日志，从原始MATLAB矩阵中提取6个健康相关指标：轨迹跟踪误差、姿态不稳定、推力指令负担、电机指令不平衡、ESC指令不稳定和电池电量应力。相对于健康基线进行归一化，通过候选评分策略、变形充分性关系和冗余调整后的AAS公式进行评估。

Result:  Severity 1案例以ESC指令不稳定为主，分配至维护审查；Severity 2案例达到最大电机指令和ESC指令负担；Severity 3案例达到最大轨迹跟踪误差；后两者均触发强制检查。螺旋桨故障效应可通过不同操作通道显现。

Conclusion: 螺旋桨故障效应可能通过不同操作通道显现，支持建立多指标决策支持层用于飞行后维护优先级排序和自主系统监管，AAS可作为结构化策略充分性和负担度量而非时间年龄度量。
...
### [92] [Position: Multi-Agent Systems Should Prioritize Concurrency Control](https://arxiv.org/abs/2608.18092)
*Xin Yang,Letian Li,Zimo Ji,Terry Jingchen Zhang,Wenyuan Jiang*

Main category: cs.AI

TL;DR: 本文认为LLM多智能体系统(MAS)的许多失败本质上是由并发控制问题导致的，而非传统的协调或通信问题。


<details>
  <summary>Details</summary>
Motivation: 当前MAS框架在增加智能体时可靠性反而下降，需要找出根本原因并提出解决方案。

Method: 将MAS中的失败模式映射到经典的并发异常，论证需要显式并发控制机制。

Result: 发现许多被归因于协调或通信问题的失败，实际上是并发控制问题：智能体并发读写共享状态，长LLM推理窗口放大 stale reads、lost updates 和不一致结果的风险。

Conclusion: 提出MAS框架应内置并发控制机制（冲突检测、隔离保证、结构化资源访问），并发控制应成为首要设计考量而非事后补救。
...
### [93] [FinSkillBench: Evaluating AI Agents and Domain Skills for Investment Management](https://arxiv.org/abs/2608.18099)
*Jermyn Zhen Yong Bek,Zhuang Qiang Bok,Zhongtian Sun*

Main category: cs.AI

TL;DR: 本文介绍了FinSkillBench评估套件，用于衡量语言模型智能体是否能够有效运用金融领域技能解决投资管理任务。该基准涵盖投资组合构建、风险管理和基本面分析三个领域，包含12个子任务和2,603个任务 episodes。研究比较了无技能、策划技能包和自生成技能三种条件，发现策划技能包能将平均分数从0.366提升至0.528，而自生成技能几乎无益。


<details>
  <summary>Details</summary>
Motivation: 投资管理是一个高风险领域，智能体AI系统不仅要生成合理的文本，还需要检索时间点数据、组装正确的计算输入、调用专业方法并产生可审计的结构化输出。现有基准无法评估智能体使用金融领域技能的能力，因此需要构建专门的评估套件。

Method: 构建了包含三个领域（投资组合构建、风险管理、基本面分析）、12个子任务的FinSkillBench评估套件，共2,603个任务episodes。每个episode提供时间点输入、隐藏的真实值和任务特定验证器。对9个模型进行大规模评估，比较三种条件：无技能、策划技能包（包含程序文档和可执行组件）、自生成技能（智能体自行编写并在episode内重用程序）。此外使用独立智能体框架（Hermes Agent，8个模型，5,280个episodes）进行验证。

Result: 策划技能包持续提升性能，平均分数从0.366提升至0.528，在投资组合构建和风险管理领域提升最大。相比之下，自生成技能几乎没有收益，尽管计算成本更高。独立评估验证了各领域技能效果的方向性，但效果幅度因任务和测试工具不同而异。

Conclusion: 在投资管理智能体中，获得可靠的程序化技能访问可能与模型选择同等重要，而 naive的自生成技能通常无效。策划技能包是提升智能体性能的关键因素。
...
### [94] [Self-Evolving Agents as Dynamic Graph Transformation: A Survey and New Perspective](https://arxiv.org/abs/2608.18104)
*Yuanyuan Xu,Wenjie Zhang,Yin Chen,Xuemin Lin,Ying Zhang*

Main category: cs.AI

TL;DR: 该论文是一篇关于自演化代理与动态图结合的综述，将代理状态建模为动态图，将代理演化形式化为动态图转换，并提出了统一的分类框架。


<details>
  <summary>Details</summary>
Motivation: 现有研究将图作为代理功能的辅助结构，而非动态演化基质；自演化代理研究关注代理级别机制，却很少讨论图拓扑演化。两者的耦合关系——动态代理状态与动态图拓扑之间的联系——尚未被充分探索。

Method: 将代理状态建模为动态图（记忆、工具、技能、工作流和代理间关系表示为带类型的节点、边和子图，通过模式约束的重写进行更新），将现有基于动态图的自演化代理方法组织为四个分类：节点/特征演化、边/拓扑演化、子图激活、跨组件协同演化。

Result: 提出了动态图学习作为自演化代理的可复用基础设施，将九个动态图学习子领域映射到代理演化能力，并讨论了其适应性和可能的失败模式。同时从动态图视角提出了五种图感知的评估和治理协议。

Conclusion: 为自演化代理的设计和治理提供了一个紧凑的结构化视角，连接了动态图学习与自演化代理两个研究领域。
...
### [95] [Emergence of Agentic AI: A Review on Evolution, Background, Working Principles, Applications, Adoption Factors, and Future Research Directions](https://arxiv.org/abs/2608.18110)
*AKM Bahalul Haque,Al Amin Islam Ridoy,Mohammad Rayhan,Ivan Porres*

Main category: cs.AI

TL;DR: 本文是对Agentic AI（代理人工智能）的综合系统性综述，涵盖其基本原理、架构、工作原理、功能、实际应用、当前挑战及未来研究方向，并提出了一个利益相关者采用Agentic AI的意愿框架。


<details>
  <summary>Details</summary>
Motivation: 1) Agentic AI正在人工智能领域获得新的进展和进步；2) 需要更深入地理解和掌握该技术；3) 需要调查Agentic AI的最先进研究方向；4) 需要全面评估改进和应用的潜在范围

Method: 系统性综述方法：1) 综合考虑近期在各个领域发表的学术贡献；2) 讨论Agentic AI的基本原理和工作原理；3) 追踪代理在人工系统中的历史和理论演变；4) 探索和讨论Agentic AI的架构、工作原理和功能；5) 探索Agentic AI在各领域的实际应用；6) 分析研究发现；7) 识别当前挑战；8) 讨论未来研究方向；9) 提出利益相关者使用和采用Agentic AI的意愿框架（借助系统质量维度）

Result: 提供了对Agentic AI的全面理解，涵盖其当前发展、应用、关键研究空白和未来研究方向

Conclusion: 该系统性综述为研究人员和从业者提供了对Agentic AI的全面理解，突出了关键研究空白，并概述了未来研究方向
...
### [96] [Solving Is Not Drawing: A Benchmark for Diagrammatic Reasoning in Olympiad Geometry](https://arxiv.org/abs/2608.18111)
*Hsien Xin Peng,Anthony Kim,Alvin Li,Calvin Supasanya,Shivank Garg,Kevin Zhu*

Main category: cs.AI

TL;DR: 研究揭示当前基础模型在几何图表构建方面存在显著不足，平均编译成功率仅36.14%，证明数学推理能力不能自动转化为准确的图表绘制能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如MathVista和MathVerse）仅测量模型是否能得出正确答案，但未考虑几何图表构建这一独立能力。解决几何问题和绘制相关图表是两种不同的技能，而模型能否在推理出答案的同时生成准确的图表尚不明确。

Method: 构建了一个包含954道奥林匹克几何问题的基准数据集，其中包含297道困难子集，每个问题都配有解决方案和人类编写的高保真Asymptote代码图表。开发了一套基于文本、代码、图像、VLM和约束的度量标准来评估图表推理能力。

Result: 评估显示模型在解决问题与绘制图表之间存在明显差距：图表的平均编译成功率仅为36.14%。这表明强大的数学推理能力并不意味着能够构建准确的几何图表。

Conclusion: 当前基础模型在数学推理与图表绘制之间存在显著差距。强大的数学推理能力并不能自然转化为准确的几何图表构建能力。这一发现揭示了图表绘制作为数学推理中一项独立且尚未被充分测量的重要能力。
...
### [97] [Position: AI Leaderboards Are Underserving the Global South: A Case Study from India](https://arxiv.org/abs/2608.18117)
*Sourav Banerjee,Saikat Saha*

Main category: cs.AI

TL;DR: AI leaderboards are structurally biased against the Global South due to institutional design flaws (缺乏独立治理、利益冲突政策和指标演进机制)，而非数据缺失。已有高质量区域基准数据集存在，但全球 leaderboard 拒绝纳入。解决方案是建立具有独立治理的区域 leaderboard。


<details>
  <summary>Details</summary>
Motivation: 解决 AI 领域对全球南方的系统性忽视问题，揭示全球 AI leaderboard 缺乏治理机制导致印地语、斯瓦希里语或阿拉伯语使用者的需求长期被忽视的现状

Method: 以印度为案例研究（14亿人口，22种预定语言，高质量基准但缺乏可信聚合），对58位AI从业者进行咨询调查

Result: 58位AI从业者一致倾向于正式治理和基于披露的利益冲突管理；现有全球leaderboard不包含区域基准且无治理机制迫使它们这样做

Conclusion: 解决方案不是更多数据，而是更好的制度：從一開始就建立具有獨立治理的区域 leaderboard
...
### [98] [Safety Alignment Illusion: The Cross-Lingual Safety Gap in LLMs](https://arxiv.org/abs/2608.18131)
*Namya Bhatnagar*

Main category: cs.AI

TL;DR: 本文介绍了INCLUDE评估基准，用于衡量大型语言模型对印度社会文化偏见的检测能力。该基准包含2,604个提示，涵盖六种语言（英语、印地语、孟加拉语、马拉地语、泰米尔语和印地英语混合语），评估了10个开源和闭源LLM模型，共计14,988个偏见分数。


<details>
  <summary>Details</summary>
Motivation: 当前LLM的安全对齐训练严重以英语为中心，当安全过滤器对非英语语言失效时，会导致语音助手和对话系统产生刻板印象强化输出，绕过标准英语安全对齐，向非英语社区传播有害偏见。这对于印度这样的多语言国家来说是一个关键性问题。

Method: 构建了包含2,604个提示的多语言评估基准INCLUDE，涵盖六种语言；评估了10个开源和闭源LLM，分析了14,988个偏见分数。

Result: 两个关键发现：1）孟加拉语在开源模型中产生最高平均偏见分数；2）英语表现出明显逆转现象，在开源模型中偏见最低，但在闭源模型中偏见最高。

Conclusion: LLM的偏见存在显著的语言差异，开源和闭源模型在不同语言上的偏见表现不同，需要针对非英语语言开发专门的安全对齐方案。
...
### [99] [Optimized Fuzzy Logic Approach with the IEEE Key Gas Method for Diagnosing Power Transformer Faults Using Dissolved Gas Analysis](https://arxiv.org/abs/2608.18133)
*Kim-Anh Nguyen,Huy Hoang Le,Ba Tu Phung*

Main category: cs.AI

TL;DR: 本文提出了一种结合模糊逻辑与IEEE关键气体方法的增强型变压器故障诊断模型(FL-KGM)，通过改进隶属度函数、优化模糊规则集以及分离CO和CO2来解决传统KGM方法的局限性，在实际数据集上达到98.6%的诊断准确率。


<details>
  <summary>Details</summary>
Motivation: IEEE关键气体方法(KGM)在溶解气体分析(DGA)中广泛应用，但存在数据模糊性处理不足和诊断准确率不高的问题，需要一种更可靠的变压器故障诊断方法来保障电力系统稳定性。

Method: 提出FL-KGM增强模型，包含：改进的隶属度函数、优化模糊规则集、CO和CO2的新型分离方法、多维气体比率分析以及自适应分类框架。

Result: 实验验证表明FL-KGM达到最高98.6%的准确率，显著优于传统KGM和其他基于模糊逻辑的方法。

Conclusion: FL-KGM方法能够推进变压器监测、智能故障检测和现代电力系统预测性维护策略的发展，具有重要的工程应用价值。
...
### [100] [Improving Rural Medication Safety with AI: A Scoping Review](https://arxiv.org/abs/2608.18135)
*Jeong-ah Kim,Muhammad Ashad Kabir,Daniel Terry,Maryam Rouhi*

Main category: cs.AI

TL;DR: 本 scoping review 探讨了人工智能技术在农村医疗环境中减少用药错误的应用效果。系统检索2012-2025年文献，纳入12项来自9个国家的研究。AI技术已整合到用药管理全流程，机器学习监测可将处方和转录错误减少34%-80%，但基础设施不足、缺乏治理框架和临床医生抵触等障碍仍待克服。


<details>
  <summary>Details</summary>
Motivation: 用药错误是全球医疗系统面临的重要安全问题，对患者造成严重伤害。农村地区医疗资源匮乏，更需要借助人工智能技术提升患者安全水平，减少用药错误发生。

Method: 采用系统性文献检索，涵盖EBSCohost、Emcare (Ovid)、MEDLINE和ProQuest Consumer Health Database等数据库，时间跨度为2012年至2025年。共纳入9个国家的12项原始研究，采用主题分析法提取数据。

Result: AI技术已整合到用药管理全流程（处方、调配、给药及给药后监测）。形成四个主题：(1)使用的AI类型（临床决策支持系统、机器学习、自然语言处理、智能泵）；(2)受影响的用药流程阶段；(3)减少错误和提高工作流程安全性的效果；(4)农村特殊挑战。机器学习监测可将处方和转录错误减少34%-80%。主要障碍包括缺乏治理框架、资金限制和临床医生抵触。

Conclusion: AI技术在农村医疗环境中具有提升用药安全的巨大潜力，可实现数据驱动监测、流程自动化和临床决策支持。但需解决基础设施、培训、系统整合和警示疲劳等农村特殊挑战。
...
### [101] [FraudBench: Stress-Testing Policy-Grounded Banking Agents Against Adaptive Fraud](https://arxiv.org/abs/2608.18136)
*Dheeraj Mohandas Pai,Lu Xian*

Main category: cs.AI

TL;DR: FraudBench是一个针对对话式银行代理的反欺诈安全基准测试，基于双控制框架和环境构建，包含150个对抗性场景，用于评估代理在身份、授权和信任被操纵时是否能安全执行操作。


<details>
  <summary>Details</summary>
Motivation: 现有金融欺诈基准仅分类静态交易或消息，通用代理安全基准针对提示注入或通用有害使用，均未测试政策驱动的银行代理在呼叫者通过对话操纵身份、授权和信任时是否能安全行动。

Method: 在τ²-bench双控制框架和τ-Knowledge银行环境上构建FraudBench；代理和模拟呼叫者通过工具对共享可变账户状态操作；环境暴露698份内部政策文档供代理检索；包含150个对抗性场景，其中107个公开（90个欺诈机制+17个链式自适应攻击），43个留作测试集。

Result: 四个代理在107个分级任务上的单次试验评估显示，攻击安全率在49%至65%之间；money-mule（洗钱帮凶）和第一方欺诈是最常见的跨模型弱点。

Conclusion: FraudBench成功揭示了现有银行代理在处理自适应攻击时的安全漏洞，表明需要加强对话式代理在身份验证、信任建立和链式攻击防御方面的能力。
...
### [102] [Efficient Adaptation of LLMs for Hate Speech Detection in Low-Resource Languages: A Comparative Study on Roman Urdu](https://arxiv.org/abs/2608.18142)
*Toneema Zubair,Muhammad Junaid Asif,Faisal Kamiran,Hafiz Hassan Saeed,Rana Fayyaz Ahmad*

Main category: cs.AI

TL;DR: 本研究针对低资源语言罗马乌尔都语(Roman Urdu)进行仇恨 speech检测(HS D)，评估了大语言模型(LLMs)的零样本推理能力，并使用低秩适应(LoRA)参数高效微调方法对Mistral、LLaMA、Falcon和多语言BERT进行微调。在包含72,000+条注释评论的PURUTT数据集上，零样本模型表现中等(F1=0.56)，而通过更新少量可训练参数，分类性能显著提升(F1>0.93)。


<details>
  <summary>Details</summary>
Motivation: 低资源语言(LRLs)面临缺乏注释数据、语言结构非正式、缺乏标准化语法等挑战。罗马乌尔都语在社交媒体上广泛使用但拼写变体高且缺乏一致语境，是进行仇恨 speech检测的典型困难案例。

Method: 使用参数高效微调(PEFT)方法中的低秩适应(LoRA)对多种transformer模型(Mistral、LLaMA、Falcon、多语言BERT)进行微调，评估零样本推理和PEFT微调性能对比。

Result: 零样本模型表现中等(F1=0.56)，但通过PEFT仅更新少量可训练参数即可将分类性能显著提升至F1>0.93。

Conclusion: PEFT方法在低资源语言处理任务中表现出色，兼具优异的性能和计算效率，为资源受限语言的NLP任务提供了高效可行的解决方案。
...
### [103] [RDFdL: Integrating RDF with Differential Dynamic Logic](https://arxiv.org/abs/2608.18165)
*Yuyang Li,Lukas Kubelka,Julia Butte,Tobias Käfer*

Main category: cs.AI

TL;DR: RDFdL框架将RDF与微分动态逻辑(dL)集成,以表示和推理静态知识及物理系统的连续动力学,使dL的安全性和可达性验证结果可通过SPARQL查询在RDF数据上获取。


<details>
  <summary>Details</summary>
Motivation: RDF知识图谱虽能描述静态知识,但无法捕获或推理物理系统的动态行为(如微分方程描述的系统),这是AI驱动网络物理系统的关键缺陷。

Method: 提出RDFdL框架:1)用RDF和SHACL语法表示微分方程和状态空间范围;2)通过翻译到dL提供语义;3)利用共享的一阶逻辑基础实现RDF与dL的集成。实现管道使用Apache Jena进行本体驱动的RDF推理,使用dL定理证明器KeYmaera X。

Result: 动态逻辑域的安全性和可达性验证结果可作为推论,供SPARQL查询在RDF数据上使用。

Conclusion: 成功实现RDF与微分动态逻辑的集成,为网络物理系统的形式化验证与知识图谱推理提供了统一框架,并在制造业中具有应用前景。
...
### [104] [Adversarial Review: Structured Disagreement for Grounded Agentic Code Review](https://arxiv.org/abs/2608.18167)
*Eric S. Qiu,Joyce Gill*

Main category: cs.AI

TL;DR: 本文研究多Agent LLM系统在代码任务中的协作问题，提出Adversarial Review (AR)方法，仅用3个Agent（主编码Agent、审查员、批评者）实现最小化协作，在多个基准测试中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 当前多Agent系统存在两个极端：1）大规模Agent团队收益递减；2）被动工具化（subagent）完全丧失Agent交互优势。研究者试图寻找一种中间方案：在保持协作优势的同时控制复杂度。

Method: 提出Adversarial Review (AR)对抗性审查协议，包含三个角色：主编码Agent负责编写代码，审查员评估代码质量，批评者审计审查过程并在结构化分歧后让主编码Agent修改。在SWE-PRbench中发现"虚假共识"失败模式，通过一次提示迭代显式添加分歧来解决。

Result: 1）LiveCodeBench：AR达到最高通过率，优于五Agent基线；2）SWE-PRbench：原始AR暴露虚假共识问题，但添加显式分歧后达到最高F1；3）SWE-bench Verified：在仓库级编码任务上优于基线。

Conclusion: 协作代码审查不需要大量Agent或复杂通信结构，关键是让分歧最小化、结构化并基于证据。
...
### [105] [Looped Language Models Improve Compositional Tool Calling](https://arxiv.org/abs/2608.18171)
*Andrei Cristian Popescu,Haitz Sáez de Ocáriz Borde,Pietro Liò*

Main category: cs.AI

TL;DR: 研究循环语言模型在智能体工具调用中的表现，发现循环计算有助于组合式工具使用，递归深度越高准确率越高，自适应推理可实现更好的计算-性能权衡。


<details>
  <summary>Details</summary>
Motivation: 循环语言模型在推理基准上表现出色，但其智能体工具调用潜力尚未被充分探索，尤其是在需要协调多个API调用、维护中间状态和保持工具交互依赖性的组合工具调用场景中。

Method: 在API-Bank、BFCL和NESTful数据集上评估原生和改造的循环语言模型，比较循环与非循环模型在匹配监督微调方案下的表现，并在推理时测试不同递归深度。

Result: 递归计算通常有利于组合式和依赖感知的工具使用；在孤立API调用上收益较小且模型依赖性较高。多步工具使用准确率随递归深度增加而提升；自适应推理通过仅在需要时分配额外计算，实现更优的计算-性能权衡。

Conclusion: 循环语言模型是实现需要可靠规划、协调和执行组合工具工作流的智能体系统的有前景架构。
...
### [106] [On the Triangle Inequality for the Jaccard Distance in Arbitrary Lattices](https://arxiv.org/abs/2608.18194)
*Costin Bădică,Amelia Bădică*

Main category: cs.AI

TL;DR: 本文将Jaccard距离推广到格结构和实值赋值，证明了在特定条件下三角不等式成立，并探讨了其应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有Jaccard距离的推广高度依赖分配律，适用范围受限，需要在更一般的格结构上建立有效的广义Jaccard距离度量。

Method: 通过引入严格正、单调、模性质的赋值，在任意格上证明Jaccard距离满足三角不等式；对相对补分配格，引入正、单调、超模和对数子模条件；最后证明超模性是作为有效度量的必要条件。

Result: 当赋值严格正、单调且模时，Jaccard距离在任意格上满足三角不等式；对相对补分配格，正、超模且对数子模条件下三角不等式成立；超模性是标准广义Jaccard距离作为有效度量的必要条件。

Conclusion: 研究将Jaccard距离推广到更一般的格结构，扩展了在量子信息论、形式概念分析和机器学习等计算领域的应用，为进一步开放数学问题奠定基础。
...
### [107] [GenEx: A Graph-Based Representational Paradigm for SARS-CoV-2 Variant Detection via Codon Co-occurrence Networks](https://arxiv.org/abs/2608.18238)
*Arefin Amin,Labiba Faiza Karim,M. Monir Uddin*

Main category: cs.AI

TL;DR: 本文提出GenEx管道，将SARS-CoV-2基因序列转换为密码子共现图并提取图特征，用于检测病毒变体。


<details>
  <summary>Details</summary>
Motivation: 现有基因组分析方法（如序列比对、系统发育分析）将基因序列视为线性字符串，未能捕捉复杂的上下文依赖关系。

Method: 提出MSCG和LAPCG两种密码子共现图生成方法；采用SVD谱图特征提取，使用平方奇异值(σ²)增强类间可分性；训练23个基准机器学习模型。

Result: 在最新SARS-CoV-2变体（Beta、Gamma、Delta、Omicron）检测中取得显著效果。

Conclusion: 图表示学习方法可有效捕捉基因序列的上下文依赖，比传统线性方法更适合病毒变体检测。
...
### [108] [Redakto - The Incognito Tab for LLMs](https://arxiv.org/abs/2608.18260)
*Saurav Kumar Saha,Tom Röhr,Felix Bießmann*

Main category: cs.AI

TL;DR: Redakto是一个用于在将文本输入LLM或其他下游文本处理之前对其进行匿名化的工具，提供最先进的PII编辑和假名化功能，可通过Web应用、REST API和MCP钩子使用，完全开源，需要适度的计算资源，可在本地硬件上部署。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在日常应用中的广泛使用，确保隐私（尤其是去除个人可识别信息PII）成为重要挑战。欧盟新立法使这一问题更加紧迫，LLM使用相关的隐私不确定性可能阻碍创新速度和研究向应用的转化。

Method: 开发了Redakto工具，提供PII编辑和假名化功能，通过Web应用、REST API和MCP钩子暴露功能。在法律和医疗领域文本数据上进行广泛实证评估，检验匿名化文本的隐私性和效用。

Result: 使用不同编辑策略的匿名化文本在效用分数上与原始文本相当，表明使用Redakto进行匿名化可用于LLM任务，不会对所探索的任务产生实质性负面影响。

Conclusion: Redakto是一个有效的文本匿名化工具，能够在保护隐私的同时保持文本效用，可用于LLM应用且不影响任务性能。
...
### [109] [Cacheable by Design? Training Mixture-of-Experts Routers for Locality Against the Edge Memory-Bandwidth Wall: A Pre-Registered Negative Result with a Systems Measurement Study](https://arxiv.org/abs/2608.18261)
*Shriniwas Ramesh Suram*

Main category: cs.AI

TL;DR: 本文研究了如何在消费级硬件（单GPU 8GB显存）上服务大型MoE（混合专家）模型，发现内存带宽是主要瓶颈，而非计算能力。研究团队开发了llama-moe-trace路由遥测工具，量化了Qwen3-235B的解码带宽墙（0.44 tok/s），并探索了通过训练辅助损失来提高专家缓存命中率的方法，但发现缓存改进与模型质量（困惑度）之间存在紧密权衡。


<details>
  <summary>Details</summary>
Motivation: 在消费级硬件上服务235B参数的MoE模型受限于内存带宽而非计算能力，需要从慢速SSD加载活跃专家。作者旨在量化这一"带宽墙"，测量路由模式，并探索通过训练提高缓存命中率的可能性。

Method: 1. 构建llama-moe-trace工具进行零手术路由遥测；2. 在Qwen3-30B上测量相邻token专家复用率（2.0x）、流量分布（95%流量使用52.5%专家）和LRU缓存效果；3. 预注册训练137M MoE模型，使用辅助局部性和领域路由损失；4. 测试训练无关的缓存感知重路由与训练局部性的组合。

Result: 1. Qwen3-235B实测解码仅0.44 tok/s，符合带宽模型；2. 相邻token专家复用概率2.0x，95%流量集中于52.5%专家，13.4%专家的LRU缓存可服务66%请求；3. 辅助损失可将缓存miss降低60%，静态固定命中率达99%，但所有配置均未通过≤1%困惑度门槛；4. 340M模型显示tax不随规模缩小反而略增；5. 训练无关的缓存感知重路由+训练局部性可实现~80% miss降低且困惑度仅增3.4%。

Conclusion: 缓存改进与模型质量紧密耦合，预注册的更严格标准下，单独使用任一方法均无法同时满足缓存和质量要求。训练无关的缓存感知重路由与训练局部性的组合可在保持低困惑度的同时实现显著的miss降低。代码、追踪数据和预注册信息已开源。
...
### [110] [Evaluating Structured Information Extraction with Open Models in a High Risk Public Sector Application](https://arxiv.org/abs/2608.18289)
*Elias Schubert,Felix Bießmann*

Main category: cs.AI

TL;DR: 本文提出了一个综合基准测试，评估开源OCR引擎、大型语言模型(LLM)和视觉语言模型(VLM)在处理复杂现实文档任务(学生入学申请)上的端到端性能。研究发现VLM通常优于OCR+LLM管道，但即使是最先进的开源模型在零样本设置下也难以可靠处理此类任务。


<details>
  <summary>Details</summary>
Motivation: 开源文档提取工具生态系统快速发展，但缺乏在现实多步骤提取管道上的系统评估。鉴于这些解决方案将成为欧盟AI法案归类为高风险的公共部门应用的关键组件，需要对现实任务进行综合评估。

Method: 对国际研究项目学生申请这一高风险复杂现实文档处理任务进行了全面的实证评估，测试了先进的OCR引擎、LLM和VLM，共35种配置。

Result: 仅4/35配置F1分数超过0.5，最佳OCR+LLM管道与顶级VLM性能相当，但大多数OCR+LLM组合表现明显更差，约75%配置分数低于0.25。模型规模影响性能但呈非线性关系，输入质量(尤其是OCR输出的结构保留)成为独立于下游模型能力的关键因素。

Conclusion: 开源模型在零样本设置下处理复杂文档提取任务的能力有限，模型规模并非越大越好，输入数据质量对最终性能有决定性影响。未来需要进一步改进开源工具以满足高风险应用需求。
...
### [111] [The Lifecycle of LLM-as-a-Judge for Large-Scale Recommendation Explanations](https://arxiv.org/abs/2608.18300)
...
TL;DR: Netflix提出了一套完整的LLM评判器生命周期框架，用于评估其推荐解释系统。该框架包含四个阶段：定义评估标准和构建人工标注数据集(Birth)、通过RART方法微调评判准则(Training)、部署担任质量门控和反思生成双重角色(Monitoring)、以及持续监控和重调(Monitoring)。A/B测试显示该方法显著提升了用户观看新内容和浏览-播放会话成功率。


<details>
  <summary>Details</summary>
Motivation: 当前大多数研究将LLM评判器视为静态产物，仅在构建时评估一次。然而生产环境中的LLM评判器实际上具有完整生命周期：需要构建、训练、部署和持续维护，且每个阶段都面临独特的技术和运维挑战。

Method: 提出了Reasoning-Aligned Rubric Tuning (RART)方法，利用元评判器对推理输出的学习信号来精炼评判准则。框架包含四阶段：Birth（定义评估标准、构建带人工标签和理由的基准数据集）、Training（通过RART微调）、Deployment（质量门控和反思生成双重角色）、Monitoring（持续人在回路对齐流程，检测漂移并触发重调）。

Result: 在数千万会员进行的五周A/B测试中，评判器对齐的解释使会员观看行为转向新内容（之前未观看），并提升了从浏览到播放的成功会话次数，相较于无解释对照组，且无质量相关的下架情况。

Conclusion: LLM评判器在生产环境中应被理解为一个动态生命周期系统，需要持续的维护和优化。RART方法和四阶段框架为实际生产部署提供了可行路径，实验证明了其在提升用户参与度方面的有效性。
...
### [112] [SESSE: Sketch, Expand, Sort, Summarize, Evaluate -- LLM-as-Judge Evaluation via Structured Decomposition](https://arxiv.org/abs/2608.18303)
*Dae Lee,Mihai Delgeanu,Adel Youssef*

Main category: cs.AI

TL;DR: SESSE是一个无训练框架，将LLM作为评判者的整体偏好判断分解为结构化子问题，从评判者自身的错误案例中挖掘，无需 oracle 响应、任务特定评分标准或微调。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-as-judge评估方法仅提供单一的A/B偏好选择，无法识别哪些质量维度影响了偏好，也无法区分模型错误与标签模糊性。

Method: SESSE框架包含五个步骤：Sketch（草拟问题）、Expand（扩展问题）、Sort（排序问题）、Summarize（总结）、Evaluate（评估），从评判者自身的错误案例中直接挖掘结构化子问题。

Result: 在RewardBench（n=1,000）上，SESSE与思维链基线接近持平，与RISE-Judge-32B（92.7%）竞争，同时保持完全无训练。

Conclusion: SESSE通过按标准投票的证据提供了可解释的审计追踪，能够诊断标签模糊性和评判者失败模式，这是单一整体输出token无法提供的。
...
### [113] [ComponentBench: Diagnosing Component-Level Failures in Computer-Use Agents](https://arxiv.org/abs/2608.18307)
*Tianchen Guan,Xinlei Lin,Royce Cheng-Yue,Xiangjun Wang,Shuyan Zhou*

Main category: cs.AI

TL;DR: ComponentBench是一个针对现代web UI组件级评估的基准和诊断pipeline，通过97个canonical UI组件的2910个任务评估7个模型，发现观察/动作空间设计是性能关键因素，模型间差距超30%，且agents仍需人类3.7倍时间完成简单交互。


<details>
  <summary>Details</summary>
Motivation: 当前计算机使用代理的评估存在"中间层"空白：已有的长周期工作流基准测试过于宏观，原子级GUI基础测试过于简单。缺少一种既可诊断又足够丰富的组件级评估方法来捕捉现代界面的复杂性。

Method: 1) 构建了97个canonical UI组件的ontology，在4个主流组件库中实例化2910个可编程验证的任务；2) 收集了cleaned human reference trajectories用于评估成功率和交互效率；3) 设计了可扩展的pipeline用于审计实现后的结构难度并合成结构化失败分析；4) 在四种观察和动作空间配置下评估7个模型（GPT-5.4、Gemini 3 Flash等）

Result: 1) 观察和动作空间设计显著影响性能：GPT-5 mini在accessibility-tree观察下成功率达83.1%，但在仅坐标的Pixel控制下仅48.9%，差距超30%；2) 即使最快配置也需人类3.7倍时间；3) 对人类简单的空间操作仍难倒当前agents

Conclusion: ComponentBench通过严格评估揭示了当前计算机使用代理的关键瓶颈：观察/动作空间设计对性能影响巨大（同一模型成功率相差超过30%），且即使最快配置也需3.7倍人类时间完成，空间操作仍是核心挑战。这为后续研究指明了改进方向。
...
### [114] [Governance Records as Supervision: Verifier-Selected Self-Training for Structured Workflow Repair](https://arxiv.org/abs/2608.18324)
*Jesus Salas*
...
### [115] [Measuring the Partial-Credit Gap: A Strict Benchmark on Vietnam's 2025 Convex Marking Scheme](https://arxiv.org/abs/2608.18336)
*Nguyen Quoc Hung,Nguyen Dang Minh,Le Nhu Quynh,Tran Khanh Linh,Nguyen Kieu Linh*

Main category: cs.AI

TL;DR: 标准准确率指标高估了语言模型在非加性评分考试中的能力，因为它们对部分正确给予比例积分，而实际考试会惩罚这种部分正确。以越南2025年高考改革为例，Part II的真假判断题中，答对3/4题仅得0.5分而非0.75分，导致标准准确率指标使分数被夸大。


<details>
  <summary>Details</summary>
Motivation: 2025年越南国家高中毕业考试的改革揭示了基准评分与实际考试评分之间的根本性不匹配。标准基准假设部分知识值得相应比例的学分，但当考试使用非加性评分方案时，这个假设就失败了。

Method: 构建THPT-Ladder基准测试，包含632道来自21场官方考试、涵盖11个科目的题目，严格按照教育部对学生的评分标准进行评分。将模型与超过100万真实考生的成绩进行直接比较。

Result: 八个大模型中，官方评分比比例积分每道Part II题目少付0.020-0.159分。例如Qwen3.5-27B在2025年历史考试中，0.042分的差距使其排名从第90百分位下降到第77百分位（481,293名考生中）。标准准确率无法预测这种惩罚。

Conclusion: 标准基准报告的能力是考试机构不会认证的。准确率不能预测非加性评分带来的惩罚，基准需要采用与目标机构完全一致的评分方案。
...
### [116] [A Jagged Frontier: Evaluating Robustness of Code Agents to Semantics-Preserving Transformations](https://arxiv.org/abs/2608.18389)
*Hasan Najib Mahmud,Shreya Gupta,Isha Chaudhary,Nathaniel Enis,Ravi Mangal,Gagandeep Singh,Corina Pasareanu*

Main category: cs.AI

TL;DR: 研究评估了AI代码代理在代码被重写为语义等价形式时的可靠性。使用随机变体采样器应用语义保持变换(SPT)，对两种代理框架和四种前沿模型进行测试，发现最严重配置下解决率下降6.7个百分点，揭示了AI代码代理对表面代码变化的敏感性。


<details>
  <summary>Details</summary>
Motivation: AI代码代理日益部署于解决真实软件问题，但其对表面代码变化的可靠性尚未被深入理解。研究旨在探索当周围代码库被重写为语义等价形式时，编码代理是否仍能可靠工作。

Method: 引入随机变体采样器，应用语义保持变换(SPT)生成扰动变体，包括控制流重写、死代码注入和标识符重命名。评估两种代理框架(mini-SWE agent和OpenCode)与四种前沿模型的组合，在SWE-bench Verified和SWE-bench Pro数据集上进行多轮运行。

Result: 大多数配置出现小幅下降：最严重配置下解决率平均下降6.7个百分点，16种配置中有6种显示统计显著降级。没有单一模型在所有框架下保持最稳健的排名——Qwen在mini-SWE agent下最稳健但在OpenCode下最脆弱。较简单的框架(mini-SWE agent)对扰动更稳健。

Conclusion: 即使是最先进的前沿模型也容易受到语义保持变换的影响，尽管效果不均匀。这引发了对AI代码代理在多样化真实代码库中部署可靠性的担忧。
...
### [117] [When Clean Signals Are Not Enough: Detecting Structural Ambiguity for Safe Wearable Stress Classification](https://arxiv.org/abs/2608.18397)
*Saba A. Farahani,Hung Cao,Amir M. Rahmani*

Main category: cs.AI

TL;DR: 针对可穿戴压力分类器在个体层面可能完全失效的问题，提出ICCM监控器来检测"结构性模糊"现象，无需重新训练分类器即可实现对高风险样本的 defer/abstain 决策。


<details>
  <summary>Details</summary>
Motivation: 可穿戴压力分类器虽能达到93%的平均准确率，但对特定个体（如WESAD中的Subject 14）完全失效（F1=0）。现有评估指标掩盖了这种个体失败，需要一种能在推理时检测结构模糊的方法。

Method: 提出ICCM（个体保形耦合监控器）：(1)量化个体特异性耦合发散度；(2)在推理时将每个窗口路由到分类、延迟或弃权；(3)无需重新训练下游分类器。基于WESAD（N=15）和Stress-Predict（N=35）数据集验证。

Result: Pearson相关显示模糊度与准确率负相关（WESAD: r=-0.607, p=0.016; Stress-Predict: r=-0.412, p=0.014）。但鲁棒性分析显示：排名相关不显著，去除Subject 14后WESAD相关性消失。ICCM将误报从29降至27、94降至92（均不显著）。对Subject 14的21个压力窗口扣留3个，但未能修复漏检问题。

Conclusion: ICCM可作为个体失败和Unsupported生理学的可解释信号，但不能作为独立的安全保障。其效果高度依赖特定失败案例，在去除极端个体后效果不显著。
...
### [118] [Improving Natural-Language Combinatorial-Optimization Accuracy in Resource-Constrained Language Models via Formal Abstractions](https://arxiv.org/abs/2608.18409)
*Shrenil Shaun Sharma,Avi Sharma*

Main category: cs.AI

TL;DR: SDDL是一种神经符号框架，通过将自然语言调度问题转换为紧凑的求解器对齐表示，使较小的语言模型能够在资源受限环境中解决组合调度问题，显著提高可行性率。


<details>
  <summary>Details</summary>
Motivation: 语言模型在组合调度中面临挑战，特别是在资源受限环境中，较小的模型在直接从自然语言调度时往往无法保持可行性。

Method: SDDL将自然语言调度问题转换为任务、资源、约束和目标的紧凑求解器对齐表示，同时将低级建模和搜索委托给确定性编译器和外部求解器。在300个实例的多家族调度问题子集上进行测试。

Result: SDDL为每个资源受限模型测试提高了独立验证的可行性。两个最强配置分别达到55.3%和28.3%，而直接生成基线为23.7%和1.3%，求解器代码基线为21.7%和7.0%，可行调度的中位最优性差距为0.0%。

Conclusion: 通过表达问题结构而非生成解决方案或求解器代码，SDDL使较小的模型能够接近评估的最强直接配置和求解器代码配置，包括更大的前沿模型。
...
### [119] [FM-Bench: A Benchmark for Long-Horizon Management with Competing Agents](https://arxiv.org/abs/2608.18423)
*Tianyou Wang,Chongyang Gao,Kezhen Chen,Chen Dong,Yinghao He,Donghan Li,Wangcheng Xu,Hongjiu Zhang,Chi Li*

Main category: cs.AI

TL;DR: FM-Bench是一个评估语言模型代理在长视野决策能力的新基准，测试LLM代理运营足球俱乐部20个游戏年，涉及26个工具和约340-400个决策点，涵盖选秀、球员交易、合同谈判、设施投资和青训等复杂管理任务。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型代理能在有界任务中可靠执行，但能否在长期视野中维持有效决策（其行动具有累积后果且环境会响应其选择）尚未被充分测量，需要一个标准化的评估基准来填补这一空白。

Method: 设计了FM-Bench足球管理基准：单一模式让15个前沿模型各自对抗冻结的脚本世界；竞技场模式让所有模型加脚本锚在同一共享20年世界中对抗。测量六个行为能力：选秀、球员交易、合同谈判、设施投资、青训和排兵布阵。使用确定性引擎累积年度得分，最终得分无LLM评判或人工评分。

Result: 所有15个模型完成全部20年视野，脚本基线大多失败。claude-fable-5在单一模式和竞技场中均表现最佳，但竞技场冠军在十个模型间轮换。规模、价格和供应商均无法预测排名，排名直到视野后期才确定，最好的人类玩家仅排在模型底部。高分模型在末期减少慢回报投资、保持现金投入、提前开启续约谈判。 token消耗与得分无关。模型均未从数百次被拒报价中学习市场隐藏价格，自管理内存出现两种失败模式：仅增长的档案或每季重写的计划。

Conclusion: 区分模型的是管理行为而非计算能力。LLM代理在长视野决策任务中的表现取决于其对投资时机、现金流管理和谈判策略的理解，而非模型规模或成本。该基准揭示了当前LLM在持续学习和自适应策略方面的局限性。
...
### [120] [UMER: Unifying Embedding and Ranking via Pair-Aware Discriminative Reasoning for Universal Multimodal Retrieval](https://arxiv.org/abs/2608.18504)
*Libiao Chen,Xiyang Liu,Yanheng Wei,Tao Wang,Zhenyu Tang*

Main category: cs.AI

TL;DR: UMER是一个统一的多模态嵌入和排序框架,用于通用多模态检索。该框架通过成对感知判别推理来比较查询-候选对,识别与指令相关的匹配和差异证据,联合学习对比嵌入用于高效全局匹配和判别排序用于显式成对相关性判断,在MMEB-V2基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有CoT方法通常对查询和候选进行独立推理,无法提供明确证据来区分正样本与语义混淆的困难负样本。对比嵌入捕获全局相似性,但难以处理需要答案验证、类别判断或细粒度推理的元任务。

Method: 提出UMER框架,采用成对感知判别推理(Pair-Aware Discriminative Reasoning)替换逐项推理,比较查询-候选对以识别指令相关匹配和差异证据。在单一MLLM中联合学习对比嵌入用于高效全局匹配,以及判别排序用于显式成对相关性判断,并采用互补的相互蒸馏策略在嵌入和排序函数之间转移可靠的成对偏好。

Result: 在MMEB-V2基准上,UMER在 comparable experimental settings下实现了最先进的性能,同时支持预算可调的推理。

Conclusion: UMER通过结合对比嵌入和判别排序两种能力,配合成对感知判别推理和相互蒸馏策略,有效地提升了通用多模态检索的性能,特别是在处理困难负样本和需要细粒度推理的任务上。
...
### [121] [Which Negatives Matter? Ask Your Text Encoder: Adaptive Similarity Margins for Dense-Caption Retrieval](https://arxiv.org/abs/2608.18521)
*Haoyue Liu,Ye Chen,Zhichao Wang,Xiaoying Tang*

Main category: cs.AI

TL;DR: HN-CLIP是一种用于密集标题检索的新方法,通过利用文本编码器的文本-文本几何结构为每个负样本构建自适应相似度边界,解决了传统InfoNCE目标在强预训练初始化下过早饱和的问题。


<details>
  <summary>Details</summary>
Motivation: 当前密集标题检索方法继承InfoNCE目标,但在强预训练初始化下优化过早饱和:在80%批次的第一个epoch内损失降至10^{{-3}}以下,47%的测量中梯度在fp32下完全为零。这与密集标题基准中存在大量近重复标题有关。

Method: HN-CLIP使用文本编码器自己的文本-文本几何结构为每个负样本构建自适应相似度边界。具体做法是添加一个分离的标题相似度矩阵到负对数概率中,为更相似的标题分配更大的边界,无需挖掘、合成或重采样负样本。

Result: 在四个密集标题检索基准上,HN-CLIP相比最强竞争对手提升R@1达+2.4至+4.3,训练速度比GOAL快2.4倍,比StructXLIP快5.4倍。该方法在域内基准上改进了所有六种测试的微调框架,并仅用20%训练数据达到最强全数据基线。

Conclusion: HN-CLIP通过引入基于文本-文本几何的自适应边界机制有效解决了InfoNCE目标在密集标题检索中的过早饱和问题,在性能和训练效率上均取得显著提升,且无需额外数据、参数或推理开销。
...
### [122] [Pairwise Ranking Outperforms Single-Action RL for Offline Explanation Selection: A Practical Lesson](https://arxiv.org/abs/2608.18531)
*Tanay Chowdhury,Saeideh Shahrokh Esfahani*

Main category: cs.AI

TL;DR: 本文提出一种工业级可解释推荐系统，将LLM生成与选择解耦：提前生成候选解释池，请求时用CPU小模型选择。LambdaRank在Google Local上F1=0.500，KG-path方法USR达1.000，构建成本仅约15美元。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的可解释推荐系统存在高延迟（数百毫秒）和高成本问题，且成本随流量线性增长。现有方法将生成与选择耦合，导致每次请求都需要LLM推理。

Method: 将解释生成与选择解耦：1）提前用两个 commodity LLMs 生成六种提示风格的冻结候选池；2）请求时使用小型CPU-resident选择器挑选。评估了六种离线池选择器（LambdaRank、PPO、GRPO、DPO、蒸馏）和三种KG路径选择器（随机游走、边不相交枚举、MMR重排序路径）。

Result: LambdaRank在Google Local上F1=0.500（超越G-Refer和XRec），MovieLens-1M上F1=0.329；KG路径方法在Google Local上USR=1.000，MovieLens-1M上USR=0.997-1.000。成对学习排序优于单动作RL方法。不同生成器（Claude 3 Haiku vs 4.5）F1变化仅0.001-0.006。

Conclusion: 解耦生成与选择可有效降低延迟至100ms以下且无需GPU。成对学习排序方法（LambdaRank）显著优于单动作RL；KG路径方法可避免模板崩溃问题。生成器与选择器可独立评估，系统构建成本约15美元。
...
### [123] [FinRCA-Bench: Benchmarking Evidence Retrieval and Reasoning for Financial AI Systems](https://arxiv.org/abs/2608.18534)
*Pratik Ghawate*

Main category: cs.AI

TL;DR: 本文提出了FinRCA-Bench，一个用于评估金融对账任务的确定性合成基准，包含2,250个应付账款到银行对账案例，涵盖14个操作表，含1,500个注入失败和750个合法/硬负例，用于独立评估检索性能而非推理正确性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在金融运营中的推理表现可能依赖于是否获得正确的证据，而金融对账所需的证据分散在发票、采购订单、审批、分配、付款、账簿条目和银行活动等多个表中，由交易关系而非文本相似性链接。端到端准确率会将证据获取与推理质量混为一谈。

Method: 构建了包含14个操作表的2,250个合成案例基准（1,500个注入失败跨15个因果类别，750个合法/硬负例），隐藏根因标签和记录级证据契约，比较了Rules/SQL、经典ML、密集语义检索、确定性关系扩展和Typed Provenance Graph Retrieval (TPGR)五种方法。

Result: Rules/SQL达到84.97%准确率，经典ML达到95.44%。固定推理模型、提示和生成设置，仅改变检索方式 macro required-record recall从0.83%提升至77.70%，16类准确率从2.05%提升至72.44%。结构检索失败（95个）远超推理失败（15个），254个正确预测发生在不完整检索下，严格返回证据契约准确率仅5.72%。

Conclusion: 在FinRCA-Bench上，检索架构强烈影响观察到的AI系统性能，正确的根因标签并非可审计诊断的良好代理。TPGR通过类型化遍历持久化交易关系显著优于其他方法。
...
### [124] [Bridging Search and CRM: Productionizing AI Product Research Agents for Customer Re-Engagement](https://arxiv.org/abs/2608.18543)
*Mandar Kulkarni,Pooja A.,Samir Shah*

Main category: cs.AI

TL;DR: 本文提出了一种基于AI产品研究代理的可扩展框架，用于在电子商务平台上通过WhatsApp进行主动客户重新参与。该系统识别具有探索性购买意图的用户，进行多代理产品研究，并通过WhatsApp提供个性化推荐。


<details>
  <summary>Details</summary>
Motivation: 现代电子商务平台独立运营搜索、推荐、个性化和CRM系统，限制了主动客户重新参与的机会。对于最佳智能手机或最新5G手机等探索性意图，用户可能在购买前离开平台进行外部研究。

Method: 1) 识别具有探索性购买意图和低参与度的用户 2) 使用行为信号、外部知识和企业目录数据进行基于AI的多代理产品研究 3) 通过WhatsApp发送个性化推荐

Result: 在23天的生产部署中发送了约15K条WhatsApp通知，移动产品发现活动的点击率较传统WhatsApp推荐活动有显著提升，显示出消息转发和分享的二次参与证据，并产生了下游购买和GMV影响。

Conclusion: AI产品研究代理对于主动客户重新参与和端到端客户旅程优化具有实际效果。
...
### [125] [FACET: Preserving Source Intent and Executable State in Terminal Task Synthesis](https://arxiv.org/abs/2608.18580)
...
TL;DR: FACET是一个用于合成终端任务可执行监督的框架，通过环境共享状态对齐指令、解决方案和验证器，解决终端智能体训练中的任务合成不一致问题。


<details>
  <summary>Details</summary>
Motivation: 训练终端智能体需要可扩展的可执行监督，但合成高质量终端任务仍具挑战性。任务组件（指令、环境、参考解决方案、验证器）若从不一致假设生成，可能导致任务不可解或评估错误；多阶段合成会丢失原始源码中的目标、依赖、状态转换和程序约束。

Method: FACET框架包含两个核心机制：1）将相关智能体技能重构为信息丰富的连贯场景；2）在生成任务工件前实现并修复执行环境。容器状态作为指令、解决方案和验证器的共享接地，通过基于执行的验证和针对性修复纠正工件特定故障，避免不必要地重新生成有效组件。

Result: FACET生成了具有密集可执行检查的复杂终端任务，从这些任务收集的成功轨迹提供了有效、数据高效的监督。在多个规模上微调模型持续提升Terminal-Bench 2.1性能。替代生成方案分析支持环境接地构建对任务有效性和解决方案-验证器对齐的重要性。

Conclusion: 源意图保留和共享可执行状态接地是实现可扩展终端任务合成的关键原则。
...
### [126] [Can a Lightweight Multimodal Model Estimate LLM Reasoning Performance? A Study for Compute-Optimal Document Inference](https://arxiv.org/abs/2608.18591)
*Zishan Ahmad,Vishal Vaddina*

Main category: cs.AI

TL;DR: BudgetDoc是一个多模态基准数据集，用于在文档任务中显式监督模型-预算-性能权衡。DRB是一个约10亿参数的预飞行估计器，可预测不同预算级别下的模型性能，在动态分配推理预算时能在降低成本的同时保持或提升F1分数。


<details>
  <summary>Details</summary>
Motivation: 为LLM均匀分配推理预算成本高昂且容易导致过度思考惩罚，特别是在视觉布局影响复杂度的文档任务中。需要一个方法来动态分配推理预算以平衡成本和性能。

Method: 构建了BudgetDoc基准数据集，包含三个文档任务的模型-预算-性能权衡显式监督数据。训练了DRB(Document-Reasoning Balancer)模型，这是一个约10亿参数的预飞行估计器，由SigLIP-2和Qwen3-0.6B组成，用于预测不同预算级别下的序贯模型性能。

Result: DRB实现了0.753的加权F1分数。在五个前沿模型和三个数据集上动态分配推理预算时，DRB在15种配置中的9种实现了与最大预算基线相当或更好的F1分数，同时大幅降低了成本。初步评估还表明DRB具有跨模型选择的泛化潜力。

Conclusion: DRB能够有效平衡文档理解任务中的推理成本和模型性能，为多模态大模型的高效推理提供了一种有前景的方法。
...
### [127] [CTIFoundry: An Agent-Native Corpus Scaffold for Cyber Threat Intelligence](https://arxiv.org/abs/2608.18613)
*Yutong Cheng,Changze Li,Qian Cui,Wei Ding,Lingzhi Wang,Yan Chen,Peng Gao*

Main category: cs.AI

TL;DR: CTIFoundry是一个面向LLM智能体的网络威胁情报(CTI)语料库框架,通过构建结构化知识图谱、实体索引层和混合检索接口,显著提升了智能体在CTI调查任务中的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 当前CTI语料库(威胁报告、漏洞数据库)仍采用RAG方式打包,以嵌入索引后的不透明文本块形式存在。但随着LLM智能体越来越多地执行多步调查任务,这种以人为中心的语料库设计成为了瓶颈,而非模型能力本身。

Method: 构建时:(1)基于CVE、CWE、CAPEC、ATT&CK四个权威知识库构建确定性本体图谱,通过官方交叉引用形成类型化可遍历边;(2)构建基于跨度的报告层,对跨供应商实体进行规范化别名解析,索引携带溯源信息的文本块;(3)混合密集+词汇检索表面。查询时:通过七个类型化工具和三个程序化技能暴露结构,挂载在开源智能体框架上。

Result: 在CTIConnect公开基准测试中,仅替换动作表面即可使相同框架的智能体F1提升0.19-0.28(跨4个模型、2个提供商)。小模型使用CTIFoundry可超越大模型在扁平基材上的表现,且准确率提升未以搜索努力为代价——两个Claude模型上,有框架的智能体工具调用减少约一半仍更准确。消融实验表明:类型化结构贡献更大,程序化技能将结构转化为规范性,二者超叠加组合。

Conclusion: CTIFoundry通过提供智能体原生的语料库结构,解决了CTI调查的关键瓶颈。类型化结构和程序化技能的组合是关键,其效果超过各自独立贡献之和,为下一代智能体CTI系统提供了新的研究方向。
...
### [128] [Preference Reasoning under Indeterminacy in Large Language Models](https://arxiv.org/abs/2608.18631)
*Hadi Hosseini,Samarth Khanna,Xiyuan Wang*

Main category: cs.AI

TL;DR: 随着大语言模型演变为决策智能体，基于偏好的推理能力对AI对齐、协调和集体智能至关重要。然而，现实世界的偏好推理本质上是不确定的：信息可能不完整，有效解决方案可能不存在。现有语言模型无法区分确定性与不确定性实例，这是核心挑战。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型向决策智能体演进时，偏好推理能力成为对齐、协调和集体智能的基础。但与标准基准不同，现实世界的偏好推理本质是不确定的——信息可能不完整，有效解决方案可能不存在。作者认为不确定性本身是AI推理的核心挑战。

Method: 作者沿两个轴的形式化这一挑战：(1) 认知不确定性，源于不完整、部分或表达性偏好；(2) 结构性不确定性，源于标准社会选择概念下解决方案的不存在。通过任务层次结构，测试当前最先进的语言模型在区分确定性与不确定性实例方面的能力。

Result: 研究表明，最先进的语言模型系统性地无法区分确定性与不确定性实例，即使在验证设置中也表现出校准不当的推理能力。

Conclusion: 不确定性本身而非仅仅是正确性，是AI推理的核心挑战。现有LLM在处理偏好推理的不确定性方面存在根本性缺陷，需要新的方法来建模和推理不确定性。
...
### [129] [Candidate-Fate Accounting for Transparent Sensor Diagnostic Pipeline Search](https://arxiv.org/abs/2608.18665)
*Haotao Xie,Yutian Chen,Yangqi Liu,Xiaoyu Jiang*

Main category: cs.AI

TL;DR: 本文提出候选者命运会计(candidate-face accounting)框架,解决AutoML/AutoDL搜索中只保留成功试验而忽略无效/剪枝候选者的问题,为工业传感器诊断的自动化管道搜索提供可审计的证据记录。


<details>
  <summary>Details</summary>
Motivation: 现有AutoML/AutoDL报告仅保留拟合试验、分数和获胜者,遗漏了无效、被剪枝、跳过、缓存或未拟合的候选者,导致审查者无法检查信号约束、预算使用和未评估的合法替代方案。

Method: 提出候选者命运会计框架,包含四个核心机制:1)哈希合并重复观测;2)合法性检查标记无效候选者;3)分配理由解释预算决策;4)封闭命运账本为每个候选者分配唯一终端命运。

Result: 在三个轴承诊断数据集上的实验表明:该框架能检测无效候选者,识别出仅保留拟合试验报告中遗漏的30-41个候选者,封闭命运记录验证了完整的候选者 accounting,同时保持具有竞争力的诊断性能。

Conclusion: 候选者命运会计框架为AutoML搜索提供了透明可审计的候选者级别追踪,填补了现有报告机制的空白,具有实际应用价值。
...
### [130] [Sanyu Studio: A Multi-Agent System for Art-Historical Narrative Construction](https://arxiv.org/abs/2608.18677)
*Zhaoxi Wei,Hongye Yang,Shuyuan Tian*

Main category: cs.AI

TL;DR: 本研究探讨了基于大语言模型的交互是否能支持多元艺术史叙事建构。研究者开发了"Sanyu Studio"多智能体对话系统，将321幅常玉油画建模为具有事实、解读、组织和记忆过滤机制的智能体。基于为期7天、8位艺术院校参与者的工作坊研究表明，用户提示、证据组织和认知倾向塑造了多元但连贯的数字常玉版本。


<details>
  <summary>Details</summary>
Motivation: 生成式AI可能标准化艺术解读的担忧，研究旨在探索AI是否能支持多元艺术史叙事建构，而非使其同质化。

Method: 开发Sanyu Studio多智能体对话系统；将321幅常玉油画建模为具有事实、解读、组织和记忆过滤机制的智能体；开展为期7天的工作坊，8位艺术院校参与者参与。

Result: 用户提示、证据组织和认知倾向塑造了多元但连贯的数字常玉版本，表明在有限历史证据条件下，AI可以放大人类能动性。

Conclusion: 在有限历史证据条件下，AI可以放大人类能动性，为公众提供艺术史解读的交互式入口点。
...
### [131] [RTPO: Reverse-Turn Policy Optimization for Stabilizing Agentic RL Training](https://arxiv.org/abs/2608.18682)
*Yugu Li,Jimmy Cao,Jianglin Qiao,Siyi Hu*

Main category: cs.AI

TL;DR: 本文研究多轮智能体工作流中的强化学习训练稳定性问题，提出RTPO方法通过反向Turn策略优化解决展开轨迹优化中的三个核心不稳定源。


<details>
  <summary>Details</summary>
Motivation: 多轮RL训练高度不稳定，随着轮数增加性能严重下降，需要理论分析和系统解决方案。

Method: 提出反向Turn策略优化(RTPO)，将多轮rollout组织为稀疏反向树，按时间逆序执行Turn级策略更新，使每个决策与其下游延续对齐。

Result: 在多轮智能体RL基准测试中，RTPO分别比轨迹级和Turn级基线提升21.50%和10.76%。

Conclusion: RTPO消除了上下文不匹配和异步漂移，降低了信用偏差，收敛到递归最优，为工具使用智能体的更稳定训练提供了潜力。
...
### [132] [Competence, Not Accuracy: A Diagnostic for Reference-Free Judge Gates in Skill Optimization](https://arxiv.org/abs/2608.18719)
*Chenle Chen,Yangbo Wei,Chao Yao,Shaoqiang Lu,Junhong Qian,Chen Wu,Lei He*

Main category: cs.AI

TL;DR: 研究提出了一个参考无自由的法官作为潜在求解器的形式化方法，通过法官能力c和答案空间大小k得出判别能力(ROC-AUC)的闭式界限，提供了必要的条件c > 1/k，并发现边际AUC受项目难度混淆影响，而问题内估计器则不会。最终提供了一个廉价的法官门预部署诊断方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于可验证奖励，限制了自动验证器的任务范围。用LLM法官门替代验证器可以解除这一限制，但法官门是否能提供可用信号尚不确定。研究首先提出一个问题：在将法官放入循环之前，能否判断其分数是否能区分正确和错误的答案？

Method: 将参考无自由的法官形式化为潜在求解器，其裁决取决于与自身结论的一致性，因此其评估能力受限于其求解能力。通过数学推导得出判别能力(ROC-AUC)在法官能力c和答案空间大小k上的闭式界限。建立必要条件c > 1/k。设计非干预探针在真实优化运行中记录法官分数而不改变任何决策。

Result: 在能力接近下限处判别能力处于随机水平，在能力超过下限时可用；发现法官的基准准确率高估了重要能力；在闭环研究中，筛选预测了哪种门控错误发生。

Conclusion: 研究提供了一个廉价的法官门预部署诊断工具，能够在部署前评估LLM法官门是否具有实际的判别能力，有助于避免在无效的法官门上浪费计算资源。
...
### [133] [A Multi-Agent Platform for Automated Enterprise Analytics and Insight Generation](https://arxiv.org/abs/2608.18740)
*Manoj N M,Vijayakrishna S,Manjunath Srinivas,Rohit Pahan*

Main category: cs.AI

TL;DR: 本文提出一个基于CrewAI的多智能体框架，用于对话式商业智能。该框架包含五个专业AI代理，按顺序Pipeline处理自然语言查询、检索分析数据、通过MCP生成可视化，并提供可操作洞察。平台采用纵深防御安全架构实现多租户数据隔离，并提供查询参数化机制将对话洞察转化为可复用仪表板组件。


<details>
  <summary>Details</summary>
Motivation: 解决传统商业智能系统需要用户具备SQL或数据分析专业知识的问题，通过对话式界面降低数据分析门槛，实现自然语言驱动的自助式商业智能分析。

Method: 构建基于CrewAI的五智能体顺序Pipeline架构：自然语言处理、数据检索、数据分析、可视化生成、报告聚合。引入MCP协议实现可视化生成，采用防御纵深安全架构保障多租户数据隔离，设计查询参数化机制支持仪表板组件复用。

Result: 在300个端到端测试用例上达到95.3%功能准确率，平均响应延迟24秒，LLM-as-a-Judge评估响应质量4.52/5.0，幻觉率仅7%（93%无幻觉）。相比单智能体基线，准确率提升22.6个百分点，质量提升20.2%。跨四个LLM后端评估和人类专家验证确认架构可推广性。消融实验表明数据分析代理和报告聚合代理是输出质量的主要驱动因素。

Conclusion: 多智能体协作架构能有效提升对话式商业智能的准确性和质量，数据分析和报告聚合智能体对最终输出质量至关重要。该框架在企业级场景中具有良好的应用前景。
...
### [134] [Metrics That Write Themselves: Evolving an Evaluator from Its Own Blind Spots](https://arxiv.org/abs/2608.18744)
...
### [135] [Pairwise Logical Selection of Enthymeme Completions under Semantic-Link Uncertainty](https://arxiv.org/abs/2608.18820)
*Xuyao Feng,Antonis Bikakis*

Main category: cs.AI

TL;DR: 本文研究论辩中缺失前提或论断的对选择问题，提出PWAL方法通过边缘化逻辑阻抗 over替代语义链接配置来改进选择准确性，在五个基准任务上显著优于Top-Link基线方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于：现有自然语言方法虽能识别或生成候选项，但通常无法暴露所选候选如何完成推理；而基于逻辑的方法通常假设所需公式和背景知识可用。实际挑战是如何在自然语言论辩中有效地进行逻辑推理选择。

Method: 研究方法包括：(1)将Top-Link从缺失前提选择扩展到缺失论断选择；(2)用逻辑阻抗分数替代二元蕴含结果；(3)使用加权Partial MaxSAT在单一高置信度语义链接配置下进行最优链接选择；(4)提出PWAL方法，固定翻译公式并对替代跨公式语义链接配置进行逻辑阻抗边缘化处理。

Result: 实验结果表明：PWAL相对于Top-Link在五个任务上严格准确率提升2.95-30.86个百分点，平局率降低4.57-58.00个百分点；当平局获半数 credit时，准确率仍提升0.45-6.04个百分点。PWAL还记录了翻译公式、采样链接配置及每个比较的阻抗成分，提供了透明的分数追溯。

Conclusion: PWAL通过在替代语义链接配置上边缘化逻辑阻抗，成功解决了现有神经符号管道中链接配置单一的问题。实验在五个任务上显著提升了准确率（2.95-30.86个百分点）并大幅降低了平局率（4.58-58.00个百分点），同时提供了透明的可追溯性。该方法为论辩挖掘中的缺失成分选择提供了更可靠和可解释的解决方案。
...
### [136] [Verifiable abstention makes AI leak diagnosis accountable in water distribution networks](https://arxiv.org/abs/2608.18836)
*Tianwei Mu,Yue Wang,Mingzhe Yuan,Manhong Huang,Wenhong Wang,Xuerui Yin,Qing Luo,Min Xiao,Hui Yang,Jun Li,Dan Xue*

Main category: cs.AI

TL;DR: 本文提出一种基于可验证弃权的泄漏定位决策框架解决供水管网泄漏问题。框架包含物理驱动的执行智能体（基于数字孪生验证假设）和独立监督智能体（大语言模型审计员核对可验证合同），实现决策可追溯可审计。在现场级噪声下，决策精度从32%的强制基线提升至96%；基准测试仅对33个泄漏中的4个采取行动，全部正确；真实泄漏事件注册表产生5次挖掘调度，3次正确，区域精度下恢复率达44%。可验证弃权为自主供水基础设施运营提供了可辩护的路径。


<details>
  <summary>Details</summary>
Motivation: 供水公用事业因泄漏损失大量处理水，但通常不信任人工智能定位系统来派遣维修队伍。核心问题不在于准确性，而在于问责制：现有方法无法证明何时不应采取行动。传统方法在整个管网"猜测"泄漏位置，无法为挖掘作业提供正当理由。

Method: 将泄漏定位重新定义为可验证弃权下的决策问题。构建两个核心智能体：(1)物理驱动的执行智能体，针对数字孪生验证泄漏、需求、传感器、阀门等假设并证伪；(2)独立监督智能体配备大语言模型审计员，根据可验证合同检查证据，决定批准调度、要求补充证据或弃权。系统仅在证据充分时才会触发挖掘调度。

Result: 在现场级噪声条件下，强制基线32%的精度提升至96%（对已行动事件的决策精度）；独立生成的基准测试中，系统仅对33个泄漏中的4个采取行动，全部正确；194个真实泄漏事件注册表（带数字孪生模拟的压力和流量）产生5次挖掘调度，其中3次正确，完整区域精度下达到44%的恢复率。

Conclusion: 可验证弃权的问责框架弥合了准确性与可问责性之间的鸿沟。该方法通过仅在证据充分时行动并提供完整审计轨迹，使人工智能系统能够在实际基础设施运营中部署，为自主供水基础设施运营提供了可辩护、可验证的可行路径。
...
### [137] [ORBITER: Conflict-Aware Decision-Making for Agentic Last-Mile Delivery](https://arxiv.org/abs/2608.18846)
*Mingzhao Li,Chenxi Liu,Yan Zhao,Hao Miao*

Main category: cs.AI

TL;DR: ORBITER是一个基于LLM的智能代理系统，用于解决最后一公里配送中的下一个订单决策问题。该系统通过决策点建模快递员服务状态，结合固定排序器和LLM推理来做出更可靠的配送决策。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法虽能预测快递员服务序列，但无法解释下一个订单的决策过程。直接使用LLM作为预测器时，对任务呈现方式敏感且决策不可靠。需要一种能够明确推理空间、时间和行为线索的方法。

Method: ORBITER采用代理架构：(1)通过决策点建模快递员服务状态，包含时空状态和可见订单；(2)固定排序器对候选订单排名；(3)结构化报告识别排名分歧；(4)LLM使用任务特定工具收集证据；(5)独立评论家根据证据检查决策。

Result: 在四个城市的数据集上进行了广泛评估，ORBITER比现有最先进的基线方法性能提升高达9.2%。

Conclusion: ORBITER通过结合LLM推理和结构化验证机制，有效解决了最后一公里配送中的下一个订单决策问题，证明了基于代理的方法在物流优化领域的有效性。
...
### [138] [SkillGate: Training In-Policy Skill Selection in Long-Horizon Agents](https://arxiv.org/abs/2608.18852)
*Qingyao Li,Wenxiang Jiao,Shuai Shao,Kangning Zhang,Yuan Lu,Yi Guo,Weiwen Liu,Weinan Zhang,Yong Yu*

Main category: cs.AI

TL;DR: 论文提出SkillGate方法解决agent框架中技能选择的训练信号缺失问题。核心问题是传统outcome-rewarded RL无法训练技能选择决策，因为token级别的信用分配存在"选择器信用饥饿"现象——命名技能的少量token承载的损失权重随轨迹长度衰减，导致正确选择被错误惩罚。SkillGate通过将token支持分割为两个独立信用通道来解决这个问题。


<details>
  <summary>Details</summary>
Motivation: agent框架将程序性知识打包为技能（按需读取的指令文件），公共库包含数千个技能。agent需要在episode中途决定读取哪个技能，但现有方法缺乏训练这一决策的信号。默认方案outcome-rewarded RL存在结构性缺陷，无法有效训练技能选择器。

Method: 识别并命名"选择器信用饥饿"(selector credit starvation)问题：在广播式序列级优势下，少量命名技能的token承载的损失权重 vanishing，且继承的信用随轨迹延长愈发错误符号化。SkillGate通过构造消除此问题：1)将token支持划分为两个独立信用通道；2)结果信用仅到达执行token；3)单独的动作-局部优势精确到达技能命名token，仅当轨迹单次读取正确时为正。

Result: 在5个agentic基准测试、16候选技能 slate条件下，SkillGate将9B策略从40.8%提升到53.2%的trial成功率，优于相同预算的结果奖励alone，减少2/3的误导性候选暴露，且读取更少技能。

Conclusion: 技能选择是轨迹中最有价值的决策之一，但现有RL方法无法正确训练。SkillGate通过双通道信用分配机制解决了选择器信用饥饿问题，显著提升agent在多技能场景下的表现。
...
TL;DR: 本文提出了一种名为Class Expression Simplifier (CES)的新算法,用于在描述逻辑中简化OWL类表达式的句法复杂性,同时保留形式语义。该算法通过系统地应用重写规则来消除冗余,生成更紧凑、更易读的表达形式,并在OWLAPY Python框架中实现。


<details>
  <summary>Details</summary>
Motivation: 类表达式学习通常会产生复杂的OWL类表达式,难以解释和推理。需要一种有理论基础的简化方法来在保持形式语义的前提下降低复杂性,使本体驱动的应用更易访问、可维护和可扩展。

Method: CES算法系统地应用重写规则来消除冗余,识别更简单但等价的表达式。算法在两个中等规模的本体上学习到的类表达式进行评估,测量推理效率和冗长程度的改善。

Result: 实验证明CES在推理效率方面有可测量的提升,并显著减少了表达式的冗长程度,同时不改变逻辑蕴含关系。

Conclusion: CES为使本体驱动的应用更易访问、更易维护和更可扩展做出了贡献,对知识图谱构建、语义搜索和Web规模推理具有直接意义。该算法已作为OWLAPY开源Python框架的一部分公开发布。
...
TL;DR: TestifAI是一个深度学习测试框架，用于高效准确地估计模型在组合扰动下的鲁棒性。通过部分模型层析成像技术，利用低阶（1-2个扰动）测试结果预测高阶（3-4个扰动）测试结果，将推理次数减少60-80%，同时保持7%以下的估计误差。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在安全关键领域（如自动驾驶）的部署增加，相关风险也增大。深度学习模型需要进行彻底的测试以确保其正确行为。现有测试框架缺乏系统探索和总结组合扰动空间下模型鲁棒性的手段。

Method: 提出TestifAI框架，允许用户将操作条件指定为语义输入扰动（如图像模糊、亮度、缩放）和离散严重程度的空间。通过部分模型层析成像技术，从仅应用少量扰动的测试中重构多扰动空间下的模型行为。训练辅助模型利用最多两个扰动的测试结果来估计至少三个扰动的鲁棒性，避免执行指数级数量的测试。

Result: 在五个图像和语言分类任务上的实验表明，TestifAI可以从低阶（1-2个扰动）观测预测高阶（3-4个扰动）测试结果，聚合鲁棒性估计误差小于7%，同时减少60-80%的推理次数。

Conclusion: TestifAI能够实现对组合扰动的高效准确鲁棒性估计，通过利用低阶测试结果预测高阶扰动效果，显著降低计算成本同时保持可接受的估计精度。
...
TL;DR: 本文提出了一个适用于智能体辩论的评判理论框架，探讨两种辩论评判方法：LLM作为评判者和计算论证形式语义学，并比较其性能表现。


<details>
  <summary>Details</summary>
Motivation: 辩论已成为提升智能体性能和可解释性的有效方法，但现有辩论评判多由外部LLM事后决定，缺乏形式化保证和原则性框架。

Method: 识别辩论评判应满足的形式化属性（可复现性、鲁棒性、可解释性等），分别在声明验证场景下对两种评判方法进行形式化和实验分析。

Result: 两种方法在准确率上表现相似，但LLM评判方法缺乏形式语义学方法所提供的形式化保证。

Conclusion: 计算论证语义学是辩论驱动AI中原则性评判的理想候选方案。
...
TL;DR: 研究探索了前沿浏览器大型语言模型（LLM）在从学术论文中提取细致语境化数据的能力，并设计了四个递进的工作流程来评估其性能。


<details>
  <summary>Details</summary>
Motivation: 从学术论文中准确提取细致、语境化的数据既费时又费力，需要找到一种可扩展的方法来辅助科学数据整理，同时保持专家监督。

Method: 设计了四个递进的工作流程：1）专家编写提示词+论文；2）LLM自主编写提示词；3）自主发现研究文献；4）根据已发布指南创建新数据集。

Result: 工作流程1中大多数前沿LLM表现良好，但在理解科学语境和细微差别方面存在困难；工作流程2中LLM自编提示词效果几乎与专家编写的一样；工作流程3中代理工具容易遗漏或幻觉引用；工作流程4中LLM创建的数据集与人类专家判断高度匹配，但仍需要人工参与。

Conclusion: 研究定义了一种可审计的分工模式：专家设定证据标准，模型交叉检查重复提取，研究人员解决争议案例，这提供了一条在不移交专家监督的情况下扩展科学数据整理的实用路径。
...
TL;DR: Eureka是一个任务条件化的元智能体架构，将长 horizon 任务编译为具有明确接受语义的动态义务图，通过递归 horizon 规划、架构提升和最小充分编译形成宏智能体。实验表明该系统完成170/170递归任务，生成3948个证书无假阳性，并在多个基准上展现优异性能。


<details>
  <summary>Details</summary>
Motivation: 当前智能体架构难以有效处理长 horizon 复杂任务，缺乏将任务认知结构与架构匹配的能力，导致科学发现和数学推理等领域的应用受限。

Method: 提出任务条件化元智能体架构 Eureka，包含：1) 动态义务图编译；2) 宏智能体形成机制（状态、记忆、算子、工具、验证器、本地拓扑）；3)  receding-horizon 规划；4) 架构提升；5) 最小充分编译；6) 成本效益门控演化。理论建立关于 regret、规划失效、分摊、子树接口、可序列化性和验证的结果。

Result: 实验结果：完成170/170递归任务；生成3948个证书无假阳性；主动上下文将中位数输入从9490压缩到4005 token；增量处理在12000个任务中避免65.38%重计算；16000并发执行一致序列化；成功实例化 Theory-Discovery Agent 和 Math/Conjecture Agent，后者将 Suzuki localized Weil 二次型的 positivity 证书推进到 0 < a ≤ 69/200 ≈ 0.345。

Conclusion: 提出一种通用的任务条件化元智能体架构，证明科学智能体能力不仅取决于基础模型，还取决于架构能否匹配任务的认知结构。该架构在复杂任务规划、验证和科学发现方面展现显著优势。
...
TL;DR: 本文研究了LLM代理在AI训练中的两种能力：执行级能力（策略内的局部调整）和策略级能力（根据实验证据重新评估高层判断）。通过对大量公开的后训练轨迹分析，发现代理的策略在训练初期就已锁定，剩余预算只用于策略内的局部调整。研究了三种解释：经验、指导和推理，并进行递进干预实验。结论：代理缺乏在执行过程中自发重新评估策略的机制。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理能够端到端地后训练LLM，包括编写代码、启动训练、评估检查点和改进下游性能。但作者认为这混淆了两种不同的能力：执行级能力和策略级能力。通过分析大量公开的后训练轨迹，发现代理的策略在训练初期就已锁定，这促使作者探究其根本原因。

Method: 1) 分析大量公开的后训练轨迹数据；2) 进行三种递进干预实验：经验驱动脚手架、人类指导、额外推理计算

Result: 1) 经验驱动脚手架提升执行能力（GSM8K提升12.6分，HumanEval提升40.8分），但策略保持不变；2) 人类指导可有效重定向初始策略，但训练开始后代理仍会回到局部调整循环；3) 额外推理计算在简单任务上有收益，但在最困难的任务上几乎无增益

Conclusion: 代理所缺乏的不是经验、指导或推理计算，而是一种在执行过程中自发重新评估策略的机制。这一发现揭示了当前AI-for-AI系统的根本局限性。
...
TL;DR: 提出了一种新的风险度量——Wasserstein熵风险值(WEVaR)，使用最优传输球替代相对熵球，以捕获传统熵风险值无法触及的可达灾难场景，并将其应用于安全智能体的鲁棒动态规划。


<details>
  <summary>Details</summary>
Motivation: 标准熵风险值的相对熵球无法到达名义模型认为不可能的灾难情景，这对于必须对冲最坏情况的安全智能体而言是根本性缺陷。

Method: 使用最优传输(Wasserstein)球定义风险度量，推导其变分对偶形式，通过信念熵驱动传输半径，得到封闭形式的鲁棒动态规划算子。

Result: 新度量具有变分对偶形式，在风险层次中占据明确位置，可证明地考虑了熵度量忽略的可达灾难。该方法产生闭环形式的鲁棒动态规划算子，谨慎性随信念清晰而收缩，具有 certified safety sandwich 和锐利的安全切换机制。

Conclusion: Wasserstein熵风险值为安全智能体提供了基于信念不确定性的谨慎-大胆平衡原则框架，具有经过认证的安全保证和安全切换机制。
...
TL;DR: 作者认为LLM助手的错误纠正无法持久化是一个运维问题而非工具问题，需要建立一套七原则的运维 discipline 来管理纠正措施，包括版本控制、来源追踪、复发监控、计数器指标和过时规则退休等机制。


<details>
  <summary>Details</summary>
Motivation: 专家纠正LLM错误时，纠正通常随会话结束而消失，错误会再次出现；虽然持久化纠正的技术机制已存在，但缺乏管理它们的规范（版本控制、来源追踪、复发监控、计数器指标、过时规则退休）。

Method: 将LLM技术栈映射到传统计算机系统层次（冻结硅、固件、可加载模块、持久配置、易失性内存），识别映射失败之处（随机生成、概率性配置、缺乏通用的退休/验证阶段），从中推导出一个以错误循环为核心的七原则运维 discipline，并用三个案例说明。

Result: 识别出LLM运维与传统系统运维的关键差异：随机性、概率性绑定、缺乏验证阶段；建立了包含七原则的运维 discipline 框架。

Conclusion: 需要建立测量框架来量化该方法的效果，并进行实验研究来验证。
...
TL;DR: 本文提出语言模型评估的新视角：区分"能力"(capability)与"精度"(precision)，认为当前基准测试错误地关注能力而忽视精度。精度衡量模型输出的集中程度，是当前前沿模型真正的差异化因素。


<details>
  <summary>Details</summary>
Motivation: 当前前沿语言模型在准确率上已饱和，系统间的真正差异在于输出的稳定性和一致性，而非平均表现。

Method: 提出通过在固定温度下多次运行确定性评分任务，计算每项任务的成果一致性来测量精度，无需模型参与的评分器。

Result: 实验表明：单一规则可完全弥补差距(0/5→5/5)，但从规则本身构建的任务未发现价值，因为前沿模型已内化显性最佳实践。

Conclusion: 精度而非能力是前沿系统的真正区分因素；基准文化应测量离散度而非集中趋势；测量方法具有决策指导价值，可区分可纠正的稳定失败与需更换模型的散射失败。
...
TL;DR: VLA框架通过链接隐藏状态与公开行为来监控和引导语言模型代理间的隐蔽通信渠道，在多智能体拍卖基准测试中实现了高检测精度和有效干预。


<details>
  <summary>Details</summary>
Motivation: 语言模型代理可通过不可见的连续隐藏状态进行隐蔽的有害协调，需要方法来监控和引导这些私人通信渠道。

Method: 1)中性三层监控器：表征异常检测、反事实行为分布影响、稀疏自编码器解释支持；2)可操纵性框架：黑盒行为指令与白盒匹配中性反事实；3)在同质/异质模型对、多智能体规模下评估干预效果。

Result: 顺序监控器对同质智能体AUROC达0.993，异质智能体对达0.854；Qwen3-0.6B拍卖中，白盒 steering实现100%投标分布恢复，降低47.3个百分点的共谋低投标行为。

Conclusion: 所评估的私人通道攻击可在不训练攻击样本情况下被监控，且在有匹配反事实访问时可被缓解。
...
### [151] [Contracting for LLM Delegation: Moral Hazard in Technology and Effort Choice](https://arxiv.org/abs/2608.18232)
*Nanda Kishore Sreenivas,Kate Larson*

Main category: cs.MA

TL;DR: 将标准委托代理框架扩展到代理从一套具有不同成本-能力配置的技術（如LLM）中选择的场景，模型输出质量与努力的关系为凹的饱和函数，推导最优线性合约，并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 在LLM时代，代理需要同时选择模型和努力水平（如token预算），现有委托代理框架缺乏对技术选择的建模

Method: 将输出质量与努力建模为凹的饱和函数，推导最优线性合约；使用开源LLM在MATH和MMLUPro基准上校准模型；用多臂老虎机算法验证理论和均衡策略

Result: 代理的最佳响应由触发技术切换的阈值奖励份额表征；委托人和代理人的策略与理论均衡高度一致

Conclusion: 简单的线性合约可以有效激励代理工作流中复杂的技术感知委托行为
...
### [152] [A Locally Deployable Tool-Grounded LLM Multi-agent Framework for Automating Methane Emission Analysis and Reporting](https://arxiv.org/abs/2608.18473)
*Yang Yan,Zifan Zhou,Xuan Wang,Erum Hassan,Bilguunzaya Mijiddorj,Jie Cao,Bin Li,Binbin Weng*

Main category: cs.MA

TL;DR: 开发了一种本地部署的、基于工具的LLM多智能体框架，用于低成本甲烷传感和野外观测。该框架将LLM智能体作为工作流协调器，连接现场测量、气象数据、确定性传感器处理、高斯羽流反演和报告生成。部署于废水处理设施、垃圾填埋场和油气场所等真实环境。


<details>
  <summary>Details</summary>
Motivation: 甲烷现场监测需要整合采样设计、气象解释、传感器处理、羽流分析、可视化和报告等多个步骤，但这些步骤通常分散在不同的专家驱动的工作流中。需要一种集成方法来降低时间、劳动力和数据安全障碍。

Method: 使用LLM智能体作为工作流协调器，连接现场测量、气象数据、确定性传感器处理程序、高斯羽流反演和报告生成，而非直接估计甲烷浓度或排放量。

Result: 工作流路由和参数提取准确率92.0%，排放率估计和羽流预测成功率85.0%，可编辑报告生成成功率95.0%。将工作流时间从小时级缩短到分钟级，降低了人工协调和提示工程要求，大部分处理可在本地完成以减少敏感数据暴露。

Conclusion: 基于工具的LLM协调可以降低甲烷现场监测的时间、劳动力和可用性障碍，同时提高数据安全性。这种方法在多种真实环境部署中表现有效。
...
### [153] [Bayesian Partner Modelling enables Adaptive Replanning for LLM Coordination](https://arxiv.org/abs/2608.18490)
*Harsh Goel,Aditya Sai Ellendula,Vaishnav Tadiparthi,Ehsan Moradi Pari,Hossein Nourkhiz Mahjoub,Sandeep P. Chinchali*

Main category: cs.MA

TL;DR: 本文提出BayesBeliefAgent方法，解决多智能体LLM系统中智能体难以快速响应队友策略变化的问题。该方法结合分层LLM规划器和贝叶斯跟踪模块，仅在队友行为与推断技能直接矛盾时才中断当前技能执行。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1) 将伙伴跟踪作为被动上下文处理，导致智能体意识到策略变化但响应迟缓；2) 无差别地重新规划，造成计算资源浪费。智能体在执行多步骤或时间延展技能时，常常在公开证据显示伙伴已改变技能后仍继续执行过时计划。

Method: 提出BayesBeliefAgent方法：1) 使用分层LLM规划器进行任务规划；2) 配备贝叶斯跟踪模块推断伙伴技能；3) 采用矛盾条件控制策略——仅当伙伴行为与推断技能直接矛盾时才中断当前技能并触发重新规划。

Result: 在Overcooked环境的基准测试中，矛盾条件控制显著缩小了信念-行动差距（拥有正确伙伴估计的智能体执行非互补技能的比例），同时与启发式方法相比，需要的重新规划次数减少了一个数量级。

Conclusion: 通过仅在检测到直接矛盾时才中断执行的策略，BayesBeliefAgent实现了高效的多智能体协作，在保持高任务性能的同时大幅降低了不必要的重新规划开销。
...
### [154] [Computing All Optimal Partial $p$-Wasserstein Matchings on the Line](https://arxiv.org/abs/2608.18875)
*Sebastian Angrick,Jacobus Conradi,Mónika Csikós,Niko Hastrich,Danny Mittal,André Nusser,Krzystof Onak,Sharath Raghvendra*

Main category: cs.CG

TL;DR: 本文提出了一种基于FFT的数据结构，用于平衡区间传输查询，解决了p≥1时一维最优部分传输计划计算的二次时间瓶颈问题，算法时间复杂度为O(p n log² n)，并提供了C++实现。对于p=∞的情况，证明了任何亚二次时间算法都会违背(min,+)-卷积假设。


<details>
  <summary>Details</summary>
Motivation: 当p=1时，一维最优部分传输计划可在O(n log n)时间内计算；但对于p>1，现有方法需要Ω(n²)时间，存在二次时间瓶颈问题，需要更高效的算法。

Method: 通过FFT技术实现平衡区间传输查询，将计算复杂度从Ω(n²)降低到O(p n log² n)时间。

Result: 算法在多种合成实例上性能优于现有最先进方法，并提供了开源C++实现。

Conclusion: 建立了p=∞条件下的条件lower bound，表明该问题在计算复杂度上比完整最优传输问题更难。
