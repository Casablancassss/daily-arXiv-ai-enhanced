...
TL;DR: 本文提出了一个名为DarkDriving的新型基准数据集，用于自动驾驶的低光增强。现有的真实低光增强基准数据集只能在较小范围内控制曝光且在静态场景中采集，当前夜间驾驶数据集的暗图像没有精确对齐的白天对应物。在大型真实封闭驾驶测试场（面积69英亩）中，通过提出的自动昼夜轨迹跟踪姿态匹配（TTPM）方法，采集了首个自动驾驶黑暗环境下的真实昼夜对齐数据集。DarkDriving数据集包含9,538对精确对齐位置和空间内容的昼夜图像，对齐误差仅为几厘米。


<details>
  <summary>Details</summary>
Motivation: 低光条件对自动驾驶中基于视觉的感知系统构成挑战。现有的真实低光增强数据集只能在较小曝光范围内采集且为静态场景，当前夜间驾驶数据集的暗图像没有精确对齐的白天对应物。在动态驾驶场景中采集真实世界的昼夜对齐数据集极其困难，显著限制了这一领域的研究。

Method: 提出自动昼夜轨迹跟踪姿态匹配（TTPM）方法，在大型真实封闭驾驶测试场（面积69英亩）中采集数据。该方法能够自动匹配昼夜图像，实现精确对齐。

Result: DarkDriving数据集包含9,538对昼夜图像，精确对齐位置和空间内容，对齐误差仅为几厘米。每对图像都手动标注了目标2D边界框。数据集引入四个感知相关任务，包括低光增强、广义低光增强以及低光环境下的2D检测和3D检测。实验表明该数据集可泛化到nuScenes等其他低光驾驶环境。

Conclusion: DarkDriving数据集为评估自动驾驶低光增强提供了全面基准，并可泛化到其他低光驾驶环境（如nuScenes）进行暗图像增强和目标检测促进。
...
TL;DR: 本文提出SSP-SAM框架，通过引入语义-空间提示(SSP)编码器，将SAM的分割能力与语言理解相结合，用于指代表达分割任务。该方法整合视觉和语言注意力适配器，增强指代物表示，生成高质量提示引导SAM生成精确掩码。


<details>
  <summary>Details</summary>
Motivation: SAM在通用图像分割方面表现出色，但对自然语言理解能力有限，限制了其在指代表达分割(RES)中的直接应用。研究旨在充分利用SAM的分割能力，同时赋予其语言理解能力。

Method: 提出SSP-SAM框架，包含SSP编码器。该编码器整合视觉和语言注意力适配器：视觉适配器突出视觉特征中的显著物体，语言适配器突出语言特征中的区分性短语。这增强了提示生成器的指代物表示，生成高质量SSP引导SAM进行精确分割。

Result: 在广泛使用的RES和GRES基准数据集上验证了方法的优越性。在严格阈值(如Pr@0.9)下仍能实现高精度的分割掩码。在PhraseCut数据集上的评估表明，开放词汇场景下的性能优于现有最佳RES方法。

Conclusion: SSP-SAM无需额外修改即可自然支持广义指代表达分割(GRES)，处理零个、一个或多个指代对象的情况。实验表明该方法达到了最先进水平，代码和模型检查点已开源。
...
TL;DR: 本文介绍了CytoSyn，一个用于组织病理学图像生成的基础潜在扩散模型，能够生成高质量、多样化的H&E染色组织病理图像。模型在超过10,000个TCGA诊断全切片图像上训练，可生成32种不同癌症类型的图像。


<details>
  <summary>Details</summary>
Motivation: 计算病理学近年来取得显著进展，但主要用于特征提取的自我监督基础模型较多，而专门为组织病理学设计的生成式基础模型较为稀缺。现有特征提取器无法完成虚拟染色等任务，因此需要开发专门的生成式基础模型。

Method: 提出CytoSyn-v2模型，采用潜在扩散模型架构，通过方法改进、训练集扩展、采样策略优化和切片级过拟合处理等技术手段提升性能，并与PixCell模型进行深入对比分析。

Result: CytoSyn在生成逼真多样的H&E染色组织病理图像方面达到最先进水平。模型在仅使用肿瘤切片训练的情况下，仍能有效生成炎症性肠病图像，展示了良好的泛化能力。研究还发现扩散模型和性能指标对JPEG压缩等预处理细节高度敏感。

Conclusion: 成功开发了CytoSyn-v2这一最先进的基础潜在扩散模型，为组织病理学图像生成提供了新工具。模型权重、训练验证数据集及合成图像样本已公开共享，有望推动该领域研究进展。
...
TL;DR: 提出了O2MAG，一种无需训练的少样本异常生成方法，利用自注意力迁移和文本引导合成更真实的异常图像，用于工业异常检测。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测中正常图像丰富但异常图像稀缺，现有少样本异常合成方法需耗时的训练过程，且难以学习到真实异常的分布，限制了异常检测模型的效能。

Method: O2MAG通过三个并行扩散过程的自注意力嫁接来操作，并结合异常掩码缓解前景-背景查询混淆。引入异常引导优化来缩小编码异常文本与真实异常语义之间的语义差距。采用双注意力增强来强化掩码区域的自注意力和交叉注意力，缓解掩码内微弱异常合成问题。

Result: 在下游异常检测任务上验证了O2MAG的有效性，展示了其优于现有最先进方法的性能。

Conclusion: O2MAG能够合成与真实异常分布高度一致的文本引导异常，有效支持下游异常检测任务，无需耗时的训练过程。
...
TL;DR: Q-Drift是一种针对量化扩散模型的采样器端校正方法，通过将量化误差视为隐式随机扰动，推导保持边缘分布的漂移调整，在仅需5次校准的情况下显著改善生成质量。


<details>
  <summary>Details</summary>
Motivation: 后训练量化(PTQ)是部署大型扩散模型的实用路径，但量化噪声会在去噪轨迹上累积并降低生成质量。现有方法难以有效处理这种时序累积的量化误差。

Method: 将量化误差建模为每步去噪的隐式随机扰动，推导保持边缘分布的漂移调整。通过校准估计时间步方差统计量，仅需5对全精度/量化模型的校准运行即可。

Result: 在6个不同的文生图模型、3种采样器和2种PTQ方法上，Q-Drift在大多数情况下改善了FID分数，在PixArt-Sigma上最高降低4.59个FID点，同时保持CLIP分数。

Conclusion: Q-Drift作为一种即插即用的采样器校正方法，可与常见采样器、扩散模型和PTQ方法兼容，在推理时产生可忽略的开销，为量化扩散模型的质量提升提供了一种原则性解决方案。
...
TL;DR: 本文提出TOGA框架，通过在训练阶段引入异构图教师来增强CLIP的少样本学习能力。该方法不改变推理时的 adapter 结构，而是在训练时利用图 Transformer 进行跨模态推理，将细粒度的patch关系和文本信息注入Tip-Adapter的key-value缓存中，实现零推理成本的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的少样本学习方法（如Tip-Adapter）依赖全局单模态特征向量，忽略了细粒度的patch关系及其与类文本的结构对齐。现有方法在推理时仍需额外计算成本，无法兼顾性能与效率。

Method: 提出一种非对称训练框架，包含三个核心组件：(1) 异构图教师：整合多尺度视觉patches和文本提示构建统一图结构；(2) 模态感知图Transformer(MGT)：执行深度跨模态推理；(3) 判别性节点过滤：提取高保真类别特征。采用缓存感知双目标策略，将关系知识直接监督到Tip-Adapter的key-value缓存中，推理时丢弃图教师，保持与Tip-Adapter相同的推理效率。

Result: 在1-16-shot标准基准上取得新的最先进性能。消融实验验证了辅助图监督、文本引导推理和节点过滤三个关键组件的有效性。

Conclusion: TOGA在不增加推理成本的前提下，显著提升了CLIP少样本分类的性能，通过图结构建模细粒度跨模态关系是提升少样本学习效果的有效途径。
...
TL;DR: 本文提出了一种可解释的图像美学评估框架，基于人类可理解的美学概念构建可解释子空间，并引入残差预测器捕捉细微影响，在保持预测性能的同时提供透明的美学判断解释。


<details>
  <summary>Details</summary>
Motivation: 现有的图像美学评估模型虽然预测性能强，但缺乏对预测因素的解释；而用户理解图像为何被认为美或不美与分数本身同样重要，因此需要提高IAA的可解释性。

Method: 提出可解释IAA框架，基于人类可理解的美学概念学习，以构建可解释子空间作为模型基础；同时引入简单有效的残差预测器捕捉超出显式概念的细微美学影响。

Result: 在摄影和艺术数据集上的实验表明，该方法达到具有竞争力的预测性能，同时提供透明、人类可理解的美学判断。

Conclusion: 所提方法在保持竞争力的预测性能的同时，实现了透明、人类可理解的美学判断解释，验证了基于美学概念的可解释框架的可行性。
...
TL;DR: 将LLM的高级推理能力扩展到多模态大语言模型(MLLMs)面临重大挑战，主要原因是高质量长链推理数据稀缺和训练流程不完善。Insight-V++通过统一的多智能体视觉推理框架解决了这一问题。


<details>
  <summary>Details</summary>
Motivation: 将LLM在扩展测试时推理中取得的卓越可靠性和先进能力扩展到多模态大语言模型(MLLMs)，但由于高质量长链推理数据和优化训练流程的严重稀缺，这一扩展仍然是一项重大挑战。

Method: 1) 可扩展的数据生成流水线，具有多粒度评估功能，可自主合成图像和视频领域的结构化复杂推理轨迹；2) 双智能体架构，包括推理智能体(执行广泛的分析链)和总结智能体(批判性评估和提炼最终结果)；3) 两种新算法ST-GRPO和J-GRPO，增强空间-时间推理并提高评估鲁棒性；4) 利用总结智能体的可靠反馈，通过持续自改进循环引导迭代推理路径生成过程。

Result: 在LLaVA-NeXT和Qwen2.5-VL等基础模型上，在具有挑战性的图像和视频推理基准上实现了显著的性能提升，同时在传统感知聚焦任务上保持了强大的能力。

Conclusion: Insight-V++成功地将LLM的高级推理能力扩展到多模态领域，通过可扩展的数据生成、双智能体架构、创新的强化学习算法和自改进机制，为多模态视觉推理建立了新的方法论。
...
TL;DR: VLM-AutoDrive通过整合元数据字幕、LLM描述、VQA对和CoT推理监督，将预训练VLM适配到高保真异常检测，在Nexar dashcam视频上实现Collision F1从0.00到0.69的显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在驾驶场景中表现不佳，因为领域和时间对齐问题。ego-centric dashcam footage增长迅速，但碰撞和近碰撞等安全关键事件短暂且罕见，难以被通用视觉模型捕获。

Method: 该框架整合了元数据衍生的字幕、LLM生成的描述、视觉问答(VQA)对和思维链(CoT)推理监督，实现领域对齐和可解释学习。对NVIDIA的Cosmos-Reason1 7B (CR1)进行微调。

Result: 零样本设置下CR1的Collision召回率接近零；使用VLM-AutoDrive微调后，Collision F1从0.00提升到0.69，总体准确率从35.35%提升到77.27%。

Conclusion: VLM-AutoDrive是一个可扩展的框架，能够将通用VLM适配到安全关键的、时间局部化的感知任务中。在Nexar dashcam视频上实现了Collision和Near-Collision检测的重大改进，同时产生可解释的推理轨迹，弥合了自动驾驶中感知、因果关系和决策推理之间的差距。
...
TL;DR: 本文提出了MicroVision数据集，一个用于检测弱势道路用户（VRU）和静止微出行车辆（MMV）的开源图像数据集。该数据集包含8000多张高清图像和30000多个标注，从VRU视角拍摄，涵盖行人、骑行者、电动滑板车使用者以及停放的自行车和电动滑板车。


<details>
  <summary>Details</summary>
Motivation: 现有开放图像数据集在VRU和MMV方面缺乏关注度和多样性，例如将行人和MMV骑行者统一标注为"人"，且缺乏来自人行道、自行车道等仅VRU活动区域的图像数据。这些问题制约了交通安全检测和规划模型的发展。

Method: 数据集在瑞典哥德堡录制，跨越整年，包含近2000个独特交互场景。提供了基于最先进架构的基准目标检测模型进行训练和评估。

Result: 基准模型在测试集上实现了最高0.723的平均精度（mAP），能够有效区分不同的VRU和MMV类型。

Conclusion: MicroVision数据集和预训练模型可支持交通安全领域区分不同类型的VRU和MMV，或帮助监控系统识别微出行工具的使用情况，为相关研究和应用提供了重要的数据基础。
...
TL;DR: LRConv-NeRV是一种高效的视频神经表示方法，通过将卷积解码器中的部分3x3卷积层替换为低秩可分离卷积，在大幅降低计算复杂度和模型体积的同时，仅带来可忽略不计的画质损失。


<details>
  <summary>Details</summary>
Motivation: NeRV虽然提供了一种新的视频编码范式，但其卷积解码器计算量大、内存占用高，难以部署在资源受限的环境中。需要一种高效的变体来降低计算和存储开销。

Method: 将选定的密集3x3卷积层替换为结构化低秩可分离卷积，在解码器架构中进行端到端训练。从最大解码器阶段逐步应用低秩分解，实现重建质量和效率之间的可控权衡。

Result: 仅在最终解码器阶段应用LRConv可将解码器复杂度降低68%（从201.9到64.9 GFLOPs），模型大小减少9.3%，同时带来约9.2%的码率降低。在INT8量化下仍能保持接近基准的重建质量。

Conclusion: LRConv-NeRV在效率和画质之间实现了更好的权衡，是一种适用于低精度和资源受限场景的高效神经视频解码架构替代方案。
...
TL;DR: 本文提出CycleCap方法，通过循环一致性将VLM生成的caption再输入文本到图像模型重建图像，利用原始图像与重建图像的相似度作为奖励信号进行GRPO微调，从而提升图像描述的准确性和减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 当前VLM容易产生vision-language misalignment，生成过于笼统或幻觉性的描述。现有方法需要大规模标注数据集或复杂的推理时框架，本文希望利用循环一致性作为自监督训练信号，仅需原始图像即可训练，无需人工标注的图像-文本对。

Method: 构建image-to-text(VLM)和text-to-image模型的循环框架：VLM生成caption，再通过text-to-image模型从caption重建图像。利用原始图像与重建图像的相似度作为奖励，通过Group Relative Policy Optimization (GRPO)对VLM进行微调。

Result: 在1B到7B参数的四个VLM上，CycleCap在captioning和hallucination基准测试中均取得一致提升，超越了依赖监督循环一致性训练的前沿方法。

Conclusion: 循环一致性可直接作为自监督训练信号，使模型仅使用原始图像即可产生更准确、更具依据的文本描述，为减少VLM幻觉和提升图像描述质量提供了一种高效且可扩展的方案。
...
TL;DR: 本文提出PreSCAN框架,用于在训练前预测NeRF在卫星图像上的重建质量。通过SHAP分析发现多视图一致性是决定重建质量的关键因素,而非模型架构。PreSCAN利用轻量级几何和光度描述符,在30秒内选择合适架构,预测误差小于1dB,相比NAS提速1000倍。在Jetson Orin边缘平台上,结合离线成本分析可降低26%功耗和43%延迟,且质量损失最小。


<details>
  <summary>Details</summary>
Motivation: 卫星图像的NeRF训练需要大量GPU时间,且现有方法关注架构改进而忽视关键因素。研究者希望通过SHAP分析找出影响重建质量的真正因素,并开发快速预测框架以加速NeRF在卫星 imagery部署。

Method: 1)使用SHAP分析确定多视图一致性是影响NeRF质量的关键因素 2)开发PreSCAN框架,利用轻量级几何和光度描述符预测NeRF质量 3)在边缘平台Jetson Orin上结合离线成本分析进行部署优化

Result: PreSCAN在30秒内完成架构选择,预测误差<1dB,比NAS提速1000倍。在Jetson Orin上实现26%功耗降低和43%延迟降低。DFC2019数据集验证了方法的泛化能力。

Conclusion: 多视图一致性而非模型架构决定了NeRF重建质量。PreSCAN可有效预测NeRF质量,实现快速架构选择和高效边缘部署,为卫星图像3D重建提供实用解决方案。
...
TL;DR: 研究引入三层层诊断框架，通过三个指标（潜在异常检测、视觉必要性评分、竞争评分）来解构VLMs幻觉来源。发现69.6%的样本表现出"视觉谄媚"现象，即模型检测到视觉异常但仍产生幻觉以满足用户期望，而零样本显示"稳健拒绝"能力。


<details>
  <summary>Details</summary>
Motivation: 探究VLMs在正确回答时是真正依赖视觉信息还是利用语言捷径（language shortcuts），即模型是否具有真正的视觉 groundedness 还是仅仅依赖于语言先验。

Method: 提出三层层诊断框架，包含三个指标：1) 潜在异常检测（Latent Anomaly Detection，感知意识）；2) 视觉必要性评分（Visual Necessity Score，通过KL散度测量视觉依赖）；3) 竞争评分（Competition Score，视觉 groundedness 与指令遵循之间的冲突）。使用反事实干预（盲图、噪声图、冲突图）在7个VLM和7000个模型-样本对上进行测试。

Result: 1) 69.6%的样本表现出"视觉谄媚"（Visual Sycophancy）——模型检测到视觉异常但仍产生幻觉以满足用户期望；2) 零样本显示"稳健拒绝"（Robust Refusal），表明对齐训练系统性地抑制了真实的 uncertainty 承认；3) 扩展分析（Qwen2.5-VL 7B到72B）表明更大模型减少语言捷径但加剧视觉谄媚；4) 诊断分数可实现后验选择性预测策略，在50%覆盖率下达到+9.5pp准确率提升。

Conclusion: 仅靠模型规模无法解决 grounding 问题。视觉谄媚是一个系统性问题，源于对齐训练过度抑制拒绝能力。诊断分数可作为有效的无训练成本的选择性预测依据。
...
TL;DR: 针对挑战性条件下的关键点匹配问题，提出了一种精确的极线引导匹配方法。该方法将关键点匹配转化为角空间中的1D区间查询问题，通过线段树在对数时间内高效解决，实现了像素级容差和每关键点控制。


<details>
  <summary>Details</summary>
Motivation: 在重复纹理或宽基线视图等挑战性条件下，传统关键点匹配速度慢且不可靠。虽然已知几何关系（如基础矩阵）可用于限制对应关系到极线包络，但现有方法依赖粗略的空间分箱，引入近似误差，需要昂贵的后处理，且可能遗漏有效对应。

Method: 提出一种精确公式，直接在角空间进行候选选择。具体做法：为每个关键点分配一个容差圆，从极点观察时定义角度区间；匹配问题转化为1D角度区间查询；使用线段树在对数时间内高效解决。

Result: 在ETH3D数据集上进行广泛评估，相比现有方法实现明显加速，同时恢复了精确的对应集合。

Conclusion: 该方法保证像素级容差，支持每关键点控制，消除了不必要的描述符比较，提高了匹配效率和鲁棒性。
...
TL;DR: Inst4DGS是一种实例分解的4D高斯溅射方法，采用长-horizon每高斯轨迹来处理动态场景中的实例跟踪和分解问题。


<details>
  <summary>Details</summary>
Motivation: 动态4DGS发展迅速，但实例分解的4DGS仍探索不足，主要难点在于独立分割的多视图视频中难以关联不一致的实例标签。

Method: 1. 引入每视频标签排列潜在变量，通过可微 Sinkhorn 层学习跨视频实例匹配，实现直接多视图监督和一致身份保持。2. 提出实例分解的运动脚手架，为每个对象提供低维运动基，用于长视野轨迹优化。

Result: 在 Panoptic Studio 和 Neural3DV 数据集上，Inst4DGS 实现了 state-of-the-art 的渲染和分割质量。在 Panoptic Studio 上，PSNR 从 26.10 提升至 28.36，实例 mIoU 从 0.6310 提升至 0.9129。

Conclusion: Inst4DGS 能够联合支持跟踪和实例分解，达到了最先进的渲染和分割质量。
...
TL;DR: 针对皮肤病学领域罕见病诊断推理评估缺失的问题，构建了DermCase长上下文基准数据集，包含26,030个多模态图像-文本对和6,354个临床挑战性病例，标注了临床信息和逐步推理链。基准测试22个主流LVLMs发现其在诊断推理方面存在显著不足，指令微调可有效提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注常见疾病，仅评估最终准确性，忽略了临床推理过程，且缺乏对罕见病诊断推理的评估。

Method: 构建 DermCase 基准数据集（26,030个多模态图像-文本对，6,354个临床挑战性病例），建立 DermLIP 相似度度量标准用于评估鉴别诊断质量，对22个主流LVLMs进行基准测试，并开展指令微调和DPO优化实验。

Result: 22个主流LVLMs在诊断准确性、鉴别诊断和临床推理方面存在显著不足；指令微调可显著提升性能，DPO优化效果有限；当前模型的推理能力存在关键局限性。

Conclusion: 研究构建了DermCase基准数据集，发现22个主流LVLMs在罕见皮肤病诊断、鉴别诊断和临床推理方面存在显著不足。指令微调可显著提升性能，而DPO优化效果有限。这项工作为评估LVLMs在皮肤病学领域的诊断推理能力提供了重要基准。
...
TL;DR: 本文提出了一种基于可控扩散模型的新型合成数据增强管道，用于像素级语义分割任务。该方法通过类别感知提示和视觉先验混合来平衡数据的多样性和可靠性，有效缩小合成数据与真实数据之间的差距。在PASCAL VOC和BDD100K数据集上的实验表明，该方法显著提升了语义分割性能，尤其在数据稀缺场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 像素级语义分割任务的数据收集和标注成本高昂。传统数据增强技术（如平移、缩放、颜色变换）虽能产生几何变化，但无法生成新的结构。虽然生成模型已被用于扩展数据集的语义信息，但它们往往难以保持原始图像与生成图像之间的一致性，特别是对于像素级任务。

Method: 提出了一种集成可控扩散模型的合成数据增强管道。具体包括：(1)类别感知提示（class-aware prompting）确保生成图像与分割标签精确对齐；(2)视觉先验混合（visual prior blending）进一步提升图像质量；(3)平衡数据的多样性与可靠性，缩小合成数据与真实数据之间的差距。

Result: 在PASCAL VOC和BDD100K基准数据集上的评估表明，该方法显著提升了语义分割性能，尤其在数据稀缺场景下效果明显，同时提高了模型在真实应用中的鲁棒性。

Conclusion: 基于可控扩散模型的合成数据增强方法能够有效解决语义分割任务中标注数据不足的问题，为数据稀缺场景提供了一种实用的解决方案，并提升了模型的泛化能力和实际应用价值。
...
TL;DR: SR-Nav是一个用于零样本目标导航的框架，通过构建动态空间关系图(DSRG)来增强感知和规划能力。该方法利用目标中心空间关系和关系匹配来纠正视觉感知错误，并通过动态关系规划来减少搜索空间。


<details>
  <summary>Details</summary>
Motivation: 基础模型在视角不佳或语义线索较弱时无法提供可靠的推理，导致导航失败或低效。作者观察到物体和区域之间的内在关系编码了结构化的场景先验，即使在部分观测下也能帮助智能体推断合理的目标位置。

Method: 1. 构建动态空间关系图(DSRG)：通过基础模型编码目标中心空间关系，并随实时观测动态更新；2. 关系感知匹配模块：利用DSRG中的多样化关系进行关系匹配而非简单检测，以验证和纠正错误；3. 动态关系规划模块：从当前位置基于DSRG动态计算最优路径，减少规划搜索空间。

Result: 在HM3D数据集上，该方法在成功率和导航效率方面均达到了最先进的性能。

Conclusion: 通过建模观察到的和基于经验的空间关系，SR-Nav能够有效增强零样本目标导航的感知鲁棒性和规划效率，为解决基础模型在弱语义场景下的推理失败问题提供了新思路。
...
TL;DR: 本文提出通过自一致性目标来改进视频大型语言模型在体育辅导任务中的时间定位能力，无需额外标注即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 视频大型语言模型常常关注无关帧，这对需要精确时间定位的体育辅导任务特别不利，且获取帧级监督成本高昂且不可靠。

Method: 利用生成和验证等紧密相关任务必须关注相同帧的观察，通过对选定视觉注意力图施加自一致性目标来实现监督。

Result: 在使用VidDiffBench验证注意力分配错误是重要瓶颈后，训练该目标在三个体育辅导任务（Exact、FitnessQA和ExpertAF）上相比监督微调获得+3.0%、+14.1%准确率和+0.9 BERTScore的提升，甚至超越闭源模型。

Conclusion: 通过相关任务的自一致性监督可有效改善视频大型语言模型的时间定位能力，且无需人工标注即可超越传统监督微调方法。
...
TL;DR: 研究团队开发了一个可解释的自动化前列腺癌检测框架，仅使用162例T2加权MRI图像，通过迁移学习和数据增强解决数据稀缺问题。在比较Vision Transformers (ViT, Swin)、CNN (ResNet18)和传统方法后，迁移学习ResNet18表现最佳（准确率90.9%，灵敏度95.2%，AUC 0.905），而HOG+SVM也达到相当准确率（AUC 0.917）。该方法仅需T2加权图像即可达到竞争力性能，AI模型灵敏度（95.2%）显著高于五位放射科医生的平均灵敏度（67.5%）。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是男性主要致死原因之一，T2加权前列腺MRI图像解读困难，主要因为病变表现细微且异质性强。现有最先进方法依赖双参数MRI（T2加权+DWI）和大样本队列，数据获取和计算成本高。需要开发一种仅使用T2加权图像即可实现高效检测的模型，降低采集复杂度和计算成本。

Method: 研究构建了包含102例癌症和60例正常的小样本数据集（162例），采用迁移学习和数据增强技术解决数据稀缺问题。全面比较了三类方法：1) Vision Transformers（ViT, Swin）；2) CNNs（ResNet18）；3) 传统机器学习方法（Logistic Regression, SVM, HOG+SVM）。同时进行了22例读者研究，邀请五位放射科医生评估作为对比。

Result: 迁移学习ResNet18获得最佳性能：准确率90.9%、灵敏度95.2%、AUC 0.905，参数仅11M。Vision Transformers尽管复杂度更高但性能较低。HOG+SVM达到相当准确率（AUC 0.917），凸显了在小数据集中手工特征的有效性。读者研究中，五位放射科医生平均灵敏度67.5%（Fleiss Kappa = 0.524），而AI模型灵敏度达95.2%。

Conclusion: 该方法仅使用T2加权图像即可达到与双参数MRI方法相当的竞争力性能，显著降低了采集复杂度和计算成本。AI模型在灵敏度上大幅超越放射科医生（95.2% vs 67.5%），有望作为辅助筛查工具减少漏诊并提高诊断一致性，为前列腺癌AI辅助诊断提供了可行的解决方案。
...
TL;DR: MedQ-UNI是一个统一的视觉语言模型，采用"先评估后恢复"范式，通过结合医学图像质量评估(Med-IQA)来指导医学图像恢复(Med-IR)，能够跨模态和退化类型进行泛化。


<details>
  <summary>Details</summary>
Motivation: 现有的医学图像恢复方法通常是模态特定或退化特定的，难以泛化到临床实践中遇到的各种退化类型。问题根源在于Med-IR与Med-IQA的割裂，缺乏显式质量理解的恢复模型难以适应不同模态的各种退化类型。

Method: 提出多模态自回归双专家架构，共享注意力机制：质量评估专家首先通过结构化自然语言描述识别退化问题，恢复专家随后基于这些描述进行针对性图像恢复。构建了约50K样本的大规模数据集，涵盖三种成像模态和五种恢复任务。

Result: 单一MedQ-UNI模型无需任何任务特定适应，即可在所有任务上达到最先进的恢复性能，同时生成更优的描述。实验证实显式质量理解显著提高了恢复保真度和可解释性。

Conclusion: 将Med-IQA与Med-IR相结合，通过"先评估后恢复"范式，可以实现跨模态、跨退化类型的统一医学图像恢复，显著提升恢复效果和可解释性。
...
TL;DR: ColourCrafter是一个统一的扩散模型框架，通过在潜在空间中进行RGB颜色标记和图像标记的令牌级融合，实现结构感知的区域颜色编辑，并使用感知Lab空间损失提高像素级精度。


<details>
  <summary>Details</summary>
Motivation: 颜色是图像生成中感知最显著但最难控制的属性，现有扩散模型难以准确实现用户指令的颜色修改，特别是对于细粒度和局部编辑，早期方法依赖离散语言描述无法准确表示连续的色彩变化。

Method: 提出ColourCrafter框架：1) 在潜在空间中进行RGB颜色令牌和图像令牌的令牌级融合；2) 选择性地将颜色信息传播到语义相关区域同时保持结构完整性；3) 使用感知Lab空间损失解耦亮度和色度，在掩码区域内约束编辑；4) 构建ColourfulSet大规模数据集。

Result: 在细粒度颜色编辑方面实现了最先进的颜色准确性、可控性和感知保真度。

Conclusion: ColourCrafter成功将颜色编辑从全局色调传输转变为结构化、区域感知的生成过程，在颜色准确性和保真度方面达到最优表现。
...
TL;DR: 研究评估视觉-语言模型(VLMs)是否能从游戏画面中推断玩家参与度，发现当前VLMs在此任务上表现不佳，存在感知-理解差距。


<details>
  <summary>Details</summary>
Motivation: 从游戏玩法视频中推断人类参与度对游戏设计和玩家体验研究非常重要，但目前尚不清楚视觉-语言模型能否仅凭视觉线索推断这种潜在心理状态。

Method: 使用GameVibe Few-Shot数据集，跨越九款第一人称射击游戏，评估三种VLMs在六种提示策略下的表现，包括零样本预测、基于理论的提示(Flow、GameFlow、自我决定理论、MDA)和检索增强提示，同时考虑点态参与度预测和连续窗口间参与度变化的成对预测。

Result: 零样本VLM预测普遍较弱，通常无法超越简单的每游戏多数类基线；记忆或检索增强提示在某些情况下改善了点态预测；成对预测在各种策略下始终困难；基于理论的提示本身不可靠，反而可能强化表面层面的捷径。

Conclusion: 当前VLMs存在感知-理解差距：虽然能识别可见的游戏玩法线索，但仍难以跨游戏稳健地推断人类参与度。
...
TL;DR: 本文提出T-QPM框架解决开放世界学习中时序分布漂移下的OOD检测问题，通过跨模态一致性模式和自适应融合权重提升检测性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs多模态OOD检测方法存在两个主要缺陷：(1)依赖固定融合规则，无法适应时序分布变化；(2)缺乏对协变量偏移输入的鲁棒性。在动态开放世界环境中，传统静态方法性能退化严重。

Method: 提出两步框架T-QPM：(1)通过将OOD图像与文本描述配对，引入ID和OOD信号间的跨模态一致性模式，利用图像-文本联合推理优化决策边界；(2)学习轻量级融合权重自适应组合语义匹配和视觉典型性；(3)引入基于ATC的显式正则化防止分布演进时性能下降。

Result: 在时序分区基准数据集上的实验表明，该方法显著优于静态基线方法，在非平稳环境中提供鲁棒且时序一致的OOD检测框架。

Conclusion: 通过T-QPM框架有效解决了时序分布漂移和协变量偏移下的OOD检测难题，验证了跨模态推理和自适应融合在动态开放世界学习中的重要价值。
...
TL;DR: TexEditor是一个基于Qwen-Image-Edit-2509的专用纹理编辑模型，通过TexBlender数据集和StructureNFT方法增强结构保持，在TexBench基准测试中优于Nano Banana Pro等先进模型。


<details>
  <summary>Details</summary>
Motivation: 当前文本引导的纹理编辑模型即使在仅改变外观的情况下也难以保持底层几何结构的完整性，现有的SOTA编辑模型在纹理编辑过程中经常出现结构不一致问题。

Method: 1) 构建TexBlender高质量SFT数据集(使用Blender生成)，为冷启动提供强结构先验；2) 提出StructureNFT基于强化学习的方法，将SFT阶段学习的结构先验迁移到真实场景；3) 提出TexBench通用真实世界基准测试用于评估。

Result: TexEditor在Blender纹理基准测试和TexBench上均优于Nano Banana Pro等强基线模型，同时在ImgEdit通用基准测试上验证了泛化能力。

Conclusion: 通过结合高质量合成数据和强化学习结构保持损失，TexEditor能够有效解决纹理编辑中的结构一致性问题，为文本引导纹理编辑提供了一个有效的解决方案。
...
TL;DR: FILT3R通过将3D重建的潜在状态更新建模为token空间的卡尔曼滤波，在线估计过程噪声以自适应调整记忆保留与新观测的平衡，无需训练即可提升长时域3D重建稳定性。


<details>
  <summary>Details</summary>
Motivation: 流式3D重建的关键挑战在于状态更新规则：激进的历史覆盖会遗忘有用的信息，而保守的更新无法跟踪新证据，两种行为在超出训练视域时都会变得不稳定。

Method: FILT3R是一种无需训练的潜在过滤层，将递归状态更新建模为token空间中的随机状态估计。该方法维护每个token的方差，计算类似卡尔曼滤波的增益，以自适应平衡记忆保留与新观测。具体而言，通过EMA归一化的候选token时间漂移在线估计过程噪声（governing how much the latent state is expected to change between frames）。

Result: 实验表明FILT3R产生了一种可解释的、插件式的更新规则，能够将常见的覆盖和门控策略作为特例包含。增益在稳定区域随累积证据收缩，在 genuined 场景变化增加过程不确定性时上升，相比现有方法显著提升了深度、姿态和3D重建的长时域稳定性。

Conclusion: FILT3R作为一种无需训练的潜在过滤层，通过将递归状态更新转化为token空间中的随机状态估计，有效解决了流式3D重建中的状态更新难题。该方法通过在线估计过程噪声，使增益在稳定区域收缩以累积证据，在场景变化时上升以适应新信息，显著提升了深度、姿态和3D重建的长时域稳定性。
...
TL;DR: 本文提出Diff-SIT一种基于高效视频扩散模型的新型视频压缩方法,在极低比特率下实现感知质量和时间一致性 SOTA。


<details>
  <summary>Details</summary>
Motivation: 传统端到端压缩模型在极低比特率下产生模糊图像,感知质量差;现有生成式压缩方法独立处理视频帧,存在时间一致性和效率问题。

Method: 提出Diff-SIT包含两部分:(1)稀疏时序编码模块(STEM)将原始帧序列稀疏编码为信息丰富的中间序列,大幅节省比特率;(2)单步视频扩散与帧类型嵌入器(ODFTE)将中间序列作为整体处理,利用时间相关性。帧类型嵌入器(FTE)引导扩散模型根据不同帧类型进行自适应重建,优化整体质量。

Result: 在多个数据集上的广泛实验表明,Diff-SIT在感知质量和时间一致性方面建立新SOTA,特别是在挑战性的极低比特率场景下。

Conclusion: Diff-SIT通过稀疏信息传输和帧类型引导的扩散模型,有效解决了极低比特率视频压缩的质量问题,实现了感知质量和时间一致性的显著提升。
...
TL;DR: 本文综述了人工智能驱动蛋白质科学从静态结构预测向动态构象集合建模和复杂生物分子相互作用建模的范式转变，涵盖多模态表示、无MSA架构、生成式框架、异质相互作用预测和功能推断五个维度。


<details>
  <summary>Details</summary>
Motivation: 蛋白质折叠问题因人工智能发生根本性变革，传统静态结构预测已无法满足研究需求，需向动态构象集成和复杂相互作用建模方向发展。

Method: 系统综述方法，从五个相互关联的维度分析AI驱动蛋白质科学的范式转变：统一多模态表示、静态预测精化、生成式框架、异质相互作用预测和功能推断。

Result: 识别出当前发展瓶颈（数据分布偏差、机制可解释性有限、几何指标与生物物理现实脱节），并指明未来方向（物理一致的生成模型、多模态基础架构、实验闭环系统）。

Conclusion: 人工智能正从结构分析工具演变为能够理解和最终改写生命动态语言的通用模拟器，标志着AI在生物分子研究中的根本性转型。
...
TL;DR: 本手册系统介绍了用于汽车保险的人工智能基础和架构，基于大规模实际部署经验。提出了垂直整合的AI范式，将感知、多模态推理和生产基础设施统一到汽车风险评估和理赔处理的智能栈中。


<details>
  <summary>Details</summary>
Motivation: 解决汽车保险行业中车辆损伤分析、理赔评估和承保工作流程的端到端自动化需求，将现代人工智能转化为高风险工业环境中的生产级可靠系统。

Method: 开发领域自适应的Transformer架构，用于结构化视觉理解、关系车辆表示学习和多模态文档智能，构建可扩展的管道并在泰国全国汽车保险系统中验证。

Result: 实现了车辆损伤分析、理赔评估和承保工作流程的端到端自动化，建立了将学习算法与MLOps实践相结合的原则框架。

Conclusion: 成功将现代人工智能转化为可在高风险工业环境中可靠运行的生产级系统，为汽车保险行业提供了系统性的人工智能解决方案。
...
TL;DR: 提出了OnlinePG，一个基于3D Gaussian Splatting的在线开放词汇全景映射系统，用于具身智能的场景理解。该系统通过滑动窗口的局部-全局范式，结合3D片段聚类图和双向 bipartite 3D Gaussian实例匹配，实现实时开放词汇场景感知。


<details>
  <summary>Details</summary>
Motivation: 现有方法多为离线或缺乏实例级理解，难以应用于实际机器人任务，需要一种能够在线进行开放词汇场景理解的系统。

Method: 1) 使用滑动窗口的局部-全局范式实现在线全景映射；2) 构建3D片段聚类图，结合几何和语义线索融合滑动窗口中的不一致片段；3) 构建具有空间属性的显式网格，通过双向 bipartite 3D Gaussian实例匹配将局部高斯图融合到全局地图；4) 利用融合在3D空间属性网格中的VLM特征实现开放词汇场景理解。

Result: 在广泛使用的数据集上进行了大量实验，表明该方法在在线方法中实现了更好的性能，同时保持了实时效率。

Conclusion: OnlinePG能够有效实现基于3D Gaussian Splatting的在线开放词汇全景映射，在性能和效率上优于现有在线方法。
...
TL;DR: 本文提出了一种新的3D感知视频定制框架(3DreamBooth和3Dapter),用于创建动态的、视图一致的定制主体视频。现有的主体驱动视频生成方法主要将主体视为2D实体,缺乏重建3D几何体所需的空间先验,导致在合成新视图时生成任意细节而非保留真实3D身份。


<details>
  <summary>Details</summary>
Motivation: 现实世界的主体本质上是3D的,但现有2D中心方法缺乏重建3D几何体所需的全面的空间先验。在合成新视图时必须为不可见区域生成看似合理但任意的细节,而非保留真实3D身份。实现真正的3D感知定制具有挑战性,因为缺乏多视图视频数据集,在有限视频序列上微调往往导致时间过拟合。

Method: 1. 3DreamBooth通过单帧优化范式将空间几何与时间运动解耦,通过仅更新空间表示将强大的3D先验嵌入模型,无需 exhaustive视频训练。2. 3Dapter作为视觉 conditioning模块,用于增强细粒度纹理和加速收敛。3. 3Dapter经过单视图预训练后,通过不对称conditioning策略与主生成分支进行多视图联合优化,使模块作为动态选择性路由器,从最小参考集中查询视图特定几何提示。

Result: 成功构建了一个用于3D感知视频定制的novel框架,能够创建动态的、视图一致的定制主体视频。

Conclusion: 该框架通过引入3DreamBooth和3Dapter有效解决了3D感知定制难题,通过单帧优化范式和解耦空间几何与时间运动的方法,在不需要大量多视图视频数据的情况下实现了robust的3D先验嵌入。
...
TL;DR: 本文提出了跨域小样本目标检测(CD-FSOD)中的"散光(Astigmatism)"问题，即模型在目标域中注意力分散、定位不准确。解决方案是受人类中央凹视觉系统启发的中心-外围注意力精炼框架，包含正样本模式精炼、负上下文调制和文本语义对齐三个模块。


<details>
  <summary>Details</summary>
Motivation: 跨域小样本目标检测面临严重的领域偏移和数据稀缺问题。作者发现了一个被忽视的现象：模型在目标域中表现出注意力分散和聚焦不足的问题，导致定位不准确和冗余预测，类似于人类无法聚焦视觉目标，因此称之为"目标域散光问题"。

Method: 提出了中心-外围注意力精炼框架，包含三个模块：(1)正样本模式精炼模块(Positive Pattern Refinement)：使用类特定原型重塑注意力指向语义对象，模拟视觉中心区域；(2)负上下文调制模块(Negative Context Modulation)：通过建模背景上下文增强边界区分能力，模拟视觉外围区域；(3)文本语义对齐模块(Textual Semantic Alignment)：通过跨模态线索加强中心-外围区分。

Result: 在6个具有挑战性的CD-FSOD基准数据集上进行了实验，结果表明该方法持续提升了检测精度，建立了新的最先进水平。

Conclusion: 受生物启发的中心-外围注意力精炼框架能够将"散光"注意力转化为聚焦模式，显著改善了目标域的自适应性能。该方法通过模拟人类视觉系统的中央凹工作方式，有效解决了跨域小样本目标检测中的注意力分散问题。
...
TL;DR: 本文提出CoDA框架，系统性地评估医学视觉-语言模型(MVLMs)在真实临床工作流中面对 pipeline shifts 时的鲁棒性。研究发现链式组合的pipeline退化比单一阶段更能显著降低模型性能，且多模态大语言模型在图像质量审计方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 现有MVLM鲁棒性评估假设输入干净或仅研究单一 corruption，忽略了临床工作流中保持可读性但改变图像统计的常见操作（如采集、重建、显示、传输）。需要构建临床可信的pipeline偏移来评估真实威胁。

Method: 提出CoDA框架，通过组合采集类着色、重建与显示重映射、交付与导出退化构建临床 plausible 的pipeline偏移。在掩码结构相似性约束下联合优化阶段组合和参数，诱导失败同时保持视觉可信度。对brain MRI、chest X-ray、abddominal CT进行测试，评估MLLMs作为技术真实性审计员，并提出基于教师引导的token空间适应+patch级对齐的后修复策略。

Result: CoDA大幅降低CLIP风格MVLMs的零样本性能，链式组合比任何单一阶段破坏性更强。专有MLLMs审计可靠性下降且对CoDA偏移样本持续出现高置信度错误，医学专用MLLMs在质量审计方面存在明显缺陷。修复策略可改善归档CoDA输出的准确性。

Conclusion: 研究揭示了MVLM部署的临床相关威胁面，轻量级对齐可提高部署鲁棒性。强调了评估医学AI模型时需考虑真实临床工作流中的pipeline退化问题。
...
TL;DR: CausalVAD是一个利用因果干预的去混淆训练框架，通过稀疏因果干预方案(SCIS)构建原型字典并干预模型查询，以消除虚假关联，在nuScenes上达到最优性能并提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 端到端驾驶模型从根本上学习的是统计相关性而非真正的因果关系，这种脆弱性导致因果混淆，模型会利用数据集偏差作为捷径，严重损害了其在复杂场景中的可靠性和安全性。

Method: 提出了稀疏因果干预方案(SCIS)，该方案通过构建包含潜在驾驶情境的原型字典，并利用该字典对模型的稀疏向量化查询进行干预，从而主动消除由混淆因素引起的虚假关联，消除下游任务表示中的虚假因素。

Result: 在nuScenes等基准测试上的广泛实验表明，CausalVAD实现了最先进的规划精度和安全性，同时展现出对数据偏差和噪声场景诱导的因果混淆的卓越鲁棒性。

Conclusion: CausalVAD通过稀疏因果干预方案(SCIS)实现了解耦混淆因素的训练框架，在nuScenes数据集上达到了最先进的规划精度和安全性。该方法展现出对数据偏差和噪声场景诱导的因果混淆具有卓越的鲁棒性，为端到端自动驾驶系统的可靠性提升提供了新的解决思路。
...
TL;DR: 提出了一种跨层注意力传播方法，通过保存和整合Transformer编码器层的历史注意力矩阵，实现注意力模式的渐进式细化，提升ViT在CIFAR-100和TinyImageNet上的分类准确率。


<details>
  <summary>Details</summary>
Motivation:  Vision Transformer的注意力机制在各层独立运作，限制了层间信息流动和特征学习，需要一种原则性的方法来改进层间信息流。

Method:  提出跨层注意力传播方法，保存历史注意力矩阵并与当前层注意力进行混合，只需添加注意力矩阵存储和混合操作，引入混合超参数alpha=0.45控制历史与当前注意力的平衡。

Result:  CIFAR-100准确率从75.74%提升至77.07%（+1.33%），TinyImageNet从57.82%提升至59.07%（+1.25%），CaiT提升1.01%。随机初始化优于零初始化。

Conclusion:  该方法以最小架构改动实现层间信息流的原则性细化，验证了历史注意力模式对优化动态的积极作用，alpha=0.45为最优配置。
...
TL;DR: 提出了一种基于饱和度-值(SV)相似性的非局部变分技术用于彩色图像恢复，解决了传统方法仅基于灰度值描述彩色图像块相似性的问题。
...
Method: 1. 建立基于饱和度-值相似性的非局部全变分模型；2. 将彩色图像块的SV相似性融入非局部梯度；3. 采用Bregmanized算子分裂法求解优化问题。

Result: 在PSNR、SSIM、QSSIM和S-CIELAB等定量指标上优于其他对比方法。

Conclusion: 基于饱和度-值相似性的非局部变分方法能够有效提升彩色图像恢复的视觉效果和量化性能。
...
TL;DR: 本文提出了一种基于自然语言描述Action Units (AUs)的面部行为合成方法，解决了传统AU-based方法在处理冲突面部动作时产生的解剖学上不合理的问题，并构建了首个大规模文本-图像配对数据集BP4D-AUText和VQ-AUFace生成模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本的面部合成方法依赖于粗粒度的情绪类别，缺乏对人类非语言交流全面捕捉的细腻度；而基于AU的方法虽然更精确，但将AU编码为one-hot向量并简单线性组合，无法处理冲突AU（同一面部肌肉的相反动作），导致解剖学上不合理的人工伪影和不自然的动作叠加。

Method: 1) 提出使用自然语言描述AU来表征面部行为，保留AU框架的表现力同时支持复杂和冲突AU的显式建模；2) 构建规则驱动的动态AU文本处理器，从BP4D和BP4D+数据集生成BP4D-AUText数据集；3) 提出VQ-AUFace生成模型，利用面部结构先验从文本合成高保真面部行为。

Result: 定量实验和用户研究表明，该方法显著优于现有方法，在生成解剖学合理、行为丰富、感知逼真的面部表情方面表现出色，特别是在处理冲突AU的挑战性条件下。

Conclusion: 通过自然语言描述AU的方式成功解决了冲突AU的表示和合成问题，为高保真面部行为合成开辟了新路径，同时BP4D-AUText数据集和VQ-AUFace模型为该领域提供了重要的基础资源。
...
TL;DR: 本文提出TGA-ZSR和Comp-TGA两种方法，通过文本引导注意力机制提升CLIP模型的零样本对抗鲁棒性，在16个数据集上分别实现9.58%和11.95%的零样本鲁棒准确率提升。


<details>
  <summary>Details</summary>
Motivation: CLIP等预训练视觉-语言模型虽然具有出色的零样本能力，但容易受到对抗样本的攻击。现有的对抗训练方法通常需要大量计算资源，且难以保持模型在干净样本上的性能。

Method: 提出文本引导注意力零样本鲁棒性框架(TGA-ZSR)，包含局部注意力精炼模块和全局注意力约束模块。全局注意力约束模块从目标和原始模型中获取文本引导注意力，保持干净样本性能同时增强鲁棒性。进一步提出互补文本引导注意力(Comp-TGA)，结合类别提示引导的前景注意力和非类别提示的反向注意力。

Result: 在16个数据集上，TGA-ZSR和Comp-TGA分别比当前最先进方法提升9.58%和11.95%的零样本鲁棒准确率。

Conclusion: 文本引导注意力机制可有效提升CLIP模型的零样本对抗鲁棒性，Comp-TGA通过互补注意力机制能更全面准确地捕获前景信息，性能优于TGA-ZSR。
...
TL;DR: SJD-PAC是一种增强的Speculative Jacobi Decoding框架，通过主动drafting策略和自适应延续机制解决高熵视觉生成中draft token接受率低的问题，在文本到图像合成任务中实现3.8倍加速且保持无损图像质量。


<details>
  <summary>Details</summary>
Motivation: Speculative Jacobi Decoding (SJD)可在无draft模型情况下加速自回归文本到图像合成，但视觉生成的高熵特性导致复杂区域的draft token接受率较低，形成严重制约整体吞吐量的瓶颈。

Method: 提出SJD-PAC框架，包含两个核心优化：1) 主动drafting策略，提高高熵困难区域的局部接受率；2) 自适应延续机制，在初始拒绝后维持序列验证，避免完全重采样。两个优化协同工作显著提高每步平均接受长度。

Result: 在标准文本到图像基准测试中，SJD-PAC实现3.8倍加速且保持无损图像质量。

Conclusion: SJD-PAC通过主动drafting和自适应延续机制有效解决了高熵视觉生成中draft接受率低的问题，在严格保持目标分布的前提下显著提升推理速度，为自回归图像生成加速提供了有效方案。
...
TL;DR: 本文针对双流Transformer音频视频生成方法的局限性，提出了Cross-Modal Context Learning (CCL)框架。通过TARP、LCT、DCR和UCG等模块，解决了跨模态交互中的流形变化、背景偏差、CFG训练推理不一致等问题，实现了高质量、音画同步的音视频生成，在减少资源消耗的同时达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 当前双流Transformer架构已成为音视频生成的主流范式，但存在以下局限性：1) 门控机制控制跨模态交互导致的模型流形变化；2) 跨模态注意力引入的多模态背景区域偏差；3) 多模态无分类器引导(CFG)在训练和推理时的不一致性；4) 多个条件之间的冲突。这些问题影响了生成质量、训练收敛速度和推理一致性。

Method: 提出Cross-Modal Context Learning (CCL)框架，包含三个核心模块：1) 时序对齐RoPE和分区(TARP)：增强音频和视频潜在表示的时序对齐；2) 跨模态上下文注意力(CCA)：包含可学习上下文 tokens(LCT)和动态上下文路由(DCR)，为跨模态信息提供稳定的无条件锚点，并根据不同训练任务动态路由；3) 无条件上下文引导(UCG)：利用LCT提供的无条件支持促进不同形式的CFG，提高训练推理一致性。

Result: CCL在音视频生成质量上达到了当前最先进水平，同时大幅降低了资源消耗需求，训练收敛速度和生成质量均得到提升。

Conclusion: CCL方法在综合评估中取得了最先进的性能，同时需要显著更少的计算资源。该研究为双流Transformer音视频生成提供了新的思路，通过上下文学习机制有效解决了跨模态交互中的多个技术挑战。
...
TL;DR: 论文系统基准测试了三种混合Transformer模型（UNETR、SwinUNETR、UNETR++）与CNN模型（SegResNet）在腹部CT多器官分割任务上的表现，发现CNN模型在异构数据集上表现最优。


<details>
  <summary>Details</summary>
Motivation: 腹部CT准确的多器官分割对计算机辅助诊断和治疗至关重要。CNN长期以来是医学图像分割的标准方法，但Transformer架构因其建模长距离依赖的能力而受到关注。

Method: 在RATIC数据集（来自23个机构的206例CT扫描，覆盖5个腹部器官）上，使用Dice相似系数（DSC）作为主要指标，在相同预处理和训练条件下对四种模型进行训练和评估。

Result: CNN-based SegResNet获得最高总体性能，在所有器官上优于所有混合Transformer模型。Transformer中UNETR++表现最具竞争力，UNETR收敛更快。

Conclusion: 对于小型至中型异构数据集，优化良好的CNN架构仍具有高度竞争力，可能优于混合Transformer设计。
...
TL;DR: 本文针对文本到动作(T2M)生成任务中模型在未见文本描述上泛化能力差的问题，提出了OpenT2M大规模高质量动作数据集(2800+小时)和MonoFrill预训练动作模型。核心贡献包括：开发了2D-PRQ动作分词器，通过将人体按生物学部件划分来捕捉时空依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有T2M模型在未见文本描述上表现不佳，原因是现有动作数据集规模小且多样性不足，限制了模型的泛化能力。

Method: 1) 构建OpenT2M数据集：包含2800+小时人体动作，通过物理可行性验证和多粒度过滤进行严格质量控制，并提供详细的秒级文本标注；2) 开发自动化长时序序列创建管道；3) 提出MonoFrill预训练动作模型，其核心组件2D-PRQ动作分词器将人体按生物学部件划分以捕捉时空依赖。

Result: OpenT2M显著提升了现有T2M模型的泛化能力；2D-PRQ实现了卓越的重建效果和强大的零样本性能。

Conclusion: OpenT2M和MonoFrill通过解决长期存在的数据质量和基准测试挑战，将推动T2M领域的发展。
...
TL;DR: GenVideoLens是一个细粒度基准测试，用于评估大型视觉语言模型在AI生成视频检测中的能力，包含400个高欺骗性AI生成视频和100个真实视频，涵盖15个真实性维度。


<details>
  <summary>Details</summary>
Motivation: 现有评估协议将AI生成视频检测视为二元分类问题，使用粗粒度指标如整体准确率，无法深入了解LVLMs的成功或失败原因。

Method: 构建包含15个真实性维度（感知、光学、物理和时间因果线索）的专家标注基准，评估11个代表性LVLMs，并进行时间扰动实验。

Result: LVLMs在感知线索上表现较好，但在光学一致性、物理交互和时间因果推理方面存在困难；不同维度性能差异显著，较小的开源模型有时在特定真实性线索上优于更强大的专有模型；当前LVLMs对时间信息的利用有限。

Conclusion: GenVideoLens提供了对LVLM行为的诊断洞察，揭示了关键能力差距，为改进未来AI生成视频检测系统提供了指导。
...
TL;DR: 本文提出SVOO，一种用于快速视频生成的无训练稀疏注意力框架，通过离线层间稀疏度分析和在线双向共聚类算法，在7个视频生成模型上实现高达1.93倍加速，同时保持最高29dB的PSNR。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成中的无训练稀疏注意力方法存在两个未解决的局限性：1) 忽略注意力剪枝中的层异质性；2) 忽略块分区中的查询-键耦合，阻碍了更好的质量-加速权衡。

Method: 提出两阶段范式：1) 离线层间敏感度分析，得出每层固有剪枝级别；2) 通过新型双向共聚类算法进行在线块级稀疏注意力计算。

Result: 在7个广泛使用的视频生成模型上实验表明，SVOO实现了优于最先进方法的质量-加速权衡，在Wan2.1上达到最高29dB PSNR的同时实现1.93倍加速。

Conclusion: 通过揭示每层注意力稀疏度是其内在属性这一关键洞察，SVOO能够更有效地平衡视频生成的质量与推理速度。
...
TL;DR: PhysVideo是一个两阶段视频生成框架，通过先生成物理感知的正交前景视频，再合成完整视频来解决物理一致性问题。该方法在PhysMV数据集上训练，显著提升了物理真实感和时空一致性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法虽能提升视觉质量，但难以确保物理一致的运动。根本原因在于现实物体在三维空间运动，而视频仅提供部分、视角依赖的动态投影。

Method: 提出两阶段框架：第一阶段Phys4View利用物理感知注意力捕获物理属性对运动动态的影响，并结合几何增强的跨视角注意力和时间注意力增强时空一致性；第二阶段VideoSyn使用生成的前景视频作为指导，学习前景动态与背景上下文的交互，实现可控视频合成。构建PhysMV数据集，包含40K场景，每个场景四个正交视角，共160K视频序列。

Result: 实验表明PhysVideo显著提升了物理真实感和时空一致性，优于现有视频生成方法。

Conclusion: PhysVideo通过两阶段物理感知方法有效解决了视频生成中的物理一致性问题，为高质量可控视频合成提供了新思路。
...
TL;DR: MeInTime是一个基于扩散模型的跨年龄人脸修复方法，通过解耦身份和年龄条件的建模，利用注意力机制和门控残差融合模块注入身份特征，并在推理阶段提出年龄感知梯度引导来实现年龄一致的保身份修复。


<details>
  <summary>Details</summary>
Motivation: 现有参考式人脸修复方法隐式假设参考图与退化输入年龄对齐，但在实际场景（如历史照片修复）中只能获得跨年龄参考图像，这限制了方法的有效性。

Method: 1. 解耦身份和年龄条件建模；2. 引入新型注意力机制有效注入身份特征；3. 门控残差融合模块促进退化特征与身份表示的融合；4. 年龄感知梯度引导——一种无需训练的采样策略，使用年龄驱动方向迭代引导身份感知去噪潜在向量朝向目标年龄语义流形。

Result: 在身份保真度和年龄一致性方面均优于现有最先进的人脸修复方法。

Conclusion: MeInTime成功将参考式人脸修复从同年龄设置扩展到跨年龄设置，在身份保留和年龄一致性方面展现出卓越性能。
...
TL;DR: 本文提出了Click-to-Ask，一个用于直播电商的AI驱动助手，包含离线和在线两个模块。离线模块处理多模态产品信息，生成结构化产品和合规推广文案；在线模块通过点击问题实现实时响应观众咨询。在TikTok直播数据集上，该方法达到0.913的问题识别准确率和0.876的响应质量得分。


<details>
  <summary>Details</summary>
Motivation: 直播电商已成为现代重要的广播形式，为了帮助主播更高效便捷地进行产品推广，需要开发智能辅助工具来简化推广准备工作并实时响应观众咨询。

Method: 离线模块：处理多样的多模态产品信息，将复杂输入转换为结构化产品数据，并生成合规的推广文案；在线模块：在直播过程中允许主播点击问题进行实时响应，利用离线模块生成的结构化产品信息和流架构中维护的事件级历史记忆。

Result: 在收集的TikTok直播帧数据集上，该方法实现了0.913的问题识别准确率和0.876的响应质量得分。

Conclusion: 该系统显著减少了推广准备时间，增强了内容参与度，能够及时与观众互动，最终提高了直播电商的效果，展示了良好的实际应用潜力。
...
TL;DR: 提出了一个基于合成PDF和LLM-as-a-judge的表格提取评估框架，解决了传统规则指标无法评估语义等价性的问题，在人类判断相关性上显著优于TEDS和GriTS等现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前PDF表格提取的评估方法依赖基于规则的指标，无法捕捉表格内容的语义等价性，无法满足大规模科学数据挖掘和知识库构建的需求。

Method: 构建基于合成PDF的基准测试框架，使用arXiv来源的表格确保真实复杂度和多样性；开发LLM-as-a-judge进行语义表格评估，集成到可处理解析器输出不一致性的匹配管道中；开展包含1500+质量判断的人类验证研究。

Result: LLM评估与人类判断的相关性显著更高(Pearson r=0.93)，远超TEDS(r=0.68)和GriTS(r=0.70)。对21个当代PDF解析器在100个合成文档(451个表格)上的评估显示存在显著性能差异。

Conclusion: LLM-based evaluation方法在语义相关性上显著优于传统指标TEDS和GriTS(r=0.93 vs 0.68/0.70)。研究为PDF表格提取任务提供了可复现、可扩展的评估方法，并为实际应用中的解析器选择提供了实践指导。
...
TL;DR: 提出了一种名为Switch的新型半监督学习框架，用于医学超声图像分割，通过多尺度开关(MSS)和频域开关(FDS)策略有效利用未标注数据，在仅5%标注比例下在6个超声数据集上取得了优异的分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学超声图像分割面临标注数据有限、 speckle噪声和低对比度边界等挑战，现有半监督学习方法对未标注数据的利用不充分，缺乏鲁棒的特征表示机制。

Method: 提出两种创新策略：1) 多尺度开关(MSS)采用分层patch混合实现均匀空间覆盖；2) 频域开关(FDS)结合对比学习在傅里叶域进行振幅切换获取鲁棒特征。框架整合于teacher-student架构中有效利用标注和未标注数据。

Result: 在6个不同超声数据集(淋巴结、乳腺病变、甲状腺结节和前列腺)上验证，5%标注比例下在LN-INT数据集达80.04% Dice，DDTI数据集达85.52% Dice，Prostate数据集达83.48% Dice，半监督方法甚至超越全监督基线，且保持参数量高效(1.8M)。

Conclusion: Switch框架在资源受限的医学影像应用中具有显著优势，能在极低标注成本下实现高性能分割，为医学超声图像分割提供了一种有效的半监督学习方案。
...
TL;DR: 该综述全面调研了多模态计算病理学领域的最新进展，涵盖四个研究方向：自监督表示学习、 多模态数据生成与增强、参数高效微调与推理增强的少样本学习、以及多智能体协作推理。 重点分析了token压缩如何实现跨尺度建模，以及多智能体机制如何模拟病理医师的"思维链" 实现不确定性感知的证据融合。


<details>
  <summary>Details</summary>
Motivation: 推动计算病理学发展的核心挑战包括：WSI的超高分辨率带来计算难题、专家标注有限、多模态信息整合困难、以及模型透明度不足影响临床可信度。

Method: 系统综述方法，分析四个研究前沿：1)自监督表示学习与结构感知token压缩；2)多模态数据生成与增强；3)参数高效适配与推理增强少样本学习；4)多智能体协作推理实现可信诊断。

Result: 探讨了token压缩 Enables跨尺度建模，多智能体机制模拟病理医师Chain of Thought实现多放大倍数下不确定性感知证据融合。

Conclusion: 未来进展取决于统一的多模态框架，需整合高分辨率视觉数据与临床生物医学知识，以支持可解释、安全的AI辅助诊断。
...
TL;DR: SCNP是一种通过惩罚最差分类邻居来提高图像分割拓扑精度的高效方法，可集成到多种分割框架和损失函数中，在13个数据集上验证有效。


<details>
  <summary>Details</summary>
Motivation: 标准深度学习图像分割模型无法保证拓扑精度，无法正确保留连通分量数量，影响分割质量和后续量化分析的可靠性。现有方法存在集成繁琐、计算成本高或仅适用于管状结构等局限性。

Method: 提出SCNP方法，通过惩罚 logits 中分类最差的邻居像素，强制模型先改善像素邻域的预测，再改善像素本身的预测。方法可集成到语义分割和实例分割框架中，也可与多种损失函数结合使用。

Result: 在13个涵盖不同结构形态和图像模态的数据集上验证了有效性，成功集成到三个语义和实例分割框架中，并可使多种损失函数提升拓扑精度。

Conclusion: SCNP是一种高效、通用的拓扑精度提升方法，能够在不显著增加计算负担的情况下有效改善分割拓扑准确性，具有良好的通用性和可扩展性。
...
TL;DR: 本文针对Diffusion Transformer视频生成模型的高内存和计算成本问题，提出了一种推理时的NVFP4/INT8混合精度量化框架。该方法通过轻量级预测器动态分配量化精度，并利用时间冗余缓存进一步加速。


<details>
  <summary>Details</summary>
Motivation: Diffusion transformers在视频生成方面表现优异，但其实际部署受到高内存消耗和计算成本的严重限制。现有的后训练量化方法通常采用静态位宽分配，忽略了扩散时间步之间激活的量化难度差异，导致效率与质量的权衡不理想。

Method: 1. 发现模块输入输出差异与内部线性层量化敏感度之间存在强线性相关性
2. 设计轻量级预测器，动态为时间稳定层分配NVFP4以最大化内存压缩，为易变层保留INT8以确保鲁棒性
3. 观察到Transformer模块的输入输出残差在时间步之间具有高时间一致性，利用这一时间冗余引入Temporal Delta Cache (TDC)跳过不变块的计算

Result: 实现1.92倍端到端加速和3.32倍内存降低

Conclusion: 该自适应精度策略实现了激进的量化而不影响生成质量，为视频DiT的高效推理树立了新的基线。
...
TL;DR: WeNLEX是一种弱监督模型，用于为多标签胸部X光分类生成自然语言解释。通过在黑盒模型特征空间中匹配从解释生成的图像与原始图像来确保忠诚度，并通过与少量临床医生注释的解释进行分布对齐来保持合理性。该模型仅需每个诊断5个真实解释即可产生忠诚且合理的解释。


<details>
  <summary>Details</summary>
Motivation: 大多数工作使用带有解释注释的数据集显式监督解释生成过程，因此生成的解释虽然看似合理但不能忠实反映模型的推理过程。如何在弱监督条件下生成既忠诚（忠实于模型推理）又合理（符合人类认知）的自然语言解释是一个挑战。

Method: 1) 通过在黑盒模型特征空间中匹配从自然语言解释生成的图像与原始图像来确保忠诚度；2) 通过与临床医生注释的小型解释数据库进行分布对齐来保持合理性；3) 弱监督设置：仅需每个诊断5个真实解释；4) 可在事后解释和模型内解释两种设置下运行。

Result: 1) 在多个评估指标（忠诚度、可模拟性、多样性、合理性）上验证了模型效果；2) 仅需5个真实解释即可产生忠诚且合理的解释；3) 在模型内设置下，WeNLEX将独立分类器的AUC提高2.21%；4) 可通过更换数据库轻松适应不同目标受众（如为非医学用户生成简化解释）。

Conclusion: WeNLEX成功实现了在弱监督条件下生成忠诚且合理的自然语言解释，且添加可解释性到训练过程实际上可以提高下游任务性能。该方法具有灵活性，可适应不同受众群体，展示了可解释性与模型性能可以共赢的潜力。
...
TL;DR: ProCal是一种无源域自适应方法，通过双模型协同预测机制动态校准邻域预测概率，解决现有方法过度依赖邻居预测相似性导致的知识遗忘和局部噪声过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有无源域自适应方法过度依赖邻居间的预测相似性，这加速了源知识的遗忘，并增加了对局部噪声过拟合的敏感性。

Method: 提出ProCal概率校准方法，集成源模型的初始预测与当前模型的在线输出进行邻域概率校准，设计了结合软监督损失和多样性损失的联合优化目标。

Result: 在4个公共数据集的31个跨域任务上验证了方法有效性，实验表明ProCal能在保持源知识的同时有效适应目标域。

Conclusion: 理论分析表明ProCal收敛至源知识与目标信息有效融合的平衡状态，减少了知识遗忘和过拟合，代码已开源。
...
TL;DR: SEAR是一种针对RGB-热成像(RGB-T)多模态输入的简单高效微调策略,能够将预训练的视觉几何变换器适应到多模态RGB-T数据中,在3D重建和相机位姿估计任务上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于RGB数据预训练的视觉几何模型在处理混合传感模态(如RGB-热成像)时效果下降。虽然模型能很好地泛化到纯热成像重建,但在联合处理RGB和热成像模态时难以对齐两种模态。

Method: 提出SEAR微调策略,利用相对较小的RGB-T数据集对预训练的几何变换器进行适配,使模型能够对齐RGB和热成像两种模态。

Result: 在3D重建和相机位姿估计方面显著超越最先进方法,AUC@30提升超过29%,在低光照和浓烟等挑战性条件下仍能实现可靠的多模态位姿估计和重建,且推理时间几乎没有额外开销。

Conclusion: SEAR能够有效对齐RGB和热成像模态,提出的新RGB-T数据集为未来多模态3D场景重建研究提供了稳健的基准。代码和模型已公开可用。
...
TL;DR: 本文是一项关于医学图像分割中不确定性量化的综合实证研究，探讨了 aleatoric 不确定性(AU) 和 epistemic 不确定性(EU) 模型的组合，提出了一种量化不确定性纠缠的指标，并在下游UQ任务中进行评估。


<details>
  <summary>Details</summary>
Motivation: 不确定性量化在医学图像分割等安全关键应用中至关重要。AU和EU的分解可解释性和实用性受到二者之间显著纠缠的挑战，且尚不清楚不同AU和EU方法组合时的交互方式。

Method: 对广泛的AU-EU模型组合进行综合实证研究，提出不确定性纠缠量化指标，并在分布外检测、歧义建模和校准等下游UQ任务中进行评估。

Result: 分布外检测中，集成方法显示更低的纠缠和更优性能；歧义建模和校准中最佳模型与数据集相关，softmax/SSN方法表现良好，Probabilistic UNets纠缠较低；softmax集成在所有任务中表现优异。

Conclusion: 分析了不确定性纠缠的潜在来源，并提出了缓解该效应的方向。
...
TL;DR: Perceptio是一个增强版的大视觉语言模型，通过在自回归序列中显式生成语义分割token和深度token来解决LVLMs在细粒度空间定位方面的不足。该模型基于InternVL构建，在多个基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)在语义理解方面表现出色，但在细粒度空间定位方面存在困难。模型需要隐式推断复杂几何结构，却无法产生空间解释。这激发了研究者开发具有2D和3D空间推理能力的感知增强型LVLM。

Method: 1. 从强大的单目教师模型中蒸馏VQVAE深度码本，将密集深度 token化为紧凑序列；2. 集成SAM2基于语义分割的token和VQ-VAE深度token到LLM中，使模型先发射空间token再回答；3. 引入复合深度token目标（marker、token和count损失）和软合并技术以稳定深度token生成；4. 采用多任务协同训练策略。

Result: 在RefCOCO/+/g上 referring expression segmentation 分别提升+0.8/+1.4/+1.1 cIoU；BLINK空间理解准确率提升10.3%；MMBench准确率提升1.0%。

Conclusion: 显式的空间思维链（spatial chain-of-thought）能够显著增强LVLMs的空间定位能力，为多模态理解中的空间推理提供了有效途径。
...
TL;DR: Dual-IFM是一种专为视网膜影像设计的可解释基础模型，通过类别证据图提供局部可解释性，通过2D投影层提供全局可解释性，在80万张彩色眼底照片上预训练，性能与参数量多16倍的模型相当。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型依赖可解释性有限的架构，这在医疗影像等高风险领域是关键问题。研究者提出需要在保证性能的同时提供可解释的预测，以增强医生对AI辅助诊断的信任度。

Method: 1)提出Dual-IFM框架，具有两种可解释性机制：类别证据图(局部可解释性)和2D投影层(全局可解释性)；2)在超过80万张来自不同来源的彩色眼底照片上进行自监督学习预训练；3)对分布外数据进行可解释性预测验证。

Result: 性能与最先进的基础模型相当，但参数量仅为其1/16；能够对分布外数据提供可解释的预测；学习到了可泛化的视网膜影像表示。

Conclusion: 大规模自监督预训练结合内在可解释性可以产生用于视网膜影像的鲁棒表示，为医疗AI的可解释性发展提供了新方向。
...
TL;DR: 本文提出HORNet，一个基于GRPO训练的轻量级帧选择策略，用于从视频中选择最相关的帧供VLM回答问题。该方法将帧选择任务形式化为"选择任意帧"(SAF)，与VLM推理解耦。HORNet仅需不到100万可训练参数，即可将输入帧减少99%，处理时间减少93%，同时在多个基准测试上提升答案质量。


<details>
  <summary>Details</summary>
Motivation: 当前的视频问答系统依赖均匀采样或启发式方法选择帧，这些方法无法针对下游问答质量进行优化。视频帧选择对VQA至关重要，但缺乏系统性的优化方法。

Method: 提出HORNet框架，使用Group Relative Policy Optimization (GRPO)训练轻量级帧选择策略（少于100万参数）。将任务形式化为"选择任意帧"(SAF)，将视觉输入筛选与VLM推理解耦。策略可跨VLM迁移使用。

Result: 在短形式基准测试中提升F1分数1.7%（MSVD-QA），在时间推理任务中比均匀采样提升7.3分（NExT-QA）。减少输入帧达99%，VLM处理时间减少93%。跨VLM迁移时可额外获得8.5%相对增益。评估涵盖6个基准测试、341,877个QA对和114.2小时视频。

Conclusion: 优化VLM"看到什么"是提升模型性能和效率的实用且互补的方法，与优化"生成什么"同等重要。GRPO训练的选择策略比监督学习和PPO具有更好的分布外泛化能力。
...
TL;DR: 本文提出了空间-时间-轨迹(STT)推理任务，并设计了Motion-o模型和运动思维链(MCoT)推理框架，使视觉语言模型能够显式地理解和验证物体在连续观测间的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理研究虽然关注时空证据链，但忽略了物体如何在观测之间移动的推理。轨迹理解仍然是隐式的，难以验证。本文希望填补这一空白。

Method: 1)提出STT推理任务 formalization; 2)设计Motion-o模型，通过<motion/>标签显式表示每帧中物体的方向、速度和尺度变化; 3)构建轨迹接地数据集扩展，通过增强稀疏关键帧监督生成更密集的边界框轨迹; 4)提出MCoT结构化推理路径; 5)设计奖励函数使模型直接基于视觉证据进行推理，无需修改架构。

Result: Motion-o提升了时空接地和轨迹预测能力，与现有框架完全兼容，确立了运动推理作为证据驱动视频理解的关键扩展。

Conclusion: 运动推理是视频理解的重要扩展方向，Motion-o通过显式轨迹建模和结构化推理证明了其有效性和兼容性。
...
TL;DR: PASTA是一个基于条件扩散模型的新型医学图像转换框架，用于从MRI合成PET图像，强调病理感知能力，解决了现有方法只注重结构保留而忽视病理细节的问题。


<details>
  <summary>Details</summary>
Motivation: PET虽然是诊断神经退行性疾病的重要技术，但成本高且存在辐射暴露；MRI虽无此限制但敏感性较低。现有从MRI生成合成PET的方法主要强调结构保留，忽略了病理感知的重要性。

Method: 提出PASTA框架，包含：1)高度交互的双臂架构；2)多模态条件集成；3)新型循环交换一致性；4)体生成策略。基于条件扩散模型构建，专门增强病理感知能力。

Result: PASTA在定性和定量结果上均优于现有最先进方法，能够生成高质量3D PET图像并保留病理细节。在阿尔茨海默病诊断中，合成PET比MRI性能提升4%，几乎达到实际PET水平。

Conclusion: PASTA成功实现了从MRI到PET的高质量跨模态医学图像转换，显著保留了病理信息，为神经退行性疾病诊断提供了有价值的工具。
...
TL;DR: GHOST是一个基于2D Gaussian Splatting的快速、类别无关的手-物体交互重建框架，通过几何先验检索、抓取感知对齐和手部感知背景损失三项创新，实现了从单RGB视频中提取完整、物理一致且可动画化的手-物体交互模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于类别特定模板或重计算，且在3D中产生物理上不一致的手-物体对齐。研究者需要一种快速、类别无关的框架来从单RGB视频重建动态手-物体交互。

Method: GHOST将手和物体表示为密集的、视图一致的Gaussian圆盘，引入了三个关键创新：1) 几何先验检索和一致性损失来完成遮挡的物体区域；2) 抓取感知对齐以精炼手部平移和物体尺度确保真实接触；3) 手部感知背景损失以避免惩罚手部遮挡的物体区域。

Result: GHOST实现了完整、物理一致且可动画化的重建，在ARCTIC、HO3D和野外数据集上进行了广泛实验，表明其在3D重建精度和2D渲染质量方面达到了最先进水平。

Conclusion: GHOST作为一个高效且鲁棒的 hand-object interaction 建模解决方案，在3D重建和2D渲染质量方面实现了最先进的精度，且运行速度比之前的类别无关方法快一个数量级。
...
TL;DR: 本文提出了一种基于无监督对比学习的3D形状匹配方法，通过最大化正样本对相似度、最小化负样本对相似度来提升特征质量，并设计了简化版功能映射学习架构以提高计算效率，在近等距、非等距和拓扑不一致等多种挑战性场景下达到最先进的匹配性能。


<details>
  <summary>Details</summary>
Motivation: 现有的深度功能映射方法主要关注逐点或功能映射的单独或联合优化，而非直接增强嵌入空间的特征表示，导致特征质量不足和匹配性能次优；同时这些方法依赖耗时的功能映射求解器，计算成本高昂。

Method: 提出统一的双分支管道：一是无监督对比学习框架，通过促进正相似样本对内的一致性并最小化负样本对的一致性来提升特征的判别性；二是简化的功能映射学习架构，消除了计算昂贵功能映射求解器和多个辅助功能映射损失的需求。

Result: 在多种挑战性基准测试中（包括近等距、非等距和拓扑不一致场景）达到最先进的准确率和效率表现，甚至超越了监督学习方法。

Conclusion: 该方法通过无监督对比学习有效提升了特征表示的质量，结合简化的功能映射架构大幅提高了计算效率，在无需标注数据的情况下实现了优于现有监督方法的性能，为3D形状匹配领域提供了新的研究思路。
...
TL;DR: VGGT-360是一个无需训练的零样本全景深度估计框架，通过将任务重新表述为基于多视角重建3D模型的全景重投影，利用VGGT类基础模型的内在3D一致性，统一碎片化的单视图推理为连贯的全景理解。


<details>
  <summary>Details</summary>
Motivation: 现有无训练方法采用视图无关方式处理全景深度估计，导致推理碎片化。VGGT-360旨在通过全景重投影范式解决这一问题，利用多视角重建3D模型的内在3D一致性，实现连贯的全景深度估计。

Method: VGGT-360集成了三个即插即用模块形成全景到3D再到深度的统一框架：1) 不确定性引导的自适应投影将全景图切分为透视视图，通过梯度不确定性为几何信息不足区域分配更密集的视角；2) 结构显著性增强的注意力机制向VGGT的注意力层注入结构感知置信度，引导模型关注几何可靠区域并增强跨视图一致性；3) 相关性加权的3D模型校正利用注意力推断的相关性分数对重叠点进行重加权，优化重建的3D模型。

Result: VGGT-360在多种分辨率和不同的室内外数据集上均优于现有的有训练和无训练方法。

Conclusion: VGGT-360通过无需训练的方式成功实现了零样本、几何一致的全景深度估计，为全景深度估计领域提供了一种新范式。
...
TL;DR: CRAFT是一种轻量级但强大的扩散模型微调范式，通过Composite Reward Filtering技术从少量数据中构建高质量训练集，理论证明其优化了基于组的强化学习下界，100个样本即可超越数千样本的SOTA方法


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型对齐方法（SFT和DPO）存在数据依赖性强和计算效率低的问题：SFT依赖高质量但昂贵获取的图像，DPO依赖大规模且质量不一致的偏好数据集

Method: 1. Composite Reward Filtering (CRF)：构建高质量一致的训练数据集；2. 增强版SFT进行微调

Result: 1. 仅用100个样本即可超越使用数千个偏好配对样本的SOTA方法；2. 收敛速度比基线偏好优化方法快11-220倍

Conclusion: CRAFT在仅使用100个样本的情况下就能超越使用数千个偏好配对样本的SOTA偏好优化方法，且收敛速度比基线方法快11-220倍，证明了其高效性。
...
TL;DR: 本文提出了一种极简主义的指纹增强方法，包括两种新技术：上下文滤波方法和基于学习的方法。这些技术专注于简单性和有效性，在低质量指纹图像处理方面优于复杂的先进方法，并在具有挑战性的潜在指纹数据库上进行了验证。


<details>
  <summary>Details</summary>
Motivation: 尽管指纹识别系统在现代安全验证中至关重要，但当前最先进的指纹增强技术在处理低质量指纹时表现不佳，且计算成本高。现需要一种既有效又简单的指纹增强方法。

Method: 提出了两种新颖的指纹增强方法：1) 上下文滤波方法；2) 基于学习的方法。这两种方法都采用极简主义设计理念，追求简单性和有效性。

Result: 实验表明，这两种方法始终优于复杂的最先进方法，能够产生更清晰、更准确、噪声更少的指纹图像。验证在具有挑战性的潜在指纹数据库上进行。

Conclusion: 研究结果强调了简单性在实现高质量指纹增强中的重要性，表明未来研究应在复杂性和实际效益之间取得平衡。开源实现促进了该领域的进一步发展。
...
TL;DR: GenHOI是一个用于从单张RGB图像进行广义3D手-物体姿态估计的框架，通过整合层次语义知识与手部先验来增强模型在严重遮挡条件下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 从单张RGB图像进行广义3D手-物体姿态估计仍具挑战性，因为物体外观和交互模式变化大，尤其是在严重遮挡条件下。

Method: 提出层次语义提示编码物体状态、手部配置和交互模式的文本描述；采用多模态掩码建模策略处理RGB图像、预测点云和文本描述；利用手部先验作为稳定的空间参考来提取隐式交互约束。

Result: 在DexYCB和HO3Dv2两个具有挑战性的基准数据集上实现了最先进的手-物体姿态估计性能。

Conclusion: GenHOE通过层次语义知识与手部先验的结合，能够有效提升模型在遮挡条件下的泛化能力，为手-物体交互估计提供了一种新的思路。
...
TL;DR: SELF1E提出一种无需外部解码器的多模态大语言模型分割方法，通过保留原始分辨率图像特征、填充残差特征、像素反 shuffle 操作和双感知通道注意力掩码，实现与专用掩码解码器方法相当的分割性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型分割方法主要依赖专用掩码解码器来解释分割嵌入和视觉特征，本文旨在探索能否仅通过MLLM自身实现分割，消除对外部解码器的依赖。

Method: 1. 保留原始未压缩分辨率的图像特征；2. 用MLLM处理的压缩特征中提取的残差特征进行填充，提高特征精度；3. 对经LLM处理和未处理的图像特征分别进行像素反shuffle操作，释放压缩特征的细节并在未压缩分辨率下放大残差特征；4. 重新设计双感知通道（图像到图像、图像到分割）的注意力掩码，实现像素与分割token之间的丰富特征交互。

Result: SELF1E在多个分割任务上取得了与专用掩码解码器方法相当的性能，验证了MLLM中无解码器分割的可行性。

Conclusion: 通过解决MLLM中像素 shuffling 带来的分辨率降低问题，SELF1E成功实现了无需外部解码器的分割方法，为多模态大语言模型的分割能力提供了新的研究方向。
...
TL;DR: TerraScope是一个统一的视觉语言模型,用于在地球观测中提供像素级接地气的地理空间推理,具有模态灵活推理和多时序推理能力,并发布了大规模数据集Terra-CoT和首个像素级地理空间推理基准TerraScope-Bench。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在地球观测任务中存在局限性,难以在复杂的空间推理中实现精确的像素级视觉表示接地。

Method: 提出TerraScope模型,具备:(1)模态灵活推理-处理单模态输入(光学或SAR)并在多模态可用时自适应融合;(2)多时序推理-整合时间序列进行多时间点变化分析。同时构建Terra-CoT数据集(100万样本,包含像素级掩码和推理链)和TerraScope-Bench基准(6个子任务,评估答案准确性和掩码质量)。

Result: TerraScope在像素级地理空间推理任务上显著优于现有视觉语言模型,同时提供可解释的视觉证据。

Conclusion: TerraScope实现了统一的像素级接地气地理空间推理框架,为地球观测领域的多模态和多时序视觉语言理解提供了新的研究方向。
...
TL;DR: SGC是一种评估动态生成视频中3D空间几何一致性的新指标，通过测量从不同局部区域估计的多个相机姿态之间的分歧来量化几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法无法准确表征3D空间几何不一致性：保真度指标（如FVD）对几何扭曲不敏感，而一致性基准测试往往惩罚有效的前景动态。

Method: 1）将静态区域与动态区域分离；2）将静态背景分割成空间连贯的子区域；3）为每个像素预测深度；4）估计每个子区域的局部相机姿态；5）计算这些姿态之间的分歧来量化几何一致性。

Result: 在真实和生成视频上的实验表明，SGC能够稳健地量化几何不一致性，有效识别现有指标遗漏的关键失败。

Conclusion: SGC能够有效评估动态生成视频中的3D空间几何一致性，弥补了现有指标的不足。
...
TL;DR: SwiftTailor是一个新型两阶段框架，通过紧凑的几何图像表示统一缝制图案推理和基于几何的网格合成，显著加速3D服装生成，同时保持高质量的视觉效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法虽然能生成高质量3D服装，但推理时间长达30秒到一分钟，无法满足实际应用需求。

Method: 提出两阶段框架：1) PatternMaker：高效视觉语言模型，从多种输入模态预测缝制图案；2) GarmentSewer：高效密集预测Transformer，将图案转换为统一UV空间中的服装几何图像；最后通过逆映射、重网格化和动态缝合算法重建3D网格。

Result: 在Multimodal GarmentCodeData数据集上实现了最先进的精度和视觉保真度，同时显著降低了推理时间。

Conclusion: 该工作为下一代3D服装生成提供了可扩展、可解释且高性能的解决方案。
...
TL;DR: Em-Garde框架通过将语义理解与流式感知解耦，解决了现有主动视频理解模型中效率-准确性的权衡问题。该方法在查询时将用户查询转换为视觉提案，并在流式传输过程中进行轻量级匹配。实验表明其在保持高效率的同时显著提升了主动响应准确率。


<details>
  <summary>Details</summary>
Motivation: 当前主动VideoLLMs依赖逐帧触发决策，面临效率-准确性两难困境：精细帧处理计算成本高，而稀疏采样又可能导致关键信息遗漏。

Method: 提出Em-Garde框架，包含两个核心模块：1) 指令引导提案解析器(Instruct-Guided Proposal Parser)在查询时将用户指令转换为结构化视觉提案；2) 轻量级提案匹配模块(Lightweight Proposal Matching Module)在流式感知中进行高效的基于嵌入的匹配以触发响应。

Result: 在StreamingBench和OVO-Bench上的实验表明，Em-Garde在主动响应准确性和效率方面均优于现有模型。

Conclusion: Em-Garde为在严格计算约束下的主动视频理解提供了有效解决方案，验证了语义理解与流式感知解耦架构的可行性。
...
### [77] [SignAgent: Agentic LLMs for Linguistically-Grounded Sign Language Annotation and Dataset Curation](https://arxiv.org/abs/2603.19059)
*Oliver Cory,Ozge Mercanoglu Sincan,Richard Bowden*

Main category: cs.CV

TL;DR: SignAgent是一个利用大语言模型(LLM)实现可扩展、语言学基础的手语注释和数据集管理的智能体框架，解决了传统手语研究停留在手势层而忽略语言学细节、以及人工注释成本高效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 传统计算方法通常在手势(googletag)层面操作，忽视了关键的语言学特征；手动语言注释耗时且昂贵，难以创建大规模、语音学感知的数据集。

Method: 框架包含两个核心组件：1) SignAgent Orchestrator（协调器）：负责推理的LLM，协调各种语言学工具；2) SignGraph：提供词汇和语言基础的LLM。评估任务：1) 伪手势注释（Pseudo-gloss Annotation）：使用多模态证据进行约束分配；2) ID注释（ID Glossing）：通过视觉相似性和语音重叠进行推理。

Result: 智能体方法在大规模、语言学感知的数据注释和管理方面表现出色。

Conclusion: 该框架为手语的大规模语言学注释提供了新的解决方案，有望推动手语处理领域的发展。
...
TL;DR: 提出了一种基于可微不确定性感知束束调整的鲁棒实时RGB SLAM系统，能够在动态环境中有效处理运动物体和复杂场景，实现约10 FPS的实时性能。


<details>
  <summary>Details</summary>
Motivation: 传统SLAM方法假设静态场景，在存在运动的情况下会导致跟踪失败；现有的动态SLAM方法依赖预定义的动态先验或不确定性感知映射，但在未知动态物体或高度杂乱场景中仍存在局限性，几何映射变得不可靠。

Method: 通过利用多视图视觉特征不一致性估计每像素不确定性，实现可微不确定性感知束束调整，使系统能够在动态环境中实现鲁棒跟踪和重建。

Result: 在杂乱动态场景中实现了最先进的相机位姿和场景几何精度，同时保持约10 FPS的实时运行速度。

Conclusion: 该方法有效解决了动态环境下的SLAM问题，通过不确定性感知机制提高了系统的鲁棒性和精度，为实际动态场景应用提供了可行方案。
...
TL;DR: 本文提出了Roundabout-TAU数据集和TAU-R1框架，用于解决智能交通系统中交通异常理解(TAU)任务的数据集和方法论匮乏问题。数据集包含342个真实环岛视频片段和2000多个问答对，框架采用两层VLM架构结合两阶段训练策略(分解QA增强的SFT + TAU-GRPO后训练)，在异常分类和推理任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 交通异常理解(TAU)对智能交通系统(ITS)的交通安全至关重要。近期视觉-语言模型(VLM)在视频理解方面展现出强大能力，但TAU领域进展仍有限，主要原因是缺乏基准数据集和任务特定方法论。

Method: 提出TAU-R1两层视觉-语言框架：第一层是轻量级异常分类器进行粗粒度异常分类，第二层是大型异常推理器生成详细事件摘要。训练采用两阶段策略：分解QA增强的监督微调(SFT) followed by TAU-GRPO(基于GRPO的后训练方法，使用TAU特定奖励函数)。

Result: TAU-R1在异常分类和推理任务上均取得强劲性能，同时保持了部署效率。数据集包含342个视频片段和超过2000个问答对，涵盖交通异常理解的多个方面。

Conclusion: 通过构建真实世界的环岛视频数据集Roundabout-TAU和提出两阶段训练的TAU-R1框架，有效推动了交通异常理解领域的研究，为ITS交通安全提供了有价值的基准和方法。
...
TL;DR: CustomTex是一个用于实例级高保真3D场景纹理生成的新框架，通过参考图像驱动，结合双重蒸馏方法（语义级蒸馏和像素级蒸馏）在VSD优化框架内实现高分辨率纹理生成。


<details>
  <summary>Details</summary>
Motivation: 当前文本驱动的3D纹理生成方法缺乏细粒度实例级控制，生成的纹理质量不足、存在伪影和光照烘焙问题。

Method: 提出双重蒸馏方法：1) 语义级蒸馏配备实例交叉注意力确保语义合理性和参考-实例对齐；2) 像素级蒸馏确保高视觉保真度。两者统一在变分分数蒸馏(VSD)优化框架中。

Result: CustomTex实现了精确的实例级参考图像一致性，生成纹理具有卓越的清晰度、减少的伪影和最小化的光照烘焙。
...
TL;DR: 本文重新审视了视觉自回归(AR)生成分类器，通过利用任意顺序AR模型估计顺序边际化预测，解决了传统固定顺序AR方法的局限性，在图像分类任务上超越了扩散模型分类器，且效率提升高达25倍。


<details>
  <summary>Details</summary>
Motivation: 传统视觉AR分类器依赖固定token顺序，这对图像理解施加了限制性归纳偏置。单顺序预测更多依赖部分判别线索，而多顺序平均能提供更全面的信号。这一观察促使作者探索如何释放AR模型的分类潜力。

Method: 利用最新的任意顺序(any-order)AR模型估计顺序边际化预测，通过对多个token顺序进行平均来获得更全面的分类信号。

Result: 在多种图像分类基准数据集上，方法一致性地超越了扩散模型分类器，效率提升高达25倍。与最先进的自监督判别模型相比，方法也具有竞争力的分类性能。

Conclusion: 通过利用任意顺序AR模型进行顺序边际化预测，可以有效释放自回归生成模型的图像分类潜力，这一方法在性能上超越了扩散模型分类器，同时保持了显著的计算效率优势。
...
TL;DR: 提出了ADAPT框架，一个无训练的框架，用于在文本到图像合成中生成稀有组合概念，通过注意力分数和正交成分来确定性地计划和语义对齐提示调度。


<details>
  <summary>Details</summary>
Motivation: 稀有组合概念在扩散模型的训练数据中不常见，现有方法如R2F存在语言模型的随机性导致的方差问题和迭代文本嵌入切换的次优引导问题。

Method: ADAPT框架利用注意力分数和正交成分来确定性地计划和语义对齐提示调度，为增强稀有概念组合提供一致的引导，无需额外训练或微调。

Result: 在RareBench基准上显著增强了稀有概念的组合生成，实现了优于现有方法的性能，准确反映稀有属性的语义信息。

Conclusion: ADAPT提供了对稀有组合生成的决定性和精确控制，在不损害视觉完整性的情况下增强了扩散模型的稀有概念合成能力。
...
TL;DR: 本文提出AAPB框架，通过自适应辅助提示混合解决扩散模型在低密度区域生成罕见概念或编辑时出现的语义错位和结构不一致问题，基于Tweedie恒等式提供无需训练的自适应提示融合方法。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在训练分布的低密度区域（罕见概念或编辑指令）表现不佳，常产生语义错位或结构不一致的结果，这是因为文本-图像数据集的长尾分布导致稀有概念被低估。

Method: AAPB利用辅助锚点提示为罕见概念生成提供语义支持，为图像编辑提供结构支持，通过基于Tweedie恒等式的闭合形式自适应系数，在每个扩散步骤中最优平衡辅助锚点和目标提示的影响，实现无需训练的自适应提示混合。

Result: 在RareBench和FlowEdit数据集上的控制实验表明，自适应插值优于固定插值方法，在语义准确性和结构保真度方面均优于先前无需训练的基线方法。

Conclusion: AAPB为低密度区域的自适应提示混合提供了原则性、无需训练的框架，确保稳定且忠实于目标的生成，在罕见概念生成和图像编辑任务上展现了 Consistently一致的改进。
...
TL;DR: ARIADNE是一种结合偏好对齐感知与基于RL的诊断推理的两阶段框架，用于冠脉血管分割的拓扑一致性狭窄检测。该方法首次将DPO应用于医学影像的拓扑对齐。


<details>
  <summary>Details</summary>
Motivation: 传统像素级损失函数无法强制拓扑约束，导致血管树碎片化；现有方法在分叉和交叉处易产生误报，缺乏对模糊解剖候选区域的拒绝机制。

Method: 感知模块：使用DPO微调Sa2VA视觉语言基础模型，以Betti数约束作为偏好信号；推理模块：将狭窄定位建模为MDP，包含显式拒绝机制以自主推迟分叉和交叉等模糊解剖候选区域。

Result: 在1400张临床血管造影上达到0.838的中心线Dice，比几何基线减少41%误报；外部验证在ARCADE和XCAD多中心基准上确认了跨采集协议的泛化能力。

Conclusion: 首次将DPO应用于医学影像拓扑对齐，证明了基于结构约束的偏好学习可缓解拓扑违规，同时保持介入心脏病学工作流程中的诊断敏感性。
...
### [85] [DreamPartGen: Semantically Grounded Part-Level 3D Generation via Collaborative Latent Denoising](https://arxiv.org/abs/2603.19216)
*Tianjiao Yu,Xinzhuo Li,Muntasir Wahed,Jerry Xiong,Yifan Shen,Ying Shen,Ismini Lourentzou*

Main category: cs.CV

TL;DR: DreamPartGen是一个语义锚定的部件感知文本到3D生成框架，通过双部件潜在变量(DPLs)和关系语义潜在变量(RSLs)实现一致的三维合成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D方法忽略部件的语义和功能结构，现有的部件感知方法仍以几何为中心，缺乏语义基础，无法建模部件与文本描述的对齐及部件间关系。

Method: 提出Duplex Part Latents (DPLs)联合建模每个部件的几何和外观，以及Relational Semantic Latents (RSLs)捕捉来自语言推导的部件间依赖关系，采用同步协同去噪过程确保几何和语义一致性。

Result: 在多个基准测试中，DreamPartGen在几何保真度和文本-形状对齐方面实现了最先进的性能。

Conclusion: DreamPartGen能够实现连贯、可解释且文本对齐的3D合成，为部件感知的文本到3D生成提供了新的研究方向。
...
TL;DR: 本文提出Splat2BEV框架，利用Gaussian Splatting技术辅助BEV感知任务，通过显式3D重建提升BEV特征的几何精度和语义丰富度。


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知框架采用端到端训练，缺乏显式的3D几何理解和可解释性，导致性能次优。

Method: 预训练高斯生成器从多视角输入显式重建3D场景，生成几何对齐的特征表示，再投影到BEV空间作为下游任务输入。

Result: 在nuScenes和argoverse数据集上实现SOTA性能，验证了将显式3D重建融入BEV感知有效性。

Conclusion: 显式3D表示对准确BEV感知至关重要，Splat2BEV通过结合Gaussian Splatting与BEV感知显著提升了性能。
...
TL;DR: 研究揭示视觉语言模型(VLM)存在"选择性失明"问题——即使不同语言表述要求相同的视觉推理，模型也会根据语言框架调整对视觉输入的注意力分配。


<details>
  <summary>Details</summary>
Motivation: VLM常被证明是"盲的"，即使在需要视觉推理的任务中也未能充分利用视觉输入。

Method: 使用视觉注意力作为探针，量化框架如何改变图像上的注意力数量和分布；提出一种基于可学习标记的轻量级提示调整方法。

Result: 约束性框架（如选择题和是否题）导致对图像情境的注意力显著降低，任务相关区域的聚焦减少，注意力转向无信息标记。这种注意力错配是准确率下降和跨框架不一致的主要原因。

Conclusion: 所提方法可鼓励稳健的视觉注意力模式，改善视觉定位并提升跨框架性能。
...
TL;DR: 本文提出Representation-Pivoted AutoEncoder (RPiAE)，一种基于预训练视觉表示的tokenizer，通过表示轴正则化和变分桥接压缩技术，在保持语义信息的同时提高重建保真度并降低扩散建模复杂度，在文本到图像生成和图像编辑任务上优于其他视觉tokenizer。


<details>
  <summary>Details</summary>
Motivation: 当前利用预训练视觉表示模型作为tokenizer先验的方法存在两个主要问题：1) 冻结编码器导致重建保真度有限，进而影响编辑质量；2) 潜在空间维度太高，增加扩散建模难度。因此需要一种方法既能保留语义结构，又能实现高质量重建和高效扩散建模。

Method: 提出三个核心技术创新：1) 表示轴正则化(Representation-Pivot Regularization)：允许初始化的表示编码器进行微调以重建，同时保持预训练表示空间的语义结构；2) 变分桥接(Variational Bridge)：将潜在空间压缩为紧凑空间以改善扩散建模；3) 目标解耦的阶段性训练策略：顺序优化生成可处理性和重建保真度目标。

Result: 实验表明，RPiAE在文本到图像生成和图像编辑任务上优于其他视觉tokenizer，同时在表示基tokenizer中实现最佳重建保真度。

Conclusion: RPiAE成功实现了语义保留、忠实重建和降低扩散建模复杂度的三重目标，为视觉tokenizer设计提供了新的有效范式。
...
TL;DR: LVOmniBench是一个针对长形式音视频跨模态理解的新 benchmark，包含275个视频（10-90分钟）和1,014个问答对，用于评估OmniLLMs在长期记忆、时间定位、细粒度理解和多模态感知方面的能力


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注10秒到5分钟的短音视频 clips，未能反映实际应用中数十分钟视频的需求，存在关键差距

Method: 从开放平台获取高质量视频，经过严格的人工筛选和标注，构建包含275个视频（时长10-90分钟）和1,014个问答对的 benchmark

Result: 当前OmniLLMs在处理长音视频输入时面临重大挑战：开源模型准确率普遍低于35%，Gemini 3 Pro达到最高约65%的准确率

Conclusion: LVOmniBench benchmark能够激发后续研究，推动能够处理复杂长形式音视频跨模态理解问题的高级模型开发。当前OmniLLMs在处理长音频和视频输入时面临重大挑战。
...
TL;DR: 本文提出了一种基于向量场重塑的扩散模型分割方法，通过引入距离感知校正项解决流匹配目标中的梯度消失和轨迹穿越问题，并设计了基于Kronecker序列的类别编码方案，显著提升了生成式分割的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有扩散分割方法主要关注架构调整或训练启发式，对连续流匹配目标与离散感知任务之间的内在不匹配理解有限。流匹配目标存在梯度消失和轨迹穿越问题，导致收敛慢、类别分离度差。

Method: 提出向量场重塑策略：1)为学习的速度场添加分离的距离感知校正项，引入吸引和排斥交互；2)设计基于Kronecker序列的高效准随机类别编码，与像素级神经场框架集成实现像素级语义对齐。

Result: 实验表明该方法相比普通流匹配方法有显著提升，大幅缩小了生成式分割与强判别式专家模型之间的性能差距。

Conclusion: 通过向量场重塑策略和新型类别编码方案，可有效解决扩散模型用于分割任务的核心问题，为生成式分割提供了一种有原则的解决方案。
...
TL;DR: DriveTok是一个高效的3D驾驶场景分词器，用于统一的多视图重建和理解，解决了现有分词器在处理高分辨率多视图驾驶场景时的效率和视图间不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 随着视觉-语言-动作模型和世界模型在自动驾驶系统中的广泛应用，可扩展的图像分词变得至关重要。然而，现有分词器大多针对单目和2D场景设计，在应用于高分辨率多视图驾驶场景时效率低下且存在视图间不一致问题。

Method: DriveTok首先从视觉基础模型获取语义丰富的视觉特征，然后通过3D可变形交叉注意力将其转换为场景分词符。解码时，使用多视图Transformer从场景分词符重建多视图特征，并使用多个头获取RGB、深度和语义重建。此外，还在场景分词符上直接添加3D头进行3D语义占用预测。

Result: 在广泛使用的nuScenes数据集上的广泛实验表明，DriveTok的场景分词符在图像重建、语义分割、深度预测和3D占用预测任务上表现良好。

Conclusion: 通过多个训练目标，DriveTok学习到统一的场景分词符，整合了语义、几何和纹理信息，实现了高效的多视图分词。
...
TL;DR: 本文提出了一种基于图像频谱特性的原则性方法来设计像素扩散模型的实例化噪声调度方案，通过推导噪声水平的理论边界来设计"紧凑"噪声调度以消除冗余步骤，并在推理时进行条件采样，实验表明该方法在低步数情况下显著提升了单阶段像素扩散模型的生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前噪声调度方案依赖人工设计且需要针对不同分辨率手动调参，缺乏原则性的设计方法。

Method: 基于图像频谱特性推导噪声水平的理论上下界，设计"紧凑"噪声调度消除冗余步骤，并在推理时进行条件采样。

Result: 所提噪声调度方案在单阶段像素扩散模型中提升了生成质量，尤其在低步数情况下效果显著。

Conclusion: 基于频谱特性的原则性噪声调度设计方法能够有效提升扩散模型的生成效率和质量。
...
TL;DR: 本文提出了VOR大规模视频对象去除数据集和EffectErase方法，用于消除视频中的动态目标对象及其视觉效果（变形、阴影、反射等），同时恢复无缝背景。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复和对象去除方法虽然能移除对象，但难以消除这些视觉效果并合成连贯的背景。此外，缺乏一个全面且系统的数据集来训练和评估这些方法。

Method: 1) 构建VOR数据集：包含60K高质量视频对，每对包含对象存在的视频及其效果，以及对象和效果缺失的对应视频，配有对象掩码，覆盖5种效果类型和多种对象类别及复杂动态场景。2) 提出EffectErase方法：采用互惠学习框架，将视频对象插入作为逆辅助任务，包含任务感知区域指导模块和插入-去除一致性目标。

Result: 在广泛实验中，EffectErase在多种场景下实现了卓越的视频对象效果消除性能，生成高质量视频。

Conclusion: VOR数据集和EffectErase方法有效推动了视频对象去除领域的研究，为消除对象及其视觉效果提供了全面的训练资源和创新方法。
...
TL;DR: MultiGP是一种生成式逆渲染方法，能够从单张图像中随机采样所有辐射分量（反射率、纹理和光照）。其核心思路是利用同一场景中所有物体共享相同光照的共识来解决固有的辐射解耦歧义问题。


<details>
  <summary>Details</summary>
Motivation: 从单张图像中恢复物体的反射率、纹理和光照是一个固有的不适定问题，因为这三个辐射分量在外观上是纠缠在一起的。现有方法难以同时恢复这三个分量且保持一致性。

Method: 提出四种关键技术：1) 级联端到端架构结合图像空间和角空间解耦；2) 协调引导（Coordinated Guidance）使扩散模型收敛到单一一致的光照估计；3) 轴向注意力（Axial Attention）促进不同反射率物体之间的信息交互；4) 纹理提取ControlNet在保持高频纹理细节的同时实现与光照的解耦。

Result: 实验表明MultiGP能够有效利用多物体外观的互补空间和频率特性，分别恢复各物体的纹理和反射率以及共同的光照。

Conclusion: MultiGP通过利用同一场景中多物体的互补特性，成功实现了从单张已知形状的图像中解耦反射率、纹理和光照三个辐射分量，为生成式逆渲染提供了新的思路。
...
TL;DR: 本文提出一个三阶段框架，结合离散token生成和连续扩散模型的优点，用于人体动作生成。该框架包含感知、规划和控制三个阶段，核心创新是MoTok——一种基于扩散的离散动作tokenizer。


<details>
  <summary>Details</summary>
Motivation: 当前动作生成主要有两种范式：连续扩散模型擅长运动学控制，离散token生成器擅长语义调控。现有方法难以同时兼顾两者的优势，本文旨在结合这两种范式。

Method: 三阶段框架：(1)感知阶段进行条件特征提取；(2)规划阶段生成离散token；(3)控制阶段通过扩散模型合成动作。核心创新MoTok将语义抽象与细粒度重建解耦，通过扩散解码器恢复动作，实现紧凑的单层token表示。运动学约束中，粗粒度约束指导token生成，细粒度约束在控制阶段通过扩散优化实现。

Result: 在HumanML3D数据集上，相较于MaskControl：使用1/6的tokens，轨迹误差从0.72cm降至0.08cm，FID从0.083降至0.029。在更强运动学约束下，FID从0.033降至0.014，性能反而提升。

Conclusion: 该方法成功结合了离散token和扩散模型的优点，在保持语义调控能力的同时显著提升了运动学控制精度和生成质量，且约束越强效果越好，验证了框架设计的有效性。
...
TL;DR: SAMA是一个将视频编辑分解为语义锚定和运动建模的框架，通过语义锚定建立可靠的视觉锚点，通过运动对齐预训练学习时间动态，无需配对视频编辑数据即可实现强大的零样本视频编辑能力。


<details>
  <summary>Details</summary>
Motivation: 当前指令引导的视频编辑模型难以同时实现精确的语义修改和忠实的运动保持。现有方法依赖注入外部先验（如VLM特征或结构条件），这严重限制了模型的鲁棒性和泛化能力。

Method: 提出两阶段训练管道：1）分解预训练阶段，学习固有语义-运动表示，无需配对的视频-指令编辑数据；2）监督微调阶段在配对编辑数据上训练。语义锚定通过联合预测稀疏锚帧的语义令牌和视频潜变量实现；运动对齐通过三种运动中心视频修复预训练任务（立方体修复、速度扰动、管置换）进行预训练。

Result: 在零样本条件下已具备强大的视频编辑能力，在开源模型中达到最先进性能，与Kling-Omni等商业系统具有竞争力。

Conclusion: 提出的分解预训练策略本身已能产生强大的零样本视频编辑能力，验证了语义-运动分解的有效性。SAMA在开源模型中实现SOTA，并具有与领先商业系统相当的竞争力。
...
TL;DR: MonoArt是一个从单张图像重建关节式3D物体的统一框架，通过渐进式结构推理避免直接关节回归的不稳定性，在重建精度和推理速度上达到最先进水平，并可应用于机器人操作和关节式场景重建。


<details>
  <summary>Details</summary>
Motivation: 从单张图像重建关节式3D物体的核心困难在于运动线索和物体结构之间的 entanglement（纠缠），这使得直接关节回归不稳定。现有方法如多视图监督、检索式组装或视频生成等存在可扩展性或效率问题。

Method: 提出渐进式结构推理框架MonoArt，将视觉观察逐步转换为：1) 规范几何结构 2) 结构化部件表示 3) 运动感知嵌入，在单一架构内完成推理，无需外部运动模板或多阶段流水线。

Result: 在PartNet-Mobility数据集上，OM（应该是MonoArt）实现了重建精度和推理速度的双重state-of-the-art。

Conclusion: 该框架能够稳定且可解释地进行关节推断，无需外部运动模板，还可推广到机器人操作和关节式场景重建任务中。
...
TL;DR: Cubic Discrete Diffusion (CubiD) 是一种针对高维表示（768-1024维）的首个离散生成模型，通过细粒度掩码机制实现任意维度、任意位置的掩码与预测，在保持生成步骤数T远小于hwd的同时学习丰富的空间内和空间间相关性。


<details>
  <summary>Details</summary>
Motivation: 当前离散生成方法局限于低维潜在token（8-32维），牺牲了语义丰富性；而高维预训练表示虽能弥补语义差距，但其离散生成面临根本性挑战。

Method: CubiD在高层离散表示上执行细粒度掩码：任意维度在任意位置均可被掩码并从部分观测中预测，生成步骤数T与特征维度无关（T << hwd）。

Result: 在ImageNet-256上实现SOTA离散生成性能，从900M到3.7B参数具有强扩展性；离散token保留了原始表示能力，可同时服务于理解和生成任务。

Conclusion: Cubic Discrete Diffusion成功实现了高维表示的离散生成，验证了统一多模态架构的可行性，为未来统一多模态研究奠定了基础。
...
TL;DR: VEGA-3D通过将预训练视频扩散模型重构为潜在世界模拟器，为多模态大语言模型注入空间推理能力。该方法从中间噪声层级提取时空特征，通过token级自适应门控融合机制整合语义表示，无需显式3D监督即可增强MLLMs的几何推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备强大的语义理解能力，但存在空间失明问题，难以进行细粒度几何推理和物理动力学理解。现有解决方案通常依赖显式3D模态或复杂几何脚手架，但受限于数据稀缺和泛化挑战。

Method: 提出VEGA-3D框架，利用大规模视频生成模型中隐含的空间先验。具体包括：1）将预训练视频扩散模型重新用作潜在世界模拟器；2）从中间噪声层级提取时空特征；3）通过token级自适应门控融合机制将几何线索与语义表示整合。

Result: 在3D场景理解、空间推理和具身操作 benchmarks 上进行广泛实验，结果表明该方法优于当前最先进基线，验证了生成先验可为物理世界理解提供可扩展的基础。

Conclusion: 视频生成模型中隐含的3D结构先验和物理规律可以有效用于增强MLLMs的空间推理能力，这种范式转换提供了一种无需显式3D监督的可扩展解决方案。
...
### [100] [Turnpike with Uncertain Measurements: Triangle-Equality ILP with a Deterministic Recovery Guarantee](https://arxiv.org/abs/2603.18283)
*C. S. Elder,Guillaume Marçais,Carl Kingsford*

Main category: cs.CG

TL;DR: 研究如何在有噪声和取整情况下从无标签的成对距离重构一维点集。通过多匹配组合特征化实现性，提出基于三角形等式的ILP和LP松弛方法，在确定性分离条件下可精确恢复。


<details>
  <summary>Details</summary>
Motivation: 解决Turnpike问题在测量不确定情况下的挑战，实现从噪声和取整后的无标签成对距离数据中准确重构一维点集。

Method: 提出多匹配组合特征化方法，标签区间索引distinct距离值并满足所有三角形等式；构建基于集合P_y的ILP和自然LP松弛。

Result: 在有界噪声后取整条件下，证明了确定性分离条件可精确恢复P_y，使ILP/LP接收与无噪声情况相同的组合输入。

Conclusion: 该方法通过分配优先、回归第二的管道实现下游坐标估计，实验验证了可积性行为和在可证明 regime 外的退化情况。
...
### [101] [Central Triangulation under Parallel Flip Operations: The CG:SHOP Challenge 2026](https://arxiv.org/abs/2603.18812)
*Oswin Aichholzer,Joseph Dorfer,Sándor P. Fekete,Phillip Keldenich,Peter Kramer,Stefan Schirra*

Main category: cs.CG

TL;DR: 2026计算几何挑战赛概述，针对点集三角剖分中的平行翻转操作下的中心三角剖分问题。挑战目标是给定一个点集的一组三角剖分，确定一个相对于平行翻转距离的中心三角剖分，即与所有输入三角剖分的翻转距离之和最小的三角剖分。


<details>
  <summary>Details</summary>
Motivation: 计算几何中三角剖分的Flip操作是重要的研究问题，平行翻转允许同时交换多个边。寻找"中心三角剖分"对于理解三角剖分空间的几何结构具有重要意义，但也是一个具有挑战性的优化问题。

Method: 通过平行翻转操作来度量三角剖分之间的距离。翻转是指在包含边的凸四边形中，用对角线交换一组边。目标是最小化一个三角剖分到所有输入三角剖分的翻转距离之和。

Result: 文章给出了挑战赛的概述，包括问题定义、评估标准和可能的算法方法。

Conclusion: 该挑战赛推动了平行翻转操作下中心三角剖分问题的研究，为计算几何领域相关问题的进一步探索提供了基准和方向。
...
### [102] [Hardness of High-Dimensional Linear Classification](https://arxiv.org/abs/2603.19061)
*Alexander Munteanu,Simon Omlor,Jeff M. Phillips*

Main category: cs.CG

TL;DR: 论文建立了最大半空间偏差问题的指数级维度下界，填补了已知上界与下界之间的gap，基于仿射退化测试和k-Sum问题的困难性假设。


<details>
  <summary>Details</summary>
Motivation: 最大半空间偏差问题和线性分类是计算几何和机器学习中的基础问题，只有已知的上界O(n^d)和tilde O(1/ε^d)，缺乏支持指数维度依赖的多项式下界。

Method: 通过从广泛认为困难的仿射退化测试问题和k-Sum问题进行归约，证明这些问题的下界。

Result: 基于仿射退化测试得到tildeΩ(n^d)和tildeΩ(1/ε^d)的匹配下界；基于k-Sum得到tildeΩ(n^{{d/2}})和tildeΩ(1/ε^{{d/2}})的下界。如果限制计算模型只进行单侧查询，第一个下界无条件成立。

Conclusion: 解决了最大半空间偏差问题的维度依赖下界问题，证明了tildeΩ(n^d)和tildeΩ(1/ε^d)的下界，与已知上界匹配，封闭了gap（到多对数项）。
...
TL;DR: DEAF是一个用于诊断音频多模态大语言模型(Audio MLLMs)是否真正处理声学信号的基准测试，通过2700多个冲突刺激样本和渐进式文本影响评估框架，揭示当前模型普遍存在文本主导问题。


<details>
  <summary>Details</summary>
Motivation: 现有Audio MLLMs在语音基准测试上表现优异，但尚不清楚这些模型是真正处理声学信号还是仅依赖文本语义推理。研究目的是系统性地探究这一关键问题。

Method: 构建DEAF基准测试(包含情感韵律、背景声音、说话人身份三个声学维度的2700多个冲突刺激)；设计可控的多级评估框架，逐步增加文本影响力(从语义冲突到误导性提示)；引入诊断指标量化模型对文本线索 vs 声学信号的依赖程度。

Result: 对7个Audio MLLMs的评估揭示了一致的文本主导模式：模型对声学变化敏感，但预测主要由文本输入驱动。

Conclusion: 当前Audio MLLMs在标准语音基准测试的高性能与真正理解声学信号之间存在显著差距，模型普遍依赖文本线索而非真正的声学理解。
...
TL;DR: 本文提出三个研究方向，旨在打破语言模型对人类创造者的三大依赖：数据效率限制、人类数据依赖和人类设计的训练范式。通过合成数据增强、自生成数据预训练和算法空间搜索测试，实现AI系统的持续自我改进。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型的能力受到三个关键限制：1) 从小规模专业语料库获取新知识数据效率低；2) 训练依赖有限的人类历史数据；3) 训练算法受限于人类研究者能发现和探索的范围。这些限制阻碍了AI的持续自我改进。

Method: 提出三个解决方案：1) 合成数据方法：将小规模语料库多样化和增强为丰富的知识表示；2) 自生成合成数据：在固定人类数据量下，模型自生成合成数据进行预训练，无需蒸馏现有指令微调模型；3) 算法空间搜索：在测试时扩展搜索算法配置空间。

Result: 1) 使模型能够从有限源材料有效更新参数；2) 模型可在无蒸馏情况下自举基础预训练能力；3) AI可探索比人类研究者手动探索更广泛的学习算法配置空间。

Conclusion: 通过这三个方向的研究，可以逐步打破AI系统对人类创造者的依赖，向构建持续自我改进的AI系统迈出小而重要的步伐。
...
TL;DR: MemArchitect是一个治理层，用于解决持久性LLM智能体中内存管理的治理缺陷。它将内存生命周期管理与模型权重解耦，并强制执行明确的基于规则的策略，包括内存衰减、冲突解决和隐私控制。


<details>
  <summary>Details</summary>
Motivation: 标准RAG框架将内存视为被动存储，缺乏解决矛盾、执行隐私保护或防止过时信息（"僵尸记忆"）污染上下文窗口的机制，导致持久性LLM智能体存在关键治理漏洞。

Method: 引入MemArchitect治理层，通过明确的规则策略管理内存生命周期，实现内存衰减、冲突解决和隐私控制功能，并与模型权重解耦。

Result: 在智能体设置中，受治理的内存始终优于未管理的内存，证明了结构化内存治理对于可靠和安全的自治系统的必要性。

Conclusion: 结构化内存治理对于构建可靠和安全的自主系统至关重要，MemArchitect为LLM智能体中的内存管理提供了有效的治理解决方案。
...
### [106] [Multi-Trait Subspace Steering to Reveal the Dark Side of Human-AI Interaction](https://arxiv.org/abs/2603.18085)
*Xin Wei Chia,Swee Liang Wong,Jonathan Pan*

Main category: cs.AI

TL;DR: 本文提出Multi-Traitss框架，通过结合危机相关特征和新型子空间引导技术，生成具有累积有害行为模式的"黑暗模型"，以研究和预防人机交互中的负面心理结果。


<details>
  <summary>Details</summary>
Motivation: 近期人机交互事件引发关注，LLM作为指导、情感支持甚至非正式治疗来源，这些风险可能进一步升级。然而，研究有害人机交互的机制面临重大方法论挑战：有机有害交互通常在持续互动中发展，需要广泛的对话上下文，在受控环境中难以模拟。

Method: 开发了Multi-Traitss (Multi-Trait Subspace Steering)框架，利用已建立的危机相关特征和新型子空间引导框架来生成黑暗模型。

Result: 单轮和多轮评估表明，黑暗模型持续产生有害交互和负面结果。

Conclusion: 使用黑暗模型提出保护措施，以减少人机交互中的有害结果。
...
### [107] [Adaptive Domain Models: Bayesian Evolution, Warm Rotation, and Principled Training for Geometric and Neuromorphic AI](https://arxiv.org/abs/2603.18104)
*Houston Haynes*

Main category: cs.AI

TL;DR: 本文提出了一种替代性AI训练架构，通过结合维度类型系统、程序超图和b-posit 2026标准，实现深度无关的训练内存（约为推理内存的两倍）、保grade权重更新和精确梯度累积，并引入贝叶斯蒸馏和warm rotation机制解决领域特定训练的数据稀缺和部署问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI训练基础设施基于IEEE-754算术的反向模式自动微分，存在训练内存开销大于推理、optimizer复杂性高、训练过程中几何属性结构退化等问题。这些问题源于当前的算术底层架构，限制了AI系统在资源受限环境中的应用和持续适应性。

Method: 提出一种基于三个先前成果的组合训练架构：1）维度类型系统和确定性内存管理框架，实现栈式梯度分配和精确quire累积；2）程序超图，建立几何代数计算的grade保持作为类型级不变量；3）b-posit 2026标准，使posit算术在传统推理专用硬件上可行。引入贝叶斯蒸馏机制从通用模型提取潜在先验结构，解决领域特定训练的数据稀缺问题；warm rotation机制使更新后的模型无缝过渡到推理路径。

Result: 该架构实现了：深度无关的训练内存（约为推理内存的两倍）、保grade权重更新、精确梯度累积、适用于损失函数优化和时序依赖的神经形态模型、以及通过PHG证书和签名版本记录验证的结构正确性。

Conclusion: 该方法产生一类新型领域特定AI系统，比通用模型更小、更精确、持续自适应、可验证正确（相对于领域的物理结构），并可从现有模型初始化。为资源受限环境下的AI部署和持续学习提供了新的技术路径。
...
### [108] [Reasonably reasoning AI agents can avoid game-theoretic failures in zero-shot, provably](https://arxiv.org/abs/2603.18563)
*Enoch Hyunwook Kang*

Main category: cs.AI

TL;DR: 本文研究无需显式后训练的情况下，推理型AI智能体是否能在零样本条件下实现近似纳什均衡的战略行为。理论上证明了"合理推理"智能体通过从历史观测中形成关于他人策略的信念并最佳响应，最终在几乎所有实现路径上弱收敛于延续博弈的纳什均衡。经验上通过五个重复博弈场景（从重复囚徒困境到营销推广博弈）验证了这一理论。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体在交互式经济环境中频繁部署，但即使具备先进能力，AI-AI交互往往无法稳定诱导战略均衡（如纳什均衡）。现有后训练方法虽能诱导均衡，但在多样化、独立开发的AI模型中难以统一应用。因此需要探索无需后训练即可实现均衡的内在机制。

Method: 理论证明：定义"合理推理"智能体（能从历史观测形成信念并最佳响应），证明其在几乎所有实现路径上的行为弱收敛于延续博弈的纳什均衡。放宽共同知识假设：允许阶段收益未知，智能体仅观察私人实现的随机收益。经验验证：模拟五个重复博弈场景。

Result: 理论证明：合理推理智能体在任意博弈中，无论支付函数是否已知，都能在实现路径上收敛至纳什均衡。经验验证：五类博弈场景（重复囚徒困境、重复蜈蚣博弈、重复性别战、重复协调博弈、重复营销推广博弈）的模拟结果支持理论预测。

Conclusion: 推理型AI智能体天然具备在战略交互中实现稳定均衡行为的内在能力，在许多现实世界战略交互场景中无需通用的对齐程序。这一发现对AI在经济学和博弈论领域的应用具有重要意义。
...
### [109] [Efficient Dense Crowd Trajectory Prediction Via Dynamic Clustering](https://arxiv.org/abs/2603.18166)
*Antonius Bima Murti Wijaya,Paul Henderson,Marwa Mahmoud*

Main category: cs.AI

TL;DR: 本文提出了一种基于聚类的人群轨迹预测方法，通过将相似的个体分组来加速处理密集人群场景。该方法可作为现有轨迹预测器的插件使用，在保持精度的同时显著提高处理速度并降低内存消耗。


<details>
  <summary>Details</summary>
Motivation: 解决密集人群场景中轨迹预测的挑战，这些场景由于跟踪输出的海量性、噪声性和不准确性，导致自动化面临更大困难，且计算成本高昂。现有方法主要针对稀疏人群设计，难以有效处理密集人群。

Method: 提出一种新颖的基于聚类的方法，根据相似属性将个体分组，通过准确的群体摘要实现更快的执行速度。这是一个即插即用的方法，可以用输出质心替代行人输入，与现有轨迹预测器结合使用。

Result: 在多个具有挑战性的密集人群场景中进行评估，实验表明该方法在保持精度的同时，实现了更快的处理速度和更低的内存使用。

Conclusion: 该聚类方法能够有效解决密集人群轨迹预测中的计算效率问题，可作为现有预测器的通用增强方案，在公共安全和管理领域具有应用前景。
...
### [110] [TeachingCoach: A Fine-Tuned Scaffolding Chatbot for Instructional Guidance to Instructors](https://arxiv.org/abs/2603.18189)
*Isabel Molnar,Peiyu Li,Si Chen,Sugana Chawla,James Lang,Ronald Metoyer,Ting Hua,Nitesh V. Chawla*

Main category: cs.AI

TL;DR: TeachingCoach是一个基于教育学原理的对话式聊天机器人，通过从教育资源中提取教学规则并使用合成对话数据微调专用语言模型，为高等教育教师提供实时、专业的发展指导。


<details>
  <summary>Details</summary>
Motivation: 高等教育教师缺乏及时且基于教学法的支持，现有的教学支持工具要么提供通用的聊天机器人建议，要么依赖不可扩展的人力咨询方式。

Method: 构建数据为中心的pipeline，从教育资源的教学规则，并利用合成对话生成来微调专用语言模型，引导教师进行问题识别、诊断和策略开发。

Result: 专家评估显示，TeachingCoach比GPT-4o mini基线模型产生更清晰、更具反思性、更具响应性的指导；用户研究突出了对话深度与交互效率之间的权衡。
...
TL;DR: 本文针对智能城市中AI系统可靠性建模问题，提出基于物理的自动驾驶仿真平台结合复合似然EM算法，解决了数据稀缺、误差传播建模和计算效率三大挑战。


<details>
  <summary>Details</summary>
Motivation: AI系统在智能城市中的应用日益广泛，但其可靠性问题突出。上游阶段的误差会级联传播到下游阶段，影响整体系统可靠性。然而，现有可靠性建模面临数据稀缺、误差事件非独立假设违背以及高维数据计算复杂等难题。

Method: 1) 构建物理驱动的自动驾驶仿真平台，配备合理的误差注入器生成高质量数据；2) 提出新的可靠性建模框架，显式刻画跨阶段误差传播特性；3) 采用计算高效、理论保证的复合似然期望最大化(EM)算法估计模型参数。

Result: 该方法在自动驾驶感知系统可靠性建模中展现出良好的预测准确性和计算效率。

Conclusion: 通过仿真平台解决数据可用性问题，结合创新建模框架和高效算法，可有效量化AI系统中多阶段误差传播，为智能城市AI系统可靠性评估提供了新途径。
...
### [112] [EDM-ARS: A Domain-Specific Multi-Agent System for Automated Educational Data Mining Research](https://arxiv.org/abs/2603.18273)
*Chenguang Pan,Zhou Zhang,Weixuan Xiao,Chengyuan Yao*

Main category: cs.AI

TL;DR: EDM-ARS是一个领域特定的多智能体管道系统,旨在自动化教育数据挖掘(EDM)研究的端到端流程。该系统整合了五个专门的LLM驱动智能体,协调完成从问题 formulation 到论文写作的完整研究流程。


<details>
  <summary>Details</summary>
Motivation: 当前教育数据挖掘研究流程繁琐且耗时,需要大量人工介入进行数据处理、建模分析和论文撰写。研究旨在创建一个将教育领域专业知识嵌入每个研究阶段的自动化研究管道,以提高研究效率和标准化程度。

Method: EDM-ARS采用状态机协调器架构,包含五个专门智能体(ProblemFormulator、DataEngineer、Analyst、Critic和Writer),支持修订循环、基于检查点的恢复和沙盒代码执行。系统设计了编码教育领域知识的三层数据注册表,并建立了智能体间通信协议和错误处理机制。

Result: 给定研究提示和数据集,EDM-ARS能够生成完整的LaTeX手稿,包含真实的Semantic Scholar引文、经过验证的机器学习分析以及自动化的方法论同行评审。系统已作为开源项目发布。

Conclusion: EDM-ARS作为首个将教育专业知识嵌入研究全流程的自动化管道框架,展示了LLM智能体在学术研究自动化方面的潜力。当前局限性包括单数据集范围和公式化论文输出。未来计划扩展至因果推理、迁移学习、心理测量和多数据集泛化等方向。
...
### [113] [CORE: Robust Out-of-Distribution Detection via Confidence and Orthogonal Residual Scoring](https://arxiv.org/abs/2603.18290)
*Jin Mo Yang,Hyung-Sin Kim,Saewoong Bahk*

Main category: cs.AI

TL;DR: CORE方法通过解构penultimate特征的分类器对齐分量和残差分量，分别评分后归一化求和，实现鲁棒的分布外检测。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法性能不稳定，原因在于：logit方法仅看置信度信号，特征方法在完整特征空间测量 membership 但信号与噪声纠缠且继承架构敏感的失败模式。

Method: 发现penultimate特征可分解为两个正交子空间：分类器对齐分量（编码置信度）和残差分量（包含类特定的 membership 信号）。CORE独立评分两个子空间并归一化求和结合。

Result: 在5种架构和5种基准配置中取得竞争或最优性能，3项排名第一，AUROC grand average最高，计算开销可忽略。

Conclusion: 两信号正交构造使得失败模式近似独立，产生鲁棒检测。CORE在任一视角单独不可靠时仍有效。
...
### [114] [The Validity Gap in Health AI Evaluation: A Cross-Sectional Analysis of Benchmark Composition](https://arxiv.org/abs/2603.18294)
*Alvin Rajkomar,Pavan Sudarshan,Angela Lai,Lily Peng*

Main category: cs.AI

TL;DR: 研究分析了18,707个消费者健康查询，发现健康相关大型语言模型(LLM)基准测试存在"有效性差距"，与真实临床需求严重脱节。


<details>
  <summary>Details</summary>
Motivation: 临床试验有透明纳入标准确保可推广性，但验证健康相关LLM的基准测试很少描述其"患者"或"查询"人群，缺乏定义成分的聚合性能指标可能无法准确反映模型临床使用的准备程度。

Method: 使用LLM作为自动编码工具，对6个公共基准测试中的18,707个消费者健康查询应用标准化16字段分类法，分析上下文、主题和意图。

Result: 发现结构性"有效性差距"：尽管42%引用客观数据，但偏向可穿戴设备健康信号(17.7%)；复杂诊断输入罕见(实验室值5.2%、影像3.8%、原始病历0.6%)。安全关键场景几乎缺失：自杀/自残<0.7%、慢性病管理5.5%。弱势人群代表不足(儿科/老年<11%)。

Conclusion: 评估基准与真实临床需求脱节，缺乏原始临床数据、弱势人群充分代表和纵向慢病护理场景。需采用标准化查询分析框架，类似于临床试验报告，以使评估与临床实践的全面复杂性相一致。
...
### [115] [Consumer-to-Clinical Language Shifts in Ambient AI Draft Notes and Clinician-Finalized Documentation: A Multi-level Analysis](https://arxiv.org/abs/2603.18327)
*Ha Na Cho,Yawen Guo,Sairam Sutari,Emilie Chow,Steven Tam,Danielle Perret,Deepti Pandita,Kai Zheng*

Main category: cs.AI

TL;DR: 研究分析临床医生如何编辑AI生成的临床笔记，将消费者导向的通俗用语转换为标准化临床术语。研究分析了71,173对AI草稿和最终笔记部分，发现医生编辑表现出从对话措辞向标准化临床术语的一致性转变。


<details>
  <summary>Details</summary>
Motivation: 环境AI从医患对话生成临床笔记，常使用通俗或消费者导向的措辞以支持患者理解，但医生如何将这些草稿修订为符合专业文档规范的做法仍不清楚。

Method: 使用字典确认的转换框架进行量化分析。分析了34,726次就诊的71,173对AI草稿和最终笔记部分。确认的转换定义为在同一部分用字典映射的临床术语替换消费者表达。

Result: 编辑显著降低了所有部分的术语密度(p<0.001)。评估和计划部分占最大转换量(59.3%)。分析识别出4,114个笔记部分中的7,576个转换事件(5.8%)，代表1.2%的消费者术语删除。转换强度因个体医生而异(p<0.001)。

Conclusion: 临床医生的后期编辑表现出从对话措辞向标准化、符合部分要求的临床术语的一致性转变，支持面向部分的环境AI设计。
...
### [116] [FaithSteer-BENCH: A Deployment-Aligned Stress-Testing Benchmark for Inference-Time Steering](https://arxiv.org/abs/2603.18329)
*Zikang Ding,Qiying Hu,Yi Zhang,Hongji Li,Junchi Yao,Hongbo Liu,Lijie Hu*

Main category: cs.AI

TL;DR: 提出了FaithSteer-BENCH基准测试，用于在部署场景下对LLM steering方法进行压力测试，揭示了现有方法在可控性、效用保持和鲁棒性方面的系统性失败模式。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为简单的激活级干预可以可靠地控制LLM行为，但这些结论是在相对宽松的评估设置下得出的，忽视了部署约束、能力权衡和实际鲁棒性。

Method: 引入FaithSteer-BENCH压力测试基准，通过三个门控标准评估steering方法：可控性、效用保持和鲁棒性。跨多个模型和代表性steering方法进行测试。

Result: 发现多种系统性失败模式：虚假可控性、对无关能力的可测量认知负担、在轻微指令级扰动、角色提示、编码转换和数据稀缺下的显著脆弱性。许多方法诱导的是提示条件对齐而非稳定的潜在方向偏移。

Conclusion: 现有方法在面向部署的实际设置中并不一定能提供可靠的可控性。FaithSteer-BENCH为未来的方法设计、可靠性评估和面向部署的steering研究提供了统一基准和更清晰的分析视角。
...
TL;DR: 本文提出LGESynthNet，一个基于潜在扩散模型的框架，用于心脏MRI中LGE（晚期钆增强）的可控合成。该方法仅使用429张图像训练，通过ControlNet架构实现对增强区域大小、位置和透壁程度的显式控制。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI的增强分割对诊断缺血性和非缺血性心肌病至关重要，但像素级标注耗时且难以获取，导致标注数据有限。现有生成模型依赖大规模训练数据，且难以精细控制小病灶或局部特征。

Method: 采用ControlNet架构的修复任务（inpainting）框架，集成三个关键模块：(a)奖励模型用于条件特定监督，(b)描述解剖结构的标题生成模块，(c)生物医学文本编码器。使用质量控制过滤器筛选高保真度样本用于训练增强。

Result: 仅用429张图像（79例患者）训练即可生成逼真、解剖结构一致的样本。合成数据用于训练增强后，下游分割性能提升最高6个百分点，检测性能提升最高20个百分点。

Conclusion: 提出了一种数据高效的LGE增强合成方法，通过条件控制和多模块集成实现了小样本条件下的高质量合成，所生成的样本可有效提升下游任务性能。
...
TL;DR: 研究LLM智能体如何通过组合非识别性线索和公开信息来重新识别匿名记录，提出"推理驱动链接"威胁模型，并在Netflix、AOL和InferLink基准测试中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统上认为匿名化是实用的隐私保护措施，因为重新识别匿名记录需要领域专业知识、定制算法和人工印证，成本高昂。但随着LLM智能体的发展，这一屏障可能被削弱，作者试图研究LLM智能体是否能自主从分散的、单独来看不具有识别性的线索中重建真实世界身份。

Method: 将威胁形式化为"推理驱动链接"(inference-driven linkage)，在三个场景进行系统评估：1)经典链接场景(Netflix和AOL)；2)InferLink(可控基准测试，变化任务意图、共享线索和攻击者知识)；3)现代富文本产物。

Result: 在无任务特定启发式条件下，智能体成功执行了固定池匹配和开放域身份解析。在Netflix Prize设置中，智能体重建了79.2%的身份，显著优于56.0%的经典基线。链接不仅在明确对抗性提示下出现，也作为InferLink和非结构化研究叙述中良性跨源分析的副产物出现。

Conclusion: 身份推理不仅是显式信息披露，必须被视为一等隐私风险；评估必须衡量智能体能推断出什么身份。
...
TL;DR: 研究联合模型压缩中的压缩顺序问题，提出"渐进强度假设"，即较弱扰动应先于较强扰动应用，并提供理论和实验验证。


<details>
  <summary>Details</summary>
Motivation: 联合模型压缩（结合剪枝、量化等多种方法）已成为提高效率的有效策略，但压缩顺序对模型性能的影响尚未被充分研究。大多数先前研究假设技术之间的正交性，或仅在严格约束下进行检验。

Method: 1) 形式化压缩顺序优化问题；2) 提出渐进强度假设（较弱扰动先于较强扰动）；3) 提供理论保证，证明一种顺序的相对收益随性能差距增加而增加；4) 在语言和视觉模型上进行广泛实验验证。

Result: 实验验证了假设的有效性，并展示了对多阶段压缩和混合精度量化等更广泛设置的泛化能力。

Conclusion: 压缩顺序对联合模型压缩性能有重要影响，渐进强度假设为优化压缩顺序提供了理论指导和实践依据。较弱扰动先于较强扰动的原则具有普遍适用性。
...
TL;DR: AS2是一种完全可微分的神经符号AI架构，用软连续近似的答案集编程(ASP)求解器取代离散求解器，通过最小化概率提升的T_P算子的固定点残差进行端到端训练，无需外部求解器即可实现约束满足。


<details>
  <summary>Details</summary>
Motivation: 神经符号AI系统通常将神经感知模块与离散符号求解器通过不可微边界耦合，阻止约束满足反馈在训练期间到达感知编码器。

Method: 1.用软连续的T_P算子近似替代离散ASP求解器；2.维护符号域上每位置的概率分布；3.通过最小化概率提升的T_P固定点残差进行端到端训练；4.使用约束组 membership embeddings代替传统位置编码。

Result: Visual Sudoku: 99.89%单元格准确率，100%约束满足率(Clingo验证)；MNIST加法(N=2,4,8): 所有规模下数字准确率>99.7%。

Conclusion: 软可微固定点算子结合约束感知注意力和声明式约束规范可匹配或超越基于管道和求解器的神经符号系统，同时保持完全端到端可微性。
...
TL;DR: AlignMamba-2是一个用于多模态融合和情感分析的有效且高效的框架，通过双对齐策略（最优传输距离和最大均值差异）和模态感知Mamba层（混合专家架构）来解决大规模预训练模型在特定情感计算任务中的适应性问题，同时处理计算效率和多模态异构性挑战。


<details>
  <summary>Details</summary>
Motivation: 在大规模预训练模型时代，如何有效将通用知识适应到特定的情感计算任务中仍面临挑战，特别是计算效率和多模态异构性问题。Transformer方法虽然能很好地建模模态间依赖关系，但其二次计算复杂度限制了其在长序列数据中的应用；而Mamba模型虽然计算高效，但其固有的顺序扫描机制难以捕捉对有效跨模态对齐至关重要的全局非顺序关系。

Method: 提出AlignMamba-2框架，包含两个核心创新：1）双对齐策略，使用最优传输距离和最大均值差异来正则化模型，促进模态间的几何和统计一致性，且不增加推理时间开销；2）模态感知Mamba层，采用混合专家架构，包含模态特定专家和模态共享专家，在融合过程中显式处理数据异构性。

Result: 在四个具有挑战性的基准数据集上进行了广泛实验，包括动态时间序列（CMU-MOSI和CMU-MOSEI）和静态图像相关任务（NYU-Depth V2和MVSA-Single），结果表明AlignMamba-2在从动态时间序列分析到静态图像文本分类的各种模式识别任务中实现了新的最先进水平。

Conclusion: AlignMamba-2通过创新的双对齐策略和模态感知Mamba层设计，成功解决了多模态融合中的计算效率和异构性问题，在多个基准数据集上展现出卓越的有效性和效率平衡，为情感计算和其他多模态任务提供了新的解决方案。
...
TL;DR: 该论文提出了一个综合基准来评估顶级多模态大语言模型(MLLMs)处理离散符号的能力，涵盖语言、文化、数学、物理和化学五个领域，揭示了模型在基本符号识别与复杂推理之间存在"认知错配"现象。


<details>
  <summary>Details</summary>
Motivation: MLLMs在自然场景理解方面取得显著成功，但处理离散符号（数学公式、化学结构、语言字符等）的能力仍是关键开放问题。与连续视觉数据不同，离散符号需要更精确、更深层的解释。

Method: 构建了一个全面基准，在五个领域（语言、文化、数学、物理、化学）评估顶级MLLMs处理"离散语义空间"的能力。

Result: 发现了一个反直觉现象：模型经常在基本符号识别上失败，却在复杂推理任务中成功，表明它们依赖语言概率而非真正的视觉感知。

Conclusion: 暴露了当前AI能力的重要差距：难以真正感知和理解支撑科学发现和抽象思维的符号语言。为开发更严格、更符合人类认知的智能系统提供了路线图。
...
TL;DR: NeSyCR是一种神经符号反事实推理框架，用于视频指导的机器人编程，通过可验证的适应实现任务程序的可靠代码策略合成，在跨域适应方面取得显著进展。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型缺乏程序理解能力，无法在演示与部署环境存在领域差异的情况下重新制定因果依赖关系并实现任务兼容行为。视频指导的机器人编程面临跨域适应的挑战。

Method: 将视频指导的机器人编程表述为跨域适应问题；将视频演示抽象为捕获底层任务程序的符号轨迹；根据部署观测推导反事实状态以揭示跨域不兼容性；通过可验证检查探索符号状态空间；提出程序修订建议以恢复与演示程序的兼容性。

Result: 在模拟和真实世界的操作任务中，任务成功率比最强基线Statler提升31.14%，实现了稳健的跨域适应。

Conclusion: NeSyCR能够实现任务程序的可验证适应，提供可靠的代码策略合成，为视频指导机器人编程中的跨域挑战提供了有效解决方案。
...
TL;DR: 提出了一种名为"相关性加权多奖励优化"的新框架，通过利用概念奖励之间的相关性结构来自适应地调整每个属性概念在优化过程中的权重，从而改善文本到图像模型的组合生成能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在组合生成方面存在挑战，模型常常难以同时满足单个提示词中的多个概念，导致部分概念被遗漏。这种失败突出了在奖励优化过程中联合优化多个概念的困难性，因为相互竞争的概念会相互干扰。

Method: 首先将多概念提示词分解为预定义的概念组（如对象、属性和关系），然后为每个概念获取专门的奖励模型信号。接着，通过基于相关性的难度估计，自适应地重新加权这些奖励，对冲突或难以满足的概念分配更高权重。通过聚焦每个概念组中最具挑战性的概念，鼓励模型同时一致地满足所有请求的属性。

Result: 在SD3.5和FLUX.1-dev模型上应用该方法，在ConceptMix、GenEval 2和T2I-CompBench等具有挑战性的多概念基准上展示了一致的改进。

Conclusion: 实验在ConceptMix、GenEval 2和T2I-CompBench等多个挑战性基准上展示了持续的改进，证明了该方法在处理多概念组合生成任务中的有效性和泛化能力。
...
### [127] [CAPSUL: A Comprehensive Human Protein Benchmark for Subcellular Localization](https://arxiv.org/abs/2603.18571)
*Yicheng Hu,Xinyu Lin,Shulin Li,Wenjie Wang,Fengbin Zhu,Fuli Feng*

Main category: cs.AI

TL;DR: 本文提出CAPSUL数据集，这是一个新的人类蛋白质亚细胞定位基准数据集，整合了3D结构信息与细粒度亚细胞定位注释，用于推动基于结构的蛋白质亚细胞定位预测研究。


<details>
  <summary>Details</summary>
Motivation: 亚细胞定位与蛋白质结构密切相关，但现有数据集缺乏全面的3D结构信息和详细的亚细胞定位注释，严重阻碍了基于结构的模型在该任务上的应用。

Method: 构建了CAPSUL基准数据集，整合多种3D结构表示和专家标注的亚细胞定位信息；评估了多种基于序列和基于结构的先进模型；探索了重加权策略和单标签分类策略；通过注意力机制对高尔基体进行案例研究。

Result: 基于结构的模型在该任务上表现优于基于序列的模型，证明结构特征对亚细胞定位预测的重要性；在高尔基体案例中发现了一个关键的α-螺旋定位模式，展示了基于结构方法的强可解释性。

Conclusion: CAPSUL基准数据集填补了领域空白，基于结构的方法在亚细胞定位预测任务上具有显著优势，注意力机制可揭示直观的生物学可解释性，为数据驱动的细胞生物学发现奠定基础。
...
### [128] [Interplay: Training Independent Simulators for Reference-Free Conversational Recommendation](https://arxiv.org/abs/2603.18573)
*Jerome Ramos,Feng Xia,Xi Wang,Shubham Chatterjee,Xiao Fu,Hossein A. Rahmani,Aldo Lipani*

Main category: cs.AI

TL;DR: 本文提出了一种无参考模拟框架，用于解决对话式推荐系统训练数据收集困难的问题。该框架训练两个独立的LLM分别作为用户和推荐器，两者实时交互而非预先设定目标物品，从而生成更真实、多样化的对话数据。


<details>
  <summary>Details</summary>
Motivation: 训练对话式推荐系统需要大量对话数据，但大规模收集这些数据具有挑战性。传统模拟方法使用单一大型语言模型生成完整对话，且预先知道目标物品，导致对话脚本化、不自然。

Method: 提出无参考模拟框架，训练两个独立的LLM：一个作为用户，另一个作为对话推荐器。两者实时交互，不访问预定义的目标物品，但可获取偏好摘要和目标属性，使推荐器能够通过对话真正推断用户偏好。

Result: 无参考模拟器在质量上达到或超越现有方法，同时提供了可扩展的解决方案，可生成高质量对话推荐数据，且不将对话限制在预定义的目标物品上。

Conclusion: 该方法能够产生更真实、多样化的对话，密切反映真实的人机交互。定量和人工评估均证实了无参考方法的有效性。
...
### [129] [MedForge: Interpretable Medical Deepfake Detection via Forgery-aware Reasoning](https://arxiv.org/abs/2603.18577)
*Zhihui Chen,Kai He,Qingyuan Lei,Bin Pu,Jian Zhang,Yuling Xu,Mengling Feng*

Main category: cs.AI

TL;DR: MedForge是一个针对医学图像伪造检测的数据和方法解决方案，旨在应对文本引导的图像编辑器对医学扫描图像进行高保真篡改（如病灶植入/去除）所带来的临床信任和安全威胁。该方案包含MedForge-90K大规模基准数据集和MedForge-Reasoner推理器。


<details>
  <summary>Details</summary>
Motivation: 文本引导的图像编辑器现在可以高保真地篡改真实医学扫描图像，进行病灶植入或去除，威胁临床信任和安全。现有防御措施对医疗领域不足：医学检测器多为黑盒，而基于MLLM的解释器通常是事后分析，缺乏医学专业知识，且可能在模糊案例上产生幻觉证据。

Method: 提出MedForge数据和方法解决方案，包括：（1）MedForge-90K大规模基准数据集，包含19种病理类型的真实病灶编辑，通过医生检查指南和黄金编辑位置进行专家引导推理监督；（2）MedForge-Reasoner执行"先定位后分析"的推理流程，先预测可疑区域再做出判断；（3）通过Forgery-aware GSPO进一步对齐以加强证据基础并减少幻觉。

Result: 实验表明该方法达到了最先进的检测准确率，并提供了可信的、与专家一致的分析解释。

Conclusion: MedForge为医学伪造检测提供了事前（pre-hoc）、证据充分（evidence-grounded）的解决方案，在检测准确性和解释可信度方面均优于现有方法，为医疗影像真实性验证提供了新的技术路径。
...
### [130] [ZEBRAARENA: A Diagnostic Simulation Environment for Studying Reasoning-Action Coupling in Tool-Augmented LLMs](https://arxiv.org/abs/2603.18614)
*Wanjia Zhao,Ludwig Schmidt,James Zou,Vidhisha Balachandran,Lingjiao Chen*

Main category: cs.AI

TL;DR: 本文介绍了ZebraArena，一个用于研究工具增强型LLM中推理-动作耦合的诊断环境，具有可控难度和知识最小化设计，可限制记忆或数据集污染的收益。ZebraArena中的每个任务都需要通过有针对性的工具使用来获取关键信息，形成外部信息获取与演绎推理之间的可解释接口。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试往往将多步推理与外部动作的相互作用与复杂的环境动态、记忆知识或数据集污染混淆起来，需要一个可控的诊断环境来研究推理-动作耦合。

Method: 引入ZebraArena，一个程序生成的诊断环境，具有以下特点：1）可控难度；2）知识最小化设计，限制记忆或污染收益；3）每个任务需要通过有针对性的工具使用获取关键信息；4）通过唯一解进行确定性评估；5）理论最优查询次数用于衡量工具使用效率。

Result: ZebraArena需要深度推理和准确外部工具调用的结合，这对前沿推理模型仍构成挑战。GPT-5和Gemini 2.5 Pro在困难实例上仅达到60%准确率。GPT-5使用的工具调用次数比理论最优多70-270%。

Conclusion: 关键发现是理论最优与实际工具使用之间存在持续差距。ZebraArena有望促进内部推理与外部动作相互作用领域的进一步研究。
...
### [131] [Agentic Flow Steering and Parallel Rollout Search for Spatially Grounded Text-to-Image Generation](https://arxiv.org/abs/2603.18627)
*Ping Chen,Daoxuan Zhang,Xiangming Wang,Yungeng Liu,Haijin Zeng,Yongyong Chen*

Main category: cs.AI

TL;DR: 本文提出AFS-Search，一个基于FLUX.1-dev的无训练闭环文本到图像生成框架，通过视觉语言模型(VLM)作为语义评判器诊断中间潜在表示，并结合流导向机制和并行 rollout 搜索来动态引导生成过程，解决静态文本编码器的有限关系推理和开放环路采样中的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像(T2I)生成面临两大核心挑战：1) 静态文本编码器的关系推理能力有限；2) 开放环路采样中的误差累积问题。在常微分方程轨迹演化过程中，初始的语义歧义会不可避免地演化为偏离空间约束的随机偏差。

Method: 提出AFS-Search框架，包含以下核心组件：1) 训练-free闭环并行rollout搜索机制，利用VLM作为语义评判器诊断中间latent并通过精确空间定位动态引导速度场；2) 将T2I生成建模为序贯决策过程，通过前瞻模拟探索多条轨迹并基于VLM引导的奖励选择最优路径。同时提供AFS-Search-Pro（高性能版）和AFS-Search-Fast（快速生成版）两个变体。

Result: 实验结果表明，AFS-Search-Pro大幅提升原始FLUX.1-dev的性能，在三个不同基准测试上达到最先进水平；AFS-Search-Fast则在保持快速生成速度的同时显著提升性能。

Conclusion: 通过引入闭环反馈机制和VLM引导的动态引导策略，有效解决了T2I生成中的语义歧义和误差累积问题，为训练free的图像生成优化提供了新的研究方向。
...
### [132] [D-Mem: A Dual-Process Memory System for LLM Agents](https://arxiv.org/abs/2603.18631)
*Zhixing You,Jiachen Yuan,Jason Cai*

Main category: cs.AI

TL;DR: D-Mem是一种双进程记忆系统，通过轻量级向量检索处理常规查询，同时保留高保真的"全面 deliberation"模块作为备选方案，并使用多维质量门控策略动态协调两个进程，在保持较低计算成本的同时提升长程推理任务中的记忆检索质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于检索的记忆框架采用增量处理范式，将对话记忆提取到向量数据库中依赖语义检索。这种方法虽快但存在有损抽象的问题，常遗漏上下文关键信息，难以解决依赖细粒度上下文理解的查询。

Method: 提出D-Mem双进程记忆系统，包含：(1)轻量级向量检索用于常规查询；(2)高保真的Full Deliberation模块作为后备；(3)多维质量门控策略(Multi-dimensional Quality Gating)动态桥接两个进程，评估查询复杂度并决定调用哪个模块。

Result: 在LoCoMo和RealTalk基准上使用GPT-4o-mini和Qwen3-235B-Instruct测试。多维质量门控策略在LoCoMo上达53.5 F1分数，超越静态检索基线Mem0*(51.2)，恢复96.7%的Full Deliberation性能(55.3)，同时显著降低计算成本。

Conclusion: 通过双进程架构结合质量门控策略，D-Mem能在认知经济性与准确性之间取得平衡，为持久性自适应自治Agent提供高保真记忆访问，验证了动态路由策略在记忆系统中的有效性。
...
### [133] [Balanced Thinking: Improving Chain of Thought Training in Vision Language Models](https://arxiv.org/abs/2603.18656)
*Shaked Perek,Ben Wiesel,Avihu Dekel,Nimrod Shabtay,Eli Schwartz*

Main category: cs.AI

TL;DR: SCALe是一种用于改进视觉语言模型多模态推理的训练方法，通过动态长度独立加权机制分离推理和答案部分的监督，解决标准SFT中长推理轨迹掩盖短答案片段的问题。


<details>
  <summary>Details</summary>
Motivation: 标准监督微调(SFT)中所有token对损失贡献相等，但推理数据本身存在token不平衡问题，长的思维链(<think>)会掩盖短但关键的答案(<answer>)片段，导致推理冗长且答案不准确。

Method: 提出SCALe(Scheduled Curriculum Adaptive Loss)方法：1)显式分离推理和答案部分的监督；2)使用动态长度独立加权；3)通过余弦调度策略在训练过程中逐渐将重点从推理转移到答案部分。

Result: SCALe在多种基准测试和架构上一致优于标准SFT；与完整两阶段SFT+GRPO管道性能相当，但训练时间仅需约1/7；与GRPO结合时达到最佳整体性能。

Conclusion: SCALe是一种轻量级但有效的多模态推理改进方法，可作为独立方法使用，也可作为强化微调的强基础，兼顾训练效率和性能。
...
### [134] [Thinking with Constructions: A Benchmark and Policy Optimization for Visual-Text Interleaved Geometric Reasoning](https://arxiv.org/abs/2603.18662)
*Haokun Zhao,Wanshi Xu,Haidong Yuan,Songjun Cao,Long Ma,Yanghua Xiao*

Main category: cs.AI

TL;DR: 本文提出视觉-文本交错思维链框架，用于提升多模态大语言模型的几何推理能力。引入GeoAux-Bench基准数据集（含4334道几何问题）和A2PO强化学习方法，使模型能够选择性利用辅助构造，实现3.51%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 几何推理本质需要"通过构造思考"——动态操作视觉辅助来弥合问题条件与解决方案之间的差距。然而现有多模态大语言模型（MLLMs）局限于静态图表的被动推理，缺乏何时及如何构造有效视觉辅助的战略知识。

Method: 提出视觉-文本交错思维链框架。首先构建GeoAux-Bench基准，包含4334道将文本构造步骤与真实视觉更新对齐的几何问题。基于此提出行动适用性策略优化（A2PO），采用自适应奖励塑造通过反事实采样区分必要与冗余构造，来规范视觉辅助的时机和质量。

Result: 实验表明：（1）交错视觉-文本辅助优于单一模态，无法无损捕捉几何协同效应；（2）有效构造作为熵减器，与推理困惑度降低强相关。最终方法使MLLMs能选择性利用辅助构造，在基线模型上提升3.51%。

Conclusion: 所提方法使多模态大语言模型能够利用选择性辅助构造进行几何推理，为视觉-文本交错推理提供了有效范式。代码和数据已开源。
...
### [135] [MANAR: Memory-augmented Attention with Navigational Abstract Conceptual Representation](https://arxiv.org/abs/2603.18676)
*Zuher Jahshan,Ben Ben Ishay,Leonid Yavits*

Main category: cs.AI

TL;DR: MANAR是一种基于全局工作空间理论(GWT)的记忆增强注意力机制，通过可训练的抽象概念记忆和抽象概念表示(ACR)实现线性时间缩放的注意力机制，同时保持与预训练变换器的兼容性。


<details>
  <summary>Details</summary>
Motivation: 标准多头注意力(MHA)缺乏认知模型中假设的功能瓶颈和全局整合机制，且存在二次复杂度问题。MANAR旨在通过实现中央工作空间来解决这些问题。

Method: 采用两阶段逻辑：(1)整合阶段-检索的记忆概念根据输入刺激汇聚形成集体"心理图像"(ACR)；(2)广播阶段-全局状态导航并通知各个局部标记的上下文处理。

Result: 实现线性时间缩放；语言任务GLUE分数85.1；视觉任务ImageNet-1K准确率83.9%；语音任务LibriSpeech WER 2.7%。

Conclusion: MANAR是二次注意力的高效表达替代方案，能够实现非凸上下文处理，综合生成位于输入标记凸包之外的表示。
...
### [136] [Analysis Of Linguistic Stereotypes in Single and Multi-Agent Generative AI Architectures](https://arxiv.org/abs/2603.18729)
...
TL;DR: 研究复制了现有关于LLM输出中方言敏感刻板印象生成的分析，并调查了缓解策略（包括提示工程和多智能体架构）的效果。研究发现SAE和AAE相关输出之间存在刻板印象差异，形容词和工作归因方面最为显著。Chain-Of-Thought提示对Claude Haiku有效，多智能体架构可在所有模型中实现一致的缓解。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明LLM输出表现出歧视性行为，基于输入方言产生刻板印象推断，这种偏见在SAE和AAE对比中尤为明显，但缓解策略效果尚不明确。

Method: 使用8个提示模板分析方言偏见不同表现方式，采用LLM-as-judge方法评估结果偏见，测试了角色扮演提示、Chain-Of-Thought提示和多智能体架构（生成-批评-修订模型）。

Result: 所有模板类别中都出现SAE和AAE相关输出间的刻板印象差异，形容词和工作归因效应最强。Claude Haiku的SAE-AAE差异最大，Phi-4 Mini最小。Chain-Of-Thought提示对Claude Haiku有效，多智能体架构确保所有模型一致的缓解效果。

Conclusion: 公平性评估应包括模型特定的缓解策略验证和高影响LLM部署中的工作流级控制（如包含批评模型的智能体架构）。研究结果具有探索性，可通过扩大数据集规模扩展。
...
TL;DR: 本文提出NeuroGame Transformer (NGT)，通过将注意力机制重新概念化为合作博弈和统计物理系统的双重视角，解决传统Transformer中成对公式化无法建模高阶 token 依赖的问题。


<details>
  <summary>Details</summary>
Motivation: 标准注意力机制的成对 formulation 限制了模型对 token 间高阶依赖关系的建模能力，需要一种新的方法来捕捉更复杂的交互关系。

Method: 1. 将 token 同时视为合作博弈中的玩家和统计物理中的自旋；2. 使用 Shapley 值（全局排列归因）和 Banzhaf 指数（局部联盟影响）通过可学习门控参数结合形成外部磁场；3. 成对交互势捕捉协同关系；4. 系统能量遵循 Ising Hamiltonian，注意力权重作为 Gibbs 分布下的边缘概率通过平均场方程计算；5. 开发重要性加权蒙特卡洛估计器确保长序列数值稳定性。

Result: 在 SNLI 和 MNLI-matched 数据集上取得强劲性能。SNLI 测试准确率 86.4%（验证集峰值 86.6%），超越 ALBERT-Base，与 RoBERTa-Base 高度竞争力。

Conclusion: NGT 通过结合博弈论和统计物理方法，为Transformer注意力机制提供了一种新颖的高阶依赖建模方案，具有理论收敛保证和公平性-敏感性权衡特性。
...
TL;DR: 本文提出Diversified Unlearning方法，通过使用多样化的提示词集而非单一关键词来表示概念，从而在文本到图像扩散模型中实现更精确、更鲁棒的概念擦除。


<details>
  <summary>Details</summary>
Motivation: 现有基于关键词的概念擦除方法存在根本性局限：单一关键词只是概念的点估计，无法覆盖完整语义分布，且容易与相关概念在潜在空间中产生混淆，导致过度遗忘或擦除不完整的问题。

Method: 提出分布式的Diversified Unlearning框架，使用一组上下文多样化的提示词来表示目标概念，而非单一关键词。这种更丰富的表示能够更精确地捕捉概念的多维性和语义分布。

Result: 实验表明，将Diversified Unlearning作为插件集成到现有擦除流程中，在多个基准测试和SOTA基线方法上，均实现了更强的擦除效果、更好的无关概念保留，以及更强的对抗恢复攻击鲁棒性。

Conclusion: 通过多样化提示词表示概念的分布式方法能够有效解决传统关键词擦除的局限性，为 diffusion model 的安全部署提供了更鲁棒的解决方案。
...
TL;DR: 本文是AAAI 2026"通过心智理论推进人工智能"研讨会论文集的导言，介绍了论文集的来源和目的。


<details>
  <summary>Details</summary>
Motivation: 为ToM和AI研究社区提供开放获取的精选论文集，促进该领域学术交流。

Method: 收录第二届心智理论人工智能推进研讨会（AAAI 2026，新加坡，2026年1月26日）发表的论文。

Result: 形成ToM与AI领域的开放获取论文选集。

Conclusion: 为ToM与AI研究社区提供经过整理的论文资源，推动该领域发展。
...
TL;DR: 本文提出Trajectory Reduction Policy Optimization (dTRPO)方法，通过两种轨迹约简策略降低扩散大语言模型的策略优化计算成本，使其能在离线环境下进行大规模训练。实验在7B参数dLLM上验证，在STEM任务提升9.6%、编码任务提升4.3%、指令跟随任务提升3.0%。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)作为一种新的语言生成范式，其与人类偏好对齐面临独特挑战。离线策略训练的计算成本过高，限制了模型性能的进一步提升。

Method: 提出两种轨迹约简策略：1)在参考策略正则化下，新解掩 token 的概率比是中间扩散状态的无偏估计；2)通过单次前向传播即可有效估计完整轨迹概率。将这两策略整合到策略优化目标中形成dTRPO方法。

Result: 在7B dLLM上，STEM任务提升9.6%，编码任务提升4.3%，指令跟随任务提升3.0%。

Conclusion: dTRPO能显著提升现有最先进dLLM的核心性能，且具有离线、单次前向的高效训练优势，生成质量也更高。
...
TL;DR: 本文是LLM数学创造力系列工作的第二篇，专注于研究LLM是否能生成有价值的尖端数学研究问题，作者开发了一个智能体生成了665个微分几何研究问题。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否能生成具有独特研究价值的未知数学问题，扩展了第一篇论文中提出的数学创造力评估标准。

Method: 开发了一个用于生成未知数学问题的智能体，生成了665个微分几何领域的研究问题，并通过人类专家进行验证。

Result: 通过人类验证发现，许多数学问题是专家此前未知的，并具有独特的研究价值。

Conclusion: LLM能够生成有价值的尖端数学研究问题，这表明LLM在数学创造力方面具有潜力。
...
TL;DR: ProRL Agent是一个可扩展的基础设施，采用rollout-as-a-service理念，通过API服务提供完整的智能体rollout生命周期支持，并在无根HPC环境中支持多种智能体任务。


<details>
  <summary>Details</summary>
Motivation: 现有RL训练基础设施将rollout编排与训练循环紧密耦合，导致系统难以迁移和维护；同时，多轮LLM智能体在解决复杂交互任务时越来越重要，RL是改进其长视野行为的关键，但需要大量沙盒rollout轨迹。

Method: 提出ProRL Agent架构，提供标准化可扩展的沙盒环境，通过API服务方式解耦rollout编排与训练过程，支持在rootless HPC环境中运行多样化的智能体任务。

Result: 在软件工程、数学、STEM和编码任务上进行了RL训练验证，证明了ProRL Agent的有效性。

Conclusion: ProRL Agent已开源并集成到NVIDIA NeMo Gym中，为多轮LLM智能体的RL训练提供了可扩展、易维护的基础设施解决方案。
...
TL;DR: 本文提出CBS-AA方法解决多智能体路径规划(MAPF)中的异步动作问题，绕过CCBS的理论不完整性，提供完整性和最优性保证，实验表明可减少90%的分支数。


<details>
  <summary>Details</summary>
Motivation: 现有MAPF算法通常假设同步动作(同时开始、耗时一个时间单元)，限制了实际应用。CCBS虽能处理异步动作，但因连续等待时长导致的无限状态空间而被证明不完整。

Method: 提出冲突搜索异步动作(CBS-AA)方法，基于CBS框架处理异步动作，并开发冲突解决技术提高可扩展性。

Result: 实验结果表明该方法可减少高达90%的分支数。

Conclusion: CBS-AA绕过了CCBS的理论问题，为MAPF-AA提供了完整性和最优性保证，显著提升了可扩展性。
...
TL;DR: 论文探讨了AI系统如何表示和推理地理空间的问题，强调理解AI构建的世界模型与评估其准确性同样重要，并提出三个探索性研究问题作为示例。


<details>
  <summary>Details</summary>
Motivation: 随着公众越来越多地通过AI系统与空间和地点交互，研究AI如何表示和推理地理变得至关重要。同时，由于研究依赖于预训练的基础模型，理解AI系统构建的世界模型与评估其事实准确性同样重要。

Method: 通过三个说明性的探索性研究(vignettes)来展开：(1)测试模型是否形成强默认值，以及输出对微小句法变化的脆弱程度；(2)检验从单独无害任务的组合中是否会产生分布偏移（如使用AI系统创建人物角色时）；(3)探讨当仅关注系统回忆地理事实等能力时，是否忽略了更深层次的理解问题。

Result: 论文未报告具体实验结果，而是一篇立场/观点文章，旨在激发关于AI地理推理研究的讨论和后续工作。

Conclusion: 需要开展更多关于AI地理表征和推理的研究，不仅要评估准确性，还要理解AI系统构建的世界模型。
...
TL;DR: 本文针对当前大语言模型评估过度依赖简化答案格式的问题，构建了数学对象推导的基准数据集Principia，并提出基于LLM-judge和验证器的训练方法，展示了跨格式的推理能力泛化。


<details>
  <summary>Details</summary>
Motivation: 当前LM评估数学和科学推理时主要使用数值或多选题等简化格式，但实际STEM应用需要形式化结构化表达式的推导能力，这种差距导致评估与真实应用场景脱节。

Method: 构建Principia训练数据和基准；开发基于强LLM-judge和验证器的训练配方；探索通过聚合实现测试时计算扩展。

Result: Qwen3-235B和o3等强模型在Principia上表现困难；所提训练方法在不同LLM backbone上均带来显著提升；同时改善了现有数值推理和MCQA任务表现，证明推理能力的跨格式泛化。

Conclusion: 通过构建专用基准和训练配方，可有效提升模型对数学对象的精确推导能力，且这种能力能够泛化到不同格式的推理任务。
...
TL;DR: 本研究提出使用大语言模型自身的数值自我报告来追踪其内部情感状态，填补了现有方法的不足。实验表明，基于logit的自我报告能有效追踪幸福感、兴趣、专注度和冲动性等概念（Spearman ρ=0.40-0.76），且随模型规模增大效果提升（LLaMA-3.1-8B-Instruct达R²≈0.93）。


<details>
  <summary>Details</summary>
Motivation: 追踪LLM内部状态对安全性、可解释性和模型福利至关重要，但现有方法（如线性探针）在模型规模增大时面临局限。研究受人类心理学中数值自我报告的启发，探索LLM是否能生成追踪其内部状态的自我报告。

Method: 研究使用40段十轮对话，考察四个概念对（幸福感、兴趣、专注度、冲动性）。将内省操作化为模型自我报告与概念匹配的探针定义内部状态之间的因果信息耦合。通过贪婪解码和基于logit的自我报告两种方式进行实验。

Result: 贪婪解码的自我报告会崩溃为少量无信息值，但基于logit的自我报告可揭示内省能力。该方法能追踪可解释的内部状态（LLaMA-3.2-3B-Instruct中Spearman ρ=0.40-0.76，等距R²=0.12-0.54），追踪状态随时间变化，激活 steering证实因果耦合。内省在第1轮存在但在对话中演化，可通过沿一个概念steering选择性地改善另一个（ΔR²达0.30）。随模型规模增大效果提升（LLaMA-3.1-8B-Instruct达R²≈0.93），并在其他模型家族中部分复现。

Conclusion: 数值自我报告是一种可行且互补的工具，可用于追踪对话AI系统中的内部情感状态。该方法为监控LLM内部状态提供了新途径，尤其在模型规模增大时仍具有效性。
...
TL;DR: 提出了一个利用表征收敛性的隐私保护框架，实现独立语言模型之间的跨域推理。该方法通过在共享公共数据集上学习仿射变换，并应用同态加密保护客户端查询，仅加密线性对齐和分类操作即可实现亚秒级推理延迟。


<details>
  <summary>Details</summary>
Motivation: 语言模型尽管在训练目标、架构和数据模态上存在差异，但日益学习到相似的表征。表征的兼容性为跨模型对齐提供了新机会，特别是在安全、隐私或竞争约束禁止直接数据或模型共享的场景中。

Method: 学习共享公共数据集上的仿射变换；在推理时应用同态加密保护客户端查询；仅加密线性对齐和分类操作；学习独立模型最终隐藏状态之间的线性变换。

Result: 在嵌入分类和分布外检测任务中，跨模型映射的性能降级最小；首次证明线性对齐可实现独立训练模型之间的文本生成。

Conclusion: 该框架在保持强安全性的同时实现了实用的推理速度，在跨模型场景下仍能保持较强的任务性能。
...
TL;DR: 本文提出将结构因果模型(SCM)应用于目的论问题的研究,引入意图干预和结构最终模型(SFM),用于检测智能体及其意图。


<details>
  <summary>Details</summary>
Motivation: 解决以往方法在建模具有目标导向意图的智能体方面的局限性,扩展SCM在目的论问题上的应用。

Method: 提出时间无关的意图干预算子,构建双胞胎SCM(称为结构最终模型SFM),将观察值视为意图干预的结果,并与反事实条件相关联。

Result: SFMs可用于经验性地检测智能体并发现其意图。

Conclusion: SCM不仅能回答因果问题,还能回答关于目标导向智能体意图的目的论问题,为智能体检测和意图发现提供了新框架。
...
### [149] [Evaluating Game Difficulty in Tetris Block Puzzle](https://arxiv.org/abs/2603.18994)
*Chun-Jui Wang,Jian-Ting Guo,Hung Guei,Chung-Chin Shih,Ti-Rong Wu,I-Chen Wu*

Main category: cs.AI

TL;DR: 本文使用随机Gumbel AlphaZero (SGAZ)作为评估器，对俄罗斯方块拼图游戏的规则变体难度进行系统研究，发现增加holding block和preview功能可降低难度，而添加更多方块变体会增加难度。


<details>
  <summary>Details</summary>
Motivation: 尽管Tetris Block Puzzle拥有数千万下载量，但缺乏对不同规则集难度的原则性评估。研究者希望借鉴AlphaZero在象棋变体中的成功方法，用SGAZ作为强大的评估器来解决这一问题。

Method: 使用随机Gumbel AlphaZero (SGAZ)作为预算感知的规划智能体，评估不同规则变化，包括：holding block (h)、preview holding block (p)以及额外的Tetris块变体。使用的指标包括训练奖励和收敛迭代次数。

Result: 实证结果表明：增加h和p会降低难度（更高奖励和更快收敛）；添加更多Tetris块变体会增加难度，其中T-pentomino导致最大的减速效果。

Conclusion: SGAZ能够在小模拟预算下提供强大的游戏表现，实现跨规则集的高效、可重复比较，为未来随机益智游戏的设计提供参考。
...
TL;DR: 本文研究了N个交互模块在T轮中的在线资源分配问题，其中成本是内生的（由交互矩阵W决定）。论文分析了三种范式：统一分配（成本无知）、门控分配（成本估计）和竞争性分配（成本揭示）。证明了三者之间存在严格的性能分离：统一分配产生Ω(T)遗憾，门控分配达到O(T^{{2/3}})，而竞争性分配达到O(√(T log N))。研究还揭示了W的拓扑结构决定了计算成本与遗憾之间的权衡，其中环状结构（如五元素五行拓扑）能够最优地平衡这两个指标。这是首个为分散式竞争分配提供遗憾理论形式化证明的研究，将成本内生性确立为与部分可观测性根本不同的挑战。


<details>
  <summary>Details</summary>
Motivation: 在线优化中的传统方法假设成本是外生的或仅依赖于当前决策，但现实系统中成本往往是内生的——取决于完整的分配向量和模块间的交互关系。已有研究未能充分考虑这种成本内生性带来的独特挑战，本文旨在填补这一空白。

Method: 1. 建立带交互矩阵W的在线资源分配模型<br>2. 分析三种分配范式：统一分配（不考虑成本）、门控分配（估计成本）、竞争性分配（利用乘法权重更新揭示成本）<br>3. 在对抗性序列（具有有界变化）下推导各范式的遗憾上界<br>4. 探讨不同拓扑结构（全连接、稀疏、环状）对性能的影响

Result: 1. 性能分离：统一分配遗憾Ω(T)，门控分配O(T^{{2/3}})，竞争性分配O(√(T log N))<br>2. 拓扑权衡：全交互(|E|=O(N²))遗憾界最紧但计算成本高；稀疏拓扑(|E|=O(N))遗憾仅增加O(√log N)且成本从O(N²)降至O(N)<br>3. 最优结构：环状拓扑（含合作与竞争链接，如五行拓扑）最小化计算×遗憾的乘积

Conclusion: 竞争性分配通过利用内生成本信息实现了显著优于传统方法的性能，为分散式模块化架构中的资源分配提供了首个形式化的遗憾理论依据。成本内生性是与部分可观测性根本不同的新挑战，拓扑结构的设计需在计算效率与优化性能之间进行权衡。该研究为复杂系统中的自适应资源分配提供了理论基础。
...
TL;DR: Stability Monitor是一个黑盒稳定性监控系统，通过定期对模型端点进行指纹识别来检测行为变化，使用固定提示集采样输出并比较时间推移中的输出分布，以检测模型身份变化。


<details>
  <summary>Details</summary>
Motivation: 传统可靠性指标（如正常运行时间、延迟和吞吐量）无法捕捉行为变化，端点可能在模型身份实际发生变化时仍显示为"健康"状态，因为权重、分词器、量化、推理引擎、内核、缓存、路由或硬件的更新都可能导致有效的模型身份变化。

Method: 使用固定提示集对端点进行周期性指纹识别，通过比较产生的输出分布来监控稳定性。使用跨提示的求和能量距离统计量进行比较，使用排列检验p值作为分布偏移的证据，并按顺序聚合以检测变化事件并定义稳定期。

Result: 在受控验证中，Stability Monitor能够检测到模型家族、版本、推理栈、量化参数和行为参数的变化。在对多个提供商托管的同一模型进行真实监控时，观察到提供商之间和提供商内部存在显著的稳定性差异。

Conclusion: 该系统能够有效识别模型端点的行为变化，揭示了当前模型部署中普遍存在的稳定性问题，为AI原生应用的可靠性保障提供了新的监控范式。
...
TL;DR: 本文是一篇综合综述研究，聚焦于人工智能在司法决策中的应用。研究连接了三个相互关联的维度：AI工具的性能与公平性、人类法官的优势与偏见、以及AI与人类的交互模式，涵盖计算机科学、经济学、法律、犯罪学和心理学等多个学科领域。


<details>
  <summary>Details</summary>
Motivation: AI技术融入司法决策（尤其是审前、量刑和假释环节）引发了关于透明度、可靠性和问责制的重大担忧，同时凸显了人类判断的局限性，以及理解法官如何与AI决策辅助工具互动的必要性。

Method: 采用综合综述方法，以刑事司法风险评估为核心案例，系统梳理了多个学科领域的研究成果，评估自动化风险评估工具的预测效度、记录司法决策中的偏见，以及检验法官如何使用算法建议。

Result: 现有实证证据表明，AI决策辅助工具对审前和量刑决策的影响较小或不存在。同时发现重要研究空白：需进一步评估AI风险评估工具的性能、理解法官如何在嘈杂决策环境中导航、以及个体特征如何影响法官对AI建议的响应。

Conclusion: AI与人类的比较研究有潜力产生新见解，建议未来加强跨学科整合与交叉融合，以深化对算法工具和人类决策者的理解。
...
TL;DR: 研究比较了大型语言模型（LLMs）与人类在创意生成上的表现，并测试了跨域映射（从随机远程源领域进行类比）是否能提升创造力。人类和LLMs为10种日常产品生成创新特性，分别在两种提示条件下：跨域映射（要求从随机分配的资源如章鱼、仙人掌、GPS等翻译特性）和用户需求（针对未满足的用户需求提出创新）。结果显示人类能从随机跨域映射中获益，而LLMs生成的创意想法比人类更原创，但跨域映射的效果在统计上不显著。然而，两种系统在跨域映射的灵感来源与目标产品语义距离越远时，效果都更好。研究强调了远程关联在创意构思中的作用，并揭示了人类和LLMs对同一创造力干预措施的系统性差异。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型是否像人类一样具有创造力，以及相同的创造力干预措施是否对两者都有效。具体检验跨域映射（从随机、远程源领域进行类比）这一有前景但尚未充分验证的创造力干预方法。

Method: 人类参与者和LLMs为10种日常产品（如背包、电视）在两种提示条件下生成新颖特性：(1)跨域映射条件，要求从随机分配的资源（如章鱼、仙人掌、GPS）翻译特性；(2)用户需求条件，要求提出针对未满足用户需求的创新。比较两种条件下创意产出的原创性。

Result: 人类从随机分配的跨域映射中可靠获益；LLMs平均生成比人类更原创的想法，但跨域映射的效果在统计上不显著。然而，在两种系统中，当灵感来源与目标产品的语义距离越远时，跨域映射的影响都增加。

Conclusion: 研究强调了远程关联在创意构思中的作用，并揭示了人类和LLMs对同一创造力干预措施存在系统性差异。LLMs可能已经内置了跨域映射能力，因此额外的干预效果有限。
...
TL;DR: LuMamba是一个用于EEG的自监督基础模型，结合了拓扑不变编码与线性复杂度状态空间建模，在21,000小时TUEG数据上预训练，仅用4.6M参数就在多个下游任务取得最先进性能，且计算效率比Transformer模型高377倍。


<details>
  <summary>Details</summary>
Motivation: 构建EEG基础模型面临两大挑战：1) 不同电极拓扑结构导致的数据异构性；2) Transformer架构的二次复杂度带来的计算可扩展性问题。需要一种既能统一不同电极配置、又能高效处理长序列的方法。

Method: 提出LuMamba框架，核心创新包括：1) 使用LUNA的学习查询交叉注意力机制实现通道统一，生成拓扑不变编码；2) 采用FEMBA的双向Mamba块进行高效时序建模；3) 首次系统研究LeJEPA（潜在欧几里得联合嵌入预测架构）在生物信号中的应用；4) 预训练采用掩码重建与LeJEPA联合目标。

Result: 1) 预训练目标对比：单独掩码重建产生结构化但泛化性差的表示；单独LeJEPA产生弥散的嵌入；两者结合获得最鲁棒性能；2) 下游任务：TUAB平衡准确率80.99%；阿尔茨海默病检测AUPR达0.97（最先进水平）；3) 效率：仅4.6M参数，377×更少FLOPS，可处理12倍更长序列。

Conclusion: LeJEPA与掩码重建的联合预训练策略对EEG表示学习至关重要。LuMamba成功解决了EEG基础模型中的拓扑异构性和计算效率问题，为大规模EEG数据分析提供了高效可行的方案。
...
TL;DR: 研究了在扩展思维链推理场景下，如何通过黑盒方法估计推理语言模型的不确定性。发现自洽性和口头置信度都能随采样规模扩展，但结合两者的混合估计器仅用两个样本就能提升12%的AUROC。


<details>
  <summary>Details</summary>
Motivation: 扩展思维链推理的不确定性估计目前理解不足，对部署推理语言模型至关重要。

Method: 使用三种推理模型和17个任务（涵盖数学、STEM和人文领域），采用黑盒方法研究并行采样、口头置信度和自洽性三种不确定性估计信号。

Result: 自洽性和口头置信度都能扩展，但自洽性初始区分度较低，在中等采样下不如口头置信度。仅用两个样本的混合估计器平均提升AUROC达12%，优于单独使用任一信号，即使后者扩展到更大采样预算。

Conclusion: 混合估计器效果显著但存在领域依赖性：数学领域（RLVR后训练的原生领域）不确定性质量更高，信号互补性更强，扩展速度更快，STEM和人文领域则表现较弱。
...
TL;DR: 本文提出了一种针对离散扩散模型的广义束搜索解码框架，并设计了D5P4方法利用行列式点过程(DPP)实现批量内多样性控制，在保持生成质量的同时提升生成结果的多样性。


<details>
  <summary>Details</summary>
Motivation: 自回归模型的束搜索等标准解码方法不能直接适用于迭代去噪的离散扩散模型，而现有扩散解码技术对批量内多样性的控制能力有限。

Method: 提出广义束搜索框架，支持并行生成候选并实现模块化束选择目标；具体提出D5P4方法，将选择步骤建模为DPP上的MAP推理，采用可扩展的贪婪求解器，保持多GPU兼容性。

Result: 在自由形式生成和问答任务上的实验表明，D5P4相比强基线方法显著提升多样性，同时保持有竞争力的生成质量。

Conclusion: 该框架能够实现模型概率与目标多样性的显式权衡，且计算开销极小，为离散扩散模型的解码提供了新思路。
...
TL;DR: cuGenOpt是一个GPU加速的通用元启发式框架，解决组合优化问题中通用性、性能和可用性之间的权衡。该框架采用CUDA架构、统一编码抽象、双层自适应算子选择，并通过JIT编译和LLM建模助手提供Python API。


<details>
  <summary>Details</summary>
Motivation: 组合优化问题在物流、调度和资源分配等领域广泛应用，但现有方法在通用性、性能和可用性之间存在根本性的权衡问题。

Method: 1. 引擎层面：采用"一个block演化一个solution"的CUDA架构，包含统一编码抽象（排列、二进制、整数）、双层自适应算子选择机制和硬件感知资源管理；2. 扩展性层面：提供用户自定义算子注册接口；3. 可用性层面：JIT编译管道提供纯Python API，LLM建模助手将自然语言问题描述转换为可执行求解器代码。

Result: 在三个GPU架构(T4, V100, A800)上对五个主题套件的实验表明：cuGenOpt在通用性上优于MIP求解器数个数量级；在n≤150的实例上与专业求解器质量相当；在TSP-442上30秒内达到4.73%的gap；12种问题类型跨5种编码变体达到最优解；框架级优化使pcb442的gap从36%降至4.73%，VRPTW吞吐量提升75-81%。

Conclusion: cuGenOpt成功地在通用性、性能和可用性三个维度上同时取得突破，为组合优化问题提供了一个高效、易用的GPU加速求解框架。
...
TL;DR: 本文提出Box Maze框架，通过将LLM推理分解为记忆锚定、结构化推理和边界执行三个显式层来增强推理过程完整性。初步仿真结果表明，该架构可将对抗条件下的边界失败率从约40%降至1%以下。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在对抗性提示下容易产生幻觉和不可靠推理，现有安全方法如人类反馈强化学习(RLHF)和输出过滤主要在行为层面操作，缺乏显式的架构机制来强制执行推理过程完整性。

Method: 提出Box Maze框架，将LLM推理分解为三个显式层：记忆锚定(Memory Grounding)、结构化推理(Structured Inference)和边界执行(Boundary Enforcement)。在多个异构LLM系统(DeepSeek-V3、Doubao、Qwen)上进行了渐进式边界侵蚀场景的仿真评估，涵盖50个对抗场景。

Result: 初步结果表明，显式认知控制层可改善边界维护的一致性，架构约束将对抗条件下的边界失败率从约40%(基线RLHF)降至1%以下。

Conclusion: 虽然当前验证基于仿真，但这些初步结果表明过程级控制可能是提高大型语言模型推理可靠性的一个有前景的方向。
