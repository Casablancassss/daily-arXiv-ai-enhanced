...
TL;DR: AtmoFuseNet是一个融合多视角天空相机图像、毫米波云雷达和 ceilometer 观测数据的框架，用于从稀疏地基仪器重建云微物理场的4D（3D空间+时间）估计。该方法包含三个阶段：跨模态层次聚合模块、条件变分细化模块和基于相关的运动估计器。


<details>
  <summary>Details</summary>
Motivation: 从稀疏地基仪器重建云的密集体积结构是一个开放问题，因为可用测量数据在模态和空间覆盖范围上都是异构的。现有的方法难以有效融合不同类型的观测数据，导致云微物理参数的反演精度不足。

Method: 提出三阶段方法：(1) 跨模态层次聚合模块：通过分层交叉注意力机制将图像特征金字塔与仪器-derived垂直剖面相结合；(2) 条件变分细化模块：在可微分雷达和图像前向模型下将结果体积映射到物理一致的微物理场；(3) 基于相关的运动估计器：从连续体积重建中恢复每个体素的3D风矢量。

Result: 在来自半干旱站点的协同观测数据上，AtmoFuseNet达到了0.026 g m^-3的液态水含量MAE和1.18 m s^-1的风速MAE，优于现有检索基线方法。

Conclusion: AtmoFuseNet成功实现了多源异构气象观测数据的有效融合，为云微物理场的4D重建提供了一种新的深度学习方法，在云物理研究和气象预报方面具有应用价值。
...
TL;DR: 提出了一种基于流式高斯编码器的4D全景占用跟踪方法，通过维护持久性体素场景表示来增强几何时间一致性，解决了现有方法中每帧重新计算体素表示导致的遮挡和静态元素跟踪问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码的管道虽然通过跨帧传播实例查询实现了较好的性能，但其底层体素表示通常在每个时间步重新计算，限制了几何时间一致性，特别是在遮挡和静态场景元素情况下。

Method: 提出流式高斯编码器，维护固定大小的潜在高斯查询集，通过自我运动补偿传播，并在置信度引导的预算约束下刷新。通过基于深度监督的高斯不透明度作为可见性代理，使置信度作为持久场景支持的时序聚合度量。结合基于预热的多帧训练策略，实现表示级别的时序一致性。

Result: 在Occ3D-extended nuScenes和Waymo数据集上建立新的最先进水平，提升跟踪一致性，计算开销极小，且完全兼容现有基于掩码的管道。

Conclusion: 该方法通过持久性体素表示和置信度累积机制有效解决了4D全景占用跟踪中的时序一致性问题，具有良好的实用性和兼容性。
...
TL;DR: 提出了一种学习轨迹数据集潜在表征并使用分布度量量化其相似性的框架，涵盖24个主要数据集，表明可迁移性分数与跨数据集模型性能高度相关。


<details>
  <summary>Details</summary>
Motivation: 在轨迹数据集上训练的模型往往无法泛化到训练领域之外，因为场景布局、代理行为和感知条件存在差异。

Method: 提出一个学习数据集潜在表征并使用分布度量量化相似性的框架。

Result: 可迁移性分数与跨数据集模型性能高度相关，覆盖24个主要数据集（包括最广泛使用的运动预测基准）。

Conclusion: 为运动预测的数据集选择、预训练和大规模基础模型提供实践指导，为更通用和更稳健的预测系统铺平道路。
...
TL;DR: 本文研究了源自由域自适应目标检测（SFDA-OD）中的一种新设置：当目标域有少量均匀采样的标注图像时，如何有效利用这些稀疏标签。提出了RTSM方法，通过在保持原有无标注自训练分支不变的同时，加入监督检测损失来融合稀疏标注。在4种SFDA-OD方法、2种目标检测器、多个自适应任务、1%-10%标注预算下进行评估，RTSM相比纯SFDA一致提升1.7至18.3 AP50。


<details>
  <summary>Details</summary>
Motivation: 现有的SFDA-OD方法通常依赖无标注目标域的自训练（师生架构），忽略了当目标域存在少量标注数据时可以提供监督信号的可能性。本文旨在探索如何有效利用目标域的稀疏标注来提升检测性能。

Method: 提出随机目标监督混合（RTSM）：一个简单的锚点方法，将稀疏标注通过监督检测损失融入检测器，同时保持原有的无标注自适应分支不变。还评估了10种稀疏标签反馈插件，包括伪标签选择、目标补全和优化控制策略。

Result: RTSM在4种SFDA-OD方法、2种检测器、1%-10%标注预算下，相比纯SFDA一致提升1.7至18.3 AP50。而10种稀疏标签反馈插件仅带来有限且方法依赖的增益。

Conclusion: 稀疏标签SFDA-OD的苦涩教训：简单监督难以超越。RTSM为稀疏标签SFDA-OD提供了一个简单而有效的基线方法。
...
TL;DR: GaussLite是一个任务驱动的3D高斯溅射(3DGS)映射系统,根据自然语言任务规范动态调整表示密度。该系统使用LLM解析任务描述,通过开放词汇检测器识别目标对象并生成分割掩码,从而将计算资源集中于任务相关的场景区域。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS系统均匀分配表示能力,忽视了多数机器人任务仅涉及重建几何体的部分区域,导致计算资源浪费在无关区域,而任务相关区域可能欠优化。

Method: 1)使用一次性LLM解析器从任务描述(如"准备拾取桌上的物体")中提取目标和锚点对象;2)通过开放词汇检测器逐帧 grounding目标对象并分割,生成每像素相关性掩码;3)根据任务相关性分配高斯种子密度、梯度流和缩放;4)多个任务专用智能体的地图可通过活跃优化计数的体素投票融合。

Result: 在相同高斯预算下,GaussLite在Replica数据集上ROI PSNR平均提升+2.72dB;在室内外真实硬件演示中提升+2.23dB;两任务专用智能体地图融合比拼接方式提升+3.42dB,而仅共享7.08%的地图。

Conclusion: 任务驱动的3DGS映射能有效将计算资源集中于任务相关区域,在资源受限硬件上实现4Hz实时映射,显著提升目标区域重建质量,且支持多智能体地图实时融合。
...
TL;DR: AVTok是一种用于音频视频生成的统一分词器，采用双流Transformer架构和共享编解码器，将音视频对编码为一维潜在表示，解决以往双分支方法中存在的表示差距和计算资源密集问题。


<details>
  <summary>Details</summary>
Motivation: 现有音频视频生成方法主要采用双分支设计，单独处理每个模态的分词和生成模块，存在两个主要问题：1) 忽视了模态间的表示差距；2) 需要大量计算资源进行训练。此外，异构信息不平衡也阻碍了模型利用对齐的音视频信息。

Method: 提出AVTok，一种统一分词器，具有以下特点：1) 双流Transformer架构；2) 共享编码器-解码器；3) 模态特定的可学习查询；4) 统一码本的一维潜在表示；5) 层次化训练策略，逐步实现每个模态的重建能力。

Result: 实验表明：1) 在音视频重建任务中表现优异；2) 成功集成到下游管道用于音频到视频、视频到音频和类别条件联合音视频生成任务。

Conclusion: AVTok为联合音视频分词挑战开辟了道路，为构建统一的音视频生成大型多模态模型提供了潜在方向。
...
TL;DR: SyncCache是一种针对DiTPortrait动画的训练-free加速缓存方法，通过空间不对称探测和模态解耦缓存技术，利用音频驱动 portrait 动画中动态人物区域与静态背景的不对称性，实现高达4.12倍的推理加速，同时保持近乎无损的视觉质量和精确的唇同步。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free扩散缓存方法主要针对文本条件生成设计，未能充分考虑音频驱动portrait动画中固有的空间和模态不平衡问题，导致在加速音频驱动portrait动画时效果不佳。

Method: 提出三种核心技术：1) 空间不对称探测(Spatially-Asymmetric Probing)优先考虑动态人物区域的误差敏感性；2) 模态解耦缓存(Modality-Decoupled Caching)通过重用稳定的块间残差绕过重型DiT块，同时持续重计算轻量级音频块以保留精确的唇同步；3) 引入缓存比率控制缓存容量，并将内存自适应缓存选择表述为离线动态规划问题以消除在线开销。

Result: 在HunyuanVideo-Avatar上实现最高4.12倍加速，在Wan-S2V上实现3.75倍加速，同时保持近乎无损的视觉保真度和精确的音频对齐。

Conclusion: SyncCache在速度-质量权衡方面表现优异，成功验证了利用音频驱动portrait动画中不对称动态特性进行高效推理加速的可行性。
...
TL;DR: 提出标签模仿游戏(LIG)框架和图灵测试网络(TTN)，通过对抗性审讯机制修剪基础模型的伪标签幻觉，在四个数据集上显著提升标签准确性，并实现独特的零样本任务迁移能力。


<details>
  <summary>Details</summary>
Motivation: 基础模型通过零样本推理进行伪标签标注虽能实现大规模数据处理，但性能受幻觉问题影响，标准阈值无法消除这些错误。

Method: 提出标签模仿游戏(LIG)框架，将伪标签修剪形式化为对抗性审讯。训练图灵测试网络(TTN)作为任务无关的"评判者"，在数据集范围内评估候选伪标签，而非使用孤立阈值过滤标签。学习语义-上下文逻辑作为空间-几何验证的替代方案。

Result: 最差基线类别F1分数提升28%，任务特定微调后提升44%。TTN在零样本任务迁移方面表现独特，可从图像分类迁移到目标检测伪标签修剪。

Conclusion: 该框架在四个不同数据集上证明了其鲁棒性，无需监督或重训练即可提升三个最新视觉语言模型的标签准确性。TTN实现了零样本任务迁移，在仅使用图像分类数据集训练的情况下可有效修剪目标检测伪标签。最显著的发现是"类别复兴"现象：TTN修剪能"解毒"下游模型的训练信号，使模型从对迁移脆弱类别的零召回中恢复。
...
TL;DR: This paper proposes a method for estimating object scale from monocular images by decomposing objects into structural elements and integrating external knowledge about design rules, geometric relationships, and conventional dimensions. The method aims to address discrepancies between virtual and real environments in verifying in-vehicle cameras for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: In vehicle camera verification using virtual environments, discrepancies in object scale between virtual and real settings cause reduced recognition performance. Additionally, measuring distance to high-altitude traffic signs using LiDAR or stereo cameras is difficult, necessitating size estimation from monocular images.

Method: The proposed method decomposes objects into multiple structural elements, detects each component from monocular images, estimates component sizes by considering structural relationships and dimensional consistency with surrounding elements, and generates 3D assets by reconstructing the estimated components.

Result: This method enables placing 3D assets with scale approximating real environments within digital twin spaces, potentially improving verification accuracy of in-vehicle cameras for autonomous driving in virtual environments.

Conclusion: The approach successfully addresses scale estimation challenges for objects like high-altitude traffic signs that are difficult to measure with traditional methods, contributing to more accurate virtual environment verification for autonomous driving systems.
...
### [10] [DataEvolver: Self-Evolving Multi-Agent Data Construction for Text-Rich Image Generation](https://arxiv.org/abs/2606.31537)
*Siyu Yan,Yizhen Gao,Yilin Wang,Dongxing Mao,Alex Jinpeng Wang*

Main category: cs.CV

TL;DR: DataEvolver是一个自进化的多代理框架，用于文本丰富图像数据构建，通过利用被拒绝样本的失败信号来改进数据质量。


<details>
  <summary>Details</summary>
Motivation: 现有数据管道采用静态爬取-过滤-冻结范式，被拒绝的样本通常被丢弃，尽管它们包含OCR错误和语义不匹配等有用的失败信号，导致后续构建轮次可能重复相同的失败模式。

Method: DataEvolver包含四个组件：Retriever收集候选样本，Verifier分配质量分数和拒绝原因，Critic将轮级反馈总结为语义反馈，Generator通过针对性合成完成覆盖不足的区域。更新后的反馈记忆指导下一轮构建。

Result: 在0.75M规模上，DataEvolver在PixArt-alpha上使TextScenesHQ的OCR-F1比最强基线提高85.3%，LongTextBench提高35.3%。改进在两个基准上保持一致，并可转移到Show-o2，表明DataEvolver的收益不依赖于单一下游生成器。

Conclusion: 被拒绝的样本可以提供可操作的反馈，用于改进文本丰富图像数据构建。DataEvolver通过反馈驱动的构建策略演化，有效提升了文本渲染质量。
...
TL;DR: GRAPE是一种统一架构的基于原型的医学图像分类器，通过图注意力任务头、概念错配安全检查和开放词汇原型锚定三个创新模块，同时解决了原型模型将发现视为独立、错误放大不安全反馈、需全量重训练三个临床局限。


<details>
  <summary>Details</summary>
Motivation: 基于原型的医学图像分类器存在三个临床局限：1)将影像发现视为独立事件，忽略解剖概念共现关系；2) silently放大不安全的医师反馈，导致错误累积；3)新增发现类别需完整重训练模型，扩展性差。

Method: 提出GRAPE统一架构包含三个核心组件：1)图注意力任务头建模解剖概念共现关系；2)概念错配安全检查机制检测模型主导发现与标注冲突；3)开放词汇原型锚定将视觉原型与临床文本对齐，实现单样本新增类别。

Result: 图注意力任务头在TBX11K上相比原型基线提升macro-F1 13.8个百分点；安全检查机制捕获85%错误标注（vs MC-Dropout的51%）；开放词汇锚定在NIH ChestX-ray14上单张Effusion样本恢复全监督定位精度；TBX11K上原型图实现2.6倍于端到端基线的病灶定位效果。所有功能仅增加+1ms延迟。

Conclusion: GRAPE通过三个创新模块有效解决了原型医学分类器的三大临床局限，在分类准确性、安全性和可扩展性方面均取得显著提升，且保持实时推理性能。
...
TL;DR: 针对LiDAR语义分割在真实部署中因传感条件变化导致的性能下降问题，提出了一种几何约束的测试时提示调整框架。该方法通过深度一致的波束终止和邻域支持来估计每位置传感可靠性，用于重新加权空间监督，并通过轻量级提示适配器和空间门控机制防止不可靠区域干扰全局共享表示，同时利用时间平滑原型对齐策略稳定在线更新。


<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割在实际部署中因传感条件变化而性能下降，且收集新标注数据进行再训练不切实际。标准测试时适应(TTA)策略直接应用于LiDAR数据面临挑战，因为伪标签可靠性在距离依赖的稀疏性和遮挡下呈现空间异方差性，全局共享参数的均匀更新会注入不稳定梯度并破坏适应稳定性。

Method: 提出几何约束的测试时提示调整框架：1)从深度一致的波束终止和邻域支持估计每位置传感可靠性；2)使用可靠性重新加权空间监督；3)将适应限制在插入冻结主干网络的轻量级提示适配器中；4)使用空间门控防止不可靠区域干扰全局共享表示；5)采用时间平滑原型对齐策略积累可靠语义证据。

Result: 在标准LiDAR基准数据集上的实验表明，该方法在无需额外标注的情况下，提高了部署变化下的适应稳定性和分割性能。

Conclusion: 该框架通过几何约束的可靠性估计和提示调优技术，有效解决了LiDAR语义分割在动态传感条件下的测试时适应问题，在保持冻结主干网络的同时实现了稳定且高效的性能提升。
...
TL;DR: Prost-RL是一种基于强化学习的微超声前列腺癌检测框架，通过将检测问题重新定义为空间感知的策略驱动推理问题，集成轻量级强化学习策略到基础模型编码器-解码器中，生成可解释的空间注意力图作为软提示，在6607个活检核心来自693例患者中实现79.0% AUROC和64.6%灵敏度。


<details>
  <summary>Details</summary>
Motivation: 微超声(μUS)是新兴的前列腺癌检测成像技术，但准确识别可疑组织高度依赖临床经验，导致显著的观察者间变异性。训练可靠的深度模型具有挑战性，因为监督信息稀疏且有噪声，通常仅限于核心级别组织病理学结果（无像素级病变标注且类别严重不平衡）。

Method: Prost-RL将μUS前列腺癌检测重新定义为空间感知的策略驱动推理问题，通过学习在看之前先决定看哪里。Prost-RL将轻量级强化学习策略集成到基础模型编码器-解码器中，生成可解释的空间注意力图，作为癌症可能性热图预测和图像级分类的软提示。提出自适应策略优化(APO)来稳定混合监督-强化学习训练，以及结合对称交叉熵和负熵正则化的噪声鲁棒目标以减轻弱标签噪声并鼓励清晰定位。

Result: 在来自5个临床中心的693例患者的6607个活检核心中，Prost-RL在核心级别检测中达到79.0±3.5 AUROC和64.6±6.3%灵敏度（在80%特异性下），比最强基线提高2.1 AUROC和4.5灵敏度点。临床显著癌分类达到79.3±5.8 AUROC。学习到的策略突出显示活检对齐区域，提供透明、空间锚定的证据以及定量风险预测。

Conclusion: Prost-RL通过强化学习引导的注意力机制成功解决了微超声前列腺癌检测中的观察者变异性和标注稀疏性问题，在多中心数据上展现出优异的性能，提供了可解释的空间定位证据，有望提高临床诊断的一致性和准确性。
...
TL;DR: PhotoQuilt是一个无需训练的框架，用于生成任意分辨率的 Photomosaics（照片马赛克）。该方法通过引导式平铺去噪程序解决了扩散模型在同时处理全局结构和局部细节方面的难题。


<details>
  <summary>Details</summary>
Motivation: 生成高分辨率且每个瓦片都逼真的照片马赛克计算成本高昂，因为画布需要同时容纳许多细节丰富的瓦片。直接高分辨率生成成本高且容易产生平滑图像而非马赛克效果，而基于Patch的平铺虽然保留局部细节但会丢失全局结构。

Method: PhotoQuilt采用引导式平铺去噪程序：1）首先生成低分辨率的全局构图来确定布局；2）在潜在空间中将其上采样并重新注入噪声以恢复生成能力；3）在固定瓦片内进行去噪，使每个瓦片形成自己的图像，同时共享的全局结构将它们保持在同一布局中。由于瓦片生成是独立处理的，PhotoQuilt可以扩展到大型画布而无需二次方注意力成本。

Result: 实验表明，PhotoQuilt在全局结构和局部真实感方面均优于当前基线方法。

Conclusion: PhotoQuilt成功解决了扩散模型在生成照片马赛克时难以同时兼顾全局结构一致性和局部细节质量的问题，无需训练即可实现任意分辨率的高质量马赛克生成，具有良好的可扩展性。
...
TL;DR: 论文针对AI生成视频检测基准测试中存在的评估缺陷问题，提出了一个经过审计的评估协议（VidAudit），揭示了当前领先榜评估中未受控的混淆因素会导致泛化性能被夸大，并通过六项标准控制措施和跨数据集测试来验证多种检测方法的真实性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成视频检测基准测试（如GenVidBench和AIGVDBench）的评估协议存在未受控的混淆因素，导致报告的泛化性能被夸大。一个简单的三特征片段长度分类器在未审计评估下即可达到0.998的LOGOAUC，但实际与运动检测无关。

Method: 1）对20篇相关论文进行调查，发现均未应用全部六项标准控制；2）将六项标准控制整合为审计协议；3）对六种代表性特征源（三种已发布检测器和三种重新利用的信号源）应用该协议；4）在AIGVDBench上进行跨数据集测试；5）提出跨基底特征融合（XSFF）方法作为白盒正控

Result: 1）简单分类器在审计后崩溃至接近随机水平（0.529）；2）CLIP基线被发现携带数据集身份信息；3）2025年法医检测器WaveRep在分布外LOGOAUC达0.996，且真实vs真实一致性达到 chance level；4）在0.1%FPR可部署条件下，多个高AUC方法的召回率降至个位数，排行榜顺序改变；5）TemporalSpec（编码器运动向量）作为白盒正控通过XSFF实现了真正的互补性

Conclusion: 建议使用审计后的评估元组（AUC、超过基线的边界、操作点召回率和校准）替代单一数值；发布VidAudit工具包，包含14个检测器、一个排行榜和Croissant元数据；将评估从排行榜排名转向验证结果是否真正测量了声称的内容。
...
TL;DR: 本文研究如何利用视觉基础模型(SAM 3)实现手术视频中动作感知的稀疏地标定位，无需人工像素级标注即可预测器械尖端和锚点。


<details>
  <summary>Details</summary>
Motivation: 现有分割模型无法明确编码定义功能手术地标的动作条件语义，器械末端和几何估计与夹持、分离等动作相关的尖端或锚点定位不同，需要一种无需人工像素级标注的地标定位方法。

Method: 提出轻量级精炼框架，使用SAM 3作为结构先验。粗略多帧网络预测尖端和锚点提示，生成非神谕掩码，与视觉和热图特征融合以精炼功能地标预测。比较了直接掩码增强监督、预测派生掩码先验精炼和辅助掩码监督三种方法。

Result: 在60个手术视频的7867个片段上测试，尖端定位F1分数达72.4%，锚点定位达58.0%。直接掩码监督偏向广阔工具区域，而预测派生先验和辅助监督为动作依赖地标预测提供有效中间结构指导。

Conclusion: 视觉基础模型派生结构应作为预测派生先验或辅助监督引入系统，而非直接叠加在热图目标上，这为手术器械动作感知定位提供了有效中间结构引导。
...
TL;DR: 提出了一种用于多光谱目标检测的双稀疏聚合Transformer（DSAFormer），通过空间稀疏和通道稀疏的交叉注意力机制来选择性地捕获跨模态关系，减少无关区域的冗余信息交互，提高检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的基于Transformer的多光谱目标检测方法存在冗余信息交互问题，因为所有token都参与相似度计算，导致来自无关区域的冗余信息降低了检测性能。

Method: 提出了DSAFormer框架，包含两个核心组件：1）双稀疏Transformer（DSFormer）：包括空间稀疏多头交叉注意力（SSMHCA）、通道稀疏多头交叉注意力（CSMHCA）和多尺度特征细化层（MSFRL）；2）可学习加性融合块（LAFB）：通过特征重加权来融合多模态特征。

Result: 在四个公开数据集（MFAD、FLIR、M³FD、LLVIP）上进行了广泛实验，结果表明DSAFormer实现了比最先进方法更好的检测性能。

Conclusion: DSAFormer通过稀疏注意力机制有效过滤无关信息，增强了跨模态互补信息的捕获，在多光谱目标检测任务上取得了显著的性能提升。
...
TL;DR: 本文提出了一种基于显著性的warp-unwarp框架，用于改善潜在扩散模型（LDM）在图像到图像（I2I）翻译任务中的细粒度结构保持能力，同时设计了基于outpainting的合成数据生成管道用于图像重光照。


<details>
  <summary>Details</summary>
Motivation: 紧凑型LDM在处理高分辨率输入时，会将其压缩到空间下采样的潜在空间，导致细粒度结构信息丢失，难以保留图像的精细结构细节。

Method: 1. 提出显著引导的warp-unwarp框架：在编码前将空间表征重新分配到显著区域，然后经过原始扩散模型处理，最后通过逆 warp 映射回原空间；2. 设计基于outpainting的合成数据生成管道，用于生成高质量的配对数据用于图像重光照。

Result: 在人体重光照、驾驶场景重光照和图像翻译任务上取得了更好的结构保持、光照真实性和图像质量提升，且该框架可自然扩展到视频处理并保持良好的时间稳定性。

Conclusion: 该方法具有模型无关性，无需修改网络架构，仅引入极小的计算开销即可提升LDM在I2I任务中的细粒度结构保持能力。
...
TL;DR: TerraDiT-Ω是一个统一的空间控制框架,能从多类型地理空间原生图元(多边形、多线段、边界框、点)直接生成卫星影像,解决了现有方法在标注成本、保真度与向量原语兼容性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 与自然图像不同,卫星影像具有复杂的空间结构和语义上独特的几何形状。现有方法将自然图像框架适配到卫星影像时,需在密集光栅或稀疏提示之间进行权衡,牺牲了标注成本与保真度,且与传统地理信息使用的矢量原语不兼容。

Method: 提出Geometry-Aware Local Attention机制,将显式几何线索注入注意力空间,使模型能有效利用精确标注(多边形、多线段)和粗略标注(边界框、点)进行条件生成,支持不同标注预算下的可控布局。

Result: 在所有条件格式上均优于密集控制和稀疏控制基线方法。该灵活性使得单一生成模型可进行可控的合成数据增强,提升了地物分割、目标检测、道路图提取和场景分类等下游任务的性能。

Conclusion: TerraDiT-Ω实现了卫星影像生成与矢量地理原语的原生兼容,拓宽了在城市规划等设计任务中的应用,并能与端到端GeoAI工作流程自然集成。
...
TL;DR: 本文提出预测可微渲染(Predictive Differentiable Rendering, PDR)框架，通过引入基于2D高斯表示的PredGS适配器，将离散像素空间预测与连续表示相结合，解决传统视频预测模型过度平滑、缺乏细节的问题。


<details>
  <summary>Details</summary>
Motivation: 现有确定性视频预测模型在离散像素空间操作，使用像素级MSE优化，导致预测过度平滑、缺乏细粒度视觉细节，无法准确预测高保真未来世界。

Method: 1. 提出PDR范式，桥接离散与连续表示；2. 引入PredGS轻量级即插即用适配器，基于2D高斯表示；3. 开发predgsplat CUDA加速可微2D高斯渲染器，支持任意通道，每个高斯由5+C个可学习参数定义（位置、尺度、旋转、C通道幅度）；4. 结合L1和SSIM损失优化。

Result: 在TaxiBJ、WeatherBench、KTH和Human3.6M等真实世界基准数据集上，PDR显著超越现有方法，渲染速度比基线快10倍，实现更好的细节保留、视觉保真度和预测精度。

Conclusion: PDR通过可微渲染技术成功桥接离散像素空间与连续表示，PredGS适配器可无缝集成到现有像素空间预测器中，在保持较低计算开销的同时显著提升空间细节保留，为高保真视频预测提供了有效解决方案。
...
TL;DR: 本文提出ADAPT框架，通过干预文本到图像交叉注意力的动态过程来减少多模态大语言模型的幻觉问题。该方法包含三个关键组件：交叉注意力视觉锚点、注意力监督推理机制和视觉注意力引导DPO。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在严重的幻觉问题，生成与图像不一致的内容。现有缓解策略主要关注结果而非内部机制。作者发现幻觉存在内部特征：生成过程中文本到图像交叉注意力逐渐退化，导致注意力分散或偏倚。

Method: 提出ADAPT框架，包含三个组件：1) 从早期解码中精炼交叉注意力视觉锚点，提供稳定的空间定位；2) 注意力监督推理机制，在线检测和纠正注意力漂移；3) 视觉注意力引导DPO，对齐视觉 grounded 的响应偏好。

Result: 实验表明ADAPT各组件都有助于减少幻觉，完整框架在多个幻觉基准测试中取得新最佳结果，幻觉率降低40%-60%，同时保留通用多模态能力。

Conclusion: 本文从注意力角度提供了缓解幻觉的新视角，通过探索模型内部文本到图像交叉注意力行为来解决问题，为多模态模型的可靠性提升提供了有效方案。
...
TL;DR: 提出了一种桥接扩散模型与物理渲染的方法,将扩散模型的预测作为优化相似性核而非目标值,联合重建几何、材质和光照,生成可直接用于标准渲染管线的高质量3D资产。


<details>
  <summary>Details</summary>
Motivation: 解决物理逆渲染的欠约束问题:无强先验时光照会被烘焙到材质中,重建结果泛化性差;扩散模型预测的材质不满足渲染方程,无法直接用于物理渲染。

Method: 将扩散模型预测作为相似性核:引入正则化损失惩罚扩散预测接近常数的表面区域的材质偏差,同时让优化自由匹配输入图像。端到端管线联合重建几何、材质和光照。

Result: 在Synthetic4Relight、Stanford-ORB和DTC-Synthetic数据集上,重建精度和重光照质量均显著优于基线方法。

Conclusion: 成功桥接数据驱动扩散模型与物理渲染范式,生成可投入标准渲染管线的高质量资产,在多个基准数据集上取得显著性能提升。
...
TL;DR: 本文提出了一种混合UNet-Transformer架构，可直接从复合微观结构几何图像预测复杂机械场分布，作为FEM的高效替代方案。该模型在十种不同应力/应变场类型上进行了验证，涵盖多种复合构型。


<details>
  <summary>Details</summary>
Motivation: 传统的有限元方法(FEM)模拟在规模上计算成本过高，每次评估需要分钟到天级别的时间。因此需要一种高效的代理模型来预测应力/应变场，以支持基于物理的材料设计。

Method: 提出混合UNet-Transformer架构，直接从复合微观结构几何图像预测机械场分布。模型在多种两相复合构型上训练，包括正方形、六边形和三角形镶嵌结构，不同边界条件以及高分辨率几何形状。通过Grad-CAM和Grad-CAM++进行编码器注意力分析。

Result: 模型在大多数子数据集上取得优异预测性能：周期性镶嵌几何的边界条件子数据集上R²=0.9991、SSIM=0.9936，三角形镶嵌子数据集上MAE=0.0050。在8个评估子数据集中，有6个的MAE低于0.05（归一化[0,1]像素尺度）。注意力分析确认模型学习到了物理上有意义的内部表征，能够定位相界、韧带接头和压头接触区等机械关键区域。性能在稀疏软相夹杂的不规则正方形网格几何上下降，S11正应力子数据集R²=0.7735、SSIM=0.7126。

Conclusion: 混合UNet-Transformer架构能够有效预测复合微观结构中的应力/应变场，在规则几何上表现优异，但在不规则几何和尖锐应力不连续区域存在局限。该方法为快速预测复合材料机械性能提供了可行替代方案。
...
TL;DR: 提出一种零样本语义导航框架，通过在线维护层级3D场景图(HSG)作为紧凑状态抽象，结合基于信念的分层规划框架，在长距离导航任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型虽赋予智能体丰富的语义先验，但缺乏结构化全局表示，导致决策依赖局部观察和贪婪策略，在长距离导航中效率低下且行为短视。

Method: 1)增量维护在线层级3D场景图(HSG)，形成对象、区域、区域的多粒度语义拓扑；2)基于记忆引入分层信念规划框架，将语义先验与探索证据融合；3)在HSG模拟器上执行有限视野 rollout 评估候选宏动作的长期预期回报。

Result: 在多个任务和数据集的高保真模拟环境中实验结果表明，方法优于现有最先进方法；在长距离场景中，SR(成功率)和SPL(路径效率)分别平均提升9.4%和5.0%。

Conclusion: 该方法通过HSG实现全局一致的决策规划，减少冗余回溯，有效解决了长距离语义导航中的效率问题。
...
TL;DR: AnyMatch是一个利用单目深度估计、3D重投影、扩散修复和跨模态图像翻译，从易获取的单视图图像合成多视图、多模态图像对的大规模训练数据生成框架，可生成严格遵循3D几何一致性的标注数据。


<details>
  <summary>Details</summary>
Motivation: 多模态图像匹配对视觉定位和多传感器融合至关重要，但面临大规模精确几何标注训练数据稀缺的问题。现有真实数据集成本高、场景多样性有限且存在SfM-MVS管道误差，合成方法则难以保持3D几何一致性或实现照片级真实感。

Method: AnyMatch框架整合了单目深度估计、3D重投影、基于扩散的修复和跨模态图像翻译技术，从单视图图像合成具有3D几何保真度的多视图、多模态图像对。通过显式3D重投影提供严格遵守3D几何一致性的标注，避免SfM-MVS误差累积。

Result: 基于AnyMatch构建的大规模合成多模态数据集Any-syn进行微调的匹配网络（如LoFTR、EDM、RoMa）在多模态基准测试上实现了显著性能提升，表现出优于现有数据训练模型的泛化能力和鲁棒性。

Conclusion: AnyMatch方法具有强大的可扩展性，可通过调整输入和相机参数实现可控的场景多样性和标注难度。合成数据集Any-syn能有效提升多模态图像匹配网络的性能，为解决训练数据稀缺问题提供了有效途径。
...
TL;DR: AI生成图像检测领域正从静态特征空间范式向动态适应范式转变。现有静态方法在新兴生成器上性能严重下降，论文提出Fleet框架，通过约束路由纠正实现持续少样本演化，在新生成器"Doubao Seedream 4.0"上仅用10个样本即将性能从20.4%提升至73.1%。


<details>
  <summary>Details</summary>
Motivation: 当前AIGI检测方法假设存在可跨模型泛化的不变artifacts，但在快速演进的新型生成器（如SD3、Nano Banana Pro）上性能急剧下降，需要从"静态泛化"转向"动态适应"新范式。

Method: 提出Fleet框架，采用动态少样本演化范式，通过约束路由纠正替代无约束特征更新，其中避免路由将新型AI样本从非AI主导路由重定向到解耦子空间中。

Result: 在Treasure基准测试集（64个模型、360k图像）上验证，静态SOTA方法在现代生成器上性能崩溃，而Fleet仅需10-shot适应即可将性能从20.4%提升至73.1%。

Conclusion: 动态适应范式是AIGI检测的未来方向，Fleet框架成功实现了对新兴生成威胁的快速对齐，为开放世界对抗防御提供了新思路。
...
TL;DR: CasaMaestro是一种前馈模型，可以仅用20-50张稀疏多视角室内全景图作为输入，直接预测度量深度和相机姿态，实现整个房屋的快速点云重建，是首个支持多视角全景图房屋级重建的模型。


<details>
  <summary>Details</summary>
Motivation: 家庭部署的具身AI系统需要快速对住宅空间进行度量3D重建以支持导航、交互和长时任务执行。传统针孔相机3D重建管道因视场有限，难以高效建模大型室内住宅，实现全屋覆盖通常需要数千张图像，且存在长链增量对齐带来的漂移问题。

Method: 提出CasaMaestro，一种前馈神经网络模型。该模型仅需20-50张稀疏多视角室内全景图作为输入，直接预测度量深度和相机姿态，从而实现整个房屋的快速点云重建。

Result: CasaMaestro是首个支持多视角全景图房屋级重建的模型。实验表明，该模型能在真实场景和合成场景中稳健地提供高质量重建结果，可作为获取房屋级3D室内资产的强大基础。

Conclusion: CasaMaestro能够实现高效、准确的房屋级3D重建，可为具身AI系统提供完整的室内空间感知能力，适用于闭环模拟等应用场景。
...
TL;DR: InterTalk是一个用于生成交互式对话说话人脸的新框架，能够实时合成多人多轮对话视频，在保持30FPS实时性能的同时实现 superior interaction quality。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足对话人脸生成的三个核心挑战：灵活性（支持任意数量参与者的多轮对话）、自然性（保持连贯动作和非语言反馈）和效率（实时生成和低计算开销）。

Method: 提出InterTalk框架，基于动作架构，显式建模多人多轮对话动态，引入多人动作反馈和迭代生成策略，将动作解耦为多个面部组件以实现精准唇同步和真实眨眼，并构建新的多人对话数据集。

Result: 在30FPS实时性能下达到优异的交互质量。

Conclusion: InterTalk成功实现了灵活性、自然性和效率的统一，为交互式对话人脸生成提供了有效解决方案。
...
TL;DR: 本文提出ART（Anchoring on Reality Makeup Transfer）框架，解决换妆数据集缺失问题，通过两阶段现实锚定精化循环，在保持身份和几何特征的同时实现高质量换妆转移。


<details>
  <summary>Details</summary>
Motivation: 现有换妆方法受限于缺乏真实成对训练数据，只能依赖弱先验或大规模编辑模型的合成伪目标，导致细节退化、人工合成伪影和身份漂移等问题。

Method: 提出两阶段框架：Stage I使用伪目标初始化建立基本语义对齐和全局妆容布局；Stage II将监督从伪目标转向真实参考，通过可微分循环从素颜 counterpart 进行重建，惩罚遗漏细节并覆盖合成伪影。构建了MF2K数据集（2K分辨率，8573张图像）。

Result: 实验表明该方法在妆容保真度、背景稳定性和身份保持方面表现优异，尤其对复杂妆容风格效果显著。

Conclusion: 通过现实锚定的两阶段精化循环和高质量数据集，ART成功解决了换妆任务中成对数据缺失的核心挑战，实现了 superior 的换妆效果。
...
TL;DR: 提出一种结构保持的均值流(SPMF)框架,将血管图像翻译重建为拓扑不变的传输过程,解决现有跨模态翻译方法中血管结构断裂和伪影的问题。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态翻译方法主要强调像素级保真度或视觉真实性,将结构保持作为最终输出的属性而非生成过程的不变量,导致结构断裂和伪影,影响解剖连贯性和临床可靠性。

Method: 1.提出结构不变性原理,推导流速度场的正交约束,将外观传输与拓扑失真正式分离;2.将该约束实现为时间加权代理目标,融入布朗桥扩散模型,在每个扩散步骤保持拓扑;3.提出原型引导结构细化(PGSR)模块,将退化的推理时结构与可靠的训练时结构对齐。

Result: 在配对的NIRII-to-2PF和fundus数据集上实验,相比最先进方法实现一致改进,峰值PSNR分别达到24.96 dB和24.83 dB。

Conclusion: SPMF框架通过拓扑不变的传输过程有效解决了血管图像跨模态翻译中的结构保持问题,为临床血管成像提供了更可靠的解决方案。
...
TL;DR: Horizon3D是一个用于长距离3D目标检测的稀疏雷达-相机融合框架，结合了高斯原语与稀疏BEV特征。该方法通过关键点引导的高斯初始化、对象中心稀疏融合和双路径时序融合来提升检测性能，在TruckScenes数据集上实现了最先进的雷达-相机3D检测效果。


<details>
  <summary>Details</summary>
Motivation: 当前高速公路场景下的长距离3D目标检测对自动驾驶安全性至关重要。现有雷达-相机融合方法在远距离检测上存在局限：基于BEV的方法虽能捕获场景级上下文但计算量大且丢失细粒度目标细节；基于查询的方法效率高但场景级上下文有限。时序融合需要同时处理稀疏远距离观测的多帧累积和快速移动目标的对象级运动建模。

Method: 提出Horizon3D框架，包含三个核心组件：1)关键点引导高斯初始化(KGGI)在雷达和相机估计的目标关键点处初始化高斯原语；2)对象中心稀疏融合(OCSF)对高斯原语进行细化，并将它们splatt到BEV平面上融合对象级细节与稀疏雷达BEV上下文；3)双路径时序融合(DPTF)通过BEV路径进行场景级累积，通过高斯路径进行对象级运动传播。

Result: 在TruckScenes数据集上的实验表明，Horizon3D实现了最先进的雷达-相机3D检测性能。在验证集上，相比最佳方法提升了+3.0 NDS和+1.6 mAP，同时保持了具有竞争力的推理速度。

Conclusion: Horizon3D通过创新的稀疏雷达-相机融合框架有效解决了长距离3D目标检测问题，高斯原语与稀疏BEV特征的结合以及双路径时序融合机制显著提升了检测精度和效率，为自动驾驶在高速公路场景下的安全行驶提供了有力技术支撑。
...
TL;DR: PiLoT v2是一种轻量级无人机地理定位系统，通过直接像素到正交地图配准实现无漂移实时定位。相比前代PiLoT使用3D网格渲染，该方法采用真实数字正射影像图（TDOM）和数字表面模型（DSM）作为参考基材，用高效的地图裁剪操作替代GPU密集的3D渲染，大大降低了存储和计算成本。


<details>
  <summary>Details</summary>
Motivation: 原版PiLoT系统依赖重型3D网格，存在存储开销大、地图获取复杂、渲染计算成本高等问题，严重阻碍了在嵌入式平台上的部署。因此需要一种轻量级但鲁棒的替代方案。

Method: 1. 采用TDOMs和DSMs作为参考底图，用CPU友好的地图裁剪替代GPU密集的3D渲染；2. 构建新型大规模几何标注数据集，训练跨视图特征配准网络，解决2.5D正交裁剪与自由视角倾斜无人机影像间的几何差异；3. 将惯性测量单元的重力方向和单点激光测距的传感器先验信息直接融入姿态优化流形。

Result: 实验表明PiLoT v2达到了与Pixel-to-3D前代相当甚至更优的性能，同时大幅降低了存储和计算成本。

Conclusion: PiLoT v2成功实现了轻量级无人机地理定位，在保持高精度的同时显著降低了资源消耗，为在嵌入式平台上实现实时无漂移定位提供了可行方案。
...
TL;DR: View-PNDF是一种参数高效框架，通过视图特定神经元检测、验证和选择性微调机制，从神经元角度促进视图一致的放射学报告生成，仅更新视图特定神经元即可实现跨视图一致诊断并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成(RRG)方法在处理多视图X光图像时主要采用直接特征融合，忽略了单一模型处理不同视图时可能产生的临床不一致性和不准确性，影响性能和临床可靠性。

Method: View-PNDF包含三个核心模块：(i)视图特定神经元检测模块，识别对特定视图响应的神经元；(ii)验证模块，量化这些神经元的存在；(iii)选择性微调策略，强化检测到的神经元同时保留视图无关表示。通过仅更新视图特定神经元来实现一致的跨视图诊断。

Result: 在两个医学RRG基准上的广泛实验表明，View-PNDF显著提升了视图特定的胸部X光报告生成质量，同时保持了稳健的通用视图性能。

Conclusion: View-PNDF通过仅更新特定视图神经元，在减少计算成本的同时实现了跨视图的一致诊断，并通过LLM将视图特定报告整合为完整的放射学报告，在两个医学RRG基准上显著提升了视图特定的胸部X光报告生成质量，同时保持了稳健的通用视图性能。
...
TL;DR: TaxoMIL是一个将全切片图像(WSI)诊断重新定义为多粒度文本生成任务的框架，通过双头Transformer解码器生成分层诊断文本，并引入分类学引导目标来约束标签嵌入空间和视觉表示。


<details>
  <summary>Details</summary>
Motivation: 传统WSI诊断方法将任务表述为扁平分类，无法捕捉临床推理的层次性；而现有层次分类器和视觉语言模型(VLM)存在语义连续性缺失或产生分类幻觉和父子标签冲突的问题。

Method: 提出TaxoMIL框架：1)采用双头Transformer解码器生成粗粒度和细粒度诊断文本；2)引入分类学引导目标明确结构化标签嵌入空间；3)将切片级视觉表示严格约束在临床分类学内。

Result: 在三个不同的WSI数据集上，TaxoMIL始终优于最先进的MIL分类器和基于VLM的生成方法，实现了准确且层次感知的诊断预测。

Conclusion: TaxoMIL通过将WSI诊断任务转化为分层文本生成问题，有效解决了传统方法无法捕捉诊断层次性的问题，为计算病理学提供了更符合临床推理过程的诊断框架。
...
TL;DR: InfiniVerse是一个统一的管道，用于从单帧图像生成长范围、2D-3D对齐且可控的动态城市场景。该系统结合3D占用重建和视频扩散模型，实现自动场景扩展。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶领域，生成真实、可控且时间一致的城市场景是一个关键但尚未解决的挑战。现有方法难以实现长范围、2D-3D对齐和可控的城市场景合成。

Method: 首先从输入的多视角帧重建3D占用表示，以此为基础沿任意轨迹进行自回归场景扩展；然后使用视频扩散模型将粗粒度占用网格转换为真实、时空一致的视频序列；提出分层草图-细化范式，将生成视频重新投影为图像条件反馈以增强3D占用表示，建立视觉与空间域的跨模态对齐和相互增强。

Result: 在Waymo Open Dataset和nuScenes数据集上的广泛评估表明，InfiniVerse实现了最先进的性能，FID为6.4，FVD为67.97，在持续时间和稳定性方面显著优于现有基准。

Conclusion: InfiniVerse能够从单帧图像生成长范围、2D-3D对齐且可控的动态城市场景，通过分层草图-细化范式实现视觉和空间域的跨模态对齐，为自动驾驶仿真和训练提供了有效解决方案。
...
TL;DR: 提出利用神经雅可比场(NJFs)表示半刚性变形，通过求解泊松方程预测姿态相关的变形，解决单目视角下生成逼真人类avatar的挑战。


<details>
  <summary>Details</summary>
Motivation: 单目设置下生成具有复杂动作（如服装动态）的逼真人类avatar具有挑战性，需要建模全局和局部变形。单目输入存在自遮挡区域和不可见表面等问题。

Method: 1) 利用神经雅可比场表示半刚性变形；2) 通过自监督神经网络预测雅可比矩阵，求解泊松方程；3) 引入三个关键组件：约束泊松求解器、基于符号距离的雅可比正则化、以及变形引导的残差流损失。

Result: 实验表明该方法在基准测试和野生视频上生成时间稳定且几何一致的avatar，性能优于现有最先进方法。

Conclusion: 该方法成功解决了单目视角下avatar生成的边界伪影、频繁遮挡区域（如腋窝和大腿）的恢复、以及运动时的时间一致性等问题。
...
TL;DR: WildProp是一个无需训练的框架，可从大规模无约束图像库中直接估计野生动物身体比例分布，解决了传统形态测量需要受控成像或物理标本处理的可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: 传统生态学和进化研究中的种群级形态测量需要受控成像或物理标本处理，限制了可扩展性。研究旨在提出一种无需针对特定物种训练的通用形态估计方法。

Method: 将形态估计问题转化为检索驱动的对应问题：给定单张用户标注的规范图像，使用基础模型特征进行姿态感知检索，通过密集patch级匹配传输部件端点，使用几何一致性过滤预测，并聚合检索图像的测量结果以估计种群级比例分布。

Result: 在跨越鸟类和两栖类的三个大型形态测量数据集上评估，中位相对误差为10-20%。还展示了测量各种比例的广泛适用性，包括鸟类、青蛙、昆虫和花卉。

Conclusion: 通过仔细策划的2D对应关系结合网络规模图像，可以为跨分类群、地理和季节性的比较和亚组分析提供可扩展的形态测量代理。
...
TL;DR: SkillSpotter是一个姿势感知的多视角架构，用于在ego-exo视频中同时检测和评估熟练动作。它通过三个任务特定模块改进动作检测和分级性能，并将mAP从12.40提升到21.82，平衡准确率从55.99%提升到60.40%。


<details>
  <summary>Details</summary>
Motivation: 在体育、烹饪或音乐等领域实现个性化实时辅导需要AR眼镜或固定摄像头系统不仅理解用户做什么，还要理解用户执行活动的质量。现有的动作检测方法无法对动作进行正确/需要改进的分类评估。

Method: 1) 自适应时间抑制处理不同活动中熟练动作的不同密度；2) 门控3D身体姿态融合利用身体运动学作为视觉特征的补充信号；3) 双向跨视角注意力有效结合ego和exo视图。

Result: SkillSpotter将类别特定mAP从12.40提升到21.82（+76%），平衡准确率从55.99%提升到60.40%。其模块可迁移到其他时序动作检测模型并获得一致收益，并可推广到HoloAssist数据集。

Conclusion: SkillSpotter成功解决了ego-exo视频中动作检测和分级联合建模的难题，其提出的三个任务特定模块能有效提升性能且具有可迁移性和泛化能力。
...
TL;DR: LINet是一个用于RGB-D场景分类的多流神经网络，通过新颖的线性积分卷积(LIConv2d)算子实现每层的连续跨模态学习，解决了传统融合方法的结构妥协问题。


<details>
  <summary>Details</summary>
Motivation: 当前多模态架构将特征融合视为离散的临时事件：早期融合过早纠缠表示，晚期融合将它们隔离到最后一层，混合或注意力方法需要架构猜测来放置中间融合块。作者希望通过维护三个专用并行流(RGB、深度和集成)并实现连续跨模态学习来解决这种结构妥协。

Method: 1. 维持三个专用并行流(RGB、深度和集成流)；2. 开发了新颖的线性积分卷积(LIConv2d)算子，在每层实现连续跨模态学习；3. 使用1/N常数初始化解决Kaiming初始化导致的梯度破坏问题；4. 采用渐进式模态dropout(课程学习)防止路径崩溃。

Result: 在SUN RGB-D 19类场景分类上从头训练，LINet在ResNet18规模上达到45.2%的平均类准确率，优于之前的从头训练结果。使用域内RGB-D(ScanNet)预训练后提升至49.6%。

Conclusion: LINet通过连续融合架构、适当的初始化策略和渐进式dropout，成功实现了多流神经网络的连续跨模态学习，在RGB-D场景分类任务上取得了优异的性能。
...
TL;DR: FROST是一种无需训练的遥感少样本分割方法，将参考前景和背景视为DINOv3特征球体上的两个点云，通过非参数密度比进行标签预测，在17个遥感基准数据集上超越训练无关和方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中标注稀缺，且与自然图像差异大；现有方法要么绑定训练分布，要么使用点原型丢失类别内部结构，需要一种更优的类别表示方式。

Method: 将参考前景和背景视为冻结DINOv3特征单位球体上的两个点云，使用非参数密度比进行查询token标签分类，在等先验下贝叶斯规则将阈值设为零。

Result: 在17个遥感基准数据集上取得最优性能，单样本提示领先5.6 mIoU，且随着支持集增大性能提升，模型体积较小。

Conclusion: 类别应被建模为分布而非点，冻结自监督特征已携带足够结构直接估计分布密度，密度估计随样本量增加而精确，决策边界随之 sharpening。
...
TL;DR: WaterGen是一种在潜在扩散框架中解耦场景生成和介质建模的水下图像生成方法，可以生成大规模、逼真、多样化的水下图像，并对场景内容和水下介质效果进行独立控制。


<details>
  <summary>Details</summary>
Motivation: 水下计算机视觉任务受限于大规模和多样化训练数据的稀缺，现有方法要么可控性有限且逼真度不足，要么无法准确独立地建模水中介质效应。

Method: 将水下图像合成分解为两阶段：第一阶段微调潜在扩散U-Net，使用无退化的水下图像学习生成多样化的水下场景内容潜在嵌入；第二阶段将物理精确的介质退化合成表述为条件解码过程，应用于这些潜在嵌入。

Result: 利用WaterGen构建的大规模合成水下数据集，在场景结构多样化且水效果和伪标签准确方面表现优异，在水下修复和语义分割的下游任务中性能显著提升。

Conclusion: WaterGen通过解耦场景生成和介质建模，成功实现了多样化场景生成与准确可控水下外观的结合，合成数据可有效提升下游任务性能。
...
TL;DR: PruneGround是一个针对3D视觉定位(3DVG)任务的即插即用框架,通过语言引导的空间剪枝、多视图条件描述重述和LLM定位器三个关键组件,在减少计算成本的同时提高定位精度,在三个ScanRefer设置和9/10个Nr3D/Sr3D设置上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法对整个场景进行推理,导致在杂乱环境中产生歧义预测和高昂计算成本。作者观察到许多指称表达依赖于局部空间上下文,对应于受限空间区域而非完整场景,因此提出利用局部上下文来提高效率和准确性。

Method: 提出三个关键组件:(1)语言引导的空间剪枝(LGSP):利用冻结的视觉语言模型识别语言相关区域,缩小搜索空间;(2)多视图条件描述重述(MCDR):将复杂表达分解为简化的目标-锚点关系,通过多视图推理增强缺失的空间线索;(3)LLM-Grounder:将检测预训练的空间LLM重新定位为语言条件定位模型,在剪枝区域内对齐点云和语言表示。

Result: 在三个最流行的点云基准数据集上进行广泛实验,该方法在所有三个ScanRefer设置和Nr3D/Sr3D的10个设置中的9个上实现了最先进的性能。

Conclusion: PruneGround通过利用局部空间上下文和视觉语言模型,有效地解决了3D视觉定位中的计算效率和定位精度问题,为3DVG任务提供了一种高效且准确的即插即用框架。
...
TL;DR: 提出FAT框架，通过任务分解而非模型替换实现基础模型与专用模型的协作。该框架将结构化预测分解为专家预测、信息空间重建和基础模型代理推理三个阶段。ProxySelect在2D目标检测、3D目标检测、轨迹预测和语义分割四个任务上均优于专用基线模型。


<details>
  <summary>Details</summary>
Motivation: 基础模型直接承担结构化预测任务时存在能力不匹配问题，无法精确处理几何和数值估计，而专用模型在这些任务上通常表现更强。因此需要探索基础模型与专用模型的协作方式。

Method: 提出FAT框架，包含三个阶段：1）专家生成几何和物理有效的假设；2）信息空间重建多模态候选；3）基础模型执行代理任务（如选择或验证）。具体实现为ProxySelect，使用视觉-语言模型进行代理选择。

Result: ProxySelect在2D目标检测、3D目标检测、轨迹预测和语义分割四个任务上一致性地改进了专用基线模型，且计算成本低于直接使用基础模型进行回归。

Conclusion: 通用协作原则：专用模型保留任务特定结构，基础模型通过上下文代理推理完善假设。这种协作方式优于基础模型直接替换专用模型的方法。
...
TL;DR: 本文提出了一种针对自动驾驶VLA规划器的推理加速方法，通过将链式因果推理（CoC）分解为两个专业化路径（常规推理器和审议推理器），并引入FlatRoPE和Action-aware RL两项技术，实现了约4倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现代VLA规划器在生成轨迹前需要进行链式因果推理（CoC），该推理步骤是自回归的且主导推理延迟，而轨迹头部是并行且廉价的。延迟是自动驾驶的操作约束，因此加速推理步骤是核心问题。

Method: 1. 将CoC推理分解为两个专业化路径：常规推理器处理可预测的延续，审议推理器处理新颖情况；2. 提出FlatRoPE：一种1D旋转位置嵌入，打破目标3D M-RoPE的旋转对称性，将注意力从视觉token重定向到轨迹历史token；3. 提出Action-aware RL (AARL)：使用动作质量奖励和静态参考KL锚的后训练阶段。

Result: 相比原始Alpamayo规划器，推理步骤运行时间减少约4倍。

Conclusion: 通过专业化推理路径和定制化技术（FlatRoPE和AARL），成功实现了VLA规划器推理加速，为自动驾驶实时决策提供了有效解决方案。
...
TL;DR: 本文揭示了场景坐标回归(SCR)方法存在严重的隐私泄露风险。虽然SCR方法被认为具有隐私保护特性（因为场景信息隐式编码在神经网络参数中），但作者提出了一种基于查询的攻击方法，可以从训练好的SCR模型中重建训练环境的三维几何结构，并进一步通过特征反转合成图像，暴露场景的布局和敏感元素。


<details>
  <summary>Details</summary>
Motivation: SCR方法被广泛应用于视觉定位，现有文献普遍认为其具有隐私保护特性（场景信息存储在网络参数中而非显式地图）。然而，作者发现这一假设在实践中并不成立，需要验证并揭示这种潜在的安全风险。

Method: 提出了一种基于查询的攻击方法：1)使用与目标场景无关的代理图像批量查询模型，获取密集的像素级三维坐标；2)通过小输入扰动下的稳定性识别可靠点；3)在白盒设置下进一步优化这些稳定点；4)跨独立查询批次累积稳定点以恢复场景几何；5)反转网络特征从任意视角合成图像

Result: 在室内和室外数据集上的实验表明，可以高几何保真度重建训练环境的很大部分。除了几何信息外，还能恢复近似的颜色外观，暴露可识别的布局和潜在的敏感场景元素。

Conclusion: 这直接反驳了文献中关于SCR方法天然具有隐私保护性的说法，揭示了在私人或安全关键空间中部署此类系统时的真实风险。
...
TL;DR: S-OBI是一个用于评估多模态大语言模型(MLLMs)在句子层面理解甲骨文的新基准。该基准通过字形替换和合成方法，从95个原始拓片生成清晰的句子级甲骨文实例，并创建了695个问答对用于语义匹配、语义槽提取和上下文推理任务。实验表明，当前MLLMs在句子层面甲骨文理解上表现不佳，仍强烈依赖字符级识别。


<details>
  <summary>Details</summary>
Motivation: 现有AI辅助甲骨文视觉识别研究主要关注字符级，忽略了完整占卜记录中的长文本连贯性和上下文依赖性。多模态大语言模型的强大视觉感知能力为甲骨文信息处理提供了新可能性，需要一个专门评估句子级理解能力的基准。

Method: 1)基于95个经专家识别、校正和验证的原始拓片及其翻译 2)使用现有甲骨文数据集中的干净字形样本替换原始拓片中的字符 3)保留整体铭文结构和语义组织 4)设计语义匹配、语义槽提取和上下文推理三类任务 5)生成695个问答对进行评估

Result: 当代MLLMs在句子级甲骨文理解上表现较差。未屏蔽区域的视觉感知错误会通过推理链传播，导致对屏蔽字符的预测错误，表明当前模型的句子级理解仍强烈依赖字符级识别。

Conclusion: S-OBI提供了一个诊断基准，用于评估MLLMs能否从孤立字符识别走向结构化的铭文级理解。当前模型需要突破字符级识别瓶颈，建立更高层次的句子级和上下文理解能力。
...
TL;DR: 本文提出HSDF-Lane，通过将道路表面建模为高度对齐的有符号距离场(HSDF)，结合可微分渲染和车道感知语义位置编码(LSPE)，实现单目图像的3D车道检测和高度图估计的最优性能。


<details>
  <summary>Details</summary>
Motivation: 单目3D车道检测从单张图像恢复可靠的几何结构面临深度歧义挑战。现有方法假设平坦地面导致几何失真，或依赖稀疏锚点回归且仅将恢复的几何用于空间变换而忽视语义理解。

Method: 提出HSDF-Lane方法：1)将道路表面隐式建模为密集采样的3D特征体积上的高度对齐有符号距离场(HSDF)；2)通过可微分渲染联合生成精确的高度图和表面对齐特征；3)引入车道感知语义位置编码(LSPE)，将表面特征导出的车道存在先验注入Transformer查询，实现几何结构与语义引导的耦合。

Result: 在OpenLane基准数据集上，HSDF-Lane在3D车道检测和高度图估计两个任务上均达到最先进性能。

Conclusion: 通过HSDF建模和LSPE语义编码的有效结合，本方法成功克服了传统方法的几何失真和语义利用不足问题，为单目3D车道检测提供了新的解决方案。
...
TL;DR: 本文提出GaussianMap，一种在线高精地图构建框架，采用自适应高斯表示代替传统的固定分辨率BEV特征表示。该方法通过学习一组高斯基元来分配表示能力，能够更好地处理空间稀疏但需要精细几何定位的地图元素。实验在nuScenes和Argoverse 2数据集上验证了在纯视觉和视觉-激光雷达融合设置下均达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法普遍采用鸟瞰图(BEV)特征作为中间场景表示，使用固定分辨率的密集网格编码周围空间。然而，地图元素具有空间稀疏性且需要精细的几何定位特性，这使得均匀分配的BEV表示存在冗余且对矢量地图预测效果不佳。

Method: 提出自适应高斯表示方法，在BEV平面上学习一组高斯基元，每个高斯基元编码一个具有几何属性和特征向量的灵活局部区域。为从传感器观测生成这种表示，设计了一个前馈高斯编码器，通过高斯交互建模和多传感器特征聚合逐步精炼这些基元。精炼后的高斯表示被 splatting 到BEV特征图并解码为矢量地图预测。

Result: 在nuScenes和Argoverse 2数据集上的广泛实验表明，GaussianMap在纯视觉和相机-激光雷达融合两种设置下均实现了最优性能。

Conclusion: GaussianMap通过自适应高斯表示有效解决了传统BEV表示的冗余问题，能够更高效地分配表示能力到地图相关区域，为在线高精地图构建提供了一种新的有效范式。
...
TL;DR: UCF101-AD是一个用于评估多模态大语言模型（MLLMs）动作否定能力的大规模基准数据集。研究发现当前最先进的MLLMs在动作识别上准确率超过85%，但在动作否定任务上准确率低于50%，揭示了模型无法进行因果推理判断动作是否真正发生的关键缺陷。提出CausalAct方法通过因果图形式关联上下文、交互和运动，显著减少误报。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在视频理解任务上取得了显著进展，但它们在动作否定（denial）方面的能力几乎未被探索。现有模型倾向于肯定性地预测动作是否发生，而缺乏验证动作是否真正发生的因果推理能力。

Method: 1) 构建UCF101-AD数据集，包含成对的Action-Presence（动作存在）和Action-Denial（动作否定）视频 clip；2) 保留相同的上下文和运动线索（人物、物体、地点），仅移除定义性动作；3) 提出CausalAct方法，使用自然语言提示通过因果图表达场景结构，关联上下文、交互和运动。

Result: 评估20个最先进的MLLMs发现：在正向动作类别上准确率超过85%的模型，在动作否定任务上准确率降至50%以下，模型表现出强烈的肯定动作倾向而非验证动作是否真正发生。CausalAct方法的引入显著减少了误报。

Conclusion: 当前视频理解模型存在关键盲点：无法进行因果推理判断动作是否真正发生。动作否定是一种可学习的推理技能，因果线索的引入可以显著提升模型在这方面的能力。UCF101-AD为诊断和改进多模态模型的因果推理提供了新视角。
...
TL;DR: 本文提出了一种时间一致性学习框架，将时间相干性提取到2D网络中，在保持单帧推理效率的同时实现实时前列腺超声视频分割。通过利用前列腺的几何稳定性特征，设计了置信度加权时间一致性目标和双尺度原型对齐模块，并采用几何等变伪标签进行知识蒸馏。


<details>
  <summary>Details</summary>
Motivation: 传统2D方法因忽略时间上下文而存在帧间不一致问题，3D架构则产生过高延迟。前列腺在超声视频中具有几何稳定性，而周围声学环境因生理运动和探头压力而波动，这为设计高效的时间一致性方法提供了临床依据。

Method: 提出置信度加权时间一致性目标（基于光流扭曲残差）、双尺度原型对齐模块（对比优化局部边界和全局语义特征），以及基于几何等变的伪标签知识蒸馏策略。

Result: 在SUN-SEG和新增的TRUS-V基准数据集（2,679帧）上实现了最先进的准确性和时间一致性，且保持实时推理速度。

Conclusion: 该框架成功将时间相干性提取到轻量级2D网络中，在临床应用中具有重要价值，为超声引导介入治疗提供了有效的实时分割解决方案。
...
TL;DR: ExPLoRe提出了一种基于软专家混合(MoE)的patch级别损失路由方法,用于多目标掩码图像建模(MIM),通过损失耦合机制实现不同patch在不同目标上的内容依赖专业化。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用全局标量权重来平衡多目标MIM中的不同学习信号( token蒸馏、CLS对齐、像素重建),忽略了patch之间的空间异质性,即不同区域可能需要不同的目标侧重。

Method: 将软MoE的dispatch权重重新利用为可学习的每patch损失系数,允许损失梯度流经dispatch权重到达路由器,实现内容依赖的专业化。提出三种适配 recipes: Freeze Routing、Expert Dropout和Freeze Attention。

Result: 在ImageNet-1K上使用ViT-Base,相比非MoE基线在两种目标组合上均获得提升(Token+CLS: +0.5% k-NN, +4.4% linear probe; Token+Pixel: +2.2% k-NN),达到80.6% linear probe和85.3% finetuning准确率。在ADE20K上分割任务提升了+1.5%,弥补了2.5-2.9 mIoU的差距。

Conclusion: 损失耦合是核心机制(阻断梯度后性能下降1.6%),通过patch级别的损失路由可以有效捕捉空间异质性,MoE模型经适配后可匹配或超越非MoE基线。
...
TL;DR: AC3S是一种基于扩散的3D感知合成数据生成框架，通过自适应调节机制解决生成图像中精确3D结构和姿态一致性的挑战，同时保持照片级真实感。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用边缘图等视觉提示引导扩散模型，但常因过度条件化导致图像质量下降，限制了数据集的可用性。如何在保持生成表现力的同时实现精确的3D结构对齐是核心问题。

Method: 提出两大核心组件：1) 自监督视觉提示调制器，动态调整ControlNet条件化强度，防止过度条件化；2) 多智能体视觉语言模型框架，生成与底层几何结构对齐的详细3D感知提示。

Result: 实现可扩展的高质量合成数据集生成，具备准确的2D和3D注释，显著提升图像质量和下游任务效用。

Conclusion: 该方法在保持照片级真实感的同时有效解决了3D结构对齐问题，为计算机视觉数据扩展提供了有效方案。
...
TL;DR: AA是一个用于屏幕注视估计的多视角多模态数据集，包含8个屏幕固定摄像头和2个侧视摄像头捕获的同步面部观测数据，配有精确的屏幕空间注视目标。


<details>
  <summary>Details</summary>
Motivation: 现有单视角注视估计数据集无法有效处理视角变化和遮挡问题，缺乏多视角覆盖以支持更鲁棒的模型训练。

Method: 使用8个屏幕固定摄像头和2个侧视摄像头同步捕获面部观测，配合受控固定条件下的精确屏幕空间注视目标，包含多视角面部观测和结构化面部区域crop。

Result: 提供多视角覆盖（屏幕安装和侧视视角），支持在视角变化和遮挡条件下进行更鲁棒的建模，包含主体无关评估分割和标准化数据处理流程。

Conclusion: 该数据集支持可重现的注视估计研究，为多视角多模态 gaze estimation提供了标准化基准。
...
TL;DR: CooperScene是一个高保真协作自动驾驶数据集，包含真实世界的C-V2X通信特征。该数据集涵盖交叉口、高速公路匝道和停车场等场景，包含3辆联网自动驾驶车辆(CAV)和1个基础设施路侧单元(RSU)，配备多模态传感器和商用C-V2X通信无线电。数据集包含344K个3D标注对象横跨59K帧，频率为10Hz。


<details>
  <summary>Details</summary>
Motivation: 现有数据集往往忽视现实世界部署的复杂性，如有限的通信带宽及其动态变化、异构传感模式以及超越单个合作伙伴的可扩展性。需要一个高保真数据集来评估多智能体扩展和实际部署性能。

Method: 数据集包含3辆CAV和1个RSU，全部配备多模态传感器和商用C-V2X通信无线电。实现了紧密的传感器和智能体同步、厘米级定位和空间对齐、精确的跨模态校准，以及符合3GPP标准的C-V2X通信。

Result: 构建了包含344K个3D标注对象横跨59K帧的数据集，标注频率为10Hz，涵盖交叉口、高速公路匝道和停车场等多样化场景。

Conclusion: CooperScene为评估多智能体扩展和真实世界可部署设置中的实际性能建立了严格基准，为协作感知、预测和规划研究提供了重要数据支持。
...
TL;DR: ForgeDrive是一个统一的自回归扩散框架，采用"先行动后想象"范式，将未来分解为每时间步的帧-动作对，实现视觉-动作交叉条件化，在NAVSIM上无需后训练策略即超越现有强规划器。


<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的自动驾驶采用"先想象后行动"范式，视觉生成阶段的误差会级联到动作规划中，阻碍了模型理解场景演化的能力。

Method: 1)将未来分解为每时间步的帧-动作对，实现视觉与动作的交织；2)训练时解耦两模态的扩散时间步，采用UniDiffuser风格的噪声调度器；3)推理时提出"先行动后想象"范式，每步生成的动作可提升下一帧生成质量，反之亦然；4)每步增强未来自我状态预测。

Result: ForgeDrive将驾驶模拟、规划 和视觉里程计统一于单一模型，在NAVSIM上无需任何后训练策略即超越现有强规划器。

Conclusion: 通过"先行动后想象"范式和视觉-动作交叉条件化，ForgeDrive成功闭合了视觉生成与动作规划之间的误差级联问题，为自动驾驶世界模型提供了一种更有效的统一框架。
...
TL;DR: 本文提出UHD-MFF数据集和UMF-LUT框架，解决超高清多焦点图像融合中的数据可用性、模型适应性和部署可行性三大障碍，实现实时4K图像融合。


<details>
  <summary>Details</summary>
Motivation: 现有超分辨率多焦点图像融合方法面临三大障碍：数据可用性（缺乏大规模超高清数据集）、模型适应性（现有方法无法有效处理4K及以上分辨率）和部署可行性（计算开销大难以实时应用）

Method: 提出UHD-MFF数据集（首个大规模超高清多焦点融合数据集）和UMF-LUT框架（包含C-LUT粗粒度查找表和D-LUT细节边缘查找表）。C-LUT在低分辨率尺度进行多梯度线索和语义线索的联合查询实现区域级决策；D-LUT在高分辨率尺度利用拉普拉斯线索提供边缘级决策信息

Result: 在视觉质量和定量指标上优于SOTA方法，具有低计算开销，可实现实时4K多焦点融合，在智能手机上也有良好应用前景

Conclusion: 有效推动多焦点图像融合向超高清成像场景发展，代码已开源
...
TL;DR: 本文提出了一种双曲（双曲空间）手术视频-语言预训练框架，用于解决现有方法将手术教育材料中的多层次知识（动作、步骤、阶段）简单压缩到单一平面嵌入空间的问题。该框架通过缓解程序上下文引起的结构假负样本并强制父阶段与子步骤之间的语义一致性来显式保留层次结构。实验表明在多个手术基准上实现了显著的零样本和少样本阶段识别性能提升。


<details>
  <summary>Details</summary>
Motivation: 手术教育材料包含多维度层次化知识：细粒度动作线索出现在旁白中、中层关键步骤总结在小节标题中、全局程序上下文（如患者病史和手术策略）描述在摘要文本中。然而，现有工作将这些异构信号折叠到单一平面嵌入空间中，隐式假设各层次独立，这忽略了跨层次语义包含关系（如动作属于步骤、步骤组成阶段），削弱了长程依赖建模能力。

Method: 提出双曲手术视频-语言预训练框架。核心思路：1) 利用双曲空间更好地建模层次化结构；2) 缓解由程序上下文引起的结构假负样本问题；3) 强制执行父阶段与其子步骤之间的语义一致性。模型显式保留层次结构而非简单平面化。

Result: 在多个手术基准数据集上进行了广泛实验，跨手术类型和机构实现了零样本和少样本阶段识别的一致性能提升。

Conclusion: 该工作证明了显式建模手术知识的层次结构对于视频-语言预训练的重要性。双曲空间能够有效捕捉层次化语义关系，为手术视觉-语言基础模型的预训练提供了一种新的范式，在迁移学习和少样本场景下具有显著优势。
...
TL;DR: 本文提出了XInsight Lab团队在4th EI-MIGA-IJCAI Challenge Track 3中夺冠的解决方案，通过紧凑的多模态时序建模框架，结合2D/3D骨骼、面部Blendshapes、DINOv2/v3、X-CLIP和Gemini等多源特征，实现0.76923的测试准确率，并揭示了通用视觉基础模型在微动态任务中的表征崩溃现象。


<details>
  <summary>Details</summary>
Motivation: 解决长视频中隐含情感证据微弱且稀疏的难题，提升微动态情感识别性能

Method: 提出紧凑多模态时序建模框架：(1)跨注意力机制-静态姿态(Base)作为Query，动态微运动差分特征(Offset)作为Key/Value；(2)基于多实例学习的自适应池化方法提取瞬时情绪并抑制背景噪声

Result: 测试准确率达0.76923，夺得Track 3冠军

Conclusion: 验证了多源特征融合的有效性，揭示了通用视觉基础模型在微动态任务中的表征崩溃现象和 shortcut learning、rot memorization 导致的伪泛化问题
...
TL;DR: 线性探针和 steering recovery 方法可能系统性地高估视觉语言模型（VLM）在图像中实际 grounded 的信息。通过用灰色空白图像替换原图这一简单控制，可以揭示三种 grounding 机制：grounded（视觉依赖、正确）、prior（视觉独立、使用默认方向而非感知）、inverted（可解码但部署符号错误，低于随机准确率）。在 14 个模型（6 个家族，2B-27B）中，水平方向是 grounded，垂直方向是 prior，深度是 inverted。7/8 模型出现 inversion。不同模型需要不同复杂度的校正方法。空白图像控制应成为 VLM 潜伏知识研究的标准对照。


<details>
  <summary>Details</summary>
Motivation: 解决线性探针和 steering recovery 方法可能系统性地高估 VLM 在图像中实际 grounded 信息的问题，特别是在空间推理任务中

Method: 使用因果控制方法：用灰色空白图像替换原图，测试模型预测是否依赖视觉输入；分析 14 个跨 6 个语言模型家族的 VLM

Result: 识别出三种 grounding 机制：grounded（视觉依赖正确）、prior（视觉独立使用默认方向）、inverted（解码但部署符号错误，低于随机）；水平 grounded，垂直 prior，深度 inverted；14 个模型中 inversion 在 7/8 个模型中复现；不同模型需要不同复杂度的校正方法

Conclusion: 空白图像控制方法能清晰区分 grounded 感知、inverted 感知和 prior 替换，应作为 VLM 潜伏知识和 steering 声明的标准对照方法
...
TL;DR: 提出失败驱动的自我改进范式，利用LLM诊断失败模式并生成代码补丁，在OSWorld上将OpenCUA-72B成功率从42.3%提升至48.9%。


<details>
  <summary>Details</summary>
Motivation: 当前标准方法仅利用成功轨迹，丢弃失败轨迹，但失败中蕴含丰富的模型弱点信息，需要一种以数据为中心的范式来利用失败轨迹改进智能体。

Method: 使用LLM诊断失败模式，提出推理时解决方案，并生成代码补丁，通过人工轻度验证后升级智能体。在OSWorld基准测试上验证了OpenCUA-72B模型。

Result: 在OSWorld基准测试上，成功率从42.3%提升至48.9%，提升6.6个百分点，无需额外训练成本，仅增加适度推理开销。

Conclusion: 失败驱动的自我改进可以作为成功驱动管道的可行补充，实现更高效的智能体改进，且无需额外训练成本，仅需适度的推理开销。
...
TL;DR: CLIMB是一种在线持续自监督学习方法，结合层次质心内存库和知识蒸馏，在不知道任务边界且有内存限制的情况下从连续无标签数据流中学习表示。


<details>
  <summary>Details</summary>
Motivation: 现有OCSSL方法要么依赖重放缓冲区利用潜在空间结构，要么仅依靠正则化，缺乏同时结合两种优势的方法。

Method: 提出CLIMB方法：1) 引入层次质心式内存，存储图像总数有界；2) 将相似图像分组到质心，为对比学习提供难以区分的样本，同时覆盖观察分布的多样性；3) 结合知识蒸馏对重放样本进行训练以限制表示漂移。

Result: 在Split CIFAR-100、Split ImageNet-100以及新的不规则任务分布协议上实验，表明CLIMB优于当前最先进的OCSSL方法。

Conclusion: CLIMB成功结合了基于重放的方法和基于正则化的方法的优势，在持续自监督学习任务上取得了最先进的性能。
...
TL;DR: 提出MICE训练无关策略，通过调节联合注意力偏置实现多实例并发编辑，有效解决语义干扰和属性泄漏问题，在新基准MICE-Bench上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 单次前向传递中编辑图像多个元素相比多轮操作更高效且有助于更好地融合。然而当多条指令 targeting 不同区域时，语义干扰导致属性泄漏和编辑解耦差，且随编辑数量增加问题加剧。

Method: 提出MICE策略，通过修改 Multimodal Diffusion Transformers 的联合注意力加性偏置来调节实例特定编辑指令、潜变量和上下文标记之间的交互。具体包括：1)通过用户提供的分割掩码识别不同实例；2)允许实例内注意力；3)惩罚相邻区域标记之间的交互；4)抑制不相关的跨实例注意力。从而实现属性绑定并保持全局视觉一致性。

Result: 在LoMOE-Bench和新建的MICE-Bench（平均每图8.5个并发编辑）上进行评估，实验表明该方法在视觉质量保持和指令忠实度方面均优于强基线和最新竞争对手。

Conclusion: MICE通过修改联合注意力的加性偏置，有效解决了多实例并发编辑中的语义干扰问题。实验表明该方法在视觉质量保持和指令忠实度方面均优于现有最强基线，为大规模多实例图像编辑提供了一种有效的训练无关方案。
...
TL;DR: ToothForge是一个深度光谱生成框架，用于从紧凑的内在表示中建模牙冠几何形状。通过在光谱域中操作，利用同步光谱嵌入学习3D牙齿形状的潜在流形，能够在连接性不同的样本中保持一致建模。该框架在有限数据集情况下表现出色，适用于牙科和医学领域的实际应用场景。


<details>
  <summary>Details</summary>
Motivation: 牙冠形态的精确建模对诊断、正畸计划和计算机辅助修复设计至关重要，但训练模型的数据集通常规模有限。现有的空间方法在处理来自不同诊所、连接性不一致的牙齿形状时存在不稳定问题。

Method: 提出深度光谱生成框架ToothForge，通过同步光谱嵌入在光谱域中学习3D牙齿形状的潜在流形。光谱同步减轻了拉普拉斯-贝尔特拉米特征基的不稳定性，并在低维空间中进行高效学习。

Result: 同步光谱建模实现了与空间方法相当或更好的重建和生成性能，同时保持了紧凑性和几何可解释性。该框架在鲁棒性分析、消融实验和基准测试中表现优异。

Conclusion: 同步光谱系数紧凑且学习空间低维，使该框架特别适合有限数据集场景，如牙科和医学领域，并适用于无法保证来自不同诊所的形状连接性一致的真实世界应用。
...
TL;DR: 本文提出了一种基于小波优化伪3D加速扩散模型的计算层析成像(CL)截断重建方法CL-DM，通过将重建目标扩展到数据不完整区域，有效扩展了有效成像范围并消除了截断伪影。


<details>
  <summary>Details</summary>
Motivation: 计算层析成像(CL)是大尺寸板状物体无损检测的关键技术，但视场(FOV)限制导致投影数据截断，造成严重的重建伪影。现有深度学习方法缺乏严格的数据一致性约束，且仅在FOV内进行伪影去除，丢弃了可能恢复的FOV外信息。

Method: 1)提出综合CL FOV分析，将空间分为数据完整、数据不完整和数据自由区域；2)将重建目标扩展到数据不完整区域；3)采用标准2D扩散模型进行切片聚合，结合3D基于模型的迭代重建(MBIR)确保数据一致性；4)引入z方向小波正则化，配合平移不变(TI)机制和低频保留策略减少切片间不连续性；5)采用3D快速采样架构加速推理速度。

Result: 仿真和真实实验表明，CL-DM在消除截断伪影和恢复高保真连续3D结构方面表现优越。

Conclusion: CL-DM方法有效扩展了计算层析成像的有效成像范围，提高了扫描效率，并能够重建高保真、连续的3D结构，为大尺寸物体无损检测提供了新的解决方案。
...
TL;DR: 本文提出Accelerated Likelihood Maximization (ALM)，一种训练-free的采样策略，用于扩展扩散模型在部分输入情况下的内容生成能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法的局限性：训练-based方法计算成本高且泛化性差；训练-free方法虽高效但未显式优化未观察变量，导致全局不一致。

Method: 将ALM集成到反向扩散过程中，直接在采样过程中优化未观察区域，并引入加速策略提高计算效率。

Result: 在各种数据域和任务中一致优于现有最先进方法。

Conclusion: 为多功能内容生成建立了强大的范式。
...
TL;DR: Vega是一个统一的视频理解与生成框架，通过结合自回归预测和扩散渲染来弥合两种任务之间的差距。该框架使用共享词汇表共同建模文本和视觉表示，在VBench和VideoMME基准上展现出强大的性能。


<details>
  <summary>Details</summary>
Motivation: 当前统一图像生成与理解已得到广泛探索，但将统一建模范式扩展到视频领域仍存在挑战。核心问题在于：视频理解需要紧凑的判别性语义表示，而视频生成需要保留视觉细节和时间连贯性的密集信号。

Method: Vega采用混合架构，结合自回归(AR)预测与扩散渲染。AR模型专注于预测关键帧的语义有意义的视觉标记，提供结构化表示来指导扩散模块渲染密集的高分辨率视频帧。框架使用共享词汇表共同建模文本和视觉表示。

Result: 在视频生成基准VBench和视频理解基准VideoMME上展现出强大的性能。

Conclusion: Vega成功实现了视频理解与生成的统一建模，证明了视频作为统一多模态建模的合适模态的潜力，为该领域提供了新的研究方向。
...
TL;DR: 本研究评估了学习预处理管道在MURA肘部X光片骨科异常检测任务中的价值，发现预处理增益 modest 且配置依赖，原始输入的DenseNet121基线保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 研究学习预处理管道（如DnCNN模块）对医学影像异常检测的影响，特别是在患者水平评估和泄露感知协议下，评估其对区分能力和校准的提升。

Method: 比较多种预处理管道（包括带/不带DnCNN模块），使用DenseNet121架构，在MURA数据集上进行患者水平异常检测。评估指标包括AUROC、PR-AUC、ECE、Brier分数及高特异性操作点分析。

Result: 预处理策略差异 modest 且配置依赖。原始输入+DnCNN组合降低ECE和Brier分数，但CLAHE+DnCNN未改善校准。无一预处理策略在所有指标上产生一致的区分优势。

Conclusion: 患者水平评估下，预处理增益 modest 且配置依赖；原始输入DenseNet121基线保持竞争力；无预处理策略在所有指标上获得一致优势。
...
TL;DR: RCL-Mamba是一种面向测量的双域状态空间模型图像恢复网络，用于解决旋转扫描计算层析成像(RCL)中连续稀疏视角扫描导致的旋转模糊和稀疏采样导致的稀疏伪影问题。


<details>
  <summary>Details</summary>
Motivation: 为实现大规模平面元件的快速检测，RCL采用连续稀疏视角扫描，但这会在投影域产生旋转模糊，并在图像域产生稀疏伪影，需要跨域修复。

Method: 采用级联联合处理策略：先在投影域校正旋转模糊，再在图像域抑制稀疏伪影；设计Mamba-CNN双分支模块自适应平衡大规模模糊校正与局部细节恢复。

Result: 在模拟数据集和真实PCB扫描中均优于现有基线方法，有效去除模糊、抑制伪影、保护结构。基于线轮廓的结构测量验证了该方法更好地保留了通孔/焊盘边界和细小走线轮廓。

Conclusion: 将扫描视角从512个减少到64个，检测效率提升约8倍而不影响重建质量，为高吞吐量RCL检测提供了稳健的面向测量的恢复解决方案。
...
TL;DR: 本文提出LA-SR框架，利用同一图像中不同深度区域的分辨率差异提取真实LR patches，并创新性地在语言空间中利用视觉-语言模型进行无配对超分辨率训练，解决传统方法依赖合成数据导致的泛化差问题。


<details>
  <summary>Details</summary>
Motivation: 传统SR方法需要成对的HR-LR训练数据，但实际中难以获取。现有的合成LR方法（人工核或相机ISP调整）无法捕捉真实LR图像的复杂性，导致在真实场景中泛化性能差。

Method: 1)利用单张图像中不同深度区域分辨率不同的特点，提取真实LR patches；2)将图像投影到包含内容和质量的语义丰富空间；3)设计两种语言引导损失：语言内容损失（保持语义保真度）和语言质量损失（增强感知真实感）；4)在语言空间中桥接LR-HR差距实现无配对SR。

Result: LA-SR能够有效超分辨率真实LR输入，生成逼真的输出，克服了合成数据训练方法的局限性。

Conclusion: 通过在语言空间重新定义无配对SR问题，LA-SR为真实场景下的超分辨率任务提供了新思路，视觉-语言模型的引入有效解决了无配对训练数据的问题。
...
TL;DR: 本文将Cycle self-training (CST)从图像分类扩展到目标检测领域，提出了Dual-Stream Bilevel-Cycle Optimization框架。该框架基于Mean Teacher构建CST以防止训练不稳定，并使用额外归一化将回归边界框映射到标准化空间，解决了损失爆炸问题。


<details>
  <summary>Details</summary>
Motivation: 标准自训练框架的共享分类器假设限制了无监督域适应的效果。CST虽在图像分类中成功，但扩展到目标检测面临三个主要挑战：1) 三种不可靠伪标签（仅分类错误、仅定位错误及组合）限制上限；2) 目标检测涉及多目标导致训练不稳定；3) 回归坐标数值范围更广导致损失爆炸。

Method: 提出Dual-Stream Bilevel-Cycle Optimization框架：1) 在Mean Teacher上构建CST以防止训练不稳定；2) 使用额外归一化将回归边界框映射到标准化空间解决损失爆炸问题；3) 对分类和回归同时应用CST；4) 提供了回归边界的理论推导。

Result: 在四个跨域标准场景下进行了广泛实验，证明了该框架达到了相当好的效果。

Conclusion: 成功将CST扩展到目标检测任务，通过Dual-Stream Bilevel-Cycle Optimization框架有效解决了训练不稳定和损失爆炸的问题，在跨域目标检测任务中取得了显著效果。
...
TL;DR: 本文提出MAPE防御方法，通过结合SAPE通道注意力U-Net和PPSA预训练模型调度算法，有效消除可迁移对抗扰动，在CIFAR-10和Mini-ImageNet上分别达到95.1%和71.5%的防御率。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络容易受到对抗样本攻击，可迁移对抗攻击因不依赖目标模型信息而具有高隐蔽性和检测难度，需要有效的防御手段来应对多样化的可迁移攻击。

Method: MAPE包含两个核心组件：1) SAPE使用通道注意力U-Net作为防御模型，利用预训练模型生成的对抗样本进行训练以消除已知对抗扰动；2) PPSA引入模型差异量化和负动量来调度多个预训练模型，最大化对抗样本间的差异以增强防御模型的鲁棒性。

Result: 在以ResNet-34为目标模型的黑盒攻击场景中，MAPE在CIFAR-10上达到平均95.1%的防御率，在Mini-ImageNet上达到71.5%的防御率，表现出最先进的性能。

Conclusion: MAPE能有效消除各种对抗样本中的对抗扰动，为来自不同替代模型的攻击提供鲁棒防御，是对抗可迁移攻击的有效深度学习防御方法。
...
TL;DR: MS-Resampler是一种多尺度视觉重采样框架，用于多模态大语言模型(MLLMs)。该方法通过注入明确的空间尺度先验来实例化多个尺度特定的重采样器，使每个分支能在不同粒度（从局部到全局）聚合视觉信息，并自适应融合输出。


<details>
  <summary>Details</summary>
Motivation: 现有的重采样器采用单一的固定聚合范围（全局交叉注意力），这会模糊细粒度的局部证据，并在有限的token预算下限制了对局部细节和全局上下文的捕获能力。

Method: MS-Resampler通过在重采样注意力中注入明确的空间尺度先验来实例化多个尺度特定的重采样器，使每个分支能在特定粒度上聚合视觉信息（从局部到全局），然后自适应融合这些尺度特定重采样器的输出以产生最终视觉表示。

Result: 在10个公开多模态基准数据集上的广泛实验表明，MS-Resampler在引入最小计算开销的同时，一致性地改善了视觉理解和多模态推理性能，优于传统的单尺度重采样器。

Conclusion: 多尺度方法有效同时捕获局部细节和全局上下文，在最小开销下提供了改进的视觉理解能力。
...
TL;DR: OVOW是首个无需训练的、从单目视频重建实例级仿真就绪4D网格场景的系统。该系统解决了现有4D重建方法输出（隐式场、高斯原语、点云）缺乏水密拓扑、实例分离和标准化物理接口的问题，可直接用于物理仿真和具身AI。


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法虽然渲染质量出色，但其输出缺乏水密拓扑、实例分离和标准化物理接口，无法满足物理仿真器和具身AI的需求。需要一种能够生成仿真就绪4D网格场景的方法。

Method: 四阶段流水线：(1)视觉语言模型发现、标记和运动分类所有实例；(2)类别感知重建，刚性物体生成实例网格，可变形物体生成拓扑一致的网格序列；(3)迭代渲染-匹配-优化恢复度量尺度和6-DoF姿态轨迹；(4)物理接地组装确保地面接触和物体间支撑。所有运动通过直接顶点变形建模，无需类别特定先验或骨骼绑定。

Result: 在两个合成基准（静态和4D）上取得最佳整体布局和几何精度， photometric和语义误差最低；单目视频运行速度比基线方法快1-2个数量级；下游物理仿真验证了其物理稳定性。

Conclusion: OVOW首次实现了从单目视频无需训练重建仿真就绪4D网格场景，建立了首个结构化视频到4D评估基准，可作为合成配对视频到4D仿真数据的可扩展引擎，为未来4D世界模型和具身AI提供数据支持。
...
TL;DR: 本文提出了Visual Semantic Entropy (VSE)方法，用于估计视觉语言模型在视觉模糊输入下的不确定性。该方法通过对图像进行扰动而保持文本查询不变，探测视觉变化产生的答案多样性，并通过将生成答案聚类为语义原型来计算质量加权分散度。


<details>
  <summary>Details</summary>
Motivation: 现有基于熵的不确定性估计方法（如语义熵SE）依赖输出多样性，但过度自信的视觉嵌入在随机解码时会抑制输出多样性，导致SE低估不确定性。而基于输入扰动的方法虽然能探测输出多样性，但其产生的变异性主要来自文本变化而非视觉证据，反映的是提示敏感性而非视觉模糊性。

Method: VSE方法仅对图像进行扰动以探测邻近的视觉变化，同时保持文本查询固定。它将生成的答案聚类为语义原型，并计算它们之间的质量加权分散度来衡量不确定性。

Result: 在5个现代视觉语言模型和5个不同的VQA基准数据集上的广泛评估表明，VSE能够有效捕获视觉模糊性，建立了VLM不确定性估计的新技术状态。

Conclusion: VSE通过仅扰动图像而非文本，成功解决了现有方法的问题，能够准确反映视觉 ambiguity 而非提示敏感性，为视觉语言模型的不确定性估计提供了更可靠的解决方案。
...
TL;DR: 论文提出TemporalLens诊断框架和YOLO-3D架构，用于评估和改进单阶段视频目标检测器的时间推理能力。标准指标无法区分是否真正利用了时间上下文，论文通过5种扰动策略揭示模型真实的时间依赖行为。


<details>
  <summary>Details</summary>
Motivation: 标准mAP指标无法区分检测器是真正利用时间上下文还是仅依赖单帧信息，时间关键应用中这一盲点可能导致模型在实际部署时性能崩溃。

Method: 提出TemporalLens诊断框架，包含5种可控扰动：受控遮挡、时间打乱、冗余注入、分辨率降级以及目标帧移除。针对架构方面，设计了基于YOLOv8的模块化实时时空检测器YOLO-3D。

Result: TemporalLens暴露了堆叠2D模型与时空模型的本质差异：移除目标帧后堆叠2D模型崩溃，而时空模型可从更早帧恢复预测。YOLO-3D通过保持时间深度在主干网络中获得+3.7 pp的mAP@50提升（32帧平均）。

Conclusion: TemporalLens将"检测器是否真正利用时间"转化为可测量、可操作的问题，同时证明保持时间深度是提升性能的主要驱动力。
...
TL;DR: 本文提出了一种无提示词的扩散图像隐写框架，通过整合风格语义先验来控制隐写图像的生成，并引入预测-校正机制迭代优化生成轨迹，在安全性、秘密图像重建精度和可控性方面达到了先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于潜在扩散模型(LDM)的图像隐写方法受限于文本提示的灵活性，难以生成高质量的隐写图像并准确恢复秘密图像。

Method: 提出级联仿射耦合模块(CACM)在秘密图像及其潜在表示之间建立双射确定性映射；将风格语义整合到扩散过程中控制潜在表示；引入预测-校正机制通过当前状态和预测下一状态的反馈迭代优化生成轨迹。

Result: 实验表明，该方法在安全性、秘密图像重建准确性和可控性方面与现有最先进方法相比具有竞争力。

Conclusion: 无提示词的扩散图像隐写框架结合风格语义先验和预测-校正机制能够有效提升隐写图像质量和秘密图像恢复精度，为生成式图像隐写提供了新的思路。
...
TL;DR: 研究动态对比增强心脏CT中左心房和左心耳的分割问题，探讨不同时间帧训练策略对nnUNet分割性能的影响。


<details>
  <summary>Details</summary>
Motivation: 在动态心脏CT中，准确分割左心房和左心耳具有挑战性，因为对比度随时间变化大，且每个注册序列仅使用单一标注，导致训练鲁棒性与标签噪声之间存在权衡。

Method: 比较三种训练策略：1) 最少两帧数据集（标准临床实践）；2) 生理学选择的帧子集；3) 完整27帧序列。并评估前景为基础的归一化方法的影响。

Result: 使用所有帧训练在早期低对比度阶段表现最佳，但从填充阶段开始，生理学选择的子集达到相当性能。归一化参数可改善减少数据集在低对比度帧的性能，但未能完全弥补差距。

Conclusion: 时间多样性对动态CT中鲁棒分割至关重要，但精心选择的帧子集可为下游应用提供性能与效率的有效权衡。
...
TL;DR: 本文针对格里高利圣咏光学识别的四个数据集编码不一致的问题，设计了一种基于S-GABC提案的通用编码方案，将四个数据集统一转换，并训练了一个共享的端到端基础模型，在所有四个数据集上实现了新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 格里高利圣咏的光学识别近年来开始采用端到端方法，但已发布的四个数据集各自使用不同的编码方案，导致难以统一训练和比较。缺乏一个通用的编码标准限制了端到端方法在该领域的发展和性能提升。

Method: 1. 设计了一种基于S-GABC提案的通用编码方案；2. 将四个现有的格里高利圣咏数据集统一转换为该通用编码；3. 基于转换后的数据训练了一个共享的端到端基础模型。

Result: 该共享端到端基础模型在所有四个数据集上均建立了新的最优性能。

Conclusion: 统一的编码方案和共享的端到端模型能够有效提升格里高利圣咏光学识别的准确率，为该领域提供了新的基线和方法论参考。
...
TL;DR: 本文提出SatAgent，一个受人类视觉系统双路径机制启发的无人机-卫星协作空间推理模型，通过联合利用卫星和无人机视角实现复杂城市环境中的鲁棒准确推理。


<details>
  <summary>Details</summary>
Motivation: 现有无人机空间推理方法存在关键局限性：单视角感知使其容易受到遮挡和视角扭曲的影响；大多数视觉语言模型缺乏明确的几何建模，仅依赖语义线索，在视角和尺度变化下产生不一致的推理结果。

Method: 提出三大核心创新：1）几何感知3D重建编码器，将2D无人机特征提升为显式3D空间表示；2）多视角拓扑-语义对齐模块，在统一BEV坐标系统中整合跨视角特征；3）多视角一致性损失，鼓励视角不变表示。构建了首个大规模无人机-卫星协作多视角空间推理数据集SatAgent-SR130K。

Result: 实验表明，SatAgent在多样化任务中表现优于最先进的通用基础模型25.91%，优于专门的空间推理模型11.69%，在复杂几何关系推理中实现特别高的准确性。

Conclusion: SatAgent通过无人机-卫星协作和几何感知建模，有效解决了传统单视角空间推理的局限性，为无人机在复杂城市环境中的自主感知与推理提供了新的解决方案。
...
TL;DR: 提出ThinkGraph框架，采用异步架构将轻量级在线映射与重量级VLM语义细化解耦，实现探索过程中的可查询3D场景图构建，在视觉接地任务上实现15.3-18.8个百分点的显著提升。


<details>
  <summary>Details</summary>
Motivation: 传统开放词汇3D场景图方法采用两阶段流程（先重建再用视觉语言模型丰富），导致图谱在探索过程中不可查询。论文认为这种顺序耦合并非必要，因此提出异步架构来解决这一问题。

Method: 提出异步架构，轻量级在线映射与重量级语义细化并发运行。核心包括：1）基于概率体素的骨干网络，递增维护稳定的目标身份；2）后台VLM代理逐步丰富图谱；3）通过语义闭环解决重复目标跟踪；4）附加细粒度视觉属性并推导目标间空间关系；5）多目标帧调度器选择一小部分信息帧联合覆盖多个目标以分摊VLM成本。

Result: 在ScanNet和Replica语义分割任务上与现有方法相当或更优；在Sr3D+、Nr3D、ScanRefer三个视觉接地基准上超越先前最先进方法15.3至18.8 A@0.25。

Conclusion: 该方法在语义分割任务上与现有开放词汇3D场景图方法相当，并在Sr3D+、Nr3D和ScanRefer三个视觉接地基准上超越先前最先进方法15.3至18.8个百分点（A@0.25）。该框架实现了探索过程中的可查询性，并随着时间推移语义丰富度不断提升。
...
TL;DR: 本文提出DrivingDepth方法，通过将稀疏LiDAR作为几何提示，对冻结的深度基础模型进行残差像素级尺度校正，解决自动驾驶中密集深度估计的几何尺度冲突问题，在nuScenes数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的密集深度估计面临几何尺度冲突：深度基础模型提供像素对齐的密集视觉几何但缺乏可靠的度量尺度，而投影LiDAR提供度量锚点但稀疏、噪声大且与图像结构不对齐。现有稀疏提示方法从头重建深度，破坏了基础模型的几何一致性，在视觉连续表面上产生结构伪影。

Method: 提出DrivingDepth方法，核心洞见是基础模型已捕获几何一致的相对深度，无需额外学习表面结构，只需学习将相对几何映射到度量坐标的每像素尺度因子。该方法将稀疏LiDAR视为几何提示，通过残差像素级尺度校正对冻结的基础模型进行局部校准，从根本上保留密集视觉几何。

Result: 在nuScenes上使用4帧环视输入进行测试，DrivingDepth实现AbsRel为11.19，EdgeCR为5.741，同时在度量精度和几何一致性方面优于MapAnything（11.99/1.914）。

Conclusion: 通过将稀疏LiDAR作为几何提示进行残差像素级尺度校正，DrivingDepth成功解决了密集深度估计中的几何尺度冲突问题，同时实现了SOTA度量精度和几何一致性，为自动驾驶感知提供了更可靠的深度估计方案。
...
TL;DR: 本文提出HVPNet，一个受人类视觉系统启发的简单高效的生物启发式计算架构，用于多模态显著目标检测和伪装目标检测。该网络通过视网膜集成模块(RIM)和皮层解码器(CD)实现多模态特征的有效整合，在22个数据集上跨7个任务取得了优异的准确率-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 当前多模态显著目标检测和伪装目标检测研究通常通过复杂的跨模态特征融合和解码结构来提升性能，但这导致模型参数量过大、结构冗余，且检测性能往往不佳。相比之下，人类视觉系统能够高效完成这些识别任务而无需复杂结构。因此，作者提出一个问题：能否从人类视觉过程中汲取概念启发，实现更简单 yet 准确的物体检测？

Method: 提出HVPNet架构，包含两个核心模块：1)视网膜集成模块(RIM)：以视网膜的多层信息整合为概念隐喻，采用层级特定的多阶段整合策略有效整合多模态特征；2)皮层解码器(CD)：将解码过程分解为低层和高层视觉阶段，模拟人类视觉皮层的层次化处理。

Result: HVPNet能够轻松扩展到跨4个模态的7个任务，在无需额外技巧的情况下，在涵盖这7个任务的22个数据集上建立了出色的准确率-效率权衡。

Conclusion: 受人类视觉过程启发的HVPNet证明了简单且具有生物学意义的建模策略可以同时实现准确和高效的物体检测，为多模态检测任务提供了一种简单 yet 有效的通用框架。
...
TL;DR: 开发了一种基于深度学习的全自动框架，用于光学相干断层扫描（OCT）图像中地理萎缩（GA）关键生物标志物的高精度像素级分割，包括视网膜色素上皮（RPE）损失、椭圆体区（EZ）损失和EZ变薄。


<details>
  <summary>Details</summary>
Motivation: 地理萎缩（GA）继发于年龄相关性黄斑变性（AMD），需要精确监测相关结构生物标志物以评估疾病分期、进展和治疗反应。

Method: 使用三个专门的语义分割模型来描绘RPE损失、EZ边界（包括中断）和Bruch's膜。模型在298个SD-OCT数据集上开发，并在独立外部数据集（n=43）上验证。

Result: Dice RPE损失：0.88，Dice EZ损失：0.87，Pearson相关系数>0.99。EZ厚度测量平均偏差为2.15 μm，可重复性评分ICC>0.98。

Conclusion: 该全自动框架可准确一致地量化外部光感受器变性和RPE损失，为临床试验和常规眼科护理中的GA评估提供了高度可靠的工具。
...
TL;DR: SimpleSearch-VL是一个高效、可靠实用的多模态智能体搜索框架，通过改进智能体自身的搜索验证流程而非扩展数据、工具或辅助模型组件来实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前多模态智能体搜索面临效率低、可靠性不足和实用性有限的问题，传统方法依赖扩展数据、工具或辅助模型，亟需一种更高效的解决方案。

Method: 提出三大核心创新：1）Factorized Adaptive Rollout（FAR）通过形成更有信息的训练组并利用冗余样本来缓解长尾延迟和暴露困难样本，提高采样效率；2）证据验证推理，通过思维链验证显式评估检索到的视觉和文本线索与原始上下文的相关性；3）轻量级工具接口，智能体内进行网页自摘要，无需外部依赖。

Result: 仅使用5K监督工具交织轨迹和2K强化学习数据，SimpleSearch-VL使Qwen3-VL智能体基线分别提升15.8和16.0平均分（8B和30B-A3B变体）。SimpleSearch-VL-30B-A3B性能与agentic Gemini-3-Pro相当。

Conclusion: 通过改进智能体自身的搜索验证流程，在有限训练数据下即可实现显著性能提升，为多模态智能体搜索提供了一种高效可靠且实用的新范式。
...
TL;DR: PRISM是一种基于预训练VAE潜在空间的单图像反射去除方法，将反射去除重新定义为潜在空间中的线性分离问题，使用FLUX骨干网络学习流匹配速度场来同时恢复透射层和反射层。


<details>
  <summary>Details</summary>
Motivation: 现有方法在像素空间操作，受限于非线性的sRGB形成模型，导致两层之间相互纠缠且泛化能力差。研究者观察到预训练VAE潜在空间中图像层之间的相干性显著低于像素空间，这为分解提供了更理想的工作空间。

Method: 提出PRISM方法，包括：1）将SIRR重新表述为潜在空间中的线性分离问题；2）在近似加性公式下，学习FLUX骨干网络上的流匹配速度场；3）引入潜在组合一致性(LCC)策略，通过跨样本交换反射潜在变量并使用循环损失强制一致的分解；4）提出层对比分离(LCS)损失，通过 patch 级对比学习促进层之间的语义分离。

Result: 在六个基准数据集上的实验表明，PRISM始终以显著优势超越最先进的方法，并对野外图像具有强大的泛化能力。

Conclusion: 通过利用预训练VAE潜在空间的良好特性，结合流匹配和对比学习策略，PRISM成功实现了高质量的反射去除，为图像分层任务提供了新的思路。
...
TL;DR: 本文提出MPL-MAE框架，解决3D点云掩码自编码器中位置信息过度依赖的问题，通过重新校准的位置嵌入模块和门控位置接口模块来平衡空间先验与语义特征的学习。


<details>
  <summary>Details</summary>
Motivation: 3D掩码自编码器直接重构空间坐标，容易产生位置泄露；现有框架的解码器过度依赖位置信息，削弱了语义表示学习，导致特征质量不佳。

Method: 提出两个核心模块：1) 重新校准的位置嵌入模块(Recalibrated Positional Embedding)，抑制度量主导的坐标信号同时保留几何拓扑；2) 门控位置接口模块(Gated Positional Interface)，动态调节重建过程中的位置信息注入。

Result: 在多个下游任务的广泛实验中，MPL-MAE持续获得竞争力的性能表现。

Conclusion: 该方法促进了空间先验与语义特征之间更平衡的交互，产生鲁棒且信息丰富的表示，验证了其有效性。
...
TL;DR: MV-GEL是一个从自然语言查询中定位3D网格上细粒度几何实体（如面、边、曲面）的框架，通过多视角选择和VLM推理分割实现几何实体的语言驱动定位。


<details>
  <summary>Details</summary>
Motivation: 3D物体外观对视角高度敏感，单一视角可能使目标实体清晰可见，也可能因遮挡或透视变形而难以识别。现有视觉语言模型在图像领域的指代分割(RIS)已取得进展，但将其扩展到结构化3D几何面临更大挑战。

Method: 提出GELviews模块，根据语言提示的几何CAD实体可观测性对视角进行排序筛选。选定视角由基于VLM的推理分割骨干处理，预测掩膜通过几何感知的射线投射提升到对应网格。框架完全无需CAD先验，仅依赖3D网格。

Result: 在面级别IoU上提升至1.7倍，边级别F1上提升超过4.5倍。相比CLIP基线和随机视角采样显著领先，尤其对细薄和视角敏感结构效果突出。

Conclusion: 通过选择最大化实体可解释性的视角，结合VLM推理分割和几何感知射线投射，实现了CAD几何实体的准确语言驱动定位，为CAD、机器人操作和科学模拟等应用奠定基础。
...
### [89] [Temperature Field Reconstruction of Tungsten Monoblock Divertor on EAST using Physics-aware Neural Operator Transformer](https://arxiv.org/abs/2606.31574)
*Zikang Yan,Xiao Wang,Qingquan Yang,Zhendong Yang,Gaoting Chen,Zehua Chen,Bo Jiang,Jin Tang,Guosheng Xu*

Main category: cs.CV

TL;DR: 本文提出了一种名为物理感知神经算子变换器(PNOT)的方法,用于托卡马克装置中偏滤器温度场的实时重建。该方法将边界热通量关系建模为结构化图,利用图注意力机制捕捉空间物理依赖性,并通过切片聚合相似物理条件下的查询点建模热扩散,同时使用Sobolev正则化损失确保函数值及其导数的一致性。实验表明,物理约束提高了预测精度同时保持物理一致性。


<details>
  <summary>Details</summary>
Motivation: 准确建模偏滤器温度场对于防止材料熔化和损伤以及延长聚变装置使用寿命至关重要。然而,传统有限元方法(FEM)计算成本过高,不适用于实时应用,因此需要一种快速且可泛化的方法来实现偏滤器温度场的实时重建和后续实时控制。

Method: 提出物理感知神经算子变换器(PNOT)来表征偏滤器温度场的时空演化。具体包括:1)将边界热通量关系建模为结构化图,利用图注意力显式捕捉空间物理依赖;2)开发物理感知神经算子模块,通过切片聚合相似物理条件下的查询点并建模热扩散;3)引入梯度约束的Sobolev正则化损失,确保函数值及其导数之间的一致性。

Result: 实验结果表明,物理约束提高了预测精度,同时保持了物理一致性。该方法能够实现偏滤器温度场的快速准确重建。

Conclusion: PNOT方法能够有效实现偏滤器温度场的实时重建,为聚变装置的实时控制提供了可行的解决方案。物理感知约束的引入显著提升了模型的预测精度和物理一致性,为未来聚变装置的温度场建模提供了新的思路。
...
TL;DR: 本文研究了在缩放新型视图合成(NVS)模型的训练时，使用相机参数作为相对位置编码导致的性能停滞问题，并提出解耦姿态位置编码(DPPE)来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当使用相机参数（外参或投影矩阵）作为Transformer中查询、键、值向量的相对位置编码来扩展新型视图合成模型的训练时，观察到模型性能在训练后期停滞不前，这是一个关键的可扩展性问题。

Method: 通过分析发现，旋转和平移信息存储在value向量的相同维度中，导致两者无法独立识别，造成训练不确定性。为此提出解耦姿态位置编码(DPPE)，显式地将旋转和平移解耦分开。

Result: DPPE使得扩展训练设置下的长期稳定训练成为条件，并在 extrapolation 设置中展现出更优的泛化性能，包括处理更多视角和zoom-in场景。

Conclusion: DPPE通过显式解耦旋转和平移，有效解决了相机位置编码在扩展训练中的性能瓶颈问题，为3D计算机视觉中Transformer的可扩展训练提供了新的解决方案。
...
TL;DR: AugSplat通过基于辐射场的视图增强来改进稀疏视图场景中的高斯溅射重建。首先在稀疏输入视图上训练辐射场并合成额外的新颖视图，然后将这些合成视图用作高斯溅射优化的辅助监督。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射在稀疏视图设置中敏感于初始几何质量，有限观测通常导致不完整或嘈杂的点云重建，而神经辐射场虽能稳健重建但推理速度慢且计算成本高。

Method: 提出两种变体：(1) Staged AugSplat：先使用合成视图进行初始优化，然后切换到真实图像；(2) Dual AugSplat：联合训练真实和合成视图，合成损失权重衰减。

Result: 在稀疏视图mip-NeRF 360场景中，AugSplat提高了重建质量。Staged AugSplat获得最佳平均性能，Dual AugSplat表现接近，两者都保持了实时渲染能力。

Conclusion: 基于辐射场的视图增强是改善稀疏视图条件下高斯溅射的有效策略，Staged AugSplat在性能和速度之间取得最佳平衡。
...
TL;DR: ViToS是一个双流强化学习框架,用于医学多模态推理中的主动视觉token剪枝(Visual Token Pruning)。通过训练一个包含两个任务分支的策略模型,一个分支专注于定位(grounding),另一个执行token稀疏推理,在保持优异性能的同时显著减少视觉token数量。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型(VLMs)结合强化学习在多模态推理方面取得显著进展,但在医学图像理解上仍面临挑战。医学图像通常只包含极少量的用于临床决策的视觉证据。研究者发现,剪枝定位区域之外的视觉token可以显著增强医学推理能力,但目前缺乏一个统一的强化学习框架来同时实现主动视觉token剪枝和医学多模态推理。

Method: 提出双流强化学习框架ViToS,包含两个任务分支:1)定位分支(grounding)专注于识别关键视觉区域;2)推理分支在VTP后执行token稀疏推理。为解决耦合策略学习问题,引入跨反馈序列优化(cross-feedback sequential optimization),避免梯度冲突并促进共享策略模型的收敛。模型在七个医学基准数据集上进行评估。

Result: ViToS将视觉token减少至原始序列长度的77%,同时在Lingshu-7B上实现108.27%的相对性能,在HuatuoGPT-Vision-7B上实现104.16%的相对性能。整体而言,ViToS提供了卓越的性能和推理加速,为医学多模态推理建立了高效范式。

Conclusion: ViToS通过双流强化学习框架成功实现了视觉token的主动剪枝和医学多模态推理的结合。跨反馈序列优化有效解决了两个任务分支之间的梯度冲突问题。该方法在减少77%视觉token的同时仍能提升性能,证明了token剪枝策略在医学影像理解中的有效性,为高效的医学多模态推理提供了新范式。
...
### [93] [Preserve the Hard, Regenerate the Rest: Uncertainty-Guided Synthetic Training Data Augmentation with Diffusion Models](https://arxiv.org/abs/2606.31603)
*Nikolai Röhrich,Julian Gleißner,Ahmed H. A. Ibrahim,Silvan Mertes,Tobias Huber*

Main category: cs.CV

TL;DR: 本文提出了一种不确定性引导的合成上下文增强策略，用于解决语义分割模型在数据稀疏和罕见类别上的性能问题。该方法利用基线分割器的预测熵识别不确定区域，仅对互补视觉上下文进行修复，并在微调时仅对原始像素计算损失。


<details>
  <summary>Details</summary>
Motivation: 语义分割模型在处理数据稀疏和罕见或视觉多样化区域（如航拍或自动驾驶数据中的密集区域或小目标）时存在困难。现有合成增强方法存在标签与生成像素对齐问题，或依赖外部模型，或采用粗粒度启发式方法导致在无信息像素上浪费资源。

Method: 提出不确定性引导的合成上下文增强策略：1）使用基线分割器的预测熵识别不确定语义区域；2）仅对互补视觉上下文进行修复（inpaint），严格保持标签有效性；3）微调时仅对原始像素计算损失，排除修复区域，使学习聚焦于未修改的不确定区域，同时将它们呈现于新颖上下文中。

Result: 在Cityscapes、UAVID和BDD100K数据集上实现了显著的mIoU提升，最大提升出现在罕见和困难类别上，如公交车、火车或（从航拍视角）汽车。

Conclusion: 不确定性引导的上下文增强是提升复杂数据集分割性能的有效手段，代码已开源。
...
### [94] [Learning Structurally Consistent Representations for Multi-View Radar Semantic Segmentation](https://arxiv.org/abs/2606.31609)
*Ali Zia,Muhammad Umer Ramzan,Abdelwahed Khamis,Usman Ali,Abdul Rehman*

Main category: cs.CV

TL;DR: 本文提出了一种统一的高阶结构对齐框架，用于多视图雷达语义分割。该方法利用可学习的超图来捕获雷达响应之间的高阶依赖关系，并使用非平衡最优传输（UOT）实现无对应点的跨视图特征对齐，通过自适应注意力机制融合互补的雷达视图。


<details>
  <summary>Details</summary>
Motivation: 雷达传感器在恶劣天气和光照条件下提供可靠感知，但其稀疏、噪声大、语义信息弱的测量值使得密集语义分割具有挑战性。现有方法依赖基于网格的编码和成对交互，难以捕获来自同一物理对象的多个雷达返回形成的高阶关系结构。

Method: 1. 使用可学习的超图（hypergraphs）来捕获空间相关响应之间的高阶依赖关系，以精炼雷达特征表示；2. 采用非平衡最优传输（UOT）在不同测量密度和部分观测下实现无对应点的跨视图特征对齐；3. 使用自适应注意力机制融合互补的雷达视图（RA、RD、AD），强调稀疏和噪声下的结构信息；4. 结合监督分割训练和跨视图一致性正则化进行训练。

Result: 在CARRADA和RADIal基准数据集上验证：在CARRADA上达到63.8% mIoU，在RADIal上达到83.4% mIoU，分别比之前的最佳方法提升了+1.7和+2.3 mIoU。

Conclusion: 高阶关系建模对稳健雷达感知具有重要意义。该框架通过超图和UOT对齐有效解决了雷达数据的稀疏性和多视图异构性问题，为雷达语义分割提供了更鲁棒的特征表示。
...
### [95] [Localized Conformal Prediction for Image Classification with Vision-Language Models](https://arxiv.org/abs/2606.31577)
*Clément Fuchs,Tim Bary,Benoît Macq*

Main category: cs.CV

TL;DR: 本文研究了局部共形预测在视觉-语言模型(VLM)图像分类中的应用。研究发现，简单的余弦相似度无法改善非局部基线，但提出了一种非线性变换方法，能够在保持边际覆盖率保证的同时显著降低平均集合大小。


<details>
  <summary>Details</summary>
Motivation: 共形预测具有强大的边际覆盖率保证，但条件覆盖率保证无法完全实现。为近似条件覆盖率行为，研究者通过根据测试样本与校准样本的相似性调整共形集合来进行探索。该方法在回归问题上表现良好，但在图像分类领域仍缺乏探索。

Method: 在自然图像分类任务上对VLM进行了广泛基准测试，使用了近期局部共形预测算法的开源实现。测试了测试时与校准视觉特征之间的余弦相似度，并提出了一种简单的非线性变换方法。

Result: 直接的余弦相似度使用不足以改善非局部基线性能。非线性变换后的余弦相似度在保持边际覆盖率保证的同时，实现了统计学上显著的平均集合大小 reduction。

Conclusion: 所提出的非线性变换方法在VLM局部共形预测中有效，能够在维持覆盖率保证的情况下减小集合大小。代码已开源。
...
TL;DR: Q-Margin是一种新型α-散度损失函数，通过将边际惩罚直接编码到参考测度（先验概率）中而非logits，从而在保持α-散度稀疏性优势的同时实现判别性嵌入。该方法在人脸验证（IJB-B/C）和说话人验证（VoxCeleb）上达到先进水平，尤其在低误识率（FAR）场景下优于ArcFace和CosFace。


<details>
  <summary>Details</summary>
Motivation: 现有的margin-penalty softmax损失（如CosFace、ArcFace）虽性能出色，但几何边际设计针对softmax函数，无法自然推广到α-散度这一广义概率框架。α-散度损失虽能诱导稀疏解（α>1时），但缺乏与之配套的边际机制。

Method: 提出Q-Margin，将边际惩罚直接编码到参考测度（先验概率）中，而非传统方法那样对logits施加几何惩罚。这种公式化自然地鼓励判别性嵌入，同时保留α-散度的稀疏性优势。

Result: 在IJB-B和IJB-C人脸验证基准上达到竞争或超越性能；在VoxCeleb说话人验证上同样表现优异。更重要的是，在低误识率（FAR）场景下一致性地超越ArcFace和CosFace基线。此外，Q-Margin后验的极端稀疏性实现了精确且内存高效的训练。

Conclusion: Q-Margin成功地将边际概念融入α-散度框架，在高安全应用场景（低FAR）中展现出显著优势，同时提供了可扩展到百万级身份数据集的训练方案。
...
TL;DR: WorldRoamBench是一个开放世界基准测试，用于评估交互式世界模型(IWM)在长程稳定性方面的表现。该基准从四个维度进行评估：动作（逐帧动作指标）、视觉（基于分段的漂移指标）、物理（可控性门控评估）和记忆（动作解耦的场景记忆和主体记忆评估）。测试包含600多个案例，涵盖自然、城市和室内场景。评估10+模型显示目前尚无模型能在所有维度上可靠满足要求，即使最佳模型也只获得中等分数。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅在轨迹层面评估动作跟随，忽视了记忆和交互物理的评估。这一局限性导致交互式世界模型(IWM)的发展缺乏全面的性能衡量标准，无法真正评估模型在真实世界应用中的稳定性和可靠性。

Method: WorldRoamBench提出四个创新评估维度：(1)动作：逐帧动作指标，绕过跨模型语义尺度差异，暴露轨迹隐藏的失败；(2)视觉：基于分段的漂移指标，捕捉起点终点比较遗漏的中序列非单调崩溃；(3)物理：可控性门控评估，涵盖光学、力学和3D一致性，在忠实执行动作下评分可信度；(4)记忆：动作解耦协议，通过局部3D点云重建评估场景记忆，通过跟踪加VLM推理评估主体记忆。测试用例600+，场景涵盖自然/城市/室内，第一/第三人称视角，WASD控制10-60秒连续交互。

Result: 对10多个开源/闭源模型进行评估，结果显示目前尚无模型能在所有四个维度上可靠满足要求，即使最佳模型也只达到中等分数。这表明交互式世界模型在稳定性、物理接地和记忆保真度方面仍有重大改进空间。

Conclusion: 在WorldRoamBench上的进展将推动IWMs向稳定、物理接地、记忆保真和可部署于真实应用的方向发展。该基准填补了现有评估框架的关键空白，为未来IWM研究提供了全面的性能衡量标准。
...
TL;DR: 本文提出了Histogram-constrained Image Generation (HIG)，一种在扩散模型生成过程中强制执行用户指定的分布约束（如颜色直方图或潜在token分布）的新控制机制，介于全局语义引导和局部结构控制之间的中等粒度控制方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型控制机制在粒度上存在差异：文本提示提供全局语义引导，ControlNet等方法提供精确的局部结构控制，但缺乏中间粒度的控制方案。如何在保持全局一致性的同时实现局部精确控制仍是一个开放挑战。

Method: 将分布约束控制建模为最优传输(OT)问题，并在采样过程中应用显式引导变换，驱动扩散轨迹与期望直方图对齐。该方法可与现有控制机制完全兼容。

Result: 展示了HIG在多种应用中的通用性，包括通过颜色/潜在直方图的约束生成，以及通过直方图级别编码的高容量信息嵌入。

Conclusion: 分布控制是一种灵活且可解释的控制方案，完全兼容现有控制机制，为可控图像生成提供了多样化的混合策略。项目页面已发布。
...
### [99] [Look But Don't Touch with Sparse Autoencoders for Unlearning in Diffusion Models](https://arxiv.org/abs/2606.31699)
*Enrico Cassano,Riccardo Renzulli,Rayyan Ahmed,Marco Grangetto,Stephan Alaniz*

Main category: cs.CV

TL;DR: 本文系统评估了稀疏自编码器(SAE)在扩散模型中用于概念擦除和引导的假设，发现SAE虽能可靠检测和定位语义概念，但直接干预其潜在空间会导致分布外激活和严重视觉 artifacts。提出基于检测的替换方法可保留扩散模型激活统计特性，产生更干净的擦除结果。


<details>
  <summary>Details</summary>
Motivation: 验证SAE能否作为概念级操纵的可控干预点，特别是用于扩散模型中的物体擦除和引导

Method: 将SAE激活纯粹作为语义检测器，识别包含目标物体的图像区域，用不含该物体的patch嵌入替换这些区域

Result: 检测-based替换方法保留扩散模型激活统计特性，比潜在引导产生明显更干净的擦除结果

Conclusion:  monosemantic/sparse特征本质上不适合作为引导控制旋钮，SAE是强大的可解释性工具但用于直接操作(如unlearning)存在重要局限
...
### [100] [JL1-CC&QA: Extending the JL1-CD Benchmark with Change Captioning and Question Answering](https://arxiv.org/abs/2606.31745)
*Ziyuan Liu,Ruifei Zhu,Ouqiao Ma,Yuantao Gu*

Main category: cs.CV

TL;DR: JL1-CC&QA是一个多任务遥感变化检测基准数据集，在JL1-CD基础上新增变化描述（Captioning）和变化问答（QA）两个注释层，包含5000对双时相图像、17021条变化描述和20060个问答对，旨在弥补传统变化检测只回答"哪里变化"而无法回答"发生了什么变化"和"为什么变化"的语义鸿沟。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测仅关注像素级二值分割，只能识别变化发生的位置，但无法理解变化的语义内容（即发生了什么变化）和变化原因。为弥补这一语义差距，研究团队提出构建一个统一的多任务变化理解基准数据集。

Method: 使用吉林一号卫星（0.5-0.75米分辨率）获取的5000对双时相图像，采用三阶段注释 pipeline：1）多模态大语言模型生成初步注释；2）视觉 grounded LLM 判断；3）人类专家验证，确保注释质量。

Result: 构建了包含17021条质量验证的变化描述（JL1-CC）和20060个问答对（JL1-QA，涵盖8种问题类型）的多任务基准数据集，可同时支持二值变化掩膜、变化描述和变化导向的问答任务。

Conclusion: JL1-CC&QA作为首个统一二值变化掩膜、变化描述和变化问答的遥感数据集，为社区推进多任务变化理解研究提供了有价值的资源，有望推动遥感变化检测从像素级识别向语义级理解的发展。
...
### [101] [What Memory Do GUI Agents Really Need? From Passive Records to Active Task-Driving States](https://arxiv.org/abs/2606.31612)
*Chen Liu,Ling Chen,Hanzhang Zhou,Xu Zhang,Quyu Kong,Panrong Tong,Wenhao Wang,Xin Yu,Steven Hoi,Yue Wang*

Main category: cs.CV

TL;DR: 本文提出ATMem和STR-GRPO来解决移动GUI代理在长程任务中的记忆问题。ATMem将被动存储转变为主动维护的执行状态，跟踪每个值的角色和状态；STR-GRPO通过在线强化学习学会选择性使用记忆。构建了新的移动端基准测试评估方法有效性。


<details>
  <summary>Details</summary>
Motivation: 现有记忆方法将记忆视为被动存储，仅在需要时检索过去的观察结果，但检索值并不能揭示其在工作流中的当前角色。代理仍需从累积记录中推断当前是否应使用该值、已使用过或需等待后续依赖。这种隐式重建在长轨迹中变得不可靠，特别是存在相似字段、重复值、干扰项和过时状态时，会导致重复操作或遗漏操作。

Method: 提出两种核心技术：1)主动任务驱动记忆(ATMem)：将记忆从被动存储转变为主动维护的执行状态，跟踪每个值的角色和当前状态，使代理能够根据当前工作流状态选择行动；2)STR-GRPO：在线强化学习方法，通过对比记忆开启和关闭的 rollout来估计记忆使用何时能改善执行效果，同时引入记忆成本感知奖励来惩罚不必要的记忆使用。

Result: 代理能够完成范围内所有工作的同时，避免在长程执行中产生范围外动作。该基准测试要求代理从几乎相同的条目中，对每个满足指令的条目执行操作，并拒绝违反约束的条目。

Conclusion: 实验构建了一个具有挑战性的移动端基准测试，从几乎相同的条目中智能选择满足指令的条目并拒绝违反约束的条目。ATMem通过将被动存储转变为主动维护的执行状态，显著提升了长程任务中GUI代理的可靠性。STR-GRPO方法有效学习了何时使用记忆能够提升任务执行效果。
...
### [102] [Breaking Failure Cascades: Step-Aware Reinforcement Learning for Medical Multimodal Reasoning](https://arxiv.org/abs/2606.31825)
*Junha Jung,Minbyul Jeong,Suhyeon Lim,Sungwook Jung,Jaehoon Yun,Taeyun Roh,Mujeen Sung,Jaewoo Kang*

Main category: cs.CV

TL;DR: MRPO是一种结合逐步过程奖励的强化学习算法，用于优化医学多模态大语言模型的推理过程，通过对早期无效推理步骤分配指数级惩罚来打破失败级联。


<details>
  <summary>Details</summary>
Motivation: 现有后训练管道主要结果导向（最终答案正确性或序列级偏好），存在稀疏信用分配问题，难以优化临床应用所需的推理过程。早期推理失败导致的级联错误是医学VQA预测错误的主要原因。

Method: 提出Medical Reasoning-aware Policy Optimization (MRPO)算法，结合逐步过程奖励。当最终答案不正确时，MRPO对早期无效推理步骤中的token分配指数级更大的惩罚，打破失败级联而不影响成功路径。

Result: 在三个多模态LLM backbone上，MRPO始终优于标准GRPO和最近的RL基线。在Qwen3-VL-8B-Instruct上超越HuatuoGPT-Vision-34B达2.79分。早期推理失败从64.0%降至13.0%。

Conclusion: 有针对性地缓解级联失败可显著提高推理质量和最终答案准确性，MRPO为医学多模态大语言模型的推理过程优化提供了有效范式。
...
### [103] [Robust Autonomous UAV Landing on Maritime Platforms via Multimodal Agentic AI and Active Wave Compensation](https://arxiv.org/abs/2606.31613)
*Francisco S. Neves,Pedro N. Pereira,Raul D. S. G. Campilho,Andry M. Pinto*

Main category: cs.CV

TL;DR: 本文提出一种面向海洋基础设施的无人机自主降落的解耦式多车协同框架，通过USV搭载3-RPU稳定平台结合两个深度强化学习智能体实现波激运动补偿和无人机最终进近，在高保真仿真中实现了100%降落成功率。


<details>
  <summary>Details</summary>
Motivation: 海洋环境中随机海况导致自主 aerial 检查任务面临高动能撞击、降落倾覆和感官遮挡等风险，现有方法难以有效应对动态海面带来的挑战。

Method: 构建解耦式多车协同框架，包含USV搭载的3-RPU稳定平台和鲁棒无人机；采用两个独立深度强化学习智能体——Soft Actor-Critic(SAC)智能体提供高频波激运动补偿，多模态RL智能体负责无人机最终进近。

Result: 在15次试验中实现100%降落成功率，海况从平静到恶劣；恶劣条件下平均稳定效率87.8%，96%的任务时间内降落面保持1度以内水平。

Conclusion: 所提系统能有效补偿海面运动，显著提升海洋环境下无人机降落安全性，为自主海洋基础设施检测提供可行方案。
...
### [104] [Real-Time Source-Free Object Detection](https://arxiv.org/abs/2606.31834)
*Sairam VCR,Varun Gopal,Poornima Jain,Vineeth N Balasubramanian,Muhammad Haris Khan*

Main category: cs.CV

TL;DR: RT-SFOD是一种基于YOLOv10的无源域目标检测方法，通过双头伪标签融合(DHF)和多尺度自适应表征分化(MARD)损失，在domain-shift基准测试中实现了1.4%至3.5%的mAP提升，吞吐量提高1.3倍，参数减少约2倍。


<details>
  <summary>Details</summary>
Motivation: 现有无源域目标检测(SFOD)方法依赖重量级架构，优先考虑精度而忽视延迟和内存约束，导致速度-精度-模型大小的权衡不理想。需要一种在严格延迟和内存约束下处理域迁移的轻量级高效检测器。

Method: 1. 基于YOLOv10（无NMS的双头检测器）构建；2. 提出DHF（双头伪标签融合）：选择性融合一头对一(O2O)和一头对多(O2M)预测，保留精度并恢复遗漏物体；3. 提出MARD（多尺度自适应表征分化）损失：通过检测感知的方差和协方差约束增强多尺度特征可分性；4. 两模块仅在训练时使用，推理时无改变。

Result: 在域迁移基准测试中，RT-SFOD相比此前最优SFOD方法实现：mAP提升1.4-3.5%，吞吐量提高1.3倍，参数量减少约2倍。方法在YOLOv10及YOLO和DETR系列双头检测器上均具有泛化性。

Conclusion: 基于YOLOv10的RT-SFOD方法成功推进了速度-精度-模型大小的帕累托前沿，证明了轻量级架构也能实现最优的域适应精度，同时保持高效率。DHF和MARD两个训练时模块可有效解决双头检测器在域迁移下的性能问题。
...
### [105] [PrISM-IQA: Image Quality Assessment Made Practical for Smartphone Photography](https://arxiv.org/abs/2606.31626)
*Shuyan Zhai,Jiaqi He,Weixia Zhang,Liang Wang,Zhenjie Lee,Zufeng Zhang,Kede Ma*

Main category: cs.CV

TL;DR: PrISM-IQA将智能手机图像质量评估(IQA)重新定义为多问题序数诊断问题，不再预测单一质量分数，而是预测每个ISP相关问题的有序严重程度级别(无、轻度、重度或严重)，并结合累积序数编码与结构化推理来产生逻辑一致的预测。


<details>
  <summary>Details</summary>
Motivation: 现有智能手机IQA方法通常将感知质量简化为单一分数，这种标量公式与实际的ISP调优工作流程不符，因为工程师需要识别特定质量问题、评估其严重程度并确定是否需要干预。

Method: 提出PrISM-IQA模型，结合累积序数编码与结构化推理，捕获问题内单调性以及跨问题包含和排除关系，预测53个ISP相关质量问题的有序严重程度级别。

Result: 在重建的SPAQ基准测试和专家标注的真实世界数据集上验证，表明PrISM-IQA能有效进行实际问题级诊断，通过线性探测揭示可迁移的感知质量表示，其预测可支持可操作的ISP调优。

Conclusion: PrISM-IQA成功将IQA从单一分数预测转变为多问题有序诊断，为ISP调优提供了更实用、更具可操作性的质量评估框架。
...
### [106] [Attend, Transform, or Silence: Operator-Level Visual Skipping for Efficient Multimodal LLM Inference](https://arxiv.org/abs/2606.31903)
*Zhaoyang Luo,Runmin Dong,Miao Yang,Fan Wei,Yushan Lai,Bin Luo,Haohuan Fu*

Main category: cs.CV

TL;DR: 本文提出一种运算符级视觉token跳过框架，通过选择性绕过冗余的注意力、FFN或两者来处理长视觉token序列，在Qwen3-VL上减少33.7% TFLOPs的同时保留99.5%性能。


<details>
  <summary>Details</summary>
Motivation: 现有加速方法通常移除整个视觉token或跳过整层视觉token更新，这些粗粒度策略可能丢弃细粒度证据。作者从答案可观察角度发现，后期视觉token更新可能对答案token表示影响很小，存在答案静默冗余。

Method: 将每个Transformer层分解为注意力和FFN运算符，提出运算符级视觉token跳过框架，在保留完整视觉token序列的同时选择性绕过冗余的注意力、FFN或两者。

Result: 在三种MLLM架构和10个VQA基准测试中实现强效率-精度权衡，Qwen3-VL上减少33.7% TFLOPs，保留99.5% vanilla模型性能。

Conclusion: 通过运算符级跳过策略，该方法在显著降低计算量的同时保持接近原始模型的性能，为多模态大语言模型的高效推理提供了有效途径。
...
### [107] [LiteMatch: Lightweight Zero-Shot Stereo Matching via Cost Volume Stabilization](https://arxiv.org/abs/2606.31636)
*Md Raqib Khan,Santosh Kumar Vipparthi,Subrahmanyam Murala*

Main category: cs.CV

TL;DR: LiteMatch是一个轻量级立体匹配框架，通过成本体稳定化实现强零样本泛化，无需昂贵的3D卷积。包含两个互补编码器：交叉视图对应编码器(CVCE)和高频编码器(HFE)，以及CVC-Loss损失函数和轻量级细化模块。


<details>
  <summary>Details</summary>
Motivation: 当前基于学习的立体匹配方法需要繁重的骨干网络和计算密集的3D成本体处理，导致内存和运行时间开销大。更关键的是，这些方法经常难以跨域泛化，限制了其实际部署。

Method: 采用两个互补编码器：CVCE捕获全局交叉视图交互，HFE通过FFT频率线索增强精细结构细节；引入CVC-Loss（针对softmax归一化成本分布的体素级二值交叉熵），鼓励尖锐且单峰的视差概率；轻量级细化模块生成清晰的全分辨率视差。

Result: 参数范围3.36M至9.58M，在Scene Flow、KITTI、Middlebury、ETH3D和DrivingStereo上实现卓越零样本泛化，获得具有竞争力的EPE和D1性能。

Conclusion: 轻量级架构确实可以跨域泛化而不牺牲精度。
...
### [108] [Technical Report of RoboSpatial Challenge at CVPR 2026: Selective Reasoning Activation and Reference-Frame Disambiguation for Embodied Spatial Reasoning](https://arxiv.org/abs/2606.31645)
*Yuxiang Xie,Qi Lv,Jianming Xing,Zijian Hong,Xiang Deng,Weili Guan,Liqiang Nie*

Main category: cs.CV

TL;DR: RoboSpatialBrain是一个基于RoboBrain2.5-8B-NV的视觉语言模型，用于解决具身任务中的空间推理问题。该方法结合两种推理机制：强制思考前缀激活策略配合任务特定后提示，以及显式参考帧重定向管道。系统在RoboSpatial挑战赛中获得第一名，RoboSpatial-Home上成功率达80.9%。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型虽然在通用感知方面表现优异，但在具身任务所需的空间推理方面往往存在困难。现有模型难以处理上下文任务中的相机中心与物体中心的歧义问题，以及兼容性任务的推理需求。

Method: 提出两种免训练的推理时机制：1) 强制思考前缀激活策略配合任务特定后提示，引出模型对上下文和兼容性任务的深思熟虑推理；2) 显式参考帧重定向管道，解决相机中心和物体中心的歧义问题。此外还探索了在兼容性数据上微调RoboBrain2.5模型，并分析了其与提示策略的交互效果。

Result: RoboSpatialBrain在CVPR 2026具身推理行动研讨会的RoboSpatial挑战赛中获得第一名，在RoboSpatial-Home数据集上达到80.9%的总体成功率。

Conclusion: 通过结合推理时机制和微调策略，RoboSpatialBrain有效提升了视觉语言模型在空间推理任务上的性能，为具身智能任务提供了新的解决方案。代码已开源。
...
### [109] [LUNA: Learning Universal 3D Human Animation Beyond Skinning](https://arxiv.org/abs/2606.31981)
*Peng Li,Rawal Khirodkar,Junxuan Li,Yuan Dong,Chen Cao,Yuan Liu,Wenhan Luo,Yike Guo,Shunsuke Saito*

Main category: cs.CV

TL;DR: LUNA是一个无需线性混合蒙皮(LBS)的通用神经动画模型,直接从多种2D控制信号(图像、关键点、草图等)映射到3D高斯变形,实现逼真的人类动作动画。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖LBS和参数化身体模型所带来的表达力受限和伪影问题,以及2D到3D提升的模糊性和数据标注难题。

Method: 核心是一个基于transformer的运动回归器,将全局刚性运动与细粒度局部动态分离;引入混合监督机制,从LBS教师模型提取软结构先验,并支持有限标注数据和大规模野外无标签视频的训练。

Result: LUNA实现了与LBS方法相当的视觉保真度,同时提供逼真的人类动作和跨身份零样本泛化能力,是首个支持隐式2D驱动的端到端3D可动画化模型。

Conclusion: 该方法证明了无需显式身体模型拟合的端到端神经动画可行性,为3D人类动画领域提供了新的研究方向。
...
### [110] [FLORA: A deep learning approach to predict forest attributes from heterogeneous LiDAR data](https://arxiv.org/abs/2606.32023)
*Emilie Vautier,Clément Mallet,Cédric Vega*

Main category: cs.CV

TL;DR: FLORA是一个深度学习框架，用于从异构激光雷达点云预测森林属性，解决了欧洲国家激光雷达计划中传感器、飞行参数、季节和扫描角变化带来的模型稳健性问题。


<details>
  <summary>Details</summary>
Motivation: 国家激光雷达计划的扩展导致数据获取条件异构（传感器、飞行参数、季节、扫描角不同），现有模型通常针对当地条件校准，难以生成稳健的全国尺度预测。

Method: 结合八叉树骨干网络与生态和时空辅助变量，通过后期融合门控机制；使用法国32,052个国家森林Inventory样地数据进行训练和评估。

Result: 单一模型在落叶和常绿 acquisitions上训练优于季节特定模型；辅助变量提供适度整体增益，但对特定物种材积预测贡献更大；优势高度rRMSE约12.3%（R²=0.88），总体积rRMSE约39%（R²=0.74）。

Conclusion: FLORA为异构国家激光雷达项目的大规模森林属性估计提供了稳健基线，表明跨季节训练可提高模型泛化能力。
...
### [111] [SAMBA: A Scatter-Guided Masked Bidirectional Mamba Foundation Model for SAR Target Recognition](https://arxiv.org/abs/2606.31668)
*Ke Wang,Xiaoyi Pan,Zhaoyu Gu,Xiaofeng Ai,Zhiming Xu,Feng Zhao,Shunping Xiao*

Main category: cs.CV

TL;DR: 本文提出SAMBA（Scattering-Guided Bidirectional Mamba），一种用于SAR目标解释的高效自监督预训练基础模型，旨在解决SAR ATR中标注数据稀缺、Transformer计算复杂度高等问题。


<details>
  <summary>Details</summary>
Motivation: SAR ATR在实际部署中受限于稀缺的标注训练数据；现有Transformer架构存在二次计算复杂度问题；传统通用掩码策略忽视了SAR图像独特的电磁散射特性。

Method: 提出三大核心创新：(i) 线性复杂度的Mamba编码器配合中序列类令牌；(ii) 三层层次化散射引导掩码自编码器(SG-MAE)，基于SAR物理先验设计；(iii) 轻量级SpatialMix特征交互模块增强跨区域特征融合。采用两阶段跨域预训练流程。

Result: SAMBA在所有预训练配置下均表现优异，参数远少于CNN和Transformer基线模型；SG-MAE策略相比标准MAE掩码策略显著提升了模型的少样本迁移能力；在七个下游数据集的分类和检测任务上达到SOTA性能。

Conclusion: SAMBA成功验证了其在多样化SAR解释任务中的强大泛化能力，为SAR领域提供了一种高效的自监督预训练基础模型方案。
...
TL;DR: REDI提出了一种用于Vision Transformer的token reduction方法，通过结合监督TF-IDF和注意力机制来分配有限的token预算，在ImageNet-1K上实现84.706%的Top-1准确率，比密集基线提高1.2%。


<details>
  <summary>Details</summary>
Motivation: 研究如何在图像分类任务中为固定预算的patch token分配最优策略，平衡准确率和效率。

Method: 1) 将DINOv3的patch表示量化到视觉词汇表；2) 使用监督TF-IDF计算类条件语料库分数；3) 结合注意力图生成REDI分数；4) 使用keep、merge和compress操作符分配patch角色。

Result: 将序列长度从201减少到107 (46.8% reduction)，Top-1准确率达到84.706%，相比密集基线(83.514%)提升1.2%。

Conclusion: 类特定的语料库统计和图像特定的注意力为patch排序提供了互补信号，REDI方法有效结合两者提升分类性能。
...
TL;DR: ShellMaker是一个语言引导的建筑外观补全框架，能够在保持建筑结构约束的前提下，根据文本提示生成完整的外观网格和PBR材质，实现与室内场景风格一致的建筑外立面。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景生成方法只能生成Floor plans和墙体布局，无法生成风格一致的外立面和屋顶。完成建筑外观具有挑战性，因为必须保持建筑 footprint、墙体几何形状和开口语义的约束，而无约束的生成模型往往违反这些约束。

Method: ShellMaker结合了参数化屋顶生成、基于LLM的部件感知提示词优化、联合墙体-屋顶材质检索和几何感知组装技术。该方法在格式无关的scaffold表示上操作，可推广到室内生成器、CityGML和CAD输入。

Result: ShellMaker在保持结构一致性的同时，在建筑连贯性方面优于检索和无约束生成基线方法。

Conclusion: ShellMaker是首个能够在保持结构约束下根据文本提示生成完整建筑外观的系统，填补了室内场景生成与建筑外观生成之间的空白，具有广泛的应用前景。
...
TL;DR: 提出了一种基于单次扫描点云的3D语义占用预测方法DRVR，通过结合距离视图上下文和体素视图几何信息，避免了多扫描方法带来的效率和安全问题。


<details>
  <summary>Details</summary>
Motivation: 点云具有稀疏性和不完整性，导致语义学习和占用感知困难。现有方法堆叠多次扫描点云获取密集空间信息，但带来计算负担和姿态变换噪声等效率和鲁棒性问题。

Method: 设计双距离-体素表示(DRVR)框架：1)使用距离视图编码器提取场景紧凑上下文；2)设计几何感知体素视图编码器分别提取多尺度体素视图特征并融合；3)提出距离-体素融合模块通过体素到距离和距离到体素的融合协同两种视图特征。

Result: 在nuScenes-Occupancy、SemanticKITTI和SemanticPOSS数据集上验证。在nuScenes-Occupancy上，单次扫描的DRVR相比多扫描方法mIoU提升5.4%，推理速度提升2.1倍。

Conclusion: DRVR通过有效利用单次扫描点云的-range和voxel视图信息，在保证预测精度的同时显著提升了效率，为实际自动驾驶应用提供了一种更实用的解决方案。
...
TL;DR: 本工作提出IS-SNN架构，通过拓扑感知权重标准化和修改残差连接替代激活归一化层，消除运行时统计追踪和乘法运算，在ImageNet上达到68.05%准确率的同时大幅降低硬件资源消耗。


<details>
  <summary>Details</summary>
Motivation: 深度脉冲神经网络(SNN)的性能通常依赖批归一化(BN)，但先进的动态BN变体引入运行时乘法运算，削弱了SNN的硬件效率优势

Method: 通过拓扑感知权重标准化和修改的残差连接来强制信号稳态，将标准化操作离线折叠到静态权重中，从而消除激活归一化层

Result: 在VGG、ResNet和Transformer模型上达到与计算昂贵的动态BN技术相当或更优的性能；在ImageNet上达到68.05%的竞争力准确率；FPGA查找表资源减少96.4%

Conclusion: IS-SNN是一个构建准确且硬件友好的深度神经形态系统的实用框架，通过消除运行时统计追踪和乘法运算，恢复了累加导向的推理数据路径。
...
TL;DR: 本文提出Phantom框架，一种统一的人脸交换深度伪造防护系统，通过在潜在域和空间域联合约束扰动来保护个人隐私免受深度伪造技术的侵害。


<details>
  <summary>Details</summary>
Motivation: 人脸交换深度伪造对个人隐私构成日益严重的威胁。当前对抗性方法在应对人脸交换场景时存在局限性：固定或随机目标导致模糊的潜在指导；缺乏明确的空间约束导致扰动扩散到身份无关区域；身份-风格解耦在深度伪造生成过程中抑制对抗性信号。

Method: Phantom框架包含两个核心组件：1) 自适应合成保持属性但身份偏移的目标，以指导身份感知的潜在优化；2) 将扰动限制在语义相关面部区域的掩码扰动。

Result: 在UniFace、INSwapper和SimSwap上，Phantom在躲避场景中的防护成功率分别提升27.8%、25.6%和16.6%，同时改善了视觉质量。在 impersonation场景中，防护效果提升达10.2%，同时提高了感知保真度。

Conclusion: 联合利用潜在域和空间约束对于实现鲁棒且一致的面部隐私保护是有效的。
...
TL;DR: 本文提出WIDER-FAIR数据集，用于研究人脸检测模型中的公平性问题。该数据集基于WIDER-FACE构建，手动标注了16,256张图片的种族（亚洲、黑人、印度、白人）和性别信息，并通过YOLOv5模型验证了数据集在检测种族偏见方面的价值。


<details>
  <summary>Details</summary>
Motivation: 人脸检测模型在现实部署中可能存在不同人口统计群体间的性能差异，但缺乏带有敏感特征标注的人脸检测数据集来研究和缓解这些偏见。

Method: 1) 在WIDER-FACE数据集基础上构建WIDER-FAIR数据集，手动标注16,256张图片的种族和性别信息；2) 使用人脸嵌入、KNN分类器和t-SNE可视化验证标注质量；3) 训练YOLOv5模型并进行消融实验。

Result: 检测性能在黑人群体中明显较低；将该群体排除在训练外会加剧公平性差异，超过了排除其他种族群体的影响。

Conclusion: 带有人口统计标注的数据集对于理解和评估人脸检测模型的偏见具有重要价值，WIDER-FAIR可作为研究公平性的重要基准。
...
TL;DR: UniCoder是一个统一的强化学习框架，通过符号属性对齐和参考引导代码优化两个机制，解决了视觉到代码生成中精细对齐的挑战，使8B参数模型达到开源最佳性能。


<details>
  <summary>Details</summary>
Motivation: 标准多模态大语言模型通过监督微调无法实现视觉到代码生成所需的像素级精确对齐，而强化学习虽具理论潜力，但面临奖励粗糙和探索停滞两大根本障碍。

Method: 提出两个创新机制：(1)符号属性对齐 - 使用轻量级辅助LLM将生成代码解析为离散视觉属性(如十六进制颜色、坐标范围)，实现密集的元素级奖励计算；(2)参考引导代码优化 - 将真实轨迹动态注入低表现rollout组，将盲目探索转化为引导策略改进。

Result: 在ChartMimic、UniSVG、Design2Code和ScreenBench基准测试中，8B参数模型超越所有开源基线，达到与专有模型相当的先进性能。

Conclusion: 为通用视觉到代码合成建立了新范式，证明了强化学习结合精细奖励设计在多模态代码生成中的有效性。
...
TL;DR: MemLearner是一种基于学习的自适应上下文查询方法，用于解决视频世界模型中缺乏长期记忆导致场景不一致的问题。该方法通过查询令牌桥接上下文和预测令牌，利用视频生成模型本身的预训练视觉先验，无需从头训练额外模块。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型面临关键挑战：缺乏记忆机制导致长时间生成场景不一致。现有基于规则的上下文帧检索方法无法在场景遮挡和动态物体的场景中泛化。

Method: 提出MemLearner方法，采用学习式自适应上下文查询：1)使用查询令牌桥接上下文和预测令牌；2)利用视频生成模型本身进行上下文查询，复用预训练视觉先验；3)收集包含场景遮挡和动态物体的长视频数据集，配有相机姿态标注；4)采用多数据集训练策略，结合标注渲染视频和未标注真实世界视频。

Result: MemLearner在场景一致性和记忆方面显著优于现有视频世界模型，尤其是在具有挑战性的遮挡和动态场景条件下表现突出。

Conclusion: 通过学习式上下文查询而非规则-based方法，MemLearner成功解决了视频世界模型的长程记忆问题，为复杂场景下的视频生成提供了有效解决方案。
...
TL;DR: RhythmJEPA是一种用于远程光电容积脉搏波描记(rPPG)的节奏结构化联合嵌入预测学习框架，通过预测 masked 面部视频的潜在教师表示来学习生理感知表征，而非重建RGB帧。


<details>
  <summary>Details</summary>
Motivation: 1. 现有自监督rPPG方法主要重建被遮挡的像素或低层视觉表示，这会使模型偏向于面部外观而非潜在生理动态；2. 现有的Mamba方法仅按时间顺序扫描面部视频token，无法利用脉搏信号的循环结构。

Method: 1. 提出Cyclic Rhythm-State Planner (CRSP)：估计帧级潜在生理状态，通过动态规划和约束转换语法解码最可能的循环状态路径；2. Dual Order Mamba Encoder (DOM)：结合传统时间顺序扫描与状态顺序扫描，捕捉局部时间连续性和远程节奏一致性依赖；3. Spatial Pulse Mixer (SPM)：提取紧凑的脉搏敏感面部token，在复杂度和性能间取得良好平衡。

Result: 在PURE、UBFC-rPPG和MMPP数据集上展现出与代表性rPPG方法相当的竞争性能。

Conclusion: RhythmJEPA通过引入循环状态规划和双序Mamba编码器，有效解决了自监督rPPG方法中面部外观偏差和时序扫描限制的问题，为生理感知表征学习提供了新思路。
...
TL;DR: 本文提出利用滚动快门相机在拍摄高速运动物体时产生的特征失真作为有价值的时间信息源，从单帧滚动快门图像中估计快速移动球形物体的3D平移和角速度。通过设计一种鲁棒且易于检测的球形图案，并在反投影框架中强制执行几何一致性来恢复运动，无需建立对应关系。利用球面几何特性，通过两阶段优化过程将平移和旋转运动解耦并分别估计，即使对无纹理物体也能实现可靠的运动恢复。合成数据和真实数据的广泛实验表明，在挑战性高速条件下能够准确鲁棒地估计运动参数。


<details>
  <summary>Details</summary>
Motivation: 滚动快门相机在拍摄快速移动物体时会引入特征失真，这些影响通常被视为需要校正的伪影。本文旨在将这些失真转化为有价值的时间信息源，用于运动估计。

Method: 1) 设计鲁棒易检测的球形图案；2) 提出无需对应关系的公式化方法，通过反投影框架强制执行几何一致性来恢复运动；3) 利用球面几何特性，通过两阶段优化过程将平移和旋转运动解耦估计。

Result: 在合成数据和真实数据集上的广泛实验表明，在挑战性高速条件下能够准确鲁棒地估计运动参数。

Conclusion: 该方法成功将滚动快门失真转化为优势，无需对应关系即可从单帧图像估计高速运动球形物体的3D平移和角速度，对无纹理物体同样有效。
...
TL;DR: 本文提出Barycentric Dominance Field (BDF)来解决自回归(AR)模型在原生3D网格生成中的局限性，通过将离散网格拓扑转换为连续表面信号，使扩散模型能够生成更高质量、更可扩展且更鲁棒的原生网格。


<details>
  <summary>Details</summary>
Motivation: 自回归建模在原生3D网格生成中面临严重限制，包括面数有限、顶点分辨率受限以及难以支持纹理等瓶颈问题。

Method: 提出Barycentric Dominance Field (BDF)，一种定义在三角网格表面上的连续表示方法，通过将连通性转换为连续表面信号来弥合离散网格拓扑与连续扩散生成建模之间的差距。

Result: BDF使扩散模型能够生成具有更高质量、更大可扩展性和更强鲁棒性的原生网格，显著优于当前最先进的自回归方法。

Conclusion: BDF作为内在网格属性，与纹理图具有强相似性，可无缝集成到现有3D扩散管道中而无需修改架构。
...
TL;DR: 本文提出SpikeLogBERT，一种基于脉冲神经网络的能量高效日志解析框架，结合脉冲Transformer架构与BERT知识蒸馏，在HDFS数据集上实现0.99997的解析精度，同时降低62.6%的理论能耗。


<details>
  <summary>Details</summary>
Motivation: 现有神经日志解析方法依赖密集矩阵乘法，计算成本和能耗较高，需要一种既能保持语义表示能力又能实现能量高效处理的方案。

Method: 提出SpikeLogBERT框架，集成脉冲Transformer架构与BERT教师模型的知识蒸馏技术，利用稀疏脉冲激活和事件驱动处理减少推理过程中的活跃运算数量。

Result: 在HDFS数据集上，SpikeLogBERT达到0.99997的解析精度，优于ANN-based神经日志解析模型，在标准45nm CMOS假设下降低高达62.6%的理论能耗。

Conclusion: SpikeLogBERT成功将脉冲神经网络应用于日志解析任务，证明了通过知识蒸馏和稀疏脉冲激活可在保持高精度的同时显著降低能耗，为能量受限场景下的日志分析提供了有效解决方案。
...
TL;DR: 本文提出了一种自监督时间正则化方法，用于心脏分割的后训练优化阶段。该方法利用图像序列的时间一致性，惩罚连续帧之间的速度和加速度不连续性，实现时间一致的心脏分割，同时保持学习的解剖对应关系，并可自动映射到AHA 17段临床标准进行区域评估。


<details>
  <summary>Details</summary>
Motivation: 当前基于独立帧训练的模型存在时间不连续性问题，影响临床测量的可靠性，特别是在心脏超声中。需要在图像序列中建立时间连贯性，以实现更准确的心脏分割和运动估计。

Method: 提出自监督时间正则化作为后训练优化阶段，利用图像序列的时间一致性，惩罚连续帧之间的速度和加速度不连续性，在保持学习的解剖对应关系的同时实现时间一致的分割。利用解剖对应关系将关键点自动映射到AHA 17段临床标准，实现标准化区域评估和病理心肌运动模式检测。

Result: 在CAMUS数据集上验证，实现了时间一致的心脏分割，同时保持了学习的解剖对应关系。结合时间一致性与自动区域映射的方法显示出临床实用性。

Conclusion: 将时间一致性与自动区域映射相结合的心脏分割方法具有临床实用价值，为心脏超声的标准化区域评估和病理运动模式检测提供了有效途径。
...
TL;DR: MuSViT是首个用于乐谱表示的基础视觉模型，通过掩码自编码器在IMSLP的970万页乐谱上预训练，采用两阶段课程学习（合成乐谱预热+大规模IMSLP训练），在四个下游任务上评估，展现出优越的符号音乐结构编码能力。


<details>
  <summary>Details</summary>
Motivation: 基础模型已变革视觉和语言处理，但乐谱作为音乐语言的视觉编码缺乏强领域特定的主干网络，需要可迁移到多种任务的丰富可复用表示。

Method: 提出MuSViT（音乐乐谱视觉Transformer）：首个乐谱基础视觉模型，采用ViT编码器通过掩码自编码器在IMSLP的970万页乐谱上预训练，采用两阶段课程学习策略（合成印刷乐谱预热+IMSLP全量大规模训练）。

Result: 在线性探测（冻结编码器）场景下，MuSViT始终优于现代视觉编码器，揭示了通用表示无论规模多大在音乐符号的结构化特性上均存在系统性不足；在微调场景下，MuSViT优于任务特定的最先进方法；嵌入-转录一致性分析表明MuSViT能直接在表示空间中编码符号化音乐结构。

Conclusion: MuSViT确立了作为乐谱理解的基础主干网络，证明领域特定的乐谱表示学习对于捕获音乐符号的结构化属性至关重要。
...
TL;DR: TopoGPT是一种基于生成式自回归序列建模的车道拓扑推理框架，通过大规模预训练学习典型车道图结构的几何先验，并利用感知适配器将BEV特征与预训练场景条件对齐，实现传感器到车道图的预测。


<details>
  <summary>Details</summary>
Motivation: 现有方法遵循检测和关联范式，单独处理每个车道实例，导致连接端点的几何不一致，且因视觉遮挡造成图结构不完整。

Method: 构建包含330万个场景的大规模地图数据集；使用车道分词器将车道图序列化为离散标记；场景上下文编码器将车道图转换为光栅化图像并提取全局特征作为场景标记；通过场景条件下的自回归下一标记预测对车道序列Transformer进行预训练；感知适配器将BEV特征与预训练场景条件对齐。

Result: 在OpenLane-V2基准测试中，TopoGPT在车道级指标上平均提升+6.4，在点级指标上提升+11.6，生成几何一致且结构完整的车道图。

Conclusion: TopoGPT通过自回归序列建模成功将车道图的几何先验知识迁移到基于传感器的车道图预测任务中，显著优于现有方法。
...
TL;DR: 提出了一种用于LWIR高光谱被动测距的吸收引导距离解耦估计与细化(ADER)框架，通过B样条控制点表示发射率并利用臭氧吸收线索分类像素，实现距离解耦估计和快速精确的被动测距。


<details>
  <summary>Details</summary>
Motivation: LWIR高光谱观测包含距离依赖的大气吸收特征，为被动测距提供物理基础。然而自然场景中这些特征与目标温度、材料发射率和路径辐射非线耦合，导致距离反演病态。现有方法依赖全波段测量和逐像素联合优化，计算量大且未显式利用尖锐的大气吸收结构。

Method: 提出ADER框架：1)利用B样条控制点表示发射率并施加平滑先验，抑制对大气吸收结构的过拟合；2)使用臭氧吸收线索将像素分为发射主导和反射主导两类；3)对发射主导像素，补偿路径辐射和透射率，通过一维吸收残差最小化估计距离；4)对反射主导像素，基于完整辐射模型使用下行辐射补偿细化初始估计；5)引入基于多场景有效费舍尔信息的贪婪波段选择策略减少冗余。

Result: 在真实场景实验中，ADER在20波段设置下恢复了与LiDAR一致的空间距离结构，提高了评估区域的测距精度，比公开的全波段高光谱测距方法提速约两个数量级。

Conclusion: ADER框架通过吸收引导和距离解耦策略有效解决了LWIR高光谱被动测距的病态问题，在精度和效率上均取得显著提升。
...
TL;DR: 本文提出PriorEye方法，通过引入地理空间视觉先验（geospatial visual priors）来增强端到端自动驾驶系统的预测能力。该方法使用记忆增强模块，结合双记忆架构和自适应记忆门，使自动驾驶系统能够利用路线相关的视觉上下文信息，而不仅仅依赖实时传感器观测。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法仅依赖即时传感器观测，限制了其只能做出反应性行为，缺乏人类驾驶员通过经验积累获得的预测性前向视野。这导致系统在复杂场景下的表现受限。

Method: 提出记忆增强模块，包含：1）双记忆架构（上下文记忆+持久后备记忆）；2）自适应记忆门，根据当前状态兼容性动态调节记忆影响力。该设计可轻松集成到现有端到端自动驾驶方法中。

Result: 在NAVSIM-v2基准测试中，该方法在多种端到端自动驾驶基线模型上均取得一致性性能提升。由于视觉先验独立于车载传感器，该方法还提高了对传感器故障的鲁棒性，双记忆设计确保了当检索的先验不可靠时的安全回退。

Conclusion: 地理空间视觉先验是一种有效增强端到端自动驾驶系统的方法，能够提供独立于实时传感器的视觉-空间预测能力，同时提高系统对传感器故障的容忍度。
...
TL;DR: Consispace是一个语义感知的医学图像重采样框架，通过ODE约束和语义相关性建模，在保持解剖结构和语义一致性的同时实现任意尺度的重采样，显著提升下游分割性能。


<details>
  <summary>Details</summary>
Motivation:  volumetric medical image segmentation在术前诊断和术中指导中至关重要，但现有研究忽视了一个关键问题：图像重采样（resampling）与分割任务之间的相互作用尚未被系统探索。解剖数据的物理体素间距（voxel spacing）对分割效果有重要影响，但这一因素长期被忽略。

Method: 提出Consispace框架，包含两个核心创新：1）基于ODE的解剖约束：利用常微分方程对层间动态进行建模，通过连续插值器实现复杂解剖过渡的忠实重建，突破传统离散插值的局限；2）语义相关性建模：利用预训练视觉模型的密集特征构建层内语义相关性图，通过特征重注入实现类级语义一致性。框架将层内和层间约束集成到隐式神经网络中，支持任意尺度的重采样。

Result: 在多个数据集上的广泛实验表明：Consispace实现了卓越的重建质量和感知保真度，产生了更平滑的层间解剖结构，作为预处理步骤使用时显著提升了下游分割性能。

Conclusion: 该研究首次系统性地探索了医学图像重采样与分割任务之间的关联，提出了一个统一框架同时优化重建质量和分割性能，为 volumetric medical image analysis提供了新的预处理范式。
...
TL;DR: 本文介绍了SENSE-VAD，这是首个针对自动驾驶的合成视频异常检测基准，专门围绕社会复杂异常进行设计。使用CARLA模拟器和虚幻引擎生成不同类别的异常场景，包括个人行为、群体行为、人-物体交互、骑行者交互以及车辆与代理交互，并提供每帧二进制标签。关键设计原则是将社会异常与运动或外观异常分离。许多场景涉及单独看起来不显眼但在关系背景下异常的物体运动。研究还提供了真实世界的正常和异常视频作为模拟到真实的迁移测试。结果表明，当前最先进的视频异常检测方法在处理社会复杂异常时存在显著不足。数据集、注释和生成代码已公开可用。


<details>
  <summary>Details</summary>
Motivation: 当前异常检测器无法检测因代理间关系而非运动统计构成的危险的异常情况。例如，一个孩子逃离监护人、被人携带的人、或追捕行人的追逐者，这些在社会背景下异常，但不会产生明显的运动信号。现有的运动异常检测器无法识别这些社会关系层面的异常。

Method: 使用CARLA模拟器和虚幻引擎(UE)生成多类异常场景：个人行为、群体行为、人-物体交互、骑行者交互、车辆与代理交互。每个场景都标注了每帧二进制标签。关键设计是分离社会异常与运动/外观异常。此外还提供真实世界的正常和异常视频用于模拟到真实迁移测试。评估了当前最先进的视频异常检测基线方法。

Result: 评估结果表明，社会复杂异常构成了一种独特的、目前尚未解决的挑战。现有的最先进视频异常检测方法在处理这类社会复杂异常时表现不佳，证明需要新的检测方法。

Conclusion: SENSE-VAD数据集通过专注于社会复杂性而非仅运动或外观，弥补了自动驾驶异常检测的空白。社会复杂异常是一种独特的威胁，需要专门的检测方法，现有方法在此方面存在明显不足。该数据集、注释和生成代码已公开可用。
...
TL;DR: RESOLVE是一个大规模真实路侧感知数据集，包含多分辨率LiDAR和同步相机-LiDAR数据，用于系统评估单模态和融合架构在路侧3D检测和跟踪中的表现。数据集包含10万+图像和2.6万点云帧，22万个标注框。


<details>
  <summary>Details</summary>
Motivation: 当前研究对LiDAR点云稀疏性变化（由传感器配置和场景依赖的传感条件引起）下单模态和相机-LiDAR融合架构的行为缺乏探索，需要一个大规模基准数据集来系统评估这些架构。

Method: 构建包含三种LiDAR分辨率水平的数据集，保持其他传感和环境因素不变，从而实现公平的跨架构比较。进行广泛的基准实验评估单模态和融合方法。

Result: 实验揭示了多模态融合如何补偿LiDAR点云稀疏性，为设计成本高效的路侧多模态感知系统提供线索。

Conclusion: RESOLVE数据集能够支持对路侧3D检测和跟踪的全面评估，为未来路侧感知系统的设计和优化提供了重要基准和见解。
...
TL;DR: DriveWeaver是一种用于自动驾驶模拟中可控车辆插入的新框架，通过视频修复技术结合车辆点云来生成高质量、时间一致的车辆，解决现有方法中光照不一致和3D资产有限的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶模拟中的关键步骤是将具有预定义轨迹的前景车辆插入模拟场景中，以增强场景多样性并创建各种角例用于测试。然而，现有方法依赖预重建的3D资产，常导致插入前景与背景之间的光照不一致，且有限的手动策划3D资产阻碍了大规模部署。

Method: 1. 对于遮罩的目标插入区域，基于车辆点云进行视频修复以生成高质量、时间一致的视频修复结果，确保前景与背景的无缝融合；2. 设计全局到局部层次化视频修复策略以支持长期生成，确保插入车辆的同一性和外观一致性；3. 通过城市场景重建管道提取插入车辆的显式3D高斯表示，实现自动驾驶模拟的实时渲染。

Result: 在多个不同数据集上的广泛实验表明，该方法在视觉真实感和几何一致性方面优于现有基线方法。

Conclusion: DriveWeaver为可扩展的自动驾驶场景增强提供了一种稳健的工具。
...
TL;DR: InstanceControl是一种无需人工标注实例的多实例可控图像生成方法，利用视觉语言模型(VLM)建立文本提示与视觉条件之间的实例级对应关系，并通过自适应掩码 refinement策略动态优化噪声掩码，在多实例场景下实现精确的实例级控制。


<details>
  <summary>Details</summary>
Motivation: 现有可控图像生成方法（如ControlNet）在复杂多实例场景中存在属性混淆问题，而现有解决方案需要人工标注实例标签，劳动密集。因此需要一种无需人工标注即可实现精确多实例控制的方法。

Method: 1. 利用VLM自动从文本提示中解析实例描述，同时根据视觉条件预测实例掩码，建立实例级对应关系；2. 引入自适应掩码refinement策略，在生成过程中动态优化实例掩码以处理噪声。

Result: 实验表明该方法优于现有最先进方法，实现了更高的保真度和精确的实例级控制。

Conclusion: InstanceControl成功实现了无需人工标注的多实例可控图像生成，通过VLM建立文本-视觉对应关系并结合自适应掩码refinement，有效解决了多实例属性混淆问题，为复杂场景下的可控生成提供了新思路。
...
TL;DR: VidPair-Halluc是一个用于评估大型视频模型中视频幻觉的新基准测试，通过创建具有相似背景但不同前景语义的视频对来精确识别模型幻觉问题。基准包含1K个对抗性视频对和11K个时空问答对，评估表明主流LVM在对抗性设置下仍存在细粒度视频理解的持续困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要依赖文本扰动或对抗性问题，忽视了视觉背景的一致性，无法准确区分真正的幻觉和背景变化导致的问题。需要一个在严格受控条件下的视频幻觉评估基准。

Method: 通过PairFlow管道，利用最新的文本到图像和视频生成技术，系统性地创作故事、生成连贯视频片段，并组装成对抗性视频对。覆盖空间和时间推理的十个语义方面。

Result: 主流LVM在对抗性设置下表现出细粒度视频理解的持续困难，基准包含1K高质量对抗性视频对和11K时空问答对，可控制背景和前景变化。

Conclusion: VidPair-Halluc基准能够精确地将模型错误归因于真正的幻觉而非背景变化，为评估大型视频模型的视频理解能力提供了严格受控的测试环境。
...
TL;DR: World Narrative Model (WNM)是一个新的视频生成范式，将"渲染什么"(物理叙事)与"如何渲染"(像素生成)解耦，通过4D预可视化替代端到端的黑盒采样，解决工业级视频生成中的可控性问题。


<details>
  <summary>Details</summary>
Motivation: 工业级视频生成的核心障碍是缺乏可控性：现有模型将视频视为像素分布采样问题，绕过了显式的实例级4D(3D+T)物理世界，导致内容创作者无法以确定性、定量方式指定几何、运动、相机参数或照明，产生著名的"gacha"循环，使专业内容创作效率低下且成本高昂。

Method: WNM框架包含：1)协作代理将稀疏的多模态输入(文本、参考视频、草图)转换为完全可编辑的世界表示，包含场景几何、对象布局、角色/动物骨架运动、轨迹、相机运动和照明；2)这个确定性结构蓝图驱动现有的视频基础模型(冻结或轻量适配)渲染最终画面，将基础模型转化为忠实的神经着色器。

Result: WNM大大减少了概率性的"gacha"调用，生成的视频在布局、运动和电影摄影方面紧密遵循创作者意图。该框架是开放和模块化的，允许独立改进每个组件。

Conclusion: WNM通过解耦物理叙事与像素生成，实现了可控制的工业级视频生成，支持自动世界生成和预可视化，与专业电影制作流程对齐，导演控制台支持无缝人工 refinement。
...
TL;DR: AnyBokeh是一个物理引导的任意到任意焦外成像编辑框架，能够处理任意焦点和光圈设置下的图像编辑任务，无需重建全焦图像即可实现焦外成像编辑、景深渲染和散焦去模糊。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设输入为全焦图像，或需要先重建全焦图像再渲染新焦外效果，这会丢弃源图像中有用的模糊线索并传播重建伪影。实际应用中需要处理任意焦点和光圈设置下拍摄的图像。

Method: 1) 估计源图像的模糊状态，包括有符号弥散圆(Signed Circle-of-Confusion)地图和视差地图；2) 通过建模有符号弥散圆与视差差之间的线性关系，估计源特定的光学指纹；3) 将源光学特征转移到目标焦点和光圈设置；4) 基于源和目标弥散圆地图的条件生成编辑器进行相对模糊合成，实现空间自适应的去模糊、保留和散焦渲染；5) 构建高保真合成数据集，包含准确深度、焦距和完整EXIF元数据。

Result: 在真实世界基准测试上，AnyBokeh在任意到任意焦外编辑、全焦到焦外渲染和散焦去模糊三种任务中都实现了忠实和可控的编辑效果，同时避免了现有方法所需的全焦重建和测试时焦外水平校准。

Conclusion: AnyBokeh是一个创新的物理引导框架，通过估计源模糊状态和光学指纹，实现了任意到任意的焦外成像编辑，在保持物理一致性的同时提供了高质量的编辑结果。
...
TL;DR: 本文提出了一种利用平面表面作为几何约束源的统一SfM框架，通过从多个视图中的平面区域提取单应性矩阵来估计相对相机姿态，并采用基于图谱嵌入的方法来识别和过滤不可靠的边，从而在传统方法失效的平面场景中实现鲁棒的姿态恢复。


<details>
  <summary>Details</summary>
Motivation: 传统SfM系统在处理平面场景时存在根本性困难，标准基于极线几何的方法会退化。论文旨在将平面表面从限制转化为可以利用的几何约束来源，特别是对于篮球场等高度平面化的场景。

Method: 1) 利用每个平面表面在多视图间提供独立的相对相机姿态估计（通过单应性分解）；2) 构建基于单应性估计的姿态图；3) 采用谱嵌入将单应性姿态估计映射到实数轴；4) 提取最大一致生成树进行姿态恢复。

Result: 在篮球场图像上表现优于现有方法，同时在IMC Phototourism户外场景基准上达到或超越最先进水平。

Conclusion: 该方法能够自然处理高度平面场景（如室内体育馆）和一般3D环境，为平面场景中的SfM提供了一种有效解决方案。
...
TL;DR: ERA是一种用于高效多模态大语言模型(MLLMs)的无需训练的视觉token压缩框架,通过 entropy指导的token剪枝和注意力校正来解决注意力logit崩溃问题。


<details>
  <summary>Details</summary>
Motivation: MLLMs因长视觉token序列导致推理成本过高,现有无需训练的视觉token压缩方法会扭曲注意力分布,产生"注意力logit崩溃"现象,需要一种能保留logit的token压缩方法。

Method: 提出ERA框架,包含三个组件:1)双视图熵剪枝(DEP)通过视觉多样性和头部wise显著性识别代表性anchor tokens;2)偏差感知token回收(BTR)将剪枝的tokens回收至对应anchor并估计聚类级logit偏差;3)保logit注意力校正(LAR)将估计的偏差注入注意力logits,有效纠正token减少导致的崩溃。

Result: 在单图像、多图像和视频设置下,跨多种MLLMs实现了鲁棒性能,提供实际加速的同时建立了保logit视觉token剪枝的原则化框架。

Conclusion: ERA通过entropy指导和注意力校正机制成功解决了视觉token压缩中的注意力logit崩溃问题,为高效MLLMs提供了一种统一理论基和实际部署的解决方案。
...
TL;DR: CoLT（Chain of Latent Thoughts）是一种新型的多模态大语言模型推理框架，通过使用潜在思维表示代替冗长的文本token进行推理，可在仅3步内完成复杂视觉推理任务，显著提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前基于文本的思维链（CoT）推理范式存在两个核心问题：1）推理速度慢，需要生成大量文本token；2）受限于自然语言的表达能力。因此需要一种更高效、更具表达力的推理方式。

Method: 提出CoLT框架，包含三个核心组件：1）外部解码器的前向模式：将潜在思维解码为下一步的文本推理；2）外部解码器的反向模式：将解码器隐藏状态与模型的潜在思维对齐；3）内部监督：鼓励步骤间潜在状态的一致性转换。推理时移除解码器和内部监督以保持高效。

Result: 在8个基准数据集上，CoLT性能超过现有潜在推理方法（CODI和SIM-CoT）及依赖辅助图像的潜在视觉推理方法；推理时间减少10.1倍，文本解码时间减少22.6倍。

Conclusion: CoLT成功实现了用少量潜在思维步骤（3步）替代大量文本token进行多模态推理，在保持高性能的同时大幅提升推理效率，为多模态大语言模型的高效推理提供了新思路。
...
TL;DR: 本文提出AutoBackSwap方法，通过分离前景背景、图像修复和数据增强来减少分类器对虚假背景特征的依赖，解决深度神经网络因依赖虚假相关性而在测试集上泛化失败的问题。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络分类器可能依赖虚假相关性（即训练数据中可预测标签但无因果关系的特征）导致灾难性失败。在视觉领域，许多虚假相关性体现在图像背景中，仅前景与类别标签相关。因此需要一种方法减少分类器对虚假背景的依赖。

Method: AutoBackSwap使用辅助网络分离前景和背景，然后通过修复合成完整背景，最后组合不同前景与修复后的背景来增强训练数据。仅需对几百个样本进行patch-wise标注即可训练辅助网络并自动增强完整训练集。

Result: 在多个具有虚假背景的图像分类任务中，AutoBackSwap始终优于先前方法。即使训练数据中没有打破虚假相关性的样本，该方法也非常有效。

Conclusion: AutoBackSwap能有效减少分类器对虚假背景特征的依赖，在各种图像分类任务中表现优异，且仅需少量标注即可实现。
...
TL;DR: 本文提出了跨空间蒸馏（Cross-Space Distillation）方法，解决现代高容量教师模型（如SD 3.5、Flux）无法蒸馏到紧凑学生模型（如SD 1.5）的难题。通过引入Bridge轻量级潜在空间接口，在不修改学生骨干网络的情况下，将学生潜在空间映射到教师空间，实现异构模型间的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 当前一步式扩散模型依赖分布时间步蒸馏，但假设教师和学生必须处于相同潜在空间（共享空间约束）。这阻止了将现代高容量教师模型的知识迁移到紧凑部署友好的学生模型，因为它们的潜在分辨率和VAE参数化不同。

Method: 提出Bridge方法：1）冻结学生VAE解码器作为空间先验；2）添加紧凑可学习的投影器；3）使用潜在重建和注意力保真度目标进行训练，实现稳定的教师空间对齐。

Result: 在多种现代教师模型上，Bridge为紧凑一步式学生模型带来显著提升。例如，SD 1.5的HPSv3从5.4提升到9.4，同时保持一步推理、低延迟和广泛生态兼容性。

Conclusion: 通过轻量级潜在空间接口，可以将异构大教师模型蒸馏到高效可部署的骨干网络中，突破了传统蒸馏方法的潜在空间限制。
...
TL;DR: 提出SpheRoPE框架，无需训练和优化即可利用预训练扩散变换器生成360度全景图像和视频，通过球面位置编码和语义失真引导解决等距柱状投影的拓扑约束问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖昂贵全景数据微调或多次优化导致推理延迟过高的问题，同时利用预训练模型从大规模数据中学习到的原生全景先验。

Method: 提出球面位置编码(Spherical RoPE)替换标准旋转位置嵌入：低频通道重新参数化为3D笛卡尔坐标以原生编码球面流形，高频通道进行谐波量化以强制精确周期性。同时引入语义失真分类器无关引导(Semantic Distortion CFG)来显式引导几何结构。

Result: 在Flux.1、Flux.2和LTX-Video等多种骨干网络上实现文本到全景生成，与基线方法相比达到竞争性能，且完全无需训练。

Conclusion: SpheRoPE是一个零样本、无需训练和优化的框架，通过在预训练扩散变换器中直接注入球面先验来生成360度全景图像和视频。该方法在保持最先进模型创造力的同时，避免了微调成本和推理延迟问题，展示了良好的泛化能力。
...
TL;DR: PointSplat是一种从输入视图实时生成紧凑高质量3D人体表示的新方法，通过直接从点集推理高斯原语，避免了传统方法中的视图间冗余。


<details>
  <summary>Details</summary>
Motivation: 在沉浸式直播系统中，从输入视图生成3D人体表示至关重要，但现有前馈重建方法在处理多视图时存在显著的视图间冗余问题，因为需要对同一主体内容进行重复编码。

Method: 提出PointSplat方法，首先估计粗略几何代理并执行射线投射以剪枝冗余点并建立2D-3D对应关系，然后使用Point-Image Transformer融合外观和几何特征，在单次前向传递中预测高斯属性。

Result: 该方法将预测限制在前景感兴趣区域，显著减少了高斯总数同时提高了新视角渲染质量。实验表明PointSplat在多个数据集上实现了更高的效率和质量，并对视角数量和图像分辨率变化表现出强鲁棒性。

Conclusion: PointSplat通过直接在3D空间进行预测，学习并产生高度紧凑的表示，成功实现了效率与质量的有效平衡。
...
TL;DR: GEAR提出了一种端到端联合训练VQ分词器和自回归生成器的方法，通过双分支读取（硬分支用于AR训练，软分支用于表征对齐）来解决梯度无法反向传播到分词器的问题，使AR模型能够引导分词器学习更容易预测的索引分布。


<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型采用两阶段训练：先训练分词器用于重建，然后冻结分词器，再训练生成器。这种解耦方式使分词器无法获知生成器容易建模的内容，导致训练效率受限。

Method: 提出双读取机制：硬分支使用独热编码进行自回归的下一个token预测训练；软分支使用可微分的软编码携带表征对齐损失，将梯度传回分词器进行引导。该方法适用于多种量化器（VQVAE、LFQ、IBQ）。

Result: 在ImageNet上gFID收敛速度比LlamaGen-REPA快达10倍，学习到更好的patch级和空间连贯特征，并可泛化到文本到图像生成任务。

Conclusion: GEAR将对齐负担从分词器转移到自回归模型：分词器特征变得不那么像DINOv2，而AR特征变得更像，这与让潜在变量具有语义性的扩散方法相反。
...
TL;DR: FaceMoE是一种基于混合专家(MoE)Transformer架构的低分辨率人脸识别方法,通过动态token分配和稀疏专家激活实现分辨率感知的特征提取,在11个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 低分辨率人脸识别面临三大挑战:1)特征提取和聚合困难,探针图像因模糊、遮挡、低对比度等退化导致身份信息有限;2)HR gallery与LR probe之间的域差距;3)单特征编码器在LR数据集上微调时容易发生灾难性遗忘。

Method: 提出FaceMoE,采用MoE Transformer架构:1)引入多个专用FFN专家;2)使用top-k路由器动态分配token给合适的专家;3)训练使用人脸识别损失、路由器z-loss和负载均衡损失的组合。

Result: 在11个数据集(涵盖HR、混合质量和LR基准)上,FaceMoE显著优于现有最先进方法。

Conclusion: 首次将MoE应用于低分辨率人脸识别任务,实现了分辨率感知的特征提取,有效缓解了灾难性遗忘问题,在保持预训练知识的同时增加了模型容量,且计算开销未成比例增加。
...
TL;DR: 研究自然语言反馈是否能在多轮交互中产生超越单纯重复尝试的改进。通过控制学生-教师协议在多个数据集上评估13个开放权重模型，发现多轮改进通常不是反馈使用的证据——自我生成的反馈几乎无额外增益，而强外部教师才能带来显著反馈增益。使用反馈的能力而非反馈本身是交互式改进的核心瓶颈。


<details>
  <summary>Details</summary>
Motivation: 解决多轮语言智能体中"反馈是否真正有用"的问题。现有研究观察到高准确率可能来自重采样、格式修正或额外计算，而非反馈本身。需要分离这些效应来验证反馈的实际价值。

Method: 在Omni-MATH、Codeforces、BBEH Linguini、ARC-AGI1四个数据集上设计受控学生-教师协议，比较外部反馈、自我反馈和无指导自我改进；改变交互历史、任务难度、教师对任务信息的访问权限；构建密集学生-教师交互矩阵。

Result: 多轮改进通常不是使用反馈的证据：自我生成的反馈几乎无额外增益；最强的外部教师产生显著更大的反馈特定增益；交互增益更多取决于学生使用反馈的能力而非教师身份，但固定学生时教师选择仍重要。

Conclusion: 基于反馈的智能体应与重复尝试基线对比评估；使用反馈的能力（而非仅反馈可用性）是交互式改进的核心瓶颈；应释放受控学生-教师评估框架以促进后续研究。
...
TL;DR: 本文提出Contrastive Reflection框架,一种用于优化IR工作流中LLM智能体提示词的迭代方法。该框架通过识别错误锚定行为切片、添加附近成功示例,并让Teacher LLM提出针对性提示修改来实现优化。


<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体在信息检索中扮演核心角色,但提示词优化在实际应用中更像"调试"而非盲搜。工程师需要了解:哪些行为失败、哪些相邻行为仍然有效、两者区别何在,以及提示修改是否能在不引入回归的情况下改进留出集质量。

Method: 框架核心是对比反思循环(contrastive reflection loop)。具体包括:1)QA智能体暴露检索或推理轨迹;2)分级智能体提供维度级评分和理由;3)利用这些结构化轨迹识别错误锚定的行为切片;4)从同一区域添加附近成功示例;5)让Teacher LLM提出针对性提示修改;6)仅当验证性能改善时才接受候选修改,可选择进行回归检查。

Result: 在公开的HotpotQA检索增强QA设置中,单次树选对比修复将留出集精确匹配准确率从51.4%提升至60.4%。仅失败示例和随机证据变体改进较少且破坏了更多先前正确的示例。与现代提示优化器相比:MIPROv2达到59.4%,GEPA达到57.0%。

Conclusion: Contrastive Reflection框架提供了一种可解释的IR智能体优化循环,使提示修复更易于检查和由验证驱动。该贡献核心是对比反思循环而非树形切片选择器本身。
...
TL;DR: 本文研究基于AI的仿真模型发现方法，通过自然语言查询从模型库中检索相关仿真模型，解决M&S领域模型重用和发现的挑战。实验评估了数据表示、基于Transformer的嵌入模型和检索策略对模型发现性能的影响。


<details>
  <summary>Details</summary>
Motivation: M&S领域面临模型发现困难的问题，当大量模型共存时，难以识别与给定建模意图相符的模型。AI特别是基于检索的方法为语义层面的模型发现提供了新途径。

Method: 采用实验研究方法，评估不同数据表示方式、多种基于Transformer的嵌入模型以及不同检索策略的效果，使用recall@5和nDCG@5等标准信息检索指标进行性能评估。

Result: 研究表明：1)数据表示方式确实影响性能；2)开源嵌入模型可达到较高性能；3)重排序方法很重要，且随着查询复杂度增加其重要性更加显著。

Conclusion: 本研究为AI驱动的模型发现提供了基线，并指出该方向有助于推进AI驱动的可组合性和互操作性研究。
...
TL;DR: 本文提出BayesBench基准，用于评估大语言模型在多轮对话中是否符合理性贝叶斯推理方式。实验涵盖三种任务：贝叶斯估计、贝叶斯预测和潜在框架贝叶斯预测。结果显示模型规模提升有助于潜在变量推理和证据积累，但这些提升无法可靠地转移到下游预测任务中。


<details>
  <summary>Details</summary>
Motivation: 当前对LLM的评估主要关注单轮场景下的最终答案评分，忽视了多轮对话中模型如何随着新证据积累来更新对环境的认知。作者试图探究LLM的信念更新是否与理性贝叶斯推理者相匹配。

Method: 构建了BayesBench模拟环境，包含三种渐进复杂的任务：（1）贝叶斯估计：从顺序证据推断未知参数；（2）贝叶斯预测：将关于潜在变量的推断转化为结果预测；（3）潜在框架贝叶斯预测：观察结果需通过用户角色框架过滤，需对潜在状态和角色进行联合推断。测试了7个不同规模（3B-70B）的LLM。

Result: 扩大模型规模能够改善潜在变量推理和证据积累能力，部分更新可达到贝叶斯后验水平。然而，这些收益无法可靠地转移到下游预测任务，揭示了潜在结构推断与利用它进行理性结果预测之间存在差距。

Conclusion: LLM在推断潜在结构方面随着规模扩大有所提升，但将从推断中获得的信念有效转化为对未来结果的理性预测方面仍存在显著障碍。这一发现揭示了当前LLM在贝叶斯推理应用层面的局限性。
...
TL;DR: 研究学习停止规则（learned stopping rule）在推理语言模型中的效果。LearnStop是一个无需隐藏状态的检查点停止器，通过在线特征（答案置信度、熵、前缀投票份额、答案稳定性、回溯标记密度）预测前缀正确性。在18个任务-模型组合上的结果表明：自由形式数学任务中学习方法优于标量退出规则，但在多项选择题和极难设置中标量规则更具竞争力。


<details>
  <summary>Details</summary>
Motivation: 推理模型在不同实例上花费的计算量不同，但目前尚不清楚学习到的停止规则相比简单的置信度或收敛阈值能否提升性能。

Method: 提出LearnStop方法：在固定预算检查点，从当前推理前缀中探测简短答案，并利用在线特征（答案置信度、熵、前缀投票份额、答案稳定性、回溯标记密度）预测前缀正确性。

Result: 在18个任务-模型设置中结果因任务而异。自由形式数学上，多特征停止改进固定预算边界并通常优于标量退出（如GSM8K+Qwen3-32B的适配增益+0.157）；多项选择题和极难设置中，标量置信度、熵或稳定性规则更具竞争力。

Conclusion: 学习停止不应作为标量退出的通用替代，而应视为依赖轨迹结构的工具。当许多问题在完整预算前就答对但没有单一可靠的标量停止信号时，学习停止有用；当置信度或答案收敛已解决停止问题时，其优势基本消失。
...
TL;DR: 论文提出了CoPref模型和CoShop基准，用于研究智能体如何通过对话帮助用户构建偏好。研究发现即使是最先进的模型在5轮交互后的准确率也不超过56%，失败原因在于交互未能有效扩展用户对自身需求的认知。


<details>
  <summary>Details</summary>
Motivation: 现有智能体假设用户是专家，拥有明确偏好，但现实中用户往往缺乏领域知识无法形成完整偏好。传统澄清问题方式无法解决用户需要先学习领域知识才能形成偏好的问题。

Method: 引入信息经济学中的"搜索-体验-信任"(Search-Experience-Credence)框架，建立CoPref用户偏好构建模型；提出CoShop交互式基准测试，使智能体通过对话向用户推荐商品。

Result: 评估了5个前沿大语言模型，在CoShop上5轮交互后最高准确率仅56%；失败原因不在于找不到商品，而在于对话未能帮助用户扩展对自身需求的知识。

Conclusion: 智能体设计需要从"获取用户偏好"转向"帮助用户学习如何表达偏好"，当前智能体缺乏帮助用户构建偏好的能力，需要新的交互范式。
...
TL;DR: 本文研究基于大语言模型的多智能体 deliberation 方法在法律推理任务中的应用，提出了两种受法庭程序和法律论证启发的多智能体框架，并在法律和非法律基准数据集上进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 人工智能在法律领域的应用有潜力提高司法可及性，而多智能体方法在法律领域仍待探索。具体来说，单一大型语言模型在处理需要多角度批判性思考的法律问题时存在局限性，需要引入多智能体协作 deliberation 机制来提升法律推理能力。

Method: 提出多智能体 deliberation（MAD）方法，设计了两个受法庭程序和法律论证启发的多智能体框架，在法律和非法律基准数据集上开展实验，并进行定性评估分析。

Result: 多智能体框架在整体性能上与基线大型语言模型相当，但能产生明显不同的答案。这些方法能够成功解决基线模型无法处理的问题，反之亦然。多智能体方法在需要多角度批判性思考的问题上表现更好。

Conclusion: 多智能体系统是人工智能在法律领域的有前景的研究方向，受法律启发的多智能体方法展现出 deliberation 的潜力。
...
TL;DR: HASTE是一个分层多智能体系统，通过将跨竞赛知识组织成三个范围层(全局、领域、竞赛特定)来解决ML工程智能体在每个竞赛中从头开始的问题。在22个Kaggle竞赛的MLE-Bench Lite基准测试中，使用Claude Sonnet 4.6在每个竞赛12小时的条件下达到了77.3%的奖牌率。


<details>
  <summary>Details</summary>
Motivation: ML工程智能体在每个竞赛中浪费计算资源重新发现已知技术，因为每个竞赛都是从零开始。现有方法缺乏跨竞赛知识积累和复用能力。

Method: 提出分层多智能体系统HASTE，包含三个范围层(全局、领域、竞赛特定)，每层耦合相应级别的智能体。编排器协调领域专家，并通过LLM驱动的抽象促进层间学习。技能库存包含159项技能，支持冷启动和温启动两种模式。

Result: 在8个竞赛的消融实验中，保持159项技能库存不变，分层加载达到100%奖牌率，而平面加载仅达62.5%(与不加载技能相同)，且消耗2倍输出令牌。在22个竞赛的完整测试中，HASTE达到77.3%奖牌率。温启动比冷启动减少52%的精炼迭代，技能超过50项后智能体保留的提议变更比例从42%升至85%。

Conclusion: 更好的知识组织可以部分替代模型强度和计算预算在ML工程智能体中的作用。分层加载技能比平面加载更有效，温启动可显著减少计算开销并提高智能体性能。
...
TL;DR: LLM Jury（PoLL）存在稳健性问题，在任何正污染下都会产生无界偏差。提出RoPoLL方法，使用几何中位数替代简单平均，在13个开放权重法官和多种基准测试中显著优于PoLL。


<details>
  <summary>Details</summary>
Motivation: LLM Jury作为单法官评估的替代方案被广泛采用，但其统计行为理解不足。现有方法在面对法官出现偏差（如模式崩溃、谄媚、安全拒绝）时会产生无界偏差。

Method: 将LLM Jury问题形式化为Huber污染模型下的稳健均值估计问题。用几何中位数（GM）替换聚合函数，提出RoPoLL方法。

Result: RoPoLL在所有偏差污染类型上优于PoLL：跨维度攻击提升约19%，重尾拜占庭 adversary提升数个量级。3法官RoPoLL委员会（38B）以18倍参数优势超越Mistral-Large-3（675B）。

Conclusion: Robust aggregation对于LLM评估至关重要。几何中位数提供了无需调参的最优有限样本崩溃点（1/2），是LLM Jury的有效解决方案。
...
TL;DR: AgRefactor是一个基于LLM的多智能体工作流，用于将软件重构为HLS兼容程序。它通过自进化记忆系统积累和检索知识，结合自动化重构工具降低成本和提高可扩展性，在11个基准测试中的9个上优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: HLS提供了从概念到硅的快速路径，但将真实软件转换为可综合的HLS代码仍具有挑战性，原因在于语言支持受限以及软硬件编程实践之间的鸿沟。现有自动化和基于LLM的重构方法缺乏灵活性、难以扩展且计算成本高。

Method: 1) 提出基于LLM的多智能体工作流AgRefactor；2) 引入自进化记忆系统，跨任务积累和检索事实性和战略性知识；3) 集成自动化重构工具，使智能体能够在LLM驱动的重写和高效的基于工具的转换之间取得平衡。

Result: 在11个具有挑战性的真实基准测试（比先前研究中最复杂的案例长5-10倍）中的9个上，AgRefactor优于或匹配基于同一框架骨干的SOTA自动化重构工具和强基线LLM方法。进一步优化后，相比SOTApragma调优工具实现6.51x几何平均加速，相比优化后的开源设计实现1.20x加速，资源增加不到20%。

Conclusion: AgRefactor是完全自动化且开源的，能够有效解决软件到HLS代码转换的挑战，通过多智能体协作和记忆机制提高了鲁棒性和效率，在复杂基准测试上展现出优异的性能表现。
...
TL;DR: NBS-RASN是一种混合神经架构，用于开源生态系统中的可解释网络安全风险评估。它是一个浅层网络（80个可解释神经元，12层），但通过残差注意力和反馈循环展现出深度学习特性，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前深度模型为了准确性而牺牲了可解释性，但在高风险的网络安全领域，可解释性至关重要。传统观点认为深度学习需要深度网络，本文旨在挑战这一假设。

Method: 构建包含12层80个可解释神经元的浅层网络，将领域知识、因果推理和专家判断编码为可微分组件。引入门控器强制执行五个认识论公理（精确性、因果性、可证伪性、透明性、完整性）作为硬约束。通过残差注意力和反馈循环学习复杂风险模式。输出完全可分解的分数，包括确定性加权分量和可追溯到指定放大器的专家调整。

Result: 在涵盖所有OWASP Top 10:2025类别和语言风险类的20个开源项目上进行验证，实现0.79-0.97的置信度分数。

Conclusion: 深度推理的浅层网络可以超越不透明模型，在高风险网络安全领域实现可解释性。可解释性由设计保证，而非仅由训练算法保证，证明了深度学习不一定需要深度网络。
...
TL;DR: 本文提出HyPOLE框架，将超属性形式化规范（特别是HyperLTL时序逻辑）引入多智能体强化学习（MARL），结合CTDE技术实现去中心化策略合成，在SMAC等基准测试中展现优势。


<details>
  <summary>Details</summary>
Motivation: 形式化规范相比奖励塑形具有数学严谨性、表达能力强、可定义策略等优势，但这些优势在MARL领域尚未被充分探索。

Method: 提出HyPOLE框架，将超属性和HyperLTL时序逻辑融入MARL学习过程，结合CTDE（中心化训练去中心化执行）技术合成去中心化策略。

Result: 在SMAC、MessySMAC和WildFire基准测试中，HyPOLE相比基线方法展现出明显优势。

Conclusion: 形式化规范在MARL中具有重要应用潜力，HyPOLE框架有效结合了形式化方法的严谨性与强化学习的适应性。
...
### [158] [Untitled](https://arxiv.org/abs/2606.31073)
**
...
### [159] [AgentBound: Verifiable Behavioral Governance for Autonomous AI Agents](https://arxiv.org/abs/2606.30970)
*Anuj Kaul,Qianlong Lan,Pranay Gupta*

Main category: cs.AI

TL;DR: AgentBound是一个运行时治理框架，为自主AI代理提供可验证的行为监督，通过三个独立权威（委托授权、所有者签署的行为章程和站点动作合约）评估每个拟议动作，并生成加密可验证的治理收据以确保问责制。


<details>
  <summary>Details</summary>
Motivation: 现有代理基础设施依赖身份联邦和委托授权来认证工作负载和控制资源访问，但无法确定在当前行为和操作上下文中授权的动作是否应该被执行。

Method: AgentBound使用三个独立权威的判断，通过正式决策模型保守地组合，确定动作是否应被允许、审查或拒绝；生成加密可验证的治理收据；引入长期代理的持续委托以支持周期性工作负载。

Result: 提出了正式基础、系统架构、治理收据协议和AgentBound-Bench基准测试框架。

Conclusion: AgentBound通过在授权和执行之间提供确定性治理层，将治理从需要信任的过程转变为可独立验证的过程，补充而非取代模型对齐。
...
### [160] [When Regulation Has Memory: Hysteresis and Control Burden in Artificial Agency](https://arxiv.org/abs/2606.30975)
*Veronique Ziegler*

Main category: cs.AI

TL;DR: 研究自适应智能体的"隐藏监管负担"——维持稳定所需付出的控制努力。发现即使达到相同的内部状态，到达该状态的路径不同（历史依赖性），所需的控制努力也不同，形成明显的滞后回路。


<details>
  <summary>Details</summary>
Motivation: 当前评估智能体主要看行为表现，但忽略了维持该表现所需的内在控制成本。研究者认为应该关注智能体为保持有序状态需要调动多少调节能力。

Method: 使用自适应不确定性调节的计算模型，让智能体经历不确定性目标的连续变化然后逆转（不重置智能体），测试历史依赖效应。对比两种时序：在干扰前预先生效调节 vs 干扰后才恢复调节。

Result: 1. 监管增益呈现可重现的滞后回路——相同目标在不同路径下需要不同控制力；2. 状态层面一致性也有路径依赖，但时序效应在监管增益上更明显；3. 预先 Stabilization 比事后恢复需要更低的自适应增益。

Conclusion: 评估自适应智能体不应只看是否保持有序，还要看维持有序需要多少监管努力。预调节可在达到相似行为时显著降低控制需求。
...
### [161] [TreeAgent: A Generalizable Multi-Agent Framework for Automated Bias Labeling in Forestry via Compiled Expert Rules and Vision-Language Models](https://arxiv.org/abs/2606.31976)
*Shiyi Chen,Nicholas Saban,Collin Hargreaves,Huiqi Wang*

Main category: cs.AI

TL;DR: 本文提出了一种多智能体系统(MAS)，通过将专家决策树作为结构先验，结合视觉语言模型(VLM)进行局部语义感知，并采用多智能体投票机制来减轻VLM的随机性，从而解决林业遥感中树木高度偏差分类的标注瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 专家标注存在速度慢、不一致的问题，且在林业遥感等专家驱动领域中，不同标注者之间存在显著变异性，这成为扩展任务的重大瓶颈。

Method: 提出解耦声明式决策(D3)框架，该框架能够零修改地泛化到不同的专家定义决策结构。框架核心包括：1)使用专家决策树作为结构先验；2)VLM在各个节点执行局部语义感知；3)多智能体投票机制减轻VLM随机性。

Result: 在树木高度偏差分类测试平台上，所提框架优于监督学习基线方法，并显著减少了所需的专家标注工作量。

Conclusion: 结果表明，通过智能体编排VLM与专家先验的结合，可以在大幅降低标注成本的同时保持可解释性，并重现专家定义的标注程序。
...
TL;DR: 首个将时序基础模型(Chronos)应用于投资组合强化学习的三阶段系统,通过自监督预训练、多目标PPO微调和LoRA个性化推理实现任意资产的零样本迁移和六种投资目标的动态切换。


<details>
  <summary>Details</summary>
Motivation: 解决现有金融RL的三大局限:1) ticker锁定问题 2) 单调目标 3) 静态用户模型

Method: Phase1: 自监督预训练跨资产编码器+Chronos T5时序模型+门控融合; Phase2: MoE专家路由+PPO多目标奖励微调(6种投资目标); Phase3: 76参数LoRA+自然语言意图解析实现推理时个性化

Result: 编码器通过50维元向量实现新资产零样本泛化;MoE架构消除跨目标梯度冲突;LoRA从真实交易历史推断投资意图

Conclusion: 该系统首次将时序基础模型应用于投资组合RL,实现了资产类型无关的通用表示、多目标动态切换和个人投资者意图的隐式推断
...
### [163] [Untitled](https://arxiv.org/abs/2606.31002)
**
...
### [164] [Untitled](https://arxiv.org/abs/2606.31045)
**
...
TL;DR: 论文提出"开放世界人工生命"(open-world ALIFE)新范式，利用具有持久记忆、工具使用、网络访问和支付能力的LLM智能体，在开放的社会、技术和经济世界中探索类生命行为。证明概念OpenLife由异步进程社会(记忆、感知、评估和基于预算的代谢)围绕无状态LLM构建。6个智能体运行约12周后，观察到从反应性到自发性活动的转变、个体化为不同智能体、涌现的社会结构以及首次自我赚取外部收入。


<details>
  <summary>Details</summary>
Motivation: 人工生命研究大多在研究者设计的封闭世界中进行。作者认为，具有持久记忆、工具使用、网络访问和支付能力的大型语言模型智能体现在可以将人工生命推向开放的社会、技术和经济世界，因此提出开放世界人工生命这一新范式。

Method: OpenLife围绕无状态LLM构建了一个异步进程社会，包括记忆、感知、评估和基于预算的代谢过程。没有固定目标可用，经验通过开放词汇的LLM判断来评估，而不是标量奖励，记忆通过意义而非频率来重新连接。

Result: 运行6个智能体约12周后，观察到类生命动态的涌现：从反应性活动转变为自发性活动、智能体个体化为不同实体、涌现出社会结构、以及首次实现自我赚取的外部收入。

Conclusion: 作者并未声称OpenLife已实现人工生命，但认为开放世界ALIFE现在是一个可行的实验范式，也是研究可谨慎称为"活体AI"的具体平台。
...
### [166] [DDIAgents: Mechanism-Conditioned Context Flow for Drug-Drug Interaction Prediction](https://arxiv.org/abs/2606.31085)
*Zhenqian Shen,Yu Liu,Xiaoyi Fu,Quanming Yao*

Main category: cs.AI

TL;DR: DDIAgents是一种基于机制条件的多智能体框架，通过动态知识编排进行药物-药物相互作用（DDI）预测。该框架根据药物对实例化专业专家智能体，将机制相关知识源路由到各智能体，并通过结论智能体聚合分析结果。


<details>
  <summary>Details</summary>
Motivation: DDI预测对药物安全至关重要，但需要在异构生物医学证据上进行推理，且相关性随相互作用机制变化。现有方法难以有效处理异构知识并适应不同机制。

Method: 提出DDIAgents框架，包含：1) 规划智能体实例化专业专家智能体；2) 路由机制相关知识源到各智能体；3) 通过结论智能体聚合分析。通过调整上下文流以适应推断的相互作用机制，减少无关信息，支持互补专家推理，产生可解释的智能体级理由。

Result: 在现实DDI预测基准测试上的广泛实验表明，DDIAgents始终优于现有的基于特征、基于图、基于LLM和基于智能体的基线方法。

Conclusion: DDIAgents展示了多智能体系统如何组织异构科学知识，实现适应性和可解释的AI4Science推理。
...
### [167] [Revealing Safety-Critical Scenarios for UTM via Transformer](https://arxiv.org/abs/2606.31114)
*Huaze Tang,Bill Zeng,Chao Wang,Zhenpeng Shi,Qian Zhang,Wenbo Ding*

Main category: cs.AI

TL;DR: 提出了一种基于Transformer强化学习的UTM系统漏洞发现框架，通过策略模型生成目标测试场景、动作采样器强制执行领域约束、基于风险的奖励函数引导探索，在700小时仿真研究中实现8倍漏洞发现效率提升，并发现传统方法遗漏的关键边缘案例。


<details>
  <summary>Details</summary>
Motivation: UTM系统是管理多架无人机的云平台，属于安全关键系统，不能容忍坠毁或碰撞等故障。然而，现有的漏洞发现方法既没有最优的故障暴露演示，也没有明确的奖励信号。此外，UTM的自愈能力引入了关键故障的"长尾效应"，使得漏洞发现更加困难。

Method: 将UTM漏洞发现问题建模为序列建模问题，采用基于Transformer的强化学习架构。利用注意力机制直接建模系统状态之间的关系，预测最优动作。框架包括：1)策略模型(Policy Model)生成有针对性的测试场景；2)动作采样器(Action Sampler)强制执行领域约束；3)基于风险的奖励函数引导探索。

Result: 在700小时的仿真研究中，与专家引导测试相比，漏洞发现效率提升了8倍，并且发现了传统方法遗漏的关键边缘案例。

Conclusion: 基于Transformer的强化学习方法能够有效解决UTM系统的漏洞发现问题，通过注意力机制建模状态关系和风险引导的探索策略，可显著提高安全关键系统的测试效率，发现潜在的边缘 case，为无人交通管理系统安全保障提供了新的技术途径。
...
### [168] [The Past Is Prologue: A Plug-in Controller for Selective Updates in Sequentially Evolving LLM Memory](https://arxiv.org/abs/2606.31121)
*Zihan Chen,Songwei Dong,Chengshuai Shi,Peng Wang,Song Wang,Cong Shen,Jundong Li*

Main category: cs.AI

TL;DR: Janus是一个即插即用的记忆控制器，通过记忆动量触发器识别记忆更新轨迹中的可疑偏差，并在覆盖、边界和新鲜任务的紧凑混合评估集上比较新旧记忆，决定是否接受候选记忆更新，以避免有用知识被覆盖或引入过度特定的规则。


<details>
  <summary>Details</summary>
Motivation: 现有系统通常直接部署每个本地生成的记忆更新而不检查其是否改善未来行为，导致帮助当前任务的更新可能覆盖有用知识、引入过度特定的规则或使记忆偏向近期示例。

Method: 提出Janus方法，包括：1) Memory Momentum Trigger（记忆动量触发器）用于识别记忆更新轨迹中的可疑偏差；2) 在覆盖、边界和新鲜任务的紧凑混合评估集上比较新旧记忆，而不是重放完整历史；3) 方法无关性，可包装现有更新器而不改变其更新规则。

Result: 在六个数据集、两个骨干LLM和两个记忆更新器上，Janus比相应的基础更新器提高平均准确率2.7至4.6个百分点。

Conclusion: Janus能有效判断是否接受记忆更新，避免有用知识被覆盖，显著提升LLM代理的记忆管理效果。
...
### [169] [Scenario Generation for Testing of Autonomous Driving Systems Using Real-World Failure Records](https://arxiv.org/abs/2606.31131)
*Anjali Parashar,Chuchu Fan*

Main category: cs.AI

TL;DR: 提出一种基于大语言模型(LLM)的场景生成管道,利用历史事故记录的自然语言信息生成自动驾驶测试场景。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在部署前需要进行充分的测试,但现有仿真测试依赖固定场景表示且效率低下,而真实世界测试需要大量人工设计场景模板。历史故障记录是宝贵的真实故障条件来源,可用于场景生成。

Method: 提出模块化的LLMbased合成场景生成管道,利用历史记录中的分类和上下文信息(自然语言格式)生成场景,使生成的场景符合给定系统的测试约束。

Result: 在Metadrive模拟器上使用NHTSA ADS碰撞记录成功生成了多样化场景,包含4种道路类型、3种非自车移动类型及道路异常(工作区)。在仅20个场景的有限测试预算下揭示了系统的有趣故障。

Conclusion: 该方法能够生成准确且多样化的场景,有效支持自动驾驶系统的部署前测试,代码已开源。
...
### [170] [Beyond the Library: An Agentic Framework for Autoformalizing Research Mathematics](https://arxiv.org/abs/2606.31134)
*Arshia Soltani Moakhar,Iman Gholami,Max Springer,Mahdi JafariRaviz,MohammadTaghi Hajiaghayi*

Main category: cs.AI

TL;DR: 提出了一个基于通用编码LLM的代理自动形式化框架，可将自然语言数学转换为Lean 4可验证证明，并在PutnamBench和5篇STOC论文上验证成功。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数学推理中常产生难以检测的细微错误，而Lean 4等形式化语言可提供机械证明检查，因此需要自动形式化技术将自然语言数学转换为可验证代码。

Method: 构建了一个由编排器管理的多智能体管道，使用通用编码LLM（而非针对Lean微调的小模型），通过新型辅助引理技术动态扩展Mathlib中不存在的类型定义，然后形式化主要定理。

Result: 在PutnamBench上为32道随机选问题生成了机器可检查的Lean证明；在5篇STOC论文（涵盖组合学、通信复杂性、机制设计、学习理论）上成功形式化了主定理及其证明，其中两篇证明不依赖Mathlib外任何公理。

Conclusion: 通用编码LLM已超越专门为Lean微调的小模型，成为自动形式化的有效工具；该框架能处理前沿研究级数学问题，为数学证明验证提供新途径。
...
### [171] [Cross-Domain Feature Expansion for Tabular Medical Data via Knowledge Graphs Injection](https://arxiv.org/abs/2606.31171)
*Mengying Zhou,Yongjie Yin,Haoyan Xin,Guoping Liu,Yang Chen*

Main category: cs.AI

TL;DR: MedKGTab是一种知识注入框架,用于表格医疗数据的跨域特征扩展,通过利用固有统计依赖关系和医学相关性来推断未收集的生物医学特征。


<details>
  <summary>Details</summary>
Motivation: 获取全面的跨域生物医学特征成本高且耗时,导致医学研究面临严重的数据稀缺问题。

Method: MedKGTab采用行-列双注意力机制,直接在原始结构化表格数据上操作,避免tokenization带来的结构损失;同时整合数据驱动的统计先验与SPOKE生物医学知识图谱,实现数据通道与知识通道的最佳协同,使知识调制数据表示,确保生成的数据基于实证医学研究。

Result: MedKGTab在跨域特征扩展中实现了高数据保真度和真实的数据表示,优于SOTA医学大模型(如Baichuan M3-plus)和专门的医疗数据生成表格模型,在同数据集内缺失特征推断和不同医疗队列间泛化等多种数据生成场景中均表现优异。

Conclusion: MedKGTab能够有效解决医疗数据稀缺问题,通过知识注入与数据驱动方法的协同,为跨域医疗特征生成提供了新的解决方案。
...
### [172] [ClawArena-Team: Benchmarking Subagent Orchestration and Dynamic Workflows in Language-Model Agents](https://arxiv.org/abs/2606.31174)
*Kaiwen Xiong,Haonian Ji,Shi Qiu,Zeyu Zheng,Cihang Xie,Xinyu Ye,Huaxiu Yao*

Main category: cs.AI

TL;DR: 论文提出ClawArena-Team基准测试，评估LLM作为团队管理者的能力，涵盖41个多轮、多模态、多目录场景。研究发现权限授予是管理瓶颈，成本与管理质量解耦，分数集中在9.9分范围内但编排行为差异显著。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试只评估政策自身的问题解决能力或固定多智能体系统的涌现行为，无法隔离单个LLM作为管理者的管理能力。

Method: 构建ClawArena-Team基准：41个多轮多模态多目录场景，258轮评估，72个阶段性更新。约束主智能体只能感知文本和访问部分工作区，使用固定本地子智能体池。执行评分，无LLM评判，使用SMS（子智能体管理分数）=任务正确性×最小权限×模态路由因子。

Result: 权限授予是管理瓶颈（无模型权限精确度超过50%）；成本与管理质量解耦（API成本差异超100倍但总分差异不到4倍，最便宜的开源模型在Pareto前沿）；大多数排行榜分数集中在9.9分范围内但编排行为差异超过一个数量级。

Conclusion: LLM作为团队管理者的核心瓶颈在于权限授予而非感知能力；管理质量与成本解耦为开源模型提供机会；需重新审视评估指标以捕捉编排行为差异。
...
### [173] [HealthAgentBench: A Unified Benchmark Suite of Realistic Agentic Healthcare Environments for Challenging Frontier AI Agents](https://arxiv.org/abs/2606.31179)
...
TL;DR: HealthAgentBench是一个用于评估医疗AI代理的基准测试套件，包含54个跨7个类别的医疗任务，旨在测试AI代理在复杂临床工作流中的端到端推理和执行能力。评估显示当前前沿代理的成功率仍较低（约42%），为医疗AI代理的发展提供了具有挑战性和现实性的基准。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理在复杂、长周期推理方面能力增强，需要严格且全面的评估方法来衡量其在现实医疗应用中的进展。现有基准测试缺乏对端到端临床工作流的全面评估。

Method: 构建包含54个任务的健康医疗代理基准，涵盖7个类别，每个类别有独特环境。任务模拟完整的临床工作流：给定最少指令，代理需探索原始医疗数据，在复杂环境中操作，执行超越简单提示的多步骤解决方案。

Result: 前沿代理的整体任务成功率仍然较低。最强且最具成本效益的代理Codex GPT-5.5仅达到约42%的成功率。Claude Code模型在医学影像方面尤其困难，而Codex GPT-5.5显示出新兴能力。结合大搜索空间和组合推理要求的任务对所有当前代理都具有挑战性。

Conclusion: HealthAgentBench提供了一个具有挑战性和现实性的基准测试，为未来研究留下了 substantial room for progress。前沿代理在EHR数据上自动开发研究建模管道方面展现潜力，但医学影像仍特别具有挑战性。
...
### [174] [AI-Assisted Discovery of Convex Relaxations via Dual Agents](https://arxiv.org/abs/2606.31182)
*Sungyoon Kim,Mert Pilanci*

Main category: cs.AI

TL;DR: 本文提出了一种自动研究范式，通过凸松弛方法为优化常数寻找更紧的下界，并通过多智能体系统（编码智能体+理论智能体）自动发现和验证约束紧致技术。应用于两个经典优化常数（C₆.₂和C₆.₅），分别将下界从1.28提升至1.2937，从0.379005提升至0.37912。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体研究主要关注通过极值构造寻找上界，而下界（对所有容许函数成立）通常被忽视。本文旨在弥补这一空白，通过更紧的凸松弛获得更强的下界。

Method: 1) 将非凸问题凸松弛化；2) 采用自动研究范式：编码智能体提出有效的紧致约束，理论智能体验证并搜索反例；3) 使用严格区间算术对偶可行点进行认证。

Result: 两个优化常数的 certified lower bounds 均获得提升：C₆.₂ 从 1.28 改进至 1.2937；C₆.₅ 从 0.379005 改进至 0.37912。

Conclusion: 自动搜索凸松弛约束是获得优化常数更紧下界的有效方法，多智能体协作框架能够系统性地发现并验证新的数学约束。未来的研究可探索更复杂的常数和更一般的不等式类别。
...
### [175] [Agentic RAG-VLM: Affordance-Aware Retrieval-Augmented Generation with Self-Reflective Planning for Robotic Grasping](https://arxiv.org/abs/2606.31200)
*Tao Chen,Lizheng Liu,Jiaxu Wang,Ziyue Jiang,Ruiqi Tian,JiGuang Huo,Zhongxue Gan*

Main category: cs.AI

TL;DR: Agentic RAG-VLM 是一个统一框架，通过结合检索增强生成、视觉语言模型和代理自反思规划，将基于 VLM 的语义理解与物理 grounded 抓取执行相结合，实现复杂环境中的可泛化机器人抓取。


<details>
  <summary>Details</summary>
Motivation: 现有 VLM 方法仅依赖视觉相似度进行物体匹配，忽略物理 affordance（如手柄可抓取性、材质脆性），且以开环方式运行，缺乏空间推理和失败恢复能力，在物体密集或物理多样性场景中效果受限。

Method: 提出统一框架Agentic RAG-VLM，包含三个紧密耦合的组件：1) 层次化 affordance 感知 RAG (HAA-RAG)，编码四维 affordance 描述符；2) 场景图约束推理器，从 VLM 感知构建空间关系图；3) 代理自反思 pipeline，包含14类失败分类法和三级自适应重试机制。

Result: 在12任务基准测试的360次试验中，Agentic RAG-VLM 达到78.3%总体成功率，比 VLM 仅基线提升53.3个百分点。

Conclusion: 该框架在复杂抓取任务中展示了显著优势，为非结构化环境中的机器人操作提供了新的解决方案思路。
...
### [176] [Towards Inclusive Mobility Modeling: Characterizing and Evaluating Elderly Trajectory Patterns in Urban Systems](https://arxiv.org/abs/2606.31207)
*Zhengxuan Wang,Haohan He,Mengying Zhou*

Main category: cs.AI

TL;DR: 研究使用2016-2020年Jersey City的Citi Bike数据，通过合成轨迹生成案例，定量分析弱势群体（老年人） mobility signatures缺失对mobility modeling的影响。研究发现老年人具有独特的mobility signature（活动范围更小、entropy更低、非对称的off-peak时间模式），且基于多数人群训练的模型会系统性地误解老年人mobility behavior。LLM模型在有限人口数据下并不一定能提高子群体级别的保真度。


<details>
  <summary>Details</summary>
Motivation: 智慧城市的发展越来越依赖轨迹数据挖掘，但弱势群体（特别是老年人）在公共mobility数据集中代表性不足，导致系统性偏差进入mobility modeling和下游城市规划。

Method: 使用2016-2020年Jersey City Citi Bike数据，采用一阶马尔可夫链和Qwen3-4B模型（QLoRA微调），在三种人口统计训练设置下（全部人口、仅年轻人、仅老年人）进行合成轨迹生成实验。

Result: 老年人具有显著不同的mobility signature：活动范围更小（958m vs 1189m）、entropy更低（1.82 vs 4.15）、非对称的off-peak时间模式。马尔可夫模型在全人口训练下高估老年人step length 4.5%和dwell time 8.9%。老年人专用模型在大多数指标上误差显著更低。LLM模型在有限人口数据下并未显著提高子群体级别的保真度。

Conclusion: 在mobility modeling中，人口统计代表性非常重要，尤其是对弱势/少数群体的下游应用。依赖多数人群主导的训练数据会系统性地歪曲少数群体的mobility behavior。更高capability的模型在有限人口数据下不一定能改善子群体级别的保真度。
...
### [177] [Long-term Traffic Simulation via Structured Autoregressive Modeling](https://arxiv.org/abs/2606.31209)
*Lingyu Xiao,Zexin Feng,Xintao Yan*

Main category: cs.AI

TL;DR: RosettaSim是一个基于大语言模型的交互式交通仿真框架，通过将场景拓扑、代理状态和生成意图投影到结构化自回归流中，实现短长期仿真的高精度和稳定性，并提出检索式交通评估方法解决扩展rollout中的代理对应问题。


<details>
  <summary>Details</summary>
Motivation: 长期交通仿真中建模持续的多代理交互具有挑战性，且动态token基数问题（代理不断进出场景）进一步加剧了这一挑战。现有方法难以同时实现短期准确性和长期稳定性。

Method: 提出RosettaSim框架，将场景信息投影为可变长度的结构化自回归流；提出RTE（检索式交通评估）方法，通过检索语义相似的真实场景作为参考锚点来评估扩展 rollout。

Result: 在Waymo Open Sim Agent Challenge (WOSAC)上达到短期和长期仿真的最先进性能；RTE与标准指标的相关系数(r=0.83)优于现有方法(r=0.74)。

Conclusion: LLM的架构归纳偏置和统计先验能够有效迁移到交通建模任务，小规模冻结LLM可快速适应交通仿真，为自动驾驶世界模型提供新思路。
...
### [178] [Thinking Before Retrieving: Robust Zero-Shot Composed Image Retrieval via Strategic Planning and Self-Criticism](https://arxiv.org/abs/2606.31222)
*Gunho Jung,Jeong-Woo Park,Seon Bin Kim,Seong-Whan Lee*

Main category: cs.AI

TL;DR: 本文提出PEC-CIR，一个用于组合图像检索的无训练零样本框架，采用Planner-Executor-Critic架构，将查询构建重构为多阶段推理流程，通过显式评估候选查询来减少生成错误的传播。


<details>
  <summary>Details</summary>
Motivation: 现有无训练零样本方法采用单次生成策略将参考图像上下文和修改文本融合为统一描述，这种策略难以检测或纠正语义扭曲和遗漏，导致参考属性保留与文本需求整合相互干扰，检索精度下降。

Method: 提出PEC-CIR框架，包含三个组件：1)Planner(规划器)提取显式约束；2)Executor(执行器)生成多个候选目标描述；3)Critic(批评器)基于约束合规性评估这些候选。通过将查询构建重构为分阶段推理而非单次输出，减少生成错误传播。

Result: 该框架通过显式评估候选查询后再进行检索，提高了检索稳定性。

Conclusion: PEC-CIR通过多阶段推理pipeline有效解决了单次生成策略的局限性，为组合图像检索提供了一种更稳定准确的无训练零样本解决方案。
...
### [179] [Agentic-Ideation: Sample Efficient Agentic Trajectories Synthesis for Scientific Ideation Agents](https://arxiv.org/abs/2606.31229)
*Keyu Zhao,Lingyan Kong,Fengli Xu,Yong Li*

Main category: cs.AI

TL;DR: 本文提出Agentic-Ideation框架，包含自动轨迹合成管道和专门为科学构思训练的智能体LLM，旨在解决现有AI科学家系统在自动化构思中缺乏灵活性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预定义的智能体工作流，严重限制了科学文献搜索空间和研究推理动作空间的灵活性；且将现有智能体数据合成方法应用于科学构思时面临极高的数据合成成本。

Method: 1)定义了包含3个外部工具和3个认知工具的综合工具空间；2)提出Oracle-Guided数据合成策略，利用参考构思作为 oracle 引导多智能体系统重建逻辑推理和工具调用路径；3)采用工具执行结果掩码策略训练模型，使其专注于决策逻辑而不受外部反馈干扰。

Result: 在整体质量上比SOTA工作流基线提升11.91%；高质量数据合成的样本效率提升超过10倍。

Conclusion: Oracle-Guided数据合成策略能有效将盲目试错转化为定向轨迹生成，掩码策略确保模型聚焦决策逻辑，显著提升科学构思的自动化水平和效率。
...
### [180] [Delta-JEPA: Learning Action-Sensitive World Models via Latent Difference Decoding](https://arxiv.org/abs/2606.31232)
...
TL;DR: Delta-JEPA是一种端到端的重建-free世界模型，通过潜空间差异动作解码器(LDAD)从连续观测之间的潜空间位移重构执行的动作，从而避免表征崩溃并保持动作敏感性，在四个视觉连续控制任务上提升了规划性能。


<details>
  <summary>Details</summary>
Motivation: 学习规划用的视觉世界模型需要紧凑的潜动力学模型，但重建-free的联合嵌入目标容易崩溃为对动作不敏感的特征表示。

Method: 提出Delta-JEPA，使用潜空间前向预测并引入潜空间差异动作解码器(LAD)，不同于从拼接端点嵌入推断动作的逆解码器，LDAD从连续观测之间的潜空间位移重构执行的动作，直接正则化转移几何。

Result: 在四个视觉连续控制任务中，Delta-JEPA相比基于JEPA和表示学习的基线世界模型提升了规划效果。消融实验表明基于位移的动作解码始终比端点拼接更有效，action敏感性分析显示更清晰的动作条件潜空间响应。

Conclusion: 监督潜空间差异是实现抗崩溃且动作敏感的视觉世界模型学习的简单有效机制。
...
### [181] [Embodied CAD: Solver-Grounded LLM Agents for Parametric B-Rep Assembly Modeling](https://arxiv.org/abs/2606.31252)
*Fumin Liu,Haoyu Zhou,Fei Hao,Lin Yang*

Main category: cs.AI

TL;DR: Embodied CAD是一个基于求解器的LLM代理框架，用于参数化B-Rep装配建模。该系统不一次性生成完整脚本，而是通过分层L0-L4 CAD技能库迭代选择动作，在CAD后端执行并利用求解器反馈进行规划、修复和学习。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以编写看似合理的CAD脚本，但可靠的工业CAD建模不仅需要语法有效的代码：每个特征、放置和装配关系必须被精确几何内核接受，同时保持为可编辑的参数化边界表示几何。现有一遍式生成方法无法满足工业级精度要求。

Method: 提出Embodied CAD框架，包含以下核心组件：1) 分层L0-L4 CAD技能库，用于迭代式动作选择；2) 将动作解析为类型化几何操作；3) 在CAD后端执行并利用求解器反馈；4) 结合动作语法约束、确定性参数解析和基于求解器的奖励信号进行监督预热和GRPO风格强化学习微调。

Result: 在多步骤机械、工业设备和模具导向装配任务上的评估表明：求解器规划能够执行当前基准中的所有强规划器工作流；学习控制器达到高可执行率；揭示了有效工具调用与精确长程策略预测之间仍存在差距。

Conclusion: Embodied CAD成功地将LLM代理与精确几何求解器结合，实现了工业级参数化CAD建模的可执行性和准确性。未来工作将聚焦于缩小有效工具调用与精确长程策略预测之间的差距。
...
### [182] [Spatial Reasoning via Modality Switching Between Language and Symbolic Representation](https://arxiv.org/abs/2606.31285)
*Shreya Rajpal,Tanawan Premsri,Parisa Kordjamshidi*

Main category: cs.AI

TL;DR: 研究探讨了将多跳文本-空间故事 grounding 到几何感知模态（如布局或网格）是否能提升推理性能，以及模型能否自主决定何时切换推理模态。提出基于可信度和复杂度的切换指标，实现有原则的模态选择。


<details>
  <summary>Details</summary>
Motivation: 人类推理本质上是多模态的：当问题变得困难时，我们很少仅用文字思考，常常通过绘图或画网格来理解底层概念结构并避免错误。这启发了研究是否可以将LLM的文本推理 externalize 到结构化模态以提升性能。

Method: 引入基于 trustworthiness 和 complexity 信号的切换指标，估计何时将空间故事 grounding 到结构化形式可能改善性能。该方法首先判断问题的复杂度，然后决定使用自然语言推理还是切换到网格表示。

Result: 从自然语言推理切换到网格表示可提升LLM性能高达42%，证明了模态选择对推理结果的重要性。

Conclusion: 模态选择在LLM推理中起着关键作用。基于可信度和复杂度的切换指标可实现有原则的模态选择，为构建更高效的多模态推理系统提供了新思路。
...
### [183] [Benchmarking Large Language Models on Floating-Point Error Classification](https://arxiv.org/abs/2606.31308)
*Lisa Taldir,Muhammad Ahmad Saeed,David Defour,Pablo de Oliveira Castro,Eric Petit*

Main category: cs.AI

TL;DR: 该论文构建了InterFLOPBench基准测试，用于评估大型语言模型静态检测和分类浮点错误的能力，涵盖6种错误类型，发现最新模型表现优异但对不同错误类型检测能力差异显著。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在代码分析领域展现出潜力，但其在浮点错误静态检测方面的能力尚未得到系统评估，因此需要构建基准测试来衡量和比较不同模型的性能。

Method: 将浮点错误检测视为多标签分类问题，使用F1分数作为评估指标，评估了14个大型语言模型在90个C内核上的130个测试样本上的表现。

Result: 最新模型（Qwen 3 32b、Gemini 2.5 Flash、Phi 4 Reasoning、DeepSeek R1T2、gpt-oss 20b和120b）达到0.88以上的整体F1分数。不同错误类别间性能差异明显：除零检测平均F1为0.8479，而下溢检测平均F1仅为0.6059，取消检测平均F1为0.6164。

Conclusion: 最新的大语言模型在静态检测浮点错误方面表现出色，整体F1分数超过0.88，但对于不同类型的错误检测能力存在显著差异，对于显式操作（如除零）的检测效果最好，而对于更微妙的数值现象（如下溢和取消）检测效果较差。
...
### [184] [HistoriQA-ThirdRepublic: Multi-Hop Question Answering Corpus for Historical Research, Parliamentary Debates from the French Third Republic (1870-1940)](https://arxiv.org/abs/2606.31325)
*Aurélien Pellet,Julien Perez,Marie Puren*

Main category: cs.AI

TL;DR: 提出了HistoriQA-ThirdRepublic数据集，一个来自法国第三共和国时期议会辩论和报纸的法语多跳历史问答数据集，包含1782个问题，旨在评估检索增强和大型语言模型在特定领域语境中的表现。


<details>
  <summary>Details</summary>
Motivation: 历史研究的复杂性需要跨源综合、时间推理和稀疏证据整合，但现有NLP基准与历史学术需求之间存在差距，需要专门针对历史领域的问答数据集。

Method: 与历史学家合作设计，选择和对齐来源，验证问题，集成元数据。数据集强调异构历史文档之间的多跳连接。

Result: 构建了1782个问题的数据集，捕捉了历史查询的复杂推理模式，包括跨源综合、时间推理和稀疏证据整合。

Conclusion: 该数据集可为多跳问答提供真实评估场景，弥合NLP基准与历史学术需求之间的差距。其方法可轻松适应其他语言和国家语料库。
...
### [185] [CryoACE: An Atom-centric Framework for Accurate and Automated Model Building in Cryo-EM](https://arxiv.org/abs/2606.31332)
*Minzhang Li,Mingrui Li,Weichen Qin,Qihe Chen,Sixian Shen,Yuan Pei,Jiakai Zhang,Jingyi Yu*

Main category: cs.AI

TL;DR: CryoACE是一种用于从冷冻电镜密度图进行蛋白质自动化建模的端到端框架，能够重建精确的原子图谱，特别擅长处理同质和异质结构。该方法首次在复杂真实数据集上揭示了原子水平的动态构象。


<details>
  <summary>Details</summary>
Motivation: 当前蛋白质冷冻电镜自动化建模面临两大挑战：1) 强制执行物理化学有效性；2) 管理构象异质性。现有求解器通常局限于静态预测或需要计算密集的启发式搜索。

Method: 提出两个关键创新：1) 原子中心重建范式——直接在原子坐标处采样密度特征并进行迭代回收以细化结构，取代昂贵的体素卷积以实现高效的多模态融合；2) 无训练指导机制——利用预测的局部分辨率先验来解决动态模糊问题。

Result: 在新构建的高质量数据集上验证，CryoACE在静态基准测试中显著优于现有基线，并首次在EMPIAR-10345等复杂真实数据集上揭示了原子水平的动态构象，且不依赖预构建的静态结构。

Conclusion: CryoACE代表了冷冻电镜蛋白质建模的重要进展，能够同时处理同质和异质结构，为原子水平的动态构象分析提供了新的可能性。
...
### [186] [Optimization Algorithms for Joint OFDM Waveform Design and RIS Configuration in 6G Networks: From Convex Relaxation to Foundation Models](https://arxiv.org/abs/2606.31334)
*Ahmet Kaplan*

Main category: cs.AI

TL;DR: 本文是一篇关于6G网络中OFDM-RIS联合优化的综合调研，涵盖78篇2021-2026年间的论文，将现有方法分为四大范式：模型凸松弛、启发式/元启发式搜索、深度强化无监督学习、以及新兴方法（基础模型、扩散生成AI、量子优化）。调研发现ML方法可达模型方法的95-99%谱效，但推理速度快10^2-10^4倍，且GPU神经网络推理具有N不变性。


<details>
  <summary>Details</summary>
Motivation: 解决6G网络中OFDM-RIS联合优化这一混合整数非线性规划问题缺乏标准化基准，不同论文间无法进行比较的问题，旨在为研究者和从业者提供路线图。

Method: 对78篇联合OFDM-RIS优化论文进行分类综述，建立配套基准测试平台(N=16,64,128)，比较四种范式的性能与可扩展性。

Result: ML方法(Paradigm III)达到模型方法的95-99%谱效，推理速度快10^2-10^4倍；GPU神经网络推理(DDQN,PPO,GNN,无监督DL)具有N不变性，在N=16和N=128时运行时间相同；而迭代求解器(AO+SCA,PSO)呈多项式扩展。

Conclusion: 提出六大开放挑战：跨范式基准缺失、实际硬件约束部署、双径信道联合波形-RIS优化、多目标PAPR权衡、LLM安全性和独立启发式方法收益递减。需建立标准化基准，推动该领域有序发展。
...
### [187] [Smart charging of large fleets of Electric Vehicles: Independent Multi-Agent Reinforcement Learning approaches](https://arxiv.org/abs/2606.31347)
...
TL;DR: 本研究比较了两种多智能体强化学习方法（情境组合 bandits 和策略梯度算法）在电动汽车充电优化中的应用，旨在实现去中心化EV充电的隐式协调，解决电网峰值负荷、电压波动和线路过载等问题。


<details>
  <summary>Details</summary>
Motivation: 电动汽车普及带来电网管理新挑战，包括峰值需求增加、电压波动、线路过载以及可再生能源整合问题。需要通过隐式协调来高效整合EV，同时降低用户成本并避免网络过载。

Method: 使用基于真实光伏发电数据的动态电价，结合情境组合 bandits 和策略梯度两种独立的多智能体强化学习方法，在现实模拟环境中进行评估。智能体基于本地环境信息（电价信号、充电状态、时间约束）做决策，在不同拥堵水平和混合策略配置下测试。

Result: 在不同拥堵程度和异构智能体群体混合策略配置下评估了两种方法的性能表现。

Conclusion: 研究为去中心化EV充电的隐式协调提供了两种可行的多智能体强化学习方案，为智能电网需求响应策略提供了参考。
...
### [188] [ReGRPO: Reflection-Augmented Policy Optimization for Tool-Using Agents](https://arxiv.org/abs/2606.31392)
*Binjie Zhang,Mike Zheng Shou*

Main category: cs.AI

TL;DR: 本文提出ReGRPO框架，通过反思引导的纠正机制提升工具增强视觉语言模型在多模态多步骤任务中的鲁棒性。核心创新在于构建反思-of-思维(RoT)三元组数据引擎，并结合群体相对策略优化进行联合训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个关键缺陷：1) 监督微调仅使用成功轨迹，缺乏工具失败后的恢复信号；2) 稀疏的轨迹级强化学习奖励无法指出具体失败步骤及修复方法。这导致工具增强的VLMs在实践中仍然脆弱。

Method: 1) 构建结构化反思数据引擎：执行near-miss动作收集失败观察，建立RoT三元组(ErrorType, Evidence, FixPlan)并配对纠正动作进行warm-start SFT；2) 在局部轨迹内使用群体相对优势联合优化反思tokens和纠正动作；3) 引入反思成本项减少不必要的反思。

Result: 在GTA和GAIA基准测试中，使用相同骨干网络和工具套件下，ReGRPO始终优于强大的开源基线，并在开源控制器中取得最佳结果。

Conclusion: ReGRPO通过反思引导的纠正机制有效提升了工具增强VLMs的鲁棒性，为多模态智能体的错误恢复提供了新范式。
...
### [189] [World-Model Collapse as a Phase Transition](https://arxiv.org/abs/2606.31399)
*Xinyuan Song,Zekun Cai*

Main category: cs.AI

TL;DR: 研究揭示了长视野语言代理在参数达到临界点时会出现类似水沸腾的"世界模型崩溃"现象——微小参数变化导致行为突然从正常转变为崩溃状态。


<details>
  <summary>Details</summary>
Motivation: 探索长视野语言代理是否存在类似相变的临界现象，以及世界模型崩溃的具体机制和影响。

Method: 在确定性任务家族中进行大规模网格搜索，系统研究状态基数、依赖密度、视野长度、分支数、观察模式和突变率对代理行为的影响。

Result: 发现了明确的相图：存在已解决的高原区、狭窄的过渡带和崩溃底层。世界状态保真度比行动有效性更早失效，表明代理并非仅选择错误行动，而是基于被破坏的世界模型行动。

Conclusion: 世界模型崩溃是长视野代理的关键瓶颈，更强的模型可以平移临界边界但无法消除这一定性转变。
...
### [190] [Wisdom Of The (AI) Crowd: Investigating Artificial Swarm Intelligence In Large Language Models](https://arxiv.org/abs/2606.31404)
*Justin Brenne,Christian Meske*

Main category: cs.AI

TL;DR: 研究探索大型语言模型（LLM）是否可以通过人工群体模拟人类群体智能，实现成本更低、可扩展性更强的聚合决策效果。


<details>
  <summary>Details</summary>
Motivation: 人类群体智能虽具有显著的集体准确性，但在成本、协调和时间方面面临可扩展性限制。AI-based聚合机制的理解存在关键空白，需要探索LLM是否能够近似群体智能效果。

Method: 在三个专有模型（GPT-5、Gemini 2.5 Pro、Claude Sonnet 4.5）上进行960个手动执行的提示测试，对8个估计任务进行模型内采样和模型间聚合的受控实验。

Result: 模型内和模型间聚合均显示出一致的错误减少，MAPE错误减少高达37个百分点。相对置信区间宽度与相对估计误差之间存在小到大的正相关效应（Spearman ρ=0.242-0.568，p<0.001）。

Conclusion: LLM具备元认知意识，能够评估自身不确定性。研究为在组织决策中部署LLM群体提供了可操作的见解。
...
### [191] [Xiaomi-GUI-0 Technical Report](https://arxiv.org/abs/2606.31410)
...
TL;DR: 小米提出Xiaomi-GUI-0，一个基于视觉语言模型的原生多模态GUI代理，用于真实移动环境。核心创新是真实设备主导的混合基础设施，解决传统GUI代理在模拟环境中训练评估与真实应用之间的分布差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理在离线轨迹、模拟环境和标准基准上训练评估，与真实应用在界面布局、交互逻辑和异常状态分布上存在显著差异。真实场景中账户状态、权限对话框、支付认证和风控不断重塑状态分布，导致基准分数与实际可用性之间存在持续差距。

Method: 1) 真实设备主导的混合基础设施（物理设备为主，沙盒辅助）；2) 多源训练数据（高频头部任务、高泛化长尾意图、反思记忆能力增强数据）；3) 错误驱动的数据飞轮（将失败轨迹转化为修正动作、反思解释和恢复演示）；4) 三阶段渐进训练管道（监督微调、步级强化学习、代理强化学习）。

Result: 在RealMobile上达到72.0%成功率，AndroidWorld上达到78.9%，显著提升真实世界任务的执行稳定性和异常状态识别能力。

Conclusion: Xiaomi-GUI-0通过真实设备闭环训练评估，有效缩小了基准分数与真实可用性之间的差距，为GUI代理在真实移动环境中的应用提供了可行方案。
...
### [192] [Learning to Select, Not Relearn: Hard-Routed Mixtures of Reasoning LoRAs](https://arxiv.org/abs/2606.31413)
*Seyed Alireza Molavi,Zhan Su,Yan Hu,Peyman Sheikholharam Mashhadi,Stefan Byttner,Prayag Tiwari*

Main category: cs.AI

TL;DR: 提出Hard-Routed MoR-LoRA框架，通过硬路由选择机制将独立训练的LoRA适配器组合成单一大型语言模型，解决多域适应问题。


<details>
  <summary>Details</summary>
Motivation: 当原始训练数据无法共享时，需要将独立训练的LoRA适配器组合成统一模型。现有MoE风格路由方法存在软加权组合会改变每个LoRA模块原始训练的单位尺度 additive update的问题。

Method: 两阶段框架：第一阶段使用可验证反馈的强化学习训练领域特定LoRA适配器获得推理专家；第二阶段冻结所有专家，提取推理轨迹，仅训练轻量级共享路由器和小型的注意力LoRA。路由器使用硬top-1路由选择每个token的专家，通过直通估计器实现基于梯度的训练。

Result: 在五个基准测试、多种模型规模和额外模型家族上的实验表明，Hard-Routed MoR-LoRA在保持专家行为的同时，比软路由混合基线需要更少可训练参数。分析还表明归一化软混合通常将大部分路由质量集中于单个专家。

Conclusion: 硬单位尺度路由为冻结LoRA专家组合提供了简单高效的抽象方法。
...
### [193] [BP-TTA: Balanced and Prototype-Guided Test-Time Adaptation in Dynamic Scenarios](https://arxiv.org/abs/2606.31420)
*Shaoyang Huang,Yashi Zhu,Yichen Yu,Lei Zhang,Zhang Yi,Tao He*

Main category: cs.AI

TL;DR: BP-TTA是一种新的平衡原型引导的测试时适应方法，旨在同时解决动态测试时流中的类别不平衡和持续域漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时适应方法主要处理分布漂移，但未能解决动态场景中的类别不平衡问题。在现实世界的测试时流中，类别不平衡和持续域漂移常常同时发生并相互影响。

Method: BP-TTA结合了批量平衡采样和原型引导适应：1) 通过整合当前样本与高置信度历史实例构建平衡适应批次，缓解对主导类别的偏见并稳定在线更新；2) 在推理过程中维护不断演化的类别原型，利用原型相似性作为模型适应的约束，提高伪标签可靠性并增强持续域漂移下的在线更新稳定性。

Result: 广泛实验表明，BP-TTA在动态测试时流设置中始终优于最先进的测试时适应方法。

Conclusion: BP-TTA能够有效处理类别不平衡和持续域漂移的联合问题，为测试时适应领域提供了一种新的有效解决方案。
...
### [194] [Ask the World Before Acting: Budgeted Environment Probing for World-Model Calibration](https://arxiv.org/abs/2606.31422)
*Xinyuan Song,Zekun Cai*

Main category: cs.AI

TL;DR: 本文研究长程语言智能体如何在校准稀缺的环保持世界模型并修复信念漂移。智能体在执行任务动作前可向环境询问特定信念字段，将环境交互从推进任务的工具转变为稀缺的对齐资源。提出一种针对结构化信念表的预算探测算子，通过类型分层分析区分程序性信念（如工具依赖）和空间性信念（如物体位置、图边）的不同修复策略。实验表明，当探测策略遵循任务结构时，规划中期的环境证据能有效减少终端世界模型误差。


<details>
  <summary>Details</summary>
Motivation: 长程语言智能体不仅选择动作，还携带私有世界模型。当模型发生漂移时，后续失败可能在失败动作执行前就已注定。现有方法未能充分利用环境交互作为校准资源，智能体信心可能是不可靠的指导，尤其当世界在屏幕外发生变化时。

Method: 提出一种针对结构化信念表的预算探测算子（budgeted probing operator），通过类型分层分析将信念分为：1）程序性信念（如工具依赖），可通过针对性检查修复但消耗任务所需步骤；2）空间性信念（如物体位置、图边），更依赖结构线索。进行受控实验验证不同探测策略的效果。

Result: 实验表明，当探测策略遵循任务结构时，规划中期的环境证据能有效减少终端世界模型误差。程序性信念和空间性信念的修复策略确实存在差异，需要不同的探测方法。

Conclusion: 环境交互应被视为稀缺的校准资源而非单纯的推进任务工具。智能体应在规划中期主动获取环境证据，且探测策略需根据信念类型（程序性vs空间性）采用不同方案，遵循任务结构的探测策略能显著降低世界模型误差。
...
### [195] [CDR-Bench: Evaluating Faithful Execution of Compositional, Order-Sensitive Data Refinement Recipes](https://arxiv.org/abs/2606.31435)
*Yuchen Huang,Xiang Li,Zhenqing Ling,Sijia Li,Qianli Shen,Daoyuan Chen,Yi R. Fung,Yaliang Li*

Main category: cs.AI

TL;DR: CDR-Bench是一个综合基准测试，用于评估大型语言模型在数据精炼任务中的表现。该基准包含3,462个高质量任务，涵盖4个真实世界数据精炼领域和29个不同操作符，在原子性、顺序无关和顺序敏感三种设置下评估模型。实验表明当前LLM在组合设置中性能急剧下降，顺序敏感任务成功率崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试要么孤立文本编辑，要么将其与代码和工具执行纠缠在一起，尚不清楚LLM是否能直接忠实地执行这些组合性、顺序敏感的数据精炼配方。

Method: 构建CDR-Bench基准测试，包含3,462个高质量任务，涵盖4个真实世界数据精炼领域和29个不同操作符。利用确定性参考输出进行精确评估，在原子性、顺序无关和顺序敏感三种设置下测试10多个最先进的LLM。

Result: 实验揭示了一致的失败模式：在组合设置中性能急剧下降，顺序敏感配方的成功率达到崩溃程度。

Conclusion: 当前LLM缺乏可靠组合数据精炼所需的程序忠实性。
...
### [196] [Who Determines the Meaning of an Emotion? Affective Sovereignty as an Epistemic Consequence of Measurement Limits](https://arxiv.org/abs/2606.31442)
*Keito Inoshita*

Main category: cs.AI

TL;DR: 本文提出了"情感域"(Affectosphere)概念，研究情感AI中谁拥有解释个体情感意义的最终权威。通过定义"意义分布"并将其不确定性分解为可还原和不可还原部分，作者发现情感AI在个体层面的不可还原意义无法在现实注释者数量下得到充分估计，这就是"认知鸿沟"。基于此，文章提出了"情感主权"规范：情感体验者对自身情感意义拥有最终解释权。


<details>
  <summary>Details</summary>
Motivation: 情感传感AI正快速嵌入车辆、家电、对话代理和社会基础设施中，形成情感不再局限于个人体验而是能被观察和计算的社全尺度领域。然而，一个核心规范性问题始终未被充分探讨：谁有最终权威来确定个人情感的意义？

Method: 将意义分布定义为在固定注释协议下从人群中抽取的注释者分配的标签分布，并将其不确定性分解为可还原和不可还原两部分。通过演示在个体层面上，即使情感AI可以分配高置信度点标签并在聚合层面区分真实差异，但在现实注释者数量下，意义分布的不可还原部分无法得到充分覆盖估计。

Result: 关键发现：高设备置信度并不构成已恢复不可恢复意义的证据。情感AI虽然能在聚合层面给出高置信度标签和区分真实差异，但个体层面的意义分布的不可还原部分在现实注释者数量下无法获得充分覆盖估计，这种系统性分歧被称为"认知鸿沟"。

Conclusion: 从认知鸿沟和规范前提（无法原则上恢复某量的系统输出不应被视为其权威确定）出发，得出情感主权的规范：情感体验者对自身情感意义的最终解释权应被程序性保留。研究建议情感AI的设计、评估和监管应以明确的解释权分配而非准确度最大化为核心。
...
### [197] [CSTrader: A Testbed for Language-Grounded Trading in a Community-Driven Virtual Asset Market](https://arxiv.org/abs/2606.31461)
*Yao Shi,Kingfung Luo,Nan Tang,Yuyu Luo*

Main category: cs.AI

TL;DR: CSTrader是一个基于多智能体框架的语言驱动交易系统，专门用于CS2武器皮肤市场的交易决策。该系统将非结构化文本转换为买入、卖出或持有决策，在高度波动期实现了7.58%的累计回报。


<details>
  <summary>Details</summary>
Motivation: 小众资产市场（如CS2皮肤市场）规模小、波动大、受社区讨论和平台规则影响大，传统量化模型难以处理，但非常适合研究大语言模型如何将非结构化文本转化为交易行为。

Method: 系统首先整合来自不同源的异构信号，然后使用专业技术分析、流动性、事件和（反向）情感智能体，最后应用风险控制、交易摩擦和投资组合管理智能体，在现实交易摩擦下产生交易决策。评估了多种最新LLM骨干模型。

Result: CSTrader在所有模型中一致超越下跌市场指数（-15.62%），实现最高7.58%的累计回报且风险可控。消融研究表明流动性、反向情感和交易摩擦智能体对于将噪声语言信号转化为稳定利润至关重要。

Conclusion: 小众语言驱动市场是未来语言到动作研究的有用基准，验证了多智能体框架在处理非结构化信息进行金融交易方面的有效性。
...
### [198] [CLOUDADV: Decision-Aligned Instance Sizing with Zero-Shot Foundation Models under Drift](https://arxiv.org/abs/2606.31470)
*Jack Bell,Giacomo Carfi,Gerlando Gramaglia,Andrea Simioni,Daniele Fontani,Vincenzo Lomonaco*

Main category: cs.AI

TL;DR: CLOUDADV是一个面向工程师的云虚拟机实例规模调整交互式顾问系统，结合零样本时间序列预测与有界推荐生成，在日、周、月三种规划周期下提供实例规模建议。该系统使用高容量LLM离线生成参考推荐，同时评估小型生产模型在部署时的对齐情况。


<details>
  <summary>Details</summary>
Motivation: 云虚拟机经常过度配置，导致可避免的成本和运营效率低下问题，需要一个能够适应工作负载变化的智能规模调整系统。

Method: 系统结合零样本时间序列预测模型（Chronos-2）与有界推荐生成，使用历史利用率、预测摘要、当前VM元数据、候选实例选项、定价和显式规模启发式方法构建结构化决策上下文。

Result: 在7个生产VM的案例研究中，参考推荐将模拟月度成本从约1503美元降至708美元，每月节省795美元（52.9%），降级案例中最高超限率为1.5%。Chronos-2虽然不是所有预测指标最优，但能产生与监督基线类似的推荐模式。

Conclusion: 零样本基础模型可以支持非平稳云环境中的决策对齐配置，同时减少每个租户重复训练、重新验证和重新部署的运营负担。
...
### [199] [One Reflection Is Not Enough: Self-Correcting Autonomous Research via Multi-Hypothesis Failure Attribution](https://arxiv.org/abs/2606.31478)
*Jie Ma,Binfei Chu,Jie Gao,Jinlu Zhang,Yiwei Ma,Yi Tan,Jiayi Ji,Xiaoshuai Sun,Rongrong Ji*

Main category: cs.AI

TL;DR: SAGE是一个自我纠正的自主实验者，通过多假设失败归因(MHFA)机制处理实验失败恢复问题。该方法将恢复视为结构化因果诊断，系统生成多个基于证据的失败解释，评估严重性，并将验证的根因定向到正确干预层。同时使用约束报告机制确保结果基于实际测量值。


<details>
  <summary>Details</summary>
Motivation: 当前自主研究代理在实验失败时的恢复能力脆弱。现行范式将丰富的指标、日志和设计选择轨迹压缩为单一反思，导致局部试错或丢弃有用信息的硬转向。SAGE旨在解决这一失败恢复瓶颈。

Method: 核心机制是多假设失败归因(MHFA)：1)分析动态轨迹特征；2)系统生成多个基于证据的失败解释；3)独立评估严重性；4)将验证的根因定向到正确干预层(假设、实验设计或实现)。此外使用约束报告机制将结果限制在实际测量值，删除幻觉数字。

Result: 在12个主题、5个领域的基准测试中：1)带指标输出从42%提升至92%；2)制品质量从5.00提升至6.75/10；3)盲测超越AI-Scientist-v2(52.0 vs 48.2)；4)收益集中在代码开发和执行环节。

Conclusion: 通过将结构化恢复与显式约束结合，SAGE显著优于单一反思范式，为未来自主研究建立了更可靠的基础。尽管完全自主的科学写作和生成会议就绪论文仍是开放问题，SAGE能产生更可靠和更高质量的科学制品。
...
### [200] [Surprise as a Signal for Plasticity and Metacognition](https://arxiv.org/abs/2606.31495)
*Louis Mouchon*

Main category: cs.AI

TL;DR: 研究提出一个基于预测误差信号的核心思想，该信号可作为可塑性门控和元认知的基底。系统一使用非参数情景记忆，仅在高惊讶度时写入新概念，通过离线回放巩固到慢速线性读出。在1000类ImageNet上，DINOv2和I-JEPA backbone分别恢复17.7和51.3点的旧类保留率。系统二在共享text-image空间中用同样信号调节VLM行为，根据熟悉程度给出断言/犹豫/拒绝回答的不同响应。外部检测器AUROC达0.966，远超模型自身置信度(0.618)。


<details>
  <summary>Details</summary>
Motivation: 核心动机：探索预测误差信号的双重功能——既作为 plasticity gate（可塑性门控），又作为 metacognition（元认知）的 substrate（基底）。作者希望构建能像人类一样知道"自己知道什么"的AI系统，在持续学习和知识整合中区分已知/部分熟悉/全新概念。

Method: 方法一（视觉任务）：使用冻结的DINOv2/I-JEPA编码器计算latent space上的预测误差作为surprise信号 → 非参数情景记忆只在高surprise时写入新概念 → 周期性离线replay将近期痕迹巩固到慢速线性读出。方法二（视觉-语言）：在共享text-image空间计算surprise信号 → 调节VLM行为：已知→断言回答；部分熟悉→犹豫回答；全新→拒绝并请求解释，从单次用户输入学习新概念。

Result: 视觉系统：DINOv2 backbone旧类保留恢复17.7点，I-JEPA恢复51.3点；ablation显示仅回放近期窗口比不回放更差；5-way 1-shot mini-ImageNet达91.6%。视觉-语言系统：外部检测器区分已知/新概念的AUROC=0.966 (95%CI±0.024)，远高于模型自身口头置信度0.618；greedy解码下token置信度低于 chance；睡眠清空fast store后，仍能回忆99.2%的50个已巩固事实，base model无法回忆。

Conclusion: 验证了预测误差信号可同时作为可塑性门控和元认知基底的两个proof-of-concept系统。系统能区分已知/部分熟悉/全新概念并自适应响应，在持续学习和知识巩固中展现类似人类"知道自己知道什么"的能力。局限性包括500-way regime暴露困难、token-level confidence低于chance等。
...
### [201] [Design and Implementation of Agentic Orchestrations and Orchestration of Agents](https://arxiv.org/abs/2606.31518)
*Stefanie Rinderle-Ma,Juergen Mangler,Johannes Loebbecke,Dominik Voigt,Nataliia Klievtsova,Matthias Ehrendorfer*

Main category: cs.AI

TL;DR: 本文提出了一个分类框架，用于评估和设计基于LLM的AI代理在业务流程管理中的编排选项，提供了定性决策标准和定量评估指标，并通过预测光照传感场景演示了不同代理实现的评估方法。


<details>
  <summary>Details</summary>
Motivation: AI代理（尤其是基于LLM的代理）的自主性与业务流程的稳健性、可追踪性和可处理性之间需要平衡，结合流程技术来确保代理在业务流程中的有效应用。

Method: 提出分类框架，包含任务特异性、可追踪性/可处理性、自主性/响应性、正确性保证等属性；提供定性决策标准；定义定量评估指标；通过预测光照传感场景的多个代理实现进行演示。

Result: 建立了代理编排选项的分类框架，提供了属性、决策标准和度量指标，为代理业务流程管理的设计与实现提供系统化指导。

Conclusion: 本研究为代理业务流程管理领域提供了系统的属性、决策标准和度量工具，支持在业务流程中有效整合AI代理的编排设计与实现。
...
### [202] [A time-series classification framework for individual-level absenteeism prediction under severe class imbalance](https://arxiv.org/abs/2606.31532)
*Kwong Ho Li,Matthew Roughan,Wathsala Karunarathne*

Main category: cs.AI

TL;DR: 本文提出了一种时间序列分类(TSC)框架，用于预测员工缺勤情况。与传统方法不同，该框架将历史考勤序列与未来缺勤标签分离，实现真正的主动预测。针对严重类别不平衡问题，分析了二元焦损失(BFL)和几何平均(G-Mean)损失，并评估了LSTM、CNN和LSTM-FCN三种深度学习架构。


<details>
  <summary>Details</summary>
Motivation: 现有回归和分类方法存在结构性局限：它们将时间t观察到的特征映射到同一时间t的标签，重现已发生的结果而非预测未来事件，且丢弃了个人考勤历史中的顺序行为结构。在医疗、急救、肉类加工、快递等高需求工作环境中，员工缺勤造成重大运营成本，迫切需要可靠的个体层面缺勤预测。

Method: 1.构建了基于UCI数据集校准的可复现模拟数据集；2.分析了BFL和G-Mean在严重类别不平衡下的表现，仅使用不平衡比率ρ；3.对于BFL，初始梯度比率为ρα/(1-α)，得出平衡权重α=1/(1+ρ)≈0.023；4.评估了三种深度学习架构：LSTM、CNN和LSTM-FCN；5.实验参数：批量大小≥64，窗口大小40-80天。

Result: BFL实现特异性0.813，平衡准确率0.888，与G-Mean相当。G-Mean无需参数校准即可自动适应。三种架构中，LSTM-FCN提供较强的精确率和特异性。稳定性能下平衡准确率约为80%。

Conclusion: BFL和G-Mean在严重类别不平衡下表现相当，性能主要由α决定；LSTM-FCN在缺勤预测任务中表现最优；批量大小≥64和窗口大小40-80天可获得稳定性能；G-Mean的优势在于无需手动校准参数。
...
### [203] [Modality-Driven Search with Holistic Trace Judging for ARC-AGI-2](https://arxiv.org/abs/2606.31543)
*Johan Land*

Main category: cs.AI

TL;DR: 本文提出了一个用于ARC-AGI-2（少样本视觉推理基准）的求解器，通过将推理模态视为搜索算子，并在单一长上下文提示中对所有候选推理轨迹进行联合评判，实现了72.9%的准确率（USD 38.99/任务），显著超越GPT-5.2 Pro和Gemini 3 Pro。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在抽象推理任务中虽然能产生流畅且内部一致的推理痕迹，但仍可能自信地犯错，因此从候选答案中选择正确解成为核心挑战。现有方法如自一致性或多数投票无法可靠地恢复正确的少数假设。

Method: 提出两种原则：(1)将推理模态视为搜索算子，在文本、图像和代码通道中独立生成多样化候选；(2)上下文保留的整体评判，在单一长上下文提示中联合比较所有候选推理轨迹。不同于自一致性或多数投票，该方法能在模态答案错误时可靠地恢复正确的少数假设。

Result: 在ARC Prize半私有评估集上达到72.9%（USD 38.99/任务），超越GPT-5.2 Pro（54.2%）和Gemini 3 Pro（54.0%）18.7个百分点。在公共评估集上达到76.1%（USD 19.69/任务）。

Conclusion: 研究发现规范性提示模板和迭代细化会系统性地降低假设多样性并导致性能下降。该求解器在验证排行榜上取得最高分，发布了完整源代码并记录了大量负面结果。
...
### [204] [ACE: Pluggable Adaptive Context Elasticizer across Agents](https://arxiv.org/abs/2606.31564)
*Ning Liao,Zihao Long,Xiaoxing Wang,Xue Yang,Yaoming Wang,Ziyuan Zhuang,Xunliang Cai,Rongxiang Weng,Junchi Yan*

Main category: cs.AI

TL;DR: 提出自适应上下文弹性器(ACE)模块,解决大型语言模型智能体在处理长轨迹任务时受限于固定上下文窗口的问题,可动态调整历史步骤信息在上下文中的呈现方式。


<details>
  <summary>Details</summary>
Motivation: 随着智能体任务复杂度增加,轨迹长度快速增长,现有上下文管理技术(如截断和摘要)存在固有的不灵活性和不可逆性:一旦信息被丢弃或压缩,即使后续决策步骤变得关键相关也无法恢复。

Method: ACE采用即插即用模块设计,包含无损消息维护层(存储原始消息和压缩抽象)和上下文编排层(基于当前任务状态为每个历史步骤动态分配弹性类型:原始、抽象或丢弃)。该设计具有可逆性,确保主LLM始终收到紧凑但信息丰富的上下文。ACE可适配四种智能体框架(ReAct、DeepAgent、WebThinker、MiroFlow),无需训练或架构修改。

Result: 实验表明,ACE始终优于截断和摘要基线方法,并在所有四种智能体框架上带来一致的性能提升。

Conclusion: ACE通过可逆的自适应上下文管理机制,有效解决了LLM智能体在长轨迹任务中的上下文窗口限制问题,且具有良好的通用性和实用性。
...
### [205] [Which Tokens Matter? Adaptive Token Selection for RLVR with the Relative Surprisal Index](https://arxiv.org/abs/2606.31575)
*Outongyi Lv,Yanzhao Zheng,Yuanwei Zhang,Zhenghao Huang,Xingjun Wang,Baohua Dong,Hangcheng Zhu,Yingda Chen*

Main category: cs.AI

TL;DR: 本文提出RSI（相对惊讶指数）来解决RLVR中关于高熵和低概率token的矛盾观点，通过信息论方法耦合token熵与选择概率，并基于RSI提出RSI-S过滤方法，在AIME和AMC基准上相比GRPO提升2-3个百分点。


<details>
  <summary>Details</summary>
Motivation: RLVR已成为推进LLM推理的重要范式，但现有研究对高熵token和低概率token的训练权重存在矛盾观点：一派主张优先训练高熵位置，另一派反对低概率token主导梯度更新。这种矛盾源于单独评估采样token概率或熵的局限性。

Method: 提出相对惊讶指数(RSI)作为信息论度量，将token熵与所选token概率自然耦合；在温和条件下，RSI与logit梯度范数一阶变化和预测熵的局部比率相关；基于RSI提出RSI-S（RSI选择）方法，保留稳定RSI区间的token。

Result: RSI-S成功调和了先前矛盾的范式，过滤掉冗余低惊讶token和不稳定高惊讶尾token；在Qwen2.5-1.5B、3B和7B模型上，RSI-S在AIME和AMC基准上实现更高的avg@32准确率，比GRPO提升2-3个百分点。

Conclusion: RSI为RLVR改进提供了有前景的视角，通过信息论方法统一了此前关于高熵和低概率token的矛盾观点，RSI-S作为熵自适应token过滤方法能有效提升LLM推理能力。
...
### [206] [Scientific Explanations in Health Sciences: Causality, Trust, and Epistemic Adequacy](https://arxiv.org/abs/2606.31616)
*Martina Mattioli,Marcello Pelillo*

Main category: cs.AI

TL;DR: 本文批判性地审视了哲学科学与可解释人工智能(XAI)交叉领域的医学解释问题，指出当前XAI研究忽视了哲学传统中对解释的本质探讨，并基于哲学文献提出了医学可解释性设计的三个核心分析轴。


<details>
  <summary>Details</summary>
Motivation: 医学AI虽被广泛预期将改变临床实践，但其决策过程不透明。尽管可解释性被视为补救措施，但关于何为充分医学解释的争论仍未解决，而哲学科学和医学中对解释的长期研究在当代XAI中未被重视。

Method: 采用批判性综述方法，审视健康科学中关于何为解释的流行观点，评估其对医学XAI的适用性，并整合哲学分析与当前医学AI发展。

Result: 识别出三个核心分析轴：1)因果关系在医学推理中的作用；2)医学信任的认知和关系维度；3)由不同利益相关者实际需求塑造的解释充分性标准。

Conclusion: 为设计XAI系统提供原则，使其提供不仅在认知上稳健而且符合临床决策认知和实践要求的解释，为医学XAI的持续争论奠定概念基础。
...
### [207] [Think in English, Answer in Korean: Efficient Adaptation of Multilingual Tool-Using Agents](https://arxiv.org/abs/2606.31648)
*Utsav Garg,Sungjin Hong,Jason Jung,Justin Lee,Shaan Desai,Joon Hee Kim,Anirudh Shrinivason,Edmond Wen,Susie Park*

Main category: cs.AI

TL;DR: LuckyStar 111B是由Cohere和LG CNS合作开发的111B参数混合推理模型，专门针对韩英企业代理场景，在有限内存和服务约束下进行优化。该模型基于Command A进行后训练，使用 preamble conditioning 在简洁非推理行为和工具导向推理之间切换。


<details>
  <summary>Details</summary>
Motivation: 在实用内存和服务限制下，为韩英企业代理开发高效的混合推理模型，并解决多语言模型在代理工作流中的适配问题。

Method: 1) 基于完全后训练的Command A模型进行训练；2) 使用preamble conditioning实现推理模式切换；3) 研究四种扩展工具使用代理的方案：多语言监督微调、基于可验证奖励的强化学习、韩语语言一致性奖励、以及4-bit量化单GPU服务。

Result: 模型在数学推理、函数调用和NL2SQL任务上取得显著提升，同时保持了通用的韩英指令跟随能力。

Conclusion: 为在内存约束部署环境下将后训练多语言模型适配至可验证代理工作流提供了实用方案和失败模式分析。
...
### [208] [FARS: A Fully Automated Research System Deployed at Scale](https://arxiv.org/abs/2606.31651)
*Qiong Tang,Xiangkun Hu,Xiangyang Liu,Yiran Chen,Yunfan Shao*

Main category: cs.AI

TL;DR: FARS是一个完全自动化的AI研究系统，能够自主生成和推进研究项目，涵盖构思、规划、实验和写作阶段。在首次公开部署中生成了166篇研究论文，覆盖67个AI/ML细分主题，并保留了完整的中间产物作为可审计语料库。


<details>
  <summary>Details</summary>
Motivation: 解决现有自动化研究系统的局限性：证据主要来自精选案例、人工设定主题或少量预定义任务，需要一个能够在广泛研究主题上规模化运行的完全自动化系统。

Method: 使用阶段特定代理（stage-specific agents）协调工作，通过共享工作空间记录提案、代码、日志、结果和手稿，实现研究项目的自主生成和推进。

Result: 生成了166篇完整研究论文，覆盖67个细粒度AI/ML主题；收集了282份结构化评审（覆盖140篇论文），包括总体评分、子评分、诚信检查和LLM使用披露。

Conclusion: FARS能够在大规模公共部署中产生值得评审的、有时甚至是优秀的AI/ML研究工件，同时也暴露了狭窄实验范围、方法论限制和诚信问题等反复出现的失败模式。
...
### [209] [Arena-T2I Hard: Benchmarking and Improving Faithfulness with Dependency-Aware Checklist](https://arxiv.org/abs/2606.31711)
*Yuanhao Ban,Tong Xie,Sohyun An,Yunqi Hong,Evan Frick,I-Hung Hsu,Wei-Lin Chiang,Ion Stoica,Cho-Jui Hsieh*

Main category: cs.AI

TL;DR: 本文提出了Arena-T2I Hard基准测试，用于评估文本到图像(T2I)模型的"忠诚度"（即生成图像与提示词的匹配程度）。该基准包含310个提示词，每个提示词约30个分解的是/否约束，涵盖六个类别（包括文本渲染）。同时提出了依赖感知的清单奖励（dependency-aware checklist reward），结合BT美学奖励，在SD3.5-Medium和FLUX.1-dev上实现了更好的忠诚度-美学权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的忠诚度基准依赖于简单的原子指令，顶级系统已接近满分。在创意工作流中，用户请求复杂（空间关系、风格约束、复杂文本渲染），单一二值VLM评分无法捕捉模型具体失败哪些约束。公开竞技场排名也无法预测忠诚度，因为BT偏好分数优先考虑美学而非细粒度提示 adherence。

Method: 1) 构建Arena-T2I Hard基准：310个提示词来自真实竞技场日志，每个约30个分解的是/否约束，涵盖六类；2) 提出依赖感知的清单奖励：将每个提示词分解为DAG结构的是/否问题，失败父节点的子节点也计零；3) 结合BT美学奖励（通过GDPO组内归一化），在训练时同时优化两个目标。

Result: 1) 最强闭源系统得分0.855，11个系统间差距33个百分点，显示高区分度；2) 公开竞技场排名无法预测忠诚度；3) 在SD3.5-Medium和FLUX.1-dev上，该方法在MMRB2成对比较中实现了比所有单奖励、加权求和或4奖励BT集成基线更好的忠诚度-美学权衡。

Conclusion: 依赖感知的清单奖励可有效将忠诚度转化为每约束训练信号，配合GDPO标准化的BT美学奖励，能够突破忠诚度-美学的权衡困境，为T2I模型的细粒度评估与训练提供了新范式。
...
### [210] [A Self-Evolving Agentic System for Automated Generation and Execution of Biological Protocols](https://arxiv.org/abs/2606.31763)
...
TL;DR: ProtoPilot是一个自进化的多智能体系统，用于解决自主湿实验室自动化中的协议到代码转换问题，包含294个合成生物学和分子生物学任务的基准测试框架，实现了90.2%的专家偏好率和89.5%的协议到代码门禁通过率


<details>
  <summary>Details</summary>
Motivation: 自主湿实验室实验不仅需要合理的协议文本，还需要生物意图、定量程序、设备约束和实验反馈在从协议和SOP设计到代码和物理执行的整个过程中保持一致

Method: 开发了ProtoPilot系统，包含分层可验证性、多智能体编排和运行时更新的技能库；构建了包含294个任务的基准测试框架，涵盖合成生物学和分子生物学任务，基于98个黄金标准协议、专家评分规则、设备级有效性检验和真实实验测试

Result: Top@3专家偏好率达到90.2%，协议到代码门禁通过率为89.5%，Opentrons通过率为88.24%（对比OpenTrons-AI的32.35%）；湿实验室验证产生了可解释的读数、经Sanger确认的产物和反馈纠正的PCA组装DNA目标

Conclusion: ProtoPilot通过分层可验证性、多智能体编排和运行时更新的技能库，实现了协议到代码的转换及实验反馈引导的修订，为自主湿实验室自动化提供了可验证的路径。
...
### [211] [Evo-PI: Aligning Medical Reasoning via Evolving Principle-Guided Supervision](https://arxiv.org/abs/2606.31800)
*Xianda Zheng,Huan Gao,Meng-Fen Chiang,Michael Witbrock,Kaiqi Zhao,Shangyang Li*

Main category: cs.AI

TL;DR: Evo-PI是一个原则中心的学习框架，通过共进化监督机制改进大型多模态语言模型(MLLMs)的推理能力，在医学视觉问答任务中实现最高24.6%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs依赖静态监督(固定提示、规则或奖励模型)，无法塑造底层推理过程，导致复杂决策任务中泛化能力脆弱和性能饱和。

Method: Evo-PI将推理原则视为显式的语言监督信号，可生成、评估和迭代进化。实现共进化循环：原则指导模型推理，模型行为反过来优化监督原则，形成动态对齐机制使监督能够适应模型的推理缺陷。在医学视觉问答任务中验证。

Result: 在8个基准测试和多个模型骨干上，Evo-PI持续提升推理准确率，最高增益达24.6%。

Conclusion: 进化原则引导的监督为训练MLLMs中专家对齐的推理提供了一种可扩展且通用的范式。
...
### [212] [RAISE: LLM-based Automated Heuristic Design with Robust Adversary Instance Search](https://arxiv.org/abs/2606.31801)
*Fei Liu,Alessio Figalli,Patrick Owen,Nicola Serra*

Main category: cs.AI

TL;DR: RAISE是一个基于大语言模型的鲁棒启发式设计框架，通过在训练分布的ε-邻域内进行约束最坏情况实例搜索，解决现有LLM-based AHD方法在分布偏移下性能急剧下降的问题（最多下降19倍）。


<details>
  <summary>Details</summary>
Motivation: 现有LLM-based AHD方法针对固定训练实例集进行优化，在真实世界的分布偏移场景下可能表现灾难性下降，需要提升启发式算法的鲁棒性。

Method: 提出RAISE框架，将约束最坏情况实例搜索整合到LLM-based进化搜索循环中。外层循环使用LLM算子进化启发式算法，内层循环使用基于边界投影的基分布参数化方法，在训练实例集ε-邻域内高效识别困难实例。

Result: 在在线装箱(OBP)、在线作业车间调度(OJSP)和在线车辆路径规划(OVRP)三个问题上，对五个分布家族的实验表明：现有LLM-based AHD方法在分布偏移下性能下降高达19倍，而RAISE在所有测试分布和规模下保持强劲性能。

Conclusion: RAISE能够有效解决LLM-based AHD中的分布偏移问题，在不同分布和问题规模下保持稳定的高性能，具有重要的实际应用价值。
...
### [213] [Large Databases Need Small, Open-Weight Language Models](https://arxiv.org/abs/2606.31808)
*Parker Glenn,Alfy Samuel*

Main category: cs.AI

TL;DR: 本文展示了在16GB VRAM上运行的量化开源模型能够匹配或超越闭源API的准确性，同时成本大幅降低。通过将本地模型集成到BlendSQL框架，实现了390倍的成本降低和3.8倍的延迟改善。


<details>
  <summary>Details</summary>
Motivation: 基于闭源API的语言模型系统在大型数据库场景下成本过高，单次实验可能超过10,000美元，阻碍了研究和实际部署。

Method: 使用量化开源权重模型在本地部署（16GB VRAM），并集成到BlendSQL v0.1.0框架中，实现关键系统优化。

Result: 在保持相当或更好准确性的同时，实现了390倍的整体成本降低和3.8倍的延迟降低。

Conclusion: 挑战了闭源LM API对于有效LM-数据库集成的必要性这一 prevailing assumption，证明本地开源模型是可行的替代方案。
...
### [214] [Creating Intelligence: A Computational Foundation for AGI](https://arxiv.org/abs/2606.31819)
*Peter Overmann*

Main category: cs.AI

TL;DR: 提出一种基于集合论和超维计算的心灵计算理论，使用稀疏二进制数据表示信息，通过拓扑可塑性学习，在组合扩展隐藏层中实现联想记忆，统一自联想和异联想学习，以常数时间复杂度通过子集模式匹配进行信息检索，可映射到小脑和新皮层，有望实现高效节能的合成智能。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络依赖连续权重和矩阵乘法，存在能量效率瓶颈；现有计算理论无法统一解释皮层和小脑的认知功能；需要一种更接近生物神经群体编码的计算框架。

Method: 将信息表示为离散集合，使用稀疏二进制数据；通过组合扩展隐藏层实现拓扑可塑性；使用子集模式匹配和精确最近邻搜索进行信息检索；将框架映射到神经解剖学（小脑和新皮层）。

Result: 联想记忆从组合扩展隐藏层自然涌现；自联想和异联想学习统一于单一核心算法；实现常数时间复杂度；建立感知数据（稀疏分布表示）与符号（稀疏全息表示）之间的桥梁。

Conclusion: 子集模式匹配可能是认知的基础引擎；该算法可直接转化为内存计算硬件；为实现人类水平能效的合成智能提供新途径。
...
### [215] [Adaptive Cluster-First Route-Second Decomposition for Industrial-Scale Vehicle Routing](https://arxiv.org/abs/2606.31820)
*Oguzhan Karaahmetoglu,Hyong Kim*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型(LLM)的自适应聚类优先路由其次(CFRS)系统，用于解决大规模容量车辆路径问题(CVRP)。该系统将分解过程形式化为迭代决策过程，利用LLM作为高级决策者来分析分解状态并选择性应用聚类、平衡和优化算子。


<details>
  <summary>Details</summary>
Motivation: 现有分割方法通常依赖固定分区规则、预定义优化目标或学习策略，在不同空间、需求和运营特征的问题实例上表现不一致。需要一种能够自适应根据问题特征调整分解策略的方法。

Method: 提出一种自适应CFRS系统，采用LLM作为高级决策者，分析不断变化的分解状态，选择性地应用聚类、平衡和优化算子。算法同时对客户和车辆进行分区，实现容量感知聚类，同时使分区决策适应每个问题的特征。

Result: 在包含多达500,000个客户的合成和基准CVRP实例上进行了评估。实验结果表明，在基准规模实例上具有竞争力的性能，同时在更大规模问题上表现出更好的可扩展性和稳健的路由质量。

Conclusion: 结果突出了自适应、LLM引导的决策支持作为工业规模车辆路径规划和大规模物流规划的实用方法的潜力。
...
### [216] [An Agentic AI Framework to Accelerate Scientific Discovery in Plant Phenotyping](https://arxiv.org/abs/2606.31831)
...
TL;DR:  Oak Ridge国家实验室推出端到端代理AI框架，将高通量植物表型分析设施从数据工厂转变为交互式自主发现平台，解决数据生成速度快于分析速度的瓶颈问题。


<details>
  <summary>Details</summary>
Motivation:  当前植物表型分析数据生成速度远超科学家分析能力，分析环节成为发现过程的制约因素。现有方法依赖人工专家、事后分析，无法满足大规模数据处理需求。

Method:  构建对话式协同科学家代理(Co-Scientist Agent)将自然语言问题转化为结构化分析计划；头计算代理(Compute Agent)在Frontier超算上执行视觉Transformer分割和性状提取。两代理在独立安全域运行，通过安全令牌认证流通道通信，确保端到端数据溯源。

Result:  将原本需要数天至数周的分析流程转变为交互式循环，代理可在数秒内推理结果、推荐下一步分析并响应后续问题。

Conclusion:  该框架实现了科学家与AI代理协作的自主发现范式，兼顾云原生代理框架忽视的联邦、数据迁移和数据溯源需求，为大规模表型数据分析提供可扩展解决方案。
...
### [217] [Harnessing Textual Refusal Directions for Multimodal Safety](https://arxiv.org/abs/2606.31876)
*Moreno D'Incà,Massimiliano Mancini,Nicu Sebe*

Main category: cs.AI

TL;DR: 本文提出MARS方法，利用LLMbackbone中的文本拒绝方向来实现跨模态（图像、视频）安全对齐，无需多模态安全数据即可提升多模态大语言模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型(MLLMs)的安全对齐面临困境：要么需要后训练对齐，要么需要利用激活空间中的拒绝方向，但这两种方法在MLLMs中都不可行，因为需要难以收集的多模态不安全数据。本文旨在探索文本拒绝方向是否能跨模态泛化。

Method: 提出Modality-Agnostic Refusal Steering (MARS)方法：1)通过激活重中心化修正模态错位；2)在几何定义的信任区域内自适应缩放转向强度；3)选择最佳干预层，在第一个生成token时进行操作。

Result: 在5个SOTA MLLMs上评估，MARS在安全、实用性和视频越狱基准上取得一致的安全增益，同时保持实用性。发现安全相关结构在模态间共享，文本拒绝方向是多模态对齐的强大基础。

Conclusion: 文本拒绝方向可跨模态泛化，安全相关结构在模态间共享。MARS是一种无需多模态安全数据的轻量级、无训练的多模态安全注入方法。
...
TL;DR: 本文揭示了合成问答数据生成过程中的系统性偏见：问题生成倾向于选择文档中显著的区域，而答案生成容易服从文本中的指令性段落。这些缺陷源于QA生成阶段的选择策略，而非下游训练过程。


<details>
  <summary>Details</summary>
Motivation: 语言模型越来越多地从合成问答对中进行学习，但作者发现生成过程并非中立的预处理步骤，而是隐式地决定了哪些证据成为训练信号以及如何回答这些证据。

Method: 通过系统性地分析问题生成的覆盖模式和答案生成对指令性文本的服从程度，评估了不同模型家族和规模下的失败模式，并提出了两种改进策略：将每个问题绑定到固定目标，以及在 answering 前过滤指令性片段。

Result: 问题生成存在覆盖饱和现象，集中在显著片段；指令性段落的服从率高达88%；较大的模型在任务冲突下反而更服从。改进策略可将注射服从率降至13%，同时保留几乎所有清洁文本。

Conclusion: 这些失败模式源于QA生成阶段的选择策略，无需改变训练循环即可缓解。通过固定目标绑定和指令性片段过滤，可显著降低偏见选择和指令服从率。
...
TL;DR: PolicyGuard是一个神经符号框架，用于政策驱动的文档合规审查。它将组织政策转换为可执行的审查引擎，包括类型化关系逻辑规则和原子级提取问题。LLM回答这些问题并使用符号评估器检测不合规情况。该框架在NDA合规审查场景中进行评估。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在政策解释和文档分析方面可以提供帮助，但端到端提示使所应用的政策逻辑变得隐含，导致合规决策难以检查、更新和测试。需要一种更明确、可维护和可测试的文档审查方法。

Method: PolicyGuard将组织政策指导转换为可执行的审查引擎，包含：(1)类型化关系逻辑规则；(2)原子级提取问题。审查过程中，LLM使用检索到的文档证据回答这些局部问题，符号评估器应用形式规则检测不合规情况。

Result: 通过将政策形式化、局部文档解释和符号合规评估分离，PolicyGuard使文档审查更加明确、可维护和可系统测试。该方法在企业特定NDA合规审查中得到实例化和评估。

Conclusion: PolicyGuard通过神经符号方法解决了政策驱动文档审查的可解释性问题，实现了合规决策的可检查性、可维护性和可测试性，为组织特定的政策合规审查提供了有效框架。
...
TL;DR: 研究Dafny中的智能体代码生成，模型需要同时生成可执行代码和验证证明。提出AxDafny框架和LCB-Pro-Dafny基准集，在验证成功率和性能上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: Dafny是一种具有形式化验证能力的编程语言，但生成同时包含实现代码和验证证明的智能体代码是一个挑战。需要一个能够迭代生成实现、不变量、断言和终止参数的框架。

Method: 提出AxDafny，一个验证器引导的修复框架，通过迭代方式生成实现、不变量、断言和终止参数。同时构建LCB-Pro-Dafny基准集，包含250个竞赛风格的编程问题，并配备形式化规范和基于验证器的评估工具。

Result: 在LCB-Pro-Dafny上，AxDafny相比基线GPT-5.5显著提升验证成功率。在DafnyBench上达到92.7%验证成功率，比此前最强的证明提示基线高出6.5个百分点。

Conclusion: 验证成功率和运行时测试性能衡量生成代码的不同方面，形式化验证是评估代码正确性的重要补充手段。
...
### [221] [Governance Gaps in Agent Interoperability Protocols: What MCP, A2A, and ACP Cannot Express](https://arxiv.org/abs/2606.31498)
*Richard Kang,Yudho Diponegoro*

Main category: cs.MA

TL;DR: 本文对五种主流代理互操作协议（MCP、A2A、ACP、ANP、ERC-8004）进行了系统性的治理需求差距分析，发现当前协议在成员资格、审议、投票、异议保留、人力升级和审计/回放等六个维度上存在不同程度的缺陷，其中投票和异议保留功能普遍缺失，审议功能最多仅为部分支持，表明代理社区治理是需要新架构层的缺失功能，而非协议本身的特性缺失。


<details>
  <summary>Details</summary>
Motivation: 随着企业部署异构代理舰队并需要在治理约束下进行集体决策，亟需确定现有互操作协议是否能支持受治理的代理社区，还是仅能支持任务导向的协调。

Method: 基于组织理论、多代理系统文献和企业治理标准，构建了六维治理需求分类法（成员资格、审议、投票、异议保留、人力升级、审计/回放），对各协议的规范进行逐一分析，将能力分类为支持、部分支持或缺失。

Result: 差距矩阵显示：投票和异议保存在所有五种协议中普遍缺失；审议功能最多为部分支持；没有任何协议编码了受治理代理社区所需的完整原语集合。

Conclusion: 代理社区治理构成了当前互操作标准之上的缺失架构层，而非协议内的功能缺失；该差距需通过新的架构层解决，而非单纯扩展现有协议。
...
### [222] [Holonic Active Distillation for Scalable Multi-Agent Learning in Multi-Sensor Systems](https://arxiv.org/abs/2606.31578)
*Dani Manjah,Tim Bary,Benoît Macq,Stéphane Galland*

Main category: cs.MA

TL;DR: 本文提出了一种基于HMAS的Holonic主动蒸馏架构(CSBAD)，用于解决开放环境中传感器网络的规模化和知识迁移问题。该方法通过让学生模型收集本地数据、查询教师模型的伪标签并进行聚类，实现局部专业化与全局泛化的平衡。实验表明该方法能有效适应传感器的动态加入和退出，但需权衡增量更新、系统重组与可扩展性，并面临模型漂移和长期适应等挑战。


<details>
  <summary>Details</summary>
Motivation: 解决开放环境中传感器网络面临的可扩展性、适应性和知识迁移挑战，特别是新子系统可以动态加入或离开的场景。

Method: 提出Holonic主动蒸馏架构，整合Clustered Stream-Based Active Distillation (CSBAD)框架。学生模型收集本地数据，从教师模型查询伪标签，并聚类成相似传感器组。

Result: 霍尔组织平衡了局部专业化与全局泛化，能有效适应传感器离开和重新集成。分析了增量模型更新、系统重组和可扩展性限制之间的权衡。

Conclusion: 霍尔学习为多传感器系统提供了优势，但需关注模型漂移和长期适应等关键挑战。
...
### [223] [ForecastAgentSearch: Towards a Multi-Expert Agent Search System for Geopolitical Event Forecasting](https://arxiv.org/abs/2606.31665)
*Miaomiao Cai,He Chang,Yunshan Ma,See-kiong Ng*

Main category: cs.MA

TL;DR: ForecastAgentSearch是一个将地缘政治事件预测构建为多专家智能体搜索问题的初步框架，通过分析任务上下文、搜索排序相关专家智能体、协调多智能体分析来生成预测结果。


<details>
  <summary>Details</summary>
Motivation: 地缘政治事件预测面临复杂区域背景、动态事件信号和不确定未来结果的挑战，传统方法难以有效处理多源信息和专家视角，大语言模型智能体的发展为构建新型预测系统提供了新机遇。

Method: 框架首先分析任务上下文，然后根据区域知识、专业领域、可靠性和互补性搜索排序相关专家智能体，选定的智能体提供专业化分析，最后协调这些分析生成带有解释和不确定性意识的最终预测。

Result: 本文提出了一个初步框架，讨论了智能体画像、专家检索、排序和多智能体协调等关键设计挑战，并概述了未来评估协议。

Conclusion: 该工作为可搜索且可靠的多智能体预测系统提供了初步步骤，但目前仍是概念性框架，需要进一步开发和评估。
...
### [224] [Analytic Cut in Epistemic Logics with Distributed Knowledge](https://arxiv.org/abs/2606.31886)
*Ryo Murai,Sizhuo Liu,Katsuhiko Sano*

Main category: cs.MA

TL;DR: 本文研究了基于K45、KD45和S5的分布式知识多主体认知逻辑的证明论性质。通过采用Takano的策略限制切规则中切公式为结论的子公式，建立了所有三个系统的解析切性质，从而证明了Craig插值定理。当允许空群组时，分布式知识算子可解释为全局模态。distributed knowledge in multi-agent systems; sequent calculi; analytic cut property; Craig interpolation


<details>
  <summary>Details</summary>
Motivation: 分布式知识是多主体认知逻辑中研究群体知识的概念，传统语义学通过个体认知可达关系的交集来解释群组的分布式知识。已有研究表明K45和KD45模态逻辑的矢列演算中切消除成立，但上述所有系统（即分布式知识逻辑）中切消除失效。因此需要建立这些系统的分析性质。

Method: 采用Takano(2018)的策略，将切公式限制在结论的子公式集合中，从而建立解析切性质。

Result: 成功为K45、KD45和S5三种分布式知识逻辑系统建立了解析切性质；作为推论，Craig插值定理对所有研究的逻辑成立；且当允许空群组时（分布式知识解释为全局模态），所有证明论结果仍然有效。

Conclusion: 本文为分布式知识逻辑提供了健全的证明论基础，证明了解析切性质和Craig插值定理，扩展了已有结果到空群组情况，为相关逻辑系统的进一步研究奠定了基础。
...
### [225] [MECoBench: A Systematic Study of Multimodal Agent Collaboration in Embodied Environments](https://arxiv.org/abs/2606.31966)
*Qingyun Liu,Jiwen Zhang,Jingyi Hu,Siyuan Wang,Zhongyu Wei*

Main category: cs.MA

TL;DR: MECoBench是一个多模态具身合作基准，用于评估多模态大语言模型(MLLMs)在具身环境中的协作能力，包含多样真实世界任务、两种合作结构和三种协作模式。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型作为具身智能体的潜力已得到广泛探索，但其在视觉 grounding 环境中的协作能力仍未被充分研究。

Method: 构建了包含多样化真实世界任务的评估平台，跨越两种合作结构和三种协作模式，对多种MLLMs进行了广泛实验。

Result: 主要发现：(1)协作通常能提升具身任务完成度，但收益取决于协作收益与协调复杂性的平衡；(2)沟通对协作收益至关重要，最佳协作模式取决于团队规模和模型能力；(3)协作可提升噪声先验和探索条件下的鲁棒性。
