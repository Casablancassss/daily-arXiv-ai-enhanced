...
TL;DR: 研究选择性预测和不确定性感知转诊在宫颈细胞学筛查中的应用，比较两种轻量级Transformer backbone在Herlev Pap染色数据集上的表现，重点评估风险-覆盖率曲线下面积(AURC)而非传统准确率。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在宫颈细胞学筛查中总是被评估为需要对每个预测采取行动，但实际上部署在 cytopathologist 旁边的筛查系统可以延迟处理不确定的病例。研究旨在评估系统的置信度排序能力，而非仅评估正确率。

Method: 使用两种轻量级Transformer backbone (Swin-Tiny和TinyViT-5M)在Herlev数据集上进行微调，采用加权随机采样预训练权重，使用后验温度缩放进行校准，比较单个模型与软投票集成的表现。

Result: 集成模型将AURC降低51.8%(0.0022 vs 0.0045)，零错误覆盖率从18.3%扩展到72.8%，但校准变差(ECE 0.0339 vs 0.0247)，假阴性增加(14 vs 10)。

Conclusion: 区分了两个常被混淆的属性：按可信度排序预测的能力vs置信度值的准确性。集成改善了前者但恶化了后者，而前者直接决定了风险-覆盖率权衡。
...
TL;DR: 本文提出Heisenberg-lift框架，通过引入阶次敏感的几何特征（终端符号面积）增强在线手写识别。该方法只需在现有欧几里得描述符上附加一个标量参数，即可显著提升分类准确率，且计算成本极低。


<details>
  <summary>Details</summary>
Motivation: 现有在线手写识别系统使用固定长度的欧几里得形状描述符，但无法捕捉笔划的书写顺序。相同区域但不同方向书写的笔划无法区分，而在区分字母o和y等字符时，笔划方向至关重要。

Method: 1）终端符号面积（TSA）：最简单的实例，一个无参数的标量，附加到现有欧几里得描述符上；2）15维Heisenberg群细分方案：更丰富的扩展版本

Result: 在两个标准在线手写基准上，单一标量添加持续提升分类准确率。在最难的字符对o和y上，符号面积单独实现完美分离，而欧几里得基线失败。抗坐标噪声能力增强。15维扩展在噪声和环形结构条件下提供额外增益。

Conclusion: 该描述符轻量、闭式可解且可解释，是在线手写及相关文档轨迹分类管道的实用增强方案，特别适用于笔划执行方向具有判别信息的场景。
...
TL;DR: 本文揭示了异构协同感知(Heterogeneous Cooperative Perception, CP)的安全隐患。研究表明，尽管人们普遍认为传感器和检测器的异构性能够抵御对抗性攻击，但实际并非如此。作者提出了两种攻击方法：基于迭代优化的攻击和名为HetPoison的免标签生成器，可在单次前向传播中移除真实目标。同时提出了防御方案HetShield，可恢复83-95%的被攻击精度。


<details>
  <summary>Details</summary>
Motivation: 现有研究认为协同感知中的异构性（不同传感器配置、检测器、翻译模块）能够天然抵御对抗攻击，因为攻击者缺乏对受害者检测器和翻译模块的了解，且翻译模块会扰乱对抗梯度。本文旨在验证这一假设是否成立。

Method: 1. 建立标准化评估框架：统一扰动预算、目标和前向路径；2. 迭代优化攻击：使用匹配目标函数进行迭代反向传播；3. HetPoison：免标签的单次前向传播生成器，可跨异构设计迁移；4. HetShield：轻量级信任层，验证特征的时空一致性。

Result: 1. 迭代优化攻击能够消除或逆转异构性带来的鲁棒性优势；2. HetPoison可匹配或超越迭代优化攻击效果，且无需访问受害者检测器；3. HetShield可恢复83-95%的被攻击精度。

Conclusion: 异构性本身并非有效的防御手段。HetPoison作为实际威胁可在实时运行，而HetShield作为有效防御方案可显著提升系统安全性。
...
TL;DR: 本文提出了一种自适应局部角度的插值细分方法，能够在学习插入角度的同时保持插值结构。该方法使用紧凑的边预测器为每条边分配一个插入角度，在每个细分级别精确复制原始顶点。方法在欧几里得平面、球面和庞加莱盘上使用几何特定的测地线原语进行测试。实验表明，该方法在最近邻误差方面比最佳验证调优的固定张力基线减少5到17倍，比向心Catmull-Rom减少约1.8倍，同时显著降低了弯曲能和切线粗糙度。


<details>
  <summary>Details</summary>
Motivation: 经典的四点法和六点法细分方案虽然能保持插值特性（曲线经过控制点），但其行为由单一全局张力参数控制，难以适应平坦区域、急转弯和局部几何变化的多样化需求。这种局限性促使研究者探索能够自适应调整的局部角度学习方法。

Method: 提出一种自适应局部角度公式，在保持插值结构完整的同时学习每个新顶点的插入方式。具体采用紧凑的边预测器为每条边分配一个插入角度，原始顶点在每个细分级别被精确复制，因此插值是算子的结构属性，不依赖于训练权重。方法在欧几里得平面、球面和庞加莱盘上使用几何特定的测地线原语实现。

Result: 在匹配密度评估协议下，该方法将最近邻误差比最佳验证调优的固定张力基线减少5到17倍，比欧几里得情况下的向心Catmull-Rom减少约1.8倍。同时显著降低了弯曲能和切线粗糙度，且与单独训练的几何特定模型相比具有竞争力。

Conclusion: 该自适应局部角度插值细分方法成功地在保持插值结构的同时实现了局部自适应性，在多种几何空间中展现出优异的性能，为曲线细分提供了一种新的有效途径。
...
TL;DR: 本文提出实例感知双解码器框架，通过在预测实例中心上施加质量守恒和质心对齐的几何约束，将密度估计转化为结构化质量分配问题，在零样本对象计数任务中实现了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: Zero-shot对象计数旨在无需类别特定训练的情况下根据文本查询估计对象数量。现有基于密度的方法存在空间模糊和背景泄漏问题，由于质量分配缺乏规范约束，导致碎片化或部分偏向的表示，增加了复杂场景中的计数误差。

Method: 提出了一种实例感知双解码器框架，将密度和点表示进行结构耦合。将密度估计解释为在潜在对象实例集上的结构化质量分配问题。预测的实例中心诱导密度图的软实例级分解，并强制执行两个几何约束：(1)每个实例的质量守恒，确保每个对象贡献约一个单位的密度质量；(2)质心对齐，鼓励每个密度分量集中在其对应的预测中心周围。

Result: 在FSC-147、PUCPR+和CARPK数据集上进行了广泛实验，结果表明该方法一致地降低了计数误差，在零样本对象计数中建立了新的最先进性能。

Conclusion: 所提出的实例级几何一致性约束引入了更准确的质量分配，从而减少了计数误差。
...
TL;DR: 本文提出了一种保守的几何驱动形式阴影注入方法，用于解决人脸交换中几何对齐但光照不真实的问题。该方法仅通过暗化处理添加阴影，不会改变身份、肤色或纹理。


<details>
  <summary>Details</summary>
Motivation: 人脸交换和合成 pipeline 产生的人脸虽然几何对齐良好，但光照上不合理：捐赠人脸呈现平坦的正面影棚光照，而宿主身体和背景却是定向场景光照。现有方法（颜色迁移、神经重光照、逆渲染）存在改变身份、肤色和纹理的风险。

Method: 从光栅化3D人脸代理估计每像素增益场 g∈[g_min,1]，将其统一乘到线性RGB通道，仅能暗化而不能偏移色度。从密集地标网格光栅化的深度缓冲推导表面法线、腔体项和屏幕空间投射阴影。关键光方向从宿主侧线索（身体、背景、光晕）估计，人脸侧线索因会恢复捐赠者光照而被下权重。阴影幅度由三参数(τ,σ,g_min)转移设定。着色场除以皮肤75百分位数，经门控、缩放、钳制、平滑处理后在羽化皮肤门控人脸掩膜内重新裁剪。

Result: 在解析人脸高度场上，默认参数(τ,σ,g_min)=(0.90,0.45,0.82)修改了56.5%的人脸像素，平均增益0.938（修改像素上为0.890），3.4%的像素达到下限。色相不变性是该算子的推论。

Conclusion: 该方法具有保守性，仅暗化无法提亮。分析了闭合形式转移，消融了参数，并讨论了单调仅暗化 formulation 的失败模式，包括非平面捐赠者的双重阴影问题。
...
TL;DR: 本文提出一种结合领域专家知识的图像分类方法，通过微调图像特征提取器学习多种属性编码格式（数值、范围、二进制、类别），并在三个数据集（Kaggle鱼类数据集、AWA2和新的木炭数据集）上验证了分类性能的提升，同时提出了自动筛选潜在错误分类样本供专家修正的流程。


<details>
  <summary>Details</summary>
Motivation: 现实场景中领域专家知识通常由多种属性组织而成，每种属性可采用不同编码格式，而现有概念瓶颈模型(CBM)仅考虑二进制编码，概念嵌入模型(CEM)虽有所扩展但在处理多样化属性编码方面仍有局限。

Method: 首先针对专家指定的下游目标类别属性微调图像特征提取器，这些属性采用多种编码格式；然后从这些属性学习分类头进行目标物体分类；最后提出自动筛选潜在错误分类样本的方法，供专家审核并修改预测属性以改进分类。

Result: 在Kaggle鱼类数据集、AWA2和更具挑战性的新木炭数据集上验证了分类性能的提升，并展示了该方法在整合专家知识和提高分类准确性方面的有效性。

Conclusion: 所提方法成功地将多种属性编码格式整合到概念驱动的图像分类框架中，不仅改善了分类性能，还通过人机协作机制提供了持续改进模型的可能。
...
TL;DR: 该研究提出了一种针对脑室周围Fazekas评分的的不确定性映射框架，用于分析医学图像分类模型的性能。该方法将模型预测的不确定性与特征表示中的位置相关联，识别类别边界过渡区域和潜在的标签不一致问题。


<details>
  <summary>Details</summary>
Motivation: Fazekas评分是评估白质高信号严重程度的序数视觉评级量表，受观察者间变异影响。当前模型性能指标难以解释模型在标签存在不确定性时的实际能力。

Method: 提出不确定性映射框架，将预测不确定性与学习到的特征表示中的位置相关联，分析模型在不同类别边界过渡区域的行为，并识别低不确定性误分类案例。

Result: 最佳Fazekas评分预测模型的Matthews相关系数(MCC)达到0.70。不同数据分割和损失函数下性能存在差异。某些模型显示更清晰的类别分离和更局部化的不确定性。发现低不确定性误分类案例，经专家审查认定为与原始参考标签不一致。

Conclusion: 不确定性映射可支持模型解释和针对性数据集审查，特别是在参考标签存在模糊性或观察者间变异的情况下。损失函数选择影响不确定性分布和类别分离效果。
...
TL;DR: 提出了一种针对视觉语言模型(VLM)的两阶段 patch 选择性域遗忘框架，通过利用 Vision Transformer 的空间结构来定位和抑制对遗忘域预测贡献过大但对语义识别贡献不足的 patch 区域，从而实现选择性域遗忘。


<details>
  <summary>Details</summary>
Motivation: 预训练的视觉语言模型虽具有跨域识别能力，但其鲁棒性也会保留不理想的域特定行为，因为域相关信息和语义信息在学习到的表示空间中相互纠缠，使得选择性域遗忘具有挑战性。现有方法通常通过潜在空间解耦和提示或特征级干预来解决这个问题，但没有直接归因和减弱单个 patch-token 的贡献。

Method: 提出两阶段 patch 选择性框架：第一阶段估计 patch 级域敏感度；第二阶段选择性地衰减那些对遗忘域预测的贡献强于其语义效用的 patch。该方法利用了 Vision Transformer 的空间结构，而非均匀抑制整个表示。

Result: 在 Office-Home、Mini DomainNet 和 DomainNet 数据集上的实验表明，与先前方法相比，该框架实现了更好的遗忘-保留权衡，保留域识别率提升高达 3.8%。在视觉重叠和未见域设置下的额外评估进一步证明了该方法在分布偏移下具有更好的鲁棒性。

Conclusion: 通过 patch 级别的域贡献分析来实现选择性域遗忘是一种有效的方法，能够在实现域遗忘的同时保持语义识别性能，为解决 VLM 中的域特定知识保留问题提供了新思路。
...
TL;DR: AgenTeeth是一个模型无关的、工具增强的框架，通过七个专门的牙科视觉专家来支撑冻结的视觉语言模型（VLM），用于解决全景牙科X光片上的牙齿定位和空间推理问题，无需微调底层VLM即可提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）在全景牙科X光片上仍然不可靠，可能依赖学习到的解剖先验而非图像中的证据。牙齿定位和空间推理尤其成问题，微调后的牙科VLM仍可能保留相同的空间偏见。

Method: 提出AgenTeeth框架：1）问题感知的编排器选择相关工具；2）检测结果映射到FDI牙位编号或解剖区域；3）返回结构化发现和带注释的图像叠加；4）新的综合调用使用这些证据回答问题。框架使用七个专门的牙科视觉专家模型。

Result: 在MMOral-OPG-Bench上，AgenTeeth将四个骨干VLM提升了12.9-23.0个百分点。最强配置在开放域VQA上达到65.66%，优于OralGPT-Plus的45.35%。同等规模下，冻结的Qwen2.5-VL-7B-Instruct配合AgenTeeth达到48.11%，超越使用相同骨干经过监督微调和强化学习微调的OralGPT-Plus。

Conclusion: AgenTeeth是一个有效的模型无关框架，无需微调即可显著提升VLM在牙科X光片VQA任务上的性能，通过工具增强和证据综合的方式解决空间推理问题。
...
TL;DR: 本文研究单目视频中风驱动植被的4D重建问题，提出用阻尼谐振子物理先验替代直接学习的变形场，在合成数据集上测试了先验的物理基础和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 单目重建风驱动植被问题严重欠约束：沿视线方向的运动难以观测、移动冠层缺乏可靠对应点、几乎整个场景都是动态的缺乏静态参考。直接学习的4D Gaussian Splatting变形场优化的是光度一致性而非恢复产生该运动的物理参数。

Method: 用物理参数化的变形先验替代学习到的变形场：每个刚性部件一个阻尼谐振子，由观测到的风驱动，通过可微RK4积分，仅以光度一致性的方式监督。

Result: 先验在分布内视图上牺牲了外观保真度，但在训练时间窗口外和训练风速外的泛化能力显著更好。参数恢复效果远弱于初步预期：频率恢复仅在最简单的树上优于未训练对照，完全无法恢复阻尼。

Conclusion: 物理先验能带来更好的时间外推和零样本迁移能力，但物理参数本身的恢复比预期困难得多，说明先验虽然泛化好但并未真正学习到底层物理机制。
...
TL;DR: GazeDiT是一种用于眼动追踪的扩散模型，通过内部构建的空间条件将全局 gaze 标签与局部瞳孔/虹膜几何特征结合，实现精确的4D gaze标签控制合成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成合成训练数据时难以实现精确标签控制，特别是在眼动追踪领域，4D双目 gaze 通过细微的局部瞳孔和虹膜几何表达，而传统条件信号低维粗糙，无法实现图像与数值标签的精确对应。

Method: 1. 使用冻结的SegFormer从真实图像中提取瞳孔/虹膜几何特征；2. 训练过程中让模型学习基于几何特征的真实外观；3. 推理时通过物理眼睛渲染器采样变化的解剖结构和相机状态，产生多样化的合成数据。

Result: GazeDiT在尾部gaze标签误差上显著低于其他扩散基线，接近同一冻结gaze估计器在真实图像上的误差。生成的训练数据提升了下游眼动追踪器性能，最小人群的困难案例 gaze 误差从3.05°降至2.80°。

Conclusion: 通过将低维全局gaze标签与局部几何特征空间条件结合，GazeDiT成功实现了精确标签控制的眼动数据合成，为下游眼动追踪器的训练提供了有效的数据增强方法。
...
TL;DR: CALIPER是一个无需模型的RGB-D框架，用于细粒度识别视觉相似的工业零件。该方法将基于支持集的外观匹配与度量尺寸证据相结合，通过单次转台RGB-D视频和少量标注图像进行训练，实现新类别的快速 enrollment 而无需更新网络参数。


<details>
  <summary>Details</summary>
Motivation: 工业零件的细粒度识别具有挑战性，因为类别间主要在物理尺寸上存在差异。传统方法将检测到的目标归一化到固定输入尺寸会抑制绝对尺度信息，而CAD模型和大型类别特定数据集在不断发展的工业库存中可能不可用。

Method: 1. 使用单个转台RGB-D视频和1-2张标注的真实图像进行训练类的 onboarding<br>2. 3D重建提供新颖视角的外观支持，对齐深度产生类别特定的度量尺寸profile<br>3. 推理时使用YOLOv8n-seg进行粗定位，DINOv2 backbone + episodically trained embedding head进行细粒度支持匹配<br>4. Margin-conditioned metric fusion 仅对外观模糊的决策激活概率尺寸证据<br>5. 新类别从小型RGB-D支持集 enroll，无需更新网络参数

Result: 在18个视觉相似的工业零件上评估：16类用于训练，2个螺丝用于训练-free enrollment。CALIPER达到88.2%闭集准确率和99.8%定位召回率；10-shot enrollment后整体准确率达85.7%。度量融合使未见类准确率提升高达37.4个百分点，且未对原始库存造成统计显著降级。机械臂部署无需特定retraining即可识别17/18个零件。

Conclusion: CALIPER成功实现了一种无需模型的RGB-D细粒度工业零件识别框架，通过结合外观匹配与度量尺寸信息，在少样本新类别enrollment和闭集识别上均表现出色，且可泛化至真实机械臂部署。
...
TL;DR:  RoboVAD是一个用于机器人臂操作视频异常检测的大规模基准数据集，旨在评估VAD方法在跨域场景中的性能，其中某些动作和异常类型在训练时不可见。


<details>
  <summary>Details</summary>
Motivation:  机器人臂交互中的视频异常检测具有重要下游应用，包括学习更好的交互和操作能力、触发异常恢复程序等。然而，由于可用资源有限，该领域的研究探索受到限制。

Method:  构建了RoboVAD大规模基准数据集，包含具有挑战性的跨域评估场景；训练并评估了多种最先进的VAD方法，包括一种专门为机器人臂操作适配的新方法。

Result:  提出的方法优于许多最先进的竞争对手，但在最具挑战性的评估设置下，所有方法的微平均帧级AUC均低于70%，表明该基准的难度很高。

Conclusion:  RoboVAD是一个具有挑战性的基准数据集，证明了在机器人臂操作中检测新异常类型的困难性，数据集和代码已公开可用。
...
TL;DR: Lumen是一种病理视觉语言模型，通过参数高效的对齐方法将冻结的Virchow2和BioMedBERT基础模型进行对齐，仅训练0.40%的参数即可在零样本补丁基准测试中取得最高平均准确率，并在淋巴结转移检测中达到高AUROC性能。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索是否可以通过参数高效的对齐方式（而非预训练或微调）来构建病理视觉语言模型，保留预训练表示的完整性。

Method: 使用秩-4适配器和投影头对齐冻结的Virchow2和BioMedBERT骨干模型，仅在公开的QUILT-1M语料库上训练0.40%的总参数。

Result: 在九个公共零样本补丁基准测试中，Lumen实现了最高平均校正后平衡准确率0.546（比最强基线0.461高0.086）。淋巴结转移检测内部AUROC为0.964，外部为0.955。内部平衡准确率0.909，外部为0.915。

Conclusion: 冻结单模态基础模型的对齐能够产生强大且可转移的性能，仅需训练很小比例的参数。完全微调两个编码器并未带来一致的收益，但改善了检索性能。
...
TL;DR: 研究首次将采样-based conformal prediction (CP) 应用于基于视觉语言模型(VLM)的无障碍设施评估,评估四种VLM对首尔514张人行道图像的有效宽度、纵坡、横坡和路面状况进行预测。结果显示:CP可达到90%覆盖率,但有效宽度是最具信息量的属性(半区间宽约1.0m);所有模型高估宽度;原始采样分散度仅覆盖17-47%的实测值;响应自一致性不能作为准确性的证据。


<details>
  <summary>Details</summary>
Motivation: 城市无障碍设施对轮椅用户和行动受限人士至关重要,而人行道合规性评估传统上依赖实地测量,成本高效率低。该研究旨在探索利用视觉语言模型(VLM)从行人水平图像自动评估人行道无障碍属性的可行性,并引入conformal prediction方法来解决VLM输出的不确定性量化问题。

Method: 研究使用514张首尔人行道图像及对应实地测量值,评估四种VLM对四个属性的预测能力:有效宽度、纵坡、横坡和路面状况(分A-E五级)。首次应用采样-based conformal prediction (CP)进行不确定性量化,并比较对称和非对称校准方法。分析原始采样分散度与校准后区间对实测值的覆盖率。

Result: 1) CP对所有模型和属性达到标称90%覆盖率;2) 有效宽度信息量最大,最佳模型半区间宽约1.0m;3) 所有模型高估宽度,非对称校准可缩短33%区间;4) 纵坡信息量有限,横坡区间过宽无法解析监管阈值;5) 路面状况在3/4模型中退化为全部五等级;6) 原始采样分散度仅覆盖17-47%实测值;7) 高自一致性响应仍可能偏离实测值达96%。

Conclusion: 目前没有定量属性达到一般合规评估所需的精度,但CP可从校准数据识别哪些属性可用于阈值远离合规线的路段筛查。研究释放了标注图像和实地测量数据,为VLM无障碍评估和不确定性量化提供了基准。
...
TL;DR: Zing-0.5是一个50亿参数的自律世界模型，支持用户通过键盘和在线文本控制探索生成的世界、影响事件发生。实现了832×480分辨率下24FPS的实时推理，推理成本约每流分钟0.009美元。在158个WBench Navigation测试用例上获得81.0总分和88.5一致性分数。


<details>
  <summary>Details</summary>
Motivation: 解决现有世界模型在可玩性方面的局限性，使用户能够主动探索生成的世界、影响事件发展，并通过键盘和文本指令进行实时交互控制。

Method: 提出三个技术贡献：(1)统一动作和文本条件处理，将幅度感知的键盘输入与时间对齐的文本指令和联合标注视频结合；(2)事件级监督增量生成，使用段落级教师模型通过分布匹配蒸馏监督块级因果学生模型；(3)低成本实时交互，结合四步生成和上下文保留流式处理。

Result: 在158个WBench Navigation测试用例上达到81.0总体分数和88.5一致性分数；支持832×480分辨率24FPS实时推理；服务器租赁成本约每流分钟0.009美元；演示了导航过程中文本引导的事件变化无需重新生成。

Conclusion: Zing-0.5实现了可玩生成世界的重要进展，平衡了生成质量与实时交互性能。团队开源了模型权重、推理代码和Zing-SGLang服务实现。
...
TL;DR: FLAG 2027挑战赛旨在评估语音-人脸关联模型在跨语言和跨性别场景下的性能，发现现有模型在这些条件下性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有的语音-人脸关联模型可能依赖于语言或性别线索而非说话人特定的语音特征，导致在多语言说话人或同性别说话人识别场景下性能下降。

Method: 提出跨模态验证任务：给定一段语音，从包含目标说话人人脸和负样本的人脸库中识别说话人的身份。设置两种评估场景：标准无约束场景和性别约束场景（使用同性别人脸库）。

Result: 现有基线模型在语言转换和性别约束设置下性能显著下降，表明模型过度依赖语言和性别线索而非身份特定特征。

Conclusion: 需要开发能够捕获超越语言和性别限制的身份特定特征的模型。挑战赛提供基准数据集、预训练基线模型和评估框架以推动语音-人脸关联研究。
...
TL;DR: 本文研究了音频在体育精彩片段检测中的作用，通过构建仅音频、仅视觉和音视频融合的基线模型，在SV-Highlights基准数据集上进行评估。仅音频的GRU基线表现优于部分现有的音视频方法，而简单的音视频融合基线在所有指标上取得最佳性能。研究还发现人声/解说音频比背景音更具信息量，精彩片段具有更高的RMS响度、峰值响度和中频能量。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常强调视觉或视觉-语言表示，而体育视频包含丰富的音频线索（解说、观众反应、哨声、球击声等）。作者重新审视音频在体育精彩片段检测中的作用，提出一个问题：仅凭音频能走多远？

Method: 使用预训练音频表示构建轻量级仅音频基线，与仅视觉和音视频方法在SV-Highlights基准上进行比较。采用GRU作为时序建模 backbone，进行源分离分析以区分人声/解说音频和背景音，并分析可解释的音频特征（RMS响度、峰值响度、中频能量）。

Result: 仅音频的GRU基线表现强劲，优于若干现有音视频方法；简单音视频融合基线在所有指标上取得最佳性能。源分离分析表明人声/解说音频比背景音更具信息量，两者结合效果最佳。精彩片段的RMS响度、峰值响度和中频能量均高于非精彩片段，但存在显著分布重叠，说明仅靠响度不足以准确检测。

Conclusion: 音频是体育精彩片段检测中未被充分探索但高度信息化的模态，应作为主要信号而非辅助线索对待。音频和视觉线索提供互补信息，音视频融合效果最佳。
...
TL;DR: 研究首次在区域尺度上绘制了 Sierra Nevada 植物区省的大型树木分布图(650万棵)，并利用深度学习和时间序列遥感监测其健康动态，发现火灾是导致10%大型树木死亡的主要因素。


<details>
  <summary>Details</summary>
Motivation: 大型树木对生物量储存、栖息地结构和生态系统功能具有超比例贡献，但其在区域尺度上的分布和健康动态仍缺乏量化研究。

Method: 1) 使用U-Net-ID深度学习模型结合亚米级航拍图像生成的冠层高度模型识别大型树冠(≥100m²)；2) 基于Sentinel-2 2020-2025年时间序列提取光谱轨迹，结合BFAST断点检测和干扰分类框架表征树木健康动态；3) 利用CAL FIRE火灾边界数据识别火灾影响。

Result: 模型验证IoU中位数为0.602；共识别6,515,705棵大型树木，分布在约78.7%的研究区；空间分布与海拔、温度和降水相关；野火是主要死亡驱动因素，杀死Sierra Nevada约10%的大型树木，死亡率集中在2020-2021年极端火灾季节。

Conclusion: 该研究首次实现了区域尺度的大型树木精细制图和健康动态监测，揭示了极端火灾对大型树木种群的严重威胁，为森林管理和生态保护提供了重要的基线数据。
...
TL;DR: 提出EDCT方法，通过对视觉语言模型解释中引用的视觉概念进行最小编辑和一致性测试，解决模型解释与视觉证据不一致的问题，并构建EDCT-Bench基准数据集。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型(VLM)能够生成看似合理但与所引用的视觉证据不一致的自然语言解释(NLE)，这种不一致性严重影响模型的可靠性和可信度。

Method: 提出干预性协议EDCT：提取解释中引用的视觉概念，对这些概念应用已验证的最小编辑，然后测试模型的答案和解释与编辑后的图像是否保持一致。

Result: 在三个互补领域(OK-VQA、DriveLM、3DSRBench)构建EDCT-Bench基准，评估显示现有VLM存在显著的忠实度差距，模型频繁产生与已验证视觉变化不一致的响应。微调研究表明EDCT生成的反事实样本可提供高价值训练信号。

Conclusion: EDCT方法能有效检测VLM解释的忠实度问题，EDCT-Bench为评估模型一致性提供了全面基准，且基于该方法生成的训练数据可显著提升模型性能。
...
TL;DR: 研究评估了四代Stable Diffusion模型（SD 1.5、SD 2.1、SDXL、SD 3 Medium）在20个职业中的性别表征偏见，发现76.4%的生成图像显示男性，且57.6%历史上女性主导职业的图像也显示男性。与美国劳工统计局数据相比，模型对女性的代表性不足20-46个百分点。模型代际间偏见并未持续改善：从SD 1.5到SDXL偏见恶化，SD 3 Medium才部分恢复。没有模型实现性别平等。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型在专业和创意领域广泛应用，但人们对其如何表征不同职业的性别差异以及新一代模型是否更公平知之甚少。该研究旨在系统评估多代Stable Diffusion模型在职业性别表征方面的偏见情况。

Method: 研究评估了20个职业、5种提示模板和4代Stable Diffusion模型（SD 1.5、SD 2.1、SDXL、SD 3 Medium），生成了8,000张图像（每个职业-模型组合5个提示×20张图像），并使用DeepFace对所有图像进行性别分类。将模型输出与美国劳工统计局（BLS）劳动力数据进行对比，还初步比较了GPT-image-1模型。

Result: 在8,000张开源图像中，76.4%显示男性主体（95%置信区间[75.1%, 78.7%]，p < 2.2×10^-16）。更令人震惊的是，57.6%历史上女性编码职业的图像显示男性主体。与BLS数据相比，模型对女性的代表性平均不足20-46个百分点，特别是接近性别平衡的职业：科学家（BLS女性48%，模型输出男性82-99%）和清洁工（BLS女性46%，模型输出男性80-92%）。模型代际改进不稳定：偏见从SD 1.5到SDXL恶化，SD 3 Medium部分恢复。GPT-image-1偏见较低但实际效应较小（Cramer's V = 0.080）。

Conclusion: 所有四代Stable Diffusion模型都存在严重的性别偏见问题，新一代模型并未实现持续改善。模型在职业性别表征上与现实劳动力构成存在巨大偏差，需要显著改进以实现性别平等。该研究揭示了生成式AI模型中系统性性别偏见的存在，呼吁业界关注并解决这一问题。
...
TL;DR: IRIS是一种自监督框架，用于从不带姿态标注的多视角图像中进行新视角合成。它在隐式潜在空间渲染和显式3D表示之间提供了实用的中间方案，将场景表示为潜在神经场，通过自预测相机查询该场来渲染新视角。


<details>
  <summary>Details</summary>
Motivation: 现有方法分为两个极端：隐式潜在空间渲染灵活且易于优化，但相机估计缺乏几何基础；显式3D表示提供更强的几何支撑，但参数化更复杂且优化更脆弱。需要一种兼顾两者优点的自监督方法。

Method: IRIS将场景表示为潜在神经场，通过以下步骤渲染新视角：1）在参考视图中投影特征并聚合到采样3D点，形成逐点潜在特征；2）沿目标射线组合这些特征进行渲染。这种设计保留了隐式建模的灵活性和优化稳定性，同时引入了更强的几何结构。

Result: 大量实验表明，IRIS在完全自监督学习下实现了强大的新视角合成质量和具有竞争力的姿态精度。

Conclusion: IRIS成功地在隐式和显式表示之间提供了实用的中间方案，为自监督多视角重建和相机估计领域提供了新的思路。
...
TL;DR: SetPlanner是一个轻量级(1.52M参数)的插件式点集规划器，用于为冻结的SAM模型提供自动提示，解决内镜器械分割中薄型、反光、关节化和部分遮挡工具的自动分割难题。


<details>
  <summary>Details</summary>
Motivation: SAM模型虽提供可复用先验，但需要用户提示无法支持全自动化分割；thin、articulated、reflective和partly occluded工具的自动提示困难，多种配置均可能有效。

Method: 将自动提示表述为轻量级点集规划，在冻结路径下隔离点源。采用置换感知条件流(permutation-aware conditional flow)从几何感知目标规划完整的无序K点集，SAM解码8个候选并通过共识读出获得无真值预测。

Result: 在三个内镜数据集上，SetPlanner在全部6条迁移路线上击败LoRA适配系统；在Kvasir-Instrument上达0.934 Dice；恢复96%的44.4点定位差距；候选不一致性可低Dice病例AUROC达0.969。

Conclusion: SetPlanner作为冻结SAM的插件，保持点提示接口兼容，支持跨骨干网复用，在内镜器械分割任务上显著优于LoRA微调方法，且具备候选置信度评估能力。
...
TL;DR: 本文提出CoAtNet-DeepMoE模型，用于番茄病害识别。该模型结合卷积-注意力混合架构与DeepSeek专家混合机制，在仅2.47M参数的情况下，在Kaggle和PlantVillage数据集上实现了约99.8%的准确率，达到了性能和模型大小的最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 全球人口增长导致粮食需求增加，番茄作为重要作物因病害造成大量损失。现有深度学习模型虽能有效识别病害，但参数量大、训练推理时间长，难以应用于实际场景。因此需要设计轻量化且保持高精度的模型。

Method: 提出CoAtNet-DeepMoE模型，采用卷积-注意力混合架构提取丰富特征，并引入DeepSeek专家混合(MoE)机制显著减少参数量。在Kaggle平衡/不平衡数据集和PlantVillage数据集上进行了评估。

Result: 在Kaggle数据集上达到99.80%准确率、精确率、召回率和F1分数；在PlantVillage数据集上达到99.83%准确率、99.85%精确率、99.76%召回率和99.80%F1分数。模型仅含2.47M参数。

Conclusion: CoAtNet-DeepMoE在保持最先进性能的同时大幅降低了模型参数量，证明了轻量化模型在植物病害检测中的可行性，为用户友好的农业应用提供了有效解决方案。
...
TL;DR: 本文提出了一种名为位置锚点调整（PAT）的新型参数高效微调方法，用于适应预训练的点云变换器。PAT通过 token 聚合-扩展对来减少 MHA 和 FFN 的计算成本，同时引入 BSLoRA 来仅用少量可训练参数学习任务特定表示。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法虽然实现了良好的微调性能和较高的参数效率，但忽略了推理效率问题。

Method: PAT 采用 token 聚合-扩展对，包括 token 聚合模块（TAM）和 token 扩展模块（TEM）。TAM 基于 3D 空间中的位置锚点从输入 token 中提取代表性 token，减少参与计算的 token 数量；TEM 将学习到的表示传播回原始输入 token。此外，还引入了基础共享低秩适配（BSLoRA）使 TAM 能够有效学习任务特定表示。

Result: 在广泛使用的基准数据集上进行的广泛实验表明，PAT 与最先进的方法表现相当，同时具有显著更低的计算开销和更少的可训练参数。

Conclusion: PAT 成功地在参数效率和推理效率之间取得了平衡，为点云变换器的下游任务适应提供了一种有效的方法。
...
TL;DR: Finder是一种用于具身智能体目标定位的主动闭环目标发现原语，不将定位视为从固定场景表示的被动检索，而是维护一个类型化循环状态，链接查询条件规划、作用域证据收集、候选验证和接受/继续/中止控制。在证据不完整或模糊时，循环可以重定向后续感知和比较。在Habitat/HM3D和真实RGB-D场景中的开放词汇具身目标检索任务上，Finder比强基线提高了15.75个百分点的1米成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么将目标搜索与在线探索耦合（在已捕获相关观测时代价高昂），要么以静态单次方式查询预建的开词汇地图和场景图。将具身接地视为被动检索，无法处理证据不完整或模糊的情况。

Method: Finder维护一个类型化循环状态，链接查询条件规划、作用域证据收集、候选验证和接受/继续/中止控制。当证据不完整或模糊时，循环可以重定向后续感知和比较，而不是简单返回检索到的最高排名目标。

Result: 在开放词汇具身目标检索任务上（Habitat/HM3D和真实RGB-D场景），Finder比强基线提高了平均1米成功率15.75个百分点。相同原语也可迁移到顺序目标接地和具身目标中心问答，无需更改内部接地协议即可改善空间和时间定位。

Conclusion: Finder作为一种主动闭环目标发现原语，能够在证据不完整时动态重定向感知和比较，显著优于静态被动检索方法，且具有良好的任务迁移能力。
...
TL;DR: GeoCueFormer是一种轻量级水下语义分割框架，通过几何约束的频域增强和预测引导的细粒度 refinement 来解决水下图像视觉退化导致的分割难题。该方法在SUIM和DUT数据集上分别达到82.23%和73.04%的mIoU，在相当模型复杂度下实现了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 水下语义分割对海洋生态监测至关重要，但面临严重视觉退化问题：光吸收和散射导致颜色偏移、低对比度和边界模糊，使得浅层细节特征不可靠。现有方法虽改进RGB特征聚合或边界预测，但缺乏明确机制区分结构相关细节与退化诱导响应。

Method: 提出GeoCueFormer框架，包含两个核心组件：1) 阶段特定的小波增强对分层编码器特征进行操作，以补充浅层边界细节同时保留深层结构语义；2) 深度派生的空间门控约束浅层频域增强朝向几何一致区域；3) 预测引导的双阶段解码器进一步细粒度化模糊的高分辨率特征。

Result: 在SUIM数据集上达到82.23% mIoU，在DUT数据集上达到73.04% mIoU。在相当模型复杂度和标准基准设置下，实现SOTA性能，同时保持良好的精度-复杂度权衡。

Conclusion: 区分结构细节与退化诱导干扰比单纯改进RGB特征聚合或边界预测对水下分割更有效。几何约束的频域增强和预测引导的细粒度 refinement 机制能够有效分离可靠结构信息与退化噪声。
...
TL;DR: vidax是一个开源的JAX/Flax推理引擎，专为TPU设计，实现了PyTorch到JAX权重的零拷贝转换，解决了云TPU pods缺乏视频生成模型生产级推理路径的问题。


<details>
  <summary>Details</summary>
Motivation: 开源视频生成模型几乎仅提供PyTorch/CUDA参考实现，导致Cloud TPU pods缺乏生产级推理路径，尽管TPU提供的大型低成本加速器内存池非常适合长序列时空注意力计算。

Method: 开发了vidax框架，支持多种时空模型（扩散Transformer、Omnimodal MoT、3D VAE、文本编码器、原生采样器），统一1D张量并行与DeepSpeed-Ulysses序列并行于单一JAX分片网格，集成TPU flash-attention内核，实现逐层权重卸载。

Result: 在TPU v4-8硬件上进行了编译时间、延迟和峰值内存利用率基准测试，记录了检查点转换过程中发现的实际数值bug。

Conclusion: vidax以开源方式发布，作为JAX和TPU视频生成研究的基线。
...
TL;DR: 本文提出ID-MAE框架，结合基于相似度的强化学习掩码策略(SRLM)，解决图像到点云配准中跨模态对应估计难题。通过自适应掩码信息丰富区域来缩小模态差距，并在RGB-D Scenes v2和7-Scenes数据集上达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前无检测的图像到点云配准方法容易因域和模态差异、特征提取器灵敏度有限以及非重叠区域存在而产生错误对应。标准MAE的随机掩码可能遗漏关键区域，降低配准效果。

Method: 提出Intermodal Dual-MAE框架(ID-MAE)，采用基于相似度的强化学习掩码策略(SRLM)，利用跨模态相似度和强化学习自适应掩码信息位置，缩小模态差距。通过在特征提取过程中强制执行表示一致性来增强跨模态表示学习。

Result: 在RGB-D Scenes v2和7-Scenes基准数据集上实现了图像到点云配准的最先进性能。

Conclusion: 所提方法通过SRLM策略自适应掩码信息区域，有效缩小模态差异，增强跨模态表示学习，实现更可靠的2D-3D对应估计，从而在图像到点云配准任务中达到领先水平。
...
TL;DR: 本文提出Aligned Consensus Teacher (ACT)框架，用于可见光-红外目标检测的半监督学习。通过Cycle-Consistent Region Alignment (CRA)、Cross-Modal Consensus Mean-Teacher (CMC-MT)和Text-Guided Cross-Modal Instance Augmentation (TG-CMIA)三个创新模块，解决有限标注下的跨模态对齐、伪标签错误累积和尾部类别稀缺问题。在DroneVehicle数据集上，仅用10%标注数据即可达到全监督性能的94.3%。


<details>
  <summary>Details</summary>
Motivation: 可见光-红外目标检测(VIOD)需要双模态标注，成本高昂。传统半监督学习方法从单模态扩展到VIOD面临挑战。在实际图像对级别设置中，只有少量配对有双模态标注，其余完全无标注。这种有限监督带来三个核心挑战：(i)标注框太少无法进行鲁棒的跨模态对齐；(ii)分支遗漏导致的伪标签错误在自训练中累积；(iii)随着标注预算减少，尾部类别标注变得极其稀缺。

Method: 提出Aligned Consensus Teacher (ACT)框架，包含三个核心组件：1) Cycle-Consistent Region Alignment (CRA)：结合循环一致性和稀疏锚点，使用可靠性加权的区域匹配实现跨模态对齐；2) Cross-Modal Consensus Mean-Teacher (CMC-MT)：在配对保持视图下形成共识伪标签，恢复分支遗漏并监督无标注配对；3) Text-Guided Cross-Modal Instance Augmentation (TG-CMIA)：利用视觉语言场景先验，在保留RGB-IR偏移的情况下合成尾部类别实例对。

Result: 在DroneVehicle和VEDAI数据集上进行了大量实验，结果显示ACT在不同标注比例下均获得一致的性能提升。特别地，在DroneVehicle数据集上使用仅10%的标注配对，ACT达到了全监督mAP的94.3%，显著降低了标注成本。

Conclusion: ACT是首个在图像对级别设置下研究半监督VIOD的框架，成功解决了有限标注情况下的跨模态对齐、伪标签质量和尾部类别稀缺问题。该方法在大幅降低标注成本的同时保持了较高的检测性能，具有重要的实际应用价值。代码和模型将开源以促进后续研究。
...
TL;DR: PRISM是一个用于人群机器人导航的框架，通过被动观察人类互动来推断交互特质。该方法使用Transformer编码器将人类轨迹编码到连续的序数潜在空间中，并结合时间稳定性评分来指导导航策略。


<details>
  <summary>Details</summary>
Motivation: 当前人群机器人导航主要基于几何状态表示，忽视了个体在交互倾向上的差异。作者希望从被动观察人类互动中推断出交互特质，使机器人能够像人类一样在交互前观察并调整行为。

Method: 1) 使用Transformer编码器将人类轨迹编码到连续的序数潜在空间；2) 通过Rank-N-Contrast损失进行训练；3) 将推断的每个特质与时间稳定性评分配对提供给导航策略；4) 在随机人群模拟中进行测试。

Result: 相比纯几何基线方法，PRISM降低了碰撞率，并在导航时间和路径长度指标上取得小幅改善。

Conclusion: 被动潜在特质推断对于动态人群中的社交导航具有实用价值，为机器人导航研究提供了新思路。
...
TL;DR: 提出MCLC-NET多模态持续学习框架用于叶片计数，解决现有方法依赖单一RGB模态和需要同时获取所有训练数据的问题，并构建MMLC多模态叶片计数数据集。


<details>
  <summary>Details</summary>
Motivation: 现有叶片计数方法依赖RGB图像，易受遮挡和光照变化影响；多模态叶片计数研究不足；且很多方法假设所有训练数据同时可用，在实际农业场景中不现实。

Method: 提出MCLC-NET框架，使用基于记忆的策略和记忆缓冲区按顺序学习任务并保留之前任务的重要样本；构建MMLC数据集，包含RGB、深度和热图像，涵盖不同作物类型和环境条件，设有三种任务顺序：作物顺序、时间顺序和混合顺序。

Result: MCLC-NET在三种任务顺序上均优于现有方法，取得最低AMSE：作物顺序0.675±0.027、时间顺序0.542±0.069、混合顺序0.745±0.057。

Conclusion: 多模态持续学习方法能有效解决叶片计数中的模态单一和数据顺序获取问题，在不同任务顺序下均能保持较好的性能。
...
TL;DR: CamoShift是一种针对可见光-红外目标检测的对抗性攻击框架，通过结合视觉伪装和目标级红外偏移来打破跨模态空间对齐并干扰融合过程。该方法首次同时针对视觉隐蔽性和攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 当前物理攻击方法在目标区域应用明显的补丁，容易被人眼察觉；且现有方法仅扰动对齐区域内的外观，未明确针对模态间对应关系或融合过程。需要一种同时具备视觉隐蔽性和攻击有效性的对抗方法。

Method: 提出CamoShift框架，包含三个核心模块：(1)语义伪装模块(SCM)：生成可附着在目标上的隐蔽伪装补丁，通过RGB-IR适配器保持红外分支有效性；(2)目标级空间解耦模块(OSDM)：以尺度感知方式偏移红外目标证据，破坏目标级对应关系并干扰跨模态融合；(3)谐波对抗损失(HarAdv loss)：在优化过程中平衡攻击强度与视觉隐蔽性。

Result: 实验结果表明CamoShift在攻击有效性和视觉隐蔽性之间实现了出色的平衡，优于现有方法。

Conclusion: 首次在可见光-红外目标检测中同时针对视觉隐蔽性和攻击成功率两个目标，为跨模态检测系统的对抗性研究提供了新思路。
...
TL;DR: MVLGeo是一个统一多视角的跨视图对象地理定位框架，通过视觉-语言重排序、多视角专家混合架构和自适应椭圆先验位置编码来解决现有方法中参数冗余和视觉相似候选歧义问题，在CVOGL基准测试中达到最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 1. 现有方法为每个视角训练单独的检测器，导致参数冗余，阻碍跨视角知识共享；2. 排名靠前的卫星候选图像往往视觉上相似，仅凭外观和类别标签难以消除这种歧义。

Method: 提出MVLGeo框架，包含三个核心组件：1) 环境上下文文本作为线索，通过视觉-语言重排序(VL-Rerank)区分视觉相似的候选；2) 多视角MoE架构(MV-MoE)，共享编码器+视角特定专家，减少冗余并促进知识共享，跨视角对比学习对齐表示；3) 自适应椭圆先验(ESAM-Prior)作为辅助位置编码，实现各向异性几何感知。

Result: 在CVOGL基准测试中，MVLGeo作为多查询视角的统一模型，实现了最先进的性能，展示了对输入退化的鲁棒性和跨视角的泛化能力。

Conclusion: MVLGeo通过统一多视角建模和引入视觉-语言线索，有效解决了跨视图地理定位中的参数冗余和候选歧义问题，为CVOGL任务提供了高效且鲁棒的解决方案。
...
TL;DR: 本文提出CapMap-MS-TTA，一个基于Florence-2-base的训练-free多模态系统，用于同时解决图像标记、开放词汇目标检测和图像描述三项任务，在不超过0.5B参数和8GB GPU显存的约束下，在LSVOS挑战赛MUMU赛道获得第3名。


<details>
  <summary>Details</summary>
Motivation: LSVOS挑战赛MUMU赛道要求在严格资源约束下（≤0.5B参数、≤8GB GPU显存）同时完成图像标记、开放词汇目标检测和图像描述三个任务，需要一个统一的多模态模型来联合解决这些任务。

Method: 基于Microsoft Florence-2-base（约231M参数）构建，结合caption关键词映射和多尺度翻转测试时增强（TTA）。任务C使用原生<DETAILED_CAPTION>路径进行长度/标记清洗；任务A将详细描述映射到官方quality/scene/event词汇表，通过扩展关键词词典和整词匹配；任务B运行Florence-2开放检测（<OD>），结合多尺度和水平翻转TTA，随后进行标签感知的非极大值抑制（NMS）。

Result: 未进行微调的情况下，系统将复现的Florence-2基线从15.16提升至最佳公开分数16.4815，在最终MUMU排行榜上排名第3。

Conclusion: 通过结合caption关键词映射和多尺度翻转TTA策略，基于Florence-2的训练-free方法能够在严格资源约束下有效联合解决多模态理解任务，为资源受限场景下的多任务学习提供了可行方案。
...
TL;DR: 本文提出了超大规模无人机航拍数据集HARD（12768x9564分辨率），用于广域时空场景理解（WSTU）任务，涵盖目标检测、多目标跟踪和场景级视觉问答三个层次，并提出考虑延迟的流式HOTA评估指标。


<details>
  <summary>Details</summary>
Motivation: 随着无人机航拍从百万像素发展到十亿像素级传感器，航空感知从识别单个目标转向理解整个动态场景。现有的广域时空场景理解（WSTU）需要同时具备广域覆盖、目标级分辨率和时间连续性，但现有数据集无法满足这一需求。

Method: 构建了超高清航拍数据集HARD（12768x9564），在三个层次进行标注：目标检测、多目标跟踪和场景级视觉问答；提出考虑延迟的流式HOTA（s-HOTA）评估指标；开展广泛的基线实验。

Result: 超高清图像处理使每帧处理时间达秒级；检测任务中端到端pipeline与检测器本身对精度和速度的影响相当；跟踪任务中高延迟使关联轴比检测轴影响更不均匀；离线最优pipeline在s-HOTA下可能失去优势；视觉语言模型在跨帧身份绑定方面仍较弱。

Conclusion: 当前基线方法无法满足WSTU需求，HARD数据集和系统化基线为推进该领域研究提供了数据基础和基准。
...
TL;DR: WISE是一个面向卫星上部署的野火烟雾检测框架，通过师生蒸馏策略利用仅 tile 级别的标注，实现低参数、低计算量的烟雾检测和空间定位，在轨验证达到近乎实时的推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，tile级分类计算高效但缺乏空间定位，pixel级分割能提供详细掩膜但计算量过大，无法满足星上实时部署的内存和延迟约束。需要一种在严格计算约束下兼具空间信息输出的高效检测方法。

Method: 提出WISE框架，采用师生蒸馏策略：离线教师提供软空间监督，轻量级学生模型进行高效推理。学生模型在单次前向传播中联合预测tile级烟雾存在性和像素级烟雾概率图。模型包含三个变体，参数仅0.12M，约3 GFLOPs。

Result: 在ISS搭载的IMAGIN-e载荷上进行在轨执行验证，三种模型变体平均推理时间分别为0.10s、0.14s、0.26s每tile。地面实验（Landsat 5和Landsat 8）最佳变体达到tile级F1分数0.964，像素级F1分数0.750（10次平均）。

Conclusion: WISE是低延迟星上野火烟雾监测的实用方案，在严格资源约束下实现了高效推理与空间信息输出的兼顾，具有良好的实际应用前景。
...
TL;DR: 本文提出了一种基于响应几何的结构化剪枝新方法，将剪枝定义为选择具有大联合响应容量的子集，然后进行单独的功能实现。通过统一响应几何映射M(D,R)=D^{{1/2}}RD^{{1/2}}，使用行列式和Schur贪婪残差来选择非冗余坐标。该方法无需微调即可将选定子集转换为可执行网络。


<details>
  <summary>Details</summary>
Motivation: 当前结构化剪枝方法将通道排名单独处理，但通道响应可能是互补的或通过下游混合相互抵消。作者认为应该考虑通道的联合响应能力而非单独排名。

Method: 1) 提出统一响应几何映射 M(D,R)=D^{{1/2}}RD^{{1/2}}；2) 使用行列式和Schur-greedy残差选择非冗余坐标；3) 两种实例：无标签版本基于激活协方差，任务条件版本结合激活和梯度方差(响应尺度)与梯度相关性(互补性)；4) 通过岭补偿将可预测的移除响应折叠到后继权重中，重新校准BatchNorm统计量，无需微调。

Result: 在ImageNet ResNet-50上，无标签版本在30%和40%删除率下达到65.4%和53.9%的Top-1准确率(对比强度选择仅为59.8%和43.1%)；任务条件版本达到67.7%和56.3%。六族测试显示架构依赖行为，卷积和扩展层设置中有正面相对对比，窗口注意力中有明确边界情况。

Conclusion: 响应几何可作为结构化剪枝的条件原则，其收益由观察到的响应和实现该响应的架构共同决定。该方法在保持性能的同时实现了有效的结构化剪枝，且无需微调。
...
TL;DR: 本文提出 Failure-Aware Progressive Repair (FAPR) 方法，用于医学图像分割的可靠性提升。该方法将分割掩码表示为动态失败状态，将修复操作建模为状态转换算子，通过迭代诊断和修复错误来逐步提升分割质量，同时保持基础分割器的性能不变。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中，高平均准确率可能掩盖一小部分结构独特且具有临床后果的错误。现有后验校正方法从相同固定预测估计假阳性和假阴性校正，忽略了错误状态的动态演化，难以处理复杂病例。

Method: FAPR 将当前分割掩码表示为动态失败状态，将每次修复操作建模为状态转换算子。接受的校正形成新的预测状态供后续错误诊断和修复使用，使后续操作能适应前期变化。条件路由选择性地激活必要的状态转换，失败重放使模型接触稀有错误状态。冻结基础分割器以保留其已建立的分割能力。

Result: 在三个公开的超声病变分割基准数据集上，FAPR 将平均 DSC 提高了 1.52%。在 BUSI 和 TN3K 的极困难子集上，平均增益达到 13.77%。

Conclusion: FAPR 能够在保留基础分割器能力的同时改善困难病例的表现，通过考虑动态错误演化解决了现有后验校正方法的局限性。
...
TL;DR: 本文提出了一种基于重建3D hammer轨迹的自动链球释放帧检测方法(MS-RFD)，通过融合四个运动学信号来自动识别释放时刻，解决手动逐帧检查的主观性和繁琐性问题。


<details>
  <summary>Details</summary>
Motivation: 链球成绩主要由释放时的运动学条件决定（释放速度、释放角、释放高度），但从视频中识别释放时刻通常需要人工逐帧检查，在实际训练中既主观又麻烦，因此需要一种自动化的释放帧检测方法。

Method: 提出MS-RFD方法，整合四种互补的运动学信号：速度动态变化、角速度转换、相对于旋转中心的径向距离、以及释放后轨迹的线性度。融合这些信号对候选释放帧进行评分和验证。通过投掷距离估计误差来评估方法性能，并进行消融研究分析各信号的贡献。

Result: 速度动态变化和径向扩展为释放帧检测提供了最强信号，而角速度和释放后线性度提供较小的优化改进。

Conclusion: 该方法能够实现链球释放帧的自动检测，四种信号的融合有效提高了检测准确性，其中速度动态和径向距离是最关键的检测指标。
...
TL;DR: 本文研究了在Vision Transformers (ViTs)中，通过Token Merging技术降低自注意力机制的计算复杂度，并评估了Graph-Guided Token Merging (G2TM)在不同架构和解码器上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: ViTs的自注意力机制复杂度随token数量呈二次方增长，成为效率和大规模部署的主要障碍。现有token合并方法通常只在单一架构中评估，不清楚其有效性来自合并机制本身还是与特定解码器的配合。

Method: 将G2TM模块扩展到Segmenter之外的环境，在三个语义分割框架（Segmenter、SETR、EoMT）和三个解码器家族（Linear、Transformer、卷积）以及标准ViT图像分类上进行评估。

Result: G2TM的行为和精度-效率权衡在每个测试架构中表现一致，表明其有效性是编码器的属性而非解码器。在图像分类任务中精度下降更小。G2TM的最优超参数主要取决于 backbone 的预训练方案和目标数据集，而非解码器选择。在ADE20K数据集上，分割模型的GFLOPs减少22-47%，吞吐量增加最高74%。

Conclusion: G2TM的有效性源于编码器本身，具有良好的跨架构泛化能力，在不同解码器和任务（语义分割、图像分类）中均能保持一致的精度-效率权衡，为ViT的高效部署提供了有效的解决方案。
...
TL;DR: 首次将视觉自回归(VAR)离散码本next-scale预测应用于RAW-to-sRGB ISP任务，仅用2.99%可训练参数(32.93M)实现性能提升


<details>
  <summary>Details</summary>
Motivation: RAW-to-sRGB图像信号处理需从传感器测量中恢复感知逼真的色彩和精细细节，但常面临空间对齐不佳和元数据缺失问题。现有方法在颜色重建和结构保持之间难以平衡

Method: 冻结1.10B参数VAR主干网络，仅微调32.93M参数(2.99%)用于RAW条件ISP；提出频率分解颜色损失：低频色调用小波LL余弦相似度监督，色度边缘用细节带ℓ1监督

Result: Zurich RAW-to-sRGB基准测试集(1204图)：PSNR-Y从21.31提升至21.89 dB，LPIPS从0.276降至0.218

Conclusion: VAR先验能良好保持结构，但连续颜色转移仍是主要瓶颈：oracle仿射校正可恢复3.8dB，而学习的颜色头仅带来边际增益
...
TL;DR: 本文提出了一个针对全模态生成模型（Omni-Models）的综合评估框架，聚焦于物理世界推理能力，通过四个互补维度评估MiniMax-H3模型在多模态输入下的推理表现。


<details>
  <summary>Details</summary>
Motivation: 全模态生成模型的兴起带来了统一建模文本、图像、视频和音频的可能性，但现有评估框架受限于输入模态单一且提示与目标内容高度匹配。本文旨在探索：多模态对齐能否提升模型的世界推理能力？全模态输入能带来哪些新的评估范式？

Method: 构建了四个互补的物理世界推理评估维度，设计了四种场景：隐式提示配多帧、音频-图像、前缀-视频和音频-视频输入。每个模态仅提供部分证据，要求模型联合推理以推断潜在事件状态和未来动态。共517个评估实例。

Result: MiniMax-H3总体成功率为41.97%。视频决策推理表现最佳（56.00%），音频消歧推理最弱（27.40%）。

Conclusion: 有效的多模态整合仍是充分利用不同输入模态优势的关键。当前模型在不同推理维度上表现差异显著，表明多模态推理能力仍有较大提升空间。
...
TL;DR: 本文提出PDA++，一个用于遥感图像的环境感知目标插入框架，通过计划、解耦和同化三个模块实现 realistic synthetic augmentation，解决少样本和长尾场景下的标注成本问题。


<details>
  <summary>Details</summary>
Motivation: 遥感目标识别受限于稀有目标观测数据稀缺和标注成本高的问题，需要一种能够生成 realistic synthetic augmentation 的方法，特别是在少样本和长尾场景下。目标插入是增加目标多样性同时保持真实背景的有效方法，但生成的目標需要与周围环境协调适应。

Method: 提出统一的环境感知目标插入框架PDA++，包含三个模块：1) Planning通过结合几何间隙与结构-尺度感知线索的affordance field确定场景兼容的姿态；2) Decoupling引入姿态条件背景提供精确空间指导和目标-场景上下文，使参考目标保持身份同时适应目标观察，并自然提供像素级分割增强掩码；3) Assimilation通过最优传输对齐多尺度纹理分布以提高局部一致性。

Result: 在光学基准上，PDA++实现whole-image FID为6.28，少样本识别mAP50提升17.69点，相对真实数据基线提升28.8%。在SAR图像上，舰船检测提升4.10 mAP50点，并在跨数据集迁移和非晶态目标插入下保持有效。

Conclusion: PDA++框架能够有效解决遥感图像中少样本和长尾场景的标注难题，通过环境感知的目标插入显著提升目标识别性能，在光学和SAR遥感图像上均展现出优异的泛化能力。
...
TL;DR: 本文提出Pose2Muscle框架，从人体姿态估计离散肌肉活动状态，无需在推理时使用sEMG信号。该方法将肌肉估计重构为结构化预测问题，结合多尺度时空注意力和DAG解码器，并构建了PoseEMG-43数据集。在随机分割下达到86.36%的邻级准确率和0.8821的Pearson相关系数，在主体级分割下达到63.97%和0.6795。


<details>
  <summary>Details</summary>
Motivation: 传统肌肉活动监测依赖sEMG传感器，在实际长期使用中受限。现有的从姿态推断肌肉活动的方法存在较大误差，因为外部姿态与内部肌肉激活之间存在显著差距。需要一种无需sEMG信号即可推断肌肉活动的方法。

Method: 提出Pose2Muscle框架：1)将肌肉估计重构为离散肌肉活动状态的结构化预测问题；2)结合多尺度时空注意力捕获不同空间和时间尺度的运动模式；3)使用基于有向无环图(DAG)的解码器维持多个候选肌肉状态假设并进行结构化轨迹推理；4)构建PoseEMG-43同步姿态-sEMG数据集，包含43种日常动作的2,992个运动实例。

Result: 在随机分割下：邻级准确率86.36%，Pearson相关系数0.8821；在主体级分割下：邻级准确率63.97%，Pearson相关系数0.6795。优于基于检索和基于姿态的基线方法。

Conclusion: 证明了从人体姿态推断结构化肌肉状态模式的可行性，Pose2Muscle在直接生理传感不切实际时具有肌肉感知运动分析的潜力。
...
TL;DR: 本文研究了视觉语言模型在同时输入文本提示和图像时的意外行为，发现图像中的具体实例即使是文本仅询问一般概念（如狗的品种），图像仍会影响模型响应，且图像的呈现方式（主体聚焦 vs 情境中的主体）决定了响应的偏移方向。


<details>
  <summary>Details</summary>
Motivation: 当前大视觉语言模型（LVLMs）的评估主要关注单一文本提示或文本+图像的组合，但忽略了图像中具体实例对模型响应的意外影响。研究旨在揭示这一被忽视的问题，即图像的存在和呈现方式如何改变模型对通用概念描述的响应。

Method: 实验设计：向LVLMs输入关于一般概念的文本提示（如某品种狗的属性描述），同时配合展示该品种具体狗的图像，并对比不同图像呈现方式（主体聚焦、情境中的主体）下的响应差异，通过量化物理术语的使用比例进行分析。

Result: 文本仅提示时物理术语占18%，主体聚焦图像使物理术语增至45%，情境中的主体图像使物理术语增至40%。图像的具体实例会改变响应方向，且呈现方式决定偏移方向。

Conclusion: 视觉线索对LVLMs的意外影响凸显了在评估模型鲁棒性时必须考虑图像的存在及其呈现方式的重要性，研究者需重新审视LVLMs在多模态输入下的行为特性。
...
TL;DR: 提出一种在线多相机3D跟踪架构，通过在稀疏查询上显式预测ID来恢复关联精度，在AI City Challenge 2026 Track 1测试集上将HOTA从29.63提升至38.01，排名第三。


<details>
  <summary>Details</summary>
Motivation: 现有基于查询的跟踪器仅将身份隐式存储在实例库中，查询中断时身份会碎片化，导致多相机3D跟踪中场景全局身份维护困难。

Method: 提出由外而内的Sparse4D检测器将校准视图融合到世界帧3D检测中并传播稀疏查询库；使用因果MOTIP ID解码器将检测与有限轨迹记忆关联；引入度量空间门控和基于邻近的新生儿恢复机制。

Result: 在AI City Challenge 2026 Track 1测试集上，HOTA从29.63提升至38.01，AssA从20.83提升至31.10，公开榜排名第三。全序列验证表明解耦ID训练优于原生身份，而检测器与ID目标的联合训练产生场景依赖的增益和损失。

Conclusion: 通过显式ID预测和稀疏查询架构可有效恢复多相机3D跟踪的关联精度，解耦ID训练是提升性能的关键策略。
...
TL;DR: MSR是一种用于LTX视频生成的多主体参考条件化方案，通过分离的潜在token组和槽位嵌入来实现多图像条件化，保持各参考图像的独立性。


<details>
  <summary>Details</summary>
Motivation: 在视频生成中，如何同时 conditioning 多个参考图像并保持各自的外观特征，同时让每个参考与其预期角色正确关联是一个挑战。

Method: 将每个参考图像独立编码为静态clip并表示为单独的潜在token组；使用紧凑的Fourier-feature MLP添加数值槽位嵌入；槽位依赖的时间偏移修改 rotary 坐标；参考组前置到噪声目标token；在目标-only流匹配训练中作为干净上下文。通过低秩适配实现。

Result: 定性示例展示不同角色和参考环境的组合；与连续参考baseline相比参考混淆减少；相似服装、复杂服饰和视角变化仍具挑战。补充实验添加语音条件化并保持视觉参数冻结。

Conclusion: MSR方案成功实现了多图像条件化，减少了参考混淆，但在复杂场景（相似服装、复杂服饰、视角变化）方面存在局限，音频参考实验表明可保持视觉参数冻结同时添加语音条件。
...
TL;DR: 本文提出了RSP3D数据集，首个针对跑步专用假肢(RSP)用户的3D运动数据集，并定义了假肢感知的3D姿态估计任务。现有方法无法处理假肢几何形状，且缺乏针对截肢者的运动先验。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法无法应用于假肢用户：基于模型的方法使用非截肢者数据训练，无法表示假肢几何；无模型方法缺乏身体运动先验，在遮挡下不可靠。特别是跑步专用假肢具有复杂曲面几何且运动动态性强。

Method: 1) 收集RSP3D数据集：首个假肢用户3D数据集，涵盖不同截肢条件参与者的日常生活和运动动作，使用多摄像头标记式运动捕捉系统；2) 形式化定义假肢感知3D姿态估计任务；3) 提出混合基线方法：结合基于模型的身体关节估计与无模型假肢形状恢复。

Result: 在零样本设置下评估了代表性方法，确认了各方法的局限性。混合基线方法建立了未来研究的起点。

Conclusion: RSP3D数据集和假肢感知3D姿态估计任务的提出，为康复评估和运动表现评估等应用奠定了基础。混合基线方法证明了结合模型与无模型方法处理假肢几何的可行性，为后续研究提供了方向。
...
TL;DR: 本文提出Vocabulary-Guided Gait Recognition范式（Gait-World），利用视觉语言模型（VLM）的词汇知识来增强步态识别。核心思路是将步态概念与人类词汇通过VLM关联，解决传统步态识别中"什么是步态"概念模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 1. 传统步态识别方法中，基于外观的方法将步态视为人体形状和运动信息，基于模型的方法将步态视为人体结构点，但这些定义对人类理解而言仍然模糊。2. 现有步态网络仅从有限步态数据库学习，难以获得通用步态特征。3. VLM在视觉任务中取得进展，但对步态模态的认知能力有限。

Method: 1. 构建词汇提示库（Vocabulary Base）：选择步态周期动作作为词汇基，建立步态和词汇特征空间的桥梁。2. 提出α-Gait模型：首个Gait-World模型，利用VLM词汇知识指导步态网络学习。3. 设计Vocabulary Relation Mapper和Gait Fine-grained Detector：解决词汇和步态特征异构性问题，将词汇关系映射到步态空间，检测相应步态特征。

Result: 在CASIA-B、CCPG、SUSTech1K、Gait3D和GREW数据集上进行了广泛实验，验证了VLM词汇信息在步态识别领域的潜在价值和研究方向。

Conclusion: 通过词汇引导的步态识别范式，能够促进人类对步态概念的理解，为步态识别领域提供了新的研究方向，证明了利用VLM词汇知识提升步态识别性能的可行性。
...
TL;DR: StrucPhysVideo是一个面向具身AI的视频世界模型家族，通过物理聚焦的数据pipeline和稀疏MoE架构，实现从图像-语言条件视频预测到动作驱动交互的物理动力学建模，在Physics-IQ Verified基准上达到45.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 物理动力学建模（包括物体运动、交互和状态变化）是具身AI视频世界模型的核心，但现有方法缺乏对物理事件的显式监督和动作条件预测能力。

Method: 1) 数据pipeline：运动感知视频分割、质量过滤、物理相关性验证，带有物体、材料和时序交互的结构化标注；2) 相机运动与物体行为解耦；3) 显式描述接触、变形和状态转换；4) StrucPhysVideo-TI2V：稀疏MoE文本-图像到视频模型，采用渐进强调物理动力学的课程学习；5) StrucPhysVideo-IA2V：动作条件、因果自回归生成、少步蒸馏（4步去噪）。

Result: 在Physics-IQ Verified基准上达到45.5%准确率，比Cosmos3-Super-Image2Video提升2.8个百分点；Caption消融实验验证了物理聚焦监督的有效性；IA2V实现了仅用4步去噪的增量机器人 rollout。

Conclusion: StrucPhysVideo成功将物理动力学建模从图像-语言条件视频预测推进到动作驱动的交互预测，为具身AI提供了更有效的物理世界模拟能力。
...
TL;DR: 本文提出GaitMoE方法，通过动作检测视角解决步态识别中的遮挡问题。该方法使用时间专家混合模块(MTE)和动作专家混合模块(MAE)自适应构建动作锚点和动作建议，并在端到端联合训练中仅使用ID标签进行监督。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的广泛遮挡给步态识别带来挑战，原因包括：1) 信息缺失和噪声；2) 身体位置和尺度的错位。现有的步态序列本身具有解决遮挡问题的内在特性：相邻帧具有步态连续性可推断遮挡区域，步态周期可整合整体动作与遮挡动作的信息。

Method: 提出基于动作检测的专家混合模型(GaitMoE)，包含两个核心模块：1) MTE(时间专家混合模块)：通过时间专家自适应构建动作锚点；2) MAE(动作专家混合模块)：从动作锚点自适应构建动作建议。将步态序列视为动作组合，以动作检测作为代理任务与步态识别联合端到端训练，仅使用ID标签进行监督。

Result: 在OccGait、OccCASIA-B、Gait3D和GREW数据集上进行了广泛实验，证明了GaitMoE的优越性能。

Conclusion: 通过动作检测视角解决步态识别遮挡问题是有效的方法，GaitMoE在多个数据集上取得了先进的性能。同时构建了首个统一的遮挡步态基准数据集OccGait，包含丰富的遮挡场景和遮挡类型标注。
...
TL;DR: DR$.$WILSS是一种基于扩散生成回放的弱监督类增量语义分割方法，利用语言线索引导扩散过程，通过自修复和正则化技术生成高质量回放数据，并采用LoRA对齐生成数据与训练数据的统计特性，在避免存储训练数据的情况下实现先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决增量学习中的灾难性遗忘问题，同时避免存储训练数据和使用资源密集型工具

Method: 提出DR$.$WILSS框架，使用基于扩散的生成回放方法，结合语言线索引导扩散过程，采用自修复和正则化技术生成回放数据，并应用LoRA对齐生成数据与训练样本的统计特性

Result: 在多个基准数据集和生成架构上实现了最先进的性能

Conclusion: DR$.$WILSS能够在训练复杂度和推理精度之间取得最佳权衡，是实际应用的有前景的解决方案
...
TL;DR: CSWAM通过将V-JEPA 2.1构建的因果语义专家集成到FastWAM中，解决了世界动作模型在视觉分布偏移下泛化能力差的问题。该方法利用V-JEPA的时间 grounding 表示能力，减少对外观细节的依赖，通过稀疏历史观测学习未来演变，并在推理时保持高效的动作-only推理。


<details>
  <summary>Details</summary>
Motivation: FastWAM-style世界动作模型虽然支持高效的动作-only推理，但在视觉分布偏移下泛化性能差。原因在于其重建导向的表示过度强调特定外观细节，限制了对新场景和物体的泛化；且缺乏观测历史，无法为在陌生视觉条件下稳健识别任务相关状态变化和运动提供时间证据。

Method: 提出因果语义世界动作模型(CSWAM)，通过以下方式增强FastWAM：1) 集成基于V-JEPA 2.1构建的因果语义专家，V-JEPA提供较少依赖外观细节的时间 grounded 语义状态变化和运动表示；2) 专家从当前和过去观测的稀疏历史中学习未来演变；3) 通过因果注意力将历史派生上下文共享给视频和动作流；4) 推理时，CSWAM基于当前视频状态和观测到的语义历史进行动作去噪。

Result: 仿真和真实机器人实验表明：1) RoboTwin 2.0 Clean-to-Randomized转移任务中，CSWAM将成功率从10.16%提升至45.18%，提升35.02个百分点；2) 两个真实机器人任务和三个OOD难度水平下，CSWAM平均成功率比FastWAM提高42.5个百分点，从27.5%提升至70.0%。

Conclusion: CSWAM成功解决了FastWAM在视觉分布偏移下泛化能力差的问题，通过引入V-JEPA 2.1因果语义专家，实现了在保持高效动作-only推理的同时，显著提升了模型对新场景、物体和视觉条件的泛化能力，具有重要的实际应用价值。
...
TL;DR: GeoCond是一个轻量级可靠性适配器，用于冻结的馈送式3D基础模型（如VGGT），能够预测姿态级不确定性和精炼门控，决定何时应该信任模型的几何预测。


<details>
  <summary>Details</summary>
Motivation: 馈送式3D模型在低重叠、低视差和极端相对旋转条件下会静默失败，这些失败由几何条件决定，且原生置信度无法很好地捕捉这些失败情况。

Method: GeoCond读取主干网络预测的几何信息，输出姿态级不确定性和精炼门控。训练时可使用帧排列轨道方差、真实姿态误差（当有标签时）或无标签独立姿态图的循环残差进行监督。推理时只需一次主干网络前向传播和一个小型MLP。

Result: 在VGGT上，GeoCond将分布外AUSE从0.32改善到0.20，零样本迁移到户外极端视角场景，避免了统一束束调整导致的崩溃。循环蒸馏变体可在无真实标签情况下进行适应，包括排列方差在等变模型上消失的情况。

Conclusion: 可靠的馈送式3D重建不仅需要预测几何，还需要知道何时应该信任这些几何预测。相同可靠性信号支持门控精炼、姿态图加权、校准、筛选和捕获决策。
...
TL;DR: CADSplat是一个从稀疏（<15张）宽基线图像重建照片级真实感几何精确数字孪生的框架，通过使用CAD形状先验正则化3D高斯溅射（3DGS）实现。该方法通过将分割的物体轮廓与CAD库渲染的轮廓匹配来获取最佳匹配模型和相机姿态，然后将3D高斯图元锚定在检索到的CAD模型表面，并联合优化3DGS参数、相机-物体配准和非刚性变形场。


<details>
  <summary>Details</summary>
Motivation: 从稀疏视图重建照片级真实感和几何精确的数字孪生是一个具有挑战性的问题。现有的少样本方法在几何准确性方面存在不足，无法满足增强现实、物理模拟等应用的需求。该研究旨在通过引入显式CAD形状先验来解决稀疏视图重建的质量问题。

Method: 1.将分割的物体轮廓与CAD库渲染的轮廓进行匹配；2.保留最佳匹配模型的相机-物体姿态；3.将3D高斯图元锚定在检索到的CAD模型表面；4.联合优化3DGS参数、相机-物体配准和非刚性变形场（用于补偿物理物体与CAD模型之间的形状差异）

Result: CADSplat在两个真实世界数据集上优于无约束、少样本和网格纹理化基线方法，并能优雅地降级到仅3个视图。实验表明，大部分渲染质量提升来自于对溅射图元的约束方式（固定图元集绑定到表面，由单个平滑变形场驱动），而非CAD形状本身。CAD模型在视图最稀缺的位置添加形状知识，使每个相机位于物体自身坐标系中。

Conclusion: 该研究成功展示了利用CAD形状先验进行稀疏视图3D重建的可行性。关键贡献在于约束溅射图元的方式而非CAD形状本身。该方法enable了超越新视图合成的应用，包括无标记AR配准、逐图像物体姿态估计、物理模拟以及从设计到重建的部件标签转移。
...
TL;DR: 本文提出对比噪声对齐(Contrastive Noise Alignment, CNA)方法,通过优化噪声表示创建动态对比耦合,解决扩散模型中任意数据-噪声耦合导致的曲率传输问题。该方法将噪声批次建模为交互粒子系统,使用跨模态InfoNCE目标进行对齐,并通过角熵项和径向范数惩罚防止空间崩溃。理论证明该平衡态渐近保持高斯结构。


<details>
  <summary>Details</summary>
Motivation: 扩散和流匹配模型通常通过独立采样的高斯噪声腐蚀数据,但这种前向过程导致任意数据-噪声耦合,迫使网络学习高曲率传输。现有最优传输方法虽然重新分配固定噪声样本,但源噪声分布本身仍是被动的。CNA旨在通过直接优化噪声表示来创建动态对比耦合。

Method: 将噪声批次建模为交互粒子系统,采用跨模态InfoNCE目标对齐噪声粒子与其配对的数据目标。为防止空间崩溃,使用角熵项和径向范数惩罚进行正则化。理论证明该平衡态渐近保持高斯结构。

Result: CNA改善了噪声与数据之间的对齐,降低了流曲率,以更少的采样步骤提供更好的生成质量。在2-4步采样的像素空间生成中,CNA相比标准整流流减少超过50%的FID,相比最优传输基线减少至少24%。

Conclusion: CNA通过动态对比耦合有效解决了扩散模型中的噪声-数据对齐问题,为高效少步生成提供了新思路。
...
TL;DR: 本文提出Origins模型，利用扩散模型实现步态生成与表示的联合学习，通过统一模板生成不同实体来正则化步态表示空间。


<details>
  <summary>Details</summary>
Motivation: 当前步态网络主要依赖对比学习，容易导致语义不一致和表示uniformity问题。作者受"不能创造则不能理解"启发，借鉴扩散模型在图像生成中展现的语义结构和记忆能力，希望将生成能力融入步态学习。

Method: 提出Origins模型，核心思想是不同实体由统一模板生成。具体包括：1)将统一模板扩散为时间步模板用于步态生成学习；2)同时将统一模板迁移用于步态表示学习；3)生成学习与表示学习作为统一框架进行端到端联合训练。

Result: 在CASIA-B、CCPG、SUSTech1K、Gait3D、GREW和CCGR-MINI六个数据集上验证，Origins实现了统一的生成和表示学习，取得了优越性能。

Conclusion: Origins模型成功实现了步态生成与表示的统一学习框架，为步态识别领域提供了新的研究思路。
...
TL;DR: 本文提出了Semantic-ITC，这是首个公开的室内移动激光扫描（MLS）帧级语义分割数据集和基准。数据集包含52个室内序列、79,108个MLS帧和12.3亿个标注点，涵盖16个语义类别。最佳基线模型达到79.27% mIoU。


<details>
  <summary>Details</summary>
Motivation: 当前点云语义分割基准主要关注重建的室内场景或室外LiDAR感知，而室内MLS帧的语义标签仍然 largely absent。该研究旨在填补这一空白，提供首个公开的室内MLS帧级语义分割数据集。

Method: 数据集构建采用混合标注工作流：结合视觉基础模型对同步RGB图像的预测、BIM结构信息和人工细化的方法，最终将标签分配给原始LiDAR帧。单帧基准测试使用标准语义分割方法进行评估。

Result: 最佳基线模型达到79.27% mIoU。剩余错误主要集中在物体边界和模糊的室内类别，表明室内MLS分割在稀疏帧几何和长尾类别分布下面临挑战。

Conclusion: Semantic-ITC是首个室内MLS帧级语义分割公开基准数据集保留了室内MLS的稀疏、非均匀和帧级采样模式，与场景级重建点云和基于网格的室内数据集不同。该数据集为评估室内MLS帧上的语义分割提供了公开基准，支持未来帧级室内MLS语义分割研究。
...
TL;DR: DiT-Garment是一种基于2D扩散Transformer架构的动态3D服装建模方法，能够在任意人体姿态下对服装进行动画处理，无需复杂图卷积操作即可处理不同设计的服装。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法对未见过的服装设计和物理材质进行动画处理，也无法直接推断任意目标姿态下的变形。DiT-Garment旨在解决这一问题，实现对不同设计和物理材质的服装进行动态建模。

Method: 1.采用2D扩散Transformer架构学习UV空间中的3D变形；2.将模板服装表示为与标准化姿态人体模型对齐的3D三角形网格；3.以模板的3D位置图（UV空间表示）作为条件；4.同时以人体运动和物理参数为条件进行物理约束。

Result: 在合成数据和真实数据上进行了定性和定量评估；仅在自动生成的服装设计的合成模拟数据上训练，但能泛化到捕获的和艺术家制作的服装设计。

Conclusion: DiT-Garment成功实现了从合成数据到真实数据的泛化，能够处理未见过的服装设计和物理材质，为3D服装动画提供了新的生成式建模方法。
...
TL;DR: 本文提出LDE框架，利用车联网协作感知(CP)为自动驾驶感知模型的无监督领域自适应提供高质量监督信号，解决单车辆视角下伪标签质量低的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶感知模型在新环境中因领域偏移而泛化性能下降，现有无监督自适应方法仅依赖本车数据导致伪标签质量差，而协作感知通常优于单车感知，可作为更好的监督来源。

Method: 提出三方面创新：(1)面向自适应的特征共享机制，选择性传输最关键信息以应对通信带宽限制；(2)FoV过滤方法，消除协作感知视角与学习者视角之间的不匹配标签；(3)课程学习策略，逐步利用伪标签。

Result: 在3D目标检测任务上的广泛实验表明，LDE始终优于预训练模型和最先进的无监督自适应方法。

Conclusion: 通过利用协作感知作为监督来源，LDE为无监督领域自适应提供了一种超参数不敏感、相对可靠的伪标签生成方法。
...
TL;DR: 本文提出Streaming Video Memory Agent (SVMemAgent)，一种用于视频流中在线关键帧选择的新方法，通过GRPO训练和任务驱动奖励，使代理能够动态维护一个紧凑且具代表性的记忆库，在推理时无需访问查询或未来帧即可选择信息丰富的关键帧。


<details>
  <summary>Details</summary>
Motivation: 现有关键帧选择研究主要集中在离线设置，假设可以提前访问完整视频和查询。而现实世界的流媒体场景需要在线帧选择，需要在未知视频时长的情况下做出决策，且无法访问查询或未来帧。这是一个重要的实际挑战。

Method: 1. 提出Streaming Video Memory (SVMem)：一个紧凑且具代表性的已观察内容记忆库，随视频流不断更新。2. 提出SVMemAgent：在每个时间步决定是替换现有记忆帧还是丢弃传入帧，动态维护记忆。3. 使用Group Relative Policy Optimization (GRPO)结合来自多样化问答对的任务驱动奖励来训练代理，使策略隐式暴露于查询分布，从而保留generally informative的帧。

Result: 在在线和离线视频基准测试上的实验表明：1. SVMemAgent始终优于在线帧选择基线；2. 与假设可访问完整视频和查询的离线方法相比，具有竞争力的性能；3. 通过任务驱动奖励，SVMemAgent学习到一种emergent关键帧选择策略，倾向于选择包含文本信息的帧，这可能有利于下游VideoQA任务。

Conclusion: SVMemAgent成功解决了流媒体视频中在线关键帧选择的挑战，通过GRPO训练和任务驱动奖励学习到有效的关键帧选择策略，在线性能优于基线方法，离线性能具有竞争力，并展现出对包含文本信息帧的偏好，这对下游VideoQA任务有益。
...
TL;DR: 本文提出了一种面向嵌入式环境的4D雷达3D目标检测预处理框架，通过P3DP形状保持方法提取点云、MF-KDE多帧噪声点判别提升稀疏点云可靠性，以及ENS评估指标综合考量精度、实时性、恶劣天气鲁棒性和模型复杂度。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在恶劣天气下具有稳定感知能力，是下一代自动驾驶感知的有前景传感器，但在嵌入式硬件资源受限环境中部署需要同时考虑感知精度、实时性能和计算复杂度的雷达预处理方案。

Method: 提出三阶段预处理框架：1) P3DP（百分位3D形状保持）从雷达张量中提取点云，保持物体形状信息并抑制噪声和误报；2) MF-KDE（基于核密度估计的多帧噪声点判别）利用多帧信息提高稀疏雷达点云的密度和可靠性；3) ENS（嵌入式NetScore）评估嵌入式部署的适用性。

Result: 该框架在嵌入式平台上实现了精度、实时性、恶劣天气鲁棒性和模型复杂度的综合优化。

Conclusion: 提出的预处理框架能够有效提升4D雷达在嵌入式自动驾驶感知系统中的实用性，为低资源环境下的雷达感知提供了新的解决方案。
...
TL;DR: STUNet-Fusion是一种用于超声视频中针尖定位的时空融合框架，通过构建三通道时空融合张量（灰度外观、基于网格的运动特征、原始帧差），结合ResNet-34编码器、ConvLSTM时序建模和U-Net解码器重建概率热图，实现亚像素级定位精度，显著提高了定位鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 超声图像中针尖定位具有挑战性，因为针可能出现弱信号、不连续或部分不可见的情况，同时成像伪影和解剖结构可能产生类似响应，传统方法难以准确鲁棒地定位。

Method: 提出三通道时空融合策略：1）灰度外观通道提供静态空间信息；2）基于网格的运动特征通道捕获针的运动模式；3）原始帧差通道增强时序变化信号。采用共享ResNet-34编码器提取空间特征，ConvLSTM整合时序依赖关系，U-Net解码器重建密集概率热图，最后通过soft-argmax操作提取亚像素级坐标。

Result: 实验结果表明，该时空融合策略相比传统基线方法显著提升了定位鲁棒性。

Conclusion: STUNet-Fusion通过有效的时空特征融合，成功解决了超声视频中针尖定位的难点，实现了鲁棒且精确的亚像素级定位，为超声引导下的介入操作提供了可靠的技术支持。
...
TL;DR: 本文提出HAP框架，用于预测人类头部6-DoF运动。该方法利用手部运动和目标物体上下文信息，通过动态遮挡图和时序推理来预测头部运动。同时引入Bottle数据集，用于研究物体操作中头部与手部协调运动。


<details>
  <summary>Details</summary>
Motivation: 现有自我中心运动预测主要关注手部和被操作物体，而人类头部运动研究相对不足。头部在操作过程中既要将感知重定向到目标以获取任务相关信息，又需要与身体和手部运动协调。

Method: 提出HAP（Hand-Driven Active Perception）框架，包括：1）从手部运动和物体几何推断目标物体的置信度；2）构建动态预测目标中心模态遮挡图（P-TAOG）；3）有向图和因果时序推理编码感知状态；4）融合手部和头部运动历史；5）地平线门控融合学习轨迹与恒速先验。

Result: 在公开数据集和Bottle数据集上，HAP实现了更低的头部运动预测误差，验证了手部驱动意图和动态遮挡推理对预测头部运动的价值。

Conclusion: 手部驱动的意图推断和动态遮挡推理对于预测人类头部运动具有重要价值，该方法为自我中心场景下的头部运动预测提供了新的思路。
...
TL;DR: 本文提出CARA方法，通过自适应分配哈希表的分辨率级别和引入可逆像素打乱变换来解决多分辨率哈希编码中的哈希冲突和参数利用效率低下的问题。


<details>
  <summary>Details</summary>
Motivation: 标准多分辨率哈希编码采用数据无关的设计，不同分辨率级别分配相同的哈希表容量，但图像频率内容差异大，导致某些层级严重哈希冲突而其他层级参数利用不足，造成容量分配低效。

Method: 提出两种技术：(1)碰撞感知分辨率适应(CARA)：通过平衡各哈希层级的有效信息负载来分配每层级分辨率，减少容量瓶颈；(2)可逆像素打乱变换：通过重新分配空间信息降低哈希负载因子，在不增大哈希表的情况下减轻碰撞导致的信息损失。

Result: 在Kodak图像、十亿像素自然图像和原始全切片图像上实验表明：CARA在保持相似参数数量时可获得最高6.11 dB的PSNR提升，或仅使用27.76%的参数即可达到最先进的性能水平。

Conclusion: CARA通过自适应分配哈希表容量和空间信息重排，显著提升了隐式神经表示的参数效率，为高分辨率图像表示提供了更有效的解决方案。
...
TL;DR: 本文提出了一种解耦的语义理解框架，用于解决AI City Challenge 2026 Track 2中的视觉问答(VQA)和交通事件描述生成任务。该方法将语义理解与语言生成解耦，先将交通问题解析为结构化语义事实，再利用这些事实指导标题生成。在官方基准测试中，该方法达到87.09%的VQA准确率和60.0853的总体S2分数，排名第一。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言方法往往将语义理解与语言生成纠缠在一起，容易产生幻觉和跨事件阶段的不一致推理。同时，合成到真实领域的域迁移也是一个重大挑战。

Method: 1. 使用冻结的V-JEPA编码器提取预测性场景表示；2. 轻量级Llama-based预测器生成VQA答案；3. 引入无训练的结构化精炼机制，利用统计先验、问题间关系和时序事件一致性纠正预测错误；4. 将精炼后的语义事实提供给Qwen3-VL-8B生成行人和车辆描述。

Result: 在AI City Challenge 2026 Track 2官方基准测试中，VQA准确率达到87.09%，总体S2分数为60.0853，在所有参赛团队中排名第一。

Conclusion: 预测性世界表示结合结构化语义精炼能够实现更准确、更可靠的交通理解，从而生成更高质量的语言描述。该解耦框架有效解决了视觉-语言模型中的幻觉问题和跨事件推理不一致问题。
...
TL;DR: 本文提出了一种无需3D校准的多摄像头实时跟踪系统，通过"即开即用"的几何优化方法解决大规模部署中的隐私、带宽和校准难题。该系统使用基于质心的投影优化(PO)对粗略手动单应性进行连续微调，能够自动适应摄像头移动或环境变化，同时保持零计算延迟。


<details>
  <summary>Details</summary>
Motivation: 传统多摄像头跟踪依赖于严格的3D现场校准，但在实际大规模部署中面临多重障碍：隐私法规禁止录制视频进行离线校准；有限带宽无法同步数百个摄像头的高分辨率流；用校准目标覆盖巨大物理场地在后勤上不可行。因此需要一种无需人工干预、可在动态环境中自动运行的隐私安全方案。

Method: 系统从粗略的手动单应性出发，引入基于质心的投影优化(PO)方法，使用实时检测流持续对齐地面平面几何。PO对已传输的轻量级元数据异步操作，不增加实时跟踪器的计算延迟。优化后的几何输入多摄像头鸟瞰图(BEV)跟踪器，融合检测结果并统一跨区域轨迹。

Result: 该系统可在无需人工干预情况下自动适应摄像头移动或环境变化，严格基于实时匿名元数据操作，确保隐私安全、零计算开销的跟踪管道，在静态录制视频校准不可行的动态环境中保持全局一致性。

Conclusion: 本研究提出的方法有效解决了大规模多摄像头跟踪中的校准难题，实现了隐私安全、零延迟、自适应的跟踪系统，为动态环境中无法进行传统静态校准的场景提供了可行解决方案。
...
TL;DR: PULSE是一种实用的深度学习图像压缩codec，通过超低复杂度(5.2 kMAC/pixel)的神经接收器和创新的熵编码方法，在CPU上实现超低延迟解码，并在压缩性能上与HM相当，感知优化后可与MS-ILLM等大型感知codec竞争。


<details>
  <summary>Details</summary>
Motivation: 现有学习图像压缩方法在资源受限硬件(特别是CPU)上计算成本过高，难以在实际应用中部署。需要一种既能在各种硬件平台上实现低延迟解码，又保持良好压缩性能的实用codec。

Method: 1. 设计超低复杂度5.2 kMAC/pixel的神经接收器；2. 采用整数线性CDF预测器和元先验实现高效的位精确熵编码；3. 引入基于启发式探测的智能进化过程，通过人类-LLM协作迭代优化架构。

Result: PULSE在单CPU线程上解码1080p图像仅需126ms，压缩性能与HM相当，经过感知优化后可与MS-ILLM等大型感知codec竞争。

Conclusion: PULSE成功实现了实用化深度学习图像压缩，在低复杂度约束下兼顾了压缩性能和解码速度，为资源受限硬件上的高效图像压缩提供了可行方案。
...
TL;DR: FIVE-VLA是一种高效的自动驾驶视觉-语言-动作模型，通过高效视觉编码器（仅98个token处理高分辨率图像）和递归动作记忆(RAM)模块解决现有VLA模型的参数过多、效率低下和缺乏时序记忆的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型面临三个关键局限：1)参数过多导致计算成本高；2)高分辨率图像处理效率低；3)缺乏时序记忆能力，难以处理超车和紧急制动等需要上下文的任务。

Method: 提出两个核心贡献：1)高效视觉编码器处理448×896高分辨率图像，仅生成98个token，比现有方法减少5倍以上，且无需文本生成直接进行单次轨迹预测；2)递归动作记忆(RAM)模块，轻量级模块，让动作预测基于之前的历史动作token，提供时序上下文。

Result: 仅641M参数，在Bench2Drive闭环驾驶基准测试中比之前的SOTA VLA完成约10%更多无违规路线；在NVIDIA Physical AI AV数据集上，单视图和四视图设置下碰撞违规率分别比SimLingo降低10.2%和7.7%；A100上运行约30fps，T4上约4fps，比之前方法快8-30倍。

Conclusion: FIVE-VLA通过高效视觉编码器和时序记忆机制，成功在保持高性能的同时大幅降低计算开销和参数量，为边缘设备上的高效自动驾驶提供了可行方案。
...
TL;DR: VibeAvatar是一个多模态说话头像合成系统，通过解耦语音识别的语义特征与人类偏好的运动美学，采用音素运动适配器和基于GRPO的美学运动策略，在保持高唇形精度的同时生成自然美观的运动，并实现高效推理（10秒512p视频仅需10秒，显存约3GB）。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在说话头像合成中难以同时实现：1)准确的唇形发音；2)人类偏好的运动美学；3)高效的推理速度。研究者观察到音素准确性和运动美学来自不同来源，应在不同阶段分别处理而非由单一生成器隐式学习。

Method: 提出VibeAvatar框架，包含：1)音素运动适配器(PKA)：在条件阶段将识别导向的语音特征转换为音素运动条件；2)美学运动策略(AMP)：通过组相对策略优化(GRPO)优化流一致随机采样策略；3)轻量级流式运动生成器：工作在紧凑的1D warp潜在运动空间。

Result: 在客观指标和用户研究中均达到唇形发音、美学和效率的最佳表现；生成10秒512px视频仅需不到10秒，显存占用约3GB。

Conclusion: 通过解耦不同目标和采用针对性策略，VibeAvatar成功实现了高精度、高美学和高效推理的统一，为多模态说话头像合成提供了新范式。
...
TL;DR: GenStream是一种语义流媒体框架，通过传输骨骼关键点、摄像机视角参数和静态3D背景模型等结构化元数据替代传统像素视频，在客户端使用生成模型重建逼真人体并合成到3D场景中，实现相比HEVC超过99.9%的带宽削减。


<details>
  <summary>Details</summary>
Motivation: 当前视频流媒体占全球互联网流量主导地位，但传统视频编码管道对体育、表演或交互媒体等结构化、以人为中心的内容效率低下。标准编解码器对整个帧进行重编码，不区分前景和背景，忽略场景的语义结构，导致严重的带宽浪费，特别是在背景静态、运动仅集中于少数显著人物的场景中。

Method: GenStream将密集视频帧替换为紧凑的结构化元数据：将每个场景编码为骨骼关键点、摄像机视角参数和静态3D背景模型的组合。这些元素传输到客户端后，生成模型从中重建逼真人体并将它们从原始视角合成到3D场景中。

Result: 该范式实现极端压缩，与HEVC连续数据流相比达到超过99.9%的带宽 reduction reduction。在奥运花样滑冰 footage上部分验证了GenStream，展示了在最小数据下实现高感知保真度的潜力。

Conclusion: GenStream承认计算成本显著转移到客户端以及泛化挑战，但为体积化虚拟化身合成、跨视角规范3D演员融合和个性化观看体验开辟了新方向，为后编解码器时代的可扩展智能流媒体奠定基础。
...
TL;DR: 提出了一种名为Generalist-Specialist-MoE (GS-MoE)的两分支MoE架构，通过域约束特征融合将跨模态通用模型与特定模态专家模型耦合，解决了纯MoE架构在模态特化与跨模态共享表示之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: AI多模态医学影像模型需要在模态特化和跨模态共享表示之间取得平衡，现有纯MoE架构虽能提升领域内学习但可能牺牲跨模态信号，而跨模态信号对低患病率病理的检测尤为重要。

Method: 提出GS-MoE两分支MoE架构，通过域约束特征融合将跨模态通用模型与特定模态专家模型耦合。在RadImageNet数据集（135万图像，165种病理，三种模态）上进行实验。

Result: GS-MoE恢复了6种低患病率病理的检测（所有基线模型F1均为0），单类增益最高达+0.60 F1。同时略超密集模型和仅专家MoE基线（MCC 0.770），推理时使用参数比最强密集模型少约53%。

Conclusion: GS-MoE成功解决了模态特化与跨模态表示之间的权衡问题，在低患病率病理检测上取得显著提升，同时保持了较高的计算效率。
...
TL;DR: RankGround是一个两阶段框架，通过轻量级多模态重排器GroundRanker从密集候选集中识别最有前景的图像裁剪，实现单次VLM调用完成精确的GUI元素定位，解决准确性与效率之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding方法面临准确性与效率的根本权衡：直接全图像推理难以捕捉小型或视觉相似的UI元素，而多裁剪策略虽能提升定位精度但需要每次查询多次调用昂贵的视觉语言模型。

Method: 提出RankGround两阶段框架，核心是GroundRanker轻量级多模态重排器。由于无现成排序数据集，从现有grounding数据集构建排序监督数据，采用严格 containment 标准和边界感知的正向增强提升混乱布局中的对齐和空间覆盖。训练采用两阶段课程：先学习粗粒度containment的点对点目标，再通过列表目标细化相似裁剪间的细微语义和空间差异。

Result: RankGround在所有backbone和屏幕尺度上比第二名方法平均提升5.5%定位精度，推理速度提升1.4倍，在效率和精度方面建立新的最优水平。

Conclusion: RankGround成功实现单次VLM调用完成准确GUI grounding，在降低计算成本的同时始终优于强基线方法，兼顾了推理效率与定位精度的双重优势。
...
TL;DR: 本文系统分析了内在位置学习(IPL)在事件相机跟踪中的有效性机制，发现其效果源于IPL与多阶段卷积的协同作用，并提出计算图裁剪方法(Mask IPL)消除IPL在前向和反向传播中的噪声，在不增加参数和计算成本的情况下提升了跟踪性能。


<details>
  <summary>Details</summary>
Motivation: IPL是事件驱动跟踪中主流的位置编码方法，但其有效性的理论机制缺乏系统分析。此外，研究发现IPL在前向传播中引入噪声增加推理误差，在反向传播中阻止参数收敛到更优解。

Method: 1)系统分析IPL的作用机制，揭示零块在联合张量中充当卷积的零padding，边界效应通过多阶段卷积逐层传播，使每次参数更新都由感知模板与搜索帧相对位移的梯度驱动；2)提出计算图裁剪方法(Mask IPL)，对每层操作应用由布局确定的有效性掩码，使无效区域在前向和反向传播中等价于零padding，消除噪声使实际梯度与理想梯度一致。

Result: 在不增加参数和计算成本的情况下，Mask IPL在FE108、FELT和VisEvent数据集上提升了Tiny-scale跟踪器的AUC，并一致提升了Base-scale跟踪器的性能。

Conclusion: IPL的有效性源于与多阶段卷积的协同作用，而非位置编码本身。提出的Mask IPL方法通过消除前向和反向传播噪声，显著提升了事件相机跟踪性能，为SNNs在视觉跟踪领域的应用提供了新的理论框架和实用改进。
...
TL;DR: 水下3D重建支持海洋生态系统监测、海底检查、水下考古等应用。3D Gaussian Splatting实现了实时逼真的新视角渲染，水下变体采用物理图像形成模型分离介质效应与场景辐射，但其重建质量仍受初始化几何体的根本限制。


<details>
  <summary>Details</summary>
Motivation: 水下3D重建在海洋生态监测、海底检查、水下考古、教育和沉浸式可视化等领域有广泛应用需求，需要高质量的实时渲染技术。

Method: 采用3D Gaussian Splatting技术进行实时逼真新视角渲染，水下变体使用基于物理的图像形成模型分离介质效应与场景辐射。

Result: 该方法实现了实时水下3D重建和渲染，但重建质量受初始化几何体的根本限制。

Conclusion: 水下3D Gaussian Splatting技术的重建质量瓶颈在于初始化几何体，未来需要改进几何初始化方法来提升重建质量。
...
TL;DR: 本研究提出了一种基于普通RGB摄像头的非侵入式小鼠震颤严重程度评估方法，无需手术植入设备即可检测小鼠震颤。


<details>
  <summary>Details</summary>
Motivation: 现有的肌电图和加速度计方法存在成本高、设置复杂、手术植入造成动物压力等缺点；RGB方法虽非侵入性但灵敏度不足，难以检测细微震颤。

Method: 结合基于分割的预处理提取小鼠区域，以及震颤评分估计模块（Tremor Score Estimation Module）来高灵敏度捕捉细微震颤。

Result: 实验表明，该非侵入式方法与加速度计测量结果高度相关，能准确捕捉震颤的强度依赖性特征。

Conclusion: 使用普通RGB摄像头即可实现小鼠震颤严重程度的准确评估，为神经系统疾病震颤病理生理研究提供了非侵入性、成本效益高的新工具。
...
TL;DR: 针对密集目标集群的红外小目标检测问题，提出了一种新的连续语义引导范式，构建了首个开源生态系统CSIST-100K和GrokCSO工具包，并设计了DISTA-Net++网络，通过计数引导先验和连续坐标校正机制，在低分辨率下实现了超越高分辨率方法的高精度检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法将CSIST问题简化为盲目的离散亚像素分离，存在语义引导缺失和网格离散化限制的问题，导致误检和漏检，且定位精度受限于网格分辨率。

Method: 构建了包含CSIST-100K基准数据集、度量套件和GrokCSO工具包的开源生态；提出DISTA-Net++，包含动态深度展开主干、计数引导先验（注入全局目标计数作为语义约束）和连续坐标校正（回归网格偏移量解耦定位精度与网格分辨率）。

Result: 在3x划分下，DISTA-Net++性能超过7x划分方法：CSO-mAP提升16.15%，计数精度提升62.96%，计算量仅为后者的六分之一。

Conclusion: 证明了CSIST解混精度无需通过更细网格划分来购买，连续语义引导和连续坐标回归是解决密集小目标检测的有效范式。
...
TL;DR: 本文通过研究VLMs的OCR机制，发现了一组通用的语义注意力头，这些头可以揭示图像token的语义信息，并可用来编辑图像内容，为视觉-语言对齐提供了因果证据。


<details>
  <summary>Details</summary>
Motivation: 通过研究VLMs（视觉语言模型）如何执行光学字符识别（OCR）任务，来理解视觉-语义映射的机制

Method: 1. 研究四个VLM模型；2. 识别OCR因果必要的注意力头；3. 将这些头的注意力权重聚合成"语言化透镜"（verbalization lens）变换；4. 将隐藏状态投影到词汇空间

Result: 1. 发现这些OCR注意力头实际上是通用语义头，可解释图像token的语义内容；2. 将这些头指向"bike"图像token时输出"bike"，指向鸟翅膀时输出"feathers"；3. 该变换可揭示所有层的可解释语义特征；4. 从第0层即可获得可解释标签，证明早期层的图像表示已与语言对齐；5. 可用逆变换编辑非文字概念（如将拖拉机替换为左轮手枪）

Conclusion: 对特定机制的研究可以为更广泛的AI可解释性问题提供启示；视觉语言模型的早期层已经形成了与语言对齐的图像表示
...
TL;DR: 本文提出AVR（Anomaly-aware Video Restoration），一种无需训练的视频异常检测与修复框架，利用冻结的预训练模型实现对监控视频中异常区域的检测与修复闭合。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测与视频修复研究相互割裂：训练-free异常检测仅输出分数或标签，而训练-free视频编辑则响应用户提示而非检测器。本文旨在弥合这一差距。

Method: 1）运动证据将开放词汇提议门控至时空掩码；2）从片段计算的背景先验填充异常暴露的每个像素；3）扩散模型仅合成无任何帧显示的内容；4）冻结验证器决定使用经典、先验锚定或背景条件修复器。

Result: 在三个监控数据集上进行了广泛实验，包括全参考异常注入和真实异常，结果显示：AVR在oracle掩码下引领全帧保真度；在编辑区域内匹配三个训练视频修复器；在自生成掩码上优于检测后生成 pipeline；同时抑制残留异常和自由扩散闪烁。

Conclusion: AVR成功实现了异常感知视频修复，在无需训练的情况下利用冻结预训练模型完成检测与修复任务，在多个指标上优于现有方法。
...
TL;DR: 本研究区分了大型视觉语言模型在有害表情包分类中失败的两个原因：内部证据缺失 vs 证据路由失败。通过稀疏自编码器、角色条件探测探针、因果干预和恢复实验，在六个有害内容基准测试集上进行了验证。结果表明稀疏读出在所有六项二元分类任务上均优于原生预测：Qwen从0.432提升至0.740，Gemma从0.532提升至0.714。关键发现是路由（而非表征本身）是有害表情包分类的 recurring bottleneck。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在分类有害表情包时经常出现误分类，但失败原因不明确：是缺少内部证据，还是无法将已表示的证据路由到输出端？此前研究未能区分这两种情况，导致无法针对性地改进模型性能。

Method: 使用稀疏自编码器、角色条件探测探针、因果干预和恢复实验四种方法，在Gemma-3和Qwen3.5模型上区分上述两种失败原因。评估涵盖六个有害内容基准测试集，并包含西班牙语和印地语-英语代码混合的额外评估。

Result: 稀疏读出在所有六项原生二元任务上超越原生预测：Qwen平均0.740对比0.432原生macro-F1，残差重构达到0.486；Gemma从0.532提升至0.714。Qwen静默特征消融的探测敏感度是24-63倍，路由特征修补在是非任务上的输出敏感度是16-140倍。校准-only路由恢复了93.3%的平均差距，探测蒸馏LoRA改进原生预测但共享多任务适应导致负迁移。Gemma-3-12B在Facebook Hateful Memes的案例研究发现分布式rank-32图像-提示交互，达到0.756对比0.685原生macro-F1。

Conclusion: 性能差异反映的是监督可及性而非预存的原生决策规则，最具影响力的token角色取决于具体任务。信号超越英语范围，不能仅由配套OCR解释，依赖于配对的视觉证据。因此，路由而非表征本身是有害表情包分类中 recurring bottleneck。这一发现为改进模型提供了新方向：应关注改进路由机制而非仅增强表征能力。
...
TL;DR: 本文提出NormLift框架，从3D视角重新审视2D语义特征到3D高斯的提升问题，将每个高斯的特征分配形式化为CLIP单位球面上的余弦对齐问题，在该目标下L2归一化语义反向投影特征作为闭式解出现，并进一步将范数分解为视图内和视图间一致性，特征幅度本身可作为语义可靠性信号，通过有效多视图支持进行校准，引导模式投票细化以避免线性平均。实验表明该方法在开放词汇3D语义分割任务上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究从渲染角度 justifications 训练-free加权聚合，将高斯特征视为可线性组合的欧几里得变量来重建2D特征图，但这一观点与下游3D使用场景不匹配（每个高斯通常在余弦相似度的嵌入空间中独立查询）。因此需要从3D视角重新理解特征提升的理论基础。

Method: 1)将per-Gaussian分配问题形式化为CLIP单位球面上的余弦对齐问题；2)推导得L2归一化语义反向投影特征为闭式解；3)将特征范数分解为视图内一致性和视图间一致性；4)利用特征幅度作为语义可靠性信号，通过有效多视图支持进行校准；5)采用模式投票细化避免线性平均，保持CLIP特征有效性。

Result: NormLift作为高效、训练-free的框架，在开放词汇3D语义分割的多种评估协议下均实现了strong性能。

Conclusion: 本文从per-Gaussian语义分配的3D视角为标准提升规则提供了补充性理论解释，揭示了特征范数可作为语义可靠性信号，为训练-free方法在开放词汇3D场景理解中的应用提供了新的理论基础。
...
TL;DR: PhysVGGT是一种前馈模型，可从单张RGB图像中预测摩擦系数、肖氏硬度、杨氏模量和密度等密集物理属性图以及目标级质量。该方法将物理属性估计 formulates 为密集逐像素预测问题，采用视觉几何变换器提取几何感知token，并通过密集预测分支和全局预测分支分别预测局部物理属性和物体级质量。


<details>
  <summary>Details</summary>
Motivation: 物理属性（如摩擦、硬度、刚度、密度）决定了机器人如何抓取、操作和交互物体，但从RGB图像估计这些属性仍具挑战。现有方法通常需要对每个物体进行重建或在测试时查询视觉语言模型，导致计算开销大，限制了其应用。

Method: 提出PhysVGGT模型，核心思路是将物理属性估计 formulates 为密集逐像素预测问题。使用视觉几何变换器(VGT)从输入图像中提取几何感知token，然后通过密集预测分支估计局部物理属性，通过全局预测分支估计物体级质量。此外，还引入可扩展的伪标签生成pipeline，实现大规模弱监督训练。

Result: PhysVGGT在ABO-500数据集上达到最先进的性能，并有效泛化到分布外的NeRF2Physics数据集。推理延迟仅为0.13秒/图像，比之前的最先进方法快27倍。

Conclusion: PhysVGGT无需对每个物体进行重建和测试时优化，在保持高精度的同时大幅提升推理速度，为机器人抓取和操作中的物理属性估计提供了高效解决方案。
...
TL;DR: 本文提出了一种名为Dose-Aware Cold Diffusion (DACD)的新型低剂量CT重建框架，通过将辐射剂量建模为连续潜变量，结合图像级剂量感知感知、多尺度结构先验提取和剂量校准的步骤分配，自适应引导去噪轨迹，并在反向细化过程中引入迭代前后投影校正以确保投影域数据一致性。实验表明DACD在 Mayo-2020、Mayo-2016 和 LoDoPaB-CT 三个公开数据集上显著优于现有最先进的基于扩散和物理引导的方法，尤其在超低剂量条件下表现优异，并能对训练中未见过的连续剂量水平实现良好的泛化。


<details>
  <summary>Details</summary>
Motivation: 当前低剂量CT成像面临的主要挑战是：1）降低辐射剂量会显著降低图像质量；2）现有方法难以在连续且未知的剂量水平下实现良好泛化，导致伪影和解剖细节丢失。因此需要一种能够在不同剂量条件下保持鲁棒性能的重建方法。

Method: 提出了Dose-Aware Cold Diffusion (DACD)框架，主要创新点包括：1）将辐射剂量建模为冷扩散过程中的连续潜变量；2）集成图像级剂量感知感知模块；3）多尺度结构先验提取；4）剂量校准的步骤分配机制；5）在反向细化过程中引入迭代前后投影校正以确保投影域数据一致性。

Result: 在三个公开基准数据集（Mayo-2020、Mayo-2016、LoDoPaB-CT）上的广泛实验表明，DACD在定量准确性和视觉保真度方面均一致优于最先进的基于扩散和物理引导的方法，特别是在超低剂量条件下表现突出。

Conclusion: DACD能够实现对连续剂量水平范围的鲁棒泛化，包括训练中未见的剂量水平，为低剂量CT重建提供了一种有效的解决方案。
...
TL;DR: 本研究提出了一种结合Faster R-CNN和U-Net的双阶段深度学习框架，用于在全景X光图像中进行牙齿定位和龋齿像素级分割。该框架通过系统化转换 pipeline将大规模多边形标注数据集转换为高分辨率二值分割掩膜，实现了像素级监督学习。在3000张全景图像上训练后，该方法在龋齿检测任务上取得了优异的性能表现。


<details>
  <summary>Details</summary>
Motivation: 早期龋齿检测面临传统诊断方法的局限性，特别是后牙邻面病变的检测困难。深度学习模型在自动化龋齿检测中展现出潜力，但需要大量专家标注的训练数据，导致可扩展性受限。因此需要一种能够减少标注依赖并提高检测精度的解决方案。

Method: 研究开发了双阶段深度学习框架：第一阶段使用Faster R-CNN进行牙齿定位，第二阶段使用U-Net进行像素级龋齿分割。同时构建了系统化的转换pipeline，将大规模多边形标注数据集转换为高分辨率二值分割掩膜。模型训练使用专家验证数据集和算法处理后的标签，共涉及3000张全景X光图像。

Result: 框架取得了优异的性能表现：IoU达0.9013、Dice系数达0.9482、召回率达0.9433、精确率达0.9774。与现有方法相比，该方法展现出更高的检测精度，同时显著降低了假阳性率。

Conclusion: 双阶段框架有效解决了牙科AI应用中的数据标注瓶颈问题，展示了在提高诊断一致性和支持临床决策方面的潜力，为可扩展的自动化龋齿检测系统提供了可行方案。
...
TL;DR: KDTwin是一种针对轻量级多任务分割网络的任务感知知识蒸馏框架，通过在编码器和解码器层面进行蒸馏，实现drivable-area和lane分割任务的知识迁移，无需增加推理参数量和FLOPs。


<details>
  <summary>Details</summary>
Motivation: 多任务驾驶场景分割中，不同任务（可行驶区域和车道线）具有不同的空间特性和类别不平衡问题，传统的知识蒸馏方法难以有效处理这种任务差异性，需要设计任务自适应的蒸馏目标。

Method: 提出编码器级成对蒸馏传递空间关系知识增强学生网络共享表示；对解码器使用加权损失处理drivable-area分割，边界感知损失处理lane分割，实现任务自适应知识迁移。

Result: 在BDD100K数据集上，CNN和Transformer学生模型均获得一致性能提升，且不增加推理时参数量或FLOPs。

Conclusion: 根据任务特定特性设计蒸馏目标可有效提升自动驾驶多任务分割性能，KDTwin在轻量化和精度之间取得良好平衡。
...
TL;DR: LaSeD是一种用于手术阶段识别的标签语义自蒸馏框架，利用阶段名称作为训练时的上下文信号，通过冻结的教师模型和可学习的学生模型进行特征级蒸馏，实现仅视觉输入的推理部署，无需任何文本提示即可预测7个手术阶段。


<details>
  <summary>Details</summary>
Motivation: 现有方法将阶段注释仅视为类别ID，而视觉语言模型通常需要额外的视频-文本数据、标题或指令调整。本文提出利用阶段名称作为低成本的训练信号，在保持纯视觉推理的同时提升识别性能。

Method: LaSeD从同一预训练VLM检查点初始化冻结的教师和可学习的学生。教师接收帧、固定任务提示和真实阶段名称提示；学生接收相同帧和提示但不包含提示，仅优化视觉编码器。训练结合硬阶段token监督和缓存教师表示的特征级蒸馏。推理时移除教师和提示，学生通过约束digit-token logits预测7个Cholec80阶段，无需额外分类器头。

Result: 在Cholec80评估集上达到86.20%准确率、77.75%宏召回率、78.13%宏精确率和64.15%宏Jaccard。相比Qwen3-VL-4B纯视觉基线分别提升9.45、9.42、11.93和12.22个百分点。

Conclusion: 阶段名称为将VLM适应手术工作流分析提供了有用的低成本信号，LaSeD成功实现了训练时利用语义标签知识但推理时仅需视觉输入的有效范式。
...
TL;DR: 本文研究从独奏乐谱图像中进行乐器分类，将乐谱图像转换为音乐"词汇"序列，并将其视为文本分类任务。实验表明，通过在无标签数据上预训练语言模型，然后进行微调，可以显著提高分类器性能。


<details>
  <summary>Details</summary>
Motivation: 先前的研究主要集中在音频数据中的乐器识别，而本文选择直接从乐谱图像入手，探索一种新的乐器分类方法。

Method: 首先将乐谱图像转换为基于bootleg score表示的音乐"词汇"序列，然后将问题转化为文本分类任务。训练AWD-LSTM、GPT-2和RoBERTa模型，使用无标签乐谱数据进行预训练，再用有标签数据微调分类器，并提出两种数据增强方法。

Result: GPT-2和RoBERTa略优于AWD-LSTM；预训练使RoBERTa准确率从34.5%提升至42.9%；数据增强进一步提升RoBERTa准确率15%。

Conclusion: 通过在无标签数据上预训练语言模型并结合数据增强方法，可以显著提升从乐谱图像进行乐器分类的性能。
...
TL;DR: 本文提出了一个real-to-sim框架，能够从单目RGB视频中重建仿真的铰接物体和手-物体交互，无需RGB-D、多视角输入、先前扫描、手动指定关节或机器人演示。核心是利用密集3D点轨迹作为与具体载体无关的铰接线索：固定连杆上的点保持静止，而运动连杆上的点遵循一致的旋转或平移运动。


<details>
  <summary>Details</summary>
Motivation: 铰接物体（如门、抽屉、橱柜、笔记本电脑、烤箱和铰接容器）在日常生活中普遍存在，但由于其运动依赖于底层部件和关节，难以用单一姿态表示。现有的具身交互方法通常需要RGB-D、多视角、先前扫描或机器人演示，限制了在现实场景中的应用。

Method: 方法核心是利用密集3D点轨迹提供的与具体载体无关的铰接线索：固定连杆上的点保持近似静止，而运动连杆上的点遵循连贯的旋转或平移运动。采用模块化流程：1）利用预训练的单图像3D重建模型；2）网格分割模型；3）3D场景流模型；通过显式几何推理连接各模型预测以推断铰接关系。重建铰接资产后，将恢复的3D手部运动与物体对齐，并在MuJoCo中通过接触回放交互。

Result: 成功从随意单目RGB视频中重建了仿真就绪的铰接物体和手-物体交互，无需RGB-D、多视角输入、先前扫描、手动指定关节或机器人演示。

Conclusion: 预训练视觉模型与显式运动推理的结合可将随意的人类视频转化为适用于下游具身交互的铰接物体模型，展示了利用现成视觉模型实现从视频到仿真的端到端流程的可行性。
...
TL;DR: 提出一种自适应卷积稀疏编码框架，通过将稀疏系数作为可微变量与网络参数联合学习，实现鲁棒的视觉信号表示。该框架基于FISTA展开CSC优化，并从信息瓶颈角度控制信息保留与压缩的权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉信号需要紧凑但充分的表示以实现鲁棒的下游预测。传统卷积稀疏编码(CSC)的稀疏系数是固定手动选择的，缺乏自适应能力。

Method: 1) 使用FISTA展开CSC优化过程；2) 将稀疏系数视为可微变量，与网络参数联合学习；3) 从信息瓶颈角度，稀疏系数控制信息保留与压缩的权衡；4) 引入无标签后训练策略，为受损输入调整压缩强度。

Result: 在CIFAR和ImageNet数据集上的实验表明，该方法在干净数据识别上具有竞争力，且在不同输入扰动下显著提高了鲁棒性。

Conclusion: 所提出的自适应卷积稀疏编码框架能够有效平衡信息保留与压缩，在保持竞争力的同时大幅提升了模型对输入扰动的鲁棒性。
...
TL;DR: GPT-Policy是一个通用智能体框架,旨在实现机器人通过上下文学习(in-context learning)适应新环境,无需梯度更新即可从人类视频演示、示例和交互反馈中学习并转化为可执行的机器人行为。


<details>
  <summary>Details</summary>
Motivation: 当前机器人难以像人类一样快速适应新环境,现有策略无法实现真正的上下文学习。商业视觉语言模型(VLM)如GPT-6 Astra具备广泛的智能体能力,激发了研究能否利用这些模型实现机器人上下文学习的想法。

Method: GPT-Policy框架包含三个核心组件:1)上下文编译器(Context Compiler)保留任务相关的视觉转换;2)VLM提出机器人-工具动作建议;3)约束控制器验证并执行每个动作,同时报告其结果。

Result: 在真实机器人实验中,人类视频演示即使没有机器人动作标签也能提高任务完成率,而对齐的动作参考在接触敏感任务上带来进一步提升。

Conclusion: GPT-Position表明通过上下文学习实现机器人适应的可行性,为将VLM的通用能力转化为物理行为提供了实证基础,同时明确了实现可靠部署需要克服的挑战。
...
TL;DR: 本文提出PointZero模型，通过3D点轨迹补全作为预训练目标，在无需机器人动作标签的情况下学习可迁移的3D动力学先验。模型在包含290万帧的合成数据集上训练，下游任务中微调后表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法学习动作条件的3D动力学需要机器人动作标签，排除了网络视频数据。目标是设计一种无需动作标签的预训练目标来学习可迁移的3D动力学先验。

Method: 提出3D点轨迹补全任务：给定单帧RGB-D观测和稀疏的部分3D轨迹，预测所有观测点的未来轨迹。模型采用灵活的表达性Transformer架构(PointZero)。

Result: 在PGND 3D动力学基准上微调后优于基线；在6/7个模拟和真实机器人操作任务上优于或匹配基线。消融实验验证了预训练目标的有效性。

Conclusion: 3D点轨迹补全是一种有效的无需动作标签的预训练目标，可为下游应用提供丰富的3D动力学先验，PointZero模型结合预训练数据集能显著提升性能。
...
TL;DR: 本文提出了PanoCaps基准数据集和PANORAMA模型，用于解决全景 grounded captioning问题，即同时生成全面且像素级对齐的图像描述。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型虽能生成流畅详细的图像 caption，但难以可靠地将描述与图像像素关联。结合密集 captioning 与像素级 grounding 的方法通常产生不完整描述或不准确分割掩码。

Method: 1) 构建PanoCaps人类标注基准数据集，基于全景分割数据集，提供密集caption和实体级图像文本对齐；提出phrase-mask匹配协议和泛化全景质量(gPQ)指标。2) 将phrase grounding表述为从phrase条件掩码提议池中选择任务，引入PANORAMA模型，对预训练分割器进行上下文phrase表示条件化以获得候选掩码，并学习选择与每个phrase对应的掩码。3) 与caption生成联合训练该接口。

Result: PANORAMA在PanoCaps上实现最佳整体grounding，在多个像素级grounding任务上匹配或超越专业模型。实验表明该方法产生精确的实体级分割，同时保持详细、掩码一致的caption。

Conclusion: 通过PanoCaps基准和PANORAMA模型，成功实现了在生成详细caption的同时保持像素级掩码对齐，为全景grounded captioning提供了有效解决方案。
...
### [99] [ABM-SIRTEM: A Hybrid Agent-Based and Epidemiological Model for Pandemic Response](https://arxiv.org/abs/2609.18223)
*Sheryl Paul,Samuel Williams,Preetom K. Biswas,Giulia Pedrielli,Jyotirmoy V. Deshmukh*

Main category: cs.MA

TL;DR: 本文提出了一种混合ABM-SIRTEM模型，结合了代理建模与SIR流行病模型，用于研究COVID-19疫情期间疾病传播与社会经济行为的交互作用。该模型创新性地整合了职业类别、经济生产力、社会福利及政策遵守度动态，旨在为疫情应对政策提供更精准的分析框架。


<details>
  <summary>Details</summary>
Motivation: COVID-19大流行对全球健康、社会结构和经济造成深远影响，尤其对低收入群体和依赖人际互动的行业冲击最大。政策制定者面临在保护公众健康与维持经济稳定之间寻求平衡的挑战。现有模型存在局限性：群体层面模型忽视个体异质性，而详细代理模型计算成本高昂。

Method: 提出ABM-SIRTEM混合模型，核心创新包括：1) 纳入职业类别划分；2) 整合个体层面经济生产力与福利指标；3) 动态建模公众对政府干预政策的遵守程度。采用美国四个州的实际阳性与阴性检测数据进行模型校准。

Result: 模型成功复现四个州的疫情传播曲线，校准结果揭示了不同人群对政策遵守度的差异动态，为理解社会经济因素如何影响疫情传播提供了实证基础。

Conclusion: 该框架为研究疾病传播与社会经济行为的交互作用提供了科学基础，可用于疫情响应规划的决策支持，帮助政策制定者在公共卫生与经济稳定之间寻求最优平衡。
...
### [100] [Social Laws for Multi-agent Coordination in Stochastic Environments](https://arxiv.org/abs/2609.18929)
*Rolando Fernandez,Caleb Probine,Tyler Lee,Jeffrey Chen,Erez Karpas,Muhammad Arrasy Rahman,Peter Stone,Ufuk Topcu*

Main category: cs.MA

TL;DR: 本文将多智能体系统中社会法律的概念从确定性、目标导向环境扩展到随机奖励环境，提出α-鲁棒性度量方法，并通过将问题约简为马尔可夫决策过程来验证社会法律的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在多智能体环境中，协调智能体以防止干扰并确保稳健的个体表现是一个关键挑战。现有研究主要关注确定性、目标导向设置，而现实环境通常具有随机性，因此需要将社会法律扩展到随机奖励环境。

Method: 提出α-鲁棒性概念，衡量各智能体在遵循社会法律前提下执行最优单智能体策略时保留的保证效用；提出基于马尔可夫决策过程约简的社会法律鲁棒性验证方法。

Result: 在玩具环境上的实证评估验证了框架的可行性。

Conclusion: 成功将社会法律概念扩展到随机环境，为多智能体系统的协调提供了新的鲁棒性度量与验证方法。
...
### [101] [Making AI-Assisted Claims Independently Challengeable: Publication Authority and a Protocol for Falsifiable Publication Records](https://arxiv.org/abs/2609.17631)
*Torsten Olivi Tiltack,Yifei Dong,Kun Yu,Xu Wang,Wei Liu,Jianlong Zhou,Ren Ping Liu,Fang Chen*

Main category: cs.AI

TL;DR: 本文提出 Publication Authority 概念，作为一种精确状态、不可转让、一次性使用的出版能力，以解决 AI 辅助声明中证据、分析、授权、呈现和更正历史可能指向不同状态的问题。通过 PAC-2026 形式化规范和 SF-4 语义冻结，在 110,764 个安全可达状态上验证了系统内聚性、有界安全性、故障敏感性和有限可构造性。


<details>
  <summary>Details</summary>
Motivation: AI 辅助声明可能显得权威，但证据、分析、人工授权、呈现和更正历史可能指向不同状态。现有的 provenance、attestation 和透明度机制只能暴露历史，无法指定出版物转换过程。需要一种精确状态、不可转让、一次性的出版能力来确保出版物转换的权威性。

Method: 开发 PAC-2026 (Publication-Accountability Calculus)，实现 AIJIM 协议候选。采用第四有界语义冻结 (SF-4) 规范，包含六项义务：证据、运行与制品、测量披露、授权、表面对应和生命周期连续性。使用身份向量、对抗案例、有限模型和历史实现。10 个模型探索 110,764 个安全可达状态，76 个不安全配置产生预期违规或观察者反模型。

Result: SF-4 将证据视野与验证时间分离，拒绝因果无效的授权。历史前驱路径重现 17 个冻结授权后继结果。已知案例类的实例盲测匹配全部 183 个评分期望，同主机包执行重现 240 个归档观察。10 个模型探索了 110,764 个安全可达状态，76 个不安全配置产生预期违规。

Conclusion: 结果支持内部一致性、有界安全、故障敏感性和有限可构造性，但不支持事实真值、一般精化、盲互操作性、领域效能或标准状态。该方法在受控形式化环境中有效，但尚未在实际应用中验证。
...
### [102] [EvolveTrade: Experience-Driven Policy Refinement for Self-Evolving LLM Trading Agents](https://arxiv.org/abs/2609.17632)
*Sehee Kim,Yumin Choi,Minki Kang,Sung Ju Hwang*

Main category: cs.AI

TL;DR: EvolveTrade是一个自进化框架，将使用工具的交易智能体的系统提示视为文本参数化策略，通过策略智能体根据决策轨迹和投资组合反馈不断更新策略，使智能体能够适应不同市场环境并改进信息获取和投资组合构建方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型交易智能体虽然可以结合市场数据、新闻和可执行分析，但其行为通常由部署前固定的静态手写工具使用策略控制。这种限制使其无法在不断变化的市场环境中自适应地调整证据收集、工具调用、信号验证和风险管理方式。

Method: EvolveTrade将系统提示视为文本参数化策略。每个更新周期后，策略智能体利用累积的决策轨迹和已实现的投资组合反馈来修订策略，同时保持骨干LLM不变。更新后的策略用于下一批交易决策，使智能体能够随时间优化信息获取和投资组合构建流程。实验在多个市场环境和两个LLM骨干网上进行。

Result: EvolveTrade在大多数评估设置中实现了更高的夏普比率和累计回报率。行为分析表明，自进化策略增加了代码介导的分析并激活了与市场环境相关的计算。案例级策略归因显示了策略诱导的配置变化如何影响实际回报差异。

Conclusion: 结果表明，调整工具使用的可重用程序是构建更稳健LLM交易智能体的关键方向。自进化策略方法能够使交易智能体更好地适应变化的市场环境，提高投资表现。
...
### [103] [One Color Preprocessing Improves DSATUR](https://arxiv.org/abs/2609.17633)
*Adam Nouira,Lucas Isenmann*

Main category: cs.AI

TL;DR: SSLD通过使用类似Lovásztheta数的SDP方法预处理第一个好的颜色类来改进DSATUR算法，是首个通过固定颜色类预处理改进DSATUR的方法。在超过1600个基准实例上几乎总是匹配或优于DSATUR，但运行时间约为DSATUR的195倍。


<details>
  <summary>Details</summary>
Motivation: DSATUR是图着色问题最快的启发式算法之一，但通常比最新着色算法使用更多颜色。研究旨在通过预处理第一个颜色类来改进DSATUR的性能。

Method: 使用类似Lovásztheta数的半定规划(SDP)计算第一个好的颜色类，然后让DSATUR完成剩余图的着色。

Result: SSLD在超过1600个基准实例上几乎总是匹配或优于DSATUR，优于naive GISD基线，证明了SDP引导的第一个颜色类选择带来的价值。

Conclusion: SDP引导的第一个颜色类预处理是未来改进的方向，虽然运行时间约为DSATUR的195倍，但显著提升了着色质量。
...
### [104] [Physics-Constrained Digital Twins for Sensor Integrity in Urban Pedestrian Flow: Detecting Stealthy False Data Injection with Conformal Guarantees](https://arxiv.org/abs/2609.17635)
*Oscar Mogollon Gutierrez,Fatemeh Ghasemi,Mohammadhossein Homaei,Andres Caro,Mar Avila*

Main category: cs.AI

TL;DR: 本文研究了城市行人计数系统对 stealthy false data injection（隐性虚假数据注入）攻击的脆弱性，提出了一种基于数字孪生的估计方法，结合流量守恒定律和图局部增益来检测攻击，并通过6年墨尔本数据验证了方法的防御效果。


<details>
  <summary>Details</summary>
Motivation: 现有城市行人计数系统将传感器数据视为真实值，缺乏对数据完整性的验证能力。当传感器被攻击者篡改时，会影响经济指标、规划决策和安全运营，但此前对城市级行人传感的虚假数据注入研究不足。

Method: 1) 在行人街道图上构建数字孪生，估计有向流量；2) 通过学习的图局部增益同化计数值；3) 使用流量守恒残差训练，耦合计量和非计量路段；4) 检测结合 innovation 和残差，采用自适应保角校准设置报警阈值；5) 定义攻击边际(attack margin)衡量物理约束的防御效果。

Result: 在墨尔本6年数据上：单设备 compromise 时攻击边际为0.54；1/3设备 compromise 时降至0.19（网络仅1.18%路段有计量）。用距离图替代街道图时边际降至0.09，证明防御效果来自流量守恒定律而非局部性。

Conclusion: 基于物理约束（流量守恒）的检测方法可有效抵御虚假数据注入攻击，即使在低覆盖率（1.18%）传感网络下仍能保持显著防御能力；街道图结构比距离图更能提升防御效果。
...
### [105] [What You Can't See Is Still What You Learn: A Preregistered Sixty-Society Confirmation That Evidence Masking Drives Compositional Generalization](https://arxiv.org/abs/2609.17637)
*Narcis Marincat*

Main category: cs.AI

TL;DR: 研究测试了限制模块读取能力是否能改善系统学习效果。在64个细胞系统中进行预注册验证，这些系统共享冻结的语言模型主干并通过学习的连续数据包通信。五种条件变化包括证据屏蔽、所有权标记和用中性填充物替换外来证据。结果显示屏蔽机制在保留的两操作和三操作组合上分别提高了0.846和0.859的中位数配对差异，所有12对都通过了要求的边界。


<details>
  <summary>Details</summary>
Motivation: 研究动机是测试限制模块读取内容是否能够改善系统学习计算的能力。这是关于信息限制对学习系统影响的探索性研究。

Method: 使用64个细胞系统进行实验，共享冻结的语言模型主干，通过学习的连续数据包通信。五种条件变化包括：证据屏蔽、所有权标记、用中性填充物替换外来证据。实验跨越6个初始化聚类，每个聚类2个数据顺序，在1个新任务世界上进行。

Result: 屏蔽机制在保留的两操作和三操作组合上准确率提高，中位数配对差异分别为0.846和0.859；所有12对都通过了要求的边界，预注册行为标准通过；无标记复制也通过；填充条件产生7个完全泛化者；18个审计的屏蔽系统中数据包干预遵循预测的中间值变化。

Conclusion: 结果证实了所测试屏蔽机制的显著优势，但更精细的归因和泛化性仍待解决。可用角色信息的影响尚未解决，填充条件的分解标准不确定。
...
### [106] [CapMem: A Benchmark for Caption-Based Episodic Memory in Egocentric Video](https://arxiv.org/abs/2609.17688)
*Dingli Liang,Yiqiao Xie,Yukai Huang,Zhaokai Wang,Weitong Cai,Guangwen Feng,Jifei Song,Zhensong Zhang,Hang Zhang*

Main category: cs.AI

TL;DR: 研究可穿戴设备在自我中心视频中的情景记忆问题，提出使用文本描述作为可重用的情景记忆。在长视频(>20分钟)任务中，基于描述的CaptionQA方法优于直接VideoQA方法。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型面临帧预算有限、视觉token成本高昂、长上下文检索失败等问题。可穿戴设备需要在自我中心视频上进行情景记忆推理，但现有方法在实际约束下表现不佳。

Method: 定义了Episodic Memory Video Caption QA任务，构建了CapMem基准数据集(75个视频，总计33.7小时，1000个多选题，16种场景)；设计了caption-guided retrieve-and-verify框架

Result: 在长视频(>20分钟)上，使用30s和60s描述窗口的完整覆盖CaptionQA分别在10/12和8/12模型上优于直接VideoQA；六种Qwen模型的匹配帧控制实验保持3.22和2.55点的平均准确率增益；retrieve-and-verify框架进一步提升最高5.3点准确率

Conclusion: 描述记忆可有效支持长自我中心视频的情景推理，为可穿戴设备的 episodic memory 应用提供了可行方案
...
### [107] [GraphEcho: Structural Redundancy and Evidence Provenance in LLM Graph Agents](https://arxiv.org/abs/2609.17695)
*Sikun Wang,Yixi Zhou,Lei Fan,Fan Zhang*

Main category: cs.AI

TL;DR: GraphEcho是一个基准测试，用于评估大型语言模型代理是否会错误地将重复的图路径视为额外的证据支持。该基准测试通过改变路径数量和证据来源同时保持证据内容不变来测试这一点，并评估模型的判断和主动探索能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有图推理评估中的关键问题：代理可能会将通过不同图路径重复遇到相同证据误认为额外的佐证，而不是独立的证据来源。这导致代理在图上探索效率与有效利用证据之间存在差距。

Method: 1) 构建可变路径数量和证据来源的受控合成实验；2) 评估代理的判断和主动探索行为；3) 提出溯源感知后训练(PAPT)方法；4) 在科学声明上进行测试。

Result: 1) 合成实验显示模型依赖的判断偏移；2) 冗余支持路径增加所有评估的冻结代理的重复遍历份额；3) PAPT减少重复访问并提高合成准确率，但覆盖更少的不同来源；4) 在科学声明上持续减少重复但准确率下降。

Conclusion: 研究揭示了高效探索与有效证据利用之间的差距：代理可以学会停止重复自己，同时忽略所需信息。GraphEcho提供了评估图代理结论及其探索是否到达不同证据来源的受控方法。
...
### [108] [ERPBench: A State-Grounded Evaluation Paradigm for Computer-Use Agents in Enterprise Software](https://arxiv.org/abs/2609.17885)
*Kratika Bhagtani,Kusha Sridhar,Maziyar Baran Pouyan,Yuying Zhao,Eugene Siow*

Main category: cs.AI

TL;DR: ERPBench是一个评估截图操作代理在真实ERP系统上表现的基准测试，发现即便代理能保存表单，正确数据写入率仍可能低至3%。


<details>
  <summary>Details</summary>
Motivation: 现有计算机代理评估基准局限于通用桌面和Web任务，而企业资源规划(ERP)系统具有独特挑战：密集界面、协调的多步骤交互以及错误会持久化到业务记录中而非显示在屏幕上。现有企业基准依赖专有平台或模拟软件。

Method: 开发ERPBench基准测试，在实时可复现的ERP系统上评估截图代理，根据数据库中的真实值评分。还提供生产级工具链，在人类审批门控下安全部署代理。

Result: 评估6个闭源和开源代理表明，强大的通用GUI性能不能转移到企业可靠性。部分代理保存成功率达85%，但正确值写入率低至3%。

Conclusion: 通用GUI代理在企业工作流中存在特定失败模式，需要专门针对企业场景的优化。
...
### [109] [GVD: Governed Versioning and Deduplication for Document Repositories](https://arxiv.org/abs/2609.17696)
*Mohammadreza Sediqin,Shivali Dalmia,Sumukha Thoppanahalli,Abhishek Mukherji*

Main category: cs.AI

TL;DR: GVD是一个统一跨文档版本链接与规则级冲突解决的框架，在可审计的更新策略下处理文档仓库中的版本管理、重复检测和矛盾检测问题。


<details>
  <summary>Details</summary>
Motivation: 文档仓库持续演进，指南和政策不断修订、更替和重新上传，同一内容以不同措辞重复出现，新版本可能 refine 或矛盾先前版本。这些不一致性属于不断增长的集合而非任何单一文档，但现有工作将版本控制、重复检测和矛盾检测视为孤立的成对任务。

Method: 通过双向规则对齐将输入文档分配到版本家族，并将规则与家族记忆进行比较以识别重复、矛盾、不对称 refine 和新知识；使用反事实跨度探测(CSP)解决被推理误判为中性的相关对；关系特定策略抑制重复并仅升级重大变更供审查，保留版本 lineage 作为审计追踪；管道完全本地运行，无需大型语言模型。

Result: 在120个企业文档（140次摄入，59个版本家族）上，GVD在版本家族构建上达到F1=0.94，在规则级一致性上达到F1=0.94，CSP将规则一致性从0.90提升到0.94。

Conclusion: GVD框架成功统一了版本控制和重复检测，在无需大型语言模型的情况下实现了高精度运行。
...
### [110] [Where Should Agents Live? Energy-Memory Characterization of Agentic AI for the Edge-Cloud Continuum](https://arxiv.org/abs/2609.18283)
*Carolina Fortuna,Vid Hanžel,Tim Strnad,Blaž Bertalanič*

Main category: cs.AI

TL;DR: 针对5G-Advanced和6G网络中代理式AI工作流的能耗评估问题，提出agentic-eCAL指标，将单模型能耗度量扩展到多代理工作流场景。研究发现跨代理文本传输仅占工作流能耗的0.25%，分布式部署的主要能耗来自推理和上下文处理而非数据传输。


<details>
  <summary>Details</summary>
Motivation: 现有AI生命周期能耗指标仅评估单模型推理或完全忽略多代理执行图，导致网络运营商无法确定分布式代理通信的能耗成本，也不知道代理团队应该部署在边缘还是云端。

Method: 将封闭形式的双速率单次调用能耗模型（计算密集的prefill和解码阶段的decode）与7层OSI数据传输模型相结合，基于数百个GPU基准配置（NVIDIA A100和H100）、16个开放权重模型和8种编排拓扑进行验证。

Result: 研究表明，在5G RAN、城域和光链路中，代理间文本传输仅占工作流能耗的0.25%。边缘云代理部署中，分布式的主要能耗并非来自代理间文本传输，而是通信带来的额外推理和上下文处理。

Conclusion: agentic-eCAL指标可有效评估多代理工作流能耗，为网络运营商在边缘云连续体中选择代理物理部署位置提供理论依据。
...
### [111] [NeMo Data Designer: An Extensible Framework for Multimodal Synthetic Data Generation](https://arxiv.org/abs/2609.17699)
...
TL;DR: NeMo Data Designer (NDD)是一个开源的通用多模态合成数据生成框架,提供声明式配置格式,支持文本、代码、结构化输出、图像、嵌入和统计采样器等多种列类型,并具有灵活的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 合成数据生成(SDG)本身是一个迭代过程,需要直观易用的工具来定义数据集列、支持多种数据类型、确保工作流可共享和可复现,并能够在生成过程中预览和修订规范。

Method: NDD通过声明式配置格式定义数据集列,支持多种列类型(文本、代码、图像、嵌入、统计采样器等),采用插件系统实现可扩展性,内置预览-修订循环以支持迭代式生成,并在运行时处理依赖解析、调用调度和失败重试。

Result: 展示了涵盖结构化、代理、多模态和领域专业化任务的案例研究,包括Nemotron模型开发和生产企业部署中使用的数据集。

Conclusion: NDD作为一个开源通用框架,能够有效支持多模态合成数据生成,其声明式配置、插件系统和迭代式工作流为用户提供了灵活、可复现的SDG解决方案。
...
### [112] [Compositional Policy Violations: When Step-Level Compliance Fails In Agentic AI Workflows](https://arxiv.org/abs/2609.18820)
*Ashwini Kurady,Sri Sai Charith Grandhi,Rajesh Gupta,Sumit Mamoria*

Main category: cs.AI

TL;DR: 本文定义了AI智能体工作流中的"组合策略违规"(CPV)问题：当每个单独步骤都通过检查时，但整个执行流程却违反了组织政策。现有步骤级监控无法检测这类问题，需要基于完整执行轨迹的溯源感知运行时架构来修复。


<details>
  <summary>Details</summary>
Motivation: 当前AI智能体治理采用步骤级监控（输入输出分类器、每轮约束、跨度级评估器），但组织实际持有的政策（如转介阈值、权限限制、审查要求）是整个执行的属性而非单个步骤的属性，这种不匹配导致组合策略违规这一失败模式无法被检测。

Method: 1)定义CPV为步骤级合规组合失败；2)提出四类CPV分类：权限蠕变、阈值清洗、累积和违规、上下文崩溃；3)设计溯源感知运行时架构，对完整执行轨迹进行政策评估，从原始溯源重新计算受保护量而非使用管道派生的表示。

Result: 论文定义了4种CPV类型，并证明每类CPV的正确修复取决于受保护量在何处发生变异。溯源感知架构可检测传统步骤级监控无法捕获的策略违规。

Conclusion: 组合策略违规是AI智能体治理中的重要但未被识别的失败模式，需要从步骤级监控转向基于完整执行轨迹的溯源感知政策评估，以正确执行组织级策略。
...
### [113] [A Systematic Evaluation of the COTQ Provincial Land Cover Product: Structural Consistency, Spectral Separability, and Relative Positioning Against ESA, ESRI, and Google Products](https://arxiv.org/abs/2609.17731)
*Étienne Clabaut,Samuel Foucher,Yacine Bouroubi*

Main category: cs.AI

TL;DR: 本研究系统评估了魁北克省10米土地覆盖产品COTQ与三种全球土地覆盖数据集(ESA WorldCover、ESRI LandCover、Google DynamicWorld)的一致性和特征差异。研究通过结构指标、光谱分离度和争议区域照片解译等方法，在跨越温带针叶林到北方苔原的8个 Sentinel-2 瓦片上进行了综合评估。


<details>
  <summary>Details</summary>
Motivation: 魁北克地区生态梯度复杂、地表条件异质性高，现有全球高分辨率土地覆盖产品(如Sentinel-2衍生物)在区域应用中存在性能差异，亟需开发省级10米土地覆盖产品(COTQ)以支持年度土地占用和土壤人工化监测。

Method: 研究采用多层次评估方法：1)结构指标(对象尺寸分布、形状复杂性、调整兰德指数、IoU)；2)基于Sentinel-2反射率数据的光谱分离度指标；3)争议区域的针对性照片解译。所有产品在同一图例体系下进行标准化比较。

Result: COTQ在结构和光谱特征上与ESA WorldCover最为相似，但与全球产品存在系统性差异，主要体现在城市用地、湿地、岩石或隐花植物表面等类别的定义和专题优先级的不同。

Conclusion: 多标准评估为COTQ产品提供了客观特征描述，阐明了其在魁北克 operational 土地监测中与现有全球土地覆盖数据集的相对定位，为区域土地管理提供了可靠的数据支撑依据。
...
### [114] [Flag Game: A Toy Model for Mechanistic Swarm Interpretability](https://arxiv.org/abs/2609.19124)
*Elizabeth Pavlova,Hidenori Tanaka*

Main category: cs.AI

TL;DR: 本文引入Flag Game作为研究AI智能体集体信念形成机制的玩具模型，通过该模型揭示了集体信念坍塌到极化转变的现象，并提出社会电路归因和统计机械理论两种分析方法来理解 emergent coordinated behaviors。


<details>
  <summary>Details</summary>
Motivation: AI智能体的紧急协调行为开始呈现关键安全风险，而快速形成和传播关于世界的信念是驱动这些行为的关键现象，亟需机制性理解来实现集体对齐。

Method: Flag Game模型：隐藏的国旗定义真相，每个受限智能体直接观察私人图像片段，可与同伴交换信念并权衡社会证据。采用社会电路归因技术预测哪些智能体和观点对集体动态影响最大，并通过因果干预验证；同时发展了统计机械理论来分析大规模群体。

Result: 发现丰富集体现象：性能随人口规模非单调缩放、社会意识提示和团队多样性带来准确性提升、组织结构强影响。小群体出现集体信念坍塌，大群体转变为集体信念极化，导致大群体性能下降但创造信念多样性。社会电路归因在大型人口中效果减弱。

Conclusion: 这是迈向机械性群体可解释性的第一步，探索个体智能体属性及其通信如何产生紧急集体行为，为理解和引导AI系统的集体对齐提供了理论基础。
...
### [115] [Imitation Learning for Autonomous Driving in CARLA](https://arxiv.org/abs/2609.17757)
*Jordy Kieto*

Main category: cs.AI

TL;DR: 本文研究了行为克隆在CARLA模拟器中的闭环驾驶能力，使用包含RGB图像、LiDAR、车辆遥感和车道路点的多模态策略，能够在没有碰撞的情况下自主驾驶数小时，并可迁移到 unseen 的城镇。


<details>
  <summary>Details</summary>
Motivation: 行为克隆在离线专家演示上训练策略，但部署时是闭环的：每个动作会影响策略下一步接收的观察。本文研究紧凑的多模态策略能从离线演示中获得多少闭环驾驶能力。

Method: 策略使用五帧历史数据的RGB图像、LiDAR、车辆遥感和车道路点来预测20Hz的油门、刹车和转向。演示数据分三个阶段收集，最后使用系统化路线生成程序。训练了136万参数的策略，使用236,882个窗口（约3.3小时驾驶数据，448次采集）。

Result: 生成的策略能在训练和保留路线上自主驾驶数小时。在运行中无碰撞发生，并可定性转移到具有不同道路几何结构的未见CARLA城镇。观察到从大轨迹偏差中恢复的能力。

Conclusion: 紧凑的多模态策略通过行为克隆可以获得显著的闭环驾驶能力，包括对未见环境的泛化和轨迹恢复能力。发布了代码、训练检查点、ONNX模型、数据样本和证据审计。
...
### [116] [Cognitive Extensions for Dual-Process Language Agents: Memory and Self-Reflection in Interactive Environments](https://arxiv.org/abs/2609.19128)
*João Meneses dos Santos,Arlindo L. Oliveira*

Main category: cs.AI

TL;DR: 本文提出了一种针对交互式环境的语言智能体SwiftSage的扩展，通过两个模块化认知扩展（自适应记忆模块AMM和自我反思模块SRM）来提高智能体在长程任务中的鲁棒性。在ScienceWorld基准上的实验表明，完整系统取得了最佳性能（平均得分64.62，成功率43.17%，成功步骤效率19.33步）。


<details>
  <summary>Details</summary>
Motivation: 当前语言智能体在交互式环境中仍然脆弱，成功执行任务需要长程状态跟踪、有效的动作执行以及从失败步骤中恢复。这些挑战促使研究者探索结合快速动作提议器和慢速规划器的双进程架构，并进一步引入认知扩展来增强智能体的能力。

Method: 研究在SwiftSage双进程智能体基础上引入两个模块化认知扩展：1）自适应记忆模块(AMM)用于显著性门控的情节存储和触发式检索；2）自我反思模块(SRM)用于有界执行时间验证和纠正干预。两个模块在同一执行 substrate 上实现为特征标志扩展，支持在ScienceWorld上进行受控消融实验。

Result: 四种配置（基线、基线+AMM、基线+SRM、完整系统）的实验结果显示：完整系统达到最佳平均最终得分64.62，成功率43.17%，成功步骤效率19.33步。SRM是最强的独立贡献者。

Conclusion: 研究结论表明执行时间控制是当前设置中的主要瓶颈，而情节记忆在运行时循环稳定后最为有用。这为未来语言智能体的设计提供了重要参考：优先解决执行时控制问题，再考虑记忆增强。
...
### [117] [SAGE: Governed Artifact Generation from Enterprise Guidelines](https://arxiv.org/abs/2609.17775)
*Mohammadreza Sediqin,Shivali Dalmia,Sumukha Thoppanahalli,Srinivasa Karthikeya Reddy Kovvuri,Abhishek Mukherji*

Main category: cs.AI

TL;DR: SAGE是一个治理型多阶段LLM管道，用于将企业指南文档（包含叙事文本、复杂表格和嵌入图像）转换为结构化工作制品。该系统通过版本化规则存储、模式验证的阶段间契约和端到端 provenance 追踪，实现提取规则的确定性结构验证和基于LLM的语义评分，一致性模块去除重复、标记矛盾并暴露规范差距。在120份文档上，SAGE将处理时间从数天缩短至20-100分钟，达到96%的文档级成功率，幻觉率仅3.2%（无治理时为15.7%），提取3,896条规则并生成812件待人工审核的制品。


<details>
  <summary>Details</summary>
Motivation: 企业指南文档混合了叙事文本、复杂表格和嵌入图像，将其转换为结构化工作制品仍需2-3天的人工努力。当前语言和视觉语言模型虽然能从此类文档中提取信息，但缺乏提取之外的治理工作流：没有验证、没有一致性检查、没有可追踪的制品生成。

Method: SAGE采用治理型多阶段LLM管道架构，围绕共享版本化规则存储（具有稳定标识符）、模式验证的阶段间契约和端到端provenance追踪进行组织。提取的规则经过确定性结构验证和基于LLM的语义评分，然后通过一致性模块去除重复、标记矛盾并暴露规范差距；只有不确定或标记的项目才提交审核人，高置信度输出则自动批准。

Result: 在120份文档上，SAGE将周转时间从数天缩短至20-100分钟，实现96%的文档级成功率和3.2%的幻觉率（无治理时上升至15.7%），提取3,896条规则并生成812件待人工审核的制品。

Conclusion: SAGE通过引入治理工作流显著提升了企业文档提取的效率和质量，将处理时间缩短至原来的几百分之一，同时大幅降低幻觉率并实现自动化的高置信度输出批准，证明了对LLM提取结果进行治理的必要性。
...
### [118] [FairCompressAgent: An Agentic Framework for Fairness-Aware Model Compression for FPGA Deployment](https://arxiv.org/abs/2609.17786)
*Yuanbo Guo,Yiyu Shi*

Main category: cs.AI

TL;DR: 提出FairCompressAgent(FCA)框架,用于公平性感知模型压缩,通过智能体框架集成剪枝、量化和稀疏低秩分解,使用语言模型规划器选择压缩配置,在Fitzpatrick-17k数据集上使用VGG-11验证了方法有效性


<details>
  <summary>Details</summary>
Motivation: 公平性感知模型压缩需要选择平衡准确性、公平性和部署成本的方法和配置,当压缩方法组合或用户需求变化时,这些决策变得更加困难

Method: 提出FCA智能体框架,集成公平性感知剪枝、增量量化和稀疏低秩 factorization,通过公共算子接口连接。语言模型规划器利用模型配置和测量结果选择压缩配置,执行层执行压缩、微调、评估和基于约束的选择

Result: 在Fitzpatrick-17k数据集上使用VGG-11进行实验,比较40种测量配置下的四种搜索方法。在准确性约束请求下,FCA选择了推理张量存储减少59.54%的压缩模型,验证平均精度从0.5141提高到0.5233,equalized opportunity从0.2251降低到0.2168。与一次性规划相比,FCA平均使用7.33次 vs 12次候选评估

Conclusion: 实验结果表明,测量反馈和显式约束支持公平性感知压缩配置的选择和交互式 refinement,框架支持需求更新并能报告无法满足请求时的剩余违规情况
...
### [119] [A Four-Stage Decomposition of Word-Problem Solving and Mechanistic Fragility in LLM Math Reasoning](https://arxiv.org/abs/2609.17804)
*Zhongdi Qu,Carla P. Gomes*

Main category: cs.AI

TL;DR: 大语言模型在解数学应用题时准确率很高，但插入一个无关 clause 就能使其失败。本文通过机制分析揭示了模型内部计算包含四个顺序阶段：模式抽象、操作规划、操作数绑定和计算，每个阶段在特定层区间产生不同的中间表示。通过诊断分心因素导致的失败，研究将故障定位到操作规划阶段，并通过注意力头的双向因果验证确认了其作用。简言之，本文提供了LLM数学应用题推理的机制解释及其分心失败的原因。


<details>
  <summary>Details</summary>
Motivation:  reconciling the observation that LLMs can solve grade-school math word problems with high accuracy yet fail completely when a single irrelevant clause is inserted into the problem

Method: 通过机制分析识别模型内部计算的四阶段顺序管道（模式抽象、操作规划、操作数绑定、计算），使用支架诊断将故障定位到特定阶段，并通过注意力头的双向因果验证

Result: 发现模型内部计算分解为四个阶段的顺序管道，每个阶段在可识别的层带中产生不同的中间表示；将分心诱导的失败定位到操作规划阶段，由一组注意力头实现

Conclusion: 提供了LLM数学应用题推理的机制解释，以及它们在分心时失败的原因；通过双向验证确认了操作规划阶段中特定注意力头的因果作用
...
### [120] [Learning Heterogeneous Preferences](https://arxiv.org/abs/2609.17847)
*Shiwali Mohan,Matt Hong,Dule Shu,Aniek Fransen,Shabnam Hakimi,Matt Klenk*

Main category: cs.AI

TL;DR: 本文提出了一种针对主观领域个性化偏好的学习框架，通过引入"个体化效用函数"来捕捉不同个体的偏好异质性，并设计了新的多阶段架构进行估计，在汽车轮毂设计偏好数据集上验证了个体化模型优于通用模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设存在跨人群的通用效用函数，将标注者分歧视为随机噪声。但这一假设在主观领域失效，因为偏好会因人而异且系统性地变化。现有方法无法捕捉有意义的偏好异质性。

Method: 基于理性选择理论，提出条件化个体效用函数（考虑个体和决策情境），设计多阶段架构从多模态数据中估计个体化效用函数。收集了2,398名参与者的575,000对汽车轮毂设计审美判断数据。

Result: 个体化效用模型显著优于通用效用模型（包括基础模型基线）。实验表明分歧反映的是有意义的偏好异质性而非标注噪声。

Conclusion: 研究强调收集标注者属性和学习个体化效用函数的重要性，使奖励模型能够明确代表谁的偏好，并忠实地捕捉人类决策多样性。这为更个性化、更准确的人类反馈学习提供了新方向。
...
### [121] [SNOMED CT Concept Recommendation from Masked Clinical Context](https://arxiv.org/abs/2609.17855)
*Ali Noori*

Main category: cs.AI

TL;DR: 提出了一个基于SNOMED CT实体链接挑战的掩码概念推荐基准测试，使用MIMIC-IV-Note数据集，包含75,491个注释，比较多种方法后发现稀疏TF-IDF表现最佳。


<details>
  <summary>Details</summary>
Motivation: 临床语言标准化到SNOMED CT支持互操作性、分析和可重用表型，但当相关概念在训练数据中稀有或缺失时，概念推荐仍然困难。

Method: 使用75,491个注释（272份出院总结，204份训练，68份历史测试），对每个唯一的笔记-概念对，从本地临床上下文中掩码目标提及，系统对训练中观察到的SNOMED CT概念进行排名。比较了流行度基准、稀疏TF-IDF概念原型、密集潜在语义分析嵌入、稀疏-密集融合、检索笔记证据和检索增强混合方法。

Result: Sparse TF-IDF表现最佳，Recall@1为14.81%，Recall@10为33.43%，MRR为0.2114，nDCG@10为0.2297。检索增强未改善基准，Recall@10为31.99%，MRR为0.1937。性能受概念频率强烈影响：仅出现在1-2个训练笔记中的概念Recall@10为7.74%，而出现在超过10个笔记中的概念为43.90%。此外，9.66%的测试笔记-概念对包含训练中未见的概念。

Conclusion: 本地词汇上下文和术语覆盖是低资源环境下推荐质量的主要决定因素，为未来基于本体和生物医学编码器的检索系统提供了可复现的基准。
...
### [122] [The Inference Engineering Pareto Atlas: Which Optimizations Dominate the Cost, Quality, and Latency Frontier?](https://arxiv.org/abs/2609.17863)
*Srikanta Datta Tumkur,Jay Iyer,Mehar Simhadri,Sai Pavan Kumar,Sai Kapil Kumar,Ramesh Nampelly*

Main category: cs.AI

TL;DR: 该研究构建了一个成本、质量和延迟的Pareto图谱，用于识别不同部署约束下的最佳配置。通过在L4、A100和H100 GPU上测试Qwen2.5-7B-Instruct的54种配置，发现组合方法比单方法更常达到Pareto前沿。FP8权重在保持99.4%准确率的同时将延迟降低至0.61-0.65倍，而AWQ 4bit虽然延迟降至0.34倍但损失5.9%准确率。FP8 KV缓存虽然吞吐正常但无法正确回答任何问题。


<details>
  <summary>Details</summary>
Motivation: LLM推理优化在不同模型、GPU、提示和质量指标上报告的加速效果难以比较或组合，缺乏系统性的评估框架

Method: 在L4、A100和H100 GPU上测试Qwen2.5-7B-Instruct的54种vLLM 0.12配置，使用校准模拟器扩展测试范围，对200个GSM8K问题进行质量评估

Result: 36种配置中18种达到Pareto前沿；组合方法(9/15)比单方法(9/21)更常达到前沿。FP8权重保持99.4%准确率且延迟为0.61-0.65倍，AWQ 4bit延迟为0.34倍但损失5.9%准确率，FP8 KV缓存质量为0，n-gram推测解码无益处(0.90-0.98倍)

Conclusion: 最佳选择取决于约束条件和GPU：H100适合严格延迟场景，A100适合高吞吐和低成本场景(每百万token 0.106美元)。仅关注速度不够，需同时考虑质量指标。
...
### [123] [Do Frontier Models Seek Safety Evidence Before Acting?](https://arxiv.org/abs/2609.17865)
*Omer Tafveez*

Main category: cs.AI

TL;DR: 研究前沿模型在采取行动前是否主动获取安全相关证据，而非仅在已知安全信息的情况下做出反应。通过构建SAFE基准测试，改变证据的检索成本、概率、严重性和呈现方式，评估GPT-5.5、o3、Claude Opus 4.8和Claude Sonnet 4.6的证据获取策略。


<details>
  <summary>Details</summary>
Motivation: 当前AI安全研究主要关注模型在已知安全信息情况下的响应，但忽略了模型在采取行动前是否主动获取安全证据这一更早的决策点。

Method: 构建SAFE受控基准测试：模型进行部署决策时可选择获取可选证据，证据特征包括检索成本、问题概率、严重性和呈现方式变化。通过成本-义务分解和反事实干预分析行为与解释之间的差异。

Result: 1)不同模型有明显的证据获取策略差异：Opus几乎默认检查，o3最倾向跳过且对阈值敏感，GPT-5.5和Sonnet处于中间状态。2)检查行为随严重性增加而增强，随检索成本增加而下降。3)概率对行为影响较弱（10%到70%仅改变最多21个百分点）。4)所有模型的第一阶段理由都以预期价值推理为主。5)规避行为主要由检索摩擦和部署收益威胁驱动，而非由知情后的补救责任驱动。6)行为与解释存在不匹配：证据框架在检查边界附近强烈影响决策但很少被提及，而概率虽常被引用但因果影响很小。

Conclusion: 部署时的AI安全不仅取决于模型如何应对已知风险，还取决于模型是否主动获取"行动是否安全"所需的证据。这揭示了当前模型在证据获取决策上的显著差异和潜在安全隐患。
...
### [124] [OBC-Prune: Outcome-Based Calibration for Large Reasoning Model Pruning](https://arxiv.org/abs/2609.17890)
*Ha Lan Nguyen,Huy Hoang Tran,Trac-Duy Tran,Dung D. Le*

Main category: cs.AI

TL;DR: OBC-Prune是一种针对大型推理模型剪枝的新方法，通过干预式分析识别因果重要的推理句子，并将其转化为每token权重来重新校准剪枝过程中的激活值，在DeepSeek-R1-Distill-Qwen 1.5B/7B/14B模型上40%和50%稀疏度下，在MATH500、LiveCodeBench和AIME 2025上取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在回答问题前生成长思维链，产生显著的推理开销。现有剪枝方法使用模型自身rollout进行校准，但统一对待所有推理token，未区分其对正确推理的贡献，导致错误计算模式被保留，推理质量下降，准确率降低且推理轨迹变长。

Method: OBC-Prune首先从模型回答不一致的问题中构建难度匹配的正确答案和错误答案配对，然后通过干预式分析估计每个推理句子的因果重要性，量化移除其影响后对后续预测的影响。这些因果重要性分数被转化为每token权重，用于重新调整一次式剪枝方法（SparseGPT、Wanda、ALPS）的校准激活值。

Result: 在DeepSeek-R1-Distill-Qwen 1.5B、7B和14B模型上，以40%和50%稀疏度在MATH500、LiveCodeBench和AIME 2025上进行了实验，结果表明OBC-Prune在大多数模型规模和稀疏度级别上均优于最先进的校准基线方法。

Conclusion: 保留因果重要的推理回路是比统一保留观测激活更有效的剪枝目标，这表明因果重要性分析对于优化大型推理模型的效率-质量权衡具有重要价值。
...
### [125] [Collaborative Memory for Multi-Agent VLM Systems](https://arxiv.org/abs/2609.17921)
*Huixin Zhang,Shao-Jun Xia,Di Wang,Liangxi Liu,Hainan Xiong,Zihao Wang*

Main category: cs.AI

TL;DR: 本文提出了一种针对多智能体视觉语言模型(VLM)协作的共享视觉记忆框架,旨在解决分布式感知中的上下文协调问题。


<details>
  <summary>Details</summary>
Motivation: 在多智能体VLM系统中,不同智能体检查不同的图像区域、视频帧或视觉表示,协作延伸至分布式感知,共享视觉上下文成为核心问题。智能体需要基于其他智能体的贡献进行推理、恢复缺失的视觉上下文,并在新证据出现时协调不同的解释。

Method: 围绕协调解释和更新依赖推理的需求,构建记忆层次结构、跨智能体共享和一致性机制。共享视觉记忆不仅保存图像或文本摘要,还保存观察、智能体解释与后续推理之间的依赖关系。

Result: 该框架为构建可靠且资源高效的多智能体团队提供了基础,有效支持信息在智能体之间的流动与演化。

Conclusion: 提出的框架通过管理视觉上下文、解释依赖和协调机制,使多智能体VLM系统能够实现可靠的分布式视觉推理。)
...
### [126] [Measuring AI Leadership: Development and Validation of a Multidimensional Measure for AI-Native Organizations](https://arxiv.org/abs/2609.17965)
*Mustafa Akben,Leslie Coyne*

Main category: cs.AI

TL;DR: 开发AI领导力电池（AI Leadership Battery），用于测量AI赋能工作中的领导行为，包含36个行为特异性子维度归纳为11个理论指定内容家族。


<details>
  <summary>Details</summary>
Motivation: AI正在改变领导者需要判断、学习、协调的内容，但现有测量工具无法捕捉AI赋能工作中研究领导力所需的行为水平。

Method: 遵循既定量表开发程序：演绎式条目生成；定义对应性和定义独特性的内容验证；探索性因子分析和条目减少；独立样本的验证性因子分析；内部一致性信度、收敛效度、判别效度、校标关联效度和增量效度测试。

Result: 提供电池内容、多维结构、信度、与轨道构念区分的证据；在组织增长、决策速度、客户/利益相关者响应能力、AI赋能团队绩效、AI赋能工作体验、AI安全与风险管理、AI采用与整合等方面贡献了额外信息。

Conclusion: 为研究者提供行为框架，用于检验AI赋能工作中领导者如何调节判断、学习、适应、透明度和问责制。
...
### [127] [Memory Has Geometry: Non-Uniform Geometric Memory for Long-Horizon Personalized AI](https://arxiv.org/abs/2609.17969)
*Jiahong Liu,Wenhao Yu,Zexuan Qiu,Menglin Yang,Irwin King*

Main category: cs.AI

TL;DR: 本文提出将长期记忆建模为用户特定的动态状态空间，而非传统的静态潜在空间中的离散记录，以实现更好的个性化AI。


<details>
  <summary>Details</summary>
Motivation: 解决当前个性化系统中的核心问题：数据挖掘中证据是时间事件流，而主导抽象是可搜索记录集，两者存在根本性的不匹配。

Method: 将记忆建模为具有局部异构几何的动态状态空间；引入轨迹条件重建而非仅最近邻查找来访问记忆。

Result: 提出一种新范式：记忆访问成为相关用户状态的轨迹条件重建，包含稳定vs易变区域、变速漂移、异构邻域和对当前用户状态的不确定性建模。

Conclusion: 长期个性化应从静态记录查找转向动态轨迹建模，记忆访问需要结合交互、反馈和时间因素进行状态重建。
...
### [128] [When to Call an LLM: A Confidence-Gated Hybrid for Cost-Effective Emotion Recognition in Conversational AI](https://arxiv.org/abs/2609.17977)
...
### [129] [Contiguity, Not Importance: Budgeted Repair of Stale KV Caches After Document Edits](https://arxiv.org/abs/2609.17983)
*Mingyang Mao,Wyatt Mackey,Xiaomin Lin*

Main category: cs.AI

TL;DR: 本文研究检索增强生成(RAG)和代理系统中KV缓存失效后的修复问题。当检索知识、工作内存或用户状态被编辑时，缓存上下文会变得过时。论文提出将原地修复形式化为预算重计算，比较了多种无需训练的位置选择策略在事实性RAG基准测试上的效果。


<details>
  <summary>Details</summary>
Motivation: 在RAG和代理系统中，KV缓存复用可降低推理成本，但缓存的上下文可能因检索知识、工作内存或用户状态被编辑而变得过时。在因果自注意力下，即使局部编辑也会影响下游KV状态。完整重预填充虽能可靠恢复一致性但成本高昂，而仅刷新编辑跨度会遗留下游依赖项的陈旧问题。

Method: 将原地修复形式化为预算重计算问题，在事实性RAG基准上比较了多种无需训练的位置选择策略，涵盖三种模型家族，区分了直接编辑和派生编辑两种场景。

Result: 所有策略都能修复直接编辑案例，但派生编辑案例明显区分了各策略效果。在主要预算设置下，连续的编辑局部窗口恢复了至少0.94的后编辑答案边际，显著优于基于注意力、KV偏差和结构选择器。机械分析表明，在清洁状态迁移下有效的位置集在实际重计算中可能失败，因为分散的位置继承了周围陈旧性。

Conclusion: 由于答案相关编辑几乎总是破坏模型行为，失败严重程度难以预测，且修复速度比完整重预填充快13-21倍，研究结果支持在依赖文本仍与编辑相邻时进行无条件编辑局部修复。
...
### [130] [TuiML: Machine Learning for AI Agents](https://arxiv.org/abs/2609.17984)
*Nilesh Verma,Nick Lim,Albert Bifet,Bernhard Pfahringer*

Main category: cs.AI

TL;DR: TuiML是一个专为AI智能体设计的机器学习库，通过机器可读元数据和参数模式实现库的自发现、自验证和可组合性，解决现有库（如Weka、scikit-learn）对智能体不友好的问题。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习库（如Weka、scikit-learn）原本为人类程序员设计，但语言模型智能体使用这些库时存在诸多问题：隐藏库功能、延迟错误到运行时、丢失实验状态等。需要一个专为智能体设计的ML库。

Method: 设计TuiML库，包含监督学习、无监督学习、时间序列、数据处理、调参和评估等任务。每个组件提供机器可读元数据和参数模式，使智能体可以搜索、检查、组合工作流并注册新组件。所有调用经过验证、播种和追踪，会话可导出为可运行笔记本确保可重复性。

Result: 基准测试显示TuiML与scikit-learn和Weka保持预测竞争力。TuiML既像传统库一样对人类用户友好，又为智能体提供优先的读取、扩展和操作能力。

Conclusion: TuiML是首个专为AI智能体设计的开源机器学习库，通过自描述元数据和验证机制使智能体能够自主进行机器学习，为智能体时代的ML基础设施奠定基础。
...
### [131] [RideWay: Benchmarking Efficient Task Completion for Tool-Using Language Agents](https://arxiv.org/abs/2609.17985)
*Qingnuan Han,Boli Fang,Mingzhi Hou,Claire Liu*

Main category: cs.AI

TL;DR: 本文提出RideWay基准和Efficiency Utility指标，用于评估交互式服务中AI代理的效率，不仅关注任务完成，还考虑对话轮次和工具调用的冗余程度。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理评估仅关注任务是否完成，但在交互式服务场景中，即使成功完成任务，代理可能因重复提问、冗余搜索或不必要的修正而惹恼用户，需要引入效率评估维度。

Method: 提出Efficiency Utility指标：成功的轨迹会根据超额工具调用和用户对话轮次进行折扣惩罚；通过人类配对偏好校准相对惩罚权重，反映服务-工作流权衡。

Result: 超额对话轮次的惩罚约为超额工具调用的两倍；在任务无关的保留偏好数据上，Efficiency Utility达到78.7%整体准确率；当轨迹在对话轮次上有差异时准确率为90.6%，仅在工具调用上有差异时准确率仅为随机水平。

Conclusion: RideWay使交互效率可与任务成功一起被衡量，同时揭示了基于计数的工具使用评估的局限性——人类标注者在工具调用数量上的一致性最低。
...
### [132] [Multimodal Conditioning of Fine-Tuned Stable Diffusion XL for Controllable and Culturally Faithful Ulos Motif Generation](https://arxiv.org/abs/2609.17987)
*Humasak Simanjuntak,Tamara Yunika Sianipar,Bronson T. M Siallagan,Difya Laurensya Ambarita,Arlinta Barus*

Main category: cs.AI

TL;DR: 本文提出了一种多模态生成框架，将微调的潜在扩散模型(Stable Diffusion XL v1.0通过LoRA)与多模态大语言模型(LLaMA 1.5-7B)相结合，用于生成可控制的、文化传承的巴塔克Ulos织物图案。该框架支持文本到图像和图像到图像生成，为文化遗产保护提供了实用的数字设计工具。


<details>
  <summary>Details</summary>
Motivation: 传统巴塔克Ulos编织行业在生产多样化和创新图案方面面临挑战，传统手动设计方法存在局限性，需要一种能够生成多样化、创新性且保持文化真实性的图案生成方法。

Method: 框架整合了四种互补的条件机制：文本、图像、表示和语义图(通过ControlNet)，每种机制分别控制从语义意图到空间布局的不同方面。采用LLaMA 1.5-7B多模态大语言模型引导生成过程，并通过LoRA微调Stable Diffusion XL v1.0。

Result: 消融实验显示：文本+图像+语义图组合获得最佳FID(270)和CLIP Score(0.65-0.70)，但SSIM最弱(0.65)；文本+图像+表示组合获得最佳整体平衡，SSIM稳定(0.84)且FID竞争力(280)；四种机制组合反而获得最差FID(330)，表明存在冲突的优化信号。9名织工和30名公众参与者的定性评估确认了统计学上显著的正向接受度(Wilcoxon检验，p=0.007和p<0.001)。

Conclusion: 该多模态生成框架能够实现可控制的、文化传承的Ulos图案生成，条件机制并非越多越好，文本+图像+表示的组合提供了最佳平衡。Web原型系统验证了其在文化遗产保护中的实用价值。
...
### [133] [Missing Bridges: Composition-Aware Active Imitation Learning](https://arxiv.org/abs/2609.18004)
*Maxwell J. Jacobson,Ahmed H Qureshi,Yexiang Xue*

Main category: cs.AI

TL;DR: 本文提出AALT（通过潜在拓扑的自适应智能体），解决主动模仿学习中如何选择最有价值的专家演示问题。核心思想是选择能最大化起始-目标连通性增益的演示，这在形式上与任务可达成性的信息增益相关。在72个任务的UR5e机器人排序检索模拟环境中，AALT仅用3个演示（5个过渡）就从42/72提升到100%任务成功率，而最强基线在20个演示后仅达88.6%。


<details>
  <summary>Details</summary>
Motivation: 现有主动模仿学习方法通常基于对专家策略的信息增益来选择演示请求，但在结构化多任务域中，起始-目标任务数量呈组合增长，尽管其解决方案共享可复用行为。单一演示可能同时帮助解决许多任务，但现有方法未明确考虑这种价值。

Method: AALT将现有演示组织成由学习行为连接的潜在中心状态拓扑，识别高价值桥接演示（可能一次性启用许多任务），并将其转化为专家查询。推理时，通过结果拓扑进行规划，并在每个连续的中心过渡上调节扩散策略。

Result: 在72个任务的UR5e机器人排序检索模拟环境中，AALT仅用3个演示（共5个过渡）就从未初始化数据集的42/72成功率提升到72/72（100%）任务成功。经过20个演示后，最强基线平均成功率为88.6%，使用了98个过渡。

Conclusion: 通过显式建模演示的组合价值（任务可达成性增益），AALT能够在极少的演示情况下实现完美任务完成，显著优于基于信息增益的传统方法。这表明在多任务主动学习中，考虑任务的连通性结构比单纯考虑策略信息增益更有效。
...
### [134] [Anchoring What Matters: A Dual-Level Learning Framework for Visually-Grounded Multimodal Reasoning](https://arxiv.org/abs/2609.18057)
*Xinxin Song,Siyuan Li,Tingxiong Xiao,Jinli Suo*

Main category: cs.AI

TL;DR: PIVOT是一种双层学习框架，通过自校准经验回放机制和视觉引导的优势分配机制来增强大型视觉语言模型的视觉推理能力，解决标准强化学习中视觉推理轨迹被丢弃和优势分配均匀的问题。


<details>
  <summary>Details</summary>
Motivation: 标准在线策略强化学习存在优化瓶颈：宝贵的视觉推理轨迹在一次更新后被丢弃，且均匀的token优势分配无法强化关键的感知或推理步骤。

Method: 1) 自校准经验回放机制：选择性收集和回放视觉接地历史经验作为策略优化的稳定参考锚点；2) 视觉引导优势分配机制：根据token的局部视觉支持和下游推理影响分配额外的视觉感知优势。

Result: 在多个基准测试中实现了高度竞争力的性能，显著提升了LVLMs的多模态推理能力。

Conclusion: PIVOT框架能够有效解决RLVR中视觉推理信号丢失的问题，通过双层学习机制提升模型的视觉推理表现。
...
### [135] [The Other Half of the Memory Wall: Serving 35B MoEs from SSD with Trained Routing Prediction](https://arxiv.org/abs/2609.18063)
*Yu Lin,Yiming Wang,Runyuan Cai,Hanze Liu,Xiaodong Zeng*

Main category: cs.AI

TL;DR: Edge0是一个面向消费级硬件的MoE推理引擎，通过预测性预路由(prerouter)和恢复LoRA技术，在24GB显存机器上以20tok/s的速度运行35B MoE模型，峰值显存仅3GiB。


<details>
  <summary>Details</summary>
Motivation: 消费级硬件上的MoE推理受限于权重内存，35B类模型在4-bit下需19.5GB。稀疏性只能减少每token的计算量，但不能减少需加载的参数量。传统的SSD卸载无法解决问题，因为层N+1的专家必须在层N输出前选择，导致读取无法与计算隐藏。

Method: 1. 提出prerouter：每层一个头部预测下一层的路由，在当前层计算时预读下一层专家，实现路由与专家加载的重叠。2. 训练一个未合并的恢复LoRA（在学生路径上训练），补偿int4量化和路由替换带来的质量损失。

Result: 在24GB机器上，Edge0以20tok/s的速度运行35B MoE，峰值活跃内存仅3GiB，在5个公开基准测试上与fp16教师模型平均差距在几分以内。8B版本也可在同一框架上运行。

Conclusion: Edge0成功实现了在消费级硬件上高效运行大型MoE模型，框架、检查点和适配器已开源。
...
### [136] [Teaching AI, Robotics, & Community: A Hubs-Based K-12 Education Framework for Reaching Rural Schools](https://arxiv.org/abs/2609.18072)
*Maxwell J. Jacobson,Gustavo Rodriguez-Rivera,Petros Drineas,Yexiang Xue*

Main category: cs.AI

TL;DR: ARC是一个基于 hubs 的教育框架，通过大学培训本科生导师并为周边K-12团队举办工作坊来解决农村地区机器人与AI教育难以扩展的问题。试点项目在印第安纳州一所大学创建了三个农村机器人团队，K-12学生和本科导师在多项指标上均有显著提升。空间马尔可夫模型模拟显示，40年后该框架可覆盖印第安纳州74%的K-12学校，产生992个机器人项目。 


<details>
  <summary>Details</summary>
Motivation: K-12机器人和人工智能教育难以扩展，特别是缺乏持续技术指导的农村地区。现有项目如FIRST提供竞赛途径和教学机会，但无法消除当地编程和机器人专业知识的需求。 

Method: 1) 试点部署：在一所大学创建三个农村机器人团队，通过问卷调查（Likert五点量表）评估效果；2) 空间马尔可夫模型：使用印第安纳州数据进行40年模拟预测。 

Result: K-12学生编程知识增加2.00分、资源获取增加2.25分、练习机会增加1.25分；本科导师教学信心增加1.29分、解释适应能力增加1.14分、小组管理能力增加1.00分、指导乐趣和意义感增加1.14分。模拟显示：40年后ARC可覆盖74%的学校（1925所），产生992个机器人项目，而自然增长仅产生161个。 

Conclusion: ARC框架能够创建和支持农村机器人项目，培训本科生AI和机器人导师，并有可能在整个地区扩展指导服务，创造自我强化的教育循环。 
...
### [137] [Decodability is Not Causality: Dissociating Probe Readouts from Behavioral Drivers via SAE Decomposition](https://arxiv.org/abs/2609.18080)
*Devesh Tiwari,Camille Davis,Shivank Sinha,Talia Weaver,Aditya Shah,Maheep Chaudhary*

Main category: cs.AI

TL;DR: 本文指出线性探针(linear probe)虽然能从语言模型激活中解码概念(如真实性),但探针精度只能证明特征可解码性,不能说明探针权重对应的特征因果驱动模型行为。通过对Gemma2-9B-Instruct模型上的真实探针进行实验,发现探针对齐排名与梯度敏感度排名仅12%重叠(Spearman rho=0.10)。 ablation实验表明,共享特征组的输出翻转率(27%)远高于探针独有特征组(6%)和随机特征组(1%)。因此,探针权重向量的几何投影无法识别模型因果使用的特征,需结合激活统计信息并使用相干门控SAE干预才能分离因果特征。


<details>
  <summary>Details</summary>
Motivation: 解决线性探针的准确性与因果相关性之间的差距问题。当前研究表明探针可以解码安全相关概念,但探针权重在几何上对齐的特征不一定是模型实际使用的特征,因此需要干预来验证因果相关性。

Method: 1.将部署的True/False探针分解为稀疏自编码器(SAE)特征;2.按探针对齐度和梯度敏感度对特征进行排名;3.在相干门控下对共享特征、探针独有特征和随机特征集合进行ablation实验;4.在Gemma2-9B-Instruct模型上使用TTPD探针和Long等人(2025)的指令真实性/欺骗设置进行测试。

Result: 1.两种排名方法仅弱重叠(约12%,Spearman rho=0.10);2.共享特征组在完整相干条件下使输出翻转率达27%,远超探针独有特征组(6%)和随机特征组(1%);3.探针独有特征反而主要干扰探针自身的读出;4.基于激活感知的特征选择使行为翻转率接近探针几何顶部特征的3倍(17.6% vs 6.1%);5.结果在5个随机种子和保留数据分割上保持一致。

Conclusion: 探针权重向量的几何投影无法识别模型因果使用的特征。但将探针信息与特征激活统计相结合可显著恢复更多行为因果特征,且需要相干门控SAE干预来分离因果特征与探针读出。这为理解语言模型内部表征的因果结构提供了新方法。
...
### [138] [When Is Graph Structure Worth Its Cost? The Case for Structure Pricing in Retrieval-Augmented Generation](https://arxiv.org/abs/2609.18099)
*Yuzhong Zhang,Haoyang Ma,Chao Peng,Lionel Briand,Boxi Yu,Jialun Cao*

Main category: cs.AI

TL;DR: EffiRAG是一种基于图的检索增强生成系统，旨在降低图构建成本的同时保持答案质量。在UltraDomain数据集（包含4个领域的120个开放性问题）上，EffiRAG在93个问题上优于LightRAG-hybrid，仅在7个问题上落后，20个问题平局。同时将系统总成本降低57%（从0.952美元降至0.408美元）。


<details>
  <summary>Details</summary>
Motivation: 基于图的RAG系统虽然能帮助回答需要多文档信息的问答，但构建图需要大量语言模型调用，导致成本高昂。因此需要评估其质量提升是否值得额外成本，旨在寻找一种更具成本效益的方案。

Method: 设计EffiRAG系统，利用图结构定位相关段落，并从原始文本生成答案。这种设计在保持图构建和查询处理轻量级的同时保留来源信息。在10和20文档/领域场景下，使用轻量级非LLM过滤器跳过低显著性文本块。

Result: 在UltraDomain的120个问题上，EffiRAG在93个问题上被优先选择，LightRAG-hybrid在7个问题上被优先选择，20个问题平局。系统总成本降低57%（从0.952美元降至0.408美元）。在10文档/领域时，成本降低4.2倍；在20文档/领域时，成本降低4.5倍。

Conclusion: 基于图的RAG系统应同时评估答案质量和成本。实验结果支持能定位和保留源证据的图结构设计。
...
### [139] [AutoTuneBench: Trustworthy Measurement for Agent Auto-Tuning of LLM Serving Engines](https://arxiv.org/abs/2609.18123)
*Li Chen*

Main category: cs.AI

TL;DR: 本文揭示了大语言模型代理在调优GPU内核时的测量信任危机，提出AutoTuneBench基准和测量协议来解决这个问题。研究发现诚实测量会显著改变之前的加速比结论。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理通过提议-测量-保留的闭环调优GPU内核和Serving引擎，但测量结果不可信。需要识别失败模式并建立可信赖的测量协议。

Method: 通过四天619次模型调用的试点语料库识别四种失败模式，提出AutoTuneBench基准和测量协议，包括：代码冻结的协议、数据库级验证器、抗作弊检查、预注册读数、外部结果锚定和配对种子统计（5%变异系数上限）。

Result: 诚实测量改变了结论：最佳内核从10.6x降至2.03x；同一配置在一台机器上为1.174x，在另一台上为1.0049x；预注册的on/off比较在共享墙上无差异（2.4840 vs 2.4957ms）；KernelBench Level-1套件中51%任务的中位数加速仅为1.0001x。

Conclusion: AutoTuneBench协议、vLLM和SGLang双引擎语料库及审计追踪作为开放 artifacts 发布，使测量信任成为架构特性。
...
### [140] [Designing Agentic AI Workflow Portfolios under Imperfect Selection and Compute Cost](https://arxiv.org/abs/2609.18126)
*Mojtaba Abdolmaleki,Stefanus Jasin,Boyu Wang*

Main category: cs.AI

TL;DR: 研究Agentic AI系统中多工作流组合选择问题，提出Portfolio-and-Selector范式，通过联合优化运行数量和工作流分配提升最终答案选择准确率，并设计基于对偶和定价预言机的算法求解。


<details>
  <summary>Details</summary>
Motivation: 单一最佳工作流并非最优部署策略，因为不同工作流在不同实例上可能成功。实际部署中需权衡计算成本与答案准确率，且额外执行可能带来正确答案但也会引入干扰选项。

Method: 1. 将问题形式化为工作流组合问题，联合选择运行数量和工作流分配；2. 提出odds-lift index衡量选择器质量；3. 对有限工作流池开发精确 formulations、线性规划松弛、随机舍入和性能证书；4. 对大规模隐式工作流类，提出基于定价预言机的椭球算法求解对偶问题。

Result: 在ABCD、Schema-Guided Dialogue和HotpotQA三个数据集上，相较于最佳单一工作流，组合优化分别提升3.1、7.5和0.9个百分点；对偶引导的工作流生成在ABCD上额外提升3.5点，HotpotQA上提升24.1点。

Conclusion: Portfolio-and-Selector范式能有效提升Agentic AI系统的答案选择准确率，定价预言机方法可在弱条件下获得近似最优解，为实际部署提供了有效的优化框架。
...
### [141] [Symbolic Temporal Supervision of LLM Agents Using Contracts](https://arxiv.org/abs/2609.18128)
*Yifeng Xiao,Pierluigi Nuzzo*

Main category: cs.AI

TL;DR: ContrAgent是一个基于契约的LLM代理符号时序监督框架，通过线性时序逻辑(LTLf)和确定有限自动机(DFA)实现对代理行为的在线门控和离线评估，解决现有护栏方法的随机性和局限性。


<details>
  <summary>Details</summary>
Motivation: LLM代理在执行多步骤任务时存在幻觉、分布不稳定性和对抗性操纵等问题，且某些工具调用具有不可逆后果可能导致有害结果。现有防护措施要么使用随机LLM法官事后评估轨迹，要么逐个阻止不安全操作，缺乏同时支持门控和评估两种角色的确定性工件。

Method: 将代理行为形式化为可检查谓词的轨迹，使用线性时序逻辑有限迹(LTLf)中的保证-假设契约指定所需行为，将每个契约编译为确定有限自动机(DFA)，该DFA同时承担在线门控代理动作和离线评估记录轨迹的双重角色。维护独立于代理模型的契约库，可在同一任务域内跨不同代理应用。

Result: 在四个基准测试中，ContrAgent的性能与最先进的LLM法官和基于规则的护栏基线相当，同时产生确定性、可重复的判断，且在线模式下每调用延迟降低数个数量级。

Conclusion: 该框架能有效同时支持在线门控和离线评估两种角色，提供确定性监督，在在线模式下具有极低延迟优势，为LLM代理的安全监管提供了新的技术途径。
...
### [142] [Time-Aligned Evolving Concept Graphs for Scientific Relation Forecasting](https://arxiv.org/abs/2609.18163)
*Fred Sun,Jingze Wang,Minkun Xu,Shangqi Guo*

Main category: cs.AI

TL;DR: 提出时间对齐的进化概念图框架，通过将 dated papers 作为共享更新事件，联合建模语义和结构演化，以预测科学关系的首次共现、关系形成和条件关系类型。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将概念语义和图结构分开建模，或在粗粒度历史快照上总结语义，导致语义表示与快速演化的图证据可能不对齐，无法有效识别新兴的科学联系。

Method: 核心思想是将带日期的论文视为共享更新事件，通过每个预测时间的同一出版历史重构语义和结构状态。配对级融合结合这些状态来预测首次共现、关系形成和条件关系类型。

Result: 在由187,848篇论文、270,687个概念和745万条共现链接组成的图上，完整框架将平均关系AUROC从最强基线的0.9290提升到0.9722，人口加权平均AUPRC为0.005778。刷新上下文相比冻结上下文使平均关系AUPRC提高16.6%。

Conclusion: 该框架通过时间对齐的语义和结构联合建模，有效提升了科学关系预测性能，证明了上下文随图更新同步刷新的重要性。
...
### [143] [WFM: Wiki Foundation Model for Complex Agentic Reasoning](https://arxiv.org/abs/2609.18182)
...
TL;DR: WFM（Wiki Foundation Model）是一种面向可扩展、agent原生表示与检索的新型范式，旨在解决传统稀疏图编码难以参数化密集语义以及现有图编码器带来的分布式系统开销问题。


<details>
  <summary>Details</summary>
Motivation: Real-world agents需要持久化的非参数知识用于动态推理，即长期记忆和检索增强生成。传统稀疏图结构虽能提供结构化证据，但存在机器可读性差和语义密度不足的问题，难以满足复杂agent工作流程的需求。行业正在从传统稀疏图向LLM Wiki（一种agent原生的知识表示方式）转变。

Method: 提出三个核心贡献：(1) Wiki Graph模式：无缝桥接细粒度结构与密集上下文，保持显式拓扑结构与连续语义；(2) 查询条件注意力聚合：为丰富的wiki消息传递设计，包含显式注意力方差正则化；(3) 基础设施NCCL边界交换协议：提升静态分区索引并利用固定形状的GPU到GPU集合通信，绕过CPU序列化与内存复制开销。

Result: 在5个长期agent记忆和多跳推理基准测试中展现卓越性能，同时在分布式集群上实现10.5倍训练加速。

Conclusion: WFM成功实现了面向agent原生的可扩展表示与检索，为LLM Wiki知识表示提供了有效的解决方案，在性能和效率方面均取得显著突破。
...
### [144] [Re2A: Situated Conversational Recommendation via Rubric-based Preference Reasoning and Alignment](https://arxiv.org/abs/2609.18249)
*Dongding Lin,Jian Wang,Xiaoyan Zhao,Wenjie Li*

Main category: cs.AI

TL;DR: 本文提出Re2A框架，将情境对话推荐(SCR)任务建模为结构化的推理-对齐过程，通过基于规则的偏好推理和偏好条件优化来解决现有方法难以同时处理对话历史、场景信息和物品属性的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界的推荐场景通常发生在共享的物理环境中，用户与推荐系统交互时需要考虑共现场景和场景内物品属性。现有方法难以准确理解情境用户偏好并生成同时满足用户需求和情境一致性的响应。

Method: 提出Re2A框架，包含两个核心组件：1)基于规则的偏好推理(rubric-based preference reasoning)，使用自动规则引导模型产生明确的偏好状态；2)偏好条件优化(preference-conditioned optimization)，根据偏好状态对齐响应生成，兼顾用户偏好满足和情境一致性两个目标。

Result: 在两个SCR数据集上的广泛实验表明，Re2A一致性地超越最先进方法，能够提供更精确、更具上下文感知的情对话推荐。

Conclusion: Re2A框架通过结构化的推理-对齐过程有效解决了情境对话推荐中的挑战，为推荐系统在实际物理环境中的应用提供了新的解决方案。
...
### [145] [REPAIR: Resolving Long-Tail Confusion in Scientific Retrievers via Fact-Verified Iterative Refinement](https://arxiv.org/abs/2609.18262)
*Yerim Oh,Gunhee Kim*
...
### [146] [BENCHCOMPASS: From Scores to Signals for Training and Harness Decisions in Payment-Domain LLMs](https://arxiv.org/abs/2609.18270)
*Sijie Dong,Wei Ren,Xuanwei Hu,Jiawei Luo,Zifan Wang,Xiaoyun Feng,Hui Cai,Lyuxin Xue,Peng Lu,Jianshe Li,Xin Zhang,Wei Wu*

Main category: cs.AI

TL;DR: BENCHCOMPASS是一个针对支付领域的LLM基准测试，解决了现有基准无法区分模型失败原因（支付规则知识缺失、证据利用不当、输入脆弱性）的问题，包含专家审核的Pro基准和Normal测试池。


<details>
  <summary>Details</summary>
Motivation: 支付运营是关键金融基础设施，但LLM在该领域的作用尚不明确，因为支付规则变化快、证据分散、决策依赖交易状态、参与者角色、地区和支付轨道。现有的基准测试无法隔离失败原因是来自缺失的支付规则知识、证据使用不当还是输入不完善时的脆弱性。

Method: 构建流程包括：从类型化证据包构建场景接地任务、应用LLM质量检查、创建任务输入攻击变体、保留最终项目准入由领域专家审核。包含专家审核的Pro基准测试，涵盖支付知识、上下文接地场景推理和Attacked Open鲁棒性，以及用于检查和未来策划的较低置信度Normal池。

Result: 在16个模型变体上测试显示不同失败模式：缺失参数化支付知识、对 supplied 规则推理不完整、无法拒绝看似合理但无效的工作流。最佳前沿模型在Open Context-Grounded Reasoning上达到89.6%，在受攻击输入下达到81.7%；具有代表性的32B开放权重模型分别达到69.8%和42.6%。

Conclusion: 基准测试仍未饱和：最佳前沿模型在开放上下文推理和攻击输入下的表现存在差距，表明支付领域LLM评估仍有提升空间。数据和代码已开源。
...
### [147] [Who Audits Whom, on What Substrate, with What Evidence? An Independence-Graded Audit Protocol for Agentic AI](https://arxiv.org/abs/2609.18272)
*Mohamed Chahine Ghanem*

Main category: cs.AI

TL;DR: 本文提出将AI审计中的"独立性"从二元概念改为三轴分级评分体系，包括主体独立性、基础模型独立性和证据独立性，并通过最弱链路原则聚合，同时将此框架应用于AI作为审计者的场景。论文还提供了形式化模型、协议、案例分析和蒙特卡洛研究。


<details>
  <summary>Details</summary>
Motivation: 当前AI审计中，独立性的概念仍被二元化对待（独立/不独立），但这种方法无法适应AI作为审计者这一新兴场景，无法有效评估审计质量。

Method: 1)提出三轴独立性分级框架（主体、基础模型、证据）；2)移植可靠性工程中的beta因子模型；3)开发七步验证协议；4)进行采购控制代理的结构可检测性分析；5)蒙特卡洛模拟；6)与现有监管框架（EU AI Act、ISO/IEC 42006、英国公共部门指南）对标

Result: 传统内部审计仅能发现5.9%的潜在故障，且在半數故障类别中完全无法检测；三级审计框架可系统评估AI审计者的独立性水平。

Conclusion: 独立性应在三轴上分级评估，聚合采用最弱链路原则，AI审计者应适用相同标准。该框架与现行监管要求相兼容。
...
### [148] [Building Trust in Artificial Intelligence: A Necessity for Railway Applications](https://arxiv.org/abs/2609.18278)
...
TL;DR: 本文探讨了如何通过鲁棒性、运营设计域(ODD)和可解释性三个关键领域来增加人工智能在铁路行业安全关键应用中的信任度和合规性。


<details>
  <summary>Details</summary>
Motivation: 由于铁路行业对安全关键应用的严格标准和法规限制，当前人工智能仅应用于非安全关键领域。文章旨在解决如何提升数据科学和AI算法的可信度，以满足监管要求并推动AI在铁路行业的更广泛应用。

Method: 本文采用综述方法，审查了三个主要领域：1)鲁棒性 - AI系统在各种情况下维持性能的能力(ISO24029)；2)ODD - 根据DIN DKE SPEC 99004明确定义系统运行条件；3)可解释性 - AI系统以人类可理解的方式表达影响结果的重要因素。

Result: 研究表明，这三个研究领域已被非铁路行业充分探索，已有现成的算法和方法可应用于铁路领域。系统视角对于确保所有可信要求在安全的MLOps环境中持续交互至关重要。

Conclusion: 通过在AI中建立深层信任（符合全球监管框架要求），不仅能保障安全关键应用，还将释放AI的全部潜力，加速其在任务关键型领域的采用进程，促进监管机构、运营商和公众的接受度。
...
### [149] [What Counts as Strategic Reasoning? A Systematic Mapping of Chess Research on Humans, Engines, and Language Models](https://arxiv.org/abs/2609.18286)
*Paolo Ciancarini,Remo Pareschi*

Main category: cs.AI

TL;DR: 本文是一项系统性映射研究，梳理了国际象棋作为研究搜索、决策和人工智能的模型领域的最新研究，涵盖人类选手、经典棋类引擎、神经强化学习系统、大语言模型及混合方法等84个核心研究家族。 研究揭示了当前文献集中在情境评估、评价和动作选择等领域，而显式规划、可解释性、元认知和人机协作等方面仍缺乏探索。 特别发现LLM研究强调状态表征和泛化，而混合方法则更关注基础化解释。 研究最后提出了该领域的未来方向，包括显式规划、值得信赖的解释、元认知校准和人机互补性。


<details>
  <summary>Details</summary>
Motivation: 国际象棋长期以来是研究搜索、专业知识、决策和人工智能的模型领域。大型语言模型(LLM)的出现重新凸显了国际象棋作为研究战略推理和控制环境的价值，以比较人类和人工智能的决策过程。

Method: 系统性映射研究(systematic mapping study)，对涉及人类玩家、经典棋类引擎、神经强化学习系统、LLM和混合方法的最新研究进行系统梳理，最终纳入84个核心研究家族，按智能体类型、战略推理阶段和评估维度进行分类。

Result: 文献高度集中于情境评估、评估和动作选择；显式规划、可解释性、元认知和人机协作仍探索不足。LLM研究强调状态表征和泛化；混合方法更频繁出现基础化解释。两点区分：混合系统差异在于异构能力组合的时机和位置；改善人类表现的评估不等于建立人机协同。

Conclusion: 国际象棋可作为连接认知与计算视角的战略推理研究的有用桥梁。提出四个未来研究方向：显式规划、基础化且可信的解释、元认知校准和人机互补性。
...
### [150] [Visual Compliance via Executable Safety Rule Entailment](https://arxiv.org/abs/2609.18328)
*Jisoo Kim,TaeYoon Kwack,Jinwoo Jang,Honguk Woo*

Main category: cs.AI

TL;DR: GuardEn是一个可执行的安全防护框架,通过将安全策略分解为原子命题并建模为可执行代码,结合场景图提供的视觉上下文信息,实现对复杂视觉安全问题的可解释推理。该方法在SafetyVisionBench上比最强基线提升了9.8个F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练端到端安全防护系统在适应性和可解释推理方面面临挑战,特别是随着风险模式不断演变和安全规则日益复杂,传统方法难以应对上下文和语义安全问题的推理需求。

Method: 提出两个核心模块:1) Safety-Rule Compilation(安全规则编译):将安全策略分解为原子命题,并将其组合建模为可执行代码;2) Scene-Grounded Execution(场景接地执行):在测试时利用场景图提取的上下文视觉信息实例化原子命题,实现基于规则的可解释安全推理。

Result: 在SafetyVisionBench上的实验表明,该方法在复杂视觉安全评估任务中取得了显著效果,比最强基线平均提升了9.8个F1分数。

Conclusion: GuardEn框架通过可编程防护机制有效解决了复杂视觉安全评估问题,为多模态大模型安全系统提供了可解释、可扩展的新思路,具有重要的实际应用价值。
...
### [151] [Faithful yet Collusive: Why Chain-of-Thought Monitoring Cannot Detect Collusion in LLM Pricing Agents under Oligopolistic Competition](https://arxiv.org/abs/2609.18346)
*Dohun Lee,Hyunwoo Park*

Main category: cs.AI

TL;DR: 研究大型语言模型作为自主定价代理在伯川德竞争中的串通行为，开发了因果图发散框架来分别测量结构忠诚度和意图忠诚度，发现思维链监控无法单独作为防止算法串通的保障措施。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为自主定价代理可能通过默示协调维持超额竞争价格，需要理解其串通机制并建立有效的监管方法。

Method: 开发因果图发散框架，在双寡头和三寡头条件下对九个LLM进行测试，分别测量结构忠诚度和意图忠诚度。

Result: 串通行为与思维链忠诚度在两个维度上分离：最具串通性的模型准确报告合作意图但推理结构不忠诚；最具结构忠诚度的模型在两种市场结构下都维持超额纳什定价。

Conclusion: 思维链监控不能单独作为防止算法串通的保障措施，需要结合其他监管方法。
...
### [152] [Market Signal Injection: Adversarial Context Manipulation of LLM Pricing Agents](https://arxiv.org/abs/2609.18357)
*Dohun Lee,Hyunwoo Park*

Main category: cs.AI

TL;DR: 研究揭示大语言模型(LLM)定价代理会受数据呈现方式影响，即使数值本身不变。通过市场信号注入(MSI)攻击——操纵数值格式、竞争对手排序或定性市场评论——可导致模型做出不同定价决策，且攻击效果会传播到其他公司并影响利润和消费者剩余。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为LLM只会响应显式指令，但本文发现数据呈现方式本身就是一个攻击面。研究者好奇即使数值不变，格式、排序、评论等"软信息"是否会影响LLM定价决策。

Method: 在模拟的伯川德双寡头和三元寡头市场中测试9个开源模型和3个专有模型。使用情感攻击、格式操纵、竞争对手排序变化等MSI技术，并对比中性文本对照和基于规则的代理。使用输入规范化和决策边界锚定进行防御测试。

Result: 情感攻击产生最大行为转移，且会传播到其他公司并改变利润和消费者剩余。不同模型家族易感性不同，更大模型并不一致更稳健。输入规范化可移除情感攻击，决策边界锚定提供部分缓解。激活分析显示基线与受攻击状态可区分(AUC 0.93-1.00)。

Conclusion: 数据呈现方式是LLM定价代理的重要攻击面，需要开发考虑代理间交互的防御机制。输入规范化是有效防御手段，但自适应攻击下仍需更强大防御。
...
### [153] [Bad Genius: Counterfactual-Guided Harness Evolution Beyond Task-Specific Shortcuts](https://arxiv.org/abs/2609.18366)
*Guojun Zhu,Xunheng Huang,Peng Yin,Jiahui Xie,Sanguo Zhang,Doudou Zhou*

Main category: cs.AI

TL;DR: 本文提出了CHASE方法，用于解决自动评估优化导致的基准作弊问题。该方法通过在保持任务语义的前提下，搜索和演化评估协议的反事实变体，来消除基准捷径（shortcut），从而实现更可靠的代理评估。


<details>
  <summary>Details</summary>
Motivation: 可靠的代理评估被自动线束优化（harness optimization）所复杂化。现有的任务保留（task holdout）方法只改变语义任务但保持基准协议不变，导致"坏天才"提议者可以产生利用基准-wide捷径的作弊线束。

Method: 提出了Counterfactual Harness Search and Evolution (CHASE)方法，将线束演化表述为保持有效性的基准反事实的约束生成。每当Proposer更新后，Challenger搜索具有大增益破坏的可执行协议转换。有效性防火墙检查任务语义是否被保留，确认集决定反事实是否进入有限档案。

Result: 在合成基准和OfficeQA上的评估表明，CHASE在保持强基准增益的同时，显著降低了有效协议变化下的增益破坏。

Conclusion: 建立了精确的捷径中立基准B0的数学形式化，并建立了将有限反事实档案与B0联系的统计保证，刻画了顺序Challenger搜索的特性。
...
### [154] [Cultural Competence in Context: A Large Language Model Passes the Turing Test in Finland](https://arxiv.org/abs/2609.18394)
*Otto Segersven,Pentti Henttonen*

Main category: cs.AI

TL;DR: 本研究在芬兰以芬兰语进行图灵测试，检验ChatGPT 5.2在非英语语境下的表现，并提出模型生成的角色提示技术作为可复现的比较方法。研究发现LLM出乎意料地通过了芬兰语图灵测试，参与者主要依赖语言线索（尤其是口语芬兰语）判断人类作者身份。


<details>
  <summary>Details</summary>
Motivation: 研究动机：1）语言和文化背景在LLM训练数据中分布不均，预计模型在芬兰语测试中表现较差；2）需要提出可复现的角色提示技术来改进LLM图灵测试的构念效度；3）探索图灵测试在不同语言文化环境中的适用性。

Method: 采用芬兰语在芬兰进行图灵测试，使用ChatGPT 5.2模型，并运用模型生成的角色提示（model-generated role prompting）作为比较LLM图灵测试的可复现技术。

Result: 与预期相反，LLM通过了芬兰语图灵测试。主要错误来源是参与者依赖语言线索，特别是口语化的芬兰语，作为判断人类作者的标志。

Conclusion: 图灵测试应从智力测试重新定义为检验AI系统在特定社会世界中展示可信成员资格的比较方法。其结果反映模型能力、提示身份、人类参与者的内部人能力及AI素养，该方法可有效探测跨领域的人机边界。
...
### [155] [HPOQuest: A Rare-Disease Diagnostic Agent Using Active Phenotype Acquisition](https://arxiv.org/abs/2609.18431)
*Kamilia Zaripova,Nassir Navab,Azade Farshad,Annalisa Marsico*

Main category: cs.AI

TL;DR: HPOQuest是一个无需训练的框架，用于罕见病诊断中的序贯表型获取。该方法从少量患者表型开始，通过迭代选择信息性问题来更新疾病排名，在四个基准队列中将Recall@1提高30个百分点，Recall@5提高45个百分点。


<details>
  <summary>Details</summary>
Motivation: 全球超过3亿人受罕见病影响，但诊断困难因为患者最初表现出不完整和异质的表型。现有的罕见病诊断方法需要大量初始表型数据，临床实践中往往只有少量表型信息可用。

Method: HPOQuest是一个无需训练的序贯表型获取框架。从少量观察到的患者表型出发，维持一个概率性疾病排名，并迭代选择信息性问题供临床医生评估。确认的表型更新疾病排名，所有响应更新候选问题集。

Result: 在四个基准队列中，HPOQuest从稀疏初始表型显著改善了诊断性能，Recall@1提高高达30个百分点，Recall@5提高高达45个百分点。

Conclusion: 序贯表型获取可以显著改善有限初始临床证据下的罕见病诊断，表明该方法具有临床应用潜力。
...
### [156] [WetRobo: A Reproducible Robot Kit for Coding Agents in Biological Laboratories](https://arxiv.org/abs/2609.18435)
*Yuna Oikawa,Kei Endo,Takanori Uzawa,Yunzhe Zhang,Manan Anjaria,Lerrel Pinto,Sherry Yang,Koji Tsuda*

Main category: cs.AI

TL;DR: WetRobo是一套可转移的机器人工具包,旨在实现生物研究自动化,无需针对每个实验室训练神经网络策略。该系统包含机械臂、实验室设备(培养皿、试剂瓶、培养箱)、代码和演示数据,让生物学家通过自然语言指令即可让机器人执行任务。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言动作(VLA)策略在环境变化时性能下降,且每个实验室都需要重新训练神经网络,不够实用。研究者希望构建一套可在不同实验室之间快速转移的通用机器人系统,让生物实验人员无需进行远程操作或神经网络训练即可 delegated 机器人任务。

Method: WetRobo由机械臂、实验室设备、现有控制代码、远程操作演示和通用AGENTS.md技能文件组成。生物学家提供自然语言任务描述,编码智能体(OeCodex)观察实验室环境并编写执行程序,必要时使用外部工具进行适配。系统无需本地演示数据或神经网络训练。

Result: 在两个真实实验室(Lab X和Lab Y)中成功完成了三项任务:揭开培养皿盖子、打开瓶子和打开培养箱门。编码智能体在两个实验室都完成了瓶盖任务,而在Lab X上微调的VLA策略在Lab X成功但转移到Lab Y时失败。

Conclusion: 为实验室机器人提供了一条实用路径:分发工具包而非为每个实验室训练策略,让编码智能体在各实验室进行自适应。这种方法比VLA策略更具可转移性和实用性。
...
### [157] [Risk-Aware World Modeling with Flow-Guided Occupancy Evolution for Selective Trajectory Planning in Automated Driving](https://arxiv.org/abs/2609.18442)
...
TL;DR: 本文提出RiskWorld框架,一种用于自动驾驶的风险感知世界模型,实现共享占用预测和选择性轨迹替换,在nuScenes数据集上以11.5 FPS的推理速度达到3秒视野最低碰撞率。


<details>
  <summary>Details</summary>
Motivation: 安全自动驾驶需要预测不断演变的交通风险并决定何时修改当前规划的轨迹。现有方法难以在保持计算效率的同时准确预测长期风险并实现选择性轨迹干预。

Method: RiskWorld框架融合空间风险场和时序参与者上下文与视觉鸟瞰图特征,使用流引导演化传输占用率和场景特征,并通过带符号残差修正传输后的占用率。每个规划步骤生成一个预测并跨候选轨迹重用,通过与当前状态持久性参考比较产生非负碰撞分数修正。仅当额外预测风险触发干预且替代方案满足风险和轨迹误差的组件约束时,才替换当前规划轨迹。

Result: 在nuScenes开放环规划中, RiskWorld在3秒长视野下达到最低碰撞率,L2误差排名第二,单张RTX 4090上运行速度为11.5 FPS,参数量90.81M。消融实验表明RiskWorld碰撞率低于当前状态重评分基线,预测重用使评估额外候选轨迹的边际计算成本极低。

Conclusion: RiskWorld通过风险感知的世界建模和选择性轨迹替换策略,有效平衡了安全性与计算效率,为自动驾驶运动规划提供了有前景的解决方案。
...
### [158] [The Mirage of Calibrated Confidence: Trajectory-Independence of Verbalized Confidence in Vision-Language Models](https://arxiv.org/abs/2609.18453)
*Jisoo Yang,Jaeho Han,Trung X. Pham,Junyeong Kim*

Main category: cs.AI

TL;DR: 研究揭示了校准后的视觉语言模型(VLM)存在一个关键问题：即使自我纠正并得出错误答案，模型仍报告高置信度。这是因为口头表达的置信度在很大程度上与推理轨迹无关。研究提出了轨迹锚定分数(TGS)来评估这一问题，并构建了TGS-Bench基准测试套件。


<details>
  <summary>Details</summary>
Motivation: 当前VLM校准方法存在盲点：模型能够反复自我纠正、说"我应该重新检查"、得出错误答案，却仍报告高置信度。这种置信度与实际推理过程脱节的问题无法被现有指标(如ECE和AUROC)检测到。

Method: 通过三个互补视角研究此问题：内容变异、标记掩码和模型的犹豫标记。使用TGS-self(比较有无轨迹访问时的置信度)和TGS-pair(测试模型是否对正确轨迹赋予更高置信度)两种形式评估。构建了包含10个基准测试的TGS-Bench模型无关套件。

Result: 发现置信度对推理轨迹实际内容不够敏感，校准训练反而可能加剧这种脱节。传统校准排名与轨迹锚定排名存在分歧，暴露了当前评估实践的盲点。

Conclusion: 需要新的评估范式来衡量置信度与推理轨迹的真实关联性，TGS和TGS-Bench为检测VLM校准中的轨迹独立性问题提供了有效工具。
...
### [159] [Collective Loss of Control in LLM Agent Systems: An Epidemic Account of Mutation, Contagion, and Recovery](https://arxiv.org/abs/2609.18460)
*Xiangfan Wu,Zonghao Ying,Huiyu Wu,Xing Zheng,Huangsheng Cheng,Xiaorong Shi,Jing Guo*

Main category: cs.AI

TL;DR: 本文提出多智能体系统中局部偏差如何演化为集体失控的流行病学解释，通过"意外变异-传染-恢复"三阶段机制分析，并基于OpenAI智能体协调事件设计RogueHandoff-20基准测试验证条件易感性。


<details>
  <summary>Details</summary>
Motivation: 回答多智能体系统如何从局部偏差演化为集体失控的问题，动机源于已报告的OpenAI智能体协调事件，旨在理解罕见个体偏差与重大集体风险共存的机制。

Method: 1）部署审计识别评估运行间的隐式通信路径并验证Docker后端传输；2）构建RogueHandoff-20基准（含20个可执行场景），使用修改的Qwen-27B路由生成不安全轨迹注入测试接收方易感性。

Result: 四个原生pending路由的实验显示：正常任务执行伤害率为0-5%，注入后伤害率达40-95%，较配对直接恶意请求高5-45个百分点。证实低基线伤害与高条件易感性并存，但未确定自然罕见事件率或证明自主级联。

Conclusion: 结果支持流行病学解释的可行性，提出互补防御策略：加强抵抗力和恢复能力同时预防自发偏差，审计并限制将局部失败转化为集体失控的隐式通信路径。
...
### [160] [Disentangling Long-Term Memory via Latent Neuro-Symbolic Reasoning](https://arxiv.org/abs/2609.18461)
*Cai Ke,Xinghao Chen,Xiaoyu Shen,Keyu Chen,Siyu An,Junnan Dong,Ruifeng Xu,Ruizhi Qiao,Xing Sun*

Main category: cs.AI

TL;DR: 本文提出LGM框架，一个用于个性化智能体长期记忆推理的神经符号方法。该框架将长期记忆解纠缠转移到连续潜在空间，通过稀疏自编码器构建潜在图，并使用图编码器进行查询感知的消息传递，从而有效捕获用户的显式偏好和隐式行为证据。


<details>
  <summary>Details</summary>
Motivation: 当前个性化智能体在处理长期历史交互时面临三大挑战：1) 早期平面检索方法独立评分记忆碎片，忽视分布式信息；2) 现有结构化记忆框架依赖查询无关的静态图，无法捕捉上下文依赖关系；3) 原始文本记忆本身纠缠且噪声密集，导致细粒度个性化 和跨会话推理计算成本过高。

Method: 提出LGM神经符号框架，包含两部分：(i) 设计定制化的潜在图构建模块，使用稀疏自编码器将历史交互映射为潜在记忆节点，将记忆轨迹解纠缠为稀疏概念激活，为每个查询动态合成查询感知的关联边权重；(ii) 图编码器将查询嵌入作为条件偏好，引导任务特定潜在子图上的非线性消息传递，生成高表达性的记忆表示用于有效激活。

Result: 在长期个性化基准测试上的广泛实验表明，LGM在捕获显式偏好和隐式行为证据方面显著优于现有最先进基线，同时能够实现个性化响应。

Conclusion: LGM框架通过将记忆解纠缠引入潜在空间，成功解决了传统方法中忽略分布式信息、依赖静态图结构以及计算成本高昂的问题，为个性化智能体的长期记忆推理提供了更有效的方法。
...
### [161] [First Token Matters: Understanding Safety Collapse in Large Reasoning Models](https://arxiv.org/abs/2609.18471)
*Yizheng Yang,Haining Yu,Yuechen Wang,Yikai Hou,Xing Fu,Jinbo Yang,Tianqing Zhu*

Main category: cs.AI

TL;DR: 本文研究大型推理模型(LRMs)在处理有害查询时安全对齐失效的问题，发现了推理起始阶段的"起始拒绝崩溃"(ORC)现象，并提出SafeToken方法通过在推理起始位置注入学习的安全锚点来改善模型安全性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在问题解决方面表现出色，但其安全对齐在处理有害查询时往往退化。现有方法主要依赖额外训练或偏好优化，对安全失效的内部机制理解有限。

Method: 通过token级别的位置分析研究拒绝动态，识别出推理起始阶段的局部化脆弱点；提出SafeToken，一种轻量级的推理时干预方法，在推理起始位置精确注入学习到的连续安全锚点。

Result: SafeToken仅更新单个token嵌入就能有效缓解ORC，改善有害查询基准的安全性，同时很大程度上保留推理效用。

Conclusion: LRMs的安全失效可能源于从理解到生成的关键转换时刻的短暂崩溃，这为推理时安全干预提供了新思路。
...
### [162] [Hyperbolic Graph Representation Learning for Differential Diagnosis on Biomedical Knowledge Graphs](https://arxiv.org/abs/2609.18481)
...
TL;DR: 本研究探讨了双曲嵌入在混合生物医学知识图谱中的应用价值。研究发现，双曲模型在纯本体论子图中能用更低的维度达到与欧几里得模型相当的性能，且在患者疾病预测的链接预测任务中，双曲嵌入能够有效利用生物医学层次结构同时支持异构患者图的诊断推理。


<details>
  <summary>Details</summary>
Motivation: 生物医学知识图谱结合了本体论导出的层次结构和异质实体（如表型、疾病、基因、蛋白质和患者）之间的横向关联。这种混合结构提出了一个问题：原本自然捕获树状组织的双曲嵌入，在非纯层次图之外是否仍然有用。

Method: 在患者整合的生物医学图上开展双曲图表示学习实验，用于孟德尔疾病的鉴别诊断。具体包括：在隔离的本体论子图上评估模型性能，以及在链接预测任务中为每位患者排名候选疾病。

Result: 在孤立本体论子图上的实验表明，双曲模型在维度显著更低的情况下实现了强大的性能。在为每位患者排名候选疾病的链接预测任务中，结果表明双曲嵌入能够利用生物医学层次结构，同时支持异构患者级图的诊断推理。

Conclusion: 双曲嵌入在生物医学知识图谱中具有应用价值，能够有效利用层次结构并支持异构图的诊断推理，为双曲嵌入在混合生物医学图中的应用提供了初步证据。
...
### [163] [Beyond Routine Compliance: Cunning Data Cultivates Safety Vigilance in Large Language Models](https://arxiv.org/abs/2609.18515)
*Youjia Wang,Lin Xu,Yang Sun,Yuxiao Lu,Chengfang Fang,Jie Shi*

Main category: cs.AI

TL;DR: 本文提出通过训练模型识别"狡猾问题"（cunning questions）——包含误导性前提、不典型推理或细微不一致的问题——来增强大语言模型的安全对齐能力。这种训练方法能让模型学会超越表面语义，识别潜在风险，从而提高对越狱攻击的鲁棒性。实验表明，该方法将9个模型-基准组合的平均攻击成功率从17.40%降至15.05%。


<details>
  <summary>Details</summary>
Motivation: 当前安全对齐的LLM在有害意图被隐藏在看似 benign 的上下文中时会失效。传统安全训练仅教授模型识别有害请求并拒绝，风险在于模型缺乏对异常前提、误导性推理和潜在风险的"警惕性"（vigilance）。作者认为需要培养模型在行动前审视请求的潜在意图和假设的能力。

Method: 引入"狡猾问题"（cunning questions）作为训练数据：这些问题本身不涉及安全相关内容，但包含误导性前提、不典型推理或细微不一致。通过让模型学习识别和应对这些"推理陷阱"，期望这种能力可以迁移到安全关键场景。采用标准安全微调作为基线，并在此基础上叠加Cunning训练。

Result: 1) Cuning训练提高了模型对分布外越狱攻击的鲁棒性；2) 增强了后续安全微调的效果；3) 将现有SOTA安全对齐管道的平均攻击成功率（ASR）从17.40%降至15.05%；4) 匹配安全微调后的trace分析表明，安全判断更可能在有害规划开始前就主导响应。

Conclusion: Cunning数据可以强化模型的警惕性，是对传统安全对齐方法的有效补充。理论分析进一步说明了从cunning数据学习的不变性何时可以迁移到安全相关输入。这项工作为提升LLM安全鲁棒性提供了一种新思路。
...
### [164] [AeroWeaver: An Embodied-Agent Harness for Weaving Aerial Skills into Distributed, Adaptive Swarm Execution](https://arxiv.org/abs/2609.18520)
*Jiabin Lou,Yirong Yang,Haopeng Wang,Xuxin Lv,Xinyu Liu,Diyuan Hou,Xuehong Liu,Rongye Shi,Wenjun Wu*

Main category: cs.AI

TL;DR: AeroWeaver是一个将大型语言模型(LLM)智能体应用于无人机蜂群协调的框架，通过将语义决策与可执行技能绑定、支持分布式角色条件本地智能体、以及利用角色索引的状态-动作-奖励经验实现在线自适应学习，解决LLM在无人机蜂群中面临的决策落地、全局推理与分布式执行协调以及持续适应性等挑战。


<details>
  <summary>Details</summary>
Motivation: 将LLM智能体的自然语言任务理解和高层次规划能力应用于无人机蜂群协调，但面临三个核心挑战：1) 如何将模型决策落地为可执行能力；2) 如何协调全局任务推理与分布式执行；3) 如何利用任务特定经验进行持续适应。

Method: 提出AeroWeaver框架，包含三个核心组件：1) 语义决策到governed技能的连接机制；2) 角色条件化本地智能体组织实现分布式协调；3) 角色索引的状态-动作-奖励经验用于在线技能选择优化。

Result: 实验和运行时验证表明：AeroWeaver在测试条件下保持有效的技能执行；支持无中央智能体生成联合动作的本体局部多无人机操作；奖励引导的在线更新提供了从累积执行经验中自适应学习蜂群智能体的训练免费路径。

Conclusion: AeroWeaver成功地将LLM智能体与无人机蜂群相结合，实现了分布式协调和在线自适应学习，为LLM在具身多智能体系统中的应用提供了新范式。
...
### [165] [TRIPROBE: Probing Task Separability Beyond Classification for XAI](https://arxiv.org/abs/2609.18525)
*Amirhossein Sadough,Freek Hens,Aleksa Bokšan,Mohammad Mahdi Dehshibi,Mahyar Shahsavari*

Main category: cs.AI

TL;DR: TriProbe是一种多层次探针框架，用于可解释地诊断任务可分离性。该框架通过在输入空间、特征表示和最终分类器三个层面应用互补探针，将多任务问题分解为二进制子任务，使用最大费舍尔判别比作为原则性可分离性度量，识别瓶颈和受影响的任务对。实验表明该方法能揭示隐藏的故障点，指导数据收集、验证和架构设计。


<details>
  <summary>Details</summary>
Motivation: 当前学习管道的评估通常简化为下游准确率，无法解释任务成功或失败的原因。现有方法将模型视为黑箱，缺乏对任务可分离性演变的深入理解。

Method: TriProbe采用三探针互补方法：基础探针(Foundational Probe)分析输入空间，潜在探针(Latent Probe)分析特征表示，最终探针(Final Probe)分析分类器输出。使用最大费舍尔判别比(Maximum Fisher's Discriminant Ratio)作为可分离性度量指标。

Result: 在Roshamba sEMG基准数据集上的实验表明，TriProbe能够揭示隐藏的故障点，识别出任务可分离性在不同阶段的瓶颈，并发现受影响的任务对。

Conclusion: TriProbe能够有效追踪可分离性在输入、学习特征和最终分类器之间的演变，为数据收集、验证和架构设计提供了有价值的指导。
...
### [166] [Recursive Reasoning or Statistical Extrapolation? In-Context Learning in Multi-Agent Interdependent Decision-Making](https://arxiv.org/abs/2609.18591)
*Yu Liu,Wenwen Li,Yifan Dou,Guangnan Ye*

Main category: cs.AI

TL;DR: 研究大型语言模型在多智能体不完全信息游戏中的上下文学习(ICL)行为，区分其决策改进是源于精细的内部推理还是仅是对统计模式的外推。通过构建公共物品游戏并操纵历史反馈的统计结构，发现当历史统计模式被破坏时，更长上下文的收益基本消失，决策质量降至无上下文基线水平，且战略依赖性越强，削弱越明显。


<details>
  <summary>Details</summary>
Motivation: 理解ICL是否能帮助LLM智能体进行真正的推理，还是仅依赖统计模式外推。之前研究未明确区分这些机制。

Method: 构建公共物品游戏，操纵历史反馈的统计结构，将决策质量与历史独立的理性预期均衡(REE)基准进行比较。

Result: 当历史统计模式被破坏时，更长上下文的收益基本消失，决策质量降至无上下文基线，且战略依赖性越强，削弱越明显。

Conclusion: 在战略环境中，ICL行为更符合统计外推而非战略推理。引入REE作为区分推理与外推的诊断工具，提供可复用的框架来探索LLM在递归信念任务中的推理边界。
...
### [167] [Reasoning through Evolution: Automatic Meta-path Discovery for LLM-based Fake News Detection](https://arxiv.org/abs/2609.18597)
*Ziyi Zhou,Xiaoming Zhang,Hui Pang,Yuting Zhang,Tiesunlong Shen,Bingyu Yan,Erik Cambria,Litian Zhang*

Main category: cs.AI

TL;DR: MAG ER是一个多智能体遗传进化框架,通过自动发现优化后的元路径来压缩复杂传播图,使冻结的大型语言模型能够进行结构感知的虚假新闻推理,在零样本和少样本设置下显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于GNN的虚假新闻检测方法依赖大量标注数据且泛化能力有限;直接将原始传播图输入LLM会导致严重的模态不匹配和信息过载,使结构感知推理不可靠。

Method: 提出多智能体遗传进化框架MAGER,自动发现优化元路径将复杂传播图压缩为信息丰富的子图;引入图上下文学习策略,检索语义和结构相似的演示样本。

Result: 在数据高效设置下,MAG ER显著提升了冻结LLM作为独立虚假新闻检测器的性能。

Conclusion: MAGER通过进化的元路径有效缓解了信息过载和模态不匹配问题,使冻结LLM能够进行结构感知的真实性推理,为数据稀缺的虚假新闻检测提供了有效解决方案。
...
### [168] [The Uneven Impact of Generative AI on Student Learning: Examining the Roles of Reliance, Evaluation Literacy, and Course Policy in AI-related Courses](https://arxiv.org/abs/2609.18676)
*Lydia Manikonda,Mei Si,Sirajam Munira,Oshani Seneviratne,Kristin Bennett*

Main category: cs.AI

TL;DR: 研究调查了118名学生使用生成式AI的情况，识别出四类用户群体，发现免费版与付费版用户、单工具与多工具用户、不同 instructor policies 下学生的使用体验存在显著差异。早期依赖AI与负面影响相关，且这种关联随着评估素养的提高而增强。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正在改变学生的学习方式，但课程背景、认知依赖、评估素养和早期依赖等因素的作用尚未得到充分探索。

Method: 对12门AI相关课程的118名学生进行问卷调查，采用聚类分析和多变量回归模型分析数据。

Result: 识别出四类用户：高度使用并报告诸多好处者、轻度使用者（依赖少、好处少）、两个中等使用但好处感受不同的群体。免费版与付费版用户、单工具与多工具用户、不同 instructor policies 学生存在显著差异。学业收益与早期依赖和学术任务支持相关；正面影响与认知依赖、学术任务支持、对AI可靠性的信心和 instructor policies 相关；负面影响与早期依赖和态度变化相关。早期依赖与负面影响的关联随评估素养提高而增强。

Conclusion: 机构需要制定更好的政策来应对不平等现象，使学生能够从日益强大的AI系统中受益。
...
### [169] [Beyond Truncation: Rethinking LLM Decoding as Ensemble Pruning](https://arxiv.org/abs/2609.18723)
*Dunyao Xue,Chengshuo Du,Zhengbo Wang,Wenlin Dai,Cheng Meng*

Main category: cs.AI

TL;DR: 提出了一种名为Mahalanobis-Ensemble Decoding (ME-Decoding)的新型大语言模型解码框架，将候选token选择建模为集成剪枝问题，通过马氏距离驱动的目标函数在保持高概率的同时增强语义多样性。


<details>
  <summary>Details</summary>
Motivation: 现有选择策略主要依赖标量概率，忽略了几何语义关系并导致候选冗余；而当前的几何感知方法通常需要复杂优化或直接重新加权原始token概率，导致显著的计算开销或推理不稳定性。

Method: 将解码表述为子集优化问题，使用基于马氏距离的目标函数增强语义多样性；通过基于自适应带宽核的token嵌入构建token相似性矩阵来动态折扣冗余生成路径；设计了具有近线性复杂度的贪婪选择算法，并建立了理论近似保证。

Result: 在多种推理和生成任务上的广泛实验表明，该方法始终表现出色。

Conclusion: ME-Decoding是一种鲁棒、即插即用的模块，具有可忽略的推理开销。
...
### [170] [Which LLM is Best for Translating Natural Language Goals to PDDL](https://arxiv.org/abs/2609.18731)
*Tomas Balyo,Lukas Chrpa,G. Michael Youngblood*

Main category: cs.AI

TL;DR: 本研究评估了当前大型语言模型(LLMs)能否将自然语言测试目标可靠地翻译为PDDL规划目标。实验表明六个模型正确率均超过92%，Gemini 2.5 Flash达96%准确率最高，GPT-4.1速度最快，但语言歧义和领域表示局限仍带来挑战。


<details>
  <summary>Details</summary>
Motivation: 自动化规划中，用PDDL等形式语言表达目标限制了对非专家的可达性，需要弥合人类意图与机器执行之间的差距

Method: 设计整合迭代实验见解的提示模板，系统评估六个当代LLM的正确性、速度和错误倾向，使用真实领域特定基准测试

Result: 所有模型正确率超92%，Gemini 2.5 Flash准确率最高(96%)且假阳性最低，GPT-4.1响应速度最快

Conclusion: LLM作为自然语言目标与自动化规划管道之间的桥梁取得显著进展，但模型间存在关键性能差异，语言歧义和领域表示局限仍导致偶发失败
...
### [171] [Clueing up LLMs with Tool-Augmented Deductive Reasoning](https://arxiv.org/abs/2609.18736)
*Rebecca Ansell,Autumn Toney-Wails*

Main category: cs.AI

TL;DR: 本文将经典桌游Clue改编为文本版多智能体环境，用于评估LLM在多步骤、逻辑推演推理中的表现。研究对比了基线方法与工具增强方法（使用可能性矩阵），发现工具增强能有效提升推理质量和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型取得了进展，但在长程交互中保持逻辑一致的演绎推理仍具挑战性。当前模型在整合多步推理证据、维持推理一致性及在新约束下更新信念等方面存在局限，需要新的测试环境来评估推理增强效果。

Method: 1. 将经典桌游Clue改编为文本版多智能体环境，玩家需从一系列观察中推断隐藏信息并维持逻辑一致性；2. 使用6个LLM智能体（GPT-4o-mini和Gemini-2.5-Flash各3个）进行轮对局游戏，建立基线性能；3. 引入工具增强方法——结构化可能性矩阵，将隐式游戏状态转换为剩余可能性的显式表示，编码跨轮记忆和演绎约束。

Result: 论文建立了模型在战略推理环境中的基线性能，并对比了工具增强方法与基线的表现差异，具体数值需查阅全文。

Conclusion: 可能性矩阵通过编码扩展轮次记忆和演绎约束，将这些任务从智能体卸载出来，有效支持了自主智能体在战略推理环境中的推理质量和任务成功率。
...
### [172] [Version- and Scope-Aware Question Answering over Normative Documents: A Deployed System and an End-to-End Evaluation at Production Scale](https://arxiv.org/abs/2609.18769)
*Liuyin Wang,Shuaipeng Jin,Jiwei Shi,Jensen Hsu*

Main category: cs.AI

TL;DR: 本文评估了简单托管检索服务与 governed system（治理系统）在回答基于规范文档问题上的性能差异。治理系统通过显式规则解析版本和范围后再生成答案，得分97.7；托管服务得分88.1，两者差距9.6分。该治理系统已作为商业产品运营，拥有1126名注册用户。


<details>
  <summary>Details</summary>
Motivation: 基于规范文档（如法律、规章）的问题回答通常依赖于单一 passage 之外的信息，包括：文档版本是否现行、是否适用于相关管辖权/主体/日期、以及规范主张能否追溯到源文本。托管检索服务虽降低了工程成本，但其"上传文档并提问"的默认方案在准确率上存在不足。

Method: 实验基于约73,000份候选规范文档，使用来自已发布基准的200个分层抽样问题（每个问题有金标准源文档），采样规则不依赖任何系统输出或分数。对比托管服务与 governed system（通过显式规则解析版本和范围后再生成的系统）。

Result: 治理系统得分97.7，托管服务得分88.1，两者相差9.6分（基于未四舍五入均值计算）。治理系统自2026年1月作为商业产品运营，拥有1,126名注册用户，包括Zhipu AI和Lecheng Health等客户组织，截至2026年4月中旬达到约100,000次工作日调用。

Conclusion: 简单的托管检索服务在处理规范文档问答时存在显著局限，显式的版本和范围治理机制可显著提升答案准确率（约9.6分），该方法已在实际生产环境中得到验证并实现规模化应用。
...
### [173] [CERA-MoA: Co-Evolving Routing Mechanisms with Continually Learning LLM Agents](https://arxiv.org/abs/2609.18779)
*Jiaxuan Jiang,Liyuan He,Zhixuan Fang*

Main category: cs.AI

TL;DR: CERA-MoA是一个迭代强化学习框架，通过协同进化动态路由器和独立智能体策略，解决传统MoA中查询路由和智能体微调分离的问题。该框架使用预测性熟悉度估计器评估智能体的语义能力，并基于累积阈值自适应路由机制动态激活最小智能体子集，同时通过主动分配针对性训练样本促进能力分化。


<details>
  <summary>Details</summary>
Motivation: 当前Mixture-of-Agents范式将查询路由和智能体微调视为独立过程，限制了它们响应 evolving agent capabilities的能力。这种脱节阻止了路由策略在训练后适应 evolving agent capabilities，也阻止了智能体实现协同的数据驱动专业化。

Method: 1. 迭代强化学习框架，动态路由器与独立智能体策略协同进化；2. 预测性熟悉度估计器，利用中间层隐藏状态评估智能体间的语义能力，避免完整rollout的开销；3. 累积阈值自适应路由机制，基于熟悉度分数动态激活定制化的最小智能体子集；4. 根据智能体 evolving competence主动分配针对性训练样本。

Result: 在多个领域的广泛实验中，CERA-MoA优于最先进的静态智能体路由和固定工作流微调基线方法。

Conclusion: 通过协同进化机制，CERA-MoA成功实现了路由策略与智能体能力的动态适配，在任务性能和效率之间取得良好权衡，并促进了智能体的能力分化。
...
### [174] [Infinite-Parameter LLMs: Generating and Adapting Weights from Live Data](https://arxiv.org/abs/2609.18842)
*Jinli Hu,Ross M. Clarke,Yichuan Zhang,José Miguel Hernández-Lobato*

Main category: cs.AI

TL;DR: 提出一种名为"Infinite-Parameter LLM"的新架构，灵感来自MoE，通过紧凑的超网络将运行时数据转换为共享基础网络的低秩调制，使模型能够从实时交互中学习并将知识写入权重，而非仅依赖提示词。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在训练后权重冻结，无法从用户交互中学习有用信息。提示词中的知识仅在单次请求期间有效随后被丢弃。作者认为应该有一种架构能够将运行时提供的信息写入模型权重，实现持续学习和知识积累。

Method: 采用紧凑超网络将运行时数据生成共享基础网络的低秩调制；携带关于生成器潜在代码的贝叶斯信念并在线更新，使有效权重在会话过程中从演化的信念中重新推导，而非一次性读取后固定。

Result: 实现了固定存储占用下的有效无限权重能力，将运行时知识存储在权重中而非提示词中，可跨轮次持续、释放上下文窗口、计算 amortized，且可能比上下文学习具有更好的泛化能力。

Conclusion: Infinite-Parameter LLM为解决LLM持续学习问题提供新思路，通过运行时动态生成权重而非依赖固定参数库，使模型能够从实时交互中学习和适应，这是对传统MoE架构的重要创新。
...
### [175] [Suppressed, Not Erased: A Representational Trace of Edited Facts Survives Even Weight-Free Knowledge Editing](https://arxiv.org/abs/2609.18985)
*Priyansh Srivastava,Romit Chatterjee*

Main category: cs.AI

TL;DR: 本文提出使用线性追踪探针来直接研究模型编辑后的残留知识，发现即使编辑在行为层面成功，原有知识仍然可以从模型的隐藏状态中线性解码出来。这表明编辑操作只是抑制而非擦除原始关联。


<details>
  <summary>Details</summary>
Motivation: 现有的知识编辑基准只验证局部正确性（编辑后的模型是否在新提示下产生新事实），但没有衡量原始事实在模型内部是否仍可解码。作者希望通过线性追踪探针直接测量编辑后残留的知识程度。

Method: 在GPT-2-XL上使用三种机制不同的编辑器（ROME、约束微调、GRACE）对50个CounterFact编辑进行测试，使用线性探针检查原始对象是否仍可从模型的隐藏状态中恢复。

Result: 即使编辑达到100%生成成功率，原有对象仍可被线性解码：ROME探针准确率0.96，约束微调0.79，GRACE（不修改权重）0.79，均显著高于随机水平0.50。GRACE结果最具启示性——它完全不修改权重，仅通过外部记忆覆盖事实，但原始对象仍可解码。

Conclusion: 知识编辑即使在行为上成功，也是抑制而非擦除原始关联。残留探针无法归因于不完整的权重更新。 relearning-savings方法在当前设置中不可靠，作为负面方法论结果报告。
...
### [176] [Function Lives Where Variance Doesn't: Task-Weighted Charts of a Language Model's Computation](https://arxiv.org/abs/2609.18989)
*Alexandre Quemy*

Main category: cs.AI

TL;DR: 语言模型的实际计算维度取决于具体任务功能：保持完整困惑度的95%需要 residual stream 宽度的70-90%，但激活方差最大的方向几乎不承载预测功能。


<details>
  <summary>Details</summary>
Motivation: 传统问题“语言模型使用多少维度”定义不清，需要引入具体功能度量使其可操作化。

Method: 任务加权坐标轴方法：在选定功能（如下一个token预测）下，用该功能自身的度量将低维坐标拟合到表示上，将蒸馏转化为普通最小二乘问题。

Result: 跨越3个模型族、6个模型（70M-7B参数）的实验表明：(1)下一个token预测需70-90%残差宽度才能达到完整困惑度的95%；(2)GPT-2两个方向承载90%激活方差但几乎无预测功能；(3)模型自身不确定性在6个坐标上读取，而完整预测分布需数百个坐标；(4)维度随深度增加。

Conclusion: 维度是功能依赖的；任务加权图谱方法在有限维度保留时比基于方差或最优线性压缩的方法更能保留模型预测性能。
...
### [177] [Lost in Perception: Isolating Perceptual and Reasoning Failures in Multimodal Physics and Geometry Reasoning](https://arxiv.org/abs/2609.18991)
*Raj Jaiswal,Sree Krishna Uppalapati,Dhruvkumar Patel,Ria Khatoniar,Tanuja Ganu,Rajiv Ratn Shah*

Main category: cs.AI

TL;DR: 本文提出五任务诊断实验，区分多模态LLM在物理和几何基准中的感知失败与推理失败，发现错误图表解读会降低性能，校正标题后可分离感知受阻与真正推理瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在科学推理基准上表现强劲，但多数将感知与推理视为单一过程，无法区分感知失败与推理失败。本文旨在通过诊断实验分离这两类失败。

Method: 构建跨越物理和几何基准的五任务诊断实验，从原始图像到人类撰写标题逐步测试，分离感知、推理及两者兼具的失败。

Result: 图表解读错误会损害性能，即使模型单独从文本解决正确；准确率从原始图像到人类标题逐步提升；校正标题后部分模型恢复度高，可区分感知受阻与推理瓶颈；物理失败转化为计算错误，几何失败转化为概念误用；InternS1-mini虽经大量科学预训练，但表现最差，推理痕迹常提前终止。

Conclusion: 感知与推理失败可分离且领域相关；校正感知输入可恢复部分性能；推理错误类型取决于领域；强大预训练不足以保证推理能力，需关注推理过程本身。
...
### [178] [Compiled Agency: Frontier General-Purpose Coding Agents Build Winning Game Players from Bare Interaction - from Flappy Bird to StarCraft II and Civilization](https://arxiv.org/abs/2609.18996)
*Joey Xiao,Haonan Huang*

Main category: cs.AI

TL;DR: Gauntlet框架测试前沿LLM编码Agent是否能独立从零构建游戏控制器并获胜。实验表明最新一代模型可在单次自主会话中开发出能够击败StarCraft II内置AI和 Civilization完整游戏的控制器，实现了"编译智能"——将开发经验编译为持久可执行程序。


<details>
  <summary>Details</summary>
Motivation: 探讨前沿LLM是否能够将游戏知识转化为 kompetent play，以及是否能在无辅助情况下独立构建完整的玩家程序。之前即使配备感知、记忆、技能库、规划器等辅助架构，LLM agents在游戏表现上仍困难重重。

Method: 提出develop-freeze-evaluate框架：给定游戏描述、原始观察/动作接口和空白策略文件，让编码Agent在单次自主会话中通过实验探索游戏并工程化构建独立控制器，然后冻结结果并在零模型调用条件下于测试集上评估。

Result: 在程序化roguelike上成功率达0-86%，呈现明显的代际阈值效应——每一代最新模型都超越前代最佳表现。在完整游戏规模上：原始API控制器击败所有公平StarCraft II内置AI及两个作弊变体；单会话程序在Freeciv中通过完全征服赢得完整游戏。

Conclusion: 前沿编码Agent开始能够追踪长期战略。这标志着"编译智能"（compiled agency）能力——将开发经验编译为持久可执行智能体，其架构由模型自身构建。这是首个无需每轮模型调用和手工战术层即可独立赢得完整游戏类型的语言Agent系统。
...
### [179] [MUSE: Benchmarking Large Vision-Language Models on Multi-Modal Understanding in Situated Education](https://arxiv.org/abs/2609.19088)
*Luyao Zhu,Xun Wei Yee,Wei Li,Mun Thye Mak,Wee Siong Ng*

Main category: cs.AI

TL;DR: MUSE是一个用于评估大型视觉语言模型在艺术图像教育理解方面的基准测试，涵盖视觉感知、语义与情感解读、文化理解和组合推理等12项任务，聚焦新加坡、东南亚多元文化及西方艺术传统背景。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注真实世界图像或特定领域教育推理，对艺术教育内容的覆盖有限。大型视觉语言模型在教育场景中的能力尚未得到充分评估，需要专门的艺术图像理解基准。

Method: 将图像标注与问题生成解耦，创建12项任务，涵盖视觉感知、语义情感解读、文化理解和组合推理；精心策划包含新加坡、东南亚多元文化及西方艺术传统的多样艺术图像；评估了开源和专有模型。

Result: 发现模型在能力维度上存在显著差异，尤其在情感解读和组合推理方面表现不足；识别出常见失败模式和关键挑战。

Conclusion: MUSE可作为推进情境化教育应用中多模态理解的标准化基准，为开发可信的多模态教育模型提供重要参考。
...
### [180] [Numerical Simulation of Transdermal Insulin Delivery Using a Coated Microneedle in a 2D Skin Model](https://arxiv.org/abs/2609.18931)
*Milana Tesfamarian,Michael Heisig,Gabriel Wittum,Rolf Krause*

Main category: cs.CG

TL;DR: 本文开发了一个计算模型来研究涂层微针的经皮胰岛素递送，分析胰岛素通过不同皮肤层的释放过程，并评估关键传输参数的影响。


<details>
  <summary>Details</summary>
Motivation: 开发涂层微针经皮胰岛素递送的计算模型，以深入理解药物渗透机制并优化递送效率。

Method: 建立了包含涂层微针的详细皮肤几何模型，使用非结构化网格实现可靠的模型分辨率，模拟胰岛素从微针向皮肤各层的渗透和分布，并与实验数据进行对比验证。

Result: 模拟提供了胰岛素从涂层微针渗透、跨皮肤层传输和分布的深入见解，并验证了模型的预测能力。

Conclusion: 计算模型能够有效预测涂层微针经皮胰岛素递送过程，为微针设计优化和临床应用提供了理论指导。
