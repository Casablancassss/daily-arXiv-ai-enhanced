...
TL;DR: H²MAF框架结合EfficientNet-B3和ConvNeXt-Tiny进行决策级融合，利用Gemma和Qwen3.5多模态大语言模型进行语义仲裁，实现可解释的作物病害诊断、风险评估和治疗紧迫性判断。


<details>
  <summary>Details</summary>
Motivation: 田间作物病害诊断需要融合不确定且冲突的感知证据，现有方法缺乏对冲突证据的可解释性处理和语义仲裁能力。

Method: 提出混合层次化多智能体框架(H²MAF)，结合EfficientNet-B3和ConvNeXt-Tiny的决策级融合，使用开源多模态大语言模型(Gemma 4B和Qwen3.5 4B)进行语义仲裁，生成结构化JSON格式的诊断证据。

Result: 在PlantDoc数据集上，Gemma将准确率从63.9%提升至68.5%，在CNN冲突子集上提升7.6个百分点；Cornell数据集准确率达99.3%和98.9%，仅1.7-4.1%分歧；Gemma关键风险错误率为0.14-0.5，Qwen过度标记率达3.5-14.4。

Conclusion: 多模态大语言模型仲裁是实现可解释农业AI和机器人田间决策支持的有前景方法，但其性能依赖校准，且Qwen存在过度标记问题需要进一步优化。
...
TL;DR: 本研究提出了一个轻量级的多模态视觉语言框架(TinyCLIP),用于苹果果园中早期果实解剖结构的细粒度分类,可识别花萼、果实和果柄三个部位。该框架在600张高分辨率RGB图像数据集上训练,采用滑动窗口推理策略将patch级预测聚合为空间热图,实现可解释的整图定位。模型经ONNX和TensorRT优化后,可部署在NVIDIA Jetson硬件上,支持INT8量化,模型大小约127-137MB。


<details>
  <summary>Details</summary>
Motivation: 准确识别早期苹果果实解剖结构(包括花萼、果实和果柄)对于机器人疏果、作物负载管理和精准园艺操作至关重要。现有方法在复杂果园环境中进行细粒度果实解剖分类面临挑战,需要开发轻量级、可边缘部署的感知方案。

Method: 1)数据集:600张来自Scilate和Scifresh苹果园的高分辨率RGB图像,标注三个解剖类别 2)图像预处理:将图像转换为224×224的图像块 3)多模态对齐:使用领域特定语言提示(如"a photo of a class")引导果园图像与园艺结构的对齐 4)推理策略:采用步长112像素的滑动窗口推理,将patch级预测聚合为空间热图,实现整图定位

Result: patch级评估在NVIDIA T4 GPU上取得F1分数:花萼0.95、果实0.98、果柄0.85,宏F1为0.93。ONNX和TensorRT优化后支持INT8量化,模型大小约127-137MB,patch推理达毫秒级,可部署于NVIDIA Jetson硬件。

Conclusion: 轻量级视觉语言模型可提供可解释且可边缘部署的感知能力,适用于自动果实分析和未来机器人疏果系统。该方法在复杂果园环境中展现出良好的细粒度分类性能和实用部署潜力。
...
TL;DR: 本文提出了一种局部地理与空间上下文融合(LGSCF)策略，用于改进滑坡敏感性制图。该策略通过特征级调制机制整合滑坡点的地理环境特征与其空间上下文，并集成到多种CNN架构中。实验表明，LGSCF模型在台湾南投县仁爱乡和新故乡约2644 km²区域表现优异，F1分数达87.09%，AUC达0.9472。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的滑坡敏感性制图方法存在局限：基于像素的模型仅关注滑坡点的地理环境特征，忽略周围环境的影响；基于斑块的模型虽纳入空间上下文，但可能包含与目标滑坡位置空间相关性较弱或无关的像素。因此需要一种能有效融合地理环境特征与空间上下文的策略。

Method: 提出Local-Geo and Spatial Context Fusion (LGSCF)策略，通过特征级调制机制协同整合滑坡点的地理环境特征及其对应空间上下文。将该策略集成到多种代表性CNN架构中，创建了9个不同的LGSCF模型。实验区域为台湾南投县仁爱乡和新故乡，面积约2644 km²，数据集包含5332个滑坡样本和等量非滑坡样本。

Result: LGSCF模型一致性地优于其原始版本，最高达87.09%的F1分数和0.9472的AUC值。LGSCF模型生成的敏感性地图显示，已知滑坡更准确地集中在"极高"敏感性区域，误分类更少。

Conclusion: LGSCF融合策略能显著提升滑坡敏感性制图的准确性，为滑坡敏感性制图提供了一种有效的新方法。
...
TL;DR: 针对视觉语言模型(如LLaVA-1.5-7B)在生成描述时产生幻觉对象的问题,本文提出了一个从诊断到干预的流程。通过分析注意力头在幻觉对象词附近的图像注意力下降情况,筛选出32个关键头,并开发了两种干预方法:头部切片LoRA适配器和推理时间接地控制器。实验表明,该方法显著降低了幻觉率,同时通过对照实验验证了头部选择的有效性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在生成图像描述时常幻觉出不存在的对象,需要理解这种失败的根本原因并开发针对性的修复方法。

Method: 1)根据注意力头在幻觉对象词附近的图像注意力下降程度进行排序 2)通过消融候选头并测量幻觉标记对数概率变化来筛选短名单,得到32个头 3)两种干预措施:头切片LoRA适配器和推理时间接地控制器 4)在400张保留的COCO图像上评估 5)使用随机头LoRA控制进行对照实验

Result: CHAIRs从0.370降至0.230,CHAIRi从0.156降至0.096(p<0.001)。在固定解码预算下,CHAIR降低持续存在并随预算增加(64 tokens时23%,128 tokens时58%)。但对象召回率也从0.78降至0.70。

Conclusion: 诊断驱动的干预方法能有效定位具有真正、非随机杠杆作用的头部,从而减少幻觉对象提及。该研究强调了报告行为特征而非单一分数的重要性,并提供了从诊断到干预的完整流程。
...
TL;DR: SHIFT-LLM是一种无需训练的后剪枝校正框架，通过在每个剪枝位置插入线性残差适配器(LRA)来恢复因深度剪枝造成的精度损失。LRA保留原始残差块的恒等路径并添加轻量级仿射残差校正，通过闭式最小二乘回归在小型保留集上进行校准，无需梯度计算。实验表明该方法在5个模型家族、6种层选择标准和7个零样本基准上均能有效恢复精度，在Llama-3.1-8B-Instruct上最高提升15.7个点。


<details>
  <summary>Details</summary>
Motivation: 深度剪枝移除整个Transformer块以降低大语言模型的推理成本，但这会破坏下游层期望的隐藏状态分布，导致显著的精度损失。现有方法难以有效解决这种由层移除引起的分布不匹配问题。

Method: 在每个剪枝位置插入线性残差适配器(LRA)，该适配器保留原始残差块的恒等路径并添加轻量级仿射残差校正。校正通过在小型保留集上进行闭式最小二乘回归来校准，无需梯度计算。LRAs支持低秩因式分解和连续剪枝层的精确合并，可与参数高效微调自然结合。

Result: 在5个模型家族、6种层选择标准和7个零样本基准上，SHIFT-LLM一致地恢复了深度剪枝造成的精度损失。在Llama-3.1-8B-Instruct上获得最高+15.7个点的增益，仅需几百个校准样本且无需梯度计算。

Conclusion: SHIFT-LLM提出了一种有效的训练免费后剪枝校正框架，通过轻量级适配器近似被剪枝块的残差更新，成功缓解了层移除带来的分布不匹配问题，且可与参数高效微调结合进一步提升性能。
...
TL;DR: RefLAM是一个将手稿页面图像和干净转录文本转换为经过验证的行级真值的管道,用于阿拉伯语手写文本识别(HTR)训练数据标注。该方法结合了深度学习页面分割、多模态大语言模型(MLLM)和音调符号无关的模糊对齐引擎,并发布AraMS-28k数据集。


<details>
  <summary>Details</summary>
Motivation: 现有构建阿拉伯语手写文本识别训练数据的方法要么依赖无法扩展的完全手动标注,要么使用尚未扩展到多脚本、双区域(正文+边注)手稿布局的自动OCR对齐方法,且缺乏可证明的正确性保证。

Method: RefLAM管道包含三个核心组件:1)深度学习页面分割模型;2)用于结构化OCR的多模态大语言模型(MLLM);3)音调符号无关的模糊对齐引擎,可将每个OCR行锚定在参考文本的连续跨度中,并提供0-100的字符级置信度分数。完美分数(100分)在数学上等同于规范化字符串的逐字符一致性(置信度100规则),并在发布的数据集中得到验证。

Result: 在7本经过完全页面验证的书籍上测量显示,与手动标注相比实现了75倍吞吐量提升(3,000 vs 40行/小时);对另外7本书应用相同保证,在一周内保留了16,533个置信度100的正文行,排除了低于100的分数而不是手动纠正。发布AraMS-28k:14本历史阿拉伯手稿书籍、3,043页、27,971个正文行和629个边注行标注,包含边界框、布局标签和191个边注条目(30.4%)的插入锚点。在AraMS-28k上微调Muharaf预训练基线(包括HATFormer)并报告CER结果,证实了其对下游HTR训练的实用价值。

Conclusion: RefLAM通过人机协作方式实现了高效、可验证的阿拉伯手稿行级标注,置信度100规则确保了完美分数的可信度,使标注工作得到 triage,仅需关注不确定的对齐结果。该方法大幅提升了标注效率,为阿拉伯语HTR研究提供了大规模高质量训练数据。
...
TL;DR: 该研究审计了15种冻结编码器（血液学、病理学和通用视觉模型）在四个公共单细胞获取域上的准确性和校准性。研究发现域内线性探测准确率饱和（0.98-0.997），但跨数据集准确率下降34-72%，且排名发生重组。更严重的是，源域校准良好的模型在目标域出现严重的错误置信度问题（ECE从0.004升至0.35）。研究还发现预训练数据泄露问题，并提出Class-Balanced Re-standardization (CBR)方法来改善类别先验偏移下的性能。


<details>
  <summary>Details</summary>
Motivation: 血液学基础模型在域内达到接近饱和的WBC准确率，但临床部署需要跨扫描仪、站点、染色和制备流程的可靠性。现有基准测试无法评估模型在真实场景下的鲁棒性和校准能力。

Method: 对15种冻结编码器进行四大单细胞获取域的审计，采用两种探测方法（1-NN检索和线性分类头），从准确性鲁棒性和校准性两个维度评估，并分析预训练数据泄露和类别先验偏移的影响。

Result: 域内准确率饱和（macro-F1 0.98-0.997），但跨域准确率下降34-72%，DinoBloom-L从第1位跌至第10位。校准也严重崩溃：源域ECE仅0.004， off-domain达0.35。1-NN检索比线性头更稳定（中位数ρ 0.65 vs 0.45）。CBR方法可改善类别先验偏移下的性能和部分校准问题。

Conclusion: 血液学基础模型基准测试必须同时审计准确性、校准、预训练暴露和类别先验鲁棒性。当前基准存在预训练数据泄露问题，需设计更严格的评估协议。CBR作为训练无关的方法可提供一定改善，但不能完全解决校准问题。
...
TL;DR: 研究对四种AV同步指标（AV-Align、ImageBind AV-relevance、JavisScore、Synchformer/DeSync）进行联合审计，发现各指标在不同维度表现各异，无单一最优指标，建议以可靠性卡片形式报告AV同步指标。


<details>
  <summary>Details</summary>
Motivation: 当前自动AV同步指标广泛用于音视频生成器的排名和训练，但这些指标作为测量工具很少被审计，研究旨在评估其可靠性。

Method: 采用统一可靠性协议对四种指标进行审计，评估维度包括：受控失真单调性、预处理敏感性、排名不确定性、跨指标一致性、PEAVS代理一致性及学习融合。

Result: Synchformer/DeSync是最强的时间偏移追踪器（τ=0.84）；ImageBind/JavisScore更匹配PEAVS人类对齐代理（τ=0.20）和内容干扰类别；AV-Align是最弱的独立指标；各指标相互分歧（Krippendorff α=0.066）；线性或k-NN融合均未能提升PEAVS一致性。

Conclusion: 建议将AV同步报告为可靠性卡片形式（包含置信区间的指标家族分解），而非单一同步分数。
...
TL;DR: 本文提出了GLAD框架，解决多视图异常检测中的跨视图信息泄漏问题。该问题是指在基于重构的多视图异常检测中，完整视图的正常线索会帮助解码器重构异常区域，从而破坏了检测器依赖的重构差距。


<details>
  <summary>Details</summary>
Motivation: 当前多视图异常检测方法简单融合跨视图信息，但作者发现这反而会损害检测性能。具体而言，当多个检查视图被 naive 地融合到重构流程中时，来自完整视图的正常线索会传播到解码器，解码器会忠实地重构异常区域，从而瓦解了检测器所依赖的重构差距。作者将这种失败模式称为"跨视图信息泄漏"。

Method: 提出GLAD(Global-Local Attention Driven)框架，是首个结合视觉基础模型特征与局部和全局跨视图融合的多视图异常检测框架。主要包含两个模块：1) 多视图融合注意力(MMA)模块：以O(N)复杂度进行局部跨视图融合，具有可学习的视图重要性加权和逐标记门控；2) 目标引导注意力(OGA)模块：通过聚合所有视图的类标记来捕获全局上下文，并通过温度缩放的sigmoid门控将其广播回标记，以替换原始标记表示而非添加残差，从而保持重构差距。

Result: 在Real-IAD和MANTA-Tiny数据集上的实验表明，GLAD在样本级、图像级和像素级指标上均优于最先进的方法。

Conclusion: 有效的多视图融合必须明确限制到达解码器的信息，原则性的信息限制是多视图异常推理的关键。GLAD框架通过局部和全局跨视图融合机制，在保持重构差距的同时实现了精确的异常检测。
...
TL;DR: YOLOEZ是一个开源的、基于GUI的工具，用于端到端的YOLO模型应用，集成了数据标注、训练和推理功能，使非编程用户也能开发高性能缺陷检测模型，降低了AI在结构健康监测中的使用门槛。


<details>
  <summary>Details</summary>
Motivation: 结构健康监测(SHM)对于现代工程的维护和决策至关重要。传统方法依赖人工目视检测，早期计算机视觉方法虽试图自动化但对噪声、成像变化和多尺度缺陷敏感，限制了其可靠性。虽然CNN和YOLO等深度学习方法提高了检测精度，但由于数据标注、模型训练和部署等技术障碍，在SHM领域的采用仍然有限，且通常需要编程专业知识。

Method: 提出了YOLOEZ，一个开源的、基于图形用户界面(GUI)的端到端YOLO模型应用工具。YOLOEZ将数据标注、训练和推理整合到单一界面中，支持无代码开发高性能模型，同时支持可重复的工作流程。

Result: 评估结果表明，YOLOEZ不仅在大多数检测指标上优于传统方法，还降低了其他现代计算机视觉工具中存在的采用障碍。

Conclusion: YOLOEZ通过结合准确性和可访问性，有助于推动AI驱动的监测在预测性维护、数字孪生和智能结构系统中的更广泛应用。
...
TL;DR: 本文提出了一个用于微移动系统（micromobility）的人行道路径提取的单目视觉管道，经过三次设计迭代：基于骨架图的基线、距离变换走廊规划、轻量级图像空间架构，并对BEV和图像空间域的五种路径规划方法进行了系统比较。最终推荐的架构在CPU上每帧运行时间低于50ms，适合嵌入式行人速度微移动系统。


<details>
  <summary>Details</summary>
Motivation: 人行道尺度的路径提取需要在紧凑、低功耗硬件上可靠运行，且在杂乱、地图稀疏的环境中需要感知和规划能力。

Method: 1) 使用SegFormer-B0学生模型，通过半监督教师-学生框架和OneFormer Swin-L伪标签进行训练；2) 系统比较了五种路径规划方法（BEV和图像空间域）；3) 三次设计迭代：从骨架图基线到距离变换走廊规划再到轻量级图像空间架构。

Result: 1) SegFormer-B0达到0.946的IoU（11.7ms/帧），优于基线（0.758 IoU，18.9ms）；2) 图像空间中间点规划达到最低横向中心误差（14.3px），速度比BEV距离变换规划快421倍（2.2ms vs 926.8ms），同时保持相当的掩码路径对齐（98.5% vs 98.6%）；3) 全视频 replay显示时间不稳定从1.46%降至0.33%，模板路径可用性从73.7%升至79.3%；4) BEV路径提取在单目设置中脆弱：99.3%的帧无法产生有效BEV路径。

Conclusion: 推荐的最终架构（图像空间中间点为主、图像空间距离变换为备选、BEV仅用于可视化）在CPU上每帧感知到路径的完整堆栈运行时间低于50ms，适合嵌入式行人速度微移动系统。
...
TL;DR: 研究医学视觉-语言模型(VLM)在领域、配对监督或评估协议变化时的失败模式，将其视为与认知智能相关的表征级盲点。使用NIH ChestXray14和CheXpert进行跨数据集视觉迁移评估，发现自监督视觉初始化优于ImageNet监督初始化，对抗适应仅在狭窄范围内有效且不稳定。多模态精确配对检索在外部测试中表现较低，源代理信息仍可从学习表征中恢复。总体表明单一协议下的表面能力可能掩盖迁移、对齐和捷径相关的失败模式，需要在分布偏移下进行压力测试评估。


<details>
  <summary>Details</summary>
Motivation: 医学VLMs在领域内可能表现可靠，但在 acquisition domain、配对监督或评估协议变化时失败。作者将此失败模式研究为表征级别的盲点，与 epistemic intelligence（认知智能）相关，目标是揭示表面可靠性下隐藏的迁移失败模式。

Method: 1) 使用NIH ChestXray14和CheXpert进行源_only跨数据集视觉迁移（无监督域适应诊断）；2) 使用PadChest和OpenI在严格配对索引检索下评估多模态对齐；3) 量化冻结嵌入中保留的元数据派生源代理信息；4) 比较自监督与ImageNet监督初始化；5) 测试对抗适应的有效性；6) 进行定性最近邻和Grad-CAM分析

Result: 1) 自监督视觉初始化在NIH到CheXpert迁移中优于监督ImageNet初始化；2) 对抗适应仅在狭窄范围内有用，增大压力时变得不稳定；3) 外部OpenI压力测试下多模态精确配对检索仍然较低；4) 源代理信息仍可从学习表征中恢复；5) 定性分析显示许多病例有临床合理的跨数据集结构和胸腔注意力模式，但设备密集型和假阳性病例仍模糊；6) 辅助架构检查任务依赖，不支持通用主干网排名

Conclusion: 单一评估协议下的表面能力可能掩盖迁移、对齐和捷径相关的失败模式。研究强调了需要在分布偏移下对医学VLMs进行压力测试评估，以揭示隐藏的失败模式并提高模型的鲁棒性和可靠性。
...
TL;DR: OpenCVL是一个大规模、开放的数据集，包含617,388对地面-航空图像，跨越41个城市，用于细粒度跨视图定位（CVL）任务，旨在解决现有数据集依赖高端传感器导致图像多样性和可扩展性受限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有CVL数据集依赖高端传感器套件采集，图像多样性和可扩展性受限；而野生图像虽丰富，但其噪声地理标签不适合可靠评估。需要一个大规模、多样化且开放的数据集来推动该领域研究。

Method: 1) 从许可平台收集617,388对地面-航空图像，跨越4个欧洲国家的41个城市；2) 开发数据过滤和姿态标注校正框架，构建可靠的野生评估数据；3) 训练集结合高端传感器图像与多样化野生图像；4) 设计跨区域和雪天测试集评估泛化能力和鲁棒性。

Result: 实验表明，将噪声野生数据纳入训练可显著提升在清洁测试集上的性能，证明了使用多样化真实世界图像扩展CVL的可行性。

Conclusion: OpenCVL数据集为CVL研究提供了大规模、开放的基准，推动了使用真实世界图像进行跨视图定位的探索，表明噪声野生数据可作为有价值的训练资源。
...
TL;DR: PointRL是一个可验证的强化学习框架，用于从现有异构标注数据中学习点级视觉定位。核心思想是将边界框、分割掩码等标注转换为点定位指令，并通过外部验证器评估预测质量。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型中点坐标作为紧凑可执行的视觉定位接口很重要，但学习可靠的指向行为困难：监督空间本质上非唯一（同一目标区域内多坐标有效），多实例指令需要目标覆盖、计数一致性和重复抑制。

Method: 将边界框、掩码和实例标签转换为点定位指令，保留目标支撑、实例成员资格和集合约束作为隐藏验证器证据。奖励评估包括：可解析性、点有效性、实例覆盖、基数一致性、冗余/缺失预测。在PointArena上使用Qwen3.5-4B进行训练。

Result: 在PointArena上，PointRL将Qwen3.5-4B的总体准确率从56.11%提升至65.58%。在RoboSpatial、BLINK和Ref-Adv外部基准测试中也获得了一致的性能提升。

Conclusion: 可验证的点级反馈可能有益于空间定位任务，表明从异构标注数据学习点级定位是可行且有效的方向。
...
TL;DR: WAVE是一种新型的引导深度超分辨率方法，通过多级离散小波变换(ML-DWT)实现粗到细的特征重建，解决了传统方法中细到粗偏差导致的边缘模糊和伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有GDSR方法采用卷积层级提取RGB引导特征，存在细到粗偏差：早期层保留低层空间线索，深层抑制非真实深度边界的线索，导致边缘模糊和伪影。语义方法也存在同样问题。需要在高层级明确控制特征重建过程。

Method: 1)提出ML-DWT作为显式可解释的特征控制机制，按与小波子带生成相反的顺序消费子带和语义标记，实现粗到细重建。2)利用子带区分处理高频和低频内容，从源头过滤误导性的RGB颜色和纹理线索。3)设计结构重建和细节重建专用模块：模内交互模块处理小波子带、深度特征和语义先验的交互；语义门控模块处理高频带；可逆耦合机制融合多模态防止单一模态坍缩。

Result: 在多个基准数据集上进行了广泛实验，WAVE达到或超越现有方法性能，在高上采样因子下提升最大，此时低分辨率深度包含最少结构信息。

Conclusion: WAVE通过显式小波变换机制提供了直观且可解释的特征控制，有效解决了深度超分辨率中边缘模糊和伪影问题，特别是在高倍率场景下表现优异。
...
TL;DR: MULVEC是一种无需训练的零样本组合图像检索方法，通过角色感知的结构化查询记录，将检索任务分解为四个角色：全局描述、期望保留、应保留和应禁止，从而在细粒度层面改进图像检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将目标图像作为整体描述并与全局图像表示匹配，这种全局匹配会混合不同的语义线索并丢失细粒度细节。

Method: 提出角色感知方法MULVEC，其编译器生成结构化查询记录，映射到四个检索角色：Global（全局描述）、Desired（期望出现）、Preserve（应保留）、Forbidden（应消失）。冻结编码器将查询映射到一个目标描述向量和角色特定探针向量，每个候选图像用一个全局视觉向量和一组局部视觉向量表示。

Result: 在CIRCO、CIRR和FashionIQ数据集以及三种骨干网络规模上，MULVEC相比最强比较方法在CIRCO mAP@5上提升高达23.0%，并在CIRR和FashionIQ上取得最佳结果。

Conclusion: 通过角色感知的结构化查询方法，MULVEC成功解决了全局匹配导致的语义混合和细节丢失问题，实现了显著的性能提升。
...
TL;DR: V-Link是一种新型视觉-语言-动作(VLA)模型架构，通过在视觉语言模型(VLM)到动作(DiT)的特征传输过程中恢复视觉表示来解决VLA架构中动作专家对3D几何和2D语义信息访问受限的问题。


<details>
  <summary>Details</summary>
Motivation: VLA架构的动作专家对VLM特征中可用的3D几何和2D语义信息访问有限，这种可访问性差距削弱了感知基础，影响了细粒度机器人操作的性能。

Method: V-Link在VLM内学习互补的空间查询(Spatial Queries)和语义查询(Semantic Queries)表示，并通过非对称路径将它们注入到Action DiT中。语义查询补充原始VLM图像标记，空间查询为空间基础的动作生成提供专门的几何条件。

Result: 在LIBERO、LIBERO-Plus和RoboTwin 2.0基准上，V-Link相比基线模型GR00T N1.6分别提升平均成功率+1.9%、+31.2%和+18.8%。在AGIBOT A3 Ultra真实机器人任务上分别提升+20%和+24%。

Conclusion: V-Link通过显式恢复视觉表示，有效弥合了VLM到动作特征传输过程中的可访问性差距，显著提升了细粒度机器人操作性能。
...
TL;DR: 本文提出ProViP，一种无需训练的非线性视觉token剪枝框架，通过发现少数注意力头能更准确地识别关键视觉token，在LLaVA-1.5-7B上仅保留5.1%的视觉token即可达到95.9%的原始性能，并实现1.62倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLM)在各种视觉场景表现出色，但生成大量视觉token导致推理时产生巨大的内存和计算开销，增加延迟。现有的剪枝方法聚合所有注意力头的分数来估计token重要性，但本文发现只有少数注意力头具备准确识别关键视觉token的能力。

Method: 提出ProViP框架，包含两个阶段：1）推理前基于嵌入相似度移除冗余视觉token；2）推理期间使用头感知剪枝进一步剪枝token，仅利用少数关键注意力头的聚合分数进行重要性评估。

Result: 在LLaVA-1.5-7B上，ProViP在88.9%剪枝率下保留95.9%原始性能，推理速度提升1.62倍。

Conclusion: 通过识别少数关键注意力头并进行渐进式剪枝，ProViP在保持高性能的同时显著提升了VLM推理效率，为高效视觉语言模型提供了有效方案。
...
TL;DR: GraftSR是一种基于扩散的真实世界图像超分辨率框架，通过利用相同实例的参考图像来恢复真实纹理，解决 diffusion model 严重的纹理幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的真实世界图像超分辨率虽然能取得令人印象深刻的感知质量，但存在严重的纹理幻觉问题。严重的空间错位给低质量输入与其参考图像之间的对齐带来挑战，往往导致模糊的传输目标和背景特征泄漏。

Method: 提出了一种新颖的双掩码参考引导机制，系统性地解耦跨视图纹理注入过程。通过明确隔离从参考图像中提取的真实纹理，并精确定位其在目标图像中的应用位置，实现稳健的纹理传输。同时构建了TexRefSR-141K数据集，这是首个提供高质量参考图像对及互补空间掩码的大规模数据集。

Result: GraftSR在新建的基准测试TexRefSR-Eval上设定了新的最先进水平。相较于最佳基线方法，LPIPS降低了20.2%，实现了卓越的参考保真恢复效果。

Conclusion: GraftSR无需依赖脆弱的空间对齐即可实现稳健的纹理传输，为参考引导的图像超分辨率任务提供了新的解决方案。
...
TL;DR: 本文提出CoRE框架，通过弱监督方式从粗粒度的视频级标注学习细粒度的时序和实体支持证据，无需昂贵的时间和实体级别标注即可实现精细预测。


<details>
  <summary>Details</summary>
Motivation: 驾驶中的感知风险随时间演变并由特定场景实体支持，但监督通常仅限于粗粒度的视频级判断。学习何时出现支持证据以及哪些实体支持风险预测通常需要昂贵的时序和实体级标注。

Method: CoRE首先训练视频级预测器并冻结它。然后对候选时序区域或实体轨迹进行结构化干预，测量每个候选如何改变粗粒度预测，产生分级预测效果目标。这些目标被蒸馏到一个学生模型中，使其能够直接从原始视频预测时序和实体支持，无需在推理时进行干预。

Result: 在三个互补设置中评估：RISEE测试主观片段级判断的感知风险支持；DoTA提供独立时序事件注解用于评估弱监督交通异常定位；UCF-Crime测试相同粗到细机制是否扩展到标准非驾驶异常检测基准。CoRE从粗监督中学习到信息丰富的细粒度支持，在DoTA上具有强大的时序定位能力，在UCF-Crime上具有竞争力。

Conclusion: 粗粒度视频预测可以为恢复支持它们的细粒度证据提供有用的监督，无需相应的细粒度标签。
...
TL;DR: 本文提出Clue-OPSD框架,利用线索间隔作为特权监督信号,通过自蒸馏方式提升长视频理解能力。该方法在推理时无需线索标注或额外模块,在多个长视频理解基准上相比基础模型取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 随着视频变长,全视频上下文包含更多与问题无关的时间内容,会分散模型对回答问题所需证据的注意力,导致预测准确率下降。

Method: 提出Clue-OPSD(线索特权策略自蒸馏框架)。训练时,全视频学生模型从基于线索间隔的自教师模型学习,通过对齐学生生成轨迹的下一个token分布实现自蒸馏。推理时无需线索标注或额外模块。

Result: 在多个长视频理解基准和Qwen3.5不同模型规模下,相比使用完整视频,聚焦于包含问题相关证据的短线索间隔能一致提高预测准确率,同时需要更少的输入帧。

Conclusion: Clue-OPSD无需依赖ground-truth答案标签或推理时的额外模块,即可利用线索间隔作为特权监督,在多个基准上相比基础模型和监督后训练基线方法均展现出优异的性能。
...
TL;DR: FlashNormal是一种基于扩散模型的表面法线估计方法，利用闪光/无闪光图像对实现高质量表面细节恢复和形状-反射去歧。


<details>
  <summary>Details</summary>
Motivation: 现有单图像方法难以恢复精细表面细节且对形状-反射固有歧义敏感；光度立体方法虽效果好但需要多光照采集 setup，不够实用。因此需要一种既实用又能恢复高质量表面法线的方法。

Method: 提出FlashNormal方法：(1)利用闪光灯引起的阴影变化获取额外几何信息；(2)采用曲率引导的细节增强策略提升表面细节恢复；(3)基于扩散模型进行表面法线估计。同时构建EvalFlash数据集，包含20个物体及对应真值表面法线。

Result: 在EvalFlash数据集上，FlashNormal显著优于现有最先进单图像方法和闪光/无闪光法线估计方法。

Conclusion: FlashNormal在保持智能手机实用性的同时，有效提升了表面细节恢复能力并缓解了形状-反射歧义问题，为移动端高质量表面重建提供了新方案。
...
TL;DR: 本文创建了RGB-Sonar多模态目标检测数据集RSFusion，并提出RSFusionDet检测器，通过交叉注意力融合模块和目标匹配头解决水下RGB-Sonar图像空间对齐和跨模态目标匹配问题，在RSFusion数据集上实现了76.4/48.6 AP的检测性能。


<details>
  <summary>Details</summary>
Motivation: 水下单模态目标检测面临挑战：光学图像受水下噪声和可视距离限制，声纳图像缺乏目标结构信息。RGB和声纳图像具有互补信息，但缺乏公开的多模态数据集和评估基准。

Method: 构建RSFusion数据集；提出RSFusionDet检测器，包含CAFusion交叉注意力融合模块解决空间对齐问题，OMHead目标匹配头和OMLoss损失函数实现跨模态目标匹配。

Result: RSFusionDet在RSFusion数据集上达到76.4/48.6 AP（RGB/声纳），目标匹配F1-Score为83.4，优于其他目标检测模型。与DINO基线相比，RGB提升0.7 AP，声纳提升1.4 AP。

Conclusion: 提出的RGB-Sonar多模态目标检测框架有效融合了光学和声纳图像的互补信息，CAFusion模块和OMHead匹配机制能有效解决空间对齐和跨模态匹配问题，为水下多模态目标检测提供了新的研究基准。
...
TL;DR: 本文识别了Vision MoE（视觉专家混合）架构中一个被忽视的攻击面——批次依赖的执行行为，并提出了一种隐蔽的供应链后门攻击方法。该攻击利用容量边界令牌调度机制，通过三阶段框架在小型批次审计时保持休眠，在大型批次部署时激活。


<details>
  <summary>Details</summary>
Motivation: Vision MoE架构采用容量有界的令牌分发机制以确保计算可扩展性，每个专家的处理预算取决于推理批次大小。作者发现这种批次依赖的行为是一个被忽视的攻击面，需要引起安全重视。

Method: 提出三阶段框架：1）在早期MoE层注入后门；2）在更深层的MoE层训练一个中和器，在正常容量下抑制后门；3）配置批次自适应容量因子，对小批次保持高容量，对大批次降低容量，从而在大批次下通过令牌溢出自然禁用中和器。

Result: 在V-MoE和Swin-MoE上针对ImageNet-100和GTSRB数据集的实验表明：激活模式攻击成功率为76-87%，而休眠模式ASR（攻击成功率）低于9%，同时能规避Neural Cleanse、STRIP、Fine-Pruning和Activation Clustering等防御方法。

Conclusion: 揭示了可扩展Vision MoE架构中批次依赖执行带来的基本安全风险，为未来MoE架构的安全设计提供了重要参考。
...
TL;DR: 本文提出多令牌自回归(MTAR)框架,通过多令牌预测、令牌级对比正则化和语义丢弃三种技术改进自回归图像生成,在保持高质量的同时显著提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 传统下一令牌预测(NTP)方法存在三方面问题:1)监督稀疏且短视;2)表示区分性不足;3)因对完整令牌序列进行密集计算导致训练成本高昂。

Method: MTAR包含三个核心组件:1)多令牌预测(MTP):通过联合预测多个未来令牌缓解传统NTP的稀疏性和短视问题;2)令牌级对比正则化(TCR):显式增强采样令牌表示的可分离性,提高表示区分度;3)语义丢弃(SD):作为语义感知的训练加速策略,减少低信息令牌的冗余计算,同时保留有价值的学习信号。

Result: 在ImageNet上,MTAR相比LlamaGen实现最多0.95更低的FID分数,训练速度提升39%;即使仅使用1/3的训练迭代次数,仍能达到与基线相当或更好的性能。

Conclusion: MTAR在图像生成质量与训练效率之间实现了更好的平衡,为自回归图像生成提供了一种高效且高质量的训练框架。
...
TL;DR: PIVOT是一个用于评估NeRF和3DGS等新视图合成方法的测试平台，针对机器人、无人机等实际应用中遇到的非理想条件进行评估，解决了现有基准测试过于理想化的问题。


<details>
  <summary>Details</summary>
Motivation: 当前NeRF和3DGS方法在过于理想的条件下进行评估（如优化的相机位姿、友好的轨迹等），与机器人和自动驾驶系统遇到的真实场景存在差距，需要一个更贴近实际应用场景的评估框架。

Method: 构建了多轨迹数据集PIVOT v1，包含5个真实场景，使用DJI Mini 4 Pro采集；保留测量位姿和COLMAP优化位姿两种版本；提供校准和优化后的相机内参；定义了三个基准测试系列：1)已见与未见过轨迹的新视图泛化 2)测量与优化位姿的敏感性 3)校准与优化内参的敏感性；引入有向位姿空间Chamfer距离来量化训练位姿对评估轨迹的覆盖程度。

Result: 基准测试结果显示：已见轨迹上的保留视图与未见过轨迹之间存在一致的 quality gap；对位姿来源和相机内参高度敏感。

Conclusion: PIVOT提供了一个更贴近实际应用的评估框架，揭示了现有方法在真实条件下的性能差距，为未来研究提供了有价值的基准和工具链。
...
TL;DR: 本文提出样本自适应多向量表示(SAMVR)问题，为多模态检索中的每个样本动态分配不同数量的向量，而非传统方法的固定容量。通过AdaptiveEmbed框架，结合多组对比学习(MGCL)和效用策略优化(UPO)实现样本级别的表示容量自适应分配。实验表明该方法在图像、文本、视频、音频等多模态检索任务上优于固定容量方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有样本采用固定数量的向量表示，忽视了不同样本可能需要不同的表示容量来有效检索这一问题。该研究旨在解决如何根据样本特性动态分配表示容量。

Method: 提出AdaptiveEmbed框架，包含两个核心组件：1)多组对比学习(MGCL)配合对称的集合到集合相似度(SetSim)学习结构化多向量表示；2)效用策略优化(UPO)通过边际效用分配(MUA)确定样本特定的表示容量。

Result: 在图像、文本、视频、音频等多模态检索基准数据集上的实验表明，样本自适应容量分配相比固定容量多向量表示能够获得更好的检索性能。

Conclusion: SAMVR作为一种自适应容量分配的有效范式，为多向量多模态检索中的容量分配问题提供了可行的解决方案。
...
TL;DR: LiDAR-SAM2是一个将2D视频基础模型SAM2转换为4D LiDAR领域监督源的框架，通过多视角投影和时空聚合自动生成时序一致的LiDAR标签，实现无需人工标注的高质量训练数据生成。


<details>
  <summary>Details</summary>
Motivation: 4D LiDAR分割的进展受限于数据标注成本高、难以扩展的问题，每个新任务或领域都需要重新进行密集标注，因此寻求一种无需人工标注即可自动生成高质量LiDAR训练数据的方法。

Method: 在数据层面，通过多视角投影和时空聚合将SAM2视频掩码转换为时序一致的LiDAR级标签；在模型层面，设计定制的模态接口和两阶段学习目标，将SAM2的视频分割核适应到时空LiDAR结构，实现单次点击即可获得整个序列的一致掩码跟踪。

Result: 在SemanticKITTI上生成的语义和泛视标签接近人工标注质量，仅需少量点即可达到全人工标注的效果；基于这些标签训练的模型性能接近全监督 ground-truth 训练的模型性能。

Conclusion: LiDAR-SAM2作为一种可扩展的标注工具，能够显著降低3D和4D场景理解的标注负担，为LiDAR分割领域提供了一种高效、低成本的自动标注解决方案。
...
TL;DR: 针对无人机通信塔组件细粒度分割任务，提出显著性-深度前景条件化策略结合零样本分割模型，通过构建塔架先验抑制背景干扰，在TOW-300数据集上验证了方法有效性。


<details>
  <summary>Details</summary>
Motivation: 无人机影像中通信塔组件的细粒度分割对自动化巡检至关重要，但因实例级标注有限难以开发任务专用模型；零样本分割模型虽具前景，但在杂乱场景中视觉相似的背景结构会干扰组件定位，导致实例遗漏和假阳性。

Method: 提出了一种模型无关的显著性-深度前景条件化策略，结合基于外观的显著性与单目相对深度构建粗略塔架先验以抑制无关内容；SD-Grounded-SAM进一步应用几何和深度感知的边界框细化再生成掩码，SD-SAM 3则依赖SAM 3的内部设置。

Result: 在包含340张通信塔无人机影像的TOW-300数据集上，所提策略提升了两个基线模型性能：SD-SAM 3实现最强实例分割性能，SD-Grounded-SAM产生更少假阳性。

Conclusion: 所提出的SD-Grounded-SAM和SD-SAM 3方法在TOW-300数据集上验证了有效性，SD-SAM 3实现最强实例分割性能，SD-Grounded-SAM产生更少假阳性。消融实验证实了显著性、深度和边界框细化的互补增益，提高了杂乱场景中的鲁棒性。
...
TL;DR: VGA-BenchV2是一个扩展的以人为中心视频生成基准和优化框架，用于联合评估和改进视频生成质量和美学价值，包含1,016个多样化提示词、60,000+个视频和36,000个人工标注。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准缺乏足够的以人为中心的监督，难以评估和改进视频生成模型在真实感和美学质量方面的表现，需要建立从基准构建到模型优化的完整闭环。

Method: 基于VGA-Bench的细粒度分类学（美学和生成两个维度，52个子维度），扩展人工标注至36,000个任务级标注；开发混合评估器架构：VAQA-Net用于连续美学评分，VTag-Net和VGQA-Net（基于Qwen的LVLM）用于美学标签和生成质量评估；建立评估到优化的pipeline，使用学习到的美学评估器作为强化学习的奖励模型。

Result: 在多种生成模型上展现出与人类判断的强对齐，证明了评估器的有效性。

Conclusion: VGA-BenchV2建立了从基准构建、人工监督到自动评估和模型优化的完整闭环，使视频生成器能够在真实感、美学质量和人类偏好对齐方面得到改进。
...
TL;DR: 本文评估了自动焊缝分割在RGB和偏振成像中的可行性，比较了受控实验室采集与真实非受控条件下的图像表现。CNN和Transformer架构在统一协议下进行基准测试。


<details>
  <summary>Details</summary>
Motivation: 焊接组件的视觉检测仍高度依赖人工操作经验，存在操作员间差异问题。专用机械舱制造是典型场景，亟需自动化解决方案。

Method: 使用CNN和Transformer架构进行焊缝分割，在统一阈值无关协议下基准测试。每个CNN用三个随机种子训练以分离真实效果与种子噪声。对比RGB和偏振成像在受控与非受控采集条件下的表现。

Result: 受控RGB条件下CNN模型达到0.87的mAP50，但在非受控条件下降至0.22-0.48。偏振成像结合保角几何增强在非受控RGB上达到0.93的mAP50，与受控RGB最佳结果相当但无需采集控制。Transformer模型（尤其是RF-DETR）在测试时视角变化下保持高精度，而所有CNN均崩溃。

Conclusion: 采集设置是检测系统的首要因素。Transformer在视角变化时显著优于CNN，RF-DETR最具视角鲁棒性。固定视角时小CNN足够，可变视角需用Transformer。
...
TL;DR: PAGS是一个可微分的 blind autofocusing 框架，用于光声计算机层析成像(PACT)，通过速度声适应高斯 splatting 技术解决声速(SoS)异质性导致的散焦伪影问题。


<details>
  <summary>Details</summary>
Motivation: 光声成像中，未知的声速异质性会改变声的飞行时间，在假设均匀声速重建时导致散焦伪影。现有SoS自适应方法依赖校准的声学先验或优化密集物理模型，在3D中计算昂贵且难以扩展。

Method: 提出PAGS框架：1)用稀疏高斯光声源表示初始压力场；2)用球谐探针参数化的各向异性路径平均声速(ASoS)场替代显式介质恢复；3)潜在传播场直接控制源到传感器的到达时间对齐；4)解析高斯声学投影将源表示高效映射到传感器信号；5)闭环信号域优化联合更新高斯光声源参数和ASoS场。

Result: 在模拟和物理体模数据上验证：在异质声学介质下重建清晰度提高，对稀疏视角采样具有鲁棒性，解析高斯投影提供计算优势。

Conclusion: PAGS无需校准声速先验即可从测量数据中联合优化源参数和ASoS场，为复杂介质中的高质量光声成像提供了可扩展的解决方案。
...
TL;DR: 4DStreamCtrl是一个统一的3D点轨迹表示框架,将相机运动、物体轨迹和深度融合到一个接口中,实现联合相机和物体控制、深度编辑和运动迁移的实时流式视频生成。该模型在单个高端GPU上以20 FPS运行,可生成数百帧 temporally coherent的视频。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅能捕捉运动控制的某个方面:相机参数方法只能控制视角但无法移动物体,2D轨迹方法忽略深度和遮挡,3D方法只能离线运行且长度固定。没有任何方法能结合相机和物体的3D一致性控制与实时流式生成。

Method: 1)将相机运动、物体轨迹和深度统一为单个3D点轨迹表示;2)从野外视频中挖掘3D运动监督信号,构建OpenVidHD-Motion3D数据集;3)设计轻量级几何运动头(Geometric Motion Head)并插入预训练视频扩散模型;4)将模型蒸馏为因果流式学生模型,在4个去噪步骤内生成长视频,且内存消耗与长度无关。

Result: 在运动控制精度上超越之前的相机专属、2D和离线3D方法,同时覆盖它们单独处理的各种模态。在单个高端GPU上以20 FPS运行480p视频,并在数百帧上保持时间一致性,首次实现交互式4D可控流式生成。

Conclusion: 将生成建立在显式3D几何基础上,并采用高效因果推理,为交互式世界模型指明方向,可应用于可控模拟器、实时视觉想象和嵌入式智能体等领域。
...
TL;DR: DeCO通过注意力rollout识别信息丰富的patch，使用空间多样化减少冗余覆盖，将同类区域打包成紧凑的网格图像，实现细粒度数据集蒸馏。


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法主要保留全局图像统计信息，忽略了细粒度视觉分类(FGVC)所需的关键局部证据，如对象部件、细微纹理和区域特定结构。

Method: 提出判别性证据组合(DeCO)方法：1)使用预训练TransFG教师的注意力rollout识别信息丰富的patch；2)应用空间多样化减少冗余覆盖；3)将结果区域组织成类级证据库；4)将同类区域打包成紧凑的网格合成图像。下游学生仅使用标准硬标签监督训练，无需教师logits。

Result: 在CUB-200-2011、FGVC-Aircraft和Stanford Cars数据集上，DeCO在不同IPC预算下均一致优于代表性的coreset和数据集蒸馏基线方法。

Conclusion: DeCO通过保留判别性局部证据有效提升了细粒度分类的蒸馏效果，为FGVC任务的数据集压缩提供了新思路。
...
TL;DR: 本文评估了Gaussian Splatting方法在不同水质条件下的水下3D重建性能。研究在五个公开数据集上测试了五种系统，涵盖不同浑浊度、光照衰减和颜色衰减情况。结果表明：水质清澈时SfM可注册99.5%的帧，而12 NTU浑浊度时为0%；移动人工光源下，medium-blind splatting优于medium-aware系统；带恢复预处理的开源3DGS在几何精度上优于基准测试的领先者。


<details>
  <summary>Details</summary>
Motivation: 水下环境对3D重建具有挑战性，因为水中悬浮粒子散射光、浑浊度变化、吸收取决于波长、光照很少均匀。现有的Gaussian splatting方法通常针对良好图像质量的条件开发，主要在浅水区测试，缺乏系统性的对比评估。

Method: 在五个公开的水下数据集上运行五个公开代码的系统，使用统一的协议：共享的姿态、初始化、预算和评估器。数据集代表不同的浑浊度、光照损失和颜色衰减程度。同时进行工业调查对比。

Result: 方法表现更多取决于实验设置而非架构本身。水质清澈度影响上游处理：清晰水中SfM注册99.5%的帧，12 NTU时为0%。光照几何决定中等模型是否有帮助：移动人工光源下，medium-blind splatting优于两种medium-aware系统。调查中发现基准测试的光度领先者在几何上被带恢复预处理的vanilla 3DGS超越，但这些差异未在领域报告的分数中体现。

Conclusion: 需要建立更严格的评估协议来比较水下3DGS方法。光照几何和浑浊度是决定方法有效性的关键因素，简单的vanilla 3DGS配合预处理在某些情况下优于复杂的medium-aware系统。
...
TL;DR: SE-CLIP是一种用于遥感场景分类的半监督框架，通过递归标签挖掘技术，在少量标注样本下实现CLIP模型在卫星影像上的有效适应。


<details>
  <summary>Details</summary>
Motivation: CLIP等视觉-语言模型在自然图像上表现优异，但在卫星影像上性能受限，原因在于卫星影像具有独特的特征。同时，标注样本的稀缺性进一步限制了参数高效适应技术的效果。

Method: 采用双阶段pipeline：(1) 预热阶段：使用少量标注样本进行初始训练；(2) 递归发现阶段：从无标注池中迭代筛选高置信度样本。为保证扩展支持集的完整性，采用类别平衡选择策略，防止模型被简单类别主导。

Result: 在UCM和NWPU基准数据集上，SE-CLIP显著优于现有的半监督方法。

Conclusion: 该框架为视觉-语言模型在遥感领域的适应提供了最小人工干预的有效解决方案。
...
TL;DR: SMART是一个基于多模态大语言模型(MLLM)的连续手语识别与定位联合框架，通过MLLM生成的运动描述作为辅助语义线索，在小批量训练下实现稳定的视频-文本对齐，并引入多尺度时序适配器和CSLR引导的定位模块实现手语识别与定位的统一。


<details>
  <summary>Details</summary>
Motivation: 现有连续手语识别方法依赖于句子级标注，缺乏细粒度的时序和语义指导；传统视频-文本对齐需要大 batch size，对内存密集型手语视频训练效率低下。

Method: 1) 使用MLLM生成运动描述作为辅助语义线索；2) 在小批量训练下实现稳定的视频-文本对齐；3) 引入多尺度时序适配器(Multi-Scale Temporal Adapter)在transformer编码中建模时序交互；4) 设计CSFormer模块，将识别衍生的gloss证据注入边界感知定位网络。

Result: 在PHOENIX14-T、CSL-Daily、Large-scale KSL和Disaster and Safety KSL四个手语基准数据集上，SMART在识别和定位任务上均展现出优异性能。

Conclusion: SMART统一框架使CSLR特征能够惠及定位任务，同时定位监督可补充弱CTC-based识别，实现了手语识别与定位的协同增强。
...
TL;DR: 提出PoseOFF，一种结合姿态锚定的光流表示方法，用于在人类动作执行早期进行识别，以支持人机交互中的安全、高效、自然的机器人响应。该方法通过在人体关节周围捕获局部运动信息，在语义有意义的身体位置编码局部运动动态。


<details>
  <summary>Details</summary>
Motivation: 现有的人类动作识别方法要么依赖稀疏的骨骼表示（缺乏细粒度运动线索），要么依赖密集光流（计算量大，不适合低延迟感知）。人机交互需要机器人尽早解释人类动作以实现安全、高效、自然的响应。

Method: 提出PoseOFF方法，将运动特征提取条件化为人体姿态，在语义有意义的身体位置编码局部运动动态，形成与人体运动学明确对齐的结构化运动表示。该方法无需全帧运动处理。

Result: 在多个基准数据集和骨干架构上评估PoseOFF，证明在动作预测（尤其是早期观察比例）方面有一致性改进。PoseOFF使模型能够以观察更少的动作序列达到相当或更好的性能。

Conclusion: PoseOFF等以姿态为中心的运动表示可以增强交互式机器人系统推断人类动作的能力，支持人机交互中更具响应性和预见性的行为。该方法无需全帧运动处理，适用于实时和资源受限环境。
...
TL;DR: OpenVeinNet是一个用于跨数据集和开放集评估的手指静脉验证框架，结合了动态蛇形卷积和基于图的特征建模，并引入了一种新的中心角混合损失函数来提高嵌入空间的判别质量。


<details>
  <summary>Details</summary>
Motivation: 手指静脉验证是一种有前景的生物特征认证模态，因为血管模式是内部的，难以从外部观察，且相对抵抗展示攻击。然而，在开放集设置中进行可靠验证仍然具有挑战性，因为测试身份在训练时是未见的，且未注册的探针必须在推理时被拒绝。

Method: 1) 使用动态蛇形卷积（Dynamic Snake Convolution）提取局部曲线和管状静脉结构；2) 使用图卷积主干建模静脉区域之间的长程拓扑关系；3) 引入中心角混合损失（Centroid Angular Hybrid Loss），共同促进类内紧凑性和类间角度分离。

Result: 在五个公共手指静脉数据集（FV-300、MMCBNU、FV-USM、PolyU和VERA）上进行了实验，采用留一数据集训练方法，在基于注册未知拒绝和完全主体验证协议下进行评估。结果表明OpenVeinNet实现了强大的跨数据集泛化、持续保持较低的等错误率，并在固定误接受率工作点下具有竞争力的真接受率。

Conclusion: 显式建模局部静脉几何形状、全局血管关系和角度紧凑嵌入对于开放集手指静脉验证是有效的。
...
TL;DR: ACF-Net是一种用于非对称视听细粒度视觉分类的新颖框架，针对弱对齐音视频对的细粒度识别任务。提出光流引导运动模块(OFGM)和非对称跨模态自适应融合模块(ACAF)，并构建了包含194种鸟类、11965个视频和1919个音频的BirdPro数据集。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度视觉分类数据集缺乏非严格时间和实例对应的音视频关联数据；非对称跨模态场景下音视频未严格同步甚至不对应同一主体，导致表征学习和模态对齐面临重大挑战。

Method: 1) OFGM模块：利用光流捕获运动敏感视觉线索，抑制无关背景干扰；2) ACAF模块：估计弱匹配音视频对的模态可靠性，进行不确定性感知的自适应融合；3) 构建BirdPro数据集：1919个音频 recordings + 11965个视频，覆盖194个鸟类物种。

Result: ACF-Net在融合设置和错配设置下分别比最强基线方法提升2.97%和1.92%。

Conclusion: 提出的ACF-Net框架能有效处理非对称跨模态细粒度视觉分类任务，光流引导运动模块提升了动态表征能力，不确定性感知融合提升了识别鲁棒性，BirdPro数据集为该领域研究提供了重要基准。
...
TL;DR: Video-IFBench是一个用于评估视频理解中指令跟随能力的基准测试。现有基准测试主要关注任务准确性而非指令遵循能力，该基准填补了这一空白。评估了20多个多模态大语言模型，表明视频指令跟随对当前模型仍具挑战性。


<details>
  <summary>Details</summary>
Motivation: 现实世界的视频理解不仅需要正确解释视频内容，还需要满足用户指定的多样化约束。现有的基准测试主要关注任务准确性，而非指令遵循能力，导致这一能力评估不足。

Method: 开发了包含四种指令模板的指令分类体系：单任务、多任务、选择和嵌套指令，涵盖32种任务类型和39种约束类别。采用半自动数据构建管道，结合MLLMs、程序化处理和人工验证，构建了1.5K样本。

Result: 大规模评估表明，对于包含大量约束、语义约束或复杂条件结构（需要根据视频内容选择正确分支或路径）的指令，视频指令跟随仍然具有挑战性。

Conclusion: Video-IFBench基准测试能够有效评估视频理解中的指令跟随能力，为该领域未来研究提供了重要工具。
...
TL;DR: CropCop是一个包含120个操作植物健康类别的封闭集识别系统，从117,546张审计图像中筛选出109,107张无重复的基准图像。DINOv3 ConvNeXt-Tiny达到98.51%准确率和96.87%宏F1，最终量化的22.60 MiB PTE模型达到98.46%准确率和96.23%宏F1。系统建立了强大的泄漏控制内部识别和软件运行时保真度，但未在未见过的农场、相机管道或实际Android硬件上验证性能。


<details>
  <summary>Details</summary>
Motivation: 现有植物健康评分可能因重复图像、长尾标签空间或未评估的运行时文件而显得精确，但实际上存在数据泄漏和评估不可靠的问题。需要一个经过严格审计、无数据泄漏的可靠植物健康识别系统。

Method: 从117,546张审计图像开始，在确认3,233个跨分割边界的重复关系后拒绝继承的分割，创建了109,107张图像的基准数据集，审计的信任泄漏组之间零交叉，类别最大/最小比率为151.7。训练了DINOv3 ConvNeXt-Tiny参考模型和MobileNetV4 Conv-Medium紧凑模型，采用动态激活和per-channel权重的训练后量化，生成ExecuTorch/XNNPACK PTE格式的最终模型。

Result: DINOv3 ConvNeXt-Tiny达到98.51%准确率和96.87%宏F1；MobileNetV4 Conv-Medium达到98.46%准确率和96.27%宏F1；最终22.60 MiB PTE达到98.46%准确率和96.23%宏F1。INT8图和PTE之间仅6/16,363个top-1决策改变。事后分析显示类别平衡损失较小，但水果标签子集的召回率下降比聚合准确率揭示的更大。

Conclusion: CropCop建立了强大的泄漏控制内部识别和软件运行时保真度，但未在未见过的农场、相机管道或物理Android硬件上建立性能。该工作强调了数据审计和运行时验证的重要性，但实际部署效果仍需进一步验证。
...
TL;DR: 本文提出AdaVDR，一个自适应视频深度研究智能体，能够联合理解视频内容并从开放网络检索外部知识。该智能体根据任务和自身能力选择工具，仅在中间结果不可靠时进行回溯。通过构建高质量QA训练数据和VDR-EE基准测试，结合监督微调和强化学习，显著提升了视频深度研究任务的性能。


<details>
  <summary>Details</summary>
Motivation: 视频深度研究需要同时理解视频内容和检索外部知识，但不同问题和视频需要不同的工具调用策略。不恰当的工具调用会产生错误结果，不确定的锚定和检索会增加不必要的交互成本和错误风险，导致延迟增加和推理错误。

Method: 提出AdaVDR自适应视频深度研究智能体，具备自适应工具调用和反思能力。具体包括：1）开发视频深度研究数据构建流水线，发现检索相关事件和实体，通过锚定和外部检索构建高质量QA对；2）引入模型条件工具必要性过滤，评估工具调用是否可被模型绕过；3）使用监督微调结合带冗余感知奖励的强化学习来训练模型。

Result: 在VDR-EE基准测试上，AdaVDR在评估的开源模型中表现最佳；在VideoDR上显著优于基线模型。

Conclusion: AdaVDR通过自适应工具调用和反思机制有效解决了视频深度研究中的工具选择和错误纠正问题，数据构建流水线和模型条件过滤为训练提供了高质量数据，冗余感知强化学习进一步提升了模型的适应性。
...
TL;DR: 本文提出CrossMambaTuning框架，通过整合状态空间模型与跨层交互机制，实现对预训练图像压缩模型的高效微调，降低72%参数开销的同时在多个机器视觉任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将微调模块独立插入冻结的骨干网络中，缺乏跨层协调机制，导致参数效率低且难以充分利用预训练模型的表达能力。

Method: 设计了一种配备任务特定提示和多尺度分支的高效Mamba适配器，用于同时捕获局部特征和全局依赖；引入尺度不变跨层适配器(SICA)，采用参数共享策略融合不同尺度的任务信息并减少冗余。

Result: 在多个机器视觉任务上达到SOTA性能，参数开销比现有SOTA方法降低72%。

Conclusion: CrossMambaTuning为预训练图像压缩模型的迁移学习提供了高效解决方案，验证了状态空间模型与跨层交互机制结合的有效性。
...
TL;DR: 提出MLLMCLIP框架，通过将多模态大语言模型(MLLM)的知识直接蒸馏到CLIP模型中，提升CLIP的组合性能力，避免使用合成数据的高昂成本。


<details>
  <summary>Details</summary>
Motivation: CLIP等预训练视觉语言模型在零样本识别方面表现出色，但在属性-对象和关系结构的组合性方面存在不足。现有方法通过级联大语言模型和文本到图像模型生成合成硬负样本进行训练，但这种方法会带来巨大的pipeline开销。

Method: 提出异构蒸馏框架MLLMCLIP，将生成式多模态大语言模型(MLLM)的知识直接迁移到判别式CLIP学生模型中，完全绕过合成数据。具体包括：1) 基于注意力的逐层token选择方法来解决架构不匹配问题；2) 基于CKA的蒸馏损失函数。

Result: MLLMCLIP在组合性准确率上达到最先进水平，同时在标准零样本分类和图像文本检索任务上保持一致的性能提升。

Conclusion: 特征级蒸馏不仅能增强组合性能力，还能提升通用视觉语言表示能力，证明从生成式MLLM到判别式CLIP的知识迁移是有效且可行的。
...
TL;DR: 本文提出Visual Rubrics-Based Reinforcement Learning（视觉评分表强化学习），通过将参考答案分解为原子命题，从视觉忠实度、推理一致性和指令遵循三个维度评估答案，解决多模态后训练中的信用分配问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型常产生流畅但缺乏视觉证据支撑的回答，这是因为标量结果奖励无法识别哪些视觉事实被 grounding、哪些推理步骤有效或哪些指令约束被忽视，这是多模态后训练中的信用分配失败问题。

Method: 1) 将参考答案分解为原子命题；2) 从视觉忠实度(VF)、推理一致性(RC)、指令遵循(IF)三个维度评分；3) 构建V-Rubrics 50K训练集(50,248样本)；4) 在Qwen3-VL-8B-Instruct上微调得到SFT基线；5) 使用组件级、前缀局部化的评分表奖励进行GRPO训练。

Result: 基于评分表的GRPO相比共享SFT基线和仅答案GRPO均有提升，在知识导向和视觉 grounding推理基准上提升最大。

Conclusion: 评分表是一种有用的视觉后训练奖励抽象，能够提供结构化的部分信用并定位评分表信用，从而改善多模态推理的 grounding 效果。
...
TL;DR: 本文提出Dual-Transformer架构结合Cross-Attention用于多摄像机编辑任务，在TVMCE数据集上实现56.60% Precision@0.5，使用SwinV2骨干网络达到69.65%，并验证了仅用20%数据微调即可实现编辑风格个性化。


<details>
  <summary>Details</summary>
Motivation: 多摄像机系统是现代媒体制作的基础，多摄像机编辑涉及在每个时刻选择合适的摄像机视角，这是一个关键任务。

Method: 提出Dual-Transformer架构：1)专用时序编码器处理过去帧序列以构建近期历史记忆；2)候选摄像机视角作为查询通过Cross-Attention模块查询历史上下文。使用SwinV2作为骨干网络。

Result: 模型达到56.60% Precision@0.5，相比之前最佳结果37.16%大幅提升。使用SwinV2骨干网络达到69.65% Precision@0.5。仅用20%视频数据进行微调即可提升Precision@0.5，表明数据高效的编辑风格个性化具有很强潜力。

Conclusion: Dual-Transformer架构显著超越当前最优水平。SwinV2骨干网络表现最佳。模型可仅用少量数据（20%）微调即可适应特定编辑风格，为电视节目或制作人的编辑风格个性化提供了良好前景。
...
TL;DR: 本文比较了从光栅化建筑平面图中恢复墙壁、开口和房间的两种输出方式：自回归坐标序列和基于热图检测的图组装。在相同训练网络下，检测方法在真实扫描数据上各项墙壁指标更优，而序列方法在干净矢量渲染上表现更好。两种输出表示的质量差异小于预期，融合两种输出可提升墙壁F1分数7点。


<details>
  <summary>Details</summary>
Motivation: 建筑平面图重建领域存在两种主流输出表示方法，但缺乏系统性的对比研究。作者希望澄清输出表示对最终质量的影响程度，以及两种方法是否能互补。

Method: 在相同训练网络上比较两种输出方式：1)自回归坐标序列生成；2)基于热图检测和图组装。同时探索了输出融合和条件输入两种结合策略。

Result: 检测方法在CubiCasa5K真实扫描数据上墙壁F1提升2.7-5.1分；使用开口热图解码使开口F1提升2.6倍。序列方法在干净矢量渲染上优5-8分。确定性融合使墙壁F1提升7点，但条件输入三种形式均无增益。房间中心系统与墙壁优先序列模型质量相当。

Conclusion: 输出表示对质量的影响小于通常假设，两种方法具有互补性但仅在输出层融合有效。提供了编辑成本指标、修正的CubiCasa5K标注和ResPlan-FP基准数据集。
...
TL;DR: 研究揭示CLIP模型在处理人类和AI生成绘画时，其嵌入空间中的主成分方向能够自发分离这两类图像，即使没有监督目标。研究通过可解释图像表示和梯度反演探究这一现象，发现多尺度散射提供了部分解释，但更关键的是微小的、对人类几乎不可见的图像扰动就能在CLIP嵌入空间中引起显著位移，表明CLIP对人类难以感知的图像变化高度敏感。


<details>
  <summary>Details</summary>
Motivation: 识别CLIP表征中人类与AI生成绘画自发分离的现象，并追溯这一分离从嵌入空间到图像域的视觉信息基础，而非仅利用此现象进行检测。

Method: 结合可解释图像表示与梯度反演的渐进式研究方法，使用多尺度散射等统计描述符进行鲁棒性实验，并通过对嵌入方向的反演观察微小扰动的影响。

Result: 多尺度散射是最具信息量的可解释表示，但仅能部分解释该现象；微小扰动（对人类几乎不可感知）可在CLIP主导方向上引起显著位移。

Conclusion: CLIP表征反映的视觉证据与人类可直接感知的证据存在显著差异，这一人工视觉与人类视觉之间的关系问题最终延伸至人工与人类审美判断的比较。
...
TL;DR: RefineCut是一个用于可执行视频编辑规划的紧凑开放权重规划器,通过多教师蒸馏和偏好优化训练,在视频编辑任务上达到0.924的Video-Editing Score,且推理时无需调用教师模型。


<details>
  <summary>Details</summary>
Motivation: 实际视频编辑不仅是像素生成,编辑者需要将简要说明、素材池、音乐元数据和硬约束转化为可执行的时间线。现有的工作流系统只是简单包装提示的前沿模型,缺乏对决策层的研究。

Method: 提出两阶段训练方法:第一阶段通过验证器重放多教师分支,保留验证器最佳修复作为监督信号进行蒸馏;第二阶段RefineCut-Evo让学生规划器自己评分修复结果,基于验证器和任务评分标准构建偏好对进行偏好优化。

Result: 在RefineCut-Bench数据集(3578个任务、7971个配字幕片段、499首音乐轨道)上,验证器重放蒸馏将规划器的Video-Editing Score从0.620提升至0.858,RefineCut-Evo达到0.924。该方法可迁移到Llama-3.1-8B和GLM-4-9B,8B规划器在闭合验证器循环下匹配或超越前沿教师模型。

Conclusion: RefineCut成功将视频编辑的决策层建模为可执行规划问题,通过验证器引导的蒸馏和偏好优化训练出高效的开放权重规划器,为视频编辑自动化提供了新范式。
...
TL;DR: 本文提出SeVeR框架解决 volumetric medical VQA 中多序列MRI视觉冗余问题，并发布BreMRIs-VQA乳腺癌MRI数据集（1.19M QA对）。


<details>
  <summary>Details</summary>
Motivation: Volumetric medical VQA需要处理长且冗余的3D视觉token序列，多序列MRI虽然提供多样化诊断线索，但解码器面临大量重复解剖区域的问题。

Method: 提出SeVeR选择性视觉暴露框架：1）将密集体压缩为模态级原型；2）使用变化感知门控注意力检索互补多层级证据；3）采用边际效用自一致性目标训练以抑制无帮助的检索。

Result: 在BreMRIs-VQA和公开基准上，SeVeR在判别和生成任务上均提升性能，同时显著减少视觉token暴露。

Conclusion: SeVeR通过选择性视觉暴露策略有效解决了多序列医学影像中的视觉冗余问题，为 volumetric medical VQA 提供了新的解决方案。
...
TL;DR: 本文提出MAMA-FLUX.2方法，用于从乳腺MRI的增强前图像合成增强后图像。该方法基于FLUX.2-Klein-4B模型，采用条件潜在流匹配框架，通过LoRA微调和区域训练目标实现高效适应。最佳配置为LoRA rank/α=64/64，λ_tumor=0.25，λ_stable=0.1，在肿瘤区域准确性和图像保真度之间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 动态对比增强乳腺MRI是癌症诊断和监测的核心技术，但需要使用钆基对比剂。研究者希望通过深度学习方法从增强前图像合成增强后图像，减少对对比剂的依赖。

Method: 1. 基于FLUX.2-Klein-4B构建条件潜在流匹配模型；2. 将增强前图像编码为空间条件，预测增强后目标潜在量的流场；3. 采用LoRA进行参数高效微调；4. 引入区域训练目标：全局流匹配、肿瘤区域监督和稳定前景正则化；5. 开展消融实验研究LoRA秩、强度窗口化和区域损失权重。

Result: 适度调整肿瘤和稳定前景权重可改善图像保真度与肿瘤区域准确性之间的权衡。最终模型使用LoRA rank/α=64/64, MHA_max=25, λ_tumor=0.25, λ_stable=0.1取得最佳整体平衡。

Conclusion: 紧凑的预训练整流流变压器可通过参数高效微调和任务感知的区域损失适应于对比增强MRI合成任务。
...
TL;DR: RoofDiT是一个用于2D屋顶图合成和重建的生成框架，通过扩散变换器生成屋顶顶点，边预测模块推断图拓扑，支持无条件生成、足迹条件合成和图像引导重建。


<details>
  <summary>Details</summary>
Motivation: 现有屋顶图方法依赖固定几何规则或直接重建目标，作者提出应直接对顶点-边图建模，学习条件生成先验。

Method: 两阶段设计：扩散变换器生成屋顶顶点，边预测模块推断拓扑；结合相对几何感知注意力与足迹和航空图像条件；使用对齐正则化器鼓励常见水平、垂直和对角屋顶模式。

Result: 相比扩散基线改进图生成质量；足迹条件设置中优于straight-skeleton先验；图像引导重建中获得最高边F1。

Conclusion: RoofDiT是一个有效的屋顶图生成和重建框架，在图生成质量、足迹条件合成和图像引导重建三个任务上均表现出色。
...
TL;DR: 本文提出PuRF方法，解决多标签测试时自适应(MLTTA)中的问题，通过区域净化和缓存净化提升视觉-语言模型在多标签场景下的性能，在5个数据集上实现4.05%的mAP提升。


<details>
  <summary>Details</summary>
Motivation: 现有测试时适配(TTA)方法主要针对单标签场景，而现实世界图像常含多对象。直接将基于缓存的TTA方法扩展到多标签场景面临一对多映射问题：共享全局表示纠缠共存对象，存储为类级缓存原型，导致主导标签偏差和缓存校准受损。区域级线索虽可分离类特定证据，但分布偏移下区域证据也可能不可靠。

Method: 提出PuRF方法，包含两部分：1)区域净化：识别可靠区域，为多标签识别提供全面区域线索并实现细粒度对齐；2)缓存净化：基于净化区域增强缓存表示和适应性，包括情景净化构建区分性基于区域的缓存，时间刷新促进长期缓存适应性。

Result: 实验表明PuRF始终优于最先进方法，在ViT-B/32上跨5个数据集实现4.05%的mAP显著提升。

Conclusion: PuRF通过净化驱动的策略有效解决了多标签测试时适配中的主导标签偏差和缓存校准问题，为视觉-语言模型在多标签场景下的分布偏移适应提供了有效解决方案。
...
TL;DR: 研究评估了nnU-Net深度学习模型在急性缺血性卒中(AIS)磁共振弥散加权成像(DWI)病变分割中的表现，发现基础版nnU-Net仅使用DWI图像即可实现快速准确的病变分割，Dice系数达0.84，且无需复杂预处理，推理时间满足临床需求。


<details>
  <summary>Details</summary>
Motivation: 弥散加权磁共振成像(DWI-MRI)是急性缺血性卒中(AIS)可视化与量化的金标准，尽管深度学习方法能准确分割AIS病变，但最优图像输入和模型架构尚不明确，需要评估是否能以最小预处理和临床可行的推理时间实现准确的AIS病变分割。

Method: 使用1,744例DWI病例（来自本地、国家和开放获取数据集）训练自配置nnU-Net模型，在436例测试集上评估。采用五折交叉验证评估四种实验条件：有无脑提取、DWI单独或DWI联合表观扩散系数(ADC)图像。比较两种架构：基础nnU-Net(base)和残差编码器nnU-Net(ResEnc)。以2022 ISLES挑战赛的DeepISLES集成模型作为基准。

Result: 测试集(n=436)中，基础模型中位Dice相似系数(DSC)为0.84(IQR 0.19)。基础模型六对输入配置中仅两对显示显著差异。ResEnc与基础模型相比，在DWI、DWI+脑提取、DWI+ADC输入时DSC有小幅显著改善(均p<0.02)，但DWI+ADC+脑提取无显著差异(p>0.50)。基础模型显著优于DeepISLES，尤其在较小梗死体积患者中(符号秩检验，p<0.01)。

Conclusion: 仅使用DWI图像训练的基础nnU-Net，无需预处理即可实现快速准确的AIS病变分割。这种简化方法可能有助于临床研究并支持急性卒中影像工作流程。
...
TL;DR: CloSeR是一个用于广义类别发现(GCD)的即插即用框架,通过向GCD训练中注入闭集关系知识来解决现有方法的目标冲突和表示扰动问题。


<details>
  <summary>Details</summary>
Motivation: 现有GCD方法存在三个问题:1)闭集识别和开放集发现的耦合训练导致目标冲突;2)有限标签和噪声伪标签可能干扰预训练表示的语义几何结构。

Method: 1)构建领域自适应的闭集教师:在标记数据上通过轻量级块级适配器微调,保持主干网络冻结以保留预训练先验知识。2)统一关系蒸馏(URD):将教师知识转移到下游GCD,包括全局样本-原型关系(锚定已知类语义)和局部样本-样本关系(保留邻域结构),使用独立特征路径减少优化干扰。

Result: 在CIFAR-10/100、ImageNet-100、CUB、Stanford-Cars、FGVC-Aircraft六个基准数据集上使用DINO和DINOv2主干网络进行广泛实验,均取得一致的性能提升,达到最先进水平。

Conclusion: CloSeR是一个头无关的即插即用框架,可轻松集成到参数化和非参数化GCD方法中,在保持低训练成本的同时有效解决了GCD中的目标冲突和表示扰动问题。
...
TL;DR: 本研究提出利用无监督去噪扩散概率模型(DDPM)从未标注的腹部CT扫描中提取解剖特征，将编码器权重迁移到下游分割任务。在BTCV多器官数据集上，肝脏Dice从0.75提升至0.93，肾脏从0.90提升至0.95，且在低标注数据场景下仍保持鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要像素级标注，成本高昂。传统U-Net仅学习局部纹理模式，缺乏全局解剖结构感知能力，在低数据量下容易出现边界描绘失败。因此需要一种无监督方法从大量未标注医学图像中提取解剖先验知识。

Method: 1) 在21张未标注的腹部CT扫描上训练DDPM学习结构表征；2) 将DDPM编码器权重迁移至下游U-Net分割网络；3) 在BTCV多器官数据集评估分割性能；4) 测试冻结编码器及低数据量场景下的表现。

Result: 肝脏分割：Dice从0.75±0.36提升至0.93±0.16 (p<5.33×10⁻²⁶)，ASD降低66%，HD95降低45%；肾脏分割：Dice从0.90±0.19提升至0.95±0.10 (p<4.01×10⁻¹¹)；多器官 pooled Dice达0.95±0.07，方差减少68%，边界精度提升74%；冻结编码器保留>80%性能；低数据量(50%/25%/10%标注)仍保持鲁棒：10%标注时肝脏Dice=0.89，肾脏Dice=0.71。

Conclusion: 使用未标注图像进行扩散预训练可成功嵌入解剖特征先验，将U-Net转化为具有解剖感知能力的系统。这证明了学习到的解剖先验的存在，为减少医学图像分割对大量标注数据的依赖提供了有效方案。
...
TL;DR: 提出了一种基于骨架的零样本时空动作定位的预训练策略，通过Skeleton-Language特征池化切换机制和场景混合判别对比学习来解决新动作的高标注成本问题。


<details>
  <summary>Details</summary>
Motivation: 解决零样本时空动作定位中标注成本高的问题，无需通过目标动作训练即可对人物实例进行未见动作的估计。

Method: 提出两种方法：1)Skeleton-Language特征池化切换：弱监督视觉-语言预训练机制，将预训练的池化核从视频级骨架特征聚合过渡到推理阶段的实例级特征计算；2)Scene-Mixed判别对比学习：通过MIL框架在组合场景中区分实例级动作。

Result: 在四个公开的时空动作定位和分类数据集上验证了该方法能有效克服标注限制。

Conclusion: 该方法通过弱监督预训练范式实现了高效的零样本动作定位，为降低动作识别标注成本提供了新思路。
...
TL;DR: 本文提出Difficulty-Aware Sample Allocation (DASA)框架，根据预测模糊度、训练损失、类别稀有度和边界复杂度等多维困难因素，为不同样本分配差异化的数据增强强度，提升语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法对所有训练样本均匀分配变换或仅基于单一困难信号（如损失值）进行自适应，忽略了分割困难的多维性——模糊预测、优化错误、稀有类别和复杂边界都会以不同方式使样本变得重要。

Method: DASA是一个架构无关的框架，它将预测模糊度、训练损失、类别稀有度和边界复杂度整合为标准化的困难分数，在迭代训练过程中将该分数映射到样本特定的增强强度。实验在Oxford-IIIT Pet和二值Pascal VOC数据集上使用U-Net、DeepLabV3和SegFormer-B0进行评估。

Result: 在Oxford-IIIT Pet上，DASA将DeepLabV3的mIoU从0.633提升至0.740。在二值Pascal VOC上，DASA在所有三种架构上获得最佳前景IoU。

Conclusion: 多因素困难估计是指导增强资源分配的有效实用机制，DASA在多个数据集和架构上优于标准训练和单信号自适应基线，验证了多维困难估计的价值。
...
TL;DR: LongVU-TTT是一种长视频多模态大语言模型，通过在视觉编码器和LLM之间插入卷积测试时训练(TTT)重采样器来解决有限视觉token预算导致大部分帧证据丢失的问题。该方法使用分组2D快速权重适应每个视频并在压缩前对帧特征进行上下文处理，同时使用混合均匀且变化感知的选择器保留明确的视觉证据。


<details>
  <summary>Details</summary>
Motivation: 长视频多模态大语言模型必须在有限的视觉token预算下建模时间变化，但大多数帧证据会被丢弃。需要一种方法在压缩前有效地聚合和上下文处理帧特征。

Method: 1. 在视觉编码器和LLM之间插入卷积TTT重采样器，具有因果快速权重更新
2. 分组2D快速权重适应每个视频，在压缩前对帧特征进行上下文处理
3. 混合均匀且变化感知的选择器保留明确的视觉证据用于下游推理
4. 处理最多512帧，然后将其减少到128个LLM帧

Result: TTT-Conv相比TTT-MLP在MLVU上提升高达+2.12，相比双向Mamba2提升高达+3.04。在三个基准测试中优于注意力和固定状态循环重采样器。在五个视频理解基准测试中达到竞争性性能。

Conclusion: 快速权重表现为时间聚合状态而非可靠的长期情景记忆，其优势随着证据距离增加而衰减，这验证了显式帧保留的必要性。LongVU-TTT成功实现了高效的长视频处理和理解。
...
TL;DR: 本文提出MIMONet网络，通过多尺度输入和多尺度输出架构解决显著性检测中目标尺度变化学习不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有显著性检测方法虽利用多层次特征，但输入为单尺度图像，难以学习显著目标的尺度变化知识。

Method: 提出MIMONet网络，包含三个不同分辨率图像的编码器分支实现特征交互；设计多尺度感知模块(MSP)将输入特征层分为不同分辨率子层；提出联合显著性损失(JSL)约束多个输出显著性图识别相同前景目标并清晰保留边界。

Result: 实验表明MIMONet在多个数据集上具有更强的检测能力和更好的评估分数。

Conclusion: 多尺度输入输出架构能有效提升模型对不同尺度显著目标的检测能力。
...
TL;DR: 本文提出了InteractGesture，一个模型无关的推理时方法，用于实现可空间控制的语音伴随手势生成。该方法通过可微分的RVQ-VAE解码器引导扩散采样器的目标潜在估计，通过反向传播空间控制梯度来调整采样过程中的运动潜在变量。同时提出渐进分块引导策略，解决流式生成中分块依赖导致的边界不一致问题。在BEAT2数据集上的实验表明，该方法在保持手势整体质量的同时提升了多关节空间控制能力。


<details>
  <summary>Details</summary>
Motivation: 现有语音伴随手势生成模型缺乏对单个关节的细粒度空间可控性。此外，流式共语生成中存在分块依赖问题：标准顺序推理会冻结前面的分块，阻止空间约束调整先前的轨迹并导致边界不一致。

Method: 1. InteractGesture：通过可微分的RVQ-VAE解码器引导扩散采样器的目标潜在估计，反向传播空间控制梯度以在采样过程中调整运动潜在变量。2. 渐进分块引导(Progressive Chunk Guidance)：保持一组具有交错延迟的可编辑分块潜在变量，使空间约束能够在流式生成过程中跨分块边界反向传播梯度。

Result: 在BEAT2数据集上的实验表明，InteractGesture在保持整体手势质量的同时提升了多关节空间控制能力。该方法支持多种应用场景，包括稀疏关节定位、密集关节轨迹控制和方向指向。

Conclusion: InteractGesture是一个模型无关的方法，可在推理时为现有的手势生成模型添加空间可控性，具有广泛的应用潜力。
...
TL;DR: 本文提出AdaMVS和VIB-GB两个模块来解决EgoExo proficiency estimation中多视角融合导致的性能下降问题，实现动作质量评估的 state-of-the-art。


<details>
  <summary>Details</summary>
Motivation: EgoExo proficiency estimation通过融合第一人称视角的细粒度运动线索和第三人称视角的空间上下文来评估动作质量。然而，简单添加更多外视角反而会降低性能，原因包括：(1)多视角冗余-某些视角信息有限或含有噪声，稀释了判别性线索；(2)过拟合-传统融合增加了表示复杂度，导致模型记忆视角特定模式而非学习可泛化表示。

Method: 提出两个互补模块：(1) AdaMVS - 从数据角度自适应识别和融合信息量最大的视角tokens，利用弱监督信号；(2) VIB-GB - 结合梯度混合(Gradient Blending)和变分信息瓶颈(VIB)正则化，从特征角度压缩冗余信号并抑制训练过拟合。

Result: 在EgoExo-4D和EgoExo-Fitness数据集上实验表明，方法学会了关注哪些视角以及如何融合它们，取得了新的state-of-the-art结果。

Conclusion: 通过数据驱动的视角选择和特征正则化相结合的方法，有效解决了多视角融合中的冗余和过拟合问题，为EgoExo动作质量评估提供了有效框架。
...
TL;DR: TDFNet是首个用于全景显著目标检测的三投影可变形融合网络，通过结合等距矩形投影、立方体映射投影和切线投影来缓解球面到2D投影带来的几何畸变问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于投影的全景显著目标检测方法存在几何失真问题：等距矩形投影(ERP)存在严重的极地拉伸畸变，立方体映射投影(CMP)在立方体面边界处存在不连续性，导致特征判别力下降和几何一致性受损。

Method: 提出三分支编码架构，基于ERP、CMP和切线投影同时保留全局空间连续性、局部几何细节和细粒度边界信息。核心模块包括：1)跨投影可变形注意力(CDA)模块，利用不同投影间的空间对应关系构建几何感知的采样位置；2)纬度引导融合模块(LGF)，利用球面纬度先验构建几何置信权重，并引入切线投影的低畸变语义参考进行跨投影特征细化和空间对齐。

Result: 该方法有效缓解了投影引起的几何畸变，提高了特征判别力和几何一致性，在全景显著目标检测任务中取得更好的检测性能。

Conclusion: TDFNet通过三投影融合策略，为全景显著目标检测提供了一种有效处理几何畸变的解决方案，为该领域后续研究提供了新的思路。
...
TL;DR: 本文提出SUDO框架中的AURCC（Area Under Region-wise Contamination curve）指标，用于在无需目标域标注的情况下，选择最适合特定目标医院部署的基础模型。解决了跨机构医疗数据分布偏移下，模型性能不可知的问题。


<details>
  <summary>Details</summary>
Motivation: 基础模型在跨机构部署时性能波动大，且目标域标注数据稀缺，现有方法无法在无标签情况下评估模型在目标域的表现，导致无法选择最佳部署模型。

Method: SUDO框架通过将目标域数据按预测概率分区，测量每个区域的伪标签污染度（反映类别混杂程度），聚合后得到AURCC分数。该方法无需目标域标注和微调即可评估模型。

Result: 在三种跨医院迁移场景下，对6种视觉语言模型（BioMedCLIP、CXR-CLIP、CheXzero、MedCLIP、MedImageInsight、CLIP）进行胸部X光分类测试，AURCC排名与真实排名Spearman相关系数高达0.943（p<0.05）。当源域标注数据量大时与源域准确率基线相当，数据量小时表现更优。

Conclusion: AURCC可有效实现无标签目标域的模型选择，在资源受限场景（源域数据少）尤为有价值，为医疗AI的跨机构部署提供了实用的选择依据。
...
TL;DR: 提出一种几何导向的局部算子，用于3D医学图像分割中的细长结构（如血管），可在不改变网络宏观架构的情况下提升边界分割质量并减少参数量。


<details>
  <summary>Details</summary>
Motivation: 传统的encoder-decoder网络（如U-Net）在处理细长解剖结构时存在困难：重复下采样和固定网格卷积会模糊或混叠细结构，早期错误会在多尺度间传播。血管等细长结构的一体素边界误差就可能导致分支断开，改变临床相关拓扑结构。

Method: 提出几何导向的局部算子，在单一定义下同时实现特征精化（stride 1）和分辨率降低（stride >1）。该算子在每个体素处预测局部方向和有界步长，沿这些方向对称采样，并将成对样本转换为紧凑的几何和边界线索；跨尺度共识模块在skip连接处对齐encoder和decoder特征以减少几何失配。将其替换3D U-Net中的所有stride 1和stride 2算子。

Result: 在BraTS、MSD Hepatic Vessel和TDSC-ABUS数据集上一致提升。在BraTS上：Dice从86.1提升至88.9，HD95从7.1降至6.2；在TDSC-ABUS上：HD95从39.1降至27.8。参数量从2.3M降至0.8M。还能集成到其他backbone（nnU-Net、Swin-UNETR、MedNeXt）中并获得一致性能提升。

Conclusion: 该几何导向算子能有效提升细长结构的分割质量，在边界指标上提升显著，同时大幅减少参数量，且可作为即插即用模块集成到多种网络架构中，具有良好的通用性。
...
TL;DR: FlowMoDL是一种用于高加速4D flow MRI重建的展开神经网络，直接优化解剖结构 magnitude 和相位导出的 velocity 精度。该方法在 MoDL 框架基础上，交替使用学习的 (3+1)D 时空去噪器和基于 SENSE 前向模型的共轭梯度数据一致性更新。


<details>
  <summary>Details</summary>
Motivation: 4D flow MRI 可以同时采集解剖结构和血流动力学信息，但重建时间较长。该研究旨在实现高加速（10× 至 50×）的 4D flow MRI 重建，同时保持图像质量和速度场精度。现有的深度学习方法在有限梯度步数预算下性能下降显著。

Method: 1. 基于 MoDL 框架，构建展开神经网络；2. 交替使用学习的 (3+1)D 时空去噪器和共轭梯度数据一致性更新；3. 引入双路径 conditioning 方案，使单一模型可处理 10× 至 50× 不同加速因子；4. 使用深度监督复合损失函数，明确惩罚速度 magnitude 和角度误差，并采用课程学习 schedule 稳定训练。

Result: 在多中心 CMRx4DFlow 数据集上，FlowMoDL 在所有加速因子下均优于 CG-SENSE、MoDL、FlowVN 和 FlowMRI-Net 对比方法。在 magnitude SSIM、nRMSE、相对速度误差和角度误差等指标上全面胜出，能恢复清晰的解剖细节和时间相干的速度场。更重要的是，FlowMoDL 具有卓越的梯度步效率，在有限梯度步预算下仍保持鲁棒性。

Conclusion: FlowMoDL 成功实现了一个高效、鲁棒的 4D flow MRI 重建网络，能够处理高加速因子并同时保证图像质量和速度场精度。其双路径 conditioning 设计和深度监督训练策略使其在各种加速条件下均优于现有方法，具有重要的临床应用价值。
...
TL;DR: 本文提出Socialized Detector Learning (SDL)框架，用于处理异构、类别专化的目标检测器之间的知识迁移问题，并提出Trajectory-Guided and Reciprocal Distillation (TGRD)方法，通过估计IDTD构建迁移轨迹，实现知识的渐进式整合与双向Reciprocal Transfer。


<details>
  <summary>Details</summary>
Motivation: 现有方法中，基于聚合的Socialization未明确规划迁移顺序，而渐进式多教师蒸馏虽考虑顺序但仅是单向学生增强且共享类别空间。异构检测器的知识分散在不同专家中，需要一种考虑顺序的渐进式迁移机制。

Method: 1) 提出Directed Operational Inter-Detector Transfer Difficulty (IDTD)从保留特征对齐残差中估计；2) 预计算固定分数表，贪心构建迁移轨迹；3) 沿轨迹将知识渐进整合到联合类别载体，然后通过Reciprocal Transfer返还给各专家。

Result: 在MS COCO上，使用4个异构专家和2种载体初始化设置，最终载体比同时聚合对照方法高出2.6 AP；Reciprocal检测器在之前不支持的类别上达到20.8-28.4 AP，同时保持与原专家性能在1.3 AP以内。

Conclusion: 支持顺序感知的渐进式整合后接Reciprocal Transfer是检测器社会演进的有效机制。
...
TL;DR: THA-Flow是一种条件流匹配模型，可根据术前CT直接生成三维假体几何形状，将全髋关节置换术(THA)术前规划从单一方案选择转变为基于条件概率分布的一对多问题解决。


<details>
  <summary>Details</summary>
Motivation: 传统THA术前规划通常是为患者骨骼解剖选择单一假体配置，但实际同一解剖结构可能存在多个临床合理解决方案，因此规划本质上是一个一对多问题，更适合用条件概率分布来表示。

Method: 使用AutoencoderKL模型分别压缩术前骨骼解剖和假体几何形状；使用三维UNet学习从高斯噪声到假体潜在空间的修正流，并在空间骨骼条件和可选结构化假体参数下进行训练。术后CT经刚性配准后，将实际术后假体按骨盆和股骨配准独立变换，以双通道截断符号距离场表示。

Result: 假体自编码器在验证集上达到47.11 dB的峰值信噪比和0.9964的结构相似性指数。成功生成涵盖7种主要柄型号的完整髋臼和股骨几何形状，覆盖93.4%的患者队列。重复骨骼条件采样保留组件位置、对齐和主要骨-假体界面，同时允许有限的局部几何变化。

Conclusion: THA-Flow是生成式AI在THA三维手术规划中的首次应用，为术前规划提供了一种基于条件概率分布的解决方案，能够生成多个临床合理的假体配置方案。
...
TL;DR: DEFUSE是一种基于条件扩散生成模型的通用后门检测框架，用于自监督学习编码器。该方法将后门检测重新表述为表示条件图像似然估计问题，利用语义重建来区分正常和后门编码表示。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法通常仅针对视觉SSL或视觉-语言编码器之一设计，且依赖难以满足的限制性假设（如访问无感染分布数据或预计算的伪标签），需要一种更具通用性的解决方案。

Method: 受贝叶斯后验推理启发，将后门检测重新表述为表示条件图像似然估计问题，由条件扩散生成模型参数化。通过语义重建在良好分离的表示空间中评估，利用预训练扩散模型进行微调，将数据映射到自然图像流形同时保留语义内容。

Result: DEFUSE在各种攻击设置中显著优于现有检测器，可泛化到视觉SSL和视觉-语言编码器，极大减少了对受害者编码器或攻击策略的先验知识依赖。

Conclusion: DEFUSE通过扩散语义重建为SSL编码器提供了一种有效的通用后门检测方法，在不同范式的编码器中均表现出色。
...
TL;DR: 研究探讨了三种条件策略在去噪扩散概率模型应用于每日降水统计降尺度中的表现：通道拼接、交叉注意力（可学习卷积编码器）、交叉注意力（Prithvi WxC天气基础模型冻结编码器）。交叉注意力策略在分布真实性和极端事件保留下优于简单拼接，Prithvi-WxC条件模型在数据受限场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 全球气候模型输出分辨率太粗且存在偏差，无法直接用于水文影响评估，需要进行降尺度处理。AI驱动的统计降尺度方法虽有前景，但大尺度大气预测因子如何条件化生成过程的机制尚未被深入探索。

Method: 在科罗拉多河流域，使用概率、分布、频谱和极端事件指标，在相同条件下评估三种条件策略与无条件基线模型：1) 通道拼接上采样粗预测因子；2) 使用可学习卷积编码器的交叉注意力条件化；3) 使用预训练Prithvi WxC天气基础模型冻结编码器的交叉注意力条件化。

Result: 拼接条件化实现最低的点-wise CRPS和MSE，但倾向于产生过度平滑的场，抑制高强度事件。交叉注意力条件化提供明显更好的分布真实性和频谱保真度的适度改善。对极端事件改进最大：Prithvi-WxC条件模型保留超过一半的>100mm/天事件。完整数据集训练时，可学习卷积模型表现与基础模型条件化方法相似，但计算资源需求更低。Prithvi-WxC条件模型仅用5年训练数据即可达到可比性能。

Conclusion: 交叉注意力条件化相比简单拼接在概率降水降尺度中具有优势，预训练基础模型表示在数据受限场景下可能提供优势。
...
TL;DR: 本文提出Semantic Boundary Predictor (SBP)框架，通过在反向去噪过程中一次性干预来提高合成人脸生成的公平性，无需重新训练或微调底层潜在扩散模型。


<details>
  <summary>Details</summary>
Motivation: 合成人脸生成中的 demographic imbalance 会传播到下游人脸识别系统，使公平性成为重要考量。现有公平性感知生成方法通常需要模型重新训练、架构修改或在反向扩散过程中重复引导。

Method: SBP利用时间步长解耦：学习来自后期潜在表示的线性语义边界，仅在初始噪声潜在表示处应用一次，允许剩余反向去噪过程保持不变。方法无需对底层潜在扩散模型进行重新训练或微调，也不需要外部平衡数据集。

Result: 在CelebA-HQ上展示实质性改进：性别公平性差距减少98%，二元种族减少95%，四类种族减少15%，同时保持各人口统计组的感知图像质量。

Conclusion: 由于其一次性推理策略和模型无关设计，SBP仅引入较小计算开销，可轻松集成到现有预训练潜在扩散模型中。
...
TL;DR: 本文提出了LUTSeg数据集和TiSage半监督组织分割框架，用于慢性溃疡伤口组织的量化分析，特别关注麻风病等被忽视的疾病。


<details>
  <summary>Details</summary>
Motivation: 伤口组织成分量化对慢性溃疡进展监测和治疗决策至关重要，但像素级标注成本高，多组织伤口数据集稀缺，尤其对于麻风病等被忽视的疾病。

Method: 提出TiSage半监督组织分割框架，整合冻结医学视觉语言模型的多尺度语义先验，采用师生架构设计。在LUTSeg（141张图像，39名患者，5种组织类别，由5位专家Clinicians标注）和DFUTissue数据集上评估。

Result: 在低标注量的多数场景下，TiSage相比全监督和半监督基线方法表现出改进。

Conclusion: LUTSeg是首个针对麻风病慢性溃疡的多专家标注数据集；TiSage框架结合视觉语言模型先验和师生架构，在少标注场景下有效提升伤口组织分割性能。
...
TL;DR: EigenCL是一种生理引导的对比学习框架，利用Sentinel-2 NDRE轨迹对玉米胁迫进行分期，在无重训练情况下可迁移至新地区，显著优于传统聚类方法，为精准农业决策支持系统提供可解释的胁迫诊断。


<details>
  <summary>Details</summary>
Motivation: 当前植被指数阈值或图像聚类方法难以捕捉胁迫演进过程，无法为农场决策提供有效支持。及时检测作物胁迫对维持干旱频发下的产量至关重要，需要更先进的胁迫分期方法。

Method: 提出EigenCL框架，这是一种生理引导的对比学习框架，从Sentinel-2 NDRE轨迹中分期作物胁迫。该方法：1）构建10,000个玉米NDREpatch训练集；2）利用对比学习机制学习胁迫表征；3）生成四个生理一致的胁迫聚类（健康、轻度、中度、重度）。测试时在Nebraska 2023年数据进行零样本迁移验证。

Result: EigenCL显著优于基线方法（K-Means、SimCLR、ProtoCLR及消融模型），Silhouette=0.748，DBI=0.35，CHI=49,624。胁迫聚类与玉米生长阶段对齐，严重胁迫在吐丝期（VT-R1）达到峰值；与土壤水分在0-14天滞后期相关（rho最高达0.72）；与干旱县产量异常相匹配。

Conclusion: EigenCL通过将NDRE轨迹动态嵌入对比学习，实现了早期胁迫预警和可解释的DSS输出（热图、侦察优先级、区域风险指数），超越了单日期NDRE阈值方法，支持气候智能农业的可扩展监测。该方法在无重训练情况下具有良好的迁移性，为干旱胁迫监测提供了新的技术途径。
...
TL;DR: 本文探讨视觉智能是否能成为通往通用人工智能(AGI)的路径,称为视觉通用智能(VGI)。通过汇集不同领域研究者的观点,本文旨在阐明AGI时代计算机视觉应追求的原则、视觉输入模态、基准测试、学习范式,以及视觉与语言等其他模态的关系。


<details>
  <summary>Details</summary>
Motivation: 语言领域GPT系列通过大规模文本预训练和缩放展示了向未见任务的迁移能力,作者由此提出问题:图像、视频、几何等视觉模态能涌现出什么样的智能?视觉智能是否有望成为AGI的 pathway?

Method: 本文采用多学科视角,汇集不同立场和机构的研究者观点,讨论视觉智能的多个维度:AGI时代的计算机视觉原则、视觉输入模态、基准测试、学习范式,以及视觉与语言模态的关系。

Result: 文章未提供具体实验结果,而是提出一个框架性讨论,旨在澄清视觉智能的概念,为AGI时代的计算机视觉研究指明方向。

Conclusion: 视觉智能可能成为AGI的重要路径;需要明确视觉智能的定义、原则和评估基准;视觉应作为核心模态与其他模态(尤其是语言)协同发展。
...
TL;DR: 本文提出Code World Model框架，通过将语言模型的推理和编码能力与视频模型的生成先验相结合，分离世界演化与视觉实现。该框架使用编码代理作为世界大脑，通过代码维护持久世界状态，并引入代理表示连接可执行状态与视觉生成。实验表明MiniMax-H3能够遵循基于代理的时空规范，同时保留丰富的视觉细节和动力学特征。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频的世界模型主要从视觉观测中学习动力学，揭示的是结果而非支撑世界演化的底层知识、规则和机制，难以维持持久的后果并支持连贯、开放的世界演化。

Method: 提出Code World Model框架，包含三部分：1) 编码代理作为世界大脑，推理事件及其后果，生成可执行代码维护持久世界状态并执行规则一致的演化；2) 代理表示编码帧级时空约束并编译为代理视频，作为视频模型条件生成高保真视觉观测；3. 开发数据管道从游戏和现实世界视频构建对齐的代理-观测对。

Result: 在配对游戏数据上微调后，MiniMax-H3能够遵循编码代理构建的简单交互世界的基于代理的时空规范，同时保留丰富的视觉细节和动力学特征。

Conclusion: 结合代码实现持久世界演化与视频模型实现灵活视觉实现的潜力，为开放世界模型提供了新路径。
...
TL;DR: 本文构建了Compositional AI-generated Image Defect (CO-AID)数据集,用于研究和分析文本到图像(T2I)模型在处理复杂组合因素(多个实体、多个属性)时出现的系统性缺陷。通过人工标注方式收集了651张参考图像及其对应提示词,并让29名参与者对3个T2I模型生成的图像进行多标签评估,标注缺陷类型和位置。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到图像(T2I)模型在处理复杂的组合因素(如多个实体和多个属性)时存在明显且系统性的缺陷,但尚不清楚人类如何识别这些缺陷,因此需要构建一个系统的数据集来研究和理解这一问题。

Method: 从人物、手部、物体和场景四类中手动选取651张具有复杂组合特征的参考图像,通过手动编辑ChatGPT生成的提示词来强调组合因素。将提示词输入三个T2I模型生成AI图像,进行综合主观研究以识别缺陷。每幅图像由29名参与者提供多标签评估,标注缺陷类型和位置。

Result: 实验表明,在CO-AID数据集上训练深度模型可以预测AI生成图像中的缺陷,并能优化AI图像生成，验证了数据集的可用性和有效性。

Conclusion: CO-AID数据集能够支持对T2I模型组合缺陷的深入研究,为改进图像生成质量提供了重要基准,且该数据集已开源供研究使用。
...
TL;DR: TAU-Agent是一个用于交通异常理解的智能体检索增强框架。该系统通过中央检索代理协调视频字幕工具和开放词汇跟踪工具来检索相关证据（字幕、时间间隔、物体轨迹），并结合采样视频帧和输入查询由微调的视觉语言模型进行最终推理和答案生成。在AI City Challenge 2026的三个赛道测试中分别获得第2名、第12名和第5名的成绩。


<details>
  <summary>Details</summary>
Motivation: 交通异常理解（TAU）需要模型和系统能够检测、推理和解释交通视频中的异常事件。现有方法在处理复杂交通场景和进行深层次推理方面存在挑战，需要一个能够整合多模态信息并进行智能推理的框架。

Method: 提出TAU-Agent框架，包含以下核心组件：1）中央检索代理，负责协调两个视觉感知工具；2）视频字幕工具，用于生成视频描述；3）开放词汇跟踪工具，用于追踪物体轨迹；4）监督微调的视觉语言模型，用于最终推理和答案生成。系统输入任务查询后，检索代理选择相关的字幕、时间间隔和物体轨迹证据，结合采样视频帧和查询语句，由视觉语言模型生成最终答案。

Result: 在AI City Challenge 2026的基准测试中，TAU-Agent取得如下成绩：Track 3得分0.6779（排名第2），Track 7得分0.3998（排名第12），Track 8得分67.9275（排名第5）。

Conclusion: TAU-Agent作为一个智能体检索增强框架，能够有效整合视频字幕、物体跟踪等多模态信息进行交通异常理解，在多个赛道展现了竞争力，但不同赛道的表现存在差异，表明该框架在某些场景下仍有改进空间。
...
TL;DR: 本文研究了医学影像基础模型在CT图像预测临床表型时是否读取疾病特异性发现还是走捷径（相关性）。提出Auditable CT phenotyping (ACT)方法，基于报告衍生的放射学观察进行训练评估。


<details>
  <summary>Details</summary>
Motivation: 医学影像基础模型在CT图像上表现出色，但高性能掩盖了一个关键问题：模型读取的是疾病特异性发现还是仅与诊断相关的捷径特征？

Method: 构建ACT系统，基于报告衍生的放射学观察训练，在38,317例患者上训练，挖掘376,194个观察，在25,183例保留患者上评估221个EHR表型。采用零样本标注和线性探测两种方式进行评估。

Result: ACT在零样本标注上超越五个视觉语言基线；在221个表型上超越CT-CLIP（零样本0.651 vs 0.572，线性探测0.709 vs 0.662）。关键发现：仅97个观察占据221个排名第一位；一个关于主动脉和冠状动脉钙化的短语在20个表型中排名第一（包括骨质疏松、尿路感染、抑郁症）。限制为临床医生指定的证据后，86个表型重定向到表型相关观察，准确率无损失（0.751 vs 0.741）。

Conclusion: 准确的CT基础EHR表型预测可能基于非有效证据的观察，ACT可以识别并干预这些捷径，提供可解释的医学影像AI系统。
...
TL;DR: 提出4DGS-WAM，一种基于4D Gaussian Splatting的对象中心世界动作模型，通过将2D观测提升到持久4D表示，实现静态背景复用和动态对象预测的分离。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型(WAMs)通常在2D视觉数据上操作，虽然可以达到优异的视觉质量，但缺乏单个对象的显式空间结构，并且重复处理冗余的背景内容。虽然点云可以在3D空间中表示世界，但它们难以跨视角对齐和累积。

Method: 提出一种对象中心的4D Gaussian Splatting世界动作模型(4DGS-WAM)。该方法使用显式的4DGS表示，将动态对象和静态背景分开建模。对于动态对象，使用策略模型预测未来actor动作，使用世界模型预测观测到的高斯splat的变换。静态背景不需要为未来状态重新生成，因为大部分内容已在过去的帧中观测到。

Result: 该方法在KITTI-MOT数据集上评估了短视野预测和过去重建任务。

Conclusion: 4DGS-WAM通过将2D观测提升到持久的4D表示，使先前观测到的静态内容可以在未来预测中重复使用，未来状态外推可以专注于建模动态对象的演变。在KITTI-MOT数据集上进行了短视野预测和过去重建的实验验证。
...
TL;DR: 本研究提出了一种 lesion-guided ROI 深度学习方法，用于卵巢超声病变的分类，在减少像素级标注负担的同时实现高诊断性能。在 MMOTU 数据集上达到 93.10% 准确率和 0.99 AUC，在 OUD 数据集上达到 97.56% 准确率和 0.99 AUC。


<details>
  <summary>Details</summary>
Motivation: 卵巢超声图像特征重叠且依赖专家解读，导致卵巢病变分类困难。同时，像素级病变分割需要大量标注工作，限制了深度学习方法的临床应用。

Method: 在统一框架下比较四种策略：1) 全局图像深度学习；2) 病变引导 ROI 深度学习；3) 病变轮廓深度学习；4) 轮廓放射组学结合机器学习分类器。评估四种深度学习架构（MaxViT-Tiny、Swin Transformer、EfficientNet-B7、ResNet18）和三种放射组学分类器（SVM、KNN、ANN）。

Result: 病变引导 ROI 策略表现最佳，MaxViT-Tiny 在 MMOTU 上准确率 93.10%、AUC 0.99，在 OUD 上准确率 97.56%、AUC 0.99。轮廓方法准确率相当但需要更高的标注工作量。

Conclusion: 病变引导 ROI 深度学习在诊断性能和标注效率之间提供了有效平衡，为可扩展的 AI 辅助卵巢超声分析提供了实用方案。
...
TL;DR: PANDA（原型锚定数据对齐）是一种两阶段框架，用于在推理时不需要辅助模态输入的情况下，将辅助信息迁移到主模态模型。该方法通过学习共享嵌入估计类别原型，并在所有受试者上训练主编码器，可处理任意配对率（包括零重叠）。


<details>
  <summary>Details</summary>
Motivation: 多模态医学预测中经常面临不完整配对问题：辅助模态仅对部分受试者可用，且在部署时无法假设其存在。现有方法难以处理任意配对率的情况。

Method: 两阶段框架：阶段1从配对子集学习共享嵌入并从辅助模态估计类别原型；阶段2使用交叉熵加上与冻结原型的对齐来训练主编码器。由于监督在类别原型层面定义，可容纳任意配对率。

Result: 在ADNI队列（1021名受试者）上，PANDA相对于MRI-only基线达到AUC 0.868±0.020（+7.9pp），降低1.5T CN假阳性24.3pp；最佳达AUC 0.893。在TCGA-Lung数据集上，2年OS的AUC提升3.5pp，Cox PH的C-index提升9.0pts。配对率 ablation显示在75%到5%配对率下联合锚点保持稳定。

Conclusion: PANDA提供了一种面向部署的机制，无需推理时的辅助模态即可利用不完整的辅助模态改善主模态预测，为实际临床应用提供了可行方案。
...
TL;DR: FRAME是一个两步框架，用于审计医学影像中的公平性偏差声称。第一步推导公平模型参考（fair-model reference），即在观察到的亚组规模下精确公平时差异的分布。第二步在表示空间中用两个算子测试剩余差异。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像中通常使用亚组性能差异作为公平性偏差的证据，但这些差异可能部分来自采样变化而非真正的偏差。当前方法直接移除模型编码的人口统计信息，缺少对偏差来源的精确审计。

Method: FRAME框架包含两步：第一步，基于观察到的亚组规模，在精确公平条件下推导差异分布作为参考；第二步，在表示空间中用两个算子测试剩余差异。其中一个算子在结构上无法改变组内排名。

Result: 在702,206张图像和36个编码器上，参考解释了报告的种族差异中位数41%和年龄差异中位数22%。注入人口统计可解码性保持剩余差异不变，而将组与疾病方向纠缠使种族差异从0.077增至0.118。测试的干预措施均未比随机种子变化更能改变剩余差异。应用于9项已发表研究的89个差异（6种医学影像模式），参考解释了率差异中位数25%和AUC差异中位数70%。图像-文本预训练使最差组性能提升约0.05。

Conclusion: 在选择干预措施之前应用FRAME可以区分需要机械解释的差异与当前队列规模下与采样变化兼容的差异。这为公平性审计提供了更精确的方法，避免了不必要的干预。
...
TL;DR: 本文提出UGDiff，一种基于扩散引导的单图像超分辨率新方法，通过估计重建不确定性来平衡感知质量与保真度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 感知-保真度权衡是单图像超分辨率中的基本挑战。现有的扩散方法虽能生成感知真实的图像，但难以实现高保真度；而强调保真度的方法又往往牺牲感知质量。

Method: 首先估计与高保真图像对应的潜在特征的重建不确定性；然后利用该不确定性引导扩散过程，选择性地在高不确定性区域恢复高频细节，同时在其他区域保持保真度；此外，方法还自适应地结合估计的不确定性和扩散采样器在每个时间步的后验方差来识别高不确定性区域，从而在采样后期降低对高保真图像的依赖。

Result: 实验表明，该方法在感知-保真度平衡方面表现优于现有的先进扩散超分辨率方法。

Conclusion: 通过引入不确定性引导的扩散范式，UGDiff能够自适应地平衡感知质量与保真度，为扩散超分辨率提供了一种有效的改进思路。
...
TL;DR: 研究提出在医学超声影像中，使用自然图像预训练的LPIPS感知相似度度量可能不适用，因为超声图像具有独特的散斑模式和声学特定图像统计特征。实验表明，特定领域的超声骨干模型比传统模型和自然图像模型与下游任务性能更相关。


<details>
  <summary>Details</summary>
Motivation: 自然图像预训练的模型被广泛用于计算LPIPS，但B型超声图像具有独特的散斑模式和声学特定图像统计特征，与自然图像和放射学其他图像有本质区别，因此需要领域特定的模型来测量超声数据中的感知相似度。

Method: 在不同下游任务（分类、分割、重建）上比较了使用自然图像、医学通用和超声骨干模型的LPIPS指标，并使用超声骨干模型优化LPIPS损失进行重建实验。

Result: 超声骨干模型比经典模型和自然图像模型与监督模型的下游性能更具相关性；使用超声骨干模型优化LPIPS损失在重建质量和真实感之间取得了良好的平衡。

Conclusion: LPIPS骨干模型的选择是一个重要的设计选择，超声特定模型对于超声影像的感知相似度测量至关重要，UltraPIPS库的引入为该领域提供了实用的工具。
...
TL;DR: 本文提出MyoMechanix，一个多模态生态系统，用于评估负重动作质量。该数据集包含7500+样本、20种动作、38个受试者，同步采集多视角RGB视频、3D姿态、表面肌电(sEMG)和其他生理信号，是目前最大的多模态动作质量评估基准。作者还构建了健身知识图谱(FKG)来组织动作、阶段、关键步骤、错误和纠正反馈之间的关系，开发了CUBIST系统进行分解-分析-重构，实现细粒度错误归因和反馈生成。实验表明多模态感知和结构化表示可提升性能、可解释性和错误归因，CUBIST达到最先进结果。


<details>
  <summary>Details</summary>
Motivation: 现有动作质量评估(AQA)数据集和方法主要依赖RGB和姿态等视觉输入，忽略了肌肉力学等生理动力学，且通常将动作建模为整体模式。这些局限性阻碍了细粒度、生物力学基础的反馈生成。

Method: 1)构建MyoMechanix多模态数据集，包含多视角RGB视频、3D姿态、sEMG和生理信号；2)构建健身知识图谱(FKG)组织动作、阶段、关键步骤、错误和纠正反馈的结构化关系；3)开发CUBIST(组合本体推理引擎)，采用分解-分析-重构策略进行细粒度错误归因和反馈生成；4)定义三个任务：MyoMechanix-AQA、MyoMechanix-VideoQA和MyoMechanix-Video2EMG。

Result: 多模态感知和结构化表示提升了性能、可解释性和错误归因能力；CUBIST达到最先进结果；VideoQA增强了语言基础的动作理解；Video2EMG表明视频可以成为肌电感应的可行替代方案。

Conclusion: MyoMechanix推动了技能活动理解向生物力学基础、多模态和组合推理方向发展，为健身、康复、医疗和Physical AI应用提供了新范式。
...
TL;DR: StreamPI是一个流式多模态时间建模框架，为单帧VLA模型赋予时间推理能力，无需额外参数。通过指令锚定的时间建模和随机间隔流式训练策略，实现连续观察保持和精确空间感知。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的VLA模型（如pi0.5）采用单帧范式，限制了保留过去观察和发展精确空间感知的能力，需要一种无需额外参数的时间推理能力增强方法。

Method: 1. 指令锚定的时间建模：将每个（视觉观察、语言指令）对作为原子时间单元，双向注意力实现跨模态融合，因果注意力保持自回归流式推理；2. 随机间隔流式训练策略：使用适当的帧间间隔（如每3帧）实现更快更平滑的动作执行，随机化间隔提高对帧时间扰动的鲁棒性；3. 利用LLM骨干的长度外推能力，继承预训练单帧权重。

Result: 在真实机器人任务（记忆依赖和精确感知场景）以及模拟基准LIBERO上，StreamPI在多样化任务中优于pi0.5。

Conclusion: StreamPI成功为单帧VLA模型添加了时间推理能力，无需额外参数，支持灵活的单帧和多帧推理，在机器人操作任务中展现出优于现有最先进方法的性能。
...
TL;DR: 本文提出多模态无监督持续后训练(MU-CPT)任务，利用token级视觉依赖(VD)特性解决跨模态灾难性遗忘问题。提出视觉依赖感知(VDA)框架，包含视觉约束最优传输(VC-OT)和视觉调制适应(VMA)两个组件。


<details>
  <summary>Details</summary>
Motivation: 现有无监督后训练方法对目标token均匀优化，忽视了其异质性视觉依赖(VD)。作者发现token级VD的结构扭曲可作为跨模态灾难性遗忘的指标，其固有异质性可指导新任务学习。

Method: 提出VDA框架：1)VC-OT将旧任务VD在新任务学习中的结构扭曲建模为最优传输问题，设计区域感知代价和依赖分层传输惩罚；2)VMA利用VD异质性强调视觉 grounded 的新任务学习。

Result: 在MU-CPT设置下进行广泛实验，验证了VDA框架的有效性。

Conclusion: 该方法在MU-CPT中同时维持旧任务稳定性和新任务可塑性，为部署的MLLMs从流式无标注数据持续进化提供了新思路。
...
TL;DR: VBVR-Pro是一个原生视觉推理的闭环测试平台，通过300个程序生成的任务、可验证的奖励机制和跨30+生成器的控制实验，使视觉生成本身成为推理媒介。


<details>
  <summary>Details</summary>
Motivation: 原生视觉推理将视觉状态（图像和视频）视为推理的一阶媒介，而非仅是理解输入或渲染输出。但该领域进展受限于缺乏可扩展的训练任务、可靠的反馈和跨生成基底的对照比较。

Method: 1) 任务扩展：构建300个程序化生成任务，用于训练并迁移到7个外部视觉推理基准；2) 可验证奖励：设计基于确定性任务规则的奖励评分器，替代VLM-as-a-judge范式；3) 机制研究：跨图像、视频和交错生成器进行控制模态研究。

Result: 模型在VBVR-Pro上训练后展现出强大的迁移能力，可泛化到RISE-Video、MME-CoF-Pro、BabyVision等外部基准。视频生成在需要持续时空状态跟踪的任务中表现最佳，交错生成是计算效率的替代方案。探针分析表明存在对视觉推理至关重要的视觉原生轨迹。

Conclusion: VBVR-Pro成功使原生视觉推理变得可训练、可验证、可优化和可实验控制，为视觉推理研究提供了标准化测试平台和可靠评估框架。
...
TL;DR: RefVideo-6M是一个大规模参考引导的视频编辑数据集，包含500万个视频编辑样本和100万个图像编辑样本，旨在解决现有数据集依赖自动编辑模型导致伪影、缺乏视觉参考的问题。基于该数据集训练了Ref-MoT参考引导视频编辑模型，实验表明该数据集能提供更可靠的监督信号，显著提升编辑模型的视觉质量、可控性和参考一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑数据集存在两个关键局限性：1）目标视频由自动编辑模型生成，可能引入可见伪影和不可靠的监督信号；2）大多数公开数据集主要依赖文本指令，缺乏对精确、可控保身份编辑至关重要的视觉参考。

Method: 1）构建流程：使用无伪影的真实视频作为编辑目标，通过多个编辑专家生成质量过滤的输入条件；2）提供约600万个视觉参考，涵盖多样化的参考类型和编辑场景；3）基于RefVideo-6M训练Ref-MoT参考引导视频编辑模型

Result: 1）RefVideo-6M提供比现有数据集更可靠的监督信号；2）训练出的编辑模型在视觉质量、可控性和参考一致性方面有显著提升

Conclusion: RefVideo-6M数据集能够支持训练更强大的编辑模型，推动参考引导视频编辑领域的发展。
...
TL;DR: 本文提出基于视觉语言模型的建筑平面图多粒度图表示自动构建方法，通过节点识别、边推理、文本解析和图粗化流程，在147个图书馆平面图上验证了该方法的有效性，为建筑全生命周期的设计信息利用提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 建筑平面图图像包含功能空间之间的丰富关系知识，支持设计检索、基于知识的推理和BIM全生命周期 Enrichment。然而，如何自动构建面向公共建筑的任务自适应图表示仍具有挑战性。

Method: 提出多粒度图层次(LoGs)定义，并采用视觉语言模型(VLM)自动构建LoG图，具体通过节点识别、边推理、文本解析和图粗化四个步骤。以147个全球学术图书馆平面图作为案例研究进行验证。

Result: VLM生成的图与人工标注的图高度一致(匹配节点比例≥92%)；每个平面图生成三层图的平均时间为509.3秒。中粒度图在节点级区域预测中表现最佳(Macro F1=0.647，复杂度为细粒度的65%)；粗粒度图在图级布局质量评估中最有效(Spearman ρ=0.610，复杂度为细粒度的16%)。

Conclusion: 该研究实现了从平面图图像中可扩展、无标注地提取结构化布局信息，将平面图图像转换为知识表示，推动了设计信息学发展，增强了建筑全生命周期设计信息的利用。
...
TL;DR: SIMGUIDE是一种通过将用户上下文结构化为类型化、领域特定的"Sims"块来个性化AI智能体的新方法，每个约束都配有来自过去决策的程序性示例。实验表明，这种程序性接地的方法在偏好遵循上比RAG提升7.9分，且在大规模任务上得到验证。


<details>
  <summary>Details</summary>
Motivation: 当前个性化AI智能体将用户视为单一实体，使用扁平化配置简化为提示词。这在用户在不同生活场景下持有不同优先级时会失败，尤其当这些优先级冲突时。核心问题不在于智能体缺乏用户信息，而在于用户表示的格式决定了智能体能否利用这些信息行动。

Method: 提出SIMGUIDE方法，将用户上下文结构化为类型化、领域特定的"Sims"块，每个约束都配有来自过去决策的程序性示例。构建SIMBENCH基准，包含47个偏好条件规划任务正确答案取决于激活的用户上下文。同时研究参数级适应，使用任务匹配的LoRA微调和按Sim类型路由适配器。

Result: 声明式Sim约束本身并不优于基于检索的个性化(RAG)。但程序性接地的Sims在GPT-4o上优于RAG（+7.9偏好遵循点，p=0.013），并在100个τ-bench任务上复制（GPT-4o和Claude Sonnet 4.5，p≤0.023）。参数级实验中，任务匹配的LoRA微调比未适配基础模型提升12.8 ROUGE-L点，按Sim类型路由适配器额外增加7.3点，对28%路由错误具有鲁棒性。

Conclusion: 表示格式——而非表示内容——是决定个性化AI智能体性能的首要设计变量。这一发现挑战了仅增加用户信息的传统思路，转向关注如何结构化用户表示以支持上下文敏感的行为。
...
TL;DR: 本文提出了一种名为LLM-Powered Decision Engine (LLM-DE)的混合架构，将大型语言模型与数学优化、概率预测和安全约束决策过滤相结合，用于大规模供应链决策。该框架实现端到端决策，包括需求预测、库存优化、运输路由和中断缓解。


<details>
  <summary>Details</summary>
Motivation: 当前大规模供应链高度不确定、动态且易受干扰，传统基于规则和纯优化系统难以做出及时且有韧性的决策。异构数据源（如交易需求信号和非结构化中断报告）的增加为智能系统提供了机遇，这些系统能够同时推理、适应和优化。

Method: 提出了一种混合架构LLM-DE，将LLM的语义推理能力与数学优化、概率预测和安全约束决策过滤相结合。该框架支持需求预测、库存优化、运输路由和中断缓解等端到端决策。

Result: 研究结果表明，结合优化和形式约束的语言推理可以产生更智能、更安全且更可扩展的供应链决策。

Conclusion: 本研究为整合LLM的运营决策系统提供了新架构、完整算法流程和数学公式化依据，为下一代智能供应链基础设施奠定了实用和理论基础，可在不确定性和大规模复杂性下可靠运行。
...
TL;DR: 系统综述52项实证研究（2017-2025），探讨AI聊天机器人在非洲心理健康领域的应用，发现大多数系统仍基于西方模型，英语设计，缺乏非洲本土化适应，并提出文化适应性数字心理健康框架（CADMH）。


<details>
  <summary>Details</summary>
Motivation: 非洲心理健康挑战因基础设施不足、污名化和专业人员短缺而长期未得到解决，需要探索低成本、可及的人工智能解决方案。

Method: 系统综述方法，纳入52项实证研究，从文化、语言和基础设施相关性角度进行批判性分析。

Result: AI聊天机器人有潜力改善心理健康服务的可及性、减轻焦虑和抑郁症状、扩展心理社会支持，但非洲本土化适应仍然有限，大多数系统仍植根于西方模型和英语设计。

Conclusion: 提出CADMH框架，整合非洲文化价值观、多语言设计、移动优先优化和伦理保障，为AI聊天机器人在非洲医疗保健中的整合提供研究、实践和政策指导。
...
### [95] [Poisoning Agentic Alpha: Adversarial Vulnerabilities Across Roles and Architectures in Multi-Agent Trading Systems](https://arxiv.org/abs/2608.24069)
...
TL;DR: 首个针对金融领域基于LLM的多智能体交易系统的系统性对抗性研究，揭示了跨角色通信中的安全威胁，发现即使面对受限的对手（仅能访问源数据和提示），对抗信号也能在系统中传播并导致财务损失，且没有任何架构是天然鲁棒的。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体交易系统正从研究原型快速部署到控制真实资产的场景。这些系统通过结构化通信协作产生交易决策，但对抗信号可以传播并转化为真实的金融损失。之前的研究假设攻击者具有系统内部特权访问权限，而本文限制对手只能访问源数据和提示，更符合实际威胁场景。

Method: 沿角色轴将交易流程分解为四个功能角色（分析师、研究员、交易员、风险管理人），为每个角色配备匹配其接口的攻击；沿结构轴评估四种通信拓扑在数据级和智能体级攻击下的表现，使用对抗信号保留分数(APS)作为后验分析工具。实验涵盖五个资产、两个主干网络、两个目标方向。

Result: 核心发现是没有任何架构是天然鲁棒的。不同通信拓扑和角色配置在对抗信号传播方面表现各异，APS分析揭示了某些设计更鲁棒的原因。这些发现为未来设计更安全、更鲁棒的交易系统提供了洞察。

Conclusion: 首次在金融领域系统性地研究了多智能体交易系统中的对抗性信号传播问题，提供了针对实际可达威胁模型的经验证据，表明需要重新思考多智能体交易系统的架构设计以提升安全性。
...
### [96] [Measurement-Budget Allocation in Quantum Learning with Finite-Shot Generalization Guarantees](https://arxiv.org/abs/2608.24891)
*Ferhat Ozgur Catak*

Main category: cs.AI

TL;DR: 研究近 term 量子硬件上二进制量子分类器的测量预算分配问题，证明了将有限样本和有限 shots 贡献分离的分布无关泛化界，并推导出最优训练状态数 n* 和每个状态的 shots 数 S* 的分配规则。


<details>
  <summary>Details</summary>
Motivation: 在近 term 量子硬件上，Born 概率的估计需要重复执行量子电路。给定固定测量预算 B，必须在训练状态数 n 和每个状态的 shots 数 S 之间做出权衡，这直接影响量子学习实验的性能。

Method: 证明分布无关泛化界，分离有限样本和有限 shots 的贡献。样本项按 √(d/n) 缩放，shots 项按 √((log n)/S) 缩放。在约束 B=nS 下最小化保守的闭式代理，得到分配规则 n* = 2√(2dB/log(2B/δ)) 和 S* = B/n*。使用 PennyLane 进行 2-qubit 和 4-qubit 变分量子电路模拟验证。

Result: 理论 bound 与经验泛化 gap 一致。最差情况速率为 B^(-1/4)。在所有测试配置中，单侧经验泛化 gap 保持在理论 bound 以下。

Conclusion: 为近 term 量子学习系统中的有限 shots 评估和实验前规划提供了保守的统计指导。扩展到完全自适应 shot-noisy 训练仍是开放问题。
...
### [97] [Federation Is Nearly Free, Reasoning Is Not: Tradeoffs for AI Co-Scientists in Protein Characterization Workflows](https://arxiv.org/abs/2608.25215)
*Maia Kapur,Timothy Boe,Abby Jerger,Paul Rigor*

Main category: cs.AI

TL;DR: 研究自主科学代理工作流中灵活性与确定性之间的权衡，比较不同LLM、RL策略和联邦拓扑结构在蛋白质功能表征任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 自然语言驱动的自主科学家工作流在灵活性和推理能力与可决定性、可重复性和可观测性之间存在根本权衡，且跨机构通信的联邦拓扑结构会影响延迟和成本。

Method: 在生产级代理平台上进行受控消融实验，比较联邦拓扑、经典RL与LLM驱动框架、语言模型和提示专家水平，针对蛋白质序列功能表征任务进行评估，并按蛋白质新颖性分层分析。

Result: LLM选择主导预测质量（Opus 92%-94% vs o4-mini 40%-50%）。PPO策略达到88%准确率，零token成本、最快延迟和完美一致性，但无推理过程。专家提示LLM达到最高准确率但成本高且一致性较低，任务越难提示依赖性越大。联邦拓扑对性能影响可忽略。

Conclusion: 对于常规可验证任务，便宜的确定性策略可实现接近前沿准确率和完全可重复性；灵活的LLM推理最适合开放性发现任务。
...
### [98] [Natural Language Input, Semantic Track Representation, and LLM Inference: Making the Maritime Information Exchange Model Tractable](https://arxiv.org/abs/2608.24892)
*Frederick Roth*

Main category: cs.AI

TL;DR: 本文提出了一种利用当前大语言模型(LLM)技术使 Maritime Information Exchange Model (MIEM) 和 Rich Semantic Track 模型实际可用的架构。该方案将操作员的自然语言输入转换为语义断言记录(SARs)，构建知识图谱，并通过第二次LLM传递进行推理、异常检测和假设排序。


<details>
  <summary>Details</summary>
Motivation: 采用语义追踪模型在国防和执法领域的主要障碍在于：操作员需要学习形式化本体语言并手动将观察结果编码为类型化逻辑断言，这限制了技术的实际应用。

Method: 提出完整流程：1)操作员以自然语言输入观察结果；2)LLM将其翻译为SARs（语义断言记录），这是一种捕获n元关系的紧凑结构；3)知识图谱积累SARs；4)第二个LLM流程在图谱上进行推理、异常检测和假设排序。文中通过两个详细案例验证：9/11时代预攻击指标场景和海上货物检查场景。

Result: 该架构使Track Model和MIEM能够利用当前技术立即部署，确立了针对专有封闭方法的先发优势，并将方法论建立在将语义追踪表示与神经流形几何学相连接的理论框架上。

Conclusion: 该方法消除了操作员学习形式本体语言的技术障碍，使语义追踪模型在国防和执法领域具有实际部署可行性。
...
### [99] [Candidate supply and answer selection shape the value of LLM judging in multi-agent systems](https://arxiv.org/abs/2608.25937)
*Jia-Hao Ji,Sijie Li,Jiabei Cheng,Zixi She,Jin-Tai Yu,Zhiyuan Yuan*

Main category: cs.AI

TL;DR: 研究探讨了多智能体系统中正确答案已生成但仍被放弃的现象，提出通过LLM判断器提供选择压力来改善答案选择，准确率从63.82%提升至70.82-70.95%。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统(MAS)有时已生成正确答案，但仍报告错误答案。原因难以解释，因为生成、通信和最终答案选择规则同时变化。研究者将多智能体推理概念化为演化管道：候选生成、同行通信和终端选择，其中缺乏质量控制的共识表现出迷因漂移模式。

Method: 1)分析来自MMLU-Pro、GPQA、MedXpertQA、MuSR的15,336个问题，Humanity's Last Exam单独分析；2)重放16,278个问题上的81,390个固定候选池；3)研究LLM判断器何时提供有效选择压力。

Result: (1)正确答案常已存在于候选中，但系统仍收敛并报告错误答案；(2)判断器可靠性非固定特性，随任务、生成器和正确答案稀有程度变化；(3)结合答案频率与判断器评估仅改变最终选择规则，准确率从63.82%提升至70.82-70.95%，主要通过拯救被流行错误淹没的正确答案。

Conclusion: 在当前系统中，生成更多候选的价值取决于这些样本是否使正确答案存在、频繁或可识别。通过隔离生成、识别和选择，这些发现为设计保护生成正确答案的多智能体架构提供了诊断基础。
...
### [100] [Same-Player Verification for Account Consistency in Counter-Strike 2](https://arxiv.org/abs/2608.24893)
*Xuchen Zhang*

Main category: cs.AI

TL;DR: 本文提出了一种基于游戏行为指纹的同玩家验证系统，用于CS2（反恐精英2）中的账号一致性审查。该方法从比赛回放中提取玩家行为轨迹，训练模型判断两个行为观测是否来自同一真实玩家。在13,300个样本和663,590个配对上训练后，最终模型达到0.931的ROC AUC，在95%精确率下达到0.722的不同玩家召回率。特征分析表明，低级操作习惯（尤其是准星控制、射击节奏和移动-停止-射击协调）是最强的身份信号，比单场比赛表现结果更具区分力。


<details>
  <summary>Details</summary>
Motivation: 在竞技FPS游戏中，账号完整性审查经常需要判断账号的近期行为是否与历史操作者一致。这种一致性验证需求出现在临时换人、代练、高手使用低分段账号等情况。传统人工审核效率低下，需要将当前比赛与多个历史比赛进行对比。

Method: 1.将单场比赛中玩家的行为轨迹编码为"演示玩家行为指纹"
2.基于CS2游戏理解，提取5类特征：准星控制、移动-停止-射击协调、经济/购买、战斗/交火、时间节奏
3.从1,330场CS2比赛回放中提取13,300个演示玩家观测样本
4.从8,840万候选配对空间中采样663,590个相同/不同玩家配对进行监督训练
5.使用账户历史聚合方法，将K个配对结果融合提升验证准确率

Result: 最终配对模型达到0.931的平均ROC AUC；在95%精确率下达到0.722的不同玩家召回率。特征分析显示，最强的身份信号来自低级操作，尤其是准星控制、射击节奏和移动-停止-射击协调，表明稳定的低级机械习惯比单场比赛表现结果更具信息量。账户历史聚合评估中，增加历史深度使AUC从K=1的基线0.931提升到K=10的0.986。

Conclusion: CS2比赛回放行为可以支持监督学习下的同玩家验证，并通过多比赛历史聚合实现账户级身份一致性建模。该方法能够有效识别临时换人、代练、高手使用低分段账号等违规行为，具有较高的实际应用价值。
...
TL;DR: ProgRouter是一个在线进展引导的路由框架，用于多智能体LLM工作流，通过自适应选择LLM智能体来平衡任务质量与时间和成本预算。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM工作流通过协作推理解决复杂任务，但存在高运营成本问题。现有的级联路由方法仅做一次性决策，无法适应多步骤工作流的动态特性。正确选择LLM智能体需要考虑任务进度、剩余难度和成本效率要求。

Method: 提出多视角任务进度评分器，结合粗粒度工作流结果状态与细粒度子任务完成信号、进度趋势和工作流状态质量。采用双路径任务进度预测器和自适应元门控机制估计每个候选路由LLM的进度增益，实现平衡进度增益、时间预算和长期运营成本的在线逐步路由决策。

Result: 在HumanEval Plus、MBPP、MATH-500和ASQA上展开实验，涵盖智能体代码生成、数学推理和检索增强的长问答任务。结果表明ProgRouter在保持强大任务解决性能的同时降低了运营成本。

Conclusion: ProgRouter通过自适应进展引导的路由策略，有效平衡了多智能体LLM工作流中的任务解决质量与时间和成本预算。
...
TL;DR: post-graph-rag是一个开源图检索增强生成引擎，解决了现有实现的三个痛点：基础设施成本、图质量和时间演化问题。它将文本块、嵌入向量、实体图和社区摘要存储在单一PostgreSQL数据库中，通过提取时不变式确保图质量，并提供时间层支持关系时效性和 Superseded 关系。


<details>
  <summary>Details</summary>
Motivation: 当前图检索增强生成(Graph-RAG)实现存在三个问题：1)基础设施成本高，需要维护向量库、图数据库和文档存储并保持一致；2)图质量问题，提取管道输出无意义的边；3)时间演化问题，图只累积事实而不区分过时和当前事实。

Method: post-graph-rag将文本块与嵌入、规范化实体图和社区摘要存储在单一PostgreSQL数据库中，使用pgvector搜索和边表遍历。提取时不变式在写入前运行：拒绝模糊谓语、代词名称和裸数量；将谓语规范化到可选词汇表；实体通过模型提供的别名解析为单个顶点；拒绝的关系保留否定标志。时间层让关系携带有效期，支持文档顺序自动 Superseded 不兼容断言，并回答as-of查询。

Result: 在三个语料库上与LightRAG对比，post-graph-rag构建了更密集的图（最多2.4倍的关系/实体），更具可查询性（每关系唯一边标签0.46-0.58个 vs 0.77-1.33个），查询延迟更低，支持基线缺乏的时间演化功能（在novel sequence和十年 filings 上分别有13和8个关系被取代）。

Conclusion: post-graph-rag通过单一PostgreSQL数据库架构、提取时质量控制和内置时间层，有效解决了Graph-RAG的三个核心痛点。工程测量表明其在图密度、可查询性和时间处理方面优于LightRAG，是Graph-RAG领域的有效工程解决方案。
...
TL;DR: 本文介绍了FrontierChallenge基准测试，用于评估科学智能体在跨领域科学工作流中的端到端执行能力，包含300个科学工作流任务。


<details>
  <summary>Details</summary>
Motivation: 当前科学智能体基准测试主要关注最终答案、孤立程序或单一领域，缺乏对跨领域完整科学工作流的评估。

Method: 发布97个跨领域科学任务（涵盖量子化学、分子动力学、材料表征、分析化学、生命科学、电化学/环境科学），评估12个前沿模型和3种智能体框架，使用通过率（Pass Rate）和平均分（Avg. Score）衡量任务完成情况。

Result: 最佳配置仅完成20/97任务，通过率20.6%。分析化学和电化学/环境科学的部分进展转化率极低：分析化学平均分87.6但最高通过率仅4%，电化学/环境科学平均分94.9但通过率0%。非通过轨迹中75.5%仍声称完成。

Conclusion: 高部分得分或自信的完成声明不能可靠表明科学任务已完全交付，需要同时评估端到端工作流执行和科学交付物的完整性。
...
TL;DR: 本文研究具有一般omega-regular目标的鲁棒POMDPs（RPOMDPs）的求解问题，建立了(s,a)-矩形RPOMDPs与部分可观察随机博弈（POSGs）之间的语义等价性，并据此推导了RPOMDPs和RMDPs的计算复杂度边界。


<details>
  <summary>Details</summary>
Motivation: 传统POMDP假设转移概率已知，但在实际应用中转移概率往往不确定。RPOMDPs通过不确定性集合来建模这种不精确性，但如何有效求解具有omega-regular目标（如可达性、安全性、LTL等）的RPOMDPs仍是开放问题。

Method: 对于具有多面体不确定性集合的(s,a)-矩形RPOMDPs，证明可以将其求解问题规约为POSGs的求解问题；同时首次实现了双向规约，建立了两种模型形式之间的语义等价性。

Result: 建立了(s,a)-矩形RPOMDPs与POSGs的语义等价性，获得了针对不同omega-regular目标的计算复杂度上下界，并作为推论获得了RMDPs的新计算复杂度结果。

Conclusion: 该研究揭示了RPOMDPs与POSGs的内在联系，为求解具有复杂目标的鲁棒决策问题提供了理论基础，并拓展了相关计算复杂性的理解。
...
TL;DR: 本文提出CVE-SAI方法，解决多模态电商产品属性模型中缺乏视觉支持验证、混淆瞬时预测与持久索引准入、缺乏事实错误或视觉不支持值的风险控制等问题。该方法通过焦点区域扭曲(FZD)构建属性特定视觉依赖代理，使用证据引导注意力重定向(EGAR)优化本体约束评分，在证据必要性、证据保留、干扰变换稳定性和候选特定目录文本冲突审计后冻结候选值。实验在Amazon Berkeley Objects的5个视觉属性上表明，该方法在属性推理、证据定位、认证准入覆盖率和受控检索性能方面均达到最优。分离推理与准入实现了视觉支持的属性补全，同时限制了持久索引污染。


<details>
  <summary>Details</summary>
Motivation: 当前多模态产品模型存在三个关键问题：1) 优化属性答案准确性时未验证视觉支持；2) 混淆瞬时预测与持久索引准入；3) 缺乏对事实错误或视觉不支持值的显式风险控制。这些问题导致电商属性索引存在污染风险。

Method: 提出Counterfactual Visual Evidence-Guided Selective Attribute Indexing (CVE-SAI)框架，包含：(1) Focus-Zone Distortion (FZD)：通过受控反事实干预构建属性特定视觉依赖代理；(2) Evidence-Guided Attention Redistribution (EGAR)：使用代理优化本体约束评分；(3) 候选冻结机制：在证据必要性、证据保留、干扰变换稳定性和目录文本冲突审计后冻结候选值；(4) 独立家族级校准：在5%不安全准入预算下选择策略。

Result: 在Amazon Berkeley Objects的5个视觉属性上，CVE-SAI在属性推理和证据定位方面取得提升，在共享风险协议下达到最高认证准入覆盖率，在自动准入系统中实现最强受控检索性能且不安全自动诱导曝光最低。

Conclusion: 将推理与准入分离能够实现视觉支持的属性补全，同时限制持久索引污染，为电商平台属性索引系统提供了有效的风险控制方法。
...
TL;DR: 本文介绍了一个生产级别的产品链接级联系统，通过分阶段处理解决大规模电商平台的产品匹配问题：检索阶段筛选候选对，轻量级文本交叉编码器自动处理高置信度案例，多模态视觉语言模型处理剩余模糊案例。


<details>
  <summary>Details</summary>
Motivation: 市场平台上数十亿条噪声产品记录需要与数千万个标准产品匹配，传统单一模型评分方案在困难案例上精度不足，在简单案例上成本过高，需要一种按难度分配计算资源的级联架构。

Method: 提出retrieve-then-match级联架构：(1)检索阶段生成候选匹配对；(2)轻量级文本交叉编码器自动处理高置信度案例；(3)代理式多模态视觉语言模型处理模糊案例，通过检查产品图片和发起网页搜索获取证据。交叉编码器从数百万双VLM共识标签蒸馏训练，在98%精度bar下自动接受链接。

Result: 交叉编码器达到98%精度验证；代理模型达到88%精度和92%召回（对比闭源前沿VLM的92%召回），但成本仅为1/7，无需微调。端到端链接覆盖率从简单阶段的68%提升到77%。

Conclusion: 按难度分配计算资源的级联架构可有效平衡精度、召回和成本。跨编码器到前沿VLM的每对成本跨越近5个数量级，仅将困难案例升级到代理模型即可显著提升覆盖率。
...
TL;DR: LifePlanner是一个用于评估LLM智能体在地理空间规划任务中的基准测试，通过整合大规模本地社交媒体帖子丰富地图数据，并通过MCP工具集提供访问。实验表明前沿LLM在简单检索任务上表现良好，但在复杂规划任务上性能急剧下降，通过率降至40.2%。


<details>
  <summary>Details</summary>
Motivation: 现有的地理空间规划基准仅提供干净的地理空间数据和工具，缺乏人们在日常规划中使用的开放性社会信号。需要一个更现实的测试平台来评估LLM智能体在多约束推理、噪声证据检索和基础工具使用方面的能力。

Method: 构建LifePlanner基准，整合大规模本地社交媒体帖子丰富地图数据，通过MCP工具集提供访问。评估套件涵盖四个任务类别和三个难度级别。

Result: 前沿LLM在简单检索任务上表现良好，但在复杂规划任务上性能急剧下降，通过率降至40.2%。失败主要源于从大型多模态数据库中获取证据不完整、工具使用不精确以及约束整合薄弱，而非模型规模或推理长度。

Conclusion: 未来进展需要有效的 grounded planning（基础化规划），而非仅仅扩大模型规模。LLM在复杂规划任务中需要改进证据获取、工具使用和约束整合能力。
...
TL;DR: SimVerity是一个判决转移保障框架,通过在目标智能家居部署中重放匹配场景并与独立认证的物理见证人交叉验证来解决模拟评估与物理部署之间的证据传递问题。研究发现模拟器清除了所有240个光照试验,但摄像头发现了42个次秒级故障,这些故障在settled-state检查中不可见。


<details>
  <summary>Details</summary>
Motivation: 模拟评估被广泛用于基准测试AI智能体,但模拟通过提供关于物理部署的证据量尚未被系统量化。部署成功是一个真实世界的过程,而非模拟中的静态属性:在同一执行中,完成状态、报告状态、可观察效果和settled结果存在分歧。

Method: SimVerity框架:在目标智能家居部署中重放匹配场景,并与独立认证的物理见证人交叉验证智能体执行。通过风险画像(从测量试验学习并在评估前锁定)预测失败,比较不同智能体配置的场景匹配率,并评估第二认证模拟器的独立交叉检查能力。

Result: 1)高级模拟器清除所有240个光照试验,但摄像头发现42个次秒级故障(settled-state检查不可见);2)风险画像可预测失败,在11个保留会话中击败属性盲基线;3)切换智能体循环的模型客户端/服务配置将其场景匹配率从52-88%提升到100%;4)第二认证模拟器未提供独立交叉检查:在重叠案例上从未不同意,只有物理测量暴露了它们的共同盲点。

Conclusion: SimVerity将判决转移转化为明确决策:清除、弃权或部署前升级。物理测量对于暴露模拟器的盲点至关重要,模拟评估不能替代真实世界验证。
...
TL;DR: 本文提出了"Borrowed Authority"（借用权限）攻击概念，指出当前Agent系统中Skill（技能）描述行为但缺乏Policy（策略）来控制行为是否被允许执行，导致恶意技能可以驱动物理伤害。作者设计了Edge Skillguard系统，在技能工件内部嵌入类型化权限层和防护机制，在边缘控制平面测试中成功拦截了60/60次借用权限攻击，且不阻塞良性请求。


<details>
  <summary>Details</summary>
Motivation: 当前Self-evolving Skill系统（如AutoSkills、Hermes Agent）仅关注效率提升，忽视了安全风险。实际问题是：Skill描述代理行为方式，但Policy决定行为是否被允许执行。生成更多Skill会扩大安全差距，特别是错误调用可能导致开门或转账等危险操作。已发现两类攻击：恶意技能入侵云软件、越狱LLM控制机器人造成物理伤害，但两者交集（恶意技能导致物理伤害）尚未被报道。

Method: 提出Edge Skillguard系统，一种嵌入在Skill工件内部的类型化权限层（而非工作流引擎的工具间防护），包含对世界状态和传感器证据的防护机制。在实时边缘控制平面测试床上实现并验证。

Result: 在五种攻击变体下，防护机制成功拒绝60/60次借用权限请求，同时不阻塞良性请求。该结果在5倍规模下和跨Tailscale mesh的多主机环境中保持有效。

Conclusion: 高风险技能应将类型化调用策略与程序化知识打包，使物理动作依赖机器可验证的证据而非对等代理的声明。
...
### [111] [PhysElite: How Far Are LLMs from Solving Olympiad-Level Physics Problems?](https://arxiv.org/abs/2608.25097)
*Ruoran Xu,Wending Gao,Liyunfeng Chen,Aixin Shi,Haoyu Cheng,Zixiang Fang,Yiqiang Zou,Qiufeng Wang*

Main category: cs.AI

TL;DR: PhysElite是一个大规模双语多模态基准测试，用于评估奥赛级物理推理能力，包含11,586道奥赛级别物理问题，每个问题配有图表、双语解题步骤和最终答案。


<details>
  <summary>Details</summary>
Motivation: 现有物理基准测试存在两个重要局限：(1)缺乏高难度数据集，(2)缺乏对视觉形式、知识点和逐步解题过程的全面覆盖，导致当前数据集无法充分代表模型解决复杂物理问题的能力。

Method: 构建了包含11,586道奥赛级物理问题的PhysElite基准，每个问题提供视觉图表、双语（中英）逐步解题推导和最终答案；对18个开源和闭源多模态大语言模型进行了基准测试，并进行了步骤级过程评估以诊断模型在推理链中的失败点。

Result: 即使是最强的模型也仅能达到33.7%的答案准确率，表明当前多模态大语言模型在奥赛级物理推理方面仍有很大提升空间。

Conclusion: PhysElite基准的发布填补了高难度物理推理评估的空白，步骤级过程评估有助于深入理解模型的推理能力局限，为未来多模态大语言模型的物理推理能力提升提供了重要参考。
...
### [112] [Towards Reliable, Generalizable, and Specific In-Context Knowledge Editing via Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2608.25100)
*Xuzhong Wang,Maiqi Jiang,Tejal Nair,Girija Bhusal,Yanfu Zhang,Haipeng Chen*

Main category: cs.AI

TL;DR: 大型语言模型(LLM)的知识会过时,传统知识编辑方法需要全量重训练。MO-IKE是一种基于多目标强化学习的上下文知识编辑方法,将提示构建建模为约束马尔可夫决策过程,在保持模型可靠性的同时提升泛化性和特异性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的知识截止于预训练时间,无法及时更新;现有基于强化学习的方法未能将提示构建为结构化实体来同时优化可靠性、泛化性和特异性三个目标。

Method: 提出MO-IKE算法,将上下文知识编辑的提示构建问题形式化为约束马尔可夫决策过程(CMDP),训练动态检索器来平衡多个竞争性目标。

Result: 在Llama-3.2上,编辑成功率从85.0%提升至92.0%,释义一致性从77%提升至79%,保留率较先前RL方法提升23.0%。

Conclusion: MO-IKE通过多目标优化框架有效提升了知识编辑的可靠性、泛化性和特异性,为LLM知识更新提供了一种更优的提示构建策略。
...
### [113] [FuzzingBrain-Bench V1: Evaluating Open-Ended Bug Discovery by LLMs](https://arxiv.org/abs/2608.25158)
*Ze Sheng,Aleksandar Kezic,Zhicheng Chen,Jeff Huang*

Main category: cs.AI

TL;DR: FuzzingBrain-Bench是一个用于评估AI模型发现开源软件bug能力的基准测试。通过Docker容器提供开源项目和带 sanitizer 的测试工具，让模型生成能触发更多不同崩溃的输入，按崩溃签名数量和难度加权计分。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试只评估模型是否能生成匹配预定义漏洞的概念验证输入，忽略了模型发现的与预定义目标不匹配的有效崩溃，导致无法真实反映模型的bug发现能力。

Method: 在自包含的Docker镜像中向模型提供开源项目和带sanitizer的测试工具，目标是生成能通过工具触发尽可能多不同崩溃的输入。性能按每个挑战产生的不同崩溃签名数量计分（有上限）并乘以难度系数。

Result: FuzzingBrain-Bench V1包含77个挑战，来自43个开源项目（36个C、32个C++、9个Java/JVM）。Claude Opus 4.8表现最佳，在60个挑战中触发崩溃，得分196/579。三个模型在13个挑战中均未触发崩溃。

Conclusion: 该基准测试能更全面地评估LLM的bug发现能力，Claude Opus 4.8表现最好，但仍有很大提升空间。基准测试和工具已公开可用。
...
### [114] [Tunable Tool-Call Rates in LLM Agents via Representation Steering](https://arxiv.org/abs/2608.25198)
*Yuqi Chen,Vincent Siu,Yang Liu,Dawn Song,Chenguang Wang*

Main category: cs.AI

TL;DR: 本文研究如何通过残差流中的单个线性方向来控制LLM智能体的工具调用行为，无需训练即可在推理时干预模型决策。


<details>
  <summary>Details</summary>
Motivation: LLM智能体经常错误地决定是否调用工具：过度调用增加延迟、成本和副作用，不足调用则导致模型在可借助工具回答的问题上犯错。现有方法如后训练和提示工程成本高且难以在推理时修改。

Method: 从模型自身的工具使用偏好信号中提取一个线性方向，在不改变提示的情况下进行推理时干预。通过在残差流中添加方向并调整强度α来控制调用率。

Result: 添加方向可将调用率从接近0%单调提升至超过90%，且保持调用格式良好。抑制方向可减少调用，增强方向可诱导模型在知识不足的问题上调用工具。该方向可泛化到未见过的工具，且跨dense、MoE和多模态模型均有效。

Conclusion: 使用真实工具执行时，推理时干预可追踪成本/准确率Pareto前沿，将开放域QA准确率从0.29提升至0.56，几乎翻倍，且无需任何训练即可迁移到多种模型架构。
...
### [115] [LLM-Driven, Datasheet-Aware Automated Hardware Compatibility Verification for Early-Stage, Pre-Schematic Embedded System Design](https://arxiv.org/abs/2608.25217)
*Haotian Qiao,Robert P. Dick*

Main category: cs.AI

TL;DR: 本文提出了一种基于大语言模型的早期硬件兼容性验证框架，能够在详细原理图仿真之前，根据硬件 datasheet 和高级组件连接描述识别文档级接口不兼容性。该框架通过构建设计图、检索工程属性并生成确定性脚本，实现了模块化的兼容性分析，在7个嵌入式系统设计（34个datasheet）上达到97.5%的验证准确率，且输入上下文大小比传统"上传-查询"工作流减少8.6倍。


<details>
  <summary>Details</summary>
Motivation: 当前硬件设计早期阶段缺乏有效的兼容性验证手段，现有方法需要等到详细原理图仿真阶段才能发现接口不兼容问题，这会导致设计返工成本高、周期长。传统的LLM"上传-查询"工作流存在上下文开销大、透明度低、数值计算不可靠等问题。

Method: 框架将兼容性分析分解为模块化阶段：1）根据datasheet和连接描述构建设计图，捕获设备连接和共享交互域；2）检索显式领域验证标准所需的工程属性；3）生成兼容性评估的确定性脚本。通过保留中间结果降低上下文开销、提高透明度和可追溯性，避免依赖LLM进行数值计算。

Result: 在包含34个datasheet的7个嵌入式系统设计上评估，达到97.5%的兼容性验证准确率，输入上下文大小比"上传-查询"工作流减少8.6倍。

Conclusion: 证明了LLM辅助的早期设计阶段基于规范的硬件兼容性验证的可行性，验证了模块化任务分解、形式化验证标准和任务感知紧凑上下文构建的必要性及显著优势。
...
### [116] [FLARE: Verifying MILP Reformulations with LLM-Based Theorem Proving](https://arxiv.org/abs/2608.25220)
*Henry Robbins,Connor Lawless,Madeleine Udell,Ellen Vitercik*

Main category: cs.AI

TL;DR: 本文提出FLARE方法，利用大语言模型和Lean证明助手验证混合整数线性规划(MILP)重构的正确性，解决了现有方法无法对一般问题实例进行推理的局限性。


<details>
  <summary>Details</summary>
Motivation: MILP是组合优化的重要工具，但设计高效formulation具有挑战性。LLM为自动化建模带来新机遇，但现有验证方法仅做数值评估，无法对一般问题实例进行推理，需要鲁棒的验证方法确保重构的正确性。

Method: 提出MILP重构的构造性定义，可在Lean中形式化验证。开发FLARE方法，结合LLM智能体和Lean证明助手验证重构。提出FLARE-NL作为快速便宜的LLM代理。构建FormulationBench数据集，包含20个问题109个formulation。

Result: FLARE在FormulationBench的NP-hard子集上达到100%准确率，并为每个通过的重构生成机器可检查证书。FLARE-NL准确率与FLARE相当，但无证书。

Conclusion: FLARE方法实现了自动化优化建模中的可靠验证，为MILP重构提供了形式化保证。FLARE-NL适用于不需要形式化保证的场景。
...
### [117] [Hierarchical MoE for Multi-Modal ILD Diagnosis](https://arxiv.org/abs/2608.25261)
...
TL;DR: 本文提出了一种层次化多模态专家混合模型(MoE)，用于间质性肺病(ILD)分类，通过双阶段门控机制整合预训练影像专家和结构化电子健康记录(EHR)数据，在严格患者级交叉验证下获得最高平均AUC (0.8750)。


<details>
  <summary>Details</summary>
Motivation: 医学数据存在异质性，现有方法难以有效利用多模态信息。影像数据和EHR数据具有不同特征，需要一种原则性机制来整合异构预测器，同时保持可解释性。

Method: 提出层次化多模态MoE模型，包含：(1)冻结的预训练影像专家；(2)结构化EHR分支；(3)两阶段门控：模态级门控分配患者特定的影像和EHR权重，子门控模块将EHR分支分解为临床定义的特征组，学习组特定贡献。

Result: 模型达到最高平均AUC (0.8750 ± 0.0443)，优于影像-only REN (0.8646)和SwinUNETR (0.7685)。框架可扩展到解剖区域、影像-EHR利用率和临床定义的EHR特征组解释。

Conclusion: 层次化多模态MoE框架能有效整合影像和临床数据，保持稳定影像表示的同时实现输入依赖的临床加权和显式EHR专门化，在ILD分类任务上优于现有方法。
...
### [118] [PhaseShift: Topology-Aware Data Harmonization and Model Consolidation Across Signalized Intersections](https://arxiv.org/abs/2608.25275)
*Yash Ranjan,Artur Kumik,Rahul Sengupta,Anand Rangarajan,Sanjay Ranka*

Main category: cs.AI

TL;DR: PhaseShift是一个拓扑感知的框架，将异构路侧轨迹统一为共享的演员中心表示，训练一个可重用的主干模型，实现跨路口的知识迁移和零样本部署。


<details>
  <summary>Details</summary>
Motivation: 当前学习交通行为模型需要为每个路口单独训练，无法跨站点共享知识，导致模型组合无法泛化且数据利用效率低。

Method: 提出PhaseShift框架，采用自我相对坐标、轨迹诱导的运动路径、标准化信号上下文和可变基数交互token来移除站点惯例同时保留行为相关的拓扑结构。主干模型支持池化操作、零样本迁移和低数据适应。

Result: 在五个佛罗里达州路口的评估中，10秒预测时池化模型相比本地模型在所有站点上降低minADE和36.8%和minFDE 22.0%的中位数。留一路口out部署在4/5站点超越本地训练。微调后进一步提升。本地模型在高流量路口落后于校准的IDM，但预训练主干模型不落后。

Conclusion: PhaseShift展示了跨异构物理控制设置的整合能力，同时识别出仍需适应的站点。该框架实现了交通行为模型从单点训练向可迁移学习的范式转变。
...
### [119] [BixBench3: Benchmarking AI agents on research-study-scale computational biology tasks](https://arxiv.org/abs/2608.25286)
...
TL;DR: BixBench3是一个评估AI代理执行完整计算生物学研究能力的基准测试。通过20个任务评估13个前沿模型，发现最高得分为0.48（GPT-5.6 Sol），模型在大数据集和复杂分析步骤下表现较差，平均每个任务耗时6.8小时和43美元。


<details>
  <summary>Details</summary>
Motivation: AI承诺加速生物研究，但AI代理能否在完整研究规模上执行计算生物学任务尚未被系统评估。

Method: 设计BixBench3任务模拟科学家委托工作给AI代理：给定研究目标、方法指导和来自已发表研究的原始数据，代理执行分析序列，生成产物与原研究产物进行程序化评分。

Result: 20个任务产生138个独特产物。13个前沿模型得分0-0.48，GPT-5.6 Sol最高。大数据集任务表现更差（<100GB: 0.36 vs >100GB: 0.10），多步骤分析也更差（1-2步: 0.36 vs 3+步: 0.24）。平均每任务6.8小时、102M tokens、43美元。

Conclusion: LLM在执行多步骤连贯分析、管理大量原始数据和跨科学领域工作方面能力差异显著。最高分代理反而使用更少tokens和更低成本。
...
### [120] [FinRiskAtlas: Decision-Aligned Evaluation of Large Language Models for Financial Risk Review](https://arxiv.org/abs/2608.25325)
*Suyang Zhong,Jingzhe Zhu,Qi Xu,Liyao Sun,Yin Wang,Qingqing Sun,Shuai Chen,Tianyi Zhang*

Main category: cs.AI

TL;DR: 本文提出FinRiskAtlas，一个中文金融风险基准，用于从两个维度评估金融大语言模型：固定证据状态下的操作执行和 evolving review conditions下的证据状态控制。基准包含9,742个实例，涵盖53个任务族。FinRisk-Ask通过104条专业轨迹的680个预操作状态进行离线回放测试。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准围绕数据集或任务 formulation组织，而非围绕部署系统支持的决策。专业金融审查不仅需要通用金融能力，还需执行特定审查操作并判断证据是否足以支持决策。现有评估单元与实际工作流程中的决策和证据状态不对齐。

Method: 构建包含9,742个实例的静态基准(53个任务族，42个领域知识族+11个下游审查操作)；通过FinRisk-Ask对680个预操作状态进行离线回放，推理时扣留未来证据，仅用于构建专家验证的证据目标；对33种模型配置进行操作级评估。

Result: 操作级评估产生非冗余排序(下游操作间平均Spearman相关系数0.42)；知识型筛选在单个操作上可导致高达18.01分的遗憾；更频繁进入Ask分支不一定改善请求定位或端到端证据获取。

Conclusion: 广泛的金融能力评分无法完全捕捉模型在专业工作流程中的可靠性，需要采用与部署系统决策和证据状态对齐的评估单元。
...
### [121] [Learning What to Share and What to Personalize: Hierarchical Strategy Co-Evolution for Agent Memory](https://arxiv.org/abs/2608.25329)
*Yupeng Han,Shuochen Liu,Kai Zhang,Ze Liu,Zhihong Pan,Xianquan Wang*

Main category: cs.AI

TL;DR: HiPS框架通过将记忆管理解耦为全局共享基础和用户特定自适应层来解决记忆增强智能体的个性化策略问题。全局通用策略从跨人格轨迹中提取共享原则，而人格增量蒸馏则为行为偏离一般模式的用户生成定制规则。跨层规则流通过促进广泛验证的个性化规则并淘汰矛盾的全集规则来动态调整边界。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用静态的"一刀切"记忆管理策略，在训练前就已确定。但实际上，最佳记忆决策具有用户特异性，并随策略优化动态演变。现有静态方法无法适应不同用户的个性化需求。

Method: 提出HiPS层次化个性化策略框架，包含三个核心组件：1) 通用策略(Universal Strategy)从跨人格轨迹中提取共享原则；2) 人格增量蒸馏(Persona Delta Distillation)为行为偏离一般模式的用户生成定制规则；3) 跨层规则流(Cross-Level Rule Flow)通过促进验证通过的个性化规则和淘汰矛盾的全集规则来动态校准边界。架构建立共进化循环，确保所有策略改进都锚定在任务结果上。

Result: 广泛实验表明，HiPS在记忆增强基线方法上实现了一致的改进。

Conclusion: HiPS通过分层解耦机制有效解决了记忆管理中静态策略无法个性化的问题，实现了全局共享知识与用户特定适应性之间的动态平衡。
...
### [122] [Where vs What: Decomposing Structural and Content Failures in LLM-Generated Structured Outputs](https://arxiv.org/abs/2608.25358)
*Yiwei Zhang,Chengke Wu,Li Wang,Jianqiang Li*

Main category: cs.AI

TL;DR: 本文提出结构-内容分解(SCD)框架，用于独立衡量LLM生成结构化输出(JSON/表格)时的结构保真度和内容准确性。研究发现结构保真度随复杂度增加下降更快，SA-RLVR方法通过将SCD指标转化为可验证奖励进行强化学习，有效提升结构定位能力。


<details>
  <summary>Details</summary>
Motivation: 当前对结构化输出生成失败的评估是整体性的，混淆了两种不同的错误模式：位置错误(正确值放在错误位置)和值错误(错误值放在正确位置)。需要独立评估这两种错误模式以更好地理解和改进LLM的结构化输出能力。

Method: 1)提出结构-内容分解(SCD)框架，独立衡量结构保真度和内容准确性;2)在6个模型(7B到前沿模型)的嵌套JSON和表格任务上应用SCD;3)提出SA-RLVR方法，将SCD指标转化为基于GRPO的可验证奖励进行强化学习优化。

Result: 结构保真度随复杂度增加比内容准确性下降更快、更明显。最高复杂度下，DeepSeek-V4-Flash错放35%的召回值，Qwen2.5-7B错放74%。SA-RLVR将JSON值放置准确率(VPA)从26%提升到63%，并能泛化到保留模式，在表格领域也一致提升VPA。

Conclusion: 结构感知奖励可直接增强多域结构定位能力。SCD框架为理解和改进结构化输出生成提供了有价值的方法，强调结构保真度是当前LLM的薄弱环节，需要专门的结构感知训练方法。
...
### [123] [Can your AI agent be cheaper? Investigating the effects of task specifications on token spend in agentic coding tasks](https://arxiv.org/abs/2608.25399)
*Jakub Smékal*

Main category: cs.AI

TL;DR: 本文研究了任务规范对AI编码智能体代币消耗的影响。通过2700次实验，使用Kimi K3模型发现：将完整任务规范简化为基本用户故事会使代币消耗增加29.7%，但运行间方差不受提示词变化影响。提示词敏感度因任务而异（13%-115%）。提出一种简单预测器，仅需对未见任务进行一次廉价探测，即可在36%误差内预测任务规范和思考努力配置的代币消耗分布，优于先前工作。


<details>
  <summary>Details</summary>
Motivation: 随着智能体编码工作流在现实系统中的广泛部署，长程推理和工具使用导致代币消耗成为成本和效率的关键考量。两个工程师使用AI解决同一问题的方式不同，任务规范如何影响智能体的代币消耗，以及能否提前预测这些消耗，都是开放性问题。

Method: 使用Kimi K3模型在三种思考努力配置下，对2700次运行进行实验，测试不同任务规范对代币消耗的影响。提出一种简单预测器，通过对未见任务进行单次廉价探测来预测代币消耗分布。

Result: 将完整任务规范简化为基本用户故事使代币消耗增加29.7%；运行间方差不受任何提示词变化影响；提示词敏感度因任务而异，从13%到115%不等；预测器可在36%误差内预测代币消耗分布，优于先前工作。

Conclusion: 本研究提供了量化任务规范对智能体代币消耗影响的初步结果，并引入了一种可系统评估AI编码工作流成本的方法。研究表明充分的任务规范有助于减少代币消耗，且可通过廉价探测提前预测成本。
...
### [124] [Paint What You See: Benchmarking Dexterous Visual Tool Use in Multimodal Agents](https://arxiv.org/abs/2608.25417)
*Shudong Liu,Dongyang Chen,Enci Zhang,Jinwei Liang,Zheng Ma,Lewei Lu*

Main category: cs.AI

TL;DR: EASEL是一个评估多模态智能体进行精细视觉工具使用的新基准测试,通过参考图像引导的视觉重建作为核心任务,并包含语义任务如区域标注、手写和路径规划。评估25个模型发现当前多模态智能体在该基准上存在系统性困难,重建相似度仅0.40-0.54,轨迹诊断显示严重的闭环不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试涵盖web导航、GUI操作和软件工程,但很少关注视觉证据与执行精度之间的耦合。研究者提出"精细视觉工具使用"这一关键但未被充分探索的能力:模型从视觉证据推断工具参数,这些参数直接决定最终结果。

Method: 提出EASEL基准,采用参考引导的视觉重建作为主要代理任务:智能体逐步绘制画布以匹配参考图像。同时包含语义任务(区域标注、手写、路径规划)。提供EASEL-Data(44万样本的两阶段课程数据集)和EASEL-9B模型用于轨迹监督训练。

Result: 评估25个模型显示:重建相似度低(0.40-0.54),轨迹诊断揭示严重的闭环不稳定(模型通常早期饱和或峰值后退化)。语义任务显示精确标注和路径规划能力存在明显边界。EASEL-9B相比基线模型提升6.3%,在所有模型中排名第三。

Conclusion: 当前多模态智能体在精细视觉工具使用方面存在系统性不足,需要改进视觉-动作闭环能力。EASEL-9B通过大规模轨迹监督训练取得显著进步,证明训练数据和方法的有效性,但仍与最佳模型存在差距。
...
### [125] [Distance Is Not Enough: Forget-Retain Alignment Gap Predicts LLM Relearning Robustness](https://arxiv.org/abs/2608.25429)
*Yi Chen,Hanna Hsieh,Shuhong Liu,Chuanbo Hua,Zihan Ma,Kun Wang,Joo-Young Kim*

Main category: cs.AI

TL;DR: 本文提出FRAG（遗忘-保留对齐间隙）作为无需训练的预测器，评估模型更新在遗忘关键权重和保留关键权重上的选择性，并基于此提出FRP（遗忘-保留剪枝）方法来增强机器学习的抗重新学习能力。实验表明权重选择性比单纯的空间距离更能解释抗重新学习鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在遗忘特定数据后，经过短暂的微调即可恢复被遗忘的知识。现有的鲁棒性预测器依赖全局权重空间位移，但单纯距离指标在随机或破坏性更新导致性能崩溃时具有误导性。作者认为抗重新学习能力取决于更新结构：鲁棒的遗忘应该影响遗忘关键权重而保留保留关键权重。

Method: 1. 提出FRAG（遗忘-保留对齐间隙）：无需训练的预测器，通过评估更新在遗忘关键权重和保留关键权重上的对齐程度来预测抗重新学习鲁棒性，无需运行重新学习攻击。2. 提出FRP（遗忘-保留剪枝）：基于遗忘关键权重和保留关键权重原则的剪枝方法，提高抗重新学习鲁棒性。

Result: FRAG能够比全局距离更可靠地区分选择性更新和密集更新。FRP显著提高了模型的抗重新学习能力。权重选择性比距离更能解释鲁棒性。

Conclusion: 权重选择性（更新对遗忘关键和保留关键权重的差异化影响）比单纯的全距离更能解释抗重新学习鲁棒性。这一发现为设计更鲁棒的机器遗忘方法提供了新思路。
...
### [126] [Training Alignment Auditors via Reinforcement Learning](https://arxiv.org/abs/2608.25460)
*Paul Rosu,Rowan Wang*

Main category: cs.AI

TL;DR: 本文提出利用强化学习改进大型语言模型审计员，通过让审计策略调查目标模型中植入的隐藏行为，并使用LLM裁判比较调查质量进行奖励训练。实验表明，该方法提高了调查质量和审计真实感，同时保持低于1%的误报率。


<details>
  <summary>Details</summary>
Motivation: 当前前沿模型的 alignment 审计越来越依赖 LLM 审计员来大规模揭示不良行为，但现有自动审计员在连贯性调查和审计真实感方面存在不足。

Method: 构建训练环境，让策略调查可能通过系统提示植入隐藏行为的目标模型。LLM 裁判（知道目标是否有隐藏行为）整体比较策略的调查与参考调查来确定奖励。通过系统消融实验，发现成对奖励比逐点奖励更稳健，且添加未植入行为的目标有助于保持低误报率。

Result: 训练提高了对植入行为目标的调查质量、未修改生产模型中揭示不良行为的比率和审计真实感，同时误报率保持在1%以下。审计能力还能跨脚手架泛化，在 AuditBench 对抗微调目标上性能显著提升。

Conclusion: 强化学习训练可以有效提升 LLM 审计员的调查能力和审计质量，且具备良好的泛化性能。
...
### [127] [PonsRAG: A Pons-Inspired RAG Bridging Cognitive Islands for Coordinated Long Narrative Reasoning](https://arxiv.org/abs/2608.25486)
*Rongchen Zhao,Yu Chen,Juyuan Wang,Zhouting Mo,Jianxing Yu,Wenqing Chen,Jingping Liu*

Main category: cs.AI

TL;DR: PonsRAG是一个受生物脑桥启发的协调RAG框架，用于处理复杂叙事的长上下文推理，解决了认知孤岛和跨层证据断开问题。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法在处理长叙事推理时面临两个关键挑战：认知孤岛(cognitive islanding)和跨层证据断开(cross-layer evidence disconnection)。

Method: 提出PonsRAG框架，包含两个关键组件：1) Triple-Layer Indexing（三层索引）：将文档组织成连接的知识结构以桥接认知孤岛；2) Coordinated Reasoning（协调推理）：跨不同层检索证据并将跨层信息整合到统一上下文中。

Result: 在四个长上下文叙事基准测试上进行评估，实验结果表明PonsRAG优于最强基线，在多选题任务上平均准确率实现了11.56%的相对提升。

Conclusion: PonsRAG框架通过三层索引和协调推理机制有效解决了长叙事推理中的认知孤岛和跨层证据断开问题，显著提升了长上下文叙事推理的性能。
...
### [128] [CaSKG: Counterfactual-Causal Skill Graphs for Scalable Agent Skill Retrieval](https://arxiv.org/abs/2608.25500)
*Zhiyuan Li,Linyuan Gao,Xuechun Ding,Hongwei Chen,Yuan Wu,Yi Chang*

Main category: cs.AI

TL;DR: 提出CaSKG反事实因果技能图框架，通过边置信度校准实现紧凑可执行的技能检索，在ALFWorld和ScienceWorld上6个LLM均超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理的可重用技能库面临检索难题：全库提示上下文成本高，向量检索将技能视为独立文本忽略工作流上下文，图检索依赖可靠边。需在检索前校准程序关系。

Method: 构建高召回有向候选图（融合语义、词汇、输入/输出、结构证据）→修复证据和LLM法官进一步细化候选分数→应用方向条件文本反事实探测（移除、替换、重排序技能对）→贝叶斯平滑聚合证据→发布状态过滤加权图用于任务条件扩展

Result: 12种模型-基准组合中均获最高任务得分；相对GoS：ScienceWorld六模型宏平均分72.62→80.50，ALFWorld成功率80.01%→86.79%；减少两基准平均环境步数；校准边有助保留前置条件、状态变更动作、验证程序、完成步骤。

Conclusion: CaSKG通过边置信度校准，为大规模可执行技能检索提供了一种有效方案。该框架在6个LLM骨干模型、2个基准测试的12种组合中均达到最高任务得分，相对GoS在ScienceWorld和ALFWorld上分别提升约8%和7%，同时减少环境步数。消融实验表明校准边有助于保留前置条件、状态变更动作、验证程序和最终完成步骤。代码开源。
...
### [129] [Agentic Game Development as a Verifiable Trajectory Data Engine for Scaling World Models](https://arxiv.org/abs/2608.25518)
*Pengfei Zhou,Hexin Wang,Zhengfeiyang Zhang,Yixing Ma,Zhenglin Wan,Kaipeng Zhang,Wangbo Zhao,Yang You*

Main category: cs.AI

TL;DR: 本文提出通过游戏引擎为空间世界模型提供可执行的奖励信号，结合人类验收反馈进行强化学习后训练（RLHEV），解决当前空间生成依赖模糊代理信号（如CLIP分数）的问题。


<details>
  <summary>Details</summary>
Motivation: 当前扩展世界模型的策略是增加训练数据和计算资源，但作者认为这种方法效率低下。空间生成仍主要依赖CLIP分数等模糊代理信号，缺乏可执行的、高质量的奖励信号来支持强化学习后训练。相比之下，代码代理因为代码可执行性能够获得高质量奖励。

Method: 提出RLHEV（Reinforcement Learning with Human-Engine Verification）后训练范式，结合游戏引擎的密集信号（碰撞检测、物理模拟、可导航性、边界可玩性检查）与开发过程中隐含的人类接受反馈。

Result: 游戏引擎编码的场景是世界规范的可执行规范：引擎可高效检查碰撞、物理、可导航性和边界可玩性，开发者提供全局验证信号判断场景是否可接受。游戏开发还提供真实世界的长视野轨迹数据用于强化学习后训练。

Conclusion: 游戏开发为空间世界模型提供了缺失的奖励环境，是比模糊代理信号更优的强化学习后训练方案。
...
TL;DR: 本文提出了一种用于AI部署的认知能力分析框架，通过认知能力分析来评估AI系统在特定工作任务中的适用性，帮助组织决定哪些任务应自动化、由人类执行或人机协作。


<details>
  <summary>Details</summary>
Motivation: 解决组织部署AI时的作用域问题：哪些任务可以自动化，哪些应保留给人类，哪些最好人机共享。现有基准测试分数无法揭示系统在实际场景中的成败，而对模型能力的人类判断很快会过时。

Method: 提出一个包含两个核心组件的管道：1）认知能力分析——从标注了认知需求的基准测试表现推断智能体的能力；2）任务需求加权——从领域专家处获取各项能力对工作的相对重要性。使用共同认知维度，可随模型和角色变化独立更新。

Result: 在合成智能体上验证能力恢复，分析六个人工智能系统，从六个职业领域的410名员工获取任务需求。发现：AI系统在认知维度上的差异大于不同模型家族间的差异，工作活动收敛于共享的认知核心。

Conclusion: 该方法提供比较性作用域工具，识别有希望的试点候选区域和当前系统不太适合的领域。框架可扩展至同时分析人类工作者，从AI适用性走向人机任务分配。
...
TL;DR: Narcissus是一个程序合成器，通过保留LLM建议的语法树结构并对每个候选程序扩展进行评分，解决了LLM在处理罕见语法时生成错误程序的问题。它使用上下文评分机制和正则化项确保所有规则可达，让错误的建议只会延迟解决方案而不会隐藏它。


<details>
  <summary>Details</summary>
Motivation: 当任务涉及训练数据中罕见的语法时，LLM生成的程序通常会违反语法或无法满足规格。现有的枚举合成器通过将LLM建议转换为规则频率来引导搜索，但这会丢失语法结构信息，并会修剪掉建议中遗漏的规则，尤其在建议错误时问题更加严重。

Method: Narcissus保留LLM建议作为语法树，对候选程序的每个扩展进行评分：具有相同周围结构的建议是否以相同方式继续，以及扩展是否重建了建议重复的片段。使用正则化项保持每个规则可达，使错误的建议只会延迟而非隐藏解决方案。

Result: 在五个领域和两个搜索后端的实验中，Narcissus在所有预算下都优于静态指导，并始终优于重新提示LLM修复建议。它比原始建议快一个数量级找到类似建议的程序，并在ARC任务中解决了40%的任务（而原始建议仅解决13%），整个搜索过程中没有进行任何LLM调用。

Conclusion: Narcissus通过保持语法树结构而非转换为规则频率，成功地将LLM的推理能力融入枚举合成器，在不牺牲语法正确性的前提下显著提高了搜索效率和任务解决率。
...
### [132] [ToST: A Tree-of-Thought Socratic Teaching Framework for Multi-Path Guidance and Parallel Thinking](https://arxiv.org/abs/2608.25775)
*Feng Ling,Heng Yu*

Main category: cs.AI

TL;DR: ToST是一种基于思维树的苏格拉底教学框架，旨在支持"一题多解"范式下的多路径指导。该框架通过并行播种策略鼓励学生从多角度思考问题，并引入多路径自适应指导机制提供非线性教学指令。同时构建了MPSG-Bench基准数据集，包含31K多路径教学对话和基于SOLO理论的五维评估框架。实验表明ToST能显著提升指导成功率，帮助学生更有效地探索多条解题路径。


<details>
  <summary>Details</summary>
Motivation: 现有苏格拉底教学研究多采用"一题一解"范式，限制教学灵活性、削弱错误恢复能力、阻碍学生并行思考多解法。LLMs虽具备强问题解决能力，但现有方法仅支持单一线性推理路径，无法满足多样化教学需求。

Method: 提出ToST框架，包含两个核心机制：1)并行播种(Parallel Sowing)：面向并行思维的提问策略，鼓励学生从多角度思考；2)多路径自适应指导(Multi-Path Adaptive Guidance)：在备选解题轨迹上提供更稳健的非线性教学指令。同时构建MPSG-Bench基准，含31K多路径教学对话和基于SOLO理论的五维评估框架。

Result: 自动评估和人工评估均表明，ToST显著提升了指导成功率，使学生更有效地探索多条解题路径。

Conclusion: ToST框架成功实现了多路径苏格拉底教学指导，为LLM辅助教学提供了新范式，对提升教学灵活性和学生并行思维能力具有重要价值。
...
### [133] [LocalLSTC: A Long Short-Term Control Architecture for Locally Deployed GUI Agents](https://arxiv.org/abs/2608.25777)
*Weiming Li,Helen Paik,Yulei Sui*

Main category: cs.AI

TL;DR: 本文提出LocalLSTC架构，通过时间范围组织控制信息来解决GUI代理中控制信息隐式化问题。该架构维护跨步骤的持久状态，将长期控制（子目标、证据、运行时反馈）与短期执行（当前步骤的有限承诺）分离，实现64.7% OSWorld和65.3% WindowsAgentArena的SR-100性能。


<details>
  <summary>Details</summary>
Motivation: 当前GUI代理框架中，控制信息（任务阶段、累积证据、运行时反馈）在交互轨迹中保持隐式，导致规划器需在每步重建状态，特别是在弱推理能力的本地模型上性能下降严重。使用GPT-5替换Qwen3.5-9B使OSWorld SR-100从60.9%降至37.7%，失败轨迹中91.6%存在控制失败。

Method: 提出LocalLSTC训练无关架构，按时间范围组织控制信息：1)长期控制维护活跃子目标、子目标对齐证据和跨交互的运行时反馈；2)短期执行实现当前步骤的有界承诺；3)长期到短期规划从持久状态形成承诺；4)短期到长期控制将执行结果整合回持久状态用于进度评估、恢复和终止。

Result: 使用Qwen3.6-27B，LocalLSTC在OSWorld达到64.7% SR-100，在WindowsAgentArena达到65.3%，均优于此前最强本地结果。消融实验支持执行两侧机制的贡献。

Conclusion: 研究确定了控制信息的时间组织是本地部署GUI代理的关键架构维度，为提升本地模型在桌面任务性能提供了有效方案。
...
### [134] [Choose Your Game Wisely: Measuring Game-Theoretic Structures in Real-World Vehicle Interactions](https://arxiv.org/abs/2608.25917)
*Yueyuan Li,Rongcheng Nie,Weijie Xi,Mingyang Jiang,Songan Zhang,Hanyang Zhuang,Ming Yang*

Main category: cs.AI

TL;DR: 本文开发了一个基于轨迹的交互测量框架，用于从车辆轨迹中识别交互事件并量化行为变化 onset、时间组织、响应动态和排序稳定性，并在6个真实轨迹数据集上评估，发现现实世界的交互表现出并发、顺序和持久有序的时间结构，不同博弈论建模方法应被视为不同交互制度的互补抽象。


<details>
  <summary>Details</summary>
Motivation: 博弈论模型在建模车辆交互方面提供了原则性框架，但其底层时间假设尚未与真实驾驶行为进行系统检验，特别是同时、顺序和不对称交互结构如何从车辆轨迹中测量仍不清楚。

Method: 开发了基于轨迹的交互测量框架，使用行为偏差来验证候选交互事件，识别交互事件并量化行为变化 onset、时间组织、post-onset 响应动态和排序稳定性。在6个真实轨迹数据集上评估：INTERACTION、highD、inD、rounD、Waymo Open Motion 和 nuPlan。

Result: 并发和顺序行为变化都构成了跟车、汇入和冲突交互的显著比例；在顺序交互中，稳定排序比交替排序更普遍，表明持久不对称角色是常见的交互结构；时间优先并不一定与可测量的行为响应相符，表明时间排序 alone 可能不足以描述行为依赖。

Conclusion: 现实世界交互表现出并发、顺序和持久有序的时间结构。不同的博弈论建模方法应被视为不同交互制度的互补抽象，而非统治所有车辆交互的通用结构。
...
### [135] [Repair or Resample? Rethinking Failure Debugging in LLM Multi-Agent Systems](https://arxiv.org/abs/2608.25920)
*Zhongwen Luan,Xiaoyu Zhang,Ming Hu,Yue Yang,Jiongchi Yu,Xiaohong Chen*

Main category: cs.AI

TL;DR: 本文提出了SymTrace框架和SymFail数据集，用于评估LLM多智能体系统的调试和修复方法。研究发现现有重运行方法可靠性低，症状驱动干预方法可显著提升修复效果。


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统在长周期复杂任务中的应用日益广泛，但其可靠性是阻碍实际部署的核心瓶颈。现有MAS调试方法依赖重运行和重采样整个执行轨迹，但这些方法是否真正因果性地修复失败还是仅通过LLM采样的随机性进行随机修复，这个问题尚待回答。

Method: 1. 提出SymTrace控制评估框架，记录MAS执行轨迹并建立干预锚点，在回放时通过记录日志重建锚点前的执行，仅重新生成下游轨迹，从而可靠地重现MAS失败。2. 构建SymFail数据集，包含536条人类注释的失败轨迹，具有图形链接的位置、类别和trace证据。3. 在三个主流MAS框架上进行大规模实证研究。

Result: 研究发现现有无引导重运行方法高度不可靠，失败重现率仅为67.97%，修复率仅为6.90%。症状驱动干预方法成功修复了20.15%的失败案例，比最先进的修复方法提升了191.89%。

Conclusion: 本研究为MAS调试和修复研究提供了可操作的见解，为多智能体系统的稳健部署铺平了道路。
...
TL;DR: 研究对自动化事实核查（AFC）系统进行了全面基准测试，涵盖9种模型和4个数据集，发现模型排名高度依赖领域和评估指标，检索仍是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前AFC系统评估缺乏简单基线比较，系统针对单一基准开发，缺乏跨领域泛化能力。现有研究仅关注检索阶段或单阶段基准测试，未对完整的检索-验证两阶段流程进行跨数据集评估。

Method: 在四个涵盖科学、开放网络和气候领域的数据集上，对九种模型进行基准测试，包括随机基线、稀疏基线、微调Transformer、零样本LLM以及AVeriTeC 2025共享任务的前两名系统。

Result: 三个主要发现：(1) 在ClimateCheck数据集上，仅 claim 模型和微调模型优于零样本LLM和AVeriTeC 2025系统；(2) 系统排名强依赖领域和指标：SciFact上最佳模型（macro-F1 0.70）在ClimateCheck上降至0.31；(3) 用黄金证据替换检索证据可提升14-22个百分点的准确率。

Conclusion: 检索是AFC系统的首要瓶颈。研究释放了代码、预处理数据集和所有结果以支持可复现的AFC研究。
...
### [138] [LivingRAG: Augmenting Graph RAG with Experience](https://arxiv.org/abs/2608.25960)
*Yuzhuo Cui,Zongye Zhang,Qingjie Liu*

Main category: cs.AI

TL;DR: LivingRAG是一个基于图的RAG框架，通过添加可写且可重用的推理经验存储来改进多跳问答。它允许存储的经验在后续推理中复用，减少重复检索和推理。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统存在两个问题：1) 每个查询独立处理，不保留LLM的推理结果；2) 后续相关查询需要从头开始检索证据和推理，造成资源浪费。

Method: LivingRAG在图检索基础上添加经验存储模块，通过两种方式复用验证过的经验：1) 存储的图信号帮助检索找到之前相关查询中有用的实体和段落；2) 存储的摘要提供参考推理模式用于答案生成。分析在线QA流，从共享实体、图邻域和问题模板中发现可复用信号。

Result: 在多跳QA基准测试中，LivingRAG相比强基线RAG提高了准确率，并在复用相关先验经验时减少了completion-token的使用。

Conclusion: LivingRAG通过可重用的推理经验有效解决了RAG系统中重复推理的问题，在提升准确率的同时降低了计算成本。
...
### [139] [Quantitative Analysis of $ω$-Regular Robust MDPs](https://arxiv.org/abs/2608.25968)
*Ali Asadi,Krishnendu Chatterjee,Ehsan Kafshdar Goharshady,Mehrdad Karrabi,Alipasha Montaseri,Ali Shafiee*

Main category: cs.AI

TL;DR: 本文研究具有线性定义不确定性集的鲁棒马尔可夫决策过程(RMDPs)中的平价目标(一种规范的ω-正则目标)定量分析问题。之前的工作研究了定性分析(几乎必然和正概率问题),本文解决了精确的定量问题,证明了代理人和环境都存在纯无记忆最优策略,并给出了多项式时间算法。


<details>
  <summary>Details</summary>
Motivation: RMDPs通过允许转换概率中的不确定性并针对最坏情况优化来推广经典MDPs。现有工作研究了定性分析(几乎必然和正概率问题),但精确的定量问题仍未解决,这促使作者研究具有线性定义不确定性集的RMDPs上的平价目标的定量分析。

Method: 本文采用以下方法:(1)证明纯无记忆最优策略的存在性;(2)针对线性定义的鲁棒马尔可夫链开发多项式时间定量平价算法,作为RMDPs策略迭代算法的子程序;(3)该算法结合定量单步改进和定性几乎必然改进两种技术。

Result: 本文的主要结果包括:(1)证明了代理人和环境都存在纯内存less最优策略;(2)给出了线性定义鲁棒马尔可夫链上定量平价问题的多项式时间算法;(3)开发了结合定量和定性改进的策略迭代算法;(4)实验表明该方法优于显式简化为随机博弈的方法。

Conclusion: 本文成功解决了具有线性定义不确定性集的RMDPs上平价目标的定量分析问题,为鲁棒决策过程的形式验证提供了有效的算法基础。
...
### [140] [SciMIF: Understanding Multimodal Instruction Following in Scientific Domains](https://arxiv.org/abs/2608.25973)
*Ye Shen,Yuting Zheng,Dun Pei,Zijian Chen,Wenlong Zhang,Qi Jia,Guangtao Zhai*

Main category: cs.AI

TL;DR: SciMIF是一个评估多模态大语言模型(MLLMs)在科学领域遵循复杂指令能力的新基准，基于22个任务和5个科学学科提出10个约束组的综合分类法，并开发高保真指令注入管道


<details>
  <summary>Details</summary>
Motivation: 理解多模态大语言模型在科学领域的指令-following能力对于推动科学领域发展至关重要，当前缺乏评估科学领域多模态指令遵循的系统性方法

Method: 基于22个任务和5个代表性科学学科的广泛分析，提出综合分类法包含10个约束组；开发高保真指令注入管道系统性地增强现有科学数据集；对多个最先进的闭源和开源MLLMs进行综合实验

Result: 发现不同科学学科之间存在显著性能差异，化学对当前MLLMs挑战更大；增加模型规模不会带来约束遵循能力的相应提升；当前模型在细粒度约束和需要深度学科知识的指令上仍严重困难

Conclusion: SciMIF填补了科学领域多模态指令遵循评估的空白，为未来增强MLLMs在严格科学应用中的能力奠定重要基础
...
### [141] [Multi-Granularity Context-Enhanced RAG over Multimodal Knowledge Graphs](https://arxiv.org/abs/2608.25986)
*Zongyu Wu,Yilong Wang,Xiaochen Wang,Minhua Lin,Zhichao Xu,Fenglong Ma,Xiang Zhang,Suhang Wang*

Main category: cs.AI

TL;DR: 本文提出CEMMKG框架，通过在局部和全局范围内为图像补充文本上下文，解决多模态知识图谱RAG中图像与文本之间的语义鸿沟问题，提升多模态GraphRAG性能。


<details>
  <summary>Details</summary>
Motivation: 现有MMKG-based RAG方法中，不同模态在融合前基本独立处理，导致文本上下文在视觉信息提取和多模态知识融合中的作用有限，造成图像与文本之间的语义鸿沟，限制了多模态GraphRAG的性能。

Method: 提出CEMMKG框架，在局部和全局范围内为图像丰富互补的文本上下文。局部上下文包含与图像语义相关的句子（超越周围文本），并采用多粒度设计捕获不同细节层次的信息；全局上下文提供整个段落的摘要。

Result: 在选定的以视觉为中心的数据集上的广泛实验表明，CEMMKG能够有效利用上下文信息提升MMKG-based RAG性能，且在不同的MMKG-based RAG方法中均表现出有效性，证明了其广泛适用性。

Conclusion: CEMMKG框架通过引入局部和全局文本上下文，有效弥合了图像与文本之间的语义鸿沟，为多模态知识图谱增强的RAG系统提供了更丰富的知识支持，具有良好的通用性和应用前景。
...
### [142] [AsymSpec: Context-Asymmetric Speculative Decoding for Agentic LLMs](https://arxiv.org/abs/2608.26004)
*Sheng Liang,Yongyue Zhang,Nathanael Brian,Hang Lv,Hao Wang,Chen Zhang,Yong Liu*

Main category: cs.AI

TL;DR: AsymSpec是一种非对称投机解码框架，通过让轻量级draft模型读取完整输入，而大型验证模型在压缩视图上运行，结合对比度δ融合和分歧感知接受门控制，实现约90%的完整上下文准确率，同时在文本能力上获得1.3-1.7倍吞吐量提升和0.2-0.3倍的计算成本。


<details>
  <summary>Details</summary>
Motivation: Agentic LLM管道中推理成本随上下文累积而上升，常规的输入压缩会降低任务准确率。传统投机解码假设draft模型和验证模型共享相同上下文，无法解决准确率与开销的矛盾。

Method: 提出非对称投机解码框架AsymSpec：轻量级draft模型读取完整输入，大型验证模型在压缩视图上运行。通过对比度δ融合的logits引导验证模型，并由分歧感知接受门调节以保持验证稳定性和高接受率。

Result: 在四个agentic能力和两个端到端agent基准测试中，AsymSpec达到约90%的完整上下文平均准确率，在文本能力上实现1.3-1.7倍吞吐量加速和0.2-0.3倍的计算成本。

Conclusion: 非对称上下文访问在压缩会丢弃关键推理信号时能带来显著收益，为解决LLM推理中的准确率-开销权衡提供了新思路。
...
### [143] [Imitation Learning for Connection-Tableau Construction](https://arxiv.org/abs/2608.26009)
*Fredrik Rømming,Mantas Bakšys,Martin S. Fixman,Sean B. Holden*

Main category: cs.AI

TL;DR: 将自动定理证明构建为基于策略的转换系统，结合图神经网络通过模仿学习训练，在M2k等基准上比leanCoP多解决46%的问题，且证明步数减少一个数量级。


<details>
  <summary>Details</summary>
Motivation: 自动定理证明器需要高效地选择证明步骤，传统方法依赖符号回溯搜索，效率较低，需要引入学习方法来提升性能。

Method: 将定理证明构建为基于正式演算的转换系统中的策略，使用图神经网络对证明编辑进行评分，通过模仿学习从已找到的证明中训练策略。

Result: 在固定步数预算下，学习后的策略在M2k、MPTP2078-bushy和TPTP v9.2.1上比leanCoP多解决最多46%的问题，且证明步数减少一个数量级。

Conclusion: 基于图神经网络的策略学习方法可直接应用于定理证明领域，显著提升证明效率，为构建更高效的自动定理证明器提供了新思路。
...
### [144] [Trace Integrity for LLM Data Agents: A Vision for Auditable Structured Reasoning in Real-World Systems](https://arxiv.org/abs/2608.26036)
*Srimonti Dutta,Akshata Kishore Moharir*

Main category: cs.AI

TL;DR: 论文提出Trace Integrity（追踪完整性）作为LLM数据智能体的新型可靠性评估标准，解决仅凭答案正确性无法验证计算过程有效性的问题。研究发现即使答案正确，也可能是由无效轨迹（invalid trace）产生的，这种"答案正确/无效轨迹"现象在真实部署中存在高风险。


<details>
  <summary>Details</summary>
Motivation: 自然语言推理和自由形式的理由无法可靠地指定真实系统所需的运算符级程序，存在"结构鸿沟"（Structure Gap）这一部署失败模式。现有评估仅看答案是否匹配参考答案，忽略了计算过程的可审计性。

Method: 引入Trace Integrity评估框架，从显式、可执行、模式有效、运算符忠实、可重放、答案一致和可审计七个维度验证计算轨迹；提出CAIT Rate（Correct Answer/Invalid Trace Rate）衡量仅评估答案时将计算不支持的输出误判为成功的比率；通过BIRD Mini-Dev数据集进行实证对比。

Result: Direct SQL、Operation Summary+SQL、Contract-First SQL三种方法的答案准确率分别为20%、22%、24%；Trace Integrity通过率分别为39%、43%、40%；CAIT Rate分别高达55%、59.1%、45.8%。表明答案准确率、轨迹有效性和静默失败风险是三个独立的评估信号。

Conclusion: LLM数据智能体应不仅按输出是否匹配参考答案来评估，更应评估输出是否由可审计的计算支撑。Trace Integrity应作为部署可靠性标准，而不仅仅是答案正确性。
...
### [145] [SwarmWorld: Stigmergic technological evolution in societies of language-model agents](https://arxiv.org/abs/2608.26081)
*Subhadeep Pal,Fiona Y. Wang,Markus J. Buehler*

Main category: cs.AI

TL;DR: 研究探讨了LLM智能体在无预设角色或工作流程的分散式环境中是否能通过共享环境自组织形成功能性的技术社会。SwarmWorld系统展示智能体可自主分化出探索、建设、维护和协调行为，无需外部干预即可积累技术。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体系统多依赖直接对话、预定义角色或集中式工作流。尚不清楚去中心化智能体是否能构建功能性技术并超越独立搜索。研究旨在填补这一空白，探索分散式LLM智能体是否能自组织形成技术社会。

Method: 在SwarmWorld环境中，初始同质的LLM智能体在无分配角色或配方的情况下运行。智能体探索空间环境、处理资源、测试材料、构建持久 artifacts，并编写由确定性模拟器评估的可执行控制器。实验在智能体移除后对系统施加未见干扰进行评估，并与强大的独立搜索基准进行对比。

Result: 共享社会开发出更广泛、更具弹性的技术组合，最强个体 artifact 仍由独立搜索产生。智能体分化为探索、建设、维护和协调行为，随世界成熟而转变。技术通过协作构建、可执行继承和持久智能体-artifact网络积累，大部分重用始于物理观察而非通信。显式文化机制放大协作和组织效益，但功能效益取决于结果和时间尺度。

Conclusion: 仅物理刺激即可支持 capable societies，而交互驱动持久技术生态而非普遍优越的个体发明。分散式LLM智能体能够在无预设角色情况下自组织形成功能性技术社会，集体智能可通过共享环境涌现。
...
### [146] [Planetary Prediction Engine: Autonomous Geospatial Prediction via Intelligent Data Selection and Foundation Model Embeddings](https://arxiv.org/abs/2608.26088)
...
TL;DR: 本文提出行星预测引擎(PPE)，一个自主AI系统，可直接从自然语言查询执行端到端地理空间建模工作流程。PPE动态合成多模态数据集，从开放网络和地球观测平台检索时空协变量，并与地理空间基础模型嵌入融合，同时搜索定制的模型架构族。在美国空间回归、尼日利亚粮食安全数据降尺度、刚果民主共和国埃博拉疫情预测等任务中，PPE均优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 应对全球关键挑战（粮食安全、灾害风险、疾病暴发、社会经济脆弱性）需要高保真地理空间建模。然而，构建预测性行星模型仍受限于碎片化的数据生态系统，需要手动数据检索、多模态数据整理与融合以及迭代模型选择。

Method: PPE系统包括：1)动态合成多模态数据集，从Data Commons、Google Earth Engine等平台检索时空相关协变量；2)与地理空间基础模型嵌入(PDFM、AlphaEarth)融合；3)使用自动过拟合保护搜索定制化模型架构家族。

Result: 美国空间回归任务：CDC健康指标R²从60.0%提升至76.8%，FEMA国家风险指数从60.0%提升至64.9%，社会脆弱性指数从58.6%提升至66.2%。尼日利亚粮食安全降尺度：R²从31.5%提升至66.1%（基线翻倍）。刚果民主共和国埃博拉疫情预测：Recall@10达83.3%（识别18个中的15个健康区域），较公开先进模型提升10.3个百分点。

Conclusion: PPE通过结合自主多模态行星数据发现与针对性模型优化，降低了行星级分析的技术门槛，使快速、定制化、专家级部署成为可能。
...
### [147] [BVR Sim: An Open and High-Throughput Environment for Heterogeneous Air-Combat Reinforcement Learning](https://arxiv.org/abs/2608.25419)
*Haocheng Sun,Mulai Tan*

Main category: cs.MA

TL;DR: BVR Sim是一个开源的Gymnasium风格超越视距(BVR)空战强化学习环境，支持异构平台、多后端、实体观测和组合奖励，可实现跨飞机模型策略迁移


<details>
  <summary>Details</summary>
Motivation: BVR空战是一个具有挑战性的强化学习领域，具有部分可观测性、长时序决策、能量管理和有限武器等特点，现缺乏支持异构平台的标准开源仿真环境

Method: 开发BVR Sim环境，支持多JSBSim飞机模型(F-15/F-16/F/A-18/F-22)，提供统一战术动作接口和可配置武器/传感器/控制器，实现Python和C++双后端，集成实体观测、组合奖励、脚本对手和多智能体框架适配器

Result: C++后端在0.4秒决策间隔下1v1达到104模拟秒/实际秒，10v10仍实用；F-16训练的策略无需再训练即迁移到四架未见飞机，配合飞机特定控制器适配后达到45.5%平均胜率；MAPPO和HAPPO实验验证了与标准多智能体强化学习管道的兼容性

Conclusion: BVR Sim为BVR空战强化学习提供了功能完整、高性能、可扩展的开源基准，支持异构平台策略迁移和多智能体训练
...
### [148] [Trust-Aware Sequential Decision Making and Rollout Planning for Resilient Multi-Robot Systems](https://arxiv.org/abs/2608.25690)
*Roee M. Francos,Daniel Garces,Orhan Eren Akgün,Nathaniel D. Bastian,Stephanie Gil*

Main category: cs.MA

TL;DR: 本文研究了在定位欺骗环境下的多机器人系统顺序决策问题，提出了距离约束的欺骗模型和分层二分图匹配策略，并开发了结合概率定位信任和行为证据的信任感知监测器来检测和移除恶意代理，从而恢复rollout策略的优势。


<details>
  <summary>Details</summary>
Motivation: 现有的多机器人路由系统假设规划信息可靠且代理能执行预期动作，但受损代理会违反这些假设，导致规划模型与物理执行之间产生不匹配，这在定位欺骗攻击下尤为严重。

Method: 1. 提出距离约束的欺骗模型，描述监视感知的对手行为；2. 设计分层二分图匹配策略，最大化分配影响力同时限制欺骗规模；3. 开发信任感知监测器，结合基于真实GPS欺骗数据校准的概率定位信任与任务执行行为证据进行代理分类；4. 移除检测到的对手以恢复规划-执行一致性。

Result: 实验使用真实GPS欺骗数据集和旧金山出租车需求数据，表明该方法在不同欺骗能力、对手数量、自适应攻击、监测配置和rollout horizon下均能有效检测并实现弹性路由。

Conclusion: 通过信任感知的对手移除可以恢复规划-执行一致性，使rollout策略稳定并恢复其相对于基础策略的经验优势，为多机器人系统在对抗环境下的安全路由提供了有效解决方案。
...
### [149] [HypoForge: A Self-Improving Multi-Agent Framework for Automated Hypothesis Generation and Testing via Scientific Skill Learning](https://arxiv.org/abs/2608.25770)
*Ziqing Qian,Jiaying Lei,Yifang Wang,Nan Cao*

Main category: cs.MA

TL;DR: HypoForge是一个经验引导的多智能体框架,通过对抗性生成器-判别器机制和改进的测试技能学习来实现自动假设生成和假设测试的持续改进,而无需微调基础模型。


<details>
  <summary>Details</summary>
Motivation: 现有AI科学家系统主要依赖静态提示或固定工作流程,无法积累经验进行持续改进,且假设生成阶段缺乏显式反馈,而假设测试阶段有经验反馈可用。

Method: 提出分阶段技能学习策略:(1)假设生成阶段采用对抗性生成器-判别器机制通过比较批评改进推理;(2)假设测试阶段从执行结果和真实标签学习测试技能;(3)将技能学习策略与阶段特定监督信号匹配,实现无需微调基础模型的持续改进。

Result: 在假设生成和测试基准测试中,HypoForge始终优于现有AI科学家框架和技能级变体。进一步分析验证了所提出的阶段特定技能学习范式的有效性。

Conclusion: 通过将技能学习策略与不同阶段的监督信号相匹配,HypoForge能够实现基础模型的持续改进,为自动化科学发现提供了有效的多智能体协作框架。
...
### [150] [AI Agentic Selective Laser Sintering Process Optimization](https://arxiv.org/abs/2608.25928)
*Peter Pak,Victor Alvarado,Amir Barati Farimani*

Main category: cs.MA

TL;DR: 研究人工智能代理系统在选择性激光烧结（SLS）中的工艺参数优化能力，通过迭代优化提升材料机械性能。


<details>
  <summary>Details</summary>
Motivation: 增材制造中的工艺参数优化复杂且耗时，传统方法依赖大量试错，需要自动化和智能化的优化方法。

Method: 采用AI代理系统，在Inova Mk1设备上对三种材料（PA12 GF、PA11 Onyx、PA12 Blend）进行迭代优化，利用历史构建数据和用户最小指导调整工艺参数。

Result: 代理系统在少量迭代内成功优化工艺参数，达到与TDS规格相当的机械性能（拉伸性能和弯曲性能）。

Conclusion: 代理系统能够持续从新数据中学习，实现SLS工艺参数优化的智能自动化，证明其在复杂制造任务中的应用潜力。
...
### [151] [Praxist: From Experimental Artifacts to Solution Lineages](https://arxiv.org/abs/2608.25955)
...
TL;DR: Praxist是一个以谱系为中心的代际系统，将可复现制品和评估器结果转换为带类型的证据图，包含发现成果、车道结构的前沿和议程。它将本地制品构建与队列级证据合成分离，使后续尝试能够继承已验证的机制、未解决的索赔和有价值的约束。


<details>
  <summary>Details</summary>
Motivation: 当前自主R&D智能体主要作为实验室仪器运作——在精心设计的基准上展示，但收益难以追溯到具体原因，成本远高于持续工程实践的吸收能力。局限性是结构性的：大多数系统将每次尝试视为几乎独立的，因此日志、记忆和搜索树记录了发生了什么，但没有确定哪个设计元素产生了改进、其证据是否通过了验证、或如何与其他元素重新组合。长期研究不断重复学习相同的经验教训。

Method: 提出Praxist系统：一个以谱系为中心的代际系统，将可复现制品和评估器结果转换为带类型的证据图，包括发现成果、车道结构的前沿和议程。关键创新是将本地制品构建与队列级证据合成分离，使后续尝试能够继承已验证的机制、未解决的索赔和有价值的约束，并将结果附加到可检查的谱系上。

Result: 在标准化的75项任务MLE-bench套件上，最终官方评分结果：Praxist获得60枚奖牌（80.0%），其中49枚金牌；Claude Code基线在Claude Opus 4.8上获得55枚奖牌（73.3%），34枚金牌。记录在案的模型支出：Praxist为3,054美元对比38,370美元，约为成本的1/12。四个案例研究（量化交易、LiDAR-惯性视觉SLAM、托卡马克磁控制、火箭着陆）在开放性工程问题上应用相同流程，在准确率、生存率或资源成本方面均超越任务原生基线。

Conclusion: 据我们所知，这是首次将更强的人工制品与低一个数量级的支出相结合，且每项成果都有可审计的谱系支持。这代表了生产研究所需的运作模式，而非基准演示所需的模式。
...
### [152] [Sequential Euclidean tree construction with exponential memory: distributional performance and worst-case guarantees](https://arxiv.org/abs/2608.25298)
*Pedro M. M. de Castro*
