...
### [1] [Untitled](https://arxiv.org/abs/2609.10572)
**
...
### [2] [Untitled](https://arxiv.org/abs/2609.10723)
**
...
### [3] [Untitled](https://arxiv.org/abs/2609.11573)
**
...
### [4] [Untitled](https://arxiv.org/abs/2609.10743)
**
...
### [5] [Untitled](https://arxiv.org/abs/2609.10749)
**
...
TL;DR: 本文提出了一个基准测试，用于评估图像生成模型的光照理解和协调能力。核心观点是：通过测试模型将新物体插入真实照片时是否能保持一致的光照，来评估其光照理解能力。使用多光照数据集，将物体作为"光线探针"，让模型对同一物体进行修复，然后与真实光线探针进行比较，从而量化光照方向、颜色和辐射分布的准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管图像合成取得了显著进展，但生成模型是否真正理解物理上准确的光照仍然是一个开放问题。现有的评估方法缺乏对模型光照理解能力的系统评估，因此需要建立一个可扩展的评估协议来系统地评估生成模型捕捉和再现真实世界光照的能力。

Method: 使用多光照数据集中的图像，其中包含作为"光线探针"的简单物体提示模型将同一物体修复到原始图像中，然后比较生成结果与真实光线探针。从修复的探针中估计光照方向、颜色和辐射分布，从而提供光照准确性和光度真实感的定量测量。

Result: 建立了可扩展的评估协议，能够系统地评估生成模型捕捉和再现真实世界光照的能力，为未来任何模型的光度准确性基准测试提供了基础。

Conclusion: 该工作提出了一个评估生成模型光照理解能力的基准框架，通过物体插入任务来量化光照准确性，为未来模型的光照理解能力评估奠定了基础。所有代码和数据已公开可用。
...
TL;DR: 本文提出了一种基于非自治ODE的深度学习 diffeomorphic 图像配准框架，通过强制执行余圈一致性（cocycle consistency）来直接学习连续时间流映射，避免了训练过程中的时间离散化和速度场积分。该方法可无缝集成标准配准骨干网络，在9个数据集上保持 diffeomorphic 结构的同时提升配准精度。


<details>
  <summary>Details</summary>
Motivation: 1. Diffeomorphic 图像配准是医学图像分析的核心技术；2. 现有学习方法通过参数化平稳速度场并使用缩放-平方方法恢复形变；3. 非自治ODE（时变速度场）虽更具表达力，但现有方法依赖数值积分，限制了模型表达力与离散化精度的解耦；4. 需要一种不依赖时间离散化和速度场积分的训练框架。

Method: 1. 将非自治ODE表述为双参数流映射（two-parameter flow map）；2. 强制执行余圈一致性——这是时变流的基本结构性质；3. 训练过程中直接学习流映射，无需时间离散化和速度场积分；4. 推理时使用少量组合（compositions）恢复 diffeomorphic 映射；5. 可无缝集成现有标准配准骨干网络。

Result: 在9个数据集上一致提升配准精度并保持 diffeomorphic 结构：1. 脑部MRI：Dice平均提升2.1%；2. 肺部CT：TRE降低12%；3. 心脏MRI和超声：Dice提升2.6%。

Conclusion: 提出的非自治ODE框架通过余圈一致性约束成功实现了连续时间流映射的直接学习，解决了传统方法中模型表达力与离散化精度耦合的问题。该方法在多种医学图像模态上展现了显著的精度提升，具有重要的临床应用价值。
...
### [9] [Untitled](https://arxiv.org/abs/2609.10801)
**
...
### [10] [Untitled](https://arxiv.org/abs/2609.10806)
**
...
### [11] [Untitled](https://arxiv.org/abs/2609.10811)
**
...
TL;DR: 本文提出了一种针对执法领域 body-worn camera (BWC) 视频的自适应视觉问答 (VQA) 框架，旨在从低质量、多模态的执法录像中提取关键法医证据，解决当前视觉语言模型忽略重要细节的问题。


<details>
  <summary>Details</summary>
Motivation: BWC 视频具有场景混乱、画质模糊、快速运动交互、高噪音音频等特点，现有 SOTA 视觉语言模型常遗漏关键法医细节（如证据存在性、警民交互的隐含ニュアンス），影响执法公正性和警民安全。

Method: 提出自适应 VQA 框架，采用结构化推理方法提取细粒度视觉证据。实验对比了多种问题生成模型，包括基础模型和微调开源模型，以观察不同实现的性能差异。

Result: VQA 驱动的架构能提供更可靠、客观、详细的事件记录，作为 AI 辅助法医分析工具，可有效保护执法人员和公众的安全。

Conclusion: 该框架为高风险执法场景提供了更精准的视觉证据提取方案，有望成为提升执法透明度和问责性的重要技术手段。
...
### [14] [Untitled](https://arxiv.org/abs/2609.10855)
**
...
### [15] [Untitled](https://arxiv.org/abs/2609.10898)
**
...
TL;DR: HiPerViT是一种紧凑的纯视觉架构，通过将二阶统计先验显式注入基于transformer的识别流程来提升纹理识别性能。该方法结合全局和局部图像视图，将双线性描述符编码为统计令牌，并通过Perceiver风格的潜在蒸馏将其与一阶空间表示集成。


<details>
  <summary>Details</summary>
Motivation: 现代视觉模型在纹理识别方面仍具挑战性，因为判别性证据通常由高阶空间统计而非仅由物体形状携带。Vision Transformers虽提供强长程建模能力，但其标准以对象为中心的表示不会显式暴露这种统计结构，限制了细粒度识别中的纹理敏感性。

Method: HiPerViT结合全局和局部图像视图，将紧凑的双线性描述符编码为统计令牌，并将其与一阶空间表示通过Perceiver风格的潜在蒸馏进行集成，使空间令牌与二阶特征共现统计信息直接交互。

Result: 在六个纹理识别基准测试中，HiPerViT相比强视觉基线取得一致改进：DTD上+3.05个百分点，GTOS-Mobile上+10.48，1200Tex上+10.10。分析表明这些改进很大程度上不受用于提取二阶统计的骨干深度以及交互和蒸馏阶段顺序的影响。

Conclusion: 主要改进来源不是特定的融合拓扑，而是二阶统计信息作为一等表示信号的显式可用性。这支持将显式统计令牌化作为纹理中心视觉识别的有效和鲁棒设计原则。
...
### [17] [Untitled](https://arxiv.org/abs/2609.10943)
**
...
### [18] [Untitled](https://arxiv.org/abs/2609.11022)
**
...
TL;DR: M2Heat是一个受物理学启发的多模态融合框架，将热传导原理应用于高光谱与LiDAR数据融合，通过vHeat模块和FVEs实现各向异性信息流动，以亚二次复杂度捕获全局依赖，并结合CFF模块提升特征表示的判别性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有融合方法难以在保持计算效率的同时建模长程依赖和复杂各向异性交互；希望通过热传导的物理视角为多模态融合提供新的解决思路。

Method: 提出了物理驱动的视觉热传导模块(vHeat)和增强的频率值嵌入(FVEs)来模拟各向异性信息流动；设计了跨频率融合(CFF)模块实现混合空间-频率融合策略；在三个基准数据集上进行评估。

Result: 在Trento、Houston2013和Augsburg三个基准数据集上取得了具有竞争力的整体性能，同时提供了可解释的热传导引导视角来理解多模态特征融合。

Conclusion: M2Heat框架为遥感多模态融合提供了一种新的物理启发式方法，在保持计算效率的同时实现了可解释的特征融合。未来将探索将热传导引导的神经算子应用于更多遥感任务。
...
### [20] [Untitled](https://arxiv.org/abs/2609.11041)
**
...
TL;DR: TailProp是一种基于尾部传播算子(TPO)的新型视觉骨干网络，使用高斯和柯西稳定过程传播器作为互补基，结合内容条件的通道系数进行自适应融合，在图像分类、目标检测、语义分割等任务上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉传播模型通常在特定动力学家族内构建，忽视了不同样本、通道和网络阶段需要截然不同的空间交互。现有方法无法灵活适应跨 regime 的自适应传播需求。

Method: 提出尾部传播算子(TPO)，使用高斯和柯西稳定过程传播器作为互补基（分别具有快速衰减和重尾空间影响），预测内容条件的通道系数来自适应组合它们。通过DCT/IDCT单对在DCT域直接融合，实现O(N^1.5)空间混合复杂度。

Result: TailProp-B在ImageNet-1K上达到84.4% Top-1准确率；3x Mask R-CNN schedule下达到50.3/44.8 box/mask AP；ADE20K上达到50.8% mIoU。消融实验表明双基传播优于单基传播、同家族分支和家族内自适应阶数。

Conclusion: 双基传播是视觉表示学习的有效设计原则，TailProp在多个视觉任务上一致优于匹配的传播基线，证明了跨regime自适应传播的优越性。
...
### [22] [Untitled](https://arxiv.org/abs/2609.11126)
**
...
### [23] [Untitled](https://arxiv.org/abs/2609.11129)
**
...
TL;DR: 该论文通过构建包含100万张智能手机图像的LAION-Mobile数据集，揭示了当前最先进的深度伪造检测器在现代计算摄影图像上几乎完全失效：所有检测器在现代AI生成内容上AUC不超过0.624，且无法同时实现可接受的误报率。


<details>
  <summary>Details</summary>
Motivation: 解决现有深度伪造检测器评估中忽略现代智能手机计算摄影影响的问题。现有检测器在传统基准上表现接近完美，但现代智能手机的神经图像信号处理（neural-ISP）可能改变图像生成范式，导致误判普通手机照片为伪造。

Method: 构建LAION-Mobile数据集（包含约100万张带有EXIF元数据的智能手机图像），对12个最先进的深度伪造检测器进行大规模评估，使用9,115张图像的评估样本（DIRE子集738张）

Result: （1）在现代AI生成内容上，所有检测器AUC均不超过0.624，5个检测器低于随机概率；（2）基于传统GAN数据校准的阈值会导致可部署的误报率（<11%），但同一检测器在现代内容上误报率高达17-91%；（3）没有检测器能同时满足在现代AI内容上超过随机概率并保持可部署的误报率这两个条件。

Conclusion: 当前最先进的深度伪造检测器在现代智能手机图像上表现极差，无法同时满足两个条件：在现代AI生成内容上超过随机概率，并保持可部署的误报率。研究强调了开发针对现代计算摄影优化的深度伪造检测器的紧迫性，以及创建包含当前旗舰设备图像的新数据集的必要性。
...
TL;DR: UniH3是一个统一的医学图像修复框架，通过利用医学图像的层次同质性（共享解剖结构）和异质性来解决多种模态和退化类型的一体化医学图像修复问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注建模任务间的异质性（如不同数据分布和退化类型），但忽视了医学图像内在的同质性，如模态内和解剖结构之间的广泛共享特性，这些同质性可用于简化模型训练并提高泛化能力。

Method: 提出UniH3框架，包含两个核心模块：1）层次同质性记忆（H2M）模块：在训练过程中从高质量图像中逐步提取任务内和任务间的同质性先验，并针对输入自适应检索最相关的先验；2）同质性引导注意力（HGA）机制：将检索的先验注入修复管道；3）层次异质性平衡器（H2B）：缓解优化过程中的任务间和任务内冲突，促进平衡有效的多任务学习。

Result: 在两个大规模基准数据集（MedIR-2D-500K和MedIR-3D-3K）上的广泛实验表明，UniH3在一体化和单任务医学图像修复方面均达到了最先进的性能。

Conclusion: UniH3框架通过有效统一层次同质性和异质性，为通用医学图像修复模型建立了强有力的基准，推动了通用医学图像修复模型的发展。
...
### [26] [Untitled](https://arxiv.org/abs/2609.11172)
**
...
TL;DR: 本文提出了一种基于多视角和混淆引导的集成框架，用于合成图像归因任务。该方法结合了FFT-ConvNeXt、DINOv2、CLIP和Xception等多种架构，从频率、语义和取证角度捕获不同的归因线索。通过广泛的数据增强策略提高模型对未知退化和图像操作的鲁棒性。针对Stable Diffusion 3和SD 3.5之间的严重混淆问题，引入了专用的二值专家分类器。同时应用类别自适应置信度校准来提高腾讯混元等困难类别的区分度。最终在公开榜上达到99.53%，私有榜上达到99.20%的准确率。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像生成模型的快速发展，合成图像归因变得越来越重要。然而，由于现代基于扩散的生成器之间日益相似，以及存在多样化的后处理操作，准确识别生成图像的来源模型仍然具有挑战性。

Method: 1. 多架构集成：整合FFT-ConvNeXt、DINOv2、CLIP和Xception四个互补架构
2. 多视角特征提取：从频率、语义和取证角度捕获归因线索
3. 数据增强：模拟压缩、resize、灰度转换、模糊等真实后处理操作
4. 二值专家分类器：针对SD 3和SD 3.5的混淆问题，在低置信度条件下激活
5. 类别自适应置信度校准：提高腾讯混元等困难类别的区分度

Result: 在公开排行榜上达到99.53%的准确率，在私有排行榜上达到99.20%的准确率。

Conclusion: 所提出的多视角和混淆引导集成框架能有效应对合成图像归因中的多源模型识别和后处理干扰问题，通过集成多架构和针对性策略可实现高精度归因。
...
TL;DR: 本文提出CEM-TUDASR，一种基于Transformer的无监督超分辨率框架，用于增强无线胶囊内镜（WCE）图像质量，无需成对的低分辨率/高分辨率训练数据。


<details>
  <summary>Details</summary>
Motivation: WCE图像由于微型光学元件、传感器限制和无线传输约束导致分辨率低、诊断重要结构可见度差，需要提升图像质量。

Method: 1) 设计领域自适应退化网络从常规内镜图像合成逼真的WCE类低分辨率图像，缩小领域差距；2) SR生成器整合深度注意力块（DAB）和融合注意力块（FAB）捕捉长程上下文依赖和精细局部结构；3) 在Kvasir Capsule数据集上训练，在KID和GIANA上评估。

Result: CEM-TUDASR在无参考质量指标（BRISQUE、PIQE、NIQE、EndoQM）上显著优于现有无监督SR方法，定性结果显示黏膜纹理、血管模式和临床相关解剖细节得到改善。仅需270万参数和169.94 GFLOPs即可实现高质量重建。

Conclusion: CEM-TUDASR在实现优异图像重建质量的同时保持了计算效率，适用于资源受限的临床和嵌入式内镜应用，并展示了跨域（视网膜图像）泛化能力。
...
TL;DR: Tri-DehazeGS提出了一种场景-介质解耦的高斯溅射框架，用于从有雾多视角图像中恢复清晰3D场景。该方法使用独立视角共享的三平面场建模参与介质，并通过物理散射模型合成雾观测。


<details>
  <summary>Details</summary>
Motivation: 现有方法将 haze 退化应用于图像空间或将介质相关变量绑定到高斯原语，导致清洁场景辐射与大气效应混淆。低透射率区域提供弱监督，导致远处或浓雾区域重建不足。

Method: 1. 使用高斯原语表示清洁场景
2. 采用独立视角共享的三平面场建模参与介质
3. 通过物理散射模型合成雾观测
4. 提出介质解耦透射率梯度补偿(MD-TGC)，在介质冻结后补偿雾抑制梯度

Result: 在真实和合成雾基准测试上的实验表明，Tri-DehazeGS提高了清洁新视角重建质量。

Conclusion: 通过场景-介质解耦和透射率感知优化重平衡，Tri-DehazeGS能够有效从雾图像中恢复清晰3D场景。
...
TL;DR: 研究测试时适应(TTA)中选择器能否从无标签测试数据可靠地预测最佳适应策略。核心问题：选择器获得的证据是否足以确定最佳行动？研究发现：当观测通道使两种部署看起来相同但TTA排名不同时，即使完美选择器也无法可靠选择。通过有限批次高斯TTA模型精确界定这一边界：在小偏移时"不做任何改变"优于均值重中心化；超过临界偏移后重中心化更好；边界随1/√n收缩。在CIFAR-100-C和DomainNet-126上验证了现代TTA方法的失败模式。贡献：区分两种常被混合的失败模式——弱选择器与信息不足通道。


<details>
  <summary>Details</summary>
Motivation: 当前TTA方法尝试从无标签测试数据预测哪种适应策略有效，但作者提出了一个更根本的问题：选择器获得的证据本身是否包含足够信息来做出正确决策？即使用完美的选择器也无法保证做出正确选择。

Method: 1)理论分析：建立有限批次高斯TTA模型，推导最优策略的精确边界条件；2)实证验证：在CIFAR-100-C和DomainNet-126基准数据集上测试现代TTA方法

Result: 1)理论上证明：当观测通道使不同部署看起来相同但TTA排名不同时，无法实现可靠选择；2)有限批次模型显示：小偏移时"不做任何改变"最优，超过临界偏移后均值重中心化最优，边界随样本量1/√n衰减；3)实验证实：仅改变部署结构可逆转oracle行为，而全局顺序无关证据保持不变

Conclusion: 提出区分两种失败模式的实用方法：弱选择器 versus 本身无法支持所需决策的信息通道。强调了TTA研究中一个基本限制——不仅需要更好的选择器，还需要确保观测通道包含足够区分不同部署的信息。
...
TL;DR: HALDETECT是一个用于检测多模态模型视觉幻觉的系统,在ImageEval 2026英语幻觉检测赛道(Task 1b)中获得第三名。系统需要从一张图片和三个文化上合理的陈述中识别出唯一的视觉 grounded陈述。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型倾向于流畅地产生视觉细节幻觉,这限制了其在细粒度解释任务中的部署。解决这一问题的动机是提高模型在视觉问答中的可靠性。

Method: 采用对比决策框架,在给出解释前先输出答案,并围绕颜色/纹理、形状/形式和上下文进行推理。使用4-bit QLoRA对Qwen2.5-VL-7B-Instruct进行适配微调,同时冻结视觉编码器。

Result: 在1000项测试集上达到Contrastive Instability (CI) 0.035,在8个团队中排名第三。实验表明答案顺序可能比模型规模更重要,适配优于单纯提示。回顾性分析确认了QLoRA相对于最佳提示的增益,但训练数据规模曲线在改变随机种子后不成立。35个残余错误涉及文化上合理的功能、材料和识别区分。

Conclusion: 该研究证明了适配微调方法在视觉幻觉检测任务中的有效性,但同时也揭示了答案顺序敏感性、数据Scaling的不稳定性以及残余错误主要涉及文化相关区分等挑战。
...
### [32] [Untitled](https://arxiv.org/abs/2609.11237)
**
...
### [33] [Untitled](https://arxiv.org/abs/2609.11240)
**
...
### [34] [Untitled](https://arxiv.org/abs/2609.11242)
**
...
### [35] [Untitled](https://arxiv.org/abs/2609.11265)
**
...
TL;DR: 本文提出了一种深度学习流水线，通过自动恒星去除和背景重建来增强光学空间态势感知（SSA）图像中微弱移动目标的检测能力。该方法结合Tiny-U-Net分割网络生成恒星掩码和部分卷积变分自编码器（astro-VAE）进行上下文感知的背景修复。


<details>
  <summary>Details</summary>
Motivation: 在光学观测中，特别是cislunar（X-GEO）区域，低信噪比（SNR）目标的检测仍然极具挑战性。 classical检测算法在处理结构化天空背景、密集恒星场和散射月光时性能显著下降。

Method: 提出了一种结合轻量级分割网络（ Tiny-U-Net）和部分卷积变分自编码器（astro-VAE）的深度学习流水线。Tiny-U-Net用于生成恒星掩码，astro-VAE学习天文背景的统计分布并对掩码区域进行上下文感知修复。该方法与shift-and-stack检测方案集成，并在实际地面望远镜观测X-GEO区域的真实数据上进行了验证。

Result: 实验结果表明，该方法能够高保真地重建无恒星背景，同时保留移动目标，显著增强了检测能力。

Conclusion: 为光学SSA场景中微弱移动目标检测提供了一种有效的数据驱动预处理策略。
...
### [37] [Untitled](https://arxiv.org/abs/2609.11271)
**
...
### [38] [Untitled](https://arxiv.org/abs/2609.11279)
**
...
### [39] [Untitled](https://arxiv.org/abs/2609.11310)
**
...
### [40] [Untitled](https://arxiv.org/abs/2609.11312)
**
...
### [41] [Untitled](https://arxiv.org/abs/2609.11317)
**
...
### [42] [Untitled](https://arxiv.org/abs/2609.11330)
**
...
TL;DR: R4Tun是一个基于大语言模型(LLM)的隧道衬砌自适应分割框架，通过记忆(m)、状态(s)和知识(k)三种结构化上下文信息来调整专家设计的SAM4Tun管道参数，解决隧道条件变化导致的分割性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有专家调优的分割管道在隧道条件变化时性能下降，需要人工重新调参；传统方法缺乏自适应能力，无法应对隧道多样性。

Method: 提出R4Tun框架，利用LLM根据结构化上下文（记忆m、状态s、知识k）自动调整SAM4Tun管道参数，在30个Seg2Tunnel子集（13个常规、17个复杂）上测试三种LLM，评估270次运行。

Result: mIoU从0.18提升至0.43-0.48，OA从0.42提升至0.59-0.65；常规子集达mIoU 0.784-0.796；三种LLM显示相似参数调整趋势，95%置信区间重叠。

Conclusion: R4Tun作为可控、无标签、跨LLM的适配机制，在SAM4Tun-Seg2Tunnel设置中实现一致的精度提升，为隧道检测提供可审计的参数量化调整方案。
...
### [44] [Untitled](https://arxiv.org/abs/2609.11375)
**
...
### [45] [Untitled](https://arxiv.org/abs/2609.11378)
**
...
### [46] [Untitled](https://arxiv.org/abs/2609.11380)
**
...
TL;DR: MOCO是一种基于扩散模型的多模态3D avatar运动生成框架，能够同时处理语音音频、文本描述和轨迹数据，在没有对齐多模态数据的情况下生成连贯、自然的运动。


<details>
  <summary>Details</summary>
Motivation: 人类自然地同时行走和说话，但现有方法受限于对齐多模态数据的稀缺性，通常通过顺序组合运动或加权求和来生成多模态运动，导致动作不匹配或不自然。

Method: 提出解耦运动生成过程：在每个去噪步骤中，扩散模型从输入噪声独立生成各模态的运动，然后根据预定义的空间规则组装身体部位；将组合后的运动扩散作为下一个去噪步骤的输入噪声，实现迭代优化。

Result: 在构建的多模态基准测试上，MOCO优于现有基线方法，在多模态运动生成任务上取得先进水平。

Conclusion: 该方法通过迭代 refinement 使各模态在整体运动上下文中逐步协调，生成的运动越来越自然流畅，实现了连贯同步的行为。
...
TL;DR: BruNet是一个结合ViT编码器和SAM解码器的 bruise（瘀伤）分割框架，首次实现自动瘀伤分割，在跨域泛化方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 瘀伤分割面临标注数据有限、边界模糊、外观变化大等挑战。现有研究主要集中在检测、分类或颜色分析，缺乏像素级定位的自动分割方法。

Method: BruNet结合了基于ViT的视觉编码器（DINOv3自监督或预训练的LingBot-Vision）与SAM掩码解码器，在HAM10000皮肤病变数据集上训练，在独立的瘀伤数据集上评估。

Result: BruNet优于CNN模型、最先进的分割模型、ChatGPT-4o/5辅助的SAM2零样本基线以及医学导向的MedSAM模型，展示了强大的跨域泛化能力。

Conclusion: 这是首个实现自动瘀伤分割的研究，BruNet在跨域泛化方面表现出色，为医学影像中瘀伤的精确分割提供了新思路。
...
### [49] [Untitled](https://arxiv.org/abs/2609.11472)
**
...
### [50] [Untitled](https://arxiv.org/abs/2609.11477)
**
...
### [51] [Untitled](https://arxiv.org/abs/2609.11486)
**
...
### [52] [Untitled](https://arxiv.org/abs/2609.11499)
**
...
TL;DR: UBone3D是一个基于物理校正条件流匹配的新框架,用于从部分超声观测中直接完成点云重建。该方法通过模拟物理代理对确定性物理伪影进行建模,并引入测试时物理校正来引导形状完成。


<details>
  <summary>Details</summary>
Motivation: 三维超声(US)是一种安全、无辐射的补充成像方式,可用于纵向监测,但其分割导出的部分点云充满伪影,难以从中恢复出干净、完整的解剖结构。现有方法难以处理超声点云中的表面增厚、条纹和缺失等伪影问题。

Method: 提出UBone3D框架,基于物理校正条件流匹配(CFM)。具体包括:(1)通过模拟物理代理建模确定性物理伪影(表面增厚、条纹、缺失);(2)引入测试时物理校正机制;(3)使用CT训练的生成形状先验BoneFM执行解剖合理性约束;(4)使用USimNet在超声形成空间实施物理一致性约束。

Result: 在模拟数据和体内数据上的广泛实验表明,该方法在重建精度和解剖保真度方面相比现有基线方法取得了显著改进。

Conclusion: UBone3D能够有效地从部分超声观测中恢复完整的解剖结构,通过结合解剖合理性(来自CT先验)和物理一致性(来自超声形成模型)两种解耦力,为医学影像点云补全提供了新思路。
...
### [54] [Untitled](https://arxiv.org/abs/2609.11507)
**
...
### [55] [Untitled](https://arxiv.org/abs/2609.11514)
**
...
TL;DR: 本文提出InterIL模型，用于从输入文本生成图形设计模板（包括背景图像和前景元素布局）。该模型采用联合生成方式，通过可学习的通信模块连接预训练的图像和布局扩散模型，实现双向图像-布局交互。


<details>
  <summary>Details</summary>
Motivation: 现有图形设计生成方法多采用顺序范式，按序生成设计元素。然而这种顺序方案难以准确捕捉背景与布局之间的依赖关系，也无法很好地建模联合图像-布局分布，从而限制了生成质量。

Method: 提出InterIL模型，在单个生成过程中联合生成背景图像和布局。核心创新包括：1)设计可学习的通信模块连接预训练的图像和布局扩散模型；2)显式建模双向图像-布局交互；3)训练时冻结图像和布局骨干网络，仅更新通信模块以专注于学习交互；4)引入测试时引导策略使用户能施加特定偏好。

Result: 实验表明，与先前方法相比，该模型在图像、布局及图像-布局和谐性方面均能生成显著更好的结果，输出更接近真实样本。同时展示了模型在推理时无需重新训练即可强制执行用户偏好的灵活性。

Conclusion: 联合生成方案结合双向交互设计能有效捕捉联合图像-布局分布，比顺序方法更能保留真实设计作品的原始特征，为图形设计模板生成提供了更优的解决方案。
...
### [58] [Untitled](https://arxiv.org/abs/2609.11548)
**
...
### [59] [Untitled](https://arxiv.org/abs/2609.11550)
**
...
TL;DR: 针对多模态大语言模型(Omni-LLMs)的KV缓存量化问题，提出OmniKVQuant框架，通过窗口级key量化范围设置和按模态分离value旋转，实现2-bit KV缓存同时保持性能。


<details>
  <summary>Details</summary>
Motivation: Omni-LLMs同时处理音频、视频和文本，导致KV缓存内存成本大幅增加。文本-only LLM中常用的KV缓存量化方法在Omni-LLMs上的应用尚未被探索。

Method: 识别出TurboQuant在多模态缓存上的两个关键问题：时序key漂移和异构value几何。提出OmniKVQuant框架：(i)对输入流的每个短窗口设置key量化范围；(ii)按模态分别旋转value。

Result: 在Qwen2.5-Omni和Qwen3-Omni上，OmniKVQuant实现了2-bit KV缓存，同时在七个视听基准上基本保持性能。

Conclusion: OmniKVQuant是一个无需训练的框架，能够有效解决多模态KV缓存量化的关键问题，为Omni-LLMs的高效部署提供了可行方案。
...
### [61] [Untitled](https://arxiv.org/abs/2609.11601)
**
...
### [62] [Untitled](https://arxiv.org/abs/2609.11616)
**
...
### [63] [Untitled](https://arxiv.org/abs/2609.11638)
**
...
### [64] [Untitled](https://arxiv.org/abs/2609.11650)
**
...
TL;DR: 针对浮游生物成像数据集中稀有分类群样本不足的问题，提出一种基于CLIP编码器和扩散Transformer生成合成浮游生物图像的方法，通过Taxonomy-conditioned合成数据改善长尾分布下的分类器性能。


<details>
  <summary>Details</summary>
Motivation: 自动化浮游生物成像产生严重长尾分布的数据集，稀有分类群（生态学上最重要的）图像数量不足以训练或评估分类器，需要生成合成图像来解决这一问题。

Method: 1) 在大规模浮游生物语料库上使用扩展到深度、不规则分类学的排名对比目标来适应CLIP编码器；2) 将冻结的CLIP编码器作为条件，训练参数高效的扩散Transformer；3) 基于分类学生成合成浮游生物图像。

Result: 合成样本质量在分布保真度和下游分类器实用性两个维度进行评估。

Conclusion: 该方法能够有效生成高质量的合成浮游生物图像，为长尾分布下的稀有分类群分类提供数据支持。
...
### [66] [Untitled](https://arxiv.org/abs/2609.11680)
**
...
### [67] [Untitled](https://arxiv.org/abs/2609.11703)
**
...
TL;DR: LASP是一个语言增强的语义先验框架，利用大语言模型从CAD建模历史中推断结构化的B样条先验参数，作为传统几何求解器的归纳偏置，实现语言引导的几何优化。


<details>
  <summary>Details</summary>
Motivation: 传统CAD几何内核依赖预定的启发式初始化，忽略了建模历史中的程序化语义和设计意图，导致拟合结果次优且语义不一致，需要弥合高层设计意图与求解器可执行几何配置之间的差距。

Method: 将建模历史翻译为捕获设计意图、几何上下文和功能关系的丰富文本描述，使用微调LLM预测结构化B样条先验参数，采用两阶段训练方案结合局部几何规则性与长程上下文依赖性。

Result: 实验表明语言驱动推理可作为强大的几何求解归纳偏置，使B样条拟合过程更准确封装设计目标，呈现更高的语义一致性。

Conclusion: 建立了现代CAD系统中语言引导几何优化的新范式，证明了语言增强推理在几何求解中的有效性。
...
### [69] [Untitled](https://arxiv.org/abs/2609.11717)
**
...
### [70] [Untitled](https://arxiv.org/abs/2609.11722)
**
...
TL;DR: 本文提出Logit Refiner模块，解决Visual Autoregressive Models (VAR)中并行解码导致的同尺度token间空间依赖丢失问题。该模块通过轻量级自回归方式恢复尺度内依赖，仅增加约10%参数和不足5%的训练计算，即可插入任何预训练VAR模型。


<details>
  <summary>Details</summary>
Motivation: VAR模型采用next-scale预测范式，在每个尺度内并行生成所有token，这种并行解码构成了mean-field风格近似，丢弃了同尺度token之间的空间依赖，导致局部不一致的样本，这是解码规则的固有局限性，与主干网络容量无关。

Method: 提出Logit Refiner，一个轻量级自回归模块，通过在冻结的主干特征条件下顺序采样token来恢复尺度内依赖。该模块可插入任何预训练VAR检查点而无需重新训练，仅增加约10%参数和不足5%的基线模型训练计算。控制性消融实验表明，联合尺度内采样而非额外容量或训练才是关键要素。

Result: 在310M到2B参数的多种主干网络上，refiner一致提升生成质量，使1.1B参数模型超越两倍大小的模型。该方法还泛化到text-to-image生成，证实mean-field瓶颈在各种VAR变体中普遍存在且能被有效缓解。

Conclusion: 尺度内依赖的恢复是提升VAR生成质量的关键，Logit Refiner作为轻量即插即用模块可有效解决并行解码的固有局限，无需大规模重训练即可显著提升生成效果。
...
### [72] [Untitled](https://arxiv.org/abs/2609.11886)
**
...
### [73] [Untitled](https://arxiv.org/abs/2609.11894)
**
...
TL;DR: 针对边缘设备长视频理解的难题，提出Caption-once, Frames-onDemand (CFD)框架，通过离线构建视觉-文本双轨索引和运行时查询条件下的按需关键帧检索，实现精度-效率权衡。


<details>
  <summary>Details</summary>
Motivation: 边缘设备处理长视频（数小时内容）面临严格的计算和带宽约束。现有方法存在问题：采样视觉token会丢失时间结构，纯文本视频记忆则丢失细粒度视觉属性。

Method: 提出视觉-文本对偶性洞察：语言记忆更适合携带长程时间结构，像素更适合属性级感知。在此基础上设计CFD框架：1) 边缘端单次离线标注构建双轨叙事索引（事件级故事骨架+片段级微日志）；2) 云端MLLM基于故事优先循环推理；3) Visual-Need Router根据查询类型决定是否触发关键帧检索，仅对感知问题（外观、屏幕文字、属性消歧）调用视觉处理。

Result: 在长视频基准测试中实现强精度-效率权衡，显著减少在线视觉处理开销。

Conclusion: 通过将视觉访问转变为查询条件下的可计算成本，CFD实现了与视频长度无关的每查询帧消耗上限，为边缘云协同的长视频理解提供了新范式。
...
### [75] [Untitled](https://arxiv.org/abs/2609.11929)
**
...
### [76] [Untitled](https://arxiv.org/abs/2609.10817)
**
...
### [77] [Untitled](https://arxiv.org/abs/2609.10939)
**
...
### [78] [Untitled](https://arxiv.org/abs/2609.11108)
**
...
### [79] [Untitled](https://arxiv.org/abs/2609.11737)
**
...
### [80] [Untitled](https://arxiv.org/abs/2609.10584)
**
...
### [81] [Untitled](https://arxiv.org/abs/2609.10724)
**
...
TL;DR: 本文提出了一种端到端的多智能体框架，用于从自然语言问题描述自动生成QUBO（非约束二进制优化）公式，并引入包含100个组合优化问题的QUBOBench基准测试。


<details>
  <summary>Details</summary>
Motivation: 将自然语言问题描述转换为正确的QUBO公式非常困难，需要识别二进制变量、约束条件、目标函数、惩罚项和合适的惩罚权重，这一过程耗时且需要大量领域专业知识。

Method: 提出一个端到端的多智能体框架，可根据自然语言问题描述和结构化或非结构化测试用例自动生成QUBO公式。

Result: 在QUBOBench上达到68%的准确率，比直接单次调用基线方法提高22%。迭代自修复被识别为提高性能的最重要组件。

Conclusion: 该框架能有效自动化QUBO公式生成过程，降低对领域专业知识的需求，且迭代自修复机制对性能提升至关重要。数据和代码已开源。
...
### [83] [Untitled](https://arxiv.org/abs/2609.11018)
**
...
### [84] [Untitled](https://arxiv.org/abs/2609.10654)
**
...
### [85] [Untitled](https://arxiv.org/abs/2609.11493)
**
...
### [86] [Understanding LoRA Rank Trade-offs in Diffusion Model Fine-Tuning](https://arxiv.org/abs/2609.10656)
*Iman Khazrak,Narges Nejad,Mostafa M. Rezaee,Robert C. Green*

Main category: cs.AI

TL;DR: 本文研究了扩散模型微调中LoRA秩的选择问题，通过在CIFAR-10数据集上使用DDPM U-Net进行控制实验，比较不同秩{{2,4,8,16,32}}对模型质量和计算成本的影响。


<details>
  <summary>Details</summary>
Motivation: 在扩散模型的微调中，选择合适的LoRA秩需要在模型质量和计算成本之间进行权衡，但目前缺乏系统性的研究来指导这一选择。

Method: 在CIFAR-10上使用DDPM U-Net进行控制实验，测试ranks {{2,4,8,16,32}}，固定优化设置，使用本地文件夹的pytorch-fid协议进行评估。还包括扩展预算的DDPM运行（20 epochs，ranks 4/8/16）和Tiny DiT骨干网络（10 epochs，ranks 4/8/16）的验证实验。

Result: rank 4取得了最佳DDPM FID（124.1380），rank 8紧随其后（124.2136），更高的秩带来的提升有限，但增加了适应的计算成本。

Conclusion: 在固定训练预算下，小到中等秩的LoRA是最实用的选择，rank 4或rank 8可以作为实际的默认值。
...
### [87] [When Agents Disagree: Bayesian Backward Reasoning as a Label-Free Anchor for Multi-Agent Collective Decision-Making](https://arxiv.org/abs/2609.11709)
*Ken Chen,Wei Wang,Sachith Seneviratne,Hansani Weeratunge,Saman Halgamuge*

Main category: cs.AI

TL;DR: 本文研究多个LLM智能体产生冲突答案时的集体决策问题，提出通过贝叶斯反向推理构建反向后验概率，并结合Jensen-Shannon散度评估跨路径一致性，从而改进聚合策略。实验表明LogLin方法在DDXPlus数据集上表现最佳。


<details>
  <summary>Details</summary>
Motivation: 当多个LLM智能体给出不同答案时，现有的投票、选举规则和LLM评判方法都使用前向推理，这些方法虽然能结合多样的前向轨迹，但共享相同的证据到标签的分解方式，可能继承前向池中的相关性错误。因此需要探索不同分解方式的后验近似。

Method: 1)通过贝叶斯反向推理从显式似然函数为每个实例构建反向后验概率；2)使用Jensen-Shannon散度对智能体进行跨路径一致性排序；3)提出三种策略：硬选择(MinJS)、软重加权(FwdJS)和对数线性融合(LogLin)；4)在有标签数据时采用两阶段校准进一步优化。

Result: MinJS在所有后端上优于随机选择；FwdJS通常比最强基线有所改进；LogLin在评估方法中表现最佳，尤其在智能体意见不一致的子集上增益最大；反向后验作为锚点比纯前向方法提供更互补的信息。

Conclusion: 不同分解方式的后验估计倾向于较少共享相同错误，反向后验虽单独准确度较低，但作为锚点比纯前向替代方案更有价值，为集体决策提供了互补信息。两阶段校准可进一步提升反向锚点的效果和聚合性能。
...
### [88] [Quantifying the Memorization-to-Generalization Transition: Scaling Laws and Phase Structure in Grokking](https://arxiv.org/abs/2609.10657)
*Anish Kataria*

Main category: cs.AI

TL;DR: 研究者在384个双层MLP配置上绘制了记忆到泛化的边界，发现泛化 onset time 服从幂律缩放关系：T_grok ∝ H^-0.27 D^-2.04 η^-0.50 λ^-0.64。数据复杂度是主要驱动因素（双倍数据加速约4倍），而非模型容量（双倍宽度仅加速约1.2倍）。权重衰减 λ≥1.0 构成清晰的相边界。


<details>
  <summary>Details</summary>
Motivation: 尽管已有理论解释 grokking 现象为何发生，但关于该转变在超参数空间中何时发生的定量结构尚未被表征。

Method: 在模运算任务上对384个双层MLP配置进行实验，拟合泛化 onset time 的幂律缩放关系，并分析权重范数轨迹。

Result: 幂律关系为 T_grok ∝ H^-0.27 D^-2.04 η^-0.50 λ^-0.64（R²=0.732，加入交互项达0.821）。指数层级揭示数据复杂度（D^-2.04）是 regime transition 的主导驱动因素：双倍数据使泛化加速约4倍，而双倍宽度仅加速约1.2倍。权重衰减 λ≳1.0 构成sharp phase boundary，权重范数轨迹显示转变期间单调压缩。

Conclusion: 这些结果为预测和控制过参数化网络中的 regime transitions 提供了定量基础，表明数据复杂度而非模型容量是泛化延迟的主要决定因素。
...
### [89] [An Open Recipe for IMO Gold: Training Nemotron for Olympiad Mathematics](https://arxiv.org/abs/2609.10712)
*Ivan Moshkov,Stephen Ge,George Armstrong,Wei Du,Sadegh Mahdavi,Igor Gitman*

Main category: cs.AI

TL;DR: 本研究探讨模型后训练和测试时推理设计对自然语言证明生成的影响。基于Nemotron 3 Ultra，通过监督微调和强化学习训练了两个专业检查点，并提出了一种开放模型的测试时计算管道。该系统在IMO 2026中获得30/42分，达到金牌分数线。


<details>
  <summary>Details</summary>
Motivation: 解决IMO级别的奥数问题需要复杂的数学推理能力，而现有模型在自然语言证明生成方面仍面临挑战。本研究旨在探索后训练策略和测试时推理设计如何影响奥数问题的自然语言证明生成。

Method: 1.基于Nemotron 3 Ultra进行后训练：监督微调和强化学习；2.评估检查点选择、验证和优化策略；3.设计迭代搜索管道：生成-验证-优化候选证明；4.高计算阶段选择最终提交方案

Result: 在IMO 2026中获得30/42分，达到金牌分数线。系统完全使用自然语言运行，无形式化证明器、外部工具或互联网访问。

Conclusion: 通过结合监督微调和强化学习的后训练策略，以及迭代验证和优化的推理管道，可以显著提升大语言模型在奥数级别数学推理任务上的表现，达到IMO金牌水平。
...
### [90] [Towards a Deterministic Math Solver for Clinical Language Models](https://arxiv.org/abs/2609.10728)
...
TL;DR: 针对大语言模型在算术计算中不可靠的问题，提出了一种Program-Solve接口方案：模型不直接计算，而是生成病例特定的Python代码由受限执行器运行。在MedCalc-Bench Verified（1100例，55个计算器）上评估，发现该方法对7B模型提升有限（75.31% vs 72.02%），但对32B模型有显著提升（90.53% vs 83.47%）。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在算术计算方面不可靠，这对于临床计算器来说是个问题，因为单个数值错误会改变诊疗建议。传统做法是将每个计算器硬编码为经验证的功能，但这种方法费时费力。研究旨在测试一种替代方案：通过程序执行器而非模型直接计算来提高准确性。

Method: 模型生成病例特定的Python代码，由受限本地执行器作为确定性求解器运行，模型任务简化为决定如何使用该求解器。在MedCalc-Bench Verified（1100例，55个计算器）上评估，对比直接模型算术和手写22计算器库，使用Qwen2.5-7B和Qwen2.5-32B-AWQ。审计发现55个计算器中有16个存在版本、使用或系数问题。

Result: 当公式和黄金变量供给且两条路径都读取完整笔记时：7B模型使用求解器准确率75.31%，直接计算72.02%，配对提升3.29个点（95%计算器集群区间[-3.49, 10.38]）；32B模型使用求解器准确率90.53%，直接计算83.47%，配对提升7.05个点（95%区间[0.47, 14.60]），明显超过零。手写库在其支持的440个案例上精确，但其他案例弃权（总体40.0%）。

Conclusion: 即使在公式、变量和笔记访问匹配的情况下添加执行器，对某些开源权重模型的帮助也大于其他模型，且无论如何都不能替代经验证的公式或可靠的变量提取。执行器方法并非万能解决方案。
...
### [91] [Studying Without a Syllabus: Task-Agnostic Environment Preprocessing](https://arxiv.org/abs/2609.10824)
*Vinay Samuel,Varun Ursekar,Vijay S. Kalmath,Apaar Shanker,Veronica Chatrath,Yuan Xue*

Main category: cs.AI

TL;DR: 研究LLM智能体在无任务指导的情况下自主研究新环境并构建可复用资源的问题。智能体在测试前探索环境并生成制品（如索引、脚本或程序指导），供冻结的求解器使用。通过在六个异构基准测试上对比无辅助智能体与配备档案的元智能体，发现元智能体变体在5个基准上获得最高Avg@3奖励，但固定语料库处理在最大语料库基准上仍最优。


<details>
  <summary>Details</summary>
Motivation: 现有自动化适应方法依赖于任务示例、轨迹或评估反馈来决定构建什么，而任务无关方法虽避免这种监督，但会预先承诺特定类型环境的准备策略。作者研究更开放的设置：智能体能否在没有大纲的情况下（即测试前且不了解下游任务分布）研究不熟悉的环境并选择如何准备。

Method: 形式化了任务无关的环境预处理问题：学习系统在预算约束下探索环境，并为冻结的求解器生成制品。比较了无辅助智能体和配备档案的元智能体，以及固定合成实践和语料库处理方法，在六个异构基准测试上进行评估。

Result: 元智能体变体在五个基准上获得最高Avg@3奖励，而固定语料库处理在最大语料库基准上表现最佳。较大的研究预算并不能可靠地提高下游奖励。然而，研究产生的制品减少了达到给定分数所需的测试时采样，证明了可复用准备可以将计算从重复的测试时尝试转移到任务前的研究阶段。

Conclusion: 智能体可以在无任务监督的情况下自主研究环境并生成有用的制品。最佳准备策略因环境规模而异：元智能体适合中小规模环境，大规模语料库环境仍需固定的语料库处理方法。研究制品能有效降低测试时计算成本，为LLM智能体的环境适应提供了新的范式。
...
### [92] [When Validation Stops Learning: Auditing Update Admission for Continual Embodied Agents](https://arxiv.org/abs/2609.10873)
*Qinzhen Ma,Ruihai Wu*

Main category: cs.AI

TL;DR: 本文研究持续学习中策略更新的 admissibility 问题，提出需要同时考虑误差控制和保留学习机会，并证明范围置信度门无法在足够预算内认证旧任务行为不变，提出配对二项式构造方法来缓解此问题。


<details>
  <summary>Details</summary>
Motivation: 独立评估可能拒绝有害策略更新，但也会阻止有用的持续学习。作者认为更新 admissibility 需要通过误差控制和保留学习机会两个维度在给定交互预算下进行评估。

Method: 使用标准配对二项式构造，当结果分歧稀少时可减少负担；指定认证历史参考提升和轮次级错失机会指标；在32种子的一步推动诊断任务中进行实验。

Result: 配对检查在每阶段2000回合下可接受31.6%的常见更新流，而范围门接受率为0；无条件回放在闭环运行中学习效果更好。

Conclusion: 贡献是具有分析和合成证据的 admission-audit 协议，物理机器人和VLA验证仍有待后续工作。
...
### [93] [Untitled](https://arxiv.org/abs/2609.10964)
**
...
### [94] [Demystifying the Privacy-Utility Trade-off in LLM Interactions](https://arxiv.org/abs/2609.10992)
*Zhenhua Liu,Zhanxu Xie,Junjie Yu,Tong Zhu,Lijun Li,Wenliang Chen*

Main category: cs.AI

TL;DR: 本文提出了一种名为Veilmind-4B的意图驱动本地保护框架，通过动态提取-消毒-恢复流程，在保证低泄露隐私的同时显著提高响应效用，解决大型语言模型在实际应用中面临隐私保护与效用之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 当前隐私保护方法采用上下文无关的静态规则，导致严重的效用下降；且隐私清洗如何影响下游性能的具体机制尚未被充分探索。

Method: 通过系统性分析解构隐私-效用权衡，揭示三大机制：1)上下文依赖效用——根据用户意图判断数据价值；2)策略性适应——根据任务对事实完整性和结构连贯性的依赖程度选择移除或替换；3)组合交互——属性形成语义依赖网络。在此基础上引入意图驱动的本地保护框架，提炼轻量级模型Veilmind-4B驱动动态提取-消毒-恢复流程。

Result: 该方法在保持低泄露隐私的同时，显著优于现有隐私导向基线方法，实现了更高的响应效用。

Conclusion: 将隐私-效用权衡推向帕累托前沿，为LLM的隐私保护提供了新的解决思路。
...
TL;DR: 本文介绍了Agent Incident Registry (AIR)，一个包含AI智能体相关事件记录的来源链接目录，旨在填补公开失败案例与智能体安全评估之间的对比空白。记录包含支持证据、稳定标识符以及因果角色、披露类别、机制和结果等缺失感知标签。


<details>
  <summary>Details</summary>
Motivation: AI智能体日益通过工具和委托授权行动，但通用事件存储库很少捕获比较公共失败与智能体安全评估所需的机制。现有缺乏一个系统性的、来源可验证的智能体相关事件目录。

Method: 建立AIR目录，包含记录、支持证据、稳定标识符和缺失感知标签；由第二人类审核员检查所有记录和现有标签的完整性和正确性；进行部署类比审计比较InjecAgent案例与安全失败案例。

Result: 在智能体行动的生成系统记录中，Rprimary%涉及实际伤害；实际结果集中在野生部署和安全失败记录中；负责任披露和研究演示占主导；InjecAgent的三个案例占AIR十二个暴露面中的三个，且均为攻击者触发。

Conclusion: AIR支持来源可验证的案例检索和评估范围审计，但不适用于失败率或控制效能估计。实际结果集中于特定类型记录的特征表明，集合组成而非部署风险。
...
TL;DR: 本文提出环境探测 curation 方法，为异步 curator 代理提供只读世界工具来检查、更新候选记忆，无需模型重训练即可将记忆 curation 转变为环境知情、可审计的过程。在 GitHub Copilot 平台上，CLBench 任务通过率从 39% 提升至 73%，APEX 管理咨询任务中 18 项记忆对比均获正向收益。


<details>
  <summary>Details</summary>
Motivation: 生产环境中长期记忆代理会累积错误、过度泛化部分证据或保留过时知识。现有 post-task curator 代理仅限于已完成轨迹，无法验证和更新记忆内容。

Method: 环境探测 curation (Environment-Probing Curation): 为现有异步 curator 代理提供最小权限、只读的世界工具来检查、限定和刷新候选记忆。保持任务代理、检索器、记忆表示和生产写权限不变，无需模型重训练。

Result: CLBench: 通过率 39%→73%，奖励 8.60→22.60，单问题查询 8.8→4.7，代理成本 $3.38→$1.68。APEX 6 个世界 18 项记忆对比均为正向，工具调用减少 16-75%，5 个世界获最佳每美元奖励增益。在 Sonnet 4.6 和 Opus 4.7 上均优于 GHCP+Mem 且无模式漂移。

Conclusion: 环境探测将现有代理记忆 curation 转变为环境知情、可审计的过程，同时保持紧凑的任务时接口。为生产级长期记忆系统提供了实用的改进方案。
...
TL;DR: 本文提出了一种名为"信念转移分支"(belief-shift branching)的方法，用于优化树结构 rollout 中的分叉点 placement，以提高无 critic 强化学习(RLVR)的 step-level 信用分配效率。


<details>
  <summary>Details</summary>
Motivation: 在树结构 rollouts 中，每个分叉增加采样成本，但放置在结果已基本确定的位置会产生几乎无用的信用信号。因此，如何在给定树大小下确定最佳分叉位置成为关键问题。现有方法包括固定长度、中点和基于 next-token 熵的分叉，但缺乏系统性优化。

Method: 提出三种基于"信念转移"的实现：(1) 黑盒探针 (2) logit-lens 深度剖面 (3) 离线学习的激活方向。核心思想是在链的价值曲线"支点"处进行分叉，即模型答案信念发生最大连续分歧的前一步。

Result: 在验证中，信念转移信号在8个模型×基准面板中均排名第一，超越熵、结构化和 LLM-judge 基线。在 RL 训练中，OLMo-3-7B 数学任务提升 +2.6 聚合分数，AIME 2026 提升 +2.9，LiveCodeBench-medium 代码任务提升 +6.5。

Conclusion: 信念转移分支方法无需 step-level 监督，探针成本仅为 step 计算的约1%(数学)至5%(代码)，可显著提升 RLVR 在数学和代码任务上的性能，为大语言模型的强化学习训练提供了一种高效的分叉点定位策略。
...
TL;DR: Mosaic是一个无需训练的GraphRAG框架，将图谱检索表述为每个查询的控制器问题，通过LLM分析器将查询特定的证据需求转化为有界策略（种子选择、图遍历、停止和证据选择），在GraphRAG-Bench上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前GraphRAG系统使用统一的探索策略处理所有查询，存在结构不匹配问题：直接事实需要紧凑的局部邻域，比较需要平衡覆盖多个目标，而中介问题需要通过弱相关连接器进行更深的路径遍历。

Method: 提出Mosaic框架，包含：(1)LLM分析器将查询特定证据需求转化为有界策略，控制种子选择、图遍历、停止条件和证据选择；(2)共享的语料图、索引、评分函数、接地程序和答案生成器。无需针对特定基准进行检索器训练。

Result: 在GraphRAG-Bench上，医学领域答案正确率达76.97（+5.13），新颖领域达64.33（+4.43）；医学领域证据召回率95.1%，上下文相关性86.1%。与最强固定策略相比提升9.96分。相对Fixed Wide，路径评估减少81.9%，证据项保留减少47.2%。在HotpotQA、MuSiQue、2WikiMultiHopQA上展示可迁移性。

Conclusion: Mosaic通过为每个查询动态生成检索策略，有效解决了GraphRAG中固定策略与多样化查询需求之间的结构不匹配问题，且具有良好可迁移性，无需基准特定训练即可应用于新任务。
...
### [99] [Untitled](https://arxiv.org/abs/2609.11115)
**
...
TL;DR: KuaiRP是一套角色扮演模型的技术解决方案，通过多阶段训练pipeline实现四个核心目标：简化提示工程、稳定输出质量、内置领域知识、低部署成本。核心创新在于提出两阶段离线策略蒸馏(OPD)配合累积分歧衰减(CDD)的自蒸馏范式，有效平衡领域知识注入与通用能力保留。


<details>
  <summary>Details</summary>
Motivation: 角色扮演模型面临两难困境：深度领域知识注入往往导致模型通用智能能力的严重灾难性遗忘。现有方法难以同时实现深度角色扮演保真度和通用能力保留。

Method: 1)设计标准化角色模板，基于用户行为模拟和反向档案过滤构建SFT数据pipeline；2)在强化学习阶段使用基于规则的复合奖励函数消除长度膨胀和重复生成等问题；3)提出两阶段离线策略蒸馏(OPD)配合累积分歧衰减(CDD)的自蒸馏范式，以领域适配模型为教师、原始基础模型为学生进行知识迁移。

Result: KuaiRP模型在目标领域的角色扮演保真度方面达到当前最先进的专有模型水平，同时成功恢复通用智能体能力，并保持极低的部署成本。

Conclusion: 通过多阶段训练和创新的自蒸馏方法，KuaiRP成功实现了深度领域知识注入与通用能力保留的平衡，为高效部署的角色扮演模型提供了完整技术方案。
...
TL;DR: 该研究质疑金融情感分析工具验证流程的传统假设，通过证券集体诉讼数据测试发现：人工标注一致性建立的语义有效性并不能自动保证工具的预测能力，采样方法和分数表示方式会影响两者关系。


<details>
  <summary>Details</summary>
Motivation: 验证金融情感分析工具的传统假设：即人工标注验证（语义有效性）与预测市场收益的能力（预测有效性）衡量的是同一构念。作者质疑这一假设是否成立。

Method: 使用2002-2025年证券集体诉讼数据集，包含70,500条X(原Twitter)消息与异常股票收益的关联。通过单一标注员构建人工标注金标准，将五种工具（VADER、Loughran-McDonald、FinBERT、Twitter-RoBERTa、LLM注解器）置于同一Pipeline中测试。采用两种采样策略：传统方法特定采样与固定N面板。

Result: 1.语义有效性与预测有效性的关系取决于采样约定和分数表示；2.传统方法特定采样下，人工一致性更接近同一天分级关联而非一日领先；3.固定N面板上，一致性在两个时间窗口的分级排序相关性相似，但粗略排序仍然较弱；4.基准一致性确立了语义有效性但不决定预测排名；5.当消息中17.6%为垃圾信息时，消息量既不能预测市场损害也不能预测和解规模。

Conclusion: 金融NLP的标准工作流程存在局限性：人工标注验证仅能建立语义有效性，无法单独决定工具的预测能力排序。未来研究需明确区分语义有效性与预测有效性，并考虑垃圾信息的影响。
...
TL;DR: 研究了在AI生成文本回流训练数据的场景下，市场集中度（寡头垄断）是否会加速模型崩溃或导致模型被主导者牵引。实验使用13个开源1-4B模型构建3-13个玩家的生态系统，模拟5代训练。结果表明：集中度对崩溃速度和方向影响甚微，真正决定崩溃速度的是哪些来源的文本填充了训练池。


<details>
  <summary>Details</summary>
Motivation: 现实生成式AI是寡头垄断格局，但现有研究多假设市场均分。作者担心：1）更少且更统一的信息源可能加速模型崩溃；2）后来模型可能被寡头输出牵引。需要验证这两个担忧。

Method: 控制性生态系统实验：13个开源1-4B模型组成3-13个玩家的自然生态系统，注入探针模型将顶部份额推至90%；每代按市场份额将各模型输出混合成共享池，从干净基权重重新训练各模型，共5代。

Result: 在测试范围内，两个担忧均未出现。增加不平等份额几乎不改变崩溃速度；目的地（模型输出方向）变化也更小——5代终点仅移动几个百分点。极端份额搭配最强注入偏置也无法保证导向成功。即使最极端设置，主题偏移在测量崩溃的标尺上仅留下微弱痕迹。

Conclusion: 在测试范围内，集中度既不决定崩溃目的地也不决定崩溃速度。真正决定崩溃速度的是谁供给文本池：固定份额下更换K=3生态系统成员，5代漂移变化2.8倍；份额加权成员易感性指数可解释19个分支的速度差异（R²=0.68）；用人类文本替代一半池约减半漂移但不改变方向。
...
TL;DR: ARCHE是一个自主智能体系统，将通用推理模型、领域专用计算化学模型和结构化工具注册表集成，用于自动化反应机制研究。该系统在三个场景中验证了其能力：立体控制过渡态重建、自由基途径验证以及选择性描述符识别，代表了化学研究自动化的重要进展。


<details>
  <summary>Details</summary>
Motivation: 反应机制研究是现代化学的核心，但自动化这些研究仍具有挑战性，因为计算工作流仍然严重依赖专家干预。这限制了化学研究的可扩展性和效率。

Method: ARCHE系统整合了三个核心组件：通用推理模型、领域专用计算化学模型和结构化工具注册表。该系统采用闭环架构，能够解释科学问题、生成和优先排序机制假设、编排计算工作流，并根据计算证据迭代优化结论。

Result: ARCHE在三个难度递增的场景中得到验证：1）重建不对称催化反应中的立体控制过渡态并验证反应机制；2）为新发现的α-碘硼烷C-I裂解反应提出并验证合理的自由基途径；3）识别镍催化迁移交叉偶联反应中选择性的化学可解释描述符。结果表明该系统能够自主完成复杂的机制发现任务。

Conclusion: ARCHE通过将智能体推理与严格计算验证相结合，推进了自主机制发现，并为更广泛的机器辅助化学研究奠定了基础。该系统代表了化学研究自动化的重要进展，有潜力改变科学家探索反应机制的方式。
...
TL;DR: 提出了一种名为层次动态角色图多智能体近端策略优化(DRG-MAPPO)的新型MARL框架,用于空中战斗协同决策。该框架结合了基于图的关系建模与动态角色分配,在空战中实现了87%的胜率。


<details>
  <summary>Details</summary>
Motivation: MARL在空战中的应用面临两大挑战:1)缺乏结构化的关系建模,无法捕捉战场实体之间复杂的时间变化交互;2)传统扁平架构缺乏显式建模战术角色的能力,导致动态环境中的任务分配模糊。

Method: 构建战场交互的图表示,利用图注意力机制提取盟友、敌人和威胁之间的关键关系特征;高级策略使用动态角色分配机制确定战术职责(如"领导者"和"支援者");低级策略基于角色和图关系特征执行离散机动动作;设计目标优先级辅助任务以促进集火行为。

Result: DRG-MAPPO实现了87%的最先进胜率,有效平衡了关系建模、可解释性和协作空中战斗的优化稳定性。

Conclusion: 该框架通过整合图关系建模与动态角色分配,有效解决了MARL在协同空战中的关系建模和任务分配问题,为自主空战系统提供了具有可解释性的层次化决策方法。
...
TL;DR: 提出了一种指定反事实(Specified-Foil Counterfactual)问题，用于在时序图上寻找使特定替代结果成为预测首选项的过去事件干预方法。该方法通过对比已完成预测A的执行轨迹与重建的预测B的不完整执行轨迹，映射出DELETE、INSERT、REWIRE、RELABEL和SHIFT操作，并在时序动态图和时序知识图上验证。


<details>
  <summary>Details</summary>
Motivation: 传统时序图反事实解释只改变过去事件以改变或否定原始预测，但未指定替代预测。用户往往想知道哪些过去条件会使特定的替代结果发生，而不是仅仅想知道如何改变当前预测。

Method: 提出指定箔反事实问题：给定原始预测A和预先固定的箔B，找到低成本干预使同一预测器选择B为首选。方法包括：对比A的完整执行轨迹与B的重建不完整执行轨迹，将差异映射为DELETE、INSERT、REWIRE、RELABEL和SHIFT操作，通过精确重放验证B。在CTDG上实例化为LiTVER，在TKG上实例化为TLogic。

Result: 在CTDG上，方法保留了85.7-93.6%的黑盒贪心成功同时减少75.0-80.0%的预测器评估；在TKG上，600次比较中74.8%达到指定箔。

Conclusion: 可执行轨迹成为构建未选择替代条件而非仅解释已做预测的计算结构。
...
TL;DR: 本文提出能力边界过程监督(Capability-Bound Process Supervision)来解决工业查询-智能体匹配中的核心问题:将语义相关性误判为可执行能力。通过 Debate-to-Skill 方法,利用可复用决策原则、结构化推理、验证器判决提取和分歧驱动优化,在 Query2Agent 基准上验证了方法有效性,尤其在语义相关性 与可执行能力分离的灰色地带场景。


<details>
  <summary>Details</summary>
Motivation: 工业查询-智能体匹配中,现有方法将语义相关性(capability)与可执行能力(executable capability)混淆,导致在长尾请求和边界敏感请求上匹配失败。核心问题在于:语义上相关的智能体不一定真正具备执行该任务的能力,而现有监督方式未能捕捉这一关键差异。

Method: 提出能力边界过程监督框架,具体实现为 Debate-to-Skill 方法:
1. 可复用决策原则(Reusable Decision Principles):建立跨场景的决策规则
2. 结构化 deliberation:让模型进行显式推理
3. 验证器判决提取(Verifier-based Verdict Extraction):利用验证器判断能力边界
4. 分歧驱动 refinement:通过辩论识别并解决不一致

Result: 在 Query2Agent 工业基准上的实验表明:
- Debate-to-Skill 优于直接标签监督、推理-SFT 和结构消融实验
- 增益主要来自对能力关键决策过程的监督
- 在语义相关性与可执行能力分离的灰色地带案例上效果显著

Conclusion: 监督能力关键决策过程本身比仅监督最终标签更有效。Debate-to-Skill 通过显式建模能力边界判断,有效解决了工业查询-智能体匹配中的核心问题,为长尾和边界场景提供了新思路。
...
TL;DR: 首次对LLM版本约束语义理解能力的系统评估，发现现有模型存在可预测的机制性盲点，建议编码代理将版本解析任务委托给专用解析器而非自行推理。


<details>
  <summary>Details</summary>
Motivation: LLM编码代理经常需要判断版本号是否满足约束条件（如^1.2.3或>=2.0,<3），但其对版本约束语义的理解程度从未被直接测量过。

Method: 构建了SemVerBench基准测试，涵盖3个生态系统（npm、PEP 440、Cargo）共240个机器可验证的条目，由官方测试套件和前沿LLM建议者平衡构建，采用双实现预言机进行标注。评估了6个前沿模型。

Result: 所有模型在Cargo上因部分比较器携带规则（>1.2意味着>=1.3.0）均出现约60%的失败；GPT-5.1在PEP 440零填充/后发布角 case上完全失败（0/26），而Claude保持在97-100%。Opus显著优于其他模型，Sonnet优于OpenAI模型。失败更像是激活/应用差距而非知识差距——注入规则或轻微正确提示可恢复大多数错误。

Conclusion: 由于任务可验证且存在免费、100%正确的解析器，工具委托可达~100%准确率。编码代理应将版本解析委托给解析器而非自行推理。实验未发现作者自我偏好的统计显著证据。
...
TL;DR: LogiMed-RoB是一个基于Cochrane RoB 2.0专家逻辑的基准测试，用于评估大语言模型在医学证据推理中的逻辑一致性。研究发现即使最先进模型的原子一致性高达98.88%，端到端一致性仍暴跌至45.13%，揭示了严重的错误累积效应和证据-推理差距。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估过度依赖表面标签匹配，忽视真正的推理能力。医学领域需要严格逻辑一致性的循证实践，但现有评估方法无法有效检测模型在复杂推理任务中的缺陷。

Method: 构建包含860个随机对照试验和14,820个查询的LogiMed-RoB基准测试，采用层级逻辑一致性(HLC)框架从四个维度评估：原子一致性、领域一致性、聚合一致性、证据忠实性。

Result: 顶级模型原子一致性达98.88%，但端到端一致性暴跌至45.13%。部分开源架构几乎降至0%。模型在检索到高质量证据时，仍有18.63-40.05%无法推导出正确结论。盲目猜测率达48.28%。

Conclusion: 高准确率可能掩盖关键推理缺陷，临床部署需采用白盒逻辑验证而非仅依赖黑盒准确率指标。现有LLM在医学推理任务中存在系统性逻辑缺陷。
...
TL;DR: 提出了一个多智能体AI系统，用于在LLM介导的电子商务中自动测量竞争可见性并诊断根本原因。该系统引入Agentic Share-of-Search (ASoS)作为决策目标，部署查询智能体跨平台收集数据，并使用基于ReAct的诊断智能体推荐优先的营销干预措施。


<details>
  <summary>Details</summary>
Motivation: AI购物助手越来越多地重定向消费者发现过程，迫切需要支持卖方侧竞争决策的工具

Method: 系统由三部分组成：1) ASoS作为决策目标；2) 跨领先AI平台部署查询智能体；3) 基于ReAct的诊断智能体进行根因诊断和干预推荐。进行100次试验的消融研究作为可行性评估

Result: 智能体在39%的试验中恢复了消融信号（95%CI: 30.0%-48.8%，是随机 chance的5.5倍），在高相关性消融中上升到63.9%

Conclusion: 该原型系统显示出较好的可行性，为LLM介导的电子商务竞争分析提供了新的研究方向
...
TL;DR: 本文针对巴基斯坦大学生开发了一款AI驱动的文化敏感型压力检测与心理健康支持系统，使用Random Forest模型在1100份学生压力数据上达到89.09%的准确率，通过OpenRouter API接入开源大语言模型提供多语言心理健康对话服务。


<details>
  <summary>Details</summary>
Motivation: 现有数字心理健康工具专为西方环境设计，巴基斯坦大学生面临学术、经济、家庭和关系等多重压力源，这些压力严重影响学生的学业和心理发展，需要开发适合当地文化背景的心理健康系统。

Method: 采用Random Forest机器学习模型，基于20个特征（涵盖心理、生理、学业、环境和社会方面）的1100份验证学生压力数据集进行训练；分类结果通过OpenRouter API传输至开源大语言模型，结合文化敏感的系统提示提供英语、乌尔都语和罗马乌尔都语的心理健康对话。

Result: 模型达到89.09%的准确率和0.89的宏F1分数，可识别三个压力严重程度等级。特征重要性分析显示师生关系是第二重要的预测因素。

Conclusion: 师生关系作为重要的文化特有压力因素，凸显了区域认知心理健康系统的必要性。未来将使用DASS-21工具从大一新生等心理脆弱群体收集更多数据。
...
TL;DR: CryptoL是一个统一的多变量时间序列预测框架，专门用于解决加密货币预测中的极端跨资产规模异质性、非平稳动态和OHLC变量结构依赖性问题。


<details>
  <summary>Details</summary>
Motivation: 加密货币预测面临独特挑战：1) 极端的跨资产规模异质性（不同资产价格跨度数个数量级）；2) 非平稳动态；3) OHLC（开盘、最高、最低、收盘）变量间的结构依赖关系。现有方法难以有效处理这些问题。

Method: 提出CryptoL框架，包含四个核心技术创新：1) RevIN管道中的上下文归一化坐标评估预测误差，防止逆归一化在MSE目标中引入额外的平方尺度权重；2) 共享通道依赖仿射变换以保持蜡烛图顺序关系；3) 自适应数值稳定化减少固定归一化常数造成的失真；4) 软可行性损失惩罚OHLC不等式违反。

Result: 在异质加密货币资产上的实验表明：预测精度提升、训练稳定性改善、金融有效的OHLC预测频率增加。

Conclusion: CryptoL提供了scale平衡优化、结构保持归一化、数值稳定化和约束感知加密货币预测的集成方法，为多变量加密货币时间序列预测建立了新框架。
...
TL;DR: SurgicalRoomAgent是一个基于大语言模型的语音交互多智能体系统，专为智能手术室设计。该系统通过分层架构实现自然语言理解、设备控制、术中记录和手术报告生成，采用Qwen3-27B模型作为核心，在16,384个token的上下文限制内有效运行，满足手术室实时控制响应要求。


<details>
  <summary>Details</summary>
Motivation: 智能手术室需要实现自然语言理解、设备控制、术中记录和手术报告生成的综合自动化。当前挑战包括：低延迟推理需求、术中端到端延迟优化、以及在有限上下文窗口内最大化信息密度的需求。

Method: 采用分层架构，包含语音交互管道（唤醒、ASR、转向检测、智能体推理、TTS）和智能体核心（技能注册、任务规划器、设备管理器）。研究了三项关键技术：(1) KV Cache前缀预热技术，通过字节级最长公共前缀重用，将重新计算开销从约500ms降至数十毫秒；(2) 流式部分JSON解析结合早期并行任务执行，端到端延迟降低约30%；(3) 渐进式技能提示披露，根据用户角色、连接设备和手术阶段动态过滤系统提示。使用Qwen3-27B模型和llama.cpp/sglang推理引擎实现。

Result: 实验表明系统在16,384个token的上下文限制内有效运行，多设备并行控制响应时间满足手术室实时要求。

Conclusion: SurgicalRoomAgent系统成功实现了智能手术室的语音交互控制，通过创新的缓存优化、并行执行和动态提示技术，有效解决了低延迟推理和上下文管理的挑战，为智慧手术室提供了可行的LLM应用解决方案。
...
TL;DR: NovGauge是一个用于细粒度新颖性评估诊断的基准测试，旨在评估大型语言模型在AI会议同行评审中识别论文创新点的能力。该基准包含619对论文和50个多论文集，从ICLR审稿人重叠声明和调查共引两个专家来源获取，标注了任务、问题和方法三个维度。评估18个LLM显示 hallucination率0-39%，超过70%的正确判断存在证据逻辑支持问题，最佳模型GPT-5.5仅达43-72% Verified F1。


<details>
  <summary>Details</summary>
Motivation: 当前LLM被广泛应用于AI会议的同行评审，但新颖性评估仍是持续性弱点。现有基准将新颖性作为单一整体分数评估，难以诊断模型在哪个维度判断错误或证据是否忠实。因此需要构建一个细粒度的诊断基准。

Method: 构建了NovGauge基准，包含619对论文和50个多论文集，来源于ICLR审稿人重叠声明和调查共引两个专家来源。论文按三个维度独立标注：任务（应用目标）、问题（技术挑战）和方法（解决方案）。提出级联诊断管道，验证每维度正确性、证据基础和逻辑支持。对18个LLM进行评估。

Result: Hallucination率范围0-39%；在非Hallucination的正确阳性判断中，超过70%的引用证据无法逻辑支持所陈述的原因。最佳模型GPT-5.5在三个维度上达到43-72%的Verified F1；大多数模型的原始F1在忠实性验证后损失超过一半。

Conclusion: 当前LLM在可靠的科学新颖性评估方面仍有较大差距，特别是在正确性依赖于忠实证据基础的条件下的表现不佳。需要在证据 grounding和逻辑支持方面进行显著改进。
...
### [114] [Sci-MMR: Benchmarking Multi-Step Evidence-Grounded Scientific Reasoning in Multimodal Agents](https://arxiv.org/abs/2609.11243)
...
TL;DR: Sci-MMR基准测试评估多模态模型的证据支撑推理能力，发现答案准确率比完整证据恢复率高出20%以上，揭示了当前答案中心评估方法的结构性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅评估最终答案准确性，未验证预测是否由可追踪的科学证据支持，无法判断模型是否真正基于证据进行推理。

Method: 构建Sci-MMR基准测试，包含235个跨4个科学领域的多跳推理任务，平均每个任务9个图表面板，基于结构化论证图链接科学主张、引用知识、视觉证据和支持区域。评估8个前沿多模态模型并进行受控干预实验。

Result: 答案准确率持续超过完整证据恢复率20%以上。证据获取失败占57.2%（模型难以从科学图表中提取完整结构化证据），裁剪工具仅提升4.5分，提供金标准证据可提升37.0分。证据整合失败占31.8%，即使提供金标准证据，最强模型在最难任务上仅达69.1%准确率。

Conclusion: 当前答案中心基准测试严重高估了多模态研究代理的证据支撑推理能力，需要更注重证据推理过程的评估。
...
### [115] [AI-Powered Flare Combustion Efficiency Estimation](https://arxiv.org/abs/2609.11262)
...
TL;DR: 提出了一种基于轻量级视觉语言编码器和紧凑多层感知器的低成本解决方案，从热视频预测燃烧效率，实现99%正常运行时间。


<details>
  <summary>Details</summary>
Motivation: 传统气体分析仪和高光谱相机成本高、易碎且需频繁校准，难以在偏远或预算有限的工业场地使用。

Method: 结合轻量级视觉语言编码器与紧凑多层感知器，直接从低成本热视频 footage 预测燃烧效率，并集成到易于部署的GUI中。

Result: 系统运行6个月期间达到99%正常运行时间，每周维护时间少于15分钟。

Conclusion: 基于热视频的燃烧效率预测方法可行，能以较低成本实现持续监测，为偏远工业场地提供实用解决方案。
...
### [116] [Predicting Train Delays in Finland Using Machine Learning and Weather Data](https://arxiv.org/abs/2609.11277)
*Vinicius Pozzobon Borin,Jean Michel de Souza Sant'Ana,Nurul Huda Mahmood*

Main category: cs.AI

TL;DR: 研究利用芬兰铁路-天气数据集(FI-TW)结合芬兰气象局约200个气象站的传感器数据,使用XGBoost算法预测火车延误。实验表明,基于天气类别(如暴风雪、大雪、极端寒冷)的特征工程方法优于原始气象观测,R²达到0.78,RMSE为8.5分钟,MAE为3.7分钟,相比其他配置R²提升11%,误差降低10%。


<details>
  <summary>Details</summary>
Motivation: 北极地区恶劣天气是导致火车延误的主要原因,现有预测方法未充分利用领域知识的特征工程。6G网络下的无线传感器基础设施为实时环境感知提供了技术支撑,但如何在边缘设备上实现高效的延迟预测仍是挑战。

Method: 使用XGBoost算法在Oulu中央车站(101,146条观测记录)上评估三种特征配置:1)完整天气特征;2)即时天气观测;3)派生天气类别场景。数据集整合了铁路运营记录与芬兰气象局传感器网络数据。

Result: 基于分层天气分类(暴风雪、大雪、极端寒冷)的类别方法表现最佳,R²=0.78,RMSE=8.5分钟,MAE=3.7分钟,相比其他配置R²提升11%,误差降低10%。

Conclusion: 紧凑的领域知识特征优于原始气象观测,具有更高的带宽效率,适合在当前及未来无线基础设施上进行边缘部署,为6G网络的集成感知与边缘计算提供了应用前景。
...
### [117] [Bio-inspired Learning and Decision-Making with Probabilistic In-Memory Computing Hardware: Part 1](https://arxiv.org/abs/2609.11281)
*Thomas Dalgaty,Eiji Kawasaki,Miguel de Prado,Devendra Vyas,Tommaso Salvatori*

Main category: cs.AI

TL;DR: 该论文提出了一种基于生物学的框架，将神经和突触的噪声动力学视为执行推理和学习的随机采样过程，通过将不确定性映射到神经活动变异性中，实现类似贝叶斯推断的能力，并探讨了模拟存算一体化硬件在实现高效概率推理方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 动物学习和决策常被建模为贝叶斯过程，但神经系统中实现这种能力的内在神经动力学机制尚不清晰，且如何在计算系统中复制这种能力仍是挑战。此外，传统数字计算在实现大规模概率推理时面临能效和可扩展性的瓶颈。

Method: 提出一种生物学启发的框架，利用噪声神经和突触动力学进行推断和学习，通过随机采样从内部能量函数中获取样本，将潜在状态的不确定性映射到神经活动变异性，将模型参数不确定性映射到突触可塑性。该框架可使预测编码网络通过马尔可夫链蒙特卡洛采样来解释认知不确定性。此外，借鉴生物系统内在噪声与新兴概率模拟存储技术中电噪声的类比，探索模拟存算一体化硬件实现概率推理的可行性。

Result: 论文建立了神经噪声与模拟硬件电噪声之间的等价关系，论证了模拟存算一体化硬件是实现大规模、低能耗概率推理的理想平台。

Conclusion: 通过将生物神经系统的随机动力学与新兴模拟硬件相结合，该研究为构建高效、可扩展的类脑智能系统提供了新思路，并指出了模拟存算一体化在概率计算领域的应用前景。
...
### [118] [When Does Text Inform? Benchmarking Information-Theoretic Metrics for Multimodal Time-Series Forecasting](https://arxiv.org/abs/2609.11282)
*Emma Andrews,Gianmarco Mengaldo*

Main category: cs.AI

TL;DR: 本文创建了一个合成基准数据集，用于评估信息论指标（KSG、MINE、InfoNCE、CCA、PID和V-information）在衡量文本注释对时间序列预测贡献方面的有效性。通过合成数据验证了六种估计器的性能，并在七个真实数据集上验证了其局限性。


<details>
  <summary>Details</summary>
Motivation: 多模态预测模型结合时间序列和文本注释，但缺乏评估文本注释预测价值的基准和可靠的信息论度量方法。

Method: 创建合成时间序列信号及三类注释（语义正确、不相关、错误），使用受控数据生成过程确定ground-truth信息内容，评估六种互补的互信息估计器。

Result: 六种估计器都能正确识别最有信息量的注释，无需模型训练即可审核混合语料库质量并选择最佳注释。但各估计器在弱信号上表现不同，存在各自局限。

Conclusion: 建立了信息论指标用于注释审核和融合选择的具体实施规则，为多模态预测中文本注释的价值评估提供了可靠方法。
...
### [119] [Generating a Consistent Enterprise: Synthesis and Reference-Free Evaluation of Multi-System Business Data](https://arxiv.org/abs/2609.11286)
*Benjamin Gruenbaum,Doron Porat,Assaf Natanzon,Roy Zavida,Chen Dinachi,Or Itzahary,Omer Niv*

Main category: cs.AI

TL;DR: 本文描述了一个无需真实数据集即可生成完整虚构企业的合成数据生成器，可生成包含员工、客户、销售交易、支持工单、录音、聊天记录和文档的企业数据，并投影到66个商业产品的原生格式中。


<details>
  <summary>Details</summary>
Motivation: 解决合成数据生成中依赖真实数据集的问题，无需真实数据即可生成高质量、多产品兼容的企业级关系数据。

Method: 1)基于行业、公司规模、业务模型、业务应用和随机种子生成企业数据；2)使用引用统计数据构建真实感；3)采用五轴28项统计检查、无标记检测器和分类器测试进行无参考验证；4)第二个生成器从业务问题列表构建关系数据库。

Result: 经过23家生成公司的测试，平均真实感从60.1提升至99.1，最弱公司从41.1提升至94.9，检测器标记率从55.2%降至0%。

Conclusion: 成功实现了无需真实数据集的高质量合成关系数据生成，验证了方法的有效性，并提供托管服务和容器镜像供离线使用。
...
### [120] [Off-Target Effects of Response-Style Alignment in a Korean 27B Language Model](https://arxiv.org/abs/2609.11291)
*Hyojung Han*

Main category: cs.AI

TL;DR: 本研究对Qwen3.8-27B进行韩语响应风格的后训练，测量其在模糊社交问题（KoBBQ）和证券指导中的表现变化，发现行为变化主要通过模型的发射策略（回答频率和输出长度）体现，而非内容偏好。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索后训练目标如何影响模型在非目标行为上的表现，特别是回答倾向性和内容组合的变化

Method: 通过固定提示词、配方、数据量和服务参数，仅改变目标文本，比较不同风格种子的效果，并进行长度匹配对照实验

Result: 积极风格种子使回答率平均增加0.82个百分点，中性风格种子减少1.53个百分点；变化主要由回答倾向性驱动，组合项保持较小；规则检测器一致性在0.44到0.99之间变化

Conclusion: 在回答状态依赖治疗的情况下，条件刻板印象份额的对比无法识别内容偏好变化；模型行为变化主要体现在发射策略层面而非深层偏好
...
### [121] [Memory Compression for High-Fanout Agent Sandboxes](https://arxiv.org/abs/2609.11294)
*Mengming Li,Ceyu XU,Qijun Zhang,Jiangnan Yu,Xiangfeng Sun,Haohui Mai,Zhiyao Xie*

Main category: cs.AI

TL;DR: AgentZip是首个专为AI-agent沙箱设计的内存压缩系统，通过利用模板级和跨沙箱的内存冗余性来应对高扇出agent工作负载带来的内存瓶颈问题。相比Linux配置（2.1x），AgentZip可将沙箱内存降低最多8.7倍，并通过恢复预取和agent执行感知调度将激进压缩的性能开销从3.1倍降至1.40倍。


<details>
  <summary>Details</summary>
Motivation: 高扇出agent工作负载造成严重的内存瓶颈，因为单个任务可能产生许多并发沙箱会话。这些沙箱虽然来自共享模板并执行相关轨迹，但传统内存压缩在三个维度上无法有效处理：如何压缩（无法利用非相同沙箱页的相似性）、压缩什么（通过保守页面选择控制页面错误开销）、何时压缩（由内存压力触发或与agent执行阶段无关）。

Method: AgentZip引入三种创新机制：1）利用模板级和跨沙箱冗余的压缩机制；2）将压缩范围扩展到任何具有可盈利表示的页面，并将开销控制从压缩时页面选择转移到恢复时预取；3）将昂贵压缩与LLM等待时期对齐以避免干扰前台工具执行。

Result: 在LLM训练和推理工作负载上，AgentZip相比Linux配置（2.1x）可将沙箱内存降低最多8.7x。恢复预取和agent执行感知调度将激进压缩的 slowdown从3.1x降至1.40x，同时保留几乎所有内存节省效益。

Conclusion: AgentZip是首个专门针对AI-agent沙箱的内存压缩系统，成功解决了高扇出agent工作负载的内存瓶颈问题，在大幅降低内存占用的同时将性能开销控制在可接受范围内。
...
### [122] [Routing by Reasoning Need: Trajectory-Aware Decoding Control for Diffusion Vision-Language Models](https://arxiv.org/abs/2609.11315)
*Yixiang Liu,Zhongxing Xu,Zhonghua Wang,Xiaoying Tang*

Main category: cs.AI

TL;DR: 本文研究扩散视觉语言模型解码过程中的推理预算不匹配问题，提出了一种无需训练的控制方法，根据答案轨迹信号动态选择不同的解码策略（提前承诺、基线保留或推理支持解码），在多个基准测试上提升了鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 扩散视觉语言模型通过迭代细化生成答案，但采用统一的生成长度处理不同推理需求的问答，导致推理预算不匹配：视觉封闭问题在形成稳定答案后继续细化可能有害，而推理敏感问题过早承诺也可能有害。

Method: 提出无需训练的控制器，利用答案闭合、承诺证据和表征修正压力等轨迹信号，将每个样本路由到提前承诺、基线保留或推理支持解码三种策略，无需使用真实答案。

Result: 在答案聚焦、混合推理和CoT敏感基准测试中，路由控制相比固定长解码、短解码和单规则干预均提升了鲁棒性。答案闭合示例受益于承诺，而CoT敏感示例需要保留或支持中间推理。

Conclusion: 扩散VLM解码应根据观察到的轨迹状态动态路由推理时控制，而非依赖统一解码长度。
...
### [123] [Mr.LHDR: A Benchmark for Multimodal Real-World Long-Horizon Deep Research Agents](https://arxiv.org/abs/2609.11318)
...
TL;DR: Mr.LHDR是一个用于评估真实世界长周期深度研究的新基准，测试多模态证据分析和跨8个类别的依赖性证据链整合能力。每个问题平均需要12.1个必要中间结论和10.4的依赖深度才能得出简短、可验证的答案。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估中等 horizon 的探索能力，很少测试代理是否能维持长周期、依赖性强的研究过程。需要一个新的基准来评估真实世界深度研究。

Method: 构建基于隐藏节点-关系图的问答系统，包含图像、地图、PDF、标志、图表、表格和视频帧等多模态证据，至少一个非文本元素改变推理状态。同时评估最终答案和中间结论的正确性。

Result: 最强系统仅达到43.1%的总体准确率和34.3%的严格准确率。最终答案准确率显著高估了完整研究的成功。移除图像使依赖感知清单得分下降12.6分。SA随推理链增长持续下降。

Conclusion: 当前深度研究代理的关键瓶颈是持续的、依赖一致的证据整合能力，而非孤立的的事实检索。多模态证据在研究中起着重要作用。
...
### [124] [Magenta: Closing the Loop Between Mathematical Reasoning and Lean Verification](https://arxiv.org/abs/2609.11319)
*Joshua Ong Jun Leang,Haonan Li,Zheng Zhao,Xinyi Shang,Wenda Li,Zhengzhong Liu,Erix Xing,Shay Cohen,Eleonora Giunchiglia*

Main category: cs.AI

TL;DR: 本文提出Magenta系统，将自然语言数学问题转化为Lean 4形式化证明，实现机器可验证的数学推理。在AIME 2025/2026和HMMT February 2026上达到100%准确率，配合K2-Horizon-7B解决器可解IMO 2026全部6道题。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在非形式化数学推理上表现优异但仍有局限，未能利用机器可验证证明的能力。研究者希望通过整合Lean证明助手，将自然语言问题转化为形式化证明，弥合非形式化与形式化推理之间的鸿沟。

Method: Magenta是一个免训练的智能体流程，包含：1)将自然语言问题转化为Lean 4陈述；2)构建机器可验证的证明；3)statement judge验证形式化是否保持原问题语义；4)error-attribution judge将失败尝试路由至数学重新推导或局部Lean修复。

Result: 在所有评估的奥赛基准上达到100%准确率，包括AIME 2025、AIME 2026和HMMT February 2026。使用开源K2-Horizon-7B推理器时解决了全部6道IMO 2026问题。

Conclusion: 研究表明：1)陈述判定对防止虚假证明证书至关重要；2)反馈引导的修正优于对困难问题的独立重采样。该工作证明了结合形式化验证与LLM推理的有效性。
...
### [125] [AI Exposure and AI Resilience: A Two-Dimensional Assessment Framework for Software and Software-Based Business Model](https://arxiv.org/abs/2609.11321)
*Paul Darius Mandl,Peter Mandl,Martin Häusl*

Main category: cs.AI

TL;DR: 本文提出了人工智能暴露与弹性（AI-ER）双维度评估框架，用于评估AI如何影响公司价值主张、竞争地位、利润和客户获取等方面。AI暴露衡量AI对商业模式带来的变革压力，AI弹性衡量公司吸收压力、适应变化并以经济可行方式使用AI的能力。


<details>
  <summary>Details</summary>
Motivation: 传统技术尽职调查主要考察架构、可扩展性和技术债务等技术属性，这些标准无法完全捕捉AI如何影响公司价值主张、竞争地位、利润率或客户获取能力，因此需要新的评估框架。

Method: 从当前AI能力、部署条件以及商业模式和组织适应性相关研究中推导暴露和弹性两个维度的指标，将暴露和弹性分开评估，并添加证据质量和置信度的明确评估，可先使用公开信息后用内部证据细化。

Result: 形成一个可追踪的公司档案，支持比较同时不掩盖底层证据的不确定性，并指定了初步评分逻辑和实证验证程序。

Conclusion: AI-ER框架能够评估AI对商业经济的影响，为技术尽职调查提供补充，支持公司在AI时代进行战略决策和风险管理。
...
### [126] [Exploring Diffusion Transformers for Cross-Modal Augmentation in Multimodal Brain State Decoding](https://arxiv.org/abs/2609.11341)
*Ziwei Wang,Xingyi He,Hongbin Wang,Tianwang Jia,Bohan Fang,Dongrui Wu*

Main category: cs.AI

TL;DR: CoMA-DiT是一种双向跨模态扩散变换器，用于潜在增强，将成对模态视为相互生成监督的来源，而不仅仅是融合的输入。通过跨模态注意力条件化速度预测，并通过可靠性门控残差机制自适应注入变化。实验表明在多模态听觉注意解码和情绪识别任务中，该方法一致优于20个基线模型，分别获得4.28%和6.70%的准确率和宏F1绝对提升。


<details>
  <summary>Details</summary>
Motivation: 多模态脑状态解码主要关注融合成对模态进行预测，但很少探索如何利用它们的对应关系来丰富训练数据和改进多模态表示学习。现有方法将成对模态仅视为需要融合的输入，而没有将其作为相互生成的监督来源。

Method: 提出CoMA-DiT，一种双向跨模态扩散变换器，用于潜在增强。主要创新点包括：1)将成对模态作为相互生成的监督来源而非仅仅是融合输入；2)通过跨模态注意力对速度预测进行条件化；3)通过可靠性门控残差机制自适应注入变化。

Result: 在多模态听觉注意解码和情绪识别任务中，CoMA-DiT一致优于20个代表性基线模型。相比无增强基线，在准确率上获得4.28%的绝对增益，在宏F1上获得6.70%的绝对增益。

Conclusion: 研究结果支持一种更广泛的多模态学习观点：成对模态不仅可以作为融合的输入，还可以作为相互增强的监督来源。这一发现为多模态表示学习提供了新的思路，强调了模态间对应关系的潜在价值。
...
### [127] [Portable Semantics, Private Dialects: Reuse and Negative Transfer in Latent Communication Between Language-Model Cells](https://arxiv.org/abs/2609.11365)
*Narcis Marincat*

Main category: cs.AI

TL;DR: 该研究探讨了独立训练的受限语言模型社会之间是否存在可共享的"数据包语言"，以及继承的接口状态对后续学习的影响。研究通过30对独立训练模型的互操作性审计、零样本转移失败分析、负迁移实验等四个主要实验，得出结论：六种语义相似的接口无法形成通用语言，大多数跨初始化方向无法对齐；全局训练的通信接口作为严重的负迁移先验；继承接口从未显著超越新鲜接口对照。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在回答两个核心问题：1) 独立训练的社会是否共享一种数据包语言；2) 继承的接口状态是有助于还是损害后续学习。背景是之前研究发现全局可见模型学习了一种 episode-entangled code，而受限模型倾向于使用可重用的值索引潜在数据包接口。

Method: 研究采用四种主要方法：1) 泄漏控制的30对六独立训练受限社会的因果互操作性审计，使用密封的保留结构和预注册的原始/正交/线性/非线性对齐阶梯；2) 源跨度控制定位零样本失败；3) 匹配适应因子设计测试全局接口的负迁移；4) 两组受限检查点和两组独立冻结目标流的继承接口实验。

Result: 主要结果：1) 六个语义相似接口不构成统一原始语言：一对相同初始化模型完全双向互操作，另一对显示不对称部分兼容性，其余26个跨初始化方向在所有冻结对齐阶梯上失败；2) 零样本失败严格定位到新操作指令的解释和执行；3) 全局接口作为严重负迁移先验，仅重置数据包读取器/写入器/输出端口将深度三准确率从0.169提升至0.857；4) 继承接口从未超过新鲜接口对照的0.10预设边界。

Conclusion: 所有主要结论限制在近迁移17状态设置；负迁移因子设计涉及一个全局可见的父代队列检查点；附录补充了后验标记全局孪生案例研究。研究揭示了独立训练语言模型之间缺乏可迁移的通用接口语言，且全局预训练可能对后续学习造成严重负面影响。
...
### [128] [RAMamba-Net: A Reliability-Aware and Mamba-Based Multimodal Fusion Network for Auditory Attention Detection](https://arxiv.org/abs/2609.11372)
*Xingyi He,Ziwei Wang,Dongrui Wu*

Main category: cs.AI

TL;DR: RAMamba-Net是一种基于Mamba的可靠性感知多模态融合网络，用于从脑电(EEG)和眼电(EOG)信号中解码听觉注意力，实现被关注说话者的识别。


<details>
  <summary>Details</summary>
Motivation: 现有AAD方法存在三个局限：1)跨模态交互弱；2)时间建模效率低；3)对样本变化的鲁棒性差。EEG在自然视听场景中提供不完整证据，需融合EOG信息。

Method: 提出RAMamba-Net，包含：1)Mamba增强的带感知卷积Transformer捕获EEG特定频带模式和长程时间动态；2)双分支时-空编码器建模EOG时间和通道依赖；3)跨模态注意力实现显式模态交互；4)可靠性感知模块估计样本级模态权重以增强特征和预测一致性。

Result: 在两个AAD基准数据集上，RAMamba-Net相比单模态基线提升5.76%准确率，展现更鲁棒解码和判别性表示。显式跨模态交互改善多模态对齐，可靠性感知模块可抑制不可靠模态证据，对信号扰动和参数变化具有鲁棒性。

Conclusion: RAMamba-Net有效利用EEG-EOG互补信息，通过可靠性感知机制增强多模态融合，为神经控制助听设备和自然人机交互提供了更优的听觉注意力解码方案。
...
### [129] [Beyond Confidence: Stability-Aware Test-Time Adaptation for LLM Reasoning](https://arxiv.org/abs/2609.11393)
*Bincheng Gu,Min Gao,Zongwei Wang,Yibing Bai,Yulan He,Junliang Yu*

Main category: cs.AI

TL;DR: TASCO是一种在测试时通过稳定性感知置信度优化来提升LLM推理能力的框架。该方法针对现有基于预测熵的测试时适应方法的局限性——高置信度并不等同于正确性——提出利用局部扰动下置信度的稳定性作为更可靠的推理正确性信号。TASCO通过优化轻量级任务级前缀来纳入局部稳定性，保持LLM冻结状态，采用两种扰动策略（随机扰动和尖锐度感知扰动）来评估和优化局部稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时适应方法使用预测熵作为信号引导模型朝向更高置信度的推理状态，但高置信度并不必然意味着正确性，因为LLM可能对错误推理路径保持高度自信。因此需要更可靠的信号来指导测试时适应。

Method: 提出TASCO框架，包含两个核心创新点：(1)观察到高置信度推理在局部扰动下保持稳定时更可能正确；(2)通过优化轻量级任务级前缀来实现稳定性感知置信度优化，采用两种扰动策略：随机扰动促进附近扰动前缀诱导的轨迹间的分布稳定性，尖锐度感知扰动针对最坏情况局部敏感性。

Result: 实验表明TASCO在不同的LLM和推理基准上提升了推理准确率和token效率。行为分析显示该方法在局部扰动下保持稳定置信度，且不会过早集中模型的预测分布。

Conclusion: TASCO通过引入局部稳定性概念成功地改进了基于置信度的测试时适应方法，在不微调LLM的情况下有效提升了推理性能，为低成本增强LLM下游任务能力提供了新思路。
...
### [130] [Untitled](https://arxiv.org/abs/2609.11403)
**
...
### [131] [LLMs as Post-hoc Auditors of Physiological Plausibility in Symbolic Regression: A Clinician-Evaluated Case Study](https://arxiv.org/abs/2609.11431)
...
TL;DR: 本研究探索利用大型语言模型(LLM)来提升基于遗传编程的符号回归模型的可解释性。研究使用LLM作为后处理工具,分析和排名由语法遗传编程进化的体脂率预测表达式,并由临床医生评估其医学合理性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 符号回归虽能产生可解释的数学表达式,但进化得到的模型可能过于复杂或缺乏科学一致性。如何在保持预测准确性的同时提升模型的医学可解释性和合理性是一个挑战。

Method: 使用四种符号表达式由三种LLM进行三次重复运行分析。由三名临床医生组成专家组,对LLM产生的解释和排名进行评估。

Result: 三种LLM的对比模型排名输出比单独的术语级解释获得更有利的临床评估。然而,LLM也产生了一些生理学和数学上存疑的解释。

Conclusion: LLM更适合在专家监督下进行对比审计,而非自主验证。LLM可作为符号回归模型可解释性的辅助工具,但需要专家审核以确保医学和数学上的准确性。
...
### [132] [Calibration-Aware Uncertainty Cascades for Efficient Heterogeneous Model Collaboration](https://arxiv.org/abs/2609.11446)
*Yilin Zhang,Han Jiang,Cai Xu,Ying Liu,Wei Zhao*

Main category: cs.AI

TL;DR: 本文提出Calibration-Aware Uncertainty Cascades (CAUC)框架，通过对不同模型的置信度进行校准，建立统一的可靠性衡量标准，从而在异构模型协作中实现预测性能与推理成本的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有异构模型协作方法存在两大问题：1) 训练的路由器将路由决策与固定任务和模型池绑定；2) 原始置信度级联的阈值在不同模型间缺乏一致的可靠性语义。这些方法难以适应变化的模型池和部署预算。

Method: CAUC是一个简单的后处理框架，包含三个核心步骤：1) 使用验证数据对每个模型的置信度进行独立校准；2) 基于校准后的置信度建立统一的决策标准；3) 根据该标准选择接受早期预测、调用更强模型或选择性组合模型输出。校准后的置信度建立了共同的可靠性尺度。

Result: 在六个语言基准测试中，CAUC相比强模型单独推理平均相对准确率提升1.9%，同时避免了约47%的强模型调用。在图像分类基准测试中，在降低高达57% GFLOPs的同时保持或提升了预测性能。

Conclusion: 理论证明校准使置信度阈值具有明确的选择性风险解释，而未校准的分数无法提供 comparable reliability guarantee。CAUC将部署策略与特定模型池或操作预算解耦，实验验证了其在语言和图像任务上的有效性。
...
### [133] [RouteRepair: Instance-Level Failure Diagnosis and Targeted Repair in LLM-Based Automated Heuristic Design for Routing Optimization](https://arxiv.org/abs/2609.11452)
*Binghao Ji,Di Huang,Jiahui Fang,Zhiyuan Liu*

Main category: cs.AI

TL;DR: RouteRepair是一种针对路由优化问题的新型LLM驱动启发式设计方法，通过诊断父代启发式在特定实例结构上的弱点并进行针对性修复，在保持已有良好性能的同时提升困难实例上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM自动启发式设计方法的聚合评估可能掩盖在特定实例结构上的 recurrent failures，需要开发能够识别并修复特定弱点的方法。

Method: RouteRepair结合路由证据、求解器行为和程序上下文来定义有界的修复目标，通过实例级性能诊断父代特定弱点，针对性修改相应启发式组件，同时保护已表现良好的行为，并通过父子配对评估验证干预效果。

Result: RouteRepair-GLS将TSP平均最优性差距从1.7476%降至0.7587%；构造性CVRP启发式相对节约启发式降低平均路线成本1.91%；生成的ACO先验优于匹配的手工设计先验。

Conclusion: 失败感知、证据约束的细粒度修复方法能够在保留已有良好实例性能的同时，有效提升困难实例上的路由启发式表现。
...
### [134] [Untitled](https://arxiv.org/abs/2609.11458)
**
...
### [135] [Untitled](https://arxiv.org/abs/2609.11489)
**
...
### [136] [Published Unlearning Numbers Move Per Checkpoint, and Not Because the Removed Data Survives: An Audit of 263 Released Batch-Normalized Checkpoints](https://arxiv.org/abs/2609.11490)
*Junlong Shen Xingyu Li*

Main category: cs.AI

TL;DR: 本文研究了机器学习模型中数据遗忘（unlearning）的审计方法，发现批量归一化统计数据会在模型未明确更新的情况下发生变化，这会影响对模型是否成功遗忘特定数据的判断。


<details>
  <summary>Details</summary>
Motivation: 当前对模型遗忘效果的审计方法存在漏洞：仅检查模型权重和loss变化可能遗漏重要信息。需要一种更可靠的审计方法来验证模型是否真正遗忘了特定数据。

Method: 通过在保留数据上重新拟合批量归一化统计量（bit-identical weights），比较原始发布模型与重新训练参考模型的表现差异，追踪模型状态的漂移程度。

Result: 221个发布检查点中有47个通过了基于批量归一化统计的审计方法；在12个判决中，有4个通过了测量重校准预算，2个在每个重复实验中通过，而接近自身标准训练的模型群体则无法通过。

Conclusion: 批量归一化统计数据的变化会影响遗忘审计结果，建议在发布批量归一化视觉模型时明确说明拟合约定，这为更可靠的模型发布和审计提供了重要参考。
...
### [137] [ActMap: Single-Pass Uncertainty Quantification from Generation-Time Activation Maps](https://arxiv.org/abs/2609.11498)
*Jacopo Dardini,Roberta Calegari*

Main category: cs.AI

TL;DR: ActMap是一种白盒表示方法，将LLM生成时的隐藏状态轨迹压缩为固定大小的张量，用于从单次生成中判断答案是否可信，实现实际的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的实际不确定性量化面临挑战：现有方法需要多次采样、仅读取输出token概率，或将内部计算简化为单一隐藏状态。需要在单次生成中判断特定答案是否可信。

Method: 将生成时隐藏状态轨迹（每层、每个生成的token）压缩为12×32×128的固定张量，包含时间统计通道，保留transformer深度和池化隐藏坐标的结构。使用轻量级分类器（紧凑Vision Transformer或MLP）从张量读取正确性概率。

Result: 在短问答、数学答案和摘要事实性三个任务上，使用三个指令微调的7-8B模型评估，ActMap一致优于采样、token概率、attention和embedding基线方法。与ACT-ViT（使用67倍大的密集激活张量训练）相比，AUROC相当，12对中10对的校准误差更低。

Conclusion: ActMap支持从单次生成中进行放弃、路由和选择性验证，是部署模型可扩展监督的实用原语。固定形状跨模型深度和隐藏大小，压缩至96 KiB，可在审计相关生成中保留。
...
TL;DR: 本文提出了一种结合非消解式冲突分析的SMT求解演算框架，将CDCL(T)风格推理与非消解式冲突分析相结合，在保持可判定性的同时获得指数级更短的证明。


<details>
  <summary>Details</summary>
Motivation: 当前非消解SMT求解主要采用量化实例化方法，但冲突分析仅学习到实例化的具体子句，而非产生冲突的原始非消解子句。这导致学习到的子句过于具体，缺乏通用性，且可能产生冗余。

Method: 提出一种包含三类规则的演算框架：(1)实例化规则生成具体实例；(2)CDCL(T)风格推理规则；(3)非消解冲突分析规则。求解器在具体实例上推理，但冲突分析的消解步骤在原始非消解子句上进行，从而生成更通用的学习子句。同时引入时间回溯机制。

Result: 该方法可生成比具体冲突更通用的学习子句，在适当策略下甚至可生成非冗余子句。该演算为CDCL(T)风格SMT求解、多种实例化过程及非消解子句学习提供了统一框架。

Conclusion: 证明了该演算模拟CDCL、SCL(FOL)、SCL(T)以及消解证明系统，表明其具有极强的表达能力和理论根基，为SMT求解技术提供了新的统一视角。
...
### [139] [Lightweight LiDAR-Based Cone Detection Framework Using Random Forest for Formula Student Driverless](https://arxiv.org/abs/2609.11527)
*Márk Mező-Kerekes,Péter Praksz,Chang Liu*

Main category: cs.AI

TL;DR: 本文提出了一种轻量级的纯LiDAR感知Pipeline，专为CPU设备设计，适用于Formula Student Driverless赛车。通过结合地面移除、IMU运动补偿、DBSCAN聚类和基于几何特征的随机森林分类，实现了高效的障碍物检测与分类。


<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶感知Pipeline通常依赖深度学习和多传感器融合，需要GPU加速，难以在资源受限的FSD赛车上实现低延迟感知。因此需要一种轻量级、CPU可执行的感知方案。

Method: 该Pipeline包含四个核心模块：地面移除、IMU运动补偿、DBSCAN聚类和基于几何特征的随机森林分类。通过特征重要性分析，将模型输入从12个特征精简至7个特征，在保持性能的同时降低了计算复杂度。

Result: 在2,371个真实FSD赛事采集的标注聚类数据集上评估，该Pipeline达到了98.33%的F1分数，端到端推理时间仅为3.13ms（CPU-only）。

Conclusion: 该研究提供了一个实用且可复现的轻量级感知方案，并开源了数据集、标注工具和训练模型，为资源受限的自动驾驶赛车团队提供了有价值的baseline。
...
### [140] [Prompt Revision as a Source of Cultural Bias in Text-to-Image Systems](https://arxiv.org/abs/2609.11532)
...
TL;DR: 本文提出了WORLDVIEW基准，用于审计商业文本到图像系统中的提示修订层，发现非西方和非英语语境被过度标记、扁平化并降维为文化刻板印象，揭示了修订层本身是 stereotyping 的因果来源。


<details>
  <summary>Details</summary>
Motivation: 现有文化偏见审计仅检查最终图像，将生成视为单一管道，无法确定偏见来源。商业文本到图像系统在生成前会悄悄修订用户提示，但用户无法禁用或查看此步骤，因此需要专门审计修订层。

Method: 构建包含8960个提示的多语言基准，涵盖15种语言和31种语言-语境配对；对DALL-E-3、Imagen-4、GPT-Image-1.5进行三步分析：1)测量每个文化语境被标记的程度；2)检查是否将语境扁平化为狭窄词汇；3)验证该词汇是否包含刻板印象；比较原始提示与修订提示在无修订层的模型上生成的图像。

Result: 相对于无语境英语基线，美国是最少被标记的语境；非西方和非英语语境被显著更频繁地标记、被扁平化为狭窄词汇（跨不同主题提示应用）、并被简化为可识别的文化刻板印象。修订层本身被确定为此前未文档化的 stereotyping 因果来源。

Conclusion: 要定位和修复文化偏见，必须审计部署后的完整系统而非仅模型本身。修订层是 stereotyping 的独立来源，需要被纳入偏见审计和修复的考量范围。
...
### [141] [Untitled](https://arxiv.org/abs/2609.11542)
**
...
### [142] [Untitled](https://arxiv.org/abs/2609.11569)
**
...
### [143] [Untitled](https://arxiv.org/abs/2609.11607)
**
...
### [144] [Untitled](https://arxiv.org/abs/2609.11615)
**
...
### [145] [MAPLE: Memory-Augmented Planning with Language and Evolution](https://arxiv.org/abs/2609.11636)
*Kesheng Chen,Yamin Hu,Wenjian Luo*

Main category: cs.AI

TL;DR: MAPLE是一个基于LLM的优化智能体,支持通过自然语言请求维护和更新优化问题,结合语言建模、数学规划和进化搜索,在NLDO基准测试中取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 现实运营环境是动态变化的,需求、资源和优先级不断变化,传统优化方法难以支持快速适应和保留之前决策及搜索结果。领域从业者缺乏运筹学专业知识,需要更易用的优化工具。

Method: MAPLE结合基于语言的问题构建、数学规划和进化搜索,保留优化程序、已接受计划、早期更新和候选解决方案。引入NLDO基准测试,包含15条轨迹和180个更新,涵盖选择、调度、排班、路由和云资源放置等场景。

Result: MAPLE完成所有轨迹,在线标量质量达0.951,帕累托超体积比达0.875。控制实验表明,维护可执行状态可提高更新有效性,并能在重大修订中保留有用搜索信息。

Conclusion: MAPLE能有效支持优化问题的动态维护和更新,通过保留搜索信息和可执行状态,提高了更新有效性和信息重用能力,使优化更加易于访问和适应动态需求。
...
### [146] [Autonomy, Social Norms, and Alignment: Towards a Developmental Framework for Autonomous Artificial Agents](https://arxiv.org/abs/2609.11660)
*Marica Notte,Ludovica Marinucci,Vieri Giuliano Santucci*

Main category: cs.AI

TL;DR: 本文探讨了具身AI智能体在动态环境中如何通过经验学习和内在动机实现与人类目标的对齐。作者提出规范必须根植于经验，通过认识论过程逐步构建更复杂的规则，类似于儿童学习社会规范的方式。监管沙箱可被视为AI对齐的教育环境。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的AI系统依赖于预存数据集和人类反馈策略，但在动态或未知环境中这些方法不足。具身智能体需要通过与环境的直接交互来获取知识，同时确保与人类目标的对齐。传统规则在非结构化动态环境中失效，需要新的对齐方法。

Method: 概念性/哲学分析论文，引用Dennett关于道德主体地位的观点，将监管沙箱概念化为 pedagogical（教育学）环境。

Result: 提出通过内在动机（好奇心、能力感）引导探索；规范需通过经验认识论过程逐步构建；监管沙箱可作为AI对齐的动态教育空间。

Conclusion: AI对齐应被视为一个形成性过程，类似于儿童通过探索环境和参与集体实践学习社会规范。监管沙箱是培育AI对齐的 pedagogical 环境，智能体需通过与环境的交互和与其他道德主体的合作逐步获得道德主体地位。
...
### [147] [Geospatial AI, Dataverse Metadata, and the Study of Place-Based Government](https://arxiv.org/abs/2609.11674)
*Danny EBanks,Devika Jain*

Main category: cs.AI

TL;DR: 哈佛Dataverse知识图谱项目从超过15万个研究数据集中提取地理信息，构建了包含10万余数据集、21.5万节点和52.8万边的知识图谱，其中42.9%的数据集具有地理元数据，17.4%与政策相关。


<details>
  <summary>Details</summary>
Motivation: 哈佛Dataverse存储超过15万个研究数据集，但其中的地理信息以自由文本形式输入，无法被搜索和利用，研究者试图将这些地理元数据结构化以便检索和分析。

Method: 从哈佛Dataverse公共数据和元数据构建知识图谱，使用保守关键词搜索识别政策相关数据集，并展示社区语言模型、立场检测和党派语言工具在地理聚合中的应用。

Result: 构建了包含102,650个数据集、215,985个节点和528,003条边的知识图谱；43,991个数据集(42.9%)具有地理空间元数据；7,654个数据集(17.4%)与政策直接相关，其中选举和立法是最大聚类，其次是政府管理、健康政策、交通和教育。

Conclusion: 知识图谱为AI驱动的元数据丰富和实体解析提供了具体场景，但存在地点解析障碍（同一地点出现为多个断开节点），且覆盖范围偏向美国城市级数据。
...
### [148] [Untitled](https://arxiv.org/abs/2609.11682)
**
...
TL;DR: SIRF通过持续预训练将平台复杂策略内化到模型权重中，实现高精度、低延迟的风险控制。SIRF-8B-SFT在Black Recall@P95达到71.3%，较基线提升15.1个百分点，仅用约70M CPT tokens，且不影响通用能力。


<details>
  <summary>Details</summary>
Motivation: 工业内容风险控制的实际部署约束不是平均准确率，而是高 precision 下二级别延迟内能自动处理多少风险。现有方法依赖外部规则系统，延迟高、精度低。

Method: 通过EntiGraph、MAGA重写和账户级思维链(CoT)合成策略数据，无需人工标注，使用持续预训练(CPT)将策略内化到模型权重中，实现超低延迟的判决式部署。

Result: SIRF-8B-SFT达到71.3% Black Recall@P95，较基线(Qwen3-8B-SFT)提升15.1pp；作为树模型判决层，误处罚样本恢复提升20%；迁移到冻结场景误处罚相对减少约70%。

Conclusion: 策略内化是一种有效的风险控制方法，可在小规模持续预训练(~70M tokens)下显著提升风险处理能力，且不损害模型通用能力，为工业风险控制提供了新范式。
...
### [150] [Untitled](https://arxiv.org/abs/2609.11768)
**
...
### [151] [Untitled](https://arxiv.org/abs/2609.11859)
**
...
### [152] [Untitled](https://arxiv.org/abs/2609.11860)
**
...
### [153] [Untitled](https://arxiv.org/abs/2609.11876)
**
...
### [154] [MindTopo: Can Foundation Models Reason in Topological Space?](https://arxiv.org/abs/2609.11900)
...
TL;DR: MindTopo是一个基于认知科学和拓扑学的拓扑直觉基准测试，评估多模态大语言模型(MLLM)在5种拓扑属性(连续性、分离性、顺序性、包围性、结)上的推理和规划能力，包含11,030个实例和13种任务类型。


<details>
  <summary>Details</summary>
Motivation: 空间推理不仅依赖度量属性(距离、角度、形状)，还依赖在连续变形下保持不变的拓扑关系。认知科学将这些关系视为空间理解的基础，但现有基础模型评估主要关注度量或视角相关关系，拓扑直觉长期被忽视。

Method: 构建包含5种拓扑属性的基准测试，评估两个认知层次：(1)推理-识别拓扑关系或推断其变化；(2)规划-将模型作为闭环智能体选择环境动作。包含11,030个实例，涵盖13种程序生成任务类型，测试14个MLLM，并研究图像和视频生成增强的智能体配置。

Result: 所有MLLM在推理任务上表现优于规划任务；最佳模型仍远低于人类表现；在Qwen3-VL-2B-Instruct上，监督微调和强化学习对推理的提升大于规划；生成的观测保留局部线索并到达合理端点，但审计 rollout 无法可靠地遵循环境动力学或在转换中保持拓扑结构。

Conclusion: 当前多模态大语言模型在拓扑直觉方面存在显著不足，推理能力优于规划能力，但两者均远未达到人类水平，表明基础模型在理解拓扑关系方面面临根本性挑战。
...
### [155] [Untitled](https://arxiv.org/abs/2609.11911)
**
...
### [156] [Untitled](https://arxiv.org/abs/2609.11916)
**
...
### [157] [Untitled](https://arxiv.org/abs/2609.10916)
**
...
### [158] [Untitled](https://arxiv.org/abs/2609.11599)
**
