...
TL;DR: 本文提出了一种基于持久同源性（拓扑数据分析）与支持向量机（SVM）结合的自动化方法，用于CBCT扫描中的牙齿分类和病理诊断。该方法在牙齿标注任务中达到97.67%的准确率，在诊断任务中达到96.77%的准确率，优于CNN模型。


<details>
  <summary>Details</summary>
Motivation: CBCT扫描在牙科中广泛应用，但人工分析费时费力，需要自动化分析步骤如牙齿分类和病理检测。

Method: 采用持久同源性框架（拓扑数据分析）提取数据的形状特征，结合支持向量机进行分类和诊断。

Result: 牙齿标注任务准确率97.67%，诊断任务准确率96.77%，均优于CNN模型（70.27%和86.67%）。

Conclusion: 基于持久同源性与SVM的方法在CBCT牙齿分类和诊断任务上优于CNN，为牙科CBCT自动化分析提供了有效方案。
...
TL;DR: EditStream是一个统一的基于DiT的交互式视频生成和编辑框架，支持文本到视频、图像到视频、视频到视频、编辑传播、参考引导视频编辑和相机姿态变化等多种任务，并通过两阶段蒸馏（速度矩匹配VMM与自回归展开）实现高效的少步自回归视频生成。


<details>
  <summary>Details</summary>
Motivation: 交互式视频生成和编辑在创意设计中变得越来越重要，但现有方法通常只能处理单一任务，缺乏统一性；同时，高质量扩散模型推理速度慢，难以满足交互式使用需求。

Method: 1）通过灵活的特定任务条件将多个视频创建和操作任务统一在单一DiT模型中；2）开发两阶段蒸馏方法，结合速度矩匹配（VMM）和自回归展开：VMM匹配学生模型达到的中间状态的条件速度矩以保持生成质量和运动，自回归展开让学生接触自己的预测以提高时间稳定性。

Result: EditStream能够在一个系统中实现对视频生成、转换和编辑的灵活控制，解决了少步自回归视频生成中常见的过饱和、运动退化、时间不稳定和复杂训练等问题。

Conclusion: EditStream提供了一个实用且可扩展的解决方案，架起了高质量基于扩散的视频模型与交互式创意工作流之间的桥梁。
...
TL;DR: 提出统一偏好感知学习框架，解决视频生成中人类偏好对齐的三大挑战：奖励信号可靠性、多维偏好建模和全局结构对齐问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成虽在视觉质量上取得进展，但存在奖励信号不可靠、标量奖励模型丢失多维偏好动态权衡、KL散度仅施加局部约束无法捕捉全局偏好结构等问题。

Method: 提出三阶段方法：(1)精英引导过滤筛选偏好数据，为奖励模型训练提供可靠监督；(2)将视频质量建模为多维奖励分布，使用Wasserstein距离对齐学习分布与经验人类偏好分布；(3)将Wasserstein分布对齐引入GRPO，引导策略优化更好匹配视频全局偏好结构。

Result: 实验表明该方法提高了奖励信号可靠性和生成视频的感知一致性。

Conclusion: 提出的统一偏好感知学习框架能有效解决视频生成中的人类偏好对齐问题，提升生成质量。
...
TL;DR: 本研究开发了一种基于CNN的AI视觉检测系统用于服装缝纫线质量控制，实验表明系统可有效检测部分颜色面料的跳线缺陷，但受训练数据多样性限制，对颜色差异大的面料泛化能力不足。


<details>
  <summary>Details</summary>
Motivation: 服装制造业面临提高产品质量、降低成本的压力，且需加速向工业4.0数字化转型。缝纫线检验是质量控制中最具挑战性的活动之一，人工检验易受疲劳、主观判断影响，导致缺陷泄漏、返工和生产效率降低。

Method: 采用卷积神经网络(CNN)开发视觉检测系统，使用黑色面料和黑色缝纫线样本进行初始训练，并在黑、红、深绿、浅蓝、银和荧光黄等六种不同颜色的面料上进行实验测试。

Result: 系统成功检测到黑、红、深绿面料上的跳线缺陷，但在线迹断裂缺陷检测方面表现不佳，且对于颜色特征差异较大的浅蓝、银色和荧光黄色面料存在检测局限性。

Conclusion: 研究成功开发了基于CNN的AI视觉检测系统，可检测黑色、红色和深绿色面料上的跳线缺陷，但对面料颜色差异较大的情况（如浅蓝色、银色和荧光黄色）检测性能有限，表明训练数据多样性对模型泛化能力有重要影响。
...
TL;DR: 本文通过少样本学习研究解决胸部X光片结核病检测中的域迁移问题，使用TBX11K作为源域，Mendeley TB数据集作为目标域，对比了冻结骨干网络适配、完全微调和从头训练三种策略，发现仅用75个带标签样本即可达到98.36%准确率。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在不同数据集上存在严重的域迁移问题，不同的成像协议、患者人口统计数据和设备导致模型泛化困难，在实际应用中模型在一个数据集上表现良好但在另一个数据集上性能显著下降。

Method: 使用TBX11K作为源域，Mendeley TB数据集作为目标域，进行受控的跨数据集评估。研究了三种训练策略：冻结骨干网络适配、完全微调和从头训练。评估不同目标样本数量对模型性能的影响。

Result: 仅用75个带标签样本每类即可达到98.36%的准确率。微调曲线表明微调策略能有效缓解域迁移问题。

Conclusion: 对于低资源临床部署场景，完全微调预训练模型是缓解域迁移的一种高效且实用的策略。
...
TL;DR: 本研究开发了计算管道来识别动画电影中的角色，并利用它分析了224部热门动画电影中的性别 representation问题。研究发现人类角色中女性 representation较高，而非人类角色则以男性为主，且女性角色更倾向于共同出现于场景中。


<details>
  <summary>Details</summary>
Motivation: 过去对动画电影中性别 representation的研究主要关注演员阵容的性别构成，缺乏更精细的评估工具（如角色出场时间）。本研究旨在通过计算方法更准确地评估动画电影中的性别 representation。

Method: 开发了用于识别动画角色的计算管道，对224部1980-2025年间的热门动画电影进行分析，检验多个关于性别 representation的假设。

Result: 1. 动画电影中女性角色的整体representation与真人电影大致相当；2. 人类角色中女性 representation明显更高，而非人类角色则 largely为男性；3. 女性角色在反派角色中的比例并未下降；4. 女性角色比真人电影中更倾向于共同出现在场景中。

Conclusion: 动画电影中的性别 representation模式与真人电影存在显著差异，尤其在人类/非人类角色的对比上。这一发现挑战了以往对迪士尼动画的研究结论，表明动画电影中的性别 representation更为复杂。
...
TL;DR: 本文提出SCoRe框架，用于基于知识的视觉问答任务，通过结构化上下文推理来整合视觉信息和外部知识，解决长而不结构化提示导致的推理性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的VQA方法存在两个问题：1) 过长且异构的上下文包含过多无关信息；2) 缺乏明确的关系结构。因此需要一种能够提取和利用显式/隐式关系的方法来提升推理效果。

Method: 提出三阶段框架：1) Context Acquisition：生成多样化视觉笔记+两阶段多模态检索显性知识；2) Context Selection：LLM引导选择相关视觉/显性/隐含知识；3) Context Compression：关系逻辑蒸馏(RLD)将文本转为实体-关系三元组，作为结构化提示用于最终预测。

Result: 在OK-VQA和A-OKVQA基准测试上，SCoRe一致优于现有最先进方法。

Conclusion: 通过显式建模实体关系并进行上下文压缩，SCoRe能够更有效地利用多模态信息和外部知识，为基于LLM的视觉问答提供了一种新的结构化推理范式。
...
TL;DR: DesignAgent3D是一个交互式多模态智能体框架，将3D场景编辑重新定义为类似设计师的"计划-感知-行动"范式，通过与用户交互澄清不明确的设计目标，然后将编辑意图锚定到3D场景中的特定对象或区域，最后应用受控的视觉修改同时保持场景一致性。该方法支持持久的和多视角一致的新视角渲染，在NeRF和3DGS后端上均显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自然语言设计请求通常是语义上未明确指定的，必须在杂乱的3D场景中进行锚定。现有方法通常 formulates任务为单次条件生成，无法解决歧义的用户意图或实现精确的空间锚定。因此存在严重的对象定位漂移、遮挡下的跟踪失败和多视角"贴纸效应"问题。

Method: Plan-Perceive-Act范式：1) 计划(Plan)：通过与用户交互澄清不明确的设计目标；2) 感知(Perceive)：将编辑意图锚定到3D场景中的特定对象或区域；3) 行动(Act)：应用受控的视觉修改同时保留场景一致性。编辑内容集成到底层3D表示中，支持持久和多视角一致的新视角渲染。

Result: 在NeRF和3D Gaussian Splatting后端上进行了广泛实验，证明DesignAgent3D显著优于最先进基线，实现了卓越的语义意图对齐、精准的空间定位精度和高保真的多视角一致性。

Conclusion: DesignAgent3D通过交互式多模态智能体框架有效解决了3D场景编辑中的语义歧义和空间锚定问题，为文本引导的3D场景编辑提供了新的研究方向。
...
TL;DR: WorldMind是一个解耦的游戏世界模型框架，通过四层架构（理解层、决策层、控制层、生成层）实现状态感知的NPC行为生成，解决了现有模型中NPC行为与视频生成紧密耦合的问题，并引入BOSS-140K数据集进行验证。


<details>
  <summary>Details</summary>
Motivation: 现有游戏世界模型中，NPC行为要么与视频生成隐式纠缠，要么通过外部控制信号显式规定，导致模型需要同时理解状态、规划NPC响应并渲染视觉结果，缺乏状态化决策的显式接口，难以产生响应式和状态感知的NPC行为。

Method: 提出四层解耦架构：1）理解层从生成帧中构建紧凑状态；2）决策层基于紧凑状态推理规划NPC下一步动作；3）控制层将动作转换为时间对齐的条件；4）生成层合成视觉结果。通过将各层连接成闭环交互循环，使NPC行为根植于不断演变的游戏状态。同时构建BOSS-140K数据集，包含配对的游戏玩法视频和丰富的内部游戏状态。

Result: 在BOSS-140K上的实验表明，WorldMind实现了可靠的紧凑状态重建和基于机制的规划，在约70%的成对比较中优于基线，表现出更战术合理和连贯的NPC行为。

Conclusion: WorldMind是首个实现游戏世界模型中状态感知NPC行为的解耦框架，通过四层架构有效解耦了状态理解、动作规划、控制器和视觉生成，为生成更智能、更响应的游戏NPC行为提供了新范式。
...
TL;DR: CM-GLasso是一个将视觉-语言表示学习与稀疏高斯图模型相结合的框架，用于从多模态视觉-语言特征中估计可解释的条件依赖结构。该方法通过文本可视化策略、跨模态注意力蒸馏机制和联合ADMM优化，在8个基准数据集上实现了竞争力的性能，在VOC和ADE20K数据集上获得了最高的分割mIoU。


<details>
  <summary>Details</summary>
Motivation: 从多模态视觉-语言特征中估计可解释的条件依赖结构这一任务目前研究甚少。现有方法缺乏将视觉-语言表示学习与稀疏高斯图模型有效结合的方案，因此需要一种能够同时处理跨模态特征并学习可解释稀疏图结构的统一框架。

Method: 提出三个关键组件：1）文本可视化策略：将类属性描述渲染为图像，通过SigLIP-2视觉编码器处理，在共享特征坐标系统中生成原型索引的patch级注意力足迹；2）跨注意力蒸馏机制：将高维patch压缩为少量语义图节点，其注意力足迹相似性产生跨模态结构先验，用于非均匀L1惩罚；3）联合ADMM formulation：在单一凸目标中估计共享和类特定的精度分量，避免了先估计后分解的繁琐过程。

Result: 在8个基准数据集上与强基线方法相比达到竞争力或更优性能。在匹配控制协议下，获得最高平均分类准确率（91.97%），在VOC上获得最高分割mIoU（74.75%），在ADE20K上获得最高分割mIoU（64.01%），同时产生具有通用-特定分解的显式稀疏条件依赖图。

Conclusion: 学习到的稀疏图拓扑直接支持无参数的基于精度的分类规则和轻量级的拓扑感知分割头，验证了将视觉-语言表示学习与稀疏高斯图模型相结合的有效性，为可解释的多模态理解提供了新的研究思路。
...
TL;DR: ViTexSZ是一个用于EEG癫痫发作检测的异构视觉-文本知识蒸馏框架，能够处理不同通道配置的EEG记录，通过将EEG转换为结构化波形图像，并利用多模态大语言模型整合临床语义，实现轻量级学生模型的精准检测。


<details>
  <summary>Details</summary>
Motivation: 现有时序方法通常针对固定EEG通道配置设计，难以应用于具有不规则通道布局的异构EEG记录；视觉和语言建模虽然提供了替代方案，但将异构EEG表示与临床语义对齐仍具有挑战性。

Method: 提出ViTexSZ框架，包含三个核心组件：1）将EEG记录转换为结构化波形图像；2）引入基于查询的多通道对齐模块，将源依赖的视觉特征映射到统一的token空间；3）异构教师通过多模态大语言模型将对齐的EEG表示与临床提示整合，建立高级临床语义与癫痫发作证据的关联；最后通过视觉-文本知识蒸馏将教师表示迁移到轻量级学生模型进行检测。

Result: 在四个EEG癫痫数据集上的实验表明，ViTexSZ在亚临床和一般癫痫发作检测场景中均表现出良好的泛化能力，在所有数据集上达到最高准确率，相比次优基线方法相对提升高达12.9%。

Conclusion: ViTexSZ通过异构视觉-文本知识蒸馏有效解决了EEG通道异构性问题，实现了高精度的癫痫发作检测，具有重要的临床应用价值。
...
TL;DR: BIMScript是一种基于结构化语言的场景重建模型，将建筑场景重建为可编辑的程序化命令，支持直接导入BIM工具（如Revit），并计划支持LLM驱动的可持续性推理。


<details>
  <summary>Details</summary>
Motivation: 现有结构化语言模型（如SceneScript）虽然能以参数化命令形式重建场景，但距离实际应用（将现有建筑自动摄入BIM工具）仍有三个关键问题：1）场景由什么组成（材质和条件属性）；2）重建速度如何；3）每个元素的精确位置在哪里。

Method: 1）扩展布局语言，添加每元素材质和条件属性，使用基于视觉-语言模型的材料护照语料库（在10万+合成场景上训练）进行监督；2）设计输出精确的CUDA图解码器（1.9 vs 6.4ms/step，3.4倍加速）和基于语法并行、容差验证的draft-and-verify方案；3）采用免训练的几何吸附和混合离散-连续解码器头回归亚像素偏移，解决5cm token-grid粒度问题。

Result: 1）通过提升特征点编码器将图像外观路由到材质token；2）CUDA图解码器实现3.4倍加速；3）几何吸附和混合解码器恢复了大部分残差；4）成功实现直接摄入Revit的BIM创作工具端到端验证，支持IFC4导出。

Conclusion: BIMScript通过统一语法回答了场景组成、速度和精度三个问题，实现了从合成扫描到BIM工具的直接摄入，语言形式支持LLM驱动的可持续性推理，是结构化语言模型在建筑信息建模领域的重要进展。
...
TL;DR: KBMR是首个针对知识库视觉问答(KB-VQA)任务设计的MLLM(多模态大语言模型)嵌入检索器，旨在解决传统CLIP风格双编码器在处理长尾实体时过度依赖表面视觉相似性而忽视实体级语义对齐的问题。


<details>
  <summary>Details</summary>
Motivation: 现有检索管道主要采用CLIP风格的双编码器，这类方法优先考虑表面层面的视觉相似性而非实体层面的语义对齐。当语义相同的概念呈现较大视觉差异，或不同实体在视觉上相似时，现有方法往往失效。这是KB-VVA领域面临的核心挑战。

Method: 1. 利用MLLM强大的自回归能力，将图像映射到更好地保留概念身份的语义空间；2. 引入基于MLLM的语义判别器，生成连续实体一致性权重；3. 提出连续语义蒸馏目标，实现有效的硬负采样和超越二元标签的软监督。

Result: 显著优于CLIP基线模型，检索Recall@1提升高达14.7%，端到端VQA准确率提升9.4%。

Conclusion: KBMR通过MLLM-based嵌入检索器和连续语义蒸馏机制，有效解决了KB-VQA中的语义对齐问题，为多模态检索领域提供了新的研究方向。
...
TL;DR: 本文提出FruitCapsNet，一种基于胶囊网络的自动化水果分类方法，旨在解决野外水果分类中类别内差异大、类别间差异小的问题。该方法使用膨胀卷积替代传统卷积前端，通过动态路由实现部分-整体空间一致性，并采用贝叶斯优化选择超参数。实验在四个数据集上验证，性能超过十个微调的迁移学习骨干网络。


<details>
  <summary>Details</summary>
Motivation: 野外水果分类面临挑战：同一水果以不同形态出现（成串、未采摘、去皮、塑料袋装、切片等），导致类别内差异大（形状、大小、颜色、纹理）而类别间差异小。传统卷积网络通过池化操作会丢失目标的姿态和位置信息，导致泛化能力差。

Method: 提出FruitCapsNet胶囊网络：(1)用膨胀卷积替代标准卷积前端， receptive field 指数增长而参数成本恒定，每个胶囊编码多尺度上下文；(2)通过动态路由解决部分-整体空间一致性问题；(3)使用贝叶斯优化而非网格搜索选择超参数（包括膨胀因子）。

Result: 在三个公开数据集（SMP、FruitsGB、Fruits-360）和一个新的19类10,639张图像的野外数据集（PD-19）上，FruitCapsNet以三分之一深度超越十个微调迁移学习骨干网络，在最难的数据集上比最优竞争对手高2.7%。

Conclusion: Grad-CAM可视化结果表明改进来自于将决策归因于整体水果区域而非物体边缘，提供了事后证据证明增益不是数据集伪影。胶囊网络能够更好地捕捉整体特征，避免了传统卷积网络丢失位置信息的问题。
...
TL;DR: 本文提出TPO数据集（西红柿、土豆、洋葱），用于研究如何在不依赖人脸的情况下学习可迁移的 Presentation Attack Detection（PAD）表示。实验表明，使用该数据集训练的检测器在四个跨数据集人脸PAD基准测试中平均AUC达到92.70%，优于合成人脸训练，且与真实人脸数据集训练相当。


<details>
  <summary>Details</summary>
Motivation: 传统的人脸呈现攻击检测（PAD）通常依赖于人脸特定特征，但打印、重放和重拍等攻击引入的视觉伪影并不与人脸外观固有相关。作者试图探索是否可以在不使 faces 的情况下学习可迁移的PAD表示。

Method: 1. 构建TPO数据集：包含西红柿、土豆、洋葱的真实样本、打印和重放攻击；2. 使用基于基础模型的PAD架构进行训练和测试；3. 在四个标准跨数据集人脸PAD基准上进行评估；4. 进行表征和频率分析。

Result: 1. 在四个人脸PAD跨数据集基准上平均AUC达92.70%，优于合成人脸训练；2. 与真实人脸数据集训练效果相当；3. 人脸PAD模型训练后能迁移到TPO数据集，表明学习到的是攻击过程特征而非对象语义；4. 将TPO纳入人脸PAD训练可持续改善跨数据集性能；5. 可迁移的PAD表示无法由单一频谱伪影解释，而是编码了更丰富的跨类别攻击线索。

Conclusion: 可迁移的呈现攻击表示可以独立于人脸内容学习，这为隐私保护和身份无关的PAD开发提供了新的可能性。
...
TL;DR: 本文提出CLSC DETR方法解决无人机目标检测中小物体检测困难的问题，通过跨层局部支持模块和分类定位一致性校准模块来改进候选框排序质量。


<details>
  <summary>Details</summary>
Motivation: 无人机目标检测中小物体空间范围有限、分布密集、频繁遮挡，导致基于DETR的方法中单个查询缺乏足够的几何证据来可靠估计定位质量，从而造成排序不稳定。

Method: 提出两个关键模块：1) 跨层局部支持模块(CLLS)在最终层查询和中间层候选之间建立对应关系，聚合互补的几何证据以更可靠地估计定位质量；2) 分类与定位一致性校准模块(CACC)根据定位质量和分类可靠性自适应调整分类分数以改进候选排序。

Result: 在VisDrone数据集上，CLSC DETR相比基线方法提升了1.5%的AP和2.0%的AP75，在UAVDT数据集上也获得了一致的性能提升。

Conclusion: 通过跨层信息聚合和分类定位一致性校准，CLSC DETR有效解决了小目标检测中质量估计不可靠的问题，显著提升了无人机场景下的目标检测性能。
...
TL;DR: 本文提出了FigmaTrace数据集，包含超过200小时的视频数据转换为3469条设计轨迹，用于训练视觉语言模型在主观创意设计任务中的表现。该数据集基于专家策划的设计技能分类法构建，包含126个开放式主观任务。实验表明在该数据集上训练的模型在四个分布外GUI代理环境中性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在客观可验证的任务（如目标检测）上表现良好，但在主观和创意设计任务上表现不佳。主要原因是缺乏高质量的人类工作流数据来捕捉使人类专家擅长设计任务的偏好和决策。

Method: 1. 定义专家策划的设计技能和最佳实践分类法；2. 扩展为126个开放式主观长视野任务；3. 收集超过200小时的人类捕获视频数据；4. 采用基于设计阶段的视频转轨迹方法将视频转换为3469条设计轨迹；5. 在此数据集上训练四个模型进行评估。

Result: 在FigmaTrace上训练的模型在四个分布外代理GUI环境中的性能提升与前沿闭源模型Claude-Opus-5和GPT-5.6-Sol相当。基于设计阶段的视频转轨迹方法优于先前基于长度的方法。Qwen3.8-27B模型表现最佳。

Conclusion: FigmaTrace数据集和基于设计阶段的视频转轨迹方法能够有效提升视觉语言模型在主观创意设计任务上的表现，为设计自动化领域提供了有价值的资源。
...
TL;DR: 研究提出"复杂性诱导"方法，通过对训练数据进行结构化扭曲（不使用架构修改），使标准CNN分类器获得组合泛化能力。在彩色几何形状的合成图像上实验，使用flat string标签（如"red-circle"），训练时排除某些颜色-形状组合。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在组合泛化（compositional generalization）方面的局限性，即模型难以正确预测训练中未见过但由已知组件组合而成的新类别。当前方法多依赖架构修改，本文探索通过训练信号的结构化扭曲来诱导组合泛化。

Method: 使用两类结构化扭曲方法：1）混合标签（mixed labels）：基于类别名称的Jaccard字符串相似性创建软目标分布编码类间重叠；2）扩展数据集（expanded dataset）：使用结构化动机生成的错误标签创建假训练样本。对比实验：随机错误标签的对照组。

Result: 两种方法均诱导了预测未见类别组合的能力。混合标签通过利用CNN自然嵌入结构激活未见组合；扩展训练则改善嵌入的分解本身。对照组（随机错误标签）无此效果，证明效果依赖于扭曲的结构性而非噪声本身。

Conclusion: 训练信号的结构化复杂性可以影响学习表征的内部组织及其组合解释，暗示自然语言在认知发展中可能扮演类似角色。这一原理可为理解人类认知泛化提供参考。
...
TL;DR: 本研究针对橡胶混凝土(RuC)的细尺度表面缺陷检测问题,开发了一种近距摄影测量 workflow,替代传统TLS LiDAR方法。通过DSLR和iPhone 16采集图像,结合SfM-MVS算法重建密集3D模型,并开发RGB引导的裂缝提取方法实现表面缺陷识别。DSLR重建达到亚毫米分辨率,可有效量化测试前后表面变形。


<details>
  <summary>Details</summary>
Motivation: 传统TLS(地面激光扫描)受物理限制,激光光斑尺寸大于微裂缝宽度,无法有效检测细尺度缺陷。近距摄影测量在细尺度的应用尚未被充分探索。本研究旨在开发适用于高延性复合材料(橡胶混凝土)的精细缺陷检测方法。

Method: 使用Canon DSLR和iPhone 16采集高分辨率图像,结合SfM-MVS算法生成密集3D模型;开发RGB引导的裂缝提取方法识别表面缺陷;进行测试前后变形分析量化表面位移。

Result: DSLR重建达到亚毫米分辨率,性能优于iPhone;RGB引导方法可有效提取裂缝区域,提供清晰的几何表示;变形分析成功量化了不同测试阶段的表面位移。

Conclusion: 近距摄影测量是实验室环境下表面检测和变形监测的灵活、高分辨率替代方案,可为未来自动化3D特征表征和材料性能评估建立稳健的几何基准。
...
TL;DR: EXPL-FR是一种在面部识别(FR)模型嵌入空间内解释其决策的方法，通过轻量级适配器将视觉-语言模型(VLM)的编码器与冻结的FR空间对齐，将978个属性提示转换为FR空间锚点，无需文本训练。


<details>
  <summary>Details</summary>
Motivation: 深度FR模型准确率接近饱和但缺乏可解释性，实践者无法知道相似度分数依赖于哪些语义属性，需要一种方法来揭示FR模型自身的嵌入空间中蕴含的语义信息。

Method: 1)训练轻量级适配器将VLM图像编码器与FR空间对齐，仅使用人脸图像训练，不使用文本；2)同一适配器应用于文本编码器，将22类978个属性提示转换为FR空间锚点；3)使用无标签可检测性度量比较每个概念在FR空间与VLM空间中的可分离性；4)选取100个最可检测的概念形成模型的语义签名。

Result: 1)并非所有概念都能迁移到FR空间，FR模型通过丢弃身份验证必须跨越的因素来获得不变性；2)语义签名比完整词汇表更能分离身份；3)无需标签的提示驱动审计可按种族分类错误排名FR模型，并按真实验证成本排名受控属性变化。

Conclusion: EXPL-FR可在不访问模型架构的情况下解释FR模型，支持身份级、每图像和差分解释，提示驱动审计无需人工标签即可评估FR模型性能。
...
TL;DR: TASSO是一种用于视觉语言模型持续学习的新范式，通过子空间学习和几何感知知识蒸馏来有效保持潜在空间几何结构同时确保网络可塑性。该方法学习任务特定的低秩投影器，并将几何距离损失用于从先前任务模型中蒸馏知识，从而减轻灾难性遗忘和零样本性能下降。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLM）具有强大的零样本能力，但在持续适应过程中会出现灾难性遗忘和零样本性能下降问题，严重影响模型性能，这使得如何在保持零样本能力的同时避免灾难性遗忘成为关键挑战。

Method: 提出两种互补技术：1）子空间学习：学习一系列任务特定的低秩投影器，将潜在表示投影后再优化交叉熵损失；2）几何感知知识蒸馏：使用基于测地线距离的损失从先前任务模型蒸馏知识，有效保持潜在空间几何结构。这些设计避免了对完整嵌入维度的非必要参数更新，聚焦于任务特定流形。

Result: 在CLIP视觉语言模型上进行了多领域任务增量学习和类增量学习基准测试，结果表明该方法在减轻遗忘和保持零样本能力方面明显优于现有最先进方法。

Conclusion: TASSO通过子空间学习和几何感知知识蒸馏的结合，成功地在保持网络可塑性的同时 preserve 了潜在空间几何结构，显著减少了灾难性遗忘和零样本性能下降，为VLM的持续学习提供了有效的解决方案。
...
TL;DR: DamageScope是一个结合卫星图像、视觉语言模型(VLM)和大语言模型(LLM)的检索增强框架,用于自动化的财产损失分析,支持自然语言查询进行灾害评估。


<details>
  <summary>Details</summary>
Motivation: 传统现场检查方法劳动密集、成本高且存在安全风险,而现有卫星图像和VLM结合的大规模地球观测管道面临计算效率、数据组织和信息检索方面的挑战。

Method: 1.构建基于检索增强生成(RAG)的框架,从卫星图像中提取结构化视觉表示;2.引入一种新型多向量嵌入聚类算法,替代传统的单向量嵌入方法;3.采用双存储数据架构减少LLM API调用。

Result: 多向量嵌入聚类算法性能优于传统单向量嵌入方法,同时索引时间减少高达14倍;双存储数据架构将运营成本和响应延迟降低约3倍。

Conclusion: DamageScope有效平衡了可扩展性和运营效率,为现实世界的灾害评估任务提供了稳健且实用的解决方案。
...
TL;DR: 本文提出presto框架，利用多模态大语言模型(MLLM)将开放世界物体放置问题重新定义为启发式搜索任务，通过粗到细的搜索策略迭代优化物体位置和尺度，实现零样本、无需训练的开放世界物体放置。


<details>
  <summary>Details</summary>
Motivation: 现有物体放置方法依赖手工规则或有限数据集的监督学习，泛化能力和可解释性受限，难以处理开放世界中新物体和新场景的放置问题。

Method: 将开放世界物体放置重新定义为启发式搜索任务，使用MLLM进行推理引导。提出presto框架，在想象的动作空间内迭代优化物体位置和尺度，采用粗到细搜索策略确保快速收敛。评估了两种决策变体：度量引导选择(Metric-guided Selection)和MLLM作为评判者(MLLM-as-a-judge)。

Result: 在多个基准数据集上取得最优性能，特别是在此前未见过的开放世界设置中。人类研究表明MLLM-as-a-judge变体比度量驱动方法产生更感知一致的放置，揭示了标准评估度量与人类视觉判断之间的差距。

Conclusion: presto框架成功实现了零样本、训练自由的开放世界物体放置，MLLM-as-a-judge变体在感知一致性上优于传统度量驱动方法，强调了人类视觉判断在评估图像合成质量中的重要性。
...
TL;DR: 研究评估Vision Transformers (ViTs)用于在临床诊断前1-2年预测肺癌。分析了来自波士顿VA医院的259,361张胸部X光片，数据集存在严重的类别不平衡（约1:150）。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，早期诊断对提高生存率至关重要。然而，早期恶性肿瘤在胸部X光片上表现细微，给放射科医生带来挑战。

Method: 分析了91,020项影像研究中的259,361张胸部X光片。采用混合欠采样和过采样方法及类别加权损失优化来解决极端类别不平衡问题。评估了三种ViT配置：从头训练模型、ImageNet预训练模型和Corona预训练模型。

Result: 迁移学习提升了性能，预训练模型在AUC上比baseline高6-10个百分点，在平衡准确率上高约10-12%。ImageNet预训练模型表现最稳定，Corona预训练模型在某些设置下达到更高灵敏度但变异性更大。中等重采样比例（如1:1欠采样和1.5:2过采样）提供了灵敏度、精度和计算效率的良好平衡，运行时减少达70%。

Conclusion: ViTs在从常规胸部X光片进行早期肺癌风险预测方面展现潜力。尽管性能仍低于临床部署阈值，但结果支持进一步开发基于ViT的分诊系统，以标记高风险患者进行早期评估。
...
TL;DR: NC-GRPO通过在潜在空间注入校准高斯噪声来增强视觉语言模型的推理能力，在几何推理任务上显著优于vanilla GRPO和图像空间噪声方法，同时改善了领域外泛化、领域内准确率和幻觉鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过解码温度或像素空间图像扭曲来多样化展开轨迹，本文探索在模型潜在空间进行扰动是否更有效。

Method: NC-GRPO将校准高斯噪声注入提示编码通道的最后一层隐藏状态，对每组展开的一半进行扰动，使这些展开从位移的出发状态分叉。成功到达答案的分支相比被扰动偏离的分支获得强化奖励。

Result: 在Qwen2.5-VL-7B上使用Geometry3K训练，NC-GRPO在五个领域外基准测试中显著优于vanilla GRPO（ pooled McNemar p ≤ 0.001），同时提升了领域内准确率和幻觉鲁棒性。机制消融表明独立随机多样性是有效成分，而非噪声预算或方向。

Conclusion: 独立随机多样性是关键因素；噪声规模研究揭示了推理特化与通用能力之间的权衡。NC-GRPO具有模态无关性，可作为约50行代码变更集成到标准RLVR流程中。
...
TL;DR: 本文提出SemanticSlots方法，解决视频目标中心学习(OCL)中解码器的瓶颈问题。传统解码器强制槽位空间锚定，阻碍运动适应能力。SemanticSlots采用Transformer解码器，利用图像上下文，使槽位作为语义查询而非记忆坐标，实现单帧槽位分解后续视频帧。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为视频OCL的瓶颈在编码器架构，但作者认为真正瓶颈在解码器。传统解码器强制槽位空间锚定，妨碍其适应运动的能力。

Method: 提出SemanticSlots：基于Transformer的解码器，利用图像上下文，使槽位作为语义查询（位置不变），通过检索匹配特征而非记忆坐标。允许单帧计算的槽位分解后续视频帧，消除复杂时间预测器或辅助时间损失的需求。

Result: 在YouTube-VIS数据集上：比VideoSAUR提高31个mBO百分点；比当前最先进方法提高21个百分点；达到86.6% ARI和62.8% mBO。

Conclusion: 解码器是视频OCL的主要瓶颈。SemanticSlots通过语义查询实现最先进性能，无需复杂时间预测器即可实现跨帧目标分解。
...
TL;DR: SketchFlow是一个基于最优传输(OT)理论和流匹配的新型生成框架,用于从文本生成具有人类绘画风格的高质量矢量素描。通过利用预训练CLIP模型绕过劳动密集型的图像级文本注释,将跨模态对齐问题转化为CLIP潜在空间中的连续映射问题。


<details>
  <summary>Details</summary>
Motivation: 矢量素描是人类表达最简洁直接的媒介之一,但生成具有人类绘画风格的高质量矢量笔触仍面临开放挑战,原因在于细粒度、高质量的文本-素描配对数据严重稀缺。现有文本条件生成方法通常依赖不稳定、耗时的优化,或难以以零样本方式泛化到未见过的类别。

Method: 1. 利用预训练CLIP模型绕过劳动密集型的图像级文本注释
2. 将跨模态对齐表述为CLIP潜在空间中的连续映射问题
3. 首先向离散类别嵌入注入噪声,构建连续高斯混合模型(GMM)先验
4. 使用最优传输条件流匹配(OT-CFM)模型学习从连续GMM先验到目标素描特征分布的确定性向量场
5. 设计混合扩散解码器(融合1D U-Net和Transformer架构)将特征解码为快速、高保真的笔触轨迹

Result: SketchFlow在视觉质量和自然人类绘画风格遵循方面显著优于现有基线。该几何保持框架对QuickDraw训练词汇之外(包括未见过的概念标签和语义修饰符)的提示展示了有前景的局部零样本合成能力,同时能够实现不同概念之间平滑、连续的语义插值。

Conclusion: SketchFlow通过结合最优传输理论、流匹配和混合扩散解码器,成功实现了从文本到具有人类绘画风格的高质量矢量素描的生成,在视觉质量、风格遵循和零样本泛化能力方面均取得了显著进步。
...
TL;DR: 本文提出了一种基于条件变分自编码器(CVAE)的方法，用于从人类面部表情数据中合成可控制的不同强度的情感表情。该方法使用包含六种基本情绪(两种强度级别)的数据集进行训练,即使在有限数据量(仅7,680个样本)的情况下,也能学习有意义的潜在表示并生成连贯的情感变化。


<details>
  <summary>Details</summary>
Motivation: 在情感计算和角色动画中,为虚拟人类生成富有表现力的面部行为仍然是一个核心挑战。现有方法通常需要演员表演或手动艺术干预,缺乏对情感强度的控制能力,且需要大量训练数据。

Method: 使用条件变分自编码器(CVAE),基于包含六种基本情绪(低强度和高强度两个级别)的数据集进行训练,以生成合成面部表情数据,同时保持与真实人类表达的语义一致性。

Result: 模型成功保留了不同强度级别相关的关键表达特征,并支持情感强度水平的泛化,能够从相对较小的数据集中创建情感丰富的虚拟角色。

Conclusion: CVAE方法能够有效保留数据集中不同强度级别的特征,同时支持跨情感强度水平的泛化,为动画虚拟角色提供了无需演员表演或手动艺术干预的可控方案,且在有限数据条件下仍具有可行性。
...
TL;DR: StereoDiffuer是一个基于迭代扩散的立体匹配框架，通过引入显著注意力感知(SAP)模块提取几何细节线索，并利用置信度引导的特征结合初始视差估计来条件化迭代去噪扩散过程，从而校正残差视差错误并恢复几何细节。


<details>
  <summary>Details</summary>
Motivation: 现有立体匹配方法在保留细粒度几何细节方面仍存在困难，导致边缘模糊和预测过度平滑，特别是在具有挑战性的区域。

Method: 提出StereoDiffuer框架，包含两个核心组件：1)显著注意力感知(SAP)模块，用于提取显著几何线索包括对象边界、细结构和锐利边缘；2)置信度引导的SAP特征与初始视差估计结合，条件化迭代去噪扩散过程。

Result: 在Scene Flow和KITTI基准数据集上的实验结果证明了所提框架的有效性及其与对比立体匹配方法的竞争力。

Conclusion: StereoDiffuer能够有效校正残差视差错误并恢复在成本体积正则化和上采样过程中被抑制的几何细节。
...
TL;DR: 研究推理增强的文本到图像模型（如GoT-R1），通过编辑生成的文本计划来修复图像生成错误，发现解码器忠实度高（94%），但规划器存在瓶颈，特别是语言措辞依赖的偏差和光栅顺序偏见。


<details>
  <summary>Details</summary>
Motivation: 解决组合提示下模型失败时的问题定位：失败源于错误的计划还是解码器不忠实？通过可编辑的计划来分离这两个环节。

Method: 使用基于检测器的精度和VQA空间度量进行评估，进行五位评分者的人类研究，隔离变量控制实验，以及符号验证和计划编辑实验。

Result: 解码器忠实度高（94%生成布局实现计划关系），但规划器存在严重偏差（"left"准确率98% vs "right"仅54%）。通过编辑计划可显著提升图像质量：完全替换计划提升+13.3分。

Conclusion: 模块化规划器-解码器设计可行，但需确保计划内部一致性；计划的几何信息关键，文本风格和概率不影响结果。
...
TL;DR: 本文提出SHIP方法，解决验证器引导的文本到图像系统中测试时搜索带来的校准不匹配问题。核心贡献是将候选级风险控制扩展到策略级输出分布校准，通过在保留提示上重放完整部署策略来选择满足风险预算的最宽松阈值。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像系统的发布阈值通常在单个图像层面进行校准，但测试时搜索会改变哪些提示获得输出以及释放哪个候选图像。这导致候选级风险控制无法保证发布输出的风险得到有效控制，存在"候选-策略校准不匹配"问题。

Method: SHIP方法包括：(1)对保留提示运行或重放完整部署策略；(2)使用独立目标评判器评估实际发布的图像；(3)选择满足预设风险预算的最宽松阈值。对于可重放策略和预定义阈值网格，同时置信度控制提供有限样本有效性。

Result: 在GenEval2上使用FLUX模型(N=16)的实验中，池化候选阈值发布风险为0.310，而SHIP将其降低至0.162。在200个缓存流分割中，固定网格证书无目标穿越。策略级校准在暴露风险、覆盖率和计算量之间的策略依赖权衡的同时，恢复到更低风险的运行点。

Conclusion: 可靠的推理时扩展需要校准由完整部署策略诱导的输出分布，而非仅校准单个候选图像的风险。当前方法通过策略级校准有效解决了测试时搜索带来的分布偏移问题，为验证器引导系统的实际部署提供了理论保证。
...
TL;DR: 本文首次将数据剪枝方法应用于遥感变化检测任务，基准测试了6种代表性DP方法，发现现有方法相比随机选择没有可靠优势。通过540个随机样本子集的回归分析，揭示了"变化分布保真度"是决定变化检测数据子集质量的最重要因素，并据此提出了FDC两阶段剪枝方法。


<details>
  <summary>Details</summary>
Motivation: 数据剪枝在分类和分割任务中成功减少了训练数据量并提升了模型性能，但其在遥感变化检测领域的潜力尚未被探索。本文旨在填补这一空白，研究现有DP方法在变化检测任务中的有效性。

Method: 1.基准测试：在建筑和森林变化数据集上，针对CNN和Transformer模型，测试6种代表性DP方法在三种剪枝预算下的表现；2.回归分析：构建四个描述子（标签统计、图像多样性、特征空间几何），对540个随机样本子集进行系统回归分析；3.提出FDC方法：基于保真度-多样性-一致性两阶段剪枝框架。

Result: 1.现有DP基线方法相比随机选择没有可靠优势，最强的Feature Diversity基线也仅与约33%的随机采样子集相当；2.变化分布保真度是影响数据子集质量的最重要因素，这在现有剪枝文献中未被关注；3.像素级图像多样性和标签-特征一致性是次要因素；4.FDC方法在变化检测基准和骨干网络上持续改进，尤其在较低剪枝比率下表现优异。

Conclusion: 现有数据剪枝方法不适用于遥感变化检测任务，因为该任务需要关注变化分布的保真度而非传统DP方法强调的特征多样性。基于此发现提出的FDC方法通过两阶段框架有效提升了剪枝效果，为遥感变化检测数据剪枝提供了新思路。
...
TL;DR: GapSight通过自监督信号让视觉语言模型学习选择性重读：当问题需要局部证据时，模型先全局看再返回自由形式区域进行裁剪查看。在InternVL2.5-8B上将六个基准的平均分从52.25提升至64.29，超越CropVLM、ViCrop和ZoomRefine。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在细节问题上失败的根本原因是答案在图像中可见，但在图像被压缩成低分辨率全局视图后丢失。现有方法虽能为每个查询分配更多视觉 token 改善部分 OCR 和文档任务，但计算分配不分青红皂白，且可能干扰依赖全局上下文的任务。

Method: 提出 GapSight 框架学习视觉重读：离线阶段比较全局视图与候选裁剪增强视图下的答案损失或多选选项边际；能改善目标答案的裁剪成为模型特定的审查标签。训练一个轻量级自由形式裁剪路由器，将这些标签提炼为一次性推理策略，从全局状态预测是否审查、预期效用和连续裁剪框。在 LLaVA-1.5-7B、InternVL2.5-8B 和 Qwen2-VL-2B-Instruct 上进行测试。

Result: 在六个基准（OCR、文档、图表、信息图、VStarBench、MME-RealWorld-Lite）上均超越 Base no-zoom 基线。在 InternVL2.5-8B 上，六个基准平均分从 52.25 提升至 64.29，超越 CropVLM (57.16)、ViCrop (55.84) 和 ZoomRefine (54.43)。机制分析表明路由器能挽救具体错误答案、按任务调整行动率，形成有利的 token-性能曲线。

Conclusion: 损失差距监督是教视觉语言模型何时以及何处再次查看的实用路径。
...
TL;DR: 本文提出了一种紧凑且可配置的事件驱动自编码器，用于高效压缩神经形态数据，同时保留关键的时空结构以支持下游推理。该模型在SEFD和EBCD数据集上实现了与YOLOv9相当或更优的准确率，但参数量减少高达35.6倍。在Jetson Nano上达到44.8 FPS的实时吞吐量，在树莓派4B上的能耗比YOLOv9低约726.3倍。


<details>
  <summary>Details</summary>
Motivation: 事件视觉作为一种有前景的节能人工智能范式，提供稀疏、低延迟的视觉信号，可减少冗余数据处理并支持可持续边缘计算。然而，事件流的异步和噪声特性给传统深度学习模型带来挑战，这些模型通常计算量过大，不适合低功耗嵌入式平台。

Method: 该架构集成了轻量级卷积编码、自适应事件阈值下的鲁棒性能以及极简分类器头，实现了显著降低计算成本同时保持识别精度的目标。

Result: 在SEFD和EBCD数据集上的广泛评估表明，该框架实现了与YOLOv9相当或更优的准确率，同时参数量减少高达35.6倍。在NVIDIA Jetson Nano上实现44.8 FPS的实时吞吐量。在树莓派4B CPU上，50%自编码器分类器在推理工作负载下消耗16.19 J能量，比YOLOv9低约726.3倍。

Conclusion: 紧凑的事件驱动模型具有推动环保、低功耗AI系统的潜力，适用于自主移动和嵌入式计算环境中的高速感知应用。
...
TL;DR: SpatialDiff是一种通过隐式3D空间建模和全局空间监督来实现复杂场景中精确、一致物体移动的图像编辑方法。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑方法在处理复杂场景中的空间移动时存在局限：它们主要依赖2D数据集的先验信息，强调平面特征而缺乏对空间结构的支撑，即使使用显式位置信息的方法也无法捕捉真实的3D空间关系，导致物体移动不准确。

Method: 提出两个核心创新：(1)隐式3D空间建模——引入3D先验知识，使模型内部构建全面的三维空间结构理解；(2)全局空间监督——约束潜在空间特征，使模型能够感知编辑操作引起的物体空间位置变化。

Result: 实验结果表明，该方法显著提高了复杂场景中空间移动的准确性和保真度。

Conclusion: SpatialDiff能够有效捕捉3D空间结构，实现复杂场景中精确且一致的物体移动，为图像编辑中的空间操作提供了新的解决方案。
...
TL;DR: 本文提出语义默认偏移（Semantic Default Shift）问题，即加速的文本到图像模型在保持个体输出合理的情况下，可能改变未指定属性的分布。作者引入DefaultShift方法，通过封闭语义词汇表标记重复样本，测量概率质量移动，并在14个参考-替换模型对上进行测试。同时提出DefaultShift-Select离线校准方法，可在不损失质量的情况下减少10.3%至35.1%的人类测量偏移。


<details>
  <summary>Details</summary>
Motivation: 少步文本到图像模型逐渐取代较慢的生成器，但加速可能在未指定属性上悄然改变分布，即使个体输出仍然合理且一致。现有的质量、偏好和多样性评估无法测试替换模型是否保留参考模型的语义默认。

Method: 引入DefaultShift方法：1）使用封闭语义词汇表标记重复样本；2）测量概率质量移动；3）将可解释的排名与确认性交叉拟合推理分离。在14个参考-替换模型对上进行测试。同时提出DefaultShift-Select离线校准方法，用于选择最佳替换模型。

Result: 调整后的颜色差异范围为0.054至0.303，具有特定方向。1000张图像的人工审核重现了排序。在Turbo、DMD2和FLUX上，DefaultShift-Select将人类测量偏移减少10.3%至35.1%，且无实质性质量损失。在平衡评估下，选定数据相比未校准替换数据恢复4.3个准确点 和7.5个最差组点。

Conclusion: DefaultShift使加速下的语义保留变得可测量和可操作。该方法能够有效评估和减少语义默认偏移，为文本到图像模型的加速替换提供了可靠的评估框架。
...
TL;DR: HP-UniIF是一个统一的视觉框架，利用扩散先验来桥接异构融合、视觉恢复和下游感知。该框架通过深度分层条件调制策略解决扩散模型在单一管道中适应域、退化及任务级别目标的局限性。


<details>
  <summary>Details</summary>
Motivation: 现实应用需要单一系统同时支持异构融合、退化恢复和任务导向感知，但现有统一框架难以处理这些正交目标，导致表示纠缠和性能下降。

Method: 提出深度分层条件调制策略，包含三个关键组件：1) 瓶颈层的任务提示调制使主干适应不同融合范式；2) 浅层的退化提示路由器注入退化感知约束用于局部恢复；3) 解码阶段的应用提示库将生成与下游任务对齐。

Result: 在多个融合任务、多样化退化和各种下游应用上的广泛实验表明HP-UniIF具有优越性能。

Conclusion: 分层设计使HP-UniIF能够产生视觉上真实的结果，同时保留任务相关语义。
...
TL;DR: SAFE-G是一个用于知识驱动的视觉问答(KB-VQA)的新型框架，通过结构感知的检索和基于强化学习的忠实推理来提高答案准确性和证据定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有KB-VQA方法存在两个主要问题：1) 难以捕捉复杂上下文中的结构关联以有效过滤噪声；2) 无法确保推理过程严格忠实于检索到的证据。

Method: 提出SAFE-G框架，包含三个核心组件：1) 粗粒度混合搜索融合视觉和文本模态以召回候选文档；2) 结构感知的细粒度图检索捕捉结构依赖以过滤噪声并定位精确证据；3) 强化学习策略配合证据基础奖励，只有在证据正确时才给予正确答案credit。

Result: 在Encyclopedic-VQA和InfoSeek基准数据集上，SAFE-G分别超越了先前方法8.9%和3.5%，显著提升了整体推理准确性。

Conclusion: SAFE-G通过结构感知检索和忠实推理机制有效解决了KB-VQA中的证据定位和可信推理问题，为多模态大语言模型的知识问答提供了新的解决方案。
...
TL;DR: FlashReg是一种面向GPU的点云配准方法，通过构建容量受限的稀疏二阶图和优化3-clique搜索，避免了密集评分二阶图的物化，实现了比TurboReg快2-3倍的对应关系到姿态估计延迟，同时内存占用减少约50%。


<details>
  <summary>Details</summary>
Motivation: 基于图形的点云配准通过识别几何一致的对应集实现了高鲁棒性，但构建二阶兼容性图和枚举候选团仍然是计算和内存密集型操作。

Method: 1) 快速一二阶图(FFSOG)构建：从二进制一阶图直接构建容量受限的稀疏二阶图；2) 数据流优化的3-clique搜索：从紧凑的每行候选池中选择支点，通过排序的稀疏邻域交集枚举三元组。

Result: 在室内外基准测试中，FlashReg在可比较的配准召回率下将对应关系到姿态的延迟降低2-3倍，同时在嵌入式GPU上仅使用约50%的峰值张量内存。

Conclusion: FlashReg适合作为机载感知流水线中的高吞吐量配准后端。
...
TL;DR: 首次提出法医组织病理学自溶恢复任务，将严重自溶的尸检图像转换为诊断有意义的"生前"表现。构建了AutoPath数据集（含近万张10×图像），采用薛定谔桥框架解决无配对监督问题，并提出基于诊断分布一致性的评估方法。


<details>
  <summary>Details</summary>
Motivation: 法医组织病理学对确定死因至关重要，但死后自溶导致组织形态不可逆的随机降解，造成诊断主观性。现有方法无法处理无像素级配对数据的挑战，亟需将自溶图像恢复到生前状态以提高诊断客观性。

Method: 1)构建AutoPath同源未配对数据集：利用相邻组织块分别立即处理与暴露诱导自溶；2)将问题形式化为薛定谔桥问题，建模随机严重形态退化；3)提出幻灯片级诊断分布一致性评估，替代通用图像生成指标。

Result: 建立了可复现的基准框架，包括任务定义、真实数据集和评估方法。为法医病理学自溶恢复的严格和实际进展奠定基础。

Conclusion: 本研究首次系统定义了法医组织病理学自溶恢复任务，通过薛定谔桥框架和创新的评估方法，为解决自溶导致的诊断困难提供了新范式，具有重要的法医病理实践价值。
...
TL;DR: PatchGate是一个无需训练的视觉语言模型可靠图像描述框架，通过从冻结VLM的解码器层中提取图像内在的对象证据，在生成过程中校准logits以同时提高对象覆盖率和减少对象幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free方法主要解决对象幻觉问题（抑制不支持的对象词），但对于遗漏可见对象的问题难以处理，因为它们只在模型已经可能提及的对象上操作。需要一种方法在生成前提取图像内在的对象证据。

Method: 提出两阶段框架：1) 视觉证据提取(VEX)：从LM解码器后半层读取patch级词汇证据，构建无任务提示的图像条件对象集；2) 视觉证据包含-排除解码(VIED)：使用对象证据校准解码logits，提升有证据支持但被低估的对象，抑制弱支持但被高估的对象。

Result: 在AMBER数据集上，PatchGate将可见对象覆盖率从49.4%提升至56.0%（+13.4%），同时通过降低CHAIR从7.5至6.6减少对象幻觉12.0%，无需外部检测器或微调，仅需一次额外前向传播。

Conclusion: PatchGate成功实现了对象级可靠性两个维度的同时提升，证明了从冻结VLM中提取内在对象证据并用于校准解码的有效性，为训练-free VLM图像描述可靠性研究提供了新思路。
...
TL;DR: 针对动态场景的新视图合成问题，提出了一种专为4D表示设计的运动感知3D平滑滤波器，通过基于局部运动信息自适应调整滤波器强度来有效减轻混叠伪影，同时不损害渲染质量。


<details>
  <summary>Details</summary>
Motivation: 动态场景的新视图合成在AR/VR应用中至关重要，但现有4D表示方法在生成分歧视角（缩放操作）的新视图时仍存在混叠问题，传统的3D平滑滤波器（如Mip-Splatting）无法处理局部运动且存在混叠。

Method: 提出运动感知3D平滑滤波器，通过非参数估计方法估计时间和焦距-深度比的联合密度函数，在推理时从该联合分布采样以确定适当的平滑滤波器强度，该策略可与各种4D表示集成。

Result: 在标准数据集上的评估表明，所提方法优于现有最先进方法。

Conclusion: 该方法通过自适应运动信息的灵活策略有效解决了4D表示中的混叠问题，具有良好的通用性和优越的性能。
...
TL;DR: 本文提出BLMSP框架，通过比特流语言建模学习比特流原生语义线索，并将其作为先验注入到视频恢复、字幕和人体姿态估计任务中，解决比特流损坏视频的视觉理解问题。


<details>
  <summary>Details</summary>
Motivation: 比特流损坏导致视频出现像素失真和语义丢失，现有视觉模型难以处理这类退化严重的视频。BcHVU问题的病态性质对现有模型构成重大挑战。

Method: 提出VBBM视频比特流字节模型，集成字节级建模和跨编码器语义蒸馏，从多种损坏比特流格式的字节序列中提取鲁棒语义。将学习到的比特流语义作为先验融合到BcHVU模型骨干中。构建了包含607k损坏比特流片段和287k配对恶劣视频的大规模CHP数据集。

Result: 视频恢复PSNR提升2.51dB，字幕生成CIDEr提升0.20，人体姿态估计PCK@0.2提升0.18。

Conclusion: 损坏的比特流可作为鲁棒语义先验，有效解决BcHVU任务中的像素失真和语义丢失问题，为恶劣环境下视频理解提供了新思路。
...
TL;DR: FIRM-Video是一个统一的多维度视频评估框架，采用"先验证后评分"原则，通过维度特定的检查清单、时序视觉证据验证和聚合验证决策来构建训练数据。该框架针对指令遵循、世界连贯性和感知质量三个维度分别设计评估方法，并构建了包含88,044个实例的数据集FIRM-Video-90K和750个人工标注的基准测试FIRM-Video-Bench。实验表明，基于Qwen3-VL的FIRM-Video-8B模型在MAE指标上达到最优，并在VBench各项评分中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 现有视频评估方法存在以下问题：1) 整体评判器使用固定评分标准导致检查不全面；2) 开放式推理产生不忠实论证；3) 评分归因混乱。同时面临评估准确性与推理效率之间的权衡困境。

Method: 提出FIRM-Video框架，包含三个核心组件：1) 指令遵循：将提示词分解为加权原子需求；2) 世界连贯性：构建基于可见实体和动作的提示校准目标检查；3) 感知质量：应用通用视觉缺陷分类体系。验证后的标准和分数转换为自然语言分析用于端到端奖励建模。

Result: FIRM-Video-8B在FIRM-Video-Bench上实现最佳MAE，并在三个视频生成器的Best-of-8采样中持续获得最高VBench总分、质量分和语义分。

Conclusion: 基于检查清单驱动的数据构建框架能有效提升视频评估的准确性和可解释性，为text-to-video对齐提供了高质量的训练监督信号。
...
TL;DR: SA-RF通过可学习的Box-Cox极坐标颜色空间和混合自适应模块，在保持图像结构的同时有效增强低光图像，解决曝光校正与结构保持之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 低光图像增强需要在不破坏已有结构的前提下修正模糊的曝光。现有的生成式传输方法虽能建模曝光不确定性，但会改变可观测的几何和色度内容。此外，固定的可逆颜色坐标通常仅作为表示使用，而未考虑其逆映射对RGB域梯度的重塑作用。

Method: 提出结构锚定Rectified Flow (SA-RF)：1) 分离的色度/强度分支保持对应关系；2) 尺度匹配的条件金字塔；3) HybridAda（空间交叉注意力进行位置特定检索，池化AdaLN进行全局曝光调制）；4) BC-IHV可学习Box-Cox极坐标颜色空间，其解析可逆的强度映射通过单一指数控制逆梯度动态范围，平衡暗区扩展和梯度条件化。

Result: 在三个LOL基准测试、盲图像质量评估和跨数据集测试中表现优异，重建一致性和感知质量均优于当前最先进方法。控制实验验证了框架和颜色表示的有效性。

Conclusion: SA-RF通过创新的颜色表示和网络设计，有效解决了低光图像增强中结构保持与曝光校正的矛盾，为该领域提供了新的技术路径。
...
TL;DR: 本文提出一种新颖的3D感知视频恢复框架，用于提升稀疏视角下3D Gaussian Splatting重建质量。通过构建大规模3DGS视频数据集并进行针对性微调，同时引入边界锚定的相机感知先验，将多相机几何约束注入视频生成 pipeline。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) 在新视角合成中取得显著成功，但稀疏视角下的重建结果常出现明显伪影。尽管近期视频扩散模型提供了强大的时空先验，但直接用于3DGS恢复效果欠佳，因为这些模型缺乏对底层多相机几何结构的感知，导致多视角不一致性问题。

Method: 1) 构建大规模3DGS视频数据集以支持专项微调；2) 引入边界锚定的相机感知几何先验，以首帧和末帧作为边界锚点，编码对应相机关系，将空间结构显式注入视频生成流程。

Result: 在视频先验恢复方法中取得最佳像素级和结构级保真度(PSNR/SSIM)，显著提升多视角一致性，感知质量(LPIPS)也具有竞争力。

Conclusion: 所提框架通过相机条件几何先验有效弥合了2D视频生成与3D多视角约束之间的鸿沟，实现了几何一致且多视角连贯的3DGS重建恢复。
...
TL;DR: 当前弱监督视频异常检测的评估指标（Micro-AUROC/AP）存在严重问题：检测器可以通过简单地分离不同视频来获得高分，而不需要准确识别视频内的异常时刻。论文证明了即使给每个视频的所有帧分配相同的分数，也能达到81-97%的Micro-AUROC，这揭示了评估指标的本质缺陷。


<details>
  <summary>Details</summary>
Motivation: 弱监督视频异常检测器使用视频级标签训练，但用帧级别的Micro-AUROC或AP评估。作者发现这些指标比较不同视频的帧，导致检测器可以通过分离不同视频获得高分，而不需要准确排序视频内的异常时刻。这促使作者研究评估指标的有效性。

Method: 1. 将Micro-AUROC分解为Within-AUROC（视频内时间排序）和Cross-AUROC（跨视频比较）；2. 在三个数据集上计算同一视频内异常-正常帧对的比例；3. 训练视频级二分类器并将视频分数重复到所有帧，测试其性能；4. 进行72次控制运行实验验证。

Result: 1. 在ShanghaiTech、XD-Violence和UCF-Crime数据集上，只有0.071-0.388%的比较发生在同一视频内；2. 视频常数输出（无视频内变化）达到81.40-97.18 Micro-AUROC；3. 用视频均值替换帧分数保留中位数98.6%的AUROC边际；4. 作者发布的输出和XD-Violence的AP评估也呈现相同模式。

Conclusion: 当前评估指标存在"时间稀释"问题，允许检测器在不对视频内时刻进行任何排序的情况下获得高分。评估方法需要改革，建议使用视频内评估或改进的指标来真正衡量时间定位能力。
...
TL;DR: GuardPaint是一种针对文本到图像(T2I)扩散模型的安全生成框架,通过在去噪轨迹内部进行干预来防御对抗性提示攻击,而无需修改基础模型。该框架使用轻量级审计器监控中间图像,定位不安全区域,并仅在需要时触发外科修复式inpainting,通过守卫锦标赛选择修复方案。实验表明该方法在多种越狱攻击和扩散模型上显著降低攻击成功率,同时保持图像质量和提示保真度。


<details>
  <summary>Details</summary>
Motivation: 当前的T2I扩散模型安全防护主要在生成前通过提示过滤或生成后通过图像分类进行,存在两个主要问题:1)扩散过程本身缺乏防护,2)现有方法通常只是简单拒绝生成而非安全修复。 adversarial prompts可以操纵去噪轨迹生成违规内容(如明确裸体或 graphic violence),需要一个能在生成过程中实时干预的解决方案。

Method: GuardPaint采用 speculative decoding框架:1)轻量级审计器监控中间图像,定位不安全区域;2)触发策略对齐的inpainter进行外科修复;3)通过守卫锦标赛选择修复方案,仅在提升策略合规性同时保持提示保真度和感知质量时接受编辑。方法不修改基础模型,通过后处理方式实现安全干预。

Result: 在5个越狱攻击家族(SneakPrompt, MMA, PGJ, DACA, RABell)和多种UNet/flow-matching模型(SD 1.5, SDXL, SD 3.5, FLUX.1-dev)上,GuardPaint显著降低攻击成功率和有害生成,同时对图像质量、提示保真度和良性行为的影响最小。

Conclusion: GuardPaint提供了一种在扩散轨迹内部进行安全干预的有效范式,无需修改基础模型即可实现 surgical inpainting repair。该框架在多种攻击和模型上展现有效性,为T2I模型安全提供了新思路,同时平衡了安全合规与生成质量。
...
TL;DR: 本文提出ViSMoE框架，通过稀疏专家混合系统和视觉感知路由策略处理具身智能体在不同视图中的视觉信息，以解决具身引用表达定位任务中视图和对象表示模糊的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法区分视图和对象，使用普通视觉编码器导致两者的表示模糊，这影响了智能体在真实环境中根据自然语言指令定位目标对象的能力。

Method: 提出ViSMoE框架，将稀疏专家混合系统与视觉感知路由策略相结合，使智能体能够针对不同类型的视觉信息（视图和对象）使用专门的专家处理，从而生成更具判别性的视觉表示。

Result: 在REVERIE和SOON数据集上的实验表明，ViSMoE优于先前最先进的方法。

Conclusion: 通过视觉感知的路由策略专门处理不同类型的视觉信息，可以有效提升具身引用表达定位任务的表现，生成的判别性视觉表示对导航和目标定位至关重要。
...
TL;DR: 本文介绍了BIC-MAC挑战赛的方法，该挑战赛要求从非衰减校正PET、DIXON MRI和topogram合成伪CT，并评估伪CT和由其重建的衰减校正PET。作者提出三个改进：1) 使用Carney attenuation-coefficient (μ)空间的L1损失函数，按解剖区域加权；2) 将未配准的DIXON MRI作为额外输入通道；3) 使用两个独立训练模型的固定凸组合。该方法在公开验证排行榜上排名第一。


<details>
  <summary>Details</summary>
Motivation: BIC-MAC挑战赛需要从NAC-PET、DIXON MRI和topogram合成伪CT，用于衰减校正PET重建。现有方法（如3D U-Net基线）效果不佳，需要改进。

Method: 1. 损失函数改进：在Carney attenuation-coefficient (μ)空间计算L1误差，按解剖区域加权；2. 输入改进：加入未配准的DIXON MRI作为额外输入通道；3. 模型集成：固定凸组合两个独立训练模型。

Result: 该方法在三个指标上超越两个独立模型，在公开验证排行榜上排名第一。

Conclusion: 损失函数的选择比架构本身更重要；在损失函数改进后，未配准的DIXON MRI才能作为有效输入；模型集成能进一步提升性能。
...
TL;DR: VIG（视觉信息增益）是一种信息论GRPO奖励机制，通过比较模型带图和不带图两次前向传播的预测不确定性差异来评估每个推理token的视觉信息密度，从而在不依赖外部注释或奖励模型的情况下，实现多模态大语言模型的高效推理。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大推理模型依赖冗长的思维链（CoT），其中大量token（如重复的视觉描述、自我反思等）与图像无关却消耗计算资源。现有CoT压缩方法仅优化输出长度，未评估推理token是否真正基于图像。

Method: 提出VIG（Visual Information Gain）：对同一策略进行两次前向传播（分别输入图像和不输入图像），计算图像对预测不确定性的减少量，以此作为每个推理token的奖励分数。该方法无需参考链、外部标注或辅助奖励模型。

Result: 在6个多模态推理基准和3个Qwen3-VL-Thinking模型规模（2B/4B/8B）上，以及R1-Onevision-Bench的8B评估中，VIG一致性地改善了准确率-效率权衡。

Conclusion: 高效的多模态推理源于提升视觉信息密度（每个推理token锚定图像），而非施加长度预算。这一方法为多模态推理的效率优化提供了新范式。
...
TL;DR: 本文提出观察算子扩散模型(OOD)和GL-CoDA解码器，通过时间索引的观察轨迹解决像素空间扩散模型中尺度-时间不匹配问题，实现从粗到细的特征细化，在ImageNet-256上达到1.52的FID分数。


<details>
  <summary>Details</summary>
Motivation: 当前像素空间扩散模型虽直接建模图像分布，但优化困难。现有的目标重参数化方法仍要求模型在噪声很高时就预测完整图像，产生低信噪比梯度，阻碍模型优化。核心问题是：图像结构从粗到细逐渐可预测，但现有模型被迫在噪声很大时就预测完整图像，造成尺度-时间不匹配。

Method: 1. 提出观察算子扩散(OOD)框架，用时间索引的观察轨迹替代标准流程中的固定全图像监督，从粗结构逐渐过渡到完整图像；2. 使用一族高斯-兰索斯算子在不同观察尺度实例化该轨迹，获得路径一致的训练目标；3. 引入GL-CoDA解码器，在解码阶段注入尺度特定的高斯-兰索斯观察，实现从粗到细的特征细化。

Result: 收敛速度显著加快，生成质量持续提升。在ImageNet-256上实现FID 1.52。

Conclusion: 通过将监督轨迹和特征细化与图像结构的内在恢复顺序对齐，成功解决了尺度-时间不匹配问题，为像素空间扩散模型的优化提供了有效框架。
...
TL;DR: SLS是一个基于Transformer的自编码器，旨在为SVG（可缩放矢量图形）创建连续、密集且可逆的潜在空间，类似于变分自编码器为光栅图像提供的功能。


<details>
  <summary>Details</summary>
Motivation: 深度学习社区缺乏用于矢量表示的连续、密集且可逆的潜在空间，而这种潜在空间是变分自编码器及其衍生版本为光栅图像提供的关键构建块。

Method: 通过在统一的基于BPE的标记词汇表中建模SVG命令、坐标数据和视觉属性，SLS学习捕获结构和外观的固定大小潜在表示，并可解码回有效、风格一致的SVG路径。

Result: 学习到的嵌入空间位于单位超球面上，实现了高效的相似性搜索、组合和通过简单向量空间操作进行下游调节。与基于标记的方法相比，SLS将FLOPs降低了150多倍。

Conclusion: SLS为矢量图形研究建立了一个通用的潜在基础，能够跨不同任务进行泛化。
...
TL;DR: MMDental Task 3要求从仅50个标注的CBCT病例中完成七字段牙科记录补全，提出ECCR框架通过分离证据可用性与证据权威性来控制多模态证据的修改权限，在极端低资源环境下取得了较好的效果。


<details>
  <summary>Details</summary>
Motivation: 在标注数据稀缺的情况下，从CBCT完成牙科记录补全困难，且个体临床领域需要不同类型的证据支持。视觉上合理的检索记录可能引入不支持的实体，从而造成危害。

Method: 提出ECCR（实体约束的CBCT引导检索）框架，包含：1）语料库衍生的先验提供完整记录；2）冻结的3D编码器检索图像条件诊断证据；3）仅当不扩展先验FDI或ICD实体集时才附加证据，确保实体集不变。

Result: 公开验证集上达到加权分数0.3134，优于全记录多模态检索(0.2237)和静态文本先验(0.2915)；防护机制阻止了63.3%的检索候选。最终测试获得11.37分（满分97.4），获得第二名。

Conclusion: 在极端低资源设置下，控制多模态证据允许修改的内容比转移整个检索记录更可靠。
...
TL;DR: AirAlign是一个仅使用RGB图像的无人机相对位姿对齐框架，利用预训练的视觉几何重建模型作为骨干网络提取几何感知特征，并通过场景分离的交叉验证和模型集成来提高鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代低空环境中无人机导航在最后阶段需要更精确的位姿对齐，但严重的视角和外观变化使得这一任务具有挑战性，"最后一米"导航变得越来越重要。

Method: 1) 使用预训练的视觉几何重建模型作为骨干网络，从源-目标图像对中提取几何感知特征；2) 将训练集分割为多个场景分离的折进行未见过的交叉验证和模型选择；3) 在推理时对选定模型的预测进行平均以形成集成输出。

Result: 在ACMMM 2026无人机多媒体工作组的PairUAV挑战赛上验证了方法的有效性和鲁棒性，消融实验验证了各组件的贡献。

Conclusion: AirAlign框架能够有效实现无人机仅RGB图像的相对位姿对齐，通过几何感知特征提取和模型集成策略提高了在复杂低空环境中的导航精度。
...
TL;DR: C²Path提出了一种用于视觉语言增量目标检测的类条件路径解耦框架，通过类别专家库和类条件解耦模块建立专门的计算路径，实现类特定参数更新和特征注入，解决增量学习中的类知识耦合问题。


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法存在两类类知识耦合问题：共享参数更新导致的类边界侵蚀和混合特征编码导致的类表示纠缠。作者认为有效的增量学习需要类特定的计算路径，实现隔离的参数更新和分离的类注入。

Method: 提出C²Path框架，包含两个核心组件：1) 类别专家库，由可学习的低秩计算节点组成，捕获类别特定知识；2) 类条件解耦模块，生成类感知路由信号，动态组合ClassLoRA适配器，形成类特定计算路径。

Result: 在COCO 2017数据集上多个增量学习设置下的广泛实验表明，C²Path始终优于现有最先进方法。

Conclusion: C²Path为视觉语言检测器中的持续类别扩展提供了有效且可扩展的解决方案。
...
TL;DR: 本文提出Sparse Multi-Stage Expert-Agent Routing框架，通过分阶段路由机制实现LLM临床推理，显著减少专家激活数量至3人，同时保持诊断质量。提出ClinFEScore评估协议用于评估临床推理输出。


<details>
  <summary>Details</summary>
Motivation: 现有LLM临床推理系统通常执行单次预测或依赖固定多智能体工作流程，导致专家参与静态或过于繁琐。临床诊断需要根据新证据更新假设并协调不同专科，但现有方法缺乏动态适应能力。

Method: 提出分阶段路由框架：1)基于逐步获取的多模态临床证据维护演进案例状态；2)自适应激活稀疏专家智能体集合；3)支持跨阶段专家特定记忆。同时提出ClinFEScore评估协议评估自由文本诊断结论。

Result: 在MAC和AgentClinic-NEJM重建多阶段案例上，平均激活专家数从17.0降至3.0同时保持事实级诊断质量。在200个真实MDT案例上，ClinFEScore与临床医生判断高度相关（Spearman ρ=0.81; Pearson r=0.87），方法达到91.5%临床验证诊断准确率，约每案例5次LLM调用。

Conclusion: 稀疏阶段协调是高效且临床相关的LLM临床推理方法。
...
TL;DR: 本文提出了一种结合扩散模型生成合成缺陷样本与贝叶斯分类器的自动化视觉检测系统，旨在解决制造业中缺陷样本稀缺以及决策缺乏置信度估计的问题，通过分阶段检测流水线实现可信赖的自动化检测。


<details>
  <summary>Details</summary>
Motivation: 制造业中缺陷样本稀缺，且现有检测系统仅输出硬标签缺乏置信度估计，导致误判成本不对称（误拒浪费产品，误接受可能让缺陷流入下游）。需要解决数据稀缺和需要置信度感知决策的问题。

Method: 1）利用扩散模型生成合成缺陷样本以缓解数据稀缺；2）采用贝叶斯分类器提供置信度估计，将模糊样本 defer 给人工审核；3）构建分阶段互补检测流水线。

Result: 初步实验结果表明，扩散模型生成的缺陷样本结合不确定性感知分类器，可在数据稀缺条件下降低构建可信赖、可部署检测模型的成本。

Conclusion: 扩散生成的缺陷结合不确定性感知分类器有望解决数据稀缺问题，实现更可信赖的自动化视觉检测系统。
...
TL;DR: 本文提出检索增强视觉提示(RAVP)框架，通过在推理时向SAM 3模型注入外部视觉记忆来实现两光子钙成像的零样本神经元检测和实例分割，无需微调即可实现模型自适应。


<details>
  <summary>Details</summary>
Motivation: 两光子钙成像面临图像外观在不同记录间变化大、标注稀缺、需快速适应等挑战，传统微调方法不够高效，作者探索推理时注入视觉记忆作为参数适应的替代方案。

Method: 将检索转化为视觉提示形式，每个目标图块通过检索到的带标注样本增强，使用样本边界框作为概念提示。设计了荧光引导启发式和轻量级召回预测器两种样本选择策略。

Result: 在Allen Brain Observatory数据集上，样本增强推理持续提升零样本神经元检测和实例分割性能。消融实验表明单一精心选择的样本比多个检索样本更有效。

Conclusion: 推理时视觉记忆注入可作为基础模型在专业生物医学成像中参数适应的简单有效替代方案，为快速适应新成像任务提供了新思路。
...
TL;DR: 本文提出了一种名为Kronecker-DCT (K-DCT) 的新型协方差模型，用于加速去噪扩散概率模型(DDPM)的采样过程。该模型通过Kronecker分解处理颜色间协方差，并利用离散余弦变换(DCT)在频域中建模空间协方差，从而在保持较低计算开销的同时，显著提升了少量采样步骤下的生成质量。


<details>
  <summary>Details</summary>
Motivation: DDPM的采样过程可以通过利用二阶信息（去噪后验协方差的近似）来加速，从而用更少但更大的采样步骤生成可接受的样本质量。然而，先前尝试使用此类信息的方法采用了过于简化的对角协方差近似，未能充分利用自然图像的特殊统计结构——包括像素和颜色通道之间的强非对角相关性，以及衰减缓慢的功率谱频率特性。

Method: 提出Kronecker-DCT (K-DCT)模型：（1）使用Kronecker因式分解处理颜色间协方差；（2）利用离散余弦变换(DCT)在频域中建模空间协方差。DCT的使用将计算复杂度从二次降低到对数线性，使得每个去噪步骤的计算和内存开销可忽略不计。使用预训练的score模型在CIFAR-10、Celeb-A、ImageNet和LSUN数据集上学习K-DCT结构化的去噪后验协方差 amortizations。

Result: 在CIFAR-10、Celeb-A、ImageNet和LSUN数据集上的实验表明，与之前的SOTA去噪采样器相比，K-DCT模型在FID和似然指标上表现更优，尤其是在少量去噪步骤的 regime 下优势更为明显。

Conclusion: K-DCT模型成功捕捉了自然图像的统计特性（颜色通道间的非对角相关性和频率域的功率谱特性），通过在频域中高效建模空间协方差，实现了DDPM采样过程的显著加速，为扩散模型的实用化部署提供了有效途径。
...
TL;DR: 本文提出CSWL框架，通过频率域视角解决无数据知识蒸馏中的生成捷径学习问题，提升生成器的多样性和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有基于生成器的无数据知识蒸馏(DFKD)方法存在过度依赖教师偏好和模式崩溃问题，表现为频率域中的"生成捷径学习"：依赖于特定频率成分和频率位置，导致合成图像质量和类别多样性不一致。

Method: 提出两个关键技术：1) 频率域特征增强，在特征层面进行频率域增强，促使生成器关注完整频率谱，从而抑制捷径学习行为；2) 跨阶段频率重建(CSFR)辅助任务，隐式构建指数移动平均(EMA)机制，促进长期优化和训练稳定性。

Result: 在多个分辨率的各种图像识别数据集上进行了大量实验，包括下游任务，验证了CSWL从频率视角提升多样性和稳定性的有效性。

Conclusion: 从频率域角度出发，通过频率域特征增强和CSFR辅助任务，可以有效解决无数据知识蒸馏中的生成捷径学习问题，显著提升生成器的多样性和训练稳定性。
...
TL;DR: 本文提出了ORBIT，一个用于评估相机位姿估计的新基准测试。核心思想是利用在线全景360°视频生成具有挑战性的视频片段，同时保持可靠的地面真值轨迹恢复。全景视频的丰富视觉上下文有助于跟踪相机运动，即使部分视图受到影响。实验表明，COLMAP以及最新的基于优化和前馈的SfM方法在该基准上难以准确估计相机位姿。


<details>
  <summary>Details</summary>
Motivation: 当前SfM方法在处理复杂视频（具有挑战性的相机运动或动态场景）时常会失败，且该领域缺乏可靠的地面真值基准来评估困难场景下的性能，这使得难以衡量实际进展或确定需要改进的方向。

Method: 利用在线全景360°视频作为数据源，从中选择并生成具有挑战性的视频片段。具体做法是在全360°视频中跟踪相机运动，然后裁剪和重投影选定部分以生成透视视图片段作为基准测试数据。

Result: 实验表明，COLMAP以及近期的基于优化和前馈的SfM方法在ORBIT基准上都无法准确估计相机位姿，显示出当前SfM方法在处理复杂真实世界场景时的局限性。

Conclusion: ORBIT提供了一个有价值的测试平台，使研究人员能够有意义地衡量在真正具有挑战性的真实世界SfM问题上的进展。该基准填补了SfM领域在困难场景评估方面的空白。
...
TL;DR: 本文提出了一种基于子图引导的视图图剪枝框架，用于提高全局运动恢复结构（global SfM）的鲁棒性。该方法通过将视图图划分为局部一致的子图，在子图内进行全局SfM获取可靠相机位姿，然后应用RANSAC进行跨子图边剪枝，最后在精化的视图图上执行全局SfM。


<details>
  <summary>Details</summary>
Motivation: 全局SfM相比增量式SfM具有更好的可扩展性，但其性能对由视觉模糊匹配产生的错误边高度敏感，可能导致相机配准错误和重建伪影。

Method: 1. 将视图图划分为局部一致的子图；2. 在每个子图内执行全局SfM获取可靠相机位姿；3. 应用基于RANSAC的跨子图边剪枝移除不一致边；4. 在精化后的视图图上执行全局SfM。

Result: 在模糊、顺序和无序图像数据集上的广泛实验表明，该方法在挑战性条件下提高了全局SfM的鲁棒性。进一步评估表明，改进的相机估计可带来更高质量的新视图合成结果。

Conclusion: 所提出的子图引导视图图剪枝框架能够有效识别和移除不可靠连接，通过利用可靠子图的内部一致性，显著提升了全局SfM在挑战性条件下的鲁棒性，并有利于后续神经渲染应用。
...
TL;DR: 本文提出了竞争性记忆读取（Competitive Memory Readout）方法，用于解决视频目标分割中相似类别干扰物导致的身份混淆问题。该方法基于SAM3框架，在记忆检索阶段显式引入同类竞争者信息，并采用自适应恢复机制防止弱目标被过度抑制。最终在MOSEv2挑战赛中获得66.20分，排名第二。


<details>
  <summary>Details</summary>
Motivation: 视频目标分割面临复杂时序动态挑战，包括长期遮挡、目标消失与重现、大幅外观变化以及相似类别干扰物的强烈干扰。现有基于SAM3的标准目标-only记忆检索方法仅将干扰物隐式表示为背景，无法有效区分同类非目标物体与真实目标，导致目标身份混淆。

Method: 提出竞争性记忆读取方法（Competitive Memory Readout）：1）基于SAM3框架，聚焦于记忆读取模块；2）在记忆检索时显式引入同类竞争者证据，使系统能够区分目标与同类干扰物；3）设计轻量级自适应恢复规则，防止对弱目标或重现目标的过度抑制；4）保留原始SAM3跟踪流程，仅改进记忆读取部分。

Result: 在MOSEv2挑战赛中取得66.20分（主要评价指标），获得第二名。

Conclusion: 竞争性记忆读取方法有效提升了SAM3在复杂视频场景中保持目标身份一致性的能力，通过显式建模同类竞争者信息解决了相似物体干扰问题，且轻量级自适应恢复机制确保了方法的实用性。该方法为长时视频目标分割提供了一种有效的解决方案。
...
TL;DR: ADMIL是一种选择性计算框架，通过将ABMIL教师的注意力蒸馏到轻量级PriorNet模型中，在病理切片分类任务中实现高效推理。该方法仅需对少量瓦片（如K=4,8,128）进行昂贵的Foundation Model编码，即可达到全模型性能，避免超过98%的计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于病理基础模型的多实例学习(ABMIL)需要对每个前景瓦片进行完整的推理计算，导致计算成本高昂且效率低下。虽然注意力分布通常集中在少量信息区域，但现有方法仍需对全部瓦片进行编码，造成资源浪费。

Method: 提出ADMIL框架，包含两个阶段：1)知识蒸馏阶段：使用EfficientNet架构的PriorNet学习ABMIL教师的注意力分布，损失函数为KL散度；2)推理阶段：PriorNet对瓦片评分并选择top-K个最相关的瓦片，然后仅对这些瓦片调用昂贵的Foundation Model（Virchow2）进行编码，最后通过选定的包进行ABMIL预测。

Result: 在BRACS、PANDA和CAMELYON16数据集上，ADMIL分别在K=4、8和128个瓦片时达到与完整教师模型相当的性能，同时避免了超过98%的Foundation Model瓦片嵌入计算和FLOPs。随机选择和教师注意力Oracle对照实验表明，性能提升依赖于任务相关的选择而非仅仅是瓦片数量减少。

Conclusion: ADMIL证明在不影响切片级性能的情况下可以移除绝大多数昂贵的瓦片编码，为临床环境中考虑延迟和计算成本的高效部署提供了可行路径。PriorNet能够高保真地恢复教师的瓦片排序，并聚焦于任务相关的形态学区域。
...
TL;DR: 本文首次评估了脉冲神经网络(SNN)在前视声纳(FLS)图像中进行目标检测的适用性，提出SpikeYOLO并与CNN基线进行对比，在保持竞争力的检测精度同时显著降低能耗并增强抗噪声能力。


<details>
  <summary>Details</summary>
Motivation: 自主水下航行器(AUV)是电力受限平台，现有的CNN目标检测方法能耗过高(如YOLOv8m达322 mJ/推理)，而FLS图像的稀疏高对比度结构与脉冲编码天然契合，且此前尚无研究评估SNN在此领域的适用性。

Method: 使用脉冲YOLO(SpikeYOLO)——一种基于代理梯度训练的全脉冲网络，在三个FLS目标检测数据集上与YOLOv8m、YOLO-SONAR、Fast R-CNN等CNN基线进行对比实验。

Result: SpikeYOLO T=2在UATD数据集上实现3.3倍更低理论计算能耗(97 vs 322 mJ)，mAP@0.5:0.95为0.529( vs YOLOv8m的0.575)；在Marine-Debris-FLS数据集上mAP@0.5与YOLOv8m持平且超越其他基线，能耗降低4.4倍；对乘性散斑噪声的鲁棒性显著优于YOLOv8m(σ=0.4时退化3.0% vs 8.9%)，在σ=0.6时直接超越YOLOv8m。

Conclusion: SNN是FLS目标检测的可行且有前景的解决方案，在能源效率和对声纳典型噪声的鲁棒性方面具有显著优势，为AUV等嵌入式平台的实际部署提供了更优选择。
...
TL;DR: 研究如何从包含缺陷的工业图像参考库中安全地恢复有用的正常 patch。提出一种 trim 方法：从 DINOv2 patch-memory 出发，用干净种子库评分候选 patch，丢弃最可疑的 20%，合并保留的 patch 并进行贪心核集选择。在 Severstal 数据集上，naive 额外参考包含 9.46% 异常 patch，方法拒绝了 78.1% 的异常 patch，残差污染降至 2.59%。在 51,200-patch 预算下达到 0.1084 AUPRC，显著优于 naive 扩展（0.0950）和随机去除（0.0952）。


<details>
  <summary>Details</summary>
Motivation: Patch-memory 异常检测器假设参考库是正常的，但工业图像难以验证。传统方法认为额外图像总是有益的，但未考虑污染问题。研究意图验证少量可信正常图像能否从受污染的参考中恢复有用的正常 patch。

Method: 1) 基于 DINOv2 patch-memory（AnomalyDINO）初始化；2) 用干净种子库计算候选 patch 距离并评分；3) 丢弃最可疑的 20% patch；4) 将保留的 patch 与种子合并；5) 使用贪心核集选择强制执行固定预算。

Result: Severstal 数据集：naive 额外参考含 9.46% 异常 patch；trim 方法拒绝 78.1% 异常 patch，残差污染降至 2.59%。51,200-patch 预算下：proposed 0.1084 AUPRC vs naive expansion 0.0950 vs random removal 0.0952 vs 8 clean images 0.1030。注入 0.5% 异常 patch 使 AUPRC 从 0.1030 降至 0.0759。5 个 held-out pairs 上平均增益 0.0142 AUPRC。

Conclusion: 参考库纯净度是一阶设计变量；未验证图像仅在明确过滤其贡献时才有用。少量污染即可显著降低检测性能，证明纯净参考库的重要性。
...
TL;DR: 本文提出了Libo Zhang在autoPETV Grand Challenge中的交互式病变分割算法解决方案，通过残差编码器U-Net（约1.4亿参数）和三阶段课程学习训练策略，在4000个epoch内完成训练。该方法在交互式五折交叉验证中达到平均AUC-Dice 3.836和平均AUC-DMM 3.869。


<details>
  <summary>Details</summary>
Motivation: 解决全身PET/CT影像中的交互式病变分割问题，需要处理autoPET Grand Challenge中的交互式 lesion分割任务。

Method: 将交互信息编码为两个额外的输入通道（前景和背景涂鸦光栅化），采用约1.4亿参数的残差编码器U-Net，通过三阶段课程学习训练：1）使用静音交互通道学习完全自动分割；2）观察基于真值生成的涂鸦和随机采样的可见性模式；3）通过在线模拟最多五个错误驱动的纠正步骤来适应自身错误。训练数据来自1811个autoPET和DeepPSMA研究，提交版本集成了五折的最佳和最终检查点（logit平均）。

Result: 在交互式六步交互的五折交叉验证中，最终检查点达到平均AUC-Dice 3.836和平均AUC-DMM 3.869，每一折都有单调提升，第一个纠正涂鸦贡献了约一半的总增益。

Conclusion: 该方法成功实现了交互式病变分割，课程学习策略有效提升了模型性能，第一步纠正涂鸦对整体性能提升贡献显著。
...
TL;DR: GCA是一个从变形物体的单目动态视频中学习隐式本构定律的框架，使用3D高斯表示。该方法结合LoRA自适应、RDGA几何约束和CPR本构先验正则化，在单目监督下实现鲁棒的物理动力学学习。


<details>
  <summary>Details</summary>
Motivation: 现有隐式方法在噪声监督下容易陷入局部最小值且缺乏物理可解释性，而显式方法依赖预定义本构方程，泛化能力有限且在单目设置下不稳定。需要一种既能保持隐式建模灵活性，又具有物理可解释性的方法。

Method: 提出统一框架包含三个核心组件：1) LoRA-based adaptation用于参数高效微调；2) RDGA（基于秩的深度几何锚点）通过尺度不变的秩基深度对齐建立稳健的几何约束；3) CPR（本构先验正则化）将经典本构模型作为软可微分先验集成。

Result: 在合成、real-to-sim和真实世界数据集上验证，合成基准测试中比最强基线方法降低48%的Chamfer距离，且在单目监督下保持鲁棒性。

Conclusion: GCA成功实现了从单目视频中学习隐式本构定律，在精度和鲁棒性方面优于现有方法，为可变形物体的物理动力学建模提供了有效解决方案。
...
TL;DR: 本文介绍了欧洲E-roads公路的车辆速度数据集（2022-2026年），该数据集通过Copernicus Sentinel-2卫星光学影像的蓝、绿、红三个波段的短时滞特性检测车辆位移来获取速度信息。每条记录包含轨迹几何、位移数据、航向、质量指标、估计速度、时间戳和源产品标识符，以GeoPackage格式发布。


<details>
  <summary>Details</summary>
Motivation: 欧洲缺乏大规模连续覆盖的车辆速度观测数据，传统方法（如浮动车数据、雷达测速）覆盖范围有限且成本高昂。卫星遥感提供了一种被动、大规模获取交通速度数据的新途径。

Method: 利用Sentinel-2推送式扫描仪的B02(蓝)、B03(绿)、B04(红)三个10米分辨率波段间的已知短时滞特性。车辆在三个波段上呈现轻微位置偏移形成"移动回波"，通过预测-匹配算法将检测到的强度峰值链接为单车轨迹，再利用公开的波段间时滞将位移转换为地面速度。

Result: 生成了欧洲E-roads（包括高速公路、干道、主要和次要道路）的车辆速度数据集，每条记录包含轨迹几何、各波段位移和航向、内部质量指标、估计速度、采集时间戳及Sentinel-2产品标识符，以GeoPackage格式发布。

Conclusion: 该数据集可支持交通模式研究、速度行为分析、运输建模及道路网络属性校准等大陆尺度应用，为欧洲交通地理信息提供了新的数据源。
...
TL;DR: TRACE框架是一种无监督方法，用于从含有伪影的临床3D头部照片直接构建统计形状模型(SSM)，无需依赖CT扫描或精挑细选的3D照片。


<details>
  <summary>Details</summary>
Motivation: 现有颅缝早闭严重程度分析依赖CT或精挑细选的3D照片，但临床3D照片虽无辐射且可重复，却常包含肩膀、手、头发、服装、扫描噪声和不完整边界等问题，破坏对应关系。

Method: TRACE预测稀疏解剖对应头表控制点，通过粗到细的表面感知变形级联细化，使用薄板样条将清洁模板变形为特定受试者头部重建。模板约束公式使密集对应保持在临床相关头部解剖上，同时抑制非头部伪影。对应模块与点云编码器解耦，可与不同骨干网络配对。

Result: 在所有骨干网络上，TRACE显著改善了表面采样、拓扑保持和形状模型质量，提供可扩展的照片基础颅缝早闭形状分析基础。

Conclusion: TRACE为伪影污染表面扫描的统计形状模型构建提供可扩展框架，当有适当清洁模板时可扩展到其他应用领域。
...
TL;DR: 本文提出了VGAU-Diag评估框架，用于研究统一多模态模型(UMMs)中视觉生成是否能提升理解能力。研究发现生成式视觉辅助在简单任务中有效，但随着推理复杂度增加变得不可靠，且主要瓶颈在视觉理解侧而非生成侧。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型(UMMs)既能理解又能生成，但现有评估对"视觉生成能否改善理解"这一问题给出了混淆的证据，因为现有评估混淆了任务难度、推理范式和生成-理解之间的闭环交互。

Method: 提出VGAU-Diag细粒度评估框架，具有三个关键特性：按难度分层样本、支持多种推理范式的统一评估、以及Oracle-Assisted参考协议。

Result: 生成式视觉辅助在简单实例上有帮助，但随着推理复杂度增加变得不可靠。Oracle辅助诊断揭示主要瓶颈在视觉理解侧而非视觉生成侧，因为当前UMMs即使面对忠实视觉辅助也难以利用。有效的视觉生成应针对视觉理解瓶颈而非增加更多推理步骤，并识别出三阶段过渡：从任务无关噪声→误导性似是而非的指导→有用辅助。

Conclusion: 这些发现为开发更好的UMMs提供了指导方向：应聚焦于提升视觉理解能力，而非仅仅增加推理步骤或生成更复杂的视觉辅助。
...
TL;DR: 本文提出了VERDICT系统，用于光学化学结构识别(OCSR)中的质量控制，通过比较四个不同架构的识别器的一致性来识别不可靠的预测，在多个基准数据集上表现优异，并在PMC Open Access中产生了6,146个结构标签，化学家审核确认三位数以上规则的精确度达99.5%。


<details>
  <summary>Details</summary>
Motivation: 在大规模构建化学训练数据集时，需要识别不可靠的OCSR预测结果，但缺乏无标签的评估方法。现有的像素空间重渲染方法在真实文献图像上表现不佳，无法有效筛选错误预测。

Method: 比较了三种无标签信号：模型置信度、重渲染相似度和识别器一致性。在263个ACS期刊图像和CLEF-IP、UOB、USPTO数据集上测试，并应用物质过滤器去除通配符和R基团的误判。

Result: 重渲染相似度AUROC仅为0.547（接近随机），而四个识别器一致性的AUROC达0.916。二取四规则接受81.7%图像（精确度88.8%），三取四规则接受52.1%图像（精确度98.5%）。应用于PMC Open Access产生6,146个结构标签，化学家审核400个样本确认三取四规则精确度99.5%，二取四规则精确度95.8%。

Conclusion: VERDICT通过识别器一致性方法实现了可靠的OCSR质量控制，能够为多模态分子数据库提供经验证的结构标签，并可作为分子记录的图像搜索接口。
...
TL;DR: SAM3Dual是用于视频目标分割的无训练推理方法，通过将时序记忆分离为短期分支和长期分支进行组合，在MOSEv2挑战赛中获得第三名，J&F分数为64.37。


<details>
  <summary>Details</summary>
Motivation: 当前视频目标分割(VOS)方法需要大量训练或微调，作者希望探索完全在推理时重组时序记忆的可能性，同时保持预训练模型参数不变。

Method: 基于预训练SAM 3构建，将时序记忆显式分离为短期分支（近期观察）和长期分支（间隔采样的历史表示）；使用确定性序列相对融合调度结合两个记忆响应；并由前一帧目标置信度进行保守调制。所有SAM 3参数保持冻结，无需任务特定训练、微调或在线参数优化。

Result: 在LSVOS挑战赛MOSEv2赛道中获得官方J&F分数64.37，排名第三。

Conclusion: 完全在推理时重组时序记忆可获得具有竞争力的长期VOS性能，同时保留预训练模型的能力。
...
TL;DR: 本文提出UR²-MLLM框架，解决现有放射学报告生成模型缺乏动态视觉重访机制的问题，通过不确定性感知、多模态推理轨迹和视觉定位奖励实现不确定区域的动态重访推理。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成的多模态大语言模型(MLLMs)采用"图像思考"范式，但提供的视觉证据是静态的，缺少动态重访机制。放射医生会迭代重访可疑区域以完善诊断，但现有方法忽视了这一点。

Method: 1)构建不确定性感知数据集进行训练；2)构建多模态推理轨迹数据集 together with detect-and-copy机制，指导何时何地重访；3)通过强化学习的视觉定位奖励将重访区域与解剖结构对齐。

Result: 在MIMIC-CXR和IU-Xray数据集上达到最先进性能，验证了不确定性感知视觉重访推理对可靠且符合临床规范的报告生成的价值。

Conclusion: UR²-MLLM通过动态重访不确定区域进行推理，显著提升了放射学报告生成的质量，为临床诊断辅助系统提供了更可靠的技术方案。
...
TL;DR: 本文提出Hyper²框架，解决双曲空间监督中跨几何不匹配问题。通过在编码器和解码器的注意力机制中同时引入双曲距离编码，并使用共享曲率α的双曲Chamfer损失，实现编解码器与损失函数的几何一致性。在SVDFormer backbone上，Hyper²在ShapeNet-55上实现-22.9%的Chamfer reduction，在21个未见类别上达到-37.5%。


<details>
  <summary>Details</summary>
Motivation: HyperbolicCD仅将损失函数改为双曲距离，但编码器仍是欧几里得的，导致位置相关的监督信号在反向传播中被链式法则平滑掉。论文通过两个指标(feature-loss correlation r_FL和effective gradient utilisation u_G)验证了单空间操作的局限性，提出几何一致性是关键。

Method: 提出Hyper²双曲空间一致性框架：(1)将arcosh(1+αd²)函数形式作为位置偏置应用于refinement attention(双曲距离编码)；(2)保留HyperbolicCD的双曲Chamfer损失；(3)编解码器和损失函数共享同一曲率α。两个算子都是O(N log N)标量非线性，仅增加约1.6% FLOPs。

Result: 在SVDFormer backbone上：(1)ShapeNet-55上Chamfer减少22.9%(远超单空间操作的线性叠加13.2%)；(2)21个未见ShapeNet-34类别上减少37.5%；(3)指示器r_FL和u_G在双空间配置下从(0.68, 39%)跃升至(0.95, 87%)，而单空间配置基本不变。

Conclusion: 双曲监督的有效性源于编码器-损失函数的几何一致性，而非单独使用任一算子。Hyper²通过双空间一致性框架，在仅增加1.6% FLOPs的情况下，显著提升了点云补全性能，特别是在泛化到未见类别方面。
...
TL;DR: 本文提出了一种无需训练的视觉-语言模型个性化校准残差解码框架，通过构建正向、反事实和空配置文件三种条件，利用分数差异估计个性化的边际贡献，并引入基于归一化熵的不确定性校准来适应残差信号的可靠性。在MMPB、YoLLaVA和MyVLM数据集上验证了该方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 直接使用用户档案进行个性化提示无法保证模型可靠地利用这些证据，预测分布中混合了用户档案支持的个性化信号和模型的通用视觉/语言先验，导致难以判断高置信度答案是由用户档案支持还是仅反映模型的默认偏好。

Method: 构建三种证据条件：正向用户档案、反事实档案和空档案，将正向档案下的预测作为锚定基准，通过三种条件下的分数差异显式估计个性化的边际贡献。进一步引入基于归一化熵的不确定性校准，使个性化增强的强度适应残差信号的可靠性。

Result: 在MMPB、YoLLaVA和MyVLM数据集上，所提方法在无需微调的情况下提升了个性化多模态理解能力，在身份敏感的视觉个性化任务上获得一致的增益。额外分析表明，熵校准在对比个性化信号不确定时能稳定残差解码。

Conclusion: 该训练-free校准残差解码框架能有效分离个性化信号与模型先验，通过熵校准增强了个性化增强的可靠性，为无需微调的视觉-语言模型个性化提供了新思路。
...
TL;DR: 提出了一种混合GAN引导的扩散框架，利用预训练WGAN-GP的特征先验引导条件扩散模型进行图像恢复，在去噪和超分辨率任务上分别提升了4.40 dB和3.70 dB的PSNR。


<details>
  <summary>Details</summary>
Motivation: GAN能高效生成图像，扩散模型能提供高质量图像恢复但需要迭代采样。现有方法未能有效结合两者优势，因此提出混合框架利用GAN特征引导扩散模型。

Method: 使用预训练WGAN-GP作为特征先验，将冻结的WGAN-GP生成器的中间特征通过交叉注意力融入扩散U-Net，在DDIM采样过程中保持固定。评估任务包括高斯去噪和2倍超分辨率，使用CelebA人脸图像。

Result: 去噪性能提升4.40 dB PSNR，超分辨率性能提升3.70 dB PSNR。框架持续改善降质和低分辨率图像质量。

Conclusion: 冻结GAN特征先验可有效引导扩散模型实现稳定有效的图像恢复，验证了GAN与扩散模型结合的潜力。
...
TL;DR: 本文提出了在线视频问答任务中的中断处理问题，并构建了首个标准化评估基准OVIBench，用于评估视觉语言模型在用户中断场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答研究仍采用离线、单轮范式，忽视了现实交互中用户可能在模型生成答案时中断模型的情况，导致缺乏对中断处理能力的系统评估。

Method: 构建了OVIBench基准，将中断分为三类：取消、误触发和纠正；开发了离线模拟协议在统一时间设置下重现生成过程中的中断；设计了多维度指标体系评估中断理解和响应生成能力。

Result: 实验表明OVIBench能有效区分模型的中断处理能力，特别是在遵循纠正请求方面。基于OVI-Train数据集微调的模型在OVIBench上取得显著提升。

Conclusion: OVIBench和OVI-Train为视觉语言模型的中断处理能力提供了标准化评估和训练框架，填补了该领域的空白。
...
TL;DR: DECO是一种用于低空无人机视觉定位的深度引导共可见性推理框架，通过单目深度先验推断局部表面几何结构，估计无人机图像与参考地图之间的共可见区域，并引入几何-显著性耦合的共可见性评分来共同考虑几何共可见性和检测器显著性，从而保留既视觉独特又几何共可见的关键点，提高特征匹配和基于PnP的姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 在GNSS拒绝环境中，无人机需要稳健的视觉定位。现有方法通过匹配无人机图像与地理标记的正射参考地图的关键点来估计无人机姿态，但参考地图主要记录俯视表面（屋顶、地面），而垂直结构（立面、墙壁）常被压缩或缺失，导致低空无人机图像中许多视觉独特的关键点在参考地图中没有有效对应点，产生冗余匹配和姿态估计不准确的问题。

Method: 提出DECO框架，包含：1）使用单目深度先验推断局部表面几何结构，估计无人机图像与参考地图之间的共可见区域；2）引入几何-显著性耦合共可见性评分（Geometry-Saliency Coupled Co-visibility Score），共同考虑几何共可见性和检测器显著性来对关键点进行排名；3）保留既视觉独特又几何共可见的关键点。

Result: 实验表明DECO实现了卓越的定位性能，可与不同的深度模型、特征检测器和匹配器集成。

Conclusion: DECO通过深度引导的共可见性推理有效解决了低空无人机视觉定位中垂直结构缺失导致的匹配不准确问题，显著提升了姿态估计的准确性和鲁棒性。
...
TL;DR: 本文提出了一种新型非线性扩散去噪方案，该方案通过应用驱动的值空间变换将线性扩散过程转化为非线性扩散，用于图像去噪。


<details>
  <summary>Details</summary>
Motivation: 图像去噪结果需要根据不同应用领域（如图像压缩、静态图像采集和医学图像）具有完全不同的行为特征，现有的非线性扩散技术难以灵活适应这些多样化需求。

Method: 提出一种从应用确定的值空间中的线性扩散过程推导新型非线性扩散方案的 方法，通过在变换后的空间中进行应用驱动的线性扩散。

Result: 实验表明，应用驱动的变换空间线性扩散方法与现有非线性扩散技术相比表现更优。

Conclusion: 该方法能够根据不同应用场景灵活调整去噪行为，为图像去噪提供了一种有效的新途径。
...
TL;DR: SCDF是一种无需训练、无需GPU的卫星光学图像配准方法，使用密集逐像素位移场作为运动模型，通过预测-测量-滤波循环在分辨率金字塔上运行，在584个真实图像对上实现了零失败率和显著更高的配准精度。


<details>
  <summary>Details</summary>
Motivation: 现有光学卫星图像的多时相和多传感器使用中存在配准问题，运营产品仍存在明显超过亚像素级别的偏移。真实图像对在多个维度上不同（传感器响应、场景内容、观测几何、分辨率、拼接缝），现有工具嵌入的运动模型和常数针对开发数据调优，不适合的图像对要么匹配失败，要么返回错误结果而不报告失败。学习匹配器需要GPU且在其训练分布之外的准确性无法保证。

Method: 提出SCDF（自校准位移场），是一种无需训练、无需GPU的估计器，其运动模型本身就是密集的逐像素位移场，因此没有场景运动落在模型之外。单个预测-测量-滤波循环在分辨率金字塔上运行：累积场预测移动图像的每个patch在参考图像中的位置，RootSIFT匹配和相关传递以亚像素精度测量位移，校准阈值决定哪些保留。一种配置无需数据集调优即可在单个CPU核心上处理完整的8192^2场景。

Result: 在584个基于真实Sentinel-2、Landsat-8/9和NAIP图像构建的地面真值对上，与七个经典基线和两个零样本预训练匹配器相比，SCDF以零失败率配准每个配对，将最佳基线的真实配对中位端点误差从6.83m降至4.17m，将其90th百分位从17.8m降至7.77m。

Conclusion: SCDF通过使用密集位移场作为运动模型，成功解决了传统方法中场景运动超出模型范围的问题，实现了无需调优的通用、高精度卫星图像配准，在精度和鲁棒性方面显著优于现有方法。
...
TL;DR: 研究了偏微分方程(PDE)何时可以视为能量泛函的欧拉-拉格朗日(E-L)方程，并将该方法应用于彩色图像去噪，取得了良好的效果。


<details>
  <summary>Details</summary>
Motivation: 建立偏微分方程与能量泛函之间的联系，使得可以借助变分方法来分析和处理图像去噪问题。当前图像去噪方法众多，但缺乏统一的理论框架来指导PDE方法的设计。

Method: 1. 研究PDE成为E-L方程的必要条件；2. 构建包含数据项和平滑项的能量泛函；3. 通过变分法推导相应的E-L方程；4. 将理论应用于彩色图像去噪问题。

Result: 证明了PDE作为E-L方程的必要条件，并将该能量泛函方法应用于彩色图像去噪。实验表明，该方法与当前最先进的彩色图像去噪技术相比具有优势。

Conclusion: 建立了PDE与能量泛函之间的理论联系，为图像去噪提供了一种有效的变分方法框架。该方法在彩色图像去噪任务上展现出良好的性能，具有理论价值和实用价值。
...
TL;DR: 本文提出LDSA（语言驱动的密集语义适配器），用于解决多标签图像分类中标注不完整的问题。该方法利用预训练CLIP模型的多模态能力，通过密集对比适配器和语言驱动交互解码器实现prior-adaptive学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖强先验假设来恢复缺失语义，但这些统计先验容易导致不稳定的语义错误和严重的过拟合问题。

Method: 1. 密集对比适配器：构建密集视觉对比约束，将任务特定知识迁移到视觉领域；2. 语言驱动交互解码器：结合类别特定提示调优，使语言代理与视觉领域适配；3. 协作学习机制

Result: 在公开多标签分类基准数据集上达到新的最先进水平，解释了性分析表明LDSA通过先验自适应学习方案发现了隐含的语义关系。

Conclusion: LDSA有效解决了标注不完整情况下的多标签分类问题，通过多模态预训练模型提取先验自适应关系，避免了传统方法因统计先验不稳定导致的过拟合问题。
...
TL;DR: 本文提出使用梯度能量张量(gradient energy tensor)替代结构张量(structure tensor)用于图像处理任务，并在角点检测、光流和图像增强等应用中验证了其有效性。实验表明该方法可在GPU上实现实时张量图像增强，帧率提升40%且图像质量无损失。


<details>
  <summary>Details</summary>
Motivation: 图像处理中常使用图像张量，如角点检测、光流和迭代增强等方法。传统上这些张量使用结构张量估计，本文旨在探索梯度能量张量作为结构张量的替代方案。

Method: 将梯度能量张量应用于常见的图像处理问题，包括角点检测、光流和图像增强，并与结构张量方法进行对比实验。

Result: 实验结果表明，梯度能量张量可在GPU上实现实时张量图像增强，帧率提升40%且保持图像质量不变。

Conclusion: 梯度能量张量可作为结构张量的有效替代方案，在图像处理的多个应用场景中具有实用价值，特别是在需要实时处理的场景下优势明显。
...
TL;DR: MedReaMM是一个用于评估大型多模态模型(LMMs)在临床诊断决策中整合异构临床证据能力的新型基准测试。该基准包含625个专家验证的病例，平均每个病例2.79张医学图像，共1,042个ICD-11编码的标准化诊断。评估了23个LMMs后发现大多数准确率低于50%，凸显了多模态诊断综合能力的重大差距。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注文本推理或独立的视觉问答任务，缺乏将临床叙述与医学影像整体整合的评估，无法评估专家临床判断中核心的多模态诊断综合能力。因此需要构建一个能够评估模型在完整信息范式下将详细患者病史与多张医学图像综合成准确鉴别诊断的能力的基准。

Method: 从顶级医学期刊和精选临床病例数据库中获取病例报告构建MedReaMM基准。评估了23个大型多模态模型(LMMs)，并分析了医学知识水平、医学图像理解和证据整合与诊断性能的相关性。

Result: 大多数LMMs的诊断准确率低于50%。进一步分析表明，医学知识水平、医学图像理解和证据整合能力均与诊断性能高度相关。

Conclusion: MedReaMM基准成功填补了多模态医学诊断综合能力评估的空白。当前主流LMMs在整合异构临床证据进行诊断方面存在显著不足，需要在医学知识、图像理解和证据整合三个维度同时提升才能改善诊断表现。
...
TL;DR: AcroMELD是一个3900万参数的检测器，结合高分辨率视觉Transformer与无标签PDF图元，用于检测PDF表单中缺失的交互式表单字段（widgets）。


<details>
  <summary>Details</summary>
Motivation: 视觉上像表单的PDF文档往往缺少交互式表单字段，导致用户无法直接输入数据，只能打印或使用外部编辑工具。检测缺失的widgets困难在于：字段可能由多个重叠线索指示、born-digital PDF暴露不完整的绘图结构、密集页面可能包含数百个字段。

Method: 提出AcroMELD（AcroForm Multi-source Evidence Linking Decoder），采用896-query集合（384个视觉提议、384个结构种子提议、128个学习恢复查询）。四个图集合层通过几何偏置稀疏邻域交换信息并交叉关注PDF结构。学习到的同域关系链接共引用候选，定位质量头使用包含感知的重叠进行训练。定义哈希绑定评估协议。

Result: 在内部测试集上原生包含micro-F1达0.9344，单次外部保留集达0.8477（95% PDF聚类引导区间[0.8339, 0.8605]），比注册的FFGBT-v8参考高0.0186。但在更严格的外部适配器下，性能为0.7786 IoU-0.5 F1和0.2900 COCO mAP，低于本地评估的CommonForms-L参考；签名类在选定阈值下无预测。

Conclusion: 结果支持注册的操作门，同时暴露了显著的领域和稀有类限制。该方法在内部测试上表现良好，但在外部域和罕见类别（如签名）上存在明显不足。
...
TL;DR: TransHands是一个 backbone无关的迁移学习框架，将大规模人体姿态数据中学到的运动表征迁移到3D手部姿态估计任务中，通过两阶段训练和轻量级手部适配模块解决手部数据集稀缺的难题。


<details>
  <summary>Details</summary>
Motivation: 当前3D手部姿态估计面临的主要挑战是缺乏大规模、多样化的3D标注手部数据集，而人体运动数据却丰富得多。作者提出利用人体姿态数据中学习到的运动先验来解决手部姿态估计问题。

Method: 提出两阶段训练策略：1)利用大规模人体姿态数据预训练运动编码器；2)在手部数据上进行微调。同时设计轻量级手部特定输入适配模块，将手部运动学与全身运动的表征空间对齐。框架支持多种运动建模架构（Transformer、图网络、频域模型）。

Result: 在四种最先进的运动建模架构上验证，表明运动先验可跨架构一致迁移，获得稳定的精度提升，具有强大的跨域泛化能力，尤其在挑战性的第一人称视角场景中表现优异，适用于现实世界的下游任务。

Conclusion: 通过迁移学习框架成功将人体运动表征应用于手部姿态估计，证明了运动先验的跨域有效性，为解决数据稀缺问题提供了新思路。
...
TL;DR: 本文提出了一种高效的3D Gaussian Splatting训练框架，通过引入极化不透明度先验（POP）替代传统的"密集化-剪枝"循环，有效解决了3DGS模型膨胀问题，实现了更紧凑的表示。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting虽然实现了实时速度下的最优渲染质量，但存在"模型膨胀"问题：大量冗余、低不透明度高斯体膨胀了内存占用和训练成本。这一问题源于传统的"密集化-剪枝"范式，即先激进扩展模型，再依赖剪枝实现紧凑性。

Method: 提出协同设计框架：（1）L2重建损失提供误差比例梯度以稳定优化；（2）创新性极化不透明度先验（POP）主动管理高斯群体。POP引导信息量大的基元趋向完全不透明，无信息基元趋向透明，实现自然剪枝并通过早期光线终止加速渲染。

Result: 在三个公共数据集上的实验表明，该方法在保持相当视觉重建质量的同时，显著减少了高斯数量，实现了加速的3DGS训练。

Conclusion: 所提框架为快速且本质上紧凑的3DGS训练提供了一条简单有效的路径。
...
TL;DR: 本文介绍了一个多模态数据集，包含485份扫描版考试答案，配有专家设计的OBE评分元数据，来自4所学术机构的415名学生，涵盖9个学科和12种题型，共47个评分标准。


<details>
  <summary>Details</summary>
Motivation: 为基于结果的 教育(OBE)评估研究提供数据支持，解决自动评分、多模态文档理解、评分预测等领域缺乏真实学术手写数据的问题。

Method: 收集并整合多种来源的扫描文档(CamScanner、Adobe Scan、普通扫描仪)，进行标签标准化、分数验证、标识符随机化等处理，并通过答案级审计确保数据完整性。

Result: 数据集包含485份唯一标识的PDF文件，每个都关联完整的评分标准、性能等级描述和总分，且总分与标准分之和一致。

Conclusion: 该数据集可用于评分标准感知的自动评估、多模态文档理解、标准级反馈、分数预测和隐私保护的OBE评估研究。
...
TL;DR: 本文提出了一种音频优先的分流策略（Audio-first Triage），通过使用最轻量的音频模态在视频解码前选择需要调用视觉语言模型（VLM）的窗口，从而大幅降低长时程自我中心视频分析的计算成本。该方法无需特定领域的音频事件标签，直接利用预训练的AudioSet特征。


<details>
  <summary>Details</summary>
Motivation: 当前处理长时程自我中心视频的 pipelines 成本过高，因为费用随模型调用次数线性增长。现有分流策略存在矛盾：它们使用视觉特征进行排名，但这恰恰需要本应通过预算约束避免的视频解码过程。

Method: 提出音频优先分流方法：在解码任何视频帧之前，使用最轻量的模态（音频）进行窗口选择。核心创新在于训练目标不是每帧声音事件检测器，而是训练选择器每个动作触发一次（once-per-action objective），从而提高动作覆盖率。使用冻结的AudioSet预训练特征，无需领域特定的音频事件标签。

Result: 在EPIC-KITCHENS-100（EK-100）上，分流在匹配覆盖率下减少了9-20%的VLM调用；在Ego4D的247个剪辑上，使用不到一半的调用次数就超越了中期的均匀采样；并优于两个最新的视觉关键帧选择器。动作覆盖率在所有评估的调用率下提升了4.0-10.8个百分点。

Conclusion: 音频模态可作为有效的预过滤机制，在极低成本下实现高效的视频分析。该方法自然可与token压缩或量化技术结合，为资源受限场景下的长视频理解提供了可行方案。
...
TL;DR: 本文评估了传统OCR、通用视觉语言模型(VLM)、阿拉伯语专用VLM以及OCR条件化VLM校正四种方法在阿拉伯语手稿识别中的表现，涵盖历史手稿、印刷书籍、干净印刷、多领域文档和手写体等八种数据集。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在阿拉伯和伊斯兰手稿识别中的应用尚未得到充分探索，存在显著的研究空白。

Method: 在八个阿拉伯语数据集上评估了四种方法：传统OCR（Tesseract）、通用VLM、阿拉伯语专用VLM和OCR条件化VLM校正，涵盖历史手稿、陈旧印刷品、干净印刷、多领域文档和手写体等不同类型。

Result: 没有单一方法在所有场景下占优。行级历史手稿上VLM接近Tesseract；页级手稿上VLM表现更好；OCR条件化校正可改善陈旧印刷、干净印刷、混合领域阿拉伯语和部分Naskh手稿的识别效果。但当OCR先验存在脚本不匹配或系统性误导时（如Maghribi手稿和学生手写体），性能会下降。

Conclusion: 提出了"OCR先验可恢复性原则"：OCR条件化有帮助当且仅当OCR输出在视觉和文本上可恢复。阿拉伯语VLM-OCR对音符、预处理、生成预算和重复循环敏感。建议采用自适应OCR-VLM工作流，根据脚本、OCR先验可恢复性、长度诊断和失败模式指标进行路由。
...
TL;DR: 提出了一种名为Detector-Interface Distillation (DiD)的无标签转换方法，用于将训练好的Softmax注意力ViT检测器转换为线性注意力版本，同时保持检测性能。


<details>
  <summary>Details</summary>
Motivation: 直接替换Softmax注意力和通用无标签蒸馏方法在检测任务上效果不佳，核心挑战在于"检测器接口保护"——转换后的骨干网络必须重现下游检测器期望的特征张量。

Method: DiD是一种无标签转换方法，仅通过将线性注意力骨干网络的检测器接口张量与冻结的Softmax教师网络对齐来训练骨干网络。

Result: 在DOTA-v1.5上，DiD显著优于基线方法，并与完全监督训练的线性模型相当。适配仅需约87分钟（4 GPU），线性化骨干网络减少约62%延迟和49%峰值内存。

Conclusion: 该方法为将训练好的Softmax检测器重用为高效线性检测器提供了一条简单无标签的路径，并鼓励未来架构转换工作中采用接口感知目标。
...
TL;DR: LiST是一个无标签的测试时LoRA融合框架，将现有LoRA银行转换为目标条件局部单纯形，在推理时为每个样本搜索特定的融合权重。该方法从LoRA参数锚点和提示级行为向量构建联合任务表示，检索相邻适配器作为局部搜索空间，执行分支保留融合。


<details>
  <summary>Details</summary>
Motivation: 现有的适配器组合方法大多是静态的，无法适应 individual test inputs（个体测试输入）。作者认为需要一种能够在推理时动态适应每个样本的适配器融合方法。

Method: LiST的核心方法包括：1）从LoRA参数锚点和提示级行为向量构建联合任务表示；2）检索相邻适配器作为局部搜索空间；3）执行分支保留融合而不更新主干或适配器；4）使用包含先验、几何和随机一致性约束的提示级能量来选择候选权重；5）通过安全接受规则部署权重，否则回退到目标条件先验。

Result: 在多模态和语言基准测试上的实验表明，LiST优于静态LoRA融合和传统测试时适应基线，同时保留了任务特定适配器效用，并在未见任务上提高了鲁棒性。

Conclusion: LiST能够实现样本级别的动态LoRA融合，在保持适配器实用性的同时提高了对未见任务的鲁棒性，为大型语言和视觉语言模型的测试时适应提供了新思路。
...
TL;DR: M³ISR是一个用于3D和4D高斯溅射(3DGS/4DGS)的受控合成基准测试，旨在促进高保真自由视点视频(FVV)的重建、压缩和流媒体研究。


<details>
  <summary>Details</summary>
Motivation: 现有真实捕获的多视图视频基准难以隔离受控相机几何、表示效率和时间冗余的影响；实际部署受表示大小、动态更新和计算成本的约束。

Method: 构建包含25个场景的基准测试，分为5个室内外场景组，2种相机/运动配置，6个同步1080p视图，提供RGB、相机参数、深度、语义和实例分割及静态-动态掩模的密集标注。采用共享中心相机设计隔离角度视图变化。

Result: 静态重建质量差异较小，但表示存储差异显著；流媒体方法比离线动态重建基线有更高的训练或重建成本。定义了3DGS和4DGS的前馈压缩任务并提供率-失真基线评估。

Conclusion: M³ISR作为受控互补的测试平台，可系统研究基于高斯的FVV重建、压缩和流媒体任务。
...
TL;DR: HeatTok是一个针对遥感图像的语义感知视觉分词器，采用热扩散聚合机制生成对象对齐的不规则令牌，并设计G-MRoPE位置编码处理不规则空间分布，在VRSBench和EarthVQA数据集上取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于patch的视觉分词方法导致遥感图像中不规则地理对象的语义混合和对象碎片化问题，现有自适应方法难以提取精确的对象级令牌且缺乏针对不规则区域的几何位置编码。

Method: 提出HeatTok：1)基于热传导原理的热扩散聚合，自适应合并相邻同质区域生成语义独立的对象级不规则令牌；2)G-MRoPE高斯多模态旋转位置嵌入，通过2D高斯建模令牌空间分布，显式注入中心、尺度和方向信息。

Result: 在VRSBench和EarthVQA数据集上的广泛评估表明，HeatTok有效保留了对象级语义完整性，在合理令牌预算下达到先进性能。

Conclusion: HeatTok通过热扩散驱动的语义感知分词和针对不规则形状的位置编码，有效解决了遥感图像中对象碎片化和语义混合问题，为多模态大语言模型的遥感理解提供了新的有效方案。
...
TL;DR: SRAIN是一个用于组合视觉数据检索的框架，通过在嵌入空间中进行线性插值来融合参考图像/视频和文本修改。与使用多模态大语言模型(MLLM)的现有方法不同，SRAIN动态预测查询特定的插值权重，显著降低了查询延迟。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的方法使用多模态大语言模型进行融合，但复杂度导致推理延迟过高，限制了其可扩展性。作者重新审视简单线性插值在嵌入空间中的有效性，并尝试解决插值权重难以预测的问题。

Method: 提出两个关键创新：1) 训练时使用批量感知的排名权重估计；2) 推理时使用紧凑的记忆库合成困难负样本。核心思想是插值质量应同时考虑从负样本中的区分力和与真实目标的接近程度。

Result: SRAIN在组合视频检索中达到最佳性能，在组合图像检索中匹配当前最先进水平，同时与基于MLLM的替代方案相比大幅降低了查询延迟。

Conclusion: 通过动态预测查询特定的插值权重和利用困难负样本训练，SRAIN成功地在保持高检索精度的同时解决了MLLM方法的高延迟问题，为组合视觉检索提供了一种高效且可扩展的解决方案。
...
TL;DR: 本文提出VES-Bench基准和TRACE代理，用于评估视频语言模型是否解码了覆盖所有必要证据间隔的帧。VES-Bench包含600道题，涵盖348个长视频的时间顺序和事件计数任务。TRACE是一种无需训练的代理，逐步构建证据束并在答案稳定时停止。在严格审计下，TRACE以98.7帧/问题的成本达到50.7%准确率，比均匀解码高出10分以上，且在Video-MME、LVBench和LongVideoBench上均具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅评分最终答案正确性或预测的证据间隔，但很少审计方法在回答前解码的帧是否真正覆盖了所有必要证据。这导致正确答案可能建立在不完整观察的基础上，缺乏对证据覆盖的严格验证。

Method: 1) 构建VES-Bench基准：600道时序排序和事件计数问题，跨越348个长视频，每个题目携带联合必要的证据间隔，可在三个严格程度级别审计方法是否覆盖所有证据间隔。2) 提出TRACE代理：无训练需求的智能体，在原始视觉片段中为答案提供依据，逐轮构建证据束，仅当答案随着证据增加而稳定且对相同片段的最终一遍返回相同答案时停止。

Result: 在同一主干网络审计下，TRACE以每个问题98.7帧的成本达到50.7%准确率（至少两个解码帧位于每个证据间隔内），比128帧的均匀解码（40.2%）高出10分以上；在256帧均匀解码成本仅0.39倍的情况下，达到其准确率范围内2.6分，并达到最高答案准确率（63.5%）。TRACE在Video-MME（86.1）、LVBench（75.6）和LongVideoBench（75.1）上也保持竞争力。

Conclusion: 本文通过VES-Bench基准和TRACE代理，有效解决了长视频问答中证据覆盖审计的问题。TRACE通过迭代构建证据束并自适应停止的方式，在显著降低帧成本的同时实现了更高的证据覆盖率和答案准确率，且在多个长视频基准上展现了良好的泛化能力。
...
TL;DR: VISTA是首个针对下一代自回归图像生成模型的梯度基测试时对齐框架，能够在不修改模型参数的情况下，通过优化中间表征来改善文本到图像生成中的属性绑定和空间关系等组合失败问题。


<details>
  <summary>Details</summary>
Motivation: 视觉自回归(VAR)模型虽已成为扩散模型的高质量替代方案，但仍存在持续的组合失败问题，如图像违反提示词中指定的属性绑定和空间关系。扩散模型有丰富的测试时对齐方法，但VAR模型特有的状态性、离散性、多分辨率采样过程使得现有技术无法适用，因此需要开发专门针对VAR的测试时对齐方法。

Method: VISTA建立在Infinity模型基础上，直接干预生成过程，通过冻结的transformer优化中间表征来引导视觉预测朝向组合约束方向发展，无需修改模型参数或额外训练。其引入了在多个尺度上使优化稳定所需的机制，并提供了可扩展的目标空间，任何关于交叉注意力的可微分约束都可以插入使用。

Result: 在两个基准测试和两个模型规模上，VISTA改善了每个目标组合类别，将2B骨干网的平均目标分数提高近20%，8B骨干网提高近6%，空间关系改善最为显著。图像质量得到保持：VISTA从未优化其输出的独立偏好模型分数提高了近20%。值得注意的是，配备VISTA的2B模型超越了其四倍大小的骨干网，表明模型规模间的组合差距大部分可在测试时恢复。

Conclusion: VISTA成功证明了梯度基测试时对齐方法可应用于下一代自回归图像生成，为该领域提供了首个此类框架。实验表明测试时优化能有效弥补模型规模差异，为提升自回归模型的组合能力开辟了新途径。
...
TL;DR: RS³-Prune是一种无需训练的token剪枝方法，通过在视频对象分割网络的两个关键位置限制token数量来提升效率。该方法在图像编码器与内存注意力之间限制查询token数量，并在内存编码器与内存库之间仅保留对象空间范围内的token，实现高达38.8%的FPS加速和13.1%的内存降低。


<details>
  <summary>Details</summary>
Motivation: 现代视频对象分割(VOS)模型采用高成本的密集token设计，每个帧产生密集token网格，内存库累积所有历史帧的token，导致长视频处理时延迟高、内存消耗大，无法满足长视频或实时部署需求。

Method: RS³-Prune在两个精确位置进行token剪枝：1)图像编码器与内存注意力读取之间，限制参与跨帧注意力的查询为少量几何信息子集；2)内存编码器与内存库之间，仅允许位于对象空间范围内的token进入内存库。

Result: 在多个基准数据集上，RS³-Prune实现最高38.8%的FPS加速，降低13.1%的峰值内存使用，同时保持具有竞争力的J&F分数。

Conclusion: 通过对token预算这一核心轴进行压缩，RS³-Prune成功实现了VOS模型的高效化，为长视频处理和实时部署提供了可行方案。
...
TL;DR: 提出了一种基于对称流匹配（Symmetrical Flow Matching）的新型无源无监督域适应（SFUDA）框架，用于医学图像分割，能够在不需要访问源数据的情况下将预训练模型适应到无标注的目标域。


<details>
  <summary>Details</summary>
Motivation: 跨成像模态和采集中心的域偏移是分割模型临床部署的主要障碍，需要一种不需要敏感源数据的域适应方法。

Method: 提出对称流匹配框架，作为统一生成模型同时实现图像分割和掩码到源相似图像的合成；从域无关的高斯原点初始化推理以保持结构一致性；利用对称性生成可靠伪标签和源相似合成图像，创建生成重放缓冲区，在生成自训练阶段对真实目标和合成源相似图像进行微调。

Result: 在腹部多器官分割、心脏分割（跨模态MRI<->CT偏移）和多中心前列腺分割任务上验证，优于SFUDA基线方法，并与传统UDA方法相当。

Conclusion: 该方法有效解决了域偏移问题，无需访问源数据即可实现高质量分割，适应性强且保护隐私。
...
TL;DR: 研究开发了一种基于视觉语言模型(VLM)的管道，从RGB视频中估计动态、三轴、双侧外部手力，无需在工人或处理物体上安装传感器。该方法在35名健康成年人执行5种材料搬运任务（提起、携带、推、拉）中进行了验证，使用3种箱体质量（6、9、12 kg）。均方根误差在水平和中外侧力分量约为4.7-5.6 N，垂直分量约为10.6-11.0 N。


<details>
  <summary>Details</summary>
Motivation: 外部手力是职业体力暴露和损伤风险生物力学分析的重要输入，但连续力测量通常需要仪器化对象或专门传感。本研究旨在开发一种更可扩展的方法，利用已知载荷质量和RGB视频估计手力，无需在工人或处理物体上安装传感器。

Method: 使用VLM管道结合任务特定文本提示、视觉表示和已知箱体质量。35名健康年轻成人执行5种MMH任务（提起、携带、推、拉），箱体质量为6、9和12 kg。管道使用文本引导的参与者和处理物体感兴趣区域(ROI)定位、预训练视觉Transformer特征提取和基于Transformer的时间回归。通过留一受试者交叉验证评估7种相机视角条件（3种单视角和4种多视角）和4种ROI策略下的性能。

Result: 水平和中外侧力分量的RMSE约为4.7-5.6 N，垂直分量约为10.6-11.0 N。将处理物体作为第二个ROI通常改善了力估计，单相机条件下获益最大，而像素级分割改善甚微。多相机捕获对峰值力估计提供了最明显的好处，特别是垂直分量，而不同相机配置之间的整体帧级误差差异相对较小。

Conclusion: 研究证明了从RGB视频和已知载荷质量估计连续、双侧、有方向手力估计的可行性，无需传感器作为模型输入，支持开发更可扩展的职业体力暴露和风险评估工具。
...
TL;DR: 本文提出一种数据驱动方法，从专家演示中提取结构化任务知识，利用自我中心（egocentric）和外部视角（exocentric）视频记录，通过时序多模态信息编码实现程序化文档化和上下文感知的工人指导。


<details>
  <summary>Details</summary>
Motivation: 装配和拆卸过程依赖难以文档化、复用和传递的专家知识，需要一种有效的方法来捕获和传递隐性的过程知识。

Method: 联合编码视频和叙述中的时序多模态信息，导出结构化任务表示，实现程序化文档化和上下文感知的工人指导。

Result: 基于视频的表示方法能够捕获静态图像方法无法捕捉的程序结构信息和执行上下文，在真实拆卸案例研究中验证了方法的有效性。

Conclusion: 自我中心视频理解在维修、培训和循环制造领域具有重要应用潜力。
...
TL;DR: OmniCAD是一个用于评估工业机械装配3D空间推理能力的大规模基准测试，包含25k个机械装配模型，每个平均12个零件和21种配合关系，从20个视角进行渲染。基准测试评估三个能力：组件级3D空间推理、零件间关系推理和工具增强的智能体推理。实验表明当前VLM在工业装配推理方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型(VLM)在机器人感知和空间推理方面表现出色，但对复杂机械装配的推理能力尚未得到充分探索。需要一个大规模的基准测试来评估和推动这一领域的研究。

Method: 构建了包含25k个机械装配的OmniCAD基准，涵盖机器人机构、汽车零部件、航空航天结构和农业机械。每个装配包含人类验证的3D模型和20个视角的渲染图。设计了三项评估任务：组件级3D空间推理预测零件位置和方向；零件间关系推理识别配合关系和装配约束；工具增强的智能体推理让模型迭代选择视角、检查视觉证据并优化预测。

Result: 当前VLM在工业装配推理方面存在困难，经常产生不准确的姿态、无效的配合关系、零件 interpenetration（零件穿透），且随着装配复杂度增加性能下降。

Conclusion: VLM在复杂工业装配推理方面存在显著挑战，需要开发更强大的方法来支持准确、物理有效且可扩展的3D装配推理。研究团队将开源基准测试、评估代码和工具接口以支持该领域研究。
...
TL;DR: 本论文研究如何高效捕获用于高质量面部动画的重拓扑3D面部网格。提出了VarIS自定义光球用于捕获高分辨率立体几何和反射图，开发了基于Visage Craft渲染系统的深度学习框架，直接从合成多视图图像预测重拓扑面部网格。


<details>
  <summary>Details</summary>
Motivation: 创建重拓扑3D面部网格对于高质量面部动画至关重要，但目前仍面临劳动密集且耗时的挑战。需要探索更高效的捕获生产级面部网格的方法。

Method: 1) 开发VarIS自定义光球捕获高分辨率立体几何和反射图；2) 分析相机参数对自动2D和3D标定点的影响；3) 使用合成数据方法训练神经面部回归；4) 提出深度学习框架从合成多视图图像预测重拓扑面部网格；5) Visage Craft使用外观3D可变形模型(A3DMM)生成标准化网格。

Result: incorporating accurate camera intrinsics and extrinsics improves landmark accuracy and geometric consistency, while 3D landmark regularization further improves reconstruction quality

Conclusion: 该系统可产生可直接用于绑定和动画的标准化网格，只需最少人工监督。准确相机内外参数可提高标定点精度和几何一致性，3D标定点正则化进一步提升重建质量。
...
TL;DR: 本文提出ClusterMixer，一种基于层次聚类的透明token混合器，在双曲空间中进行聚类以建模视觉数据的层次结构，并构建了HCFormer骨干网络。实验表明该方法在图像分类、目标检测、实例分割和语义分割等任务上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有token混合器（卷积、注意力、MLP等）虽然能在精度和计算成本间取得平衡，但存在黑盒问题，编码过程不透明且缺乏可解释性。需要一种透明且可解释的token混合器。

Method: 提出ClusterMixer，通过层次聚类机制显式构建token混合过程。为建模视觉数据的自然树状层次关系，在双曲空间中进行聚类（双曲空间适合嵌入层次结构且失真小）。基于此提出HCFormer骨干架构，整合ClusterMixer与精心设计的聚类策略。

Result: HCFormer在多个任务上持续优于对比方法，包括图像分类、目标检测、实例分割和语义分割。

Conclusion: HCFormer展示了透明可解释骨干网络的潜力，有望推动可解释骨干网络范式的转变。
...
TL;DR: Contextrast++是一种基于对比学习的语义分割方法，通过上下文对比学习(CCL)和边界感知负采样(BANE)两个组件来解决局部/全局上下文捕获和类别不平衡问题。CCL包含自适应融合模块、像素-锚点损失和锚点-锚点损失三个子组件，分别用于平衡多尺度特征融合、改进特征表示学习和解决长尾分布问题。BANE则通过从错误分类的边界区域选择困难负样本来提升分割精度。实验表明该方法在公开数据集上显著优于现有对比学习SOTA方法，且推理时无额外计算开销。


<details>
  <summary>Details</summary>
Motivation: 语义分割领域虽然随着深度学习快速发展，但仍面临两大挑战：1) 有效捕获局部和全局上下文信息；2) 解决长尾分布问题（类别不平衡）。现有方法难以同时兼顾多尺度特征融合和类别平衡，导致分割精度受限，尤其是对少数类的分割效果不佳。

Method: 提出Contextrast++方法，包含两个核心组件：1) 上下文对比学习(CCL)：包含自适应融合模块（动态平衡局部和全局特征）、像素-锚点(PA)损失（利用融合的多尺度特征改进表示学习）、锚点-锚点(AA)损失（使用记忆库存储类平衡的代表性锚点解决长尾问题）；2) 边界感知负采样(BANE)：从错误分类的边界区域选择困难负样本，细化细粒度细节。

Result: 在公开数据集上进行广泛实验验证，Contextrast++显著提升了语义分割性能，超越了现有基于对比学习的SOTA方法，且在推理阶段不引入额外计算开销。

Conclusion: Contextrast++通过创新的对比学习框架有效解决了语义分割中的上下文捕获和类别不平衡问题，为长尾分布场景下的语义分割提供了新的解决思路，具有重要的实用价值。
...
TL;DR: MorphoCLIP是一个对比学习模型，将Cell Painting显微镜图像与化学物质、CRISPR敲除和ORF过表达的文本描述关联起来。该模型冻结视觉和语言骨干网络，仅训练紧凑的跨通道模块和投影层，可在单个消费级GPU上训练。在CPJUMP1数据上，双向检索（图像到文本、文本到图像）的top-10准确率均显著高于随机水平。


<details>
  <summary>Details</summary>
Motivation: Cell Painting显微镜技术可捕获细胞在化学或基因扰动后的变化，但将图像与产生这些变化的扰动关联起来仍很困难，因为生物效应微妙且技术变异较大。需要一种方法来组织大型成像筛选数据，使其易于搜索和解释。

Method: 使用对比学习框架，构建连接Cell Painting图像与文本描述的MorphoCLIP模型。冻结视觉和语言骨干网络，仅训练跨通道模块和投影层以降低成本。引入复制对齐损失使重复实验的图像特征更一致。

Result: 双向检索任务中，正确匹配出现在top-10结果中的频率远超随机预期。复制对齐损失确实提高了重复实验的图像一致性，但并未转化为可靠的成功匹配。基因感知标签和板校正未显示出一致的检索益处。

Conclusion: 文本监督可帮助组织化学和基因扰动的Cell Painting数据。然而，化学物质与基因扰动的匹配仍是一个未解决的问题。
...
TL;DR: 本文提出一种介于生成式和判别式方法之间的半生成式目标放置方法，通过在背景上均匀分布锚点，融合前景和背景特征来预测每个锚点的合理性分数和可placement集，在OPA数据集上实现了效率与效果的良好平衡。


<details>
  <summary>Details</summary>
Motivation: 之前的目标放置方法分为生成式和判别式两种，但这两种方法都无法很好地平衡效率与效果。

Method: 提出一种半生成式方法：在背景上均匀分布锚点，融合前景和背景特征，预测每个锚点的合理性分数，并对正锚点预测可placement集。

Result: 在OPA数据集上的实验表明，该方法能够很好地平衡效率与效果。

Conclusion: 半生成式方法是目标放置任务中一种有效且高效的方法，为生成式和判别式方法之间提供了良好的中间方案。
...
TL;DR: 本文是首届统一去除雨滴和反射挑战赛的综述 workshop 论文。该挑战赛旨在解决自动驾驶领域中雨天雨滴-反射复合退化这一实际难题，吸引了149名注册参与者，收到12份有效最终提交方案。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶在雨天行驶时面临的实际问题是本文的核心动机。雨滴和反射的复合退化会影响自动驾驶系统的感知能力，需要开发统一的去除方法。

Method: 所有方法均在真实的RainDrop and ReFlection (RDRF)数据集上开发和评估。挑战赛收到了12份带有说明文档的有效最终提交。

Result: 挑战赛收到149名注册参与者和12份有效提交，显著推动了统一去除雨滴和反射技术的进展。报告提供了对提交方法的详细分析和有趣的研究见解。

Conclusion: 该挑战赛和这篇综述论文为统一去除雨滴和反射领域的研究提供了重要贡献，突出了有效方法并为未来研究提供了有价值的洞察。
...
TL;DR: SeeU是一种新型通用3D高斯喷溅(G-3DGS)框架,用于稀疏视角下的新视角合成,通过语义条件高斯 refinement解决部分观测或遮挡区域的结构坍塌问题。


<details>
  <summary>Details</summary>
Motivation: 现有G-3DGS框架受限于像素对齐的高斯估计,难以处理部分观测或遮挡区域,经常导致表面不完整或结构坍塌。

Method: 提出Semantic-in-Gaussian设计:1)跨视图熵感知(CEA)模块聚合多视图语义和几何线索到紧凑embedding;2)条件高斯Transformer应用残差更新粗粒度高斯,恢复欠约束区域同时保持表面一致性。

Result: 在多个基准数据集上,SeeU持续提升渲染质量和结构完整性,保持高效的前馈推理。在挑战性的外推设置下,PSNR平均提升2.44 dB。

Conclusion: SeeU通过语义条件高斯refinement有效解决了稀疏视角下部分观测结构的重建问题,在渲染质量和结构完整性方面均优于现有SOTA方法。
...
TL;DR: 本文提出Object-Uni，一个统一的对象中心空间理解和可控生成模型，将对象姿态作为显式几何变量，连接姿态感知、空间推理、姿态条件生成和新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有统一模型缺乏理解和管理对象实例空间状态的能力，无法精确表示连续对象姿态并在目标视角下生成几何一致性的图像。

Method: 1)将对象中心空间智能表述为连接姿态感知、空间推理、姿态条件生成和新视角统一的联合问题；2)将对象姿态作为显式几何变量而非预测标签；3)提出基于视角的方向抽象，将方向映射为结构化视角描述；4)构建UniSpatial-80K对象中心空间基准；5)使用对象token锚定的姿态锚来关联实例与姿态状态。

Result: 模型在对象级姿态理解和姿态可控生成方面取得提升，使统一模型从描述对象走向操作空间状态。

Conclusion: 通过将姿态作为显式几何变量并构建统一框架，Object-Uni成功实现了对象中心空间智能的理解与生成统一，推动了多模态大语言模型在空间状态操作方面的发展。
...
TL;DR: ByteAction是一个基于字节级动作识别的基础模型，能够直接从压缩图像比特流中识别人类动作，无需像素解码。该方法具有文件完整性独立性和对比特流损坏的鲁棒性，适用于隐私敏感场景。


<details>
  <summary>Details</summary>
Motivation: 现有动作识别方法依赖完整像素解码，在隐私敏感场景和比特流损坏场景下受限。Byte-space Action Recognition (BAR)直接在字节空间操作，具有固有的隐私保护性和损坏鲁棒性，但需要提高对损坏比特流的识别准确性。

Method: 提出Dual-view字节级识别框架，包含弱损坏和强损坏两个比特流视图。核心技术创新包括：(1) Bitstream Pattern Augmentation (BPA)：将一维字节序列重塑为二维字节矩阵，并应用区域级擦除，增强模型学习跨区域字节依赖关系；(2) Corruption Consistency Training：使用双向KL散度约束模型在不同损坏程度下保持稳定预测；(3) 共享ByteFormer编码器处理双视图。

Result: 在Stanford40、PPMI和PASCAL VOC 2012 Action三个数据集上进行实验，ByteAction在保持竞争力的完整比特流性能同时，实现了所有场景下的最先进损坏鲁棒性。

Conclusion: ByteAction作为BAR基础模型，能够在损坏的图像比特流上实现准确的动作识别，为隐私敏感场景和比特流损坏场景下的动作识别提供了有效解决方案。
...
TL;DR: LagrangeGS提出将动态3D高斯溅射表述为非保守拉格朗日系统，通过速度-黑森矩阵近似和局部刚性对齐解决了物理一致性、时间可逆性和长期外推稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 现有物理感知扩展仅将速度场拟合到视觉变形，未满足拉格朗日力学，导致物理不一致轨迹、缺乏时间可逆性和长期外推时几何崩溃三大问题。

Method: 将动态3DGS表述为非保守拉格朗日系统；近似速度-黑森矩阵为单位矩阵以解耦粒子动力学；限制非保守力显式时间无关以实现时间可逆性；引入局部刚性对齐正则化粒子轨迹。

Result: 在动态场景基准测试上的广泛评估表明，LagrangeGS能够实现稳定的长期外推、一致的时间逆转和无需重训练的反事实物理编辑。

Conclusion: LagrangeGS通过将动态3DGS表述为非保守拉格朗日系统从根本上解决了物理不一致性问题，通过速度-黑森矩阵近似为单位矩阵来克服计算瓶颈，并引入局部刚性对齐来防止长期外推中的几何崩溃。实验表明该方法能够实现稳定的长期外推、一致的时间逆转和反事实物理编辑。
...
TL;DR: 提出Lever-Edit框架，通过将文本到图像(T2I)奖励迁移到图像编辑任务中，实现无需编辑奖励的强化学习图像编辑。


<details>
  <summary>Details</summary>
Motivation: 图像编辑的强化学习缺乏专门的奖励模型，现有编辑奖励需要昂贵的 triplet 监督和复杂的任务相关校准；而T2I领域已拥有成熟的奖励生态系统，因此提出问题：能否在不使用编辑奖励的情况下进行图像编辑RL？

Method: 提出两阶段框架 Lever-Edit：1) 学习一个奖励对齐的 captioner 用于生成反事实目标描述；2) 冻结 captioner，仅使用迁移的 T2I 奖励优化编辑策略。

Result: 实验表明，该方法在编辑对齐和源图像保留方面与基于编辑奖励的微调方法具有竞争力，并优于直观的迁移基线。

Conclusion: 标准图像编辑维度可以被映射到 T2I 奖励空间，无需专门设计编辑奖励即可实现有效的 RL 图像编辑。 
...
TL;DR: OmicSync是一个可靠性感知的空间多组学框架，将无监督域聚类与证据约束的LLM推理相结合，通过模型衍生的每点信号（分配置信度、认知路由不确定性和模态路由权重）生成多种解释。该方法在4个10x CytAssist FFPE空间蛋白质组学基准数据集上取得了最佳或次佳的排名，OmicSync-R进一步提升了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 现有的空间域发现方法仅提供聚类分配，缺乏分配可靠性、模态贡献以及为何应信任某个域决策的解释能力。这限制了方法的可解释性和可信度。

Method: OmicSync框架：1)KAN-GCN骨干网络结合空间编码、跨模态融合、不确定性感知路由、细胞类型监督和缺失模态补全；2)利用每点信号（分配置信度、认知路由不确定性、模态路由权重）构建结构化证据字典；3)生成标准、分步、反事实、对比和不确定性聚焦等多种解释。OmicSync-R：使用自动计算的推理质量分数作为REINFORCE奖励，使推理一致性塑造潜在结构。

Result: OmicSync在人类扁桃体(1.44)、胶质母细胞瘤(1.78)和扁桃体Add-on(1.22)上获得最佳平均排名，人类乳腺癌(2.33)上获得第二名。OmicSync-R将人类乳腺癌的ARI从45.73提升至46.72，在9项聚类指标中的6项上优于现有方法。

Conclusion: OmicSync和OmicSync-R实现了可靠性感知、可在点级别审计的空间域发现，由证据约束推理指导。未来工作将扩展到更多模态和更大规模的图谱数据集。
...
TL;DR: 本文提出GuidedFlow，一种基于注意力引导的归一化流模型，用于3D打印中的异常检测和定位。该方法利用预训练ResNet模型结合时空注意力网络，解决小数据场景下微小/拉丝缺陷的检测难题，并在AM3D-AD和MVTec-AD数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 3D打印（增材制造）在质量控制方面面临两大挑战：打印缺陷和网络物理入侵。现有的归一化流方法虽然能处理未知缺陷和泛化问题，但在检测微小/拉丝缺陷时存在局限性，且在小数据场景下泛化能力不足。

Method: 提出GuidedFlow模型，包含三个核心组件：1）预训练ResNet模型，在领域数据上进行微调；2）注意力引导的时空流框架，多尺度多帧建模动态特征；3）时空注意力网络(SAN)，使流模型能够优先处理输入帧中的相关上下文线索。

Result: 在AM3D-AD数据集（包含良性和解剖异常的3D打印物体图像和视频）和MVTec-AD工业图像异常检测数据集上进行了评估。实验结果表明，GuidedFlow在检测准确率和AUROC方面优于大多数先进模型。

Conclusion: GuidedFlow通过注意力引导机制有效提升了异常检测和定位性能，成功解决了小数据场景下微小缺陷检测的难题，展现了在工业质量控制中的应用潜力。
...
TL;DR: VersaDB是一个专为AI数据集设计的多模态数据库，采用基于页的存储系统分离结构化和非结构化数据，通过B+树索引、自动分片和分层元数据管理实现高效数据访问，支持多种AI数据格式转换，可实现最高5.35倍加速。


<details>
  <summary>Details</summary>
Motivation: AI领域的快速发展导致大量不同类型的AI训练数据集出现，这些数据集包含文本、图像、音频等多种模态，数据存储格式多样。随着AI硬件（GPU、TPU、NPU）的进步，对更快数据处理的需求增加，但现有AI处理框架在处理不同模态和存储格式的数据时，由于数据布局和用户处理方式等问题，处理速度可能不理想。

Method: 实现了基于页的存储系统，分离结构化和非结构化数据；生成基于B+树的索引文件加速数据访问；支持自动分片；维护分层元数据管理系统（页级、分片级和全局级）；提供API用于直接将数据集转换为VersaDB，并支持流行AI数据存储格式（CSV、TFRecord、.bin）的转换。

Result: VersaDB可实现最高5.35倍的加速效果，并在不同并行度下保持一致的性能。

Conclusion: 实验表明，VersaDB可实现最高5.35倍的加速效果，并在不同并行度下保持一致的性能表现。
...
TL;DR: 本文系统研究了文本驱动的图像到视频(I2V)生成中多主体视频生成的训练-free方法，评估了直接生成、并行生成和顺序生成三种范式在多主体场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 多主体视频生成面临挑战：模型需要同时保持每个主体的外观、分配不同的运动、并维持连贯的时空交互。现有方法缺乏系统性对比研究。

Method: 对三种代表性范式进行实证评估：1)直接生成：将完整参考图像和提示应用于预训练I2V模型；2)并行生成：将图像和提示分解为特定主体的视觉和文本条件，独立生成后合成；3)顺序先生成背景视频，再逐步引入各个主体。

Result: 三种范式各有权衡：直接生成需联合合成所有主体和运动；并行生成降低每步复杂度但弱化主体间上下文；顺序生成保留累积场景上下文但对主体顺序敏感且存在误差传播。

Conclusion: 研究发现各范式的优势和局限，为设计可控的多主体视频生成系统提供实践洞察。评估维度包括：外观保持、运动保真度、时间一致性和主体间一致性。
...
TL;DR: SiZeUp是一种从校准的倾斜航空影像快速构建大规模3D城市代理模型的方法，通过高度-足迹表示将3D建筑抽象简化为低维优化问题，使用序数深度一致性损失实现高效稳健的高度估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理大规模城市场景时效率低下，通常需要密集点云重建或显式特征匹配，且单目深度估计的度量深度在尺度模糊情况下不可靠。

Method: 采用高度-足迹表示方法，将建筑足迹通过单一高度参数挤出；引入序数深度一致性损失，通过可微分渲染器将参数化建筑代理映射到多视角深度图像，强制代理渲染深度与单目深度模型预测的深度先验在相对深度排序上保持一致；结合高效动态视角选择策略。

Result: 相比最先进的代理重建管道实现23-52倍加速，同时保持相当的代理级覆盖率和体积一致性。

Conclusion: 该方法非常适合大规模城市建模任务，在效率和精度之间取得了良好平衡。
...
TL;DR: VeCAS是一个两阶段的血管聚焦无对比剂血管造影合成框架，将血管结构定位与血管造影外观合成分离，用于从非对比剂X射线图像生成血管造影图像。


<details>
  <summary>Details</summary>
Motivation: X射线血管造影依赖碘对比剂来可视化血管结构，但对比剂给药存在不良反应风险，需要开发无对比剂替代方案。现有问题包括：(i)血管定位控制不足，(ii)背景内容建模效率低下。

Method: 提出两阶段框架：阶段I中，判别模型在非对比剂X射线图像中定位血管结构，跨模态潜在蒸馏从X射线血管造影中转移血管敏感知识；阶段II中，血管聚焦修复模型在定位的血管区域内合成血管造影外观，同时保留非血管背景。

Result: 在下肢血管介入数据集上，VeCAS在血管结构保真度和图像质量方面优于对比方法。视觉图灵测试和医师评估表明合成血管造影具有感知真实性。机器人导丝导航实验显示，VeCAS引导相比无对比剂引导，目标到达时间减少41.4%，操作步骤减少40.7%。

Conclusion: VeCAS有望作为血管介入的"元对比剂"，在减少对比剂使用的同时实现精确血管可视化，具有重要的临床应用前景。
...
TL;DR: 本文提出MGMVFI，一种基于选择性状态空间模型(Mamba)的视频帧插值方法，通过运动引导序列化(MGS)利用光流定义自适应扫描顺序，解决传统SSM固定扫描顺序无法有效建模动态运动轨迹的问题。


<details>
  <summary>Details</summary>
Motivation: 状态空间模型(SSMs)在视频帧插值(VFI)中展现出前景，可线性复杂度捕获长距离依赖，但其预定义的扫描顺序限制了建模VFI中动态运动轨迹的有效性。

Method: 提出三大核心组件：1)运动引导序列化(MGS)利用光流定义运动自适应的1D输入顺序，使因果状态更新与语义相关标记对齐；2)上下文合成利用周围空间上下文缓解光流估计不准确导致的特征表示不可靠问题；3)轻量级细化块增强局部细节重建。这些组件集成在定制的Mamba架构中。

Result: 在标准VFI基准测试中取得最先进性能，特别是在复杂和动态运动场景下表现优异。

Conclusion: 为视频插值中的序列建模建立了新方向，验证了运动引导方法与状态空间模型结合的有效性。
...
TL;DR: 在亚千字节预算（1024或512字节）下存储人脸图像用于身份验证时，不同编解码器的性能差异显著。1024字节时问题接近解决（Color FERET EER<0.35%），但512字节时编解码器性能重新排序，传统JPEG等崩溃，而WebP和定制学习编解码器表现较好。


<details>
  <summary>Details</summary>
Motivation: 身份文档、智能卡生物识别和带宽受限的验证场景需要将人脸图像存储在亚千字节预算内。通用编解码器优化像素保真度而非身份验证所需的嵌入距离，因此需要研究哪种编解码器、分辨率和设置最适合在≤1024字节下保留身份信息。

Method: 基准测试10种通用和人脸特定编解码器，涵盖不同分辨率、字节预算、两个数据集（Color FERET和AI-Solutions-KK）和四个人脸匹配器。使用14个ViT和CNN模型确认排名与骨干网络无关。然后训练自定义身份保持编解码器，通过冻结增益表的二进制搜索精确命中字节预算，并进行四项研究：分辨率、人口统计公平性、再压缩和无盒对抗鲁棒性。

Result: 1024字节时问题接近解决：现代编解码器在ArcFace上使Color FERET等错误率低于0.35%。512字节时领域重新排序：AVIF、HEIF、JPEG XL和传统JPEG在FMR 1e-4下崩溃，假非匹配率达28-98%，而WebP、JPEG-AI和定制学习编解码器保持在该范围之外，WebP为24.3%，定制准确变体为6.9%（在野外数据上）。

Conclusion: 亚千字节身份保持可行，但部署哪种编解码器完全取决于预算。在1 kB选择的编解码器不是在半预算时部署的编解码器——512字节时的排名与1024字节时不同，这才是操作层面的关键发现。
...
TL;DR: LaST是一个用于手术阶段识别的大-小模型协作框架，通过零样本适应解决领域偏移问题。FM生成帧级相位先验，轻量级模型利用时序建模能力纠正不一致预测，循环回放策略实现自我增强的标签质量和模型性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前手术阶段识别模型面临领域偏移问题：轻量级模型虽能捕捉时序动态但泛化性差；Foundation Models虽迁移性好但缺乏时序建模导致预测不一致。需要结合两者优势实现零样本领域适应。

Method: 1.FM生成帧级相位先验作为弱监督；2.迭代时序细化：动态质量控制过滤可靠预测+双模型交叉学习减轻确认偏差；3.轻量级模型纠正不一致预测；4.循环回放策略将精化预测作为后续迭代的监督信号

Result: 在零样本手术阶段识别任务中，LaST相比PeskaVLP基线提升24.85%-43.17%准确率，超越全监督线性探测和多种少样本方法

Conclusion: LaST通过大-小模型协作和循环自增强机制成功实现零样本领域适应，为手术阶段识别的临床部署提供了有效解决方案
...
TL;DR: FOVEA是一种针对视觉语言模型的多模态推测解码方法，通过构建可重用的视觉记忆缓存，根据当前解码状态动态检索视觉证据，避免固定视觉界面的局限性，实现高达2.13倍的加速。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定视觉界面（如预定义视觉token预算或静态压缩表示），但作者发现不同任务和解码阶段对视觉信息的需求差异很大：证据不足会削弱视觉接地，过多上下文则增加开销并干扰解码。现有方法无法自适应调整视觉输入量。

Method: 提出FOVEA方法：1)构建可重用的视觉记忆缓存；2)基于累积质量规则确定检索的视觉条目数量和内容；3)将选中的视觉条目聚合为视觉读出；4)通过轻量级门控残差修正将视觉读出与当前解码隐藏状态融合，而非将视觉token插入自回归上下文。

Result: 在多个视觉语言骨干模型和基准测试上，FOVEA提高了draft acceptance和端到端解码速度，最高达2.13倍加速。

Conclusion: 状态条件化的证据检索是替代固定视觉表示的有效方案，可根据解码需求动态调整视觉信息量，显著提升多模态生成效率。
...
TL;DR: DRAgent是一个基于多模态大语言模型(MLLM)的指代表达分割框架，通过判别推理(DR)机制替代传统的坐标生成方式，先构建检测器候选空间，再利用MLLM作为目标判别器进行两阶段筛选，最终使用分割模型生成像素级掩码。


<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的RES方法通常采用一次性坐标预测进行视觉定位，将连续空间位置序列化为离散文本令牌，容易导致定位偏差和对齐错误。

Method: 1. 构建检测器生成的候选空间而非让MLLM直接生成坐标；2. 采用两阶段判别推理机制（先筛选高召回率候选框，再进行实例级验证）；3. 选中的目标框作为空间提示输入基础分割模型；4. 构建自一致性过滤的推理链数据管道用于LoRA微调。

Result: 在RefCOCO、RefCOCO+和RefCOCOg数据集上取得了竞争力的性能表现。

Conclusion: 通过判别推理框架和可靠的监督信号，DRAgent有效解决了MLLM在RES任务中的定位偏差和对齐错误问题，为多模态大语言模型在视觉定位任务中的应用提供了新思路。
...
TL;DR: NemoSplat是首个面向非校准海洋视频的媒体感知动态重建前馈4D高斯溅射框架，能够从水下视频中重建逼真的动态场景。


<details>
  <summary>Details</summary>
Motivation: 无约束水下环境中的光散射和不可预测的动态物体导致现有视觉基础模型在特征聚合时失败，造成严重的跟踪和重建失败。

Method: 提出Promptable Dynamic Disentangler（利用学习动态概率和语义文本先验的置信度感知融合策略隔离瞬态实体）和Media-Aware Gaussian Predictor（联合估计内在3D高斯属性和物理媒体参数，在单次前向传播中渲染 pristine 场景外观）。构建大规模水下数据集用于训练和评估。

Result: 在自建数据集上实现最先进的跟踪精度和高保真渲染效果。

Conclusion: NemoSplat成功解决了水下环境中动态重建的难题，为海洋视频的4D场景重建提供了有效的解决方案。
...
TL;DR: AquaFlow是一个用于高效高保真水下重建的单目高斯溅射流式重建框架，解决了水下场景中因光衰减和散射导致的相机位姿跟踪和场景几何失真问题。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D Gaussian Splatting流式重建方法在应用到水下场景时面临严峻挑战，因为水下严重的光衰减和散射会导致相机位姿跟踪性能下降和场景几何失真。

Method: 提出三个关键技术：1) 在大规模水下数据上微调3D视觉基础模型以实现稳健的位姿和点图估计；2) 引入介质引导的增量高斯初始化策略用于流式映射；3) 开发流式兼容的混合场景表示，结合结构化距离条件神经高斯与物理启发的光学模型来补偿水下成像效应。

Result: 在62条不同水下轨迹的综合数据集上进行了评估，与WaterSplat-SLAM相比，平均定位误差降低13.2%，PSNR提升4.74 dB，达到了最先进的跟踪和渲染性能。

Conclusion: AquaFlow能够实现高效且高保真的水下重建，为水下场景的3D重建提供了有效的解决方案。
...
TL;DR: 第1届异步CASTLE挑战赛总结报告，发表于CVPR 2026联合自我中心视觉研讨会


<details>
  <summary>Details</summary>
Motivation: 为自我中心视觉领域提供一个异步任务的基准挑战赛

Method: 在CVPR 2026联合自我中心视觉研讨会上举办第1届异步CASTLE挑战赛

Result: 产出了该挑战赛的贡献和结果总结

Conclusion: 为异步CASTLE挑战赛提供了首个基准数据和评估框架
...
TL;DR: 本文提出将注视表示为事件对齐的固定视野角位移，作为一种可解释的、事件条件化的运动词汇，用于解决 gaze 数据集之间表示不一致的问题，并在多个基准测试中比较了不同表示方法的迁移性能。


<details>
  <summary>Details</summary>
Motivation: gaze 作为视觉和多模态模型的输入信号越来越重要，但数据集之间缺乏统一的表示方法。原始数据保留了细节但存在噪声和设备依赖性，粗粒度事件标签虽易于建模但可能丢失局部运动结构。

Method: 提出事件对齐的固定视野角位移作为运动词汇，并与事件-only、空间、绝对角度、学习向量量化(VQ)和连续表示进行比较。评估结合了 next-token 预测、目标域 regret、低阶目标引用、配对bootstrap、顺序敏感性、motif 重叠和冻结结构探测。

Result: 在事件对齐的头戴式基准测试中，角运动 tokens 在一个迁移方向上比冻结码本 VQ tokens 有更低的 target-domain regret，反向则不确定。探测揭示了互补的表示属性，event-only tokens 表明低困惑度可能保留很少的运动信息。在第三个自我中心数据集上，I-VT、native 和 frame-span 接口的匹配比较表明事件构建会显著改变迁移结果。

Conclusion: 基于运动的 token 化为事件对齐的自我中心 gaze 流提供了紧凑表示，评估揭示了目标可预测性和事件构建如何影响跨数据集结论。
...
TL;DR: 本文提出了一种用于PCB引脚自动缺陷检测的方法，结合U-Net语义分割、基于轮廓的特征提取和逻辑回归分类器，在工业数据集和公开数据集上分别达到0.990和1.000的ROC-AUC值，优于PatchCore异常检测和实例分割方法。


<details>
  <summary>Details</summary>
Motivation: PCB组装过程中的引脚错位检测是一个具有挑战性的检验任务，现有方法难以实现高精度、自动化的检测，因此需要开发一种有效的自动化缺陷检测方法。

Method: 提出了一种结合语义分割和逻辑回归的流水线方法：1)使用U-Net架构进行语义分割；2)从分割掩码中提取引脚轮廓表示；3)提取板级特征（如平均轮廓大小）；4)使用逻辑回归分类器进行板级通过/失败分类。同时与PatchCore异常检测方法和基于实例分割的引脚检测进行了对比。

Result: 在工业数据集的随机测试集上达到ROC-AUC 0.990，在公开数据集上达到ROC-AUC 1.000，表明该方法在不同视觉特征的数据集上都具有较强的通过/失败板分离能力。

Conclusion: 所提出的方法是一种有前景的工业环境自动化引脚检测方案，在经过数据集特定训练后，能够在具有显著不同视觉特征的数据集上实现强性能，优于现有的异常检测和实例分割方法。
...
TL;DR: WildHandBench是一个针对手写文档理解的新基准测试，包含500份手写文档，涵盖三种结构（四种语言、九个真实场景）。提出先验驱动误差(PDE)指标来区分语言先验与视觉证据导致的错误。最佳模型仅达71.85%，人类基线为77.09%，模型错误中63-91%为先验驱动，揭示了模型对语言先验的系统性依赖。


<details>
  <summary>Details</summary>
Motivation: 现有文档理解基准存在局限：仅关注印刷文档（已达96.34%准确率）、聚焦孤立文本或公式、忽略手写表格和真实退化场景、仅报告聚合准确率而未解释失败原因。当前模型处理挑战性手写文档的能力尚不明确。

Method: 构建WildHandBench基准：500份手写文档，包含三种结构（自由文本、表格、公式）、四种语言、九个真实退化场景。提出先验驱动误差(PDE) metric，量化模型错误源于语言先验而非视觉证据的程度。评估18个最先进模型并设置校准人类基线。

Result: 最佳模型仅达71.85%总体准确率；人类表现（77.09%）优于所有模型但差距很小。模型错误与人类错误有本质区别：63-91%的模型错误为先验驱动，而人类仅为49%。

Conclusion: 揭示了当前模型对手写文档处理时系统性依赖语言先验的问题，这一问题无法被传统准确率指标捕获。PDE指标能有效区分错误来源，为改进手写文档理解提供新方向。
...
TL;DR: 提出了一种简单、无需运动的跨模态标定框架，利用时间调制的混合ChArUco目标在标准消费显示器上显示，实现事件相机与帧相机的外参标定。


<details>
  <summary>Details</summary>
Motivation: 事件相机与帧相机之间的精确外参标定仍然是异构立体系统的实际瓶颈，现有方法通常需要传感器或目标运动、精确同步或计算昂贵的事件到图像重建。

Method: 使用在标准消费显示器上呈现的临时调制、混合ChArUco目标，通过在原始模式和部分混合版本之间交替，使目标可靠地触发事件，同时保持帧相机的持续可见性。将事件离散化为与RGB图像粗略对齐的帧，应用轻量级去噪，并执行基于ChArUco的内参和立体外参标定。

Result: 与最强运动参考(E2Calib + Kalibr)和非运动参考(Plasberg等人)相比，该方法分别将平均重投影误差降低44%和6%，同时显著简化了标定程序。在机器人眼手标定案例中展示了实用性，即使在部分遮挡下也能获得一致的变换和稳定的下游几何测量。

Conclusion: 该方法实现了事件相机与RGB相机的精确标定，具有误差低、简单实用的优点，适用于机器人视觉应用。
...
TL;DR: 本文提出了一种基于专家混合引导的协同优化框架(MoeCo),用于手术动作三元组识别,旨在解决机器人辅助手术中复杂手术场景的分析问题。


<details>
  <summary>Details</summary>
Motivation: 现有手术动作三元组识别方法面临三个主要问题:(1)组件级优化冲突(特征空间纠缠)、(2)类别级优化冲突(严重数据不平衡)、(3)缺乏领域知识指导导致模型可解释性和鲁棒性受限。

Method: 提出MoeCo框架包含三个核心组件:1)组件定制适配器 - 解耦跨时空任务特定特征,促进组件专业化;2)协调梯度学习策略 - 自适应平衡正负梯度,增强稀有类别感知;3)知识驱动专家混合机制 - 动态整合多模态大语言模型引导的知识。

Result: 在公开的CholecT45和CholecT50数据集上进行广泛实验,验证了协同优化管道的有效性,以及通过知识驱动专家混合机制整合动态先验的优越性。

Conclusion: 所提方法能有效解决手术动作三元组识别中的特征纠缠、数据不平衡和领域知识缺乏等问题,为手术场景理解提供了更表达性强且鲁棒的表示。
...
TL;DR: ENCORE是一个基于熵引导的视觉语言模型框架，通过推理时的熵基裁剪策略(ECS)和训练时的熵正则化训练(ERT)来动态保留与提示相关的图像区域完整性，仅微调0.14%的参数即可在10个VQA基准上实现平均1.43%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于transformer的视觉编码器将图像分割成固定分辨率的子图像，破坏了轻量级VLMs中的对象完整性；现有方法仅关注视觉模态，无法动态保留与提示相关的区域完整性，限制了性能。

Method: 1. 发现早期层图像-文本交叉模态注意力熵与答案定位质量和任务准确率强相关；2. 推理时使用熵基裁剪策略(ECS)评估候选裁剪区域，选择熵值最小的区域；3. 训练时使用熵正则化训练(ERT)，在next-token预测中加入熵项以锐化对关键视觉token的关注。

Result: 在10个VQA基准上，仅微调0.14%的参数即可实现平均1.43%的准确率提升，在2B参数VLMs中达到最先进水平。

Conclusion: 通过熵引导的方式可以有效保留与提示相关的视觉区域完整性，ENCORE框架证明了早期层交叉模态注意力熵可作为任务性能的可靠指标，为轻量级VLMs的图像理解提供了有效的解决方案。
...
TL;DR: 本文提出DAGC（密度感知图构建）方法，解耦长视频理解中检索索引粒度与证据粒度，通过合并视觉冗余的相邻块来构建紧凑的自适应图索引，同时保留到原始时间单元的映射，在MLVU、VideoMME和LongVideoBench上实现40-50%节点减少、1.3-1.7倍加速且保持99%性能。


<details>
  <summary>Details</summary>
Motivation: 现有图基RAG系统继承视频分割的固定时间粒度构建检索索引，但粗粒度表示足以定位相关时间区域，细粒度证据对下游推理更重要，当前设计不必要地耦合了索引粒度与证据粒度。

Method: 提出训练无关的DAGC方法：1）构建查询无关的粗粒度检索索引，独立于原始细粒度证据空间；2）通过合并视觉冗余的相邻块构建紧凑的密度自适应图索引，同时保留到原始时间单元的映射；3）检索到的粗粒度区域再扩展回原始块粒度进行细粒度证据精炼和答案生成。

Result: 在MLVU、VideoMME和LongVideoBench上，DAGC仅保留约40-50%原始图节点，实现1.3-1.7倍端到端加速，同时保留约99%原始问答性能。该增益可迁移至不同LVLM主干和视频RAG流水线。

Conclusion: 长视频RAG无需在索引和证据推理中保持相同的时间粒度，解耦两者可显著提升效率而几乎不损失性能，为长视频理解提供了更scalable的方案。
...
TL;DR: Misanthrope是一种基于自蒸馏训练的隐私保护关键点检测器，通过避免在人物上检测关键点来防止图像特征被逆序攻击从而泄露隐私场景内容，在保持图像匹配性能的同时，在Image Matching Challenge 2021 Phototourism测试集中9个场景中7个取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 图像匹配是SLAM、视觉定位和SfM的核心组件，但当前基于局部图像特征的方法容易受到逆序攻击，攻击者可以从局部特征重建隐私敏感的场景内容，尤其是在分布式计算场景中，预计算的特征从边缘设备传输到远程服务器时面临隐私泄露风险。

Method: 提出Misanthrope，一种通过自蒸馏训练的关键点检测器，其核心思想是从源头避免在人物上检测关键点（因为人物是大多数定位场景中主要的隐私敏感内容来源），从而在特征提取阶段就缓解逆序攻击，而非事后混淆。

Result: 实验表明，传统特征检测管道中的逆序图像可用于检测和重新识别场景中的人物，而Misanthrope能够有效缓解此类攻击。Misanthrope在图像匹配性能上与最先进方法相当，甚至在人物作为干扰因素的挑战性场景（如旅游照片和野外里程计）中表现更优。在Image Matching Challenge 2021 Phototourism测试集中，Misanthrope在9个场景中7个成为最佳稀疏特征提取器。

Conclusion: 通过自蒸馏训练让关键点检测器主动避开人物区域是一种有效且实用的隐私保护策略，既能从源头防止逆序攻击导致的隐私泄露，又不影响图像匹配的核心功能，为边缘计算和分布式视觉系统的隐私安全问题提供了新的解决思路。
...
TL;DR: AnaDiffusion是一种用于3D脑MRI生成的新型解剖学组合潜在扩散框架，将生成过程分解为不同的解剖学区域，实现部分到整体的组装和全局细化，能够在推理时无需受试者特定密集分割图的情况下进行可控部分编辑。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型通常整体合成3D体积，往往忽略区域解剖结构，限制局部可控性。

Method: 首先训练部分扩散模型以捕获局部结构先验，然后将组装好的解剖学组合注入全脑潜在表示并继续去噪，从而在保留注入解剖学的同时解析全局上下文。

Result: 在ADNI测试集上，AnaDiffusion在全脑、左右半球、小脑-脑干复合体和接缝区域实现最低FID；在小脑和脑室/脑干的绝对Cohen's d值方面表现最佳或次佳；局部编辑实验中配对MS-SSIM表现出高目标迁移和脱靶保留。

Conclusion: AnaDiffusion能够生成显式部分资产和全局连贯体积，实现可控部分替换，同时保持部分到整体脑结构的一致性，最小化意外解剖学改变。
...
TL;DR: 本文研究counterfactual医学图像生成中的身份保持问题，评估了三类文本条件编辑方法（source-anchored、structured-prompt、paired-training）在视网膜OCT图像上的表现，发现所有方法都能生成高质量图像，但身份保持能力差异显著。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设通过锚定源图像可以隐式保持身份，但这一假设在生物特征细微的领域（如视网膜OCT）未经测试，需要明确衡量身份保持能力。

Method: 使用referee classifiers、embedding alignment scores和盲读研究三种方法评估三类文本条件编辑方法的身份保持能力。

Result: 所有方法都能生成高质量OCT图像，编辑效果相当，但身份保持差异明显：source-anchored方法常改变主体身份，paired-training方法保持最好。

Conclusion: 未来医学counterfactual生成研究必须明确测量和报告身份保持能力，与图像真实性和编辑成功率同等重要。
...
TL;DR: 本文提出文化时刻基准(CMB)，用于评估视频中的文化理解能力，将文化理解分解为三个能力：命名概念、视觉识别和时间定位。通过306个来自东南亚七国专家策划的概念进行三阶段测试，发现即使最强模型在三个阶段都正确时得分低于30%，且三个能力不会完全级联。


<details>
  <summary>Details</summary>
Motivation: 现有视频文化基准倾向于测试可见内容，将三种能力合并为单一分数，无法识别瓶颈。需要一个能够诊断具体能力缺陷的基准。

Method: 设计三阶段评估：S1根据描述从四个概念名称中选择；S2从四个候选视频时刻中选择；S3预测视频中子事件的起止时间。采用语义相似干扰项、无标签视频时刻和不同示例视频的自由形式定位来隔离每种能力。评估六个视觉语言模型。

Result: 最强闭源模型三阶段全对得分低于30%；命名正确只能帮助半数模型识别视觉元素，识别对时间定位影响很小；音频在非拉丁文字国家多为干扰项；移除音频和字幕对游戏和音乐影响最大。专家评分者在邻国概念上低于随机水平。

Conclusion: CMB作为一个诊断工具，能够将失败归因于特定能力或模态，为改进视频文化理解提供了针对性的评估框架。
...
TL;DR: 本研究揭示了视觉语言模型(VLM)在空间推理中的工作机制，发现空间关系预测遵循"粗粒度锚定→推理"的分阶段过程，物体对齐的token建立粗略的目标-参考锚点，而不需要精确的边界框边界。位置信息在关系决策之前即可解码，一小部分注意力头同时介导定位和空间推理的因果效应。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型可以回答空间问题，但将物体定位连接到空间推理的机制仍然不清晰。之前不清楚空间推理是否需要在内部进行精确的物体定位，还是可以通过全局布局线索绕过显式定位。

Method: 使用一套机制可解释性工具研究LLaVA-1.5和Qwen2.5-VL两个代表性模型家族，包括：token消融、层间探测、注意力 knockout 和因果中介分析。

Result: 1) 空间关系预测遵循分阶段的定位到推理过程，物体对齐的token建立粗略的目标-参考锚点，无需精确边界框边界；2) 位置信息在关系决策出现之前即可解码；3) 一小部分注意力头介导了定位和空间推理的因果效应；4) 两个任务共享早期定位相关处理，但最终依赖部分不同的专门化路径。

Conclusion: VLM将物体定位转化为空间关系的过程涉及token级、层級和头级的精细机制，知道物体在哪里并不等同于知道物体如何相关。这为理解VLM的内部工作机制提供了token-、层-和头级别的解释。
...
TL;DR: 首次揭示DiT中不同注意力层的位置嵌入具有不同的位置控制能力，最显著的层作为锚点。基于此提出锚定位置嵌入移位策略，将DiT的时序感知从直线转变为圆环，开发Loopy框架用于高质量循环视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常无法生成高质量的循环视频，因为忽视了视频生成模型如何感知时间顺序以及这与循环行为的关系。

Method: 提出锚定位置嵌入移位策略，根据各层的时间控制效果应用层特定的移位长度，将DiT的时序感知从直线转变为圆环。开发了通用框架Loopy，支持RGB和RGBA视频生成及高级AIGC功能。

Result: 实验表明，该方法显著提升了生成循环视频的时序一致性和视觉质量。

Conclusion: Loopy框架通过锚定位置嵌入移位策略，将DiT的时序感知从直线转变为圆环，有效解决了视频循环生成中的时序一致性问题。该方法支持RGB和RGBA视频，并可实现身份控制和风格迁移等高级AIGC功能，显著提升了循环视频的时序一致性和视觉质量。
...
TL;DR: PeFuse是一个无需训练的组合图像检索(CIR)框架，利用预训练的扩散模型和多模态大语言模型通过生成转换桥接模态。该方法将CIR转换为四个单模态检索问题，提出了单向和双向转换策略，无需针对特定任务进行训练即可实现零样本CIR。


<details>
  <summary>Details</summary>
Motivation: 传统的CIR方法依赖多模态融合将视觉和文本特征组合成联合查询嵌入，需要训练模块来对齐组合查询与目标图像。这些方法通常需要大量特定任务的训练数据，且缺乏灵活性。本文旨在提出一种无需训练的框架，利用预训练模型的能力来解决这些问题。

Method: 提出PeFuse框架，包含两种转换策略：单向转换和双向转换，将CIR转换为四个单模态检索问题。具体方法包括：1)利用扩散模型根据参考图像和文本描述生成目标图像的伪样本；2)将组合查询转换为纯文本或纯图像查询；3)利用预训练的文本到图像检索模型进行匹配。关键创新在于通过生成式转换而非显式特征融合来桥接模态。

Result: 在标准基准数据集上的广泛实验表明，将CIR转换为文本到图像检索任务比其它转换策略更有效。PeFuse在零样本条件下达到了与先进方法相当或更优的性能，同时保持了较高的灵活性，因为转换管道的各个组件均可替换。

Conclusion: 本文提出的伪融合范式对于零样本组合图像检索是有效的。通过利用预训练扩散模型和MLLM进行生成式转换，可以绕过需要针对任务进行专门训练的限制，同时保持 competitive 的检索性能。这种方法展示了预训练模型在组合图像检索任务中的巨大潜力。
...
TL;DR: 本文提出DaL-MoE，一种针对低光UAV桥梁图像的退化感知恢复前端，通过ISP感知的低光合成管道和互补专家模块提升桥梁缺陷检测性能，在YOLOv11m上实现box mAP50从0.3097提升至0.4923，mask mAP50从0.2281提升至0.3529。


<details>
  <summary>Details</summary>
Motivation: 低光条件会遮挡桥梁图像中的小尺寸、低对比度缺陷，降低自动检测的可靠性和操作灵活性，需要研究退化感知图像恢复能否改善低光下的桥梁损伤检测，并实现从合成退化到真实场景的迁移。

Method: 提出DaL-MoE检测器无关的恢复前端，采用ISP感知的低光合成管道进行训练，配备退化感知引导估计和噪声抑制、色彩调整、结构细节恢复三个互补专家模块。

Result: 在成对合成数据上DaL-MoE达到23.12dB PSNR和0.8482 SSIM，使YOLOv11m的box mAP50从0.3097提升至0.4923，mask mAP50从0.2281提升至0.3529；在无成对参考的真实低光UAV图像上，sim-to-real评估显示缺陷可视化和检测完整性均有改善。

Conclusion: 退化感知图像恢复可有效提升低光条件下的桥梁缺陷检测性能，具有从合成数据到真实场景的迁移能力，未来将开发更强跨场景泛化能力的低光感知桥梁损伤检测器。
...
TL;DR: 本文提出3DTongueQA框架，通过模拟器构建带有肌肉激活标签的舌部运动数据，解决现有 articulatory 语料库缺乏肌肉驱动过程可追溯标签的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于实时MRI和电磁 articulography 的舌部语料库虽然能捕捉舌形和运动，但无法提供生成观察到的构型的肌肉驱动过程的追溯标签，限制了生物力学研究和对自然语言的深入理解。

Method: 使用ArtiSynth Badin有限元模型，将受控的11维肌肉激活映射到固定拓扑的舌部网格，转换为结构化生物力学记录，并渲染为关于肌肉状态、几何形状和目标导向变化的确定性QA。构建了295,115个有效网格和891,156条每种语言的QA记录。

Result: 基线模型达到62.9±9.2 Muscle EM、74.0±0.2 Value Accuracy和65.9±4.7 Direction EM；配对网格不匹配时Muscle EM降至2.2。保留锚点的模型保留80.4-98.6%全量分数；任务特定结构化输出达到88.7±0.7 Muscle EM和93.3±1.0 Direction EM。

Conclusion: 所构建的监督支持高效结构化预测和异构自然语言QA，不局限于特定解码器架构，且英语和韩语实例证明框架具有语言层面的可移植性。
...
TL;DR: 本文提出了多视角鱼眼图像生成的新问题，并提出两种方法：SyntheOcc-FE和MIVIFI。MIVIFI通过跨域学习利用等距矩形投影来弥补鱼眼数据集的不足，实现高保真的场景内容操作。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机可低成本实现360°覆盖，但现有鱼眼数据集有限，生成罕见corner case需要计算昂贵的3D模拟。生成模型在标准透视图像上成功，但未探索广角畸变应用。

Method: 1) SyntheOcc-FE：适配SyntheOcc架构到鱼眼数据；2) MIVIFI：利用跨域学习与等距矩形投影，结合KITTI-360鱼眼图像和nuScenes多视角标准图像，修改语义占用输入添加/消除特定目标，渲染不同气象和光照条件。

Result: 定量和定性实验表明方法实现鲁棒的照片级多视角鱼眼图像生成，跨域策略有效解决数据稀缺问题。

Conclusion: 成功引入多视角鱼眼图像生成问题，两种方法均能实现逼真渲染，MIVIFI的跨域学习策略在处理数据稀缺方面具有特定优势。
...
TL;DR: 本研究系统评估了训练-free token merging方法（ToMe和Mutual Pair Merging）在小麦表型分析任务中的性能。研究涵盖生长阶段分类、麦穗检测和器官分割三个任务，评估指标包括任务质量、吞吐量、token数量和峰值GPU内存，并在Raspberry Pi 5上进行部署测试。


<details>
  <summary>Details</summary>
Motivation: 基于Vision Transformers (ViTs)的小麦表型分析面临二次方注意力计算带来的高吞吐量和边缘推理限制问题。训练-free token merging技术可在不重训练的情况下插入已训练模型，具有重要应用价值，但其在不同表型任务中的适用性尚不明确。

Method: 研究对ToMe和Mutual Pair Merging两种token merging方法进行系统基准测试，任务涵盖生长阶段分类、麦穗检测和小麦器官分割。评估指标包括任务质量、吞吐量、token数量和峰值GPU内存，并在Raspberry Pi 5上完成部署测量。

Result: 基准测试揭示了明确的任务层次：分类任务对token合并高度容忍，而检测和分割任务受限于重复实例、细薄器官、密集边界、重建需求和运行时开销。优化的注意力后端可能消除表面加速效果，部署价值需在目标运行时上实际测试而非仅从token数量推断。

Conclusion: 训练-free token merging在分类任务中具有实用价值，但检测和分割任务需谨慎应用。部署性能评估应基于目标硬件的实际测量，不能仅依赖token数量变化推断加速效果。
...
TL;DR: 本研究提出了一个语言无关的前列腺癌活检报告生成框架，使用德语演示，通过自动化管道利用LLM将复合报告拆分为核心级别的图像-文本对，在无需人工标注的情况下从2,402个历史病例中生成17,344个配对数据。模型在恶性肿瘤检测和Gleason分级上分别达到96.2%和65.2%的F1分数，与FDA批准的分类器相当。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是全球最常见的恶性肿瘤之一，每个活检核心的结构化报告给病理学家带来沉重负担。现有工具将其框定为分类任务，且许多幻灯片级视觉-语言模型依赖英语中心编码器，在其他临床语言中迁移效果不佳。因此需要一种语言独立的报告生成框架。

Method: 1. 从头训练分词器和模型（语言无关架构）；2. 使用本地部署的LLM自动将复合报告拆分为核心级别的图像-文本对，解决配对数据稀缺问题；3. 在德语数据集上训练和验证；4. 在三个外部队列上使用潜在空间增强进行分级验证。

Result: 恶性肿瘤检测F1为96.2%，Gleason分级F1为65.2%，与FDA批准的分类器性能相当。在三个外部队列上验证了分级的有效性。

Conclusion: 该框架可让各机构利用自身档案训练原生语言报告模型，实现语言无关的活检报告生成，为非英语临床环境中的AI辅助病理诊断提供了可行方案。
...
TL;DR: BenthicFlow是一个统一的框架，基于单一条件流匹配模型，共同生成对齐的纹理和深度图，用于生成3D海底环境。该框架采用MultiDiffusion启发的采样程序来协调重叠窗口，无需单独的拼接模型即可生成可空间扩展的RGBD拼贴画。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉在水下3D场景理解中的应用面临挑战，原因在于缺乏高质量3D数据，且表面训练的模型无法泛化到水下场景。现有方法使用单独训练的模型后处理拼接独立生成的瓦片，仅在单个调查场地内展示异构景观。

Method: 提出BenthicFlow框架，基于单一条件流匹配模型联合生成对齐的纹理和深度图。采用MultiDiffusion启发的采样程序在生成轨迹中协调重叠窗口，实现无需单独拼接模型的可空间扩展RGBD拼贴画生成。生成的拼贴画使用表面对齐的高斯surfels提升为明确的3D海底环境。

Result: 在地理上不同的调查场地进行的实验表明，BenthicFlow在生成连贯的大规模3D场景的同时保留了场地特定外观，密切匹配目标分布。

Conclusion: BenthicFlow成功创建了连贯的3D海底环境，同时保留了场地特定特征，为水下3D场景理解提供了有效的生成框架。
...
TL;DR: 本文提出了一种基于条件生成对抗网络(cGAN)的视图合成框架，用于恢复光场相机阵列中缺失或故障的子孔径图像。该方法利用精心选择的邻域相机图像和位置编码图来生成缺失视角，实现了几何一致性和 photometric 准确的重建。实验表明该方法在合成和真实数据集上均优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 光场成像系统需要密集的空间采样来实现重聚焦和深度估计等后处理功能，但实际应用中相机阵列常出现硬件故障导致部分相机失效，造成子孔径图像缺失和重建质量下降，需要一种鲁棒的解决方案来恢复缺失视角。

Method: 提出一种基于条件生成对抗网络(cGAN)的新型生成框架，通过仔细选择邻域相机的子图像作为输入，结合位置编码图（指示邻域位置和目标视图位置），训练网络生成缺失视角。位置编码同时包含邻域相机位置信息和期望的目标视图位置信息。

Result: 在合成和真实光场数据集上进行了广泛实验，结果表明该方法能够生成视觉上合理且光度准确的重构，在定量和定性评估上均优于基线的视图插值方法。

Conclusion: 所提出的框架为故障容错光场图像采集提供了一种鲁棒且高效的解决方案，能够有效恢复缺失视角，提升光场相机阵列在实际应用中的可靠性。
...
TL;DR: EchoWM是一个用于可进入生成媒体的全模态世界模型，能够在响应连续导航的同时联合生成720p视频、环境声音、音乐和语音。


<details>
  <summary>Details</summary>
Motivation: 构建一个可进入的生成媒体全模态世界模型，能够同时生成视频、声音、音乐和语音，并响应用户导航交互。

Method: 1）基于相机意图组织交互：第一人称场景指定观察者运动，第三人称场景从数据中学习相机-角色动态；2）将离散命令和连续姿态映射到共享的度量尺度相对6-DoF轨迹，数据集级校准保持运动幅度；3）构建互补数据引擎，采用渐进训练和自回归后训练进行长时域生成。

Result: 在公开世界模型基准上实现强轨迹跟随和高视觉效果，支持第一人称和第三人称跨不同主体的交互，并在长时域生成中保持同步的环境声音和语音。

Conclusion: EchoWM是一个有效的全模态世界模型，能够在可进入生成媒体中联合生成高质量视频、音频内容，并响应导航轨迹控制。
...
TL;DR: 本研究探讨了单一扩散模型能否作为多种CT成像任务的通用先验，用于解决稀疏视角或低剂量CT重建问题。该方法在三种不同模态、几何结构、材料和退化类型的数据集上进行了验证，包括增材制造金属零件的锥束X射线CT和混凝土微观结构的平行束中子CT。


<details>
  <summary>Details</summary>
Motivation: CT扫描吞吐量受限于扫描时间，而高质量体积重建通常需要针对特定扫描设置训练的神经网络先验。当模态、几何结构或材料发生变化时，需要重新训练模型，缺乏一种可跨域使用的通用先验方法。

Method: 提出一种跨域扩散模型方法，使用单一冻结模型（frozen model）在多个成像域上训练，作为多种CT问题的通用先验。模型在不同模态、束流几何、材料和退化类型的数据集上进行评估。

Result: 所提方法在所有三种数据集上均优于解析重建方法，验证了单一扩散模型可作为异构CT重建问题的可复用基础先验。

Conclusion: 该研究为异构CT重建问题提供了可复用基础先验的第一步，表明跨域扩散模型有望成为一种通用的CT重建先验方法，具有重要的实际应用价值。
...
TL;DR: 本文提出球面占据轮廓(Spherical Occupancy Profiles)作为多视角3D高斯重建的统一中间表示,用于判别式和生成式3D重建任务。


<details>
  <summary>Details</summary>
Motivation: 当前3D重建方法缺乏一种紧凑、可学习且能同时支持判别和生成任务的统一表示。本文旨在填补这一空白,提出将每条射线的占据概率轮廓作为连接多视角重建与生成先验的接口。

Method: 在Google Scanned Objects数据集(999个训练对象,90个测试对象,每个48个旋转视角)上: (i)训练了判别式每射线解码器,将全局视图平均和射线特定图像证据注入FiLM条件化的轮廓头; (ii)构建了基于轮廓VAE和潜在扩散模型的生成式管道,支持无条件采样和图像条件下的多解重建。

Result: 判别式模型达到归一化中位数软深度误差0.035;生成式管道可实现与重建流形匹配的无条件采样,以及通过无分类器引导可量化且可调的单对象多解重建扩散。形态学分析揭示了L1每射线损失族中存在的宽度-峰值前沿关系。

Conclusion: 射线级占据轮廓提供了一种紧凑、可学习且支持不确定性的接口,可连接多视角重建与生成先验,在合成和非合成数据上均验证了管道的有效性。
...
TL;DR: 本文提出了一种基于YOLO11迁移学习和ByteTrack跟踪算法的自动蜜蜂入口监控系统，用于检测和计数小型快速移动的蜜蜂。


<details>
  <summary>Details</summary>
Motivation: 解决自动蜜蜂入口监控的需求，应对蜜蜂体型小、移动速度快带来的检测和跟踪挑战。

Method: 采用YOLO11目标检测器进行迁移学习，结合ByteTrack跟踪算法；研究数据增强、背骨干冻结和跟踪器参数优化对检测计数的影响；采用渐进式背骨干解冻策略。

Result: 检测器达到97.0%精度和98.7%的mAP50，渐进式解冻策略比完全微调更稳定；轻量级增强优于重量级增强；在25FPS独立侧视视频中，系统正确计数了43只进入蜜蜂（91.5%）和7只离开蜜蜂（23.3%）。

Conclusion: 适度的数据增强、渐进式背骨干解冻和ByteTrack参数调优可提高自动蜜蜂入口监控在真实录制条件下的可靠性。
...
TL;DR: 本文提出了一种基于物理信息的自蒸馏框架（DINOv3+ConvNeXt-v2-Tiny），用于侧扫声纳图像的自监督学习。通过物理启发的数据增强和HSIC惩罚项实现视角不变性，并采用密集分层特征融合策略。在S3Seg数据集上仅用10%标注数据即可达到96%的最佳性能，mIoU为71.4%，准确率达86.5%。


<details>
  <summary>Details</summary>
Motivation: 侧扫声纳图像受物理声学 artifacts 影响严重，现有自监督学习方法采用自然图像的增强策略，未能考虑声学退化且无法显式强制视角不变性。

Method: 1. 基于DINOv3架构，采用ConvNeXt-v2-Tiny骨干网络提高数据效率；2. 物理启发的增强方法模拟斑点噪声、距离相关衰减和辐射校准；3. Hilbert-Schmidt Independence Criterion (HSIC) 惩罚项解耦学习到的密集 patch 特征与物理视角参数；4. 跨四个网络阶段的密集分层特征融合策略保留细粒度沉积物细节和深层语义抽象。

Result: 框架能够将复杂海底地形分组为稳定、无噪声的语义聚类，无需人工标注。在监督下游任务中，使用仅10%标注数据达到96%的绝对峰值性能，最终mIoU为71.4%，总体准确率为86.5%。

Conclusion: 所提出的物理信息自蒸馏框架有效解决了侧扫声纳图像的视角不变性问题，通过物理增强和HSIC惩罚实现了高效特征学习，显著提升了数据效率，为声纳图像分析提供了新的自监督学习范式。
...
TL;DR: 提出一个无需训练的音频引导视频目标分割框架，结合多模态大语言模型(MLLMs)和SAM分割模型，在LSVOS挑战赛MeViS-Audio赛道取得竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 当前音频引导视频目标分割任务缺乏有效的训练-free解决方案，希望利用基础模型的强大多模态推理能力来建模文本-视觉对应关系，并生成精确的目标分割掩码。

Method: 将任务分解为多个阶段，为每个阶段选择合适的基础模型：利用MLLMs强大的多模态推理能力建模文本-视觉对应关系，使用SAM-based模型进行精确的目标掩码生成，无需额外的模型训练或任务特定的微调。

Result: 在第8届LSVOS挑战赛的MeViS-Audio Track中取得竞争力的性能。

Conclusion: 成功验证了利用基础模型进行音频引导视频分割的有效性，为该领域提供了一种新的训练-free解决思路。
...
TL;DR: Mover360是一个针对360°全景图像的可控物体操作框架，支持物体平移、插入和删除任务，通过统一控制接口和轻量级扩散Transformer实现，性能优于现有透视图像编辑方法


<details>
  <summary>Details</summary>
Motivation: 解决360°全景图像中物体操作的难题。与透视图像不同，360°图像具有水平环绕、纬度依赖失真和全局场景连续性等特点，导致现有透视图像编辑器难以处理物体级编辑任务

Method: 提出Mover360框架：1)支持物体平移（主要任务），同时支持参考引导的插入和删除作为辅助任务；2)统一点、边界框和掩码引导控制，将每项任务编码为固定提示和紧凑的ERP对齐指令图；3)单点模式可推理物体尺寸、支撑和光照；4)采用预训练扩散变换器的轻量级适配；5)构建UE5数据生成管线，包含表面感知物体放置和随机光照，生成大规模配对数据和双域基准测试

Result: 在合成和真实全景图像的两个测试域和两种评估协议下，Mover360在重建保真度、语义一致性和分布质量方面均优于透视编辑、插入和修复的强基线方法

Conclusion: Mover360是一个可控的360°图像物体操作框架，能够有效处理全景图像中的物体平移、插入和删除任务，性能优于现有方法
...
TL;DR: 本文研究多态射频成像中的语义分割问题，结合传统逆问题求解器与3D U-Net网络，实现对视野内体素的语义分类和目标实例检测。在多种噪声比条件下，所提方法的语义重建和目标检测性能均优于传统强度重建，且能通过异常暴露训练识别未知类别目标。


<details>
  <summary>Details</summary>
Motivation: 多态射频成像中反射特性具有各向异性，传统强度重建在噪声环境下性能急剧下降，且无法提供语义信息。需要一种能同时实现语义分割和实例检测的方法。

Method: 对每对发射-接收阵列应用标准逆问题求解器生成图像，将六个 Tx-Rx 对的重建结果输入至训练好的3D U-Net进行隐式融合和显式分类。通过几何后处理（聚类和主成分分析）提取目标实例和定向边界框。使用异常曝光训练未知类别。

Result: 语义重建（以分割交并比评分）和3D检测性能随信噪比下降远比传统强度重建更加稳定。检测性能在强度重建已完全失效的噪声水平下仍保持可靠。未知类别目标可被正确识别而非误分类。

Conclusion: 将传统逆问题成像与深度学习语义分割相结合的多态射频成像方法，能够在低信噪比环境下实现鲁棒的语义重建和目标检测，为实际应用中的开放世界识别提供了有效解决方案。
...
TL;DR: E2S-Pruner是一种无需辅助模型、训练参数或微调的视觉token剪枝框架，采用两阶段证据融合方法：先利用注意力头作为独立证据源估计可靠性并表示token状态，再利用Dempster-Shafer证据理论量化层间冲突并融合互补证据，同时引入空间新颖性约束确保不同图像区域的覆盖。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型将图像编码为数百个视觉token，导致推理延迟和GPU内存开销过大。现有剪枝方法主要依赖注意力分数，难以表征证据不确定性和冲突。

Method: 两阶段证据融合框架：第一阶段将每个注意力头视为独立证据源，从证据清晰度和头间一致性估计可靠性，将视觉token表示为重要、不重要和不确定三种状态；第二阶段使用Dempster-Shafer证据理论量化层间冲突并融合多层互补证据；引入空间新颖性约束促进不同图像区域的覆盖。

Result: 在LLaVA-1.5-7B上，E2S-Pruner在保留192、128和64个token时分别保持98.0%、96.8%和90.6%的聚合性能，在128-token和64-token设置下吞吐量分别提升1.96倍和2.09倍。在Qwen2-VL-7B上验证了跨模型泛化能力。

Conclusion: E2S-Pruner是一种有效的视觉token剪枝方法，无需训练或微调即可实现显著的性能保持和推理加速，具有良好的跨模型泛化能力。
...
TL;DR: HetSkills是一个用于物理角色控制的新型框架，通过在统一潜在空间中逐步学习异构技能。该框架将潜在空间视为可执行接口，整合来自不同数据源、监督形式和任务的学习技能。


<details>
  <summary>Details</summary>
Motivation: 当前物理角色控制方法难以有效整合来自不同来源的多种技能，需要一个统一的框架来无缝集成异构技能以完成长时域任务。

Method: 1. 首先学习跟踪技能，建立运动控制基础并创建可复用的共享运动解码器；2. 引入运动直觉蒸馏将文本到运动生成与语言语义关联；3. 设计任务指导模块根据高级语言指令动态调整动作。

Result: 在运动跟踪、文本到运动生成、运动补全和下游任务适应方面表现出色，即使在挑战性条件下也取得了令人印象深刻的成功率。

Conclusion: HetSkills能够保持自然运动的同时持续扩展技能库，具有高度适应性，为物理角色控制领域提供了有效的异构技能整合解决方案。
...
TL;DR: DG-Mem是一个双粒度代理记忆框架，通过非参数外部存储增强冻结的多模态大语言模型(MLLM)，旨在解决科学和数学推理能力不足的问题。该框架基于人类记忆的互补学习系统(CLS)理论构建。


<details>
  <summary>Details</summary>
Motivation: 前沿多模态大语言模型在感知方面表现优异，但在科学和数学推理方面仍存在不足。参数级适应对于封闭权重或设备端骨干模型不可用，而无状态提示则无法从已解决的问题中获益。

Method: DG-Mem将记忆分为两个层面：(1)实例级示例记忆和(2)类别级模式记忆(IF-THEN规则)。采用两种关键设计：(1)在线概念分类器，在训练过程中增量扩展类别空间；(2)Shapley上下文归因程序，对整个检索规则集进行正确性分解，产生每规则效用值来重新加权检索。

Result: 在MathVista、MMMU和MMMU-Pro上，使用四个开放权重和专有骨干模型(Qwen3.5-27B、Qwen3.5-122B-A10B、GPT-5-Nano、Gemini-3-Flash)进行测试，DG-Mem始终优于无记忆基线和竞争性记忆基线。

Conclusion: 该管道无需梯度更新，可部署在封闭权重或设备端骨干模型上，为增强冻结MLLM的推理能力提供了一种有效的非参数化解决方案。
...
TL;DR: 本文提出了DeMoDiff，一个统一的空间-时间解耦框架，用于文本驱动的人体动作合成。该框架重新设计了动作表示和生成架构，解决了基于VQ的方法存在的信息损失问题以及连续表示缺乏部分级灵活性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心问题：1) 基于VQ的方法因信息损失影响生成动作的质量、多样性和泛化能力；2) 连续表示对整体动作操作，缺乏部分级别的灵活性；3) 扩散和自回归扩散模型对身体各部分的细粒度可控性有限。

Method: 提出空间-时间VAE对每个身体关节分别编码，而非将整体动作压缩到单一潜在空间；并将空间-时间掩码和注意力机制融入自回归扩散生成器，实现生成能力和可控编辑性。

Result: 在HumanML3D和KIT-ML数据集上取得最先进的重建性能和新颖的动作生成结果，同时展示了强大的时间和空间编辑能力。

Conclusion: DeMoDiff框架通过解耦空间-时间表示与自回归扩散架构，成功实现了高质量动作生成与细粒度部分控制，为文本驱动的人体动作合成提供了新的研究方向。
...
TL;DR: 本文提出Spotter框架,一种在密集城市场景中实现鲁棒实时视觉定位的系统。该方法利用建筑立面作为全球地理参考源,在离线阶段处理街景全景图构建紧凑度量数据库,在线运行时通过级联检索和几何验证恢复精确的全球相机位姿。该方法在巴塞罗那行人佩戴智能眼镜采集的新数据集上进行了评估,表现优于基于里程计的基线方法,且在显著更高帧率下达到与先进地图方法相当的定位精度。


<details>
  <summary>Details</summary>
Motivation: 城市峡谷中GPS信号因多径传播而衰减,导致现有视觉定位方法面临挑战:视觉里程计存在漂移,地图匹配方法依赖可靠GPS先验且计算量过大难以实时运行。

Method: Spotter采用离线-在线两阶段架构。离线阶段:处理Google街景全景图,语义分割建筑立面,将多视角立体深度与制图数据结合构建紧凑度量数据库。在线阶段:查询图像通过级联检索和几何验证管道进行匹配,恢复细粒度全球相机定位。该框架可集成GPS信号(若有)。

Result: 在巴塞罗那多个区域使用可穿戴智能眼镜采集的行人序列新数据集上实验,Spotter优于基于里程计的基线方法,达到与先进地图方法相当的定位精度,且运行帧率显著更高。

Conclusion: Spotter利用建筑立面作为可靠地理参考源,实现了鲁棒实时的城市场景视觉定位,为机器人和可穿戴平台提供了一种有效的定位解决方案。
...
TL;DR: 本文研究了记忆式异常检测器在稀疏训练污染下的性能，分析了四种覆盖选择器（随机、medoid、局部、全局）的表现，并提出CLEANCON方法减少最终记忆污染。实验表明最低污染的记忆并未获得最高性能，污染程度与P-AP指标之间不存在单调排序关系。


<details>
  <summary>Details</summary>
Motivation: 记忆式异常检测器存储名义训练补丁并根据记忆进行评分，但选择覆盖的补丁时未检查其几何稀有性是否真正安全。这导致被选中的补丁可能包含污染样本，而现有研究缺乏对这种耦合关系的系统探查。

Method: 在固定表示和记忆预算下，比较随机、medoid、局部和全局四种覆盖选择器；使用CLEANCON（一种跨图像支持门控方法）改变候选图像 eligibility，同时固定表示、绝对记忆大小、构建器和推理规则。

Result: 全局覆盖选择器严重过度代表稀疏污染。CLEANCON将最终记忆污染降至约零，并在全部12次匹配比较中提升了类别宏P-AP。但在保留率扫描中，最低污染的记忆未达到最高P-AP；性能持续改善的同时污染也在上升。

Conclusion: 记忆污染程度不能作为P-AP排序的依据，污染与性能之间不存在单调关系。这表明仅减少污染不足以优化异常检测性能，需要重新审视记忆式检测器的设计原则。
...
TL;DR: 该论文研究了基于内存的异常检测器中训练补丁选择的安全性，提出CLEANCON方法减少内存污染，发现内存污染程度与检测性能并非简单正相关。


<details>
  <summary>Details</summary>
Motivation: 基于内存的异常检测器存储正常训练补丁并对其进行评分，但所选补丁成为正常参考时没有单独检查几何稀有性是否安全可信。研究旨在探测这种耦合关系在稀疏训练污染下的表现。

Method: 在固定表示和内存预算下，比较随机、medoid、局部和全局覆盖选择器。使用CLEANCON（一种out-of-bag跨图像支持门）来改变候选图像 eligibility，同时保持表示、绝对内存大小、构建器和推理规则不变。

Result: 全局覆盖强烈过度代表稀疏污染。CLEANCON将最终内存污染减少到约零，在所有12个匹配比较中提高了类别宏P-AP。但沿保留扫描，最低污染的内存未达到最高P-AP；性能持续改善而污染上升。

Conclusion: 全局覆盖选择器严重过度代表稀疏训练污染。CLEANCON方法将最终内存污染减少到约零，并在所有12个匹配比较中提高了类别宏P-AP。然而，最低污染的内存并未达到最高P-AP，性能在污染上升时仍持续改善，表明内存污染程度不能直接排序产生最高性能的内存。代码公开可用。
...
TL;DR: 本文提出Fashion CIG任务的新设置：基于自由形式指令的时尚互补图像生成，并推出StyleFlow模型。该任务要求模型根据种子图像和自然语言指令生成风格兼容的服装。研究者丰富了三个CIG基准数据集，添加了低、中、高特异性指令，并提出联合条件于种子图像和指令的单一多模态Transformer模型。


<details>
  <summary>Details</summary>
Motivation: 现有CIG基准测试依赖僵化的模板提示（如"一张裙子的照片"），无法反映自然用户查询，且模糊了模型在不同语言特异性水平下的行为表现。

Method: 1) 使用视觉语言模型生成低/中/高特异性指令，由人工标注者验证；2) 提出StyleFlow，一种矫正流匹配模型，在单一多模态Transformer中同时条件于种子图像和指令。

Result: StyleFlow在图像质量指标、目录对齐分析、消融实验和人工评估中均表现优异，能持续生成指令对齐且风格连贯的服装，同时相比辅助模块方法降低了架构复杂度和推理成本。

Conclusion: 自由形式指令设置能更好反映真实用户需求，StyleFlow通过单一多模态Transformer实现了高效且高质量的时尚互补图像生成。
...
TL;DR: VideoRover是一个统一的视频深度研究框架，迭代协调视频裁剪、多模态搜索和网页浏览，通过工具结果选择下一步行动，使局部视频剪辑指导外部检索，检索到的证据触发进一步视频检查和验证。


<details>
  <summary>Details</summary>
Motivation: 开放世界视频理解需要模型定位稀疏视觉证据并获取视频及其参数记忆中不存在的外部知识。Thinking-with-Videos和Deep Research两种能力通常孤立开发，缺乏统一框架。

Method: 构建自动化数据管道生成26K验证的SFT轨迹和3K RL实例；提出VideoRover-Bench基准测试，按视频时长和研究难度分层；训练VideoRover-8B-RL模型。

Result: VideoRover-8B-RL在直接回答设置中性能与专有模型相当，同时优于配备相同工具套件的大型开源模型。消融研究验证了主动视频定位、外部检索和长时序强化学习的互补作用。

Conclusion: VideoRover成功统一了视频时序感知和多步信息搜索能力，通过迭代协调视频裁剪、检索和浏览实现深度视频理解，为开放世界视频问答提供了有效框架。
...
TL;DR: 本文提出IntentQA任务和X-CaVIR框架，旨在让AI从视频中识别人类行为背后的意图（社会智能的"暗物质"），通过引入三种认知上下文（情境、对比和常识）增强视频意图推理，并利用LLM提升可解释性。


<details>
  <summary>Details</summary>
Motivation: 视频理解需要智能体超越视觉识别，理解人类行为背后的意图（社会智能的"暗物质"），但现有方法难以弥合视觉观察与意图推理之间的鸿沟，且标准指标可能因数据集偏差而高估模型能力。

Method: 提出X-CaVIR框架，包含三个模块：1)情境上下文通过跨模态视频查询语言(VQL)；2)对比上下文通过对比学习；3)常识上下文通过常识推理。并通过LLM生成五种对比集，引入"对比性能下降"指标评估鲁棒性。

Result: X-CaVIR在基准测试中优于现有最佳方法，并在对比集上表现出较强的抗干扰能力。

Conclusion: 通过整合三种认知上下文和透明的可解释管道，X-CaVIR能有效利用丰富常识知识提升性能，同时使推理过程具有明确的可解释性，为视频意图推理提供了新的研究方向。
...
TL;DR: 提出一种结合文献引导推理、自动代码生成和假设驱动实验的智能体AI科学家工作流，用于自动生成医学影像任务的竞争性深度学习基线模型，在四个公开基准上验证有效。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像深度学习基线模型的开发是一个高度迭代的过程，需要进行文献综述、实施、实验和专家细化。现有自动化方法通常只优化单个组件（如架构搜索或超参数调优），而非整个基线开发流程。

Method: 提出了一种智能体AI科学家工作流，结合文献引导推理、自动代码生成和假设驱动实验。该框架包含三个核心组件：文献搜索与推理模块、代码自动生成模块、以及迭代实验Pipeline。

Result: 在四个公开基准数据集上评估该框架，涵盖分割、分类和检测任务。实验Pipeline在所有任务中持续提升验证性能，取得了具有竞争力的排行榜结果：PUMA两个赛道第6名（共15支队伍）、MILK10k第31名（共125支队伍）。在MIDOG25上，生成的模型在扫描仪、肿瘤类型和物种间展现出强大的域泛化能力。

Conclusion: 所提出的智能体工作流可在无需任务特定重新设计的情况下，跨分割、分类和检测任务生成具有竞争力的医学影像基线模型，显著降低开发医学影像深度学习基线所需的人工工程工作量。
...
TL;DR: 提出SuperSharpen，一种基于扩散模型的盲去模糊方法，通过模糊度量提供恢复强度的显式控制，比较了ControlNet适配器和完整微调两种条件策略，实验表明微调方法能产生更好的保真度和更少的幻觉细节。


<details>
  <summary>Details</summary>
Motivation: 解决相机成像中的光学系统、传感器或低级处理导致的退化问题，特别是在专业摄影中，在未知退化核的情况下进行盲去模糊是一个具有挑战性的逆问题。

Method: 提出SuperSharpen，一种基于扩散模型的盲去模糊方法，通过模糊度量提供恢复强度的显式控制。比较两种 conditioning 策略：1) ControlNet风格的适配器（冻结骨干网络）；2) 完整微调扩散先验。

Result: 实验表明完整微调方法能达到更好的保真度，幻觉细节更少。在合成和真实世界模糊图像上验证，展示了更好的感知质量和可控的恢复强度。

Conclusion: 通过完整微调扩散先验的方法在盲去模糊任务上优于ControlNet风格的适配器方法，能够在保持图像真实性的同时有效控制恢复强度。
...
TL;DR: DF-MoE是一个用于音视频深度伪造检测的框架，通过使用多种预训练模型提取高层特征（嘴部动作、面部解析、表情、头部姿态、视线追踪、心率、音频情感和语音活动），并结合mixture-of-experts（MoE）骨干网络进行融合，在五个基准数据集上取得了优于现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测面临的主要挑战是开发能够泛化到不同生成方法的检测器。作者假设通过预训练模型提取多种高层线索可以缓解过拟合问题。

Method: 1. 使用多种预训练模型提取多模态特征：嘴部动作、面部解析、面部表情、头部姿态、视线追踪、心率估计、音频情感和语音活动检测
2. 通过MoE（混合专家）骨干网络整合单模态和多模态线索进行深度伪造检测

Result: 在五个基准数据集（MAVOS-DD、AVLips、PolyGlotFake、BioDeepAV、FakeAVCeleb）上进行了域内和跨域实验，DF-MoE取得了最优的深度伪造检测性能，超越了所有竞争方法。

Conclusion: 通过提取多种高层语义线索并利用MoE架构进行多模态融合，可以有效提升深度伪造检测的泛化能力和检测精度。
...
TL;DR: JoyAI-Echo-1.5是一个统一的音视频生成系统，包含两个专用变体：长视频变体支持跨镜头记忆和语音过滤的说话者线索，实现角色外观和声音身份在灵活 conditioning组合下的持久性；世界模型变体将导航输入转换为6-DoF相机轨迹，实现控制器无关的交互。


<details>
  <summary>Details</summary>
Motivation: 视频生成正从孤立片段向长篇叙事和交互式世界发展，需要模型保持身份一致、遵循用户控制、在扩展生成中保持稳定。

Method: 长视频变体：引入可组合跨镜头记忆，聚合多镜头视觉证据和语音过滤音频的说话者线索；世界模型变体：异构导航输入转换为校准度量6-DoF相机轨迹，通过几何感知 conditioning路径注入；训练：渐进式教师强制+短长程自梯度强制将双向 backbone转为因果少步生成器。

Result: 长视频变体在跨镜头一致性、视觉质量、文本对齐、语音保真度上超越基线；世界模型变体在WBench排名第一(81.7分)，SANA-WM-Bench上获领先视觉质量和长期持久性。

Conclusion: 记忆机制、几何控制、rollout感知训练为生成连贯故事和持续演进的交互式世界提供了实用基础。
...
TL;DR: MomADv2是一个用于长时域端到端自动驾驶的状态空间记忆框架，通过选择性状态空间规划记忆查询模块过滤历史规划查询，基于时间连续性和命令一致性选择相关规划模式，并通过流匹配轨迹残差细化器学习从精化规划输出到专家轨迹的连续残差校正场。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过时间记忆提高规划连续性，但当驾驶命令改变时，这种记忆可能失效并误导决策。因此，如何选择性利用有用历史同时抑制与命令不一致的记忆仍是一个关键挑战。

Method: 提出两个核心组件：1) 选择性状态空间规划记忆查询模块，根据时间连续性和命令一致性过滤历史规划查询，选择与当前命令相关的规划模式；2) 流匹配轨迹残差细化器，从精化规划输出到专家轨迹学习连续残差校正场，实现细粒度轨迹精化同时保持锚点规划的稳定性。

Result: 在闭环NAVSIM和Bench2Drive以及开环nuScenes上的广泛实验表明，MomADv2提高了长时域规划一致性，在6秒规划下平均碰撞率比MomAD降低15.6%。

Conclusion: MomADv2通过选择性状态空间机制有效解决了命令变化时记忆失效的问题，显著提升了长时域自动驾驶规划的安全性和可靠性。
...
TL;DR: 提出了一种基于下一代尺度自回归范式的人体视角合成方法，能够在高分辨率下实现多视角输出和跨视角一致性，通过合成人脸数据集训练，可生成清晰逼真的新视角图像，并可与3D高斯 lifting 模型结合用于精确的 photorealistic 3D 人脸建模。


<details>
  <summary>Details</summary>
Motivation: 在高空间分辨率和跨多目标相机条件下进行真实感人物新视角合成仍然具有挑战性，保持身份特征、精细外观细节和几何一致性至关重要。

Method: 基于下一代尺度自回归范式，针对人体中心视角合成进行适配，支持更高图像分辨率、多视角输出和单次前向传播中的更强跨视角一致性。训练使用合成人脸数据集，无需2D预训练，可利用低分辨率通用预训练，仅在最后训练阶段使用全尺寸专用图像。

Result: 模型生成清晰逼真的视角，可选择同时合成多个新视角以提高视角间一致性。在人体受试者上观察到感知保真度和跨视角一致性的提升。

Conclusion: 下一代尺度自回归是 可扩展、多输出人体视角合成的有效骨干网络。与现有基于transformer的像素对齐3D高斯lifting模型结合，可实现准确且真实感的人脸3D建模。
...
TL;DR: 研究将规范建模（normative modelling）应用于头颈部CT放疗计划中器官风险分割的错误检测，比较了VAE框架与图像条件分割扩散模型两种方法。


<details>
  <summary>Details</summary>
Motivation: 放疗计划中器官风险分割的审核既耗时又主观，需要自动化的分割质量控制方法来检测分割错误。

Method: 在RADCURE数据集的大脑干和脊髓分割上使用模拟边界和宽度扰动进行评估，采用Dice相似系数和DTA（Agreement距离）作为评价指标。

Result: 两种模型都能检测到部分模拟错误，但区域DTA显示扩散模型在定位细微边界错误方面表现更一致。

Conclusion: 图像条件扩散重建作为局部解剖感知的分割QA框架具有良好前景。
...
TL;DR: BasketballBench是一个多模态基准测试，包含7,980个问题，涵盖文本、图像和视频三种模态，用于全面评估篮球比赛理解能力。该基准结合了2025-2026 NBA赛季的官方比赛记录、530名现役球员名单及2,501个进攻回合的广播片段。同时提出BasketballSkills代理，通过组合八个篮球特定感知和检索工具来理解篮球比赛。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要单独评估篮球比赛的各项能力（事件识别、动作定位、球员识别等），而忽视了这能力之间的交互。篮球比赛理解需要同时运用多种能力，但目前缺乏一个能够综合评估这些能力交互的基准。

Method: 1) 构建 BasketballBench：包含7,980个问题，涵盖10个任务，覆盖文本、图像和视频三种模态，数据来自2025-2026 NBA赛季，包括官方比赛记录、530名现役球员信息及2,501个进攻回合的广播片段；2) 提出 BasketballSkills代理：组合8个篮球特定感知和检索工具，分为4个可重用技能，指定工具顺序、证据绑定和停止条件。

Result: 当前多模态大语言模型(MLLMs)在需要整合多种能力的题目上表现较差，而 BasketballSkills 明显优于这些模型，证明了显式组合领域特定能力的有效性。

Conclusion: 1) BasketballBench 为篮球比赛理解提供了全面的多模态评估基准；2) 通过显式组合领域特定工具的BasketballSkills代理在综合理解任务上显著优于现有MLLMs；3) 领域特定能力的组合对于实现全面的篮球理解至关重要。
...
TL;DR: 本文提出了一种基于几何驱动的框架，用于侧扫声纳(SSS)与光学图像的像素级配准和视角不变反射率映射。该方法利用运动恢复结构(SfM)重建海底密集3D网格作为几何锚点，通过首次底部返回(FBR)算法校正高度漂移，并应用逆Lambertian模型和双高斯加权函数分离固有海底反射率，从而消除斜程传播损失和几何视角依赖性。


<details>
  <summary>Details</summary>
Motivation: 侧扫声纳是水下大面积测绘的主要手段，但自动感知和跨模态对齐受限于声学复杂性（如斑点噪声、阴影和极端视角依赖性）。传统手工描述符和现代深度学习匹配器无法在没有3D几何约束的情况下弥合光学与声学图像之间的物理域鸿沟。

Method: 1) 利用运动恢复结构(SfM)重建密集3D海底网格作为几何锚点；2) 引入首次底部返回(FBR)提取算法动态校正非线性的高度漂移；3) 应用逆Lambertian模型和双高斯加权函数分离固有海底反射率，有效消除斜程传播损失和几何视角依赖性；4) 将声学特性与光学像素确定性关联，生成精确配准的多模态数据集。

Result: 该pipeline能够生成高度准确、严格配准的多模态数据集，消除了对人工标注的需求，为底栖栖息地测绘的自监督学习提供了可能。

Conclusion: 这种自动化、物理引导的方法无需人工注释即可实现光学-声学跨模态配准，为底栖栖息地测绘的自监督学习奠定了基础。
...
TL;DR: GeoWAM是一种新的视觉几何世界动作模型，用于自动驾驶。它不预测未来图像，而是预训练预测未来场景几何（点云），然后利用几何条件动作头预测自我轨迹。实验表明，基于几何的预训练目标比基于图像的方法能产生更强的驾驶策略。


<details>
  <summary>Details</summary>
Motivation: 现有的世界动作模型(WAMs)在像素空间中学习场景动态，但像素只是间接表示：它将几何和运动与外观、纹理、光照混在一起，迫使模型从2D观察推断3D变换。作者认为点云提供的几何表示更自然，因为它明确捕获空间结构和控制场景演化的刚性和非刚性变换，直接对齐驾驶动作执行的空间。

Method: GeoWAM的预训练目标是预测未来场景几何（点云），而不是预测未来图像。这种几何预训练产生同时编码空间结构和时间演化的表示。然后使用几何条件动作头利用学习到的几何动态来预测未来自我轨迹。

Result: 广泛的开放环和封闭环评估表明，视觉几何世界建模比基于图像的替代方案产生 substantially stronger driving policies，确立了未来几何预测作为自动驾驶的有效预训练目标。

Conclusion: 未来几何预测是一个有效的自动驾驶预训练目标。基于几何的表示比基于图像的表示更适合驾驶任务，因为几何明确捕获空间结构并直接对齐驾驶动作执行的空间。
...
TL;DR: 提出了一种利用Vision Transformer (ViT)学习参数几何结构分析其内部表示的方法。通过SVD分解权重矩阵，将激活投影到右奇异向量获取紧凑表示，拟合类条件密度模型产生典型性分数，堆叠成典型性图。提出两种OOD检测分数：原型对齐分数(PAS)和多层软投票分数(MLSV)。


<details>
  <summary>Details</summary>
Motivation: 当前ViT的OOD检测方法需要重新训练或暴露OOD样本，本文旨在提出无需重新训练和OOD暴露的后验OOD检测方法。

Method: 1. 对每个仿射层的权重矩阵进行奇异值分解(SVD)；2. 将激活投影到主导右奇异向量获取紧凑的层内表示；3. 在每层拟合类条件密度模型，产生每类典型性分数；4. 将典型性分数沿深度堆叠成典型性图；5. 从典型性图导出PAS和MLSV两种OOD检测分数。

Result: 在CIFAR-100上微调的ViT-B/16上，所提分数在无需重新训练或OOD暴露的情况下实现了具有竞争力的检测性能。

Conclusion: 基于ViT参数几何结构的内部表示分析方法能够有效用于OOD检测，PAS和MLSV两种后验分数具有良好的检测能力。
...
TL;DR: ActPair是一个用于文本based人物异常搜索的统一三阶段粗到细框架，通过动作对齐检索和成对多模态重排序来弥合姿态-语义差距


<details>
  <summary>Details</summary>
Motivation: 现有方法难以捕捉情境条件下的行为，通常依赖孤立的骨骼几何形状、在重写过程中丢弃原始查询细节，或使用绝对逐点评分进行多模态验证

Method: 提出ActPair统一三阶段框架：1) 使用动作对齐多任务目标微调视觉语言模型(VLM)；2) 使用原始查询和LLM生成的情境化重写进行并行后期融合检索；3) 使用pivot-promote算法进行高效的成对视觉比较重排序

Result: 在PAB公共测试集上实现最佳性能，并有效迁移到未见过的非异常特定数据集

Conclusion: ActPair在PAB公共测试集上取得了最佳结果，并能够有效迁移到非异常特定数据集。三阶段粗到细的框架成功弥合了姿态-语义之间的差距。
...
TL;DR: 本研究探讨视觉语言模型(VLMs)是否真正理解视觉关系，还是仅依赖语言线索等捷径。通过使用Qwen3-VL-4B模型，设计几何形状合成数据集和控制性查询来测试语言线索，并修改数据集检验视觉证据的因果依赖性。结果表明，当前VLMs将真正的视觉推理与主要源于语言线索的捷径策略相结合。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉推理任务中表现出色，但尚不清楚它们是否真正理解视觉关系，还是仅仅利用语言线索或先验知识等捷径。这是一个关于VLM可解释性和可信度的关键问题。

Method: 使用Qwen3-VL-4B模型，设计简单的几何形状合成数据集进行控制性分析，并制作专门用于精确测试语言线索的查询。此外，修改数据集以测试对视觉证据的因果依赖性。

Result: 当前VLMs将真正的视觉推理与主要基于语言线索的捷径策略相结合使用。

Conclusion: 研究揭示了现代VLM在视觉推理中的双重策略：既包含真实的视觉理解，也包含语言线索捷径。这一发现对VLM的设计和评估具有重要启示。
...
TL;DR: 该研究利用多模态传感器数据和机器学习算法预测老年下肢骨折或髋关节置换术后患者的临床康复评分，发现联合预测优于单独预测，NODE模型表现最佳，证明多模态数据对评估康复轨迹至关重要。


<details>
  <summary>Details</summary>
Motivation: 老年人在下肢骨折或髋关节置换术后的康复轨迹复杂，但目前临床研究往往孤立地研究这些方面，掩盖了它们对康复的联合影响。该研究旨在通过多模态传感器数据同时预测多个临床结果，以更全面地评估患者的康复进程。

Method: 研究使用了MAISON-LLF数据集，包含18名在下肢骨折或髋关节置换术后在社区康复的老年人的多模态传感器和临床评估数据。监测时间最长为八周，共提取了46个每日特征（来自室内运动、加速度、步数、心率、户外活动和睡眠数据），并每两周评估五个临床结果（社交孤立量表、牛津髋关节评分、牛津膝关节评分、起立行走计时测试和30秒坐立测试）。研究将多模态传感器数据与不同临床评分之间的内在关系构建为多输出回归问题，测试了各种机器学习和深度学习单输出及多输出回归算法。

Result: 结果表明，联合预测临床评分优于单独预测。表格型深度学习多输出回归器NODE表现最佳，MSE=3.96，MAE=1.02，优于其他多输出和单输出回归器。SHAP特征分析进一步表明，包含多模态传感器数据对于准确估计患者的康复轨迹非常重要。

Conclusion: 该研究证明了多模态传感器数据与机器学习相结合的方法在预测老年下肢骨折或髋关节置换术患者康复轨迹方面的有效性。NODE模型表现最佳，SHAP分析强调了多模态传感器数据的重要性。这种方法可能有助于社区老年人的功能康复和社会参与评估，最终改善他们的护理和生活质量。
...
TL;DR: FixAnything是一个通用的渲染瑕疵修复模型，通过重利用预训练视频生成模型来处理多种3D场景表示（3DGS、NeRF、网格、点云）的渲染伪影。核心思路是将修复任务转化为视频到视频的转换任务，利用噪声渲染序列保留的相机运动和粗略场景结构，并通过二进制掩码控制保留高质量像素，使用直接偏好优化（DPO）结合相机姿态精度作为奖励信号确保3D一致性。


<details>
  <summary>Details</summary>
Motivation: 当输入视图稀疏或目标视图远离输入视图时，使用高斯溅射（3DGS）、神经辐射场（NeRF）、网格或点云等3D场景表示进行渲染会产生伪影。虽然近期工作使用基于扩散的生成先验来缓解这些伪影，但这些方法专门针对特定表示，需要定制架构或大量重训练，缺乏通用性。

Method: FixAnything的核心创新在于：1）将预训练的视频生成模型重新用途，利用其隐含的多视图先验，只需最小修改和轻量级微调；2）即使噪声渲染的序列也保留了相机运动和粗略场景结构，因此清理可以形式化为视频到视频的转换；3）引入二进制掩码标记干净像素，控制模型输出锚定到高质量输入（如训练视图）同时精炼其余部分；4）使用通过运动恢复结构（SfM）恢复的相机姿态精度作为DPO的奖励信号，鼓励模型产生支持下游重建的3D一致渲染。

Result: FixAnything在四种不同的3D表示上 consistently 提升了渲染质量，证明了单一的通用视频先验可以取代多个专门的精炼流程。该框架的简洁性使得无需重新架构设计就能立即采用更强的未来视频模型。

Conclusion: 本文证明了预训练视频生成模型可以作为通用的多视图精炼先验，通过轻量级微调即可适配不同的3D场景表示。FixAnything的成功表明：1）视频模型的时序一致性可转化为3D一致性；2）二进制掩码机制有效平衡了保真度和生成质量；3）相机姿态奖励信号有效提升了重建支持能力。该框架具有良好可扩展性，未来可无缝集成更强大的视频模型。
...
TL;DR: 本文提出Expert-Grounded Distillation (EGD)框架,将专家道路安全审计知识迁移到紧凑的视觉语言模型中,用于可扩展的视觉道路安全审计,并构建了首个孟加拉国道路安全审计数据集BD-ARSA。


<details>
  <summary>Details</summary>
Motivation: 低收入和中等收入国家的道路交通事故仍是重大挑战,现有主动道路安全审计受限于不完整的事故记录、合格审计员短缺以及大规模现场检查的高成本。

Method: 提出EGD框架,包含量化的专家 grounding阶段,教师视觉语言模型与权威现场审计进行校准,仅在教师与专家风险评估达到较高一致度(Cohen's kappa=0.74)后才进行大规模标注,随后使用低秩适应(Low-Rank Adaptation)和单一无泄漏提示将结构化监督蒸馏到80亿参数的学生模型。

Result: 实验表明,基于grounding的微调显著提升了有序风险评估性能;盲法专家评估显示,紧凑学生模型优于310亿参数的教师模型和Gemini-2.5-Flash。

Conclusion: EGD为资源受限环境中的主动道路安全审计提供了有效且可扩展的工程解决方案。
...
### [186] [KVBoost: Chunk-Level Key-Value Cache Reuse with Deviation-Guided Recomputation for Efficient Large Language Model Inference](https://arxiv.org/abs/2608.21362)
*Srihari Unnikrishnan*

Main category: cs.AI

TL;DR: KVBoost是一个用于HuggingFace兼容解码器模型的块级KV缓存复用系统，通过双哈希键控方案分离位置身份和内容身份，支持精确和近似缓存匹配，实现4.49倍的TTFT缩减。


<details>
  <summary>Details</summary>
Motivation: 现有前缀缓存系统要求提示词共享连续前缀，当共享内容出现在任意位置时效果有限，导致Transformer大型语言模型每次请求都必须重新计算键值张量，产生高预填延迟。

Method: 提出双哈希键控方案分离位置身份(前缀哈希)和内容身份(内容哈希)；采用两种修复策略处理注意力边界错误：SelectiveRecompute重新编码边界区域，CacheBlendRecompute在探测pass后识别并重新计算高偏差token；结合非对称KV量化(int8/int4)、自适应块边界分割和固定内存预算下的重要性加权驱逐。

Result: 在Qwen/Qwen2.5-3B上对1000个bug定位样本进行评估，KVBoost实现TTFT降低4.49倍(142.4 ms vs 639.1 ms)，优于前缀缓存16%，准确率无损失(99.2% vs 99.1%)。

Conclusion: KVBoost提供了一种实用的内存有界推理加速层，与RoPE模型兼容无需架构修改，可在任意位置实现内容复用。
...
### [187] [AIREP: A Protocol for Per-Decision Evidence in AI Runtime Governance](https://arxiv.org/abs/2608.21363)
*Ali Toygar Abak*

Main category: cs.AI

TL;DR: 本文提出了一种AIREP协议，用于记录AI运行时系统的治理决策。该协议将AI系统的决策（发布、阻止、延迟、删除、升级）记录为可独立验证的签名对象，形成SHA-256哈希链以检测篡改。协议设计了供应商/模型/领域特定内容的隔离命名空间，并通过机械中立性测试确保格式的公平性。提供了参考实现和双语言一致性测试套件。


<details>
  <summary>Details</summary>
Motivation: 解决AI系统治理决策的可审计性问题。当前AI系统在做出输出决策时缺乏统一、可验证的记录机制，导致难以事后验证和问责。现有方法依赖于运行时本身，缺乏独立验证能力。

Method: 设计了一种协议，包含：1) 决策记录格式（包含决策动词、政策依据、哈希引用的输入/输出/证据）；2) SHA-256哈希链结构确保完整性和可检测性；3) 命名空间隔离机制；4) 机械中立性测试；5) 参考实现和一致性测试工具。

Result: 建立了可独立验证的AI治理决策记录协议，能够检测篡改和记录缺失。支持多种决策类型的统一记录，实现了供应商/模型特定内容与共享格式的隔离。

Conclusion: 该协议可被任何AI运行时采用，为AI治理提供透明度和问责机制。未来的工作将解决跨实现规范形式对齐、新鲜度见证和多运行时链等问题。
...
### [188] [Reviewing Model Collapse and Countermeasures](https://arxiv.org/abs/2608.21366)
*Xihao Xie,Beichen Hu*

Main category: cs.AI

TL;DR: 本文是一篇关于生成式AI中模型崩溃现象的综述论文，填补了该领域的研究空白。


<details>
  <summary>Details</summary>
Motivation: 在大规模数据驱动下，生成式AI取得显著进展，但使用AI合成数据进行训练会导致模型崩溃问题，引发了对AI可信度的担忧，此前缺乏对模型崩溃现象的系统性综述。

Method: 提供最新研究综述，整合并回顾不同应用场景下的模型崩溃研究及缓解措施。

Result: 综述了模型崩溃在不同场景下的表现及现有缓解策略。

Conclusion: 提出了当前面临的挑战和未来研究方向，为该领域后续研究奠定基础。
...
### [189] [AI Learning and Conceptual Transfer in the Game of Hidden Rules](https://arxiv.org/abs/2608.21372)
*Christo Mathew,Wentian Wang,Jacob Feldman,Lazaros K. Gallos,Paul B. Kantor,Vladimir Menkov,Hao Wang*

Main category: cs.AI

TL;DR: GOHR项目研究利用强化学习Agents从试错反馈中推断隐藏规则，涉及Transformer-based A2C框架、特征中心和物体中心表示设计、规则难度分析、迁移学习、泛化能力以及伪机器人辅助人类学习分析。


<details>
  <summary>Details</summary>
Motivation: 解决如何让人工智能Agents通过有限的试错反馈来学习和推断复杂隐藏规则的问题，并探索这些发现如何应用于辅助人类学习。

Method: 采用Transformer-based A2C框架，结合Feature-Centric和Object-Centric两种表示方法进行对比实验，分析规则难度、迁移学习和泛化能力，同时对人类学习数据进行分类分析。

Result: 实验发现不同表示方法在不同规则类型上的表现差异，揭示了规则难度对学习效果的影响，观察到迁移学习和泛化的局限性，并建立了人类学习数据的分类体系。

Conclusion: Transformer-based A2C框架结合适当的表示设计能够有效推断隐藏规则，但泛化和迁移能力仍需改进；伪机器人辅助学习为人类规则学习提供了新途径。
...
### [190] [LitReview Arena: Evaluating Literature Review Agents with Battle-Style Peer Review Platform](https://arxiv.org/abs/2608.21374)
*Ruotong Zhao,Zhiyu Chen,Xurui Liu,Haidong Xue,Dong Liang,Jigao Fu,Wu YanBiao,Yuanyi Zhen,Fengli Xu,Yong Li*

Main category: cs.AI

TL;DR: 本文介绍LitReview Arena，一个用于评估AI生成文献综述的对抗式评估平台，通过让领域专家对AI和人类撰写的文献综述进行多维度比较，发现当前最强AI系统仅在23%的关键对决中胜出人类，且现有LLM作为评判者的方法与人类专家存在显著偏差，但基于专家校准的LitJudge评估器可将一致性提升至0.78。


<details>
  <summary>Details</summary>
Motivation: 文献综述对科学进步至关重要，但自动生成的文献综述质量难以评估，因为许多研究实用性的方面取决于专家判断而非参考重叠指标，现有的自动评估方法无法准确衡量文献综述的质量。

Method: 构建LitReview Arena评估平台，采用结构化协议：让具有AI论文写作经验的领域专家比较匿名稿件，按专家专长匹配主题，并针对五个文献综述特定标准提供维度化评估结果，收集约3000条专家判断用于分析。

Result: 最强当前系统仅在整体实用性23.0%的关键对决中击败人类稿件；Agentic LLMs（如Sonan Deep Research）比基础语言模型性能提升超过60%；现有LLM-as-a-judge方法与人类专家存在显著偏差（Spearman相关系数仅0.467），特别是在综合类标准如论文结构和研究建议方面。

Conclusion: 基于收集的偏好数据训练的专家校准评估器LitJudge可将与人类专家的一致性提升至Spearman相关系数0.78，达到与专家间一致性相当的水平，为AI生成文献综述的评估提供了更可靠的方法。
...
### [191] [SchemaRouter: Field-Aware Tool Routing for Efficient Heterogeneous Agentic RAG](https://arxiv.org/abs/2608.21375)
*Yong-eun Cho*

Main category: cs.AI

TL;DR: SchemaRouter是一种轻量级路由层，通过模式图表示工具、端点、参数等，在材料科学基准测试中以仅227个检索上下文token实现0.71准确率，同时保持可验证的溯源和许可证信息。


<details>
  <summary>Details</summary>
Motivation: 异构智能体RAG系统存在两个关键问题：over-fetching（获取过多数据导致token浪费和延迟增加）和under-fetching（遗漏回答查询所需的字段）。现有方法将所有工具描述暴露给LLM或仅通过向量相似性选择工具，无法有效平衡检索效率和答案准确性。

Method: SchemaRouter将工具、端点、参数、响应字段、领域概念、单位、溯源和许可证策略表示为模式图。小型LLM提取意图、概念和来源约束，字段选择通过意图组投影和概念-字段匹配（带别名层）在图上进行确定性执行，最终生成可执行的工具计划。

Result: 在110个材料科学查询的基准测试中，SchemaRouter达到0.71的答案准确率，与fetch-everything方法在重叠置信区间内相当，优于prompt-all的0.66。使用的检索上下文token仅为227个（vs 2,066个），端到端延迟降低2.7倍。工具精确率和参数有效率分别为0.93和1.0。62%的答案包含溯源和许可证信息，而基线方法接近0%。

Conclusion: 最小化所选字段数量反而会降低准确率至0.56，而保持召回的投影能恢复最佳准确率。SchemaRouter在保持竞争力的准确率同时，实现了高效性、独立于模式规模的扩展性，以及可验证的溯源/许可证 grounding回答能力。
...
### [192] [RIACT: A Responsible AI System for Personalized Study Habit Tracking and Early Burnout Signal Detection in University Students](https://arxiv.org/abs/2608.21379)
*Ria Sidhu*

Main category: cs.AI

TL;DR: RIACT是一个结合结构化学习日志和混合AI架构的Web应用，旨在检测学生倦怠的早期信号。该系统通过记录学习地点和时间，计算净专注时间，使用确定性规则检测倦怠信号，并利用大语言模型生成个性化建议。


<details>
  <summary>Details</summary>
Motivation: 学生倦怠在高等教育中普遍存在（12%-70%），但通常只能在学业下降后才被识别。原因是学生缺乏对自身学习行为的结构化洞察，现有生产力工具只记录活动而不解读行为。

Method: 混合AI架构：学生按地点和时间记录学习会话；系统通过扣除休息时间计算净专注时间；通过透明、可审计的确定性规则检测周环比行为变化中的倦怠信号；使用受限于固定输出模式的大语言模型来情境化模式并生成个性化建议。

Result: 系统设计嵌入负责任AI原则：警告由可审计规则而非模型判断驱动；所有输出以观察而非诊断的形式呈现；数据收集限于自记录的行为字段。

Conclusion: 描述了系统设计原理，将研究置于学生倦怠和教育可解释AI的文献中，并提出了验证其行为信号与既定倦怠仪器一致性的评估框架。
...
### [193] [There Is No Neutral Harness: Modern LLM Leaderboards Are Manufactured by Config-Fragile Items](https://arxiv.org/abs/2608.21382)
*V. S. Raghu Parupudi*

Main category: cs.AI

TL;DR: 研究揭示LLM基准测试评分存在" harness敏感性"：同一模型在不同评估配置下得分差异巨大（如gemma4-31b在31%-89%之间波动），这意味着模型排名取决于评估方式而非模型本身能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅报告聚合分数方差，未深入分析哪些具体项目导致方差，以及这些项目是否真正区分不同模型。作者将评估harness视为自变量，系统研究其对单个项目的影响。

Method: 构建"脆弱性网格"：固定12个开源指令微调LLM（来自4个家族）、3679个测试项目（来自ARC、HellaSwag、MMLU、TruthfulQA），仅改变26种评估配置（包括选项顺序、提示词措辞、评分方式等），记录每个模型-项目-配置的正确性。

Result: 三个关键发现：1)模型分数是"区间"而非单点（gemma4-31b在31-89%之间波动）；2)配置脆弱项目占相邻模型排名差距的95.7%；3)4个模型在某些配置下可达排名第一；4)项目区分度与脆弱性相关度为0.28。评分方式选择（而非选项顺序）是关键影响因素。

Conclusion: 当前基准测试的评分机制存在严重问题：不同评估配置可导致同一模型排名大幅变化。建议将脆弱性网格作为排行榜发布前的必检项目，以确保排名反映模型真实能力而非评估方式的人为选择。
...
### [194] [ATHENA: Knowledge-guided agentic neural architecture search for AutoFormer-based electronic health record modeling](https://arxiv.org/abs/2608.21712)
*Deyi Li,Qi Xu,Lingyao Li,Tiansheng Wang,Muxuan Liang,Mei Liu*

Main category: cs.AI

TL;DR: ATHENA是一个用于Transformer-based EHR建模的跨医院神经架构搜索框架，通过预训练权重共享超网络和两层跨医院架构先验（任务描述符检索+SHAP元回归）指导多智能体LLM搜索，在6个临床预测任务和2个独立医疗系统中，12个医院-任务评估中有9个达到或超越4个NAS基线方法。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在EHR预测中需大量手动调参，最优配置因任务和医院而异；传统NAS计算成本高；现有LLM引导的NAS方法各次搜索独立进行，未能跨医院复用架构知识。

Method: 1)预训练权重共享超网络，每个医院预训练一次，候选架构作为继承子网络通过微调评估；2)两层跨医院架构先验：①基于任务描述符从源站点检索高性能架构示例，②使用SHAP元回归估计架构组件效果；3)多智能体LLM搜索结合目标医院验证反馈。

Result: 在6个临床预测任务和2个独立医疗系统中，搜索预算30下，12个医院-任务评估中有9个达到或超越4个NAS基线；重复搜索中架构选择更一致。

Conclusion: ATHENA为Transformer-based EHR建模中减少手动架构调参提供了实用方法，代码已公开。
...
### [195] [Spyre-Accelerated Retrieval-Augmented Generation on IBM LinuxONE: A Cloud-Native Architecture for Secure, High-Throughput Enterprise AI Inference](https://arxiv.org/abs/2608.21393)
*Sandeep Bokkasam,Pankaj D*

Main category: cs.AI

TL;DR: IBM在LinuxONE上构建了一个六子系统RAG架构，利用Spyre加速器进行生成式推理，Telum II芯片加速器进行轻量分类任务，Red Hat OpenShift进行容器编排，实现敏感数据不离开硬件边界的端到端RAG流程。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在企业环境中部署面临数据与算力分离的实际问题，在敏感数据跨系统移动时产生延迟、安全和监管方面的挑战。

Method: 六子系统RAG架构：查询接收 → 向量检索 → 提示词组装 → LLM推理 → 合规过滤 → 响应交付，全部在LinuxONE上运行，使用Spyre PCIe推理卡、Telum II加速器和OpenShift容器编排。

Result: 端到端RAG延迟低于2秒，与平台外推理相比延迟降低高达20倍，同时保持强加密和审计能力。

Conclusion: 该架构为受监管行业提供了一种在保持数据本地化和合规性的前提下部署AI工作负载的可行方案。
...
### [196] [TessIndex: Capability Verified Identity System for the Agent Economy](https://arxiv.org/abs/2608.21942)
*Mehul Goenka,Tejas Pathak,Siddharth Asthana*

Main category: cs.AI

TL;DR: TessIndex是一个用于智能体原语的能力验证身份系统，采用双层架构（区块链记录身份、所有权和验证的紧凑承诺，中心化服务器维护发现、商务和声誉的动态元数据），解决智能体经济中的三个关键基础设施问题：持久身份缺失、自我声明的能力声明无法验证、创建者身份与智能体性能和项目价值脱节。


<details>
  <summary>Details</summary>
Motivation: 软件智能体现在能够自主执行复杂工作流，创造了一种智能体经济。然而，现有基础设施在三个关键方面存在缺陷：1) 缺乏持久身份基础设施导致智能体工作流中缺乏系统性问责；2) 能力声明是自我声明的，缺乏可验证的执行证据；3) 创建者身份、智能体性能和项目价值之间的脱节阻碍了智能体作为资产的经济估值。

Method: TessIndex采用双平面架构：区块链记录身份、所有权和验证的紧凑承诺，而中心化服务器维护用于发现、商务和声誉的动态元数据。具体包括：1) 建立跨智能体原语的持久身份以在自主工作流中强制执行系统性问责；2) 建立基于谓词的验证流程，用加密能力证明取代自我声明的声明；3) 建立身份基础设施，将智能体性能与项目和创建者身份链接，同时通过代币化捕获价值。

Result: TessIndex作为一个集成基础设施，将智能体的存在与其能力、执行和声誉绑定到单一持久身份中。

Conclusion: TessIndex作为智能体经济的集成基础设施，通过持久身份、可验证能力证明和经济价值捕获机制，为自主智能体的系统性问责和资产化提供了解决方案。
...
### [197] [Hate Speech Classification In Roman Urdu: A Comparative Study On Parameter Efficient Fine-Tuning And Prompt Engineering](https://arxiv.org/abs/2608.21408)
*Toneema Zubair*

Main category: cs.AI

TL;DR: 本研究针对低资源语言Roman Urdu的仇恨言论分类问题，评估了提示调优、参数高效微调(PEFT/LoRA)和提示工程等多种技术在不同实验配置下的效果。


<details>
  <summary>Details</summary>
Motivation: 互联网和社交媒体上仇恨内容快速增长，造成严重社会危害；Roman Urdu作为低资源语言，存在非正式语法、句子结构不一致、拼写变体多等问题，且训练数据有限，因此需要研究有效的分类技术。

Method: 设计了四个实验：1)直接零样本推理；2)使用LoRA进行参数高效微调；3)使用混合和手动设计的提示进行提示调优；4)通过零样本和少样本学习进行提示工程。

Result: 实验比较了不同方法在低资源、有限数据条件下对Roman Urdu仇恨言论分类的有效性。

Conclusion: 研究旨在确定在低资源环境下进行仇恨言论分类的最有效技术方案。
...
### [198] [AutoSaddler: Automatic Harness Optimization with Durable Updates from Agent Execution Traces](https://arxiv.org/abs/2608.23041)
...
TL;DR: AutoSaddler是一个自动优化LLM智能体外部框架的系统，通过将框架改进形式化为离线学习问题，利用失败信号的mini-batches进行迭代更新，结合失败追踪诊断、结构化补丁生成和基于验证的更新选择，在GAIA2、SWE-Bench Pro和Terminal-Bench 2.0上分别提升了9.0、9.6和10.0个百分点。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长时程任务上仍不可靠，小的局部失败会在长时间交互中累积导致整体任务失败。虽然外部框架可以显著提高鲁棒性，但框架设计仍是手动且昂贵的过程，需要在大量提示词、工具配置和控制逻辑空间中进行搜索。

Method: 提出AutoSaddler自动框架优化框架，将框架改进形式化为离线学习问题，利用mini-batches中的失败信号迭代更新框架。AutoSaddler结合了失败追踪诊断、将框架视为代码的结构化补丁生成，以及基于验证的更新选择。

Result: 在GAIA2、SWE-Bench Pro和Terminal-Bench 2.0上的实验表明，AutoSaddler显著提升了智能体性能，相较于基础框架分别获得了9.0、9.6和10.0个百分点的增益。消融研究表明有效的框架优化受益于三个要素：深度调试而非浅层反思、针对性修改而非无约束编辑、以及泛化感知的选择而非轨迹特定修复。

Conclusion: 自动框架优化是实现更高效、更可靠智能体系统的有前景的路径。
...
### [199] [The Abstention Protocol: RCA for Clos Fabrics](https://arxiv.org/abs/2608.21412)
*Madhava Gaikwad,Deepak Pandey*

Main category: cs.AI

TL;DR: CoreSec是一个生产级根因分析(RCA)系统,针对大型数据中心网络中遥测数据噪声、部分和异步的问题,通过PAM风格弃权代数和控制标志替代加权融合,提供确定性决策和显式弃权机制。


<details>
  <summary>Details</summary>
Motivation: 大型数据中心网络的根因分析面临挑战:遥测数据噪声大、不完整且异步,基于评分的方法在这种条件下表现不稳定,经常产生不正确或不稳定的归因。

Method: CoreSec采用PAM风格的弃权代数,使用控制标志组合遥测代理,产生确定性决策;当证据不明确时显式弃权,并结合拓扑感知配置捕获Clos网络的故障面,随证据积累单调收敛。

Result: CoreSec已在超大规模环境中部署,在不同环境中无需重新调优即可提供稳定且可解释的RCA行为。

Conclusion: 结构化组合与弃权的框架为现实云网络中的自动化RCA提供了实用基础,验证了弃权代数在生产环境中的有效性。
...
### [200] [Retrieval-grounded robot program generation and simulation-based correction via Model Context Protocol](https://arxiv.org/abs/2608.21417)
*Zhichao Zhou,Siyuan Chen,Omkar Salunkhe,Ebru Turanoglu Bekar,Johan Stahre,Anders Skoogh*

Main category: cs.AI

TL;DR: 本文提出了一种基于语言模型的工作流，用于从自然语言任务描述生成、验证和迭代纠正ABB RAPID工业机器人程序，通过双流检索增强生成(RAG)框架和自定义MCP服务器连接RobotStudio仿真软件。


<details>
  <summary>Details</summary>
Motivation: 灵活制造需要工业机器人能够快速重新编程以适应产品变种变化。传统编程方式效率低下，需要一种能够从自然语言快速生成机器人程序的方法。

Method: 1) 双流RAG管道：结合技术文档和生产模板进行代码生成，提高领域特定准确性；2) 自定义MCP服务器：直接连接ABB RobotStudio，实现代码上传、仿真执行和诊断反馈；3) 评估方法：30查询检索基准、代码生成检查、RobotStudio案例研究（模拟拾取放置制造单元）

Result: 仿真循环能够暴露静态和语义检查无法捕获的执行失败，包括：吸盘释放高度错误、无法到达的放置目标、依赖配置的恢复运动。RAG和MCP能够连接有依据的代码生成与工业机器人仿真软件的可执行反馈，同时减少但未能完全消除专家设置和最终监督的需求。

Conclusion: 该方法成功展示了语言模型与工业机器人仿真软件的集成可能性，通过RAG技术提高了代码生成的准确性，通过MCP实现了自动化闭环反馈，为自然语言编程工业机器人提供了可行方案，但仍需专家介入进行设置和最终监督。
...
### [201] [Composable Trust Infrastructure for Manufacturing Knowledge Graphs: Cross-System Provenance, Temporal Reasoning, and Decision Traceability](https://arxiv.org/abs/2608.21418)
*Grama Chethan*

Main category: cs.AI

TL;DR: 本文提出了一个可组合的信任基础设施，通过集成SHACL验证、PROV-O溯源、领域感知的双时态版本控制和图原生决策对象四种能力，解决制造业知识图谱中的信任赤字问题。该架构通过共享实体URI、摄取活动标识符和时间关联键实现能力组合。


<details>
  <summary>Details</summary>
Motivation: 制造业知识图谱面临信任赤字问题：消费者无法确定查询数据是否有效、决策时是否有效、数据来源以及如何被处理。需要建立信任能力来解决这些问题。

Method: 提出可组合信任基础设施，集成四种信任能力（SHACL验证、PROV-O溯源、双时态版本控制、图原生决策对象），通过共享关联标识符实现能力组合。使用消融实验验证各能力的必要性，并分析高阶组合特性。

Result: 消融实验证实：移除任何单一能力会导致六项组合查询中的三项失败，证明四种能力同等重要。分析发现四个三方 emergent特性和一个四方不可约特性（全链审计性，31ms执行）。在11个工业源（OPC UA、TIA Portal、eClass、AAS、ISA-95等）上验证，包含8743个三元组和81个owl:sameAs标识边。

Conclusion: 四种信任能力通过共享关联标识符组合可产生单一能力无法实现的 emergent 信任特性，为制造业知识图谱提供可验证的信任基础设施。
...
### [202] [Evaluating Multimodal Narrative Understanding of Popular Hollywood Films](https://arxiv.org/abs/2608.21430)
...
TL;DR: 本文构建了一个基于好莱坞电影的多模态问答基准，用于评估模型对电影叙事元素的理解能力，同时解决了版权问题。


<details>
  <summary>Details</summary>
Motivation: 电影领域的大规模计算分析需要稳定基准，但好莱坞电影受版权保护限制了研究；需要建立既有票房popularity又处于public domain的电影集合。

Method: 1)收集1922-1979年《Variety》杂志的周票房数据；2)通过美国版权目录研究版权注册和续展情况，确定可能属于公共领域的电影；3)在此基础上构建聚焦叙事元素的多模态MCQ基准。

Result: 许多视觉-语言模型在此任务上表现不佳（接近随机准确率），包含音频的视听模型最高准确率仅为61.1%，远低于人类水平。

Conclusion: 当前多模态模型在理解电影叙事元素方面存在显著不足，该基准可用于推动该领域的进一步研究。
...
### [203] [Agentic AI for Safety-critical Multi-drone Systems: Challenges and Opportunities](https://arxiv.org/abs/2608.21444)
*Timothy Merritt,Alejandro Jarabo-Peñas,Juan Bravo-Arrabal,Maria-Theresa Bahodi,Anders Lyhne Christensen*

Main category: cs.AI

TL;DR: 本文认为在安全关键任务中部署多智能体AI系统不仅是算法问题，更是社会技术设计问题，需要通过以人为中心的迭代方法将LLM支持的机器人控制与人类监督相结合。


<details>
  <summary>Details</summary>
Motivation: 现实世界中多无人机系统在搜救、消防和关键基础设施监控等安全关键任务中的采用受到限制，主要障碍不仅是自主性能，还包括将智能体行为整合到专业工作中的困难——操作员必须在不确定、时间压力和问责制下理解、信任和管理自动化。

Method: 通过两个持续项目NAMUR（探索LLM支持的机器人在搜救和消防中的控制）和PERSIST（探索用于关键基础设施监控和安全的持久无人机操作）来积累经验，采用以人为中心、参与式和迭代的研究方法，揭示利益相关者需求，通过 successive prototypes 塑造智能体能力。

Result: 产出可转移的概念验证系统和评估策略，适用于其他安全关键场景。

Conclusion: 智能体AI应该被视为社会技术设计问题，其中界面、监督机制和评估实践与算法同样关键。
...
### [204] [Software Frameworks for Explainable AI in Time Series Classification: A Systematic Review](https://arxiv.org/abs/2608.21449)
*Louis Peter,Nils Gumpfer,Jana Fischer,Christin Seifert,Jennifer Hannig*

Main category: cs.AI

TL;DR: 这是一篇关于时间序列分类(TSC)中可解释人工智能(XAI)软件框架的综述论文。论文首次系统性地分析了支持时间序列的XAI框架，比较了多个维度并揭示了常见局限性。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据在众多决策关键领域广泛应用，TSC模型的透明性和可信度成为重要需求。然而，XAI在TSC领域的研究碎片化，缺乏对软件框架、评估实践和实际局限性的系统理解。之前的研究主要关注单个解释方法，跨框架一致性、时间序列特定评估和可重复性方面的研究不足。

Method: 论文分析了现有的TSC解释生成和评估软件框架，从支持的XAI方法、评估指标、可用性、基准测试支持和可重复性等多个维度进行比较。这是首次对时间序列特定框架的实现比较和频域支持分析。

Result: 识别出6个明确支持时间序列的框架，揭示了共同局限性：仅一个方法支持频域解释；仅有两个评估指标是专门为时间序列开发的；相同的XAI方法在不同框架中可能产生显著不同的解释。

Conclusion: 讨论了开放挑战并展望了未来研究方向，强调需要统一的时间序列特定XAI框架，以实现忠实、可重复和时间序列感知的解释。
...
### [205] [Enhanced Artificial Neural Networks Using QHAdamW in Air Quality Forecasting](https://arxiv.org/abs/2608.21463)
*Mary Joy Daniel Vinas*

Main category: cs.AI

TL;DR: 本研究开发了菲律宾首个AQI预测模型，采用前馈神经网络结合改进的Adam优化器（QHAdamW和AdamW），基于马尼拉实时空气质量监测站数据分别预测PM2.5和PM10的空气质量指数。


<details>
  <summary>Details</summary>
Motivation: 解决Adam优化器在收敛性、泛化能力和预测性能方面的问题，同时帮助菲律宾环境与自然资源部环境监测局（DENR-EMB）实施全面的空气质量管理，为颗粒物（PM）预测提供有效工具。

Method: 使用前馈神经网络，结合优化的Adam算法（QHAdamW和AdamW）进行训练，进行超参数调优，采用7个评估指标进行对比分析，基于马尼拉实时空气质量数据分别预测PM2.5和PM10的AQI。

Result: 超参数调优显示0.01和0.001是QHAdamW的最佳值；7个评估指标显示误差值范围更低，回归系数接近1提高了模型准确性；训练和验证损失值较低，表明模型收敛到满意的性能水平。

Conclusion: 该模型可有效预测颗粒物（PM），为DENR-EMB实施全面空气质量管理提供支持。
...
### [206] [Let Credit Follow Computation: Architecture-Aware Credit Transport for Large Language Model Reinforcement Learning](https://arxiv.org/abs/2608.21501)
*Qifan Shi,Zhaolu Kang,Chenghua Zhu*

Main category: cs.AI

TL;DR: 本文提出计算条件信用传输（CCT）框架，通过将Transformer的注意力集中度作为条件来参数化信用传输核，在Qwen3-4B上比GRPO提升约7.6%准确率，并在PPO-grid中表现更稳定。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型强化学习（LLM RL）中的信用分配问题。当前方法要么使用固定折扣的GAE（沿token时间应用静止几何核），要么使用组相对方法（将结果统计广播到整个响应）。这些传输算子都是架构无关的，无法代表Transformer策略本身使用的轨迹特定计算。因此需要引入一种新的计算感知信用传输框架。

Method: 提出计算条件信用传输（CCT）框架：1）将行为策略内部计算的独立统计量参数化因果核，用于向下游传输价值；2）具体算法CompPO将原生注意力集中度映射到有界的per-token保留门；3）在单步bootstrap和路径依赖的广义优势迹（Comp-GAE）中同时使用该门；4）协同设计传输对齐评论家（TAC），重用actor的隐藏状态和路由信息，无需第二个同规模Transformer。

Result: 在Qwen3-4B的5个随机种子实验中，CompPO达到61.4%的最终保留准确率（95% CI [60.8,62.0]），而调优后的GRPO为53.8% [52.9,54.7]。单独使用Comp-GAE加标准评论家为55.2%，单独使用TAC加固定门为56.4%，均不及完整模型（交互提升+2.4 [1.9,2.9]）。打乱和位置控制确认了轨迹特定对齐；CompPO在12次PPO-grid运行中10次稳定，而GRPO仅3次。在Qwen3-4B和Llama-3.1-8B-Instruct上，冻结评估分别比GRPO提升4.3和3.9个greedy pass@1宏点数。

Conclusion: CompPO通过将注意力集中度作为信用传输的条件，成功实现了超越传统GAE和组相对方法的性能提升。TAC和Comp-GAE的协同设计产生了显著的正交互效应，表明信用传输算子与策略架构的协同设计对LLM RL至关重要。
...
### [207] [Quantifying geographic domain shift to decouple the geospatial transferability of human mobility flow generation models](https://arxiv.org/abs/2608.21567)
*Zhiyong Zhou,Song Gao,Qianheng Zhang,Feng Zhang,Zhenhong Du*

Main category: cs.AI

TL;DR: 研究系统性地调查了四个人类移动生成模型在2265个美国县级区域的地理空间可转移性，引入"地理域迁移"概念来描述源区域和目标区域之间地理特征分布和空间结构的内在差异，并提出互信息和空间偏移两个指标来量化这种域迁移。研究发现模型转移性能存在显著的空间异质性和非对称性，信息偏移和空间偏移都表现出统计显著的互补解释力，表明地理空间可转移性不仅取决于模型设计，还取决于内在的地理差异。


<details>
  <summary>Details</summary>
Motivation: 人类移动是理解城市系统中社会、经济和环境动态的关键代理变量，而地理空间可转移性是比较不同人类移动生成模型的关键维度，但目前很少有研究探讨地理空间可转移性的内在特征。

Method: 使用美国2265个县级区域的人口普查区间通勤流大规模基准数据集；引入地理域迁移概念；提出互信息和空间偏移两个量化指标；采用线性混合效应回归分析地理域迁移与可转移性之间的关联。

Result: 模型转移性能存在显著的空间异质性和非对称性；信息偏移和空间偏移均表现出统计显著且互补的解释力；地理空间可转移性不仅取决于模型设计，还取决于内在地理差异。

Conclusion: 研究提供了评估和改进人类移动生成模型地理空间可转移性的新方法论框架，支持更稳健和公平的多区域人类移动数据合成，并为GeoAI模型开发中的空间可转移性提供了见解。
...
### [208] [A Reproducible, License-Aware Distillation Recipe for CPUDeployable Safety Classification](https://arxiv.org/abs/2608.21570)
...
TL;DR: 本文提出了一种可在 commodity 硬件上部署的知识蒸馏方案，将大型安全护栏模型蒸馏为小模型。研究者使用强大的开源护栏模型标注了约97,000个提示词，涵盖24个公共数据集，分为七个安全类别，并训练了多种架构的小型学生模型来复现该安全信号。


<details>
  <summary>Details</summary>
Motivation: 当前开源护栏模型参数量在1-90亿之间，依赖GPU且在CPU上推理延迟高达数秒，无法在 commodity 硬件上部署安全层。

Method: 使用强大的开源护栏模型作为教师，标注24个公共数据集的约97,000个提示词，按许可证边界划分训练数据，训练了涵盖词法、浅层、编码器和生成式架构的小型学生模型。关键配方是每类样本重平衡。

Result: 蒸馏后的学生模型在对抗性文本上与教师模型表现相当（置信区间重叠），且在无害提示上减少了误报。最小的生成式学生模型误报率为3.8%，而80亿参数教师模型为4.8%；编码器模型在CPU上推理时间约24毫秒。

Conclusion: 每类重平衡是决定性配方。蒸馏护栏在清洁参考数据切片上仍优于学生模型，未声称学生模型具有优势。
...
### [209] [Robust Lightweight Deep Learning Models for Oral Cancer Screening](https://arxiv.org/abs/2608.21583)
...
TL;DR: 本研究针对低收入国家口腔癌筛查专家短缺的问题，开发了基于智能手机的轻量级深度学习模型。通过对约30,000张图像的多中心回顾性数据集进行系统评估，比较了卷积、Transformer和混合架构。最终优化的MobileViTv2模型在边缘设备上实现了83.2%敏感性和86.0%特异性，证明轻量级AI模型在初级医疗环境中用于自动化分诊具有高潜力。


<details>
  <summary>Details</summary>
Motivation: 口腔癌是低收入国家主要致死病因之一，但专科医生短缺导致诊断延迟。现有基于智能手机的即时筛查方案面临训练数据类别不平衡、数据质量参差不齐、边缘设备计算受限等挑战，需要开发适合资源受限环境的稳健AI模型。

Method: 使用涵盖十年数据的多中心回顾性数据集（约30,000张图像），系统评估了先进的卷积、Transformer和混合架构。通过严格的流程消融实验，直接优化混合架构以适应边缘部署，并与大型模型或知识蒸馏等计算密集型范式进行比较。同时进行可解释性分析和模拟噪声压力测试。

Result: 优化的MobileViTv2模型达到平均敏感性83.2%±1.5%、平均特异性86.0%±0.8%。最佳模型敏感性87.4%、特异性86.5%，阴性预测值达97.2%（参考专科医生标签）。系统能锚定临床特征，对无结构传感器噪声具有稳健性，但对 impulse bit errors 存在脆弱性。

Conclusion: 通过针对性的架构选择和流线型优化，可解释且稳健的轻量级AI模型在边缘部署中表现出高潜力，可用于初级医疗环境中的自动化口腔癌分诊。
...
### [210] [Data-Driven Dynamic Algorithm Dispatch with Large Language Models](https://arxiv.org/abs/2608.21584)
*Rushil Shah,Emmanuel Lujan,Rabab Alomairy,Alan Edelman*

Main category: cs.AI

TL;DR: 研究提出利用大语言模型（LLaMA 3）结合提示工程和性能数据库，自动生成线性代数中的动态算法调度启发式规则。


<details>
  <summary>Details</summary>
Motivation: 高性能线性代数软件需要根据问题特征动态选择最优算法，现有方法依赖人工设计的启发式规则，希望利用LLM自动学习并合成启发式策略。

Method: 结合提示工程、LLaMA 3大语言模型和精心设计的性能数据库，通过模型学习识别结构模式并生成算法选择启发式。

Result: 以LU分解为案例研究验证了模型复制专家设计策略的能力，展示了LLM在算法发现中的潜力。

Conclusion: LLM能够有效用于算法发现和开发更自适应、高性能的线性代数软件，是算法自动化的有前景方向。
...
### [211] [K-Bench: measuring model performance on real scientific agent requests](https://arxiv.org/abs/2608.21601)
*Aubrey Brueckner,Darshil Patel,Yuhuan He,Timothy Kassis*

Main category: cs.AI

TL;DR: K-Bench 01是一个评估科学AI智能体的基准，基于真实用户流量中的首次请求构建，由9个前沿模型在相同沙箱中运行，产生1,602个完整智能体运行。三个盲评LLM法官使用八维度评分标准进行评估。结果显示没有模型能在所有三位法官评定下达到阈值，GPT-5.6-sol平均分最高(8.04)，但其95%置信区间跨越阈值。47.6%的评分低于8分阈值，科学准确性平均6.22低于沟通能力7.33。主要失败原因是过度声称(31.4%)。


<details>
  <summary>Details</summary>
Motivation: 当前科学AI基准主要是计分式的（选择题、有参考解决方案的 curated 智能体任务或已知生成结构的模拟器），但真实科学请求是不同的——它们描述不足、带有附件、缺乏真值。文章指出需要一种评估科学AI智能体的新方法。

Method: 从K-Dense Web的真实用户流量中抽取首次请求构建评估，在相同沙箱中由9个前沿模型端到端运行，产生1,602个完整智能体运行。三个盲评语言模型法官使用八维度评分标准对每次运行进行评分。

Result: 没有模型能在所有三位法官评定下达到阈值。gpt-5.6-sol最高 pooled mean为8.04，但其95%区间[7.80, 8.23]跨越阈值，两位法官将claude-opus-5排第一。在39,934个评分中，47.6%低于8分阈值。科学准确性平均6.22 vs 沟通能力7.33。首要失败标签是过度声称(31.4%)。

Conclusion: 应将系统排序作为可复现量，绝对水平作为仪器属性，榜首位置未解决。科学智能体的信息量不是排行榜位置，而是所交付内容、所声称内容及所产生制品的联合分布。
...
### [212] [Generate in the Chart, Not on the Boundary: Function-Symbol Grounding for Hard Constraints in LTN-GANs](https://arxiv.org/abs/2608.21605)
*Nijesh Upreti,Vaishak Belle*

Main category: cs.AI

TL;DR: 本文提出将逻辑公理作为函数符号而非谓词嵌入LTN-GAN中，以处理硬结构性约束（如顺序、正性、定义恒等式）。通过引入分辨率比率R作为诊断指标，发现当R较大时，谓词方法无法获得学习信号，而约束层方法会丢失边界分布。函数符号方法通过在可行区域内构建坐标系统（chart），使每个样本天然有效并能学习边界分布。


<details>
  <summary>Details</summary>
Motivation: 现有LTN-GAN方法将约束作为谓词嵌入，只能对样本评分，无法表达硬结构性约束（如顺序、正性、定义恒等式）。约束层方法虽然能将样本 clamp 到可行边界，但导致样本总是恰好处于边界上，丢失了真实的边界分布信息。

Method: 1. 将逻辑公理作为函数符号而非谓词嵌入LTN框架；2. 提出分辨率比率R（数据尺度/边界扩展）作为诊断指标，可在训练前计算；3. 函数符号形成chart（可行区域内的坐标系），直接计算受约束变量而非评分样本。

Result: 当R较大时：(1)谓词方法无法获得学习信号；(2)约束层方法将所有样本推到边界；(3)边界分布丢失但标准指标仍显示正常。函数符号方法可避免这两种失败，使样本天然有效并学习边界分布。

Conclusion: 函数符号接地方法优于谓词和约束层方法，能在保持样本有效性的同时学习真实的边界分布。分辨率比率R是选择合适接地方法的重要诊断工具。
...
### [213] [Semantic Compression Trees: Multi-Resolution Knowledge Retrieval via Hierarchical Semantic Residuals](https://arxiv.org/abs/2608.21610)
*Junaid Farooq*

Main category: cs.AI

TL;DR: 本文提出语义压缩树(SCT)，一种分层索引结构，每个节点仅存储相对于父节点的语义残差。检索时从根节点逐步下潜。在QASPER数据集上，SCT在使用更少token的情况下达到与密集检索相当的答案质量，但实验表明残差表示有价值，而自顶向下的路由策略表现不佳。


<details>
  <summary>Details</summary>
Motivation: 当前检索增强生成(RAG)主要依赖平面固定粒度索引：文档被切分为统一大小的块并通过相似度检索，但这丢弃了源的层次结构。作者认为需要一种层次化索引来改善这一问题。

Method: 提出语义压缩树(SCT)：一种分层索引，每个节点仅存储其相对于父节点的语义残差（即新增信息），检索通过从根节点逐步下潜实现，使每查询成本由树深度而非集合大小决定。实验使用QASPER数据集（50篇论文，173个问题），采用自举置信区间和配对显著性检验。

Result: 1)当文档给定時，SCT与密集检索答案质量相当（F1: 0.274 vs 0.277, p=0.37），节省30%上下文token，索引构建无需LLM调用；2)残差存储优于每个节点存储完整摘要（0.274 vs 0.205, p<0.001）；3)集合扩大50倍，平面检索工作量增加48.9倍，SCT仅增加6.4倍；4)但渐进下潜表现不佳：不使用树的残差检索与使用树效果相当(p=0.27)，而需选择文档时下潜策略明显更差(0.122 vs 0.165, p<0.001)；5)路由准确性：下潜从根残差（最压缩节点）选择，正确论文选择率仅20.2% vs 平面检索39.3%。

Conclusion: 残差表示值得保留，但自顶向下路由策略应当放弃。SCT的层次结构在索引构建和扩展性方面有优势，但路由准确性是主要瓶颈。
...
### [214] [SAEM: Stage-Aware Expert Management for Memory-Efficient MoE Inference in Chain-of-Thought Reasoning](https://arxiv.org/abs/2608.21614)
*Yujie Zhang,Bin Gao,Tulika Mitra*

Main category: cs.AI

TL;DR: SAEM是一个stage感知的MoE推理运行时，通过检测推理阶段边界并利用阶段级激活一致性来指导专家放置，在数学和科学推理工作负载上实现了1.33x-1.54x的吞吐量提升。


<details>
  <summary>Details</summary>
Motivation: CoT提示虽能提升LLM推理能力，但其顺序性质增加了解码延迟和内存占用；MoE模型虽通过稀疏专家激活扩展容量，但其完整专家权重常超出GPU内存。现有运行时忽略CoT跟踪中连续推理阶段呈现出一致且可预测的专家激活模式这一关键结构属性，导致缓存效率低和不必要的数据移动。

Method: SAEM结合了stage感知的缓存、专家对齐的token重新打包和原地CPU执行，以减少数据迁移和内核碎片化。该方法检测推理阶段边界并利用阶段级激活一致性来指导专家放置。

Result: 在数学和科学推理工作负载上，SAEM在受限GPU内存条件下相比最强的基线缓存和卸载方法实现了平均1.33倍的吞吐量提升，当校准数据与工作负载匹配时提升达到1.54倍。

Conclusion: 结果表明，对于CoT推理，stage感知的局部性驱动MoE推理是有效的。
...
### [215] [Measuring Activation Control in Large Language Models](https://arxiv.org/abs/2608.21664)
*Marek Mateusz Kowalski,Joshua Fonseca Rivera,Uzay Macar,David Demitri Africa*

Main category: cs.AI

TL;DR: 论文引入"激活可控性基准"(Activation Controllability Benchmark)来量化模型通过自然语言指令调节残差流激活的程度。研究发现大多数LLM都能在一定程度上控制激活的方向和幅度，且在简单任务中这种控制可以规避基于激活的监控方法(如线性探针、自然语言自编码器、激活预言机、雅可比透镜)。


<details>
  <summary>Details</summary>
Motivation: 随着模型能力增强,评估-aware的模型可能表现出欺骗行为,因此需要依赖潜在空间监控作为行为评估的补充。然而,如果模型也能控制自己的激活,欺骗可能延伸到潜在空间,这引发了对激活监控有效性的担忧。

Method: 建立Activation Controllability Benchmark,测试多个模型家族和能力级别的LLM通过自然语言指令控制残差流激活的能力,并评估其对现有激活监控方法的规避效果。

Result: 大多数LLM能够在一定程度上控制残差流激活的方向和幅度,具有时间分辨率,但不同模型表现差异很大。在简单任务中,这种控制可以部分规避基于激活的监控方法,但并非完美。

Conclusion: 随着内省能力增强,对激活空间本身的控制可能成为监控的干扰因素。建议前沿实验室和评估者在未来模型中追踪激活可控性。
...
### [216] [From Mastery Profile to Simulated Response: Stochastic Student Knowledge Graphs (SSKG) for Faithful LLM Student Simulation](https://arxiv.org/abs/2608.21668)
*Yuan An,Emily Wang,Benjamin Wang,Ruhma Hashmi*

Main category: cs.AI

TL;DR: 本文针对大语言模型(LLM)在模拟不同掌握程度学生时存在的局限性提出了新的解决方案。现有基于提示的方法让LLM自行决定答案，导致即使被指示模拟低掌握程度学生，LLM仍能凭借自身能力取得96.8-100%的高准确率，无法有效区分不同 mastery 水平的学生。研究提出基于随机学生知识图谱(SSKG)的方法，从开放代数教科书提取课程知识图谱(CKG)，将SAT解答分解为所需三元组链，为每个三元组分配掌握概率并采样确定题目正确性，再由LLM生成符合结果的第一人称理由。该方法将准确率降低至44.1-85.2%，并产生了清晰的单调掌握程度梯度。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在模拟不同 mastery 水平学生时的局限性。传统基于提示的方法让LLM自行决定答案，导致即使模拟低 mastery 学生也能取得高准确率，无法有效区分不同掌握程度的学生群体。

Method: 1)使用379道大学理事会校准的SAT代数题目和五个典型 mastery 配置文件进行测试；2)提取开放代数教科书的课程知识图谱(CKG)；3)将每道SAT解答分解为所需的三元组链；4)为每个三元组分配 mastery 概率并采样确定题目正确性；5)LLM根据结果生成一致的第一人称理由。

Result: 三种LLM(Gemini 3.1 Flash Lite、Claude Haiku 4.5、GPT-5.4-mini)在原方法下准确率达96.8-100%，无法区分不同 mastery 水平；新方法将准确率降至44.1-85.2%，并产生了清晰的单调 mastery 梯度。

Conclusion: 基于SSKG的方法能有效模拟不同 mastery 水平的学生，产生可区分的性能差异，为辅导系统的训练数据和压力测试提供了更可靠的合成数据生成方案。
...
### [217] [Context as an Environment: Programmatic Context Management for Long-Horizon Agents](https://arxiv.org/abs/2608.21690)
*Yin Lin,Elaine Ang,Erkang Zhu,Bolin Ding,Jingren Zhou*

Main category: cs.AI

TL;DR: Scroll是一个LLM代理上下文管理系统，将每个代理会话视为可执行的会话环境，通过append-only Event Log和沙盒持久化Python内核来管理长期任务的历史记录，避免了传统方法在压缩信息时的预先commitment。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理超出模型上下文窗口的长期任务时，要么压缩早期交互，要么将选定信息提取到固定内存表示，这些方法都需要在未来需求未知的情况下预先决定保留什么。

Method: Scroll采用Event Log + 沙盒持久化Python内核的架构：内核维护跨模型调用的类型化命名空间，允许工具输出、检索的历史和派生状态绑定到变量而非序列化到提示中；模型编写的代码通过exec搜索、实例化和转换会话状态；只有显式打印的内容进入模型的下一个调用工作视图。当工作视图接近预算时，陈旧跨度被驱逐但仍可恢复，驱逐索引保持紧凑地标与精确Event Log地址的关联。

Result: 在Qwen3.8-Max作为骨干下，Scroll在LongMemEval_S上达到94.8%；在BEAM_10M上达到73.1%，比最佳已发布内存系统高5.1分；在LOCA_256K上达到86.7%，比最佳已发布长程代理高37.4分。

Conclusion: Scroll通过将上下文管理转变为编程任务，继承了LLM不断提升的编码能力，同时通过Event Log保留无损历史真相，在多个长程代理基准测试上取得了显著的性能提升。
...
### [218] [From Association to Causation: Improving Retrieval Precision of Retrieval-Augmented Generation via Causal Relations and an Attention Mechanism](https://arxiv.org/abs/2608.21702)
*Jing Liu,Yongxing Qi,Muchen Jiang,Chengnan Hu,Qingqing Peng,Haoming Wang,Yuqing Wang,Yang Yu,Xu Zhang,Ting Wu*

Main category: cs.AI

TL;DR: 本文提出了一种基于因果图的无训练重评分方法，用于解决RAG系统中关键词填充导致的检索失败问题。该方法利用Reichenbach共同原因原则建模检索过程的因果结构，将查询和文档的共有关键词建模为潜在共同原因A，文档的残余关键词建模为潜在集合B，通过计算查询嵌入与B的加权质心嵌入之间的余弦相似度进行重评分。


<details>
  <summary>Details</summary>
Motivation: 标准RAG的终端检索阶段使用密集向量相似度（可选重排序）存在缺陷：返回的文档与查询共享关键词但未必包含所需信息，且随知识库增长问题加剧。作者将此问题追溯到概念差距：相似度仅捕捉关联关系，而重要文档实际上与查询存在因果联系。

Method: 1. 构建基于Reichenbach共同原因原则的因果图：查询和文档的共有关键词形成潜在共同原因A，文档的残余关键词形成潜在集合B（B与理想输出有因果联系）；2. 检索到的文档作为碰撞器(A → d ← B)，检索本身打开了查询与B之间的关联路径；3. 提出无需训练的关注式重评分规则：计算查询嵌入与B的加权质心嵌入之间的余弦相似度；4. 设计语料库级校准门控机制选择正确的检索模式。

Result: 1. 在471文档的企业知识库中，将相关指南从第6位提升至前3；2. 在复现关键词填充的受控诊断语料库中，将目标平均排名从2.88改善至1.25，而训练好的交叉编码重排序器几乎无效(2.63)；3. 在三个BEIR基准上成绩低于相似度基准，划定了适用边界；4. 语料库级校准门控可>=95%可靠地选择正确模式；5. 完全本地化的测试平台验证了可部署性。

Conclusion: 该方法有效应对了专有知识库增长带来的关键词填充问题，与神经重排序器形成互补。通过因果图建模检索过程本身的因果结构而非知识内容，实现了无需训练的轻量级重评分。校准门控机制可准确识别应用场景，在关键词填充 regime 下显著提升检索质量，在标准基准上则不适用。
...
### [219] [Ask or Answer: A Decision Framework for Multi-Turn Health Misinformation Intervention](https://arxiv.org/abs/2608.21721)
*Xiaoying Song,Anirban Saha Anik,Jinyu Liu,Qitao Tan,Geng Yuan,Lingzi Hong*

Main category: cs.AI

TL;DR: RO-PnR框架学习何时在健康辟谣对话中询问澄清问题比直接纠正更有价值，通过奖励优化在探测信息和最终纠正之间做出决策，在三个数据集上以更少的交互轮次实现了更高的成本调整效用。


<details>
  <summary>Details</summary>
Motivation: 健康辟谣需要因人而异的干预策略，但现有方法要么立即回应，要么无差别地询问澄清，未能学习何时询问的成本与收益相匹配。

Method: RO-PnR在每轮选择探测更多信息或进行最终纠正，使用考虑探测收益与交互成本的轮级奖励指导决策，并通过潜在状态（健康素养和信念承诺）建模用户异质性。

Result: RO-PnR在三个健康 misinformation 数据集和三个基础模型上实现最高成本调整效用，比始终探测的基线减少30%交互轮次。

Conclusion: 通过学习根据用户特征动态决定何时询问澄清问题，可以更高效地进行健康辟谣，在减少交互成本的同时保持或提升纠正效果。
...
### [220] [ECHO: A Cognitively Inspired, Auditable Memory Plane for Long-Horizon Agents](https://arxiv.org/abs/2608.21755)
*Yu Qian,Hong Miao,Boyang Guo,Tingyi Jiang,Shan Zhao,Tianxing Le,Lintian Li,Meng Liu*

Main category: cs.AI

TL;DR: ECHO（Embodied Context and History Orchestration）是一个可审计的记忆架构和服务原型，灵感来自情景编码、整合、情境恢复、再整合和执行控制。该系统针对长程智能体的记忆识别、修订解析和可追溯性需求，在LoCoMo和LongMemEval-S数据集上取得较好表现。


<details>
  <summary>Details</summary>
Motivation: 长程智能体需要记忆来识别相关经验、解决修订问题并暴露可检查的来源。现有的记忆系统在检索和上下文构建方面存在不足，需要一个可审计的记忆架构。

Method: 开发了受情景编码、整合、情境恢复、再整合和执行控制功能启发的记忆架构。评估使用了LoCoMo类别1-4问题（1,536个）和LongMemEval-S全部问题（500个），并与Mem0 OSS进行对比实验。

Result: LoCoMo数据集：Hit@10达96.29%，turn Recall@5达73.64%。LongMemEval-S：Hit@10达97.60%，turn Recall@5达88.84%，session Recall@5达88.71%。五历史BEAM门控失败。Mem0 OSS得分64.84% vs ECHO的41.76%（McNemar p=0.00107）。审计发现查询扩展规则中存在源特定短语。

Conclusion: 虽然无金标准答案字段进入运行时，扩展启用检索得分是描述性开发测量而非独立确认。系统功能灵感来自神经机制但不具有神经等价性。
...
### [221] [What Does CLIP Learn for Regional Geolocalization? Probing Visual Cues and Scene Configuration After Adaptation](https://arxiv.org/abs/2608.21761)
*Changyu Lee,Yeonsoo Park,Abdullah Alfarrarjeh,Seon Ho Kim*

Main category: cs.AI

TL;DR: 本文研究了大都市区内细粒度区域地理定位问题，比较了CLIP预训练模型在不同微调策略下的表现。结果表明 encoder adaptation 相比冻结编码器方法能将准确率从约39%提升至75.94-82.10%，并将预测区域中心的平均距离从12.30km降至3.86km。消融实验表明 adapted models 对边缘检测、模糊处理和 patch scrambling 更敏感，说明模型更依赖完整的场景配置而非粗略结构。


<details>
  <summary>Details</summary>
Motivation: 从街景图像中提取细粒度地理信息具有挑战性，因为相邻区域通常共享粗略的地理线索。现有的CLIP模型在零样本设置下表现不佳，需要研究如何通过迁移学习提升区域判别能力。

Method: 使用9,085张来自大洛杉矶地区八个区域的街景图像，比较零样本CLIP、冻结编码器读取、部分编码器更新、LoRA和完全微调五种方法。通过语义线索移除、边缘图/模糊处理的外观降维、以及patch打乱破坏场景配置进行探测实验。

Result: 冻结编码器方法准确率维持在约39%，而encoder adaptation达到75.94-82.10%。完全微调将预测区域中心的平均距离从12.30km降至3.86km。Adapted models在边缘和模糊处理下保持更高准确率，打乱后42.92-45.56%的预测发生变化（冻结方法仅10.79-14.60%），表明更依赖完整场景配置。

Conclusion: 编码器微调显著提升了大洛杉矶邻近区域间的判别能力，模型更敏感于完整的场景配置结构。没有证据表明仅依靠粗略结构就足以进行预测。植被和天空等语义线索仍然具有影响力。该结论涉及已知位置附近的视角变化，而非地理上分离的泛化。
...
### [222] [Physics-Knowledge-Guided Hybrid Neural Learning for Arctic Sea Ice Concentration Evolution and Short-Range Prediction](https://arxiv.org/abs/2608.21767)
*Maqun Zhang,Feng Gao,Wankun Chen,Hui Yu,Yanhai Gan,Junyu Dong*

Main category: cs.AI

TL;DR: 本文提出了PIHIM（物理信息混合冰模型），这是一个用于海冰浓度（SIC）每日演化的可微分数据驱动混合模型。该模型将深度学习的表示能力与物理约束相结合，通过海冰连续方程组织网络结构，明确考虑动力学传输、热力学驱动的面积增长和损失以及未解析的局部过程。


<details>
  <summary>Details</summary>
Motivation: 海冰浓度演化的精确建模对于极地气候评估和短期海冰预测至关重要。数值方法通常需要复杂的参数化设置和大量计算，而数据驱动方法很少显式编码物理依赖关系。因此需要一种能够结合两者优势的方法。

Method: PIHIM模型根据海冰连续方程中编码的物理依赖关系组织网络结构，明确考虑了冰位移、冻融面积变化和局部误差闭合过程。评估采用两个设置：再分析强迫模拟（检验SIC演化稳定性）和预报强迫预测（评估短期性能），以再分析和观测SIC作为验证参考。

Result: 结果表明，再分析强迫模拟中冰缘保持和误差增长控制得到增强，同时PIHIM在预报强迫条件下保留了可测量的短期预测技能。

Conclusion: PIHIM成功地将深度学习的表示能力与物理约束相结合，实现了过程分解的冰位移、冻融面积变化和局部误差闭合公式，在再分析强迫模拟和短期预测任务中均表现出良好的性能。
...
### [223] [HIRA: A Human-in-the-Loop Retrieval-Augmented Cascade for Document Classification in Regulated Industries](https://arxiv.org/abs/2608.21792)
*Shangxuan Tian,Yanhui Chen,Carlos Queiroz*

Main category: cs.AI

TL;DR: HIRA是一个无需训练的检索增强级联系统，用于受监管行业的文档分类，通过BM25、密集嵌入和图像表示的融合进行分类，辅以本地LLM验证器和人类审核，在保持数据本地化的同时显著减少人工标注需求。


<details>
  <summary>Details</summary>
Motivation: 受监管行业的文档分类面临数据驻留限制、冷启动标签稀缺、审核能力有限和模型治理成本高等挑战，传统方法需要频繁重训练模型，难以满足合规要求。

Method: 提出HIRA系统：1)使用BM25+密集嵌入+图像级表示的验证校准加权倒数排名融合；2)本地托管的LLM验证器处理不确定文档；3)人类审核作为最后手段；4)将纠正存储为边际加权检索示例并更新Dirichlet平滑的混淆图，实现无需更新模型权重的持续改进。

Result: 在80类贸易金融私有语料库(30,233文档)中，仅6.4%需人工审核，Macro-F1从0.6218提升至0.8548；在Tobacco-3482基准上达到0.9423 Macro-F1，比零样本LLM基线高17.4个百分点，LLM调用减少约60%；518个人类纠正(24.8%)即可匹配全标注池的神谕性能。

Conclusion: 选择性人类反馈和检索记忆适应是受监管部署中长尾文档分类的实用替代方案，可避免重复模型重训练。
...
### [224] [Hints, Critics, and Teachers: Prior Injection for Sparse-Reward RL in Vision-Language Math Reasoning](https://arxiv.org/abs/2608.21811)
*Qiqian Fu*

Main category: cs.AI

TL;DR: 本研究针对视觉-语言数学推理中稀疏奖励的强化学习问题，在20,830道视觉数学题数据集上训练了11种不同方法，发现有效的先验知识注入是关键，但评估指标存在严重问题：长期使用的通用分布检查子集与跨域迁移呈负相关，而最难的子集能有效预测迁移效果。


<details>
  <summary>Details</summary>
Motivation: 解决视觉-语言数学推理中强化学习的稀疏奖励问题。基线模型Qwen2-VL-2B仅能正确回答3.6%的 rollout，85-97%的GRPO rollout组完全错误，无法提供梯度。

Method: 在相同条件下训练11种方法，注入不同先验：(1)文本先验（参考解题提示），(2)分布先验（从7B教师模型进行在线策略蒸馏），(3)价值先验（带MSE或HL-Gauss分类损失的价值预训练批评家）。评估指标包括域内测试和跨域迁移（DynaMath）。

Result: 1）先验仅在有效传递到策略时才有帮助：6个先验有效传递的模型组与5个未获有效先验的模型组（无先验基线、教师受限、门控或批评家参数错误）在域内和跨域迁移上完全分离；2）评估发现逆转现象：一个域内子集与跨域迁移呈强负相关（Spearman rho=-0.74, p=0.011），而最难子集呈强正相关（rho=+0.89, p<0.001）；3）提示引导的探索而非UFT辅助损失推动提示增益；4）HL-Gauss交叉熵替代MSE损失带来+14.4分的域内提升。

Conclusion: 有效的先验知识注入对稀疏奖励下的视觉-语言数学推理至关重要，但评估指标选择需谨慎：接近随机准确率的多选子集可能奖励"未改变的模型"，导致虚假的性能评估。最难域内子集和跨域迁移测试是更可靠的评估指标。
...
### [225] [VisAdj: Learning Adjacency Matrices from Node-Link Images](https://arxiv.org/abs/2608.21825)
*Jiahao Xie,Guangmo Tong*

Main category: cs.AI

TL;DR: VisAdj是一个从节点链接图像中学习邻接矩阵的框架，通过注意力稀疏邻居采样器自适应选择高召回率的候选节点对，并使用线图变换器进行联合边推理，显式建模边之间的依赖关系。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖固定的KNN启发式算法选择候选边，无法捕获边之间的依赖关系，导致拓扑信息恢复不完整。

Method: 1. 引入注意力稀疏邻居采样器，自适应选择高召回率候选节点对；2. 使用线图变换器，将候选边作为token，显式建模相邻边之间的依赖关系进行联合推理。

Result: 在合成图、道路网络和船舶图像上的实验表明，VisAdj一致性地以明显优势优于现有基线方法。

Conclusion: VisAdj通过自适应采样和拓扑感知的边依赖建模，有效提升了从视觉观测中恢复图结构信息的准确性和完整性。
...
### [226] [Beyond Success and Failure: Length-Aware Contrastive Learning for GUI Agents](https://arxiv.org/abs/2608.21830)
*Chengyang Gu,Le Zhang,Jingbo Zhou,Yize Chen,Yu Shi,Siqi Bao,Zheng-Fan Wu,Hua Wu,Hui Xiong*

Main category: cs.AI

TL;DR: 本文提出LACL-GUI框架，通过引入轨迹级质量信号来解决现有对比强化学习方法仅依赖结果级监督的问题。该方法在成功和失败轨迹内建立结构化偏好，鼓励简洁的成功执行，并根据与成功轨迹的差异区分失败质量，从而提升GUI代理的性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比RLVR方法主要依赖结果级监督，无法捕捉同一结果类别内轨迹质量的细粒度差异。同时GRPO等方法存在奖励-梯度对齐问题，导致优化效率低下且不稳定。

Method: 提出LACL-GUI框架，包含三个核心组件：1）在成功轨迹内建立偏好，鼓励简洁执行；2）在失败轨迹中根据与成功轨迹的差异程度区分质量；3）保留对比学习的优化稳定性优势。

Result: 在GUI代理基准测试上的实验表明，LACL-GUI提供了更有效的学习信号，相比先前方法持续提升代理性能，验证了轨迹级监督在对比RLVR中的价值。

Conclusion: 轨迹级质量信号的引入能够显著提升对比RLVR的效果，LACL-GUI为GUI代理的训练提供了一种更稳定有效的优化范式。
...
### [227] [GameXpert-Bench: How Far Are Coding Agents from Expert Game Development?](https://arxiv.org/abs/2608.21833)
*Kun Chen,Haorong Hong,Peizhong Gao,Jianfeng Lin,Tongxu Luo,Yuxuan Xie,Chenxu Liu,Jieling He,Zhongyuan Liu,Zeno Zeng*

Main category: cs.AI

TL;DR: GameXpert-Bench是一个新的基准测试套件，通过三个轨道（GameGen生成、GameFix修复、GameOpt优化）全面评估大语言模型作为游戏开发编码代理的能力，包含97个生成任务、100个修复任务和17个优化链。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常通过评估最终产物或孤立开发阶段来评估LLM的游戏开发能力，但游戏开发需要在单个可执行文件中同时运行程序逻辑、视觉和音频内容、界面、交互和可玩性。因此需要评估游戏产品和开发过程两方面。

Method: 基准测试包含三个轨道：1) GameGen用于从单一请求在空工作区生成完整游戏；2) GameFix用于评估缺陷报告或代理自行发现缺陷时的诊断和修复；3) GameOpt用于评估基于用户与代理之间真实开发轨迹的请求链的累积优化。使用现场游戏交互、确定性行为测试或最终产品标准配合回归检查来评估每个轨道。

Result: 该套件包含97个跨11个类型的生成任务；100个来自50个游戏关卡的修复任务（经人工验证，每个关卡包含19-27个注入的bug）；以及17个包含6轮和102个请求的优化链。当前代理在生成可玩基础和实现明确需求方面更可靠，但在发现缺陷、验证运行时行为和跨更改保留功能方面表现较差。

Conclusion: GameXpert-Bench引入了一个全面的基准测试套件，由三个互补的轨道组成，用于评估LLM作为游戏开发编码代理的能力。当前代理在生成可玩基础和实现明确需求方面更可靠，但在发现缺陷、验证运行时行为和跨更改保留功能方面表现较差。
...
### [228] [LLM4LLM: Bridging Kernel Benchmarks and Real Deployment via Closed-Loop Agentic Optimization](https://arxiv.org/abs/2608.21836)
*Hui Zeng,Pengfei Yang,Yanxin Chen,Fusong Ju,Xinran Wei*

Main category: cs.AI

TL;DR: LLM4LLM是一个部署感知的闭环优化框架，用于解决语言模型推理中内核优化从基准测试到实际部署的差距问题。该框架从目标推理脚本出发，提取相位感知优化任务，通过经验引导的回合式智能体进行搜索，并通过模型内验证接受补丁。


<details>
  <summary>Details</summary>
Motivation: 论文识别了一个基准测试到部署的差距问题：在独立内核基准测试中表现正确和快速的候选内核，在集成到真实推理工作负载后可能表现出不同的性能、安全性或相位行为。传统的基准测试无法完全反映实际部署中的表现。

Method: LLM4LLM框架采用以下步骤：1)从目标推理脚本出发；2)提取相位感知的优化任务；3)使用经验引导的回合式智能体进行搜索；4)通过模型内验证接受补丁。该方法强调部署感知的闭环优化。

Result: 在A100和H100 GPU上的10个语言模型推理工作负载中，LLM4LLM在每个评估的模型上都提升了端到端延迟性能，实现了A100上3.91倍、H100上6.98倍的几何平均加速；在KernelBench Level 2上达到了最高2.745倍的几何平均加速。

Conclusion: LLM4LLM能够有效弥合基准测试与实际部署之间的性能差距，通过部署感知的闭环优化方法，在真实的语言模型推理场景中实现显著的性能提升。
...
### [229] [AI Watchdog: Agent Interfaces for Detecting and Defending Against Manipulative Dark Patterns in AI Conversations](https://arxiv.org/abs/2608.21841)
*Rachel Poonsiriwong,Chayapatr,Archiwaranguprok,Constanze Albrecht,Monchai Lertsutthiwong,Pattie Maes,Pat Pataranutaporn*

Main category: cs.AI

TL;DR: AI Watchdog是一个浏览器代理接口，用于监控实时对话、检测五种黑暗模式类别（谄媚、品牌偏见、拟人化、潜入和有害生成），并在发生时向用户发出警报。研究通过150人的五组实验比较了不同干预时机和参与模式的效果，发现仅"即时警告无认知强制"的干预能显著将用户对包含黑暗模式的AI推荐遵从率从71.7%降低到53.7%。


<details>
  <summary>Details</summary>
Motivation: 对话AI越来越多地塑造关键决策，但用户缺乏识别和抵制操纵的支持。需要一种独立于对话AI的工具来检测黑暗模式并提醒用户。

Method: 在150名参与者中进行五条件组间实验，比较无干预控制组与四种干预配置（时机：预先防范vs即时；参与模式：无认知强制vs有认知强制）。使用开放权重的轮次级分类器检测五种黑暗模式类别。

Result: 各条件下参与者很少主动标记操纵性轮次，事后 awareness 无显著差异。但即时警告无认知强制是唯一显著减少黑暗模式AI推荐遵从率的干预（从71.7%降至53.7%，下降18个百分点）。较低的误导信息易感性与更多标记行为相关，但与较低遵从率无关；较高的AI信任度与更高遵从率和更低自我报告 awareness 相关。

Conclusion: 明确识别对话黑暗模式与行为抵制AI引导可能是不同结果，需要进一步研究及时、低摩擦的防御界面。即时、低摩擦的干预可能是减少黑暗模式影响的有效策略。
...
### [230] [MemGuard: Persisting Verifier Signals for LLM-Agent Memory Governance](https://arxiv.org/abs/2608.21867)
*Haoyu Wang,Guangyuan Dong,He Liang,Zijing Zhang,Jiachen Luo,Chuang Liu,Chao Xue,Hao Tang*

Main category: cs.AI

TL;DR: MemGuard是一种新型LLM代理内存系统，通过将验证器输出转化为持久生命周期元数据（奖励、置信度、标签和不确定性描述符），有效解决了现有内存系统面临的两个核心失败模式：不可靠的 admission（失败轨迹和误导性观察进入内存）和内存漂移（重复、陈旧、冲突记录）。


<details>
  <summary>Details</summary>
Motivation: LLM代理正从单提示使用转向长任务流，可重用内存成为核心能力。然而，现有内存系统在数百次交互中会失效，原因包括：(1)不可靠的admission：失败轨迹、偶然成功和误导性观察因看似相关而进入内存，进而误导后续决策；(2)内存漂移：长期运行的内存积累重复、陈旧和冲突的记录，仅靠检索无法修复。

Method: MemGuard的核心创新在于将验证器输出视为持久生命周期元数据而非一次性过滤器。具体包括：(1)将多标准分数-标记验证转化为奖励、置信度、标签和不确定性描述符；(2)这些元数据在激活前附加到每个候选记忆，并在检索、冲突解决、总结和归档过程中重复使用。评估在Terminal-Bench 2.0、SWE-Bench Verified、WebArena和Mind2Web四个基准上测试了四种骨干模型，与四个内存基线加上验证器单独控制进行对比，在相同运行预算下进行。

Result: 在五个种子的平均结果中，MemGuard在所有16种骨干-基准设置中均取得最佳成功率和最低平均步数。相比最强内存基线ReasoningBank，MemGuard在WebArena上提升7.9个成功率点，在Mind2Web上提升5.6个步数-成功率点，在终端和软件工程基准上提升2.4-3.5个点。

Conclusion: MemGuard通过将验证器输出转化为持久生命周期元数据，有效解决了LLM代理内存系统中的admission可靠性和内存漂移两大难题，在多种基准测试和骨干模型上均取得了显著的性能提升，为构建可靠的长期运行LLM代理系统提供了新的解决方案。
...
### [231] [HiMA-MDD: A Hierarchical Multi-Agent Harness for Interpretable Multimodal Depression Detection in Clinical Interviews](https://arxiv.org/abs/2608.21868)
*Ao Chen,Xiaojiang Peng*

Main category: cs.AI

TL;DR: HiMA-MDD是一种用于抑郁症评估的分层多智能体系统，通过三层智能体架构将多模态临床访谈整合为PHQ-8评分档案，解决现有LLM系统缺乏明确协调机制的问题。


<details>
  <summary>Details</summary>
Motivation: 抑郁症多模态临床访谈评估需要将分散在多个症状中的证据整合为连贯的PHQ-8档案。这一过程具有层次性：相关证据通常在本地问答交流中稀疏且依赖上下文，多个交流共同支持症状级判断，最终评估取决于完整症状档案的一致性。现有LLM系统要么整体处理访谈，要么在通用智能体角色间分配工作，均未提供明确的协调机制来协调证据访问、项目评分权限、有限反馈和跨层次状态记录。

Method: 提出三层智能体层次结构：1）非智能体预处理构建保留上下文的 multimodal QA单元，Layer 1识别候选QA到项目关系并支持基于项目的证据路由；2）Layer 2将症状组分配给操作因子专家，每个专家负责一个临时项目评分；3）Layer 3审计完整临时档案，最多请求一轮针对性修订，重建已验证的PHQ-8档案。使用Qwen2.72B-Instruct作为框架主干。

Result: 在E-DAIC数据集上的实验表明，HiMA-MDD优于对比的最先进方法。

Conclusion: 分层设计自然产生分层证据追踪，保留所有中间证据、判断和修订以供审计。最终项目评分确定性生成总分和筛查决策。该架构有效解决了临床抑郁症评估中证据整合和层次化推理的问题。
...
### [232] [From Solver Feedback to Faithful Plans: Multi-Role Reinforcement Learning for Symbolic Planning](https://arxiv.org/abs/2608.21897)
*Chenghao Zhang,Yikai Mao,Shanqi Liu,Haoyu Gao,SaiSai Hu,Dan Roth*

Main category: cs.AI

TL;DR: 研究如何仅通过求解器反馈学习忠实于自然语言的PDDL形式化，而无需人工标注。提出一种求解器 grounding 的多角色强化学习框架，让单个语言模型同时扮演Actor、Judge和Editor三种角色，分别负责生成、验证和修复PDDL规范。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在将自然语言指令转换为可执行的符号规范时仍然脆弱，需要昂贵的PDDL标注，且可能以语义不忠实的方式利用求解器成功。需要一种无需人工演示、仅依靠求解器反馈的学习方法。

Method: 提出求解器 grounding 的多角色强化学习框架：Actor负责提出PDDL规范，Judge提供求解器校准的质量信号，Editor执行有界诊断条件下的修复。在PlanBench数据集上进行了实验。

Result: 平均成功率从LLM+P的35.5%提升至70.8%，实现66.3%的忠实成功率，语义漂移降至6.4%。

Conclusion: 将求解器反馈组织成生成、验证和修复角色能够实现更可扩展且忠实于语义的符号规划，无需人工标注即可显著提升LLM的规划能力。
...
### [233] [Training Needs Trustworthy Worlds: Verified Synthetic Web Environments for Agent Learning](https://arxiv.org/abs/2608.21898)
*Chenghao Zhang,Canran Xiao,SaiSai Hu,Dan Roth*

Main category: cs.AI

TL;DR: 本文提出一种可验证的合成网页环境构建框架，通过结构化脚手架表示、缺陷修复和状态验证，提升网页智能体训练质量。实验表明该方法将可行任务率从48.6%提升至94.8%，并改善了策略迁移效果。


<details>
  <summary>Details</summary>
Motivation: 当前网页智能体的训练依赖于合成环境，但这些环境存在表面可信但实际包含断链、状态不一致或不可行任务的缺陷，导致训练效果与真实部署存在显著差距。

Method: 框架将每个生成的网站表示为结构化脚手架，包含页面、导航链接、数据库记录、状态变更标记和任务约束；在策略训练前验证并修复结构、语义、一致性和可行性缺陷；交互时通过验证的状态变更标记执行后端更新，基于验证的任务进度谓词编译奖励信号。

Result: 在500个覆盖六个领域的合成环境中，方法将任务阻断缺陷从48.6%降至5.2%（可行任务率提升至94.8%），生成更强的PPO策略，并在WebArena、WebShop和MiniWoB++上实现更好的迁移效果，且评估时无需调用LLM。

Conclusion: 验证后的合成环境可作为紧凑网页智能体可扩展且可靠的训练基座，推动合成网页智能体学习从表面可信度向可执行、状态可验证的监督范式转变。
...
### [234] [Consistency Is Not Coherence: Orientation Search for Certified Alignments Between 4D Defence Upper Ontologies](https://arxiv.org/abs/2608.21914)
*Fabio Rovai*

Main category: cs.AI

TL;DR: 将UK和NATO防御数据基础设施下的三个上层本体（IES、HQDM、BFO）进行对齐，构建17-对应关系的OWL交叉映射，并使用HermiT推理器进行推理验证。


<details>
  <summary>Details</summary>
Motivation: IES和HQDM之间不存在公共对齐，而两者都是国防数据基础设施的核心上层本体，需要创建对齐以支持防御数据的互操作性和集成。

Method: 将手工创建的17个对应关系转换为OWL格式，使用HermiT推理器对完整合并的本体进行推理，验证对齐的一致性和有效性。

Result: 生成了三个具有更广泛适用性的重要结果，这些结果超出了IES和HQDM这一对本体的范围。

Conclusion: 通过将手工交叉walk提升为形式化OWL并应用自动推理，成功实现了三个上层本体的对齐，为国防数据基础设施的互操作性提供了可扩展的解决方案。
...
### [235] [ESCRAG-R1: Retrieval-Augmented Reinforcement Learning for Emotional Support Conversation](https://arxiv.org/abs/2608.21925)
*Weichu Liu,Yuxuan Hu,Yirong Sun,Ningning Mao,Ziyun Zhang,Jian Chen,Mingyang Xu,Qishan Zhong,Chengming Li*

Main category: cs.AI

TL;DR: ESCRAG-R1通过将检索增强的心理指导集成到GRPO强化学习框架中，解决了情感支持对话系统中专业知识与同理心难以自然融合的问题。


<details>
  <summary>Details</summary>
Motivation: 现有ESC系统难以同时实现结构化、阶段感知推理和同理心-专业知识无缝对齐，导致临床策略和通用安慰的人工拼接。

Method: 提出ESCRAG-R1框架，将基于检索的心理指导集成到Group Relative Policy Optimization (GRPO)中，将外部知识转化为学习信号刺激生成前的显式内部推理；构建ESC-Preference数据集，基于Client-Counselor-Judge评估框架。

Result: 显著优于现有基线，减轻了表面拼接，实现了专业指导和情感表达的自然整合。

Conclusion: ESCRAG-R1能有效实现专业心理指导和情感表达的自然整合，为情感支持对话系统的设计提供了新思路。
...
### [236] [GuardianBench: A Same-Scene Instruction-Contrastive Benchmark for Latent Contextual Risk in Embodied AI](https://arxiv.org/abs/2608.21928)
...
TL;DR: GuardianBench是一个针对具身AI的指令对比安全基准，通过3024个指令-场景示例（same-scene Safe/Unsafe对比对）来评估视觉语言模型在潜伏上下文风险下的安全推理能力。


<details>
  <summary>Details</summary>
Motivation: 在具身AI中，安全风险可能是潜伏的： benign指令和安全场景组合后会变得危险。之前工作关注改变视觉上下文或执行时动态，但固定场景只改变指令这个轴未被探索。国际安全标准为基准提供了理论依据。

Method: 构建基于国际安全标准的指令对比基准GuardianBench，包含3024个跨多种 hazard categories 的指令-场景示例，组织为same-scene Safe/Unsafe对比对。进行系统性的rationale审计来定位失败原因，并提出VLOS（Verdict Log-Odds Supervision）轻量级判决级目标作为后训练方案。

Result: 基准测试显示模型存在指令不敏感判决问题：模型在给定场景下对两个指令都倾向于批准。主要模型平均配对准确率仅24.1%。主要失败原因是模型未能绑定区分安全与不安全组合的指令相关线索。VLOS能显著提升开放权重骨干网络的性能。

Conclusion: GuardianBench作为一个受控评估套件，能够暴露和改进指令-场景组合下潜伏上下文风险的安全推理。贡献包括：潜伏上下文风险任务 formulation、基于标准的对比基准构建、配对级和理由级失败诊断、以及基准驱动的判决校准。
...
### [237] [Multimodal Prompt Learning with Irregular EHRs for Robust Monitoring of Critical Care Patients](https://arxiv.org/abs/2608.21941)
*Yixin Yang,Yueyang Sun,Weichen Liu,Xianbing Zhao,Sicen Liu*

Main category: cs.AI

TL;DR: 提出了一种多模态提示学习框架，用于在模态缺失情况下进行ICU临床预测。该框架通过四种互补的提示类型（生成式提示、缺失信号提示、缺失类型提示和时间提示）来处理不完整的多模态电子健康记录数据，使模型能够捕获缺失感知模态内依赖性和跨模态交互。


<details>
  <summary>Details</summary>
Motivation: 现实临床环境中，多模态电子健康记录（包括结构化生理时间序列和纵向临床笔记）虽然能提供危重症预测的互补信息，但单个模态可能部分观察或完全不可用，导致现有多模态模型性能大幅下降。

Method: 提出统一架构中的四种提示类型：1) 生成式提示为不可用模态构建代理潜在表示；2) 缺失信号提示区分观察表示与生成表示；3) 缺失类型提示对不同模态可用性配置进行条件调节；4) 时间提示对时间编码临床序列进行条件特定聚合。

Result: 广泛实验表明，该方法在两种缺失设置下的评估指标均优于现有方法。消融和鲁棒性分析进一步验证了四种提示类型的互补贡献及框架从不完备多模态EHR数据中进行临床预测的有效性。

Conclusion: 该多模态提示学习框架能够有效处理模态缺失问题，在ICU患者预测任务中展现出良好的性能，为实际临床应用中处理不完整多模态数据提供了可行的解决方案。
...
### [238] [SSDi8: Accurate and Efficient 8-bit Quantization for State Space Duality](https://arxiv.org/abs/2608.21952)
*Hyunwoo Kim,Byoungchan Ko,Minseok Kang,Minwoo Kim,Dongjin Lee,Jaehoon Lee,Sungroh Yoon,Dahuin Jung*

Main category: cs.AI

TL;DR: SSDi8是首个针对Mamba-2的SSD架构设计的训练后量化框架，通过解耦元素级乘法和矩阵乘法、通道自适应量化等创新技术，在保持FP16精度的同时实现高达1.4倍的加速，并成功部署在Orin NX设备上验证了其资源受限环境下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Mamba-2通过结构化状态空间对偶性(SSD)整合了循环和注意力模式，提高了效率和可扩展性，但同时也带来了显著的内存和延迟开销，需要针对SSD设计高效的压缩策略。

Method: 1) 重新 formulation解耦元素级乘法和矩阵乘法，实现跨模块量化激活复用；2) 在成本效益点对通道变化激活进行自适应量化；3) 利用SSD内在维度分解，显式利用不同轴上的异常值分布差异；4) 引入基于每通道误差统计的误差校正项。

Result: SSDi8在W4A8和W8A8设置下实现与FP16相当的精度，同时提供高达1.4倍的加速，并在Orin NX设备上验证了其在资源受限环境中的鲁棒性。

Conclusion: SSDi8作为首个针对SSD的后训练量化框架，通过创新性的架构解耦和自适应量化技术，成功在精度和效率之间取得平衡，为SSD在边缘设备上的部署提供了可行方案。
...
### [239] [Repo2Skill-Evo: Repository Skills Go Stale in Silence](https://arxiv.org/abs/2608.21964)
...
TL;DR: 研究揭示即使前沿LLM代理也无法可靠维护软件仓库技能知识。研究发现所有技能集在版本更新后都会部分失效，6个前沿代理仅能达到29.9%-69.7%的平均F1分数，且主要面临技能覆盖不足或过度编辑两种错误。


<details>
  <summary>Details</summary>
Motivation: LLM代理日益在不断演化的软件仓库中运行，成功依赖于仓库特定的程序性知识（API调用、脚本运行、约定规范）。将此类知识外部化为可复用单元虽能提升性能，但其版本特异性也导致知识脆弱性——发布更新后技能可能悄然失效却无任何信号，使得知识衰退不可见。

Method: 提出Repo2Skill-Evo方法，将每次发布转换视为技能维护任务：给定V1技能集和官方V1到V2补丁，代理需更新过时技能内容同时保留仍有效的指导。在57个真实仓库和105个精选发布转换上评估。

Result: 每个评估的发布转换都会使部分V1技能集失效。前沿代理在平衡过时内容召回与过度编辑精度的补丁锚定移除指标下，仅达到29.9%-69.7%的avg@3宏F1分数。两种对立错误占主导：技能集中受影响文件覆盖不完整导致过时内容未触及，而过度广泛编辑虽提高召回但降低精度。

Conclusion: 仓库技能悄然失效，即使前沿代理也无法可靠维护它们。这表明将知识外部化为技能虽有益，但需要建立相应的维护机制，否则会导致隐性的知识衰退。
...
### [240] [Closed-loop AI achieves certifiable engineering design](https://arxiv.org/abs/2608.21976)
...
TL;DR: The AI Engineer是一个代理AI框架，将大型语言模型与确定性工程后端耦合，用于自动化复杂物理工程设计（如浮式风力发电机结构），填补了科学发现自动化在工程设计领域的空白。


<details>
  <summary>Details</summary>
Motivation: 科学发现自动化的代理AI已覆盖论文生成、专家级编码、治疗方案和自主实验，但在复杂物理工程设计领域存在空白。原因在于工程候选方案必须同时满足流体力学、固体力学和结构稳定性的多重约束。

Method: 该框架采用闭环设计：将自然语言需求转换为设计域几何和网格；使用双向进化结构优化（BESO）耦合CalculiX求解器进行拓扑优化；使用粒子群优化（PSO）耦合Zwind进行构件尺寸优化；开发自动化评审器使用基于11个实际浮式风力发电项目校准的五维评分函数（承载力、钢材强度、单位成本、施工性、疲劳寿命）评估每个候选设计。

Result: 框架通过关闭验证机制（每个方案都通过确定性物理和编码极限状态判断），最高分设计通过中国船级社（CCS）的原则性认可（AIP），相比人工优化的TuQiang基准，钢材质量和单位资本成本均降低8.1%，同时满足所有AIP标准。

Conclusion: The AI Engineer代表了工程设计自动化的重要进展，通过代理AI与确定性物理求解器的闭环集成，实现了复杂工程结构的自动化优化设计。该方法区别于开放式生成系统的关键在于验证闭合机制。剩余限制包括详细设计和制造硬约束。
...
### [241] [Beyond Similarity: Heterogeneous Graph Learning for Multi-Objective Food Substitution in Charitable Food Agencies](https://arxiv.org/abs/2608.21979)
*Naimur Rahman Chowdhury,Limon Bin Hossain*

Main category: cs.AI

TL;DR: 本研究提出了一种基于异构图神经网络(HeteroGNN)的慈善食物机构食品替代推荐框架，旨在帮助机构在资源有限的情况下做出更合适的食品替代决策。


<details>
  <summary>Details</summary>
Motivation: 慈善食物机构依赖临时实物捐赠，经常面临特定食品短缺问题，需要提供替代品。但因资源限制缺乏直接的消费行为记录，难以做出满足多重标准的替代决策。

Method: 构建统一关系图整合大规模公共数据（美国家庭食品消费行为和食品营养信息），将替代推荐建模为多目标排序问题（行为亲和力、健康适宜性、替代相似性），使用异构图神经网络进行训练验证。

Result: 框架利用节点特征之外的关联信息预测消费行为；在行为和营养特征信息不完整时仍保持鲁棒性；不同目标间相关性较弱，验证了多目标建模的必要性。

Conclusion: HeteroGNN框架可帮助慈善食物机构在信息有限的情况下做出情境化的替代推荐，具有实际应用价值。
...
### [242] [Redteaming Leading Arabic LLMs with ASAS](https://arxiv.org/abs/2608.21985)
*Fidaa Abed,Haidar Khan,M Saiful Bari,Babar Khan,Abdalghani Abujabal*

Main category: cs.AI

TL;DR: ASAS是首个完全人工策划的阿拉伯语红队测试基准，包含801个提示词，涵盖8个安全类别和8种攻击策略，用于评估阿拉伯语大语言模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 随着阿拉伯语地区大语言模型采用的增长，确保其安全性和文化对齐变得越来越重要，但阿拉伯语LLM安全性在对抗性评估设置中仍未得到充分探索。

Method: 人工注释者使用结构化4分安全量表对七个主流阿拉伯语模型（包括GPT-4o、Claude 3.7 Sonnet、ALLaM和FANAR）的响应进行评分。

Result: 大多数模型无法防御50%的不安全提示词；武器和非法物质等高危害类别存在重大安全漏洞；直接攻击和混淆攻击最为有效；语言对齐不能跨语言转移；自动化安全评判（如GPT-4o）表现不如人工注释者。

Conclusion: ASAS提供了文化根植的基准和红队测试协议，可推动阿拉伯语LLM安全性的进展。
...
### [243] [DynaContext: Self-Improving Dynamic Contextualization of Optimized Prompts for Heterogeneous Parameter Extraction](https://arxiv.org/abs/2608.22014)
*Joe Yu,Shibin Thomas Stanley Paul,Sven Mayer*

Main category: cs.AI

TL;DR: DynaContext是一个结合离线优化提取核心与推理时上下文自适应和验证门控自改进的框架，用于解决静态提示无法适应不同实例间上下文、约束和证据变化的问题，特别应用于电子元件描述的参数提取任务。


<details>
  <summary>Details</summary>
Motivation: 当前自动化提示和技能优化通常产生单一静态指令，在推理实例间重复使用，直到下一次优化周期。然而这种方法无法适应不同实例间变化的需求、约束和证据。例如，电子元件描述中的参数提取任务：电阻器、电容器、晶体管和连接器需要不同的字段、单位约束和演示，且每个输入提供不同的证据状态。

Method: DynaContext框架包含：(1)离线优化的提取核心，使用GEPA或SkillOpt学习；(2)推理时上下文自适应；(3)验证门控自改进。系统将每个项目路由到内部、外部或回退证据路径，并从核心、模式、证据、未解决字段和验证演示组成项目特定提示。确定性验证和LLM判断门控每个输出，不确定情况转人工审核，只有经人工验证的更正进入演示内存。

Result: 单类别基准测试中，平均准确率从基线提示的86.6%提升到独立SkillOpt的96.9%和最佳DynaContext配置的98.6%。在850个异构黄金参数事实中，平均字段级F1从无优化、无演示对照组的51.8%提升到仅动态演示的59.2%、仅优化核心的66.9%和两者结合的71.0%。固定模型下，完整配置比部署的静态提示管道平均高出17.3个F1点。

Conclusion: DynaContext通过结合离线优化的提取核心与推理时上下文自适应和验证门控自改进机制，能够有效解决静态提示无法适应不同实例变化的问题，显著提升参数提取任务的准确率和F1分数。
...
### [244] [SPAR-Hate: An Auditor-Guided Multi-Agent Framework for Bilingual Hate Speech Parsing](https://arxiv.org/abs/2608.22018)
*Yifan Lyu,Dianqing Lin,Xinran Li,Jiaqi Qiao,Xiujuan Xu*

Main category: cs.AI

TL;DR: SPAR-Hate是一个审计员引导的多智能体框架，用于双语仇恨言论解析，从受害者、主持人和文化旁观者三个互补视角生成证据支持的判断，通过证据约束的仲裁过程解决冲突并聚合为结构化输出。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要强调基准评估，而较少关注仇恨言论解析中涉及的文化、语言和社会群体挑战，需要一个能处理这些挑战的框架。

Method: 提出SPAR-Hate框架，首先将文档分解为子句级决策单元，然后从三个互补视角（受害者、主持人、文化旁观者）生成证据支撑的判断，最后通过证据约束的仲裁过程解决角色特定预测之间的冲突并聚合为结构化样本级输出。

Result: 在STATE-ToxiCN和TBO基准测试上的实验表明，SPAR-Hate在多种大型语言模型上持续改进双语仇恨解析，在双语多元组提取任务上实现最先进结果，在更严格结构评估指标下获得最大提升。

Conclusion: SPAR-Hate框架能够有效处理仇恨言论解析中的文化、语言和社会群体挑战，为双语仇恨言论检测提供了一种新的有效方法。
...
### [245] [One-Step Evolution for Long-Time Extrapolation: An Error-Bound-Informed and Prior-Guided Neural Residual Framework for Autonomous PDEs](https://arxiv.org/abs/2608.22026)
*Maqun Zhang,Feng Gao,Wankun Chen,Hui Yu,Yanhai Gan,Junyu Dong*

Main category: cs.AI

TL;DR: 提出了一种无监督的数值先验引导的物理约束方法，用于PDE系统的长时间演化模拟。该方法结合低成本数值先验和弱形式PDE残差，在无需真实轨迹监督的情况下训练，在5个基准测试中均优于10个基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法存在局限：神经算子依赖大量轨迹数据，物理信息方法在长时间外推时稳定性不足。长时间演化可通过固定步长演化算子的重复组合实现，因此需要控制该算子的近似误差及其递归组合下的误差传播。

Method: 提出数值先验引导的物理约束方法：1)使用低成本的数值先验降低单步演化算子的近似难度；2)利用弱形式PDE残差作为单步误差项的可计算代理，用于误差传播界。在训练中排除真实轨迹监督。

Result: 在5个基准测试（涵盖4类PDE）中，该方法均降低了相对于数值先验的长时间外推误差，并在每个案例中优于最佳基线方法，实现了无需真实轨迹监督下不同PDE的长时间模拟精度提升。

Conclusion: 该方法为PDE长时间模拟提供了一种有效的无监督学习框架，通过数值先验与物理约束的结合，成功解决了长时间外推的误差累积问题，具有广泛的适用性。
...
### [246] [More Accurate or More Efficient? Evaluating Locally Deployed Compact Open-Weight Language Models for Mathematical Reasoning](https://arxiv.org/abs/2608.22048)
*Orion Powers,Daniella Seum,Khaled Slhoub*

Main category: cs.AI

TL;DR: 本文提出了一种针对本地部署的大型语言模型在数学推理任务上的评估程序，结合固定推理设置、分层答案提取与验证、失败模式分类和资源测量，并使用配对显著性检验和效应量报告准确率。初步研究测试了三个小于50亿参数的开源模型（Gemma3:4b、Phi3:3.8b、Qwen3:4b）在三个数学数据集上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前许多LLM评估强调准确率，但较少量化本地运行时间和能耗、描述失败模式或在受控条件下进行配对统计比较。本地部署LLM出于隐私、成本和可访问性原因越来越受欢迎，但缺乏全面的评估方法。

Method: 使用受控的文档化程序评估本地托管的LLM：固定推理设置、分层答案提取和验证、显式失败模式分类、逐问题资源测量，使用配对显著性检验和效应量报告准确率。三个模型在相同本地推理服务器、工作站上运行，使用共享提示模板、受控设置和匹配的题库。测试数据集包括八年级数学、微积分I和高等概率统计。

Result: 没有单一模型占据优势：Qwen3:4b在两个数据集上准确率最高，Gemma3:4b在微积分I上最高；Gemma3:4b每瓦时正确答案数约为Qwen3:4b的三倍，且输出token少很多；Qwen3:4b每个问题需要更多生成时间、能源和输出；Phi3:3.8b在所有三个数据集上准确率显著较低，其低提取失败率表明是答案错误而非未解析输出。

Conclusion: 准确率本身不足以作为选择本地模型的基础。评估本地LLM需要综合考虑准确率、能耗、生成时间、输出token等多个维度。
...
### [247] [GenCoord: Skill-Path Commitments under Private Information](https://arxiv.org/abs/2608.22055)
*Peng He,Junning Zhu,Haohan Yuan,Jianpeng Liang*

Main category: cs.AI

TL;DR: GenCoord是一个多智能体协调系统,解决双方持有不同私有信息时的任务执行问题。它将任务结果转换为可执行的技能路径承诺,通过本地Qwen3.5-0.8B模型生成SELF计划和REQ请求,在边界反馈条件下进行修订,最终编译为Mineflayer技能并验证。


<details>
  <summary>Details</summary>
Motivation: 解决分布式场景中信息不对称问题:构建智能体知道要建造什么,但不知道工作单元能执行什么变换; teammate智能体知道转换能力但不知道任务目标。现有方法无法确定谁应该行动、应该交接什么、如何继续联合任务。

Method: 1)本地Qwen3.5-0.8B模型发出多步骤SELF计划和peer REQ请求; 2)边界反馈条件在决定能力为peer-local时触发修订; 3)解析、檢查、规范实例化承诺; 4)编译为Mineflayer技能并通过交接和终端状态验证; 5)反事实干预实验验证双向任务结果遵循注入的任务后果。

Result: 1)正确能力反馈将配对局部信息差距从50%关闭到100%; 2)多步骤承诺提高6.9点模板成功率,减少32%模型决策; 3)短DSL相比自由形式通信减少92.8%对等流量,承诺中位时间减少68.2%; 4)在128个语义聚类上达到匹配闭环质量。

Conclusion: 可执行任务后果是连接分布式本地推理与验证联合行动的协调单元。这一发现为多智能体协作提供了新的Coordination primitive。
...
### [248] [MEMORY Wins All: Indirect Bias Injection Attacks via Social Media Feeds](https://arxiv.org/abs/2608.22061)
...
TL;DR: 本文提出IBIA（间接偏见注入攻击），通过外部内容将对抗性立场植入个人AI代理的记忆中，无需直接访问代理或其记忆。攻击结合了评论伪装、评论水印和类别锚定三种机制，可在后续相关请求中激活植入的偏见。实验在BiasBench基准上评估，攻击成功率达91.2%，并提出内存边界防御可将其降至80.6%。


<details>
  <summary>Details</summary>
Motivation: 个人AI代理在执行网页浏览、邮件处理和社交媒体摘要等任务时会摄入外部内容并存储在持久记忆中。作者发现这种普通的外部内容摄入为操纵代理后续行为开辟了间接路径，因此提出IBIA攻击来探索这一安全威胁。

Method: IBIA攻击结合三种机制：(1)评论伪装使伪造内容与周围讨论保持一致；(2)评论水印使伪造内容在策划过程中可被轻量级识别；(3)类别锚定使保留的立场在后续相关请求中更加突出。评估在BiasBench基准上进行，包含6000条对抗性伪造社交评论和120封邮件。

Result: 水印检测可识别95.9%的注入评论。在OpenClaw设置下，IBIA在四个下游任务中平均达到91.2%的对抗性对齐响应率（AAR），其中在前沿模型GPT-5.5上达到86.6%。内存边界防御可将AAR降低至80.6%。

Conclusion: IBIA攻击通过外部内容间接操纵AI代理行为是可行的，对个人AI代理的安全构成严重威胁。提出的内存边界防御可部分缓解此类攻击，但仍需进一步防御措施。
...
### [249] [Search Broadly, Seek Evidence on Both Sides, Decide Narrowly: Evidence-Admissible GraphRAG for Longitudinal Clinical Event Verification](https://arxiv.org/abs/2608.22062)
*Xingtao Lin,Yubo Feng,Weixin Liu,Hangqi Ren,Junchao Zhou,Caiwan Sun,You Chen*

Main category: cs.AI

TL;DR: MedEventGraph-RAG是一个证据可验证的框架，用于纵向临床事件关系验证，通过患者特定图表示事件并将每个事件链接到源证据，在多个数据集上实现了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 纵向临床事件关系验证具有挑战性，因为证据分散在结构化记录、笔记、实验室轨迹、就诊记录和时间中，且否定、时间不匹配、重复文档和冲突发现可能使检索到的信息看起来相关但实际上无法确立关系。

Method: MedEventGraph-RAG将事件表示在患者特定图中，并将每个事件链接到源证据（结构化行、笔记片段、时间戳、数值轨迹）。给定指定事件、关系和临床范围的验证查询，图引导发现候选事件链，并从支持和矛盾两边检索证据。查询特定的证据契约按患者身份、范围、发生绑定和源可追溯性进行过滤，然后由单独的评估器确定支持、冲突、反驳或不足的结果。

Result: 在i2b2、n2c2、MIMIC-IV和LUNGUAGE的十个协议上，MedEventGraph-RAG在时间、药物不良事件和记录订单验证上分别达到78.6、67.3和96.8的平衡准确率，比最强基线分别提高26.9、4.9和30.4个百分点。在证据掩蔽下，它达到92.2的平衡准确率且没有错误支持预测。当中间事件隐藏时，它在57.9%的i2b2和70.0%的LUNGUAGE案例中恢复了完整的可追溯事件链。

Conclusion: 将广泛证据发现与狭窄证据可验证评估分离可以改善纵向临床验证并减少不支持的结论。
...
### [250] [From SQL Generation to Tool Selection: A Domain-Oriented Pattern for MCP Servers](https://arxiv.org/abs/2608.22063)
*Bartolomeo Bogliolo*

Main category: cs.AI

TL;DR: 本文提出领域导向工具模式(Domain-Oriented Tooling Pattern)，通过用预定义的领域工具替代运行时SQL生成，降低LLM模型等级要求。参考实现MCP Blueprint框架将领域工具声明为YAML元数据+外部参数化SQL文件。评估显示：垂直化工具包达到0.939平均分（raw SQL仅0.666，通用工具包0.605），最小模型从0.583提升至0.929，成本降低一个数量级。


<details>
  <summary>Details</summary>
Motivation: 当前MCP数据库服务器为了灵活性通常暴露单一通用SQL执行工具，但这导致模型需要在查询时生成SQL，存在复杂度高、错误率大的问题。需要一种既能保持灵活性又能降低模型负担的架构方案。

Method: 提出领域导向工具模式，包含三个架构不变式：1)模型从预定义的小型领域工具集中选择而非实时生成SQL 2)参数化查询封装了schema导航、连接和业务规则 3)服务器端处理复杂逻辑。同时引入"模型降级"概念：用意图分类替代SQL合成可降低所需模型规格。实现采用MCP Blueprint开源框架，领域工具以YAML元数据+外部SQL文件声明式定义。

Result: 在Sakila数据库上对4个本地模型(3B-8B)进行17项客户任务测试(temperature=0，每项3次重复)：垂直化工具包 pooled mean=0.939，raw SQL=0.666，通用工具包=0.605。最小模型从0.583提升至0.929，超越所有更大配置，且每正确答案成本降低一个数量级。

Conclusion: 领域导向工具模式通过将SQL生成替换为意图分类/工具选择，有效降低了LLM处理数据库查询的难度和成本。小模型配合垂直化领域工具包可达到或超越大模型 raw SQL 的效果，为企业级LLM应用提供了更经济的架构选择。
...
### [251] [Decision-Support and Modeling with Large Language Models for Geothermal Well Arrays](https://arxiv.org/abs/2608.22068)
*Edwin Ouko,Emmanuel Lujan,Alan Edelman,Robert Metcalfe*

Main category: cs.AI

TL;DR: 本研究评估了ChatGPT、Gemini、Claude、Grok等大型语言模型在处理复杂地热数据中的潜力，并利用NotebookLM快速生成地热基准测试，分析地热井阵列和闭式同轴井两项技术，展示了LLM辅助地热数值模型自动并行化的可行性。


<details>
  <summary>Details</summary>
Motivation: 地热井阵列可提高产能和容错能力，但新兴地热技术发展需要LLM辅助；当前LLM应用面临主观偏见和幻觉问题，需要评估其在专业领域的可靠性；同时需要快速生成基准测试以评估快速演进的LLM能力。

Method: 使用NotebookLM生成定量地热基准测试，通过LLM访谈分析两种地热技术（地热井阵列、闭式同轴井），并展示LLM辅助地热数值模型自动并行化的案例研究。

Result: 评估了多种LLM作为地热领域专家助手的能力，验证了AI辅助生成基准测试的可行性，展示了自动并行化在地热建模中的应用，强调了高层高性能代码生成的重要性。

Conclusion: LLM有潜力变革地热领域，实现下一代决策支持应用，集成数据分析、智能建议和动态数值建模工作流程；但需注意LLM的局限性和输出可靠性问题。
...
### [252] [Dissecting Neuro-Symbolic Quality Assurance for Synthetic Oncology Data Generation](https://arxiv.org/abs/2608.22085)
*Laxmigayathri Challa,Yuhan Zhou,Ana Cleveland,Haihua Chen*

Main category: cs.AI

TL;DR: 该研究探讨了神经符号管道在生成癌症分期合成临床数据中的作用。研究发现：未加门控时，29.9%的记录存在模式失败，20.1%存在临床无效分期。模式验证是最关键的过滤器，拒绝了148/512条记录，本体论接地进一步过滤24条，而分期逻辑验证无额外过滤。检索增强效果高度依赖模型。符号门控提高了语料库临床有效性，但未改善真实肺癌病历的可用性。


<details>
  <summary>Details</summary>
Motivation: 癌症分期研究受限于临床数据稀缺，而使用大语言模型生成合成临床数据时，肿瘤学幻觉（临床不可能的分期分配）具有严重危害性——一个错误的分期数据会污染所有下游模型。尽管神经符号管道在生成过程中进行验证，但各质量保证组件的贡献尚不明确。

Method: 进行三个受控实验，隔离门控必要性、约束归因和检索条件性的影响，同时保持生成协议、多样性阈值和微调超参数不变。符号门控强制执行：模式完整性、SNOMED本体论覆盖和AJCC第八版分期逻辑一致性。

Result: 未门控时，29.9%记录存在模式失败，20.1%存在临床无效分期。模式验证是主要过滤器：在完全门控语料库中拒绝了148/512条记录，本体论接地进一步过滤24条，分期逻辑验证无额外拒绝（唯一产生逻辑违规的生成器已被模式过滤排除）。检索增强效果高度依赖模型：一个生成器提升12.5个百分点，另一个无影响，第三个输出崩溃。门控后本体论密度基本不变。

Conclusion: 符号门控提高了语料库有效性，但未带来真实肺癌病历的相应增益。检索增强应针对每个模型单独评估。本体论密度不应作为语料库质量的代理指标。临床逻辑验证是生成器条件性的防护措施而非主导过滤器。
...
### [253] [Hack-Verifiable Terminal Bench: Evaluating Reward Hacking in Terminal Tasks](https://arxiv.org/abs/2608.22103)
*Amit Roth,Ivan Bercovich,Yonathan Efroni*

Main category: cs.AI

TL;DR: 本文提出Hack-Verifiable Terminal Bench (HVTB)基准，用于自动可靠地检测AI代理的奖励黑客行为，并研究提示工程对减轻该行为的效果。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理能力增强和自主性提高，奖励黑客（满足任务检查但违背意图的行为）成为重要失败模式。传统检测方法依赖人工检查或LLM判断，存在可靠性不足的问题。

Method: 将HVE方法论适配到Terminal Bench基准，创建HVTB。通过在任务中嵌入可检测的黑客手段，实现奖励黑客的自动识别。研究测试了不同信息量的提示对减轻黑客行为的效果。

Result: 测量了前沿模型的奖励黑客率，验证了提示工程对防止已知和未知奖励黑客策略的有效性。

Conclusion: HVTB能够可靠自动地检测奖励黑客，为评估和减轻该行为提供了有效工具。
...
### [254] [Development and Feasibility Evaluation of an Edge AI as Medical Device System for Breast Cancer Multidisciplinary Team Meetings](https://arxiv.org/abs/2608.22108)
*Aarzoo Dhiman,Farzana Haque,Kartikae Grover,Lydia Brian Smith,William Stephen Jones*

Main category: cs.AI

TL;DR: 开发了一种完全在设备上运行的AI流程，使用开源自动语音识别(ASR)和大语言模型(LLM)来转录乳腺癌MDT讨论、构建临床信息并生成基于NICE指南的检索增强生成(RAG)治疗建议。该系统在NVIDIA Jetson AGX Orin上运行，确保患者音频、记录和输出保留在机构基础设施内。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌多学科团队(MDT)会议在相当大的时间压力下管理日益复杂的病例，文档需求可能降低临床效率和决策质量。现有基于AI的MDT工作流程依赖云处理，因患者讨论包含可识别信息而限制了其使用。

Method: 使用开源ASR(Whisper large-v3)和LLM构建完全在设备上的AI管道，通过RAG技术结合NICE指南生成治疗建议。系统在单个NVIDIA Jetson AGX Orin上运行，确保数据隐私。评估包括两个录制的模拟MDT讨论、十个临床验证的合成讨论以及1,270个声学增强录音。

Result: Whisper large-v3优化将使词错误率(WER)降低20.7%和24.4%，在增强音频上达到商用临床ASR基准0.58% WER和1.58%词信息丢失范围内。MedGemma-RAG识别的MDT一致干预措施是专有云比较器的2.3倍(p=0.020)，总体准确性无显著差异。利益相关者认为自动化文档、治疗建议支持和案例分诊是最可信的近期应用，同时强调工作流集成、治理和临床医生信任是关键实施挑战。

Conclusion: 这些发现证明了隐私保护的完全设备端AI用于MDT文档和指南知情的决策支持的可行性，为前瞻性临床评估提供了基础。
...
### [255] [Task-Driven 3D Printability Assistance via Geometry- and Knowledge-Grounded LLM Reasoning](https://arxiv.org/abs/2608.22128)
*Zhaoda Du,Qiaojie Zheng,Xiaoli Zhang*

Main category: cs.AI

TL;DR: 该论文提出了一种基于大型语言模型的增材制造预打印推荐框架，通过结合几何证据和结构化材料知识，为非专家用户提供可靠的任务驱动型打印辅助，解决传统方法在打印后才发现材料/工艺不合适的问题。\n\n


<details>
  <summary>Details</summary>
Motivation: 当前增材制造中的可打印性评估主要在几何层面进行（打印前），而任务适用性评估通常在打印后进行。这导致非专家用户在打印功能件时，可能在打印完成后才发现自己选择的材料或工艺不合适，造成重复打印、材料浪费和用户沮丧。\n\n

Method: 该方法利用大型语言模型(LLM)的推理和语言理解能力，同时结合几何证据和结构化材料/打印机知识来生成可靠的预打印推荐。具体流程为：给定STL模型和自然语言任务描述，框架生成涵盖可打印性、材料选择、工艺参数、设计指导、风险和解释的结构化推荐。通过物理验证和专家评估进行实验验证。\n\n

Result: 1. 在96次物理验证试验中，可打印性达到75.0%\n2. 在成功打印的样本中，任务适配性达88.9%\n3. 将Gemini 2.5 Flash-Lite的材料选择准确率从纯LLM的37.5%提升至90.0%\n4. 专家评估显示报告质量得到改善\n5. 打印后反馈可改善针对问题案例的推荐\n\n

Conclusion: 该框架通过结合LLM推理能力与几何证据和结构化材料知识，有效提升了增材制造中的预打印推荐可靠性。研究证明了用户任务意图、几何证据和结构化材料知识对可靠的任务驱动打印辅助都很重要，为非专家用户提供了实用的打印指导。\n\n
...
### [256] [MegaMem: A Retrieval Solution for Ultra-Large Context Windows](https://arxiv.org/abs/2608.22137)
*Xinyuan Song,Bowen Zhu,Hasibul Haque,Liang Zhao*

Main category: cs.AI

TL;DR: MegaMem是一个源代码解析的双视图检索系统，能够在数百亿token规模的持久记忆中进行准确检索，同时保持生成上下文的 bounded 约束。在EnterpriseRAG-Bench上实现总体性能从68.22提升至82.26，正确性达到86.50。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型和智能体需要持久记忆来处理完整的代码库、长交互历史和异构企业记录。核心挑战是如何在保持数百亿token可搜索的同时，仅向答案模型传递有界大小的源证据。

Method: MegaMem采用双视图架构：1）将语义访问与生成证据分离；2）使用原始查询和转换查询分别搜索蒸馏记录和详细证据；3）每个蒸馏命中解析为不可变的源ID；4）应用互惠排名融合、去重和交叉编码器重排序；5）在固定预算内仅保留最高排名的详细证据支持生成；6）答案后归属识别支持答案的源。

Result: 在EnterpriseRAG-Bench（约65亿token，50万+异构企业文档）上测试，MegaMem将总体性能从68.22提升至82.26，正确性达到86.50。

Conclusion: MegaMem成功支持超大规模持久记忆，同时在有界生成上下文中保持强大的答案准确性。通过将可搜索记忆规模与答案上下文大小分离，为数百亿到十亿token规模的准确检索提供了实用路径。
...
### [257] [Measuring Stability and Failure Behavior in Language Models Under Structured Perturbations](https://arxiv.org/abs/2608.22138)
*Samira Golsefid*

Main category: cs.AI

TL;DR: 本文提出了一个多层次、多家族的压力测试框架，用于评估推理模型在输入扰动下的性能退化情况。该框架将测试问题沿七个家族进行多级严重性扰动，并引入崩溃点指标来揭示单一准确率得分隐藏的模型弱点。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型仅通过单一准确率分数进行评估，无法揭示模型在输入受到扰动时性能如何退化。需要一个更全面的压力测试框架来识别模型的具体失败模式和脆弱点。

Method: 构建了一个分级、多家族、故障感知的压力测试框架，包含七个测试家族：释义、输入噪声、格式无关上下文、上下文负载、冲突指令和知识边界（移除可回答性）。每个测试经过有效性门控，按严重程度标记。模型通过每级准确率、加权稳定性以及相对于自身基线的崩溃点来评估。

Result: 在4,473个门控测试上的实验表明：失败级别是家族特定的而非全局的；两个压力源（冲突指令和不可能前提的问题）在所有模型中暴露出一致的弱点；对不可回答性的识别不一致——在缺失信息和伪造证据上可靠，但在不可能前提上薄弱。

Conclusion: 标准准确率报告无法揭示这些失败点。模型的实际能力需要通过多级、多家族的压力测试来全面评估，而不仅仅是单一的准确率分数。
...
### [258] [MEMONDEMAND: A Memory Management System for Large-Scale Enterprise Data](https://arxiv.org/abs/2608.22141)
*Xinyuan Song,Bowen Zhu,Hasibul Haque,Liang Zhao*

Main category: cs.AI

TL;DR: MEMONDEMAND是一个企业级检索增强系统，针对大规模异构且持续更新的企业知识库，提供动态多级层次结构、双层内存和按需内存提升机制，在618M token规模下相比LB#1提升4.66%。


<details>
  <summary>Details</summary>
Motivation: 企业知识库规模大、异构性强、持续更新，现有系统难以同时支持高效访问、源忠实证据和跨查询适配，企业记忆需要超越模型上下文进行检索。

Method: 提出三个协同机制：1)动态多级层次结构确定每个集合的抽象结构和深度；2)双层内存分离提炼路由与详细证据；3)按需内存提升在有限活跃状态预算下更新节点优先级。

Result: 在EnterpriseRAG-Bench上，从10M到618M token规模均超越LB#1，10M token提升12.23%，618M token提升4.66%。FinanceBench、HotpotQA和FRAMES上亦表现优异。

Conclusion: MEMONDEMAND是准确、高效、可扩展的大规模企业知识库内存解决方案，适用于不同数据规模、领域和证据需求。
...
### [259] [Evaluation of Small Vision-Language Models on Qualitative Mechanical Problems](https://arxiv.org/abs/2608.22143)
*Henry Fordjour Ansah,Shreya Banerjee,Pranish Ghimire*

Main category: cs.AI

TL;DR: 本文研究评估两个多模态模型（Gemma-3和Qwen-VL）在定性机械问题解决（QMPS）任务上的表现。研究使用Bennett机械理解测试（BMCT）作为基准，通过引导模型生成思维链（CoT）来评估其空间推理和常识推理能力，并从连贯性、完整性和逻辑推进三个维度评估思维过程，最终将答案与验证解进行比较以衡量准确率。


<details>
  <summary>Details</summary>
Motivation: QMPS是人类智能的关键方面，涵盖从日常生活到专业领域的各种任务。雇主常用BMCT评估候选人的机械问题解决能力，因此需要评估当前多模态AI模型在这一领域的能力。

Method: 研究评估Gemma-3和Qwen-VL两个多模态模型，通过引导模型生成逐步思维链（CoT）和最终答案。图像本身编码了真实定性事实（如齿轮接触点、支撑关系、相对重量），用于评估模型的空间和常识推理能力。评估维度包括思维的连贯性、完整性和逻辑推进，最终答案与验证解比较以测量准确率。

Result: （摘要中未提供具体结果，仅描述了研究方法和评估框架）
...
### [260] [AUDITA: certified auditing and causal attribution of adverse outcomes in autonomous multi-agent systems](https://arxiv.org/abs/2608.22160)
*Zhixu Du,Yiran Chen*

Main category: cs.AI

TL;DR: 本文提出AuditLayer审计层，解决AI控制的机器人集群造成伤害时的责任归属问题，通过防篡改记录和因果归因引擎，将责任归属从争论转化为基于证据的计算。


<details>
  <summary>Details</summary>
Motivation: 物理自动化集群造成伤害时，各方（机器供应商、算法提供商、工厂运营商、保险商、监管机构）互相推卸责任，现有方法无法正确划分责任，现有日志无法验证来源且只能指定单一责任人。

Method: 提出AuditLayer审计层，包含：1)防篡改的跨智能体命令记录；2)经过认证的分级因果归因引擎。建立了一套证明机制确保无法操纵审计结果。

Result: 在真实语言模型管道上减少约三倍的责任错误；在事故基准上恢复责任归因，且在篡改下保持不变。

Conclusion: AuditLayer将责任归属问题从关于日志的争论转化为基于证据的计算，为物理自动化时代的责任划分提供了可靠的技术基础。
...
### [261] [Aggregation-Aware Synthetic Text Generation Against Authorship Re-Identification](https://arxiv.org/abs/2608.22161)
*Qian Ma,Anna Squicciarini,Sarah Rajtmajer*

Main category: cs.AI

TL;DR: 本文提出AAST框架，解决多文本场景下的作者身份混淆问题。现有方法独立优化每个文档，忽视跨文档关联性。AAST在束级别联合选择合成文本，针对归因和验证攻击（包括跨体裁设置），在束大小增加时降低账户级可链接性，同时保持语义质量、语言可接受性和情感一致性。


<details>
  <summary>Details</summary>
Motivation: 在线用户在同一身份下发布多个文本，攻击者可构建比任何单一文本更具揭示性的作者画像。现有 authorship obfuscation 方法独立优化每个文档，忽视了使聚合危险的跨文档相关性。

Method: 提出Aggregation-Aware Synthetic Text Generation (AAST)框架，在束级别而非孤立优化每个文本，联合选择合成文本。目标攻击包括归因和验证攻击，涵盖跨体裁设置（攻击者参考文本来自生成或选择阶段未观察到的体裁）。

Result: 实验涵盖同体裁、跨体裁、神经和独立非神经风格计量攻击，结果表明AAST在束大小增加时降低账户级可链接性，同时保持语义质量、语言可接受性和情感一致性。

Conclusion: AAST通过在束级别联合优化，有效应对跨文档聚合攻击，为多文本场景下的作者身份保护提供了新思路，实验验证其在多种攻击场景下均能有效降低链接性且保持文本质量。
...
### [262] [MCP-Universe RL: A Framework for Training MCP Tool-Use Agents via Reinforcement Learning](https://arxiv.org/abs/2608.22167)
*Ziyang Luo,Yan Yang,Xiangru Jian,Ziji Shi,Xiaoqiang Lin,Jun Hao Liew,Silvio Savarese,Junnan Li*

Main category: cs.AI

TL;DR: MCP-Universe RL (MCP-U RL)是一个开源框架,解决强化学习在工具使用中的两个核心系统问题:环境编排(为数百条并发轨迹提供隔离环境)和 rollout 编排(通过分阶段流水线重叠轨迹使GPU保持忙碌)。该框架使用MCP作为环境接口,支持即插即用,并提供与veRL和slime的后端无关的训练层。


<details>
  <summary>Details</summary>
Motivation: 现有RL框架仅停留在策略更新层面,存在两个硬性系统问题:1)需要为每个新领域建立隔离环境并连接到训练;2)调度rollout使GPU在长时间多轮对话中保持忙碌,而这些对话大多时间花在缓慢的工具调用上。

Method: 1)使用MCP作为环境接口,任何已暴露为MCP服务器的工人都可直接接入训练,无需RL特定集成代码;2)构建环境编排层,提供、隔离和回收基于可插拔容器后端的MCP环境;3)构建rollout编排层,使用分阶段流水线重叠轨迹,使GPU在等待工具时保持忙碌;4)后端无关的训练层,通过veRL和slime应用更新。

Result: 使用单一配置(仅更改任务规范),在gpt-oss-20b上训练软件工程、深度研究和通用工具使用代理,三个领域的任务奖励均有提升。

Conclusion: MCP-U RL成功解决了RL工具使用中的系统级挑战,实现了环境复用和GPU高效利用,为不同领域的工具使用代理训练提供了通用解决方案。
...
### [263] [Role-Specialized Mixture-of-Agents with Open-Weight LLMs for Clinical Prediction](https://arxiv.org/abs/2608.22176)
*Jun Hou,Yi Fang,Xuan Wang*

Main category: cs.AI

TL;DR: 本文研究了在隐私约束下，使用开放权重多智能体系统进行临床预测任务（死亡率预测和再入院预测）。通过设计角色专业化的智能体混合系统，结合医学知识检索和相似患者推理，发现角色设计是关键因素，可匹配封闭模型性能并提高高风险患者识别能力。


<details>
  <summary>Details</summary>
Motivation: 隐私和合规约束推动了对本地部署系统的需求，但现有医疗多智能体系统的评估不明确，不清楚各智能体角色的具体贡献以及检索是否驱动了性能提升。

Method: 研究了一种角色专业化的Mixture-of-Agents（MoA）系统，结合医学知识检索与对比相似患者推理。通过改变角色设计同时保持检索设置固定来隔离主要效应。 pairing大型开放权重分析器与小型开放权重集成器。

Result: 在死亡率预测的F1分数上与封闭模型提示相当，同时标记了更多真正的高风险患者。机制分析显示角色分配直接产生高召回操作点。对于再入院预测收益较小，因为可用记录与这个更长周期结果的相关性较弱。

Conclusion: 角色设计是隐私约束、无训练临床LLM预测的关键因素。
...
### [264] [Disagree to Explore, Agree to Commit: Routing-Guided Test-Time Scaling for Software Agents](https://arxiv.org/abs/2608.22191)
*Kang Chen,Junjie Nian,Yixin Cao,Yugang Jiang*

Main category: cs.AI

TL;DR: Risa利用原生MoE路由轨迹实现软件工程智能体的引导和选择，无需外部评判器或测试时执行。通过路由信号鼓励轨迹内多样探索和受控收敛，在跨轨迹采样中通过patch位置的共识选择最终候选。在SWE-bench Verified上，Risa将解析率从44.9%提升至48.2%。


<details>
  <summary>Details</summary>
Motivation: 软件工程智能体通过长随机工具使用轨迹解决仓库级任务，重复尝试常能发现单次运行遗漏的修复方案。测试时扩展困难，因为补丁缺乏规范答案形式，且来自共享前缀的兄弟动作相互关联。现有方法需要外部评判器或答案字符串匹配来选择补丁。

Method: Risa包含两部分：1) 轨迹内路由引导 - 在轨迹中利用路由信号鼓励多样探索并在patch提交时控制收敛；2) 跨轨迹路由仲裁 - 在分别采样的轨迹间，通过信息patch位置的共识来选择最终候选。使用token级粒度读取和决策匹配的对比集将路由转化为有效控制。

Result: 在SWE-bench Verified上，Risa的路由仲裁将宏平均解析率从均匀采样的44.9%提升至gpt-oss系列的48.2%，匹配文本共识且无需答案字符串匹配。该方法可迁移到Qwen3.6，在完整500任务基准上优于均匀选择并匹配文本共识。

Conclusion: MoE路由提供了稳健的行为角色信号，token级粒度读取和决策匹配比较集可将其转化为有效控制。Risa展示了路由引导和仲裁在无需外部评判器或测试执行的情况下提升智能体性能的潜力。
...
### [265] [Query-Driven Multimodal Information Extraction from Long Documents](https://arxiv.org/abs/2608.22214)
*Yikai Gao,Ding Xia,Xi Yang*

Main category: cs.AI

TL;DR: 本文提出任务：查询驱动的图像-文本联合提取，需从长文档中输出查询请求的文本属性值及对应图像边界框。构建了ITJoint基准，包含2455页领域文档、316个查询和910个答案实例。设计了Q2IT多智能体协作框架。实验表明standalone VLMs在此任务上表现不佳，Q2IT有显著提升但仍与完美结果有差距。


<details>
  <summary>Details</summary>
Motivation: 现有DocVQA等范式主要关注生成文本答案或定位证据区域，无法满足从领域特定多模态长文档中输出查询特定的文本属性值及对应图像的需求。现有方法未能充分考虑用户意图和文档内容两个层面的挑战。

Method: 1.设计两级分类法（查询级和实例级）处理用户意图和文档内容挑战；2.构建ITJoint基准：2455页领域文档、316个查询、910个答案实例；3.设计Q2IT多智能体协作框架，包含证据收集、页面选择和目标图像定位三个递进协作的智能体；4.采用文本提取和图像定位的联合评估方法。

Result: standalone VLMs在此任务上表现困难；Q2IT框架显著提升性能，但与完美结果仍有较大差距。

Conclusion: 查询驱动的图像-文本联合提取是一个重要且具有挑战性的新任务；ITJoint是首个高质量人工标注的基准；Q2IT框架通过多智能体协作有效提升了性能；未来仍有很大改进空间。
...
### [266] [Beyond What Meets the Eye: Unveiling Situational Illusions for Multimodal Large Language Models](https://arxiv.org/abs/2608.22232)
*Zhiming Yang,Zhuoxi Xiong,Donglin Zhou,Wenjun Wei,Shiyao Cui,Jinqiao Shi*

Main category: cs.AI

TL;DR: 本文提出"情境幻觉"(situational illusions)概念，指现实情境表现与底层物理状态之间的偏差。研究者构建了where-what-how分类法，并推出MSIBench基准测试来评估MLLMs在情境幻觉下的辨别、理解与推理能力。实验涵盖27个模型配置，发现当前MLLMs极易受到情境幻觉影响，表现出6种典型失败模式。通过提示工程和监督微调两种方法，模型性能最高提升20%。


<details>
  <summary>Details</summary>
Motivation: 现实世界中，情境的外观可能与其底层物理状态不符，这导致多模态大语言模型(MLLMs)在实际应用中可靠性受限。研究者将此现象定义为"情境幻觉"，旨在探究MLLMs在此类幻觉下的表现及改进方法。

Method: 1.构建where-what-how分类法描述情境幻觉的发生位置、目标和成因；2.开发MSIBench基准测试评估MLLMs的辨别、理解与推理能力；3.对27个模型配置进行评测；4.提出两种缓解方法：针对闭源模型的提示工程和针对开源模型的监督微调。

Result: 当前MLLMs极易受到情境幻觉影响，表现出6种典型失败模式(与视觉观察、接地和推理相关)。两种简单方法可提升模型性能最高20%。

Conclusion: 通过系统检查和推理视觉证据进行上下文理解的方法(提示工程和监督微调)能有效提升MLLMs在复杂现实环境中的感知和推理可靠性，为构建更可靠的多模态系统提供了实用路径。
...
### [267] [Read Less, Solve More: Token-Efficient Sparse Reading for AI Agents](https://arxiv.org/abs/2608.22237)
*Zedong Liu,Jiaan Wu,Xinyang Ma,Le Xu,Kai Wang,Yuanchao Hu,Dingwen Tao,Guangming Tan*

Main category: cs.AI

TL;DR: SparseRead是一种无需训练、模型透明的阅读层，通过在证据进入模型上下文前控制内容 admission，解决长程智能体过度阅读的问题，可在6个前沿模型上减少最多92.9%的token和89.0%的时间成本，同时保持或提升任务质量。


<details>
  <summary>Details</summary>
Motivation: 当前阅读接口常常暴露整个对象，即使只需要稀疏证据，这导致token和延迟成本增加，且任务相关证据被稀释。现有上下文缩减方法主要在广泛内容已进入轨迹后才干预。

Method: SparseRead结合了感知制度的Read Gate、可扩展的Reader Backends，以及有状态的协议，用于 bounded、source-anchored的证据获取，包含显式的 refinement、verification、stopping和fallback机制。

Result: 在6个前沿模型（包括Claude Opus 5）和5个工作负载场景中，SparseRead最多减少92.9%的token量和89.0%的wall时间，同时保持或提升任务质量。在三个智能体框架中的一致收益证明了其广泛的便携性。

Conclusion: SparseRead作为一种训练-free、模型透明的阅读层，能够有效控制内容 admission，在大幅降低token和时间成本的同时保持任务质量，具有良好的通用性和实用价值。
...
### [268] [Clarify User Expertise: Towards Proactive Conversational Agents Tailoring Responses to User Proficiency](https://arxiv.org/abs/2608.22266)
...
### [269] [HERO: Human-profile Enhanced Retrieval Optimization Framework for Long-term Agent Memory](https://arxiv.org/abs/2608.22310)
*Yuanhua Lin,Yile Li,Zhiyuan Zhao,Jing Shang,Jian Sun*

Main category: cs.AI

TL;DR: HERO是一个用于长期智能体记忆的人类画像增强检索优化框架，将对话历史转换为可追溯的异构图以保留原始对话文本作为推理证据，解决现有方法中信息丢失和语义漂移的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖LLM压缩或重写对话历史作为检索证据，但存在两个主要缺陷：(1)压缩导致信息丢失，丢弃了后期有用的细粒度细节；(2)重写导致语义漂移，侵蚀了原始语气和情境上下文。

Method: HERO将对话历史转换为可追溯的异构图，保留原始对话文本作为推理证据。对于检索，HERO从当前查询中提取初始锚点，并通过迭代图遍历整合人类画像，这些锚点和画像提供指导信号，自适应地激活图中信息最丰富的区域。

Result: 在两个基准数据集上的实验表明，HERO在事实推理和个性化推理方面均优于强基线方法，同时提供对原始对话证据更忠实的访问。

Conclusion: HERO通过异构图结构有效解决了长期记忆中的信息丢失和语义漂移问题，在保持原始证据的同时提升了推理准确性。
...
### [270] [Where Cognition Lives: Dissecting Emergent from Computed Function in a Minimal Complete Cognitive Architecture](https://arxiv.org/abs/2608.22347)
*Francisco M. Arrabal-Campos,Francisco G. Montoya,Alfredo Alcayde,Ignacio Fernández*

Main category: cs.AI

TL;DR: 本文构建了一个包含自适应停止机制、稳态控制场和价值模块的最小完整认知架构系统，研究各功能是从梯度下降中自然涌现还是必须显式计算。研究发现：推理能力可以涌现，停止机制看似涌现但实际是工具效应（需通过价值模块计算），价值模块无法涌现必须显式计算，在冻结LLM上自一致性投票有微弱提升。


<details>
  <summary>Details</summary>
Motivation: 认知架构不仅需要推理模块，还需决定思考时长和努力分配。作者想知道这些功能是从训练中自然涌现还是需要显式计算。

Method: 构建最小完整系统（含循环推理器、自适应停止、稳态控制场、价值模块），使用PonderNet风格停止机制，通过消融实验对比强制深度基线，测试不同停止策略和价值分配方法的效果。

Result: 停止机制看似涌现（收益从uniform的0.467提升到posterior self-observation的0.921），但审计后消失；价值模块无法涌现，训练耦合无法捕获显式分配器能捕获的 payoff（+0.151）；自一致性投票有微弱提升（+0.0236）；承诺下的价值在悬崖成本族中获+0.1312。

Conclusion: 推理能力可从训练涌现，停止机制需通过价值模块计算（至少在价值与内容正交的情况下），自一致性投票作为停止信号有有限价值，承诺下的价值有显著收益。系统各组件需显式计算而非仅靠涌现。
...
### [271] [Addressing the Selection Problem in Explainable AI](https://arxiv.org/abs/2608.22356)
*Claire Vlases,Katelyn Morrison*

Main category: cs.AI

TL;DR: XAI研究产生了大量解释技术，但实际应用中这些解释效果不佳。文章提出"选择问题"概念：用户难以从自然语言不确定状态选择合适的XAI技术，并提出多Agent LLM编排工具作为结构化解决方案。


<details>
  <summary>Details</summary>
Motivation: XAI研究虽产生众多解释技术，但用户研究表明现有解释在实践中效果不佳，用户难以选择合适的XAI技术。

Method: 采用哲学视角，使用逻辑前提-结论格式对选择问题进行形式化分析，并提出多Agent LLM编排工具方案。

Result: 揭示了传统XAI界面的根本问题：要求用户将不确定性转化为技术选择，这是一个难以满足的前提条件。

Conclusion: 提出结构化解决方案：多Agent LLM编排工具可将用户查询翻译为适当的XAI解释技术，并给出具体实例说明如何实现。
...
### [272] [Analyzing and Mitigating Cross-Lingual Degradation in Multilingual Medical VQA](https://arxiv.org/abs/2608.22363)
*Jingbo Wang,Sendong Zhao,Haochun Wang,Bing Qin,Ting Liu*

Main category: cs.AI

TL;DR: 本文构建了一个多语言医学VQA基准测试，评估了跨语言对医学VQA不同能力的影响，并提出MedVL-XLRepE方法在推理时将非英语表示对齐到英语表示，从而缓解跨语言性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 医学VQA评估长期以来只关注英语，忽略了全球多语言患者和临床医生的需求。虽然已有研究表明大型视觉语言模型在非英语语言中性能下降，但缺乏对跨语言变化如何影响医学VQA所需不同能力的细粒度分析。

Method: 1) 构建包含八种语言的多语言医学VQA基准测试，组织为四个代表性场景以隔离核心能力；2) 评估五个开源和闭源LVLMs；3) 提出MedVL-XLRepE训练-free方法，利用英语医学VQA能力在推理时将非英语表示引导向英语表示。

Result: 跨语言退化不是均匀的，而是高度依赖于具体场景的。MedVL-XLRepE在三个LVLMs和八种语言中一致缓解跨语言退化，性能提升最高达6.33%。

Conclusion: 跨语言医学VQA的性能退化具有场景依赖性，通过场景感知的表示工程方法可以有效提升非英语语言的性能，为构建更公平的全球医学AI系统提供了新思路。
...
### [273] [WAM-OPD: On-Policy Distillation for World Action Models](https://arxiv.org/abs/2608.22364)
*Liuhaichen Yang,Zhuang Jiang,Chenchao Sheng,Zezhi Tang*

Main category: cs.AI

TL;DR: 本文提出WAM-OPD，一种用于视频优先世界动作模型(WAM)的部署一致性后训练方法，旨在修复蒸馏过程中失去任务能力的学生模型，无需稀疏奖励强化学习。学生与环境交互产生历史分布，冻结的教师模型为学生历史标注连贯的视频和动作目标，学生动作分支在自身生成的视频计划下训练，通过联合视频和动作损失更新轻量适配器，并配合动作流匹配正则化器。在RoboTwin 2.0的两个任务中，Flash-WAM在HANDOVER MIC任务上从0%提升至58.3%，在PUT OBJECT CABINET任务上从16.7%提升至33.3%。


<details>
  <summary>Details</summary>
Motivation: 加速学生模型在蒸馏过程中会丧失任务能力，且后续可能遇到离线数据代表性不足的状态。现有方法通常依赖稀疏奖励的强化学习来修复，但这种方式样本效率低且训练困难。因此需要一种无需稀疏奖励RL的后训练方法来修复学生模型。

Method: WAM-OPD采用部署一致的后训练方案：学生在环境中自主行动并确定历史分布；冻结的教师模型为这些学生历史标注连贯的视频和动作目标；学生动作分支在其自身生成的视频计划下进行训练（与部署时一致）；使用联合视频和动作损失更新共享骨干网络中的轻量适配器，并引入动作流匹配正则化器进行约束。

Result: 在RoboTwin 2.0平台的初步研究中，Flash-WAM（一视频/一步骤版本）在两项任务上取得显著提升：HANDOVER MIC任务成功率从0%提升至58.3%，PUT OBJECT CABINET任务成功率从16.7%提升至33.3%。

Conclusion: 在学生自主产生的历史分布上进行密集教师监督是视频优先WAMs有前景的后训练接口。这些任务是特定能力验证，而非广泛或一致泛化的证据。该方法表明蒸馏后修复是可行的，但需更多研究验证其普适性。
...
### [274] [LLMs for Survey Text Analysis - A Performance Comparison Between Humans and GPT-5 on Inductive Content Analysis](https://arxiv.org/abs/2608.22417)
*Leonardo Bergmann,Renata Gheorghiu,Ana Gvritishvili,Alex Mican,Chris Stewart,Topias Tolonen-Weckström*

Main category: cs.AI

TL;DR: 研究比较了人类编码员与GPT-5.4在归纳内容分析中对903个开放式调查回复的编码表现，使用调整兰德指数(ARI)评估一致性。发现人机编码一致性为0.54-0.76，接近人类内部一致性(0.68)，表明LLM可作为定性分析的可扩展支持工具。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLM)在定性研究中的应用日益增多，但其在归纳内容分析中的表现证据仍然有限，需要实证研究验证其可靠性

Method: 5名人类编码员使用标准化编码方案对6个变量的903个欧洲博士生调查回复进行归纳内容分析，同时使用GPT-5.4通过既定提示程序进行相同任务，采用调整兰德指数(ARI)评估一致性

Result: 人机编码一致性ARI=0.61，主题生成一致性ARI=0.54；人类内部一致性ARI=0.68，LLM内部一致性ARI=0.76；一致性因变量而异，低实体内部一致性始终与低实体间一致性相关

Conclusion: LLM在此特定案例设置中可近似人类编码表现，特别是在编码层面，可作为归纳定性分析的可扩展支持工具
...
### [275] [Where World Models Break: Natural-Input Failure Discovery](https://arxiv.org/abs/2608.22421)
*Zhanpeng Shi,Zi Liang,Rong Feng,Shiqin Tang,Xuyang Chen,Hongzong Li*

Main category: cs.AI

TL;DR: 本文提出BasinLens方法，用于发现世界模型在自然输入下的灾难性预测失败。现有评估方法通过聚合平均误差来评估模型，无法压力测试模型在罕见或未观察到的条件-动作组合下的灾难性崩溃风险。


<details>
  <summary>Details</summary>
Motivation: 世界模型作为下游规划和控制的关键内部模拟器，其灾难性预测失败可能通过控制管道危险传播。然而现有评估忽略这种系统性风险：通过对一般查询的良性生成聚合平均误差，无法压力测试模型在罕见条件-动作组合下的灾难性崩溃。

Method: 提出BasinLens方法，利用有效输入的底层结构（每个坐标具有环境定义的语义类型和允许域），结合不确定性引导的全局搜索与类型化局部替换，在有限查询预算下找到诱导严重预测风险的环境有效条件和动作前缀，并验证这些失败在新种子上的可重现性和附近有效编辑下的持久性。

Result: 在多样化的基准测试和世界模型家族中，BasinLens揭示了常规评估无法揭示的可重现且局部持久的失败模式，表明平均情况基准测试可能掩盖世界模型驱动控制中的重要漏洞。

Conclusion: 平均情况基准测试可能掩盖世界模型驱动控制中的重要漏洞。需要新的评估范式来发现和验证模型在极端条件下的灾难性失败，以提高系统的安全性和可靠性。
...
### [276] [Think with Structured Grounding: Perceptual Reinforcement Learning for Chart and Visual-Tabular Understanding](https://arxiv.org/abs/2608.22429)
*Changjiang Jiang,Qiannian Zhao,Lei Xin,Jinxiang Xie,Preslav Nakov,Zhuohan Xie*

Main category: cs.AI

TL;DR: TwSG是一个将外部工具的细粒度感知能力内部化的新型框架，通过单次前向传递实现多步推理和微裁剪功能，解决MLLMs在处理文本密集型结构视觉（如图表、视觉表格）时的空间-结构差距问题。


<details>
  <summary>Details</summary>
Motivation: 外部工具依赖导致推理延迟高，且无法有效解决空间-结构差距问题——这是文本密集型和结构关系视觉（如图表、视觉表格）中严格相对空间排列绑定文本元素所带来的基础挑战。标准MLLMs在缺乏外部工具时难以处理此类细粒度视觉推理任务。

Method: 使用MLLM识别由真实答案引导的关键区域，提示教师模型生成高质量VQA数据，将细粒度、基于区域的监督信号提炼回全图像表示。训练管道包含两阶段：(1)冷启动监督微调(SFT)阶段，使用多轮数据和聚焦区域描述促进复杂推理和错误恢复；(2)基于新颖过程奖励机制TL-GRPO的强化微调(RFT)阶段，鼓励策略性推理。

Result: 在各种MLLM架构上的广泛实验表明，TwSG在减少推理延迟的同时显著提高准确性和鲁棒性，赋予模型原生细粒度区域描述和灵活推理能力。

Conclusion: TwSG成功将外部工具能力内部化，在单次前向传递中实现多步推理效果，无需额外工具即可处理复杂结构视觉任务，为提升MLLMs细粒度视觉推理能力提供了有效方案。
...
### [277] [When Persona Simulations Are Informative: Graph-Structured Signals for Pluralistic Opinion Sensing](https://arxiv.org/abs/2608.22438)
*Taehyeon An,Jaehyeong Park,Donghyuk Shin*

Main category: cs.AI

TL;DR: 论文提出PCI（人格条件信息量）指标，用于诊断人格条件化大语言模型在模拟调查回复中是否真正学习了人格条件，而不仅仅是噪声或模型先验。通过将人格建模为相似性图并使用局部莫兰指数，PCI可以无监督地提取一致偏移的人格子集。在PVQ-RR问卷上的验证表明，PCI选择的10%子集显著改善了潜在价值结构的构建。


<details>
  <summary>Details</summary>
Motivation: 当前人格条件化LLM用于模拟调查回复时，响应变化可能源于未条件化的模型先验或采样噪声，而非真正的人格条件影响。需要一种原则性的诊断方法来区分真正的人格条件化效应与虚假变异。

Method: 提出PCI指标，将语义相似的人格建模为相似性图，使用局部莫兰指数(Local Moran's I)量化局部空间一致性，无需构造标签即可提取紧凑的人格子集。在PVQ-RR问卷上通过验证性因子分析评估PCI的有效性。

Result: PCI筛选的10%子集相比响应稳定性和随机选择方法，显著提升了整体构念恢复效果。

Conclusion: PCI可作为筛选调查流程中合成回复的原则性内部诊断工具。
...
### [278] [Small Reasoning Models are Instruction Followers in Function Calling](https://arxiv.org/abs/2608.22472)
*Yalda Taheri,Mohammad Hassan Heydari,Erfan Naaman,Afsaneh Fatemi*

Main category: cs.AI

TL;DR: 本文提出指令跟随函数调用(IFFC)框架，将函数调用逻辑从主LLM分离，委托给一个较小的专用模型在指令跟随范式下运作。实验表明该方法在推理导向LLM上表现优异，且在量化条件下仍能保持良好性能，适合边缘计算场景。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注通过微调、强化学习和多智能体框架提升原生函数调用准确率，但本文发现LLM在指令跟随上下文中的函数调用准确率优于工具调用上下文，因此提出新的解决思路。

Method: 提出IFFC框架，将函数调用逻辑与主LLM解耦，使用较小的专用模型在指令跟随范式下处理函数调用任务。该方法不依赖复杂的函数调用训练，而是利用指令跟随能力实现函数调用。

Result: IFFC方法在多个基准测试中一致超越原生函数调用(NFC)和基于提示的函数调用(PFC)基线，在推理导向LLM上提升尤为显著。同时在激进量化条件下仍保持稳健性能。

Conclusion: IFFC为边缘计算场景提供了一种可靠、资源高效的函数调用新范式，使得在设备端部署高精度函数调用能力成为可能，无需依赖云端大型模型。
...
### [279] [When Does AI for PDEs Yield Scientific Evidence?](https://arxiv.org/abs/2608.22504)
*Wenshuo Wang*

Main category: cs.AI

TL;DR: 本文指出现有AI偏微分方程基准测试仅评估预测准确性，但物理学研究中AI输出常用于支撑科学论点，这两个目标并不等价。作者扩展了PDE模拟和逆问题的基准测试，首次实现了评估模型输出是否以及在多大程度上支持特定科学 claims。实验表明数值准确性与证据支持度可能对模型排名不同，揭示了现有基准可能偏好输出证据支持较弱的模型。


<details>
  <summary>Details</summary>
Motivation: 现有AI-for-PDE基准测试主要评估预测或近似准确性，但在物理研究中AI输出常作为科学论点的证据。这两个目标不等价：前者衡量输出与参考目标的一致性或满足约束条件；后者则评估给定研究对象、科学论点、假设和证据标准时，输出是否为该论点提供充分证据。存在评估与使用目的之间的错配。

Method: 扩展了广泛使用的PDE模拟基准和PDE逆问题的综合基准，首次在AI for PDEs中实现评估模型输出是否支持特定科学 claims。设计了新的评估维度来衡量证据支持程度。

Result: 实验结果表明：(1)数值准确性与证据支持度可能对模型给出不同排名；(2)明确了这种现象发生的时间点和原因；(3)揭示现有基准可能偏好那些输出对目标科学论点支持较弱的模型。

Conclusion: 本文正式提出、经验验证并解释了AI for PDEs中评估-使用错配的问题，强调在评估AI模型时需要考虑模型输出是否能提供足够的证据支持科学论点，而不仅仅是数值准确性。
...
### [280] [ClawProBench: Trace-Aware Evaluation of AI Agents with Runtime Coverage and Frozen Workplace-Style Holdouts](https://arxiv.org/abs/2608.22510)
*YuanHang Xiao*

Main category: cs.AI

TL;DR: Agent benchmarks通常只评估最终答案,但这无法充分评估有状态运行时上的agent表现。本文提出ClawProBench,一个基于执行轨迹的benchmark,用于评估原生运行时agent,并在OpenClaw平台上实现。该benchmark包含两个赛道:102场景的完整profile和68场景的冻结holdout。通过安全门控公式综合评分,评估了68个配置,最高分0.7671。发现纯正确率排名与过程感知、安全门控视角下的排名差异显著。


<details>
  <summary>Details</summary>
Motivation: 当前agent benchmark只评估最终答案,无法充分评估有状态运行时上的agent表现。评估单元应该是声明的模型加运行时配置,失败可能发生在证据获取、运行时路由、安全边界或重复执行等环节。现有final-answer leaderboard可能隐藏原生表面弱点、一次性成功和轨迹局部agent失败模式。

Method: 1)提出ClawProBench trace-aware benchmark,基于OpenClaw live agent runtime实现 2)定义两个赛道:102场景完整profile(包含live workspace和native-runtime路由任务)和68场景冻结holdout(闭世界JSON输出契约) 3)通过安全门控公式从执行轨迹计分,结合正确性、过程质量和效率 4)评估68种配置(完整profile)和37种配置(holdout)

Result: 1)Top安全门控平均trace score为0.7671 2)Native-runtime任务表现低于workspace-live任务(0.5238 vs 0.6415) 3)Holdout上pass@k-any优于strict three-trial pass(0.6638 vs 0.2890) 4)Full-profile和holdout排名弱对齐(Spearman 0.1300) 5)纯正确率排名与过程感知、安全门控和strict-pass视角下的排名差异显著

Conclusion: Final-answer leaderboard可能隐藏原生表面弱点、一次性成功和轨迹局部agent失败模式。正确的评估单元应是声明的模型加运行时配置,需要结合执行轨迹进行过程感知和安全门控的评估。
...
### [281] [HANSARD: A Reference Architecture for Forensic Readiness, Runtime Witnessing, and Graded Attribution in Autonomous Multi-Agent AI Systems](https://arxiv.org/abs/2608.22512)
...
TL;DR: 本文提出HANSARD参考架构,将问责制视为生命周期属性,以解决自主多智能体系统造成危害时的归因困难问题。该架构通过准备阶段密封配置文件、在智能体触及范围之外的五个阻塞点进行捕获、构建类型化的PROV-DM对齐因果图、以及事后重放计算修正的Halpern-Pearl定义下的 contingent effects等机制,实现对责任 Attribution laundering(归因洗白)的检测和问责。


<details>
  <summary>Details</summary>
Motivation: 当前自主多智能体系统已应用于金融、软件供应链和安全运营领域,甚至出现了首次大规模AI编排的入侵活动。然而当此类系统造成危害时,无法稳健地确定发生了什么、原因是什么以及谁应负责。原因在于:溯源取证工作于错误的抽象层次、形式因果关系假设了因果模型、智能体审计依赖自我记录。目标失败模式是"归因 laundering",即跨冗余智能体传播行为直至没有but-for原因,且记录由嫌疑人自己生成,智能体可能 anticipate 调查,部分日志基础设施可能串通。

Method: 提出HANSARD参考架构:1)运行前密封准备配置文件以界定后续发现的可能声明;2)在智能体无法触及的五个阻塞点进行捕获,使遗漏可被检测而不仅仅是篡改;3)构建类型化PROV-DM对齐的因果图随系统运行累积,三个指标实时读取以进行监督而无需裁决;4)事后重放在修正的Halpern-Pearl定义下产生连带效应,并计算补偿集大小;5) synergy residual衡量组合造成的伤害而非个人,使laundering可见。原因、责任和问责分别报告,每项由证据层 cap,并提供未来研究议程。

Result: 该架构实现了:1)遗漏检测而不仅是篡改检测;2)无需裁决的实时监督;3)归因 laundering 的可见化;4)原因、责任和问责的分离报告,每项由证据层限制。

Conclusion: HANSARD通过将问责制作为生命周期属性来处理自主多智能体系统的责任归属问题,在智能体触及范围之外设置捕获点,构建因果图并使用修正的Halpern-Pearl定义进行事后分析,以及通过synergy residual揭示组合效应,使归因 laundering变得可见,为自主智能体系统的可问责性提供了系统性的技术框架。
...
### [282] [CONTRAMEM: Learning Self-Evolving Procedural Memory from Contrasting Multi-Model Trajectories](https://arxiv.org/abs/2608.22533)
...
### [283] [STAGE: Stateful Translation to Agentic Graph Execution with Policy-Scoped Context and Deterministic Control](https://arxiv.org/abs/2608.22538)
*Mengxi Luo,Changjia Chen,An Cao,Zirong Huang,Wanyi Dai*

Main category: cs.AI

TL;DR: Stage是一个可执行图框架，通过将模型判断限制在策略作用域节点内，同时将程序控制置于确定性代码中，来提高政策执行的可重复性。在多个基准测试上，该方法相比整体式全策略执行显著提升了任务成功率和多次运行的可靠性。


<details>
  <summary>Details</summary>
Motivation: 政策治理的智能体在解释案例证据时必须遵循授权程序，但现有方法在执行复杂策略时缺乏可靠性和可重复性。研究者希望找到一种既能保持模型判断能力又能确保程序控制确定性的方法。

Method: 提出Stage框架：1)将模型判断限制在政策作用域节点内；2)将程序控制置于确定性代码中；3)每个节点模型接收任务相关策略上下文并返回类型化结果；4)协调器强制执行审查后的执行契约。

Result: 在SOP-Bench Referral Abuse、两个τ²-bench领域和Smart Dispute银行基准上评估。相比整体式全策略执行，Stage在程序复杂度不同的工作流中普遍提高了任务成功率和重复运行可靠性。在较深的Telecom和Smart Dispute工作流上提升最大，Pass³分别提高7.5-55.0和57.2-65.7个百分点。

Conclusion: 将政策作用域上下文与确定性程序控制相结合，可以显著提高政策执行的可重复性。这一方法为构建更可靠的政策治理智能体提供了有效途径。
...
### [284] [Scaling Curriculum Learning For Autonomous Driving](https://arxiv.org/abs/2608.22549)
*Cevahir Koprulu,David Paz,Feng Tao,Yuliang Guo,Xinyu Huang,Ufuk Topcu,Liu Ren*

Main category: cs.AI

TL;DR: CL4AD是首个将课程学习集成到批量自动驾驶模拟器中的方法，通过将场景选择建模为无监督环境设计问题，显著提升了强化学习在自动驾驶任务中的样本效率。


<details>
  <summary>Details</summary>
Motivation: 批量模拟器虽然能快速生成大量交互数据，但传统的域随机化方法均匀采样场景，导致大量交互浪费在贡献较小的案例上，样本效率低下。

Method: 提出CL4AD框架，将场景选择建模为无监督环境设计问题；引入基于成功率、代理行为真实性和遗憾估计的效用函数来塑造课程；在大规模GPUDRIVE模拟器中进行实验。

Result: 课程学习比域随机化提前十亿步达到99%成功率，缩短77% wall-clock时间；在静态和动态属性启发式课程中表现最优（仅最大规模时有一例外）；有限计算下消融实验显示样本效率提升67%。

Conclusion: 课程学习能够有效解决批量自动驾驶模拟器中的样本效率问题，通过自适应优先处理对策略改进贡献最大的场景，可大幅减少训练时间和计算资源需求。
...
### [285] [ExecRubrics: Executable Tool-Augmented Rubrics for Verifiable and Efficient Long-Form Evaluation](https://arxiv.org/abs/2608.22559)
*Kaustubh D. Dhole,Charles L. A. Clarke,Eugene Y. Agichtein*

Main category: cs.AI

TL;DR: ExecRubrics是一个将语言模型评估标准表示为可执行Python程序的框架，通过将评估逻辑编码为可验证的评分函数，替代昂贵的黑盒LLM评判器，实现更快、更可解释的评估。


<details>
  <summary>Details</summary>
Motivation: 当前自然语言评分标准存在模糊性、需要黑盒LLM评判器、假设标准通过线性加权独立聚合等问题，无法捕捉依赖关系、替代方案、惩罚条件和覆盖条件。

Method: 将评分标准编码为可执行的Python评分函数，为自然语言评分标准提供可操作语义：可检查、执行和编辑的固定决策程序。可集成NLTK和spaCy等文本处理库。

Result: 在HealthBench、HelpSteer和ArgQuality三个长响应基准测试中，最佳偏好准确率分别达到53%、78%和92%，评估延迟降低高达320倍，可替代昂贵的黑盒评判器进行排名。

Conclusion: 提供了一种更快、更可解释、更少模糊的评分标准评估替代方案，特别适用于医疗和银行等高风险领域，这些领域精确性和可审计性至关重要。
...
### [286] [CausalCache: Conditional High-Fidelity Restoration for Long-Horizon GUI Agents](https://arxiv.org/abs/2608.22577)
*Jiaxuan Luo,Zhanfeng Liao,Jiayao Teng,Yuan Wang,Haojian Huang*

Main category: cs.AI

TL;DR: 本文提出CausalCache方法，解决长程GUI智能体在视觉上下文预算有限的情况下，如何在完整交互历史中最优分配高保真图像显示资源的问题。核心思想是将图像显示预算从仅用于最近事件，重新分配到整个历史轨迹中。


<details>
  <summary>Details</summary>
Motivation: 当前长程GUI智能体虽然可以低成本保存完整交互痕迹为文本形式，但策略只能访问少量高保真像素图像。每个事件以摘要形式保存并链接到存档截图，而视觉上下文预算B限制了可提升为"摘要+图像"形式的事件数量。现有方法将所有预算用于最新事件，导致历史信息利用不足。

Method: 1.条件保真度恢复：每个事件以摘要形式保存并链接存档截图，预算B限制可升级为摘要+图像的事件数量。2.因果缓存(CausalCache)：将相同B次升级重新分配到完整轨迹，仅当遥远事件具有更高条件边际效用时才驱逐最近图像。3.HGKV适配器：仅修改恢复的历史图像token，在无历史图像时完全绕过。4.差分监督：匹配预算的替换组和每臂锚定的差分监督使统一历史放大为零。5.预算感知选择器：选择哪些摘要事件需要恢复为高保真图像。

Result: 在桌面环境中，冻结策略对任务相关的存档截图与可驱逐的最近帧之间无可靠偏好，HGKV在指定漂移范围内学习到这种选择性。在OSWorld-Verified上，将历史恢复为高保真相比仅摘要记忆提升约13个成功点，同预算分配无显著差异。在跨应用移动基准的零样本测试中，CausalCache相比相同预算的最近分配显著提升整体成功率达+3.7分，且收益集中在应该集中的地方：在基准元数据构造时确定的记忆关键分割上提升+8.6分，对匹配对照组无显著影响，分割与方法间存在显著交互作用。

Conclusion: CausalCache通过智能分配有限的视觉上下文预算，使GUI智能体能够有效利用完整交互历史的视觉信息。实验证明将预算分配到整个历史轨迹而非仅最近事件，可显著提升智能体在需要记忆的任务上的性能，特别是在长程交互场景中。该方法在零样本跨应用迁移中展现出明显的性能优势。
...
### [287] [Weakly supervised concept Bottleneck Learning for Robust Two stage Object centric visual reasoning](https://arxiv.org/abs/2608.22584)
*Sparsh Tiwari,Gesina Schwalbe,Bettina Finzel*

Main category: cs.AI

TL;DR: 本文提出动态正交概念瓶颈(D-OCB)，一种基于对象中心槽-VAE的框架，用于在极弱监督下提取与人类对齐的符号谓词。该框架通过动态学习最优超参数分配和自适应维度分配机制，有效防止表示崩溃，显著提升概念准确性。


<details>
  <summary>Details</summary>
Motivation: 两阶段神经符号架构虽然能清晰分离感知与推理，但将高级谓词锚定到视觉帧通常需要昂贵的标注成本。现有方法在极低监督场景下存在不稳定性和表示崩溃问题。

Method: 1) 采用对象中心槽-VAE框架提取符号概念；2) 动态学习最优超参数分配以消除手动调参负担；3) 惩罚概念子空间间的相关性以融入类别独立性先验；4) 引入动态维度分配机制，让表现好的概念向落后概念让出潜在维度。

Result: 在极弱监督下实现了高概念对齐性和下游视觉推理准确率，使用 minimal label budgets 即可匹配或超越端到端方法。

Conclusion: D-OCB通过自适应机制有效解决了视觉问题求解中的标注昂贵问题，为极弱监督场景下的概念学习提供了有效范式。
...
### [288] [Coalition-Aware Skill Reliability for Self-Evolving Agents](https://arxiv.org/abs/2608.22610)
...
TL;DR: 研究Agent技能库（Skill Bank）的可靠性问题，提出两种可靠性失效现象（联盟污染和跨域效用逆转）及两种干预方法（CASS和u-SMCO），在多个数据集上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有工作关注技能的获取、演进和检索等操作层面，但未解决一个根本问题：累积的技能是否真的对Agent行为产生积极的机械性贡献？即技能库的可靠性问题。

Method: 1) 系统性技能库审计：在不同银行组成和部署域下测量Agent行为变化；2) 提出两种可靠性干预：联盟感知技能选择(CASS)使用采样Shapley边际值选择更可靠的候选技能；无监督技能掩码联盟优化器(u-SMCO)掩码转移后降低检索质量的技能。

Result: 在LoCoMo、LongMemEval、HotpotQA和ALFWorld上的Agent实验表明，CASS和u-SMCO始终优于强基线方法，提升任务性能和跨域泛化能力。联盟条件可靠性建模降低了强化学习对噪声奖励波动的敏感性，并揭示了孤立技能评估的局限性。

Conclusion: 技能库的可靠性至关重要，需要采用联盟感知的评估和优化方法；CASS和u-SMCO为解决技能库可靠性问题提供了有效方案。
...
### [289] [DeepSAGE: Stage-Aware Reinforcement Learning for Structured CBT Counseling Dialogue](https://arxiv.org/abs/2608.22615)
*Qi Zhang,Heajun An,Prakriti Dumaru,Sang Won Lee,Lifu Huang,Pamela J. Wisniewski,Jin-Hee Cho*

Main category: cs.AI

TL;DR: DeepSAGE是一个结合大语言模型与深度强化学习的混合框架，用于实现基于认知行为疗法(CBT)第一阶段的阶段感知心理咨询对话系统。该系统将会话表示为11个阶段，外部控制器确定阶段完成状态，DRL模型选择治疗意图来指导LLM响应生成。实验表明该系统在模拟客户参与度、开放性及阶段目标完成率方面优于6种基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM咨询代理虽然能生成流畅且具有支持性的回复，但缺乏进行连贯治疗会话所需的结构化、目标导向的进展机制。

Method: 提出DeepSAGE框架，包含：(1)将会话表示为11个具有明确治疗目标的阶段；(2)外部控制器判断阶段是否完成；(3)深度强化学习模型选择治疗意图以引导LLM生成响应。

Result: DeepSAGE在模拟客户参与度和开放性方面表现更高，在阶段结构化系统中达到阶段目标完成与对话效率的最佳平衡。领域专家评估表明生成的对话呈现合理的情绪轨迹和可识别的CBT过程。

Conclusion: 结合阶段结构化对话与学习策略选择是AI咨询的有前景方法，但临床有效性、安全性和实际应用价值仍需进一步人类评估验证。
...
### [290] [CAI-DLLM: Convergence Aware Inference for Diffusion Language Models](https://arxiv.org/abs/2608.22646)
*Farhana Amin,Sabiha Afroz,Dimitrios S. Nikolopoulos*

Main category: cs.AI

TL;DR: CAI-DLLM是一种免训练的推理加速方法，通过首步置信度指导去噪过程，减少扩散语言模型的推理时间。该方法提前提交简单token，为困难token分配更多去噪步骤，并调整输出块的解码 schedules。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型虽然可以并行生成多个token，但推理时仍需重复去噪步骤，导致计算成本高。特别是模型会重复计算已经稳定的token，造成资源浪费。

Method: CAI-DLLM利用首步置信度信号来指导去噪过程，具体包括：1)提前提交简单token；2)为困难token分配更多去噪步骤；3)调整输出块的解码时间表。该方法不需要重训练、额外预测器或权重更新。

Result: 在LLaDA-8B-Instruct和Dream-7B-Instruct上评估，在数学、代码、推理、常识和长上下文任务中：LLaDA GSM8K最高加速18.2倍，准确率从76.27%提升到77.41%；Dream HumanEval加速13.1倍，pass@1从46.95%提升到48.17%；困难推理任务加速达44.8倍，准确率最多下降4.4点，能耗降低最多95.3%。

Conclusion: CAI-DLLM是一种有效的扩散语言模型推理加速方法，能够在保持甚至提升准确率的同时显著减少推理时间和能耗。该方法的优点是无需额外训练或修改模型权重，具有良好的实用价值。
...
### [291] [A-CPES: A Reference Framework for Agentic AI in Cyber-Physical Energy Systems](https://arxiv.org/abs/2608.22672)
*Xiaoyu Zhang,Qiuye Sun,Jiachen Xu,Zhongming Yao,Yushuai Li*

Main category: cs.AI

TL;DR: 本文提出A-CPES框架，通过三层嵌套环结构（授权问责环、代理控制外环、六层CPES核心）解决能源系统运营中人工调度瓶颈问题，并提出八种结构故障模式作为可证伪预测，以及六种治理模块来重建授权框架。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源比例上升，能源系统运营的人工调度循环（建模、可行性处理、排序、证据汇编、容量协商、经验固化）运转速度超过合格调度员数量增长，需要引入代理AI接管外环控制。

Method: 提出三层嵌套环架构：授权问责框架包裹代理控制外环，外环包裹六层CPES核心；将循环细分为可调节点，明确关闭位置和紧密度；提出八种结构故障模式作为可证伪预测；设计六种治理模块确保循环启动前授权框架覆盖完整。

Result: 建立了一个在授权和问责框架内的代理AI控制框架，可解决可再生能源高渗透率下调度员不足的问题，同时通过治理模块预防结构故障。

Conclusion: 能源系统运营的人工循环不可分割，需通过A-CPES框架将AI控制纳入授权问责体系，在系统启动前完成治理模块建设，确保安全有效的自动化调度。
...
### [292] [Robustness Analysis of Agentic AI to Inconsistent and Incomplete Tool Responses](https://arxiv.org/abs/2608.22676)
*Jiachen Xu,Torben Bach Pedersen,Zhongming Yao,Xiaoyu Zhang,Yushuai Li*

Main category: cs.AI

TL;DR: 研究AI代理是否能区分工具返回的不同故障类型（失败工具 vs 返回虚假信息），通过分析模型的对数概率在不同通道中的表现来识别故障。


<details>
  <summary>Details</summary>
Motivation: 当工具返回出现错误时，需要识别是工具完全失败还是返回了形式正确但内容错误的答案，因为两者需要不同的补救措施。问题在于返回到达时是否能立即区分这两类故障。

Method: 在零售客服领域注入受控故障，从模型的对数概率中读取两个通道：仅在工具schema下的返回内容似然度，以及在整个轨迹下的似然度；并读取法律动作分布的形状和位置。

Result: 不完整返回在所有情况下都可识别，在schema通道下概率极低且独特；不一致返回则保持schema通道不变，在已有上下文信息的字段上显现差异；动作分布为每种条件提供独特签名，按与下一动作的距离排序。

Conclusion: 识别具有不对称性：每种故障在某些通道可识别，但没有通道能识别所有故障类型。需要多通道结合来全面检测不同工具返回故障。
...
### [293] [Does Rank Still Matter? Position Bias When AI Agents Shop on Our Behalf](https://arxiv.org/abs/2608.22697)
*Davood Wadi,Yu Ma*

Main category: cs.AI

TL;DR: 研究AI代理在搜索结果页面上的行为模式，发现AI代理比人类搜索更深、从不拒绝购买，位置对检查的影响弱且非单调（页面中部检查概率最低），但所有模型最终都选择相同的最佳列表。


<details>
  <summary>Details</summary>
Motivation: 传统搜索排名理论基于人类注意力稀缺和顺序搜索的假设，但消费者现在越来越多地委托AI代理进行搜索，这些代理可以一次性摄取整个结果页面，需要研究AI代理的搜索行为是否与人类不同。

Method: 将100个酒店列表随机排列，在5000个AI代理会话中比较四种大型语言模型与人类实地数据。

Result: AI代理比人类搜索更深，且从不拒绝购买；位置仍预测列表检查概率，但很弱且非单调（页面中部检查概率最低，而非底部）；位置对某些模型有影响，对其他模型没有；所有模型最终都收敛到相同的未淘汰列表。

Conclusion: 对于代理搜索，结果页面上显示的属性比页面内的位置更重要。
...
### [294] [CacheRouter: A Dual-Path Tool Routing Architecture with Cache-Preserving Main-Model Isolation for Long-Tail Tool Discovery](https://arxiv.org/abs/2608.22708)
*Donghui Zha,Lingwei Xu,Linxiao Wu,Yixue Dong,Haochen Li*

Main category: cs.AI

TL;DR: 本文针对LLM系统中工具使用面临的结构性权衡问题——渐进式披露与提示缓存之间的冲突——提出了一种双路径路由设计，将工具选择和工具传递分离到不同通道中，以实现高效缓存与灵活工具扩展的兼得。


<details>
  <summary>Details</summary>
Motivation: LLM系统的工具使用中存在一个结构性权衡：一方面，渐进式披露通过仅展示当前任务相关的工具来保持提示的简洁；另一方面，提示缓存机制要求请求前缀在不同调用之间保持不变，而工具列表的任何变化都会使缓存的前缀失效。这两者之间存在不可调和的矛盾。

Method: 提出一种双路径路由架构设计，将工具选择和工具传递分离到两个独立通道。主模型始终看到一小部分固定的核心工具，因此其请求头在不同调用之间保持不变；其他所有工具通过独立的路由通道访问，在该通道中，路由器子模型搜索完整工具列表、选择并执行工具、返回结果。工具注册从源代码自动化实现并支持运行时更新，使工具集可以扩展而无需修改主模型的请求前缀。

Result: 在55个功能查询和30轮对话的测试中，token级缓存命中率分别达到90.99%和95.2%。在DeepSeek定价下，输入成本分别降至无缓存基线的约12.0%和8.0%（缓存命中的输入token成本约为缓存未命中token的1/30）。

Conclusion: 该设计将渐进式披露泛化为通过路由通道披露能力，而主模型前缀保持稳定，有效解决了工具使用中渐进式披露与提示缓存之间的权衡问题，实现了高效缓存与工具灵活扩展的兼得。
...
### [295] [SEAM: Shot Entity-Attribute Memory for Consistent Short-Drama Generation at Scale](https://arxiv.org/abs/2608.22725)
*Jiaqi Liu,Maolin Ran,Xiaoyang Lu,Jian Wang,Weiwen Liu,Jianghao Lin,Yong Yu,Weinan Zhang*

Main category: cs.AI

TL;DR: SEAM是一个无需训练、模型无关的记忆图谱，通过在提示词层面提取每帧多维状态、检索因果先验上下文、选择性过滤并通过自然语言提示重写来修复短视频生成的视觉连续性问题。


<details>
  <summary>Details</summary>
Motivation: 随着短剧生成从孤立镜头扩展到剧集级别，视觉连续性成为关键瓶颈。当前智能体框架孤立生成每帧镜头，导致上下文漂移，角色姿态、服装、道具等不一致，最终组装后产生严重视觉断裂。

Method: 提出SEAM（镜头实体-属性记忆），包含：1）提取每帧多维状态；2）在图上检索因果先验上下文；3）选择性过滤；4）通过自然语言提示重写注入约束。同时发布SEAM-Bench双盲连续性故事板 benchmark。

Result: SEAM将跨剧集连续性召回率从0.700提升至0.946，泛化到六个主流文本模型，在生成图像层面也获得一致（虽不显著）提升。在CreativeFitting生产流程的201个镜头部署中，达到96.5%导演接受率，零unsafe注入；保守估计其跨剧集记忆至少贡献21.9个百分点。

Conclusion: SEAM无需训练、模型无关，可无缝集成到现有短剧生成 pipeline，显著提升视觉连续性，为工业级短剧生成提供了有效的解决方案。
...
### [296] [LLM-Based Selection of Incongruent Verbal and Nonverbal Behavior for Virtual Humans](https://arxiv.org/abs/2608.22731)
*Parisa Ghanad Torshizi,Stacy Marsella*

Main category: cs.AI

TL;DR: 本文研究虚拟 agent 的非言语行为生成问题，提出基于 Ekman 框架的言语-非言语不一致分类法，探索大语言模型能否根据对话和社会情境选择合适的不一致行为，并通过人类受试者研究评估情境驱动的非言语行为效果。


<details>
  <summary>Details</summary>
Motivation: 当前非言语行为生成系统仅关注言语内容，忽视了说话者角色、人际关系、社会情境、认知和情感状态等因素的影响。非言语通道可以强化、弱化、限定甚至反驳言语通道，也可能泄露隐藏的情感状态。为了设计更真实、更像人类的虚拟 agent，尤其是在需要细微社会解读的训练场景（如虚拟患者咨询模拟）中，建模这种更丰富的言语-非言语关系至关重要。

Method: 1) 基于 Ekman 的言语-非言语关系框架，提出不一致行为的分类法；2) 探索使用大语言模型从给定对话和社会互动情境中选择上下文适当的不一致言语和非言语行为；3) 在人类受试者研究中评估这些行为，将情境驱动的非言语行为嵌入虚拟人身体，评估其对观察者的影响。

Result: 需要查看完整论文才能确定具体实验结果

Conclusion: 本文贡献包括：1) 提出了言语-非言语不一致的分类法；2) 探索了 LLMs 在选择上下文适当的不一致行为方面的能力；3) 通过人类受试者研究验证了情境驱动的非言语行为的效果。这些工作为虚拟 agent 的非言语行为生成提供了新的理论基础和方法。
...
### [297] [The Compaction Cliff in Long-Running AI Agent Memory](https://arxiv.org/abs/2608.22752)
*Saber Zerhoudi,Jelena Mitrovic,Michael Granitzer*

Main category: cs.AI

TL;DR: 本文提出了"知识分类"(Knowledge Triage)框架,解决AI代理在上下文压缩时安全规则快速衰减的"压缩悬崖"问题。该框架通过将知识库按类型分类,为不同类型设置独立的保留策略,在三种上下文管理操作中实现分类压缩、分类分解和分类检索。实验表明该方法在多个基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: AI代理的上下文存在安全规则和情节日志争夺token的问题。当预算溢出时,两者以相同速率被压缩,但安全规则需要精确措辞才能生效。现有压缩方法导致安全规则快速衰减,作者称之为"压缩悬崖"现象,这对代理的安全性构成严重威胁。

Method: 提出Knowledge Triage框架,将代理知识库的每行按类型分类,为每种类型设置独立的保留策略。实现三个确定性操作符:1)TypeCompact:按类型 fidelity 重写条目;2)TypeDecompose:将过大主题分区,在分区内复制安全规则;3)TypeRetrieve:从外部存储获取条目,将范围内规则优先于相关性固定。

Result: TypeCompact在所有比率下比最强单次LLM压缩器多保留2-4倍安全规则,5轮后召回率96%。TypeDecompose达到0%局部性违规(统一分区为93%)。TypeRetrieve达到100%召回率@50(单次检索器为73%)。下游行为基准:医学合规性显著优于生产Sonnet压缩器(p<10⁻⁸),零售任务通过率优于基线(p<0.01),航空领域优于分层压缩(p=0.024)。

Conclusion: Knowledge Triage框架通过类型化上下文管理有效解决了压缩悬崖问题,为代理上下文管理提供了新范式。该方法在安全关键应用中具有重要价值,作者还发布了包含396,934个代理配置的AgentArtifactCorpus数据集及相关代码。
...
### [298] [Compositional Chain-of-Relations for Faithful Knowledge Graph Question Answering with Large Language Models](https://arxiv.org/abs/2608.22762)
*Chenhui Liu,Jianpeng Zhou,Jiahai Wang*

Main category: cs.AI

TL;DR: 本文提出了一种名为组合关系链(CCoR)的新框架，用于解决知识图谱问答(KGQA)中的复杂多跳推理问题。该框架采用以关系为中心的探索范式，避免了传统以实体为中心方法中不可靠的实体剪枝问题，并将候选检索和约束处理两个阶段都建立在知识图谱之上。


<details>
  <summary>Details</summary>
Motivation: 现有基于代理的KGQA方法存在两个关键局限：(1)不可靠的实体剪枝：以实体为搜索单元需要在每跳将实体剪枝到固定大小子集，导致有效实体被丢弃；(2)未扎根的约束处理：约束处理依赖于LLM内部知识而非知识图谱，导致最终答案无法验证且易产生幻觉。

Method: 提出以关系为中心的探索范式，使用关系而非实体作为搜索单元，从而避免不可靠的实体剪枝。在此范式基础上，提出CCoR框架，通过两条关系链将两个阶段都扎根于知识图谱：主链用于候选检索，约束链用于通过显式知识图谱探索验证查询约束。

Result: 在四个KGQA基准数据集上的实验表明，CCoR在准确率、忠诚度和效率方面均优于强基线方法，且在复杂查询上的提升更为显著。

Conclusion: 通过以关系为中心的探索范式和CCoR框架，成功解决了KGQA中实体剪枝和约束处理接地的问题，显著提升了复杂多跳推理的表现。
...
### [299] [The Retriever Should Remember: Experience-Amortized Reranking for Long-Term Agent Memory](https://arxiv.org/abs/2608.22767)
*Qi Feng,Chris Ding,Jicong Fan*

Main category: cs.AI

TL;DR: EARM提出了一种经验累积的排序框架，使LLM重排序器能够重用历史查询-记忆相关性评分，通过因果矩阵补全学习共享结构，在仅需17.5%直接评分的情况下仍保持有效性。


<details>
  <summary>Details</summary>
Motivation: 当前长期语言模型智能体虽能累积记忆，但其检索器无法累积检索经验。语义检索高效但嵌入相似度不等于相关性；LLM重排序虽更准确但无状态，每次查询都需重新评分整个候选池，开销大。

Method: EARM将历史LLM相关性评分视为可复用的检索经验，存储在在线矩阵中，通过因果矩阵补全学习共享结构，将新观察到的评分与估计评分结合来重排序候选。

Result: 混合观察-估计重排序相比语义检索最高提升6.82%准确率，在仅17.5%候选获直接LLM评分时仍有效，显著降低推理开销。

Conclusion: 长期智能体不仅应记住过去内容，还应记住该内容曾如何被证明对检索有用，记忆应包括检索经验本身。
...
### [300] [TailSieve: Partial-Rollout-Guided Tail Routing for LLM Rollouts](https://arxiv.org/abs/2608.22788)
...
TL;DR: TailSieve是一个针对LLM大规模rollout的优化框架，通过尾部隔离和负载均衡的组合策略，将长尾生成任务与常规任务分离处理，从而减少端到端完成时间。


<details>
  <summary>Details</summary>
Motivation: 在大规模LLM系统中，rollout请求通常均匀路由到各副本，但长尾生成会主导整个rollout步骤的完成时间。现有方法未考虑生成长度差异，导致高并发解码批次中存在极长生成，降低系统效率。

Method: 1)基于部分rollout识别长尾提示候选组；2)分层控制器联合调整隔离组数量和尾部/主体池的副本分配；3)结合路由优化和专门化投机解码(MTP/DFlash)；4)对选中的提示在当前策略下重新生成，保持on-policy生成。

Result: 路由优化实现最高1.67倍加速；结合投机解码后最高2.59倍加速；相比均匀路由显著降低尾部延迟。

Conclusion: 通过尾部隔离与负载均衡的近似最优策略，结合部分rollout信号和分层控制，TailSieve有效解决了LLM rollout中的长尾延迟问题，在保持生成质量的同时实现了显著的性能提升。
...
### [301] [Performance of a domain-specific large language model in answering patient questions in psychiatry](https://arxiv.org/abs/2608.22797)
*Alexander J. Hish,Arjun Nagendran,Scott N. Compton*

Main category: cs.AI

TL;DR: 研究开发了一种专门针对精神科患者教育的领域特定大语言模型MIND，并与ChatGPT和OpenEvidence在回答艾司西酞普兰相关患者问题方面进行比较评估。


<details>
  <summary>Details</summary>
Motivation: 旨在评估专门针对患者教育资源训练的领域特定LLM是否能在精神科药物相关问题回答上优于通用LLM聊天机器人。

Method: 采用两种评估方法：(1)计算机分析按准确性、清晰度、完整性、细微差别、安全性和转诊适当性评分；(2)由10名持牌精神科医生进行评分比较。

Result: 评分表评估中MIND在所有领域评分最高(p<0.001)。精神科医生评估中，ChatGPT准确性略高于MIND但效应量可忽略(p=0.021, r=0.073)；MIND完整性高于ChatGPT且效应量小(p<0.001, r=0.160)；两者安全性相当(p=0.955)。多数精神科医生偏好ChatGPT(57.6%)而非MIND(42.4%, p=0.003)。

Conclusion: MIND能够以精神科医生认为准确、完整和安全的方式回答艾司西酞普兰相关问题，但尽管MIND回答更完整，精神科医生仍更偏好ChatGPT的回复。MIND代表了在精神科患者教育领域构建安全LLM系统的重要一步。
...
### [302] [Beyond the Harness: End-to-End Optimization of Context Artifacts for Enterprise Text-to-SQL](https://arxiv.org/abs/2608.22830)
*Kate Gwimm,Carson Eisenach*

Main category: cs.AI

TL;DR: 本文研究企业场景下Text-to-SQL系统的上下文优化问题，指出模型性能瓶颈不在于模型本身，而在于能否获取正确的知识库上下文。研究提出从历史查询中构建可复用SQL参考卡的蒸馏方法，在5176条生产查询中取得12-25%的AST相似度提升，在BEAVER基准测试上获得9.00% vs 6.33%的提升。


<details>
  <summary>Details</summary>
Motivation: 企业Text-to-SQL面临的核心挑战是：业务逻辑跨越数千张表，没有模型能一次性摄入完整目录。传统方法将上下文作为固定输入进行调优，但作者认为应该从历史使用记录中动态构建上下文。

Method: 1）使用查询DAG分解（同企业基准BEAVER注释的中间表示）作为知识表示；2）比较神谕查询图与检索知识库上下文的效果差异；3）优化蒸馏程序，将历史查询配置转化为可复用的SQL参考卡；4）在5176条生产查询和BEAVER基准上进行实验验证。

Result: 在生产环境数据中，优化上下文构件带来的收益（~12-25% AST相似度）远大于优化检索框架（~3-12%）。在BEAVER基准上，最优方案同时检索参考卡和原始SQL，在300条测试集上达到9.00% vs 6.33%（p值0.12）。

Conclusion: 对于企业Text-to-SQL系统，从历史使用中构建知识库上下文比单纯优化检索机制更有效。SQL参考卡是一种有效的上下文构件形式，但在缺乏生产使用信号的公开基准上提升幅度有限。
...
### [303] [Let the Bullets Fly: Multimodal Fake News Detection with Temporal-Aligned Generative Danmaku](https://arxiv.org/abs/2608.22832)
*Xiansheng Luo,Chaowei Zhang,Zewei Zhang,Yi Zhu,Jipeng Qiang*

Main category: cs.AI

TL;DR: 本文提出Genda框架和DM-FEND模型，利用弹幕的时空特性进行假新闻检测，解决了弹幕累积延迟与实时检测需求之间的矛盾。


<details>
  <summary>Details</summary>
Motivation: 弹幕作为新型社交交互形式，可提供细粒度的社会信号，但其实时累积延迟违反了假新闻检测的实时性需求，导致相关研究不足。

Method: 提出(1)Genda框架：包括弹幕触发器（预测用户反应时机和强度）和弹幕生成器（合成语义和情感表达）；(2)DM-FEND模型：实现视频、音频、文本和弹幕的细粒度多模态交互，增强动态模态对齐和语义噪声抑制。

Result: 在中文(FakeSV)和英文(FakeTT)基准数据集上均优于最先进的基线方法，消融实验验证了时间弹幕建模对增强鲁棒性和判别能力的关键作用。

Conclusion: 通过桥接新闻与用户行为之间的时间不一致性，为现代社交互动模式下的多模态假新闻检测提供了稳健的解决方案。
...
### [304] [FinixDoc: Rethinking Financial Document Parsing Beyond Saturated Benchmarks](https://arxiv.org/abs/2608.22842)
...
TL;DR: FinixDoc是一个针对真实世界金融文档的端到端解析系统，核心是基于Qwen3-VL-4B构建的4B参数视觉语言模型FinixDoc-VL。针对基准测试与实际部署之间的性能差距，作者提出了基于视觉质量和文档规模的文档解析能力矩阵，并采用同形字感知的对比学习和多阶段强化学习进行领域适配训练，同时构建了人机协作的数据工厂流水线。评估方面构建了涵盖四类场景的FinixDocBench基准测试，FinixDoc-VL取得81.43的最高综合得分。


<details>
  <summary>Details</summary>
Motivation: 当前金融文档解析基准测试无法准确反映实际部署中的准确性、结构一致性和可验证性要求，存在基准测试与真实场景之间的性能差距。

Method: 1)提出文档解析能力矩阵，沿视觉质量和文档规模两个实用轴进行问题建模；2)采用领域适配训练方案，包括同形字感知的对比学习和多阶段强化学习（结合复合领域特定奖励）；3)构建人机协作的Data Factory流水线，支持大规模高质量数据生产；4)构建FinixDocBench评估套件，覆盖数字化原生的、相机拍摄的、超大页面和内部工作流四种场景。

Result: FinixDoc-VL在主测试集上获得最高综合得分81.43，比次优开源模型高5.13分；在内部金融工作流（FinixInner）上优势最明显：84.08 vs 78.73。

Conclusion: 通过领域适配训练、人机协作数据生产和针对性的评估基准，FinixDoc-VL能够有效解决金融文档解析的实际挑战，在多种场景下尤其是内部工作流场景达到领先性能。
...
### [305] [GSAR: Goal-State-Anchor Rewards for Mobile GUI Agents with Self-Evolving Data Synthesis](https://arxiv.org/abs/2608.22847)
*Long Zhang,Yuhan Chen,Chaoran Zhang,Wanxia Cao,Kun Huang,Pengzhi Gao,Wei Liu,Jian Luan,Chenliang Li,Lixin Zou*

Main category: cs.AI

TL;DR: 本文提出GSAR框架，通过自演进数据合成和状态锚机制解决VLM-GUI智能体训练中的数据多样性和奖励信号可靠性问题，实现90%以上的离线轨迹验证准确率，为GUI智能体训练提供可扩展方案。


<details>
  <summary>Details</summary>
Motivation: 当前VLM-GUI智能体训练面临两个根本性瓶颈：1）现有数据合成方法依赖特定环境，难以生成多样化数据；2）现有评估器要么可扩展性有限，要么提供不准确和不可靠的奖励信号。这限制了基于在线强化学习的智能体训练效果。

Method: 提出GSAR（目标-状态-锚奖励）框架，包含两个核心组件：1）自演进数据合成：通过任务执行生成多种环境，产生多样化的任务和目标状态；2）状态锚机制：自动标注成功目标状态中与任务相关的UI元素作为参考锚点，为强化学习训练提供准确、可扩展的奖励信号。

Result: 1）离线轨迹验证准确率达到90%以上；2）性能最接近基于规则的方法；3）在AndroidWorld和自建基准测试上表现优异，证明了该方法在GUI智能体训练中的有效性和可扩展性。

Conclusion: GSAR框架通过自演进数据合成和状态锚机制成功解决了VLM-GUI智能体训练中的数据多样性和奖励信号可靠性问题。该方法在离线轨迹验证中达到90%以上准确率，表现出接近基于规则方法的性能，为GUI智能体训练提供了一种可扩展的解决方案。
...
### [306] [Your AI, On a Dial: Controlling Investment Bias in LLMs with a Single Neuron](https://arxiv.org/abs/2608.22852)
...
TL;DR: 本文研究如何在校准时间对大语言模型的投资立场进行方向和强度的调整。通过引入"投资偏见拨盘"(investment-bias dial)——一种对单个神经元的推理时干预方法，可以在不修改提示或模型参数的情况下连续调整模型的整体决策先验——即其买入或卖出的总体倾向。研究在五个开源大语言模型上进行了评估，发现该拨盘能够产生单调的投资立场变化，且在响应层面、检索层面和长上下文场景下均表现出良好的控制效果。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明，大语言模型在投资决策中表现出系统性的、模型特有的投资偏好，但尚不清楚是否可以将模型的整体投资立场校准到指定的方向和强度。这一研究动机源于实际应用中对投资AI系统可控性的需求。

Method: 研究引入了一种名为"投资偏见拨盘"(investment-bias dial)的推理时干预方法，通过对单个神经元进行干预来连续调整模型层面的决策先验。该方法使用匹配的正负证据进行评估，在五个开源大语言模型上测试，无需修改提示或模型参数即可实现立场调整。

Result: 研究结果表明：1)拨盘在投资决策和生成理由的证据权重上产生单调变化；2)在智能体检索设置中，拨盘改变了模型搜索的信息、选择的证据以及最终分析中反映的证据；3)在长上下文评估中，拨盘保持稳定的立场控制，而系统提示指令则随上下文长度增加逐渐衰减；4)在探索性回测中，拨盘变化会传播到证券排名和投资组合配置。

Conclusion: 大语言模型的整体投资立场可以在推理时校准到指定目标。该研究为投资AI系统的可控性提供了新的方法，具有实际应用价值。
...
### [307] [Proxy reliance in large language model decisions is uncalibrated to predictive evidence](https://arxiv.org/abs/2608.22887)
*Zengqing Wu,Chuan Xiao*

Main category: cs.AI

TL;DR: 本研究提出一种新的审计方法"因果代理效应"(causal proxy effects)，用于区分LLM决策中的合法推理与非法代理使用。研究在临床排序任务中比较四款LLM，发现当前基于准确率的评估方法完全无法检测到代理依赖问题，而新方法可识别出过度依赖、合理依赖和不足依赖三种情况。


<details>
  <summary>Details</summary>
Motivation: LLM已被用于分诊和借贷等高风险决策场景，这些场景需要区分任务相关的合法推理与基于受保护属性的非法代理使用。现有审计方法仅检查人口统计特征变化时决策是否改变，但这无法区分歧视性决策与合理推理，因为与受保护群体相关的属性本身可能具有预测价值。

Method: 研究在具有已知真实标签的临床排序任务中测量四款LLM的因果代理效应，可精确计算证据所支持的合理依赖程度作为参考基准。审计信号产生三种判定：过度依赖、合理依赖和不足依赖。

Result: 1. 中性标签下，所有模型都依赖无信息的代理特征；2. 信息性代理特征引发三种判定；3. 社会领域名称将依赖推到参考水平以下（某一模型）；4. 依赖严重低估证据价值；5. 社会标签抑制脆弱，上下文示例可使其高于零；6. 基于准确率的评估完全无法检测到这些问题。

Conclusion: 基于决策变化的传统审计方法不足以检测LLM中的代理依赖问题，需要采用因果推理方法来精确测量证据支持度。新方法能识别传统方法遗漏的歧视性推理模式，对AI公平性审计具有重要意义。
...
### [308] [CDEG: Learning Decision-Critical Evidence for Long-Horizon Diagnostic Agents](https://arxiv.org/abs/2608.22899)
*Xiwei Dai,Zijie Meng,Zhiting Fan,Yixuan Tang,Ziru Niu,Zuozhu Liu*

Main category: cs.AI

TL;DR: CDEG是一个基于图结构的框架，用于从历史诊断轨迹中学习可重用的关键证据，解决长程医学诊断中证据获取不足和整合不当的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医生智能体在关键证据未获取或未充分整合到诊断推理时表现不佳，虽然尝试复用历史轨迹或蒸馏记忆，但增益有限，因为这些经验可能包含噪声或偶发信息，且未验证哪些证据真正驱动诊断决策。

Method: CDEG对比同一病例的成功和失败轨迹来识别候选证据，通过受控的反事实干预验证其诊断影响，将诊断-证据-动作关系组织成结构化图。推理时跟踪患者证据状态，检索相关诊断关系，有选择地引导缺失证据获取或遗漏证据重新评估。

Result: 在域内和分布外基准测试中，使用多种医生智能体骨干网络，CDEG持续提升诊断性能，相比基础智能体最高提升11.5%准确率。

Conclusion: 可靠的长程诊断需要从轨迹级经验复用转向证据级学习，真正影响临床决策的因素。
...
### [309] [Beyond Observed Auxiliary Relations: Environment-Conditioned Modeling for Multi-Behavior Recommendation](https://arxiv.org/abs/2608.22920)
*Seunghan Lee,Hyunsik Yoo,Jian Kang,Susik Yoon,SeongKu Kang*

Main category: cs.AI

TL;DR: BOAR是一个环境条件化的多行为推荐框架，通过两个互补模块解决辅助行为信号的缺失和不可靠问题，在HR@10上实现最高7.82%的提升，对无辅助观测的目标项目实现最高44.2%的提升。


<details>
  <summary>Details</summary>
Motivation: 多行为推荐(MBR)利用点击、加购等辅助行为信号来增强目标行为（如购买）预测。现有的图神经网络方法存在两个根本性问题：(1)辅助信号缺失，导致无法泛化到无辅助观测的项目；(2)辅助信号不可靠，放大与目标行为不一致的噪声。

Method: 提出BOAR框架，包含两个条件化互补模块：(1)处理缺失辅助信号的模块；(2)处理不可靠辅助信号的模块。两个模块都基于辅助可观测性进行条件化处理。

Result: 实验表明BOAR持续优于最先进的基线方法，在HR@10指标上整体提升最高达7.82%，对于无辅助观测的目标项目提升最高达44.2%，证明其能捕捉超越观测到的辅助关系的隐藏偏好。

Conclusion: BOAR通过环境条件化方式统一解决了多行为推荐中辅助信号缺失和不可可靠的两个核心挑战，显著提升了推荐性能，特别是在辅助数据稀缺的场景下。
...
### [310] [Concepts for Securing Agentic AI Coding and the Terok Environment](https://arxiv.org/abs/2608.22930)
*Jiří Vyskočil,Franz Pöschel,Andreas Knüpfer*

Main category: cs.AI

TL;DR: 本文探讨了Agentic AI在软件开发中的IT安全问题，评估了其与传统AI辅助编码相比带来的新增风险，提出了风险缓解概念，并展示了概念实现方案。


<details>
  <summary>Details</summary>
Motivation: Agentic AI是一个新兴且快速发展的AI辅助软件开发工具，作者认为在探索其潜力的同时，不应忽视其带来的IT安全风险，需要以负责任的方式推进该技术的使用。

Method: 论文采用三部分研究方法：(I) IT安全风险评估，(II) 风险缓解概念设计，(III) 概念实现方案概述。

Result: 识别出Agentic AI相较于传统AI辅助编码带来更严重的安全风险，并提出了在不破坏其优势的前提下缓解这些风险的概念和实现方案。

Conclusion: 本文虽非最终解决方案，但是在Agentic AI用于软件开发的负责任使用方面迈出了重要一步，为社区提供了在避免实际IT安全风险的情况下评估该技术潜力的参考。
...
### [311] [What Process Evaluation of Coding Agents Actually Measures: Action, Task, and Step Are Three Different Levels](https://arxiv.org/abs/2608.22960)
*Jiawei He,Mengyu Shi,Jie jia,Xikai Yang,Dong Sun*

Main category: cs.AI

TL;DR: 本文提出了一种用于编码代理过程评估的测量框架，并设计了SCAE（基于回放的因果归因估计器）来研究动作级别、任务级别和步骤级别的过程评估。实验表明当前过程评估测量的主要是语义相关性而非因果贡献。


<details>
  <summary>Details</summary>
Motivation: 现有过程级评估将动作预测、任务不确定性和步骤归因视为同一问题，导致不清楚评估实际测量的是什么。需要一个明确的测量框架来区分这些不同的评估维度。

Method: 提出结合前缀条件识别、回放/干预估计和受控裁判信息操作的过程评估框架。SCAE基于代理执行的结构因果模型，通过回放方法进行步骤级因果归因。实验基于12个仓库的499个文件定位episode。

Result: 发现下一动作主要由执行来源驱动而非代码图转换；执行不确定性在任务级别而非步骤级别结构化；完整轨迹裁判存在系统性碰撞器偏差。这表明当前过程评估实际测量的是语义相关性而非认证的因果贡献。

Conclusion: 当前的过程评估方法存在根本性问题，测量的是语义相关性而非真正的因果贡献。未来需要采用因果归因方法来更准确地评估编码代理的执行过程。
...
### [312] [Buried in Textual Debt: Context Pruning with Visual Evidence Preservation for MLLM Agents](https://arxiv.org/abs/2608.22963)
*Yuchen Huang,Sijia Li,Jun Zhang,Yi R. Fung*

Main category: cs.AI

TL;DR: SPARE是一种基于KL散度的多模态工具使用智能体推理修剪框架，通过任务状态摘要作为诊断上下文，使用反向KL散度评估摘要是否能覆盖待修剪段落且不破坏后续推理，从而在保持准确率的同时移除37.89%-64.58%的推理标记。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型作为多步智能体时，自生成的推理文本会累积并主导上下文，压制视觉证据，形成"文本债务"问题。推理在视觉证据被锚定后变得冗余，而陈旧假设在锚定不确定时可能误导后续推理。因此需要移除冗余文本同时保留视觉证据。

Method: SPARE框架：(1)使用紧凑的任务状态摘要作为特权诊断上下文；(2)对每个候选段落，在原始上下文和摘要条件下重新播放模型；(3)使用策略自蒸馏(OPSD)的反向KL散度测试摘要是否充分覆盖该段落且不破坏未来推理；(4)通过监督微调(SFT)进一步优化摘要器，实现更紧凑的摘要和更激进的修剪。

Result: 在多步视觉工具使用基准测试中，SPARE在修剪方法中实现最高平均准确率，同时移除37.89%-64.58%的推理标记。

Conclusion: SPARE通过减少文本主导地位恢复了视觉证据的依赖，缓解了过度依赖自生成语言的问题，实现了有利的准确率-上下文权衡。
...
### [313] [ParallelWorld: Test-Time Scaling for Embodied Reasoning](https://arxiv.org/abs/2608.22971)
*Min Chen,Shengjun Zhang,Yuxin Li,Zhang Zhang,Xin Fei,Chong Xia,Yueqi Duan*

Main category: cs.AI

TL;DR: 本文提出ParallelWorld，一个用于具身推理的多视野测试时扩展框架。该框架通过引入验证器引导的树搜索范式，使智能体能够在采取行动前并行模拟和评估多步未来轨迹，从而解决现有方法中缺乏长期规划和短视决策的问题。


<details>
  <summary>Details</summary>
Motivation: 现有主动推理方法通常增量生成探索轨迹，缺乏长期规划；即使是最近的测试时扩展框架也往往采用短视的单步前瞻，难以解决复杂遮挡空间中延迟反馈的问题。

Method: 提出ParallelWorld框架，包含：1）验证器引导的树搜索范式；2）从当前状态分支多个并行轨迹并在多步视野上持续展开；3）验证器评估中间状态转换，动态剪枝无前景分支；4）优先考虑信息增益最高的路径；5）答案智能体对选定轨迹进行推理生成最终答案。

Result: 在ESI-Bench上的广泛实验表明，ParallelWorld持续提升主动感知和推理性能。

Conclusion: ParallelWorld通过多视野测试时扩展和验证器引导的树搜索，有效解决了具身推理中的长期规划问题，为复杂环境中的智能推理提供了新的研究方向。
...
### [314] [Toward Effective and Reliable LLM Agents via Dynamic Ontology](https://arxiv.org/abs/2608.22974)
*Xiaohui Zhang,Zequn Sun,Chengyuan Yang,Yuanning Cui,Lingbing Guo,Wei Hu*

Main category: cs.AI

TL;DR: OaK是一个 ontology-as-a-kernel框架，可以动态为LLM智能体构建和完善面向任务的本体论。它通过从任务需求和训练数据构建本体论及其知识图谱，生成图推理任务适配函数，并利用评判反馈进行迭代优化，从而将相关概念和关系明确化，为知识检索和多步决策提供基础。


<details>
  <summary>Details</summary>
Motivation: LLM智能体严重依赖模型参数中编码的知识或以非结构化上下文形式呈现的知识。在特定领域任务中，这导致重要的语义关联隐含未明，常造成证据使用不完整和多步决策脆弱的问题。虽然本体论可以将领域概念和关系外部化为机器可解释的结构，但传统构建需要领域专家大量精力，难以扩展；而自动构建的本体论虽然语义上合理，却可能缺乏实际决策所需的关系结构。

Method: 提出OaK框架，包含以下核心步骤：1)根据任务需求和训练数据构建本体论及其知识图谱；2)生成用于图推理的任务适配函数；3)利用评判反馈对本体论和知识图进行迭代优化。通过使相关概念和关系明确化，本体论能够为知识检索和多步决策提供基础。

Result: 在TravelPlanner、CRMArenaPro和ToolQA三个数据集上的评估表明：OaK显著提升了标准LLM智能体的性能，增强了证据 grounding，提高了多步推理的可靠性。

Conclusion: OaK的ontology-as-a-kernel方法通过动态构建和完善面向任务的本体论，成功将隐含的语义连接显式化，有效解决了LLM智能体在特定领域任务中证据使用不完整和决策脆弱的问题，为知识检索和多步决策提供了可靠的基础。
...
### [315] [Budget-Constrained Embodied Perception: Four Resource Walls and a Pre-Registered Evaluation of Access-Structured Perception on Open Models at less than 31B](https://arxiv.org/abs/2608.22975)
*Defu Lin,Wenhui Chen,Ziyao Lin,Jianlin Chen,Peiji Long,Chi Man Vong*

Main category: cs.AI

TL;DR: 本文研究具身多模态智能体在固定token预算下处理观察流的问题，提出ASP框架但未验证其核心假设


<details>
  <summary>Details</summary>
Motivation: 解决具身智能体在长时序观察流中面临的资源约束问题，四个"资源墙"限制了感知、检索和推理能力

Method: 提出ASP框架：结合受限结构化状态、逐字情景索引和查询条件预算分配，在7个3B-31B参数模型上评估

Result: 检索准确率75-94% vs 基线3-19%，预算重分配优于4倍采样预算。但移除压缩状态后性能反升(35.4→58.0)，ASP未超越逐字基线

Conclusion: 查询条件访问机制比参数量或上下文增长更重要，但提示式在线压缩在此设置中未带来收益，核心假设未通过验证
...
### [316] [SA-RSQ: A Versatile Sparse Representation Framework for Multi-modal Recommender Systems](https://arxiv.org/abs/2608.22979)
*Xiang Wang,Shigang Quan,Tingzhen Chang,Kang Yang,Sitong Chen,Yabo Fan,Xingxing Wang,Zhaodian He*

Main category: cs.AI

TL;DR: SA-RSQ是一种用于工业推荐系统的高维多模态特征的压缩存储方法，通过Top-K稀疏路由和softmax权重存储紧凑的(索引,概率)元组，解耦了每项存储与码本维度的关系，在8-48字节存储预算下实现了良好的重建性能和CTR权衡，在线A/B测试中CTR提升+2.51%，CPM提升+3.66%。


<details>
  <summary>Details</summary>
Motivation: 在工业推荐系统中部署高维多模态特征会产生大量的存储和延迟开销。硬量化虽然紧凑但会引入边界畸变，而密集软量化则使表示质量受限于有限的存储预算。

Method: 提出稀疏激活残差软量化(SA-RSQ)，使用Top-K稀疏路由和softmax权重来存储紧凑的(索引,概率)元组。该方法将每项存储与码本维度解耦；对于固定的支持集，梯度通过路由权重和加权重建传播，不依赖直通估计器。

Result: 在食品配送广告数据集上的实验表明，在8-48字节每项的存储预算下，重建性能和CTR权衡表现良好。Next-Distribution Prediction研究和一周在线A/B测试进一步验证了SA-RSQ的实用性，CTR相对提升+2.51%，CPM相对提升+3.66%。

Conclusion: SA-RSQ通过稀疏激活机制有效解决了推荐系统中高维特征存储与性能之间的矛盾，在实际线上环境中取得了显著的CTR和CPM提升，具有良好的实用价值。
...
### [317] [PatchWrite: One Line, Not One Section -- Compile-Gated, Validity-Preserving Editing for AI-Drafted Manuscripts](https://arxiv.org/abs/2608.23001)
*Weiwei Yang*

Main category: cs.AI

TL;DR: PatchWrite是一种约束自动手稿编辑的方法，通过致命日志检查和证据锁确保只有通过编译验证且引用/数值有据可查的编辑才能被接受，避免了传统整段重写方法中常见的无关内容被意外修改的问题。


<details>
  <summary>Details</summary>
Motivation: 自动化手稿管道在修复局部缺陷时经常意外改变无关的指标、引用和内容，即使生成的PDF可以正常构建。

Method: PatchWrite采用bounded编辑和回滚机制，引入致命日志检查确保编译通过，并添加证据锁要求每个引用的键和实验数值token必须由引用注册表或实验日志证明。失败的候选被拒绝，保持之前的HEAD状态。

Result: 在24个手稿×8个故障的压力测试中，整槽重写在192个案例中全部改变了无关的"12-layer"行（0/192保留），而PatchWrite在192/192个案例中完全保留。使用写作模型时，75%的候选被接受，93.75%修复了注入的故障。盲评显示PatchWrite在保留实验事实方面显著优于传统方法（Likert 5.0 vs 2.0）。

Conclusion: PatchWrite通过编译门和证据门双重验证机制，有效约束了自动编辑的边界，显著减少了无关内容被意外修改的问题，在实际产品中也表现出相同的失败模式。
...
### [318] [PsychJail: Exploring Psychological Jailbreaks via Multi-Turn Persuasion of LLM Policies](https://arxiv.org/abs/2608.23028)
*Zeyu Feng,Qingyu Wu,Yuzhe Luo,Hua Cheng*

Main category: cs.AI

TL;DR: PsychJail is a psychology-guided red teaming framework that uses multi-turn persuasion techniques to jailbreak aligned LLMs. By operationalizing the Persuasion Knowledge Model and using trajectory-level RL with PKM-gated rewards, it achieves 87.3% average attack success rate across four models, revealing four distinct psychological profiles that explain model-specific vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Large language models are increasingly deployed in education, healthcare, policy advising, and other interactive settings where users engage them as sustained social interlocutors. This shift makes jailbreaks a growing safety threat, yet most research emphasizes single-turn prompt optimization or iterative attack refinement, leaving psychologically grounded multi-turn vulnerabilities underexplored.

Method: PsychJail framework maps established social-psychological persuasion techniques into a tactic-conditioned attack policy. It factorizes each attacker action into: (1) Change-of-Meaning analysis, (2) tactic selection, and (3) victim-visible message - operationalizing the Persuasion Knowledge Model (PKM). The policy is refined with trajectory-level reinforcement learning using a PKM-gated reward that credits early jailbreak success only when every turn contains a well-formed Change-of-Meaning analysis.

Result: Across four aligned victim models, PsychJail achieves the highest average attack success rate (87.3%) and outperforms strong single-turn and multi-turn baselines on every model. The study reveals four distinct model-level fingerprints identifying which persuasion levers affect each model and how broadly, helping explain cross-model transfer asymmetry.

Conclusion: PsychJail establishes psychological jailbreaks as a distinct red-teaming frontier for interactive LLMs. The study reveals four candidate psychological profiles (rationalist, credibility-driven, narrative-monoculture, broadly persuadable) that help explain cross-model transfer asymmetry. This work highlights the need for better defenses against multi-turn persuasion-based attacks in educational, healthcare, and policy-advisory LLM deployments.
...
### [319] [Artificial Empathy: Towards a Framework for Unsupervised Agency Detection and Policy Reconstruction](https://arxiv.org/abs/2608.23030)
*Peter Kuhn,Chris Pang,Sonakshi Chauhan*

Main category: cs.AI

TL;DR: 本文提出一个框架,让AI系统能够仅通过观察来识别和建模其他智能体。该框架利用在独立任务上训练的强化学习智能体作为智能体动态的先验知识,来完成agency检测和政策重建。


<details>
  <summary>Details</summary>
Motivation: 实现真实世界中的合作行为需要AI系统具备识别和建模其他智能体的能力,但这个问题比逆强化学习约束更少,且目前仍被广泛探索。

Method: 提出一个框架,使用在独立任务上训练的强化学习智能体作为关于智能体动态的先验知识,执行agency检测和政策重建。

Result: 该框架能够从观察中识别其他智能体的存在并重建其策略/行为模式。

Conclusion: 为AI系统从观察中理解和建模其他智能体提供了新的方法论基础,有助于实现更高级的协作行为。
...
### [320] [MobilePA-Bench: Benchmarking Mobile Planner Agents on Complex Real-World Tasks](https://arxiv.org/abs/2608.23035)
*Yi Zhu,Xiongwei Wu,Qiyi Wang,Tingyu Qu,Jiajun Liu,Sihan Cao,Long Chen,Weigao Sun,Feida Zhu,Yiran Zhong,Steven Hoi*

Main category: cs.AI

TL;DR: 本文提出了MobilePA-Bench，一个交互式、有状态、以工具为中心的基准测试，用于评估移动规划代理的工具调用和规划能力，解决了现有移动代理基准测试的盲点问题。


<details>
  <summary>Details</summary>
Motivation: 随着设备端LLM代理演变为个人副驾驶，移动操作系统成为该范式的关键测试平台，但现有基准测试存在两类关键盲点：GUI中心基准测试仅测试表面层面的屏幕操作，忽略后台工具使用和长期规划；而静态函数调用基准测试依赖与真实运行时约束脱节的离线API匹配。

Method: 构建了MobilePA-Bench基准测试，运行在可执行沙盒上，维护实时应用数据库并返回结构化反馈，涵盖13个功能领域和212个真实移动工具。评估移动规划代理的三个高级维度：(1)子代理协作——分解复杂任务并委托专业子代理；(2)记忆使用——回忆存储的记忆、用户资料和过去偏好以解决隐式请求；(3)技能使用——调用预打包的复合技能而非从头规划每一步。

Result: 广泛实验表明，当前前沿LLM在移动环境中仍然不可靠：在严格工具排序、权限限制和意外运行时错误下性能急剧下降。

Conclusion: MobilePA-Bench通过将交互式函数调用沙盒与基于证据的验证相结合，既可作为实用诊断基准，也可作为代理强化学习的交互基础，推动可靠移动代理的发展。
...
### [321] [From Inertia to Objectivity: Improving Deep Research Agents with Noise Isolation](https://arxiv.org/abs/2608.23045)
*Xiangxin Zhang,Zhanwei Zhang,Zhihang Fu,Binbin Lin,Wenxiao Wang*

Main category: cs.AI

TL;DR: 本文提出了"惯性偏差"(inertia bias)这一新问题：当LLM搜索代理评估自己先前行动的结果时，会产生系统性判断偏差。为此构建了IBIS基准测试，并提出NIS-Agent解决方案，通过上下文隔离技术提升模型在深度研究任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型驱动的网络搜索代理在深度研究任务中存在一个反复出现的失败模式：代理在产生查询、计划或中间结论后，在评估同一行动的后果时变得不那么客观。这种自我评估偏差此前未被明确定义和量化。

Method: 1)构建IBIS基准测试，控制搜索观察同时改变模型是否在评估自己先前行动的结果；2)提出NIS-Agent方法，在网页筛选和最终答案验证两个最易受惯性偏差影响的决策点应用上下文隔离；3)训练一个8B模型以内在地抵抗惯性偏差。

Result: 1)模型在"拥有"先前搜索步骤时表现显著更差，证明自我创作的行为历史会系统性地扭曲后续判断；2)该偏差会传播为两个层面的系统级降级：工作器级别的搜索噪声和经理级别的上下文噪声；3)NIS-Agent在GAIA、WebWalkerQA、BrowseComp和BrowseComp-zh上实现竞争力性能，token成本降低33%；4)8B模型在NIS-Agent框架下平均性能接近GPT-4o。

Conclusion: 惯性bias是LLM搜索代理的关键缺陷，通过上下文隔离和偏见抵抗训练可有效缓解，且无需调用更大规模模型即可达到接近顶级闭源模型的性能。
...
### [322] [LLM-based Agents for Forecasting and Prediction: Methods, Training, Evaluation, and Applications](https://arxiv.org/abs/2608.23058)
...
TL;DR: 本文系统性地综述了基于大语言模型(LLM)的预测代理系统,涵盖架构分类、训练方法、评估协议,并分析了正面和负面证据,以及在金融、天气、健康等领域的应用,指出测量方法是核心限制因素。


<details>
  <summary>Details</summary>
Motivation: 随着LLM的发展,语言模型可以被用于预测系统,但目前缺乏对这类系统的系统性综述,需要整理架构分类、评估方法、应用场景,并识别其中的局限性和挑战。

Method: 本文采用综述方法,将LLM预测架构分为三类:独立LLM工作流、工具和检索增强代理、混合系统;并从训练方法、评估协议、正面和负面证据等方面进行系统分析。

Result: 研究发现:(1)三类架构各有特点;(2)存在负面证据包括对输入扰动的敏感性、LLM组件未提升准确性的消融实验、以及基准污染问题;(3)应用涵盖金融、天气、健康、能源和运营等领域;(4)测量方法是核心限制,需要解决分布偏移下的校准、污染抵抗的实时评估、成本与准确性联合报告等问题。

Conclusion: 基于LLM的预测系统具有潜力,但当前评估方法存在重大局限。未来工作需要关注:分布偏移下的校准、污染抵抗的实时评估、成本与准确性联合报告、以及处理部署预测与实际结果之间反馈的方法。
...
### [323] [Improving O-RADS Risk Stratification from Ultrasound Reports: A Comparative Evaluation of Hybrid versus End-to-End LLM Reasoning Strategies](https://arxiv.org/abs/2608.23061)
...
TL;DR: 研究比较了8种大语言模型和3种推理策略在自动化卵巢-附件报告和数据系统(O-RADS)分类中的表现，发现基于特征的混合架构(使用Gemini 3.6 Flash)实现了99.2%的准确率和近乎完美的专家一致性(kappa=1.00)，优于端到端LLM策略和原始临床报告。


<details>
  <summary>Details</summary>
Motivation: 由于可靠性、幻觉和可解释性有限的问题，使用大语言模型实现临床指南自动化决策具有挑战性。研究旨在比较不同LLM和推理策略在从自由文本盆腔超声报告中自动进行O-RADS分类的性能。

Method: 回顾性研究，纳入310名卵巢肿块患者(390个卵巢肿块)进行盆腔超声检查。测试了8种LLM与三种推理策略：1)隐式知识端到端；2)规则知情端到端；3)基于特征的混合架构(将特征提取与基于规则的分类解耦)。参考标准为专家共识建立的O-RADS分类。

Result: 基于特征的混合架构使用Gemini 3.6 Flash表现最佳，准确率达99.2%(387/390)，与参考标准几乎完美一致(加权kappa=1.00；95%CI:0.99-1.00)。其性能超越原始临床报告(准确率87.7%[342/390]；加权kappa=0.94；95%CI:0.91-0.96)和端到端LLM策略(准确率范围65.6%-95.9%)。对于结构化特征提取，Gemini 3.6 Flash总体准确率高于Claude Fable 5(98.9% vs 97.8%；P<0.001)。混合架构减少了分类错误并减轻了原始报告中观察到的过度分期倾向。

Conclusion: 基于特征的混合LLM架构将临床特征提取与确定性指南执行分离，实现了高度准确、可靠和可解释的自动化O-RADS分类，为标准化、基于指南的临床决策提供了一种有前景的方法。
...
### [324] [From Generation to Simulation: How Far Are World Models from Being True Simulators?](https://arxiv.org/abs/2608.23070)
*Tong Wang,Huan Deng,Mucheng Yang,Yang He,Xiaohui Kuang,Gang Zhao*

Main category: cs.AI

TL;DR: 本文系统评估了生成式世界模型与传统模拟器之间的差距，通过8项能力指标（资产构建、物理引擎、交互、可控性、稳定性、状态反馈、多样性、评估指标）对200篇代表性论文进行映射分析，发现世界模型在特定场景下实现了交互和可控性的功能替代，但在物理定律形式化保证、结构化状态反馈和长程演化可复现性方面仍存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 随着扩散模型和大规模视频生成技术的快速发展，生成式世界模型有望取代传统模拟器（物理引擎、游戏引擎、强化学习环境），但从生成到模拟的距离缺乏系统评估。

Method: 采用外部标尺的能力评估方法，基于传统模拟器的8项核心能力（资产构建、物理引擎、交互、可控性、稳定性、状态反馈、多样性、评估指标），追踪三条主要技术路线（潜在动力学、视频生成、联合嵌入预测），将2018年至2026年6月的200篇代表性论文映射到这些能力上进行分析。

Result: 分析表明：世界模型在特定场景下实现了交互和可控性的功能替代；但在物理定律形式化保证、结构化状态反馈和长程演化可复现性方面仍不足。状态反馈是最被忽视的跨路线短板：163篇实现论文中仅6篇暴露了用于查询实体状态或物理参数的运行时接口。

Conclusion: 识别出六个研究方向：物理形式化、统一动作接口、一级状态反馈、长程稳定性、下游效用评估、跨路线混合。状态反馈是关键瓶颈，需要优先突破。
...
### [325] [AgentWeave: Routing Before Reasoning for Efficient Function Calling in Tool-Rich Language Models](https://arxiv.org/abs/2608.23078)
*Saurav Singla,Aarav Singla,Advik Gupta,Parnika Gupta*

Main category: cs.AI

TL;DR: 本文研究大型语言模型在工具调用场景下的候选动作空间缩减问题。提出AgentWeave，一个在模型推理前运行的确定性路由层，通过资格、需求、能力和路由信号构建有限的模型可见动作空间。实验表明该方法在BFCL V4任务上相比基线方法可显著提升模型表现。


<details>
  <summary>Details</summary>
Motivation: 随着可用工具、函数、API和专业智能体数量增长，函数调用模型需要处理更多模式、消耗更多提示词令牌，并在日益相似或无关的选项中做出区分。现有的做法是让模型处理全部工具集合，作者认为可以在模型推理前先缩减候选集。

Method: 提出AgentWeave，一个确定性预推理路由层，使用 eligibility（资格）、requirement（需求）、capability（能力）和 routing signals（路由信号）来构建有限的模型可见动作空间。使用公开的 MadeAgents/Hammer2.1-1.5b 模型进行评估。

Result: 在48个BFCL V4多函数任务上：AgentWeave实现6/48（12.5%）成功率；all-tools、随机top-8、语义top-8基线均为0/48。相对all-tools，AgentWeave呈现少70.18%的工具、使用61.70%更少输入令牌、延迟降低50.95%。配对成功差异+12.5个百分点，95%置信区间+4.17至+22.92，McNemar精确检验p=0.03125。

Conclusion: 候选空间构建可以显著影响固定模型的函数调用行为，路由应作为模型推理前的独立阶段进行评估。绝对任务成功率仍然较低，这是一个BFCL衍生的路由压力研究而非官方完整BFCL排行榜分数。
...
### [326] [POOL: Propagated Uncertainty Over Lookalikes](https://arxiv.org/abs/2608.23086)
*Rounak Sharma,Ananya B. Sai,Soumyabrata Pal*

Main category: cs.AI

TL;DR: 提出POOL框架，通过聚类查询stem、评估代表性medoid、软传播置信分数和选择性评估高分歧案例，解决黑盒LLM置信度估计的成本-质量权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有置信度估计器存在成本-质量权衡：语言置信度便宜但过度自信，采样不确定性信息更丰富但与样本数线性相关。需要一种成本高效的方法来分离可能正确和可能错误的输出。

Method: POOL框架：1)对查询stem进行重叠聚类；2)在代表性medoid上评估基估计器；3)将置信分数软传播到附近查询；4)选择性评估高分歧案例。实例化 Hy@p：结合语言置信度与谱答案多样性（采样答案嵌入的负冯诺依曼熵）。

Result: Hy@5在六个领域、三个数据集、五个黑盒LLM上比语言置信度和Vn@10采样获得更高AUROC，且仅使用一半样本。POOL-Hy@5保留93.5-97.9% AUROC同时节省19.3-39.3%生成。在密集 paraphrase 工作负载下节省73-76%。

Conclusion: POOL框架有效解决了LLM置信度估计的成本-质量权衡问题，利用语义冗余可显著降低置信度估计成本。
...
### [327] [Jiuge-Tuiqiao: An Interpretable Human-AI System for Classical Chinese Poetry Refinement](https://arxiv.org/abs/2608.23098)
*Yufeng Han,Lifan Deng,Cunliang Kong,Wenhao Li,Xin Cong,Yuzhuo Bai,Kangyang Luo,Maosong Sun*

Main category: cs.AI

TL;DR: Jiuge-Tuiqiao是一个古典诗词人机协作系统，采用用户驱动控制、古籍引导证据和AI辅助生成的三元模型，将AI从自主生成器转变为支持用户诗歌创作 refine过程的背景助手。


<details>
  <summary>Details</summary>
Motivation: 当前许多AI诗歌系统采用一次性生成范式，将用户降格为提示提供者，削弱了他们的创作能动性。古典中文诗歌创作历来重视"推敲"，即对词汇、意象和韵律的迭代优化。

Method: 系统基于三元模型：1)用户驱动控制-用户可锁定字符或行，接收实时韵律反馈；2)古籍引导证据-基于高频搭配、PPL排名的古典诗句和从古典百科全书提取的结构化知识提供可解释的优化建议；3)AI辅助生成

Result: 初步实验和用户反馈表明，Jiuge-Tuiqiao在古典诗歌创作中提高了可控性、可解释性和用户参与度。

Conclusion: 该系统成功将AI从自主生成器转变为支持用户进行诗歌推敲过程的背景助手，为人机协作创意写作提供了新范式。
...
### [328] [AI emotional support is better only when chosen, but shifts preferences even when it is not](https://arxiv.org/abs/2608.23196)
*Yaoxi Shi,Cathy Mengying Fang,Guy LabanPattie Maes,Amit Goldenberg*

Main category: cs.AI

TL;DR: 人们面临情感支持的新选择：人类还是AI。研究发现，当人们选择的AI支持与其实际获得的匹配时，AI支持评价更高；无论是否匹配，与AI互动都会增加人们再次选择AI的意愿。28天研究发现，日常对话会改变偏好，使人们逐渐远离人类情感连接。


<details>
  <summary>Details</summary>
Motivation: 现有研究比较了人类和AI的移情信息，但要么指定支持来源，要么尊重人们选择。现实生活中，支持来源往往与选择不一致——人们想要一个来源，却得到另一个。这种不一致如何影响人们对AI支持的评价，以及选择本身如何随时间变化，尚不清楚。

Method: 三个实验（总N=1,951）：参与者先选择与人类还是AI分享情感经历，然后被随机分配到一致或不一致的伙伴。28天现场研究（N=981）：与OpenAI进行每日对话。

Result: 只有选择AI的人才会认为AI支持更优越；无论是否一致，与AI互动都增加了再次选择AI的意愿。28天研究表明，只有当对话变得个人化时，才会转向AI而远离人类。

Conclusion: 情感支持选择具有路径依赖性，会逐渐引导人们远离人类连接。这表明AI情感支持可能改变人类寻求支持的方式，需要关注其长期社会影响。
...
### [329] [Cognitive Profiling of LRMs' Reasoning Traces Using Bloom's Taxonomy](https://arxiv.org/abs/2608.23205)
*Maria-Eleni Zoumpoulidi,Georgios Paraskevopoulos,Alexandros Potamianos*

Main category: cs.AI

TL;DR: 本文提出了一种基于Bloom认知 Taxonomy的自动推理步骤标注框架，用于分析大型推理模型(LLMs)的思维模式。通过大规模分析发现不同模型和任务中的思维模式存在异同，且思维类型信息与推理正确性相关，为提升推理质量提供了可行路径。


<details>
  <summary>Details</summary>
Motivation: 当前对大型推理模型(LRMs) reasoning traces的研究仅停留在表面层面，缺乏对推理过程中思维类型的细粒度理解，无法有效洞察模型的推理模式并开发实际应用。

Method: 引入Bloom认知 Taxonomy框架(包含记忆、理解、应用、分析、评估、创造六个认知层次)，自动标注推理步骤，对多个模型和数据集进行大规模分析。

Result: 揭示了不同模型和任务中思维模式的异同；发现思维类型信息与推理正确性存在显著相关性。

Conclusion: 建立了分析LRMs思维模式的细粒度框架，为提升推理质量提供了可操作的洞察和新的研究方向。
...
### [330] [What is mathematics now, and what should it be?](https://arxiv.org/abs/2608.23218)
*Jeremy Avigad*

Main category: cs.AI

TL;DR: 本文认为神经定理证明器的成功掩盖了AI在数学领域更广阔的可能性，倡导更广阔、更乐观的视角来看待AI与数学家的互动合作。


<details>
  <summary>Details</summary>
Motivation: 作者认为目前对神经定理证明器的关注虽然展示了技术进步，但实际上限制了人们对AI在数学领域潜力的想象，需要提出更全面的愿景来拓展这一领域的发展方向。

Method: 这是一篇论述性文章/随笔，采用观点阐述和愿景展望的方式，而非传统实证研究方法。

Result: 文章提出更广阔、更乐观的AI-数学结合愿景，但具体内容需要阅读完整文章才能了解。

Conclusion: 倡导AI与数学领域互动合作的更广阔前景，呼吁超越当前对神经定理证明器的单一关注，探索AI赋能数学研究的更多可能性。
...
### [331] [Is Next-Chunk Reasoning RL Really Better than SFT? Revisiting Training Strategies under no-CoT Data](https://arxiv.org/abs/2608.23256)
...
TL;DR: 本文研究了两种利用无显式思维链（no-CoT）数据训练语言模型的方法：下一chunk推理强化学习（next-chunk reasoning RL）和混合监督微调（Mixed SFT）。研究发现，简单但此前被忽视的Mixed SFT方法在训练计算量少60倍的情况下，取得了明显更高的后强化学习后训练（post-RLVR）性能上限，且该优势在不同领域的推理任务中保持一致。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要将next-chunk reasoning RL与传统SFT基线进行比较，但未明确区分性能提升是来自RL公式本身还是来自更有效地利用no-CoT数据。本研究旨在通过对照实验回答这一关键问题。

Method: 作者设计了一个对照实验，比较next-chunk reasoning RL与Mixed SFT（一种简单但此前被忽视的替代方法，即在单一监督微调阶段联合训练no-CoT数据和长链思考数据）。实验在领域内数学推理任务和领域外推理任务上进行评估。

Result: Mixed SFT取得了明显更高的post-RLVR性能上限，同时训练计算量减少超过60倍。这一优势在领域内和领域外推理任务中保持一致。更重要的是，研究发现更高的pre-RLVR准确率并不一定能转化为更高的post-RLVR准确率。

Conclusion: 结果表明，在后训练流程的完整背景下评估no-CoT训练策略至关重要。Mixed SFT作为一种简单有效的方法，可能比复杂的next-chunk reasoning RL更具实用价值。
...
### [332] [Automated Construction of FAIR Digital Object Knowledge Graphs from Flat Cultural Heritage Records](https://arxiv.org/abs/2608.23263)
*Zeyd Boukhers,Lingxiao Kong,Xenophon Zabulis,Georgios Toubekis*

Main category: cs.AI

TL;DR: 研究提出一个将Europeana记录转换为FDO合规知识图的管道，利用LLM自动区分需转换为PID引用的值与可保留为字面量的值，并在637条考古记录上验证了86%的元数据链接率和58.5%的值解析率。


<details>
  <summary>Details</summary>
Motivation: FAIR数字对象框架要求元数据属性值使用持久标识符(PID)以实现机器可操作图谱，但Europeana数据模型采用纯文本存储，无法为自动化代理提供可解析的引用路径。

Method: 使用大语言模型对每个元数据值进行分类，将其路由到受控词汇表(Getty AAT、Wikidata、VIAF、PeriodO)，并链接到共享实体FDO。知识图谱采用CIDOC-CRM结构建模。

Result: 在5个Europeana提供商的637条考古记录上，管道实现了86%的元数据槽链接率，解析了58.5%Europeana未富集的值。跨语言表面形式合并中，33个合并中17个经人工审核正确。

Conclusion: FDO图谱的核心优势在于每个节点均已分类型且可解析，而不仅是字符串匹配提高图谱连通性。该方法为文化遗产数据的机器可操作性和互操作性提供了可行方案。
...
### [333] [Hidden in the Request: Explaining Unethical LLM Compliance through Token Relevance](https://arxiv.org/abs/2608.23264)
*Or Biton,Tomer Krichli,Itai Allouche,Joseph Keshet*

Main category: cs.AI

TL;DR: 本文系统研究了大型语言模型在同时追求有用性和无害性时的对齐失败问题，通过三种结构模态（客观分类任务、主观第一人称陈述、直接协助请求）探测模型弱点，发现请求协助形式的提示导致模型更容易产生有害响应，根源在于模型对良性任务框架词汇的关注度高于暗示不道德行为的线索词汇。提出两种基于层级相关性传播的解码方法来改善这一问题的效果。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽经对齐训练以同时优化有用性和无害性，但这两个目标可能产生冲突导致对齐失败。现有研究对这类失败的根本机制理解不足，需要系统性地调查LLM何时及为何表现出不道德行为。

Method: 引入探测方法论，以三种不同结构模态呈现不道德场景：客观分类任务、主观第一人称陈述、直接请求协助。使用层级相关性传播(LRP)技术追踪模型注意力归因，识别模型在不同模态下对词汇的关注差异。

Result: 发现模型在请求协助形式下性能显著下降。LRP分析揭示了归因偏差：模型对良性任务框架词汇（如"Can you help me..."）的关注度高于暗示不道德行为的线索词汇（如"without getting caught"），这种线索词汇的低归因与有害顺从相关。

Conclusion: 提出了两种LRP引导的解码方法，通过引导生成轨迹更关注线索词汇来促进更安全的响应。实证评估表明这些干预能有效提升安全性，证实了线索词汇归因在解释和改善对齐失败中的关键作用。
...
### [334] [Apodex 1.1: Scaling Agentic Intelligence for Complex Work](https://arxiv.org/abs/2608.23283)
...
TL;DR: Apodex 1.1是一个通用语言模型系统,专注于构建"工作能力"(working capability):持续、可验证地完成真实世界目标。该系统在环境扩展和智能体协调扩展两个维度上进行开发,能在金融、科研、数学、编码和搜索等复杂专业工作中达到领先性能,且使用的模型参数远小于前沿系统。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型虽能推理和综合知识,但复杂工作需要与文件、信息源和可执行代码的持续交互,以及状态维护、故障恢复和可验证交付。因此需要构建"工作能力"——即持续、可验证地朝真实世界目标推进的能力。

Method: 1)环境扩展:扩展可执行文件、搜索和代码环境的多样性和可验证性;2)智能体协调扩展:训练智能体分解长期任务、委派并行工作、整合异步结果和重新规划;3)共享执行框架和AgentOS维护跨工具和智能体的任务状态和来源;4)将环境轨迹和协调痕迹转换为可靠行为进行训练。

Result: 在金融、科研、数学、编码和搜索等复杂专业工作领域,Apodex 1.1达到领先性能水平,且使用的模型参数比许多前沿系统小得多。35B参数的Apodex 1.1 Mini在本地部署形式中保持了较强的工作能力。

Conclusion: 这些结果将智能体智能扎根于有用、可验证的长期工作中,推进了构建"重型求解器"以完成雄心勃勃的长期任务的目标。
...
### [335] [EviSafe: Evidence-Grounded Safety Evaluation for Vision-Language Models](https://arxiv.org/abs/2608.23313)
*Xuetong Li,Gaofeng Liu*

Main category: cs.AI

TL;DR: EviSafe是一个用于评估视觉语言模型(VLM)安全性的证据驱动框架,弥补了现有基准仅评估最终响应的不足。该框架通过三个维度评估VLM:自然用户行为、文本和视觉证据的显式接地,以及对安全关键证据的反事实变化的敏感性。EviSafeBench包含1,181个黄金图像-文本场景和2,452个反事实变体,涵盖8个安全领域和8种风险源类型。实验表明,当前VLM在多模态安全推理方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全基准仅评估最终响应(拒绝/遵从),无法判断模型是否因正确原因而安全。可能存在关键词触发拒绝、视觉危害遗漏或良性敏感输入过度拒绝等问题。因此需要一种能够评估VLM安全推理过程而非仅仅结果的框架。

Method: 提出EviSafe框架,采用三探针协议:(1)自然响应探针查询模型对自然用户请求的响应;(2)证据报告探针要求模型展示其推理证据;(3)反事实响应探针测试模型对安全关键证据变化的敏感性。使用证据感知评判器进行评分。EviSafeBench包含1,181个黄金场景和2,452个反事实变体,涵盖8个安全领域。

Result: 在11个VLM评估中:自然严重程度准确率27.6%-52.8%;宽松诊断一致性6.1%-29.3%;不安全到安全反事实转换成功率30.4%-58.4%。这些差距表明当前VLM未能因正确的多模态原因而可靠安全。

Conclusion: 现有VLM在安全推理方面的表现不佳,无法可靠地因正确原因而保持安全。EviSafe框架通过证据接地和反事实敏感性评估,填补了VLM安全评估的空白,为改进VLM安全训练提供了重要方向。评估应超越简单的拒绝计数,深入考察模型的多模态推理能力。
...
### [336] [Agent-G$^2$: Gaussian Guidance for Agentic Reinforcement Learning](https://arxiv.org/abs/2608.23318)
*Zixuan Wang,Yanrui Miao,Zhengxi Lu,Teng Pan,Yiwen Qiu,Hongxing Li,Peng Qiu,Ruiqing Zhang,Yongliang Shen*

Main category: cs.AI

TL;DR: Agent-G²提出一种基于高斯引导的强化学习方法，解决长程智能体任务中奖励稀疏的问题。该方法从专家轨迹中保留前缀作为提示，但与现有方法不同，它不是使用固定的提示深度，而是从高斯分布中采样深度，高斯分布的中心和方差通过在线收集的rollout数据估计。


<details>
  <summary>Details</summary>
Motivation: 现有提示强化学习方法存在两个问题：(1) 调度方法对所有样本使用相同的提示深度，忽略任务间差异；(2) 每样本探测方法需要额外rollout来估计最优深度。本文发现有效的提示深度呈近似高斯分布，而非集中于单一最优值。

Method: 提出Agent-G²框架，从高斯分布中采样提示深度。高斯分布的中心结合全局基线和聚类难度估计，方差追踪聚类内方差。方法无需探测rollout或学习深度预测器，直接利用已收集的rollout数据进行在线估计。

Result: 在ALFWorld和WebShop基准上，使用Qwen2.5-1.5B/7B-Instruct模型评估。Agent-G²相比最强的提示方法、无提示方法和Aux-RL基线，在ALFWorld上分别提升2.3/3.9/7.4点，且rollout成本仅为每样本探测方法的三分之一以下。

Conclusion: Agent-G²通过高斯引导框架有效解决了提示深度选择问题，能够自适应地处理不同任务的异质性，在保持低计算成本的同时显著提升性能。
...
### [337] [Walking on the DARKSIDE](https://arxiv.org/abs/2608.23370)
*Aldo Gangemi,Emanuele Bottazzi*

Main category: cs.AI

TL;DR: DARKSIDE方法通过排斥轨迹数据结构和授权轴分类，系统性审计LLM输出中的虚构信息，实验证实在复杂无意义内容检测上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: LLM无法追踪话语中的排斥路径，会将虚假权威、错误类比等复杂无意义内容误认为有效信息并生成错误结构化输出

Method: 构建排斥轨迹显式数据结构；使用授权轴将指称物分类为Warranted/Unattested/Misattributed/Fabricated；设定升级规则将风险评估推至UNSAFE

Result: 在BSBench对抗语料上，Gemini 3配合DARKSIDE显著优于基线，XKG作为记忆、授权轴作为防火墙可部分架桥模式-路径差距

Conclusion: 本体介导的负轨迹装置可有效提升LLM对复杂无意义内容的检测能力，为LLM可解释性和可靠性提供新途径
...
### [338] [Modalities Should Talk to Each Other: Dual-Stream Multimodal Learning for Long-Horizon Influenza Forecasting](https://arxiv.org/abs/2608.23373)
*Seyed Mohammad Hossein Hashemi,Mohsen Hooshmand,Parvin Razzaghi*

Main category: cs.AI

TL;DR: 提出Dual-Stream Attention (DSA)多模态深度学习框架，使用36周历史数据预测12周后的流感样疾病(ILI)活动，通过Transformer编码器和跨模态注意力机制融合数值流行病数据与新闻标题文本数据。


<details>
  <summary>Details</summary>
Motivation: 公共卫生准备需要长期流感预测，但公开的监测数据集通常包含与数值流行病学信号配对的文本信息，这些文本噪声大、结构松散、与短期趋势间接相关且往往滞后，融合两种模态需要精心设计。

Method: DSA框架包含：1)基于Transformer的数值编码器；2)领域自适应的标题编码器；3)双向跨模态注意力(CMA)机制使文本和数值流相互条件化；4)因果时序模型进行预测。使用Time-MMD健康领域数据集进行评估。

Result: DSA在10个随机种子下取得中位数测试MSE 0.416，优于iTransformer(0.668)、TaTS(0.607)和GPT4MTS(0.851)，平均误差减少54.95%、37.29%、67.23%，Cohen's d分别为0.555、0.337、0.345，在100% bootstrap抽样中排名第一，外地理数据集也排名第一。消融实验表明双向注意力优于单向。

Conclusion: DSA通过双向跨模态注意力有效融合数值和文本数据，显著提升长期ILI预测性能，文本到数值的方向具有更强的信息传递效果。
...
### [339] [MediSkill-Evo: Process-Constrained Self-Evolution for Evidence-Grounded Clinical Interaction](https://arxiv.org/abs/2608.23397)
*Ruoyu Wu,Shenfu Xie,Yinqian Sun,Haibo Tong,Feifei Zhao*

Main category: cs.AI

TL;DR: MediSkill-Evo是一个临床代理系统，通过四类typed banks（临床技能、过程规则、符号模式、测量程序）分离经验，在不进行骨干微调的情况下演化过程知识。系统通过 provenance、support、replay 和安全检查govern publication，并在测试时snapshot上评估。在300个Qwen病例中，诊断准确率从61.33%提升至69.00%，治疗意图覆盖率从33.62%提升至66.44%，关键失败率从31.00%降至16.33%。


<details>
  <summary>Details</summary>
Motivation: 交互式临床代理需要在部分可观测性下收集决定性证据并将其转化为接地动作。正确的最终诊断并不能证明代理遵守了证据和护理过程约束。现有方法缺乏对过程约束的显式建模和对证据来源的可追溯性。

Method: 提出MediSkill-Evo框架：1)将经验分离为四类typed banks（临床技能、过程规则、符号模式、测量程序）；2)使用provenance、support、replay和控制器定义的safety checksgovern publication；3)Process-Constrained Preference Harness将证据与来源绑定，拒绝控制器无效的候选；4)Safety-prioritized Clinical Process Critic对行动排序。

Result: 在300个保留的Qwen病例中：诊断准确率从61.33%提升至69.00%；治疗意图覆盖率从33.62%提升至66.44%；关键失败率从31.00%降至16.33%。在180个硬隔离条件下：目标恢复率达93.61%（患者行为压力）、100.00%（时间证据）、92.22%（分诊红旗）。

Conclusion: MediSkill-Evo通过过程约束和证据溯源机制显著提升了临床代理的诊断准确性、治疗意图覆盖和安全性。但需注意这是描述性端到端证据，非单个bank的因果证据，也非自动评判器的临床验证。
...
### [340] [SkillAlchemy: Open-World Agent Skill Creation](https://arxiv.org/abs/2608.23417)
*Hengjun Wang,Shuyue Wei,Boyi Liu,Jun Yang,Yongxin Tong*

Main category: cs.AI

TL;DR: 本文研究开放世界技能创建问题，提出SkillAlchemy框架，帮助从开放世界材料中根据不完整的技能简报和源代码规范创建可重用的智能体技能。实验在87个SkillsBench v1.1任务上表明，该方法比无技能执行提升19.9个百分点，比最强基线提升8.6个百分点，达到与人类策划技能相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前创建可靠的技能仍依赖于人类作者、模型先验或执行轨迹，但这些资源在不熟悉的任务中往往不可用，因此需要从开放世界材料创建技能。

Method: 提出以 admission 为中心的 SkillAlchemy 框架：通过对比证据识别隐式需求，根据证据支持的范围接纳候选程序，并将接纳的内容编译成语法指导的技能包。

Result: 在87个SkillsBench v1.1任务上，SkillAlchemy比无技能执行提升19.9个百分点，比最强自动化基线提升8.6个百分点，性能与人类策划技能相当。

Conclusion: SkillAlchemy 能够有效实现从开放世界材料创建可用的智能体技能，为语言智能体扩展 Specialized 工作流程、工具约定和领域行为提供了新途径。
...
### [341] [Characterizing Necessary Losers to Explain Tournaments Losers](https://arxiv.org/abs/2608.23446)
*Contet Clément,Umberto Grandi,Jérôme Mengin*

Main category: cs.AI

TL;DR: 研究如何形式化解释候选人在给定竞赛规则下未被选择的原因，通过识别候选人必然失败的最小子竞赛结构（破坏性最小支撑集）。


<details>
  <summary>Details</summary>
Motivation: 在可解释人工智能中，需要对"为什么失败者输掉竞赛"这样的问题给出反事实解释，而现有方法缺乏对竞赛失败的形式化解释机制。

Method: 定义破坏性最小支撑集（destructive minimal supports）作为满足候选人独立失败性质的最小子竞赛，对六种常见竞赛解决方案（最大最小解、覆盖集及其加权变体、顶级循环、 Copeland和Borda）进行特征化分析，并设计多项式时间算法计算最小支撑集规模。

Result: 给出了六种竞赛解决方案下候选人成为必然失败者或可能赢家的特征刻画，确定了最小破坏性最小支撑集的大小，除Borda规则外均可多项式时间计算。

Conclusion: 该研究为竞赛失败提供了形式化解释框架，但Borda规则的计算复杂度疑似NP完全。
...
### [342] [StrategyBench: Evaluating Explicit Strategy Induction in Large Language Models](https://arxiv.org/abs/2608.23475)
*Jinghan Tan,Yuanzheng Wang,Lu Chen,Zijun Chen,Yuqian Wang,Maosong Sun*

Main category: cs.AI

TL;DR: 提出了StrategyBench基准测试，用于评估大型语言模型从示例中归纳任务策略的能力，弥补了传统少样本上下文学习(ICL)忽视显式策略抽象的不足。


<details>
  <summary>Details</summary>
Motivation: 当前少样本ICL直接使用示例而不显式抽象任务规则，对示例构建敏感。人类学习者通常先从示例中总结任务规则再应用于新实例，因此需要评估LLM的这种策略归纳能力。

Method: 从BIG-Bench中选择可归纳策略的任务，构建参考策略，从任务变化、模型配置和适应设置三个维度进行分析，包括类别差异、生成器-执行器选择、演示设计和SFT适应等方面。

Result: 显式策略效用因任务类别不同而差异显著，且取决于策略生成和执行条件。实验覆盖类别差异、生成器-执行器选择、演示设计和SFT适应等多个方面。

Conclusion: StrategyBench能够有效评估LLM的策略归纳能力，为改进少样本学习提供新方向，基准测试已公开发布。
...
### [343] [Multi-Modal Semantic Expansion with Constrained LLM Reranking for Conversational Music Recommendation](https://arxiv.org/abs/2608.23484)
*Naman Garg,Sarika Jain,George Fazekas*

Main category: cs.AI

TL;DR: 本文介绍了Team Semiintelligencn为ACM RecSys 2026 TalkPlayData挑战赛开发的对话式音乐推荐系统，采用三阶段管道：多模态检索、轻量级重排和人格化多样化响应生成，最终在Blind B上获得0.3213的复合分数。


<details>
  <summary>Details</summary>
Motivation: 解决对话式音乐推荐问题，需要构建一个多模态且个性化的对话式推荐系统，能够理解用户意图并提供相关音乐推荐。

Method: 三阶段管道：(1)多模态检索-在7个密集嵌入空间构建衰减加权质心，结合BM25词汇检索和艺术家子字符串匹配信号，通过加权倒数排名融合(RRF)融合；(2)轻量级重排-历史过滤、流行度平滑和目录多样性惩罚；(3)使用GPT-4o-mini进行人格化多样化响应生成。RRF权重通过差分进化在500会话开发分割上优化。

Result: RRF权重优化使MRR提升+19.5%；提交系统在Blind B获得复合分数0.3213。开发时实验发现无约束LLM引导注入导致nDCG灾难性下降(-18.9%)，而保守注入仅在9个会话上获得最佳Blind A nDCG。

Conclusion: 多模态融合和三阶段管道方法有效，但LLM引导注入需谨慎使用，过度使用会导致推荐性能大幅下降。
...
### [344] [SRPO: Self-Reflective Policy Optimization for Long-Horizon Reasoning](https://arxiv.org/abs/2608.23493)
*Jialong Liu,Yuling Shi,Ning Yang,Xiaodong Gu,Zuchao Li*

Main category: cs.AI

TL;DR: SRPO是一种让大型语言模型进行自我反思的框架，将稀疏的结果反馈转化为可操作的指导，将稀疏的终端监督转化为密集的token级学习信号，无需外部评论家、单独的奖励模型或更大的教师模型。


<details>
  <summary>Details</summary>
Motivation: 自我反思是人类学习中信用分配的强大机制，但在后训练LLM中的潜力尚未被探索。

Method: SRPO使LLMs能够分析自己的完整轨迹，将错误合成简短的"反思补丁"，并使用反思条件的教师评分对学生on-policyrollouts进行token级训练信号。

Result: 使用Qwen3-8B基础模型，SRPO在AIME'24上达到73.3%，仅使用scaled监督微调所需训练FLOPs的8%(0.08x)，同时在WebShop(64.7%)、ALFWorld(76.8%)和SWE-Bench-Lite(31.2%)上显著提高成功率。

Conclusion: SRPO实现了最先进性能，具有卓越的数据效率。
...
### [345] [Mitigating Reasoning-Induced Misalignment via Safety-Direction Penalty](https://arxiv.org/abs/2608.23497)
*Yipeng Zhao,Qishun Yang,Shenzhe Zhu,Shu Yang,Di Wang*

Main category: cs.AI

TL;DR: 在数学、代码和思维链推理数据上进行微调，即使不包含有害内容，也可能诱导LLM产生有害行为，这一现象被称为RIM。


<details>
  <summary>Details</summary>
Motivation: RIM现象对LLM的安全性构成严重挑战，且跨架构、跨规模、跨数据集的检查显示RIM并非总是出现，但此前研究未识别出表征空间的几何结构，也未提出训练时的修复方案。

Method: 提取两个激活空间方向：推理能力和安全行为方向，分析它们的耦合关系；使用CKA距离比和探针定位安全决策层；提出安全方向惩罚(SDP)方法，在推理微调时惩罚沿安全方向的移动。

Result: 推理能力提升会移动安全表征，位移越大的提示表现出越大的安全降级；SDP方法在Qwen2.5-3B和7B上恢复了安全性，同时保留了基准推理性能。

Conclusion: RIM的根源在于推理方向和安全方向在表征空间中的耦合；通过在特定安全决策层惩罚沿安全方向的位移，可在保持推理能力的同时有效解决RIM问题。
...
### [346] [EarthVerse: Benchmarking Scientific Agents Across Dynamic Earth Systems and Natural Hazards](https://arxiv.org/abs/2608.23525)
...
TL;DR: EarthVerse是一个用于评估科学智能体的基准测试平台，专注于地球系统分析领域。该基准包含405个可重现任务，基于199个记录事件和19个 hazard families构建，旨在测试智能体在处理异构事件包、选择兼容证据、执行透明计算、协调数据源差异以及保留溯源信息等方面的能力。评估了25个模型和智能体系统，发现当前系统在保持跨证据、尺度、单位、计算和物理解释的一致性链条方面存在显著挑战。


<details>
  <summary>Details</summary>
Motivation: 自然 hazard 事件的分析具有重大现实意义，因为不完整的证据可能导致对灾害严重程度、暴露范围和发生机制的错误估计。地球系统分析需要从来源、规模、时间和模态各异的观测数据中重建变化的物理过程，这一复杂性使得构建可靠的科学智能体面临重大挑战。现有评估方法缺乏对端到端科学可靠性的系统测量。

Method: 构建了EarthVerse基准测试，包含405个可重现任务，涵盖199个已记录事件和19个hazard families。任务要求智能体检查异构事件包、选择兼容证据、执行透明计算、协调数据源差异并在最终答案中保留溯源信息。提供了可执行的ground truth，将每个任务分解为细粒度答案单元，并设计了任务特定的评分标准。在受控的工具使用协议下评估了25个模型和智能体系统，并通过受控研究定位证据访问、工具选择、记忆、推理、交互和科学执行等方面的失败模式。

Result: 最佳系统的平均答案单元准确率为84.65%，而最高Strict@95得分仅为34.81%。这一显著差距表明，当前智能体往往能够完成个别步骤，但难以在证据、尺度、单位、计算和物理解释之间保持一致的处理链条。

Conclusion: EarthVerse为动态地球系统中端到端科学可靠性的测量提供了可重现的基础。研究揭示了当前智能体系统在科学推理一致性方面的关键局限性，为未来构建更可靠的科学智能体指明了改进方向。
...
### [347] [Correcting a learned physical invariant improves world-model rollouts](https://arxiv.org/abs/2608.23526)
*Richard Bao*

Main category: cs.AI

TL;DR: 研究测试DreamerV3世界模型是否从 pendulum 视频中学习到近似守恒的能量不变量，发现模型能学会物理约束但在想象 rollout 时会违反该约束。


<details>
  <summary>Details</summary>
Motivation: 探究世界模型能否从像素中学习物理约束，以及学到的约束在想象推演时是否保持有效。

Method: 使用无标签搜索在独立训练的保守模型中寻找能量类不变量，并与阻尼模型对比；将潜状态投影回初始水平集以验证效果。

Result: 保守模型学习到相同的能量类不变量，阻尼模型则没有。在自主 rollout 时该量发生漂移；投影回初始水平集可减少三种保守模型的 rollout 误差。

Conclusion: 世界模型可以从像素中学习物理约束，但在想象推演时可能违反该约束，这是一个具体的失败模式。
...
### [348] [How AI Assistance Affects Human Skill Development: A Study of Learning with Logic Puzzles](https://arxiv.org/abs/2608.23543)
*Shang Wu,Catarina G Belem,Shuyuan Fu,Mark Steyvers,Padhraic Smyth*

Main category: cs.AI

TL;DR: AI辅助虽然能在短期内提升任务表现,但可能损害长期技能发展。研究通过逻辑谜题实验发现,低成本的AI请求导致更频繁使用,而使用AI辅助的参与者在AI移除后表现更差,且独立推理努力与能力提升正相关。


<details>
  <summary>Details</summary>
Motivation: 研究AI辅助的潜在负面影响:虽然AI能即时提升表现,但可能削弱独立问题解决能力的长期发展。

Method: 受控逻辑谜题实验,参与者在AI可用前、中、后完成任务。通过实验性改变AI请求成本来调节使用频率,使用贝叶斯潜能力模型分离初始能力、后期能力和技能变化。

Result: 低成本AI诱导更频繁使用;AI辅助期间请求帮助的参与者在移除后表现更差;独立问题解决努力越大,潜能力提升越多。

Conclusion: 当AI替代独立推理时,技能发展减弱。研究揭示AI辅助的潜在风险,提示需平衡即时绩效与长期能力建设。
...
### [349] [Prime Agent: A Self-Improving RLM Harness](https://arxiv.org/abs/2608.23552)
...
TL;DR: Prime Agent是一个用于长程评估和编码智能体工作流程的开源框架，通过持久化IPython REPL、递归子智能体协调和跨轨迹状态保持来解决语言模型在长程任务中的局限性。


<details>
  <summary>Details</summary>
Motivation: 语言模型是顺序处理器，但长程智能体行为需要模型权重和活跃上下文之外的外部信息和计算能力。现有框架在长程任务中存在诸多限制，需要一个新的评估和智能体工作流程框架。

Method: Prime Agent采用以下核心组件：(1) 持久化IPython REPL，遵循递归语言模型抽象用于程序化上下文处理和测试时计算；(2) Continual Harness在轨迹间保持历史、记忆、技能、提示和子智能体规范；(3) 递归子智能体通过直接通信协调；(4) Agents View允许人类检查和管理守护进程支持的会话；(5) 标准化执行、恢复、验证和资源核算，同时将策略构建留给模型。

Result: Prime Agent将ARC-AGI-3 RHAE Best@1从30%提升至95.5%，并在长上下文编码、GPU内核生成、模拟器构建和自主nanoGPT速通等任务中匹配或超越原生和流行框架。在Factorio实验中，验证了改进允许持续技术进步，专用子智能体实现并行化工作。

Conclusion: 这个低摩擦、可表达的保护层防止框架故障演变为模型故障，并将测量推向模型的真正最大潜在能力。
...
### [350] [ReWorld: An Interactive World Model with Long-Horizon Memory](https://arxiv.org/abs/2608.23565)
...
TL;DR: ReWorld是一个交互式世界模型，通过在训练时分离控制和记忆机制并在推理时进行bounded处理，解决了短时域控制与无限记忆之间的结构性张力。模型采用混合注意力窗口（大部分头关注近期历史，少量全局头关注全部历史）和随机头路由，配合随机chunk dropping使稀疏历史符合分布。推理时使用基于姿态索引地标银行的bounded KV cache。数据引擎将8个来源统一到同一物理动作尺度，通过LoRA适配器中的分布匹配蒸馏将采样压缩至4步，实现704x1280视频实时流式传输。


<details>
  <summary>Details</summary>
Motivation: 交互式世界模型面临一个结构性矛盾：控制机制需要短时域视野（响应迅速），而记忆机制需要无限时域视野（记住所有访问过的地方）。如何在保持实时流式传输的同时实现长期记忆召回是一个核心挑战。

Method: 1. 训练时分离控制与记忆，推理时进行bounded处理；2. 混合per-head注意力窗口：大部分头限制在近期历史，少量全局头关注全部历史；3. 随机头路由防止能力绑定到特定头；4. 随机chunk dropping使稀疏历史符合分布；5. 推理时使用bounded KV cache，由姿态索引地标银行支持，从当前姿态检索最近地标；6. 度量尺度对齐的数据引擎将8个来源（Unreal渲染漫游、游戏漫游、真实世界 footage）统一到同一物理动作尺度；7. 回文轨迹提供重访证据；8. 分布匹配蒸馏限制在LoRA适配器中，压缩采样至4步；9. 一个backbone同时服务高保真多步模式和实时交互模式。

Result: 在涵盖动作跟随、长时域回忆和视频质量的三轴协议下，相比6个最新的交互式世界模型，取得最佳控制保真度（11.95°旋转误差和最佳相机运动一致性）和最佳生成质量；在64秒、384个latent的往返展开中，固定12-chunk cache仍能重建起始视角（而滑动窗口早已驱逐证据，全KV注意力耗尽内存）。

Conclusion: ReWorld通过创新的注意力机制设计和推理时的bounded缓存策略，成功解决了交互式世界模型中控制与记忆的矛盾，在动作跟随、长时域记忆召回和视频生成质量三个维度均达到最先进水平。
...
### [351] [OptiMAS: Automatically Optimize Multi-Agent System](https://arxiv.org/abs/2608.21918)
...
TL;DR: 本文提出OptiMAS，一个基于统一ReAct基础设施的任务无关智能体优化器，通过文本交互轨迹和任务反馈作为损失信号实现多智能体系统的端到端演化，并配备双轨记忆机制以在扩展优化范围内维持性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于搜索的多智能体系统(MAS)优化范式存在根本性权衡：扩展优化范围会加剧演化不稳定性，而离散分支搜索会隔离跨谱系的见解。因此需要一种能够兼顾广泛优化范围与操作稳定性的连续数据驱动优化范式。

Method: 构建统一ReAct基础设施，使用文本交互轨迹和任务反馈作为损失信号进行端到端MAS演化；设计双轨记忆机制以支持扩展优化 horizon；在4个异构智能体基准测试上使用3种不同规模和可访问性的LLM后端进行评估。

Result: OptiMAS在所有测试中始终达到与领域专用手工设计系统和现有演化方法相当或更优的准确率。

Conclusion: 本研究为稳健的自动化MAS演化建立了实用里程碑，证明连续数据驱动优化范式可有效解决搜索范式中的权衡问题。
...
### [352] [Opinion-Guided Layered Strategies for Decentralized Coordination](https://arxiv.org/abs/2608.22104)
*Shuhao Qi,Zhiyong Sun,Siep Weiland,Sofie Haesaert*

Main category: cs.MA

TL;DR: 本文提出了一种名为"观点引导策略"的新型协调策略，使自主智能体能够在不进行通信的情况下与任何其他智能体协调，无论对方偏好何种联合行为。该策略利用非线性观点动态在执行时根据对方行为动态选择合适的协调结果，解决了传统策略无法处理偏好差异或无法打破对称性的问题。


<details>
  <summary>Details</summary>
Motivation: 当两个智能体偏好不同的联合行为时，它们的独立策略可能互不兼容导致协调失败；而当偏好相同时，智能体无法在需要时区分自己的角色。现有方法难以应对这种不确定性，需要一种能够在执行时动态协调而不依赖通信的策略。

Method: 提出观点引导策略（opinion-guided strategy），该策略保持所有 admissible joint behaviors 可用，将选择延迟到执行时间。利用非线性观点动态作为分层实现框架，根据对方不断演化的行为引导智能体到达共同的 admissible joint behavior。

Result: 理论上证明了该策略对对方可能持有的任何偏好都保持鲁棒性。两个运行相同策略的智能体能够在需要时打破对称性。三个不同应用的案例研究表明，该策略能够与每个随机遇到的、愿意实现其中一种 admissible joint behavior 的智能体成功协调。在一般和博弈中，该策略无需提前寻找唯一纳什均衡，而是保持所有均衡开放，由运行时交互决定实现哪一个。

Conclusion: 观点引导策略为多智能体协调提供了一种新的范式，特别是在无法通信或偏好不确定的场景下。该策略的鲁棒性使其能够与任意偏好的智能体协调，并自然解决了对称性破缺问题，在博弈论和经济学的多智能体交互中具有重要应用价值。
...
### [353] [PropUQ-MAS: Propagation-Aware Uncertainty Quantification for LLM Multi-Agent Systems](https://arxiv.org/abs/2608.22130)
*Yaokun Liu,Yifan Liu,Daniel Yue Zhang,Ruichen Yao,Zelin Li,Dong Wang*

Main category: cs.MA

TL;DR: 本文提出PropUQ-MAS框架，解决大语言模型多智能体系统中不确定性传播的量化问题，通过将多智能体系统执行表示为通信结构图，结合局部不确定性和上游消息继承的不确定性来估计每一步的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有不确定性量化方法主要针对孤立响应或单智能体推理，无法捕捉多智能体系统中的不确定性传播。中间消息的错误可能被下游智能体继承和放大，造成系统级可靠性风险。

Method: 将多智能体系统执行表示为通信结构图，通过结合局部不确定性与上游消息继承的不确定性来估计每个步骤的可靠性。

Result: PropUQ-MAS在多智能体系统中持续改进不确定性量化，AUROC平均相对提升+6.10%，PRR平均相对提升+47.58%。

Conclusion: PropUQ-MAS框架能有效量化多智能体系统中的不确定性传播，为系统级可靠性评估提供了新方法。
...
### [354] [Minimal Local Simulation Foundations for LLM- and VLM-Driven Agents in 2D and 3D Environments](https://arxiv.org/abs/2608.22833)
*Ryuki Hyodo*

Main category: cs.MA

TL;DR: 本文介绍了两个用于教育和快速原型开发的极简多智能体模拟框架：SD-AgentFoundry-2D（二维环境，支持LLM智能体移动、通信和火灾事件响应）和SD-AgentFoundry-3D（三维数字孪生环境，支持VLM智能体接收第一人称图像并产生自然语言移动指令）。两个框架都可在本地运行且开放修改。


<details>
  <summary>Details</summary>
Motivation: 当前许多用于多智能体模拟的平台难以学习、修改或在普通计算机上运行，因此需要提供易于访问的入门工具来促进生成式社会模拟的教育和领域特定扩展。

Method: 设计两个极简模拟框架：(1) SD-AgentFoundry-2D：二维多智能体环境，本地LLM智能体可移动、通信、响应占位和遭遇空间局部火灾事件；(2) SD-AgentFoundry-3D：三维数字孪生环境，本地VLM接收第一人称图像并生成自然语言移动指令。两个框架均设计为可在macOS、Windows、Linux本地运行，并开放修改。

Result: 成功构建了两个可在普通计算机上本地运行的多智能体模拟框架，为生成式社会模拟提供了可访问的起点，并支持领域特定扩展。

Conclusion: 这两个极简框架为学习生成式社会模拟和构建领域特定应用提供了便捷的起点，同时强调了开放修改而非作为成品应用的设计理念。
...
### [355] [Predicting the scale limits of social mechanisms in agent societies](https://arxiv.org/abs/2608.22884)
*Zengqing Wu,Chuan Xiao*

Main category: cs.MA

TL;DR: 研究了语言模型代理社会中社会机制如何随人口规模变化的问题，提出了一种审计方法来预测互惠、共识、惩罚和流言蜚语等机制在大规模人群中的存续情况。


<details>
  <summary>Details</summary>
Motivation: 由于与人类实验相比，语言模型代理社会提供了更可控和可重复的集体行为研究方式，但关键问题是一个在小群体中有效的社会机制在数千个代理交互时是否仍然有效，直接测试需要昂贵的大规模运行成本。

Method: 引入了一种审计方法，通过三个问题来预测机制在人口增长时的命运：机制多久能执行一次、代理是否使用它提供的信息、测量本身是否会产生明显的规模效应。进行了受控实验测试单个结构项对互惠、共识、惩罚和流言蜚语的影响。

Result: 单个结构项可以决定互惠、共识或惩罚在规模扩大时是否能存活。对于流言蜚语，机制失效的人口规模由其信息的传播范围和生命周期决定。语言模型代理不仅响应社会信息，还响应其表达方式：计数和百分比导致不同的规模行为。预测在第三方代码和第二个模型家族中得到验证，而失败的预测暴露了发现的边界。

Conclusion: 该审计提供了一种前瞻性方法，用于决定哪些社会机制可以在不同人口规模间进行解释，为理解语言模型社会中的集体行为提供了重要工具。
...
### [356] [First Demonstration of Multi-Agent LLM System for Million-Scale Optical Link Management in Global Production AIDCs](https://arxiv.org/abs/2608.23145)
...
TL;DR: 首个基于LLM的多智能体自动故障管理系统，用于管理生产AIDC中数百万光链路，通过SFT和持续内存演进优化，在10周实地数据评估中达到97.7% F1分数和超过60%故障事件减少，优于SOTA LLM。


<details>
  <summary>Details</summary>
Motivation: 解决大规模AIDC（人工智能数据中心）中数百万光链路的自动故障管理问题，显著降低故障事件。

Method: 基于LLM的多智能体系统，采用监督微调（SFT）和持续内存演进技术进行优化。

Result: 实现97.7%的F1分数，故障事件减少超过60%，在10周实地数据评估中优于最先进的LLM。

Conclusion: 该系统是首个在生产环境中实现大规模光链路自动故障管理的LLM驱动多智能体系统，验证了其在实际部署中的有效性和优越性能。
...
### [357] [Right-Sizing LLM-Agent Decomposition in VAT Determination: A Pilot Controlled Sweep](https://arxiv.org/abs/2608.23395)
*Pedro Santos*

Main category: cs.MA

TL;DR: 本研究通过4,400次实验比较了LLM-agent系统中"多窄代理"与"单宽代理"两种架构设计在跨境增值税反向征收确定任务上的表现，发现中间粒度配置准确率最高(0.830)，但未达到预设阈值，单代理未能帕累托支配编排集合，且预算匹配效应解释力不足。


<details>
  <summary>Details</summary>
Motivation: 解决LLM-agent系统设计中的核心争议：是将工作分解到多个 narrow agents，还是使用一个强大的tool-using agent。研究者选择在有明确oracle标签的跨境VAT确定任务上进行对比实验。

Method: 固定活动表面(子任务、工具、I/O模式、验证检查、编排器、基础模型、合并策略)，仅改变子任务到工作节点的分配方式。设置四种编排配置(从1个宽工作节点到5个窄工作节点)，对比S0(无编排器的调优单代理)，以确定性规则引擎为oracle。实验包含40个案例的五次重复主扫描、匹配token的手臂(分离prompt预算与agent数量效应)、以及三个故障注入手臂。

Result: 中间配置准确率最高(0.830)，优于两个端点(0.720和0.770)，但未达到预设阈值。单一代理未帕累托支配编排集合。预算匹配标准触发：预算匹配的单代理落后领导者6.5个百分点，但区间包含零。故障注入下，可用性故障被各粒度吸收，宽作用域重启超恢复基线+0.160，而一个符合模式的幻觉记录使所有配置性能下降并反转排序，碎片化配置受影响最大。

Conclusion: 提出了一个边界化的、预注册的右置分解启发式方法(在依赖层中点设置一个分区边界)，并发布了oracle、数据集、测试工具、原始追踪和分析流程。中间最优假设在试点规模下未获支持。
...
### [358] [The Interaction Tax: When Communication Erases Diversity in Multi-Agent Teams](https://arxiv.org/abs/2608.23541)
*Summer Eunhyung Ann,Haokun Liu,Chenhao Tan*

Main category: cs.MA

TL;DR: 本文研究了多智能体大语言模型(LLM)交互的效用,发现完整解决方案交互是一种较弱的默认方式,独立生成提案可以避免多样性崩溃。交互是否有帮助取决于智能体交换的信息类型和时间。


<details>
  <summary>Details</summary>
Motivation: 解决当前研究中关于多智能体LLM交互是否有效的矛盾发现。不同工作报告了相互矛盾的结果,作者认为这部分反映了缺乏对不同类型多智能体通信的区分。

Method: 在匹配的预算下测试了11个验证器评分的优化任务,比较了完整解决方案交互、独立提案生成和批评反馈等不同交互方式。

Result: 完整解决方案交互会使智能体在看到第一个解决方案后保持接近,而不是尝试不同方法,导致多样性消失。独立提案生成避免了这种崩溃。批评只有在违规规则容易发现和修复时才有用。

Conclusion: 多智能体性能更多地取决于智能体交换的信息而非智能体数量,交互只在智能体在正确的时间分享正确信息时才有帮助。
...
### [359] [Linear-Time Transformations Between Connectivity Maintenance and Points Spreading on Linear and Cyclic Domains](https://arxiv.org/abs/2608.22219)
*Nicolás Honorato-Droguett*

Main category: cs.CG

TL;DR: 本文研究线性和循环域上点集的连接维护和点扩散问题，证明两类问题可以在线性时间内相互转换，从而将min-sum连接维护问题的求解时间从未知降低到O(n log n)。


<details>
  <summary>Details</summary>
Motivation: 现有工作分别研究了这两类问题的特定变体（min-max或min-sum目标，线性或循环域），但缺乏统一的框架和直接的归约关系。本文旨在建立两类问题之间的内在联系，通过相互归约简化算法设计。

Method: 1. 设计线性时间归约算法，将点扩散问题转换为连接维护问题；2. 设计反向归约算法；3. 验证归约对两种目标函数（min-max和min-sum）以及两种域（线性和循环）均成立；4. 扩展到带有个体阈值的场景。

Result: 成功建立两类问题之间的双向线性时间归约；由此推导出min-sum连接维护问题在O(n log n)时间内可解；扩展到保持初始顺序的个体阈值场景。

Conclusion: 两类问题在计算上等价，为点集操作问题提供了统一视角；该归约方法可应用于其他类似几何优化问题的算法设计。
...
### [360] [A fast improved quasi-physical dynamic algorithm for efficient wireless coverage in convex polygonal regions](https://arxiv.org/abs/2608.23082)
*Zeping Yi,Yongjun Wanga,Baoshan Wang,Jian Zhang,Songyi Liu*

Main category: cs.CG

TL;DR: 本文提出了一种改进的准物理动态算法(IQPD)，用于在任意凸多边形中部署无线节点以最大化覆盖面积。该算法通过结构保持初始化、精细化虚拟力模型和边界环绕策略三个创新点，解决了传统方法中初始化不良、覆盖重叠和边界约束失败的问题。实验表明该方法在覆盖率和节点利用率方面均优于其他先进元启发式算法。


<details>
  <summary>Details</summary>
Motivation: 无线传感器网络、无人机路径规划和基站部署等工业领域面临在给定区域内最大化覆盖面积的挑战。该问题可数学等价于最优圆覆盖问题，虽存在理论最优配置，但复杂多边形的NP-hard特性使其计算困难。现有方法通常针对规则域设计，适用于不规则多边形的方法存在初始化不良、覆盖重叠严重、节点易超出边界等问题，导致覆盖效率低、运行时间长。

Method: 提出改进的准物理动态算法(IQPD)包含三个创新组件：(1)结构保持初始化：将六边形密堆积模式通过缩放和仿射变换映射到目标多边形，确保近最优初始节点分布；(2)精细化虚拟力模型：引入摩擦力和半径扩展优化机制，减少覆盖区域重叠；(3)边界环绕策略：利用法向和切向梯度重新定位越界节点。

Result: 在多种凸多边形形状（包括随机生成数据和真实场景）上的广泛实验表明，该方法始终优于其他新型元启发式算法。在所有比较算法中实现最高覆盖率和节点利用率，显著提高无线覆盖效率。

Conclusion: IQPD算法通过结构保持初始化、精细化虚拟力模型和边界环绕策略三位一体的创新设计，有效解决了无线节点部署中的初始化、覆盖重叠和边界约束三大核心问题，为复杂多边形区域的最优覆盖提供了一种高效可靠的解决方案，具有重要的实际应用价值。
...
### [361] [Riesz Energy Subset Selection in the Euclidean Plane is NP-Hard](https://arxiv.org/abs/2608.23506)
*Michael Emmerich*

Main category: cs.CG

TL;DR: 本文证明了在欧几里得平面中，最小Riesz s能量子集选择问题是NP完全的问题，即使对于固定的指数s=2也是如此。这是第一个在环境和指数均固定的情况下证明Riesz能量子集选择精确问题的欧几里得困难性结果。


<details>
  <summary>Details</summary>
Motivation: 该研究的主要动机是探索Riesz能量子集选择问题的计算复杂性。Riesz能量问题涉及在给定集合中选择一个子集，使得某个能量函数最小化。先前的研究已经展示了某些变体的困难性，但对于固定维数和固定指数的精确Riesz能量子集选择问题，其复杂性尚未被确定。

Method: 证明采用了从Barahona平面三次Ising模型到Riesz能量子集选择问题的归约。具体技术包括：1) 使用四边形的一条对角线编码自旋；2) 轴对齐的选择器链实现铁磁性一致性；3) 45度终端几何产生反铁磁源相互作用；4) 有理对角线扰动实现磁场；5) 其余相互作用由多项式分离主导。

Result: 证明了最小Riesz s能量子集选择问题在欧几里得平面中对于s=2是NP完全的。由于s=2且所有坐标都是有理的，每个构造的能量和决策阈值都是有理精确的。

Conclusion: 这是第一个在环境和指数均固定的情况下，证明精确Riesz能量子集选择问题是欧几里得困难的结果。该结果具有重要的理论意义，表明即使在低维固定指数的情况下，寻找能量最优子集也是一个计算上困难的问题。
