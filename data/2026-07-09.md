...
TL;DR: CanvasCraft是一个大规模多模态工具使用数据集，包含14万条带完整注释的可执行轨迹和1万条强化学习任务规范。CanvasAgent是一个工具增强的多模态代理，通过多轮交互协调异构视觉工具。代理首先通过SFT学习可执行推理-动作轨迹，然后使用结合结果级和过程级信号的混合奖励进行GRPO优化。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态工具使用代理主要针对感知、搜索或特定领域编辑进行优化，缺乏大规模的可执行图像创建轨迹监督。用户请求通常涉及合成图像、定位对象、分割区域、编辑选定内容、合成中间资产、读取文本和增强最终结果等复杂任务，需要从感知增强推理转向以操作为中心的视觉创建。

Method: 构建了大规模多模态工具使用数据集CanvasCraft，包含14万条带注释的可执行轨迹和1万条强化学习任务规范。提出CanvasAgent代理：(1)使用SFT学习可执行推理-动作轨迹；(2)使用结合结果级和过程级信号的混合奖励进行GRPO优化；(3)在部署时检查中间结果、跟踪视觉资产、根据视觉状态调整工具决策。

Result: 实验评估了最终图像质量和轨迹行为，证明了CanvasAgent和提出的数据集对复杂多工具图像创建工作流的有效性。

Conclusion: 本文提出的CanvasCraft数据集和CanvasAgent代理能够有效支持复杂多工具图像创建工作流，为多模态代理从感知向视觉创建的任务转移提供了新的研究方向和数据基础。
...
### [2] [Benchmarking the Robustness of Autonomous Driving to Environmental Illusions: A Lane Perception Perspective](https://arxiv.org/abs/2607.05783)
*Tianyuan Zhang,Xianglong Liu,Aishan Liu,Lu Wang,Yitong Zhang,Peng Yue,Mingchuan Zhang,Siyuan Liang,Dacheng Tao*

Main category: cs.CV

TL;DR: 首个评估环境幻觉对自动驾驶车道感知影响的开源benchmark LanEvil++，涵盖14种幻觉类型、94个高保真3D场景、90,292张标注图像、1,596个视频片段和41,855个视觉问答对，并提出MIDA防御方法提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 环境幻觉（阴影、反光、轮胎痕迹）在真实驾驶环境中普遍存在但被忽视，会干扰视觉感知导致场景误判，对自动驾驶系统造成严重安全隐患，而现有研究对此关注不足。

Method: 构建LanEval++ benchmark，使用CARLA模拟器生成94个高保真3D场景；评估传统车道检测(LD)和视觉语言模型(ADVLMs)在14种环境幻觉下的性能；提出多模态幻觉防御方法(MIDA)增强鲁棒性。

Result: 环境幻觉显著降低SOTA车道检测性能：LD模型平均准确率下降5.27%，F1分数下降10.49%；ADVLMs的GPT分数下降2.03%，语言分数下降0.75%。阴影是最具破坏性的因素，准确率最高下降7.20%。闭环模拟显示幻觉导致错误驾驶决策。MIDA使LD模型鲁棒性提升4.23%，ADVLMs提升3.82%。

Conclusion: 环境幻觉对自动驾驶车道感知构成重大安全威胁，现有SOTA模型在该条件下性能显著下降。LanEvil++ benchmark为评估幻觉鲁棒性提供了首个标准化平台，MIDA方法有效提升了模型在幻觉条件下的鲁棒性。
...
### [3] [Unlearnable Faces: Privacy Protection Surviving Extraction Pipeline](https://arxiv.org/abs/2607.05996)
*Byunghoon Oh,Sunghwan Park,Jaewoo Lee*

Main category: cs.CV

TL;DR: LPID是一种基于不可学习示例的人脸保护方法，通过将人脸提取过程内置于优化目标中，解决现有方法在面对裁剪+缩放攻击时失效的问题。该方法对人脸提取过程进行建模，将扰动能量集中在提取过程保留的频带中，能够保护未见过的身份。实验表明，LPID在所有设置下均达到最低攻击准确率，在对未见身份进行裁剪+缩放提取时将攻击准确率控制在10%以下，同时保持32.7 dB PSNR和0.161 LPIPS的不可感知性。


<details>
  <summary>Details</summary>
Motivation: 当前不可学习示例方法在保护照片免受未授权人脸识别模型学习时存在根本缺陷：攻击者可以对提取后的人脸（经过裁剪和缩放）进行训练，在此提取过程中保护失效。这使得公开分享的照片仍然面临被滥用训练的风险。

Method: LPID的核心创新是将人脸提取过程整合到不可学习示例的优化目标中。具体包括：(1)构建人脸提取的可微分模型；(2)将扰动限制在提取的人脸区域内；(3)通过该可微分模型优化扰动；(4)将扰动能量集中在提取过程保留的频带中；(5)按相册重新优化以适应不同保护需求。

Result: LPID在所有评估设置中均取得最低攻击准确率。具体成果包括：(1)在针对未见身份进行裁剪+缩放提取时，攻击准确率保持在10%以下；(2)不可感知性指标达到32.7 dB PSNR和0.161 LPIPS；(3)能够保护训练时未见过的用户身份。

Conclusion: LPID通过将人脸提取过程建模并整合到不可学习示例优化中，成功解决了现有方法在提取攻击下失效的问题。由于这种鲁棒性是转换过程的固有属性而非依赖于任何特定身份，LPID可以保护训练时未见过的用户，具有重要的实际应用价值。
...
### [4] [OBBSeg: Irregular Lesion Segmentation under Oriented Bounding Box Annotations](https://arxiv.org/abs/2607.06007)
*Jun Wei,Xinchang Liu,Yu Liu,Chuhua Yang,Shuhui Wang,Hui Huang*

Main category: cs.CV

TL;DR: OBBSeg是一种基于定向边界框（OBB）的中间监督范式，用于医学图像分割。该方法通过OBB提供紧凑的几何监督，更好地适应细长或各向异性的病变区域，并引入Mask-to-OBB损失来缓解OBB的矩形偏差，同时结合PAFE和DBFE模块增强前景表示并抑制背景干扰。


<details>
  <summary>Details</summary>
Motivation: 像素级标注仍是医学图像分割的主要瓶颈，弱监督虽然有吸引力但缺乏充分约束。现有方法在完全监督和弱监督之间存在较大性能差距，需要一种中间监督范式来弥补这一空白。OBB能同时编码空间范围和方向，比粗略的边界框注释更能适应细长或各向异性病变的形态特点。

Method: 1.提出OBBSeg框架，利用定向边界框作为中间监督信号；2.设计Mask-to-OBB损失函数，实现预测掩膜与OBB区域之间的几何一致性；3.引入提示驱动的语义引导模块PAFE和DBFE，分别增强前景表示和抑制背景干扰；4.在13个数据集、5种成像模态上开展广泛实验。

Result: OBBSeg不仅优于现有弱监督方法，还达到了与全监督方法相当的性能表现，证明了其在高效、可扩展医学图像分割方面的潜力。

Conclusion: OBBSeg成功实现了介于完全监督和弱监督之间的中间监督范式，为医学图像分割中的标注效率问题提供了有效解决方案，具有重要的临床应用价值。
...
### [5] [Structured Data Extraction from Real Estate Documents using Clustering, Classification, and Large Language Models](https://arxiv.org/abs/2607.06012)
*Muhammad Assad Shehbaz,Carlos Francisco Moreno-García*

Main category: cs.CV

TL;DR: 本文提出了一种端到端管道，用于从异构的房产问卷文档中提取结构化数据。该管道使用DeepSeek R1大语言模型从3965份问卷文档中提取35个预定义的房产属性，成功处理了2781份文档，最终生成2766条唯一的房产记录。验证结果显示Jaccard一致性分数为0.82，轮廓系数为0.2088，表明大规模提取方案可行且可靠。


<details>
  <summary>Details</summary>
Motivation: 房产挂牌信息虽通过API暴露结构化元数据，但最丰富的房产级别信息（如法律状态、结构状况、配套设施、供暖系统）存在于附件问卷文档中，目前尚无自动化系统能够大规模处理这些文档。这些文档具有异构性：包括可选择文本的数字文档、扫描的纸质表单，以及包含复选框注释的复杂布局文档。

Method: 构建端到端管道获取、分类和提取结构化数据。通过逆向工程REST API从实时房产平台收集3965份问卷文档。首先将每份文档分类为三种结构类别之一（纯文本、扫描件、特殊字符），然后使用DeepSeek R1大语言模型从符合条件的文档中提取35个预定义房产属性，提示其返回结构化JSON对象。共成功处理2781份提交文档，生成2766条唯一房产记录。

Result: 成功处理2781份提交文档，生成2766条唯一房产记录。余弦相似度匹配实现Jaccard一致性分数为0.82，K-Means聚类产生可解释的市场细分，轮廓系数为0.2088。下游验证确认了数据质量。

Conclusion: 结果表明，在该规模下从房产文档中进行提取是可行且可靠的。提出的方法能够有效处理异构房产问卷文档，为房产数据的自动化提取提供了可扩展的解决方案。
...
### [6] [KOAL: Knowledge-Driven Prostate Cancer Grading with Ordinal-Aware Learning](https://arxiv.org/abs/2607.06019)
*Zheng Guo,Jiaqi Cui,Haocheng Xiong,Jize Han,Bo Liu,Qianwen Zhang,Rui Chen,Yan Wang*

Main category: cs.CV

TL;DR: 本文提出了一种名为知识驱动序贯感知学习(KOAL)的新框架,用于通过多参数MRI非侵入性预测前列腺癌的Gleason分级组(GGG)。该方法整合了临床上下文信息、LLM提取的专家知识以及层级序贯约束,在公开数据集PI-CAI和内部数据集上均优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有GGG预测方法存在两大局限性:1)忽视非图像信息(如年龄、PSA和放射学报告中的专家先验);2)将GGG简单视为扁平分类标签,未考虑原发性和继发性Gleason模式的内在层级结构。

Method: 提出KOAL框架包含三个协同模块:1)临床上下文调制(CCM)模块使用临床变量动态调制判别性图像表示;2)知识引导原型对齐(KGPA)模块利用LLM从训练放射学报告和临床指南中提取分级特异性专家知识,生成离线语义锚点,通过原型对比对齐促进病理对齐的表示学习;3)层级序贯感知约束(HOC)模块解耦原发和继发Gleason模式预测,通过可微生医逻辑映射层(DBML)将概率输出映射到GGG,确保病理分级一致性。

Result: 在公开PI-CAI数据集和内部数据集上的实验表明,KOAL优于现有最先进方法。

Conclusion: 通过整合临床上下文、专家知识表示和层级序贯约束,KOAL框架能有效提升GGG预测准确性,为临床减少不必要活检提供重要支持。
...
### [7] [AEGIS: A Mechanism-Guided Defense against Visual Synonym Jailbreaks in Text-to-Image Models](https://arxiv.org/abs/2607.06120)
*Yuanmin Huang,Zhenfei Zhang,Mi Zhang,Geng Hong,Qinqin He,Jialing Tao,Hui Xue,Min Yang*

Main category: cs.CV

TL;DR: 本文提出了AEGIS，一种针对文本到图像扩散模型的推理时防御框架，通过识别和引导稀疏语义注入注意力头来抵御视觉同义词攻击(VSA)。该方法在不抑制良性概念的情况下，实现了显著的安全性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有的对齐范式（包括输入清理、特征剪枝等）主要针对显式暴露的不安全概念，无法有效防御视觉同义词攻击(VSA)。VSA通过隐式视觉关联使 benign-looking 的提示产生禁止图像，当前防御面临安全-效用困境：要么未能充分缓解VSA威胁，要么过度抑制视觉相似的良性概念。

Method: 本文核心创新在于从静态抑制预定义不安全概念转变为动态追踪生成过程中不安全语义的涌现。机制分析表明，VSA和显式不安全提示通过稀疏语义注入注意力头在推理时聚合，这些头是不安全视觉语义的关键瓶颈。AEGIS采用相似性感知排斥仅在识别的脆弱注意力头上进行防御。

Result: 在SD 1.4上，AEGIS将域内暴力/裸露VSA的ASR降至0.00/0.03，在域外显式和对抗性攻击上ASR≤0.09。它保留了良性保真度，避免抑制硬负面概念，并可迁移到SD 2.1和FLUX.1（在为每个主干重新识别关键头后）。

Conclusion: AEGIS通过识别关键注意力头并实施针对性防御，成功解决了VSA攻击的安全-效用权衡问题。该方法具有良好的可迁移性，为扩散模型的推理时安全提供了新思路。
...
### [8] [MAC-XA: Multi-view Anatomy-Correspondence Fusion for Coronary Stenosis Reporting from X-ray Angiography](https://arxiv.org/abs/2607.06268)
*Chen Jia,Baochang Zhang,Fatia Kusuma Dewi,Amir Yousefi,Heribert Schunkert,Reza Ghotbi,Nassir Navab*

Main category: cs.CV

TL;DR: 本文提出了一种基于对齐约束的多视角冠状动脉X光血管造影报告生成方法，通过可控的合成血管造影生成策略提供监督信号，并使用解剖对应模块显式对齐特征，提高 stenosis 报告的准确性和解剖一致性。


<details>
  <summary>Details</summary>
Motivation: 当前冠脉造影的多视角报告生成面临关键挑战：3D血管拓扑导致投影依赖的分支重叠和缩短，单视角建模对病变定位和狭窄分级本质上不完整且不稳定；多视角融合虽然有前景，但跨视角对齐不可观测且无法显式监督，传统融合依赖隐式相关性而非验证后的解剖对应。

Method: 将多视角狭窄报告重新表述为对齐约束的聚合问题；引入可控的合成血管造影生成策略，暴露几何衍生的 patch 级别对应监督信号；设计解剖对应模块学习跨视角对应矩阵，在融合前将辅助特征显式对齐到主视角坐标空间，约束证据聚合到解剖一致区域。

Result: 在合成数据和零样本迁移到真实血管造影的实验中，该对齐约束设计提高了对应一致性，相比单视角建模和传统多视角融合方法改进了结构化狭窄报告性能。

Conclusion: 通过引入合成数据监督和解剖对应约束，有效解决了冠脉造影多视角报告中跨视角对齐不可观测的核心问题，显著提升了狭窄报告的准确性和解剖一致性，为临床诊断提供了更可靠的辅助工具。
...
### [9] [Visual graphs for image classification: does the structure affect performance?](https://arxiv.org/abs/2607.06295)
*Alessandra Ibba*

Main category: cs.CV

TL;DR: 本文系统比较了当前图神经网络在视觉任务中的图构建技术，通过固定三层GCN架构进行实证研究，展示了网络结构如何影响性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型虽然成功，但仍无法完全编码图像的内在视觉结构，常忽略空间、拓扑和语义信息。图神经网络虽提供良好框架，但其在视觉任务中的有效应用尚未被充分探索。

Method: 在固定三层GCN架构的背景下，系统比较了当前的图构建技术，通过实证研究分析网络结构对性能的影响。

Result: 展示了网络结构如何影响性能，提供了关于图利用前计算阶段的重要方法论贡献。

Conclusion: 图构建技术对视觉任务性能有重要影响，为图神经网络在视觉领域的应用提供了重要的方法论指导。
...
### [10] [Synthetic-to-Real Translation for Class-Agnostic Motion Prediction](https://arxiv.org/abs/2607.06319)
...
TL;DR: 本文研究了自动驾驶中的运动预测问题，提出从合成数据到真实数据的运动知识迁移框架(SRMP)，以解决真实运动标签成本高的问题。该方法包含两个关键组件：物体感知运动预测和物体感知运动增强，并发布了首个用于SRMP研究的合成4D LiDAR数据集Motion4D。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中运动理解对安全性和鲁棒性至关重要，但获取真实世界运动标签的成本很高。因此，从合成数据迁移运动知识到真实数据是理想选择。然而，传统的运动回归方法对合成到真实的域偏移非常敏感，导致知识迁移不可靠。

Method: 提出了一种运动知识翻译框架，包含两个关键组件：(1)物体感知运动预测：显式建模运动模式与物体先验的联合分布，以改善域不变特征学习；(2)物体感知运动增强：利用学习的物体先验来过滤运动噪声的运动标签细化机制。此外，还提出了生成Motion4D的物理基础流程，这是首个专为SRMP研究设计的合成4D LiDAR数据集。

Result: 实验结果表明，该方法有效桥接了域差距，在真实场景上产生了优异的性能。

Conclusion: 所提出的方法成功解决了SRMP中的域偏移问题，并为该领域研究提供了新的数据集，推动了合成到真实运动预测的发展。
...
### [11] [Bridging Diffusion Pruning and Step Distillation with Teacher-Aligned Repair](https://arxiv.org/abs/2607.06335)
*Jincheng Ying,Li Wenlin,Minghui Xu,Yinhao Xiao*

Main category: cs.CV

TL;DR: 本文研究扩散模型推理成本压缩，探索将剪枝后的重训练阶段替换为步骤蒸馏。直接替换失败，因此引入教师对齐修复阶段作为桥梁。在ImageNet-512上，原始EDM2-XS基线使用1.247亿参数和63次网络评估，FID为3.53。经20%剪枝的一步生成器使用9883万参数和1次网络评估，FID为3.12。30%剪枝模型使用8803万参数，FID为4.26。


<details>
  <summary>Details</summary>
Motivation: 扩散模型推理成本高，来源于大型去噪网络和重复去噪步骤。现有的压缩管道通常分别处理这两个问题。剪枝减少网络规模但仍需长时间重训练恢复多步采样器；步骤蒸馏减少去噪步骤但假设学生已经能够很好地跟随教师。本文旨在探索是否可以用步骤蒸馏替代剪枝后的重训练。

Method: 提出一种教师对齐修复阶段作为剪枝和步骤蒸馏之间的桥梁。该桥接阶段在噪声真实图像潜在变量上匹配剪枝后的生成器与教师，然后将修复后的检查点交给一步蒸馏。同时采用合适的蒸馏目标。

Result: 在ImageNet-512上，原始EDM2-XS基线使用124.713M参数和63次网络评估，FID为3.53。20%剪枝后的一步生成器使用98.826M参数和1次网络评估，FID为3.12。30%剪枝模型使用88.029M参数和1次网络评估，FID为4.26。

Conclusion: 直接用步骤蒸馏替代剪枝重训练失败，需要引入教师对齐修复阶段作为桥梁。这一方法成功实现了将剪枝与步骤蒸馏结合，显著降低了扩散模型的推理成本，同时保持了可比的图像生成质量。
...
### [12] [AirflowAttack: Thermal-Airflow Adversarial Perturbations against Infrared Remote-Sensing Vision-Language Models](https://arxiv.org/abs/2607.06485)
*Cong Su,Jiaju Han,Xuemeng Sun,Chengyin Hu,Qike Zhang,Jiujiang Guo,Yiwei Wei,Jiahuan Long*

Main category: cs.CV

TL;DR: 本文提出了AirflowAttack，这是首个针对红外遥感视觉语言模型(VLM)的对抗攻击，也是首个利用热空气流湍流作为扰动先验的攻击方法。该攻击通过轻量级生成器合成单一输入无关的扰动，在物理上模拟空气流模式。


<details>
  <summary>Details</summary>
Motivation: 红外遥感图像在安全关键场景中的应用日益广泛，但VLM的对抗鲁棒性尚未得到检验。现有对抗攻击缺乏物理可行性，而物理可行的攻击在红外领域仍是空白。

Method: 设计轻量级生成器合成空气流模式正则化的扰动，在单一代理CLIP模型上优化，评估其跨五个不同CLIP骨干网络和六个最先进的VLM的零样本场景分类攻击成功率。

Result: 在五个CLIP骨干网络上达到48.5%的平均零样本场景分类攻击成功率，远超四个红外专用物理基线方法(27.7%-37.0%)。在六个VLM上使场景分类准确率降低高达38.2%，但有趣的是某些模型反而更"自信"，将扰动误认为真实热证据。

Conclusion: 空气流先验提高了物理可行性且不影响攻击成功率。这项研究揭示了快速扩展的红外VLM生态系统中关键漏洞，为11个模型和4个任务的基准测试铺平了道路。
...
### [13] [VaseMuseum: Digital Intelligent Museum for Ancient Greek Pottery](https://arxiv.org/abs/2607.06374)
*Jiazi Wang,Nonghai Zhang,Qiushi Xie,Zeyu Zhang,Yufeng Chen,Yang Zhao,Ling Shao,Hao Tang*

Main category: cs.CV

TL;DR: VaseMuseum是一个轻量级模块化的多模态智能体框架,用于古希腊陶器的数字博物馆,结合交互式虚拟博物馆与VaseAgent,可支持2D图像和3D文物,具有多模态感知、3D感知推理、外部知识检索和推理时可靠性控制功能。


<details>
  <summary>Details</summary>
Motivation: 解决文化遗产领域VLM的两大挑战:1)开放性解读需要将细粒度2D/3D视觉证据与专业策展知识结合,但检索过程可能引入弱来源和无法验证的引用;2)当证据不完整、有噪声或模糊时,VLM常产生自信但无依据的回答。

Method: 1)多模态感知支持2D图像和3D文物;2)从权威网络和博物馆知识来源检索证据;3)源级控制选择多样化和可验证的证据;4)响应级控制检查生成 claims 并在支持不足或冲突时鼓励中性、以证据为界的回答;5)训练-free GRPO-style选择机制 favoring有效引用和校准置信度的响应。

Result: 实验表明VaseMuseum在真实数字博物馆模拟中:改善了引用有效性、减少了知识密集型查询的幻觉、在模糊情况下产生更中性的回答。

Conclusion: VaseMuseum是一个有前景的古希腊陶器数字博物馆智能辅助框架,能提高VLM在文化遗产领域应用的可靠性。
...
### [14] [ELSA3D: Elastic Semantic Anchoring for Unified 3D Understanding and Generation](https://arxiv.org/abs/2607.06565)
*Tianjiao Yu,Xinzhuo Li,Yifan Shen,Onkar Susladkar,Yuanzhe Liu,Xiaona Zhou,Ismini Lourentzou*

Main category: cs.CV

TL;DR: ELSA3D是一种统一的3D基础模型，通过弹性语义锚（Elastic Semantic Anchoring）机制解决文本-3D交互中隐式建模和粗细粒度信息混淆的问题，实现语言与几何推理在匹配抽象尺度上的联合结构化。


<details>
  <summary>Details</summary>
Motivation: 现有方法将文本和3D标记拼接为平坦序列并依赖自注意力，导致粗粒度结构线索和细粒度几何细节被压缩为无差异的表示，文本-3D交互仍 largely implicit。

Method: 提出三大核心创新：1）尺度感知八叉树分词器表示几何；2）锚定标记（Anchor Tokens）作为稀疏跨模态单元，选择语义线索、路由至最相关3D尺度、检索尺度特定几何证据并将融合信号写回统一表示；3）轻量级每块路由器实现计算和推理弹性，选择文本标记在哪个几何尺度实例化锚点。

Result: 在图像到3D生成、文本到3D生成和3D_captioning三个任务上均达到最先进性能，优于最强统一基线方法，同时相比同模型非弹性版本减少约一半FLOPs和推理延迟。

Conclusion: ELSA3D通过弹性语义锚机制实现了稀疏而精确的跨模态交互，在保持统一架构优势的同时显著提升效率与效果，为多尺度3D视觉语言建模提供了新范式。
...
### [15] [Verification of Dynamic Holographic Behavior in Identity Documents](https://arxiv.org/abs/2607.06466)
*Glen Pouliquen,Joseph Chazalon,Guillaume Chiron,Thierry Géraud,Ahmad Montaser Awal*

Main category: cs.CV

TL;DR: 本文针对身份文档上全息图（光学可变设备）的远程真实性验证问题，提出新数据集MIDV-DynAttack和无需动态攻击样本训练的验证方法，并基准测试现有方法的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能检测静态欺诈（如复印），难以应对动态欺诈（如手工制作的全息图），且缺乏公开数据集，导致无法评估方法在真实攻击场景下的泛化能力。

Method: 1) 构建MIDV-DynAttack数据集，扩展原有MIDV-Holo数据集，包含真实静态和动态攻击样本，攻击样本数增加两倍；2) 提出通过分析全息图的动态行为和外观来验证真实性的方法，可在无动态攻击样本情况下训练；3) 设计清晰的评估协议基准测试现有方法。

Result: 该方法达到新的最优性能，基准测试表明其他方法无法处理动态攻击和新出现的攻击类型。

Conclusion: 该工作填补了动态攻击检测和泛化能力评估的空白，为全息图验证领域提供了数据集、方法和基准。
...
TL;DR: CAIRN是一个用于多房间3D场景理解的拓扑感知3D-LLM，在新构建的CAIRN-MR基准测试集上进行了评估。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs仅能处理简化的单房间3D场景，缺乏对包含多个相互连接房间和多样物体类别的真实家庭环境进行推理的能力。

Method: 1) 将transformer注意力与场景层次结构对齐，使模型明确感知物体级关系和房间级连通性；2) 通过图神经网络为物体token注入房间局部关系上下文；3) 引入可学习的room token进行房间级抽象；4) 应用具有几何偏差的层次注意力掩码，根据场景拓扑路由信息。模型在HM3D上构建的CAIRN-MR基准上进行训练和评估。

Result: CAIRN在所有CAIRN-MR任务上大幅超越之前的3D-LLMs，同时在五个单房间基准测试上保持竞争力。

Conclusion: 通过拓扑感知的层次化注意力机制，CAIRN成功实现了多房间3D场景理解，能够从房间内感知逐步扩展到跨房间推理。
...
### [17] [Shifting is Optimal under Gap-ETH: A Lower Bound Framework for Geometric Approximation Schemes](https://arxiv.org/abs/2607.06069)
*Manuel Cáceres,Sándor Kisfaludi-Bak,Saeed Odak*

Main category: cs.CG

TL;DR: 本文证明了Hochbaum和Maass的移动技术（shifting technique）产生的PTAS算法在常数维度下是最优的。作者开发了一个框架，利用De Berg等人的立方体布线定理和Marx-Sidiropoulos的约简步骤，证明这些算法的运行时间在Gap-ETH假设下是最优的。该框架适用于单位球图上的最大独立集、最大诱导森林等问题，以及单位球的 piercing 问题。


<details>
  <summary>Details</summary>
Motivation: 虽然移动技术在二维情况下已知是最优的（Marx FOCS'07），但对于更高维度尚未被证明。作者旨在建立一个新的框架，证明这些算法在任意常数维度下都是条件最优的。

Method: 作者开发了一个框架，基于De Berg等人的立方体布线定理和Marx-Sidiropoulos的约简步骤，创建了一个方便的几何CSP最大化版本，可用作约简的基础。

Result: 证明了移动技术产生的运行时间 n^{{O(1/ε^{{d-1}})}} 在Gap-ETH假设下对于每个常数维度都是最优的。并将该框架应用于单位球图上的几个问题（最大独立集、最大诱导森林等）以及单位球piercing问题。

Conclusion: 本文成功建立了证明移动算法条件最优性的框架，填补了高维度情况下最优性证明的空白，为几何近似算法的复杂性理论做出了重要贡献。
...
### [18] [Generalized altitudes and their bounds](https://arxiv.org/abs/2607.06187)
*Hana Dal Poz Kourimska,Mathijs Wintraecken*

Main category: cs.CG

TL;DR: 本文引入广义高线概念，推广传统顶点到对面的高线到任意相对面，提供角度统一公式并建立与行列式工具的联系，证明广义高线下界定理。


<details>
  <summary>Details</summary>
Motivation: 推广单纯形中高线的概念，建立统一的几何框架，用于研究单纯形质量和Riemannian流形三角剖分准则。

Method: 定义广义高线编码相对位置，建立统一角度公式，运用广义叉积和Gram行列式得到等价代数表达式。

Result: 得到广义高线统一公式，建立与行列式工具联系，证明了广义高线受普通高线控制的下界。

Conclusion: 传统高线或厚度假设可控制更广泛的几何量，为单纯形质量研究提供紧凑框架，应用于三角剖分准则。
...
### [19] [Deciding monotonicity of simple drawings of the complete graph](https://arxiv.org/abs/2607.06240)
*Oswin Aichholzer,Thomas Hackl,Alexander Pilz,Gelasio Salazar,Birgit Vogtenhuber*

Main category: cs.CG

TL;DR: 本文研究了完全图K_n的简单绘图是否为x-单调绘图的同构判定问题，提出了O(n^5)时间算法来判定弱同构和强同构。


<details>
  <summary>Details</summary>
Motivation: x-单调性是图绘图的重要性质，垂直线最多与每条边相交一次。判定一个给定绘图是否可同构于x-单调绘图具有理论和应用价值。

Method: 设计了一个O(n^5)时间复杂度的算法，通过分析图的拓扑结构和边的交叉关系来判定同构性。

Result: 成功给出了判定完全图K_n的绘图是否与x-单调绘图弱同构的算法，并扩展到强同构的判定。

Conclusion: 该算法为图绘图的同构判定提供了有效解决方案，时间复杂度为O(n^5)。
...
### [20] [The Minimum Dominating Set Problem on Bipartite Circle Graphs: Complexity and Approximation](https://arxiv.org/abs/2607.06251)
*A. Karim Abu-Affash,Paz Carmi,Joseph S. B. Mitchell*

Main category: cs.CG

TL;DR: 本文研究二分圆图中最小支配集问题。证明该问题在二分圆图上仍是NP难的，但给出了2-近似算法和基于局部搜索的PTAS。


<details>
  <summary>Details</summary>
Motivation: 最小支配集问题在一般圆图上已知是NP难的，作者试图探究在受限的二分圆图类上该问题的计算复杂性。二分圆图是指弦图可以表示为两组不相交的弦，且同组弦互不相交。

Method: 使用从平面单调3-SAT的归约证明NP难性；使用局部搜索技术设计PTAS。

Result: 证明最小支配集问题在二分圆图上仍是NP难的；提出多项式时间2-近似算法；开发基于局部搜索的多项式时间近似方案(PTAS)。

Conclusion: 即使在受限的二分圆图类上，最小支配集问题仍保持NP难性，但存在常数近似和PTAS算法。
...
### [21] [Prompt-to-Paper: Agentic AI System for Bioinformatics](https://arxiv.org/abs/2607.05456)
*Ramsha Kamran,Maheera Amjad,Zartasha Mustansar,Arsalan Shaukat,Salma Sherbaz,Muhammad U. S. Khan*

Main category: cs.AI

TL;DR: 提出了Prompt-to-Paper框架，通过三个集成创新解决现有LLM手稿生成系统的关键缺陷：基于60-100篇论文的可验证语料库进行确定性检索增强生成、自主权执行真实计算生物学实验、八维自动质量评分器。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型驱动的自动手稿生成系统存在三个关键缺陷：(i)生成的声明无法确定性扎根于可验证的文献，(ii)实验结果经常被捏造而非真实执行，(iii)缺乏标准化、多维度的评估框架来判断AI生成的手稿是否满足实际发表所需的质量和严谨性。

Method: 三个核心创新：(1)确定性检索增强生成管道，具有分区感知相关性评分和滚雪球引文扩展，将每个声明扎根于60-100篇论文的可验证语料库；(2)自主编码代理执行真实计算生物学实验，用真实数值结果替代合成输出；(3)八维自动质量评分器，基于已发表论文的近似参考统计进行基准测试，并加入明确的幻觉惩罚。质量驱动改进循环使用上下文丰富的修订器，每十次迭代触发深度研究循环重新运行实验和重新生成手稿。

Result: 在五个生物信息学案例研究中验证：全部五个案例生成了提交格式的PDF文件，零超范围引用。改进循环使手稿质量平均提升17.96分（满分100分），最高提升26.04分。人类审稿人平均给出手稿7.0/10的评分。每篇论文生产成本约0.31美元。

Conclusion: Prompt-to-Paper成功弥合了AI生成手稿的评估差距，通过确定性文献检索、真实实验执行和标准化质量评估实现了高质量、可验证的自动学术手稿生成，为计算生物学领域的自动化研究提供了可行方案。
...
### [22] [StateFuse: Deterministic Conflict-Preserving Memory for Multi-Agent Systems](https://arxiv.org/abs/2607.05844)
*Sergey Volkov,Yang Li,Ye Luo*

Main category: cs.AI

TL;DR: StateFuse是一种冲突感知的复制内存合约，通过标准OpSet/CRDT合并实现，无需引入新的连接代数。它提供不可变历史、显式冲突对象、精确和语义修正句柄、确定性谓词合约以及投影时解析功能。


<details>
  <summary>Details</summary>
Motivation: 当前代理系统的内存层将冲突观察隐藏在难以检查或纠正的覆盖规则后面，导致矛盾被隐藏而非可见。

Method: 基于标准OpSet/CRDT合并构建冲突感知内存合约，定义代理面向的语义层，包含不可变历史、显式冲突对象、claim_id/claim_ref修正句柄、确定性谓词合约和投影时解析。

Result: 在282题的MemoryAgentBench上与多种基线方法相比，答案准确率相当，但冲突保留方法使矛盾可见而折叠方法则不能；保留歧义支持更安全的弃权和纠正；语义句柄在精确先前标识符不可用时尤为重要。

Conclusion: StateFuse最适合作为更安全的公共内存合约，用于矛盾呈现、弃权和可审计纠正，而非作为普遍准确率提升的工具。
...
### [23] [From Graphs to Gradients: Physics-Inspired Structural Attribution for Cyber-Physical IoT Systems and Beyond](https://arxiv.org/abs/2607.05563)
*Spyridon Evangelatos,Christos Diou,Georgios Th. Papadopoulos,Evangelos Markakis,Panagiotis Sarigiannidis*

Main category: cs.AI

TL;DR: 本文提出了一种基于统计力学的可解释AI框架，用于建模网络物理物联网系统中的变量依赖关系，无需恢复有向因果图即可进行依赖感知的归因分析。


<details>
  <summary>Details</summary>
Motivation: 在大规模混合网络物理系统中，恢复显式有向因果结构往往不切实际，且传统可解释性方法主要关注输入输出变量的相关性，缺乏对干预性问题的因果解释能力。

Method: 提出一种基于无向能量表示的框架，通过分析能量景观的变化来反映各组件的影响，实现依赖感知的归因分析。

Result: 在工业物联网测试平台上进行仿真验证，结果表明该方法在归因准确性、鲁棒性和可扩展性方面均优于基于图的先进方法。

Conclusion: 该框架虽然不旨在完全恢复系统的生成动力学，但可提供有价值的依赖感知解释，支持人类理解和下游预测及诊断任务，适用于网络物理系统和socio-technical系统。
...
TL;DR: 本文提出了一种黑盒评估框架，用于系统评估大型语言模型(LLM)从结构化技术文档生成设计结构矩阵(DSM)的能力。该框架通过将生成的DSM(GEN-DSM)与手动验证的地面真值矩阵(GT-DSM)进行基准测试，解决了当前闭源Auto-DSM管道的可重复性问题。


<details>
  <summary>Details</summary>
Motivation: 当前Auto-DSM管道缺乏透明度且闭源，难以系统评估LLM生成DSM的能力。研究者希望通过引入可重现的评估方法来解决这一问题。

Method: 框架整合了单次运行和多角度运行视角，结合结构指标(完整性、正确性、耦合密度)、分类指标(选择性准确率、弃权覆盖率)和稳定性指标(熵、Fleiss' κ)，并提出综合质量分数(Q)进行合成评估。在两个数据集上进行受控实验：虚构抽象系统和真实冰箱分解。

Result: 结果表明，LLM可以生成结构上合理的DSM并在结构良好的输入下实现高可重复性，但对歧义、不一致的依赖定义和提示 formulation仍敏感。研究揭示了幻觉和弃权失败的系统性来源。

Conclusion: 该框架为审计Auto-DSM管道提供了透明基准，并为将基于LLM的分解方法集成到基于模型的系统工程(MBSE)工作流程中奠定了基础。
...
TL;DR: PolyWorkBench是一个评估LLM代理在多语言长视野工作流程中表现的基准，包含67个任务横跨商业、知识工作、法律分析、本地化和制造五个领域。研究发现当前最先进的LLM代理在多语言工作流程设置中相比单语言设置存在显著性能下降。


<details>
  <summary>Details</summary>
Motivation: 现有基准大多假设单语言设置，整个执行过程包括推理、工具调用和输出生成都在单一语言内完成。然而现实世界的应用通常涉及多语言输入和输出在统一工作流程中，而多语言性与代理执行之间的相互作用尚未被充分探索。

Method: 提出一个混合框架，结合结构化评分、可执行验证和基于LLM的语义评估。该设计能够捕捉复杂工作流程中的功能正确性和语言一致性。基准包含67个任务横跨五个领域，代理需要处理异构多语言输入、执行迭代推理、调用外部工具并产生结构化输出。

Result: 实证结果表明，最先进的LLM代理在多语言工作流程设置中相比单语言对应物遭受显著的性能下降。分析表明多语言性在推理和执行步骤中引入复合效应。

Conclusion: 多语言性在推理和执行步骤中引入复合效应，强调了在代理评估中联合建模语言变化和程序化决策的重要性。
...
TL;DR: 本研究提出了一种名为Efficiency启发式的动态多车分配方法，用于解决结合VRP和 Orienteering Problem的动态在线路径规划问题，在保持与元启发式算法相当解质量的同时，将规划时间缩短2-3个数量级。


<details>
  <summary>Details</summary>
Motivation: 现代物流和城市移动中需要实时处理动态到达的任务，同时最大化奖励收集并控制计算成本。传统方法难以在解质量和计算效率之间取得平衡。

Method: 提出Efficiency启发式方法（基于奖励密度），与四种构造启发式和三种元启发式算法（ALNS、GA、SA）在相同条件下进行比较评估，应用于无人机任务分配和城市出租车调度两个领域。

Result: Efficiency启发式在所有测试配置中达到与最佳元启发式算法相当的解质量，但规划时间减少2-3个数量级，在奖励-计算前沿上对所有竞争方法形成帕累托支配。

Conclusion: 在动态、时间约束的路由环境中，精心设计的贪心启发式可以匹配复杂搜索程序的输出且计算成本极低，是实时分配和调度系统的优选方案。
...
TL;DR: 本文解决了预测市场中的一个关键问题：预测准确性何时能转化为交易利润？作者证明了对于任何严格正确的评分规则S，存在一种"适当的"投注策略，仅依赖于预测者的预测p和市场价格q，当p在S下优于q时就能获得正预期利润，并在Kalshi平台上实现了月回报率+80.33%、夏普比率3.35的实战成果。


<details>
  <summary>Details</summary>
Motivation: 古典理论建立了预测准确性与交易利润之间的等价关系，但仅限于自动做市商(AMM)设计。而现实中最大的交易所基于中央限价订单簿，知情的预测者经常亏损，而不知情的策略却能通过简单启发式方法盈利。这一 discrepancy 激发了作者建立预测准确性与盈利性之间的正式等价关系。

Method: 对于任何严格正确的评分规则S，作者展示了一种"适当的"投注策略，仅依赖于预测者的预测p和市场价格q以及市场流动性。通过对预期利润进行分解，严格推广了古典AMM保证，并解释了没有准确性优势的策略如何盈利。实证研究涵盖数千个AI模型预测，并进行了为期一个月的Kalshi实时部署。

Result: 1. 理论结果：对于任何严格正确的评分规则S，当p在S下优于q且市场有足够流动性时，适当投注策略获得正预期利润；2. 实证结果：跨越数千个AI模型预测，适当投注是唯一能可靠将准确性转化为利润的策略；3. 识别了系统性预测人物角色(personas)；4. 一个月Kalshi live部署实现+80.33%投资回报率，夏普比率3.35。

Conclusion: 本文建立了预测准确性与盈利能力之间的正式等价关系，证明了适当投注策略是唯一具有稳健盈利保证的策略。这不仅推广了古典AMM理论，解释了订单簿市场中的盈利机制，还通过实际部署验证了其有效性，为预测市场参与者提供了明确的盈利指导。
...
### [28] [A Physics-Informed Neural Network Framework for Elastodynamic Wave Propagation in Bimaterial Systems](https://arxiv.org/abs/2607.06479)
*Sonal Ankush Chibire,Jenn-Terng Gau,Bo Zhang*

Main category: cs.AI

TL;DR: 本研究提出了一种基于物理信息神经网络(PINNs)的框架,用于模拟双材料系统中瞬态弹性动力波传播问题。该方法结合有限元验证数据,成功预测了波在钢-铝界面的透射和反射,以及轴向和径向位移历史、应力应变演化等关键参数。


<details>
  <summary>Details</summary>
Motivation: 针对双材料系统(如分离式霍普金森压杆装置)中的弹性动力波传播问题,传统有限元方法计算成本高,缺乏能够快速预测的替代模型。因此需要开发一种结合物理约束的机器学习方法,以实现高效且准确的波传播模拟。

Method: 采用基于PINN的框架,将轴对称线弹性方程、初始条件、边界条件和界面条件嵌入网络损失函数。使用ANSYS Workbench Explicit Dynamics进行高保真有限元模拟,作为验证和补充训练数据约束。

Result: 该框架准确预测了波在双材料界面的透射和反射,再现了轴向和径向位移历史、面平均响应以及应力应变演化,与有限元解高度一致。训练后的网络能够预测未见过的时间点和修改材料属性下的波响应,且具有网格不敏感性,适用于不同材料组合。

Conclusion: 将物理信息神经网络与显式有限元分析相结合,为异质固体中的弹性动力波传播提供了准确且计算高效的框架,为高应变率固体力学和冲击工程应用提供了有效的替代建模方法。
...
TL;DR: 本文针对爱尔兰乳制品行业的可再生能源集成和碳排放减排问题，提出了一种基于差分进化和多智能体深度强化学习的双层多目标优化控制系统。上层采用动态定价，下层使用多智能体强化学习进行电池管理。仿真结果表明，该控制框架相比基于规则模型可提高18%的能源套利利润，同时满足爱尔兰电网规范。 


<details>
  <summary>Details</summary>
Motivation: 爱尔兰乳制品行业在可再生能源集成和碳排放减排方面具有巨大潜力，但当前分布式发电控制研究主要集中在住宅和商业应用领域，缺乏针对乳制品行业的专门研究。

Method: 提出了一种双层多目标优化控制系统：上层使用动态定价，下层基于多智能体深度强化学习进行电池管理。并在农村配电线路中模拟了所提控制系统的电气响应。

Result: 仿真结果表明，所提控制框架相比基于规则模型可将能源套利利润提高18%，在不显著增加成本的情况下提高分布式发电利用率，并符合爱尔兰电网的电压变化规范。

Conclusion: 所提出的双层优化控制方案能够有效提升乳制品行业的能源经济效益，同时满足电网合规性要求，为该行业的绿色可持续发展提供了可行的技术方案。
...
TL;DR: 本文提出一种基于LLM代理内部隐藏状态表示的早期失败预测方法，通过在每个交互轮次设置校准的"中止门"来提前终止注定失败的任务轨迹，从而节省大量推理计算资源。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在解决多步骤任务时经常选择注定失败的轨迹，但在失败可观察之前就消耗了大量推理计算资源。作者希望能够尽早预测失败以节省计算资源。

Method: 1）使用轻量级每轮探针（probe）分析隐藏激活状态，在第一轮交互时就能预测最终 episode 失败；2）构建实际的中止级联：每轮设置一个分布无关的校准门，按用户指定的全局召回率目标联合搜索每轮召回预算；3）该机制保证最终成功的 episode 能以指定概率通过所有门。

Result: 在 TextCraft 上测试两个代理模型，中止级联在 90%-97% 的所有召回率目标上都达成目标；在 90% 目标下，Qwen-2.5-7B 节省 47.1%±10.3% 推理计算，Llama-3.2-3B 节省 37.2%±8.8%，是最佳单门策略的 1.6-1.7 倍。仅读取行为特征的级联只能节省约一半资源，向探针添加行为特征没有额外收益，说明隐藏状态已捕获行为信息。

Conclusion: 代理的内部隐藏表示可以预测早期失败，这比仅观察行为更为有效。提出的中止级联系统能有效节省推理计算资源，同时保证用户指定的召回率目标。此外，文章还给出了高召回率目标的样本复杂度认证方法。
...
### [31] [RMISC: A Large-scale Real-world Multivariate Corpus for Time Series Foundation Models](https://arxiv.org/abs/2607.06504)
*Qian Sun,Yong-Ming Tian,Jia-Wei Huang,Cheng Feng,Shao-Qun Zhang*

Main category: cs.AI

TL;DR: 本文研究了时间序列基础模型（TSFMs）在真实世界多变量数据与合成数据上的预训练效果差异，并构建了包含约200个数据集和1420亿时间点的大规模RMISC语料库。实验表明，真实世界多变量数据能显著提升模型的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有TSFMs多在合成多变量数据上预训练，虽然易于扩展，但可能无法捕捉真实世界时间序列的复杂时间动态和跨变量关系。作者旨在探究真实世界数据训练的TSFMs是否优于合成数据训练的模型。

Method: 构建RMISC语料库（包含约200个数据集、1420亿时间点，涵盖多领域）；在单变量、合成多变量和真实世界多变量数据上预训练四种先进TSFMs；在标准分布内和分布外基准上评估零样本泛化能力。

Result: 实验表明，纳入真实世界多变量数据主要能提升单变量和多变量TSFMs的泛化性能。

Conclusion: 真实世界多变量数据对开发更强的TSFMs具有重要贡献，为理解数据类型对模型能力的影响提供了更深入的见解。
...
### [32] [FootsiesGym: A Fighting Game Benchmark for Two-Player Zero-Sum Imperfect-Information Games](https://arxiv.org/abs/2607.06514)
*Chase McDonald,Nathan Tsang,Wesley N. Kerr*

Main category: cs.AI

TL;DR: FootsiesGym是一个基于极简2D格斗游戏Footsies的开源学习环境，专注于非传递性策略交互，专门用于双人零和不完全信息博弈的研究。


<details>
  <summary>Details</summary>
Motivation: 构建一个能够隔离格斗游戏中性游戏循环非传递性战略交互的环境，同时保持足够简单以进行高效分析，并提供高吞吐量训练能力。

Method: 基于HiFight的Footsies游戏构建环境，提供向量化模拟器以实现高吞吐量训练，并基准测试多种强化学习算法。

Result: 完成了环境设计描述，提供了强化学习算法的基准测试结果，并讨论了开放的研究方向。

Conclusion: 该环境通过开源代码发布，为不完全信息博弈和对抗性学习研究提供了可访问和可复现的工具平台。
...
### [33] [Bridging Physical Reasoning and Task Generalization via Visual Action Outcome Reasoning Alignment](https://arxiv.org/abs/2607.06522)
*Han-Jun Ko,Jr-Jen Chen,Haobo Yuan,Hsin-Ying Lee,Tiancheng Shen,Ming-Hsuan Yang,Yu-Chiang Frank Wang*

Main category: cs.AI

TL;DR: VAORA是一种针对视觉语言模型(VLM)在交互式物理推理中泛化能力不足的新方法，通过引入视觉对齐奖励和视觉-动作对齐奖励来解决幻觉思维链和推理-行为不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在交互式物理推理中难以泛化到未见任务和环境，存在两种主要失败模式：1) 产生与物理现实矛盾的幻觉思维链推理；2) 模型的推理与行为之间存在对齐偏差。

Method: 提出VAORA框架，包含两个互补的奖励：1) 视觉对齐奖励，将VLM推理锚定到独立于智能体动作的视觉上下文；2) 视觉-动作对齐奖励，将推理建立在模型动作引发的视觉结果之上。此外使用预训练领域专家智能体估计成功概率来获得平滑的密集奖励，以提高训练稳定性。

Result: 在PHYRE和Virtual Tool基准测试上的实验表明，VAORA在新任务和新环境设置下均取得良好性能，验证了可以通过该方法诱导出基于现实基础且可泛化的物理智能。

Conclusion: 通过视觉对齐奖励和视觉-动作对齐奖励的结合，可以有效抑制幻觉思维链并缩小推理与行为之间的差距，从而实现视觉语言模型在物理推理任务中的泛化能力。
...
### [34] [DepthWeave-KV: Token-Adaptive Cross-Layer Residual Factorization for Long-Context KV Cache Compression](https://arxiv.org/abs/2607.06523)
*Anna Cordoba,Adam Puente Tercero,Nerea Angulo Hijo,Mar Linares Tercero,Julia Barrientos,Ainhoa Miranda,Jesus Olivera*

Main category: cs.AI

TL;DR: DepthWeave-KV是一种针对长上下文语言模型推理的token自适应缓存压缩方法，通过跨深度残差因子分解和token条件深度路由器，在仅使用8.3x KV内存压缩的情况下实现接近完整缓存的任务质量，在64K上下文下达到72.8 tokens/秒的吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有压缩方法在层或token上应用统一预算，导致词汇线索和语义状态需要不同保留时检索质量下降，长上下文语言模型推理受到存储键值缓存的内存带宽和容量的限制。

Method: DepthWeave-KV使用共享低秩通道基对相邻transformer层的键值状态进行因子分解，同时保留对注意力行为敏感的轻量级token特定残差；结合跨深度残差因子分解与token条件深度路由器，为承载指令和检索关键token分配更高重构秩；使用来自注意力输出探针的无校准在线误差跟踪，在生成过程中自适应调整压缩。

Result: 在LongBench、Needle-in-a-Haystack、L-Eval和长形式问答及摘要基准测试中，DepthWeave-KV以显著更低的内存使用实现接近完整缓存的任务质量，在64K上下文下实现8.3x KV内存 reduction和72.8 tokens/秒的吞吐量，提升了平均分数和检索准确率。

Conclusion: DepthWeave-KV通过创新的跨深度因子分解和token自适应压缩策略，成功在大幅降低内存占用的同时保持长上下文语言模型的推理质量，为高效长上下文推理提供了有效的解决方案。
