...
TL;DR: MotionMERGE是一个统一的细粒度语言引导运动控制框架，通过在单一LLM中显式建模身体部位和时间级别的运动，并设计Reasoning-Aware Granularity-Synergy预训练策略，结合精心构建的MotionFineEdit数据集，实现更精确的运动生成、理解和编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有运动-语言模型操作粒度粗糙，缺乏对身体部位的细粒度理解和精细控制能力。问题的根源在于模型无法聚焦于运动的局部模式，且训练数据缺乏细粒度监督信号。

Method: 提出三大创新：1）细粒度语言引导运动控制研究，在单一LLM中显式建模部位级和时间级运动；2）Reasoning-Aware Granularity-Synergy预训练策略，采用跨粒度对齐、时间定位、局部对齐、运动连贯性和运动grounded CoT推理的联合监督；3）构建MotionFineEdit大规模数据集（837K原子+144K复杂三元组），包含首个细粒度时空修正指令和运动grounded CoT标注。

Result: 实验表明MotionMERGE具备更精确的运动生成、理解和编辑能力，并能够零样本泛化到其他复杂运动任务。

Conclusion: 该工作代表了向更细粒度和类人推理方式与运动交互迈出的重要一步。
...
TL;DR: 本论文研究艺术作品分类任务，比较监督和自监督 backbone 作为特征提取器的效果，重点评估 DINO 和 CLIP 模型在绘画分类和检索任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 艺术作品分类面临复杂挑战，需要兼顾细粒度细节和抽象风格特征的平衡。自监督学习方法可能在学习泛化特征表示方面具有优势。

Method: 使用 DINO 系列模型和 CLIP 模型作为特征提取器，进行广泛的实验评估，比较多种分类策略和特征表示方法。

Result: 使用自监督 backbone 可以显著提升艺术作品分类性能，同时为 VR 应用等实际场景提供分类和检索模块的可行性见解。

Conclusion: 自监督学习方法在艺术作品分类任务中优于监督学习方法，具有一致的性能提升，为博物馆导航等实际应用提供了技术基础。
...
TL;DR: 本文提出了Artifact-Bench基准，用于评估多模态大语言模型(MLLMs)在AI生成视频artifacts检测和分析方面的能力。通过建立涵盖写实、动画和CG风格视频的三层层次分类法，设计了三个互补任务。实验对19个主流MLLMs进行测试，揭示了当前模型在artifact感知和推理方面的重大局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管视频生成模型的真实感有所提升，但其输出仍存在时间不一致、结构扭曲和语义不连贯等artifacts。现有的多模态大语言模型表现出强大的视觉理解能力，但它们感知和reasoning这些artifacts的能力尚不清楚。现有benchmarks缺乏对artifact感知和细粒度诊断reasoning的系统评估，尤其在不同AI生成视频领域beyond photorealistic内容。

Method: 构建了Artifact-Bench，一个全面的benchmark用于评估MLLMs在AI生成视频artifact检测和分析方面的工作。建立了一个三层层次taxonomy的真实性artifacts，涵盖写实、动画和CG风格视频。基于此定义了三个互补任务：(1)真人vs AI生成视频分类；(2)成对真实度比较；(3)细粒度artifact识别。对19个领先的MLLMs进行了实验。

Result: 实验揭示了在artifact感知和reasoning方面的重大局限性，许多模型在挑战性设置中接近随机甚至低于随机的性能。进一步观察到MLLM判断与人类感知偏好之间存在显著的不一致，突显了它们作为AI生成视频真实性的一般评估器的有限可靠性。

Conclusion: 当前的多模态大语言模型在AI生成视频artifact感知和reasoning方面存在严重不足，需要开发更强大的artifact感知和细粒度reasoning能力。 Artifact-Bench为系统评估MLLMs在这方面的能力提供了重要工具，有助于推动该领域的研究进展。
...
TL;DR: EgoTraj是一个开放的元Quest Pro（MQPro）自我中心多模态数据集，包含75个人类导航序列，收集自真实城市环境中的多名MQPro佩戴者。


<details>
  <summary>Details</summary>
Motivation: 准确预测第一人称视角的人类轨迹在人形机器人、可穿戴传感系统和辅助导航中至关重要，但因真实环境中的自我中心轨迹数据集稀缺，相关研究进展受限。

Method: 引入EgoTraj数据集，包含75个序列，使用Meta Quest Pro记录，每个记录提供同步RGB视频及地面真值数据（6自由度头部姿态、每帧3D眼注视向量、场景注释）。并对多种最先进的自我中心轨迹预测方法进行基准测试和消融研究。

Result: EgoTraj与其他自我中心轨迹数据集不同，能够捕捉跨越不同城市路线的长期自主导航，具有广泛的参与者多样性。结果显示注视、场景和运动线索有助于整体性能提升，EgoTraj对AR感知的潜在应用价值。

Conclusion: EgoTraj数据集、代码和EgoViz Dashboard已在GitHub上公开可用，有助于推动AR感知、导航和辅助系统的研究。
...
### [5] [Untitled](https://arxiv.org/abs/2605.19020)
**
...
TL;DR: 本文对医学基础模型进行了系统分类，将其划分为医学视觉语言模型和分割基础模型两大类，并指出这些模型在临床广泛应用前需要进行严格可靠性评估。


<details>
  <summary>Details</summary>
Motivation: 医学基础模型(MedFMs)在医疗领域展现出变革性潜力，但缺乏针对其真实世界临床部署可靠性的系统评估。

Method: 将MedFMs分为两大范式：医学视觉语言模型(Med-VLMs)和分割基础模型，并对各类代表性模型进行分类综述。

Result: 构建了完整的MedFMs分类体系，涵盖从专用模型(LLaVA-Med、MedGemma)到通用模型(GPT-4o、Gemini)的Med-VLMs，以及基于SAM的医学分割模型(SAM-Med2D、MedSAM)。

Conclusion: 该研究为后续评估MedFMs在真实世界临床条件下的可靠性奠定了分类框架基础。
...
TL;DR: FaceCloak是一种个人化的人脸隐私保护系统，可从单张用户图像生成防御性的人脸隐私掩码，导致人脸识别失败。该系统通过三阶段方法生成轻量级的像素级cloak，可高效应用于任何用户人脸图像。


<details>
  <summary>Details</summary>
Motivation: 在线上传的人脸图像容易被恶意行为者抓取，并利用人脸识别模型未经授权入侵个人隐私。

Method: 三阶段个性化人脸扰动学习：(1)基于单张图像生成高多样性合成人脸图像；(2)通过迭代扰动生成增强关键面部身份泄露区域的保护，将用户身份嵌入引向遥远锚身份；(3)生成像素级cloak形式的个性化身份保护掩码。

Result: 在三个人脸数据集和十个人脸识别模型上，比29种现有代表性方法更有效。

Conclusion: FaceCloak能有效保护用户人脸隐私，同时保持良好的感知质量。
...
TL;DR: 提出LiFT框架，通过将3D体数据分解为slice级别生成和inter-slice轨迹学习，实现高效的3D医学图像合成。


<details>
  <summary>Details</summary>
Motivation: 高分辨率3D医学图像合成的挑战：完全体素模型计算成本高，而高效的2D切片生成器无法保持第三维度的解剖学一致性。

Method: 将3D体数据视为特征空间中的有序轨迹，建模解剖结构在深度维度上的外观、变换和消失；引入三平面漂移损失对齐生成切片与真实体数据的轨迹；在配对翻译中使用双向z-context mixer提供平面间一致性。

Result: 在BraTS 2023（无条件和缺失模态MR）和SynthRAD2023（MR到CT）上评估，达到了接近cWDM缺失-MR重建质量的水平，推理成本降低约135倍，并改善了MR到CT的平面间一致性。

Conclusion: 轻量级的inter-slice轨迹学习是高分辨率3D医学合成的可行路径，具有高效性和保真度。
...
TL;DR: 本文提出CRAFT系统，用于基于真实世界新闻事件的多视频问答任务，通过动态关键帧选择、ASR多语言回退和混合批评循环来验证和修复声明，最后合并引用并输出每个事实及其支持的源标识符。


<details>
  <summary>Details</summary>
Motivation: 现实世界的新闻视频档案具有异构性，现有系统在surfacing查询相关证据和将声明归属于支持来源方面存在困难，需要一个能够迭代验证和修复声明的pipeline。

Method: CRAFT结合动态关键帧选择、per-video ASR（含多语言回退）、UNLI时间蕴含、DeBERTa-v3跨声明筛选、Llama-3.2-3B裁判器和引用合并阶段，形成混合批评循环以迭代验证和修复声明。

Result: 在MAGMaR 2026上达到0.739平均分、0.810引用召回率和0.635引用F1；在WikiVideo上达到0.823平均分。消融实验表明原子声明、ASR和批评循环是主要增益来源。

Conclusion: CRAFT通过claim-centric的证据聚合方法，在两个基准数据集上均取得先进性能，证明了其良好的泛化能力。代码已公开。
...
### [10] [FAGER: Factually Grounded Evaluation and Refinement of Text-to-Image Models](https://arxiv.org/abs/2605.19111)
*Youngsun Lim,Cusuh Ham,Pin-Yu Chen,Deepti Ghadiyaram*

Main category: cs.CV

TL;DR: 本文提出FAGER框架，解决文本到图像模型中事实正确性评估难题，通过构建结构化事实规则和VLM评估，在多个领域数据集上表现优越，并支持无训练的图像优化。


<details>
  <summary>Details</summary>
Motivation: 现有T2I评估指标主要关注显式信息，忽略了隐式、外部 grounding 或身份定义的事实性需求，无法评估涉及科学知识、历史事实、产品和文化特定概念的prompt

Method: 1) 构建结构化事实规则：结合LLM事实提议与参考引导的视觉事实提取验证
2) 将规则转换为问答对用于VLM评估
3) 使用Factual A/B测试验证指标效果
4) 无需训练的图像精炼方法

Result: 在五个数据集（科学、历史、产品、文化和知识密集概念）上，FAGER一致优于先前指标；且能带来显著的事实性提升

Conclusion: FAGER是一个有效的用于评估文本到图像模型事实性的指标，能够用于无训练方式的图像改进
...
TL;DR: 研究自监督学习预训练时长如何影响医学图像模型在糖尿病视网膜病变筛查中的置信度校准和选择性 abstention 性能。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像模型的评估主要关注下游准确率，但对于安全性关键的筛查任务（如糖尿病视网膜病变分级），模型不仅需要准确预测，还需要知道何时预测不可靠从而 defer 不确定案例进行临床审核。现有研究忽略了置信度行为的评估。

Method: 使用多个SSL检查点，在固定微调协议下评估，分析预训练时长对校准置信度、覆盖率、选择准确率和选择 macro-F1 的影响。

Result: SSL预训练提高了选择性预测性能。一旦准确率饱和，不同检查点之间的选择性性能仍可能发生显著变化，且更长的预训练并不一致地提高可靠性。在不同数据集和数据规律下均观察到类似现象。

Conclusion: 研究强调了基于abstention的评估重要性，提示预训练时长应被视为可靠性相关的设计选择而非仅是计算细节。需要将置信度校准和不确定性量化纳入模型评估标准。
...
TL;DR: 本文探索了一种轻量级视频模型训练范式：通过冻结图像基础模型作为空间编码器，只训练循环时间模块处理视频，从而避免大规模视频预训练。实验表明这种方法能在多个视频理解任务上取得良好性能，为未来循环视频基础模型的研究提供了可行性证据。


<details>
  <summary>Details</summary>
Motivation: 视频基础模型通常需要大规模预训练，消耗大量数据和计算资源。而现代图像基础模型已经提供了强大的空间表示能力。作者提出一个问题：能否通过重用这些空间表示，仅针对时间推理进行轻量级预训练来构建有竞争力的视频模型？

Method: 冻结预先训练好的图像基础模型作为空间编码器，只训练一个循环时间模块来处理流式视频。这种方法避免了端到端的视频预训练，大大减少了所需的视频数据和计算资源。

Result: 在多个视频理解任务上的实验表明，强大的时间性能可以在没有大规模视频预训练的情况下出现，证明了这种轻量级训练范式的可行性。

Conclusion: 研究结果支持了未来构建循环视频基础模型的工作，即在冻结的图像基础模型上预训练时间模块，为视频理解提供更高效的训练方案。
...
### [13] [Quantized Machine Learning Models for Medical Imaging in Low-Resource Healthcare Settings](https://arxiv.org/abs/2605.19207)
*Sumanth Meenan Kanneti,Aryan Shah*

Main category: cs.CV

TL;DR: 本文提出了一种针对脑肿瘤MRI分类的多策略压缩框架，结合知识蒸馏、量化感知训练和Float16后训练量化技术，在MobileNetV2上实现6.14x压缩比（35.34MB→5.76MB）且精度几乎无损（82.37% vs 82.20%）。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分析中表现优异，但在低资源临床环境中部署面临计算、内存和功率限制的挑战。

Method: 多策略压缩框架包括：1)从DenseNet-101教师到DenseNet-32学生的知识蒸馏配合低位后训练量化；2)MobileNetV2轻量级骨干网上的Float16后训练量化；3)三阶段迁移学习训练流程，使用TensorFlow Lite进行Float16量化。

Result: 量化模型在包含胶质瘤、脑膜瘤、垂体瘤和健康对照的多类别脑肿瘤MRI数据集上达到82.37%验证准确率，相比全精度基线82.20%无明显下降；模型大小从35.34MB压缩至5.76MB，压缩比6.14x；各肿瘤类别的诊断性能保持一致。

Conclusion: 轻量级量化模型可在资源受限的医疗环境中实现临床可行的脑肿瘤筛查，为低资源临床环境下的医学AI部署提供了可行方案。
...
TL;DR: 本文提出了一种基于智能手机的轻量化树木测量管道，利用单段步行视频实现林木DBH和空间位置的自动测量，整合单目深度估计、实例分割与SLAM技术，在管理林分和天然林分中分别达到1.51cm和2.30cm的平均绝对误差，精度相当于传统野外方法，但大幅降低了设备成本和操作复杂度。


<details>
  <summary>Details</summary>
Motivation: 圆形样地是森林资源调查的基石，但精确测量树高（DBH）和空间位置仍面临挑战。传统方法依赖昂贵的地面激光雷达或耗力的人工卡尺和罗盘测量，限制了在大规模环境中的可扩展性和可及性。

Method: 提出一种基于智能手机的轻量化管道，只需一段步行视频即可完成样地内所有林木的测量。整合预训练的单目深度估计模型、树木实例分割与SLAM框架，联合优化相机轨迹和深度；通过融合SLAM导出的相机姿态与分割的深度图恢复树木位置和DBH估计，并利用校准的参考长度锚定绝对尺度。

Result: 在管理林分和天然林分中进行评估，平均绝对误差分别为1.51cm（MARE 3.98%）和2.30cm（MARE 5.69%），在不同起始方向和位置下均表现一致。跨视频一致性分析表明，从不同起始位置开始的测量均能实现稳定、可重复的树木定位。

Conclusion: 该方法达到了与已有野外方法相当的精度，同时大幅降低了设备成本和操作复杂度，使专业研究人员和非专家森林管理者在各种作业环境中都能轻松使用。
...
TL;DR: RotateK是一种基于旋转的结构化键通道剪枝框架，用于缓解视觉语言模型推理中的KV缓存压力。它通过在线PCA旋转将token依赖的通道重要性对齐到共享低维子空间，在轻量级头掩码下实现准确剪枝，并配合Triton注意力内核实现高效解码。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型推理中，单张图像常被编码为数千个令牌，导致严重的KV缓存压力。现有方法使用token剪枝，但会丢弃重要视觉信息，在细粒度感知任务上造成显著降级。特征维度压缩可在相同内存成本下保留更多视觉token。之前的方法在结构化程度上存在两难：token-wise通道剪枝表达力强但非结构化且慢，head-wise方法硬件友好但鲁棒性不足。

Method: 提出RotateK，一种基于旋转的结构化键通道剪枝框架。核心创新包括：(1)应用在线PCA-based旋转，将token依赖的通道重要性对齐到共享低维子空间，使头级别掩码能实现准确剪枝；(2)设计融合的Triton注意力内核，直接在稀疏通道KEY上操作实现高效解码。在两个代表性VLM backbone上验证，与token-only基线相比，联合token-通道剪枝在匹配KV缓存预算下性能更优。

Result: 实验表明：(1)RotateK在准确率和解码延迟上均一致优于之前key通道剪枝方法；(2)联合token-通道剪枝在匹配的KV缓存预算下优于纯token剪枝基线。RotateK兼顾了表达力和硬件效率，解决了原有方法中结构化程度与效率的两难问题。

Conclusion: RotateK成功实现了在硬件友好条件下保持高表达力的key通道剪枝，为VLM推理效率提升提供了新思路。该方法证明了特征稀疏性与token稀疏性联合优化的潜力，为未来高效VLM推理研究指明了方向。
...
TL;DR: 本文提出了HAVEN，一个用于统一视频理解的分层对齐多模态基准。现有视频摘要基准只关注单一粒度（如关键帧、关键镜头或离散文本摘要），无法捕捉跨模态对齐的层级结构。HAVEN提供了完全多模态（视频和文本）和完全粒度（帧、镜头和视频级别）的数据集架构，包含显式的连续模态对齐。


<details>
  <summary>Details</summary>
Motivation: 在复杂叙事摘要和推理方面，多模态大语言模型（MLLMs）的能力仍缺乏充分评估。现有基准将监督分散在孤立的粒度上，fragmenting supervision across isolated granularities，无法捕捉跨模态对齐的固有的层级结构。

Method: 构建了HAVEN数据集，包含帧级别、镜头级别和视频级别的标注，以及视频和文本的显式连续对齐；开发了综合评估套件，涵盖摘要生成、时间推理、多模态定位和显著性排序；广泛测试了最先进的MLLMs。

Result: 对顶尖MLLMs的广泛基准测试揭示了表面层面文本流利性与基于多模态的深入理解之间持续存在差距。

Conclusion: HAVEN推动多模态系统评估超越传统问答格式，提供了严格的标准化测试平台，以促进未来可解释的、分层视频理解研究。
...
TL;DR: 本文提出一种鲁棒的框架，旨在减轻医学图像分类中年龄依赖的混淆效应，同时保留临床上有意义的非线性年龄信息。通过对样本难度进行年龄相关趋势建模，并使用Huber加权亲和力权重解耦年龄与主要年龄难度特征，实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 年龄在医学图像分类中是重要的混淆因素，它将成像形态与疾病患病率联系起来。这导致在疾病患病率较高的年龄组过度诊断，在患病率较低的年龄组诊断不足，且在训练-测试年龄分布变化时问题加剧。传统方法强制年龄不变性会丢失诊断意义上与年龄相关的信息。

Method: 1) 先进行warm-up阶段；2) 表征样本难度，并以标签为条件建模其年龄依赖趋势；3) 使用鲁棒的Huber加权亲和力权重，将年龄与主导的年龄难度趋势解耦，削弱混淆驱动的捷径同时保留临床有意义的非线性年龄信息；4) 引入年龄覆盖分数，按小批量年龄方差缩放解耦惩罚，确保在有限年龄多样性下的稳定优化。

Result: 在两个放射学数据集上，该方法减少了年龄依赖的真阳性率和假阳性率差异，对AUC影响很小，并在训练-测试年龄分布偏移增加时保持鲁棒性。

Conclusion: 该框架通过针对虚假的年龄关联趋势而非强制 invariance，有效平衡了公平性与诊断 성능，为医学影像AI中的年龄偏见问题提供了新思路。
...
TL;DR: PhyWorld是一个用于物理AI的世界模拟器视频生成模型，通过流程匹配微调和直接偏好优化的两阶段后训练，提升视频延续的时空一致性和物理可信度，在VBench和物理忠诚度基准上均取得领先成绩。


<details>
  <summary>Details</summary>
Motivation: 大型视频生成模型有潜力成为物理AI系统的世界模拟器，但作为世界模拟器需要物理上忠实的视频延续——即保持输入暗示的物理状态并以符合基本物理原则的方式演化的生成视频。现有的视频生成模型难以满足这一要求。

Method: 提出PhyWorld视频生成世界模型，采用两阶段后训练方法：第一阶段使用流程匹配(Flow Matching)微调改进视频到视频的延续能力，促进稳定的视觉属性和跨帧连贯的运动动态；第二阶段使用直接偏好优化(DPO)在物理偏好对上进行训练，使生成的动态符合物理原理。同时构建专用的物理忠诚度基准测试进行评估。

Result: PhyWorld在标准视频质量基准VBench上平均得分0.769，优于最先进基线的0.756；在专用物理忠诚度基准上平均得分3.09，优于最强基线的2.99。

Conclusion: 研究证明了通过两阶段后训练（流程匹配微调和直接偏好优化）可以显著提升大型视频生成模型作为物理AI世界模拟器的能力。PhyWorld在视频质量和物理可信度方面均优于现有最佳基线方法，验证了后训练大型视频生成模型进行场景延续和物理偏好信号优化的有效性。
...
TL;DR: 本文系统分析了扩散模型生成的合成图像对视觉分割任务的影响，发现稠密场景构图和细粒度实例保真度是关键因素，并提出SENSE框架，利用合成数据显著提升分割性能，且适用于多种架构。


<details>
  <summary>Details</summary>
Motivation: 大型生成式模型的快速发展使合成数据成为视觉理解的有望解决方案，但扩散模型在复杂视觉分割任务中的潜力尚未被充分探索，缺乏对影响合成图像实用性的因素的深入分析。

Method: 对当前最先进的扩散模型生成的合成图像进行系统性分析，探索影响其用于分割任务效用的因素；基于分析结果提出SENSE统一框架，利用灵活可扩展的合成数据增强分割性能，该框架与多种架构（DPT、Mask2Former）兼容且具有良好的可扩展性。

Result: 具有稠密场景构图和细粒度实例保真度的合成图像能产生更具判别性的空间表示，显著提升分割性能；在Cityscapes、COCO和ADE20K数据集上验证了方法的有效性和泛化能力。

Conclusion: 首次系统性地分析了扩散模型合成图像对分割任务的影响；SENSE框架是模型无关的，能有效利用合成数据提升不同容量模型的分割性能，具有良好的泛化和可扩展特性。
...
TL;DR: 本文提出iGSP框架，通过隐式梯度子空间投影实现视觉-语言模型的高效任务适应，解决传统方法中参数爆炸和负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在持续适应下游任务时面临两大挑战：1)参数高效微调虽缓解灾难性遗忘，但孤立模块导致参数激增；2)现有的相似性共享机制错误地将表观视觉相似等同于对齐一致性，引起视觉相似但逻辑不同任务间的严重负迁移，同时未能利用视觉多样性任务间的对齐复用。

Method: 提出iGSP框架，基于 MoE 路由器早期收敛建立子空间基，将适应过程分为两阶段：(1)子空间识别阶段通过基预扩展引入候选专家，实施子空间约束正则化将新任务梯度隐式投影到历史子空间，并利用路由概率作为梯度流指示器精确剪枝冗余维度；(2)正交子空间微调阶段固定结构基并移除正则化以快速拟合任务特定残差损失。

Result: 在MTIL基准上实现SOTA精度，训练效率显著提升，可训练参数平均减少42.7%，最终总参数相比同类方法减少86.9%。

Conclusion: iGSP将对齐共享问题重新诠释为低秩子空间中的优化轨迹重叠几何问题，通过隐式梯度投影有效平衡知识复用与任务特异性学习，为视觉-语言模型的持续高效适应提供了新范式。
...
TL;DR: 提出了一种将3D高斯优化问题形式化为全局几何分布匹配的新框架，通过多视图贡献排序、OT聚合和OT密集化三个组件，实现了仅用10%基元达到SOTA渲染质量，训练速度提升10倍。


<details>
  <summary>Details</summary>
Motivation: 3DGS存在大量冗余基元导致显著开销，现有压缩方法依赖局部采样或固定剪枝阈值，难以在冗余减少与高质量渲染间取得平衡。

Method: 提出全局几何分布匹配框架，包含三组件：(1)多视图3D高斯贡献排序机制，使用几何一致性过滤基元；(2)基于全局最优传输(OT)的聚合算法，合并冗余基元同时保留几何；(3)OT密集化算子，保持高斯分布属性以实现稳定优化。

Result: 仅用10%基元达到SOTA渲染质量，训练速度比vanilla 3DGS加速10倍。

Conclusion: 该工作通过将高斯优化重新定义为全局几何分布匹配问题，有效解决了3DGS的冗余压缩难题，在保持渲染质量的同时大幅降低了基元数量和训练开销。
...
TL;DR: 论文提出MetaRA框架，将元形态测试概念应用于多模态大语言模型的视觉问答任务评估，通过生成受控的输入变体来系统性探测模型漏洞。实验证明该方法能揭示传统准确率指标无法发现的多种失败模式，为可信赖的多模态AI评估提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要依赖静态数据集和基于准确率的指标，无法捕捉模型的鲁棒性、一致性和泛化能力。这些传统的评估方式存在局限性，难以全面发现多模态大语言模型在VQA任务中的潜在缺陷。

Method: 提出元形态鲁棒性评估(MetaRA)框架，采用元形态关系(MRs)来系统性地探测基于多模态大语言模型的VQA系统的漏洞。MetaRA根据特定的MRs生成受控的图像-问题输入变体，并在不同条件下评估多个模型。

Result: 实验结果表明，MetaRA比传统准确率指标提供了更丰富的诊断洞察，成功揭示了多种细微的失败模式，包括：对语言扰动的敏感性、对浅层视觉线索的过度依赖，以及多模态推理能力的深层弱点。这些失败模式在标准基准测试中往往被隐藏。

Conclusion: MetaRA框架作为一种可扩展的、与模型无关的方法，能够揭示传统基准测试中隐藏的失败模式，为可信赖的多模态AI评估提供了新的思路。该方法不仅能识别语言扰动的敏感性，还能发现模型对浅层视觉线索的过度依赖以及多模态推理的深层弱点。
...
TL;DR: 本文提出SWEET框架,创新性地将图像编辑模型作为稀疏视觉世界模型替代传统视频生成方法,通过逐步生成任务相关关键帧来实现机器人操纵的视觉规划,并结合扩散动作预测器将想象的关键帧转换为可执行动作。


<details>
  <summary>Details</summary>
Motivation: 密集视频生成本身计算昂贵,且对于许多操纵任务来说并不必要,因为任务进展可以用少量任务相关的视觉状态(summary)来描述。之前没有人系统研究过图像编辑模型是否可以作为稀疏视觉世界模型用于机器人操纵。

Method: 1)首先在相同机器人数据设置下对比视频生成模型Wan2.2与图像编辑模型FLUX-Kontext;2)提出SWEET框架,通过连续图像编辑逐步生成任务相关操纵关键帧,以语言指令和可选的箭头式空间引导为条件;3)使用目标条件扩散动作预测器将相邻想象关键帧转换为可执行动作块;4)引入混合训练策略,通过过滤编辑目标减少真实与编辑视觉子目标之间的不匹配。

Result: 实验在DROID和RoboMimic数据集上表明:1)图像编辑比视频生成产生更可靠的任务级关键帧,具有更好的视觉保真度且推理成本显著降低;2)SWEET在可见和未见场景下改善了关键帧预测;3)实现了从顺序关键帧规划到可执行机器人动作的完整pipeline。

Conclusion: 图像编辑是一个有前景但尚未充分探索的嵌入式视觉预测方向,为机器人操纵中的稀疏视觉世界建模提供了新思路。
...
TL;DR: 本文将文本渲染问题重新定义为后训练偏好对齐任务，提出TextAlign框架，利用分层VLM将渲染错误分解为多层次并生成偏好信号以支持GRPO/DPO优化，在多个模型上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 大型文生图模型在文字渲染方面存在持久弱点，需要同时遵循语义指令和精细的字形结构。现有的方法通常通过架构特定的模块或编码器修改来改进，但这使得在不同基础模型上部署变得复杂。

Method: 提出TextAlign，一种非侵入性框架，保持生成器架构不变。核心组件是基于分层视觉语言模型(VLM)的奖励，将渲染错误分解为全局、单词和字形级别，然后，将二进制缺陷判断转换为标量偏好信号。该信号支持_GROUP Relative Policy Optimization（GRPO）和Direct Preference Optimization（DPO）。

Result: 在FLUX.1-dev和Z-Image-Turbo上的实验表明，OCR文本准确性持续提升，且不损害整体生成质量。与包括SD3.5、Qwen-Image、AnyText和TextDiffuser在内的强大基线模型相比，该方法显示了奖励设计作为模型重新设计的可扩展替代方案的潜力。

Conclusion: 通过将文本渲染视为偏好对齐问题而非架构改造问题，提供了一种通用且可扩展的解决方案。分层VLM奖励设计能够有效捕捉不同粒度的渲染错误，GRPO和DPO两种优化策略均适用，且无需修改基础模型架构，具备良好的可迁移性。
...
TL;DR: DynaTok是一种无需训练的、时间自适应且偏置感知的视频语言模型token压缩框架,通过时序预算分配(TBA)和空间预算分配(SBA)模块,在保持超过95%基线准确率的同时实现90%的token削减。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs处理长视频序列时产生海量visual tokens,导致极高的计算成本。现有无训练token压缩方法仅基于注意力大小选择tokens,忽视位置偏差且仅考虑短期时序局部性,导致时空冗余覆盖和token使用效率低下。

Method: 提出DynaTok框架,包含两个核心模块:1)TBA通过轻量级指数移动平均(EMA)存储器动态分配token预算,给冗余帧分配更少tokens,给新颖帧分配更多tokens,捕捉长期时序变化;2)SBA使用激活式注意力图选择空间多样且语义重要的特征,利用空间存储器减少先前选中区域的冗余并缓解位置偏差。该框架可无缝集成到LLaVA-OneVision和LLAVA-Video等现有Video-LLMs中。

Result: 在四个代表性VideoQA基准(MVBench、LongVideoBench、MLVU、VideoMME)上的实验表明,DynaTok在90%token削减下仍保留超过95%的基线准确率,优于现有的无训练压缩方法。

Conclusion: DynaTok为高效且稳健的视频推理提供了原则性基础,为未来Video-LLMs实现实时流式视频理解铺平了道路。
...
TL;DR: RE-VLM是首个双流视觉语言模型，联合利用RGB图像和事件流进行鲁棒的场景理解。通过并行RGB和事件编码器以及渐进式训练策略，在光照挑战条件下显著优于现有的RGB-only和事件-only模型。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型在低光、高动态范围或快速运动等不利条件下表现不佳，因为标准RGB图像在此类环境中会退化。事件相机提供了一种互补模态，能够以高时间分辨率和宽动态范围异步记录每像素亮度变化。

Method: 提出RE-VLM双流视觉语言模型，采用并行RGB和事件编码器，结合渐进式训练策略对齐异构视觉特征与语言。为解决RGB-事件-文本监督数据稀缺问题，提出图驱动管道将同步的RGB-事件流转换为可验证的场景图，从中合成字幕和问答对。

Result: 构建了两个数据集：PEOD-Chat（针对光照挑战场景）和RGBE-Chat（覆盖多样化场景）。在字幕生成和VQA基准测试中，RE-VLM始终优于参数量相当的SOTA RGB-only和事件-only模型，在挑战性条件下获得尤其显著的提升。

Conclusion: 事件增强的VLMs能够在各种真实世界环境中实现鲁棒的视觉语言理解，为多模态感知提供了有效途径。
...
TL;DR: 本文针对跨域小样本分割(CD-FSS)任务提出HERA框架，利用视觉基础模型(VFM)处理领域偏移下的新类别分割问题。该方法通过三层选择-正则-校准pipeline：层次化层选择(HLS)使用示例迁移风险(ETR)自适应选择最优VFM层；先验引导正则化(PGR)正则化选定表示的交互；像素级自适应校准(PAC)结合选定表示与精炼交互图进行像素预测校准。方法在测试时仅微调不到2.7%的参数，在多个CD-FSS基准上超越SOTA超过4.1mIoU。


<details>
  <summary>Details</summary>
Motivation: 跨域小样本分割面临两大核心挑战：(1)新类别标注样本有限，相较于VFM预训练规模而言极易过拟合；(2)目标领域偏移在预训练中未被充分覆盖，导致跨域不一致性和层间敏感性。当前方法难以有效利用冻结VFM特征且适应新领域。

Method: 提出Hierarchical Exemplar Representation Adaptation (HERA)框架，包含三个阶段： (1)层次化层选择(HLS)：设计数据依赖的示例迁移风险(ETR)计算每个候选层的可迁移性，自适应识别最具信息量的VFM层； (2)先验引导正则化(PGR)：对选定表示的交互进行正则化，产生结构化的局部信号； (3)像素级自适应校准(PAC)：将选定表示与精炼交互图结合，校准像素级预测生成一致掩膜。三阶段形成层次化选择-正则-校准pipeline，在新领域引导冻结VFM特征，测试时微调参数少于2.7%。

Result: 在多个CD-FSS基准数据集上进行了广泛实验，HERA方法超越了当前最先进方法超过4.1 mIoU。

Conclusion: 本文提出的HERA框架成功解决了跨域小样本分割中标注样本有限和领域偏移带来的挑战。通过三层select-regularize-calibrate pipeline设计，实现了仅微调少量参数即可有效适应新领域，为基于VFM的少样本学习提供了新的有效范式。
...
TL;DR: 本文提出SLVR（语义增强的潜在视觉推理）框架，用于在紧凑的潜在空间中进行视觉推理。该方法通过两阶段学习丰富潜在表示的语义，并将其与多样化推理目标对齐。实验表明SLVR提升了潜在视觉推理的鲁棒性和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 现有 multimodal latent-space reasoning 方法主要依赖视觉监督，生成的潜在表示缺乏足够的语义丰富度，限制了其在多样化区域级推理任务中的应用能力。

Method: 提出两阶段学习框架SLVR：第一阶段在细粒度属性监督下学习语义丰富的以区域为中心的潜在表示；第二阶段设计Multi-query Group Relative Policy Optimization (M-GRPO)将同一区域基于的多个查询的潜在表示对齐。同时构建SLV-Set数据集（约40万区域级属性注释和80万多样查询问答样本），并引入SV-QA基准测试来评估语义变化下的潜在推理。

Result: 实验证明SLVR相比现有基线方法，提升了潜在视觉推理的鲁棒性和语义一致性。

Conclusion: SLVR框架成功地将属性级视觉语义融入潜在表示，并将其与多样化推理目标对齐，在区域级推理任务中展现出更优的性能表现。
...
TL;DR: 本文提出MAM-CLIP，一种利用对比学习结合乳腺X线图像与图谱文本的多模态模型，通过预训练吸收文本信息后在少量标记样本下微调进行BI-RADS分类，在1K样本时可获得14%的性能提升，证明图像-文本对的预训练策略在小样本场景下十分有效。


<details>
  <summary>Details</summary>
Motivation: 深度学习在从乳腺X线照片预测BI-RADS评分方面显示出良好的前景。然而，这些图像的解释可能存在差异，即使在放射科医生之间也是如此。由于乳腺X线照片的复杂性，仅基于图像标签训练分类模型往往性能有限。

Method: 作者策划了2313张乳腺X线照片及其来自两个乳腺X线图谱的对应标题。提出的方法使用多模态模型，采用预训练的PubMedBERT作为语言组件。通过对比学习训练图像-文本对，使视觉编码器能够吸收标题中包含的丰富信息。然后在两个数据集上微调视觉编码器进行BI-RADS预测。

Result: 与未经此预训练的模型相比，实现了卓越的性能。在3类平均F1分数方面改进从+1%到+14%不等：使用40K训练样本增加+1%，使用1K样本增加+14%。实验表明，来自乳腺图谱的2K图像-文本对可能比2K标记样本更具信息量，在有超过10K训练样本时平均边际为+1.1%。

Conclusion: 提供了一个用于乳腺X线检查的视觉-语言模型，突出了来自乳腺图谱的文本信息的价值。公开释放了TEKNOFEST数据集的预处理乳腺X线照片、训练代码、预训练模型权重和数据提取脚本。
...
TL;DR: 提出一种混合数字-模拟深度伪造视频检测框架，结合轻量级数字前端与空间复用光 学解码后端，通过可编程空间光调制器实现大规模并行模拟推理，单次光学传播即可同时处理15个或更多视频流。


<details>
  <summary>Details</summary>
Motivation: AI生成视觉媒体的快速普及迫切需要高效、可信的深度伪造检测系统，但现有基于深度学习的检测方法计算密集、能耗高，限制了其可扩展性。

Method: 构建混合数字-模拟深度伪造视频处理器：轻量级数字前端结合空间复用光学解码后端，利用可编程空间光调制器进行大规模并行模拟推理。在可见光波段实验装置上测试不同数据集（经典换脸、真实世界深度伪造、全AI生成视频）。

Result: 在Celeb-DF数据集上实现平均深度伪造检测准确率97.79%、灵敏度99.86%、特异度95.72%；单次光学传播可并行处理15个视频；对视频退化、噪声、压缩、实验失调和黑盒对抗攻击具有鲁棒性。

Conclusion: 光学计算集成到AI推理中可同时实现吞吐量、能量效率和对抗鲁棒性的提升，这三个特性在纯数字系统中难以兼顾。为高效的深度伪造检测提供了新的技术路径。
...
TL;DR: 本文提出了Heat Dissipation Flow Matching (HDFM)，一种将连续模糊（热耗散）过程集成到Flow Matching中的新框架，以注入多尺度先验信息。HDFM通过插值热耗散路径解决逆问题的不适定性，并采用x-prediction来缓解高维回归困难。实验表明该方法在多个数据集上表现优于大多数基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型大多依赖噪声进行腐蚀和解噪，而模糊基模型虽然能更好地保留颜色预算和多尺度细节，但仍然基于SDE框架，未能集成到ODE框架（如Flow Matching）中。同时，逆热耗散过程面临不适定挑战，且从高维噪声空间回归模糊图像也存在困难。

Method: 提出Heat Dissipation Flow Matching (HDFM)方法，将连续模糊（热耗散）过程引入FM框架以注入多尺度先验；通过插值热耗散路径对齐来解决不适定性问题；采用x-prediction策略来缓解高维回归难度。

Result: 玩具实验和消融研究表明HDFM一致性地受益于blur和x-prediction；在所有数据集上，HDFM的性能优于大多数基线方法。

Conclusion: HDFM成功地将模糊机制与Flow Matching框架相结合，通过引入多尺度先验解决了传统方法的局限性，为图像生成任务提供了一种更有效的解决方案。
...
TL;DR: CoNNS是一个概念引导的噪声负样本抑制框架，用于胸部X光与放射报告的视觉-语言对齐任务。该方法通过构建层级概念本体来解决传统对比学习中不同患者数据被视为负样本带来的语义模糊问题，并在零样本分类和定位任务上超越了现有最佳模型。


<details>
  <summary>Details</summary>
Motivation: 标准对比学习方法将不同患者的X光片和报告简单视为负样本对，但不同患者常表现出相似的影像发现，这种假设产生噪声负样本，导致语义模糊并在零样本理解任务中表现下降。

Method: 1)使用大语言模型构建包含41个关键临床概念的层级概念本体，明确建模存在性、属性(位置和特征)和文本(证据片段和存在陈述)；2)实现跨患者配对重标记策略：细粒度分解按发现存在性分类配对、噪声负样本过滤去除假负样本解决语义冲突、硬负样本挖掘使用轻量级语言模型识别细微属性差异；3)提出概念感知的NCE损失在_align视觉特征与文本的同时抑制噪声负样本。

Result: 在多粒度零样本定位任务和5个零样本分类数据集上进行广泛实验，验证了CoNNS优于现有最先进模型。

Conclusion: CoNNS通过概念引导的抑制机制有效解决了视觉-语言对齐中的噪声负样本问题，在多个零样本任务上实现了卓越性能。
...
TL;DR: 本文系统诊断了Token-Choice稀疏MoE在视频Diffusion Transformers上的训练失败模式，从约50亿参数的预训练密集模型转换为MoE架构，发现了五个层级的失败模式：线性路由器的全局软饱和、MLP路由器的选择性死锁、交叉注意力路由器的初步自恢复、死亡层的U形分布、以及bfloat16混合精度导致的权重截断问题。基于6500万token和5000训练步的路由决策时间序列，提出了功能冗余假设。


<details>
  <summary>Details</summary>
Motivation: 研究为何Token-Choice稀疏MoE在视频Diffusion Transformers上训练失败，系统诊断失败模式并理解其根本机制

Method: 从预训练密集模型按三定律转换为MoE架构，通过分析6500万token跨越5000训练步的路由决策时间序列，识别五种失败模式并验证功能冗余假设

Result: 发现五个层级失败模式：(1)线性路由器全局软饱和导致专家同质化 (2)MLP路由器约1/3层退化单专家模式 (3)交叉注意力路由器9层持续死锁 (4)死亡层呈U形分布在浅层视觉处理和深层语义集成 (5)bfloat16精度导致小权重更新截断；提出功能冗余假设解释死锁为门-共享专家-路由专家三联系统的理性等待策略

Conclusion: 总结了密集到MoE转换的三定律和bfloat16精度陷阱的完整解决方案，标定当前Token-Choice范式的能力边界，并提出从视觉统一到世界模型的三步进化路线图
...
TL;DR: MatPhys是一个材料感知的前馈框架，用于从单视角视频预测弹簧-质量参数，重建可仿真变形的数字孪生对象。该方法解决了现有物理驱动方法的两个基本限制：假设均匀材质和跨场景参数不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有物理驱动的重建方法存在两个根本性缺陷：(1)假设整个物体为同质材料，(2)由于单目观察的固有歧义性和场景特定的逆优化，同一材质在不同场景或交互中产生不一致的参数。这导致重建的可变形对象难以复用和泛化。

Method: 提出两个耦合设计：一是利用DINO特征将对象分解为语义上有意义的部分，并为每个部分查询对应的材料先验，实现非均匀材质假设；二是引入学习到的共享材料嵌入码本作为外观与物理之间的桥梁，并使用部分级先验约束解码器，确保相同材质在不同场景和交互中产生一致参数。

Result: 实验表明，该方法在重建和未来预测任务上与每场景优化基线相当，同时在 unseen interactions 和未见过的对象上展现出更强的泛化能力，并获得更一致的物理参数。

Conclusion: 通过将欠约束的单目问题转化为基于共享可复用材料概念的前馈推理，MatPhys成功地实现了材料感知的可变形对象重建，为计算机视觉、图形学和机器人学提供了新的解决方案。
...
TL;DR: 本文提出了轨迹 grounding 的多轮时空对话任务，并构建了 Track4D-Bench 基准和 LMM-Track4D 模型，以增强大型多模态模型在 4D 时空动态推理方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态模型虽然在图像和视频理解上表现进步，但在维持 4D 连续时空动态推理方面仍存在能力差距。

Method: 提出 Track4D-Bench 基准（526 个clip级别对话样本，涵盖23.5k帧和7.5k目标注释）；构建 LMM-Track4D 模型，包含：RTGE（射线-时间几何编码）用于流式状态token，OSK-RA 解码器（目标槽 kinematic + 残差锚点）用于4步3D状态估计。

Result: 在 Track4D-Bench 上的一致性改进表明，显式动态状态建模是激发 LMMs 4D 动态推理的有效设计原则。

Conclusion: 通过显式动态状态建模可以有效提升大型多模态模型的4D时空推理能力，为该领域后续研究提供了新思路和基准。
...
TL;DR: 研究提出 NIR（神经元关联重分布）方法，通过正则化预测概率加权的 Penultimate 层神经元平均激活方差，在无需人口统计学标签的情况下，减少医学图像分类模型在年龄、性别和种族等人口统计学属性上的子群性能差异。实验证明该方法在 HAM10000 和 Harvard OCT-RNFL 数据集上显著降低了 TPR/FPR 差异，同时保持或提升 AUC 性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习医学图像分类模型在跨人口统计学属性（如年龄、性别、种族）时存在子群性能差异，导致对某些人群过度诊断或诊断不足。现有方法需要敏感属性标签，且缺乏对这种 Disparity 形成机制的深入理解。

Method: 识别了迁移学习模型中的一个潜在表征机制：在正预测中主导的 Penultimate 层通道与疾病阳性样本和优势人群（男性、老年患者）共同激活，导致过度诊断；在负预测中主导的通道则与弱势群体（女性、年轻患者）共同激活，导致诊断不足。基于此，提出 NIR（神经元关联重分布）方法，该方法惩罚预测概率加权的 Penultimate 层神经元平均激活方差，无需训练时的 demographic 标签。

Result: 在 HAM10000 数据集上，年龄组 TPR 差异从 10.81% 降至 0.93%，性别 TPR 差异从 12.04% 降至 0.74%，AUC 提升 0.51 点。在 Harvard OCT-RNFL 数据集上，种族 FPR 差异从 15.68% 降至 10.66%，年龄 FPR 差异从 12.69% 降至 1.80%。

Conclusion: 将潜在的疾病证据分布在整个 Penultimate 层是提高医学 AI 人口统计学公平性的有效原则性策略。NIR 作为一种轻量级正则化方法，无需敏感属性标签即可显著改善公平性指标，为构建更公平的医学诊断系统提供了新思路。
...
TL;DR: 本文提出VASA（Vision-guided Ad-hoc Segmentation Agent），首个无需训练的开放概念分割智能体。VESA结合VLM智能体、分割基础模型和视觉引导工作流，通过持久工作掩码进行推理、构建和验证解决方案。


<details>
  <summary>Details</summary>
Motivation: 分割对于已知概念（存在预学习掩码）相对容易，但对于开放随意概念（grounding非预学习掩码，需通过图像证据构造）仍具挑战性。现方法仅依赖文本提示修订，无法处理开放概念分割。

Method: 提出训练免费的VASA框架：1）耦合VLM智能体+分割基础模型+视觉引导工作流；2）使用持久工作掩码进行推理、构建和验证；3）规划视觉操作、调用分割工具、检查结果、编辑掩码并从错误中恢复。构建PARS基准：将PartImageNet部件级标签转化为开放概念定义查询。

Result: 在PARS上，VASA优于所有基线方法，比SAM3 Agent高14-25%。在RefCOCOm标准多粒度指代分割基准上，比SAM3 Agent高5-9%，比其他智能体基线高高达20%。

Conclusion: 验证了开放概念分割的智能体视觉构建方法。指出AI智能体发展新路径：不仅将基础模型包装为工具，而是通过任务知识、VLM行为、视觉例程、工作记忆和故障感知工作流对其进行编程。
...
TL;DR: 本文提出了一种针对预训练编码器的目标性下游无关攻击（TDAA）方法，通过引入"威胁图像"作为特征级锚点，生成示例特定的对抗扰动，使受害编码器对DAE和威胁图像输出相同特征。


<details>
  <summary>Details</summary>
Motivation: 现有DAA方法在宽松威胁模型下工作，只需改变预测即可；作者认为需要更严格的 Targeted DAA，要求攻击既是目标性的又是下游无关的，但实现难度大因为下游任务未知且编码器不直接产生预测。

Method: 设计了一个生成器，为每个图像生成示例特定的对抗扰动，通过威胁图像（有预选的目标）作为特征级锚点，迫使受害编码器对DAE和威胁图像输出相同的特征表示。

Result: 在10种自监督方法和3个基准数据集上进行广泛实验，证明了方法的有效性并揭示了预训练编码器的显著脆弱性。

Conclusion: TDAA方法能够在不知道下游任务的情况下实现针对性攻击，揭示了预训练编码器存在严重的安全隐患。
...
### [39] [EventPrune: Cascaded Event-Assisted Token Pruning for Efficient First-Person Dynamic Spatial Reasoning](https://arxiv.org/abs/2605.19506)
*Pengtao Ma,Ziliang Zhou,Ciyu Ruan,Haoyang Wang,Kaiyuan Li,Zihang Gong,Wenhua Ding,Chen Gao,Jingao Xu,Xinlei Chen*

Main category: cs.CV

TL;DR: 本文提出Event Cascade Pruning (ECP)，首个利用事件相机高频运动线索引导token选择的无训练框架，用于降低视频语言模型的计算成本同时保留运动和几何推理能力。


<details>
  <summary>Details</summary>
Motivation: 第一人称动态空间推理需要追踪连续运动和精确几何结构，但Transformer视频语言模型的二次注意力成本使得密集视觉token计算昂贵。现有token剪枝范式主要依赖离散静态快照，无法保留推理所需的运动和几何线索。

Method: ECP包含三阶段：1) Event-Triggered Causal Sampling事件触发因果采样，用于锚定运动信息关键帧；2) Event-guided Motion Saliency Filtering事件引导运动显著性滤波，用于抑制事件非活动视觉token；3) Event-Attention Ranking Fusion事件注意力排名融合，用于校准空间注意力与运动显著动态。

Result: 在80%视觉token减少下，ECP优于全token基线（37.62% vs. 36.31%），实现1.89倍推理加速和52% GFLOPs减少。在新引入的真实世界RGB-event基准ESR-Real上，ECP比全token基线提高2.68个百分点。

Conclusion: ECP是首个无训练的事件相机驱动token剪枝框架，成功实现了高效计算与推理精度的平衡，为第一人称动态空间 reasoning提供新思路。
...
### [40] [Return of Frustratingly Easy Unsupervised Video Domain Adaptation](https://arxiv.org/abs/2605.19510)
*Pengfei Wei,Yiqun Sun,Zhiqiang Xu,Yiping Ke,Lawrence B. Hsieh*

Main category: cs.CV

TL;DR: MetaTrans是一种针对无监督视频域适应（UVDA）的简便方法，通过仅包含两个基本损失项的学习目标来处理跨域视频的空间和时间分歧问题。核心创新在于 temporal-static subtraction 模块，分别处理空间和时间差异。实验表明在跨域动作识别任务上取得了显著的适应性能提升。


<details>
  <summary>Details</summary>
Motivation: 无监督视频域适应（UVDA）是一个实用但尚未被充分探索的问题。现有的域适应方法通常比较复杂，需要多个损失项或复杂的对齐机制。该论文的动机是提出一种简单有效的UVDA方法，能够处理视频中空间和时间差异的分歧问题。

Method: MetaTrans采用了一个简洁的学习目标，仅包含两个基本损失项。核心通过模型架构设计实现对空间和时间分歧的分别处理。具体使用temporal-static subtraction模块来有效去除空间和时间分歧。

Result: 在各种跨域动作识别任务上的广泛实证评估显示，MetaTrans实现了显著的绝对适应性能提升，并且与最新的UVDA基线方法相比，具有显著优越的相对性能增益。

Conclusion: 该论文提出了一种简单而有效的UVDA方法MetaTrans，通过精妙的模型架构设计分别处理空间和时间分歧。这种"令人沮丧的简单"方法展示了在处理视频域适应问题时的创新思路，在跨域动作识别任务上取得了state-of-the-art的性能表现。
...
### [41] [Are Watermarked Images Editable? SafeMark for Watermark-Preserving Text-Guided Image Editing](https://arxiv.org/abs/2605.19511)
*Xiaodong Wu,Qi Li,Xiangman Li,Zelin Zhang,Lingshuang Liu,Jianbing Ni*

Main category: cs.CV

TL;DR: SafeMark是一个用于水印保护的文本引导图像编辑框架，通过在扩散编辑器的训练目标中添加阈值化水印解码损失，使语义有效的编辑同时保留嵌入的水印。


<details>
  <summary>Details</summary>
Motivation: 一个基本但尚未探索的问题：水印图像是否可以保持可编辑性而不损害水印完整性？

Method: 提出SafeMark框架，在扩散编辑器的训练目标中直接添加阈值化水印解码损失，微调编辑器使得语义有效的编辑也能在最终输出中保留嵌入的水印。

Result: SafeMark在多种数据集、文本引导编辑方法和后编辑扭曲设置下实现了高水印位精度，同时保持高质量的语义编辑，且不牺牲对常见后编辑扭曲的鲁棒性。

Conclusion: 语义可编辑性和水印完整性是根本兼容的，能够在生成编辑管道中实现可信的图像来源追踪。
...
### [42] [Dual-Prompt CLIP with Hybrid Visual Encoders for Occluded Person Re-Identification](https://arxiv.org/abs/2605.19527)
*Zhangjian Ji,Shaotong Qiao,Kai Feng,Wei Wei*

Main category: cs.CV

TL;DR: 本文提出了DPL-ReID模型，用于解决遮挡行人重识别问题。该方法结合了双提示学习策略、双向门控特征融合模块和真实世界遮挡增强技术，在多个基准数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的行人重识别方法仅关注提示学习，忽视了遮挡物的语义信息。遮挡会破坏身体区域线索，增加跨视图匹配的难度。因此需要一种能够捕获完整行人语义并对遮挡保持鲁棒性的方法。

Method: 提出三个核心创新点：1）双提示学习策略(Dual-PL)，利用文本线索捕获完整行人语义并保持对遮挡的鲁棒性；2）真实世界遮挡增强方法(RWOA)，模拟真实场景中的遮挡情况以丰富训练样本；3）加权门控特征融合(WGFF)方法，结合LSNet捕获全局信息作为特征门控机制，有效引导CLIP视觉编码器生成更全面的特征表示。

Result: 在多个基准遮挡行人重识别数据集上进行广泛实验，表明DPL-ReID达到了最先进的性能。

Conclusion: 通过双提示学习策略和加权门控特征融合机制，DPL-ReID能够有效处理遮挡行人重识别任务，在性能和鲁棒性方面均取得了显著提升。
...
### [43] [Towards Camera-Robust 3D Localization: Equation-Anchored Tool-Use for MLLMs](https://arxiv.org/abs/2605.19528)
*Xueying Jiang,Wenhao Li,Quanhao Qian,Deli Zhao,Shijian Lu,Gongjie Zhang,Ran Xu*

Main category: cs.CV

TL;DR: 提出一种方程锚定的工具使用框架，将空间工具作为公式变量，在思维链中显式写出针孔反投影方程并代入工具输出，解决多模态大语言模型中相机内在参数不确定性问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D定位任务受限于相机内在参数不确定性：同一图像在不同相机下可对应不同3D场景。现有MLLMs忽略相机参数或将其作为可选参考线索，导致相机信息无法确定性传播到预测结果。

Method: 提出方程锚定工具使用框架：(1)将空间工具重新用作公式变量而非参考线索；(2)主动检索相机内在参数并采样多点度量深度；(3)在Chain-of-Thought中显式写出针孔反投影方程；(4)将工具输出代入公式后回归9-DoF边界框。

Result: 在0.5×至1.5×重新缩放相机内在参数的3D目标检测和3D视觉定位任务上，性能优于RGB-only和工具增强基线；在相机偏离训练尺度最大时提升最为显著。

Conclusion: 通过显式公式化相机参数与工具输出的数学关系，实现了相机信息的确定性传播，为MLLMs的3D定位提供了新范式。
...
### [44] [Boosting Text-to-Image Diffusion Models via Core Token Attention-Based Seed Selection](https://arxiv.org/abs/2605.19532)
*Yunzhe Zhang,Hongfu Liu,Pengyu Hong*

Main category: cs.CV

TL;DR: 本文提出注意力基础种子选择(ABSS)方法，通过分析文本到图像扩散模型在前几步去噪过程中对核心词汇的跨注意力动态来预测和选择最佳随机种子，从而提升生成质量。该方法无需训练，可作为现有种子优化管道的轻量级预选组件。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型的输出对随机种子高度敏感，不同种子会产生图像质量和提示对齐的大幅差异。现有方法需要大量生成或额外训练来优化种子Selection，缺乏高效prediction指标。

Method: 核心观察：在前几步去噪过程中，注意力对提示核心词汇(内容词)的跨注意力动态可强预测最终生成质量。ABSS方法：1)为给定提示生成多个候选噪声种子；2)在去噪初期测量跨注意力到核心词汇的分布；3)对所有种子进行评分和排序；4)只保留top-k个进行完整生成；5)无需固定阈值。

Result: 在三个基准上实验表明，ABSS为Stable Diffusion变体带来一致的文本图像对齐和视觉质量改进，得到人类偏好和对其度量的验证。

Conclusion: 该工作揭示了去噪早期注意力模式与最终生成质量之间的强相关性，为扩散模型的可控生成提供了新视角。ABSS作为无需训练的即插即用组件，可与其他种子优化管道结合实现额外增益。
...
TL;DR: Trust3R是一个用于前馈3D重建的轻量级证据不确定性框架，结合门控残差均值细化和正态逆Wishart证据头，产生封闭形式的多变量Student-t分布，用于逐点几何不确定性感知。实验表明在ScanNet++上AURC降低25%，AUSE降低41%。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型的预测置信分数是启发式的，缺乏概率解释，无法准确指示预测结果的可信任程度和区域。

Method: 提出Trust3R框架，融合门控残差均值细化与正态逆Wishart证据头，产生封闭形式的多元Student-t分布来估计逐点几何不确定性，同时保持适度的推理开销。

Result: 在室内外多样式基准数据集上，Trust3R一致性地改善了风险覆盖和稀疏化性能，提高了geometric accuracy，在不确定性排序方面更强，在ScanNet++上实现了25%更低的AURC和41%更低的AUSE。

Conclusion: Trust3R通过证据学习方法提供了概率上有根据的点图不确定性估计，在不显著增加推理成本的情况下，为下游几何管道中的不确定性感知加权提供了实用的可靠性信号。
...
TL;DR: 提出了EgoCoT-Bench，一个针对自我中心视频的可验证操作中心推理基准测试，包含3172个可验证QA对，涵盖351个自我中心视频，分为4个任务组和12个子任务组


<details>
  <summary>Details</summary>
Motivation: 现有自我中心视频基准测试缺乏有限的根基推理评估，支持细粒度操作中心推理不足，很少检查模型推理是否建立在明确的时空证据基础上

Method: 通过时空场景图(STSG)引导生成框架构建基准测试，并由人工注释者进一步细化以确保正确性、自我中心相关性和细粒度质量

Result: 实验结果表明，自我中心细粒度推理仍然存在持续困难，且许多多模态模型产生的解释答案正确但证据与答案不一致

Conclusion: 希望EgoCoT-Bench能作为自我中心视频理解中可验证推理的有用测试平台
...
TL;DR: 该论文提出Lens Privacy Sealing (LPS)硬件方案，通过可调节层压膜物理遮挡相机镜头实现拍摄前隐私保护，并构建P³AR数据集和MSPNet模型进行隐私保护下的动作识别。MSPNet结合帧间噪声抑制器、跨帧语义聚合器和对比语言-图像预训练，几乎将动作识别准确率翻倍，同时将身份识别抑制到低水平，在隐私-效用权衡上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于RGB相机的监控系统用于公共安全和医疗保健中的人类动作识别，但引发严重的隐私担忧。现有软件方法无法在数据采集阶段保护隐私，存在根本性漏洞。

Method: 提出LPS硬件方案使用随机多层散射的层压膜物理遮挡镜头；构建包含大规模重放采集(P³AR-NTU，114K视频)和真实世界采集(P³AR-PKU)子集的P³AR数据集及隐私属性标注；设计单阶段MSPNet框架，包含帧间噪声抑制器(IFNS)、跨帧语义聚合器(CFSA)，并结合对比语言-图像预训练增强语义提取鲁棒性。

Result: MSPNet配合IFNS和CFSA几乎将动作识别准确率相比基线方法翻倍，同时将身份识别抑制到低水平。LPS在隐私-效用权衡上优于SOTA硬件方法，能抵抗PSF反转和数据驱动的重建攻击，在不同光学配置和环境条件下具有良好泛化能力。

Conclusion: 所提硬件方案LPS与软件框架MSPNet相结合，首次实现拍摄前传感器级别的物理隐私保护，在动作识别任务上同时保障高可用性和强隐私性，为监控系统的隐私保护提供新范式。
...
### [48] [A novel YOLO26-MoE optimized by an LLM agent for insulator fault detection considering UAV images](https://arxiv.org/abs/2605.19595)
...
TL;DR: 提出一种优化的YOLO26-MoE目标检测架构，将稀疏混合专家模块集成到YOLO26高分辨率分支，用于无人机电力线路绝缘子故障检测。该方法通过工具增强的LLM辅助训练，在绝缘子故障检测任务上达到了较高的检测精度。


<details>
  <summary>Details</summary>
Motivation: 无人机结合深度学习的视觉系统已成为电力线路绝缘子故障检测的有效方案，但检测仍面临挑战：小缺陷区域、异构故障模式、复杂背景和成像条件变化等因素导致检测困难。

Method: 提出优化YOLO26-MoE架构，将稀疏混合专家(MoE)模块集成到YOLO26检测器的高分辨率分支，实现自适应特征细化，保持一阶段检测框架的效率。通过工具增强的大型语言模型协调超参数优化、最终训练和评估。

Result: 模型达到0.9900 mAP@0.5和0.9515 mAP@0.5:0.95，优于最新的YOLO版本。

Conclusion: 提出的模型为基于无人机的绝缘子故障检测提供了有效且可靠的解决方案。
...
TL;DR: 本论文介绍了两个主要用于树木覆盖和死亡分割的开放机器学习数据集(DTE-aerial-train和DTE-aerial-bench)，用于从厘米级航空影像中全球首次实现树木覆盖和死亡联合分割。训练集包含385K张图像块，测试集包含25个全球分布的正射影像。基线模型在所有生物群落中都有显著改善，特别是北方森林，F1分数从0.40提高到0.58，相对提升约45%。


<details>
  <summary>Details</summary>
Motivation: 全球森林正受到气候变化和火灾、害虫、病原体等干扰的日益威胁，亟需对树木覆盖和死亡进行规模化监测。虽然航空影像 detail 是关键数据源，但目前缺乏用于树木覆盖和死亡联合分割的全球代表性、协调一致的 datasets，极大限制了相关研究进展。

Method: 构建了两个开放的机器学习就绪数据集：1) DTE-aerial-train训练集，包含385K张1024x1024像素的图像块，分辨率2.5-20cm，包含树木覆盖和死亡的多类别专家标注和审计的伪标签；2) DTE-aerial-bench基准测试集，包含25个全球分布的正射影像(共525个图块)，具有高质量专家标注。两个数据集涵盖热带、温带、北方和干旱生物群落，覆盖广泛的森林结构和死亡模式。

Result: 使用基准测试集评估，建立了强有力的参考基线，在所有生物群落和尺度上改善了死亡分割，特别是在挑战性区域如北方森林，F1分数从0.40提升到0.58，实现了约45%的相对提升。

Conclusion: 所有数据、模型和代码将根据许可的开源协议公开发布。基准数据集的交互式可视化可在 deadtrees.earth/releases/dte-aerial-bench 获取。该数据集填补了树木覆盖和死亡联合分割领域全球代表性数据的空白，为大规模森林监测提供了重要的数据基础。
...
### [50] [Spectral Integrated Gradients for Coarse-to-Fine Feature Attribution](https://arxiv.org/abs/2605.19607)
*Soyeon Kim,Seongwoo Lim,Kyowoon Lee,Jaesik Choi*

Main category: cs.CV

TL;DR: 本文提出光谱积分梯度(SIG)方法，通过对基线到输入的差异进行奇异值分解(SVD)，按奇异值从大到小的顺序逐步激活奇异分量，实现从粗粒度到细粒度的特征 attribution 生成。该方法有效减少了传统Integrated Gradients方法中的噪声累积问题，在多个图像分类数据集上展现出更清晰的 attribution 可视化效果和更好的定量指标。


<details>
  <summary>Details</summary>
Motivation: 现有Integrated Gradients (IG)方法虽满足良好的公理化性质，但其积分路径的选择对 attribution 质量有显著影响。标准直线路径会同时引入所有输入特征，沿途累积大量噪声，影响解释结果的清晰度和准确性。

Method: 提出光谱积分梯度(Spectral Integrated Gradients, SIG)，利用输入与基线差异的奇异值分解(SVD)来构造积分路径。具体做法是将基线到输入的差异矩阵进行SVD分解，然后按奇异值从大到小的顺序逐步激活各个奇异分量，从而按照从粗到细的顺序引入特征。

Result: 在多种图像分类数据集上的广泛评估表明，SIG生成的 attribution 地图更加清晰干净，噪声显著降低，相比现有的基于路径的 attribution 方法取得了更好的定量性能表现。

Conclusion: 该研究提出了一种基于SVD的路径构造方法SIG，通过奇异值分解将基线到输入的差异分解为多个分量，并按从大到小的顺序逐步激活，从而实现从粗粒度到细粒度的特征 attribution。该方法有效解决了传统IG方法中所有特征同时引入导致的噪声累积问题。
...
### [51] [Inverse Design of Metasurface based Absorbers using Physics Guided Conditional Diffusion Models](https://arxiv.org/abs/2605.19611)
*Vineetha Joy,Jamshed Palai,Satwik Sahoo,Anshuman Kumar,Amit Sethi,Hema Singh*

Main category: cs.CV

TL;DR: 本文提出了一种基于物理引导的条件质量增强扩散框架，用于超表面吸波器的逆向设计。该方法通过特征线性调制(FiLM)将目标反射特性集成到模型中，并嵌入预训练的电磁仿真器实现物理感知正则化。实验表明，该框架在2-18 GHz频率范围内实现了平均光谱均方误差0.0006和带对准精度0.958，且设计生成时间仅需30秒。


<details>
  <summary>Details</summary>
Motivation: 超表面的逆向设计需要满足严格的光谱约束同时保持可制造性。传统迭代优化方法使用全波模拟极其耗时且计算密集。现有的生成方法通常条件保真度有限，且生成的designs包含难以加工的细小或不规则特征。

Method: 提出了一种物理引导的条件质量增强扩散框架。关键创新包括：(1)使用特征线性调制(FiLM)将目标反射特性作为条件信息集成到模型中；(2)嵌入预训练的替代电磁仿真器，引入基于谱级损失函数的物理感知正则化，确保对目标光谱的遵循。

Result: 在2-18 GHz频率范围内生成了不同反射特性的实际可实现超表面。达到了0.0006的平均光谱均方误差和0.958的带对准精度，表明高条件保真度。模型能为同一条件生成多个几何结构，提供多样化的设计选择。生成合适的设计约需30秒，而传统方法在相同计算资源下可能需要数月。实验测量也验证了模型的效率。

Conclusion: 所提出的框架成功实现了超表面逆向设计的高条件准确性和高效率，解决了传统方法耗时长的问题，同时保证了生成设计的可制造性，为工程师提供了多样化的设计 alternatives.
...
### [52] [White-Balance First, Adjust Later: Cross-Camera Color Constancy via Vision-Language Evaluation](https://arxiv.org/abs/2605.19613)
*Shuwei Li,Lei Tan,Robby T. Tan*

Main category: cs.CV

TL;DR: VLM-CC是一种基于视觉语言模型(VLM)反馈的颜色恒常性框架，将颜色恒常问题重新表述为迭代细化过程，而非直接估计光照。通过LoRA微调的VLM评估每次校正后的图像，识别残余色偏并提供定性反馈，反馈映射到残差光照方向用于更新光照估计，实现跨相机的鲁棒颜色恒常性。


<details>
  <summary>Details</summary>
Motivation: 跨相机泛化是颜色恒常性领域的核心难题。基于学习的模型往往过拟合训练相机的色彩响应特性，导致在其他相机拍摄的图像上性能下降。现有方法难以实现跨不同相机设备的通用性。

Method: 提出VLM-CC反馈引导框架，包含迭代修正流程：1)使用当前光照估计进行白平衡并转换为伪sRGB；2)轻量级LoRA微调VLM评估校正图像，识别主要残余色偏并提供定性反馈；3)将反馈映射到残差光照方向(R/G/B)；4)更新光照估计直至收敛。核心思想是将颜色恒常重新定义为迭代感知反馈问题，利用VLM评估替代直接的RGB回归。

Result: 在多个数据集上实现跨相机颜色恒常性的最先进鲁棒性，代码将开源发布于GitHub。

Conclusion: 该工作通过创新性地引入VLM感知反馈机制，有效解决了颜色恒常模型的跨相机泛化问题。这种将传统视觉任务转化为迭代感知反馈问题的思路为其他图像增强任务提供了新范式。
...
### [53] [Bézier Degradation Modeling for LiDAR-based Human Motion Capture](https://arxiv.org/abs/2605.19620)
*Xiaoqi An,Lin Zhao,Jun Li,Chen Gong,Jian Yang*

Main category: cs.CV

TL;DR: 本文提出BMLiCap，一种基于LiDAR的3D人体动作捕捉框架，使用时间可压缩的贝塞尔曲线进行运动建模。该框架包含时间尺度运动变换器(TMT)和多层运动聚合器(MMA)两个核心模块，分别用于多尺度运动曲线预测和自适应融合。在四个基准数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理不稳定输入和严重遮挡时表现不佳，导致姿态预测抖动或失败。准确的动作重建对于自动驾驶和机器人等领域至关重要。

Method: 提出粗到细的BMLiCap框架，使用时间可压缩的贝塞尔曲线建模运动，通过轨迹保持策略减少控制点。设计渐进式运动重建模块，包含时间尺度运动变换器(TMT)预测多尺度运动曲线，以及多层运动聚合器(MMA)自适应融合多尺度曲线恢复详细、时间连贯的姿势。

Result: 在LiDARHuman26M、FreeMotion、NoiseMotion和SLOPER4D四个主流基准数据集上实现了最先进精度和时间连续性，有效补偿了严重遮挡并减少了预测抖动。

Conclusion: BMLiCap通过使用时间可压缩的贝塞尔曲线建模运动，采用轨迹保持策略减少控制点，获得连贯且易于学习的运动表示。在四个主流基准数据集上实现了最先进的精度和时间连续性，证明了其在复杂场景中补偿严重遮挡和减少预测抖动的能力。
...
### [54] [UniRefiner: Teaching Pre-trained ViTs to Self-Dispose Dross via Contrastive Register](https://arxiv.org/abs/2605.19622)
*Congpei Qiu,Zhaoyu Hu,Wei Ke,Zhuotao Tian,Yanhao Wu,Tong Zhang*

Main category: cs.CV

TL;DR: 本文针对Vision Transformer模型中spurious tokens问题，提出更广泛的定义和系统分类，开发UniRefiner框架通过对比学习fine-tune，使模型能自我清除spurious tokens。在ADE20K等数据集上取得显著改进，特别是EVA-CLIP-8B提升9.4% mIoU，为大规模视觉模型的空间应用开辟新途径。


<details>
  <summary>Details</summary>
Motivation: 大型视觉Transformer模型在空间敏感任务中受spurious tokens影响，性能受限。现有方法仅将这些伪影定义为简单的norm outliers，范围过窄，无法满足密集预测任务需求。

Method: 提出UniRefiner通用精化框架，使用对比register来显式隔离和重新分配spurious tokens，采用双目标：(i)对齐图像tokens与filtered正常tokens以保留语义；(ii)对齐register tokens与检测到的spurious tokens以捕获伪影信号。仅需~5k图像微调几个epoch即可精化多种ViTs模型。

Result: 精化后的EVA-CLIP-8B在ADE20K上达到51.9% mIoU（+9.4%），超越DINOv2(49.1%)；零样本分割精度提升高达22%。

Conclusion: 该工作揭示了spurious tokens的更复杂问题，提出了系统分类和表征三种基本类型spurious tokens的方法，通过UniRefiner框架释放现有大规模基础模型的潜在空间能力，为其在空间敏感任务中的更广泛应用奠定基础。
...
### [55] [PrAda: Few-Shot Visual Adaptation for Text-Prompted Segmentation](https://arxiv.org/abs/2605.19623)
*Gabriele Rosi,Fabio Cermelli,Carlo Masone,Barbara Caputo*

Main category: cs.CV

TL;DR: 针对文本提示分割的少样本视觉适应问题，提出PrAda方法，通过学习类别特定原型（融合细粒度像素特征和高级transformer表示），并与文本预测加权融合，在保持零样本能力的同时实现对新领域的强适应。


<details>
  <summary>Details</summary>
Motivation: 图像分割需要大量像素级标注。当前基础模型虽支持文本提示零样本分割，但在专业目标领域性能显著下降。作者发现主要问题是误分类而非掩码生成。为解决这一问题，首次提出文本提示分割的少样本视觉适应新问题。

Method: 提出Prototype Adaptation (PrAda)方法：1)冻结原始文本提示分割模型；2)学习类别特定原型，融合细粒度像素特征和高级transformer表示；3)通过学习的重要性因子将原型与原始文本预测融合；4)实现参数高效适应。

Result: 在语义分割、实例分割、全景分割五种基准数据集上实验表明，PrAda相比现有方法和提出的基线取得显著改进。

Conclusion: PrAda在保持模型零样本潜力的同时，能够有效适应新领域，为文本提示分割的少样本适应提供了新的研究方向。
...
### [56] [Component-Aware Structure-Preserving Style Transfer for Satellite Sim2Real 6D Pose Estimation](https://arxiv.org/abs/2605.19624)
*Yonglong Zhang*

Main category: cs.CV

TL;DR: 本文提出一种组件感知结构保持风格迁移框架，用于解决卫星6D姿态估计中合成到真实数据迁移的问题，通过掩码对齐调制和多种约束实现几何注释保留与外观迁移的平衡。


<details>
  <summary>Details</summary>
Motivation: 非合作卫星的单目6D姿态估计需要大量带注释的训练数据，但获取具有可靠姿态标签和组件级掩码的真实卫星图像非常困难。合成渲染可以提供精确的几何注释，但渲染图像与真实图像之间的外观差距限制了直接迁移到真实域。

Method: 提出了一种组件感知结构保持风格迁移框架。首先从校准的真实采集、ArUco相机姿态测量、CAD渲染和组件掩码构建弱配对真实-合成样本。然后从未标记的真实图像中提取部分级真实域风格代码，并通过掩码对齐调制将其注入相应的合成卫星区域。为了保持生成图像对下游监督的可用性，结合了对抗训练、局部对比一致性、自正则化和边缘保留约束。

Result: 在5000张渲染卫星图像和100张校准采集的真实图像上进行实验。实现了最低的图像分布差异，FID为54.32，KID为0.048。使用翻译数据训练GDRNet时，ADD通过率提高到0.260，AUC提高到0.611。

Conclusion: 组件级外观迁移可以在保留模拟派生几何注释的同时，改善考虑校准设置下的卫星Sim2Real姿态估计。
...
### [57] [P2DNav: Panorama-to-Downview Reasoning for Zero-shot Vision-and-Language Navigation](https://arxiv.org/abs/2605.19634)
*Kai Sheng,Liuyi Wang,Haojie Dai,Jinlong Li,Yongrui Qin,Zongtao He,Chengju Liu,Qijun Chen*

Main category: cs.CV

TL;DR: 本文提出P2DNav，一个用于零样本视觉-语言导航的层级框架。该框架通过全景到局部（P2D）两阶段决策、滑窗对话记忆（SDM）和反思性重定位机制（RRM）解决现有方法中方向推理与局部定位纠缠的问题。在R2R-CE基准上，相比SOTA零样本方法和无航点方法分别实现146.6%和58.9%的成功率增益。


<details>
  <summary>Details</summary>
Motivation: 现有的零样本VLN方法依赖额外航点预测模块，但这些模块通常将高层方向推理与细粒度局部定位纠缠在一起，导致决策容易出错且不稳定。需要一种能够清晰解耦这两个过程的方案。

Method: P2DNav包含三个核心组件：1) P2D显式将导航决策分解为两阶段——先从360°全景中选择指令相关方向，再从该方向的局部RGB图像预测像素级目标点；2) SDM将导航历史组织为多轮对话上下文并用滑窗维护近期视觉观察，支持长程导航；3) RRM基于局部图像评估定位可靠性，必要时返回全景方向选择进行反思性重定位。

Result: 在R2R-CE基准测试中，P2DNav取得strong zero-shot performance。相比SOTA零样本航点方法和无航点方法，分别实现146.6%和58.9%的成功率(SR)增益。

Conclusion: P2D的两阶段分解、SDM的历史上下文组织和RRM的可靠性评估机制共同构成有效的零样本VLN方案，验证了分层决策策略和解耦设计的重要性。
...
TL;DR: 本文提出了一种名为Reason-Reflect-Rectify (R³) 的多轮反思视觉生成框架，旨在解决现有文本到图像模型在处理复杂提示词时无法进行迭代细化的局限性。通过构建包含600多个专家标注样本的R³-Bench基准测试集，以及采用双阶段框架R³-Refiner（结合群体相对策略优化GRPO和层级奖励机制HRM），显著提升了模型的反思推理和修正能力。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像(T2I)模型和多模态大语言模型(MLLMs)依赖于单次生成范式，难以处理需要迭代细化的复杂提示词。虽然现有模型能够识别生成错误，但缺乏生成可执行修正指令的能力，这一关键差距限制了它们在复杂视觉生成任务中的表现。

Method: 1) formal化了Reason-Reflect-Rectify (R³)循环作为核心框架；2) 构建了R³-Bench基准测试集，包含超过600个专家标注实例；3) 提出R³-Refiner双阶段框架，融合群体相对策略优化(GRPO)和层级奖励机制(HRM)；4) 通过可无缝集成到各种MLLMs的方式增强生成质量。

Result: 在R³-Bench评估中，R³-Refiner实现了显著提升：反思判断分数(Reflective Verdict Score)提升12.0%，修正分数(Rectification Score)提升9.0%。同时，该方法在与不同MLLMs集成后，在GenEval++和T2I-CompBench上提升了各类T2I模型的生成质量。

Conclusion: 本文成功弥合了模型在识别生成错误与生成可执行修正指令之间的关键差距。R³框架和R³-Refiner为多轮反思视觉生成提供了有效的解决方案，且具有良好的通用性和可扩展性，可与多种视觉生成模型无缝集成以提升整体性能。
...
TL;DR: Cross-View Splatter是一种前馈方法,通过融合卫星影像和地面照片来预测对齐的Gaussian splats,实现户外场景的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 地面图像采集耗时且难以大规模获取,而卫星影像可通过公共API轻松获取,但单独使用卫星影像无法提供地面视角的细节。现有方法在使用地面图像alone时面临覆盖率不足的问题。

Method: 将正射纠正的卫星视图与GPS标记的地面照片在统一的3D坐标框架中融合,通过对齐鸟瞰图和地面特征表示来预测Gaussian splats。模型在从开放地图服务中挖掘的地理参考数据集和配对的卫星-地面数据上进行训练。

Result: 相比仅使用地面图像,该方法改善了场景覆盖和新视角合成效果,并建立了一个新的地理参考图像新视角合成 benchmark。

Conclusion: 通过跨视角融合卫星和地面 imagery,Cross-View Splatter能够实现更完整的3D场景重建,为大规模城市场景建模提供了有效方案。
...
TL;DR: 针对文档伪造定位模型在实际应用中泛化失败的问题，证明了训练数据中JPEG量化表的多样性是关键因素。使用真实操作量化表（而非标准质量因子）训练可显著提升性能，但仅对能显式摄取量化表的架构有效。


<details>
  <summary>Details</summary>
Motivation: 文档操作定位模型在公开基准上表现良好，但在实际保险文档工作流中失败。研究者发现了一个关键但被忽视的原因：训练时使用的JPEG量化表分布狭窄（仅限于标准libjpeg质量因子），与真实世界保险文档管道中遇到的多样化压缩配置文件不匹配。

Method: 进行了一项对照因子研究，比较两种架构FFDN和Mesorch（量化表感知水平不同），分别在标准质量因子增强(Standard-QT)或来自DocQT（从MAIF操作图像语料库提取的量化表库）的操作校准量化表(Real-QT)下训练，并在三种再压缩条件下评估。

Result: 使用Real-QT训练在DocTamper上获得了显著的定位收益，并在真实操作文档上显著降低了像素级误报率，但仅对于明确摄取量化表作为输入的架构有效。

Conclusion: 标准质量因子增强不能充分代替操作压缩多样性；明确基于量化表调节的架构选择为实际部署提供了有意义的鲁棒性优势。研究还发布了DocQT量化表数据集。
...
### [61] [WBCAtt+: Fine-Grained Pixel-Level Morphological Annotations for White Blood Cell Images](https://arxiv.org/abs/2605.19692)
*Satoshi Tsutsui,Winnie Pang,Shuting He,Bihan Wen*

Main category: cs.CV

TL;DR: 本文提出WBCAtt+，首个密集标注的白细胞图像数据集，包含11种形态属性和5种像素级细胞成分注释，为可解释AI在血液疾病诊断中的应用提供基础。


<details>
  <summary>Details</summary>
Motivation: 现有白细胞数据集仅标注细胞类别，缺乏详细的形态学特征注释，而病理学家在解释细胞时使用这些特征。为支持可解释AI研究，需要提供包含11种形态属性和5种像素级细胞成分的全面标注数据集。

Method: 构建WBCAtt+数据集，包含113k图像级标签和10k分割图；提供属性识别和语义分割的基线模型；设计融入细胞成分结构的属性识别模型。

Result: WBCAtt+是首个提供白细胞图像全面注释的数据集，支持多种应用如可解释AI模型和反事实示例生成。

Conclusion: WBCAtt+数据集填补了白细胞图像详细形态学注释的空白，为病理学家提供更丰富的解释依据，推动可解释AI在医学影像领域的发展。
...
### [62] [Physics-informed simulation framework for realistic sonar image generation and statistical validation](https://arxiv.org/abs/2605.19712)
*Kamal Basha S,Athira Nambiar*

Main category: cs.CV

TL;DR: ACOUSIM是一个物理信息的声纳模拟和验证平台，旨在评估合成声纳图像与真实声纳图像之间的统计对齐，而不依赖生成模型。


<details>
  <summary>Details</summary>
Motivation: 合成声纳数据集是昂贵真实采集的可扩展替代方案，但其效用受到缺乏严格定量验证的限制。

Method: 使用基于Gazebo的环境生成声纳图像，通过显式控制海底纹理、照明驱动的阴影、平台高度和噪声。 使用全局强度和局部纹理（LBP）分布通过Kullback-Leibler散度、Jensen-Shannon散度和Earth Mover距离进行量化。

Result: 结果显示所有类别的纹理对齐都很强（KL < 0.07），由于阴影几何复杂性，飞机类的强度对齐优于船只类。

Conclusion: ACOUSIM建立了可复现的声纳sim-to-real评估分布级基线，并直接支持水下图像分析的可靠数据集验证。
...
TL;DR: 提出了Block Approximate Sparse Attention框架(BA-Att),通过块级预下采样操作识别信息区域,避免依赖脆弱的位置先验,在注意力计算中实现高达6.95x加速,同时保持接近全注意力性能。


<details>
  <summary>Details</summary>
Motivation: Diffusion Language Models虽然具有优势但扩展到超长序列成本高;现有块稀疏注意方法使用固定采样模式容易遗漏关键标记且在分布变化下不稳定。

Method: 定义Oracle后下采样注意力图并形式化前后下采样方案之间的近似误差;引入轻量级范数排序模块和协方差补偿校正,使用对角QK方差近似完整协方差降低计算复杂度。

Result: 在注意力计算中实现最高6.95x加速;在50%稀疏度下跨语言模型、多模态语言模型和视频生成模型保持接近全注意力性能。

Conclusion: BA-Att框架能有效平衡效率与性能,在各种模型架构上展现强大的效率和泛化能力。
...
TL;DR: 本文提出Aero-World方法，将预训练的图像到视频扩散模型转换为可控的空中视频生成器，并引入AeroBench基准来评估生成的无人机视频是否遵循低层动作信号。


<details>
  <summary>Details</summary>
Motivation: 基础视频模型虽能产生视觉效果优异的结果，但在具身AI中的应用受限，因为它们主要在自然语言而非低级控制信号上训练。空中飞行尤其如此，因为在无约束的6-DoF空间中运动时，小的自我运动误差会产生较大的轨迹漂移。

Method: Aero-World通过动作token流将平移加速度和角速度序列注入预训练的潜在扩散Transformer。冻结的潜在空间Physics Probe在LoRA微调期间提供可微分的惯性一致性监督，避免了计算昂贵的视频解码。AeroBench使用动作对齐分数(AAS)测量与 commanded 惯性动作的一致性，使用物理一致性比率(PCR)测量时间运动稳定性。

Result: 在AeroBench上，Aero-World将平均AAS从57.7提升至63.6，优于仅动作微调的方法。相比AirScape，具有更低的FVD(596.5 vs 1058.6)、更高的SSIM(0.595 vs 0.505)和更高的Flow-IMU相关性(0.44 vs 0.20)。

Conclusion: 冻结的Physics Probe监督是将预训练视频生成器适应于更具动作对齐性的空中运动的实用机制。
...
TL;DR: GeoMamba是一种针对非对齐光学-SAR跨模态细粒度目标检索的几何驱动框架，提出几何特征注入(GFI)模块和几何一致性约束(GCC)模块来解决模态差异、斑点噪声和结构不一致问题，在新构建的FGOS-as数据集上实现63.3%mAP和77.0%Rank-1准确率。


<details>
  <summary>Details</summary>
Motivation: 跨模态遥感检索在实际场景中面临显著的模态差异、斑点噪声和结构不一致问题，传统的成对或空间对齐样本依赖方法不适用于非对齐光学-SAR检索任务，亟需一种能够增强跨模态特征交互并保持几何一致性的鲁棒表示学习方法。

Method: 提出GeoMamba框架，包含两个核心模块：1)几何特征注入(GFI)模块，增强跨模态特征交互并引入结构先验，提高SAR表示的鲁棒性；2)几何一致性约束(GCC)模块结合深度监督(DS)策略，使用经典算子施加层次几何约束，保持目标结构的完整性。此外构建了包含11个航空航天和海上类别的FGOS-as数据集用于评估。

Result: 在FGOS-as数据集的全到全检索设置下，GeoMamba达到63.3%的mAP和77.0%的Rank-1准确率，优于现有方法。

Conclusion: 通过几何驱动的特征学习和结构约束，GeoMamba有效解决了光学-SAR跨模态细粒度检索中的几何不一致问题，为非对齐跨模态遥感检索提供了新的有效方案。
...
TL;DR: FlowErase-RL是首个基于GRPO的概念擦除框架，用于流匹配模型的安全生成。该方法将概念擦除重新表述为奖励优化问题，通过动态双路径奖励机制同时优化概念擦除和生成保真度，在无需显式监督的情况下实现稳定优化。


<details>
  <summary>Details</summary>
Motivation: 流匹配模型虽提升了文本到图像生成质量，但也带来了生成有害内容的安全风险。现有概念擦除方法存在局限：推理时干预效果有限，监督微调(SFT)需要精确对齐数据且难以扩展到多概念场景。

Method: 提出FlowErase-RL，首个基于GRPO的流匹配模型概念擦除框架。将概念擦除重构为奖励优化问题，引入动态双路径奖励机制：(1)概念擦除(CE)奖励抑制目标概念；(2)非目标空间(NS)奖励保持生成保真度。采用性能驱动切换策略自适应平衡两条奖励路径，实现无显式监督的稳定优化。

Result: 在裸体、物体和艺术风格擦除任务上实现最先进的擦除性能，同时保持较强的图像质量和语义对齐。对抗攻击鲁棒性强，能有效扩展到多概念场景。

Conclusion: 为流匹配模型建立了安全可控生成的新范式，标志着概念擦除技术从推理时干预或监督微调向强化学习优化的转变。
...
TL;DR: 本文提出一种基于预训练视觉Transformer嵌入的实时交通场景异常检测方法，通过最近邻相似度在潜在语义特征空间中检测异常区域。


<details>
  <summary>Details</summary>
Motivation: 交通场景异常检测对自动驾驶安全至关重要，但收集代表性异常数据具有挑战性。现有方法高度专业化且依赖抽象语义类别，难以适应多样化的真实场景。

Method: 提出一种基于补丁处理的适应性强的方法，利用预训练视觉Transformer嵌入作为基础模型，通过潜在语义特征空间中的最近邻相似度检测偏差。该方法通过单张参考图像建模正常状态，无需显式监督和特定数据集训练即可生成密集异常掩码实现异常定位。

Result: 该方法在Road Anomaly基准测试中取得了良好的性能表现，并在实际自动驾驶车辆的真实场景中展现了一致的定性行为，成功突出显示了不同场景中的语义异常对象。

Conclusion: 结果表明，简单基于参考的方法可以在现实操作条件下提供有用的异常信号，具有良好的实时性和适应性，适合实际部署。
...
TL;DR: 首个系统研究视觉自回归(VAR)模型在持续个性化生成中的方法，提出梯度式概念神经元选择(GCNS)解决灾难性遗忘，以及上下文感知组合策略实现多概念解缠合成。


<details>
  <summary>Details</summary>
Motivation: 现有基于VAR的个性化方法仅限静态设置，无法适应用户演进需求，存在灾难性遗忘和多概念特征纠缠问题。

Method: 提出统一框架包含两部分：1) GCNS通过识别概念相关神经元并约束冲突参数缓解遗忘；2) 上下文感知组合策略进行多分支特征建模和局部跨注意力融合。

Result: 实验表明该方法在长序列持续个性化任务中显著提升性能，多概念图像合成优于现有基线。

Conclusion: 为VAR模型的可扩展可控个性化生成提供了有效方案，具有重要应用潜力。
...
TL;DR: 该论文通过机械可解释性工具研究了LLaVA-1.5和InternVL-3.5两族VLM模型的对象定位机制，发现定位由"容器化机制"驱动，即对象对齐的token定义空间范围，而内部token排列不影响预测框，且仅有少量注意力头介导定位和分类任务，为未来VLM模型设计提供了解释性指导。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLM)在连接视觉和文本信息方面表现出色，但在基本分类和定位任务上仍存在困难。虽然分类机制已被广泛研究，但支持对象定位的过程仍不清楚，因此需要深入研究定位的底层机制。

Method: 使用机械可解释性工具套件进行研究，包括token消融、注意力knockout和因果中介分析。针对两个代表性模型家族LLaVA-1.5和InternVL-3.5进行实验。

Result: 定位由"容器化机制"驱动：对象对齐的token定义对象的空间范围，而边界内token的语义排列与预测框无关。分类和定位仅由非常小的注意力头集合介导因果效应，它们集中在LLaVA的早中层和InternVL的中后层。两个任务共享一些早期处理，但最终依赖明显不同的专门化头。

Conclusion: 该研究首次提供了VLM中定位任务的层级和注意力头级别的详细解释，揭示了狭窄的计算通路，可为未来模型设计和 grounding 目标提供指导。研究强调了模型架构中特定专业化路径的重要性。
...
TL;DR: CADENet是一个无需训练的三线程系统,用于在恶劣天气条件下提升自动驾驶车辆的目标检测性能。Thread S使用YOLOv11n提供实时检测,Thread Q通过条件自适应增强(CAPE)和熵引导NMS融合结果,Thread E利用CLIP进行零样本天气分类。该系统在DAWN数据集上评估,但由于标注完整性偏差,报告的F1值被视为真实收益的下界。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用"增强后检测"的两步流程,会阻塞安全关键的感知循环,违反硬实时要求。此外,存在一个未被充分认识的评估天花板问题:在退化图像上标注的ground truth无法认可检测器恢复出的目标(因为标注者自己也无法看到),导致真正有用的增强可能只显示近乎平坦的F1增益。

Method: 提出CADENet(condition-Adaptive Asynchronous Dual-stream Enhancement Network),一个无需训练的三线程系统:1)Thread S使用YOLOv11n以完整帧率提供检测,零额外延迟;2)Thread Q应用条件自适应增强(CAPE)并通过熵引导NMS(EG-NMS)融合结果,不阻塞Thread S;3)Thread E提供CLIP零样本天气分类,新天气类别只需新文本提示,无需标注数据或重训练。

Result: 在1327张DAWN图像上评估(YOLOv11m, IoU=0.5, confidence=0.25),CADENet在雪天实现Recall=0.0103(微平均),F1=0.0230;雨天F1=0.0038。由于标注完整性偏差,报告的F1值是真实收益的下界,recall是无标注间隙影响的关键指标。Thread S维持约44 FPS,无需模型重训练或额外传感器硬件。

Conclusion: CADENet通过异步三线程架构有效解决了恶劣天气下目标检测的实时性问题,同时揭示了DAWN类数据上存在的标注完整性偏差这一评估陷阱。该方法无需重训练或额外硬件,为自动驾驶安全感知提供了新思路。
...
TL;DR: 本文研究了一种新的偏好对齐方法，使用真实数据作为监督来源而非模型生成的图像。通过将真实图像作为参考点，与生成或扰动样本进行对比来构建偏好信号，无需人工标注的偏好对。实验表明这种方法可以有效对齐扩散模型，性能与现有基于偏好的方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有偏好对齐方法依赖于模型生成的图像构建偏好对，这种监督本质上是相对的且当样本都存在伪影或视觉质量有限时难以判断什么是真正理想的输出。需要探索真实数据是否可以作为替代监督源。

Method: 采用数据中心的视角，使用真实图像作为参考点，通过与生成或扰动样本对比来构建偏好信号，无需人工标注的偏好对。

Result: 基于真实数据的监督提供了有效的指导来对齐扩散模型，实现了与现有基于偏好方法相当的性能。

Conclusion: 真实数据为偏好对齐提供了一种实用且互补的监督来源，揭示了标签高效对齐策略的方向。
...
### [74] [A Framework for Evaluating Zero-Shot Image Generation in Concept-based Explainability](https://arxiv.org/abs/2605.19855)
*Giacomo Astolfi,Matteo Bianchi,Riccardo Campi,Antonio De Santis,Marco Brambilla*

Main category: cs.CV

TL;DR: 本文研究利用零样本文本到图像(T2I)生成模型为基于概念的可解释人工智能(XAI)方法生成合成概念数据集，以解决大规模标注图像依赖的问题。通过四个互补分析维度评估合成概念与真实概念的相似性和有效性。


<details>
  <summary>Details</summary>
Motivation: 基于概念的可解释人工智能(XAI)方法主要依赖于大量标记图像来表示每个概念，这限制了方法的扩展性。现有研究希望探索使用零样本T2I生成模型作为合成概念数据源的可行性。

Method: 1、通过概念表示相似度比较合成与真实概念图像；2、通过逐步增加子集大小评估同概念不同子集间的内相似性；3、使用相关类别图像评估下游解释任务的性能；4、评估从测试类别图像中移除概念对合成概念解释的影响。

Result: 当前T2I生成模型为基于概念的XAI提供了一条快捷路径，但研究表明在模型分析中使用零样本管道生成的合成数据仍面临挑战，并提出了开放性问题。

Conclusion: 零样本T2I生成模型可以用于基于概念的XAI方法，但需要注意合成数据带来的挑战和局限，需要进一步研究和验证。
...
TL;DR: 本文提出了EyeVLM框架，系统评估视觉语言模型(VLM)在人类注视理解方面的能力，测试了零样本和微调两种设置，发现当前VLM在精确注视理解方面存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型快速发展但其在理解人类注视行为方面的能力尚未被探索。注视分析是理解人类行为的中心任务，需要物理场景推理、活动理解和社交上下文理解等多维能力。

Method: 构建了EyeVLM系统评估框架，在两个核心任务上评估多种VLM：1)注视跟随任务，预测人看向的2D位置；2)社交注视预测，理解多人群互动。同时测试零样本设置(不同提示策略)和微调设置(QA问答方式)。

Result: 当前VLM缺乏精确的注视理解能力。标准训练可以缩小与纯视觉模型的差距，但仍有很大改进空间。

Conclusion: VLM在人类注视理解领域具有潜力但目前能力有限，需要进一步研究提升其几何/视觉处理和社交推理能力。
...
TL;DR: SplitQ是一种针对视觉语言模型的后训练量化框架，通过模态特定异常值通道解耦和自适应跨模态校准两个模块，解决了文本和视觉模态激活分布异构性导致的量化精度下降问题，在多个数据集和量化配置下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）在资源受限设备上的部署面临挑战，特别是低比特后训练量化（PTQ）方法会导致准确性下降，因为文本和视觉模态的激活分布存在异构性，且这种跨模态异构性在不同通道中分布不均匀，少量通道包含大部分模态特定的异常值。

Method: 提出SplitQ框架，包含两个核心模块：1）模态特定异常值通道解耦（MOCD）模块，有效隔离显著的模态特定异常值通道；2）自适应跨模态校准（ACC）模块，使用双轻量级可学习分支动态缓解模态引起的量化误差。

Result: 在6个流行多模态数据集上，SplitQ显著优于现有方法，在W4A8、W4A4、W3A3和W3A2等所有评估的量化设置下均取得更好性能。在具有挑战性的W3A3设置下，SplitQ保持了93.5%的FP16性能（69.5 vs 74.3）。

Conclusion: SplitQ通过通道分裂策略有效解决了VLMs量化中的跨模态异构性问题，为在资源受限设备上部署高级VLMs提供了高效的解决方案。
...
### [78] [Landscape-Awareness for Geometric View Diffusion Model](https://arxiv.org/abs/2605.19865)
*Yan-Ting Chen,Hao-Wei Chen,Tsu-Ching Hsiao,Chun-Yi Lee*

Main category: cs.CV

TL;DR: 在两视图稀疏视图条件下，利用Zero123等扩散模型进行视点估计时存在非凸损失 landscapes的优化挑战，本文提出-score based方法重塑优化景观并结合视点条件扩散模型进行refinement，提高收敛性和样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有利用扩散模型进行视点估计的方法虽然能利用Zero123等模型合成新视图，但面临的非凸损失 landscapes存在大量局部最小值，对初始化敏感且依赖暴力多起始采样。特别是几何歧义如对称性和自相似性会误导梯度更新导致错误视点。

Method: 提出一种score-based方法，首先重塑优化landscape以引导梯度更新朝向ground-truth视点，然后使用viewpoint-conditioned diffusion model进行refinement。该方法旨在改善优化过程的收敛性，减少对暴力采样的依赖。

Result: 实验表明本文方法提高了收敛性能，减少了对brute-force采样的依赖，实现了具有更高sample-efficiency的竞争性精度。

Conclusion: 通过score-based方法重塑优化景观可有效解决扩散模型视点估计中的局部最小值问题，结合viewpoint-conditioned扩散模型的refinement阶段可实现更准确的视点估计，且具有更好的样本效率。
...
TL;DR: 解决卫星 lidar 与地面样方异构数据融合估计森林生物量的多任务回归问题，提出结合物理约束和 MNAR 校正的 StruMPL 方法，在两个生物群落中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 从地球观测估算森林地上生物量(AGB)面临独特挑战：卫星激光雷达提供结构数据但无生物量标签，地面样方提供生物量数据但存在偏差且缺失结构指标。传统方法无法处理这种异构 disjoint 部分监督且标签缺失非随机(MNAR)的多任务回归问题。

Method: 提出StruMPL框架：(1)共享编码器+每变量回归/插补/倾向头用于空间MNAR校正；(2)可学习物理模块评估任务间物理约束；(3)使用带stop-gradient的Augmented IPW(AIPW)伪目标进行联合优化，同时保证损失有界。

Result: 在两个生态不同的生物群落中，StruMPL在AGB RMSE和偏差上优于消融变体和已发表方法。分层分析显示AIPW将高AGB偏差降低约54%。

Conclusion: 所提出的 StruMPL 成功解决了遥感生物量估算中标签来源异构且 MNAR 的难题，通过 AIPW 校正和物理约束模块实现了准确的 AGB 估计，具有重要的生态监测应用价值。
...
TL;DR: 本文提出一种解决视觉语言模型(VLM)在处理非分布布局文档时失败的两跳瓶颈方法，通过在解码器外部运行轻量级RT-DETR检测器并将布局先验注入提示中，显著提升了结构提取性能。


<details>
  <summary>Details</summary>
Motivation: VLM在解析与训练分布不同的文档布局时经常失败，作者将此归因于两跳瓶颈：第一跳需要分类和定位布局实体，当第一跳失败时第二跳会崩溃导致遗漏、畸形结构或自回归重复等问题。

Method: 在解码器外部预先解决第一跳问题，运行轻量级RT-DETR检测器，将其输出序列化为解析器原生DocTags词汇表的标记，并与全页图像一起注入提示中。与裁剪页面的分析后解析方法或纯文本提示 Priors不同，该方法让先验共享解码器的生成空间，并保留全局图像作为噪声检测的备用。

Result: 在10k页面结构化OOD基准测试上，markdown F1从0.37提升到0.92；OmniDocBench中文子集上，table TEDS从0.01提升到0.36；ViDoRe V3 26k页面基准测试上，每个工业领域的无限循环解码失败均下降。这些提升仅增加15%时钟延迟和74个中位数提示标记，无需修改基础VLM架构。

Conclusion: 通过注意力级别分析揭示了双峰相移：解码器在发出结构时关注注入的布局标记，在发出内容时关注图像补丁，证实两跳瓶颈得到缓解。该方法成本低、无架构修改且效果显著，为VLM文档解析提供了有效的布局先验注入方案。
...
TL;DR: 本文提出World-Ego Modeling新范式，将世界模型分解为世界组件和自我组件两部分，解决现有世界模型中世界与自我纠缠导致的长期任务性能下降问题，并构建了首个混合导航-操作任务的长期世界建模基准HTEWorld。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常在单一流程中预测世界和自我的演化，这种世界-自我纠缠导致在长-horizon具身场景中性能下降，特别是在包含导航和操作行为的混合任务中。当前缺乏专门针对此类混合任务的世界建模评估基准。

Method: 从运动、语义和意图三个视角定义世界-自我边界，提出后置、前置和完全三种解纠缠策略。将该范式实例化为耦合隐式分离世界-自我规划器与级联并行专家混合扩散生成器的统一具身世界模型WEM。

Result: 在新建的HTEWorld基准上达到最佳性能，该基准包含125K视频片段(超4.5M帧)和300条多轮评估轨迹(超2K指令)。同时在现有纯操作基准上保持竞争力。

Conclusion: 提出的World-Ego Modeling范式通过解纠缠世界和自我组件，有效提升了长-horizon混合任务的建模性能，验证了新范式的有效性和泛化能力。
...
TL;DR: 提出了WoundFormer，一个基于Transformer的多类伤口组织分割框架，通过保留空间信息的多尺度聚合头增强层次化空间特征融合，在伤口组织数据集上实现了81.9%的Dice分数。


<details>
  <summary>Details</summary>
Motivation: 慢性伤口（如糖尿病足溃疡和压力性损伤）需要精确的组织级评估来指导治疗计划和监测愈合。现有的深度学习方法大多聚焦于二分类分割，难以处理高类内变异性和标注数据有限导致的多类伤口组织分割问题。

Method: 提出WoundFormer框架，采用基于Transformer的架构增强层次化空间特征融合。具体使用保留空间信息的多尺度聚合头部替代标准SegFormer解码器，在跨尺度整合时保持特征拓扑结构，并通过卷积融合加强上下文交互，从而提高边界定位和相似组织类别的区分能力。

Result: 在WoundTissueSeg数据集（147张图像，6个组织类）和DFUTissue数据集上进行评估。WoundFormer取得81.9%的总体Dice分数，在WoundTissueSeg基准上比最强的CNN和Transformer基线方法最高提升4.3个Dice点，并在少数类组织上保持一致的性能提升。

Conclusion: 显式建模层次化空间交互可以增强Transformer表示能力，从而改善异质性伤口组织分割性能，支持更可靠的去定量伤口评估。该方法在处理视觉相似的组织类别边界时表现出更好的 discrimination 能力。
...
### [83] [Probability-Conserving Flow Guidance](https://arxiv.org/abs/2605.20079)
*Parsa Esmati,Junha Hyung,Amirhossein Dadashzadeh,Jaegul Choo,Majid Mirmehdi*

Main category: cs.CV

TL;DR: 本文提出自适应流形引导（AdaMaG），通过分析连续性方程将引导分解为散度项和得分平行项，解决强引导下样本脱离学习流形的问题，实现无需额外推理成本的保形引导。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型中的无分类器引导（CFG）和外推方法采用启发式线性组合，忽略生成流形几何结构，在强引导下破坏概率守恒并使样本脱离学习流形，导致饱和和幻觉问题。

Method: 1）通过连续性方程分析引导效应，将其分解为散度项和得分平行项；2）证明散度项在采样接近数据流形时结构性地增长；3）提出时间依赖调度结合得分平行衰减的方案；4）AdaMaG在无额外推理成本下同时约束两项。

Result: 在图像生成基准测试上，AdaMaG提高了真实感，减少了幻觉，并在高引导强度下实现受控的去饱和。

Conclusion: 大多数改善生成质量的实证启发式方法与本研究分解的两个项直接对应，AdaMaG提供了无需额外推理成本的plug-and-play引导规则，具有重要应用价值。
...
### [84] [Structural Energy Guidance for View-Consistent Text-to-3D Generation](https://arxiv.org/abs/2605.19876)
*Qing Zhang,Jinguang Tong,Jing Zhang,Jie Hong,Xuesong Li*

Main category: cs.CV

TL;DR: 本文提出SEGS（结构能引导采样）框架，通过利用U-Net特征的PCA子空间构建结构能量并注入去噪过程，解决文本到3D生成中的Janus问题（多视角不一致），无需训练即可集成到现有SDS/VSD pipeline中。


<details>
  <summary>Details</summary>
Motivation: 文本到3D扩散模型常出现Janus问题，导致不同视角下几何形状不一致。本文将此归因于2D扩散先验中的视角偏见，旨在消除这种偏见同时保持外观保真度。

Method: SEGS是一种免训练、即插即用的框架。它在U-Net特征的PCA子空间中构建结构能量，并将该能量的梯度注入去噪过程。该方法可轻松集成到DreamFusion、Magic3D、LucidDreamer等现有SDS/VSD pipeline中，无需重新训练。

Result: 实验表明，SEGS平均降低Janus Rate约10%，并在多个基线模型（DreamFusion、Magic3D、LucidDreamer）上提升了View-CS分数。该方法有效缓解了视角伪影，同时保留了外观保真度。

Conclusion: SEGS为高画质文本到3D内容生成提供了一种灵活的解决方案，通过解决视角偏见问题显著改善了多视角一致性，为无需额外训练的扩散模型改进提供了新思路。
...
### [85] [Rethinking Visual Attribution for Chest X-ray Reasoning in Large Vision Language Models](https://arxiv.org/abs/2605.20158)
*Guangzhi Xiong,Qiao Jin,Sanchit Sinha,Zhiyong Lu,Aidong Zhang*

Main category: cs.CV

TL;DR: 针对医学影像问答中LVLM可信度问题，开发因果评估框架验证归因方法的准确性，发现现有方法普遍失效，并提出MedFocus新方法通过非最优传输定位解剖区域，显著提升归因效果。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型(LVLMs)在医学应用中表现出色，但其无法忠实基于视觉证据进行推理的问题严重影响了临床信任度。现有的视觉归因方法虽然广泛用于解释LVLM预测，但这些解释是否真正反映模型决策背后使用的视觉证据尚未得到验证，因为模型内部推理的真实标注通常不可用。

Method: 开发了一个因果评估框架，通过反事实编辑保留仅对模型预测有因果贡献的专家标注区域。在11种归因方法、6个开源LVLM和两种输出模式(直接回答和逐步推理)上进行了评估。提出MedFocus方法，利用非平衡最优传输定位临床有意义解剖区域，并通过定向干预测量其因果效应。

Result: 研究发现现有归因方法常常无法识别LVLMs实际使用的证据。MedFocus产生空间、概念级和token级归因，性能显著优于先前方法。

Conclusion: 该研究为医学LVLMs的可信归因提供了新方向，数据和代码已公开。
...
TL;DR: 本文提出了GOTTA（引导观察测试时适应），一种结合类别平衡与特征空间多样性的多样性感知内存策略，作为测试时适应中记忆管理的新方法。基准测试表明，该方法在受限内存和非i.i.d.流下表现最优。


<details>
  <summary>Details</summary>
Motivation: 现有研究很少系统性地评估TTA中的记忆设计选择，而现有记忆机制通常与特定TTA算法耦合，难以分离各因素的实际影响。研究者需要明确哪些记忆设计原则在什么条件下重要。

Method: 构建了统一的基准测试框架，将记忆管理与适应算法解耦，在四种测试流（i.i.d.、非i.i.d.、持续和实际场景）下评估多种内存策略。提出intra-class diversity作为关键因素，并设计了GOTTA系列多样性感知内存策略。

Result: 研究表明有效的内存管理需要超越简单保留近期或类别平衡样本的策略。Intra-class多样性是避免冗余缓冲和在时间相关及标签偏斜流下保持代表性适应信号的关键。GOTTA在受限内存预算和挑战性非i.i.d.流下提升效果最显著。

Conclusion: 记忆管理应被视为鲁棒测试时适应的一类核心组件，多样性是实用TTA的中心设计原则。GOTTA可作为drop-in替换与不同TTA目标结合使用。
...
TL;DR: AnyCity是一个用于稀疏航空城市场景的观察接地生成重建框架，通过预测观察支持的geometry潜在变量来锚定可靠结构，并使用脚手架条件化的航空补全token预测门控残差更新，在高斯解码前完成弱约束内容的预测。


<details>
  <summary>Details</summary>
Motivation: 由于biased top-down和shallow-oblique相机pose，稀疏航空捕获存在严重的evidence不平衡：屋顶和开放区域被重复观察，而立面，远处建筑和遮挡结构获得很少的多视角支持。现有前馈3D Gaussian Splatting方法直接从稀疏输入回归确定性表示，导致重影、融化的立面和拉伸的纹理。伪视角和视频生成重建方法缺乏观察几何与先验驱动内容之间的清晰分离。

Method: AnyCity首先预测observation-supported geometry latent来锚定可靠结构，然后使用scaffold-conditioned aerial completion tokens预测gated residual update用于弱约束内容，再进行高斯解码。训练中使用dense-to-sparse蒸馏从密集视角重建转移结构线索，航空适配的视频扩散先验通过门控token conditioning提供细粒度城市外观线索，观察保持目标保持精炼表示与输入支持几何的一致性。推理时单次前馈pass重建最终3D Gaussian场景。

Result: 在合成、航空域、UAV纹理化和真实场景上的一致改进。在秒级推理时间下实现连贯的城市新视角合成。

Conclusion: AnyCity通过观察接地生成框架结合密集到稀疏蒸馏和航空适配视频扩散先验，在稀疏航空城市场景重建中有效分离了观察几何与先验驱动内容，实现了连贯的城市场景重建和新视角合成。
...
### [88] [Untitled](https://arxiv.org/abs/2605.19950)
**
...
TL;DR: 针对视觉语言模型易受对抗性攻击的问题,本文提出了Attention-Guided Test-Time Prompt Tuning (A-TPT)方法。该方法首先利用改进的梯度注意力Rollout机制识别语义有意义的区域,然后利用这些区域指导空间变化的增强强度和多视图集成进行提示调整和推理。实验表明该方法在对抗性和干净数据上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) 如CLIP虽然在大规模预训练中表现出色,但在实际应用中面临对抗性攻击的巨大风险,因为对抗攻击会显著降低VLMs的推理能力。现有的测试时适配方法通常依赖多视图增强来实现各种微调策略,但这些方法难以识别语义信息,且在细粒度场景中容易破坏区分区域。

Method: 1. 改进梯度注意力Rollout机制,识别在对抗攻击下仍能存活的语义有意义区域;2. 利用这些区域指导空间变化的增强强度和多视图集成;3. 将上述方法用于提示调整和推理过程。

Result: 大量实验表明,A-TPT在对抗性和干净数据上均优于现有的测试时适配方法。

Conclusion: 本文提出了保留语义的测试时适配方法A-TPT。该方法通过注意力引导的梯度rollout识别语义区域,并利用这些区域自适应地指导增强强度和多视图集成,从而在不破坏语义信息的前提下有效提升VLMs对对抗攻击的鲁棒性。
...
### [90] [SphericalDreamer: Generating Navigable Immersive 3D Worlds with Panorama Fusion](https://arxiv.org/abs/2605.19974)
*Antoine Schnepf,Karim Kassab,Flavian Vasile,Andrew Comport*

Main category: cs.CV

TL;DR: SphericalDreamer是一种从文本提示生成完全沉浸式和长距离3D户外环境的方法，通过生成多个全景图像并将其提升为3D并融合，同时保持视觉和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法同时满足两个关键要求：(i)在长距离空间范围内可导航，(ii)覆盖完整的全向视场（水平360°和垂直180°）。随着虚拟现实和3D内容的普及，生成沉浸式和可导航的3D环境变得越来越重要，但面临根本性限制。

Method: SphericalDreamer建立在生成多个全景图像的基础上，这些图像随后被"lifted"（提升）到3D空间，并在保持视觉和几何一致性的前提下融合在一起。

Result: SphericalDreamer生成了高度详细、完全沉浸式的3D环境，同时在规模和可导航性方面相较于先前的方法有显著改进。

Conclusion: 该方法成功解决了生成同时具有长距离可导航性和完整全向视场的3D世界的挑战，为虚拟现实和3D内容生成领域提供了新的解决方案。
...
### [91] [RECIPE: Procedural Planning via Grounding in Instructional Video](https://arxiv.org/abs/2605.19976)
*Luigi Seminara,Antonino Furnari,Lorenzo Torresani*

Main category: cs.CV

TL;DR: 本文提出RECIPE框架，利用基于ASR转录本的时间接地质量作为奖励信号，通过GRPO（分组相对策略优化）训练视觉规划模型，解决注释数据稀缺和单轨迹限制的问题。该方法在零标注场景下，通过验证生成的步骤序列是否在原始ASR转录本中 temporally grounded，将噪声语料库转化为验证器而非标签源。


<details>
  <summary>Details</summary>
Motivation: 视觉规划任务的进展受限于注释瓶颈：(1)干净标注数据集规模小且领域窄；(2)每个示例只编码单一执行轨迹，尽管存在多种有效排序方式。现有的大规模教学视频语料库虽然能提供更多程序内容，但使用噪声ASR narration的伪标签进行监督微调会传播分段和对齐错误，且仍保持单轨迹特性。作者发现一个关键不对称：虽然从噪声视频中提取干净的步骤标签很困难，但验证生成的步骤序列是否在ASR转录本中 temporally grounded（时间接地）却是廉价且可扩展到数百万视频的。

Method: 核心创新：利用接地质量作为GRPO的奖励信号。具体包括：
1. 不再依赖ASR narration作为伪标签，而是将其作为验证器——检查生成的步骤序列是否能对应到ASR转录本中的时间戳
2. 使用预计算的文本嵌入实现大规模验证
3. 适用于两种输入配置：Socratic模式（使用冻结VLM提取的文本历史）和Video模式（直接消费视频tokens）
4. 适用于标注和弱监督场景
5. 评估采用基于参考的LLM-as-judge协议，在6个程序性标准上评分

Result: RECIPE-RL在所有规模(0.5B, 3B, 7B)和所有基准测试上都优于基线checkpoint：
- 领域内宏准确率提升+7至+8点
- 零样本场景最高提升+16点
- 优于监督微调（包括在标注计划和伪标签计划上，后者反而降低基线性能）
- 在无人类注释情况下保持鲁棒
- 在Propose-Assess-Search规划器中作为提议阶段使用时，在Visual Planning for Assistance任务上超越最强零样本基线
- 在COIN任务上保留了SFT会collapse的生成多样性

Conclusion: 该工作证明了一个重要的范式转变：将噪声数据源从"标签来源"转为"验证器"，结合强化学习可以在没有人工注释的情况下显著提升视觉规划模型的性能。这种基于验证的方法不仅解决了数据稀缺问题，还避免了伪标签训练带来的误差传播问题，为构建更robust的程序化AI助手开辟了新路径。
...
### [92] [InterLight: Leveraging Intrinsic Illumination Priors for Low-Light Image Enhancement](https://arxiv.org/abs/2605.19982)
*Ziqi Wang,Xu Zhang,Laibin Chang,Shi Chen,Jiaqi Ma,Huan Zhang*

Main category: cs.CV

TL;DR: InterLight是一种通过深度挖掘光照先验来提升低光图像质量的新框架。它使用物理引导的增强、自适应提示和亮度门控内存机制，在暗部优先重建、在亮部保持保真度，并通过自监督一致性目标提炼光照不变特征，能够有效消除过增强和颜色失真，获得更清晰纹理和视觉连贯的增强结果。


<details>
  <summary>Details</summary>
Motivation: 现有的基于Retinex理论的深度学习低光图像增强方法常出现过度增强或颜色失真，且往往假设噪声均匀或光照理想，无法应对实际复杂场景中的光照不足、细节损失和噪声问题。因此需要一种能够系统性地挖掘和利用内禀光照先验的增强方案。

Method: 该框架首先通过物理引导的数据增强注入传感器级光照响应先验，将退化过程建模为受场景潜在光照状态调节的自适应提示。随后，亮度门控的内在记忆机制依据提示指导信息补偿选择性地恢复暗区细节，同时保留亮区的保真度。最后，利用自监督一致性目标提取光照不变特征，对整个流程进行正则化。

Result: 在多个基准数据集上的大量实验表明，InterLight 能够生成纹理更清晰、视觉上更一致的低光增强图像，在抑制过增强和颜色失真方面明显优于现有最先进方法。

Conclusion: 通过系统地挖掘和操作内禀光照先验，InterLight 为低光图像增强提供了一种有效的光照感知pipeline。它在各种真实低光场景中展现了优越的增强性能，为后续研究提供了新的思路。
...
### [93] [CogOmniControl: Reasoning-Driven Controllable Video Generation via Creative Intent Cognition](https://arxiv.org/abs/2605.19995)
*Hongji Yang,Songlian Li,Yucheng Zhou,Xiaotong Zhao,Alan Zhao,Chengzhong Xu,Jianbing Shen*

Main category: cs.CV

TL;DR: CogOmniControl是一个推理驱动的可控视频生成框架,将视频生成分解为创意意图认知和生成两个阶段,专门针对专业动画制作场景设计。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在抽象、稀疏或复杂条件下的视频生成表现脆弱,在专业制作工作流(如故事板草图和黏土渲染)中难以产生符合用户创意意图的视频。现有方法通过适配器注入条件或结合通用视觉语言模型,存在能力差距。

Method: 1) 使用真实动画制作数据训练专门的CogVLM用于准确识别用户的稀疏和抽象条件并输出密集推理;2) CogOmniDiT通过上下文生成统一各种条件控制;3) 通过强化学习将CogOmniDiT与CogVLM推理输出对齐;4) 利用CogVLM作为规划评估器实现Best-of-N选择,形成闭环架构。

Result: 在CogReasonBench和CogControlBench两个专业工作流基准上超越现有开源模型,并引入基于真实创意意图而非模拟的专业工作流数据。

Conclusion: 该框架将可控视频生成为一个闭环" harness-like"架构,有效弥合了用户创意意图与生成结果之间的差距,为专业动画制作提供了更精准的video generation方案。
...
### [94] [A Nash Equilibrium Framework For Training-Free Multimodal Step Verification](https://arxiv.org/abs/2605.20033)
*Rohit Sinha,Kunal Tilaganji,Tanuja Ganu,Nagarajan Natarajan,Amit Sharma,Vineeth N. Balasubramanian*

Main category: cs.CV

TL;DR: 提出了一种无需训练的验证方法，通过将逐步验证建模为专门法官之间的纳什均衡博弈，利用分歧信号检测推理步骤的有效性，在六个基准上相比基线模型提升2.4%至5.2%。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的推理链验证存在明显局限：学习型批评器需要大量标注数据且跨任务表现不稳定；而现有无训练方法只是简单平均分数，忽略了一个关键洞察——当分数不一致时，这种分歧本身就携带了推理步骤是否真正有效的重要信息。

Method: 将逐步验证建模为专门评判者之间的协调问题，将其交互形式化为纳什均衡博弈，其中一致性表示有效步骤，分歧揭示不稳定性。通过闭合形式解计算均衡分数，实现分歧感知过滤和稳定性感知排序两种功能。

Result: 在六个基准上实现相比基线模型一致提升2.4%至5.2%，与学习型批评器表现相当，表明跨模态一致性（而非仅仅是 평균置信度）提供了稳健的验证信号，且无需任务特定适配。

Conclusion: 证实了推理步骤验证中分歧本身具有重要的信息价值，无需训练的纳什均衡方法可作为多模态大语言模型推理链验证的有效方案。
...
### [95] [Stage-adaptive Token Selection for Efficient Omni-modal LLMs](https://arxiv.org/abs/2605.20035)
*Zijie Xin,Jie Yang,Ruixiang Zhao,Tianyi Wang,Fengyun Rao,Jing Lyu,Xirong Li*

Main category: cs.CV

TL;DR: 本文提出SEATS——一种针对omni-modal大语言模型的训练无关、阶段自适应token选择方法，通过分析层-wise token依赖发现视觉和音频依赖呈块状模式并随深度减弱，从而在LLM前、内部及晚层采用分阶段渐进剪枝策略实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有om-LLM推理加速方法存在局限：仅聚焦视觉输入或在LLM前以固定模态比例剪枝，无法捕捉跨模态token重要性随层深的变化规律，同时带来过高计算开销。

Method: 提出SEATS方法，包含三个阶段：1) LLM前通过注意力加权多样性选择去除时空冗余；2) LLM内部跨块渐进剪枝并用查询相关度动态分配模态保留预算；3) 晚层在跨模态融合完成后移除所有非文本token。

Result: 实验在Qwen2.5-Omni和Qwen3-Omni上验证，仅保留10%的视觉和音频token，实现9.3x FLOPs减少和4.8x预填充速度提升，同时保留96.3%的原始性能。

Conclusion: SEATS通过分层级联剪枝策略，在保持96.3%性能的同时实现9.3倍FLOPs减少和4.8倍预填充加速，为多模态大模型的高效推理提供了有效的训练无关解决方案。
...
### [96] [OP2GS: Object-Aware 3D Gaussian Splatting with Dual-Opacity Primitives](https://arxiv.org/abs/2605.20044)
*Guiyu Liu,Niklas Vaara,Janne Mustaniemi,Juho Kannala,Janne Heikkilä*

Main category: cs.CV

TL;DR: OP2GS通过为每个3DGS原语添加实例身份和双重不透明度机制（视觉不透明度σ和实例不透明度σ*），解耦了视觉存在与实例占用，解决了标签污染问题，实现了高效的开放词汇场景理解。


<details>
  <summary>Details</summary>
Motivation: 3DGS缺乏固有的对象级身份，阻碍了下游任务如开放词汇场景理解。现有方法要么产生大量存储和解码开销（特征方法），要么在2D到3Dlifting过程中易受标签污染影响（lift方法）。

Method: 提出双重不Opacity公式：在每个高斯原语中添加显式实例身份和专用实例不透明度的σ*用于对象掩膜渲染，同时保留原始不透明度σ用于视觉重建；引入随机对象损失使用基于标准透射率的可见性来优化1D实例占用场；通过多视图聚合在对象级别附加语义描述符，消除每高斯特征存储。

Result: 与特征训练方法相比实现了具有竞争力的开放词汇性能，同时显著降低计算开销；与免训练管道相比，通过物理一致的占用学习解决了可见性歧义。

Conclusion: OP2GS通过引入实例身份和双重不透明度机制，实现了视觉重建与实例占用的解耦，在保持竞争力的开放词汇性能的同时显著降低了计算开销，并能够解决2D到3D投影中的标签污染问题。
...
### [97] [X-Ray cardiac angiographic vessel segmentation based on pixel classification using machine learning and region growing](https://arxiv.org/abs/2605.20073)
*E O Rodrigues,L O Rodrigues,J J Lima,D Casanova,F Favarim,E R Dosciatti,V Pegorini,L S N Oliveira,F F C Morais*

Main category: cs.CV

TL;DR: 提出了一种基于像素分类的血管分割方法，用于X射线血管造影。该方法结合了各向异性扩散、 Hessian矩阵特征、数学形态学和统计特征等多种纹理特征，并使用ELEMENT区域增长控制策略和随机森林分类器。实验达到了95.48%的准确率，为当前文献最佳。


<details>
  <summary>Details</summary>
Motivation: X射线血管造影图像中的血管分割是医学诊断的重要步骤。传统方法在精度和鲁棒性方面存在不足，需要一种能够有效区分血管与非血管组织的自动化分割方法。

Method: 首先提取多维纹理特征：各向异性扩散、Hessian矩阵特征、数学形态学特征和统计特征。然后采用ELEMENT方法论进行区域生长控制的像素分类，使分类结果影响后续像素的分类过程。最后使用随机森林分类器预测每个像素是否属于血管结构。

Result: 该方法在血管分割任务中达到了95.48%的准确率，这是文献中报告的最高准确率，成功超越了现有的无监督最先进方法。

Conclusion: 所提出的像素分类方法结合多特征融合和ELEMENT迭代策略有效地解决了X射线血管造影中的血管分割问题，随机森林的使用进一步提升了分类性能。该方法为医学图像分割提供了一个高精度的解决方案。
...
TL;DR: 本文提出SP-VTP（空间提示视觉轨迹预测）新任务，使用初始空间提示（边界框或点）定义机器人操作任务目标，从egocentric视频流预测未来末端执行器轨迹为此首创。


<details>
  <summary>Details</summary>
Motivation: 机器人操作在杂环境中用语言描述不够方便，需要用空间方式指示要移动的对象和放置位置。现有的视觉中心挑战是对象和目标规范，现有方法未解决此问题。

Method: 收集标注EgoSPT数据集含egocentric空间提示操作轨迹，含首帧对象和目标ground注释及恢复3D末端运动。提出SPOT模型结合：任务编码器处理首帧视觉和坐标空间提示，观察编码器处理当前视觉和历史上下文，轨迹生成器预测未来末端运动。

Result: 在严格场景级分割下实验表明SPOT相比非提示或单源提示基线改善跨场景轨迹预测性能。

Conclusion: EgoSPT和SPOT建立新空间提示问题SP-VTP作为egocentric操作的简单可扩展任务条件。未来可扩展到动作预测和任务规划。
...
TL;DR: 提出SetCon方法，将开放域指代分割重新定义为集合级别概念预测，使用LVLM生成的自然语言概念作为语义条件进行联合掩码集解码，实现对多实例、跨类别、开放目标集的分割。


<details>
  <summary>Details</summary>
Motivation: 现有基于LVLM的方法将多个目标用特殊token顺序表示，缺乏对集合级别属性（完整性和互斥性）的建模能力，难以处理复杂场景中的多实例、跨类别组合或开放目标集。

Method: 提出层次化语义分解：首先预测定义目标范围的共享集合级概念，然后将其细化为与目标子集对齐的细粒度概念组；构建两阶段标注pipeline，扩展现有推理分割数据集（236k样本，784k概念短语）；使用自然语言概念而非分割token作为语义条件进行联合掩码集解码。

Result: 在图像基准上取得最先进结果：gRefCOCO上+3.3 gIoU，MUSE上+12.1 gIoU，且随目标数量增加优势增大；在视频上也实现新SOTA：MeViS上+10.9 J&F，Ref-SeCVOS上+12.4 J&F。

Conclusion: SetCon成功将开放域指代分割问题重新定义为集合级别概念预测，通过自然语言概念接口实现了高效的多目标分割，在图像和视频基准上均显著超越现有方法，展现了概念驱动方法的优势。
...
TL;DR: 该论文提出PixVerve-95K数据集（包含9.5万张最小100MP的超高分辨率图像及七维标注），并首次将多种文本到图像基础模型扩展到原生100MP生成能力，同时建立了综合评估基准PixVerve-Bench。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像（T2I）模型在1K和2K分辨率方面取得进展，人们对更好视觉体验的渴望以及成像技术的快速发展，对Ultra-High-Resolution（UHR）图像生成的需求显著增长。然而UHR图像生成面临高分辨率内容稀缺和复杂性的重大挑战。

Method: 1. 构建PixVerve-95K数据集：包含9.5万张不同场景的高质量UHR图像（每张最小像素数100M），采用精心设计的数据管道，具有七维注解。2. 提出三种训练方案将各种T2I基础模型扩展到原生100MP生成。3. 建立PixVerve-Bench基准，结合传统指标和多模态大语言模型进行评估。

Result: 在所提 benchmark 上进行了广泛实验，并对训练策略进行了建设性探索，为未来突破提供了有价值的见解。

Conclusion: 该工作为UHR图像生成领域提供了大规模数据集、训练方法和综合评估协议，推动了T2I模型向原生100MP方向的发展。
...
TL;DR: TideGS是一个out-of-core训练框架，能够在单个24GB GPU上训练超过10亿个3DGS高斯原语，通过SSD-CPU-GPU层次结构管理参数，利用块虚拟化几何、异步流水线管道和轨迹自适应差分流式传输技术。


<details>
  <summary>Details</summary>
Motivation: 3DGS训练本质上是内存受限的，每个高斯原语携带大的属性向量，聚合参数表快速超过GPU容量，之前的系统限制在单GPU硬件上最多几千万个高斯。而3DGS训练具有稀疏性和轨迹条件性的特点：每次迭代只激活从当前相机批次可见的高斯，因此GPU内存可以作为工作集缓存而非持久参数存储。

Method: 提出三种协同技术：1）块虚拟化几何实现SSD对齐的空间局部性；2）层次化异步流水线重叠I/O与计算；3）轨迹自适应差分流式传输在迭代之间只传递增量工作集差异。

Result: 在单个24GB GPU上能够训练超过10亿个高斯，在大型场景上实现了最佳重建质量，扩展性超过了之前的out-of-core基线（如约1亿高斯）和标准in-memory训练（如约1100万高斯）。

Conclusion: 通过利用3DGS训练的稀疏性和轨迹条件性质，TideGS成功实现了亿级规模的高斯训练，证明了GPU内存作为工作集缓存而非持久参数存储的可行性，为大规模3DGS训练提供了新的思路。
...
TL;DR: 本文提出AUDITS基准数据集，用于研究图像篡改检测中的领域迁移、质量、类型和尺寸等因素。该数据集包含超过53万张图像，来源于用户和新闻照片两种渠道，并使用基于扩散的图像修复技术生成多样化的篡改样本。


<details>
  <summary>Details</summary>
Motivation: 生成式AI使得图像篡改变得更加容易和逼真，这类manipulated图像虽然大多无害，但可能传播错误信息、创造虚假叙事并影响人们对重要问题的看法。然而，关于跨不同视觉领域检测高级图像篡改的研究仍然有限，因此需要建立全面的基准数据集来推动该领域的研究。

Method: 构建了AUDITS(Analysis Under Domain-shifts, qualIty, Type, and Size)基准数据集，包含超过53万张图像，来源于两个不同来源（用户照片和新闻照片）。通过最新的基于扩散的图像修复技术生成多样化 manipulators，涵盖不同类型的篡改方法和尺寸。在不同的领域迁移条件下评估现有图像篡改检测方法的鲁棒性。

Result: 实验表明现有方法在不同领域迁移条件下的表现存在差异，为开发更可靠、更具泛化能力的图像篡改检测方法提供了新的见解和数据支持。

Conclusion: AUDITS基准数据集的提出填补了图像篡改检测领域在跨域分析方面的空白，为后续研究提供了丰富的测试资源和系统性的评估框架，有望推动图像 manipulation detection技术的进一步发展。
...
### [104] [Untitled](https://arxiv.org/abs/2605.20183)
**
...
### [105] [STAR-PólyaMath: Multi-Agent Reasoning under Persistent Meta-Strategic Supervision](https://arxiv.org/abs/2605.19338)
...
### [106] [LLM Agents Make Collective Belief Dynamics Programmable: Challenges and Research Directions](https://arxiv.org/abs/2605.19915)
*Xin He,Junxi Shen,Yuchen Mou,David M. Bossens,Caishun Chen,Ivor W. Tsang,Yew Soon Ong*

Main category: cs.MA

TL;DR: 本文提出"可编程集体信念控制"概念，研究基于LLM的智能体如何通过大规模在线讨论协调诱导人群信念变化。通过多智能体模拟实验证明，协同AI智能体可在几轮交互内引发稳定的信念偏移，并识别出四个使检测和防御根本困难的结构性特征（不可区分性、持久性、情境性和可配置性）。


<details>
  <summary>Details</summary>
Motivation: LLM-based agents introduce a qualitative shift in opinion dynamics - they can participate at scale, maintain consistent persuasion strategies, and coordinate systematically. This makes collective belief dynamics "programmable," enabling deliberate steering of population-level beliefs. The authors term this new challenge "programmable collective belief control."

Method: Controlled multi-agent simulations were conducted to provide proof-of-concept evidence that coordinated AI agents can induce measurable belief shifts.

Result: Coordinated LLM agents can induce measurable belief shifts that stabilize within a few interaction rounds. Four structural properties make detection and defense fundamentally difficult: indistinguishability (agents are hard to distinguish from humans), persistence (belief shifts last), contextuality (effects depend on context), and configurability (parameters can be adjusted).

Conclusion: This problem demands urgent attention from the research community. The authors propose a research agenda covering three areas: (1) theoretical foundations for adversarial belief dynamics, (2) operational methods for system-level detection and intervention, and (3) simulation infrastructure for scalable experimentation.
...
TL;DR: 本论文提出了"数据探针"(data probes)的概念，通过从适当定义的随机过程生成合成序列来系统研究数据特征如何影响LLM行为，为理解数据在LLM训练和推理中的作用提供原则性方法。


<details>
  <summary>Details</summary>
Motivation: 当前对数据的理解不足：为何某些数据在LLM的不同阶段（训练、调优、对齐、上下文学习等）有用，以及理解数据驱动模型行为的本质。当前方法依赖大量实验和启发式过滤，缺乏系统性原则。现有的数据过滤和数据集构建方法计算密集，且缺乏理解数据特征如何驱动LLM行为的理论框架。

Method: 提出数据探针方法：1)定义适当的随机过程；2)从这些过程中生成合成序列（数据探针）；3)通过观察LLM在这些探针上的行为来系统研究数据特征如何影响模型性能、泛化和鲁棒性。这些序列具有统计性质，可以用典型集等理论概念来描述，这些概念被推广以描述LLM的行为。

Result: 该方法提供了一个途径来揭示数据在LLM训练和推理中作用的基础性洞察，超越现有的经验性启发式方法。通过系统性地使用数据探针，研究者可以更有效地理解和优化数据选择，而不是依赖于大规模的试错实验。

Conclusion: 数据探针方法是理解和优化LLM数据的有前景的新范式，通过使用具有可控统计特性的合成序列来系统研究数据特征的影响，这为未来的LLM数据工程提供了理论基础。
...
### [108] [Operationalizing Document AI: A Microservice Architecture for OCR and LLM Pipelines in Production](https://arxiv.org/abs/2605.18818)
...
TL;DR: 本文提出了一种微服务架构，用于在生产环境中运行文档理解流水线（包括分类、OCR和大语言模型结构化字段提取），可在数千份多页文档/小时的规模下运行，并揭示了OCR而非语言模型解析主导端到端延迟这一关键发现。


<details>
  <summary>Details</summary>
Motivation: 学术研究通常关注新模型的定义，但在生产规模上运行模型存在明显差距。本文旨在弥合这一差距，为从业者提供可在生产环境中有效运行的文档理解系统架构模式。

Method: 采用微服务架构设计，包含混合分类策略、GPU绑定推理与CPU绑定编排的分离、异步处理IO密集型操作、独立水平扩展策略，并通过批处理分析进行性能评估。

Result: 两个出人意料的定性发现：1) OCR而非语言模型解析主导端到端延迟；2) 系统在由共享GPU推理容量而非worker数量决定的并发量下饱和。测试规模达数千份多页文档/小时。

Conclusion: 本文为从业者提供了构建超越基准的文档理解系统的具体架构模式，强调了关注OCR优化和GPU推理容量管理的重要性。
...
### [109] [Evaluating the Utility of Personal Health Records in Personalized Health AI](https://arxiv.org/abs/2605.18937)
...
TL;DR: 研究评估了大型语言模型（LLM，Gemini 3.0 Flash）在提供患者健康查询回答方面的潜力，使用个人健康记录（PHR）作为临床背景。研究发现，添加PHR数据显著提高了答案的帮助性、准确性、个性化和安全性。


<details>
  <summary>Details</summary>
Motivation: 患者管理的个人健康记录虽然有望赋权患者更好地理解自身健康，但记录中的信息复杂，可能阻碍患者获取有价值的见解。需要探索LLM是否能帮助患者理解这些复杂的健康数据。

Method: 研究使用来自3种不同分布的2257个患者查询，以及1945份去标识化PHR。测试了3种条件：(1)无PHR背景；(2)基本人口统计、状况和药物摘要；(3)完整临床笔记。采用SHARP评分框架和新开发的PHR特定错误模式评估框架，由自动评分员进行全面评分，临床评分员对子集(n=95)进行评分。

Result: 添加PHR数据后，各类型问题的答案帮助性均显著提高(p<0.001，配对t检验)。同时观察到答案在安全性、准确性、相关性和个性化方面的潜在提升。评估框架识别出LLM理解复杂PHRs时的差距，如时间定向障碍和罕见的虚构性错误。

Conclusion: 研究结果表明，PHR数据有潜力帮助满足广泛的用户需求，并提供了基于PHR背景监测LLM答案差距的框架。这项研究推动了进一步工作的发展，以评估和实现让用户理解其健康记录的潜在益处。
...
### [110] [Learn-by-Wire Training Control Governance: Bounded Autonomous Training Under Stress for Stability and Efficiency](https://arxiv.org/abs/2605.19008)
*Anis Radianis*

Main category: cs.AI

TL;DR: 本文提出LBW-Guard，一种基于AdamW之上的训练治理层，通过监控训练遥测数据并应用有界控制来应对大规模语言模型训练中的不稳定性问题。实验表明该方法可显著提升训练稳定性和效率，在高学习率压力条件下效果尤为突出。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型训练日益面临不稳定性、退化运行和计算浪费的问题，特别是在激进的学习率、规模和运行时压力条件下。现有方法缺乏有效的训练过程监控与干预机制。

Method: 提出LBW-Guard（Learn-by-Wire Guard），一个运行在AdamW之上的有界自主训练控制治理层。该方法不替代优化器更新规则，而是观察训练遥测数据，识别不稳定敏感阶段，并对优化器执行应用有界控制，同时保持固定训练目标。

Result: 在Qwen2.5-7B上，LBW-Guard将最终困惑度从13.21降至10.74（提升18.7%），训练时间从392.54s降至357.02s（加速1.10x）。在学习率压力测试中，AdamW在LR=3e-3和1e-3下分别退化至1885.24和659.76困惑度，而LBW-Guard仅降至11.57和10.33。梯度剪裁基线无法复现此效果。

Conclusion: 研究证明：1）不稳定敏感的LLM训练可从优化器之上的治理平面受益；2）有界运行时控制可在压力下保持生产力计算；3）该方法不同于优化器替换和局部梯度抑制，属于系统级的训练稳定化方案。
...
### [111] [Interference-Aware Multi-Task Unlearning](https://arxiv.org/abs/2605.19042)
*Ying-Hua Huang,Rui Fang,Hsi-Wen Chen,Ming-Syan Chen*

Main category: cs.AI

TL;DR: 本文研究多任务场景下的机器遗忘问题，提出全任务遗忘和部分任务遗忘两种设置，并设计结合任务感知梯度投影和实例级梯度正交化的干扰感知框架，在两个多任务计算机视觉基准上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注单任务设置，而现代模型常在多任务环境中运行，移除一个任务或实例的监督可能影响其他任务或实例。共享参数会耦合遗忘集和保留集，导致任务级和实例级干扰。

Method: 提出干扰感知框架，包含两个方面：1) 任务感知梯度投影，将更新约束在任务特定的子空间内；2) 实例级梯度正交化，减少遗忘信号与保留信号之间的冲突。

Result: 在两个多任务计算机视觉基准上（五个任务）进行实验，全任务遗忘中UIS较最强基线降低30.3%，部分任务遗忘中降低52.9%。

Conclusion: 所提方法在有效实现遗忘的同时保持了较强的泛化能力，为多任务学习环境中的机器遗忘提供了可行的解决方案。
...
### [112] [DecisionBench: A Benchmark for Emergent Delegation in Long-Horizon Agentic Workflows](https://arxiv.org/abs/2605.19099)
*Yuxuan Gao,Megan Wang,Yi Ling Yu,Zijian Carl Ma,Ao Qu*

Main category: cs.AI

TL;DR: DecisionBench是一个用于评估长周期智能体工作流中委托能力的基准测试框架，包含任务套件、模型池、委托接口、技能标注层和多维度指标，可对学习路由器、peer记忆等进行评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对智能体委托能力的系统化评估，需要一个能够衡量质量、成本、延迟、委托率、路由保真度等多维指标的通用基准 substrate，以推动委托方法的发展。

Method: 构建包含GAIA、tau-bench、BFCL多-turn等任务套件的固定基线，使用11个模型和7个厂商家族的peer池，通过call_model和read_profile通道进行委托，配备确定性技能标注层和五条件参考扫描（n=23,375任务实例）。

Result: (i)四种意识条件下端任务质量统计分析无显著差异(|beta|<=0.010,p>=0.)；(ii)路由保真度在7.5%-29.5%之间，传递通道主导描述内容；(iii)反事实上限显示完美委托比实测性能高15-31个百分点，存在较大未实现提升空间。

Conclusion: DecisionBench为委托系统的评估提供了标准化、可扩展的基准框架，揭示了当前委托方法的局限性，为未来编排方法的发展指明了方向。
...
### [113] [Swimming with Whales: Analysis of Power Imbalances in Stake-Weighted Governance](https://arxiv.org/abs/2605.19264)
*Yuzhe Zhang,Manvir Schneider,Qin Wang,Davide Grossi*

Main category: cs.AI

TL;DR: 研究PoS区块链中质押权重投票制度的权力失衡问题，利用Penrose-Banzhaf权力指数量化分析，并结合真实链上治理数据实证验证。


<details>
  <summary>Details</summary>
Motivation: 当前PoS区块链采用质押权重投票作为核心治理范式，但存在权力扭曲现象：大户可能完全控制决策，即便不拥有全部质押代币。现有研究对这种权力失衡的程度和性质缺乏深入理解。

Method: 从计算社会选择理论角度，运用Penrose-Banzhaf权力指数量化权力；理论分析在特定条件下权力与质押ownership的期望近似对齐；实证使用Project Catalyst真实链上治理数据验证。

Result: 证明完美对齐一般不可达，但在特定条件可近似期望对齐；基于真实数据揭示当前质押权重治理系统中的权力失衡特征。

Conclusion: 为PoS治理系统的权力分布提供理论基础和实证洞察，有助于设计更公平的链上治理机制。
...
### [114] [EngiAI: A Multi-Agent Framework and Benchmark Suite for LLM-Driven Engineering Design](https://arxiv.org/abs/2605.19743)
*Gioele Molinari,Florian Felten,Soheyl Massoudi,Mark Fuge*

Main category: cs.AI

TL;DR: 本文提出了EngiBench基准套件，用于评估大型语言模型在工程设计中的多智能体系统表现，包含工作流、RAG和HPC三个评测维度，并开源了基于LangGraph的多智能体系统EngiAI作为参考实现。


<details>
  <summary>Details</summary>
Motivation: 现有评估框架未能充分涵盖结合仿真、检索和制造准备的多智能体系统，需要专门的工程设计任务评估基准。

Method: 构建了三维度评估基准：(1)包含七种提示风格的工作流基准，测试不同认知需求；(2)RAG基准通过门控评分隔离检索贡献；(3)HPC基准评估SLURM集群上的ML训练编排。同步提出EngiAI多智能体参考实现，通过监督架构协调七个专业化智能体。

Result: 在四个LLM后端和两个EngiBench问题上，专有模型在Beams2D上达到96-97%平均任务完成率，开放式4B参数模型达55-78%。条件分支最具挑战性，Photonics2D条件样式降至20-53%。RAG门控验证了检索增强的近满分与无检索的近零分对比。HPC编排中，一模型100%完成所有步骤，另一模型降至50%。

Conclusion: 条件分支和多步指令遵循是当前LLM的显著短板，RAG对参数选择至关重要，多智能体协同可有效提升工程设计任务完成率，但长流程下的指令following能力仍需改进。
...
### [115] [Progressive Autonomy as Preference Learning: A Formalization of Trust Calibration for Agentic Tool Use](https://arxiv.org/abs/2605.19151)
*Changkun Ou*

Main category: cs.AI

TL;DR: 本文将智能体工具使用的信任校准形式化为偏好学习问题，通过高斯过程后验和probit似然来建模人类风险容忍度函数，并在结果最不确定时升级至人工审批，展示了这是优先贝叶斯优化的一个实例。


<details>
  <summary>Details</summary>
Motivation: 在自主智能体系统中，需要决定何时可自动执行动作、何时需人工审批。现有方法缺乏系统的信任校准机制，难以根据人类风险偏好动态调整自动化决策边界。

Method: 构建策略网关：使用高斯过程后验估计潜在人类风险容忍度函数，通过二值批准/拒绝反馈的probit似然观测该函数，在审批结果不确定性最大处升级至人工。

Result: 该方法在结构上等同于优先贝叶斯优化，可继承其近似高斯过程分类的推理框架和不确定性目标查询的样本效率优势，但目标是将动作空间分类为允许/阻止/询问区域而非优化设计。

Conclusion: 该方法为自主智能体的可信部署提供了系统性的偏好学习和不确定性感知决策框架，在保持安全的同时最大化自主性。
...
### [116] [Memory-Augmented Reinforcement Learning Agent for CAD Generation](https://arxiv.org/abs/2605.19748)
*Yin Xiaolong,Liu Yu,Shen Jiahang,Lu Xingyu,Ni Jingzhe,Fan Fengxiao,Sang Fan*

Main category: cs.AI

TL;DR: 本文针对复杂CAD模型生成的难题，提出了一种记忆增强强化学习框架。该框架通过双轨记忆模块（案例库和技能库）、动态utility检索以及闭环验证机制，使智能体能够避免语义相似但几何不可行的检索陷阱，实现无需额外大规模标注数据的在线自修正和持续进化。实验验证了该方法在复杂CAD模型生成中的有效性和优越性。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的CAD生成方法在处理复杂长操作序列、多样化操作类型和强几何约束的CAD模型时存在不足，主要因为推理链中断且缺乏有效的错误修正机制。

Method: 提出了记忆增强强化学习框架，整合几何内核为可调用工具链，构建设计意图理解、全局规划、执行和多维验证的闭环机制；设计了包含案例库和技能库的双轨记忆模块，提出动态utility检索算法；将强化学习引入检索和策略优化。

Result: 实验表明，该方法显著提高了复杂CAD模型生成任务的成功率和几何一致性。

Conclusion: 本文提出的记忆增强强化学习框架能够有效解决复杂CAD模型生成中的推理链断裂和错误修正问题，通过双轨记忆模块和动态utility检索算法实现无需大规模标注数据的在线自修正和持续演化。
...
### [117] [Not all uncertainty is alike: volatility, stochasticity, and exploration](https://arxiv.org/abs/2605.19215)
*Payam Piray*

Main category: cs.AI

TL;DR: 本文研究了在可变环境中，自适应决策如何平衡探索与利用。先前工作将不同来源的不确定性视为等价，但本文发现：波动性(volatility)和随机性(stochasticity)虽都增加后验不确定性，却驱动最优探索朝相反方向发展——波动性增强探索，随机性抑制探索。通过将Gittins索引框架扩展到高斯状态空间bandits并结合控制as推理方法，导出CAUSE探索策略，在异构噪声结构环境中表现优异，并预测病态噪声推断会导致反向探索而非仅损害性探索。


<details>
  <summary>Details</summary>
Motivation: 核心动机是挑战传统观点——先前研究通常将不同来源的环境不确定性（如波动性与随机性）等价处理，忽视了它们对探索行为的差异化影响。论文旨在建立理解环境不确定性与探索之间关系的统一理论框架。

Method: 采用两类方法：一是扩展Gittins索引框架至具有潜在动态的高斯状态空间bandits，通过形式化推导建立波动性与随机性的不对称性；二是通过控制as推理（control-as-inference）方法推导出封闭形式的探索奖励机制——CAUSE。

Result: 主要发现：（1）波动性和随机性虽都增加后验不确定性，但驱动最优探索方向相反；（2）CAUSE在异构噪声结构环境中优于标准探索策略；（3）Gittins-per-arm策略的restless-bandit最优性不能转移；（4）学习与探索受同一噪声-推理不对称性支配；（5）病理噪声推断产生"反转"探索而非仅损害性探索。

Conclusion: 论文建立了环境不确定性与探索之间的系统性理论关系，揭示了波动性与随机性对决策的差异化影响。这一"噪声-推理不对称"原理为理解认知功能及精神疾病的计算模型提供了新视角。
...
TL;DR: 研究提出五阶段框架，将大语言模型整合到调查研究的完整流程中，在2024年飓风米尔顿 Preparedness调查中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 调查研究面临多重结构挑战：回复率下降、样本偏差、高风险人群的块状缺失数据、以及在线面板中的AI辅助欺诈性回复。虽然LLM被提议作为解决方案，但在整个调查工作流程中严格评估仍然缺乏，尤其是在数据质量至关重要的灾害背景下。

Method: 该研究提出了一个五阶段框架，整合问卷设计、样本选择、试点测试、缺失数据插补和后收集分析。使用2024年飓风米尔尔顿 Preparedness调查(n=946)作为共享实证测试床。引入了保护动机理论(PMT)约束的共现知识图谱，并开发了七种LLM配置，包括零样本推理、检索增强基线和新的理论知情变体。提出了锚定边际理论知情LLM(A-TLM)。

Result: 提出的A-TLM在灾害相关块状MNAR条件下优于三类经典插补基线(IPW/MI, MICE+PMM, missForest)，RMSE为1.439对比下一最佳1.496实现了更低的RMSE，同时实现接近零的有符号偏差(-0.121)，而随机森林插补产生最大绝对偏差(-0.631)。围绕PMT因果结构组织检索并在单次模型调用中整合所有证据，优于非结构化检索和分阶段顺序推理(MAE 0.993 vs 1.097)。研究表明接近零的总体偏差可能掩盖对立的子组误差。

Conclusion: 该研究证明了LLM可以有效整合到调查研究工作流程中，特别是通过PMT因果结构约束的检索增强方法。A-TLM方法在灾害相关的缺失数据条件下表现最优。研究强调需要将子组分层偏差审计作为报告标准，以发现总体聚合偏差可能掩盖的子组误差。检索约束知识图谱聊天机器人表明，通过接地拒绝可以在架构上管理幻觉问题。
...
TL;DR: 研究多模态大语言模型的模态冲突幻觉问题，通过因果分析发现两类作用相反的注意力头，提出MACI条件干预方法可在检测到冲突时有针对性抑制导致幻觉的头部，显著降低幻觉同时保持较好的准确率


<details>
  <summary>Details</summary>
Motivation: 探究多模态大语言模型(MLLMs)在文本与视觉信息冲突时，为何视觉证据无法有效约束生成，即"模态冲突幻觉"问题

Method: 对五个开源MLLMs进行头部级别的因果分析，使用path patching技术，识别 hallucination-driving 和 hallucination-resisting 注意力头，并进行消融实验验证

Result: MACI在五个MLLMs上于MMMC基准测试中实现了最大的幻觉降低，且具有有利的幻觉-准确率权衡，并可零样本迁移到SCI-SemanticConflict测试

Conclusion: 模态冲突幻觉源于驱动影响分散与局部抵抗之间形成的 imbalance routing 结构；MACI条件干预通过选择性地抑制驱动幻觉的注意力头来缓解此问题
...
### [120] [Agentic Trading: When LLM Agents Meet Financial Markets](https://arxiv.org/abs/2605.19337)
*Yihan Xia,Panpan You,Taotao Wang,Fang Liu,Han Qi,Xiaoxiao Wu,Shengli Zhang*

Main category: cs.AI

TL;DR: 该论文是一项关于大型语言模型(LLM)在交易系统中作为智能体的系统性综述，审查了77项研究，重点关注19项满足核心实证标准的研究，旨在评估该领域的评估协议可比较性和可复现性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在金融交易系统中的应用快速增长，但缺乏对该领域研究方法的系统性审查和评估，尤其关注可复现性、评估协议标准化等关键问题。

Method: 采用审计导向的证据地图方法，对截至2026年3月9日的77项研究进行协议编码筛选，建立主要实证子集(n=19)和背景设计上下文(n=58)的分类体系，运用架构-能力-适应分析框架进行评估。

Result: 核心发现是协议不可比较性：仅2/19研究有可提取的时间一致分割协议，1/19有明确的交易成本模型，1/19记录了宇宙或幸存者处理，11/19报告了执行时间或语义，15/19被编码为R0水平，无研究达到R3可复现性。

Conclusion: LLM交易智能体的架构实验正在快速扩展，但可比较的评估协议、执行语义和可复现 artifact 仍是该领域面临的紧迫瓶颈。研究呼吁建立更标准的评估框架和报告规范。
...
### [121] [Generative Recursive Reasoning](https://arxiv.org/abs/2605.19376)
*Junyeob Baek,Mingyu Jo,Minsu Kim,Mengye Ren,Yoshua Bengio,Sungjin Ahn*

Main category: cs.AI

TL;DR: GRAM是一种将递归.latent推理转化为概率多轨迹计算的框架，通过随机.latent轨迹支持多假设和多种解法，采用摊销变分推断训练，在结构和约束推理任务上取得改进。


<details>
  <summary>Details</summary>
Motivation: 现有递归推理模型(RRM)是确定性的，只能遵循单一.latent轨迹并收敛到单个预测。无法实现多假设、多解法或推理时的计算扩展。

Method: 采用摊销变分推断(amortized variational inference)训练，将递归.latent推理建模为随机.latent轨迹，支持递归深度和并行轨迹采样两种 inference-time 扩展策略。

Result: 在结构化推理和约束满足任务上优于确定性递归和递归基线方法，并展示了无条件生成能力。

Conclusion: GRAM通过将递归推理转化为概率多轨迹计算，为神经推理系统提供了一种新的扩展方式。它不仅支持条件推理p_θ(y|x)，还支持无条件生成p_θ(x)，在结构化推理和约束满足任务上优于确定性基线方法。
...
### [122] [Conflict-Resilient Multi-Agent Reasoning via Signed Graph Modeling](https://arxiv.org/abs/2605.19418)
*Longgang He,Longzhu He,Daojing He,Chaozhuo Li*

Main category: cs.AI

TL;DR: SIGMA是一个基于大语言模型的多智能体系统框架，通过显式建模智能体间的信任、冲突和中立关系（使用带符号的关系图），结合冲突感知的消息传递和结构感知的聚合机制，解决现有图中多智能体系统在处理冲突信号时的误差传播问题，显著提升准确性和冲突弹性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多智能体系统在聚合机制上存在naive假设，导致(1)冲突信号出现时无控制地传播错误，(2)缺乏对冲突的智能体间关系的显式建模和结构感知，无法识别可靠交互模式。

Method: 提出SIGMA框架：1)选择相关且多样化的智能体集合；2)构建带置信度加权的符号交互图；3)通过冲突感知的符号消息传递，强化可信智能体信息、抑制冲突信号；4)采用结构和冲突感知的加权聚合生成最终预测。

Result: 在6个基准数据集、多LLM主干和多样化多智能体配置下，SIGMA一致优于SOTA基线，在准确性和冲突弹性方面取得显著提升。

Conclusion: 通过显式建模智能体间的信任/冲突/中立关系并引入冲突感知的消息传递机制，SIGMA成功解决了传统多智能体系统中的误差传播和冲突处理问题，为LLM多智能体协同推理提供了新的有效范式。
...
### [123] [Beyond Mode Collapse: Distribution Matching for Diverse Reasoning](https://arxiv.org/abs/2605.19461)
...
TL;DR: 本文提出DMPO（分布匹配策略优化）方法，通过前向KL散度的原则性近似来防止强化学习中的模式崩溃问题，在NP难组合优化任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前基于GRPO等策略的强化学习方法存在模式崩溃问题，一旦发现高回报轨迹就会停止探索，导致解的多样性降低。这是因为反向KL最小化具有寻求模式的特性，倾向于强化单一高回报解而不是维持多个多样化解的分布。

Method: DMPO通过构建组级目标分布来近似前向KL散度。具体而言：对采样的轨迹按照其回报进行加权构建目标分布，然后使策略分布与该目标分布对齐。这种方法无需从难以处理的全局目标分布中采样，实现了训练过程中的持续探索。

Result: 在NP难组合优化基准测试中，DMPO在文本NP-Bench上达到43.9%的质量比（GRPO为40.1%），在视觉NP-Bench上达到43.1%（GRPO为38.4%），分别实现9%和12%的相对提升。此外，在数学推理任务上提升2.0%，域外任务上提升2.3%，表明多样性保持的训练增强了跨模态的泛化能力。

Conclusion: 本文证明了分布匹配是防止on-policy RL中模式崩溃的实用且原则性的方法，通过持续探索在各种推理任务上带来一致的质量提升。DMPO作为前向KL散度的有效近似，为强化学习的探索提供了新的理论基础和实践方法。
...
### [124] [Attention-Guided Reward for Reinforcement Learning-based Jailbreak against Large Reasoning Models](https://arxiv.org/abs/2605.19485)
*Zheng Lin,Zhenxing Niu,Haoxuan Ji,Yuzhe Huang,Haichang Gao*

Main category: cs.AI

TL;DR: 本文研究大型推理模型(LRMs)面临的越狱攻击问题,发现攻击成功率与注意力模式密切相关,并提出一种基于强化学习的新型越狱方法,通过在奖励函数中融入注意力信号来增强攻击效果。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在生成结构化推理内容方面表现出色,但暴露内部推理过程会引入安全风险。研究表明LRMs比标准LLMs更容易受到越狱攻击,因此需要深入理解其脆弱性机制。

Method: 1)分析越狱攻击与LRMs注意力模式的相关性;2)提出基于强化学习的越狱方法,将注意力信号纳入奖励函数设计;3)引入多样化的说服策略来丰富RL动作空间。

Result: 在5个开源和闭源LRMs上进行了三大基准测试,实验表明该方法在攻击成功率、效率和可迁移性方面均优于现有方法。

Conclusion: 成功越狱往往表现为对输入中有害token分配较低注意力,而对推理内容中的token分配较高注意力。提出的RL方法通过显式利用注意力信号能更有效地实施越狱攻击,为LRMs的安全评估提供了新思路。
...
### [125] [Position: The Turing-Completeness of Real-World Autoregressive Transformers Relies Heavily on Context Management](https://arxiv.org/abs/2605.19514)
*Guanyu Cui,Zhewei Wei,Kun He*

Main category: cs.AI

TL;DR: 本文澄清了关于Transformer图灵完备性的常见误解，区分了两种设置：固定系统设置（单个固定模型处理不同长度输入）和缩放族设置（模型家族随上下文窗口增长）。作者指出现有证明多在缩放族设置中成立，但实际LLM部署对应的是固定系统设置，因此这些证明未能真正确立图灵完备性。


<details>
  <summary>Details</summary>
Motivation: 当前文献常宣称Transformer是图灵完备的，但忽视了两种设置的根本差异。固定系统设置更符合真实世界LLM的实际运作方式，而现有证明往往建立在缩放族设置上，这导致了概念上的混淆和误导。

Method: 首先形式化固定系统设置，提供具体表征；然后分析比较两种设置的理论差异；最后探讨不同上下文管理方法对计算能力的 影响。

Result: 明确了固定系统设置与缩放族设置的根本区别；揭示了现有在缩放族设置中的图灵完备性证明并不适用于真实世界的LLM部署；表明上下文管理方法是决定Transformer计算能力的关键组件。

Conclusion: 图灵完备性的验证需在固定系统设置下进行才有实际意义；上下文管理方法应当被视为确定真实世界自回归Transformer计算能力的重要因素，而非可忽略的技术细节。
...
### [126] [BLINKG: A Benchmark for LLM-Integrated Knowledge Graph Generation](https://arxiv.org/abs/2605.19518)
*Carla Castedo,Enrique Iglesias,Manuel Lama,Alberto Bugarin-Diz,Maria-Esther Vidal,David Chaves-Fraga*

Main category: cs.AI

TL;DR: BLINKG是一个用于评估LLM从异构数据源构建知识图谱映射能力的基准测试，包含基于真实用例的复杂程度递增的场景，并对多个先进LLM进行了广泛评估，发现它们在简单场景下表现良好但复杂场景仍有局限。


<details>
  <summary>Details</summary>
Motivation: 当前生成知识图谱仍需大量人工工作，虽然LLM引起了研究兴趣，但尚无标准框架来评估它们在数据模式与本体概念之间建立对应关系的有效性。

Method: 设计了一套基于真实用例的复杂程度递增的测试场景，对多个最先进的LLM进行了广泛的实验评估。

Result: LLM已显示出不错的解决方案，但在复杂场景下表现仍有局限。

Conclusion: 该研究提出了BLINKG基准，为评估LLM在知识图谱构建中的映射能力提供了标准化框架，定义了实现半自动化KG构建的要求，为该领域开辟了新的研究方向。
...
### [127] [Efficient Elicitation of Collective Disagreements](https://arxiv.org/abs/2605.19521)
*Mohamed Ouaguenouni,Felipe Garrido-Lucero,Umberto Grandi,César Hidalgo,Magdalena Tydrichova*

Main category: cs.AI

TL;DR: 本文提出了一种新的 pluralism matrix 框架来解决选民偏好分歧测量问题。研究发现传统成对比较方法不足，需三元组才能准确表达多个重要分歧测度（如rank-variance、divisiveness等）。通过定义分歧测度的层级概念，证明多数现有方法位于第三层级，并设计了两个数据收集协议实现更优的偏好诱导。


<details>
  <summary>Details</summary>
Motivation: 现有调查方法中，成对比较无法区分结构化分歧和噪声，而完整排名虽然能获取全部偏好信息但对参与者认知负担较重。文章旨在解决如何在最少的偏好信息下有效测量选民之间的分歧程度。

Method: 研究采用了分层框架分析方法，引入plurality matrix（多数矩阵），定义分歧测度的层级为表达该测度所需的最小子集大小。通过数学推导证明多个现有分歧概念（包括排名方差和分裂度）位于第三层级，并通过理论分析和实验验证超越第三层级的可能性和价值。设计了两个诱导协议来估计plurality matrix，平衡参与人数和认知负荷。

Result: 研究结果显示：1）成对比较不足以表达许多重要的分歧测度；2）多项现有分歧概念（包括rank-variance和divisiveness）位于第三层级，需要三元组而非成对比较才能准确表达；3）超越第三层级的分析具有理论和实验价值；4）设计了两种有效的elicitation协议，可以在减少参与者数量和降低认知负荷之间取得平衡。

Conclusion: 本研究的结论是现有成对比较方法不足以捕捉完整的偏好分歧结构，通过plurality matrix框架可以更准确地表达和测量选民之间的分歧程度。实验证明超越第三层级的分析在理论和实践上都有重要价值，同时设计了两个诱导协议来估计plurality matrix，为实际应用提供了可行的数据收集方案。
...
### [128] [Generative-Evaluative Agreement: A Necessary Validity Criterion for LLM-Enabled Adaptive Assessment](https://arxiv.org/abs/2605.19529)
*Grandee Lee,Yue Wang,Che Yee Lye,Luke Peh*

Main category: cs.AI

TL;DR: 本文引入生成-评估一致性(GEA)作为LLM评估的新有效性标准，用于衡量LLM的评分功能是否能恢复其生成功能被要求呈现的技能水平。在对两阶段自适应评估的首次直接测量中，模型恢复了约一半的预期方差(r=0.698)，存在系统性正偏差。GEA在句法可验证技能上表现强劲(r>0.7)，但在设计层面技能上接近为零，低技能高估会夸大路由阈值附近的分数。


<details>
  <summary>Details</summary>
Motivation: 当同一LLM生成评选项、模拟学生响应并对其进行评分时，验证循环变成自我指涉的。传统评估有效性理论假设生成和评分是独立函数，但LLM可能模糊这一界限，需要新的有效性衡量标准来检测这种系统性偏差。

Method: 在两阶段自适应评估上进行GEA的直接测量，使用LLM的生成功能产生不同技能水平的项目，评分功能对这些响应进行评分，然后检验评分是否恢复到预期的技能方差。

Result: 模型仅恢复了约一半的预期方差(r=0.698)，且存在系统性的正偏差。对于句法可验证技能，GEA较强(r>0.7)；但对于设计层面技能，GEA接近零。低技能高估导致在路由阈值附近的分数被夸大。

Conclusion: 建议采用granular, skill-decomposed rubrics（细粒度、技能分解的评分规则）作为加强GEA的主要机制，并概述了补充性缓解措施。这是首次对LLM自生成评估中自我指涉问题的直接测量，为AI辅助评估的有效性研究提供了新框架。
...
### [129] [Library Drift: Diagnosing and Fixing a Silent Failure Mode in Self-Evolving LLM Skill Libraries](https://arxiv.org/abs/2605.19576)
*Xing Zhang,Yanwei Cui,Guanghui Wang,Ziyuan Li,Wei Qiu,Bing Zhu,Peiyang He*

Main category: cs.AI

TL;DR: 本文提出了自Evolution技能库中的一个关键失败模式——"库漂移"（library drift）：无限制的技能积累导致检索下降、误报注入和性能停滞。通过消融实验隔离了这一问题，并提出了包含结果驱动退休、边界主动cap和元技能创作先验的治理方案，在MBPP+ hard-100上将pass@1从0.258提升到0.584。


<details>
  <summary>Details</summary>
Motivation: 当前自Evolution技能库存在一个静默失败模式——库漂移，表现为无限制的技能积累但缺乏结果驱动的生命周期管理，导致检索退化、误报注入和性能停滞。已有评估显示LLM编写的技能为+0.0pp增益，而人工策划的技能为+16.2pp增益，但潜在机制尚未被分离出来。因此需要识别并解决这一根本问题。

Method: 通过三种主要方法进行研究：(1)消融实验——禁用技能注入（平面地板，+0.002）和强制过早退休（主动伤害，-0.019）来隔离漂移；(2)追踪级诊断——创建仅追加的证据日志，包含每技能贡献分数、归因判断和路由参与度指标，使失败在任务终分数之前可见；(3)验证修复——实施最小治理方案（结果驱动退休+边界主动cap+元技能创作先验），在100轮内监控性能变化。

Result: 治理方案将held-out pass@1从0.258基线提升到晚窗口均值0.584，滚动增益+0.328。八个消融实验分解了哪些治理机制是_load-bearing_（可承载的），哪些是被吸收的。最关键的发现是：无结果驱动退休会导致性能停滞，无边界cap会导致技能无限累积，元技能创作先验能有效引导新技能的生成。

Conclusion: 本文成功识别、诊断并修复了自Evolution技能库中的库漂移问题。通过可重现的触发器、追踪级诊断工具和经过验证的最小治理方案，为构建可靠的自Evolution智能体提供了具体的操作手册。未来工作可以探索如何自动检测漂移阈值，以及将治理方案扩展到其他类型的自修改系统。
...
### [130] [SceneCode: Executable World Programs for Editable Indoor Scenes with Articulated Objects](https://arxiv.org/abs/2605.19587)
*Puyi Wang,Yuhao Wang,Linjie Li,Zhengyuan Yang,Kevin Qinghong Lin,Yangguang Li,Yu Cheng*

Main category: cs.AI

TL;DR: SceneCode是一个将室内场景合成问题形式化为程序化世界生成的框架，将自然语言提示编译为可执行的代码驱动室内世界，而非静态网格。该方法使用房间级Agent主干通过规划器-设计器-批评者循环将提示转换为结构化房屋布局，并为每个对象发出AssetRequest，然后路由到五种代码生成策略之一，转换为合成的分部分Blender Python程序，通过执行引导的修复和优化循环进行验证，最终编译为模拟就绪资产并导出为SDF格式用于物理模拟。


<details>
  <summary>Details</summary>
Motivation: 现有管道通常将生成内容表示为静态网格，并仅从策划的资产库中继承关节连接，这限制了对象级可控性，无法按需生成新的可交互资产。室内场景合成支持embodied AI、机器人物理操作和基于模拟的策略评估，需要的场景不仅要指定环境外观，还要指定对象结构。

Method: 首先，房间级agentic骨干通过planner-designer-critic循环将提示转换为结构化房屋布局，并为每个对象发出AssetRequests。每个请求然后被路由到五种代码生成策略之一，并转换为合成的分部分Blender Python程序。这些程序通过execution-guided repair-and-refine循环进行验证。生成的程序被编译为模拟就绪资产，并导出为SDF用于物理模拟。持久的场景状态注册表将对象请求、可执行程序、渲染几何体和模拟资产链接起来，使场景组装成为可追踪且本地可编辑的世界构建过程。

Result: 在场景级合成、对象级资产质量、人体判断和下游机器人交互方面对SceneCode进行了评估。结果表明，可执行的世界程序提高了符合提示的室内场景生成，并产生了具有更清洁网格结构和模拟器可加载关节元数据的资产。

Conclusion: 该框架通过将场景组装转变为可追踪且本地可编辑的世界构建过程，有效弥合了静态网格表示与物理可交互室内场景合成之间的差距，提供了更好的对象级可控性和按需生成新交互资产的能力。
...
### [131] [Towards Multi-Model LLM Schedulers: Empirical Insights into Offloading and Preemption](https://arxiv.org/abs/2605.19593)
*Mert Yildiz,Pietro Spadaccino,Alexey Rolich,Francesca Cuomo,Andrea Baiocchi*

Main category: cs.AI

TL;DR: 本文研究了现代LLM多模型服务场景下，如何在共享异构硬件上进行资源分配、调度以及部分CPU-GPU卸载和抢占的实证分析。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型部署需要支持多模型、多架构、不同规模和专业化的混合服务，但现有系统主要针对单一模型优化吞吐量，对多模型调度研究不足，特别是在GPU显存受限下如何进行部分卸载和预empt机制缺乏深入理解。

Method: 本文通过对不同LLM在不同硬件平台上的实证研究，聚焦于层卸载和预empt对性能的影响，分析了解码吞吐量、模型状态重载、KV缓存传输开销等因素，并考察了序列长度和互连带宽对数据移动和执行效率的影响。

Result: 研究表明：卸载导致解码吞吐量呈现强烈非线性和模型依赖性的下降，小模型对降低的GPU residency更敏感；预empt产生显著开销，主要由模型状态重载而非KV缓存_transfer驱动，该成本因模型和硬件平台差异显著不同；序列长度和互连带宽会放大数据移动和执行低效问题。

Conclusion: 为未来调度器识别出一组关键特征：模型特定的卸载敏感性、工作负载特性、预empt和数据_transfer的成本结构等，为设计能够高效管理异构多模型工作负荷和混合CPU-GPU执行的下一代LLM服务系统提供指导。
...
### [132] [EMO-BOOST: Emotion-Augmented Audio-Visual Features for Improved Generalization in Deepfake Detection](https://arxiv.org/abs/2605.19630)
*Aritra Marik,Marcel Klemt,Anna Rohrbach*

Main category: cs.AI

TL;DR: Emo-Boost是一个多模态深度伪造检测框架，通过融合情感识别模块(EmoForensics)和现有的低级特征检测器来提升对未见过 manipulated类型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI技术快速发展，使得针对每种新型 manipulations 都收集训练数据变得不可能。现有深度伪造检测模型难以泛化到未见过的伪造类型，这是当前研究的核心挑战。

Method: 提出Emo-Boost框架，融合RGB和音频特征的低级检测器与基于情感的高级语义检测器EmoForensics。EmoForensics利用视觉和音频情感识别模块，对音视频流中的情感表征进行模态内和模态间的时间一致性建模。

Result: 在FakeAVCeleb数据集上，Emo-Boost将跨 manipulation泛化的平均AUC提升了2.1%。研究表明情感检测器与低级特征检测器捕获互补信号。

Conclusion: 高级语义线索（如情感信息）可有效支持低级特征方法，提升对未知深度伪造类型的检测泛化能力，多模态融合策略是有效的解决方案。
...
### [133] [When Tabular Foundation Models Meet Strategic Tabular Data: A Prior Alignment Approach](https://arxiv.org/abs/2605.19662)
...
TL;DR: 研究表格基础模型（PFN）在战略环境中（即个体可能策略性修改特征的设置）的泛化能力，提出SPN框架在不重新训练的情况下适应战略环境


<details>
  <summary>Details</summary>
Motivation: 现实中个体可能在分类器部署后策略性地修改特征以获取有利结果，导致部署后分布偏移，而传统PFN设计的非战略设置不适用于此场景

Method: 提出Strategic Prior-data Fitted Network (SPN)，通过构建战略性的上下文示例来近似策略后输入，并对齐PFN预测与诱导的战略分布

Result: 在真实和合成表格数据集上的实验表明，SPN在战略操纵下相比表格基础模型和经典表格方法一致提高鲁棒性和预测性能

Conclusion: PFN-style表格基础模型可以在战略环境中实现良好泛化，但需要像SPN这样的策略感知框架来弥补预训练的非战略先验与后操纵战略先验之间的差距
...
### [134] [Pseudocode-Guided Structured Reasoning for Automating Reliable Inference in Vision-Language Models](https://arxiv.org/abs/2605.19663)
*Weicong Ni,Tianbao Jiang,Linlin Wang*

Main category: cs.AI

TL;DR: PStar是一种针对视觉语言模型幻觉问题的伪代码引导结构化推理框架，通过难度特征向量自适应选择推理路径，在POPE和MMStar基准取得SOTA，超越GPT-4V。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)在机器人自动化中易产生幻觉导致决策失败，且现实任务开放性和多样性要求模型具备强健的自适应推理能力，现有方法难以应对复杂多变的问答场景。

Method: 设计抽象推理函数库，构造结构化伪代码表示模块化推理策略；引入难度特征向量(DFV)评估问题复杂度并自适应选择合适推理策略，实现灵活的分步推理。

Result: 在POPE基准达到87.1%准确率，在MMStar基准达到68.0%准确率，超越GPT-4V，显著降低幻觉率。

Conclusion: PStar提出了一种通过伪代码指导的结构化推理框架，通过难度特征向量(DFV)自适应选择推理策略，显著降低了VLMs的幻觉率，在POPE和MMStar基准上达到SOTA性能，超越GPT-4V，为实际机器人系统中部署更可信的视觉语言模型提供了关键一步。
...
### [135] [Transforming Constraint Programs to Input for Local Search](https://arxiv.org/abs/2605.19671)
*Jo Devriendt,Patrick De Causmaecker,Marc Denecker*

Main category: cs.AI

TL;DR: 本文提出了一种通过约束优化问题的对称性自动生成局部搜索邻域的方法，并在IDP系统中实现，实验验证了对六种经典优化问题的有效性。


<details>
  <summary>Details</summary>
Motivation: 将局部搜索算法应用于组合优化问题时，通常需要人工干预来将约束编译为元启发式算法的输入数据，这一过程繁琐且耗时。

Method: 建立约束优化问题的对称性与局部搜索邻域之间的联系，利用这种联系从约束规范自动生成邻域，并在IDP系统中实现该技术。

Result: 对六种经典优化问题进行了评估实验，结果表明生成的邻域是有效的。

Conclusion: 该技术具有良好的可行性，可以自动化生成局部搜索邻域，减少人工干预的需求。
...
### [136] [Beyond Rational Illusion: Behaviorally Realistic Strategic Classification](https://arxiv.org/abs/2605.19674)
...
TL;DR: 本文提出行为现实主义战略分类问题，针对现有战略分类框架假设完全理性但实际决策受认知偏差影响的局限性。提出基于前景理论的Pro-SF框架，通过三个关键机制（非对称损益、主观参考点、非理性概率扭曲）来模拟行为现实主义的战略响应。


<details>
  <summary>Details</summary>
Motivation: 现有战略分类（SC）框架通常依赖严格理性假设，但行为经济学和心理学证据表明真实世界决策常受认知偏差影响，与纯理性偏离。作者识别并定义了这一新问题设置——行为现实主义战略分类问题。

Method: 提出基于前景理论的原则性框架Pro-SF来解决该问题。具体做法：将Stackelberg式互动重新建模，纳入前景理论启发的三个关键机制：1）损益不对称；2）不同主观参考点；3）非理性概率扭曲。通过合成和真实数据集实验验证。

Result: Pro-SF作为行为接地的方法，在合 成和真实数据集上验证有效，成为将机器学习与行为经济学相结合的更可靠现实部署方案。

Conclusion: 本文通过引入行为现实主义视角拓展了战略分类研究，提出Pro-SF框架能够更准确地模拟现实中受心理偏差影响的策略性操纵行为，为机器学习在真实世界的部署提供了更可靠的指导。
...
### [137] [Projecting Latent RL Actions: Towards Generalizable and Scalable Graph Combinatorial Optimization](https://arxiv.org/abs/2605.19721)
...
### [138] [CogScale: Scalable Benchmark for Sequence Processing](https://arxiv.org/abs/2605.19758)
*Yannis Bendi-Ouis,Romain de Coudenhove,Xavier Hinaut*

Main category: cs.AI

TL;DR: 本文提出CogScale基准，包含14个可扩展合成任务，用于评估架构的认知和记忆能力。在1k-100k参数预算下测试7种架构（GRU、LSTM、xLSTM、ESN、Mamba、Transformer Decoder/Encoder-Decoder），发现经典RNN和ESN适合资源受限的基本任务，但注意力机制和状态空间模型更具可扩展性优势。


<details>
  <summary>Details</summary>
Motivation: 现代模型虽然在自然语言处理等任务上取得了显著成功，但评估新型架构处理序列信息的能力仍然计算昂贵且耗时。测试新架构通常需要扩展到大规模数据集和模型，导致巨大的计算成本和缓慢的迭代周期。作者提出需要一个轻量级、标准化的基准来快速验证架构创新。

Method: 作者提出了CogScale，一个包含14个可扩展合成任务的基准，专门用于在不同参数化规模下隔离和评估特定的认知和记忆能力。他们在严格参数预算（1k、10k和100k）下评估了七种不同架构：GRU、LSTM、xLSTM、ESN、Mamba、Transformer Decoder和Transformer Encoder-Decoder。

Result: 结果表明，在严格的参数预算下，经典RNN和Echo State Networks在基本 Retention 任务上表现优异；但只有注意力机制和现代状态空间模型在推理复杂性和任务难度增加时能够保持高性能。

Conclusion: CogScale作为一个标准化、轻量级的框架，允许研究人员在commit到大规模训练之前快速验证架构创新。注意力机制和现代状态空间模型（如Mamba）在Scaling方面具有优势，而传统RNN和ESN在资源受限的基本任务上仍有价值。
...
### [139] [What Really Improves Mathematical Reasoning: Structured Reasoning Signals Beyond Pure Code](https://arxiv.org/abs/2605.19762)
*Yuze Zhao,Junpeng Fang,Lu Yu,Zhenya Huang,Kai Zhang,Qing Cui,Qi Liu,Jun Zhou,Enhong Chen*

Main category: cs.AI

TL;DR: 本文研究了代码在大型语言模型训练中的作用，通过10T token语料库的受控预训练实验，发现代码仅能提升编程能力，但无法作为通用推理增强器，反而会与知识密集型任务形成竞争关系。


<details>
  <summary>Details</summary>
Motivation: 代码已成为大型语言模型训练的标准组成部分，但其除编程外的其他作用尚不清楚。之前研究认为代码可以提升推理能力，但这一假设缺乏严格控制的实验验证。

Method: 在10T-token语料库上进行受控预训练实验，采用细粒度领域分离方法，将代码限制为独立可执行程序并控制Code-NL数据，单独分析代码的影响。

Result: 研究发现：1)代码只能显著提升编程能力，不能作为通用推理增强器，反而与复杂数学推理等知识密集型任务形成竞争；2)之前归因于代码的推理收益更可能来自跨域结构化推理轨迹（如code-text和math-text混合）；3)在固定数学预算内增加结构化数学样本密度可在保持编程性能的同时提升数学推理能力；4)路由分析显示数据组成效应反映在专家激活模式中。

Conclusion: 代码的推理增益主要源于跨域结构化推理轨迹而非单纯的可执行代码。认知脚手架（如结构化数学样本）可有效缓解跨域权衡问题。研究结果澄清了哪些数据特征可在能力维度间迁移，为更精准的数据优化策略提供了方向。
...
### [140] [GroupAffect-4: A Multimodal Dataset of Four-Person Collaborative Interaction](https://arxiv.org/abs/2605.19765)
...
TL;DR: GroupAffect-4是一个新的多模态数据集，包含40名参与者组成10个四人小组，完成4种不同类型的协作任务，旨在研究共处群体中的情感作为耦合的个人、人际和群体层面过程。


<details>
  <summary>Details</summary>
Motivation: 现有情感计算、社交信号处理和会议语料库很少支持将情感作为耦合的个人、人际和群体层面过程进行分析；所需的信号通常分散在不同的数据集传统中。

Method: 40名参与者组成10个四人小组，每组完成四个生态多样化的协作任务：信息汇总、谈判、创意生成和公共产品游戏。每位参与者佩戴手腕生理传感器、眼动追踪眼镜和近距麦克风；收集连续的自我报告情感、问卷后测、任务结果和大五人格评分，所有数据按共享时钟时间对齐。

Result: 数据集覆盖了预期生理窗口的91%以上和眼动追踪窗口的98%以上；谈判模块的情感操纵检查确认了任务有效性；定义了15个跨三个分析层次的可基准测试目标（个人内状态、人际间特质、群体动态）。

Conclusion: GroupAffect-4以BIDS启发的结构发布，包含Croissant元数据、数据表、每会话质量报告和开放处理脚本，为研究共处群体情感提供了重要的多模态基准数据集。
...
### [141] [Minimax Optimal Variance-Aware Regret Bounds for Multinomial Logistic MDPs](https://arxiv.org/abs/2605.19768)
*Pierre Boudart,Pierre Gaillard,Alessandro Rudi*

Main category: cs.AI

TL;DR: 本文研究基于多分类logistic (MNL)模型的 episodic Markov Decision Processes (MDPs) 的强化学习。提出一种问题相关的常数 ᾱ_T ≤ 1/2，衡量最优下游价值函数的归一化平均方差。通过新算法实现 regret Õ(dH²ᾱ_T√T)，在一般情况下恢复现有界，在结构化 MDPs 中有改进。例如对于 KL 约束的鲁棒 MDPs，ᾱ_T = O(H^{{-1}})，将水平依赖性降低 H 倍。还建立了匹配的下界 Ω(dH²ᾱ_T√T)，证明 minimax 最优性。


<details>
  <summary>Details</summary>
Motivation: 现有 MNL 混合 MDPs 算法 regrets 为 Õ(dH²√T)，但在结构化 MDPs 中可以进一步改进。logistic bandit 文献中的思路启发了引入问题依赖常数来获得更紧的界。

Method: 引入问题相关常数 ᾱ_T ≤ 1/2，度量最优下游价值函数沿学习者轨迹的归一化平均方差。基于此设计新算法。

Result: 达到 regret Õ(dH²ᾱ_T√T)，在一般情况恢复现有界；对于 KL 约束的鲁棒 MDPs，ᾱ_T = O(H^{{-1}})，将horizon 依赖降低 H 倍。

Conclusion: 建立了匹配下界 Ω(dH²ᾱ_T√T)，证明 minimax 最优性（对数因子内），首次完全刻画了 MNL 混合 MDPs 的 regret 复杂度。
...
### [142] [Distribution-Free Uncertainty Quantification for Continuous AI Agent Evaluation](https://arxiv.org/abs/2605.19779)
*Yuxuan Gao,Megan Wang,Yi Ling Yu*

Main category: cs.AI

TL;DR: 本文将 split conformal prediction 和 adaptive conformal inference 应用于连续AI agent评估，提供distribution-free coverage guarantees。实验表明方法在校准误差、条件覆盖率和排名预测方面均表现优异，并支持多agent系统和大规模排名场景。


<details>
  <summary>Details</summary>
Motivation: AI agent evaluation 需要可靠的 quality forecasts 和排名稳定性保证，但现有方法缺乏 distribution-free 的理论保证，且无法处理 agent 发布后的 distribution shift 和多agent系统的复杂依赖关系。需要一个统一框架来提供 coverage guarantees 并控制错误率。

Method: 文章采用 split conformal prediction 和 adaptive conformal inference (ACI) 方法，并将这两个技术适配到continuous AI agent evaluation场景。具体包括：1) 构建 distribution-free coverage guarantees；2) 开发 compositional uncertainty bounds 处理多agent pipeline（通过模拟验证 inter-stage correlations）；3) 设计 pairwise rankings 的 conformal abstention rule 以控制 false-ranking rate；4) 开发 leaderboard-scale multiple testing 的 FDR-corrected abstention。使用50个agents和18个real-time signals进行 hourly 评估。

Result: 实验结果显示：1) Conformal intervals 在24h horizon 达到校准误差低于0.02；2) ACI 在 agent 发布后正确加宽35%区间然后 reconverges；3) Per-agent conditional coverage 围绕名义水平集中（平均80.4%，90%的agent在[72%,90%]内），满足 distribution-free guarantee；4) Cross-source 情感分歧预测排名不稳定性（r=0.64, p<0.01）；5) Circularity-controlled validation 确认框架捕获超出基准的信号（rho_s=0.52, p<0.01, n=35）。

Conclusion: 该研究为连续AI agent评估提供了 distribution-free 的理论保证框架。方法在实证上展现出良好的校准性能，Conditional coverage 达到预期水平，cross-source 信号能有效预测排名稳定性。该框架可扩展至多agent系统和大规模排名场景，具有 practical 和 theoretical 价值。
...
### [143] [Prior Knowledge or Search? A Study of LLM Agents in Hardware-Aware Code Optimization](https://arxiv.org/abs/2605.19782)
...
### [144] [From Prompts to Pavement Through Time: Temporal Grounding in Agentic Scene-to-Plan Reasoning](https://arxiv.org/abs/2605.19824)
*Ahmed Y. Gado,Omar Y. Goba,Alaa Hassanein,Catherine M. Elias,Ahmed Hussein*

Main category: cs.AI

TL;DR: 该研究探讨了时间条件化在多智能体通信中对自动驾驶车辆场景解释和规划的影响。研究者提出了三种具有不同时间整合程度的设计方案，并在BDD-X数据集的子集上进行评估。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型和多模态模型在支持自动驾驶车辆高级场景解释时，将时间视为次要属性，导致连续动作推理时出现不一致，危及安全性和可解释性。需要解决时间接地问题以维持推理连贯性。

Method: 引入三种具有递进时间整合程度的规划器架构，在BDD-X数据集的精选子集上使用语义、句法和逻辑指标进行评估，并进行定性分析。

Result: 时间条件化重塑了推理风格，但在标准NLP正确性指标上未显示统计显著改进。定性分析揭示了预测性危险推理、稳定纠正行为和策略分歧。

Conclusion: 基于提示的时间接地存在局限性，但定性优势表明时间条件化对危险预测和行为稳定有潜在价值。该研究建立了首个时间场景到规划推理的经验基准。
...
### [145] [Robotics-Inspired Guardrails for Foundation Models in Socially Sensitive Domains](https://arxiv.org/abs/2605.19940)
*Rebecca Ramnauth,Drazen Brscic,Brian Scassellati*

Main category: cs.AI

TL;DR: 提出Grounded Observer框架，将AI guardrails重新定义为交互轨迹的运行时行为控制问题，借鉴机器人学形式化方法，在三个敏感领域（小对话、居家自闭症治疗、学校行为降级）实现运行时干预，防止交互滑向不良状态。


<details>
  <summary>Details</summary>
Motivation: 基础模型广泛应用于教育、心理医疗、护理等社会敏感领域，此类领域的失败具有累积性和上下文依赖性。现有guardrail方法仅提供经验性风险降低，缺乏强制性的行为保证，且将安全性视为单个输出的属性而非交互轨迹的属性。

Method: 借鉴机器人学中不确定闭环系统的约束执行形式化构念，将guardrails重新定义为交互轨迹的运行时行为控制问题，在三个真实场景（小对话、居家自闭症治疗、学校行为降级）中部署Grounded Observer框架，实现运行时干预以减轻向不良交互状态的漂移。

Result: 该框架成功实现了运行时干预，使系统能够在保持对不同社会情境适应性的同时，缓解交互滑向不良状态的倾向，并在三个真实部署场景中验证了有效性。

Conclusion: 为确保更强有力的保证奠定基础，并讨论框架的扩展方向。该研究将AI安全从经验性的单轮输出审核推向交互轨迹的主动式运行时调控，为社交敏感领域AI部署提供了新范式。
...
### [146] [Probabilistic Tiny Recursive Model](https://arxiv.org/abs/2605.19943)
*Amin Sghaier,Ali Parviz,Alexia Jolicoeur-Martineau*

Main category: cs.AI

TL;DR: PTRM通过在递归步骤中注入高斯噪声实现随机探索，无需重训练即可在多个基准测试上获得显著精度提升，7M参数即超越大型语言模型。


<details>
  <summary>Details</summary>
Motivation: TRM的确定性递归容易收敛到次优解，缺乏逃逸机制；现有方法需任务特定的输入扰动，缺乏通用性。

Method: PTRM在每个深度递归步骤注入高斯噪声，使并行轨迹探索不同解 Basin，利用模型现有的Q头进行选择，无需重训练或任务特定增强。

Result: Sudoku-Extreme: 87.4%→98.75%；Pencil Puzzle Bench: 62.6%→91.2%；以7M参数达到91.2%精度 vs LLMs的55.1%，成本低于0.0001倍。

Conclusion: PTRM提供了一种任务无关的测试时计算扩展框架，通过随机探索有效解决了TRM的收敛问题，在复杂推理任务上以极小参数量超越了大型语言模型。
...
### [147] [GeoX: Mastering Geospatial Reasoning Through Self-Play and Verifiable Rewards](https://arxiv.org/abs/2605.20006)
*Kyeongjin Ahn,Seungeon Lee,Krishna P. Gummadi,Meeyoung Cha*

Main category: cs.AI

TL;DR: GeoX是一种基于自我对弈的地理空间推理框架，通过可执行程序和验证器在无需人工标注数据的情况下学习空间逻辑，在三种推理模式下联合优化问题生成与求解任务，显著提升视觉语言模型的地理空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 地理空间推理需要解决复杂的场景结构问题，但人工注释大规模组合问题的成本高昂，制约了能力发展；现有方法依赖大量人工标注数据，缺乏可扩展的自监督机制。

Method: 提出单一多模态策略，在三种推理模式（溯因、演绎、归纳）下生成并求解空间问题，配合图像理解工具和验证器执行程序获取奖励信号，通过强化学习联合优化问题生成与求解两个角色。

Result: 相比基线VLMs平均提升5.5分，与或超过在百万级人工数据上训练的传统基线水平，并发布了通过自Play积累的地理空间理解基准数据集。

Conclusion: GeoX是一个自监督框架，通过可执行程序获取空间逻辑，无需大规模人工标注数据即可实现地理空间推理能力的提升。
...
### [148] [AutoResearchClaw: Self-Reinforcing Autonomous Research with Human-AI Collaboration](https://arxiv.org/abs/2605.20025)
...
TL;DR: AutoResearchClaw是一个多智能体自主研究pipeline，通过五种机制（多智能体辩论、自愈执行器、可验证结果报告、人在环协作、跨运行演化）在ARC-Bench上比AI Scientist v2提高54.7%，定位为增强而非替代人类科学判断的研究放大器。


<details>
  <summary>Details</summary>
Motivation: 现有自主研究系统将研究过程建模为线性管道，依赖单智能体推理，执行失败时停止，不在运行间积累经验。这导致研究效率低下，无法从错误中学习。

Method: 提出五种核心机制：1)结构化多智能体辩论用于假设生成和结果分析；2)自愈执行器带Pivot/Refine决策循环将失败转化为信息；3)可验证结果报告防止伪造数字和虚假引用；4)人在环协作含七种干预模式；5)跨运行演化将过去错误转化为未来保障。

Result: 在ARC-Bench（25主题实验基准）上，AutoResearchClaw比AI Scientist v2性能提高54.7%。人对环消融实验表明，在高杠杆决策点的精准靶向协作始终优于全自主和逐步监督两种极端。

Conclusion: AutoResearchClaw作为研究放大器，增强而非替代人类科学判断。为自主研究系统提供了从线性管道向迭代学习、从单智能体向多智能体协作的重要范式转变。
...
### [149] [Probing Embodied LLMs: When Higher Observation Fidelity Hurts Problem Solving](https://arxiv.org/abs/2605.20072)
*Oussama Zenkri,Oliver Brock*

Main category: cs.AI

TL;DR: 研究大型语言模型(LLM)作为机器人认知组件，通过实证方法评估其在封闭循环具身任务中的行为表现。使用Lockbox机械谜题测试不同观测输入下的性能，发现反直觉结果：原始RGB输入表现最佳，完美符号观测反而最差。模拟中发现适度噪声(40%翻转概率)可提升2.85倍成功率。


<details>
  <summary>Details</summary>
Motivation: LLM在机器人系统中应用日益广泛，但其不透明的决策过程使得难以解释成功或失败原因。现有评估方法仅关注成功率，无法区分感知误差与推理失败的相互作用。

Method: 采用实证AI方法论，通过改变agent可用信息并测量行为变化来研究具身LLM agent。使用Lockbox（具有隐藏依赖性的序列机械谜题）在物理机器人环境中评估RGB、RGB-D和真实符号观测，并使用受控模拟探测行为。

Result: 反直觉发现：agent在原始RGB输入下表现最佳，在完美真值符号观测下表现最差。模拟中随机翻转感知动作结果，适度噪声改善性能，40%翻转概率时成功率比无噪声基线提升2.85倍。该增益与减少重复动作循环有关。

Conclusion: 单纯依靠成功率评估LLM是不足的，因为所测量的性能可能反映的是感知误差与推理失败之间的相互作用，而非稳健的问题解决能力。
...
### [150] [Neurosymbolic Learning for Inference-Time Argumentation](https://arxiv.org/abs/2605.20098)
*Gabriel Freedman,Adam Dejl,Adam Gould,Mansi,Lihu Chen,Jianqi Jiang,Francesca Toni*

Main category: cs.AI

TL;DR: 本文提出推理时论证框架(ITA)，用于三元(真/假/不确定)声明验证，将形式化论证语义与LLM训练和预测相结合。


<details>
  <summary>Details</summary>
Motivation: 高风险领域(如健康和金融)的声明验证很重要，当支撑信息不完整或冲突时，不确定性答案比二元分类更合适，且需要对判断依据提供忠实解释。

Method: 提出推理时论证Argumentation(ITA)框架：(1)在训练时使用形式化论证语义指导LLM学习生成论证并分配基础分数；(2)在推理时从生成的已评分论证计算三元预测。核心创新是将论证质量优化与最终预测计算统一在同一个形式化框架内。

Result: 在两个三元声明验证数据集上，ITA相比论证基线有提升，可与非论证直接预测基线竞争，同时产生由明确可检查论证结构决定的确定性判决。

Conclusion: ITA提供了可追溯、忠实于论证结构的判决，而非后验推理追踪。相比传统推理模型具有更好的可解释性和确定性。
...
### [151] [Using Aristotle API for AI-Assisted Theorem Proving in Lean 4: A Formalisation Case Study of the Grasshopper Problem](https://arxiv.org/abs/2605.20120)
*Gabriel Rongyang Lau*

Main category: cs.AI

TL;DR: 本文报告了一个Lean 4形式化案例研究，将AI生成的Aristotle API证明尝试应用于2009年IMO第6题（蚱蜢问题）。形式化开发包含4个已验证的辅助引理和一个未解决的主要定理，展示了AI辅助形式化中局部证明搜索成功但全局组合计数账务 unresolved 的中心性限制。


<details>
  <summary>Details</summary>
Motivation: 探索AI辅助定理证明生成的可验证Lean开发的证据状态，特别是确定哪些声明被实际验证，哪些仍待解决。

Method: 使用Lean 4对Aristotle API的蚱蜢问题证明尝试进行形式化_CASE研究_buildArtifact，构建推广的Lean版本定理，包含四个局部组件的辅助引理。

Result: 建立了4个已验证的辅助引理：(1)最终部分和等于总和 (2)相邻转置仅影响相关中间部分和 (3)变化的部分和具有预期形式 (4)最大性位置允许相邻后继交换则强制对应禁止集成员资格事实。主定理仍有一个未解决的sorry。

Conclusion: 该案例展示了AI辅助形式化的核心限制：局部证明搜索可以成功，但完成定理所需的全局组合计数账务（即证明禁止集成员资格产生至少n个不同禁止值以矛盾|M|<n的基数假设）仍然未解决。提供了可复现的Lean工件和对已验证/未验证证明内容的精确分析。
...
### [152] [Not Every Rubric Teaches Equally: Policy-Aware Rubric Rewards for RLVR](https://arxiv.org/abs/2605.20164)
*Utkarsh Tyagi,Xingang Guo,MohammadHossein Rezaei,Daniel George,Anas Mahmoud,Jackson Lee,Bing Liu,Yunzhong He*

Main category: cs.AI

TL;DR: POW3R是一种新型的策略感知rubric奖励框架，通过在训练过程中动态调整criterion级别奖励权重来解决静态rubric聚合的问题。实验表明它在30个比较中赢得24个，并在多模态和纯文本设置中以更少的训练步骤实现了更好的rubric奖励和完成率。


<details>
  <summary>Details</summary>
Motivation: 标准静态聚合将criterion的人类重要性与其作为优化信号的当前有用性混为一谈。在rubric RL中，许多重要criteria已经饱和或当前无法达到，而区分rollouts的criteria不一定是人类权重最大的。标准方法假设所有criteria在训练过程中同等重要，这导致优化效率低下。

Method: POW3R是一种策略感知的rubric奖励框架，它在训练过程中动态调整criterion级别的奖励权重，同时保留人类权重和类别平衡作为rubric目标。该方法使用rollout级别的对比来强调当前区分策略输出的criteria，使GRPO奖励信息更丰富，而不改变底层评估目标。

Result: 在跨多模态和纯文本设置的 两个数据集上的三个基础策略中，PO3R赢得了30个基础策略/指标比较中的24个。它提高了平均rubric奖励和严格完成率（满足每个所需rubric标准的响应比例），并在减少2.5-4倍训练步骤的情况下达到相同的性能水平。

Conclusion: Rubric rewards should distinguish what should matter in the final answer from what can teach the current policy. POW3R通过在训练过程中动态调整criterion级别的奖励权重，同时保持人类分配的类别权重和平衡，解决了静态聚合的问题。
