...
TL;DR: 该论文提出了COD10K-C，一个针对伪装目标检测的图像腐蚀鲁棒性基准数据集，包含8种腐蚀类型和5个严重等级，并评估了多种模型的抗腐蚀能力。提出的RobustCODLite在腐蚀条件下能够保持92.3%的清洁Dice分数，显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有的伪装目标检测基准仅使用干净图像进行评估，与实际应用场景不符。真实环境中相机常会遇到运动模糊、高斯模糊、亮度变化、雾效、噪声和压缩伪影等腐蚀问题，这导致模型在实际部署时性能显著下降。

Method: 构建了COD10K-C基准数据集，包含8种腐蚀类型（运动模糊、高斯模糊、亮度、雾效、噪声、压缩等）和5个严重等级，共40种条件组合和81,040个评估样本。评估了SINet-v2、PFNet、ZoomNet和RobultCODLite四种模型，其中RobustCODLite采用了三种关键技术：腐蚀增强训练、频率先验分支和不确定性一致性损失。

Result: 所有模型在腐蚀图像上都出现明显性能下降。运动模糊和高斯模糊造成的性能下降最为严重，SINet-v2在运动模糊下损失18.5个Dice分数。RobustCODLite在腐蚀条件下保持92.3%的清洁Dice分数，优于SINet-v2的87.7%、ZoomNet的84.8%和PFNet的84.1%。在最严重的腐蚀条件下，RobustCODLite可匹敌或超越在干净数据上表现更好的模型。

Conclusion: 该研究填补了伪装目标检测鲁棒性评估的空白，证明通过引入专门的防御机制可以有效提升模型对抗图像腐蚀的鲁棒性。RobustCODLite在复杂真实场景中具有更强的实用性价值。
...
TL;DR: AVTrack是一个新提出的人类中心视听实例分割(AVIS)数据集，用于动态真实场景中的说话人跟踪任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集局限于简单或同质化的视听场景和粗粒度注释，评估偏向于静态视听共现，无法有效评估复杂动态场景中的时空建模和跨模态推理能力。

Method: 构建了AVTrack数据集，包含多样化和挑战性条件如相机运动、视觉遮挡和位置变化，并提供了简单的基线方法。

Result: 在AVTrack上评估代表性AVIS方法显示现有方法存在显著的性能下降，AVTrack成为具有挑战性的基准。

Conclusion: AVTrack为复杂环境下鲁棒的人类中心视听场景理解提供了具有挑战性的 benchmark 和新的研究方向。
...
TL;DR: ViewDiag评估协议发现前沿VLMs在多视角度量距离查询中产生稳定但错误的预测，一致性并不等于几何理解，而是先验驱动的崩溃。


<details>
  <summary>Details</summary>
Motivation: 空间推理对机器人和具身AI至关重要，但现代VLM在度量距离查询上不可靠。研究假设跨视角一致性反映几何基础，这一假设此前未经检验。

Method: 构建ViewDiag多视角评估协议，使用Hypersim、ScanNet和KITTI360数据集，包含176个对象轨跨80个场景，每轨2-10视角，从三轴评估：度量准确性、分布集中度、潜在特征探测（区分决策崩溃与表示崩溃）。

Result: 跨多样性模型观察到高预测稳定性与重大错误共存的模式，处于强一致性但低准确性的状态。

Conclusion: 挑战了跨视角一致性作为几何理解代理的常见做法，表明稳定预测可能反映先验驱动崩溃而非证据敏感推理。ViewDiag提供受控基准和诊断框架。
...
TL;DR: 本文提出了Plan2Map，一个包含208个案例的多模态基准数据集，用于从英国规划文件中重建地理空间边界，并设计了GeoPlanAgent系统通过证据提取、定位、地图配准、边界分割、投影和验证六个步骤完成该任务。


<details>
  <summary>Details</summary>
Motivation: 规划记录定义了地理区域限制，但其源文档通常只提供间接的空间证据而非机器可读的边界，因此需要一种方法从规划文档中自动重建有效的地理空间边界。

Method: GeoPlanAgent是一个文档接地、多模态地理空间工具集成的系统，将任务分解为六个子任务：证据提取、定位、地图配准、边界分割、投影和验证。

Result: 在Plan2Map上，GeoPlanAgent达到了0.736的平均IoU和0.904的中位数IoU，67.8%的预测达到或超过0.8 IoU，显著优于直接的VLM-to-GeoJSON基线方法。

Conclusion: 直接VLM预测仍不可靠，剩余错误主要集中在定位和地图配准阶段；监督边界分割显著改善了像素级掩膜质量；Plan2Map为公共规划文件的多模态地理空间重建提供了具体的测试平台。
...
TL;DR: MetaWorld是一个新型框架，用于将单智能体视频世界模型扩展到多智能体开放领域环境，直接从单视角视频进行扩展。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型方法仅支持单一智能体的单一视角，多智能体设置面临两个关键挑战：数据稀缺（协调的多视角录制成本高昂）和世界状态对齐（独立生成的视频流无法确保共享物理环境和事件在不同视角间一致演化）。

Method: 提出三个核心技术：1) 单目世界状态展开(MWSU)将单目 footage分解为相机操作者的自我运动和可见对象的时空轨迹；2) 开发了主体感知世界生成器，基于每个智能体的身份图像进行外观驱动模拟；3) 提出世界状态对齐(WSA)，一种每帧跨分支交叉注意力机制，插入到视频DiT的每个transformer层中。

Result: 通过相机-轨迹分解自然提取共享3D空间内的多智能体同步运动数据；WSA联合同步去噪过程，强制实现静态几何一致性和动态运动一致性，确保共享3D环境和物理事件在两个自我视角间保持对齐。

Conclusion: MetaWorld实现了卓越的跨视角一致性和身份保真度，为多智能体视频世界建模建立了高度可扩展的物理驱动范式。
...
TL;DR: 本研究评估了机器学习和深度学习方法在卫星测深（SDB）中的可转移性，使用Sentinel-2图像在0-20米深度范围内进行测试。研究发现保持空间连续性的训练策略和SWF加权RMSE损失是关键创新，深度模型在跨区域转移时比Random Forest更加鲁棒。


<details>
  <summary>Details</summary>
Motivation: 当前卫星测深方法虽然在成本上有优势，但在不同区域尤其是光学复杂的沿海环境中难以扩展应用。研究者希望找到更具可转移性的深度学习方法来解决这一问题。

Method: 使用Random Forest作为基线，并测试四种CNN架构（ResNet-50、ResNet-101、EfficientNet-B4、ConvNeXt-Large）。关键设计选择包括：1)保持空间连续性的训练策略；2)引入SWF加权RMSE损失强调近水面深度；3)利用多时相重复图像进行训练和推理。

Result: Intra-regional RMSE范围为1.15-1.92m（0-20m），≤3m深度可达0.26m。Random Forest在跨区域转移时RMSE从1.53m急剧下降至2.99-3.78m，而深度模型保持更稳健的性能（2.46-2.98m）。在MagicBathyNet基准上达到0.19-0.22m RMSE，优于U-Net基线和任务特定Transformer架构。

Conclusion: 保留空间连续性是单一最具影响力的设计选择。深度模型在跨区域转移方面显著优于传统机器学习方法，且参数量更少。多时相图像处理可进一步降低预测噪声。研究团队发布了优化架构和预训练权重以支持向新站点的可扩展转移。
...
TL;DR: 提出了GeoDrive-Bench基准测试，用于评估视觉语言模型(VLM)在不同地区特定交通规则下的驾驶推理能力


<details>
  <summary>Details</summary>
Motivation: 当前VLM在处理区域特定交通规则方面的能力尚未得到充分探索，在不同全球环境中部署存在不确定性

Method: 构建了包含5,053个人类验证的多选题QA对，涵盖六个国家的多样化驾驶文化；设计了一种蒸馏算法将区域特定交通规则知识注入VLM内部表示

Result: 在九个最先进的VLM上的实验显示，每个任务在不同地理驾驶文化之间存在显著的性能差异；提出的基线模型表现出改进的跨区域地理文化推理能力

Conclusion: 当前VLM仍然缺乏稳健的区域感知驾驶智能；GeoDrive-Bench作为可部署自动驾驶基础模型的诊断和训练Oriented测试平台
...
TL;DR: 本文提出了一种诊断驱动的框架，用于将预训练的HOI模型适应到真实教育环境。针对Critical Care Air Transport Team混合现实医学训练数据集，通过三要素级HOI错误分类法和错误因素归因分析，识别失败模式并指导模型针对性改进，最终将CDN模型的宏F1分数从48.6提升至90.2。


<details>
  <summary>Details</summary>
Motivation: 现有HOI检测器在基准数据集上表现良好，但在真实教育环境中性能下降，原因包括领域特定目标、遮挡和复杂视觉条件。这种域迁移问题限制了模型的实际应用。

Method: 提出诊断驱动框架，整合三要素级HOI错误分类法与错误因素归因分析，对真实教育视频数据进行故障诊断。基于诊断结果开发诊断引导的 refinement策略，针对性调整预训练HOI模型以适应目标领域。

Result: 在CCATT数据集上，该方法将预训练CDN模型的宏F1分数从48.6提升至90.2，显著超越了 baseline性能。

Conclusion: 详细诊断分析对于为真实教育环境中的HOI模型提供针对性适应指导具有重要价值，这一方法可推广至其他领域的域迁移任务。
...
TL;DR: Cosmos 3是一个全能世界模型家族，通过统一的Mixture-of-Transformers架构同时处理语言、图像、视频、音频和动作，在多个benchmark上达到SOTA，是目前最佳的开源文本到图像和图像到视频模型。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够统一处理和生成多种模态（语言、图像、视频、音频和动作）的全能世界模型，以支持Physical AI的发展，解决现有模型架构分散、无法有效整合的问题。

Method: 采用混合-transformer架构（mixture-of-transformers architecture），支持灵活的输入输出配置，统一处理多种模态。模型经过后训练（post-training）并在多个基准测试上评估。

Result: 在理解 and generation任务上建立新的SOTA，在Artificial Analysis的Text-to-Image和Image-to-Video排名中位列最佳开源模型，在RoboArena中成为最佳policy模型。

Conclusion: 证明了全能世界模型可以作为可扩展的通用基础模型用于具身智能体（embodied agents）。研究团队开源了代码、模型检查点、数据集和评估基准。
...
TL;DR: 本文提出了一种自动化代理管道，从私人癌症影像报告生成多个选择VQA数据集，用于评估视觉语言模型在医学影像上的表现。该方法无需人工标注即可生成污染控制的基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有医学VLM基准测试存在规模有限、人工标注成本高、且可能泄漏到VLM预训练数据中导致评估结果不准确的问题。需要一种临床相关、可扩展且能控制评估干扰因素的基准生成方法。

Method: 1) 建立自动化代理驱动管道，从成对的私人放射学报告和3D肿瘤影像直接生成多选题VQA数据集；2) 生成两种互补问题类型：RADS风格问题（从临床定义的报告模式推导）和报告派生问题（由LLM从放射学家发现生成并验证）；3) 应用到四个内部癌症队列；4) 对六个VLM进行零样本评估并进行盲 ablation 实验。

Result: 1) 生成的基准测试在实例级别控制污染且无需每题人工标注；2) 零样本评估显示没有主导模型，各指标均有很大提升空间；3) 盲 ablation 发现视觉依赖高度数据集特异性：肝脏报告派生问题真正需要图像，而肺CT即使不用图像也可解决——顶级闭源模型在肺CT上失明后准确性反而提升，表明即使是私人临床数据也无法保证对视觉能力的无污染评估。

Conclusion: 该自动化管道能够大规模生成无污染的医学VLM评估基准，但揭示出当前模型在特定任务上存在严重的数据泄露问题。管道已作为开源代理技能发布供内部重新部署。
...
TL;DR: 本文针对单图像反射分离问题，指出传统线性合成模型无法捕捉真实图像信号处理 Pipeline 中的非线性耦合，提出可学习的非线性叠加模型和双向特征交互的通用双流框架，在多个真实基准数据集上实现优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖简化假设或独立建模，难以处理真实场景。单图像反射分离的根本挑战在于透射层与反射层在复杂图像形成过程中的纠缠。现有广泛采用的 sRGB 域线性合成模型未能捕捉实际图像信号处理流程引入的非线性耦合。

Method: 提出一种可学习的非线性叠加模型，更准确地描述层间交互；基于此构建双向特征交换的通用双流交互框架，显式建模透射层与反射层之间的双向依赖。该框架统一了基于激活、门控和注意力的交互机制，同时兼容 CNN 和 Transformer 主干网络。

Result: 在多种真实世界基准数据集上的广泛实验表明，该方法具有优越的性能和强大的泛化能力。

Conclusion: 研究表明反射分离的本质不是撤销线性混合，而是学习非线性形成与交互，为设计原则性图像分解模型提供新思路。代码和模型已公开可用。
...
TL;DR: MoPE（Mixture of Pathway Experts）是一种知识蒸馏框架，通过将多模态学习重新表述为特权蒸馏，实现仅使用组织病理学图像进行癌症风险预测。该方法利用RNA衍生的通路知识，在训练时监督病理模型，从而在推理时无需RNA-seq数据。


<details>
  <summary>Details</summary>
Motivation: 整合转录组学和 组织病理学可以改善癌症风险建模，但常规临床实践中RNA分析难以获取。MoPE旨在解决这一矛盾：如何在训练时利用分子信息，而在推理时仅依赖病理切片。

Method: MoPE编码RNA衍生通路，将分子监督信息通过记忆使用对齐方式迁移到通路索引的病理专家模型。该框架将多模态学习重新表述为特权蒸馏问题，利用RNA-derived pathways作为特权信息训练 histology-only 模型。

Result: 在多个公共基准数据集和两个独立乳腺癌队列中，MoPE始终改善了WSI（即Whole-Slide Image，仅组织切片图像）推理性能，相较于基线方法获得提升。通路使用分析和人工审查的视觉检查提供了模型行为的边界 inspection。

Conclusion: 通路结构化的特权蒸馏是一种有前景的方法，可以在训练时利用分子信息，同时保持推理时无需RNA分析。这为克服常规临床实践中分子数据不可用的限制提供了新思路。
...
TL;DR: 在超低功耗边缘设备(<1MB SRAM)上实现鲁棒的目标检测，通过结合MCUNet、YOLOv2和TensorFlow Lite量化，并采用决策级融合(WBF)协同多设备视角提升遮挡场景下的目标检测精度。


<details>
  <summary>Details</summary>
Motivation: 边缘设备运行计算机视觉面临内存和算力限制、多设备协作通信开销、以及遮挡导致的精度损失问题，需要一种适合超低硬件规格的遮挡鲁棒目标检测方案。

Method: 1) 采用MCUNet作为骨干网络，YOLOv2作为检测头，TensorFlow Lite量化部署；2) 评估两种协作推理策略：特征级融合(拼接中间特征图)和决策级融合(WBF)；3) 在Coral Dev Board Micro上进行Wi-Fi点对点部署实验。

Result: WBF在非对称遮挡场景下优于特征级融合，最高获得+0.2736 mAP增益；三视角融合可进一步提升至+0.3827 mAP，通信开销约1.3KB/次；在301.9s自主会话中，融合输出覆盖61帧vs单板47帧，覆盖率提升29.8%。

Conclusion: 决策级融合是提升超低功耗边缘目标检测遮挡鲁棒性的可行方案，支持无主机的多板超低功耗硬件分布式协作运行。
...
TL;DR: 本文提出Any2Poster Bench基准和Any2Poster Agent代理，用于评估和实现任意来源的海报生成任务，支持8种输入模态和5种内容领域。


<details>
  <summary>Details</summary>
Motivation: 现有海报生成评估受限于仅限论文输入、狭窄领域或表面视觉相似性，缺乏全面可量化的评估标准。

Method: 构建了包含8种输入模态（PDF、URL、PPTX、DOCX、Markdown、LaTeX、notebooks、video）和5种内容领域的基准；设计基于quiz的事实保留和解释性理解探测，以及基于VLM的视觉质量、布局、可读性、内容完整性和逻辑流判断；提出Any2Parser解析异构源、组织内容、规划海报布局、渲染并迭代优化。

Result: Any2Poster Agent在基准上跨输入模态达87.25%平均准确率，跨内容领域达87.28%；相比Prior PosterAgent-4o从51.06-51.33%提升至72.58%总准确率，密度增强分数从116-121提升至145.16。

Conclusion: Any2Poster Bench提供可复用的评估资源，Any2Poster Agent成为多模态、领域通用的海报生成竞争基线。
...
TL;DR: 提出了一种基于扩散模型的动态人像视频重光照方法，能够在保持时间一致性和真实感的同时，对动态人像进行逼真的重光照。模型使用真实捕获和渲染的混合数据集进行训练，通过LED照明系统采集数据，并利用预训练视频扩散模型的图像先验，结合高动态范围环境图作为光照控制，实现身份保真的动态人像视频重光照。


<details>
  <summary>Details</summary>
Motivation: 现有的图像或视频重光照方法难以同时满足 photorealism（照片级真实感）、lighting harmony（光照和谐）和 temporal consistency（时间一致性）三个关键要求，特别是在处理动态人像视频时，如何在改变光照条件的同时保持人物身份特征、表情和面部细节是一个具有挑战性的问题。

Method: 构建了一套LED照明系统用于真实光照模拟和高速视频重光照数据采集；使用预训练视频扩散模型作为图像先验；以每帧HDR环境图作为光照控制信号；额外引入合成背景图像实现对相机曝光水平和色调的控制；在包含多样主体外观、面部动作、头部姿态和已知光照条件的真实捕获和渲染混合数据集上训练生成模型。

Result: 模型能够产生时间一致的重新光照人像视频，外观逼真且与新环境和谐融入；能够忠实地保留主体的表情和精细面部特征，包括肤色、皱纹和胡须；对未见过的数据（在主体外观、动作和光照条件方面）具有良好的泛化能力；在各种环境图下对自然视频进行重光照的广泛实验表明，该方法在人像摄影的实际应用中表现出色。

Conclusion: 该方法在照片级真实感、光照和谐和时间一致性方面达到了最先进的性能水平。
...
TL;DR: ATLAS是首个针对LiDAR感知模型的黑盒传感器攻击大规模评估基准，模拟点注入和点删除两种攻击模式，揭示了先进模型的鲁棒性不对称现象。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR基准缺乏对传感器攻击导致黑盒感知失败的评估，且先前研究聚焦于攻击硬件和早期检测器，未探索现代感知系统的鲁棒性。

Method: 构建ATLAS基准，模拟两种攻击模式（点注入和点删除），在真实驾驶序列上评估多款最先进的LiDAR感知模型，并将漏洞归因于标准数据增强方法。

Result: 强基准性能的模型更抗删除攻击但更易受注入攻击；漏洞源于训练中的对象数据库采样增强，导致架构无关的鲁棒性失败。

Conclusion: 发布ATLAS代码以支持可扩展评估，推动将黑盒传感器鲁棒性纳入未来LiDAR感知开发考量。
...
TL;DR: 本文提出了SALU（饱和自适应线性单元），一种可学习的激活函数，可以完全替代归一化层（如BatchNorm、LayerNorm）。通过用SALU替换归一化层，SWALU和GALU替换标准激活函数，构建了SaluNet架构，在CIFAR-10、CIFAR-100和ImageNet数据集上取得了优异的性能，且在batch size为1时仍能保持较好效果。


<details>
  <summary>Details</summary>
Motivation: 本文的动机是发现标准归一化层存在"可塑性抑制效应"：当可学习激活参数与归一化层配合使用时，会快速失去适应性。作者认为这抑制了网络的"全局可塑性"（total plasticity），而这是生物神经元固有的属性。

Method: 提出SALU函数：SALU(x;a,b) = a*x / √(1 + a*b*x²)，其中a>0, b>0为可学习参数。该函数是有界的、可学习的激活函数，能够提供内在的信号稳定化，不依赖批统计或外部仿射参数。在此基础上提出SaluNet：用SALU替代归一化层，SWALU和GALU替代标准激活函数，构建完全基于全局可塑性的网络架构。

Result: ResNet-18变体SaluNet-C-18在CIFAR-10上达到97.35%、CIFAR-100上达到83.25%（无需归一化）；在batch size为1时仍保持93.44%和76.23%的准确率。对于Transformer，SaluNet-T相比LayerNorm-GELU从90.92%提升到91.01%（CIFAR-10）、从66.54%提升到68.10%（CIFAR-100）。SaluNet-C-50在ImageNet-1K上达到78.67% Top-1（224×224），79.23%（288×288）。

Conclusion: 研究结论表明归一化层会抑制总可塑性（total plasticity）——这是生物神经元固有的特性。移除归一化层后，网络能够更有效地学习，并且在小batch甚至batch size为1的情况下也能稳定训练，这证明了全局可塑性对于深度网络学习的重要性。
...
TL;DR: 提出了一种两阶段几何配准方法，用于将CAD模型与CT扫描对齐，实现圆柱体目标（如电离室）的免标定高精度注册。


<details>
  <summary>Details</summary>
Motivation: 工业CT中建立精确几何真实值的需求日益增长，传统强度法和点云法在无标定参考或缺乏特征对应的情况下均失效。

Method: 第一阶段：通过椭圆检测和PCA估计3D旋转轴；第二阶段：体素化CAD模型后沿检测轴方向进行平移优化，最大化与CT的体积重叠。

Result: 实现倾角和方向误差小于0.1°的鲁棒配准，无需强度标定或特征匹配。

Conclusion: 该方法为工业CT中的机器学习目标定位和自动化分析提供了可靠的几何真实值，适用于大规模数据集构建。
...
TL;DR: KITScenes Multimodal是一个欧洲自动驾驶数据集，采用高分辨率传感器（摄像头、激光雷达、4D成像雷达）和高精度GNSS/INS定位，配合ToF深度估计和HD地图构建。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集在传感器精度、地图完整性和地理多样性方面存在不足，需要构建一个更高质量的数据集来推动空间学习研究。

Method: 数据集采用同步传感器套件，包括高分辨率全局快门摄像头、远距离激光雷达(>400m)、4D成像雷达和高精度GNSS/INS定位。HD地图经过实车 autonomous driving验证，涵盖交通元素并进行3D重建。

Result: 该数据集在四个任务上实现了benchmark:在线HD地图构建、远距离深度估计、新视图合成和端到端驾驶。数据录制于不规则街道布局和混合交通模式的城市环境中。

Conclusion: KITScenes Multimodal数据集通过高保真传感器、全面HD地图和独特地理多样性,为自动驾驶领域提供了新的研究基础,有潜力推进embodied AI的空间学习能力。
...
TL;DR: 本文提出一种基于手部轨迹编码器的自我中心自然语言查询( NLQ )接地方法，通过将手部骨骼序列转换为运动学特征，并与视频-文本特征进行跨注意力融合 gated ，在 Ego4D NLQ 数据集上取得显著提升，尤其在手-物体交互和数量/状态查询任务上。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅融合视频外观与查询文本，忽略了手部运动信息。实际上 Ego4D NLQ 中约 41% 的查询涉及手-物体操作或其即时结果，手部轨迹能提供超越纯外观的定位线索。

Method: 提出手部轨迹编码器，将连续手部骨骼序列编码为高语义手部运动学特征；通过跨注意力融合策略并结合自适应门控机制，将手部特征与预训练视频-文本特征对齐并组合。

Result: 在 Ego4D NLQ v2 验证集上，手-物体交互查询的 R1@IoU=0.3 提升 +2.54，数量/状态查询提升 +4.32，表明手部轨迹提供了超越外观的定位线索。

Conclusion: 引入手部轨迹信息可有效补充视觉外观特征的不足，为自我中心视频中的自然语言查询定位提供更全面的上下文理解。
...
TL;DR: 提出了一种紧凑的深度多任务学习模型，可以在单次前向传播中同时处理多种自动驾驶感知任务，包括语义分割、深度估计、激光雷达分割和鸟瞰视图投影。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要同时处理多个感知任务，现有方法通常依赖多个独立模型，导致计算资源消耗大且效率低下。同时面临多任务学习中任务不平衡学习的挑战。

Method: 提出自适应损失加权算法解决任务不平衡问题；采用数据预处理和中间传感器融合技术处理来自RGB相机、动态视觉传感器(DVS)和激光雷达的多种输入模态。

Result: 模型变体在 ablation study 中表现更好，与近期模型组合相比保持更优性能，且参数更少推理更快、GPU内存占用更低。在3个CARLA仿真数据集和1个真实nuScenes-lidarseg数据集上结果一致。

Conclusion: 该紧凑多任务学习模型能够高效处理自动驾驶多种感知任务，在保持较好性能的同时显著降低了计算资源需求，适用于资源受限的自动驾驶场景。
...
TL;DR: 本文提出MUSE框架，通过改进冻结的多模态大语言模型(MLLM)的外围执行脚手架而非重新训练模型，来提升模型在视觉空间规划、视觉感知、多模态推理和细粒度视觉辨识等任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在人类轻松解决的任务（如网格迷宫导航、拼图块选择）上仍存在失败，作者探讨能否通过改进执行脚手架来激发冻结模型的潜在能力，而非对模型进行重训练。

Method: 提出MUSE（多模态统一结构化执行 harness），包含可组合模块：任务表示、视觉处理、感知工具使用、结构化解析、确定性验证器和验证器引导的修复。无需任何模型重训练即可包装任意的现成MLLM。

Result: MUSE在多种 benchmarks 上相比裸模型实现一致的性能提升，在挑战性最大的实例上提升幅度最显著。分析表明许多MLLM失败源于脚手架层面的缺陷而非模型本身的基础不足，可以通过验证器引导的修复来解决。

Conclusion: 研究揭示了代理性多模态脚手架是一个关键但未被充分探索的设计维度，为超越以模型为中心的优化提供了正交路径。未来应重视脚手架设计作为提升MLLM能力的有效途径。
...
TL;DR: FCUS-rPPG是一种快速收敛的无监督远程光电容积脉搏波(rPPG)框架，通过多光谱特征解耦、梯度掩码验证、损失景观平滑和噪声感知空域正则化等技术实现在仅1个训练轮次内完成训练，并在跨数据集评估中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督rPPG方法面临梯度噪声和不稳定问题，导致收敛缓慢且跨域泛化能力有限，需要大量训练轮次（数十到数百）才能达到较好效果。

Method: 提出三大核心技术创新：1）光谱共享骨干网络促进BVP特征解耦；2）后验证掩码机制基于弱振幅生理先验过滤误导梯度；3）基于扰动的损失景观平滑策略引导优化至更泛化的平坦极小值；4）噪声感知空域正则化将特征更新约束至噪声子空间的正交补空间。

Result: 在5个数据集上实验表明：仅需1个训练轮次即可完成训练（现有方法通常需要数十到数百轮）；在跨数据集评估中始终保持最先进性能。

Conclusion: FCUS-rPPG为无监督rPPG的实际部署提供了一种高效且鲁棒的解决方案。
...
TL;DR: 针对医学图像分割中跨域泛化难题，提出对WT-PSE框架的四项训练级改进：域适应数据增强、混合BCE+Dice损失、课程学习Dice权重调度及 ablation控制开关。在视盘分割数据集上最终Dice达0.956，ASD为13.31。


<details>
  <summary>Details</summary>
Motivation: 现有WT-PSE存在四大局限：训练增强不足以模拟真实扫描器变化、二元交叉熵损失对边缘噪声敏感、缺乏scheduled loss权重策略导致训练不稳定、缺少ablation开关影响科学对比。通过改进训练流程而非修改核心架构实现跨域分割性能提升。

Method: 提出四项增强：(1)域适应增强含随机擦除、gamma校正、盐椒噪声模拟设备差异；(2)混合BCE和Dice损失函数实现边缘感知分割抗噪；(3)课程学习Dice权重调度策略改善早期训练稳定性；(4)命令行ablation控制标志支持系统化消融研究。

Result: 在眼底视盘分割基准数据集上，改进pipeline最终epoch Dice得分0.956，ASD得分13.31，相较于基线epoch-5 Dice得分0.939有显著提升。表明无需修改WT-PSRE底层架构即可获得持续性能增益。

Conclusion: 通过训练流程优化而非架构修改可有效提升跨域医学图像分割性能，域适应增强、混合损失函数及课程学习调度是关键改进措施。该方法为其他医学影像任务的鲁棒性提升提供了可迁移的改进范式。
...
TL;DR: 本文提出TGV-KV方法解决视觉语言模型(VLM)中KV缓存内存开销过大的问题。通过利用文本指导来评估视觉信息的重要性，结合三个子模块(TVB、TWR、TPR)进行高效的KV缓存淘汰。在LLaVA-NeXT上实现99.2%精度保持的同时，将吞吐量提升52.6%。


<details>
  <summary>Details</summary>
Motivation: VLM采用自回归生成范式，会缓存所有先前token的KV以加速推理，导致内存消耗随上下文长度线性增长。视觉模态存在大量冗余，现有的KV缓存淘汰方法主要针对语言模型设计，忽视了文本与视觉之间的固有差距，在VLM上往往导致显著的性能下降。

Method: 提出文本导向的KV缓存淘汰方法TGV-KV，包含三个子模块：(1)文本-视觉预算分配(TVB)基于互信息交互为每层分配预算；(2)文本加权排序(TWR)基于加权文本-图像注意力评估文本优先级并排序视觉重要性；(3)文本优先保留(TPR)策略性地保留文本KV以避免关键信息损失。

Result: 在5个不同规模和架构的模型上评估，TGV-KV在VizWiz-VQA任务上使用LLaVA-NeXT可保留99.2%的完整KV精度，在5%的极端保留预算下将端到端吞吐量提升52.6%。

Conclusion: TGV-KV通过充分利用文本与视觉之间的模态差距，结合文本指导来评估视觉信息的重要性，有效解决了VLM中KV缓存内存开销与性能之间的权衡问题，为多模态大模型的高效推理提供了新的思路。
...
TL;DR: 针对结构健康监测中的双重异构性问题，提出分层联邦学习框架，结合宏观动态聚类和微观区域自适应正则化，实现跨区域基础设施网络的稳健诊断。


<details>
  <summary>Details</summary>
Motivation: 数据孤岛困境限制了数据驱动SHM模型的部署；联邦学习虽提供隐私保护方案，但面临双重异构性挑战——宏观层面结构类型物理差异，微观层面本地数据集统计不平衡。

Method: 提出分层联邦学习框架，采用两层级优化：1)宏观层面动态梯度聚类，基于结构退化轨迹自动聚合客户端为专家组；2)微观层面DRAPR模块，计算非IID强度分数，自适应调节近端惩罚以缓解客户漂移和灾难性遗忘。

Result: 宏观聚类和微观正则化的分层集成成功中和双层异构性，产生高度健壮和专业的复杂基础设施诊断模型。

Conclusion: 该框架有效解决跨区域基础设施网络中双重异构性问题，为全国性结构健康监测提供可行的隐私保护协作学习方案。
...
TL;DR: 提出了KeyVT，一种利用2D视觉语言模型进行零样本3D场景理解的分层方法，通过视图级和token级的输入上下文收集来选择任务相关的关键视图和代表性token。


<details>
  <summary>Details</summary>
Motivation: 零样本3D场景理解由于2D VLMs的空间推理能力受到关注，但有限的输入预算如何保留尽可能多的任务相关3D细节是一个挑战。现有方法忽略了视图重要性的评估和token冗余问题。

Method: 提出分层方法KeyVT：1) 结合像素特征和相机参数，基于语义内容和几何位置评估视图重要性，选择空间一致且任务相关的视图；2) 在最优传输框架下，将视图token和关键token形式化为两个离散分布，通过最小化OT距离识别代表性token以消除patch间的冗余。

Result: 在三个广泛使用的基准数据集上评估，相较于现有的免调优方法取得显著改进，性能与训练基方法相当。

Conclusion: 通过分层视图和token选择有效解决了有限输入预算下的上下文质量问题，为基于2D VLMs的零样本3D场景理解提供了有效框架。
...
TL;DR: 本文研究DDIM图像生成过程的逆问题，旨在从生成的图像中恢复初始噪声图等潜在变量。提出一种混合方法：先用梯度下降直接反演第一步，再用不动点方法迭代后续步骤。在三个数据集上评估表明，该方法显著提升了初始潜在变量的预测精度和重建准确性，并引入自插值测试来评估插值 latent 点生成的图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从DDIM生成图像中恢复初始潜在变量（尤其是初始噪声图）时存在精度不足的问题，需要一种更准确的反演方法。

Method: 提出一种混合反演方法：(1) 第一步使用梯度下降直接反演；(2) 后续步骤使用不动点迭代法。此外引入了自插值测试作为新的评估指标。

Result: 实验在三个数据集上表明，该方法显著提升了初始潜在变量的预测精度和重建质量，在所有指标上均优于现有方法。

Conclusion: 所提混合方法能更准确地预测DDIM的初始潜在变量，显著提高重建和自插值性能，为扩散模型的应用提供了有价值的见解。
...
TL;DR: FAF-CD是一种面向远感异构观测的变化检测框架,采用DINOv3预训练ConvNeXt编码器与线性复杂度VMamba解码器,通过傅里叶和小波比较、可变形空间对齐及自适应门控聚合多尺度特征,实现跨传感器、跨模态图像的鲁棒变化检测。


<details>
  <summary>Details</summary>
Motivation: 现实监测中异构观测(跨传感器、跨模态、光照季节变化)普遍存在,现有方法难以有效区分结构损伤与干扰变化,亟需设计能抑制无关干扰变化、聚焦真实变化的鲁棒变化检测方法。

Method: 提出频率感知混合框架FAF-CD:1)DINOv3预训练ConvNeXt编码器提取视觉特征;2)VMamba解码器保持线性复杂度;3)矫正感知三支路融合模块结合可变形空间对齐、傅里叶/小波比较和自适应门控,跨尺度聚合互补线索。

Result: BRIGHT数据集上tc-mIoU/tc-mAP优于NeXt2Former-CD;LEVIR-CD达0.924 cF1,WHU-CD达0.955 cF1;伪变化压力测试中平均 perturbed cIoU/cF1最佳;计算成本降低约24 GFLOPs同时保持或提升精度。

Conclusion: FAF-CD有效解决异构观测变化检测难题,频率域分析能区分结构损伤与干扰变化,具有良好泛化性和实用性,为灾害制图等实际应用提供技术支撑。
...
TL;DR: 本文提出Prior Guidance (PG)方法，通过对比弱先验与可见先验来增强桥接模型中的先验利用，并进一步设计频率调制先验_guidance和级联框架CFG-FMPG，用于图像翻译任务。


<details>
  <summary>Details</summary>
Motivation: 桥接模型虽能利用清洁先验进行数据到数据生成，但弱先验（训练中未见过）会阻碍先验利用，降低去噪效果。如何在不改变模型参数的情况下增强先验利用是关键问题。

Method: 提出训练无关的Prior Guidance (PG)方法：引入弱先验与可见先验进行对比，通过缩放因子增强先验利用。进一步分析桥接过程中先验利用机制，设计频率调制先验_guidance (FMPG)，根据低频和高频波段调整引导尺度。针对图像修复，开发级联框架CFG-FMPG，先用CFG生成噪声隐藏表示，再用FMPG将其作为生成先验进行利用。

Result: 实验表明，PG方法在多种图像翻译任务中持续提升预训练桥接模型的性能。

Conclusion: 先验对比和频率调制是增强桥接模型先验利用的有效策略，PG方法为数据到数据生成任务提供了新的训练无关引导范式。
...
TL;DR: KC-3DGS通过引入小波域监督来解决3DGS在稀疏视角下oversmoothing和结构伪影的问题。


<details>
  <summary>Details</summary>
Motivation: 标准像素空间损失(L1, SSIM)只约束聚合重建误差，允许优化在不同频率尺度间重新分配误差，导致oversmoothing和结构伪影，特别是在稀疏视角设置下。

Method: 提出三种组件：(1)多尺度小波系数对齐损失，明确惩罚缺失的高频细节；(2)监督峰度集中度损失，鼓励渲染图像匹配ground-truth的重尾频率统计；(3)跨带协方差惩罚，促进频率专业化。

Result: 在MipNeRF360、Tanks&Templates、MVImgNet、DeepBlending和WRIVA-ULTRRA数据集上实验，显示在感知质量上一致改进。在WRIVA-ULTRRA户外数据集上，KC-3DGS在DreamSim上实现9.48%提升，同时改善PSNR、SSIM和LPIPS。在只有12张训练图像的稀疏视角设置下，在MipNeRF360上PSNR提升高达0.5 dB。

Conclusion: 该方法可作为即插即用的正则化策略无缝集成到现有3DGS管道中。
...
TL;DR: 本文提出CVLAT框架，用于区分LVLMs是否真正进行视觉推理还是仅回忆训练时习得的事实先验。研究发现多个模型在标准测试中达到人类水平，但可能是事实Recall而非真正的视觉理解。模型可分为视觉导向型和知识导向型，prompt干预效果因模型而异。


<details>
  <summary>Details</summary>
Motivation: 当前评估混合了视觉正确性和事实正确性，无法清楚判断LVLMs的正确响应是基于真实视觉推理还是基于训练时记忆的事实先验。这导致高估或低估模型的视觉素养。

Method: 提出三个实验：(1)使用标准VLAT和随机化数据测试reVLAT对比；(2)使用反事实视觉素养评估测试CVLAT和VFRI指标分类模型；(3)prompt干预测试。研究包含15个SOTA LVLMs和30人基线对比。

Result: 多个模型在标准测试达到人类水平，但可能反映事实Recall；reVLAT在视觉解释被事实先验超越时会低估视觉素养；根据VFRI将模型分为视觉导向型、事实知识导向型和中间型；人类基线显示人在冲突时绝大多数遵循图表；Prompt干预可改变优先级但效果高度依赖模型且方向不对称。

Conclusion: 高视觉准确率不足以证明忠实的视觉推理；可靠的视觉分析集成不仅需要评估视觉素养，还需评估模型在视觉证据与事实先验分歧时如何仲裁。视觉能力不能预测prompt可控性。
...
TL;DR: OmniDreams是一个基于Cosmos扩散模型的基础生成式世界模型，通过在21k小时驾驶场景数据上进行训练后，能够在闭环系统中实时生成动作条件的视频，用于自动驾驶策略的训练和评估。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶能力的发展，长尾场景下的安全评估成为关键瓶颈。传统重建型神经模拟器受限于初始采集数据，难以泛化到高度动态或新颖的场景。

Method: 利用Cosmos扩散模型的丰富视觉先验，在21k小时驾驶场景数据上进行训练和后训练，构建能够自动回归生成动作条件视频的OmniDreams世界模型，并将其部署在包含Alpamayo 1策略模型和AlpaSim编排器的闭环系统中。

Result: 基于OmniDreams后训练的WAM（世界-动作模型）在NuRec数据集上性能超越VLA基础的Alpamayo 1.5研究策略模型，同时仅使用1/5的参数总量。

Conclusion: OmniDreams展示了实时生成式世界模型作为策略架构backbone的潜力，为下一代自动驾驶策略的训练和评估提供了可扩展的整体解决方案。
...
TL;DR: SRENet是一种基于频谱分析的点云动作识别框架,通过小波变换分解低频和高频分量来学习全局运动上下文和细粒度时间动态,在MSR-Action3D、NTU-RGBD和NTU-RGBD120数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 点云序列的不规则结构和时间不一致性给时空表示学习带来独特挑战,现有方法难以同时捕捉全局运动上下文和细粒度时间动态,而频谱分析视角提供了一种解耦不同频率特征的潜在方案。

Method: 提出SRENet框架包含三个核心创新:1)光谱分解块(SDeBlock)对时间和空间轴进行小波分析,将特征解耦为低频和高频分量并使用频率特定注意力;2)光谱重入块(SReBlock)执行二次时间分解以恢复残差动态;3)光谱感知学习策略通过对比损失和课程调度增强频率子空间的区分性。

Result: 在MSR-Action3D、NTU-RGBD和NTU-RGBD120三个主流点云动作识别数据集上进行了广泛实验,验证了频谱建模的有效性,SRENet实现了最先进的性能。

Conclusion: 从频率视角建模动作为点云动作理解提供了新思路,低频组件捕获粗略运动模式,高频组件编码精细时间动态,该框架为主动驾驶和人体交互等3D感知应用提供了有效的技术方案。
...
TL;DR: JAVEdit-100k是首个用于指令引导的联合视听编辑的大规模高质量数据集，包含约10万个人类中心视频的编辑三元组，涵盖五个不同类别，并配有JAVEditBench基准测试和JAVEdit基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前基于指令的视频编辑已取得显著进展，但联合视听编辑缺乏专门的数据集和基准测试，导致该领域研究受到约束。

Method: 通过四人精心设计的生成管道构建数据集，并与"人在环中"的质量控制机制配对；同时创建了包含精选源视频和对齐人类指令的综合基准测试；提出JAVEdit作为联合视听编辑的基线模型。

Result: JAVEdit-100k是首个大规模高质量的视听编辑数据集；在六个评估指标中的五个上优于所有基线模型。

Conclusion: 该研究填补了联合视听编辑领域数据集和基准测试的空白，为该方向的后续研究提供了重要的资源和基线。
...
TL;DR: 针对实例目标导航(IGN)任务，将交互式导航重新定义为成本敏感的不确定性减少问题，提出基于信息增益的分析方法来确定最优问题类型，并构建了新的基准和加权成功率评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有交互式IGN方法将轻量级澄清和路线级指导同等对待，导致智能体通过重复高信息量问题而非高效消除底层歧义来提高成功率，且现有基准未考虑不同问题类型的成本及交互效率。

Method: 对现有导航语料库进行信息增益分析，识别可减少导航不确定性的线索类型；构建含加权成功率指标的新基准；提出零样本MLLM导航器选择性查询策略。

Result: 生成紧凑的问题类型集合和基于数据的权重，建立了可诊断交互行为和效率的基准测试。

Conclusion: 所提框架可实现成本敏感的智能交互，使智能体仅在预期收益大于成本时才发起询问，从而更高效地解决导航中的歧义问题。
...
TL;DR: 本文提出GLINT框架，解决医学影像中图像报告对标注粒度不匹配的问题。传统方法对所有图像区域均匀分配权重，但实际诊断发现仅占图像的稀疏区域。GLINT通过门控对齐机制实现稀疏的查询特定定位，并结合密集特征正则化保持细粒度特征。该方法首次在无需掩码监督的情况下实现3D CT的零样本分割。


<details>
  <summary>Details</summary>
Motivation: 核心动机是解决医学VLM中的尺度不匹配问题：诊断发现仅占用图像的稀疏区域，但现有方法对全局图像-报告级别进行监督，导致模型将注意力分散到所有patch而非相关区域。这限制了需要精确 localization 的任务性能（如grounding和segmentation）。

Method: 提出两个核心技术：1）稀疏门控对齐(Sparsely Gated Alignment)：使用sigmoid门控机制基于查询嵌入空间激活相关的patch子集，实现显式稀疏性；2）密集特征正则化(Dense Feature Regularization)：将可训练编码器的中间特征锚定到冻结的自监督学习教师网络，保持细粒度patch特征。基于DINOv3(2D CXR)和V-JEPA 2.1(3D CT)构建，支持零样本分类、grounding和分割。

Result: GLINT在下游任务中超越自监督编码器和医学VLM，在分类、报告生成和分段任务上取得最优性能。尤其在需要查询特定定位的零样本grounding和分割任务上提升最显著，验证了设计意图。首次实现了3D CT volumes的零样本分割，无需任何掩码监督。

Conclusion: GLINT成功解决了医学VLM中的稀疏对应建模问题，通过门控机制实现查询相关的patch选择，结合特征正则化保持表示质量。该框架统一处理2D和3D医学影像，为放射学AI提供了高效的无需大规模标注的训练范式，具有重要的临床应用价值。
...
TL;DR: XIPER是一个跨域视频预测奖励模型，用于从视觉上不同的域中收集的专家视频进行强化学习，能够在存在领域差距（如智能体颜色、形态差异或模拟到真实差距）的情况下学习。


<details>
  <summary>Details</summary>
Motivation: 当从专家视频学习时，强化学习面临缺乏奖励信号和领域差距的挑战，特别是当智能体的外观因颜色、形态等因素而有所不同，或在模拟到真实的转移中存在差距时。

Method: XIPER训练一个跨域视频预测模型，将智能体观察映射到专家域，并使用预测似然作为奖励信号。在DMC Color Suite（8个任务）和DMC Body Suite（3个任务）上进行实验。

Result: XIPER始终优于基线方法，尽管存在领域差距如智能体颜色和形态的差异。在sim-to-real转移数据集上也展示出能够为真实机器人观察产生有意义的奖励信号。

Conclusion: XIPER可以有效地从视觉上不同的域中收集的专家视频学习，为解决模仿学习和域适应问题提供了一个新的奖励建模方法。
...
TL;DR: 本研究重新审视图像修复中的偏好对齐问题，采用直接偏好优化(DPO)框架，通过9个奖励模型构建偏好训练数据，在两个基准和两个baseline模型上进行实证研究。


<details>
  <summary>Details</summary>
Motivation: 解决图像修复中的偏好对齐问题，当前缺乏对该领域核心挑战的系统性重新评估。

Method: 采用直接偏好优化(DPO)框架，使用公开奖励模型构建偏好训练数据，设计简单集成和校准集成方法来缓解奖励黑客问题。

Result: 简单集成可缓解奖励模型的偏差（亮度、构图、色彩方案），提升泛化性能；偏好对齐可迁移到目标移除任务；在标准指标、LVLM评估和人类评估上均超越现有最先进模型。

Conclusion: 大多数奖励模型可提供有效的偏好信号，但存在明显偏差；简单集成和校准集成方法能有效提升稳健性和泛化能力，无需修改模型架构或引入额外数据集即可显著提升性能。
...
TL;DR: 提出MemoGen——一种无需训练的框架，通过为图像生成器添加智能演进层实现测试时自进化，利用显式经验记忆（包含任务理解、参考选择、视觉反馈、成功策略和失败教训）在不更新参数情况下持续改进生成质量，两轮迭代后超越GPT-Image-1等专有系统。


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型在显式视觉约束方面已取得较强成果，但在隐式视觉约束、关系推理或外部知识方面仍不可靠。现有检索增强和智能体生成方法通常将每次生成视为孤立事件，未能系统性地保存过去成功或失败的经验供未来使用。因此需要探索文本到图像系统是否能够从自身生成经验中持续改进而无需更新底层生成器。

Method: MemoGen是一个无需训练的框架，为现有图像生成器添加了智能演进层。其核心机制包括：1)显式推断视觉需求；2)在必要时检索外部证据和参考；3)将信息转化为可执行的生成约束；4)评估生成结果；5)将任务理解、参考选择、视觉反馈、成功策略和失败教训存储为可重用经验记忆。在演进过程中，智能体检索相关经验来改进未来相似生成 selectively修复先前失败的案例同时保留成功的。

Result: 在知识密集型和推理导向的基准测试上的广泛实验表明，该范式是有效的。仅经过两轮演进，基于开源Qwen-Image骨干构建的MemoGen就在WISE和Mind-Bench上超越了Nano Banana Pro和GPT-Image-1等强大的专有系统。

Conclusion: MemoGen通过显式经验记忆实现了文本到图像系统在不更新参数的情况下进行测试时自我进化，仅经过两轮迭代就超越了GPT-Image-1等强大专有系统，证明了经验记忆可以作为可靠文本到图像生成的持续学习信号。
...
TL;DR: 针对海上自主船舶(MASS)训练数据稀缺的难题，提出使用CycleGAN-turbo单步无配对翻译架构，通过零卷积跳跃连接绕过VAE瓶颈，有效保留小尺度航行物体（如远距离船只和航标）的结构细节。


<details>
  <summary>Details</summary>
Motivation: 海上自主船舶的感知系统受限于恶劣天气和低光照条件下的训练数据稀缺，而动态海况中收集成对图像物理上不可能，因此合成数据生成成为关键突破口。

Method: 采用CycleGAN-turbo一-step无配对图像翻译架构，引入零卷积跳跃连接（Zero-Convolution Skip Connections）绕过Variational Autoencoder潜在压缩瓶颈 PRESERVING小目标结构完整性。

Result: 构建7,000张海上图像数据集，完成Day-to-Foggy、Day-to-Sunset、Day-to-Night三种域翻译任务。定性评估表明：Day-to-Foggy和Day-to-Day模型表现良好结构保留；Day-to-Night模型存在语义幻觉（如生成人工海岸灯光）。

Conclusion: 本研究建立了高效的结构感知数据合成管道，直接解决了自主 Maritime导航中的数据稀缺瓶颈。CycleGAN-turbo与零卷积跳跃连接的组合可有效保持小尺度导航物体的语义完整性，为 MASS 感知系统训练提供可行的数据增强方案。
...
TL;DR: FreeStreamGS提出了一种用于在线新视角合成的鲁棒前馈框架，针对流式输入中相机内在偏差累积和姿态-深度漂移问题，通过解耦内在恢复头和动态点精修偏移策略实现长期稳定的高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有在线前馈方法虽可处理流式深度和点云重建，但无法应用于新视角合成，因为新视角合成对高斯尺度多视图一致性和姿态-几何对齐要求更严格，即使是轻微偏差也会在时序上累积导致明显伪影。

Method: 提出两个关键机制：1)解耦内在恢复头(Decoupled Intrinsic Recovery Head)去除累积的相机内在偏差并防止长期流式过程中的场景尺度抖动；2)动态点精修偏移策略(Dynamic Point Refinement Offset)放松刚性反投影以校正耦合的姿态-深度漂移。

Result: 实验表明FreeStreamGS在不具备未来帧访问能力的条件下达到了与离线前馈3DGS方法相当的渲染质量。

Conclusion: FreeStreamGS成功实现了从无姿态流式图像序列进行高效、高质量在线新视角合成，为实时场景渲染应用提供了可行方案。
...
TL;DR: PaddleOCR-VL-1.6是一个升级的紧凑文档解析模型，在前代0.9B参数基线基础上，通过区域感知数据优化框架和改进的训练后配方实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 前代PaddleOCR-VL-1.5仍存在错误，主要集中在模型行为不稳定、数据覆盖稀疏或监督不可靠的区域，需要有针对性的优化而非盲目扩大训练数据。

Method: 引入区域感知数据优化框架识别弱区域并进行针对性增强，同时采用基于精选数据选择和强化学习的渐进式训练后配方进行分阶段优化。

Result: 在OmniDocBench v1.6上达到96.33%的新的最先进水平，展示了与顶级视觉语言模型的强大竞争力。

Conclusion: PaddleOCR-VL-1.6为PaddleOCR-VL系列提供了一套实用的训练后配方，证明了针对特定弱区域的精准优化比大规模数据扩展更有效。
...
TL;DR: VistaHop是一个用于评估视觉深度搜索中多模态大推理模型的新基准，包含300张高分辨率图像、25个视觉搜索场景和350个多跳问答任务，并开发了VistaArena统一评估环境。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单步视觉理解或静态图像问答，缺乏对迭代图像检查、视觉锚点 grounding 和多跳证据集成的评估能力，因此需要新的基准来推动 Visual DeepSearch 领域的发展。

Method: 构建了VistaHop基准，包括300张高分辨率图像、25个视觉搜索场景和350个多跳QA任务；开发了VistaArena统一评估环境，支持文本搜索、图像搜索、图像裁剪和基于证据的答案验证；对7个代表性MLRM进行了实验。

Result: VistaHop基准显示当前最先进的MLRM在该任务上表现受限，揭示了视觉grounding、证据 revisit、长链推理和多锚点信息融合等方面的持久性局限性。

Conclusion: VistaHop作为一个具有挑战性的新基准，能够有效评估视觉深度搜索中的多跳视觉推理能力，为 Visual DeepSearch 领域提供了更强的基准和训练方法的研发方向。
...
TL;DR: 提出BA-T，一种受经典束束调整(BA)启发的迭代Transformer，通过在隐式token空间中实现BA风格的结构化更新来实现轻量级3D重建，用仅16%的解码器参数达到与大型模型相当或更优的性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D重建模型依赖重型解码器堆栈，缺乏几何精化的结构化机制，导致多视图一致性较差。受经典束束调整(BA)可视为姿态与局部几何之间迭代信息传播过程的启发来解决此问题。

Method: BA-T将BA风格的结构化更新实现为隐式token空间中的可重复层，通过单一轻量层基于潜在残差精化预测，而非依赖深度注意力堆栈，实现迭代改进。

Result: 实验表明BA-T在迭代中逐步提升姿态和重建精度，比传统解码器获得更强的跨视图一致性，且仅使用16%的解码器参数即可匹配或超越大型模型。

Conclusion: BA-T提供了一种紧凑、高效且结构化的深度注意力替代方案，能够在轻量级架构中实现准确的3D重建，并实现姿态与几何的联合优化。
...
TL;DR: 本文提出TASE方法,通过将预训练的2D语义特征投影到截断感知嵌入空间,实现高保真3D场景的灵活编辑,并支持文本驱动的编辑控制。


<details>
  <summary>Details</summary>
Motivation: 当前3D场景表示方法对可控编辑的支持有限,需要一种能够灵活编辑、可控制编辑强度的方法,以便更容易适应特定目标应用场景。

Method: 提出截断感知嵌入空间:将2D语义特征投影到该空间,逐步减少特征通道得更抽象表示,保留更多通道则保留细节;使用尺度和平移等变损失提高多视图一致性;引入编辑扩散模型的微调阶段以减轻几何变化产生的伪影。

Result: 在3D场景编辑任务中达到有竞争力的性能,在涉及大几何修改的编辑上显著优于先前方法。

Conclusion: TASE方法成功实现了高保真3D场景的可控编辑,为机器人、自动驾驶和模拟等应用提供了更灵活的解决方案。
...
TL;DR: RegNetMamba-2是一种新型的多模态图像配准算法，将结构化状态空间对偶性（SSD）引入粗到细匹配过程中，在三个不同尺度上进行多模态特征提取，并通过跨模态特征交互模块和多尺度特征融合模块实现共享特征提取和特征融合，在VIS-SAR、VIS-IR和VIS-NIR数据集上实现了良好的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 多模态图像配准的主要挑战在于提取共享结构信息。相比Transformer，SSD在训练和推理中能够以更高效率提取全局结构特征。

Method: 首先在三个不同尺度应用SSD进行多模态特征提取，通过SSD的特征缩放函数增强前景边缘和结构信息的局部表示；其次提出基于SSD的跨模态特征融合模型，包含跨模态特征交互（CMI）模块和多尺度特征融合（MSF）模块，分别用于跨模态特征提取和渐进式向上融合；最后按照粗到细策略，使用CMI的1/8尺度和MSF的1/2尺度特征计算匹配概率分数，进行像素级对应匹配。

Result: RegNetMamba-2在VIS-SAR（OSDataset）、VIS-IR（LGHD/RoadSence）和VIS-NIR（RGB-NIR sense）数据集上进行广泛实验，表明其在性能和效率方面均优于最先进的深度学习算法。

Conclusion: 所提出的RegNetMamba-2算法通过引入SSD到多模态图像配准任务中，有效提取了局部和全局结构特征，在多个公开数据集上取得了优异的配准效果。
...
TL;DR: 提出P-Topics（感知主题）建模新问题，旨在理解图像在不同文化中的情感感知方式。PercepT是一个两阶段架构，用于发现图像感知体验并将其映射到相关主题。


<details>
  <summary>Details</summary>
Motivation: 解决如何在数据集中发现和建模不同的感知体验，以及如何将图像与其相关的感知体验关联起来的问题。这些感知体验包含客观事实和主观情感两个方面。

Method: 提出PercepT（感知主题转换器），采用两阶段架构：1）形成阶段通过无监督训练目标发现P-Topics作为视觉-文本聚类，动态选择聚类数量；2）映射阶段通过注意力池学习P-Topic映射函数，将图像与对应聚类关联。

Result: 在ArtELingo数据集上，PercepT实现轮廓系数0.97（对比基线0.37），AUC得分0.94（对比基线0.77）。人类评估确认其能捕捉语义上有意义的感知体验并显著优于现有方法。

Conclusion: P-Topics建模为理解跨文化图像情感感知提供了新框架，PercepT能有效发现感知主题聚类并实现准确的图像-主题映射，性能显著优于现有基线方法。
...
TL;DR: 本文提出了合成可信度（synthetic credibility）这一新型视觉虚假信息威胁，即生成模型可以创建带有逼真嵌入文本和布局的视觉内容。研究者构建了SYNCRED-Bench基准（含600张AI生成的虚假信息图像）和FP450真实图像负集，用于评估现有检测系统的性能。


<details>
  <summary>Details</summary>
Motivation: 生成模型如今可以生成带有逼真嵌入文本和布局的视觉产物，形成了新的虚假信息威胁：合成可信度。这是一个严重但尚未被充分探索的视觉虚假信息挑战。

Method: 构建了SYNCRED-Bench基准数据集，包含600张AI生成的虚假信息图像，平衡分布在六个可信形式类别和七个细粒度传播风格中；同时构建了FP450真实图像负集用于测量误报率。对15个多模态大语言模型（MLLMs）、开源AIGC检测器和商业API进行了广泛评估，并在5%误报率约束下测试了真阳性率（TPR）。

Result: 在5%误报率约束下，现有系统仍然不可靠：15个MLLMs仅达到10.5%的真阳性率，开源AIGC检测器低于5%，商业API达到57.6%。人类标注者也难以识别合成可信度，仅达到63%的真阳性率。

Conclusion: 合成可信度是一个严重且尚未被充分探索的视觉虚假信息挑战。该研究为开发能够超越表面可信度线索进行推理的检测器提供了基准数据集和评估框架。
...
TL;DR: 本文提出P²‑DPO，通过自生成(on‑policy)偏好对聚焦增强感知与视觉鲁棒性，显著降低大视觉语言模型中的幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有直接偏好优化(DPO)未专门解决注意区域的感知瓶颈，且使用的偏好数据往往是视觉无关(vision‑agnostic)且离策略(off‑policy)，导致对图像退化的鲁棒性不足，因而需要新的训练范式。

Method: ①构建针对“聚焦‑增强感知”(Focus‑and‑Enhance)和“视觉鲁棒性”的on‑policy偏好对；②设计校正损失(Calibration Loss)将视觉信号与文本生成的因果关系精确对齐。

Result: 在同等规模训练数据和计算成本下，P²‑DPO在多个基准上超越依赖昂贵人类反馈的强基线；在注意力区域保真度(ARF)和图像退化场景中显著提升性能。

Conclusion: P²‑DPO能够针对性解决感知瓶颈并增强对退化输入的视觉鲁棒性，为大型视觉语言模型的 hallucinations 问题提供了一种高效、可扩展的训练方案。
...
TL;DR: 提出了SIU²A框架，用于评估科学图像的实用性和可升级性，涵盖错误检测、修正可行性和修正质量评估，构建了SIU²A-Benchmark数据集，表明当前多模态系统在科学错误评估方面存在显著局限性。


<details>
  <summary>Details</summary>
Motivation: 现有评估范式不足：感知质量指标与科学有效性相关性差，语言模型缺乏领域特定的验证能力。AI生成内容对科学图像完整性构成新威胁。

Method: 提出SIU²A框架，包含两个维度：实用性（错误检测和修正可行性）和可升级性（修正质量）；将图像 corruption分为四类：细节扭曲、不完整性、虚假内容和实体混淆；构建专家注释的SIU²A-Benchmark数据集；实施两阶段评估协议。

Result: 当前多模态系统在科学错误评估和忠实修正方面存在显著局限性，暴露了视觉感知与科学可用性之间的根本差距。

Conclusion: 需要开发更好的领域特定验证能力，以弥合视觉感知与科学可用性之间的差距。
...
TL;DR: 本文提出一种基于隐式运动的两阶段音频驱动人体动作视频生成框架，从单张静态图像和音频合成逼真、时间一致的人体动画。


<details>
  <summary>Details</summary>
Motivation: 现有基于关键点的方法难以捕捉细微运动动态，需要更灵活的架构来实现高质量的说话头部合成、共手势生成和动态演示。

Method: 两阶段管道：阶段一整合外观先验和层级深度线索到区域感知注意力机制建模潜在运动特征；阶段二使用Mamba增强的扩散模型直接从音频和源图像预测这些特征，实现细粒度运动模式的无监督学习。

Result: 在380小时高质量数据集上训练后，该方法在多个公开基准和我们收集的数据上优于先前工作，在准确性、自然性和时间一致性方面达到最优。

Conclusion: 解耦运动预测与渲染的架构提高了灵活性和效率，建立了音频驱动人体动作视频生成的新技术标杆。
...
TL;DR: SAMatcher是一种基于共可见性建模的特征匹配框架，通过在Segment Anything Model (SAM)基础上预测共可见区域 masks 和边界框作为结构化先验，实现跨视角特征交换与语义对齐，在大规模视角和尺度变化场景下显著优于现有匹配方法。


<details>
  <summary>Details</summary>
Motivation: 可靠的对应关系估计是图像处理的基础问题，支撑着运动恢复结构、视觉定位和图像配准等应用。现有学习方法虽然改进了局部特征表示，但大多在像素或patch级别操作，缺乏对跨视图联合可见区域的显式建模，需要区域级别的对应推理能力。

Method: 提出共可见性建模的对应估计框架：1) 基于SAM预测共可见区域mask和边界框作为结构化先验；2) 引入对称跨视角交互机制实现双向特征交换和跨视角语义对齐；3) 开发统一监督方案，联合优化mask预测、box回归和mask-box一致性约束。

Result: 在挑战性基准数据集上实验表明，相较于现有匹配管道获得实质性提升，尤其在大视角和尺度变化条件下表现优异。

Conclusion: 证明原本为单目分割设计的基础模型可以通过显式共可见性建模有效扩展到多视角对应推理，为图像匹配的_structured表示学习提供了新视角。
...
TL;DR: Enginuity是首个用于评估VLMs在复杂工程图上能力的开放数据集和基准，包含两个任务：结构化零件表提取和自由形式可视化问答。实验表明当前模型在描述保真度方面存在系统性差距。


<details>
  <summary>Details</summary>
Motivation: 工程图在服务、维修和设计工作流中处于核心地位，但目前没有公开基准来衡量VLM在该领域的能力；现有数据集主要关注流程图、科学图形或商业文档。

Method: 定义了基于美国军事服务和维护手册的两个任务：结构化零件表提取（Task 1）和自由形式视觉问答（Task 2）。使用四种前沿VLM（GPT-5.2 Chat、Claude Opus 4.7、Gemma 4、Qwen3-VL-32B-Instruct）在零样本和思维链提示下进行评估。

Result: Task 1模型达到Recall@all为0.61-0.87，但Token F1仅为0.03-0.18，暴露出零件识别与描述保真度之间的系统性差距。Task 2显示所有模型都存在一致的事实推理差距。分析表明，基于token重叠的指标比语义相似度，低估了模型在技术描述上的能力2-6倍。

Conclusion: 发布数据集、注释、评估工具和每样本模型输出，以支持对工程内容VLM能力的可重复研究，并建议使用LLM作为裁判进行特定领域评估校准。
...
TL;DR: IMT-CXR是一种可解释的多任务Transformer框架，模拟放射科医生的诊断流程，通过疾病识别、属性表征和证据整合报告生成三个阶段进行胸部X光分析。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态深度学习系统多为黑盒模型，仅执行孤立任务，忽略了临床诊断作为多任务过程的信任敏感性。放射科医生的工作是多阶段的，包括识别疾病、表征病变特征并生成报告，但现有AI系统缺乏这种可解释的诊断路径。

Method: 提出统一Transformer架构，通过医学领域指令优化，依次执行四个临床任务：多标签疾病分类、病变定位、解剖分割和放射学报告生成。该框架包含三个证据驱动阶段：1)疾病识别；2)属性表征(大小、位置、严重程度)；3)带可追溯决策路径的证据整合报告生成。

Result: 在10个CXR基准数据集上验证，性能具有竞争力。对来自4个医疗中心的160份历史报告进行盲评，3名放射科医生认为66%的AI生成报告在诊断清晰度方面与或优于原始临床报告。

Conclusion: 通过建立从解剖发现到结论的可追溯诊断路径，该工作弥合了AI技术指标与临床实用性之间的差距，推动了可信AI系统在医学影像领域的发展。
...
TL;DR: 本文提出Incongruity-aware Distribution Optimization (IDO)方法,通过事实不一致性和模态不一致性两个角度改进假新闻检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态假新闻检测方法主要关注跨模态一致性,往往难以明确建模欺骗性多模体内容中语义不一致的特征,而错误信息通常包含与事实不符的语义信息。

Method: 针对事实不一致性,引入通道重加权策略获取语义可区分的嵌入,并利用高斯分布建模由事实不一致性引起的不确定相关性;针对模态不一致性,利用不一致对比学习来学习跨模态语义信息。

Result: 实验表明IDO实现了最先进的性能。

Conclusion: IDO通过同时处理事实不一致性和模态不一致性问题,有效提升了多模态假新闻检测的准确率。
...
TL;DR: PHAF是一种个性化的照片级真实感手部头像系统，能够从两张图像（背面和掌面视图）生成高质量的多视图渲染。该方法专为边缘设备的实时部署而设计，结合了语义引导的网格对齐和密集纹理提取技术，实现了高效的细节传输。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖于耗时的优化技术，难以满足AR/VR应用对实时性和边缘设备部署的需求。需要一种能够快速生成个性化纹理的方法，同时保持视觉质量。

Method: 1) 结合语义引导的网格对齐与密集纹理提取，实现高频细节的高效转移；2) 采用基于视图的修复网络进行纹理细化，确保平滑连续的外观；3) 利用参数化手部模型实现准确的手势表达和视角泛化。

Result: 该方法在视觉质量上与现有方法相当，但纹理生成速度提升了30倍，可支持实时边缘设备部署。

Conclusion: PHAF为实际AR/VR应用提供了实用解决方案，兼顾了视觉质量与计算效率。其泛化到新视角的能力使其兼容标准图形引擎，具有广泛的应用潜力。
...
TL;DR: PRISM提出一种双流MoE框架，通过模块化专业化和动态重组整合多种视觉基础模型，解决特征冲突问题，在分割任务上取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 由于单体蒸馏存在负转移问题，将不同视觉基础模型的优势统一到一个高效模型中具有挑战性，需要解决特征冲突问题。

Method: 提出两阶段范式：(1)专业去构阶段：教师条件路由引导专家专门化到不同的表征子空间以减少干扰；(2)动态重组阶段：路由学习将专家组装成针对下游任务的定制计算路径。

Result: 在PASCAL-Context和NYUD-v2数据集上建立新的最先进水平，验证了稀疏-emergent专业化是集成不同视觉知识的可扩展方法。

Conclusion: 稀疏、emergent专业化是整合多样化视觉知识的有效且可扩展的方法，PRISM成功通过模块专业化解决了VFMs间的特征冲突问题。
...
TL;DR: 提出了一个结合3D语义感知、基于不确定性的异常检测、规则检查、设备端LLM推理和GraphRAG记忆分析的连续监测框架，用于地下煤矿安全监控。该系统将3D点云转换为结构化、可追踪的安全推理输出。


<details>
  <summary>Details</summary>
Motivation: 地下煤矿作业空间狭窄、照明不良，设备和人员接近违规、结构不稳定、盲点等危险难以预测。传统监控系统缺乏3D场景理解和上下文记忆能力，无法识别复杂或演变的危险。

Method: 融合真实巷道扫描、受控物体放置和高保真长壁模拟生成多样化危险场景；使用自监督预训练改进分割效果。框架整合3D语义感知、不确定性异常检测、规则危险检查、设备端LLM推理及GraphRAG记忆分析，利用场景图和时间图作为显式知识结构链接各阶段推理输出。

Result: 感知模型达到92.7%准确率、30FPS处理速度、低内存占用。115个危险场景测试中：规则检查覆盖57%，结合上下文LLM推理提升至76%，结合历史记忆的推理达93%。定性结果表明不确定性衍生的异常信号有助于解释超出预定义类别的分布外危险。

Conclusion: 基于图的知识表示结合3D感知与分层安全推理为地下矿井监控智能决策支持提供了实用基础。
...
TL;DR: 本文提出了Dual Face-Hair Retrieval（DFHR）任务，一个新的混合模态双参考图像检索任务，需要根据面部图像（身份）+发型参考（图像或文本）的查询组合进行跨组件推理。同时构建了首个基准数据集DFHR-Bench并提出统一框架MFHC。


<details>
  <summary>Details</summary>
Motivation: 现有检索设置无法处理来自异构模态的两个语义独立属性（身份和发型）的跨组件推理，缺乏统一的嵌入空间来融合身份特征和发型特征。

Method: 构建包含超过18万标注三元组的DFHR-Bench基准数据集；提出MFHC框架，通过token注入和多视图监督融合解耦的身份和发型embedding，在统一嵌入空间中进行混合模态组合。

Result: 建立了首个混合模态人脸-发型检索基准数据集，提出统一框架实现身份感知的属性可控视觉检索。

Conclusion: DFHR和DFHR-Bench为跨模态身份感知、可控属性的视觉检索建立了新范式，推动了混合模态检索领域的研究。
...
TL;DR: PersistGS通过将可微分刚体模拟与3D高斯溅射相结合，在目标被完全遮挡时恢复对象永久性。该方法将场景分解为每目标高斯和碰撞网格，从遮挡前观察到的轨迹估计摩擦和速度，并使用SE(3)轨迹在遮挡期间定位目标高斯。


<details>
  <summary>Details</summary>
Motivation: 当前3DGS方法在目标被完全遮挡时会退化，因为高斯无法接收梯度信号。现有方法依赖学习生成先验，重视视觉可信度而非物理正确性。

Method: 1.将场景分解为每目标高斯和碰撞mesh；2.从遮挡前观察到的轨迹估计摩擦和速度；3.使用可微模拟预测SE(3)轨迹；4.引入质心轮廓损失隔离位置梯度

Result: 比光度监督低40%轨迹误差；比常速外推提高+2.46dB PSNR；与真实轨迹上界仅差0.19dB

Conclusion: PersistGS能够准确捕捉接触事件（弹跳、摩擦减速、方向变化），在合成场景中优于现有方法，实现高质量时变场景重建。
...
TL;DR: 提出一种基于稀疏点轨迹的高效动作识别方法，使用2.5D轨迹Transformer架构和掩码轨迹预训练，在仅使用部分输入的情况下达到与Dense RGB方法相当的性能，且轨迹特征与外观特征互补。


<details>
  <summary>Details</summary>
Motivation: 解决现有动作识别模型过于依赖密集RGB视频计算资源消耗大且容易利用场景/物体捷径的问题，探索更高效且避免偏差的输入模态。

Method: 开发2.5D轨迹Transformer架构处理稀疏点轨迹作为输入，设计掩码轨迹预训练任务提升下游任务准确率。

Result: 在SSV2达到45% top-1，EPIC-Kitchens-100达54%；与DINOv2融合提升8.7点，与V-JEPA 2融合提升1.6点。

Conclusion: 稀疏轨迹是一种高效且无捷径偏差的替代输入模态，轨迹特征与外观特征具有互补性，融合后可显著提升性能。
...
TL;DR: 本文提出Poison-3DGS基准，用于研究3D高斯基 splatting中投毒攻击的阶段依赖性检测问题。通过系统分析发现：检测能力在不同阶段显著变化，无单一阶段能持续主导；不同攻击呈现独特的阶段特异性取证信号；后期阶段信号（如训练动态、高斯参数统计）提供前期不可观测的有效线索。


<details>
  <summary>Details</summary>
Motivation: 3DGS在实时新视角合成中表现出色，但易受多种投毒攻击。当前研究主要关注攻击成功率，而防御和检测机制缺乏探索。3DGS重建管道的多阶段特性产生异质中间表示，为检测带来挑战和机遇——某阶段引入的攻击可能仅在后续阶段显现可检测信号，这促使从阶段视角研究可检测性。

Method: 构建Poison-3DGS基准，在多样场景和攻击类型下暴露阶段特异性伪影，包括多视图图像、几何、训练动态和高斯参数。对管道德各个阶段进行系统性可检测性研究。

Result: 三点关键发现：(1)检测能力随阶段显著变化，跨攻击类型无单一阶段持续主导；(2)不同攻击展现独特阶段特异性取证信号，检测效果高度依赖信号观察位置；(3)后期阶段信号如训练动态和高斯参数统计提供前期不可见的强线索。

Conclusion: 本研究提供原则性基准和首个3DGS阶段依赖性检测的系统性表征，为未来鲁棒可靠的3DGS系统研究奠定基础。
...
TL;DR: 本文提出AvatarMix方法，通过组合两个高保真Gaussian头像来实现3D头像服装迁移，解决了现有方法的服装质量下降和交叉伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D头像服装迁移方法存在两大挑战：2D编辑提升到3D会导致服装或身份质量下降；分别建模身体和服装层容易产生交叉伪影。

Method: 采用组合范式，直接从头部和身体两个高保真Gaussian头像组成；提出两步精炼策略：SeamFix局部扩散模块修饰头发和颈部，FullbodyFix可选的全身精炼恢复服装外观；使用基于网格的Gaussian表示实现身体重塑。

Result: 实验表明该方法在服装保真度和身份保持方面达到最先进水平，为现实的3D服装个性化提供新视角。

Conclusion: AvatarMix通过组合范式和精炼策略有效解决了3D服装迁移中的质量和伪影问题，保留了用户的身体身份特征。
...
TL;DR: 本文提出了一种两阶段PCB缺陷检测框架，结合结构引导的混合掩码预训练和空间连续性正则化。该方法首先利用无标签PCB图像进行稀疏卷积掩码预训练，通过结构引导的混合掩码构建信息丰富的掩码输入，使检测器 backbone 能够从可见导电模式推断缺失的 PCB 结构。在微调阶段引入空间连续性正则化项约束同一缺陷实例的分散正预测，促进紧凑定位。在 DsPCBSD+ 数据集上实现了 85.5% mAP0.5 和 52.3% mAP0.5:0.95 的性能。


<details>
  <summary>Details</summary>
Motivation: PCB缺陷检测是自动光学检测(AOI的重要组成部分，但在实际中面临挑战。许多缺陷微小、低对比度且嵌入在密集的电路背景中，难以被准确检测。

Method: 该方法采用两阶段框架：1) 预训练阶段设计稀疏卷积掩码预训练方案，使用结构引导的混合掩码构建信息掩码输入，稀疏卷积重建管道抑制掩码区域的无效响应，使backbone学习PCB结构先验；2) 微调阶段引入空间连续性正则化项约束分散的正预测，促进在细长缺陷区域上的紧凑定位。

Result: 在DsPCBSD+数据集上，该方法达到了85.5%的mAP0.5和52.3%的mAP0.5:0.95，优于多个强基线检测器。消融研究和定性结果进一步验证了该框架在工业AOI场景中进行稳健PCB缺陷检测的有效性。

Conclusion: 该两阶段框架通过结构引导的混合掩码预训练有效学习了PCB结构先验知识，空间连续性正则化提升了缺陷定位的紧凑性，在公开数据集上取得了领先的检测性能，为工业AOI中的PCB缺陷检测提供了有效的解决方案。
...
TL;DR: EvoMemNav是一个针对零样本具身导航的高效自演进细粒度记忆框架，通过构建视觉-语义记忆图(VSMGraph)保留原始视角作为一等公民，结合粗到细的两阶段策略实现高效搜索，并在子任务后执行反思驱动的回写更新先验知识。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两大缺陷：1) 以检测器为中心的场景图将观察压缩为稀疏节点，丢弃细粒度视觉证据并累积噪声；2) 基于3D重建的方法计算成本过高。因此需要一种既能保留细粒度细节又能保持计算效率的记忆系统。

Method: 1) 构建VSMGraph：将原始视角作为一等公民，用轻量级语义线索和拓扑关系组织成"房间-视角-对象"层级结构；2) 采用预算化的粗到细策略：粗阶段压缩搜索空间到有前景的区域，细阶段仅调用VLM进行针对性验证和决策；3) 实现反思驱动的回写机制：每个子任务后更新图附先验知识，无需重训练即可 refined未来决策。

Result: 在GOAT-Bench和HM3D数据集上，针对对象、文本描述和图像目标三种模态的实验表明：SR(成功率)和SPL指标持续提升，多实例消歧能力增强，过早停止减少，零样本泛化能力更强。

Conclusion: EvoMemNav通过保留细粒度视觉细节的自演进记忆架构，配合高效的粗到细验证策略和反思式知识积累，为零样本具身导航提供了一种有效且可扩展的解决方案。
...
TL;DR: 本文提出Null-Space Tuning (NST)框架，用于解决低质量视频的时空视频定位问题。该方法利用零空间的几何性质，将可学习残差注入输入特征，使高质量成分保持冻结，低质量成分得到恢复。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对高质量视频，而忽视了现实中普遍存在的低质量视频。现有的LoRA等调优方法会破坏预训练知识，因此需要一种在不破坏预训练知识的情况下适应低质量输入的方法。

Method: 提出零空间调谐(NST)框架，结合质量自适应单元和双空间重参数化，将可学习残差注入输入特征。对于高质量输入，成分被限制在零空间从而被冻结权重消除；对于低质量输入，修复成分指向非零空间实现恢复。

Result: 在自建的Mixed-Quality基准数据集上实验表明，NST方法优于当前最先进的方法。

Conclusion: 通过利用零空间的几何性质，NST能够在修复低质量输入的同时保留高质量输入的预训练知识，为实际应用中的混合质量视频定位提供有效解决方案。
...
TL;DR: 本文提出CR-Seg框架，用于解决基于复杂语言描述的推理分割任务，通过粗到精两阶段方法结合注意力提取与思维链引导实现目标分割。


<details>
  <summary>Details</summary>
Motivation: 现有推理分割方法存在两个主要局限：一是依赖学习语义标记的方法存在跨模态对齐困难；二是使用边界框等显式空间提示会丢失整体响应语义。此外，还存在推理-答案不一致的问题。

Method: 提出两阶段粗到精推理分割框架CR-Seg，包含三个核心模块：1) EAP模块用于提取注意力图进行粗定位并选择信息点；2) 将注意力图和信息点输入SAM进行Mask精炼；3) GLCoT思维链引导从全局场景上下文到局部目标细节进行渐进式推理。

Result: 在推理分割基准数据集上进行了广泛实验，验证了CR-Seg的有效性。

Conclusion: 该方法通过注意力引导和思维链增强的方式，成功解决了推理分割中的跨模态对齐和语义缺失问题，为多模态大语言模型与分割模型的结合提供了新思路。
...
TL;DR: 研究开发了一种基于SwinUNETR的自动化管道，利用小 patch 采样技术对多发性硬化症(MS)患者的侧脑室脉络丛(LVCP)进行自动分割。该方法在三个数据集上进行了验证，在扩展测试集上使用MPRAGE和FLAIR组合获得了0.868的平均Dice相似系数，显著优于3D UXNET模型(0.858)，同时计算成本降低了99%。


<details>
  <summary>Details</summary>
Motivation: 目前LVCP作为MS相关物理残疾和神经炎症的关键影像学生物标志物受到越来越多的关注，但手动分割脉络丛非常繁琐，限制了其在大型临床试验和纵向评估中的应用。因此需要开发自动化的分割方法来解决这一瓶颈。

Method: 使用SwinUNETR架构，基于32×32×32体素的小块进行训练，采用针对性的室内和小周patch采样策略。评估了三种数据：两个MS主导队列的三个数据集（数据集1：n=177；数据集2：n=177；扩展测试集：n=388），并与3D UXNET模型进行比较。使用Dice相似系数(DSC)作为主要评价指标，补充使用计算量(GFLOPs)和95%豪斯多夫距离(HD95)。

Result: 在扩展测试集上，使用MPRAGE和FLAIR组合输入时，SwinUNETR模型的平均DSC为0.868（95%CI：0.863-0.872），相比UXNET的0.858（95%CI：0.853-0.862）有统计学显著提升(p<0.0001)。单独使用FLAIR输入时，transformer方法维持了0.863的高DSC，而UXNET的空间定位明显恶化（HD95：1.86 vs 3.00mm）。该框架将计算成本降低了99%（91.8 vs 22080 GFLOPs）。

Conclusion: 通过整合局部patch采样与SwinUNETR架构，该方法为LVCP分割提供了准确、稳健且统计上优于当前领先模型的替代方案。其计算成本的大幅降低使其非常适合在临床和研究环境中广泛推广应用。
...
TL;DR: 本文提出两种学习式过滤模块（D2D-Rescore和GossipNet3D）替代传统的启发式非极大值抑制（NMS），用于LiDAR 3D目标检测中的候选框后处理阶段，通过检测间的注意力机制或消息传递实现更紧凑可靠的感知结果。


<details>
  <summary>Details</summary>
Motivation: 现有3D目标检测方法依赖手工设计的NMS进行候选框筛选，存在对小物体和稀有类别召回不足、无法捕捉检测间关联信息的问题，亟需一种数据驱动且与评估指标对齐的后处理方案。

Method: 提出两种基于关系学习的过滤方法：1）D2D-Rescore采用Transformer架构构建检测到检测（D2D）的自注意力机制，重评分并筛选提案；2）GossipNet3D将2D图神经网络扩展到3D，在鸟瞰视角下进行局部消息传递。同时设计了匹配nuScenes评估协议的度量感知训练策略。

Result: 实验在nuScenes数据集上验证，两种方法均显著提升了mAP（平均精度）、NDS（nuScenes检测分数）和真阳性质量，相比CircleNMS尤其在小物体和稀有类别上改进明显，且仅引入少量计算开销。

Conclusion: 学习式的检测层过滤方法可在不修改基础网络的情况下增强3D检测器的可靠性，为传统启发式抑制提供了一种原则性的替代方案。
...
TL;DR: 本文提出STS框架，一种用于视觉语言模型的两阶段视觉token剪枝方法，通过解耦剪枝过程解决传统方法中注意力分数过度集中导致特征多样性下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉token剪枝方法主要依赖初始注意力分数，但高注意力分数本质上会聚集在语义相似的区域，严重降低特征多样性并丢弃关键上下文信息，需要一种能同时保证结构多样性和语义相关性的剪枝方案。

Method: STS采用两阶段框架：第一阶段使用基于排斥的采样机制最大化空间和结构多样性；第二阶段利用指令感知的交叉注意力精确过滤与提示无关的token。两阶段协同确保几何覆盖后根据语义相关性精炼保留的token。

Result: STS缓解了基于注意力选择带来的冗余问题，提升了保留token的结构多样性和细粒度任务对齐能力。

Conclusion: STS框架通过显式解耦剪枝过程，为VLM推理阶段的计算效率优化提供了一种有效方案，在保持结构多样性的同时实现了语义相关的精准剪枝。
...
TL;DR: 提出了ReasonMatch-Bench基准测试，用于评估多模态大语言模型(MLLMs)在宽基线匹配任务中的空间推理能力，并提出动态对应强化学习(DCRL)方法来提升模型的细粒度宽基线对应能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs缺乏针对宽基线匹配的系统和评估框架，而WBM需要整合几何理解、视点变化、细粒度感知和遮挡推理能力，是物理环境中空间推理的重要测试。

Method: 构建了分层基准测试ReasonMatch-Bench，按视点位移和匹配粒度划分场景；设计了可扩展的数据生成管道从视频-3D语料库自动提取宽基线视图对；提出DCRL方法结合图像级视点进展和点级对应课程，通过可验证奖励进行训练。

Result: 在90样本困难子集上，人类标注者达到84.0 F1，而最佳现有基线仅为37.2。DCRL显著提升了ReasonMatch-Bench性能，并能转移到相关空间基准测试，同时保持通用视觉理解性能。

Conclusion: DCRL方法有效弥补了MLLMs在细粒度宽基线对应方面的能力差距，为物理环境中的空间推理训练提供了新范式。
...
TL;DR: 研究潜空间扩散性(latent diffusability)，系统性地评估不同tokenizer配置与训练策略对下游扩散生成质量的影响，并提出VIV指标来预测生成质量。


<details>
  <summary>Details</summary>
Motivation: 当前tokenizer主要关注重建质量，但高重建质量不一定带来好生成质量。现有研究提出了多种解释diffusion友好潜空间的理论，但缺乏在多样化设置下的系统验证。

Method: 训练大量不同正则化策略、架构和潜配置的tokenizer，在多个下游扩散 backbone上进行评估，系统分析潜空间属性与生成质量的相关性。

Result: 识别出多个在不同实验设置下稳定预测生成质量的潜空间属性，并引入VIV(Velocity Irreducible Variance)指标，该指标是预测生成质量最稳定的指标之一。

Conclusion: VIV可作为评估潜空间质量的有效指标，帮助设计更好的tokenizer以提升扩散模型的生成效果。
...
TL;DR: 本文提出UnsOcc框架,针对非结构化场景(如露天矿)的3D语义占用预测问题,通过渲染融合模块和Gaussian Splatting辅助监督提升预测精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 非结构化场景中不规则障碍物和稀疏场景布局破坏了传统3D目标检测方法的有效性,且场景稀疏性阻碍跨模态融合,长尾分布进一步降低预测性能。

Method: 提出RenderFusion渲染融合模块实现双向渲染监督增强跨模态特征对齐;提出GSRefinement基于Gaussian Splatting将稀疏3D占用预测投影为密集2D语义分割图,为长尾类别提供有效监督。

Result: 在自建露天矿数据集和nuScenes数据集上实验表明,该方法显著优于现有state-of-the-art方法。

Conclusion: 通过渲染融合和Gaussian Splatting细节感知监督,有效解决了非结构化场景中跨模态融合困难和长尾类别监督不足的问题,提升了3D语义占用预测的鲁棒性。
...
TL;DR: 提出了PF-OPSD方法，结合世界模型和多模态大语言模型进行受控具体推理，并在VRQABench和OpenWorldQA两个基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 世界模型可以生成视觉rollout，但可能看起来逼真却任务错误，因此需要确定何时视觉模拟有用、rollout是否可信以及如何影响最终答案，这是一个关键的开放问题。

Method: 构建了VRQABench和OpenWorldQA两个人类验证基准，提出PF-OPSD方法，在训练时使用真实未来视频和答案作为教师侧特权上下文评估策略具体推理轨迹，而部署的学生在测试时无法观察真实未来。

Result: PF-OPSD在VRQABench上超越基线10.6%，在OpenWorldQA上超越10.9%，同时增强了对噪音或冲突rollout的鲁棒性。

Conclusion: 该方法能学习调用、验证和整合视觉未来模拟与抽象推理，提升任务正确性和对错误rollout的抵抗力。
...
TL;DR: SkelHCC是一个统一的基于骨架动作识别的单样本学习框架，利用双曲空间和CLIP模型实现跨模态对齐


<details>
  <summary>Details</summary>
Motivation: 单样本设置下仅有一个标注样本，学习能捕捉人体运动层级结构并与动作语义有效对齐的表示是核心挑战。现有欧几里得嵌入方法难以建模骨架数据的树状组织，限制了跨模态对齐和对未知动作类别的泛化。

Method: 提出EH-HCLIP模块将骨架序列和动作语言嵌入共享双曲空间，利用负曲率和指数体积增长特性编码人体关节-部分-身体的层级结构；同时集成训练无关的LLM引导多粒度投票缓存实现上下文感知的推理。

Result: 在NTU RGB+D 60、NTU RGB+D 120和PKU-MMD数据集上验证，SkelHCC一致优于现有最先进方法。

Conclusion: 双曲几何和跨模态CLIP对齐为单样本骨架动作识别提供有效解决方案，通过显式层级双曲嵌入和LLM缓存适配突破了数据稀缺下的表示学习瓶颈。
...
TL;DR: TurtleAI是一个面向教育领域的视觉编程基准数据集，包含823个海龟图形（ Turtle Graphics）任务，用于评估视觉语言模型生成代码的能力。评估20+个主流VLM后发现大多数模型准确率低于30%，通过提出的数据生成技术对Qwen2-VL-72B进行微调可提升约20%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程研究多聚焦于生产力应用，缺乏对教育-oriented视觉编程能力的探索。研究旨在填补这一空白，评估当前VLM在教育领域的表现及其局限性。

Method: 构建TurtleAI基准数据集（823个真实世界的Turtle Graphics任务），评估GPT-5、GPT-4o、Qwen2-VL-72B等20+个VLM，提出仅需少量种子样本的数据生成技术用于微调。

Result: 大多数VLM表现不佳，成功率低于30%；微调Qwen2-VL-72B后提升约20%。失败分析显示GPT-4o在空间推理和精确视觉复现方面存在困难。

Conclusion: 空间推理和精确视觉复现是限制当前VLM教育视觉编程能力的主要因素；微调主要改善了视觉推理与代码实现之间的对齐。
...
TL;DR: VidMsg是一个评估短视频中隐含信息理解能力的基准测试，包含400个YouTube视频片段，涵盖9个主题领域和52个细粒度目标消息。基准采用"消息优先"pipeline构建，通过LLM生成间接搜索场景来检索候选视频，再由人工标注筛选出能传达目标信息但不过于明确的片段。


<details>
  <summary>Details</summary>
Motivation: 理解短视频不仅需要识别可见的物体和动作，视频创作者通常会在片段中加入潜在的意图或目的。现有视觉语言模型在处理这类需要务实推理、整合语境线索的任务时表现不佳，因此需要一个新的benchmark来评估模型的隐含信息理解能力。

Method: 采用消息-first pipeline：(1)使用LLM将目标消息转换为间接搜索场景；(2)利用这些场景检索候选视频片段；(3)由人工标注者保留能传达意图信息但不过于直白的片段。同时设计了双向消息-视频检索任务和多选QA诊断基准。

Result: 当代视频语言和检索模型在VidMsg上经常失败，因为该任务需要务实的推理、整合语境线索以及区分语义相近的消息。VidVec-Msg基线方法有一定提升，但仍存在较大改进空间。

Conclusion: 隐含消息理解是短视频理解的重要组成部分，当前模型在需要 pragmatics inference 和语义区分的任务上仍有显著不足。VidMsg为相关研究和应用提供了有价值的评估基准和挑战。
...
TL;DR: 本文提出图正则化非负简化双四元数矩阵分解(GNRBMF)模型用于颜色图像识别。该模型在NRBMF基础上引入图拉普拉斯正则化，使原始空间中相近的样本在特征空间中也具有相似的表示，同时保留了简化双四元数域中的非负性保持特性。


<details>
  <summary>Details</summary>
Motivation: NRBMF主要关注重建准确性，未能利用图像数据的局部几何结构，可能限制学习到的低维特征的区分能力。需要一种能在保持非负性约束的同时，利用数据局部几何结构的 factorization 方法。

Method: 将图拉普拉斯正则化项整合到简化双四元数系数矩阵中，推导了逐分量交替投影梯度算法来求解优化问题，并分析了其收敛性质。

Result: 实验结果表明，提出的GNRBMF模型在颜色图像识别任务中达到或超过了现有方法的识别性能。

Conclusion: 通过引入图正则化机制，GNRBMF成功将数据的局部几何结构信息融入分解过程，提高了特征表示的判别能力，在颜色图像识别方面具有有效性和优势。
...
TL;DR: 提出一种多假设协作深度展开压缩感知网络(MHC-DUN),通过联合优化多个解空间来解决传统CS方法的ill-posed问题,在测量保真度、假设多样性和重建精度之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有CS深度展开网络局限于单一解空间,忽视了CS问题的本质ill-posed特性(即允许多个合理解)。为此提出多假设协作框架来显式建模和利用多个候选假设。

Method: 基于近端梯度下降算法设计MHC-DUN:1)AlphaNet预测空间变化的步大小,实现跨多解的协作梯度更新;2)多假设协作近端映射模块利用假设内和假设间相关性先验联合细化各解;3)设计复合损失函数平衡测量保真度、假设多样性和重建精度。

Result: 实验表明该方法优于现有CS网络。

Conclusion: 提出的MHC-DUN能有效探索互补解同时保持重建精度,为CS问题提供新的多假设协作解决思路。
...
TL;DR: 本研究提出了一种针对 hyperspectral satellite imagery 的快速甲烷泄漏检测方法。通过开发 Mag1c-SAS 算法（比原版快约80倍）结合 U-Net/LinkNet 机器学习模型，并在新引入的 EMIT-MSeg 数据集和 STARCOP 数据集上进行验证，实现了明显的性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统甲烷检测方法计算量大，难以在资源受限的卫星硬件上运行；同时现有 hy spektral 任务多为手动targeted，可能遗漏潜在泄漏事件。需要开发低功耗、高效的检测算法来实现 onboard 检测。

Method: 测试了快速目标检测 ACE 和 CEM 方法，提出 Mag1c-SAS 算法变体；集成 U-Net 和 LinkNet 机器学习模型；引入并开源 EMIT-MSeg 数据集及高质量标注策略；评估两种新颖的 band selection 策略；通过硬件 profiling 验证 onboard 可行性。

Result: Mag1c-SAS 比原版 Mag1c 快约80倍；在 EMIT-MSeg 上 AUPRC 提升超过30个百分点；在 STARCOP 上 F1 分数提升约4个百分点；系统功耗低，CPU/RAM 利用率高，满足 onboard 部署要求。

Conclusion: 成功开发了高效、低功耗的甲烷检测系统，通过 Mag1c-SAS + LinkNet 组合实现了速度与精度的平衡，具有显著的 onboard 应用价值。
...
TL;DR: 多模态大型语言模型(MLLM)在视觉对抗攻击面前存在脆弱性，研究通过诊断协议和端到端训练来提升其对抗鲁棒性，发现大规模多模态对抗预训练是关键，可显著提升在Captioning和VQA任务上的对抗性能。


<details>
  <summary>Details</summary>
Motivation: 现有防御通过约束对齐CLIP嵌入空间来保持兼容性，但这限制了可实现的鲁棒性；需要系统研究MLLM的对抗鲁棒性问题并探索更有效的防御策略。

Method: 提出诊断CLIP对齐协议预测哪些鲁棒视觉编码器能有效迁移到多模态设置；采用端到端多模态对抗预训练；测试时视觉随机变换等方法。

Result: 集成鲁棒编码器后相比即插即用基线获得28 CIDEr分数(Captioning)和11.7% VQA准确率提升；从鲁棒骨干进行端到端对抗训练额外获得1.9 CIDEr和4.3% VQA提升；测试时变换可将非鲁棒模型的对抗性能从接近零提升到与鲁棒模型相当；且显著减少白盒视觉越狱攻击下的毒性生成。

Conclusion: 鲁棒视觉表示是对抗训练成功的严格前提条件；大规模多模态对抗预训练比单模态规模更重要；端到端多模态训练结合多种防御策略是提升MLLM对抗鲁棒性的最有效途径。
...
TL;DR: 论文研究文本到图像模型如何使用文本嵌入信息，发现模型实际上只需要两个简单特征：多token合并为单词表示和词序，而不需要丰富的上下文信息。


<details>
  <summary>Details</summary>
Motivation: 探究文本到图像模型是否利用了文本嵌入中的丰富上下文信息，如组合性和属性绑定等。

Method: 构建了一种仅编码个别词义和词序但缺乏完整提示上下文信息的"位置标记词袋"表示新文本嵌入。

Result: 这种简化的表示足以成功引导图像生成，在视觉质量和文本保真度方面与完整文本嵌入引导的生成相当。

Conclusion: 文本到图像模型通常不使用文本嵌入中除词义和词序之外的丰富信息，复杂的语言结构解码由图像模型本身完成。
...
TL;DR: 本文发现CLIP在弱噪声下对对抗样本存在"虚假稳定性"现象，而在强噪声下对抗表征反而变得更不稳定。基于此提出一种无需训练的即插即用漂移门控机制，可触发现有测试时防御措施，从而改善CLIP的清洁-鲁棒准确率权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型如CLIP虽具有零样本泛化能力，但易受对抗攻击。对抗训练计算昂贵，现有的随机测试时防御虽然改善鲁棒性但往往损害清洁准确率，形成不利的清洁-鲁棒权衡问题。

Method: 作者分析CLIP表征空间在不同噪声强度下的行为，提出利用高噪声特征漂移作为门控信号，检测到对抗式不稳定时才触发现有测试时防御的即插即用机制。

Result: 该方法在13个数据集上均改善了清洁-鲁棒权衡。8个细粒度数据集上，反击防御从65.7%提升至71.4%，噪声锚定从68.4%提升至73.2%；ImageNet及其4个变体上，分别从56.1%升至66.2%和62.1%升至67.6%。

Conclusion: 弱噪声下对抗表征表现出虚假稳定性，而强噪声下变得比清洁表征更不稳定，这种噪声区域转换不一致性是非鲁棒CLIP的独特几何特性，可被利用来设计无需训练的轻量级防御机制。
...
TL;DR: 本文重新审视少步蒸馏的训练配方而非蒸馏目标，使用Qwen-Image-2.0研究数据组成、教师指导和任务混合三个因素，提出Qwen-Image-Flash模型。


<details>
  <summary>Details</summary>
Motivation: 现有少步蒸馏工作主要关注蒸馏目标设计，而忽视了训练配方对学生模型性能的关键影响。需要从训练管道角度系统优化蒸馏效果。

Method: 在统一文生图和指令引导图像编辑任务上，系统研究数据组成、教师指导和任务混合三个因素对学生蒸馏性能的影响。

Result: 发现多个非直观行为，表明有效的少步蒸馏不仅需要精心设计的蒸馏目标，还需原则性的训练管道组织，最终开发出Qwen-Image-Flash。

Conclusion: 少步蒸馏的成功取决于整体训练流程的系统优化，而非单一目标的改进。这为视觉生成模型的加速蒸馏提供了新的研究方向。
...
TL;DR: YOLO26是Ultralytics推出的新一代实时视觉模型家族，通过双头设计实现端到端NMS自由推理，移除DFL以减轻检测头，并采用MuSGD优化器、Progressive Loss训练策略和STAL标签分配方法解决小物体漏检问题。模型覆盖5个规模(n/s/m/l/x)，支持检测、分割、姿态估计、定向检测和分类等多种任务，在COCO上达到40.9-57.5 mAP，延迟仅1.7-11.8ms。


<details>
  <summary>Details</summary>
Motivation: 现有YOLO检测器存在四大核心痛点：1)推理依赖非极大值抑制(NMS)；2)Distribution Focal Loss导致检测头繁重；3)需要长训练周期；4)小物体容易缺乏正样本标签。因此需要构建更高效、准确、易部署的统一实时视觉模型。

Method: 架构创新：(1)双头Design实现原生NMS-free端到端推理；(2)移除DFL，使用无约束回归范围，大幅减轻检测头。训练创新：(1)MuSGD混合优化器，源自大语言模型训练；(2)Progressive Loss将监督逐步转移至推理头；(3)STAL标签分配保证小物体正样本覆盖。任务扩展：针对实例分割、姿态估计、定向目标检测设计专用头和损失函数。支持开放词汇扩展YOLOE-26，支持文本/视觉/提示自由推理。

Result: 五个规模在COCO上达成40.9-57.5 mAP，T4 TensorRT延迟1.7-11.8ms，精度-延迟Pareto前沿超越前代实时检测器。YOLOE-26x在LVIS minival文本提示下达40.6 AP。一套pipeline支持检测/分割/姿态/分类/定向检测五种任务。

Conclusion: YOLO26通过协调的架构与训练创新，系统性解决了YOLO系列的历史痛点，在精度、效率、部署简洁性上全面突破，构建了统一的实时视觉基座模型，为开源社区提供了高性能、可扩展的视觉基础方案。
...
TL;DR: 本研究探索了仅使用被动红外相机在自然阳光下进行眼睛跟踪的可行性，并构建了大规模数据集AmbientEye来评估这一场景的性能。


<details>
  <summary>Details</summary>
Motivation: 现有眼动追踪系统依赖主动红外照明，限制了智能眼镜在户外长时间使用的便携性。研究旨在验证无主动光源时，仅凭环境阳光是否能实现可靠的瞳孔检测。

Method: 构建包含260万+眼图的大规模数据集AmbientEye，采集自19个国家35名参与者，使用两种离轴相机配置和两种太阳 orientation条件。采用SAM2自动分割后经人工修正进行高精度标注，并在该数据集上基准测试先进瞳孔分割算法。

Result: 在受控红外照明数据集上性能达0.928，但在AmbientEye数据集上骤降至0.767，性能差距显著，凸显环境光设置的挑战性。

Conclusion: AmbientEye成为首个针对这一实际户外眼动追踪场景的基准数据集，为无需主动照明的被动式眼动追踪研究开辟了新方向。
...
TL;DR: 将手语翻译(SLT)评估从表面指标(BLEU/ROUGE)转向语义理解(SLU)，提出SLU-2K数据集(2350个问答对)评估系统对动作、人物、物体等语义元素的理解能力，发现当前多模态大语言模型表现接近随机，最先进翻译系统存在显著语义差距。


<details>
  <summary>Details</summary>
Motivation: 现有手语翻译评估仅关注词汇重叠的表面指标，无法衡量翻译是否真正保留源语句意义，这与辅助技术集成的最终目标不符，亟需转向语义理解评估。

Method: 基于PHOENIX-2014T和CSL-Daily数据集构建包含7类问题的SLU-2K数据集，自动生成2350个封闭域视频问答对，评估多模态大语言模型及MMSTL、SpaMo等先进翻译系统。

Result: 多模态大语言模型表现接近随机；最先进翻译系统在仔细微调后语义准确率仅为56.7%至75.2%，存在显著语义差距。

Conclusion: 当前SLT评估协议高估了真实理解能力，未来进展应同时衡量语义正确性而非仅关注流利度和n-gram重叠，手语理解研究需从翻译转向语义理解评估。
...
TL;DR: 本文提出了一种简单而有效的多概念定制方法，通过最优组合多个LoRA模块的输出来解决文本到图像生成中的多概念个性化问题。


<details>
  <summary>Details</summary>
Motivation: LoRA虽然能够实现单概念的文本到图像生成个性化，但扩展到多概念定制时存在挑战。简单合并多个LoRA权重或输出会导致概念间干扰，视觉质量下降，对参考图像的保真度降低。

Method: 提出两种基于提示感知重要性加权的方法：W-Switch和W-Composite。每个LoRA根据其触发词在目标提示中的语义影响进行加权。同时提出了一个新的基于图像相似度的评估框架，通过比较真实参考图像与生成图像中自动分割的概念区域来评估图像保真度和身份保留。

Result: 在ComposLoRA测试基准上的一致性改进，视觉质量、身份保留和组合性方面均优于现有最先进方法。定性和定量评估（包括LLM评估和用户研究）进一步验证了所提方法的有效性。

Conclusion: 通过提示感知的LoRA加权组合策略，成功实现了高质量的多概念定制生成，为多概念个性化图像生成提供了一种简单有效的解决方案。
...
TL;DR: 研究通过空间频率可访问性分析视觉-语言模型中学到的投影层如何改变视觉信息结构，发现中间层达到频率可访问性峰值，最终变换因架构而异（CLIP为中性，DINOv2[CLS]池化导致结构化损失）。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型通过学习的投影层将视觉特征映射到共享嵌入空间，但这些转换如何改变视觉信息结构尚不清楚，需要理解模型对视觉信息的transform。

Method: 引入残差光谱损失(RSL)评估相对于维度匹配随机投影基线的变化；使用预训练模型（参数冻结）排除优化混淆；在ImageNet和MS-COCO数据集上分析CLIP和DINOv2的表征。

Result: 光谱可访问性随深度呈非单调轨迹，在中间层达到峰值后向输出表征递减；最终变换：CLIP的学习投影是光谱中性（变化由压缩解释），而DINOv2的[CLS]池化在光谱上引起结构化损失。

Conclusion: 中间层和池化机制是现代视觉编码器中光谱变换的主要驱动因素；研究揭示了架构差异对表征 transformative 的关键影响。
...
TL;DR: 研究consumer camera远程光电容积脉搏波(rPPG)能否从单周期波形中提取个体动脉僵硬度的形态学特征。评估了16种架构在153个受试者上的表现，发现没有任何架构能恢复个体特异性的形态学结构。SupCon达到log N=4.844，是测试中最强的编码器，但VAE解码器只能恢复群体水平的谐波内容，存在"幻觉"现象。


<details>
  <summary>Details</summary>
Motivation: Consumer face camera remote photoplethysmography (rPPG) enable passive cardiovascular monitoring, but whether single-cycle waveform morphology encoding arterial stiffness biomarkers is recoverable from this measurement has not been characterized. This study aims to determine if individual arterial morphology can be encoded in consumer camera readings.

Method: Evaluated 16 architectures spanning six families on 153 subjects across three datasets, introducing cross-subject Pearson r to distinguish subject-specific recovery from template collapse.

Result: No architecture recovered subject-specific morphology (cross-subject r range 0.773-0.9999; ground-truth ceiling 0.601). Supervised Contrastive (SupCon) converged to log N = 4.844, constituting the strongest available empirical evidence that no discriminative morphological structure is extractable from single-cycle rPPG. The VAE decoder restores population-level harmonic content absent from the rPPG input (H2/H1: 0.310 output vs. 0.275 input), generalising zero-shot to UBFC (r = +0.708); a directional hallucination gap (p = 0.150) suggests partial signal reading. Anti-collapse objectives fail when input carries no discriminative structure.

Conclusion: Consumer cameras cannot encode individual arterial morphology; cross-subject r is a necessary collapse diagnostic for waveform reconstruction benchmarks. Individual-specific arterial stiffness biomarkers are NOT recoverable from single-cycle rPPG measurements using current encoder architectures.
...
TL;DR: TeX-1500是一个新发布的大规模配对LWIR HSI-TeX数据集和基准，用于长波红外高光谱图像的监督学习式温度-发射率-纹理分解。


<details>
  <summary>Details</summary>
Motivation: 现有TeX管道主要是场景特定的逆求解器，缺乏成对的LWIR HSI-TeX监督数据，限制了基于学习的分解方法发展。

Method: 构建了包含1522组校准真实场景配对的数据集，包含5个地点、4个季节、不同采集时间、异构波长布局和两个传感器系列；提出TeX-UNet作为简单的波长感知基线方法。

Result: TeX-1500提供了可用的配对监督，为以物理属性为中心的热感知提供了可测量的基准。

Conclusion: 该数据集和基准能够支持数据驱动的物理属性热感知研究。
...
TL;DR: 提出了4D F-MeshLDM框架，一个用于生成虚拟3D+t心脏 mesh序列的条件生成模型，结合卷积mesh VAE、傅里叶级数运动参数化和扩散先验，可实现可控的心脏解剖Morphing生成。


<details>
  <summary>Details</summary>
Motivation: 心血管领域的虚拟解剖通常表示为3D+t网格，但现有网格生成器多聚焦于静态解剖，序列模型又缺乏明确的周期性表达，因此需要一种能生成周期一致性解剖轨迹的虚拟人群生成方法。

Method: 构建了包含三个核心组件的条件生成框架：(1)卷积网格VAE用于编码网格；(2)结构潜在空间使用截断傅里叶级数参数化运动；(3)扩散先验学习傅里叶系数token的潜在分布。通过仿射调制对临床协变量进行条件控制，实现可控合成。采样token并进行逆傅里叶合成产生周期一致的潜在轨迹，再解码为3D+t心脏网格序列。

Result: 在5000名UK Biobank受试者上的实验表明：4D F-MeshLDM在解剖保真度上优于最先进的基线方法，达到近零的cycle闭合误差，生成的队列准确保留了临床功能指标。

Conclusion: 该框架展现了用于可靠的心血管器械in-silico临床试验的潜力，为虚拟人群生成提供了有效的解决方案。
...
TL;DR: 本文系统研究了视频理解中模型适配策略，对比了参数高效微调(PEFT)和探测方法在外观聚焦、运动聚焦和空间密集设置下的表现，特别关注有限数据场景。


<details>
  <summary>Details</summary>
Motivation: 视频理解任务标注和计算成本高，PEFT和probing提供了参数高效的模型适配方案。现有工作主要关注图像预训练模型向视频的迁移，且将时间推理限制在单一组件，缺乏对时间上下文如何分配的 系统性研究。

Method: 在不同设置下（外观聚焦、运动聚焦、空间密集）系统比较PEFT和probing方法，重点评估有限数据场景下的表现，分析时间上下文在backbone、PEFT和probe中的分配方式。

Result: 发现了PEFT和probing在不同设置下的新见解，证明了时间上下文分配对有效视频适配的重要性。

Conclusion: 时间上下文的合理分配对于视频模型适配效果至关重要，需要在模型各组件中综合考虑时间信息的整合。
...
TL;DR: Donk是一个统一的视频-动作去噪模型,用于灵巧手操作。该模型通过保持视频先验的分布更宽泛(建模交互视频和可执行手部轨迹的联合空间),在语言、初始图像和初始手部状态的条件下,可以采样未来视频和双手机器人轨迹作为动作策略,且无需图像条件时也可从文本条件分布中采样配对的视频-动作 rollout。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将对齐的先验收窄为观察条件的动作策略分布。本文认为应该保持分布更宽泛,建模交互视频和可执行手部轨迹在多种 Conditioning  Regime 下的联合空间。

Method: 提出Donk:一个统一的视频-动作去噪模型,用于灵巧手。(1)以语言、初始图像和初始手部状态为条件,可采样未来视频和双手MANO轨迹作为动作策略;(2)无图像条件时,同一去噪架构可从文本条件分布中采样配对的视频-动作 rollout,将对齐的视频先验转化为数据引擎。

Result: 在同一统一训练配方下,Donk在动作、视频和纯文本生成评估中都取得提升:提高了灵巧轨迹精度、保持了较强的视频保真度、并产生了平滑的文本条件动作 rollout。

Conclusion: Donk展示了统一的视频-动作去噪方法在动作生成、视频生成和文本条件动作生成上的有效性和泛化能力,为灵巧手操作的视频基础模型对齐提供了新思路。
...
TL;DR: 研究视觉指令微调如何在预训练大语言模型(LLM)的层级结构中嵌入视觉特征，发现中间语义层是视觉-语言处理的核心。)


<details>
  <summary>Details</summary>
Motivation: 探索视觉特征如何嵌入LLM的层级抽象架构，以及指令微调在多模态融合中的作用机制。

Method: 通过探针分析和因果干预方法，对比不同视觉-语言架构，并分析视觉与文本表示的几何关系。

Result: 视觉特征被直接嵌入到中间语义层，绕过早期单模态处理层；中间层是多模态性能的关键；微调扩展并强化了现有的抽象阶段，使视觉特征与文本特征对齐。

Conclusion: 多模态融合是一个局部现象，由LLM内部抽象引擎的重用驱动；仅对中间层进行微调即可保持视觉中心基准的性能，同时减少训练时间。
...
TL;DR: DyaPlex是一种流式全双工语音-动作模型，专门用于双人交互场景。该模型能够同时感知和生成语音及身体动作，实现多模态实时交互。


<details>
  <summary>Details</summary>
Motivation: 解决现有模型无法实现真正全双工通信的问题，需要让智能体能够同时感知和生成语音与动作，以捕捉人类沟通的连续性和交互性。

Method: 采用双塔Transformer架构，保留冻结基础语音模型的零样本对话推理能力；引入统一双人标记交织机制；使用时间对齐的语音-动作RoPE引导跨注意力；结合4000小时的无缝交互数据集进行训练。

Result: 模型有效捕捉跨说话者依赖关系，在单人交互和双人交互基准测试中均达到最先进的性能水平。

Conclusion: 该研究成功实现了完全同步的多模态交互，为人机交互领域提供了新的技术方案。
...
TL;DR: Seg2Track++是一个将实例分割与SAM2结合的零样本多目标跟踪与分割(MOTS)框架，通过引入轨迹管理模块增强时间一致性，无需微调即可在KITTI MOTS数据集上实现改进的identity保留和减少假阳性传播。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要在动态环境中进行稳健的多目标跟踪与分割(MOTS)，以确保一致的对象身份和精确的掩码级分割。当前基础模型(如SAM2)在零样本泛化方面表现强劲，但其直接应用于MOTS时受到不可靠的轨迹关联和假阳性传播的限制。

Method: 提出Seg2Track++框架，集成了实例分割与SAM2以及新的轨迹管理模块。使用掩码质心距离(MCD)和置信度感知成本调制(CCM)进行轨迹关联，同时采用概率轨迹验证(PTV)利用伯努利滤波器验证轨迹存在并抑制幽灵轨迹。

Result: 在KITTI MOTS数据集上的实验结果表明，所提方法实现了更好的identity保留、减少的假阳性传播和稳健的轨迹管理，且无需进行微调。

Conclusion: Seg2Track++成功实现了具有增强时间一致性的零样本MOTS，为自动驾驶系统在动态环境中的可靠运行提供了有效的解决方案。
...
TL;DR: 提出MLP-Splatting方法，通过将场景表示为多个紧凑的MLP神经原语，实现可分解的3D场景渲染，在保持照片级真实感的新视角合成同时，支持交互式对象级编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D Gaussian Splatting和NeRF方法虽然能实现高质量的渲染，但难以将场景分解为单独的对象进行操作。需要额外的分割或分组算法才能进行对象级处理，这限制了场景理解和交互能力。

Method: 将每个神经原语建模为一个独立的紧凑MLP，具有局部空间支撑用于预测辐射率和不透明度。通过稀疏体积合成对射线-原语交互进行高效渲染。仅使用RGB监督训练原语，使原语自然对应场景区域或对象部件。结合可选的语义特征蒸馏可实现开放词汇场景交互和开放集即时分割。

Result: 相比SOTA方法，内存使用降低至1/15，加速渲染3倍。在实验中比语义3DGS方法表现更优。

Conclusion: MLP-Splatting成功实现了场景的神经原语分解，在照片级真实感渲染和用户可控编辑之间取得平衡，为3D场景交互提供了新的可能。
...
TL;DR: 本文通过系统性地重新训练和评估Cambrian-1基准测试套件中五个视觉编码器的所有31个子集，揭示了多编码器 LVLM 设计中的关键方法和设计原则。


<details>
  <summary>Details</summary>
Motivation: 随着基础模型向融合更多异构视觉流扩展，理解不同编码器在联合训练下的交互变得至关重要。然而，当前的大型视觉语言模型缺乏相应的分析工具，且参数高效的编码器配置在训练前难以确定。

Method: 作者在16个基准测试的Cambrian-1套件上，对五个常见视觉编码器的所有31个非空子集进行统一pipeline的重新训练和评估（约消耗20k GPU小时）。他们将每个编码器的贡献分解为两个维度：Capacity（编码器独立达到的分数）和Necessity（从完整池中移除时的分数下降）。

Result: 三个关键发现：1）从头重新训练每个子集揭示的编码器排名与在固定checkpoint上屏蔽编码器得到的排名不同，包括总体排名第一的编码器；2）Capacity和Necessity两个维度不可互换——配对两个最高Capacity编码器并非最优，而将高Capacity锚点编码器与自适应互补编码器配对可以达到完整五编码器模型的性能，再添加更多编码器仅带来边际增益；3）在固定参数计数下，每个编码器的预投影器有效秩解释了剩余分数变化。

Conclusion: Capacity-Necessity分解和预投影器秩分析，加上通过重新训练的全面评估，暴露了多编码器 LVLM 设计中的方法论差距，并提供了具体的解决方案设计原语。
...
TL;DR: CoralBay是一个自监督学习框架，通过扩展DINO，使用3D Swin骨干网络和多尺度特征自蒸馏，实现3D医学影像的预训练，解决2D预训练无法充分建模3D医学影像空间连续性和解剖结构的问题。


<details>
  <summary>Details</summary>
Motivation: 自然图像的2D自监督预训练无法有效迁移到CT等3D医学影像任务，因为3D医学影像具有空间连续性、器官解剖结构和组织密度特性（如Hounsfield单位），当前方法缺乏对这类体积数据的有效建模。

Method: 采用层次化3D Swin骨干网络，将多尺度特征进行拼接后应用自蒸馏机制，实现数据高效的自监督学习，无需人工标注即可学习包含全局语义和细粒度局部结构的丰富空间表示。

Result: CoralBay能够有效迁移到多种下游放射学任务，在不同解剖目标上均表现出色且一致的性能。同时构建了开源的3D放射学基准测试平台，统一多个数据集并建立了体积表示学习方法的标准化评估基准。

Conclusion: 该研究成功填补了2D自监督学习与3D医学影像之间的鸿沟，为体积医学影像的自监督预训练提供了有效的框架和数据支持，推动了医学影像领域无监督表示学习的发展。
...
TL;DR: OVO-S-Bench是一个用于评估流式空间智能的完全人工标注基准，包含1680个问题跨四个抽象级别。评估显示Gemini-3.1-Pro得分59.2，落后人类专家27分，allocentric mapping是最大瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么在完整视频上离线评估，要么针对事件而非空间结构。多模态智能体需从连续自我中心流中推理地点和布局，常需使用当前视图之外的证据。现有方法无法有效评估这种流式空间智能能力。

Method: 构建完全人工标注的benchmark，包含1680个问题跨越348个源视频，经过12名训练有素的注释者进行了约804人时的多轮质量保证。每问题包含查询时间戳和证据间隔，评估时模型仅看到查询前的视频前缀。问题涵盖四个递进抽象级别：即时自我中心感知、时空上下文跟踪、空间模拟与推理和异中心映射。对38个专属和开源MLLMs进行评估。

Result: Gemini-3.1-Pro落后人类专家27分（59.2 vs 86.6），allocentric mapping是最主要的瓶颈。流式和空间微调的MLLMs反而不如其原始骨干模型。chain-of-thought推理在未扎根于流时会放大空间错误。

Conclusion: OVO-S-Bench成为下一代流式空间多模态大语言模型的要求严格的测试平台，揭示了当前模型在空间智能尤其是allocentric mapping方面的显著局限性。
...
TL;DR: 本文提出了一种基于电磁追踪（EMT）的股骨截骨手术导航系统，通过术前CT规划结合术中C臂校准和仅2张透视图像实现X-CT配准，从而实现实时、无透视的电磁导航。该系统在18具合成股骨模型中验证，角度误差显著优于自由手方法，并与患者定制器械（PSI）达到统计等价。


<details>
  <summary>Details</summary>
Motivation: 目前股骨截骨术前计划的精确执行仍面临挑战。现有技术（自由手方法和患者定制器械PSI）分别需要超过30张和6张透视图像，且准确度、侵入性和辐射暴露均存在局限。

Method: 将CT术前计划与一次性C臂校准相结合，通过初始化时获取的2张透视图像进行准确的X射线到CT配准，实现实时、无透视的电磁追踪系统对锯片和骨块的导航，兼容单平面和双平面截骨。

Result: 在18具合成股骨的可行性研究中，EMT引导的总角度误差（3.05±0.75°）显著优于自由手执行（6.32±2.36°），p=0.031。EMT引导的试验无一超过5°临床阈值，而自由手组6次试验中出现4个异常值。该系统在总角度误差（p≤0.02）和总平移误差（p=0.048）上与PSI达到统计等价（±2°，±2mm），用户问卷评分无显著差异。

Conclusion: 该系统仅使用2张透视图像即可转移术前计划，同时达到PSI精度且无需额外手术暴露，为后续尸体研究和临床验证提供了依据。
...
TL;DR: 本文提出了一种针对扩散加权成像(DWI)的噪声感知注意力驱动去噪框架。该方法结合分层Swin Transformer窗口注意力和多维门控细化模块，利用显式噪声水平条件和残差重建来自适应抑制异方差噪声。实验表明该模型在1%至15%的噪声水平下可实现33.69 dB的PSNR和0.8539的SSIM，并在严重噪声条件下保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: DWI用于全身癌症筛查，但传统方法需较长采集时间。减少扫描时间会导致图像质量下降和噪声增加。幅度重建引入的信号依赖性Rician噪声使传统卷积去噪方法面临挑战，需要更有效的解决方案来处理这类特定噪声。

Method: 提出噪声感知注意力驱动去噪框架，整合分层Swin Transformer窗口注意力与基于Transformer的多维门控细化模块。模型包含显式噪声水平条件化机制和残差重建策略，实现对宽范围异方差噪声的自适应抑制。

Result: 在 corrupted DWI扫描上验证了强大的去噪性能。在1%至15%噪声水平下，平均PSNR达33.69 dB，SSIM达0.8539，在严重噪声条件下保持稳定行为。

Conclusion: 注意力引导的上下文建模结合通道自适应细化为DWI去噪提供了稳健且通用化的解决方案，能够有效处理医学影像中的Rician噪声。.
...
TL;DR: ByG是一种无需配对数据的图像/视频编辑模型训练框架，通过从冻结模型中提取指令-following线索并结合循环一致性来实现结构保持，无需外部奖励模型即可达到最先进的编辑效果。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型训练需要大量配对数据，这在图像编辑特别是视频编辑中成本高昂，限制了可扩展性。现有方法依赖大规模监督数据，难以应用于数据稀缺的场景。

Method: 提出Bootstrap Your Generator框架，利用基础模型的内在知识作为监督信号。具体包括：1)从冻结模型中提取instruction-following语义线索；2)使用cycle-consistency保持结构；3)通过梯度路由将下游损失从干净预测传递到噪声训练状态，弥合训练与推理的差距。

Result: 在数据稀缺的图像和视频编辑场景中达到最先进水平。广泛的评估和用户研究表明，该方法能有效泛化到未见领域，在百万级样本监督基线的基础上进一步提升性能。

Conclusion: 梯度路由技术弥合了训练与推理的差距，从基础模型提取语义线索提供了强大的训练信号，消除了对外部奖励模型的需求，为低成本、高质量的生成式编辑开辟了新路径。
...
TL;DR: PatchScene是一种基于扩散模型的大规模LiDAR场景补全框架，采用局部体素块扩散策略替代全局表示或密集体素网格，通过置信度引导的时空融合机制和利用激光雷达径向密度模式的环形 flow 扩散策略，实现高质量、空间无界的场景补全。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖全局潜在表示或密集体素网格，难以生成细粒度的局部几何信息，且在空间和时间尺度上难以保证一致性；同时无法实现无界场景补全（如从20m扩展到50m）。

Method: 提出三个核心创新：(1) 基于局部体素块的扩散范式，显式生成局部3D区域的几何细节；(2) 置信度引导的时空融合机制，整合重叠体素块和相邻帧；(3) Annular-Flow扩散策略，从近处到远处逐步传播高保真信息。

Result: 在SemanticKITTI基准数据集上达到最先进性能，跨越所有标准指标；在几何精度和时间一致性上均超越先前方法；在20m训练数据上无需重新训练即可泛化到50m场景。

Conclusion: PatchScene通过局部扩散范式和创新的时空融合机制成功实现了高质量、可扩展的场景补全，为自动驾驶应用提供了强大的 Scalability 和泛化能力。
...
TL;DR: VSTAT是一个用于评估多模态大语言模型(MLLMs)视频视觉状态追踪能力的基准测试，包含834个视频片段和1500个问题。研究发现，尽管现有模型在现有基准上表现良好，但在VSTAT上远低于人类水平，仅略高于随机猜测基线。失败原因不是推理能力不足，而是视觉感知层面的缺陷。


<details>
  <summary>Details</summary>
Motivation: 当前视频理解评估忽略了连续状态追踪这一核心能力。现有的MLLM视频基准测试主要关注单帧或短片段理解，无法诊断模型是否具有跨时间的实体、状态和事件追踪能力。这导致即使在现有基准上表现优秀的模型，在真实视频理解任务上仍然存在明显的能力缺陷。

Method: 该研究构建了VSTAT基准测试，包含合成视频和真实世界视频共834个片段，每个问题都需要整合整个视频流的信息才能回答，而非仅凭单一帧或短片段。通过对比分析MLLMs的思维轨迹与底层视频流，诊断失败原因。同时评估了最新的agentic方法（包括MLLM视频智能体和编码智能体）的表现。

Result: 1) 最先进的MLLMs在VSTAT上表现远低于人类，仅略高于答案先验基线；2) 失败原因是视觉感知层面的缺陷，而非推理能力不足——模型能够正确进行文本推理和状态追踪，但在视觉感知需要追踪的事件时出现失败；3) 现有的agentic方法（包括视频智能体和编码智能体）仍然无法解决这些失败问题。

Conclusion: 视觉状态追踪是视频理解的基础能力，但当前MLLMs在此方面存在根本性缺陷。未来工作需要着重提升模型的视觉感知能力，而不仅仅是推理能力_agentic方法也未能有效解决此问题，需要新的技术突破来实现真正的连续视觉理解。该基准测试为诊断MLLMs的视频理解能力提供了重要工具。
...
TL;DR: 本文研究视觉变换器(ViT)中特征绑定信息的学习问题，提出信息论形式的绑定问题形式化方法和探测方法，通过实验验证绑定是强视觉识别和推理的关键要素。


<details>
  <summary>Details</summary>
Motivation: 多对象场景理解需要解决绑定问题，即确定哪些特征属于同一对象。尽管现有研究表明ViT知道哪些补丁在一起，但尚不清楚深度学习模型是否学习了特征层面的绑定信息，且ViT常将特征错误归属到错误对象。

Method: 引入信息理论方法形式化绑定问题，设计探测方法来测量模型表示中的绑定信息。在不同绑定挑战的数据集上对多种预训练ViT进行实验，测量来自架构不同组件（如[CLS]标记或空间标记）的绑定信息。

Result: 实验表明绑定是关键 ingredients 到强视觉识别和推理，不同ViT架构组件表现不同，数据集涵盖特征共享、遮挡和自然特征等挑战。

Conclusion: 绑定作为关键要素对强视觉识别和推理至关重要，该工作为理解ViT及其他视觉模型的特征绑定机制提供了理论框架和方法工具。
...
### [115] [Adaptive Causal Alignment for High-Confidence Adversarial Training](https://arxiv.org/abs/2606.03925)
*Zhiming Luo,Kejia Zhang,Yingxin Lai,Junwei Wu,Juanjuan Weng,Shaozi Li*

Main category: cs.CV

TL;DR: 提出HICAT框架，通过可学习的背景偏差估计器和前景对数正交增强损失，解决对抗训练中因背景过拟合导致的鲁棒性下降问题。


<details>
  <summary>Details</summary>
Motivation: 视觉上下文作为双刃剑（支持性先验或虚假混淆因素），需要自适应诊断其效用并精准解耦。

Method: 提出Measure-Debias-Align管道：LBBE可学习背景偏差估计器诊断上下文效用，适应性修正logits，结合FLOE前景对数正交增强损失实现严格的特征解纠缠。

Result: 在CIFAR-10/100和ImageNet-1K上跨CNN和ViT架构一致提升基线性能，显著缩小鲁棒泛化差距。

Conclusion: 通过语义平衡和因果对齐实现了背景辅助与消除的有序切换，为可解释鲁棒学习提供了新范式。
...
### [116] [Demo2Tutorial: From Human Experience to Multimodal Software Tutorials](https://arxiv.org/abs/2606.03951)
*Zechen Bai,Zhiheng Chen,Yiqi Lin,Kevin Qinghong Lin,Difei Gao,Xiangwu Guo,Xin Wang,Mike Zheng Shou*

Main category: cs.CV

TL;DR: Demo2Tutorial是一个将人类数字操作经验（屏幕录制+交互日志）自动转换为结构化多模态软件教程的框架，可同时服务于人类学习和AI代理训练。


<details>
  <summary>Details</summary>
Motivation: 人类在数字环境中的操作经验包含丰富的程序性知识，但目前未被充分利用。现有的软件教程多为预编辑内容，缺乏灵活性和个性化，且难以被AI代理利用。需要一种方法从真实的人类交互中自动提取和结构化知识。

Method: 1)Recorder收集人类体验；2)多模态Action Parser解析原始经验，重建感知、行动和意图；3)Step Planner将步骤抽象为层次化任务图；4)Tutorial Composer将解析经验转化为结构化图像-文本指令

Result: 新benchmark评估表明：生成的教程质量超越人类-authored教程，显著优于基线方法；人类学习者任务完成速度更快；下游GUI代理规划能力和泛化性能明显提升。

Conclusion: 从人类真实经验中蒸馏的结构化教程是一种有效的知识表示，可同时促进人类学习和AI代理能力，代表了人机协作知识提取的新方向。
...
### [117] [VLESA: Vision-Language Embodied Safety Agent for Human Activity Monitoring](https://arxiv.org/abs/2606.03954)
*Hanjiang Hu,Yiyuan Pan,Jiaxing Li,Xusheng Luo,Alexander Robey,Na Li,Yebin Wang,Changliu Liu*

Main category: cs.CV

TL;DR: VLESA是一个视觉语言具身安全智能体，通过监控第一人称视角视频来预测危险动作并在必要时触发实时安全干预。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地协助人类完成物理任务，确保安全问题变得至关重要——物理动作具有即时且不可逆的后果，而相同的动作可能根据意图上下文是安全或危险的。

Method: 提出了三个核心组件：1）构建了带有目标条件安全标注的第一人称视角数据集；2）通过GRPO训练了目标条件安全Q过滤器，可在不重训练的情况下评估动作安全性；3）提出了意图-动作预测智能体，用于从视频中联合推断目标并预测未来动作。

Result: 在ASIMOV-2.0基准上，VLESA在精确的真实帧位置实现了更高的干预准确率，且通过目标条件约束解码，GRPO训练的Q过滤器将动作安全性提升了超过41个百分点。

Conclusion: 该工作为具身AI安全提供了新的框架思路，通过结合意图推断与安全过滤，在保持更高干预精度同时显著提升动作安全性。
...
TL;DR: Video-Mirai是一种训练方法，通过引入前向观察encoder和轻量级预测器，将未来帧监督信号蒸馏到因果状态表示中，从而关闭自回归视频生成中的"表示级规划差距"，使模型能够在保持原有架构和计算效率的前提下，提高长期 rollout 的主体和背景一致性。


<details>
  <summary>Details</summary>
Motivation: 问题背景：流式自回归视频扩散中，每个发出的片段成为未来片段必须保留的承诺，但标准训练只要求每个因果状态解释当前片段，这导致适合当前片段的状态可能丢弃了保持一致性未来所需的身份、布局和运动信息。核心贡献：提出"表示级规划差距"(representation-level planning gap)这一概念，并设计训练方法在不改变推理架构的情况下关闭该差距。

Method: Video-Mirai采用三阶段训练：(1)基础生成器按因果方式rollout；(2)冻结的前瞻encoder非因果读取完整rollout；(3)轻量级预测器将停止梯度的目标蒸馏到因果状态中。未来帧监督表示，从不监督generator输入。推理时丢弃encoder和预测器，保持原架构、单步FLOPs和KV-cache行为不变。

Result: 在VBench 5秒测试集上，总分从83.8提升至84.6；在超出训练范围的30秒rollout上，主体一致性从84.9提升至88.5，背景一致性从90.2提升至91.9。消融实验确认未来条件目标是关键成分；探针实验显示未来帧可从当前特征中更易解码。

Conclusion: 因果性应约束推理而非表示监督。研究表明视觉自回归模型需要前瞻能力。当前方法在保持原架构和计算效率的同时有效提升了长期视频一致性，为视频生成模型提供新训练范式。
...
TL;DR: AAD-1是一种用于单步自回归图像到视频生成的非对称对抗蒸馏框架，通过打破生成器和判别器之间的对称性设计，以及分阶段训练策略来解决运动崩溃和训练不稳定问题，在VBench上实现最先进的单步自回归视频生成性能。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的自回归图像到视频生成方法采用对抗蒸馏，但存在运动崩溃（生成静态视频）和训练不稳定的显著问题，导致模型无法产生动态连贯的视频内容。

Method: 提出两个关键设计：1) 非对称架构——保持生成器的因果结构以保留自回归采样能力，同时让判别器双向Attend全场域时空上下文并生成单一整体真实度评分，使判别器能够有效检测全局时间失败和长距离漂移；2) 分阶段训练策略——先使用分布匹配 bootstrapping 稳定单步生成器作为预热阶段，再进行对抗蒸馏。

Result: 在VBench上的广泛实验表明，AAD-1在单步自回归视频生成任务中达到最先进的性能。

Conclusion: AAD-1通过非对称设计和分阶段训练策略有效解决了自回归生成中的运动崩溃和训练不稳定问题，为单步自回归视频生成提供了新的有效方案。
...
TL;DR: 本文介绍了NewtPhys，一个基于真实场景多视图图像构建的4D物理标注数据集，用于评估视觉语言模型和视觉foundation模型的低层次牛顿物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 先前的工作使用合成或半合成场景进行物理推理评估，缺乏真实世界的视觉保真度，无法评估真正的低层次牛顿物理理解能力。

Method: 构建了包含密集细粒度标注的4D物理数据集（涵盖3D力、模态前景、语义和几何信息），系统评估了56个VLM（含54个开源和2个闭源模型）及10个VFM。

Result: 揭示了当前模型在低层次物理推理方面的局限性。

Conclusion: NewtPhys数据集为未来物理 grounding 的视觉研究和下一代物理感知评估奠定了基础。
...
TL;DR: 提出一种在焦平面传感器处理器上实现的全并行视觉里程计和深度估计方法，通过高斯置信传播(GBP)在像素间交换信息，结合关键帧锚定机制实现相机运动共识和深度推断。


<details>
  <summary>Details</summary>
Motivation: 传统图像处理将原始像素数据传输到后端计算效率低下，存在冗余和噪声数据传输问题，需要转向在焦平面传感器处理器上进行像素级分布式计算，减少下游负载。

Method: 提出完全并行化的视觉里程计和深度估计方法：传感器处理器通过高斯置信传播(GBP)在像素间交换信息，从每像素光度观测和表面法线先验达成相机运动共识并推断深度；引入类似关键帧的锚定机制调节帧间有效基线，保持几何稳定性。

Result: 在现实数据集上验证了基于GBP的像素级分布式里程计和深度估计以及传感器上关键帧锚定的可行性。

Conclusion: 该方法实现了在传感器上直接进行分布式视觉里程计和深度估计，为高效嵌入式视觉处理提供了新途径。
...
TL;DR: 本文提出了一种无需复杂网络架构重新设计即可提升LiDAR语义场景补全(SSC)性能的"免费午餐"策略。通过为输入点云添加语义伪标签和可见性信息两个简单增强，使旧模型也能达到甚至超越当前最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR语义场景补全方法需要复杂的网络架构重新设计才能提升性能，本文旨在探索是否存在简单有效的改进策略，在不修改核心架构的情况下显著提升模型性能。

Method: 提出了两种简单的输入增强策略：1)使用现成分割器的语义伪标签作为输入先验；2)为激光雷达扫描添加可见性信息以区分空区域和未知区域。通过oracle实验验证高质量语义先验是mIoU提升的主要驱动力。

Result: 这两种简单增强在多个测试架构上均带来显著性能提升。旧模型在加入这些增强后，其性能可与最先进的系统相媲美，甚至超越它们。

Conclusion: 语义先验和可见性信息是提升LiDAR语义场景补全性能的关键因素。这表明通过简单的输入增强即可在不改变模型架构的情况下获得显著收益，为该领域提供了实用且高效的改进方向。
...
TL;DR: 本文提出SimuScene，一种将物理引擎集成到3D场景重建流程中的新方法，解决单图像重建中物体穿插、悬空和下沉导致的物理仿真实崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 单图像3D重建是机器人操作的关键瓶颈，现有单图像提升器虽能恢复合理的单个物体形状，但组合成的场景在物理仿真下会崩溃，因为存在物体间穿插、悬空或下沉等问题。现有物理感知方法仅作为后处理布局修正，未解决根本的几何误差。

Method: 提出SimuScene组合3D重建管道，将物理置于形状和布局估计的循环中。核心创新是将物理引擎作为诊断测量工具，而非仅用于布局清理。通过对重建对象进行诊断性仿真（施加重力），将穿透和支持失败转化为定量校正信号，驱动重力轴拉伸和非模态形状重采样。

Result: 在物理稳定性和几何对齐基准上实现了最先进的性能，并展示了在人形机器人控制和机械臂操作任务中的实用性。

Conclusion: 物理信息反馈循环能够减轻累积重建误差，产生稳定且仿真就绪的组合3D场景，为机器人操作提供了可靠的3D环境重建方案。
...
### [124] [The Epi-LLM Framework: probing LLM behavioral priors through epidemiological agent-based models](https://arxiv.org/abs/2606.02867)
*Petra Ferenz,Ava Keeling,Tobias O'Keefe,Lorenzo Stigliano,Francesco Di Lauro,Andres Colubri,Jasmina Panovska-Griffiths*

Main category: cs.MA

TL;DR: Epi-LLM框架将基于智能体的建模、大型语言模型与真实流行病情境结合，构建合成社会来模拟疫情中的人类行为决策，并验证其与人类参与者行为的一致性。


<details>
  <summary>Details</summary>
Motivation: 流行病期间的人类行为影响疾病动态，但量化这一影响仍具挑战性，需要可扩展、低风险的模拟环境用于疫情 Preparedness研究。

Method: 整合ABM、真实epigame与LLM，在15天暴发接触网络上让合成智能体动态推理适应；对比无干预SEIR基线与人类AUIB数据，使用二项GLM分析行为预测因子。

Result: 四个LLM架构均降低峰值感染，隔离合规率在第6天达58-65%；感知健康严重度是最强预测因子(β=0.33, p=0.002)，pseudo-R²=0.055vs人类0.072；架构方差影响 epidemic dynamics。

Conclusion: Epi-LLLM作为疫情Prepareness模拟环境可行，LLM架构是关键差异因素，需显式态度参数化而非仅地理标签才能诱导文化差异化行为。
...
### [125] [Capability Advertisement as a Market for Lemons: A Trust Layer for Heterogeneous Agent Networks](https://arxiv.org/abs/2606.03034)
*Gaurav Naresh Mittal*

Main category: cs.MA

TL;DR: 研究大型语言模型Agent之间相互委托工作时面临的信任问题。现有协议(MCP、A2A)假设能力声明是静态的真实信息，但实际Agent的能力是概率性的、会随输入变化、模型更新而漂移，且可能自信地描述自己但实际上做不到。论文将这种"市场柠檬"问题建模，并提出Trust Layer解决方案。


<details>
  <summary>Details</summary>
Motivation: 当前Agent协议假设广告能力是静态的、真实的，但真实Agent的能力是概率性的、会因底层模型更新而变化，且可以完全自信地描述自己做不了的事。调用者只能看到声称的能力，无法区分可靠提供者与虚假夸大者。这导致市场陷入低信任均衡，好坏Provider无法区分，诚实可靠性得不到回报。

Method: 提出四个贡献：(1)失败分类学——将"自信错误"定义为非对抗性、相关性的Byzantine故障子类；(2)市场柠檬模型——证明基于信任的协议只支持低信任均衡；(3)Trust Layer——在MCP/A2A之上的协议无关层，添加概率性能力描述、筛选和声誉机制，支持分离均衡；(4)委托链的可靠性组合界限。设计无需模型重训练，在信任锚缺失或损坏时优雅降级。

Result: 当维持夸大声明的成本超过收益时，Trust Layer能够实现分离均衡，使可靠Provider与不可靠Provider可以被区分。该设计在协议之上添加了一个薄的"腰部"层，不依赖具体模型实现。

Conclusion: 通过经济学 Remedies(信号传递、筛选、声誉)解决LLM Agent市场的柠檬问题，提出无需模型重训练的Trust Layer设计，为多Agent系统信任问题提供原则性解决方案。
...
### [126] [OpenAgenet/OAN: Open Infrastructure for Trusted Agent Interconnection](https://arxiv.org/abs/2606.03161)
*Jinliang Xu*

Main category: cs.MA

TL;DR: OpenAgenet（OAN）是一个开放的信任层基础设施项目，旨在实现跨多运营商网络的可信Agent互联。它解决了Agent从孤立应用迁移到开放网络中面临的身份验证、授权和信任证明问题。


<details>
  <summary>Details</summary>
Motivation: 当Agent从孤立应用进入开放的多运营网络时，需要一种方式来验证身份来源、治理状态、发现授权、最新性和连接前的信任证据。现有协议无法满足这些需求，因此需要建立一个独立于交互协议、工具协议和编排框架的信任层。

Method: OAN被设计为一个协议中立的信任层，提供：Root治理的身份准入、Registrar辅助的入职引导、Root验证的包发布、授权感知的发现机制以及签名的可信调用。同时支持与MCP、A2A、ANP协议的集成，并提供区块链支持的授权公告系统。

Result: 已完成原型开发，具备Root-governed身份管理、Registrar辅助入职、授权感知发现等功能，性能测试表明其可用性，部署模式涵盖多种场景。

Conclusion: OAN为开放Agent网络提供了必要的安全基础设施，是连接可信Agent的关键中间件。其协议中立性和治理模型的结合为未来的多运营Agent网络奠定了基础。
...
### [127] [OpenAgenet/OAN: Technical Architecture for Trust-Governed Agent Identity and Discovery](https://arxiv.org/abs/2606.03163)
*Jinliang Xu*

Main category: cs.MA

TL;DR: OAN（开放代理网络）是一个协议中立的信任层，旨在实现开放代理之间的互联互通。该架构定义了角色结构、身份对象、注册工作流程、根级治理的生命周期、根级验证的包模型、授权感知的发现、可信调用、验证要求、状态转换、安全属性、实施边界和部署考虑等核心技术规范，支持异构代理框架和交互协议。


<details>
  <summary>Details</summary>
Motivation: 当前异构代理框架（MCP、A2A、ANP等）和协议缺乏统一的信任机制，导致代理身份验证、发现和安全接入存在障碍。研究旨在提供一个协议无关的信任基础设施，使代理在协议特定交互之前即可实现身份准入、发现、验证和安全接入。

Method: 采用系统架构设计方法，定义完整的信任层技术规范：包括角色架构建模、身份对象定义、注册工作流设计、根治理生命周期管理、包模型验证、授权感知发现机制、可信调用协议、状态机转换、安全属性形式化、明确实施边界和部署考量。

Result: 设计并规范了一个完整的协议中立信任层架构，能够支持多种异构代理框架和交互协议，实现代理身份的准入、发现、验证和安全接入，为开放代理互联提供通用可信基础设施。

Conclusion: OAN作为协议中立的信任层，解决了异构代理系统间的互信问题，通过定义身份管理、发现、验证和调用规范，使代理在业务对话前即可建立安全可信的交互基础，但不干涉具体的业务对话内容。
...
### [128] [MeDxAgent: Multi-Agent Consultation for Interactive Medical Diagnosis](https://arxiv.org/abs/2606.03416)
*Akshat Sanghvi,Naren Akash,Raza Imam,Amit Sharma,Mohit Jain*

Main category: cs.MA

TL;DR: 本文提出MeDxBench基准（包含4421个临床病例）和MeDxAgent多智能体诊疗系统，通过交互式诊断弥补现有LLM评估中单次问答与实际临床动态诊断的差距。


<details>
  <summary>Details</summary>
Motivation: 当前LLM健康决策支持评估存在严重问题：将诊断视为一次性任务，由用户预先提供完整信息并选择答案。这与实际临床实践不符，因为真实诊断是交互式的、开放性的，需要通过针对性提问逐步验证和修正假设。

Method: 构建包含20个专科4421个临床病例的大规模基准MeDxBench；设计多智能体诊疗系统MeDxAgent，系统研究其提示设计、工作流和智能体层面的设计选择，比较不同配置的效果差异。

Result: MeDxAgent在MeDxBench上相比基线提升10.3%准确率，缩小了52.3%的全信息神谕（oracle）差距。关键设计包括：先收集人口统计学信息、传递汇总对话用于诊断、输入候选诊断用于针对性提问，这些改进仅在组合使用时才能充分发挥效果。

Conclusion: 交互式诊断策略有效，但特定设计需组合使用才能最大化收益。该研究为LLM在临床诊断中的应用提供了更贴合实际的评估框架和优化方向。
...
### [129] [D2MDT: Department-aware Multidisciplinary Team Consultation with Deliberation for Efficient Clinical Prediction](https://arxiv.org/abs/2606.03543)
*Yongqi Liang,Qidong Liu,Chunze Yang,Lei Wu,Jiusong Ge,Ni Zhang,Chen Li*

Main category: cs.MA

TL;DR: D2MDT是一个新型的部门感知多学科团队咨询系统,通过多智能体协作和残留推理提高临床预测效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决现有电子健康记录(EHR)预测方法的问题:基于相关性的深度模型缺乏可解释性,单一大型语言模型缺乏多学科推理支持,且当前的多智能体系统存在证据区分不足和冗余多轮交互的问题。

Method: 1)构建结构化EHR证据和咨询就绪的语义证据;2)为医生智能体分配患者特定科室视角并检索补充证据进行协作咨询;3)引入残留推理,仅更新未解决的共识而非重播完整讨论历史;4)将精细化的共识报告与结构化EHR表示融合进行预测。

Result: 在死亡率预测任务上的实验表明,D2MDT提高了预测性能和咨询效率。

Conclusion: D2MDT通过部门感知的多学科团队协商机制和残留推理策略,为临床预测提供了一种更高效、更准确的多智能体协作方法,为多学科临床决策支持系统的发展提供了新的思路。
...
### [130] [Visual Graph Scaffolds for Structural Reasoning in Large Language Models](https://arxiv.org/abs/2606.02673)
*Runlin Lei,Xiaokui Xiao,Zhewei Wei*

Main category: cs.AI

TL;DR: 本文研究图结构能否作为大语言模型（LLM）的内部推理辅助工具，而非仅仅作为外部知识来源。研究者在多跳问答任务中，将教师提供的推理轨迹重写为图示思维导图来指导学生模型。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要将图作为外部知识源提供给模型，但作者提出图的价值不仅在于提供信息，还在于组织推理。借鉴人类使用图结构思维导图组织分支和汇聚思维的方式，探索图是否能作为内部推理辅助工具。

Method: 在多跳问答任务中，将教师提供的推理轨迹重写为图示思维导图，并用于指导学生模型。通过比较文本形式的图结构与视觉图指导的效果进行实验。

Result: 存在明显的模态差异：当图结构被展平为文本时，去除直接答案提示后，其益处变得有限。在抽象指导设置下，推理效率和答案质量都大幅下降。相比之下，视觉图指导无需直接答案线索仍能保持有效，且该优势在监督微调和KL散度蒸馏后仍然保持。

Conclusion: 图不仅应作为LLM的外部知识结构研究，还应作为组织推理的视觉脚手架来研究。这一发现拓展了图在LLM中的应用视角，强调了其作为推理组织的潜在价值。
...
### [131] [AURA: Action-Gated Memory for Robot Policies at Constant VRAM](https://arxiv.org/abs/2606.02775)
*Josef Chen*

Main category: cs.AI

TL;DR: 针对边缘设备上具身智能体的内存受限场景，提出AURA-Mem方法，通过学习门控机制只在观察改变未来动作时才写入记忆，相比KV-cache大幅减少写入次数，推理状态保持常量大小。


<details>
  <summary>Details</summary>
Motivation: 数据中心使用的KV-cache不适合机器人：边缘硬件带宽受限、内存稀缺、闪存写入耐力有限，且对于长episode场景，内存写入可能成为约束因素而非计算。

Method: AURA-Mem使用冻结的视觉-语言-动作骨干网络+常量大小循环内存+学习门控。门控直接针对闭环动作误差信号训练，而非重建 Loss。只在当前观察会改变下一个动作时写入记忆，实现"知道何时沉默的记忆"。

Result: 合成基准上匹配最佳O(1)基线精度，使用5.19-6.13倍更少写入（简单配置达9.19倍）。OpenVLA-OFT 7B在LIBERO-Long上，匹配无门控基线(0.233)，超过常写KV臂(0.217)，使用7.0倍更少写入。推理状态4224字节vs KV-cache 100000步时6061倍增大。

Conclusion: 动作惊喜信号驱动的门控记忆有效减少写入而不损害性能，为边缘具身智能提供可行内存方案。未来工作可探索层级记忆架构。
...
### [132] [Evaluating Transformer and LSTM Frameworks for Prediction in Ungauged Basins](https://arxiv.org/abs/2606.02791)
*Taye Akinrele,James Halgren,Noorbakhsh Amiri Golilarz,Sudip Mittal,Shahram Rahimi*

Main category: cs.AI

TL;DR: 研究比较了Encoder-only Transformer与LSTM在无观测流域上游流量推断中的表现，使用NOAA国家水资源模型的回溯模拟数据。研究发现LSTM在两种配置下均优于Transformer，融合下游信息可使所有模型的NNSE中位数提升超过60%。


<details>
  <summary>Details</summary>
Motivation: 流域网络具有汇流拓扑结构，无观测流域缺乏直接观测增加不确定性且限制极端事件预测能力。研究旨在评估Encoder-only Transformer相比LSTM是否在有限水文信息下提供上游流量推断优势。

Method: 使用NOAA国家水资源模型(NWM)的回溯模拟数据，比较上游-only和组合两种配置，应用Encoder-only Transformer和LSTM模型进行序列到序列的流量推断。

Result: LSTM在两种配置下整体表现均优于Transformer模型。融入下游信息进一步提升了所有模型的性能，中位数NNSE提升超过60%。

Conclusion: 循环记忆机制比Encoder-only Transformer更适应该上游重建任务的归纳偏置，下游水 Context提供了显著提高预测技能的一致性约束。
...
### [133] [BehaviorBench: Modeling Real-World User Decisions from Behavioral Traces](https://arxiv.org/abs/2606.02798)
...
TL;DR: 本文介绍了BehaviorBench，一个用于评估个性化决策建模的基准测试，通过重构真实世界的预测市场和链上记录中的钱包级决策历史来评估模型性能。该基准包含2000个评估钱包、141,445个信念实例和1,485,972个交易实例，用于评估生成模型在不同历史接口下的个性化表现。


<details>
  <summary>Details</summary>
Motivation: 许多决策支持系统需要适应个体用户，但此类问题的评估数据有限。现有的用户理解基准依赖模拟用户或模型生成行为，而研究表明模型模拟可能与人类行为产生系统性偏离，因此需要基于真实世界行为的评估基准。

Method: BehaviorBench从公开的预测市场和链上记录中重建钱包级决策历史，组织为两个任务层：信念预测（预测用户最终立场和信心）和交易预测（交易方向和金额）。在四种历史接口下评估模型：无个性化、直接近期历史、生成用户画像、检索支持证据。

Result: 个性化对信念预测的改进比交易预测更一致；模型排名随任务层和指标变化；不同历史接口暴露不同失败模式。

Conclusion: BehaviorBench提供了一个使用真实世界行为证据而非模拟用户的评估设置，有助于研究个性化方法在实际场景中的应用价值。
...
### [134] [ChatHealthAI: Aligning Electronic Health Record Representations with Large Language Models for Grounded Clinical Reasoning](https://arxiv.org/abs/2606.02802)
*Bo-Hong Wang,Baicheng Peng,Ruilin Wang,Jun Bai,Ziyang Song,Yue Li*

Main category: cs.AI

TL;DR: 提出ChatHealthAI框架，桥接大语言模型(LLM)的自然语言推理能力与电子健康记录(EHR)基础模型的预测能力，通过任务感知重采样器对齐结构化EHR表示与LLM语义空间，实现可解释的临床预测。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs缺乏有效建模结构化纵向EHR的能力，而EHR基础模型虽然能学习预测性患者表示，但缺乏可解释的语言推理能力，两者之间存在鸿沟。

Method: 设计多模态推理框架ChatHealthAI，包含：(1)预训练EHR基础模型提取结构化患者表示；(2)任务感知重采样器将EHR表示对齐到LLM语义空间；(3)整合纵向患者表示与精炼临床事件描述，输入冻结的LLM进行推理。

Result: 在EHRSHOT基准的三个临床预测任务上评估，ChatHealthAI在保持竞争力的预测性能同时，提高了推理质量和可解释性。

Conclusion: 研究成果表明，将EHR基础模型与预训练LLM集成是实现可解释临床预测的有效途径，具有重要应用潜力。
...
### [135] [Traj-Evolve: A Self-Evolving Multi-Agent System for Patient Trajectory Modeling in Lung Cancer Early Detection](https://arxiv.org/abs/2606.02812)
*Sihang Zeng,Matthew Thompson,Ruth Etzioni,Meliha Yetisgen*

Main category: cs.AI

TL;DR: 提出了Traj-Evolve，一个用于从纵向电子健康记录中建模患者轨迹的自演化多智能体系统。该系统结合了经验池（非参数记忆）和多智能体强化学习两种演化机制，在肺癌预测任务上优于9个强基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统虽然解决了上下文长度问题，但孤立处理患者，无法像临床医生那样利用相似既往病例的累积经验。

Method: 1) 经验池(ExPool)作为非参数记忆，对拒绝采样的推理轨迹进行索引检索相似患者作为少样本上下文；2) 通过奖励排序微调的多智能体强化学习(MARL)优化智能体间协作；3) 留一交叉检索策略统一训练和推理行为。

Result: 在利用长达五年多模态EHR的肺癌预测任务中，Traj-Evolve在整体人群和挑战性的从未吸烟人群中均优于9个强基线。分析表明：扩展ExPool使最优检索从多样本转向特定样本；MARL中管理者智能体预测 loss 快速收敛，而工作智能体时间推理持续受益于更多验证患者；两机制互补——ExPool提高特异性，MARL提高敏感性。

Conclusion: ExPool和MARL两种演化机制相互补充，分别改善了模型的特异性和敏感性，为利用纵向EHR进行患者轨迹建模提供了有效的自演化框架。
...
### [136] [Toward a Modular Architecture for Embedded AI Agent Systems at the Edge](https://arxiv.org/abs/2606.02862)
*Marcus Rüb,Michael Gerhards*

Main category: cs.AI

TL;DR: 本文提出了一个针对嵌入式系统的模块化参考架构，通过分层设计将本地设备上的轻量级代理与云端增强的Small Language Model相结合，并引入治理层来实现分布式自主设备的管理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽能实现复杂推理和工具使用，但嵌入式微控制器的内存和能源限制阻碍了其在深度嵌入环境中的部署，现有框架通常假设服务器级资源或持续连接能力，导致深层嵌入式系统存在技术缺口。

Method: 提出分层设计架构，将On-Device Agents（执行压缩神经网络和规则逻辑用于低延迟、隐私关键任务）与Cloud-Augmented Agents（利用小语言模型进行高级推理和规划）解耦，并集成跨切面的Governance Layer确保可观测性、政策执行和安全性。

Result: 通过分析架构设计原则和权衡，提供了一种在资源受限环境中实现延迟、能源和可靠执行之间平衡的方法，弥合了确定性实时控制与代理智能之间的差距。

Conclusion: 该架构为分布式自主设备车队提供了在严格资源约束下实现代理智能的可行路径，通过模块化设计和治理层的集成，可在隐私敏感的低延迟任务与云端高级推理之间取得平衡。
...
### [137] [An Exploration of Collision-based Enemy Morphology Generation](https://arxiv.org/abs/2606.02832)
*Johor Jara Gonzalez,Matthew Guzdial*

Main category: cs.AI

TL;DR: 本文探讨了如何基于玩家碰撞信息生成视频游戏中敌人的形态（morphology），即敌人的基本体型和碰撞数据。作者提出了三种新颖的方法，并发现这些方法在性能上均优于或等同于从机器人学相关研究中改编的进化基准方法。


<details>
  <summary>Details</summary>
Motivation: 虽然程序化内容生成（PCG）已有大量研究，但针对游戏敌人形态生成的研究却相对匮乏。敌人形态是指敌人的基本体型或碰撞信息，这在游戏中是非常重要的元素。之前几乎没有这方面的专门研究，即使机器人学中有关于形态生成的工作。

Method: 作者探索了三种不同的方法来基于玩家的碰撞信息生成敌人的形态，并将这些方法与之前在机器人学领域中改进的进化基准方法进行了比较。

Result: 结果表明，这三种方法各具优势与不足，但在性能上都达到或超过了从机器人学改编的进化基准方法。

Conclusion: 基于玩家碰撞信息生成敌人形态是可行的，且可以取得良好的效果，为视频游戏的程序化内容生成提供了一个新的研究方向。
...
### [138] [When Helping Hurts and How to Fix It: Multi-Agent Debate for Data Cleaning](https://arxiv.org/abs/2606.02866)
*Chirag Parmar,Akshat Mehta,Henglin Wu,Jagadish Ramamurthy,Shweta Medhekar*

Main category: cs.AI

TL;DR: 研究多智能体辩论对数据清洗的影响，发现辩论效果会反转：虽然通过批评诱导混淆(CIC)降低所有模型的生成质量(-1.6到-15.5pp)，但显著提升错误检测能力(+27.4pp F1)。并推导出辩论有益的条件公式。


<details>
  <summary>Details</summary>
Motivation: 探究多智能体辩论在数据清洗中何时有帮助、何时有损害，解决当前研究中辩论效果不一致的问题。

Method: 跨三个基准、四个模型家族、超过6,000个任务-条件对进行大规模评估，并设计因子实验验证对抗分离的必要性。

Result: 辩论效果反转：降低生成质量但提升错误检测。辩论有益条件为 rescue概率 > destroy概率。分离的 Critic+代码执行接地+证据门控生成首次在生成任务上显著超越单智能体(+5.3pp)。该条件正确预测九种任务类型，泛化19个已发表比较无假阳性。

Conclusion: 辩论并非普遍有益，需要满足特定条件才有效。自验证失败，对抗性分离配置是关键。该条件可推广到七个领域的多种任务。
...
### [139] [Thinking Past the Answer: Evaluating Harmful Overthinking in Large Reasoning Models](https://arxiv.org/abs/2606.02835)
*Simone Caldarella,Davide Talon,Rahaf Aljundi,Elisa Ricci,Massimiliano Mancini*

Main category: cs.AI

TL;DR: 研究大型推理模型（LRMs）在得出正确答案后继续推理是否会是多余的甚至有害的。通过引入前缀级轨迹评估协议，发现在首次出现正确答案时停止推理反而能提高准确率高达21%，揭示了当前模型不仅缺乏推理能力，也缺乏适时停止的能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型通过增加测试时间计算来生成中间推理步骤以提升性能，但假设更长的推理总是有益的尚未得到充分验证。近期证据表明额外推理可能导致模型过度思考，因此需要研究：当模型已经得出正确答案后，进一步推理是在精炼解决方案还是让它偏离正确方向？

Method: 引入基于推理充足性的前缀级轨迹评估协议，定义模型首次生成正确答案所需的最小推理预算，从而区分冗余但无害的verbose overthinking和会破坏已正确轨迹的harmful overthinking。从多模态基准开始实验。

Result: 发现许多被认为是需要深度推理的任务实际上只需要很少的推理。在第一个正确前缀处停止比标准推理提高准确率高达21%。常见的效率策略如early stopping可以显著减少verbose overthinking（高达50%），但无法缓解harmful overthinking。正确性偏差主要由逻辑漂移和视觉重新解释驱动。这些发现在纯语言推理基准上也能泛化，突显harmful overthinking是更广泛的可靠性风险。

Conclusion: 当前LLMs的限制不仅在于推理能力不足，也在于无法在正确时机停止。过早停止可提升性能，有害过度思考是一个需要解决的可靠性问题，尤其在涉及逻辑推理和多模态理解的任务中。
...
### [140] [AUDITFLOW: Executable Symbolic Environments for Structured Financial Reporting Verification](https://arxiv.org/abs/2606.03031)
...
TL;DR: 本文提出AuditFlow，一个用于财务审计验证的图grounds多智能体框架，通过分离自适应搜索与确定性验证提升语言模型的审计准确性。


<details>
  <summary>Details</summary>
Motivation: 结构化财务审计验证对语言模型智能体困难，因为正确性依赖结构化证据而非文本。模型需将报告事实链接到taxonomy概念、遍历计算或维度关系、在应用审计规则前重新计算预期值。

Method: 提出AuditFlow图 grounded多智能体框架，分离自适应搜索与确定性验证。基于US-GAAP taxonomy构建符号环境，通过XBRL filing graph动态扩展，暴露typed tools用于事实检索、taxonomy遍历、数值检查和规则评估。采用双Junior Auditor多视角检查+Senior Auditor争议解决+证据融合的报告生成流程。

Result: 在FinAudiving-derived FinMR样本上，AuditFlow在GPT-5.5下达到82.09%的联合审计准确率，比最强基线高出14.93个百分点。去除确定性检查后准确率降至17.91%。

Conclusion: 实验表明确定性检查对准确率至关重要，去除后准确率降至17.91%，证明了符号环境在验证步骤中不可替代的作用。
...
### [141] [Solipsistic Superintelligence is Unlikely to be Cooperative](https://arxiv.org/abs/2606.03237)
*Rakshit S Trivedi,Natasha Jaques,Logan Cross,Alexander Sasha Vezhnevets,Joel Z Leibo*

Main category: cs.AI

TL;DR: 论文提出AI研究范式需要从"孤立智能"转向"合作智能"，认为当前以开发强大但孤立的代理为中心的范式存在根本性缺陷——这种"自我削弱"特性导致训练与部署环境不匹配。解决方案是构建将相互依存作为核心设计原则的非孤立研究范式。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究的主流范式是开发强大的智能体，将世界视为外生和静态的反馈来源。然而这种方法存在根本性问题：超级智能在这种"孤立主义"设计下不太可能具有合作性，因为部署AI系统会引入内生非平稳性，导致历史分布与部署上下文产生差距（即训练-测试-部署gap）。作者将此称为"单边优化的自我削弱特性"。

Method: 论文提出三项具体研究方向：(1) 构建包含自适应对手方的动态评估测试平台；(2) 将制度（institutions）视为设计原语；(3) 在系统中保留人类能动性作为结构性特征。核心是将合作视为equilibrium-selection过程，通过这一过程多个行为者导航其相互依存关系。

Result: 论文的核心贡献在于识别了当前AI范式的根本性缺陷——"自我削弱特性"，并指出解决训练-测试-部署差距需要AI参与合作——即作为equilibrium-selection过程而非作为待解决问题的任务。提出将相互依存作为核心设计原则而非事后考虑的"非孤立主义"研究范式。

Conclusion: AI的中心挑战正从能力转向共存。需要彻底转变研究范式：从将AI视为独立强大的问题解决者，转变为将AI视为参与复杂性 equilibrium-selection 的参与者，其中相互依存而非个体能力成为核心设计原则。这要求在构建系统时将人类能动性作为结构性特征保留。
...
### [142] [Don't Gamble, GAMBLe: An Analytical Framework for AI-Driven Research Systems](https://arxiv.org/abs/2606.02863)
*Marquita Ellis,Paul Castro*

Main category: cs.AI

TL;DR: 提出了一个用于分析AI驱动研究系统(ADRS)的新框架GAMBBe，该框架通过四个参数(生成器G、评估器A、发现机制M、预算B)和有效景观Leff来分解ADRS行为，揭示了不同生成器-评估器对诱导结构不同的优化景观。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏分析ADRS的工具，而标准收敛假设在ADRS过程中不成立。ADRS性能依赖于组件交互，这些交互难以理解和探索。

Method: 引入GAMBLe框架，将ADRS行为分解为四个参数和有效景观Leff=A°G；在760+个重复运行（>46,000次迭代）上测试，覆盖从单个LLM到动态自适应集成、不同机制、三种NP-hard问题。

Result: 没有生成器或机制的完全排序：前沿模型可能不如开源替代品，最简单的机制有时优于最先进的元搜索；在60次迭代的有限预算下，正确的组件选择可提高性能13-67%，搜索效率6-39倍。

Conclusion: 需要新的ADRS分析框架；组件选择比组件本身的质量更重要；简单机制在某些情况下表现更好。
...
### [143] [A formal definition and meta-model for a machine theory of mind](https://arxiv.org/abs/2606.03471)
*Fabio Cuzzolin*

Main category: cs.AI

TL;DR: 首次提出机器心智Theory of Mind的严格形式化定义，融合认知心理学、神经科学和人工智能证据，构建整体元模型，并审视当前研究与基准测试。


<details>
  <summary>Details</summary>
Motivation: 该领域缺乏严谨的形式化定义，现有研究分散且无统一框架，需要建立理论基础以推动后续研究真正"破解"机器心智理论问题。

Method: 基于认知心理学、神经科学和人工智能的支持证据，建立概念的形式化定义；构建通用整体元模型；审视最先进的方法与实证基准测试。

Result: 首次提出Machine Theory of Mind的严格形式化定义；开发了通用整体元模型；分析了当前模型的基准测试状态。

Conclusion: 为该领域提供了潜在的研究议程，有望推动解决机器理解他人心智状态的难题。
...
### [144] [Handoff Debt: The Rediscovery Cost When Coding Agents Take Over Interrupted Tasks](https://arxiv.org/abs/2606.02875)
*Dipesh KC,Anjila Budathoki*

Main category: cs.AI

TL;DR: 研究编码代理benchmarks中缺失的"交接"维度，提出handoff debt（交接债务）概念——当前任工作不透明或不完整时，后续代理需要重新发现的成本。通过设计一个交接协议，在确定性交接点中断编码代理并冻结仓库，从75个源任务生成181个交接点任务，对三个后续模型进行724次接管运行，评估四种交接视图（仅仓库状态、原始trace、摘要笔记、结构化笔记）的效果。


<details>
  <summary>Details</summary>
Motivation: 现有编码代理基准测试假设单一连续工作流程，但真实软件开发中任务会被中断、重分配、审查，并从另一代理或工程师留下的部分状态恢复。这种"Handoff Debt"维度被忽视了，需要评估交接时的成本。

Method: 设计了takeover协议：在确定性交接点中断编码代理，冻结仓库状态，然后在四种交接视图下评估后续代理：(1)仅仓库状态 (2)原始trace (3)摘要笔记 (4)结构化笔记。从75个源任务生成181个交接点任务，每个后续模型执行724次接管运行，覆盖三种后续模型。

Result: 带有上下文的交接方式相比仅仓库状态接管，减少20-59%的中位代理事件和42-63%的累计提示tokens。解决率效果较小且模型依赖，但效率提升一致。

Conclusion: 编码代理评估不仅应报告任务是否解决，还应报告该工作对另一代理恢复的代价。建议在benchmark中增加handoff debt指标。
...
### [145] [Large AI Models in Dental Healthcare: From General-Purpose Systems to Domain-Specific Foundation Models](https://arxiv.org/abs/2606.02914)
*Sema Helali,Lina Abu Nadab,Sausan Alqawas,Alaa Abd-Alrazaq,Faleh Tamimi,Rafat Damseh*

Main category: cs.AI

TL;DR: 本系统综述对牙科领域的大规模AI模型进行了全面分类与分析，涵盖语言生成模型、视觉基础模型和牙科专用模型，提出两维分类框架，并指出集成 pipeline 为最优方案。


<details>
  <summary>Details</summary>
Motivation: 口腔疾病影响全球35亿人，但牙科AI模型的临床潜力尚不明确，缺乏统一综述来理清三类模型的关系与局限性。

Method: 遵循PRISMA-ScR指南，系统检索四大数据库(PubMed、Google Scholar、Scopus、arXiv)，两名独立审稿人筛选，最终纳入97项研究(2020-2026)，提出基于架构范式与牙科专业化程度的两维分类框架。

Result: 语言生成模型擅长文本任务(临床推理、执照考试、患者沟通)，但图像诊断表现不稳定；改编的SAM/CLIP变体在牙齿分割和病变检测上表现优异；牙科专用模型(DentVFM、DentVLM、OralGPT)在复杂多模态任务上最强；集成pipeline一致优于单一模型；存在数据不对称：牙科预训练几乎集中于视觉领域，大规模牙科文本语料库匮乏。

Conclusion: 通用模型与牙科专用模型互为补充，最佳系统需将两者结合于结构化pipeline中。安全自主部署需解决三大障碍：生成模型幻觉、标注牙科数据集有限、缺乏标准化临床评估基准。
...
### [146] [What Benchmarks Don't Measure: The Case for Evaluating Abstention Competence in Autonomous Agents](https://arxiv.org/abs/2606.02965)
*Victor Ojewale,Suresh Venkatasubramanian*

Main category: cs.AI

TL;DR: 本文研究自主智能体中的"服从偏差"问题，即智能体在缺乏必要输入、证据或授权时仍倾向于继续执行任务。提出三场景分类法（规范缺口、验证缺口、权威缺口）和三个评估指标，可在保持可用性的同时阻断高达89.2%的危险操作。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试只关注任务是否完成，忽略了智能体是否应该在某些情况下"拒绝行动"。这种偏好处罚智能体暂停或无法区分合理拒绝与失败，导致训练中产生"服从偏差"，带来安全隐患。

Method: 通过144个企业智能体场景和5个模型家族进行实验，提出三Gap分类法（规范缺口/规范缺失、验证缺口/世界状态无法确认、权威缺口/缺少明确授权）来界定应拒绝场景，并设计 Safety Rate、Usability Rate、Informed Refusal Rate 三个运行时评估指标。

Result: 运行时强制拒绝机制可实现89.2%危险操作阻断率和87.5%授权场景可用率，安全-可用性权衡可通过参数调节，且不同模型家族表现差异显著。

Conclusion: 安全-可用性权衡非固有矛盾，可通过设计 Abstention-aware 基准测试和评价指标改善。提出三Gap分类法和复合指标作为进一步讨论的起点。
...
### [147] [WISE-HAR: A Generalizable Ensemble Deep Learning Framework for WiFi-Based Human Activity Recognition](https://arxiv.org/abs/2606.02974)
...
### [148] [Inducing Reasoning Primitives from Agent Traces](https://arxiv.org/abs/2606.02994)
*Zhihan Lei,Jiarui Yan,Joshua Momo,William W. Cohen*

Main category: cs.AI

TL;DR: 提出一种名为"推理原语归纳"的方法,从ReAct代理的轨迹中自动挖掘和提取可重用的推理模式,将其转换为伪工具库,在测试时通过标准ReAct循环组合使用。


<details>
  <summary>Details</summary>
Motivation: ReAct风格的LLM代理经常在不同问题上重复相同的推理步骤,但这些推理过程只存在于临时笔记中,无法被保留和复用。本文希望从成功的ReAct轨迹中提取常见的推理模式,建立可重用的推理库。

Method: 该方法包含三个步骤:(1)单次遍历挖掘成功的ReAct轨迹;(2)聚类分析找出反复出现的推理移动;(3)将最频繁的推理移动转换为带有自然语言文档说明的紧凑型伪工具库。推理原语由LLM在调用时解释的标准文档字符串指定,测试时通过标准ReAct循环组合这些原语。

Result: 诱导得到的推理库超越了生成这些轨迹的原始代理:RuleArena NBA上提升44个百分点(30→74),MuSR团队分配任务上提升30个百分点(38→68),NatPlan会议规划任务上提升22个百分点(7→29)。在五个可比对的子任务上,单一固定配置始终优于零样本思维链,与专家编写的分解方法相当或超越,且推理成本低于AWM。

Conclusion: 从成功代理轨迹中自动诱导的推理原语库可以显著超越原代理的性能,并且能够匹配或超越人类专家设计的任务分解方法,为构建更高效、可复用的LLM推理代理提供了新思路。
...
### [149] [TriEval: A Resource-Efficient Pipeline for LLM Bias, Toxicity, and Truthfulness Assessment](https://arxiv.org/abs/2606.03036)
*Akshatha Srikantha,Manpreet Singh,Yash Jajoo,Shyamal Lakhanpal*

Main category: cs.AI

TL;DR: TriEval是一个在标准笔记本上运行的开源LLM评估工具，可同时评估偏见、毒性和真实性等多个参数，兼容开源和闭源模型。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估工具存在局限性，多数仅能测试单一参数或需要大量计算资源。部署后的LLM常出现输出不一致和幻觉等问题，亟需全面评估以确保其安全性和公平性。

Method: 该工具已在四种模型上进行测试：Llama 3 8B、Mistral 7B、Gemma 2 9B和Claude Haiku，结果显示开源模型和闭源模型之间存在明显差异，尤其在毒性和真实性方面。

Result: 该工具作为开源软件发布，旨在让计算资源有限的研究人员也能获得更广泛的访问权限。

Conclusion: 该工具作为开源发布，降低了LLM评估的门槛，使更多研究人员能够参与LLM的安全性和公平性评估工作。
...
### [150] [RelGT-AC: A Relational Graph Transformer for Autocomplete Tasks in Relational Databases](https://arxiv.org/abs/2606.03040)
*Phillip Jiang*

Main category: cs.AI

TL;DR: 本文提出RelGT-AC（面向自动填充任务的 Relational Graph Transformer），通过扩展 RelGT 架构解决关系数据库预测问题。核心贡献包括：(1) 列掩码策略防止子图编码时的trivial解；(2) 统一任务头支持二分类/多分类/回归三种自动填充任务；(3) TF-IDF文本编码器自动检测并编码自由文本列。在RelBench v2的3个数据集( rel-trial, rel-f1, rel-stack)共7个任务上验证，RelGT-AC在所有回归任务上超越GraphSAGE基线，并在文本密集型 eligibility任务上获得+10 AUROC提升。


<details>
  <summary>Details</summary>
Motivation: 关系数据库是企业、科学和医疗系统的核心，但因其多表、异构和时序结构，在其上进行预测性机器学习仍有挑战。RelBench v2引入autocomplete任务——根据关系上下文预测现有列值，类似于智能表单填写助手，这是一项实用且新穎的任务类型。

Method: RelGT-AC提出三项针对性改进：(1) 列掩码策略——在子图编码阶段对目标列进行掩码，防止模型学习到trivial捷径解；(2) 统一任务头——单一模型框架内支持二分类、多分类和回归三类自动填充任务；(3) TF-IDF文本编码器——自动检测free-text列并用TF-IDF方法编码，充分挖掘类别编码器会丢失的词汇信号。

Result: 实验覆盖7个任务、3个RelBench v2数据集(rel-trial, rel-f1, rel-stack)。RelGT-AC在全部3个回归自动填充任务上超越GraphSAGE基线，且在text-heavy eligibility任务上通过TF-IDF encoder获得最高+10 AUROC的性能增益。

Conclusion: RelGT-AC通过列掩码、统一任务头和TF-IDF文本编码三项创新，有效解决了关系数据库自动填充任务的核心挑战。实验结果表明该方法在回归任务和文本密集型任务上均显著优于基线，为实际关系数据预测提供了有效方案。
...
### [151] [ToolGate: Token-Efficient Pre-Call Control for Tool-Augmented Vision-Language Agents](https://arxiv.org/abs/2606.03054)
*Anjie Liu,Yan Song,Zhixun Chen,Ziqin Gong,Zhongwei Yu,Jun Wang*

Main category: cs.AI

TL;DR: ToolGate是一种轻量级外部控制器，解决视觉语言模型代理的预调用控制问题，决定是否执行或跳过感知工具调用。在保持精度的同时，将token成本降低至64-69%。


<details>
  <summary>Details</summary>
Motivation: 工具增强的VLM代理可以调用OCR、检测、分割等外部感知工具，但执行每个工具调用代价高昂且有时不必要。当前ReAct风格的VLM代理在本地选择性上表现不佳：有益和有害调用发生率相似（11.8% vs 9.9%），大多数调用不会改变即时强制回答预测。因此需要研究预调用控制问题：在代理提出感知工具调用后，是否应该执行该调用，还是在输出进入上下文前直接跳过。

Method: 引入ToolGate，一个轻量级外部控制器，从轨迹文本和简单结构特征预测执行/跳过决策。采用五个基准测试评估，在两个Qwen3-VL backbone上进行跨域训练，并在Qwen3-VL-30B上进行匹配域轨迹训练。

Result: 基线代理表现出较差的本地选择性：有益调用和有害调用发生率相近（11.8% vs 9.9%），大多数调用不会改变即时强制回答预测。ToolGate将token成本降低至无限制ReAct基线的64-69%，同时在跨域设置中保持平均精度。通过在Qwen3-VL-30B上进行匹配域轨迹训练，进一步提高平均精度1.65个百分点。

Conclusion: 工具增强的VLM代理不仅受益于更好的感知工具，还受益于对工具输出是否值得付费的显式控制。ToolGate证明了一种有效的轻量级方法来实现这种控制，在成本和精度之间取得良好平衡。
...
### [152] [SkillDAG: Self-Evolving Typed Skill Graphs for LLM Skill Selection at Scale](https://arxiv.org/abs/2606.03056)
*Tong Bai,Zhenglin Wan,Pengfei Zhou,Xingrui Yu,Wangbo Zhao,Yang You,Ivor W. Tsang*

Main category: cs.AI

TL;DR: SkillDAG是一种将技能间关系建模为类型化有向图的系统，为LLM智能体提供推理时可调用的结构化检索接口，支持向量匹配、类型边邻居和冲突信号搜索，并通过提议-提交协议允许智能体注册执行支持的边，使图在执行过程中累积结构。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体采用大型技能库，选择正确的技能子集成为结构性问题而非相似度匹配问题：技能之间存在依赖、冲突、特化或重复关系，这种结构对全枚举和嵌入相似度均不可见。

Method: 将技能间关系建模为类型化有向图，作为推理时可调用的结构化检索接口，每次搜索返回向量匹配、类型边邻居和冲突信号，并使用提议-提交协议让智能体注册执行支持的边，使图跨episode累积结构。

Result: 在ALFWorld和SkillsBench上，SkillDAG达到67.1%成功率和27.3%奖励，比最强Graph-of-Skills基线分别提升+12.8和+8.6点；优势迁移到gpt-5.2-codex，SkillsBench内在Ret@K从65.5升至78.2。候选排名在池增长10x时保持鲁棒，而固定seeding-diffusion管道退化；集合单调在线编辑扩大真值召回而不驱逐先前命中。

Conclusion: SkillDAG通过结构化技能关系建模和动态图更新机制，有效解决大规模技能库的选择问题，其可隔离机制包括随池增长的鲁棒候选排名和无驱逐的集合单调在线编辑，为LLM智能体的技能选择提供了有效方案。
...
### [153] [CORE: Conflict-Oriented Reasoning for General Multimodal Manipulation Detection](https://arxiv.org/abs/2606.03066)
*Jinjie Shen,Yaxiong Wang,Yujiao Wu,Lechao Cheng,Tianrui Hui,Nan Pu,Zhihui Li,Zhun Zhong*

Main category: cs.AI

TL;DR: 本文提出CORE框架，通过检测多模态假新闻中的内在冲突来实现 robust 的假新闻检测。该方法创新性地构建了冲突归因语料库(CAC)，并赋予多模态大语言模型显式捕获冲突的能力，能够在少样本或零样本设置下快速适应新出现的操纵类型。


<details>
  <summary>Details</summary>
Motivation: 生成式AI使得多模态假新闻越来越逼真和普遍，对公众信任和社会稳定造成严重威胁。现有检测方法依赖特定Manipulation模型和大规模标注数据，对新出现的操纵类型泛化能力差。作者观察到操纵的假新闻本质在于存在语义或物理上的内在冲突。

Method: 提出CORE框架，包含三个核心部分：1)构建带有细粒度注释的冲突归因语料库(CAC)，提供冲突因素和来源的标注；2)基于CAC进行冲突导向的表示增强和推理训练；3)实现对未见操纵类型的少样本或零样本适应。

Result: 实验表明CORE框架性能超越了当前的state-of-the-art模型。

Conclusion: CORE框架通过利用多模态内容中的内在冲突信息，实现了有效且通用的多模态假新闻检测，能够快速适应新的操纵类型，不需要大规模特定领域的标注数据，具有良好的泛化性和实用性。
...
### [154] [DELTAMEM: Incremental Experience Memory for LLM Agents via Residual Trees](https://arxiv.org/abs/2606.03083)
*Haoran Tan,Zeyu Zhang,Zhicheng Cao,Rui Li,Xu Chen*

Main category: cs.AI

TL;DR: DeltaMem是一个用于LLM代理的内存组织框架，通过将经验存储为残余树结构来解决传统平面记忆的冗余和检索冲突问题。该框架包含两种残余树：目标条件任务经验树和场景级环境知识树，并通过失败惩罚的相似度扫描进行检索，以及自动整合机制将高频路径蒸馏为新的根节点。


<details>
  <summary>Details</summary>
Motivation: 当前LLM代理使用记忆学习持续交互经验，但将经验存为独立平面单元导致相似经验重复存储且检索时出现冲突，因为相似情节包含重叠内容而细微场景变化导致检索到的记忆提供矛盾指导。

Method: 提出残余经验概念，认为新获取的经验往往是现有知识的增量变体。构建两个独立的残余树：一个存储目标条件任务经验作为可重用技能，另一个存储场景级环境知识。每棵树用根节点存储泛化基础经验，增量delta节点存储后续变体，使相关经验可共享公共基础而无重复。检索时采用失败惩罚的相似度扫描找到最佳匹配，通过根到匹配链重构完整经验。自动整合机制将高频路径蒸馏为新根节点，使树从泛化启发式自组织到专业化变体。

Result: 在多样化交互环境中的实验表明DeltaMem始终优于现有基线方法。

Conclusion: DeltaMem通过残余经验建模有效解决了LLM代理记忆系统中的冗余和检索冲突问题，为代理的记忆组织提供了新的思路。
...
### [155] [The Shadow Price of Reasoning: Economic Perspective on Optimal Budget Allocation for LLMs](https://arxiv.org/abs/2606.03092)
*Xu Wan,Speed Zhu,Jianwei Cai,Guang Chen,XiMing Huang,Wiggin Zhou,Mingyang Sun*

Main category: cs.AI

TL;DR: 本文提出CLEAR方法，通过将推理预算分配建模为受经济约束的全局优化问题，在有限计算资源下实现LLM推理时间的智能分配。核心思想是基于边际效用均衡的影子价格进行资源配置，实现从无法解决的问题到可解决问题的资源再分配。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的推理时间扩展已成为提升性能的关键途径，但实际部署受限于严格的计算预算约束。现有方法难以在稀缺资源条件下实现最优的资源利用效率。

Method: 将推理预算分配形式化为受经济原理支配的全局约束优化问题。使用移位激增函数(shifted-surge function)建模每查询推理效用，推导出基于全局影子价格的最优分配策略，实现理性的资源放弃与再分配。

Result: 在多个推理任务和不同流量流的广泛实验中，CLEAR显著改善了总token成本与平均准确率的帕累托前沿。在资源稀缺条件下，相比均匀分配获得最高3倍的全局准确率提升。

Conclusion: 基于经济学原理的约束优化框架能够有效解决LLM推理资源的智能分配问题，为推理时间扩展的实际部署提供了可行且高效的解决方案。
...
### [156] [Decomposing how prompting steers behavior](https://arxiv.org/abs/2606.03093)
*Fan L. Cheng,Nikolaus Kriegeskorte*

Main category: cs.AI

TL;DR: 研究提示词(prompt)如何在不更新权重的情况下改变LLMs和VLMs的内部表征。提出嵌套几何分解框架，将prompt视为对表征几何结构的变换，通过不同复杂度层级(平移→刚体变换→仿射变换→非线性变换)来对齐和分析表征变化，发现跨维线性混合是prompt重组表征的关键机制。


<details>
  <summary>Details</summary>
Motivation: 探究性理解：prompting是一种无需权重更新的高效控制方法，但其工作机制不明——不清楚指令如何重塑内部表征进而产生行为变化。需要量化分析prompt诱导的表征变化的几何结构。

Method: 嵌套几何分解框架：设计5个递进层级的刺激不变映射(平移、均匀缩放刚体变换、顺序轴缩放、仿射变换、非线性变换)；对同一刺激在不同prompt下的表征进行对齐；通过替换单层隐藏态进行因果测试，测量表征几何和行为恢复程度。实验涉及3个LLM、3个VLM及6个文本/图像数据集(风格、情感、场景、内容、数字)。

Result: 发现：(1)Prompt一致地将表征重塑向指令任务结构；(2)大部分激活变化可由保形映射(尤其平移和刚体变换)解释；(3)存在模型和任务特定的跨层路由策略；(4)仿射变换是首个能够几乎完全恢复目标prompt任务几何并带来相应行为增益的层级；(5)跨维线性混合是prompt重组表征的核心机制。

Conclusion: 提出一种将prompt诱导的表征变化分解为可解释几何分量的框架，揭示了模型如何路由任务相关结构以产生prompt驱动行为。核心贡献在于识别出仿射变换层面的跨维线性混合是关键作用机制，为理解和优化prompting提供理论依据。
...
### [157] [From Long News to Accurate Forecast: Importance-Aware Fusion and PRM-Guided Reflection for Time Series Forecasting](https://arxiv.org/abs/2606.03097)
*Mingyang Liu,Qingcan Kang,Yuke Wang,Shixiong Kai,Kaichao Liang,Hui-Ling Zhen,Tao Zhong,Mingxuan Yuan,Linqi Song*

Main category: cs.AI

TL;DR: 一种新型LLM时间序列预测框架，通过重要性感知新闻压缩和过程级检索监督来解决现有方法的两个实际问题：新闻文章超过模型上下文窗口的限制，以及无指导迭代检索导致的冗余更新和收敛缓慢。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的新闻预测管道面临两个实际限制：相关新闻文章经常超过模型的上下文窗口，且补充新闻的迭代检索通常是无导向的，导致冗余更新和收敛缓慢。

Method: 提出两个组件：1）重要性奖励模型估计每篇文章的预测效用，并在顺序成对融合过程中分配压缩预算；2）过程奖励模型(PRM)根据当前错误画像和先前选择文章的历史对多个候选新闻进行排序。

Result: 在金融、能源、交通和比特币预测基准上的实验表明，该方法相比强基线提高了预测精度，显著减少了迭代次数，并且在相关文章达数千token时依然有效。

Conclusion: 该框架有效解决了LLM新闻预测中上下文窗口限制和低效检索的问题，在减少迭代次数的同时实现了更好的预测精度。
...
### [158] [DeskCraft: Benchmarking Desktop Agents on Professional Workflows and Human-in-the-Loop Collaboration](https://arxiv.org/abs/2606.03103)
*Wenkai Wang,Tao Xiong,Jingchen Ni,Yunpeng Bao,Xiyun Li,Tianqi Liu,Hongcan Guo,Zilong Huang,Shengyu Zhang*

Main category: cs.AI

TL;DR: DeskCraft是一个针对专业创意和工程软件的长时序桌面GUI基准测试，重点关注人机协作场景，涵盖设计、视频、音频和3D创作等专业软件，以及中turn和post-turn交互协议。


<details>
  <summary>Details</summary>
Motivation: 现有桌面GUI基准测试大多简化为短任务，用户指令一次性给出，无法反映真实专业工作流程中的长时序和主动式人机协作需求。

Method: 构建多层级难度分类体系，设计538个任务评估18种代理，形式化人Agent协作为包含mid-turn和post-turn交换的交互协议。

Result: GPT-5.4在标准任务上达到31.6%，在交互任务上达到27.6%，显示出长时序工作流交付和主动澄清仍存在持续失败。

Conclusion: 当前代理在处理专业桌面工作流和主动人机协作方面仍有显著不足，需要进一步研究和改进。
...
### [159] [EvoTrainer: Co-Evolving LLM Policies and Training Harnesses for Autonomous Agentic Reinforcement Learning](https://arxiv.org/abs/2606.03108)
...
### [160] [Uncertainty-Aware Clarification in LLM Agents with Information Gain](https://arxiv.org/abs/2606.03135)
*Mengyi Deng,Zhiwei Li,Xin Li,Tingyu Zhu,Ying Zhao,Zhijiang Guo,Wei Wang*

Main category: cs.AI

TL;DR: 本文提出一种目标导向的LLM智能体澄清框架，通过Information Gain Reward奖励机制优化澄清问题质量，在τ-Bench环境中实现成功率提升3.7%。


<details>
  <summary>Details</summary>
Motivation: LLM智能体常在用户指令不明确的情况下操作，对用户意图的不确定性导致工具执行错误，需要一种能够有效降低不确定性、提高任务完成率的澄清机制。

Method: 提出信息增益奖励(Information Gain Reward)度量标准，通过贝叶斯信念更新来量化澄清问题对真实目标的效用，训练clarifier LLM优化高信息增益，确保澄清有效减少不确定性。

Result: 在澄清增强的τ-Bench环境中进行五个异构骨干模型的跨智能体评估，结果显示方法比无澄清基线成功率提升3.7%，平均仅增加0.3个交互步骤。

Conclusion: 所提框架能够一致性地改善任务完成率，同时保持较低的操作复杂度，为LLM智能体在处理模糊用户指令方面提供了有效的解决方案。
...
### [161] [Think-Before-Speak: From Internal Evaluation to Public Expression in Multi-Agent Social Simulation](https://arxiv.org/abs/2606.03137)
*Kaiqi Yang,Tai-Quan Peng,Sanguk Lee,Hui Liu*

Main category: cs.AI

TL;DR: TBS是一个基于间隔的多智能体模拟框架,将智能体的私人推理与公共话语生成分离,通过结构化内部状态(不和谐评估、感知观点气候、感知孤立风险、响应策略和发言意愿)来模拟从内部评估到公共表达的路径,使社会互动机制可观察和分析。


<details>
  <summary>Details</summary>
Motivation: 现有对话模拟框架主要将交互表示为可观察的轮次交换或聚合输出,难以考察内部评估过程(如沉默、发言意图和公共表达背后的心理机制),需要一种能分离私人推理与公共话语生成的框架。

Method: 在每个间隔,所有智能体基于共享对话历史和自身记忆更新结构化内部状态,包括不和谐相关评估、感知观点气候、感知孤立风险、响应策略和发言意愿。编排器解决竞争性发言意图并将一个话语提交给公共对话,允许内部评估与公共交互协同演化。

Result: TBS产生连贯的内部状态轨迹,且这些轨迹在不同轮次分配、沉默和记忆条件下系统变化。不和谐相关评估增加发言意愿,而沉默压力评估降低发言意愿。公共表达主要由轮次分配规则塑造。

Conclusion: TBS通过使从内部评估到公共表达的路径可见和可分析,支持机制敏感的社会模拟,为研究社会互动、 deliberative和集体意见动态提供有前景的方法。
...
### [162] [GTBench: A Curriculum-Grounded Benchmark for Evaluating LLMs as Mathematical Research Assistants in Graph Theory](https://arxiv.org/abs/2606.03144)
*Noujoud Nader,Ibrahem Aljabea,Patrick Diehl,Deepti Gupta*

Main category: cs.AI

TL;DR: GTBench是一个基于课程图的论基准，用于评估大型语言模型作为数学研究助手的可靠性，包含63个难度递增的问题，来自Diestel's Graph Theory等验证学术材料。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在技术学科中作为自学助手的使用日益增加，但它们作为数学推理助手的可靠性尚未得到充分理解和评估。

Method: 评估五个前沿模型（GPT-5、Claude Sonnet 4.6、Gemini 2.5 Flash-Lite、Llama 3.3 70B、Mistral Large 3），使用零样本和思维链提示，采用精确匹配和LLM作为评判者进行第1、2组评估，第3组采用人类专家和LLM混合评判协议。

Result: GPT-5在第1组接近天花板（95.8%零样本），在研究生证明题上保持有意义准确率（82%），而其他模型随难度增加显著下降，Llama在第3组零样本人类评估中为0%。失败模式分析显示算法正确但执行错误是关键问题，第3组还存在不完整推理失败，人类评估者与自动评判之间存在系统性分歧（κ=0.48-0.83）。

Conclusion: GTBench提供了首个基于课程的图论推理评估框架，对AI工具在数学教育和科学研究中的治理具有直接意义。
...
### [163] [ClinicalMC: A Benchmark for Multi-Course Clinical Decision-Making with Large Language Models](https://arxiv.org/abs/2606.03157)
*Ruihui Hou,Siyi Zhu,Ziyue Huai,Guangya Yu,Yongqi Fan,Chunming Wang,Tong Ruan*

Main category: cs.AI

TL;DR: 提出了ClinicalMC基准，用于评估大型语言模型在多疗程临床决策中的表现，包含1,275个中文和5,804个英文样本，涵盖入院到出院四个阶段，并构建了多智能体评估框架进行单轮静态和多轮动态实验设置。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准主要评估单一疗程设置，缺乏在多疗程场景下的系统评估，而临床中患者病情随时间演变，现有方法无法有效评估LLM在这一复杂过程中的决策能力。

Method: 构建了包含四阶段的ClinicalMC基准（分诊、首次诊疗、多次复诊/评估/治疗、最终诊断），设计多智能体评估框架（患者、检查者、医生智能体），并设置单轮静态和多轮动态两种实验环境，测试封闭源、开源和医学专用三类LLM。

Result: 英文数据集平均每个患者经历5.11个疗程，中文数据集为3.44个疗程；已对GPT5-mini、DeepSeek-V3.2、HuatuoGPT-o1等模型进行评估。

Conclusion: 该基准和方法有助于更好理解LLM在医学领域的表现，支持医疗健康领域的有效部署。
...
### [164] [MedCUA-Bench: A Screenshot-Only Benchmark for Clinical Computer-Use Agents](https://arxiv.org/abs/2606.03203)
*Jia Yu,Zilong Wang,Xinyang Jiang,Dongsheng Li,Shuo Wang*

Main category: cs.AI

TL;DR: MedCUA-Bench是一个用于评估临床计算机使用智能体的交互式基准测试，覆盖18个临床场景和10个医学领域。最佳闭源模型达到54.2%的严格成功率，但在真实OpenEMR系统上所有模型均低于9%，开源模型平均仅2.5%。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试专注于通用网页或桌面任务，未能代表医疗软件的特殊性。医疗软件具有独特的UI设计、需要领域知识、缺乏公开测试环境，且需要超越任务完成的安全性验证。因此需要专门的医疗GUI智能体基准。

Method: 构建了包含18个临床场景的MedCUA-Bench，涵盖10个医学领域，从真实产品手册和开源医学系统重建，以捕捉真实临床界面。每个任务配备意图级和步骤级目标，通过确定性检查器评估任务完成情况和五个临床安全维度。对23个智能体进行了评估。

Result: 最佳闭源模型达到54.2%的严格成功率，但所有模型在真实OpenEMR上均低于9%。开源智能体平均仅2.5%，最佳为16.2%。

Conclusion: 当前智能体与可靠的临床软件使用之间存在显著差距。MedCUA-Bench提供了可复现的测试环境，为未来研究奠定了基础。
...
### [165] [Effect of Demographic Bias on Skin Lesion Classification](https://arxiv.org/abs/2606.03214)
*Ralf Raumanns,Gerard Schouten,Veronika Cheplygina,Josien P. W. Pluim*

Main category: cs.AI

TL;DR: 本研究评估了ResNet卷积模型在皮肤病变分类中的性能，重点分析了训练数据中的人口统计学偏见(性别和年龄)。通过线性编程生成控制人口统计学特征的数据集，评估了三种学习策略的效果。


<details>
  <summary>Details</summary>
Motivation: 解决皮肤病变分类模型中存在的性别和年龄偏见问题，确保模型在不同人口统计学群体中的公平性和准确性。

Method: 使用线性编程生成具有控制人口统计学特征的数据集，评估三种学习策略:单任务模型、强化多任务模型和对抗学习方案，并在两个外部数据集上进行跨域验证。

Result: 性别偏见主要由数据不平衡引起，对抗学习和强化多任务学习可有效消除平衡和女性为主数据集中的偏见，但在男性为主数据集中效果有限。年龄偏见始终有利于年轻组，与数据分布无关。跨数据集验证显示领域转换显著影响性能和偏见模式。

Conclusion: 性别和年龄偏见具有不同的形成机制，需要针对性的缓解策略。由于领域转换的影响，在实际应用中需考虑模型的可迁移性和泛化能力。
...
### [166] [Perceive Before Reasoning: A Pre-Reasoning Perception Framework for Efficient and Reliable Proactive Mobile Agents](https://arxiv.org/abs/2606.03236)
*Zhijie Ding,Weinan Hong,Zicheng Zhu,Lei Li,Dezhi Kong,Hao Wang,Peng Zhou,Xuchu Jiang,Jiaming Xu*

Main category: cs.AI

TL;DR: 本文提出预推理感知框架(PRPF)，解决移动端多模态大语言模型中主动干预时机决策与帮助生成之间的目标冲突问题，通过引入轻量级的多模态主动感知器(MPP)进行干预门控和上下文压缩，只有在需要干预时才激活主动Agent推理器(PAR)。


<details>
  <summary>Details</summary>
Motivation: 现有系统将"何时干预"和"如何帮助"两个决策统一在MLLM管道中实现，导致保守的干预过滤与全面的帮助生成之间存在目标错位，且当智能体应该保持沉默时会产生冗余推理。

Method: 提出两阶段框架PRPF，包含：1)轻量级多模态主动感知器(MPP)用于干预门控和上下文压缩；2)主动智能体推理器(PAR)，仅当干预被触发时才激活。

Result: 在ProactiveMobile基准上的实验表明，PRPF显著降低了误触发率(FTR)，同时提升了成功率(SR)和推理效率。

Conclusion: 通过分离感知与推理阶段，PRPF有效解决了移动智能体中主动干预决策的目标冲突问题，实现了更精准的干预门控和更高的计算效率。
...
### [167] [Do Real-World Datasets Contain Natural Experiments? An Empirical Study Using Causal Feature Selection](https://arxiv.org/abs/2606.03251)
*Gautam Gare,John Galeotti,Michael Mozer,Deva Ramanan,Nan Rosemary Ke*

Main category: cs.AI

TL;DR: 研究如何检测和利用真实世界数据集中隐含的自然实验，提出基于因果发现的检测方法，并通过实证验证了真实数据集确实存在自然实验这一假设。


<details>
  <summary>Details</summary>
Motivation: 自然实验在现实中广泛存在（如COVID-19疫情），但尚不清楚现有真实世界数据集中是否也包含自然实验，以及如何识别和利用这些自然实验来提升模型性能。

Method: 首先使用因果发现算法恢复底层因果图并基于因果链接进行特征选择；然后通过比较将数据视为干预性 versus 观察性数据时的下游模型性能来判定是否存在自然实验；在合成数据集上验证假设，最后在大量真实数据集上进行系统实证评估。

Result: 在合成数据和真实数据上的实验结果表明：(1)真实世界数据集确实包含自然实验；(2)通过因果推断方法处理这些自然实验可以显著改善模型性能。

Conclusion: 本研究填补了自然实验检测领域的空白，证实了真实数据集中存在可被利用的自然实验，为后续更广泛的探索奠定了基础。
...
### [168] [Distilling Answer-Set Programming Rules from LLMs for Neurosymbolic Visual Question Answering](https://arxiv.org/abs/2606.03269)
*Thomas Eiter,Nelson Higuera Ruiz,Johannes Oetsch*

Main category: cs.AI

TL;DR: 提出一种从大型语言模型中提取规则的方法，用于扩展VQA推理理论，仅需少量示例即可实现有效的规则学习。


<details>
  <summary>Details</summary>
Motivation: 模块化方法在VQA任务中具有可解释性优势，但当任务需求变化时，扩展或调整逻辑表示会给开发者带来负担。

Method: 通过提示LLM扩展初始的答案集程序(ASP)表达VQA推理理论，利用VQA数据集中的示例引导LLM，并通过ASP求解器的反馈验证和纠正错误规则。

Result: 在多个VQA数据集上证明了有效性，仅需少量示例即可从LLM中提取正确规则。

Conclusion: 从LLM中提取规则是传统数据驱动规则学习的有前景替代方案。
...
### [169] [A Negative Result on Cross-Model Activation Transfer in a Pythia Multi-Hop Setting](https://arxiv.org/abs/2606.03280)
*Peiyan Zhang*

Main category: cs.AI

TL;DR: 研究语言模型能否通过直接传递隐藏激活而非自然语言文本来进行推理状态的通信。发现在Pythia-160M到410M的多跳推理设置中，尽管可以实现高精度的表示对齐（cosine相似度达0.97），但这些转换后的激活无法改善接收模型的下游推理表现。


<details>
  <summary>Details</summary>
Motivation: 之前的工作表明语言模型可以通过生成数据中的隐藏信号传递行为特性，作者想知道是否存在更直接严格的通道：不通过自然语言，而是直接传递隐藏激活来实现模型间的推理状态传递。

Method: 在受控的Pythia-160M到Pythia-410M多跳推理设置中，使用线性翻译层学习发送方和接收方隐藏状态之间的归一化空间映射，并尝试多种注入方式：低强度加法注入、替换式注入、以及对翻译向量进行范数缩放。

Result: 线性翻译层能学习到强归一化空间映射，跨种子的归一化余弦相似度接近0.97。但将翻译的激活注入接收者后，下游问答表现未获改善——低强度加法注入仍接近无注入基线，置信区间跨越零点；替换式注入始终具有破坏性；将翻译向量缩放到接收者隐藏状态范数也未能挽救性能。

Conclusion: 这是一个有限范围的负结果：在该设置下，离线表示对齐不足以在接收者内部实现有用的因果通信。即使隐藏表示能够高度对齐，这种对齐也不能转化为有效的推理能力传递。
...
### [170] [LEAP: Supercharging LLMs for Formal Mathematics with Agentic Frameworks](https://arxiv.org/abs/2606.03303)
...
TL;DR: LEAP是一个使大型语言模型能够在Lean形式化定理证明中达到最先进水平的智能体框架，通过将复杂问题分解为更小单元，结合非正式推理与形式化验证，在2025年Putnam竞赛上解决全部12道题，并在新基准Lean-IMO-Bench上将一次性形式求解率从不足10%提升至70%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在非正式数学推理方面表现强劲，但在生成可机械验证的形式化证明（如Lean语言）方面存在困难。现有基准日益饱和，需要更加严格的评估标准来衡量模型的正式定理证明能力。

Method: LEAP框架利用基础模型的能力（非正式推理、指令遵循、迭代自我 refinement），通过将复杂问题分解为更小单元，系统性地架设非正式推理蓝图与形式化证明构建之间的桥梁，并通过与Lean编译器的持续交互实现验证与修正。

Result: 在2025年Putnam竞赛上解决全部12道题；在Lean-IMO-Bench上，将通用LLM的一次性形式求解率从低于10%提升至70%，显著超越了专业IMO系统设定的48%基准线；并成功自主形式化了Knuth偶阶Cayley图Hamiltonian分解的关键子问题。

Conclusion: LEAP证明了智能体框架在形式化数学证明方面的有效性，实现了研究级别的实用价值，能够自主形式化复杂证明，为自动形式化定理证明领域树立了新的里程碑。
...
### [171] [The Reliability Gap in Benchmark Auditing: Distribution Shift and Scale as Failure Modes of Contamination Detection](https://arxiv.org/abs/2606.03305)
*Wojciech Zarzecki,Jan Dubiński,Sebastian Cygert*

Main category: cs.AI

TL;DR: 论文研究了大型语言模型训练数据污染检测方法的实用性，发现现有统计方法在真实审计场景中存在系统性可靠性缺陷。


<details>
  <summary>Details</summary>
Motivation: 基准污染是LLM评估中的严重问题——评估例子可能混入训练数据，导致性能虚高。现有的成员资格检测方法（如LLM Dataset Inference、Post-Hoc Dataset Inference、CoDeC）仅在受控的学术环境中验证过（如大规模同质预训练语料库、透明单阶段训练流程），在现实审计场景中是否仍可靠尚不清楚。

Method: 系统评估三种领先检测范式（LLM Dataset Inference、Post-Hoc Dataset Inference、CoDeC）在27个不同模型家族（包括Pythia、OLMo 2及专业文化/医学LLM）上的表现，模型规模达27B，并扩展到前沿工业模型，总计335次评估。

Result: 三种方法均存在显著问题：LLM Dataset Inference在分布偏移下产生误报，Post-Hoc Dataset Inference统计功效不足，CoDeC仅能提供粗粒度来源信号。在335次评估中只有59%（199次）产生正确结果。

Conclusion: 统计检测方法与透明数据溯源之间存在系统性可靠性差距，当前统计检测尚无法取代透明的数据来源披露。
...
### [172] [The Violation Situation Pattern: A Knowledge-Graph Pattern for Compliance Violations](https://arxiv.org/abs/2606.03326)
*Nima Kamali Lassem,Fuqi Song,Seyid Amjad Ali*

Main category: cs.AI

TL;DR: 论文提出违规情况模式(VSP)，将检测到的违规重新实现为具有规则标识符、时间有效性、生命周期状态和证据链接的持久图对象，解决现有合规管道无法保留违规审计历史的问题。


<details>
  <summary>Details</summary>
Motivation: 现有合规管道将违规检测为瞬态查询结果，不保留违规本身作为具有审查状态、受影响实体和审计历史的持久图对象，导致无法进行完整的审计追踪。

Method: 基于Gangem和Mika的情况模式构建VSP，将违规重新实现为图节点，包含规则ID、时间有效性间隔、生命周期状态和证据链接；生命周期转换为不可变的PROV-O对齐事件；在法律实体和合同生命周期属性图中实例化，通过FCL->Cypher->MERGE管道操作化四个义务规则，并在BODACC、GDPRhub数据上测试，使用SHACL进行跨形式主义检查。

Result: 将V4从条款存在扩展到截止日期检查使F1分数从0.312提高到0.602，同时模式的身份、生命周期和证据语义保持不变，证明规则体独立性可以分离模式贡献和检测器贡献。

Conclusion: VSP成功实现了违规的持久化表示和审计历史图遍历；规则体独立性允许检测逻辑演进而不影响已积累的审计历史，为合规检测提供了模式与检测器分离的架构。
...
### [173] [InfoMem: Training Long-Context Memory Agents with Answer-Conditioned Information Gain](https://arxiv.org/abs/2606.03329)
*Tiancheng Han,Yong Li,Wuzhou Yu,Qiaosheng Zhang,Wenqi Shao*

Main category: cs.AI

TL;DR: InfoMem是一种针对长上下文任务中分块记忆智能体的奖励机制，通过评估最终记忆对真实答案的token级别对数似然增加量来提供奖励信号，帮助模型更有效地积累和利用上下文信息。


<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的分块记忆智能体仅依赖稀疏的最终答案奖励或词汇级中间奖励，无法直接评估最终记忆是否真正支撑了正确答案。需要在最终记忆层面提供更直接的监督信号。

Method: InfoMem提出答案条件的信息奖励机制，衡量最终记忆对模型预测真实答案的per-token log-likelihood的提升程度。该信号仅应用于成功轨迹，并在奖励组合前进行归一化处理，以稳定强化学习优化。

Result: 在相同GRPO框架和训练预算下，InfoMem相比其他记忆智能体强化学习基线方法提升了长上下文记忆智能体的性能。分析表明有效的最终记忆奖励应仅在成功轨迹上操作。

Conclusion: 答案条件且经过归一化处理的最终记忆奖励机制能够显著改善分块记忆智能体在长上下文任务上的表现，为训练更高效的记忆智能体提供了新的奖励设计思路。
...
### [174] [CP-Agent: Context-Aware Multimodal Reasoning for Cellular Morphological Profiling under Chemical Perturbations](https://arxiv.org/abs/2606.03435)
*Yuxin Zhang,Yiyao Li,Ping Shu Ho,Simon See,Zhenqin Wu,Kevin Tsia*

Main category: cs.AI

TL;DR: 本文介绍了CP-Agent，一种用于细胞成像（Cell Painting）的多模态大语言模型，能够生成药物诱导下细胞形态变化的可解释性理由。核心模块CP-CLIP联合嵌入高内涵图像和实验元数据，实现治疗和作用机制（MoA）区分。


<details>
  <summary>Details</summary>
Motivation: 现有Cell Painting工作流程缓慢、昂贵且难以解释；当前药物筛选建模方法主要关注分子表示学习，忽略了实际实验背景（如细胞系、给药方案等），限制了模型的泛化能力和作用机制分辨率。

Method: CP-Agent通过上下文感知对齐模块CP-CLIP，联合嵌入高内涵图像和实验元数据，实现稳健的治疗和MoA鉴别；结合智能体工具使用和推理能力，将理由编译成结构化报告以指导实验设计和假设优化。

Result: CP-CLIP在治疗和MoA区分上达到了最高0.896的F1分数

Conclusion: CP-Agent有望加速药物发现，使表型筛选更加可解释、可扩展和具上下文意识——简化药物发现中的迭代假设生成循环
...
### [175] [What Makes Interaction Trajectories Effective for Training Terminal Agents?](https://arxiv.org/abs/2606.03461)
...
TL;DR: 研究代码智能体在post-training中的教学效果，发现standalone性能不决定教学质量的"教育学悖论"，提出Environment-Grounded Supervision (EGS)方法，仅用15.3k数据即达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前假设更强的代码智能体是更好的教师，但这一假设未考虑任务难度、智能体容量等因素，需要验证教学效果与standalone性能的真正关系。

Method: 使用Terminal-Lego管道将真实世界问题转化为环境验证的智能体任务，比较不同智能体(Claude Opus 4.6 vs DeepSeek-V3.2)的教学轨迹效果，进行scaling分析。

Result: DeepSeek-V3.2(较低分数)训练的学生比Claude Opus 4.6(更高分数)训练的学生泛化更强；Qwen3-32B仅用15.3k数据就在Terminal-Bench 2.0上达到24.3%分数，媲美之前需30x数据的SOTA。

Conclusion: 教学效果取决于显式暴露inspect-act-verify行为的轨迹，而非分数本身；未来关键是"Harness Engineering" - 系统设计环境grounded交互结构而非outcome-matching。
...
### [176] [DMF: A Deterministic Memory Framework for Conversational AI Agents](https://arxiv.org/abs/2606.03463)
*Matteo Stabile,Enrico Zimuel*

Main category: cs.AI

TL;DR: 本文提出DMF框架，用确定性数学评分替代LLM生成式压缩，实现零tokens准备上下文，5x-242x降低整体token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有对话AI记忆系统依赖LLM进行写时摘要，导致非确定性、token成本激增和修剪决策不透明的问题。

Method: DMF采用经典NLP分析、向量几何和数学评分替代生成式压缩。每个对话交互计算生存分数Ω（逻辑投影），通过交互数量衰减律Ω_eff(Δn)而非wall-clock时间驱动相关性演化，形成结构化召回管道和修剪决策流程。

Result: 在LoCoMo和LongMemEval数据集上与Mem0对比，DMF达到相当准确率，记忆上下文准备零tokens，全程消耗降低5倍至242倍。

Conclusion: 可从记忆管理循环中完全消除LLM调用，将token成本降至近零，实现完全确定性的对话AI记忆系统。
...
### [177] [StepFinder: A Temporal Semantic Framework for Failure Attribution in Multi-Agent Systems](https://arxiv.org/abs/2606.03467)
*Taiyu Zhu,Yifan Wu,Weilin Jin,Ying Li,Gang Huang*

Main category: cs.AI

TL;DR: 提出了StepFinder，一个轻量级失败归因框架，用于在LLM多智能体系统中自动识别导致级联故障的根因步骤。该方法先用LLM编码执行日志为时序语义序列，再用参数高效的时序建模和注意力模块捕捉轨迹的跨步骤依赖性，最后通过多尺度差异和位置偏差精炼步骤级错误分数。


<details>
  <summary>Details</summary>
Motivation: 现有失败归因方法完全依赖LLM推理原始执行轨迹，存在高推理成本和高延迟问题，且冗余噪声的执行日志干扰LLM准确识别真正根因步骤，导致效率和准确性不足。

Method: StepFinder分为三阶段：1) 仅在特征构建阶段使用LLM将执行日志编码为时序语义序列；2) 采用参数高效的时序建模和注意力模块组合，捕捉轨迹的时序演化和跨步依赖；3) 通过多尺度差异和位置偏差精炼步骤级错误分数，实现精确根因识别。

Result: 在Who&When基准上的实验表明，StepFinder在步骤级失败归因上优于基于LLM的方法，推理效率显著提升，比最快的LLM方法减少79%推理时间，且无文本生成开销。

Conclusion: StepFinder是一种高效的失败归因框架，通过分离LLM使用和轻量级时序建模，实现了高精度根因识别和高效率推理的平衡，为提升LLM多智能体系统可靠性提供了新思路。
...
### [178] [ThoughtFold: Folding Reasoning Chains via Introspective Preference Learning](https://arxiv.org/abs/2606.03503)
*Ziyan Liu,Xueda Shen,Yuzhe Gu,Songyang Gao,Kuikun Liu,Guangran Cheng,Chengqi Lyu,Dahua Lin,Wenwei Zhang,Kai Chen*

Main category: cs.AI

TL;DR: ThoughtFold框架通过细粒度偏好学习解决大型推理模型的过度思考问题，将冗长的思维链折叠成高效简洁的推理路径。


<details>
  <summary>Details</summary>
Motivation: 长思维链(CoT)自然包含试错过程，主流RLVR方法选择结果正确的轨迹进行记忆化，导致冗余探索被强化产生过度思考问题。之前方法给短轨迹优势但仍基于结果学习，无法减少冗余探索的记忆化。

Method: ThoughtFold采用内省策略识别每个正确轨迹中的冗余性，生成候选子轨迹谱；引入掩码偏好优化目标明确惩罚冗余探索，鼓励模型直接连接基本推理段，将推理链折叠成更简洁路径。

Result: 显著提升效率，DeepSeek-R1-Distill-Qwen-7B令牌使用减少约56%同时保持最先进准确性。

Conclusion: ThoughtFold通过细粒度偏好学习有效缓解冗余探索，实现高效推理，为LRM效率优化提供新思路。
...
### [179] [Overlaying Governance: A Compositional Authorization Framework for Delegation and Scope in Agentic AI](https://arxiv.org/abs/2606.03518)
*Amjad Ibrahim,Yong Li*

Main category: cs.AI

TL;DR: 针对传统IAM系统无法支持具备自主行动、协作和任务委托能力的智能体AI的问题，提出了一种组合治理框架，引入了资源范围衰减、可递归委托链等原语，为智能体AI提供可追责的授权语义。


<details>
  <summary>Details</summary>
Motivation: 传统授权框架基于固定主体、显式请求和静态权限范围，无法治理具备自主性和协作能力的智能体AI系统。现有身份与访问管理系统缺乏递归委托、上下文边界和动态作用域等机制。

Method: 将委托视为契约条款而非静态令牌同意凭证，定义委托类型及其权限和追责含义，引入资源范围衰减来约束智能体访问范围。通过组合算子将新的智能体语义叠加到现有关系策略上。

Result: 提供了一种形式化且实用的智能体AI可追责授权基础，可应用于金融系统等领域。

Conclusion: 该框架通过形式化证明和实证评估验证了其有效性，为智能体AI系统的可追责授权提供了正式而实用的基础。
...
### [180] [SAGE: A Quantitative Evaluation of Socialized Evolution in Agent Ecosystems](https://arxiv.org/abs/2606.03544)
*Linyue Pan,Yaoming Zhu,Lin Qiu,Xuezhi Cao,Xunliang Cai*

Main category: cs.AI

TL;DR: 研究自改进语言智能体在社会环境中是否能获得超越单独自我改进的优势。引入SAGE框架，比较社交协作(SelfEvo)和独立自改进(SelfEvo)在三个领域的表现。


<details>
  <summary>Details</summary>
Motivation: 探索共享经验是否能让智能体实现自我改进无法达到的突破。这是一个未被充分研究的问题：同伴经验能否帮助那些在孤立环境中陷入瓶颈的智能体取得进步。

Method: 构建SAGE评估框架，在三个任务领域（开放性机器学习研究、长期经济规划、策略性多人游戏中）比较两种计算匹配的条件下五个不同模型家族的表现。

Result: 发现群体历史并非通用放大器，最强智能体不会超过其自我进化上限。但对于在自我改进中陷入瓶颈的智能体，同伴经验的引入能带来显著突破。在竞争环境中，智能体普遍改进而非发展对手特定策略。经过抽象处理的同伴轨迹和反思性摘要往往优于原始日志。

Conclusion: 同伴历史增益具有智能体特异性、领域依赖性，取决于从公共轨迹中提取可迁移知识的能力。社交增益依赖于信息抽象而非暴露量。
...
### [181] [From Prompt to Service: An SLM-Based Agent Orchestration Gateway for AI-Driven Virtual Worlds](https://arxiv.org/abs/2606.03557)
*Louis Nisiotis,Aimilios Hadjiliasi*

Main category: cs.AI

TL;DR: 本文提出一种基于SLM的智能体编排网关(SLM-based Agent Orchestration Gateway)，通过在边缘部署的小型语言模型对用户提示进行语义意图分类，结合可配置服务注册中心实现动态路由，使虚拟世界客户端能够透明调用分布式AI后端服务，无需修改客户端应用即可扩展AI能力。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI能力扩展，虚拟世界面临日益复杂的架构挑战。用户以多模态方式交互，但请求需要不同的AI后端模型和计算资源。直接将AI能力嵌入虚拟世界系统会降低可扩展性、增加维护难度，并限制跨边缘和云基础设施的服务协调能力。

Method: 提出基于SLM的智能体编排网关方案：1)边缘部署的SLM对用户提示进行语义意图分类；2)可配置服务注册中心验证并解析路由决策；3)透明调用选定的AI后端。在InterwovenXR虚拟博物馆测试床中实现并评估了该网关。

Result: 紧凑型SLM可作为边缘硬件上的可靠意图路由器；任务特定微调可将子十亿参数模型转变为实用低延迟路由器；分层配置(微调子十亿参数模型作为路由器+较大SLM用于对话生成)可在中端边缘硬件上部署，比单一模型承担双重职责更高效。

Conclusion: SLM可以支持虚拟世界中实用的AI服务编排，研究贡献了一种可扩展、可扩展且支持边缘AI交互的架构，使虚拟智能体成为分布式生成式AI服务的访问点。
...
### [182] [Cross-Lingual Token Arbitrage: Optimizing Code Agent Context Windows via Local LLM Preprocessing](https://arxiv.org/abs/2606.03618)
*Mehmet Utku Colak*

Main category: cs.AI

TL;DR: 本文提出了一种边缘侧的预运行提示词重写中间件，通过本地小模型将非英语文本翻译成英语并将对话式提示结构化为紧凑任务格式，从而减少提示词 tokens 开销。


<details>
  <summary>Details</summary>
Motivation: AI 辅助编码代理受输入 token 成本限制，非英语文本的 tokenization 低效和对话提示的结构熵是造成开销的主要原因，现有方法只能被动应对。

Method: 使用本地 Llama 3.2 (3B) 模型执行跨语言翻译、结构重写为紧凑任务导向格式，并通过正则验证的回退机制确保优化后提示词不大于原始长度。在 OMH-Polyglot 多语言编码基准上评估。

Result: 在三种商业 LLM 后端上，提示词 tokens 减少 34-47%，总 tokens 减少高达 18.8%，同时保持或提升任务准确率。消融实验表明收益主要来自重写阶段而非简单函数名提取。相比 LLMLingua-2 在相同压缩率下实现更优的 OckScore 性能。

Conclusion: 主动式提示词优化可显著降低推理成本而不牺牲编码质量，该方法为边缘端主动优化提供了有效范式。
...
### [183] [Bridging Auxiliary Constraints to Resolve Instruction Following in Large Reasoning Models](https://arxiv.org/abs/2606.03624)
*Zhengyi Zhao,Shubo Zhang,Huimin Wang,Zezhong Wang,Yutian Zhao,Yefeng Zheng,Binyang Li,Yulan He,Kam-Fai Wong,Xian Wu*

Main category: cs.AI

TL;DR: 本文提出约束关系图补全(CRGC)框架,通过构建约束知识显式建模指令间的关係,并识别"桥接约束"来帮助大语言模型更好地满足多个冲突指令,从而解决大型推理模型的约束遵守问题。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在同时遵循多条指令时表现不佳,常出现约束违反或难以平衡相互竞争的约束,该问题被称为约束遵守问题(CAP),现有方法仅通过通用训练增强指令跟随能力,缺乏对约束间关係的显式建模。

Method: 将指令表示为结构化知识图,通过约束关系图补全(CRGC)框架显式建模约束间的关係,识别遵守挑战并发现能提高主要约束显着性和兼容性的"桥接约束",利用模型自身知识创建更好的生成路径。

Result: 在三个流行指令跟随数据集上实验表明,与标准提示相比,本方法可将约束违反降低39%,同时保持大型推理模型的推理能力。

Conclusion: CRGC框架通过约束知识图和桥接约束机制有效解决了多约束场景下的指令跟随难题,为提升大型推理模型的约束遵守能力提供了新思路。
...
### [184] [TSQAgent: Rating Time Series Data Quality via Dedicated Agentic Reasoning](https://arxiv.org/abs/2606.03629)
*Shunyu Wu,Dan Li,Haozheng Ye,Weibin Feng,Jian Lou,Bo Zhang,Wenjie Feng,Chenjuan Guo,See-Kiong Ng*

Main category: cs.AI

TL;DR: 本文提出了TSQBench基准和一个名为TSQAgent的新型agent框架，用于评估和改进大语言模型在时间序列数据质量评估方面的能力。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在时间序列质量评估方面存在两个关键问题：1)无法自主识别真正相关的质量维度；2)缺乏基于证据的定量质量比较能力。现有方法依赖于预定义的质量维度和纯文本推理，限制了实际应用效果。

Method: 1)构建TSQBench基准测试集，评估LLM的两项渐进能力：理解识别相关质量维度和特定维度下的质量比较；2)提出TSQAgent框架，包含三个协作角色——Perceiver(感知器)负责聚焦选择相关维度，Inspector(检查器)负责维度级定量分析，Adjudicator(仲裁器)负责聚合 refine最终判断；3)引入agentic推理策略使LLM具备识别和优先排序最相关质量维度的能力，并配备外部分析工具进行精确的定量比较。

Result: 实验在TSQBench基准和11个真实世界数据集上验证显示：TSQAgent不仅显著提升了LLM的质量理解能力和定量比较能力，还有效转化为更好的质量感知数据选择，从而提升了下游任务性能和数据效率。

Conclusion: 通过引入协作式agent架构和外部工具支持，TSQAgent成功解决了现有LLM在时间序列质量评估中的维度识别和定量比较难题，为质量感知的数据选择提供了有效方案，具有重要的实践应用价值。
...
### [185] [Gender-Dependent Diagnostic Substitution in LLM Medical Triage: Same Symptoms, Unequal Urgency](https://arxiv.org/abs/2606.03641)
*Qi Han Wong*

Main category: cs.AI

TL;DR: 研究大型语言模型在医疗分诊中是否存在性别和年龄相关的偏见。三个模型（Gemini、Claude、GPT）对相同神经系统症状的年轻女性给出更低急诊转诊率，因为模型倾向于用与性别关联的流行病学诊断（如特发性颅内高压）来"解释"女性患者症状，导致分诊降级。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型已逐步应用于临床决策支持，但它们可能复制人类临床中已知的偏见。现实中，年轻女性患者的疼痛常被低估或误诊。该研究旨在检验AI模型是否同样存在这种系统性偏见。

Method: 使用标准化症状描述（持续头痛、视力模糊、晨吐、视觉障碍），在三种模型家族中进行630次实验，覆盖三种年龄（25、38、65岁）和两种性别（男、女）及性别未指定的基线条件，测量急诊转诊率和诊断分类。

Result: 发现显著的性别依赖性分诊差异：年轻女性获得远低于男性的急诊转诊率（Gemini: 0% vs 23.3%；Claude: 6.7% vs 96.7%；GPT: 6.7% vs 66.7%，p<0.001），但在65岁时差异消失。核心机制是诊断替换——模型优先将年轻女性归因于特发性颅内高压（IIH），而将男性归因于更具紧迫性的占位性病变，尽管严重程度评分相当（7-9/10）。

Conclusion: 临床LLM复制了已知的人类临床偏见，通过流行病学先验压制分诊紧迫性。AI分诊引擎必须将紧迫性评估与概率性诊断先验解耦，否则可能导致对女性患者的系统性护理不足。
...
### [186] [Towards Non-Monotonic Entailment in Propositional Defeasible Standpoint Logic](https://arxiv.org/abs/2606.03655)
*Nicholas Leisegang,Thomas Meyer,Ivan Varzniczak*

Main category: cs.AI

TL;DR: 本文将KLM风格的可废止蕴含关系从命题逻辑提升到PDSL分支，通过引入情境立场条件句，实现了非单调蕴含的形式化，并保持了算法复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有可废止推理研究主要关注可满足性检查和单调蕴含，推理能力较弱。需要将非单调的理性蕴含关系扩展到命题立场逻辑(PDSL)中，以增强表达能力。

Method: 通过引入情境立场条件句(situated standpoint conditionals)来扩展PDSL的表达能力，允许在给定立场上下文中讨论可废止条件句。将基于排序的蕴含关系(理性闭包和词汇闭包)从命题情况转移到PDSL情况。

Result: 大部分PDSL可表达为情境条件句集合；非单调蕴含检查可主要使用命题情况的算法，同时保持复杂度界限；实现了理性闭包和词汇闭包的忠实翻译。

Conclusion: 成功将非单调蕴含机制嵌入到模态立场逻辑中，为PDSL提供了更强的推理能力，同时保持了计算可行性。
...
### [187] [Diagnosing Knowledge Gaps in LLM Tool Use: An Agentic Benchmark for Novel API Acquisition](https://arxiv.org/abs/2606.03657)
*Jinnuo Liu,Yue Peng,Jinhan Niu,Hongyi Wen*

Main category: cs.AI

TL;DR: NovelAPIBench是一个全自动动态基准测试，用于评估大语言模型使用新API的能力。实验发现：使用示例是最强信号；签名+机制/示例的两组件组合效果最佳；添加源码会因导入路径错误反而降低性能；参数化微调不替代检索而是互补关系。


<details>
  <summary>Details</summary>
Motivation: 现有新型API基准测试通常为静态、使用粗糙的通过/失败指标、或使用合成API，无法真实反映实际库演变。同时大语言模型生成代码需要协调多种API知识组件（签名、模块路径、输入输出契约、语义等），而现有研究缺乏对不同知识组件可互换性的系统分析。

Method: 构建了完全自动化的动态基准测试NovelAPIBench，能够针对任何基础模型和目标库发现新API、提取分解的知识包、生成可执行的编码任务，并将失败样本分配到六个诊断类别。在约1900个任务、4个基础模型和5个域中进行对比实验，比较检索增强与参数化微调两种知识注入方式。

Result: 知识组件不可互换：使用示例是最强的独立信号；最佳两组件设置取决于领域和骨干网，为签名+机制或签名+示例；增加上下文尤其是源码可能因导入路径错误而损害性能；参数化微调在外部知识移除后不能替代检索，微调主要教授模型如何使用提供的知识包，且该能力可迁移到未知库。

Conclusion: 检索和微调扮演互补角色：检索提供易失的API内容，而微调改善程序整合。未来设计AI编程助手时需同时考虑两者的协同作用，而非单纯依赖某一种方法。
...
### [188] [From Answers to States: Verifiable Process-Level Evaluation of Chemical Reasoning in Large Language Models](https://arxiv.org/abs/2606.03660)
*Hongyu Guo,Hao Li,He Cao,Gongbo Zhang,Li Yuan*

Main category: cs.AI

TL;DR: 本文介绍了ChemCoTBench-V2，一个用于评估大型语言模型化学推理过程的可验证诊断基准。


<details>
  <summary>Details</summary>
Motivation: 现有化学基准只评估最终答案，掩盖了推理过程错误；现有过程级评估成本高、不一致且易产生幻觉，需要低成本、可审计的方法。

Method: 构建包含5,620样本、18任务的基准，涵盖分子理解/编辑/优化/预测；要求模型展示中间步骤并用化学规则或参考轨迹验证；开放任务用oracle验证。

Result: 前沿模型存在最终答案与推理一致性差距：模型常遵循格式但未通过化学检查，或用弱推理正确回答。

Conclusion: 该基准支持细粒度模型比较，可识别推理首次违反验证器的具体步骤。
...
### [189] [EvoDrive: Pareto Evolution for Safety-Critical Autonomous Driving via Self-Improving LLM Agents](https://arxiv.org/abs/2606.03678)
*Tong Nie,Yuewen Mei,Yihong Tang,Junlin He,Jie Deng,Jian Sun,Wei Ma*

Main category: cs.AI

TL;DR: EvoDrive是首个基于LLM的智能体进化框架，用于自动驾驶场景生成中的多目标对抗性场景生成，通过模拟器驱动的Actor-Critic架构和Pareto存档机制平衡攻击有效性与现实真实性。


<details>
  <summary>Details</summary>
Motivation: 安全关键场景生成对自动驾驶系统验证至关重要，但现有方法倾向于将多目标张力简化为单标量最大化，且缺乏严格的模拟器 grounding，导致生成的场景要么过于简单（依赖手工启发式），要么不切实际。

Method: EvoDrive采用记忆驱动的Actor迭代改进生成器，Critic过滤不可行候选，自进化世界评估器路由有前景的提案并优化模拟预算，同时维护Pareto存档以保留多样化的攻击-现实权衡并通过模拟反馈指导进化。

Result: 在MetaDrive和CARLA基准测试中，EvoDrive显著扩展了各种生成器的Pareto前沿，并产生了对策略训练有价值的场景。

Conclusion: 该框架首次成功地将LLM智能体与模拟器约束相结合用于安全关键自动驾驶场景生成，为多目标优化问题提供了有效的自动化解决方案。
...
### [190] [The DeepSpeak-Agentic Dataset](https://arxiv.org/abs/2606.03686)
*Sarah Barrington,Maty Bohacek,Hany Farid*

Main category: cs.AI

TL;DR: 提出了DeepSpeak-Agentic数据集，包含超过37小时的人机对话视频，用于评估AI智能体的自动法医鉴定并提供基准测试。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型和AI生成语音/面容技术的发展，需要构建专门的数据集来研究和评估AI智能体的法医识别问题，同时现有缺乏针对具身AI智能体的大规模人机交互基准数据集。

Method: 构建了一个可扩展的数据捕获系统，该系统能够创建AI智能体、自动将智能体与人类众包工作者配对、记录指定场景下的视听对话，并从混合流中分离人和智能体。

Result: 生成了包含超过37小时的半结构化对话的DeepSpeak-Agentic数据集，可用于音频、视频或文本模式的AI智能体自动法医鉴定，为研究人机交互本质以及未来大型语言模型和AI生成语音/面容技术的进步提供基准。

Conclusion: 该数据集和配套系统为AI智能体法医识别领域的研究提供了重要的资源基础，有助于推动具身AI智能体技术的进一步发展。
...
### [191] [SkillPyramid: A Hierarchical Skill Consolidation Framework for Self-Evolving Agents](https://arxiv.org/abs/2606.03692)
*Yuan Xiong,Ziqi Miao,Qian Chen,Lijun Li,Yequan Wang,Shizhu He,Jun Zhao,Kang Liu*

Main category: cs.AI

TL;DR: 提出SkillPyramid框架，通过层次化技能拓扑和自演化机制实现技能的动态构建、积累与迁移，解决AI代理长期能力提升受限的问题。


<details>
  <summary>Details</summary>
Motivation: 解决AI代理缺乏系统性技能构建、积累和转移机制的问题，避免重复构建相似能力，无法将经验转化为可复用资产。

Method: 构建层次化技能拓扑，引入自演化机制（技能组合、验证、整合），在任务执行过程中动态更新技能库。

Result: 在ALFWorld、WebShop和ScienceWorld上测试，平均奖励提升38.0%，执行步骤减少27.7%。

Conclusion: SkillPyramid 将技能集合从静态资源池转变为动态进化系统，显著提升AI代理的任务完成效率和泛化能力。
...
### [192] [Dynamic Objective Selection with Safeguards and LLM Oversight for Financial Decision-Making](https://arxiv.org/abs/2606.03704)
*Keigo Sakurai,Takahiro Ogawa,Miki Haseyama,Anjyu Anan,Kei Nakagawa*

Main category: cs.AI

TL;DR: DOSS（动态目标选择与保障）是一种学习型选择器，直接从近期收益的统计摘要中选取相关决策目标函数（如追求回报、损失厌恶、风险调整后收益），无需引入中间 regime 变量，并通过置信度感知门控和切换频率控制来防止错误选择和过度切换。


<details>
  <summary>Details</summary>
Motivation: 金融决策任务如股票推荐和投资组合配置需要估计未来收益和风险，但固定目标在不同的市场环境下可能次优；依赖潜在 regime 估计的切换管道可能存在噪声或延迟，且频繁切换会增加换手率和运营不稳定风险。

Method: 将目标选择表述为分类问题，使用滚动窗口进行顺序更新以实现前瞻性选择而不产生时间泄露；输出置信度分数；应用置信度感知门控配合失效保护，将低置信度提议覆盖至保守默认值；强制执行切换频率的显式控制；集成LLM作为监督组件，仅接受提议目标或覆盖至预定义安全默认值，触发确定性规则约束。

Result: 提出了DOSS框架，整合了分类器、置信度机制、保障约束和LLM治理监督，用于动态选择金融决策目标函数。

Conclusion: DOSS通过直接学习目标选择而非间接估计regime，结合多重保障机制和LLM监督，为金融决策中的目标适配提供了可解释且稳健的解决方案。
...
### [193] [Code-on-Graph: Iterative Programmatic Reasoning via Large Language Models on Knowledge Graphs](https://arxiv.org/abs/2606.03705)
*Weiwei Ding,Zixuan Li,Long Bai,Zhuo Chen,Kun Su,Fei Wang,Xiaolong Jin,Jin Zhang,Jiafeng Guo,Xueqi Cheng*

Main category: cs.AI

TL;DR: 提出了Code-on-Graph (CoG)框架，通过将知识图谱模式表示为Python类并生成可执行代码，实现灵活且可扩展的大型语言模型与知识图谱集成，解决传统方法的不灵活性和不可扩展性瓶颈。


<details>
  <summary>Details</summary>
Motivation: 解决现有LLM-KG集成框架的两个关键瓶颈：1)预定义操作符缺乏组合表达力，无法完全捕捉复杂语义；2)直接注入大量事实知识到提示中限制了可扩展性。

Method: 给定每个推理步骤检索的事实知识，CoG首识别相应KG模式并表示为Python类作为抽象接口，然后生成基于这些类的可执行代码，将检索事实实例化为类的对象进行执行，实现灵活的代码推理同时避免直接注入大规模知识。

Result: 在WebQSP、CWQ和GrailQA数据集上实验，CoG性能比之前最优模型提升高达10.5%。

Conclusion: CoG通过编程式推理方法成功解决了传统LLM-KG集成的不灵活性和不可扩展性问题，为大型语言模型与知识图谱的深度融合提供了新的有效范式。
...
### [194] [Unveiling the Structure of Do-Calculus Reasoning via Derivation Graphs](https://arxiv.org/abs/2606.03719)
*Clément Yvernes,Emilie Devijver,Marianne Clausel,Eric Gaussier*

Main category: cs.AI

TL;DR: 本文引入了导出图(derivation graphs)来表示do-calculus规则的应用和组合方式，并描述了在do-calculus下等价的观测和干预概率的完整空间。研究提出了一种最多使用四次do-calculus规则的简单程序，以及如何从多个等价的因果查询中产生更有效的估计量。


<details>
  <summary>Details</summary>
Motivation: do-calculus定义了干预查询的推理系统，但其规则的应用和组合顺序具有挑战性，需要一种系统性的方法来处理丰富的等价表达式空间。

Method: 本文引入导出图结构来表示do-calculus规则的应用和组合方式，通过对这些图结构的分析来得到最多四次规则应用的简单程序。

Result: 提出了一个最多使用四次do-calculus规则的简单程序，展示了如何从多个等价的因果查询中产生多个有效的估计量，从而获得更高效的估计。

Conclusion: 通过引入导出图结构，本文提供了一个系统性的框架来处理do-calculus规则的应用和组合，为因果推断中的识别和估计问题提供了更有效的方法。
...
### [195] [When to Re-Plan: Subgoal Persistence in Hierarchical Latent Reasoning](https://arxiv.org/abs/2606.03741)
*Ayushi Chadha*

Main category: cs.AI

TL;DR: 研究隐含推理中的稳定-适应权衡，通过在HRM模型中加入封建式管理器-工作者界面，周期性发出持续P步的子目标，发现中等持续期P∈[3,6]表现最优，其中P=3达到最低LM损失1.544。


<details>
  <summary>Details</summary>
Motivation: 研究者试图解决长程推理中"过度重规划导致计算无法凝聚成多步结构"与"过度坚持导致计划过时"之间的根本矛盾。特别关注隐含推理 setting——其中多步计算发生在隐藏状态而非外化令牌追踪中。

Method: 扩展HRM模型，加入封建式管理器-工作者接口：慢速高层模块周期性发出标准化方向性子目标，持续P个低层步骤，偏置工作者的隐藏状态更新，并提供内在余弦对齐损失。

Result: 子目标持续期P是关键；P∈[3,6]中等人群一致优于极频繁(P=1)和极长视野，P=3时LM损失最低(1.544 vs 1.674)；内在对齐权重λ存在窄最优区(λ≈0.05)；消融实验表明学习到的方向性结构是超过最优时干扰的来源。

Conclusion: 组合规划的设计原则：隐含推理系统中，中等视野意图必须在足够计算步骤上保持连贯，才能形成组合结构。
...
### [196] [Proof-Refactor: Refactoring Generated Formal Proofs into Modular Artifacts](https://arxiv.org/abs/2606.03743)
*Yiming Fu,Peixuan Liu,Zichen Wang,Kun yuan*

Main category: cs.AI

TL;DR: 提出了一个名为Proof-Refactor的智能框架,用于改进LLM生成的Lean形式证明的质量,将证明重构分为四个阶段:提取候选证明片段、设计辅助声明、形式化验证组件、使用验证组件修复原证明。


<details>
  <summary>Details</summary>
Motivation: 当前LLM生成的形式证明在可读性、模块化、可维护性和可重用性方面与成熟形式数学库中的证明存在差距,这一问题部分源于现有证明生成流水线中隐含的"编译优先"目标,它鼓励产生单体或临时性的证明脚本而非库级别的产物。

Method: 提出了一种过程引导的方法,包含四个阶段:(1)提取候选证明片段,(2)设计辅助声明,(3)形式化证明提取和设计的组件,(4)使用验证组件修复原证明。在PutnamBench和Putnam2025上生成的Lean证明上进行了实验评估。

Result: Proof-Refactor在基于评分标准的重构质量上优于强大的Claude Code重构基线,其中在签名质量和人类可读性方面提升最大。

Conclusion: 过程引导的重构方法可以在不以证明长度为主要目标的情况下改善证明结构,为自动证明质量提升提供了新思路。
...
### [197] [LAP: An Agent-to-Instrument Protocol for Autonomous Science](https://arxiv.org/abs/2606.03755)
*Linwu Zhu,Liqiang Gao,Yan Chen,Dan Zhu,Jian Huang*

Main category: cs.AI

TL;DR: 本文提出Lab Agent Protocol (LAP)，填补现有agent生态系统协议中缺失的agent-to-instrument边缘，设计了包含InstrumentCard、reservation、safety-fence handshake和MeasurementResult schema四个物理世界原语的协议规范。


<details>
  <summary>Details</summary>
Motivation: 当前自主科学系统虽能利用大语言模型agent规划实验并通过自驱动实验室执行，但每个系统都需从头重建推理agent与物理仪器之间的链接，且面临碎片化的vendor SDK和为确定性软件客户端设计的标准。MCP标准化了agent-to-tool边缘，A2A标准化了agent-to-agent边缘，但均未建模agent-to-instrument边缘——该边缘具有状态性、安全临界性、排他拥有性、物理具身性，并产生带有单位、校准和不确定性的测量结果。

Method: LAP保留A2A的对等式、发现优先、任务生命周期结构，引入四个物理世界原语：(i) InstrumentCard——带签名的能力和物理限制描述；(ii) 一级预留——排他仪器和样品锁定；(iii) 安全栅栏握手——操作员确认令牌密码绑定到特定任务及其参数，管控危险和不可逆操作；(iv) MeasurementResult模式—— физическиtyped(QUDT/UCUM)、校准锚定、带不确定性、结构化可复现。并指定角色、六层架构、JSON-RPC方法集、任务和安全状态机、错误模型、跨实验室联邦，演示闭环自主Campaign。LAP与A2A/MCP生态系统传输兼容，封装而非取代SiLA 2和OPC UA等既有设备标准。

Result: 本文规范了LAP的角色定义、六层架构、JSON-RPC方法集、任务和安全状态机、错误模型和跨实验室联邦，通过端到端协议演示了一个闭环自主Campaign的执行流程。LAP与A2A/MCP生态系统传输兼容，能够封装SiLA 2和OPC UA等既有设备标准而非取代之。

Conclusion: LAP成功填补了agentic生态系统中缺失的agent-to-instrument边缘，通过引入四个物理世界原语实现了对状态性、安全临界性、排他拥有性、物理具身性和测量不确定性的完整支持。该协议保留了A2A的peer-to-peer和discovery-first特性，与现有A2A/MCP生态系统及SiLA 2、OPC UA等设备标准兼容，为自主科学从演示走向基础设施提供了关键的协议基础。
...
### [198] [From Control Boundary to Insurance Claim: Reconstructing AI-Mediated Losses Through the CER Framework](https://arxiv.org/abs/2606.03777)
*Alex Leung,Rex Zhang,Kentaroh Toyoda,SiewMei Loh*

Main category: cs.AI

TL;DR: 本文探讨了生成式AI或代理AI系统导致的AI损失保险问题。由于AI系统在推理、检索、调用工具和行动过程中状态不断变化，传统的"事件重建"方法不足，需要采用"状态重建"来评估损失。作者提出了CER框架，用于诊断AI残余风险转移，包括控制边界(Control boundary)、证据重建(Evidence reconstruction)和保险响应(Insurance response)三个维度。


<details>
  <summary>Details</summary>
Motivation: 传统保险理赔通常基于"事件重建"，关注"发生了什么损失"。但AI系统具有自主推理和行动能力，其状态在整个运行过程中持续变化，简单的"事件重建"无法捕捉因果链的完整图景。作者认为需要重新思考AI损失的根本性质：不仅要问"发生了什么损失"，还要问"系统被允许做什么"、"系统实际做了什么"、"重建的损失是否能支持保险索赔"。

Method: 本文提出了CER框架作为AI残余风险转移的用例级诊断工具。C维度检查系统是否具有可执行的运营边界；E维度评估系统状态和因果链能否从保留的Artifacts中重建；R维度确定重建的损失是否有保险覆盖，市场是否提供该保险，投保人是否已投保，以及支持保险索赔所需的证据。论文还引用了PocketOS、Replit代理数据库删除事件及Moffitt v. Air Canada案例作为公开示例。

Result: 本研究定义了AI特异性的重建问题，将CER框架操作化，并明确了AI重建所需的索赔级证据。通过建立这一框架，能够系统性地评估AI系统在致损过程中的因果关系，判断损失是否可被重建，以及是否满足保险索赔的条件。这为AI保险产品的设计提供了理论基础。

Conclusion: AI系统造成的损失需要"状态重建"而非传统意义上的"事件重建"。CER框架为评估AI残余风险转移提供了系统性诊断工具，明确了控制边界、证据重建和保险响应三个关键维度。作者呼吁保险公司和AI系统使用者关注AI特异性的风险敞口，同时指出在AI保险市场中存在证据要求和可保性方面的挑战。
...
### [199] [Enhancing Operational Safety via Agentic Dialogue Hazard Identification Analysis](https://arxiv.org/abs/2606.03812)
*Sanjay Das,Ran Elgedawy,Ethan Seefried,Ryan Burchfield,Tirthankar Ghosal*

Main category: cs.AI

TL;DR: HAZDIAL是一个研究多代理多轮对话是否能够改进基于NLP的危险识别质量的框架,通过对抗性辩论和建设性讨论两种对话模式与单次推理基线进行比较.


<details>
  <summary>Details</summary>
Motivation: 当前大规模语言模型在自动化安全分析任务中表现出潜力,但单次整体推理缺乏安全工程师应用的迭代式自我纠正、审议和情境细化能力,无法满足工业过程控制、自动驾驶等高风险领域的可靠危险识别需求.

Method: 提出两种对话模式(对抗性辩论和建设性讨论),设计基于算法的代理交互优化方案,在金标准数据集上使用标准分类指标(准确率、精确率、召回率、F1)和新型对话指标进行评估比较.

Result: 建立了一个精选的金标准数据集,系统比较了不同配置的系统性能,为对话驱动的危险分析提供了实证证据.

Conclusion: 该工作推进了对话系统、多代理推理和AI安全的交叉研究,为高风险领域的安全分析提供了新的方法论参考。
...
### [200] [Leveraging BART to Assess CS1 C++ Programming Assignments using Rubric-based Criteria](https://arxiv.org/abs/2606.03814)
*Kelsey Rainey,Jesse Roberts*

Main category: cs.AI

TL;DR: 本文研究使用BART编码器-解码器结合LoRA适配器对C++编程作业进行自动评分的多任务微调，通过引入分布匹配项和边界软标签来提高分数预测的准确性和分布对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有通用LLM在自动评分时无法很好地反映 instructor 的评分行为，且缺乏对预测分数分布与真实分数分布对齐的关注。

Method: 使用BART编码器-解码器配合LoRA适配器进行多任务训练，同时预测数值分数和字母等级桶，并加入分布匹配损失项。实验比较了单任务/多任务、硬标签/软标签、有 rubric/无 rubric 等条件。

Result: 多任务 BART 结合边界软标签和 rubric 上下文达到了更低的MAE和更强的分数分布对齐；完全微调的T5进一步提升分布保真度；成对预训练降低了数值误差但牺牲了少数类别敏感性。

Conclusion: 校准感知、rubric引导的训练比纯优化精度的方案能产生更符合 instructor 评分行为的自动评分模型。
...
### [201] [Calibrating Urban Traffic Simulation from Sparse Road Observations via Genetic Optimization](https://arxiv.org/abs/2606.03823)
*Hunter Sawyer,Jesse Roberts,Simon Matei*

Main category: cs.AI

TL;DR: 本研究提出了一种基于遗传算法的框架，用于解决城市交通仿真中的数据限制问题，在仅有少量道路观测数据且缺乏详细就业分布数据的情况下，校准城市交通仿真模型。


<details>
  <summary>Details</summary>
Motivation: 现实城市交通仿真面临两个基本数据限制：1）大多数城市的道路中只有一小部分有详细的真实交通测量数据；2）用于建模通勤交通的关键就业分布数据很少以仿真所需的分辨率提供。这些限制阻碍了真实交通仿真在多个城市的部署。

Method: 使用SUMO交通仿真平台针对北卡罗来纳州格林斯伯勒市，提出了一种基于遗传算法的框架，该方法优化就业分布和交通闸道参数，使模拟交通与已知交通流量的少量道路样本相匹配，无需详细的就业位置数据。

Result: 该方法产生的模拟交通与真实世界测量结果高度相关，能够推广到 withheld from training 的道路段，并产生了与人口普查就业数据具有定性一致的就业分布，尽管从未直接在这些就业数据上进行训练。

Conclusion: 这项工作表明，即使从最小的真实世界观测也能实现逼真的城市交通仿真，提供了一种可扩展且数据轻量的仿真校准方法，降低了在不同城市部署交通模型的障碍。
...
### [202] [BigFinanceBench: A Workflow-Grounded Benchmark for Financial-Research Agents](https://arxiv.org/abs/2606.03829)
*Alex Wang,Georg Meinhardt,Jacob Katz,Joseph H. Kim,Pratyush K. Chaudhary,Chase Blagden,Eric Xu*

Main category: cs.AI

TL;DR: 金融研究AI评估新基准BigFinanceBench问世，聚焦可审计推导过程而非仅最终答案


<details>
  <summary>Details</summary>
Motivation: 现有金融基准仅评估孤立子技能或最终答案，无法验证分析师如何得出结论，缺乏对可审计推导过程本身的测量

Method: 构建928项开放式金融研究任务基准，每项包含真实参考答案和点加权评分规则，将推导分解为独立可检查步骤，支持部分信用评估

Result: 评测10个前沿及开源AI代理发现：最佳系统仅达58.8% rubric分数，最终答案准确率是推导质量的损耗性代理指标，模型能力在不同金融工作流程中非均匀分布

Conclusion: 当前AI系统存在实质性提升空间，金融研究任务的评估需从单纯最终答案转向完整的可审计推导过程
...
### [203] [EvoDS: Self-Evolving Autonomous Data Science Agent with Skill Learning and Context Management](https://arxiv.org/abs/2606.03841)
*Zherui Yang,Fan Liu,Yansong Ning,Hao Liu*

Main category: cs.AI

TL;DR: EvoDS是一个自进化的自主数据科学智能体，通过自主技能获取(ASA)和自适应上下文压缩(ACC)策略来解决现有LLM智能体在数据科学任务中的局限性，在四个基准测试中平均超越最先进开源数据科学智能体28.9%。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体的静态动作集和缺乏长期上下文管理原则，阻碍了在多阶段迭代数据科学管道中累积可重用经验和在复杂任务中可靠运行的能力。

Method: 引入两个关键策略：1)自主技能获取(ASA)机制，使智能体能够合成、验证和重用可执行技能；2)自适应上下文压缩(ACC)策略，将上下文管理视为学习控制问题而非被动截断。这些策略在两阶段多智能体训练方案中进行编排。

Result: EvoDS在四个不同基准测试中平均优于最先进的开源数据科学智能体28.9%，同时消除了超出令牌限制的失败。

Conclusion: 理论上证明了层次设计降低了工具选择误差，优化目标与信息瓶颈原则对齐。在实践中展示了显著的性能提升，为自主数据科学智能体的发展提供了新的方向。
...
### [204] [PyraMathBench: Evaluating and Improving Mathematical Capability in Large Language Models](https://arxiv.org/abs/2606.03858)
*Zetian Ouyang,Linlin Wang,Gerard de Melo,Liang He*

Main category: cs.AI

TL;DR: PyraMathBench是一个综合性的层级基准测试，包含32,505个问题，源自7,404道数学应用题，用于评估大型语言模型的数值推理能力。


<details>
  <summary>Details</summary>
Motivation: 数值推理是数学能力的基石，但现有基准测试缺乏对数值处理和数学推理的整合，导致数学任务失败原因难以解释。

Method: 构建了包含4个关键认知方面、14个子类别和2种模态的综合基准测试PyraMathBench；提出SOLVE模块和IRPO训练方法，通过高效工具调用（模糊匹配和低质量调用拒绝）提升LLMs的数值-数学协同能力。

Result: 实验揭示LLMs的数值计算能力不足且处理抽象数值问题能力弱；Qwen-2.5结合SOLVE和IRPO训练后获得5.0分的提升。

Conclusion: 该基准测试和训练方法有效解决了LLMs在数值推理方面的核心问题，显著提升了模型的数学能力。
...
### [205] [Reasoning Structure of Large Language Models](https://arxiv.org/abs/2606.03883)
...
### [206] [scTranslation: A Comprehensive Benchmark for Single-Cell Multi-Omics Modality Translation](https://arxiv.org/abs/2606.03906)
*Jiabei Cheng,Jingbo Zhou,Jun Xia,Changkai Li,Zhen Lei,Chang Yu,Stan Z. Li*

Main category: cs.AI

TL;DR: 提出了scTranslation，一个单细胞多组学模态翻译任务的综合基准测试框架，解决了该领域缺乏系统化评估基准的问题。


<details>
  <summary>Details</summary>
Motivation: 单细胞多组学测量成本高、噪音大且模态覆盖不完整，虽然已出现多种计算模态翻译方法，但尚缺乏系统性的基准评估，包括数据集、评估指标和影响因素等方面。

Method: 构建了包含多样化翻译数据集的基准，整合了最先进的翻译模型，并提供全面的评估指标，还评估了在特征选择、特征质量和少样本设置等不同场景下的模型性能。

Result: 对当前方法进行了大规模研究，获得了许多有价值的发现，为未来的研究开辟了新的可能性，代码已在GitHub上开源。

Conclusion: scTranslation基准测试框架的提出为单细胞多组学模态翻译任务提供了标准化的评估平台，推动了该领域方法的系统性比较和发展。
...
### [207] [Hedge-Bench: Benchmarking Agents on Hard, Realistic Tasks Pertaining to Financial Reasoning](https://arxiv.org/abs/2606.03918)
*Eric Cho,Shawn Huang,Alice Lu,Andy Lyu*

Main category: cs.AI

TL;DR: Hedge-Bench 1.0 是一个基准测试数据集，包含102个真实的、对冲基金分析师的实际工作任务，用于评估AI模型在开放性金融推理任务上的表现，现有前沿模型的得分低于16%。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法捕捉专业分析师的开放式推理任务，而那些试图评估开放式推理的基准依赖模型评判输出，引入噪声和循环性问题，因此需要一个基于真实专业推理过程的新基准。

Method: 构建了一个包含102个实际工作任务的数据集，这些任务来源于专业对冲基金分析师的显式推理痕迹，并配备了相关信息源，使用确定性评分方法对照验证过的专家步骤进行评估。

Result: 前沿模型和代理在该基准测试上的得分均低于16%，表明当前AI系统在处理开放式金融推理任务方面仍有很大提升空间。

Conclusion: 该研究提供了一个开源的基准测试数据集和评估工具，为衡量和改进AI在专业金融分析推理方面的能力提供了重要参考，同时也揭示了现有AI系统的局限性。
...
### [208] [Entropy Is Not Enough: Unlocking Effective Reinforcement Learning for Visual Reasoning via Vision-Anchored Token Selection](https://arxiv.org/abs/2606.03937)
...
TL;DR: 本文研究视觉推理任务中token级熵的信用分配机制失效问题，提出VEPO框架通过视觉敏感性与token熵的乘法耦合来重新定向梯度信用，在7B和3B规模上分别超越基线2.28和3.15个百分点。


<details>
  <summary>Details</summary>
Motivation: 文本强化学习中token级熵被证实能有效进行信用分配，但在视觉推理中该机制失效，因为低熵的视觉敏感token被忽略。现有多模态RL方法虽认识到视觉感知重要性，但缺乏系统的视觉测量或忽视token熵驱动语义探索的本质。

Method: 提出VEPO框架，通过乘法耦合将视觉敏感性与token熵显式整合，使梯度信用重定向至同时具备视觉grounding和高信息量的token。设计视觉敏感性测量并与语义信息度结合进行token选择。

Result: 实验表明VEPO在7B规模上超越entropy-only基线2.28分，在3B规模上超越3.15分，消融实验验证了方法的有效性。

Conclusion: 视觉推理中需显式整合视觉敏感情境信息与语义探索机制，VEPO通过乘法耦合的token选择策略成功解决了传统entropy机制的失效问题，为多模态RL的视觉推理提供了新思路。
...
### [209] [Imaginative Perception Tokens Enhance Spatial Reasoning in Multimodal Language Models](https://arxiv.org/abs/2606.03988)
...
TL;DR: 本文提出Imaginative Perception Tokens (IPT)中间表征，帮助视觉语言模型在空间推理任务中模拟未观察视角下的视觉信息，在透视taking路径追踪和多视图计数任务上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在关键信息不可直接观察的空间推理方面存在困难，无法进行想象性感知如推断未观察视角应看到什么穿过遮挡空间的路径或整合部分观察为连贯空间表征。

Method: 构建三个空间推理任务PET PT MVC各约20K样本的评估基准数据集使用统一VLM BAGEL作为骨干通过IPT监督信号训练并与文本链式思考方法对比

Result: IPT监督持续提升空间推理能力且常优于文本链式思考训练，MVC任务准确率提升3.4%，PT任务达到与强闭源模型相当的性能，结合IPT和标签监督可获得额外增益，而文本链式思考可能显著降低性能

Conclusion: IPT为推理未观测空间结构提供了原则性监督信号，提升泛化能力同时产生可解释的中间表征，解决空间计算forced through language时的模态不匹配问题
