...
TL;DR: 提出一种查询高效、预算自适应的黑盒对抗攻击方法ContextTS-NES，用于生成对抗补丁以抑制目标检测器的检测能力


<details>
  <summary>Details</summary>
Motivation: 解决现有对抗补丁研究中的三个关键空白：缺乏联合优化补丁位置、纹理和尺寸的黑盒攻击；未将成功与补丁的视觉足迹关联；混淆了EOT鲁棒性与纯视图抑制

Method: 提出Contextual Thompson-Sampling放置器结合NES风格像素更新的攻击方法，仅在进展停滞时增长补丁，采用严格的纯图像抑制测试

Result: 在YOLOv5、Faster R-CNN和YOLOS上实现强抑制，使用紧凑补丁暴露了查询-足迹权衡，print-transfer实验显示跨物理对象和视角的迁移能力

Conclusion: 该方法在CNN和Transformer检测器上均有效，揭示了对抗补丁的视觉足迹与攻击效果之间的权衡关系
...
TL;DR: 本文提出CAOA方法，将点云补全模块与对称性感知的相对姿态估计相结合，实现CAD模型与室内RGB-D扫描中物体的精确对齐，并引入针对室内场景的合成数据生成策略及S2C-Completion数据集。


<details>
  <summary>Details</summary>
Motivation: 室内RGB-D扫描存在噪声、不完整以及分割错误导致的的几何畸变，使得9自由度姿态估计（位置、旋转、尺度）面临挑战，现有补全方法在合成数据集上训练难以泛化到真实扫描。

Method: 提出Completion-Assisted Object-CAD Alignment (CAOA)方法，整合语义和上下文感知的点云补全模块与对称性感知的相对姿态估计算法；引入针对室内场景的合成数据生成策略以缩小域差距；构建包含8500+物体-CAD对的S2C-Completion数据集；设计对称性感知损失函数处理对称模糊性。

Result: 在Scan2CAD基准测试中，CAOA相比现有最优方法实现17%的精度提升。

Conclusion: 通过补全模块与对称性感知相结合的策略，CAOA有效提升了CAD模型与真实扫描物体的对齐精度，为室内场景的3D语义重建提供了更鲁棒的解决方案。
...
TL;DR: RegimeVGGT是一种针对VGGT的训练无需加速方法，通过分析Transformer层的三种不同 regime（浅层、中层、深层），采用双轴U形压缩策略，在保护几何显著token和姿态关键路径的同时，实现6.7倍加速。


<details>
  <summary>Details</summary>
Motivation: VGGT虽能从多视图图像一次性恢复密集3D场景结构，但其二次方跨帧注意力限制了可扩展性。现有训练无需加速器沿单一轴均匀减少计算，未能利用层的异质性。

Method: 通过频谱、探测和因果分析揭示三层regime：浅层缺乏跨视图结构，中层驱动跨视图对齐，深层对密集几何冗余但其跨帧注意力对姿态至关重要。RegimeVGGT采用双轴U形压缩：1）显著性导向的带状合并保护几何和边缘显著token；2）选择性保护的K/V下采样保留跨帧空间覆盖和姿态关键路径（相位偏移空间网格、参考帧锚点、未压缩相机/寄存器token）。

Result: 训练无需的RegimeVGGT在匹配重建质量下实现相对VGGT*的6.7倍加速。

Conclusion: 通过识别Transformer不同层的功能差异并采用针对性压缩策略，可在保持重建质量的同时显著提升效率，为3D场景重建的实时应用提供了可行方案。
...
TL;DR: 本文提出CF-GRPO（共识帧GRPO）框架，无需人工时间标注即可增强视频多模态大模型的证据感知推理能力。该方法通过构建共识帧先验，结合视频内在线索（时序覆盖、场景转换、查询条件视觉相关性）来识别关键证据帧，并利用共识帧奖励（CFR）优化模型对视觉证据的利用。


<details>
  <summary>Details</summary>
Motivation: 当前强化学习虽能提升大语言模型的推理能力，但对视频多模态大模型（Video-MLLMs）仅使用结果奖励，难以指导模型理解哪些视觉证据应支持答案。受多感官整合启发——一致性线索可增强感知估计的显著性和可靠性——作者认为需要一种能够识别和利用关键证据帧的奖励机制。

Method: CF-GRPO分三步构建：1）从视频内在线索（时序覆盖、场景转换、查询条件视觉相关性）构建共识帧先验；2）从视觉和响应表示中计算模型侧帧使用分数；3）通过共识帧奖励（CFR）优化两者一致性，配合显著性感知稀疏聚合和分布锐化，无需人工时间标注即可提供高对比度奖励信号。

Result: VideoCFR在复杂视频推理基准上达到竞争性能，多项指标优于代表性的Video-MLLM和RL基线方法，共识先验还提供了训练中强调的证据帧的可解释视图。

Conclusion: CF-GRPO成功实现了无人工时间标注的过程级奖励框架，通过共识机制引导视频多模态模型聚焦关键视觉证据，为证据感知视频推理提供了一种有效且可解释的训练范式。
...
TL;DR: ReFine3D是一种用于3D大型多模态模型(LMMs)的域自适应微调框架,通过选择性层调优、多视图一致性、文本多样性、点渲染视觉监督和测试时增强等正则化策略,在有限数据下实现域泛化微调。


<details>
  <summary>Details</summary>
Motivation: 3D视觉中的域适应是核心挑战,尤其是将3D点云与视觉和文本数据对齐的多模态基础模型。虽然这些模型展现出强大的通用能力,但在有限数据条件下将其适配到下游领域常导致过拟合和灾难性遗忘。

Method: ReFine3D框架结合了:(1)选择性层调优;(2)多视图一致性(跨增强点云);(3)文本多样性(通过大型语言模型生成的同义词提示);(4)点渲染视觉监督;(5)测试时增强机制与置信度聚合。

Result: 在不同3D域泛化基准测试中,ReFine3D将基类到新类泛化提升1.36%,跨数据集迁移提升2.43%,抗腐蚀鲁棒性提升1.80%,少样本精度提升最高3.11%,以最小计算开销超越先前最先进方法。

Conclusion: ReFine3D通过多正则化策略有效解决了3D LMMs在有限数据域适应中的过拟合和灾难性遗忘问题,在多个基准测试中取得显著性能提升,为3D视觉域泛化提供了一种高效实用的微调范式。
...
TL;DR: 本文提出Data-Forcing Distillation (DFD)框架，通过教师得分差异指导学生模型恢复真实数据分布，解决DMD方法中样本多样性下降和过度饱和问题，仅需100-300步微调即可在视频生成模型上取得优于教师模型的效果。


<details>
  <summary>Details</summary>
Motivation: DMD及其改进版DMD2在蒸馏多步视频扩散模型时存在两个持久性问题：1)样本多样性显著下降(模式崩溃)；2)输出过度饱和而偏离真实视频外观。现有方法未能有效解决这些缺陷。

Method: 提出DFD框架，通过教师得分差异引导学生模型向真实数据分布优化：1)引导学生向缺失模式移动以缓解模式崩溃；2)引导远离真实数据中不存在的异常模式以避免过度饱和。该方法仅需一行代码修改即可实现。

Result: 在Wan2.1-1.3B和Cosmos-Predict2.5-2B模型上验证，仅用100-300步微调即可有效恢复多样性和保真度，解决过度饱和伪影，显著提升视频动态和外观效果，甚至优于教师模型。

Conclusion: DFD是一个简单有效的后训练框架，通过教师信号差异引导成功解决了蒸馏过程中的模式崩溃和过度饱和问题，为高效视频生成模型的训练提供了新思路。
...
TL;DR: Vines-DB是一个用于观赏藤本植物实例分割的数据集，包含1,218张原始高分辨率RGB图像（扩充后达2,307张），涵盖7种藤本植物物种，支持深度学习模型开发，用于精准园艺和城市生态学中的多类实例分割任务。


<details>
  <summary>Details</summary>
Motivation: 精准园艺和城市生态学需要自动化工具进行藤本植物物种识别、冠层覆盖估算和田间表型分析，但缺乏专门针对观赏藤本植物的高质量标注数据集，限制了深度学习模型在园艺领域的应用发展。

Method: 在犹他州立大学Greenville研究农场收集168株藤本植物图像，使用iPhone 16 Pro（48MP）在自然光下拍摄，经Roboflow平台手动标注生成多边形实例分割掩膜，进行数据增强后采用分层采样划分为2,019训练、192验证和96测试图像。

Result: 构建了包含7种藤本植物（Akebia quinata、Campsis radicans、Hydrangea anomala petiolaris等）及背景的实例分割数据集，支持多类实例分割模型的训练与评估。

Conclusion: Vines-DB数据集可应用于自动化冠层覆盖估算、物种识别和可扩展田间表型分析，为精准 horticulture和城市生态学中的计算机视觉研究提供了重要的数据基础。
...
TL;DR: 本研究比较了Vision Transformer架构与CNN基线模型在人脸呈现攻击检测(PAD)系统中的 demographic bias 差异。在CeFA数据集上的实验表明，预训练的DeiT-S模型达到97.27%准确率和0.86%的EER，在保持更高准确率的同时，将非裔与东亚人群的ACER差距降低83%，并在零样本中亚人群上展现3.6倍的泛化优势。


<details>
  <summary>Details</summary>
Motivation: 现有的人脸PAD系统存在明显的性能差异，深色皮肤人群的识别准确率显著低于浅色皮肤人群，这种技术偏见可能导致生物识别认证系统对特定人群产生不公平待遇，需要寻找更公平的架构设计方案。

Method: 在CASIA-SURF CeFA数据集上比较三种架构：从零开始训练的Multimodal ViT-Tiny、ResNet18 CNN基线、以及在CeFA上微调的预训练DeiT-S，针对非洲、东亚和零样本中亚三个人群进行评估。

Result: DeiT-S达到最高97.27%准确率和最低0.86%EER，显著优于ResNet18的90.15%。在公平性方面，DeiT-S将非裔与东亚人群的ACER差距降至0.13%，相比LBP方法的0.75%降低83%。更重要的是，ResNet18在中亚人群的BPCER为10.44%，而DeiT-S仅2.89%，展现3.6倍泛化优势。

Conclusion: 预训练的Vision Transformer架构在人脸PAD任务中不仅实现更优的识别准确率，还能产生更小的demographic performance gaps，并在未见人群上展现更好的泛化能力，表明跨人群公平性部分受到架构设计的影响。
...
TL;DR: 本文提出一种基于原型签名的数据驱动策略，用于离线手写签名验证中的负样本生成，以解决随机选择导致的冗余和效率低下问题。


<details>
  <summary>Details</summary>
Motivation: 该方法与线性SVM结合使用。

Method: 基于实验结果得出三个结论：(i)原型签名能生成信息更丰富的负样本，提高熟练伪造检测能力；(ii)该方法与主干网络无关，在不同架构中表现出鲁棒性；(iii)结合线性SVM可作为RBF基模型的替代方案，显著提高可扩展性和计算效率。

Result: 原型签名能生成信息更丰富的负样本，提高熟练伪造检测能力；该方法与主干网络无关，在不同架构中表现出鲁棒性；结合线性SVM可作为RBF基模型的替代方案，显著提高可扩展性和计算效率。

Conclusion: 本文提出的基于原型签名的负样本生成策略能有效提高离线手写签名验证系统的性能，具有良好的可扩展性和计算效率。
...
TL;DR: 提出了一种检索增强的图像标题生成框架，利用外部知识生成更深入、更全面的图像描述。


<details>
  <summary>Details</summary>
Motivation: 传统图像标题生成方法难以生成全面、上下文丰富的描述，特别是对于无法直接从视觉线索观察到的细节。

Method: 提出层次化多模态文章检索机制，考虑文章结构感知特征（标题、正文部分、视觉放置模式），计算多面相似度（内容-视觉、视觉-视觉、话语位置）；然后通过上下文相关性 refinement 阶段增强检索信息。生成采用三阶段：VLM 生成简洁图像描述→从检索文章中分割相关信息→LLM 生成全面上下文标题。

Result: 在ACM Multimedia EVENTA 2025挑战赛中获得第5名，在OpenEvent-V1数据集测试集上总体得分为0.2824。

Conclusion: 该方法通过检索增强生成有效利用外部知识生成上下文详细的图像标题。
...
TL;DR: 论文提出Forged Calamity基准数据集，用于检测AI生成的灾难图像。该数据集包含30,000张图片（6,000张真实图片和24,000张由四个扩散模型生成的合成图片）。实验揭示当前检测方法在跨模型和跨灾难类型泛化方面存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像扩散模型的快速发展，AI生成的图像越来越难以与真实照片区分。这对网络安全、数字取证和灾害响应构成挑战，因为关于洪水、火灾或地震的虚假图像可能传播错误信息或干扰紧急救援行动。

Method: 构建包含30,000张图像的Forged Calamity基准数据集，其中6,000张为真实灾难图像，24,000张由四种扩散模型生成。在微调和零样本设置下进行综合实验，评估检测器在不同生成器和灾难类型上的性能。

Result: 微调检测器在分布内数据上表现良好，但在 unseen 生成器或灾难类型上损失高达50%准确率，表现出对模型特定伪影的过拟合。零样本泛化检测器也难以保持稳定准确率，仅有少数表示鲁棒模型表现出有限的韧性。

Conclusion: 当前检测方法存在持续的泛化差距，亟需开发领域无关和模型无关的检测方法，以确保扩散时代视觉内容的真实性。
...
TL;DR: 本文提出利用Stable Diffusion生成合成室内图像用于数据增强，并引入DIRE方法检测AI生成的假图像，在MIT数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 室内图像识别面临训练数据不足的困境，因为真实室内场景图像获取困难，且存在光照、遮挡等复杂因素影响识别效果。

Method: 1) 使用Stable Diffusion生成多样化的合成室内场景图像作为数据增强；2) 提出DIffusion Reconstruction Error (DIRE)方法来检测SD生成的合成图像；3) 使用轻量级MobileNetV3训练分类器识别合成图像。

Result: 在MIT Indoor Scene数据集上的实验表明，该方法能有效增强训练数据；DIRE方法可100%准确识别SD生成的合成图像。

Conclusion: Stable Diffusion是有效的室内图像数据增强工具，DIRE可作为检测AI生成图像的可靠方法，具有重要的应用价值。
...
TL;DR: 本文提出了MolmoMotion，一个用于3D点运动预测的通用模型，通过大规模数据集MolmoMotion-1M（包含116万段视频注释的3D点轨迹）和基准测试集PointMotionBench（涵盖111个物体类别和61种运动类型）进行训练和评估。该方法支持自回归坐标预测和流匹配轨迹生成两种模式，在多样化的运动模式预测上表现出色，并能迁移到机器人操作和视频生成等下游任务。


<details>
  <summary>Details</summary>
Motivation: 运动预测是视觉智能的核心：智能体需要预测物体运动以规划动作、推理物理交互并合成逼真的未来。当前方法缺乏统一且可直接用于下游任务的表示方法。本文主张使用3D世界坐标点作为通用表示，它具有类别无关、视图稳定、紧凑且可直接用于下游任务的优势。

Method: 1) 构建MolmoMotion-1M：包含116万段无约束视频的动作描述、物体锚定的3D点轨迹数据集；2) 构建PointMotionBench：人工验证的基准测试集，涵盖111个物体类别和61种运动类型；3) 提出MolmoMotion模型：支持自回归坐标预测和流匹配轨迹生成两种方式的通用运动预测模型，输入包括短期视觉历史、目标物体的3D查询点集和语言目标描述。

Result: MolmoMotion能准确预测不同语言指令下的多样化运动模式，在PointMotionBench上显著优于现有运动预测基线方法。

Conclusion: 学习到的3D运动先验知识可良好迁移到下游应用：提高机器人操作的训练效率和泛化能力，其预测的轨迹为生成模型合成更逼真的物体运动视频提供有效的运动指导。
...
TL;DR: 在CIFAR-10数据集上使用6个卷积层和3个最大池化层的卷积神经网络进行图像分类实验，训练10个epoch后达到约74.77%的验证准确率，展示了表示学习与过拟合（记忆化）的实际差异。


<details>
  <summary>Details</summary>
Motivation: 提供一个紧凑的可复现图像分类教育实验基准，强调完整的学习流程，并说明表示学习与记忆化之间的实际区别，为未来正则化、数据增强和更深架构的研究奠定基础。

Method: 使用完整学习流程：图像向量化、归一化、单热类编码、监督损失最小化、学习率选择、小批量训练、卷积特征提取、最大池化及基于验证的泛化评估。卷积架构包含6个卷积层和3个最大池化阶段，训练10个epoch，批量大小128，Adam优化器，学习率0.001。

Result: 验证准确率约74.77%，验证损失在训练中期后开始上升，而训练损失持续下降，呈现出典型的过拟合/记忆化特征。

Conclusion: 该研究为图像分类教育提供了紧凑的实验基准，清晰展示了表示学习与记忆化的实际差异，可作为正则化、数据增强、更深架构等未来研究的可复现基准。
...
TL;DR: 本文提出了一种针对低光环境人群计数的鲁棒网络(LCNet)，通过融合RGB外观、深度几何和边缘结构线索，结合多模态超图融合模块和可变形矩形稀疏注意力机制，在三个新建基准数据集上实现了最佳性能。


<details>
  <summary>Details</summary>
Motivation: 低光环境下的人群计数在实际应用中具有重要意义，但现有方法主要针对良好光照场景或仅依赖RGB单模态表示，在极端黑暗和复杂非均匀光照条件下表现不可靠，该问题此前研究不足。

Method: 1)构建三个新基准数据集：SHA_Dark和SHB_Dark(合成)、LC-Crowd(真实场景)；2)基于Retinex物理建模，引入深度和Canny边缘作为补充的几何和结构先验；3)提出多模态超图融合模块，将RGB外观、深度几何和边缘结构作为超图节点，通过动态超边构建和消息传递捕捉高阶互补关系；4)提出可变形矩形稀疏注意力模块(DRSA)，通过锚点感知估计和自适应矩形窗口建模，将计算集中在信息丰富区域。

Result: 在三个基准数据集上与现有最先进方法相比，提出的LCNet实现了最佳整体性能。

Conclusion: 通过多模态信息融合和创新的注意力机制，本研究有效解决了低光环境下人群计数的难题，所提方法在三个新基准数据集上验证了有效性，为低光视觉任务提供了新的研究思路。
...
TL;DR: GOOSE 2D细粒度语义分割挑战赛第一名方案，采用DINOv3 ViT-L/16主干网络结合ViT-Adapter和Mask2Former解码器，通过粗粒度辅助损失及测试时增强集成策略，在64类细粒度类别和11个粗粒度类别的户外图像分割任务中取得76.57%的综合得分。


<details>
  <summary>Details</summary>
Motivation: 该研究旨在解决户外非结构化环境下的细粒度语义分割问题，需同时识别64个细粒度类别和11个粗粒度类别，这对自动驾驶和 field robotics 等应用具有重要价值。

Method: 方案包含两部分：(1)网络级设计：结合自监督DINOv3 ViT-L/16主干网络、ViT-Adapter和Mask2Former掩码分类解码器，并添加粗粒度类别辅助损失于全局[CLS] token；(2)推理时聚合策略：采用多尺度和水平翻转的测试时增强，并集成Codabench得分前三的检查点模型。

Result: 最终获得官方综合得分76.57%，其中细粒度类别mIoU为69.32%，粗粒度类别mIoU为83.81%，在最终阶段排行榜上位列第一。

Conclusion: 该方案通过结合先进的视觉Transformer架构、多层次监督信号和测试时增强集成策略，成功解决了细粒度语义分割的挑战，为户外机器人感知提供了有效的解决方案。
...
TL;DR: 本文提出了一种针对空-地基激光雷达（LiDAR）地点识别的新型检索和重排序框架，解决航空与地面点云之间的领域差距和初始检索中的误报问题。


<details>
  <summary>Details</summary>
Motivation: 传统的地面LiDAR地点识别存在预访问要求、覆盖不完整和视角受限等缺点。使用预先获取的机载激光扫描（ALS）数据作为航空先验地图可以克服这些限制，但空-地基LiDAR识别面临领域差距和误报挑战。

Method: 提出检索网络在多尺度引入补丁级自监督学习模块，并与场景级学习集成以提高全局特征判别性；同时提出扩展互惠（ER）重排序算法，利用ALS点云的结构化空间分布信息更新相似度矩阵进行最终排序。

Result: 检索网络在CS-Urban-Scenes数据集上实现平均Recall@1提升9.8%、平均Recall@1%提升3.2%，在CS-Campus3D数据集上表现最佳。ER重排序算法在CS-Campus3D上提升4.9%、在CS-Urban-Scenes上提升10.2%的平均Recall@1，且无需额外训练。

Conclusion: 该框架有效解决了空-地基LiDAR地点识别中的领域差距问题，显著提升了跨视图地点识别的准确性，为自动驾驶和机器人定位提供了新的技术方案。
...
TL;DR: 本文提出原子物理转换(APTs)作为物理视频理解的因果监督信号，将视频表示为有序的因果转换序列而非单一聚合事件标签，并推出APT-Tune参数高效微调方法，在Qwen3-VL-2B上显著改善APT召回率同时保持事件级性能。


<details>
  <summary>Details</summary>
Motivation: 当前视频事件标签(如"bounce")可能正确但隐藏了使事件物理有效的底层过程(支撑丧失、接触 onset、反弹、沉降)，VLMs缺乏对过渡级别物理的理解，需要一种将可见线索与主动物理机制绑定的表示方法。

Method: 构建混合来源APT数据(人类注释+模拟器真值)，覆盖14种过渡类型(接触、重力、摩擦、旋转/稳定性)，27,303个时间实例，1,246个试验；提出APT-Tune方法，结合图像pad感知监督、格式条件共同训练和机制条件域到类型解码，使用11M LoRA参数微调。

Result: 当前VLMs零样本APT召回率最高14%，错误以遗漏过渡为主；直接微调改善过渡检测但导致事件级遗忘；APT-Tune在仅11M参数下显著改善APT召回率，同时改善事件级视频迁移。

Conclusion: APTs不是新答案格式，而是人类对齐的因果监督信号，可教会VLMs使用因果转换而不遗忘视频问答能力，参数高效微调是可行的路径。
...
TL;DR: CHIEF是一个将创作者置于人机协作视频生成循环中心的人类-AI协作框架,通过基于角色条件的多模态大语言模型提供自动化的主观反馈,帮助没有电影制作经验的学生创作1-10分钟的短片。


<details>
  <summary>Details</summary>
Motivation: 生成式AI虽使视频创作更便捷,但AI生成的视频常缺乏叙事连贯性和创意方向,且这些问题在长视频中更严重。视频生成需要关于情节、场景和叙事的主观反馈,而现有方法缺乏有效的创作者参与机制。

Method: 提出CHIEF框架:1)将创作者置于人机循环迭代视频优化中心;2)由专门的精修代理(Refiner Agent)纳入创作者的修改;3)使用基于角色条件的多模态LLM观看生成视频并从观众视角产生主观批评,提供自我评估无法捕捉的反馈。

Result: 与高中和大学学生(无电影制作经验)合作,成功生成了从1分钟短视频到10分钟完整短片(包含复杂情节)的作品。

Conclusion: CHIEF框架能够有效帮助非专业创作者生成具有叙事连贯性的视频,证明人机协作结合自动化主观反馈在视频生成领域的可行性。
...
TL;DR: 针对医疗诊断中视觉语言模型(VLM)的幻觉问题，提出CoEV框架，通过双向证据验证检测和修正幻觉，无需重新训练即可实现可靠的幻觉检测和修正。


<details>
  <summary>Details</summary>
Motivation: VLMs在医疗诊断中的可靠性受制于幻觉问题，现有方法主要关注文本与参考数据的事实不一致性，缺乏对视觉证据是否真正支持生成文本的验证。

Method: 提出Counter-Evidence Verification (CoEV)框架，执行文本断言与视觉证据之间的双向验证，将每个声明分类到四象限诊断图中，捕捉文本真实性和视觉基础的不同组合。

Result: 在四个医疗数据集上，CoEV在幻觉检测方面，PR-AUC和ROC-AUC分别提升3.0%和3.9%，特定VQA场景提升达18.5%；幻觉修正方面，Micro-F1提升达12.5%，医疗报告生成幻觉率降低11.9%，医疗VQA准确率也得到提升。

Conclusion: CoEV能够实现可靠的幻觉检测和修正，为临床医生提供可信赖的诊断依据。
...
TL;DR: 本文提出Intrinsic-GS，一种无需训练、无需掩码的4D高斯分割方法，通过从高斯原语本身提取外观、方向、尺度、变形轨迹和渲染边界等线索构建亲和图，并使用Leiden社区检测进行分割。在标准基准上达到0.746/0.575 mIoU，几何变体达到0.902 mIoU，速度提升12.5倍。


<details>
  <summary>Details</summary>
Motivation: 当前4D高斯分割依赖外部2D掩码（如SAM），生成成本高且结果依赖掩码质量。作者探索能否直接从高斯原语本身恢复物体级结构，提供一种更快、更鲁棒的分割方向。

Method: 构建稀疏亲和图：利用 appearance（外观）、orientation（方向）、scale（尺度）、deformation-trajectory（变形轨迹）和非学习的rendered-boundary（渲染边界）线索表征高斯原语间关系。使用Leiden算法进行社区检测分割，无需任何学习特征或基础模型监督。

Result: 在Neu3D和HyperNeRF基准上，无掩码监督达到0.746和0.575 mIoU；Neu3D上几何-only变体达0.902 mIoU，与SAM监督的TRASE相当；HyperNeRF上比掩码生成方法快12.5倍。

Conclusion: 大部分分割信号已编码在高斯原语本身，该方法提供了一种快速、无掩码的3D/4D高斯分割方向，在外部掩码不可靠或昂贵场景中具有潜力。
...
TL;DR: 本文提出了一种基于离散小波变换的尖峰金字塔小波模型(SPWM)，用于图像修复任务。通过开发尖峰双金字塔小波(SDPW)块来建模长程依赖性，克服了传统尖峰CNN感受野受限的问题，在保持图像质量的同时显著降低了计算成本和能耗。


<details>
  <summary>Details</summary>
Motivation: 尖峰神经网络(SNNs)因其高效性和生物启发性在计算机视觉中受到关注，但现有基于尖峰CNN的图像修复方法受限于CNN操作的感受野局限性，导致性能受限。

Method: 提出利用离散小波变换，并开发了尖峰双金字塔小波(SDPW)块来建模长程依赖性，同时利用小波域中的退化特性。

Result: 在多个基准数据集上的实验表明，SPWM在维持图像质量的同时显著降低了计算成本和能耗。

Conclusion: 该方法展示了SNNs在图像修复领域的潜力，为资源受限设备的未来应用提供了新见解。
...
TL;DR: 提出一种基于流形锚定的变分框架，利用几何感知的EM算法学习医学影像的无监督表示，通过图medoid和扩散中心性选择子群原型，确保原型保持在数据流形上。


<details>
  <summary>Details</summary>
Motivation: 现有深度潜在变量模型使用欧几里得平均估计高斯混合先验，导致原型偏离弯曲的数据流形，且当子群数量增加时出现退化。

Method: 构建基于几何感知EM算法的流形锚定变分框架，M步选择热核加权潜在图上具有最高扩散中心性的图medoid作为子群原型；引入Dirichlet能量正则化器确保潜在空间的几何平滑性；提供每子群的不确定性评分实现无标签质量评估。

Result: 在心脏瘢痕和脑MRI基准数据集上，该框架达到最高精度，产生迄今为止最清晰的原型，且在大量子群数量下保持稳定，而所有基线方法均退化。

Conclusion: 流形锚定EM是通用几何工具，可扩展标准EM并轻松应用于其他潜在变量模型，在医学影像聚类分析中具有重要应用价值。
...
TL;DR: 提出了一个面向滑坡灾害智能解译的指令驱动agentic框架，包含LandslideBench数据集、 LandslideVLM模型和LandslideAgent三个组件，解决现有视觉语言模型在地质场景中的感知局限和领域幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 当前智能滑坡灾害解译范式难以同时提取视觉特征和高级地球科学语义，而通用视觉语言模型在复杂地质场景中存在感知局限和领域幻觉问题。

Method: 构建LandslideBench多模态细粒度数据集，通过多VLM交叉验证和交互标注；基于LandslideBench使用LoRA微调 LandslideVLM；设计双规则控制器构建领域规则增强的LandslideAgent。

Result: LandslideBench在五个主流模型上提供了有效的细粒度分类和语义分割基线；LandslideVLM在滑坡判别、细粒度分类和语义描述质量上分别提升了10.96%、32.87%和15.91%；LandslideAgent实现了多源空间数据的自主推理。

Conclusion: 该框架能够实现滑坡识别与分析的全流程智能化，为地质灾害防治提供了有效技术支撑。
...
TL;DR: 提出一种BrainFusionNet模型，结合CNN、Vision Transformer(ViT)和Gated Recurrent Units(GRU)用于MRI图像的脑肿瘤分类，并集成SHAP、LIME和GradCAM等可解释AI方法，在两个公共数据集上达到98%准确率。


<details>
  <summary>Details</summary>
Motivation: MRI噪声给深度学习带来挑战，当肿瘤边界模糊、肿瘤位置和形态复杂时，传统方法难以准确提取特征进行分类。

Method: 构建混合模型BrainFusionNet：CNN和ViT提取空间和局部特征，GRU进行序列特征整合用于最终分类；集成可解释AI可视化决策区域；分析像素强度分布对分类的影响；使用K折验证在两个公共MRI数据集上评估。

Result: 98%准确率，优于DenseNet121和VGG16等SOTA CNNs(96%)；混合模型能有效提取局部和全局特征；像素强度分布影响深度学习性能。

Conclusion: BrainFusionNet的混合架构能有效提取低层和深层特征，自定义ViT稳定梯度流减少梯度消失风险。像素强度分布对图像分类性能有重要影响。
...
TL;DR: SPARE是一种无需训练的子空间重建方法，通过将视觉token剪枝重新表述为列子集选择问题，最小化重建误差来减少VLM的计算开销，同时引入了反相关性现象来选择低相关性的token以更好地保留上下文信息。


<details>
  <summary>Details</summary>
Motivation: 当前VLM因大量视觉token产生高计算成本，现有的多样性最大化方法使用基于余弦的归一化相似度，丢弃了幅值信息，无法忠实近似原始特征表示，在组合多技能推理任务上表现不佳。

Method: 将token剪枝重新表述为列子集选择问题，通过迭代选择具有大投影残差的token进行重建驱动的剪枝，并引入反相关性作为额外选择标准促进上下文感知的token选择。

Result: 在多个VLM和基准上实现最先进的性能，在组合任务上获得显著提升。应用于LLaVA时，可移除高达94%的视觉token同时保留95%的基线性能。

Conclusion: 提出的SPARE方法通过子空间重建和反相关性机制，为VLM的token剪枝提供了新的有效范式，无需训练即可实现高效推理。
...
TL;DR: 本研究评估了五种CNN架构在脑肿瘤MRI图像分类中的性能，使用约10,000张临床来源的MRI图像数据集，发现EfficientNetB0以95%的准确率表现最佳，尤其在难以检测的脑膜瘤方面召回率高达89%。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习有所进展，但从MRI图像中准确分类脑肿瘤仍然面临挑战，需要寻找更有效的模型架构。

Method: 在统一的实验框架下测试和训练了五种CNN架构：定制的基线模型以及四个预训练模型（VGG16、VGG19、DenseNet121、EfficientNetB0），使用约10,000张MRI图像进行多分类任务，通过总体准确率和肿瘤特异性召回率评估性能。

Result: EfficientNetB0达到最高分类准确率95%，VGG16为94.37%，VGG19为92.29%，DenseNet121为90.91%，定制CNN为78%。关键发现是EfficientNetB0对脑膜瘤的检测召回率高达89%，而简单CNN仅约20%。此外，较深的VGG19反而比VGG16表现更差。

Conclusion: EfficientNetB0在分类准确率、模型参数量和临床意义性能之间提供了最佳平衡，表明对于医学图像任务，模型架构的效率可能比深度更重要。
...
TL;DR: 针对异构雷达（4D汽车雷达vs旋转雷达）之间的地点识别难题，提出空间分层蒸馏(SSD)方法，通过非对称空间对齐处理稀疏性问题，在重叠区域强制特征对齐，在稀疏区域使用折扣蒸馏权重。


<details>
  <summary>Details</summary>
Motivation: 4D汽车雷达极度稀疏，只能捕获旋转雷达数据库中的小部分结构，导致跨异构平台的地点识别性能受限，现有方法在多会话环境中性能下降。

Method: 提出空间分层蒸馏(SSD)策略：替换标准均匀蒸馏为基于物理雷达返回的非对称空间对齐；在两个雷达都有重叠返回的区域强制强特征对齐；在4D学生缺少返回但教师在共享视场内有有效结构的稀疏区域应用大幅折扣的蒸馏权重。

Result: 在HeRCULES数据集上显著优于 prior place recognition方法，在具有挑战性的动态序列上达到最先进水平。

Conclusion: SSD通过非对称空间对齐策略有效解决了异构雷达间的稀疏性匹配问题，为自动驾驶雷达地点识别提供了新的技术途径。
...
TL;DR: 本文提出UniTemp框架，解决自回归视频扩散模型只能单向(从前到后)生成的问题，实现任意时间方向的视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散模型仅支持前向时间生成，但实际视频创作需要灵活的时间顺序(如后向扩展或双向插值生成)。

Method: 引入块锚点 latent (blockwise anchor latents)解决Causal 3D VAE导致的块间不连续问题，建立双向蒸馏框架训练单一自回归学生模型支持任意方向生成。

Result: UniTemp在短视频和长视频生成上保持与前向方法相当的性能，同时支持双向视频扩展、插值生成、循环视频生成、场景转换和视觉故事生成等多种工作流。

Conclusion: UniTemp是首个支持任意时间方向生成的自回归视频扩散模型，提高了视频生成的可控性和灵活性。
...
TL;DR: 提出了一种参数高效微调方法PEFT-MedSAM，用于将MedSAM模型适配到皮肤病变自动分割任务中，在ISIC 2018和PH2数据集上取得了优异的分割性能。


<details>
  <summary>Details</summary>
Motivation: 皮肤癌早筛对黑色素瘤的早期发现至关重要，但现有深度学习方法在皮肤病变分割上表现不佳，需要一种参数高效且高精度的分割方法。

Method: 提出PEFT-MedSAM参数高效微调方法，仅对MedSAM的mask decoder进行训练，保持image encoder和prompt encoder冻结，从而实现参数高效迁移学习。同时使用Grad-CAM和pointing game进行可解释性评估。

Result: 在ISIC 2018数据集上达到0.9411 Dice系数和0.8918 IoU，优于完全训练的U-Net基线(0.8715)和零样本MedSAM推理(0.8997)。在PH2数据集外部验证中达到0.9467 Dice系数(±0.0310标准差)，Wilcoxon检验p值<0.0001，95%置信区间[0.9364,0.9447]。可解释性评估在519张验证图像上达到98.27%准确率。

Conclusion: PEFT-MedSAM是一种有效的皮肤病变分割方法，仅通过微调少量参数即可实现高性能分割，具有良好的临床应用潜力和可解释性。
...
TL;DR: 本文提出Geometry-Aware Pointer (GAP) Loss，通过逆距离加权重新加权交叉熵损失，使模型聚焦于相邻单元格错误，从而提升表格结构识别性能，在PubTabNet和SynthTabNet上达到新的最先进水平。


<details>
  <summary>Details</summary>
Motivation: -pointer网络在表格结构识别中失败案例的79.6%发生在空间相邻单元格（曼哈顿距离≤2），而标准交叉熵损失对所有负候选样本同等权重，无法有效处理这一局部错误集中问题。

Method: 提出GAP Loss：基于空间邻近度对交叉熵目标进行逆距离加权，使梯度流更集中在模型最困难的区域——相邻单元格，保持模型架构不变，仅修改损失计算，实现零额外推理成本。

Result: 在PubTabNet和SynthTabNet上大量实验表明，GAP Loss持续降低相邻单元格错误率达到新的最先进性能。

Conclusion: 在损失层面引入几何归纳偏置是一种简单而有效的鲁棒表格结构识别方法。
...
TL;DR: GeoCat是一个几何一致性网络,用于5帧IVUS视频的双视角编码器分割,结合可微几何损失函数监督临床相关指标,在12,242帧数据上实现Dice 0.93,95HD 0.14mm,拓扑错误率1.0%的性能。


<details>
  <summary>Details</summary>
Motivation: IVUS管腔和外弹性膜(EEM)分割对冠状动脉斑块负荷定量评估至关重要,但传统方法优先考虑重叠分数导致边界漂移和拓扑错误,直接影响斑块面积、斑块负荷和几何测量的准确性。

Method: 提出GeoCat几何一致性网络,处理5帧IVUS切片,采用双笛卡尔-极坐标编码器结合跨域注意力机制和时序融合。可微几何一致性损失函数直接监督临床相关描述符,包括直径、方向和横截面积。

Result: 在146例患者12,242帧标注数据上训练评估:Dice达0.93,95HD降至0.14mm,拓扑违规率降至1.0%,直径误差0.13-0.16mm,角度误差约8度。

Conclusion: GeoCat显著提高几何保真度,支持可靠的斑块负荷量化,为IVUS临床定量分析提供准确的几何测量基础。
...
TL;DR: CS3F是一个用于3D医学图像无监督异常检测的框架，利用2D预训练模型处理3D医学影像，通过多轴切片分解、体素化和交叉主体相似性实现异常评分。


<details>
  <summary>Details</summary>
Motivation: 临床医学影像面临患者群体异质性、标注数据稀缺等问题；现有零样本异常检测方法主要针对2D图像，难以直接扩展到3D医学体数据；缺乏大规模3D基础模型。

Method: 将3D体积沿多个解剖轴分解，用2D视觉Transformer逐片编码；通过相邻切片特征池化生成本地化体素标记；采用粗到细的标记策略实现细粒度异常定位；利用交叉主体相似性计算异常分数。

Result: 在脑MRI（转移瘤、胶质瘤、脑卒中）和肺CT上验证；证明冻结的2D基础模型可用于3D医学图像异常检测；细粒度标记的收益取决于病变对比度和成像模态。

Conclusion: CS3F成功实现训练自由的3D医学图像零样本异常检测，证明2D基础模型可迁移到3D医学影像任务，但细粒度评分的效果受影像特征影响。
...
TL;DR: SMART是一个从纵向MRI图像中学习灵活、可解释、可扩展的时空脑图谱的框架，用于预测阿尔茨海默病进展。该方法将全局疾病动力学与患者特定解剖表现解耦，通过区域特异性微分方程建模可解释的进展轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有时空图谱构建方法依赖黑盒生成模型，存在灵活性不足、可解释性差、难以扩展到高维数据等挑战。

Method: SMART通过区域特异性微分方程建模沿共享疾病时间线的全局进展轨迹，使用多尺度神经细胞自动机进行个性化映射，在五个纵向MRI数据集上验证。

Result: 在ADNI-1/GO/2、OASIS-3、AIBL数据集(超过1300名受试者)上，SMART产生了解剖学上有意义的疾病进展预测，实现了最先进的预测精度和改善的时间一致性。
...
TL;DR: SpectralDiT是一种针对流匹配扩散变换器的轻量级修改,通过在MLP残差分支中添加时间步调度的频谱校正来提升生成质量。该方法将残差更新分解为低频和高频分量,并使用零初始化加法门,使模型初始状态与基线DiT匹配。实验表明,该方法在CIFAR-10和ImageNet-100上均能提升FID指标,且仅增加极少的计算和参数量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在高分辨率图像生成领域取得显著进展,但现有方法在处理不同频率特征时缺乏针对性优化。文章指出基线DiT在处理低频和高频信息时可能存在效率问题,希望通过频谱分解来增强模型对不同频率成分的处理能力,从而提升生成质量并缩小径向傅里叶频谱差距。

Method: SpectralDiT在流匹配扩散变换器的MLP残差分支中添加时间步调度的频谱校正模块。具体包括:1)将残差更新在patch-token网格上分解为低频和高频分量;2)学习一个零初始化的加法门,使模型训练初期行为与基线DiT一致;3)该模块仅增加0.6%的理论FLOPs和1.36%的参数。

Result: 在CIFAR-10像素空间生成中,SpectralDiT将FID从20.78提升到19.71(patch大小为1),并缩小了径向傅里叶频谱差距。在ImageNet-100潜在扩散中,使用分类器无关引导(CFG 2.0)实现了8.7%的相对FID降低。所有结果均为五个随机种子的平均值。

Conclusion: SpectralDiT通过轻量级的频谱校正机制有效提升了流匹配扩散变换器的生成质量,且保持与基线模型的兼容性。该方法在极低的计算和参数开销下实现了显著的性能提升,为扩散模型的改进提供了一种简单而有效的方向。消融实验和门可视化显示了该方法在不同块上具有稳定的频谱校正模式。
...
### [37] [URDF Synthesis from RGB-D Sequences via Differentiable Joint Inference and Energy-Consistent Verification](https://arxiv.org/abs/2606.18861)
*Xinze Zhang*

Main category: cs.CV

TL;DR: KinemaForge是一个约束驱动的管道,从RGB-D序列中联合推断关节对象的部件级形状、关节拓扑和关节参数,并通过可微分的刚体动力学验证器验证结果是否符合能量守恒约束,从而生成可在物理模拟器中准确重放的数字孪生模型。


<details>
  <summary>Details</summary>
Motivation: 解决两个持续存在的差距:(1)部件级几何重建与运动参数估计解耦;(2)恢复的模型经常违反能量守恒等基本动态不变量,导致在URDF重放时产生漂移。

Method: 提出三个核心组件:1)运动约束图,将关节-部件关联编码为软边;2)可微分的螺轴求解器,通过Featherstone关节体算法反向传播到关节参数;3)能量残差损失,惩罚非物理的自由响应。管道从短RGB-D序列中联合推断部件级形状、关节拓扑和关节参数,并通过基于可微刚体动力学的能量一致性验证器验证。

Result: 在五个PartNet-Mobility类别和内部RGB-D基准上:关节轴平均误差从4.52度降至2.83度(比PARIS降低37.4%),从5.30度降至2.83度(比Ditto降低46.6%);50秒 rollout的长期模拟漂移降低64%(比PARIS);URDF封闭环操作成功率提高14.6个百分点(比Ditto)。

Conclusion: KinemaForge成功地将几何重建与运动参数估计联合优化,并通过能量守恒约束确保重建模型在物理模拟器中的动态准确性,为可仿真数字孪生的构建提供了新的方法。
...
TL;DR: 本文提出SAMA框架,一种统一的多模态数据增强方法,用于缓解多模态信息提取任务中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 多模态信息提取任务(如MNER、MRE、MEE)严重受限于数据稀缺问题,现有数据增强方法受限于粗粒度跨模态对齐和碎片化的任务特定设计,未能利用共享语义知识。

Method: SAMA框架包含三个核心组件:1)构建结构化语义锚引导CME-MLLM;2)使用锚保留扩散机制进行图像合成;3)引入双约束过滤模块消除人工验证需求。

Result: 在MNER、MRE和MEE任务的基准数据集上,全监督和低资源设置下,SAMA均一致优于最先进的增强基线。

Conclusion: SAMA展示了在多模态数据增强任务中的通用性、鲁棒性和有效性,为多模态信息提取任务提供了新的解决方案。
...
### [39] [ProductConsistency: Improving Product Identity Preservation in Instruction-Based Image Editing via SFT and RL](https://arxiv.org/abs/2606.19103)
*Mukund Khanna,Raj Singh Yadav,Kunal Singh*

Main category: cs.CV

TL;DR: 本文提出了ProductConsistency数据集和基准测试，用于提升产品中心图像编辑中保持产品特征、品牌和文字元素的能力，包含87k监督微调样本和869张强化学习图像，并设计了Cyclic Consistency奖励机制。


<details>
  <summary>Details</summary>
Motivation: 当前指令-based图像编辑模型在产品中心场景中难以保持细粒度对象身份（产品特征、品牌、文字元素），且缺乏专门的数据集和评估基准。

Method: 构建了ProductConsistency数据集（含87k SFT样本和869张RL图像）及ProductConsistency Benchmark基准；提出Cyclic Consistency奖励机制（通过比较原产品描述与编辑图像生成caption的相似度来强制保持产品身份语义）；对Qwen-Image-Edit-2511和Flux.1-Kontext-dev进行监督微调和强化学习训练。

Result: 在OCR和Perceptual指标及MLLM评估上均取得一致提升；Qwen-Image-Edit-2511模型实现字符错误率降低5倍。

Conclusion: 该数据集和训练方法能有效提升模型的产品一致性、文字渲染和整体视觉质量，为指令-based产品图像编辑提供了标准化的训练和评估框架。
...
### [40] [SCR-Guided Difficulty-Aware Optimization for Infrared Small Target Detection](https://arxiv.org/abs/2606.18783)
*Yunus Sevim,Behçet Uğur Töreyin*

Main category: cs.CV

TL;DR: 红外小目标检测面临严重背景干扰、低对比度和弱空间响应的挑战。提出REEM框架，通过将局部信号杂波比(SCR)作为物理可见性先验引入训练过程，使用可微分调制增强低可见性目标的软IoU学习信号，实现难度感知的优化。该方法集成到U-Net-based MSHNet中，不引入额外参数或推理时间开销。


<details>
  <summary>Details</summary>
Motivation: 传统方法仅依靠几何重叠（如IoU）来评估检测质量，对于低可见性目标效果不佳。现有方法未充分考虑目标的可见性困难程度，需要引入物理上有意义的先验来指导训练。

Method: REEM框架的核心是：1)从输入图像计算局部SCR作为可见性先验；2)使用可微分调制函数修改软IoU学习信号，自动强调低可见性目标的损失权重；3)将REEM集成到U-Net-based MSHNet中，不修改架构，不增加参数和推理时间。

Result: 相比基线模型，REEM在IoU和检测概率(Pd)上有一致提升，特别是在低可见性条件下显著降低虚警(FA)。实验验证了SCR引导的难度感知优化对红外小目标检测的有效性。

Conclusion: SCR引导的难度感知优化为红外小目标检测提供了有效且物理上合理的补充，比传统仅基于重叠的目标函数更好，尤其适用于低可见性场景。
...
### [41] [Learned Radius Estimation for UDF-Based Point Cloud Reconstruction](https://arxiv.org/abs/2606.18787)
*Eito Ogawa,Hiroshi Watanabe*

Main category: cs.CV

TL;DR: 提出一种基于学习的连续支持半径选择器，用于点云表面重建的局部_patch无符号距离场(UDF)方法，可自适应预测半径并集成到冻结的LoSF-UDF骨干网络中。


<details>
  <summary>Details</summary>
Motivation: 现有局部_patch UDF方法依赖固定或基于一维曲率的启发式半径选择，无法捕捉复杂异构局部几何结构，影响细尺度重建精度。

Method: 设计可学习的每查询半径预测器，通过抛物线插值缓存UDF误差曲线获取离网格目标半径进行训练，预测连续支持半径并接入冻结的LoSF-UDF骨干网络。

Result: 实验表明细尺度重建精度得到提升。

Conclusion: 该方法成功实现自适应半径选择，验证了连续半径预测在复杂几何重建中的有效性，为消费级3D捕获提供更精确的解决方案。
...
### [42] [OneCanvas: 3D Scene Understanding via Panoramic Reprojection](https://arxiv.org/abs/2606.19253)
*Bartłomiej Baranowski,Dave Zhenyu Chen,Matthias Nießner*

Main category: cs.CV

TL;DR: OneCanvas是一种新型的3D场景理解方法，将多视图patch特征聚合到单个等距长方投影全景画布上，无需复杂的几何编码器或大规模训练即可实现精确的空间推理。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法要么依赖复杂的模型特定几何编码器，要么需要大量训练预算来追求空间推理，缺乏一种简单而有效的空间推理方法。

Method: OneCanvas将所有视图的patch特征解投影到3D世界坐标，放置在连续经纬度位置，添加3D位置嵌入恢复深度信息。通过程序化放置物体patch特征生成空间推理预训练课程。

Result: 在SQA3D和VSI-Bench上达到最先进的准确性，在SPBench上泛化到分布外数据，使用比最强竞争方法少一个数量级的训练计算。

Conclusion: OneCanvas通过单一全景画布表示实现了高效的空间推理，为机器人和具身AI中的情境推理提供了直接支持。
...
### [43] [HandwritingAgent: Language-Driven Handwriting Synthesis in Scalable Vector Space](https://arxiv.org/abs/2606.18788)
*Jaward Sesay,Yue Yu,Börje F. Karlsson*

Main category: cs.CV

TL;DR: HandwritingAgent是一个无需针对特定风格训练的语言驱动智能体，可直接在SVG格式下合成自然手写笔迹序列。该方法利用大语言模型在离散网格画布环境中进行几何分析和自回归生成。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在手写合成方面存在局限性：需要针对特定风格的架构设计、依赖大规模数据集、计算成本高，且难以通过自然语言灵活控制书写风格。

Method: HandwritingAgent采用大推理模型进行几何分析，在离散网格画布环境中自回归生成目标手写字形作为笔迹序列。生成过程可基于对话式或非对话式文本，以及参考手写风格图像进行条件化。

Result: 在多样化的手写任务（包括模仿、识别、多语言手写合成、复杂数学科学表达式生成）上取得显著性能提升，HandwritingAgent达到或超越现有最先进的手写生成模型。

Conclusion: 该方法提供了一种更高效、可控且泛化能力更强的手写合成方式，无需针对特定风格进行训练即可实现自然手写序列的合成。
...
### [44] [A Multi-Domain Benchmark for Detecting AI-Generated Text-Rich Images from GPT-Image-2](https://arxiv.org/abs/2606.19259)
*Yijin Wang,Shuyi Wang,Wenhan Zhang,Yuqi Ouyang*

Main category: cs.CV

TL;DR: 本文提出了一个针对OpenAI GPT Image 2生成的多领域文本丰富图像检测基准，包含8602张图像，涵盖商业海报、信息图表、学术海报、收据、表格和UI截图六类。评估了五种AI生成图像检测器，发现性能高度依赖领域，且JPEG压缩会显著影响检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注目标图像，缺乏对文本语义和布局组织为核心的文本丰富图像的覆盖。随着多模态图像生成模型能力的增强，检测AI生成的文本丰富图像对于数字信任和内容真实性至关重要。

Method: 构建包含六类共8602张图像的基准数据集；在零样本设置下评估五种代表性AI生成图像检测器；分析类别-wise和后处理鲁棒性；使用多模态视觉语言模型进行探索性评估。

Result: 检测器性能高度领域依赖：在某些类别表现好的方法在另一些类别可能失效；即使最强的传统检测器对JPEG压缩极其敏感；视觉语言模型在结构化格式上既有潜力也有局限。

Conclusion: 需要开发针对现代AI生成图像的文本和布局感知检测方法，以应对不同领域的多样化挑战。
...
TL;DR: 针对手写识别中数据稀缺和结构失真问题，提出模糊几何驱动的结构感知(FGSA)增强框架，通过模糊分支点 membership 场实现无监督笔划解耦，结合Bézier曲线重建合成样本，并在新建的LZUSig数据集上显著降低词错误率。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法在处理复杂汉字时易造成拓扑结构损伤，尤其是笔画交叉、连笔和急弯处，传统分支点检测不可靠，导致识别精度下降。

Method: 将分支点建模为模糊集合，构建结合拓扑邻域证据和方向场散度的连续分支点membership场；通过无监督代理目标自适应优化；使用参数化三次Bézier曲线重建和多策略扰动合成 kinematically-aligned 样本。

Result: 在CASIA-HWDB1.1、ChiSig和LZUSig数据集上显著降低词错误率(ΔWER)，在任务增益、结构保真度和判别性特征保留之间达到良好平衡。

Conclusion: FGSA框架为手写数据增强提供了高度可控的解决方案，尤其适用于中文手写签名的细粒度结构退化场景。
...
TL;DR: 从自我中心视角进行行人轨迹预测的框架MMPM，通过将未来轨迹分布分离为语义上有意义的模式（过马路和非过马路）来改进预测。


<details>
  <summary>Details</summary>
Motivation: 现有随机预测器从单峰分布中采样多个未来，导致混合模式轨迹处于不同运动模式之间，在实际场景中变得不合理。

Method: 提出MMPM框架包含两个模块：1) 行为感知的行人交互模块(PIM)通过引入眼神、头部和手部姿势来共同捕捉行人与车辆和环境的交互；2) 基于CVAE的模式感知轨迹预测器(MTP)分别对过马路和非过马路两种模式的未来轨迹分布进行建模。查询解码器进一步强制模式一致性。

Result: 在PIE和JAAD数据集上的实验表明，该方法优于最先进的基线方法。MTP是模型无关的，可以集成到现有框架中进一步提高轨迹预测性能。

Conclusion: 该框架通过模式感知方法有效解决了轨迹预测的多模态问题，提高了预测的合理性和准确性。
...
TL;DR: DreamReg是一种基于信念驱动的世界模型框架，用于手术导航中的2D-3D超声配准。该方法通过维护潜在信念状态来总结过去的观察和姿态信息，并随着新切片的到来通过学习到的动态持续细化变换。在训练过程中，DreamReg暴露于模仿临床扫描行为的探头运动轨迹，并学习通过基于当前超声观察来细化姿态更新其信念。在推理过程中，DreamReg通过内部想象来细化配准：它展开学习到的世界模型来模拟候选探头运动及其预测观察，并将这些想象的结果整合起来以收敛到准确的刚性变换。


<details>
  <summary>Details</summary>
Motivation: 超声在手术导航中广泛应用，但术中2D切片与术前3D体积之间的实时配准仍具挑战性，原因包括部分可观测性、散斑噪声以及依赖动作的超声采集。现有的方法是一次性或短视野的，难以随时间收集证据，也难以捕捉外科医生如何根据屏幕反馈调整探头运动。

Method: DreamReg维护一个潜在信念状态来总结过去的观察和姿态信息，并随着新切片的到来通过学习到的动态持续细化变换。在训练过程中，DreamReg暴露于模仿临床扫描行为的探头运动轨迹，并学习通过基于当前超声观察来细化姿态更新其信念。在推理过程中，DreamReg通过内部想象来细化配准：它展开学习到的世界模型来模拟候选探头运动及其预测观察，并将这些想象的结果整合起来以收敛到准确的刚性变换。

Result: 在CAMUS和u-RegPro数据集上的实验表明，与最先进的方法相比，DreamReg具有更好的鲁棒性和具有竞争力的配准精度，适用于实时导航。

Conclusion: DreamReg提供了一种新的信念驱动方法来解决2D-3D配准的挑战，可处理部分可观测性、散斑噪声和依赖动作的超声采集等问题，具有更好的鲁棒性和具有竞争力的精度。
...
TL;DR: 本文提出Air-Ground Progressive Collaboration (AGPC)基准和Socialized Co-Perception (SCP)框架，解决空地协同感知中的任务依赖和视图异构性问题，通过Dual-Layer Router模块实现选择性跨视图跨任务交互。


<details>
  <summary>Details</summary>
Motivation: 现有空地协同感知研究存在两个关键问题：1)将协作简化为单任务跨视图融合，忽视了定位、目标关联和细粒度解析之间的功能依赖；2)航空与地面视图的异构性导致几何、尺度和遮挡差异，统一特征共享易产生负迁移。

Method: 构建了包含74.5万帧原始视频的AGPC时空对齐基准；提出粗到细的SCP框架，组织从航空全局定位到地面目标关联再到身份感知解析的渐进式协作；核心模块Dual-Layer Router解耦输入侧多尺度专家选择和输出侧任务条件调制，实现选择性跨视图跨任务交互同时抑制有害干扰。

Result: SCP框架取得3.73%的协同进化增益和7.86%的下游任务平均性能提升，验证了任务条件协作优于异构空地感知的统一融合。

Conclusion: 将空地感知建模为渐进式跨任务协作是有效的，任务条件调制协作比统一融合更适合处理航空与地面视图的异构性问题。
...
TL;DR: 提出开源标注工具ScreenAnnotator，解决视觉语言模型训练中空间坐标、开放词汇描述、结构属性和拓扑关系统一标注的系统性瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有数据标注工具存在三大系统性瓶颈：表达有限、标注与训练严重脱节、数据可重用性差，无法满足视觉语言模型对复杂结构化视觉推理的训练需求。

Method: 定义统一标注原子模式绑定空间、语义和结构原语；实现嵌入贝叶斯标注验证器的策略式标注循环；设计模板驱动的多任务数据合成流程，将静态原子转换为多样化多维推理任务。

Result: 流程图标注接受率接近100%，GUI截图达77%，随标注数据积累标注时间递减；微调VLM在流程图场景获76.1%平均准确率，绝对提升35.1个百分点。

Conclusion: ScreenAnnotator有效解决了标注基础设施缺口，通过统一标注模式和策略式循环显著提升标注质量与效率，为视觉语言模型的复杂推理训练提供了可行方案。
...
TL;DR: 本文提出QUAM-SM，一个用于医学图像中像素级不确定性量化的后处理框架。该框架通过对抗性搜索识别"对抗性脆弱"像素，区分认知不确定性和偶然不确定性，旨在解决深度学习模型在医学图像分析中的过度自信预测问题。


<details>
  <summary>Details</summary>
Motivation: 在临床应用中，可靠的像素级不确定性量化对于纵向监测、区分病理变化与伪影至关重要。标准深度学习模型常存在校准不当的问题，产生过度自信的预测，掩盖细微病理边界处的潜在脆弱性，这可能影响治疗规划和手术干预的决策。

Method: QUAM-SM采用后处理框架，使用有针对性的对抗性搜索来识别"对抗性脆弱"像素。通过主动寻找暴露预测不稳定性的扰动，该方法高亮显示最容易发生预测翻转的区域，并从认知不确定性中分离出偶然不确定性。

Result: 在两个具有多位专家标注的公共数据集上的实验表明，QUAM-SM在可靠性和边界敏感性方面均优于标准方法和近期的不确定性估计方法。

Conclusion: QUAM-SM能够为关键的治疗规划和手术干预提供所需的稳定性，有效识别决策最易受影响的区域，为临床工作流程的转型提供了潜力。
...
TL;DR: 本文提出了一种弱监督图像质量迁移(IQT)框架，用于校正单次激发回波平面前列腺扩散加权成像(DWI)中的几何畸变。


<details>
  <summary>Details</summary>
Motivation: 前列腺DWI常受几何畸变影响，缺乏成对的失真和未失真临床扫描作为训练数据，导致自动校正方法开发困难。

Method: 1)提出弱监督IQT框架，利用图像质量评估(IQA)信号作为监督，无需成对数据；2)开发弱监督原型流匹配算法，合成逼真的失真伪影；3)利用合成数据训练第二个IQT模型进行失真校正。

Result: 生成的图像成功模拟真实临床伪影的诊断干扰，产生了更有效的失真校正IQT模型。在PI-RADS和Gleason评分分类的下游临床任务中，性能优于现有非配对方法(CycleGAN、UNIT-DDPM、OT-FM)。

Conclusion: 该方法通过原型流匹配合成训练数据，有效解决了非配对图像校正的训练难题，为前列腺DWI的临床畸变校正提供了新思路。
...
TL;DR: 本研究提出了一种少样本双参数原型网络，用于前列腺多参数MRI的自动图像质量评估(IQA)，旨在解决临床中标注数据稀缺和质量评分主观性的问题。


<details>
  <summary>Details</summary>
Motivation: 前列腺MRI的扩散加权成像(DWI)常受几何失真影响，当前PI-QUAL评分系统主观、耗时且存在类别不平衡问题。PRIME临床试验显示6%图像PI-QUAL评分低于4，87%的DWI问题源于失真。标注临床数据的双重稀缺性限制了深度学习模型的应用。

Method: 提出少样本双参数原型网络：1)双分支3D ResNet融合T2加权像和DWI特征，提供解剖学上下文以区分真实形态与失真；2)特征级线性调制(FiLM)和梯度反转层(GRL)对齐不同b值条件下的特征分布，抑制获取相关偏差；3)仅使用5个代表性样本即可适应预测PI-QUAL评分。

Result: 在两个数据集上的实验表明，该方法显著优于少样本学习基线，为这一具有挑战性的IQA任务提供了切实可行且数据高效的解决方案。

Conclusion: 该框架可通过少量样本有效适应复杂的多因素临床质量评分，为前列腺MRI质量控制的临床工作流标准化提供了实用可行的方法。
...
TL;DR: 提出了一种基于流匹配（flow-matching）的测试时适应方法，用于从低质量OCT图像生成高质量替代图像，以支持AMD分期的生物标志物分割。


<details>
  <summary>Details</summary>
Motivation: 低成本设备上的OCT图像质量不一致，阻碍了自动化分析；同时，测试数据与训练数据之间的领域差异导致去噪过程中像素分布不匹配。

Method: 1) 使用流匹配生成高质量替代图像；2) 将测试图像直方图与合成参考轨迹匹配以对齐分布；3) 移除网络的时间调节以适应真实世界噪声偏差。

Result: 在两个年龄相关性黄斑变性（AMD）分期的关键生物标志物分割上达到了最先进的性能。

Conclusion: 该方法成功解决了领域分布不匹配问题，显著提升了低质量OCT图像的可用性，为AMD的自动化诊断提供了有效技术支撑。
...
TL;DR: 本研究通过统一的CRNN模型对阿拉伯语和拉丁语手写文本识别进行综合对比分析，揭示了阿拉伯语识别性能始终低于拉丁语约5-7个CER百分点，且在低资源场景下差距更大。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏控制性对比研究，阿拉伯语手写识别性能落后于拉丁语的根本原因尚不明确，亟需系统性的对比分析来理解这一差距的来源。

Method: 使用统一的CRNN模型在9个数据集（阿拉伯语KHATT、Muharaf、乌尔都语NUST-UHWR、波斯语PHTD、英语IAM、德语READ-2016等）上进行线级手写识别，测试不同训练样本量（100至完整数据集）的性能表现，并分析标注质量、字符频率分布和错误类型。

Result: 性能差距随数据量增加而缩小，但在完整数据下仍保持5-7个CER百分点；标注错误普遍存在，清理后可缩小差距；阿拉伯语因视觉变异性更高需要更多训练样本；阿拉伯语字符频率呈重尾分布；约30%的阿拉伯语替换错误源于相似字符混淆，拉丁语仅约15%。

Conclusion: 阿拉伯语手写识别的性能差距源于多重因素，包括标注质量、字符视觉相似性高导致的混淆以及字符分布的重尾特性，仅通过增加数据无法完全弥合这一差距。
...
TL;DR: 本文提出LARE框架，通过双编码策略显式建模低注意力区域，以解决拥挤场景中图像检索的传统视觉编码器偏差问题，并构建了Dense-Set数据集进行评估。


<details>
  <summary>Details</summary>
Motivation: 拥挤场景中的图像检索面临显著挑战，传统视觉编码器存在显着性偏差，倾向于关注主导对象而忽略低注意力区域，但这些区域对于细粒度检索往往至关重要。

Method: LARE采用双编码策略，并行编码图像的低注意力区域和完整图像，生成更多样化和信息丰富的图像嵌入。同时构建了Dense-Set数据集，从COCO和Flickr30K中提取并重新标注以提供更丰富的低注意力区域描述。

Result: 实验结果表明，所提出的框架通过在共享潜在空间中保留细微的非主导视觉线索，提高了检索性能。

Conclusion: LARE框架能够有效解决拥挤场景中的图像检索问题，为细粒度视觉检索提供了新的研究方向。
...
TL;DR: DINO-Med3D是一个两阶段渐进框架,用于将预训练的DINOv3模型从2D自然图像适配到3D医学分割任务。


<details>
  <summary>Details</summary>
Motivation: DINOv3在自然图像上表现出色,但由于维度差异(2D vs 3D)和领域差异(自然图像 vs 医学图像),无法直接应用于 volumetric 医学分割。

Method: 提出两阶段渐进框架:1)多切片嵌入模块+分割代理任务缩小维度和领域差距;2)轻量级3D适配器增强切片间连续性;3)并行细节恢复流保留高频边界信息。

Result: 在5个公共数据集上显著优于当前最先进方法,成功将DINOv3适配到医学领域。

Conclusion: 该框架有效解决了维度差距和领域迁移问题,为医学影像分割提供了新的预训练范式。
...
TL;DR: 提出一种自动化的方法来区分碳纤维增强聚合物（Carbon-Fiber Reinforced Polymer, CFRP）高分辨率显微图像中语义分割mask的不同层（ply）实例。


<details>
  <summary>Details</summary>
Motivation: 语义分割可以识别纤维像素，但无法区分不同层；现有方法缺乏从语义分割到层实例分割的桥梁，需要利用全局信息实现层实例分割。

Method: 将语义分割mask视为图（以像素为顶点），使用最短路径算法生成层分离路径（ply-separating paths），将每根纤维像素分配给对应层。

Result: 成功应用于具有不同特征的高分辨率显微图像，包括单层/多层人为添加的间隙、不同的堆叠序列和层穿透裂纹；可进行局部纤维体积分数、层厚度的定量分析。

Conclusion: 该方法可揭示制造引起的微观结构不均匀性，建立制造参数与力学性能之间的联系，为CFRP材料的质量分析提供有效工具。
...
TL;DR: BindEdit是一种用于真实图像编辑的方法，通过在单一扩散轨迹中实施注意力级约束来解决多目标编辑中的注意力泄漏问题，包括编辑-token泄漏和源主导泄漏。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂多目标场景中常出现语义混合、目标重复或编辑不完整等问题，这些失败归因于注意力泄漏——去噪过程中空间区域和文本token信号相互纠缠。

Method: 提出BindEdit方法：1)联合正则化交叉和自注意力以抑制编辑-token泄漏；2)交叉注意力再平衡机制放大目标token影响；3)区域保真度项确保目标概念在编辑掩码内一致表达。同时构建综合多目标基准测试。

Result: BindEdit在单目标和多目标编辑场景中均一致优于现有方法，在单一扩散轨迹内保持稳健性能。

Conclusion: 通过注意力级约束有效解决了多目标图像编辑中的注意力泄漏问题，显著提升了编辑质量。
...
TL;DR: 论文提出了Physics-IQ Verified基准，用于评估视频生成模型(VGM)对物理世界的理解能力，改进了原有Physics-IQ基准的样本质量和评分系统。


<details>
  <summary>Details</summary>
Motivation: 现有Physics-IQ基准存在缺陷，可能无法准确衡量视频生成模型的物理理解能力，需要系统性审计和改进。

Method: 对Physics-IQ基准进行系统性审计，提出三个解决方案：1)改进提示词质量 2)改进真值视频质量以减少混淆因素 3)引入样本级评分系统以等权重量化每个样本和指标。

Result: Physics-IQ Verified改进了57.6%的样本和34.8%的提示词，使用6个图像转视频模型进行对比研究，观察到中等但有意义的排名变化(Kendall's τ= 0.46)。

Conclusion: Physics-IQ Verified基准能够提供更可靠的信号，帮助社区推进物理准确视频生成模型的发展。
...
TL;DR: 构建了SP-TransientBench，一个用于单光子感知的真实多任务基准数据集，包含10个场景、10297个视图和13类3D语义标注。


<details>
  <summary>Details</summary>
Motivation: 现实世界的单光子感知面临独特的测量噪声和复杂的多回波瞬态现象，导致几何重建和语义场景理解困难，而现有研究主要局限于模拟数据或小规模控制捕获，缺乏系统性的真实世界评估。

Method: 使用固态单光子LiDAR在256×192分辨率下捕获10个多样场景和10297个视图，每个视图提供完整的时间飞行直方图、多回波行为、标准化元数据和校准的相机姿态，并提供13类3D语义注释。

Result: STB提供了专门的数分割和评估协议，支持深度估计、多视图重建和3D语义理解等多任务评估，实现了对真实世界单光子感知的可重复基准测试。

Conclusion: SP-TransientBench填补了真实世界单光子感知benchmark的空白，为单光子3D视觉研究提供了标准化的数据和评估框架。
...
TL;DR: 提出了一种基于潜动作的框架，从无标注的人类操作视频中提取通用动作先验知识，通过 Hybrid Disentangled VQ-VAE 解耦运动动力学与环境背景，构建跨化身动作码书，实现仅需50条轨迹即可完成下游适应的竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 训练通用VLA模型通常需要大规模、多样化且带有高精度动作标注的机器人数据集，而带标注的机器人数据稀缺；虽然第一人称人类操作视频丰富且具有环境多样性，但缺乏动作标签导致难以用于传统训练范式。

Method: 1) Hybrid Disentangled VQ-VAE：通过物理掩码解耦运动与环境，实现跨化身动作码书构建；2) 预训练阶段：在人类视频上学习动作意图的深度表征；3) 意图-感知解耦：VLM预测动作意图+冻结视觉编码器提供状态特征，减少幻觉。

Result: 在模拟和真实环境中，该方法仅使用无标注人类视频进行预训练，即可与使用大规模标注数据集训练的最先进VLA模型性能相当，下游适应仅需50条轨迹。

Conclusion: 该框架成功证明了从无标注人类视频中学习通用动作先验的可行性，为解决机器人数据稀缺问题提供了新思路，具有重要的实际应用价值。
...
TL;DR: Mem-World是一种记忆增强的多视角动作条件世界模型，用于机器人操作任务。其核心是W-VMem，一种基于手腕视角的surfel索引记忆，通过将历史观测锚定到时间演变的表面元素上来实现几何感知的记忆检索。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型在操作任务中面临挑战：频繁的末端执行器遮挡和快速的手腕相机运动导致观测不足，模型会遗忘或产生幻觉。现有记忆检索策略在动态操作场景中往往无法识别有意义的历史信息。

Method: 提出W-VMem（4D手腕视角中心surfel索引记忆），显式建模场景元素何时何地被观测，通过基于surfel的渲染和评分选择相关的历史帧，为预测提供信息丰富且无冗余的上下文。

Result: Mem-World在复杂操作场景中生成持久的视频rollout，相比Ctrl-World，与真实世界性能的皮尔逊相关性提高14.5%，在长时域任务中通过合成数据生成将成功率从58%提升至72%。

Conclusion: 该方法实现了更可靠的政策评估，并通过合成数据生成支持有效的策略改进，为机器人操作中的世界建模提供了新的解决方案。
...
TL;DR: 本文提出Visual-OPSD方法，通过自蒸馏将包含视觉思考的生成路径中的语义知识迁移到纯文本学生模型，在保持推理能力的同时实现14.3倍加速。


<details>
  <summary>Details</summary>
Motivation: 统一 multimodal models (UMMs)通过生成视觉思考(VTs)来提升空间任务推理，但推理成本高出一个数量级，且直接收益有限。作者试图探究生成路径中的语义知识是否可以在不增加推理成本的情况下被迁移到纯文本模型。

Method: 提出Visual On-Policy Self-Distillation (Visual-OPSD)方法：教师和学生模型共享权重，但教师能看到privileged VTs而学生只能看到问题。通过on-policy学生轨迹上的token-level JSD蒸馏将教师的推理能力迁移到纯文本学生模型。

Result: 在9个基准数据集上，Visual-OPSD相比生成式教师提升+3.40pp，实现14.3倍加速(10.0s vs 142.8s/样本)，在VSP上比同规模VLMs提升+63.83pp。Gaussian噪声控制实验(+0.40pp vs +10.28pp for real VTs)和58.4%的KL gap闭合证实收益来自生成路径的语义内容。

Conclusion: 视觉思考生成路径编码了超越渲染像素的有用推理知识。Visual-OPSD通过自蒸馏成功将这些知识迁移到纯文本学生模型，在大幅降低推理成本的同时提升性能，验证了知识迁移的有效性。
...
TL;DR: CLARA是一个解决组合图像检索(CIR)中用户意图歧义的框架，通过向用户展示视觉替代原型图像来获取澄清，而不是依赖文本问题。该方法使用重新加权校准来维持跨多轮交互的保形预测保证。


<details>
  <summary>Details</summary>
Motivation: 现有CIR方法存在两个局限：1) 保形预测的覆盖率保证仅在第一轮交互有效；2) 文本问题难以解决细粒度视觉差异（如外观、属性、视角）。这些限制导致用户意图仍然模糊，无法准确检索目标图像。

Method: CLARA框架：1) 显示包含视觉替代原型的小面板而非文本问题；2) 用户选择最接近目标图像的原型；3) 使用用户选择产生的似然比重新加权校准，维护多轮交互的保形保证；4) 原型被约束表示当前候选集并对齐到真实语料库图像。

Result: 实验表明：CLARA匹配单轮最先进的检索性能；在多轮交互中保持标称覆盖率；比强文本问题基线更少的轮次找到目标；当歧义涉及视角或细粒度属性时，视觉澄清优势尤其明显。

Conclusion: CLARA通过视觉澄清而非文本问题有效解决CIR中的用户意图歧义，为细粒度视觉差异提供更直接的信号，避免依赖模型预测用户回答，并维持有效的保形预测保证。
...
TL;DR: 本文提出FlowObject框架，解决从稀疏视角图像恢复完整3D表示的难题。该方法结合 Flow-Matching 生成模型与 3D Gaussian Splatting 优化技术，通过双空间引导策略在无需训练的情况下重建遮挡区域的几何和纹理。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两大局限：Flow-Matching等生成模型存在"合成偏差"，学习到的先验会覆盖真实观测证据；3DGS等优化方法虽然能高精度重建可见表面，但无法推理未观测区域的几何。在稀疏视角和严重遮挡场景下，现有方法难以同时实现几何完整性和观测一致性。

Method: 提出双空间引导策略，将稀疏视图3D重建重新表述为无需训练的引导逆问题。通过引导Flow-Matching模型的常微分方程(ODE)轨迹，使其在生成未见区域时同时保持与真实观测的几何和外观一致性。随后集成3DGS精炼阶段，消除生成结果的"合成感"，实现照片级真实感重建。

Result: 在合成和真实数据集上进行全面基准测试，实验表明该方法在几何完整性和视图依赖外观保真度方面显著优于最先进的生成模型和优化框架。特别是在严重遮挡条件下，能同时保证观测一致性。

Conclusion: FlowObject成功实现了生成模型先验与观测证据的平衡，在稀疏视图3D重建任务中同时达到几何完整性、观测一致性和视觉真实感，为从少量 casual图像重建完整3D物体提供了有效解决方案。
...
TL;DR: 提出张量核范数与Ky Fan p-k范数比值(TNPK)作为新的非凸代理函数来解决低秩张量补全问题，并设计了高效的ADMM算法


<details>
  <summary>Details</summary>
Motivation: 低秩张量补全（LRTC）广泛应用但传统凸代理函数存在局限性，需要更准确的张量秩近似方法来提升恢复效果

Method: 提出TNPK非凸代理函数，具有尺度不变性、参数灵活性；在特定参数下可简化为TNK或TNF；构建LRTC模型并利用张量零空间性质证明低秩张量是局部最小解；推导近端算子并开发ADMM算法

Result: 在合成和真实数据集上验证了方法相比现有先进方法的优越性能，证明了ADMM算法在温和条件下的次优收敛性

Conclusion: TNPK能够更准确地近似张量管秩，为低秩张量补全提供了有效的非凸正则化框架，具有重要的理论价值和实际应用前景
...
TL;DR: FG-BMK是一个用于评估大型视觉语言模型(LVLMs)细粒度图像任务能力的新基准,包含101万道问题和28万张图片,覆盖从常见目标域到专业域的多种场景。该基准通过人机两种范式联合评估细粒度语义识别和视觉可辨别性,揭示了当前LVLMs在细粒度识别方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准对LVLMs的细粒度图像任务能力评估不足,而细粒度任务是计算机视觉的基础。文章指出需要理解LVLMs在细粒度图像任务(如精细目标识别、特征区分等)上的表现,这些任务对于人类视觉感知和计算机视觉应用都至关重要。

Method: 构建了FG-BMK基准,包含101万道问题和28万张图片,涵盖通用目标域到专业域的多样化场景。该基准采用两种评估范式:(1)人机对话层面的细粒度语义识别;(2)特征层面的视觉可辨别性。通过诊断分析判断LVLM失败源于视觉表征不足、视觉-语义对齐薄弱还是细粒度知识缺乏。

Result: 实验表明当前LVLMs作为细粒度识别器仍不达标,失败源于多重交织的瓶颈:视觉表征、语义对齐、模态对齐和类别级知识。进一步分析了训练设计因素对细粒度能力的影响,以及视觉和语言扰动对预测的影响。

Conclusion: 该研究为当前LVLMs的局限性提供了诊断性见解,为未来数据构建和模型设计提供了指导,有助于开发更可靠的细粒度视觉任务LVLMs。
...
TL;DR: DREAM是一种新型多模态框架，通过双路径表示增强和对齐机制实现基于自然语言查询的视频检索。模型结合层次化视觉编码器和混合语言建模策略，在MSRVTT、MSVD和LSMDC三个基准数据集上分别达到49.4%、49.7%和27.3%的R1分数，创下新纪录。


<details>
  <summary>Details</summary>
Motivation: 随着视频内容在监控、教育和娱乐等领域的指数级增长，通过自然语言查询检索语义相关视频的需求日益迫切。早期视频检索系统依赖手工特征或浅层跨模态映射，难以捕捉复杂语义和时间动态；现有大规模视觉语言模型虽然在跨模态对齐方面有所改进，但在建模细粒度时间依赖和复杂语言结构方面仍面临挑战。

Method: 提出DREAM框架，包含两部分核心设计：1) 混合语言建模策略，结合掩码语言建模(MLM)和排列语言建模(PLM)目标，以捕捉局部和全局语言语义；2) 层次化视觉编码器，采用级联组注意力机制，通过多阶段令牌交互和粗到细的注意力细化来整合空间和时间信息。

Result: 在MSRVTT、MSVD和LSMDC三个广泛使用的视频检索基准数据集上进行综合评估，分别取得49.4%、49.7%和27.3%的R1分数，达到当前最先进水平。定性分析表明模型能够保持跨帧的一致性注意力，并将复杂查询与动态视频内容对齐。

Conclusion: 层次化注意力和双目标文本建模能够实现强健的上下文感知视频检索，为跨模态表示学习的未来研究奠定基础。
...
TL;DR: 本文提出HOI-Edit基准和SCPE框架，解决现有图像编辑方法无法处理复杂人物-物体交互(HOI)的问题，通过图像到视频模型的迭代提示约束实现自纠正编辑。


<details>
  <summary>Details</summary>
Motivation: 当前图像编辑方法擅长静态属性但无法处理复杂的人物-物体交互任务，现有基准将HOI与静态属性混为一谈，使用全局指标无法同时评估动态交互有效性和人物-物体对保真度。

Method: 提出HOI-Edit基准包含三个递进认知层级，引入HOI-Eval自动度量通过VLM问答评估实例级交互；采用图像到视频(I2V)模型进行动态编辑；提出SCPE自纠正框架通过迭代细化提示约束I2V模型生成。

Result: SCPE在HOI-Edit上达到与Nano Banana等SOTA编辑模型相当的交互性能，I2V模型的时序生成能力提供了"失败过程回放"的可诊断性。

Conclusion: I2V模型因其时序生成能力天然适合动态编辑任务，SCPE通过迭代提示约束实现更准确的目标HOI呈现，为复杂人物-物体交互编辑提供了新思路。
...
TL;DR: 本文介绍了PorTEXTO，这是首个针对现代欧洲葡萄牙语(pt-PT)的视觉文本提取基准数据集，旨在填补pt-PT在OCR基准测试中的空白。


<details>
  <summary>Details</summary>
Motivation: 欧洲葡萄牙语在现有OCR基准测试中基本缺失，现有基准测试主要覆盖高资源语言或历史文献，缺乏针对现代应用场景的pt-PT OCR评估资源。

Method: 构建了PorTEXTO基准数据集，采用前沿LVLM生成转录结合母语者详细审核的标注流程，测试了多种模型在合成与真实样本上的性能表现。

Result: 大多数模型存在从合成样本到真实样本的显著性能下降，研究发现专门的 multilingual 数据比模型规模或分辨率更能驱动pt-PT性能的提升。

Conclusion: 研究结果强调了开放pt-PT OCR资源的必要性，为未来pt-PT OCR系统发展提供了重要参考。
...
TL;DR: 本文提出DVANet，一个基于深度展开的统一图像恢复框架，通过结合退化感知观测一致性和视觉先验引导重建来处理复杂退化类型。方法创新性地引入DINOv3作为层次化视觉先验，在统一的半二次分裂优化框架下实现对多种退化场景的适应性处理。


<details>
  <summary>Details</summary>
Motivation: 现有端到端方法将恢复过程视为黑盒映射，缺乏显式优化解释；而深度展开方法虽提供可解释的迭代建模范式，但多依赖固定退化假设，难以适应复杂退化下的统一恢复需求，尤其是在局部损坏内容的处理上存在局限性。

Method: 提出DVANet深度展开网络，基于半二次分裂优化算法，将统一图像恢复表述为退化感知观测一致性与视觉先验引导重建的协作展开过程。退化感知分支包含退化表示模块提取全局退化属性和局部退化线索，通过退化条件映射增强模型对不同退化类型的适应性；视觉先验分支引入DINOv3提供结构和语义信息作为层次化视觉先验，补充损坏区域的缺失结构信息。

Result: 在多场景退化和跨域图像恢复任务上进行了广泛实验，DVANet实现了优越或具有竞争力的性能，展现出良好的退化适应性和泛化能力。

Conclusion: 通过深度展开框架与视觉先验的创新结合，DVANet有效解决了复杂退化环境下统一图像恢复的可解释性和适应性问题，为多类型退化图像恢复提供了新的研究思路。
...
TL;DR: AMALIA-VL是首个针对欧洲葡萄牙语(pt-PT)的开源指令微调大型视觉语言模型(LVLM)，通过高分辨率视觉编码器、动态图像切片和完全开放的pt-PT优化语言模型构建，采用三阶段训练流程和pt-PT中心的多模态数据混合，为欧洲葡萄牙语LVLM开发奠定坚实基础。


<details>
  <summary>Details</summary>
Motivation: 欧洲葡萄牙语(pt-PT)在现有开源多模态模型中持续被忽视，这些模型要么将其与巴西葡萄牙语混淆，要么在训练数据中严重低估欧洲葡萄牙语，导致缺乏针对欧洲葡萄牙语的开源多模态资源。

Method: 构建了高分辨率视觉编码器+动态图像切片+完全开放的pt-PT优化语言模型的三组件架构；采用三阶段训练流程——视觉-语言对齐、通用视觉指令微调和偏好优化；设计了以欧洲葡萄牙语为中心的多模态数据混合，结合 curation数据集、翻译公共数据集和针对欧洲葡萄牙语的新数据集。

Result: AMALIA-VL建立了欧洲葡萄牙语开源LVLM的强基线，模型权重、训练数据、构建流程和机器翻译的欧洲葡萄牙语评估基准将全部开源发布。

Conclusion: AMALIA-VL是首个为欧洲葡萄牙语原生构建的开源指令微调LVLM，其三阶段训练流程和pt-PT中心的数据混合为欧洲葡萄牙语多模态模型开发提供了可复现的基线和资源 democratization。
...
TL;DR: 本研究提出了首个乌尔都语离线手写文本行数据集UKHD，专门从历史Katibs手写材料中收集，采用Nastalique书法风格。同时评估了多种CRNN混合模型，其中CNN-BGRU-CTC模型在乌尔都语手写识别中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语手写识别研究相对滞后，主要由于其脚本的特殊挑战和基准数据集的匮乏。该研究旨在促进UKHR系统开发，保护乌尔都语手写文献。

Method: 收集并标注了UKHD数据集；评估了基于CRNN的混合模型（CNN-BGRU-CTC等），使用CTC解码进行训练。

Result: CNN-BGRU-CTC模型表现更稳健，字符错误率(CER)和词错误率(WER)均较低。

Conclusion: UKHD是首个专门针对Katibs手写材料的乌尔都语离线数据集，为构建鲁棒的乌尔都语手写识别系统提供了重要基础。
...
TL;DR: 提出首个前馈框架Hand-4DGS，实现从 egocentric（自我中心）视频直接重建动态4D手部模型，在约60FPS推理速度下实现强泛化能力，无需昂贵3D手部姿态标注。


<details>
  <summary>Details</summary>
Motivation: 动态3D手部重建对AR/VR和AI眼镜等下一代计算平台至关重要，但egocentric 4D手部重建面临头部快速运动、手部动态变化、严重遮挡和单视角观察固有歧义等挑战，此前缺乏有效解决方案。

Method: 提出基于mesh引导的表示方法提供结构先验，结合时序卷积建模动态运动；采用Gaussian splatting实现有效的2D图像监督，避免使用昂贵的3D手部姿态真值标注。

Result: 在H2O和ARCTIC两个具有挑战性的egocentric数据集上验证，相较于基线方法取得显著提升，实现约60FPS的快速推理和强泛化能力。

Conclusion: Hand-4DGS是首个前馈框架，能够从egocentric视频直接重建动态4D手部，平衡了推理速度、泛化能力和标注成本，为下一代AR/VR设备提供关键技术支撑。
...
TL;DR: 针对深伪检测提出了一种新的评估指标Cross-AUC，用于更好地评估检测器在领域偏移下的泛化能力，同时考虑预测极化程度。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅使用单一数据集的AUC，无法反映真实世界中检测器面对多种数据源和不同伪影类型的情况，忽略了领域偏移带来的性能下降。

Method: 提出Cross-AUC指标，通过对每个领域的AUC取平均值，并结合使用Wasserstein距离量化类别分数分布的预测极化程度来评估模型泛化能力。

Result: 在七个基准数据集上的实验表明，Cross-AUC能更真实地评估深伪检测器的泛化能力，并且具有更好的可解释性，能解释性能下降的原因。

Conclusion: Cross-AUC是一个更现实、更可解释的评估指标，能够更好地评估深伪检测器在领域偏移下的泛化能力。
...
TL;DR: Moebius是一个高效轻量的图像修复框架，通过局部-λ混合交互块和自适应多粒度蒸馏策略，在仅使用0.22B参数（不足2%）的情况下，实现了与10B级工业模型FLUX.1-Fill-Dev相当甚至更优的修复质量，同时推理速度提升超过15倍。


<details>
  <summary>Details</summary>
Motivation: 10B级别的工业基础模型虽然推动了图像修复的发展，但其高昂的计算成本严重阻碍了实际部署；而极度的结构压缩又会导致严重的表示瓶颈。因此需要一种高效且保持高质量的轻量级解决方案。

Method: 1. 提出Local-λ Mix Interaction (LλMI)块，包含Local-λ和Interactive-λ模块，将空间上下文和全局语义先验总结为固定大小的线性矩阵，在大幅减少参数的同时保留复杂潜在交互；2. 采用自适应多粒度蒸馏策略，仅在潜在空间内操作以避免昂贵的像素空间解码，动态平衡多个基于梯度的损失以实现高保真对齐。

Result: 在自然图像和人像基准测试上的广泛实验表明，Moebius能够与10B级工业通用模型FLUX.1-Fill-Dev相媲美甚至超越其生成质量；使用0.22B参数（不足11.9B的2%），推理速度提升超过15倍。

Conclusion: Moebius开创了高效高保真图像修复的新标准，验证了通过精心设计的轻量级架构配合先进的蒸馏策略，可以在极低参数和计算成本下实现与大型模型相当的性能，为实际部署提供了可行的解决方案。
...
TL;DR: ROSA-TFormer是一种雷达-光学传感器感知的时间Transformer模型，用于利用Sentinel-1/2时间序列数据在Google Earth Engine上对樟子松进行精确分类。


<details>
  <summary>Details</summary>
Motivation: 准确识别樟子松人工林对监测陕北地区造林质量和生态恢复具有重要意义。

Method: 该模型整合了独立的SAR和光学嵌入分支、传感器感知门控和时序注意力池化，以捕获多源季节性特征。实验在月度和半月点级数据集上进行。

Result: ROSA-TFormer在HalfMonth-dataBig数据集上达到99.67%的总体准确率、99.56%的宏F1分数和98.91%的樟子松F1分数。空间块验证和消融实验结果表明雷达-光学时序融合和传感器感知建模的有效性。

Conclusion: ROSA-TFormer在点级樟子松人工林分类中展现出良好的性能，但仍需进行更广泛的wall-to-wall验证。
...
TL;DR: GUMP-Net是一种结合改进的测地活动轮廓模型与深度神经网络的可解释骨盆分割方法，通过目标检测、边缘检测和迭代三个网络模块实现多类别骨盆分割，在小样本训练数据下表现出更高的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 骨盆分割是精准诊断治疗及骨折手术规划导航的基础研究问题。现有方法在训练数据有限时分割精度不足，且缺乏可解释性。

Method: 提出GUMP-Net框架，包含三个模块：1)目标检测模块用于自动水平集初始化；2)边缘检测模块学习解剖感知的边缘检测函数；3)迭代模块实现深度水平集演化。融合测地活动轮廓模型与深度学习的优势。

Result: 在骨盆数据集上实验表明，该方法相比现有最优方法具有更高准确性、鲁棒性和一致性，尤其在小样本训练数据场景下表现优异。扩展到踝关节数据集验证了方法的泛化能力。

Conclusion: GUMP-Net为复杂骨折复位提供了高效分割方法，同时为深度学习分割提供了可解释的几何视角，具有广泛的临床应用前景。
...
TL;DR: 论文提出TGO框架，系统研究Vision Transformers的表示几何特性，发现训练过程中维度利用率增加、各项异性减少、谱熵增加，CLS token具有最高有效维度。


<details>
  <summary>Details</summary>
Motivation: 尽管ViT在计算机视觉领域广泛应用，但对其维度表示几何的基础理解仍相对缺乏，需要系统性的实验和分析框架来揭示ViT表示的内在特性。

Method: 使用TGO-I框架，对ViT-Small/16模型（在ImageNet-100上训练）进行系统分析，测量有效秩、稳定秩、参与比、谱熵、谱平坦度、谱各项异性、协方差结构、特征谱和奇异值谱等指标。

Result: 训练过程中维度利用率持续增加，各项异性降低，谱熵增加，参与比增加，特征谱逐渐变平。信息并非集中到少数主导方向，而是逐步在表示维度间重新分布，CLS token具有最高有效维度和最低各项异性。

Conclusion: 与常规认知相反，ViT训练并非将信息压缩到少数主成分，而是实现方差的跨维度重分配，这一现象在最终CLS token中最显著。
...
TL;DR: CABLE是一个用于车联网(V2X)系统中边缘-云端感知的带宽高效LMM编码框架，通过ROI掩码传输和反馈机制显著降低通信开销和预填充延迟。


<details>
  <summary>Details</summary>
Motivation: 云托管的大语言多模态模型(LMM)虽能为V2X系统提供强大的开放词汇感知能力，但直接将全分辨率帧从边缘传输到云端会导致严重的通信开销和云端预填充延迟过高的问题。

Method: CABLE框架包含三个核心机制：1)利用自我运动补偿在边缘传播前一云的分割掩码；2)使用残差运动线索进行细化；3)通过走廊包络线整合断开区域形成鲁棒的兴趣区域(ROI)。仅上传ROI掩码图像，云分割输出作为下一帧的先验，形成mask-ROI-LMM反馈循环。

Result: 在nuScenes、WOD-ZB、Waymo、KITTI和CADC五个数据集上的实验表明，实现了73%-87%的ROI像素覆盖 reduction，5-8倍的LMM预填充加速，同时保持感知性能仅有轻微检测质量下降。

Conclusion: CABLE在显著降低通信开销的同时有效保持了感知性能，为边缘-云端协同感知提供了一种实用的带宽高效解决方案。
...
TL;DR: 研究提出RS Adapter参数高效微调策略，将轻量级瓶颈适配器注入三种视觉语言模型(CLIP、BLIP、FLAVA)的冻结骨干网络中，在RSVQAx数据集上实现仅用不到5%的可训练参数即达到收敛，其中Hybrid FLAVA架构表现最优。


<details>
  <summary>Details</summary>
Motivation: 遥感VQA面临高分辨率航拍图像的领域偏移问题和全参数微调的计算成本过高问题，需要一种参数高效且能快速适配的方法。

Method: 统一架构手术管道：将轻量级瓶颈适配器注入到冻结骨干的注意力和MLP层中，在RSVQAx高分辨率数据集上进行对比实验。

Result: 所有适配模型均能实现收敛，Hybrid FLAVA在多模态推理和检索能力上表现优于单模态 counterparts，为资源高效VQA建立新基线。

Conclusion: 参数高效微调策略在遥感VQA任务中有效，Hybrid FLAVA架构更适合灾难评估和城市监测等应用场景。
...
TL;DR: 研究MC Dropout能否通过不确定性估计识别脑胶质瘤分割中临床关键子区域的错误。在126例BraTS21患者中比较SegResNet和UNet-Res两种模型，发现强不确定性-错误对齐(AUROC≈0.97)虽必要，但子区域校准评估对临床部署不可或缺。


<details>
  <summary>Details</summary>
Motivation: 当前分割模型在治疗关键子区域上的静默失败构成患者安全风险，而传统Dice等重叠指标无法暴露这种错误。亟需验证不确定性估计是否能可靠识别临床关键子区域的分割错误。

Method: 在126例BraTS21患者上比较预训练高性能SegResNet与本地训练UNet-Res，采用MC Dropout进行体素级不确定性估计，通过熵H的AUROC评估不确定性-错误对齐能力，使用Expected Calibration Error (ECE)评估校准失败模式。

Result: MC Dropout保持分割精度(|ΔDice|<0.01)，熵基AUROC≈0.97表明不确定性正确排序错误体素。高不确定性亚组whole-tumour Dice中位数0.835 vs 0.925。但UNet-Res显示增强肿瘤熵≈0.054、ECE=0.915、Dice仅0.714，严重校准失败隐藏于标准指标下。

Conclusion: 强不确定性-错误对齐对临床安全必要但不足，子区域特异性校准评估必须伴随AUROC报告。建议临床部署前进行子区域校准评估，而非仅依赖全局指标。
...
TL;DR: 本文提出了一种基于网格的神经辐射场表示方法，通过将几何、纹理和语义代码解耦编码到网格顶点，实现高效的编辑功能，包括网格引导的几何编辑、指定纹理编辑（交换、填充、绘画）和语义引导的编辑。


<details>
  <summary>Details</summary>
Motivation: 现有神经隐式渲染方法在编辑功能方面受限，仅支持刚性变换和类别特定编辑，缺乏灵活、全面的编辑能力。

Method: 提出一种新型网格表示方法，将神经辐射场与解耦的几何、纹理和语义代码编码到网格顶点；开发了局部空间参数化以增强渲染质量和训练稳定性；引入顶点可学习修改颜色以提高纹理编辑保真度；设计空间感知优化策略实现精确纹理编辑；采用语义辅助区域选择简化隐式场编辑的标注工作。

Result: 在真实和合成数据集上的广泛实验和编辑示例表明，该方法在表示质量和编辑能力方面均表现出优越性。

Conclusion: 该方法成功实现了全面的编辑功能，为神经隐式渲染的编辑应用提供了有效解决方案。
...
TL;DR: RNG-Bench是一个基准套件，用于隔离基础模型重建过去观察并在多步交互中根据它们采取行动的能力。该基准包含两个互补游戏：配对游戏（Matching Pairs）和3D迷宫游戏（3D Maze），在三个难度轴上进行评估：网格大小、视觉模式和观察模态。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么暴露完整状态，要么将隐藏状态重建与其他智能体技能混为一谈，或仅在 episode 结束后测试回忆。部署多模态基础模型作为闭环策略越来越需要根据不再可见的观察来调节动作。

Method: 引入RNG-Bench基准，包含两种互补游戏：配对游戏（玩家回忆特定位置短暂显示的卡片身份）和3D迷宫游戏（将自我中心视图整合成空间地图）。在三个受控难度轴上进行评估：网格大小、视觉模式、观察模态。引入一对一 duel 协议控制实例级方差，以及Memory Gap指标来解耦遗忘与糟糕的动作选择。

Result: 最难配置需要约128K tokens和350张图像输入每 episode，前沿多模态大语言模型仍远未达到饱和。Memory Gap分析表明大多数残余错误源于遗忘早期观察而非次优决策。对Qwen3.5-9B进行微调可提升RNG-Bench性能并迁移到现有基准而不会降低通用多模态能力。

Conclusion: 该基准成功隔离了多模态模型重建过去信息的能力，揭示当前模型的主要瓶颈是遗忘而非决策制定。最佳策略 rollout 微调后的Qwen3.5-9B能在该基准上取得进展且泛化能力良好。
...
TL;DR: 提出OmniAgent，首个原生全模态代理，将长视频理解建模为基于POMDP的迭代观察-思考-动作循环，通过按需行动选择性提取音视觉线索到持久文本记忆中，解耦推理复杂度与原始视频时长。


<details>
  <summary>Details</summary>
Motivation: 现有被动视频理解模型采用"观看全部"范式，无论查询难度统一处理所有帧，导致计算成本随视频时长线性增长。交互框架虽已出现，但仍依赖全局预扫描，上下文成本仍随视频长度扩展。

Method: 1) 代理监督微调：通过N条轨迹最佳合成与双阶段质量控制引导原生主动感知；2) 代理强化学习TAURA（轮次感知自适应不确定性重缩放优势）：利用轮次级熵将信用分配转向关键发现轮次。

Result: 在10个基准测试（如VideoMME、LVBench）上达到开源模型最优性能。LVBench上，7B代理超越10倍规模的Qwen2.5-VL-72B（50.5% vs 47.3%）。

Conclusion: OmniAgent展现出正向测试时扩展性——性能随推理轮次增加而提升，验证了主动感知的有效性。
...
### [86] [Denoising Distances in Metric Measure Spaces](https://arxiv.org/abs/2606.18301)
*Han Huang,Pakawut Jiradilok,Elchanan Mossel*

Main category: cs.CG

TL;DR: 研究在更一般的度量测度空间中从噪声距离中提取聚类和去噪的问题，提出了一种算法可以在任何固定精度下去噪距离，且运行时间接近线性，同时表明高精度的去噪存在统计-计算间隙。


<details>
  <summary>Details</summary>
Motivation: 先前工作仅研究流形上采样点的聚类发现和距离去噪问题，作者希望将问题推广到更一般的度量测度空间中，并探索在更一般条件下的计算复杂性。

Method: 提出一种算法，提取每个采样点周围的大规模局部聚类，并利用这些聚类对距离进行去噪；在密集固定精度制度下实现接近线性运行时间；对于更高精度采用非高效算法。

Result: 算法可以在任何固定精度下对距离进行去噪，运行时间接近线性；表明在更一般的度量空间中，高精度去噪需要更多的计算资源，存在统计-计算间隙。

Conclusion: 与黎曼流形情况不同，在更一般的度量空间中，高精度去噪存在统计-计算间隙，这揭示了更一般度量空间的固有计算复杂性。
...
### [87] [Repair Entropy in Dynamic Geometric Nearest-Neighbour Structures](https://arxiv.org/abs/2606.18314)
*Faruk Alpay,Bugra Kilictas*

Main category: cs.CG

TL;DR: 研究动态几何数据结构，实现小运动下的精确最近邻维护。通过存储包含最近邻和两个最小邻居距离的证书（间隙ci=d2-d1），利用三角形不等式给出清晰的有效性半径：位移ε后，ci>4ε的证书保持有效。引入修复前沿熵H(Ft)作为工作负载描述符，在事件驱动修复、批量修复和完全重建之间选择。修复规则仅修复前沿，时间复杂度O(|Ft|log N)，完全重建需Θ(N)。在d∈{{2,3}}、N达16000的10种运动族上评估，2400个标记转换中有效性规则无误报，低压前沿通常更便宜，弥散前沿对事件驱动修复更贵。


<details>
  <summary>Details</summary>
Motivation: 实现小运动下精确最近邻的高效动态维护，避免每次更新都进行完全重建，提高动态最近邻查询的效率。

Method: 证书方法：每个点存储最近邻和两个最小邻居距离，间隙ci=d2-d1；基于三角形不等式确定有效性半径ci>4ε；引入修复前沿熵H(Ft)作为选择修复策略的工作负载描述符；修复规则仅处理前沿在O(|Ft|log N)时间内完成。

Result: 有效性规则在2400个标记转换中无误报；低压前沿通常更适合增量修复；相同大小的弥散前沿对事件驱动修复更贵但对批量修复不然；完全重建需Θ(N)时间。

Conclusion: 修复前沿熵H(Ft)下界事件驱动修复触及的前沿单元数量，改变修复-重建交叉点经验值；该方法在动态最近邻维护中有效平衡了效率与准确性。
...
### [88] [Tangent Spheres and Integer Distances](https://arxiv.org/abs/2606.18569)
*David Eppstein*

Main category: cs.CG

TL;DR: The Erdős-Anning theorem states that any infinite set of points with integer distances in Euclidean space must be collinear. This paper extends the theorem to hyperbolic space of any dimension and provides quantitative bounds for both Euclidean and hyperbolic spaces. A key lemma about the external tangencies of sphere systems is also applied to multilateration problems.


<details>
  <summary>Details</summary>
Motivation: The original Erdős-Anning theorem only applies to Euclidean spaces. The researchers aimed to: (1) prove the same result holds in hyperbolic space, (2) provide quantitative bounds for point sets with integer distances in higher dimensions, and (3) develop a lemma that can be applied to multilateration problems.

Method: The researchers formulated a lemma concerning the graph of external tangencies of a system of spheres in Euclidean or hyperbolic space. They proved that if this graph contains a K_{{a,b}} subgraph (complete bipartite graph with a,b ≥ 3), then the sets of spheres on each side have centers lying on a hyperplane. This geometric lemma was then used to establish the integer distance results and multilateration bounds.

Result: The main results include: (1) proving the Erdős-Anning theorem in hyperbolic space of any dimension, (2) establishing a quantitative bound: if a set of points with integer distances in E^D or H^D has a subset of D+1 points in general position with diameter d, then the whole set has size O(D(d+1)^D), and (3) showing that in multilateration, D+1 non-coplanar landmarks always suffice to limit the position to two possibilities.

Conclusion: This work extends the classical Erdős-Anning theorem from Euclidean to hyperbolic spaces, provides the first quantitative bounds for higher-dimensional Euclidean spaces, and demonstrates the practical applicability of the sphere tangency lemma to multilateration problems. The theorem ensures that infinite integer-distance sets remain collinear even in hyperbolic geometry.
...
### [89] [A Neural Network Framework for Geodesic-Like Curve Computation on Parametric Surfaces](https://arxiv.org/abs/2606.18759)
*Sheng-Gwo Chen,Chen-Chang Peng*

Main category: cs.CG

TL;DR: 提出了一种基于深度学习和物理信息神经网络（PINN）的计算参数曲面上测地线类曲线的高效方法，可处理单参数曲面和多曲面系统。


<details>
  <summary>Details</summary>
Motivation: 2010年陈提出了测地线类曲线的概念用于估计参数曲面上的最短路径，但缺乏有效的数值计算框架。

Method: 利用深度学习和物理信息神经网络（PINN）框架计算测地线类曲线。

Result: 能够高效处理单参数曲面以及具有C^0或更高连续性的多曲面系统和旋转曲面。

Conclusion: 为各类参数曲面上的测地线类曲线计算提供了一种优雅高效的解决方案。
...
### [90] [Characterizing Opinion Evolution of Networked LLMs](https://arxiv.org/abs/2606.18276)
*Caleb Probine,Yigit Ege Bayiz,Filippos Fotiadis,Samuel Li,Yunhao Yang,Ufuk Topcu*

Main category: cs.MA

TL;DR: 研究大型语言模型(LLM)在多智能体系统中交互时的意见动态行为，检验经典舆论动态模型是否能准确预测LLM网络中的观点演变。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在多智能体系统中的应用日益增多(如人类话语模拟、舆论操作和LLM驱动的社交平台)，理解这些交互如何形成集体信念变得至关重要，但目前对LLM的新兴舆论传播机制理解不足。

Method: 将经典舆论动态模型应用于LLM网络，通过实验测试朴素平均模型的预测能力，并探索加入修正因素后的建模效果。

Result: 朴素平均模型无法准确追踪LLM的意见动态，但简单修改可显著提升保真度。偏差(bias)作为智能体内在回归倾向被发现是关键驱动因素，包含偏差后累计平均观点误差降低高达88%，该结论可跨模型家族、讨论主题和网络泛化。

Conclusion: 经典舆论动态模型经过适当修正可应用于LLM网络，偏差是影响LLM意见动态的核心因素，这一发现对理解和预测多智能体LLM系统中的观点演变具有重要意义。
...
### [91] [PersonalPlan: Planning Multi-Agent Systems for Personalized Programming Learning](https://arxiv.org/abs/2606.18633)
*Zhiyuan Wen,Jiannong Cao,Peng Gao,Haochen Shi,Wengpan Kuan,Bo Yuan,Xiuxiu Qi*

Main category: cs.MA

TL;DR: 本文提出MAP-PPL数据集和PersonalPlan双阶段多智能体规划器，用于个性化编程学习。通过分层SFT和奖励自适应GRPO实现具有教学支架的个性化计划生成。


<details>
  <summary>Details</summary>
Motivation: 现有LLM多智能体系统缺乏学习者画像理解和教学支架能力，难以实现真正的个性化编程教育。现有规划器无法根据学习者背景调整教学内容和方法。

Method: 提出两阶段MAS规划器：第一阶段使用分层SFT配合独立LoRA适配器进行画像感知任务分解和步骤依赖规划；第二阶段应用奖励自适应GRPO鼓励模型生成可执行、个性化且具有教学支架的计划。

Result: 在MAP-PPL数据集上，PersonalPlan仅用8B和32B参数模型即可达到最先进的计划可执行性、个性化程度和教学支架质量，有效协调多智能体系统实现智能体-学习者交互。

Conclusion: 通过结合画像条件化和教学支架设计，PersonalPlan成功实现了LLM多智能体系统在个性化编程教育中的应用突破。
...
### [92] [Gender Bias in LLM Hiring Decisions: Evidence from a Japanese Context and Evaluation of Mitigation Strategies](https://arxiv.org/abs/2606.18649)
...
TL;DR: 该研究检验了LLM在日本企业招聘环境中的性别偏见，发现所有5个模型都存在显著的女性偏好偏见，但现有的两种缓解策略（提示词指令和隐私过滤器）均无法有效消除这种偏见，且隐私过滤器与GPT-4o的内容安全过滤器存在不兼容问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注英语语境和西方格式简历的性别偏见，缺乏对非西方招聘情境的研究。日本作为重要的非西方经济体，其招聘文档格式（力石书）与西方简历有显著差异，因此有必要验证西方研究发现是否适用于日本企业语境，并探索实际可行的缓解策略。

Method: 采用反事实简历设计，使用60份日文力石书格式简历，12对基于语言学性别信号标准选取的姓名配对，对5个主流LLM（Claude Sonnet 4.6、GPT-4o、DeepSeek-V3、 Gemini 2.5 Flash、Llama 3.3 70B）进行43,200次API调用，涵盖基线、提示词指令和隐私过滤器三种条件，采用交叉随机效应线性混合模型进行分析。

Result: 所有5个模型均确认存在显著的女性偏好偏见，成功复制了西方研究在非西方情境中的发现。提示词层面的性别中立指令未能产生有意义的偏见减少。姓名依赖分析正式确定候选人姓名是主要性别通道，移除姓名后女性效应几乎完全消失。隐私过滤器与GPT-4o内容安全过滤器存在意外不兼容，导致42%的拒绝率。

Conclusion: LLM招聘决策中的女性偏好偏见延伸至日本企业情境。当前缓解策略无效：提示词指令无法消除偏见，而隐私过滤器存在实际部署挑战。研究者建议未来应关注匿名化技术的兼容性以及更有效的去偏方法。
...
### [93] [EARS: Explanatory Abstention for Reliable Sub-Agent Modeling in Large-scale Multi-Agent Systems](https://arxiv.org/abs/2606.18668)
*Shuang Xie,Yunan Lu,Han Li,Lingyun Wang*

Main category: cs.MA

TL;DR: EARS是一个生产级框架，通过将子代理的拒绝行为重新定义为代理间通信协议，使用LLM-as-a-Judge模型生成的标注数据微调子代理检测失败条件并返回解释，从而提升大型多代理系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: 在集中式多代理系统中，子代理常因能力限制而无法正确校准响应，导致对模糊、不支持或错误路由的请求过度回答，产生幻觉输出而非可操作的反馈。

Method: 使用校准的LLM-as-a-Judge模型集合生成人类-代理交互数据，构建子代理失败模式的结构化拒绝标签和理由 taxonomy，然后微调子代理检测失败条件并向协调器返回解释。

Result: 在大型电商业务智能助手中评估，EARS将整体响应通过率从68.5%提升至78.9%。

Conclusion: 子代理端可解释的拒绝行为能显著提升多代理系统的可靠性。
...
### [94] [Skill-MAS: Evolving Meta-Skill for Automatic Multi-Agent Systems](https://arxiv.org/abs/2606.18837)
*Hehai Lin,Qi Yang,Chengwei Qin*

Main category: cs.MA

TL;DR: Skill-MAS提出了一种新的第三路径来构建基于LLM的多智能体系统，通过将高级编排能力概念化为可进化的"元技能"，在保留经验和保持模型能力之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临两难困境：推理时MAS利用冻结的前沿LLM但无法从过去经验中学习；训练时MAS通过梯度更新内化经验但受限于较小模型的能力上限，且难以扩展到大型LLM。

Method: Skill-MAS通过闭合优化循环 refining 架构知识：(1)多轨迹 rollout 在当前元技能下为每个任务采样行为分布；(2)选择性反思自适应地选择优先任务并应用层次对比分析将系统经验提炼为可泛化的策略级原则。

Result: 在四个复杂基准和四个不同LLM上的广泛实验表明，Skill-MAS不仅实现了显著的性能提升，而且保持了良好的成本-性能权衡。进一步分析显示，进化的元技能具有高度鲁棒性，并在未见过的任务和不同LLM之间表现出强大的可迁移性。

Conclusion: Skill-MAS代表了一种新的LLM-MAS构建范式，成功地在经验保留和模型能力之间取得了平衡，为复杂任务处理提供了一种有前景的解决方案。
...
### [95] [A Technical Taxonomy of LLM Agent Communication Protocols](https://arxiv.org/abs/2606.19135)
*Linus Sander,Habtom Kahsay Gidey,Alexander Lenz,Alois Knoll*

Main category: cs.MA

TL;DR: 本研�构建了一个技术分类体系来对LLM智能体通信协议进行分类和分析，涵盖五个维度：对等方、有效载荷、交互状态、发现机制和模式灵活性。该分类体系通过分析9个开源协议揭示了架构模式并预测了发展趋势。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的发展和多智能体系统克服单一智能体局限的需求增加，强大的通信协议成为分布式智能体网络的重要基础设施。然而，协议景观的碎片化带来了显著的互操作性挑战，需要系统性的分类框架来指导协议选择和研究。

Method: 采用established迭代方法，定义了分类体系的目的、元特征和结束条件，进行了五轮迭代（三轮经验到概念，两轮概念到经验），分析了9个具有显著采用量的活跃维护的开源协议。

Result: 分类体系包含五个维度：counterparty、payload、interaction state、discovery mechanism和schema flexibility。分类结果显示：所有采样的智能体间协议结合混合payload与session-state持久化；大多数协议支持多个预定义模式；两个协议在运行时协商模式，表明向模式灵活性发展的趋势；去中心化发现仍然罕见。分析预测短期收敛压力将推动协议统一智能体到智能体和智能体到上下文（工具和数据）通信。

Conclusion: 长期而言，没有单一协议可能同时最大化多功能性、效率和可移植性，该领域更可能演化为 federated layered protocol stack。该框架指导协议选择并突出了隐私和政策执法等开放研究空白。
...
### [96] [Data Intelligence Agents: Interpreting, Modeling, and Querying Enterprise Data via Autonomous Coding Agents](https://arxiv.org/abs/2606.19319)
*Anoushka Vyas,Aarushi Dhanuka,Sina Khoshfetrat Pakazad,Henrik Ohlsson*

Main category: cs.MA

TL;DR: Data Intelligence Agents (DIA)是一个企业数据智能系统，包含三个代理（Data Interpreter、Schema Creator和Query Generator），通过自主编码 agents (ACAs) 作为核心抽象，将数据生产流程中的重复、有损耗的交接压缩简化。该系统已投入企业生产使用，重点研究了Query Generator，并在七个SQL基准测试上取得最先进结果。


<details>
  <summary>Details</summary>
Motivation: 企业数据整合工作面临瓶颈：数据所有者、工程师和分析师之间需要反复交接数据，这一过程存在信息损耗且效率低下。生产数据整合被限制在重复的、有损耗的交接流程中，各方需要协作发现、构建和查询企业数据。

Method: DIA系统采用三个专用代理架构：Data Interpreter负责数据发现和解释，Schema Creator负责模式构建，Query Generator负责SQL查询生成。这些代理不是输出文本，而是生成、执行、验证和修复具体的代码制品，并从共享内存中获取经验进行复用，同时为领域专家提供审查界面。

Result: Query Generator在七个SQL基准测试（涵盖四个任务类别和四种SQL方言）上进行评估，在完全自主模式下运行，结果匹配或超越所有已发布的最佳结果。

Conclusion: 基于执行、构建在ACAs和共享内存之上的架构，能够适应自然语言指令的调整，泛化到整个数据智能工作负载。这是一个有前景的企业数据智能解决方案。
...
### [97] [NAVI-Orbital: First In-Orbit Demonstration of a Zero-Shot Vision-Language Model for Autonomous Earth Observation](https://arxiv.org/abs/2606.18271)
*Juan Manuel Delfa Victoria,Taran Cyriac John,Andrew W. Herson*

Main category: cs.AI

TL;DR: NAVI-Orbital是首个在低轨卫星上实现完全在轨运行的多模态视觉-语言模型(Gemma 3)自主推理系统，通过LangGraph图状态机协调实现场景分类、自然语言对话和语义压缩，减少对下行带宽的依赖。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据生成速度超过下行带宽和人工处理能力，导致星上采集与地面情报之间出现越来越大的差距，传统"采集-下传-处理"模式面临带宽瓶颈。

Method: 使用Gemma 3视觉-语言模型进行场景分类和文本描述生成，通过LangGraph图状态机协调检测和对话代理，支持纯英文提示重任务调度，采用硬件加速GPU推理，无需针对飞行仪器进行微调。

Result: 在AID基准测试集(7960张图像)上达到88.16%准确率，完成Flatsat验证和实时在轨新采集地球图像(包括未校正的YAM-9图像)处理，证明了在卫星级边缘计算机上运行基础模型的可行性。

Conclusion: 通过在轨语义压缩地球观测实现了传统"采集-下传一切"带宽配置的反转，验证了卫星上运行基础模型进行自主多模态推理的可行性，为未来天基智能处理开辟了新途径。
...
### [98] [CaVe-VLM-CoT: An Interpretable Vision-Language Model Framework](https://arxiv.org/abs/2606.18385)
*Sneha Rao,Shaina Raza,Dhanesh Ramachandram*

Main category: cs.AI

TL;DR: CaVe-VLM-CoT是一种基于反思的模块化代理RAG框架，通过五阶段闭环管道解决视觉语言模型的幻觉问题


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLM)容易产生幻觉，生成流畅但视觉上不忠实的内容。现有方法无法强制执行逐步引用的 grounded 验证，也无法将验证失败反馈回检索进行修正

Method: 提出五阶段闭环管道:Extractor(提取器)、Retriever(检索器)、Solver(求解器)、Citation Injector(引用注入器)和Verifier(验证器)。检测到无grounded声明时触发结构化反馈给Extractor进行重新检索。提出23个组件指标和综合指标CaVeScore

Result: 在ScienceQA上达到87.1%准确率和56.6% CaVeScore；在MMMU(30科目)上达到55.2%准确率和35.7% CaVeScore

Conclusion: 无需任何架构或提示修改，CaVe-VLM-CoT通过证据grounded推理和闭环反馈机制有效减少VLM幻觉
...
### [99] [Searching for Synergy in Shared Workspace Human-AI Collaboration](https://arxiv.org/abs/2606.18413)
*Nachiket Kotalwar,Rohini Das,Carolyn Rose*

Main category: cs.AI

TL;DR: 研究共享工作空间中人类-AI团队的协作问题，探讨何时添加人类协作方能提升性能，以及协调结构如何影响团队表现。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理能力增强，许多科学和专业任务仍需要人类判断和情境专业知识，需要研究人类-AI团队如何有效协调职责。

Method: 使用Collaborative Gym环境和DiscoveryBench任务，分析1482个会话，测试添加模拟人类协作方的效果，并评估结合共享群体记忆和模拟人-在-环(HITL)门的脚手架方案。

Result: 当团队缺乏协调结构时添加相关协作方会降低性能；这种脚手架方案提高了平均性能，在三人团队中效果最明显，并产生更清晰的责任信号和更强的专业知识路由。

Conclusion: 人类-AI团队如何协调和整合专业知识与其所具备的能力同样重要。
...
### [100] [CEO-Bench: Can Agents Play the Long Game?](https://arxiv.org/abs/2606.18543)
*Haozhe Chen,Karthik Narasimhan,Zhuang Liu*

Main category: cs.AI

TL;DR: CEO-Bench是一个评估语言模型在模拟商业环境中运营公司500天的能力的基准测试，评估代理在长期不确定性、嘈杂信息获取、适应变化和协调多部分任务方面的技能。


<details>
  <summary>Details</summary>
Motivation: 现实世界的挑战需要多种复杂技能的结合：穿越长期不确定性、在嘈杂环境中获取信息、适应变化的世界、协调多个移动部分。但这些能力在现有代理中基本未被测试。

Method: 通过Python接口模拟运营创业公司500天，代理管理定价、营销、预算等公司运营的各个方面，面对与人类CEO相同的环境和挑战。

Result: 最强代理能编写复杂代码模拟客户队列预测未来现金流，挖掘谈判历史发现隐藏客户偏好。但大多数先进模型在此环境中表现不佳，仅Claude Opus 4.8和GPT-5.5超过100万美元起始余额，无一能持续盈利。

Conclusion: CEO-Bench为评估AI在推动长期、适应性进展所需的智能方面迈出了第一步。
...
TL;DR: 提出了Decoupled Search Grounding (DSG)框架，将实时搜索 grounding从推理模型中解耦出来，作为可优化的接口边界，而非固定的模型功能。


<details>
  <summary>Details</summary>
Motivation: 原生搜索在时间敏感的 FreshQA 上领先，但 DSG 在需要控制时展现出更强优势：在 SimpleQA 上几乎匹配原生准确率（86.1% vs 87.7%），搜索成本降低 91%，保持简洁答案契约，暖缓存命中率达 99.4%，延迟降低 68%。在电商查询理解工作负载上，DSG 匹配或略超原生搜索准确率，搜索成本降低超过 98%。

Method: 通过MCP兼容的网关实现解耦搜索 grounding，暴露提供商路由、源感知上下文渲染、配置回退、检索深度控制以及精确和语义缓存作为一等控制。在 SimpleQA、FreshQA 和 HotpotQA 三个数据集上评估了五个前沿模型。

Result: DSG 在 SimpleQA 上几乎匹配原生准确率（86.1% vs 87.7%），搜索成本降低 91%，保持简洁答案契约，暖缓存命中率达 99.4%，延迟降低 68%。在电商查询理解工作负载上匹配或略超原生搜索准确率，搜索成本降低超过 98%。

Conclusion: 实时 grounding 应被视为可优化的接口边界，而非固定的模型功能。DSG 作为大规模 Agent 工作负载的共享生产 grounding 层，实现了模型的互换性。
...
### [102] [DeFAb: A Verifiable Benchmark for Defeasible Abduction in Foundation Models](https://arxiv.org/abs/2606.18557)
*Patrick Cooper,Alvaro Velasquez*

Main category: cs.AI

TL;DR: 本文提出DeFAb（可废性推理基准），一个将四十年公共知识库转化为可废性推理评测任务的数据集和生成管道，用于评估语言模型的理论推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评测缺乏对可废性推理（defeasible reasoning）的系统评估。可废性推理是指在保留无关期望的同时，通过覆盖默认规则来解释异常假设构建，这是理论创造力和严格逻辑推理的核心。

Method: 1) 结合分类层级(OpenCyc/YAGO/Wikidata)与行为属性图(ConceptNet/UMLS)生成372,648+实例 
2) 三个难度级别，均有 polynomial-time 可验证的金标准 
3) 规则-based 逻辑求解器作为完美baseline
4) 渲染鲁棒性评估测试模型泛化能力

Result: 1) 规则求解器：100%准确率，<50微秒 
2) 最佳前沿模型：Level 1达65%，渲染鲁棒评估仅23.5% 
3) Level 2渲染鲁棒准确率：7.8-23.5% 
4) 思维链方差(~36pp)超过模型间差距 
5) 污染控制隔离出+19.4pp Level 3差距 
6) DeFAb-Hard：最佳模型53.3% vs 100%符号求解器

Conclusion: 当前前沿语言模型无法可靠内化可废性推理能力，渲染鲁棒评估显示显著性能下降。DeFAb提供了严格的逻辑推理评测基准，可用于偏好优化(DPO/RLVR)的精确奖励信号。
...
### [103] [Optimizing Lithium Production Decisions under Geological, Demand, and Pricing Uncertainties: A POMDP Framework for Multi-Objective Decision Making](https://arxiv.org/abs/2606.18598)
*Anna C. Edmonds,Mansur M. Arief,Robert J. Moss,Mykel J. Kochenderfer,Jef Caers*

Main category: cs.AI

TL;DR: 研究锂矿开采决策优化问题，将地质、价格和需求不确定性以及不同提取技术纳入决策框架。


<details>
  <summary>Details</summary>
Motivation: 传统锂矿开采决策模型未考虑价格不确定性、需求不确定性及不同采矿技术（如直接锂提取与硬岩开采），需要更 robust 的决策方法来适应市场波动。

Method: 将问题建模为部分可观测马尔可夫决策过程(POMDP)，采用信念状态规划方法求解，以处理价格和需求的不确定性。

Result: POMDP求解器通过动态适应不同的价格机制（静态、线性、指数和随机）优于人类启发式方法，在所有价格和矿床场景下实现了更高的需求满足和更平衡的经济环境结果。

Conclusion: 该框架能够优化勘探、生产和技术选择的排序，为锂矿投资和生产策略提供更优的决策支持。
...
### [104] [ForecastBench-Sim: A Simulated-World Forecasting Benchmark](https://arxiv.org/abs/2606.18686)
*Jaeho Lee,Nick Merrill,Ezra Karger*

Main category: cs.AI

TL;DR: ForecastBench-Sim是一个基于Freeciv策略游戏rollout的模拟世界预测基准测试，旨在解决真实世界预测基准的局限性（如结果解决缓慢、尾事件罕见、反事实问题难以评分）。通过提供游戏状态的固定报告，让预测者回答关于隐藏未来状态的问题，然后继续模拟并评分。该基准可生成任意时间范围的连续/二元预测问题、成对干预世界用于因果问题，以及罕见或破坏性结果的resolved示例。


<details>
  <summary>Details</summary>
Motivation: 真实世界预测基准存在固有约束：结果解决缓慢、尾事件罕见、反事实问题难以评分。这些限制阻碍了对动态世界状态下概率推理的研究。作者旨在创建一个补充真实世界基准的模拟世界基准，提供受控的、可立即解决的任务。

Method: 利用Freeciv（类文明系列回合制策略游戏）构建模拟世界。预测者接收固定的世界报告（当前游戏状态的结构化快照），回答关于隐藏未来状态的问题。基准通过继续模拟来评分问题。可生成：1）任意时间范围的连续或二元预测问题；2）用于条件或因果问题的成对干预世界；3）罕见或破坏性结果的resolved示例。

Result: 描述了基准管道、问题家族、评分协议和发布 artifacts，并报告了模型评估和匿名人类 pilot 的验证切片。

Conclusion: ForecastBench-Sim 通过提供受控的、可立即解决的任务，为研究动态世界状态下的概率推理提供了有价值的补充工具，有望推动AI预测能力的发展。
...
### [105] [What Must Generalist Agents Remember?](https://arxiv.org/abs/2606.18746)
*Khurram Yamin,Namrata Deka,Maitreyi Swaroop,Albert Ting,Jeff Schneider,Bryan Wilder*

Main category: cs.AI

TL;DR: 本文提出了一个形式化理论,说明通用智能体必须在记忆中存储什么才能在多个环境和目标中近乎最优地行动。


<details>
  <summary>Details</summary>
Motivation: 通用智能体需要在不同环境中保持有效性,但当前观测往往不足以区分需要不同行动策略的环境,因此需要理解记忆在其中的作用。

Method: 通过分析两个领域共享观测瓶颈但需要不兼容最优行动的情况,证明任何近乎最优的策略必须在该瓶颈处诱导不同的记忆分布。

Result: 提出分离定理:足够成功的智能体不能仅依赖当前状态观测,必须在记忆中保留领域相关信息;并证明如果记忆包含足够信息估计相关目标的价值,则可近似重建局部转移动力学。

Conclusion: 记忆是支持领域消歧、转移模型重建和通用智能体规划的基础 substrate(基质)。
...
### [106] [R2D-RL: A RoboCup 2D Soccer Environment for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18786)
*Haobin Qin,Baofeng Zhang,Hidehisa Akiyama,Keisuke Fujii*

Main category: cs.AI

TL;DR: R2D-RL是一个连接RCSS2D机器人足球平台与Python多智能体强化学习接口的框架，支持全场景训练、混合动作空间和并行执行，并提供基准测试。


<details>
  <summary>Details</summary>
Motivation: Robot soccer是验证多智能体强化学习（MARL）的理想测试平台，但现有RCSS2D平台的服务器-客户端架构难以与现代Python工作流程集成，需要新的接口层。

Method: 通过共享内存通信和周期级同步实现RCSS2D/HELIOS客户端与Python MARL接口的连接，支持离散/混合动作空间、动作掩码、EPV奖励塑造和并行训练。

Result: 提供前锋场景和11对11全场景基准测试及基线结果。

Conclusion: R2D-RL填补了经典机器人足球平台与现代MARL研究之间的空白，为多智能体协作与对抗学习提供了标准化实验环境。
...
### [107] [ProfiLLM: Utility-Aligned Agentic User Profiling for Industrial Ride-Hailing Dispatch](https://arxiv.org/abs/2606.18803)
*Tengfei Lyu,Zirui Yuan,Xu Liu,Kai Wan,Zihao Lu,Li Ma,Hao Liu*

Main category: cs.AI

TL;DR: ProfiLLM是一个将大语言模型应用于工业网约车调度系统的用户画像数据管道,通过工具增强的全局知识挖掘和效用对齐的画像探索两个模块,解决百万级订单日志超出LLM上下文窗口、长尾用户交互数据不足、表面流畅画像不改善下游预测等挑战。


<details>
  <summary>Details</summary>
Motivation: 工业网约车调度系统仍主要使用结构化数值特征,但决定性的行为信号(如司机对某些区域的惯常厌恶)是内在上下文性的,自然可以用LLM生成的用户画像表达。将LLM规模化到毫秒级延迟的实时调度系统面临三个约束:平台日订单数百万级,日志量超出任何LLM上下文窗口;大多数用户是长尾的,交互数据不足以进行用户级画像;表面流畅的画像不一定改善下游预测效用。

Method: 提出ProfiLLM系统,包含两个模块:(1)工具增强的全局知识挖掘:为LLM智能体配备27个分析工具来挖掘平台级数据,生成可复用的全局知识、自适应用户聚类规则和区域级供需先验。(2)效用对齐的画像探索:为每个聚类生成多个候选画像,通过轻量级下游效用代理进行评估,迭代优化最佳候选者并构建偏好对用于DPO微调。

Result: 在滴滴生产调度系统上部署,ProfiLLM在结果预测中实现最高+6.14%相对AUC提升,在调度模拟中实现最高+4.35%GMV增益,在14天在线A/B测试中实现持续改进,包括+0.47%GMV、+0.33%完单率和-0.82%接受前取消率。

Conclusion: ProfiLLM成功将LLM应用于工业级网约车调度系统,通过工具增强的知识挖掘和效用对齐的画像探索,有效解决了规模化挑战,在实际生产环境中取得了显著的效果提升。
...
### [108] [WorldLines: Benchmarking and Modeling Long-Horizon Stateful Embodied Agents](https://arxiv.org/abs/2606.18847)
*Yehang Zhang,Jianchong Su,Haojian Huang,Yifan Chang,Tianhao Zhou,Xinli Xu,Yingjie Xu,Yinchuan Li,Zexi Li,Ying-Cong Chen*

Main category: cs.AI

TL;DR: 本文提出WorldLines基准测试，用于评估长期记忆在具身 household assistance 中的应用，同时提出ObsMem框架来处理visibility-aware memories和action-native state trails。实验揭示了部分可观测性、世界状态覆盖和将长期记忆转化为具身计划的持久挑战。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准测试主要评估语言中心的检索和问答，而具身基准测试通常关注短期任务执行，未能测试动态环境中的长期记忆使用。需要一个项目驱动的基准测试来评估长期具身家庭助手的记忆能力。

Method: 构建包含对话、动作、执行反馈、对象和设备状态变化的临时扩展 household traces，并将其转换为用于Memory QA和Embodied Task Planning的evidence-linked样本。提出ObsMem框架，维持visibility-aware memories和action-native state trails以实现state-aware决策。

Result: 实验揭示了部分可观测性、被覆盖的世界状态以及将长期记忆转化为具身计划的持续挑战，同时ObsMem为此设置提供了更强的参考架构。

Conclusion: WorldLines填补了长期具身记忆评估的空白，ObsMem框架能有效处理visibility-aware和state-aware的记忆问题，为动态家庭环境中的长期助手任务提供了更优的解决方案。
...
### [109] [Externalizing Research Synthesis and Validation in AI Scientists through a Research Harness](https://arxiv.org/abs/2606.18874)
...
TL;DR: Xcientist是一个将AI科学研究过程外部化的研究框架，将文献证据、想法状态、实现计划、消融记录和修复痕迹保存为可追溯的研究工件，解决自动科学研究中"声称漂移"的失败模式。


<details>
  <summary>Details</summary>
Motivation: AI系统虽然能自动化科学工作流程，但将先前证据、生成想法、实验和最终声称联系起来的推理过程往往隐含在模型推理内部，缺乏透明度和可追溯性。

Method: Xcientist将研究综合和实验验证组织为可检查的、契约管理的流程，将文献证据、想法状态、实现计划、消融记录和修复痕迹保存为持久的研究工件，使生成的机制能够有据可依地执行、测试和修订。

Result: 在训练无关的记忆系统、图结构交通预测和多尺度物理信息神经网络三个场景中验证，Xcientist能够保留从问题表述到机制设计、验证和有限修订的可追溯轨迹。

Conclusion: AI科学家不仅应通过最终工件评估，还应通过其综合和验证过程是否可归因、可检查和科学负责来评估。
...
### [110] [Generative-Model Predictive Planning for Navigation in Partially Observable Environments](https://arxiv.org/abs/2606.18888)
*Thomas Quilter,Yifan Zhu,Guorui Quan,Mingfei Sun,Samuel Kaski*

Main category: cs.AI

TL;DR: 提出BeliefDiffusion框架，结合扩散模型和模型预测控制，在部分可观察环境中实现更 robust的导航。


<details>
  <summary>Details</summary>
Motivation: 解决现有信念基方法在高维情况下面临的多模态问题，以及生成模型需要大量数据的问题。

Method: 结合扩散模型表征多模态信念分布与MPC前向规划的两步框架。

Result: 在合成地图环境中显著优于无模型强化学习和其他生成方法。

Conclusion: 将多模态信念表示融入规划可实现更 robust的部分可观察环境导航。
...
### [111] [Skill-Guided Continuation Distillation for GUI Agents](https://arxiv.org/abs/2606.18890)
...
TL;DR: 本文提出SGCD框架，通过迭代自我改进方式解决GUI智能体在离线训练后在线执行时遇到的策略诱导的离轨状态（off-trajectory states）缺乏监督信号的问题。核心思想是先让策略自主探索到离轨状态，再由skill-guided策略从这些状态完成task并生成成功的continuation，最后将这些continuation与专家轨迹混合训练。


<details>
  <summary>Details</summary>
Motivation: GUI智能体通过行为克隆训练时，当策略在实际执行中偏离专家轨迹后会遇到从未见过的状态（离轨状态），这些状态没有专家演示提供监督，导致策略无法选择正确动作，形成"监督缺口"。这一问题的根源在于离线训练与在线执行间的分布偏移。

Method: 提出Skill-Guided Continuation Distillation (SGCD)迭代框架：1）让基础策略无skill guidance运行若干步到达离轨状态；2）从这些状态使用skill-guided策略完成任务并生成成功的continuation；3）将这些continuation与专家轨迹混合用于训练。提取的skills包括：Continuation Plans、Critical Targets、Failure Traps和Success Criteria。

Result: 在OSWorld-Verified基准上，SGCD将三个基础模型的成功率从30%多的低水平提升到超过50%，验证了方法的有效性和泛化性。

Conclusion: SGCD通过弥合专家轨迹覆盖不足的问题，成功提升了GUI智能体在真实场景中的表现。该方法具有较好的通用性，可适用于不同的基础模型。
...
### [112] [SciRisk-Bench: A Risk-Dimension-Aware Benchmark for AI4Science Safety](https://arxiv.org/abs/2606.18936)
...
TL;DR: SciRisk-Bench是一个评估AI4Science安全性的基准测试,覆盖7个学科、31个亚学科和10个风险维度,旨在诊断科学模型在何种情况下仍存在不安全因素。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在AI4Science工作流程中的深入应用(从科学问答到实验室规划和自主发现),现有AI4Science安全数据集的风险维度不够明确,缺乏针对高风险科学场景的安全评估标准。

Method: 从两个互补角度构建基准:(1)显式风险维度 (2)科学学科。覆盖7个学科、31个亚学科和10个风险维度,对主流LLM和科学导向LLM进行评估。

Result: 实现了对科学模型在风险维度、学科和亚学科层面的细粒度诊断,揭示了当前科学模型在哪些方面仍存在安全隐患。

Conclusion: SciRisk-Bench为AI4Science领域提供了标准化的安全评估框架,可识别科学模型在不同学科和风险维度上的安全薄弱环节。
...
### [113] [RTSGameBench: An RTS Benchmark for Strategic Reasoning by Vision-Language Models](https://arxiv.org/abs/2606.18950)
*San Kim,Daechul Ahn,Reokyoung Kim,Hyeonbeom Choi,Seungyeon Jwa,Jonghyun Choi*

Main category: cs.AI

TL;DR: RTSGameBench是一个基于Beyond All Reason游戏的大规模RTS基准测试，用于诊断视觉语言模型(VLM)在战略推理方面的局限性，特别关注多智能体协调、联盟协调和长期规划能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在竞争性和合作性设置中难以进行战略推理（即预测和影响其他智能体的行动），而现有RTS基准测试评估范围有限、缺乏系统性能力诊断、覆盖场景固定，无法有效诊断VLM的战略推理缺陷。

Method: 提出RTSGameBench基准测试，包含：(1)多样化的游戏对局，涵盖各种对抗结构；(2)诊断评估迷你游戏，针对个体战略能力；(3)可扩展的生成框架，将自由形式查询转换为新迷你游戏。同时提供RTSGameAgent，通过FSM和智能体记忆管理单位。

Result: 实证验证表明，多个SOTA VLM在需要更紧密协调、多智能体协调和任务规模增加的场景下表现不佳。

Conclusion: RTSGameBench能够有效诊断VLM在战略推理方面的局限性，为未来提升VLM的战略推理能力提供了有价值的测试平台和评估框架。
...
### [114] [ThinkDeception: A Progressive Reinforcement Learning Framework for Interpretable Multimodal Deception Detection](https://arxiv.org/abs/2606.18988)
*Jinhao Song,Shan Liang,Yiqun Yue,Zhuhuayang Zhang,Tianqi Gao*

Main category: cs.AI

TL;DR: ThinkDeception是一个开创性的可解释多模态欺骗检测框架，首次将多模态大语言模型(MLLMs)引入该领域，将欺骗检测从传统二分类任务转变为显式认知推理过程。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖端到端黑盒模型，严重缺乏可解释性，无法提供透明的推理轨迹，难以明确捕捉欺骗行为中的跨模态细微不一致性。

Method: 1)构建了首个精细标注的多模态思维链(CoT)数据集；2)开发了ThinkDeception Base基础模型；3)提出VAC-GRPO(视觉-音频一致性组相对策略优化)，采用四层渐进式难度训练策略，结合动态课程调度器和多维度过程感知奖励机制。

Result: ThinkDeception在主流基准测试上建立新的SOTA，显著优于现有方法的检测准确性和推理质量。

Conclusion: 本工作成功推动欺骗检测领域向可解释的多模态认知推理方向发展，验证了模态不一致性在解码欺骗中的关键作用。
...
### [115] [RODS: Reward-Driven Online Data Synthesis for Multi-Turn Tool-Use Agents](https://arxiv.org/abs/2606.19047)
*Ruishan Fang,Siyuan Lu,Chenyi Zhuang,Tao Lin*

Main category: cs.AI

TL;DR: RODS是一种解决多轮工具使用强化学习中静态数据集信息样本耗尽问题的方法，通过利用奖励方差作为边界检测器，动态合成新样本并管理动态回放缓冲区。


<details>
  <summary>Details</summary>
Motivation: 在多轮工具使用RL中，静态数据集中的信息样本会随着训练快速耗尽。GRPO的梯度信号集中在具有最高rollout奖励方差的任务上，这是因为Popoviciu上界。样本位于智能体能力边界附近时（成功和失败大致平衡）会贡献最大的策略梯度，但随着训练进行，这个边界不断移动，导致静态数据集中信息样本池逐渐枯竭。

Method: RODS通过以下步骤解决样本耗尽问题：1)利用进度奖励方差作为零成本的边界检测器（无需额外推理）；2)持续识别边界样本；3)通过技能对齐的重采样管道合成匹配其结构复杂度的新多轮变体；4)管理与策略共同演化的动态回放缓冲区。

Result: 从400个人类种子开始，维持约800个样本的活跃训练池，RODS实现了与17K样本离线pipeline相当的性能，同时需要约20x更少的轨迹，并在受控设置中优于固定数据RL和环境增强。

Conclusion: RODS有效地解决了多轮工具使用RL中的样本耗尽问题，通过闭环数据生成和动态缓冲区管理，在显著减少轨迹需求的情况下达到了与大规模离线训练相当的性能。
...
### [116] [ARIADNE: Agnostic Routing for Inference-time Adapter DyNamic sElection](https://arxiv.org/abs/2606.19079)
*Enrico Cassano,Michał Brzozowski,Zuzanna Dubanowska,Paolo Mandica,Neo Christopher Chung*

Main category: cs.AI

TL;DR: ARIADNE是一个无训练、适配器无关的路由框架,用于在推理时从多个适配器中自动选择最合适的目标。


<details>
  <summary>Details</summary>
Motivation: 随着PEFT的普及,单个 backbone 配备多个任务专用适配器,但推理时查询通常没有任务标签,现有路由方法依赖适配器内部参数或需要额外训练,缺乏可扩展性和可移植性。

Method: 通过计算适配器训练集嵌入的聚类中心来表示每个适配器,在潜在空间中测量输入与聚类中心的接近程度来选择适配器,完全在输入嵌入空间执行,兼容任意PEFT方法。

Result: 在Llama 3.2 1B Instruct上评估23个NLP任务恢复97.44%上界性能,扩展到44个任务达到89.7%平均选择准确率,无需额外训练或访问适配器内部。

Conclusion: ARIADNE提供了一种高效、可扩展的适配器选择方案,在不修改适配器或训练流程的情况下实现高选择准确率,具有重要的实际应用价值。
...
### [117] [Towards an Agent-First Web: Redesigning the Web for AI Agents](https://arxiv.org/abs/2606.19116)
...
TL;DR: 论文提出为AI代理重新设计互联网的三个层面方案：访问层引入代理识别和经济层采用意图分级以及内容层用ATML标记语言应对AI生成内容自我循环的挑战，最终构成十个设计原则创建代理优先互联网。


<details>
  <summary>Details</summary>
Motivation: 传统互联网假设内容消费者是人类，但AI代理作为中介崛起invalidates这一前提。现有Web通过屏蔽、CAPTCHA和经济模式抵制代理访问，需要从根本上重新设计。

Method: 提出三层面重新设计：访问层让代理继承人类访问权限并采用双层架构；经济层建立基于意图的分级框架和代币订阅模式；内容层用ATML和监督层级及加密溯源应对知识脱离人类真相。

Result: 形成十个设计原则，构建代理作为一等公民的"代理优先互联网"，在访问、经济和内容层面重新谈判Web基础社会契约。

Conclusion: 互联网需要根本性重构以承认AI代理的合法地位，通过跨访问、经济、内容三层的系统性设计原则实现人机共存的新型网络生态。
...
### [118] [Analysing drivers and interdependencies in European electricity markets using XAI](https://arxiv.org/abs/2606.19118)
*Antoine Pesenti,Aidan O'Sullivan*

Main category: cs.AI

TL;DR: 本研究结合深度神经网络与可解释人工智能技术，分析欧洲39个电力竞价区的电价决定因素，发现可再生能源、天然气价格和跨区域互联是影响电价的关键变量。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络虽能准确预测电价，但其黑箱特性限制了对其背后价格形成机制的理解，因此需要引入XAI技术来揭示电价驱动因素。

Method: 采用SHAP方法量化特征贡献，并扩展SSHAP聚合框架以提升高维环境下的可解释性，同时构建了合成欧盟统一电力市场来模拟完全一体化情景。

Result: 太阳能等可再生能源对价格的影响超出其发电份额；天然气价格是跨市场的共同主导因素；跨境互联显著塑造价格动态，体现欧洲电力系统的强相互依赖性。

Conclusion: 研究表明XAI与DNN的结合能有效揭示复杂电力市场的价格驱动机制，为政策制定者和市场参与者提供关键决策支持。
...
### [119] [Human-AI Coevolution Dynamics: A Formal Theory of Social Intelligence Emergence Through Long-Term Interaction](https://arxiv.org/abs/2606.19144)
*Jingyi Zhou,Senlin Luo,Haofan Chen*

Main category: cs.AI

TL;DR: 本文提出Human-AI Coevolution Dynamics Framework (HACD-H)，将人类-AI交互建模为自组织社会认知系统，整合情感适应、关系组织、社会记忆和人格一致性，构建了约14,700个交互回合的对话数据集并开发了理论驱动的评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有对话AI系统缺乏统一框架来解释长期人机交互中稳定社会关系和社会智力的涌现，现有方法仅通过孤立组件（如情绪建模、记忆检索或人格设定）建模社会行为。

Method: 提出HACD-H框架，整合情感适应、关系组织、社会记忆和人格一致性，引入多时间尺度社会认知、关系吸引子、信任 basin、发育相变和社会认知能量动力学等原则；构建约14,700个交互回合的对话数据集；开发理论驱动的实证评估框架。

Result: 揭示了社会认知的时间持久性层次、稳定的关系吸引子、类似发育相变的模式和社会认知能量景观结构；社会智力与社会认知能量呈显著负相关(r=-0.391, p<0.001)；交互轨迹随时间呈现渐进能量降低。

Conclusion: 社会智力涌现自长期社会认知共演化而非孤立的对话能力；HACD-H为建模适应性人机社会交互和开发社会智能AI系统提供了统一的理论基础。
...
### [120] [Beyond Safe Data: Pretraining-Stage Alignment with Regular Safety Reflection](https://arxiv.org/abs/2606.19168)
*Jinhan Li,Kexian Tang,Yihan Xu,Zhuorui Ye,Kaifeng Lyu*

Main category: cs.AI

TL;DR: 本文提出Safety Reflection Pretraining方法,在预训练阶段向语料库中插入安全反思文本,使模型在语言建模中整合自我监测能力。实验表明该方法提高了安全分类准确率,显著降低了推理阶段和微调攻击的成功率,并在合成环境MedSafetyWorld中展示了相比数据过滤和重写的优势。


<details>
  <summary>Details</summary>
Motivation: 当前预训练阶段的对齐工作主要关注使数据更安全,但作者指出LLM可能将看似无害的知识和能力组合成不安全行为。预训练阶段的对齐不应仅局限于使数据安全,而应塑造模型从安全数据中可能获得的行为。

Method: 在预训练语料库中定期插入短安全反思文本,将自我监测能力直接整合到语言建模中,建立基础能力后再通过后续训练强化。

Result: 在1.7B模型上的实验表明:安全分类准确率提升,推理阶段和微调攻击成功率大幅降低。MedSafetyWorld合成环境中的消融实验证明,该方法在防止模型从安全数据泛化不安全行为方面优于数据过滤和重写方法。

Conclusion: 预训练阶段的对齐不仅应使训练数据安全,还应塑造模型从安全数据中可能获得的行为。Safety Reflection Pretraining是实现更深层安全对齐的有效方法。
...
### [121] [User as Engram: Internalizing Per-User Memory as Local Parametric Edits](https://arxiv.org/abs/2606.19172)
*Bojie Li*

Main category: cs.AI

TL;DR: 提出User as Engram方法：将用户内容存储为Engram模型哈希表的外科编辑，将推理技能放入共享适配器，实现高效个性化记忆


<details>
  <summary>Details</summary>
Motivation: 当前个性化方法存在矛盾：大脑将内容和技能分离存储，但现有per-user LoRA将两者混合。LoRA写入用户事实会污染无关文本，且只能容纳单一用户

Method: 采用分层设计：用户内容作为Engram哈希表的精确编辑，推理技能放在共享适配器中。不同用户的事实落入不同哈希槽，可叠加共存

Result: 相比per-user LoRA：直接回忆性能相当，间接推理准确率平均提升5.6倍，从不使任何用户推理能力下降，记忆占用减少约33000倍

Conclusion: Engram设计实现了内容与技能的解耦，在保持高效检索的同时避免用户间污染，为大规模个性化提供了可行方案
...
### [122] [TxBench-PP: Analyzing AI Agent Performance on Small-Molecule Preclinical Pharmacology](https://arxiv.org/abs/2606.19245)
*Hannah Le,Ramesh Ramasamy,Alex Urrutia,Mahsa Yazdani,Tim Proctor,Kenny Workman*

Main category: cs.AI

TL;DR: TxBench-PP是一个用于评估AI agents在临床前药理学领域决策能力的新benchmark，包含100个评估任务，测试模型能否从真实实验数据中得出准确结论而非记忆文献知识。


<details>
  <summary>Details</summary>
Motivation: AI agents有望加速药物发现，但实际部署需要可信的评估；现有基准测试不足，无法验证AI在真实临床前药理学决策中的能力。

Method: 构建包含100个评估任务的benchmark，覆盖MoA、PD、化合物-靶点相互作用、因果靶点验证、成药性/安全性及转化疗效等维度；AI agents接收真实工作流程数据并在编码环境中进行分析，返回结构化答案进行确定性评分。

Result: 在16种模型配置（11个模型，4800条轨迹）中，没有系统能可靠地恢复临床前药理学决策；最佳配置Claude Opus 4.8/Pi通过率为59.3%（178/300），其次GPT-5.5/Pi为55.3%（166/300）。

Conclusion: 当前AI模型在临床前药理学决策任务上表现有限，最高通过率仅约60%，表明该领域AI辅助药物发现仍需重大改进。
...
### [123] [X+Slides: Benchmarking Audience-Conditioned Slide Generation](https://arxiv.org/abs/2606.19256)
*Haodong Chen,Xuanhe Zhou,Wei Zhou,Xinyue Shao,Yanbing Zhu,Bo Wang,Jiawei Hong,Anya Jia,Fan Wu*

Main category: cs.AI

TL;DR: X+Slides是一个针对受众条件化幻灯片生成的新基准，用于评估LLM从源文档生成幻灯片的质量，特别考虑受众差异（专家vs决策者）。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估幻灯片完整性和技术深度，忽略了目标受众这一关键现实因素。不受众群体需要不同类型的信息——专家需要严格证明，决策者需要可操作的结论。

Method: 基于113个主题和7个演示场景的多元语料库构建，使用8,133个去重、源引证的探针构建动态评估框架。通过为同一探针分配受众特定的效用权重，报告四个互补指标：受众覆盖率、领域覆盖率、效率和正确性。

Result: 在DeepPresenter、SlideTailor和NotebookLM上的实验表明，当前系统只能恢复部分受众-essential信息：在τ_A=0.7时，DeepPresenter达到0.714的最佳受众覆盖率，SlideTailor达0.594，NotebookLM消融达0.853，同时显示出明显的源引证差异。

Conclusion: 视觉质量和广泛主题覆盖不应被视为支持证据，必须进行源引证评估。当前系统能够恢复大量但仍不完整的受众-essential信息。
...
### [124] [NeSyCat Torch: A Differentiable Tensor Implementation of Categorical Semantics for Neurosymbolic Learning](https://arxiv.org/abs/2606.19279)
*Daniel Romero Schellhorn,Till Mossakowski,Björn Gehrke*

Main category: cs.AI

TL;DR: NeSyCat Torch是一个结合神经网络的神经符号系统，通过单子(monad)框架统一不同的神经符号语义，为神经学习谓词和函数提供计算解释，在MNIST加法任务上超越现有方法性能。


<details>
  <summary>Details</summary>
Motivation: 神经符号语义碎片化：经典、模糊、概率和神经系统的真值定义各自独立。NeSyCat虽统一了这些方法，但缺乏对神经网络学习谓词和函数的处理能力，需要补充神经网络的计算符号解释。

Method: 使用概率编程和张量后端实现NeSyCat Torch：分布单子用于参考语义和度量评估；惰性log-张量单子(基于log-半环)实现数值稳定、可微训练；批处理单子实现高效批量训练。公理以单子do-notation编写，单子bind执行边际化并惰性剪枝。

Result: 在MNIST加法任务上，HaskTorch、JAX和PyTorch实现在速度和准确率上优于LTN和DeepProbLog，接近DeepStochLog准确率，且保持统一框架适用于多种一阶神经符号方法。

Conclusion: 该框架通过单子参数化设计，可扩展至连续概率(如Giry单子)，为神经符号学习提供通用架构。
...
### [125] [Rethinking Reward Supervision: Rubric-Conditioned Self-Distillation](https://arxiv.org/abs/2606.19327)
*Siyi Gu,Jialin Chen,Sophia Zhou,Arman Cohan,Rex Ying*

Main category: cs.AI

TL;DR: 本文提出Rubric-Conditioned Self-Distillation（RCSD）框架，将评分标准（rubrics）作为结构化、细粒度的反馈信号，用于推理语言模型的自我蒸馏，解决传统监督蒸馏标注昂贵且可能包含错误，以及强化学习将反馈压缩为标量信号导致credit assignment不精确的问题。


<details>
  <summary>Details</summary>
Motivation: 现有推理语言模型的后训练方法存在两个关键问题：1）监督蒸馏依赖的思维链注释成本高且可能包含噪声或不完整信息；2）带验证奖励的强化学习将评估反馈压缩为标量信号，掩盖了响应中需要改进的具体方面。

Method: 提出Rubric-Conditioned Self-Distillation框架，将rubrics作为结构化细粒度反馈。方法核心包括：1）将教师模型置于criterion-level rubrics条件下；2）使用教师模型为学生自身的采样轨迹提供token-level指导。采用两阶段流程：第一阶段学习生成任务特定的rubrics，第二阶段训练rubric引导的推理器。

Result: 在多种科学推理基准测试集上的实验表明，rubric-conditioned self-distillation有效将rubric级别的标准转换为推理过程的token级别指导，平均性能超越GRPO 1.0分，超越OPSD 0.9分。

Conclusion: 该框架成功解决了细粒度credit assignment问题，通过rubrics指定强响应应满足的条件，实现了比标量奖励优化更精细的推理过程监督，为推理语言模型的后训练提供了新的有效范式。
