...
TL;DR: 提出了首个用于SAR船舶检测的基于Transformer的知识蒸馏框架SURGE，通过对比InfoNCE目标在共享投影嵌入空间中转移关系几何知识，实现从强大教师检测器到紧凑学生检测器的知识迁移。


<details>
  <summary>Details</summary>
Motivation: 深度卷积和Transformer检测器性能强但计算量大，难以实时或机载部署；轻量级模型难以捕捉SAR后向散射中的复杂结构关系；现有知识蒸馏方法依赖特征或logit匹配，忽视目标表示间的几何关系。

Method: 提出结构化统一关系知识蒸馏框架(SURGE)，使用对比InfoNCE目标在共享投影嵌入空间中转移关系几何知识。框架架构无关，可为两阶段、单阶段和Transformer检测器提供通用区域级蒸馏接口。

Result: 在SSDD和HRSID基准上，两阶段检测器获得最高6.2 mAP和8.0 AP75增益，超过基线学生模型甚至超越教师性能。

Conclusion: SURGE是首个Transformer-based SAR船舶检测器知识蒸馏框架，通过转移关系几何知识而非局部激活相似性，有效提升了轻量级模型的检测性能，为SAR船舶检测的实时/机载应用提供了可行方案。
...
TL;DR: SANA-Streaming是一个用于消费级GPU上高分辨率实时流媒体视频编辑的系统算法协同设计框架，通过混合扩散Transformer架构、循环逆转正则化和高效系统协同设计实现。


<details>
  <summary>Details</summary>
Motivation: 实时流媒体视频到视频编辑(V2V)在直播和游戏等交互应用中是关键需求，但由于对时间一致性和推理吞吐量的严格要求，这仍然是一项严峻挑战。

Method: 提出三个核心设计：(1)混合扩散Transformer架构在部分块中引入softmax注意力以提高局部建模能力，同时保留线性层的效率；(2)循环逆转正则化是一种新型训练策略，通过流匹配从生成内容预测源帧来强制语义一致性，无需配对的长时间编辑视频即可改善时间一致性；(3)高效系统协同设计结合融合GDN内核和针对NVIDIA Blackwell(RTX 5090)架构优化的混合精度量化(MPQ)。

Result: 在单个RTX 5090 GPU上实现了1280x704分辨率的实时编辑，端到端FPS达24，DiT核心运行于58FPS。实验结果表明，该协同设计方法在时间一致性和系统吞吐量方面显著优于现有最先进方法。

Conclusion: 通过系统-算法协同设计方法，能够在消费级GPU上实现高质量实时视频编辑，为交互式视频应用提供了新的解决方案。
...
TL;DR: 论文提出了一种无需训练的解耦时间引导(DTG)框架,用于恢复AI生成和真实世界的失真、低分辨率视频。该方法通过在更干净的扩散时间步评估无条件分支,提供lookahead先验来保留几何结构同时抑制扭曲内容的复制。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型虽然具有出色的生成保真度,但在标准无分类器引导中条件分支和无条件分支的强耦合限制了其在修复任务中的应用。作者希望在不训练的情况下利用这些先验改善失真和低分辨率视频。

Method: DTG框架的核心是在更清洁的扩散时间步评估无条件分支,提供lookahead先验保留几何结构的同时抑制扭曲内容的复制。这种时间偏差在采样过程中逐步退火,使模型能够从结构修正过渡到细节细化,且无需任何再训练。该方法可与任何现成的修复模块即插即用结合使用。

Result: 在作者 curated的GenWarp480基准(包含4400个来自不同文本到视频模型的失真480p视频)上进行广泛实验,结果表明该方法在结构保真度和时间稳定性方面实现了显著提升,且无需任何模型训练。

Conclusion: DTG是一种有效的训练-free视频修复框架,通过解耦时间引导实现了结构修正到细节细化的平滑过渡,在AI生成和真实世界视频上都能恢复合理的结构,且可与现有修复模块无缝集成。
...
TL;DR: 针对生成式AI图像编辑器产生的空间错位、纹理失真和内容幻觉问题，提出结构保持的GenAI融合框架，通过建立空间和光度对应并执行融合阶段，在保留感知增强的同时强制执行原始输入图像的结构保真度。


<details>
  <summary>Details</summary>
Motivation: 生成式AI图像编辑器如Nano Banana虽然能为非专家提供通过文本提示编辑图像的能力，产生视觉上引人注目的结果，但其生成性质常引入空间错位、纹理失真和内容幻觉，这对需要像素级保真度的下游工作流程不利。

Method: 提出一种后处理框架称为"结构保持GenAI融合"，包括：首先建立输入图像与其GenAI增强版本之间的粗略空间和光度对应；然后执行融合阶段，转移所需的增强效果同时抑制幻觉内容。在缺乏该设置直接先例的情况下，与光真实感风格传递和图像融合的代表方法进行对比评估。

Result: 实验表明，该方法在保持像素级结构一致性和输入分辨率的同时，更好地保留了美学质量。

Conclusion: 提出的结构保持GenAI融合框架能够有效解决GenAI图像编辑器的缺陷，为需要精确像素级保真的应用提供了可行的解决方案。
...
### [5] [Seeing Before Agreeing: Aligning Multi-Agent Consensus with Visual Evidence](https://arxiv.org/abs/2605.30698)
...
TL;DR: EAGLE是一个无需训练的多智能体视觉问答框架，通过让多个VLM智能体共享和验证视觉证据（图像区域）来实现更可靠的答案共识，而不是仅依赖文本层面的回答一致性。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体VQA方法主要借鉴文本问答协议，过度关注文本讨论而忽视视觉信息对齐。现有方法仅依赖答案级别的一致性，这对于可靠的多智能体VQA是不充分的——需要对齐的视觉证据作为可信赖共识的基础。

Method: EAGLE框架包含三个核心步骤：1)显式暴露每个智能体的 grounding 区域作为视觉证据；2)支持智能体间对证据的相互验证；3)使用证据一致性来引导最终决策。这是一个无需训练、轻量级、可解释的方法。

Result: 在六个VQA基准数据集上的实验表明，EAGLE在跨领域上获得了最佳的平均性能，同时保持了轻量级、可解释性和实际部署的实用性。

Conclusion: 该工作揭示了视觉证据对齐是多智能体VQA成功的关键因素，为可信的多智能体视觉问答提供了新的研究方向。
...
TL;DR: 本文提出了一种基于北极地区的高分辨率卫星遥感基础模型(RSFM)，通过结合多样性感知的区域尺度图像筛选与掩码自编码器(MAE)自监督预训练Vision Transformer(ViT)编码器，在约300万张精选图像上进行预训练，旨在提升北极地区精细化制图任务的性能。


<details>
  <summary>Details</summary>
Motivation: 北极地区的卫星影像具有独特的环境特征，现有通用地球观测基础模型（如ImageNet初始化模型、Prithvi-EO-2.0等）在北极特定任务上迁移效果有限，需要开发专门针对北极地区的高分辨率遥感基础模型以提升下游任务的准确性。

Method: 采用多样性感知的大规模图像筛选策略，结合光谱和采集元数据进行仿射传播聚类，从267TB的Vantor极高分辨率卫星影像中筛选出约300万个样本芯片，随后使用领域自适应的MAE重建目标对ViT-Large编码器进行自监督预训练，预训练得到的权重被集成到现有的位置感知检测分割框架中进行评估。

Result: 与ImageNet初始化的ViT-Large基线相比，北极MAE预训练在四个手标注数据集上实现了显著的性能提升：基础设施F1分数为0.87，IWP为0.72，RTS为0.93，TCNs为0.87，提升幅度约为5-8个百分点；该模型在所有下游对比中均优于Prithvi-EO-2.0，最小提升达15个百分点以上。

Conclusion: 在区域尺度上优化预训练数据分布，同时保持架构和MAE目标不变，能够为多个极高分辨率北极遥感应用生成可复用的北极域编码器，表明领域特定的自我监督预训练比通用地球观测基础模型提供更可迁移的表示。
...
TL;DR: 本文提出了一种名为GCSER-UNet的深度学习模型，用于从多模态MRI图像中自动分割脑肿瘤。该模型结合了空间注意力和通道注意力机制，在TCGA LGG数据集上达到94%的Dice分数，在BraTS 2020数据集上对Whole Tumor、Tumor Core和Enhancing Tumor三个区域分别达到95%、92%和90%的Dice分数，优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 脑癌的严重性要求精确的脑肿瘤分割，这对有效诊断至关重要。传统手动分割方法成本高、劳动密集且容易出错，因此需要自动化分割方法来提高效率和准确性。

Method: 提出了Global Context-aware Squeeze and Excitation Residual UNet (GCSER-UNet)，该模型融合了空间注意力和通道注意力机制（SE模块），增强了模型捕获精细空间依赖性和上下文信息的能力，能够高效地从多模态MRI切片中提取肿瘤区域。

Result: 在TCGA LGG数据集上实现了94%的Dice分数，优于之前最先进的91.8%。在BraTS 2020数据集上，Whole Tumor、D Tumor Core和Enhancing Tumor区域的Dice分数分别为95%、92%和90%，其中WT和ET优于之前的94%和88%，T略低于之前的93%。

Conclusion: GCSER-UNet在精确脑肿瘤分割方面表现出色，能够帮助神经科医生进行有效的癌症管理和治疗规划。
...
TL;DR: 提出OmniMem框架，通过稀疏KV检索解决自回归视频生成长视频生成中历史KV缓存访问成本高的问题，实现显式全范围历史细节检索。


<details>
  <summary>Details</summary>
Motivation: 自回归视频生成扩展视频时需要重复访问增长的历史KV缓存，现有方法通过截断或压缩KV缓存降低成本，但都失去了对查询相关历史细节的显式访问能力。

Method: 提出两个关键技术：(1)自适应窗口排除(Adaptive Window Exclusion)在有足够长程历史时从候选中选择中移除局部窗口块，将稀疏预算保留给信息丰富的长程检索；(2)查询共享KV选择(Query-Shared KV Selection)减少跨查询多样性，同时每头分散KV访问(Per-Head Scattered KV Access)避免将头部特定选择扩展到大型选中KV缓冲器。

Result: 在长视频生成任务上，OmniMem将动态自由度(Dynamic Degree)提高52.3%，并在保持相当内存使用量的同时保留了很强的一致性。

Conclusion: OmniMem是一种有效的显式全范围内存检索框架，能够在不显著增加内存使用的情况下，显著提升长视频生成的质量和一致性。
...
TL;DR: 本文提出SpatialUncertain评估框架，用于测试视觉语言模型在观察不完整或误导时的空间推理能力。框架引入两种挑战：遮挡（隐藏目标信息）和透视歧义（产生误导性视觉线索）。研究揭示了当前VLMs的两个关键失败模式：在视觉证据不足时仍过度自信地回答，且无法识别哪些额外视角可以提供可靠证据。


<details>
  <summary>Details</summary>
Motivation: 现有空间推理基准假设视觉观察是充分可靠的，只关注模型是否给出正确答案，而忽略了测试模型是否意识到问题无法回答以及需要哪些额外观察。这是首次有研究系统性地评估模型在观察挑战下的不确定认知能力。

Method: 构建了受控评估框架SpatialUncertain，设计了两类观察挑战配置（遮挡和透视歧义），为每种配置设计了在不同观察条件下可答或需回避的空间问题，并评估了多种前沿开源和闭源VLMs识别有用额外视角的能力。

Result: 发现两个一致的失败模式：(1)过度自信回答——即使视觉证据不完整或误导，仍尝试解决任务，遮挡下平均准确率约30%，透视歧义下低于10%；(2)无法识别可靠证据——即便有额外视图可用，部分模型的识别能力接近随机水平。

Conclusion: 研究呼吁将评估范式从单纯追求答案正确转向evalua模型是否知道何时该放弃回答以及如何寻求可靠证据，这对于真实部署至关重要。
...
TL;DR: 提出VLM3方法，证明标准VLMs通过简单设计即可实现有效的3D学习，无需复杂的专家模型架构。


<details>
  <summary>Details</summary>
Motivation: 当前3D理解主要依赖具有复杂任务特定设计的专家视觉模型，作者主张VLMs本身是原生3D学习器，旨在探索简化方案。

Method: 提出三个关键要素：1)焦距统一；2)基于文本的像素引用；3)数据混合与缩放。采用标准VLMs架构和文本训练，无需架构修改、大型模型、重数据增强或回归损失。

Result: 深度估计准确率显著提升(0.84→0.9)，并成功支持多种3D任务(像素对应、相机姿态估计、目标级3D理解)，达到与专家视觉模型相当的准确率。

Conclusion: VLM3为简单可扩展的3D学习开辟了新范式，证明有效3D学习无需复杂的专家模型设计，仅需简单的数据与训练策略。.
...
TL;DR: 该综述将工业视觉sim-to-real重新定义为基于先验可用性的域差距问题，区分了CAD可用、CAD不可用和边界先验三种设置，并将CAD检测/6D姿态估计与工业异常检测/表面检查文献连接起来。


<details>
  <summary>Details</summary>
Motivation: 当前工业部署面临更广泛的证据与决策不匹配问题，现有综述未能弥合CAD相关检测与异常检测两个独立研究领域之间的鸿沟。

Method: 提出了基于先验可用性的分类框架，以T-LESS/BOP、MVTec AD和VisA为实证锚点进行验证。

Result: 仅增加CAD渲染数量无法弥补迁移差距；源分布设计、检测器容量和小规模真实样本校准可能更为关键。CAD在测试时可通过mask、pose和深度一致性创建独立的验证通道。

Conclusion: 建议反对单一跨任务排行榜，转而根据具体部署场景的先验条件来评估方法的适用性。
...
TL;DR: ReGuLaR是一种将潜在推理显式建立在问题相关对象和对象间关系上的关系 grounded 潜在推理框架，用于提升大视觉语言模型的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法将推理移入连续潜在空间，但未充分连接潜在推理与视觉证据的组合和关系结构。文本化的中间推理步骤不足以编码连续视觉证据。

Method: 提出ReGuLaR框架，训练时使用ReGFormer引导潜在推理聚焦于问题相关的对象和对象间关系；推理时无需ReGFormer即可推理并生成答案。同时构建了RGROUNDING-351K数据集，含有关键对象边界框和对象间关系标注。

Result: 在多个基准数据集上的一致性实验表明，ReGuLaR始终优于现有方法，达到最先进性能。

Conclusion: 通过显式地将潜在状态建立在视觉证据的关系结构上，ReGuLaR有效提升了视觉语言模型的推理能力，为多模态推理提供了新的研究方向。
...
TL;DR: 论文提出Crafter和CraftEditor两个系统，用于自动化生成可编辑的科学图表，并引入CraftBench基准测试集解决现有图表生成系统只支持单一图表类型、无法编辑的问题。


<details>
  <summary>Details</summary>
Motivation: 制作publication-quality科学图表仍然是论文撰写中最劳动密集型的部分之一，现有自动化系统各有局限：仅针对单一图表类型、仅接受文本输入、输出栅格图像无法本地化修改，无法满足研究人员实际使用的多样性和灵活性需求。

Method: 提出两种互补系统：1) Crafter，一个多智能体框架(harness)，可在不改变架构的情况下泛化到不同图表类型和输入条件；2) CraftEditor，将栅格输出转换为可编辑SVG。同时创建CraftBench基准，包含三种图表类型和四种输入条件，带有人工质量标注。

Result: 实验表明，Crafter在PaperBanana-Bench和CraftBench上显著优于独立生成器和智能体基线，消融实验确认各组件的独立贡献；CraftEditor忠实地将输出转换为可编辑SVG，超越所有基线。

Conclusion: 通过多智能体框架harness的设计思想和将栅格输出转换为可编辑SVG的创新方法，成功解决了科学图表生成的多样性和可编辑性问题，为论文绘图提供了更高效的自动化工具。
...
TL;DR: 本文提出了一种基于可控潜在扩散模型的肺结节合成方法，通过引入直方图正则化项来解决传统方法中过分依赖空间重建损失导致的纹理过度平滑和不同结节亚型衰减特征表达不足的问题，从而在3D CT扫描中生成逼真的肺结节图像。


<details>
  <summary>Details</summary>
Motivation: 肺结节自动诊断系统在CT肺癌筛查中取得显著成功，但其发展受到多样化标注肺结节数据集稀缺的限制。现有条件扩散模型主要优化空间重建损失，鼓励体素级相似性，但未能充分约束结节级别的强度分布，导致生成的结节纹理过度平滑，且无法准确表征不同结节亚型（包括实性结节、部分实性结节和磨玻璃结节）的独特衰减特征。

Method: 研究提出了一种可控潜在扩散模型，用于在全3D CT体积中合成肺结节，同时准确模拟结节特异性强度分布。具体而言，团队引入了基于直方图的正则化项，而非仅依赖空间重建损失，来约束生成过程中的体素强度分布。该模型结合了结节亚型条件、空间掩模条件和HU（汉斯菲尔德单位）直方图条件，并使用可微分特征空间直方图正则化项，以更好地对齐结节级别强度分布，提高合成结节的视觉可信度和亚型一致性。

Result: 在肺CT数据上进行的广泛实验表明，该框架实现了较强的视觉真实性，并通过定量指标和视觉图灵测试得到验证。此外，当用于数据增强时，生成的结节改善了下游临床任务的性能，特别是对于代表性不足的结节亚型，并显示出对亚型知情恶性分类的潜在益处。

Conclusion: 该研究提出的直方图正则化潜在扩散框架能够有效解决肺结节数据稀缺问题，通过显式建模结节强度分布实现了更真实的结节合成。实验结果表明，该方法不仅提升了视觉质量，还在数据增强场景中显著改善了包括恶性分类在内的下游临床任务性能，尤其对 underrepresented 结节亚型效果明显，展示了在医学影像数据增强和辅助诊断中的实用价值。
...
TL;DR: 提出了主动实例验证（AIV）任务，解决现有具身智能体仅能导航到目标附近但无法验证细粒度属性匹配的问题。通过构建包含3000个评估 episodes 的PInVerify离线基准，测试了多种基于多模态大语言模型的智能体，最终LoRA微调智能体达到85.6%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能体导航任务仅要求到达目标物体附近，但细粒度属性差异（如"白色花卉"vs"白色条纹"）需要近距离多视角检查才能区分。当前缺乏针对这种主动验证能力的数据集和基准。

Method: 构建PInVerify基准（18类物体，3000 episodes，6扇区导航拓扑）；测试免训练管道和LoRA微调端到端智能体（≤8B参数）；采用属性分解、可见性加权多视角追踪器和三种NBV策略；基准模型包括Qwen3-VL、SenseNova-SI-1.2-InternVL3-8B、CLIP、SigLIP2等。

Result: 最佳MLLM基线比最佳嵌入基线高4.9pp；GT-box消融显示+3.1pp检测差距；NBV策略未产生可靠增益；SFT+GSPO微调的LoRA智能体达到85.6%。

Conclusion: 首次定义了AIV任务并建立基准，发现当前MLLMs在此任务上存在提升空间，主动视点选择策略效果有限，微调是有效提升手段。
...
TL;DR: WristCompass通过利用肢体解剖学知识（手腕运动与相机姿态的运动学耦合关系）解决自我中心视频中手部遮挡场景几何时的相机姿态恢复问题，仅用200K参数GRU实现14.3°中位测地线误差，零样本迁移到Epic Kitchens数据集。


<details>
  <summary>Details</summary>
Motivation: 模仿学习需要将手部运动与相机运动分离，但当手部遮挡画面时，现有基于场景几何的方法（如VGGT）性能甚至不如常数预测器，需要寻找替代视觉线索。

Method: 提出利用运动学耦合动力学——手腕运动与相机姿态之间的结构化物理关系（由手臂-肩膀-头部链产生），使用4维手腕间特征配合GRU时序建模，仅在桌面操作数据上训练。

Result: 4D手腕间特征优于126维完整手部关键点；WristCompass在Epic Kitchens上达到14.3°中位测地线误差，以200K GRU参数接近1B参数场景模型的性能。

Conclusion: 运动学耦合动力学是一个紧凑、时序化且物理根基的特征，因为源于解剖学而非场景外观，可零样本跨数据集迁移。该方法为受限场景下的姿态估计提供了新思路。
...
TL;DR: 提出将数学形态学引入机器学习，结合Morphological Reconstruction聚类算法和新型混合距离度量，实现高效准确的聚类和分类。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法往往忽视数据的形状和密度特征，而数学形态学作为成熟的视觉计算理论，在捕获这些特征方面具有优势。

Method: 1. 提出基于形态学重建的快速聚类算法；2. 设计结合Minkowski和Chebyshev距离的新型混合距离度量；3. 开发新型形态学分类器建模形状、密度和分形信息。

Result: 新距离度量在Z²离散邻域迭代中比Manhattan快1.3倍，比Euclidean快329.5倍；在33个UCI数据集的k-NN分类中，26/33达到Above-average，9个数据集取得最佳精度。

Conclusion: 成功将数学形态学理论与机器学习结合，所提方法在效率和精度上均表现优异，为处理形状和密度敏感的数据分析任务提供了新思路。
...
TL;DR: 本文提出Residual Latent Flow框架，解决等变表示学习中潜在的对齐问题，从而提升新视角合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的几何感知生成模型和新视角合成方法存在潜在的 misalignment问题，即潜在空间中的群变换与实际所需的变换之间存在偏差，导致学习到的潜在表示无法保持由底层对称性施加的等变关系。

Method: 提出Residual Latent Flow——一种基于流的框架，用于纠正对齐错误的潜在表示，从而提高对底层等变关系的遵循度，在旋转群SO(n)下进行实验。

Result: 该方法显著降低了潜在的misalignment，并提升了新视角合成的质量。

Conclusion: Residual Latent Flow能够有效解决等变表示学习中的潜在对齐问题，显著改善新视角合成的效果。
...
TL;DR: 研究针对中国城市村庄中GPS信号不可靠和地图数据不完整的问题，提出了一种基于视觉的地理定位解决方案。该方案使用双摄像头系统（全景相机+智能手机相机）采集同步图像数据，以广州石牌村为案例开发了专门的图像地理定位数据集，并评估了现有视觉模型在此类密集城中村环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 城市村庄是中国快速城市化过程中形成的非正式定居点，是大城市农民工的主要居住地。然而，这些地区建筑密集导致GPS信号不可靠，地图数据不完整，影响了日常出行导航和紧急救援响应。因此需要一种不依赖GPS的替代定位方案。

Method: 采用低成本的双摄像头数据采集管道，使用全景相机和智能手机相机同步采集360度全景图和查询图像。以广州石牌村为案例研究区域，开发专门的图像地理定位数据集，评估现有视觉模型在不同场景类型下的性能。

Result: 结果表明基于视觉的定位方法在密集城中村环境中既有潜力也有局限性。该框架有望改善这些地区的行人导航、最后里程配送和应急管理。

Conclusion: 提出的视觉地理定位框架为GPS覆盖不足的城中村地区提供了实用的解决方案，有助于提升在这些非正式定居点生活的弱势群体的出行安全和应急响应能力。
...
TL;DR: 提出一种高效的多WSI病理报告生成视觉语言模型，通过轻量级MLP对齐器和两阶段训练，在仅半个GPU资源下实现高精度报告生成。


<details>
  <summary>Details</summary>
Motivation: 病理报告生成面临三大挑战：千兆像素分辨率导致长序列、案例包含多张异构WSI、以及复杂案例级推理需求，现有方法计算成本过高难以实际应用。

Method: 采用三组件简洁架构——冻结病理patch编码器+轻量两级MLP视觉语言对齐器+LLM解码器，使用显式WSI标记token分隔幻灯片。训练分两阶段：(1)异构WSI-文本对进行对齐器 captioning；(2)案例-报告对进行结构化报告生成微调。为降低序列长度，每张切片使用512×512 patches at 5× magnification，相比常用20×减少64倍序列长度。

Result: 两训练阶段均获高分ROUGE-L/METEOR/BLEU-4，内存和运行时显著优化。AI评估中模型持续优于强基线。消融实验验证性能-效率权衡并识别提升多WSI鲁棒性的简单策略。

Conclusion: 该工作为高效病理报告生成提供强大可复现基线，大幅降低有限计算资源下多WSI VLAB研究的门槛，为病理AI实际应用奠定基础。
...
TL;DR: 本文研究无监督时间句子 grounding（TSG）问题，即在训练时既没有视频-查询配对注释也没有片段边界注释的情况下，将自然语言查询与视频中的相关时间段进行匹配。


<details>
  <summary>Details</summary>
Motivation: 现有的TSG方法虽然取得了一定成绩，但严重依赖大量昂贵的人工标注视频-查询配对数据，在实际应用中获取成本极高。因此，作者希望探索一种更实用的无监督设置。

Method: 提出一种跨模态知识迁移方法：1）从Image-Noun任务迁移实体感知的外观知识到独立视频帧；2）从Video-Verb任务提取事件感知动作表示，并通过新提出的copy-paste方法进一步细化；3）将-appearance和action知识调制并融合到无监督任务中，使模型无需训练即可关联视频和查询并检索相关片段。

Result: 在ActivityNet Captions和Charades-STA两个数据集上进行了广泛实验，结果表明该方法优于现有无监督方法，甚至可以与一些有监督方法相媲美。

Conclusion: 通过从其他跨模态任务迁移简单的配对对齐知识（图像-名词、视频-动词），可以有效解决复杂的无监督时间句子 grounding 问题，为实际应用提供了更实用的方案。
...
TL;DR: 大型视觉-语言模型在零样本识别中取得成功,但存在"语义傲慢"问题——将未知异常强制分类到已知类别中。为解决开放世界可信度悖论,提出Immuno-VLM框架,灵感来自生物免疫学的阴性选择原理,利用大语言模型生成"语义抗体"(近分布异常的文本描述)来界定已知类别的决策空间。ImageNet-1K和四个OOD基准测试表明该方法达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉-语言模型存在"语义傲慢"问题:缺乏明确的负知识,导致模型将未知异常以高置信度错误地分类到已知类别中,这就是所谓的"开放世界可信度悖论"。传统开集识别方法依赖被动密度估计或在像素空间低效生成异常样本,效果有限。

Method: 提出Immuno-VLM框架,核心思想是借鉴生物免疫系统的阴性选择原理。具体方法:1)利用大语言模型的生成推理能力主动幻觉出"语义抗体"——近分布异常(如相似物、上下文异常)的文本描述;2)用这些语义抗体来界定已知类别的决策空间,使模型能够识别未知/异常样本。

Result: 在ImageNet-1K和四个具有挑战性的OOD(分布外检测)基准测试上进行了广泛实验,结果表明Immuno-VLM建立了新的最先进水平(state-of-the-art)。

Conclusion: Immuno-VLM通过生物启发的免疫阴性选择机制,有效解决了大型视觉-语言模型在开放世界部署中的语义傲慢问题,为零样本识别中的OOD检测提供了新思路。
...
TL;DR: 本文提出语义最小作用原理(SLAP)，将视频插帧从概率生成范式转向变分力学方法，将潜在视频轨迹建模为黎曼流形上的路径，通过离散欧拉-拉格朗日方程求解边值问题，实现物体持久性和语义一致性。


<details>
  <summary>Details</summary>
Motivation: 大型视频语言模型中的稀疏帧采样导致"时间间隙"问题，使模型无法感知关键因果转换。现有基于生成分布式幻觉或自回归外推的方法在长时域上难以保持语义一致性，存在物体消失和能量不稳定的问题。

Method: 提出语义最小作用原理(SLAM)，建立经典力学与语义动力学的严格同构。将潜在视频轨迹建模为由语义拉格朗日量控制的黎曼流形上的路径，将插帧任务表述为边值问题(BVP)，通过离散欧拉-拉格朗日方程求解，无需像素级渲染即可自然强制物体持久性。

Result: 大量实验表明SLAP方法的有效性。

Conclusion: SLAP代表了一种从概率生成到变分力学的范式转变，为视频插帧任务提供了新的理论框架，在保持物体持久性和语义一致性方面具有优势。
...
TL;DR: 本文提出DisPlace，一个多参考视觉地点识别(VPR)框架,将多个参考描述子融合为一个紧凑且具有判别性的地点表示。


<details>
  <summary>Details</summary>
Motivation: 视觉地点识别的关键挑战是匹配在不同环境和视角下拍摄的查询图像与参考地图。现有的融合策略要么统一聚合参考数据,要么依赖启发式选择,无法区分保留稳定地点身份的描述子变化与由环境变化或视角变化引起的描述子变化。

Method: DisPlace将描述子融合表述为广义特征值问题,在抑制参考间地点内变化的同时最大化地点间可分性。该方法利用跨参考遍历的变化来识别哪些描述子维度的线性组合保留地点身份,哪些捕获条件或视角特定的变化。

Result: 在Oxford RobotCar、Nordland、Pittsburgh30k和Google Landmarks v2数据集上,针对六种先进的VPR描述子进行评估,DisPlace在54种外观变化条件中的49种中优于七个多参考基线方法,在视角和非结构化设置下持续提升描述子级融合性能,且推理时存储需求低于所有比较的融合方法。

Conclusion: DisPlace能够有效融合多个参考描述子为紧凑且具有判别性的表示,在各种条件下提升VPR性能,为多参考地点识别提供了一种新的融合范式。
...
TL;DR: 提出CameraNoise方法，通过将相机姿态编码到噪声空间中实现视频扩散的精确相机位姿控制，保持几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有直接注入数值相机参数的方法无法弥合抽象坐标与视觉内容之间的鸿沟，导致结构失真。

Method: 引入几何引导的重投影流和噪声扭曲算法，将相机姿态嵌入噪声空间，解除运动与场景外观的耦合，同时保持高斯先验和噪声传播的一致性。

Result: 在视觉质量和轨迹保真度方面显著优于现有方法，实现稳定、高保真的视频生成。

Conclusion: CameraNoise通过流到噪声扭曲的方式成功解决了视频扩散中的几何一致性问题，为相机运动控制提供了新思路。
...
TL;DR: TCFDNet是一种用于解决LiDAR-相机跨模态步态识别难题的新方法，通过利用大语言模型生成的文本语义描述作为引导，实现模态共享特征的解耦学习，在SUSTech1K和FreeGait数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: LiDAR-相机跨模态步态识别(LCCGR)是一个重要但具有挑战性的任务，因为2D视频与3D点云序列之间存在显著的模态差异，現有方法难以有效处理这种异构传感器的场景。

Method: 提出TCFDNet，包含三个关键模块：1)构建步态模态文本字典(GMTD)利用大语言模型生成丰富的语义描述；2)CLIP多粒度特征编码器对齐视觉和文本特征；3)文本引导的特征解耦(TFD)模块通过残差分解和正交约束选择topk匹配文本来重建模态特定特征并解耦模态共享特征；4)特征稳定性增强(FSE)模块建模空间和通道相关性提高鲁棒性；5)跨模态patch交换策略提升泛化能力。

Result: 在SUSTech1K和FreeGait数据集上的广泛实验表明，TCFDNet达到了新的最先进水平，验证了所提模块的有效性。

Conclusion: 通过文本引导的跨模态特征解耦策略，可以有效弥合LiDAR与RGB相机之间的模态差距，为跨模态步态识别提供了新的研究思路。
...
TL;DR: Function2Scene是一个基于功能规范的3D室内布局生成框架，将传统的以对象为中心的场景合成转变为以人为中心的空间设计。该系统接收自然语言设计 briefs，解析使用者 persona 和活动，推导17个维度的功能设计约束，并通过迭代检查-修复循环生成满足人类使用需求的室内布局。


<details>
  <summary>Details</summary>
Motivation: 现有方法关注放置什么家具，而非空间如何使用。真实室内设计中，布局质量取决于其对居住者的支持程度（如活动和身体需求）。现有以对象为中心的生成范式无法满足这一设计理念。

Method: 1)解析自然语言设计规范，提取居住者人物画像和活动；2)从17个维度（空间、人体工程学、活动、环境）的功能设计约束taxonomy中推导定制化约束；3)通过工具增强的check-and-repair循环进行迭代评估和优化，结合几何测量、LLM上下文推理和VLM视觉评估；4)不依赖LLM直接输出最终场景。

Result: 在30个专业室内设计案例上的实验表明，Function2Scene生成的布局相比近期的LLM基线更能满足功能需求，在94.3%的成对比较中被优先选择。

Conclusion: 该工作将文本驱动的室内场景合成从"放置合理的物体"重新定义为"设计支持人类使用的空间"，提出了以功能规范驱动的新范式，为人本导向的智能室内设计开辟了道路。
...
TL;DR: LegSegNet是一个用于下肢CT组织分割和身体成分量化的端到端深度学习系统，可分割骨骼、骨骼肌、皮下脂肪组织和肌间/肌内脂肪组织，并在900个测试切片上实现了89.31的平均Dice得分。


<details>
  <summary>Details</summary>
Motivation: 现有公开的分割工具缺乏针对下肢CT的综合分析能力，特别是无法识别临床上重要的肌间/肌内脂肪组织，且大多数方法仅提供mask预测而非端到端的量化系统，难以满足大规模提取临床相关信息的需求。

Method: 使用1,302个手动标注的CT切片训练分割模型（标注均经放射科医生审核），对比了CNN模型、Transformer模型和微调的基础模型等多种2D分割方法，并在外部公共CT数据集上评估泛化性能。

Result: LegSegNet在保留测试集上达到最佳分割性能，平均Dice得分为89.31分，并展现出良好的跨数据集泛化能力。

Conclusion: LegSegNet是首个公开可用的下肢CT组织分割与量化端到端系统，为医学影像分析领域的计算机视觉研究提供了实用的评估工具。
...
### [30] [Untitled](https://arxiv.org/abs/2605.30846)
**
...
TL;DR: Robust Dreamer是一个基于记忆增强的3D aware图像到视频生成框架，通过潜空间高斯记忆和动态偏差存档技术解决长时序自回归生成中的视觉退化和3D一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D感知方法在长时序自回归推演中存在灾难性漂移，主要源于两个问题：1) Latent-RGB Cycling导致的累积信息损失；2) error-free hypothesis带来的训练-推理差距。

Method: 提出两个核心技术创新：1) Latent Gaussian Memory - 将扩散生成的latent锚定到高斯原语，通过潜空间高斯splatting实现几何感知的视图对齐条件化，避免VAE转换的累积退化；2) Deviation Learning with Dynamic Deviation Archive - 通过一步近似合成推演诱导的latent偏差，按自回归阶段和时间戳存储，在训练时注入历史记忆以暴露真实的记忆损坏状态。

Result: 在ScanNet、DL3DV和OmniWorldGame数据集上实现了最先进的长时序性能。

Conclusion: 该方法通过设计高质量的3D记忆机制和稳健的记忆使用策略，有效解决了3D场景下长时间视频生成的关键挑战。
...
TL;DR: 本文提出了一种基于静态-动态分解策略的高效动态重建框架，结合前馈高斯散射编码器和光流模型，用于解决复杂时变场景的高质量重建问题。该方法通过消除静态区域的冗余计算，实现了在渲染质量、训练和渲染速度以及存储效率方面的最优性能。


<details>
  <summary>Details</summary>
Motivation: 动态场景重建和新视角合成是虚拟现实、机器人技术和数字孪生等下一代视觉智能应用的基础。从任意视点对复杂、时变场景进行高质量重建仍具挑战性。现有动态3DGS方法因将所有高斯建模为动态组件而存在计算效率问题。基于分解的方法虽解决了该问题，但在重建质量和训练时间方面仍表现不佳。

Method: 提出了一种新颖的动态重建框架，基于高效的静态-动态分解策略，使用前馈高斯散射编码器和光流模型。通过消除静态区域的冗余计算实现高效处理。

Result: 在Neural 3D数据集上，该框架仅需10分钟训练即可完成重建，在单张NVIDIA RTX 5090 GPU上（分辨率1352x1014）达到超过700 FPS的渲染速度。分解策略消除了COLMAP预处理需求并实现确定性初始化，提高了效率和可重复性。在渲染质量、训练和渲染速度及存储效率方面均优于现有基线方法。

Conclusion: 通过采用静态-动态分解策略结合前馈高斯散射编码器，本方法成功解决了动态场景重建中计算效率与质量之间的权衡问题。无需预先进行复杂预处理，实现了快速训练和高渲染速度，为实时视觉应用提供了可行的技术方案。
...
TL;DR: 该研究提出一种无需训练的医学VAE框架，利用在自然图像和视频上预训练的基础VAE作为统一接口，实现CT重建、增强和生成任务。


<details>
  <summary>Details</summary>
Motivation: 针对CT特异性的VAE训练成本高且在不同扫描器、协议和疾病下性能下降的问题，探索利用大规模预训练模型进行医学图像处理的可能性。

Method: 冻结编码器和解码器，直接使用预训练的基础VAE进行CT重建；在同一潜在空间中训练条件潜在扩散模型进行CT生成。

Result: 重建后的分割模型在胰腺肿瘤和肺肿瘤上NSD平均提升3.9%；生成模型FVD降低3.9%，CT CLIP分数提高36.2%，18种疾病生成AUC提升2.76%。

Conclusion: 基础VAE可作为医学CT表示复用和忠实生成的实用接口，为训练-free医学成像提供新思路。
...
TL;DR: 本文针对人脸识别合成数据与真实数据之间存在的显著性能差距问题，提出视觉倾向这一新限制，并推出SteerFace训练框架来缓解该问题。


<details>
  <summary>Details</summary>
Motivation: 人脸识别训练面临合法合规数据短缺的问题，学界尝试使用合成数据替代。虽然基于扩散的方法可以生成逼真的人脸图像，但下游识别性能仍存在明显的合成-真实差距。本文发现了之前未被充分探索的"视觉倾向"问题，即合成数据呈现出不切实际的视觉属性普遍性，从而偏离真实数据分布。

Method: 本文提出SteerFace框架，通过将身份嵌入引导到嵌入超球体上的随机正交方向来扰动身份嵌入。该扰动作为保持身份的正规化器，惩罚生成器对非身份成分的依赖。理论分析支持这一方法，并进一步引入自适应策略来学习扰动强度。

Result: 大量实验表明，SteerFace有效地缓解了视觉倾向问题，在下游人脸识别任务中优于先前方法，并且在不同的训练数据集和生成 pipeline 上表现出良好的泛化能力。

Conclusion: SteerFace是一个简单高效的训练框架，能够在不损害身份保持的前提下，有效减少合成数据中的视觉倾向，缩小合成-真实差距，为合成数据在人脸识别训练中的应用提供了新的解决方案。
...
TL;DR: MergeTok提出了一种统一的图像分词器，通过结合连续的VAE和离散的VQ技术，并利用token聚类作为语义桥梁，同时解决了两类传统分词器的固有局限性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器分为两大家族各有局限：连续VAE重建质量高但latent稠密且纠缠，难以进行语义控制；离散VQ模型支持自回归生成但存在梯度稀疏、训练不稳定和码本坍塌问题。现有方法无法同时实现高质量重建和语义组织。

Method: 在编码器-解码器架构中联合优化连续(VAE)和离散(VQ)分词器，利用token聚类技术作为语义桥。通过在编码阶段聚类相似token，建立结构先验提供双重监督信号：(i)在VAE分支施加合并token的语义对齐，将latent空间正则化为解耦的语义感知表示；(ii)导出组级约束，促进组内多样性和组间排他性以稳定VQ训练。

Result: 在ImageNet-256上展示具有竞争力的重建和生成性能，在相同token预算下相比强VAE和VQ模型获得显著更低的rFID，同时产生兼容自回归和扩散生成器的语义组织化token表示。

Conclusion: 单一架构可以为视觉分词器赋予稳健的语义组织和生成器友好的离散性，统一两类分词器的优势。
...
TL;DR: 本文提出CoSimUE基准测试，系统性地研究大型视觉语言模型(LVLMs)架构设计与幻觉行为之间的关系。通过将架构设计解构为语言基础、视觉表征和语义对齐三个维度，并分析三类幻觉(共现性、相似性、不确定性)，发现参数Scaling效果有限，更好的语言基础可减少共现幻觉，更强的视觉编码器和高分辨率可缓解相似性错误，有效的对齐策略可减轻不确定性幻觉，联合增强视觉保真度和对齐质量可获得最全面的改善。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型的幻觉问题严重影响其可靠性，现有研究多关注模型内部组件改进，而作者认为幻觉从根本上源于模型架构设计方式。缺乏系统性的架构设计与幻觉行为之间映射关系的研究。

Method: 提出CoSimUE基准，通过受控文本扰动和随机扰动创建细粒度幻觉场景。将架构设计分解为三个维度：语言基础(LF)、视觉表征(VR)、语义对齐(SA)；将幻觉分为三类：共现性、相似性和不确定性。实验覆盖7个设计方面。

Result: 1) 参数规模的Scaling对减少三类幻觉的效果有限；2) 更大更好的语言基础可减少共现性幻觉；3) 更强的视觉编码器和高分辨率可缓解相似性错误；4) 有效的对齐策略可减轻不确定性幻觉；5) 联合增强视觉保真度和对齐质量可获得最全面的改进。

Conclusion: 这是首次系统性探索架构级设计与幻觉鲁棒性关系的研究，为开发可靠高效的LVLMs提供了实践指导。表明单纯增大模型参数并非减少幻觉的有效途径，需要在语言基础、视觉表征和语义对齐三个维度协同优化。
...
TL;DR: EASE是一种结合视觉证据过程监督的多模态强化学习方法,通过将注释的证据区域转换为平滑的视觉token目标来引导注意力,仅在高质量轨迹上使用,推理时不需要额外注释。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法的结果奖励无法区分基于相关视觉证据的正确回答和使用语言先验或猜测的错误回答,无法提供视觉接地信号。

Method: 将标注的证据区域转换为平滑的视觉token目标,在RL训练中使用来指导response-to-image注意力,仅应用于高奖励轨迹,推理时无需注释。

Result: 在Qwen2.5-VL-7B、Qwen3-VL-4B、Qwen3-VL-8B上,EASE比DAPO在感知、幻觉、视觉数学和多模态推理基准上平均提升2.5-3.1分。

Conclusion: EASE能更好地将视觉注意力与标注的证据区域对齐,为多模态RLVR提供了有效的视觉证据过程监督信号。
...
TL;DR: 本文提出DiTTo框架，一种基于序贯感知的多退化图像修复智能体系统，通过创新的模拟器和序贯对齐技术解决传统方法中计算效率低和可扩展性差的问题。


<details>
  <summary>Details</summary>
Motivation: 现实图像通常包含多种退化，且退化移除顺序显著影响最终修复质量。现有训练式智能体需要O((N^D)^2)次专家调用构建最优修复轨迹数据集，且无法扩展到新引入的修复专家而不进行完整重训练。

Method: DiTTo框架包含两部分：(1) DiTTo Simulator，结合∪S-IR进行单步修复动作模拟和AiO-IQA进行每动作质量预测，将ORTD构建从O((N^D)^2)降至O(N^D)；(2) DiTTo Agent，通过SFT在模拟器生成的ORTD上训练，然后使用序贯对齐(ORA)将退化识别、修复动作排序和输出格式沿独立轴对齐，实现即插即用的可扩展性。

Result: 在最多包含五种并发退化的MiO-100评估集上，DiTTo Agent在多退化修复质量方面优于此前的智能体式图像修复方法。

Conclusion: DiTTo框架实现了高效且可扩展的多退化图像修复，仅需更新轻量级ORA阶段即可添加新修复专家，为实际应用中的动态退化修复需求提供了有效解决方案。
...
TL;DR: MultiAct是一个无需重新训练的组合式文本到动作合成推理框架，通过自适应增强被忽略的提示词成分的交叉注意力分数，解决现有模型处理多重同时动作描述时的语义崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成模型在处理描述多个同时发生的动作时表现脆弱，模型通常优先考虑单一主导动作而忽略其余部分，导致动作不完整或模糊。需要一种方法让模型能够完整实现复合动作描述中的所有成分。

Method: MultiAct是一个非配对的推理时间框架，直接在预训练的动作生成器上运行，无需重新训练或修改架构。该方法通过自适应放大与被低估提示词成分相关的交叉注意力分数来对抗语义崩溃。引入了一个轻量级的辅助决策方案来确定最有效的注意力增强参数化。

Result: 广泛的定量和定性评估表明，MultiAct在复合提示词上始终优于现有基线方法，在保持动作真实性的同时实现了更好的语义覆盖。

Conclusion: 该研究提出了一种无需重新训练的有效方法来增强文本到动作模型的组合理解能力，为动画和人机交互提供了更富有表现力的界面。
...
TL;DR: 提出一种名为VACSR的变分适配器方法,将图像-文本匹配中的细粒度语义稀缺问题重新表述为变分推理问题,通过构建跨模态相似度的潜空间和正则化技术来缓解对二值标注的过拟合,从而提升跨模态任务的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像-文本匹配数据集缺乏细粒度跨模态匹配标注,迫使连续的相似度空间被压缩为二分类边界,导致假负样本产生并严重损害跨模态任务的泛化性能,而先前的研究往往忽视标注文本身存在的缺陷。

Method: 提出VACSR(跨模态相似度表示的变分适配器),将图像-文本匹配问题建模为变分推理问题,构建跨模态相似度的潜在空间,并使用正则化技术来缓解对二值标注的过拟合。

Result: 在图像-文本检索、域泛化和基到新类别泛化任务上的实验表明了该方法的有效性和鲁棒的泛化能力。

Conclusion: 通过变分推断框架处理细粒度语义稀缺问题,为跨模态相似度学习提供了一种新的思路,能够更好地处理标注不完善情况下的跨模态匹配任务。
...
TL;DR: 本文提出OmniME框架，解决文本驱动动作编辑中平衡"变化"与"不变性"的挑战，通过三种互补监督组件实现高质量动作编辑。


<details>
  <summary>Details</summary>
Motivation: 现有扩散方法依赖启发式相似度线索或粗粒度全局条件，导致动作失真和语义对齐不佳，难以同时实现精确编辑目标区域并保留未编辑部分。

Method: 提出Omni-Supervised Positive-Negative Learning框架(OmniME)，包含：(1)回溯特征监督在transformer层间实施粗到细的一致性；(2)动作保持机制根据源-目标相似度聚焦细微变化；(3)三元组语义对齐增强文本-动作对应关系。

Result: 在MotionFix和STANCE Adjustment数据集上实验表明，OmniME在编辑对齐方面达到最先进性能，验证了统一学习框架的有效性。

Conclusion: 通过整合三种互补监督组件形成统一监督范式，成功平衡了动作编辑中的变化与不变性，为文本驱动动作编辑提供了有效解决方案。
...
TL;DR: Grace-BEV是一种轻量级的即插即用框架，专注于多模态BEV感知中的主动可靠性评估，实现传感器损坏情况下的优雅降级。


<details>
  <summary>Details</summary>
Motivation: 当前多模态BEV感知系统对传感器损坏高度敏感，现有的静态融合机制导致模态缺失时性能急剧下降，甚至崩溃至0%mAP。

Method: 提出三种核心方法：(1)TrustGateRouter在BEV空间评估模态可信度；(2)FailSafeFusion动态校准特征融合；(3)Three-PhaseTraining配合ModalityDropout防止模态主导。

Result: 在nuScenes-R和nuScenes-C上，LiDAR完全失效时baseline降至0%mAP，而Grace-BEV恢复至34.7%mAP；干净数据精度提升1.4%。

Conclusion: Grace-BEV在鲁棒性与效率间取得良好权衡，以轻量即插设计实现各种损坏场景下的稳健性能。
...
TL;DR: 提出了3D医学视觉语言模型(VLM)在CT报告生成中的"模板崩溃"(Template Collapse)问题，即模型生成流畅但缺乏病理检测能力的通用模板报告，并设计了CLarGen框架通过解耦检测与合成来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 解决现代3D医学VLM在CT报告生成中虽然能产生流畅的放射学文本，但病理检测能力极低、输出多样性不足的问题，该问题被称为"模板崩溃"，表现为生成通用模板而遗漏罕见但关键的发现。

Method: 提出CLarGen框架，将"说什么"(临床检测)与"怎么说"(语言合成)解耦。包括：(1)多标签病理检测的潜在查询Transformer；(2)基于病理引导的检索以获取临床匹配的示例；(3)医学语言模型根据检测到的发现和检索到的上下文合成最终报告。

Result: CLarGen在 macro-F1 (0.487 vs 0.189) 和 CRG (0.472 vs 0.368) 上显著提升临床准确性，同时保持流畅的报告生成，有效缓解了模板崩溃问题。

Conclusion: 明确的、可测量的临床基础对于抵抗模板崩溃的3D CT报告生成至关重要，未来需要进一步关注临床接地气的评估指标。
...
TL;DR: 本文研究了3D Gaussian Splatting场景中的对象移除和修复问题，比较了重建型与生成型修复器，并提出新的多目标场景数据集。


<details>
  <summary>Details</summary>
Motivation: 3D场景中对象移除面临视图间3D一致性的挑战，现有360°数据集缺乏真实 ground truth 和充分的遮挡场景。

Method: 集成2D修复器到单步方法中进行3DGS场景创建和微调，比较了重建基修复器与生成扩散模型，从头初始化与微调现有场景两种方式。

Result: 重建基修复器在3D一致性上优于生成扩散模型；从头初始化比微调现有场景效果更好；引入带ground truth的新多目标场景数据集。

Conclusion: 对于3DGS场景修复，重建基方法和从头初始化是更优的选择，建议在3D修复前进行对象移除，并需要包含充分遮挡场景的新型数据集。
...
TL;DR: PEEK是一种高效的视频动态帧采样方法，通过将强教师模型中基于标题的帧相关性排名蒸馏到轻量级时间模型中，用于视频字幕生成。该方法在仅选择1-2帧的低预算场景下表现尤为突出。


<details>
  <summary>Details</summary>
Motivation: 视频语言模型只能处理有限的帧数，使得帧选择成为高效视频字幕生成的瓶颈。现有统一采样方法计算成本低但忽略视觉内容，已有的自适应帧采样方法计算成本较高。

Method: PEEK将强教师模型中 caption-conditioned 的帧相关性排名蒸馏到一个轻量级时间模型中，该模型仅基于视觉内容运行，无需title信息即可判断帧的重要性。

Result: 在ActivityNet Captions和MSR-VTT数据集上，PEEK在所有评估的下游视觉语言模型中均优于最新方法，尤其是在仅选择1或2帧时获得最佳CIDEr评分。在ActivityNet Captions上赢得16种配置中的14种。计算效率上仅增加5.2%的字幕时间，而CSTA增加65.4%，MaxInfo增加211.9%。

Conclusion: PEEK是一种计算高效且准确的自适应帧采样方法，在低帧预算场景下尤其有效，能够以极低的计算开销实现最佳的视频字幕效果。
...
TL;DR: 本文提出SlotMemory，一种用于流媒体视频扩散模型的对象中心Key-Value内存机制，通过将Transformer的键值流形分解为离散的语义槽来解决视频生成中的身份漂移和语义不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有流媒体视频生成模型依赖时间中心内存（如原始帧、片段或未聚类令牌），在实体离开画面或交互提示转换时常导致身份漂移和语义不一致，缺乏有效的长程一致性保持机制。

Method: 提出SlotMemory方法，将内存抽象从"何时发生"转变为"表征什么"，具体包括：1)将Transformer的键值流形分解为离散可重用的语义槽；2)利用这些槽作为路由地址索引存储高保真键值令牌；3)实现跨长时域的实体级持久化和提示感知检索。

Result: 在Wan2.1-T2V-1.3B backbone上对60秒交互式叙事进行评估，SlotMemory达到81.61的质量分数，在动态一致性方面相比最强基线提升22.8%。

Conclusion: 研究表明结构化语义表示而非原始时间容量是持久长形式视频合成的关键基础，这为下一代流媒体视频生成提供了新的设计思路。
...
TL;DR: 本文提出生成式真实标签（GGT）方法，利用多模态基础模型（MFMs）从真实世界低质量图像合成高质量目标，构建了GGT-100K数据集（103,707对训练数据），显著提升了图像恢复模型在真实场景中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 真实世界的图像恢复任务受限于高质量成对训练数据的稀缺性。合成数据集虽丰富但无法准确建模真实退化，真实成对数据集获取成本高且困难，导致现有IR模型在真实场景中的泛化能力有限。

Method: 1）系统评估9种先进的多模态基础模型（包括Nano-Banana-2和GPT-Image-2）在不同场景和退化类型图像上的表现；2）筛选出基于VLM自适应提示的Nano-Banana-2作为最佳模型；3）建立多阶段质量控制的GGT合成流程；4）构建包含103,707对训练样本和500对测试样本的GGT-100K数据集。

Result: Nano-Banana-2具有最高的感知真实性和内容保真度合成能力。GGT-100K数据集能持续提升多种IR模型的真实世界泛化性能，对生成式IR模型的微调效果尤为显著。

Conclusion: 多模态基础模型可作为恢复导向数据生成的实用工具，GGT-100K是扩展真实世界IR模型泛化边界的有效资源。
...
TL;DR: 本文提出了一个结构化的多层次视觉扰动框架，用于分析VLA（视觉-语言-动作）自动驾驶模型中的视觉-行为依赖关系。该框架从通道级退化、信息级破坏和结构级修改三个维度对视觉输入进行可控扰动，并在开环轨迹预测和闭环安全评估两种场景下评估模型的行为响应。实验结果揭示了不同评估方式下的依赖模式差异以及不同抽象层级上的视觉接地不均匀现象。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在自动驾驶领域展现出良好的感知与规划统一建模潜力，但研究界对VLA驾驶行为如何依赖于视觉信息的理解仍然不足。现有评估协议主要关注整体性能指标，缺乏结构化且实用的诊断方法来量化视觉-行为依赖关系。这导致无法深入理解视觉信息如何具体影响驾驶决策，阻碍了更安全、更稳健的VLA驾驶系统的开发。

Method: 研究者设计了一个结构化的多层次视觉扰动框架，从三个互补维度对输入视觉信息进行系统性扰动：1)通道级退化（如颜色通道、亮度等）；2)信息级破坏（如遮挡、噪声注入等）；3)结构级修改（如几何变换、语义变化等）。该框架被应用于多个VLA自动驾驶系统，分别在开环轨迹预测任务和交互式闭环安全评估任务中测试模型的行为响应，记录并分析其对视觉扰动的敏感程度和依赖模式。

Result: 实验结果揭示了两个关键发现：1)依赖模式具有评估依赖性——同一个VLA模型在不同评估范式（开环 vs 闭环）下表现出显著不同的视觉依赖特征；2)视觉接地在不同抽象层级上呈现不均匀性——模型对某些视觉特征的依赖过强而对其他特征的利用不足。这些发现表明现有的VLA驾驶模型在视觉信息处理和利用方面存在结构性问题，需要更精细的分析和设计方法。

Conclusion: 本研究强调需要对VLA自动驾驶模型进行更加结构化的分析，理解视觉信息如何具体塑造驾驶行为。未来研究应着重于：1)采用多维度、多层次的系统评估框架；2)设计更加均衡和稳健的视觉接地机制；3)开发更安全的VLA驾驶系统以应对复杂的真实驾驶场景。这将为构建更可靠、更可解释的自动驾驶视觉理解系统奠定基础。
...
TL;DR: 将裂纹分割重新定义为稀疏结构恢复问题，而不是使用复杂的通用语义分割架构。


<details>
  <summary>Details</summary>
Motivation: 裂纹具有有限的类别语义但具有强烈的形态规律（薄、稀疏、各向异性、局部断裂、易与纹理混淆），现有复杂架构可能导致过拟合，而关键瓶颈在于保留弱结构证据、恢复方向连续性和抑制背景耦合。

Method: 提出RIFT，一个紧凑的形态学对齐裂纹分割模型家族。简单设计，保留局部证据，聚合协作方向连续性，通过轻量级多尺度融合恢复裂纹结构。

Result: 在四个公共基准数据集上，RIFT在16个主要指标中达到最佳或并列最佳结果。RIFT-B提供最强整体精度，RIFT-T仅用0.47M参数实现最佳部署效率和高推理速度。

Conclusion: 当模型的归纳偏置符合裂纹形态时任务对齐的简单性可以匹配或超越复杂的混合架构。
...
TL;DR: 本文系统研究了Panoptic Quality(PQ)度量在低IoU阈值下的多匹配策略问题，将分段匹配重新表述为约束的二分图分配问题，提出了四种匹配策略并扩展到part-aware panoptic分割。


<details>
  <summary>Details</summary>
Motivation: 原始PQ定义依赖One-to-One匹配，仅在IoU阈值超过0.5时straightforward。低于0.5时多种匹配策略出现，但这一问题空间尚未被系统探索。

Method: 将分段匹配重新概念化为约束的二分图assignment问题，独立bounding预测端和真值端的度数，产生四种匹配策略：One-to-One、Many-to-One、One-to-Many和Many-to-Many。核心是基于vertex而非matching edges来account TP、FN、FP。

Result: 前三种匹配策略在PQ框架内明确定义，Many-to-Many落在框架外。这些策略在实例碎片化、相邻物体难以划分或标注噪声时变得相关。框架可自然扩展到part-aware panoptic分割，并在生物医学数据上进行了验证。

Conclusion: 发布了一个基于Panoptica的统一开源包，提供Voronoi区域分析、part-aware评估和Area Under Threshold Curve计算作为可配置选项。
...
TL;DR: LVSA是一种无需训练、模型无关的块稀疏注意力机制，用于解决长视频扩散推理中密集注意力的计算瓶颈和质量退化问题，通过结构化窗口模式结合旋转全局锚点消除固定网格偏差。


<details>
  <summary>Details</summary>
Motivation: 密集自注意力是长视频扩散推理的计算和质量瓶颈：成本随序列长度二次增长，且超出训练视野后模型收敛到近乎静态的输出（冻结重复视频）。现有SOTA方法要么需要重训成本过高，无法同时满足性能和可扩展性要求。

Method: 提出Long Video Sparse Attention（LVSA）：一种训练free、模型无关的块稀疏注意力，结合结构化窗口模式与旋转全局锚点，消除导致长时间伪影的固定网格偏差。联合FlashInfer内核使用。

Result: Wan 2.1 1.3B上6倍视野减少3.17x计算，Wan 2.1 14B上6倍视野减少2.98x，HunyuanVideo 1.5上1.5倍视野减少3.33x。相比RIFLEx加速2.41x，UltraViCo加速3.27x。NPUs上Wan 2.2 A14B加速2.71x，Wan 2.1 1.3B加速3.24x。还能实现HunyuanVideo 1.5在2倍视野生成（单GPU原本OOM）。引入VQeval工具公平评估质量。

Conclusion: LVSA在训练视野长度上质量neutral，在扩展视野长度上quality-positive，是首个能同时实现显著计算加速、内存效率和无质量损失的视频扩散transformer稀疏注意力方案。
...
### [52] [FOCUS: Forcing In-Context Object Localization through Visual Support Constraints and Policy Optimization](https://arxiv.org/abs/2605.31145)
*Mohammed Asad Karim,Vinay Kumar Verma*

Main category: cs.CV

TL;DR: 提出一种基于上下文定位的两阶段训练框架,通过无类别监督的方式实现实例级视觉定位,并使用强化学习(GRPO)直接优化定位误差,在7B参数模型上超越72B参数模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖显式类别监督,存在类别偏差,且无法处理未命名或实例特定对象。需要在没有类别先验的情况下实现真正的视觉对应定位。

Method: 两阶段训练框架:(1)明确优化支持边界框与查询图像之间的上下文注意力,无需类别监督;(2)通过群体相对策略优化(GRPO)进行强化学习微调,直接最小化定位误差。

Result: 7B参数模型使用本方法后，性能超越最大72B参数的模型，表明上下文感知的定位目标可以超越单纯的规模扩展。

Conclusion: 通过强制视觉对应而非语义先验,实现了鲁棒的实例级定位,为图像编辑、个性化视觉搜索等应用提供了新方案。
...
### [53] [HQ-JEPA: Hybrid Quantum Joint-Embedding Predictive Architecture for Cross-Modal Remote Sensing Representation Learning](https://arxiv.org/abs/2605.31068)
*Md Aminur Hossain,Ayush V. Patel,Sanjay K. Singh,Biplab Banerjee*

Main category: cs.CV

TL;DR: HQ-JEPA是一种用于跨模态遥感表示学习的混合量子-经典联合嵌入预测架构,通过掩码潜变量预测实现Sentinel-1和Sentinel-2图像的互表示学习,并引入量子保真度相似性作为正则化信号。


<details>
  <summary>Details</summary>
Motivation: 遥感领域需要从多源卫星数据(Sentinel-1雷达与Sentinel-2光学)中学习有效的跨模态语义表示,传统方法难以同时处理异构模态特征并保持表征质量,因此需要一种融合量子计算优势的创新框架来提升表示学习效果。

Method: 提出四目标联合优化框架:(1)掩码token预测,(2)跨模态token对齐,(3)SIGReg高斯正则化,(4)基于SWAP测试的可微量子保真度损失(FQS);在共享潜在空间中进行几何正则化和量子状态重叠相似度度量。

Result: 在GeoBench分类和分割任务上,经线性探测和微调评估,HQ-JEPA实现了与最强自监督和遥感基础模型基线相当或更优的性能,证明了所提方法的有效性。

Conclusion: 整合预测自监督、跨模态几何正则化和量子保真度表示学习的混合架构能够有效提升遥感应用的表示质量,为多源卫星数据协同学习提供了新的研究方向。
...
### [54] [Towards Effective Long-Video Event Prediction via Multi-Level Event Semantics Mining](https://arxiv.org/abs/2605.31069)
*Bo Peng,YuanJie Lyu,PengGang Qin,Tong Xu*

Main category: cs.CV

TL;DR: 本文提出VISTA，一个用于长视频事件预测的多层次事件语义挖掘框架，解决现有长视频语言模型在事件预测任务上泛化能力不足的问题。


<details>
  <summary>Details</summary>
Motivation: 长视频事件预测具有多模态上下文和复杂叙事结构，目前研究不足。现有基于LLM和VLM构建的长视频问答和摘要模型无法精确提取事件相关细节或进行细粒度的事件发展分析，缺乏事件预测能力。

Method: VISTA采用三阶段方法：1) 角色中心视觉提示提取事件相关视觉细节，增强细节级语义；2) 知识增强迭代检索策略引导LLM逐步构建逻辑连贯的事件链，改善事件级叙事；3) 类人提出-检索策略生成多样化未来导向提案并整合多层次线索，产生鲁棒准确的预测。

Result: 在真实世界数据集上进行了广泛实验，验证了VISTA对长视频事件预测的有效性。

Conclusion: VISTA框架能够有效处理长视频事件预测任务，通过多层次语义挖掘策略提升了预测的准确性和鲁棒性。
...
TL;DR: 本文提出TouchSafeBench，一个用于评估视觉语言模型(VLMs)在人机协作中碰撞检测能力的基准测试。该基准包含2,940个模拟室内共处场景，涵盖社会导航和社会重排任务。


<details>
  <summary>Details</summary>
Motivation: 安全的人机协作不仅需要视觉描述，还需要判断机器人身体是否安全分离、已经发生碰撞或即将发生碰撞。作者称这种能力为"碰撞接地"(collision grounding)，即绑定视觉观察与机器人身体几何、相机视角、场景布局、人体接近度和时间运动，以推断当前和即将发生的接触。

Method: 在Habitat 3.0中构建了TouchSafeBench基准，包含2,940个模拟室内共处 episodes，涵盖社会导航和社会重排场景，具有同步的多视角RGB-D观测、俯视轨迹图、校准的相机元数据和模拟器导出的接触标签。研究两个面向部署的任务：分类当前安全状态和预警即将发生的碰撞。评估了三个前沿/机器人导向的VLM和九种视觉表示。

Result: 当前模型仍远未达到可靠水平：最佳平均Macro-F1低于50%，显式深度不会自动转化为机器人身体碰撞证据，机器人-场景接触识别始终比人体接触风险识别更难。

Conclusion: TouchSafeBench揭示了具身VLMs的核心局限性：视觉流利性不等于物理可问责性。可靠的机器人安全监控器需要明确绑定视角、机器人形态、度量几何和未来碰撞的表示。
...
### [56] [Benchmarking and Enhancing Text-to-Image Models for Generating Visual Representations in Early Arithmetic Education](https://arxiv.org/abs/2605.31212)
*Junling Wang,Boqi Chen,Heejin Do,Mubashara Akhtar,April Yi Wang,Mrinmaya Sachan*

Main category: cs.CV

TL;DR: 论文提出了方程-视觉生成（equation-to-visual generation, E2V）任务，旨在让AI系统根据算术方程生成具有教学意义的视觉内容，同时精确保持数值和关系结构。为此构建了E2V-Bench基准，包含四种基于教学的视觉类型及其自动评估指标。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在教育内容创作中的广泛应用，目前尚不清楚这些系统是否能够生成准确表达教学概念的输出。现有图像生成模型在将数学方程转化为教学可视化方面存在明显不足，需要系统性地评估和提升这一能力。

Method: 通过教师访谈和教育材料分析，确定了四种具有教学意义的视觉类型；构建了E2V-Bench基准数据集；开发了视觉正确性自动评估指标；对当前主流的文本到图像（T2I）模型进行了系统性评测；探索了基准引导的增强策略。

Result: 当前T2I模型在该任务上频繁失败，主要错误包括：对象数量不准确和关系结构缺失。基准引导的增强策略对部分模型有所改善，但仍存在显著差距。

Conclusion: 未来T2I模型需要更强的数值和关系 grounding 能力，以更好地支持教育内容生成任务。
...
### [57] [Beyond Classification: Dynamic Adapter Routing for Continual Multimodal Retrieval](https://arxiv.org/abs/2605.31229)
*Alicja Dobrzeniecka,Filip Szatkowski,Sebastian Cygert,Szymon Lukasik,Bartlomiej Twardowski*

Main category: cs.CV

TL;DR: 本文提出持续多模态检索(CMR)的新评估框架，并针对该任务设计动态适配器路由(DAR)方法，结合原型路由选择适配器和模型合并技术。


<details>
  <summary>Details</summary>
Motivation: 检索是视觉语言模型的核心功能，但如何持续更新检索模型仍未被充分探索。现有研究通过类别增量学习视角评估，无法完全捕捉检索任务的独特动态。

Method: 构建了跨不同视觉领域的CMR评估框架；提出DAR方法，基于原型进行适配器路由选择，并通过模型合并整合多个适配器。

Result: 标准CIL方法在挑战性场景中无法带来有意义提升；DAR方法优于基线方法，并在分布外评估中展现出强泛化能力。

Conclusion: 结果突出了CMR任务的独特挑战，为该领域的后续研究提供了新的方向和有效方法。
...
### [58] [Cross-Modal Clinical Knowledge Integration for Mammography Report Generation](https://arxiv.org/abs/2605.31093)
*Jiayi Zhu,Fuxiang Huang,Yu Xie,Xi Wang,Zhixuan Chen,Yuan Guo,Qingcong Kong,Zhenhui Li,Qiong Luo,Hao Chen*

Main category: cs.CV

TL;DR: 本研究提出MammoRG框架，旨在通过模拟放射科医生的临床报告撰写流程，自动生成乳腺X线摄影诊断报告，解决大规模筛查工作带来的放射科医生负担问题。


<details>
  <summary>Details</summary>
Motivation: 乳腺癌是全球主要健康问题，乳腺X线摄影筛查在早期检测中起核心作用。现有的自动化报告生成方法主要关注直接的视觉到文本映射，而忽略了放射科医生在实际临床实践中遵循的结构化临床推理过程。

Method: 提出了MammoRG框架，采用两阶段训练框架：第一阶段通过分类监督学习整合患者四视角乳腺X线片的临床相关先验知识；第二阶段引入术语感知监督微调策略，将乳腺X线摄影特定临床术语建模为原子语义单元。此外开发了MammoRGTool工具用于从自由文本报告中提取结构化临床信息。

Result: 实验表明MammoRG在多个临床疗效指标上始终优于现有方法，特别是在诊断相关的BI-RADS F1分数上，相比第二名模型在内部数据集、外部数据集1、外部数据集2和VinDr-Mammo数据集上分别提升了2.73%、2.04%、1.90%和3.27%。

Conclusion: MammoRG框架能够有效模拟临床报告撰写流程，产生高质量且与临床一致的诊断报告，为减轻放射科医生工作负担和提高乳腺X线摄影筛查效率提供了有价值的解决方案。
...
### [59] [ERGeoBench:A Comprehensive Benchmark for Embodied Reasoning and Geo-localization in Multimodal Large Language Models](https://arxiv.org/abs/2605.31251)
*Kaiwen Xue,Tao Wei,Guoxin Zhang,Zhonghong Ou,Kaoyan Lu,Yu Feng,Yifan Zhu,Haoran Luo*

Main category: cs.CV

TL;DR: ERGeoBench是一个用于评估多模态大语言模型(MLLM)在具身地理定位能力的诊断基准，支持单视图、全景视图和具身视图三种渐进式设置。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在具身智能体方面展现出潜力，但具身地理定位领域因缺乏细粒度评估而尚未得到充分探索。

Method: 构建包含2207个全球分布街景全景图的ERGeoBench基准，在三种设置下评估模型的四个互补能力：基础感知、空间意识、常识推理和地理定位推理。

Result: 当前模型能推断高层地理语义，但在细粒度感知操作、度量定位和跨视图空间一致性方面仍存在困难；地理定位与其他能力维度高度相关。

Conclusion: 准确的定位依赖于整合的感知、空间推理和常识推理，而非孤立的视觉识别；该基准为诊断和推进类人具身地理定位提供了统一框架。
...
### [60] [Envisioning Beyond the Few: Disentangled Semantics and Primitives for Few-Shot Atypical Layout-to-Image Generation](https://arxiv.org/abs/2605.31266)
*Nan Bao,Yifan Zhao,Wenzhuang Wang,Jia Li*

Main category: cs.CV

TL;DR: 提出一种表示驱动的框架解决布局到图像任务在少样本 Atypical 设置中出现的表示碎片化问题，通过将语义与原语分离来实现稳健的少样本适应。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像方法在少样本 Atypical 设置下产生碎片化和扭曲的生成，原因是粒度不匹配导致语义身份与视觉细节纠缠。

Method: 提出三个关键组件：1)语义锚定将类别语义聚合为锚点以保持稳定的身份；2)原语注入建模可组合的原语以实现稳健的局部细节建模；3)概念导向使用显著性感知目标来调节优化，保持前景语义一致性。

Result: 在5-shot设置下，在多样化的 Atypical 领域的视觉保真度和对齐方面均优于最先进的布局到图像方法。

Conclusion: 该框架通过语义与原语的解耦，有效解决了少样本学习中的表示碎片化问题，为布局到图像任务的少样本适应提供了新的研究方向。
...
TL;DR: 本论文提出iVGR框架，旨在解决视觉接地Chain-of-Thought（CoT）在推理阶段强制显式目标框导致性能下降的问题。研究发现强制显式的视觉定位反而会引入不必要的干扰，因此通过强化学习将定位能力内化到文本推理中，使模型能够在不依赖显式接地的情况下准确推理。


<details>
  <summary>Details</summary>
Motivation: 视觉接地CoT已被证明是增强多模态大语言模型细粒度感知能力的有效范式，但在推理阶段的有效性尚未被充分探索。论文通过实证研究发现在推理时强制要求显式目标框往往会降低模型性能，相比不使用显式视觉接地的标准文本CoT表现更差。作者猜测视觉定位能力可以被内化到文本CoT中，而强制显式接地会干扰模型的答案预测目标。

Method: 提出Internalizing Visually Grounded Reasoning（iVGR）框架，采用双流训练策略：一个文本流通过一致性奖励（consistency reward）与高质量的视觉接地流进行对齐训练，从而使模型在不显式使用视觉接地的情况下也能实现准确的目标定位。

Result: 广泛的实验表明，该方法在细粒度基准测试上显著优于已有的基线方法，同时保持了支持工具辅助推理工作流的灵活性。

Conclusion: iVGR成功地将视觉定位能力内化到文本推理过程中，证明了强制显式视觉接地并非必要，通过适当的训练策略可以让模型学习到可迁移的推理能力，为多模态大语言模型的细粒度感知提供了新的研究方向。
...
TL;DR: 提出一种新的域增量学习方法，结合MAE自监督头和LoRA适配器，不避免而是主动利用灾难性遗忘，通过在线测试时训练实现域自适应。


<details>
  <summary>Details</summary>
Motivation: 现有域增量学习方法通常试图避免灾难性遗忘，但作者认为可反过来利用这种遗忘现象处理流式数据（尤其是视频）中自然发生的域漂移。

Method: 模型包含主任务头和MAE头，学习域特定的LoRA适配器，推理时通过在线测试时训练选择最佳匹配的LoRA实现记忆恢复。

Result: 在域增量动作识别和语义分割任务上验证了方法有效性。

Conclusion: 该方法特别适合现实世界的流式视频数据，相邻样本高度相关且域移是渐变的场景。
...
TL;DR: Polyphony是一种用于双-hand动作分割的三阶段方法，旨在从未修剪视频中密集预测双手动作。该方法解决了手间依赖性复杂、视觉不对称、表示冲突和语义模糊等独特挑战。


<details>
  <summary>Details</summary>
Motivation: 双-hand动作分割面临四个关键挑战：1)复杂的手间依赖性；2)双手间的视觉不对称；3)主hand monopolize梯度导致的表示冲突；4)细粒度动作的语义模糊。这些挑战使得同时理解和分割双手活动变得困难。

Method: 提出三阶段方法Polyphony：(1)交替双hand Vision Transformer (ADH-ViT) - 在左右手mini-batch之间交替训练，确保双方平衡的梯度贡献，同时共享时空编码器；(2)语义特征条件化 (SFC) - 将视觉特征与结构化、组合动作描述对齐，增强语义相似动作的区分能力；(3)基于扩散的分割 (DBS) - 结合跨hand特征融合实现手间协调，并采用自适应损失加权平衡性能。

Result: 在双hand数据集(HA-ViD、ATTACH)上达到最先进水平，改进高达16.8点；在单流Breakfast数据集上达到82.5%，优于之前使用12倍更大backbone的最佳方法。统一模型用单一共享backbone超越了需要单独per-hand模型的baseline。

Conclusion: Polyphony成功解决了双-hand动作分割的核心挑战，通过交替训练、语义条件化和扩散分割的创新结合，实现了高效准确的双手动作分割，为理解复杂双手机械活动提供了有效方案。
...
### [64] [Feature-Optimized Vision for Adaptive 3D Scene Reconstruction](https://arxiv.org/abs/2605.31534)
*Eric Liang*

Main category: cs.CV

TL;DR: 提出一种自适应特征优化的视觉前端，根据纹理、可重复性、区分性、预期三角化角度和空间覆盖对候选特征打分，并在固定重建管道下为每视图分配特征预算，以最大化有用轨迹。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定阈值或均匀特征预算，容易在低 parallax、重复纹理或不稳定的点上浪费计算资源，导致重建质量不高且效率低下。

Method: 对每个候选特征计算五个评分维度（纹理强度、可重复性、区分性、预期三角化角、空间覆盖），基于这些评分将全局特征预算分配到各视图；构建四种选择策略并在走廊、外观、物体桌面和杂乱四类场景的合成多视图数据上测试。

Result: 相比随机、仅纹理、均匀网格基线，自适应策略获得最高的质量感知完整性并实现最低的整体重建均方根误差，同时保持广泛的图像覆盖。

Conclusion: 该自适应策略是一个模块化的前端策略，可使传统和学习驱动的三维重建管线更加有针对性地利用计算资源，提升重建质量而非取代现有的匹配或神经重建系统。
...
TL;DR: 本文提出Neural Token Reconstruction (NTR)框架，通过自蒸馏掩码潜在重构目标直接约束感知free自动驾驶中紧凑的场景token瓶颈，增强视觉表示学习而无需推理时的辅助组件。


<details>
  <summary>Details</summary>
Motivation: 现有感知free端到端自动驾驶方法的场景token仅接收规划目标的监督，提供有限的约束导致视觉信息冗余，需要引入辅助监督来增强场景token的表示能力。

Method: NTR引入自蒸馏掩码潜在重构目标，使用紧凑场景token作为重建记忆重建被掩码的patch级潜在特征，强制梯度仅通过场景token bottleneck。同时从基础模型标注引入语义先验作为弱语义偏置。所有辅助重建组件在推理时被移除。

Result: NTR在三个公开自动驾驶基准上达到最先进性能：Waymo E2E上RFS为8.0461，NavSim1&2上PDMS为94.1、EPDMS为90.9。学习到的场景token呈现更低的两两冗余和更高有效秩。

Conclusion: 有效的bottleneck监督同时改善了紧凑视觉表示学习和规划性能，证明直接在场景token层面引入表示学习目标可提升感知free自动驾驶系统的效果。
...
TL;DR: QVGGT是一款针对VGGT(视觉几何Transformer)的量化压缩框架,通过混合精度策略、token过滤补偿和任务感知scale搜索机制,实现近无损的W4A16量化,在边缘设备上实现3~4.9倍内存减少和2.8倍加速。


<details>
  <summary>Details</summary>
Motivation: VGGT虽然能单次前向传播预测相机参数、深度图和点云,但1.2B参数的规模严重限制了其在无人机、移动AR等资源受限设备上的部署,需要一种高效的模型压缩方法。

Method: 1)分析transformerblock的量化敏感度,采用选择性混合精度策略分配更高精度给最脆弱的模块; 2)引入token过滤与PCA全局补偿token处理高方差camera和register tokens; 3)开发任务感知scale搜索机制,通过层重建、多头监督和跨头几何一致性评估量化尺度。

Result: 在多个几何感知基准数据集上实验表明,QVGGT实现近无损W4A16量化,保持所有3D预测头的精度,同时提供3~4.9倍内存减少和最高2.8倍的实际硬件加速。

Conclusion: QVGGT使高保真3D感知在边缘设备上可行,为受限环境中前向3D重建模型的实用化部署铺平了道路。
...
### [67] [TunerDiT: Training-free Progressive Steering of Diffusion Transformer for Multi-Event Video Generation](https://arxiv.org/abs/2605.31590)
*Ruotong Liao,Guowen Huang,Qing Cheng,Guangyao Zhai,Lei Zhang,Xun Xiao,Thomas Seidl,Daniel Cremers,Volker Tresp*

Main category: cs.CV

TL;DR: TunerDiT是一种无需额外训练的多事件视频生成引导方法，通过发现视频扩散变压器中的内在转折点来实现从全局布局到细粒度细节的条件控制。


<details>
  <summary>Details</summary>
Motivation: 文本到视频生成在生成长视野包含多个事件的视频时面临挑战，需要理解如何在不同时间点上控制视频内容的生成。

Method: 提出两种引导策略：事件分区掩码（强制事件边界并允许跨事件过渡带）和跨事件提示融合（为后期细化注入相邻事件语义）。

Result: 在8个指标上达到最佳性能，提供视频一致性与事件分离之间的可调权衡，文本对齐改进随事件数量增加而增加。

Conclusion: 该研究揭示了DiT去噪轨迹中的内在转折点，为多事件视频生成提供了有效的无训练引导方法，具有良好的扩展性。
...
### [68] [Lumos-Nexus: Efficient Frequency Bridging with Homogeneous Latent Space for Video Unified Models](https://arxiv.org/abs/2605.31603)
...
TL;DR: 本文提出Lumos-Nexus，一个训练高效的统一视频生成框架，旨在解决将高保真生成器集成到统一训练循环中的计算成本问题。该框架采用两阶段设计，在训练时仅对齐轻量级生成器与理解块，在推理时引入统一渐进频率桥接(UPFB)将生成任务转移到预训练的高容量生成器。同时论文还提出了VR-Bench基准来评估模型的推理驱动生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于连接器的视频统一模型在指令引导的视频合成方面表现出色，但将大型高保真生成器集成到统一训练循环中在计算上不可行，限制了可实现的视觉质量提升。

Method: 1) 两阶段训练设计：训练时仅让轻量级生成器与理解块对齐，学习接收推理驱动的语义控制；2) 推理时引入统一渐进频率桥接(UPFB)：在共享潜在空间中逐步将生成任务交接给高容量预训练生成器，实现粗到细的细化；3) 提出VR-Bench基准评估推理驱动视频生成能力。

Result: Lumos-Nexus在VBench上实现了视觉真实感和时间一致性的显著提升，同时在VR-Bench上展现出强大的推理驱动生成性能。

Conclusion: 该框架成功地在保持推理质量的同时大幅提升了视觉保真度，为高效训练高质量视频统一生成模型提供了可行的解决方案。
...
TL;DR: Light Interaction是一个无需训练的交互式视频世界模型推理加速框架，通过自适应上下文管理、去噪缓存加速和3D块稀疏注意力实现高达2.59倍的加速。


<details>
  <summary>Details</summary>
Motivation: 交互式视频世界模型在生成长轨迹时面临高昂的计算成本，包括context memory增长、二次方注意力复杂度和重复的去噪步骤。

Method: 结合三大技术：1) 自适应上下文管理(动态调整spatial memory和temporal context)；2) 去噪缓存加速(重用早期模型输出)；3) 硬件软件协同设计的3D块稀疏注意力与Triton融合核。

Result: 在HY-WorldPlay和Matrix-Game-3.0数据集上实现最高2.59倍加速，同时保持有竞争力的视觉质量。

Conclusion: 该框架无需模型重训练即可显著提升交互式视频世界模型的推理效率，为实时游戏模拟、虚拟场景导航和具身AI训练等应用提供了更高效的解决方案。
...
TL;DR: 本文提出了一种无需训练的图像编辑框架，针对无条件扩散模型中的全局低层次美学变换问题，通过提取降解概念向量结合瓶颈补丁与无分类器引导，实现从退化流形中引导采样，显著提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 无条件扩散模型虽能提供强大的生成先验，但现有训练-free编辑方法（如h空间补丁）在处理全局性、低层次的感知美學优化时存在系统性缺陷，无法有效实现美学增强。

Method: 提出一种推理时机制：1）提取降解概念向量；2）结合瓶颈补丁与无分类器引导；3）在低层特征空间操作，引导采样远离退化流形，无需模型微调。

Result: 实验表明该方法能在不重训练模型的情况下持续生成质量提升的图像，在美学和感知评估中取得一致改善。

Conclusion: 该框架为无条件扩散模型的图像编辑开避了新途径，验证了通过低层特征操作实现全局美学优化的可行性，为生成式AI的推理时编辑提供了新思路。
...
TL;DR: 本文提出DetAS，一个基于多模态大语言模型(MLLM)的智能目标检测框架，将目标检测形式化为动态决策过程。该框架由自适应图像恢复和多专家检测两个关键组件构成，并通过自我进化经验收获机制扩展为DetAS-X，可在推理过程中进行经验感知推理。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的目标检测面临图像退化和目标分布异质性带来的挑战，现有检测器泛化能力受限。传统方法依赖预定义条件，缺乏对动态环境的适应性，亟需一种更加灵活、自适应的检测范式。

Method: DetAS框架以MLLM为核心智能体，从工具箱中选择合适的恢复模块和专用检测器来动态组合检测流程。(1)自适应图像恢复：动态判断是否及如何增强图像；(2)多专家检测：整合多个领域专用检测器，通过实例级推理解决预测冲突；DetAS-X进一步引入自我进化经验收获，从少量标注数据中积累节点级决策经验，使系统能渐进优化决策策略。

Result: 在6个具有挑战性的基准数据集上开展广泛实验，DetAS-X显著优于现有基于MLLM的检测器，平均F1分数提升28.36%，在DarkFace数据集上最高提升37.01%。

Conclusion: 研究验证了智能体检测的巨大潜力，为复杂动态环境下的实际应用奠定了坚实基础。
...
TL;DR: 本文提出VaViT方法，成功将vanilla（非层级）Vision Transformer应用于大规模自动驾驶激光雷达点云语义分割，通过精心设计的tokenizer、轻量级decoder和定制数据增强填补性能差距，在nuScenes、SemanticKITTI和Waymo Open Dataset数据集上达到与SOTA方法相当或更优的性能。


<details>
  <summary>Details</summary>
Motivation: 当前点云语义分割的SOTA架构仍由U-Net主导（卷积与注意力机制交织），而Plain Transformers已在多模态学习（文本、音频、图像、视频）中成为标准骨干。文章旨在探索能否像NLP/图像领域一样，用简洁的vanilla ViT架构处理点云分割任务。

Method: 核心创新包括三部分：1) 精心设计的tokenizer将点云转化为ViT可处理的token序列；2) 轻量级decoder分割头；3) 针对点云任务的定制数据增强策略。通过这些组件弥补vanilla ViT在处理大规模3D点云时的性能差距。

Result: VaViT方法在nuScenes、SemanticKITTI和Waymo Open Dataset三个主流点云分割数据集上验证有效，性能匹配或超越现有SOTA方法，同时保持了ViT架构的简洁性。

Conclusion: 研究表明vanilla ViT架构经适当设计后可有效用于点云语义分割，为该领域提供了一种统一、简洁的替代方案，代码和模型已开源。
...
TL;DR: 本文提出SPUNA框架，利用正类无标签(PU)学习和流形结构检测视觉系统中的协变量偏移，在弱监督下实现与全监督方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 构建可靠视觉系统的关键挑战：显式检测协变量偏移。现有方法需完全监督训练且需要原分布和偏移分布的标记样本，不切实际。偏移数据重叠导致经典PU方法不稳定。

Method: 提出光谱PU邻域注释(SPUNA)：一种几何感知的框架，通过利用视觉特征的局部流形结构逐步发现偏移数据，无需大量标注样本。

Result: 在PU设置下达到最先进性能，显著匹配全监督方法的性能，对不同类型偏移具有强泛化能力。

Conclusion: 协变量偏移检测可通过弱监督PU学习有效解决，SPUNA为实际应用提供了可行方案，性能接近全监督方法并具良好迁移性。
...
TL;DR: 本文提出利用多模态大语言模型(MLLMs)结合语言作为正则化机制，同时解决深度伪造检测的泛化性和可解释性两大难题。通过双编码器架构和两阶段训练课程，在仅使用二分类标签的情况下，实现了对模型推理过程的奖励，从而提升了对高层特征的使用。


<details>
  <summary>Details</summary>
Motivation: 当前深度伪造检测器普遍存在两个关键问题：1)容易过拟合于数据集特定的低层 artifacts，缺乏跨数据集泛化能力；2)缺乏可解释性，难以理解模型的决策依据。作者的直觉是：预训练于语言的大模型会自然倾向于使用高层次的、可描述的特征，这些特征通常具有更好的泛化性。

Method: 方法采用双编码器架构，将冰冻的专用检测器与LoRA调优的MLLM编码器配对。两阶段训练课程：第一阶段为二元对齐阶段，展示MLLMs结合特征来减轻对特定artifacts过拟合的能力；第二阶段使用强化学习，鼓励模型在分类前生成描述性推理，仅使用二分类标签进行奖励，通过奖励"先解释后分类"的行为，明确激励模型优先使用高层次的鲁棒特征。

Result: 该方法在基准数据集上进行了广泛实验，取得了显著的跨数据集性能提升，即使在推理链被省略的推断阶段也能保持性能提升。实验结果表明，该方法不仅实现了可解释性（生成描述性推理），还进一步提升了跨数据集的泛化性能，大幅超越现有最先进方法。

Conclusion: 本研究成功证明了通过语言作为正则化机制，可以同时实现深度伪造检测的泛化性和可解释性。dual-encoder架构结合RL阶段训练的方案有效地激励模型优先使用高层特征，在仅用二分类标签的情况下实现了可解释的推理过程输出，为构建更鲁棒、可解释的深度伪造检测系统提供了新的思路。
...
TL;DR: 本文提出FREUD模型，一种基于 rectified flow transformer 的帧式编码器-统一解码器框架，用于高效压缩时空天气数据，实现降水临近预报。通过不确定性保留的第一阶段编码器捕捉 aleatoric 不确定性，特别有利于极端天气事件。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的天气临近预报方法依赖确定性压缩来降低高维天气数据的复杂度，这限制了捕捉解码过程中不确定性的能力。极端天气事件具有高解码变异性，需要更好的不确定性建模方法。

Method: 提出FREUD框架，包含：(1) 帧式编码器实现连续预报更新；(2) 统一视频解码器确保时序一致性；(3) 不确定性保留的第一阶段通过集成方式捕捉 aleatoric 不确定性。使用compact latent-space rectified flow transformer在SEVIR基准数据集上进行评估。

Result: 在SEVIR基准数据集上实现降水临近预报的state-of-the-art性能，通过模型尺度和测试时缩放进一步提升性能。

Conclusion: FREUD成功地将不确定性建模融入扩散模型框架，为极端天气条件下的安全关键预报提供了更可靠的方法。该工作为 weather nowcasting 领域树立了新的baseline，并展示了rectified flow transformer在高效时空数据压缩方面的潜力。
...
### [76] [Latent Geometric Chords for Query-Efficient Decision-Based Adversarial Attacks](https://arxiv.org/abs/2605.31219)
*Ei Hmue Khine,Yao Li,Jiebao Sun,Shengzhu Shi,Zhichang Guo,Boying Wu*

Main category: cs.CV

TL;DR: 本文提出Latent Geometric Chords (LGC)方法，用于高效查询的决策式黑盒对抗攻击。该方法在压缩语义流形中执行曲率感知几何搜索，并通过残差对抗生成(RAG)机制将语义扰动叠加到原始图像上，解决了现有方法在视觉保真度和搜索空间方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有决策式黑盒对抗攻击存在根本性局限：像素级攻击产生不自然的高频视觉伪影，而潜在空间框架受限于低维流形的有限搜索空间和固有重建缺陷。作者认为需要一种新方法来提高视觉保真度同时扩大搜索空间维度。

Method: 提出LGC方法核心包括两部分：1)曲率感知几何搜索：在压缩语义流形中导航决策边界；2)RAG机制：将语义扰动分离为几何弦并直接叠加到原始源图像上，有效解决基线重建缺陷并将允许的搜索空间维度加倍。

Result: 实验表明LGC实现了强大的跨数据集迁移性，显著优于最新基线方法。在5000次查询时达到SSIM>0.99和LPIPS<0.01的最优视觉保真度，同时保持高攻击成功率，能够成功攻破对抗训练鲁棒模型。

Conclusion: LGC通过几何流形搜索和残差生成机制有效解决了决策式对抗攻击中的核心挑战，在最小化扰动幅度的同时实现了卓越的视觉质量，为黑盒对抗攻击研究提供了新的有效范式。
...
TL;DR: 该论文提出HiERO-StepG方法，用于从无标注的人类活动视频中自动识别程序性步骤（如烹饪或修车）。方法基于HiERO的弱监督表示学习，通过在特征空间中聚类检测步骤，无需任务特定微调。在Ego4D Step Grounding挑战中达到56.27%的R@1(IoU=0.3)得分，排名第二，且完全零样本，无需程序特定标注。


<details>
  <summary>Details</summary>
Motivation: 传统步骤定位方法需要大量人工标注，可扩展性差。研究者认为程序步骤的层次结构可以从无标注的人类活动视频中通过共现动作的模式自然涌现出来，从而实现零样本学习。

Method: 基于HiERO弱监督表示学习方法构建，该方法利用细粒度动作叙述将功能相关的动作映射到接近的特征空间。通过简单聚类检测步骤，无需额外任务特定微调。具体增强包括：确保粗细粒度步骤赋值的一致性、强制实施严格的时间单调性后处理检测到的步骤以减少噪声预测的影响。

Result: 在全球排行榜上达到56.27%的R@1(IoU=0.3)指标，提交时排名第二，且完全零样本，不需任何程序特定标注。

Conclusion: 该方法成功展示了从无标注视频中进行零样本步骤定位的可行性，为程序理解任务提供了一种可扩展的新思路。
...
### [78] [DriveMA: Driving Vision-Language-Action Models with verifiable Meta-Actions](https://arxiv.org/abs/2605.31271)
*Weicheng Zheng,Yixin Huang,Qiao Sun,Derun Li,Hang Zhao*

Main category: cs.CV

TL;DR: DriveMA是一个基于可验证元动作的驾驶视觉语言动作模型框架，通过元动作接口弥合语言与动作之间的差距，在Waymo开放数据集上实现了8.079的评分新高。


<details>
  <summary>Details</summary>
Motivation: 驾驶VLMs旨在使用语言改进端到端规划，但语言-动作差距限制了这一潜力。现有方法缺乏将高层语言意图与低层轨迹规划有效对齐的机制。

Method: 提出可验证元动作框架：1)设计轨迹接地标注流程从专家轨迹构建元动作；2)通过基于规则的投影验证生成轨迹；3)采用动作中心监督学习和数据高效的分回合信用分配强化学习，使用密集奖励和精确信用分配对齐高层决策与低层轨迹规划。

Result: 在Waymo开放数据集视觉端到端驾驶任务上创下新SOTA：2B模型达8.060分，4B模型进一步提升至8.079分；在NAVSIM上获得竞争力的闭环规划性能。

Conclusion: 研究表明，即使简单的元动作接口，只要实现可验证并针对语言-动作对齐优化，也能实现最优规划性能，为驾驶VLA研究提供了有效的语言-动作桥接方案。
...
### [79] [Topologically Consistent Multi-view 3D Head Reconstruction via Coarse-Guided Layered Surface Sampling](https://arxiv.org/abs/2605.31283)
*Timo Bolkart,Daoye Wang,Prashanth Chandran*

Main category: cs.CV

TL;DR: SHELLS是一种高效的3D头部重建前馈框架，通过分层局部采样实现密集语义对应。该方法利用DINOv2 backbone提取多视图特征，解耦特征提取与网格分辨率，在保持表面一致性的同时显著降低内存消耗和推理时间。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过局部特征体积独立细化顶点，这种方法将内存密集的特征采样与网格分辨率耦合，限制了密集拓扑的可扩展性（超过10k顶点）并引入表面噪声。因此需要一种解耦特征提取与网格分辨率的高效方法。

Method: 1)使用DINOv2 backbone配合LoRA适配器提取多视图特征；2)投影采样稀疏全局特征云并预测中间粗网格；3)构建分层、表面感知的采样壳作为最终重建的离散搜索空间。

Result: 相比体积基线方法，SHELLS减少88%的推理GPU内存（2.4GB vs 20GB）；18k顶点网格的中值配准误差降低21%-29%，推理速度提升3.5倍（0.08s vs 0.29s）。模型仅在合成数据上训练就能有效泛化到真实世界捕获。

Conclusion: SHELLS成功实现了高效、可扩展的3D头部重建，无需昂贵的预注册多视图数据集即可泛化到真实场景，为密集语义对应研究提供了新的思路。
...
### [80] [Authentication of Copy Detection Patterns via Cross-Camera Dual-Synthetic Referencing](https://arxiv.org/abs/2605.31292)
*Ivan Oleksiyuk,Roman Chaban,Slava Voloshynovskiy*

Main category: cs.CV

TL;DR: 针对Copy Detection Patterns(CDPs)的克隆检测问题，提出基于注册的双相机双合成参考框架，结合数字模板和注册捕获图像生成高质量参考图像，提升验证鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略了打印变异性，且仅通过合成参考图像处理相机效果，难以应对打印机随机性和相机失真对CDP比较的干扰，导致防伪鲁棒性不足。

Method: 引入注册阶段由受控注册相机捕获打印的CDP，利用深度学习翻译器联合利用数字模板和注册捕获生成验证图像的高质量双重参考，并提供信息论证明说明双重参考比纯模板参考更具信息量。

Result: 实验在异构移动相机上表明：认证性能提升、对机器学习克隆攻击具有鲁棒性、从小型CDP区域和低设备可可靠验证。

Conclusion: 所提框架有效解决了CDP验证中的打印和相机变异性问题，在实际场景中具有良好应用前景。
...
### [81] [TokTalk: Expressive Real-time Facial Animation from Audio-LLM Tokens](https://arxiv.org/abs/2605.31294)
*Qingcheng Zhao,Yifang Pan,Karan Singh*

Main category: cs.CV

TL;DR: TokTalk是一个直接利用Audio-LLM生成的音频令牌实时驱动3D面部动画的系统，解决传统级联管道（ASR→文本生成→TTS→面部渲染）的延迟和表现不自然问题。


<details>
  <summary>Details</summary>
Motivation: 当前对话式头像在面部表情和对话流程上仍显机械，主要原因是采用顺序处理：语音识别、文本生成、轮询响应、语音合成和音频驱动面部动画。研究者发现Audio-LLM产生的音频令牌携带足够信息来重建面部表演，因此提出直接用音频令牌驱动面部动画的方案。

Method: 构建新型音频令牌到3D面部运动数据集；使用基于Chunk的条件流匹配模型进行训练；采用轻量级适配策略使模型可连接任何基于令牌的Audio-LLM；基于chunk的处理实现延迟与面部质量的参数化权衡。

Result: 实时性能与现有方案相当；感知研究表明质量、表达力和3D面部表演控制方面显著更优；展示了三类应用场景：聊天机器人Avatar、声音驱动用户Avatar、动画导演界面。

Conclusion: TokTalk成功实现了从Streaming音频令牌直接生成expressive面部动画，延迟可控，为多样化的音频-视觉面部应用提供灵活解决方案。
...
### [82] [Learning from Fine-Grained Visual Discrepancies: Mitigating Multimodal Hallucinations via In-Context Visual Contrastive Optimization](https://arxiv.org/abs/2605.31312)
*Haolin Deng,Xin Zou,Zhiwei Jin,Chen Chen,Haonan Lu,Xuming Hu*

Main category: cs.CV

TL;DR: 本文提出IC-VCO方法，通过在共享多图像上下文中的对比优化来解决视觉语言模型中的多模态幻觉问题，并引入VCDist正则化和对比样本编辑策略。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型中多模态幻觉问题，传统文本DPO缺乏显式视觉监督，现有视觉偏好DPO存在分区函数不匹配和粗粒度负样本导致捷径学习的理论缺陷。

Method: 提出IC-VCO在共享多图像上下文放置对比图像保证数学严格目标；引入VCDist辅助可靠性门控正则器鼓励多图像对比训练与单图像推理的一致性；提出对比样本编辑策略通过精确语义扰动生成硬负样本。

Result: 在五个基准数据集上展示IC-VCO获得最佳整体性能，验证了样本编辑策略的有效性。

Conclusion: IC-VCO通过严格的数学目标和创新的对比学习框架有效解决了多模态幻觉问题，为视觉语言模型的对齐训练提供了新思路。
...
### [83] [DecMem: Towards Minute-Long Consistent World Generation with Decoupled Memory](https://arxiv.org/abs/2605.31336)
*Zhenhao Yang,Xiaoshi Wu,Zhengyao Lv,Xiaoyu Shi,Xintao Wang,Pengfei Wan,Kun Gai,Kwan-Yee K. Wong*

Main category: cs.CV

TL;DR: 提出DecMem，一种解耦记忆架构，通过稀疏全局记忆和锚定局部记忆实现长时序、高保真、可控的视频生成，解决传统学习记忆在长程推理中的计算效率低和注意力分散问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型在长时序推理中难以保持细粒度的时空一致性，传统显式3D记忆和粗粒度帧级隐式建模存在计算效率低和注意力分散的根本性局限。

Method: 通过系统分析注意力分散问题，提出解耦记忆架构DecMem，包含稀疏全局记忆（非昂贵方式访问全局历史）和锚定局部记忆（稳定高质量的外推），实现高效精确的长期记忆和卓越的外推能力。

Result: DecMem显著优于当前最先进的方法，实现了分钟级可控长视频生成，具有高保真度和一致性。

Conclusion: DecMem通过解耦设计成功解决了长程视频生成中的效率和一致性问题，为分钟级长视频的可控生成提供了新的技术路径。
...
### [84] [Triangle Splatting SLAM](https://arxiv.org/abs/2605.31419)
*Nicholas Fry,Eric Dexheimer,Kirill Mazur,Paul H. J. Kelly,Andrew J. Davison*

Main category: cs.CV

TL;DR: 提出首个利用可微三角形 splatting 的密集 RGB-D SLAM 系统，通过将三角片集合在线转换为连通网格，实现实时追踪、建图和基于网格的场景编辑。


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting 在新视角合成中表现优异，但三角形仍是传统渲染硬件、游戏引擎和需要显式几何的下游任务（仿真、碰撞检测、编辑）的标准基元。此前离线方法已证明无结构"三角汤"可通过 Delaunay 三角化优化为逼真网格，因此有必要探索在线密集 SLAM 中的三角形表示方案。

Method: 构建首个基于 Triangle Splatting 的密集 SLAM 系统，通过对三角片集合进行在线可微渲染同时完成跟踪与建图。地图可通过受限 Delaunay 三角化在线转换为连通网格，支持 mesh 变形和碰撞检测等实时应用。

Result: 在 Replica 和 TUM-RGBD 数据集上，该系统在 3D 几何精度上超越基线方法，相机跟踪精度与基线相当，并实现了在线基于网格的场景编辑功能。

Conclusion: 证明了基于三角形的密集 SLAM 的可行性，为在线网格变形、碰撞检查等新应用开辟了道路，是将可微渲染应用于机器人感知的重要进展。
...
### [85] [YARD: Y-Architecture Register Decoding for Efficient Hallucination Mitigation in Large Vision-Language Models](https://arxiv.org/abs/2605.31429)
*Ting Chen,Geng Li,Guohao Chen,Yu Hu,Guan Huang,Mai Chen,Langsheng Lei,Jun Du*

Main category: cs.CV

TL;DR: YARD提出了一种无需训练的Y形架构寄存器解码方法来缓解大型视觉语言模型中的幻觉问题，通过在中间decoder层分支，并使用保留全局图像语义但缺乏局部细节的寄存器tokens替换patch级视觉tokens，实现 faithful的对比信号同时避免双倍推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有训练-free的对比解码方法存在缺陷：完全丢弃视觉tokens过于极端导致语言幻觉，腐败输入图像方法控制粗糙且需要两次完整前向传递导致高推理延迟。研究者观察到可靠的text-to-vision grounding主要出现在中间decoder层，由此启发YARCH设计。

Method: YARD采用Y形架构在浅层计算共享后在关键中间层分支。退化分支使用register tokens替换patch-level视觉tokens，这些tokens保留全局图像语义但缺乏细粒度局部证据。这种图像感知但局部under-grounded设计提供faithful对比信号而无需极端模态不匹配，同时严格避免昂贵的第二次前向传递。

Result: 在生成性和判别性幻觉基准测试上的广泛实验表明，YARD在多个LVLMs上持续实现SOTA的幻觉缓解效果，同时显著降低推理延迟。

Conclusion: YARD通过创新的Y-architecture和register tokens设计，成功在保持高效单次前向传递的同时为对比解码提供优质的退化分支，在幻觉抑制和推理效率方面均取得显著改进，为LVLMs的实际部署提供了有效解决方案。
...
### [86] [Astra: a generalizable report generation foundation model for 3D computed tomography](https://arxiv.org/abs/2605.31437)
...
TL;DR: Astra是一个用于CT报告自动生成的基础模型，在90,678对胸腹部CT-报告数据集上训练，可处理8个器官系统。通过强化学习统一报告风格和诊断一致性，在CTRgDB和6个外部队列中实现最先进的性能，诊断指标平均提升44.1%，并能加速临床报告撰写和提高完整性。


<details>
  <summary>Details</summary>
Motivation: 当前CT报告依赖放射科医生手动审阅数百张切片，耗时且高度依赖专业知识。现有CT报告生成方法缺乏可泛化的基础模型，无法支持多区域报告且在外部真实世界队列中不够稳健。不同机构报告风格和诊断术语的不一致导致联合训练容易产生噪声监督，限制模型泛化能力。

Method: 构建CTRgDB数据集（包含90,678对CT-报告和353,671个异常标注）；通过风格协调模块统一报告风格；使用强化学习进一步提升诊断一致性；模型支持多区域报告并保持跨机构泛化能力。

Result: 在CTRgDB和6个外部队列评估，Astra在细粒度诊断指标上平均提升44.1%（P<0.001）；实际临床应用中，胸片报告撰写速度提升29.6%，腹部报告完整性提高11.3%（P<0.001）；同时可作为CT AI开发的基础，提升下游诊断性能。

Conclusion: Astra作为一个可广泛访问的临床助手和下一代AI驱动医疗的关键基础设施，成功实现了在多样化和真实世界环境中的可泛化CT报告生成，为医学影像AI领域提供了重要的基座模型。
...
### [87] [VisionPulse: Dynamic Visual Sparsity for Efficient Multimodal Reasoning](https://arxiv.org/abs/2605.31457)
*Hengbo Xu,Shengjie Jin,Yanbiao Ma,Zhiwu Lu*

Main category: cs.CV

TL;DR: 随着大型多模态模型的快速发展，推理时间开销成为实际部署的关键瓶颈。现有的方法通常在预填充阶段修剪视觉token，但本文发现视觉证据是强步骤依赖的，每个解码步骤只有稀疏的视觉token是关键的，且关键集合会随着推理演变。针对这一问题，提出了VisionPulse，一种在推理过程中逐步修剪视觉token的框架。


<details>
  <summary>Details</summary>
Motivation: 大型多模态模型(LMMs)的推理时间开销成为实际部署的关键瓶颈，需要找到高效的方法来减少推理成本

Method: 提出VisionPulse框架，通过计算轻量级的视觉注意力质量来估计步骤-wise的保留预算，利用其与LMMs有效视觉token使用的强正相关性，在预算下仅保留最关键的token

Result: VisionPulse每步仅保留5%的视觉token，推理轨迹缩短11.2%，同时保持精度几乎不变

Conclusion: 通过在推理过程中强制视觉稀疏性，VisionPulse过滤冗余视觉上下文同时保留相关视觉证据，自然缩短推理轨迹
...
### [88] [VolFill: Single-View Amodal 3D Scene Reconstruction with Volumetric Flow Matching](https://arxiv.org/abs/2605.31466)
*Tuan Duc Ngo,Chuang Gan,Evangelos Kalogerakis*

Main category: cs.CV

TL;DR: VolFill是一个用于从单张RGB图像重建完整3D场景的生成式框架，通过混合3D VAE压缩稀疏截断无符号距离场并结合潜在扩散Transformer去噪，在几何Foundation模型的条件下实现鲁棒的场景-complete推理。


<details>
  <summary>Details</summary>
Motivation: 从单张RGB图像重建完整场景几何仍然具有挑战性，尤其是在可见证据不完整的区域（隐藏结构）。现有方法受限于每光线约束或非结构化点云查询，无法提供支持大规模表面提取和占用查询的结构化表示。

Method: 采用混合3D VAE将稀疏截断无符号距离函数（TSDF）网格压缩到紧凑的潜在空间，然后使用潜在扩散Transformer（Latent Diffusion Transformer）对该表示进行去噪以恢复完整场景。以几何Foundation模型为条件，利用丰富的空间先验进行推理。

Result: 在SCRREAM和NRGB-D数据集上的广泛实验表明，该方法显著优于当前基线方法，提供了强大的整体空间理解基础。

Conclusion: VolFill能够提供结构化表示，支持直接表面提取和大规模占用查询，为整体空间理解提供了鲁棒的基础。
...
### [89] [Enhancing Computer Vision Model Generalization in Warehouse Facilities: A Case Study on Anomaly Detection in Vertical Material Handling Systems](https://arxiv.org/abs/2605.31487)
*Ruiliang Liu,Tina Dongxu Li,Joshua Migdal,Ken Meszaros,Trevor Dardik*

Main category: cs.CV

TL;DR: 本研究提出一种创新的仓库计算机视觉模型部署方法，通过在实验室环境中完成标准流程（包括相机放置、图像触发、模型选择和集成），实现了模型从实验室到实际仓库环境的有效泛化，从而简化了仓库自动化部署流程，节省资源和时间。


<details>
  <summary>Details</summary>
Motivation: 传统仓库计算机视觉部署需要大量资源，包括相机安装、图像收集、注释、训练和部署，且每个新环境都需要重复该过程。本研究旨在探索一种方法，仅在实验室完成标准流程，即可泛化到实际仓库环境，解决资源消耗大和重复工作的问题。

Method: 通过结合最佳相机放置位置、战略性图像触发机制、谨慎的模型选择以及模型集成技术，在实验室环境中针对垂直物料处理系统和叉子异常检测进行系统研究和实验验证。

Result: 实验表明所提出的综合方法能够实现从实验室条件到多样化仓库设施环境的有效泛化，仅需简单步骤（相机安装、图像收集、模型部署）即可完成部署。

Conclusion: 该方法有望变革仓库自动化实施，显著简化部署流程并节省资源。但作者指出这是实验性研究，非生产部署，实际应用仍需进一步验证。
...
### [90] [How can embedding models bind concepts?](https://arxiv.org/abs/2605.31503)
*Arnas Uselis,Darina Koishigarina,Seong Joon Oh*

Main category: cs.CV

TL;DR: 研究视觉语言模型（如CLIP）在多对象场景中进行概念绑定（确定颜色对应哪个形状）的能力。研究发现：虽然CLIP的图像和文本嵌入可以分别恢复对象信息，但其绑定函数复杂度高，难以泛化到未见过的概念组合。然而，通过从头训练的受控Transformer模型，当数据覆盖充分时，可以学习低复杂度的乘法绑定函数，实现系统性泛化。


<details>
  <summary>Details</summary>
Motivation: 解决视觉语言模型的概念绑定问题。CLIP等模型能识别单个概念，但无法表示哪些概念组成哪个对象，在跨模态检索中表现为"词袋"模型，无法泛化到新的概念组合。

Method: 通过绑定函数分析：将概念映射到场景嵌入。研究CLIP的绑定函数复杂度；训练受控Transformer模型进行对比实验；分析加性分解特性和乘法交互作用。

Result: CLIP的场景嵌入可加性分解为对象表示，单模态探针可恢复对象信息。但CLIP绑定函数复杂度高，阻止泛化。而受控Transformer模型在充分数据覆盖下学会低复杂度的乘法绑定函数，实现系统性泛化。

Conclusion: CLIP的绑定局限性并非基础性缺陷。通过适当训练数据，受控Transformer模型可学习低复杂度绑定函数，实现跨模态泛化。这表明概念绑定的泛化问题是可解决的。
...
TL;DR: 视频对象中心学习(OCL)通过slot向量表示对象并在帧间保持一致性。现有方法依赖Slot-Slot Contrastive (SSC)损失函数，但需要帧间对象一一对应。本文提出xSSC方法，通过两种零开销机制将时序一致性隐式编码：1)Chrono-Channel Decomposition (CCD)将slot表示分解为静态和动态子空间；2)Cross-Temporal Reconstruction (CTR)通过重建融合实现时序一致性的自监督学习。


<details>
  <summary>Details</summary>
Motivation: SSC损失依赖帧间对象的一一对应关系，且引入额外监督信号。作者遵循奥卡姆剃刀原则，认为时序一致性应作为隐式模型设计而非显式损失来实现，从而提高训练效率并简化模型。

Method: 提出两种协同机制：(1)CCD将slot表示沿通道维度解耦为静态子空间和动态子空间，作为信息瓶颈；(2)CTR使用标准OCL解码器，通过融合当前slot的静态通道和目标slot的动态通道，随机重建当前或前一时间步的目标特征，仅通过标准重建误差学习时序一致性。

Result: 集成xSSC到领先基线模型后，在视频对象发现和识别任务上建立新的state-of-the-art，同时提高训练效率。PCA和梯度分析证实对象的时不变语义和时变运动学分别编码到所提的子空间中。

Conclusion: xSSC成功证明了时序一致性可以作为隐式模型设计被学习，无需显式的对比损失。这一范式转变不仅简化了训练流程，还通过CCD和CTR两种零开销机制提升了性能，为视频对象中心学习提供了更优雅的解决方案。
...
TL;DR: 本研究提出了一种端到端的深度学习pipeline，用于术前基于CT影像预测胰腺切除术后胰瘘(POPF)风险。研究测试了CNN3D、R(2+1)D ResNet-18和ResNet-MC3-18等多种3D CNN架构，在自动分割的胰腺体积数据集上评估，显示出了良好的预测性能，为胰腺手术的术前决策提供了有价值的工具和方法学基准。


<details>
  <summary>Details</summary>
Motivation: 胰腺术后胰瘘(POPF)是胰腺切除术后最严重的并发症之一，会显著增加患者发病率、住院时间和医疗费用。目前缺乏有效的术前预测方法来识别高风险患者，因此需要开发一种自动化、准确的术前风险评估工具来改善外科手术决策。

Method: 研究团队构建了一个端到端的深度学习流程，包括：(1)使用自动分割方法从术前CT扫描中提取胰腺体积；(2)训练多种3D CNN架构进行分类预测。具体测试的模型包括：自定义轻量级3D CNN(CNN3D)、R(2+1)D ResNet-18和ResNet-MC3-18。模型的输入为术前CT图像，输出为POPF风险分层结果。

Result: 研究结果显示，多种3D架构都展现出了有前景的预测性能，表明基于术前CT的深度学习方法可以有效预测POPF风险。该方法为胰腺特异性CT分类提供了一个临床上有价值的工具和方法学基准。

Conclusion: 基于术前CT的深度学习方法有望成为胰腺手术前风险评估的有效工具，可帮助外科医生在术前识别高POPF风险患者，从而优化手术决策和患者管理。未来仍需进一步验证模型的可推广性和临床实用性。
...
### [93] [EGOSTREAM: A Diagnostic Benchmark for Streaming Episodic Memory in Egocentric Vision](https://arxiv.org/abs/2605.31557)
*Rosario Forte,Giuseppe Lando,Antonino Furnari*

Main category: cs.CV

TL;DR: 提出了EGOSTREAM，一个用于自我中心视觉中流式情景记忆评估的诊断基准，包含2250个涵盖七个认知维度的问题，并引入答案有效性窗口(AVW)将问题扩展为8528个recall条件评估，以控制从瞬时到超长期记忆的测试。


<details>
  <summary>Details</summary>
Motivation: 当前流式视频基准缺乏诊断模型记住什么以及记得多久的工具，无法区分真正的模型遗忘与自然世界状态变化，这限制了自主智能体在动态真实环境中的连续情景记忆能力评估。

Method: 构建了统一的流式多模态大语言模型框架，使用Qwen3-VL作为统一骨干，比较了多种记忆管理机制：滑动窗口、注意力sink、KV-cache剪枝、合并和卸载。设计了答案有效性窗口(AVW)来指定答案保持有效的时间跨度，将问题展开为8528个recall条件评估。

Result: Token剪枝比token合并能更好保留细粒度细节和时间结构，量化卸载能挽救超长期记忆。所有机制运行均低于实时性能(>1s/帧)，最佳方法准确率仅约45%，聚合准确率掩盖了不同的记忆特征差异。

Conclusion: 当前架构存在关键差距，所有方法准确率远低于实际应用需求，需要EGOSTREAM提供的诊断测试环境来缩小这些差距。
...
### [94] [nuReasoning: A Reasoning-Centric Dataset and Benchmark for Long-Tail Autonomous Driving](https://arxiv.org/abs/2605.31572)
...
TL;DR: nuReasoning是一个大规模真实世界的自动驾驶推理数据集和基准测试，包含20,000个20秒的片段，涵盖空间推理、决策推理和反事实推理，支持推理评估和规划评估。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶数据集和基准主要针对感知、预测或规划，缺乏对长尾场景中推理的监督。自动驾驶在长尾场景中需要应用常识知识、理解空间关系、推断智能体交互并做出安全决策，因此需要一个专门用于推理的数据集。

Method: 收集多个城市的20,000个20秒剪辑片段，包含同步的多摄像头图像、LiDAR数据、高清地图、目标标注和人工验证的推理标注，涵盖空间推理、决策推理和反事实推理三种类型。实验对VLMs进行微调并将推理监督融入VLA训练。

Result: 在nuReasoning上微调VLMs显著提升了驾驶特定问题的回答能力；将推理监督融入VLA训练提高了规划性能，即使在推理时间禁用文本推理输出时也是如此。

Conclusion: nuReasoning为在现实长尾设置中评估和改进稳健、可解释的推理驱动自动驾驶系统奠定了基础。
...
### [95] [Joint Multi-Camera LiDAR Extrinsic Calibration via Learned Pairwise Initialization and Geometric Refinement](https://arxiv.org/abs/2605.31576)
*Aziz Al-Najjar,Marzieh Amini,James R. Green,Felix Kwamena*

Main category: cs.CV

TL;DR: 本文提出了一种结合学习匹配与几何优化的双阶段多相机LiDAR外参标定框架，通过CMRNext生成初始估计后，再利用多帧束调整进行全局一致性优化。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法各自独立处理每个相机-LiDAR对，忽略了多相机平台中的刚性几何耦合，导致系统层面的不一致性。

Method: 首先将CMRNext独立应用于每个相机获取初始外参估计和密集2D-3D对应关系，然后通过多帧束调整（包含重投影、先验和相对位姿约束项）进行联合细化。

Result: 在KITTI数据集上达到0.89cm平移误差和0.038°旋转误差；在Walkley数据集上将平移误差从108.6cm降至3.1cm。

Conclusion: 该方法显著提升了每相机精度和相机间一致性，尤其在单相机预测不可靠时效果更为明显。
...
TL;DR: 提出 SurGe 模型，用于改进前馈 3D 重建中的局部表面几何精度。


<details>
  <summary>Details</summary>
Motivation: 当前前馈 3D 重建方法虽然能预测点图并估计全局 3D 几何，但局部表面几何仍存在误差，这些误差在定性观察中明显，但在常见指标中反映较弱。

Method: 引入点图法线指标来评估局部表面方向；提出点梯度匹配损失监督深度归一化的 3D 有限差分；以及邻域注意力解码器(NAD)用于渐进上采样和局部特征混合。

Result: SurGe 在八个零样本单目几何基准测试中获得最佳平均排名，显著提升了局部点图和点图法线评估效果。

Conclusion: 该方法有效减少了局部表面几何误差，提升了 3D 重建的质量。
...
TL;DR: 本文提出了Gesture Recognition in the Wild (GRW)数据集，这是首个大规模将非约束性人类手势与特定词语进行映射的数据集，具有帧级精确的时间边界。该数据集包含156,688个手动标注的视频剪辑，涵盖150个高度多样化的物理动作、空间描述符和抽象概念词汇分类。


<details>
  <summary>Details</summary>
Motivation: 人类在说话时自然会做手势，但只有一小部分手势具有视觉描绘性并与特定口语词汇语义关联。现有的多模态模型难以捕捉这些语义共语手势，主要受限于缺乏精确标注的训练数据。

Method: 1. 构建GRW数据集：收集156,688个手动标注的视频剪辑，建立150词分类法
2. 利用GRW训练视频模型完成三个任务：(a)将手势分类为语义或非语义；(b)识别与共语手势对应的词语；(c)手势时间定位
3. 为这三个任务建立基准测试

Result: GRW是首个具备帧级精确时间边界的大规模手势识别数据集，成功训练模型完成了手势语义分类、手势词语识别和手势时间定位三项任务，并为这些任务建立了基准。

Conclusion: GRW数据集的提出填补了语义共语手势识别领域缺乏大规模精确标注数据的空白，为该领域的future研究提供了重要的数据基础和基准。
...
TL;DR: C4G是一种基于前馈的4D重建框架，使用时间戳条件化的可学习高斯查询令牌来重建动态场景，能够实现新视角合成、鲁棒的motion建模，并支持特征提升用于点追踪和动态场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有前馈方法对每帧逐像素预测3D高斯，存在重复高斯和视图相关偏差的问题，阻碍了场景motion的有效学习，且需要大量高斯和相机位姿。

Method: 提出C4G框架，包含：1）紧凑的timestamp-conditioned可学习Gaussian query tokens，每个token聚合时序上下文特征并解码位置受目标时间戳调制的3D Gaussian；2）视频扩散模型渲染增强模块；3）扩展到feature lifting生成4D特征场支持点追踪和场景理解。

Result: 使用显著更少的高斯实现强大的新视角合成性能，无需相机位姿，展现更强的motion建模能力，对大的时间间隙具有更强的鲁棒性。

Conclusion: C4G通过compact query tokens机制有效聚合特征，实现了全局相干的Motion建模，为动态场景重建提供了一种高效的前馈解决方案。
...
TL;DR: 提出一种基于KL散度的分布外检测方法(KLIP)，用于扩散模型先验的计算成像中，可检测整图和局部patch的分布偏移。


<details>
  <summary>Details</summary>
Motivation: 现有OOD检测方法需要了解偏移分布的先验知识、难以检测细微或局部分布_shift、且无法处理逆问题中的间接测量。

Method: 提出基于Kullback-Leibler散度的检测指标，衡量扩散先验与后验分布间的差异，无需标定数据或偏移分布知识。

Result: 能检测细微但语义显著的分布偏移（如健康肝脏CT到肿瘤CT的转变），并泛化到不同扩散模型、数据集和逆问题。

Conclusion: 该方法不需标定数据即可检测整图和局部patch的分布偏移，为扩散模型在计算成像中的OOD检测提供了新思路。
...
TL;DR: SOCO是一个新的语义目标对应基准测试，用于评估视觉基础模型对结构化对象的理解能力，包含100个类别和超过100万对应配对的详细关键点标注。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模型的细粒度部分级理解评估缺乏一致的系统性协议，之前的方法难以衡量模型在外观、视点和几何形状大幅变化下匹配目标部分的能力。

Method: 构建了包含关键点语言描述的SOCO基准，对100个类别的语义对应类型进行分类，提供一致的、具有功能意义的关键点注释，并进行了综合实验评估多种视觉基础模型和大型视觉语言模型。

Result: 视觉基础骨架编码强语义结构但跨类别转移对应能力弱；LVLMs在文本提示的部分定位上优于跨图像视觉对应匹配；对应性能比ImageNet分类更能预测分割、跟踪、3D姿态估计等下游任务性能。

Conclusion: SOCO为视觉和多模态基础模型提供了评估结构化部分级表示质量的系统性基准，揭示了当前模型在细粒度视觉对应与语言定位之间存在差距。
...
TL;DR: 提出StateKV方法，通过固定容量循环状态携带跨帧上下文，将预训练长视频VLMs适配到线性时间视频预fill，同时保留第二每帧缓存用于解码，在不微调或改变架构的情况下实现高效长视频理解。


<details>
  <summary>Details</summary>
Motivation: 现有视频编码器依赖时空自注意力，导致计算和延迟随帧数呈二次方增长；现有效率方法如帧/_TOKEN丢弃或粗粒度注意力近似会损害精度。

Method: 引入StateKV推理时方法：使用固定容量、基于重要性的循环状态携带跨帧上下文，配合第二完整每帧缓存用于解码，实现线性时间视频预fill。

Result: 在三个长视频基准测试和七个模型上，StateKV性能接近全自注意力，持续优于主流滑动窗/最近性流式近似方法；同时降低FLOPs，使更强模型能在固定算力下运行。

Conclusion: 为可扩展长视频理解提供实用步骤，无需微调或架构修改即可实现高效准确的长视频处理。
...
TL;DR: 本文提出Representation Forcing (RF)技术，让统一多模态模型(UMMs)在像素空间中原生预测视觉表示作为中间tokens，消除对外部VAE的依赖，实现端到端的图像理解和生成。


<details>
  <summary>Details</summary>
Motivation: 现有UMMs仍依赖冻结的、单独预训练的VAE进行图像生成，造成结构性瓶颈；直接移除VAE会导致质量下降，因模型需从原始像素同时学习高级结构和低级细节。

Method: Representation Forcing (RF)技术：强制解码器在自回归预测像素之前，先预测视觉表示作为中间tokens；这些表示tokens保留在上下文中指导后续像素扩散过程，使表示从感知输出转变为生成目标。

Result: 在图像生成任务上，带RF的像素空间模型达到与最先进的VAE基统一模型相当的质量；在图像理解任务上，像素空间RF通常优于其VAE基变体。

Conclusion: RF技术为实现端到端、无瓶颈的统一多模态模型提供了有效途径，同时提升了图像理解和生成性能。
...
### [103] [Social Reasoning in Machines: Investigating Collective Truth-Seeking Dynamics in Large Language Model Debate](https://arxiv.org/abs/2605.30391)
*Tom Pecher*

Main category: cs.MA

TL;DR: 本文首次使用多智能体辩论(MAD)模拟论证推理理论(ATR),通过构建认知多样性的LLM-agent群体,在问卷任务上显著提升真理性寻求表现,并提出基于LLM-MAD的新型基准测试方法。


<details>
  <summary>Details</summary>
Motivation: 传统认知科学认为推理是个体心智过程,但ATR理论主张真理是通过社会对抗性辩论产生的涌现性质。本文旨在用LLM验证ATR的核心假设,并探索集体推理是否普遍优于个体推理。

Method: 设计多智能体辩论系统,构建认知多样性的LLM-agent群体,在问卷任务上进行对抗性辩论,通过严格实证分析验证ATR机制的运作。

Result: LLM-MAD在问卷任务中显著提升真理性寻求表现,即便个体参与者单独表现有限;证据支持集体推理机制是性能增益的根源;该方法还可用于测量模型内在属性(如幻觉倾向)。

Conclusion: 研究结果支持ATR核心主张:集体推理普遍优于个体推理,而非生物进化偶然;为AI对齐和模型评估提供新基准框架。
...
### [104] [Design and Evaluation of Multi-Agent AI Oracle Systems for Prediction Market Resolution](https://arxiv.org/abs/2605.30802)
*Tarun Kota*

Main category: cs.MA

TL;DR: 研究多智能体LLM架构是否能提高预测市场预言机_resolution准确率。结果显示，置信度加权投票的独立聚合达到83.43%，优于最佳单模型1.01个百分点，但协商共识反而降至76%。提出混合AI-人类路由方案。


<details>
  <summary>Details</summary>
Motivation: 现有预言机系统在快速自动human仲裁之间存在权衡，单一LLM预言机继承所有失败模式且无自纠正机制。需要探索多智能体架构能否改进resolution准确率。

Method: 在KalshiBench的1189个已resolved预测市场问题上，比较独立聚合和协商共识与三种单模型基线(GPT-5 Nano、DeepSeek V3、Llama-3.3-70B)。所有智能体通过Exa共享通用证据层，按发布日期过滤检索以隔离推理质量。

Result: 独立聚合使用置信度加权投票达到最高准确率83.43%，优于最佳单模型1.01个百分点。协商共识准确率降至约76%，低于所有单模型基线，原因是辩论中错误传播导致坚定错误的模型翻转正确答案。模型间错误相关性(0.529-0.689)解释了聚合增益未达理论Condorcet上限。许多问题无法被任何多智能体架构纠正。

Conclusion: 提出混合AI-人类预言机路由标准：仅自动resolve unanimous高置信度问题，可在47%数据集上达到97.87%准确率，其余由人工审核。
...
### [105] [Safe Equilibrium Policy Optimization for Strategic Agent Policies](https://arxiv.org/abs/2605.30854)
*Karthika Arumugam,Kiran Kumar Manku,Amit Dhanda*

Main category: cs.MA

TL;DR: 提出安全均衡策略优化(SEPO)，在语言模型强化学习微调中引入对等性、串通风险和外部性的显式惩罚，避免多智能体战略环境中agent出现剥削、串通和成本外部化等失败模式。在5个战略域实验验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型通过强化学习微调时仅优化任务奖励，忽视了多智能体博弈中的战略结构。战略失败模式（剥削弱对手、在有害均衡上协调、将成本外部化）与自然语言接口本身密不可分，需要显式的安全约束。

Method: 提出安全均衡策略优化(SEPO)训练目标：将等性、串通风险和外部性成本作为显式惩罚项加入期望收益。采用GRPO作为奖励信号，在Gemma 4 E4B-it和Qwen 3.5-4B监督微调后应用。评估域包括迭代囚徒困境、重复拍卖、两种谈判变体和Kuhn扑克。

Result: SEPO在Kuhn扑克中达到零可剥削优势；在4个域上安全性能优于基模型；纠正了监督微调引入的过度合作行为。谈判中实现正向安全结果，且为唯一具有正向归一化相对优势的谈判配置。消融实验表明每-rollout的等性计算是必需的，共享常数惩罚会在GRPO优势归一化中抵消（常数控制变量属性），产生零梯度。

Conclusion: SEPO能有效抑制多智能体环境中的战略风险行为，在保持任务性能的同时提升安全性。该方法为构建更安全的多智能体语言模型系统提供了可行的训练范式。
...
### [106] [PhyDrawGen: Physically Grounded Diagram Generation from Natural Language](https://arxiv.org/abs/2605.30512)
*Nafiul Haque,Syed Nazmus Sakib,Shifat E Arman*

Main category: cs.AI

TL;DR: PhyDrawGen是一个神经符号 Pipeline，通过解耦语义场景理解与物理约束满足，从文本生成符合物理定律的图表。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型会产生力矢量幻觉、忽略守恒定律、违反几何约束，无法保证物理准确性。

Method: 三阶段流程：1)LLM提取类型化场景图；2)确定性求解器将图转换为PSLG，编码力平衡/光学路径/场拓扑；3)微调Qwen-VL通过提议-验证循环迭代纠正约束违规。

Result: 在1449个力学、光学、电磁学问题基准上显著优于GPT-5-image、Gemini 2.5 Flash和Gemini 3 Pro。

Conclusion: PhyDrawGen实现了鲁棒的物理准确性，即使在非常规物体问题上也能保持良好表现。
...
### [107] [Physically Viable World Models: A Case for Query-Conditioned Embodied AI](https://arxiv.org/abs/2605.30542)
*Adam J. Thorpe,Stepan Tretiakov,Cheng-Hsi Hsiao,Su Ann Low,Xingjian Li,Hassan Iqbal,Neel P. Bhatt,Ufuk Topcu,Krishna Kumar*

Main category: cs.AI

TL;DR: 本文指出当前具身AI的世界模型存在根本性缺陷：仅能预测观察结果而无法保证物理可行性。作者通过控制基准实验证明，外观相同但物理参数不同的系统可能产生完全错误的干预结果。论文提出构建模块化世界模型的新框架，包含环境表示、潜在状态估计、动作规格、介入动力学和查询响应等组件，由自主编排器根据具体查询动态组合兼容的组件。核心设计原则是：最简模型而非最详细模型，是回答特定干预查询所需的正确抽象。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI系统在物理可行性问题上面临严重挑战。观察预测型世界模型可以生成视觉上 plausible 但物理上错误的rollout。当前的模型在干预查询时可能推荐不可行的动作、误预测交互结果或认证不安全行为。这个问题本质上是结构性的：不同的物理系统可能看起来完全相同，但在干预下会发散。

Method: 作者通过设计控制基准实验来暴露这个问题：固定可见场景同时改变潜在物理参数，比较不同模型的的表现。提出模块化世界模型架构，包含五个核心组件：环境表示、潜在状态和参数估计、动作规格、介入动力学、查询级响应。由自主编排器识别相关抽象并按查询组合兼容的学习型和结构化组件。

Result: 作者证明现有的观察预测模型在回答干预查询时会失败，可能推荐不可行动作、误预测交互结果或认证不安全行为。通过模块化分解方案，模型变得可解释、可验证和可审计。正确的抽象不是对世界最详细的模型，而是保留与查询相关区分的最简模型。

Conclusion: 为具身AI研究提供了新设计原则和现有模型的可行性测试标准。文章展示了方法在现有系统无法正确回答的查询上的应用，并勾勒出编排器如何动态组装和调整物理可行的模型用于规划、控制和验证。指出了未来研究方向包括发展更系统的干预查询基准测试和更高效的模型组合策略。
...
### [108] [Transforming and Encoding FTS for SAT Solving: What Helps, What Hurts (Extended Version)](https://arxiv.org/abs/2605.30563)
*João Filipe,Álvaro Torralba,Gregor Behnke*

Main category: cs.AI

TL;DR: 本文研究如何将扩展的SAS+任务（因子任务）编码为SAT问题，提出了多种转换策略来分析并行性和任务变换的影响。


<details>
  <summary>Details</summary>
Motivation: 因子任务是比传统STRIPS或SAS+更紧凑的表示方式，但现有规划方法仅局限于启发式搜索，需要拓展SAT-based规划方法。

Method: 提出了多种将因子任务的转换关系翻译成命题逻辑的编码策略，并分析了不同层级的并行性利用方式。

Result: 研究了常见任务变换对SAT规划器性能的影响，为SAT-based规划器的实际应用提供了指导。

Conclusion: SAT编码方法为因子任务提供了一种新的规划求解途径，不同的编码策略和任务变换会显著影响规划器的效率。
...
### [109] [Procedural Generation of First Person Shooter Maps using Map-Elites](https://arxiv.org/abs/2605.30570)
*Simone de Donato,Pier Luca Lanzi,Daniele Loiacono*

Main category: cs.AI

TL;DR: 研究将MAP-Elites算法应用于第一人称射击游戏(FPS)关卡设计，提出了两种新的地图表示方法(Point-Line和Spatial-Layout)，并通过定义拓扑指标和涌现指标来评估地图质量。


<details>
  <summary>Details</summary>
Motivation: 之前的FPS游戏关卡演化方法存在多样性不足的问题，需要一种能够同时优化质量和多样性的方法。MAP-Elites作为质量多样性算法的优势在于可以生成大量不同但高质量的解决方案。

Method: 引入两种新型地图表示方法(Point-Line和Spatial-Layout)；定义了拓扑属性指标(仅依赖地图布局)和涌现属性指标(需要实际游戏测试)；使用带滑动边界的MAP-Elites(MESB)进行演化；对不同特征引导效果进行深入分析。

Result: 新提出的地图表示方法能够生成比以往方法更高多样性和质量的FPS地图，验证了这些表示方法在FPS地图演化中的有效性。

Conclusion: Point-Line和Spatial-Layout表示方法相比传统的All-Black和Grid-Graph表示方法更适合用于FPS地图的演化设计，为游戏关卡生成提供了更优的工具。
...
### [110] [Healthcare Mechanisms from Policy-as-Code Search under Strategic Provider Response](https://arxiv.org/abs/2605.30680)
...
### [111] [HADT: A Heterogeneous Multi-Agent Differential Transformer for Autonomous Earth Observation Satellite Cluster](https://arxiv.org/abs/2605.31023)
*Mohamad A. Hady,Muhammad Anwar Masum,Siyi Hu,Mahardhika Pratama,Jimmy Cao,Ryszard Kowalczyk*

Main category: cs.AI

TL;DR: 针对异构卫星集群（光学卫星和合成孔径雷达卫星）的地球观测任务，提出了一种基于Transformer架构的无模型强化学习方法，实现自主资源管理。该方法通过关系观测-动作标记化和差分注意力机制，使卫星能够实时决策，适应动态空间环境变化。


<details>
  <summary>Details</summary>
Motivation: 传统调度方法依赖数学模型和优化算法，但在动态不确定的空间任务环境中，这些模型往往不可用、不精确或过于复杂。需要一种无需精确模型的自适应实时资源管理方法。

Method: 将资源管理问题重新表述为序列决策过程，采用无模型强化学习技术。提出新型Transformer架构，包含关系观测-动作标记化方法和差分注意力机制。

Result: 相比现有基线方法取得显著性能提升，显示出对不同数量卫星集群的强大适应性和可迁移性。

Conclusion: 该Transformer架构能够有效解决异构卫星集群的自主地球观测任务资源管理问题，为未来智能卫星系统提供了新的技术路径。
...
### [112] [EHRBench: An Automated and Reliable EHR-based Benchmark for Clinical Decision Making with LLMs](https://arxiv.org/abs/2605.30637)
*Yuzhang Xie,Keqi Han,Yunpeng Xiao,Hejie Cui,Guanchen Wu,Ziyang Zhang,Kai Shu,Jiaying Lu,Xiao Hu,Carl Yang*

Main category: cs.AI

TL;DR: EHRBench是一个基于真实患者电子健康记录(EHR)构建的大规模临床决策基准测试，用于评估大语言模型在诊断、治疗和预后三大核心临床推理任务上的表现。该基准通过EHR-LLM-KB交互pipeline自动化生成约96万条QA项目。


<details>
  <summary>Details</summary>
Motivation: 临床决策(CDM)是真实临床工作流程的核心，但LLM在此类任务上的可靠性尚未得到充分理解和验证。现有评估基准缺乏规模化和可靠性的平衡，且需要基于真实患者EHR来更好地支持实际临床决策任务的评估。

Method: 采用EHR-LLM-KB(知识库)交互pipeline构建基准：1)使用专用LLM将就诊级EHR轨迹自动转换为结构化模板；2)确定性实例化模板生成QA项目；3)并行进行基于KB的系统性验证和富化，过滤幻觉或模糊关系。最终构建了覆盖诊断、治疗和预后三大推理要求临床决策任务的基准。

Result: 构建了包含960,067个QA项目的大规模基准，对30余个代表性LLM进行了详细性能分析。结果显示能力趋势在各设置下一致，验证了EHRBench的可靠性，并揭示了迈向临床可靠LLM系统的可行改进方向。

Conclusion: EHRBench为大语言模型的临床决策能力提供了规模化、可靠的评估框架，推动了LLM在临床实践中应用的可行性验证，为开发更加可靠的临床LLM系统奠定基础。
...
### [113] [Structure-Induced Information for Rerooting Levin Tree Search](https://arxiv.org/abs/2605.30664)
*Jake Tuero,Michael Buro,Laurent Orseau,Levi H. S. Lelis*

Main category: cs.AI

TL;DR: 本文提出基于学习的"重根器"(rerooter)来解决基于子目标策略搜索在高复杂度环境中开销大、可扩展性差的问题，通过√LTS算法实现了隐式问题分解。


<details>
  <summary>Details</summary>
Motivation: 基于子目标的策略树搜索虽对复杂确定性单智能体问题有效，但显式生成子目标会带来大量计算开销并阻碍可扩展性。

Method: 提出三种重根器设计：1)基于聚类的重根器利用全局状态空间结构；2)基于启发式的重根器利用学习到的成本估计；3)结合两种信号的混合型重根器。框架避免了显式重建和推理生成的子目标。

Result: 基于重根器的方法能够扩展到子目标策略树搜索失效的复杂环境中，并在测试领域取得最先进的在线训练效率。

Conclusion: 使用学习到的重根器代替显式子目标生成是解决子目标策略搜索可扩展性问题的有效途径，三种重根器设计中混合型表现最优。
...
### [114] [MAVEN: Improving Generalization in Agentic Tool Calling](https://arxiv.org/abs/2605.30738)
*Omkar Ghugarkar,Vishvesh Bhat,Muhammad Ahmed Mohsin,Asad Aali*

Main category: cs.AI

TL;DR: MAVEN是一个轻量级符号推理框架，用于结构化分解、自适应工具编排和中间验证，旨在提升Agent在跨领域工具调用环境中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在单个基准上表现良好，但在跨域组合推理策略、保存中间状态和协调工具方面能力不足，这是可靠Agent系统的核心挑战。

Method: 提出MAVEN框架，包含三部分：1)结构化分解模块；2)自适应工具编排模块；3)中间验证模块。在多个基准上评估，包括BFCL v3、TauBench、Tau2Bench、AceBench，并设计MAVEN-Bench压力测试基准。

Result: MAVEN将GPT-OSS-120b基础模型的准确率从48%提升至71%，无需额外训练。与前沿闭源基线模型性能相当，且成本比约为1/10。

Conclusion: 轻量级以验证为中心的推理框架可有效增强组合推理能力，为实际Agent系统的过程感知评估提供了新方向。
...
### [115] [Generating Graph-like Rules for Knowledge Graph Reasoning via Diffusion Models](https://arxiv.org/abs/2605.30747)
*Haoxiang Cheng,Yunfei Wang,Chao Chen,Kewei Cheng,Zhipeng Lin,Haoxuan Li,Changjun Fan,Shixuan Liu*

Main category: cs.AI

TL;DR: 本文提出GRiD框架，将图结构规则（graph-like rules）的发现重新定义为离散生成过程，解决了传统方法忽略图中环状和分支结构的问题，并通过两阶段训练（监督预训练+强化学习）克服了非可微规则质量指标的优化难题。


<details>
  <summary>Details</summary>
Motivation: 现有的知识图谱规则挖掘方法主要关注简单的链式规则，忽视了图中更丰富的拓扑结构（如循环和分支）；同时图结构规则的搜索空间组合爆炸造成严重计算瓶颈；此外扩散模型等生成方法的目标函数与高质量规则学习目标不对齐，且非可微的规则质量指标无法直接指导模型优化。

Method: 提出GRiD框架，包含两个阶段：1）监督预训练阶段从知识图谱元图中采样子图来捕获结构先验；2）强化学习阶段使用策略梯度优化直接根据非可微的规则质量指标对模型进行微调。

Result: 在六个基准数据集上的实验表明GRiD在知识图谱补全任务上达到竞争性性能；消融实验验证了GRiD的效率和鲁棒性，并显示图结构规则对链式规则具有互补作用。

Conclusion: GRiD成功将图结构规则发现转化为可学习的离散生成任务，通过两阶段训练策略有效解决了非可微指标的优化问题；实验证明图结构规则能够补充链式规则，提升知识图谱补全效果。
...
### [116] [PReMISE: Policy Rubrics as Measurement Specifications for LLM Judges](https://arxiv.org/abs/2605.30803)
...
TL;DR: PReMISE是一个框架，用于从人类偏好数据中发现评分规则并审计LLM-judge使用的评分规则，从四个维度进行评估：结构充分性、可靠性、偏好拟合和对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评判依赖于rubrics（评分标准），但模糊的rubrics（如"有帮助且实事求是"）可能奖励看似流畅但实际捏造事实或违背用户意图的回答。需要将rubrics视为可重用的测量规范，并系统性地审计其质量。

Method: 给定成对人类偏好数据，PReMISE框架能够：(i)发现政策级rubric集，(ii)在四个轴心上审计任意rubric集：结构充分性、可靠性、偏好拟合、对抗鲁棒性。还贡献了两个审计定向修复操作：偏好排序选择和可靠性约束细化。

Result: 实验发现：没有任何原始rubric源能同时满足可靠性、偏好预测性和对抗鲁棒性；高评分者间一致性并不等同于低可利用性；PReMISE是唯一在适用性、特异性和有效维度上同时得分的rubric来源。偏好排序选择将法官在成对响应上的准确率从65.0%提升到68.6%；可靠性约束细化将可利用响应获得高分的比例从46.4%降低到36.0%，而评分者间一致性几乎没有变化。

Conclusion: PReMISE框架通过系统性审计揭示了现有rubrics的关键局限性，并为改进LLM评估提供了有效的修复策略。未来应进一步探索更多维度的rubrics审计和自动修复方法。
...
### [117] [SLAT: Segment-Level Adaptive Trimming for Efficient CoT Reasoning](https://arxiv.org/abs/2605.30832)
*Jian Yao,Xiongcai Luo,Ran Cheng,Kay Chen Tan*

Main category: cs.AI

TL;DR: 本文提出SLAT（Segment-Level Adaptive Trimming）框架，通过在强化学习框架中引入基于细分段落的自适应修剪策略，在保持模型精度的同时将推理长度减少50%，建立优越的精度-效率Pareto前沿。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过Chain-of-Thought（CoT）能力显著提升，但其生成的推理链存在结构性冗余（过度思考），导致高计算开销且不提升答案正确性。现有缓解策略采用统一的token长度惩罚，粗粒度且无法区分冗余与有用的推理段落。

Method: 本文首先证明低效问题集中在高概率但边际效用低的段落中。基于正确性-长度权衡目标，本文推导了段落次优性的理论表征，并提出SLAT框架，该框架根据此准则选择性地抑制冗余段落。

Result: 在标准基准测试上的实证结果表明，SLAT建立了优越的精度-效率Pareto前沿，相比未压缩基线可将推理长度减少50%，同时保持竞争力精度。

Conclusion: 本文研究表明，基于理论驱动、段落感知的修剪策略是大型语言模型实现高效CoT推理的有前景方向。
...
### [118] [Distilling LLM Feedback for Lean Theorem Proving](https://arxiv.org/abs/2605.30861)
*Gaetan Narozniak,Gérard Biau,Rémi Munos,Ahmad Rammal,Pierre Marion*

Main category: cs.AI

TL;DR: 提出Feedback Distillation方法,通过让模型在token级别匹配自身基于特权反馈的条件分布,来解决推理模型后训练中GRPO算法面临的稀疏奖励、探索有限和模式崩溃问题。该方法可用于Lean4定理证明,并可与GRPO结合使用。


<details>
  <summary>Details</summary>
Motivation: 现有推理模型的后训练主要采用监督微调和来自可验证奖励的强化学习(常用GRPO),但该算法存在稀疏奖励、探索有限和模式崩溃等缺陷,需要新的训练方法来改进。

Method: 提出Feedback Distillation训练方法:让模型在token级别匹配自身基于语言模型生成的特权反馈的条件分布,从而获得token级别的监督信号并可注入外部知识。对Lean4定理证明任务进行评估,并探索将GRPO从Feedback Distillation检查点初始化的组合方法。

Result: Feedback Distillation相比GRPO能保持更高的生成轨迹多样性,具有更高的策略熵和更好的pass@k扩展性。两种方法互补:从Feedback Distillation检查点初始化GRPO优于单独使用任一方法。

Conclusion: 结果表明Feedback Distillation是改进复杂推理模型后训练的有前途途径,可有效解决GRPO的局限性,为推理模型的训练提供了新的思路。
...
### [119] [UniScale: Adaptive Unified Inference Scaling via Online Joint Optimization of Model Routing and Test-Time Scaling](https://arxiv.org/abs/2605.30898)
*Kaiyu Huang,Xingyu Wang,Mingze Kong,Zhubo Shi,Yuqian Hou,Hong Xu,Zhongxiang Dai,Minchen Yu,Qingjiang Shi*

Main category: cs.AI

TL;DR: 本文提出统一推理扩展（UIS）框架，解决大型语言模型部署中推理质量与计算成本之间的平衡问题。现有方法将模型路由和测试时扩展作为两个独立维度处理，导致粒度粗糙、适应性受限。UIS将两者统一在单一优化空间中，结合上下文多臂bandit和LinUCB算法实现自适应推理策略。


<details>
  <summary>Details</summary>
Motivation: 在大规模语言模型实际部署中，准确率和计算成本之间的权衡是核心挑战。现有方法沿两条独立路径：模型路由（在不同规模的模型间切换）和测试时扩展（TTS，在固定模型内调整推理计算）。然而这种解耦设计存在固有局限：模型路由产生粗粒度离散的性能变化；单独使用TTS常遇到能力上限且收益递减。此外，将两种机制分离限制了动态推理环境中的适应性。

Method: 提出UniScale在线框架，将自适应UIS建模为上下文多臂 bandit问题，通过LinUCB算法学习推理策略。框架整合效率感知学习和成本建模，确保在高维动作空间上的稳定可扩展优化。该方法统一了模型路由和测试时扩展两个维度，形成单一的优化空间。

Result: 评估表明，UniScale能有效利用UIS空间的协同作用，在多样化的动态推理场景中提供细粒度且始终更好的质量-成本权衡。相比解耦的模型路由和TTS方法，统一方案具有更精细的控制粒度和更强的环境适应能力。

Conclusion: 统一模型路由与测试时扩展是提升LLM推理效率和性能的有效路径。UiScale框架通过将两者融合并采用上下文bandit学习方法，为动态推理环境中的资源优化提供了新的研究方向，具有重要的 practical 应用价值。
...
### [120] [BilliardPhys-Bench: Benchmarking Physical Reasoning and Visual Dynamics of Multimodal LLMs](https://arxiv.org/abs/2605.30900)
*Ben Wang,Xiaogang Li,Ruochen Gao,Peiyao Xiao,Chengliang Xu,Zeyu Wang,Zichao Chen,Bing Zhao,Hu Wei*

Main category: cs.AI

TL;DR: BilliardPhys-Bench是一个用于评估多模态模型物理推理能力的合成台球环境基准,揭示了当前模型在预测物体运动和交互方面的系统性问题,并指出需要改进物理归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型(MLLMs)在静态图像识别方面表现良好,但对物理推理(特别是从单张图像预测物体运动和交互)存在困难。现有模型缺乏对直觉物理推理的能力,需要基准测试来评估和推动这一领域的发展。

Method: 研究者构建了BilliardPhys-Bench基准,使用程序化引擎生成带有摩擦和弹性碰撞的随机台球场景。该基准测试三种能力:(1)预测球与球之间的碰撞;(2)推理墙壁反弹;(3)估算运动停止后的最终球位置。评估了GPT、Claude、Gemini和Qwen家族的最新MLLMs。

Result: 性能随模拟时间增加和场景几何复杂度提高而下降。研究还发现了一个一致的失败模式——"静止偏置"(stasis bias):当正确的物理结果更难推断时,模型倾向于预测无交互。

Conclusion: 这些发现揭示了当前MLLMs在视觉动力学方面的局限性,表明需要在多模态架构中引入更好的物理归纳偏置,以提升物理推理能力。
...
### [121] [A Persona-Based Evaluation Framework for Pluralistic Alignment in Generative AI](https://arxiv.org/abs/2605.31021)
*Atahan Karagoz*

Main category: cs.AI

TL;DR: 论文提出用多个代表不同人类视角的"评估人格"代替单一基准测试，解决传统AI对齐评估中忽视文化、人口和上下文多样性的问题，但发现这些模拟评估者在顺序推理中会退化。


<details>
  <summary>Details</summary>
Motivation: 现有生成式AI的对齐范式主要依赖单一的聚合统计基准，将多元人类判断简化为单一指标，掩盖了文化、人口和情境差异，评估缺乏真实世界的多样性反映。

Method: 构建状态空间约束的仿真框架，用结构化流形上的合成认知profile（多个评估人格）替代单一评估函数，让生成模型实例化和维持这些多元评估人格。

Result: 现代生成架构能以高一致性实例化并维持这些评估人格，实现多元视角的基准测试。但分析发现在顺序推理和随机提示扰动下会出现系统性退化：状态空间漂移和语义不一致，表明静态对齐约束不足以维持长期稳健行为。

Conclusion: 需要嵌入动态的、可行性驱动的监管机制来保持连贯的认知仿真。将基于人格的评估定义为潜在表示流形上的结构化动态系统，为更适应性强、更符合人类价值观和上下文敏感的AI评估方法奠定基础。
...
### [122] [GraphARC: A Comprehensive Benchmark for Graph-Based Abstract Reasoning](https://arxiv.org/abs/2605.31031)
*Saku Peltonen,August Bøgh Rønberg,Andreas Plesner,Roger Wattenhofer*

Main category: cs.AI

TL;DR: GraphARC是一个针对图结构数据的抽象推理基准数据集，它将few-shot transformation learning范式从基于网格的ARC扩展到图结构领域。每个任务需要从少量输入-输出对中推断转换规则并应用于新的测试图，涵盖局部、全局和分层图转换。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准通常局限于网格或文本格式，缺乏针对图结构数据的抽象推理评估基准。GraphARC旨在填补这一空白，提供一个可跨不同图族和规模生成的系统性评估平台，以测试模型的泛化能力。

Method: 提出GraphARC基准数据集，将ARC的few-shot learning范式推广到图数据。每个任务包含输入-输出示例对，模型需推断转换规则并应用于新的测试图。评估了当前主流语言模型在该基准上的表现。

Result: 模型能够回答关于图属性的问题，但往往无法完成完整的图转换任务，揭示了"理解-执行"差距。性能在更大规模的实例上进一步下降，暴露了规模化壁垒。

Conclusion: GraphARC为未来图基础模型提供了有前景的测试平台，融合了节点分类、链路预测和图生成于单一框架中。当前语言模型存在明显局限性，需要进一步改进。
...
### [123] [Vector Linking via Cross-Model Local Isometric Consistency](https://arxiv.org/abs/2605.31100)
*Ziying Chen,Yang Cao,He Sun,Beining Yang,Tianjian Yang*

Main category: cs.AI

TL;DR: 研究向量链接问题：从不同黑盒编码器生成的嵌入空间中，恢复跨模型对象对应关系。核心发现是对比学习编码器具有局部几何一致性（短距离近似保持），提出基于Beta-Bernoulli后验的迭代锚点引导哈希方法。


<details>
  <summary>Details</summary>
Motivation: 实际应用中，不同模型产生的嵌入向量可能表示同一对象，但无法直接比较。需要建立跨模型的匹配链接，且传统方法依赖监督数据或显式特征，泛化受限。

Method: 利用对比学习编码器的局部几何一致性特性，提出迭代参考几何嵌入哈希：(1)用配对锚点的距离表示向量；(2)通过哈希空间匹配候选链接；(3)使用Beta-Bernoulli后验聚合多视图证据；(4)自举高置信度链接作为新锚点。

Result: 在多个基准数据集和嵌入模型对上实现准确鲁棒的链接，性能在不同重叠程度、种子预算和外域锚点下均表现良好。

Conclusion: 该方法为向量数据库集成和跨模型聚类提供了有效解决方案，具有实际应用价值。
...
### [124] [LLM-FACETS: A Privacy-Preserving Framework for Evaluating LLM Transparency and Accountability](https://arxiv.org/abs/2605.31167)
*Tom Lucas,Alessio Buscemi,Alfredo Capozucca,German Castignani,Barbara Delacroix*

Main category: cs.AI

TL;DR: LLM-FACETS是一个开源框架，旨在为非技术背景的从业者提供评估大型语言模型输出真实性、校准性和可重复性的工具。该系统通过浏览器界面和插件架构，支持三种用户配置文件（技术专家、领域专家、合规官员），并整合了确定性指标、LLM判断指标和RAG三项指标来检测幻觉。


<details>
  <summary>Details</summary>
Motivation: 当前LLM审计工具对非技术从业者存在较高门槛：现有工具需要编程经验和非平凡的环境设置，云托管平台会将评估数据传输到外部服务，这为负责任AI部署带来了障碍。作者旨在降低审计门槛，使领域专家和合规官员能够独立进行AI Oversight。

Method: 该框架采用三大机制实现透明化：1)token级对数概率可视化用于认知不确定性；2)多评判器共识以减轻评判偏差；3)RAG三项指标（忠诚度、答案相关性、上下文相关性）用于检测和定位幻觉。系统包含插件架构，允许集成新的指标或数据集。

Result: 作者通过对18个指标实现与标准参考库的交叉验证，验证了框架的可行性。确定性指标完全在自托管服务器上运行，无外向传输；LLM评判指标明确联系外部API，用户保留完整凭证控制权。

Conclusion: LLM-FACETS通过将数据流明确化、支持跨指标交叉检查，使AI问责与系统构建团队脱钩，为符合EU AI Act和NIST AI风险管理框架要求的负责任AI部署提供了实践路径。
...
### [125] [Formalizing and falsifying causal pathways of rare events](https://arxiv.org/abs/2605.31254)
*Anahita Haghighat,Dominik Janzing*

Main category: cs.AI

TL;DR: 本文在结构方程模型中罕见事件根因分析的形式化基础上，提出了因果路径的形式定义及其可检验含义，并确定了仅依赖因果抽象而不依赖完整因果图的条件。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于建立一座桥梁，连接简单的口头因果解释和详细的因果建模，使得对罕见事件的因果分析更加形式化和可检验。

Method: 通过形式化定义因果路径，识别可检验含义的条件，这些条件依赖于罕见事件路径的因果抽象而非底层系统的完整因果图。

Result: 确定了可以仅通过因果抽象检验因果路径含义的条件，从而实现从复杂因果图到简单路径抽象的简化分析。

Conclusion: 成功建立了因果结构在罕见事件路径上的抽象，为口头解释和详细建模之间提供了形式化的中间层。
...
### [126] [COLLEAGUE.SKILL: Automated AI Skill Generation via Expert Knowledge Distillation](https://arxiv.org/abs/2605.31264)
*Tianyi Zhou,Dongrui Liu,Leitao Yuan,Jing Shao,Xia Hu*

Main category: cs.AI

TL;DR: 本文提出了COLLEAGUE.SKILL系统，一个自动化的从人类专家行为轨迹中提取并生成可移植、可纠正的AI技能包的端到端工作流程。该系统采用双轨设计：能力轨（实践、心智模型、决策启发式）和边界行为轨（沟通风格、交互规则、纠正历史），支持版本管理、自然语言反馈、回滚和跨智能体部署。


<details>
  <summary>Details</summary>
Motivation: 构建具备人类专业知识、判断力和交互风格的类人智能体仍面临挑战，因为与特定人物或角色相关的可行知识通常嵌入在异构行为轨迹中，而非以清晰指令形式存在。现有记忆和人格系统仅捕获碎片，技能框架提供便携封装格式，但缺少将轨迹转化为可检查、可纠正、且智能体可用的技能的端到端工作流程。

Method: COLLEAGUE.SKILL系统包含四大核心组件：工件契约定义技能包结构、生成工作流自动化提取知识、纠正生命周期支持持续改进、部署面支持跨智能体适配。系统生成包含两个协调轨道的版本化技能包：能力轨道涵盖实践、心智模型和决策启发式；边界行为轨道涵盖沟通风格、交互规则和纠正历史。通过自然语言反馈进行更新，支持版本回滚，可安装至不同智能体宿主并支持受控分发。

Result: 该开源系统目前拥有约18.5k GitHub星标，技能库收录了165位贡献者的215个技能卡，累计星标超过10万。该系统证明了类人技能可以被表示为可移植、可纠正的软件包，而非不透明的提示词或隐藏的记忆。

Conclusion: 通过将专业知识编码为结构化的双轨技能包，COLLEAGUE.SKILL实现了从专家行为轨迹到可部署AI技能的端到端转化。这一范式使得类人智能体能以透明、可审计的方式继承人类专业知识，支持持续迭代和跨平台部署，为构建更可信、更可控的AI智能体提供了新思路。
...
### [127] [Industrializing Prediction-Powered Inference: The GLIDE Library for Reliable GenAI and Agentic Systems Evaluation](https://arxiv.org/abs/2605.31278)
*Grégoire Martinon,Ibrahim Merad,Mohammed Raki*

Main category: cs.AI

TL;DR: GLIDE是一个统一预测驱动推理(PPI)方法的开源Python库,用于解决智能体系统评估中的人类标注成本高和LLM-as-judge偏差问题。


<details>
  <summary>Details</summary>
Motivation: 可靠的智能体系统评估需要无偏估计和有效不确定度,但标准实践在昂贵的人工标注和有偏的LLM判断之间权衡。现有PPI方法分散在不同论文中,缺乏统一实现。

Method: GLIDE库统一了多种先进的PPI估计器(PPI++、分层PPI、预测后去偏及其分层变体、主动统计推断)和采样器(均匀、分层、主动、成本最优),提供scipy风格API专注于均值估计。包含可复现的蒙特卡洛验证套件和方法选择的决策树,并通过智能体评估案例研究验证效果。

Result: 案例研究表明,该库能在保持同等精度下显著节省标注成本。

Conclusion: GLIDE为智能体系统评估提供了统一的PPI工具框架,兼具效率和精度的平衡,可从https://github.com/EmertonData/glide获取。
...
### [128] [TraceGraph: Shared Decision Landscapes for Diagnosing and Improving Agent Trajectories](https://arxiv.org/abs/2605.31308)
*Junjie Nian,Kang Chen,Ge Zhang,Yixin Cao,Yugang Jiang*

Main category: cs.AI

TL;DR: TraceGraph是一种基于图的框架，可将多模型智能体轨迹转化为共享决策景观，帮助分析和改进智能体基准测试的评估方式。


<details>
  <summary>Details</summary>
Motivation: 现有智能体评估通常简化为通过率或奖励分数，忽略了丰富的交互轨迹信息，无法揭示模型在任务中的具体导航差异和失败模式。

Method: 1. 为每个任务构建图谱，连接池化轨迹的行动-观察状态；2. 叠加结果知情的生产核心区和陷阱区域；3. 用Access、Trap exposure、Repair三个事件总结轨迹；4. 在SWE-bench上实现运行时陷阱检测器+轻量级延续策略的恢复管道。

Result: TraceGraph揭示了aggregate scores隐藏的导航差异，发现不同splits在奖励避免陷阱或从陷阱恢复上有分歧；在SWE-bench上，官方resolved rate从40.4%提升至43.5%（per-provider），从41.0%提升至44.8%（common-fired）。

Conclusion: TraceGraph提供了过程词汇，用于询问智能体基准测试的测试内容、模型在共享景观上的分歧位置，以及如何利用失败区域指导下游改进。
...
### [129] [Diagnosing Failure Modes of Shared-State Collaboration in Resource-Constrained Visual Agents](https://arxiv.org/abs/2605.31354)
*Yunpeng Zhou*
...
### [130] [HypoAgent: An Agentic Framework for Interactive Abductive Hypothesis Generation over Knowledge Graphs](https://arxiv.org/abs/2605.31370)
*Yisen Gao,Yixi Cai,Tianshi Zheng,Jiaxin Bai,Yangqiu Song*

Main category: cs.AI

TL;DR: HypoAgent是一个用于知识图谱上交互式归纳假设生成的智能体框架。


<details>
  <summary>Details</summary>
Motivation: 现有可控假设生成方法在交互设置中存在局限性：难以在多轮对话中理解演化的自然语言意图，且在生成假设失败时缺乏细粒度诊断。

Method: 提出HypoAgent框架，包含三个智能体：意图识别智能体（将用户话语和对话历史转化为可执行的KG条件）、假设生成智能体（根据提取的用户意图执行可控假设生成）、根因分析智能体（诊断不可靠的假设片段并利用KG邻域探测来识别支持的改进）。

Result: 在常识和生物医学领域特定知识图谱上的实验表明，HypoAgent在单轮、多轮和无条件设置下实现了最先进的语义相似度。

Conclusion: 该研究为知识图谱上的交互式假设生成提供了一种新的智能体框架，并通过实验验证了其有效性。
...
### [131] [FAM-Bench: A Multimodal Benchmark for Condition-Aware Food-as-Medicine Reasoning](https://arxiv.org/abs/2605.31410)
*Mingyang Mao,Bhargav Rishi Medisetti,Utkarsh Grover,Tanvir Ibrahim,Wenyan Li,Tingting Zhang,Xiaomin Lin*

Main category: cs.AI

TL;DR: FAM-Bench是一个用于评估食品-作为-药物( Food-as-Medicine)AI系统的多模态基准测试,填补了现有食品AI基准在健康感知决策层面的空白。该基准包含2500个经营养专家验证的样本,涵盖13种饮食相关健康 condition,并包含两个任务:菜肴适宜性评估和比较性菜肴分析。


<details>
  <summary>Details</summary>
Motivation: 现有食品AI基准主要评估菜肴识别、食谱理解、营养素估计或一般营养问答,但缺少对「特定健康状况下食物选择是否合适」这一健康感知决策能力的测试。Food-as-Medicine要求模型能够推理beyond菜肴本身和营养成分,做出符合具体健康条件的饮食决策。

Method: 构建包含2500个样本的多模态基准数据集,涵盖13种饮食相关健康condition。设计两个互补任务:(1)菜肴级别适宜性评估——根据图像和食材清单判断菜肴是否适合特定健康状况;(2)比较性菜肴分析——对四种候选菜肴按condition-specific适宜性排序。两个任务都需要整合食材证据、视觉准备线索和临床营养约束。

Result: 提供了一个标准化的测试平台,用于评估语言模型和视觉-语言模型在食品-作为-药物领域的健康感知推理能力。基准包含13种饮食相关健康condition,为Grounded health-aware reasoning提供了标准化的评估框架。

Conclusion: FAM-Bench填补了食品AI领域的重要空白,为评估模型的健康感知决策能力提供了全面、标准化的基准,有助于推动Food-as-Medicine AI系统的发展。
...
### [132] [Answer-Set-Programming-based Abstractions for Reinforcement Learning](https://arxiv.org/abs/2605.31444)
*Rafael Bankosegger,Thomas Eiter,Johannes Oetsch*

Main category: cs.AI

TL;DR: 本文提出将Answer-Set Programming (ASP)用于实现关系强化学习中的CARCASS抽象框架，以解决大规模状态空间的强化学习问题。


<details>
  <summary>Details</summary>
Motivation: 现实问题涉及巨大的状态空间，使强化学习的泛化和学习面临挑战。需要有效的抽象和近似方法来处理这些复杂域。

Method: 采用完全声明式的ASP建模语言重新实现原来的Prolog版本CARCASS框架，并在Blocks World和Minigrid两个领域进行案例研究评估。

Result: ASP实现的CARCASS能够有效构建强化学习抽象，尤其在领域知识可用的情况下表现出良好的性能。

Conclusion: CARCASS与ASP结合为强化学习构建抽象提供了一种有前景的方法，特别适用于具有领域知识的场景。
...
### [133] [AutoSci: A Memory-Centric Agentic System for the Full Scientific Research Lifecycle](https://arxiv.org/abs/2605.31468)
...
TL;DR: AutoSci是一个以记忆为中心的智能体系统，旨在实现科学研究全生命周期的自动化。该系统整合了四个核心模块：SciMem（ schema管理的科研记忆）、SciFlow（五阶段研究流程执行器）、SciDAG（ DAG结构多智能体操作器）和SciEvolve（反馈驱动的自我进化机制），能够跨项目保持结构化持久记忆并持续改进研究能力。


<details>
  <summary>Details</summary>
Motivation: 传统科学研究长期依赖人工密集型操作，研究人员需要在漫长的项目周期中协调文献、想法、实验、手稿和审稿意见。虽然LLM驱动的科学智能体出现了自动化这一过程的机会，但现有系统要么部分满足需求，要么完全无法满足，无法支持完整的研究生命周期、缺乏跨项目的结构化持久记忆，且无法随时间改进自身的研究流程。因此需要一个统一的自动化科学研究系统来填补这一空白。

Method: AutoSci围绕四个模块构建：1) SciMem提供schema管理的科研记忆，将长期知识记忆（可重用的科学知识）与主动研究记忆（项目级产物如想法、实验、手稿、审稿意见）分离；2) SciFlow通过控制状态、上下文、验证、反馈和编排的五阶段生命周期执行器，从文献理解到反驳的完整流程；3) SciDAG使用DAG形状的多智能体操作器和可重用的阶段特定模板增强困难技能；4) SciEvolve将用户、实验、审稿和外部环境的反馈信号转化为SciMem组织、SciFlow技能和SciDAG模板的版本化更新。

Result: AutoSci成为一个持久的科研环境，能够跨研究项目执行任务、记住科研进展、并持续进化。代码仓库已发布于https://github.com/skyllwt/AutoSci。

Conclusion: AutoSci提出了一种统一的以记忆为中心的智能体架构，成功整合了科学研究全生命周期所需的记忆管理、流程执行、技能增强和自我进化能力，为自动化科学研究系统提供了一个可行的解决方案。
...
### [134] [LinTree: Improving LLM Reasoning with Explicitly Structured Search Histories](https://arxiv.org/abs/2605.31492)
*Liwei Kang,Yee Whye Teh,Wee Sun Lee*

Main category: cs.AI

TL;DR: 本文研究大语言模型在推理任务中的搜索策略，提出将搜索历史表示为线性化搜索树(LinTree)，通过添加显式父指针来改进LLM的推理性能。


<details>
  <summary>Details</summary>
Motivation: 传统启发式搜索仅观察当前局部状态，而LLM可以访问完整搜索轨迹。研究者想验证LLM是否能利用这一优势，以及为什么现有方法效果不佳。

Method: 在三个受控推理环境(Blocks World、grid Navigation、Sokoban)中比较两种策略：1)跟踪条件推理策略(访问完整搜索历史) 2)最佳优先搜索+LLM启发式(仅观察当前状态)

Result: 发现仅有搜索历史不足以可靠超越启发式搜索。原因是隐式搜索树中回溯或切换分支时没有明确标识返回的前序状态。添加显式父指针后，LinTree方法在任务性能和搜索效率上均优于隐式推理和LLM启发式搜索。

Conclusion: 搜索历史在树结构被显式表示时才最有用，这启示未来应开发更具结构感知能力的LLM推理表示方法。
...
### [135] [Untitled](https://arxiv.org/abs/2605.31581)
**
